...
| Locales                   | `/locales`           | `LocalesAdaptor`          | **`IPSContentDesignWs`** find/load/create/save/delete            | Yes — `IPSContentDesignWs` locale methods                | **OK**     | Catalog GET + CD-18 write (#3959). Format-profile read still `IPSCmsObjectMgr` (no design-WS twin). Auto-translation remains a later slice.                                   |
