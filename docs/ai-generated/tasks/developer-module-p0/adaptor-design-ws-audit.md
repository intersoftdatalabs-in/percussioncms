...
| Locales                   | `/locales`           | `LocalesAdaptor`          | **`IPSContentDesignWs`** find/load/create/save/delete            | Yes — `IPSContentDesignWs` locale methods                | **OK**     | Catalog GET + CD-18 write (#3959). Format-profile read still `IPSCmsObjectMgr` (no design-WS twin).                                                                                                                                           |
| Auto-translation set      | `/locales/auto-translations` | `AutoTranslationsAdaptor` | **`IPSContentDesignWs`** `loadTranslationSettings` / `saveTranslationSettings` | Yes — same design WS SOAP uses                 | **OK**     | CD-18 GET/PUT (#3981). Held lock released on save. Empty list clears. Unknown locale/content type 400.                                                                        |
