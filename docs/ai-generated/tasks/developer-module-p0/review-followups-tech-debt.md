...
| UI-STYLE-01 | Shared **catalog/theme tokens** (`catalogStyles.ts` or CSS module) for mono/muted/padding    | Partial adoption; many panels still inline hex colors                     | `developer/catalogStyles.ts` + consumers     | Done — PR for #2086 (`catalogColors` + tableHeaderRow/tableRow; panels adopt tokens)                                     |
