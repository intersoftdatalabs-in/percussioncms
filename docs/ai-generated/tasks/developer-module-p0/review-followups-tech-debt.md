...
| REST-GAPS-01 | **`designGaps` as structured `{ code, message }`** (i18n / docs links / SPA grouping)                                                          | Free-text strings enough for P0 honesty; structured form needed for product UX                                    | `ContentTypeDetail`, `TemplateDetail`, `SlotDetail`, pipelines if any | Done - #2903 (CT/Template/Slot detail + SPA `formatDesignGap` fallback; other peers still string[])                             |
| REST-GAPS-02 | Avoid **duplicating identical designGaps arrays** on every detail row if payload size becomes a concern                                        | Currently intentional so SPA needs one call; constant list is fine                                                | Detail DTOs or `X-Design-Gaps` / static SPA list                      | Done - #2921 (list omits catalog-level gaps; detail keeps them; SPA panel fallbacks)                                             |
...
| UI-SRC-02   | Long binding **expression** preview ("show more") beyond maxWidth clamp                      | Clamp shipped; expand UX later                                            | `TemplateDetailPanel`                        | Done - [#2901](https://github.com/intersoftdatalabs-in/percussioncms/issues/2901) (`bindingExpressionPreview` + Show more/less; Vitest + Playwright) |
