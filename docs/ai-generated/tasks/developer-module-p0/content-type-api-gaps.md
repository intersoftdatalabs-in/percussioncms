...
| Public REST shared-field controls | CD-15 control/choice write (Admin)  | `GET`/`PUT …/{idOrName}/fields/{fieldName}/controlProperties` |
...
| Shared field file **write**                          | CD-15        | **Group create/save/delete shipped** (`POST /services/sharedfields`, `PUT …/{idOrName}`, `DELETE …/{idOrName}`, #3944). **Field create/delete shipped** (`POST …/fields`, `DELETE …/fields/{fieldName}`, persistable `PSField` + display mapping, Admin 403, lock 409, #3954). **Control/choice write shipped** (`GET`/`PUT …/fields/{fieldName}/controlProperties`, empty `properties` clears, omitted `choices` unchanged, request lock released on save, #3984). SPA editor still open |
...
| Import/export CT                                     | CD-14        | **REST GET export shipped** (`GET /services/contenttypes/{idOrName}/export`, Admin 403, unknown 404, no lock steal, #4018). **Import shipped** (`POST /services/contenttypes/import`, Admin 403, invalid XML 400, duplicate name 409, create-only, no lock steal, #4021). SPA import/export wizard still open |
...
5. ~~Control property value/choice catalogs~~ **Done CD-07 REST** (#3786). ~~Choice filter / null-entry / default-selected write~~ **Done** (#3995). Field-rule write/save still open. SPA Choices tab still later
6. ~~Shared field catalog read~~ **Done CD-15 read** (`GET /services/sharedfields`, Admin 403). ~~Write~~ **Done CD-15 group create/save/delete** (`POST`/`PUT`/`DELETE`, #3944). ~~Field create/delete~~ **Done** (`POST …/fields`, `DELETE …/fields/{fieldName}`, #3954). ~~Control/choice write~~ **Done** (`GET`/`PUT …/fields/{fieldName}/controlProperties`, #3984). SPA editor still open
