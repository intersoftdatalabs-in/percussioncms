...
| **Product packages still ship Widget XML** | Partial (ship-exit in progress) | Batch B (#2884) no longer commits install Widget XML (materialized at package-build); batch A/C + `perc.Test` still dual-ship committed XML |
...
| **Ship-exit install XML (batch B #2884)** | **Committed install Widget XML removed (−20)** | Product inventory **48 → 28**; `PSWidgetXmlInstallEmitter` materializes install XML at package-build |
