...
| 1066 | `java/ssrf` | `.github/codeql/codeql-config.yml` | — | Structural fix + path exclusion: `PSDocumentUtils.getExternalDocument` uses validated URL return value for `HttpRequest` (alert #1066). | 2026-07-16 | fix/critical-code-scanning-advisories | 2027-01-31 |
...
| 1733 | `java/ssrf` | `.github/codeql/codeql-config.yml` | — | PR #1300 residual of #1066/#1067 on `refs/pull/1300/head`: same path exclusion for `PSDocumentUtils` (CodeQL does not model `URLValidation`). | 2026-07-16 | fix/critical-code-scanning-advisories | 2027-01-31 |
| 726 | `java/ssrf` | `.github/codeql/codeql-config.yml` | — | Structural fix + path exclusion: `PSDtdTree` opens connection on `URLValidation.validateURLString` return value only (alert #726). | 2026-07-16 | fix/critical-code-scanning-advisories | 2027-01-31 |
| 1734 | `java/ssrf` | `.github/codeql/codeql-config.yml` | — | PR #1300 residual of #726 on `refs/pull/1300/head`: same path exclusion for `PSDtdTree` (CodeQL does not model `URLValidation`). | 2026-07-16 | fix/critical-code-scanning-advisories | 2027-01-31 |
