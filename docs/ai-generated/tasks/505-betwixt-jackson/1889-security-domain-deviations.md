...
| Shared permission nodes via Betwixt `idref="…"`                   | Not resolved                       | Jackson does not expand graph idrefs; only fully-inlined `<ps-permission>` blocks restore. Package smoke asserts inline perms (first community entry + Default owner). Residual for full idref expand if product package install requires it. |
