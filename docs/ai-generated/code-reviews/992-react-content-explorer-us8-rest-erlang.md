...
| VII (format checks)                   | ✅          | No new Java files outside `rest/src/main/java/com/percussion/rest/relationsummary/` + `rest/src/test/java/com/percussion/rest/relationsummary/`; compile + test pass on JDK 21 with `./mvnw -pl rest test` (the integrity plugin skipped via `-Dskip.ai.integrity=true`).                           |
