...
4. **T079 / T080 spot-checks** — Verified `scripts/README.md` FR-014 in-scope/out-of-scope section (already complete from T031) and confirmed no in-scope `.sh/.bat` references slipped through into root `AGENTS.md` (only `mvnw / mvnw.cmd` remains, which is exempt per Clarification Q2).
...
| SC-004 | ✅                      | `mvnw` (head: `JAVA_HOME_21` + `set -e`) and `mvnw.cmd` (`@echo off` + `JAVA_HOME_21`) unchanged vs. pre-spec baseline.                                                                                                                                                                                                                                        |
