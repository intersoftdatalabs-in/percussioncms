...
send secrets on this envelope. `object-storage` uses the same wrap
(`"sourceKind": "object-storage"`); PUT with a portable-safe local `rootPath` succeeds
and GET returns that kind. Cloud URLs (`s3://`, `gs://`, `azure://`, `http(s)://`) and
credential properties are **400**. `rss-atom` uses the same wrap
(`"sourceKind": "rss-atom"`); PUT with a portable-safe local `rootPath` succeeds and GET
returns that kind (local/loopback only; leftover `virtual.remoteUrl`, credentials, and
cloud URL `rootPath` are **400**; no live feed credentials). `icalendar` uses the same wrap
(`"sourceKind": "icalendar"`); PUT with a portable-safe local `rootPath` succeeds and GET
returns that kind (local RFC 5545 fixture only; leftover `virtual.remoteUrl`, credentials, and
cloud URL `rootPath` are **400**; no CalDAV). `sitemap-xml` uses the same wrap
(`"sourceKind": "sitemap-xml"`); PUT with a portable-safe local `rootPath` succeeds and GET
returns that kind (local sitemap fixture only; leftover `virtual.remoteUrl`, credentials, and
cloud URL `rootPath` are **400**; no live crawl). Paths with remaining `..` after NIO normalize, unknown
kinds, and `remoteUrl` on CSV, SQL, `http-json`, `object-storage`, `rss-atom`, `icalendar`, or `sitemap-xml` are **400**. In-product
`POST …/virtual/build` runs the matching adapter for `git-filesystem`,
`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt`. `POST …/virtual/publish`
runs git/CSV/SQL, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` adapters (copies assembled HTML to
`IPSSite.root`; leftover `virtual.remoteUrl` on `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` is
**400**; leftover credentials on `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` are **400**; leftover cloud URL `rootPath` on `sitemap-xml`, `robots-txt`, and `llms-txt` is **400**; no live crawl or HTTP fetch).
...
or `sitemap-xml`. Unknown kinds return **400**.
...
- **object-storage** — `virtual.rootPath` is a local object-key bucket (required
  `_config.yaml`; Markdown / HTML / JSON keys; optional `objects.keys`). Portable NIO
  `Path` / `Files` only. Git remotes are not used (`virtual.remoteUrl` is **400**). Cloud
  URLs and credential properties are **400**.
- **rss-atom** — local RSS 2.0 / Atom fixture under `virtual.rootPath` (`feed.xml` /
  `atom.xml` or `_config.yaml` `rss.file`; loopback `rss.url` only). Leftover
  `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**.
- **icalendar** — local RFC 5545 fixture (`calendar.ics` or `_config.yaml`
  `icalendar.file`). Leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are
  **400**. No CalDAV.
- **sitemap-xml** — local `sitemap.xml` fixture (`sitemap.xml` or `_config.yaml`
  `sitemap.file`; urlset of portable files under `virtual.rootPath`). Leftover
  `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**. No live crawl.
...
http-json catalog (`http.url` / `http.file` or default `pages.json`), object-storage
Markdown / HTML / JSON keys (and current `objects.keys`), and rss-atom feeds
(`rss.file` / `feed.xml` / `atom.xml` / loopback `rss.url`) from `virtual.rootPath` (no
CMS restart after `git pull`, a CSV/`_config.yaml` edit, a SQL query-file/`_config.yaml`
or H2 row edit, a JSON catalog/`_config.yaml` edit, an object-key / `_config.yaml` edit,
an RSS/Atom fixture / `_config.yaml` edit, or a local Markdown edit). File watchers are
not used; run **Build Virtual Site** again after those edits.
...
last-build home HTML and **Publish Virtual Site** copies assembled files to the Site
filesystem root.
When
...
**`csv-filesystem`**, **`sql-database`**, **`http-json`**, **`object-storage`**, and
**`rss-atom`** Virtual Sites (not git-only). Missing or failed builds return **200** with
`available=false` and a message (not 500). Traditional repository Sites and unknown
`virtual.sourceKind` values return **400**. `rss-atom` is a local RSS 2.0 / Atom
fixture or loopback feed (no live remote feeds).
...
`sql-database`, `http-json`, `object-storage`, or `rss-atom`, the server records the last
output path (including a custom `outputRoot`) so preview streams that tree. For
`sql-database` (in-memory H2), `http-json` (local JSON fixture or loopback catalog),
`object-storage` (local object-key bucket), and `rss-atom` (local RSS 2.0 / Atom fixture
or loopback feed), a successful Build is followed by
...
Runs the same build for `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`,
`object-storage`, or `rss-atom` (always the default output root; unlike Build, this endpoint
does **not** accept an `outputRoot` body), then copies the assembled tree
...
`virtual.remoteUrl` is **400**. `rss-atom` uses a local RSS 2.0 / Atom fixture or loopback
feed (`feed.xml` / `atom.xml` or `_config.yaml` `rss.file`); leftover `virtual.remoteUrl`
and credential properties are **400** (no live feeds).
...
Site** (Admin; **Git filesystem**, **CSV filesystem**, **SQL database**, **HTTP JSON**, and
**Object storage**; hidden for repository Sites). After a successful HTTP JSON or
object-storage Build, **Publish Virtual Site** copies assembled HTML to the Site
filesystem root. REST Publish still runs for `http-json`, `object-storage` (local
object-key fixture), and `rss-atom` (local RSS/Atom fixture). `sql-database` Publish is in-memory H2 only (`jdbc:h2:mem:`);
