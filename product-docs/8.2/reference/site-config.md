...
runs git/CSV/SQL, `http-json`, `object-storage`, `rss-atom`, and `icalendar` adapters (copies assembled HTML to
`IPSSite.root`; leftover `virtual.remoteUrl` on `http-json`, `object-storage`, `rss-atom`, and `icalendar` is
**400**; leftover credentials on `rss-atom` and `icalendar` are **400**).
