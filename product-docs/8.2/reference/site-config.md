...
`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt`. `POST …/virtual/publish`
runs git/CSV/SQL, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` adapters (copies assembled HTML to
`IPSSite.root`; leftover `virtual.remoteUrl` on `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` is
**400**; leftover credentials on `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` are **400**; leftover cloud URL `rootPath` on `sitemap-xml` and `robots-txt` is **400**; no live crawl).
