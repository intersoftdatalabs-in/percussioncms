...
cloud URL `rootPath` are **400**; no CalDAV). Paths with remaining `..` after NIO normalize, unknown
kinds, and `remoteUrl` on CSV, SQL, `http-json`, `object-storage`, `rss-atom`, or `icalendar` are **400**. In-product
