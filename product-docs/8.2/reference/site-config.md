...
`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, and `sitemap-xml`. `POST …/virtual/publish`
runs git/CSV/SQL, `http-json`, `object-storage`, `rss-atom`, `icalendar`, and `sitemap-xml` adapters (copies assembled HTML to
`IPSSite.root`; leftover `virtual.remoteUrl` on `http-json`, `object-storage`, `rss-atom`, `icalendar`, and `sitemap-xml` is
**400**; leftover credentials on `rss-atom`, `icalendar`, and `sitemap-xml` are **400**; leftover cloud URL `rootPath` on `sitemap-xml` is **400**; no live crawl).
