...
| Virtual preview status | `GET /services/sites/{nameOrId}/virtual/preview` | Admin-only; last-build availability for `git-filesystem` **and** `csv-filesystem` (not git-only). Missing build → `available=false` (HTTP 200). Repository / unknown `sourceKind` → 400 |
