...
| Create | `POST /services/sites` | Admin. Body Jackson wrap `{ "Site": { "name", "description?", "baseUrl?" } }`. Name required, unique, letters/digits/space/hyphen/underscore, max 50, start with a letter. Duplicate name is **409**. Same action as **Developer → Sites → New site**. |
| Update | `PUT /services/sites/{nameOrId}` | Admin. Updates description and `baseUrl` (body `name` must match the path). A mismatched body name is **400** — use Rename, not this PUT. Optional `workflowName` sets the workflow on the site folder (`sys_workflowid`). The name must already exist; an unknown workflow is **400** and the site row is not saved. **403** when the caller is not Admin. **404** when the site or its folder is missing. Omit `workflowName` to leave the association unchanged. GET detail returns the saved `workflowName`. Same action as **Developer → Sites** detail: choose **Workflow**, then Save. Transition editing and full site publish stay outside this surface. |
| Rename | `POST /services/sites/{nameOrId}/rename` | Admin. Body Jackson wrap `{ "RenameSiteRequest": { "name" } }`. Persists the new site name (folder, navigation title, and related site records). Does not copy the site. Blank or illegal names are **400**; non-admin is **403**; an existing site or site folder name is **409**. Same action as Content Explorer **Content → Rename Site**. |
| Delete | `DELETE /services/sites/{nameOrId}` | Admin. Removes the publishing site definition. Same action as **Developer → Sites** detail Delete (confirm dialog). |
| Add navigation section | `POST /sitemanage/section/create` | Admin. Same `CreateSiteSection` body Navigation uses. **Developer → Sites** detail sends name + parent; the client derives the URL segment and landing-page file and uses a site (or read-only library) template. Invalid names stay in the panel (no POST). **400** and **403** stay on the panel. **Cancel** does not post. Sites with `managedNavigation` false, and Virtual Sites, are read-only here. Reorder and delete are not this operation. |

| Virtual properties | `GET` / `PUT /services/sites/{nameOrId}/virtual` | Virtual Site source bag. PUT JSON is `{ "VirtualSiteProperties": { "sourceKind", "rootPath", "remoteUrl", "branch", "configFile", "siteKey" } }` (Jackson/JAXB root wrap). Allow-listed `sourceKind`: `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, `json-schema` (GET round-trips the kind after PUT). `json-schema` persist is a local JSON Schema fixture only (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `jsonschema.url` are **400**; no live HTTP schema fetch). Developer Sites can save and GET-roundtrip **JSON Schema** (`sourceKind=json-schema`) with a portable-safe local `rootPath`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local JSON Schema Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `schema.json` / `jsonschema.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). `graphql-sdl` persist is a local GraphQL SDL fixture only (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP or introspection). Developer Sites can save and GET-roundtrip **GraphQL SDL** (`sourceKind=graphql-sdl`) with a portable-safe local `rootPath`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local GraphQL SDL Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `schema.graphql` / `graphql.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). `asyncapi-yaml` persist is a local AsyncAPI 2/3 YAML fixture only (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch). Developer Sites can save and GET-roundtrip **AsyncAPI YAML** (`sourceKind=asyncapi-yaml`) with a portable-safe local `rootPath`. REST **Build** (`POST …/virtual/build`) writes HTML from that local `asyncapi.yaml` / `asyncapi.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local AsyncAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). `openapi-yaml` persist is a local OpenAPI 3 YAML fixture only (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch). Developer Sites can save and GET-roundtrip **OpenAPI YAML** (`sourceKind=openapi-yaml`) with a portable-safe local `rootPath`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local OpenAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `openapi.yaml` / `openapi.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200; leftover `virtual.remoteUrl` and credentials are **400**; no live spec fetch). `llms-txt` persist is a local llms.txt fixture only (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch). Developer Sites can save and GET-roundtrip **llms.txt** (`sourceKind=llms-txt`) with a portable-safe local `rootPath`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local llms.txt Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `llms.txt` / `llms.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live HTTP fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200; leftover `virtual.remoteUrl` and credentials are **400**; no live HTTP fetch). `robots-txt` persist is a local robots.txt fixture only (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can save and GET-roundtrip **Robots.txt** (`sourceKind=robots-txt`) with a portable-safe local `rootPath`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local robots.txt Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `robots.txt` / `robots.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live crawl). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200; leftover `virtual.remoteUrl` and credentials are **400**; no live crawl). `sitemap-xml` persist is a local sitemap.xml fixture only (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). REST **Build** (`POST …/virtual/build`) writes HTML from that local `sitemap.xml` / `sitemap.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live crawl). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200; leftover `virtual.remoteUrl` and credentials are **400**; no live crawl). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local sitemap.xml Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** for `sitemap-xml` after save (last-build local HTML; missing build stays unavailable). SPI/CLI assemble remains `PSVirtualSiteBuildMain … sitemap-xml`. JDBC URL/user/query for `sql-database` live in `_config.yaml` under `rootPath` (in-memory H2 `jdbc:h2:mem:` only; never send passwords on this envelope). `http-json` catalog URL/file live in `_config.yaml` (`http.url` / `http.file` or default `pages.json`); REST persists a portable-safe `rootPath` JSON fixture (no remaining `..`). `object-storage` persists a portable-safe local `rootPath` (cloud URLs and credential properties are **400**; no AWS/IAM/secrets). `rss-atom` persists a portable-safe local `rootPath` (local/loopback only; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live feed credentials). `icalendar` persists a portable-safe local `rootPath` (local RFC 5545 `.ics` fixture only; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no CalDAV). REST **Build** (`POST …/virtual/build`) writes HTML from that local `calendar.ics` / `icalendar.file` fixture (`pagesWritten > 0`). REST **Preview** streams last-build HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Publish** copies assembled HTML to `IPSSite.root`. Developer Sites can save `icalendar` and then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. Unknown kinds and `csv-filesystem` / `sql-database` / `http-json` / `object-storage` / `rss-atom` / `icalendar` / `sitemap-xml` / `robots-txt` / `llms-txt` / `openapi-yaml` / `asyncapi-yaml` + `remoteUrl` return **400** (no secrets on this envelope). A flat `{ "sourceKind": … }` body returns **400** unexpected element `sourceKind`. Optional `remoteUrl` + `branch` clone/fetch before Git Build; omit `remoteUrl` to keep a stored remote; send `""` to clear. Developer Sites can save `http-json` and then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. Developer Sites can save `object-storage` (GET round-trips the kind) and then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` for a local object-key `rootPath` (`virtual.remoteUrl` is **400**) and for `rss-atom` (local RSS/Atom fixture; leftover `virtual.remoteUrl` and credentials are **400**). REST Preview streams last-build HTML after REST or in-product Build, including `object-storage`, `rss-atom`, and `icalendar`. |
| Virtual build | `POST /services/sites/{nameOrId}/virtual/build` | Admin-only; `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, or `json-schema`. Git fetches `remoteUrl` when set; CSV reads a local CSV tree (`rootPath`; optional `_config.yaml`); SQL reads required `_config.yaml` `sql:` mapping (in-memory H2 `jdbc:h2:mem:` only; Oracle/MySQL/SQL Server URLs **400**). HTTP JSON reads a local JSON fixture or loopback catalog from required `_config.yaml` (`http.url` / `http.file` / default `pages.json`); `virtual.remoteUrl` is **400**. Object-storage reads a local object-key bucket (`rootPath`; required `_config.yaml`; optional `objects.keys`); `virtual.remoteUrl` is **400** (no cloud URLs or credentials). `rss-atom` reads a local RSS 2.0 / Atom fixture (`feed.xml` / `atom.xml` or `_config.yaml` `rss.file`; `rss.url` loopback only); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live remote feeds). `icalendar` reads a local RFC 5545 fixture (`calendar.ics` or `_config.yaml` `icalendar.file`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no CalDAV). `sitemap-xml` reads a local sitemap.xml fixture (`sitemap.xml` or `_config.yaml` `sitemap.file`; urlset of portable files under `rootPath`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live crawl). `robots-txt` reads a local robots.txt fixture (`robots.txt` or `_config.yaml` `robots.file`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live crawl). `llms-txt` reads a local llms.txt fixture (`llms.txt` or `_config.yaml` `llms.file`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live HTTP fetch). `openapi-yaml` reads a local OpenAPI 3 YAML fixture (`openapi.yaml` or `_config.yaml` `openapi.file`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live spec fetch). `asyncapi-yaml` reads a local AsyncAPI 2/3 YAML fixture (`asyncapi.yaml` or `_config.yaml` `asyncapi.file`); leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400** (no live spec fetch). `graphql-sdl` reads a local GraphQL SDL fixture (`schema.graphql` or `_config.yaml` `graphql.file`); leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400** (no live GraphQL HTTP or introspection). `json-schema` reads a local JSON Schema fixture (`schema.json` or `_config.yaml` `jsonschema.file`); leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400** (no live HTTP schema fetch). Each Build re-reads the current fixture and `_config.yaml` (no JVM restart; no file watchers). A second Admin `POST …/virtual/build` after an in-process Path/Files edit of `sitemap.xml` loc/lastmod, `sitemap.file`, or referenced page Markdown returns `pagesWritten > 0` HTML that reflects the **current** file (no Jetty restart). Same action as **Developer → Sites → Build Virtual Site** after save for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, Sitemap XML, Robots.txt, llms.txt, OpenAPI YAML, AsyncAPI YAML, GraphQL SDL, and JSON Schema. Unknown `sourceKind` is **400**. |
| Virtual preview status | `GET /services/sites/{nameOrId}/virtual/preview` | Admin-only; last-build availability for `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` (not git-only). After a successful Build, `http-json`, SQL, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` return `available=true` + `homePath`. `rss-atom` is a local RSS 2.0 / Atom fixture or loopback feed (no live remote feeds). `icalendar` is a local RFC 5545 `calendar.ics` fixture (no CalDAV). `sitemap-xml` is last-build local HTML only (`sitemap.xml` / `sitemap.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live crawl). `robots-txt` is last-build local HTML only (`robots.txt` / `robots.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live crawl). `llms-txt` is last-build local HTML only (`llms.txt` / `llms.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live HTTP fetch). `openapi-yaml` is last-build local HTML only (`openapi.yaml` / `openapi.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live spec fetch). `asyncapi-yaml` is last-build local HTML only (`asyncapi.yaml` / `asyncapi.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live spec fetch). `graphql-sdl` is last-build local HTML only (`schema.graphql` / `graphql.file`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). `json-schema` is last-build local HTML only (`schema.json` / `jsonschema.file`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch). Developer Sites **Preview assembled site** uses this last-build status for `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` after Build (missing build stays unavailable). Missing build → `available=false` (HTTP 200). Repository / unknown `sourceKind` → 400. CLI assemble is previewable only at the default output root (no last-output pointer). |
| Virtual preview file | `GET /services/sites/{nameOrId}/virtual/preview/{relPath}` | Admin-only; assembled file stream from last output (`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, `json-schema`). `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` are last-build local HTML only (no live crawl, HTTP fetch, GraphQL introspection, or HTTP schema fetch). Traversal (`../`) or file over 20 MB → 400; missing file → 404 (not 500) |
| Virtual publish | `POST /services/sites/{nameOrId}/virtual/publish` | Admin-only; `git-filesystem`, `csv-filesystem`, `sql-database` (in-memory H2 `jdbc:h2:mem:` only; Oracle/MySQL/SQL Server **400**), `http-json` (local JSON fixture or loopback catalog; catalog URL/file stay in `_config.yaml`; `virtual.remoteUrl` is **400**; no secrets on the envelope), `object-storage` (portable-safe local object-key `rootPath`; no cloud URLs, IAM, or access keys; leftover `virtual.remoteUrl` is **400**), `rss-atom` (local RSS 2.0 / Atom fixture or loopback feed; `feed.xml` / `atom.xml` or `_config.yaml` `rss.file`; leftover `virtual.remoteUrl` and credentials are **400**; no live feeds), `icalendar` (local RFC 5545 `calendar.ics` / `icalendar.file`; leftover `virtual.remoteUrl` and credentials are **400**; no CalDAV), `sitemap-xml` (local `sitemap.xml` / `sitemap.file`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl), or `robots-txt` (local `robots.txt` / `robots.file`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl; missing assemble is **400** and does not invent pages), or `llms-txt` (local `llms.txt` / `llms.file`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch; missing assemble is **400** and does not invent pages), or `openapi-yaml` (local `openapi.yaml` / `openapi.file`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages), or `asyncapi-yaml` (local `asyncapi.yaml` / `asyncapi.file`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages), or `graphql-sdl` (local `schema.graphql` / `graphql.file`; leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP; missing assemble is **400** and does not invent pages), or `json-schema` (local `schema.json` / `jsonschema.file`; leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch; missing assemble is **400** and does not invent pages). Builds then copies assembled files to the Site filesystem root (`IPSSite.root`). No `outputRoot` body. Same action as **Developer → Sites → Publish Virtual Site** for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, Sitemap XML, Robots.txt, llms.txt, OpenAPI YAML, AsyncAPI YAML, GraphQL SDL, and JSON Schema after Build. Sitemap XML, Robots.txt, llms.txt, OpenAPI YAML, AsyncAPI YAML, GraphQL SDL, and JSON Schema copy last-build local HTML (leftover remoteUrl/credentials/`graphql.url`/`jsonschema.url` fail closed; no live crawl, HTTP fetch, GraphQL introspection, or HTTP schema fetch). Repository / unknown kinds are **400**. |
...
| Save | `PUT /services/contenttypes/{idOrName}` | **Admin.** Requires a lock already held by the current user. Saves label, description, enabled, per-field searchable/occurrence, **local** field display labels (`fields[].label`), parent field order (`fields[].sequence`), workflows, and templates. Does **not** change name (use `PUT .../name`). Does **not** release the lock. POST `/lock` and PUT share the packed NODEDEF design-object id so a lock you hold is found on save. The save load (`lock=true`) **extends** a still-valid lock; a PUT after expiry returns `409` and the client must re-lock. Blank local field label is **400**. Label write on a system/shared field is **400**. Child field-set `sequence` is **400**. Unknown field is **404**. Field rule expressions use the dedicated path below (not this PUT). |
...
system or shared field is **400**.

Parent field **order** is `fields[].sequence` on the same PUT (zero-based among
parent display mappings). Omit `sequence` to leave order unchanged. A sequence
on a child field set is **400**. GET detail returns `sequence` and lists parent
fields in that order. Child field-set order remains Workbench-only.
...
## Pipelines (XML Applications catalog, lifecycle, IR, OpenAPI, HTTP execute, nested filter groups, webhook hooks, result pages, request tracing, and validation)
...
### HTML result-page persist and XSL apply (Slice D)

`PUT …/resources/{resource}/resultPage` requires **Admin** (**403** otherwise).
Body is `PipelineResultPage`:
...
(`params`, optional `rows` / `operation` / `keyColumns`, optional `requestExtension` /
`accept`) and returns `PipelineExecuteResult`. This path uses the native pipeline IR
...
| `POST` | `/services/workflows/{idOrName}/copy` | **Admin.** Copy an existing workflow, including its steps and transitions, under a new unique name (`WorkflowCreate` wrap). The source is not modified. A duplicate name is `409` and does not overwrite. Invalid name is `400`. Missing source is `404`. Non-Admin is `403`. Omit `description` to keep the source description. Returns the new `WorkflowSummary` |
...
| `POST` | `/services/workflows/{idOrName}/default` | **Admin.** Mark this workflow as the single system default (`DEFAULT_WORKFLOW`). A later call for another workflow replaces that name so the previous workflow is no longer default. Does not move existing content items. Missing workflow is `404`; non-Admin is `403`. Returns `WorkflowSummary` with `defaultWorkflow` true. No request body. |
...
| `GET` | `/services/workflows/{idOrName}/graph` | **Admin.** Graph (`WorkflowGraph`: `nodes`, `edges` with `from` / `to` / `label` / `commentRequired`, `packaged`, `defaultWorkflow`). Stock names (Default Workflow, Simple Workflow, Local Content) and the server default flag set `packaged` true. Does not create transitions. Missing workflow is `404`; non-Admin is `403`. |
| `PUT` | `/services/workflows/{idOrName}/transitions/comment-required?from={step}&label={label}&to={step}` | **Admin.** Set `commentRequired` on one existing transition (`WorkflowTransitionComment` wrap). `from` and `label` are required. `to` is required when the label is ambiguous. Returns the updated graph. Does not create or delete the transition. Packaged/default workflows are `403`. Aging transitions are `400`. Missing workflow, step, or transition is `404`. |
...
| Copy item | `POST /rest/folders/copy/item` with the same JSON root `CopyFolderItemRequest`. Explorer Copy of a selected page, file, or asset must use this endpoint — `copy/folder` 500s for non-folders. Server NewCopy check-in assigns the workflow initial state when the new row has no `CONTENTSTATEID`. Missing source **or destination** folder is HTTP **404** (not 500, and not HTTP 200 with a 404 JSON body). |
| Rename item | `POST /rest/folders/rename/item` with JSON root `RenameFolderItemRequest` (`itemPath`, `newName`). Explorer Rename of a selected page, file, or asset uses this endpoint. The server updates `sys_title` (and `filename` for pages and file assets) then checks the item in once — a nested check-in during save would roll the transaction back and return **500**. Folder rename stays on pathmanagement `POST …/path/renameFolder`. Missing item is **404**; non-admin is **403**; folder selected, name in use, or locked is **409**. Finder paths such as `/Assets/…` are accepted. |
| Item properties | `GET /rest/folders/item-properties/{itemPath}` and `POST /rest/folders/item-properties` with JSON root `ItemPropertiesRequest` (`itemPath`, `name`, optional `displayTitle`). Explorer **Item properties** uses these to load and save listing name (`sys_title`) and display title (`displaytitle`). Blank `name` is **400**; missing item is **404**; non-admin is **403**; folder selected, name in use, or locked is **409**. |
...
The React Content Editor host **Preview template** list does not write the item. **Current template** keeps Page Management render (`GET /services/pagemanagement/render/page/{id}`). Another allowed template loads `GET /services/assembly/preview-location?contentId=&templateId=` and shows that assembler URL. It does not call `PUT …/page/changeTemplate`.

The React Content Editor host **Check Out** / **Check In** actions use public REST `POST /rest/editor/items/{id}/checkout` and `POST /rest/editor/items/{id}/checkin` (JSON `EditorItemLockInfo`: `checkOutUser`, `currentUser`, `itemName`, `assignmentType`). HTTP **403** (not allowed) and **409** (checked out to another user, or check-in when you do not hold the lock) are errors — neither status is treated as success. When the open item is checked out to **another** user, **Edit** mode shows that user and offers **Force Check-in** after confirm (`GET /services/itemmanagement/workflow/forceCheckIn/{id}`, Admin assignment). Cancel does not call the server. Success clears the other user's checkout so the author can **Check Out** and save as usual. The author's own **Check In** opens an optional revision-comment prompt first. **Cancel** does not check in. A blank comment still checks in (no `comment` query). A non-blank comment is sent as `POST /rest/editor/items/{id}/checkin?comment=…` and stored on the revision. **View** mode shows the checkout owner and does not offer force check-in. HTTP **403** (not Admin), **404** (unknown id), and **409** (not checked out) are errors on the host — they are not success. Legacy `GET /services/itemmanagement/workflow/checkOut/{id}` and `…/checkIn/{id}` remain for other CM1 panels; a successful check-out there still returns `ItemUserInfo`. When `checkOutUser` is not the session user, the React editor stays view-only. An empty `currentUser` on lock info does **not** grant edit when another holder is named on `checkOutUser`. Restore prior revision uses `GET /services/itemmanagement/item/revisions/{id}` (list) and `GET /services/itemmanagement/item/restoreRevision/{revisionGuid}` (apply); HTTP **403** (no assignment) and **404** (item or revision not found) are errors — neither status is treated as success. The editor host reloads fields via `GET /services/itemmanagement/item/fields/{id}` after a successful restore. Content-type labels, control names, and **required** flags come from `GET /services/contenttypes/{type}`. The React editor maps `sys_tinymce` to TinyMCE, `sys_File` / image controls to file upload, keyword and community names to pickers. Empty required fields are blocked client-side (inline errors; no PUT; check-in is not sent). A **400** on `PUT …/item/fields/{id}` that names a field (`errorData`, `field` / `fieldName`, or a quoted field in the message) is shown on that row. The Active Assembly overlay reuses the same `GET`/`PUT` fields endpoints for known scalar text on the assembled preview. Neither host requests leftover Content Editor HTML.

Content Explorer shows the checkout owner of the selected page or asset with read-only `GET /rest/editor/items/{id}/checkout-owner` (same `EditorItemLockInfo` JSON). An empty `checkOutUser` means the item is not checked out. A lock held by another user is HTTP **200**, not **409**. HTTP **403** means the lookup was not allowed. The call does not check the item out. Folders are not queried.
...
Restore-prior-revision response is the same `PSNoContent` used by checkout / check-in
(`{ operation }`). The React Content Editor host **Show revisions / Restore prior
revision** action calls `GET …/revisions/{id}` first, lets the author pick a revision,
**confirms**, then calls `GET …/restoreRevision/{revisionGuid}`. On success the host
**reloads fields** from `GET …/item/fields/{id}` so the editor reflects the restored
revision. HTTP **403** ("not allowed to restore", including `NONE` / `READER`
