...
Checkout / check-in remain `GET /services/itemmanagement/workflow/checkOut/{id}` and `…/checkIn/{id}`. A successful check-out returns `ItemUserInfo` (`checkOutUser`, `currentUser`). When `checkOutUser` is not the session user, the React editor stays view-only. Check-in of an item you do not hold is **409**; missing assignment / not allowed is **403**. Neither status is treated as success. Restore prior revision uses `GET /services/itemmanagement/item/revisions/{id}` (list) and `GET /services/itemmanagement/item/restoreRevision/{revisionGuid}` (apply); HTTP **403** (no assignment) and **404** (item or revision not found) are errors — neither status is treated as success. The editor host reloads fields via `GET /services/itemmanagement/item/fields/{id}` after a successful restore. Content-type labels, control names, and **required** flags come from `GET /services/contenttypes/{type}`. The React editor maps `sys_tinymce` to TinyMCE, `sys_File` / image controls to file upload, keyword and community names to pickers. Empty required fields are blocked client-side (inline errors; no PUT; check-in is not sent). A **400** on `PUT …/item/fields/{id}` that names a field (`errorData`, `field` / `fieldName`, or a quoted field in the message) is shown on that row. The Active Assembly overlay reuses the same `GET`/`PUT` fields endpoints for known scalar text on the assembled preview. Neither host requests leftover Content Editor HTML.
