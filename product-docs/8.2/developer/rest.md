...
| Rename item | `POST /rest/folders/rename/item` with JSON root `RenameFolderItemRequest` (`itemPath`, `newName`). Explorer Rename of a selected page, file, or asset uses this endpoint. Folder rename stays on pathmanagement `POST …/path/renameFolder`. Missing item is **404**; non-admin is **403**; folder selected, name in use, or locked is **409**. Finder paths such as `/Assets/…` are accepted. |
