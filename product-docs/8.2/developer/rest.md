...
| Virtual preview status | `GET /services/sites/{nameOrId}/virtual/preview` | Admin-only; last-build availability for `git-filesystem`, `csv-filesystem`, `sql-database`, and `http-json` (not git-only). After a successful Build, `http-json` (and SQL) return `available=true` + `homePath`. Missing build → `available=false` (HTTP 200). Repository / unknown `sourceKind` → 400. CLI assemble is previewable only at the default output root (no last-output pointer). |
| Virtual preview file | `GET /services/sites/{nameOrId}/virtual/preview/{relPath}` | Admin-only; assembled file stream from last output (`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`). Traversal (`../`) or file over 20 MB → 400; missing file → 404 (not 500) |
