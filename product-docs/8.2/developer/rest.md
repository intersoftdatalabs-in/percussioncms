...
| Virtual build | `POST /services/sites/{nameOrId}/virtual/build` | Admin-only; Git-filesystem Virtual Sites. Fetches `remoteUrl` when set, else reads local `rootPath`. `csv-filesystem` sites return **400** on this path. |
