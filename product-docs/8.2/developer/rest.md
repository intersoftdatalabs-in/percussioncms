...
| Virtual preview status | `GET /services/sites/{nameOrId}/virtual/preview` | Admin-only; last-build availability for `git-filesystem` **and** `csv-filesystem` (not git-only). Same action as **Developer → Sites → Preview assembled site**. Missing build → `available=false` (HTTP 200). Repository / unknown `sourceKind` → 400 |
| Virtual preview file | `GET /services/sites/{nameOrId}/virtual/preview/{relPath}` | Admin-only; assembled file stream from last output. Traversal (`../`) → 400; missing file → 404 (not 500) |
