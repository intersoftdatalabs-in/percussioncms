...
(no live crawl). REST **Publish** and Developer Sites chrome stay later slices.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **local `sitemap.xml` SPI** (CLI assemble from `sitemap.xml` / `sitemap.file`; no live
  ...
  `virtual.remoteUrl` and credentials are **400**; no live crawl). REST **Publish** and
...
missing build is `available=false` HTTP **200**). REST **Publish** and Developer Sites chrome
stay later slices. Operators can also assemble offline with the CLI.
...
are **400**; no live crawl). REST **Publish** and Developer Sites chrome for `sitemap-xml`
stay later slices.
