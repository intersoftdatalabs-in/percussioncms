...
**Preview** and **Publish** stay later slices. Developer Sites can save and GET-roundtrip
`sourceKind=sitemap-xml` (local `rootPath` only; leftover `virtual.remoteUrl` and crawl
credentials are never sent); Build/Preview/Publish chrome stay hidden.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **local `sitemap.xml` SPI** (CLI assemble from `sitemap.xml` / `sitemap.file`; no live
  ...
  and cloud `rootPath` are **400**). REST **Preview** and **Publish** stay later slices.
...
**Preview** and **Publish** stay later slices. Developer Sites can save and GET-roundtrip
`sourceKind=sitemap-xml` (local `rootPath` only; leftover `virtual.remoteUrl` and crawl
credentials are never sent); Build/Preview/Publish chrome stay hidden. Operators can also
...
credentials, and cloud `rootPath` are **400**; no live crawl). REST **Preview** and
**Publish** stay later slices. Developer **Sites** can save and GET-roundtrip
`sourceKind=sitemap-xml` (local fixture `rootPath` only); **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site** chrome stay hidden.
