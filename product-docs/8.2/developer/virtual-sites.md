...
and crawl credentials are never sent) and then **Build Virtual Site**. Preview/Publish chrome stay hidden.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **local `sitemap.xml` SPI** (CLI assemble from `sitemap.xml` / `sitemap.file`; no live
  ...
  and then **Build Virtual Site**. Preview/Publish chrome stay hidden.
...
leftover `virtual.remoteUrl` and crawl credentials are never sent) and then **Build Virtual Site**.
Preview/Publish chrome stay hidden. Operators can also assemble offline with the CLI.
