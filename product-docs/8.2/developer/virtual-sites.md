...
REST GET/PUT persist, REST Build/Preview/Publish, and Developer Sites chrome for
`icalendar` stay later slices.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **GET/PUT** round-trips `object-storage` (safe local `rootPath`; no cloud URLs or
  ...
  `calendar.ics` / `icalendar.file`; no CalDAV). REST persist and Developer Sites
  chrome for `icalendar` stay later slices.
...
API keys, no Basic/OAuth. REST persist, REST Build/Preview/Publish, and Developer Sites
chrome stay later slices. Operators assemble offline with the CLI.
