...
GET-roundtrip `sourceKind=json-schema` (local `rootPath` only). **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site** stay later slices.
...
source chrome round-trip `sourceKind=json-schema` (local `rootPath` only). REST Build, Preview,
and Publish stay later slices.
...
assembled site**, and **Publish Virtual Site** stay later slices.
