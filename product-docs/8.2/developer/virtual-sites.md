...
`available=false` HTTP 200). Cloud URLs (`s3://`, `gs://`, `azure://`, `http(s)://`) and
...
`sourceKind=object-storage`, GET-roundtrip the kind, and then **Build Virtual Site**
(local object-key directory only). REST **Publish** and in-product Preview/Publish chrome
for this kind stay a later phase.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** copies
  ...
  local object-key SPI (CLI assemble) and REST **GET/PUT/Build/Preview**. REST **GET/PUT**
...
**Publish** and Developer Sites Preview/Publish chrome for this kind stay a later phase.
