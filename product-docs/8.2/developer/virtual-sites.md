...
(local fixture only). REST Publish and Developer Sites Preview / Publish chrome for this
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **GET/PUT** round-trips `object-storage` (safe local `rootPath`; no cloud URLs or
  ...
  after save (local fixture only). REST Publish and Developer Sites Preview / Publish chrome
  for `rss-atom` stay later slices.
