...
(`available=true`; missing build is `available=false` HTTP 200). **Publish Virtual Site**
stays a later slice.
...
`sourceKind=graphql-sdl` (local `rootPath` only), then **Build Virtual Site** and
**Preview assembled site**. REST **Build** writes HTML from the local fixture
(`pagesWritten > 0`). REST **Preview** streams last-build local HTML (missing build is
`available=false` HTTP 200). **Publish Virtual Site** stays a later slice.
...
assembled site** run after save. **Publish Virtual Site** stays a later slice.
...
object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, and asyncapi-yaml after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml/asyncapi-yaml).
