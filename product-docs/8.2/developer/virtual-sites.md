...
and crawl credentials are never sent), then **Build Virtual Site** and **Preview assembled site**.
**Publish Virtual Site** chrome stays hidden.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **local `sitemap.xml` SPI** (CLI assemble from `sitemap.xml` / `sitemap.file`; no live
  ...
  then **Build Virtual Site** and **Preview assembled site** (last-build local HTML;
  missing build stays unavailable). **Publish Virtual Site** chrome stays hidden.
...
and **Preview assembled site** (last-build local HTML; missing build stays unavailable).
**Publish Virtual Site** chrome stays hidden. Operators can also assemble offline with the CLI.
...
Developer Sites **Publish Virtual Site** chrome stays hidden.
...
Developer Sites **Preview assembled site** uses that last-build Preview after **Build Virtual Site**. Missing build stays unavailable (`available=false` HTTP **200**; no fake preview). **Publish Virtual Site** chrome stays hidden.
...
**`rss-atom`**, **`icalendar`**, and **`sitemap-xml`** (not
...
object-storage, rss-atom, icalendar, and sitemap-xml after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar; Sitemap XML Publish chrome stays hidden).
