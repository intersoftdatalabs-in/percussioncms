...
and **Preview assembled site**. **Publish Virtual Site** stays a later slice.
...
(local fixture `rootPath` only). REST **Build** (`POST …/virtual/build`) writes HTML from that local fixture (`pagesWritten > 0`). REST **Preview** (`GET …/virtual/preview`) streams last-build HTML (missing build is `available=false` HTTP **200**). **Publish Virtual Site** stays a later slice. Operators
...
envelope. **Build Virtual Site** and **Preview assembled site** run after save. **Publish Virtual Site** stays a later slice.
...
last-build HTML for **Preview assembled site**. **Publish Virtual Site** stays a later slice.
...
**400**. Developer Sites **Preview assembled site** uses that last-build Preview after
**Build Virtual Site**. Missing build stays unavailable (`available=false` HTTP **200**;
no fake preview). REST **Publish** stays a later slice.
...
object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, and asyncapi-yaml after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml; asyncapi-yaml Publish stays later).
