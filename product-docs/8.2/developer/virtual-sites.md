...
GET-roundtrip `sourceKind=graphql-sdl` (local `rootPath` only). **Build Virtual Site**,
...
`sourceKind=graphql-sdl` (local `rootPath` only). REST **Build/Preview/Publish** stay
later slices.
...
credentials are never sent on the REST envelope. **Build Virtual Site**, **Preview assembled
site**, and **Publish Virtual Site** stay later slices.
