...
`rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` after a successful Build (CLI preview requires
...
closed). **Robots.txt** shows **Publish Virtual Site** (copies last-build local HTML after a
local `robots.txt` Build; leftover `virtual.remoteUrl` and crawl credentials fail closed).
...
(no live crawl). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to
`IPSSite.root` after a local robots.txt Build (leftover `virtual.remoteUrl`, credentials,
and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can save and
...
markdown list link (hyphen, title in square brackets, then a parenthesized local path) maps into assemble `id` / `title` / `body`. A
...
`GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=llms-txt` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud
URL `rootPath` are **400**; no live HTTP fetch). Developer Sites can save and
GET-roundtrip `sourceKind=llms-txt` (local `rootPath` only; leftover
`virtual.remoteUrl` and fetch credentials are never sent), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**.
...
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL
`rootPath` are **400**; no live spec fetch). Developer Sites can save and
GET-roundtrip `sourceKind=openapi-yaml` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`)
copies assembled HTML to `IPSSite.root` after a local OpenAPI YAML Build (leftover `virtual.remoteUrl`,
credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages).

An **AsyncAPI 2/3 YAML** adapter (`asyncapi-yaml`) discovers pages from a **local
`asyncapi.yaml`** under `virtual.rootPath` (`asyncapi.yaml`, or `_config.yaml`
`asyncapi.file`). Each channel/operation maps into assemble `id` / `title` / `body`.
A fixture with no operations still emits one HTML page from `info`. This is a
**local fixture SPI** — **no live HTTP spec fetch, no remote AsyncAPI URLs, no
authenticated remotes**. `virtual.remoteUrl`, credential properties, `asyncapi.url`,
cloud `rootPath` URLs, and remote `$ref` values are rejected. SPI/CLI assemble is
`PSVirtualSiteBuildMain … asyncapi-yaml` (`pagesWritten > 0` from a temp fixture).
REST `GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=asyncapi-yaml` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL
`rootPath` are **400**; no live spec fetch). Developer Sites can save and
GET-roundtrip `sourceKind=asyncapi-yaml` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`)
copies assembled HTML to `IPSSite.root` after a local AsyncAPI YAML Build (leftover `virtual.remoteUrl`,
credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages).

A **GraphQL SDL** adapter (`graphql-sdl`) discovers pages from a **local
`schema.graphql`** under `virtual.rootPath` (`schema.graphql`, or `_config.yaml`
`graphql.file`). Each Query / Mutation / Subscription field maps into assemble
`id` / `title` / `body`. A fixture with no operations still emits one HTML page from
the schema. This is a **local fixture SPI** — **no live GraphQL HTTP, no
introspection, no remote GraphQL URLs, no authenticated remotes**.
`virtual.remoteUrl`, credential properties, `graphql.url`, cloud `rootPath` URLs,
and remote `#import` URLs are rejected. SPI/CLI assemble is
`PSVirtualSiteBuildMain … graphql-sdl` (`pagesWritten > 0` from a temp fixture).
REST `GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=graphql-sdl` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL
`rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). Developer Sites can save and
GET-roundtrip `sourceKind=graphql-sdl` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`)
copies assembled HTML to `IPSSite.root` after a local GraphQL SDL Build (leftover `virtual.remoteUrl`,
credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that
local `schema.graphql` / `graphql.file` fixture (`pagesWritten > 0`; leftover
`virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400**; no live
GraphQL HTTP). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML
(`available=true`; missing build is `available=false` HTTP 200).

A **JSON Schema** adapter (`json-schema`) discovers pages from a **local
`schema.json`** under `virtual.rootPath` (`schema.json`, or `_config.yaml`
`jsonschema.file`). Each `properties` / `$defs` entry maps into assemble
`id` / `title` / `body`. A fixture with no properties or defs still emits one HTML
page from `title` / `description`. This is a **local fixture SPI** — **no live
HTTP schema fetch, no remote JSON Schema URLs, no authenticated remotes**.
`virtual.remoteUrl`, credential properties, `jsonschema.url`, cloud `rootPath`
URLs, and remote `$ref` / `$id` HTTP are rejected. SPI/CLI assemble is
`PSVirtualSiteBuildMain … json-schema` (`pagesWritten > 0` from a temp fixture).
REST `GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=json-schema` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL
`rootPath`, and `jsonschema.url` are **400**; no live HTTP schema fetch). Developer Sites can save and
GET-roundtrip `sourceKind=json-schema` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`)
copies assembled HTML to `IPSSite.root` after a local JSON Schema Build (leftover `virtual.remoteUrl`,
credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that
local `schema.json` / `jsonschema.file` fixture (`pagesWritten > 0`; leftover
`virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live
HTTP schema fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML
(`available=true`; missing build is `available=false` HTTP 200).
...
|----------|----------|---------|---------|
| `virtual.sourceKind` | Yes (for Virtual) | `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, or `json-schema` | Adapter wire name. **Allow-list:** `git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, `json-schema`. **`json-schema`** is a local JSON Schema SPI (`schema.json` or `_config.yaml` `jsonschema.file`; no live HTTP schema fetch). SPI/CLI assemble is `PSVirtualSiteBuildMain … json-schema` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `json-schema` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `jsonschema.url` are **400**; no live HTTP schema fetch). Developer Sites can save and GET-roundtrip `sourceKind=json-schema` (local `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Build** writes HTML from the local fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**). REST **Preview** streams last-build local HTML (missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local JSON Schema Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch; missing assemble is **400** and does not invent pages). **`graphql-sdl`** is a local GraphQL SDL SPI (`schema.graphql` or `_config.yaml` `graphql.file`; no live GraphQL HTTP or introspection). SPI/CLI assemble is `PSVirtualSiteBuildMain … graphql-sdl` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `graphql-sdl` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). Developer Sites can save and GET-roundtrip `sourceKind=graphql-sdl` (local `rootPath` only), then **Build Virtual Site** and **Preview assembled site**. REST **Build** writes HTML from the local fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). REST **Preview** streams last-build local HTML (missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local GraphQL SDL Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP; missing assemble is **400** and does not invent pages). **`asyncapi-yaml`** is a local AsyncAPI 2/3 YAML SPI (`asyncapi.yaml` or `_config.yaml` `asyncapi.file`; no live HTTP spec fetch). SPI/CLI assemble is `PSVirtualSiteBuildMain … asyncapi-yaml` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `asyncapi-yaml` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch). Developer Sites can save and GET-roundtrip `sourceKind=asyncapi-yaml` (local `rootPath` only), then **Build Virtual Site** and **Preview assembled site**. REST **Build** writes HTML from the local fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). REST **Preview** streams last-build local HTML (missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local AsyncAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). **`openapi-yaml`** is a local OpenAPI 3 YAML SPI (`openapi.yaml` or `_config.yaml` `openapi.file`; no live HTTP spec fetch). SPI/CLI assemble is `PSVirtualSiteBuildMain … openapi-yaml` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `openapi-yaml` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch). Developer Sites can save and GET-roundtrip `sourceKind=openapi-yaml` (local `rootPath` only). REST **Build** (`POST …/virtual/build`) writes HTML from that local `openapi.yaml` / `openapi.file` fixture (`pagesWritten > 0`). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). Developer Sites can **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local OpenAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). **`llms-txt`** is a local `llms.txt` SPI (`llms.txt` or `_config.yaml` `llms.file`; no live HTTP fetch). SPI/CLI assemble is `PSVirtualSiteBuildMain … llms-txt` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `llms-txt` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch). Developer Sites can save and GET-roundtrip `sourceKind=llms-txt` (local `rootPath` only). REST **Build** (`POST …/virtual/build`) writes HTML from that local `llms.txt` / `llms.file` fixture (`pagesWritten > 0`). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). Developer Sites can **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** copies assembled HTML to `IPSSite.root` after a local llms.txt Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch; missing assemble is **400**). **`robots-txt`** is a local `robots.txt` SPI (`robots.txt` or `_config.yaml` `robots.file`; no live crawl). SPI/CLI assemble is `PSVirtualSiteBuildMain … robots-txt` (`pagesWritten > 0`). Leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are rejected. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `robots-txt` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). REST **Build** (`POST …/virtual/build`) writes HTML from that local `robots.txt` / `robots.file` fixture (`pagesWritten > 0`). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). Developer Sites can save and GET-roundtrip `sourceKind=robots-txt`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. **`sitemap-xml`** is a local `sitemap.xml` SPI (`sitemap.xml` or `_config.yaml` `sitemap.file`; no live crawl). SPI/CLI assemble is `PSVirtualSiteBuildMain … sitemap-xml`. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `sitemap-xml` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). REST **Build** (`POST …/virtual/build`) writes HTML from that local `sitemap.xml` / `sitemap.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live crawl). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200; leftover `virtual.remoteUrl` and credentials are **400**; no live crawl). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local sitemap.xml Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can save and GET-roundtrip `sourceKind=sitemap-xml` (local `rootPath` only; leftover `virtual.remoteUrl` and crawl credentials are never sent), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** (last-build local HTML; missing build stays unavailable). **`icalendar`** is a local RFC 5545 `.ics` SPI (`calendar.ics` or `_config.yaml` `icalendar.file`; no CalDAV). SPI/CLI assemble is `PSVirtualSiteBuildMain … icalendar`. REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `icalendar` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no CalDAV). REST **Build** (`POST …/virtual/build`) runs **`icalendar`** against that local `calendar.ics` / `icalendar.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**). REST **Preview** streams last-build HTML after Build (`available=true`; missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root`. Developer Sites can save and GET-roundtrip `sourceKind=icalendar` and then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. Blank or `repository` ⇒ traditional repository Site. Unknown values are rejected. CMS **Build** REST (`POST …/virtual/build`) runs git, CSV, SQL (H2), HTTP JSON (local JSON fixture or loopback catalog), **object-storage** (local object-key bucket; `virtual.remoteUrl` is **400**), **`rss-atom`** (local RSS 2.0 / Atom fixture or loopback `rss.url`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**), and **`icalendar`** (local RFC 5545 `calendar.ics` / `icalendar.file`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no CalDAV). Preview REST streams last-build HTML for git, CSV, SQL, HTTP JSON, `object-storage`, **`rss-atom`**, and **`icalendar`**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to the Site filesystem root for git, CSV, SQL, HTTP JSON, `object-storage` (local object-key `rootPath`; leftover `virtual.remoteUrl` is **400**), **`rss-atom`** (local RSS/Atom fixture; leftover `virtual.remoteUrl` and credentials are **400**; no live feeds), and **`icalendar`** (local `.ics` fixture; leftover `virtual.remoteUrl` and credentials are **400**; no CalDAV). Developer Sites can save and build Git, CSV, SQL, HTTP JSON, and object-storage, then **Preview assembled site** and **Publish Virtual Site**. Developer Sites can also save and GET-roundtrip `object-storage`, then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **GET/PUT** `/sites/{nameOrId}/virtual` also round-trips `http-json` (safe `rootPath` JSON fixture; `virtual.remoteUrl` is **400`), `object-storage` (portable-safe local `rootPath`; cloud URLs and credential properties are **400**; `virtual.remoteUrl` is **400**), and `rss-atom` (portable-safe local `rootPath`; leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; local/loopback only, no live feed credentials). Developer Sites can **Build Virtual Site** for `rss-atom` after save (local fixture only), **Preview assembled site**, and **Publish Virtual Site**. SPI/CLI assemble for `object-storage` is `PSVirtualSiteBuildMain … object-storage`. SPI/CLI assemble for `rss-atom` is `PSVirtualSiteBuildMain … rss-atom`. |
...
GET-roundtrip `sourceKind=robots-txt` (local `rootPath` only; leftover
`virtual.remoteUrl` and crawl credentials are never sent), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. Operators can also assemble
...
for **Preview assembled site** and **Publish Virtual Site**.
...
no fake preview). **Publish Virtual Site** copies last-build HTML to `IPSSite.root`.

### REST Publish for robots.txt (`robots-txt`)

`POST /sites/{nameOrId}/virtual/publish` builds the local `robots.txt` fixture then NIO-copies
assembled HTML to `IPSSite.root`. HTTP **200** returns `filesCopied > 0` and assembled HTML
under the Site filesystem root (for example `8.2/star-1.html` for a single `User-agent: *`
group). Staging `_meta` is not copied. Leftover `virtual.remoteUrl`, credential properties,
cloud URL `rootPath`, an unsafe Site root (`..` after NIO normalize), and a missing fixture
are **400** (do not invent pages). No live crawl. Git, CSV, SQL, HTTP JSON, object-storage,
rss-atom, icalendar, and sitemap-xml Publish paths are unchanged. Developer Sites
**Publish Virtual Site** copies last-build local HTML to `IPSSite.root` after a local
robots.txt Build (leftover `virtual.remoteUrl` and credentials fail closed).
...
fetch, no authenticated remotes, no cloud llms URLs. REST **GET/PUT** round-trips
`sourceKind=llms-txt` with a portable-safe local `rootPath`. REST **Build**, **Preview**,
and **Publish** run against that local fixture. Developer Sites **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site** are shown after save.
Operators can also assemble offline with the CLI.
...
### REST persist for llms.txt (`llms-txt`)

REST `PUT` / `GET /sites/{nameOrId}/virtual` round-trips `sourceKind=llms-txt` with a
portable-safe local `rootPath`. Leftover `virtual.remoteUrl`, credential properties, and
cloud URL `rootPath` are **400**. This is a **local llms.txt fixture** only — no live
HTTP fetch. Developer Sites can save and GET-roundtrip `sourceKind=llms-txt` (local fixture
`rootPath` only); leftover fetch URLs and credentials are never sent on the REST envelope.
**Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** run after
save.

### REST Build for llms.txt (`llms-txt`)

`POST /sites/{nameOrId}/virtual/build` runs `sourceKind=llms-txt` against a **local
llms.txt fixture** under `virtual.rootPath` (`llms.txt` or `_config.yaml`
`llms.file`). A successful assemble returns HTTP **200** with `pagesWritten > 0`.
Missing fixture, unsafe `rootPath` (`..` after NIO normalize), leftover
`virtual.remoteUrl`, credential properties, and cloud `rootPath` URLs are **400**. No live
HTTP fetch and no secrets on the REST envelope. Each Build re-reads the current `llms.txt`
and `_config.yaml` `llms.file` (no JVM restart; no Jetty restart; no file watchers).
Developer Sites **Build Virtual Site** is shown so operators can produce last-build HTML
for **Preview assembled site** and **Publish Virtual Site**.

### REST Preview for llms.txt (`llms-txt`)

After a successful REST or CLI assemble at the default output root, `GET
/sites/{nameOrId}/virtual/preview` reports `available=true` plus `homePath` (typically
`8.2/Quickstart-1.html` for a single markdown list link — Preview uses the sole assembled
HTML page when `index.html` is absent). `GET …/virtual/preview/{relPath}` streams the
assembled HTML. Missing build is `available=false` with HTTP **200** (not 500). Unknown
`sourceKind` remains **400**. Paths use portable NIO `Path` under the last output root
(no remaining `..`). `../` traversal is **400**. Preview is **last-build local HTML
only** (no live HTTP fetch). Leftover `virtual.remoteUrl` and credential properties are
**400**. Developer Sites **Preview assembled site** uses that last-build Preview after
**Build Virtual Site**. Missing build stays unavailable (`available=false` HTTP **200**;
no fake preview). **Publish Virtual Site** copies last-build HTML to `IPSSite.root`.

### REST Publish for llms.txt (`llms-txt`)

`POST /sites/{nameOrId}/virtual/publish` builds the local `llms.txt` fixture then NIO-copies
assembled HTML to `IPSSite.root`. HTTP **200** returns `filesCopied > 0` and assembled HTML
under the Site filesystem root (for example `8.2/Quickstart-1.html` for a single markdown
list link). Staging `_meta` is not copied. Leftover `virtual.remoteUrl`, credential
properties, cloud URL `rootPath`, an unsafe Site root (`..` after NIO normalize), and a
missing fixture are **400** (do not invent pages). No live HTTP fetch. Git, CSV, SQL, HTTP
JSON, object-storage, rss-atom, icalendar, sitemap-xml, and robots-txt Publish paths are
unchanged. Developer Sites **Publish Virtual Site** copies last-build local HTML to
`IPSSite.root` after a local llms.txt Build (leftover `virtual.remoteUrl` and credentials
fail closed).
...
**GET/PUT** persist and Developer Sites save/GET-roundtrip are available. REST
**Build**, **Preview**, and **Publish** run against that local fixture. Operators can
also assemble offline with the CLI.
...
### Offline build from AsyncAPI 2/3 YAML (`asyncapi-yaml`)

The `asyncapi-yaml` adapter discovers pages from a **local AsyncAPI 2 or 3 YAML fixture**.
`_config.yaml` is **required** (versions / site title). Git remotes are not used
(`virtual.remoteUrl` is rejected). This is a **local AsyncAPI adapter** — no live HTTP
spec fetch, no remote AsyncAPI URLs, no authenticated remotes, no cloud spec URLs. REST
`GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=asyncapi-yaml` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL
`rootPath` are **400**). Developer Sites can save and GET-roundtrip `sourceKind=asyncapi-yaml`
(local fixture `rootPath` only). REST **Build** (`POST …/virtual/build`) writes HTML from that local fixture (`pagesWritten > 0`). REST **Preview** (`GET …/virtual/preview`) streams last-build HTML (missing build is `available=false` HTTP **200**). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local AsyncAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). Operators
can also assemble offline with the CLI.

| `_config.yaml` key | Meaning |
|--------------------|---------|
| `asyncapi.file` | Portable NIO path under the site root (for example `asyncapi.yaml`). Must be relative (no remaining `..`, no Windows/Unix absolute roots). |
| *(omitted)* | Default local file `asyncapi.yaml` under `virtual.rootPath`. |
| `asyncapi.url` | **Rejected.** Live remote AsyncAPI fetches are out of scope. |

AsyncAPI YAML files larger than 2 MB fail closed. Each discover/load re-reads the current
file (no process-lifetime cache of parsed operations). After you edit `asyncapi.yaml` or
`_config.yaml` `asyncapi.file` on the CMS host, run `PSVirtualSiteBuildMain … asyncapi-yaml`
again — **no JVM restart**. File watchers are not used; the next explicit build is the
refresh.

Channel/operation mapping:

| AsyncAPI field | Assemble field |
|----------------|----------------|
| `operationId` (or operations key, or `ACTION` + channel) | required `id` (slug plus operation order) |
| `summary` (else `operationId`, else `ACTION channel`) | `title` |
| action, channel, description | Markdown in `body` |

AsyncAPI 3 top-level `operations` take precedence. When that mapping is omitted, AsyncAPI 2
channel `publish` / `subscribe` (and AsyncAPI 3 channel `send` / `receive`) become pages.
A fixture with no channels or operations still emits one page from `info` so CLI assemble
writes HTML (`pagesWritten > 0`).

Remote `$ref` values (`http(s)://`, `s3://`, or other remote/cloud schemes), leftover
`virtual.remoteUrl`, credential properties, empty files, `asyncapi.url`, documents that are
not AsyncAPI 2 or 3, and unsafe `asyncapi.file` paths fail closed (`VirtualSiteException`).
...
```text
PSVirtualSiteBuildMain <siteRoot> <outputRoot> [siteKey] asyncapi-yaml
```

Site property validation allow-lists `asyncapi-yaml` (same helper as Git/CSV/SQL/HTTP JSON /
object-storage / rss-atom / icalendar / sitemap-xml / robots-txt / llms-txt / openapi-yaml).
Unknown kinds remain rejected.

### Offline build from GraphQL SDL (`graphql-sdl`)

The `graphql-sdl` adapter discovers pages from a **local GraphQL SDL fixture**.
Default file: `schema.graphql`. Override with `_config.yaml` `graphql.file`.
`graphql.url` is **rejected** (no live GraphQL HTTP or introspection).

REST `GET` / `PUT /sites/{nameOrId}/virtual` round-trips `sourceKind=graphql-sdl` with a
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL
`rootPath`, and `graphql.url` are **400**). Developer Sites can save and GET-roundtrip
`sourceKind=graphql-sdl` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Build** writes HTML from the local fixture
(`pagesWritten > 0`). REST **Preview** streams last-build local HTML (missing build is
`available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; missing assemble is **400** and does not invent pages).

Place a local `schema.graphql` (or `_config.yaml` `graphql.file`) on the CMS host, then
run `PSVirtualSiteBuildMain … graphql-sdl` (`pagesWritten > 0` from a temp fixture).

```
PSVirtualSiteBuildMain <siteRoot> <outputRoot> [siteKey] graphql-sdl
```

Site property validation allow-lists `graphql-sdl` (same helper as Git/CSV/SQL/HTTP JSON /
object-storage / rss-atom / icalendar / sitemap-xml / robots-txt / llms-txt / openapi-yaml /
asyncapi-yaml). Unknown kinds remain rejected.

### Offline build from JSON Schema (`json-schema`)

The `json-schema` adapter discovers pages from a **local JSON Schema fixture**.
Default file: `schema.json`. Override with `_config.yaml` `jsonschema.file`.
`jsonschema.url` is **rejected** (no live HTTP schema fetch). Remote `$ref` and
`$id` HTTP values are rejected. Leftover `virtual.remoteUrl`, credentials, and
cloud URL `rootPath` are rejected.

Place a local `schema.json` (or `_config.yaml` `jsonschema.file`) on the CMS host, then
run `PSVirtualSiteBuildMain … json-schema` (`pagesWritten > 0` from a temp fixture).

```
PSVirtualSiteBuildMain <siteRoot> <outputRoot> [siteKey] json-schema
```

Site property validation allow-lists `json-schema` (same helper as Git/CSV/SQL/HTTP JSON /
object-storage / rss-atom / icalendar / sitemap-xml / robots-txt / llms-txt / openapi-yaml /
asyncapi-yaml / graphql-sdl). Unknown kinds remain rejected. REST persist and Developer Sites
source chrome round-trip `sourceKind=json-schema` (local `rootPath` only), then **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. REST **Build** writes HTML from the local fixture
(`pagesWritten > 0`). REST **Preview** streams last-build local HTML (missing build is
`available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; missing assemble is **400** and does not invent pages).

### REST persist for JSON Schema (`json-schema`)

REST `PUT` / `GET /sites/{nameOrId}/virtual` round-trips `sourceKind=json-schema` with a
portable-safe local `rootPath`. Leftover `virtual.remoteUrl`, credential properties, cloud
URL `rootPath`, and `jsonschema.url` are **400**. This is a **local JSON Schema fixture** only —
no live HTTP schema fetch. Developer Sites can save and GET-roundtrip
`sourceKind=json-schema` (local fixture `rootPath` only); leftover schema URLs and
credentials are never sent on the REST envelope. **Build Virtual Site**, **Preview
assembled site**, and **Publish Virtual Site** run after save. REST **Publish** copies last-build HTML to `IPSSite.root` after a local JSON Schema Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; missing assemble is **400** and does not invent pages).

### REST persist for GraphQL SDL (`graphql-sdl`)

REST `PUT` / `GET /sites/{nameOrId}/virtual` round-trips `sourceKind=graphql-sdl` with a
portable-safe local `rootPath`. Leftover `virtual.remoteUrl`, credential properties, cloud
URL `rootPath`, and `graphql.url` are **400**. This is a **local GraphQL SDL fixture** only —
no live GraphQL HTTP or introspection. Developer Sites can save and GET-roundtrip
`sourceKind=graphql-sdl` (local fixture `rootPath` only); leftover GraphQL URLs and
credentials are never sent on the REST envelope. **Build Virtual Site**, **Preview
assembled site**, and **Publish Virtual Site** run after save. REST **Publish** copies last-build HTML to `IPSSite.root` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; missing assemble is **400**).

### REST persist for AsyncAPI 2/3 YAML (`asyncapi-yaml`)

REST `PUT` / `GET /sites/{nameOrId}/virtual` round-trips `sourceKind=asyncapi-yaml` with a
portable-safe local `rootPath`. Leftover `virtual.remoteUrl`, credential properties, and
cloud URL `rootPath` are **400**. This is a **local AsyncAPI 2/3 YAML fixture** only — no live
spec fetch. Developer Sites can save and GET-roundtrip `sourceKind=asyncapi-yaml` (local
fixture `rootPath` only); leftover spec URLs and credentials are never sent on the REST
envelope. **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** run after save.

### REST Build for AsyncAPI 2/3 YAML (`asyncapi-yaml`)

`POST /sites/{nameOrId}/virtual/build` runs `sourceKind=asyncapi-yaml` against a **local
AsyncAPI 2/3 YAML fixture** under `virtual.rootPath` (`asyncapi.yaml` or `_config.yaml`
`asyncapi.file`). A successful assemble returns HTTP **200** with `pagesWritten > 0`.
Missing fixture, unsafe `rootPath` (`..` after NIO normalize), leftover
`virtual.remoteUrl`, credential properties, and cloud `rootPath` URLs are **400**. No live
spec fetch and no secrets on the REST envelope. Each Build re-reads the current
`asyncapi.yaml` and `_config.yaml` `asyncapi.file` (no JVM restart; no Jetty restart; no
file watchers). Developer Sites **Build Virtual Site** is shown so operators can produce
last-build HTML for **Preview assembled site**. Developer Sites **Publish Virtual Site** copies last-build local HTML to `IPSSite.root` after a local AsyncAPI YAML Build.

### REST Preview for AsyncAPI 2/3 YAML (`asyncapi-yaml`)

After a successful REST or CLI assemble at the default output root, `GET
/sites/{nameOrId}/virtual/preview` reports `available=true` plus `homePath` (typically
`8.2/onLightMeasured-1.html` for a single PUBLISH operation — Preview uses the sole assembled HTML
page when `index.html` is absent). `GET …/virtual/preview/{relPath}` streams the assembled
HTML. Missing build is `available=false` with HTTP **200** (not 500). Unknown
`sourceKind` remains **400**. Paths use portable NIO `Path` under the last output root
(no remaining `..`). `../` traversal is **400**. Preview is **last-build local HTML
only** (no live spec fetch). Leftover `virtual.remoteUrl` and credential properties are
**400**. Developer Sites **Preview assembled site** uses that last-build Preview after
**Build Virtual Site**. Missing build stays unavailable (`available=false` HTTP **200**;
no fake preview). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root`.

### REST Publish for AsyncAPI 2/3 YAML (`asyncapi-yaml`)

`POST /sites/{nameOrId}/virtual/publish` builds the local AsyncAPI 2/3 YAML fixture then
NIO-copies assembled HTML to `IPSSite.root`. HTTP **200** returns `filesCopied > 0` and
assembled HTML under the Site filesystem root (for example `8.2/onLightMeasured-1.html` for a
single PUBLISH operation). Staging `_meta` is not copied. Leftover `virtual.remoteUrl`,
credential properties, cloud URL `rootPath`, an unsafe Site root (`..` after NIO
normalize), and a missing fixture are **400** (do not invent pages). No live spec fetch.
Git, CSV, SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt,
llms-txt, and openapi-yaml Publish paths are unchanged. Developer Sites **Publish Virtual Site** copies
last-build local HTML to `IPSSite.root` after a local AsyncAPI YAML Build (leftover
`virtual.remoteUrl` and credentials fail closed).

### REST persist for OpenAPI 3 YAML (`openapi-yaml`)

REST `PUT` / `GET /sites/{nameOrId}/virtual` round-trips `sourceKind=openapi-yaml` with a
portable-safe local `rootPath`. Leftover `virtual.remoteUrl`, credential properties, and
cloud URL `rootPath` are **400**. This is a **local OpenAPI 3 YAML fixture** only — no live
spec fetch. Developer Sites can save and GET-roundtrip `sourceKind=openapi-yaml` (local
fixture `rootPath` only); leftover spec URLs and credentials are never sent on the REST
envelope. **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** run after save.

### REST Build for OpenAPI 3 YAML (`openapi-yaml`)

`POST /sites/{nameOrId}/virtual/build` runs `sourceKind=openapi-yaml` against a **local
OpenAPI 3 YAML fixture** under `virtual.rootPath` (`openapi.yaml` or `_config.yaml`
`openapi.file`). A successful assemble returns HTTP **200** with `pagesWritten > 0`.
Missing fixture, unsafe `rootPath` (`..` after NIO normalize), leftover
`virtual.remoteUrl`, credential properties, and cloud `rootPath` URLs are **400**. No live
spec fetch and no secrets on the REST envelope. Each Build re-reads the current
`openapi.yaml` and `_config.yaml` `openapi.file` (no JVM restart; no Jetty restart; no
file watchers). Developer Sites **Build Virtual Site** is shown so operators can produce
last-build HTML for **Preview assembled site** and **Publish Virtual Site**.

### REST Preview for OpenAPI 3 YAML (`openapi-yaml`)

After a successful REST or CLI assemble at the default output root, `GET
/sites/{nameOrId}/virtual/preview` reports `available=true` plus `homePath` (typically
`8.2/listPets-1.html` for a single GET operation — Preview uses the sole assembled HTML
page when `index.html` is absent). `GET …/virtual/preview/{relPath}` streams the assembled
HTML. Missing build is `available=false` with HTTP **200** (not 500). Unknown
`sourceKind` remains **400**. Paths use portable NIO `Path` under the last output root
(no remaining `..`). `../` traversal is **400**. Preview is **last-build local HTML
...
### REST Publish for OpenAPI 3 YAML (`openapi-yaml`)

`POST /sites/{nameOrId}/virtual/publish` builds the local OpenAPI 3 YAML fixture then
NIO-copies assembled HTML to `IPSSite.root`. HTTP **200** returns `filesCopied > 0` and
assembled HTML under the Site filesystem root (for example `8.2/listPets-1.html` for a
single GET operation). Staging `_meta` is not copied. Leftover `virtual.remoteUrl`,
credential properties, cloud URL `rootPath`, an unsafe Site root (`..` after NIO
normalize), and a missing fixture are **400** (do not invent pages). No live spec fetch.
Git, CSV, SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt,
and llms-txt Publish paths are unchanged. Developer Sites **Publish Virtual Site** copies
last-build local HTML to `IPSSite.root` after a local OpenAPI YAML Build (leftover
`virtual.remoteUrl` and credentials fail closed).
...
`sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, or `asyncapi-yaml`), an **Admin** can trigger the
...
**400**; no live crawl). `robots-txt` discovers pages from a local robots.txt fixture (`robots.txt` or `robots.file`; `pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live crawl). `llms-txt` discovers pages from a local llms.txt fixture (`llms.txt` or `llms.file`; `pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live HTTP fetch). `openapi-yaml` discovers pages from a local OpenAPI 3 YAML fixture (`openapi.yaml` or `openapi.file`; `pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). `asyncapi-yaml` discovers pages from a local AsyncAPI 2/3 YAML fixture (`asyncapi.yaml` or `asyncapi.file`; `pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). Unknown `sourceKind` values stay
...
`http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt`. `POST …/virtual/publish` runs for
`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`,
`rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` (copies assembled HTML to `IPSSite.root`; leftover `virtual.remoteUrl` on
`http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` is **400**; leftover credentials on
`rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` are **400**; leftover cloud URL `rootPath` on `sitemap-xml`, `robots-txt`, and `llms-txt` is **400**; no live crawl or HTTP fetch).
...
2. Run **Build Virtual Site** again (UI, `POST …/virtual/build`, `scripts/build-cms-docs.*`,
   ...
   `PSVirtualSiteBuildMain … llms-txt`, `PSVirtualSiteBuildMain … openapi-yaml`,
   `PSVirtualSiteBuildMain … asyncapi-yaml`, `PSVirtualSiteBuildMain … graphql-sdl`, or
   `PSVirtualSiteBuildMain … json-schema`).
...
object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, asyncapi-yaml, graphql-sdl, and json-schema after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml/asyncapi-yaml/graphql-sdl/json-schema).
