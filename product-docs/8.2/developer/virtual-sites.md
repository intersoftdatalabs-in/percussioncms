...
GET-roundtrip `sourceKind=openapi-yaml` (local `rootPath` only). REST **Build**,
**Preview**, and **Publish** stay later slices.
...
Build, Preview, and Publish stay later slices. Operators can also assemble offline
with the CLI.
...
envelope. **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**
stay later slices.
...
`sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, or `llms-txt`), an **Admin** can trigger the
...
object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, and llms-txt after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt).
