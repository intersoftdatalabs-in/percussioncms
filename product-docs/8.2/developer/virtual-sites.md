...
credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can
save and GET-roundtrip `sourceKind=sitemap-xml` (local fixture `rootPath` only). REST
**Build**, **Preview**, **Publish**, and Build/Preview/Publish chrome stay later slices.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **local `sitemap.xml` SPI** (CLI assemble from `sitemap.xml` / `sitemap.file`; no live
  ...
  `sourceKind=sitemap-xml` (local fixture `rootPath` only). REST **Build**, **Preview**,
  **Publish**, and Build/Preview/Publish chrome stay later slices.
...
can save and GET-roundtrip `sourceKind=sitemap-xml` (local fixture `rootPath` only). REST
**Build**, **Preview**, **Publish**, and Build/Preview/Publish chrome stay later slices.
