...
and **Preview assembled site**. REST **Build** (`POST …/virtual/build`) writes HTML from that
...
(`available=true`; missing build is `available=false` HTTP 200). **Publish Virtual Site** stays a later leftover.
...
and **Preview assembled site**. REST **Build** writes HTML from the local fixture
(`pagesWritten > 0`). REST **Preview** streams last-build local HTML (missing build is
`available=false` HTTP 200). REST **Publish** stays a later leftover.
...
credentials are never sent on the REST envelope. **Build Virtual Site** and **Preview
assembled site** run after save. REST **Publish** stays a later leftover.
