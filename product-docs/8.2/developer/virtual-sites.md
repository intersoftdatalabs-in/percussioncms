...
`virtual.remoteUrl` is **400**. Developer Sites chrome and REST **Preview** /
**Publish** for this kind stay a later phase.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **Build** (`POST …/virtual/build`) writes HTML from that local bucket.
...
`http-json` kinds are unchanged. REST **Build** (`POST …/virtual/build`) is available for
`object-storage`. REST **Preview** / **Publish** and Developer Sites chrome for this kind
stay a later phase.
