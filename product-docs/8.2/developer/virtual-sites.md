...
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL
`rootPath` are **400**; no live spec fetch). Developer Sites can save and
GET-roundtrip `sourceKind=asyncapi-yaml` (local `rootPath` only). **Build Virtual Site**,
...
(local fixture `rootPath` only). REST Build, Preview, and Publish stay later slices. Operators
...
envelope. **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** stay
later slices.
...
`sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, or `openapi-yaml`), an **Admin** can trigger the
...
object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, and openapi-yaml after save (Publish copies last-build HTML after a successful Build for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml).
