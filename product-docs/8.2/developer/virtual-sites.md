...
`available=false` HTTP 200). Developer **Sites** can **Build Virtual Site** after save
(local fixture only) and **Preview assembled site**. REST Publish and Developer Sites
Publish chrome for this kind stay later slices. No live remote feeds.
...
- Leave the door open for additional adapters (object storage) without renaming Site → Channel.
  ...
  **GET/PUT** round-trips `object-storage` (safe local `rootPath`; no cloud URLs or
  ...
  `available=false` HTTP 200). Developer Sites can **Build Virtual Site** for `rss-atom`
  after save (local fixture only) and **Preview assembled site**. REST Publish and
  Developer Sites Publish chrome for `rss-atom` stay later slices.
...
Virtual Site**. REST Publish for `rss-atom` stays a later slice.
...
**Build** and **Preview** are available for `rss-atom`. Developer Sites can **Build Virtual
Site** then **Preview assembled site**. REST **Publish** and Developer Sites Publish chrome
stay later slices.
