...
- **Publish** (`POST /sites/{nameOrId}/virtual/publish`) runs that build, then copies the
  ...
  bucket. REST Preview/Publish and Developer Sites chrome for that kind stay a later
  phase. The endpoint does not accept an `outputRoot` body (always the default staging
  root).
