...
| **Purge** | Confirm, then permanently purge a **page** or **asset** (`DELETE` `pagemanagement/page/purge/{id}` or `assetmanagement/asset/purge/{id}`). Other types stay unavailable. Distinct from **Delete** (remove from folder / recycle). **Multi-select:** **Content → Purge selected** asks once, then purges each checked page or asset with that same contract. Folders are named and skipped. A **403**, **404**, or **409** on one item does not report the whole selection as purged. |
...
### Purge selected pages and assets

**Content → Purge selected** is enabled after you check one or more rows in
the detail list (including the Recycling bin). One confirm permanently purges
each checked **page** or **asset** with the same REST as single-item **Purge**:
`DELETE /services/pagemanagement/page/purge/{id}` or
`DELETE /services/assetmanagement/asset/purge/{id}`. Checked **folders** are
not purged; Explorer names them in the result (for example `Skipped folders: News`).

**Cancel** purges nothing. Items that already purged stay gone when a later
item fails. The result is full success only when every page or asset purged.
A partial HTTP failure (**403**, **404**, or **409** on one item) stays on the
result as an alert and is not reported as full success. A selection that is
only folders purges nothing. This is not **Empty recycle bin** and not
**Recycle selected**.
