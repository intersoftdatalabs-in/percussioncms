...
| **Copy URL to Clipboard** | Copies the site-path preview URL (or CMS path) for the selected item. **Multi-select:** check two or more rows, then **Copy URL to Clipboard**. One action writes every selected page and asset URL to the clipboard, one URL per line. Folders are not copied; the **Server actions** error region names them (**Folders are not copied**). An item with no URL is named (**No URL is available for**) and that batch is not full success — URLs that could be resolved are still copied. A single highlighted row (fewer than two checkboxes) still copies only that row. |
| **Copy folder path** | **Content → Copy folder path** copies the CMS folder path of the selected folder, or the folder that contains the selected page or asset, to the clipboard. Confirm **Copy this folder path to the clipboard?** The status line then shows **Folder path copied to the clipboard** and the path. **Cancel** copies nothing and does not change the selection. A root or blank path is **No folder path is available to copy** — the clipboard is not cleared with a blank success. If the browser refuses the clipboard, Explorer shows **Could not copy the folder path** and leaves the selection unchanged. This is not **Copy URL to Clipboard**, **Copy** (duplicate an item or folder), or **Site Copy**. |
...
| **Checked out by** | When a **page** or **asset** row is selected, Explorer shows who holds the checkout (`GET /rest/editor/items/{id}/checkout-owner`). The label is the user name. **Not checked out** means nobody holds the lock. Another user's lock is shown; the lookup does not check the item out and does not throw. HTTP **403** stays on that line (**You are not allowed to see who has this item checked out**). **Folders** do not show a checkout owner. |
