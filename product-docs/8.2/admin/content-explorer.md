...
| Action | What happens |
|--------|----------------|
| **Preview** (and per-template children) | Select a **page** or **asset** row first (folder rows stay Preview-disabled). Listed pages under **Sites** open the assembled Finder site-path preview (`/Sites/…?percmobilepreview=`). Pages that have a content id but no Sites path open Page Management render (`GET /services/pagemanagement/render/page/{id}`). Preview does **not** open the React Content Editor (`spa.jsp?entry=editor` / `/cm/app/editor`). Assets open the asset view URL. Per-template children still use assembly preview (`GET /services/assembly/preview-location`). Applies to listed pages and assets, including customer-defined content types (type names are not a closed list; FastForward names stay stable). New language: **template**, not variant. |
