...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, and **Sitemap XML** (**Build Virtual Site**
   only for Sitemap XML; Preview/Publish stay later). Traditional **Repository** hides that chrome.
...
portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL
`rootPath` are **400**). REST **Build** (`POST …/virtual/build`) writes HTML from that local
...
live crawl). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local sitemap.xml Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can save **Sitemap XML** (`sourceKind=sitemap-xml`) and then **Build Virtual Site** (local `sitemap.xml` / `sitemap.file` only; leftover `virtual.remoteUrl` and crawl credentials are **400**; Preview/Publish chrome stay later). REST **Build**, **Preview**, and **Publish** run for `icalendar`
...
**Sitemap XML** (`sourceKind=sitemap-xml`) and then **Build Virtual Site** (local fixture
`rootPath` only; leftover `virtual.remoteUrl` and crawl credentials are **400**;
**Preview assembled site** and **Publish Virtual Site** stay later). After root, remote, or
...
**HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, or **Sitemap XML**, the Site detail panel shows **Build Virtual Site** after save.
**Preview assembled site** and **Publish Virtual Site** appear for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, and iCalendar (not Sitemap XML yet). Traditional
...
no CalDAV). After you save **Sitemap XML**, Build runs against a local `sitemap.xml`
fixture (`sitemap.xml` or `_config.yaml` `sitemap.file`; no live crawl). After that Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar Build succeeds,
...
5. After a `git pull` or a local Markdown/frontmatter edit on the host — or after the remote
   ...
   `_config.yaml` change — or after a sitemap.xml fixture (`sitemap.xml` / `sitemap.file`) or
