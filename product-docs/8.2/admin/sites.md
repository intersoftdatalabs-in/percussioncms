...
(`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`), required local root path when virtual and no remote,
safe remote URLs/branches for Git only (`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, and `graphql-sdl` reject `virtual.remoteUrl`), and
