...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, **Sitemap XML**, and **Robots.txt**. Traditional **Repository** hides that chrome.
...
and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Build** (`POST …/virtual/build`) writes HTML from that local
