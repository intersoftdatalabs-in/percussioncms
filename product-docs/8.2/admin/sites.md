...
`object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, and `asyncapi-yaml` (`POST …/virtual/build`). A second sitemap-xml Build after an in-process `sitemap.xml` / page edit returns `pagesWritten > 0` HTML that matches the current files (no Jetty restart). CSV assemble does not require a Git remote;
`_config.yaml` is optional for CSV and required for SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, and asyncapi-yaml.
