...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **SQL database**, and **HTTP JSON**. **Publish Virtual Site** appears for Git,
   CSV, and SQL. Traditional **Repository** hides that chrome. **HTTP JSON**
   hides Publish in this release (that remains a later slice). After Build,
   **Preview assembled site** opens last-build home HTML for HTTP JSON the same
   way as Git/CSV/SQL. REST **Publish** (`POST …/virtual/publish`) copies HTTP
   JSON assembled files to the Site filesystem root (`IPSSite.root`).
...
After a successful Build, **Preview assembled site** opens last-build home HTML.
Publish chrome for that kind is not on the panel yet.

### Build a Virtual Site from the product UI

When **Source kind** is **Git filesystem**, **CSV filesystem**, **SQL database**, or
**HTTP JSON**, the Site detail panel shows **Build Virtual Site** and **Preview
assembled site** after save. **Publish Virtual Site** appears for Git, CSV, and SQL
(not HTTP JSON in this release). Traditional **Repository** Sites do **not** show
these controls (no misleading virtual-build or virtual-publish chrome). After you
save **SQL database**, **Build Virtual Site** runs `POST /services/sites/{name}/virtual/build`
against in-memory H2 (`jdbc:h2:mem:` in `_config.yaml` — no Oracle/MySQL live matrix
on this path). After you save **HTTP JSON**, the same Build action runs against a
local JSON catalog (`http.file` or default `pages.json`) or loopback `http.url` in
`_config.yaml` (no Authorization or API keys on the REST envelope). After that
Build succeeds, **Preview assembled site** streams last-build home HTML.
