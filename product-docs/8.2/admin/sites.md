...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   (no `remoteUrl`, no live HTTP fetch URLs or credentials). After a successful save the panel reloads properties from GET so the kind
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, **Sitemap XML**, and **Robots.txt**. **llms.txt** save/GET-roundtrip is available; **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** stay hidden for that kind. Traditional **Repository** hides that chrome.
...
(`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`), required local root path when virtual and no remote,
safe remote URLs/branches for Git only (`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, and `llms-txt` reject `virtual.remoteUrl`), and
...
and GET-roundtrip the kind (local fixture `rootPath` only); **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site** stay later slices. REST **Build** (`POST …/virtual/build`) writes HTML from that local
...
`object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` (`POST …/virtual/build`). A second sitemap-xml Build after an in-process `sitemap.xml` / page edit returns `pagesWritten > 0` HTML that matches the current files (no Jetty restart). CSV assemble does not require a Git remote;
`_config.yaml` is optional for CSV and required for SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, and robots-txt.
...
**HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, or **Sitemap XML**, the Site detail panel shows **Build Virtual Site**,
...
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt.
...
**object-storage**, **`rss-atom`**, **`icalendar`**, **`sitemap-xml`**, and **`robots-txt`** (`sql-database` / `http-json` / `object-storage` /
`rss-atom` / `icalendar` / `sitemap-xml` / `robots-txt` REST last-build) — preview is not git-only. Traditional **Repository** Sites hide **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. Developer Sites **Preview
assembled site** appears for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, Sitemap XML, and Robots.txt after a successful
