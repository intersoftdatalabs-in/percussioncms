...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **SQL database**, **HTTP JSON**, and **Object storage**. **Publish Virtual Site**
   ...
   chrome. After you save **Object storage**, **Build Virtual Site** runs against the
   local object-key directory (`virtual.rootPath`); **Preview assembled site** opens
   last-build home HTML the same way as Git/CSV/SQL/HTTP JSON. Publish chrome stays hidden.
   **Publish Virtual Site** (and REST `POST …/virtual/publish`) copies HTTP JSON
   assembled files to the Site filesystem root (`IPSSite.root`). Integrators can
   REST-publish **object-storage** the same way; Developer Sites **Publish Virtual
   Site** chrome for that kind stays a later phase.
...
Developer Sites. REST **Publish** (`POST …/virtual/publish`) still copies assembled HTML
onto the Site filesystem root for a local object-key `rootPath` (`virtual.remoteUrl` is
**400**). After a successful Build, **Preview assembled site** opens last-build home HTML,
and **Publish Virtual Site** copies assembled HTML onto the Site filesystem root for Git,
...
JSON; no cloud URLs or access keys). After that Build succeeds, **Preview assembled
site** streams last-build home HTML. **Publish Virtual Site** copies that HTML to the
Site filesystem root for Git/CSV/SQL/HTTP JSON (object-storage Publish chrome later;
integrators can REST-publish).
