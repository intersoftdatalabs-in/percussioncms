...
(`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`), required local root path when virtual and no remote,
safe remote URLs/branches for Git only (`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, and `asyncapi-yaml` reject `virtual.remoteUrl`), and
...
and **Preview assembled site** after save. **Publish Virtual Site** is also shown for those kinds including **llms.txt** and **OpenAPI YAML**, except **AsyncAPI YAML** (Publish stays a later slice). Traditional
...
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml.
...
Build. **Publish Virtual Site** is shown for those same kinds including llms.txt and OpenAPI YAML, except AsyncAPI YAML. After REST or in-product Build for `sql-database` (in-memory)
