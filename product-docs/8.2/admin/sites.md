...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, and **Sitemap XML**. **Robots.txt** save
   chrome is shown; **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**
   stay hidden for that kind until later slices. Traditional **Repository** hides that chrome.
...
(`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`), required local root path when virtual and no remote,
safe remote URLs/branches for Git only (`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, and `robots-txt` reject `virtual.remoteUrl`), and
...
and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site** and **Preview assembled site** (Publish stays a later slice). REST **Build** (`POST …/virtual/build`) writes HTML from that local
   ...
   **Preview assembled site**:
   ...
   - **Robots.txt** — saved **Root path** to a local `robots.txt` fixture
     ...
     **Preview assembled site** opens last-build local HTML only (a single `User-agent`
     ...
     when `index.html` is absent). No live crawl. **Publish Virtual Site** stays a later
     slice.
...
3. Confirm **Source kind** is **Git filesystem**, **CSV filesystem**, **SQL database**,
   **HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, or **Sitemap XML** and **Save Virtual Site source** if you changed
