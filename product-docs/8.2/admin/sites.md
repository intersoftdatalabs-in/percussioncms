...
and **Preview assembled site** after save. **Publish Virtual Site** is also shown for those kinds including **llms.txt** (OpenAPI YAML Publish stays a later slice). Traditional
...
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt (openapi-yaml Publish stays a later slice).
...
Build. **Publish Virtual Site** is shown for those same kinds including llms.txt (OpenAPI YAML Publish stays a later slice). After REST or in-product Build for `sql-database` (in-memory)
