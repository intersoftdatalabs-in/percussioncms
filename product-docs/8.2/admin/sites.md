...
(`git-filesystem`, `csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, `json-schema`), required local root path when virtual and no remote,
safe remote URLs/branches for Git only (`csv-filesystem`, `sql-database`, `http-json`, `object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` reject `virtual.remoteUrl`), and
...
and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local OpenAPI YAML Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live spec fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `openapi.yaml` / `openapi.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). `asyncapi-yaml` is a local AsyncAPI 2/3 YAML fixture (`asyncapi.yaml` / `asyncapi.file`; no live spec fetch); REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `sourceKind=asyncapi-yaml` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**). Developer Sites can save **AsyncAPI YAML** (`sourceKind=asyncapi-yaml`) and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. `graphql-sdl` is a local GraphQL SDL fixture (`schema.graphql` / `graphql.file`; no live GraphQL HTTP or introspection); REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `sourceKind=graphql-sdl` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**). Developer Sites can save **GraphQL SDL** (`sourceKind=graphql-sdl`) and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. `json-schema` is a local JSON Schema fixture (`schema.json` / `jsonschema.file`; no live HTTP schema fetch); REST **GET/PUT** `/sites/{nameOrId}/virtual` round-trips `sourceKind=json-schema` with a portable-safe local `rootPath` (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `jsonschema.url` are **400**). Developer Sites can save **JSON Schema** (`sourceKind=json-schema`) and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site**, **Preview assembled site**, and **Publish Virtual Site**. REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local JSON Schema Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `schema.json` / `jsonschema.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, `jsonschema.url`, and remote `$ref`/`$id` HTTP are **400**; no live HTTP schema fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Build** (`POST …/virtual/build`) writes HTML from that local `schema.graphql` / `graphql.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, cloud `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local GraphQL SDL Build (leftover `virtual.remoteUrl`, credentials, cloud URL `rootPath`, and `graphql.url` are **400**; no live GraphQL HTTP; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `asyncapi.yaml` / `asyncapi.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live spec fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local llms.txt Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live HTTP fetch; missing assemble is **400** and does not invent pages). REST **Build** (`POST …/virtual/build`) writes HTML from that local `llms.txt` / `llms.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live HTTP fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Build** (`POST …/virtual/build`) also writes HTML from that local
...
`object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, `openapi-yaml`, `asyncapi-yaml`, `graphql-sdl`, and `json-schema` (`POST …/virtual/build`). A second sitemap-xml Build after an in-process `sitemap.xml` / page edit returns `pagesWritten > 0` HTML that matches the current files (no Jetty restart). CSV assemble does not require a Git remote;
`_config.yaml` is optional for CSV and required for SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, asyncapi-yaml, graphql-sdl, and json-schema.
...
**HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, **Sitemap XML**, **Robots.txt**, **llms.txt**, **OpenAPI YAML**, or **AsyncAPI YAML**, the Site detail panel shows **Build Virtual Site**
and **Preview assembled site** after save. **Publish Virtual Site** is also shown for those kinds including **llms.txt**, **OpenAPI YAML**, and **AsyncAPI YAML**. Traditional
...
(`llms.txt` or `_config.yaml` `llms.file`; no live HTTP fetch). After you save **OpenAPI YAML**,
Build runs against a local OpenAPI 3 YAML fixture (`openapi.yaml` or `_config.yaml`
`openapi.file`; no live spec fetch). After you save **AsyncAPI YAML**,
Build runs against a local AsyncAPI 2/3 YAML fixture (`asyncapi.yaml` or `_config.yaml`
`asyncapi.file`; no live spec fetch). Edit loc, lastmod, or
the referenced page files on the CMS host and choose **Build Virtual Site** again — assembled
output updates without a Jetty restart. After that Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml/asyncapi-yaml Build succeeds,
**Preview assembled site** streams last-build home HTML. **Publish Virtual Site** copies
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml/asyncapi-yaml.
...
**object-storage**, **`rss-atom`**, **`icalendar`**, **`sitemap-xml`**, **`robots-txt`**, **`llms-txt`**, **`openapi-yaml`**, and **`asyncapi-yaml`** (`sql-database` / `http-json` / `object-storage` /
`rss-atom` / `icalendar` / `sitemap-xml` / `robots-txt` / `llms-txt` / `openapi-yaml` / `asyncapi-yaml` REST last-build) — preview is not git-only. Traditional **Repository** Sites hide **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. Developer Sites **Preview
assembled site** appears for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, Sitemap XML, Robots.txt, llms.txt, OpenAPI YAML, and AsyncAPI YAML after a successful
Build. **Publish Virtual Site** is shown for those same kinds including llms.txt, OpenAPI YAML, and AsyncAPI YAML. After REST or in-product Build for `sql-database` (in-memory)
...
2. Confirm **Source kind** is saved and **Build Virtual Site** completed, then choose
   **Preview assembled site**:
   ...
   - **llms.txt** — saved **Root path** to a local `llms.txt` fixture
     ...
     **Publish Virtual Site** copies last-build HTML to `IPSSite.root`.
   - **OpenAPI YAML** — saved **Root path** to a local OpenAPI 3 YAML fixture
     (`openapi.yaml` or `_config.yaml` `openapi.file`), then a successful openapi-yaml Build.
     **Preview assembled site** opens last-build local HTML only (a single path operation
     typically assembles `{version}/listPets-1.html`; Preview uses that sole HTML page
     when `index.html` is absent). No live spec fetch.
     **Publish Virtual Site** copies last-build HTML to `IPSSite.root`.
