...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, **Sitemap XML**, and **Robots.txt**. **llms.txt** shows **Build Virtual Site** and **Preview assembled site** after save; **Publish Virtual Site** stays hidden for that kind. Traditional **Repository** hides that chrome.
...
and GET-roundtrip the kind (local fixture `rootPath` only), then **Build Virtual Site** and **Preview assembled site**. **Publish Virtual Site** stays a later slice. REST **Build** (`POST …/virtual/build`) writes HTML from that local `llms.txt` / `llms.file` fixture (`pagesWritten > 0`; leftover `virtual.remoteUrl`, credentials, and cloud `rootPath` are **400**; no live HTTP fetch). REST **Preview** (`GET …/virtual/preview`) streams last-build local HTML (`available=true`; missing build is `available=false` HTTP 200). REST **Build** (`POST …/virtual/build`) also writes HTML from that local
...
and **Preview assembled site** after save. **Publish Virtual Site** is also shown for those kinds except **llms.txt** (later slice). Traditional
...
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt (not llms-txt).
...
Build. **Publish Virtual Site** stays hidden for llms.txt. After REST or in-product Build for `sql-database` (in-memory
