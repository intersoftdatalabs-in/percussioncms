...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Publish Virtual Site** and **Preview assembled site** appear for Git, CSV,
   ...
   Preview and Publish in this release (those remain a later slice).
...
`http-json`) and then **Build Virtual Site**. Preview/Publish chrome for that
...
successful Build. **HTTP JSON** hides Preview in this release (Build is available
after save). After REST Build for `sql-database` (in-memory H2),
