...
5. Choose **Save Virtual Site source**. For Git the SPA sends the Jackson/JAXB
   ...
   **Object storage**, **RSS / Atom**, **iCalendar**, and **Sitemap XML** (Build and Preview;
   **Publish Virtual Site** stays hidden for Sitemap XML). Traditional **Repository** hides that chrome.
...
live crawl). REST **Publish** (`POST …/virtual/publish`) copies assembled HTML to `IPSSite.root` after a local sitemap.xml Build (leftover `virtual.remoteUrl`, credentials, and cloud URL `rootPath` are **400**; no live crawl). Developer Sites can save **Sitemap XML**, then **Build Virtual Site** and **Preview assembled site** (last-build local HTML; missing build stays unavailable). **Publish Virtual Site** chrome stays hidden. REST **Build**, **Preview**, and **Publish** run for `icalendar`
...
`rootPath` only), then **Build Virtual Site** and **Preview assembled site**. **Publish Virtual Site**
stays later. After root, remote, or
...
**HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, or **Sitemap XML**, the Site detail panel shows **Build Virtual Site** and
**Preview assembled site** after save. **Publish Virtual Site** is shown for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, and iCalendar (hidden for Sitemap XML). Traditional
...
that HTML to the Site filesystem root for Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar (not Sitemap XML chrome).
...
**object-storage**, **`rss-atom`**, **`icalendar`**, and **`sitemap-xml`** (`sql-database` / `http-json` / `object-storage` /
`rss-atom` / `icalendar` / `sitemap-xml` REST last-build) — preview is not git-only. Traditional **Repository** Sites hide **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. Developer Sites **Preview
assembled site** appears for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, and Sitemap XML after a successful
...
`sitemap-xml` (local `sitemap.xml` / `sitemap.file`; no live crawl),
...
2. Confirm **Source kind** is saved and **Build Virtual Site** completed, then choose
   **Preview assembled site**:
   ...
   - **Sitemap XML** — saved **Root path** to a local `sitemap.xml` fixture
     ...
     **Publish Virtual Site** stays hidden.
