...
`object-storage`, `rss-atom`, `icalendar`, `sitemap-xml`, `robots-txt`, `llms-txt`, and `openapi-yaml` (`POST …/virtual/build`). A second sitemap-xml Build after an in-process `sitemap.xml` / page edit returns `pagesWritten > 0` HTML that matches the current files (no Jetty restart). CSV assemble does not require a Git remote;
`_config.yaml` is optional for CSV and required for SQL, HTTP JSON, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, and openapi-yaml.
...
**HTTP JSON**, **Object storage**, **RSS / Atom**, **iCalendar**, **Sitemap XML**, **Robots.txt**, **llms.txt**, or **OpenAPI YAML**, the Site detail panel shows **Build Virtual Site**
and **Preview assembled site** after save. **Publish Virtual Site** is also shown for those kinds including **llms.txt** and **OpenAPI YAML**. Traditional
...
`openapi.file`; no live spec fetch). Edit loc, lastmod, or
the referenced page files on the CMS host and choose **Build Virtual Site** again — assembled
output updates without a Jetty restart. After that Git/CSV/SQL/HTTP JSON/object-storage/rss-atom/icalendar/sitemap-xml/robots-txt/llms-txt/openapi-yaml Build succeeds,
...
**object-storage**, **`rss-atom`**, **`icalendar`**, **`sitemap-xml`**, **`robots-txt`**, **`llms-txt`**, and **`openapi-yaml`** (`sql-database` / `http-json` / `object-storage` /
`rss-atom` / `icalendar` / `sitemap-xml` / `robots-txt` / `llms-txt` / `openapi-yaml` REST last-build) — preview is not git-only. Traditional **Repository** Sites hide **Build Virtual Site**,
**Preview assembled site**, and **Publish Virtual Site**. Developer Sites **Preview
assembled site** appears for Git, CSV, SQL, HTTP JSON, Object storage, RSS / Atom, iCalendar, Sitemap XML, Robots.txt, llms.txt, and OpenAPI YAML after a successful
Build. **Publish Virtual Site** is shown for those same kinds including llms.txt and OpenAPI YAML. After REST or in-product Build for `sql-database` (in-memory)
