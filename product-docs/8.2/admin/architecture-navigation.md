...
Folder ACL user-list editing (write principals) remains a follow-on slice. Blog
authoring stays on Home / the Blogs gadget — Navigation will not grow a second
blog editor. The classic `siteArchitecture.jsp` host is retired from the shipped
WebUI app; bookmarks still land here via `?view=arch` and the former JSP URL.
