...
| **Rename** | Renames the selected regular section (updates section title / landing link title). **Save** posts `GET /section/properties/{id}` then `POST /section/update` with the new title (folder ACL is not rewritten on a title-only rename). The dialog closes on HTTP 200 and the tree shows the new title after reload. A rollback-only HTTP 500 on Save is a defect — do not leave the dialog open as success. |
