...
define(["knockout", "pubsub", "utils", "dynatree"], function (
  ...
  return function AssetWizardViewModel(options) {
    ...
    function validFields() {
      ...
      return (
        !self.invalidAssetFolderSelected() &&
        !self.invalidAssetTypeSelected() &&
        !self.assetRootSelected()
      );
    }
      ...
      if (self.isAssetWizardVisible()) {
        if (validFields()) {
          ...
          self.options.cm1Adaptor
            .createAsset(self.assetFolder(), self.assetType())
