...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask(
    ...
    function () {
      ...
      if (matches.length) {
        grunt.log.error(
          "The following files have trailing spaces that need to be cleaned up:"
        );
  ...
  function getCombinedSources() {
    var fragments = grunt.config("fragments"),
      ...
      flattenedSourceFilenames = Array.prototype.concat.apply(
        [],
        sourceFilenames
      ),
      combinedSources = flattenedSourceFilenames
  ...
  function buildMin(output, done) {
    ...
    cc.compile(
      "/**@const*/var DEBUG=false;" + getCombinedSources(),
      options,
  ...
  grunt.registerMultiTask("test", "Run tests", function () {
    var done = this.async();
    grunt.util.spawn(
      { cmd: this.target, args: [this.data] },
