  ...
  return function BlogpostWizardViewModel(options) {
    ...
    self.constants = {
      ...
      NOT_AUTHORIZED_ERROR_MSG_RESPONSE:
        "You are not authorized to post in this blog.",
      NOT_AUTHORIZED_ERROR_MSG_RESPONSE_MULTIPLE:
        "You are not authorized to post in the blog: ",
    };
    ...
    function validFields() {
      ...
      return (
        !self.invalidBlogFile() &&
        !self.invalidBlogTitle() &&
        !self.invalidBlogSelected() &&
        !self.blogFileTooLong()
      );
    }
    ...
    self.createBlog = function () {
      if (self.isBlogWizardVisible()) {
        ...
        if (validFields()) {
          self.options.cm1Adaptor.getSiteProperties(
            self.selectedBlog().site,
            function (status, result) {
              if (status === "success") {
                //replace it with constant $.PercServiceUtils.STATUS_SUCCESS
                ...
                if (fileExt && fileName.lastIndexOf(".") < 0) {
                  if (fileName.length + fileExt.length < 255) {
                    ...
                    fileName =
                      fileName.substring(0, 254 - fileExt.length) +
                      "." +
                      fileExt; //consider a dot as one more char
                  }
              ...
              self.options.cm1Adaptor
                .createPage(
                  self.blogFile(),
                  self.blogTitle(),
                  self.selectedBlog().templateId,
                  self.selectedBlog().folderPath
                )
                .fail(function (message) {
                  if (message === self.constants.NOT_AUTHORIZED_ERROR_MSG) {
                    if (self.multipleBlogs())
                      message =
                        self.constants
                          .NOT_AUTHORIZED_ERROR_MSG_RESPONSE_MULTIPLE +
                        self.selectedBlog().title;
                    else
                      message =
                        self.constants.NOT_AUTHORIZED_ERROR_MSG_RESPONSE;
                  }
                  PubSub.publish("OpenErrorDialog", message);
                });
            }
