...
//A mildly specialized version of the jQuery dialog.
    ...
    function confirm_dialog( options ) {
        ...
        dialog = $("<div/>").append( percSafeDialogContent(settings.question) ).perc_dialog(dlgOptions);
    }
    ...
    var PERC_DIALOG_HTML_UNSAFE_ATTR_VALUE = /javascript:|data:|vbscript:|expression\s*\(/i;
        ...
        children.forEach(function (child) {
            if (child.nodeType === 1) {
                ...
                Array.prototype.slice.call(child.attributes).forEach(function (attr) {
                    ...
                    // miss that.
                    var schemeCheckValue = attr.value.replace(/[\s\u0000-\u001f\u007f]+/g, "");
                    if (
    ...
    function percSafeDialogContent(content) {
        ...
        if ((content && typeof content.jquery === "string") || (content && content.nodeType)) {
            return content;
        }
    ...
    function alert_dialog( options ) {
        ...
        var dialog;
        dialog = $("<div/>").append( percSafeDialogContent(settings.content) ).perc_dialog({
    ...
    function prompt_dialog( options )
    {
        ...
        var dialog = $("<div/>")
            .append( $("<label for='perc-prompt-dialog-question'/>").append(percSafeDialogContent(settings.question)) )
            .append( $("<br/>") )
            .append( inputField )
            .dialog(
                {
