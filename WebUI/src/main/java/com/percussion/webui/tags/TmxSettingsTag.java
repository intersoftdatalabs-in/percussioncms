...
public class TmxSettingsTag extends TagSupport {
  ...
  public void setLang(String lang) {
    if (m_lang == null || m_lang.length() == 0) return;
    m_lang = lang;
  }
  ...
  private void loadTmx() throws IOException, ParserConfigurationException, SAXException {
    ...
    String prefixStr = m_prefixes == null || m_prefixes.length() == 0 ? "*" : m_prefixes;
    if (cache.isIndexed(m_lang, prefixStr)) return;
    ...
    while (keys.hasNext()) {
      ...
      if (!prefixStr.equals("*") && !accept(prefixes, key)) continue;
