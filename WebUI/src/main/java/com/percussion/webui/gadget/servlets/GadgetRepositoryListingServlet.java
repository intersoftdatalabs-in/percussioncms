...
public class GadgetRepositoryListingServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    if (type == null || type.trim().length() < 1) type = "All";
        ...
        for (File gadgetFile : gadgetFiles) {
          ...
          if (gadget != null
              && (type.equalsIgnoreCase("All")
                  || type.equalsIgnoreCase(gadget.get("type").toString()))) {
            gadgets.add(gadget);
          }
        }
      }
  ...
  private JSONObject loadGadget(File config) {
    ...
    try (FileInputStream fin = new FileInputStream(config)) {
      ...
      if (modulePrefs.getLength() > 0) {
        ...
        gadget.put(
            "iconUrl", GADGETS_BASE_URL + iconBaseUrl + '/' + modulePref.getAttribute("thumbnail"));
      }
  ...
  protected Map<String, String> loadGadgetTypeMap() {
    ...
    try (InputStream in =
      ...
      if (in == null) {
        log.error(
            "Gadget registry file is missing from WEB-INF/classes/{}",
