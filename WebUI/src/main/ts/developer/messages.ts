...
export const DEV_MSG_KEYS = {
  ...
  SITE_VIRT_HINT:
    "perc.ui.developer@Configure whether this Site reads content from the repository or from a Virtual Site adapter (Git filesystem, CSV filesystem, SQL database, or HTTP JSON). Blank/repository keeps traditional repository content.",
  ...
  SITE_VIRT_HTTP_JSON_HINT:
    "perc.ui.developer@HTTP JSON uses the root path only (no Git remote). Put a local JSON catalog (http.file or default pages.json) or a loopback http.url in _config.yaml. Save sourceKind=http-json, then Build Virtual Site, then Publish Virtual Site to copy assembled files to the Site filesystem target. Preview assembled site for HTTP JSON is a later phase. Open JSON only — no Authorization or API keys.",
