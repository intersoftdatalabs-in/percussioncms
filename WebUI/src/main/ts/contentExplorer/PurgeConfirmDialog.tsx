...
/** One confirm for Content → Purge selected (#4883). */
export function PurgeConfirmDialog({
  onConfirm,
  onCancel,
}: PurgeConfirmDialogProps): React.ReactElement {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="explorer-purge-confirm-title"
      data-testid="explorer-purge-confirm"
      style={{
        position: "fixed",
        inset: 0,
    ...
    >
      ...
      >
        <h2
          id="explorer-purge-confirm-title"
          style={{ fontSize: 16, margin: "0 0 12px" }}
        >
          {message(EXPLORER_MSG.CONFIRM_DELETE_TITLE)}
        </h2>
        <p style={{ margin: "0 0 16px", fontSize: 14 }}>
          {message(EXPLORER_MSG.MULTI_PURGE_CONFIRM)}
        </p>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button
            type="button"
            data-testid="explorer-purge-cancel"
            onClick={onCancel}
          >
            {message(EXPLORER_MSG.CONFIRM_CANCEL)}
          </button>
          <button type="submit" ref={okRef} data-testid="explorer-purge-ok">
