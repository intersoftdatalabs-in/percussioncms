...
/**
 * Path-style Publishing deep link for schedule dates on the site workspace.
 */
export function itemScheduleShellHref(opts: {
  siteId?: string;
  itemId?: string;
}): string {
...
export function spaItemScheduleHref(opts: {
  siteId?: string;
  itemId?: string;
}): string {
  const params = new URLSearchParams();
