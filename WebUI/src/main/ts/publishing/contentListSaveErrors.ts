...
/**
 * Map content-list create/update failures to operator-visible text.
 * HTTP 403 → forbidden; HTTP 409 → duplicate name.
 */
export function mapContentListSaveError(err: unknown): string {
  if (isApiError(err)) {
    if (err.status === 403) {
      return formatApiError(err, message(MSG.PUBLISH_FORBIDDEN));
    }
    if (err.status === 409) {
      return formatApiError(err, message(MSG.PUBLISH_CONTENT_LIST_NAME_CONFLICT));
    }
  }
