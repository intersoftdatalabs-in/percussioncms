...
(function ($, P) {
  P.pageModel = function (pageManager, templateManager, pageId, callback) {
    ...
    function updateAssetRelationship(
      widgetData,
      assetid,
      relationshipId,
      callback,
    ...
    function loadAssetDropCriteria(callback) {
      $.PercTemplateService().getAssetDropCriteria(
        pageId,
        true,
        function (adc) {
    ...
    function promoteAsset(assetId, widgetData, isResource, callback) {
      $.perc_utils.alert_dialog({
        title: "Error",
        content: "Promote action has not been implmented for page.",
      });
    }

    function clearOrphanAssets(
      widgetId,
      widgetDefinitionId,
      assetId,
      callback
    ...
    function configureAsset(widgetData, assetId, isSharedAsset, callback) {
      if (isSharedAsset) {
        //Check Permission
        $.PercFolderHelper().getAccessLevelById(
          assetId,
          false,
        ...
        function (status) {
          if (status) {
            ...
            var options = {
              ...
              question:
                "The page you opened has been modified in a different session. <br/><br/>Do you want to reload the page?",
              cancel: function () {},
              success: function () {
    ...
    function newRegion(optionalPrefix) {
      ...
      if (optionalPrefix) regionId = optionalPrefix + regionId;
    ...
    function canOverride(region) {
      return (
        region.owner === "page" ||
        (region.owner === "template" &&
          region.children.length === 0 &&
          region.widgets.length === 0)
      ); //(region.owner === "template" && region.children.length == 0 && region.widgets.length == 0);
    }
     ...
     */
    var percDecorationCssLinks =
      "<link rel='stylesheet' type='text/css' href='/cm/css/perc_decoration.css'/>";

    var jQueryScripts =
      "<script src='/cm/app/js/legacy/profiles/3x/jquery/jquery-3.6.0.js'></script>";
    jQueryScripts +=
      "<script src='/cm/app/js/legacy/profiles/3x/jquery/libraries/jquery-ui/jquery-ui.js'></script>";
    var jQueryCssLinks =
    ...
    function renderAll(frame, callback) {
      ...
      var renderPath =
        rootPath + "/" + pageId + "?timestamp=" + new Date().getTime();
    ...
    function render(callback) {
      updatePageObj();
      pageManager.render_region(
        templateRootRegion.children[0].regionId,
        utils.rexml($.perc_schemata.page, pageObj),
        function (data) {
    ...
    function updatePageObj() {
      pageObj.Page.regionBranches.regions =
        branchesFromRoot(templateRootRegion);
      pageObj.Page.regionBranches.regionWidgetAssociations =
        createRegionWidgetAssociations();
    }
