...
(function ($, P) {
  ...
  P.templateModel = function (templateManager, templateId, callback) {
      ...
      templateManager.load_template(templateId, function (xml) {
        ...
        loadAssetDropCriteria(function (adc) {
          for (var widgetId in assetDropCriteria) {
            widgetContentTypes[widgetId] =
              assetDropCriteria[widgetId].supportedContentTypes;
          }
    ...
    function setAssetRelationship(widgetData, assetid, isResource, callback) {
      $.PercAssetService.set_relationship(
        assetid,
        widgetData,
        templateId,
        "1",
      ...
      eachRegion(function () {
        $.each(this.widgets, function () {
          ...
          if (!widgetPrefs[defid]) {
            $.perc_widget_definition_client.restGetWidgetPrefs(
              defid,
              function (wPrefs) {
    ...
    function loadAssetDropCriteria(callback) {
      templateService.getAssetDropCriteria(
        templateId,
        false,
        function (adc) {
      ...
      eachRegion(function () {
        $.each(this.widgets, function () {
          if (
            ...
            this.name.toUpperCase() == widgetName.toUpperCase()
          ) {
            widget = this;
          }
        });
    ...
    function _eachRegion(tree, callback) {
      ...
      if (
        ...
        typeof tree.children == "undefined" ||
        typeof tree.children.length == "undefined"
      )
        return;
    ...
    function save(postCallback, newWidgetObj, requireMigration) {
      ...
      var saveAssetsCallback = function () {
        ...
        $.PercTemplateService().updateInspectedElements(
          newWidgetObj,
    ...
    function renderAll(frame, callback) {
      ...
      var rootPath = javaScriptOff
        ? $.perc_paths.TEMPLATE_RENDER_SCRIPTSOFF
        : $.perc_paths.TEMPLATE_RENDER;

      var renderPath =
        rootPath + "/" + templateId + "?timestamp=" + new Date().getTime();
      if (typeof gPageId != "undefined" && gPageId) {
        rootPath = javaScriptOff
          ? $.perc_paths.PAGE_EDIT_SCRIPTSOFF
          : $.perc_paths.PAGE_EDIT;
        renderPath =
          rootPath +
          "/" +
          gPageId +
          "?editType=TEMPLATE&timestamp=" +
          new Date().getTime();
      }
    ...
    function updateTemplObj() {
      ...
      templObj.Template.regionTree.regionWidgetAssociations =
        createRegionWidgetAssociations(root);
    ...
    function createRegionWidgetAssociations(root) {
      if (root.widgets.length > 0) {
        return [
          {
            _tagName: "regionWidget",
            regionId: root.regionId,
            widgetItems: $.map(root.widgets, function (w) {
              return P.widgetItemFromWidget(w);
            }),
          },
        ];
      } else {
        return $.map(root.children, function (c) {
          return createRegionWidgetAssociations(c);
        });
      }
    }
  };
  ...
  P.regionsFromTree = function (tree, regionWidgetAssociations, owner) {
    return modelObject.isResponsiveBaseTemplate()
      ? P.respRegionsFromTree(tree, regionWidgetAssociations, owner)
  ...
  P.baseRegionsFromTree = function (tree, regionWidgetAssociations, owner) {
    ...
    var widgets = $.grep(regionWidgetAssociations, function (a) {
      return a.regionId == regId;
    });
    if (widgets[0]) {
      widgets = $.map(widgets[0].widgetItems, function (w) {
        return P.widgetFromWidgetItem(w);
      });
    } else {
      widgets = [];
    }
    ...
    try {
      var tempNoAutoResize =
        noAutoResize === "" || noAutoResize === undefined
          ? false
          : noAutoResize;
    ...
    var subTrees = $.grep(tree.children, function (c) {
      return c._tagName == "region";
    });
    var subRegions = $.map(subTrees, function (r) {
      return P.baseRegionsFromTree(r, regionWidgetAssociations, owner);
    });
  ...
  P.respRegionsFromTree = function (tree, regionWidgetAssociations, owner) {
    ...
    var subRegions = $.map(subTrees, function (r) {
      return P.respRegionsFromTree(r, regionWidgetAssociations, owner);
    });
    subRegions = subRegions ? subRegions : [];
  ...
  P.treeFromRegions = function (region, isTop) {
    return modelObject.isResponsiveBaseTemplate()
      ? P.respTreeFromRegions(region, isTop)
  ...
  P.baseTreeFromRegions = function (region, isTop) {
    ...
    var direction = region.vertical
      ? " perc-vertical "
      : " perc-horizontal ui-helper-clearfix ";
    ...
    if (region.height || region.width || region.padding || region.margin) {
      startTag += 'style="';
      if (region.height) {
        if (!isNaN(region.height)) region.height += "px";
        startTag += "height:" + region.height + ";";
      }
      if (region.width) {
        if (!isNaN(region.width)) region.width += "px";
    ...
    startTag +=
      ...
      '" ' +
      ' data-noautoresize="' +
      modelObject.encodeHtml(region.noAutoResize) +
      '" id="' +
      modelObject.encodeHtml(region.regionId) +
      '">' +
      ' <div class="' +
      modelObject.encodeHtml(direction) +
      '"';
    ...
    if (region.attributes) {
      for (var i = 0, len = region.attributes.length; i < len; i++) {
        treeAttributes.push({
          _tagName: "attribute",
          name: region.attributes[i].name,
          value: region.attributes[i].value,
        });
        startTag +=
          " " +
          modelObject.encodeHtml(region.attributes[i].name) +
          '="' +
          modelObject.encodeHtml(region.attributes[i].value) +
          '"';
      }
    }
    ...
    if (leaf) {
      children = [
        {
          _tagName: "code",
          templateCode: '#region("' + region.regionId + '","","","","")',
        },
      ];
    } else {
      children = $.map(region.children, function (c) {
        return P.baseTreeFromRegions(c);
      });
      if (isTop) {
        var startChild = {
          _tagName: "code",
        ...
        var endChild = {
          _tagName: "code",
          templateCode: "#perc_templateFooter()",
        };
    ...
    if (isTop) {
      ...
      return {
        _tagName: "region",
        noAutoResize: region.noAutoResize,
  ...
  P.respTreeFromRegions = function (region, isTop) {
    ...
    startTag +=
      ' class="perc-region' +
      modelObject.encodeHtml(leaf + row + columns + large + cssClass) +
      '" ' +
      'id="' +
      modelObject.encodeHtml(region.regionId) +
      '"';
    ...
    if (leaf) {
      ...
      children = $.map(region.children, function (c) {
        return P.respTreeFromRegions(c);
      });
      if (isTop) {
        var startChild = {
          _tagName: "code",
      ...
      if (widget.properties) {
        for (var prop in widget.properties) {
          properties.push({
            _tagName: "property",
            name: prop,
            value: JSON.stringify(widget.properties[prop]),
          });
        }
      }
      var cssProperties = [];
  ...
  $.PercTemplateMetadataModel = function (
    id,
