...
(function ($) {
  ...
  function openDialog(itemId, itemName, itemType) {
    ...
    function createDialog(status, result) {
      ...
      var comments = revSummary.comments
        ? Array.isArray(revSummary.comments)
          ? revSummary.comments
          : [revSummary.comments]
        : [];
      if (comments.length === 0) {
        $.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.comments.dialog@No Comments"),
          content: I18N.message("perc.ui.comments.dialog@No Comments Yet"),
        });
        return;
      }
      ...
      percDialogObject = {
        resizable: false,
        title:
          I18N.message("perc.ui.commentsDialog.title@Comments") +
          ": " +
          itemName,
        modal: true,
        closeOnEscape: false,
    ...
    function createCommentsHtml(comments) {
      ...
      if (comments.length === 0) {
        commentHTML +=
          "<div class='perc-comments-row'>" +
          I18N.message("perc.ui.comments.dialog@No Comments Selected Page") +
          "</div>";
      } else {
        $.each(comments, function () {
          ...
          commentHTML +=
            "<div class='perc-comments-row'>" +
            "<div class='perc-comments-header'><span class='perc-comment-madeby'>" +
            comment.commenter +
            " -- </span>" +
            "<span class='perc-comment-date'>" +
            commentDateDate +
            " " +
            commentDateTime +
            "</span>" +
            "<span class='perc-comment-type'> (" +
            comment.commentType +
            ")</span></div>" +
            "<div>" +
            comment.comment +
            "</div><hr class='perc-comment-sep'/>" +
            "</div>";
        });
      }
