...
(function ($) {
  ...
  function extractDeleteErrorMessage(data, name, type) {
    ...
    } else if (response.indexOf(type + ".deleteNotAuthorized") > -1) {
      id = "perc-finder-delete-auth";
      content = I18N.message(msgKeyBase + "@Not Authorized", [name]);
    } else if (response.indexOf(type + ".deleteTemplates") > -1) {
    ...
    } else if (response.indexOf(type + ".deleteApprovedPages") > -1) {
      id = "perc-finder-delete-approved";
      content =
        I18N.message(msgKeyBase + "@Approved Pages", [name]) +
