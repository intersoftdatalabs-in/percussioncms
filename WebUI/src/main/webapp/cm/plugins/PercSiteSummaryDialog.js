...
(function ($) {
  ...
  function renderSiteSummaryData(status, siteSummaryData) {
    ...
    $ui = $content
      .find(".perc-site-summary-statistics .ui-widget-content")
      .empty();
    ...
    if (siteSummaryData.abridged_log_message) {
      $ui = $content
        .find(".perc-site-summary-warnings .perc-log-top")
        .empty()
  ...
  function createDialogAndOpen(status, siteSummaryData) {
    if (!getSiteId()) {
      $.perc_utils.alert_dialog({
        ...
        content: I18N.message("perc.ui.site.summary.dialog@Select a Site"),
      });
      return;
    }
    // handle error status
  ...
  function onToggleSection(event) {
    var $this = $(this),
      $parent = $this.parents(".perc-section:first"),
      $target;
