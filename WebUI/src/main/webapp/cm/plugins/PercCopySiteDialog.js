  ...
  function openDialog(siteName, saasSiteNames) {
    ...
    function createDialog() {
      var label = $("<div/>")
        ...
        .append($("<label/>").text("Enter a new name for your site."));
      ...
      $(document).on(
        "change",
        "input:radio[name=share-type-buttons]",
        function () {
          clearCopySiteErrorMessage();
    ...
    function createConfirmDialog(newSiteName) {
      var settings = {
        ...
        question:
          I18N.message("perc.ui.copy.site.dialog@A new site") +
          newSiteName +
          I18N.message("perc.ui.copy.site.dialog@Will Be Created") +
          siteName +
          I18N.message("perc.ui.copy.site.dialog@The Copy") +
          I18N.message("perc.ui.copy.site.dialog@Running In Background") +
          I18N.message("perc.ui.copy.site.dialog@Depending On Size") +
          siteName +
          "'. '" +
          newSiteName +
          I18N.message("perc.ui.copy.site.dialog@Will Be Added") +
        ...
        success: function () {
          ...
          $.PercSiteService.copySite(
            jsonRequest,
            function (status, result, defaultCode) {
    ...
    function _validateFolders(newSiteName) {
      if (selectedCheckbox === "share-type-buttons-selected-folder") {
        ...
        $.PercSiteService.validateCopySiteFolders(
          jsonRequest,
          function (statusFolders, resultFolders, defaultCodeFolders) {
    ...
    function _validateUniqueSiteName(value, response) {
      ...
      for (i = 0; i < response.SiteSummary.length; i++) {
        if (
          (response.SiteSummary[i].name + "").toLowerCase() ===
          value.toLowerCase()
        ) {
          return false;
        }
