  ...
  $.perc_editSectionDialog = function () {
    function openDialog(sectionId, callback) {
      ...
      function createDialog(status, result) {
        ...
        dialog = $(
          "<div>" +
            ...
            "<label for='perc-section-name' class='perc-required-field'>" +
            I18N.message("perc.ui.newSectionDialog.label@Section name") +
            ":</label> <br/> " +
            "<input type='text' aria-required='true' class='required' tabindex='0' id='perc-section-name' name='section_name' maxlength='512'/> <br/>" +
            "<label for='perc-section-url' class='perc-required-field'>" +
            I18N.message("perc.ui.newSectionDialog.label@URL") +
            ":</label> <br/> " +
            "<input type='text' class='required' aria-required='true' tabindex='0' id='perc-section-url' name='page_url' maxlength='100'/> <br/>" +
            "<label for='perc-external-link-target'>" +
            I18N.message("perc.ui.editSectionDialog.label@Target window") +
            ":</label> <br/>" +
            "<select id='perc-section-target' name='perc-section-target'>" +
            "<option value='_self' selected>" +
            I18N.message("perc.ui.editSectionDialog.label@Same window") +
            "</option>" +
            "<option value='_blank'>" +
            I18N.message("perc.ui.editSectionDialog.label@New window") +
            "</option>" +
            "<option value='_top'>" +
            I18N.message("perc.ui.editSectionDialog.label@Top window") +
            "</option>" +
            "<option value='_parent'>" +
            I18N.message("perc.ui.editSectionDialog.label@Parent window") +
            "</option>" +
            "</select> <br/>" +
            ...
            "<label for='perc-section-folder-permission'>" +
        ...
        if (
          sectionProps.folderPermission.accessLevel ===
          $.PercFolderHelper().PERMISSION_READ
        )
      ...
      function _addFieldGroups() {
        ...
        var fieldGroups = [
          {
            groupName: "perc-section-general-container",
            groupLabel: "Section",
          },
          { groupName: "perc-section-users-container", groupLabel: "Users" },
          {
            groupName: "perc-section-security-container",
            groupLabel: "Security",
          },
        ];
        $.each(fieldGroups, function (index) {
          ...
          var groupHtml =
            ...
            "<div class='perc-section-label' groupName='" +
            this.groupName +
            "'>" +
            "<span tabindex='0' class='perc-min-max " +
            minmaxClass +
            "' ></span>" +
            this.groupLabel +
            "</div>" +
      ...
      function _handleRequiresLogin(sectionProps) {
        var requiresLogin = $("#perc-requires-login");
        if (sectionProps.requiresLogin) requiresLogin.prop("checked", true);
        else requiresLogin.prop("checked", false);
      ...
      function getSecureSectionFieldGroups() {
        ...
        fieldGroups =
          ...
          "<label for='perc-requires-login'>" +
          I18N.message("perc.ui.editSectionDialog.label@Requires Log in") +
          "</label> <br/>" +
