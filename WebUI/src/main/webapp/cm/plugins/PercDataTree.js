...
(function ($) {
  ...
  function init(container, config) {
    ...
    if (options.addHeader) {
      ...
      options.collapsible &&
        head.append(
          $(
            '<span role="button" tabindex="0" style="float: left;" class ="' +
              COLLAPSE_BTN_CLASS +
              " " +
              MINIMIZER_CLASS +
              '" />',
    ...
    $("#" + options.instanceId).fancytree({
      ...
      renderNode: function (event, data) {
        ...
        if (typeof checkSpan !== "undefined") {
          checkSpan.attr("title", node.data.title);
    ...
    if (
      typeof options.listItem !== "undefined" &&
      typeof options.listItem === "object"
    ) {
  ...
  function addSubtreeContents(fancytree_node, dataSubtrees, levelLimit, level) {
    ...
    if (
      dataSubtrees &&
      dataSubtrees instanceof Array &&
      dataSubtrees.length > 0
    ) {
      $.each(dataSubtrees, function (i) {
        ...
        if (
          ...
          has_workflow &&
          this.workflowName !== selectedWorkflow
        ) {
          child_template.unselectable = true;
          child_template.title =
            child_template.title + " (" + this.workflowName + ")";
          child_template.addClass =
            child_template.addClass + " " + DISABLED_ITEM_CLASS;
        }
        ...
        if (
          !this.allChildrenAssociatedWithWorkflow ||
          this.workflowName !== selectedWorkflow
        ) {
          child_template.addClass =
            child_template.addClass + " " + CHILDREN_OTHER_WORKFLOW_CLASS;
        } else {
          ...
          if (options.showCheckboxes && child_template.select === true) {
            child_template.addClass =
              child_template.addClass + " " + ALL_SAME_WORKFLOW_CLASS;
          }
        }

        // Add some useful data to the node
        child_template.workflowName = this.workflowName;
        child_template.allChildrenAssociatedWithWorkflow =
          this.allChildrenAssociatedWithWorkflow;
