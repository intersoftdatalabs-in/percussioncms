...
(function ($) {
  ...
  function createViewCommentsDialog(siteName, pagePath) {
    ...
    var url =
      $.PercCommentsGadgetService().constants.URLS.VIEW_COMMENTS_JSP +
      "?" +
      "site=" +
      $.perc_utils.encodeURL(siteName) +
      "&" +
      "pagePath=" +
      $.perc_utils.encodeURL(pagePath);
    var dlgHtml =
      "<div id='perc-gadget-comments-viewComments-frame-container'>" +
        ...
        Save: {
          click: function () {
            ...
            if (approveRejectedStateChanges != null) {
              saveApproveRejectedStateChanges(
                siteName,
    ...
    function parseStateChanges(dialog) {
      ...
      $.each(approved, function () {
        ...
        if (originalState === currentState) return true;
        approvedIds.push(id);
      });
      var rejectedIds = [];
      ...
      if (approvedIds.length > 0) {
        ...
        moderation.moderation.approves[0] = {
          site: site,
          comments: approvedIds,
        };
      }
      if (rejectedIds.length > 0) {
        moderation.moderation.rejects = [];
        moderation.moderation.rejects[0] = {
          site: site,
          comments: rejectedIds,
        };
      }
