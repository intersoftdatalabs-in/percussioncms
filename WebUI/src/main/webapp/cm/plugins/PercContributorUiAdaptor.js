...
(function ($) {
  $.PercContributorUiAdaptor = function () {
    ...
    function createPage(name, linkTitle, templateId, folderPath) {
      ...
      var fullPath =
        tempPath.substring(tempPath.length - 1) === "/"
          ? tempPath + name
          : tempPath + "/" + name;
    ...
    function getRecentTemplates(siteName) {
      var deferred = $.Deferred();
      api
        .getRecentItems("template", siteName)
        .done(function (result) {
          var templates = [];
          if (result.data) {
    ...
    function getRecentSiteFolders(siteName) {
      var deferred = $.Deferred();
      api
        .getRecentItems("site-folder", siteName)
        .done(function (result) {
          var sitefolders = [];
          if (result.data) {
    ...
    function getAssetTypes(filterDisabledWidgets) {
      var deferred = $.Deferred();
      $.PercAssetService.getAssetTypes(
            ...
            $.PercUserService.getAccessLevel(
              ...
              function (status, result) {
                if (
                  ...
                  var recFolderPath =
                    folderPath.substring(folderPath.length - 1) === "/"
    ...
    function getSearchResults(searchCriteria) {
      var deferred = $.Deferred();
      $.PercSearchService.getAsyncSearchExtendedResult(
        searchCriteria,
        function (status, result) {
          if (status) {
            deferred.resolve(result);
    ...
    function getStates(workflow) {
      var deferred = $.Deferred();
      $.PercReusableSearchService.getStates(
        workflow,
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            var states = [];
    ...
    function getFolders(path) {
      path = getNormalizedPath(path);
      var deferred = $.Deferred(),
        url = $.perc_paths.PATH_FOLDER + path;
