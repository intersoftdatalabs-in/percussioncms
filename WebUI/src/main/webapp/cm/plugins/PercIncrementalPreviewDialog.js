  ...
  function _createPagedTable(startIndex) {
    var callback = function (status, data) {
      if (status) {
        ...
        var noItemsMsg = gOnRelatedView
          ? "No unapproved related items found."
          : "No items are available for incremental publishing.";
        var configDT = {
          percRowClickCallback: $.noop,
          percRowDblclickCallback: $.noop,
        ...
        var container = gOnRelatedView
          ? gDialog.find("#perc-incremental-related-preview-table")
          : gDialog.find("#perc-incremental-preview-table");
        container.empty();
        container.PercDataTable(configDT);
        if (data.PagedItemList.childrenCount > 0) {
          var pagingConfig = {
            totalItems: data.PagedItemList.childrenCount,
            startIndex: startIndex,
            pageSize: $.PercIncrementalPreviewDialog.PAGE_SIZE,
            pagingCallback: paginateItems,
          };
          var pagingContainer = gOnRelatedView
            ? gDialog.find("#perc-incremental-related-preview-paging")
            : gDialog.find("#perc-incremental-preview-paging");
    ...
    if (gOnRelatedView) {
      $.PercPublisherService().getIncrementalRelatedItems(
        gSiteName,
        gServerName,
        startIndex,
