...
(function ($) {
  ...
  function handleOpenSite(item) {
    ...
    if (view === constants.VIEW_WORKFLOW) toView = constants.VIEW_WORKFLOW;

    if (view === constants.VIEW_DESIGN) toView = constants.VIEW_DESIGN;
    //If already on publish iew then just change site and reload publish view
    if (view === constants.VIEW_PUBLISH) toView = constants.VIEW_PUBLISH;
    changeLocation(
      item["id"],
      toView,
      mode,
        ...
        if (status === $.PercFolderHelper().PERMISSION_ERROR) {
          ...
          if (isEditMode && result === $.PercFolderHelper().PERMISSION_READ) {
            $.perc_utils.alert_dialog({
              ...
              content:
                I18N.message("perc.ui.navi.manager@No Permissions For Page") +
                item["name"] +
                "'.",
            });
          } else {
            var aSite = parseSiteFromPath(item["path"]);
            var aMode = isEditMode
              ? constants.MODE_EDIT
  ...
  function handleOpenFolder(pathItem) {
    ...
    if (pathItem.type === "FSFolder") {
      ...
      if (
        ...
        $.Percussion.PERC_FINDER_VIEW_COLUMN
      ) {
      } else {
        ...
        if (paths.length > 5) {
          ...
          $.perc_utils.alert_dialog({
            title: I18N.message("perc.ui.page.general@Warning"),
            content: I18N.message("perc.ui.navi.manager@System Folder"),
          });
        }
      }
    }
        ...
        if (status === $.PercFolderHelper().PERMISSION_ERROR) {
          ...
          if (isEditMode && result === $.PercFolderHelper().PERMISSION_READ) {
            $.perc_utils.alert_dialog({
              title: I18N.message("perc.ui.page.general@Warning"),
              content:
                I18N.message("perc.ui.navi.manager@No Permissions For Asset") +
                item["name"] +
                "'.",
            });
          } else {
  ...
  function buildParams(
    ...
    if (!isBlank(aMode)) $.extend(params, { mode: aMode });
    if (!isBlank(aId)) $.extend(params, { id: aId });
    if (!isBlank(aName)) $.extend(params, { name: aName });
    if (!isBlank(aPath)) $.extend(params, { path: aPath });
    if (debug) $.extend(params, { debug: "true" });
    if (autoTest) $.extend(params, { at: "true" });
    if (aMemento != null && typeof aMemento == "object")
      $.extend(params, { memento: JSON.stringify(aMemento) });
    if (!isBlank(aPathType)) $.extend(params, { pathType: aPathType });
    if (javascriptOff) $.extend(params, { disableJS: "true" });
    return params;
  }
  ...
  function loadUserInfoFromCookie() {
    ...
    isAccessibilityUser =
      $.cookie("perc_isAccessibilityUser") === "true" ? true : false;
  ...
  var obj = {
    ...
    registerFinder: function (finder) {
      ...
      finder.addPathChangedListener(function (p) {
        if (finder.finderOpenInProgress) return;
        var temp = p.slice(1);
        var pt = "/" + temp.join("/");
        path = pt;
      });
    ...
    loadSiteProperties: function (siteName, callback) {
      $.PercSiteService.getSiteProperties(siteName, function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
          ...
          if (typeof callback == "function") callback(result.SiteProperties);
        }
      });
    },
