...
(function ($, undefined) {
  ...
  $.widget("ech.multiselect", {
    ...
    _getAppendEl: function () {
      ...
      if (elem) {
        // NOTE: The find below handles the jQuery selector case
        elem = !!elem.jquery
          ? elem
          : !!elem.nodeType
            ? $(elem)
    ...
    _create: function () {
      ...
      var linkInfo = (this.linkInfo = $.extend(
        true,
        {},
        linkDefaults,
        options.linkInfo || {},
    ...
    _makeOption: function (option) {
      ...
      var inputID =
        ...
        self.multiselectID +
        "-" +
        (option.id || id + "-option-" + self.inputIdCounter++);
      ...
      item.className =
        (isDisabled ? "ui-multiselect-disabled " : "") +
        (option.className || "");
    ...
    update: function (isDefault) {
      ...
      if (
        self._isOpen &&
        self._savedButtonHeight != self.$button.outerHeight(false)
      ) {
        self.position();
      }
    },
    ...
    _setButtonValue: function (value, isDefault) {
      this.$buttonlabel[
        this.options.htmlText.indexOf("button") > -1 ? "html" : "text"
      ](value);
    ...
    _bindCheckboxEvents: function () {
      ...
      self.$checkboxes
        .on("click.multiselect", ".ui-multiselect-grouplabel", function (e) {
          ...
          if (
            maxSelected &&
            self.$inputs.filter(":checked").length + $inputs.length >
              maxSelected
          ) {
            return;
          }
        ...
        .on("click.multiselect", ".ui-multiselect-collapser", function (e) {
          var $this = $(this),
            $parent = $this.parent(),
            optgroupLabel = $parent
              .find(".ui-multiselect-grouplabel")
              .first()
              .html(),
          ...
          if (
            self._trigger("beforecollapsetoggle", e, {
              label: optgroupLabel,
          ...
          self._trigger("collapsetoggle", e, {
            label: optgroupLabel,
            collapsed: !isCollapsed,
          });
        })
        ...
        .on("click.multiselect", "input", function (e) {
          ...
          var options = self.options;
          var textFxn =
            options.htmlText.indexOf("options") > -1 ? "html" : "text";
          ...
          $tags.each(function () {
            this.selected =
              this.value === val ? checked : isMultiple && this.selected;
          });
    ...
    _parse2px: function (dimText, $elem, isHeight) {
      ...
      switch (unit) {
        ...
        case "mm":
          pixels =
            { pt: 4.0 / 3.0, in: 96.0, cm: 96.0 / 2.54, mm: 96.0 / 25.4 }[
              unit
            ] * value;
          break;
        case "em":
        ...
        case "%":
          if (!!$elem) {
            ...
            pixels =
              (!!isHeight ? $elem.parent().height() : $elem.parent().width()) *
              (value / 100.0);
    ...
    _setMenuWidth: function (recalc) {
      ...
      var width = !!this.options.listbox
        ? this._selectWidth
      ...
      var headerWidth =
        this.$header.outerWidth(true) + this._jqWidthFix(this.$header);
      var cbWidth =
        this.$checkboxes.outerWidth(true) + this._jqWidthFix(this.$checkboxes);
    ...
    _setMenuHeight: function (recalc) {
      ...
      if (/\d/.test(optionHeight)) {
        ...
        var headerHeight =
          $header.outerHeight(true) + self._jqHeightFix($header);
        var borderPaddingHt =
          this.$menu.outerHeight(false) - this.$menu.height();

        optionHeight = self._parse2px(optionHeight, self.element, true).px;
        maxHeight =
          Math.min(optionHeight, maxHeight) - headerHeight - borderPaddingHt;
      ...
      self.$checkboxes
        ...
        .each(function () {
          ...
          if (
            (useSelectSize && ++itemCount >= elSelectSize) ||
            ulHeight > maxHeight
          ) {
    ...
    _toggleDisabled: function (flag, groupID) {
      ...
      this.$button
        ...
        [flag ? "addClass" : "removeClass"](disabledClass);
      ...
      if (this.options.disableInputsOnToggle) {
        ...
        if (flag) {
          ...
          var matchedInputs = $inputs
            .filter("." + msDisabledClass + ":disabled")
            .get();
          for (var x = 0, len = matchedInputs.length; x < len; x++) {
      ...
      var $select =
        typeof groupID === "undefined"
          ? this.element
          : this._nativeOptgroupFilter(groupID).find("option");
      $select.prop({
        disabled: flag,
        "aria-disabled": flag,
      });
    ...
    flipAll: function (groupID) {
      ...
      var gotID = typeof groupID !== "undefined", // groupID could be 0
        ...
        $filteredOptgroupInputs = gotID
          ? this._multiselectOptgroupFilter(groupID).find("input")
          : null,
        gInputCount = gotID ? $filteredOptgroupInputs.length : 0,
        gCheckedCount = gotID
          ? $filteredOptgroupInputs.filter(":checked").length
          : 0;
      ...
      if (
        ...
        maxSelected >=
          (gotID
            ? checkedCount - gCheckedCount + gInputCount - gCheckedCount
            : inputCount - checkedCount)
      ) {
        if (gotID) {
    ...
    collapseAll: function (groupID) {
      ...
      var $optgroups =
        typeof groupID === "undefined" // groupID could be 0
    ...
    addOption: function (attributes, text, groupID) {
      var self = this;
      var textFxn =
        self.options.htmlText.indexOf("options") > -1 ? "html" : "text";
      ...
      if (typeof groupID === "undefined") {
        ...
        self
          ._multiselectOptgroupFilter(groupID)
    ...
    _nativeOptgroupFilter: function (groupID) {
      return this.element.children("OPTGROUP").filter(function (index) {
        return typeof groupID === "number"
          ? index === groupID
          : this.getAttribute("label") === groupID;
      });
    },
      ...
      switch (key) {
        ...
        case "multiple":
          ...
          if (!!$element[0].multiple !== value) {
            $menu
              .toggleClass("ui-multiselect-multiple", value)
              .toggleClass("ui-multiselect-single", !value);
  ...
  if ($.ui && "dialog" in $.ui) {
    $.widget("ui.dialog", $.ui.dialog, {
      _allowInteraction: function (event) {
        if (
          this._super(event) ||
          $(event.target).closest(".ui-multiselect-menu").length
        ) {
          return true;
        }
      },
