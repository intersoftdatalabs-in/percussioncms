...
(function ($) {
  ...
  $.perc_redirect = function (url, args) {
    ...
    var qStr = args == null || args === "undefined" ? "" : $.param(args);
    if (qStr.length > 0) qStr = (hasQuery ? "&" : "?") + qStr;
...
(function ($) {
  $.perc_textAutoFill = function (
    ...
    $(src).on("keyup.textAutoFill", { filter: filter }, function (evt) {
      ...
      if (typeof evt.data.filter == "function") filter = evt.data.filter;
...
(function ($) {
  $.perc_filterField = function (tgt, filter, callback) {
    $(tgt).on("keypress.filterField", { filter: filter }, function (evt) {
      var filt = function (txt) {
        return txt;
      };

      var rawCode = evt.charCode ? evt.charCode : evt.which;
      if (rawCode === 0 || rawCode === 8 || rawCode === 13) return;
      var theChar = String.fromCharCode(rawCode);
      if (typeof evt.data.filter == "function") filt = evt.data.filter;
    ...
    $(tgt).on("blur.filterField", { filter: filter }, function (evt) {
      ...
      if (typeof callback === "function") callback($(tgt));
    });
  };
})(jQuery);
...
(function ($) {
  $.perc_filterFieldText = function (tgt, filter, splitChar) {
    $(tgt).on("keypress.filterField", { filter: filter }, function (evt) {
      ...
      if (
        typeof fieldMaxLength != "undefined" &&
        theText.length === fieldMaxLength
      )
        return;

      theText =
        theText.substring(0, position) +
        theChar +
        theText.substring(position, theText.length);
...
(function ($) {
  $.perc_textFilters = {
    ...
    ALPHA_NUMERIC: function (txt) {
      if (typeof txt === "undefined" || txt === null) {
        return txt;
      }
      return txt.replace(/[^a-zA-Z0-9]/g, "");
    },
    DIGITS_DOT: function (txt) {
      if (typeof txt === "undefined" || txt === null) {
        return txt;
      }
      return txt.replace(/[^0-9\.]/g, "");
    ...
    WINDOWS_FILE_NAME: function (txt) {
      if (typeof txt === "undefined" || txt === null) {
        return txt;
      }
      return txt.replace(/\.*$/g, "");
...
(function ($) {
  $.perc_autoFillTextFilters = {
    ...
    IDNAMECDATA: function (txt) {
      ...
      return txt.replace(/[:\ \.]/g, "-").replace(/[^a-zA-Z0-9\-\_]/g, "");
    },
...
(function ($) {
  ...
  $.PercBlockUIMode = {
    ...
    OVERLAY: {
      message:
        '<img src="/cm/images/images/Busy.gif" alt="Processing.  Please wait..."/><br/>Processing. Please wait...',
      css: {
        border: "none",
        backgroundColor: "#fff",
        opacity: "0.9",
