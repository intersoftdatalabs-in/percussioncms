...
(function ($) {
  var style = document.documentElement.style,
    ...
    propertyName = document.createTextNode("").textContent
      ? "textContent"
      : "data",
            ...
            if (nodes[i].nodeType === 1) {
              ...
              if (index + nodes[i].length < maxIndex) {
                ...
                tmp[propertyName] = options.trim
                  ? rtrim(tmp[propertyName].substring(0, clipIndex))
                  : tmp[propertyName].substring(0, clipIndex);
                result[result.length - 1].appendChild(tmp);
              }
        ...
        return this.each(function () {
          ...
          if (clone.width() > originalWidth) {
            while (low < high) {
              ...
              clone
                .empty()
                .append(domSplit(originalElement.get(0), mid, o))
                .append(o.str);
              if (clone.width() < originalWidth) {
                low = mid + 1;
              } else {
            ...
            if (low < originalText.length) {
              element
                .empty()
                .append(domSplit(originalElement.get(0), low - 1, o))
                .append(o.str);
              if (o.title) {
