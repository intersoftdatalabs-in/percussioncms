 ...
 */
  ...
  */
      fromRaw: function (number, model) {
        ...
        return (
          (NumberFormatter.prototype.fromRaw.apply(this, args) || "0") +
          this.symbol
        );
      },
...
*/
  ...
  */
    initialize: function (options) {
      ...
      this.listenTo(
        column,
        "change:editable change:sortable change:renderable",
        function (column) {
 ...
 */
   ...
   */
    initialize: function (options) {
      ...
      if (Backgrid.callByNeed(column.editable(), column, collection))
        $el.addClass("editable");
      if (Backgrid.callByNeed(column.sortable(), column, collection))
    ...
    render: function () {
      ...
      if (sortable) {
        label = $("<button>")
          ...
          .append("<span class='sort-caret' aria-hidden='true'></span>");
      } else {
...
*/
  ...
  */
    sort: function (column, direction) {
      ...
      var comparator = this.makeComparator(
        column.get("name"),
        order,
        order
          ? column.sortValue()
      ...
      if (
        ...
        if (command.moveUp() || command.moveDown()) {
          ...
          if (row) {
            cell = row.cells[j];
            if (
              Backgrid.callByNeed(cell.column.editable(), cell.column, model)
            ) {
              cell.enterEditMode();
...
*/
   ...
   */
    initialize: function (options) {
      ...
      var filteredOptions = _.omit(options, [
        "el",
        "id",
        "attributes",
        "className",
