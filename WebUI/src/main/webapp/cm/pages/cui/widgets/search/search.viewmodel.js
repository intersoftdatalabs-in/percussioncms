...
define(["knockout", "pubsub"], function (ko, PubSub) {
  return function SearchViewModel(options) {
    ...
    self.openSearch = function () {
      $(self.constants.SECONDARY_BUTTON_TEXT_SELECTOR).text(
        self.constants.SECONDARY_BUTTON_NAME
      );
      $(self.constants.PRIMARY_BUTTON_TEXT_SELECTOR).text(
        self.constants.PRIMARY_BUTTON_NAME
      );
      //get Sites
    ...
    function generateSearchCriteria() {
      ...
      if (self.lastModified() !== "")
        searchFields.push({
          ...
          value: self.lastModified(),
        });
      if (self.workflow())
        searchFields.push({ key: "sys_workflowid", value: self.workflow().id });
      if (self.status())
        searchFields.push({
          key: "sys_contentstateid",
          value: self.status().id,
        });
      if (type === "Pages") {
        if (self.template())
        ...
        if (self.assetType())
          searchFields.push({
            ...
            value: self.assetType().contenttypeid,
          });
      }
