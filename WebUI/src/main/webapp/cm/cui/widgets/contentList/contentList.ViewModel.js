...
define(["knockout", "pubsub", "utils"], function (
  ...
  return function ContentListViewModel(options) {
    ...
    self.constants = {
      ...
      MY_BOOKMARKS_EMPTY_MSG:
        "You have not bookmarked any content. To bookmark content, open it and select the <i class='fa fa-star icon'></i> icon.",
      MY_RECENT: "My Recent",
      MY_RECENT_EMPTY_MSG:
        "This page will list the 20 most recent items you've worked on.  Add new content or open content to have it appear on this list.",
      SEARCH_RESULTS: "Search Results",
      SEARCH_RESULTS_EMPTY_MSG:
        "No content was found matching your search criteria.",
      THUMBNAIL_UNAVAILABLE_IMG:
        '<div class="perc-missing-thumb"><i title="Thumbnail is not yet available for this page." class="perc-missing-thumb-i perc-font-icon icon-camera fas fa-camera fa-5x "></i></div>',
    ...
    var item = function (data) {
      ...
      this.lastModifiedDate = ko.observable(
        utils.formatDate(data.lastModifiedDate)
      );
      this.lastPublishedDate = ko.observable(
        utils.formatDate(data.lastPublishedDate)
      );
      ...
      this.scheduledPublishDates = ko.observable(
        utils.formatDate(data.scheduledPublishDate)
      );
      this.scheduledRemovalDates = ko.observable(
        utils.formatDate(data.scheduledUnpublishDate)
      );
      ...
      this.isBookmarkAllowed = ko.computed(function () {
        return (
          utils.getPathType(itemSelf.path()) === "site" &&
          self.currentTab() !== "My Bookmarks"
        );
      });
    ...
    self.showMoreResultsRow = ko.computed(function () {
      return (
        self.currentTab() === self.constants.SEARCH_RESULTS &&
        self.hasMoreResults
      );
    });
    ...
    function getRecentList() {
      self.isLoading(true);
      self.options.cm1Adaptor
        .recentList("non-archived-item")
        .done(function (results) {
          var items = [];
          ko.utils.arrayForEach(
            results.data.ItemProperties,
            function (itemdata) {
              items.push(new item(itemdata));
            }
          );
    ...
    function getSearchResults(searchInfo, showMore) {
      ...
      if (self.searchInfo.id === searchInfo.id && !showMore) {
        ...
        searchInfo.criteria.SearchCriteria.maxResults =
          self.constants.SEARCH_MAX_RESULTS;
        self.options.cm1Adaptor
          .getSearchResults(searchInfo.criteria)
          .done(function (results) {
            var items = [];
            var children = results.PagedItemPropertiesList.childrenInPage;
            searchInfo.criteria.SearchCriteria.startIndex =
              results.PagedItemPropertiesList.startIndex +
              self.constants.SEARCH_MAX_RESULTS;
            self.searchInfo = searchInfo;
            self.hasMoreResults(
              searchInfo.criteria.SearchCriteria.startIndex <=
                results.PagedItemPropertiesList.childrenCount
            );
            ...
            self.searchResults = showMore
              ? self.searchResults.concat(items)
              : items;
            self.contentListItems(self.searchResults);
          })
          .fail(function (message) {
    ...
    function removeItemFromSearchResults(item) {
      if (!Array.isArray(self.searchResults)) return;
      var index = -1;
      ...
      if (index !== -1) self.searchResults.splice(index, 1);
    }
    /**
    ...
    self.launchDelete = function (item) {
      var forceDelCallback = function () {
        self.options.cm1Adaptor
          .forceDeleteItem(item.path(), item.id(), item.name())
          .done(function () {
      ...
      var delCallback = function () {
        self.options.cm1Adaptor
          .deleteItem(item.path(), item.id(), item.name())
          .done(function () {
            removeItemFromSearchResults(item);
            refreshView();
          })
          .fail(function (result) {
            ...
            if (!result.canForceDelete) {
              ...
              var data = {
                message: message,
                forceChkBoxId: result.chkBoxId,
                title: "Confirmation",
                primaryButton: { text: "Yes", callback: forceDelCallback },
              };
      ...
      var data = {
        message: self.constants.DELETE_PROMPT_MESSAGE,
        title: "Confirmation",
        primaryButton: { text: "Yes", callback: delCallback },
      };
    ...
    self.launchBookmark = function (item) {
      self.options.cm1Adaptor
        .bookMarkItem(item.path(), item.id())
        .fail(function (message) {
          PubSub.publish("OpenErrorDialog", message);
        })
        .always(function () {
