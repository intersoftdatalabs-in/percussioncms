...
define(["knockout", "pubsub", "utils", "fancytree"], function (
  ...
  return function PageWizardViewModel(options) {
    ...
    self.emptyRecentFolders = ko.computed(function () {
      return (
        self.selectedFolderFilter() === "Recent" && self.folders().length === 0
      );
    }, self);
    ...
    function validFields() {
      ...
      return (
        ...
        !self.invalidPageTitle() &&
        !self.invalidTemplateSelected() &&
        !self.invalidFolderSelection() &&
        !self.invalidSiteSelection() &&
    ...
    function getRecentTemplates() {
      ...
      self.options.cm1Adaptor
        .getRecentTemplates(self.selectedSite().name)
    ...
    function getAllTemplates() {
      self.isLoading(true);
      self.options.cm1Adaptor
        .getTemplates(self.selectedSite().name)
        .done(function (templates) {
          self.templates([]);
          ko.utils.arrayForEach(templates, function (template) {
            self.templates.push(template);
    ...
    function getRecentFolders() {
      self.isLoading(true);
      self.options.cm1Adaptor
        .getRecentSiteFolders(self.selectedSite().name)
