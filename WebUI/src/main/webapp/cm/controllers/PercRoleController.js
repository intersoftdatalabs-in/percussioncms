...
(function ($) {
  ...
  function selectRole(rolename) {
    ...
    for (u in rolesArrayCache)
      if (rolename == rolesArrayCache[u]) selectedRoleIndex = u;
  ...
  function deleteRole(roleName) {
    userService.deleteRole(roleName, function (status, response) {
      if (status != $.PercServiceUtils.STATUS_SUCCESS) {
        view.alertDialog(
          I18N.message("perc.ui.role.controller@Error Deleting Role") +
            " '" +
            roleName +
            "'.",
          response,
      ...
      updateListOfRoles(function () {
        if (selectedRoleIndex > rolesArrayCache.length - 1) selectedRoleIndex--;
        selectRole(rolesArrayCache[selectedRoleIndex]);
      });
    });
  }
  ...
  function getAvailableUsers(assignedUsers) {
    ...
    if (state == STATE_VIEWING_CURRENT_ROLE) {
      var roleObj = {
        Role: { name: selectedRole, users: currentAssignedUsers },
      };
  ...
  function validateAndDeleteRole(roleObj) {
    userService.validateDeleteRole(roleObj, function (status, validationMsg) {
      var htmlQuestion =
        "<p id='perc-delete-dialog-warning'>" +
        I18N.message("perc.ui.role.controller@Warning Title") +
        "</p>" +
        "<p id='perc-warning-red'> " +
        I18N.message("perc.ui.role.controller@About To Delete Role") +
        selectedRole +
        "'.</p> <br/>" +
        (validationMsg != null
        ...
        I18N.message("perc.ui.role.controller@Are You Sure Delete Role") +
        "</p>";
      var settings = {
  ...
  function validateAndRemoveUsers(usersListToDelete, usersList) {
    ...
    userService.validateDeleteUsers(userList, function (status, validationMsg) {
      if (status == $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        var htmlQuestion =
          "<p id='perc-delete-dialog-warning'>Warning</p><br/>" +
          "<strong>" +
          validationMsg +
          "</strong><br/><br/>" +
          "<p id='perc-delete-warn-msg'>" +
