...
(function ($) {
  ...
  function selectUser(username) {
    ...
    for (u in usersArrayCache)
      if (username == usersArrayCache[u]) selectedUserIndex = u;
  ...
  function deleteUser(username) {
    userService.deleteUser(username, function (status, response) {
      ...
      updateListOfUsers(function () {
        if (selectedUserIndex > usersArrayCache.length - 1) selectedUserIndex--;
        selectUser(usersArrayCache[selectedUserIndex]);
      });
    });
  }
      ...
      if (status == $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        for (u = 0; u < externalUsers.length; u++) {
          ...
          for (c = 0; c < usersArrayCache.length; c++)
            if (
              String(externalUsers[u].name).toLowerCase() ==
              String(usersArrayCache[c]).toLowerCase()
            ) {
              keep = false;
              break;
            }
          if (keep) filteredExternalUsers.push(externalUsers[u]);
        }
  ...
  function importDirectoryUsers(usernames) {
    ...
    userService.importDirectoryUsers(importUsersObj, function (status, data) {
      ...
      if (status == $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        if (showImportWarningFlag) view.showImportWarning(importedUsers);
    ...
    if (dirtyController.isDirty()) {
      ...
      if (state != STATE_ADDING_NEW_USER) {
        ...
        userService.updateUser(
          updatedUserObj,
          function (status, createdUserObj) {
            if (status != $.PercServiceUtils.STATUS_SUCCESS) {
              view.alertDialog(
                I18N.message("perc.ui.user.controller@Error"),
                createdUserObj,
