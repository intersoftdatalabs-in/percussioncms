...
(function ($) {
  ...
  function editCategories(catArray, sitename, success, error) {
    ...
    getTabLockData(function (lockinfo) {
      ...
      if (
        ...
        user !== null &&
        user !== "" &&
        typeof $.PercNavigationManager.getUserName() != "undefined" &&
        user !== $.PercNavigationManager.getUserName()
      ) {
        confirmDialog(
          I18N.message("perc.ui.category.controller@Category tab is locked"),
          user +
            " " +
            I18N.message(
  ...
  function getCategoryJson(catArray) {
    ...
    catJson = {
      title: I18N.message("perc.ui.category.controller@Categories"),
      topLevelNodes: catArray,
    };
    return catJson;
  }
