...
(function ($) {
  ...
  function load(postCallback, clearCache, siteRequest) {
    ...
    if (
      (currentSiteName = $.PercNavigationManager.getSiteName()) != null &&
      siteRequest == null
    ) {
    ...
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.TEMPLATES_ALL + currentSiteName,
      $.PercServiceUtils.TYPE_GET,
      false,
      function (status, result) {
  ...
  function renameTemplate(templateId, name) {
    ...
    for (var tempId in _cache.templates) {
      ...
      if (
        template.containsSite(currentSite) &&
        nameLowerCase == template.getTemplateNameLowerCase()
      ) {
        var defMsg1 =
          "Template name '" +
          name +
          "' is already in use in the current site.\nPlease use a different name for the template." +
  ...
  function getTemplateSitesCallback(status, result, callback, templateid) {
    if (status == $.PercServiceUtils.STATUS_SUCCESS) {
      ...
      var defMsg =
        "Unable to load sites for template " + templateid + "\nError: " + error;
      $.perc_utils.alert_dialog({
        title: "Error",
        content: defMsg,
      });
  ...
  function createTemplateFromUrl(url, postCreationCallback) {
    ...
    $.PercImportProgressDialog({
      backgroundRefreshCallback: function () {
        ...
        load(function loadAndClearCacheCallback() {
      ...
      pollingProgressCallback:
        $.PercTemplateService().createTemplateFromUrlStatus,
