...
(function ($, P) {
  ...
  P.layoutController = function (model) {
    ...
    function addRegion(regionId, direction, callback) {
      model.isResponsiveBaseTemplate()
        ? _addResponsiveRegion(regionId, direction, callback)
        : _addBaseRegion(regionId, direction, callback);
    }
    ...
    function _addResponsiveRegion(regionId, direction, callback) {
      ...
      if (!_isInsertable(parentRegion)) {
        $.perc_utils.alert_dialog({
          title: "Region creation warning",
          content: "Reached maximum allowed columns(12) for the region.",
        });
        callback(null);
        return;
      }
      ...
      if (parentRegion.children && parentRegion.children.length == 0) {
        ...
        newRegion = parentRegion.row
          ? _addToRow(parentRegion, direction, referenceRegion)
          : _addToColumn(parentRegion, direction, referenceRegion);
      }
      callback(newRegion);
    }
    function _addToColumn(parentRegion, direction, referenceRegion) {
      ...
      switch (direction) {
        ...
        case "west":
          ...
          direction == "east"
            ? wrapperRow.children.push(newCol)
            : wrapperRow.children.unshift(newCol);
          newReg = newCol;
          break;
      }
      return newReg;
    ...
    function _addToRow(parentRegion, direction, referenceRegion) {
      ...
      switch (direction) {
        ...
        case "south":
          ...
          var newRow = model.newResponsiveRegion(true);
          direction == "north"
            ? wrapperCol.children.unshift(newRow)
            : wrapperCol.children.push(newRow);
          newReg = newRow;
          break;
      }
      return newReg;
    ...
    function _isInsertable(parentRegion) {
      return !(
        ...
        parentRegion.children.length == 12
      );
    }
    ...
    function _removeRespRegion(regionId, deleteContent) {
      model.editRegion(regionId, function () {
        ...
        model.editRegionParent(regionId, function () {
          ...
          if (
            parentRegion.regionId === model.getRoot().regionId &&
            parentRegion.children.length === 1
          )
            return;
          var onlyChild = parentRegion.children.length === 1;
          var adjColPos = _getRegionPosition(parentRegion, region.regionId);
          adjColPos =
            adjColPos == parentRegion.children.length - 1
              ? adjColPos - 1
              : adjColPos;
    ...
    function _getRegionPosition(parentReg, regId) {
      ...
      if (!found) {
        throw (
          "Failed to find the position of a region with id " +
          regId +
          "as it does not exist under supplied parent region."
        );
      }
      return pos;
    }
    ...
    function _addBaseRegion(regionId, direction, callback) {
      ...
      model.editRegion(regionId, function () {
        ...
        if (model.isPage()) regionAddingTo.owner = "page";
        ...
        if (direction === "center") {
          ...
          if (callback && typeof callback === "function") callback(newRegion);
          return;
        }
        ...
        var split =
          (vertical != regionAddingTo.vertical &&
            regionAddingTo.children.length > 1) ||
          regionAddingTo.children.length === 0;
        ...
        if (split) {
          ...
          regionAddingTo.children = append
            ? [clone, newRegion]
            : [newRegion, clone];
          ...
          if (vertical) {
            ...
            if (direction === "after") {
              ...
              if (append) {
                ...
                region =
                  regionAddingTo.children[regionAddingTo.children.length - 1];
              } else {
          ...
          if (direction === "after" || direction === "below") {
            ...
            if (append) regionAddingTo.children.push(newRegion);
            else regionAddingTo.children.unshift(newRegion);
          }
    ...
    function removeRegion(regionId, deleteContent) {
      model.isResponsiveBaseTemplate()
        ? _removeRespRegion(regionId, deleteContent)
        : _removeBaseRegion(regionId, deleteContent);
    }
    ...
    function removeWidgetParentRegion(widgetId, deleteContent) {
      ...
      if (pRegionId) removeRegion(pRegionId, deleteContent);
    }
        ...
        if (this && !(this.children && this.children.length > 0)) {
          if (!this.widgets || this.widgets.length === 0 || !beforeWidgetId) {
            ...
            if (model.isResponsiveBaseTemplate() && this.row) {
              if (model.isPage()) {
                $.perc_utils.alert_dialog({
                  title: "Add widget warning",
                  content:
                    "Widgets cannot be added to this region due to a template configuration issue.  Please contact your administrator to update the template.",
                });
                return;
              } else {
            ...
            if (!regiontoadd.widgets) regiontoadd.widgets = new Array();
    ...
    function removeWidget(regionId, widgetId) {
      model.editRegion(regionId, function () {
        ...
        if (this.widgets.length === 0) this.owner = "template";
      });
    }
    ...
    function moveWidget(fromRegion, widgetId, toRegion, position) {
      model.editRegion(fromRegion, function () {
        ...
        model.editRegion(toRegion, function () {
          toRegionObject = this;
          if (position < 0 || position > toRegionObject.widgets.length) return;
    ...
    function moveRegion(parentRegionId, movedRegion, newPosition) {
      model.editRegion(parentRegionId, function () {
        ...
        $.each(parentRegion.children, function (ii) {
          if (this.regionId === movedRegion) oldPosition = ii;
        });
