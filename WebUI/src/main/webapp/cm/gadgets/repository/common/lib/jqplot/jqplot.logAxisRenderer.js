...
(function ($) {
   ...
   *  > <script language="javascript" src="plugins/jqplot.logAxisRenderer.js"></script>
   *
  ...
  $.jqplot.LogAxisRenderer.prototype.createTicks = function (plot) {
    ...
    // we don't have any ticks yet, let's make some!
      ...
      rmin = Math.pow(
        this.base,
        Math.floor(Math.log(min) / Math.log(this.base))
      );
      rmax = Math.pow(
        this.base,
        Math.ceil(Math.log(max) / Math.log(this.base))
      );
      ...
      if (this.numberTicks == null) {
        if (dim > 140) {
          numberTicks = Math.round(
            Math.log(this.max / this.min) / Math.log(this.base) + 1
          );
          if (numberTicks < 2) {
            numberTicks = 2;
          }
    ...
    // min and max are set as would be the case with zooming
      ...
      if (this.numberTicks == null && this.tickInterval == null) {
        ...
        var ret = $.jqplot.LinearTickGenerator.bestConstrainedInterval(
          this.min,
          this.max,
          nttarget
        );
