...
(function ($) {
  ...
  $.PercUserView = function () {
    ...
    function init() {
      ...
      $("#perc-users-remove-role-button")
        .off("click")
        .on("click", function () {
          removeRoleFromSelectedUser();
        });
      $("#perc-users-add-role-button")
        .off("click")
        .on("click", function () {
    ...
    function showImportWarning(users) {
      ...
      for (let u = 0; u < users.length; u++) {
        if (users[u].status !== "SUCCESS") {
          if (users[u].status === "ERROR") errorList.push(users[u]);
    ...
    function resetUserDetails() {
      ...
      assignedRolesSelectList
        .on("click", function () {
          availableRolesSelectList.attr("selectedIndex", "-1");
        })
        .html("");
      availableRolesSelectList
        .on("click", function () {
          assignedRolesSelectList.attr("selectedIndex", "-1");
        })
        .html("");
    ...
    function updateListOfUsers(userArray) {
        ...
        .on("click", function (event) {
          ...
          dirtyController.confirmIfDirty(function () {
            ...
            for (username in currentUserIds)
              if (currentUserIds[username] == id) break;
      ...
      $("#perc-username-list ul li div")
        .addClass("perc-user-delete")
        .removeClass("perc-user-delete-disabled")
        .attr("title", "Delete user");
      $("#perc-username-list #" + id + " #" + id)
        .addClass("perc-user-delete-disabled")
        .removeClass("perc-user-delete");
        ...
        .on("click", function (event) {
          ...
          var settings = {
            ...
            question:
              ...
              I18N.message("perc.ui.perc.user.view@Are You Sure Remove User") +
              "" +
              username +
              "</span>",
            success: function () {
    ...
    function updateImportUsersDialog(users) {
      ...
      if (users.length > maxNumberOfUsers) narrowSearchLabel.show();
      else narrowSearchLabel.hide();

      // iterate over the list of users, appending a row for each to the table
      for (i = 0; i < users.length; i++) {
        if (i >= maxNumberOfUsers) break;
    ...
    function updateAvailableRoles(rolesArrayCache, assignedRoles) {
      ...
      if (assignedRoles == null) assignedRoles = [];
    ...
    function removeRoleFromSelectedUser() {
      ...
      if (
        selectedAssignedRoleValue == "Admin" &&
        !controller.isAddingNewUser() &&
      ...
      if (selectedAssignedRoleValue == null) return;
      if (assignedRoleSelected) assignedRoleSelected.remove();
    ...
    function addRoleToSelectedUser() {
      ...
      if (availableRoleSelected) availableRoleSelected.remove();
      ...
      if (addingNewUser)
        for (u = 0; u < currentUserList.length; u++)
          if (
            ...
            currentUserList[u].toString().toLowerCase()
          ) {
            alertDialog(
              I18N.message("perc.ui.labels@Error"),
    ...
    var templateUserStatus =
      "<tr><td class='perc-users-row'>" +
