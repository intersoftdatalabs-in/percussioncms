...
(function ($) {
  ...
  function openDialog(itemId, itemName, itemType) {
    getItemPublishingHistory(itemId)
      .done(function (data) {
        if (data.ItemPublishingHistory.length === 0) {
          $.perc_utils.alert_dialog({
            title:
              I18N.message("perc.ui.publishing.history@No Publishing History") +
              itemType,
            content:
              I18N.message(
    ...
    function createDialog(data) {
      ...
      dialog = $(dialogHTML).perc_dialog({
        resizable: false,
        title:
          I18N.message("perc.ui.publishing.history@Publishing History") +
          ": " +
          itemName +
          " (" +
          pubHistory[0].contentId +
          ")",
        modal: true,
        closeOnEscape: true,
        percButtons: {
    ...
    function addHistoryRows(pubHistory) {
      ...
      for (var i in pubHistory) {
        ...
        var $rowHTML = $(
          "<tr>" +
            "<td style='vertical-align: middle;'><div class='data-cell perc-ellipsis perc-tooltip-container'>" +
            pubEntry.server +
            "<div class='perc-tooltip'>" +
            pubEntry.server +
            "</div></div></td>" +
            "<td style='vertical-align: middle;'><div class='data-cell perc-ellipsis perc-tooltip-container'>" +
            pubEntry.location +
            ...
            pubEntry.operation +
            "</div></td>" +
            "<td style='vertical-align: middle;'><div class='data-cell perc-ellipsis' title='" +
            errorMsg +
            "'>" +
            pubEntry.status +
            "</div></td>" +
