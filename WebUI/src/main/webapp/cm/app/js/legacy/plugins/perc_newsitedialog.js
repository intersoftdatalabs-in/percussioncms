...
(function ($) {
        ...
        if (!$perc_newSiteDialogLayout) {
          ...
          $("input[type=radio]").on("change", function () {
            ...
            if (radioSelected.attr("id") !== "type_url") {
              urlField.attr("readonly", "readonly").attr("disabled", "true");
              queryParamaterField
          ...
          $("#queryParamaterFieldRequired").click(function () {
            if (this.checked) {
              queryParamaterField.removeAttr("readonly").removeAttr("disabled");
            } else {
          ...
          v = $("#perc_newSiteDialogForm").validate({
            ...
            showErrors: function () {
              if (this.pendingRequest < 1) {
                ...
                formDialog
                  .find(".perc_dialog_field.perc_field_success")
                  .prevAll(".perc_dialog_label")
                  .removeClass("perc_dialog_label_error");
                formDialog
                  .find(".perc_dialog_field.perc_field_error")
                  .prevAll(".perc_dialog_label")
                  .addClass("perc_dialog_label_error");
  ...
  function _onNext(callbackHandlerOnCreate) {
    ...
    if (
      dialog.find("input[type=radio]").filter(":checked").attr("id") ===
      "type_url"
    ) {
      var newSite = {
      ...
      $.PercImportProgressDialog({
        ...
        startProgressCallback: function (callbackJobIdHandler) {
          $.PercSiteService.createSiteFromUrlAsync(
            importConfig,
  ...
  function _onOK(cbh) {
    ...
    fields.selectedtemplate =
      $("#perc-select-template-type").val() === "base"
        ? fields.perc_selected_basetemplate
        : fields.perc_selected_resptemplate;
  ...
  function _loadTemplateList() {
    ...
    var respTemplates = $(
      ...
      width: 590,
      baseType: "resp",
      hiddenFieldId: "perc_selected_resptemplate",
    });
  }
  /**
