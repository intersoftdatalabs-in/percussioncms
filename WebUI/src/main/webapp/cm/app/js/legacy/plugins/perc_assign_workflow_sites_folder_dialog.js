...
(function ($) {
  ...
  function createDialog(selectedWorkflow, onSave) {
    ...
    var buttons = {
      ...
      Save: {
        ...
        click: function () {
          var selectedSites = jQuery("#datatree" + "-workflow-sites")
            .fancytree("getTree")
            .getSelectedNodes();
          var selectedAssets = jQuery("#datatree" + "-workflow-assets")
            .fancytree("getTree")
            .getSelectedNodes();
          ...
          function assignFoldersWithWorkflow() {
            $.PercFolderService.assignFoldersWithWorkflow(
              jsonAssignements,
    ...
    dialog.append(
      $('<div id="perc-assign-workflow-sites-title-wrapper"></div>').append(
          ...
          .append(
            $(
              ...
              .on("click", function () {
                ...
                if (
                  combinedClass ==
                  "perc-assign-workflow-expander-image collapsed"
                ) {
                  $(topSpan).attr(
                    "title",
                    I18N.message("perc.ui.workflow.view@Minimize"),
    ...
    dialog.append(
      $('<div id="perc-assign-workflow-assets-title-wrapper"></div>').append(
    ...
    dialog.perc_dialog({
      ...
      resizeStop: function (event) {
        $("#perc-assign-workflow-wrapper").css("maxHeight", "100%");
      },
    });

    // We need to do this since the fancytree needs to be added to the DOM
  ...
  function loadSiteFolders() {
    if (siteJobId == null) return;
    $.PercFolderService.getAssociatedFoldersJobStatus(
      siteJobId,
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_ERROR) {
          $.perc_utils.alert_dialog({
  ...
  function showFolderTrees() {
    if (originalSitesJson != null && originalAssetsJson != null) {
      ...
      $(".perc-loading-warning-message")
        .addClass("perc-loading-warning-message-hidden")
        .removeClass("perc-loading-warning-message");
    }
  }
  /**
  ...
  function swapShowInfoSpans(treeType) {
    if (
      ...
      if (
        (treeType === "sites" &&
          originalSitesJson.folderItem &&
          originalSitesJson.folderItem.length > 0) ||
        (treeType === "assets" &&
          originalAssetsJson.folderItem &&
          originalAssetsJson.folderItem.length > 0)
      )
  ...
  function customOnRender(node) {
    ...
    if (span.find(".fancytree-custom-checkbox").length === 0) {
          ...
          if (
            ...
            if (
              ...
              if ($.inArray(node.data.key, selectedCustomCheckboxes) === -1) {
                selectedCustomCheckboxes.push(node.data.key);
              }
              selectCustomCheckboxRecursive(true, node);
            } else {
      ...
      } else if ($.inArray(node.data.key, selectedCustomCheckboxes) !== -1) {
  ...
  /**
   * Custom onExpand callback. Fancytree's default onExpand removes ous styling.
   * @param node FancytreeNode
   */
  function customOnExpand(node) {
    if (!node.isSelected()) {
    ...
    node.visit(function (childNode) {
      if (!childNode.isSelected()) {
        var other_selected = hasDescendantSelected(childNode);
        if (other_selected) {
          $(childNode.span).addClass("fancytree-partsel");
        } else {
          $(childNode.span).removeClass("fancytree-partsel");
        }
      }
      customOnRender(node);
    });
   ...
   * @param node FancytreeNode to deselect the custom checkbox
   */
  function selectCustomCheckbox(node) {
    $(node.span)
      .find(".fancytree-custom-checkbox")
      .addClass("fancytree-custom-checkbox-selected");
    if ($.inArray(node.data.key, selectedCustomCheckboxes) === -1) {
      selectedCustomCheckboxes.push(node.data.key);
    }
  }
   ...
   * @param node FancytreeNode to deselect the custom checkbox
   */
  function deselectCustomCheckbox(node) {
    $(node.span)
      .find(".fancytree-custom-checkbox")
   ...
   * @param node FancytreeNode to apply the changes and search for child nodes.
   */
  function selectCustomCheckboxRecursive(selected, node) {
    var theChildren = node.getChildren();
    var theChild;
    if (typeof theChildren == "undefined" || theChildren == null) return;
  ...
  /**
   * Cutom onSelect callback. Fancytree's select mode didn't fit the requirements.
   * @param selected true(false) if the checkbox was checked(unchecked)
   * @param node FancytreeNode
   */
  function customOnSelect(selected, node) {
    // Apply grey style to checkboxes and bold to labels
    checkParentNode(node);
    selectCustomCheckbox(node);
  ...
  function checkParentNode(node) {
    span = $(node.span);
    parentSpan = $(node.getParent().span);
    ...
    if (
      node.isSelected() &&
      allMyChildren.length === selectedMyChildren.length
    ) {
   ...
   */
  function hasDescendantSelected(node) {
    if (node.isSelected()) {
      return true;
    }
    ...
    for (var index = 0; index < node.children.length; index++) {
      result = result || hasDescendantSelected(node.children[index]);
    }
    return result;
  }
  ...
  function getAssignedPaths(jsonObject, workflowName, resultArray) {
    ...
    // Base case
    if (
      typeof jsonObject.children == "undefined" ||
      $.isEmptyObject(jsonObject.children)
    ) {
      return;
    }
