...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#perc-servers-wrapper")
      .on("click", "#perc-server-edit", function (evt) {
        loadServerEditor(evt);
      })
      .on("click", "#perc-define-save", function (evt) {
        save(evt);
      });

    $("#perc-servers-container")
      .on("click", "#perc-define-cancel", function (evt) {
        cancel(evt);
      })
      ...
      .on("click", "#perc-full-publish-now", function (evt) {
        publishServer(evt);
      });
  ...
  function addPubServer() {
    ...
    if (
      $("#perc-servers-list").find(".perc-staging-server-marker").length < 1
    ) {
      showServerTypePrompt()
        .done(function (flag) {
          serverType = flag;
          addServer();
        })
  ...
  function showServerTypePrompt() {
    ...
    dialog.find("#perc-staging-server-button").on("click", function (evt) {
      ...
      dialog
        .find("#perc-staging-server-button")
        .addClass("perc-text-button-selected");
      selection = "STAGING";
    });
    dialog.find("#perc-production-server-button").on("click", function (evt) {
      ...
      $(".perc-text-button-selected").removeClass("perc-text-button-selected");
      dialog
        .find("#perc-production-server-button")
        .addClass("perc-text-button-selected");
  ...
  function deleteServer() {
    var previousServerName = $(".perc-itemname[title='" + serverName + "']")
      .prev()
      .attr("title");
    if (typeof previousServerName == "undefined")
      previousServerName = $(".perc-itemname[title='" + serverName + "']")
        .next()
        .attr("title");
    var settings = {
      ...
      type: "YES_NO",
      question:
        I18N.message("perc.ui.publish.view@About To Delete Server") +
        serverName +
        "'<br /><br />" +
        I18N.message("perc.ui.publish.view@Are You Sure Delete Server"),
      success: function () {
        $.PercPublisherService().deleteSiteServer(
          siteId,
          serverId,
  ...
  function loadReadOnlyEditor() {
    ...
    service.getServerProperties(siteId, serverId, function (status, result) {
      if (status) {
        ...
        $("#perc-editor-summary").load(
          "../app/includes/serverEditors/propReadOnlyEditor.jsp?editorName=" +
            driver,
          function () {
  ...
  function _addPublishButtons(serverProperties) {
    if (
      serverProperties.serverInfo.canIncrementalPublish &&
      !serverProperties.serverInfo.isModified
    ) {
  ...
  function _addLastPublishDates(serverInfo) {
    ...
    var pubDates =
      "<li>" +
      "<span class = 'perc-prop-name'>" +
      I18N.message("perc.ui.publish.view@Last Full Publish") +
      "</span>" +
      "<span percName ='fullpublishdate' class = 'perc-prop-value'>" +
      lastFullPublish +
      "</span>" +
      "</li>";
    if (serverInfo.canIncrementalPublish) {
      ...
      pubDates +=
        "<li>" +
        "<span class = 'perc-prop-name'>" +
        I18N.message("perc.ui.publish.view@Last Incremental Publish") +
        "</span>" +
        "<span percName ='incpublishdate' class = 'perc-prop-value'>" +
        lastIncPublish +
        "</span>" +
        "</li>";
    }
  ...
  function _addSplitPublishButton(serverProperties) {
    var dropdownLabels = [
      I18N.message("perc.ui.publish.view@Incremental Publish"),
      I18N.message("perc.ui.publish.view@Full Publish"),
    ];
    var dropdownParams = [];
  ...
  function publishServer(evt) {
    if (serverProperties.serverInfo.isModified) return;
    $.PercBlockUI();
    $.PercPublisherService().publishSite(
      siteName,
      serverName,
      function (status, result) {
        $.unblockUI();
        if (result[1] == "success") {
          ...
          if (
            ...
            msg =
              I18N.message("perc.ui.publish.view@Server") +
              serverName +
              I18N.message("perc.ui.publish.view@Started Publishing");
          }
          ...
          $.perc_utils.alert_dialog({
            content:
              I18N.message("perc.ui.publish.view@Unable Publish Server") +
              serverName,
            title: I18N.message("perc.ui.publish.view@Server Publish"),
  ...
  function showIncrementalPreview(showRelated) {
    $.PercIncrementalPreviewDialog.open(
      siteName,
      serverName,
      showRelated,
      function (action) {
  ...
  function incrementalPublish() {
    if (serverProperties.serverInfo.isModified) return;
    $.PercBlockUI();
    $.PercPublisherService().incrementalPublishSite(
      siteName,
      serverName,
      function (status, result) {
        $.unblockUI();
  ...
  function getServersList(callback) {
    service.getServersList(siteId, function (status, result) {
      if (status) {
        ...
        $.perc_utils.alert_dialog({
          content:
            I18N.message("perc.ui.publish.view@Unable To Get List Of Servers") +
            siteName,
          title: I18N.message("perc.ui.publish.title@Error"),
        });
      }
  ...
  function loadSiteServers(callback) {
    service.getSitePublishProperties(
      siteName,
      function (status, defineDataJson) {
        if (status) {
          ...
          if (defineData != null) {
            ...
            $("#perc-site-id")
              .val(defineData.id)
              .data("secureSite", defineData.secure);
          }
  ...
  validateFields = function (errorObj) {
    if (errorObj.error === "Bad Request") {
      var badFields = JSON.parse(errorObj.request.responseText).ValidationErrors
        .fieldErrors;
      var badFieldsArray = [];
      ...
      if (
        defaultMsg ==
          I18N.message("perc.ui.publish.view@Server No Longer Exists") ||
        defaultMsg ==
          I18N.message("perc.ui.publish.view@One Staging Server Per Site")
      ) {
        $.perc_utils.alert_dialog({
          content: defaultMsg,
          title: I18N.message("perc.ui.publish.title@Error"),
  ...
  save = function () {
    ...
    if (selectedSite != null) {
      ...
      var isDefault = container
        .find('input[percName="isDefault"]')
        .is(":checked");
      var type = container.find("select#publishType").val();
      var ignoreUnModifiedAssets = container
        .find('input[percName="ignoreUnModifiedAssets"]')
        .is(":checked");
      var publishRelatedItems = container
        .find('input[percName="publishRelatedItems"]')
        .is(":checked");
      if (type == "Select") {
        type = "";
      }
      ...
      service.createUpdateSiteServer(
        siteId,
        server,
        serverPropObj,
