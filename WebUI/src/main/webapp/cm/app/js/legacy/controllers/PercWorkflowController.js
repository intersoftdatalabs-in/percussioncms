      ...
      if (status == $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        if (itemStatus.checkOutUser != itemStatus.currentUser) {
          if (itemStatus.assignmentType == "Admin") {
            var options = {
              title: warningDlgTitle[itemType],
              question: _getChkOutDlgContent(0, itemStatus, itemType),
              cancel: function () {
                callback(false);
              },
              success: function () {
                _forceCheckOut(itemId, callback);
              },
              type: "OVERRIDE_OK",
            };
  ...
  function transition(itemId, itemType, transitionName, comment, callback) {
    ...
    var trCb = function (status, results) {
      if (status == $.PercServiceUtils.STATUS_ERROR) {
        ...
        if (
          typeof fassets !== "undefined" &&
          (!Array.isArray(fassets) || !fassets.length) &&
          fassets.length > 0
        ) {
          var type = itemType === "percAsset" ? "asset" : "page";
          var msg =
            I18N.message("perc.ui.workflow.steps.view@Cannot Publish", [type]) +
            "<br/><br/>";
          ...
          msg +=
            "<br/>" + I18N.message("perc.ui.workflow.steps.view@Remove Assets");
...
*/
  ...
  function _getChkOutDlgContent(msgType, itemStatus, itemType) {
    var msg = "";
    var type = "page";
    if (itemType == "percAsset") type = "asset";
    if (msgType == 0) {
      msg = I18N.message("perc.ui.workflow.steps.view@Override", [
        type,
        itemStatus.itemName,
        itemStatus.checkOutUser,
      ]);
    } else if (msgType == 1) {
      msg = I18N.message("perc.ui.workflow.steps.view@Edited By", [
        type,
        itemStatus.itemName,
        itemStatus.checkOutUser,
      ]);
    } else if (msgType == 2) {
      msg = I18N.message("perc.ui.workflow.steps.view@Not Authorized", [
        type,
        itemStatus.itemName,
      ]);
    } else if (msgType == 3) {
  ...
  transitionActions.Reject = {
    name: "Reject",
    cssClass: "perc-wf-reject",
    alt: "Reject",
  };
  transitionActions.Submit = {
    name: "Submit",
    cssClass: "perc-wf-submit",
    alt: "Submit",
  };
  transitionActions.Approve = {
    name: "Approve",
    cssClass: "perc-wf-approve",
    alt: "Approve",
  };
  transitionActions.Resubmit = {
    name: "Resubmit",
    cssClass: "perc-wf-resubmit",
    alt: "Resubmit",
  };
  transitionActions["Quick Edit"] = {
    name: "Quick Edit",
    cssClass: "perc-wf-edit",
    alt: "Edit",
  };
  transitionActions.Archive = {
    name: "Archive",
    cssClass: "perc-wf-archive",
    alt: "Archive",
  };
})(jQuery);
