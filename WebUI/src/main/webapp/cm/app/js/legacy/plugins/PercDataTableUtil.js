...
(function ($) {
  ...
  function convertToDataTableData(colsInfo, pagedItemList) {
    var percData = [];
    if (pagedItemList.PagedItemList.childrenCount < 1) return percData;
  ...
  $.fn.PercPagingBar = function (config) {
    ...
    var first = pageSpan
      .clone()
      .text("<<")
      .addClass("perc-paging-first")
      .data("startIndex", 1);
    var sind =
      (activePage - 2) * pageSize + 1 < 1 ? 1 : (activePage - 2) * pageSize + 1;
    var prev = pageSpan
      .clone()
      .text("<")
      .addClass("perc-paging-prev")
      .data("startIndex", sind);
    sind =
      activePage * pageSize + 1 > totalPages * pageSize
        ? (activePage - 1) * pageSize + 1
        : activePage * pageSize + 1;
    var next = pageSpan
      .clone()
      .text(">")
      .addClass("perc-paging-next")
      .data("startIndex", sind);
    var last = pageSpan
      .clone()
      .text(">>")
      .addClass("perc-paging-last")
      .data("startIndex", (totalPages - 1) * pageSize + 1);
    ...
    for (i = 0; i < pages.length; i++) {
      var clName =
        activePage === pages[i] ? "paginate_active" : "perc-paging-number";
      var pageNum = pageSpan
        .clone()
        .text(pages[i])
        .addClass(clName)
        .data("startIndex", (pages[i] - 1) * pageSize + 1);
