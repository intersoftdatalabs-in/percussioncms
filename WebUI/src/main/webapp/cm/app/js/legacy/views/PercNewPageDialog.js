  ...
  $.PercNewPageDialog = function () {
    ...
    function _openDialog(finderPath, templateId) {
      ...
      dialogHtml =
        ...
        "<fieldset>" +
        "<label for='perc-page-linktext' class='perc-required-field'>" +
        (!templateId
          ? I18N.message("perc.ui.newpagedialog.label@Page link text")
          : I18N.message("perc.ui.newblogpostdialog.label@Post title")) +
        ":</label> <br/> " +
      ...
      if (!templateId) {
        ...
        dialogHtml =
          dialogHtml +
          "<label for='perc-page-title' class='perc-required-field' style='display: none;'>" +
          I18N.message("perc.ui.newblogpostdialog.label@Hidden Post title") +
      ...
      dialogHtml =
        dialogHtml +
        "<label for='perc-page-name' class='perc-required-field'>" +
        (!templateId
          ? I18N.message("perc.ui.newpagedialog.label@Page name")
          : I18N.message("perc.ui.newblogpostdialog.label@Post name")) +
        ":</label> <br/> " +
      ...
      if (!templateId) {
        dialogHtml =
          dialogHtml +
          "<div style='float:left;'><label for='perc-select-template'>" +
          I18N.message("perc.ui.new.page.dialog@Select A Template") +
          "</label><br/>" +
          "  <input list='perc-page-items-datalist' id='perc-page-item-filter' />" +
          "  <datalist id='perc-page-items-datalist'></datalist><br/>" +
          "<a role='button' tabindex='0' title='" +
          ...
          "' class='nextPage browse right' ></a></div>   ";
      } else {
        dialogHtml =
          dialogHtml +
          "<input type='hidden' id='perc-select-template' name='template' value='" +
          templateId +
          "'/>";
      }
      ...
      var dialog = $(dialogHtml).perc_dialog({
        title: !templateId
          ? I18N.message("perc.ui.newpagedialog.title@New Page")
          : I18N.message("perc.ui.newblogpostdialog.title@New Post"),
        buttons: {},
        percButtons: {
          Save: {
            click: function () {
      ...
      var validation = dialog.find("form").validate({
        ...
        submitHandler: function (form) {
          ...
          $.perc_pathmanager.open_containing_folder(
            finderPath,
            function (fspec, path) {
              $.perc_pagemanager.create_new_page(
                path,
                $(form).serializeArray(),
                function (page) {
                  dialog.remove();
      ...
      function _getValidationMessages() {
        var messages = {
          page_linktext: {
            required:
              (!templateId ? "Page link text" : "Post Title") +
              "  is a required field.",
          },
          page_title: {
            required:
              (!templateId ? "Page" : "Hidden Post") +
              " title is a required field.",
          },
          page_name: {
            required:
              (!templateId ? "Page" : "Post") + " name is a required field.",
          },
        };
        return messages;
      }
