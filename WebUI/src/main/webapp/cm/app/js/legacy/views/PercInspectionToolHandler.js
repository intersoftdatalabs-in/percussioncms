...
(function ($, P) {
  ...
  function init(layoutFunctions, model, iframe) {
    ...
    $("#perc-region-tool-inspector").on(
      ...
      function (e) {
        ...
        if (
          !relatedTarget.is("a.perc-dropdown-option") &&
          !relatedTarget.is("span.sf-sub-indicator")
        ) {
  ...
  function applyNonInspectableMarkup() {
    ...
    iframeJQuery(".perc-region.perc-region-leaf")
      .filter(function () {
        var regWidgets = iframeJQuery(this).find(".perc-widget");
        return (
          regWidgets.length !== 1 || //Select if there are more than 1 widget
          ...
          (regWidgets.eq(0).children().length === 1 &&
            regWidgets.eq(0).children().eq(0).is("div") &&
            regWidgets.eq(0).children().eq(0).children().length === 0 &&
            regWidgets.eq(0).children().eq(0).outerHeight(true) === 0 &&
            regWidgets.eq(0).children().eq(0).innerHeight() === 0)
        );
      })
        ...
        iframeJQuery(this)
          ...
          .filter(function () {
            return (
              ...
              iframeJQuery(this).text().trim().length > 0
            );
          })
          .wrap('<span class="perc-itool-text-node-wrapper"/>');

        if (iframeJQuery(this).children().lenght === 1) {
          //Drill into
          var currentElem = iframeJQuery(this);
          while (
            currentElem.children().length === 1 &&
            currentElem.children().eq(0).is("div")
          ) {
          ...
          iframeJQuery(this)
            ...
            .each(function () {
              if (
                iframeJQuery(this).outerHeight(true) === 0 &&
                iframeJQuery(this).innerHeight() === 0
              ) {
  ...
  function clearInspectableMarkup() {
    ...
    iframeContents
      .find(".perc-itool-selectable-elem")
      .removeClass("perc-itool-selectable-elem");
    iframeContents
      .find(".perc-region-itool-unselectable")
      .removeClass("perc-region-itool-unselectable");
    iframeContents
      .find(".perc-itool-region-elem")
      .removeClass("perc-itool-region-elem");
  }
  ...
  function _initGlobalTemplateInspectionRules() {
    var singleFormElementRule = {
      apply: function (inspectableElements) {
        var response = {
          status: false,
          //TODO: I18N TEST
    ...
    var singleNonDivElementRule = {
      apply: function (inspectableElements) {
        ...
        var filteredInspectableElems = inspectableElements.filter(function () {
          ...
          var isOneNonDivElem =
            curElem.children().length === 1 &&
            !curElem.children().eq(0).is("div");
  ...
  function detectAndLogInspectionInfo() {
    ...
    var inspectableElements = _iframe
      .contents()
      .find(".perc-widget[widgetdefid='percRawHtml']:not('.perc-locked')")
      .filter(":only-child")
      .filter(function () {
        var curElem = window.frames[0].jQuery(this);
  ...
  function assignWidgetContent() {
    $.each(splittedRegionContent, function () {
      $.each(this, function () {
        ...
        var widgetDiv = _iframe
          .contents()
          .find('div[widgetid ="' + tempContent.widgetId + '"]');
        try {
        ...
        widgetDiv
          .parents(".perc-region-puff:first")
          .removeAttr("style")
          .addClass("perc-new-splitted-region");
      });
    });
    _layoutFunctions.removeDropsSortingResizing(true);
  ...
  function updateRegionInspectToolButton() {
    ...
    var msg =
      I18N.message("perc.ui.iframe.view@Template Unsaved Changes") +
    ...
    _layoutFunctions.dirtyController.confirmIfDirty(
      function () {
        callbackFunction();
      },
      function () {},
  ...
  function activateInspectionToolButton() {
    ...
    iframeContents
      .find(".perc-new-splitted-region")
      .removeClass("perc-region-puff-gray");
    ...
    iframeContents
      .find(".perc-region:first")
      .addClass("perc-itool-custom-cursor")
      .on("mouseleave", function () {
        highlighterDiv.hide();
        iframeContents
          .find(".perc-itool-highlighter")
          .removeClass("perc-itool-highlighter");
      });
  ...
  function addNonInspectableEvents(
    ...
    iframeContents
      ...
      .on("mouseenter", function () {
        if (iframeContents.find(".perc-itool-selected-elem").length > 0) return;
        var currentElem = iframeJQuery(this);
        currentElem
          .css("background-color", "orange")
          .attr("title", "You cannot further sub-divide this region.")
          .css("cursor", "default")
          .css("z-index", "1000000");
        ...
        iframeContents
          ...
          .addClass("perc-region-puff-gray");
        currentElem
          .closest(".perc-region-puff")
          .removeClass("perc-region-puff-gray");
  ...
  function addInspectableEvents(iframeContents, iframeJQuery, highlighterDiv) {
    ...
    iframeJQuery("body").on(
      "mouseenter",
      ".perc-itool-selectable-elem, .perc-itool-multi-selectable-highlighter",
      function (event) {
        ...
        highlighterDiv
          .height(currentElem.innerHeight())
          .width(currentElem.innerWidth());
  ...
  function itoolSelectionHandler() {
    ...
    var curSelectedElem = iframeContents
      .find(".perc-itool-highlighter")
      .removeClass("perc-itool-highlighter")
      .addClass("perc-itool-selected-elem");
    ...
    if (
      curSelectedElem.prev() &&
      !curSelectedElem.prev().hasClass("perc-itool-selected-elem")
    ) {
      curSelectedElem.prev().addClass("perc-itool-multi-selectable");
    }
    if (
      curSelectedElem.next() &&
  ...
  function highlightSiblings(curSelectedElem) {
    ...
    $(curSelectedElem[0])
      .siblings()
      .filter(":not('.perc-itool-selected-elem')")
      .each(function (index) {
        currentSibling = $(this);
  ...
  function highlightSelectedElements(curSelectedElem, isMultiSelected) {
    ...
    greenborderElem
      ...
      .height(curSelectedElem.innerHeight() - 10)
      .width(curSelectedElem.innerWidth() - 10);
  }
  ...
  function itoolPreviewMouseOver(className, callback) {
    ...
    if (
      ...
      (selElems.length === 1 &&
        selElems.is("div") &&
        iframeJQuery(selElems).hasClass("perc-itools-wrapper")) ||
    ...
    selElems.siblings(":not('.perc-itool-selected-elem')").each(function () {
      if (iframeJQuery(this).is("div")) {
        iframeJQuery(this)
          .addClass("perc-itools-wrapper perc-make-me-region")
          .addClass(className);
      } else {
  ...
  function _divHighlighter(highlighterDiv, selectedElement) {
    ...
    highlighterDiv
      .height(selectedElement.innerHeight() - 10)
      .width(selectedElement.innerWidth() - 10);
  }
  ...
  function clearItoolMarkup() {
    ...
    iframeContents
      .find(".perc-pre-sib-wrapper")
      .removeClass("perc-pre-sib-wrapper");
    iframeContents
      .find(".perc-pre-next-wrapper")
  ...
  function clearItoolSelection() {
    ...
    iframeContents
      ...
      .removeClass("perc-pre-next-wrapper");
    iframeContents
      .find(".perc-itool-region-puff")
      .removeClass("perc-itool-region-puff");
    iframeContents.find(".perc-sibling-div").remove();
    updateInspectToolMenu(false);
    activateInspectionToolButton();
  }
  ...
  function deactivateInspectionToolButton() {
    ...
    _iframe
      .contents()
      .find(".perc-itool-custom-cursor")
      .removeClass("perc-itool-custom-cursor");
  ...
  function updateInspectToolMenu(status) {
    ...
    layoutRegionToolMenu.PercDropdown({
      ...
      percDropdownCallbacks: [
        function () {},
        _splitRegionHorizontal,
        _splitRegionVertical,
      ],
      percDropdownCallbackData: ["", "Stacked", "Side by Side"],
      percDropdownDisabledFlag: [false, status, status],
      percDropdownItemImage: [
        "",
        "../images/images/menuIconStacked.png",
        "../images/images/menuIconSide.png",
      ],
      percDropdownDisabledItemImage: [
        "",
        "../images/images/menuIconStackedGray.png",
        "../images/images/menuIconSideGray.png",
      ],
    });
  }
  ...
  function _splitRegions(isHorizontal) {
    ...
    var parentRegionPuff = selectedEle
      .parents(".perc-region-puff:first")
      .attr("id");
    var oriWidgetId = selectedEle
      .parents(".perc-widget[widgetdefid='percRawHtml']")
      .attr("widgetid");
    var isTransparent = selectedEle
      .parents(".perc-widget[widgetdefid='percRawHtml']")
      .hasClass("perc-widget-transparent");
    var widOwnerId = selectedEle
      .parents(".perc-widget-puff:first")
      .attr("ownerId");
    ...
    while (
      parentElement.not(".perc-widget[widgetdefid='percRawHtml']").length > 0
    ) {
    ...
    var wrappedElems = isHorizontal
      ? iframeJQuery(".perc-itools-wrapper")
      : iframeJQuery(".perc-itools-wrapper").reverse();
    ...
    parentElems.each(function (index) {
      ...
      for (i = 0; i < userRegionClasses.length; i++) {
        ...
        if (uclass.indexOf("perc-") === -1) classesToApply += uclass + " ";
      }
      ...
      _layoutFunctions.layoutController.addRegion(
      ...
      lastParentRegionPuff =
        thisRegionId && thisRegionId !== "" ? thisRegionId : originalRegionId;
    ...
    wrappedElems.each(function (index) {
      ...
      _layoutFunctions.layoutController.addRegion(
        ...
        function (region) {
          _model.editRegion(region.regionId, function () {
            if (regionId && regionId !== "") this.regionId = regionId;
            this.cssClass = classesToApply;
          ...
          var splittedContent = [];
          _layoutFunctions.layoutController.addWidget(
            region.regionId,
            "percRawHtml",
            null,
            function (widgetId) {
