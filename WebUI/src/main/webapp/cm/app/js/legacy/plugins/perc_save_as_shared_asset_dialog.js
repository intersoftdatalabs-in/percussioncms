...
(function ($) {
  ...
  function createDialog(dialogOptions) {
    ...
    dialog_content +=
      '<div class="perc-saveas-shared-asset-label-denotes-required"><label>' +
      I18N.message("perc.ui.general@Denotes Required Field") +
      "</label></div>";
    dialog_content += '<form action="">';
    dialog_content += "<div>";
    dialog_content +=
      '<label for="perc-saveas-shared-asset-dialog-name" class="perc-required-field">' +
      I18N.message("perc.ui.saveassharedassetdialog.label@Name:") +
      "</label>";
    dialog_content +=
      '<input type="text" name="perc-saveas-shared-asset-dialog-name" id="perc-saveas-shared-asset-dialog-name" maxlength="255" /><br/>';
    dialog_content +=
      '<label id="perc-saveas-shared-asset-dialog-name-error" class="perc-saveas-shared-asset-dialog-error perc_field_error"></label>';
    dialog_content += "</div>";
    dialog_content += "<div>";
    dialog_content +=
      '<label id="perc-saveas-shared-asset-dialog-path-label" for="perc-saveas-shared-asset-dialog-path" class="perc-required-field">' +
      I18N.message("perc.ui.saveassharedassetdialog.label@Where:") +
      "</label><br/>";
    dialog_content +=
      '<input type="hidden" name="perc-saveas-shared-asset-dialog-path" id="perc-saveas-shared-asset-dialog-path" />';
    dialog_content +=
      '<div id="perc-saveas-shared-asset-dialog-path-tree"></div><br/>';
    dialog_content +=
      '<label id="perc-saveas-shared-asset-dialog-where-error" class="perc-saveas-shared-asset-dialog-error perc_field_error"></label>';
    dialog_content += "</div>";
    dialog_content += "</form>";
    dialog_content += "</div>";
  ...
  function assignNewAssetToWidget(dialogOptions) {
    var assetName = dialog
      .find("#perc-saveas-shared-asset-dialog-name")
      .val()
      .trim();
    var selectedPath = dialog
      .find("#perc-saveas-shared-asset-dialog-path")
      .val()
      .trim();
    ...
    function validatePathNotExists(okCallback) {
      $.PercPathService.getLastExistingPath(
        selectedPath + "/" + assetName,
        function (status, result) {
          if (
            result ===
            selectedPath.replace($.perc_paths.ASSETS_ROOT + "/", "") +
              "/" +
              assetName
          ) {
            displayError(
