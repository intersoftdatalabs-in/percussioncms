  ...
  $.PercImportProgressDialog = function PercImportProgressDialog(config) {
    ...
    var dialogButtons = {
      ...
      "Close Normal": {
        ...
        click: function () {
          ...
          if (
            config.backgroundRefreshCallback !== undefined &&
            importFinished === true
          ) {
    ...
    function createImportProgressDialog(config) {
      ...
      function handleUnreachableURL() {
        $("#" + CONSTANTS.IDS.MAIN_DIALOG)
          .find("#perc_import_image")
          .attr("height", "315px");
      }

      function showVideoIframe() {
        $("#" + CONSTANTS.IDS.MAIN_DIALOG)
          .find("#perc_import_video")
          .attr("height", "315px");
      }
    ...
    function pollStatus() {
      ...
      config.pollingProgressCallback(
        importJobId,
        function (status, asyncJobStatus) {
          if (
            status !== $.PercServiceUtils.STATUS_SUCCESS ||
            asyncJobStatus.status < 0
          ) {
            setErrorState();
          } else {
            ...
            if (asyncJobStatus.status > importPercentage) {
              ...
              if (
                asyncJobStatus.message !== undefined &&
                asyncJobStatus.message.length > 0
              ) {
                asyncJobStatus.message =
                  asyncJobStatus.message.charAt(0).toUpperCase() +
