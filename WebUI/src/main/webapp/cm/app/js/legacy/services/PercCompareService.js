...
(function ($) {
  ...
  function PercCompareService() {
    ...
    function getRevisionDetails(passedParams) {
      $.PercRevisionService.getRevisionDetails(
        passedParams.itemId,
        function callback(status, result) {
          if (status === $.PercServiceUtils.STATUS_ERROR) {
            var defaultMsg = $.PercServiceUtils.extractDefaultErrorMessage(
    ...
    function convertRevisions(revs, passedParams) {
      ...
      if (isIteratable(revs)) {
        for (let item of revs) {
          allRevisions.set(Number(item.revId), {
            revId: item.revId,
            lastModified: item.lastModifiedDate,
            modifier: item.lastModifier,
            status: item.status,
          });
          lastRevision = item.revId;
        }
      } else if (typeof revs != "undefined") {
        allRevisions.set(Number(revs.revId), {
          revId: revs.revId,
          lastModified: revs.lastModifiedDate,
          modifier: revs.lastModifier,
          status: revs.status,
        });
    ...
    function openComparisonWindow() {
      if (
        params.assemblerRenderer &&
        params.refreshFullPage &&
    ...
    function createWindow() {
      var url =
        "/cm/app/compare.jsp?sys_revision1=" +
        $.percCompareServiceInstance.params.revision1 +
        "&sys_contentid1=" +
        $.percCompareServiceInstance.params.itemId;
      if ($.percCompareServiceInstance.params.assemblerRenderer) {
        url =
          url +
          "&sys_siteid=" +
          $.percCompareServiceInstance.params.siteId +
          "&sys_folderid=" +
          $.percCompareServiceInstance.params.folderId;
      }
    ...
    function loadComparePages() {
      ...
      if (params.revision1) {
        if (params.assemblerRenderer) {
          href1 =
            "/assembler/render?sys_revision=" +
            params.revision1 +
            "&sys_context=0&sys_siteid=" +
            params.siteId +
            "&sys_contentid=" +
            params.itemId +
            "&sys_itemfilter=preview&sys_template=" +
