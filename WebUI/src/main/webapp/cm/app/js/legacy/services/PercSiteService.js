...
(function ($) {
  ...
  function updateSiteProperties(siteProps, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_UPDATE_PROPERTIES,
  ...
  function getSiteProperties(site, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_GET_PROPERTIES + "/" + site,
    ...
    $.PercServiceUtils.makeJsonRequest(
      ...
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
          ...
          var defaultCode = $.PercServiceUtils.extractGlobalErrorCode(
            result.request,
          );
  ...
  function validateCopySiteFolders(postObject, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_COPY_VALIDATE_FOLDERS,
      $.PercServiceUtils.TYPE_POST,
      false,
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
  ...
  function getTemplates(siteName, callback, widgetDefId) {
    ...
    if (widgetDefId != null) {
      sendURL =
        $.perc_paths.TEMPLATES_BY_SITE + "/" + siteName + "/" + widgetDefId;
    } else {
  ...
  function createSiteFromUrl(siteProps, callback) {
    ...
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_CREATE_FROM_URL,
      $.PercServiceUtils.TYPE_POST,
      false,
      function (status, result) {
  ...
  function createSiteFromUrlStatus(siteCreationJobId, callback) {
    var jobId =
      siteCreationJobId.Long === undefined
        ? siteCreationJobId
        : siteCreationJobId.Long;
    $.PercServiceUtils.makeJsonRequest(
  ...
  function getSaaSSiteNames(filterUsedSites) {
    ...
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SAAS_SITES_NAMES + "?filterUsedSites=" + filterUsedSites,
      $.PercServiceUtils.TYPE_GET,
      false,
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
