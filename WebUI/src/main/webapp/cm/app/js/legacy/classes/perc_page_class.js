  ...
  $.Perc_Page_class = function () {
    ...
    this.mergePageXmlIntoTemplateXml = function (templateXml) {
      ...
      for (overridableTemplateRegionId in this.overridableTemplateRegions) {
        ...
        if (pageRegion) {
          var templateRegion =
            this.overridableTemplateRegions[overridableTemplateRegionId];
          templateRegion
            .parent()[0]
            .replaceChild(pageRegion[0], templateRegion[0]);
    ...
    this.clear = function (callback) {
      ...
      $(self.pageXml)
        .find("Page regionBranches regionWidgetAssociations")
        .empty();
      ...
      for (regionId in this.overridableTemplateRegions) {
        ...
        if ($templateRegion && $templateRegion.find("region").length > 0) {
          ...
          if (pageRegion)
            pageRegion
              .parent()[0]
              .replaceChild($templateRegion[0], pageRegion[0]);
          else
            $page
              .find("Page regionBranches regions")
              .append(templateRegions[regionId]);
        }
      }
      ...
      for (regionId in $templateWidgets) {
        ...
        $templateRegion.parents("region").each(function () {
          ...
          if (self.isRegionIdOverridable(parentId))
            $templateRegionWidgetsForPage[regionId] =
              $templateWidgets[regionId];
        });
      }
      ...
      for (var regionId in $templateRegionWidgetsForPage) {
        ...
        if (templateRegionWidget) {
          ...
          if (pageRegionWidget)
            pageRegionWidget
              .parent()[0]
              .replaceChild(templateRegionWidget[0], pageRegionWidget[0]);
          else $pageRegionWidgetAssociations.append(templateRegionWidget);
        }
      }
    };
    ...
    this.findRegionByIdHelper = function (theRegion, args, results) {
      findId = args[0];
      if (theRegion.getId() == findId) results.push(theRegion);
    };
    ...
    this.traverseRegion = function (region, visit, args, results) {
      ...
      if (!region.hasSubRegions()) return;
      else {
