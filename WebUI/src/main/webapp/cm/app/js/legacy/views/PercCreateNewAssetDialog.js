...
(function ($) {
  $.PercCreateNewAssetDialog = function (
    ...
    $.PercAssetService.getAssetEditor(
      assetType,
      assetFolderPath,
      function (status, result) {
        ...
        if ($.PercFinderTreeConstants.FOLDER_ID != null) {
          result.AssetEditor.legacyFolderId =
            $.PercFinderTreeConstants.FOLDER_ID;
        }
    ...
    function createContentEditDialog() {
      ...
      var url =
        assetEditor.url +
        "&sys_folderid=" +
        assetEditor.legacyFolderId +
        "&sys_workflowid=" +
        assetEditor.workflowId;

      var dlgHtml =
        "<div id='create-new-asset-content-dlg'>" +
        "<iframe name='create-new-asset-content-frame' id='create-new-asset-content-frame'" +
        " height='100%' FRAMEBORDER='0' width='100%' src='" +
        url +
        "'></iframe>" +
        "</div>";
  ...
  function saveAssetContent(successCallback) {
    ...
    $("#create-new-asset-content-frame")
      .contents()
  ...
  function saveAssetResponse(event) {
    ...
    $.PercPathService.getPathItemById(assetid, function (status, result) {
      ...
      if (status === $.PercServiceUtils.STATUS_ERROR) {
        ...
        $.perc_utils.alert_dialog({
          title: "Error Selecting Asset",
          content: "The asset has been saved, but failed to select it.",
        });
        $.PercCreateNewAssetDialogData.cancelCallback();
        return;
      }
