...
(function ($) {
    ...
    function prepare_folder_items(callback) {
      return function (pathitems) {
        var specs = $.map(pathitems.PathItem, function (s) {
          ...
          s.path = $.map(paths, function (p) {
            return p;
          });
          s.path_component = paths[paths.length - 1];
          return s;
        });
        specs = $.grep(specs, function (s) {
          return s.type !== "percNavTree" && s.name != ".system";
        });
        callback(specs);
      };
    }
  ...
  function _is_leaf(path, if_leaf, if_folder, if_error) {
    if (path.length <= 2) {
      ...
      var pathclone = $.map(path, function (x) {
        return x;
      });
      ...
      function check_results(specs) {
        ...
        if (specs && specs.PathItemList) {
          ...
          pathItems = Array.isArray(specs.PathItem)
            ? specs.PathItem
            : [specs.PathItem];
        } else {
  ...
  function open_path(path, folder, callback, err, paging) {
    ...
    if (!$.perc_fakes.path_service) {
      ...
      var fakes = {
        ...
        "/Sites/": {
          PathItem: [
            { name: "Test", leaf: "false", path: "/Sites/Test/" },
            { name: "TestTwo", leaf: "false", path: "/Sites/TestTwo/" },
          ],
        },
        "/Sites/Test/": {
          PathItem: [{ name: "Test", leaf: "true", path: "/Sites/Test/Test/" }],
        },
        "/Sites/Test/Test/": {
          PathItem: { name: "Test", leaf: "true", path: "/Sites/Test/Test/" },
        },
        "/Sites/TestTwo/": {
          PathItem: [
            { name: "TestTwo", leaf: "true", path: "/Sites/TestTwo/TestTwo/" },
          ],
        },
        "/Sites/TestTwo/TestTwo/": {
          PathItem: {
            name: "TestTwo",
            leaf: "true",
            path: "/Sites/TestTwo/TestTwo/",
          },
        },
      };
