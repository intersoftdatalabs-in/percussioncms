...
(function ($, P) {
  ...
  P.layoutView = function (iframe, model, layoutController) {
    ...
    regionToolDivHtml +=
      "<div id='_REGION_ID_' class='perc-region-library-tool' name='_REGION_ID_' style='overflow: hidden;white-space: nowrap;text-overflow: ellipsis;'>";
    ...
    iframe
      .contents()
      .find("body")
      .append("<div id='placeholder-region'></div>");
    /**
    ...
    var widgetMenu = [
      ...
      },
      {
        ...
        img: function (elem) {
          ...
          else imgSrc = "/cm/images/icons/editor/configureInactive";

          //if(!widgetDefs[elem.attr('widgetdefid')].hasUserPrefs)
          if (
            layoutModel.isTemplate() ||
            allWidgetDefs[elem.attr("widgetdefid")].hasUserPrefs
          ) {
            if (allWidgetDefs[elem.attr("widgetdefid")].id === "percRawHtml")
              imgSrc = "/cm/images/icons/editor/buttonConfigure2";
            else imgSrc = "/cm/images/icons/editor/configure";
          }
          return imgSrc;
        },
      ...
      },
      {
        ...
        callback: function (widget) {
          ...
          $.perc_utils.confirm_dialog({
            ...
            cancel: function () {},
            success: function () {
              layoutController.convertHTMLWidget(
                widgetObj,
    ...
    var regionMenu = [
      {
        img: function (elem) {
          ...
          else return "/cm/images/icons/editor/delete";
        },
      ...
      },
      {
        img: function (elem) {
          ...
          else return "/cm/images/icons/editor/configure";
        },
        callback: function (region) {
     ...
     */
    var widgetDecorator = P.decorationController(
      allWidgets,
      "perc-widget-puff",
      "perc-widget-active",
      widgetMenu,
    ...
    function saveLayout(afterSave) {
      ...
      layoutModel.save(
        function () {
          if (layoutModel.isTemplate()) $.PercRegionCSSHandler.mergeRegionCSS();
          $.unblockUI();
          callback();
    ...
    function newWidgetObject() {
      ...
      iframe
        .contents()
        .find(".perc-widget[widgetdefid='percRawHtml'][assetid='']")
        .each(function () {
          var curWidget = $(this);
          if (
            curWidget.hasClass("perc-widget-transparent") &&
            curWidget.find(".html-sample-content").length < 1
          ) {
     ...
     */
    $(".perc-dropdown-option-HideGuides")
      .off()
      .on("click", function () {
     ...
     */
    $("#perc-layout-menu a.perc-dropdown-option-DisableJavaScript")
      .off("click")
      .on("click", function () {
        var __this = this;
        ...
        if (dirtyController.isDirty()) {
          ...
          msg +=
            I18N.message("perc.ui.layout.view@Contains Unsaved Changes") +
    ...
    $.perc_finder().addActionListener(function (action, data) {
      ...
      if (
        action === $.perc_finder().ACTIONS.DELETE &&
        (data.type === "asset" || data.type === "page")
      ) {
    ...
    function afterRender(callback) {
      ...
      if (puff) {
        if (!layoutModel.isResponsiveBaseTemplate()) updateWidths();
      ...
      iframe
        ...
        .each(function () {
          ...
          if (subregions.length === 0) region.addClass("perc-region-leaf");
      ...
      percWidgetDropFeedback = iframe
        .contents()
        .find("#perc-widget-drop-feedback");
      ...
      var inEditTemplateView =
        $.PercNavigationManager.getView() ===
        $.PercNavigationManager.VIEW_EDIT_TEMPLATE;
      var visibility =
        checkForErrors() && inEditTemplateView ? "visible" : "hidden";
    ...
    function addRegionToolDrops(region) {
      ...
      $.each(dirs, function (index) {
        if (
          index === AFTER &&
          (!horizontalParent || siblingRegion.length === 0)
        )
          return true;
        if (index === BELOW && (horizontalParent || siblingRegion.length === 0))
          return true;
        if (index === CENTER && isParent) return true;
        ...
        if (index === NORTH || index === SOUTH) {
          ...
          dirs[index].css("height", newHeight).css("width", newWidth);
        }
      });
    }
        ...
        insideIframe(region).sortable({
          ...
          start: function (event, ui) {
            ...
            children.each(function () {
              ...
              if (childHeight > biggestHeight) biggestHeight = childHeight;
      ...
      if (typeof inside.sortable == "function") {
        insideIframe(region).sortable({
          ...
          stop: function (event, ui) {
            ...
            if (
              region.children().children("[widgetid=" + widgetid + "]").length >
              0
            ) {
              // ...then go ahead and move the widget from the region to the new region
              layoutController.moveWidget(
                fromRegionId,
                widgetid,
                toRegionId,
                newPosition,
    ...
    function addResizing(regionResizing) {
      layoutModel.isResponsiveBaseTemplate()
        ? _addRespRegionResizing(regionResizing)
        : _addBaseRegionResizing(regionResizing);
    }
    /**
     * Add region resizing
     */
    function _addRespRegionResizing(regionResizing) {
      ...
      if (
        curRegion.row ||
        (curRegion.columns && curRegion.large === "large-12")
      ) {
      ...
      if (
        position === parentReg.children.length - 1 ||
        (nextRegion != null &&
          curRegion.large === "large-1" &&
          nextRegion.large === "large-1")
      )
        return;
      ...
      insideIframe(regionResizing).resizable({
        ...
        resize: function (event, ui) {
          regionResizing
            .next()
            .width(totalSize - regionResizing.outerWidth() - widthAdj);
        },
        stop: function (event, ui) {
    ...
    function _addBaseRegionResizing(regionResizing) {
      ...
      insideIframe(regionResizing).resizable({
        ...
        start: function (event, ui) {
          if (regionResizing.parent().hasClass("perc-vertical")) return;
          // get the sibling
          rightSiblingRegion = regionResizing.next();
          // if the sibling is not auto resizable, return
          if (getFixedNoAutoResize(rightSiblingRegion) === "true") return;
    ...
    function deleteRegion(regionDiv) {
      ...
      var deletingRegionContentCanBeAdopted =
        layoutModel.isResponsiveBaseTemplate()
          ? respRegionContentCheck(regionDiv)
          : baseRegionContentCheck(regionDiv);
      ...
      else deleteContentChoice.attr("disabled", "disabled");
      ...
      deleteRegionDialog
        ...
        .on("click", function () {
          ...
          var deleteContent =
            $("#perc-delete-content-choice:checked").length !== 0;
    ...
    function baseRegionContentCheck(regionDiv) {
      ...
      var childRegionCount = $(regionDiv)
        .children()
        .children(".perc-region").length;
    ...
    function configureWidget(elem) {
      var widgetId = elem.attr("widgetid"),
        widgetDefinitionId = elem.attr("widgetdefid");
        ...
        iframe
          ...
          .each(function () {
            ...
            if (widgetid in specialWidgets.TransperantWidgets) {
              ...
              layoutModel.isTemplate() &&
              assetid !== "" &&
              !(widgetid in specialWidgets.ContentWidgets)
            ) {
              self.addClass("perc-widget-transparent");
            }
          });
    ...
    function populateWidgetLibrary(event) {
      ...
      $.getJSON(
        $.perc_paths.WIDGETS_ALL +
          "/type/" +
          $(".perc-widget-type").val() +
          "?filterDisabledWidgets=yes",
        function (js) {
    ...
    function addWidgetInfoToolTips(widgetDefs) {
      var cb = function (status, results) {
        ...
        $(results.data.WidgetPackageInfoResult.packageInfoList).each(
          function () {
            var widgetInfo = this;
            ...
            var infoHtml =
              ...
              "\nIs Responsive: " +
              (widgetDef.responsive ? "Yes" : "No") +
              "\nProvider URL: " +
              widgetInfo.providerUrl +
              "\nVersion: " +
              widgetInfo.version +
              "</p>";
      ...
      var widgetIds = [];
      for (var i in widgetDefs) widgetIds.push(i);
    ...
    function filterWidgetLibrary(event) {
      $.each($(".perc-widget-list .perc-widget-tool"), function () {
        if (containsCategory(this)) $(this).parent("a").show();
        else $(this).parent("a").hide();
      });
      $(".perc-widget-list").width("auto");
      fixIframeHeight();
    }
    ...
    function containsCategory(widget) {
      ...
      if (typeof $(widget).data("widget").category == "undefined") {
        return false;
      }
    ...
    function populateRegionLibrary() {
      ...
      for (let r = 0; r < regionIdsArray.length; r++) {
        regionLibraryHtml += regionToolDivHtml
          .replace(/_REGION_ID_/g, "perc-re-" + regionIdsArray[r])
          .replace(/_REGION_LABEL_/g, regionIdsArray[r]);
      }
      ...
      $(".perc-region-library-tool").on("click", function () {
        var regionTool = $(this);
        $(".perc-region-library-tool").removeClass("perc-region-active");
    ...
    function regionOver(region) {
      ...
      if (!widgetStacks[regionId]) widgetStacks[regionId] = [];
      updateDropFeedback();
    }
    ...
    function regionOut(region) {
      var regionId = region.attr("id");
      regionQueue.shift();
      if (widgetStacks[regionId]) delete widgetStacks[regionId];
      updateDropFeedback();
    }
    ...
    function updateDropFeedback() {
      ...
      if (currentWidgetId && currentRegionId) {
        ...
        if (
          ...
          typeof widget.offset() == "undefined"
        ) {
    ...
    function checkForErrors() {
      ...
      var percJSErrors =
        document.getElementById("frame").contentWindow.percGlobalErrors;
