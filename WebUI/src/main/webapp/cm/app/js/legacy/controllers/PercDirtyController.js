...
(function ($) {
  ...
  $.PercDirtyController = {
    ...
    setConfirmDialogOptions: function (callback, errorCallback, options) {
      ...
      if (self.isSaveOptionAllowed()) {
        settings.dontSaveCallback =
          options && options.dontSaveCallback
            ? function () {
                self.setDirty(false);
                options.dontSaveCallback();
              }
            : settings.success;
        ...
        settings.question =
          "You will lose your edits if you do not save. Save your edits before leaving this tab?";
    ...
    isSaveOptionAllowed: function () {
      ...
      return (
        view == $.PercNavigationManager.VIEW_EDIT_TEMPLATE ||
