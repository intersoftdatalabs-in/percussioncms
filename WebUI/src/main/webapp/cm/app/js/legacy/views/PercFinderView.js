...
(function ($, P) {
  P.PercFinderView = function () {
    ...
    var chooseColumnView = $("#perc-finder-choose-columnview").on(
      "click",
      function (evt) {
        setViewColumn(evt);
      },
    ...
    var chooseListView = $("#perc-finder-choose-listview").on(
      "click",
      function (evt) {
        setViewList(evt);
      },
    );
    var chooseSearchView = $("#perc-finder-search-submit").on(
      "click",
      function (evt) {
        setViewSearch(evt);
      },
    );
    var chooseMyPagesView = $("#perc-finder-choose-mypagesview").on(
      "click",
      function (evt) {
        setMyPagesView(evt);
      },
    ...
    $("#perc-finder-item-search")
      ...
      .on("keyup", function (evt) {
        ...
        if (evt.keyCode === 27 || evt.keyCode === 9) {
          $("#perc-finder-item-search")
            .css("color", "#CCCCCC")
            .css("background-color", "#41434F")
            .trigger("blur");
          evt.preventDefault();
          evt.stopPropagation();
        }
        $(".perc-finder-message").fadeOut(function () {
          $(this).empty();
        });
      });
    ...
    function pagePrevious(event) {
      percFinderListviewContainer = $(".perc-finder").find(
        "#perc-finder-listview",
      );
      percFinderListviewContainer.data(
        "startIndex",
        percFinderListviewContainer.data("startIndex") - MAX_RESULTS,
      );
    ...
    function pageNext(event) {
      var percFinderListviewContainer = $(".perc-finder").find(
        "#perc-finder-listview",
      );
      percFinderListviewContainer.data(
        "startIndex",
    ...
    function refreshView() {
      ...
      percFinderListviewContainer.append(
        $(
      ...
      percFinderListviewContainerInitialHeight =
        percFinderListviewContainer.height();
      if (currentFinderView === PERC_FINDER_VIEW_LIST) {
        ...
        currentFinderView === PERC_FINDER_SEARCH_RESULTS ||
        currentFinderView === PERC_FINDER_SEARCH_TYPE_MYPAGES
      ) {
    ...
    function fillPagingBar(dir) {
      ...
      if (totalResult !== 0)
        itemText =
          startIndex + " - " + endIndex + " of " + totalResult + " Total";
      ...
      if (totalResult - (startIndex - 1 + MAX_RESULTS) > 0)
        pagingBar
          .find(".perc-pagingbar-next")
          .removeClass("perc-disabled-navigator")
          .off("click")
        ...
        pagingBar
          .find(".perc-pagingbar-next")
          .addClass("perc-disabled-navigator")
          .off("click");

      //Check if have previous items
      if (startIndex > MAX_RESULTS)
        pagingBar
          .find(".perc-pagingbar-previous")
          .removeClass("perc-disabled-navigator")
          .off("click")
          .on("click", function (evt) {
            pagePrevious(evt);
    ...
    function showPagingBar(dir) {
      if (
        currentFinderView === PERC_FINDER_VIEW_LIST ||
        currentFinderView === PERC_FINDER_SEARCH_RESULTS
      ) {
        var finderDiv = $(".perc-finder");
        //Generate the HTML Bar
        pagingBar = $('<div class="perc-pagingbar-finder"/>')
          .append(
            $('<div class="perc-pagingbar-navigator" />')
              .append(
                $('<a class="perc-pagingbar-previous" />').attr(
                  "title",
                  "Previous",
                ),
              )
              .append(
                $('<a class="perc-pagingbar-next"/>').attr("title", "Next"),
              ),
          )
          .append($('<span class="perc-pagingbar-items" />'));
      ...
      if (
        currentView === $.PercNavigationManager.VIEW_SITE_ARCH ||
        currentView === $.PercNavigationManager.VIEW_USERS
      ) {
        fixIframeHeight();
        fixBottomHeight();
        fixHeight();
    ...
    function fillListView(config, callback) {
      ...
      $.PercPathService.getDisplayFormat(function (status, displayFormat) {
        ...
        $.PercPathService.getContentForPath(
          displayFormat.SimpleDisplayFormat,
          config,
          function (status, content) {
            if (status) {
              percFinderListviewContainer.PercFinderListView(config, content);
              var newPath = $("#mcol-path-summary").val().trim();
              $.PercPathService.getPathItemForPath(
                newPath,
                function (status, content) {
                  if (
                    content.PathItem.type === "Folder" ||
                    content.PathItem.type === "FSFolder"
                  ) {
              ...
              var error = $(
                "<span style='font-weight: normal; margin-top: 15px; display:block'/>",
              ).text(result);
              $("#perc-finder-listview")
    ...
    function fillResultView(config, callback) {
      ...
      $.PercPathService.getDisplayFormat(function (status, displayFormat) {
        config.displayFormat = displayFormat;
        config.searchCriteria.SearchCriteria.formatId =
          displayFormat.SimpleDisplayFormat.id;
        config.folderDblClickCallback = function (contentPath) {
          _updateListViewContainerData(contentPath, 1, updatePagingBar);
        };
        $.PercSearchService.getSearchResult(
          config.searchCriteria,
    ...
    function performSearch() {
      if (currentFinderView === PERC_FINDER_SEARCH_RESULTS) {
        ...
        var encodedSearchText = encodeURIComponent(
          $("#perc-finder-item-search").val().trim(),
        );
        var percFinderListviewContainer = $(".perc-finder").find(
          "#perc-finder-listview",
        );
    ...
    function setView(view, forceSet) {
      ...
      if (view === currentFinderView && !force) return;

      if (
        (currentFinderView === PERC_FINDER_SEARCH_RESULTS &&
          view === PERC_FINDER_VIEW_LIST) ||
        (currentFinderView === PERC_FINDER_SEARCH_TYPE_MYPAGES &&
          view === PERC_FINDER_VIEW_LIST)
      )
        return;
      ...
      if (view === PERC_FINDER_VIEW_COLUMN) {
        ...
        _updateListViewContainerData(
          contentPath,
          lastColumn.data("startIndex"),
          showPagingBar,
        );
        setListViewButtonOn();
      } else if (
        view === PERC_FINDER_SEARCH_RESULTS ||
        view === PERC_FINDER_SEARCH_TYPE_MYPAGES
      ) {
        ...
        if (view === PERC_FINDER_SEARCH_TYPE_MYPAGES) {
          percFinderListviewContainer.data(
            "searchType",
            PERC_FINDER_SEARCH_TYPE_MYPAGES,
          );
        }
        refreshView();
     ...
     */
    function _updateListViewContainerData(
      contentPath,
      startIndex,
      pagingBarCallback,
    ) {
    ...
    function setListViewButtonOn(view) {
      $("#perc-finder-choose-view #perc-finder-choose-columnview").removeClass(
        "ui-enabled",
      );
      $("#perc-finder-choose-view #perc-finder-choose-listview").addClass(
        "ui-enabled",
      );
    }
    ...
    function setColumnViewButtonOn() {
      $("#perc-finder-choose-view #perc-finder-choose-columnview").addClass(
        "ui-enabled",
      );
      $("#perc-finder-choose-view #perc-finder-choose-listview").removeClass(
        "ui-enabled",
      );
    }
    ...
    function openAsset(assetId, isEditMode) {
      $.PercRecentListService.setRecent(
        ...
        .done(function () {
          $.perc_utils.info(
            I18N.message("perc.ui.finder.view@Added Asset") +
              assetId +
              I18N.message("perc.ui.finder.view@Recent Item List"),
          );
        })
      ...
      var handler = isEditMode
        ? $.PercAssetController.getAssetEditorForAssetId
        : $.PercAssetController.getAssetViewForAssetId;
      handler(assetId, function (status, assetEditorUrl) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
          $.PercIFrameView.renderAssetEditor(
            finder,
            null,
            assetEditorUrl,
            null,
          ...
          $("#perc-revisions-button")
            .off("click")
            .perc_button()
            .removeClass("ui-meta-pre-disabled")
            .addClass("ui-meta-pre-enabled")
            .on("click", function () {
              var isEditMode =
          ...
          //{
          $.PercItemPublisherService.getPublishActions(
            assetId,
            function (status, result) {
              if (status) {
                var pubActions = eval(
                  ...
                  publishNowDropdown.PercDropdown({
                    ...
                    percDropdownCallbackData: [
                      ...
                      },
                      {
                        ...
                        aName: aName,
                        trName: I18N.message(
                          "perc.ui.finder.view@Remove From Staging",
                        ),
                      },
                      assetId,
                    ],
          ...
          if (
            assetEditorUrl.indexOf(
              "must be checked out by the current user",
            ) !== -1
          ) {
            dlgTitle = I18N.message(
              "perc.ui.webmgt.contentbrowser.warning.title@Open Asset",
            );
            dlgContent = I18N.message(
              "perc.ui.webmgt.contentbrowser.warning@Asset Overridden",
              [contentName],
            );
            ...
            dlgContent = I18N.message(
              "perc.ui.webmgt.contentbrowser.warning@Asset Deleted",
              [contentName],
            );
          }
    ...
    function _openRevisions(assetId, assetName, isEditMode) {
      var mode = isEditMode
        ...
        : $.PercRevisionDialog.ITEM_MODE_EDIT;
      $.PercRevisionDialog.open(
        assetId,
        assetName,
        $.PercRevisionDialog.ITEM_TYPE_ASSET,
        mode,
      );
    }
    ...
    function _openPublishingHistory(assetId, assetName) {
      $.PercPublishingHistoryDialog.open(
        assetId,
        assetName,
        $.PercPublishingHistoryDialog.ITEM_TYPE_ASSET,
      );
    }
    ...
    function doIfCheckedOutToCurrentUser(contentId, yesCallback, noCallback) {
      $.PercWorkflowController().isCheckedOutToCurrentUser(
        contentId,
        function (result) {
          if (result) {
            yesCallback();
    ...
    function addTransitionButtons(itemType) {
      ...
      function addSaveAndCloseButtons() {
        ...
        if (
          ...
          var saveButton =
            '<button style="float: right;" name="perc_wizard_save" title="Save" class="btn btn-primary" id="perc-save-content">Save</button>';
        ...
        if (
          ...
          var editButton =
            '<button name="perc_page_edit" title="Edit" class="btn btn-primary" id="perc-page-edit">' +
            I18N.message("perc.ui.finder.view@Edit") +
            "</button>";
                ...
                .on("click", function () {
                  ...
                  if (
                    ...
                    if (item.name === "Sites") {
                      $.PercPathService.getPathItemById(
                        ...
                        function (status, data) {
                          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
                            $.PercNavigationManager.handleOpenAsset(
                              data.PathItem,
                              true,
                            );
                          } else {
                    ...
                    var eMsg = I18N.message(
                      "perc.ui.finder.view@Cannot Open Unknown View",
                    );
                    $.perc_utils.alert_dialog({
        ...
        var closeButton =
          "<button class='btn btn-primary' id='perc-page-close' title='Close'>" +
                  ...
                  function () {
                    confirm_if_dirty(function () {
                      ...
                      if (
                        ...
                        $.PercWorkflowController().checkIn(
                          contentId,
                          function (status) {
                            contentId = null;
                            $.PercNavigationManager.goTo(
      ...
      function addCommentsIcon() {
        $.PercRevisionService.getLastComment(
          contentId,
          function (status, result) {
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              if (result.data && result.data.length > 0) {
                var commentIcon = $(
                  "<a style='float: right;' tooltip='" +
                    result.data +
                    "' class='perc-last-comment-menubar'><span class='perc-font-icon icon-comment'/></a>",
                );
                commentIcon.tooltip({
                  delay: 500,
                  left: -150,
                ...
                $.perc_utils.info(
                  I18N.message("perc.ui.finder.view@No Comment"),
                );
              }
            } else {
              $.perc_utils.info(
                I18N.message("perc.ui.finder.view@Failed To Get Comment Info") +
                  contentId +
                  I18N.message("perc.ui.finder.view@See Server Log"),
              );
            }
          },
        );
      ...
      if (
        ...
        $.PercWorkflowController().getTransitions(
          contentId,
            ...
            if (status) {
              ...
              $.each(results, function (index) {
                if (typeof results[index] !== "undefined") {
                  ...
                  var trNameNormal = trName
                    .toLowerCase()
                    .replace(/[^a-zA-Z0-9\/]/g, "_");
                  var trId = "perc_item_transition_" + trNameNormal;
                  var baseImageName =
                    "/cm/images/images/splitButtonWf" + trNameNormal;
                  var imageExt = ".gif";
                  ...
                  if (index === 0) {
                    ...
                    dropdownButtonImage =
                      "/cm/images/images/splitButtonArrow.gif";
                    dropdownButtonImageOver =
                      "/cm/images/images/splitButtonArrowOn.gif";
                  }
                  var param = {
                    name: trName,
                    contentId: contentId,
              ...
              if (dropdownActions.length > 0) {
                ...
                pageWorkflowDropdown
                  .append(
                    $("<button />")
                      .html(
                        I18N.message(
                          "perc.ui.edit.workflow.step.dialog@" +
                            $.perc_textFilters.IDNAMECDATA(dropdownLabels[0]),
                        ),
                      )
                      .css("display", "inline-block")
                      .addClass(
                        "btn btn-primary perc-workflow-split-button-left perc-workflow-split-button-" +
                          $.perc_textFilters.IDNAMECDATA(dropdownLabels[0]),
                      ),
                  )
                  .append(
                    $("<div />")
                      .addClass("perc-workflow")
                      .css("display", "inline-block"),
                  );
    ...
    finder.addPathChangedListener(function (p) {
      ...
      if (currentFinderView === PERC_FINDER_VIEW_COLUMN) return;
      if (finder.flagChangeView) setView(PERC_FINDER_VIEW_COLUMN);
    });

    $.PercNavigationManager.registerFinder(finder);
    $.PercNavigationManager.addLocationChangeListener(
      function (url, id, notifyComplete, params) {
        ...
        if (
          ...
          var options = {
            ...
            content:
              I18N.message("perc.ui.finder.view@The") +
              type +
              " '" +
              $.PercNavigationManager.getName() +
              I18N.message("perc.ui.finder.view@Already Open"),
          };
    ...
    finderButtons = $.percFinderButtons().createButtons(
      finder,
      percFinderViewAPI,
    );
    ...
    function checkForMigrationWarnings(contentId) {
      ...
      function onPageHasEmptyMigrationWidgets() {
        ...
        $.perc_utils.alert_dialog({
          ...
          content: I18N.message(
            "perc.ui.finder.view@Content Migration Failure",
          ),
          okCallBack: function () {
      ...
      $.PercPageService.checkForEmptyMigrationWidgets(
        contentId,
        onPageHasEmptyMigrationWidgets,
      );
    }
    if (
      type === $.PercNavigationManager.PATH_TYPE_PAGE &&
      contentId &&
      contentName
    ) {
      ...
      if (pageMode === $.PercNavigationManager.MODE_EDIT) {
        $.PercWorkflowController().checkOut(
          "percPage",
          contentId,
          function (status) {
            if (!status) {
      ...
      $("#perc-page-button")
        .html("Asset:")
        .append(
          "<span id='perc-pageEditor-menu-name' title = " +
            contentName +
            "> " +
            contentName +
            "</span>",
      ...
      if (pageMode === $.PercNavigationManager.MODE_EDIT) {
        ...
        $.PercWorkflowController().checkOut(
          ...
          function (status) {
            if (status) {
              //Workflow controller presents the appropriate error message to the user if fails to check out.
    ...
    function handlePageWorkflowDropdownAction(params) {
      ...
      confirm_if_dirty(function () {
        var type =
          view === $.PercNavigationManager.VIEW_EDIT_ASSET ? "asset" : "page";
        doIfItemExists(
          params.contentId,
          function () {
            doIfCheckedOutToCurrentUser(
    ...
    function showCommentsDialog(pageId, itemType, trName) {
      ...
      var userComment = "";
      var commentDialog = $(
        "<div><div class='perc-workflow-comment-label' data='" +
          trName +
          "'>" +
    ...
    function doTransition(contentId, itemType, trName, userComment) {
      $.PercBlockUI();
      $.PercWorkflowController().transition(
        contentId,
        itemType,
        trName,
        userComment,
    ...
    function checkIfLinkedPage(pageId, itemType, type, trName) {
      if (type === "page" && trName === "Archive") {
        var findLinkedItemsUrl =
          $.perc_paths.ITEM_LINKED_TO_ITEM + "/" + pageId;
        ...
        $.PercServiceUtils.makeJsonRequest(
          ...
          function (status, result) {
            if (status === $.PercServiceUtils.STATUS_ERROR) {
              var defaultMsg =
                $.PercServiceUtils.extractDefaultErrorMessage(result);
              console.error(defaultMsg);
              showCommentsDialog(pageId, itemType, trName);
            } else {
              if (
                result.data != null &&
                result.data.ArrayList != null &&
                result.data.ArrayList.length > 0
              ) {
                relatedLinkArchiveConfirm(
                  result.data,
                  pageId,
                  itemType,
                  trName,
    ...
    function createDialogQuestion(data) {
      var dialog =
        I18N.message("perc.ui.publish.question@Remove From Site") +
        "<br /><br />";
    ...
    function _confirmPublish(scheduleDates) {
      ...
      if (startDate !== "") {
        var settings = {
          ...
          question:
            I18N.message("perc.ui.finder.view@Item Scheduled Published") +
            startDate +
            I18N.message("perc.ui.finder.view@Continue To Publish"),
          success: function () {
            $.PercBlockUI();
            $.PercItemPublisherService.publishItem(
              itemId,
              itemType,
              _afterPublish,
            );
              ...
              $.PercItemPublisherService.getScheduleDates(
                ...
                function (status, result) {
                  if (status) {
                    ...
                    $.perc_utils.alert_dialog({
                      content: I18N.message(
                        "perc.ui.finder.view@Get Saved Schedule",
                      ),
                      title: I18N.message("perc.ui.publish.title@Error"),
                    });
                    return false;
    ...
    function _afterPublish(success, results) {
      ...
      if (!success) {
        ...
        if (
          SitePublishResponse.status ===
          $.PercItemPublisherService.PUBLISHER_JOB_STATUS_FORBIDDEN
        ) {
          $.perc_utils.alert_dialog({
          ...
          $.PercItemPublisherService
            .PUBLISHER_JOB_STATUS_BADCONFIG_MULTIPLE_SITES
        ) {
          $.perc_utils.alert_dialog({
            title: I18N.message("perc.ui.page.general@Warning"),
            content: I18N.message(
              "perc.ui.publish.errordialog.message@Bad configuration multiple sites",
              [SitePublishResponse.warningMessage],
            ),
          });
        } else if (
          SitePublishResponse.status ===
          ...
          $.perc_utils.alert_dialog({
            title: I18N.message("perc.ui.finder.view@Server Publish"),
            content: I18N.message(
              "perc.ui.finder.view@No Staging Servers Available",
            ),
          });
        } else if (
          typeof SitePublishResponse.warningMessage != "undefined" &&
          SitePublishResponse.warningMessage !== ""
        ) {
          $.perc_utils.alert_dialog({
            title: I18N.message("perc.ui.page.general@Warning"),
