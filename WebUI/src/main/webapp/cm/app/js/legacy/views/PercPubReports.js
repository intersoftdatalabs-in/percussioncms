...
(function ($) {
  ...
  publishCurrentSite = function () {
    ...
    this.service.publishSite(siteVal, function (status, result) {
      if (result[1] === "success") {
        $.perc_utils.alert_dialog({
          content:
            I18N.message("perc.ui.perc.pub.reports@Site") +
            siteVal +
            I18N.message("perc.ui.perc.pub.reports@Started Publishing"),
          title: I18N.message("perc.ui.perc.pub.reports@Site Publish"),
        ...
        $.perc_utils.alert_dialog({
          content:
            I18N.message("perc.ui.perc.pub.reports@Unable To Publish") +
            siteVal,
          title: I18N.message("perc.ui.perc.pub.reports@Site Publish"),
  ...
  publishCurrentStatus = function () {
    ...
    function renderLogs(results) {
      var logString =
        "<table class='perc-table-statusofcurrentjobs' style='width:100%;'><thead><tr>" +
      ...
      if (resLength == 0) {
        logString +=
          "<tr><td>No jobs currently running</td>" +
            ...
            },
            {
              fnRender: function (oObj) {
                if (oObj.aData[4] > 0) {
                  ...
                  return (
                    "<div class = 'perc-que-bar' style = 'width:" +
                    barWdith +
                    "%'>" +
                    oObj.aData[4] +
                    "% Completed</div>"
  ...
  publishLogs = function () {
    ...
    function renderLogs(results) {
      var logString =
        "<table class='perc-table-publishinglog' style='width:100%;'><thead><tr>" +
        "<th style='text-align: left;' class='perc-header-status'>" +
        I18N.message("perc.ui.publish.title@Status") +
        "</th>" +
      ...
      if (results.length == 0) {
        logString +=
          "<tr><td>No logs currently exist</td>" +
          "<td class = 'perc-ellipsis'></td>" +
          "<td class = 'perc-ellipsis'></td>" +
          "<td class = 'perc-ellipsis'></td>" +
          "<td class = 'perc-ellipsis'></td>" +
  ...
  getLogDetails = function (val, publishRecord) {
    this.service.getPublishingLogDetails(val, function (status, result) {
      if (status) {
        ...
        if (
          $("#perc-publish-log-details-widget").find(".perc-opened").length ===
          0
        ) {
          $("#perc-publish-log-details-widget")
            .find(".perc-foldable")
            .trigger("click");
        }
      } else {
    ...
    function renderLogs(results, publishRecord) {
      ...
      var logString =
        "<table class='perc-publish-detail-first-table' width='50%'>" +
        "<tr>" +
        "<td>Site Name:</td>" +
        "<td>" +
      ...
      logString +=
        "<table class='perc-identifier-next' style='width:100%;'><thead><tr style='text-align:left;'>" +
      ...
      $("#perc-publish-logs-details")
        .find(".perc-identifier-next")
        .dataTable({
          bFilter: false,
          bInfo: false,
          bPaginate: false,
