...
(function ($) {
  $.PercEditWorkflowStepDialog = function () {
    ...
    var dialogApi = {
      open: open,
      tempPossiblePermissions: [
        I18N.message("perc.ui.edit.workflow.step.dialog@Submit"),
        I18N.message("perc.ui.edit.workflow.step.dialog@Reject"),
        I18N.message("perc.ui.edit.workflow.step.dialog@Approve"),
        I18N.message("perc.ui.edit.workflow.step.dialog@Publish"),
        I18N.message("perc.ui.edit.workflow.step.dialog@Notify"),
      ],
    };
    ...
    function open(
      ...
      var dialog;
      var className = isReservedStep
        ? "perc-step-readonly"
        : "perc-step-editable";
      var isreadOnly = isReservedStep ? 'readonly = ""' : "";
      //TODO:I18N Dialog Title Below
      var dialogTitle = currentStep
        ? I18N.message("perc.ui.workflow.steps.view@Configure Step")
        : I18N.message("perc.ui.workflow.steps.view@New Step");
      var buttons = {};
      dialog = $(
        " <div id='perc-workflow-addstep-dialog'>" +
          "     <div id='perc-wfconfig-wrapper'>" +
          "         <span style='position: relative; float: right; margin-top: -28px;'><label>" +
          I18N.message("perc.ui.general@Denotes Required Field") +
          "</label></span>" +
          "         <div class = 'perc-required-field'>" +
          I18N.message("perc.ui.edit.workflow.step.dialog@Step Name") +
          "<br/><input maxlength = '50' required pattern='^[a-zA-Z0-9-_\\. ]{1,50}$' title='1 to 50 letters,numbers,-, _, or space' name='perc-wfstep-name' id='perc-wfstep-name' class = '" +
          className +
          "' type='text'" +
          isreadOnly +
          "/></div>" +
          "         <span style='color:red'></span>" +
          "         <div><div style='font-weight:bold; margin-bottom:5px'>" +
          I18N.message("perc.ui.edit.workflow.step.dialog@Roles") +
          "</div>" +
          "         <div >" +
          I18N.message(
      ...
      function renderRoles(rolesJson, isUpdate, workflowStep) {
        ...
        if (isUpdate) possiblePermissions = stepPermissions.concat(tempNotify);
        else
          possiblePermissions =
            $.PercEditWorkflowStepDialog().tempPossiblePermissions;
        ...
        for (var i = 0; i < roleListObject.RoleList.roles.length; i++) {
          ...
          for (var j = 0; j < possiblePermissions.length; j++) {
            ...
            if (
              (possiblePermissions[j] === "Notify" ||
                possiblePermissions[j] === "Publish") &&
              !isReadOnly
            ) {
        ...
        $(".perc-checkall").on("change", function () {
          var inputClass =
            ".perc-" + $(this).attr("data") + " input:not(:disabled)";
          if ($(this).attr("checked")) {
            $("#perc-wfconfig-roles-table")
              .find(inputClass)
              ...
              if (typeof tempName !== "undefined") {
                ...
                if (className === tempName) {
                  ...
                  if (tempName === "approve" && !isReadOnly) {
                    $(row)
                      .find("td.perc-publish input")
                      .prop("disabled", false);
                  }
                }
              }
            }
            var notifyChkBox = $(row)
              .find("td.perc-notify input")
              .prop("disabled", false);
            if (notifyStatus) {
      ...
      function updateNotification(clickedCheckbox, row) {
        ...
        if ($(row).find("td:not(.perc-notify) input:checked").length > 0) {
          ...
          $(row)
            .find(".perc-notify input")
            .prop("checked", false)
            .prop("disabled", true);
        }
        if (
          clickedCheckbox.parent().hasClass("perc-publish") &&
          clickedCheckbox.is(":checked")
        ) {
      ...
      function saveData(
        ...
        if (!$("#perc-wfstep-name")[0].checkValidity()) {
          ...
          $("#perc-wfstep-save").removeClass("ui-state-focus");
          $("#perc-wfstep-name").focus();
          $("#perc-wfstep-name").select();
        ...
        var stepRoles = [];
        if (isUpdate) var possiblePermissions = workflowStep.permissionNames;
        else
          var possiblePermissions =
            $.PercEditWorkflowStepDialog().tempPossiblePermissions;
        $(".perc-wfconfig-row").each(function () {
          ...
          $.each(possiblePermissions, function () {
            var permName = this + "";
            if (
              roleRow.find(".perc-" + permName.toLowerCase() + " :checked")
                .length > 0
            ) {
          ...
          if (rolePermissions.length > 0) {
            var stepRole = {
              enableNotification: notifyStatus,
              roleName: roleName,
              roleTransitions: rolePermissions,
            };
        ...
        if (isUpdate) {
          $.PercWorkflowService().updateWorkflowStep(
            workflowName,
            previousStepName,
            stepObj,
          ...
          $.PercWorkflowService().createNewWorkflowStep(
            workflowName,
            stepName,
            stepObj,
            function (status, result) {
