...
(function ($) {
  ...
  $.perc_template_layout_helper = function () {
     ...
     *============================================================*/
    this.lorem =
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur eleifend velit quis risus adipiscing ullamcorper. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed laoreet molestie congue. In tincidunt enim at sem faucibus eu iaculis sem viverra. ";
    ...
    this.updateWidgetsHtmlFromRest = function (
      ...
      $regionWidgetAssociations.find("regionWidget").each(function () {
        ...
        if (id === regionId) $regionWidget = $(this);
      });
...
*/
    ...
    this.templateModelToTemplateXml = function (isForServerRendering) {
      ...
      $regionWidgetAssociations
        .parent()[0]
        .replaceChild(element, $regionWidgetAssociations[0]);
    };
    ...
    this.addWidget = function (widget, regionid, append) {
      ...
      if (reg.hasSubRegions()) {
        // Cant mix widgets and regions
    ...
    this.getRegion = function (regionId, theparent) {
      if (theparent == null) theparent = this.rootRegion;
      if (theparent.getId() === regionId) return theparent;
    ...
    this.getWidgetRegion = function (widgetId, theparent) {
      ...
      if (theparent.getWidget(widgetId) != null) return theparent;
      if (theparent.hasSubRegions()) {
        var subs = theparent.getSubRegions();
        for (var reg in subs) {
    ...
    this.removeWidget = function (widgetId) {
      ...
      if (reg != null) return reg.removeWidget(widgetId);
      return null;
    };
    ...
    this.orderWidget = function (widgetId, fromRegionId, toRegionId, pos) {
      ...
      if (regFrom != null && regTo != null)
        ...
        alert(
          I18N.message("perc.ui.template.layout.helper@Unable To Reorder") +
            widgetId +
            I18N.message("perc.ui.template.layout.helper@From Reigon") +
            fromRegionId +
            I18N.message("perc.ui.template.layout.helper@To Reigon") +
            toRegionId +
            I18N.message("perc.ui.template.layout.helper@At Position") +
            pos +
            ".\n" +
            I18N.message(
    ...
    this._insertNewRegion = function (region, vertical, append) {
      ...
      if (region.hasSubRegions()) {
        ...
        region.setVertical(vertical);
        if (region.hasWidgets()) this._moveWidgets(region, newRegion2);
    ...
    this.isWidgetAllowed = function (regionId) {
      ...
      return (
        (region.isOverridable() || region.isPageRegion()) &&
