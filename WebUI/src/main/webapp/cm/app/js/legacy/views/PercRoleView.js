...
(function ($) {
  ...
  $.PercRoleView = function () {
    ...
    function showSelectedRoleEditor() {
      if (
        controller.getSelectedRole() !== "Admin" &&
        controller.getSelectedRole() !== "Designer"
      ) {
        $("#perc-roles-name-field")
    ...
    function updateAssignedUsers(assignedUsersArray) {
      ...
      for (i in assignedUsersArray) {
        ...
        if (
          ...
          liUser.on("click", function (evt) {
            selectUser.call(this, [evt]);
          });
          liUser.on("keydown", function (eventHandler) {
    ...
    function selectUser(event) {
      if (!editingRole) {
        ...
        else userRow.addClass("perc-assigned-user-selected");
        if (
          $(".perc-roles-assigned-users-list .perc-assigned-user-selected span")
            .length > 0
        )
          enableRemoveUsers();
        else disableRemoveUsers();
      }
    }
