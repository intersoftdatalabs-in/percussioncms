...
(function ($) {
  ...
  $.Perc_Region_class = function (regionid) {
    this.lorem =
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur eleifend velit quis risus adipiscing ullamcorper. ";
    ...
    this.nextSibling = function () {
      if (this.regionparent == null) return null;
      ...
      for (var i = 0; i < this.regionparent.getSubRegions().length; i++) {
        if (this.regionparent.getSubRegions()[i].regionid == this.regionid) {
          if (i < len - 2) return this.regionparent.getSubRegions()[i + 1];
          return null;
        }
      }
    };
    ...
    this.previousSibling = function () {
      ...
      for (var i = 0; i < this.regionparent.getSubRegions().length; i++) {
        if (this.regionparent.getSubRegions()[i].regionid == this.regionid) {
          if (i > 0) return this.regionparent.getSubRegions()[i - 1];
          return null;
        }
      }
    };
    ...
    this.addSubRegion = function (region, append) {
      ...
      if (this.subRegions == null) this.subRegions = new Array();
      if (append) this.subRegions.push(region);
      else this.subRegions.unshift(region);
    ...
    this.insertSubRegion = function (region, index) {
      ...
      else this.subRegions.splice(index, 0, region);
      region.setParent(this);
    };
    ...
    this.addWidget = function (widget, append) {
      ...
      if (this.widgets == null) this.widgets = new Array();
      if (append) this.widgets.push(widget);
      else this.widgets.unshift(widget);
    };
    ...
    this.insertWidget = function (widget, index) {
      ...
      else this.widgets.splice(index, 0, widget);
    };

    this.getSubRegion = function (regionid) {
      if (this.subRegions == null) return null;
    ...
    this.getIndex = function () {
      if (this.regionparent != null) {
        ...
        for (var i = 0; i < subs.length; i++) {
          var sub = subs[i];
          if (sub.regionid == this.regionid) return i;
        }
      }
      return -1;
    };
    ...
    this.getWidget = function (widgetid) {
      if (this.widgets == null) return null;
    ...
    this.generateInnerVelocity = function () {
      ...
      if (this.hasSubRegions()) {
        ...
        if (!this.vertical) buff += '<div class="clear-float"></div>';
      } else {
        //leaf
    ...
    this.getRegionWidgetXmlString = function () {
      ...
      for (w in this.widgets) {
        ...
        buff +=
          "				<definitionId>" +
          this.widgets[w].getWidgetDefinitionId() +
          "</definitionId>\n";
        ...
        for (property in this.widgets[w].properties) {
          ...
          buff +=
            "						<value>" + this.widgets[w].properties[property] + "</value>\n";
          buff += "					</property>\n";
        }
        buff += "				</properties>\n";
        buff += "			</widgetItem>\n";
    ...
    this.generateHtml = function (isForServerRendering) {
      ...
      if (!this.hasSubRegions()) clazz += " perc-region-leaf";
      buff += '<div class="';
      buff += clazz;
      buff += '" ';
