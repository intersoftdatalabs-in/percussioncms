  ...
  $.PercFolderPropertiesDialog = function () {
    ...
    function openDialog(pathItem, callback) {
      ...
      function createDialog(status, result) {
        ...
        var checkStatus = [];
        checkStatus[0] =
          permission === $.PercFolderHelper().PERMISSION_READ
            ? " selected "
            : "";
        checkStatus[1] =
          permission === $.PercFolderHelper().PERMISSION_WRITE
            ? " selected "
            : "";
        checkStatus[2] =
          permission === $.PercFolderHelper().PERMISSION_ADMIN
            ? " selected "
            : "";
        ...
        dialog = $(
          "<div>" +
            ...
            "<div>" +
            "<label for='perc-folder-name' class='perc-required-field'>" +
            I18N.message("perc.ui.folderPropsDialog.label@Name") +
            ":</label> <br/> " +
            "<input type='text' class='required' tabindex='" +
            taborder +
            "' id='perc-folder-name' name='folder_name' maxlength='50' value=\"" +
            fProps.name +
            '"/> <br/>' +
            ...
            "<label for='perc-folder-permission'>" +
            ...
            "     <option id='perc-folder-permission-option-read'  " +
            checkStatus[0] +
            " value='" +
            ...
            "</option>" +
            "     <option id='perc-folder-permission-option-write' " +
            checkStatus[1] +
            " value='" +
            ...
            "</option>" +
            "     <option id='perc-folder-permission-option-admin' " +
            checkStatus[2] +
            " value='" +
            $.PercFolderHelper().PERMISSION_ADMIN +
            "'>" +
            I18N.message("perc.ui.folderPropsDialog.permissionValue@Admin") +
            "</option>" +
            "</select>" +
            "<input type='hidden' id='perc-folder-id' value='" +
            fProps.id +
            "'/>" +
            "<input type='hidden' id='perc-folder-oldname' value=\"" +
            fProps.name +
            '"/>' +
              ...
              function (status, result) {
                if (status === $.PercServiceUtils.STATUS_SUCCESS) {
                  ...
                  for (var s = 0; s < sites.length; s++) {
                    var selected =
                      $.inArray(sites[s].siteId.toString(), allowedSites) >= 0;
                    var option = $("<option/>")
                  ...
                  $("input[name=multiselect_allowed-sites]")
                    .off("click")
                    .on("click", function (e) {
      ...
      function _saveFolderProps(newWritePrincipals) {
        ...
        if (allowedSitesMultiselect !== undefined) {
          ...
          if (allowedSites.length > 0) {
            ...
            folderProps.FolderProperties.allowedSites = allowedSites
              .sort()
              .join(",");
          } else {
        ...
        if (
          currentAllowedSites !== undefined &&
          folderProps.FolderProperties.allowedSites !== currentAllowedSites
        ) {
          var options = {
      ...
      function _addFieldGroups() {
        ...
        $.each(fieldGroups, function (index) {
          ...
          var minmaxClass =
            index === 0
              ? "perc-section-items-minimizer"
              : "perc-section-items-maximizer";
          var groupHtml =
