...
(function ($, P) {
  ...
  P.templateDesignView = function () {
    ...
    if (inline_help === "") {
      ...
      $("#perc-temp-lib-maximizer").replaceWith(
        '<a id="perc-temp-lib-disabled" style="float: left;" href="#"></a>',
      );
    } else {
      ...
      $("#perc-temp-lib-disabled").replaceWith(
        '<a id="perc-temp-lib-maximizer" style="float: left;" href="#"></a>',
      );
    }
    ...
    var actionNames = [
      I18N.message("perc.ui.template.design.view@Actions"),
      I18N.message("perc.ui.template.design.view@Create Template From Page"),
      I18N.message("perc.ui.template.design.view@Add Template"),
      I18N.message("perc.ui.template.design.view@Export Template"),
      I18N.message("perc.ui.template.design.view@Import Template"),
    ];
    ...
    /*Work around for issue with PercDropdown.js
         ...
         */
    $(".perc-dropdown-option-CreateTemplatefromPage")
      .addClass("perc-drop-disabled")
      .css("color", "#9FA3AA");
    ...
    function setPanelPreference(showPanel, pageNumber) {
      var siteName = $.PercNavigationManager.getSiteName();
      var panelSettings = JSON.parse(
        $.cookie("perc-unassigned-panel-" + siteName + "-settings"),
      );
      ...
      $.cookie(
        "perc-unassigned-panel-" + siteName + "-settings",
    ...
    function initializeUnassignedPanel() {
      ...
      panelExpanderIcon.on("click", function (evt) {
        if (typeof $.PercNavigationManager.getSiteName() != "undefined") {
          ...
          if (panelExpanderIcon.is(".perc-collapsed")) {
            ...
            panelExpanderIcon.attr(
              "title",
              '<i18n:message key = "perc.ui.workflow.view@Minimize"/>',
            );
            panel.show("slide", {}, 500, function () {
              $("#perc-assigned-templates").template_selected(
                "updateTemplatesPagingSize",
              );
            });
            panelExpanderIcon
              .removeClass("perc-collapsed")
              .addClass("perc-expanded");
            ...
            panelExpanderIcon
              .removeClass("perc-expanded")
              .addClass("perc-collapsed");
            panelExpanderIcon.attr(
              "title",
              '<i18n:message key = "perc.ui.workflow.view@Maximize"/>',
            );
      ...
      var siteName = $.PercNavigationManager.getSiteName();
      if (typeof siteName != "undefined") {
        $.PercPageService.getUnassignedPagesBySite(
          siteName,
          1,
          UNASSIGNED_MAX_RESULTS,
          function (status, result) {
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              var panelSettings = JSON.parse(
                $.cookie(
                  "perc-unassigned-panel-" +
              ...
              if (panelSettings != null) {
                ...
                if (
                  typeof unassignedResult.unassignedItemList.childrenInPage !=
                  "undefined"
                )
                  showPanel =
                    unassignedResult.unassignedItemList.childrenInPage.length >
                    0;
              }
              if (showPanel) {
                panel.removeClass("perc-closed").addClass("perc-opened");
      ...
      var percJump = $(
        ".perc-unassigned-panel .perc-template-pages-controls .perc-jump",
      );
      $.perc_filterField(percJump, $.perc_textFilters.ONLY_DIGITS);

      // Pagination controls - Previous button
      $(".perc-unassigned-panel .perc-template-pages-controls .previous").on(
        "click",
        function (evt) {
      ...
      $(".perc-unassigned-panel .perc-template-pages-controls .next").on(
        "click",
        function (evt) {
      ...
      $(".perc-unassigned-panel .perc-template-pages-controls").on(
        "submit",
        function () {
      ...
      var actionMenuAnchor_child = $("#perc-dropdown-actions")
        .children()
        .children()
        .children();
      ...
      var viewMenuAnchor_child = $("#perc-dropdown-view")
        .children()
        .children()
        .children();
        ...
        $.PercPageService.getUnassignedPagesBySite(
          ...
          function (status, result) {
            if (status == $.PercServiceUtils.STATUS_SUCCESS) {
              ...
              var totalPages =
                unassignedResult.importStatus.catalogedPageCount +
                unassignedResult.importStatus.importedPageCount;
              var childrenInPage = 0;
              if (
                typeof unassignedResult.unassignedItemList.childrenInPage !=
                "undefined"
              )
                childrenInPage = $.perc_utils.convertCXFArray(
                  unassignedResult.unassignedItemList.childrenInPage,
                ).length;
              updatePaging(
                unassignedResult.unassignedItemList.startIndex,
                childrenInPage,
                totalPages,
              );
            } else {
    ...
    function updatePaging(startIndex, childrenCount, totalItems) {
      ...
      else pageRange.text(0);
      pageTotal.text(totalItems);
      var pageNumber =
        Math.floor((startIndex - 1) / UNASSIGNED_MAX_RESULTS) + 1;
    ...
    function updateProgressBar(importStatus) {
      ...
      var progressFinishedMessage = progressPanel.find(
        ".perc-progress-finished",
      );
      var progressContainer = progressPanel.find(
        ".perc-progress-bar-container",
      );
      var progressBar = progressContainer.find(".perc-progress-bar");
      var progressMessage = progressPanel.find(".perc-progress-message");
      var progressWidth = 0;
      var totalPages =
        importStatus.catalogedPageCount + importStatus.importedPageCount;
      if (totalPages > 0)
      ...
      if (progressWidth == 100) {
        ...
        var message =
          I18N.message("perc.ui.template.design.view@Imported") +
          importStatus.importedPageCount +
          I18N.message("perc.ui.template.design.view@Of") +
          totalPages +
          I18N.message("perc.ui.template.design.view@Cataloged Pages");
        ...
        else progressContainer.css("background-color", "#E6E6E9");
      }
    ...
    function drawUnassignedPages(unassignedItemList) {
      ...
      var selectedPageId =
        panel.find(".perc-imported-page-selected").length === 1
          ? panel.find(".perc-imported-page-selected").attr("id")
          : null;
      var pageContainer = panel.find(".perc-panel-pages-list");
      pageContainer.empty();
      var pageList = $("<ul/>");
      pageContainer.append(pageList);
      var childrenInPage = $.perc_utils.convertCXFArray(
        unassignedItemList.childrenInPage,
      );
      for (var i = 0; i < 10; i++) {
        ...
        var pageObj = $("<li/>")
          ...
          //)
          .append(
            $("<img alt='Inspect Button'/>")
              .addClass("perc-left-img")
              .attr("src", "../images/images/inspectButton.png")
              .attr("data", JSON.stringify(page)),
          )
          .append($("<div/>").text(page.name))
          .append($("<span/>").addClass("perc-imported-page-dropdown"));
        ...
        switch (page.status) {
          case "Imported":
            pageObj.addClass("perc-imported-page").on("click", function (evt) {
              $(".perc-imported-page-selected").removeClass(
                "perc-imported-page-selected",
              );
              $(this).addClass("perc-imported-page-selected");
              var createTplPageMenuEntry = $(
                ".perc-dropdown-option-CreateTemplatefromPage",
              );
              createTplPageMenuEntry
          ...
          case "Importing":
            pageObj
              .find(".perc-left-img")
              .attr("src", "/Rhythmyx/sys_resources/images/running.gif");
          ...
          case "Cataloged":
            pageObj
              .find(".perc-left-img")
              .attr("src", "/Rhythmyx/sys_resources/images/iconSpider.png");
            pageObj.find(".perc-imported-page-dropdown").hide();
      ...
      if (selectedPageId) {
        panel
          .find(".perc-imported-page[id='" + selectedPageId + "']")
          .addClass("perc-imported-page-selected");
      }
    }
    ...
    function _changeTemplate(opts) {
      $.PercChangeTemplateDialog().openDialog(
        opts.data.pageId,
        "",
        $.PercNavigationManager.getSiteName(),
        function () {
    ...
    function getCurrentView() {
      if ($("#perc-activated-templates").hasClass("perc-templates-detailed"))
        return VIEW_MODE_CATALOG;
    ...
    function _createTemplateFromPage() {
      $.PercBlockUI();
      if (
        $(".perc-dropdown-option-CreateTemplatefromPage").hasClass(
          "perc-drop-disabled",
        )
      ) {
        $.unblockUI();
      ...
      if (!pageId) {
        $.perc_utils.alert_dialog({
          ...
          content: I18N.message(
            "perc.ui.template.design.view@Select Imported Page",
          ),
        });
        $.unblockUI();
      } else {
        $.PercWorkflowController().checkOut(
          ...
          function (status) {
            if (status) {
              ...
              $.PercTemplateService().createTemplateFromPage(
                pageId,
                $.PercNavigationManager.getSiteName(),
    ...
    function _createViewMenu(currentMode) {
      ...
      if (currentMode == VIEW_MODE_CATALOG) {
        viewNames = [
          I18N.message("perc.ui.template.design.view@View"),
          I18N.message("perc.ui.template.design.view@Thumbnails"),
        ];
        disableView = [true, true];
        callbackFunctions = [function () {}, _switchToThumbnailView];
        callbackData = [
          I18N.message("perc.ui.template.design.view@View"),
          I18N.message("perc.ui.template.design.view@Thumbnails"),
        ];
        ...
        viewNames = [
          I18N.message("perc.ui.template.design.view@View"),
          I18N.message("perc.ui.template.design.view@Catalog"),
        ];
        disableView = [true, true];
        callbackFunctions = [function () {}, _switchToCatalogView];
        callbackData = [
          I18N.message("perc.ui.template.design.view@View"),
          I18N.message("perc.ui.template.design.view@Catalog"),
        ];
    ...
    function createDialog(isError) {
      ...
      var dialogHTML =
        "<div id='perc-import-template-wrapper'>" +
        "<iframe name='perc-import-template-frame' id='perc-import-template-frame' height='41px' FRAMEBORDER='0' width='100%' src='../app/importTemplate.jsp'></iframe>" +
        "</div>";
      ...
      if (isError) {
        dialogHTML =
          "<div id='perc-import-template-wrapper'>" +
          "<iframe name='perc-import-template-frame' id='perc-import-template-frame' height='83px' FRAMEBORDER='0' width='100%' src='../app/importTemplate.jsp?status=PERC_ERROR&message=The file you attempted to import is not a valid XML file. Choose a valid Page template XML file for upload.'></iframe>" +
          "</div>";
      }
        ...
        percButtons: {
          Import: {
            click: function () {
              ...
              $.PercTemplateService().getSiteProperties(
                $.PercNavigationManager.getSiteName(),
                function (status, result) {
                  if (status) {
                    actionUrl = actionUrl + "/" + result.SiteProperties.id;
                    var checkValue = $("#perc-import-template-frame")
                      .contents()
                      .find(".perc-template-import-field")
                      .val();

                    // Show error message if user clicks Import without selecting any file.
                    if (checkValue == "") {
                      $("#perc-import-template-frame").css("height", "67px");
                      $("#perc-import-template-frame")
                        .contents()
                        .find(".perc-import-error")
                        .show();
                      return;
                    }
                    $("#perc-import-template-frame")
                      .contents()
                      .find("#perc-import-template-form")
    ...
    function closeDialogOnSuccess() {
      ...
      var templateName = $("#perc-import-template-frame")
        .contents()
        .find("body")
        .text();
