...
(function ($, P) {
   ...
   */
  P.decorationController = function (
    allElements,
    decorationClasses,
    selectionClasses,
    menuItems,
    ...
    function addClicks() {
      allElements()
        .off("click.decorate")
        .on("click.decorate", function (evt) {
          // unselect the other decorator if this is selected
          if (otherDecorator != null) otherDecorator.unselectAll();
      ...
      $.each(allElements(), function () {
        ...
        if (!checkLock) {
          $(this)
            .off("dblclick.decorate")
            .on("dblclick.decorate", function (evt) {
              evt.stopPropagation();
              if (_dblclickItem) {
    ...
    function addMenu(elem) {
      ...
      if (elem.hasClass("perc-locked")) {
        var item = {
          name: "locked",
          img: "/cm/images/icons/editor/locked",
        ...
        $.each(menuItems, function () {
          ...
          if (
            typeof this.widgetType === "undefined" ||
            this.widgetType === elem.attr("widgetdefid")
          )
    ...
    function addMenuItem(elem, menu, item) {
      ...
      var isInactive =
        iconSrc.indexOf("Inactive", iconSrc.length - "Inactive".length) !== -1;

      var tooltip =
        typeof item.tooltip === "function" ? item.tooltip(elem) : item.tooltip;
      ...
      var icon = $("<img src='" + normalImg + "'/>")
        ...
        .attr("alt", tooltip)
        .attr("title", tooltip);
      if (isInactive) icon.css("cursor", "default");
