...
(function ($) {
  ...
  function encryptString(str, callback, async = true) {
    var object = {
      psmap: { entries: { entry: { key: "string", value: str } } },
    };
    $.PercServiceUtils.makeJsonRequest(
  ...
  function convertMapToArray(mapEntries) {
    ...
    if (mapEntries.entry === undefined) {
      ...
      if (!Array.isArray(mapEntries.entry)) tempArray.push(mapEntries.entry);
      else tempArray = mapEntries.entry;
  ...
  function logToServer(type, category, message) {
    if (!message) return;
    var logData = {
      LogData: { type: type, category: category, message: message },
    };
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.UTIL_LOG_DATA,
      $.PercServiceUtils.TYPE_POST,
      false,
      $.noop,
      logData,
