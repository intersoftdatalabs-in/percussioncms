...
<%
    ...
    if (section == null || !section.matches("^(workflow|roles|users|categories)$")) {
        section = "workflow";
    }
%>
...
<html lang="<%= lang %>">
<head>
    <title><i18n:message key="perc.ui.navMenu.workflow@Workflow &amp; Administration"/></title>
...
<body class="perc-workflow-admin-body">
    <div id="perc-workflow-admin-root" data-testid="perc-workflow-admin-root"></div>

    <script>
      (function() {
        function mountShell() {
          if (window.PercModernUI && typeof window.PercModernUI.mount === "function") {
            window.PercModernUI.mount("perc-workflow-admin-root", "WorkflowAdminShell", {
              initialTab: "<%= section %>"
            });
          } else {
