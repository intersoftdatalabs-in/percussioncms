...
(function ($) {
  ...
  WidgetBuilderApp.FieldListView = Backbone.View.extend({
    ...
    render: function (eventName) {
      ...
      _.each(
        ...
        function (field) {
          $(this.el).append(
            new WidgetBuilderApp.FieldListItemView({ model: field }).render().el
          );
        },
        this
      );
  ...
  WidgetBuilderApp.FieldListItemView = Backbone.View.extend({
    ...
    events: {
      mouseenter: function () {
        $(this.el)
          .find(".perc-widget-field-wrapper")
          .addClass("perc-widget-field-wrapper-hover");
        $(this.el).find(".perc-widget-field-actions").show();
      },
      mouseleave: function () {
        $(this.el)
          .find(".perc-widget-field-wrapper")
          .removeClass("perc-widget-field-wrapper-hover");
      },
  ...
  WidgetBuilderApp.showFieldEditor = function (fieldModel) {
    ...
    var dialog = $("<div id='perc-widget-field-editor'/>").perc_dialog({
      ...
      open: function () {
        WidgetBuilderApp.fieldEditorView = new WidgetBuilderApp.FieldEditorView(
          { model: fieldModel }
        );
        $("#perc-widget-field-editor").html(
          WidgetBuilderApp.fieldEditorView.render().el
        );
        var fieldsForm = $("form[name=perc-widget-field-editor-form]");
        $.perc_filterField(
          fieldsForm.find("input[name=name]"),
  ...
  WidgetBuilderApp.updateFieldsModel = function () {
    $(WidgetBuilderApp.fieldEditorView)
