...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#perc-widget-def-tabs").tabs({
      ...
      select: function (event, ui) {
        ...
        if (isValid) currentTabIndex = ui.index;
        return isValid;
      },
    });
    var tabindexCounter = 122;
    $("#perc-widget-def-tabs")
      .find("li")
      .each(function () {
    ...
    $("#perc-wb-defs-container").append(
      WidgetBuilderApp.WdgDefGrid.render().$el
    );
  ...
  function confirmIfDirty(successCallback) {
    var msg =
      "The widget editor contains unsaved changes. " +
    ...
    $("#perc-widget-save").on("click", function () {
      var saveCallback = function (status, result) {
        if (status === $.PercServiceUtils.STATUS_ERROR) {
          var errorMsg = $.PercServiceUtils.extractDefaultErrorMessage(
            result.request
          );
        ...
        $.PercBlockUI();
        $.PercWidgetBuilderService.saveWidgetDef(
          prepareWidgetDef(),
          function (status, result) {
            $.unblockUI();
            if (!status) {
              $.perc_utils.alert_dialog({
                title: "Widget save error",
                content: result,
              });
              return;
            }
            ...
            if (!currentWidgetView.model.get("widgetId")) {
              currentWidgetView.model.set({
                widgetId: result.WidgetBuilderValidationResults.definitionId,
              });
            }
  ...
  function deleteWidget() {
    jQuery.PercWidgetBuilderService.deleteWidgetDef(
      WidgetBuilderApp.selectedModel,
      function (status, result) {
        if (!status) {
          $.perc_utils.alert_dialog({
            title: "Widget delete error",
            content: result,
          });
          return;
        }
        ...
        if (
          ...
          currentWidgetView.model.get("widgetId") ===
            WidgetBuilderApp.selectedModel
        ) {
          dirtyController.setDirty(false);
          handleWidgetClose();
  ...
  function handleWidgetEdit() {
    confirmIfDirty(function () {
      if (WidgetBuilderApp.isValidationError) {
        WidgetBuilderApp.dirtyController.setDirty(
          true,
          "Widget",
          WidgetBuilderApp.saveOnDirty
        );
        return;
      }
      $.PercWidgetBuilderService.loadWidgetDefFull(
        WidgetBuilderApp.selectedModel,
        renderWidget
      );
    });
  ...
  function renderWidget(status, result, isNew) {
    ...
    var wdgObject = isNew
      ? result
      : wdgModel.convertFromServerObject(result.WidgetBuilderDefinitionData);
    wdgModel.set(wdgObject);
    currentWidgetView = new WidgetBuilderApp.WidgetDefinitionGeneralView({
      model: wdgModel,
    });
    $("#perc-widget-tab-general").empty().append(currentWidgetView.render().el);
    //Content tab
    var fieldsObj = isNew
      ? result.get("fieldsList").fields
      : result.WidgetBuilderDefinitionData.fieldsList.fields;
    WidgetBuilderApp.fieldsList = new WidgetBuilderApp.FieldCollection();
    WidgetBuilderApp.fieldsList.add(fieldsObj);
    WidgetBuilderApp.fieldsListView = new WidgetBuilderApp.FieldListView({
      model: WidgetBuilderApp.fieldsList,
    });
    $("#perc-widget-fields-container")
      .empty()
      .html(WidgetBuilderApp.fieldsListView.render().el);
    //Display tab
    var widgetHtmlModel = new WidgetBuilderApp.WidgetHtmlModel();
    var widgetHtmlObj = isNew
      ? result.get("widgetHtml")
      : result.WidgetBuilderDefinitionData.widgetHtml;
    widgetHtmlModel.set({ widgetHtml: widgetHtmlObj });
    WidgetBuilderApp.widgetHtmlView = new WidgetBuilderApp.WidgetHtmlView({
      model: widgetHtmlModel,
    });
    $("#perc-widget-display-html-container")
      .empty()
      .html(WidgetBuilderApp.widgetHtmlView.render().el);
    //Resources tab
    var jsResObj = isNew
      ? result.get("jsFileList").resourceList
      : result.WidgetBuilderDefinitionData.jsFileList.resourceList;
    WidgetBuilderApp.jsResList =
      new WidgetBuilderApp.WidgetResourceCollection();
    var jsResModels = [];
    ...
    WidgetBuilderApp.jsResListView = new WidgetBuilderApp.ResourceListView({
      model: WidgetBuilderApp.jsResList,
    });
    $("#perc-widget-js-resources-container").html(
      WidgetBuilderApp.jsResListView.render().el
    );

    var cssResObj = isNew
      ? result.get("cssFileList").resourceList
      : result.WidgetBuilderDefinitionData.cssFileList.resourceList;
    WidgetBuilderApp.cssResList =
      new WidgetBuilderApp.WidgetResourceCollection();
    var cssResModels = [];
    ...
    WidgetBuilderApp.cssResListView = new WidgetBuilderApp.ResourceListView({
      model: WidgetBuilderApp.cssResList,
    });
    $("#perc-widget-css-resources-container").html(
      WidgetBuilderApp.cssResListView.render().el
    );

    //Add character input filters
    var generalForm = $("form[name=perc-widget-general-tab-form]");
    $.perc_filterField(
      generalForm.find("input[name=widgetname]"),
      $.perc_textFilters.ALPHA_NUMERIC
    );
    $.perc_filterField(
      generalForm.find("input[name=version]"),
      $.perc_textFilters.DIGITS_DOT
    );
  }
  ...
  function handleWidgetDeploy() {
    ...
    confirmIfDirty(function () {
      ...
      var settings = {
        ...
        question:
          "Are you sure you want to deploy the selected widget? Once a widget is deployed, it cannot be undeployed.",
  ...
  function deployWidget() {
    $.PercBlockUI();
    $.PercWidgetBuilderService.deployWidget(
      WidgetBuilderApp.selectedModel,
      function (status, result) {
        $.unblockUI();
        if (!status) {
          $.perc_utils.alert_dialog({
            id: "perc-widget-depoly-error",
            title: "Widget deploy error",
            content: result,
          });
          return;
        }
        $.perc_utils.alert_dialog({
          id: "perc-widget-depoly-confirmation",
          title: "Widget deploy confirmation",
          content:
            "Congratulations! selected widget has been deployed successfully.",
        });
      }
    );
  }
  ...
  function prepareWidgetDef() {
    ...
    dataObj.WidgetBuilderDefinitionData.fieldsList.fields =
      WidgetBuilderApp.fieldsList.toJSON();
    dataObj.WidgetBuilderDefinitionData.widgetHtml = editor.getValue();
    //Add Resources
    dataObj.WidgetBuilderDefinitionData.jsFileList = {};
    dataObj.WidgetBuilderDefinitionData.jsFileList.resourceList =
      WidgetBuilderApp.jsResListView.toStringArray();
    dataObj.WidgetBuilderDefinitionData.cssFileList = {};
    dataObj.WidgetBuilderDefinitionData.cssFileList.resourceList =
      WidgetBuilderApp.cssResListView.toStringArray();
    return dataObj;
  }
  ...
  function validateTabData(isOnSave) {
    ...
    var isValid = false;
    jQuery.PercWidgetBuilderService.validate(
      prepareWidgetDef(),
      function (status, result) {
        if (!status) {
          $.perc_utils.alert_dialog({
            title: "Widget validation error",
            content: result,
          });
        } else {
          var valRes = [];
  ...
  function showValidationErrors(tabCategory, errors) {
    ...
    switch (tabCategory) {
      ...
      default:
        ...
        $.perc_utils.alert_dialog({
          title: "Widget validation error(s)",
          content: content,
        });
    }
  }
...
$(document).ready(function () {
  ...
  if ($("#perc-display-html-auto-generate-button").length) {
    $("#perc-display-html-auto-generate-button").keydown(function (
      eventHandler
    ) {
      if (eventHandler.code == "Enter" || eventHandler.code == "Space") {
        document.activeElement.click();
