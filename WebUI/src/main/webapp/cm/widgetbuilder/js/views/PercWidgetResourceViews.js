...
(function ($) {
  ...
  WidgetBuilderApp.ResourceListView = Backbone.View.extend({
    ...
    render: function (eventName) {
      ...
      _.each(
        ...
        function (resource) {
          $(this.el).append(
            new WidgetBuilderApp.WidgetResourceView({
              model: resource,
            }).render().el
          );
        },
    ...
    toStringArray: function () {
      ...
      _.each(
        ...
        function (resource) {
          ...
          if (resName !== "") resArray.push(resName);
        },
        this
      );
      return resArray;
  ...
  WidgetBuilderApp.addNewResource = function (type) {
    if (type === "JS") {
      WidgetBuilderApp.jsResList.add(
        new WidgetBuilderApp.WidgetResourceModel()
      );
      setTabIndexForJsResources();
    } else if (type === "CSS") {
      WidgetBuilderApp.cssResList.add(
        new WidgetBuilderApp.WidgetResourceModel()
      );
  ...
  WidgetBuilderApp.WidgetResourceView = Backbone.View.extend({
    ...
    updateEdits: function (event) {
      ...
      var value = $(this.el)
        .find("input.datadisplay.perc-resource-entry-field")
        .val();
      this.model.set({ name: value });
    },
    initialize: function () {
      this.template = _.template(
        $("#perc-widget-resource-item-editor-template").html()
      );
    },
    render: function (eventName) {
  ...
  function setTabIndexForJsResources() {
    ...
    var tagCol = "";
    $("#perc-widget-js-resources-container")
      .find("*")
      .each(function () {
  ...
  function setTabIndexForCssResources() {
    ...
    var tagCol = "";
    $("#perc-widget-css-resources-container")
      .find("*")
      .each(function () {
        var tagName = this.tagName;
        tagCol = tagCol + ", " + tagName;
