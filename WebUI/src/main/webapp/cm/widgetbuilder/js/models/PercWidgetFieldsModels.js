...
(function ($) {
  WidgetBuilderApp.Field = Backbone.Model.extend({
    ...
    validate: function (oldName) {
      ...
      if (
        nameVal.length < 1 ||
        !WidgetBuilderApp.startsWithAlphaRegEx.test(nameVal) ||
        nameVal.length < 2
      )
        errors.push({
          name: "name",
          message:
            "This is a required field, must start with a letter and must contain at least two characters.",
        });
        ...
        if (char1 == char1.toLowerCase() && char2 == char2.toUpperCase())
          errors.push({
            name: "name",
            message:
              "The field name cannot start with a lowercase letter followed by an uppercase letter.",
          });
      }
      ...
      $.each(
        ...
        function () {
          if (nameVal != oldName && this.get("name") == nameVal) isDupe = true;
        },
        this
      );
      if (isDupe)
