...
(function ($) {
  ...
  function getContentForPath(displayFormat, config, callback) {
    var str_path =
      $.perc_utils.encodeURL(config.path) +
      "/?startIndex=" +
      config.startIndex +
      "&maxResults=" +
      config.maxResults +
    ...
    $.perc_utils.confirm_dialog({
      ...
      success: function () {
        ...
        if ($("#perc_delete_folder_force").length > 0) {
          skipItems = $("#perc_delete_folder_force").get(0).checked
            ? "NO"
            : "YES";
        }
        var guid;
  ...
  function createDelWarning(type, name, folderType, shouldPurge) {
    ...
    if (folderType === "Assets") {
      if (shouldPurge) {
        warning = I18N.message("perc.ui.finder.folder.purge@WarningAssets", [
          name,
        ]);
        confirm = I18N.message("perc.ui.finder.folder.purge@ConfirmAssets", [
          name,
        ]);
      } else {
        warning = I18N.message("perc.ui.finder.folder.delete@WarningAssets", [
          name,
        ]);
        confirm = I18N.message("perc.ui.finder.folder.delete@ConfirmAssets", [
          name,
        ]);
      }
      ...
      if (type.indexOf("AssetsNotAuthorized") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@AssetNotAuthorized") +
          "<br/><br/>";
      }

      if (type.indexOf("AssetsInUseTemplates") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@AssetInUseTemplates") +
          "<br/><br/>";
      }

      if (type.indexOf("AssetsInUsePages") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@AssetInUsePages") +
          "<br/>";
        del =
          "<br/><input type='checkbox' id='perc_delete_folder_force' style='width:15px'/> <label class='perc_dialog_label'>" +
          I18N.message("perc.ui.finder.folder.delete@DeleteLiveAssets") +
          "</label>";
      }
      ...
      if (type.indexOf("PagesNotAuthorized") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@PageNotAuthorized") +
          "<br/><br/>";
      }

      if (type.indexOf("PagesInUseTemplates") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@PageInUseTemplates") +
          "<br/><br/>";
      }

      if (type.indexOf("PagesInUsePages") > -1) {
        middle +=
          I18N.message("perc.ui.finder.folder.delete@PageInUsePages") + "<br/>";
        del =
          "<br/><input type='checkbox' id='perc_delete_folder_force' style='width:15px'/> <label class='perc_dialog_label'>" +
          I18N.message("perc.ui.finder.folder.delete@DeleteLinkedPages") +
          "</label>";
      }
    }
