...
(function ($) {
  ...
  function getSitePublishProperties(site, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_GET_PUBLISH_PROPERTIES + "/" + site,
      $.PercServiceUtils.TYPE_GET,
      false,
  ...
  function updateSitePublishProperties(sitePublishObj, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SITE_UPDATE_PUBLISH_PROPERTIES,
      $.PercServiceUtils.TYPE_POST,
      false,
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
  ...
  function createUpdateSiteServer(siteName, server, propObj, callback) {
    ...
    if (
      propObj.serverInfo.serverId === null ||
      propObj.serverInfo.serverId === "" ||
      propObj.serverInfo.serverId === "undefined"
    ) {
      $.PercServiceUtils.makeJsonRequest(
        serviceUrl,
        $.PercServiceUtils.TYPE_POST,
   ...
   * Object results data that comes from the server.
   */
  function getIncrementalRelatedItems(
    siteName,
    serverName,
    startIndex,
    pageSize,
   ...
   */
  function _getIncrementalOrRelatedItems(
    isIncremental,
    siteName,
    serverName,
    startIndex,
    ...
    var basePath = isIncremental
      ...
      : $.perc_paths.INCREMENTAL_RELATED_LIST;
    var serviceUrl =
      basePath +
      siteName +
      "/" +
   ...
   *
   */
  function publishIncrementalWithApproval(
    siteName,
    serverName,
    relatedItems,
    callback,
