...
(function ($) {
  ...
  function create(sectionObj, callback) {
    var url = sectionObj.CreateSiteSection
      ? $.perc_paths.SECTION_CREATE
      : $.perc_paths.SECTION_CREATE_EXTERNAL_LINK;
  ...
  function createSectionLink(targetSectionId, parentSectionId, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SECTION_CREATE_SECTION_LINK +
        "/" +
        targetSectionId +
        "/" +
        parentSectionId,
    ...
    $.PercServiceUtils.makeJsonRequest(
      ...
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
          ...
          var defaultErrorCode = $.PercServiceUtils.extractGlobalErrorCode(
            result.request,
          );

          if (
            defaultErrorCode ===
              $.perc_errors.NAVIGATION_SERVICE_FOLDER_ID_NOT_FOUND_FOR_PATH ||
            defaultErrorCode ===
              $.perc_errors.NAVIGATION_SERVICE_CANNOT_FIND_NAVTREE_FOR_SITE
          ) {
  ...
  function getChildren(section, callback, force) {
    if (
      !force &&
      typeof cache.getChildren[section.SiteSection.id] != "undefined"
    ) {
      callback(
        $.PercServiceUtils.STATUS_SUCCESS,
        section,
        cache.getChildren[section.SiteSection.id],
      );
    } else {
      $.PercServiceUtils.makeJsonRequest(
  ...
  function getTree(siteid, callback) {
    ...
    $.PercServiceUtils.makeJsonRequest(
      ...
      function (status) {
        $.unblockUI();
        var result = {
          data: I18N.message("perc.ui.section.service.client@Timed Out"),
        };
  ...
  function deleteSectionLink(targetSectionId, parentSectionId, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.SECTION_DELETE_SECTION_LINK +
        "/" +
        targetSectionId +
        "/" +
        parentSectionId,
