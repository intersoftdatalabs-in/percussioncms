...
(function ($) {
  ...
  function getAssetEditors(currentFolderPath, widgetId, callback) {
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.ASSET_EDITOR_LIBRARY +
        currentFolderPath +
        "?filterDisabledWidgets=yes&widgetId=" +
        widgetId,
   ...
   *
   */
  function set_asset_relationship(
    assetid,
    widgetData,
    pageid,
    assetOrder,
    ...
    if (folderPath) awr.AssetWidgetRelationship.folderPath = folderPath;
    $.ajax({
   ...
   * @param callback
   */
  function asset_from_local_content(
    assetid,
    widgetData,
    pageid,
    name,
  ...
  function update_asset_relationship(
    assetid,
    relationshipId,
    widgetData,
    pageid,
    ...
    $.PercServiceUtils.makeJsonRequest(
      $.perc_paths.ASSET_WIDGET_REL_UPDATE + "/",
      $.PercServiceUtils.TYPE_POST,
      false,
  ...
  function clear_orphan_assets(
    ...
    for (var i = 0; i < assetIds.length; i++) {
      ...
      asset =
        ...
        '"widgetName":' +
        '"' +
        widgetDefinitionIds[i] +
        '"}';
      assets.push(asset);
    }

    var json =
      '{"OrphanAssetsSummary":{"assetWidgetRelationship":[' + assets + "]}}";
