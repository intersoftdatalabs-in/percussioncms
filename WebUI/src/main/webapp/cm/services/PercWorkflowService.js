...
(function ($) {
  ...
  function getStatusByWorkflow(workflow, callback) {
    var url =
      "/Rhythmyx/services/workflowmanagement/workflows/" +
      $.perc_utils.encodeURL(workflow) +
      "/states/choices";
  ...
  function transition(itemId, transitionName, comment, callback) {
    ...
    callback = callback === null ? function () {} : callback;
    var url =
      $.perc_paths.WORKFLOW_TRANSITION_COMMENT +
      "/" +
      itemId +
      "/" +
      transitionName +
  ...
  function getWorkflowObject(workflowName, callback) {
    ...
    var serviceCallback = function (status, results) {
      if (status === $.PercServiceUtils.STATUS_ERROR) {
        callback($.PercServiceUtils.STATUS_ERROR, [
          results.request,
          results.textstatus,
          results.error,
        ]);
  ...
  function updateWorkflowStep(workflowName, stepName, StepObj, callback) {
    var Url =
      $.perc_paths.WORKFLOW_STEPPED + workflowName + "/steps/" + stepName;
  ...
  function getBulkApproveStatus(jobId, isFull, callback) {
    ...
    if (isFull) Url += "/full/" + jobId;
    else Url += "/processed/" + jobId;
