...
(function ($) {
  ...
  function csrfGetURLFromServiceCall(url) {
    ...
    else if (path.indexOf("/feeds/") !== -1) path = CSRF_FEEDS_PATH;
    else path = null;
  ...
  function csrfSafeMethod(method) {
    ...
    var csrfSafe =
      ["post", "put", "delete"].indexOf(method.toLowerCase()) !== -1;
    return !csrfSafe;
  }
  ...
  function extractDefaultErrorMessage(request) {
    ...
    if (error !== null && error.ValidationErrors !== undefined) {
      ...
      if (typeof error.defaultMessage != "undefined") {
        ...
        error.Errors &&
        typeof error.Errors.globalError.defaultMessage != "undefined"
      ) {
    ...
    if (
      typeof request.responseText != "undefined" &&
      request.responseText.indexOf("<?xml") !== -1
    ) {
    ...
    if (
      typeof xmlResponse != "undefined" &&
      xmlResponse.is("ValidationErrors")
    ) {
      if (
        xmlResponse.find("globalErrors").find("defaultMessage").text() !== ""
      ) {
        buff += xmlResponse.find("globalErrors").find("defaultMessage").text();
      } else if (
        xmlResponse.find("globalError").find("defaultMessage").text() !== ""
      ) {
        buff +=
          xmlResponse.find("globalError").find("defaultMessage").text() !== "";
  ...
  function extractFieldErrorCode(request) {
    ...
    if (error.ValidationErrors === undefined) return "";

    var verrors = error.ValidationErrors;
    if (verrors.fieldErrors !== undefined) return verrors.fieldErrors.code;
  ...
  function convertMapToArray(mapEntries) {
    var tempArray = [];
    if (Array.isArray(mapEntries.entry)) tempArray = mapEntries.entry;
    else if (mapEntries.entry) tempArray.push(mapEntries.entry);
    var result = [];
