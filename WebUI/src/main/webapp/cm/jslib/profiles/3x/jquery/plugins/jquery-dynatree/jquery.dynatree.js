    ...
    _activate: function (flag, fireEvents) {
      ...
      if (flag) {
        ...
        if (this.tree.activeNode === this) {
          if (
            opts.onQueryActivate &&
            opts.onQueryActivate.call(this.tree, false, this) === false
          ) {
            return; // Callback returned false
          }
...
*/
    ...
    addChild: function (obj, beforeNode) {
      ...
      if (typeof obj == "string") {
        ...
        // Passed null or undefined or empty array
        return;
        ...
        $("script").each(function () {
          ...
          if (this.src.search(_rexDtLibName) >= 0) {
            if (this.src.indexOf("/") >= 0) {
              // issue #47
              opts.imagePath =
                this.src.slice(0, this.src.lastIndexOf("/")) + "/skin/";
            } else {
              opts.imagePath = "skin/";
            }
            logMsg(
      ...
      switch (eventName) {
        ...
        case "enter":
          ...
          res = {
            over:
              res !== false &&
              (res === true || res === "over" || $.inArray("over", res) >= 0),
            before:
              res !== false &&
              (res === true ||
                res === "before" ||
            ...
            after:
              res !== false &&
              (res === true || res === "after" || $.inArray("after", res) >= 0),
          };
    ...
    bind: function () {
      ...
      this.element.on(eventNames, function (event) {
        ...
        try {
          switch (event.type) {
            case "click":
              return o.onClick && o.onClick.call(tree, dtnode, event) === false
                ? false
                : dtnode._onClick(event);
            case "dblclick":
              return o.onDblClick &&
                o.onDblClick.call(tree, dtnode, event) === false
                ? false
                : dtnode._onDblClick(event);
            case "keydown":
              return o.onKeydown &&
                o.onKeydown.call(tree, dtnode, event) === false
                ? false
                : dtnode._onKeydown(event);
            case "keypress":
              return o.onKeypress &&
                o.onKeypress.call(tree, dtnode, event) === false
                ? false
                : dtnode._onKeypress(event);
          }
        } catch (e) {
      ...
      function __focusHandler(event) {
        ...
        if (typeof event == "undefined" && window.event == null) return false;
    ...
    $.ui.plugin.add("draggable", "connectToDynatree", {
      ...
      drag: function (event, ui) {
        ...
        if (prevTargetNode && prevTargetNode !== targetNode) {
          prevTargetNode.tree._onDragEvent(
            "leave",
            prevTargetNode,
            sourceNode,
            event,
