...
(function ($) {
  $.extend($.fn, {
    hideJmodal: function () {
      $("#jmodal-overlay").animate({ opacity: 0 }, function () {
        $(this).css("display", "none");
      });
      $("#jquery-jmodal").animate({ opacity: 0 }, function () {
        $(this).css("display", "none");
      });
      $("select").show();
    },
    jmodal: function (setting) {
      ...
      if (jmodal.length === 0) {
        ...
        if (
          ps.buttonText.button1 !== undefined &&
          ps.buttonText.button1 !== ""
        ) {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button1" type="button" value="' +
            ps.buttonText.button1 +
            '" />';
        } else {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button1" type="button" style="display:none; visibility:hidden;" value="" />';
          ps.buttonText.button1 = "";
        }
        if (
          ps.buttonText.button2 !== undefined &&
          ps.buttonText.button2 !== ""
        ) {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button2" type="button" value="' +
            ps.buttonText.button2 +
            '" />';
        } else {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button2" type="button" style="display:none; visibility:hidden;" value="" />';
          ps.buttonText.button2 = "";
        }
        if (
          ps.buttonText.button3 !== undefined &&
          ps.buttonText.button3 !== ""
        ) {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button3" type="button" value="' +
            ps.buttonText.button3 +
            '" />';
        } else {
          buttonHTML +=
            '<input id="perc_msg_dialogue_button3" type="button" style="display:none; visibility:hidden;" value="" />';
          ps.buttonText.button3 = "";
        }
        if (ps.imageType === "Warning") {
          informationHTML =
            "<img src='images/warning.gif' title='Warning'/>" + ps.title;
        } else if (ps.imageType === "Error") {
          informationHTML =
            "<img src='images/error.gif' title='Error'/>" + ps.title;
        } else {
          informationHTML =
            "<img src='images/information.gif' title='Information'/>" +
            ps.title;
        }
        $(
