...
(function ($) {
  $.PercWizard = {
    ...
    beforeTransition: function (currentStep, nextStep, wizardObj) {
      return true;
    },
    afterTransition: function (currentStep, prevStep, wizardObj) {
      return true;
    },
    gotoStep: function (step, me) {
      ...
      if (step < 0) step = 0;
      if (step > this.steps.length - 1) step = this.steps.length - 1;
      ...
      else this.dom.find(".perc-wizard-required-fields-tip").hide();
      ...
      if (content.length == 0 || !this.cache[step]) {
        ...
        if (typeof content == "function") content = content(self.dom);
        $(".perc-wizard-content-" + step + ".perc-wizard-content-step")
          .empty()
          .remove();
        var contentStep = $(
