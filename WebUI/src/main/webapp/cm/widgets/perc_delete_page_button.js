...
(function ($) {
  $.perc_build_delete_page_button = function (mcol, content) {
    ...
    function showForceDeleteDialog(
      id,
      title,
      content,
      chkBoxId,
    ...
    function deleteFolder() {
      ...
      if ($.PercNavigationManager.isAdmin()) {
        $.PercPathService.deleteFolderSkipValidation(
          mcol_path.join("/"),
        ...
        function (specResponse) {
          ...
          if (spec.PathItem.type === "percPage") {
            ...
            // if we are not deleting the landing page
              ...
              $.PercPageService.validateDeletePage(
                spec.PathItem.id,
          ...
          // if we are deleting a folder
            ...
            if (
              spec.PathItem.category === "FOLDER" ||
              (spec.PathItem.category === "SECTION_FOLDER" &&
                mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH)
            ) {
              deleteFolder();
            } else {
              //not allowed
    ...
    function getAssetDeleteQuestionString(spec, data) {
      var message =
        mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
          ? I18N.message("perc.ui.deleteassetdialog.purge@Confirm")
          : I18N.message("perc.ui.deleteassetdialog.warning@Confirm");
      var dialog =
        I18N.message("perc.ui.deleteassetdialog.tag@Asset") +
        ": " +
        spec.PathItem.name +
        "<br/><br/>" +
        message;
    ...
    function validateIfAssetCanBeDeleted(spec, data, takeDownUrl) {
      ...
      var title =
        mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
          ? I18N.message("perc.ui.deleteassetdialog.title@Recycle Asset")
          : I18N.message("perc.ui.deleteassetdialog.title@Delete Asset");
      var message =
        mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
          ? I18N.message("perc.ui.deleteassetdialog.purge@Confirm")
          : I18N.message("perc.ui.deleteassetdialog.warning@Confirm");
      var options = {
        ...
        question:
          warnOpenSpan +
          getAssetDeleteQuestionString(spec, data) +
          warnCloseSpan,
    ...
    function getDeleteQuestionString(spec, data) {
      var message =
        mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
          ? I18N.message("perc.ui.deletepagedialog.purge@Confirm")
          : I18N.message("perc.ui.deletepagedialog.warning@Confirm");
      var dialog =
        I18N.message("perc.ui.deletepagedialog.tag@Delete Page") +
        ": " +
        spec.PathItem.name +
        "<br/><br/>" +
        message;
    ...
    function validateIfPageCanBeDeleted(spec, data, takeDownUrl) {
      ...
      var title =
        mcol_path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
          ? I18N.message("perc.ui.deletepagedialog.title@Recycle Page")
          : I18N.message("perc.ui.deletepagedialog.title@Delete Page");
      var options = {
        id: "perc-finder-delete-confirm",
        title: title,
        question:
          warnOpenSpan + getDeleteQuestionString(spec, data) + warnCloseSpan,
        success: function () {
          $.PercDirtyController.setDirty(false);
    ...
    function purge_item(id, path, type) {
      $.PercRecycleService.purgeItem(id, path, function (status, data) {
        if (status === $.PercServiceUtils.STATUS_ERROR) {
          console.error("Error!");
        } else {
    ...
    function checkIfLinkedPage(spec) {
      var findLinkedItemsUrl =
        $.perc_paths.ITEM_LINKED_TO_ITEM + "/" + spec.PathItem.id;
