 ...
 */
(function ($) {
  $.perc_asset_edit_dialog = function (
    reload_callback,
    cancelCallback,
    assetid,
    widgetData,
    ...
    $.ajax({
      url: $.perc_paths.CONTENT_EDIT_CRITERIA + "/",
      dataType: "json",
    ...
    function createContentEditDialog() {
      ...
      var url =
        contentEditCriteria.url +
        "&sys_folderid=" +
        contentEditCriteria.legacyFolderId +
        "&percpageid=" +
        parentId +
        "&sys_workflowid=" +
          ...
          Cancel: {
            click: function (event) {
              if (event.originalEvent.detail === 0) {
                // if cancel button enabled through tab
                var options = {
      ...
      dialog
        .parent()
        .find(".ui-dialog-titlebar-resize")
        .on("click", function () {
          ...
          if (maximized) {
            dialog_frame.animate(
              ...
              function () {
                ...
                $(this)
                  .find(".ui-dialog-titlebar-resize")
                  .attr("aria-label", "exit full screen");
      ...
      if (
        contentEditCriteria.producesResource === false &&
        ...
        checkboxHtml +=
          '<input type="checkbox" id="perc-content-edit-share-checkbox" name="perc-content-edit-share-checkbox" />';
        checkboxHtml +=
          '<label for="perc-content-edit-share-checkbox">' +
          I18N.message("perc.ui.saveasdialog.label@Shared Asset") +
          "</label>";
        checkboxHtml += "</form>";
        checkboxHtml += "</div>";
    ...
    function onInitialFrameLoad() {
      if (contentEditCriteria.assetType == "shared") {
        ...
        if (assetId != undefined && assetId != "") {
          var pathField =
            '<label for="perc-content-edit-meta-data-item_folder_path" >Folder path:</label> <br/>' +
            '<div id="perc-content-edit-meta-data-item_folder_path" class="datadisplay">' +
            contentEditCriteria.folderPath +
            "</div> <br/>";
    ...
    function saveAssetContent(name, folderPath) {
      ...
      if (
        !$contentFrameElem("#perc-content-form")
          .attr("action")
          .includes("sys_asset_folderid")
      ) {
    ...
    function removeEditAssetDialog() {
      var openSaveAsSharedAsset = dialog
        .parent()
        .find("#perc-content-edit-share-checkbox:checked");
