...
(function ($) {
  ...
  $.fn.PercImageTooltip = function (config) {
    ...
    else _clickCallback = function () {};
    var element = $(this)
      .on("mouseenter", function (event) {
        $.PercImageTooltip.showTooltip(event, _clickCallback);
      })
  ...
  $.PercImageTooltip = {
    tooltipDom: $("<div/>")
      ...
      .css("z-index", "10000")
      .on("mouseenter", function (event) {
        $.PercImageTooltip.enterTooltip(event);
      })
      .on("mouseleave", function (event) {
        $.PercImageTooltip.exitTooltip(event);
      })
    ...
    hiderDom: $("<div class='perc-tooltip-hider'/>")
      ...
      .css("z-index", "9000")
      .on("mouseenter", function (event) {
        $.PercImageTooltip.enterHider(event);
      })
      .on("mouseleave", function (event) {
        $.PercImageTooltip.exitHider(event);
      }),
    ...
    showTooltip: function (event, clickCallback) {
        ...
        var args = {
          ...
          error: function (request, textstatus, error) {
            if ($(".perc-tooltip").position().top !== -10000) {
              setTimeout(function () {
                url =
                  "/Rhythmyx/rx_resources/images/TemplateImages/AnySite/perc.base.plain_Thumb.png?nocache=0.16234142855889466";
      ...
      var imgSrc =
        "/Rhythmyx/rx_resources/images/TemplateImages/" +
      ...
      $.PercImageTooltip.hiderDom
        .css("width", tWidth + 5)
        .css("height", eHeight) // TODO: make 7 into a named constant
        .css("top", eTop)
        .css("left", eLeft)
        .css({
          background: "blue",
        ...
        .on("click", function (event) {
          ...
          if (
            element.data("events") &&
            element.data("events")["click"] &&
            element.data("events")["click"][0] &&
    ...
    hideTooltip: function (event) {
      ...
      if (this.tooltipHoverState === 0 && this.hiderHoverState === 0) {
        this.tooltipDom
          ...
          .find(".perc-tooltip-content")
          .html("");
        this.hiderDom.css("top", -10000).css("left", -10000);
      }
    },
  };
