...
(function ($) {
  $.widget("ui.perc_site_map", {
    ...
    load: function (site, noexpand, postCallback) {
      ...
      this.removeLevel(1);
      if (site) {
        $.Perc_SectionServiceClient.getRootSection(
          site,
          function (status, data) {
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              ...
              self.layoutLevel(1);
              if (!noexpand) self.expandSection(section.id);
              if (typeof postCallback === "function") postCallback();
            } else {
              var msg = data;
              if (data.indexOf("Cannot find object name") !== -1) {
                msg =
                  I18N.message("perc.ui.site.map@Requested Site") +
                  site +
    ...
    layoutAll: function () {
      ...
      for (var i = 1; i <= len; i++) this.layoutLevel(i);
    },
    ...
    layoutLevel: function (idx) {
      ...
      var $nodes = $level
        .children(".perc-site-map-nodes")
        .children(".perc-site-map-node");
      ...
      $level
        .children(".perc-site-map-nodes")
        .children(".perc-site-map-droparea")
        .remove();
      // Layout each node
      $nodes.each(function (i) {
        $(this).css("top", levelTop + topspacing);
        $(this).css("left", leftOffset);
        if (idx > 1) self.addDropArea($(this), levelTop, leftOffset, i);
    ...
    initDropArea: function ($dropArea, targetId) {
      ...
      $dropArea.droppable({
        accept: function (droppable) {
          return self.onDropAccept(this, droppable);
        },
        tolerance: "pointer",
    ...
    addLevel: function (parentId) {
      ...
      if (!isTopLevel) {
        $level.append(
          $("<div></div>")
            .addClass("perc-site-map-level-line")
            .append(
              $("<img/>")
    ...
    removeLevel: function (idx) {
      ...
      var $levels = $(".perc-site-map-level");
      var levelCount = $levels.length;
      if (idx < 1 || idx > levelCount) return;
      var $level = $($levels[idx - 1]);
      // remove any levels under this level first
      if (idx < levelCount) this.removeLevel(idx + 1);
    ...
    addActionsMenuItems: function () {
      ...
      if ($.PercNavigationManager.isAdmin()) {
        ...
        var $delete = $("<div/>")
          .addClass("perc-site-map-action-item")
          .addClass("perc-site-map-action-item-disabled")
      ...
      function isSiteBeingImported(callback) {
        ...
        if (typeof sitename !== "undefined") {
          $.PercSiteService.isSiteBeingImported(
            sitename,
    ...
    onCopySiteDialog: function () {
      ...
      if (
        sitename !== "" &&
        $("#perc-site-map-copy").not(".perc-site-map-action-item-disabled")
          .length > 0
      ) {
    ...
    createSection: function (sectionObj, levelIdx, parentId, idx) {
      var self = this;
      var $section = $("<div></div>")
        .addClass("perc-site-map-node")
        .attr("id", sectionObj.id)
        .attr("perc-section-title", sectionObj.title)
        .hide();
      $section.append(
        $("<div></div>")
          .addClass("perc-site-map-node-topline")
          .append(
            $("<img/>")
              .attr("src", this.getImageSrc(this.LINE_IMAGE))
              .attr("width", "2")
      ...
      if (levelIdx > 1) {
        ...
        // Add Drag & Drop
        $box.draggable({ opacity: 0.7, helper: "clone", delay: 150 });
      }
      $box.droppable({
        accept: function (droppable) {
          return self.onDropAccept(this, droppable);
    ...
    truncateSectionTitle: function (title) {
      if (title.length <= this.MAX_SECTION_TITLE_LENGTH) return title;
    ...
    addNodeButtons: function ($parent, sectionObj, levelIdx) {
      ...
      var configImageName;
      if (levelIdx > 1) configImageName = this.CONFIG_BUTTON_IMAGE;
      else configImageName = this.CONFIG_BUTTON_IMAGE_STANDALONE;
      var configAction = levelIdx > 1 ? this.onConfig : this.onSiteEdit;
      var $configButton = this.createImageButton(
        configImageName,
    ...
    addNodeCountArea: function ($parent, sectionObj, levelIdx) {
      ...
      if (
        ...
        $.Perc_SectionServiceClient.PERC_SECTION_TYPE.EXTERNAL_LINK
      ) {
        var $extLink = $("<div></div>")
          .addClass("perc-site-map-externallink")
          .append("External Link");
        $parent.append($extLink);
      } else if (
        sectionObj.sectionType ===
        $.Perc_SectionServiceClient.PERC_SECTION_TYPE.SECTION_LINK
      ) {
        var $secLink = $("<div></div>")
          .addClass("perc-site-map-sectionlink")
          .append("Section Link")
          .attr("tabindex", "0");
        ...
        var $pageCount = $("<div></div>")
          .addClass("perc-site-map-pagecount")
          .attr("tabindex", "0");
        $nodeCount.append($pageCount);
        var $arrowImg = $("<img/>")
          .attr("src", this.getImageSrc(this.ARROW_OFF_IMAGE))
    ...
    expandSections: function (sectionIds) {
      if (sectionIds == null || sectionIds.length === 0) return;
      var self = this;
      var temp = [];
      ...
      if (!isExpanded) {
        ...
        if (childCount > 0) {
          ...
          var $bottomLine = $("<div></div>")
            .addClass("perc-site-map-node-bottomline")
            .append(
              $("<img/>")
          ...
          $.Perc_SectionServiceClient.getChildren(
            self.getSectionObject($section),
    ...
    convertCXFArray: function (cxfarray) {
      ...
      if (typeof cxfarray != "object") return [cxfarray];
      return cxfarray;
    },
    ...
    onDropAccept: function (target, droppable) {
      ...
      if (
        $source.hasClass("perc-listing-category-PAGE") &&
        $target.hasClass("perc-site-map-box")
      ) {
        var targetData = JSON.parse($target.attr("data"));
        if (
          targetData.sectionType &&
          targetData.sectionType !==
            $.Perc_SectionServiceClient.PERC_SECTION_TYPE.SECTION
        )
          return false;
        var pageSite = spec.path.split("/")[2];
        if ($.PercNavigationManager.getSiteName() !== pageSite) return false;
        return true;
      }
      if (!$source.hasClass("perc-site-map-box")) return false;
      try {
        var targetData = JSON.parse($target.attr("data"));
        ...
        if (typeof targetData.parentId == "undefined") return false; // target is root
        if (targetData.parentId === sourceData.id) return false;
        if (targetData.index === 0) return true;
    ...
    onSiteEdit: function (evt) {
      ...
      self.editSiteSectionDialog.open(
        evt.data.site,
        function (dStatus, fieldData) {
          if (dStatus === "ok") {
            ...
            var secureSite =
              typeof fields["perc-enable-site-security"] != "undefined";
            var canonicalURLs =
              typeof fields["perc-enable-canonical-url"] != "undefined";
            var mobilePreviewEnabled =
              typeof fields["perc-enable-mobile-preview"] != "undefined";
            var canonicalURLsReplace =
              typeof fields["perc-replace-canonical-tags"] != "undefined";
            var generateSitemap =
              typeof fields["perc-site-gen-sitemap"] != "undefined";
            var generateSitemapExcludeImage =
              typeof fields["perc-site-gen-sitemap-exclude-image"] !=
              "undefined";
            ...
            var sitePropsObj = {
              SiteProperties: {
                ...
                folderPermission: {
                  accessLevel: fields["perc-site-folder-permission"],
                ...
                registrationConfirmationPage:
                  fields["perc-site-registration-confirmation-page"],
                resetRequestPasswordPage:
                  fields["perc-site-reset-pw-request-page"],
                ...
                defaultFileExtention:
                  fields["perc-site-pagefile-extention-default"],
                ...
                generateSiteMapOptions: {
                  generateSitemapExcludeImage: generateSitemapExcludeImage,
                  generateSitemap: generateSitemap,
                },
              },
            };
            $.PercBlockUI($.PercBlockUIMode.CURSORONLY);
            $.PercSiteService.updateSiteProperties(
              sitePropsObj,
          ...
          if (dStatus === "ok") {
            ...
            var editSectionObj = {
              SiteSectionProperties: {
                ...
                folderPermission: {
                  accessLevel: fields["perc-section-folder-permission"],
                  writePrincipals: writePrincipals,
                },
            ...
            $.PercBlockUI($.PercBlockUIMode.CURSORONLY);
            $.Perc_SectionServiceClient.edit(
              editSectionObj,
              function (status, data) {
      ...
      $.PercEditSectionLinksDialog().open(
        ...
        function (dStatus, fieldData) {
          if (dStatus === "ok") {
            ...
            if (
              evt.data.sectionId === fields["perc-section-link-targetid"] ||
              evt.data.sectionId.split("_")[0] ===
            ...
            if (self.isChild(parentSectionId, tid)) {
              ...
              $.PercBlockUI($.PercBlockUIMode.CURSORONLY);
              $.Perc_SectionServiceClient.updateSectionLink(
                secLinkObj,
                function (status, data) {
                  self.afterEditSectionCallback(status, data, evt);
                },
          ...
          if (dStatus === "ok") {
            ...
            var extLinkObj = {
              CreateExternalLinkSection: {
                ...
                cssClassNames:
                  fields["perc-external-link-navigation-cssclassnames"],
              },
            };
            $.PercBlockUI($.PercBlockUIMode.CURSORONLY);
            $.Perc_SectionServiceClient.updateExternalLink(
              sectionID,
              extLinkObj,
              function (status, data) {
                self.afterEditSectionCallback(status, data, evt);
      ...
      if (status === $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        if (
          ...
          $("[id*='" + sectionID + "_']").each(function () {
            $(this)
              .find("div.perc-site-map-sectiontitle")
              .text(self.truncateSectionTitle(data.SiteSection.title + ""))
              .attr("title", data.SiteSection.title)
              .attr("alt", data.SiteSection.title);
          });
        ...
        if (oldSectionName !== data.SiteSection.title) {
          // just if the display title change
          $(".perc-site-map-sectionlink")
            .parents(".perc-site-map-node")
            .each(function () {
    ...
    onMoveWithDialog: function () {
      ...
      if ($selected.length > 0) {
        ...
        $.PercSectionTreeDialog.open(
          ...
          selectedData.id,
          treeLabel,
          I18N.message("perc.ui.sitemap.movesection.title@Move Section"),
          "Move",
          function (targetId) {
            self.onMove(
              selectedData.id,
    ...
    onMove: function (
      ...
      var newIndex =
        isSortOnly && sourceIdx < targetIdx ? targetIdx - 1 : targetIdx;
      ...
      $.Perc_SectionServiceClient.move(req, function (status, data, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            ...
            if (isSortOnly) {
              ...
              if (sourceIdx > targetIdx) {
                ...
                Object.assign(targetData, {
                  childIds: result.SiteSection.childIds,
                });
                $target.attr("data", JSON.stringify(targetData));
                $nodes
                  .find(".perc-site-map-node .perc-site-map-box")
                  .each(function (i) {
                    var mData = JSON.parse($(this).attr("data"));
                    $.extend(mData, {
                      index: i,
          ...
          var targetData = JSON.parse($target.attr("data"));
          var newPath =
            targetData.folderPath + oldPath.substring(oldPath.lastIndexOf("/"));
    ...
    onDelete: function (evt) {
      ...
      if (
        ...
        var delConfirmHtml = $(
          '<div class="perc-remove-section-confirm-container">' +
        ...
        $.perc_utils.confirm_dialog({
          ...
          success: function () {
            ...
            if (deleteOption === "delete-section") {
              ...
              $.Perc_SectionServiceClient.convertSectionToFolder(
                evt.data.sectionId,
    ...
    afterDeleteCallback: function (evt, createRedirect) {
      ...
      if (
        (sectionType ===
          $.Perc_SectionServiceClient.PERC_SECTION_TYPE.SECTION &&
          $("[id*='" + evt.data.sectionId + "_']").length > 0) ||
        sectionType ===
          $.Perc_SectionServiceClient.PERC_SECTION_TYPE.SECTION_LINK
      ) {
        var expandState = this.getExpansionState();
    ...
    deleteSectionLink: function (evt) {
      ...
      $.perc_utils.confirm_dialog({
        ...
        success: function () {
          $.Perc_SectionServiceClient.deleteSectionLink(
            evt.data.sectionId,
            parentSectionId,
            function (status, result) {
              if (status === $.PercServiceUtils.STATUS_SUCCESS) {
    ...
    deleteExternalLink: function (evt) {
      ...
      $.perc_utils.confirm_dialog({
        ...
        success: function () {
          $.Perc_SectionServiceClient.deleteSection(
            evt.data.sectionId,
            function (status, result) {
              if (status === $.PercServiceUtils.STATUS_SUCCESS) {
                self.afterDeleteCallback(evt, false);
          ...
          if (dStatus === "ok") {
            ...
            if (fields["perc-section-type"] === "section") {
              ...
              if (allTitle.indexOf(fields.section_name) > -1) {
                ...
                $.perc_utils.alert_dialog({
                  title: "Error",
                  content: duplicateErrorMessage,
                });
                return;
              }
              $.PercSiteService.getSiteProperties(
                self.options.site,
                function (status, result) {
                  if (status === $.PercServiceUtils.STATUS_SUCCESS) {
                  ...
                  $.Perc_SectionServiceClient.create(
                    sectionObj,
                    function (status, data) {
              ...
              if (self.isChild(evt.data.parentSectionId, sid)) {
                ...
                $.Perc_SectionServiceClient.createSectionLink(
                  fields["perc-section-link-targetid"],
                  evt.data.parentSectionId,
                  function (status, data) {
                    self.addNewSectionCallback(status, data, evt);
              ...
              var sectionObj = {
                CreateSectionFromFolderRequest: {
              ...
              $.Perc_SectionServiceClient.convertFolder(
                sectionObj,
                function (status, data) {
                  self.addNewSectionCallback(status, data, evt);
                },
    ...
    getSectionParentChain: function (sectionid, results) {
      ...
      if (
        typeof temp.parentId != "undefined" &&
        temp.parentId != null &&
        temp.parentId.length > 0
      ) {
    ...
    getParentPath: function (sectionId) {
      ...
      $.each(parentArray, function () {
        ppath +=
          "/" +
          $("#" + this)
            .find(".perc-site-map-sectiontitle")
            .attr("title");
      });
      return ppath;
    },
  ...
  function site_delete_handle_error(data, textStatus, errorThrown) {
    ...
    if (data.responseText.indexOf("site.isPublishing") > 0) {
      ut.alert_dialog({
        ...
        content:
          warnOpenSpan +
          I18N.message("perc.ui.deletesiteedialog.warning@SiteTag") +
          " " +
          sitename +
          " " +
          I18N.message("perc.ui.deletesiteedialog.warning@SitePublishing") +
          warnCloseSpan,
      });
    } else if (defMessage !== "") {
      ut.alert_dialog({
      ...
      ut.alert_dialog({
        ...
        content:
          warnOpenSpan +
          I18N.message("perc.ui.deletesiteedialog.warning@GenericText") +
          warnCloseSpan,
      });
    }
    dialog.remove();
