...
(function ($) {
  ...
  function createScrollDivs() {
    ...
    if (opts.directions.indexOf("n") != -1) {
      var scrollUpDiv = createScrollDiv(
        UP,
        viewX,
        opts.offsetY + viewY,
    ...
    if (opts.directions.indexOf("s") != -1) {
      var scrollDownDiv = createScrollDiv(
        DOWN,
        viewX,
    ...
    if (opts.directions.indexOf("w") != -1) {
      var scrollLeftDiv = createScrollDiv(
        LEFT,
        viewX,
    ...
    if (opts.directions.indexOf("e") != -1) {
      var scrollRightDiv = createScrollDiv(
        RIGHT,
  ...
  function createScrollDiv(direction, x, y, width, height) {
    ...
    if (opts.debug) scrollDiv.css("background-color", opts.backgroundColor);
    return scrollDiv;
  }
  function scrollView() {
    var x = 0;
