  ...
  $.widget("ui.template_layout", {
    ...
    _init: function () {
      ...
      $("#perc-clear-page").on("click", function () {
        self.helper.clearPage(function () {
          self.render();
        });
      });
        ...
        percButtons: {
          Ok: {
            click: function () {
              ...
              if (
                self.helper.regionCSS[$("perc-region-name").val()] !=
                self.originalRegionId
              ) {
    ...
    _save: function () {
      var self = this;
      this.helper.save(this.type, function () {
        self.render();
      });
    },
    ...
    _load: function (html) {
      ...
      $("#perc-edit").on("click", function () {
        if (self.selectedRegion) {
          ...
          var widgetId = self.selectedWidget.attr("id");
          var widgetParentRegionId = self.selectedWidget
            .parent()
            .parent()
            .attr("id");
    ...
    _autoSizeHorizontal: function () {
      ...
      $(".perc-horizontal").each(function () {
        ...
        var parentInnerWidth =
          parentWidth - (2 * self.padding + 2 * self.borderWidth) * depth;
        //	if(depth > 1)
    ...
    _regionize: function () {
      ...
      $(".perc-region .perc-sortable").sortable({
        ...
        stop: function (event, ui) {
          ...
          ui.item
            ...
            .children()
            .each(function () {
              if ($(this).attr("id") != ui.item.attr("id")) pos++;
              else return false;
            });
          moveWidgetToRegion = ui.item.parent().parent().attr("id");
          self.helper.orderWidget(
            moveWidget,
      ...
      $(".perc-widget").on("click", function (event) {
        ...
        $("#perc-menu")
          ...
          .css("left", $(this).width() + $(this).offset().left - 45)
      ...
      $(".perc-region").on("click", function (event) {
        ...
        $("#perc-menu")
          .css("position", "absolute")
          .css("left", $(this).width() + $(this).offset().left - 25)
          .css("top", $(this).offset().top)
          .css("zIndex", "10")
          .css("display", "block");
      });
      ...
      $(".perc-horizontal > .perc-region").resizable({
        ...
        stop: function (event, ui) {
          if (
            self.type == self.helper.Type.PAGE &&
            !self.helper.isRegionResizable($(this).attr("id"))
          ) {
      ...
      $(".perc-vertical > .perc-region").draggable({
        ...
        helper: function () {
          var height =
            $(this).height() + 2 * self.padding + 2 * self.borderWidth;
          var width = $(this).width() + 2 * self.padding;
          return (
            "<div style='width:" +
            width +
            "px;height:" +
            height +
            "px;background-color:grey;zIndex:5;opacity:0.4'></div>"
          );
        },
        stop: function () {
      ...
      $(".perc-horizontal > .perc-region").draggable({
        ...
        helper: function () {
          var height =
            $(this).height() + 2 * self.padding + 2 * self.borderWidth;
          var width = $(this).width() + 2 * self.padding + 2 * self.borderWidth;
          return (
            "<div style='top:" +
            $(this).position().top +
            "px;width:" +
      ...
      $(".perc-region").each(function () {
        ...
        var dirs = $.map(["north", "south", "east", "west"], function (dir) {
          return make_feedback_droppable(region_id, dir);
        });
        ...
        return $("<div/>")
          ...
          .droppable({
            ...
            drop: function () {
              if (
                self.type == self.helper.Type.PAGE &&
                $.inArray(region_id, self.helper.page.lockedTemplateRegions) !=
                  -1
              ) {
                alert(
      ...
      $(".perc-region").droppable({
        ...
        out: function (event, ui) {
          $(".perc-region").removeClass("hovering");
        },
        drop: function (event, ui) {
          ...
          if (
            self.type == self.helper.Type.PAGE &&
            $.inArray(regionId, self.helper.page.lockedTemplateRegions) != -1
          ) {
