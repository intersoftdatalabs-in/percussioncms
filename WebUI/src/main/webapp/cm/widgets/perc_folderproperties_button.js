  ...
  $.perc_build_folderproperties_button = function (finder, contentViewer) {
    ...
    function finderPathChangedListener(path) {
      ...
      if (selectedItem.length > 0) {
        ...
        if (
          pathItemSpec.type !== "Folder" &&
          pathItemSpec.type !== "FSFolder"
        ) {
        ...
        folderSelector +=
          ", .mcol-listing.perc-listing-type-Folder.mcol-opened";
    ...
    function clickHandler(evt) {
      ...
      if (pathItemSpec.category === "SYSTEM") {
        ...
        pathItemSpec.accessLevel !== $.PercFolderHelper().PERMISSION_ADMIN
      ) {
        var type =
          pathItemSpec.category === "SECTION_FOLDER" ? "section" : "folder";
        $.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.page.general@Warning"),
          content:
            I18N.message("perc.ui.folder.properties.button@Permissions Error") +
            type +
            ".",
        });
        return;
      ...
      $.PercFolderPropertiesDialog().open(
        pathItemSpec,
        function (newName, status) {
          if (status === undefined) {
    ...
    function updateButtonUrl(flag) {
      ...
      if (flag) {
        downloadUrl =
          $.perc_paths.WEBRESOURCESMGT +
          "/" +
          finder.getCurrentPath().slice(3).join("/");
      } else {
        downloadUrl = "#";
      }
