...
(function ($) {
  $.fn.PercScrollingTemplateBrowser = function (config) {
    ...
    $(this).append(prev).append(scrollable).append(next).append(clearboth);

    if (config.width) scrollable.width(config.width);

    var widgetDefId = null;
    if (config.widgetDefId) widgetDefId = config.widgetDefId;

    var siteName = config.siteName;
    var hiddenFieldId = config.hiddenFieldId
      ? config.hiddenFieldId
      : "perc-select-template";
