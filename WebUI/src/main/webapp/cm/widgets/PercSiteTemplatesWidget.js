...
(function ($) {
  $.widget("ui.template_selected", {
    ...
    autoscrollToSelected: function () {
      ...
      if ($("#perc-assigned-templates .perc-selected").length != 0) {
        if (
          $.Percussion.getCurrentTemplatesView() ==
          $.Percussion.VIEW_MODE_CATALOG
        ) {
    ...
    updateTemplatesPagingSize: function () {
      ...
      if (
        $("#perc-assigned-templates .perc-template-item").length > amountOfItems
      )
        $("#perc-assigned-templates").data("scrollable").getConf().size =
          amountOfItems;
    },
    ...
    _updateSiteTemplates: function () {
      ...
      if (
        $.Percussion.getCurrentTemplatesView() ==
        $.Percussion.VIEW_MODE_THUMBNAILS
      ) {
      ...
      for (var t = 0; t < tLen; t++) {
        ...
        if (this.selectedTemplateName == template.getTemplateName())
          selected = "class='perc-selected'";
        buffer =
          '<div class="perc-template-item" tabindex="0" id="' +
          template.getTemplateId() +
          '" ' +
          selected +
          ">";
        buffer +=
          '    <button class="perc-apply" tabindex="0" id="' +
          template.getTemplateId() +
          '-apply" title="' +
          I18N.message(
          ...
          "</button>";
        buffer +=
          '    <button class="perc-edit" tabindex="0" id="' +
          template.getTemplateId() +
          '-edit" title="' +
          I18N.message("perc.ui.site.templates.widget@Edit Template") +
          '">' +
          "</button>";
        buffer +=
          '    <button class="perc-delete" tabindex="0" id="' +
          template.getTemplateId() +
          '-delete" title="' +
          I18N.message("perc.ui.admin.packed@Delete Template") +
          '">' +
          "</button>";
        buffer +=
          '    <div class="perc-template-label" title="' +
          originalName +
          '" id="' +
          template.getTemplateId() +
          '-name">' +
          ...
          "</div>";
        buffer +=
          '    <div data-base-template="' +
          template.getBaseTemplateName() +
          '" id="' +
          template.getTemplateId() +
          'id" class="perc-template-thumbnail-container perc-template-background">';
        buffer +=
          '         <img height = "122px" width="174px" src = "' +
          template.getImageUrl() +
          '" alt="' +
          I18N.message("perc.ui.new.site.dialog@Basic Template") +
          '" />';
        buffer += "    </div>";
        buffer +=
          ' <div class="perc-template-paging-container"' +
          hideInThumbnail +
          ">";
        buffer +=
          '    <ul class="perc-template-pages" data-perc-template-version ="' +
          template.getContentMigrationVersion() +
          ' "templateId="' +
          template.getTemplateId() +
          '">';
        ...
        buffer +=
          ...
          '" style="right:56px;" aria-disabled="false" class="previous">Prev</div>';
        buffer +=
          '        <input type="text" class="perc-jump" name="perc-template-pages-controls-jump" autocomplete="off" />';
        buffer +=
          '        <div role="button" tabindex="0" title="' +
          I18N.message("perc.ui.common.label@Next") +
          '"style="right:0px;" aria-disabled="false" class="next">Next</div>';
        buffer += "    </form>";
        buffer +=
          '    <div class="perc-template-showing" style="width: 100px; margin: 6px 0pt 0px 24px;">';
        buffer +=
          '        Items: <span class="perc-template-page-group-range">0-0</span>';
        buffer += "    </div>";
        buffer +=
          '    <div class="perc-template-showing" style="width: 100px; margin: 8px 0pt 13px 24px;">';
        buffer +=
          '        Total Items: <span class="perc-template-total">0</span>';
        buffer += "    </div>";
        buffer += "    </div>";
        buffer += "</div>";
        ...
        carouselContainerDiv
          .find("div#" + template.getTemplateId())
          .data("positionInCarousel", t);
        ...
        $("#" + template.getTemplateId() + " li").on("click", function (event) {
          ...
          self.selectedPageIdOriginalTemplateId = $(this)
            .parents(".perc-template-item")
            .attr("id");
        });
      ...
      $(".perc-template-pages").droppable({
        ...
        drop: function (event, ui) {
          ...
          var referencePageId = $(this)
            .find("li.perc-template-pages-items")
            .not(".perc-require-migration")
            .first()
            .attr("id");
          var droppedPageIds = [];
          ...
          var migrateContentRestData = {
            MigrateContentRestData: {
              ...
              refPageId: referencePageId,
              pageIds: droppedPageIds,
            ...
            function (status, result) {
              if (status) {
                ...
                if (
                  ...
                  typeof ui.helper.dropCallback === "function"
                ) {
                  ui.helper.dropCallback();
                }
                if (sourceTemplateId) {
      ...
      if (
        $.Percussion.getCurrentTemplatesView() == $.Percussion.VIEW_MODE_CATALOG
      ) {
        if ($(".perc-prev-templates-carousel").length == 0) {
          ...
          assignedTemplatesDiv
            .parents("#perc-activated-templates-scrollable")
            .prepend(scrollbarDivRight);
          assignedTemplatesDiv
            .parents("#perc-activated-templates-scrollable")
            .prepend(scrollbarDivLeft);
        }
      ...
      $("#perc-activated-templates .perc-template-pages-controls .next").on(
        "click",
        function () {
      ...
      $(".resetPaging").on("click", function (event) {
        if (
          typeof event.deletedPageId !== "undefined" &&
          event.deletedPageId === self.selectedPageId
        ) {
          var firstPageId =
            self.firstPagesArray[self.selectedPageIdOriginalTemplateId];
      ...
      $("#perc-activated-templates .perc-template-pages-controls .previous").on(
        "click",
        function (evt) {
          var templateContainer = $(this).parents(".perc-template-item");
          var templateId = $(templateContainer).attr("id");
      ...
      $("#perc-activated-templates .perc-template-pages-controls").on(
        "submit",
        function () {
      ...
      $("#perc-assigned-templates-container").on(
        "click",
        ".perc-apply",
        function () {
          var templateId = $(this).attr("id");
      ...
      function _migrateTemplateContent(templateId) {
        $.PercBlockUI();
        $.PercTemplateService().applyTemplate(
          templateId,
          "ALL",
          function (status, result) {
            if (status) {
            ...
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              ...
              for (var i = 0; i < listElements.length; i++) {
                $(listElements[i])
                  .html("")
                  .attr("title", "")
                  .removeClass("perc-require-migration");
              }
              ...
              for (var j = 0; j < data.childrenInPage.length; j++) {
                ...
                if (
                  curPageId == self.selectedPageId ||
                  (pageId && curPageId == pageId)
                ) {
                ...
                var dropDownElement =
                  '<span class="perc-template-pages-items-dropdown"></span>';
                var migrationEmptyWidgetIndicator =
                  typeProps["migrationEmptyWidgetFlag"] == "yes"
                    ...
                    : "";
                var html =
                  '<span class="perc-template-pages-items-thumbnail"><img tabindex="0" src="../images/images/inspectButton.png" alt="perc.ui.extend.ui.dialog@Inspect Button"/></span><div tabindex="0" class="perc-template-page-title">' +
                  data.childrenInPage[j].name +
                  "</div>" +
                  migrationEmptyWidgetIndicator +
                  dropDownElement;
                //because if pagePath is empty, then page is in recycle
                //don't add drop down and change icon to broken
                if (recycledPage) {
                  html =
                    '<span class="perc-template-pages-items-thumbnail"><img tabindex="0" src="../images/images/brokenlink.png" alt="perc.ui.recycledPage@RecycledPage"/></span><div tabindex="0" class="perc-template-page-title">' +
                    data.childrenInPage[j].name +
                    "</div>" +
                    migrationEmptyWidgetIndicator +
                    dropDownElement;
                ...
                if (!recycledPage)
                  curPageElem
                    .find(".perc-template-pages-items-thumbnail img")
                    .attr("data", JSON.stringify(data.childrenInPage[j]));
                if (typeProps["contentMigrationVersion"] !== templateVersion) {
                  curPageElem
                    .find(".perc-template-pages-items-thumbnail img")
                    .addClass("perc-require-migration");
                }
                var dropDownConfig = {
                  percDropdownRootClass:
                    "perc-dropdown-template-pages-items-dropdown",
                  percDropdownOptionLabels: [
                    "",
                    I18N.message("perc.ui.site.templates.widget@Open Page"),
                ...
                if (recycledPage) {
                  dropDownConfig = {
                    ...
                    percDropdownOptionLabels: [
                      "",
                      I18N.message("perc.ui.page.menu@Change Template"),
                      I18N.message("perc.ui.restorepage@Label"),
                      I18N.message("perc.ui.purgepage@Label"),
                    ],
                    percDropdownCallbacks: [
                      function () {},
                      _changeTemplate,
                ...
                curPageElem
                  .find(".perc-template-pages-items-dropdown")
      ...
      function _restorePage(callbackData) {
        var templateId = callbackData.data.templateId;
        var pageId = callbackData.data.pageId;
        $.PercRecycleService.restoreItem(
          pageId,
          $.perc_paths.PAGE_RESTORE,
          function (status, data) {
            if (status === $.PercServiceUtils.STATUS_ERROR) {
              $.perc_utils.alert_dialog({
                title: I18N.message("perc.ui.publish.title@Error"),
      ...
      function updateIndicators(templateId, totalNumberOfItems, startIndex) {
        ...
        if (startIndex <= 1) {
          $("#" + templateId)
            .find(".perc-template-pages-controls div.previous")
        ...
        if (endIndex >= totalNumberOfItems) {
          $("#" + templateId)
            .find(".perc-template-pages-controls div.next")
            .removeClass("next")
      ...
      function getCurrentStartIndex(templateId) {
        var currentPageGroupRange = $("#" + templateId)
          .find(".perc-template-page-group-range")
          .html();
      ...
      function editTemplate(templateId) {
        ...
        if (
          templateId == self.selectedPageIdOriginalTemplateId &&
          $.Percussion.getCurrentTemplatesView() ==
            $.Percussion.VIEW_MODE_CATALOG
        ) {
        ...
        var currentTemplateName = $("#" + templateId)
          .find(".perc-template-thumbnail-container")
          .attr("data-base-template");
        var memento = {
      ...
      var self = this;
      $(".perc-template-thumbnail-container")
        .parent()
        .on("click", function (event) {
          ...
          if (
            ($(event.target).parent().hasClass("perc-template-pages-items") ||
              $(event.target).hasClass("perc-template-pages-items")) &&
            $(event.target).attr("title") != ""
          )
            self._selectTemplate($(this).attr("id"), false);
          else self._selectTemplate($(this).attr("id"), true);
        });

      $(".perc-template-pages-items").on("dblclick", function (event) {
        var templateId = $(event.target)
          .parents(".perc-template-item")
          .attr("id");
        var pageId = $(event.target).attr("id");
        if (pageId == null)
          pageId = $(event.target)
            .parents(".perc-template-pages-items")
            .attr("id");
      ...
      function loadPageThumbnail(element, pageId) {
        $(element)
          .parents(".perc-template-item")
          .find(".perc-template-thumbnail-container img")
          .attr(
            "src",
      ...
      $(".perc-edit").on("click", function () {
        ...
        if (
          ...
          typeof self.selectedPageId === "undefined" ||
          self.selectedPageId == null ||
          $(this)
            .closest(".perc-template-item")
            .find("#" + self.selectedPageId).length == 0
        ) {
      ...
      $(".perc-delete").on("click", function () {
        ...
        var options = {
          ...
          question:
            I18N.message("perc.ui.admin.packed@Template") +
            $("#" + templateId + "-name").text() +
            "<br/><br/>" +
            I18N.message("perc.ui.sitetemplates.warning@Confirm"),
          //type:"YES_NO",
          cancel: function () {},
          success: function () {
            $.PercBlockUI();
    ...
    _selectTemplate: function (templateId, doCleanSelectedPage) {
      ...
      $(".perc-template-item")
        .removeClass("perc-template-page-highlighted-color")
        .removeClass("perc-template-highlighted-color-border")
        .removeClass("perc-selected");
      ...
      if (doCleanSelectedPage) {
        ...
        // Load Thumbnail
        $(".perc-template-pages li#" + firstPageId)
          .parents(".perc-template-item")
          .find(".perc-template-thumbnail-container img")
          .attr(
            "src",
    ...
    _assignSiteToCurrentlySelectedTemplate: function (check) {
      ...
      if (check.checked)
        this.controller.assignTemplateToSite(template, check.id);
  ...
  function createDialog(isError) {
    ...
    if (isError) {
      dialogHTML =
        "<div id='perc-import-template-wrapper'>" +
        "<iframe name='perc-import-template-frame' id='perc-import-template-frame' height='83px' FRAMEBORDER='0' width='100%' src='../app/importTemplate.jsp?status=PERC_ERROR&message=The file you attempted to import is not a valid XML file. Choose a valid CM1 template XML file for upload'></iframe>" +
        "</div>";
    }
  ...
  function getTypePropertyMap(typePropertyEntries) {
    ...
    else tempArray = typePropertyEntries.entry;
    var result = [];
  ...
  function _exportTemplate(callbackData) {
    var templateId = $("#perc-activated-templates")
      .find(".perc-selected")
      .attr("id");
    var templateName = $("#" + templateId)
      .find(".perc-template-thumbnail-container")
      .attr("data-base-template");
    $.PercTemplateService().exportTemplate(
      templateId,
      templateName,
      function (status, result) {
        if (status) {
