...
(function ($) {
  var defaultConfig = {
    expandControl:
      "<b>[+]" + I18N.message("perc.ui.collapsible.more@More") + "</b>",
  ...
  $.fn.percCollapsibleMore = function (config) {
    ...
    if (content) collapsible.append(content);
  ...
  function collapse(event) {
    ...
    collapsible.height(newHeight).css("overflow", "hidden");
    toggleControl
      .html(toggleIcon)
      .off()
  ...
  function contentDoesntFit(collapsible) {
    ...
    if (numberOfLines > maxLines) return true;
    return false;
  }
  ...
  function handleCollapse(event) {
    ...
    if (target.is(".perc-collapse-control")) collapseControl = target;
    if (!collapseControl)
