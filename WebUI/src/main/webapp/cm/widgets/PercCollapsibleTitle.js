...
(function ($) {
  ...
  $.fn.percCollapsibleTitle = function (config) {
    ...
    if (needsCollapsable) {
      ...
      collapseControl.css("position", "absolute").css("left", "15px");
  ...
  function handleCollapse(event) {
    ...
    if (target.is(".perc-collapsibletitle-toggle")) collapseControl = target;
    if (!collapseControl)
    ...
    if (isCollapsed) {
      ...
      if ($.browser.msie && newHeight <= 0) newHeight = 1;
      secondLine.height(newHeight).css("overflow", "hidden");
    }
  }
})(jQuery);
