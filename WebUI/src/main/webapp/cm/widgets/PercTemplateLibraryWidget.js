...
(function ($) {
  $.widget("ui.template_library", {
    ...
    _init: function () {
      ...
      //{
      self.controller.getSites(
        false,
        function (percSiteTemplatesController, sites) {
    ...
    _loadTemplates: function () {
      ...
      function updateTemplates(siteName) {
        var defer = $.Deferred();
        $.PercSiteTemplatesController(false).load(
          function () {
            self.templates = $.PercSiteTemplatesController(false).getTemplates(
              "site",
              filterVal,
      ...
      function displayTemplates(loadTemplatesContext) {
        ...
        if (
          !loadTemplatesContext.templates ||
          loadTemplatesContext.templates.length === 0
        )
          return;
        ...
        for (var t = 0; t < tLen; t++) {
          ...
          lastTemp = template;
          buff +=
            '<td><button type="button" data-base-template="' +
            template.getTemplateName() +
            '" id="perc-template-' +
            template.getTemplateId() +
            '" class="template" style="display:table-cell; ">\n';
          buff +=
            '<img style="border:1px solid #E6E6E9" src="' +
            template.getImageUrl() +
            '" class="perc-template-thumbnail"/>\n';
