...
(function ($) {
  ...
  $.perc_finder = function () {
    if ($.perc_finderInstance == null) $.perc_finderInstance = finder();
  ...
  function finder() {
    ...
    $("body").append(
      '<div id="perc-wide_scroll_div_one" style="width:50px;height:50px;overflow-y:hidden;position:absolute;top:-200px;left:-200px;"><div id="wide_scroll_div_two" style="height:100px;width:100%"></div></div>',
    );
    var SCROLLBAR_WIDTH =
      $("#perc-wide_scroll_div_one").width() -
      $("#wide_scroll_div_two").innerWidth();
    ...
    ((path_changed = function (p) {
      ...
      (isLibMode =
        typeof gInitialScreen !== "undefined" && gInitialScreen === "library"));
    //Preload images
    $.perc_utils.preLoadImages("/cm/images/images/loading.gif");
    ...
    $("body").on(
      "click",
      ".perc-action-goto-or-search",
      finder_do_goto_or_search,
    );
    ...
    $(document).on("mousedown", function (evt) {
      if (
        evt.target.id !== "perc-finder-go-action" &&
        evt.target.id !== "mcol-path-summary"
      ) {
        $("#mcol-path-summary").trigger("blur");
        if (
          evt.target.id !== "perc-finder-search-submit" && // Need this condition to clean path when performing search
    ...
    function goToNewPath(viaGoButton) {
      ...
      newPath =
        newPath === "/" || newPath === "" ? "/" + getCurrentPath()[1] : newPath;
      if (
        newPath === currPath &&
        viaGoButton === true &&
        $.Percussion.getCurrentFinderView() !==
          $.Percussion.PERC_FINDER_SEARCH_RESULTS && // This condition avoids the check when
        $.Percussion.getCurrentFinderView() !== $.Percussion.PERC_FINDER_RESULT
      ) {
        // in search view to force the path change
        return;
      } else {
        $.PercPathService.validatePath(newPath, function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            ...
            if (
              $.PercNavigationManager.getView() ===
              $.PercNavigationManager.VIEW_EDITOR
            ) {
              var viewWrapper = $.PercComponentWrapper(
                "perc-action-finder-go-clicked",
                ["perc-ui-component-finder"],
              );
        ...
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            ...
            $.perc_utils.alert_dialog({
              ...
              content:
                I18N.message("perc.ui.finder@Path not found") +
                newPath.join("/"),
            });
            refresh();
          }
    ...
    function showFinderErrorMessage(show, message) {
      ...
      if (show) {
        $message
          .empty()
          .append('<i class="icon-bell fas fa-bell" aria-hidden="true"></i>');
        $('<label class="perc-finder-error"></label>')
          .text(message)
          .appendTo($message);
        $message.fadeIn();
      } else {
    ...
    function set_finder_height_in_cookie(height) {
      if (!isLibMode && get_finder_height_from_cookie() !== height) {
        var state = get_finder_is_collapsed_from_cookie()
          ? "collapsed"
          : "expanded";
    ...
    $("#perc-finder-expander").on("click", function (evt) {
    ...
    var update_finder_height = function () {
      ...
      function on_stop_resize(event) {
        notify_resize();
        $(this).css("width", "auto");
        if (event) set_finder_height_in_cookie(height);
      ...
      function set_finder_column_heights() {
        ...
        $finder_columns.css(
          "height",
          height - get_scrollbar_width(n) - toolbar,
        );
      }
      ...
      function set_finder_body_height(new_height) {
        ...
        if (isLibMode) new_height = defaults.get_max_height() - 100;
      ...
      $(window).on("resize", function on_window_resize() {
        $(".perc-finder-body").resizable(
          "option",
          "maxHeight",
          defaults.get_max_height(),
        );
      });

      // initialize height from the cookie or use default
      set_finder_body_height(
        integer(get_finder_height_from_cookie(), defaults.height),
      );
    ...
    var expandFinder =
      $("[view=PERC_SITE]").length || !get_finder_is_collapsed_from_cookie();
    if (isLibMode) expandFinder = true;
    ...
    function idFromItem(item) {
      var postfix =
        typeof item.id === "undefined" ? item.path.split("/")[1] : item.id;
    ...
    function refresh(k) {
      ...
      if (
        $.PercNavigationManager.getView() ===
          $.PercNavigationManager.VIEW_EDITOR &&
        (fwrapper == null ||
          fwrapper.wrapperName !== "perc-action-finder-refresh")
      ) {
        var compArray = [];
        compArray.push("perc-ui-component-finder");

        var viewWrapper = $.PercComponentWrapper(
          "perc-action-finder-refresh",
          compArray,
        );
        var isWrapperSet = $.PercViewReadyManager.setWrapper(viewWrapper);
        if (!isWrapperSet) {
    ...
    function search(item_list) {
      ...
      dir_children(first_dir)
        .filter(".mcol-opened")
        .removeClass(".mcol-opened");
    ...
    function open(path, k) {
      if (finderOpenInProgress) return; // Only one open action can be in progress at any one time
    ...
    function _open(dir, loader, path, new_path, k) {
      ...
      if (path.length > 0) dir.data("child", path[0]);

      if (
        new_path.length > 1 &&
        new_path[1] === $.perc_paths.DESIGN_ROOT_NO_SLASH
      ) {
      ...
      dir_container(dir).append(
        '<div class="perc-finder-panel-loading"><span class="icon-spinner icon-spin icon-2x"></span>&nbsp;Loading...</div>',
      );
      loader(onLoad, dir);
      function onLoad(children, content) {
        ...
        if (content) {
          ...
          dir
            .find(".mcol-direc-wrapper")
            .empty()
            .addClass("mcol-direc-wrapper-last")
            .append(content);
        } else {
        ...
        if (path.length === 0) {
          ...
          fwrapper = $.PercViewReadyManager.getWrapper(
            "perc-ui-component-finder",
          );
          if (fwrapper != null)
            fwrapper.handleComponentProgress(
              "perc-ui-component-finder",
              "complete",
            );
    ...
    function pagePrevious(evt, target) {
      ...
      load_folder(
        dir,
        $(this).closest(".perc-paging-finder").is(".perc-paging-finder-bottom"),
      );
    }
    ...
    function load_folder(dir, scrollBottom) {
      ...
      var str_path =
        $.perc_utils.encodeURL(path.join("/")) +
        "/?startIndex=" +
        startIndex +
        "&maxResults=" +
        MAX_RESULTS;
      ...
      function getChildren(folder_spec) {
        var children = {};
        $.each(
          $.perc_utils.convertCXFArray(
            folder_spec.PagedItemList.childrenInPage,
          ),
          function () {
        ...
        update_dir(dir, children);
        if (scrollBottom)
          dir.find(".mcol-direc-wrapper").attr({
            scrollTop: dir.find(".mcol-direc-wrapper").attr("scrollHeight"),
          });
      }
    }
    ...
    function pagingHeader(dir, position) {
      ...
      if (MAX_RESULTS >= totalResult) {
        ...
        var endIndex =
          startIndex - 1 + MAX_RESULTS > totalResult
            ? totalResult
            : startIndex - 1 + MAX_RESULTS;
      ...
      var header = $('<div class="perc-paging-finder"/>')
        ...
        .append(
          $('<div class="perc-paging-finder-navigator" />')
            .append(
              $('<a class="perc-paging-finder-previous" />')
                .text("<<")
            ...
            .append(
              $('<a class="perc-paging-finder-next"/>')
                .text(">>")
                .on("click", function (evt) {
                  pageNext(evt, $(this));
                }),
      ...
      if (totalResult - (startIndex - 1 + MAX_RESULTS) > 0)
        header
          .find(".perc-paging-finder-next")
          .removeClass("perc-hide-navigator");
      else
      ...
      if (startIndex > MAX_RESULTS)
        header
          .find(".perc-paging-finder-previous")
          .removeClass("perc-hide-navigator");
      else
        header
          .find(".perc-paging-finder-previous")
          .addClass("perc-hide-navigator");
    ...
    function pagingHeaderCountOnly(dir, position) {
      ...
      var header = $(
        '<div class="perc-paging-finder perc-paging-finder-top "/>',
      )
        .data("name", position)
        .append($('<span class="perc-paging-text" />').text(headerText))
        .append("");
    ...
    function update_dir(dir, children) {
      ...
      $.each(ut.odiff(curr_children, children), function () {
        ...
        if (this.is(".mcol-opened")) close_after(dir);

        //Make sure the resizable handlers are not removed on any refresh event
        if (
          !dragging &&
          !this.hasClass("ui-resizable-e") &&
          !this.parent().hasClass("perc-view-column-fixed")
        )
          this.remove();
      });
      ...
      if (
        dir.data("path").join("/") !== "" &&
        dir.find(".perc-paging-finder-top").length === 0
      ) {
        if (
          assetPagination &&
          (dir.data("path").indexOf("Assets") > -1 ||
            dir.data("path").indexOf("Sites") > -1)
        ) {
          dir_container(dir).prepend(
            pagingHeaderCountOnly(dir, "perc-paging-finder-top"),
          );
        } else {
          dir_container(dir).prepend(
            pagingHeader(dir, "perc-paging-finder-top"),
          );
        }
      }
    ...
    function make_item(spec, open_rel) {
      ...
      if (spec.type) listing.addClass("perc-listing-type-" + spec.type);
      if (spec.category)
        listing.addClass("perc-listing-category-" + spec.category);

      if (spec.leaf) {
        listing.data("loader", function (onLoad) {
          make_leaf_summary(spec, function (itemPropsContent) {
            onLoad(null, itemPropsContent);
          });
        });
      } else {
        listing.data("loader", load_folder_path(ut.acop(item_path)));
      }
      ...
      function hoverStart(event, ui) {
        ...
        if (
          !canDrop(
            itemPath,
            targetPath,
            ui.draggable.data("spec").type,
            targetType,
          )
        )
          return;
        if (
          spec.accessLevel !== $.PercFolderHelper().PERMISSION_READ &&
          ui.draggable.data("spec").accessLevel !==
            $.PercFolderHelper().PERMISSION_READ
        )
          $(this).addClass("perc-finder-item-over");
        if ($(this).hasClass("perc-listing-type-site")) return; // do not expand a site node
      ...
      function onDrop(event, ui) {
        ...
        if (
          ...
          return false;
        if (!canDrop(itemPath, targetPath, itemType, targetType)) return false;
        $.PercPathService.moveItem(
          ...
          function (status, data) {
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              ...
              if (
                data.indexOf(
                  "item with the same name already exists in the folder",
                ) !== -1
              ) {
                var itemLabel = "asset";
                if (itemType === "percPage") {
                ...
                content = I18N.message("perc.ui.finder.move.error@Duplicate", [
                  itemLabel,
                  ui.draggable.data("spec").name,
                  targetPath,
                ]);
              }
              $.perc_utils.alert_dialog({
      ...
      function canDrop(itemPath, targetPath, itemtype, targetType) {
        ...
        if (itemtype === "Folder") {
          ...
          if (sPath[sPath.length - 1] === "") sPath.pop();
          tPath = targetPath.substr(1).split("/");
          if (tPath[tPath.length - 1] === "") tPath.pop();
          var isSame = true;
      ...
      function isDraggableItem(item) {
        ...
        if (type === "percPage" && cat === "LANDING_PAGE") return false;
        if (type === "Folder" && cat === "SECTION_FOLDER") return false;
        if (
          type === "Folder" &&
          item.accessLevel !== $.PercFolderHelper().PERMISSION_ADMIN
        )
          return false;
        if (type === "FSFile") return false;
        if (type === "FSFolder") return false;
        return true;
      }
      ...
      function isDroppableItem(item) {
        var type = item.type;
        var cat = item.category;
        if (!type && item.path === $.perc_paths.ASSETS_ROOT + "/") return true;
        if (!type) return false;
        if (type === "percPage") return false;
        return true;
      }
      ...
      function dragAcceptor(item) {
        ...
        if (
          item.data("spec").category &&
          item.data("spec").category === "ASSET"
        ) {
          if (!spec.type || spec.type !== "Folder") return false;
        }

        if (
          our_path.length >= their_path.length &&
          $.grep(their_path, function (c, ii) {
            return c === our_path[ii];
          }).length === 0
        ) {
      ...
      function onClick(evt) {
        if (evt) {
          if (
            evt.currentTarget &&
            evt.currentTarget.id === "perc-finder-listing-Search"
          ) {
            $.Percussion.setView("search");

            //Set the Search icon for when highlighted
            $("#perc-finder-listing-Search")
              .find("img")
              .attr("src", "/cm/images/images/searchIcon_on.png");
        ...
        $("#perc-finder-listing-Search")
          .find("img")
          .attr("src", "/cm/images/images/searchIcon.png");
        ...
        if (len === 0) {
          ...
          if (
            ...
            var viewWrapper = $.PercComponentWrapper(
              "perc-action-finder-item-clicked",
              ["perc-ui-component-finder"],
            );
            var isWrapperSet = $.PercViewReadyManager.setWrapper(viewWrapper);
            if (!isWrapperSet) {
    ...
    function onDragStop() {
      ...
      dragging = false;
      $("body").css("overflow", "hidden").off("mousemove.finderDrag");
    ...
    function make_leaf_summary(spec, callback) {
      var summary = " ";
      $.perc_pathmanager.getItemProperties(
        spec.path,
        function (status, itemProps) {
          if (status) {
            var nameType = null;
            var type = null;
            var linkTag = null;
            var isAssetResource =
              spec.category === "ASSET" || spec.category === "RESOURCE";
            if (isAssetResource) {
              //Asset/Type
              nameType = "Asset";
              type = "Type";
              if (!spec.path.includes("Recycling")) {
                linkTag =
                  "<a href='#' class='perc-finder-preview-link' id='perc-asset-preview-link' title='Click for preview'>";
              } else {
                linkTag = "";
              }
            } else {
              ...
              if (!spec.path.includes("Recycling")) {
                linkTag =
                  "<a href='#' class='perc-finder-preview-link' id='perc-page-preview-link' title='Click for preview'>";
              } else {
                linkTag = "";
              }
            }
            ...
            if (
              ...
              lpdate =
                '<div style="padding:9px 0 0 10px;">Last Published: <span></span></div>' +
                '<div style="padding:3px 0 0 10px;"><span "perc_finder_details_lpdate">' +
                lastPublishedDateParts.date +
                " " +
                lastPublishedDateParts.time +
                "</span></div>";
            }

            var lastModifiedDateParts = $.perc_utils.splitDateTime(
              itemProps.lastModifiedDate,
            );
            ...
            if (spec.path.split("/")[1] === $.perc_paths.DESIGN_ROOT_NO_SLASH) {
              ...
              summary =
                ...
                "Properties" +
                ': <span id="perc_finder_details_name">' +
                itemProps.name +
                "</span></div>" +
                '<div style="padding:10px 0 0 10px;">' +
                "Size" +
                ': <span id="perc_finder_details_size">' +
                fileSize +
                "</span></div>" +
                '<div style="padding:10px 0 0 10px;">Last Modified: <span></span></div>' +
                '<div style="padding:3px 0 0 10px;"><span id="perc_finder_details_lmdate">' +
                lastModifiedDateParts.date +
                " " +
                lastModifiedDateParts.time +
                "</span></div>";
            } else {
              summary =
                '<div style="padding:9px 0 0 10px;">' +
                nameType +
                ': <span id="perc_finder_details_name">' +
                linkTag +
                itemProps.name +
                ...
                '<div style="padding:9px 0 0 10px;">' +
                type +
                ': <span id="perc_finder_details_type">' +
                itemProps.type +
                "</span></div>" +
                '<div style="padding:9px 0 0 10px; width:170px;">Status: <div id="perc_finder_details_status" status="' +
                itemProps.status +
                '" workflow="' +
                itemProps.workflow +
                '" class="perc-ellipsis" title="' +
                itemProps.status +
                " (" +
                itemProps.workflow +
                ')">' +
                ...
                ")" +
                "</div></div>" +
                '<div style="padding:9px 0 0 10px;">Last Modified: <span id="perc_finder_details_lmuser">' +
                itemProps.lastModifier +
                "</span></div>" +
                '<div style="padding:3px 0 0 10px;"><span id="perc_finder_details_lmdate">' +
                lastModifiedDateParts.date +
                " " +
    ...
    function launchAssetPreview(id, revId) {
      ...
      $.PercAssetService.getAssetViewForAssetId(id, function (status, result) {
        if (status == $.PercServiceUtils.STATUS_SUCCESS) {
          var nRef = window.open(
            result,
            "percAssetPreviewWindow" + id.replace(/-/g, ""),
          );
          $(nRef.document).ready(function () {
            if (revId) {
              window.setTimeout(function () {
                nRef.document.title =
                  nRef.document.title +
                  I18N.message("perc.ui.finder@Revision") +
                  revId +
                  ")";
      ...
      $.PercPathService.getPathItemById(
        ...
        function (status, result, errorCode) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            ...
            if (
              typeof mobilePreview === "undefined" ||
              mobilePreview === null
            ) {
              mobilePreview = false;
            }
            ...
            var nRef = window.open(
              href,
              "percPagePreviewWindow" + id.replace(/-/g, ""),
            );
            $(nRef.document).ready(function () {
              if (revId) {
                window.setTimeout(function () {
            ...
            if (errorCode == "cannot.find.item") {
              msg = I18N.message(
                "perc.ui.common.error@Preview Content Deleted",
              );
            } else {
              msg = result;
            }
    ...
    function launchPageCompareView(
      itemId,
      itemName,
      selectedRev,
      latestRev,
    ...
    function launchPagePreviewByPath(path, id, revId) {
      ...
      $.PercPathService.getPathItemForPath(path, function (status, result) {
        if (status == $.PercServiceUtils.STATUS_SUCCESS) {
          ...
          if (revId) {
            href +=
              "?sys_revision=" + revId + "&percmobilepreview=" + mobilePreview;
          } else {
            href += "?percmobilepreview=" + mobilePreview;
          }
    ...
    function load_folder_path(path) {
      return function (k, dir) {
        ...
        if (
          assetPagination &&
          (path.indexOf("Assets") > -1 || path.indexOf("Sites") > -1)
        ) {
        ...
        if (
          !assetPagination &&
          typeof dir.data("child") !== "undefined" &&
          dir.data("child") !== ""
        ) {
    ...
    function fn_first_dir() {
      ...
      fdir
        .find(".ui-resizable")
        .append(
          '<div class="ui-resizable-handle ui-resizable-e perc-resize-disabled" />',
        );
      return fdir;
    }
    ...
    function getSiteNameByPath(path) {
      if (!path || typeof path != "string" || path.length < 1) {
        $.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.publish.title@Error"),
          content: I18N.message("perc.ui.finder@Nonvalid String"),
        });
        return;
      }
    ...
    function getPathItemById(objectId) {
      if (!objectId || typeof objectId != "string" || objectId.length < 1) {
        $.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.publish.title@Error"),
          content: I18N.message("perc.ui.finder@Object ID Nonvalid String"),
        });
        return;
      }
    ...
    function getParentPathItemByPath(path) {
      if (!path || typeof path != "string" || path.length < 1) {
        $.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.publish.title@Error"),
          content: I18N.message("perc.ui.finder@Path Nonvalid String"),
        });
        return;
      }
    ...
    function fireActionEvent(action, data) {
      var len = actionListeners.length;
      for (var i = 0; i < len; i++) {
        actionListeners[i](action, typeof data == "object" ? data : null);
      }
    }
  }
