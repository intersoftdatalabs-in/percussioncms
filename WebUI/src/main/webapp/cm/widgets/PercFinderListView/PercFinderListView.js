...
(function ($) {
  ...
  $.fn.PercFinderListView = function (config, serviceContent) {
    ...
    var configDT = {
      percRowClickCallback: rowClickCallback,
      percRowDblclickCallback: rowDblclickCallback,
    ...
    table.PercFixedTableHeader({
      resizable: $(".perc-finder-body.ui-resizable"),
      remove: $("#perc-finder-choose-columnview"),
      container: container,
    });

    if (typeof config.callback != "undefined") config.callback(self);
  ...
  function createItemsDragAndDrop(table) {
    ...
    $.map(allRows, function (val, i) {
      if (
        $(val).data("percRowData") !== undefined &&
        $(val).data("percRowData").category === "ASSET"
      ) {
  ...
  function getItemFolderPath(rowData) {
    ...
    var folderPath;
    if (rowData.data.type === "site") {
      // if click on a site don't need to include the item name
    ...
    return folderPath
      .replace("Folders/$System$/", "")
      .substring(2, folderPath.length)
      .split("/");
  }
  ...
  function parseContenIntoConfig(displayFormat, serviceData) {
    ...
    for (s = 0; s < serviceData.length; s++) {
      ...
      for (c = 0; c < columns.length; c++) {
        ...
        if (
          ...
          data =
            "<div title='" + dateAndTime + "'>" + dateParts.date + "</div>";
        }
        ...
        if (
          ...
          data =
            '<div title = "' +
            data +
            ' Bytes" style="text-align:right;">' +
            $.perc_utils.formatFileSize(data) +
            "</div>";
        }
          ...
          if (serviceData[s].type == "site") {
            ...
            itemPath =
              itemPathRaw
                ...
                .replace("//Folders/$System$" + $.perc_paths.ASSETS_ROOT, "") +
              "/" +
              serviceData[s].name;
          }
          data =
    ...
    var config = {
      percData: percData,
      percColNames: percColNames,
      percHeaders: percHeaders,
      percTypes: percTypes,
      percWidths: percWidths,
    };
    return config;
  }
