...
(function ($, P) {
  P.contentView = function (root, model) {
    ...
    var deleteMenu = {
      ...
      callback: function (elem) {
        ...
        if (typeof assetInfo != "undefined" && assetInfo.assetShared) {
          msg =
            I18N.message("perc.ui.content.view@Double Check Remove") +
            "\n" +
        ...
        var assetId = assetInfo && assetInfo.locked ? elem.attr("assetid") : "";
        if (assetId) $.perc_utils.confirm_dialog(options);
      },
    };
    var editMenu = {
      name: "edit",
      img: function (elem) {
        ...
        if (model.getWidgetContentTypes(wId) === "")
          ...
          if (
            (model.isTemplate() &&
              model.getWidgetPrefs(wDef).attr("is_editable_on_template") ===
                "false") ||
        ...
        if (
          ...
          !assetInfo.locked &&
          elem.attr("assetid") &&
          model.getWidgetPrefs(wDef).attr("is_editable_on_template") !== "false"
        ) {
          imgSrc += "Middle";
        }
        return imgSrc;
      ...
      callback: function (elem) {
        if (model.getWidgetContentTypes(elem.attr("widgetid")) !== "") {
          ...
          var assetInfo = model.getAssetDropCriteria()[elem.attr("widgetid")];
          var assetId =
            assetInfo && assetInfo.locked ? elem.attr("assetid") : "";
    ...
    function addWidgetAssetDrops(widget) {
      ...
      if (typeof inside.draggable == "function") {
        insideIframe(widget).draggable({
          ...
          drag: function (event, ui) {
            if (event.target.innerText.length <= 0) return false;
          },
        });
        insideIframe(widget).droppable({
          ...
          over: function (evt, ui) {
            ...
            if (
              parentRegionId === currentRegionId ||
              overlapContentWidgets >= 1
            ) {
          ...
          drop: function (event, ui) {
            ...
            widgets.each(function () {
              if (
                $(this).attr("widgetid") ===
                ui.draggable[0].getAttribute("widgetid")
              ) {
                draggedWidget = $(this);
              }
            });
            ...
            if (event.target.innerText.length === 0) {
              if (
                ui.draggable[0].innerText.length !== 0 &&
                event.target.getAttribute("widgetdefid") ===
                  ui.draggable[0].getAttribute("widgetdefid")
              ) {
            ...
            var draggedWidgetAssetInfo =
              model.getAssetDropCriteria()[
                ui.draggable[0].getAttribute("widgetid")
              ];
            var dropOnWidgetAssetInfo =
              model.getAssetDropCriteria()[widget.attr("widgetid")];
            ...
            if (isEmptyDropOnWidget) {
              ...
              model.updateAssetRelationship(
                dropOnWidgetData,
                draggedWidget.attr("assetid"),
                draggedWidgetAssetInfo.relationshipId,
                function () {
                  model.updateAssetRelationship(
                    draggetWidgetData,
                    dropOnWidget.attr("assetid"),
                    dropOnWidgetAssetInfo.relationshipId,
                    function () {
      ...
      widget.droppable({
        ...
        drop: function (evt, ui) {
          ...
          if (!accepts(ui.draggable)) return false;
          ...
          if (typeof spec != "undefined") {
            assetid =
              typeof spec.id != "undefined"
                ? spec.id
                : ui.draggable.data("percRowData").id;
            relationshipId =
              typeof spec.relationshipId != "undefined"
                ? spec.relationshipId
                : "";
          }
          ...
          if (assetid) {
            var assetInfo =
              model.getAssetDropCriteria()[widget.attr("widgetid")];
            var curAsset =
              assetInfo && assetInfo.locked ? widget.attr("assetid") : "";
            var widgetData = {
              widgetid: widgetid,
              widgetdefid: widget.attr("widgetdefid"),
              widgetName: widget.attr("widgetName"),
            ...
            var options = {
              ...
              question:
                ...
                "<br/>" +
                I18N.message("perc.ui.content.view@Continue"),
              type: "YES_NO",
              cancel: function () {},
              success: function () {
                if (relationshipId === "")
    ...
    $(document).ready(function () {
      $("#perc-content-menu a.perc-dropdown-option-DisableJavaScript")
        .off("click")
        .on("click", function () {
