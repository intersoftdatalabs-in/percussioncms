...
(function ($) {
  ...
  var dashboardConfigMetaKey =
    "perc.user." + username + ".dash.page." + currentPage;
  ...
  function removeGadget(instanceId) {
    ...
    $.perc_utils.confirm_dialog({
      ...
      question:
        I18N.message("perc.ui.dashboard@Confirm Deletion Question") +
        $theGadget.attr("name") +
        I18N.message("perc.ui.dashboard@Gadget"),
      success: function () {
        $theGadget.remove();
  ...
  function restore() {
    $.perc_utils.confirm_dialog({
      ...
      question:
        I18N.message("perc.ui.dashboard@Reset Dashboard") +
        "<br /><br />" +
        I18N.message("perc.ui.dashboard@Proceed question"),
      width: 500,
      success: function () {
        $.PercMetadataService.deleteEntry(
          dashboardConfigMetaKey,
          function (status, result) {
            if (status === $.PercServiceUtils.STATUS_SUCCESS) {
              $.PercMetadataService.deleteEntryByPrefix(
                dashboardConfigMetaKey + ".",
                function (status, result) {
                  if (status === $.PercServiceUtils.STATUS_SUCCESS) {
  ...
  function loadGadgetListing(callback) {
    $.PercDashboardService.getTrayGadgets(
      $(".perc-gadget-type").val(),
  ...
  function showSplashDialog() {
    if ($.PercNavigationManager.isAdmin()) {
      $.perc_pathmanager.open_path(
        "/Sites/?startIndex=1&maxResults=1",
        true,
        function (folder_spec) {
          ...
          if (folder_spec.PagedItemList.childrenCount === 0) {
            $.perc_utils.confirm_dialog({
              ...
              showAgainCheck: true,
              question:
                "<img style='height: 449px' src='../images/images/splash-v2.png'>",
              width: 686,
  ...
  function createGadget(jsonGadget) {
    if (!jsonGadget.url.includes("perc_optimizer_keywords")) {
      ...
      getGadgetMeta(
        ...
        [gadgets.container.userPrefStore.getPrefs(gadgetObj)],
        function (data) {
          ...
          gadgetObj.metaKey =
            dashboardConfigMetaKey + ".mid." + gadgetObj.id + ".";
  ...
  function _afterSetTitle(titleid) {
    ...
    $(gadgetId)
      .removeClass("gadgets-gadget-title")
      .addClass("gadgets-gadget-title-change");
  }
  ...
  function showMenu(gadget, titleBar, evt) {
    ...
    var menuX =
      titleBar[0].getBoundingClientRect().left +
      titleBar[0].getBoundingClientRect().width -
  ...
  function minimizeGadget(gadget) {
    ...
    try {
      ...
      var notifyFunc = $("#remote_iframe_" + instanceId)[0].contentWindow
        ._minimizeConfirm;
    ...
    gadget
      .css("min-height", "36px")
      .addClass("perc-gadget-content-minimized")
      .removeClass("perc-gadget-content-expanded");

    gadget.find(".perc-gadget-titlebar-shadow").hide();
    gadget
      .find(".gadgets-gadget-title-bar")
      .css("-moz-border-radius-bottomleft", "7px");
    gadget
      .find(".gadgets-gadget-title-bar")
      .css("-moz-border-radius-bottomright", "7px");

    gadget
      .find(".gadgets-gadget-title-bar")
      .css("-webkit-border-bottom-left-radius", "7px");
    gadget
      .find(".gadgets-gadget-title-bar")
      .css("-webkit-border-bottom-right-radius", "7px");
    saveDashboard();
  }
  ...
  function expandGadget(gadget) {
    ...
    gadget
      .removeAttr("style")
      .removeClass("perc-gadget-content-minimized")
      .addClass("perc-gadget-content-expanded");
  ...
  function handlePrefs(idx) {
    gadgets.container
      .getGadget(idx)
      .handleOpenUserPrefsDialog(function (gadgetId) {
  ...
  function updateStatusOptions(gadgetId) {
    var workflowSelect = $('[name="m_' + gadgetId + '_up_ssworkflow"]');
    $.PercWorkflowService().getStatusByWorkflow(
      workflowSelect.val(),
      function (status, result) {
        if (status === $.PercServiceUtils.STATUS_SUCCESS) {
  ...
  function createGadgetHTML(gadget) {
    ...
    gadgetHTML =
      '<div id="' +
      gid +
      '" class="perc-gadget perc-gadget-content-expanded" name="' +
      gadget.name +
      '" instanceid="' +
      gadget.instanceId +
      '"></div>\n';
  ...
  /**
   * Retrieve gadget meta from the gadget meta service.
   * @param gadgets {array}
   * @param uPrefs {array}
  ...
  function populateTrayGadgets(callback) {
    ...
    for (g = 0; g < trayGadgetsJson.length; g++) {
      ...
      if (
        !trayGadgetJson.adminOnly ||
        (trayGadgetJson.adminOnly && $.PercNavigationManager.isAdmin())
      ) {
  ...
  function addGadgetToTray(trayGadgetJson) {
    ...
    var title =
      trayGadgetJson.name.toUpperCase() + " - " + trayGadgetJson.description;
    ...
    gadgetTrayItem
      ...
      .attr("title", title)
      .attr("url", url)
      .data("gadget", trayGadgetJson)
      .css("display", "inline-block")
      .find(".perc-tray-item-icon")
      .css("background", "url(" + iconUrl + ")");

    gadgetTrayItem
      .find(".perc-tray-item-label div")
      .attr("name", trayGadgetJson.name)
      .html(name);
  ...
  function getDashboardMaxHeight() {
    ...
    dashboardColumns.each(function () {
      ...
      if (dashboardColumn.height() > maxHeight)
  ...
  function addGadgetDropAreas() {
    ...
    dashboardGadgets.each(function () {
      ...
      if (isLastGadget(dashboardGadget)) {
        ...
        var top =
          gadgetDropArea.data("height") +
          gadgetDropArea.data("top") +
          DASHBOARD_GADGET_MARGIN;
        addGadgetDropArea(
          dashboardGadget,
          null,
          rowIndex,
  ...
  function addGadgetDropArea(
    ...
    if (
      (dashboardGadget === null || typeof dashboardGadget === "undefined") &&
      (width === undefined || height === undefined)
    )
      throw (
        "Exception in PercDashboard.addGadgetDropArea(): Unable to calculate Gadget Drop Area size. " +
        "At least a Dashboard Gadget or sizes are needed to add a gadget drop area. " +
    ...
    if (
      ...
      throw (
        "Exception in PercDashboard.addGadgetDropArea(): Unable to calculate Gadget Drop Area location. " +
        "At least a Dashboard Gadget or a location is needed to add a gadget drop area. " +
        "Neither have been provided"
      );
    ...
    if (dashboardGadget !== null && typeof dashboardGadget !== "undefined") {
      var dashboardGadgetPosition = dashboardGadget
        .parent()
        .parent()
        .position();

      var gadgetParentColumnId = dashboardGadget
        .parent(".perc-gadget-column")
        .attr("id");
      ...
      columnIndex =
        columnIndex === undefined ? parseInt(columnIndexString) : columnIndex;
      rowIndex =
        rowIndex === undefined ? dashboardGadget.prevAll().length : rowIndex;
    ...
    gadgetDropArea
      ...
      .width(width)
      .height(height)
      .css("top", top - DASHBOARD_GADGET_MARGIN + 1)
      .css("left", left)
      .addClass("perc-dashboard-gadget-drop")
      .show()
      .droppable({
        tolerance: "pointer",
        ...
        drop: function (event, ui) {
          ...
          if (ui.helper.hasClass("perc-gadget")) {
            ...
            if (
              origPos &&
              origPos.col === columnIndex &&
              origPos.row === rowIndex
            )
              return false;
  ...
  function isFirstGadget(dashboardGadget) {
    return (
      dashboardGadget.prevAll(".perc-gadget[perc-helper!=true]").length === 0
    );
  }
  ...
  function isLastGadget(dashboardGadget) {
    return (
      dashboardGadget.nextAll(".perc-gadget[perc-helper!=true]").length === 0
    );
  }
  ...
  function expandGadgetTray() {
    gadgetTrayExpander
      .removeClass("perc-tray-expander-collapsed")
      .addClass("perc-tray-expander-expanded");
  ...
  function collapseGadgetTray() {
    gadgetTrayExpander
      .removeClass("perc-tray-expander-expanded")
      .addClass("perc-tray-expander-collapsed");
