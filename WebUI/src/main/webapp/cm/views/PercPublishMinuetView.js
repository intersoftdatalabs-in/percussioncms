...
function getAllPublishingServer(serverType, serverObj) {
  ...
  publishingServerListDeferred.done(function (pslLst) {
    ...
    if (
      typeof serverObj !== "undefined" &&
      typeof serverObj.serverInfo !== "undefined"
    ) {
      ...
      if (
        typeof selectedserverProp !== "undefined" &&
        selectedserverProp !== null &&
        typeof selectedserverProp.value !== "undefined"
      ) {
  ...
  $.when(processRunningAnimationDeferred).then(function () {
    ...
    if (result[1] == "success") {
      response.result.status =
        selectedServerData.serverInfo.serverName +
        " " +
        I18N.message("perc.ui.publish.title@Successfully Removed");
...
function updateServerPropertiesCallback(status, result) {
  ...
  $.when(processRunningAnimationDeferred).then(function () {
    ...
    if (result[1] == "success") {
      ...
      response.result.status =
        result[0].serverInfo.serverName +
        " " +
...
function bindServerPropertiesEvents() {
  ...
  $("#fileDriver").on("change", function (evt) {
    if (
      evt.currentTarget.options.item(evt.currentTarget.options.selectedIndex)
        .text === "SFTP"
    ) {
      // Set the SFTP fields.
      //if neither password or key is checked, trigger the key as it is preferred.
      if (
        !$("#SFTPprivateKeyFlag").is(":checked") &&
        !$("#SFTPpasswordFlag").is(":checked")
      ) {
...
function updateDriverPropertiesUi() {
  ...
  $("#percServerPropertiesFormTarget > form > section").each(function () {
    ...
    desiredSection =
      "percServerProperties" + selectedType + selectedDriver + "Target";

    // Always show database common properties if type is database, hide if type is file
    if (
      selectedType == "Database" &&
      currentSection == "percServerPropertiesDatabaseCommonTarget"
    ) {
      $(this).show("fast");
      return true;
    } else if (
    ...
    if (
      currentSection == "percServerPropertiesCommonTarget" ||
      currentSection == "percServerPropertiesOptionalTarget"
    ) {
      $(this).show("fast");
      return true;
    }
    ...
    if (
      selectedType == "Database" &&
      (selectedDriver == "MSSQL" || selectedDriver == "MYSQL")
    ) {
    ...
    if (
      selectedType == "File" &&
      currentSection == "percServerPropertiesFileLocalTarget" &&
      selectedDriver != "AMAZONS3"
    ) {
      $(this).show("fast");
      return true;
    } else if (
...
function filterSitesByString(siteFilterString) {
  ...
  $("#sitesCardView .perc-site-card").filter(function () {
    $(this)
      .parent(".perc-site-card-container")
      .toggle($(this).text().toLowerCase().indexOf(siteFilterString) > -1);
  });
}
...
function processPublish(eventData) {
  ...
  serverName = $(eventData)
    .closest(".perc-publish-button-group")
    .data("perc-server-name");
...
function loadIncrementalPage(siteName, serverName, startIndex) {
  $.PercPublisherService(false).getIncrementalItems(
    siteName,
    serverName,
    startIndex,
    PAGE_SIZE,
    function (status, result) {
      status == "error" || result.PagedItemList.childrenInPage.length == 0
        ? (incrementalPreviewObject = {})
        : (incrementalPreviewObject = result);
      ...
      if (
        ...
        serverType.toLowerCase() == "production" &&
        result.PagedItemList.childrenInPage.length > 0
      ) {
...
function createPaginatedTable(data, startIndex) {
  if (data.PagedItemList && data.PagedItemList.childrenCount > 0) {
    ...
    var first = pageSpan
      .clone()
      .text("First")
      .addClass("paginate_button first");
    first[0].setAttribute("startIndex", 1);
    var sind =
      (activePage - 2) * PAGE_SIZE + 1 < 1
        ? 1
        : (activePage - 2) * PAGE_SIZE + 1;
    var prev = pageSpan
      .clone()
      .text("Prev")
    ...
    sind =
      activePage * PAGE_SIZE + 1 > totalPages * PAGE_SIZE
    ...
    for (i = 1; i <= 4 && pages.length < 5; i++) {
      ...
      if (activePage + i <= totalPages) pages.push(activePage + i);
    }

    var pagedItems = $("<span/>");
    for (i = 0; i < pages.length; i++) {
      var clName =
        activePage === pages[i] ? "paginate_button current" : "paginate_button";
...
function processIncrementalRelatedItemsPreview() {
  $.PercPublisherService(false).getIncrementalRelatedItems(
    siteName,
    serverName,
    1,
    3,
    function (status, result) {
      status == "error" || result.PagedItemList.childrenInPage.length == 0
        ...
        : (incrementalRelatedPreviewObject = result.PagedItemList);
...
function processIncrementalPublish() {
  ...
  var itemsToApprove = [];
  $("#percIncrementalPublishRelatedItemsList tr input[type=checkbox]").each(
    function () {
...
function updateServerProperties(siteName, serverName, serverProperties) {
  startProcessRunningAlert();
  $.PercPublisherService(false).createUpdateSiteServer(
    siteName,
    serverName,
    serverProperties,
...
function processServerPropertiesForm(eventData) {
  ...
  var newserverName = serverForm
    .find('input[percServerProp="serverName"]')
    .val();
  var isDefault = serverForm
    .find('input[percServerProp="isDefault"]')
    .is(":checked");
  var publishType = serverForm.find("select#publishType").val();
  var ignoreUnModifiedAssets = serverForm
    .find('input[percServerProp="ignoreUnModifiedAssets"]')
    .is(":checked");
  var publishRelatedItems = serverForm
    .find('input[percServerProp="publishRelatedItems"]')
    .is(":checked");
  var publishSecureSiteConfigOnExactPath = serverForm
    .find('input[percServerProp="publishSecureSiteConfigOnExactPath"]')
    .is(":checked");
