...
(function ($) {
  ...
  $.PercNewAssetDialog.init = function (finder, contentViewer) {
    ...
    $.PercNewAssetDialog.open = function () {
      ...
      dialog = $(
        "<div>" +
          ...
          I18N.message("perc.ui.newassetdialog.label@Select Asset Type") +
          ": </label>" +
        ...
        percButtons: {
          Next: {
            click: function () {
              var contentId =
                $.PercNavigationManager.getMemento().lastOpenedItem;
          ...
          Cancel: {
            click: function () {
              _remove();
              if (
                typeof $.PercNavigationManager.getMemento().lastLocation !=
                "undefined"
              ) {
        ...
        submitHandler: function (form) {
          ...
          $.perc_pathmanager.open_containing_folder(
            ...
            function (fspec, pathArray) {
              ...
              // get the folder id
              $.PercAssetController.getPathItemForPath(
                pathtemp,
                function (pathItemObj) {
      ...
      $.PercAssetController.getAssetEditorLibrary(
        getCurrentFolderPath(),
        function (assetEditorLibrary) {
    ...
    $.PercNewAssetDialog.openViewer = function (folderPath, widgetId) {
      $.PercAssetService.getAssetEditorForWidgetAndFolder(
        folderPath,
        widgetId,
    ...
    function check_for_dirty_page(event) {
      $.PercFolderHelper().getAccessLevelByPath(
        ...
        function (status, result) {
          if (
            status === $.PercFolderHelper().PERMISSION_ERROR ||
            result === $.PercFolderHelper().PERMISSION_READ
          ) {
            $.perc_utils.alert_dialog({
            ...
            $.PercUserService.getAccessLevel(
              ...
              function (status, result) {
                if (
                  status === $.PercServiceUtils.STATUS_ERROR ||
                  result === $.PercUserService.ACCESS_READ ||
                  result === $.PercUserService.ACCESS_NONE
                ) {
                  $.perc_utils.alert_dialog({
    ...
    function createAssetEditorEntry(assetEditor) {
      var temp =
        "<button type='button' class='item' id='@ITEM_ID@'>" +
      ...
      ("</button>");
      return temp
        .replace(/@IMG_SRC@/, assetEditor.icon)
    ...
    function updateBtn(path) {
      ...
      if (
        $.Percussion.getCurrentFinderView() ===
          $.Percussion.PERC_FINDER_SEARCH_RESULTS ||
        ...
        if (
          path[1] === $.perc_paths.DESIGN_ROOT_NO_SLASH ||
          path[1] === $.perc_paths.SITES_ROOT_NO_SLASH ||
          path[1] === $.perc_paths.RECYCLING_ROOT_NO_SLASH
        ) {
          newAssetButton.hide();
        }
      }
