...
(function ($, P) {
  $.PercWorkflowView = function () {
    ...
    var WORKFLOW_ASSIGNMENT_INPROCESS_MESSAGE =
      "Your changes were unable to be saved. The system is currently assigning pages and assets to workflow. During this process access to workflow administration is prevented to avoid conflicting changes.  You can check the status of the assignment process using the Process Monitor on the Dashboard.";
    /**
    ...
    function showNewWorkflowEditor() {
      ...
      $("#perc-new-workflow-name")
        .val("")
        .on("focus change", function (evt) {
    ...
    function deleteWorkflow(workflowName) {
      ...
      if (typeof previousWorkflowName == "undefined")
        previousWorkflowName = $(".perc-itemname[title='" + workflowName + "']")
          .next()
          .attr("title");
      ...
      var settings = {
        ...
        type: "YES_NO",
        question:
          I18N.message("perc.ui.workflow.view@About To Delete Workflow") +
          workflowName +
          "'<br /><br />" +
          I18N.message("perc.ui.workflow.view@Are You Sure Delete Workflow"),
        success: function () {
          $.PercBlockUI();
          $.PercWorkflowService().deleteWorkflow(
            workflowName,
    ...
    function expandWrokflowList(workflowName) {
      var isWfHidden = container
        .find(".perc-itemname[title='" + workflowName + "']")
        .is(".perc-hidden");
    ...
    function selectWorflow(workflowName) {
      container
        .find(".perc-itemname[title='" + defaultWorkflow + "']")
        .addClass("perc-default-wf-list-marker");
      $(".perc-default-wf-marker").hide();
      var selectingSameWorkflow =
        workflowName ==
        container.find(".perc-itemname.perc-item-selected").attr("title");
      ...
      $("#perc-wf-edit")
        .removeClass("perc-wf-edit-disable")
        .off("click")
    ...
    function saveNewWorkflow() {
      ...
      function createWorkflowServiceCall() {
        $.PercWorkflowService().createWorkflow(
          workflowName,
          workflowObj,
          function (status, result) {
            if (status) {
    ...
    function updateWorkflow() {
      ...
      function updateWorkflowServiceCall() {
        $.PercWorkflowService().updateWorkflow(
          previousWorkflowName,
          workflowObj,
          function (status, result) {
            if (status) {
    ...
    function attachWorkflowEditorEvents() {
      ...
      //Bind Cancel event
      $("#perc-wf-cancel, #perc-wf-update-cancel")
        .off("click")
        .on("click", function (evt) {
          cancel();
        });
    ...
    function getWorkflowList(callback) {
      ...
      $.PercWorkflowService().getWorkflows(function (status, result) {
        if (status) {
          ...
          } else workflows = result[0].Workflow;
          var workflowArray = [];
    ...
    function updateSitesFolderAssignedSection(workflowName) {
      $(".perc-sa-loading-warning-message-hidden")
        .addClass("perc-sa-loading-warning-message")
        .removeClass("perc-sa-loading-warning-message-hidden");
    ...
    function showFolderTrees() {
      if (originalSitesJson != null && originalAssetsJson != null) {
        $(".perc-sa-loading-warning-message")
          .addClass("perc-sa-loading-warning-message-hidden")
          .removeClass("perc-sa-loading-warning-message");
    ...
    function setTabIndexForAdminWF() {
      ...
      var siteFound = false;
      $("#perc-workflows-assigned-sites-folders")
        .find("*")
        .each(function () {
