...
(function ($, P) {
  ...
  function init(workflowName) {
    ...
    $("#perc-workflow-steps-container table")
      .addClass("perc-workflow-steps-rendering")
      .removeClass("perc-workflow-steps-ready");
    __loadWorkflowObject(settings.workflowName, function (workflowObject) {
      __renderWorkflowSteps(workflowObject, function () {
        __attachEvents();
        $("#perc-workflow-steps-container table")
          .removeClass("perc-workflow-steps-rendering")
          .addClass("perc-workflow-steps-ready");
      });
    });
  }
  ...
  function __loadWorkflowObject(workflowName, callback) {
    $.PercWorkflowService().getWorkflowObject(
      workflowName,
  ...
  function __renderWorkflowSteps(workflowObject, callback) {
    ...
    for (var i = 0; i < workflowObject.Workflow.workflowSteps.length; i++) {
      ...
      for (var j = 0; j < stepRoles.length; j++) {
        ...
        $(percStepRolesItems)
          .html(workflowStepRole)
          .attr("title", workflowStepRole);
      ...
      if (workflowStepName != "Archive") {
        $(percStepRow)
          .find("#perc-workflow-column-wrapper")
          .append(percStepColumn, percStepColumnSpacer);
      } else {
        $(percArchiveStepRow)
          .find("td")
    ...
    $("#perc-workflow-archive")
      .parent()
      .prepend("<div id = 'perc-archive-action-text'></div>");
    callback();
  }
   ...
   */
  function __renderWorkflowActionText(
    workflowName,
    stepName,
    percWorkflowActionText,
    percStepColumnSpacer,
    ...
    switch (stepName) {
      ...
      default:
        ...
        $(percStepColumn)
          .find(".perc-reserved-step-config-bttn")
          .data({ isReservedStep: false });
    }
    // Attach the workflowStep object to each config button.
    $(percStepColumn)
      .find(".perc-step-config-button, .perc-reserved-step-config-bttn")
      .data({
    ...
    $(percStepColumn)
      .find(".perc-step-delete-button, .perc-reserved-step-config-bttn")
      .data({
  ...
  function __createNewStep(elem) {
    ...
    $.PercEditWorkflowStepDialog().open(
      isReservedStep,
      isUpdate,
      workflowStep,
      workflowName,
  ...
  function __deleteWorkflowStep(elem) {
    ...
    var settings = {
      ...
      question:
        I18N.message("perc.ui.workflow.steps.view@About To Delete") +
        stepName +
        "'<br /><br />" +
        I18N.message("perc.ui.workflow.steps.view@Are You Sure Delete Step"),
      success: function () {
        $.PercBlockUI();
        $.PercWorkflowService().deleteWorkflowStep(
          workflowName,
          stepName,
  ...
  function __calculateRowWidth() {
    ...
    var blockWidth =
      $(".perc-workflow-step").width() +
      $(".perc-workflow-step-spacer").width();
  ...
  function __activateMoreLessLink(elem) {
    $(elem)
      .parent()
      .find(".perc-more-list, .perc-moreLink, .perc-lessLink")
      .toggle()
      .toggleClass("perc-hidden perc-visible");
  }
