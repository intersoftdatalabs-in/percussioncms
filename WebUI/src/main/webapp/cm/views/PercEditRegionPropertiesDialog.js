  ...
  $.PercEditRegionPropertiesDialog = function () {
    ...
    function editRegionProperties(region) {
      ...
      _layoutModel.editRegion(region.attr("id"), function () {
        ...
        this.noAutoResize =
          this.noAutoResize === undefined ? false : this.noAutoResize;
      ...
      function createDialog() {
        ...
        if (attributes) {
          for (i = 0; i < attributes.length; i++) {
            percAttributeData.push({
              rowContent: [attributes[i].name, attributes[i].value],
            });
          }
    ...
    function addRegionPropertiesFieldGroups() {
      ...
      if (dialog.find(".perc-section-header").length > 0) return;

      var fieldGroups = [
        {
          groupName: "perc-region-edit-properties-container",
          groupLabel: "Region based CSS",
        },
        {
          groupName: "perc-region-edit-attributes-container",
          groupLabel: "Region HTML attributes",
        },
        {
          groupName: "perc-region-edit-css-container",
          groupLabel: "Use additional CSS classes",
        },
        {
          groupName: "perc-region-edit-cssoverrides-container",
          groupLabel: "CSS overrides",
        },
      ];
      $.each(fieldGroups, function (index) {
        ...
        var groupHtml =
          ...
          this.groupName +
          "'>" +
          "<span role='button' tabindex='0' title='" +
          minmaxTitle +
          "' class='perc-min-max " +
          minmaxClass +
          "' ></span>" +
        ...
        .on("click", function () {
          ...
          if (className) {
            if (className == "perc-min-max perc-items-maximizer") {
              self
                .find(".perc-min-max")
                .attr("title", I18N.message("perc.ui.workflow.view@maximizer"));
    ...
    function saveRegionEditProperties(region, newRegionId) {
      _layoutModel.isResponsiveBaseTemplate()
        ? _saveResponsiveRegionEditProperties(region, newRegionId)
        : _saveBaseRegionEditProperties(region, newRegionId);
    }
    ...
    function _saveBaseRegionEditProperties(region, newRegionId) {
      ...
      if (
        regionWidth ===
        I18N.message("perc.ui.edit.region.properties.dialog@Enter Width")
      )
        regionWidth = "";
      if (
        regionHeight ===
        I18N.message("perc.ui.edit.region.properties.dialog@Enter Height")
      )
        regionHeight = "";
      if (
        regionPadding ===
        I18N.message("perc.ui.edit.region.properties.dialog@Enter Padding")
      )
        regionPadding = "";
      if (
        regionMargin ===
        I18N.message("perc.ui.edit.region.properties.dialog@Enter Margin")
      )
        regionMargin = "";
      ...
      noAutoResize =
        noAutoResize === undefined || noAutoResize === ""
          ? false
          : noAutoResize;
    ...
    function saveRegionProperties(region) {
      ...
      for (i = 0; i < dataTableAttributes.length; i++) {
        ...
        if (
          ...
          invalidAttributeMessage +=
            invalidAttributeMessage.length === 0
              ? name + " "
              : "," + name + " ";
        }
      }
      if (invalidAttributeMessage) {
        var message1 =
          "Invalid attributes found, Cannot use reserved attribute(s) : " +
          invalidAttributeMessage;
        $.perc_utils.alert_dialog({
