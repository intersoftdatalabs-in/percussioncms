...
(function ($, P) {
  ...
  $.PercRegionCSSHandler = {
    ...
    setTemplateAndThemeNamesAndTemplateIdIfEmpty:
      setTemplateAndThemeNamesAndTemplateIdIfEmpty,
  };
  ...
  function init(layoutFunctions, model, iframe, region, dlgCallback) {
    ...
    getRegionCSS(_region.attr("id"), function (status, data) {
      ...
      if (status === $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        if (data.RegionCSS.properties != null) {
          ...
          for (var i = 0; i < regionCSSProperties.length; i++) {
            row = {
              rowContent: [
                regionCSSProperties[i].name,
                regionCSSProperties[i].value,
              ],
            };
            percData.push(row);
          }
  ...
  function saveRegionCSS(id) {
    ...
    if (data.length === 1 && data[0][0].trim() === "") data.pop();
    if (data.length > 0) {
      for (var i = 0; i < data.length; i++) {
        properties.push({
          name: $(data[i][0]).text().trim(),
          value: $(data[i][1]).text().trim(),
        });
      }
      ...
      $.PercTemplateService().saveRegionCSS(
        ...
        function (status, data) {
          if (status == $.PercServiceUtils.STATUS_SUCCESS) {
            tempDeletedRegions = jQuery.grep(
              tempDeletedRegions,
              function (value) {
                return value != id;
              },
  ...
  function reloadRegionCSSFile() {
    var elem = _iframe
      .contents()
      .find("link[href*='" + _themeName + "/perc/perc_region.css']");
    if (elem.length > 0) {
      var date = new Date();
  ...
  function mergeRegionCSS() {
    ...
    if (deletedRegions != null && deletedRegions.length > 0) {
      ...
      deletedRegionsJSON = {
        RegionCssList: { regions: deletedRegionsObjects },
      };
    }
