...
- `hot-deploy-local.py` — local hot-deploy helper for the CMS (jar modules + webui). Test: `test_hot_deploy_local.py`. H2 QA SPA copy (entry + hashed chunks, not host-install) is `docker/scripts/hot-deploy-webui-modern.py` / `perc-devctl.py qa-deploy-webui` (#3893 / #3948 / #4123; refuses bundles missing quoted `object-storage`, `rss-atom`, and `developer-am-new`).
