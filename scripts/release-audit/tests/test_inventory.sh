...
# SC-001: 100% inventoried
COUNT="$(jq 'length' "${INVENTORY}")"
[[ "${COUNT}" -eq 141 ]] || fail "expected 141 inventory entries, got ${COUNT}"
...
# Excluded file: all entries are dependabot
EX_COUNT="$(jq 'length' "${EXCLUDED}")"
[[ "${EX_COUNT}" -eq 229 ]] || fail "expected 229 dependabot entries in excluded file, got ${EX_COUNT}"

DEP_IN_EX="$(jq '[.[] | select(.author | test("dependabot"; "i"))] | length' "${EXCLUDED}")"
[[ "${DEP_IN_EX}" -eq 229 ]] || fail "expected all 229 excluded entries to be dependabot, got ${DEP_IN_EX}"

echo "PASS: test_inventory.sh — inventory 141, excluded 229, 0 dependabot leakage"