...
public class PSSimplePropertySetter implements IPSPropertySetter {
  ...
  public boolean applyProperties(Object obj, ObjectState state, List<IPSAssociationSet> aSets) {
    ...
    try {
      ...
      for (Map.Entry<String, Object> prop : properties.entrySet()) {
        if (applyProperty(obj, state, aSets, prop.getKey(), prop.getValue())) changed = true;
      }
      return changed;
    } catch (Exception e) {
      String errorMsg =
          "Failed to apply properties for object with type of \"" + obj.getClass().getName() + "\"";
  ...
  public boolean deApplyProperties(Object obj, List<IPSAssociationSet> aSets) {
    ...
    try {
      boolean changed = false;
      for (Map.Entry<String, Object> prop : properties.entrySet()) {
        if (deApplyProperty(obj, aSets, prop.getKey(), prop.getValue())) changed = true;
      }
      return changed;
    } catch (Exception e) {
      String errorMsg =
          "Failed to de-apply properties for object with type of \""
   ...
   *
   * @param obj the object, not <code>null</code>. Assumed the state of the object is {@link
   *     ObjectState#PREVIOUS}.
   * @param aSets the list of association sets, may be <code>null</code> if there is no association
   *     to be set on the design object.
   * @param propName the property name, may not be <code>null</code> or empty.
   ...
   *
   * <p><b>Note</b>, if the property name is "innerObject.innerObject2.stringValue", then the
   * property value will set to the most inner object, which is <code>
   * obj.getInnerObject().getInnerObject2().setStringValue(value)</code>; otherwise if the property
   * name is "stringValue", then the property value is simply set on the given object, which is
   * <code>obj.setStringValue(value)</code>.
   *
   * @param obj the object, not <code>null</code>.
   * @param state the state of the specified design object, not <code>null</code>.
   * @param aSets the list of association sets, may be <code>null</code> if there is no association
   *     to be set on the design object.
   * @param propName the property name, may not be <code>null</code> or empty.
   * @param propValue the new value of the property.
  ...
  /**
   * Creates the property definitions (as name/value pairs) for the specified property of the given
   * object. The created property definitions will be added to the specified map.
   *
   * @param obj the object in question, it may not be <code>null</code>.
   * @param propName the name of the property, it may not be blank.
   * @param pvalue the value of the property in its original format, which may contain
   *     ${place-holders} or <code>null</code>.
   * @param defs the holder for created property definitions, never <code>null</code>.
   * @return <code>false</code> if cannot handle the specified parameters; otherwise return <code>
   *     true</code>.
   */
  protected boolean addPropertyDefs(
    ...
    if (!(pvalue instanceof String)) return false;

    PSPair<List<String>, Boolean> refNames;
    refNames = getReferenceNames(pvalue);
    if (refNames == null) return true;
   ...
   * @return the reference names. It may be <code>null</code> if there is no reference name. The 1st
   *     value is the reference names; the 2nd value is <code>true</code> if there is only one
   *     ${place-holder} in the specified string.
   */
  protected PSPair<List<String>, Boolean> getReferenceNames(Object pvalue) {
    if (!(pvalue instanceof String)) return null;
   ...
   *
   * <p><b>Note</b>, if the property name is "innerObject.innerObject2.stringValue", then the
   * returned property value is from the most inner object, which is <code>
   * obj.getInnerObject().getInnerObject2().getStringValue()</code>; otherwise if the property name
   * is "stringValue", then the returned property value is from the given object, which is <code>
   * obj.getStringValue()</code>.
   *
   * <p>The derived class may override this method, for example, it needs to convert a IPSGuid to
   * name, where the actual property is a IPSGuid, but the value of the property in the configure
   * file is a name.
   *
  ...
  protected Object getPropertyValue(Object obj, String propName) throws PSNotFoundException {
    ...
    try {
      PSPair<Object, Method> objMethod = findSetOrGetMethod(obj, propName, false);
      Method method = objMethod.getSecond();

      return method.invoke(objMethod.getFirst(), new Object[] {});
    } catch (Exception e) {
      throw new PSConfigException(
          "Failed to get \"" + propName + "\" property for object \"" + obj.toString() + "\"", e);
    }
  }

  /**
   * Creates "fix-me" property definitions for any unknown elements of the specified list property.
   * The created elements are added into the given map holder. An unknown element is a string
   * contains one or more ${place-holders}.
   *
   * <p>Note, it only scans the element if the type of the element is {@link String}, {@link
   * PSPair}, {@link List} or {@link Map}.
   *
   * @param propName the name of the property, never blank.
   * @param pvalue the list property value. It is expected to be an instance of a {@link List}. It
  ...
  protected void addFixmePropertyDefsForList(
    ...
    if (!(pvalue instanceof List))
      throw new PSConfigException(
          "The \"pvalue\" type (of property \"" + propName + "\") must be List.");
  ...
  /**
   * Adds property definitions into the specified holder for the specified property value if it
   * contains one or more ${place-holder}s. The value of the property definition is the specified
   * reference value if the given reference value is not <code>null</code> and the string in
   * question contains only one ${place-holder}; otherwise the value of the property definition is
   * {@link #FIX_ME}.
   *
   * @param pvalue the string in question, it may be <code>null</code> or empty.
   * @param refValue the reference value, which may be used as the value of the property definition
   *     if the string in question contains only one ${place-holder}. This will not be used if it is
   *     <code>null</code>.
   * @param defs the holder for the created property definitions, assumed not <code>null</code>.
   */
  protected void addPropertyDefs(String pvalue, Object refValue, Map<String, Object> defs) {
    ...
    if (pair != null) {
      if (pair.getSecond()) {
        ...
        for (String n : pair.getFirst()) defs.put(n, FIX_ME);
      }
    }
  }

  /**
   * Creates property definitions for any "unknown" entry of the specified map property. The created
   * property definitions are added into a given holder An unknown element is a string contains one
   * or more ${place-holders}. For each entry of the map property, if the value contains one
   * ${place-holder} and the entry also exists in the supplied source map, then the referenced value
   * is taken from the source map; otherwise the referenced value is {@link #FIX_ME}.
   *
   * <p>Note, it only scans the entries of the map if the type of the map value is {@link String},
   * {@link List} or {@link Map}.
   *
   * @param propName the name of the property, never blank.
   * @param pvalue the property in question. It is expected to be an instance of {@link Map}. It may
   *     be <code>null</code>.
   * @param srcMap the source contains contains entries which may be referenced by the property
  ...
  /**
   * Converts the given value from string to the given type of the parameter of the supplied method.
   *
   * @param method the method that has one parameter, assumed not <code>null</code>.
   ...
   *
   * @param obj the object that may contain the setter method, assumed not <code>null</code>.
   * @param propertyName the name of the property, assumed not <code>null</code> or empty.
   * @param isSetter <code>true</code> if lookup for a setter method; otherwise lookup for a getter
   *     method.
  ...
  private PSPair<Object, Method> findSetOrGetMethod(
    ...
    if (m != null) return new PSPair(obj, m);

    throw new PSConfigException(
        "Cannot find a setter method for property, \'"
            + propertyName
            + "' on type \""
            + obj.getClass().getName()
            + "\".");
   ...
   * @param methodName the method name, assumed not <code>null</code> or empty.
  ...
  /**
   * Find the most inner object and the related property name if there is any. For example, for
   * property name "innerObject.innerObject2.stringValue", the most inner object is <code>
   * getInnerObject().getInnerObject2()</code>, which is the returned object (1st object), and the
   * "stringValue" is the returned property name (2nd object).
   *
   * @param origObj the original object, assumed not <code>null</code>.
   * @param origName the original property name, assumed not <code>null</code> or empty. This may
   *     delimited by ".".
  ...
  private PSPair<Object, String> findObjectPropName(Object origObj, String origName) {
    if (origName.indexOf('.') == -1) return new PSPair(origObj, origName);
  ...
  /**
   * Gets the child object for a given parent object. The child object is the specified property of
   * the parent object.
   *
      ...
      if (m.getName().equals(methodName)) {
        if (m.getParameterTypes().length == 0) {
          try {
            ...
            if (innerObj != null) return innerObj;
   ...
   *
   * @param aSets the to be applied association list, never <code>null</code> or empty.
  ...
  protected void setListAssociation(
    ...
    if (!(value instanceof List))
      throw new PSConfigException(
          "The value type of the " + type.name() + " association must be List.");
  ...
  public boolean equals(Object otherObj) {
    if (!(otherObj instanceof PSSimplePropertySetter)) return false;
    PSSimplePropertySetter other = (PSSimplePropertySetter) otherObj;

    return new EqualsBuilder()
        .append(m_curProps, other.m_curProps)
        .append(m_prevProps, other.m_prevProps)
        .isEquals();
  }

  @Override
  public int hashCode() {
    return new HashCodeBuilder().append(m_curProps).append(m_prevProps).toHashCode();
  }
  ...
  public List<PSConfigValidation> validate(
      @SuppressWarnings("unused") String objName,
      @SuppressWarnings("unused") ObjectState state,
      @SuppressWarnings("unused") IPSPropertySetter setter) {
  ...
  /**
   * This is used when creating a property definition in a default configure definition, where a
   * custom value is needed after the file is created.
   */
