...
/**
 * This class is used to map the values of the setter properties defined in an implementer's
 * configuration file (in spring bean format).
 *
 * @author bjoginipally
 * @author YuBingChen
 */
public class PSConfigMapper {
   ...
   * in the handlers are in the place-holder format, e.g. "${com.percussion.RSS.servername}".
   *
   * @param configDefFile the path of the configuration definition file in spring bean format. Must
   *     not be <code>null</code> or empty.
   * @param props the properties can be applied to the handlers specified in the <code>configDefFile
   *     </code> file. This is a list of properties that need to apply to the design object right
   *     after installed or reinstalled a package; or this may be a list of properties that have
   *     been modified since previously applied the properties to the design objects (which are
   *     specified by the <code>configDefFile</code>).
   * @param curProps the current properties, the combined default and local configure properties.
   ...
   */
  public List<IPSConfigHandler> getResolvedHandlers(
      String configDefFile,
  ...
  /**
   * Validates the parameters for {@link #getResolvedHandlers(String, Map, Map, Map)}. Output debug
   * messages for the given parameters if the debug log is on.
   *
   * @param configDefFile the path of the configuration definition file in spring bean format. Must
   *     not be <code>null</code> or empty.
   * @param props the properties can be applied to the handlers specified in the <code>configDefFile
  ...
  private void validateConfigProperties(
    ...
    if (curProps == null) throw new IllegalArgumentException("curProps may not be null.");
    if (preProps == null) throw new IllegalArgumentException("preProps may not be null.");

    if (!ms_log.isDebugEnabled()) return;

    ms_log.debug("Config Def file: \"" + configDefFile + "\".");
    if (props != curProps) debugProperties("Delta properties", props);
    else ms_log.debug("Delta properties is the same as current properties.");
   ...
   *
   * @param propName the name of the properties, assumed not <code>null</code> or empty.
   * @param props the properties, assumed not <code>null</code>, but may be empty.
   */
  private void debugProperties(String propName, Map<String, Object> props) {
    if (props.isEmpty()) ms_log.debug("\"" + propName + "\" properties is empty.");
    else ms_log.debug("\"" + propName + "\" properties are:");
  ...
  /**
   * Replaces the values of the setter property defined in the specified handler.
   *
   * @param handler the handler, assumed not <code>null</code>.
   * @param props the source properties. It is either the delta of current and previous properties,
   *     or the combined of (current) default and local configure properties. Never <code>null
   *     </code>, may be empty.
   * @param curProps the current properties, the combined default and local configure properties.
   *     Never <code>null</code>, may be empty.
   * @param preProps the previous applied properties, the combined default and local configure
   *     properties that were applied in the last configuration. Never <code>null</code>, may be
   *     empty.
   * @return <code>true</code> replaced some of the handler or setter properties; otherwise return
   *     <code>false</code>.
   */
  private boolean replaceProperties(
      IPSConfigHandler handler,
      Map<String, Object> props,
      Map<String, Object> curProps,
      Map<String, Object> preProps) {
    ...
    if (isDelta) isReplaced = replaceHandlerProperties(handler, props, true, true);
    ...
    for (IPSPropertySetter setter : handler.getPropertySetters()) {
      ms_log.debug(
          "Resolve property values for setter class: \"" + setter.getClass().getName() + "\".");

      // if process a delta & replaced handler properties,
      // then ignore delta for resolving setter properties.
      if (isDelta && isReplaced) props = curProps;

      if (replacePropertyValues(setter, props, curProps, preProps)) isReplaced = true;
    }
  ...
  /**
   * Replace properties for the given handler. Do nothing if <code>allProps</code> is empty.
   *
   * @param handler the handler that contains the properties, assumed not <code>null</code>.
   * @param srcProps the source properties. It is either the delta of current and previous
   *     properties, or the combined of default and local configure properties. Never <code>null
   *     </code>, may be empty. Do nothing if this is empty.
   * @param isCurProps <code>true</code> if <code>allProps</code> is current properties; otherwise
   *     <code>allProps</code> is previously applied properties.
   * @param isDelta <code>true</code> if <code>srcProps</code> is the delta of current and previous
   *     properties. In this case, don't log warning if failed to lookup a property; otherwise log
   *     warning.
   * @return <code>true</code> if one or more values of the handler properties have been replaced.
   */
  private boolean replaceHandlerProperties(
      IPSConfigHandler handler, Map<String, Object> srcProps, boolean isCurProps, boolean isDelta) {
    ...
    if (!isCurProps) tgtProps.putAll(handler.getExtraProperties());
    ...
    if (isReplaced) {
      if (isCurProps) handler.setExtraProperties(tgtProps);
      else handler.setPrevExtraProperties(tgtProps);
    }
   ...
   * @param props the properties that may contain only the intersection of <code>curProps</code> or
   *     <code>preProps</code>, assumed not <code>null</code>, but may be empty.
   * @param curProps the current properties, the combined default and local configure properties.
   *     Never <code>null</code>, may be empty.
   * @param preProps the previous applied properties, the combined default and local configure
   *     properties that were applied in the last configuration. Never <code>null</code>, may be
   *     empty.
   * @return <code>true</code> if replaced some of the property values defined in the setter;
   *     otherwise return <code>false</code>.
   */
  private boolean replacePropertyValues(
    ...
    if (props != curProps) {
      ...
      repCurProps = replacePropertyValues(setter.getProperties(), props, true);
      if (repCurProps == null) {
    ...
    repCurProps = replacePropertyValues(setter.getProperties(), curProps, false);

    // now, use the "preProps" to get "PrevProperties"
    if (!preProps.isEmpty()) {
      Map<String, Object> repPrevProps =
          replacePropertyValues(setter.getProperties(), preProps, false);
   ...
   *
   * @param setterProps the properties which values contain to be replaced "place-holder", assumed
   *     not <code>null</code>.
   * @param props the properties that may contain replaced value, assumed not <code>null</code>, but
   *     may be empty.
   * @param isDelta <code>true</code> if <code>props</code> is the delta of current and previous
   *     properties. In this case, don't log warning if failed to lookup a property; otherwise log
   *     warning.
   * @return the properties with resolved values. It may be <code>null</code> if there is no
   *     replacement.
   */
  private Map<String, Object> replacePropertyValues(
      Map<String, Object> setterProps, Map<String, Object> props, boolean isDelta) {
    ...
    for (Map.Entry<String, Object> prop : setterProps.entrySet()) {
      ...
      if (value.getSecond()) replaced = true;
    }
   ...
   *
   * @param value the collection of values in question, assumed not <code>null</code>.
   * @param props the properties may contains replacement value, assumed not <code>null</code>.
   * @param isDelta <code>true</code> if <code>props</code> is the delta of current and previous
   *     properties.
   * @return a pair, where the 1st element is the collection that may or may not contains the
  ...
  private PSPair<Object, Boolean> replaceCollectionValues(
    ...
    if (value instanceof Collection) {
      ...
      for (Object obj : (List) tmpList) {
        if (obj instanceof String) r = getReplacedValue((String) obj, props, isDelta);
        else r = replaceCollectionValues(obj, props, isDelta);
        if (r.getSecond()) replaced = true;
  ...
  private PSPair<Object, Boolean> replaceMapValues(
    ...
    for (Object key : map.keySet()) {
      ...
      if (value instanceof String) r = getReplacedValue((String) value, props, isDelta);
      else r = replaceCollectionValues(value, props, isDelta);
      if (r.getSecond()) replaced = true;
   ...
   *     "${com.percussion.RSS.serverName}", where the key of the value is surrounded by prefix "${"
   *     and postfix "}".
   * @param props the properties may contain the value of the place-holder, assumed not <code>null
   *     </code>, but be empty.
   * @param isDelta <code>true</code> if <code>props</code> is the delta of current and previous
   *     properties.
  ...
  private PSPair<Object, Boolean> getReplacedValue(
    ...
    PSPair<Object, Boolean> r = resolveSimplePlaceholder(origValue, props, isDelta);
    if (r != null) return r;
    ...
    if (i == -1) return new PSPair(origValue, false);
  ...
  /**
   * Resolves the value of a simple place-holder from the given property value. Note, the 1st object
   * of the returned pair can be a string or a collection (List or Map).
   *
   * @param origValue the string in question, it may not <code>null</code>.
   * @param props the properties may contain replaced value, it may not <code>null</code>.
   * @return a pair, where the 1st element is the replaced value or the original value; the 2nd
   *     element determines if a replacement has happened. It may be <code>null</code> if there is
   *     no leading {@link #PREFIX} and no ending {@link #SUFFIX}.
   */
   ...
   * @return the place-holder. It may be <code>null</code> if the given string does not contain one
   *     place-holder.
   */
  private static String getSimplePlaceholder(String origValue) {
    ...
    if ((!trimed.startsWith(PREFIX)) || (!trimed.endsWith(SUFFIX))) return null;
  ...
  /**
   * Gets a list of ${place-holder} from a given string. It returns a pair of values, where the 1st
   * value is a list of ${place-holders}, 2nd value indicates if the string contains only one
   * ${place-holder} or it contains ${place-holders} and other texts. For example, if the string
   * contains only one ${place-holder} without any other text, then the 2nd value is <code>true
   * </code>; otherwise the 2nd value is <code>false</code>.
   *
   * @param origValue the string may contain one or more ${place-holder}. It may be blank.
   * @return a pair described above. It may be <code>null</code> if there is no ${place-holder} in
   *     the given string.
   */
  ...
  /**
   * Gets one place-holder from the given string. Assumed the string does not contain {@link
   * #PREFIX}, but it may contain {@link #SUFFIX}.
   *
   * @param origValue the string in question, it may be blank.
   * @return the place-holder. It may be <code>null</code> if cannot find a matching {@link #SUFFIX}
   *     in the given string.
   */
  ...
  private static PSPair<Object, Boolean> resolveSimplePlaceholder(
      String origValue, Map<String, Object> props, boolean isDelta) {
    if (origValue == null) throw new IllegalArgumentException("origValue may not be null.");
    if (props == null) throw new IllegalArgumentException("props may not be null.");

    if ((!origValue.startsWith(PREFIX)) || (!origValue.endsWith(SUFFIX))) return null;

    String sub = origValue.substring(PREFIX.length());
    if (sub.indexOf(PREFIX) != -1) return null;

    PSPair<Object, Boolean> r = replaceOneValue(sub, props, isDelta);
    if (!r.getSecond()) r.setFirst(origValue);
    return r;
  }

  /**
   * Replace a string, which may contain replaced place-holder. The place-holder is surrounded by
   * {@link #PREFIX} and {@link #SUFFIX}.
   *
   * <p>Note, the ${place-holder} will not be replaced if the place-holder does not exist in the
   * given properties, and a warning will be logged in this case.
   *
   * @param origValue the string in question, assumed not <code>null</code>.
   * @param props the properties may contain replaced value, assumed not <code>null</code>.
   * @param isDelta <code>true</code> if <code>props</code> is the delta of current and previous
   *     properties.
   * @return a pair, where the 1st element is the replaced value or the original value; the 2nd
   *     element determines if a replacement has happened, never <code>null</code>.
  ...
  private static PSPair<Object, Boolean> replaceOneValue(
      String origValue, Map<String, Object> props, boolean isDelta) {
    int index = origValue.indexOf(SUFFIX);
    if (index == -1) return new PSPair(origValue, false);
    ...
    if (rValue == null) {
      ...
      if (index == origValue.length() - 1) return new PSPair(null, true);
      else return new PSPair(origValue.substring(index + 1), true);
    }

    if (index == origValue.length() - 1) return new PSPair(rValue, true);
    else return new PSPair(rValue.toString() + origValue.substring(index + 1), true);
  }
