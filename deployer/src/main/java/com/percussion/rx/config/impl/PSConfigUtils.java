...
public class PSConfigUtils {
    ...
    public static Collection<String> getObjectNames(Object objNames, IPSDesignModel model, String propName) {
        ...
        if (objNames instanceof String) {
            ...
            throw new PSConfigException("The design object \"" + propName + "\" property must be a \"Collection\" or a String type.");
        }
    ...
    public static PSRule createRule(Map<String, Object> map) {
        ...
        if (StringUtils.isBlank(type)) {
            throw new PSConfigException("The required property " + PROP_TYPE + " is missing for the supplied condition.");
        }
        if (type.equalsIgnoreCase(TYPE_EXTENSION)) {
            var name = (String) map.get(PROP_NAME);
            if (StringUtils.isBlank(name))
                throw new PSConfigException("The required property " + PROP_NAME + " is missing for the supplied condition.");
            ...
            if (rules == null)
                throw new PSConfigException("The required property " + PROP_RULES + " is missing for the supplied condition.");
            rule = createConditionalRule(rules);
        } else {
            var msg = "The supplied type of condition ({0}) is invalid. Valid types are \"" + TYPE_CONDITIONAL + "\" and \"" + TYPE_EXTENSION + "\"";
        ...
        if (!(op.equalsIgnoreCase("and") || op.equalsIgnoreCase("or"))) {
            throw new PSConfigException("The value of \"operators\" property must be a either \"and\" or \"or\".");
        }
    ...
    private static PSRule createConditionalRule(List<Map<String, String>> rules) {
        ...
        for (var rule : rules) {
            ...
            if (!(bool.equalsIgnoreCase("AND") || bool.equalsIgnoreCase("OR"))) {
                throw new PSConfigException("The value of \"boolean\" property must be a either \"AND\" or \"OR\".");
            }
     ...
     */
    public static PSExtensionCall createExtensionCall(String name, List<String> params, String interfaceName) {
        if (StringUtils.isBlank(name))
        ...
        if (extRef == null) {
            var msg = "The supplied extension name ({0}) does not correspond to a valid extension in the system.";
