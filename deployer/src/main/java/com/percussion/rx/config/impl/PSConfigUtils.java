...
public class PSConfigUtils {
  ...
  public static Document getDocument(String url, Map extraParams, boolean applyStyleSheet) {
    ...
    PSRequest origReq = (PSRequest) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUEST);
    ...
    try {
      ...
      IPSInternalRequest ireq = reqCtxt.getInternalRequest(url, extraParams, true);
      if (ireq == null) {
  ...
  public static Collection<String> getObjectNames(
    ...
    if (objNames instanceof String) {
      ...
      Collection<String> names = PSPatternMatch.matchedStrings((String) objNames, allNames);
  ...
  public static void saveObjectToFile(Object obj, File f) {
    ...
    if (!f.exists()) {
      ...
      if (!parent.exists()) {
        if (!parent.mkdirs()) {
          String errMsg = "Failed to create dir: \"" + parent.getAbsolutePath() + "\".";
          log.error(errMsg);
  ...
  public static PSCollection prepareConditions(Object conditonConfig) {
    if (!(conditonConfig instanceof List))
      throw new PSConfigException(
          "The value of \"" + "conditions" + "\" property must be a \"List\" type");
  ...
  public static PSRule createRule(Map<String, Object> map) {
    if (map == null) throw new PSConfigException("map must not be null for creating the rule.");
    ...
    if (type.equalsIgnoreCase(TYPE_EXTENSION)) {
      ...
      List<String> extensionParams = (List<String>) map.get(PROP_EXTENSION_PARAMS);
      rule = createExtensionRule(name, extensionParams);
    } else if (type.equalsIgnoreCase(TYPE_CONDITIONAL)) {
      List<Map<String, String>> rules = (List<Map<String, String>>) map.get(PROP_RULES);
      if (rules == null)
        throw new PSConfigException(
      ...
      String msg =
          "The supplied type of condition ({0}) is invalid. "
              + "Valid types are \""
              + TYPE_CONDITIONAL
              + "\" and \""
              + TYPE_EXTENSION
  ...
  private static PSRule createExtensionRule(String name, List<String> params) {
    ...
    PSExtensionCall extCall =
        createExtensionCall(name, params, "com.percussion.extension.IPSUdfProcessor");
    callSet.add(extCall);
  ...
  public static PSExtensionCall createExtensionCall(
    ...
    try {
      ...
      if (StringUtils.isNotBlank(interfaceName)) {
        Iterator iterator =
            mgr.getExtensionNames(null, null, interfaceName, tempRef.getExtensionName());
    ...
    if (extRef == null) {
      String msg =
          "The supplied extension name ({0}) does not "
    ...
    PSExtensionCall extCall =
        new PSExtensionCall(
            extRef, paramValues.toArray(new PSExtensionParamValue[paramValues.size()]));
  ...
  private static IPSReplacementValue createReplacementValue(String name) {
    ...
    IPSReplacementValue rv = PSReplacementValueFactory.getReplacementValueFromString(name);
    return rv;
  }
  ...
  public static PSConditional createBooleanCondition(boolean flag) {
    ...
    IPSReplacementValue val = flag ? new PSTextLiteral("1") : new PSTextLiteral("2");
