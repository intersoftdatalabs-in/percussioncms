...
public class PSSiteConfigHandler extends PSObjectConfigHandler {
  ...
  public boolean equals(Object otherObj) {
    if ((!(otherObj instanceof PSSiteConfigHandler) || (!super.equals(otherObj)))) {
      return false;
    }

    PSSiteConfigHandler other = (PSSiteConfigHandler) otherObj;
    return new EqualsBuilder().append(m_siteNames, other.m_siteNames).isEquals();
  }

  @Override
  public int hashCode() {
    return super.hashCode() + new HashCodeBuilder().append(m_siteNames).toHashCode();
  }
  ...
  public void setType(PSTypeEnum type) {
    if (!type.equals(PSTypeEnum.SITE)) throw new IllegalArgumentException("type must be SITE.");
  }
  ...
  public void setName(String name) {
    if (getNames() != null) throw new IllegalStateException("Cannot set both name and names");

    super.setName(name);
  }

  /**
   * Gets a list of Site names. This may be the {@link #NAMES} property of the handler that is wired
   * by Spring bean file.
   *
   * @return a list of Site names as a {@link List List&lt;String>} type or it may be a string that
   *     contains a ${place-holder}. It may be <code>null</code> if the {@link #setNames(Object)}
   *     has never been called or wired by Spring framework.
   */
  ...
  /**
   * Sets the name of the design objects. This is used when there are more than one design objects
   * for the handler to process or apply the properties with. This cannot be used in conjunction
   * with {@link #setName(String)}.
   *
   * @param names the names of the design objects. It is either a {@link List List&lt;String>} type
   *     or a string that contains a ${place-holder}. It may not be <code>null</code> or empty.
   */
  public void setNames(Object names) {
    if (getName() != null) throw new IllegalStateException("Cannot set both names and name.");
  ...
  public Object getDefaultDesignObject(Map<String, Object> cachedObjs) throws PSNotFoundException {
    ...
    if (StringUtils.isBlank(siteName)) {
      ...
      if ((namesObj instanceof String) && ((String) namesObj).contains("${")) {
        namesObj = "*";
      }
      Collection<String> names = PSConfigUtils.getObjectNames(namesObj, getSiteModel(), NAMES);
      if (names != null && !names.isEmpty()) siteName = names.iterator().next();
    }
  ...
  protected Collection<String> getPrevNames() {
    ...
    if (StringUtils.isNotBlank(getName())) {
      ...
      if (prevProps != null && (!prevProps.isEmpty())) {
        names = PSConfigUtils.getObjectNames(prevProps.get(NAMES), getSiteModel(), NAMES);
      }
    }
    return names;
  }
  ...
  /**
   * This method does the actual work for {@link #getDesignObjects(Map)}. The cached object map will
   ...
   */
  private Object getSite(String name, Map<String, Object> cachedObjs, IPSDesignModel model)
      throws PSNotFoundException {
    if (cachedObjs.get(name) != null) return cachedObjs.get(name);
  ...
  /**
   * Holds the {@link #NAMES} property, the map key is {@link #NAMES}, the value is a list of Site
   * names, a string with pattern names, or <code>null</code> if the property is not defined.
   */
