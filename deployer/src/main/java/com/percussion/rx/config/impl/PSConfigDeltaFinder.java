...
public class PSConfigDeltaFinder {
   ...
   * @param oldProps may be <code>null</code> or empty, in which case newProps are returned as
   *     delta.
   * @return delta of new and old properties. Never <code>null</code>, may be empty.
   */
  public Map<String, Object> getConfigDelta(
      Map<String, Object> newProps, Map<String, Object> oldProps) {
    if (newProps == null) throw new IllegalArgumentException("newProps must not be null");
    ...
    while (iter.hasNext()) {
      ...
      if (!oldProps.containsKey(key)) {
        ...
        if ((objN == null && objO != null) || (objN != null && (!objN.equals(objO)))) {
          delta.put(key, objN);
        }
      }
    }
