...
/**
 * Setter for Location Scheme-specific properties.
 * Assumes the Location Scheme already contains valid Context, Content Type, and Template.
 *
 * @author YuBingChen
 */
public class PSLocationSchemeSetter extends PSSimplePropertySetter {
    ...
    private void validateScheme(Object obj) {
        ...
        if (StringUtils.isBlank(scheme.getGenerator())) {
            throw new PSConfigException("Failed to configure Location Scheme \"" +
                    scheme.getName() + "\". Either \"" + EXPRESSION + "\" or \"" + GENERATOR +
                    "\" property must be defined.");
        }
    }

    /**
     * Processes or sets the generator property if defined.
     * Validates that both legacy and JEXL generators are not configured simultaneously.
     *
    ...
    private boolean processGenerator(Object obj) {
        ...
        if (expr != null && generator != null) {
            throw new PSConfigException("Failed to configure Location Scheme \"" +
                    scheme.getName() + "\". Both \"" + EXPRESSION + "\" and \"" + GENERATOR +
                    "\" properties cannot be specified at the same time.");
        }
        if (expr != null && params != null) {
            throw new PSConfigException("Failed to configure Location Scheme \"" +
                    scheme.getName() + "\". Both \"" + EXPRESSION + "\" and \"" + GENERATOR_PARAMS +
                    "\" properties cannot be specified at the same time.");
        }
    ...
    /**
     * Filters the given parameters for the supplied Java extension.
     * Removes entries whose keys are not defined as parameters of the slot-filter.
     */
    @SuppressWarnings("unchecked")
    private List<PSPair<String, String>> filterParameters(String extFQN, Object propValue) {
        ...
        for (var p : props) {
            if (!names.contains(p.getFirst())) {
                ...
                log.warn("Skip finder argument \"{}\" since it is not a parameter defined by generator \"{}\".",
                        p.getFirst(), ref.getExtensionName());
                continue;
            }
            params.add(p);
        }
    ...
    private PSExtensionRef getExtensionRef(String extName) {
        ...
        try {
            var iterator = mgr.getExtensionNames(null, null,
