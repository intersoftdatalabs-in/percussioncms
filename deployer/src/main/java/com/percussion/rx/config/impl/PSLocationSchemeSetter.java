...
/**
 * This setter is used to set Location Scheme specific properties. Assumed the to be configured
 * Location Scheme already contains valid Context, Content Type and Template.
 *
 * @author YuBingChen
 */
public class PSLocationSchemeSetter extends PSSimplePropertySetter {
  ...
  /**
   * Validates the Location Scheme, make sure it contains all required properties.
   *
  ...
  /**
   * Process or set the generator property if it is defined. This also validates the defined
   * properties, make sure there is confusion regarding configuring a legacy or JEXL generator, but
   * not both.
   *
   * @param obj the Location Scheme to be configured with, assumed not <code>null</code>.
   ...
   *
   * @param scheme the Location Scheme to set the parameters to, assumed not <code>null</code>.
   * @param propValue the new set of parameters. Never <code>null</code>, may be empty. Expect to be
   *     a List type.
   */
  private void setGeneratorParams(IPSLocationScheme scheme, Object propValue) {
    List<PSPair<String, String>> params = filterParameters(scheme.getGenerator(), propValue);

    // cleanup existing parameters
    List<String> pnames = new ArrayList<>();
    pnames.addAll(scheme.getParameterNames());
    for (String n : pnames) scheme.removeParameter(n);
  ...
  /**
   * Filter the given parameter for the supplied java extension. It removes the entries of the map
   * that the keys are not defined as the parameters of the slot-filter. The slot-filter is a Java
   * extension.
   *
   * @param extFQN the FQN java extension, never <code>null</code> or empty.
   * @param propValue the new parameters for the above java extension., never <code>null</code>.
