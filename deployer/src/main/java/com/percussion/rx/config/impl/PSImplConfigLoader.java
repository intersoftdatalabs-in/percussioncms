 ...
 * configuration file contains beans that implemented {@link IPSConfigHandler}, provides API to
 * retrieve these beans and ignore the beans that are not instance of {@link IPSConfigHandler}
 *
 * @author YuBingChen
 */
public class PSImplConfigLoader {
  ...
  public PSImplConfigLoader(String configPath) {
    if (StringUtils.isBlank(configPath))
      throw new IllegalArgumentException("configPath may not be null or emtpy.");
   ...
   *
   * @param name the name of the bean. It must be one of the element returned from {@link
   *     #getAllBeanNames()}.
