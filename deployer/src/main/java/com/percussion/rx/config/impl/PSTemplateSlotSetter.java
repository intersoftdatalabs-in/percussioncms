...
public class PSTemplateSlotSetter extends PSPropertySetterWithValidation {
  ...
  protected boolean applyProperty(
    ...
    } else if (FINDER_ARGUMENTS.equals(propName) || FINDER_PARAMS.equals(propName)) {
  ...
  protected List<PSConfigValidation> validate(
    ...
    if (curAssoc.isEmpty() || otherAssoc.isEmpty()) return Collections.emptyList();
    ...
    buffer.append(
        "the following pairs of Content Type / Template associations are already configured: ");
    for (PSPair<String, String> pair : curAssoc) {
      buffer.append(" (" + pair.getFirst() + ", " + pair.getSecond() + ")");
    }
    PSConfigValidation vError =
        new PSConfigValidation(objName, SLOT_ASSOCIATION, true, buffer.toString());
  ...
  private boolean removeSlotAssociations(List<IPSAssociationSet> aSets, Object propValue) {
    ...
    List<PSPair<String, String>> curAssoc = convertObjectToList(propValue);
    if (curAssoc.isEmpty()) return false;
  ...
  private void setFinderArguments(IPSTemplateSlot slot, Object propValue) {
    ...
    if (!(propValue instanceof Map))
      throw new PSConfigException("The value of \"" + FINDER_ARGUMENTS + "\" must be Map.");

    slot.setFinderArguments(filterParameters(slot, propValue));
  }

  /**
   * Filter the given filter-arguments for the supplied slot. It removes the entries of the map that
   * the keys are not defined as the parameters of the slot-filter. The slot-filter is a Java
   * extension.
   *
  ...
  private Map<String, String> filterParameters(IPSTemplateSlot slot, Object propValue) {
    ...
    params.putAll(props);
    List<String> names = PSConfigUtils.getExtensionParameterNames(slot.getFinderName());
    for (String name : props.keySet()) {
      if (!names.contains(name)) {
        ms_log.warn(
            "Skip finder argument \""
                + name
  ...
  private void setSlotAssociations(List<IPSAssociationSet> aSets, Object propValue) {
    ...
    if (curAssoc.isEmpty() && prevAssoc.isEmpty()) return;
   ...
   * @param action the action of the searched association, assumed not <code>null</code>.
   * @return the association with the specified action, never <code>null</code>.
   */
  private IPSAssociationSet getAssoc(List<IPSAssociationSet> aSets, AssociationAction action) {
    for (IPSAssociationSet assoc : aSets) {
      if (assoc.getAction().equals(action)
          && assoc.getType().equals(AssociationType.SLOT_CONTENTTYPE_TEMPLATE)) return assoc;
    }
    throw new IllegalArgumentException(
        "Cannot find associations with action = "
            + action.name()
            + ", type = "
   ...
   * @return the converted list, never <code>null</code>, but may be empty if the object is <code>
   *     null</code> or empty list.
   */
  ...
  private List<PSPair<String, String>> convertObjectToList(Object propValue) {
    ...
    if (!(propValue instanceof List))
      throw new IllegalArgumentException("The slot association value type must be List.");
  ...
  /** The property name for the finder arguments, the same as {@link #FINDER_ARGUMENTS}. */
  public static final String FINDER_PARAMS = "finderParams";

  /**
   * The property name for the associations between the Slot to a list of Content-Type / Template.
   */
