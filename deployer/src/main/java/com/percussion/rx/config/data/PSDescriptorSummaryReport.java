...
public class PSDescriptorSummaryReport {
  ...
  private void createPackagesSections(StringBuilder sb, PSExportDescriptor desc) {
    ...
    for (Map<String, String> entry : desc.getPkgDepList()) {
      ...
      boolean isImplied = Boolean.valueOf(entry.get(PSDescriptor.XML_PKG_DEP_IMPLIED));
      String display = BULLET + SPACE + name + " (" + version + ")" + NEWLINE;
      if (isImplied) {
        iDeps.add(display);
      } else {
        aDeps.add(display);
    ...
    if (!iDeps.isEmpty()) {
      for (String dep : iDeps) sb.append(dep);
    } else {
      sb.append("None");
      sb.append(NEWLINE);
    }
    ...
    if (!aDeps.isEmpty()) {
      for (String dep : aDeps) sb.append(dep);
    } else {
      sb.append("None");
      sb.append(NEWLINE);
    }
   ...
   * @param sb
   */
  private void createSectionFromMultiMap(String title, PSMultiValueHashMap map, StringBuilder sb) {
    sb.append(title);
    sb.append(NEWLINE);
    sb.append(SEPARATOR);
  ...
  private void handleElements(PSExportDescriptor desc) {
    ...
    while (elements.hasNext()) {
      ...
      if (depend.getObjectType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM)
  ...
  /**
   * Helper method to return the actual dependency, meaning if "Custom" then we need to retrieve the
   * "wrapped" dependency.
   *
  ...
  private void handleDepends(
    ...
    Iterator<PSDependency> children = (Iterator<PSDependency>) depend.getDependencies();
  ...
  /**
   * Gets the file resource paths from the passed in user dependency if specified. If not it just
   * set file resources to an empty Set.
   *
