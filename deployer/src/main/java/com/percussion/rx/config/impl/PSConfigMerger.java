...
public class PSConfigMerger {
  ...
  public PSPair<Collection<IPSGuid>, PSConfigException> merge(
    ...
    for (IPSConfigHandler handler : cfgHandlers) {
      ...
      List<PSPair<Object, ObjectState>> objs =
          getDesignObjectsWithState(type, model, handler, hasPrevProps);
      if (objs.isEmpty()) {
        m_handlerData.put(handler, new HandlerData(null, null, null));
        continue;
      }
    ...
    try {
      ...
      for (IPSConfigHandler handler : cfgHandlers) {
        ...
        List<PSPair<Object, ObjectState>> objs = m_handlerData.get(handler).mi_designObjects;
        ...
        if (objs == null || objs.isEmpty()) {
          ...
          for (PSPair<Object, ObjectState> op : objs) {
            ...
            if (model != null) {
              ...
              if (processed) {
                IPSGuid processedGuid = handler.saveResult(model, o, state, assocList);
  ...
  public PSPair<Map<String, Object>, List<Exception>> getPropertyDefs(
    ...
    PSPair<Map<String, Object>, List<Exception>> result = new PSPair(props, cfgExceptions);
    for (IPSConfigHandler handler : cfgHandlers) {
      try {
        PSTypeEnum type = handler.getType();
        if (type == null) {
