...
public class PSActionSetter extends PSSimplePropertySetter {
  ...
  private Map<String, Object> getActionVisibility(PSAction action) {
    ...
    PSActionVisibilityContexts actionContexts = action.getVisibilityContexts();
    ...
    while (ctxIter.hasNext()) {
      PSActionVisibilityContext avContext = (PSActionVisibilityContext) ctxIter.next();
  ...
  private void setVisibility(PSAction action, Object propValue) {
    ...
    Map<String, String> visContexts =
        getNormalizedMaps(getResourceLookupData(VISIBILITY_CONTEXTS_LOOKUP_KEY));
    PSActionVisibilityContexts actionContexts = action.getVisibilityContexts();
    ...
    while (iter.hasNext()) {
      ...
      String contextVal = visContexts.get(StringUtils.trimToEmpty(context).toLowerCase());
      if (contextVal == null) {
      ...
      if (!(values instanceof String || values instanceof List)) {
        String msg = "Unsupported object supplied for values of visibility " + "context ({0}).";
        Object[] args = {context};
        throwError(msg, args);
      }
      List valList =
          values instanceof String ? Collections.singletonList(((String) values)) : (List) values;
  ...
  private String[] getValidValues(String context, String contextVal, List<String> values) {
    ...
    if (contextVal.equals(PSActionVisibilityContext.VIS_CONTEXT_CONTENT_TYPE)) {
      IPSDesignModelFactory f = PSDesignModelFactoryLocator.getDesignModelFactory();
      ...
      for (String val : values) {
        String sVal = supportedVals.get(StringUtils.trimToEmpty(val).toLowerCase());
        if (sVal == null) {
          if (ArrayUtils.contains(m_staticValues, contextVal)) {
            String msg = "Unsupported value ({0}) is supplied for " + "context ({1}).";
