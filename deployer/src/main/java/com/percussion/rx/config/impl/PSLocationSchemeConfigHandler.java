...
public class PSLocationSchemeConfigHandler extends PSObjectConfigHandler {
  ...
  public boolean equals(Object otherObj) {
    if ((!(otherObj instanceof PSLocationSchemeConfigHandler) || (!super.equals(otherObj)))) {
      return false;
    }

    PSLocationSchemeConfigHandler other = (PSLocationSchemeConfigHandler) otherObj;
    return new EqualsBuilder().append(m_extraProperties, other.m_extraProperties).isEquals();
  }

  @Override
  public int hashCode() {
    return super.hashCode() + new HashCodeBuilder().append(m_extraProperties).toHashCode();
  }
  ...
  public List<PSConfigValidation> validate(IPSConfigHandler other) {
    ...
    for (PSPair<String, ObjectState> pair : commonNames) {
      String msg =
          " Context / Location Scheme pair \"" + pair.getFirst() + "\" is already configured.";
  ...
  public boolean process(Object obj, ObjectState state, List<IPSAssociationSet> aSets) {
    if (m_isUnProcess)
      throw new IllegalStateException("Cannot call process() after called unprocess().");
   ...
   *
   * @return a list of Context names as a {@link List List&lt;String>} type or it may be a string
   *     that contains a ${place-holder}. It may be <code>null</code> if the has never been called
   ...
   *
   * @param names the names of the Contexts. It is either a {@link List List&lt;String>} type or a
   *     string that contains a ${place-holder}. It may not be <code>null</code> or empty.
   */
  public void setContexts(Object names) {
    if (names == null) throw new IllegalStateException("Context names may not be null.");

    m_extraProperties.put(CONTEXTS, names);
  }

  /**
   * Returns the Template name. This is an optional property for configuring existing Location
   * Scheme. However, it is a required property for configuring a new Location Scheme.
   *
   * @return the Template name. It may be <code>null</code> if the Template name is not defined.
   */
   ...
   *
   * @param name the new Template name. It may not be <code>null</code> or empty.
   */
  ...
  /**
   * Returns the Content Type name. This is an optional property for configuring existing Location
   * Scheme. However, it is a required property for configuring a new Location Scheme.
   *
   * @return the Content Type name. It may be <code>null</code> if the Template name is not defined.
   */
   ...
   *
   * @param name the new Content Type name. It may not be <code>null</code> or empty.
   */
  public void setContentType(String name) {
    if (StringUtils.isBlank(name))
      throw new PSConfigException("Content Type name may not be null or empty.");
  ...
  public List<PSPair<Object, ObjectState>> getDesignObjects(Map<String, Object> cachedObjs)
    ...
    if (StringUtils.isBlank(getName()))
      throw new PSConfigException("Location Scheme name may not be null or empty.");
  ...
  public Object getDefaultDesignObject(Map<String, Object> cachedObjs) throws PSNotFoundException {
    ...
    // config.
    if ((contextsObj instanceof String) && ((String) contextsObj).contains("${")) {
      contextsObj = "*";
    }
    ...
    if (!names.contains(locSchemeName)) locSchemeName = names.iterator().next();
    Object scheme = getLocationScheme(locSchemeName, PSConfigUtils.getSchemeModel(), cachedObjs);
    return scheme;
  }
  ...
  protected Collection<String> getPrevNames() {
    ...
    if (prevProps == null || prevProps.isEmpty()) return Collections.emptyList();

    return getNamesFromCtxProperty(prevProps.get(CONTEXTS));
  }

  /**
   * Gets the Location Scheme names from the specified contexts property, which may be a string with
   * wild-card, such as "*s*"; or a list of strings.
   *
  ...
  private Collection<String> getNamesFromCtxProperty(Object ctxValue) {
    ...
    Collection<String> ctxNames =
        PSConfigUtils.getObjectNames(ctxValue, PSConfigUtils.getContextModel(), CONTEXTS);
   ...
   * @return the cached, loaded or created Location Scheme, never <code>null</code>.
   */
   ...
   * This method does the actual work for {@link #getDesignObjects(Map)}.
   *
   * @param uniqueSchemeName the unique location scheme name, assumed not <code>null</code> or
   *     empty.
   * @param model the Location Scheme model, never <code>null</code>.
   * @param cachedObjs the cached Location Scheme objects, never <code>null</code>, may be empty.
   * @return the cached, loaded or created Location Scheme, never <code>null</code>.
   */
  private Object getLocationScheme(
      String uniqueSchemeName, PSLocationSchemeModel model, Map<String, Object> cachedObjs)
      throws PSNotFoundException {
  ...
  /**
   * Validates the specified Location Scheme name, make sure it is unique within the specified
   * Context.
   *
   * @param schemeName the Location Scheme name in question, assumed not <code>null</code> or empty.
   * @param model the Location Scheme model, assumed not <code>null</code>.
   * @param ctxName the Context name, which is already set to the above model, assumed not <code>
   *     null</code>.
   */
  private void validateUniqueSchemeName(
      String schemeName, PSLocationSchemeModel model, String ctxName) {
    boolean found = false;
    for (String n : model.findAllNames()) {
      if (!schemeName.equalsIgnoreCase(n)) continue;
      ...
      throw new PSConfigException(
          "Failed to configure Location Scheme \""
              + schemeName
              + "\" because the name is not unique in Context \""
              + ctxName
              + "\".");
    }
  }
  ...
  /**
   * Validates the Content Type and Template specified in the given Location Scheme are the same as
   * the {@link #getContentType()} and {@link #getTemplate()}. Skip the checking of any of the
   * properties are not defined.
   *
   * @param schemeObj the Location Scheme in question, assumed not <code>null</code>.
   * @param ctxName the Context name, assumed not <code>null</code> or empty.
   */
  private void validateSameContentTypeAndTemplate(Object schemeObj, String ctxName) {
    IPSLocationScheme scheme = (IPSLocationScheme) schemeObj;
    if (StringUtils.isNotBlank(getContentType())) {
      IPSGuid ctId = PSConfigUtils.getContentTypeModel().nameToGuid(getContentType());
    ...
    if (StringUtils.isNotBlank(getTemplate())) {
      IPSGuid tpId = PSConfigUtils.getTemplateModel().nameToGuid(getTemplate());
   ...
   *
   * @param model the model used to create the Location Scheme, assumed not <code>null</code>.
   * @param ctxName the Context name, assumed not <code>null</code> or empty.
   * @param cachedObjs cached, loaded or created Location Scheme, never <code>null</code>.
   * @return the created Location Scheme, never <code>null</code>.
   */
  private IPSLocationScheme createLocationScheme(
      PSLocationSchemeModel model, String ctxName, Map<String, Object> cachedObjs) {
    validateNoneBlankContentTypeTemplate();
    PSPair<IPSGuid, IPSGuid> pair = getContentTypeTemplateAssoc();
    validateUniqueContentTypeTemplateAssoc(
        pair.getFirst(), pair.getSecond(), model, ctxName, cachedObjs);
  ...
  /**
   * Validates the specified Content Type and Template association, make sure it is different with
   * all Location Schemes under the same Context. It is validated against the Location Schemes in
   ...
   * @param cachedObjs cached, loaded or created Location Scheme, never <code>null</code>.
   */
  private void validateUniqueContentTypeTemplateAssoc(
      IPSGuid ctId,
      IPSGuid tpId,
      PSLocationSchemeModel model,
      String ctxName,
  ...
  /**
   * Validates the specified Content Type and Template association is not contained in the given
   * Location Scheme.
   *
   * @param ctId the Content Type ID, assumed not <code>null</code>.
   * @param tpId the Template ID, assumed not <code>null</code>.
   * @param ctxName the Context name, assumed not <code>null</code> or empty.
   * @param scheme the Location Scheme in question. assumed not <code>null</code>.
   * @param isExist <code>true</code> if the Location Scheme exists in the repository.
   */
  private void validateDiffContentTypeTemplate(
      IPSGuid ctId, IPSGuid tpId, String ctxName, IPSLocationScheme scheme, boolean isExist) {
    if (scheme.getTemplateId().longValue() != tpId.longValue()
        || scheme.getContentTypeId().longValue() != ctId.longValue()) return;

    if (isExist) {
      throw new PSConfigException(
          "Failed to create Location Scheme \""
              + getName()
              + "\" since the existing Location Scheme \""
              + scheme.getName()
   ...
   *
   * @return the validate Content Type and Template IDs, never <code>null</code>.
   */
    ...
    if (!validateContentTypeTemplateAssoc(ctGuid, tpGuid)) {
      throw new PSConfigException(
          "Failed to create Location Scheme \""
              ...
              + "\" since the Content Type \""
              + ctName
              + "\" does not associate with Template \""
              + template
              + "\".");
    }
  ...
  /**
   * Determines if the specified Content Type and Template is a valid association.
   *
   * @param ctId the Content Type ID, assumed not <code>null</code>.
   * @param tpId the Template ID, assumed not <code>null</code>.
  ...
  /** Validate the Content Type name and Template name are not <code>null</code> or empty. */
   ...
   *
   * @param ctxName the Context name, assumed not <code>null</code> or empty.
   * @return the unique name Location Scheme name, never <code>null</code> or empty.
   */
  ...
  /**
   * Gets the context name from a unique name, which is returned from {@link
  ...
  private String getContextFromUniqueName(String uniqueName) {
    return uniqueName.substring(0, uniqueName.length() - UNIQUE_SEP.length() - getName().length());
  }

  /**
   * Determines of the specified Location Scheme name is belong to the given Context.
   *
   * @param ctxName the Context name, assumed not <code>null</code> or empty.
   * @param uniqueName the unique name of the Location Scheme that is created by {@link
   *     #getUniqueSchemeName(String)}.
   * @return <code>true</code> if the Location Scheme is under the given Context.
   */
  ...
  /**
   * Determines if the {@link #unprocess(Object, List)} was called. Default to <code>false</code>.
   */
  private boolean m_isUnProcess = false;

  /**
   * The name of the extra/additional property. This is a required property see {@link
   * #getContexts()} for details.
   */
  public static final String CONTEXTS = "contexts";

  /**
   * The Template name property. This is optional property for existing Location Scheme. However, it
   * is a required property for configuring created/new Location Scheme.
   */
  public static final String TEMPLATE = "template";

  /**
   * The Content Type name property. This is optional property for existing Location Scheme.
   * However, it is a required property for configuring created/new Location Scheme.
   */
