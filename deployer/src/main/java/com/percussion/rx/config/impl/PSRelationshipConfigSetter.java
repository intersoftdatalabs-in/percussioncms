...
/**
 * Relationship configuration setter to set the following three properties. shallowCloning,
 * deepCloning and fieldOverrides.
 *
 * @author bjoginipally
 */
public class PSRelationshipConfigSetter extends PSSimplePropertySetter {
   ...
   *
   * @param relConfig The PSRelationshipConfig object on which the property needs to be set, assumed
   *     not <code>null</code>.
  ...
  private void setCloneFieldOverrides(PSRelationshipConfig relConfig, Object propValue) {
    ...
    for (Map<String, Object> map : tempMap) {
      ...
      PSCloneOverrideField cofld = new PSCloneOverrideField(fieldName, extCall);
      if (condition != null) {
   ...
   * @param relConfig The PSRelationshipConfig object on which the property needs to be set, assumed
   *     not <code>null</code>.
   * @param isShallow boolean flag to indicate whether the cloning property is shallow or not.
   * @return The object corresponding to the cloning property. It is a map of name of the
  ...
  private Map<String, Object> getClonningProperty(
    ...
    if (!conditions.isEmpty()) {
      List<Map<String, Object>> condDefs = PSConfigUtils.getCondtionsDef(conditions.iterator());
   ...
   * Helper method to get the clone field overrides property.
   *
   * @param relConfig The PSRelationshipConfig object from which the property needs to be set,
  ...
  private List<Map<String, Object>> getCloneFieldOverrides(PSRelationshipConfig relConfig) {
    ...
    while (iter.hasNext()) {
      ...
      if (!rules.isEmpty()) {
        List<Map<String, Object>> rulesDef = PSConfigUtils.getCondtionsDef(rules.iterator());
   ...
   * @param isShallow boolean flag to indicate whether it is shallow cloning or deep cloning.
   */
  @SuppressWarnings("unchecked")
  private void setCloningProperty(
    ...
    if (StringUtils.isBlank(enabled)) {
      String msg = "The required property ({0}) is missing for the " + "supplied property ({1}).";
