...
public class PSDefaultConfigGenerator {
  ...
  public static PSDefaultConfigGenerator getInstance() {
    if (ms_instance == null) ms_instance = new PSDefaultConfigGenerator();
    return ms_instance;
  }
  ...
  public String generateDefaultConfig(
    ...
    ms_logger.info("Generating  default configuration file for package \"" + packageName + "\"");
    ...
    PSPair<Map<String, Object>, List<Exception>> propertyDefs = merger.getPropertyDefs(handlers);
    ...
    if (!propertyDefs.getSecond().isEmpty()) {
      ...
      ms_logger.error(
          "The following errors occurred while generating the "
              + "default configuration file for package \""
              + packageName
              + "\"");
    ...
    ms_logger.info(
        "Finished generating  default configuration file for " + "package \"" + packageName + "\"");
