...
public class PSDefaultConfigGenerator {
    ...
    public String generateDefaultConfig(String publisherName,
        ...
        // Log the errors if any and pass the error messages to normalizer to put in the output xml file.
        List<String> errors = null;
        if (!propertyDefs.getSecond().isEmpty()) {
            errors = new ArrayList<>();
            msLogger.error("The following errors occurred while generating the default configuration file for package \"{}\"", packageName);
            for (var exception : propertyDefs.getSecond()) {
                msLogger.error(exception);
                errors.add(exception.getLocalizedMessage());
            }
        }
        defaultConfig = cfgDeNormalizer.getDeNormalizedXml(propertyDefs.getFirst(), errors, publisherName, publisherPrefix, solutionName);
