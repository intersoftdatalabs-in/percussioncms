...
public class PSContentTypeSetter extends PSSimplePropertySetter {
  ...
  protected boolean applyProperty(
    ...
    if (!(obj instanceof PSItemDefinition)) {
      throw new IllegalArgumentException("obj type must be PSItemDefinition.");
    }
    var itemDef = (PSItemDefinition) obj;
    switch (propName) {
      ...
      case TEMPLATES:
        setListAssociation(
            aSets, IPSAssociationSet.AssociationType.CONTENTTYPE_TEMPLATE, propValue);
        break;
      case WORKFLOWS:
        setListAssociation(
            aSets, IPSAssociationSet.AssociationType.CONTENTTYPE_WORKFLOW, propValue);
        break;
      case FIELDS:
  ...
  protected boolean deApplyProperty(
    ...
    if (FIELDS.equals(propName)) {
      var fieldMap = convertObjectToMap(propValue);
  ...
  protected Object getPropertyValue(Object obj, String propName) throws PSNotFoundException {
    ...
    switch (propName) {
      case DEFAULT_WORKFLOW:
        var wfSrv = PSWorkflowServiceLocator.getWorkflowService();
        var id = new PSGuid(PSTypeEnum.WORKFLOW, itemDef.getWorkflowId());
        var wkflow = wfSrv.loadWorkflow(id);
        return wkflow.getName();
      case ICON_VALUE:
        var iconValue = itemDef.getContentEditor().getIconValue();
        return StringUtils.isBlank(iconValue) ? "" : iconValue;
      case TEMPLATES:
        var nodeDef = PSContentTypeHelper.findNodeDef(itemDef.getGuid());
        if (nodeDef == null) return null;
        var names = new ArrayList<String>();
        for (var tid : nodeDef.getVariantGuids()) {
          var name = getTemplateName(tid);
          if (name != null) names.add(name);
        }
        return names;
      case WORKFLOWS:
        var nodeDef2 = PSContentTypeHelper.findNodeDef(itemDef.getGuid());
        if (nodeDef2 == null) return null;
        var wfNames = new ArrayList<String>();
        for (var wid : nodeDef2.getWorkflowGuids()) {
          var wfSrv2 = PSWorkflowServiceLocator.getWorkflowService();
          var wkflow2 = wfSrv2.loadWorkflow(wid);
          wfNames.add(wkflow2.getName());
        }
        return wfNames;
      case FIELDS:
        var fields = itemDef.getMappedParentFields();
   ...
   *
   * @param itemDef the item definition that contains the fields, assumed not <code>null</code>.
   * @param fnames the collection of field names, assumed not <code>null</code> but may be empty.
   */
  private void deleteShareSystemFields(PSItemDefinition itemDef, Collection<String> fnames) {
    ...
    for (String name : fnames) {
      if (!itemDef.isFieldExists(name)) continue;
   ...
   *
   * @param itemDef the item definition that may contain the field, assumed not <code>null</code>.
   ...
   */
  private void removeField(PSItemDefinition itemDef, String fieldName, int type) {
    if (type == PSField.TYPE_LOCAL) return;
    ...
    if (!itemDef.isFieldExists(justName)) return;
    ...
    if (type == PSField.TYPE_SYSTEM) {
      ...
      if (fdSet != null) fdSet.remove(justName);
    }
    itemDef.getContentEditorMapper().getUIDefinition().getDisplayMapper().removeMapping(justName);
  ...
  /**
   * Adds a shared field to the shared includes into the given item definition. However, if all
   * fields of the shared group are in the excludes, then remove the shared group and its names from
   * the item definition.
   *
  ...
  private void addFieldToSharedExcludes(PSItemDefinition itemDef, String fieldName) {
    ...
    if (containAllSharedFields(shGrp, names)) removeSharedGroup(itemDef, shGrp);
    else itemDef.getContentEditorMapper().setSharedFieldExcludes(names);
  }

  /**
   * Determines if the specified field names are the complete set of a given shared group.
   *
   ...
   * @return <code>true</code> if the field names are the complete set of the specified shared
   *     group.
   */
  private boolean containAllSharedFields(PSSharedFieldGroup shGrp, List<String> names) {
   ...
   */
  private void removeSharedGroup(PSItemDefinition itemDef, PSSharedFieldGroup shGrp) {
    PSContentEditorMapper mapper = itemDef.getContentEditorMapper();

    // remove all field names
    ArrayList<String> shNames = iteratorToList(shGrp.getFieldSet().getNames());
    ArrayList<String> excludes = iteratorToList(mapper.getSharedFieldExcludes());
    excludes.removeAll(shNames);
    mapper.setSharedFieldExcludes(excludes);

    // remove the shared group
    ArrayList<String> shGrpNames = iteratorToList(mapper.getSharedFieldIncludes());
  ...
  /**
   * Gets the field names that exist only in previous applied properties, but not in current fields.
   *
   * @param curNames current field names, assumed not <code>null</code>, may be empty.
   * @return a collection of field names, never <code>null</code>, may be empty.
   */
   ...
   *
   * @param fields the value of the {@link #FIELDS} property, it may be <code>null</code> or empty.
  ...
  private Map<String, Map<String, Object>> convertObjectToMap(Object fields) {
    ...
    if (!(fields instanceof Map))
      throw new PSConfigException(
          "The type of \"" + FIELDS + "\" property (in PSContentTypeSetter) must be Map");
  ...
  private void setIconValue(PSItemDefinition itemDef, String value) {
    if (StringUtils.isBlank(value)) {
      itemDef.getContentEditor().setContentTypeIcon(PSContentEditor.ICON_SOURCE_NONE, "");
      return;
    }

    // is there a field name that matches the value
    if (itemDef.isFieldExists(value)) {
      itemDef.getContentEditor().setContentTypeIcon(PSContentEditor.ICON_SOURCE_FROMFILEEXT, value);
    } else {
      itemDef.getContentEditor().setContentTypeIcon(PSContentEditor.ICON_SOURCE_SPECIFIED, value);
    }
  }
   ...
   *
   * @param itemDef the Content Type, assumed not <code>null</code>.
   * @param wfName the name of the default workflow, may not be <code>null</code> or empty.
   */
  private void setDefaultWorkflow(PSItemDefinition itemDef, String wfName) {
    if (StringUtils.isBlank(wfName))
      throw new IllegalArgumentException("Default Workflow name must not be null or empty.");
    ...
    if (sum.isEmpty()) throw new PSConfigException("Cannot find Workflow name \"" + wfName + "\"");
  ...
  /**
   * The property name for a list of to be added or existing fields and its properties. The value of
   * the property is a {@link Map Map&lt;String, Map&lt;String,String>>}.
   */
  ...
  /**
   * The property name for icon value. This will set both "iconValue" and "iconSource" properties on
   * the Content Type {@link PSContentEditor}.
   */
  public static final String ICON_VALUE = "iconValue";

  /** The property name for a list of templates that are associated with the Content Type. */
  public static final String TEMPLATES = "templates";

  /** The property name for a list of workflows that are associated with the Content Type. */
