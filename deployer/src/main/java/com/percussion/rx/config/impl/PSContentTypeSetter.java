...
public class PSContentTypeSetter extends PSSimplePropertySetter {
  ...
  protected boolean applyProperty(
    ...
    if (DEFAULT_WORKFLOW.equals(propName)) {
      ...
      setListAssociation(aSets, IPSAssociationSet.AssociationType.CONTENTTYPE_TEMPLATE, propValue);
    } else if (WORKFLOWS.equals(propName)) {
      setListAssociation(aSets, IPSAssociationSet.AssociationType.CONTENTTYPE_WORKFLOW, propValue);
  ...
  protected Object getPropertyValue(Object obj, String propName) throws PSNotFoundException {
    ...
    if (DEFAULT_WORKFLOW.equals(propName)) {
      ...
      if (StringUtils.isBlank(iconValue)) return "";
      else return iconValue;
    } else if (TEMPLATES.equals(propName)) {
      PSNodeDefinition nodeDef = PSContentTypeHelper.findNodeDef(itemDef.getGuid());
