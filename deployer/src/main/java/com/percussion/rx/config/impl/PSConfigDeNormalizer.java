...
/**
 * Creates default configuration document that conforms to <code>localConfig.xsd</code> schema and
 * returns it as string. This is a singleton class.
 */
public class PSConfigDeNormalizer {
  ...
  public static PSConfigDeNormalizer getInstance() {
    if (ms_instance == null) ms_instance = new PSConfigDeNormalizer();
    return ms_instance;
  }
  ...
  /**
   * Creates default configuration document with the supplied publisher prefix, publisher name and
   * solution names. Adds the properties as per the <code>localConfig.xsd</code> schema.
   *
   * @param props The properties map from which the default config property fragments needs to be
   *     created, must not be <code>null</code>, may be empty.
   * @param errors List of error strings that will be written to the output xml as comments, may be
   *     <code>null</code>.
   ...
   */
  public String getDeNormalizedXml(
    ...
    Document configDoc =
        PSXmlDocumentBuilder.createXmlDocument(ELEM_SOLUTION_CONFIGURATIONS, null, null);
    ...
    for (String name : sortedProps.keySet()) {
      hasFixMe =
          hasFixMe
              | handleProperties(configDoc, solConfig, prefix, name, sortedProps.get(name), true);
    }
    if (hasFixMe) {
      if (errors == null) errors = new ArrayList<>(1);
  ...
  /**
   * Add comment element to the document. This will include generated date and any errors that
   * occurred.
   *
  ...
  /**
   * Recursively handle transforming the passed in properties into the appropriate document
   * elements. Only top level property names are supposed to be fixed and if this method gets called
   * recursively then the local calls must pass <code>false</code> for nameTobeFixed parameter.
   *
   * @param doc assumed not <code>null</code>.
   * @param parent the parent to which elements will be appended to. Assumed not <code>null</code>.
   * @param name may be <code>null</code>.
   * @param prop assumed not <code>null</code>.
   * @param nameTobeFixed, a boolean flag to indicate whether the name to be fixed or not. If false,
  ...
  /**
   * Fix the name by removing prefix and adding a Fix me note if the prefix is invalid.
   *
   * @param name
   * @param prefix
   * @return
  ...
  /**
   * Sort a map in alpha ascending order. Passes back an underlying <code>TreeMap</code> to maintain
   * sort order.
   *
  ...
  private static final String ERROR_FIXME =
      "One or more name attributes have invalid prefixes. "
