...
/**
 * Implementation of configure handler for design objects.
 * Walks through all property setters and calls applyProperties on each setter.
 *
 * @author bjoginipally
 */
public class PSObjectConfigHandler implements IPSConfigHandler {
    ...
    public boolean equals(Object otherObj) {
        ...
        return new EqualsBuilder().append(m_name, other.m_name)
                .append(m_setters, other.m_setters).isEquals();
    }
    ...
    @Override
    public List<PSPair<Object, ObjectState>> getDesignObjects(Map<String, Object> cachedObjs) throws PSNotFoundException {
    ...
    /**
     * Gets the object names along with their related state.
     * Only called when isGetDesignObjects() returns true.
     */
    ...
    @Override
    public IPSGuid saveResult(IPSDesignModel model, Object obj, ObjectState state, List<IPSAssociationSet> assocList) throws PSNotFoundException {
     ...
     */
    protected List<PSConfigValidation> validate(String name, ObjectState state, List<IPSPropertySetter> oSetters) {
        var result = new ArrayList<PSConfigValidation>();
    ...
    /**
     * Gets the object names (and their state) that are defined in both the current and the specified handler.
     */
