...
/**
 * This implementation of configure handler is for configuring design objects. Walks through all the
 * property setters and calls <code>applyProperties</code> on each setter.
 *
 * @author bjoginipally
 */
public class PSObjectConfigHandler implements IPSConfigHandler {
  ...
  public List<PSPair<Object, ObjectState>> getDesignObjects(
      @SuppressWarnings("unused") Map<String, Object> cachedObjs) throws PSNotFoundException {
    throw new UnsupportedOperationException("getDesignObjects() methed is not supported.");
  }
  ...
  public Object getDefaultDesignObject(Map<String, Object> cachedObjs) throws PSNotFoundException {
    throw new UnsupportedOperationException("getDefaultDesignObject() methed is not supported.");
  }
  ...
  public List<PSPair<String, ObjectState>> getObjectNames() {
    ...
    PSPair<String, ObjectState> pair = new PSPair<>(getName(), ObjectState.BOTH);
    return Collections.singletonList(pair);
  }

  /**
   * Gets the object names along with its related state. This is only called when {@link
   * #isGetDesignObjects()} returns <code>true</code>.
   *
   * <p>Note, the derived class must override {@link #getCurNames()} and {@link #getPrevNames()}
   * methods if the derived class implements this method.
   *
   * @return the list of name/state pairs, never <code>null</code>, but may be empty.
   */
  private List<PSPair<String, ObjectState>> getObjectNamesFromHandlerImpl() {
    ...
    if (prevNames == null || prevNames.isEmpty()) {
      for (String name : curNames) result.add(new PSPair<>(name, ObjectState.CURRENT));
      return result;
    }
    ...
    for (String name : names) result.add(new PSPair<>(name, ObjectState.BOTH));

    // get object names in current only
    names.clear();
    names.addAll(curNames);
    names.removeAll(prevNames);
    for (String name : names) result.add(new PSPair<>(name, ObjectState.CURRENT));

    // get object names in previous only
    names.clear();
    names.addAll(prevNames);
    names.removeAll(curNames);
    for (String name : names) result.add(new PSPair<>(name, ObjectState.PREVIOUS));

    return result;
  }

  /**
   * Gets the name of the design object from current properties. This is used by {@link
   * #getObjectNames()}.
   *
   * @return the names, never <code>null</code>, but may be empty.
   */
  protected Collection<String> getCurNames() {
    throw new UnsupportedOperationException("getCurNames() methed is not supported.");
  }

  /**
   * Gets the name of the design object from previous properties. This is used by {@link
   * #getObjectNames()}.
   *
   * @return the names, never <code>null</code>, but may be empty.
   */
  protected Collection<String> getPrevNames() {
    throw new UnsupportedOperationException("getPrevNames() methed is not supported.");
  }
   ...
   * //see base class method for details
   */
  public void setExtraProperties(@SuppressWarnings("unused") Map<String, Object> props) {
    throw new UnsupportedOperationException("setExtraProperties() methed is not supported.");
  }
  ...
  public List<PSConfigValidation> validate(IPSConfigHandler other) {
    ...
    for (PSPair<String, ObjectState> pair : commonNames) {
      subResult = validate(pair.getFirst(), pair.getSecond(), other.getPropertySetters());
   ...
   * The properties of the specified setters have already applied to the given design object.
   *
   * @param name the name of the design object, never <code>null</code> or empty.
   * @param state the state of the design object, default to {@link ObjectState#BOTH} if unknown.
   * @param oSetters a list of setters with their properties that have already applied to the design
   *     object, never <code>null</code>, may be empty.
   * @return the validated result. It never <code>null</code>, but may be empty if there is no error
   *     or warning.
   */
  protected List<PSConfigValidation> validate(
      String name, IPSConfigHandler.ObjectState state, List<IPSPropertySetter> oSetters) {
    ...
    for (PSConfigValidation v : result) v.setObjectType(getType());

    return result;
  }

  /**
   * Gets the object names (and its state) that are defined in both the current and the specified
   * handler.
   *
   * @param other the specified handler, assumed not <code>null</code>.
   * @return the common name/state pairs. It may be empty if the type of the handlers is not the
   *     same or not defined, or there is no common names. The state of the object is from the
   *     current handler.
   */
  protected List<PSPair<String, ObjectState>> getCommonObjectNames(IPSConfigHandler other) {
    ...
    for (String name : names) {
      PSPair<String, ObjectState> pair = new PSPair<>(name, myNames.get(name));
      result.add(pair);
    }
    return result;
  }
   ...
   * @return the name/state pairs, it may be empty if the type and/or name property are not defined
   *     in the handler, never <code>null</code>.
   */
  ...
  /**
   * The type of the design object. It may be <code>null</code> if there is no design object for
   * this handler.
   */
  private PSTypeEnum m_typeEnum;

  /**
   * Name of the design object, will be <code>null</code> if it has not been set through a property
   * in the config definition file on the handler bean.
   */
  ...
  /**
   * The handler specific properties used in previous configuration. It may be <code>null</code> if
