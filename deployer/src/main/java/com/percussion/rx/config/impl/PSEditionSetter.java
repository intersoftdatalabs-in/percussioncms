...
public class PSEditionSetter extends PSSimplePropertySetter {
  ...
  protected Object getPropertyValue(Object obj, String propName) throws PSNotFoundException {
    PSEditionWrapper wrapper = (PSEditionWrapper) obj;
    if (PRE_TASKS.equals(propName)) {
      if (wrapper.getPreTasks().isEmpty()) return Collections.emptyList();
      else return convertListTaskToListMap(wrapper.getPreTasks());
    } else if (POST_TASKS.equals(propName)) {
      if (wrapper.getPostTasks().isEmpty()) return Collections.emptyList();
      else return convertListTaskToListMap(wrapper.getPostTasks());
    }
  ...
  private void setTasks(PSEditionWrapper wrapper, Object propValue, boolean isPreTasks) {
    ...
    if (isPreTasks) Collections.reverse(srcTasks);
    ...
    if (isPreTasks) wrapper.setPreTasks(tasks);
    else wrapper.setPostTasks(tasks);
  }
