...
public interface IPSConfigStatusMgr {
   ...
   *
   * @param nameFilter The pattern that identifies those objects you want returned. SQL-like
   *     wildcards (%) may be used. Never <code>null</code> or empty.
   * @return All PkgInfo objects whose names match the supplied filter (case-insensitive.) A <code>
   *     List</code> is returned which is sorted first by name (ascending) and then by install date
   *     (descending). Never <code>null</code>, may be empty.
   */
  public List<PSConfigStatus> findConfigStatus(String nameFilter);

  /**
   * Find the latest PSConfigStatus objects whose name matches the supplied filter.
   *
   * @param nameFilter The pattern that identifies those objects you want returned. SQL-like
   *     wildcards (%) may be used. Never <code>null</code> or empty.
   * @return The latest of each set of PSConfigStatus objects whose name matches the supplied filter
   *     (case-insensitive.) A <code>List</code> is returned which is sorted first by name
   *     (ascending) and then by date applied(descending). Never <code>null</code>, may be empty.
   */
   ...
   *
   * @param nameFilter must not be <code>null</code>. SQL-like wildcards (%) may be used. Never
   *     <code>null</code> or empty.
   */
  public void deleteConfigStatus(String nameFilter);

  /**
   * Find the last successful PSConfigStatus object whose name matches the supplied name.
   *
   * @param configName The name of the configuration for which the last successful configuration is
   *     needed.
   * @return The last successful configuration of the supplied configuration name, may be <code>null
   *     </code> if no successful configuration found.
   */
