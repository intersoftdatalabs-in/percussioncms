...
public class PSContentTypeFieldSetter extends PSSimplePropertySetter {
  /**
   * Default constructor, invoked by Spring framework. This can only used for an existing field.
   * Must call {@link #setFieldName(String)} before calling .
   */
  public PSContentTypeFieldSetter() {}

  /**
   * Creates an instance of the field setter with the given item definition and field name. This can
   * be used to add a system or shared field to the Content Type. However, this cannot be used to
   * add a local field to the Content Type.
   *
   * @param itemDef the item definition, may not be <code>null</code>.
   * @param fieldName the field name, which may be an existing field in the item definition or an
   *     system/shared field to be added into the item definition.
   */
  public PSContentTypeFieldSetter(PSItemDefinition itemDef, String fieldName) {
    if (!itemDef.isFieldExists(fieldName)) addField(itemDef, fieldName);
  ...
  /**
   * Convenience method, calls {@link #validateFieldAndConfigProperty(PSItemDefinition, String,
   * String) validateFieldAndConfigProperty(itemDef, m_fieldName, propName)}
   *
   * @param obj the object in question, not <code>null</code>, must be an instance of {@link
   *     PSItemDefinition}.
  ...
  private void validateObjAndPropertyName(Object obj, String propName) {
    if (!(obj instanceof PSItemDefinition))
      throw new IllegalArgumentException("obj type must be an instance of PSItemDefinition.");
  ...
  protected Object getPropertyValue(Object obj, String propName) throws PSNotFoundException {
    ...
    if (LABEL.equals(propName)) {
      ...
      PSPair<PSDisplayMapping, Integer> pair = mapper.getMappingAndSequence(m_fieldName);
      return pair == null ? null : pair.getSecond();
    } else if (REQUIRED.equals(propName)) {
      PSField fd = getField(itemDef);
      return PSContentTypeUtils.hasRequiredRule(fd) ? Boolean.TRUE : Boolean.FALSE;
    }

    return super.getPropertyValue(obj, propName);
  }

  private List<PSPair<String, String>> getControlParams(PSItemDefinition itemDef) {
    PSControlRef control = getControl(itemDef).getFirst();
    List<PSPair<String, String>> result = new ArrayList<>();
    List<PSParam> params = (List<PSParam>) IteratorUtils.toList(control.getParameters());
    PSPair<String, String> pair;
    for (PSParam p : params) {
      pair = new PSPair<>(p.getName(), p.getValue().getValueText());
      result.add(pair);
    }
    return result;
  }
  ...
  /**
   * Validates the existence of a parent or simple child field in the given item definition.
   *
   * @param itemDef the item definition, assumed not <code>null</code>.
   * @param fieldName the name of the field in question, assumed not <code>null</code> or empty.
   * @param configPropertyName the name of the to be configured property, assumed not <code>null
   *     </code> or empty.
   */
  private void validateFieldAndConfigProperty(
      PSItemDefinition itemDef, String fieldName, String configPropertyName) {
    if (!itemDef.isFieldExists(fieldName))
      throw new PSConfigException(
          "Cannot find field \"" + fieldName + "\" in Content Type \"" + itemDef.getName() + "\".");
  ...
  /**
   * Validates a field, disallow to set the {@link #REQUIRED} property to <code>false</code> (or
   * removing required rule) for a system/shared field which already has a required rule.
   *
  ...
  private void validateRequiredProperty(PSField field, boolean isRequired) {
    if (field.getType() == PSField.TYPE_LOCAL) return;

    if ((!PSContentTypeUtils.hasRequiredRule(field))
        || (PSContentTypeUtils.hasRequiredRule(field) && isRequired)) return;

    throw new PSConfigException(
        "The property \""
            + REQUIRED
            + "\" cannot be configured for a "
            + PSField.TYPE_ENUM[field.getType()]
            + " field \""
            + field.getSubmitName()
   ...
   *
   * @param itemDef the item definition of the Content Type, assumed not <code>null</code>.
   * @param fieldName the field name of the added field, assumed not <code>null</code> or empty.
   */
  ...
  /**
   * Updates the includes and excludes of the shared group for the given item definition and the
   * shared group. It adds the shared group to both includes and excludes list if the given shared
   * group does not exist in the item definition; otherwise do nothing.
   *
   * @param itemDef the item definition, assumed not <code>null</code>.
  ...
  private void updateSharedGroup(PSItemDefinition itemDef, String fieldName) {
    ...
    if (names.contains(shGroup.getName())) return;
   ...
   *
   * @param itemDef the item definition, assumed not <code>null</code>.
   * @param fieldName the name of the created field, assumed not <code>null</code> or empty.
   * @param type it is either {@link PSField#TYPE_SHARED} or {@link PSField#TYPE_SYSTEM}.
   * @param shGroup the shared group, assumed not <code>null</code> if the type is {@link
   *     PSField#TYPE_SHARED}; otherwise it may be <code>null</code>.
   * @return the created field, never <code>null</code>.
   */
  private PSField addFieldToFieldSet(
      PSItemDefinition itemDef, String fieldName, int type, PSSharedFieldGroup shGroup) {
    ...
    PSFieldSet fdSet = itemDef.getContentEditorMapper().getFieldSet(shGroup.getName());
    if (fdSet != null) {
      fdSet.add(field);
      return field;
    }
  ...
  /**
   * Removes a given field name into the system or shared excludes for the given item definition. Do
   * nothing if the field name exists in the related excludes.
   *
   * @param itemDef the item definition, assumed not <code>null</code>.
  ...
  private void removeFieldFromExcludes(PSItemDefinition itemDef, String fieldName, int type) {
    ...
    else excludes = itemDef.getContentEditorMapper().getSharedFieldExcludes();
    ...
    if (found_name != null) names.remove(found_name);
    if (type == PSField.TYPE_SYSTEM) itemDef.getContentEditorMapper().setSystemFieldExcludes(names);
    else itemDef.getContentEditorMapper().setSharedFieldExcludes(names);
  }

  /**
   * Gets the display mapping of a specified system field.
   *
   * @param fieldName the field name in question, may be <code>null</code> or empty.
   * @return the display mapping one of the system field. It may be <code>null</code> if cannot find
   *     a system field with the name.
   */
  private PSDisplayMapping getSystemFieldMapping(String fieldName) {
    ...
    try {
      ...
      throw new PSConfigException("Failed to catalog system field definition.", e);
    }
  ...
  /**
   * Gets the field with the name of {@link #m_fieldName}. The field will be added into the item
   * definition if it does not exist.
   *
   * @param itemDef the item definition that contains the field, assumed not <code>null</code>.
   * @return the field with the name, never <code>null</code> or empty.
   */
  private PSField getField(PSItemDefinition itemDef) {
    if (StringUtils.isBlank(m_fieldName))
      throw new IllegalStateException("m_fieldName must not be null or empty.");
   ...
   *
   * @param itemDef the item definition that contains a field with the name of {@link #m_fieldName}
  ...
  /**
   * Gets the UISet of the current field. The field may be a parent or simple child field.
   *
   * @param itemDef the item definition that contains the field, assumed not <code>null</code>.
  ...
  private PSUISet getFieldUISet(PSItemDefinition itemDef) {
    ...
    if (uiset == null)
      throw new PSConfigException("Cannot find UISet for field \"" + m_fieldName + "\"");
    return uiset;
  }
   ...
   *
   * @param itemDef the item definition in question, assumed not <code>null</code>.
   * @return the 1st value is the control and the 2nd value is the UI set, never <code>null</code>.
   */
   ...
   *
   * @param itemDef the item definition that contains the current field. Assumed not <code>null
   *     </code>.
  ...
  private void setControlParameters(PSItemDefinition itemDef, Object params) {
    if ((!(params instanceof Map)) && (!(params instanceof List)))
      throw new PSConfigException(
          "The value of \"" + CONTROL_PARAMS + "\" property must be a \"Map\" or \"List\" type");
   ...
   * @param itemDef the item definition that contains the current field. Assumed not <code>null
   *     </code>.
   * @param value the value of the property. It may be <code>null</code> or empty.
   */
  ...
  private void setRequired(PSField fd, String value) throws PSExtensionException {
    ...
    Iterator it =
        mgr.getExtensionNames(
            null, null, com.percussion.extension.IPSFieldValidator.class.getName(), null);
  ...
  /**
   * Reposition the current field according to the {@link #SEQUENCE} property value.
   *
  ...
  private void setSequence(PSItemDefinition itemDef, String seqValue) {
    ...
    else if (sequence > mapper.size()) sequence = mapper.size();
  ...
  /**
   * The "is required" property name on a field. This property is not configurable for a system or
   * shared field.
   */
  public static final String REQUIRED = "required";

  /**
   * The property name for a list of control parameters. The supported parameter type is text
   * ({@link PSTextLiteral}) only.
   */
  public static final String CONTROL_PARAMS = "controlParameters";

  /**
   * The "Default value" property name. The type of the default value is text ({@link
   * PSTextLiteral}) only.
   */
  ...
  /**
   * The name of sequence property, which is used to control the location of the field on the
   * Content Type.
   */
  public static final String SEQUENCE = "sequence";

  /** A list of configurable property names for a system or shared field. */
  public static final String[] SYS_SH_PROPERTIES =
      new String[] {LABEL, CONTROL_PARAMS, DEFAULT_VALUE, "showInPreview", "userSearchable"};
}
