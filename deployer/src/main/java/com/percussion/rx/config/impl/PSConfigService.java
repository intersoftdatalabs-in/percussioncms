...
public class PSConfigService implements IPSConfigService {
     ...
     */
    @Override
    public List<PSPair<String, Exception>> applyConfiguration(String[] configNames, boolean deltasOnly) {
        ...
        for (var cfg : configNames) {
            if (!isValidConfiguartion(cfg)) {
                var msg = "Missing one or more configuration files for configuration {0}. Skipping configuration.";
    ...
    private PSPurgableTempFile getTempConfigDefFile(String cfgName, String configDef) throws IOException {
    ...
    @Override
    public void applyLocalConfiguration(File localConfigFile, Map<String, Object> prevProps, boolean changesOnly) {
        Objects.requireNonNull(localConfigFile, "file must not be null");
        ...
        try (var defConfIs = new FileInputStream(getConfigFile(ConfigTypes.DEFAULT_CONFIG, configName))) {
            ...
            var cfgHandlers = mapper.getResolvedHandlers(cfgDefFile.getAbsolutePath(), propsToProcess, newProps, prevProps2);
            ...
            ms_logger.error("Failed to apply config for package {} Error: {}", configName, PSExceptionUtils.getMessageForLog(e));
            saveConfigStatus(configName, status);
    ...
    @Override
    public List<PSConfigValidation> validateConfig(String cfgName) throws FileNotFoundException, JAXBException {
        ...
        try (var defIS = new FileInputStream(defaultFile)) {
            try (var localIS = new FileInputStream(localFile)) {
                ...
                var mapper = new PSConfigMapper();
                var cfgHandlers = mapper.getResolvedHandlers(cfgDefFile.getAbsolutePath(), curProps, curProps, emptyProps);
    ...
    private void validateConfig(String pkgName, List<IPSConfigHandler> hdls) {
        ...
        if (hasError)
            throw new PSConfigException("Package \"" + pkgName + "\" validation failed.");
    }
    ...
    private void updateValidateResult(String myPkgName, String tgtPkgName, List<PSConfigValidation> validates) {
    ...
    private List<IPSConfigHandler> getConfigHandlers(String configName) {
        ...
        try (var cfgDefFile = getTempConfigDefFile(configName, cfgDef)) {
            ...
            ms_logger.error("Failed to get handlers from last success configuration of package {} Error: {}", configName, PSExceptionUtils.getMessageForLog(e));
        }
        return Collections.emptyList();
    }
    ...
    public void deApplyConfiguration(String cfgName) {
        ...
        try (var cfgFile = getTempConfigDefFile(cfgName, configDef)) {
            ...
            ms_logger.error("Failed to revert configuration in file: {} Error: {}", cfgName, PSExceptionUtils.getMessageForLog(e));
        }
    }

    @Override
    public void deApplyConfiguration(String configName, String configDefPath, InputStream defaultCfg, InputStream localCfg) throws PSNotFoundException {
        ...
        try {
            ...
            ms_logger.error("Failed to revert config for package {}. Error: {}", configName, PSExceptionUtils.getMessageForLog(e));
            saveConfigStatus(configName, status);
            throw new PSConfigException(e);
        }
    }

    private void notifyConfigChanges(Collection<IPSGuid> ids, ConfigStatus status) throws PSNotFoundException {
    ...
    private Map<String, Object> applyDefaultProps(Map<String, Object> localConfig, Map<String, Object> defaultConfig) {
    ...
    private PSPair<String, Map<String, Object>> getLastSuccessConfig(String configName) {
        try {
            ...
            if (sucCfg != null && StringUtils.isNotBlank(sucCfg.getConfigDef())) {
                var results = applyDefaultProps(normalizeConfig(sucCfg.getLocalConfig()), normalizeConfig(sucCfg.getDefaultConfig()));
                return new PSPair<>(sucCfg.getConfigDef(), results);
            }
        } catch (Exception e) {
            var msg = "Failed to get last successfully applied configuration for configuration({0}), full configuration will be applied.";
    ...
    private String getConfigContent(ConfigTypes type, String pkgName) {
        ...
        if (file.exists()) {
            try {
                ...
                var msg = "Failed to read the configuration file \"" + file.getAbsolutePath() + "\" for package ({0})";
    ...
    @Override
    public void saveCommunityVisibility(Collection<String> communities, String pkgName, boolean isReplace) {
        var f = getConfigFile(ConfigTypes.VISIBILITY, pkgName);
