 ...
 * normalized properties. The configuration files are in XML and defined by the XMSschema file<code>
 * localConfig.xsd</code>.
 */
public class PSConfigNormalizer {
  /**
   * Normalizes the supplied configuration file.
   *
   * @param in Input stream corresponding to the config file. Must not be <code>null</code> assumes
   *     the caller closes the stream.
   * @return Map of name value pairs of normalized properties, may be empty never <code>null</code>.
  ...
  /**
   * This is the same as {@link #getNormalizedMap(InputStream)}, except it has optional to include
   * the fully qualified names for all the map values.
   *
   * @param in Input stream corresponding to the config file. Must not be <code>null</code> assumes
   *     the caller closes the stream.
   * @param resolveValueMap <code>true</code> if the returned map includes the fully qualified names
   *     for all the map values.
   * @return Map of name value pairs of normalized properties, may be empty never <code>null</code>.
   * @throws JAXBException if error occurs.
   */
  ...
  private void appendFQNames(
    ...
    for (String k : srcMap.keySet()) {
      ...
      if (prefix != null) tgtMap.put(key, value);
   ...
   *
   * @param is Input stream corresponding to the config file. Must not be <code>null</code> assumes
   *     the caller closes the stream.
  ...
  private SolutionConfigurations getSolutionConfigurations(InputStream is) throws JAXBException {
    JAXBContext jc = JAXBContext.newInstance("com.percussion.rx.config.impl.jaxb");
    Unmarshaller unmarshaller = jc.createUnmarshaller();

    SolutionConfigurations collection = (SolutionConfigurations) unmarshaller.unmarshal(is);
   ...
   *
   * @param sc The JAXB Collection of "Solution Configurations". This is represented by the <code>
   *     <SolutionConfiguration></code> tag in the XML configuration. Within that tag, there can be
   *     multiple <code><SolutionConfig></code> tags, each one representing a different solution
   *     (configurable entity of the Percussion CM System. May be empty, never <code>null</code>.
   * @return Map of name value pairs of normalized properties, may be empty never <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  private Map<String, Object> solConfToNormMap(SolutionConfigurations sc) {
    ...
    for (SolutionConfig solConf : solConfList) {
      ...
      String prefix = sc.getPublisherPrefix().trim() + "." + solConf.getName().trim();
   ...
   *     should be stored. May not be <code>null</code>, but may be empty.
   * @param map The Map into which to store the Property data. May be <code>null</code>.
   ...
   * @param propOrPropSetList the sibling (include itself) lists, assumed not <code>null</code>.
   * @param prefix the prefix used to create map key, may be <code>null</code> or empty.
  ...
  private boolean isValidPropertySet(
    ...
    Boolean hasSiblingProperties = checkForSiblingProperties(propOrPropSetList);
    if (psName == null) {
      if (m_tagLevel <= 1) {
        ms_log.warn(
            "PropertySet ["
   ...
   * the provided property map. A map will be created if one is not provided. A PropertySet tag can
   * contain:
   *
   * <ul>
   *   <li>Property tags
   ...
   * </ul>
   *
   * Method recurses when a Property contains one or more PropertySets until it reaches the leaf
   * node.
   *
   * <p>PropertySets containing only PropertySets are treated specially:
   *
   * <ol>
   *   <li>the names of the child PropertySets are ignored.
   *   <li>a List of Maps is created instead of a Map of Maps
   * </ol>
   *
   * <p>
   *
   * @param propertySet The PropertySet tag object being parsed. May not be <code>null</code>.
   * @param prefix The property key under which this property should be stored. May be <code>null
   *     </code> or empty.
   * @param map The Map into which to store the Property data. May be <code>null</code>. If <code>
   *     null</code>, a map will be created and returned. This allows a local copy of a map to be
   *     created and used as user sees fit.
  ...
  private Map<String, Object> processPropertySet(
    ...
    if (map == null) returnMap = new HashMap<>();
    else returnMap = map;
   ...
   * provided property map. A map will be created if one is not provided. Handles the element types:
   *
   * <ul>
   *   <li>Simple Property
   *   <li>Property Value List
   *   <li>Property Value Pairs List
   *   <li>PropertySet
   * </ul>
   *
   * Method recurses when a Property contains one or more PropertySets until it reaches the leaf
   * node.
   *
   * <p>Properties containing only PropertySets are treated specially:
   *
   * <ol>
   *   <li>a Map if all propertySet elements have the name attribute.
   *   <li>a List of Maps if one of the propertySet does not have name attribute
   * </ol>
   *
   * <p>
   *
   * @param prop The Property tag object being parsed. May not be <code>null</code>.
   * @param prefix The prefix of the property key under which this property should be stored. May be
  ...
  private Map<String, Object> processProperty(
    ...
    //  A PropertySet
      ...
      for (PropertySet ps : propertySetList) objectList.add(ps);
   ...
   *
   * @param propSetList the value (or child components) of a propertySet, assumed not <code>null
   *     </code> or empty.
   ...
   * @return a list of {@link String} or {@link PSPair}. It may be <code>null</code> if the list is
   *     empty.
   */
  ...
  private List<PSPair<String, String>> getPairList(List<Pair> pairList) {
    List<PSPair<String, String>> newPairs = new ArrayList<>();
    for (Pair xpair : pairList) {
      PSPair<String, String> npair = new PSPair<>();
   ...
   *
   * @param siblingList List of sibling objects to check. May not be <code>null</code>, but may be
   *     empty.
   ...
   *
   * @param objectList Objects to test for "PropertySet"-ness. May not be <code>null</code>, but may
   *     be empty.
   ...
   * given list. This method is called when all of the children of PropertySet are themselves
   * PropertySets.
   *
   * @param childPropSetList The list for PropertySets. Each of the child Properties or PropertySets
   *     in this list should be added to the resulting list.
  ...
  private List<Object> createPropSetNameList(List<Object> childPropSetList) {
    ...
    for (Object childPropSet : childPropSetList) {
      ...
      List<Object> propSetOrPropList = ((PropertySet) childPropSet).getPropertySetOrProperty();

      Map<String, Object> propSetOrPropMap = new HashMap<>();
      propSetOrPropMap = processPropertyOrPropertySetList(propSetOrPropList, null, null);
  ...
  /**
   * The "tag level" is the embedded depth at which the tag occurs. It is used to determine whether
   * the "propertySet" tag is occurring at the first level of the configuration file. If a
   * "propertySet" tag has embedded "propertySet" tags, the embedded ones' name attributes are
   * optional and all properties of the embedded "propertySet" tags must be coalleced into a List of
   * Maps (instead of a Map of Maps.
   */
