 ...
 * and defined by the XML schema file <code>localConfig.xsd</code>.
 */
public class PSConfigNormalizer {
    /**
     * Normalizes the supplied configuration file.
     *
     * @param in Input stream corresponding to the config file. Must not be null. Assumes the caller closes the stream.
     ...
     * option to include the fully qualified names for all the map values.
     *
     * @param in Input stream corresponding to the config file. Must not be null. Assumes the caller closes the stream.
     * @param resolveValueMap true if the returned map includes the fully qualified names for all the map values.
     * @return Map of name-value pairs of normalized properties, may be empty but never null.
     * @throws JAXBException if error occurs.
     */
    public Map<String, Object> getNormalizedMap(InputStream in, boolean resolveValueMap) throws JAXBException {
    ...
    private void appendFQNames(Map<String, Object> tgtMap, String prefix, Map<String, Object> srcMap) {
     ...
     *
     * @param is Input stream corresponding to the config file. Must not be null. Assumes the caller closes the stream.
     ...
     * @param prefix The prefix of the property key under which these Property or PropertySet elements should be stored. May not be null, but may be empty.
     ...
     */
    private boolean isValidPropertySet(PropertySet propSet, List<Object> propOrPropSetList, String prefix) {
        var psName = propSet.getName();
        var hasSiblingProperties = checkForSiblingProperties(propOrPropSetList);
        if (psName == null) {
            if (m_tagLevel <= 1) {
                ms_log.warn("PropertySet [{}] at first tag level must have a name. This PropertySet has been ignored.", prefix);
                return false;
            }
            if (hasSiblingProperties) {
                ms_log.warn("PropertySet [{}] whose siblings are Properties must have a name. This PropertySet has been ignored.", prefix);
                return false;
            }
        }
        return true;
    ...
    /**
     * Process a PropertySet element of the Deployer configuration file. Adds the elements found to the provided property map.
     */
    private Map<String, Object> processPropertySet(PropertySet propertySet, String prefix, Map<String, Object> map) {
    ...
    /**
     * Process a Property element of the Deployer configuration file. Adds the elements found to the provided property map.
     */
    private Map<String, Object> processProperty(Property prop, String prefix, Map<String, Object> map) {
    ...
    /**
     * Converts the {@link Property.Pvalues} object. It may contain a list of pvalue or a list of pairs.
     */
    ...
    /**
     * Creates a List of Maps of each of the (child) properties in each of the PropertySets in the given list.
     */
     ...
     */
    private static final Logger ms_log = LogManager.getLogger("com.percussion.rx.config.impl.PSConfigNormalizer");
}
