...
/**
 * Setter for the relationship configuration effects. As it is a distributed property and as well
 * the collection, the new effects are added amd effects from previous properties are removed.
 *
 * @author bjoginipally
 */
public class PSRelationshipConfigEffectSetter extends PSPropertySetterWithValidation {
  ...
  protected boolean deApplyProperty(
    ...
    PSRelationshipConfig relConfig = (PSRelationshipConfig) obj;
    if (propName.equals(PROP_EFFECTS)) {
      if (propValue != null) removeEffects(relConfig, propValue);
    } else {
  ...
  protected List<PSConfigValidation> validate(
    ...
    if (curEffects.isEmpty() || otherEffects.isEmpty()) return Collections.emptyList();
    ...
    if (curEffNames.isEmpty()) return Collections.emptyList();
  ...
  /**
   * Helper method to get the list of effect names for the given list of effect objects.
   *
   * @param effects The list effects, assumed not <code>null</code>.
   * @return List of names, may be empty if the supplied list of effects is empty, never <code>null
   *     </code>.
   */
  ...
  /**
   * Sets the effects property on the configuration. If previous properties exist and if effects
   * property exists in the previous properties then removes the effects from the previous
   * properties. If new properties are not <code>null</code> then adds the new properties.
   *
   * @param relConfig The relationship configuration design object assumed not <code>null</code>.
  ...
  private Map<String, Object> getEffectPropertyDef(PSConditionalEffect effect) {
    ...
    String direction = (String) PSConfigUtils.getReverseMap(ms_directionConsts).get(endPoint);
    ...
    // Condtion prop
    List<Map<String, Object>> condDef = PSConfigUtils.getCondtionsDef(effect.getConditions());
  ...
  private void mergeEffects(
      PSRelationshipConfig relConfig, Object propValue, Object prevPropValue) {
    if (propValue == null && prevPropValue == null) return;
    Iterator iter = relConfig.getEffects();
    List<PSConditionalEffect> curEffects = new ArrayList<>();
    while (iter.hasNext()) {
      curEffects.add((PSConditionalEffect) iter.next());
    ...
    for (PSConditionalEffect effect : curEffects) {
      PSConditionalEffect mergedEffect = getCorrespondingEffect(effect, newEffects);
  ...
  /**
   * If an effect exists with the same name in the supplied effects list, then returns that effect
   * otherwise returns <code>null</code>.
   *
   * @param effect The effect that needs to be checked, assumed not <code>null</code>.
  ...
  /**
   * Creates the effects from the supplied property value corresponding to the relationship effect
   * configuration.
   *
  ...
  private List<PSConditionalEffect> createEffects(Object propValue) {
    ...
    if (propValue == null) return effects;
  ...
  /**
   * Creates the effect for the given map of properties corresponding to the configuration of an
   * effect.
   *
  ...
  private PSConditionalEffect createEffect(Map<String, Object> map) {
    ...
    PSExtensionCall extCall =
        PSConfigUtils.createExtensionCall(
            extension, extParams, "com.percussion.relationship.IPSEffect");
  ...
  /**
   * Helper method to validate the supplied String representation of end points and return their
   * corresponding integer values. Throws <code>PSConfigException</code> if the supplied list is
   * empty or any of the entry is not a valid execution context.
   *
   * @param execCtxts The execution contexts, must not be <code>null</code> or empty.
  ...
  private List<Integer> getExecutionContexts(List<String> execCtxts) {
    if (execCtxts == null || execCtxts.isEmpty()) {
      throw new PSConfigException(
          "The effect is missing required " + "property \"executionContext\".");
    }
    List<Integer> ecs = new ArrayList<>();
    for (String ec : execCtxts) {
      Integer ectx = PSConditionalEffect.getExecutionContextValueForName(ec);
      if (ectx == null) {
