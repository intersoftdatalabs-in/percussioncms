...
public class PSCommunityPackages {
  ...
  public PSCommunityPackages(List<PSCommunityPackage> packages) {
    super();
    if (packages != null) m_communitiesPackages = packages;
  }
  ...
  public String getAllPackages() {
    ...
    for (PSPkgInfo pinfo : pInfos) {
      if (pinfo.isSuccessfullyInstalled() && !pinfo.getLastAction().equals(PackageAction.UNINSTALL))
        result += pinfo.getPackageDescriptorName() + PSPackageService.NAME_SEPARATOR;
    }
  ...
  public void add(PSCommunityPackage commPkg) {
    if (commPkg == null) throw new IllegalArgumentException("pkg cannot be null.");
   ...
   *
   * @param commPkg the community package object to be removed. May be <code>null</code>.
   */
  ...
  /** The list of community package objects, never <code>null</code>, may be empty. */
