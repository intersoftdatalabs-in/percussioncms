...
public class PSPackageUninstall implements IPSPackageUninstaller {
  /**
   * Calls the deployment handler to unintsall the packages. Creates the uninstall messages with the
   * returned results from uninstallation.
   *
   * @param packageNames The {@link PSPackageService#NAME_SEPARATOR} separated list of package
   *     names.
   * @return list of uninstall messages, never <code>null</code> may be empty.
   */
  @Override
  ...
  public List<PSUninstallMessage> uninstallPackages(String packageName, boolean isRevertEntry)
    ...
    for (String pkgname : pkgNames) {
      if (StringUtils.isNotBlank(pkgname)) pkgNameList.add(pkgname);
    }
  ...
  /**
   * Helper method to convert PSUninstallResultType value to the UI consumable String. One of the
   * constants defined in Package Service.
   *
   * @param resultType assumed not <code>null</code>.
   * @return One of the constants defined in Package Service, never <code>null</code>.
   */
  ...
  /**
   * Checks whether there are any packages that depend on the supplied package and any elements from
   * the package have dependencies. Creates one message for each kind of dependencies found. The
   * message consists of the dependent object names with html line break.
   *
   * @param packageName the name of the package for which the dependencies needs to be checked.
   * @return list {@link PSUninstallMessage} messages, will be empty if there are no dependencies
   *     found.
   */
  ...
  /**
   * Finds all the dependent packages and creates a {@link PSUninstallMessage} message and adds the
   * dependent package names to the message with html break.
   *
   * @param pinfo The package info object for which the package dependencies needs to be checked,
   *     assumed not <code>null</code>.
   * @return A message corresponding to all the dependent packages, may be <code>null</code>, if no
   *     dependencies found.
   */
  private PSUninstallMessage checkPkgDependencies(PSPkgInfo pinfo) throws PSNotFoundException {
    ...
    if (!guids.isEmpty()) {
      ...
      String msgT =
          "Package ({0}) is a dependency for other packages "
              + "installed on the system  If you remove package ({1}), "
              + "these packages may not work correctly. {2}";
      Object[] args = {pinfo.getPackageDescriptorName(), pinfo.getPackageDescriptorName(), depPkgs};
  ...
  /**
   * Checks whether the elements of the supplied package have any dependencies, if yes creates a
   * {@link PSUninstallMessage} message and adds the design object names that have the dependencies
   * to the message with html break.
   *
   * @param pinfo The package info object for which the element dependencies needs to be checked,
   *     assumed not <code>null</code>.
   * @return A message corresponding to all the objects that have dependencies, may be <code>null
   *     </code>, if no dependencies found.
   */
  private PSUninstallMessage checkContentDependencies(PSPkgInfo pinfo) throws PSNotFoundException {
    ...
    if (StringUtils.isNotBlank(objNames)) {
      String msgT =
          "The package {0} includes design objects that are "
   ...
   * @return List of IPSGuids of objects of the supplied package, never <code>null</code>, may be
   *     empty.
   */
