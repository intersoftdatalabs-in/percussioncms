...
public class PSPackageServiceHelper {
    ...
    public static PSPkgUiResponse getValidationResults(String pkgName) throws PSNotFoundException {
        ...
        if (!missingPkgs.isEmpty()) {
            message.append("<b>Missing Packages</b><br/> The following ")
        ...
        if (!results.isEmpty()) {
            message.setLength(0);
            message.append("<b>Modified Design Objects</b><br/> The following objects ")
        ...
        var resp = new PSPkgUiResponse(PSPkgUiResponseType.SUCCESS,
    ...
    public static PSPkgUiResponse applyPackageVisibility(String packageNames) {
        var response = new PSPkgUiResponse(
        ...
        if (!errorPkgs.isEmpty()) {
            var msg = new StringBuilder("Failed to reapply the visibility settings for the following packages.<br/>");
    ...
    public static PSPkgUiResponse applyConfiguartion(String packageNames) {
        ...
        var pkgInfos = pkgsPair.getFirst();
        var validPkgs = pkgInfos.stream()
                .map(PSPkgInfo::getPackageDescriptorName)
                .collect(Collectors.toList());
        var cfgSrvc = PSConfigServiceLocator.getConfigService();
        var errors = cfgSrvc.applyConfiguration(validPkgs.toArray(new String[0]), false);
        if (!errors.isEmpty()) {
            var msg = new StringBuilder("Failed to reapply the configuration settings for the following packages.<br/>");
     ...
     *                 {@link PSPkgInfo} objects needs to be found, assumed not null.
     ...
     * @param commList          a list of community names. If this is null,
     *                          then apply all existing communities to the given package.
     * @param clearOtherEntries if true clears other community entries from the objects and applies the supplied communities.
     *                          Otherwise leaves the current communities as is.
     */
    public static void updatePkgCommunities(String pkgName, Collection<String> commList, boolean clearOtherEntries) {
