...
public class PSPackageServiceHelper {
  ...
  public static PSPkgUiResponse getValidationResults(String pkgName) throws PSNotFoundException {
    ...
    if (!missingPkgs.isEmpty()) {
      message =
          "<b>Missing Packages</b><br/> The following "
    ...
    if (!results.isEmpty()) {
      message =
          "<b>Modified Design Objects</b><br/> The following objects "
    ...
    List<PSConfigValidation> cfgValErrors = cfgSrvc.validateConfiguartion(pkgName);
   ...
   *
   * @param pkgInfo The object {@link PSPkgInfo} for which the dependencies needs to to be
   *     calculated.
   * @return List of the names of dependent package never <code>null</code>, may be empty.
   */
  private static List<String> getMissingPackages(PSPkgInfo pkgInfo) throws PSNotFoundException {
    ...
    IPSPkgInfoService pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
    List<IPSGuid> depGuids = pkgService.findDependentPkgGuids(pkgInfo.getGuid());
   ...
   * @return A successful {@link PSPkgUiResponse} object or failure object, never <code>null</code>.
   */
  ...
  public static PSPkgUiResponse applyConfiguartion(String packageNames) {
    ...
    IPSConfigService cfgSrvc = PSConfigServiceLocator.getConfigService();
    List<PSPair<String, Exception>> errors =
        cfgSrvc.applyConfiguration(validPkgs.toArray(new String[validPkgs.size()]), false);
    if (!errors.isEmpty()) {
      String msg =
   ...
   *     needs to be found, assumed not <code>null</code>.
   * @return PSPair of list of packages found and list of package names for which the packages are
   *     not found. The pair and lists are never <code>null</code> may be empty.
   */
  private static PSPair<List<PSPkgInfo>, List<String>> getValidPackages(String[] pkgNames) {
    ...
    PSPair<List<PSPkgInfo>, List<String>> result = new PSPair<>(pkgInfos, invalidPkgs);
  ...
  public static String getStringFromList(Collection<String> strList) {
    ...
    for (String str : strList) {
      if (result.length() > 0) result.append(PSPackageService.NAME_SEPARATOR);
      result.append(str);
    }
   ...
   * @param commList a list of community names. If this is <code>null</code>, then apply all
   *     existing communities to the given package.
   * @param clearOtherEntries if <code>true</code> clears other community entries from the objects
   *     and applies the supplied communities. Otherwise leaves the current communities as is.
   */
