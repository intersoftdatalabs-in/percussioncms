...
/**
 * Handles the visibility of packages and package elements with the communities.
 * Sunny Sal says: "Visibility is key—just like good comments!"
 *
 * @author bjoginipally
 */
public class PSPackageVisibility {
     ...
     * @return One ACL for each corresponding object id. Some of the entries may be {@code null} if the object does not have an ACL.
     * The results are in the same order as the supplied ids.
     */
     ...
     * @param clearOtherCommEntries if {@code true} clears other community entries on the design object.
     */
    public void setCommunities(IPSGuid objectGuid, Collection<String> communityNames, boolean clearOtherCommEntries) {
        if (objectGuid == null)
     ...
     * @param commsVisibility the list of community names to validate with, never {@code null}, may be empty.
     * @return {@code true} if the community visibility of the given object matches the supplied communities or does not have community visibility ACLs; {@code false} otherwise.
     */
     ...
     * @return {@code null} if successful load and set the communities; otherwise return the error message should an error occur.
     */
     ...
     * @return {@code true} if all the communities inside the supplied list are valid communities, otherwise {@code false}.
     */
    ...
    public void clearCommunity(String communityName, List<IPSGuid> objectGuids) {
        ...
        var aclServ = PSAclServiceLocator.getAclService();
        var guids = objectGuids.stream()
                .filter(this::isVisibilitySupportedType)
     ...
     * @return {@code true} if the supplied guid type supported by security list, otherwise {@code false}.
     */
    ...
    /**
     * Clears supplied community entry or all community acl entries if the communityName is null, from the supplied acl.
     *
     * @param acl           assumed not {@code null}.
     * @param communityName if {@code null} all entries are removed, if not specified community entry is removed.
     */
    ...
    public List<IPSGuid> getConvertedGuids(List<IPSGuid> guids) {
        ...
        for (var guid : guids) {
            ...
            var g = gmgr.makeGuid(Long.parseLong(guid.getUUID() + ""), PSTypeEnum.valueOf(guid.getType()));
            results.add(g);
        }
        return results;
    }
    ...
    /**
     * Set the community entries for the specified package. It sets the communities for all the elements in the given package.
     *
     * @param pkgGuid              the ID of the package, must not be {@code null}.
     * @param communities          community names to be set to, must not be {@code null} and must be a valid community list.
     * @param clearOtherCommEntries if {@code true} clears other community entries on the design object.
     */
    public void setPackageCommunities(IPSGuid pkgGuid, Collection<String> communities, boolean clearOtherCommEntries) {
        var pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
