...
/**
 * This class handles the visibility of packages and package elements with the communities.
 *
 * @author bjoginipally
 */
public class PSPackageVisibility {
   ...
   * @return Collection of community names never <code>null</code>, may be empty.
   */
  ...
  /**
   * Converts the supplied guids and then loads the ACLs for the converted guids. See {@link
   * #getConvertedGuids(List)}.
   *
   * @param ids list of guids for which the acls needs to be loaded, assumed not <code>null</code>.
   * @return One ACL for each corresponding object id. Some of the entries may be <code>null</code>
   *     if the object does not have an ACL. The results are in the same order as the supplied ids.
   */
  ...
  /**
   * Helper method to extract communities from the supplied Acl. If the acl has any community entry
   * then just return that community.
   *
   * @param acl assumed not <code>null</code>.
   * @return Comma separated string of communities. May be empty never <code>null</code>.
   */
  private String getCommunitiesFromAcl(IPSAcl acl) {
    ...
    for (String c : getCommunityListFromAcl(acl)) {
      if (comms.length() > 0) comms.append(PSPackageService.NAME_SEPARATOR);
      comms.append(c);
    }
  ...
  public List<String> getAllCommunities() {
    ...
    IPSSecurityDesignWs secWs = PSSecurityWsLocator.getSecurityDesignWebservice();
   ...
   * @param communityNames must not be <code>null</code>. and Must be a valid community list.
   * @param clearOtherCommEntries if <code>true</code> clears other community entries on the design
   *     object.
   */
  public void setCommunities(
    ...
    List<IPSAcl> acls = new ArrayList<>();
    try {
      acls = aclServ.loadAclsForObjectsModifiable(Collections.singletonList(objectGuid));
      IPSAcl acl = acls.get(0);
      if (acl == null) {
        IPSTypedPrincipal owner =
            new PSTypedPrincipal(
                PSTypedPrincipal.DEFAULT_USER_ENTRY, IPSTypedPrincipal.PrincipalTypes.USER);
      ...
      for (String comm : communityNames) {
        IPSAclEntry entry = acl.createEntry(new PSTypedPrincipal(comm, PrincipalTypes.COMMUNITY));
  ...
  /**
   * Validates the community visibility for the given object ID, see if it matches the supplied
   * communities.
   *
   * @param objId the object ID, it may not be <code>null</code>.
   * @param commsVisibility the list of community names to validate with, never <code>null</code>,
   *     may be empty.
   * @return <code>true</code> if the community visibility of the given object matches the supplied
   *     communities or the does not have community visibility ACLs; <code>false</code> otherwise.
   */
   ...
   * @return <code>null</code> if successful load and set the communities; otherwise return the
   *     error message should an error occurs.
   */
  public String setPkgCommunities(PSPkgInfo pkg) {
    ...
    try {
      ...
      Collection<String> comms = srv.loadCommunityVisibility(pkg.getPackageDescriptorName());
      IPSPkgInfoService pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
      List<PSPkgElement> pkgElems = pkgService.findPkgElements(pkg.getGuid());
      ...
      ms_log.error(
          "Failed to apply community visibility for package: \""
              ...
              + "\"",
          e);
      if (StringUtils.isBlank(e.getLocalizedMessage())) return e.toString();
   ...
   * @return <code>true</code> if all the communities inside the supplied list are valid
   *     communities, otherwise <code>false</code>.
   */
  ...
  public void clearCommunity(String communityName, List<IPSGuid> objectGuids) {
    ...
    for (IPSGuid guid : objectGuids) {
      if (isVisibilitySupportedType(guid)) guids.add(guid);
    }
  ...
  /**
   * Returns <code>true</code> if the supplied guid type supports visibility setting.
   *
   * @param objectGuid assumed not <code>null</code>.
   * @return <code>true</code> if the supplied guid type supported by security list, otherwise
   *     <code>false</code>.
   */
   ...
   *
   * @param acl assumed not <code>null</code>.
   * @param communityName if <code>null</code> all entries are removed, if not specified community
   *     entry is removed.
   ...
   * element in the given package.
   *
   * @param pkgGuid the ID of the package, must not be <code>null</code>.
   * @param communities community names to be set to, must not be <code>null</code>. and Must be a
