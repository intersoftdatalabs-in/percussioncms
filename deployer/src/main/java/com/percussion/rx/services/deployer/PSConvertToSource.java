...
public class PSConvertToSource {
   ...
   * @return PsPair of boolean <code>true</code> is successful and String of error message never
  ...
  /**
   * Move the descriptor from {@link PSDeploymentHandler#IMPORT_ARCHIVE_DIR} to {@link
   * PSDeploymentHandler#EXPORT_DESC_DIR}.
   *
   * @param packageName the package name, assumed not blank.
   * @return an error message if there is any error. It may be <code>null</code> if there is no
   *     error.
   */
  private PSPair<Boolean, String> moveDescriptor(String packageName) {
    ...
    File descFile = new File(PSDeploymentHandler.IMPORT_ARCHIVE_DIR, packageName + ".xml");
    if (!descFile.exists()) {
      message.setFirst(false);
    ...
    File tgtFile = new File(PSDeploymentHandler.EXPORT_DESC_DIR, packageName + ".xml");
    try {
      FileUtils.copyFile(descFile, tgtFile);
    } catch (IOException e1) {
      message.setFirst(false);
      message.setSecond("Error saving Descriptor: " + e1.getLocalizedMessage());
   ...
   * @return the directory path or <code>null</code> if not found.
   */
