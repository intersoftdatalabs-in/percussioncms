 ...
 * service allowing exposure to clients via REST.
 *
 * @author erikserating
 */
...
public class PSPackageService {
    ...
    public PSPkgUiResponse reapplyVisibility(@QueryParam("packageNames") String packageNames) {
        if (StringUtils.isBlank(packageNames)) {
            return new PSPkgUiResponse(
                    ...
                    "Skipping the reapplying of visibility settings as packageNames parameter value is empty"
            );
        }
        try {
    ...
    public PSPkgUiResponse reapplyConfiguration(@QueryParam("packageNames") String packageNames) {
        if (StringUtils.isBlank(packageNames)) {
            return new PSPkgUiResponse(
                    PSPkgUiResponseType.FAILURE,
                    "Skipping the reapplying of configuration settings as packageNames parameter value is empty"
            );
        }
        try {
    ...
    @Consumes("application/x-www-form-urlencoded")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public PSUninstallMessages postCheckPackageDependencies(@QueryParam("packageName") String packageName)
            throws PSNotFoundException {
        var msgs = new PSUninstallMessages();
        var pkgUninstall = new PSPackageUninstall();
     ...
     */
    private void updatePkgComms(String pkgName, Collection<String> commList, boolean clearOtherEntries) {
        if (commList == null)
