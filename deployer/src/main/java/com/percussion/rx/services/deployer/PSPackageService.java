 ...
 * exposure to clients via "web services" which in this case is REST.
 *
 * @author erikserating
 */
...
public class PSPackageService {
  ...
  public PSPkgUiResponse reapplyVisibility(@QueryParam("packageNames") String packageNames) {
    ...
    try {
      response = PSPackageServiceHelper.applyPackageVisibility(packageNames);
    } catch (Exception e) {
      response = new PSPkgUiResponse(PSPkgUiResponseType.FAILURE, e.getLocalizedMessage());
    }
    return response;
  }
  ...
  public PSPackageCommunities getPackageCommunities() {
    ...
    while (iter.hasNext()) {
      ...
      PSPackageCommunity pkgComm = new PSPackageCommunity(pkgInfomap.get(guid), commsMap.get(guid));
  ...
  public PSPkgUiResponse postUpdatePackageCommunities(
    ...
    PSPkgUiResponse response = new PSPkgUiResponse(PSPkgUiResponseType.SUCCESS, "");
    try {
  ...
  public PSPkgUiResponse getServerTimeout() {
    ...
    PSPkgUiResponse response = new PSPkgUiResponse(PSPkgUiResponseType.SUCCESS, sto + "");
    return response;
  }
  ...
  public PSPkgUiResponse postConvertPackage(@QueryParam("packageName") String packageName) {
    ...
    try {
      ...
      PSPkgUiResponseType type =
          result.getFirst() ? PSPkgUiResponseType.SUCCESS : PSPkgUiResponseType.FAILURE;
      String message = result.getSecond();
      msg = new PSPkgUiResponse(type, message);
    } catch (Exception e) {
      msg = new PSPkgUiResponse(PSPkgUiResponseType.FAILURE, e.getLocalizedMessage());
   ...
   * @param selectedPkgs {@link #NAME_SEPARATOR} separated list of package names.
   */
  ...
  private String getConfiguredStatus(PSPkgInfo info) {
    ...
    List<PSConfigStatus> cfgs = cfgService.getConfigStatus(info.getPackageDescriptorName());
