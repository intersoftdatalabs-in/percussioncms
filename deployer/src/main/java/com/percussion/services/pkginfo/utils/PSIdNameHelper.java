 ...
 * dependency id's to actual guids which can then be used to represent the associated design objects
 * as stored in the package element information. This utility is necessary because not all
 * dependency id values are numeric. Some dependency id's are the actual names of their associated
 * design objects. For these values, a new guid will be generated and associated with the dependency
 * id name so that the object can later be referenced by name. Utilizes the id-name service {@link
 * IPSIdNameService} for loading/saving of id-name mappings.
 */
public class PSIdNameHelper {
  /**
   * Get an id for the given dependency id and type.
   *
   * @param id The dependency id. May be an object name, a numeric ID, or an actual guid. May not be
   *     <code>null</code> or empty.
   * @param type The system type for the dependency object. May not be <code>null</code>.
   * @return An {@link IPSGuid} for the given dependency id and type, never <code>null</code>.
   */
   ...
   *
   * @param guid The id for which a name will be returned, never <code>null</code>. Must represent a
   *     supported type, see {@link #isSupported(PSTypeEnum)}.
   * @return The dependency name for the id or <code>null</code> if not found.
   * @throws IllegalArgumentException if the guid does not represent a supported type.
   */
  ...
  /**
   * Determines if a given type is supported by this helper class. A type is supported if the
   * dependency type with which it is associated uses name (non-numeric) id values.
   *
