...
public class PSIdNameService implements IPSIdNameService {
   ...
   *
   * @return A collection of id-name mappings, not <code>null</code>, may be empty.
   */
   ...
   *
   * @return Map of dependency names/types to guids, with a name-type <code>String</code> as key,
   *     see {@link #getNameTypeKey(String, PSTypeEnum)}, and the <code>IPSGuid</code> object as
   ...
   *
   * @return Map of guids to dependency names, with an <code>IPSGuid</code> as key and name as
   *     value. Never <code>null</code>.
   */
  @SuppressWarnings("unchecked")
   ...
   *
   * @param requiredMap Indicates the map which is being requested. Must be one of {@link
   *     #NAME_TYPE_ID_MAP}, {@link #ID_NAME_MAP}.
  ...
  private Map<String, IPSGuid> getNameTypeToIdMap() {
    return (HashMap<String, IPSGuid>)
        cache.get(NAME_TYPE_ID_MAP_KEY, IPSCacheAccess.IN_MEMORY_STORE);
  }

  /**
   * Get the id name map from the cache.
   *
   * @return Map of guids to dependency names, with an <code>IPSGuid</code> object as key and name
   *     as value. May be <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  private Map<IPSGuid, String> getIdToNameMap() {
    return (HashMap<IPSGuid, String>) cache.get(ID_NAME_MAP_KEY, IPSCacheAccess.IN_MEMORY_STORE);
  }

  /**
   * Loads the name/type to id and id to name maps from the repository and caches references.
   *
  ...
  /**
   * Populates the specified name/type to id map using the specified collection of mappings.
   *
   * @param mappings The collection of id-name mappings, assumed not <code>null</code>.
   * @param ntMap The name/type to id map, assumed not <code>null</code>.
   */
  ...
  /**
   * Populates the specified id to name map using the specified collection of mappings.
   *
   * @param mappings The collection of id-name mappings, assumed not <code>null</code>.
   * @param idMap The id to name map, assumed not <code>null</code>.
   */
  ...
  /**
   * Generate the name/type <code>String</code> used as the key in the name/type id map, see {@link
   * #loadNameTypeToIdMap()}. The given name will be converted to lowercase in the key.
   *
