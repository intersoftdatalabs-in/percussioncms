...
public class PSFolderContentsDescriptorBuilder {
  ...
  public PSFolderContentsDescriptorBuilder(
    ...
    if (sourceManager == null)
      throw new IllegalArgumentException("deployment manager may not be null");
    if (sourceFolderPath == null || sourceFolderPath.trim().length() == 0)
      throw new IllegalArgumentException("source folder path may not be null or empty");
  ...
  public PSDeployableElement build() throws PSDeployException {
    ...
    if (pkg == null) {
      ...
      String previousMaxDeps = System.getProperty(IPSDeployConstants.PROP_MAX_DEPS);
      ...
      if (previousMaxDeps != null)
        System.setProperty(IPSDeployConstants.PROP_MAX_DEPS, previousMaxDeps);
      else
        System.setProperty(
            IPSDeployConstants.PROP_MAX_DEPS, String.valueOf(IPSDeployConstants.MAX_DEPS));
    }
    return pkg;
  }
  ...
  private PSDeployableElement getTopFolderElement() throws PSDeployException {
    PSDeployableElement pkg = null;
    Iterator i = m_sourceMgr.getDeployableElements(IPSDeployConstants.DEP_OBJECT_TYPE_FOLDER);
  ...
  private boolean shouldRemoveContentItem(PSDependency item) throws PSDeployException {
    ...
    while (i.hasNext() && !isCancelled()) {
      ...
      if (objectType.equals(IPSDeployConstants.DEP_OBJECT_TYPE_CONTENT_EDITOR)) {
  ...
  private void include(PSDependency dep) {
     ...
     */
    m_includedCount++;
    log.debug("include #" + m_includedCount + ":" + dep + " " + dep.getDependencyId());
