...
public class PSDeployFileJobControl implements IPSDeployJobControl {
  ...
  public int getStatus() throws PSDeployException {
    ...
    synchronized (m_cancelledStatusMonitor) {
      if (m_cancelledStatus == IPSDeployJobControl.JOB_ABORTED) result = -1;
  ...
  public int cancelDeployJob() throws PSDeployException {
    synchronized (m_cancelledStatusMonitor) {
      if (m_cancelledStatus == -1) {
        ...
        if (m_streamCounter == null) m_cancelledStatus = IPSDeployJobControl.JOB_CANCELLED;
        else {
          ...
          else m_cancelledStatus = IPSDeployJobControl.JOB_COMPLETED;
        }
      }
    }
  ...
  public void setStream(IPSStreamCounter streamCounter, int totalBytes) {
    if (streamCounter == null) throw new IllegalArgumentException("streamCounter may not be null");

    if (totalBytes <= 0) throw new IllegalArgumentException("bytes may not be < 0");
  ...
  private void updateStatusMessage() throws PSDeployException {
    ...
    if (m_cancelledStatus == IPSDeployJobControl.JOB_ABORTED) {
      ...
      Object[] args = {String.valueOf(m_bytes), String.valueOf(m_totalBytes)};
      m_statusMessage = MessageFormat.format(format, args);
    }
  }
