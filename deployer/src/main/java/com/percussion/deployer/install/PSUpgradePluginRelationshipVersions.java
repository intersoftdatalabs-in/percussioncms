...
public class PSUpgradePluginRelationshipVersions implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      PSUpgradePluginRelationship relPlugin = new PSUpgradePluginRelationship();
      ...
      for (int i = 0; i < cfgSet.size(); i++) {
        PSRelationshipConfig relConfig = (PSRelationshipConfig) cfgSet.get(i);
        if (relConfig.isSystem()) {
          // load version (checksum)
          Document doc = PSXmlDocumentBuilder.createXmlDocument();
          long version = IOTools.getChecksum(PSXmlDocumentBuilder.toString(relConfig.toXml(doc)));

          // find the package element
          String msg;
          String relName = relConfig.getName();
          IPSGuid relGuid = PSIdNameHelper.getGuid(relName, PSTypeEnum.RELATIONSHIP_CONFIGNAME);
          PSPkgElement pkgElem = pkgInfoSvc.findPkgElementByObject(relGuid);
          if (pkgElem != null) {
            // update package element version
            pkgElem = pkgInfoSvc.loadPkgElementModifiable(pkgElem.getGuid());
            pkgElem.setVersion(version);
            pkgInfoSvc.savePkgElement(pkgElem);

            msg = "Updated package element version for system " + "relationship '" + relName + "'";
          } else {
            msg = "Could not find package element for system " + "relationship '" + relName + "'";
          }
