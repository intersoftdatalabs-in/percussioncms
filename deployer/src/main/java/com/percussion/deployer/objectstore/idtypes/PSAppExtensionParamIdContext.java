...
public class PSAppExtensionParamIdContext extends PSApplicationIdContext {
   ...
   *
   * @param index The index into the param list of the call this parameter is from. May not be less
   *     than 0.
  ...
  public PSAppExtensionParamIdContext(int index, PSAbstractParamValue param) {
    if (!validateIndex(index)) throw new IllegalArgumentException("index is invalid");
   ...
   */
  public PSAppExtensionParamIdContext(Element source) throws PSUnknownNodeTypeException {
  ...
  /**
   * Get index into the param list of the extension/function call this parameter is from.
   *
   * @return The index, >=0.
   */
  public int getIndex() {
  ...
  public void updateCtxValue(Object value) {
    ...
    if (!(value instanceof IPSReplacementValue))
      throw new IllegalArgumentException("value must be instanceof IPSReplacementValue");
  ...
  public Element toXml(Document doc) {
    ...
    if (m_paramName != null) root.setAttribute(XML_ATTR_NAME, m_paramName);
    root.appendChild(m_param.toXml(doc));
    PSApplicationIdContext parent = getParentCtx();
    if (parent != null) root.appendChild(parent.toXml(doc));
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    String strIndex = PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_INDEX);
    m_index = -1;
    try {
    ...
    Element paramEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (paramEl == null) {
    ...
    else m_param = new PSExtensionParamValue(paramEl, null, null);

    Element ctxEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    if (ctxEl != null) setParentCtx(PSApplicationIDContextFactory.fromXml(ctxEl));
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSAppExtensionParamIdContext)) isEqual = false;
    else {
      // do not include param name as it is dynamic and transient
      PSAppExtensionParamIdContext other = (PSAppExtensionParamIdContext) obj;
      if (m_index != other.m_index) isEqual = false;
      else if (!m_param.equals(other.m_param)) isEqual = false;
  ...
  /**
   * Index of this param in its parent extension call's param list. Intialized during ctor, modfied
   * only by calls to <code>copyFrom()</code>.
   */
  private int m_index;

  /**
   * Name of this param in its parent extension call's param list. Initially <code>null</code>,
   * modified by calls to {@link #setParamName(String)}.
   */
