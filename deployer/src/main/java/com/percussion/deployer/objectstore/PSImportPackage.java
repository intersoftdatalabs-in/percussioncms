...
public class PSImportPackage implements IPSDeployComponent {
  ...
  public Element toXml(Document doc) {
    ...
    if (m_validationResults != null) root.appendChild(m_validationResults.toXml(doc));
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    Element pkgEl = tree.getNextElement(PSDeployableElement.XML_NODE_NAME, firstFlags);
    if (pkgEl == null) {
    ...
    Element valEl = tree.getNextElement(PSValidationResults.XML_NODE_NAME, nextFlags);
    if (valEl != null) m_validationResults = new PSValidationResults(valEl);
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSImportPackage)) throw new IllegalArgumentException("obj wrong type");
  ...
  // see IPSDeployComponent interface
  public int hashCode() {
    return m_pkg.hashCode() + (m_validationResults == null ? 0 : m_validationResults.hashCode());
  }

  // see IPSDeployComponent interface
  public boolean equals(Object obj) {
    boolean isEqual = true;
    if (!(obj instanceof PSImportPackage)) isEqual = false;
    else {
      PSImportPackage other = (PSImportPackage) obj;
      if (!m_pkg.equals(other.m_pkg)) isEqual = false;
      else if (m_validationResults == null ^ other.m_validationResults == null) {
        isEqual = false;
