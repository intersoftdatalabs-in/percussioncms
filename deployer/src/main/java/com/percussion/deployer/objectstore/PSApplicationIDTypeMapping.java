...
/** The PSApplicationIDTypeMapping class is to encapsulate ID and Type mapping information. */
public class PSApplicationIDTypeMapping implements IPSDeployComponent {
  ...
  public void setParent(String parentId, String parentType) {
    ...
    if (parentId != null && (parentType == null || parentId.trim().length() == 0)) {
      throw new IllegalArgumentException("parentType may not be null or empty");
    }

    m_parentId = parentId;
    if (parentId != null) m_parentType = parentType;
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSApplicationIDTypeMapping)) {
      ...
      if (!m_ctx.equals(idTypeMap.m_ctx)) isEqual = false;
      else if (!m_value.equals(idTypeMap.getValue())) isEqual = false;
      else if (!m_type.equals(idTypeMap.getType())) isEqual = false;
      else if (m_parentId == null ^ idTypeMap.m_parentId == null) isEqual = false;
      else if (m_parentId != null && !m_parentId.equals(idTypeMap.m_parentId)) {
        isEqual = false;
      } else if (m_parentType == null ^ idTypeMap.m_parentType == null) isEqual = false;
      else if (m_parentType != null && !m_parentType.equals(idTypeMap.m_parentType)) {
        isEqual = false;
      } else if (m_hasNewValue ^ idTypeMap.m_hasNewValue) isEqual = false;
    }
