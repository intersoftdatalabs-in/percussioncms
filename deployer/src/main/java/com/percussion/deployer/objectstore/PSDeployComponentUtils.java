...
public class PSDeployComponentUtils {
   ...
   * empty.
   *
   * @param source Element to get the attribute from, may not be <code>null</code>.
   * @param attName The name of the attribute to get, may not be <code>null</code> or empty
  ...
  /**
   * Gets the specified element's data from the supplied tree, validating that it is not <code>null
   * </code> and optionally not empty.
   *
   * @param tree The tree walker to use, assumed to be set at the parent node and may not be <code>
   *     null</code>.
   * @param parent The name of the parent node, may not be <code>null</code> or empty.
   * @param node The name of the node, may not be <code>null</code> or empty.
   * @param notEmpty If <code>true</code>, will also enforce that element value is not empty.
   * @return The value, never <code>null</code>, may be empty only if <code>notEmpty</code> is
   *     <code>false</code>.
   * @throws PSUnknownNodeTypeException if the XML element node does not pass validation.
   */
  public static String getRequiredElement(
      PSXmlTreeWalker tree, String parent, String node, boolean notEmpty)
      throws PSUnknownNodeTypeException {
    if (tree == null) throw new IllegalArgumentException("tree may not be null");
  ...
  public static <T> List<T> cloneList(Iterator<T> iter) {
    if (iter == null) throw new IllegalArgumentException("iter may not be null");
  ...
  /**
   * Get next element from the <code>tree</code> and <code>flags</code>, validating the parameters
   * and the retrieved element that it is not <code>null</code>.
   *
   * @param tree The tree to get the next element from, may not be <code>null</code>.
   * @param flags The appropriate <code>PSXmlTreeWalker.GET_NEXT_xxx</code> flags.
   * @param nodeName The expected XML node name, may not be <code>null</code> or empty.
   ...
   * @throws PSUnknownNodeTypeException if fail to retrieve next element from the <code>tree</code>.
   */
  public static Element getNextRequiredElement(PSXmlTreeWalker tree, int flags, String nodeName)
      throws PSUnknownNodeTypeException {
    if (tree == null) throw new IllegalArgumentException("tree may not be null");
  ...
  public static int getEnumeratedAttributeIndex(
      ...
      throws PSUnknownNodeTypeException {
    if (null == tree) throw new IllegalArgumentException("tree cannot be null");
    if (null == attrName) throw new IllegalArgumentException("attrName cannot be null");
    if (null == legalValues || legalValues.length == 0)
      throw new IllegalArgumentException("legalValues");
    if (tree.getCurrent() == null)
      throw new IllegalStateException("tree must be positioned on an element");
  ...
  /**
   * Strips leading "../", cannonical file root, or server request root e.g. "/Rhythmyx/" from a
   * path.
   *
   * @param path The path, may not be <code>null</code> or empty, may or may not begin with the
   *     prefix.
  ...
  public static String stripPathPrefix(String path) {
    ...
    if (path.startsWith(prefix)) path = path.substring(prefix.length());
      ...
      if (tmpPath.startsWith(getNormalizedPath(rootPath))) path = path.substring(rootPath.length());

      // strip any leading slash
      if (path.startsWith(UNIX_SEP_STRING)) path = path.substring(UNIX_SEP_STRING.length());

      // remove rx root if supplied
      String rxRoot = PSServer.getRequestRoot() + UNIX_SEP_CHAR;
      // strip leading slash from root
      if (rxRoot.startsWith(UNIX_SEP_STRING)) rxRoot = rxRoot.substring(UNIX_SEP_STRING.length());

      tmpPath = PSServer.isCaseSensitiveURL() ? path : path.toLowerCase();

      if (tmpPath.startsWith(rxRoot)) path = path.substring(rxRoot.length());
    }
   ...
   *
   * @param path The path, may be a URL or just a relative file reference, never <code>null</code>
   *     or empty.
   * @return The name of the app, or <code>null</code> if it cannot be determined.
   */
  public static String getAppName(String path) {
    ...
    if (path != null && path.trim().length() > 0) {
      ...
      if (toker.hasMoreTokens()) appName = toker.nextToken();
    }
   ...
   * @throws IllegalArgumentException if <code>path</code> is <code>null</code> or empty.
   */
   ...
   * @param base Used to return the base part of the url, ignored if <code>null</code>, otherwise
   *     the base portion of the url is appended onto the buffer.
   * @return The params, key is param name as a <code>String</code>, value is the param value as a
  ...
  /**
   * Converts the suppiled parameter to a list of {@link PSParam} objects, handling params with
   * repeated values.
   *
   * @param entry Map entry where key is the param name as a <code>String</code>, and value is the
   *     value of the param either as a <code>String</code> or <code>Collection</code> of <code>
   *     String</code> objects. May not be <code>null</code>.
  ...
  /**
   * Converts the supplied parameter map to a list of <code>Map.Entry</code> objects, repeating any
   * params that have multiple values.
   *
   * @param paramMap The map to convert, may not be <code>null</code>. Key is the param name as a
   *     <code>String</code>, value is the param value as a <code>String</code> or a <code>
   *     Collection</code> of <code>String</code> objects.
   * @return An iterator over zero or more <code>Map.Entry</code> objects, never <code>null</code>.
   */
