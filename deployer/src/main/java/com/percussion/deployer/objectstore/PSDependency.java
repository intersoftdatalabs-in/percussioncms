...
public abstract class PSDependency
  ...
  public PSDependency(
    ...
    if (dependencyId == null || dependencyId.trim().length() == 0)
      throw new IllegalArgumentException("dependencyId may not be null or empty");
  ...
  public Iterator<PSDependency> getDependencies(int type) {
    ...
    if (m_dependencies != null) {
      ...
      while (i.hasNext()) {
        ...
        if (dep.getDependencyType() == type) results.add(dep);
      }
  ...
  public Iterator<PSDependency> getDependencies(String objectType) {
    ...
    if (m_dependencies != null) {
      ...
      while (deps.hasNext()) {
        ...
        if (dep.getObjectType().equals(objectType)) result.add(dep);
      }
    }
  ...
  public PSDependency getChildDependency(String id, String objType) {
    ...
    if (m_dependencies != null) {
      ...
      while (i.hasNext() && childDep == null) {
        PSDependency dep = i.next();
        if (dep.getDependencyId().equals(id) && dep.getObjectType().equals(objType)) {
          childDep = dep;
        }
      }
    }
  ...
  public Iterator<PSDependency> getAncestors() {
    ...
    if (m_ancestors != null) ancestors = m_ancestors.iterator();
  ...
  public void setAncestors(Iterator<PSDependency> ancestors) {
    if (ancestors == null) m_ancestors = null;
      ...
      while (ancestors.hasNext()) m_ancestors.add(ancestors.next());
    }
  }
  ...
  private void printDependencyTree(StringBuilder buf, List processed, String prefix) {
    ...
    if (m_dependencies != null) {
      ...
      while (deps.hasNext())
        ((PSDependency) deps.next()).printDependencyTree(buf, processed, prefix);
    }
    ...
    if (m_ancestors != null) {
      ...
      while (ancs.hasNext())
        ((PSDependency) ancs.next()).printDependencyTree(buf, processed, prefix);
    }
  }
  ...
  public PSUserDependency addUserDependency(File path) {
    ...
    if (m_dependencies == null)
      throw new IllegalStateException("child dependencies have not yet been set");
  ...
  public void setParent(String id, String type) {
    if (!supportsParentId())
      throw new IllegalArgumentException("dependency does not support parent id");
  ...
  public String getKey() {
    ...
    if (m_parentId != null) key += KEY_SEP + getParentType() + KEY_SEP + getParentId();
  ...
  private boolean includesDependency(
    ...
    if (!dep.isIncluded() && (sameInstance || !dep.canBeIncludedExcluded())) return false;
    ...
    if ((sameInstance && this == dep) || (!sameInstance && getKey().equals(dep.getKey()))) {
      ...
      } else included = isIncluded();
    }

    // recurse children if not included yet, stop at elements that arent't the
    // root
    boolean isSubElement = this instanceof PSDeployableElement && !parentStack.isEmpty();
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(
        XML_ATT_SUPPORTS_ID_TYPES, m_supportsIdTypes ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(
        XML_ATT_SUPPORTS_ID_MAPPING, m_supportsIdMapping ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(
        XML_ATT_SUPPORTS_USER_DEPENDENCIES,
        m_supportsUserDependencies ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(XML_ATT_IS_INCLUDED, m_isIncluded ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(
        XML_ATT_SUPPORTS_PARENT_ID, m_supportsParentId ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    if (m_parentId != null) root.setAttribute(XML_ATT_PARENT_ID, m_parentId);
    if (m_parentType != null) root.setAttribute(XML_ATT_PARENT_TYPE, m_parentType);
    root.setAttribute(XML_ATTR_AUTO_EXPAND, m_autoExpand ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(XML_ATTR_AUTO_DEP, m_isAutoDependency ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
    root.setAttribute(XML_ATTR_GENERIC, m_isAssociation ? XML_ATT_VAL_TRUE : XML_ATT_VAL_FALSE);
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_supportsUserDependencies =
        XML_ATT_VAL_TRUE.equals(sourceNode.getAttribute(XML_ATT_SUPPORTS_USER_DEPENDENCIES));
    m_isIncluded = XML_ATT_VAL_TRUE.equals(sourceNode.getAttribute(XML_ATT_IS_INCLUDED));
    m_supportsParentId =
        XML_ATT_VAL_TRUE.equals(sourceNode.getAttribute(XML_ATT_SUPPORTS_PARENT_ID));
    // this should default to true if not specified
    m_autoExpand = !XML_ATT_VAL_FALSE.equals(sourceNode.getAttribute(XML_ATTR_AUTO_EXPAND));
    m_isAutoDependency = XML_ATT_VAL_TRUE.equals(sourceNode.getAttribute(XML_ATTR_AUTO_DEP));
    m_isAssociation = XML_ATT_VAL_TRUE.equals(sourceNode.getAttribute(XML_ATTR_GENERIC));

    sTemp = sourceNode.getAttribute(XML_ATT_PARENT_ID);
    if (sTemp != null && sTemp.trim().length() > 0) m_parentId = sTemp;
    else m_parentId = null;

    sTemp = sourceNode.getAttribute(XML_ATT_PARENT_TYPE);
    if (sTemp != null && sTemp.trim().length() > 0) m_parentType = sTemp;
    else m_parentType = null;
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSDependency)) throw new IllegalArgumentException("obj wrong type");
  ...
  public Object clone() {
    ...
    if (m_ancestors == null) copy.m_ancestors = null;
    else {
      copy.m_ancestors = new TreeSet<>();
      for (PSDependency m_ancestor : m_ancestors)
        copy.m_ancestors.add((PSDependency) m_ancestor.clone());
    }
    if (m_dependencies == null) copy.m_dependencies = null;
    else {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSDependency)) isEqual = false;
    else {
      PSDependency other = (PSDependency) obj;
      if (!m_dependencyId.equals(other.m_dependencyId)) isEqual = false;
      else if (m_dependencyType != other.m_dependencyType) isEqual = false;
      else if (!m_displayName.equals(other.m_displayName)) isEqual = false;
      else if (m_isIncluded != other.m_isIncluded) isEqual = false;
      else if (!m_objectType.equals(other.m_objectType)) isEqual = false;
      else if (!m_objectTypeName.equals(other.m_objectTypeName)) isEqual = false;
      else if (m_supportsIdMapping != other.m_supportsIdMapping) isEqual = false;
      else if (m_supportsIdTypes != other.m_supportsIdTypes) isEqual = false;
      else if (m_supportsParentId != other.m_supportsParentId) isEqual = false;
      else if (m_supportsUserDependencies != other.m_supportsUserDependencies) {
        isEqual = false;
      } else if ((m_ancestors == null) ^ (other.m_ancestors == null)) isEqual = false;
      else if (m_ancestors != null && !m_ancestors.equals(other.m_ancestors)) {
        isEqual = false;
      } else if ((m_dependencies == null) ^ (other.m_dependencies == null)) {
        isEqual = false;
      } else if (m_dependencies != null && !m_dependencies.equals(other.m_dependencies)) {
        isEqual = false;
      } else if ((m_parentId == null) ^ (other.m_parentId == null)) isEqual = false;
      else if (m_parentId != null && !m_parentId.equals(other.m_parentId)) {
        isEqual = false;
      } else if ((m_parentType == null) ^ (other.m_parentType == null)) isEqual = false;
      else if (m_parentType != null && !m_parentType.equals(other.m_parentType)) {
        isEqual = false;
      } else if (m_autoExpand != other.m_autoExpand) isEqual = false;
      else if (m_isAutoDependency != other.m_isAutoDependency) isEqual = false;
      else if (m_isAssociation != other.m_isAssociation) isEqual = false;
    }
  ...
  private static final String XML_ATT_SUPPORTS_ID_MAPPING = "supportsIdMapping";
  private static final String XML_ATT_IS_INCLUDED = "isIncluded";
  private static final String XML_ATT_SUPPORTS_USER_DEPENDENCIES = "supportsUserDependencies";
  private static final String XML_ATT_SUPPORTS_PARENT_ID = "supportsParentId";
