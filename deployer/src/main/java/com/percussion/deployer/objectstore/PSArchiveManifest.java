...
public final class PSArchiveManifest implements IPSDeployComponent {
  ...
  public void addFiles(PSDependency dep, Iterator<PSDependencyFile> depfiles) {
    ...
    if (depfiles == null || (!depfiles.hasNext()))
      throw new IllegalArgumentException("depfiles may not be null or empty");

    // make a new list from the Iterator.
    List<PSDependencyFile> depFileList = new ArrayList<>();
    while (depfiles.hasNext()) depFileList.add(depfiles.next());
  ...
  public Iterator<PSDependencyFile> getFiles(PSDependency dep) {
    ...
    DepFilesIdTypes value = m_depMap.get(dep.getKey());
    if (value == null || value.m_depFiles == null) return Collections.emptyIterator();
    else return value.m_depFiles.iterator();
  }
  ...
  public void addDbmsInfoList(PSDependency dep, List<PSDatasourceMap> infoList) {
    if (dep == null) throw new IllegalArgumentException("pkg may not be null");
  ...
  public PSApplicationIDTypes getIdTypes(PSDependency dep) {
    ...
    DepFilesIdTypes value = m_depMap.get(dep.getKey());
    if (value == null || value.m_idtypes == null) return null;
    else return value.m_idtypes;
  }
  ...
  public Element toXml(Document doc) {
    ...
    m_depMap.values().stream().map(depChild -> depChild.toXml(doc)).forEach(root::appendChild);
    return root;
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSArchiveManifest))
      throw new IllegalArgumentException("obj wrong type, expecting PSArchiveManifest");
  ...
  private static class DepFilesIdTypes implements IPSDeployComponent {
    ...
    public DepFilesIdTypes(
        String key,
        List<PSDependencyFile> files,
        PSApplicationIDTypes idtypes,
        List<PSDatasourceMap> dbmsInfoList) {
      m_key = key;
      m_depFiles = files;
      m_idtypes = idtypes;
    ...
    public Element toXml(Document doc) {
      ...
      if (m_idtypes != null) root.appendChild(m_idtypes.toXml(doc));

      if (m_dbmsInfoList != null) {
        Element dbmsListEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_DBMS_LIST_EL);
    ...
    public int hashCode() {
      return (m_key != null ? m_key.hashCode() : 0)
          + (m_depFiles != null ? m_depFiles.hashCode() : 0)
          + (m_idtypes != null ? m_idtypes.hashCode() : 0)
          + (m_dbmsInfoList != null ? m_dbmsInfoList.hashCode() : 0);
    }
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof DepFilesIdTypes)) {
        ...
        if (m_idtypes == null ^ other.m_idtypes == null) isEqual = false;
        else if (m_idtypes != null && !m_idtypes.equals(other.m_idtypes)) isEqual = false;
        else if (m_dbmsInfoList == null ^ other.m_dbmsInfoList == null) isEqual = false;
        else if (m_dbmsInfoList != null && !m_dbmsInfoList.equals(other.m_dbmsInfoList)) {
          isEqual = false;
        } else if (m_depFiles == null ^ other.m_depFiles == null) isEqual = false;
        else if (m_depFiles != null && !m_depFiles.equals(other.m_depFiles)) {
          isEqual = false;
        }
      }
      return isEqual;
    ...
    private List<PSDependencyFile> getDepFilesFromXml(Element depfileEl, PSXmlTreeWalker tree)
      ...
      while (depfileEl != null) {
        ...
        depfileEl = tree.getNextElement(PSDependencyFile.XML_NODE_NAME, NEXT_FLAGS);
      }
    ...
    private List<PSDatasourceMap> getInfoListFromXml(PSXmlTreeWalker tree)
      ...
      while (infoEl != null) {
        ...
        infoEl = tree.getNextElement(PSDbmsInfo.DATASOURCE_XML_ELEMENT, NEXT_FLAGS);
      }
      return infoList;
    }
