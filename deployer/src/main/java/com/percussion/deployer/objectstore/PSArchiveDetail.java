...
public class PSArchiveDetail implements IPSDeployComponent {
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    // walk the mappings
    Element mappingEl = tree.getNextElement(XML_EL_DBMS_INFO_MAPPING, firstFlags);
    ...
    while (mappingEl != null) {
      ...
      while (datasrcInfoEl != null) {
        ...
        datasrcInfoEl = tree.getNextElement(PSDatasourceMap.XML_NODE_NAME, nextFlags);
      }

      // get the next mapping
      tree.setCurrent(mappingEl);
      mappingEl = tree.getNextElement(XML_EL_DBMS_INFO_MAPPING, nextFlags);
    }
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSArchiveDetail)) throw new IllegalArgumentException("obj wrong type");
    ...
    m_exportDescriptor
        ...
        .forEachRemaining(
            pkg -> {
              ...
              var pkgList =
                  Optional.ofNullable(m_externalDbmsMap.get(pkgKey)).orElseGet(ArrayList::new);
