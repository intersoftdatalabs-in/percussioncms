...
public class PSJexlBindings {
  ...
  public PSJexlBinding getByName(String name) {
    ...
    for (PSJexlBinding bind : getBindings()) {
      if (StringUtils.isNotBlank(bind.getName()) && bind.getName().equals(name)) return bind;
    }
    return null;
  }
  ...
  public PSJexlBinding getByExpression(String expression) {
    ...
    for (PSJexlBinding bind : getBindings()) {
      if (bind.getExpression().equals(expression)) return bind;
    }
    return null;
  }
  ...
  public PSJexlBinding getByIndex(int ix) {
    if (ix < 0) throw new IllegalArgumentException("binding index may not be less than 0");
    for (PSJexlBinding bind : getBindings()) {
      if (bind.getIndex() == ix) return bind;
    }
    return null;
  }
  ...
  protected PSJexlBindings clone() {
    List<PSJexlBinding> bindings = new ArrayList<>(getBindings().size());
  ...
  public PSJexlBindings backupBindings() {
    if (m_srcBindings != null) m_srcBindings.getBindings().clear();
