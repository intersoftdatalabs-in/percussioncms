...
public class PSLogSummary implements IPSDeployComponent {
  ...
  public PSLogSummary(PSDeployableElement pkg, PSArchiveSummary archSummary) {
    if (pkg == null) throw new IllegalArgumentException("pkg parameter should not be null");
    if (archSummary == null)
      throw new IllegalArgumentException("archSummary parameter should not be null");
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    // get attributes
    String sAttrValue =
        PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_ARCHIVE_EXIST);
    m_archiveExist = sAttrValue.equals(XML_VALUE_TRUE);
    sAttrValue = PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_ID);
    ...
    Element childEl =
        PSDeployComponentUtils.getNextRequiredElement(
            tree, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN, PSDeployableElement.XML_NODE_NAME);
    m_pkg = new PSDeployableElement(childEl);

    childEl =
        PSDeployComponentUtils.getNextRequiredElement(
            tree, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS, PSArchiveSummary.XML_NODE_NAME);
    m_archiveSummary = new PSArchiveSummary(childEl);

    childEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    if (childEl != null) m_detail = new PSLogDetail(childEl);
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSLogSummary))
      throw new IllegalArgumentException("obj wrong type, expecting PSLogSummary");
  ...
  // see IPSDeployComponent interface
  public int hashCode() {
    return (m_detail == null ? 0 : m_detail.hashCode())
        + m_id
        + (m_archiveExist ? 1 : 0)
        + m_pkg.hashCode()
