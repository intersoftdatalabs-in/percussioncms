...
public class PSValidationResults implements IPSDeployComponent {
  ...
  public boolean hasErrors() {
    for (PSValidationResult vResult : m_validateResults) {
      if (vResult.isError()) return true;
    }
    return false;
  }
  ...
  /**
   * Get a list of <code>PSValiationResult</code> object from the given parameters if the XML
   * contains any <code>PSValiationResult</code>.
   *
   * @param childEl The current element in the <code>tree</code>, which will be retrieved from. It
   *     may not <code>null</code>.
   * @param tree The XML tree, assuming it is not <code>null</code>. The Tree will be left on a
   *     <code>null</code> element, or the next element that is not a validastion result.
   * @return The current element in the <code>tree</code>, after the retrieving operation..
  ...
  private Element getValidateResults(Element childEl, PSXmlTreeWalker tree)
    ...
    while (childEl != null && childEl.getNodeName().equals(PSValidationResult.XML_NODE_NAME)) {
  ...
  /**
   * Get a list of dependency objects from the given parameters if the XML contains any of them.
   *
   * @param childEl The current element in the <code>tree</code>, which will be retrieved from. It
   *     may be <code>null</code>.
   * @param tree The XML tree, assume not <code>null</code>. The Tree will be left on a <code>null
   *     </code> element, or the next element that is not a dependency element.
   * @throws PSUnknownNodeTypeException if the XML is malformed.
   */
  private void getAbsentAncestors(Element childEl, PSXmlTreeWalker tree)
    ...
    while (childEl != null
      ...
      } else if (childEl.getNodeName().equals(PSDeployableObject.XML_NODE_NAME)) {
