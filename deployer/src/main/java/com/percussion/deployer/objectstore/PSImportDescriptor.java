...
public class PSImportDescriptor extends PSDescriptor {
  ...
  public Element toXml(Document doc) {
    ...
    while (pkgs.hasNext()) root.appendChild((pkgs.next()).toXml(doc));
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    Element archiveInfoEl = tree.getNextElement(PSArchiveInfo.XML_NODE_NAME, nextFlags);
    ...
    m_packages.clear();
    Element pkgEl = tree.getNextElement(PSImportPackage.XML_NODE_NAME, nextFlags);
    while (pkgEl != null) {
      m_packages.add(new PSImportPackage(pkgEl));
      pkgEl = tree.getNextElement(PSImportPackage.XML_NODE_NAME, nextFlags);
    }
