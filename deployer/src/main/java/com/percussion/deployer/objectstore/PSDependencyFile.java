...
public class PSDependencyFile implements IPSDeployComponent {
  ...
  public PSDependencyFile(int fileType, File file, File originalFile) {
    ...
    if (!isValidType(fileType)) throw new IllegalArgumentException("invalid type");
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, XML_EL_RX_FILE, m_file.getPath());
    PSXmlDocumentBuilder.addElement(doc, root, XML_EL_ARCHIVE_FILE, m_archiveLocation.getPath());
    if (m_originalFile != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_EL_ORIG_FILE, m_originalFile.getPath());
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    // get type
    m_type = UNDEFINED;
    sTemp = PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_FILE_TYPE);
    for (int i = 0; i < TYPE_ENUM.length && m_type == UNDEFINED; i++) {
      if (TYPE_ENUM[i].equals(sTemp)) m_type = i;
    }

    if (m_type == UNDEFINED) {
      Object[] args = {sourceNode.getTagName(), XML_ATTR_FILE_TYPE, sTemp};
    ...
    m_originalFile = null;
    tree.setCurrent(sourceNode);
    Element fileEl = tree.getNextElement(XML_EL_ORIG_FILE, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (fileEl != null) {
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSDependencyFile)) throw new IllegalArgumentException("obj wrong type");
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSDependencyFile)) isEqual = false;
    else {
      PSDependencyFile other = (PSDependencyFile) obj;
      if (m_type != other.m_type) isEqual = false;
      else if (!m_file.equals(other.m_file)) isEqual = false;
      else if (m_archiveLocation == null ^ other.m_archiveLocation == null) isEqual = false;
      else if (m_archiveLocation != null && !m_archiveLocation.equals(other.m_archiveLocation)) {
        isEqual = false;
      }
    }
