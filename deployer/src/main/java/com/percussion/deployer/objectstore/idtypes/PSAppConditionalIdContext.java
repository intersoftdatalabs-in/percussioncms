...
public class PSAppConditionalIdContext extends PSApplicationIdContext {
  ...
  public boolean isSameConditional(PSConditional cond, boolean compareOriginal) {
    ...
    if (!cond.getBoolean().equals(thisCond.getBoolean())) isSame = false;
    else if (!cond.getOperator().equals(thisCond.getOperator())) isSame = false;
    else if (m_type == TYPE_VALUE) {
      if (!cond.getValue().equals(thisCond.getValue())) isSame = false;
      else if (!cond.getVariable()
          .getValueDisplayText()
          .equals(thisCond.getVariable().getValueDisplayText())) {
        isSame = false;
      }
    } else {
      if (!cond.getVariable().equals(thisCond.getVariable())) isSame = false;
      else if (!cond.getValue()
          .getValueDisplayText()
          .equals(thisCond.getValue().getValueDisplayText())) {
        isSame = false;
      }
    }
  ...
  public String getDisplayText() {
    ...
    String side = null;
    if (m_type == TYPE_VARIABLE) side = bundle.getString("appIdCtxConditionalVariable");
    else side = bundle.getString("appIdCtxConditionalValue");

    Object[] args = {
      side,
      m_cond.getVariable().getValueDisplayText(),
      m_cond.getOperator(),
      m_cond.getValue(),
      m_cond.getBoolean()
    };
    String text = MessageFormat.format(getBundle().getString("appIdCtxConditional"), args);
  ...
  public void updateCtxValue(Object value) {
    ...
    try {
      if (m_type == TYPE_VARIABLE) m_cond.setVariable(replVal);
      else m_cond.setValue(replVal);
  ...
  public String getIdentifier() {
    return m_type == TYPE_VALUE
        ? m_cond.getVariable().getValueText()
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    String strType = PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_TYPE);
    m_type = -1;
    for (int i = 0; i < TYPE_ENUM.length && m_type == -1; i++) {
      if (TYPE_ENUM[i].equals(strType)) m_type = i;
    }
    ...
    Element condEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (condEl == null) {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSAppConditionalIdContext)) isEqual = false;
    else {
      PSAppConditionalIdContext other = (PSAppConditionalIdContext) obj;
      if (m_type != other.m_type) isEqual = false;
      else if (!isSameConditional(other.m_cond)) isEqual = false;
