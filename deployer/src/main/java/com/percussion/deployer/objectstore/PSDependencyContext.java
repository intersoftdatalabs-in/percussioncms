  ...
  public void addDependency(PSDependency dep, PSDeployableElement pkg) {
    ...
    if (included && !dep.isIncluded()) {
      if (!dep.canBeIncludedExcluded()) {
        // would be a bug
        throw new IllegalArgumentException("ctx is selected, cannot include dep");
      }
      ...
      if (!dep.canBeIncludedExcluded()) depIncluded = pkg.includesDependency(dep, true);

      if (depIncluded) {
        if (!setIncluded(true)) {
          // this would be a bug
          throw new IllegalArgumentException("dep is included, but ctx cannot be set as included");
        }
      }
    }
