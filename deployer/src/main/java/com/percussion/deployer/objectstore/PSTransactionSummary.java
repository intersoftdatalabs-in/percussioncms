...
public final class PSTransactionSummary implements IPSDeployComponent {
  ...
  public PSTransactionSummary(
      int logId, String depDesc, String elementName, String elementType, int action) {
    if (logId < 0) throw new IllegalArgumentException("logId may not be less than 0");
    if (depDesc == null || depDesc.trim().length() == 0)
      throw new IllegalArgumentException("depDesc may not be null or empty");
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_logId =
        Integer.parseInt(PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_LOGID));
    m_action =
        Integer.parseInt(PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_ACTION));
  ...
  private boolean validateElementType(String type) {
    return type.equals(TYPE_DATA)
        || type.equals(TYPE_EXTENSION)
        || type.equals(TYPE_FILE)
        || type.equals(TYPE_SCHEMA)
        || type.equals(TYPE_CMS_OBJECT)
        || type.equals(TYPE_SKIPPED);
  }
  ...
  public int hashCode() {
    return m_logId
        + m_elementName.hashCode()
        + m_elementType.hashCode()
        + m_action
        + m_depDesc.hashCode();
  }
