...
public final class PSDbmsMap implements IPSDeployComponent {
  ...
  public Element toXml(Document doc) {
    ...
    m_mappingList.stream().map(mapping -> mapping.toXml(doc)).forEach(root::appendChild);
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    var mappingEl =
        tree.getNextElement(PSDbmsMapping.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (mappingEl != null) {
      m_mappingList.add(new PSDbmsMapping(mappingEl));
      mappingEl =
          tree.getNextElement(PSDbmsMapping.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  }
