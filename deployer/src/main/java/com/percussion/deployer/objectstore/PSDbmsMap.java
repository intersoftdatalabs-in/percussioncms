...
public class PSDbmsMap implements IPSDeployComponent {
  ...
  public PSDbmsMap(String server) {
    if (server == null || server.trim().length() == 0)
      throw new IllegalArgumentException("server name may not be null or empty");
  ...
  public PSDbmsMapping getMapping(String sourceInfo) {
    if (sourceInfo == null) throw new IllegalArgumentException("sourceInfo may not be null");
    ...
    while (list.hasNext() && result == null) {
      ...
      if (sourceInfo.equals(currMap.getSourceInfo())) result = currMap;
    }
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    Element mappingEl = tree.getNextElement(PSDbmsMapping.XML_NODE_NAME, firstFlags);

    while (mappingEl != null) {
      PSDbmsMapping mapping = new PSDbmsMapping(mappingEl);
      m_mappingList.add(mapping);

      mappingEl = tree.getNextElement(PSDbmsMapping.XML_NODE_NAME, nextFlags);
    }
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSDbmsMap))
      throw new IllegalArgumentException("obj wrong type, expecting PSDbmsMap");
  ...
  public boolean equals(Object obj) {
    ...
    if ((obj instanceof PSDbmsMap)) {
      if (obj == this) // compare to itself
      {
        ...
        if (m_mappingList.equals(objMap2.m_mappingList) && m_srcServer.equals(objMap2.m_srcServer))
          bEqual = true;
      }
    }
    return bEqual;
