...
public final class PSArchiveInfo implements IPSDeployComponent {
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    m_created =
        Optional.ofNullable(sourceNode.getAttribute(XML_ATTR_CREATED))
    ...
    var serverInfoEl =
        Optional.ofNullable(
                tree.getNextElement(
                    PSFormatVersion.NODE_TYPE, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN))
            .orElseThrow(
                () ->
    ...
    var detailEl =
        tree.getNextElement(PSArchiveDetail.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
