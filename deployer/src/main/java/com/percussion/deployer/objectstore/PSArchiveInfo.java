...
public class PSArchiveInfo implements IPSDeployComponent {
  ...
  public PSArchiveInfo(
      String archiveRef,
      String serverName,
      PSFormatVersion serverInfo,
      PSDbmsInfo repository,
      String userName,
      String category) {
    ...
    if (serverName == null || serverName.trim().length() == 0)
      throw new IllegalArgumentException("serverName may not be null or empty");

    if (serverInfo == null) throw new IllegalArgumentException("serverInfo may not be null");

    if (repository == null) throw new IllegalArgumentException("repository may not be null");
  ...
  public Element toXml(Document doc) {
    ...
    if (m_detail != null) root.appendChild(m_detail.toXml(doc));
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    m_created =
        created == null || created.equals("-1")
            ? (new Date()).getTime()
            : (new Long(created)).longValue();

    PSXmlTreeWalker tree = new PSXmlTreeWalker(sourceNode);

    Element serverInfoEl =
    ...
    Element repositoryEl =
        tree.getNextElement(PSDbmsInfo.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    ...
    if (detailEl != null) m_detail = new PSArchiveDetail(detailEl);
  }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSArchiveInfo)) throw new IllegalArgumentException("obj wrong type");
    ...
    m_created = info.m_created == -1 ? (new Date()).getTime() : info.m_created;
  }
