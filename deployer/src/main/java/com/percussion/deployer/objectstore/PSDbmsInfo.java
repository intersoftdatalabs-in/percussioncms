...
public class PSDbmsInfo implements IPSDeployComponent {
      ...
      try {
        pwd =
            PSEncryptor.encryptString(
                PathUtils.getRxDir(null).getAbsolutePath().concat(PSEncryptor.SECURE_DIR), m_pw);
      } catch (PSEncryptionException e) {
        logger.warn(
            "Error encrypting datasource password: {}", PSExceptionUtils.getMessageForLog(e));
        logger.debug(e);
      }
    }
  ...
  public void setUserNamePwd(String usr, String pwd, boolean isPwdEncrypted) {
    ...
    if (StringUtils.isNotEmpty(usr)) m_uid = usr;
    else m_uid = "";

    if (StringUtils.isNotEmpty(pwd)) m_pw = pwd;
    else m_pw = "";
  ...
  public boolean isSameDb(PSDbmsInfo other) {
    ...
    if (m_connInfo != null && other.getConnectionInfo() != null) {
      if (!m_connInfo.getDataSource().equals(other.getConnectionInfo().getDataSource()))
        isSame = false;
    }
    if (!m_driver.equals(other.m_driver)) isSame = false;
    else if (!m_server.equals(other.m_server)) isSame = false;
    else if (!m_database.equals(other.m_database)) isSame = false;
    else if (!m_origin.equals(other.m_origin)) isSame = false;
  ...
  public Element toXml(Document doc) {
    ...
    if (getConnectionInfo() != null)
      PSXmlDocumentBuilder.addElement(
          doc, root, DATASOURCE_XML_ELEMENT, getConnectionInfo().getDataSource());
    ...
    PSXmlDocumentBuilder.addElement(
        doc, root, PASSWORD_ENCRYPTED_XML_ELEMENT, Boolean.toString(passwordEncrypted));
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException, PSDeployException {
    ...
    m_server =
        PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, SERVER_XML_ELEMENT, true);
    m_database =
        PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, DATABASE_XML_ELEMENT, false);
    m_origin =
        PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, ORIGIN_XML_ELEMENT, false);
    m_uid = PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, UID_XML_ELEMENT, false);
    m_pw =
        PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, PASSWORD_XML_ELEMENT, false);
    String temp = null;
    try {
      temp =
          PSDeployComponentUtils.getRequiredElement(
              tree, XML_NODE_NAME, PASSWORD_ENCRYPTED_XML_ELEMENT, false);
    ...
    if (temp == null || temp.equalsIgnoreCase("null") || temp.equalsIgnoreCase("")) {
      passwordEncrypted =
          true; // If we persisted the xml we can assume it was encrypted on generation.
    } else {
  ...
  public void copyFrom(IPSDeployComponent obj) {
    ...
    if (!(obj instanceof PSDbmsInfo)) throw new IllegalArgumentException("obj wrong type");

    PSDbmsInfo src = (PSDbmsInfo) obj;
    if (src.getConnectionInfo() != null)
      m_connInfo = new PSDbmsConnectionInfo(src.getConnectionInfo().getDataSource());
  ...
  public int hashCode() {
    return Objects.hash(
        m_driver, m_server, m_database, m_origin, m_uid, m_pw, m_connInfo.getDataSource());
  }
