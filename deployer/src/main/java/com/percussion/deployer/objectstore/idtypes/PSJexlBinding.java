...
public class PSJexlBinding {
  ...
  public PSJexlBinding(PSJexlBinding b) {
    if (b == null) throw new IllegalArgumentException("binding may not be null");
    setName(b.getName());
  ...
  public void setIndex(int index) {
    if (index < 0) throw new IllegalStateException("index may not be less than 0");
    this.m_index = index;
  }
  ...
  public boolean equals(Object other) {
    ...
    if (getIndex() != b.getIndex()) isEqual = false;
    else if (!getExpression().equals(b.getExpression())) isEqual = false;
    else if (StringUtils.isNotBlank(getName())
        && StringUtils.isNotBlank(b.getName())
        && !getName().equals(b.getName())) isEqual = false;
    return isEqual;
  }
  ...
  public int hashCode() {
    if (StringUtils.isNotBlank(getName()))
      return getName().hashCode() + getExpression().hashCode() + getIndex() + super.hashCode();
    else return getExpression().hashCode() + getIndex() + super.hashCode();
  }

  /** make a copy of the binding */
  public PSJexlBinding clone() {
    PSJexlBinding b = new PSJexlBinding(getIndex(), getName(), getExpression());
