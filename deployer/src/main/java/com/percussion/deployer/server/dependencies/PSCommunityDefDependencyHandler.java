...
public class PSCommunityDefDependencyHandler extends PSDataObjectDependencyHandler {
  ...
  public Iterator<PSDependency> getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    List<PSDependency> childDeps =
        getChildDepsFromParentID(
            ...
            COMMUNITY_ID,
            parentId,
            PSComponentDependencyHandler.DEPENDENCY_TYPE,
            tok);
  ...
  private List<PSDependency> getRoleChildDeps(PSSecurityToken tok, String parentId)
    ...
    Iterator ids = getChildIdsFromTable(COMM_RL_TABLE, COMM_RL_ID, COMMUNITY_ID, parentId);
    if (ids.hasNext()) {
      PSDbmsHelper dbmsHelper = PSDbmsHelper.getInstance();
      ...
      roleDeps = getDepsFromIds(names, PSRoleDefDependencyHandler.DEPENDENCY_TYPE, tok, -1);
    }
  ...
  public void installDependencyFiles(
    ...
    installDepDataForDepDef(
        depData, dep, ctx, COMMUNITY_TABLE, COMMUNITY_ID, COMMUNITY_NAME, DEPENDENCY_TYPE);

    // \/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    // delete the parent from the child relationship tables
    // \/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    deleteDepIdFromTable(dep, ctx, COMM_CP_TABLE, COMMUNITY_ID, m_commCPSchema);
    deleteDepIdFromTable(dep, ctx, COMM_RL_TABLE, COMMUNITY_ID, m_commRLSchema);
    ...
    try {
      ...
      IPSGuid communityGuid =
          PSGuidUtils.makeGuid(commMapping.getTargetId(), PSTypeEnum.COMMUNITY_DEF);
   ...
   * @throws SQLException
   */
  private Object qualify(String tablename) throws NamingException, SQLException {
    PSConnectionDetail detail = PSConnectionHelper.getConnectionDetail();
    return PSSqlHelper.qualifyTableName(
  ...
  PSDependencyData convertRoleIdToRoleName(PSDependencyData depData) throws PSDeployException {
        ...
        if (colName.equalsIgnoreCase(COMM_RL_ID)) {
          ...
          PSJdbcSelectFilter filter =
              new PSJdbcSelectFilter(
                  ROLE_ID, PSJdbcSelectFilter.EQUALS, col.getValue(), Types.INTEGER);
    ...
    PSJdbcTableData newData = new PSJdbcTableData(COMM_RL_TABLE, tgtRowList.iterator());

    PSDependencyData newDepData = new PSDependencyData(depData.getSchema(), newData);
  ...
  private PSJdbcTableData transferIdsForRLData(
      PSJdbcTableData data, PSDependency dep, PSImportCtx ctx, PSSecurityToken tok)
      throws PSDeployException {
    ...
    while (rows.hasNext()) {
      ...
      while (srcCols.hasNext()) {
        ...
        if (colName.equalsIgnoreCase(COMMUNITY_ID)) {
          ...
          PSJdbcSelectFilter filter =
              new PSJdbcSelectFilter(
                  ROLE_NAME, PSJdbcSelectFilter.EQUALS, col.getValue(), Types.VARCHAR);
          PSDependencyData pdata;
          pdata = getDepDataFromTable(ROLE_TABLE, filter, true);
    ...
    while (rows.hasNext()) {
      ...
      while (srcCols.hasNext()) {
        ...
        if (colName.equalsIgnoreCase(COMMUNITY_ID)) {
          ...
          mapChildIdForColumnNullable(col, dep, PSComponentDependencyHandler.DEPENDENCY_TYPE, ctx);
        }
        tgtColList.add(col);
      }
  ...
  private static final String NEXTNUMBER_ID = "communityid";
