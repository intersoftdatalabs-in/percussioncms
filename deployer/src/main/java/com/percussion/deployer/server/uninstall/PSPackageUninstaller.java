...
public class PSPackageUninstaller {
   ...
   public List<IPSUninstallResult> uninstallPackages(List<String> packageNames, boolean isRevertEntry) throws PSNotFoundException {
      ...
      for (var pkgInfo : pkgInfos) {
         ...
         if (msgs.isEmpty()) {
            var result = new PSUninstallResult(pkgInfo.getPackageDescriptorName(), IPSUninstallResult.PSUninstallResultType.SUCCESS);
         ...
         for (var msg : msgs) {
            if (msg.getPackageName().equals(pkgInfoModifiable.getPackageDescriptorName()) && !isRevertEntry) {
         ...
         pkgInfoModifiable.setLastActionDate(new Date());
         pkgInfoModifiable.setLastActionStatus(msgs.isEmpty() ? PackageActionStatus.SUCCESS : PackageActionStatus.FAIL);
         pkgService.savePkgInfo(pkgInfoModifiable);
    ...
    */
   private PSPair<List<PSPkgInfo>, List<IPSUninstallResult>> loadPackages(List<String> packageNames) {
      ...
      for (var pkgName : packageNames) {
         ...
         if (pkgInfo == null) {
            ...
            msg.setMessage("Skipped uninstalling the supplied package as no package exists with the name: " + pkgName);
            messages.add(msg);
   ...
   private List<IPSUninstallResult> uninstallPackage(PSPkgInfo pkgInfo) throws PSNotFoundException {
      ...
      var wasContentTypeDeleted = messages.stream()
         .anyMatch(res -> res.getResultType() == IPSUninstallResult.PSUninstallResultType.WARN &&
   ...
   public List<IPSUninstallResult> deleteConfigFiles(PSPkgInfo pkgInfo) {
      ...
      cfgErrors.forEach((file, exception) -> {
         var res = new PSUninstallResult(configName, IPSUninstallResult.PSUninstallResultType.WARN);
   ...
   private IPSUninstallResult deleteElement(PSPkgInfo pkgInfo, IPSGuid objGuid) {
      ...
      if (model != null) {
         try {
            ...
            var res = new PSUninstallResult(pkgInfo.getPackageDescriptorName(), IPSUninstallResult.PSUninstallResultType.ERROR);
