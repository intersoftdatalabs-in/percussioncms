...
public class PSContextDefDependencyHandler extends PSDataObjectDependencyHandler
  ...
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok) throws PSNotFoundException {
    ...
    for (IPSPublishingContext context : contexts) {
      ...
      deps.add(createDependency(m_def, String.valueOf(cId.longValue()), context.getName()));
    }
  ...
  public PSDependency getDependency(PSSecurityToken tok, String id) throws PSNotFoundException {
    ...
    if (context != null) dep = createDependency(m_def, id, context.getName());
  ...
  private void transferIdsInContext(
      PSSecurityToken tok, PSDependency dep, PSPublishingContext context, PSImportCtx ctx)
      throws PSDeployException {
    // xform the ids for CONTEXT, DEFAULTSCHEME
    IPSGuid schId = context.getDefaultSchemeId();
    if (schId != null) {
      PSIdMapping schemeMap =
          getIdMapping(
              ctx,
      ...
      if (schemeMap != null) {
        context.setDefaultSchemeId(
            PSGuidUtils.makeGuid(schemeMap.getTargetId(), PSTypeEnum.LOCATION_SCHEME));
      }
    }
  ...
  public void installDependencyFiles(
      @SuppressWarnings("unused") PSSecurityToken tok,
  ...
  public void doInstallDependencyFiles(
    ...
    if (ctxtMapping != null) context = findPublishingContext(ctxtMapping.getTargetId());
    else context = findPublishingContext(dep.getDependencyId());
    ...
    String packagedContextContent = PSDependencyUtils.getFileContentAsString(archive, file);
  ...
  private List<PSDependency> getLocationSchemeDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    IPSPublishingContext context = findPublishingContext(dep.getDependencyId());
    if (context == null) {
      Object[] args = {dep.getDependencyId(), dep.getObjectTypeName(), dep.getDisplayName()};
      throw new PSDeployException(IPSDeploymentErrors.DEP_OBJECT_NOT_FOUND, args);
    }

    List<IPSLocationScheme> schemes = m_siteMgr.findSchemesByContextId(context.getGUID());
    for (IPSLocationScheme scheme : schemes) ids.add(String.valueOf(scheme.getGUID().longValue()));

    Iterator<String> it = ids.iterator();
    PSDependencyHandler lsHandler =
        getDependencyHandler(PSLocSchemeDefDependencyHandler.DEPENDENCY_TYPE);
