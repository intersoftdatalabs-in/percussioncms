 ...
 * user object for table cell or elements in the list. It renderers the element as
...
public class PSMappingElement implements Comparable {
   ...
   * equals()</code> and <code>
   * toString()</code> methods) to fit its needs, otherwise using the base-class methods results in
   * <code>java.lang.NullPointerException</code>s.
   */
   ...
   *
   * @return <code>true</code> if it has parent (if {@link #setParent(String, String, String)} is
   ...
   *
   * @return the parent id, may be <code>null</code> if {@link #setParent(String, String, String)}
   *     is not called.
   */
   ...
   *
   * @return the parent name, may be <code>null</code> if {@link #setParent(String, String, String)}
   *     is not called.
   */
   ...
   *
   * @return the parent type, may be <code>null</code> if {@link #setParent(String, String, String)}
   *     is not called.
   */
   ...
   * parent.
   *
   * @return the string, may be <code>null</code> if it does not have a parent, never empty.
   */
  public String getParentDisplayString() {
    if (hasParent()) return m_parentName + "(" + m_parentId + ")";
    else return null;
  }
  ...
  public boolean equals(Object object) {
    ...
    if (object instanceof PSMappingElement) {
      ...
      if (!getId().equals(element.getId())) isEqual = false;
      else if (!getName().equals(element.getName())) isEqual = false;
      else if (!getType().equals(element.getType())) isEqual = false;
      else if (hasParent() && !element.hasParent()) isEqual = false;
      else if (!hasParent() && element.hasParent()) isEqual = false;
      else if (hasParent()) {
        if (!m_parentId.equals(element.m_parentId)) isEqual = false;
        else if (!m_parentName.equals(element.m_parentName)) isEqual = false;
        else if (!m_parentType.equals(element.m_parentType)) isEqual = false;
      }
    } else isEqual = false;
  ...
  public int hashCode() {
    ...
    if (hasParent()) {
      hashCode += hashCodeOr0(m_parentId) + hashCodeOr0(m_parentName) + hashCodeOr0(m_parentType);
    }
    return hashCode;
  }
  ...
  /**
   * Compare's this object <code>toString()</code> representation lexicographically ignoring case.
   * See {@link java.lang.Comparable#compareTo(Object) } for more information.
   *
  ...
  /**
   * The element id, initialized in the constructor and never <code>null</code> or empty or modified
   * after that.
   */
  ...
  /**
   * The element's parent id, may be <code>null</code> if {@link #setParent(String, String, String)}
   * is not called and modified through the same call.
   */
  private String m_parentId = null;

  /**
   * The element's parent name, may be <code>null</code> if {@link #setParent(String, String,
   * String)} is not called and modified through the same call.
   */
  private String m_parentName = null;

  /**
   * The element's parent type, may be <code>null</code> if {@link #setParent(String, String,
   * String)} is not called and modified through the same call.
   */
