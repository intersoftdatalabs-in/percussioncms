...
public class PSDependencyMap {
   ...
   */
  public PSDependencyMap(Iterator<PSDependencyDef> defs) throws PSDeployException {
    if (defs == null) throw new IllegalArgumentException("defs may not be null");

    while (defs.hasNext()) {
      PSDependencyDef obj = defs.next();
      if (obj == null)
        throw new IllegalArgumentException("defs may not contain <code>null</code> entries");
  ...
  PSDependencyMap(Element sourceNode, boolean buildDepMaps) throws PSUnknownNodeTypeException {
    ...
    Element defEl =
        tree.getNextElement(PSDependencyDef.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    ...
    while (defEl != null) {
      ...
      defEl =
          tree.getNextElement(
              PSDependencyDef.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }

    if (buildDepMaps) buildDependencyMaps();
  }
  ...
  public PSDependencyHandler getDependencyHandler(PSDependencyDef def) {
    ...
    PSDependencyHandler handler = m_handlerMap.get(def.getObjectType());
    if (handler == null) throw new IllegalArgumentException("def not found in this map");
  ...
  public Iterator<PSDependencyDef> getChildDependencyTypes(PSDependencyDef def) {
    ...
    if (childList == null) throw new IllegalArgumentException("def not found in this map");
  ...
  private void buildDependencyMaps() {
    ...
    while (defs.hasNext()) {
      try {
        ...
        PSDependencyHandler handler = PSDependencyHandler.getHandlerInstance(def, this);
        ...
        while (childTypes.hasNext()) {
          try {
            ...
            List<PSDependencyDef> parentList =
                m_parentDefMap.computeIfAbsent(childType, k -> new ArrayList<>());
            parentList.add(def);
          } catch (Exception e) {
