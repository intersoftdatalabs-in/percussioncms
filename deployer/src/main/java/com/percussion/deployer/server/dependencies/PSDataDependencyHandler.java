...
public class PSDataDependencyHandler extends PSSchemaDependencyHandler {
   ...
   */
  public PSDataDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap)
  ...
  public Iterator getDependencyFiles(PSSecurityToken tok, PSDependency dep)
    ...
    files.add(new PSDependencyFile(PSDependencyFile.TYPE_DBMS_SCHEMA, schemaFile));

    if (data != null) {
      doc = PSXmlDocumentBuilder.createXmlDocument();
      PSXmlDocumentBuilder.replaceRoot(doc, data.toXml(doc));
      File dataFile = createXmlFile(doc);
      files.add(new PSDependencyFile(PSDependencyFile.TYPE_DBMS_DATA, dataFile));
    }
  ...
  public void installDependencyFiles(
    ...
    try {
      fileType = PSDependencyFile.TYPE_ENUM[PSDependencyFile.TYPE_DBMS_SCHEMA];
      schema = new PSJdbcTableSchema(schemaDoc.getDocumentElement(), typeMap);

      if (dataDoc != null) {
        fileType = PSDependencyFile.TYPE_ENUM[PSDependencyFile.TYPE_DBMS_DATA];
    ...
    PSDbmsHelper dbmsHelper = PSDbmsHelper.getInstance();
    if (exists && data != null) {
      if (schema.getPrimaryKey() != null) data = setDataForReplace(data);
    ...
    // install the data
    if (data != null) dbmsHelper.processTable(schema, data);
    else dbmsHelper.processTable(schema);

    addTransactionLogEntry(
        dep, ctx, dep.getDisplayIdentifier(), PSTransactionSummary.TYPE_DATA, transAction);
  }
  ...
  private PSJdbcTableData setDataForReplace(PSJdbcTableData srcData) throws PSDeployException {
    ...
    while (rows.hasNext()) {
      ...
      PSJdbcRowData tgtRow = new PSJdbcRowData(srcRow.getColumns(), PSJdbcRowData.ACTION_REPLACE);
      tgtRowList.add(tgtRow);
    }

    PSJdbcTableData newData = new PSJdbcTableData(srcData.getName(), tgtRowList.iterator());
