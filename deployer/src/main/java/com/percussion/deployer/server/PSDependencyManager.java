...
public class PSDependencyManager implements IPSDependencyManagerBaseline {
  ...
  private PSDependencyManager() {
    ...
    try {
      ...
      log.error(
          "Failed to load config file from {}. Error: {}",
          configDir,
  ...
  private void loadConfigFiles(String dir, boolean buildDepMaps) throws PSDeployException {
    ...
    try (FileInputStream in = new FileInputStream(depMapFile)) {
      ...
      PSPackageConfiguration config =
          new PSPackageConfiguration(doc.getDocumentElement(), buildDepMaps);
  ...
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
  ...
  public Iterator<IPSDependencyBaseline> getAncestors(
    ...
    if (dep.getDependencyType() == PSDependency.TYPE_USER) return Collections.emptyIterator();
  ...
  public void addToArchive(
      ...
      PSArchiveHandler archiveHandler,
      IPSJobHandle jobHandle)
      throws PSDeployException, PSNotFoundException {
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
    ...
    if (jobHandle == null) throw new IllegalArgumentException("jobHandle may not be null");
    ...
    if (dependency.isIncluded()) {
      ...
      if (!archiveHandler.hasDependencyFiles(dependency)) {
        ...
        if (files.hasNext()) archiveHandler.addFiles(dependency, files);
        ...
        List<PSDatasourceMap> dbmsInfoList = handler.getExternalDbmsInfoList(tok, dependency);
        if (dbmsInfoList != null) archiveHandler.addDbmsInfoList(dependency, dbmsInfoList);
      }
    }
  ...
  private void addPkgElement(OrderedElement[] orderList, PSImportPackage pkgElement)
    ...
    for (OrderedElement elem : orderList) {
      if (elem.mi_objType.equals(objType)) elem.mi_elements.add(pkgElement);
    }
  }
  ...
  public List<PSImportPackage> reorderDeployedElements(List<PSImportPackage> deployList)
    ...
    if (deployList == null || deployList.isEmpty())
      throw new IllegalArgumentException("deployList may not be null or empty.");
  ...
  private void logPackageElementOrder(List<PSImportPackage> elements, boolean isOriginal)
    ...
    if (isOriginal) log.debug("Original package element order:");
    else log.debug("New package element order:");

    for (PSImportPackage elem : elements) {
      log.debug(
          "\t objectType=\"{}" + "\", displayName=\"{}" + "\".",
          getObjectType(elem),
          elem.getPackage().getDisplayName());
    }
  }
  ...
  public void restoreFromArchive(
      PSSecurityToken tok,
      PSDeployableElement dependency,
      PSArchiveHandler archiveHandler,
      PSImportCtx ctx,
      IPSJobHandle jobHandle)
    ...
    if (curMap != null) reserveNewIds(dependency, curMap);
    ...
    try {
      ...
      if (deps != null) {
        while (deps.hasNext() && !jobHandle.isCancelled()) {
          ...
          restoreDependencyFromArchive(
              tok, child, dependency, archiveHandler, ctx, deferList, jobHandle);
        }
      }
      ...
      while (deps.hasNext() && !jobHandle.isCancelled()) {
        ...
        installDependency(tok, dep, depHandler, archiveHandler, ctx, jobHandle, dependency);
      }
    } finally {
      // now save idMap if we had one
      if (curMap != null) ctx.getIdMapMgr().saveIdMap(curMap);
    }
  }
   ...
   * @throws PSDeployException if there are any other errors.
   */
  private void restoreDependencyFromArchive(
      PSSecurityToken tok,
      PSDependency dependency,
      PSDeployableElement root,
      PSArchiveHandler archiveHandler,
      PSImportCtx ctx,
      List<PSDependency> deferList,
      IPSJobHandle jobHandle)
      throws PSDeployException, PSAssemblyException, PSNotFoundException {
    // if we hit an element down in the tree, we are done
    if (dependency instanceof PSDeployableElement) return;
    ...
    if (isIncluded && !isLocal) {
      ...
      if (valResults != null) {
        ...
        if (result != null && result.isSkip()) isIncluded = false;
      }
    }
    ...
    // parent)
    if (isIncluded && (!isLocal || root.includesDependency(dependency, true))) {
      PSDependencyDef def = getDependencyDef(dependency.getObjectType());
      PSDependencyHandler depHandler = m_depMap.getDependencyHandler(def);
      if (depHandler.shouldDeferInstallation()) deferList.add(dependency);
      else if (!jobHandle.isCancelled()) {
        installDependency(tok, dependency, depHandler, archiveHandler, ctx, jobHandle, root);
      }
    }
    ...
    if (deps != null) {
      while (deps.hasNext() && !jobHandle.isCancelled()) {
        PSDependency child = deps.next();
        restoreDependencyFromArchive(tok, child, root, archiveHandler, ctx, deferList, jobHandle);
      }
    }
  }
  ...
  private void installDependency(
    ...
    if (!ctx.isDependencyInstalled(dependency, root)) {
      ...
      if (ctx.isDependencyInstalled(dependency)) {
        depHandler.addTransactionLogEntry(
            dependency,
            ctx,
        ...
        try {
          ...
          if (idMap != null && supportsIdMapping) {
            PSIdMapping depMapping = getIdMapping(dependency, ctx.getCurrentIdMap());
            if (depMapping != null) depId = depMapping.getTargetId();
          }

          if (!dependency.supportsParentId()
              && !dependency.getObjectType().equals(PSAclDefDependencyHandler.DEPENDENCY_TYPE)) {
            // Save package element data to database
            IPSPkgInfoService pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
            PSPkgElement pkgElem = pkgService.createPkgElement(ctx.getPkgGuid());

            IPSGuid pkgElemGuid =
                PSIdNameHelper.getGuid(depId, getGuidType(dependency.getObjectType()));
          ...
          if (idTypes != null) {
            ...
            if (idMap != null && supportsIdMapping) {
              String targetId = getIdMapping(dependency, idMap).getTargetId();
              if (targetId != null) {
                PSDependency clonedDependency = (PSDependency) idTypes.getDependency().clone();
          ...
          if (dependency instanceof PSUserDependency) {
            ...
            if (!userDep.getParentType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM)) {
  ...
  private String formatDependencyString(PSDependency dependency) {
    ...
    Object[] args = {
      dependency.getObjectTypeName(), dependency.getDependencyId(), dependency.getDisplayName()
    };
  ...
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok, String type)
    ...
    if (result == null) {
      ...
      if (toker.hasMoreTokens()) {
        ...
        if (tmpType.equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM) && toker.hasMoreTokens()) {
      ...
      if (custType != null) {
        ...
        if (custType.equals(PSUserDependency.USER_DEPENDENCY_TYPE)) {
          ...
          if (userDep != null) customDeps.add(userDep);
        } else {
          // get all child deps of the specified custom type
          PSDependencyHandler childHandler =
              m_depMap.getDependencyHandler(getDependencyDef(custType));
          ...
          if (childDeps != null) {
            while (childDeps.hasNext()) {
              try {
                ...
                PSDependency dep = handler.getDependency(tok, childDep.getKey());
                if (dep != null) customDeps.add(dep);
              } catch (PSDeployException e) {
                log.error(PSExceptionUtils.getMessageForLog(e));
   ...
   * @throws PSDeployException if there are any other errors.
   */
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok, String type, String parentId)
      throws PSDeployException {
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
    ...
    if (m_parentTypeMap == null) {
      ...
      if (m_depMap != null) {
        ...
        while (defs.hasNext()) {
          ...
          if (def.supportsParentId()) {
            PSDependencyHandler handler = PSDependencyHandler.getHandlerInstance(def, m_depMap);
  ...
  public void addMissingDependencies(
      ...
      PSDependencyTreeContext treeCtx,
      IPSJobHandle jobHandle)
      throws PSDeployException, PSNotFoundException {
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
   ...
   */
  public void validatePackages(PSSecurityToken tok, PSExportDescriptor exportDesc)
      throws PSDeployException, PSNotFoundException {
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
    if (exportDesc == null) throw new IllegalArgumentException("exportDesc may not be null");
    ...
    try {
      ...
      while (pkgs.hasNext()) {
        ...
        PSDependencyHandler depHandler = PSDependencyHandler.getHandlerInstance(depDef, m_depMap);

        PSDependency curPkg = depHandler.getDependency(tok, pkg.getDependencyId());
        if (curPkg == null) missingPkgNames.add(pkg.getDisplayName());
        else {
          ...
          if (checkModifiedDependencies(tok, pkg)) modPkgNames.add(pkg.getDisplayName());
          exportPkgs.add(pkg);
        }
      }
      ...
      if (!missingPkgNames.isEmpty()) exportDesc.addMissingPackages(missingPkgNames.iterator());

      if (!modPkgNames.isEmpty()) exportDesc.setModifiedPackages(modPkgNames.iterator());
    } finally {
   ...
   * @throws PSDeployException if there are any errors.
   */
  public PSExportDescriptor convertExportDescriptor(
      PSArchiveInfo info, PSLogHandler logHandler, PSIdMap idMap) throws PSDeployException {
    if (info == null) throw new IllegalArgumentException("info may not be null");

    if (logHandler == null) throw new IllegalArgumentException("logHandler may not be null");

    PSArchiveDetail detail = info.getArchiveDetail();
    if (detail == null)
      throw new IllegalArgumentException("supplied archive info's detail may not be null");
  ...
  public void deleteUserDependency(PSUserDependency dep) {
    ...
    if (depDir.exists()) {
      File depFile = new File(depDir, name + ".xml");
      if (depFile.exists()) depFile.delete();
    }
  }
   ...
   * @throws PSDeployException if there are any errors.
   */
  public PSDependency findDependency(PSSecurityToken tok, String type, String depId)
      throws PSDeployException, PSNotFoundException {
    if (tok == null) throw new IllegalArgumentException(NULL_SECURITY_TOKEN);
    ...
    PSDependencyHandler handler = PSDependencyHandler.getHandlerInstance(depDef, m_depMap);
  ...
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok, int flags)
    ...
    while (defs.hasNext()) {
      ...
      if (((flags & TYPE_SUPPORTS_ID_TYPES) == TYPE_SUPPORTS_ID_TYPES) && !def.supportsIdTypes()) {
        continue;
      }
  ...
  public Iterator<PSDependencyDef> getElementTypes() {
    ...
    if (m_depMap != null) {
      ...
      while (defs.hasNext()) {
        PSDependencyDef def = defs.next();
        if (def.isDeployableElement()
            && !def.getObjectType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM)) {
          types.add(def);
        }
      }
    }
  ...
  public Iterator<PSDependencyDef> getObjectTypes() {
    ...
    while (defs.hasNext()) {
      PSDependencyDef def = defs.next();
      if (!def.isDeployableElement()) types.add(def);
    }
    return types.iterator();
  }
      ...
      if (!def.isDeployableElement() && def.supportsIdMapping()) {
        ...
        if (handler != null) {
          ...
          if (def.supportsParentId()) {
            ...
            // possible parent
            PSDependencyDef parentDef = m_depMap.getDependencyDef(handler.getParentType());
            if (parentDef != null) {
              PSDependencyHandler parentHandler = m_depMap.getDependencyHandler(parentDef);
              Iterator<PSDependency> parentDeps = parentHandler.getDependencies(tok);
              while (parentDeps.hasNext() && !existsForType) {
                PSDependency parentDep = parentDeps.next();
                existsForType = handler.doesDependencyExist(tok, id, parentDep.getDependencyId());
              }
            }
          } else {
          ...
          if (existsForType) types.add(def);
        }
      }
    }
    return types.iterator();
  ...
  public Iterator<PSDependencyDef> getCustomElementTypes() throws PSDeployException {
    List<PSDependencyDef> types = new ArrayList<>();
    if (m_depMap != null) {
      PSDependencyDef custDef =
          m_depMap.getDependencyDef(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM);
      if (custDef != null) // should never be null
      {
        PSDependencyHandler custHandler = PSDependencyHandler.getHandlerInstance(custDef, m_depMap);
        ...
        while (childTypes.hasNext()) {
          ...
          if (childDef != null) types.add(childDef);
        }
      }
    }
    return types.iterator();
  ...
  IPSIdTypeHandler getIdTypeHandler(PSDependency dep) throws PSDeployException {
    ...
    if (!dep.supportsIdTypes())
      throw new IllegalArgumentException("dep must support id types: " + dep.getKey());

    PSDependencyDef def = getDependencyDef(dep.getObjectType());
    PSDependencyHandler handler = m_depMap.getDependencyHandler(def);

    if (!(handler instanceof IPSIdTypeHandler))
      throw new IllegalStateException("No ID Type handler found for type: " + dep.getObjectType());
  ...
  private void transformDeps(PSDependency dep, PSIdMap idMap) throws PSDeployException {
    ...
    if (idMap != null && (dep.supportsIDMapping() || handler.delegatesIdMapping())) {
      ...
      if (dep.supportsParentId())
        dep.setParent(mapping.getTargetParentId(), mapping.getParentType());
    }

    // handle child dependencies
    Iterator<PSDependency> childDeps = dep.getDependencies();
    if (childDeps != null) {
      while (childDeps.hasNext()) transformDeps(childDeps.next(), idMap);
    }
  }
  ...
  public PSDependencyDef getDependencyDef(String type) throws PSDeployException {
    if (StringUtils.isBlank(type)) throw new IllegalArgumentException("type may not be null");
   ...
   */
  public PSDependencyHandler getDependencyHandler(PSDependency dep) throws PSDeployException {
      ...
      if (depFiles != null) {
        for (File depFile : depFiles) {
          try (FileInputStream in = new FileInputStream(depFile)) {
            Document doc = PSXmlDocumentBuilder.createXmlDocument(in, false);
            PSUserDependency userDep = new PSUserDependency(doc.getDocumentElement());
  ...
  private void addMissingDependencies(
      ...
      Set<PSDependencyIdentifier> loaded,
      PSDependencyTreeContext treeCtx,
      IPSJobHandle jobHandle)
      throws PSDeployException, PSNotFoundException {
    PSDependencyIdentifier depId = new PSDependencyIdentifier(dep, !checkElements);

    if (loaded.contains(depId) || jobHandle.isCancelled()) return;
    ...
    boolean isParentType = getParentTypes().containsValue(dep.getObjectType());

    PSDependencyContext depCtx = treeCtx.getDependencyCtx(dep);
    if (depCtx == null) {
      // a bug
      throw new IllegalArgumentException("treeCtx must contain the supplied dependency");
    }
    ...
    if (deps == null
        && (depCtx.isIncluded()
            || isParentType
            || (dep instanceof PSDeployableElement && checkElements)
            || recurseLocal)
      ...
      if (dep instanceof PSDeployableElement) checkElements = false;
    }
    ...
    if (deps != null && !jobHandle.isCancelled()) {
      ...
      if (all.size() != included.size()) {
        ...
        deps = dep.getDependencies();
      } else deps = all.iterator();

      // now recurse children
      while (deps.hasNext() && !jobHandle.isCancelled()) {
        PSDependency child = deps.next();
        addMissingDependencies(tok, child, checkElements, loaded, treeCtx, jobHandle);
      }
    }
  }
  ...
  public void updateJobStatus(PSDependency dep, IPSJobHandle jobHandle) {
    ...
    String msg = MessageFormat.format(bundle.getString("processing"), depString);
  ...
  private boolean checkModifiedDependencies(PSSecurityToken tok, PSDependency pkg)
    ...
    if (deps != null) {
      ...
      if (exists) {
        ...
        if (pkg.getObjectType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM)) {
          ...
          List<PSDependency> newList = new ArrayList<>();
          List<PSDependency> depList = PSDeployComponentUtils.cloneList(getDependencies(tok, pkg));
          ...
          while (deps.hasNext()) {
            PSDependency childDep = deps.next();
            PSDependency curDep = depMap.get(childDep.getKey());
            if (curDep == null) {
  ...
  private boolean doesDependencyExist(PSSecurityToken tok, PSDependency dep)
    ...
    if (depDef.supportsParentId())
      exists = depHandler.doesDependencyExist(tok, dep.getDependencyId(), dep.getParentId());
    else exists = depHandler.doesDependencyExist(tok, dep.getDependencyId());
  ...
  public PSDependency getActualDependency(PSSecurityToken tok, PSDependency dep, PSIdMap idMap)
    ...
    if (idMap != null && (dep.supportsIDMapping() || handler.delegatesIdMapping())) {
      ...
      if (!mapping.isNewObject()) {
        ...
        if (!dep.supportsParentId()) result = handler.getDependency(tok, newId);
        else {
          newParentId = mapping.getTargetParentId();
          result = handler.getDependency(tok, newId, dep.getParentType(), newParentId);
        }
      }
    } else {
      if (depDef.supportsParentId())
        result =
            handler.getDependency(
                tok, dep.getDependencyId(), dep.getParentType(), dep.getParentId());
      else result = handler.getDependency(tok, dep.getDependencyId());
    }
  ...
  private PSIdMapping getIdMapping(PSDependency dep, PSIdMap idMap) throws PSDeployException {
    ...
    return handler.getIdMapping(
        idMap, dep.getDependencyId(), dep.getObjectType(), dep.getParentId(), dep.getParentType());
  }
  ...
  public Iterator<IPSDependencyBaseline> getParentDependencies(
    ...
    if (dep.getObjectType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM))
      return Collections.emptyIterator();

    Iterator<PSDependencyDef> defs =
        m_depMap.getParentDependencyTypes(getDependencyDef(dep.getObjectType()));
    while (defs.hasNext()) {
      PSDependencyDef def = defs.next();
      if (!def.canBeAncestor()) continue;

      Iterator<PSDependency> deps = getDependencies(tok, def.getObjectType());
      while (deps.hasNext()) {
        PSDependency parent = deps.next();
        if (isChild(tok, dep, parent)) parentDeps.add(parent);
      }
    }
  ...
  private boolean isChild(PSSecurityToken tok, PSDependency child, PSDependency parent)
    ...
    // not supported for custom
    if (parent.getObjectType().equals(IPSDeployConstants.DEP_OBJECT_TYPE_CUSTOM)) {
      return isChild;
    }
    ...
    if (parent.getDependencyType() != PSDependency.TYPE_SYSTEM
      ...
      if (children == null) children = getDependencies(tok, parent);
  ...
  public List<String> getDeploymentType(PSTypeEnum type) throws PSDeployException {
    ...
    if (m_guidToDepTypeMap == null) throw new IllegalStateException("type map not initialized");
  ...
  public PSTypeEnum getGuidType(String deploymentType) {
    ...
    if (m_depToGuidTypeMap == null) throw new IllegalStateException("type map not initialized");
  ...
  private static final File USER_DEP_DIR =
      new File(
          PSServer.getRxDir().getAbsolutePath()
              + File.separatorChar
              + PSDeploymentHandler.OBJECTSTORE_DIR,
          "UserDependencies");
  ...
  private static final class PSDependencyCache {
    ...
    public Iterator<PSDependency> getDependencies(String objectType) {
      ...
      if (m_isEnabled) {
        ...
        if (depList != null) deps = cloneDepList(depList).iterator();
      }
  ...
  private static class PSDependencyIdentifier {
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof PSDependencyIdentifier)) {
        ...
        if (!depId.m_dep.getKey().equals(m_dep.getKey())) isEqual = false;
        else if (depId.m_dep.getDependencyType() != m_dep.getDependencyType()) {
          isEqual = false;
    ...
    @Override
    public int hashCode() {
      return m_dep.getKey().hashCode()
          + m_dep.getDependencyType()
