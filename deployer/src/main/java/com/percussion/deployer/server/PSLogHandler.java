...
public class PSLogHandler {
  ...
  /**
   * Gets all <code>PSArchiveSummary</code> objects from the log tables for a given target server.
   *
   * @param tgtServer The target server, which is the combination of hostname and port number, as in
   *     "hostname":"port". It may not be <code>null</code> or empty.
   * @return An iterator over zero or more <code>PSArchiveSummary</code> objects. It will never be
   *     <code>null</code>, but may be empty.
   * @throws IllegalArgumentException if <code>tgtServer</code> is invalid.
   * @throws PSDeployException if any other error occurs.
   */
  public Iterator getArchiveSummaries(String tgtServer) throws PSDeployException {
  ...
  /**
   * Gets all <code>PSArchiveSummary</code> objects from the log tables for a given target server,
   * and optionally for a specific archive ref.
   *
   * @param tgtServer The target server, which is the combination of hostname and port number, as in
   *     "hostname":"port". May be <code>null</code> to use the local server name and port, never
   *     empty. or empty.
  ...
  public Iterator getArchiveSummaries(String tgtServer, String archiveRef)
    ...
    if (tgtServer == null) tgtServer = PSServer.getHostName() + ":" + PSServer.getListenerPort();

    PSJdbcSelectFilter filter =
        new PSJdbcSelectFilter(
            ALS_TGT_SERVER_NAME, PSJdbcSelectFilter.EQUALS, tgtServer, Types.VARCHAR);
    ...
    if (archiveRef != null) {
      ...
      filter =
          new PSJdbcSelectFilter(
              ALS_ARCHIVE_REF, PSJdbcSelectFilter.EQUALS, archiveRef, Types.VARCHAR);
      container.doAND(filter);
      filter = container;
    }

    PSJdbcTableData tData = m_dbmsHandle.catalogTableData(m_archiveLogSummarySchema, null, filter);
   ...
   *     retrieved. May not be <code>null</code> or empty, and must refer to an existing archive
   *     file on the server.
   * @return The retrieved archive summary object. It may be <code>null</code> if cannot find a
   *     record with the given archive reference.
  ...
  public PSArchiveSummary getArchiveSummary(String archiveRef) throws PSDeployException {
    ...
    if (tData != null) {
      ...
      while (rows.hasNext()) {
        PSJdbcRowData row = (PSJdbcRowData) rows.next();
        Date date = m_dbmsHandle.getColumnDate(ALS_TABLE_NAME, ALS_INSTALL_DATE, row);
  ...
  /**
   * Gets a <code>PSArchiveSummary</code> object, which is specified by a given identifier of the
   * archive summary table.
   *
   * @param archiveId The identifier of the archive summary table.
   * @return The specified <code>PSArchiveSummary</code> object. It may be <code>null</code> if
   *     cannot find in the archive log summary table.
   * @throws PSDeployException if any error occurs.
   */
  public PSArchiveSummary getArchiveSummary(int archiveId) throws PSDeployException {
   ...
   *
   * @param row The row data, retrieved record from archive log summary table, assume not <code>null
   *     </code>.
   * @return The retrieved archive summary object, never <code>null</code>.
   * @throws PSDeployException if any error occurs
   */
  private PSArchiveSummary getArchiveSummary(PSJdbcRowData row) throws PSDeployException {
    ...
    PSArchiveInfo archiveInfo =
        (PSArchiveInfo) getColumnClob(ALS_TABLE_NAME, ALS_ARCHIVE_INFO, row, PSArchiveInfo.class);
    PSArchiveManifest archiveMan =
        (PSArchiveManifest)
            getColumnClob(ALS_TABLE_NAME, ALS_ARCHIVE_MANIFEST, row, PSArchiveManifest.class);

    Iterator pkgList = getArchivePackages(archiveId);

    PSArchiveSummary arSummary = new PSArchiveSummary(archiveInfo, dateTime, pkgList);
  ...
  /**
   * Gets a list of archive packages from the archive package table for the specified archive
   * summary id.
   *
   * @param archiveId The archive summary id.
   * @return An iterator over zero or more <code>PSArchivePackage</code> objects, never <code>null
  ...
  private Iterator getArchivePackages(int archiveId) throws PSDeployException {
    ...
    PSJdbcTableData tData = m_dbmsHandle.catalogTableData(m_archivePackageSchema, null, filter);

    if (tData != null) {
      Iterator rows = tData.getRows();
      while (rows.hasNext()) {
        PSJdbcRowData row = (PSJdbcRowData) rows.next();
        String name = m_dbmsHandle.getColumnString(AP_TABLE_NAME, AP_PACKAGE_NAME, row);
        String type = m_dbmsHandle.getColumnString(AP_TABLE_NAME, AP_PACKAGE_TYPE, row);
        ...
        PSArchivePackage pkg = new PSArchivePackage(name, type, status, logId);
        pkgList.add(pkg);
      }
    }
  ...
  /**
   * Gets all log summary objects from the log tables for a given target server.
   *
   * @param tgtServer The target server, which is the combination of hostname and port number, as in
   *     "hostname":"port". It may not be <code>null</code> or empty.
   * @return An iterator over zero or more <code>PSLogSummary</code> objects. It will not be <code>
   *     null</code>, but may be empty. The summaries will not contain <code>PSLogDetail</code>
   *     objects.
  ...
  /**
   * Gets a list of log summary records (in a table data from the log summary table), where the
   * target server column of each record equals the specified <code>tgtServer</code>.
   *
   * @param tgtServer The name of the target server, assume not <code>null</code> or empty.
   * @return The table data object, it may be <code>null</code> if cannot find any qualified record.
   *     Will not contain the columns required to create a {@link PSLogDetail} object.
   * @throws PSDeployException if an error occurs.
   */
  private PSJdbcTableData getLogSummariesTableData(String tgtServer) throws PSDeployException {
    ...
    PSJdbcTableData tData =
        m_dbmsHandle.catalogTableData(
    ...
    if (tData != null) {
      ...
      while (rows.hasNext()) {
        PSJdbcRowData row = (PSJdbcRowData) rows.next();
        archiveId = m_dbmsHandle.getColumnString(ALS_TABLE_NAME, ALS_ARCHIVE_LOG_ID, row);
        if (sArchiveIds.length() == 0) sArchiveIds.append("(" + archiveId);
        else sArchiveIds.append("," + archiveId);
      }
      sArchiveIds.append(")");
    }
    if (sArchiveIds.length() != 0) {
      ...
      filter =
          new PSJdbcSelectFilter(
              LS_ARCHIVE_LOG_ID, PSJdbcSelectFilter.IN, sArchiveIds.toString(), Types.INTEGER);
  ...
  /**
   * Get a log summary object, which is specified by an id of the log summary table.
   *
   * @param logId The id used to locate the log in the repository.
   * @param includeDetail <code>true</code> to include the {@link PSLogDetail}, <code>false</code>
   *     if not.
   * @return The specified <code>PSLogSummary</code> object. It may be <code>null</code> if cannot
   *     find any from the log summary table.
   * @throws PSDeployException if any error occurs.
   */
  public PSLogSummary getLogSummary(int logId, boolean includeDetail) throws PSDeployException {
    PSLogSummary logSummary = null;

    PSJdbcSelectFilter filter =
        new PSJdbcSelectFilter(
            LS_LOG_SUMMARY_ID, PSJdbcSelectFilter.EQUALS, Integer.toString(logId), Types.INTEGER);

    String[] cols = includeDetail ? null : LS_SUM_COLS_NO_DETAIL;
    PSJdbcTableData tData = m_dbmsHandle.catalogTableData(m_logSummarySchema, cols, filter);
  ...
  /**
   * Creates a log summary object from a row data, which is retrieved from the log summary table.
   *
   * @param row The row data, assume not <code>null</code>.
   * @param includeDetail <code>true</code> to load the {@link PSLogDetail}, <code>false</code> if
   *     not. If <code>true</code>, the <code>row</code> is assumed to contain the required column
   *     data.
  ...
  private PSLogSummary getLogSummary(PSJdbcRowData row, boolean includeDetail)
    ...
    PSDeployableElement pkg =
        (PSDeployableElement)
            getColumnClob(LS_TABLE_NAME, LS_PACKAGE_DETAIL, row, PSDeployableElement.class);
    ...
    if (includeDetail) {
      ...
      PSDbmsMap dbmsMap =
          (PSDbmsMap) getColumnClobNullable(LS_TABLE_NAME, LS_DBMS_MAP, row, PSDbmsMap.class);

      // get the id map, which could be null
      PSIdMap idMap = (PSIdMap) getColumnClobNullable(LS_TABLE_NAME, LS_ID_MAP, row, PSIdMap.class);
      ;

      PSValidationResults vr =
          (PSValidationResults)
              getColumnClob(LS_TABLE_NAME, LS_VALIDATION_RESULTS, row, PSValidationResults.class);
  ...
  /**
   * Gets a <code>PSTransactionLogSummary</code> object, which is specified by an id of the log
   * summary table.
   *
   * @param logId The id of the log summary table.
   * @return The specified <code>PSTransactionLogSummary</code> object. It will never be <code>null
   *     </code>, but may be empty if cannot find any in the log transaction table.
   * @throws PSDeployException if any error occurs.
   */
  private PSTransactionLogSummary getTxnLogSummary(int logId) throws PSDeployException {
    PSTransactionLogSummary txns = new PSTransactionLogSummary();

    PSJdbcSelectFilter filter =
        new PSJdbcSelectFilter(
            TXN_LOG_SUMMARY_ID, PSJdbcSelectFilter.EQUALS, Integer.toString(logId), Types.INTEGER);

    PSJdbcTableData tData = m_dbmsHandle.catalogTableData(m_logTxnSchema, null, filter);

    if (tData != null) {
      // build sorted map
      TreeMap<Integer, PSTransactionSummary> txnMap = new TreeMap<>();
      boolean hasSeq = true;
      int seq = 0;
      ...
      while (rows.hasNext()) {
        ...
        String depDesc = m_dbmsHandle.getColumnString(TXN_TABLE_NAME, TXN_DEPENDENCY, row);
        String name = m_dbmsHandle.getColumnString(TXN_TABLE_NAME, TXN_ELEMENT_NAME, row);
        String type = m_dbmsHandle.getColumnString(TXN_TABLE_NAME, TXN_ELEMENT_TYPE, row);
        int action = m_dbmsHandle.getColumnInt(TXN_TABLE_NAME, TXN_ACTION_TOOK, row);

        PSTransactionSummary txn = new PSTransactionSummary(logId, depDesc, name, type, action);
   ...
   * @param row The row data, which contains XML document (as clob) of the to be retrieved object,
   *     assume not <code>null</code>.
   * @param compClass The <code>Class</code> of the to be retrieved object, assume never <code>null
   *     </code>.
   * @return The <code>IPSDeployComponent</code> object, will never be <code>null</code>.
   * @throws PSDeployException if any error occurs.
   */
  private IPSDeployComponent getColumnClob(
      String table, String column, PSJdbcRowData row, Class compClass) throws PSDeployException {
    ...
    try {
      Document resultDoc = PSXmlDocumentBuilder.createXmlDocument(sReader, false);
      Element root = resultDoc.getDocumentElement();

      Constructor compCtor = compClass.getConstructor(new Class[] {Element.class});
      component = (IPSDeployComponent) compCtor.newInstance(new Object[] {root});
    } catch (Exception e) {
  ...
  /**
   * Gets a <code>IPSDeployComponent</code> object from the given parameters. It does the same as
   * the <code>getColumnClob()</code>, except this method accept the value of the given column to be
   * <code>null</code>.
   *
   * @return A <code>IPSDeployComponent</code> object, it may be <code>null</code> if the specified
   *     column contains <code>null</code> value
   *     <p>See {@link #getColumnClob(String, String, PSJdbcRowData, Class)} for detail info.
   */
  private IPSDeployComponent getColumnClobNullable(
      String table, String column, PSJdbcRowData row, Class compClass) throws PSDeployException {
  ...
  /**
   * Deletes a row in the archive log summary table, where the <code>ARCHIVE_LOG_ID</code> column
   * equals <code>archiveId</code>..
   *
  ...
  private void deleteFromArchiveSummary(int archiveId) throws PSDeployException {
    deleteArchiveIdFromTable(
        archiveId, ALS_ARCHIVE_LOG_ID, ALS_TABLE_NAME, m_archiveLogSummarySchema);
  }

  /**
   * Deletes all rows in the archive package table, where the <code>ARCHIVE_LOG_ID</code> column of
   * each row equals <code>archiveId</code>.
   *
   * @param archiveId The id of the archive summary.
   * @throws PSDeployException if an error occurs.
   */
  private void deleteFromArchivePackage(int archiveId) throws PSDeployException {
    deleteArchiveIdFromTable(
        archiveId, AP_ARCHIVE_LOG_ID, AP_TABLE_NAME, m_archivePackageDeleteSchema);
  }

  /**
   * Deletes all rows (in the given <code>table</code>), where the <code>column</code> of each row
   * is <code>archiveId</code>.
   *
   * @param archiveId The id of the archive summary.
   * @param column The column name, assume not <code>null</code> or empty.
   * @param table The table name, assume not <code>null</code> or empty.
   ...
   * @throws PSDeployException if an error occurs.
   */
  private void deleteArchiveIdFromTable(
      int archiveId, String column, String table, PSJdbcTableSchema schema)
      throws PSDeployException {
    List<PSJdbcRowData> rowList = new ArrayList<>();

    PSJdbcRowData rowData =
        m_dbmsHandle.getRowDataForOneColumn(
            column, Integer.toString(archiveId), PSJdbcRowData.ACTION_DELETE);
    rowList.add(rowData);
  ...
  /**
   * Deletes all archive logs and their related package logs for the specified archive.
   *
   * @param archiveRef The name of the archive on the server, may not be <code>null</code> or empty.
  ...
  /**
   * Deletes the specified records from both log summary and log transaction tables for a id of the
   * log summary. Optionally, if the deleted package log is the last one referencing its archive
   * summary, the summary is deleted as well.
   *
   * @param logId The id of the log summary.
   * @param cascade If <code>true</code>, the delete is cascaded to the archive summary and archive
   *     package entries, otherwise only the log summary and log transaction entries are deleted.
   * @return <code>null</code> if the deletion only happened in log summary and transaction tables;
   *     archive reference (a non-empty <code>String</code>) if the deletion also affected archive
   *     summary and package tables.
   * @throws PSDeployException if any error occurs.
   */
  ...
  /**
   * Determines if a given id of archive summary exists in the <code>LS_ARCHIVE_LOG_ID</code> column
   * in the log summary table.
   *
   * @param archiveId The id of archive summary.
   * @return <code>true</code> if found a row contains the searched id; <code>false</code>
   *     otherwise.
   * @throws PSDeployException if any error occurs.
   */
  private boolean archiveIdExistInLogSummary(int archiveId) throws PSDeployException {
    PSJdbcSelectFilter filter =
        new PSJdbcSelectFilter(
            LS_ARCHIVE_LOG_ID,
            PSJdbcSelectFilter.EQUALS,
    ...
    PSJdbcTableData tData =
        m_dbmsHandle.catalogTableData(m_logSummarySchema, new String[] {LS_LOG_SUMMARY_ID}, filter);
   ...
   *
   * @param archiveRef The to be searched archive reference, may not be <code>null</code> or empty.
   * @return <code>true</code> if the archive reference exist in the archive summary table; <code>
   *     false</code> otherwise.
   * @throws IllegalArgumentException if <code>archiveRef</code> is invalid.
   * @throws PSDeployException if any error occurs.
   */
  public boolean doesArchiveRefExist(String archiveRef) throws PSDeployException {
    if (archiveRef == null || archiveRef.trim().length() == 0)
      throw new IllegalArgumentException("archiveRef may not be null or empty");
  ...
  /**
   * Deletes the specified records from log summary table for a id of the log summary.
   *
   * @param logId The id of the log summary.
   * @throws PSDeployException if any error occurs.
   */
  private void deleteInLogSummary(int logId) throws PSDeployException {
    ...
    PSJdbcRowData rowData =
        m_dbmsHandle.getRowDataForOneColumn(
            LS_LOG_SUMMARY_ID, Integer.toString(logId), PSJdbcRowData.ACTION_DELETE);

    m_dbmsHandle.processTable(m_logSummarySchema, LS_TABLE_NAME, rowData);
  }

  /**
   * Deletes the specified records from log transaction table for a id of the log summary.
   *
   * @param logId The id of the log summary.
   * @throws PSDeployException if any error occurs.
   */
  private void deleteInLogTxn(int logId) throws PSDeployException {
    PSJdbcRowData rowData =
        m_dbmsHandle.getRowDataForOneColumn(
            TXN_LOG_SUMMARY_ID, Integer.toString(logId), PSJdbcRowData.ACTION_DELETE);
  ...
  /**
   * Inserts the given parameters into the archive summary and archive package tables.
   *
   * @param archiveInfo The to be inserted archive information. It may not be <code>null</code>, it
   *     should also contain the archive detail.
   * @param archiveManifest The to be inserted archive manifest. It may not be <code>null</code>.
  ...
  public int createArchiveLog(PSArchiveInfo archiveInfo, PSArchiveManifest archiveManifest)
    ...
    if (archiveInfo == null || archiveInfo.getArchiveDetail() == null)
      throw new IllegalArgumentException("archiveInfo and its getArchiveDetail() may not be null");
    ...
    PSJdbcTableData tableData =
        getTableDataForAddPackages(id, archiveInfo.getArchiveDetail().getPackages());
    m_dbmsHandle.processTable(m_archivePackageSchema, tableData);

    // insert to the archive sumary table afterwards
    tableData = getTableDataForSaveArchiveSummary(id, savedInfo, archiveManifest);
  ...
  /**
   * Creates <code>PSJdbcTableData</code> object for inserting to archive summary table from the
   * given parameters.
   *
   * @param id The id of the archive log summary table.
   * @param archiveInfo The archive info object, assume not <code>null</code>.
   * @param archiveManifest The archive manifest object, assume not <code>null</code>.
   * @return The to be inserted <code>PSJdbcTableData</code> object. It will never be <code>null
   *     </code>.
   */
  private PSJdbcTableData getTableDataForSaveArchiveSummary(
    ...
    PSJdbcColumnData col = new PSJdbcColumnData(ALS_ARCHIVE_LOG_ID, Integer.toString(id));
    cols.add(col);
    ...
    col = new PSJdbcColumnData(ALS_SRC_SERVER_NAME, archiveInfo.getServerName());
    cols.add(col);

    col = new PSJdbcColumnData(ALS_SRC_SERVER_VERSION, archiveInfo.getServerVersion());
    cols.add(col);
    ...
    col = new PSJdbcColumnData(ALS_ARCHIVE_INFO, PSXmlDocumentBuilder.toString(infoEl));
    cols.add(col);

    Element element = archiveManifest.toXml(doc);
    col = new PSJdbcColumnData(ALS_ARCHIVE_MANIFEST, PSXmlDocumentBuilder.toString(element));
    cols.add(col);

    PSJdbcRowData rData = new PSJdbcRowData(cols.iterator(), PSJdbcRowData.ACTION_INSERT);
  ...
  public void updatePackageStatus(
      int archiveId, int logId, String pkgType, String pkgId, int status) throws PSDeployException {
    if (archiveId < 0) throw new IllegalArgumentException("archiveId may not be < 0");
    if (logId < 0) throw new IllegalArgumentException("logId may not be < 0");
    ...
    updatePkgStatusForArchivePackage(archiveId, pkgType, pkgId, status, logId);
  }
   ...
   * @param status The to be updated status for the package. Assume is one of the <code>
   *     PSArchivePackage.STATUS_XXX</code> values.
   * @throws PSDeployException if an error occurs.
   */
  private void updatePkgStatusForLogSummary(int logId, String pkgType, String pkgId, int status)
      throws PSDeployException {
    // prepare the row data
    List<PSJdbcColumnData> cols = new ArrayList<>();
    PSJdbcColumnData col = new PSJdbcColumnData(LS_LOG_SUMMARY_ID, Integer.toString(logId));
    cols.add(col);
    ...
    PSJdbcRowData rowData = new PSJdbcRowData(cols.iterator(), PSJdbcRowData.ACTION_UPDATE);
  ...
  /**
   * Updates a package status for archive summary table from the given parameters.
   *
   ...
   * @throws PSDeployException if an error occurs.
   */
  private void updatePkgStatusForArchivePackage(
      int archiveId, String pkgType, String pkgId, int status, int logId) throws PSDeployException {
    // prepare the row data
    List<PSJdbcColumnData> cols = new ArrayList<>();
    PSJdbcColumnData col = new PSJdbcColumnData(AP_ARCHIVE_LOG_ID, Integer.toString(archiveId));
    cols.add(col);
   ...
   *
   * @param archiveLogId The id of the archive log summary table. It may not be less than 0.
   * @param pkg The package object, may not be <code>null</code>, and it must contains a non-<code>
   *     null</code> valitation results object.
   ...
   * @throws PSDeployException if any error occurs.
   */
  public int createPackageLog(int archiveLogId, PSImportPackage pkg, PSImportCtx importCtx)
      throws PSDeployException {
    if (archiveLogId < 0) throw new IllegalArgumentException("archiveLogId may not be < 0");
    if (pkg == null) throw new IllegalArgumentException("pkg may not be null");
    if (importCtx == null) throw new IllegalArgumentException("importCtx may not be null");
    ...
    createPackageLog(
        ...
        archiveLogId,
        pkg.getPackage(),
        pkg.getValidationResults(),
        idMap,
        importCtx.getdbmsMap());

    return logId;
  }

  /**
   * Inserts an entry into the log summary table from the given parameters. Does the same as the
   * {@link #createPackageLog(int, PSImportPackage, PSImportCtx)}, but with different set of
   * parameters and nothing to return.
   *
   * @param logId The id of the log summary table.
  ...
  private void createPackageLog(
    ...
    cols.add(col);
    col = new PSJdbcColumnData(LS_STATUS, Integer.toString(PSArchivePackage.STATUS_IN_PROGRESS));
    cols.add(col);
    col = new PSJdbcColumnData(LS_ARCHIVE_LOG_ID, Integer.toString(archiveLogId));
    cols.add(col);

    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    Element element = pkg.toXml(doc);
    col = new PSJdbcColumnData(LS_PACKAGE_DETAIL, PSXmlDocumentBuilder.toString(element));
    cols.add(col);

    element = vr.toXml(doc);
    col = new PSJdbcColumnData(LS_VALIDATION_RESULTS, PSXmlDocumentBuilder.toString(element));
    cols.add(col);

    col = new PSJdbcColumnData(LS_ID_MAP, LS_ID_MAP);
    if (idMap == null) col.setValue(null);
    else col.setValue(PSXmlDocumentBuilder.toString(idMap.toXml(doc)));
    cols.add(col);

    col = new PSJdbcColumnData(LS_DBMS_MAP, LS_DBMS_MAP);
    if (dbmsmap == null) col.setValue(null);
    else col.setValue(PSXmlDocumentBuilder.toString(dbmsmap.toXml(doc)));
    cols.add(col);

    PSJdbcRowData rowData = new PSJdbcRowData(cols.iterator(), PSJdbcRowData.ACTION_INSERT);
  ...
  /**
   * Creates table data object for inserting to archive package table from the given parameters.
   *
   * @param archiveLogId The id of the archive log summary table.
   * @param pkgList The list of <code>PSDependency</code> objects, assume not <code>null</code> or
   *     empty.
   * @return The created table data object, never <code>null</code>.
   */
  private PSJdbcTableData getTableDataForAddPackages(int archiveLogId, Iterator pkgList) {
    ...
    PSJdbcTableData tableData = new PSJdbcTableData(AP_TABLE_NAME, rowDataList.iterator(), false);

    return tableData;
  }

  /**
   * Creates a row data object for inserting to archive package table from the given parameters.
   *
   * @param archiveLogId The id of archive log summary table.
   * @param pkg The <code>PSDependency</code> object, assume not <code>null</code>.
   * @return The row data object, never <code>null</code>.
   */
  private PSJdbcRowData getRowDataForAddPackage(int archiveLogId, PSDependency pkg) {
    List<PSJdbcColumnData> cols = new ArrayList<>();

    PSJdbcColumnData col = new PSJdbcColumnData(AP_ARCHIVE_LOG_ID, Integer.toString(archiveLogId));
    cols.add(col);
    ...
    col = new PSJdbcColumnData(AP_STATUS, Integer.toString(PSArchivePackage.STATUS_IN_PROGRESS));
    cols.add(col);
  ...
  /**
   * Inserts an entry into the log transaction table from the given parameters. This is a unit
   * testable function
   *
   * @param logId The log id, may not be less thans zero.
   * @param depString The string for <code>DEPENDENCY</code> column, it may not be <code>null</code>
   *     or empty.
   * @param elementName The element name, it may not be <code>null</code> or empty.
   * @param elementType The element type, it may not be <code>null</code> or empty.
   * @param action the action value, it must be one of the <code>PSTransactionSummary.ACTION_xxx
   *     </code> values.
   * @param sequence The sequence number of the transaction, used to retrieve the transactions in a
   *     predictable order.
   * @throws IllegalArgumentException if there is any invalid parameters.
   * @throws PSDeployException if any other error occurs.
   */
  public void addTransactionLogEntry(
      int logId, String depString, String elementName, String elementType, int action, int sequence)
      throws PSDeployException {
    if (logId < 0) throw new IllegalArgumentException("logId may not be < 0");
    if (depString == null || depString.trim().length() == 0)
      throw new IllegalArgumentException("depString may not be null or empty");
    ...
    if (elementType == null || elementType.trim().length() == 0)
      throw new IllegalArgumentException("elementType may not be null or empty");
    ...
    // prepare column data
    col = new PSJdbcColumnData(TXN_LOG_SUMMARY_ID, Integer.toString(logId));
    cols.add(col);
  ...
  */
  ...
  /**
   * Array of column names needed to load the log summary without the log detail. See {@link
   * #getLogSummary(PSJdbcRowData, boolean)} for more info.
   */
  private static final String[] LS_SUM_COLS_NO_DETAIL = {
    LS_LOG_SUMMARY_ID, LS_ARCHIVE_LOG_ID, LS_PACKAGE_DETAIL
  };

  /**
   * The schema for log summary table. Initialized by the constructor, it will never be <code>null
   * </code> or modified after that.
   */
  private PSJdbcTableSchema m_logSummarySchema;

  /**
   * The schema for log transaction table. Initialized by the constructor, it will never be <code>
   * null</code> or modified after that.
   */
  private PSJdbcTableSchema m_logTxnSchema;

  /**
   * The schema for archive package table. Initialized by the constructor, it will never be <code>
   * null</code> or modified after that.
   */
  private PSJdbcTableSchema m_archivePackageSchema;

  /**
   * The schema for archive package table, used to delete entries by archive log id. Has its update
   * key set to the archive log id. Initialized by the constructor, it will never be <code>null
   * </code> or modified after that.
   */
  private PSJdbcTableSchema m_archivePackageDeleteSchema;

  /**
   * The schema for archive log summary table. Initialized by the constructor, it will never be
   * <code>null</code> or modified after that.
   */
  private PSJdbcTableSchema m_archiveLogSummarySchema;

  /**
   * The <code>PSDbmsHelper</code> object, used to communicate with the database. It is initialized
   * by constructor, then never modified afterwards.
   */
