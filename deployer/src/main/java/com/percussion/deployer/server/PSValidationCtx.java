...
public class PSValidationCtx {
   ...
   * @param desc The import descriptor being validated, may not be <code>null</code>.
   * @param idMap The current id map, may be <code>null</code> if no transformation is required.
   */
   ...
   * @return <code>true</code> if it has already been validated, <code>false</code> if not.
   */
   ...
   *
   * @param doValidate <code>true</code> to perform ancestor validation, <code>false</code> to skip
   *     it.
   */
   ...
   *
   * @return The id map, may be <code>null</code> if transforms are not required.
   */
   ...
   *
   * @return <code>true</code> if it should, <code>false</code> if it should be skipped.
   */
  ...
  /**
   * Adds a package to this context so that previous packages validation results may retrieved by
   * calls to {@link #getValidationResult(PSDependency)}.
   *
  ...
  /**
   * Determine if the supplied dependency is included in any package within the import descriptor
   * being validated.
   *
   * @param dep The depednecy to check, may not be <code>null</code>.
   * @return <code>true</code> if the archive includes the dependency, <code>false</code> if not.
   */
  public boolean archiveIncludesDependency(PSDependency dep) {
    ...
    if (ctx != null) found = ctx.isIncluded();
  ...
  public PSValidationResult getValidationResult(PSDependency dep) {
    ...
    while (pkgs.hasNext() && result == null) {
      ...
      if (results != null) result = results.getResult(dep);
    }
    return result;
  }

  /**
   * Add an absent ancestor to this context. See {@link #getAbsentAncestors(PSDependency)} for more
   * info.
   *
  ...
  /**
   * Get any absent ancestors added for the specified dependency. Absent ancestors are ancestors of
   * the dependency on the target server that are not included in the dependency's package.
   *
   * @param dep The dep for which ancestors may have been added, may not be <code>null</code>.
   * @return An iterator over zero or more ancestors, never <code>null</code>, may be empty.
   */
  public Iterator getAbsentAncestors(PSDependency dep) {
    Iterator ancs;
    List ancList = (List) m_absentAncs.get(dep.getKey());
    if (ancList == null) ancs = PSIteratorUtils.emptyIterator();
    else ancs = ancList.iterator();

    return ancs;
  }

  /** The jobhandle supplied during construction, never <code>null</code> or modified after that. */
  private IPSJobHandle m_jobHandle;

  /**
   * Full tree context containing all packages from the import descriptor supplied during
   * construction. Used to validate ancestor inclusion in the archive. Never <code>null</code> or
   * modified after construction.
   */
  private PSDependencyTreeContext m_fullTreeCtx;

  /** The ID Map supplied during construction, never <code>null</code> or modified after that. */
  private PSIdMap m_idMap;

  /**
   * Flag to indicate if ancestors should be validated. Initially <code>false</code>, modified by
   * calls to {@link #setValidateAncestors(boolean)}.
   */
  private boolean m_validateAncestors = false;

  /**
   * Map of import packages. Key is the dependency key of the root element of the package as a
   * <code>String</code>, value is the corresponding {@link PSImportPackage}. Never <code>null
   * </code> after construction. Packages are added by calls to {@link
   * #addPackage(PSImportPackage)}.
   */
  private Map m_pkgMap = new HashMap();

  /**
   * Map of previously validated dependencies. Key is the dependency key of the dependency as a
   * <code>String</code>, value is the {@link PSDependency} object. Never <code>null</code>, entries
   * are added by calls to {@link #addValidatedDependency(PSDependency)}.
   */
  private Map m_validatedDeps = new HashMap();

  /**
   * Map of absent ancestors. Key is the dependency key of the dependency for which the ancestors
   * were added as a <code>String</code>, value is a <code>List</code> of {@link PSDependency}
   * objects. Never <code>null</code>, values are added by calls to {@link
   * #addAbsentAncestor(PSDependency, PSDependency)}
   */
