...
public class PSExportJob extends PSDeployJob {
   ...
   public void doRun() {
      ...
      try {
         ...
         m_descriptor.getPackages().forEachRemaining(de -> treeCtx.addPackage((PSDeployableElement) de, true));
         ...
         m_descriptor.getPackages().forEachRemaining(de -> {
            if (!isCancelled()) {
               var msg = MessageFormat.format(bundle.getString("analyzingDeps"), de.getDisplayIdentifier());
         ...
         // create the archive
         var archiveFile = new File(PSDeploymentHandler.EXPORT_ARCHIVE_DIR,
         ...
         var info = new PSArchiveInfo(m_descriptor.getName(),
                 PSServer.getHostName() + ":" + PSServer.getListenerPort(),
                 m_serverVersion, dbmsHelper.getServerRepositoryInfo(), getUserId(), category);
   ...
   private void handleConfigFiles(PSDeploymentHandler dh, PSArchive archive) throws PSJobException {
      ...
      if (hasConfigDef) {
         ...
         if (!config.exists() || !config.isFile()) {
            throw new PSJobException(IPSJobErrors.CONFIG_FILE_NOT_FOUND, m_descriptor.getConfigDefFile());
         }
         archive.storeFile(config, PSDescriptor.getConfigArchiveEntryPath(IPSConfigService.ConfigTypes.CONFIG_DEF));
      }
      if (hasLocalConfig) {
         configRef = m_descriptor.getName() + "_" + "localConfig";
         config = dh.getConfigTempFile(configRef);
         if (!config.exists() || !config.isFile()) {
            throw new PSJobException(IPSJobErrors.CONFIG_FILE_NOT_FOUND, m_descriptor.getLocalConfigFile());
         }
         archive.storeFile(config, PSDescriptor.getConfigArchiveEntryPath(IPSConfigService.ConfigTypes.LOCAL_CONFIG));
      }
   }
