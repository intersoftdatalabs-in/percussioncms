...
public abstract class PSDeployJob extends PSJobRunner implements IPSJobHandle {
   ...
   *
   * @param id The id used to identify this job. Must be used to then call to set the job id.
  ...
  public void init(int id, PSRequest req, Properties initParams)
    ...
    if (initParams != null) {
      ...
      if (role != null && role.trim().length() > 0 && !req.isUserInRole(role)) {
  ...
  /**
   * Initializes the user info from the supplied request.
   *
  ...
  protected void initUserInfo(PSRequest req) {
    ...
    } else m_userId = "unknown";

    m_userInfoInited = true;
  }

  /**
   * See {@link IPSJobHandle#updateStatus(String)} for method param and exception info.
   *
   * @throws IllegalStateException if {@link #initDepCount(Iterator)} has not been called.
   */
  public void updateStatus(String message) {
    if (message == null || message.trim().length() == 0)
      throw new IllegalArgumentException("message may not be null or empty");

    if (m_depTotal == -1) throw new IllegalStateException("initDepCount() has not been called");

    if (m_curDepCount < m_depTotal) // don't want to go over, shouldn't happen
    m_curDepCount++;

    int status = (m_curDepCount * 100) / m_depTotal;
    if (status == 0) status = 1; // never 0
    else if (status == 100) status = 99; // let job decide when it's finished
    setStatus(status);
    setStatusMessage(message);
  }

  /**
   * Walks the supplied packages and initializes the total number of dependencies that will be
    ...
    while (pkgs.hasNext()) {
      ...
      if (childCount == 0) {
        ...
        throw new IllegalArgumentException(
            "pkg " + dep.getKey() + " has no " + included + "children");
      }
      count += childCount;
    }
    m_depTotal = count;
  ...
  /**
   * Get the security token extracted from the request supplied to the call to <code>init()</code>.
   *
  ...
  protected PSSecurityToken getSecurityToken() {
    if (!m_userInfoInited) throw new IllegalStateException("userInfo not initialized");
  ...
  /** Name of the init param to use to get the role required for running this deployment job. */
  private static final String ROLE_PARAM_NAME = "role";

  /**
   * The security token extracted from the request supplied to the <code>init</code> call. <code>
   * null</code> until <code>init()</code> is called, never <code>null</code> or modified after
   * that.
   */
  private PSSecurityToken m_securityToken;

  /**
   * Flag to determine if the <code>init()</code> method has been called. <code>false</code> until
   * it is called, <code>true</code> after that.
   */
  private boolean m_userInfoInited = false;

  /**
   * The name of the user from the first authenticated user entry found in the user session of the
   * request passed to the <code>init()</code> method, never <code>null</code> or empty after that.
   */
  private String m_userId;

  /**
   * Current number of dependencies having been processed, updated by calls to <code>updateStatus()
   * </code>.
   */
  protected int m_curDepCount = 0;

  /**
   * Total number of dependencies to be processed, initially <code>-1</code>, set to a non-negative
   * value by a call to the <code>initDepCount()</code> method, never modified after that.
   */
