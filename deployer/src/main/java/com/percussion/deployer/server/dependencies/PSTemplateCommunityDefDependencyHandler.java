 ...
 * RXVARIANTCOMMUNITY table
 */
public class PSTemplateCommunityDefDependencyHandler extends PSDataObjectDependencyHandler {
  ...
  */
  ...
  public PSDependency getDependency(PSSecurityToken tok, String commID) throws PSDeployException {
    ...
    if (dep != null) dep.setDependencyType(PSDependency.TYPE_LOCAL);
  ...
  public Iterator<PSDependency> getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    var childDeps =
        getChildDepsFromParentID(
            VARCOMMUNITYTABLE, COMMUNITYID, TEMPLATEID, templateID, DEPENDENCY_TYPE, tok);
  ...
  public Iterator<PSDependencyFile> getDependencyFiles(PSSecurityToken tok, PSDependency dep)
    ...
    PSDependencyData vrData = getDepDataFromTable(dep, VARCOMMUNITYTABLE, TEMPLATEID, true);

    files.add(getDepFileFromDepData(vrData));
    if (vrData != null) files.add(getDepFileFromDepData(vrData));
  ...
  public void installDependencyFiles(
    ...
    try {
      IPSNodeDefinition nodeDef = contentMgr.findNodeDefinitionByName(dep.getDisplayName());
