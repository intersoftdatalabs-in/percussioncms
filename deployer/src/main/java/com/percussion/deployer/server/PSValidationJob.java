...
public class PSValidationJob extends PSDeployJob {
    ...
    public void validate(PSImportDescriptor descriptor, IPSJobHandle jobHandle, PSSecurityToken tok)
        ...
        try {
            ...
            for (var pkg : descriptor.getImportPackageList()) {
                ...
                var msg = MessageFormat.format(bundle.getString("processing"),
            ...
            if (!isCancelled()) {
                ...
                var resultsFile = new File(
                        PSDeploymentHandler.VALIDATION_RESULTS_DIR, info.getArchiveRef() + ".xml");
                resultsFile.getParentFile().mkdirs();
                resultsFile.deleteOnExit();
    ...
    /**
     * The import descriptor supplied to the {@code init()} method, never {@code null} or modified after that.
     */
