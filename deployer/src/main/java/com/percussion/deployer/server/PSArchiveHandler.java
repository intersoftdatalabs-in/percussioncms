...
public class PSArchiveHandler {
  ...
  public PSArchiveHandler(PSArchive archive) throws PSDeployException {
    ...
    m_archive = archive;
    if (m_archive.isWriting()) m_archiveMan = new PSArchiveManifest();
    else m_archiveMan = m_archive.getArchiveManifest();
  }
  ...
  public void close() throws PSDeployException {
    if (!m_archive.isClosed()) {
      if (m_archive.isWriting()) m_archive.storeArchiveManifest(m_archiveMan);
      m_archive.close();
    }
  }
   ...
   */
  public void addFiles(PSDependency dep, Iterator files) throws PSDeployException {
    ...
    String tmpName =
        (type.equals(PSAclDefDependencyHandler.DEPENDENCY_TYPE))
    ...
    try {
      ...
      while (files.hasNext()) {
        ...
        // Generate meaningful, unique names for files with names created using temporary files
        // (design objects).
        if (depFile.getFile().exists()
          ...
          depFile = new PSDependencyFile(depFile.getType(), tmp, depFile.getOriginalFile());
        }

        if (depFile.getFile().exists()) {
          setArchiveLocation(depFile, dep, dupFiles);

          m_archive.storeFile(depFile.getFile(), getNormalizedArchivePath(depFile));
  ...
  private void setArchiveLocation(
    ...
    while (!hasUniquePath(location, depFiles.iterator())) {
      String uniqueName = System.currentTimeMillis() + depFile.getFile().getName();
  ...
  public void addIdTypes(PSDependency dep, PSApplicationIDTypes idTypes) {
    ...
    if (!m_archive.isWriting())
      throw new IllegalStateException("archive is not opened for writing");
    if (m_archive.isClosed()) throw new IllegalStateException("ArchiveHandler has been closed");
  ...
  public void addDbmsInfoList(PSDependency dep, List<PSDatasourceMap> dbmsInfoList) {
    ...
    if (dbmsInfoList == null) throw new IllegalArgumentException("dbmsInfoList may not be null");
  ...
  public InputStream getFileData(PSDependencyFile file) throws PSDeployException {
    if (file == null || file.getArchiveLocation() == null)
      throw new IllegalArgumentException("file or its archive location may not be null");
  ...
  private String getNormalizedArchivePath(PSDependencyFile file) {
    ...
    // filenames contain a "\".
    return PSDeployComponentUtils.getNormalizedPath(file.getArchiveLocation().getPath());
  }
