...
public class PSFolderTranslationsDependencyHandler extends PSFolderObjectDependencyHandler {
   ...
   */
  public PSFolderTranslationsDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap)
  ...
  public Iterator getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    if (pathSpecifiesParent(path)) {
      ...
      if (sum != null) {
        PSComponentSummaries summ = getTranslatedFolderSummaries(proc, sum, relType);
        if (summ.size() > 0) {
          ...
          while (transFolders.hasNext()) {
            PSComponentSummary trans = (PSComponentSummary) transFolders.next();
            // get the path of the translated folder path
            String transFolderPath = getFolderPath(proc, trans);
            PSDependency childDep = handler.getDependency(tok, transFolderPath);
    ...
    PSDependencyHandler relHandler =
        getDependencyHandler(PSRelationshipDependencyHandler.DEPENDENCY_TYPE);

    PSDependency relDep = relHandler.getDependency(tok, relType);
    if (relDep != null) childDeps.add(relDep);
  ...
  public PSDependency getDependency(PSSecurityToken tok, String id) throws PSDeployException {
    ...
    if (pathSpecifiesParent(path)) {
      ...
      if (sum != null) {
        PSComponentSummaries summ = getTranslatedFolderSummaries(proc, sum, relType);
        if (summ.size() > 0) dep = createDependency(m_def, id, relType);
      }
    }
    return dep;
  }
  ...
  public void installDependencyFiles(
    ...
    try {
      ...
      PSComponentSummary parentFolder = getFolderSummary(proc, parentFolderPath);
      ...
      PSComponentSummaries sums = getTranslatedFolderSummaries(proc, parentFolder, relType);
      if (sums.size() > 0) {
      ...
      while (it.hasNext()) {
        ...
        if (!(childType.equals(PSFolderTreeDefDependencyHandler.DEPENDENCY_TYPE)
            || childType.equals(PSFolderDefDependencyHandler.DEPENDENCY_TYPE))) {
          continue;
        }

        String childFolderPath = childDep.getDependencyId();
        PSComponentSummary childFolder = getFolderSummary(proc, childFolderPath);
        if (childFolder == null) {
          // handle error
          Object[] args = {
            childFolderPath,
  ...
  public static String createDependencyId(String folderPath, String relationshipType) {
    if (folderPath == null || folderPath.trim().length() == 0)
      throw new IllegalArgumentException("folderPath may not be null or empty");
  ...
  private static String[] parseDependencyId(String depId) throws PSDeployException {
    ...
    if (isValid) {
      ...
      if (relType.trim().length() == 0 || folderPath.trim().length() == 0) {
        isValid = false;
      }
    }
