...
public class PSPublisherServiceHelper {
  ...
  public PSExtensionRef getExpanderExtensionRef(String name) throws PSDeployException {
    ...
    return getExtensionRef(
        name, "global/percussion/system/", "com.percussion.services.publisher.IPSTemplateExpander");
  }
  ...
  public static PSExtensionRef getItemFilterRuleExtensionRef(String name) throws PSDeployException {
    if (name == null || name.trim().length() == 0) return null;
    return getExtensionRef(
        name, "global/percussion/itemfilter/", "com.percussion.services.filter.IPSItemFilterRule");
  }
  ...
  public long getNextIdByType(PSTypeEnum type, String curId) {
    ...
    long id = -1;
    try {
      if (type.equals(PSTypeEnum.CONTENT_LIST)) id = PSIdGenerator.getNextId(PK_CONTENTLIST);
