  ...
  public static List<PSDependency> getIdTypeDependencies(
    ...
    if (dep.supportsIdTypes()) {
      ...
      while (mappings.hasNext()) {
        ...
        PSDependencyHandler depHandler = handler.getDependencyHandler(type);
        ...
        if (mapping.getParentId() != null) {
          // get parent
          PSDependencyHandler parentHandler = handler.getDependencyHandler(mapping.getParentType());
          childDep = parentHandler.getDependency(tok, mapping.getParentId());
        } else childDep = depHandler.getDependency(tok, mapping.getValue());

        // add child
        if (childDep != null && !childDep.getKey().equals(dep.getKey())) deps.add(childDep);
      }
    }
    return deps;
  }
