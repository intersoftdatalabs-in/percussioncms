...
public class PSIdMapManager {
   ...
   *     The object will not have any <code>PSIdMapping</code> objects if it does not exist in
   *     memory.
   */
  ...
  /**
   * Save the a <code>PSIdMap</code> object into memory. If an id map exists for the source server,
   * it will be replaced by the new id map.
   *
  ...
  /**
   * Validating the given <code>PSIdMap</code> object, which will be saved to memory.
   *
   * @param map The to be validated <code>PSIdMap</code> object. Assuming it is not <code>null
   *     </code>.
  ...
  private void validateSavedIdMap(PSIdMap map) throws PSDeployException {
    ...
    while (mappingList.hasNext()) {
      ...
      if ((mapping.getTargetId() == null) && (!mapping.isNewObject())) {
        Object[] args = {map.getSourceServer(), mapping.getSourceId(), mapping.getSourceName()};
   ...
   */
  private Map<String, PSIdMap> m_repToIdMap = new HashMap<>();
}
