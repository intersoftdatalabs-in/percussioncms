...
public abstract class PSPairIdDependencyHandler extends PSDataObjectDependencyHandler {
  ...
  protected PSDependency getDependency(
      PSSecurityToken tok, String id, String table, String childNameCol, String parentIdCol)
    ...
    if (childNameCol == null || childNameCol.trim().length() == 0)
      throw new IllegalArgumentException("childNameCol may not be null or empty");
    ...
    PSJdbcSelectFilter filter = getFilterForPairId(id, parentIdCol, childNameCol);
   ...
   * @throws PSDeployException if any error occurs.
   */
  protected PSJdbcSelectFilter getFilterForPairId(
      String id, String parentIdCol, String childNameCol) throws PSDeployException {
    if (id == null || id.trim().length() == 0)
      throw new IllegalArgumentException("id may not be null or empty");
    if (parentIdCol == null || parentIdCol.trim().length() == 0)
      throw new IllegalArgumentException("parentIdCol may not be null or empty");
    ...
    PSJdbcSelectFilter fltChildId =
        new PSJdbcSelectFilter(
            childNameCol, PSJdbcSelectFilter.EQUALS, pairId.getChildId(), Types.VARCHAR);
    PSJdbcSelectFilter fltParentId =
        new PSJdbcSelectFilter(
            parentIdCol, PSJdbcSelectFilter.EQUALS, pairId.getParentId(), Types.INTEGER);
   ...
   * @throws PSDeployException if any error occurs.
   */
  protected void deleteDepFromTable(
      PSSecurityToken tok,
      PSDependency dep,
      PSImportCtx ctx,
    ...
    if (parentDepType == null || parentDepType.trim().length() == 0)
      throw new IllegalArgumentException("parentDepType may not be null or empty");

    PSPairDependencyId pairId = new PSPairDependencyId(dep.getDependencyId());

    // get the target parent id
    PSIdMapping parentMapping = getIdMapping(ctx, pairId.getParentId(), parentDepType);
    String newParentId =
        (parentMapping == null) ? pairId.getParentId() : parentMapping.getTargetId();

    PSDbmsHelper dbmsHelper = PSDbmsHelper.getInstance();

    // query the table first, see if there is anything to delete
    String newPairId = PSPairDependencyId.getPairDependencyId(newParentId, pairId.getChildId());
