...
public class PSImportJob extends PSDeployJob {
   ...
   * operations during the run method. Gets a server lock on the publisher so that editions will not
  ...
  public void init(int id, Document descriptor, PSRequest req, Properties initParams)
    ...
    try {
      ...
      m_archiveFile =
          new File(
              PSDeploymentHandler.IMPORT_ARCHIVE_DIR,
  ...
  private void acquirePubLock() throws PSJobException {
    ...
    } else m_pubLockId = result.getLock().getLockId();
  }

  /**
   * Standalone method to install an archive, used by server-side services, does not check
   * authorization.
   *
   ...
   * @param applytoAllComms <code>true</code> if installing a package that should be made visibible
   *     to all communities.
   * @throws PSJobException If there are any errors.
   */
  public void install(
      PSRequest request, File archiveFile, PSImportDescriptor descriptor, boolean applytoAllComms)
  ...
  /**
   * Sets the {@link #m_isApplyToAllComms} flag according to the parameters of {@link
   * IPSDeployConstants#APPLY_TO_ALL_COMMS} in the given request.
   *
   * @param req the request, assumed not <code>null</code>.
   */
  private void setApplyToAllComms(PSRequest req) {
    String visToAllComms = req.getParameter(IPSDeployConstants.APPLY_TO_ALL_COMMS);
  ...
  /**
   * Use current ID-Map to convert the IDs in the current descriptor to the new IDs, and write the
   * converted descriptor to the location of {@link PSDeploymentHandler#IMPORT_ARCHIVE_DIR}, with
   * the same name as the archive file, but ".xml" file extension.
   *
   * @param info the info contains current descriptor, assumed not <code>null</code>
  ...
  private void writeDescriptorWithNewIDs(PSArchiveInfo info, PSImportCtx ctx)
    ...
    PSExportDescriptor cvtDesc =
        dm.convertExportDescriptor(info, dh.getLogHandler(), ctx.getCurrentIdMap());

    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    Element expEl = cvtDesc.toXml(doc);
    String descString = PSXmlDocumentBuilder.toString(expEl);

    File descFile = new File(PSDeploymentHandler.IMPORT_ARCHIVE_DIR, info.getArchiveRef() + ".xml");
   ...
   */
  private PSImportCtx createImportContext(PSArchiveInfo info) throws PSDeployException {
    PSDeploymentHandler dh = PSDeploymentHandler.getInstance();
    ...
    PSImportCtx ctx =
        new PSImportCtx(
            ...
            PSDbmsMapManager.getDbmsMap(info.getServerName()),
            dh.getIdMapMgr(),
            lh,
            dh.getAppPolicySettings());

    PSDbmsInfo sourceRepository = ctx.getSourceRepository();
    ctx.setCurrentIdMap(ctx.getIdMapMgr().getIdmap(sourceRepository.getDbmsIdentifier()));
   ...
   *
   * @param archive the archive that contains the installed elements, assumed not <code>null</code>.
   * @param ctx the import context, assumed not <code>null</code>.
   * @param info the archive info, assumed not <code>null</code>.
   * @param expDesc the package descriptor, assumed not <code>null</code>.
   * @throws Exception if an error occurs.
   */
  private void installPkgElements(
      PSArchive archive, PSImportCtx ctx, PSArchiveInfo info, PSExportDescriptor expDesc)
      throws Exception {
    ...
    if (hasPkgs) {
      ...
      int archiveLogId = lh.createArchiveLog(info, archive.getArchiveManifest());
      ...
      for (PSImportPackage pkg : pkgs) {
        ...
        try {
          ...
          if (isCancelled()) status = PSArchivePackage.STATUS_ABORTED;
        ...
        lh.updatePackageStatus(
            archiveLogId, pkgLogId, de.getObjectType(), de.getDependencyId(), status);
        // Register
        if (restoreEx != null) throw restoreEx;
      }
  ...
  /**
   * This is called after the package has been installed. Do nothing if user has canceled the
   * installation.
   *
   * @param pkgInfo the installed package info, assumed not <code>null</code>
   * @param expDesc the descriptor of the installed package, assumed not <code>null</code>.
   */
  private void postInstallPackage(PSPkgInfo pkgInfo, PSExportDescriptor expDesc)
    ...
    PSPkgInfo pkgInfoModify = pkgService.loadPkgInfoModifiable(pkgInfo.getGuid());
    ...
    IPSConfigRegistrationMgr regMgr = cfgSrvc.getConfigRegistrationMgr();
    ...
    if (m_isApplyToAllComms) {
      PSPackageServiceHelper.updatePkgCommunities(expDesc.getName(), null, true);
    }
  }

  /**
   * Creates a package info from the supplied description, context, and archive info. The existing
   * package info with the same name (if exist) will be deleted.
   *
   ...
   */
  private PSPkgInfo createPkgInfo(PSExportDescriptor expDesc, PSImportCtx ctx, PSArchiveInfo info) {
    IPSPkgInfoService pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
   ...
   *
   * @param archive the archive that may contain the configure files. assumed not <code>null</code>.
   * @param expDesc the descriptor, assumed not <code>null</code>.
   * @throws IOException if an I/O exception occurs.
   * @throws PSDeployException if any other error occurs.
   */
  private void installConfigFiles(PSArchive archive, PSExportDescriptor expDesc)
      throws PSDeployException, IOException {
    if (StringUtils.isNotBlank(expDesc.getLocalConfigFile())) {
      copyConfigFile(archive, IPSConfigService.ConfigTypes.LOCAL_CONFIG, expDesc.getName(), false);
      copyConfigFile(archive, IPSConfigService.ConfigTypes.DEFAULT_CONFIG, expDesc.getName(), true);
    }

    if (StringUtils.isNotBlank(expDesc.getConfigDefFile())) {
      copyConfigFile(archive, IPSConfigService.ConfigTypes.CONFIG_DEF, expDesc.getName(), true);
    }
  ...
  /**
   * Creates the package dependency entries for the supplied package info object. Clears the old
   * dependencies if exists, gets the new dependencies from supplied expDesc.
   *
   * @param pkgInfo The {@link PSPkgInfo} object for which the dependencies needs to be created,
   *     assumed not <code>null</code>.
   * @param expDesc The export descriptor from which the new dependencies are extracted, assumed not
   *     <code>null</code>.
   */
  private void createPkgDependencies(PSPkgInfo pkgInfo, PSExportDescriptor expDesc) {
    List<Map<String, String>> newDeps = expDesc.getPkgDepList();
    IPSPkgInfoService pkgService = PSPkgInfoServiceLocator.getPkgInfoService();
    List<PSPkgDependency> oldDeps = pkgService.loadPkgDependencies(pkgInfo.getGuid(), true);
    ...
    if ((newDeps == null || newDeps.isEmpty())) return;
    for (Map<String, String> depMap : newDeps) {
      ...
      if (depPkgInfo == null) {
        log.info(
            "Skipping package dependency entry for package {} with dependent {}, "
  ...
  /**
   * Copies the configuration file from the archive supplied to its appropriate location on the
   * server based on configuration file type and package name. Also creates parent directory
   * structure if it does not yet exist.
   *
   * @param archive the archive where the config files reside. Assumed not <code>null</code>.
   * @param type the configuration file type.
   * @param packageName the package name, assumed not <code>null</code> or empty.
   * @param overwrite flag indicating that the file should be overwritten if it already exists.
   * @throws PSDeployException
   * @throws IOException
   */
  private void copyConfigFile(
      PSArchive archive, IPSConfigService.ConfigTypes type, String packageName, boolean overwrite)
      throws PSDeployException, IOException {
    InputStream is = null;
    FileWriter fw = null;
    IPSConfigService configService = PSConfigServiceLocator.getConfigService();
    try {
  ...
  /**
   * In case of any failures, a transaction log entry for failure is created for the package
   *
  ...
  private void addTransactionLogForFailure(PSImportCtx ctx, Exception ex) {
    ...
    try {
      lh.addTransactionLogEntry(
          logId,
          depString,
          PSTransactionSummary.TYPE_SKIPPED,
          PSTransactionSummary.TYPE_SKIPPED,
          action,
          ctx.getNextTxnSequence(logId));
  ...
  /**
   * The id of the publisher lock which must be acquired by the <code>init()</code> method and
   * released by the <code>run()</code> method.
   */
