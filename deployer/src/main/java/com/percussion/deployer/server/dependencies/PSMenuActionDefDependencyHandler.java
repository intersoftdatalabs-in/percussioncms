...
public class PSMenuActionDefDependencyHandler extends PSMenuActionObjectDependencyHandler {
   ...
   */
  public PSMenuActionDefDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap)
  ...
  private PSDependencyFile getActionDepFile(
      PSComponentProcessorProxy proc, PSDependency dep, boolean isLeaf) throws PSDeployException {
  ...
  public void installDependencyFiles(
    ...
    try {
      ...
      while (files.hasNext()) {
        ...
        actionMap.put(getIdFromKey(action, action.getLabel()), action.clone());
      }
      ...
      while (txns.hasNext()) {
        ...
        if (txnIdMapping != null) txnIdMapping.setIsNewObject(false);
        ...
        if (!txnDep.getObjectType().equals(getType())) {
          PSDependencyHandler depHndlr = getDependencyHandler(txnDep.getObjectType());
          depHndlr.addTransactionLogEntry(
              txnDep,
              ctx,
              action.getComponentType(),
  ...
  private void installActions(
    ...
    PSDbComponentCollection compDeletes = new PSDbComponentCollection(PSAction.class);
    PSDbComponentCollection compInserts = new PSDbComponentCollection(PSAction.class);
    ...
    if (tgtAction != null) {
      if (srcChildActions != null) {
        ...
        if (tgtChildActions != null) {
          ...
          if (tgtChild != null) tgtChildId = getIdFromKey(tgtChild, tgtChild.getLabel());
          while (tgtChildren.hasNext()) {
            PSMenuChild menuChild = (PSMenuChild) tgtChildren.next();
            if (tgtChildId != null && menuChild.getChildActionId().equals(tgtChildId)) {
              foundChild = true;
            }
      ...
      txnList.add(
          new Object[] {tgtAction, dep, Integer.valueOf(PSTransactionSummary.ACTION_DELETED)});
    ...
    if (srcChildActions != null && !foundChild && tgtChild != null) srcChildActions.add(tgtChild);

    // add source action to list for save (will be inserted using supplied id)
    compInserts.add(srcAction);
    txnList.add(
        new Object[] {srcAction, dep, Integer.valueOf(PSTransactionSummary.ACTION_CREATED)});
    ...
    while (deps.hasNext()) {
      ...
      if (childDepType.equals(DEPENDENCY_TYPE)
          || childDepType.equals(PSMenuActionCategoryDependencyHandler.DEPENDENCY_TYPE)) {
  ...
  private void transformIds(
      PSComponentProcessorProxy proc, PSDependency dep, PSImportCtx ctx, PSAction action)
      throws PSDeployException {
    ...
    if (comCtx != null) {
      transformMultiValuedProperty(comCtx, ctx, PSCommunityDependencyHandler.DEPENDENCY_TYPE);
    }

    PSActionVisibilityContext ctCtx =
        visContexts.getContext(PSActionVisibilityContext.VIS_CONTEXT_CONTENT_TYPE);
    if (ctCtx != null) {
      transformMultiValuedProperty(ctCtx, ctx, PSCEDependencyHandler.DEPENDENCY_TYPE);
    }

    PSActionVisibilityContext wfCtx =
        visContexts.getContext(PSActionVisibilityContext.VIS_CONTEXT_WORKFLOWS_TYPE);
    if (wfCtx != null) {
      transformMultiValuedProperty(wfCtx, ctx, PSWorkflowDependencyHandler.DEPENDENCY_TYPE);
    }
