...
public class PSLocalDeployerClient implements IPSPackageInstaller {
  ...
  public void installPackage(File packageFile, boolean shouldValidateVersion)
    ...
    try {
      ...
      if (sessionId != null) dh.releaseLock(sessionId);
    }
  }
   ...
   * @param dh The deployment handler to use
   * @param info the archive info to validate
   * @param shouldValidateVersion <code>false</code> if the version should be skipped for reverted
   *     packages on uninstall of patch
   * @return A valid import descriptor to use for import
   * @throws PSDeployException If there are any errors.
   */
  private PSImportDescriptor validateArchive(
    ...
    PSMultiValueHashMap<String, String> results =
        dh.validateArchive(info, false, false, true, shouldValidateVersion);
    ...
    if (!validationErrors.isEmpty()) handleErrors(info, validationErrors);
