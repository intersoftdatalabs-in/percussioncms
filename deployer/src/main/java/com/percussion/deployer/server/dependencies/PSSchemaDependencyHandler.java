...
public class PSSchemaDependencyHandler extends PSDataObjectDependencyHandler {
  ...
  public Iterator getDependencies(PSSecurityToken tok) throws PSDeployException {
    ...
    PSDbmsInfo dbmsInfo = PSDbmsHelper.getInstance().getServerRepositoryInfo();
    ...
    try {
      ...
      rs = meta.getTables(db, schema, filterAll, new String[] {"TABLE"});
      if (rs != null) {
        while (rs.next()) {
    ...
    for (String tablename : tableList) {
      if (excludeTables.contains(tablename)
          ...
          || tablename.endsWith("_UPG")) continue;
