...
public abstract class PSContentEditorObjectDependencyHandler extends PSAppObjectDependencyHandler {
  ...
  protected List<PSDependency> checkUiSet(PSSecurityToken tok, PSUISet uiSet)
    ...
    PSDependencyHandler keywordHandler =
        getDependencyHandler(PSKeywordDependencyHandler.DEPENDENCY_TYPE);
    PSDependencyHandler controlHandler =
        getDependencyHandler(PSControlDependencyHandler.DEPENDENCY_TYPE);
    ...
    if (controlRef != null) {
      ...
      PSDependency controlDep = controlHandler.getDependency(tok, controlName);
      if (controlDep != null) {
  ...
  protected void transformUiSet(PSIdMap idMap, PSUISet uiSet) throws PSDeployException {
    ...
    if (choices != null && choices.getType() == PSChoices.TYPE_GLOBAL) {
      choices.setGlobal(
          idMap.getNewIdInt(
