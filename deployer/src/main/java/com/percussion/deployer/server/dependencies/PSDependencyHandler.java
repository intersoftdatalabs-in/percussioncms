...
public abstract class PSDependencyHandler implements IPSDependencyHandler {
  ...
  public PSDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap) {
    ...
    if (!getType().equals(def.getObjectType()))
      throw new IllegalArgumentException(
          "invalid def type; expected " + getType() + ", supplied " + def.getObjectType());

    if (dependencyMap == null) throw new IllegalArgumentException("dependencyMap may not be null");
   ...
   */
  public static PSDependencyHandler getHandlerInstance(PSDependencyDef def, PSDependencyMap map)
    ...
    try {
      ...
      Constructor<?> handlerCtor =
          handlerClass.getConstructor(PSDependencyDef.class, PSDependencyMap.class);
      handler = (PSDependencyHandler) handlerCtor.newInstance(new Object[] {def, map});
  ...
  public boolean isChildTypeSupported(PSDependency child) {
    ...
    while (childTypes.hasNext() && !isChildTypeOk) {
      if (childType.equals(childTypes.next())) isChildTypeOk = true;
    }
   ...
   * @throws PSDeployException if there are any errors.
   */
  public abstract Iterator<PSDependency> getChildDependencies(PSSecurityToken tok, PSDependency dep)
  ...
  public Iterator<PSDependency> getDependencies(
      PSSecurityToken tok, String parentType, String parentId) throws PSDeployException {
  ...
  public void addAclDependency(
    ...
    if (childDeps == null)
      throw new IllegalArgumentException("child dependencies  may not be null");

    initAclService();
    PSDependencyHandler h = getDependencyHandler(PSAclDefDependencyHandler.DEPENDENCY_TYPE);
    PSGuid guid =
        new PSGuid(
            key,
    ...
    if (acl != null) {
      ...
      if (aclDep != null) childDeps.add(aclDep);
    }
  }
  ...
  public void reserveNewId(PSDependency dep, PSIdMap idMap) throws PSDeployException {
    if (dep == null) throw new IllegalArgumentException(" may not be null");

    if (idMap == null) throw new IllegalArgumentException(" may not be null");
  ...
  public String getTargetId(PSIdMapping mapping, String id) throws PSDeployException {
    ...
    if (!mapping.getSourceId().equals(id))
      throw new IllegalArgumentException("mapping source must match supplied id");
  ...
  protected PSDependencyDef getValidDependencyDef(String type) {
    ...
    if (def == null) {
      throw new RuntimeException("DependencyDef for type " + type + " cannot be located.");
    }
  ...
  protected PSDeployableObject createDependency(
      PSDependencyDef def, Map.Entry<String, String> entry) {
    if (def == null) throw new IllegalArgumentException("def may not be null");
    ...
    if (key == null || key.trim().length() == 0)
      throw new IllegalArgumentException("entry's key may not be null or empty");

    String value = entry.getValue();
    if (value == null || value.trim().length() == 0)
      throw new IllegalArgumentException("entry's value may not be null or empty");
  ...
  protected PSDeployableElement createDeployableElement(
      PSDependencyDef def, String id, String name) {
    if (def == null) throw new IllegalArgumentException("def may not be null");
    ...
    PSDeployableElement de =
        new PSDeployableElement(
            ...
            name,
            def.supportsIdTypes(),
            def.supportsIdMapping(),
            def.supportsUserDependencies(),
            def.supportsParentId());
   ...
   * @throws PSDeployException If there are any errors.
   */
  protected PSPurgableTempFile createXmlFile(Document doc) throws PSDeployException {
    if (doc == null) throw new IllegalArgumentException("doc may not be null");
    try {
      PSPurgableTempFile xmlFile = new PSPurgableTempFile("dpl_", ".xml", null);
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected static Document createXmlDocument(InputStream in) throws PSDeployException {
    return createXmlDocument(in, null);
  }

  /**
   * Creates an Xml document from the supplied dependency file stream.
   *
   * @param in The inputstream from which the document is to be created. This method takes ownership
   *     of the stream and closes it when finished. May not be <code>null</code>.
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected static Document createXmlDocument(InputStream in, String sourceIdentity)
      throws PSDeployException {
    if (in == null) throw new IllegalArgumentException("in may not be null");
    ...
    if (dep.supportsParentId()) {
      result =
          getIdMapping(
              ...
              dep.getParentId(),
              dep.getParentType());
    } else result = getIdMapping(ctx, dep.getDependencyId(), dep.getObjectType());
  ...
  protected PSIdMapping getIdMapping(
    ...
    if (parentId != null && (parentType == null || parentType.trim().length() == 0)) {
  ...
  public PSIdMapping getIdMapping(
    ...
    PSDependencyHandler handler;
    if (type.equals(getType())) handler = this;
    else handler = getDependencyHandler(type);
   ...
   */
  protected PSIdMapping getIdMappingOfAssoc(
      PSSecurityToken tok, PSImportCtx ctx, String value, String type) throws PSDeployException {
    if (tok == null) throw new IllegalArgumentException("tok may not be null");
    if (ctx == null) throw new IllegalArgumentException("ctx may not be null");
    if (value == null) throw new IllegalArgumentException("prop may not be null");
    if (type == null || type.trim().length() == 0)
      throw new IllegalArgumentException("type may not be null or empty");
    ...
    try {
      ...
      if (idMapping != null) {
        ...
        PSDependency dependentOb = depHandler.getDependency(tok, idMapping.getTargetId());
        if (dependentOb == null) {
          idMapping = null;
        }
      }
    ...
    // Any other exception should be rethrown.
      ...
      if (idMapping != null) value = idMapping.getTargetId();
    }
   ...
   */
  protected int getNewIdInt(PSImportCtx ctx, String id, String type) throws PSDeployException {
    if (ctx == null) throw new IllegalArgumentException("ctx may not be null");
    ...
    if (idMap == null) throw new IllegalArgumentException("ctx must contain an idMap");
  ...
  protected int getRowAction(PSImportCtx ctx, PSDependency dep) throws PSDeployException {
    ...
    PSIdMapping mapping = null;
    if (dep.supportsIDMapping()) mapping = getIdMapping(ctx, dep);
    ...
    if (mapping != null && mapping.isNewObject()) rowAction = PSTransactionSummary.ACTION_CREATED;
  ...
  protected PSDependency getChildDependency(PSDependency dep, String childId, String childObjType)
    ...
    if (childObjType == null || childObjType.trim().length() == 0)
      throw new IllegalArgumentException("childObjType may not be null or empty");
  ...
  protected PSDependency doGetChildDependency(
      PSDependency dep, String childId, String childObjType) {
    if (dep == null) throw new IllegalArgumentException("dep may not be null");
    if (childId == null || childId.trim().length() == 0)
      throw new IllegalArgumentException("childId may not be null or empty");
    if (childObjType == null || childObjType.trim().length() == 0)
   ...
   */
  protected boolean isChildDependencyIncluded(PSDependency dep, String childId, String childObjType)
      throws PSDeployException {
    if (dep == null) throw new IllegalArgumentException("dep may not be null");
    if (childId == null || childId.trim().length() == 0)
      throw new IllegalArgumentException("childId may not be null or empty");
  ...
  public void addTransactionLogEntryByGuidType(
    ...
    int transAction =
        (!isNew) ? PSTransactionSummary.ACTION_MODIFIED : PSTransactionSummary.ACTION_CREATED;
    ...
    addTransactionLogEntry(
        dep, ctx, type.getDisplayName(), PSTransactionSummary.TYPE_CMS_OBJECT, transAction);
  }
  ...
  public void addTransactionLogEntry(
    ...
    if (dep.supportsIDMapping() || delegatesIdMapping()) depMapping = getIdMapping(ctx, dep);
    String id =
        (depMapping == null)
            ? dep.getDependencyId()
    ...
    if (dep.getDisplayName().equals(dep.getDependencyId())) {
      ...
      depString = dep.getObjectTypeName() + " \"" + dep.getDisplayName() + "\" (" + id + ")";
    }

    PSLogHandler lh = ctx.getLogHandler();
    lh.addTransactionLogEntry(
        logId, depString, elementName, elementType, action, ctx.getNextTxnSequence(logId));
  }
