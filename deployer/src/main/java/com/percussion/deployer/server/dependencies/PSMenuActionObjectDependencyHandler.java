...
public abstract class PSMenuActionObjectDependencyHandler extends PSCmsObjectDependencyHandler {
   ...
   */
  public PSMenuActionObjectDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap)
  ...
  public Iterator<PSDependency> getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    PSDependencyHandler appDepHandler =
        getDependencyHandler(PSApplicationDependencyHandler.DEPENDENCY_TYPE);
    String url = action.getURL();
    String appName = null;
    if (url.trim().length() > 0) appName = PSDeployComponentUtils.getAppName(url);
    if (appName != null) {
      PSDependency appDep = appDepHandler.getDependency(tok, appName);
      if (appDep != null) {
      ...
      PSActionVisibilityContext comCtx =
          visContexts.getContext(PSActionVisibilityContext.VIS_CONTEXT_COMMUNITY);
      if (comCtx != null) {
        childDeps.addAll(
            getDepsFromMultiValuedProperty(
                tok, comCtx, getDependencyHandler(PSCommunityDependencyHandler.DEPENDENCY_TYPE)));
      }

      PSActionVisibilityContext ctCtx =
          visContexts.getContext(PSActionVisibilityContext.VIS_CONTEXT_CONTENT_TYPE);
      if (ctCtx != null) {
        childDeps.addAll(
            getDepsFromMultiValuedProperty(
                tok, ctCtx, getDependencyHandler(PSCEDependencyHandler.DEPENDENCY_TYPE)));
      }

      PSActionVisibilityContext wfCtx =
          visContexts.getContext(PSActionVisibilityContext.VIS_CONTEXT_WORKFLOWS_TYPE);
      if (wfCtx != null) {
        childDeps.addAll(
            getDepsFromMultiValuedProperty(
                tok, wfCtx, getDependencyHandler(PSWorkflowDependencyHandler.DEPENDENCY_TYPE)));
      }

      // get icon url dep
      String iconPath = action.getProperties().getProperty(action.PROP_SMALL_ICON);
      if (iconPath != null && iconPath.trim().length() > 0) {
        ...
        if (iconDep == null) {
          ...
          if (iconAppName != null) iconDep = appDepHandler.getDependency(tok, iconAppName);
        }
  ...
  public Iterator<PSDependency> getDependencies(PSSecurityToken tok) throws PSDeployException {
    ...
    while (actions.hasNext()) {
      ...
      deps.add(createDependency(m_def, getIdFromKey(action, dispName), dispName));
    }
  ...
  public PSDependency getDependency(PSSecurityToken tok, String id) throws PSDeployException {
    ...
    if (action != null) dep = createDependency(m_def, id, action.getName());
  ...
  public PSApplicationIDTypes getIdTypes(PSSecurityToken tok, PSDependency dep)
    ...
    while (actionParams.hasNext()) {
      ...
      PSParam param = new PSParam(actionParam.getName(), new PSTextLiteral(actionParam.getValue()));

      PSAppTransformer.checkParam(mappings, param, null);
    }

    idTypes.addMappings(
        reqName, IPSDeployConstants.ID_TYPE_ELEMENT_URL_PARAMS, mappings.iterator());
    ...
    while (resources.hasNext()) {
      ...
      while (elements.hasNext()) {
        ...
        while (mappings.hasNext()) {
          ...
          if (element.equals(IPSDeployConstants.ID_TYPE_ELEMENT_URL_PARAMS)) {
            ...
            while (actionParams.hasNext()) {
              PSActionParameter actionParam = (PSActionParameter) actionParams.next();
              PSParam param =
                  new PSParam(actionParam.getName(), new PSTextLiteral(actionParam.getValue()));
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected PSAction loadAction(PSComponentProcessorProxy proc, String id, boolean loadLeaf)
      throws PSDeployException {
    if (proc == null) throw new IllegalArgumentException("proc may not be null");
    if (id == null || id.trim().length() == 0)
      throw new IllegalArgumentException("id may not be null or empty");
    ...
    try {
      ...
      Element[] elements =
          proc.load(
              PSAction.getComponentType(PSAction.class), new PSKey[] {PSAction.createKey(id)});
      if (elements.length > 0) {
        PSAction action = new PSAction(elements[0]);
        if (loadLeaf == isLeaf(action)) result = action;
      }
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected Iterator<PSAction> loadActions(
      PSComponentProcessorProxy proc, boolean loadLeaf, String childId) throws PSDeployException {
    if (proc == null) throw new IllegalArgumentException("proc may not be null");
    ...
    try {
      ...
      Element[] elements = proc.load(PSAction.getComponentType(PSAction.class), null);
      for (int i = 0; i < elements.length; i++) {
        PSAction action = new PSAction(elements[i]);
        if ((loadLeaf == isLeaf(action)) && (childId == null || (isChild(action, childId)))) {
          result.add(action);
        }
      }
  ...
  private boolean isChild(PSAction action, String childId) {
    ...
    while (childActions.hasNext() && !isChild) {
      ...
      if (child.getChildActionId().equals(childId)) isChild = true;
    }
