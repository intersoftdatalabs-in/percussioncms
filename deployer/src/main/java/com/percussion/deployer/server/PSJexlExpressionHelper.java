...
/**
 * A helper class for managing the ids found in a jexl binding. This class has a map of a binding
 * index and binding Parameters. This helper is used when an id is transformed, record it so that
 * next substitution within the binding expression will be known precisely
 *
 * @author vamsinukala
 */
public class PSJexlExpressionHelper {
  ...
  /**
   * This is built during deployment, and useful to update the parameter based on its index, and its
   * occurence that can uniquely identify the id in a binding.
   *
   ...
   *
   * @param oldVal the old value that needs to be replaced in the binding never <code>null</code>
   * @param pCtx the param context that has the binding context, never <code>null</code>
   * @param occurNum the nth occurence of this value, if it occurs once in the expression, then it
   *     has "0" never <code>null</code>.
   * @return the new value that was stored in the jexl expression, may be <code>null</code>
   */
  public String getJexlBindingParamOccurenceValue(
      String oldVal, PSBindingParamIdContext pCtx, int occurNum) {
    if (pCtx == null) throw new IllegalArgumentException("param context may not be null");
    ...
    PSJexlBindingParamOccurence pMap =
        getParamOccurenceFromIndex(((PSBindingIdContext) pCtx.getParentCtx()).getIndex());
  ...
  /**
   * Helper method to update the PSJexlBindingParamOccurence with the new value Locate the occurence
   * based on the old value and if the occurence exists, update with the new value.
   *
  ...
  public void updateBindingParam(String oldVal, String newVal, PSBindingParamIdContext pCtx) {
    ...
    PSJexlBindingParamOccurence pOccur =
        getParamOccurenceFromIndex(((PSBindingIdContext) pCtx.getParentCtx()).getIndex());
    if (pOccur == null) return;
