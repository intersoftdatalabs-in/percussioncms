...
public class PSJexlExpressionHelper {
    ...
    public PSJexlExpressionHelper(PSApplicationIDTypeMapping mapping) {
        ...
        if (bindingCtx == null) {
            throw new IllegalStateException("A param context exists without a parent binding information");
        }

        var pVal = paramCtx.getParam().getValueText();
        var pMap = m_paramMap.computeIfAbsent(bindingCtx.getIndex(), k -> new PSJexlBindingParamOccurence());
    ...
    public String getJexlBindingParamOccurenceValue(String oldVal, PSBindingParamIdContext pCtx, int occurNum) {
        ...
        return (occurMap != null && occurMap.hasKey(pCtx.getOccurence())) ? occurMap.getValue(pCtx.getOccurence()) : null;
    }
