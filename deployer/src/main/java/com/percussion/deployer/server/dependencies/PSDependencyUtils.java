...
public class PSDependencyUtils {
  ...
  public static long getGuidValFromString(String depId, String depTypeName)
    ...
    if (depTypeName == null || depTypeName.trim().length() == 0)
      throw new IllegalArgumentException("dependency type name may not be null or empty");
  ...
  public static Object transformElementId(
    ...
    if (o instanceof IPSCatalogItem) item = (IPSCatalogItem) o;
    PSGuid guid = null;
    long guidval = -1;
    try {
  ...
  public static String getTableName(PSTypeEnum type) {
    ...
    if (type == PSTypeEnum.SLOT) anns = PSTemplateSlot.class.getAnnotations();
    else if (type == PSTypeEnum.TEMPLATE) anns = PSAssemblyTemplate.class.getAnnotations();
    else if (type == PSTypeEnum.ITEM_FILTER) anns = PSItemFilter.class.getAnnotations();
    else if (type == PSTypeEnum.CONTENT_LIST) anns = PSContentList.class.getAnnotations();
    else if (type == PSTypeEnum.ACL) anns = PSAclImpl.class.getAnnotations();
    else if (type == PSTypeEnum.NODEDEF) anns = PSNodeDefinition.class.getAnnotations();
    else if (type == PSTypeEnum.KEYWORD_DEF) anns = PSKeyword.class.getAnnotations();
    else if (type == PSTypeEnum.LOCATION_SCHEME) anns = PSLocationScheme.class.getAnnotations();
    else if (type == PSTypeEnum.SITE) anns = PSSite.class.getAnnotations();
    else if (type == PSTypeEnum.RELATIONSHIP_CONFIGNAME)
      anns = PSRelationshipConfigName.class.getAnnotations();
    else if (type == PSTypeEnum.AUTO_TRANSLATIONS) anns = PSAutoTranslation.class.getAnnotations();
    else if (type == PSTypeEnum.EDITION) anns = PSEdition.class.getAnnotations();
    else if (type == PSTypeEnum.EDITION_TASK_DEF) anns = PSEditionTaskDef.class.getAnnotations();
    else if (type == PSTypeEnum.CONTEXT) anns = PSPublishingContext.class.getAnnotations();
  ...
  public static String getFileContentAsString(PSArchiveHandler archive, PSDependencyFile depFile)
    ...
    try {
      in = new BufferedInputStream(archive.getFileData(depFile));
  ...
  public static List<String> getContentTypeTables(PSSecurityToken tok, IPSNodeDefinition node)
    ...
    try {
      app = PSAppObjectDependencyHandler.getApplication(tok, appName);
    ...
    if (dataSetColl == null) return Collections.emptyList();

    Iterator datasets = dataSetColl.iterator();
    while (datasets.hasNext()) {
      PSDataSet ds = (PSDataSet) datasets.next();
      if (!(ds instanceof PSContentEditor)) continue;
