...
public class PSAclDefDependencyHandler extends PSDependencyHandler {
  ...
  public Iterator<PSDependency> getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    if (acl != null) {
      ...
      PSDependencyHandler ch = getDependencyHandler(PSCommunityDependencyHandler.DEPENDENCY_TYPE);
        ...
        if (e.getType() == IPSTypedPrincipal.PrincipalTypes.COMMUNITY) {
          if (!e.getName().equals(PSTypedPrincipal.ANY_COMMUNITY_ENTRY)) {
            ...
            if (cList.size() > 0) {
              ...
              d = ch.getDependency(tok, String.valueOf(c.getGUID().getUUID()));
  ...
  public PSDependency getDependency(PSSecurityToken tok, String id) throws PSDeployException {
    ...
    if (acl != null) {
      dep = createDependency(m_def, "" + acl.getGUID().longValue(), acl.getName());
   ...
   */
  private PSDependencyFile getDepFileFromAcl(PSAclImpl acl) throws PSDeployException {
  ...
  public Iterator<PSDependencyFile> getDependencyFiles(PSSecurityToken tok, PSDependency dep)
    ...
    if (acl != null) files.add(getDepFileFromAcl(acl));
   ...
   * @throws PSDeployException
   */
  private PSAclImpl generateAclFromFile(PSArchiveHandler archive, PSDependencyFile depFile)
      throws PSDeployException {
  ...
  private void doTransforms(
    ...
    if (acl == null) throw new IllegalArgumentException("acl definition may not be null");

    PSTypeEnum type = PSTypeEnum.valueOf(acl.getObjectType());

    List<String> depTypes = new ArrayList<>();
    List<String> depTypesForType =
        PSDeploymentHandler.getInstance().getDependencyManager().getDeploymentType(type);
    for (String t : depTypesForType) {
      PSDependencyDef def = m_map.getDependencyDef(t);
      if (def == null) throw new RuntimeException("No type def found in map for type :" + t);

      if (def.supportsIdMapping()) depTypes.add(t);
    }

    if (depTypes.size() == 0) return;
    ...
    for (String t : depTypes) {
      try {
        m = getIdMapping(ctx, String.valueOf(acl.getObjectGuid().getUUID()), t);
        dex = null;
        break;
  ...
  private IPSAcl createNewAcl(PSAclImpl tmp) {
    ...
    PSTypedPrincipal principal =
        new PSTypedPrincipal(PSAclEntry.DEFAULT_USER_NAME, PrincipalTypes.USER);
    acl = m_aclSvc.createAcl(tmp.getObjectGuid(), principal);

    PSAclEntryImpl defaultOwner = (PSAclEntryImpl) acl.findEntry(principal);
      ...
      if (principal.isCommunity()) {
        ...
        if (visible) {
          if (existingEntry == null) {
            changed |= true;
            existingEntry = acl.createEntry(entry.getName(), PrincipalTypes.COMMUNITY);
