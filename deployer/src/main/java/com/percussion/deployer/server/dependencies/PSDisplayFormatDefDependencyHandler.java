...
public class PSDisplayFormatDefDependencyHandler extends PSCmsObjectDependencyHandler {
  ...
  public Iterator getChildDependencies(PSSecurityToken tok, PSDependency dep)
    ...
    PSDisplayFormat df = loadDisplayFormat(getComponentProcessor(tok), dep.getDependencyId());
    if (df == null) {
  ...
  public PSDependency getDependency(PSSecurityToken tok, String id) throws PSDeployException {
    ...
    if (df != null) dep = createDependency(m_def, id, df.getInternalName());
  ...
  public Iterator getDependencyFiles(PSSecurityToken tok, PSDependency dep)
    ...
    // load the component
    PSDisplayFormat df = loadDisplayFormat(getComponentProcessor(tok), dep.getDependencyId());
    if (df != null) files.add(createDependencyFile(df));
  ...
  public void installDependencyFiles(
    ...
    try {
      ...
      PSDisplayFormat newDispFormat = (PSDisplayFormat) sourceDispFormat.clone();

      // create a list for transaction support
      PSDbComponentCollection dbCompList = new PSDbComponentCollection(PSDisplayFormat.class);
