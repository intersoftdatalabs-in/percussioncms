...
public abstract class PSFolderObjectDependencyHandler extends PSCmsObjectDependencyHandler {
   ...
   */
  public PSFolderObjectDependencyHandler(PSDependencyDef def, PSDependencyMap dependencyMap)
  ...
  public Iterator getDependencyFiles(PSSecurityToken tok, PSDependency dep)
    ...
    if (folder != null) files.add(createDependencyFile(folder));
  ...
  public void installDependencyFiles(
    ...
    try {
      ...
      PSRelationshipProcessor relProc = getRelationshipProcessor(tok, params);
      ...
      // translate ids
      if (ctx.getCurrentIdMap() != null) transformIds(compProc, dep, ctx, newFolder);
      ...
      } else action = PSTransactionSummary.ACTION_MODIFIED;
  ...
  public String getFolderPathFromId(PSSecurityToken tok, String folderId) throws PSDeployException {
    ...
    if (folderId == null || folderId.trim().length() == 0)
      throw new IllegalArgumentException("folderId may not be null or empty");
    ...
    try {
      ...
      Element[] sumEls =
          proc.load(
              ...
              new PSKey[] {new PSLocator(folderId)});
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected List getChildDependencies(PSSecurityToken tok, String path, int folderDepType)
      ...
      if (folder != null) // unlikely
      {
        ...
        while (children.hasNext()) {
          ...
          PSDependency childDep =
              createDependency(m_def, pathPrefix + child.getName(), child.getName());
        ...
        PSDependencyHandler trHandler =
            getDependencyHandler(PSFolderTranslationsDependencyHandler.DEPENDENCY_TYPE);
        Iterator configs =
        ...
        while (configs.hasNext()) {
          ...
          PSDependency trDep =
              trHandler.getDependency(
                  tok,
                  PSFolderTranslationsDependencyHandler.createDependencyId(path, config.getName()));
        ...
        if (commId != -1) {
          PSDependency commDep =
              getDependencyHandler(PSCommunityDependencyHandler.DEPENDENCY_TYPE)
                  .getDependency(tok, String.valueOf(commId));
          if (commDep != null) childDeps.add(commDep);
        }
        ...
        if (dispFormatId != null && dispFormatId.trim().length() != 0) {
          PSDependency dfDep = getDisplayFormatDep(tok, getComponentProcessor(tok), dispFormatId);
          if (dfDep != null) {
   ...
   */
  protected PSFolder getFolderObject(
      PSRelationshipProcessor relProcessor, PSComponentProcessorProxy compProcessor, String path)
      throws PSDeployException {
    if (relProcessor == null) throw new IllegalArgumentException("relProcessor may not be null");
    if (compProcessor == null) throw new IllegalArgumentException("compProcessor may not be null");
    if (path == null || path.trim().length() == 0)
      throw new IllegalArgumentException("path may not be null or empty");

    PSFolder folder = null;
    PSComponentSummary sum = getFolderSummary(relProcessor, path);
    if (sum != null) folder = getFolderObject(compProcessor, sum.getCurrentLocator());
   ...
   * @throws PSDeployException if there are any other errors.
   */
  protected PSComponentSummary getFolderSummary(PSRelationshipProcessor processor, String path)
      throws PSDeployException {
    if (processor == null) throw new IllegalArgumentException("processor may not be null");
    if (path == null || path.trim().length() == 0)
      throw new IllegalArgumentException("path may not be null or empty");

    return getChildFolderSummary(processor, parseFolderPath(path), m_rootLocator, 0);
  }
  ...
  protected String getFolderPath(PSRelationshipProcessor proc, PSComponentSummary folder)
      throws PSDeployException {
    if (proc == null) throw new IllegalArgumentException("processor may not be null");

    if (folder == null) throw new IllegalArgumentException("folder summary may not be null");
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected Iterator getChildFolderPaths(PSRelationshipProcessor processor, String path)
      throws PSDeployException {
    if (processor == null) throw new IllegalArgumentException("processor may not be null");
    if (path != null && path.trim().length() == 0)
    ...
    if (path == null) {
      ...
      if (sum != null) loc = sum.getCurrentLocator();
      else exists = false;
    }
  ...
  protected Iterator getChildFolderSummaries(PSRelationshipProcessor processor, PSLocator locator)
    ...
    try {
      ...
      PSComponentSummary[] sums = processor.getChildren(FOLDER_TYPE, locator);
      for (int i = 0; i < sums.length; i++) {
        if (sums[i].isFolder()) summaries.add(sums[i]);
      }
  ...
  protected Iterator getChildItemSummaries(PSRelationshipProcessor processor, PSLocator locator)
    ...
    try {
      ...
      PSComponentSummary[] sums = processor.getChildren(FOLDER_TYPE, locator);
      for (int i = 0; i < sums.length; i++) {
        if (sums[i].getType() == PSComponentSummary.TYPE_ITEM) summaries.add(sums[i]);
      }
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected PSFolder getFolderObject(PSComponentProcessorProxy processor, PSLocator locator)
      throws PSDeployException {
    if (processor == null) throw new IllegalArgumentException("processor may not be null");
    if (locator == null) throw new IllegalArgumentException("locator may not be null");
    ...
    try {
      PSFolder folder = null;
      Element[] elements = processor.load(FOLDER_TYPE, new PSKey[] {locator});
      if (elements.length > 0) folder = new PSFolder(elements[0]);
  ...
  protected PSComponentSummary getParentFolderSummary(
      PSRelationshipProcessor processor, String path) throws PSDeployException {
    if (processor == null) throw new IllegalArgumentException("processor may not be null");
    ...
    if (pathSpecifiesParent(path)) {
      ...
      parentSum = getChildFolderSummary(processor, parentPath, m_rootLocator, 0);
    }
  ...
  protected PSFolder[] getAllFolderObjects(
    ...
    for (int i = 0; i < paths.length; i++) {
      PSComponentSummary sum = getChildFolderSummary(relProcessor, loc, paths[i]);
      if (sum == null) break;

      PSFolder folder = getFolderObject(compProcessor, loc);
      if (folder == null) break; // unlikely but possible
      folders[i] = folder;
   ...
   * @throws PSDeployException if there are any errors.
   */
  protected PSDependency getDisplayFormatDep(
      PSSecurityToken tok, PSComponentProcessorProxy proc, String formatId)
      throws PSDeployException, PSNotFoundException {
    if (tok == null) throw new IllegalArgumentException("tok may not be null");
    ...
    try {
      Element[] els =
          proc.load(
              PSDbComponent.getComponentType(PSDisplayFormat.class),
    ...
    if (df != null) {
      PSDependencyHandler dfHandler =
          getDependencyHandler(PSDisplayFormatDefDependencyHandler.DEPENDENCY_TYPE);
   ...
   * @throws PSDeployException if there are any errors
   */
  protected void transformIds(
      PSComponentProcessorProxy proc, PSDependency dep, PSImportCtx ctx, PSFolder folder)
      throws PSDeployException {
    if (proc == null) throw new IllegalArgumentException("proc may not be null");
    if (dep == null) throw new IllegalArgumentException("dep may not be null");
    if (ctx == null) throw new IllegalArgumentException("ctx may not be null");
    ...
    int commId = folder.getCommunityId();
    if (commId != -1) {
      PSIdMapping comMapping =
          getIdMapping(ctx, String.valueOf(commId), PSCommunityDependencyHandler.DEPENDENCY_TYPE);
      if (comMapping != null) {
    ...
    if (oldDfId != null && oldDfId.trim().length() > 0) {
      ...
      PSDisplayFormatDefDependencyHandler dfHandler =
          (PSDisplayFormatDefDependencyHandler)
              getDependencyHandler(PSDisplayFormatDefDependencyHandler.DEPENDENCY_TYPE);
      PSDisplayFormat df = dfHandler.loadDisplayFormat(proc, dfDep.getDependencyId());
      if (df == null) {
        Object[] args = {
   ...
   */
  private PSComponentSummary getChildFolderSummary(
      PSRelationshipProcessor proc, String[] names, PSLocator loc, int index)
      throws PSDeployException {
  ...
  private String[] parseFolderPath(String path) {
    ...
    while (tok.hasMoreTokens()) names.add(tok.nextToken());
