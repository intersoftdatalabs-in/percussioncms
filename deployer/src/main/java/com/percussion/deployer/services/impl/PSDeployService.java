 ...
 */
// todo: reconcile with
// system/src/main/java/com/percussion/deploy/services/impl/PSDeployService.java
...
public class PSDeployService implements IPSDeployService {
  ...
  /**
   * A helper method for installing dependency files within a transaction boundary. Hibernate does
   * not like child entities loaded in one session and then trying to save, again in a different
   * session. This service handles deserialization, applying transforms, persisting/updating via
   ...
   * @param depHandler the dependency handler which performs the transform ID Types and mappings
   *     never <code>null</code>
   * @param s PSSite, may or may not be <code>null</code>
   * @param ver version of the existing site, this needs to be persisted so that hibernate mananges
   *     the versioning for optimistic locking
  ...
  public void deserializeAndSaveSite(
    ...
    if (depFile == null) throw new IllegalArgumentException("deserialization file may not be null");
    if (ctx == null) throw new IllegalArgumentException("dependency context may not be null");
    if (depHandler == null)
      throw new IllegalArgumentException("dependency handler may not be null");
   ...
   * @param depHandler which performs the transform ID Types and mappings never <code>null</code>
   * @param t the template may or may not be <code>null</code>
   * @param ver version of the existing site, this needs to be persisted so that hibernate manages
   *     the versioning for optimistic locking
   * @param bVer the versions of the bindings, hibernate is again sensitive to the version numbers
   *     when updating existing bindings
   * @throws PSDeployServiceException
   */
  public void deserializeAndSaveTemplate(
    ...
    PSTemplateDefDependencyHandler dh = (PSTemplateDefDependencyHandler) depHandler;
    try {
   ...
   * A helper method for installing dependency files within a transaction boundary. Hibernate does
   * not like child entities loaded in one session and then trying to save, again in a different
   * session. This service handles deserialization, applying transforms, persiting/updating via
   * HibernateTransactionManager
   *
   * @param tok the security token never <code>null</code>
   * @param archive the ArchiveHandler to use to retrieve the files from the archive, may not be
  ...
  public void deserializeAndSaveFilter(
    ...
    PSFilterDefDependencyHandler dh = (PSFilterDefDependencyHandler) depHandler;
    try {
      IPSItemFilter filter = dh.findFilterByDependencyID(dep.getDependencyId());
   ...
   * @param depHandler which performs the transform ID Types and mappings never <code>null</code>
   * @param t the template may or may not be <code>null</code>
   * @param ver version of the existing variant, this needs to be persisted so that hibernate
   *     manages the versioning for optimistic locking
   * @throws PSDeployServiceException
   */
  public void deserializeAndSaveVariant(
    ...
    try {
      PSVariantDefDependencyHandler dh = (PSVariantDefDependencyHandler) depHandler;
   ...
   * PSDependency, PSImportCtx, IPSServiceDependencyHandler)} for details. Invokes {@link
   * IPSServiceDependencyHandler#doInstallDependencyFiles( PSSecurityToken, PSArchiveHandler,
   * PSDependency, PSImportCtx)}
   */
