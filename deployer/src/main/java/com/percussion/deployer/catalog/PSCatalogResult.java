...
public class PSCatalogResult implements IPSDeployComponent, Comparable {
  ...
  public PSCatalogResult(String id, String displayText) {
    ...
    if (displayText == null || displayText.trim().length() == 0)
      throw new IllegalArgumentException("displayText may not be null or empty.");
  ...
  public void addColumn(Object obj) {
    if (PSCatalogResultColumn.validateObject(obj)) m_columns.add(obj);
    else throw new IllegalArgumentException("obj is not an instance of supported column types");
  }
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_id = PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, XML_ID_NODE, true);
    m_displayText =
        PSDeployComponentUtils.getRequiredElement(tree, XML_NODE_NAME, XML_DISP_TEXT_NODE, true);
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, XML_DISP_TEXT_NODE, m_displayText);
    if (!m_columns.isEmpty()) {
      Element columns = PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_COLUMNS_NODE);
      for (int i = 0; i < m_columns.size(); i++) {
        String data;
        if (m_columns.get(i) instanceof Date) // if Date write the time
        data = String.valueOf(((Date) m_columns.get(i)).getTime());
        else data = m_columns.get(i).toString();

        Element columnEl = PSXmlDocumentBuilder.addElement(doc, columns, XML_COLUMN_NODE, data);
        columnEl.setAttribute(
            XML_COL_TYPE_ATTR, PSCatalogResultColumn.getTypeString(m_columns.get(i)));
      }
    }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    if (!(obj instanceof PSCatalogResult))
      throw new IllegalArgumentException("obj must be an instance of PSCatalogResult");
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSCatalogResult)) return false;

    PSCatalogResult result = (PSCatalogResult) obj;
    boolean equals = true;

    if (!m_id.equals(result.m_id)) equals = false;
    else if (!m_displayText.equals(result.m_displayText)) equals = false;
    else if (!m_columns.equals(result.m_columns)) equals = false;
