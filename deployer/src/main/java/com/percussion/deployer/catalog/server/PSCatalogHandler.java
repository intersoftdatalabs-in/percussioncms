...
public class PSCatalogHandler {
  ...
  public static Document processRequest(PSRequest request)
    ...
    for (int i = 0; i < children.getLength(); i++) {
      ...
      if (child instanceof Element) {
        props.setProperty(((Element) child).getTagName(), tree.getElementData(child));
      }
    }
  ...
  private static PSCatalogResultSet catalogArchives(Properties props) throws PSDeployException {
    ...
    while (logs.hasNext()) {
      ...
      File archiveFile = ms_depHandler.getImportArchiveFile(info.getArchiveRef());
      if (archiveFile.exists()) {
        PSCatalogResult result =
            new PSCatalogResult(String.valueOf(summary.getId()), info.getArchiveRef());
  ...
  private static PSCatalogResultColumn[] getArchiveColumns() {
    if (ms_archiveColumns == null) {
      ...
      ms_archiveColumns[0] =
          new PSCatalogResultColumn(ms_bundle.getString("name"), PSCatalogResultColumn.TYPE_TEXT);
      ms_archiveColumns[1] =
          new PSCatalogResultColumn(
              ms_bundle.getString("installDate"), PSCatalogResultColumn.TYPE_DATE);
      ms_archiveColumns[2] =
          new PSCatalogResultColumn(ms_bundle.getString("source"), PSCatalogResultColumn.TYPE_TEXT);
      ms_archiveColumns[3] =
          new PSCatalogResultColumn(
              ms_bundle.getString("version"), PSCatalogResultColumn.TYPE_TEXT);
      ms_archiveColumns[4] =
          new PSCatalogResultColumn(ms_bundle.getString("build"), PSCatalogResultColumn.TYPE_TEXT);
    }
  ...
  private static PSCatalogResultSet catalogCustomElementTypes(Properties props)
    ...
    while (customTypes.hasNext()) {
      ...
      PSCatalogResult result = new PSCatalogResult(def.getObjectType(), def.getObjectTypeName());
  ...
  private static PSCatalogResultSet catalogDescriptors(Properties props) throws PSDeployException {
    PSCatalogResultSet descriptors = new PSCatalogResultSet(getDescriptorColumns());

    try {
      File exportDescDir = PSDeploymentHandler.EXPORT_DESC_DIR;
      if (exportDescDir.exists()) {
        File[] descFiles = exportDescDir.listFiles(new PSFilenameFilter("xml", false));
        if (descFiles != null) {
          for (int i = 0; i < descFiles.length; i++) {
            Document descDoc =
                PSDeploymentHandler.getDocumentFromFile(descFiles[i], "export descriptor");
            PSExportDescriptor descriptor =
                new PSExportDescriptor(descDoc.getDocumentElement(), true);
            PSCatalogResult result =
  ...
  private static PSCatalogResultColumn[] getDescriptorColumns() {
    if (ms_descriptorColumns == null) {
      ...
      ms_descriptorColumns[0] =
          new PSCatalogResultColumn(ms_bundle.getString("name"), PSCatalogResultColumn.TYPE_TEXT);
      ms_descriptorColumns[1] =
          new PSCatalogResultColumn(
              ms_bundle.getString("description"), PSCatalogResultColumn.TYPE_TEXT);
      ms_descriptorColumns[2] =
          new PSCatalogResultColumn(
              ms_bundle.getString("modifiedDate"), PSCatalogResultColumn.TYPE_DATE);
      ms_descriptorColumns[3] =
          new PSCatalogResultColumn(
              ms_bundle.getString("version"), PSCatalogResultColumn.TYPE_TEXT);
    }
  ...
  private static PSCatalogResultSet catalogIDTypeObjects(PSSecurityToken tok, Properties props)
    ...
    while (objects.hasNext()) {
      ...
      PSCatalogResult result =
          new PSCatalogResult(object.getDependencyId(), object.getDisplayName());
  ...
  private static PSCatalogResultSet catalogPackageLogs(Properties props) throws PSDeployException {
    ...
    while (logs.hasNext()) {
      ...
      PSArchiveInfo info = summary.getArchiveInfo();
      PSCatalogResult result =
          new PSCatalogResult(String.valueOf(log.getId()), log.getPackage().getDisplayName());
  ...
  private static PSCatalogResultColumn[] getPackageLogColumns() {
    if (ms_packageLogColumns == null) {
      ...
      ms_packageLogColumns[0] =
          new PSCatalogResultColumn(ms_bundle.getString("name"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[1] =
          new PSCatalogResultColumn(
              ms_bundle.getString("packageType"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[2] =
          new PSCatalogResultColumn(
              ms_bundle.getString("description"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[3] =
          new PSCatalogResultColumn(
              ms_bundle.getString("archiveName"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[4] =
          new PSCatalogResultColumn(
              ms_bundle.getString("installDate"), PSCatalogResultColumn.TYPE_DATE);
      ms_packageLogColumns[5] =
          new PSCatalogResultColumn(ms_bundle.getString("source"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[6] =
          new PSCatalogResultColumn(
              ms_bundle.getString("version"), PSCatalogResultColumn.TYPE_TEXT);
      ms_packageLogColumns[7] =
          new PSCatalogResultColumn(ms_bundle.getString("build"), PSCatalogResultColumn.TYPE_TEXT);
    }
  ...
  private static PSCatalogResultSet catalogUserDependencies(Properties props)
    ...
    File catalogDir;
    if (directory != null) catalogDir = new File(curDirectory, directory);
    else catalogDir = new File(curDirectory);
    ...
    for (int i = 0; i < dirFiles.length; i++) {
      ...
      PSCatalogResult result = new PSCatalogResult(path, dirFiles[i].getName());
      resultSet.addResult(result);
    }
