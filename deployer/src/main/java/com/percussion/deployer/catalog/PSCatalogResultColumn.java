...
public final class PSCatalogResultColumn implements IPSDeployComponent {
  ...
  static int getType(String typeString) {
    if (typeString == null || typeString.trim().length() == 0)
      throw new IllegalArgumentException("typeString may not be null or empty.");
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    String type = PSDeployComponentUtils.getRequiredAttribute(sourceNode, XML_TYPE_ATTR);
    m_type = getType(type);
  ...
  public void copyFrom(IPSDeployComponent obj) {
    if (!(obj instanceof PSCatalogResultColumn))
      throw new IllegalArgumentException("obj must be an instance of PSCatalogResultColumn");
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSCatalogResultColumn)) return false;

    PSCatalogResultColumn resultColumn = (PSCatalogResultColumn) obj;
    boolean equals = true;

    if (!m_name.equals(resultColumn.m_name)) equals = false;
    else if (m_type != resultColumn.m_type) equals = false;
