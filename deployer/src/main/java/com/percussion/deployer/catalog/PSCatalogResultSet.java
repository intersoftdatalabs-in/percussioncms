...
public class PSCatalogResultSet implements IPSDeployComponent {
  ...
  public PSCatalogResultSet(PSCatalogResultColumn[] columns) {
    ...
    for (int i = 0; i < columns.length; i++) {
      if (columns[i] == null)
        throw new IllegalArgumentException("the objects in columns array may not be null");
    }
    m_columns = columns;
  }
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_columns = null;
    Element columnMeta = tree.getNextElement(XML_COLUMN_META, firstFlags);
    if (columnMeta != null) {
      ...
      m_columns =
          (PSCatalogResultColumn[])
              catalogResultColumns.toArray(new PSCatalogResultColumn[catalogResultColumns.size()]);
      tree.setCurrent(sourceNode);
    }

    m_results.clear();
    Element catalogResult = tree.getNextElement(PSCatalogResult.XML_NODE_NAME, firstFlags);

    while (catalogResult != null) {
      PSCatalogResult result = new PSCatalogResult(catalogResult);
      if (validateResultToAdd(result)) m_results.add(result);
      else {
        Object[] args = {XML_NODE_NAME, PSCatalogResult.XML_NODE_NAME, "invalid column data"};
        throw new PSUnknownNodeTypeException(IPSObjectStoreErrors.XML_ELEMENT_INVALID_CHILD, args);
      }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_columns != null) {
      Element columnMeta = PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_COLUMN_META);
      for (int i = 0; i < m_columns.length; i++) columnMeta.appendChild(m_columns[i].toXml(doc));
    }
  ...
  public void copyFrom(IPSDeployComponent obj) {
    if (!(obj instanceof PSCatalogResultSet))
      throw new IllegalArgumentException("obj must be an instance of PSCatalogResultSet");
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSCatalogResultSet)) return false;

    PSCatalogResultSet set = (PSCatalogResultSet) obj;
    boolean equals = true;

    if (m_columns != null && set.m_columns != null) {
      if (m_columns.length != set.m_columns.length) equals = false;
      else {
        for (int i = 0; i < m_columns.length; i++) {
    ...
    if (equals) equals = m_results.equals(set.m_results);
  ...
  public void addResult(PSCatalogResult result) {
    if (validateResultToAdd(result)) m_results.add(result);
    else throw new IllegalArgumentException("result is invalid to add to this set.");
  }
  ...
  public boolean validateResultToAdd(PSCatalogResult result) {
    ...
    if ((columnMeta == null && result.getColumns().length != 0)
        || (columnMeta != null && columnMeta.length != result.getColumns().length)) {
      canAdd = false;
