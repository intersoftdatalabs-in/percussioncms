...
public class PSPackageServiceTest {
  ...
  private boolean allEntriesUnique(PSPackages pkgs) {
    ...
    for (PSPackage pkg : pkgs.getPackages()) {
      if (entries.contains(pkg.getName())) return false;
  ...
  private PSPackage getEntryByName(PSPackages pkgs, String name) {
    for (PSPackage pkg : pkgs.getPackages()) {
      if (name.equals(pkg.getName())) return pkg;
    }
    return null;
  }

  /**
   * Creates a new instance using the supplied name, sets the required fields, sets the description
   * to the supplied name, and saves the guid in {@link #m_pkgInfoGuids}.
   *
  ...
  /**
   * Permanently deletes all pkg info entries whose descriptor name starts with the supplied prefix.
   *
  ...
  private IPSPkgInfoService m_pkgInfoService = PSPkgInfoServiceLocator.getPkgInfoService();

  private IPSConfigStatusMgr m_cfgStatusMgr =
      ((PSConfigService) PSConfigServiceLocator.getConfigService()).getConfigStatusManager();
}
