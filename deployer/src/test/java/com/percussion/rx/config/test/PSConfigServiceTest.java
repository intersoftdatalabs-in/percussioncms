...
public class PSConfigServiceTest extends PSConfigurationTest {
  ...
  public void testProcessLocalSpecChanges() throws PSNotFoundException {
    ...
    IPSDesignModelFactory dmFactory = PSDesignModelFactoryLocator.getDesignModelFactory();
    IPSDesignModel model = dmFactory.getDesignModel(PSTypeEnum.TEMPLATE);
    IPSAssemblyTemplate template = (IPSAssemblyTemplate) model.load(callOutTemplateGuid);
    ...
    PSItemDefinition itemDef = (PSItemDefinition) model.load(autoIndexTypeGuid);
  ...
  public void testUninstallConfigs() throws Exception {
    ...
    new PSConfigFilesFactoryTest(
        uninstConfig, CONFIG_DEF_SAMPLE, DEFAULT_CONFIG_SAMPLE, DEFAULT_CONFIG_SAMPLE);
    PSConfigService cfgS = (PSConfigService) PSConfigServiceLocator.getConfigService();
    File cdFile = cfgS.getConfigFile(ConfigTypes.CONFIG_DEF, uninstConfig);
    assertTrue(cdFile.exists());
    File visFile = cfgS.getConfigFile(ConfigTypes.VISIBILITY, uninstConfig);
    assertTrue(visFile.exists());

    cfgS.uninstallConfiguration(uninstConfig);
    cdFile = cfgS.getConfigFile(ConfigTypes.CONFIG_DEF, uninstConfig);
    assertFalse(cdFile.exists());
    File dcFile = cfgS.getConfigFile(ConfigTypes.DEFAULT_CONFIG, uninstConfig);
    assertFalse(dcFile.exists());
    File lcFile = cfgS.getConfigFile(ConfigTypes.LOCAL_CONFIG, uninstConfig);
  ...
  protected void setUp() throws Exception {
    ...
    m_cfgFactory =
        new PSConfigFilesFactoryTest(
            PKG_NAME, CONFIG_DEF_SAMPLE, DEFAULT_CONFIG_SAMPLE, DEFAULT_CONFIG_SAMPLE);
  ...
  private static final String CONFIG_DEF_SAMPLE =
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
          ...
          + "              <map>\r\n"
          + "                 <entry key=\"label\""
          + " value=\"${com.percussion.PSConfigServiceTest.templateLabel}\"/>\r\n"
          + "              </map>\r\n"
          + "           </property>\r\n"
          + "        </bean>\r\n"
          + "      </property>\r\n"
          ...
          + "              <map>\r\n"
          + "                 <entry key=\"label\""
          + " value=\"${com.percussion.PSConfigServiceTest.contenttypeLabel}\"/>\r\n"
          + "              </map>\r\n"
          + "           </property>\r\n"
          + "        </bean>\r\n"
          + "      </property>\r\n"
  ...
  private static final String DEFAULT_CONFIG_SAMPLE =
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
          + "<SolutionConfigurations publisherPrefix=\"com.percussion\""
          + " publisherName=\"Percussion\" type=\"config\""
          + " xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\""
          + " xsi:noNamespaceSchemaLocation=\"localConfig.xsd\">\r\n"
