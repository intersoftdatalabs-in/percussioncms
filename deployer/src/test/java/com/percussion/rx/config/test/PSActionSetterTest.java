...
public class PSActionSetterTest extends PSConfigurationTest {
  ...
  private void validateDefaultConfiguration() throws PSNotFoundException {
    ...
    PSAction action = (PSAction) model.load(gmgr.makeGuid(203, PSTypeEnum.ACTION));
    ...
    assertTrue(
        action
            .getDescription()
            .equals(
                "Transitions the current item into a Quick Edit state, check it out and opens it in"
                    + " edit mode."));
    // Check URL
    assertTrue(action.getURL().equals("../sys_action/transitcheckoutedit.xml"));
    // Check URL params
    assertTrue(action.getParameters().size() == 4);
    assertTrue((action.getParameters().getParameter("WFAction")).equals("Quick Edit"));
    // Assignment Types
    PSActionVisibilityContext acxt =
        action
    ...
    acxt =
        action
            .getVisibilityContexts()
            .getContext(PSActionVisibilityContext.VIS_CONTEXT_CONTENT_TYPE);
    assertTrue(acxt == null);
    // Folder Security
    acxt =
        action
            .getVisibilityContexts()
            .getContext(PSActionVisibilityContext.VIS_CONTEXT_FOLDER_SECURITY);
    assertTrue(acxt == null);
    // Locales
    acxt =
        action
            .getVisibilityContexts()
            .getContext(PSActionVisibilityContext.VIS_CONTEXT_LOCALES_TYPE);
    assertTrue(acxt == null);
    // Object Types
    acxt =
        action
            .getVisibilityContexts()
            .getContext(PSActionVisibilityContext.VIS_CONTEXT_OBJECT_TYPE);
    iter = acxt.iterator();
    newCxtVals = new ArrayList<String>();
    while (iter.hasNext()) {
      newCxtVals.add(iter.next());
    ...
    // Roles
    acxt =
        action.getVisibilityContexts().getContext(PSActionVisibilityContext.VIS_CONTEXT_ROLES_TYPE);
    assertTrue(acxt == null);
    // Workflow
    acxt =
        action
            .getVisibilityContexts()
            .getContext(PSActionVisibilityContext.VIS_CONTEXT_WORKFLOWS_TYPE);
    assertTrue(acxt == null);
  }
  ...
  private void validateLocalConfiguration() throws PSNotFoundException {
    ...
    // Check URL
    assertTrue(action.getURL().equals("../sys_action/transitcheckouteditLC.xml"));
    // Check URL params
    assertTrue(action.getParameters().size() == 4);
    assertTrue((action.getParameters().getParameter("WFAction")).equals("LCValue"));
