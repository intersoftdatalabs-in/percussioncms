...
public class PSContentTypeSetterTest extends PSConfigurationTest {
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    assertTrue("Expect \"Simple Workflow\"", defWorkflow.equals("Simple Workflow"));
  ...
  public void testSimpleProperties() throws Exception {
    ...
    PSAssociationSet aset =
        new PSAssociationSet(IPSAssociationSet.AssociationType.CONTENTTYPE_TEMPLATE);
    aSets.add(aset);
    aset = new PSAssociationSet(IPSAssociationSet.AssociationType.CONTENTTYPE_WORKFLOW);
    aSets.add(aset);
  ...
  public void testFieldProperties() throws Exception {
    ...
    while (params.hasNext()) {
      ...
      assertTrue(p.getName().equals("height") || p.getName().equals("width"));
      assertTrue(v.getText().equals("100") || v.getText().equals("40"));
    }
    ...
    List<PSPair<String, String>> theParams =
        (List<PSPair<String, String>>) defs.get("perc.pre.params");
   ...
   */
  public static void removeField(PSItemDefinition itemDef, String fieldName, int type) {
  ...
  private static boolean containAllSharedFields(PSSharedFieldGroup shGrp, List<String> names) {
   ...
   */
  private static void removeSharedGroup(PSItemDefinition itemDef, PSSharedFieldGroup shGrp) {
    PSContentEditorMapper mapper = itemDef.getContentEditorMapper();
  ...
  private IPSGuid validateLocalConfig() throws Exception {
    ...
    assertTrue(itemDef.getContentEditor().getIconSource().equals(PSContentEditor.ICON_SOURCE_NONE));
    ...
    while (params.hasNext()) {
      ...
      if (param.getName().equals("width")) assertTrue(param.getValue().getValueText().equals("40"));
  ...
  private void cleanup(IPSGuid ctId) throws Exception {
    ...
    removeField(itemDef, "webdavowner", PSField.TYPE_SHARED);
    removeField(itemDef, "img2_filename", PSField.TYPE_SHARED);
    ...
    assertTrue(
        itemDef.getContentEditor().getIconSource().equals(PSContentEditor.ICON_SOURCE_SPECIFIED));
  ...
  public void testConfigFiles_AddRemoveFields() throws Exception {
    ...
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, IMPL_CFG_2, DEFAULT_CFG_2, LOCAL_CFG_2);
