...
public class PSSiteConfigHandlerTest {
   ...
   * @param eiState the state of {@link #EI_SITE} site, may be <code>null</code>.
  ...
  private void processAndValidate(
    ...
    if (eiState == null) assertTrue(sites.size() == 1);
    else assertTrue(sites.size() == 2);
    for (PSPair<String, ObjectState> site : sites) {
      if (site.getFirst().equals(CI_SITE)) assertTrue(site.getSecond().equals(ciState));
      else assertTrue(site.getSecond().equals(eiState));
    }
  }
  ...
  public static final String CONFIG_DEF =
      "/com/percussion/rx/config/test/" + PKG_NAME + "_configDef.xml";
}
