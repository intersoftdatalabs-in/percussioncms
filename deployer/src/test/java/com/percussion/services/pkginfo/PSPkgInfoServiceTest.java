...
public class PSPkgInfoServiceTest {
  ...
  final PSGuid tcDescriptorGuid = new PSGuid(PSTypeEnum.DEPLOYER_DESCRIPTOR_ID, 2);
   ...
   * @param appendStr For debugging, the tring to append to differentiate string values
   */
  void initPkgInfoPojo(PSPkgInfo pkgInfo, Boolean initIdInfo, String appendStr) {
    ...
    pkgInfo.setShippedConfigDefinition(tcShippedConfigDefinition + appendStr);
  ...
  /**
   * Test creating Package Info objects. Even though in practice, the service will be used to create
   * these objects this is a base test to be sure they are created correctly without the service
   * being involved.
   *
   * @throws Exception If the test fails.
   */
  ...
  public void testPkgInfoPojo() throws Exception {
    ...
    assertEquals(pkgInfo.getShippedConfigDefinition(), tcShippedConfigDefinition);
  ...
  public void testPkgInfoServices() throws Exception {
    ...
    PSPkgInfo pkgInfo0 = pkgInfoService.createPkgInfo(tcDescriptorName + "_0");
    ...
    assertEquals(pkgInfo0.getPackageDescriptorName(), pkgInfoTest.getPackageDescriptorName());
    assertEquals(pkgInfo0.getPackageDescriptorGuid(), pkgInfoTest.getPackageDescriptorGuid());
    ...
    assertEquals(pkgInfo0.getPackageDescription(), pkgInfoTest.getPackageDescription());
    assertEquals(pkgInfo0.getPackageVersion(), pkgInfoTest.getPackageVersion());
    assertEquals(pkgInfo0.getShippedConfigDefinition(), pkgInfoTest.getShippedConfigDefinition());
    assertEquals(pkgInfo0.getLastActionDate(), pkgInfoTest.getLastActionDate());
    assertEquals(pkgInfo0.getOriginalConfigDate(), pkgInfoTest.getOriginalConfigDate());
    assertEquals(pkgInfo0.getLastActionByUser(), pkgInfoTest.getLastActionByUser());
    assertEquals(pkgInfo0.getLastActionStatus(), pkgInfoTest.getLastActionStatus());
    assertEquals(pkgInfo0.getLastAction(), pkgInfoTest.getLastAction());
    assertEquals(pkgInfo0.getType(), pkgInfoTest.getType());
    assertEquals(pkgInfo0.getCmVersionMinimum(), pkgInfoTest.getCmVersionMinimum());
    assertEquals(pkgInfo0.getCmVersionMaximum(), pkgInfoTest.getCmVersionMaximum());
    ...
    PSPkgInfo pkgInfo1 = pkgInfoService.createPkgInfo(tcDescriptorName + "_1");
    initPkgInfoPojo(pkgInfo1, false, "_1");
    pkgInfoService.savePkgInfo(pkgInfo1);

    PSPkgInfo pkgInfo2 = pkgInfoService.createPkgInfo(tcDescriptorName + "_2");
    ...
    // previously inserted.
    PSPkgInfo pkgInfoViaFind = pkgInfoService.findPkgInfo(pkgInfo0.getPackageDescriptorName());
    assertEquals(pkgInfo0, pkgInfoViaFind);
    pkgInfoViaFind = pkgInfoService.findPkgInfo(pkgInfo1.getPackageDescriptorName());
  ...
  /**
   * Tests the {@link PSPkgInfo#isCreated()} and {@link PSPkgInfo#isSuccessfullyInstalled()}
   * methods.
   */
  @Test
  ...
  public void testPkgElementServices() throws Exception {
    ...
    assertEquals(pkgElement.getPackageGuid(), pkgElementTest.getPackageGuid());
    ...
    // Load the modifiable version persisted object into a different
    // Package Info object and Test for these values with the getter
    PSPkgElement pkgElementTest1 = pkgInfoService.loadPkgElementModifiable(testGuid);
    ...
    List<IPSGuid> guidList = pkgInfoService.findPkgElementGuids(tcParentGuid);
    ...
    List<PSPkgElement> elementList = pkgInfoService.findPkgElements(tcParentGuid);
    ...
    PSPkgElement elementByObj = pkgInfoService.findPkgElementByObject(objectGuid);
    ...
    PSPkgElement remainingElement = pkgInfoService.findPkgElement(pkgElement.getGuid());
    assertNull(remainingElement);
    PSPkgElement remainingElement1 = pkgInfoService.findPkgElement(pkgElement1.getGuid());
    assertNull(remainingElement1);
    PSPkgElement remainingElement2 = pkgInfoService.findPkgElement(pkgElement2.getGuid());
  ...
  public void testPkgDependencyServices() throws Exception {
    ...
    // Use the setters (mutators) to set values for properties
    //
    final PSGuid tcDepPkgGuid = (PSGuid) guidMgr.createGuid(PSTypeEnum.PACKAGE_INFO);
    ...
    // object
    List<PSPkgDependency> pkgDepsTest = pkgInfoService.loadPkgDependencies(tcParentGuid, true);
    assertEquals(pkgDepsTest.size(), 1);
    PSPkgDependency pkgDepTest = pkgDepsTest.get(0);
    // Test for these values with the getters (accessors)
    //
    assertEquals(pkgDep.getId(), pkgDepTest.getId());
    assertEquals(pkgDep.getOwnerPackageGuid(), pkgDepTest.getOwnerPackageGuid());
    assertEquals(pkgDep.getDependentPackageGuid(), pkgDepTest.getDependentPackageGuid());
    assertEquals(pkgDep.isImpliedDep(), pkgDepTest.isImpliedDep());

    // Load the modifiable version persisted object into a different
    // Package Info object and Test for these values with the getter
    List<PSPkgDependency> pkgDepsTest1 =
        pkgInfoService.loadPkgDependenciesModifiable(tcParentGuid, true);
    ...
    final PSGuid tcDepPkgGuid1 = (PSGuid) guidMgr.createGuid(PSTypeEnum.PACKAGE_INFO);
    ...
    List<IPSGuid> guidList = pkgInfoService.findDependentPkgGuids(tcParentGuid);
    ...
    List<PSPkgDependency> pkgDeps =
        pkgInfoService.loadPkgDependenciesModifiable(tcParentGuid, true);
    // Make sure we have two deps
    assertEquals(2, pkgDeps.size());

    pkgInfoService.deletePkgDependency(pkgDeps.get(0).getId());
    pkgDeps = pkgInfoService.loadPkgDependenciesModifiable(tcParentGuid, true);
    assertEquals(1, pkgDeps.size());

    // Clean up these test objects (remove from persisted storage)
    pkgInfoService.deletePkgInfo(pkgInfo.getPackageDescriptorName());

    List<IPSGuid> guidList2 = pkgInfoService.findDependentPkgGuids(tcParentGuid);
