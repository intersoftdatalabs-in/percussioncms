...
public class PSTransactionLogSummaryTest extends TestCase {
  ...
  /**
   * Constructs a <code>PSTransactionSummary</code> object using the supplied params and catches any
   * exception. For params, see {@link PSTransactionSummary} ctor.
   *
  ...
  private boolean testCtorValid(
    ...
    try {
      PSTransactionSummary tranx =
          new PSTransactionSummary(logId, depDesc, elementName, elementType, action);
    } catch (Exception ex) {
      return false;
    }
