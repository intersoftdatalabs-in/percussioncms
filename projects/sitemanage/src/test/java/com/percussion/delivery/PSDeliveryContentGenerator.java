...
/**
 * This class provide services to create content in a delivery server. You have to specify a server
 * URL, an XML file defining the content to be generated, and an optional user and password to
 * authenticate against the delivery server. You are also able to do a cleanup of the remote data.
 *
...
public class PSDeliveryContentGenerator extends PSGenericContentGenerator<DeliveryServicesContent> {
  ...
  /**
   * The comment service instance. It's used to handle comments generation. Some dependencies of the
   * comment service are stubbed.
   */
  private IPSCommentsService commentService;

  /**
   * Delivery info service used to store connection data. Used by unsecured and secured connection.
   */
   ...
   */
  public PSDeliveryContentGenerator(
    ...
    commentService =
        new PSCommentsService(
            deliveryInfoService, getPageServiceStub(), getFolderHelperStub(), getSiteDaoStub());
  }
   ...
   * @param allowSelfSignedCertificate <code>true</code> if the allowSelfSignedCertificate property
  ...
  private IPSPageService getPageServiceStub() {
    return new IPSPageService() {
      ...
      public PSUnassignedResults getUnassignedPagesBySite(
          String sitename, Integer startIndex, Integer maxResults) {
        return null;
      }

      @Override
      public List<PSPageReportLine> findAllPages(String siteName)
  ...
  private IPSFolderHelper getFolderHelperStub() {
    return new IPSFolderHelper() {
      ...
      @Override
      public boolean isFolderValidForRecycleOrRestore(
          String targetPath,
          String relationshipTypeName,
          String targetRelType,
          String origRelType) {
        return false;
      }
      ...
      @Override
      public PSPair<String, String> fixupLastModified(
  ...
  private IPSiteDao getSiteDaoStub() {
    return new IPSiteDao() {
      ...
      public PSSite save(PSSite object)
   ...
   *
   * @param pagePath A pagepath where the sitename will be extracted from.
  ...
  private String getSiteName(String pagePath) {
    ...
    if (tmp == null || tmp.length < 2) throw new RuntimeException("Pagepath is invalid");

    return tmp[1];
  }

  /**
   * Given an absolute pagepath, it return a pagepath relative to the site. If the following
   * absolute pagepath is given: '/Site1/folder/page', then this relative pagepath will be returned:
   * '/folder/page'.
   *
  ...
  /** Generates content for the comment service only. Automatically loads the XML data. */
  public void generateComments() {
    // Load XML data in case this method is called directly from another
    // class.
    if (!loadXmlData()) return;

    if (content.getCommentService() != null && content.getCommentService().getComments() != null) {
  ...
  /** Generates content for the membership service only. Automatically loads the XML data. */
  ...
  /**
   * Creates comments according to the given Comments and PSDeliveryInfo objects. It handles the
   * case where Comments count is greater than 1, equals to 1, and less than 1, adding " Copy N" at
   * the end of every comment field.
   *
   * @param deliveryClient A PSDeliveryClient object to communicate with the delivery server. Never
   *     <code>null</code>.
   * @param server A PSDeliveryInfo with information of the delivery server (host, port, etc). Never
   *     <code>null</code>.
   * @param allComments A Comments object with all comments objects. Never <code>null</code>.
   */
  private void createComments(
      PSDeliveryClient deliveryClient, PSDeliveryInfo server, Comments allComments) {
    ...
    for (int i = 1; i <= count; i++) {
      for (Comment comment : allComments.getComment()) {
        ...
        values.add(
            new NameValuePair("username", addCopyNumber(comment.getUsername(), i, realCount)));
   ...
   * @param server A PSDeliveryInfo with information of the delivery server (host, port, etc). Never
   *     <code>null</code>.
   * @param memberships A Memberships object with all memberships objects. Never <code>null</code>.
   */
  ...
  /**
   * Cleans up content for the comment service only, removing all copies ("Copy N") in the delivery
   * server. Automatically loads the XML data.
   *
   * <p>The cleanup method works looking for all comments defined in the XML file. Then it gets all
   * the comments from the delivery server, according to the site and pagepath in the XML file. To
   * mark one for removal, it checks whether all the fields values in delivery comment start with
   * the corresponding one in the XML file.
   */
  public void cleanupComments() {
    ...
    if (content.getCommentService() != null && content.getCommentService().getComments() != null) {
      ...
      for (Comments comments : content.getCommentService().getComments()) {
        ...
        List<PSComment> commentsList =
            getAllComments(sitename, getRelativePagePath(comments.getPagePath()).substring(1));
        ...
        for (PSComment com : commentsList) {
          if (!commentPresentInXml(comments, com)) continue;
  ...
  /**
   * Cleans up content for the membership service only, removing all existing memberships in the
   * delivery server. Automatically loads the XML data. It runs before generation methods, cleaning
   * memberships that already exist, so then they can be recreated.
   *
   * <p>The cleanup method works looking for all membership accounts defined in the XML file. Then
   * it gets all the memberships from the delivery server. To mark one for removal, it checks
   * whether the email address in the delivery membership is equal to the corresponding one in the
   * XML file.
   */
      ...
      for (Memberships memberships : content.getMembershipService().getMemberships()) {
        for (Membership membership : memberships.getMembership()) {
          try {
            ...
            log.info(
                "Account " + membership.getEmailAddress() + " didn't exist or it wasn't deleted.");
          }
        }
      }
    }
  ...
  /**
   * * Calls delivery membership service using a secure connection and deletes the account that
   * matches the given email. If there isn't a membership account with that email, it throws an
   * exception.
   *
  ...
  private void deleteAccount(String email) {
    ...
    String url =
        "/"
            + PSDeliveryInfo.SERVICE_MEMBERSHIP
            + IPSMembershipService.MEMBERSHIP
            + IPSMembershipService.ADMIN_ACCOUNT
            + "/"
            + email;
  ...
  /**
   * Checks if the given comment is present in the XML file by comparing each field value with the
   * XML one. If all the fields in the given comment <em>start with</em> the value defined in the
   * XML, then the comment is marked for removal. This algorithm may lead to incorrect deletion if a
   * comment has all its fields as a substring of another one's.
   *
   * @param psComment A PSComment object to check if it's present in the XML file. Should never be
   *     <code>null</code>.
   * @return <code>true</code> if the comment is present, <code>false</code> otherwise.
   */
  private boolean commentPresentInXml(Comments comments, PSComment psComment) {
    for (Comment com : comments.getComment()) {
      ...
      if (StringUtils.startsWith(psComment.getCommentText(), com.getBody())
          ...
          && StringUtils.startsWith(psComment.getUserName(), com.getUsername())) return true;
    }
  ...
  /**
   * Gets all comments from the delivery server, according to the site and pagepath given.
   *
   * @param site The comment site. Should never be <code>null</code>.
   * @param pagepath The comment pagepath. It has to be in the following form: 'folder/path'
   *     (without the site nor leading slash). Should never be <code>null</code>.
   * @return A list of all comments in the delivery server that match with site and pagepath given.
   */
  ...
  /**
   * Given a comment's field value, an index and count (these ones corresponds to the values in the
   * 'count' attribute in the XML file), it returns a new field value with the 'Copy N' string, only
   * if it's necessary. If 'count' is equals to 1, then the unmodified field's value is returned.
   * Otherwise, a " Copy N" string is added at the end of the field's value, and this one is
   * returned.
   *
