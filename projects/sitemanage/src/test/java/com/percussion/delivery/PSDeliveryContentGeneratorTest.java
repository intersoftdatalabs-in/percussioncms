...
@RunWith(JMock.class)
@Ignore(
    "There are no Tomcat running in the continuos machine. If you want to "
...
public class PSDeliveryContentGeneratorTest {
  ...
  private static final String VALID_XML_FILE_COUNT_EQUALS_TO_ONE =
      "data/validCommentsXml_CountEqualsToOne.xml";
  private static final String VALID_XML_FILE_COUNT_LESS_THAN_ONE =
      "data/validCommentsXml_CountLessThanOne.xml";
  private static final String VALID_XML_FILE_MULTIPLE_COMMENTS_NODES =
      "data/validCommentsXml_MultipleComments.xml";
  ...
  private Comment[] expectedValidComments =
      new Comment[] {
        createComment(
            "Comment 1",
            "Comment 1 - Email",
  ...
  public PSDeliveryContentGeneratorTest() {
    ...
    commentService =
        new PSCommentsService(
            deliveryInfoService,
            context.mock(IPSPageService.class),
            context.mock(IPSFolderHelper.class),
            context.mock(IPSiteDao.class));
  }

  private Comment createComment(
      String title, String email, String body, String username, String url) {
  ...
  private void assertIsExpectedComment(Comment actualComment, Comment[] expectedComments) {
    ...
    for (Comment anExpectedComment : expectedComments) {
      if (actualComment.getTitle().equals(anExpectedComment.getTitle())) {
        ...
        assertEquals(
            "comment username", anExpectedComment.getUsername(), actualComment.getUsername());
    ...
    if (!expectedCommentFound) fail("invalid comment title");
  }
  ...
  private List<PSComment> getAllComments() {
    ...
    try {
      ...
      if (temp != null) allComments.addAll(temp);
  ...
  public void testContentGenerator_LoadsXmlFile_XmlIsValid() throws Exception {
    ...
    for (Comment com : commentsService.getComments().get(0).getComment()) {
      if (StringUtils.isBlank(com.getTitle())) fail("Comment's title must not be null");
  ...
  public void testContentGenerator_AddComments_CountGreaterThan1() throws Exception {
    ...
    for (int i = 0; i < allComments.size(); i++) {
      ...
      assertTrue(
          "comment - expected title, current: " + com.getCommentTitle(),
      ...
      assertEquals(
          "comment username", plainCommentTitle + " - Username" + copyN, com.getUserName());
  ...
  public void testContentGenerator_Cleanup() throws Exception {
    ...
    contentGenerator =
        new PSDeliveryContentGenerator(
            SERVER_URL,
            SECURE_SERVER_URL,
            true,
            xmlFile,
