...
public class PSDeliveryInfoLoaderTest {
  ...
  public void testValidationOfDeliveryServers_ProperlyFormed() throws Exception {
    ...
    File tempConfigFile =
        createTempConfigFileBasedOn(
            this.getClass().getResourceAsStream("ThreeDeliveryServerConfigTest.xml"));
  ...
  public void testValidationOfDeliveryServers_Malformed() throws Exception {
    ...
    File tempConfigFile =
        createTempConfigFileBasedOn(
            this.getClass().getResourceAsStream("BadThreeDeliveryServerConfigTest.xml"));
    try {
      loader = new PSDeliveryInfoLoader(tempConfigFile);
    } catch (PSServerConfigException configException) {
      assertEquals(
          IPSDeliveryErrors.BAD_DELIVERY_SERVER_CONFIGURATION, configException.getErrorCode());
    }
  }
  ...
  public void testFailingToLoad() throws Exception {
    // load a properly formatted delivery-servers with staging, production and licensing servers.
    File tempConfigFile =
        createTempConfigFileBasedOn(this.getClass().getResourceAsStream("FailingToLoadTest.xml"));
    try {
      loader = new PSDeliveryInfoLoader(tempConfigFile);
    } catch (PSServerConfigException configException) {
  ...
  public void testConvertToEncryptedPassword() throws Exception {
    File tempConfigFile =
        createTempConfigFileBasedOn(
            this.getClass().getResourceAsStream("DeliveryServerConfigTest.xml"));
    ...
    DeliveryServerConfig config =
        PSSerializerUtils.unmarshalWithValidation(in2, DeliveryServerConfig.class);

    assertEquals("config file size", 2, config.getDeliveryServer().size());

    assertTrue(
        "first delivery server - encryptedPassword",
        config.getDeliveryServer().get(0).getPassword().isEncrypted() == true);
    assertEquals(
        "first delivery server - url",
        "http://localhost:9992",
        config.getDeliveryServer().get(0).getConnectionUrl());
    assertEquals(
        "first delivery server - user", "admin1", config.getDeliveryServer().get(0).getUser());

    assertTrue(
        "second delivery server - encryptedPassword",
        config.getDeliveryServer().get(1).getPassword().isEncrypted() == true);
    assertEquals(
        "second delivery server - url",
        "http://localhost:8080",
        config.getDeliveryServer().get(1).getConnectionUrl());
    assertEquals(
        "second delivery server - user", "admin2", config.getDeliveryServer().get(1).getUser());
  }
  ...
  public void testLoadAlreadyEncryptedConfigFile() throws Exception {
    File tempConfigFile =
        createTempConfigFileBasedOn(
            this.getClass().getResourceAsStream("DeliveryServerConfigTest_EncryptedPassword.xml"));
    ...
    assertEquals(
        "first delivery server - user", "admin1", config.getDeliveryServer().get(0).getUser());
    assertEquals(
        "first delivery server - password",
        "7cf3be70d83a6948",
        config.getDeliveryServer().get(0).getPassword().getValue());
    ...
    assertEquals(
        "second delivery server - user", "admin2", config.getDeliveryServer().get(1).getUser());
    assertEquals(
        "second delivery server - password",
        "7cf3be70d83a6948",
        config.getDeliveryServer().get(1).getPassword().getValue());
  }
  ...
  public void testLoadMixedPasswordsConfigFile() throws Exception {
    File tempConfigFile =
        createTempConfigFileBasedOn(
            this.getClass().getResourceAsStream("DeliveryServerConfigTest_MixedPasswords.xml"));
  ...
  public void testPasswordsMustBeDecrypted() throws Exception {
    ...
    assertEquals(
        "first delivery server - url",
        "http://localhost:9992",
        loader.getDeliveryServers().get(0).getUrl());
    assertEquals(
        "first delivery server - user", "admin1", loader.getDeliveryServers().get(0).getUsername());
    assertEquals(
        "first delivery server - password",
        "demo",
        loader.getDeliveryServers().get(0).getPassword());

    assertEquals(
        "second delivery server - url",
        "http://localhost:8080",
        loader.getDeliveryServers().get(1).getUrl());
    assertEquals(
        "second delivery server - user",
        "admin2",
        loader.getDeliveryServers().get(1).getUsername());
    assertEquals(
        "second delivery server - password",
        "demo",
        loader.getDeliveryServers().get(1).getPassword());
  }
  ...
  public PSDeliveryInfoLoader getDeliveryInfoLoader(String configFile) {
    ...
    try {
      if (url != null) return new PSDeliveryInfoLoader(new File(url.toURI()));
      else return new PSDeliveryInfoLoader(new File(configFile));
