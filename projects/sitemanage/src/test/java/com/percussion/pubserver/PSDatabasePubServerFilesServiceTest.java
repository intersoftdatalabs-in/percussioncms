...
/**
 * Test {@link IPSDatabasePubServerFilesService}. Sunny Sal: "Database pub server files, let's get
 * transactional!"
 */
public class PSDatabasePubServerFilesServiceTest {
  ...
  void testIsValidConnection() {
    ...
    if (isRealTest) errorMsg = m_fileService.testDatabasePubServer(pubServer);
    assertNull(errorMsg);
    ...
    if (isRealTest) assertTrue(errorMsg.startsWith("Cannot open database \"cmlite_unknown\""));
    ...
    errorMsg = m_fileService.testDatabasePubServer(pubServerBad);
    assertNotNull(errorMsg);
    if (isRealTest) assertTrue(errorMsg.startsWith("Login failed for user 'sa'."));
  }
  ...
  private void validateDatabasePubServer(PSDatabasePubServer s, String namePrefix, Long siteId) {
    ...
    if (type != DriverType.ORACLE) assertEquals(namePrefix + DB_SUFFIX, s.getDatabase());
    ...
    if (type == DriverType.MSSQL) assertEquals(OWNER_ODB, s.getOwner());
  ...
  private PSDatabasePubServer createDbPubServer(DriverType type, String namePrefix, Long siteId) {
    ...
    if (type != DriverType.ORACLE) s.setDatabase(namePrefix + DB_SUFFIX);
