...
public class PSPubServerServiceTest extends PSRestTestCase<PSPubServerServiceRestClient> {
  ...
  public void test010GetServer() throws IPSPubServerService.PSPubServerServiceException {
    String siteId =
        testSiteData.getSiteRestClient().getProperties(testSiteData.site1.getName()).getId();

    // Returned servers list, the call to the service
    List<PSPublishServerInfo> returnedServersList =
        pubServerRestServiceClient.getPubServerList(siteId);
  ...
  public void test020GetServersList() throws IPSPubServerService.PSPubServerServiceException {
    ...
    for (PSPublishServerInfo info : returnedServersList) {
      if (info.getIsDefault()) assertTrue(info.getCanIncrementalPublish());
      else assertFalse(info.getCanIncrementalPublish());
    }
  }
  ...
  public void test030CreateServer() throws IPSPubServerService.PSPubServerServiceException {
    ...
    // Call the service
    PSPublishServerInfo returnedServer =
  ...
  public void test060UpdateServer() throws IPSPubServerService.PSPubServerServiceException {
    ...
    PSPublishServerInfo updatedServer =
        pubServerRestServiceClient.updatePubServer(siteId, oldServerId, newServerInfo);
  ...
  public void test070UpdateServerToFTPPublishing()
    ...
    PSPublishServerInfo updatedServer =
        pubServerRestServiceClient.updatePubServer(siteId, oldServerId, serverInfo);
  ...
  public void test090DeleteServer() throws IPSPubServerService.PSPubServerServiceException {
    ...
    // Call the service
    PSPublishServerInfo returnedServer =
        pubServerRestServiceClient.createPubServer(siteId, serverNameNoSpace, server);
    // Expected publishing server
    assertEquals(serverNameNoSpace, returnedServer.getServerName());
    assertEquals("File", returnedServer.getType());
    ...
    List<PSPublishServerInfo> servers =
        pubServerRestServiceClient.deleteServer(siteId, returnedServer.getServerId().toString());
  ...
  public void test100AmazonS3Server() throws IPSPubServerService.PSPubServerServiceException {
    ...
    addServerProperty(
        server,
        "bucketlocation",
        "http://cm1-s3-publishing-test.s3-website-us-east-1.amazonaws.com");
    addServerProperty(server, "accesskey", "abcd");
    addServerProperty(server, "securitykey", "1234");
    PSPublishServerInfo s3Server =
        pubServerRestServiceClient.createPubServer(siteId, serverName, server);
    ...
    assertTrue(
        s3Server
            .findProperty("bucketlocation")
            .equalsIgnoreCase("http://cm1-s3-publishing-test.s3-website-us-east-1.amazonaws.com"));
    ...
    returnedServersList = pubServerRestServiceClient.getPubServerList(siteId);
    assertEquals(
        "number of servers is one more than original",
        preServersSize + 1,
        returnedServersList.size());
    ...
    PSPublishServerInfo updatedServer =
        pubServerRestServiceClient.getPubServer(siteId, s3Server.getServerId().toString());
    ...
    returnedServersList = pubServerRestServiceClient.getPubServerList(siteId);
    assertEquals(
        "number of servers is same as original", preServersSize + 1, returnedServersList.size());
  }
