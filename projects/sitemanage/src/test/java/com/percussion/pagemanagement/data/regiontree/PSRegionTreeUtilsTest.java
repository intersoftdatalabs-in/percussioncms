...
public class PSRegionTreeUtilsTest {
  ...
  private static final String TEMPLATE_EMPTY_REGION_CASE_1_BOX =
      "template_emptyRegion_case1_box.xml";
  private static final String TEMPLATE_EMPTY_REGION_CASE_2_CUSTOM_BOX =
      "template_emptyRegion_case2_custom_box.xml";
  ...
  public void testGetEmptyRegions_case1Box() {
    ...
    testRegionSet(
        expected, getEmptyWidgetRegions(getTestRegionTree(TEMPLATE_EMPTY_REGION_CASE_1_BOX)));
  }
  ...
  public void testGetEmptyRegions_case2CustomBox() {
    ...
    testRegionSet(
        expected,
        getEmptyWidgetRegions(getTestRegionTree(TEMPLATE_EMPTY_REGION_CASE_2_CUSTOM_BOX)));
  }

  /**
   * Tests that the list of regions has all the regions ids present in the expected list.
   *
   * @param expected {@link List}<{@link String}> holding the expected region ids.
   * @param actual {@link List}<{@link PSRegion}> holding the regions whose ids we want to check.
   */
  private void testRegionSet(List<String> expected, Set<PSRegion> actual) {
    assertNotNull(actual);
    assertTrue(
        "The size of the actual set is wrong. [Expected = "
            + expected.size()
            + ", Actual = "
            + actual.size()
            + "]",
    ...
    for (PSRegion region : actual) {
      assertTrue(
          "Region "
              + region.getRegionId()
              + " should be in the list of expected regions. ["
              + expected
              + "]",
