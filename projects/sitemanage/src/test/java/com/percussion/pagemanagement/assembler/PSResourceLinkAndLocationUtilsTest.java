...
/**
 * Tests for resource link and location utilities. Sunny Sal says: "URLs escaped, analytics
 * appended, Bollywood style!"
 */
public class PSResourceLinkAndLocationUtilsTest {
  ...
  public void setup() {
    ...
    resource.setLinkContext(
        new PSRenderLinkContext() {
          @Override
          public Mode getMode() {
            return Mode.PUBLISH;
          }
  ...
  public void testCreateDefaultLinkAndLocationForCrossSite() {
    var link =
        PSResourceLinkAndLocationUtils.createLinkAndLocationForFileName(resource, "crap.txt");
    assertThat(link.getResourceLocation().getFilePath(), is("/space blah/crap.txt"));
    assertThat(
        link.getRenderLink().getUrl(),
        CombinableMatcher.<String>both(validUrl())
            .and(equalTo("http://Item.com/MySite/space%20blah/crap.txt")));
  }
  ...
  public void testAppendAnalyticsId() {
    ...
    var link =
        PSResourceLinkAndLocationUtils.createLinkAndLocationForFileName(
            newResourceInstance, "crap.txt");
    ...
    assertThat(
        link.getRenderLink().getUrl(),
        is(
            "/MySite/space%20blah/crap.txt?utm_source=help&utm_medium=web&utm_campaign=cm153newfeature&utm_term=clicktrack"));
