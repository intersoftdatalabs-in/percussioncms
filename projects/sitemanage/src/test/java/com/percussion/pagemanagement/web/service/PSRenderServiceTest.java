...
public class PSRenderServiceTest {
  ...
  public static void setUp() throws Exception {
    ...
    pageId =
        testSiteData.createPage(
            "MyPage", testSiteData.site1.getFolderPath(), testSiteData.template1.getId());
    ...
    template3 =
        testSiteData
            .getTemplateServiceClient()
            .createTemplate("SiteTemplateServiceTest3", testSiteData.baseTemplateId);
    testSiteData.assignTemplatesToSite(testSiteData.site1.getId(), template3.getId());

    String pageId_2 =
        testSiteData.createPage("MyPage_2", testSiteData.site1.getFolderPath(), template2.getId());
  ...
  public static PSTemplate setupTemplate(String templateId) {
    ...
    PSWidgetItem wi =
        testSiteData.createWidgetItem("TESTME", PSTestSiteData.TEST_WIDGET_DEFINITION);
  ...
  /**
   * Tests the template widget/asset cannot be override by the widget/asset changes. This is true
   * when the template already has a widget in a region, which cannot be overwrite by a page's
   * widget on the same region.
   *
   * @throws Exception
   */
  @Test
  public void testRenderPageCannotOverrideTemplateWidgetItemAndAsset() throws Exception {
    ...
    PSAssetWidgetRelationship awRel =
        createAssetRelationship(newPage, widgetItemId, "adam", "percRawHtml", asset.getId());
  ...
  public void testRenderPageWithPageAsset() throws Exception {
    ...
    PSAssetWidgetRelationship awRel =
        createAssetRelationship(page_2, wi.getId(), wi.getName(), "percRawHtml", asset_2.getId());
  ...
  public void testRenderTemplateWithWidgetItemAndAsset() throws Exception {
    ...
    PSAssetWidgetRelationship awRel =
        createAssetRelationship(template2, widgetItemId, "adam", "percRawHtml", asset.getId());
  ...
  private void assertAsset(String actual) {
    assertTrue(isNotBlank(actual));
    assertTrue(
        "Widget should render content properly. Did not find 'TestHTML' marker, actual: " + actual,
        contains(actual, HTML_CONTENT));
    assertTrue("Widget should have decoration", contains(actual, "perc-widget"));
  }

  private PSAssetWidgetRelationship createAssetRelationship(
      PSAbstractPersistantObject newPage,
      String widgetItemId,
      String widgetName,
      String widgetDefName,
      String assetId) {
  ...
  private PSWidgetItem assertWidgetItemFromRegion(
      PSRegionWidgetAssociations assocations, String regionId) {
    Map<String, List<PSWidgetItem>> wr = assocations.getRegionWidgetsMap();
    log.debug("WidgetRegions: " + wr);
    assertNotNull("Should have widget region", wr.get(regionId));
