...
public class PSRestFixtures {
  ...
  public PSTemplate createTemplate(String name, String srcId) {
    WebTarget wr = r.path(templateServiceRoot).path("create").path(name).path(srcId);
    PSTemplate template =
        getBuilder(wr, c).accept(MediaType.APPLICATION_JSON_TYPE).get(PSTemplate.class);
