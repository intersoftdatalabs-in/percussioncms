...
public class PSFolderPermissionUtilsTest {
  ...
  public void testGetPermission() throws Exception {
    ...
    assertTrue(
        permission.getAdminPrincipals().size() == 1
    ...
    assertTrue(
        permission.getWritePrincipals().size() == 1
    ...
    assertTrue(
        permission.getReadPrincipals().size() == 1
  ...
  /**
   * Tests {@link PSFolderPermissionUtils#setFolderPermission(PSFolder,
   * com.percussion.pathmanagement.data.PSFolderPermission.Access)}.
   *
   * @throws Exception
   */
  @Test
  ...
  public void testGetUserAcl() throws Exception {
    PSFolder folder =
        createFolder(PSFolderPermission.Access.ADMIN, PSFolderPermission.Access.WRITE, "user1");
    PSFolderPermission.Access acl =
        PSFolderPermissionUtils.getUserAcl(folder, "user1", Collections.singletonList(""))
            .getFirst();

    assertTrue("user1 should have WRITE permission", acl == PSFolderPermission.Access.WRITE);

    acl =
        PSFolderPermissionUtils.getUserAcl(folder, "user2", Collections.singletonList("Admin"))
            .getFirst();
   ...
   * @return the created folder, never <code>null</code>.
   */
  private PSFolder createFolder(
      PSFolderPermission.Access virtualAccess,
      PSFolderPermission.Access userAccess,
      String userName) {
    ...
    else permission.setReadPrincipals(Collections.singletonList(principle));
