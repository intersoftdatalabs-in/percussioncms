...
public class PSPathServiceRestClient extends PSDataServiceRestClient<PSPathItem> {
  ...
  public PSFolderProperties findFolderProperties(String id) {
    return getObjectFromPath(
        concatPath(getPath(), "folderProperties", id), PSFolderProperties.class);
  }

  public void saveFolderProperties(PSFolderProperties props) {
    PSNoContent response =
        postObjectToPath(concatPath(getPath(), "saveFolderProperties"), props, PSNoContent.class);
  ...
  public PSPagedItemList findChildren(
      String path, Integer startIndex, Integer maxResults, String child) {
    return findChildren(path, startIndex, maxResults, child, null);
  }

  public PSPagedItemList findChildren(
      String path, Integer startIndex, Integer maxResults, String child, Integer displayFormat) {
  ...
  public PSPagedItemList findChildren(
      ...
      String sortOrder) {
    return findChildren(
        path, startIndex, maxResults, child, displayFormat, sortColumn, sortOrder, null, null);
  }
  ...
  public PSPagedItemList findChildren(
      ...
      String category,
      String type) {
    String startIndexQueryParam =
        startIndex != null ? "startIndex=" + startIndex : StringUtils.EMPTY;
    String maxResultsQueryParam =
        maxResults != null ? "maxResults=" + maxResults : StringUtils.EMPTY;
    String childQueryParam = child != null ? "child=" + child : StringUtils.EMPTY;
    String displayQueryParam =
        displayFormat != null ? "displayFormatId=" + displayFormat : StringUtils.EMPTY;
    ...
    return getObjectFromPath(
        concatPath(getPath(), "paginatedFolder", path + param), PSPagedItemList.class);
  }

  private String addQueryParam(String existingParameterList, String newQueryParam) {
    if (StringUtils.isEmpty(newQueryParam)) return existingParameterList;

    if (StringUtils.isEmpty(existingParameterList)) return "?" + newQueryParam;
  ...
  public void moveItem(PSMoveFolderItem request) {
    PSNoContent response =
        postObjectToPath(concatPath(getPath(), "moveItem"), request, PSNoContent.class);
  ...
  public List<PSItemProperties> findItemProperties(PSItemByWfStateRequest request) {
    return postObjectToPathAndGetObjects(
        concatPath(getPath(), "item/wfState"), request, PSItemProperties.class);
  }
