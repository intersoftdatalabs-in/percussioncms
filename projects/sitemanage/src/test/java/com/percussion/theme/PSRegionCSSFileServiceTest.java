...
public class PSRegionCSSFileServiceTest {
  ...
  private String getSampleFilePath()
      throws IPSDataService.DataServiceLoadException, PSValidationException,
  ...
  private PSPurgableTempFile copySampleToTempFile()
      throws IOException, IPSDataService.DataServiceLoadException, PSValidationException,
          IPSDataService.DataServiceNotFoundException {
    PSPurgableTempFile tempCss = createTempCssFile();
    List<PSRegionCSS> regions = readFromSampleFile();
    cssService.write(tempCss.getAbsolutePath(), regions);
  ...
  public void testMergeRegionCSSFile() throws Exception {
    ...
    tree = getRegionTree();
    validateSourceMergeToTarget(
        tree, new ArrayList<PSRegionCSS>(), getSampleRegions_3(), getSampleRegions_3());
  ...
  // private String[] nameChildren = new String[]{"container", "header", "middle", "leftsidebar",
  // "content", "rightsidebar", "footer"};
