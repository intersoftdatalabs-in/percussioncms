...
public class PSDashboardGadgetGeneratorTest {
  ...
  public void testGadgetGenerator_LoadsXmlFile_XmlIsValid() throws Exception {
    ...
    for (GadgetDef gad : dashboardContent.getGadgetDef()) {
      if (gad.getGadgetType().equals("perc_comments_gadget")) {
        ...
        assertEquals(
            "comments gadget - setting 1 - name", "site", gad.getUserPref().get(0).getName());
        assertEquals(
            "comments gadget - setting 1 - value", "Site1", gad.getUserPref().get(0).getValue());

        assertEquals(
            "comments gadget - setting 2 - name", "zrows", gad.getUserPref().get(1).getName());
        assertEquals(
            "comments gadget - setting 2 - value", "5", gad.getUserPref().get(1).getValue());
        ...
        assertEquals(
            "workflow gadget - setting 1 - name", "site", gad.getUserPref().get(0).getName());
        assertEquals(
            "workflow gadget - setting 1 - value", "@all", gad.getUserPref().get(0).getValue());

        assertEquals(
            "workflow gadget - setting 2 - name", "status", gad.getUserPref().get(1).getName());
        assertEquals(
            "workflow gadget - setting 2 - value", "Pending", gad.getUserPref().get(1).getValue());

        assertEquals(
            "workflow gadget - setting 3 - name", "zrows", gad.getUserPref().get(2).getName());
        assertEquals(
            "workflow gadget - setting 3 - value", "5", gad.getUserPref().get(2).getValue());
      } else {
  ...
  public void testGadgetGenerator_Cleanup() throws Exception {
    ...
    contentGenerator = new PSDashboardGadgetGenerator(SERVER_URL, xmlFile, USERNAME, PASSWORD);
    contentGenerator.cleanup();
