...
@Disabled(
    "If you want to run these unit tests, adjust the SERVER_URL constant and start your CMS"
        + " server.")
public class PSDashboardGadgetGeneratorTest {
  ...
  private static final String VALID_XML_FILE_WITH_ALL_GADGETS =
      "data/validGadgetsXmlWithAllGadgets.xml";
  ...
  public void testGadgetGenerator_LoadsXmlFile_XmlIsValid() throws Exception {
    ...
    for (var gad : dashboardContent.getGadgetDef()) {
      if (gad.getGadgetType().equals("perc_comments_gadget")) {
        ...
        assertEquals(
            "Site1", gad.getUserPref().get(0).getValue(), "comments gadget - setting 1 - value");

        assertEquals(
            "zrows", gad.getUserPref().get(1).getName(), "comments gadget - setting 2 - name");
        assertEquals(
        ...
        assertEquals(
            "@all", gad.getUserPref().get(0).getValue(), "workflow gadget - setting 1 - value");

        assertEquals(
            "status", gad.getUserPref().get(1).getName(), "workflow gadget - setting 2 - name");
        assertEquals(
            "Pending", gad.getUserPref().get(1).getValue(), "workflow gadget - setting 2 - value");

        assertEquals(
            "zrows", gad.getUserPref().get(2).getName(), "workflow gadget - setting 3 - name");
        assertEquals(
            "5", gad.getUserPref().get(2).getValue(), "workflow gadget - setting 3 - value");
      } else {
    ...
    for (var aGadget : gadgets) {
      if (aGadget.getUrl().equals(getGadgetUrl("perc_comments_gadget"))) {
        ...
        assertEquals("5", aGadget.getSettings().get("zrows"), "1 - zrows");
          ...
          .getUrl()
          .equals(getGadgetUrl("cm1_welcome_gadget/perc_welcome_gadget.xml"))) {
        count += 11;
  ...
  @Test
  // TODO: Remove me @SuppressFBWarnings({"NP_NONNULL_PARAM_VIOLATION",
  ...
  public void testGadgetGenerator_MainMethod_SomeArgumentsNotSpecified() {
    assertThrows(
        IllegalArgumentException.class, () -> PSDashboardGadgetGenerator.main(new String[2]));
  }
  ...
  public void testGadgetGenerator_MainMethod_ServerURLIsEmptyOrNull() {
    assertThrows(
        ...
        () ->
            PSDashboardGadgetGenerator.main(
                new String[] {null, "username", "password", "filepath"}));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            PSDashboardGadgetGenerator.main(
                new String[] {StringUtils.EMPTY, "username", "password", "filepath"}));
  }
  ...
  public void testGadgetGenerator_MainMethod_XMLFileIsEmptyOrNull() {
    assertThrows(
        ...
        () ->
            PSDashboardGadgetGenerator.main(
                new String[] {"serverurl", "username", "password", null}));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            PSDashboardGadgetGenerator.main(
                new String[] {"serverurl", "username", "password", StringUtils.EMPTY}));
  }
  ...
  public void testGadgetGenerator_Cleanup() throws Exception {
    ...
    var contentGenerator2 =
        new PSDashboardGadgetGenerator(SERVER_URL, xmlFile2, USERNAME, PASSWORD);
