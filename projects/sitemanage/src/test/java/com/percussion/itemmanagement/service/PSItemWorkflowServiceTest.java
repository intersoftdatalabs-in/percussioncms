...
public class PSItemWorkflowServiceTest extends PSItemWorkflowServiceTestBase {
  public void testCheckIn() throws Exception {
    ...
    // create test page
    var page = createPage("testPage", templateId);
    ...
    // add assets to page
    var awRel1 = new PSAssetWidgetRelationship(pageId, 5, "widget5", localAssetId1, 1);
    assertNotNull(assetService.createAssetWidgetRelationship(awRel1));

    var awRel2 = new PSAssetWidgetRelationship(pageId, 6, "widget6", localAssetId2, 1);
    assertNotNull(assetService.createAssetWidgetRelationship(awRel2));

    var awRel3 = new PSAssetWidgetRelationship(pageId, 7, "widget7", sharedAssetId, 1);
  ...
  private void checkinPageWithOutOfSyncRelationship(String pageId, String localAssetId)
    ...
    var rel = validateLocalAssetRelationship(localAssetId, 2, pageId);
  ...
  private void validateRevisions(
      String pageId, String localAssetId, int curRevision, int tipRevision) {
    var pageSummary = getSummary(pageId);
    var summary = getSummary(localAssetId);
  ...
  private PSRelationship validateLocalAssetRelationship(
      String assetId, int assetRevision, String pageId) {
  ...
  public void testForceCheckinLocalAsset() throws Exception {
    ...
    PSAssetWidgetRelationship awRel3 =
        new PSAssetWidgetRelationship(page1Id, 7, "widget7", localAssetId, 1);
  ...
  public void testGetApprovedPages() throws Exception {
    ...
    PSAssetWidgetRelationship awRel1 =
        new PSAssetWidgetRelationship(pageId1, 5, "widget5", assetId, 1);
  ...
  public void testTransition() throws Exception {
    ...
    PSAssetWidgetRelationship awRel1 =
        new PSAssetWidgetRelationship(page1Id, 5, "widget5", sharedAssetId1, 1);
    awRel1.setResourceType(PSAssetResourceType.shared);
    String awRelId1 = assetService.createAssetWidgetRelationship(awRel1);
    relationshipCleaner.add(idMapper.getGuid(awRelId1));

    PSAssetWidgetRelationship awRel2 =
        new PSAssetWidgetRelationship(page1Id, 6, "widget6", sharedAssetId2, 1);
    ...
    PSRelationship rel1 =
        systemWs.createRelationship(
            ...
            idMapper.getGuid(sharedAssetId1),
    ...
    PSRelationship rel2 =
        systemWs.createRelationship(
            PSWidgetAssetRelationshipService.SHARED_ASSET_WIDGET_REL_TYPE,
            idMapper.getGuid(sharedAssetId2),
    ...
    // transition the page to Pending
    PSItemTransitionResults results =
        itemWorkflowService.transition(page1Id, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
  ...
  public void testTransitionRelatedNavigationItem()
      throws LoginException, IPSPageService.PSPageException, PSValidationException {
    securityWs.login("admin1", "demo", "Enterprise_Investments", null);

    PSPage homePage =
        fixture.getPageService().findPageByPath(fixture.site1.getFolderPath() + "/index.html");
    ...
    // It should be fine to transition the related nav-item while the nav-item is already in "live"
    // state
    try {
      workflowHelper.transitionRelatedNavigationItem(
          homePageId, IPSItemWorkflowService.TRANSITION_TRIGGER_LIVE);
      assertPageNavItemState(pageId, navContentId, PSWorkflowHelper.WF_STATE_LIVE);
    } catch (Exception ex) {
  ...
  public void testIsTriggerAvailable() throws Exception {
    ...
    assertTrue(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE));
    assertTrue(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_SUBMIT));

    // all other triggers should not be available
    assertFalse(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_REJECT));
    assertFalse(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_EDIT));
    assertFalse(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_LIVE));
    assertFalse(
        itemWorkflowService.isTriggerAvailable(
            pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_ARCHIVE));
