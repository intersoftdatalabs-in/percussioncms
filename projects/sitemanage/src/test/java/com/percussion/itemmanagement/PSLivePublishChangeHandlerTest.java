...
public class PSLivePublishChangeHandlerTest extends PSItemWorkflowServiceTestBase {
  ...
  protected void tearDown() throws Exception {
    ...
    var folders =
        contentWs.loadFolders(new String[] {PSPathUtils.getFolderPath(ASSETS_TEST_FOLDER)});
    if (!folders.isEmpty()) contentWs.deleteFolders(Arrays.asList(folders.get(0).getGuid()), true);
  }
    ...
    try {
      ...
      PSAbstractAssetRequest ar =
          new PSBinaryAssetRequest(
              ASSETS_TEST_FOLDER, AssetType.FILE, "test.txt", "text/plain", in);
      ...
      PSPublishServerInfo pubServerInfo =
          pubServerService.getPubServer(siteGuid.toString(), pubServer.getGUID().toString());

      List<PSPublishServerProperty> props = pubServerInfo.getProperties();
      for (PSPublishServerProperty property : props) {
        if (property.getKey().equals("HTML")) property.setValue("false");
        else if (property.getKey().equals("XML")) property.setValue("true");
      }
      pubServerService.updatePubServer(
          siteGuid.toString(), pubServer.getGUID().toString(), pubServerInfo);
      assertNothingToPublish(siteId);
      ...
      List<Integer> changes =
          changeService.getChangedContent(siteId, PSContentChangeType.PENDING_LIVE);
