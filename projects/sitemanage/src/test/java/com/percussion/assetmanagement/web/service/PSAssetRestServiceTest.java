...
public class PSAssetRestServiceTest extends PSRestTestCase<PSAssetServiceRestClient> {
  ...
  private static final String WIDGET_REGION_HTML =
  ...
  PSTestDataCleaner<String> assetCleaner =
      new PSTestDataCleaner<String>() {
        @Override
        protected void clean(String id) throws Exception {
  ...
  public void test040CreateAssetWidgetRelationship() throws Exception {
    ...
    PSAssetWidgetRelationship awRel =
        new PSAssetWidgetRelationship(testSiteData.template2.getId(), 5, "widget5", pageId, 1);
  ...
  public void test060GetWidgetAssetCriteria() throws Exception {
    ...
    List<PSAssetDropCriteria> dropCriteria =
        getAssetEditUrlRestClient().getWidgetAssetCriteria(pageId, true);
    ...
    PSAssetWidgetRelationship awRel =
        new PSAssetWidgetRelationship(
            template2Id, Long.parseLong(crit.getWidgetId()), "widget_test", pageId, 1);
    awRel.setResourceType(PSAssetResourceType.shared);
  ...
  public void test080DeleteAsset() throws Exception {
    ...
    try {
      ...
      fail(
          "Should not be able to delete asset: "
              + id
              + ".  It is used by an non-tip revison page in quick "
              + "edit");
    } catch (RestClientException e) {
      assertTrue(e.getStatus() == 400 && e.getResponseBody().contains("asset.deleteApprovedPages"));
    }
  ...
  public void test090ForceDeleteAsset() throws Exception {
    ...
    // transition the page to pending
    getItemWorkflowServiceRestClient()
        .transition(pageId, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
  ...
  public void test110RemoveResource() throws Exception {
    ...
    try {
      ...
      assertTrue(
          e.getStatus() == 400 && e.getResponseBody().contains("resource.removeNotAuthorized"));
    }
    ...
    try {
      ...
      assertTrue(
          e.getStatus() == 400 && e.getResponseBody().contains("resource.removeApprovedPages"));
    }
  ...
  public void test130GetForms() throws Exception {
    ...
    getItemWorkflowServiceRestClient()
        .transition(form1Id, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
    ...
    // transition form to Live state
    getItemWorkflowServiceRestClient()
        .transition(form2Id, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
  ...
  private void validateWithProperty(String pageId, PSWidgetItem wi, String templateId)
      throws Exception {
    ...
    template
        .getRegionTree()
        .setRegionWidgetAssociations(page.getRegionBranches().getRegionWidgetAssociations());
    getTemplateClient().saveTemplate(template);
  ...
  private void validateGetAssetEditUrls(
      String parentId, String parentType, PSWidgetItem widget, String assetId) throws Exception {
  ...
  private void validateUrl(String url, String hiddenProp, boolean isCreate) {
    if (hiddenProp == null) {
      ...
      assertTrue(
          url.indexOf("sys_view=" + IPSConstants.SYS_HIDDEN_FIELDS_VIEW_NAME + hiddenProp) != -1);
    }
