...
public class PSSiteSectionServiceTest extends PSRestTestCase<PSSiteSectionRestClient> {
  ...
  /**
   * Creates a site with a large number of site sections. Can be tweaked to adjust number of
   * sections at first level and then number of sub sections.
   *
   * <p>BEWARE that the {@link #tearDown()} method will delete the site, so comment that out before
   * running this test. Also, the test is ignored, so you need to switch the annotations.
   *
  ...
  private List<PSSiteSection> createSubSections(
      String parentPath, String templateId, int numSections, int numLevels) {
  ...
  public void test60BlogSection() {
    ...
    try {
      ...
      PSSiteSection blogSection =
          createSection(sectionLevel3.getFolderPath(), templateId, blogIndexTempId);
      ...
      // create a blog section
      PSSiteSection blogSection2 =
          createSection(sectionLevel42.getFolderPath(), templateId, blogIndexTempId);
   ...
   * @param templateId the template ID for creating landing page of the section, assumed not <code>
   *     null</code>.
   ...
   */
  private PSSiteSection createSection(
      String parentFolder, String templateId, String blogPostTemplateId) {
  ...
  private void updateSectionTest(PSSiteSection section, PSSiteSectionRestClient client) {
    ...
    assertTrue(
        properties.getFolderPermission().getAccessLevel() == PSFolderPermission.Access.WRITE);
    ...
    assertTrue(
        propsPerm.getAdminPrincipals().size() == 1
            && propsPerm.getAdminPrincipals().get(0).equals(admin));
    assertTrue(
        propsPerm.getWritePrincipals().size() == 1
            && propsPerm.getWritePrincipals().get(0).equals(writer));
    assertTrue(
        propsPerm.getReadPrincipals().size() == 1
   ...
   */
  private void replaceLandingPageTest(
      String siteFolder, PSSiteSectionRestClient client, String templateId) throws Exception {
   ...
   * @throws Exception if an error occurs creating a page.
   */
  private void replaceLandingPageSameFolderTest(
      String siteFolder, PSSiteSectionRestClient client, String templateId) throws Exception {
    // create a section
    ...
    String newLandingPageId =
        testSiteData.createPage("New-Landing-Page", sectionFolderPath, templateId);
   ...
   */
  private void replaceLandingPageDifferentFolderTest(
      String siteFolder, PSSiteSectionRestClient client, String templateId) throws Exception {
    // create new landing page
    ...
    testSiteData
        .getWorkflowClient()
        .transition(newLandingPageId, IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
    ...
    String landingPagePath =
        section.getFolderPath().replaceAll("//Sites", PSPathUtils.SITES_FINDER_ROOT)
            + "/"
            + ldPageName;

    // transition old landing page to "Pending"
    PSItemProperties itemProps =
        testSiteData.getPathRestClient().findItemProperties(landingPagePath);
    assertTrue(itemProps.getStatus().equals("Draft"));
    testSiteData
        .getWorkflowClient()
        .transition(itemProps.getId(), IPSItemWorkflowService.TRANSITION_TRIGGER_APPROVE);
    ...
    // After replace landing page, the state of new landing page should be from "Pending" to "Quick
    // Edit"
    itemProps = testSiteData.getPathRestClient().findItemProperties(landingPagePath);
    assertTrue(itemProps.getStatus().equals("Quick Edit"));

    // After replace landing page, the state of old landing page should be from "Pending" to "Quick
    // Edit"
   ...
   * @param client the REST client, assumed not <code>null</code>.
   */
  private void testReplaceLandingPage(
      PSSiteSection section, String newLandingPageId, PSSiteSectionRestClient client) {
  ...
  private String createBlogTemplate(String name, String srcId, String siteId) throws Exception {
    ...
    if (tempId != null) return tempId.getId();
