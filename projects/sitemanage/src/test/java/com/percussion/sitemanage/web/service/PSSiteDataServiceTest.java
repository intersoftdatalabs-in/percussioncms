...
public class PSSiteDataServiceTest extends PSRestTestCase<PSSiteRestClient> {
  ...
  PSTestDataCleaner<String> siteCleaner =
  ...
  PSTestDataCleaner<String> folderCleaner =
      new PSTestDataCleaner<String>() {
        @Override
        protected void clean(String id) throws Exception {
  ...
  public void testFailToSaveInvalidSite() throws Exception {
    ...
    expected.addAll(
        asList("name", "homePageTitle", "navigationTitle", "templateName", "templateId", "label"));
    try {
      restClient.save(s);
  ...
  public void testCreateSiteWithErikJson() throws Exception {
    try {
      String siteJson =
          "{\"Site\":{\"name\":\"TestA\",\"label\":\"TestA\",\"description\":\"TestA\","
  ...
  public void testCopy() throws Exception {
    ...
    String pageId =
        testSiteData.createPage(
            "testPage", testSiteData.site1.getFolderPath(), testSiteData.template1.getId());
    ...
    PSAssetWidgetRelationship awRel =
        new PSAssetWidgetRelationship(
            pageId, Long.parseLong(widgetId), "percPageAutoList", origAssetId, 1, wi.getName());
    String awRelId = testSiteData.getAssetRestClient().createAssetWidgetRelationship(awRel);
    assertNotNull(awRelId);
    restClient.login("Admin", "demo");

    List<PSAssetDropCriteria> orgDropCriteria =
        testSiteData.getAssetRestClient().getWidgetAssetCriteria(pageId, true);
    ...
    PSPage pageAfterCopy =
        testSiteData
            .getPageRestClient()
            .findPageByFullFolderPath(siteAfterCopy.getFolderPath() + "/" + "testPage");
    ...
    List<PSAssetDropCriteria> copyDropCriteria =
        testSiteData.getAssetRestClient().getWidgetAssetCriteria(pageAfterCopy.getId(), true);
  ...
  /**
   * Gets the author of the page that is passed as the second parameter. The site is used to get its
   * children. The page must be a child of the site, otherwise it will return <code>null</code>
   *
  ...
  private String getPageAfterCopyAuthor(PSSite siteAfterCopy, PSPage pageAfterCopy) {
    ...
    PSPagedItemList siteChildren =
        testSiteData
            .getPathRestClient()
            .findChildren(siteAfterCopyPath, startIndex, maxResults, child, displayFormatId);
  ...
  public void testCopyWithAssets() throws Exception {
    ...
    // Create a page
    String pageId =
        testSiteData.createPage("testPage", site.getFolderPath(), testSiteData.template1.getId());
    PSPage page = testSiteData.getPageRestClient().load(pageId);
    ...
    PSPage pageAfterCopy =
        testSiteData
            .getPageRestClient()
            .findPageByFullFolderPath(copy.getFolderPath() + "/" + "testPage");
  ...
  /**
   * Asserts that a page auto list asset exists on each of the specified pages with a different id
   * and query field value.
   *
  ...
  private PSPair<PSWidgetItem, PSAsset> createRichTextWidgetItem(
      String name, String description, String slotid) {
    PSAsset asset = new PSAsset();
  ...
  private void addWidgetsToTemplateAndSave(
      PSTemplate template, List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairs) throws Exception {
    notNull(template);
