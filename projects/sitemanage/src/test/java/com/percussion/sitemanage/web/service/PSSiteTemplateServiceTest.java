...
public class PSSiteTemplateServiceTest extends PSRestTestCase<PSSiteTemplateRestClient> {
  ...
  public void test010FindSitesByTemplate() throws Exception {
    ...
    List<PSSiteSummary> sites =
        testSiteData.getSiteTemplateRestClient().findSitesByTemplate(templateId);
    sw.stop();
  ...
  public void test020FindTemplatesBySite() throws Exception {
    ...
    List<PSTemplateSummary> templates =
        testSiteData.getSiteTemplateRestClient().findTemplatesBySite(siteId);
  ...
  public void test030FindTemplatesWithWidget() throws Exception {
    ...
    String siteId = testSiteData.site1.getId();
    List<PSTemplateSummary> templates =
        testSiteData.getSiteTemplateRestClient().findTemplatesBySite(siteId, "percRawHtml");
    assertTrue(
        "Should not have templates with 'widget' associated to a site.", templates.isEmpty());
  ...
  public void test040RenameTemplate() throws Exception {
    ...
    List<PSTemplateSummary> templates =
        testSiteData.getSiteTemplateRestClient().save(siteTemplates);
  ...
  public void test050Blog() throws Exception {
    ...
    PSSiteSectionProperties sectionProps =
        testSiteData.getSectionClient().getSectionProperties(section.getId());
    ...
    assertEquals(
        testSiteData.template1.getSourceTemplateName(), blogIndexTemplate.getSourceTemplateName());
    assertEquals(
        testSiteData.template2.getSourceTemplateName(), blogPostTemplate.getSourceTemplateName());
  ...
  public void test060GetBlogs() throws Exception {
    ...
    List<PSSiteBlogProperties> blogList =
        testSiteData.getSectionClient().getBlogsForSite(site.getName());
    ...
    String postId =
        testSiteData.createPage(
            "blogPost1", blogSection1.getFolderPath(), blogProperties.getBlogPostTemplateId());
    ...
    assertEquals(
        1,
        testSiteData.getSectionClient().getBlogsForSite(site.getName()).get(0).getBlogPostcount());

    PSSiteSection blogSection2 = createSection(site.getFolderPath(), template1Id, template2Id);

    // Create a section under blogSection2
    PSSiteSection blogSection3 =
        createSection(blogSection2.getFolderPath(), template1Id, template2Id);
    ...
    PSSiteSection sectionLink =
        testSiteData.createSectionLink(blogSection3.getId(), blogSection3.getId());

    blogList = testSiteData.getSectionClient().getBlogsForSite(site.getName());
    assertTrue("There are two child sections under root", blogList.size() == 3);

    // Now test for get all blogs for all existing sites. With getBlogs method blogs are created for
    // Site1
    // and with above code blogs are created under the Site2. So getAllBlogs should return all blogs
    // under these
    // two sites. NC. As well as any other sites already on the system. this list needs to be culled
    // down to the two sites
  ...
  public void test070GetBlogPosts() throws Exception {
    ...
    List<PSSiteBlogProperties> siteBlogProps =
        testSiteData.getSectionClient().getBlogsForSite(site.getName());
    ...
    String post1Id =
        testSiteData.createPage("blogPost1", blogSection.getFolderPath(), blogPostTemplateId);
    ...
    String post2Id =
        testSiteData.createPage("blogPost2", blogSection.getFolderPath(), blogPostTemplateId);
  ...
  public void test090CreateTemplateFromUrl() {
    ...
    PSTemplateSummary template =
        testSiteData.getSiteTemplateRestClient().createTemplateFromUrl(siteTemplates);
