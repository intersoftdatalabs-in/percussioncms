...
/** Tool to create sites from URLs from a file at a known location. // REFACTORED: CP-JAVA11 */
public class PSSiteSuckerTest extends PSServletTestCase {
  ...
  private void generateLogFile(List<PSSiteImportLogEntry> logEntries) {
    try {
      ...
      try (var htmlFile = new FileOutputStream(outputFileName)) {
        htmlFile.write(
            XmlTransform.getTransformedHtml(xmlFile, xsltFile).getBytes(StandardCharsets.UTF_8));
      }
    } catch (Exception e) {
  ...
  private String getLogUrl(String siteName) {
    return "http://"
        ...
        + ":"
        + PSServer.getListenerPort()
        + "/Rhythmyx/services/sitemanage/site/importLogViewer?siteName="
        + siteName;
  }
  ...
  static class XmlTransform {
    public static String getTransformedHtml(File xmlFile, File xsltFile)
  ...
  private static final String WEBSITE_URLS_FILE =
      PSServer.getRxDir() + "/SiteImporter/websiteurls.csv";
  private static final String ENTRIES_XML_FILE =
      PSServer.getRxDir() + "/SiteImporter/logEntries.xml";
  private static final String ENTRIES_TEMPLATE_FILE =
      PSServer.getRxDir() + "/SiteImporter/logEntries.xsl";
  private static final String REPORT_HTML_FILE =
      PSServer.getRxDir() + "/SiteImporter/SiteSuckerReport.html";
