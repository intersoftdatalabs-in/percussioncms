...
public class PSSiteDaoTest extends PSServletTestCase {
  ...
  public void testCRUD() throws Exception {
    ...
    try {
      ...
      assertEquals(
          "The default file extension must be set to: " + fileExt,
          fileExt,
          savedSite1.getDefaultFileExtention());
      assertTrue("Canonical should be ON", savedSite1.isCanonical());
      assertEquals(
          "The site protocol should be set to: " + siteProtocol,
          siteProtocol,
          savedSite1.getSiteProtocol());
      assertEquals(
          "The default document should be set to: " + defaultDocument,
          defaultDocument,
          savedSite1.getDefaultDocument());
      assertEquals(
          "The canonical links should be point to: " + canonicalDist,
          canonicalDist,
          savedSite1.getCanonicalDist());
  ...
  public void testDefault_defaultFileExtention() throws Exception {
    ...
    assertEquals(
        "The default file extension must be set to html", "html", site.getDefaultFileExtention());
  }
  ...
  public void testDefault_defaultDocument() throws Exception {
    PSSite site = createSite("Site-" + System.currentTimeMillis(), "Site");
    assertEquals(
        "The default document should be set to: index.html",
        "index.html",
        site.getDefaultDocument());
  }
  ...
  public void testUpdateSitePublishProperties() throws Exception {
    ...
    try {
      ...
      assertEquals(
          publishProps.getPublishType().toString(), getSiteDeliveryType(modifiedSite, siteDao));
  ...
  public void testOnDemandPublish() throws Exception {
    ...
    try {
      ...
      checkOnDemandPublishEdition(
          siteDao, siteName, psSite, deliveryType, PSSitePublishDaoHelper.PUBLISH_NOW);

      checkOnDemandPublishEdition(
          siteDao, siteName, psSite, deliveryType, PSSitePublishDaoHelper.UNPUBLISH_NOW);
    } finally {
      try {
  ...
  private void checkOnDemandPublishEdition(
      IPSiteDao siteDao, String siteName, IPSSite psSite, String deliveryType, String publishType)
    ...
    pubSvc.deleteContentLists(
        Collections.singletonList(pubSvc.findContentListByName(edtnCListName)));
    ...
    if (publishType == PSSitePublishDaoHelper.PUBLISH_NOW) siteDao.addPublishNow(psSite);
    else siteDao.addUnpublishNow(psSite);
    ...
    assertEquals(
        deliveryType,
        PSUrlUtils.getUrlParameterValue(cList.getUrl(), IPSHtmlParameters.SYS_DELIVERYTYPE));
  }
  ...
  public void testCopy() throws Exception {
    ...
    try {
      ...
      assertTrue(
          contentWs.isChildExistInFolder(
              site2.getFolderPath() + "/" + "sitefolder1", "site1page2"));
   ...
   * @return <code>true</code> if the items exist, <code>false</code> otherwise.
   * @throws Exception
   */
  private boolean hasRelatedItems(String siteName) throws Exception {
    ...
    List<PSItemSummary> items = contentWs.findFolderChildren(folderRoot, false);
   ...
   * @param title the title of the home page of the site, assumed not <code>null</code>.
   ...
   *
   * @param site
   * @param checkTemplate <code>true</code> if the home page template should be verified, <code>
