...
public class PSLinkExtractionHelperTest {
  ...
  protected class TestablePSLinkExtractionHelper extends PSLinkExtractionHelper {
    /** Test IPSConnectivity implementation */
    protected class PSLinkExtractionTestConnectivity implements IPSConnectivity {
      ...
      public Document get() throws IOException {
        if (mi_Document != null) return mi_Document;
        else throw new IOException("Unhandled content type \"application/pdf\" on URL");
      }
    ...
    protected String getCmsFolderPathForImageAssetsSiteName(
        String siteName, PSURLConverter urlConverter, String remoteUrl) {
  ...
  public void setUp() {
    ...
    try {
      ...
      mPageContent =
          testHelper.createTempPageBasedOnResource(
              "PSLinkExtractionHelperTest.html", PSLinkExtractionHelperTest.class, mContext);
    } catch (Exception e) {
  ...
  public void testFileUpload() throws PSSiteImportException, IPSGenericDao.SaveException {
    PSLinkExtractionHelper linkExtractionHelper =
        new TestablePSLinkExtractionHelper(null, null, null, null, null);
    ...
    for (Element link : links) {
      assertTrue(
          link.attr(PSLinkExtractor.HREF).contains(PSLinkExtractionHelper.ASSETS_DIR_PREFIX));
    }
  }
  ...
  public void testStandardPath() throws PSSiteImportException, IPSGenericDao.SaveException {
    PSLinkExtractionHelper linkExtractionHelper =
        new TestablePSLinkExtractionHelper(null, null, null, null, null);
    TestablePSLinkExtractionHelper.PSLinkExtractionTestConnectivity testConn =
        (TestablePSLinkExtractionHelper.PSLinkExtractionTestConnectivity)
            ((TestablePSLinkExtractionHelper) linkExtractionHelper).getConnectivity();
