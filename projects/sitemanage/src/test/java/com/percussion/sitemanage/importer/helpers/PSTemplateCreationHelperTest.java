...
class PSTemplateCreationHelperTest {
  ...
  /**
   * Tests if name is correctly extracted from different URLs. In case of URLs where name cannot be
   * extracted, tests if it's generated correctly.
   */
  @Test
  @Order(10)
  ...
  /**
   * Tests if template and page creation methods work. Checks that the created page uses the
   * template.
   */
  @Test
  @Order(20)
    ...
    try {
      newTemplate =
          templateService.createNewTemplate(
              siteData.getBaseTemplateName(), "Test-Template", siteData.getId());
    } catch (PSAssemblyException e) {
      fail();
    }
    assertNotNull(newTemplate);
    var newPage =
        templateCreationHelper.createNewPage(
            "Test-Page", newTemplate.getId(), siteData.getFolderPath());
    assertNotNull(newPage);
    assertEquals(newTemplate.getId(), newPage.getTemplateId());
  }

  /**
   * Tests if page and template name generation works correctly. Name generation has to avoid all
   * possible naming collisions with existing templates and pages.
   */
  @Test
  @Order(30)
  void testPageNameGeneration() throws PSDataServiceException {
    ...
    var templateName =
        siteTemplateService.generateNewTemplateName(genericNewTemplateName, siteData.getId());
    ...
    try {
      newTemplate =
          templateService.createNewTemplate(
              siteData.getBaseTemplateName(), templateName, siteData.getId());
    } catch (PSAssemblyException e) {
      fail();
    }
  ...
  /**
   * Tests main process method of templateCreationHelper is executed succesfully and context object
   * is correctly updated afterwards. Tests if rollback method actually deletes everything the
   * helper has created.
   */
  @Test
  @Order(40)
    ...
    try {
      ...
      try {
        ...
        assertEquals(
            ...
            templateSummary.getSourceTemplateName());
