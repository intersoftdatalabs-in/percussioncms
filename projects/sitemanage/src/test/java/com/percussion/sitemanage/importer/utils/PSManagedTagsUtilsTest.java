...
public class PSManagedTagsUtilsTest {
  ...
  public void testIsManagedJSReference_noScriptTag() {
    ...
    assertFalse(
        "The element " + title.toString() + " should not have been detected as managed.",
    ...
    assertFalse(
        "The element " + metadata.toString() + " should not have been detected as managed.",
    ...
    a.html("link text");
    assertFalse(
        "The element " + a.toString() + " should not have been detected as managed.",
  ...
  public void testIsManagedMetadataTag_managed() {
    for (String managedTag : MANAGED_META_TAGS) {
      ...
      assertTrue(
          "The element " + tag.toString() + " should have been detected as managed, but was not.",
  ...
  public void testIsManagedMetadataTag_notManaged() {
    for (String notManagedTag : NOT_MANAGED_META_TAGS) {
      ...
      assertFalse(
          "The element " + tag.toString() + " should not have been detected as managed.",
          PSManagedTagsUtils.isManagedMetadataTag(tag));
    }
  }
   ...
   *
   * @param prefix {@link String} with the prefix to use. Assumed not <code>null</code> but may be
   *     empty.
   */
  private void assertManagedJSElement(String prefix) {
    for (String sufix : MANAGED_JS_SUFIX) {
      ...
      assertTrue(
          "The element "
              + managedJs.toString()
              + " should have been detected as managed, but was not.",
  ...
  private void assertNotManagedJSElement(String prefix) {
    for (String sufix : NOT_MANAGED_JS_SUFIX) {
      Element managedJs = buildJSReferenceTag(prefix + sufix);
      assertFalse(
          "The element " + managedJs.toString() + " should not have been detected as managed.",
          PSManagedTagsUtils.isManagedJSReference(managedJs));
    }
  }
   ...
   *
   * @param src {@link String} with the source attribute. Assumed not <code>null</code> nor empty.
  ...
  private final String EXTERNAL_REFERENCE_PREFIX_1 =
      "http://ajax.googleapis.com/ajax/libs/jquery/1.8/";
  private final String EXTERNAL_REFERENCE_PREFIX_2 =
      "http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/";
  private final String EXTERNAL_REFERENCE_PREFIX_3 =
  ...
  private final String[] MANAGED_JS_SUFIX =
      new String[] {
        "jquery.js",
        "jquery.min.js",
        "jquery.ui.core.js",
  ...
  private final String[] NOT_MANAGED_JS_SUFIX =
