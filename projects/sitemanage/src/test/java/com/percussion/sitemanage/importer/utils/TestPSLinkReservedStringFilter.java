...
public class TestPSLinkReservedStringFilter {
  ...
  public void testFilter() {
    assertTrue(
        "Test Space Failed",
        PSReplacementFilter.filter("/this has spaces").equals("/this-has-spaces"));
    assertTrue(
        "Test Brackets Failed",
        PSReplacementFilter.filter("/this [has] brackets").equals("/this-has-brackets"));
    assertTrue(
        "Test Multiple Dashes Failed",
        PSReplacementFilter.filter("/this       [has]     brac--kets")
            .equals("/this-has-brac-kets"));
    assertTrue(
        "Test Encoded Space Failed",
        PSReplacementFilter.filter("/test/this%20has%20spaces").equals("/test/this-has-spaces"));
    assertTrue(
        "Test Backslash Failed",
        PSReplacementFilter.filter("\\test\\this has spaces").equals("/test/this-has-spaces"));
