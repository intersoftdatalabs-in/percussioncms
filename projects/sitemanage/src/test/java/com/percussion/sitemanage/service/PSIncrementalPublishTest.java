...
/**
 * Integration test for incremental publishing. // REFACTORED: CP-JAVA11
 *
...
public class PSIncrementalPublishTest {
  ...
  public void testIncrementalPublish() throws Exception {
    ...
    var pagedItems =
        sitePublishService.getQueuedIncrementalContent(
            fixture.site1.getName(), fixture.site1.getName(), 1, 5);
    ...
    pagedItems =
        sitePublishService.getQueuedIncrementalContent(
            fixture.site1.getName(), fixture.site1.getName(), 6, 6);
    assertNotNull(pagedItems);
    assertEquals(10, pagedItems.getChildrenCount().intValue());
  ...
  private void validatePathItems(
      Map<String, PSPage> pages, Map<String, PSAsset> assets, List<PSPathItem> pathItems) {
  ...
  private void validateAsset(PSAsset asset, PSPathItem pathItem) {
    ...
    assertEquals(
        PSPathUtils.getFinderPath(paths.get(0) + "/" + asset.getName()), pathItem.getPath());
    ...
    assertTrue(
        StringUtils.isNotBlank(props.get(IPSListViewHelper.CONTENT_LAST_MODIFIED_DATE_NAME)));
  ...
  private void validatePage(PSPage page, PSPathItem pathItem) {
    ...
    assertFalse(paths.isEmpty());
    assertEquals(
        PSPathUtils.getFinderPath(paths.get(0) + "/" + page.getName()), pathItem.getPath());
  ...
  private PSAsset createAndApproveAsset(String name)
    ...
    try (InputStream in =
        IOUtils.toInputStream("This is my file content", StandardCharsets.UTF_8)) {
      var ar =
          new PSBinaryAssetRequest("/Assets/Test", AssetType.FILE, "test.txt", "text/plain", in);
