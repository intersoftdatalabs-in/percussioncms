...
/**
 * Creates a page as defined in a {@link PageDef} object in a specified site folder. Also
 * transitions the generated page to a specified state.
 *
 * @author paulhoward
 */
public class PSPageGenerator extends PSItemGenerator<PSPageRestClient> {
  ...
  /**
   * If browser title or link title is not specified in the page, the page name is used for each.
   *
   * @param def Not <code>null</code>. If link title is not specified, it defaults to name. If
   *     browser title is not specified, it defaults to link title.
   * @param path Must start w/ a '/' and is relative to /Sites. Trailing slash is optional.
   * @return The generated page, never <code>null</code>.
   */
  public PSPage createPage(PageDef def, String path) {
    log.info(
        "Creating page '/Sites" + path + (path.endsWith("/") ? "" : "/") + def.getName() + "' ...");
    ...
    if (bTitle == null || bTitle.trim().isEmpty()) bTitle = page.getLinkTitle();
    ...
    if (def.getBlogPostTemplate() != null) // If a blog is being created
    {
      ...
      if (tsum == null) tsum = templateGen.findTemplateByName(def.getBlogPostTemplate());
