...
public abstract class PSItemGenerator<CLIENT_TYPE> extends PSGenerator<CLIENT_TYPE> {
  ...
  /**
   * Key is the target state name. Value is a String[] of transition triggers that are required to
   * reach the target state from the draft state. May be empty for a target state of draft. All keys
   * are lower-cased.
   */
  protected static Map<String, String[]> STATE_TRANSITION_MAP = new HashMap<String, String[]>();

  /**
   * See {@link PSGenerator#PSGenerator(Class, String, String, String) base class} for param
   * details.
   */
   ...
   *
   * @param itemGuid Assumed not <code>null</code> and a valid item guid.
   * @param targetStateName If <code>null</code>, returns immediately, otherwise, the name is looked
   *     up in a state/transition table (case-insensitive) to determine the appropriate transitions.
   *     If it is not a known state, no action is taken and a warning is logged.
   */
  ...
  static {
    ...
    STATE_TRANSITION_MAP.put(
        "quick edit", new String[] {"Submit", "Approve", "forcetolive", "Quick Edit"});
    STATE_TRANSITION_MAP.put(
        "archive", new String[] {"Submit", "Approve", "forcetolive", "Quick Edit", "Archive"});
  }
}
