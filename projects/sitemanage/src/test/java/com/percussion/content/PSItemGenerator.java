...
public abstract class PSItemGenerator<CLIENT_TYPE> extends PSGenerator<CLIENT_TYPE> {
   ...
   * @param targetStateName If null, returns immediately, otherwise, the name is looked up in a
   *     state/transition table (case-insensitive) to determine the appropriate transitions. If it
   *     is not a known state, no action is taken and a warning is logged.
   */
