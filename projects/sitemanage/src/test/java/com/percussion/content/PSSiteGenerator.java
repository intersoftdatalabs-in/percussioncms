...
public class PSSiteGenerator extends PSGenerator<PSSiteRestClient> {
   ...
   * @param path The path of the parent section. Of the form /Sites/sitename/section1/...
  ...
  public PSSite createSite(SiteDef def) {
    ...
    if (hp == null || hp.trim().isEmpty()) hp = "Home Page for Site " + site.getName();
    site.setHomePageTitle(hp);

    var nt = def.getNavTitle();
    if (nt == null || nt.trim().isEmpty()) nt = "Home";
    ...
    if (tName == null || tName.trim().isEmpty()) tName = def.getName() + "Template";
  ...
  /**
   * Helper method to create a section in the site. Also this method will create a blog section, if
  ...
  private PSSiteSection createSiteSection(SectionDef def, String parentPath) {
    ...
    if (linkTitle == null || linkTitle.trim().isEmpty()) linkTitle = def.getName();
        ...
        if (def.getBlogPostTemplate() != null) {
          ...
          if (tsum2 == null)
            throw new RuntimeException(
                "templateBlogName for SectionDef not found: " + def.getBlogPostTemplate());
  ...
  /**
   * Helper method to create an external link section. Given the information supplied, it will
   * create an object to send in the request in order to create the section.
   *
   * @param def Used to create the section.
   * @param path The path of the parent section. Of the form /Sites/sitename/section1/...
  ...
  /**
   * Helper method to create a section link. Given the information supplied, it will determine which
   * is the parent section folder, in order to get the Id appropriately. If the target section is
