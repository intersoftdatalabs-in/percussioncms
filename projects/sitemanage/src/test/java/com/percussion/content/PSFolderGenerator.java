...
/**
 * Given a path of the form /Sites/... or /Assets/... (the internal form of cm1 paths), creates all
 * folders that are not already present.
 *
 * @author paulhoward
 */
public class PSFolderGenerator extends PSGenerator<PSPathServiceRestClient> {
   ...
   *
   * @param path Assumed not {@code null}. Of the form /Assets/... or /Sites/...
   * @return The descriptor for the last path part, whether created or existing.
  ...
  public PSPathItem createFolderPath(String path) {
    ...
    var toker = new StringTokenizer(path, "/");
    var parts = toker.countTokens();
    var partialPath = new StringBuilder(100);
    PSPathItem result = null;
    for (int i = 0; i < parts; i++) {
      partialPath.append("/");
      partialPath.append(toker.nextToken());
      var curPath = partialPath.toString();
      try {
  ...
  public Collection<PSPathItem> getFolderPaths(String path) {
    var results = new ArrayList<PSPathItem>();
  ...
  private Collection<PSPathItem> getChildFolderPaths(String path) {
    var results = new ArrayList<PSPathItem>();
    var children = getRestClient().findChildren(path);
    for (var itemPath : children) {
