...
/**
 * Generates assets for Percussion CMS using REST client. Supports local and shared asset creation,
 * including file-based fields.
 */
public class PSAssetGenerator extends PSItemGenerator<PSAssetServiceRestClient> {
  /**
   * Stores the directory that should be used for any href's in an asset def that is relative. May
   * be {@code null}.
   */
  ...
  /**
   * See {@link PSItemGenerator#PSItemGenerator(Class, String, String, String) base class} for param
   * details.
   *
   * @param hrefBase If provided, any hrefs in the asset def will be interpreted relative to this
   *     directory.
   */
   ...
   *
   * @param def Never {@code null}.
   * @param path The directory the asset will be saved to, relative to /Assets, trailing / not a
   *     problem, will be stripped.
   * @return Never {@code null}.
   * @throws RuntimeException If the content comes from an href and a problem occurs.
   */
  ...
  private PSAsset doCreateAsset(AssetDef def, String path) {
    ...
    for (var field : def.getField()) {
      ...
      if (href.isEmpty()) {
        fieldSet.put(
            field.getName(),
            Optional.ofNullable(field.getValue()).map(Object::toString).orElse(""));
      } else {
        ...
        if (!f.exists()) {
          throw new RuntimeException(
              "File not found for field " + field.getName() + ": " + f.getAbsolutePath());
        }
        byte[] encoded;
