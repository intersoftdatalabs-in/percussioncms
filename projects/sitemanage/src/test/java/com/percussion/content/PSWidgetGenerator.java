...
public class PSWidgetGenerator extends PSItemGenerator<PSPageRestClient> {
  ...
  /**
   * Parse the regions where the widgets will be add to assign each one the corresponding a widget
   * to add.
   *
   * @param widgets The list of widgets you want to add.
   * @return A list which associated the widgets with the respective regions where they will be
   *     added.
   */
   ...
   *
   * @param regionToWidgets The regions of the page where the widgets should be will. <code>null
   *     </code> when the method call comes from the creation of a template.
   * @param resultWithRegion The regions of the template where the widgets will be added. <code>null
   *     </code> when the method call comes from the creation of a page.
   ...
   *
   * @param widgets The list of widgets you want to add.
   * @param template The template where the widgets should be added. <code>null</code> when the
   *     method call comes from the creation of a page.
   * @param page The page where the widgets should be added. <code>null</code> when the method call
   *     comes from the creation of a template.
   */
  ...
  static long getWidgetInstanceId(
      ...
      Widget widgetDef,
      Map<String, Map<String, Integer>> regionNameToWidgetCount) {
    Map<String, Integer> widgetTypeToInstanceCount =
        regionNameToWidgetCount.get(widgetDef.getRegionName());
