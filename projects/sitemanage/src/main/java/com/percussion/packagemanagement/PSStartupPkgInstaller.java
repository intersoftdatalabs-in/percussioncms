...
public class PSStartupPkgInstaller implements IPSNotificationListener, IPSMaintenanceProcess {
    ...
    public void uninstallPackages() {
        ...
        try {
            ...
            var entriesToUninstall = entries.stream()
                    .filter(entry -> PackageFileStatus.UNINSTALL.equals(entry.getStatus()) ||
    ...
    public void installPackages() {
        ...
        try {
            packageFileList = getPackageFileList();
            var entries = packageFileList.getEntries();
            var entriesToInstall = entries.stream()
                    .filter(entry -> !PackageFileStatus.INSTALLED.equals(entry.getStatus()) &&
    ...
    public void notifyEvent(PSNotificationEvent notification) {
        if (EventType.CORE_SERVER_POST_INIT.equals(notification.getType())) {
            ...
            try {
                ...
                var scheduler = (Scheduler) getWebApplicationContext().getBean("org.springframework.scheduling.quartz.SchedulerFactoryBean");
