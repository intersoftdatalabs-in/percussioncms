...
public class PSCommentsService implements IPSCommentsService {
  ...
  /**
   * Returns a list of all pages with comments for a given site on all delivery servers. Note that
   * if there are multiple delivery servers which purport to host a site of the same name, comments
   * between all servers will be aggregated.
   *
   * @param site The name of the site to be queried. Must match the site hostname.
   * @param max Maximum number of comments to be returned <strong>per delivery server</strong>. Can
   *     be used for paging.
   * @param start At which index to start returning comments, <strong>per delivery server</strong>.
   *     Can be used for paging.
   * @return A JSON object containing a list of all requested comments on all delivery servers.
   *     Object returned resembles the following:
   *     <p>{ summaries: [ { pagePath: '/somepath/page1', commentCount: 2, approvedCount: 2 }, {
   *     pagePath: '/somepath/page2', commentCount: 3, approvedCount: 1 } ] }
   */
  @GET
  ...
  public List<PSComment> getCommentsOnPage(
    ...
    try {
      ...
      PSDeliveryInfo server =
          deliveryService.findByService(PSDeliveryInfo.SERVICE_COMMENTS, null, adminURl);
      if (server == null) {
