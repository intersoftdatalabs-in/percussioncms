...
public interface IPSMetadataService {
   ...
   *
   * @param key the unique key used to retrieve the metadata object. Cannot be <code>null</code> or
   *     empty.
   * @return may be <code>null</code> if no matching entry was found in the database.
   */
   ...
   * @return a collection containing all of the located metadata objects, will be empty if none were
   *     found. Never <code>null</code>.
   */
  public Collection<PSMetadata> findByPrefix(String prefix) throws IPSGenericDao.LoadException;

  /**
   * Saves the passed in metadata object to the repository, replacing any existing entry that uses
   * the same key or creating a new entry if one does not yet exist.
   *
   ...
   *
   * @param key the unique key used to delete the metadata object. Cannot be <code>null</code> or
   *     empty.
   */
