...
public class PSCloudService implements IPSCloudService {
  ...
  public PSCloudServiceInfo getInfo() throws PSCloudServiceException {
    ...
    if (poLic == null) {
      throw new PSCloudServiceException(
          "No cloud services are not enabled for this instance of CM1");
    }
  ...
  /**
   * Generates client identity, creates a json object and returns to string of it.
   *
  ...
  /**
   * Generates the thumb url, if the thumbnail doesn't exist, it returns a running message.
   *
   * @param pageId assumed not <code>null</code>.
   * @param siteName assumed not <code>null</code>
  ...
  public PSModuleLicense getLicense(PSCloudLicenseType licenseType)
      throws PSLicenseServiceException {
    String licType =
        (licenseType == PSCloudLicenseType.PAGE_OPTIMIZER)
