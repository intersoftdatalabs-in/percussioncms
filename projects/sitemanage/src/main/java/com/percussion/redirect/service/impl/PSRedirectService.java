...
public class PSRedirectService implements IPSRedirectService {
  ...
  public PSRedirectValidationResponse validate(PSRedirectValidationData data) {
    ...
    try {
      ...
      response.setErrorMessage(
          "A redirect rule cannot be created at this time, Please note the time and date and submit this problem to Percussion support.");
    }
  ...
  private RedirectValidationStatus getFolderStatus(String path)
      throws DataServiceLoadException, Exception {
    RedirectValidationStatus status =
        folderHelper.findItems(path).isEmpty()
    ...
    if (lic != null) {
      ...
      if (response != null && response.getStatus() == 200) {
        ...
        if (response != null) {
          ...
          ret.setMessage(
              "An error occurred while saving the redirect.  Remote service returned status code: null");
        }
      }
    }
