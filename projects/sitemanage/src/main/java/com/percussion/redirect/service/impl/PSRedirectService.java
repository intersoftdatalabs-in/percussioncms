...
public class PSRedirectService implements IPSRedirectService {
    ...
    public PSRedirectValidationResponse validate(PSRedirectValidationData data) {
        ...
        try {
            var path = StringUtils.isBlank(data.getToPath())
    ...
    private RedirectValidationStatus validatePathStatus(String path, RedirectPathType type, PSSiteSummary site) throws DataServiceLoadException, Exception {
        ...
        if (type == RedirectPathType.PAGE) {
            ...
            status = isSitePublished ? RedirectValidationStatus.PUBLISHED : RedirectValidationStatus.NOT_PUBLISHED;
    ...
    public PSRedirectStatus createRedirect(PSCreateRedirectRequest request) {
        ...
        if (lic != null) {
            ...
            try {
                var target = client.target(lic.getApiProvider() + "/rest/redirect")
                        .path("entries");
                var builder = target.request(MediaType.APPLICATION_JSON_VALUE)
    ...
    public PSRedirectStatus status() {
        var ret = new PSRedirectStatus(PSRedirectStatus.SERVICE_ERROR, "Error");
        try {
            var lic = this.licenseService.findModuleLicense(PSLicenseService.MODULE_LICENSE_TYPE_REDIRECT);
            ret.setStatusCode(PSRedirectStatus.SERVICE_OK);
