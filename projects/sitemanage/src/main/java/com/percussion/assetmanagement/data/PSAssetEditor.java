...
public class PSAssetEditor extends PSAbstractDataObject {
  ...
  /**
   * The folder where the asset should be created. It can be added on to the {@link #getUrl() url}
   * as
   *
   * <pre>
   * &sys_folderid=100
   ...
   *
   * <p>The folder id is needed instead of the path because of the legacy content forms use it
   * instead of folder path.
   *
  ...
  /**
   * The id of the workflow the asset goes in, if the content type produces resource then it is set
   * to Local Content workfow id, otherwise shared content workflow id.
   *
   * @return never <code>null</code>. @TODO as we don't have shared content workflow, this value
   *     needs to be set to any other workflow that is available other than Local Content.
   */
  ...
  /**
   * @return the asset type if set, otherwise <code>null</code>. This value is meaningful, when the
   *     criteria is created for an asset. If it is not for an asset the value will be <code>null
   *     </code>.
   */
