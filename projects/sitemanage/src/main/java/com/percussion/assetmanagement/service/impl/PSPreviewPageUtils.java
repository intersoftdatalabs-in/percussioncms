...
public class PSPreviewPageUtils {
   ...
   *
   * @param template {@link PSTemplate} representing the template. Must not be <code>null</code>.
   * @return {@link Set}<{@link PSWidgetItem}> never <code>null</code>, but may be empty.
   */
  public static Set<PSWidgetItem> getEmptyTemplateWidgets(PSTemplate template) {
    ...
    Map<String, PSRelationship> mapSlotIdToRelationship =
        getSlotIdToRelationshipMap(templateAssets);
  ...
  /**
   * Gets those page widgets that are places in a page region, and that region is valid in the
   * template. If the template has widgets in a given region, that region is not taken into account
   * for return.
   *
  ...
  public static Set<PSWidgetItem> getPageWidgets(PSPage page, PSTemplate template) {
    ...
    if (template.getRegionTree() != null) {
      ...
      Map<String, List<PSWidgetItem>> pageRegionWidgetsMap =
          page.getRegionBranches().getRegionWidgetsMap();
   ...
   *
   * <p>The current implementation will only retrieve:
   * <li>the page assets that do not have matching widgets in page's template
   * <li>the page assets that have been overwritten by assets that belong to the page's template
   *
   *     <p>The orphan assets are calculated against the current revision of the page.
   *
   * @param page {@link PSPage} object representing the page for which we want to get the orphan
   *     assets.
   * @param template {@link PSTemplate} object representing the template used for the given page.
   *     Must not be <code>null</code>
   * @return a {@link Collection}<{@link PSRelationship}> with the unused assets. Never <code>null
   ...
   * @param template {@link PSTemplate} object representing the template used for the given page.
   *     Must not be <code>null</code>
   * @return a map that maps the widget ID (map key) to the used page asset (map value), never
   *     <code>null</code>.
   */
   ...
   * @param template {@link PSTemplate} object representing the template used for the given page.
   *     Must not be <code>null</code>
   * @param widgetToAsset the returned map that maps widget ID (map key) to the used page asset (map
   *     value). It is used to collect the used page assets. This may be <code>null</code> if only
   *     need to return the orphaned assets.
   * @return the orphaned page assets. It may be <code>null</code> if only need to collect used
   *     assets.
   */
  private static Collection<PSRelationship> getPageAssets(
      PSPage page, PSTemplate template, Map<String, PSRelationship> widgetToAsset) {
    notNull(page);
    notNull(template);
    ...
    for (PSWidgetItem widget : widgets) {
      ...
      if (matchingRelationship != null) {
        if (widgetToAsset != null) widgetToAsset.put(widget.getId(), matchingRelationship);
        else orphanAssets.remove(matchingRelationship);
      }
    }
  ...
  /**
   * Retrieves the list of orphan assets for a given page, calling {@link
   * PSPreviewPageUtils#getOrphanedPageAssets(PSPage, PSTemplate)}, and converts the result into
   * objects of {@link PSOrphanedAssetSummary}.
   *
   * @param page {@link PSPage} object representing the page for which we want to get the orphan
   *     assets.
   * @param template {@link PSTemplate} object representing the template used for the given page.
   *     Must not be <code>null</code>
   * @return a {@link Set}<{@link PSOrphanedAssetSummary}> with the unused assets. Never <code>null
   *     </code>, but may be empty.
   */
  public static Set<PSOrphanedAssetSummary> getOrphanedAssetsSummaries(
      PSPage page, PSTemplate template) {
    notNull(page);
    notNull(template);
    ...
    for (PSRelationship relationship : orphanAssets) {
      ...
      unusedAssets.add(
          new PSOrphanedAssetSummary(dependantId, slotId, widgetName, relationship.getId()));
    }
   ...
   * @return
   */
  private static Map<String, PSRelationship> getSlotIdToRelationshipMap(
      Collection<PSRelationship> templateAssets) {
