...
public class PSAssetDao implements IPSAssetDao {
  ...
  @Autowired
  public PSAssetDao(
      IPSContentItemDao contentItemDao, IPSContentMgr contentMgr, IPSIdMapper idMapper) {
  ...
  /**
   * Finds assets of the specified type which satisfy the specified where-clause fields in a JCR
   * query.
   *
  ...
  private static final String NONCOMPLIANT_IMAGE_ASSET_QUERY =
      "select rx:sys_contentid from rx:percImageAsset WHERE rx:alttext is null or rx:displaytitle"
          + " is null or rx:alttext like '%.%' or rx:displaytitle like '%.%' or"
          + " rx:resource_link_title is null or rx:resource_link_title like '%.%' order by jcr:path"
          + " asc";
  ...
  private static final String NONCOMPLIANT_FILE_ASSET_QUERY =
      "select rx:sys_contentid from rx:percFileAsset WHERE rx:displaytitle is null or"
          + " rx:displaytitle like '%.%' or rx:alttext is null or rx:alttext like '%.%' order by"
          + " jcr:path asc";
  ...
  private static final String ALL_FILE_ASSET_QUERY =
      "select rx:sys_contentid from rx:percFileAsset";
  ...
  private static final String ALL_ENCRYPTED_CONTENT_ASSET_REPORT =
      "All Assets with Encrypted Content";
  ...
  private List<PSAsset> runReport(String query, String reportName)
    ...
    try {
      ...
      log.error(
          "An error occurred executing the query for the {} report from the Content Repository.",
          reportName,
          e);
    ...
    try {
      ...
      log.error(
          "An error occurred retrieving the {} report from the Content Repository. Error: {}",
          reportName,
    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        log.error(
            "An error occurred retrieving an Asset for the {} report from the Content Repository."
                + " Error: {}",
            reportName,
            PSExceptionUtils.getMessageForLog(e));
        log.debug(PSExceptionUtils.getDebugMessageForLog(e));
