...
public class PSAssetDao implements IPSAssetDao {
  ...
  public void delete(String id) throws PSDataServiceException {
    ...
    try {
      ...
      if (paths == null || paths.isEmpty()) localOrOrphanedContent = true;
      else {
  ...
  private List<PSAsset> runReport(String query, String reportName)
    ...
    try {
      ...
      log.error(
          "An error occurred executing the query for the "
              + reportName
              + " report from the Content Repository.",
          e);
    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        log.error(
            "An error occurred retrieving an Image Asset for the  {} report from the Content Repository. Error: {}",
            reportName,
            PSExceptionUtils.getMessageForLog(e));
        log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      }
