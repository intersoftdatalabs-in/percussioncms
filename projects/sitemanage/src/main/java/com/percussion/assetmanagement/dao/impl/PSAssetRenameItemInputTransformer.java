...
/**
 * Renames the asset title if it's not unique in the given folder.
 ...
 * </pre>
 * <p>
 * Example: three consecutive assets that want to have the name: <code>stuff.jpg</code> in the same folder:
...
public class PSAssetRenameItemInputTransformer implements IPSItemInputTransformer {
    ...
    public void preProcessRequest(Object[] args, IPSRequestContext request) {
        ...
        // Remove any bad file name characters that our new title might have to avoid publishing problems.
    ...
    protected boolean isValidUniqueName(IPSRequestContext request, String newTitle, Action action) {
        ...
        if (isNotBlank(folderId) && isBlank(redirectUrl)) {
            ...
            log.warn(IPSHtmlParameters.SYS_FOLDERID + " or " + IPSHtmlParameters.SYS_ASSET_FOLDERID + " should be in the request but was not.");
        }
        try {
