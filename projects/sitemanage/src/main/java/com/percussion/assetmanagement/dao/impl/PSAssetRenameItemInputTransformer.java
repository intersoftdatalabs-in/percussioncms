...
/**
 * Renames the asset title if its not unique in the given folder. This extension runs before the
 ...
 * </pre>
 *
 * <p>An example of three consecutive assets that want to have the name: <code>stuff.jpg</code> in
 * the same folder: <code>/assets/uploads/images</code>
 *
 * <pre>
...
public class PSAssetRenameItemInputTransformer implements IPSItemInputTransformer {
  ...
  /**
   * Fields that end in this suffix and have the value: <code>true</code> indicate the file is
   * dirty.
   */
  ...
  /**
   * The maximum number of times that renaming will occur on the current asset. This is a fail safe
   * to avoid potentially infinite renaming loops if the server is incorrectly reporting that the
   * file fails unique sys_title validation.
   */
