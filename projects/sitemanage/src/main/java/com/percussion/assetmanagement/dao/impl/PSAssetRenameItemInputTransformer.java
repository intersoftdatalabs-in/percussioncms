...
/**
 * Renames the asset title if it's not unique in the given folder. This extension runs before the
 * item is saved and will only run if a {@link PSPurgableTempFile file} is in the request.
 *
 * <p>The {@link IPSHtmlParameters#SYS_TITLE system title} will be renamed accordingly:
 *
 * <p><em>Brackets are conditional, braces are not.</em>
 *
 * <pre>
 * {FileNameWithoutExt}[ (Try #)][.FileExt]
 * </pre>
 *
 * <p>Example: three consecutive assets that want to have the name: <code>stuff.jpg</code> in the
 * same folder: <code>/assets/uploads/images</code>
 *
 * <pre>
...
public class PSAssetRenameItemInputTransformer implements IPSItemInputTransformer {
  ...
  public void preProcessRequest(Object[] args, IPSRequestContext request) {
    ...
    // If the filename field is blank or null, use the system title.
    if (isBlank(newTitle)) {
    ...
    // If the new title is blank, let the sys_title validator fail (runs after this extension).
    if (isBlank(newTitle)) return;

    // Remove any bad file name characters that our new title might have to avoid publishing
    // problems.
    newTitle = SecureStringUtils.sanitizeFileName(newTitle);
    newTitle = PSFolderPathUtils.replaceInvalidItemNameCharacters(newTitle);

    // Make sure the title is unique and correct it if it's not.
    ...
    if (!newTitle.equals(title)) {
      if (log.isDebugEnabled())
        log.debug(
            format("Renaming asset id: {0} with title: {1} to: {2}", contentId, title, newTitle));
    } else {
      if (log.isDebugEnabled())
   ...
   * @return true if a file is in the request.
   */
  private boolean isFileModified(IPSRequestContext request) {
    var params = PSLegacyExtensionUtils.getParameterNames(request);
    for (var p : params) {
      // See if there is a field marked dirty.
      if (endsWith(p, FILE_FIELD_DIRTY_SUFFIX) && "true".equals(request.getParameter(p)))
        return true;
      // See if there is a non-empty temp file.
      var obj = request.getParameterObject(p);
      boolean isTempFile = obj instanceof PSPurgableTempFile;
      if (isTempFile) {
        var t = (PSPurgableTempFile) obj;
  ...
  /**
   * Determines what kind of content editor request this is: UPDATE, INSERT, or SKIP.
   *
   * @param request never null.
   * @return never null.
   */
   ...
   *
   * @param request never null.
   * @param action never null.
   * @param newTitle never null.
   * @return never null.
   */
   ...
   *
   * @param request never null.
   * @param newTitle never null or empty.
   * @param action never null.
  ...
  protected boolean isValidUniqueName(IPSRequestContext request, String newTitle, Action action) {
    ...
    folderId =
        folderId == null ? request.getParameter(IPSHtmlParameters.SYS_ASSET_FOLDERID) : folderId;
    ...
    if (isNotBlank(folderId) && isBlank(redirectUrl)) {
      ...
      log.warn(
          IPSHtmlParameters.SYS_FOLDERID
              ...
              + " should be in the request but was not.");
    }
    try {
      PSServerFolderProcessor.validateUniqueDepName(request, action == Action.INSERT);
    } catch (PSCmsException e) {
      // Continue trying to rename in this case.
      if (log.isTraceEnabled()) log.trace("Failed to rename asset to: " + newTitle, e);
      return false;
    } finally {
  ...
  private String renameTitle(String title, Integer renameTry) {
    ...
    if (renameTry != 0) newTitle = PSFolderPathUtils.addEnumeration(title, renameTry);
    return newTitle;
  }
