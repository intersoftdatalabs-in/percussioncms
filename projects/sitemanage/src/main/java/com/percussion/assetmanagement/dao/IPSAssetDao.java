...
public interface IPSAssetDao extends IPSGenericDao<PSAsset, String> {
    ...
    /**
     * Gets the asset from its identifier, similar to {@link #find(String)}, except caller can specify the returned asset
     * includes all fields or only the summary properties of the object.
     *
     * @param id the identifier (primary key) of the asset to get
     * @param isSummary true if load summary properties of the items, which does not include Clob or Blob type fields; otherwise load all properties of the items.
     ...
     */
    Collection<PSAsset> findByTypeAndWf(String type, int workflowId, int stateId) throws LoadException;
     ...
     * @return collection of assets, never null, may be empty.
     */
    Collection<PSAsset> findAllAssetsUsingEncryption() throws LoadException, PSReportFailedToRunException;
