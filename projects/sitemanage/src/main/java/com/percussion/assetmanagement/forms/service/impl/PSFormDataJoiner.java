...
/**
 * Merges the content of 0 or more CSV files, (each with a header and 0 or more rows of data) into a
 * single CSV file, with a single header and a row for every row of data in the source data. Handles
 * the case where different files have different headers by putting empty data for each row's cell
 * if that row did not originally have that field's data. The output rows are in the same order as
 * the input rows.
 *
...
public class PSFormDataJoiner {
  ...
  /**
   * Takes a Map generated by SuperCSV and returns a new one, but with all its keys normalized
   * (using CaselessString class).
   *
   * @param csvRow a Map generated with SuperCSV. It represents a row, with the header columns as
   *     keys. Assumed not <code>null</code>.
   * @return a new Map with the same information, but using a CaselessString object as the key.
  ...
  private Map<CaselessString, String> processCsvRow(Map<String, String> csvRow) {
    ...
    for (String key : csvRow.keySet()) processedRow.put(new CaselessString(key), csvRow.get(key));
  ...
  private ParsingResult parseCSVData(String[] data) {
    ...
    if (currentDataList.isEmpty()) return new ParsingResult();
   ...
   *
   * @param parsingResult Result of parsing raw form data. Assumed not <code>null</code>.
  ...
  private String writeCSV(ParsingResult parsingResult) {
    if (parsingResult.isEmpty()) return StringUtils.EMPTY;
  ...
  /**
   * Joins an array with forms data in CSV format (each cell escaped according to Excel rules) and
   * returns a merged CSV.
   *
   * @param data Array with forms data in CSV format. Each element in the array has a header and 0
   *     or more rows of a single form. Assumed not <code>null</code>.
   * @return A CSV with all forms data merged. It has the columns of all forms given in the 'data'
   *     argument. The first 2 columns are maintained, all other columns are in ascending alpha
