...
public class PSFormDataService implements IPSFormDataService {
    ...
    public List<PSFormSummary> getAllFormData(String site) {
        ...
        try {
            ...
            var getJson = deliveryClient.getJsonObject(
                    new PSDeliveryActionOptions(server, FORM_INFO_URL + "list", HttpMethodType.GET, true));
    ...
    public PSFormSummary getFormData(@PathParam("name") String name) {
        try {
            ...
            var deliveryClient = new PSDeliveryClient();
            var getJson = deliveryClient.getJsonObject(
                    new PSDeliveryActionOptions(processor, FORM_INFO_URL + name, HttpMethodType.GET, true));
            var formInfo = (JSONArray) getJson.get("formsInfo");
            if (!formInfo.isEmpty()) {
                sum = new PSFormSummary();
                sum.setName(name);
