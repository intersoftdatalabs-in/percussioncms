...
public class PSFormDataService implements IPSFormDataService {
   ...
   *
   * @return the server, it may be <code>null</code> if cannot find the server.
   */
  private PSDeliveryInfo findServer(String site)
    ...
    PSDeliveryInfo server =
        deliveryService.findByService(PSDeliveryInfo.SERVICE_FORMS, null, adminURl);
    if (server == null)
  ...
  public String exportFormData(@PathParam("site") String site, @PathParam("name") String name) {
    try {
      ...
      if (response != null) formsData.add(response);
  ...
  /** Url to get form information. Contains leading forward slash, but no trailing forward slash. */
