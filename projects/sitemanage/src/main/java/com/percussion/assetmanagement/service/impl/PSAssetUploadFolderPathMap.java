...
/**
 * Responsible for the mapping of Asset content type to Rhythmyx system folder path.
 *
 * <p>The paths specified by {@link #getTypeToFolderPathMap()} property are relative to the root
 * asset folder and can be configured through spring.
 *
 * <p>During runtime the folders are created on demand if they do not already exist.
 *
 * @author adamgent
 */
public class PSAssetUploadFolderPathMap {
   ...
   *
   * <p>It will use one of the folders that the asset is already saved in if it is in a folder
   * otherwise it will use {@link #getLegacyFolderIdForType(String)} for the assets type.
   *
  ...
  /**
   * Retrieves the uploads folder associated with an asset type. <strong>The folder will be created
   * if it does not exist.</strong> If the folder cannot be created or used the default folder path
   * will be used. If that fails an exception is thrown.
   *
   * @param type never <code>null</code>.
   * @return never <code>null</code>.
   * @throws PSAssetServiceException cannot find a folder path because even the default path is bad.
   */
