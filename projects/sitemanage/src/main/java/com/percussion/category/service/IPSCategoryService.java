...
public interface IPSCategoryService {
   ...
   * @throws PSDataServiceException if an error occurs retrieving categories.
   */
  PSCategory getCategoryTreeForSite(
  ...
  /**
   * Gets information about whether the category tab in the Administration UI is being used by an
   * admin.
   *
  ...
  /**
   * Updates a category in the DTS when it is modified for any of its properties. The method is
   * responsible for updating the relevant DTS based on the request.
   *
   * @param siteName the site in which the category is modified; must not be {@code null}.
   * @param deliveryServer the delivery server ("Staging" or "Production"); must not be {@code
   *     null}.
   */
   ...
   */
  PSCategoryNode findCategoryNode(
