...
public class PSCategoryPostExit implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    // Getting the data from the xml. Then filter it out based on the selecatable and deleted
    // attributes.
    ...
    if (siteName.equals("null")) siteName = null;
    try {

      PSCategory categoriesToReturn =
          PSCategoryControlUtils.getCategories(siteName, parentCategory, false, true);

      if (categoriesToReturn == null)
        throw new PSExtensionProcessingException(
            "Either non of the categories is selectable or the category xml is empty ! PSCategoryPostExit.processResultDocument()",
            new PSException());
