...
public class PSCategoryServiceUtil {
    ...
    private static void checkAndMapIds(List<PSCategoryNode> newCategories, Map<String, PSCategoryNode> titleMap,
                                       Map<String, PSCategoryNode> idMap, Set<String> newCategoryIds) {
    ...
    public static void publishToDTS(String category, String sitename, String deliveryServer, IPSDeliveryInfoService deliveryService) throws PSValidationException {
        var server = deliveryService.findByService(PSDeliveryInfo.SERVICE_INDEXER, deliveryServer.toUpperCase());
        if (server == null)
    ...
    private static List<String> getAllowedSitesAsList(List<String> parentSites, PSCategoryNode node) {
        ...
        if (node.getAllowedSites() != null) {
            nodeAllowedSites.addAll(Arrays.stream(StringUtils.split(node.getAllowedSites(), ","))
                    .map(String::trim)
    ...
    public static PSCategoryNode filterForSite(PSCategoryNode currentNode, String sitename,
        ...
        if (findPath != null && !findPath.isEmpty()) {
            ...
            if (!findPath.isEmpty()) {
                for (var child : currentNode.getChildNodes()) {
                    var testNode = filterForSite(child, sitename, findPath, nodeAllowedSites, relativePath, includeDeleted, includeNotSelectable);
                    if (testNode != null) {
                        foundNode = testNode;
                        break;
                    }
        ...
        if (!DUMMYROOT.equals(currentNode.getId()) && pathElement == null && !includeNotSelectable && !currentNode.isSelectable())
            return null;
        var filteredChildList = new ArrayList<PSCategoryNode>();
        for (var child : currentNode.getChildNodes()) {
            var testNode = filterForSite(child, sitename, null, nodeAllowedSites, relativePath, includeDeleted, includeNotSelectable);
            if (testNode != null) {
