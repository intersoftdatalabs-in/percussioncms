...
public class PSCategoryControlUtils {
  ...
  public static String getCategoryXmlInString(PSCategory category) {
    ...
    try {
      ...
      log.error(
          "JAXB Exception occurred while marshalling category object to xml string- PSCategoryControlUtils.getCategoryXmlInString() Error: {}",
          PSExceptionUtils.getMessageForLog(e));
    } catch (Exception e) {
      log.error(
          "Exception occurred while marshalling category object to xml string- PSCategoryControlUtils.getCategoryXmlInString() Error: {}",
          PSExceptionUtils.getMessageForLog(e));
    }
  ...
  private static PSCategoryNode filterNode(PSCategoryNode parentNode) {
    ...
    if (parentNode.getChildNodes() != null && !parentNode.getChildNodes().isEmpty()) {
      for (PSCategoryNode node : parentNode.getChildNodes()) {
        if (node.isSelectable()) childNodeList.add(node);
      }
  ...
  /**
   * Method to convert the category xml file in the format that other controls: checkboxtree and
   * pageautolist can understand and display.
   *
   * @param doc
      ...
      if (e.getName().equalsIgnoreCase("Children")) {
        ...
        for (Attribute attr : attributes) {
          ...
          if (attr.getName().equalsIgnoreCase("selectable")) {
            ...
            else newElement.addAttribute("selectable", "no");
          }
        }
        ...
        // then check if this element has child elements create new node element for each child
        // element.
  ...
  private static void createChildElement(Element source, Element targetParent) {
    ...
    for (Attribute attr : attributes) {
      if (attr.getName().equalsIgnoreCase("id")) newChild.addAttribute("id", attr.getStringValue());
      if (attr.getName().equalsIgnoreCase("title"))
        newChild.addAttribute("label", attr.getStringValue());
      if (attr.getName().equalsIgnoreCase("selectable")) {
        if (attr.getStringValue().equalsIgnoreCase("true"))
          newChild.addAttribute("selectable", "yes");
        else newChild.addAttribute("selectable", "no");
      }
    }
