...
public class PSCategoryControlUtils {
    ...
    /**
     * Converts the category XML file to a format that other controls (checkboxtree and pageautolist) can understand and display.
     *
            ...
            if ("Children".equalsIgnoreCase(e.getName())) {
                ...
                for (var attr : (List<Attribute>) e.attributes()) {
                    ...
                    if ("selectable".equalsIgnoreCase(attr.getName())) {
                        newElement.addAttribute("selectable", attr.getStringValue().equalsIgnoreCase("true") ? "yes" : "no");
                    }
                }
    ...
    /**
     * Supporting method to convert the category XML in the format that is understandable by checkboxtree and pageautolist.
     *
    ...
    private static void createChildElement(Element source, Element targetParent) {
        ...
        for (var attr : (List<Attribute>) source.attributes()) {
            ...
            if ("selectable".equalsIgnoreCase(attr.getName())) {
                newChild.addAttribute("selectable", attr.getStringValue().equalsIgnoreCase("true") ? "yes" : "no");
            }
        }
