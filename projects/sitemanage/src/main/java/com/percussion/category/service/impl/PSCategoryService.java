...
public class PSCategoryService implements IPSCategoryService {
    ...
    private boolean createIds(PSCategoryNode node) {
        ...
        for (var sub : node.getChildNodes())
    ...
    private PSCategoryNode findCategoryNode(PSCategory category, String sitename, String rootPath, boolean includeDeleted, boolean includeNotSelectable) {
        var findPath = new LinkedList<String>();
        boolean relativePath = false;
        if (rootPath != null) {
            relativePath = (!rootPath.startsWith("/"));
            findPath = Arrays.stream(StringUtils.split(rootPath, "/")).collect(Collectors.toCollection(LinkedList::new));
        }
        if (!findPath.isEmpty()) {
            var checkElement = findPath.peek();
            while (!findPath.isEmpty() && ("Categories".equals(checkElement) || StringUtils.isEmpty(checkElement))) {
    ...
    public PSCategory updateCategories(PSCategory category, String sitename) throws PSValidationException {
        ...
        if (updatedCategory == null) {
            log.error("The updated categories are null ! - PSCategoryService.updateCategories()", new PSDataServiceException("Updated Categories are null"));
        } else if (updatedCategory.getTopLevelNodes() != null && !updatedCategory.getTopLevelNodes().isEmpty()) {
            var nodesToRemove = PSCategoryServiceUtil.removeDeletedNodes(updatedCategory.getTopLevelNodes(), new HashSet<>());
    ...
    public String getLockInfo() {
        ...
        if (jsonObject != null) {
            try {
                ...
                log.error("JSON Exception occurred while reading from the json object - PSCategoryService.getLockInfo()", new PSDataServiceException("Could not read lock information file"));
            }
        } else {
            try {
                ...
                log.error("JSON Exception occurred while creating empty json object - PSCategoryService.getLockInfo()", new WebApplicationException("No lock on category tab. Could not create an empty json to return from api."));
            }
        }
        return jsonObject.toString();
    }
    ...
    public void lockCategoryTab(@PathParam("date") String date) {
        ...
        try {
            ...
            log.error("JSON Exception occurred while reading from the json object - PSCategoryService.overrideCatTabLock()", new WebApplicationException("Could not read lock information file"));
        }
    }
