...
/**
 * This service provides various utility methods to get the activity of the content on a single site
 * or all sites.
 */
public interface IPSActivityService {
   ...
   * is <code>PSPathUtils#SITES_FINDER_ROOT</code>.
   *
   * @param path the path in question, must not be blank.
   * @param includeSite <code>true</code> to include an activity node for the site(s), <code>false
   *     </code> otherwise.
   ...
   *
   * @param start A date in the format yyyy-MM-dd. Never <code>null</code> or empty.
   * @param end A date in the format yyyy-MM-dd. Never <code>null</code> or empty.
   * @param granularity The string representation of one of the values of {@link
   *     PSDateRange.Granularity}. Never <code>null</code> or empty.
   ...
   *
   * @param path the specified folder path, not <code>null</code> or empty.
   * @param contentTypes a list of content type names, it may be <code>null</code> or empty, which
   *     is equivalent to all content types.
   * @return a list of content IDs of the page items under the specified folder.
   */
   ...
   * @param contentIds a list of specified item IDs, not <code>null</code>.
   * @param dates a list of specified date range, it must contain more than one Date element.
   * @param stateName the workflow state name the items transition to, not <code>null</code> or
   *     empty.
   * @param transitionName the transition name that is used to transition the items to the above
   *     state, not <code>null</code> or empty.
   ...
   * @param transitionName the transition name that is used to transition the items to the above
   *     state, not <code>null</code> or empty.
   * @return a list of content activities in the specified date ranges. If beginData is not <= to
