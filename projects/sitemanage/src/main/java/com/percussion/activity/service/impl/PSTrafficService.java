...
public class PSTrafficService implements IPSTrafficService {
  ...
  /**
   * Helper method to create PSdate range from sting dates and granularity. The end date of the
  ...
  private int findPageAnalyticsCount(
    ...
    for (IPSAnalyticsQueryResult visit : visitResults) {
      String analyticsPath = visit.getString(IPSAnalyticsProviderQueryService.FIELD_PAGE_PATH);
  ...
  private static final String NOT_FOUND_ERROR =
      "Unable to retrieve analytics data. Please use the Google Setup gadget to select a profile for the desired site(s).";
