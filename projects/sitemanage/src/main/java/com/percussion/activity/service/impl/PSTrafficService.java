...
public class PSTrafficService implements IPSTrafficService {
    ...
    public PSContentTraffic getContentTraffic(PSContentTrafficRequest request)
        ...
        if (dataReq.contains(PSTrafficTypeEnum.UPDATED_PAGES.toString())) {
            var activity = activityService.findNumberContentActivities(pageIds, dateList, WF_STATE_LIVE, null);
            for (int i = 0; i < activity.size(); i++) {
                pageUpdates.add(i, activity.get(i) - newPages.get(i));
            }
        }
        if (dataReq.contains(PSTrafficTypeEnum.TAKE_DOWNS.toString())) {
            takeDowns.addAll(activityService.findNumberContentActivities(pageIds, dateList, WF_STATE_ARCHIVE, WF_TAKE_DOWN_TRANSITION));
        }
        if (dataReq.contains(PSTrafficTypeEnum.VISITS.toString())) {
            try {
                visits.addAll(createAnalyticsActivity(range, dateList, siteInfo.getName(), request.getUsage()));
    ...
    public List<PSTrafficDetails> getTrafficDetails(PSTrafficDetailsRequest request)
        ...
        PSDateRange range;
        try {
            range = createPSDateRange(request.getStartDate(), request.getEndDate(), PSDateRange.Granularity.DAY.toString());
        } catch (ParseException e) {
            throw new PSTrafficServiceException(e.getMessage());
        }

        var previousRange = new PSDateRange(range.getStart(), PSDateRange.Granularity.DAY, range.getDaysInRange());
        var pageIds = activityService.findPageIdsByPath(request.getPath());
        var activityIds = activityService.findPageIdsContentActivities(pageIds, range.getStart(), range.getEnd(), WF_STATE_LIVE, null);
        ...
        try {
            ...
            previousAnalytics = analyticsService.getPageViewsByPathPrefix(siteInfo.getName(), null, previousRange);
        } catch (PSAnalyticsProviderException e) {
            throw new PSTrafficServiceException(e);
        }

        for (int j = 0; j < itemPropList.size(); j++) {
            int currentViews = findPageAnalyticsCount(currentAnalytics, itemPropList.get(j), siteInfo.getName(), request.getUsage());
            int previousViews = findPageAnalyticsCount(previousAnalytics, itemPropList.get(j), siteInfo.getName(), request.getUsage());
     ...
     */
    private PSDateRange createPSDateRange(String start, String end, String granularity) throws ParseException {
     ...
     */
    private int findPageAnalyticsCount(List<IPSAnalyticsQueryResult> visitResults, PSItemProperties itemProp, String siteName, String usage) {
        int visitCount = 0;
        var itemPath = itemProp.getPath().toLowerCase().replaceFirst("/sites/" + siteName.toLowerCase(), "");
     ...
     * Helper method to get analytics by list of dates.
     */
    public List<Integer> createAnalyticsActivity(PSDateRange range, List<Date> dates, String siteName, String usage)
        ...
        for (var visit : visitResults) {
            ...
            for (int i = 0; i < dates.size() - 1; i++) {
                if (visitDate.equals(dates.get(i)) || (visitDate.after(dates.get(i)) && visitDate.before(dates.get(i + 1)))) {
