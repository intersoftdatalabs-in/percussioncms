...
public class PSActivityService implements IPSActivityService {
  ...
  public PSContentActivity createActivity(PSActivityNode node, Date beginDate, long timeout)
    ...
    int newItems =
        ids.isEmpty() ? 0 : sysSrv.findNewContentActivities(ids, beginDate, endDate, WF_STATE_LIVE);
    checkTimeout(sw.getTime(), timeout);

    int updatedItems =
        ids.isEmpty()
            ? 0
            : sysSrv.findNumberContentActivities(ids, beginDate, endDate, WF_STATE_LIVE, null);
  ...
  private void checkTimeout(long time, long timeout) throws PSActivityServiceException {
    if (time > timeout)
      throw new PSActivityServiceException(
          "The requested data is taking too long to retrieve, sorry!");
  }
   ...
   */
  public List<Integer> findNumberContentActivities(
    ...
    for (int i = 1; i < dates.size(); i++) {
      ...
      int count =
          sysSrv.findNumberContentActivities(
              contentIds, beginDate, endDate, stateName, transitionName);
      counts.add(count);
    }
    return counts;
  }
   ...
   * @see com.percussion.activity.service.IPSActivityService#findNumberContentActivities(java.util.List, java.util.List, java.lang.String, java.lang.String)
   */
  public List<String> findPageIdsContentActivities(
    ...
    pageIds =
        sysSrv.findPageIdsContentActivities(
            contentIds, beginDate, endDate, stateName, transitionName);
  ...
  public List<Integer> findPublishedItems(Collection<Integer> contentIds, List<Date> dates) {
    ...
    for (int i = 1; i < dates.size(); i++) {
      ...
      Date endDate = dates.get(i);
      int count =
          sysSrv.findPublishedItems(
              contentIds, beginDate, endDate, WF_STATE_LIVE, WF_STATE_ARCHIVE);
      counts.add(count);
    }
    return counts;
  }
  ...
  private long getPendingPageCount(String path) throws PSPathServiceException {
    ...
    return getItemCount(
        path,
        Collections.singletonList(PAGE_CONTENT_TYPE),
        workflowService.getDefaultWorkflowName(),
        WF_STATE_PENDING);
  }
  ...
  private long getItemCount(
    ...
    // build the jcr query
    String jcrQuery =
        "select rx:sys_title from "
            + jcrCtypes
            + " where jcr:path like '"
            + path
            + "/%' and rx:sys_workflowid = "
            + workflowId;
  ...
  public List<PSActivityNode> createActivityNodesByPaths(String path, boolean includeSite) {
    ...
    for (IPSSite site : sites) {
      if (contentWs.getIdByPath(PSPathUtils.getFolderPath(site.getFolderRoot()) + "/.system")
          != null) {
        cm1Sites.add(site);
      }
    }
    ...
    if (!(PSPathUtils.SITES_FINDER_ROOT + "/").equals(path)) {
      for (IPSSite site : cm1Sites) {
        ...
        } else if ((path + "/").startsWith(folderRoot + "/")
            || ("/" + path + "/").startsWith(folderRoot + "/")) {
    ...
    for (IPSSite site : cm1Sites) {
      ...
      PSActivityNode node =
          new PSActivityNode(siteName, siteName, site.getFolderRoot(), PAGE_CONTENT_TYPE);
      result.add(node);
    }
    return result;
  }
  ...
  private List<PSActivityNode> createActivityChildNodes(String path, String siteName) {
    ...
    for (PSItemSummary sum : sums) {
      if (sum.getObjectType().equals(PSItemSummary.ObjectTypeEnum.FOLDER)
          && !sum.getName().equals(".system")) {
        String[] paths = contentWs.findItemPaths(sum.getGUID());
        PSActivityNode node =
            new PSActivityNode(siteName, sum.getName(), paths[0], PAGE_CONTENT_TYPE);
        result.add(node);
      }
    }
    return result;
   ...
   *
   * @param resAssets collection of resource asset type names. Never <code>null</code>, may be
   *     empty.
  ...
  public PSDateRange createDateRange(String start, String end, String granularity) {
    ...
    PSDateRange range =
        new PSDateRange(startDate, endDate, PSDateRange.Granularity.valueOf(granularity));
