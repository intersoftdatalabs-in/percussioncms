...
public class PSActivityService implements IPSActivityService {
  ...
  private long getItemCount(
    ...
    var jcrCtypes = contentTypes.stream().map(ct -> "rx:" + ct).collect(Collectors.joining(", "));
    var jcrQuery =
  ...
  public List<PSActivityNode> createActivityNodesByPaths(String path, boolean includeSite) {
    ...
    var cm1Sites =
        siteMgr.findAllSites().stream()
            .filter(
                site ->
                    contentWs.getIdByPath(
                            PSPathUtils.getFolderPath(site.getFolderRoot()) + "/.system")
                        != null)
            .collect(Collectors.toList());
    ...
    return cm1Sites.stream()
        .map(
            site ->
                new PSActivityNode(
                    site.getName(), site.getName(), site.getFolderRoot(), PAGE_CONTENT_TYPE))
        .collect(Collectors.toList());
  }
