...
/**
 * It has the responsibility to retrieve needed data for {@link PSPathItem} objects to properly
 * display it on the List View. For instance, it fills display properties for {@link PSPathItem}
 * objects.
 *
 * <p>Each {@link IPSPathService} should have an {@link IPSListViewHelper} implementation
 * associated.
 *
...
public interface IPSListViewHelper {
   ...
   *
   * @param criteria It has the necessary information to fill the {@link PSPathItem} objects with
   *     the display properties. It cannot be <code>null</code>, nor it's {@link PSPathItem} object
   *     list field. If the {@link PSSimpleDisplayFormat} format is <code>null</code>, then no
   *     action is performed.
   */
