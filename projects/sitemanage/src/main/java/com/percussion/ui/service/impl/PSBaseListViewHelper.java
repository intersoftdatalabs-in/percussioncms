...
/**
 * Base class for all {@link IPSListViewHelper} implementations. It validates the {@link
...
public abstract class PSBaseListViewHelper implements IPSListViewHelper {
   ...
   * @param displayProperties The new display properties map to set to the {@link PSPathItem}
   *     object.
   */
   ...
   * null and it's type is the expected. <code>null</code> otherwise.
   *
   * @param pathItem
   * @return
   */
   ...
   * it's latest modified date, whereas a "CM1 objects" implementation may fill the name, author,
   ...
   *     All supported keys resides in the {@link IPSListViewHelper} interface, for example, {@link
   ...
   *
   * @return <code>true</code> in the case null values for {@link PSPathItem#getRelatedObject()} are
