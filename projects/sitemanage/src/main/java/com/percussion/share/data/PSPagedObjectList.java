...
public class PSPagedObjectList<T> {
   ...
   *     <ol>
   *       <li>The sublist of objects (page). The real start index. This value makes sense for the
   *           consumer if a startIndex greater than the allItems.size() value is specified.
   *     </ol>
   */
  public static <T> PSPagedObjectList<T> getPage(
    ...
    if (startIndex > allItems.size()) {
      ...
      if (processedMaxResults == null) processedMaxResults = Integer.MAX_VALUE;

      newStartIndex = startIndex - processedMaxResults;
      while (newStartIndex > allItems.size()) newStartIndex = newStartIndex - processedMaxResults;
    }
