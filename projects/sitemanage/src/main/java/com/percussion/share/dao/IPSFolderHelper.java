...
/**
 * A wrapper around {@link IPSContentWs} to add items to folders and removing items from folders.
 *
 * <p>The methods mainly throw checked exceptions due to the underlying architecture. So conversion
 * to other exceptions will have to be done in another layer. This is not ideal but for now works.
 *
 * <p>Behavior of this class is also not transactional so do not expect rollbacks. This limitation
 * will have to be looked at a later date.
 *
 * @author adamgent
 */
public interface IPSFolderHelper {
  ...
  /**
   * The path separator used in CM system paths. This will almost always be '<code>/</code>'.
   *
  ...
  /**
   * Adds an item to a particular folder path, creating missing folders if they do not exist in the
   * path. Thus a path of:
   *
   * <pre>
   *  //a/b/c
   ...
   * @param purgeItem <code>true</code> to purge the item from the system, <code>false</code> to
   *     only remove the item from the folder.
  ...
  /**
   * Finds all the paths associated with an item. If the item is a folder a singleton list will be
   * returned.
   *
  ...
  /**
   * Finds all child item ids within the specified folder folder, recursing into child folders. A
   * very lightweight and fast method that uses only cached data.
   *
   * @param path must be a valid path to a <strong>folder</strong>, never <code>null</code>.
   * @return The list of item ids, excluding folders, never <code>null</code>, may be empty if none
  ...
  /**
   * Finds an item by ID. The usage of this method is to find all properties of the item, except its
   * path. So the path related properties may not be set in the returned object.
   *
   * @param id the ID of the item, not blank.
   * @return the item, never <code>null</code>.
   */
  public PSPathItem findItemById(String id)
      throws IPSDataService.DataServiceLoadException, PSValidationException, PSNotFoundException;
  ...
  /**
   * Sets the access level to the specified item. The ACL is calculated according to current user.
   * The ACL is the item itself for a "folder" or "site" item; otherwise it is the ACL of its parent
   * folder.
   *
   * @param item the item in question, assumed not <code>null</code>.
   * @return the item which contains the access level for the current user, never <code>null</code>.
   */
  public PSPathItem setFolderAccessLevel(PSPathItem item)
      throws PSValidationException, PSNotFoundException;

  /**
   * Sets the access level for current user to the specified items (siblings). The ACL is calculated
   * the same way as described in {@link #setFolderAccessLevel(PSPathItem)}.
   *
   * @param items the items in question, assumed not <code>null</code>. Assumed all items are
   *     siblings or under the same parent folder.
   ...
   *
   * @param path never <code>null</code> or empty.
   * @param shouldRecycle <code>true</code> if the item is being recycled. flag used to indicated
   *     whether the {@link
   *     com.percussion.design.objectstore.PSRelationshipConfig#TYPE_RECYCLED_CONTENT} type is used
   *     or not.
   * @return never <code>null</code>.
   */
  ...
  /**
   * Provides a unique folder name for the given folder path and given base name. The name is also
   * valid for a folder, meaning any characters specified in {@link
   * SecureStringUtils#INVALID_ITEM_NAME_CHARACTERS} will be removed. If any folder/item exists with
   * the same base name under the supplied parent folder, returns supplied baseName-n with n
   * starting at 2. The first name 'baseName-n' will be used with the first available n value >= 2.
   *
   * @param parentPath the full internal folder path, must not be blank, throws exception if it is
  ...
  /**
   * Provides a unique name within a given folder path, given base name, and suffix. If any item
   * exists with the same base name and suffix under the supplied parent folder, returns supplied
   * baseName-suffix-n with n starting at 2. The first name 'baseName-suffix-n' will be used with
   * the first available n value >= 2.
   *
   * @param parentPath the full internal folder path, must not be blank, throws exception if it is
   *     not a valid folder path.
   ...
   */
  public String getUniqueNameInFolder(
      String parentPath, String baseName, String suffix, int startingIndex, boolean skipFirstIndex)
      throws IPSPathService.PSPathNotFoundServiceException;

  /**
   * The target of a path. Could be any of {@link PathTargetType}.
   *
   * @author adamgent
   */
  public static class PathTarget {
    ...
    public IPSItemSummary getFolder() throws Exception {
      ...
      if (!item.isFolder()) throw new RuntimeException("Item is not a folder.");
      return item;
    }
    ...
    public boolean isToFolderOrNothing() {
      return targetType == PathTargetType.NOTHING || targetType == PathTargetType.FOLDER;
    }
    ...
    /**
     * Will only be set if {@link #isInvalid()} is <code>true</code> but even then it may not be
     * set.
     *
  ...
  /**
   * Creates a particular folder path, creating missing folders if they do not exist in the path.
   * Thus a path of:
   *
   * <pre>
   *  //a/b/c
   * </pre>
   *
   * Would create folders //a, //a/b, and //a/b/c if they were not created yet. An error will
   * <strong>NOT</strong> occur if the folder path already exists or parts of the folder path exist
   * so long as each component of the path is a folder.
   *
   * <p>The permission of the created folder will have {@link PSFolderPermission.Access#ADMIN} ACL.
   *
   * @param path never <code>null</code> or empty.
  ...
  /**
   * In addition of creating a folder, just like {@link #createFolder(String)}, the created folder
   * also has the specified folder permission.
   *
  ...
  /**
   * Deletes a folder identified by the specified path. This is a recursive delete, meaning all
   * sub-folders and items will also be deleted. Items are not purged from the system.
   *
   * @param path a valid folder path, never <code>null</code> or empty.
   ...
   *
   * @param path a valid folder path, never <code>null</code> or empty.
   * @param recycleFolder <code>true</code> to recycle the folder, <code>false</code> to purge the
  ...
  /**
   * Determines if the current user has "admin" access for the specified folder and all its
   * descendant folders.
   *
   * @param folderId the ID of the folder in question, not blank.
   * @return <code>true</code> if current user does have "admin" access to the folder and its
   *     descendant folders; otherwise return <code>false</code>.
   */
   ...
   * @return <code>true</code> if current user does have the specified access level to the specified
   *     folder; otherwise return <code>false</code>.
   */
   ...
   * @throws Exception If an item cannot be found for the given path.
   */
  public PSItemProperties findItemProperties(String path, String relationshipTypeName)
      throws Exception;
  ...
  public PSFolderProperties findFolderProperties(String id)
      throws PSErrorException, PSValidationException;

  /**
   * Saves the specified folder properties.
   *
   * <p>There is a special consideration for the Workflow ID property: this property is updated only
   * if its value is greater than zero. If it's equals to {@link Integer#MIN_VALUE} then the
   * workflowId property is removed. If it's anything else it's silently ignored.
   *
  ...
  /**
   * When trying to find a folder under Assets, the folders system path is required, but this is not
   * necessary when trying to find a folder under Sites. It adds the system path if necessary, an
   * also adds the slash before the path, if necessary.
   *
   ...
   *
   * <p>Note, if the item is a page, then both folders must be under a site; otherwise the item is
   * an asset, then both folders must be under the generic "Assets" folder.
   *
   * @param targetFolderPath the path of the target folder, not blank.
   * @param itemPath the full path of the moved item, not blank.
   * @param isFolder determines if the moved item is a folder or non-folder item. <code>true</code>
  ...
  /**
   * Find and return all the sites the item exists in. The sites are ordered by the name of the
   * sites it exists in. This is a thin wrapper, calls {@link IPSSiteManager#getItemSites(IPSGuid)}
   * to get the sites.
   *
   * @param contentId the string representation of the content GUID, never <code>null</code>
   * @return the sites the item exists in never <code>null</code> may be empty, if the item does not
   *     exist in any site.
   */
  ...
  /**
   * Returns a valid workflow id for the given {@link PSFolderProperties} object. It tries to get
   * the ID of the workflow currently assigned to the folder. If it's not a valid workflow, then the
   * default one is returned.
   *
   * @param folder Never <code>null</code>.
   * @return The workflow id associated to the folder, or the default workflow id if the former is
   *     invalid.
   */
  ...
  /**
   * Gets the root level folder (immediate child for assets root) for a given shared asset. Eg:
   * /Assets/RootLevelFolder/anotherFolder/anotherInnerFolder/RichTextAsset. Given the RichTextAsset
   * id, the method will return the PSFolder object of "RootLevelFolder"
   *
  ...
  /**
   * Gets the allowed sites properties of the root level folder for a given shared asset. The
   * allowed sites property represent the list of sites that the asset can be published to
   *
   * @see com.percussion.share.dao.IPSFolderHelper#getRootFolderForAsset(String)
   * @author federicoromanelli
   * @param assetId - the id of the asset. Never <code>null</code>.
   * @return the value of the allowed sites property. <code> Null </code> if the property is not set
   *     for the specific folder.
   */
  public String getRootLevelFolderAllowedSitesPropertyValue(String assetId);

  /**
   * Checks to see if the target path exists. Checks the ID of the target and source and verifies if
   * they are the same. If they are, it is the same folder. If they aren't, it's obviously a
   * different folder. Developed for checking if folders/items being recycled have a destination
   * path under the recycle bin that matches the recycled item's folder id. Always checks the target
   * path as the {@link
   * com.percussion.design.objectstore.PSRelationshipConfig#TYPE_RECYCLED_CONTENT} with the
   * originiation type as {@link
   * com.percussion.design.objectstore.PSRelationshipConfig#TYPE_FOLDER_CONTENT}.
   *
   ...
   */
  boolean isFolderValidForRecycleOrRestore(
   ...
   */
  PSPair<String, String> fixupLastModified(
      IPSGuid id, String userName, Date lastModified, boolean isPublishable);
}
