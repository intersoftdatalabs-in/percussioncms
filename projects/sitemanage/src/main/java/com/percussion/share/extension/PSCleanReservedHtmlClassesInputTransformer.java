...
/**
 * Removes reserved html class names from content in the specified html parameter value. It also
 * adds a place holder text for iframe elements if they are empty.
 *
 * @author JaySeletz
 */
public class PSCleanReservedHtmlClassesInputTransformer extends PSDefaultExtension
  ...
  private static final String[] PERC_CLASSES =
      new String[] {
        ...
        "perc-region-leaf",
        "perc-horizontal",
        "perc-itool-selectable-elem",
        "perc-itool-region-elem",
        "perc-zero-size-elem"
      };
   ...
   * cleans reserved class names from all elements and returns the modified content. Adds place
   * holder text for iframe elements if they are empty.
   *
  ...
  String processContent(String value) {
    ...
    for (Element elem : elems) {
      ...
      if (elem.tagName().equalsIgnoreCase("iframe")) {
        // jsoup may normalize iframe contents away; rely on inner HTML to decide emptiness
        if (StringUtils.isBlank(elem.html())) {
