...
public class PSFolderPermissionUtils {
  ...
  public static PSFolderPermission getFolderPermission(PSFolder folder) {
    ...
    if (aclEntry != null) {
      if (aclEntry.hasAdminAccess()) permission.setAccessLevel(PSFolderPermission.Access.ADMIN);
      else if (aclEntry.hasWriteAccess())
        permission.setAccessLevel(PSFolderPermission.Access.WRITE);
      else if (aclEntry.hasReadAccess()) permission.setAccessLevel(PSFolderPermission.Access.READ);
    }
   ...
   */
  public static PSPair<PSFolderPermission.Access, Boolean> getUserAcl(
    ...
    if (adminEntry == null) {
      log.debug(
          "Folder (id="
              ...
              + ", name="
              + folder.getName()
              + ") does not have ACL entry with Admin role.");
    } else {
      notNull(adminEntry);
  ...
  public static void setFolderPermission(PSFolder folder, PSFolderPermission perm) {
    ...
    // Set the access level for individual users, if specified.  The order in which this is done is
    // important, as it
  ...
  private static void setFolderAccessLevel(PSFolder folder, PSFolderPermission.Access acl) {
    ...
    if (permValue == PSFolderPermission.Access.READ.ordinal()) legacyAcl = READ_ACCESS;
    else if (permValue == PSFolderPermission.Access.WRITE.ordinal()) legacyAcl = WRITE_ACCESS;
  ...
  private static PSFolderPermission.Access convertAcl(PSObjectAclEntry entry) {
    if (entry.hasAdminAccess()) return PSFolderPermission.Access.ADMIN;
    else if (entry.hasWriteAccess()) return PSFolderPermission.Access.WRITE;
    else if (entry.hasReadAccess()) return PSFolderPermission.Access.READ;
    else return PSFolderPermission.Access.ADMIN;
  }
  ...
  private static void removeNonVirtualAclEntries(PSObjectAcl acl) {
    ...
    if (!entries.isEmpty()) {
      for (PSObjectAclEntry entry : entries) acl.remove(entry);
    }
  }
}
