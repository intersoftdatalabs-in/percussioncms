...
public class PSFolderPermissionUtils {
    ...
    public static PSFolderPermission getFolderPermission(PSFolder folder) {
        ...
        if (aclEntry != null) {
            if (aclEntry.hasAdminAccess())
                ...
                permission.setAccessLevel(Access.WRITE);
            else if (aclEntry.hasReadAccess())
                permission.setAccessLevel(Access.READ);
        }
     ...
     */
    public static PSPair<Access, Boolean> getUserAcl(PSFolder folder, String userName, Collection<String> roles) {
        notNull(folder);
        notNull(roles);
    ...
    public static boolean ensureAdminAccess(PSFolder folder) {
        ...
        var adminEntry = getAclEntry(folder.getAcl(), PSObjectAclEntry.ACL_ENTRY_TYPE_ROLE, ADMINISTRATOR_ROLE);
        if (adminEntry == null) {
            log.debug("Folder (id={}), name={}) does not have ACL entry with Admin role.",
                    folder.getLocator().getId(), folder.getName());
        } else {
            notNull(adminEntry);
    ...
    private static void setFolderAccessLevel(PSFolder folder, PSFolderPermission.Access acl) {
        ...
        if (permValue == Access.READ.ordinal())
            legacyAcl = READ_ACCESS;
        else if (permValue == Access.WRITE.ordinal())
            legacyAcl = WRITE_ACCESS;

        setAclEntry(folder.getAcl(), PSObjectAclEntry.ACL_ENTRY_TYPE_VIRTUAL, PSObjectAclEntry.ACL_ENTRY_EVERYONE, legacyAcl);
    }
