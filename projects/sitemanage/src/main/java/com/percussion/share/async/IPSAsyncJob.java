...
public interface IPSAsyncJob extends Runnable {
   ...
   *     successfully, <code>false</code> if it has not yet completed. Jobs must return <code>true
   *     </code> before the {@link #run()} method exits to ensure proper freeing of resources
   ...
   * #run()} method to determine if the this job should cancel.
   *
  ...
  /**
   * Determine if the job, having completed, can be discarded. Each job must determine it's own
   * lifecycle once it has completed. Completed job instances will be retained in memory at least
   * until this method returns <code>true</code>.
   *
   * @return <code>true</code> if the job can be discarded, <code>false</code> if it should continue
   ...
   * started, at least until {@link #isDiscarded()} returns <code>true</code>.
   *
   * @return The result, may be <code>null</code> if one is not ready, or if the job does not
   *     produce a result.
   */
