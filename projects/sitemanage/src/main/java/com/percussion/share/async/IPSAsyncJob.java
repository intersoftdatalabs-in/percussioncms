...
public interface IPSAsyncJob extends Runnable {
     ...
     *
     * @return the percent complete for this job as a number between 1-100 inclusive.
     *         Returns -1 if the job has terminated abnormally (see {@link #ABORT_STATUS}).
     */
    int getStatus();
     ...
     *         successfully; false if it has not yet completed. Jobs must return
     *         true before the {@link #run()} method exits to ensure proper freeing of
     ...
     * classes from the {@link #run()} method to determine if this job should cancel.
     *
     * @return true if {@link #cancelJob()} has been called, but
     *         does not indicate if the job was successfully cancelled - this can only be
     *         determined by a call to {@link #isCompleted()}.
     */
    boolean isCancelled();
    ...
    /**
     * Sets flag for this job to stop processing regardless of its status and
     * perform any cleanup required. Callers should then check
     * {@link #isCompleted()} to determine when the job has stopped.
     ...
     *
     * @param config Implementation specific object that provides any data needed by the job. Will not be null.
     */
    void init(Object config) throws IPSFolderService.PSWorkflowNotFoundException;

    /**
     * Determine if the job, having completed, can be discarded. Each job must determine its own lifecycle once it has
     * completed. Completed job instances will be retained in memory at least until this method returns true.
     *
     ...
     * until {@link #isDiscarded()} returns true.
     *
