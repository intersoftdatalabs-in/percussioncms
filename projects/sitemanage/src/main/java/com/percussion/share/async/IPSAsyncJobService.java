...
public interface IPSAsyncJobService {
  ...
  /**
   * Checks the status of the specified job. The result contains a value between <code>1-100</code>
   * to indicate the % done and a message. <code>100</code> indicates that the job has completed
   * successfully. If the job has terminated abnormally, <code>-1</code> is returned and the message
   * will indicate the error. Once a completed or terminated status has been returned, the job
   * status is no longer available.
   *
   * @param jobId The id of the job, must be a valid job id.
   * @return The current status, or <code>null</code> if the job does not exist or if the job is no
   *     longer available.
   */
   ...
   * @param jobId The id of the job. If not a currently running job, it is assumed the job has
   *     completed and the method simply returns.
   * @return The result, may be <code>null</code> if the job does not return a result or if the job
