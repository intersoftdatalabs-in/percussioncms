...
/**
 * This is typically used for an operation in REST layer that does not have anything to return to
 * the client (caller) when the operation is successfully completed.
 *
 * <p>When a server does not have anything in the response (for a request), then the client will get
 * an 204 (No Content) HTTP status, which may cause underlying jQuery to generate a JavaScript
 * error. So the REST operation can return this object to avoid the 204 HTTP status.
 *
 * @author yubingchen
 */
...
public class PSNoContent {
   ...
   * Sets the result.
   *
   * @param result the result of the operation response.
   */
