...
public class PSThumbnailRunner implements Runnable {
  ...
  private void goToWork() {
    ...
    while (workPackage != null) {
      ...
      try {
        if (!shutdownFlag.get()) performWork(workPackage);
      } catch (Exception e) {
  ...
  private void delete(PSWorkPackage workPackage) {
    try {
      File root =
          new File(
              (PSSiteConfigUtils.getRootDirectory() + "/" + TPL_IMAGES_DIR).replace("\\", "/"));
      Iterator<File> iterator = FileUtils.listFiles(root, null, true).iterator();
      ...
      log.debug(
          "Failed to delete thumbnail for Page ID: "
              + workPackage.getId()
  ...
  private void checkForPageThumbnail(PSWorkPackage workPackage) {
    String imgPath =
        (PSSiteConfigUtils.getRootDirectory()
                + "/"
                + TPL_IMAGES_DIR
                + '/'
                + workPackage.getSite().getName()
                + '/'
                + workPackage.getId()
                + PAGE_STRING)
            .replace("\\", "/")
  ...
  // ===================================================================================================
  private void handleThumbnailGeneration(
      String id, Function function, String fileSuffix, String siteFolder, PSPage page)
    ...
    if (page != null) {
      ...
      URL url =
          PSUrlUtils.createUrl(
              "127.0.0.1", null, path, sessionParameterMap.iterator(), null, requestContext, true);
      try {
  ...
  private void buildThumbnailsForTemplatesPages(PSWorkPackage workPackage)
    ...
    if ((!workPackage.getTemplate().getName().equals("Unassigned"))
      ...
      PSPagedItemList pages =
          pageService.findPagesByTemplate(
              workPackage.getTemplate().getId(), 1, 10000, null, null, null);
      List<PSPathItem> pagePaths = pages.getChildrenInPage();

      // add additional items to the in process count
      int numPages = pagePaths.size();
      if (numPages > 0) PSThumbnailProcessMonitor.incrementCount(numPages);
      ...
      for (PSPathItem pagePath : pagePaths) {
        try {
          ...
          log.debug(
              "Unable to generate thumbnail for a templates's child page with id: "
                  + pagePath.getId()
                  + " "
                  + e.getLocalizedMessage(),
              e);
          String imagePath = workPackage.getSiteFolderPath() + pagePath.getId() + PAGE_STRING;
          ...
          if (numPages > 0) PSThumbnailProcessMonitor.decrementCount(numPages);
        }
      }
    }
  }
  ...
  private void init() {
    try {
      ...
      log.debug(
          "Thumbnail Service Runner initalization failure for ID: successful thumbnail generation unlikely");
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    }
  }
  ...
  private String getSiteFolder(PSWorkPackage work)
    ...
    if (work.getSite() == null) work.setSite(getSite(work.getId(), work.getFunction()));
    String siteFolder =
        PSSiteConfigUtils.getRootDirectory()
  ...
  private boolean isTemplateFunction(Function function) {
    ...
    if (function == Function.GENERATE_TEMPLATE_THUMBNAIL
        ...
        || function == Function.DELETE_TEMPLATE_THUMBNAIL) isTemplateFunc = true;
  ...
  private boolean isPageFunction(Function function) {
    ...
    if (function == Function.GENERATE_PAGE_THUMBNAIL
        ...
        || function == Function.DELETE_PAGE_THUMBNAIL) isPageFunc = true;
    return isPageFunc;
  }
  ...
  private PSTemplateSummary getTemplate(PSWorkPackage workPackage) throws PSDataServiceException {
    ...
    if (workPackage.getTemplate() == null) {
      ...
      if (isTemplateFunction(workPackage.getFunction())) {
        ...
        template = templateService.find(workPackage.getPage().getTemplateId());
      }
    } else {
  ...
  private PSPage getPage(PSWorkPackage workPackage) throws PSDataServiceException {
    ...
    if (workPackage.getPage() == null) {
      if (isPageFunction(workPackage.getFunction())) {
        ...
        PSPagedItemList pages =
            pageService.findPagesByTemplate(template.getId(), 1, 2, null, null, null);
        List<PSPathItem> pagePaths = pages.getChildrenInPage();
