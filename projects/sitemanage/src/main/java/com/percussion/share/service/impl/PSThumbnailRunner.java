...
/**
 * Handles thumbnail generation for templates and pages. Sunny Sal says: "Thumbnails so sharp, even
...
public class PSThumbnailRunner implements Runnable {
  ...
  private void generateThumbnail(PSWorkPackage workPackage) {
    ...
    try {
      ...
      log.warn(
          "Failed to generate thumbnail for id: {} {} failure occurred in generateThumbnail",
          workPackage.getId(),
          e.getLocalizedMessage(),
          e);
    }
    ...
    if (!"Unassigned".equals(workPackage.getTemplate().getName())
      ...
      for (var pagePath : pagePaths) {
        try {
          ...
          log.debug(
              "Unable to generate thumbnail for a template's child page with id: {} {}",
              pagePath.getId(),
              e.getLocalizedMessage(),
              e);
  ...
  private void init() {
    try {
      ...
      log.debug(
          "Thumbnail Service Runner initialization failure for ID: successful thumbnail generation"
              + " unlikely");
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    }
  }
  ...
  private PSTemplateSummary getTemplate(PSWorkPackage workPackage) throws PSDataServiceException {
    ...
    if (isTemplateFunction(workPackage.getFunction())) {
      ...
      if (workPackage.getPage() == null) workPackage.setPage(pageService.find(workPackage.getId()));
