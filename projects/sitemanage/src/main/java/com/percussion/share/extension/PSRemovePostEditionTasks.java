 ...
 * These tasks aren't necessary for on demand jobs and have been causing these jobs to take quite a
 * long time.
 *
 * @author chriswright
 */
public class PSRemovePostEditionTasks implements IPSStartupProcess {
  ...
  /**
   * Creates an empty file in rxconfig/Server to denote that this process has been run. File can be
   * deleted to re-run this process.
   */
  public void markAsCompleted() {
    var workCompleted = new File(WORK_COMPLETED_FILE);
    if (!workCompleted.exists() || !workCompleted.isFile()) {
      try (var fw = new FileWriter(workCompleted);
