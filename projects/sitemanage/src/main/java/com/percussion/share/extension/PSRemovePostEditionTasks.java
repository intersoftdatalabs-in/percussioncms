...
/**
 * Startup process to remove the following post-edition tasks from publish on demand and unpublish
 * on demand jobs:
 * <li>perc_PushFeedDescriptorTask
 * <li>sys_flushPublicationCache <br>
 *     These tasks aren't necessary for on demand jobs and have been causing these jobs to take
...
public class PSRemovePostEditionTasks implements IPSStartupProcess {
  ...
  private static final Logger log = LogManager.getLogger(PSRemovePostEditionTasks.class.getName());
  ...
  private static final String WORK_COMPLETED_FILE =
      WORK_COMPLETED_FILE_LOCATION + "PSRemovePostEditionTasks.txt";

  private static final String MESSAGE =
      "Delete this file to run the PSRemovePostEditionTasks job again."
  ...
  public void doStartupWork(Properties startupProps) throws Exception {
    ...
    if (hasWorkBeenCompleted()) {
      log.info(getPropName() + " has already been completed.  Nothing to run.");
      return;
    }
    ...
    try {
          ...
          if (edition.getDisplayTitle().contains("PUBLISH_NOW")) {
            ...
            for (IPSEditionTaskDef task : tasks) {
              if ("Java/global/percussion/task/perc_PushFeedDescriptorTask"
                ...
                log.info(
                    "Deleting task Java/global/percussion/task/perc_PushFeedDescriptorTask from edition: "
                        + edition.getDisplayTitle());
                psvc.deleteEditionTask(task);
              } else if ("Java/global/percussion/task/sys_flushPublicationCache"
                  .equals(task.getExtensionName())) {
                log.info(
                    "Deleting task Java/global/percussion/task/sys_flushPublicationCache from edition: "
                        + edition.getDisplayTitle());
                psvc.deleteEditionTask(task);
              }
            }
      ...
      log.error("Error removing post-edition tasks within " + getPropName(), e);
    }

    markAsCompleted();
    log.info(getPropName() + " has completed.  File location: " + WORK_COMPLETED_FILE);
  }
  ...
  /**
   * Creates an empty file in rxconfig/Server to denote that this process has been run and does not
   * need to run again. File can be deleted to re-run this process.
   */
  ...
  public boolean hasWorkBeenCompleted() {
    ...
    try {
      workCompleted = new File(WORK_COMPLETED_FILE);
      if (workCompleted.isFile()) return true;
    } catch (Exception e) {
