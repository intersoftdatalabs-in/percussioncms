 ...
 *
 * <p>The files are read-only and are not modified.
 *
 * <p>Once the class is instantiated the expected use is:
 *
...
public abstract class PSFileDataRepository<T> {
  ...
  /**
   * Called to initialize the directory that represents the file repository by polling the files for
   * the first time.
   */
   ...
   *
   * <p>Poll should be called from quartz or some other scheduler.
   *
   * @throws IOException
   */
  public synchronized void poll()
    ...
    for (File file : files) {
      PSFileDataRepository.PSFileEntry fileEntry =
    ...
    if (data.get() != null) oldEntries = data.get().files;
    else oldEntries = new HashSet<>();
   ...
   *
   * <p>This method is safe to override if it is {@link #getRepositoryDirectory()} and {@link
   * #getFileExt()} may not be applicable.
   *
   * @return never <code>null</code>.
   * @throws IOException
   */
   ...
   *
   * <p>This method is safe to override. <em>For thread safety the returned object should be a newly
   * created object and not mutation of the current {@link #getData()}.</em>
   *
