...
/**
 * Abstract base class for asynchronous jobs. Handles request context, status, listeners, and
 * cancellation.
 *
 * @author JaySeletz
 */
public abstract class PSAsyncJob implements IPSAsyncJob {
   ...
   * requests can pass through without any login authentication.
   */
  ...
  /**
   * Required implementation for jobs running within the server that need to access the current
   * request info. Derived classes should implement {@link #doInit(Object)}.
   */
  @Override
  public void init(Object config) throws IPSFolderService.PSWorkflowNotFoundException {
    ...
    var request = (PSRequest) m_requestInfoMap.get(PSRequestInfo.KEY_PSREQUEST);
  ...
  /**
   * Sets the current status of this job as a percentage.
   *
   * @param status A number between 1-100 inclusive to indicate the percentage of completeness for
   *     this job, or {@link #ABORT_STATUS} if there has been a fatal error.
  ...
  protected void setCompleted() {
    ...
    // Using CopyOnWriteArrayList to prevent concurrent modification
  ...
  /**
   * Sets the current status message for this job.
   *
   * @param msg The status message, may not be null or empty.
   * @throws IllegalArgumentException if msg is invalid.
   */
  ...
  /**
   * Override the Thread.run() from Runnable. Derived classes should not override this method, and
   * instead implement {@link #doRun()}.
   *
  ...
  /**
   * Default implementation, returns true if {@link #getResult()} has been called at least once
   * after calling {@link #setCompleted()}.
   */
  @Override
  ...
  /**
   * Allows the current running job's thread to be gracefully interrupted. Caller must still manage
   * job state and status properly.
   */
  ...
  /** Tracks the percentage complete for this job as a number between 1-100 inclusive. */
  ...
  /** Indicates if this job is currently active. */
  private final AtomicBoolean m_isCompleted = new AtomicBoolean(false);

  /** Indicates if an attempt to cancel this job has been made. */
  private final AtomicBoolean m_cancelled = new AtomicBoolean(false);

  /** Indicates if the job can be discarded. */
  private final AtomicBoolean m_isDiscarded = new AtomicBoolean(false);

  /** The result to be returned. */
  private Object m_result = null;

  /** The id used to identify this job. Must be set by derived classes during the init() method. */
  protected long m_id = -1;

  /** List of job listeners, never null, may be empty. */
  private final CopyOnWriteArrayList<IPSAsyncJobListener> m_listeners =
