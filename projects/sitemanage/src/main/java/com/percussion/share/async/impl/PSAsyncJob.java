...
public abstract class PSAsyncJob implements IPSAsyncJob {
   ...
   *     completeness for this job, or {@link #ABORT_STATUS} if there has been a fatal error.
  ...
  /**
   * Default implementation, returns <code>true</code> if {@link #getResult()} has been called at
   * least once after calling {@link #setCompleted()}.
   */
  @Override
  ...
  /**
   * Provides a message indicating the current status of this job. Never <code>null</code>, empty
   * until first call to {@link #setStatusMessage(String)}, never <code>null</code> or empty after
   * that.
   */
