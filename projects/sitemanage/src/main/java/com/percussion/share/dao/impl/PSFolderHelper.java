...
public class PSFolderHelper implements IPSFolderHelper {
  ...
  private static final String FOLDER_WORKFLOW_ID_PROPERTY_DESCRIPTION =
      "The workflow ID assigned to the folder";
  /** Description of the allowed sites property for folders */
  private static final String ALLOWED_SITES_PROPERTY_DESCRIPTION =
      "The workflow ID assigned to the folder";
  ...
  private List<String> reservedPathNames =
      Arrays.asList("web_resources", "rx_resources", "web-inf", "meta-inf");
  ...
  /**
   * Resets the root assets folder's ACLs to Read/Write for Everyone on server startup.
   *
   * @author adamgent
   */
  protected class PSCreateBaseFoldersNotificationListener implements IPSNotificationListener {
    ...
    private String errorMessage =
        "The server could not reset the security on the base asset folders.";
  ...
  public PSItemProperties findItemPropertiesById(String id, String relationshipTypeName)
    ...
    String publishDate =
        PSDateUtils.getDateToString((pubStatus == null) ? null : pubStatus.getDate());

    // get last modifier and date
    String userName = compSum.getContentLastModifier();
    Date date = compSum.getContentLastModifiedDate();
    PSPair<String, String> modifiedInfo =
        fixupLastModified(itemId, userName, date, state.isPublishable());
    ...
    String schedPubDateStr =
        PSDateUtils.getDateToString((schedPubDate == null) ? null : schedPubDate);
  ...
  public void saveFolderProperties(PSFolderProperties folderProps) throws PSValidationException {
    ...
    if (folderProps.getAllowedSites() != null) {
      if (folderProps.getAllowedSites().length() > 0) {
        folder.setProperty(
            IPSHtmlParameters.SYS_ALLOWEDSITES,
  ...
  private void validateSaveFolderProperties(PSFolderProperties folderProps)
    ...
    if (parentPaths != null && parentPaths.length == 0) {
      String msg =
          PSI18nUtils.getString(
              "com.percussion.share.dao.impl.PSFolderHelper@Cannot Find Parent Folder",
              PSRoleUtilities.getUserCurrentLocale());
    ...
    if (containsInvalidChars(folderProps.getName())) {
      String msg =
          PSI18nUtils.getString(
                  "com.percussion.share.dao.impl.PSFolderHelper@Cannot Save Folder",
                  PSRoleUtilities.getUserCurrentLocale())
              + folderProps.getName()
              + PSI18nUtils.getString(
                  "com.percussion.share.dao.impl.PSFolderHelper@Folder Name Invalid Characters",
                  PSRoleUtilities.getUserCurrentLocale())
    ...
    if (contentId != -1 && contentId != folderId) {
      String msg =
          PSI18nUtils.getString(
              ...
              + folderProps.getName()
              + PSI18nUtils.getString(
                  "com.percussion.share.dao.impl.PSFolderHelper@Object Same Name Exists",
                  PSRoleUtilities.getUserCurrentLocale());
      builder.rejectField("name", msg, folderProps.getName());
      builder.throwIfInvalid();
    }
    ...
    if ((path.split("/")).length == 5 && !validateFolderReservedName(folderProps.getName())) {
      String msg =
          PSI18nUtils.getString(
              ...
              + folderProps.getName()
              + PSI18nUtils.getString(
                  "com.percussion.share.dao.impl.PSFolderHelper@Reserved Folder Name",
                  PSRoleUtilities.getUserCurrentLocale());
  ...
  public PSFolderProperties findFolderProperties(String id) throws PSValidationException {
    ...
    props.setWorkflowId(
        StringUtils.isEmpty(folderPropertyValue) ? -1 : Integer.parseInt(folderPropertyValue));

    String allowedSitesFolderPropertyValue =
        folder.getPropertyValue(IPSHtmlParameters.SYS_ALLOWEDSITES);
   ...
   *     It may be <code>null</code> if cannot find parent folder and <code>isRequired</code> is
   *     <code>false</code>.
   */
  private PSItemSummary getParentFolder(IPSGuid itemId, boolean isRequired)
    ...
    if (parents.isEmpty() && (!isRequired)) return null;

    if (parents.isEmpty()) {
      PSValidationErrorsBuilder builder = validateParameters("findParentFolder");
      builder
          .reject("invalid.item.id", "Cannot find parent folder with id = \"" + itemId + "\".")
          .throwIfInvalid();
      return null;
    }
  ...
  public PSPathItem setFolderAccessLevel(PSPathItem item)
    ...
    // to the file system.
    if (StringUtils.startsWith(item.getType(), "FS")) return item;
    ...
    if ("site".equals(item.getType())) {
      ...
      if (id == null) {
        log.warn(
            "Invalid Site record detect for site:"
                + site.getName()
                + " missing site folder.  Attempting to auto correct...");
        return item;
      }
  ...
  public List<PSPathItem> setFolderAccessLevel(List<PSPathItem> children) {
    ...
    for (PSPathItem item : children) {
      try {
        // FIXME Change this. We need to skip PSPathItem that point
        // to the file system.
        if (StringUtils.startsWith(item.getType(), "FS")) continue;
  ...
  public void moveItem(String targetFolderPath, String itemPath, boolean isFolder) {
    ...
    contentWs.moveFolderChildren(
        srcFolderId, tgtFolderId, Collections.singletonList(itemId), false);
    if (isFolder) handleRemoveAllowedSites(itemId);
  }

  /**
   * Helper to remove the allowed sites property from the folder that's being moved. Cases: - If the
   * folder is a root level folder and it's moved to another subfolder it's removed - If the folder
   * is not a root level folder and it's moved to a non root location, there was no property already
   * - If the folder is not a root level folder and it's moved to be a root location, it's cleaned
   * as well. The only validation here is if the folder cannot be found (the id is not from a
   * folder). In that case nothing's done
   *
  ...
  /**
   * Validating the specified target and folders. Both folders must be under the same common root
   * folder, for example, their can be one of the following: //Sites/<site-name>/... (for site
   * folders) //Folders/... (for Assets folders)
   *
  ...
  private void validateMovedItemAndFolder(String srcFolderPath, String tgtFolderPath) {
    ...
    if (srcIds.size() < 2)
      throw new IllegalArgumentException(
          "Source folder (path=" + srcFolderPath + ") is not under a site or assets");

    if (tgtIds.size() < 2)
      throw new IllegalArgumentException(
          "Target folder (path=" + tgtFolderPath + ") is not under a site or assets");

    if (!srcIds.get(0).equals(tgtIds.get(0))) {
      throw new IllegalArgumentException(
          "Target folder (path="
              + tgtFolderPath
              + ") and source folder (path="
              + srcFolderPath
              + ") are not under the same site or assets.");
    }
  ...
  public PSPair<String, String> fixupLastModified(
    ...
    if (isPublishable) {
      ...
      // unfortunately, workflow transition also "touches" both last-modifier & date of the
      // compoment summary,
   ...
   * @return page node if the item is a page. It is <code>null</code> if the item in question is not
   *     a page.
   */
   ...
   *
   * @param item the item in question, assumed not <code>null</code>.
   * @param pageNode the page node of the item. It is <code>null</code> if the item is not a page.
  ...
  /**
   * Gets the path for the item in recycling bin or vice versa. E.G if a path
   * //Sites/a-site/test-folder/ is passed in, the corresponding opposite path would be
   * //Folders/$System$/Recycling/Sites/a-site/test-folder.
   *
  ...
  public void addItem(String path, String id)
      throws PSErrorException, IPSDataService.DataServiceNotFoundException {
    validatePath(path);
    createFolder(path);
  ...
  public void createFolder(String path, PSFolderPermission.Access acl)
      throws PSErrorException, IPSDataService.DataServiceNotFoundException {
    validatePath(path);
    PathTarget p = pathTarget(path);
    if (p.isToNothing()) {
  ...
  /**
   * Sets the default ACL on a folder path. An exception will be thrown if the folder path is not a
  ...
  /**
   * The legacy ACL system uses a dangerous integer for user types. This an adapter for the user
  ...
  public boolean validateFolderPermissionForDelete(String folderId) {
    ...
    if (!hasFolderPermission(folderId, PSFolderPermission.Access.ADMIN)) return false;
    ...
    for (PSFolder subFolder : subFolders) {
      ...
      if ((access != PSFolderPermission.Access.ADMIN)) return false;
    }
    return true;
  }
  ...
  public void renameFolder(String path, String name)
      throws PSReservedNameServiceException, PSInvalidCharacterInFolderNameException,
    ...
    if ((path.split("/")).length == 5 && !validateFolderReservedName(name)) {
      throw new PSReservedNameServiceException(
          "Cannot rename folder because that name is a reserved folder name");
    }

    // If name starts with .(dot) or .. (double dot) then throw exception
    if (!validateFolderName(name)) {
      throw new PSReservedNameServiceException(
          "Cannot rename folder because the name '" + name + "' is not valid");
    }
  ...
  public PSPathItem findItemById(String id)
      throws DataServiceLoadException, PSValidationException, PSNotFoundException {
  ...
  public String getUniqueFolderName(String parentPath, String baseName)
    ...
    if (StringUtils.containsAny(folderBase, SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS)) {
      ...
      for (int i = 0; i < SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS.length(); i++) {
        folderBase =
            StringUtils.remove(
                folderBase, SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS.charAt(i));
      }
    }
   ...
   * @param startingIndex to append to copy, e.g., "MyFileName-copy-1", "MyFileName-copy-2",
   *     "MyFileName-copy-3", etc.
   * @param skipFirstIndex set to true to not append first index, e.g., "MyFileName-copy",
   *     "MyFileName-copy-2", "MyFileName-copy-3", etc.
   */
  ...
  public int getValidWorkflowId(PSFolderProperties folder) {
    ...
    if (folder.getWorkflowId() <= 0) return getDefaultWorkflowId();

    IPSGuid workflowId = PSGuidUtils.makeGuid(folder.getWorkflowId(), PSTypeEnum.WORKFLOW);
  ...
  public PSFolder getRootFolderForAsset(String assetId) {
    try {
      ...
      if (folderPath != null && folderPath.contains(PSAssetPathItemService.ASSET_ROOT)) {
        ...
        try {
          long rootLevelFolderId =
              findLegacyFolderIdFromPath(
                      StringUtils.join(folderPathList.toArray(), pathSeparator()))
                  .longValue();
          IPSGuid rootLevelFolderGuid =
              PSGuidUtils.makeGuid(Long.valueOf(rootLevelFolderId), PSTypeEnum.LEGACY_CONTENT);
  ...
  public boolean isFolderValidForRecycleOrRestore(
      String targetPath, String originalPath, String targetPathRel, String origPathRel) {
    IPSGuid targetGuid =
        contentWs.getIdByPath(targetPath, targetPathRel); // recycled (when recycling)
    IPSGuid originalGuid =
        contentWs.getIdByPath(originalPath, origPathRel); // folder (when recycling)
