...
/**
 * A Generic Content Item Dao that will store a serialized object in the field {@link
 * #getDataFieldName()}. The {@link #getUniqueIdFieldName()} property is the field used to uniquely
 * identify the content item.
 *
 * @author adamgent
 * @param <T> The object type to be serialized.
 */
public abstract class PSGenericItemDao<T extends PSAbstractPersistantObject>
  ...
  /**
   * Construct a default implementation of the item dao. Use the setters to override the defaults.
   *
  ...
  /**
   * Implementations can override this method if they need to but should probably override {@link
   * #findContentItemGuid(String)} instead.
   *
   ...
   */
  protected PSCoreItem findContentItem(String id) throws PSErrorException, PSErrorResultsException {
    notNull(id);
    IPSGuid userGuid = findContentItemGuid(id);
    if (userGuid == null) return null;
    List<PSCoreItem> items =
        contentWs.loadItems(Collections.singletonList(userGuid), true, false, false, true);
    if (items.isEmpty()) return null;
    return items.get(0);
  }

  /**
   * Implementations must determine the guid from the id. Implementations will most likely implement
   * this by calling {@link #findContentItemGuidByConversion(String)} or {@link
   * #findContentItemGuidWithSearch(String)}.
   *
   * @param id never <code>null</code>.
  ...
  /**
   * Performs a search for id. The search is parameterized by the properties on this class.
   *
   * @param id never <code>null</code>.
   * @return the guid if found, <code>null</code> otherwise.
   * @see #getContentType()
  ...
  public T save(T object) throws SaveException {
    ...
    try {
      ...
      if (id != null) coreItem = findContentItem(id);
      /*
       * Create the content item.
       */
      if (coreItem == null) {
        if (log.isDebugEnabled()) log.debug("Creating content item for object: {}", object);
  ...
  /**
   * When an item is created, right before its saved a unique id is needed. Sometimes the object
   * does not specify its id on {@link #save(PSAbstractPersistantObject)} and a surrogate must be
   * used till the item is updated.
   *
    ...
    if (item.getContentId() <= 0) {
      surrogateId =
          PSGuidManagerLocator.getGuidMgr()
              .makeGuid(item.getContentId(), PSTypeEnum.LEGACY_CONTENT)
              .toString();
    } else {
  ...
  /**
   * Implementations need to override this method as to where the items will be stored. All items
   * should be in a folder so returning empty is bad.
   *
   * @param object never <code>null</code>.
   * @return the folder paths of where the object should be stored, never <code>null</code> or
   *     empty.
   */
