...
public class PSSiteImporter {
  ...
  /**
   * Given a JSoup document creates extracts the content appropriately and creates PSPageContent
   * object and returns it.
   *
   * @param doc assumed not <code>null</code>
   * @param logger {@link IPSSiteImportLogger} to use, assumed not <code>null</code>.
   * @return PSPageContent object with all fields filled in from the supplied document.
   */
  ...
  /**
   * Retrieves the body content of the document. If the document has no body content, it tryies to
   * build one using content that is outside header element, but inside html element.
   *
   * @param doc {@link Document} to get the body from, assumed not <code>null</code>.
   * @param logger {@link IPSSiteImportLogger} to use, assumed not <code>null</code>.
  ...
  /**
   * Gets the redirected url for the given site url. It finds out if the given site url gets
   * redirected, in which case it will follow those redirections and return the correct url (the
   * final one). If the url is not redirected, the same site url is returned.
   *
   * @param siteUrl {@link String} with the site url, must not be <code>null</code>.
   * @param logger {@link IPSSiteImportLogger} with the logger to use. Must not be <code>null</code>
   *     . * @param userAgent {@link String} with the user agent to set. Must not be <code>null
   *     </code>.
   * @return {@link String} with the final url, never <code>null</code> nor empty.
   * @throws IOException if an error occurs when trying to reach the url for the site.
   */
   ...
   * @param ignoreContentType if <code>true</code>, the connection will ignore content types.
   * @param followRedirects if <code>true</code>, the connection will follow all redirections.
   * @param userAgent {@link String} the user agent to set in the request, never<code>null</code> or
   *     empty.
  ...
  /**
   * Hold the current URL connection properties. This is used to restore values after they have been
   * overridden.
   */
