...
public class PSURLConverter {
  ...
  /**
   * @param baseUrl Url to be used as base for relative paths.Cannot be <code>null</code> or empty.
  ...
  public String getFullUrl(String linkPath) {
    ...
    try {
      ...
      this.logger.appendLogMessage(
          IPSSiteImportLogger.PSLogEntryType.ERROR,
          LogCategory.ConvertURL.getName(),
          "Invalid URL: " + baseUrl);
      return "";
    }
  }
  ...
  /**
   * Converts the specified resource link (that is relative to the original site) to the theme link
   * (after the resource has been imported into the theme).
   *
   * @param resourceUrl the resource URL in question. Cannot be <code>null</code>, may be empty.
   * @return {@link String} never <code>null</code>, may be empty.
   */
  ...
  /**
   * Convenience method that converts the remote url into the theme url (See {@link
   * #convertToThemeLink(String)}), and adds the {@link FileSuffixes Css suffix} to the link, if
   * needed.
   *
   * @param remoteUrl the resource URL in question. Cannot be <code>null</code>, may be empty.
   * @return {@link String} never <code>null</code>, may be empty.
   */
  ...
  /**
   * Gets the file system path for the specified resource. This is where the resource should be
   * saved.
   *
  ...
  /**
   * Convenience method to convert the remote url into file system path (See {@link
   * #getFileSystemPath(String)}), and adds the {@link FileSuffixes Css suffix} if needed.
   *
   * @param remoteUrl Original resource Url. Cannot be <code>null</code> or empty.
   * @return {@link String} never <code>null</code>, may be empty.
   */
   ...
   *
   * @param resourceUrl the resource URL in question. Cannot be <code>null</code>, may be empty.
   * @param getPathFromQuery <code>true</code> if the file is a css and should be renamed if it is
  ...
  private String getConvertedThemeLink(String resourceUrl, boolean getPathFromQuery) {
    ...
    try {
      ...
      return importPath
          + "/"
          + url.getHost()
          + validatePath(PSReplacementFilter.filter(url.getFile()));
  ...
  /**
   * Adds the desired suffix to the given path, in case it does not already have it.
   *
   * @param path {@link String} with the path to modify, assumed not <code>null</code>.
   * @param suffixes {@link Set} with the suffix to add, assumed not <code>null</code>.
  ...
  /**
   * Create a new name for the file. Appends an index and increments. The index can be incremented
   * if it is needed.
   *
   * @param incrementIndex <code>true</code> if the index should be incremented. <code>false</code>
   *     otherwise.
   * @return the new name for the file. If it already exists, the suffix is incremented.
   */
  ...
  private String validatePath(String path) {
    ...
    if (returnString.contains("?")) {
      ...
      try {
        ...
        alteredQueryString =
            new String(Base64.encodeBase64(alteredQueryString.getBytes(StandardCharsets.UTF_8)));
        returnString =
            returnString.substring(0, returnString.lastIndexOf("/") + 1) + alteredQueryString;
      } catch (Exception e) {
        this.logger.appendLogMessage(
            IPSSiteImportLogger.PSLogEntryType.ERROR,
            LogCategory.ConvertURL.getName(),
  ...
  /**
   * Some resources has queries in their paths. Their names is generated using the imported site
   * name and a suffix is appended. The suffix needs to be initialized because other files could be
   * downloaded before. The initialization counts the existing files in the theme directory. The
   * suffix is incremented for the next generated name.
   *
