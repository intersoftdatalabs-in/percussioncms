...
public class PSFileDownLoadJobRunner implements Runnable {
  ...
  public List<PSPair<Boolean, String>> downloadFile(String url, String destinationPath) {
    ...
    try {
      ...
      // If the destination and the source are the same then skipping file creation as they are
      // external resources.
      if (url.equals(destinationPath)) {
        PSPair<Boolean, String> result =
      ...
      PSPair<Boolean, String> result =
          new PSPair<>(false, getErrorMessage(url, destFile.getName()));
      localResults.add(result);
    }
    return localResults;
  }
  ...
  /**
   * Downloads a given file from the given url to a temp file. Then an Asset is created for the
   * given resource if it was not already downloaded. Otherwise an information message is logged to
   * indicate this situation.
   *
   * @param url {@link String} with the url where the file is hosted. Assumed not be <code>null
   *     </code>.
   * @param destinationPath {@link String} with the destination path for the downloaded file. No
   *     process is done to the path. Assumed not be <code>null</code>.
   * @return {@link PSPair}<{@link Boolean}, {@link String}> never <code>null</code>. The first
   *     element is <code>true</code> if the download was successfull, and <code>false</code>
   *     otherwise. The second element is the successfull message, or an error message, depending if
   *     the download could be completed or not.
   */
  public List<PSPair<Boolean, String>> downloadCreateAsset(String url, String destinationPath) {
    ...
    try {
      ...
      if (assetExist) {
        ...
        try (PSPurgableTempFile tempImage =
            new PSPurgableTempFile("tempImage", fileExtension, null)) {
   ...
   * @param ex the {@link Exception} to grab the message from. Assumed not <code>null</code>.
   * @return a {@link String} with the message, never <code>null</code> or empty.
   */
  ...
  /**
   * Builds the success message to use when a download was finished successfully.
   *
   * @param url {@link String} with the url, Assumed not be <code>null</code>.
   * @param destinationPath {@link String}, assumed not be <code>null</code>.
   * @return a {@link String} with the message, never <code>null</code> or empty.
  ...
  /**
   * Builds the informational message to use when a file was already downloaded.
   *
   * @param url {@link String} with the url, Assumed not be <code>null</code>.
   * @param destinationPath {@link String}, assumed not be <code>null</code>.
   * @return a {@link String} with the message, never <code>null</code> or empty.
  ...
  /**
   * Logs the results into the logger. Uses the object returned from the download methods, which
   * already has the messages and the status.
   *
   * @param results {@link List}<{@link PSPair}<{@link Boolean}, {@link String}>> holding the
   *     results from the download process. Not <code>null</code>. If it is empty, nothing will be
   *     logged.
   * @param logger {@link IPSSiteImportLogger}, Not <code>null</code>.
   * @return <code>false</code> if the results contain any errors, otherwise <code>true</code>.
   */
   ...
   *
   * @param file {@link File} file to be downloaded, assumed not be <code>null</code>.
   * @return a {@link Boolean} flag that indicates if the file was already downloaded, true if file
   *     exists. Never <code>null</code>. empty.
   */
