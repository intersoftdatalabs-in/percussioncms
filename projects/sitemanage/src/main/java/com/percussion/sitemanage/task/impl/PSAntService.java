...
public class PSAntService implements IPSAntService {
  ...
  public String runAnt(AntScript script) {
    ...
    if (targets.isEmpty()) targets.add(defaultTarget);
  ...
  /**
   * If the script has a listener, use that one. If not, create an instance of {@link
   * CommonsLoggingListener} and use that one.
   *
   * @param script the {@link AntScript} to get the listener from.
   * @return the listener that the script has, or a new instance of {@link CommonsLoggingListener}
   *     if the script does not have listeners.
   */
