...
/**
 * Saves log in separate thread, waits for {@link IPSSiteImportLogger#waitForThreads(long)} to
 * return to ensure all threads have had a chance to write to the log
 *
 * @author JaySeletz
 */
public class PSDeferredLogWriter implements Runnable {
  ...
  public void run() {
    try {
      ...
      log.error(
          "Failed to save import log for ID "
              + objectId
              + " and type "
              + logger.getType().name()
              + ": "
