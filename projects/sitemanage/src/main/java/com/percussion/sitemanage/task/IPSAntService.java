...
public interface IPSAntService {
  /**
   * Check to see if a script is running. This is only really useful for reporting purposes cannot
   * be relied on for thread safety. Consequently it maybe incorrect if multiple scripts of the same
   * name are running.
   *
  ...
  public static class AntScript {
    ...
    @NotNull private boolean blocking = true;
    @NotNull private Map<String, String> properties = new HashMap<>();
    ...
    /**
     * If <code>true</code> the call to {@link IPSAntService#runAnt(AntScript)} will block and wait
     * till the script is done. It is recommended that if set to <code>false</code> to add {@link
     * BuildListener} to know when the build is complete.
     *
     * @return never <code>null</code>.
     */
