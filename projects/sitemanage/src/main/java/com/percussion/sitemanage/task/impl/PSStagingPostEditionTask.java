...
/**
 * This is a post edition task assumed to be get added to all staging editions to perform certain
 * tasks after the staging publishing happens. The initial task is clearing of incremental
 * publishing queue.
 */
public class PSStagingPostEditionTask implements IPSEditionTask {
  ...
  public void perform(
      ...
      IPSEditionTaskStatusCallback statusService)
      throws Exception {
    log.info("Started staging post edition task...");
    PSPublishServerInfo info =
    ...
    if (PSPubServer.STAGING.equalsIgnoreCase(info.getServerType())) {
      ...
      for (IPSPubItemStatus item : statusService.getIterableJobStatus()) {
        contentChangeService.deleteChangeEvents(
            site.getSiteId(), item.getContentId(), PSContentChangeType.PENDING_STAGED);
        log.debug(
            "Cleared the item with id {} from staging incremenatl queue", item.getContentId());
      }
    } else {
