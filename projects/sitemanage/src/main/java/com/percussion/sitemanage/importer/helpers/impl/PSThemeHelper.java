...
/**
 * Helper class to create new theme folder while importing an external site from URL.
 *
...
public class PSThemeHelper extends PSImportHelper {
  ...
  public void process(PSPageContent pageContent, PSSiteImportCtx context)
    ...
    try {
      ...
      logger.appendLogMessage(
          ...
          helperCategory + " - " + themeCreationCategory,
  ...
  /**
   * Helper method used to rename the 2 basic files from the new theme: theme.css, theme.png into
   * the corresponding name created from site name.
   *
   * @author federicoromanelli
   * @param newThemeName, the name of the new theme (with collision detected and avoided). Never
   *     <code> null </code>
   * @param newSiteName, the name of the new theme (using the original site name). Never <code>
   *      null </code>
   */
  protected void renameBasicFiles(String newThemeName, String newSiteName) {
    ...
    logger.appendLogMessage(
        PSLogEntryType.STATUS,
        helperCategory + " - " + fileRenamingCategory,
  ...
  /**
   * Helper method used to perform transformations on the site name to make it suitable for theme
   * name.
   *
   * @author federicoromanelli
  ...
  protected void deleteTheme(String newThemeName) {
    try {
      logger.appendLogMessage(
          PSLogEntryType.STATUS,
          helperCategory + " - " + themeDeletionCategory,
          "Delete new theme: " + newThemeName);
      themeService.delete(newThemeName);
    } catch (Exception e) {
      log.info(
          "PSCreateThemeHelper: Couldn't delete theme template. The theme might not have been created");
    }
  }
