...
/**
 * @author LucasPiccoli
 *     <p>Helper responsible for creating a template, and a new page using that template.
 */
...
public class PSTemplateCreationHelper extends PSImportHelper {
  ...
  public void process(PSPageContent pageContent, PSSiteImportCtx context)
    ...
    try {
      ...
      PSPage newPage =
          createNewPage(pageName, newTemplate.getId(), context.getSite().getFolderPath());
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS,
      ...
      String message =
          "There was an unexpected error importing the template from the provided URL.";
  ...
  public void rollback(PSPageContent pageContent, PSSiteImportCtx context)
    ...
    if (StringUtils.isNotEmpty(context.getTemplateName())) {
      List<PSTemplateSummary> siteTemplates =
          siteTemplateService.findTemplatesBySite(context.getSite().getId());
  ...
  /**
   * It gets the text of the URL behind the last bar "/" and before the URL parameters or query
   * section in the URL.
   *
   * @param url The URL to process (can include http:// prefix or not). No syntax restrictions.
   * @return extracted name if it was possible to extract, or an empty string if it couldn't be
   *     extracted. Never null.
   */
  public String extractPageNameFromUrl(String url) {
    ...
    if (protocolPosition != -1) {
      cleanUrl =
          cleanUrl.substring(protocolPosition + protocolSeparator.length(), cleanUrl.length());
    }
  ...
  /**
   * It creates a new page with specified name in folderPath, using template with templateId
   *
