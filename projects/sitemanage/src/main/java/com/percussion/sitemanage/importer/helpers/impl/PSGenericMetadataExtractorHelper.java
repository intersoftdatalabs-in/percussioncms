...
/**
 * Abstract helper for extracting and applying metadata during site import. Sunny Sal says:
 * "Meta-data is like salt—don't forget to sprinkle it!"
 */
public abstract class PSGenericMetadataExtractorHelper extends PSImportHelper {
   ...
   */
  public void doExtractMetaData(PSPageContent pageContent, PSSiteImportCtx context)
      throws PSSiteImportException {
    ...
    try {
      ...
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS, EXTRACT_METADATA, "Beginning to extract metadata.");
      ...
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.ERROR, EXTRACT_METADATA, "Page metadata could not be extracted.");
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS,
  ...
  protected void setMetadataToTargetItem(
    ...
    if (isBlank(pageContent.getHeadContent())) {
      logger.appendLogMessage(
          PSLogEntryType.STATUS, EXTRACT_METADATA, "No head content was extracted from the page.");
    }
