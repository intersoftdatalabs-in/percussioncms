...
/**
 * Represents the site section in the architecture page. The site section is comprised of the
 * folder, navon (navtree for site home page) and landing page.
 *
 * @author bjoginipally
 */
...
public class PSSiteSection extends PSAbstractPersistantObject implements IPSFolderPath {
  ...
  /**
   * Gets the title of the section. It is navigation title of the navigation node and the link title
   * of the landing page of the node.
   *
   ...
   *
   * @return the IDs of the sub-sections. It is in the displayed order of the sub-sections. It may
   *     be empty if there is no sub-sections, but never <code>null</code>.
   */
  ...
  public void setChildIds(List<String> ids) {
    if (ids == null) this.childIds = Collections.emptyList();
    else this.childIds = ids;
  }
  ...
  /**
   * @return external link url may be <code>null</code> if not set. Meaningful only for external
   *     link sections.
   */
   ...
   *
   * @param displayTitlePath the displayTitlePath to set, {@link String} may be <code>null</code> or
   *     empty.
   ...
   * link node. This path is formed by the 'display title' of the root to the current node.
   *
   * @return the display title path. It may be <code>null</code> or empty if the node is not a
   *     section link.
   */
