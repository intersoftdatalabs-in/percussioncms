...
/**
 * Executes an Apache Ant script for a POST/PRE edition task. Currently it is assumed that the
 * Apache Ant scripts are stored in Rhythmyx/sys_resources/ant directory.
 *
 * <p>TODO allow rx_resources/ant directory. TODO Make {@link #getType()} configurable through
...
public class PSAntEditionTask implements IPSEditionTask {
  ...
  public void perform(
      ...
      IPSSite site,
      @SuppressWarnings("unused") Date startTime,
      @SuppressWarnings("unused") Date endTime,
      long jobId,
      @SuppressWarnings("unused") long duration,
      @SuppressWarnings("unused") boolean success,
      Map<String, String> params,
      @SuppressWarnings("unused") IPSEditionTaskStatusCallback callback)
      throws Exception {
    ...
    try {
      ...
      PSPubServerProperty folderProperty =
          pubServer.getProperty(pubServerMgr.PUBLISH_FOLDER_PROPERTY);
      if (folderProperty != null) {
        root = folderProperty.getValue();
      }
    } catch (Exception e) {
      log.error(
          "Failed to get the publishing server properties for server id '{}'.  Error: {}",
    ...
    if (pubServer != null) {
      ...
      if (!Boolean.parseBoolean(
          pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_OWN_SERVER_PROPERTY, "false"))) {
      ...
      if (Boolean.parseBoolean(
          ...
          && Boolean.parseBoolean(
              pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_SECURE_SITE_CONF, "false"))) {
        preparePublishConfigFiles(props, site, pubServer.getServerId());
      }
      ...
      if (isFilesystemType(deliveryType)) {
        listeners.add(
            new AntEditionTaskListener(site.getName(), edition.getName(), null, pubServer));
      } else {
        listeners.add(
            new AntEditionTaskListener(site.getName(), edition.getName(), siteRootTemp, pubServer));
      }
   ...
   *
   * @param deliveryType {@link String} assumed not <code>null</code> or blank.
   * @return <code>true</code> if the delivery type if {@link PublishType#filesystem} or {@link
   *     PublishType#filesystem_only}. Returns <code>false</code> otherwise.
   */
   ...
   *
   * @param deliveryType {@link String} assumed not <code>null</code> or blank.
   * @return <code>true</code> if the delivery type if {@link PublishType#sftp} or {@link
   *     PublishType#sftp_only}. Returns <code>false</code> otherwise.
   */
  private boolean isSftpType(String deliveryType) {
    return deliveryType.equals(PublishType.sftp.name())
        || deliveryType.equals(PublishType.sftp_only.name());
  }
   ...
   *
   * @param deliveryType {@link String} assumed not <code>null</code> or blank.
   * @return <code>true</code> if the delivery type if {@link PublishType#ftp} or {@link
   *     PublishType#ftp_only}. Returns <code>false</code> otherwise.
   */
  private boolean isFtpType(String deliveryType) {
    return deliveryType.equals(PublishType.ftp.name())
        || deliveryType.equals(PublishType.ftp_only.name());
  }
  ...
  private boolean isFtpsType(String deliveryType) {
    return deliveryType.equalsIgnoreCase(PublishType.ftps.name())
        || deliveryType.equalsIgnoreCase(PublishType.ftps_only.name());
  }

  /**
   * Adds the properties to the map for publishing secure or unsecure configuration files. If the
   * site is secure, it compares the modified date of each of the configuration files with the
   * last-published-date, retrieved from the tch file. If any of the files has been modified after
   * the last-published-date, the configuration files are re-published. Otherwise it just uses the
   * default path.
   *
   * @param props the map that holds the properties. Assumed not <code>null</code>
   * @param site The site to get the properties from.
   * @throws IOException if an error occurs when comparing the files
   */
  private void preparePublishConfigFiles(Map<String, String> props, IPSSite site, long serverId)
      throws IOException, IPSDataService.DataServiceSaveException,
          IPSSiteSectionService.PSSiteSectionException, PSNotFoundException {
  ...
  /**
   * calculate the path for the live site content, if the site will be published on FTP or SFTP we
   * use a temporary directory, if not site.getRoot(); will be used.
   *
   ...
   */
  private String prepareSiteRootTemp(
      long jobId, String editionName, IPSSite site, IPSPubServer pubServer, String root) {
    // Local Live site
    String deliveryType = pubServer.getPublishType();
    if (isFilesystemType(deliveryType)) {
      return root;
    ...
    try {
      ...
      if (!tempPath.equals("")) {
        ...
        File tempDir =
            new File(
                deliveryManager.getTempDir(), site.getName() + "-" + editionName + "-" + jobId);
   ...
   *
   * @param site the site (assumed not <code>null</code>) for which the configuration files are
   *     going to be created.
   */
  private void createSecurityUrlPatternFile(IPSSite site)
      throws IPSDataService.DataServiceSaveException, IPSSiteSectionService.PSSiteSectionException,
          PSNotFoundException {
  ...
  /**
   * Add the properties for the FTP publishing type if publish type is FTP or SFTP.
   *
   * @param props the map of the publish properties.
   * @param site the current site.
   * @param pubServer the publishing server
   */
  private void prepareFtpProperties(
      Map<String, String> props, IPSSite site, IPSPubServer pubServer) {
    String deliveryType = pubServer.getPublishType();
    if (isFilesystemType(deliveryType)) {
      return;
    }
    ...
    props.put(
        "perc.ftp.username",
        pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_USER_ID_PROPERTY, ""));
    props.put(
        "perc.ftp.password",
        pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_PASSWORD_PROPERTY, ""));
    props.put(
        "perc.ftp.serverAddress",
        pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_SERVER_IP_PROPERTY, ""));

    props.put(
        "perc.ftp.sourceDir",
        pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_FOLDER_PROPERTY, ""));
    ...
    if (isFtpType(deliveryType)) {
      ...
      props.put(
          "perc.ftp.port", pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_PORT_PROPERTY, "21"));
    }
    if (isFtpsType(deliveryType)) {
      props.put("perc.publishingFtp.set", Boolean.TRUE.toString());
      props.put("perc.ftp.useFtps", Boolean.TRUE.toString());
      props.put(
          "perc.ftp.port", pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_PORT_PROPERTY, "23"));
    } else if (isSftpType(deliveryType)) {
      props.put("perc.publishSecureFtp.set", Boolean.TRUE.toString());
      props.put(
          "perc.ftp.port", pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_PORT_PROPERTY, "22"));
    }

    // We first try to use the key from the delivery handler.
    PSSFtpDeliveryHandler sftpDeliveryHandler =
        (PSSFtpDeliveryHandler) PSBaseServiceLocator.getBean("sys_sftp");
    String serverPrivateKey =
        pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_PRIVATE_KEY_PROPERTY, "");
   ...
   *
   * @param pubServer {@link IPSPubServer} object, assumed not <code>null</code>.
  ...
  @Override
  public void init(
      @SuppressWarnings("unused") IPSExtensionDef def, @SuppressWarnings("unused") File file) {
  ...
  private static class AntEditionTaskListener implements BuildListener {
    ...
    public AntEditionTaskListener(
        String sitename, String editionName, String temporaytPath, IPSPubServer pubServer) {
    ...
    public void targetFinished(BuildEvent event) {
      if (event.getTarget() != null) {
        log.debug(
            "Finished Target '{}' for edition '{}'.", event.getTarget().getName(), editionName);
      } else {
    ...
    public void targetStarted(BuildEvent event) {
      if (event.getTarget() != null) {
        log.debug(
            "Started Target '{}' for edition '{}'.", event.getTarget().getName(), editionName);
      } else {
    ...
    public void taskFinished(BuildEvent event) {
      if (event.getTask() != null) {
        log.debug(
            "Finished Task '{}' for edition '{}'.", event.getTask().getTaskName(), editionName);
      } else {
    ...
    public void taskStarted(BuildEvent event) {
      if (event.getTask() != null) {
        log.debug(
            "Started Task '{}' for edition '{}'.", event.getTask().getTaskName(), editionName);
      } else {
