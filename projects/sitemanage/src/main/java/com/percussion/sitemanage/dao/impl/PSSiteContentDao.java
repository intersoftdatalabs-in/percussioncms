...
public class PSSiteContentDao implements com.percussion.sitemanage.dao.IPSSiteContentDao {
  ...
  public void createRelatedItems(PSSite site) {
    ...
    try {
      ...
      // Means Site with the same name already existed before but was deleted, so returns all items
      // back
      if (navSummary != null) {
        ...
        if (homepage != null && !recyclerService.isInRecycler(homepage.getId())) {
          ...
          try {
            IPSAssemblyTemplate baseTemplate =
                assemblyService.findTemplateByName(site.getTemplateName());
            IPSGuid tempId =
  ...
  private PSTemplateSummary createSiteTemplate(PSSite site)
    ...
    IPSAssemblyTemplate baseTemplate =
        assemblyService.findTemplateByName(site.getBaseTemplateName());
    PSTemplateSummary templateSummary = null;
    IPSGuid tempId = null;
  ...
  private void deleteFolder(String folderPath, int retryCount) throws Exception {
    ...
    //        } catch (Exception e)
    //        {
    //            log.warn("Cannot find folder: " + folderPath + " to delete, may already be
    // deleted.");
    //            log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    //        }
