...
public class PSSiteSectionRestService {
  ...
  public PSSiteSection createExternalLinkSection(PSCreateExternalLinkSection req)
    ...
    try {
      return siteSectionService.createExternalLinkSection(req);
        ...
        | PSValidationException
        | IPSPathService.PSPathNotFoundServiceException e) {
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw e;
    }
  }
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSSiteSection createSectionLink(
      @PathParam("targetSectionGuid") String targetSectionGuid,
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSNoContent deleteSectionLink(
      @PathParam("sectionGuid") String sectionGuid,
      @PathParam("parentSectionGuid") String parentSectionGuid) {
    notNull(sectionGuid);
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSSiteSection updateExternalLink(
      @PathParam("sectionGuid") String sectionGuid, PSCreateExternalLinkSection req)
  ...
  public PSSiteSection loadRoot(@PathParam("siteName") String siteName)
    ...
    try {
      ...
      String invalidMessage =
          new String(
              MessageFormat.format("The site with name: {0} does not exist", args)
      ...
      PSParameterValidationUtils.validateParameters("loadSite")
          .reject(siteName, invalidMessage)
          .throwIfInvalid();
    } catch (IPSSiteSectionService.PSSiteSectionException e) {
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw e;
