  ...
  public void process(PSPageContent pageContent, PSSiteImportCtx context)
    ...
    try {
      ...
      if (statusMessagePrefix != null
          && statusMessagePrefix.contains("template")) { // Importing template
        baseUrl = getBaseUrl(context, sourceDoc);
      } else { // importing site
        baseUrl = context.getSite().getBaseUrl();
      }
      if (baseUrl.equals("")) baseUrl = context.getSiteUrl();
      ...
      List<PSPair<Boolean, String>> linkResults =
          fileDownloader.downloadFiles(linkPaths, context, false);
      int linksCount = 0;
      for (PSPair<Boolean, String> linkResult : linkResults) {
        if (linkResult.getFirst()) linksCount++;
      }
      ...
      List<PSPair<Boolean, String>> assetResults =
          fileDownloader.downloadFiles(assets, context, true);
      int assetsCount = 0;
      for (PSPair<Boolean, String> assetResult : assetResults) {
        if (assetResult.getFirst()) assetsCount++;
      }
      if (linksCount > 0 || assetsCount > 0) {
        summaryStats.put(
            IPSSiteImportSummaryService.SiteImportSummaryTypeEnum.STYLESHEETS,
            new Integer(linksCount));
        summaryStats.put(
            IPSSiteImportSummaryService.SiteImportSummaryTypeEnum.FILES, new Integer(assetsCount));
