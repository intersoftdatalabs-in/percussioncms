 ...
 * is currently in the state specified by the parameter {@value #STATE_PARAMETER} and not checked
 * out by any user.
 *
 * <p>The trigger used to workflow the item is specified by the parameter {@value
 * #TRIGGER_PARAMETER}.
 *
 * <p>
 *
...
public class PSWorkflowEditionTask extends PSAbstractWorkflowExtension implements IPSEditionTask {
   ...
   * {@inheritDoc}
   *
   * <p>Workflows all the items in the job if they are valid.
   */
  @Override
  public void perform(
    ...
    for (var item : items) {
      try {
        ...
        log.error(
            "Error workflowing this content: {} Error: {}",
            item.getContentId(),
