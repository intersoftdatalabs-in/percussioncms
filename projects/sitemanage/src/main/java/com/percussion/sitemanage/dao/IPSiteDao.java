...
/**
 * Data access object for Percussion CMS sites. Sunny Sal says: "A good DAO is like a good
 * chai—keeps things running smoothly!"
 */
public interface IPSiteDao extends IPSGenericDao<PSSite, String> {
   ...
   * @param isValidate {@code true} to validate the site contains a "category" folder; {@code false}
   *     to skip validation (e.g., for preview or publishing).
   * @return the site summary, or {@code null} if not found.
   */
  PSSiteSummary findByLegacySiteId(String id, boolean isValidate);

  /**
   * Updates the specified site and its related edition/content-list/pubservers with the new name
   * and description.
   *
   ...
   * @return {@code true} if a pubserver was modified as a result of the change, {@code false}
   *     otherwise.
   * @throws PSNotFoundException if the site is not found.
   */
  boolean updateSite(IPSSite site, String newName, String newDescription)
      throws PSNotFoundException;

  /**
   * Updates the specified site with the given publishing properties and updates the content-list
   * with the user-specified delivery type.
   *
   * @param site the existing site, not {@code null}.
   * @param publishProps publishing properties to update, not {@code null}.
   * @throws PSNotFoundException if the site is not found.
   */
  void updateSitePublishProperties(IPSSite site, PSSitePublishProperties publishProps)
      throws PSNotFoundException;
  ...
  /**
   * Adds the publish-now infrastructure (edition, content list) for the specified site. Assumes
   * publish-now support does not exist for the site.
   *
   * @param site the existing site, not {@code null}.
   * @throws PSNotFoundException if the site is not found.
   */
  ...
  /**
   * Adds the unpublish-now infrastructure (edition, content list) for the specified site. Assumes
   * unpublish-now support does not exist for the site.
   *
   * @param site the existing site, not {@code null}.
   * @throws PSNotFoundException if the site is not found.
   */
  ...
  /**
   * Creates a site with content copied from an existing site. The result is a new site visible in
   * the Finder, including a copy of all content from the original site (templates are not
   * included). The site will have a default publishing configuration.
   *
