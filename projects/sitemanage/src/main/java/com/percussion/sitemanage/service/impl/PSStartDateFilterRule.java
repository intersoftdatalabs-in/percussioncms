...
/**
 * This item filter is a publishnow item filter. It is used to get items only if the start date of
 * the item is not null and before the current date or equal to the current date. Returns list of
 * items which match the filtering criteria.
 */
public class PSStartDateFilterRule implements IPSItemFilterRule {
  ...
  private boolean isPublishable(PSComponentSummary summary) {
    if (passedStartPublishDate(summary)) return true;
