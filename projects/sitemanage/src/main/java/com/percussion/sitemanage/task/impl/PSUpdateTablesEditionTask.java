...
/**
 * This edition task is used to create or update the tables that is defined by perc.pageDatabase
 * template.
 *
 * @author YuBingChen
 */
public class PSUpdateTablesEditionTask implements IPSEditionTask {
  ...
  /**
   * Gets the template defined by the given parameters. The template name is default to
   * "perc.pageDatabase" if the template name is not defined in the parameters.
   *
   * @param params the parameters of the edition task, assumed not <code>null</code>.
   * @return the template with the specified name. It may be <code>null</code> if cannot find the
   *     template.
   */
  private IPSAssemblyTemplate getTemplate(Map<String, String> params) {
    IPSAssemblyTemplate template = null;
    String templateName =
        isBlank(params.get("template")) ? "perc.pageDatabase" : params.get("template");

    try {
      template = PSAssemblyServiceLocator.getAssemblyService().findTemplateByName(templateName);
    } catch (PSAssemblyException e) {
      log.error(
          "Skip update tables because cannot find template: {} Error: {}",
          templateName,
  ...
  private void updateTables(Document doc, IPSEdition edition) throws Exception {
    ...
    try {
      ...
      if (dbmsInfo == null) dbmsInfo = new DbmsInfo(doc);
      ...
      PSJdbcTableFactory.processTables(
          dbmsConn.getConnection(),
          dbmsConn.getDbmsDef(),
          dbmsConn.getTableMetaMap(),
          null,
          doc,
          new PrintStream(System.out) {
            ...
            public void println(String msg) {
              ...
              if (msg.indexOf("Error") >= 0) errorBuilder.append(msg);
      ...
      if (dbmsConn != null) dbmsConn.close();
    }
  }
   ...
   */
  private Document getTableDefs(IPSEdition edition, IPSAssemblyTemplate template)
      throws PSAssemblyException {
    var xml = template.getTemplate();
    xml =
        xml.replaceAll(
            "allowSchemaChanges=\"n\" alter=\"n\"", "allowSchemaChanges=\"y\" alter=\"y\"");
    xml =
        xml.replace(
            "<tabledefset>",
            "\n"
                + "<datapublisher dbname=\"cmlite_db\" drivertype=\"jtds:sqlserver\" origin=\"dbo\""
    ...
    try {
      ...
      log.error(
          "Failed to create XML for String: {} Error: {}",
          xml,
          PSExceptionUtils.getMessageForLog(e));
      return null;
    }
