...
/**
 * Implements the rest service layer which delegates to the {@link IPSSitePublishService} for
...
public class PSSitePublishServiceWebAdapter {
  ...
  @Autowired private IPSWorkflowHelper workflowHelper;

  @Autowired private IPSIdMapper idMapper;

  @Autowired private IPSContentWs contentWs;

  @Autowired private IPSContentChangeService changeSvc;
   ...
   *
   * @param sitePublishService the site publish service used for publishing operations.
   */
  @Autowired
  public PSSitePublishServiceWebAdapter(IPSSitePublishService sitePublishService) {
  ...
  public PSSitePublishResponse publish(
      @PathParam("name") String name, @PathParam("server") String server) {
    try {
      if (StringUtils.isBlank(name)) throw new IllegalArgumentException("name may not be blank");
  ...
  public PSSitePublishResponse publishResource(@PathParam("id") String id) {
    try {
      if (StringUtils.isBlank(id)) throw new IllegalArgumentException("name may not be blank");
  ...
  /**
   * Returns the list of PSPublishingActionProperties objects and each object contains the publish
   * action.
   *
   * @param id of the guid representation of the id of the page or resource to be published, may not
   *     be blank.
  ...
  public PSSitePublishResponse takeDownPage(
      @PathParam("id") String id, ArrayList<PSPageLinkedToItem> linkedPages) {
    if (linkedPages != null) {
          ...
          try {
            ...
            if (relationship.isInlineRelationship()) {
              ...
              final List<PSItemSummary> owners =
                  contentWs.findOwners(idMapper.getGuid(relationship.getOwner()), filter, false);
      ...
      if (!relationshipSet.isEmpty()) {
        PSRelationshipChangeEvent event =
            new PSRelationshipChangeEvent(PSRelationshipChangeEvent.ACTION_MODIFY, relationshipSet);
        PSNotificationEvent notifyEvent =
            new PSNotificationEvent(PSNotificationEvent.EventType.RELATIONSHIP_CHANGED, event);
      ...
      for (Integer contentId : contentIds) {
        ...
        List<IPSSite> siteIds =
            folderHelper.getItemSites(idMapper.getGuidFromContentId(contentId).toString());
        // Because if owner of relationship is already deleted, before child, then it will not
        // return site.
  ...
  public PSPagedItemList getQueuedIncrementalRelatedContent(
      ...
      @QueryParam("pageSize") int pageSize) {
    try {
      return sitePublishService.getQueuedIncrementalRelatedContent(
          siteName, serverName, startIndex, pageSize);
    } catch (IPSSitePublishService.PSSitePublishException e) {
      log.error(PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
  ...
  public PSSitePublishResponse publishIncrementalWithApproval(
      ...
      @PathParam("itemsToApprove") String itemsToApprove) {
    try {
      Validate.notEmpty(name);
      Validate.notEmpty(server);

      return sitePublishService.publishIncrementalWithApproval(
          name, null, false, server, itemsToApprove);
    } catch (PSDataServiceException
        | IPSItemWorkflowService.PSItemWorkflowServiceException
        | IPSPubServerService.PSPubServerServiceException
        | IPSItemService.PSItemServiceException
