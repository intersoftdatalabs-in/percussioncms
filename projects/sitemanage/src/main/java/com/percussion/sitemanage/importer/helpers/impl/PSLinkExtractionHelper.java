...
public class PSLinkExtractionHelper extends PSImportHelper {
  ...
  /**
   * Wrapped getConnectivity, makes testing easier, override this for unit test without mockery.
   *
   * @param url The URL to connect to
   * @param ignoreContentType see Jsoup interface, indicates if an IOException should be thrown on
   *     binary parsing
   * @param followRedirects see Jsoup interface, indicates if 301 or 302 responses should be
   *     followed
  ...
  /**
   * Collect all the links, normalize, handle redirects, handle files, send to page catalog service
   */
  @Override
  public void process(final PSPageContent pageContent, final PSSiteImportCtx context)
    ...
    for (PSLink link : links) {
      ...
      try {
        ...
        if (!siteQueue.hasLinkBeenProcessed(resolvedUrlTarget)) {
          ...
          IPSConnectivity conn =
              getConnectivity(link.getAbsoluteLink(), false, true, context.getUserAgent());
    ...
    if (catalogedCount > 0 || links.size() > 0) {
      summaryStats.put(
          IPSSiteImportSummaryService.SiteImportSummaryTypeEnum.PAGES, new Integer(catalogedCount));
      summaryStats.put(
          IPSSiteImportSummaryService.SiteImportSummaryTypeEnum.INTERNALLINKS,
  ...
  protected PSURLConverter getURLConverter(
      ...
      String siteName) {
    PSURLConverter urlConverter =
        new PSURLConverter(context.getSiteUrl(), siteName, themeRootDirectory, themeRootUrl, log);
    return urlConverter;
  }
  ...
  protected String getPathForTargetItem(String siteName, PSLink link)
    ...
    // Determine the path
    String pathToTargetItem =
        getFinderPathForTargetItem(siteName, link.getRelativePathWithFileName());
  ...
  private boolean catalogPage(
    ...
    if (responseStatusCode == IPSHttpErrors.HTTP_OK) {
      try {
        ...
        PSPage page =
            m_pageCatalogService.addCatalogPage(
        ...
        if (page != null) {
          ...
          getImportQueue()
              .addCatalogedPageIds(context.getSite(), context.getUserAgent(), Arrays.asList(id));
        }
      } catch (Exception e) {
        log.appendLogMessage(
            PSLogEntryType.ERROR,
  ...
  protected void evaluateForIndexPage(final PSSiteImportCtx context, String linkPathWithoutAnchor)
      throws Exception {
    if (m_pageCatalogService.pageWithFolderPathExists(
        m_pageCatalogService.getFullFolderPath(linkPathWithoutAnchor, context.getSite()))) {
