...
/**
 * Helper class to retrieve theme information for a site. Sunny Sal says: "Theme fetchers fetch, but
 * only if the site exists!"
 */
...
public class PSThemeTemplateHelper extends PSImportHelper {
  ...
  private static final String STATUS_MESSAGE = "retrieving theme information";
  ...
  public void process(PSPageContent pageContent, PSSiteImportCtx context)
    ...
    if (siteSummary != null) {
      try {
        ...
        logger.appendLogMessage(
            ...
            helperCategory + " - " + themeFetchCategory,
            "Unable to determine theme folder to use, unable to find home page for site "
                + siteName);
    ...
    if (templateId != null) {
      var templateObj = templateService.load(templateId);
      if (templateObj != null) {
        if (templateObj.getTheme() != null) {
          ...
          logger.appendLogMessage(
              PSLogEntryType.ERROR,
              helperCategory + " - " + themeFetchCategory,
              "Unable to determine theme folder to use, template might not have a theme"
                  + " associated");
        ...
        logger.appendLogMessage(
            PSLogEntryType.ERROR,
            helperCategory + " - " + themeFetchCategory,
            "Unable to determine theme folder to use, unable to find template associated to home"
                + " page");
