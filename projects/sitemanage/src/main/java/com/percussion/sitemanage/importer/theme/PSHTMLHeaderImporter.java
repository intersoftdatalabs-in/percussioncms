...
/**
 * Importer class that will be get the links and scripts from header of a given document. Also the
 * header is updated properly with the new paths for links and scripts.
 *
...
public class PSHTMLHeaderImporter {
  ...
  /**
   * Constructor, builds an instance of the importer with the given parameters.
   *
   * @param sourceDoc {@link Element} which holds the <header> element from the imported site. Must
   *     not be <code>null</code>.
   * @param siteUrl {@link String} with the url for the site the user is trying to import. Must not
   *     be <code>null</code>.
   * @param siteName {@link String} the name of the site the user is trying to import. Must not be
   *     <code>null</code>.
   * @param themeRootDirectory the theme root on the file system. Cannot be <code>null</code> or
   *     empty.
   * @param themeRootUrl the theme root URL that is relative to any site that references the theme.
  ...
  /**
   * Gets the absolute paths of scripts inside the header. The absolute remote URL as well the path
   * to download the CSS file are added into a map object. Also the source link is updated with the
   * new link, to match the correct path once the source document is saved later. If the href
   * attribute is empty or null, then it was ignored and not added into the map.
   *
   * @return {@link Map}<{@link String}, {@link String}> where the key is the url of the file, and
   *     the value is the absolute local path where the file is going to be saved to. Never <code>
   *     null</code>, but may be empty if there is no link to add.
   */
  ...
  /**
   * Gets the absolute paths for the list of scripts inside the header. The absolute remote URL as
   * well the path to download the CSS file are added into a map object. Also the source link is
   * updated with the new link, to match the correct path once the source document is saved later.
   * If the scr attribute is empty or null, then it was ignored and not added into the map.
   *
   * @return {@link Map}<{@link String}, {@link String}> where the key is the URL of the script, and
   *     the value is the absolute local path where the file is going to be saved to. Never <code>
   *     null</code>, but may be empty if there is no scripts to add.
   */
  ...
  /**
   * Processes any inline images inside the Style tag. To get those images replaced properly, calls
   * the CSS parser with the style content, and the updates the Style tag inside the document
   * header. A map is returned so the images
   *
   * <p>determined.
   *
   * @return {@link Map}<{@link String}, {@link String}> that holds the images that need to be
   *     downloaded. Never <code>null</code> but may be empty.
   */
  ...
  /**
   * Uses the {@link PSCSSParser css parser} to process the downloaded css files, to get the images
   * that need to be downloaded.
   *
   * @param cssFilesMap {@link Map}<{@link String}, {@link String}> map where the key is the url of
   *     the css file, and the value is the local path. Must not be <code>null</code>.
   * @return {@link Map}<{@link String}, {@link String}> where the key is the url of the image to be
   *     downloaded, and the value is the local path where it should be downloaded to. Never <code>
  ...
  /**
   * Processes the image URLS referenced in img tags and input type=image that are present in the
   * HTML header and body. In order to get the correct URLs if needed, calls to the PSURLConvert.
   * Then the original URL is replaced in the img tag, and the image is added into the map, to be
   * downloaded.
   *
   * @return {@link Map}<{@link String}, {@link String}> that holds the images that need to be
   *     downloaded. Never <code>null</code> but may be empty.
  ...
  public Map<String, String> processHeaderAndBodyImages() {
    ...
    appendHeaderImporterMessage(
        "Finished the processing for images referenced in <img> and <input type=image> tags. Processed: "
  ...
  /**
   * Gets the absolute paths for the list of swf files inside the header. The absolute remote URL as
   * well the path to download the CSS file are added into a map object. Also the source link is
   * updated with the new link, to match the correct path once the source document is saved later.
   * If the scr attribute is empty or null, then it was ignored and not added into the map.
   *
  ...
  /**
   * Validate if a link element has the attributes to be accepted as a valid link.
   *
   ...
   * @return <code>true</code> if the link is a valid css link. <code>false</code> otherwise.
   */
  ...
  /**
   * Validates if the {@link Element} is a valid flash object element. In order to validate the
   * supplied element is a valid flash file, the method checks the param movie, as it is a required
   * param for flash object.
   *
   * @param flashObject {@link Element} assumed not <code>null</code>.
   * @return <code>true</code> if the link is a valid flash file. <code>false</code> otherwise.
   */
  ...
  /**
   * Processes the images referenced in the style tag inside the document header. To get those
   * images replaced properly, calls the CSS parser and passes the style content to process and
   * replace the URLs as needed. Then updates the style in the header with the modified content. A
  ...
  /**
   * Processes the images referenced in the style tags inside the document body. To get those images
   * replaced properly, calls the CSS parser and passes the style content to process and replace the
   * URLs as needed. Then updates the style in the body with the modified content. A map is returned
   * with the images that need to be downloaded.
   *
   * @return {@link Map}<{@link String}, {@link String}> that holds the images that need to be
   *     downloaded. Never <code>null</code> but may be empty.
   */
  ...
  /**
   * Processes the images referenced in the style attributes referenced in the document body. To get
   * those images replaced properly, calls the CSS parser and passes the style content to process
   * and replace the URLs as needed. Then updates the style in the body with the modified content. A
   * map is returned with the images that need to be downloaded.
   *
   * @return {@link Map}<{@link String}, {@link String}> that holds the images that need to be
   *     downloaded. Never <code>null</code> but may be empty.
  ...
  private Map<String, String> processBodyStyleAtributes() {
    ...
    for (Element el : styleElements) {
      ...
      // process the content of style tag
      PSPair<Map<String, String>, String> parserResult =
          cssParser.parse(this.siteUrl, el.attr("style"));
  ...
  /**
   * Process the attributes of a flash object and update the attribute value with the new location
   * for the swf file. A map is returned with the swf files that need to be downloaded.
   *
   * @param flash {@link Element} the flash object element to be processed.
   * @param cssQuery {@link String} the jsoup query used to get the elements from flash object.
   * @param attribValue {@link String} the attribute value to get and update.
   * @param siteName {@link String} the name of the site.
   * @return {@link Map}<{@link String}, {@link String}> that holds the swf files that need to be
   *     downloaded. Never <code>null</code> but may be empty.
   */
