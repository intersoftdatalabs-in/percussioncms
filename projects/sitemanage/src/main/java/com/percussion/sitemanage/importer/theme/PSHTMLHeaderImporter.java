...
/**
 * Importer class that gets the links and scripts from header of a given document.
 * Also updates the header with new paths for links and scripts.
 */
public class PSHTMLHeaderImporter {
    ...
    public Map<String, String> getLinkPaths() {
        ...
        appendHeaderImporterMessage("Finished the processing for link paths. Processed: " + linkPaths.size() + " elements.");
        return linkPaths;
    }
    ...
    public Map<String, String> getScriptPaths() {
        ...
        appendHeaderImporterMessage("Finished the processing for script paths. Processed: " + scriptPaths.size() + " elements.");
        return scriptPaths;
    }
    ...
    public Map<String, String> processCssFiles(Map<String, String> cssFilesMap) {
        ...
        appendHeaderImporterMessage("Completed the processing for images in CSS files. Processed: " + images.size() + " elements.");
        return images;
    }
    ...
    public Map<String, String> processHeaderAndBodyImages() {
        ...
        for (var imgElement : imgElements) {
            if (!isBlank(imgElement.attr("src"))) {
                ...
                appendHeaderImporterMessage(MessageFormat.format(CONVERTED_SCRIPT_URL, imgElement.attr("src"), fullThemePath));
    ...
    public Map<String, String> processFlashFiles(String siteName) {
        ...
        appendHeaderImporterMessage("Finished the processing for swf files in <object> tags. Processed: " + embedFlashPaths.size() + " elements.");
        return embedFlashPaths;
    }

    private void appendHeaderImporterMessage(String message) {
        this.logger.appendLogMessage(IPSSiteImportLogger.PSLogEntryType.STATUS, LogCategory.ImportHeader.getName(), message);
    }
    ...
    private boolean isValidLinkElement(Element link) {
        ...
        return equalsIgnoreCase(relAttribute, ICON_REL_ATTRIBUTE) || equalsIgnoreCase(relAttribute, SHORT_ICON_REL_ATTRIBUTE);
    }
    ...
    private Map<String, String> processFlashObject(Element flash, String cssQuery, String attribValue, String siteName) {
        ...
        for (var movie : flashElements) {
            if (!isBlank(movie.attr(attribValue))) {
                ...
                appendHeaderImporterMessage(MessageFormat.format(CONVERTED_SCRIPT_URL, movie.attr(attribValue), fullThemePath));
