...
class PageSaveRunner implements Runnable {
  ...
  public PageSaveRunner(
      Map<String, Object> requestInfoMap,
      PSPage targetPage,
      PSPageContent pageContent,
      PSSiteImportCtx context,
      PSPageExtractorHelper pageExtractorHelper,
  ...
  public void run() {
    ...
    try {
      ...
      if (siteQueue.getImportingIds().contains(id)) siteQueue.addImportedId(id);
    } catch (Exception e) {
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.ERROR, STATUS_MESSAGE, PSExceptionUtils.getMessageForLog(e));
...
public class PSPageExtractorHelper extends PSGenericMetadataExtractorHelper {
    ...
    try {
      ...
      PageSaveRunner pageSaveRunner =
          new PageSaveRunner(
              ...
              this,
              idMapper,
              getPageCatalogService(),
              pageImport);

      Thread t = new Thread(pageSaveRunner);
      t.setDaemon(true);

      if (this.runSaveSyncronously) t.run();
      else t.start();
  ...
  public void doPageExtraction(
    ...
    PSAsset localAsset =
        createHTMLLocalContent(extractedBodyHtml, itemWorkflowService, assetService, nameGenerator);

    PSPair<PSWidgetItem, PSAsset> widgetAssetPair = new PSPair<>(widgets.get(0), localAsset);
  ...
  private void setDescriptionOnPage(PSPageContent pageContent, PSSiteImportCtx context) {
    ...
    try {
      context
          .getLogger()
          .appendLogMessage(PSLogEntryType.STATUS, EXTRACT_METADATA, "Updating page description.");
      ...
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.ERROR, EXTRACT_METADATA, "Page description could not be updated.");
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS,
  ...
  @Override
  protected void setMetadataToTargetItem(
      PSPageContent pageContent, IPSSiteImportLogger logger, IPSHtmlMetadata targetItem) {
  ...
  private void setDescriptionInMetadata(
      PSPageContent pageContent, IPSSiteImportLogger logger, IPSHtmlMetadata targetItem) {
  ...
  public void importPageIfNecessary(
      PSSiteImportCtx context, IPSPageCatalogService pageCatalogService, boolean pageImport)
      throws PSSiteImportException {
  ...
  /**
   * Adds the given widget on a page. Basically it creates instances of {@link PSRelationship} where
   * the owner is the page, and the dependant is the given asset, using the widget id (slot id) from
   * the {@link PSWidgetItem} object.
   *
  ...
  private static List<PSAssetWidgetRelationship> addContentToWidgetOnPage(
      PSPage targetPage,
      PSPair<PSWidgetItem, PSAsset> widgetAssetPair,
      IPSAssetService assetService)
  ...
  private static String extractBody(PSPageContent pageContent, PSSiteImportCtx context) {
    try {
      ...
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS, STATUS_MESSAGE, "Page body extraction finished successfully.");
      ...
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.ERROR, LOG_CATEGORY, "Unable to create HTML widget from page body.");
      context
          .getLogger()
          .appendLogMessage(
              PSLogEntryType.STATUS,
              LOG_CATEGORY,
              "Unable to create HTML widget from page body: " + e.getLocalizedMessage());
      return new String();
    }
  }
   ...
   *
   * @param body {@link Element} to comment the references from. Assumed not <code>null</code>.
   * @param logger {@link IPSSiteImportLogger} to append the message. Assumed not <code>null</code>.
   */
  private static void commentOutManagedJSReferences(Element body, IPSSiteImportLogger logger) {
    ...
    for (Element scriptTag : scriptTags) {
      if (isManagedJSReference(scriptTag)) {
        logger.appendLogMessage(
            PSLogEntryType.STATUS, COMMENTED_JS_REFERENCE_FROM_BODY, scriptTag.toString());
        commentTag(body, scriptTag);
      }
    }
  }

  private static PSAsset createHTMLLocalContent(
      String htmlContent,
      IPSItemWorkflowService itemWorkflowService,
      IPSAssetService assetService,
      IPSNameGenerator nameGenerator)
    ...
    int workflowId = itemWorkflowService.getWorkflowId(PSWorkflowHelper.LOCAL_WORKFLOW_NAME);
  ...
  protected void saveTargetItem(IPSHtmlMetadata targetItem) throws PSDataServiceException {
    ...
    PSHelperPerformanceMonitor.updateStats(
        "PSPageMetaDataExtractor:PageWorkflow", ((System.nanoTime() - workflowTimer) / 1000000));
  }
  ...
  protected void addHtmlWidgetToTemplate(PSSiteImportCtx context)
    ...
    PSTemplate template = unassignedTemplateCache.get(context.getSite().getSiteId());
