...
public interface IPSSitePublishService {
  ...
  /** The publishing action type. Used to indicate which edition will be invoked. */
  public enum PubType {
  ...
  /**
   * Helper method that calls {@link #publish(String, PubType, String, boolean, String)} by passing
   * the right pub type.
   *
   * @param siteName the site to be published or parent site of the page to be published. Can be
   *     <code>null</code> for resources.
   * @param id of the item to be published.
   * @param isResource <code>true</code> if the item is a resource (image, file), <code>false</code>
   *     otherwise.
   * @param server The name of the server, must not be blank for PubType {@link PubType.FULL},
   *     ignored the passed in server for other PubTypes and system uses the default server set on
   *     the site
   * @return response object including the status of the completed publishing job. See {@link
   *     PSSitePublishResponse} for details.
   * @throws PSSitePublishException if the requested publishing edition could not be found or if an
   *     invalid request is supplied for demand publishing.
   */
  ...
  public PSSitePublishResponse publish(
      ...
      throws PSDataServiceException, IPSPubServerService.PSPubServerServiceException,
          ...
          IPSItemService.PSItemServiceException, PSNotFoundException;
  /**
   * Returns the list of PSPublishingActionProperties available for the logged in user for the
   * supplied item.
   *
   ...
   */
  PSPagedItemList getQueuedIncrementalContent(
      String siteName, String serverName, int startIndex, int pageSize)
      throws PSSitePublishException;

  PSPagedItemList getQueuedIncrementalRelatedContent(
      String siteName, String serverName, int startIndex, int pageSize)
      throws PSSitePublishException;

  /** PSPublishException is thrown when an error occurs attempting to publish a site. */
