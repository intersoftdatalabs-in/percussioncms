...
public class PSPreviewItemContent extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if (requestUri.endsWith("favicon.ico")) return;
  ...
  private String createAssemblyUrl(String path, String revision, String renderType)
      throws PSDataServiceException, PSNotFoundException, PSCmsException,
    ...
    if (id == null) throw new PSNotFoundException(path);
    ...
    else renderLink = linkService.renderPreviewResourceLink(linkRequest);
  ...
  private IPSGuid getItemId(String path, String revision)
    ...
    if (id == -1) throw new PSNotFoundException("Cannot find item with path = \"" + path + "\".");

    return new PSLegacyGuid(id, revisionId);
  }

  /**
   * Manual decode the '+' character as the browser may not do it correctly, which will cause the
   * <code>URLDecoder.decode</code> method to mistakenly replace it with a whitespace. Tested in the
   * following browsers:
   *
   * <ul>
