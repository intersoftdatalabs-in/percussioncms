...
public class PSSiteSectionService implements IPSSiteSectionService {
  ...
  public PSSiteSection createExternalLinkSection(PSCreateExternalLinkSection req)
      throws PSValidationException, IPSPathService.PSPathNotFoundServiceException,
  ...
  public void generateSecurityConfigurationFiles(IPSSite site)
      throws DataServiceSaveException, PSSiteSectionException,
    ...
    if (section.getSectionType() == PSSectionTypeEnum.section) {
      ...
      for (String id : section.getChildIds()) {
        try {
          ...
          log.warn(
              "Skipping navigation section for Navigation node {}.  Unable to load section with error: {}",
              id,
              e.getMessage());
        }
      }
  ...
  public class PSUpdateSectionValidator extends PSAbstractBeanValidator<PSSiteSectionProperties> {
    /**
     * * If a missing nav landing page is detected - see if the default exists and add it back if it
     * does.
     *
     * @param navonId
     * @return
    ...
    protected void doValidation(PSCreateSiteSection req, PSBeanValidationException e)
      ...
      if (template == null) {
        ...
        if (sectionType == PSSectionTypeEnum.blog) {
          msg =
              "Cannot create a blog with the name \"{0}\" because the selected blog index template has "
