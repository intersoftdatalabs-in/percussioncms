...
public class PSSiteSectionService implements IPSSiteSectionService {
  ...
  @Autowired
  public PSSiteSectionService(
      IPSPageDao pageDao,
      IPSManagedNavService navService,
      IPSContentWs contentSrv,
      IPSContentDesignWs contentDsSrv,
      IPSSiteManager siteMgr,
      IPSIdMapper idMapper,
      @Qualifier("renderAssemblyBridge") IPSRenderAssemblyBridge asmBridge,
      IPSFolderHelper folderHelper,
      IPSWorkflowHelper workflowHelper,
      IPSPublishingWs publishingWs,
      @Qualifier("sys_templateService") IPSTemplateService templateSrv,
      IPSiteDao siteDao,
      @Qualifier("siteTemplateService") IPSSiteTemplateService siteTemplateSrv,
      IPSCommentsService commentsService,
    ...
    jcrNodeFinder = new PSJcrNodeFinder(contentMgr, IPSPageService.PAGE_CONTENT_TYPE, "sys_title");
  }
  ...
  public PSSiteSection create(PSCreateSiteSection req) throws PSDataServiceException {
    ...
    IPSGuid navonId =
        navSrv.addNavonToFolder(
            parentId, guid, folder.getName() + "-Navon", req.getPageLinkTitle(), workflowId);
  ...
  public PSSiteSection createExternalLinkSection(PSCreateExternalLinkSection req)
    ...
    String esFolderName =
        folderHelper.getUniqueFolderName(
            req.getFolderPath(), EXTERNAL_SECTION_NAME_PREFIX + req.getLinkTitle());
    ...
    IPSGuid navonId =
        navSrv.addNavonToFolder(
            parentId,
            guid,
            folder.getName() + "-Navon",
            req.getLinkTitle(),
            pageDaoHelper.getWorkflowIdForPath(createdFolderPath));

    updateExternalLinkSection(
        req.getLinkTitle(),
        idMapper.getString(navonId),
        req.getExternalUrl(),
  ...
  public PSSiteSection createSectionLink(String targetSectionGuid, String parentSectionGuid)
    ...
    if (navSrv.isNavTree(targetGuid)) {
      throw new PSSiteSectionException(
          "Cannot create a section link that points to the home page.");
    }
  ...
  public PSSiteSection createSectionFromFolder(PSCreateSectionFromFolderRequest req)
    ...
    IPSGuid navonId =
        navSrv.addNavonToFolder(
            parentId, childId, folderName + "-Navon", landingPage.getLinkTitle(), workflowId);
    ...
    if (!StringUtils.equals(pageName, filename)) {
      ...
      if (indexPageId != null) {
        ...
        contentSrv.checkinItems(
            Collections.singletonList(idMapper.getGuid(indexPage.getId())), null);
      }
    ...
    if (workflowHelper.isItemInApproveState(idMapper.getContentId(landingPageId))
        && !workflowHelper.isItemInApproveState(idMapper.getContentId(navonId))) {
      try {
  ...
  public PSNoContent deleteSectionLink(String sectionGuid, String parentSectionGuid) {
    ...
    navSrv.deleteNavonRelationship(
        idMapper.getGuid(sectionGuid), idMapper.getGuid(parentSectionGuid));
  ...
  public PSSiteSection updateExternalLink(String sectionGuid, PSCreateExternalLinkSection req)
    ...
    updateExternalLinkSection(
        ...
        req.getExternalUrl(),
        req.getTarget(),
        req.getCssClassNames());
    Set<String> idSet = new HashSet<>();
    idSet.add(sectionGuid);
    workflowHelper.transitionToPending(idSet);
    PSSiteSection section =
        loadSiteSection(idMapper.getGuid(sectionGuid), null, null, true, false, null);
    return section;
  }
  ...
  public PSSiteSection updateSectionLink(PSUpdateSectionLink req) throws PSSiteSectionException {
    ...
    if (navSrv.isNavTree(idMapper.getGuid(req.getNewSectionId()))) {
      throw new PSSiteSectionException(
          "Cannot update a section link that points to the home page.");
    }

    navSrv.replaceNavon(
        idMapper.getGuid(oldSectionId),
        idMapper.getGuid(req.getNewSectionId()),
        idMapper.getGuid(req.getParentSectionId()));
    PSSiteSection section =
        loadSiteSection(
            idMapper.getGuid(req.getNewSectionId()),
            idMapper.getGuid(req.getParentSectionId()),
  ...
  public List<PSSiteBlogProperties> getBlogsForSite(String siteName) throws PSValidationException {
    ...
    try {
      ...
      log.error(
          "Failed to load the root section for the site:{} Error:{}",
          siteName,
   ...
   */
  @Override
  public List<String> findAllTemplatesUsedByBlogs(String siteName)
      throws PSDataServiceException, com.percussion.services.error.PSNotFoundException {
  ...
  public PSSiteBlogPosts getBlogPosts(String id)
      throws PSValidationException, PSSiteSectionException {
    notEmpty(id);
      ...
      if (blogPostTemplateId != null) {
        ...
        for (IPSNode postPage : postPages) {
          PSComponentSummary summary =
              getItemSummary(((PSLegacyGuid) postPage.getGuid()).getContentId());
          ...
          try {
            ...
            log.error(
                "Error setting properties for blog post with ID: {} {}",
                postProps.getId(),
                PSExceptionUtils.getMessageForLog(e));
            log.debug(PSExceptionUtils.getDebugMessageForLog(e));
          }
          ...
          try {
            ...
            log.error(
                "Error retrieving folder path for id: {}, Error: {}",
                postPage.getGuid(),
                PSExceptionUtils.getMessageForLog(e));
            log.debug(PSExceptionUtils.getDebugMessageForLog(e));
          }
          ...
          String postDate =
              PSDateUtils.getDateToString(
                  (summary.getContentPostDate() == null) ? null : summary.getContentPostDate());
    ...
    Collections.sort(
        posts,
        new Comparator<PSItemProperties>() {
   ...
   * @return The map, key is the page finder path, value is the summary, never <code>null</code>,
   *     may be empty.
   */
  ...
  public String getBlogPostTemplateId(String path) {
    ...
    try {
      ...
      log.error(
          "Failed to load the folder for the path {}, Error: {}",
          path,
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    }
  ...
  private List<PSSiteBlogProperties> getBlogsForSingleSite(String siteName)
    ...
    List<PSFolder> allBlogs = contentSrv.getFoldersByProperty("blogPostTemplate");

    // Only return blogs for the specified site
    List<PSFolder> filteredBlogs = new ArrayList<>();
    for (PSFolder f : allBlogs) {
      List<IPSSite> sites = this.folderHelper.getItemSites(f.getGuid().toString());
      for (IPSSite s : sites) {
    ...
    for (PSFolder folder : filteredBlogs) {
      ...
      String blogPostTemplateId = getBlogPostTemplateId(folder.getFolderPath());
      if (blogPostTemplateId != null) {
        blogProperties = new PSSiteBlogProperties();
        IPSGuid sectionId = navSrv.findNavigationIdFromFolder(folder.getGuid());
        ...
        if (landingPageId == null) {
          log.warn(
              "Cannot find landing page for blog section with template id: {}", blogPostTemplateId);
          continue;
        }
        PSPage landingPage = pageDao.find(idMapper.getString(landingPageId));
        String blogPath = landingPage.getFolderPath();
        String landingPagePath = folderHelper.concatPath(blogPath, landingPage.getName());
        ...
        try {
          PSItemProperties props = folderHelper.findItemProperties(landingPagePath);
          blogProperties.setLastPublishDate(props.getLastPublishedDate());
        } catch (Exception e) {
          log.error(
              "Error getting properties for [{}], Error: {}",
              landingPagePath,
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        }
  ...
  public void updateSectionBlogTemplates(String siteName, Map<String, String> tempMap)
    ...
    for (PSSiteSection siteSection : allSections) {
      ...
      try {
        folders = contentSrv.loadFolders(new String[] {siteSection.getFolderPath()});
      } catch (PSErrorResultsException e) {
        log.error(
            "Failed to load the folder for the path {}, Error: {}",
      ...
      if (folders != null && !folders.isEmpty()) {
        ...
        try {
          ...
          log.error(
              "Cannot update blog templates on folder {} post template id={} indexTemplateid = {}",
    ...
    for (var siteSection : allSections) {
      try {
        ...
        if (!folders.isEmpty()) {
          ...
          if (StringUtils.isNotBlank(blogIndexTemplateId)) blogTemplateIds.add(blogIndexTemplateId);
          if (StringUtils.isNotBlank(blogPostTemplateId)) blogTemplateIds.add(blogPostTemplateId);
        }
      } catch (PSErrorResultsException e) {
  ...
  private class ReplaceLandingPageHelper {
    ...
    /**
     * Prepare all items (landing pages and navigation item) before modify them. Must call {@link
     * #releaseFromEdit()} after finishing editing the items. The workflow state and the revision of
     * the items may be changed after this call.
     */
    ...
    /**
     * Prepare for editing the specified item. This will throw validation error if failed to prepare
     * the item for edit.
     *
    ...
    /**
     * Checks in all modified items (landing pages and navigation item), a matching call to {@link
     * #prepareForEdit()}.
     *
     * <p>Note, this does not change workflow state of the items, only checks in items.
     *
     * <p>After {@link #prepareForEdit()} and {@link #releaseFromEdit()}, the workflow state of the
     * items may be changed. For example, if an item was in PENDING or LIVE, then it be in
     * QUICK-EDIT afterwards.
     */
    private void releaseFromEdit() {
      if (oldPageStatus != null) contentSrv.releaseFromEdit(oldPageStatus, true);
      if (newPageStatus != null) contentSrv.releaseFromEdit(newPageStatus, true);
      if (navNodeStatus != null) contentSrv.releaseFromEdit(navNodeStatus, true);
    }
     ...
     *
     * @param pageId the ID of the landing page, assumed not <code>null</code> .
     * @param newName the new name of the landing page, assumed not blank.
     * @param newLinkTitle the new link text of the landing page, assumed not blank.
     */
    ...
    /**
     * Moves the specified landing page into the same folder as the navigation node if needed, and
     * set name and link-text to the specified landing page.
     *
     * @param pageId the specified landing page, assumed not <code>null</code> .
     * @param navigationNodeId the ID of the specified navigation node, assumed not <code>null
     *     </code>.
     * @param newName the new name of the landing page, assumed not blank.
     * @param newLinkTitle the new link text of the landing page, assumed not blank.
     */
    private void updateLandingPage(
        IPSGuid pageId, IPSGuid navigationNodeId, String newName, String newLinkTitle)
        throws PSDataServiceException {
  ...
  private void moveSection(PSMoveSiteSection req)
    ...
    // ensure Admin perms
    folderHelper.hasFolderPermission(
        folderHelper.getParentFolderId(sourceParentId).toString(), Access.ADMIN);
    folderHelper.hasFolderPermission(
        folderHelper.getParentFolderId(sourceId).toString(), Access.ADMIN);
    folderHelper.hasFolderPermission(
        folderHelper.getParentFolderId(targetId).toString(), Access.ADMIN);
    navSrv.moveNavon(sourceId, sourceParentId, targetId, req.getTargetIndex());
  }

  /**
   * Validates the section for move operation, make sure the section is only under one site It is
   * invalid if the section is under more than 1 site.
   *
  ...
  private void validateMoveSection(String sourceId) throws PSValidationException {
    ...
    if ((sites != null) && (!sites.isEmpty())) {
      PSSiteCopyUtils.throwCopySiteMessageIfNotAllowed(
          sites.get(0).getName(), "move", PSSiteCopyUtils.CAN_NOT_MOVE_SECTION);
    }
  }
  ...
  private void moveSectionLink(PSMoveSiteSection req)
    ...
    if (!targetGuid.equals(parentGuid)) {
      List<IPSGuid> targetChildNavonIds =
          navSrv.findChildNavonIds(contentDsSrv.getItemGuid(targetGuid));
      for (IPSGuid ipsGuid : targetChildNavonIds) {

        if (ipsGuid.longValue() == sectionGuid.longValue())
          throw new PSSiteSectionException(
              "Section and a link to it or duplicate section links at the same level are not"
                  + " allowed");
      }
    }
   ...
   * @param props the {@link PSSiteSectionProperties} object to add the site properties. Assumed not
   *     <code>null</code>.
   */
  private void addSectionSecurityProperties(
      PSSiteSection psSiteSection, PSSiteSectionProperties props) {
    // Add site properties
  ...
  public PSSiteSection update(PSSiteSectionProperties req) throws PSDataServiceException {
    ...
    if ((sites != null) && (sites.size() >= 1)) {
      PSSiteCopyUtils.throwCopySiteMessageIfNotAllowed(
          sites.get(0).getName(), "update", PSSiteCopyUtils.CAN_NOT_EDIT_SECTION);
    }
  ...
  private void removeSiteTouchedFile(String sitename) throws DataServiceSaveException {
    try {
      ...
      String errorMsg =
          "Failed removing the tch file for site '"
              + sitename
  ...
  public void generateSecurityConfigurationFiles(IPSSite site)
    ...
    try {
      PSSiteConfigUtils.updateSecureSectionsConfiguration(
          site.getName(), site.getLoginPage(), tree);
    } catch (Exception e) {
      String errorMsg =
          "Failed updating the configuration properties for site '"
              + site.getName()
  ...
  /**
   * Clears the properties for security in the given node, and recursively process all its child
   * nodes.
   *
   * @param childNode {@link PSSectionNode} node object. Assumed not <code>null</code>
   */
   ...
   *
   * @param sectionTree {@link PSSectionNode} node object. Assumed not <code>null</code>.
   */
   ...
   * @param linkTitle the new link title of the landing page, assumed not blank.
   * @return the starting state of the landing page, which may be transitioned into different state
   *     after this call.
   */
      ...
      for (String cId : section.getChildIds()) {
        ...
        try {
          child =
              loadSiteSection(
                  childId,
                  idMapper.getGuid(section.getId()),
                  null,
                  true,
                  true,
   ...
   * @param folderPath the folder path of the section. It may be <code>null</code> if unknown.
   * @param resolvePath <code>true</code> if need to resolve or set the folder path for the returned
   *     section object.
   * @param resolveDisplayPath
   * @return the specified site section, never <code>null</code>.
   */
  private PSSiteSection loadSiteSection(
      IPSGuid id,
      IPSGuid parentGuid,
      String folderPath,
      boolean resolvePath,
      boolean resolveDisplayPath,
   ...
   * @return the section node and its descendant nodes, never <code>null</code> .
   */
      ...
      for (String id : section.getChildIds()) {
        try {
          PSSiteSection cSection =
              loadSiteSection(idMapper.getGuid(id), sectionGuid, null, true, true, displayPath);
          ...
          log.warn(
              "Skipping navigation section for Navigation node {}.  Unable to load section with"
                  + " error: {}",
              id,
              e.getMessage());
        }
  ...
  /**
   * Resets the revision of the specified ID to <code>-1</code> if it is not. This is used to keep
   * the ID consistent for all the parent and child nodes otherwise the revision of the child IDs
   * will be <code>-1</code>, but it may be a different values for the parent nodes
   *
  ...
  /**
   * Loads all sections for the provided site it provided, not only immediate child sections but as
   * many level as it is.
   *
   * @param siteName
   */
   ...
   */
  private List<PSSiteSection> getAllDescendantSections(
      List<PSSiteSection> sections, PSSiteSection section) {
   ...
   * @param parentPath the parent folder path that contains the created folder, assumed not blank.
   * @param templateId id of the template used for new blog posts, will be <code>null</code> for
   *     regular sections. This will be stored as a property on the folder.
  ...
  private PSFolder createFolder(String name, String parentPath, String templateId)
    ...
    if (contentSrv.isChildExistInFolder(parentPath, name)) {
      throw new PSSiteSectionException(
          "Cannot create a folder (name=" + name + ") under parent folder: " + parentPath);
    }
    ...
    try {
      ...
      log.error(
          "Failed to create a folder name={}, under folder-path=\"{}\" Error: {}",
          name,
          parentPath,
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
   ...
   *
   * @param req the request for creating the landing page, assumed not <code>null</code>.
   * @param folderPath the folder path that contains the created landing page, not blank.
   * @param navonId the ID of the navigation node that will link to the landing page, assumed not
   *     <code>null</code>.
  ...
  public void convertToFolder(String id) throws PSValidationException {
    ...
    try {
      ...
      log.error(
          "Failed to load section for the supplied id: {}",
          id,
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw new RuntimeException(
          "Failed to find the selected section, it might have been deleted in another session.", e);
    }
    ...
    if (parentFolderIds.size() > 0) contentSrv.deleteFolders(parentFolderIds, true);
  ...
  /**
   * Groups the child sections by type. Recursively calls the function to load the children and
   * group them
   *
   ...
   * @param sectionLinks map of section links, first argument is guid of the section and second
   *     argument is parent guid, assumed not <code>null</code>
   * @param sections list of section guids, assumed not <code>null</code>
   */
  private void groupChildSectionsByType(
      PSSiteSection parentSection,
      List<String> externalLinkSections,
      Map<String, String> sectionLinks,
      List<String> sections) {
    List<PSSiteSection> childSections = loadChildSections(parentSection);
    if (childSections.size() < 1) return;
    for (PSSiteSection section : childSections) {
      ...
      else sections.add(section.getId());
      groupChildSectionsByType(section, externalLinkSections, sectionLinks, sections);
    }
  }
  ...
  public List<PSSiteSection> findAll()
  ...
  /**
   * Renames the specified landing page to original(n), where n is > 0 such that the name is unique
   * within the section folder. A landing page named "index" will become "index(1), index(2), etc."
  ...
  /**
   * Creates and saves a template based on the specified blog name and source template. The name of
   * the template will use the following convention: {blog name} - {source template name} - (n). The
   * increment will be added if necessary to ensure a unique name under the specified site. It will
   * start at 2.
   *
  ...
  public class PSUpdateSectionValidator extends PSAbstractBeanValidator<PSSiteSectionProperties> {
    ...
    protected void doValidation(PSSiteSectionProperties req, PSBeanValidationException e)
      ...
      List<PSItemSummary> sums =
          contentSrv.findItems(Collections.singletonList(sectionFolderId), false);
      ...
      if (landingPageId == null) {
        log.warn(
            "No Landing Page detected for Section {} attempting to auto-detect Landing Page.",
            newFolderName);
        landingPageId = fixMissingLandingPage(navonId);
      }
      // If we weren't able to auto detect / repair the landing page log an error but let the save
      // proceed.
      if (landingPageId == null) {
        log.error(
            "Unable to locate the Landing Page for section {} {}",
            req.getId(),
            req.getFolderName());
      } else {
        ...
        if (isCheckedOut && !PSWebserviceUtils.isItemCheckedOutToUser(summary)) {
          e.reject(
              ...
              new Object[] {oldFolderName, summary.getCheckoutUserName()},
              "Cannot modify the section \"{0}\" "
        ...
        String[] itemPaths =
            contentSrv.findItemPaths(folderHelper.getParentFolderId(sectionFolderId));

        // see if something exists with same name
        IPSGuid existingId = contentSrv.getIdByPath(itemPaths[0] + '/' + newFolderName);
        if (existingId != null) {
          e.reject(
              "updateSiteSection.nameAlreadyExists",
              new Object[] {oldFolderName, newFolderName},
              "Cannot rename site section \"{0}\" to \"{1}\" "
  ...
  private PSAbstractBeanValidator<PSSiteSectionProperties> updateRequestValidator =
      new PSUpdateSectionValidator();
  ...
  public class PSMoveSectionValidator extends PSAbstractBeanValidator<PSMoveSiteSection> {
    @Override
    protected void doValidation(PSMoveSiteSection req, PSBeanValidationException e) {}
  }

  /** The validator used to validate a request of moving a site section. */
  private PSAbstractBeanValidator<PSMoveSiteSection> moveRequestValidator =
      new PSMoveSectionValidator();
  ...
  public class PSCreateSectionValidator extends PSAbstractBeanValidator<PSCreateSiteSection> {
    ...
    protected void doValidation(PSCreateSiteSection req, PSBeanValidationException e)
      ...
      IPSGuid existingId =
          contentSrv.getIdByPath(req.getFolderPath() + '/' + req.getPageUrlIdentifier());
      if (existingId != null) {
        e.reject(
            "createSiteSection.alreadyExists",
            new Object[] {sectionName},
            "Cannot create a section with the name \"{0}\" "
      ...
      if (template == null) {
        ...
        if (sectionType == PSSectionTypeEnum.blog) {
          msg =
              "Cannot create a blog with the name \"{0}\" because the selected blog index template"
                  + " has  been deleted.  Please select a different blog index template.";
        } else {
          msg =
              "Cannot create a section with the name \"{0}\" because the selected template has been"
      ...
      if (sectionType == PSSectionTypeEnum.blog) {
        ...
        if (template == null) {
          e.reject(
              "createSiteSection.missingTemplate",
              new Object[] {sectionName},
              "Cannot create a blog with the name \"{0}\" because the selected blog post template"
                  + " has been deleted.  Please select a different blog post template.");
        }
      }
    }
  }
   ...
   * @author YuBingChen
   */
  public class PSCreateExternalLinkSectionValidator
      extends PSAbstractBeanValidator<PSCreateExternalLinkSection> {
    @Override
    protected void doValidation(PSCreateExternalLinkSection req, PSBeanValidationException e) {
      ...
      if (existingId == null) {
        ...
        e.reject(
            "createExternalSection.invalidParentSection",
            new Object[] {sectionName},
            "Cannot create a section with the name \"{0}\" "
  ...
  /** Validating the request of creating an external site section. */
  public class PSCreateSectionFromFolderValidator
      extends PSAbstractBeanValidator<PSCreateSectionFromFolderRequest> {
    @Override
    protected void doValidation(PSCreateSectionFromFolderRequest req, PSBeanValidationException e) {
      ...
      try {
        ...
        if (!itemSum.getCategory().equals(Category.FOLDER)) {
          e.reject(
              "createSectionFromFolder.invalidFolderPath",
      ...
      if (StringUtils.isNotBlank(req.getPageName()) && pageId == null) {
        e.reject(
            "createSectionFromFolder.invalidPageName",
            new Object[] {req.getSourceFolderPath(), req.getPageName()},
            "Cannot create a section from the folder with path \"{0}\" and a page named \"{1}\""
                + " because a page with that name cannot be found in that folder.");
      }
    }
  }

  private void updateExternalLinkSection(
      String title, String navonId, String url, PSSectionTargetEnum target, String cssClassNames) {
    notNull(navonId);
    String tgt = target == null || target == PSSectionTargetEnum._self ? "" : target.name();
    Map<String, String> map = new HashMap<>();
  ...
  private PSAbstractBeanValidator<PSCreateSiteSection> createRequestValidator =
      new PSCreateSectionValidator();

  /** The validator used to validate a request of creating an external site section. */
  private PSAbstractBeanValidator<PSCreateExternalLinkSection> createExternalLinkRequestValidator =
      new PSCreateExternalLinkSectionValidator();
