...
public interface IPSImportHelper {
  ...
  /**
   * Call this method to undo all the operations done by the helper in its process method. If any
   * unexpected error occurs processing pageContent from a mandatory helper, then rollback is called
   * to undo all the operations done by process method.
   *
  ...
  /**
   * This method gets import status message from the helper implementing this interface. This
   * message will be displayed to the user in import dialog, to show how steps in the import process
   * are being executed and the overall progress made.
   *
   * @param statusMessagePrefix A prefix to attach to the beginning of the status message. For
   *     example: "Importing site:", "Importing template:".
