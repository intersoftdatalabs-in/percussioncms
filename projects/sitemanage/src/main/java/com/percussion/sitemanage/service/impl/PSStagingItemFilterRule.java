...
/**
 * This is a staging filter rule that extends public asset item filter rule. This rule while getting
 * the workflow items passes true for staging.
 */
public class PSStagingItemFilterRule extends PSPublicAssetItemFilterRule
  ...
  public List<IPSFilterItem> filter(List<IPSFilterItem> items, Map<String, String> params) {
    ...
    IPSContentChangeService contentChangeService =
        PSContentChangeServiceLocator.getContentChangeService();
      ...
      if (contentChangeService != null) {
        changedIds =
            contentChangeService.getChangedContent(
                pubServer.getSiteId(), PSContentChangeType.PENDING_STAGED);
