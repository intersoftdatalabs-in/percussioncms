...
/**
 * This filter is used for publish and unpublish items. The default behavior is filtering items
 * publishing. It will remove assets that are not in the correct workflow states (public workflow
 * states) and will correct the revisions of non-local assets. The revisions of local assets will
 * not be altered as these always come from slot finders which return the correct revision that we
 * want.
 *
 * <p>However, if the parameter contains <code>sys_publish=unpublish</code>, then it removes items
 * that have public revision (or is publishable).
 *
 * @author adamgent
 */
public class PSPublicAssetItemFilterRule extends PSAbstractWorkflowExtension
   ...
   * @param serverId
   * @param changedIdsSet
   * @return <code>null</code> indicates that the inputted item should be removed from the original
   *     list of items.
   */
  protected IPSFilterItem process(
    ...
    if (wfItem.assetType == AssetType.LOCAL) {
      ...
      Map<IPSGuid, List<PSContentStatusHistory>> auditTrails =
          systemws.loadAuditTrails(Collections.singletonList(original.getItemId()));
      ...
      for (PSContentStatusHistory historyItem : historyList) {
        if (historyItem.getStateName().equals(LIVE_STATE)) {
          ...
          log.debug(
              "Publishing previous Item Revision: "
                  + oldRevision
                  + " Name:"
      ...
      if (log.isDebugEnabled())
        log.debug(
            "Keeping original page or shared asset "
                + oldRevision
                + " due to scheduling: "
                + wfItem);

      IPSGuid newGuid = worker.makeGuidFromRevision(original.getItemId(), oldRevision);
      if (original.getItemId().equals(newGuid)) rvalue = original;
      else rvalue = original.clone(newGuid);
  ...
  protected PSPubServer findPubServer(String editionId) {
    ...
    try {
      ...
      log.info(
          "Error occurred while finding the status of ignore assets property, setting the value as false",
          e);
    }
    return pubServer;
  }
  ...
  /**
   * Determine if the item is scheduled to be published. This means that it has a start date that is
   * equivalent to the next aging date, and that both dates are in the future.
   *
