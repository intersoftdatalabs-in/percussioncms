...
public final class PSLinkExtractor {
  ...
  public static final String QUERY_STRING_LINK_TEXT_TOKEN =
      "{{{{{{{PERCUSSION|QUERY|STRING|TOKEN}}}}}}}";
  ...
  public static List<PSLink> getLinksForDocument(
      final Document doc, final IPSSiteImportLogger log, PSSiteQueue siteQueue, String siteUrl) {
        ...
        if (siteQueue != null && siteQueue.hasLinkBeenProcessed(absHref)) {
          ...
          try {
            ...
            if (absHref.equals(getRoot(doc.baseUri())) && !absHref.isEmpty()) {
              psLink =
                  createLink(
                      ...
                      absHref,
                      aHref,
                      PSSiteContentDao.HOME_PAGE_NAME,
            ...
            log.appendLogMessage(
                PSLogEntryType.ERROR, "Link Extractor", absHref + " could not be retrieved.");
            log.appendLogMessage(
                PSLogEntryType.STATUS,
                "Link Extractor",
                absHref
                    + " could not be retrieved due to the following error: "
   ...
   */
  public static List<PSLink> getImagesForDocument(
      final Document doc, final IPSSiteImportLogger log) {
    final ArrayList<PSLink> outList = new ArrayList<>();
  ...
  protected static PSLink createLink(
      Element link,
    ...
    try {
      return PSLink.createLink(
          PSReplacementFilter.filter(relativePath),
          URLDecoder.decode(getLinkText(aHref, link), "UTF-8"),
          absHref,
          PSReplacementFilter.filter(pageName),
          link);
  ...
  protected static String getLinkText(final String absHref, Element link) {
    ...
    if (absHref != null
        && !absHref.isEmpty()
        && removeTrailingSlash(absHref).equals(getRoot(absHref))) {
      return "Home";
    }
  ...
  protected static String getRelativePath(
      final String absHref, String aHref, final IPSSiteImportLogger log) {
  ...
  protected static String getPageName(final String absHref, final IPSSiteImportLogger log) {
    ...
    if (!(cleanAbsHref != null
        && !cleanAbsHref.isEmpty()
        && removeTrailingSlash(cleanAbsHref).equals(getRoot(cleanAbsHref)))) {
      ...
      if (log != null) {
        log.appendLogMessage(
            ...
            "Changed Page Name: "
                + endPartString
                + " to "
                + PSReplacementFilter.filter(endPartString));
      }
    }
