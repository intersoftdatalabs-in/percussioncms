...
public class PSSiteMapGeneratorTask implements IPSEditionTask {
   ...
   * @throws PSExtensionException if the task fails for any reason an exception should be thrown
   *     that details the reason for the failure. The exception will be caught by the job code and
   *     recorded as part of the edition task.
   */
  @Override
  public void perform(
      IPSEdition edition,
    ...
    if (site.isGenerateSitemap()) {
      ...
      for (var s : status.getIterableJobStatus()) {
        boolean skipItem =
            s.getLocation().startsWith("/Assets")
                && (excludeImage != null && excludeImage.equals("true"));
        ...
        if (!skipItem) {
          if (s.getStatus().equals(IPSSiteItem.Status.SUCCESS)
            ...
            if (isPriorVersionLiveV2(s) && s.getOperation().equals(IPSSiteItem.Operation.PUBLISH)) {
              // A previous version was live - so even though this one failed we should include it
              // in the sitemap
              addToSiteMap(wsg, site, s);
              count++;
            }
