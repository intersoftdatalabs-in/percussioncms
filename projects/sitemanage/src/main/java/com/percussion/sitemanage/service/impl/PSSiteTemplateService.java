...
public class PSSiteTemplateService implements IPSSiteTemplateService {
  ...
  public List<PSSiteSummary> findSitesByTemplate(@PathParam("id") String templateId) {
    try {
      ...
      for (PSSiteSummary site : sites) {
        if (paths.contains(site.getFolderPath())) rvalue.add(site);
      }
  ...
  public List<PSTemplateSummary> findTemplatesBySite(@PathParam("id") String siteId) {
    try {
      ...
      return new PSTemplateSummaryList(findTemplates(siteId, null, null));
        ...
        | IPSTemplateService.PSTemplateException
        | IPSDataService.DataServiceNotFoundException e) {
      throw new WebApplicationException(e.getMessage());
    }
  }
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public List<PSTemplateSummary> findTemplatesBySite(
      @PathParam("id") String siteId, @PathParam("widgetId") String widgetId) {
    try {
      rejectIfBlank("findTemplatesBySite", "siteId", siteId);
  ...
  public List<PSTemplateSummary> findTypedTemplatesBySite(String siteId, PSTemplateTypeEnum type)
      throws PSValidationException, IPSTemplateService.PSTemplateException,
  ...
  /**
   * Finds templates for the specified site which have at least one instance of the specified
   * widget.
   *
   * @param siteId The id of a site. This <strong>DOES NOT</strong> have to be a valid id of an
   *     existing site.
   * @param widgetId this is the widget definition id. May be <code>null</code> to return all
   ...
   */
  private List<PSTemplateSummary> findTemplates(
      String siteId, String widgetId, PSTemplateTypeEnum type)
      throws IPSTemplateService.PSTemplateException, IPSDataService.DataServiceNotFoundException {
  ...
  private List<PSTemplateSummary> itemsToTemplates(
      List<IPSItemSummary> items, String siteName, PSTemplateTypeEnum type)
      throws IPSTemplateService.PSTemplateException {
    List<String> templateIds = new ArrayList<>();
    for (IPSItemSummary i : items) {
      templateIds.add(i.getId());
    }
    List<PSTemplateSummary> templateSummaries =
        templateService.loadUserTemplateSummaries(templateIds, siteName);
    ...
    try {
      ...
      for (CreateTemplate createTemplate : siteTemplates.getCreateTemplates()) {
        ...
        if (StringUtils.containsAny(
            createTemplateName, SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS)) {
          for (int i = 0; i < SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS.length(); i++) {
            ...
            createTemplateName =
                StringUtils.replace(
                    createTemplateName,
                    String.valueOf(SecureStringUtils.INVALID_ITEM_NAME_CHARACTERS.charAt(i)),
                    "-");
          }
        }
        ...
        String templateFolderPathForSite =
            folderHelper.concatPath(site.getFolderRoot(), SECTION_SYSTEM_FOLDER_NAME, TEMPLATES);
        // find unique name for copied template.
        createTemplateName =
            folderHelper.getUniqueNameInFolder(
                templateFolderPathForSite, createTemplateName, "", 2, false);
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSTemplateSummary createTemplateFromUrl(
      @Context HttpServletRequest request, PSSiteTemplates siteTemplates) {
    try {
      ...
      if (site == null) {
        throw new WebApplicationException(
            "There was an unexpected error retrieving the selected site." + siteName);
      }
  ...
  public PSTemplateSummary createTemplateFromPage(PSPageToTemplatePair pageToTemplatePair) {
    ...
    try {
      templateSummary =
          buildTemplateFromPage(pageToTemplatePair.getPageId(), pageToTemplatePair.getSiteId());
    } catch (Exception e) {
      throw new WebApplicationException(
          "There was an unexpected error creating the new template from the page.");
    }
    ...
    try {
      ...
      templateSummary =
          templateService.createTemplate(
              this.generateNewTemplateName(PSPageManagementUtils.TEMPLATE_NAME, siteId),
              page.getTemplateId(),
              siteId);
      ...
      IPSPageImportQueue importQueue =
          (IPSPageImportQueue) getWebApplicationContext().getBean("pageImportQueue");
  ...
  private PSPage getPageForPageId(String pageId)
      throws PSPageToTemplateException, IPSDataService.DataServiceLoadException,
          PSValidationException, IPSDataService.DataServiceNotFoundException {
    PSPage page = null;
    if (pageId != null && !pageId.isEmpty()) page = pageService.find(pageId);
    else throw new PSPageToTemplateException("Page ID can not be null");
    return page;
  }
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public Long createTemplateFromUrlAsync(
      @Context HttpServletRequest request, PSSiteTemplates siteTemplates) {
    try {
      // Validation of required parameters
      notNull(siteTemplates);
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSTemplateSummary getImportedTemplate(@PathParam("jobId") Long jobId)
  ...
  /**
   * Finds a template by its id using templateService. If any runtime exception occurs, or if the
   * template is not found, it will return null.
   *
   * @param templateId The id of the template to retrieve.
   * @return The template summary. Can be null if not found or an exception occurred.
   */
  ...
  /**
   * Finds a site by its name, which is also its id. using siteDao. If any runtime exception occurs,
   * or if the site is not found, it will return null.
   *
   * @param siteName The id(name) of the site to retrieve.
   * @return PSSite object, with site information. Can be null if not found or an exception
   *     occurred.
   */
  ...
  private PSTemplateSummary createSiteTemplate(CreateTemplate ct) throws PSDataServiceException {
    if (ct.getSiteIds().size() != 1) {
      throw new IllegalArgumentException(
          "Can only create template for a specific site, no more or less.");
    }
  ...
  protected PSTemplateSummary assignTemplate(AssignTemplate assignTemplate)
    ...
    if (isNotBlank(newName) && !oldName.equals(newName)) {
      ...
      if (siteIds.size() == 1) templateService.save(fullTemplate, siteIds.get(0));
      else templateService.save(fullTemplate);
    }
    PSTemplateSummary templateSummary = templateService.find(templateId);
    if (!newSites.isEmpty()) {
      templateSummary.setImageThumbPath(
          templateService.getTemplateThumbPath(templateSummary, newSites.get(0).getName()));
    }
    return templateSummary;
  }
    ...
    if (site2 != null) {
      ...
      for (PSTemplateSummary site1Template : findTemplatesBySite(site1Id)) {
        ...
        if (!site2TempSet.contains(s1TempName)) {
          PSTemplateSummary site2Template =
              templateService.createTemplate(s1TempName, site1Template.getId(), site2Id);
      ...
      if (!StringUtils.isBlank(templateId)) {
        ...
        if (!site2TempSet.contains(s1TempName)) {
          PSTemplateSummary site2Template =
              templateService.createTemplate(s1TempName, templateId, site2Id);
  ...
  public PSValidationErrors validate(PSSiteTemplates siteTemplates)
  ...
  @Autowired
  public void setTemplateImportService(
      @Qualifier("templateImportService") IPSSiteImportService templateImportService) {
  ...
  /**
   * Generates a name for a new template, using site-wide naming conventions. It avoids collision
   * with existing templates appending a numerical value to the end if necessary (Example:
   * New-Template-2)
   *
   * @param templateName Initial name. If it doesn't exist, it will be returned unchanged. Otherwise
   *     a number will be appended to it, to avoid name collision.
   * @param siteId The id of the site to which this template will belong. If the site already has a
   *     template with the same name, a numerical value will be appended to the end of the generated
   *     name to avoid collisions.
   * @return String The automatically generated name for the template. Never null or empty.
   */
