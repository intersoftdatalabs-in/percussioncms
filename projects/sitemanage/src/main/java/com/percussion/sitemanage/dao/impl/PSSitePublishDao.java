...
public class PSSitePublishDao implements com.percussion.sitemanage.dao.IPSSitePublishDao {
  ...
  /** The publishing ws. Initialized in ctor, never <code>null</code> after that. */
  private IPSPublishingWs publishWs;

  /**
   * The publisher service, initialized by constructor, never <code>null</code> after that.
   *
   * <p>Note, the "publishWs" is an unnecessary layer, should make direct call to this service
   * directly.
   */
  ...
  private static final String UPDATE_TABLES_EDITION_TASK_NAME =
      "Java/global/percussion/task/perc_UpdateTablesEditionTask";
  ...
  @Autowired
  public PSSitePublishDao(
      IPSIdMapper idMapper,
      IPSPublishingWs publishWs,
  ...
  /**
   * Updates the specified site and its related edition/content-list with the new name and
   * description.
   *
   ...
   * @return <code>true</code> if the update resulted in a change to a pubserver definition, <code>
   *     false</code> if not.
   */
  @Override
  ...
  /**
   * Updates the specified site with passed in publishing properties and updates Content-list with
   * user passed in delivery type and this is part of the publish properties.
   *
   * @param site the existing site, not <code>null</code>.
  ...
  public void updateSitePublishProperties(IPSSite site, PSSitePublishProperties publishProps)
    ...
    for (IPSEdition edition : editions) {
      List<IPSEditionContentList> editionContentLists =
          publishWs.loadEditionContentLists(edition.getGUID());
      for (IPSEditionContentList editionContentList : editionContentLists) {
        IPSContentList contentList =
            publisherService.loadContentList(editionContentList.getContentListId());
        String currentUrl = contentList.getUrl();
        contentList.setUrl(
            PSUrlUtils.replaceUrlParameterValue(
                currentUrl,
                IPSHtmlParameters.SYS_DELIVERYTYPE,
                publishProps.getPublishType().toString()));
  ...
  public boolean saveSite(PSSite site)
      throws PSErrorException, IPSPubServerService.PSPubServerServiceException,
          PSNotFoundException {
  ...
  public IPSEdition findEdition(IPSGuid pubServerId, PubType pubType) {
    ...
    for (IPSEdition edition : editions) {
      if (edition.getName().endsWith(pubType.name())) return edition;
    }
  ...
  private boolean setSiteNameRelatedProperties(IPSSite site, String siteName)
    ...
    if (protocol == null || StringUtils.isBlank(protocol)) protocol = "https";
  ...
  /**
   * Generates the publishing root location for the specified site. This location is tomcat
   * specific, which means that the resulting path will end in {siteName}apps/ROOT. See {@link
   * #makePublishingDir(String)}.
   *
  ...
  /**
   * Extracts the base path from the specified publishing root location. The base path represents
  ...
  /**
   * Generates the directory which will be used as the root publishing location for the specified
   * site.
   *
   * @param siteName never blank.
   * @param publishServerType never blank
   * @param deliveryRootPath never blank
   * @return the relative directory of the publishing path for the site, never blank. Forward slash
   *     is used as the file separator.
   */
  @Override
  public String getPublishingDeliveryRoot(
      String siteName, String publishServerType, String deliveryRootPath) {
  ...
  /**
   * Generates the directory under a tomcat server which will be used as the root publishing
   * location for the specified site.
   *
   * @param siteName never blank.
   * @return the relative directory of the publishing path for the site, never blank. Forward slash
   *     is used as the file separator.
  ...
  /**
   * Creates (and saves) the publishing infrastructure for the specified site. This includes all
   * content lists and editions.
   *
   * @param site The site, may not be <code>null</code>.
   * @param pubServer The default publishing server associated with the new site, may not be <code>
   *     null</code>.
   * @param isDefaultServer boolean flag that indicates whether the server is the default <code>null
   *     </code>.
   * @throws PSErrorException If an error occurs creating the required content lists.
   */
  protected void createPublishingItems(IPSSite site, PSPubServer pubServer, boolean isDefaultServer)
      throws PSErrorException, PSNotFoundException {
    notNull(site, "site");
  ...
  /**
   * Creates (and saves) content lists for the specified {@link PSPubServer publish server}.
   *
   * @param site The {@link IPSSite site}, assumed not <code>null</code>.
   * @param pubServer the {@link PSPubServer publish server}, assumed not <code>null</code>.
   * @param isDefault <code>true</code> if this is the default pub server, <code>false</code>
   *     otherwise.
   */
  private void createContentListsForPubServer(
      IPSSite site, PSPubServer pubServer, boolean isDefault) {
    String siteName = site.getName();
    ...
    assetSearchGenParams.put(
        queryKey, PSSitePublishDaoHelper.makeJcrSearchQuery(PSAssetPathItemService.ASSET_ROOT));
    ...
    if (pubServer.isDatabaseType() || pubServer.isXmlFormat()) {
      ...
      PSSitePublishDaoHelper.createPubServerContentList(
          siteName,
          suffix + FULL_SITE,
          IPSContentList.Type.NORMAL,
          "Site Root Full",
          PSEditionType.AUTOMATIC,
          SEARCH_GENERATOR,
          searchGenParams,
          filterId,
          true,
      ...
      PSSitePublishDaoHelper.createPubServerContentList(
          siteName,
          suffix + STAGING_ASSET,
          IPSContentList.Type.NORMAL,
          "Asset Root Staging",
          PSEditionType.AUTOMATIC,
          SEARCH_GENERATOR,
          assetSearchGenParams,
          filterId,
          true,
          pubServer);
  ...
  /**
   * Creates the content lists and the publishing editions for the given {@link PSPubServer publish
   * server}. It creates one content list only (for pages) if the server is {@link PublishType}
   * Database or it is for xml publishing, otherwise it creates two content lists, one for pages and
   * one for assets.
   *
   * @param site The {@link IPSSite site}, must not be <code>null</code>.
   * @param pubServer the {@link PSPubServer publish server}, must not be <code>null</code>.
   */
  @Override
  public void createPublishingItemsForPubServer(
   ...
   *
   * @param site The site, may not be <code>null</code>.
   * @param isDefaultServer <code>true</code> if this is the default server, <code>false</code> if
   *     not.
   * @throws PSErrorException If a required item filter could not be found.
   */
  private void createContentLists(IPSSite site, PSPubServer pubServer, boolean isDefaultServer)
    ...
    String suffix = PSSitePublishDaoHelper.createSiteSuffix(site);
    String fullSiteName =
        PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_SITE);
  ...
  /**
   * Convenience method which creates a content list for the specified site with a delivery type of
   * file system.
   */
  protected void createContentList(
      String siteName,
      String name,
      IPSContentList.Type type,
      String description,
      PSEditionType edtnType,
      String generator,
      Map<String, String> genParams,
      IPSGuid filterId) {
    createContentList(
        siteName,
        name,
   ...
   * @param type The type of the new content list which determines how it will be processed, may not
   *     be <code>null</code>.
   * @param description The new content list description, may not be <code>null</code>.
   * @param edtnType The edition type of the new content list, may not be <code>null</code>.
   * @param generator The content list generator, may not be blank.
   * @param genParams The content list generator parameters.
   * @param filterId The id of the filter to be used with the content list, may not be <code>null
   *     </code>.
   * @param deliveryType The delivery type of the new content list which determines how it will be
   *     published, may not be blank.
   * @param isPublish determines if the content list is used for publishing or unpublishing. It is
   *     <code>true</code> if it is used for publishing.
   */
  protected void createContentList(
      String siteName,
      String name,
    ...
    if (!isPublish) url += "&" + IPSHtmlParameters.SYS_PUBLISH + "=unpublish";
    cList.setUrl(url);
  ...
  /**
   * Location of where the files get published on the filesystem if one is not provided by the user.
   *
  ...
  /**
   * Updates the specified list of full editions for the specified site. When the site name is
   * changed, just need to update the query for each content list associated to the full edition, in
   * order to match the new site name. Others updates regarding the full editions and content lists
   * are done as part of the update process when a publish server is modified.
   *
   * @param editions it contains the updated edition, assumed not <code>null</code>, may be empty
  ...
  /**
   * Updates the content lists contains for the specified "full" edition. Only the content list
   * which name ends with "FULL_SITE" is updated.
   *
   * @param edition the "full" edition, assumed not <code>null</code>.
   * @param site the site of the edition, assumed not <code>null</code>. it also contains the
   *     updated site name.
  ...
  private void updateContentListForEditionFull(IPSEdition edition, IPSSite site, String oldSiteName)
      throws PSNotFoundException {
    List<IPSEditionContentList> editionCLists =
        publishWs.loadEditionContentLists(edition.getGUID());
  ...
  /**
   * Updates the full site content list for the specified site. When the site name changes, the
   * query as part of the parameters of the content list, is updated to match the new site name.
   *
  ...
  /**
   * Updates the specified edition for the specified site and publish server to set the default
   * publish server id in PUBLISH NOW and UNPUBLISH NOW editions.
   *
   * @param editions it contains the updated edition, assumed not <code>null</code>, may be empty
   *     (do nothing in this case).
   * @param site the site of the edition, assumed not <code>null</code>.
   ...
   */
  private void updateOnDemandEditionsByPubServer(
    ...
    for (IPSEdition edition : editions) {
      if (edition.getSiteId().longValue() == site.getGUID().longValue()
        ...
        if (edition.getName().contains("_STAGING_")) continue;
  ...
  private void updateContentListForOnDemandEditionsByPubServer(
      IPSEdition edition, PSPubServer pubServer) throws PSNotFoundException {
   ...
   */
  protected void createEditions(IPSSite site, PSPubServer pubServer, boolean isDefaultServer)
      throws PSErrorException, PSNotFoundException {
    notNull(site, "site");
    ...
    String fullAssetName =
        PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_ASSET);
    ...
    IPSContentList[] cLists = new IPSContentList[] {assetList, siteList};

    // Full publish for server
    PSSitePublishDaoHelper.createEdition(
        site,
        FULL,
        "Full publish for publish server",
        PSEditionType.NORMAL,
        IPSEdition.Priority.LOWEST,
        cLists,
        pubServer,
        true,
        isDefaultServer);
  ...
  /**
   * Creates the full publishing edition for {@link PSPubServer servers} that should publish to
   * Database or in xml format. For database servers, it sets a pre-edition task to update the
   * tables (See {@link PSUpdateTablesEditionTask})
   *
   * @param site {@link IPSSite}, assumed not <code>null</code>.
   * @param suffix {@link String}, the suffix of the new edition name, assumed not blank.
   * @param description {@link String}, the description of the new edition, assumed not blank.
   * @param type The {@link PSEditionType} of the new edition, assumed not <code>null</code>.
   * @param priority The new edition {@link IPSEdition.Priority priority}, assumed not <code>null
   *     </code>.
   * @param cLists {@link IPSContentList}[] with the content lists to associate with the edition.
   *     The content lists should be ordered by sequence. Assumed not <code>null</code>.
   * @param pubServer {@link PSPubServer} to create the edition for, assumed not <code>null</code>.
   * @throws {@link PSErrorException} if an error occurs.
   */
  private void createEditionForDatabaseOrXml(
      IPSSite site,
      String suffix,
      String description,
      PSEditionType type,
      IPSEdition.Priority priority,
      IPSContentList[] cLists,
      PSPubServer pubServer)
      throws PSErrorException {
    ...
    IPSPublishingContext locationContext =
        publishWs.loadContext(PSSitePublishDaoHelper.LOCATION_CONTEXT);
  ...
  /**
   * Creates and saves an instance of {@link IPSEditionTask} for the datababase update tables
   * pre-task.
   *
  ...
  private void createUpdateTablesTask(IPSGuid edtnGuid) {
    ...
    updateTablesTask.setParam(
        UPDATE_TABLES_EDITION_TASK_TEMPLATE_PARAM, UPDATE_TABLES_EDITION_TASK_TEMPLATE_VALUE);
   ...
   */
  @Override
  public void updateServerEditions(
      IPSSite site, PSPubServer oldServer, PSPubServer server, boolean isDefaultServer)
      throws PSNotFoundException {
    notNull(oldServer);
    notNull(server);
  ...
  private void updateServerIncrementalEdition(
    ...
    if (tgtEdition == null) return; // do nothing if cannot find the edition
  ...
  private void updateServerFullEdition(
      List<IPSEdition> editions,
      PSPubServer oldServer,
      PSPubServer server,
      IPSSite site,
      boolean isDefaultServer)
      throws PSNotFoundException {
   ...
   */
  private void addTaskDefsToPubServerEdition(
      IPSEdition tgtEdition, PSPubServer pubServer, boolean isDefaultServer, IPSSite site) {
    ...
    if (pubServer
      ...
      preTask.setParam(
          "bucket_name",
          pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_AS3_BUCKET_PROPERTY, ""));
      preTask.setParam(
          "access_key",
          pubServer.getPropertyValue(IPSPubServerDao.PUBLISH_AS3_ACCESSKEY_PROPERTY, ""));
      preTask.setParam(
          "secret_key",
     ...
     * Add the workflow post edition task here.
     */
    if (!tgtEdition.getName().contains(PSSitePublishDaoHelper.UNPUBLISH_NOW)
        && !isStagingServer(pubServer)) {
      IPSEditionTaskDef workflowTask = publishWs.createEditionTask();
      workflowTask.setContinueOnFailure(false);
      workflowTask.setEditionId(editionGuid);
      workflowTask.setParam("state", PSSitePublishDaoHelper.PENDING_WORKFLOW_STATE);
   ...
   * @return <code>true</code> if the edition is publish now or unpublish now. <code>false</code>
   *     otherwise.
   */
  private boolean isOnDemandEdition(IPSEdition tgtEdition) {
    if (endsWith(tgtEdition.getName(), PSSitePublishDaoHelper.PUBLISH_NOW)
        || endsWith(tgtEdition.getName(), UNPUBLISH_NOW_SUFFIX)) {
      return true;
    }
    return false;
  }
  ...
  private void updateServerContentListForEditionIncremental(
      IPSEdition edition, PSPubServer server, IPSSite site) throws PSNotFoundException {
    notNull(edition, "edition");
  ...
  /**
   * Handles the update of the content lists for a given server. It deletes the full asset edition
   * if needed (when changing from non xml to xml for example).
   *
   * @param edition {@link IPSEdition} object, must not be <code>null</code>.
   * @param oldServer {@link PSPubServer} representing the old publishing server object, must not be
   *     <code>null</code>.
   * @param server {@link PSPubServer} representing the new publishing server object, must not be
   *     <code>null</code>.
   * @param site {@link IPSSite} object with the site of the server. Must not be <code>null</code>.
   */
  private void updateServerContentListForEditionFull(
      IPSEdition edition, PSPubServer oldServer, PSPubServer server, IPSSite site)
      throws PSNotFoundException {
    notNull(edition, "edition");
    notNull(server, "server");
    ...
    for (IPSEditionContentList editionClist : editionCLists) {
      ...
      if (editionClist.getSequence() >= nextSeq) nextSeq = editionClist.getSequence() + 1;
    }
   ...
   *
   * @param fullSiteCList the content list, assumed not <code>null</code>.
   * @param pubServer the publishing server of the content list, assumed not <code>null</code>.
   * @param site {@link IPSSite} object with the site of the server. Assumed not be <code>null
   *     </code>.
   */
  private void updateFullPubServerContentList(
    ...
    String fullSiteName =
        PSSitePublishDaoHelper.createName(pubServer.getName(), appendSiteId + siteSuffix);
    String assetsName =
        PSSitePublishDaoHelper.createName(pubServer.getName(), appendSiteId + assetSuffix);
    ...
    fullSiteCList.setUrl(
        PSSitePublishDaoHelper.makeContentListUrl(cListName, pubServer.getPublishType()));
  ...
  private void updateIncrementalPubServerContentList(
  ...
  /**
   * If the server changed from database or xml to other, we need to add a new content list for
   * Assets. We need to double check it, cause we may have the case of a server that was database,
   * but now it's not, but still be xml format, so we don't need to add the new content list. The
   * same happens for the case of a server that was xml and now it's not, but it is a database type.
   *
   * @param oldServer {@link PSPubServer} object, assumed not <code>null</code>.
   * @param pubServer {@link PSPubServer} object, assumed not <code>null</code>.
   * @param site {@link IPSSite} object, assumed not <code>null</code>.
   * @param nextSeq the next sequence number for the new edition content list, assumed not <code>
   *     null</code>.
   */
  private void createAssetContentListIfNeeded(
      PSPubServer oldServer, PSPubServer pubServer, IPSSite site, int nextSeq) {
  ...
  /**
   * Creates a full asset content list for the updated publishing server. It also links the created
   * content list with the corresponding edition for the given server.
   *
   * @param pubServer {@link PSPubServer} the updated server, assumed not <code>null</code>.
   * @param site {@link IPSSite} the site for the server, assumed not <code>null</code>.
   * @param nextSeq the next sequence number for the new edition content list, assumed not <code>
   *     null</code>.
   */
  ...
  /**
   * Deletes a full asset content list for the updated publishing server. Try to find the full asset
   * content list with the old name, in case it was updated.
   *
   * @param edition {@link IPSEdition} the edition, assumed not <code>null</code>.
   */
  ...
  /**
   * Links the {@link IPSContentList} with the corresponding {@link IPSEdition}. It assumes that
   * both objects are already saved into the system.
   *
   * @param cList {@link IPSContentList} to link, assumed not <code>null</code>.
   * @param edtn {@link IPSContentList} to link the content list to, assumed not <code>null</code>.
   * @param nextSeq the next sequence number for the new edition content list, assumed not <code>
   *     null</code>.
   */
   ...
   *
   * <p>The editions need to be updated if:
   ...
   * @param oldServer {@link PSPubServer} object, assumed not <code>null</code>.
   * @return <code>true</code> if the server needs is editions to be updated. <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Verifies if the server's {@link PublishType publish type} was Database and was changed to
   * other.
   *
   * @param oldServer {@link PSPubServer} object with the old values, assumed not <code>null</code>
   * @param pubServer {@link PSPubServer} object with the new values, assumed not <code>null</code>
   * @return <code>true</code> if the server was database publishing type, and now its not. <code>
  ...
  /**
   * Verifies if the server's {@link PublishType publish type} was not Database and was changed to
   * be Database.
   *
   * @param oldServer {@link PSPubServer} object with the old values, assumed not <code>null</code>
   * @param pubServer {@link PSPubServer} object with the new values, assumed not <code>null</code>
   * @return <code>true</code> if the server was not database publishing type, and now it is. <code>
   *     false</code> otherwise.
   */
  ...
  /**
   * Verifies if the server's publishing format was XML and was changed to other.
   *
   * @param oldServer {@link PSPubServer} object with the old values, assumed not <code>null</code>
   * @param pubServer {@link PSPubServer} object with the new values, assumed not <code>null</code>
   * @return <code>true</code> if the server was xml publishing format, and now its not. <code>false
   *     </code> otherwise.
   */
  ...
  /**
   * Verifies if the server's publishing format was not XML and was changed to be XML.
   *
   * @param oldServer {@link PSPubServer} object with the old values, assumed not <code>null</code>
   * @param pubServer {@link PSPubServer} object with the new values, assumed not <code>null</code>
   * @return <code>true</code> if the server was not xml publishing format, and now it is. <code>
   *     false</code> otherwise.
   */
   ...
   * @param oldServer {@link PSPubServer} object with the old values, assumed not <code>null</code>
   * @param pubServer {@link PSPubServer} object with the new values, assumed not <code>null</code>
   * @return <code>true</code> if the server name changed. <code>false</code> otherwise.
   */
  ...
  private IPSPubServerService getPubServerService() {
    if (pubServerService == null)
      pubServerService =
          (PSPubServerService) getWebApplicationContext().getBean("pubServerService");
  ...
  protected void deletePublishingItems(IPSSite site) {
    ...
    for (IPSEdition edtn : edtns) {
      ...
      try {
        ...
        throw new PSErrorException(
            "Failed to delete edition for site: " + site + " edition " + edtn, e);
      }
    }
    ...
    try {
      ...
      throw new PSErrorException(
          "Failed to delete content lists: "
              + new ArrayList<IPSContentList>(cLists)
              + " for site:"
              + site,
          e);
    }
  ...
  /**
   * Gets delivery type for this site based on the default publish server. Doesn't iterate all
   * editions and Contentlists. Just get the first Edition and content list and find delivery type
   * from there.
   *
   * @param site the existing site, not <code>null</code>.
   * @return the delivery type for the given site
   * @throws PSErrorException if no edition is found for the default publish server.
   */
  @Override
  public String getSiteDeliveryType(IPSSite site) throws PSNotFoundException {
    ...
    if (editions.isEmpty()) {
      ...
      throw new PSErrorException(
          "Cannot find an edition for default publish server " + defaultPubServer);
    }
  ...
  /**
   * Gets delivery type for this site based on the staging server. Doesn't iterate all editions and
   * Contentlists. Just get the first Edition and content list and find delivery type from there.
   *
   * @param site the existing site, not <code>null</code>.
   * @return the delivery type for the given site
   * @throws PSErrorException if no edition is found for the staging publish server.
   */
  @Override
  public String getStagingDeliveryType(IPSSite site) throws PSNotFoundException {
    ...
    if (editions.isEmpty()) {
      ...
      throw new PSErrorException(
          "Cannot find an edition for staging publish server " + stagingPubServer);
    }
  ...
  /**
   * Creates the publish now infrastructure for the specified site. This includes a new content list
   * and edition.
   *
  ...
  private void addOnDemandEdition(IPSSite site, String nowType) throws PSNotFoundException {
    ...
    boolean isPublish =
        PSSitePublishDaoHelper.PUBLISH_NOW.equals(nowType)
            || PSSitePublishDaoHelper.STAGING_PUBLISH_NOW.equals(nowType);
    boolean isStaging =
        PSSitePublishDaoHelper.STAGING_PUBLISH_NOW.equals(nowType)
            || PSSitePublishDaoHelper.STAGING_UNPUBLISH_NOW.equals(nowType);
    PSPubServer pubServer =
        isStaging
            ? getPubServerService().getStagingPubServer(site.getGUID())
            : getPubServerService().getDefaultPubServer(site.getGUID());
    ...
    IPSGuid filterGuid =
        isStaging
            ? publishWs.findFilterByName("perc_staging").getGUID()
            : publishWs.findFilterByName("perc_publishNow").getGUID();
    ...
    createContentList(
        ...
        contentListDesc,
        PSEditionType.AUTOMATIC,
        SELECTED_ITEMS_GENERATOR,
        null,
        filterGuid,
        delType,
        isPublish);

    IPSContentList pnCList =
        publishWs.loadContentList(PSSitePublishDaoHelper.createName(siteName, nowType));

    PSSitePublishDaoHelper.createEdition(
        site,
        nowType,
        editionDesc + siteName + " site immediately",
        PSEditionType.AUTOMATIC,
        IPSEdition.Priority.HIGHEST,
        new IPSContentList[] {pnCList},
        pubServer,
        false,
        true);
  }
  ...
  /**
   * Creates the editions required for publishing the specified server. They will be different if
   * the server is {@link Database type} or for xml publishing, as those editions will not publish
   * assets.
   *
   * @param site The {@link IPSSite site}, must not be <code>null</code>.
   * @param pubServer The {@link IPSSite publish server}, must not be <code>null</code>.
   * @throws PSErrorException If a required content list does not exist.
   */
  private void createEditionForPubServer(
      IPSSite site, PSPubServer pubServer, boolean isDefaultServer)
      throws PSErrorException, PSNotFoundException {
    notNull(site, "site");
    notNull(pubServer, "pubServer");
    ...
    if (pubServer.isDatabaseType() || pubServer.isXmlFormat()) {
      IPSContentList cList =
          publishWs.loadContentList(
              PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_SITE));
      IPSContentList[] contentLists = new IPSContentList[] {cList};

      createEditionForDatabaseOrXml(
          site,
          FULL,
          "Full publish for publish server",
          PSEditionType.NORMAL,
      ...
      if (isStaging) {
        siteList =
            publishWs.loadContentList(
                PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + STAGING_SITE));
        assetList =
            publishWs.loadContentList(
                PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + STAGING_ASSET));
        PSSitePublishDaoHelper.createIncrementalEdition(site, pubServer, isDefaultServer);
      } else {
        siteList =
            publishWs.loadContentList(
  ...
  public void deletePublishingItemsByPubServer(PSPubServer pubServer) throws PSNotFoundException {
    ...
    for (IPSEdition edtn : edtns) {
      ...
      try {
        ...
        log.error(
            "Failed to delete publishing items associted to publish server: "
                + pubServer
                + " edition "
                + edtn,
            e);
  ...
  /**
   * Tries to delete the publication server entry from the tch file that belongs to the given site.
   * See {@link PSSiteConfigUtils#removeServerEntry(String, long)}
   *
   * @param pubServer {@link PSPubServer} server object, assumed not <code>null</code>.
   */
  private void handleDeleteServerEntryFromTchFile(PSPubServer pubServer)
    ...
    try {
      ...
      String msg =
          "Failed to remove publication server entry in tch file for site '"
              + site.getName()
              + "', server '"
              + pubServer.getName()
              + "'.";
  ...
  private boolean isIncremental(IPSEdition edition) {
    ...
    try {
      ...
      for (IPSEditionContentList editionContentList : editionContentLists) {
        ...
        if (contentListName.endsWith(PSSitePublishDaoHelper.INCREMENTAL)
  ...
  private static final String SEARCH_GENERATOR =
      "Java/global/percussion/system/sys_SearchGenerator";

  /** Constant for the selected items generator. */
  private static final String SELECTED_ITEMS_GENERATOR =
  ...
  static {
    ...
    descriptionTexts.put(
        "CLIST-" + PSSitePublishDaoHelper.STAGING_PUBLISH_NOW, "Site Staging Publish Now");
    descriptionTexts.put(
        "CLIST-" + PSSitePublishDaoHelper.STAGING_UNPUBLISH_NOW, "Site Staging Unpublish Now");
    descriptionTexts.put(
        "EDITION-" + PSSitePublishDaoHelper.PUBLISH_NOW, "Publish selected content to the ");
    descriptionTexts.put(
        "EDITION-" + PSSitePublishDaoHelper.UNPUBLISH_NOW, "Unpublish selected content from the ");
    descriptionTexts.put(
        "EDITION-" + PSSitePublishDaoHelper.STAGING_PUBLISH_NOW,
        "Publish selected content to the Staging ");
    descriptionTexts.put(
        "EDITION-" + PSSitePublishDaoHelper.STAGING_UNPUBLISH_NOW,
        "Unpublish selected content from the Staging ");
  }
}
