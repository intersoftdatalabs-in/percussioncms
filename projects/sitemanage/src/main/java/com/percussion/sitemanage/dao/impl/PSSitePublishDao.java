...
import java.util.HashSet;
            var editionContentLists = publishWs.loadEditionContentLists(edition.getGUID());
            for (var editionContentList : editionContentLists) {
                var contentList = publisherService.loadContentList(editionContentList.getContentListId());
                var contentListName = contentList.getName();
 ...
 * Sunny Sal says: "Publishing is like Bollywood—lots of drama, but the show must go on!"
 * @author adamgent
 */
...
public class PSSitePublishDao implements com.percussion.sitemanage.dao.IPSSitePublishDao {
    ...
    public List<PSSiteSummary> findAllSummaries() {
        var sites = publishWs.findAllSites();
        var sums = new ArrayList<PSSiteSummary>();
        for (var site : sites) {
    ...
    public PSSiteSummary findByLegacySiteId(String id, boolean isValidate) {
        var siteGuid = idMapper.getGuid(id);
        var site = publishWs.findSiteById(siteGuid);
        return convertToSummary(site, isValidate);
    }

    private boolean isValidPubSite(PSSiteSummary siteSummary) {
        var containCategoryFolder = siteSectionMetaDataService.containCategoryFolder(siteSummary);
        if (log.isDebugEnabled()) {
            log.debug("{} Site: {}", containCategoryFolder ? "Valid" : "Invalid", siteSummary);
        }
    ...
    public boolean updateSite(IPSSite site, String newName, String newDescrption) throws PSNotFoundException {
        ...
        var oldName = site.getName();
        if (!oldName.equals(newName)) {
            site.setPreviousName(oldName);
        }
        var serverChanged = setSiteNameRelatedProperties(site, newName);
        site.setDescription(newDescrption);
        site.setPageBased(true);
        siteMgr.saveSite(site);
        var editions = publishWs.findAllEditionsBySite(site.getGUID());
    ...
    public void updateSitePublishProperties(IPSSite site, PSSitePublishProperties publishProps) throws PSNotFoundException {
        ...
        for (var edition : editions) {
            ...
            for (var editionContentList : editionContentLists) {
                var contentList = publisherService.loadContentList(editionContentList.getContentListId());
                var currentUrl = contentList.getUrl();
                contentList.setUrl(PSUrlUtils.replaceUrlParameterValue(
                        currentUrl, IPSHtmlParameters.SYS_DELIVERYTYPE, publishProps.getPublishType().toString()));
    ...
    public IPSSite createSite(String siteName) throws PSNotFoundException {
        var tmpSite = publishWs.createSite();
    ...
    public IPSEdition findEdition(IPSGuid pubServerId, PubType pubType) {
        ...
        var editions = publishWs.findAllEditionsByPubServer(pubServerId);
        return editions.stream()
                .filter(edition -> edition.getName().endsWith(pubType.name()))
                .findFirst()
                .orElse(null);
    }
    ...
    private boolean setSiteNameRelatedProperties(IPSSite site, String siteName) throws PSNotFoundException {
        ...
        var deliveryServerPath = getWebServerFileSystemRoot();
        notEmpty(deliveryServerPath, "webServerFileSystemRoot");
        var port = getWebServerPort();
        var protocol = site.getSiteProtocol();
        ...
        var root = getLocalPublishingRoot(siteName);
        if (oldName != null && PublishType.valueOf(getSiteDeliveryType(site)).isFtpType()) {
            var oldRoot = getPubServerService().getDefaultPubServer(site.getGUID()).getPropertyValue(IPSPubServerDao.PUBLISH_FOLDER_PROPERTY);
            var pubBase = getPublishingBase(oldRoot, oldName);
    ...
    public String getPublishingBase(String siteRoot, String siteName) {
        notEmpty(siteRoot);
        notEmpty(siteName);
        var index = siteRoot.lastIndexOf(makePublishingDir(siteName));
        if (index == -1) {
    ...
    public String getPublishingDeliveryRoot(String siteName, String publishServerType, String deliveryRootPath) {
        var folderPath = StringUtils.EMPTY;
    ...
    private void createContentListsForPubServer(IPSSite site, PSPubServer pubServer, boolean isDefault) {
        ...
        var searchGenParams = new HashMap<String, String>();
        var assetSearchGenParams = new HashMap<String, String>();
    ...
    private void createContentLists(IPSSite site, PSPubServer pubServer, boolean isDefaultServer) throws PSErrorException {
        ...
        var suffix = PSSitePublishDaoHelper.createSiteSuffix(site);
        var fullSiteName = PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_SITE);
        var assetsName = PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_ASSET);
    ...
    private void updateContentListForEditionFull(IPSEdition edition, IPSSite site, String oldSiteName) throws PSNotFoundException {
        var editionCLists = publishWs.loadEditionContentLists(edition.getGUID());
        for (var editionClist : editionCLists) {
            var clist = publisherService.loadContentList(editionClist.getContentListId());
    ...
    private void updateContentListForOnDemandEditionsByPubServer(IPSEdition edition, PSPubServer pubServer) throws PSNotFoundException {
        ...
        for (var editionClist : editionCLists) {
            var cList = publisherService.loadContentList(editionClist.getContentListId());
            var name = cList.getName();
            var url = PSSitePublishDaoHelper.makeContentListUrl(name, pubServer.getPublishType());
    ...
    protected void createEditions(IPSSite site, PSPubServer pubServer, boolean isDefaultServer) throws PSErrorException, PSNotFoundException {
        ...
        var suffix = PSSitePublishDaoHelper.createSiteSuffix(site);
        var fullSiteName = PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_SITE);
        var fullAssetName = PSSitePublishDaoHelper.createName(pubServer.getName(), suffix + FULL_ASSET);
    ...
    public void updateServerEditions(IPSSite site, PSPubServer oldServer, PSPubServer server, boolean isDefaultServer) throws PSNotFoundException {
        ...
        // Get the editions
        var editions = publishWs.findAllEditionsByPubServer(server.getGUID());
    ...
    private void updateServerIncrementalEdition(List<IPSEdition> editions, PSPubServer server,
        ...
        var suffix = isStaging?PSSitePublishDaoHelper.STAGING_INCREMENTAL:PSSitePublishDaoHelper.INCREMENTAL;
        IPSEdition tgtEdition = null;
        for (IPSEdition edition : editions) {
            if (edition.getSiteId().longValue() == site.getGUID().longValue()
        ...
        var editionName = PSSitePublishDaoHelper.createName(server.getName(), suffix);
         ...
         */
        var taskDefs = publishWs.loadEditionTaskByEdition(tgtEdition.getGUID());
    ...
    private void updateServerFullEdition(List<IPSEdition> editions, PSPubServer oldServer, PSPubServer server,
            IPSSite site, boolean isDefaultServer) throws PSNotFoundException {
        var editionName = PSSitePublishDaoHelper.createName(oldServer.getName(), FULL);
        IPSEdition tgtEdition = null;
        for (IPSEdition edition : editions) {
    ...
    private void updateFullPubServerContentList(IPSContentList fullSiteCList, PSPubServer pubServer, IPSSite site) {
        ...
        var appendSiteId = PSSitePublishDaoHelper.createSiteSuffix(site);
        boolean isStaging = PSPubServer.STAGING.equalsIgnoreCase( pubServer.getServerType());
        var siteSuffix = isStaging?STAGING_SITE:FULL_SITE;
        var assetSuffix = isStaging?STAGING_ASSET:FULL_ASSET;

        var fullSiteName = PSSitePublishDaoHelper.createName(pubServer.getName(), appendSiteId + siteSuffix);
        var assetsName = PSSitePublishDaoHelper.createName(pubServer.getName(), appendSiteId + assetSuffix);

        var cListName = PSSitePublishDaoHelper.createName(pubServer.getName(), siteSuffix);
    ...
    private void updateIncrementalPubServerContentList(IPSContentList incrementalSiteCList, PSPubServer pubServer, IPSSite site) {
        ...
        var appendSiteId = PSSitePublishDaoHelper.createSiteSuffix(site);
        boolean isStaging = PSPubServer.STAGING.equalsIgnoreCase( pubServer.getServerType());
        var incSuffix = isStaging?PSSitePublishDaoHelper.STAGING_INCREMENTAL:PSSitePublishDaoHelper.INCREMENTAL;
        var cListName = PSSitePublishDaoHelper.createName(pubServer.getName(), appendSiteId + incSuffix);
    ...
    private void createFullAssetContentList(PSPubServer pubServer, IPSSite site, int nextSeq) {
        var siteName = site.getName();
        var siteRoot = site.getFolderRoot();
        var suffix = site.getSiteId() + "_" + FULL_ASSET;
        ...
        var cList = PSSitePublishDaoHelper.createPubServerContentList(siteName, suffix, IPSContentList.Type.NORMAL,
                "Asset Root Full", PSEditionType.AUTOMATIC, SEARCH_GENERATOR, assetSearchGenParams, filterId, true,
                pubServer);

        var edtnName = PSSitePublishDaoHelper.createName(pubServer.getName(), FULL);
    ...
    private void deleteFullAssetContentList(IPSEdition edition) {
        ...
        var ecls = publishWs.loadEditionContentLists(edition.getGUID());
        for (var ecl : ecls) {
            var cList = publishWs.findContentListById(ecl.getContentListId());
    ...
    private void addOnDemandEdition(IPSSite site, String nowType) throws PSNotFoundException {
        ...
        var contentListDesc = descriptionTexts.get("CLIST-" + nowType);
        var editionDesc = descriptionTexts.get("EDITION-" + nowType);

        var edtnContentListName = PSSitePublishDaoHelper.createName(site.getName(), nowType);
