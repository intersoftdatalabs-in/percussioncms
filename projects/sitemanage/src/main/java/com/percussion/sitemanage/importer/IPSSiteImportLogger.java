...
public interface IPSSiteImportLogger {
  /**
   * Defines type of log entry, passed as <code>type</code> param to {@link
   * IPSSiteImportLogger#appendLogMessage(PSLogEntryType, String)}
   */
  ...
  /**
   * Defines the types of objects for which log entries will be created. Used for <code>objectType
   * </code> param in {@link PSImportLogEntry#PSImportLogEntry(long, String, Date, String)} ctor
   */
  ...
  /**
   * Collect errors when calls to {@link #appendLogMessage(PSLogEntryType, String, String)} are made
   * with a log entry type of {@link PSLogEntryType#ERROR}, which can be retrieved by calls to
   ...
   * @return The list, may be empty, <code>null</code> if {@link #logErrors(PSLogObjectType,
   *     String)} has not been called.
   */
  List<PSImportLogEntry> getErrors(
      PSLogObjectType errorObjectType, String errorObjectId, String description);
   ...
   *
   * @param timeoutSeconds The number of seconds to wait before continuing on without the thread
   *     count reaching zero
   */
