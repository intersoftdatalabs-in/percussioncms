...
/**
 * The user data service is responsible for managing users and their role associations.
 *
 * <p>A User is required to be in at least one role.
 *
 * @author DavidBenua
 * @author adamgent
 */
public interface IPSUserService {
  ...
  /**
   * Finds a user. If the user is not found an exception is thrown.
   *
  ...
  /**
   * Gets the current user from the callers request that is in thread-local meta data.
   *
   * @return never <code>null</code>.
   * @throws PSNoCurrentUserException if the callers thread-local does not have the user name
   *     available.
   */
  ...
  /**
   * Creates the user. {@link PSUser#getPassword()} must not be empty or <code>null</code>.
   *
   * @param user
   * @return never <code>null</code>. The password property will always be <code>null</code> on the
   *     return object.
   * @throws PSDataServiceException
   */
   ...
   *
   * @param user the {@link PSUser#getName()} is used to determine which user to update password
   *     for.
   * @return never <code>null</code>. The user object should have null on the password property.
   * @throws PSDataServiceException
   */
   ...
   * @return never <code>null</code>.
   * @throws PSDirectoryServiceException if the directory service is not enabled or is not
   *     available.
   */
  public List<PSExternalUser> findUsersFromDirectoryService(String query)
      throws PSDirectoryServiceException;

  /**
   * Imports external users usually from a directory service.
   *
   * <p>The users to import can usually be found by using: {@link
   * #findUsersFromDirectoryService(String)}.
   *
   * @param importUsers never <code>null</code>, maybe empty.
   * @return Imported users successful or not. The number of users returned should be the same
   *     number of external users {@link PSImportUsers#getExternalUsers() passed in} even if some of
   *     the users failed to import. See {@link PSImportedUser#getStatus()}.
   * @throws PSDirectoryServiceException if the directory service is not enabled or is not
   *     available.
   ...
   *
   * <p>An exception should never be thrown from this method. Instead the {@link
   * PSDirectoryServiceStatus#getError()} will contain the exception.
   *
   * @return never <code>null</code>.
   */
  ...
  /**
   * Finds the access level of the current user. The access level is the assignment type of the user
   * for the initial state of the workflow specified in the request.
   *
  ...
  public static class PSImportUsers {
    ...
    @NotNull private List<PSExternalUser> externalUsers;
  ...
  /**
   * A message object used to communicate the status of an external directory service.
   *
   * @author adamgent
   */
  ...
  public static class PSDirectoryServiceStatus {
    ...
    /**
     * The exception that was thrown either because of {@link ServiceStatus#CONFIG_ERROR
     * configuration error} or {@link ServiceStatus#CONNECTION_ERROR connection error}.
     *
    ...
    public static enum ServiceStatus {
      /**
       * The Directory service is not enabled because either config is missing or it is disabled via
       * config.
       */
      DISABLED,
  ...
  /**
   * Indicates some failure with an external directory service such as LDAP or Active Directory.
   *
   * @author adamgent
   */
  public static class PSDirectoryServiceException extends RuntimeException {
    ...
    /**
     * Get a serializable object that indicates the status of the directory service.
     *
     * @return never <code>null</code>.
     */
     ...
     *
     * <p>This method is safe to override.
     *
     * @return never <code>null</code>.
     */
   ...
   *
   * @param nameFilter The filter to match, <code>null</code> to find all user names, "%" and "_"
   *     sql wildcards are supported.
