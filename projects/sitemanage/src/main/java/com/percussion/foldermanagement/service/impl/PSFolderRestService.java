...
public class PSFolderRestService {
   ...
   *     folder items once the job is complete, otherwise it will be <code>null</code>. If the job
   *     has failed, {@link PSGetAssignedFoldersJobStatus#getStatus()} will return -1, if completed
   *     it will return 100, and if running it will return 1. *
   */
  @GET
  ...
  public Response getAssociatedFolders(
      ...
      @QueryParam("includeFoldersWithDifferentWorkflow") @DefaultValue("false")
          Boolean includeFoldersWithDifferentWorkflow) {
    String message =
        "There was an error getting the assosiated folders to workflow '"
            + workflowName
            + "', from the path '"
            + path
            + "'. ";
