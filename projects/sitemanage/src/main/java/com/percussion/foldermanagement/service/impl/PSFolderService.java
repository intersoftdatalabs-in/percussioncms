...
/**
 * Service class to handle the associations of sites or assets folders and workflows. It provides
 * methods to query and to modify the workflow associated with sites and folders.
 *
 * @author miltonpividori
 */
@Component("folderService")
public class PSFolderService implements IPSFolderService {
  private static final String ASSIGNMENT_IN_PROGRESS =
      "We are unable to process your request at this time.  A similar request has already been submitted and is still processing in the background.  Please try again later.";
   ...
   */
  @Override
  public List<PSFolderItem> getAssignedFolders(
    ...
    for (PSPathItem folderProperties : foldersProperties) {
      ...
      fullFolderTreeItem =
          getFullFolderTree(
              getFolderItem(folderProperties), workflowName, includeFoldersWithDifferentWorkflow);
  ...
  @Override
  public String startGetAssignedFoldersJob(
      String workflowName, String path, boolean includeFoldersWithDifferentWorkflow)
      throws PSWorkflowNotFoundException {
    long jobId =
        asyncJobService.startJob(
            "getAssignedFoldersJob",
            new Object[] {workflowName, path, includeFoldersWithDifferentWorkflow});
  ...
  public void assignFoldersToWorkflow(PSWorkflowAssignment workflowAssignment)
      throws PSWorkflowNotFoundException, PSWorkflowAssignmentInProgressException {
    Validate.notNull(workflowAssignment, "workflowAssignment cannot be null");
    Validate.notEmpty(
        workflowAssignment.getWorkflowName(), "workflowAssignment.workflowName cannot be null");
    ...
    try {
      ...
      assignFoldersToWorkflow(
          workflowAssignment.getAssignedFolders(),
          workflowAssignment.getUnassignedFolders(),
          workflow);
  ...
  private void decrementAssignmentOperationCount() {
    ...
    if (count < 0) assignmentOperationCount.compareAndSet(count, 0);
  }
  ...
  private void assignFoldersToWorkflow(
      String[] assignedFolderIds, String[] unassignedFolderIds, PSWorkflow workflow) {
    // Assignment
    for (String id : assignedFolderIds) {
      try {
        ...
        log.error(
            "There was an error assigning the workflow '"
                + workflow.getName()
                + "' to the folder with id '"
                + id
    ...
    for (String id : unassignedFolderIds) {
      try {
        ...
        log.error(
            "There was an error unassigning the workflow '"
                + workflow.getName()
                + "' to the folder '"
                + id
                + "'. The underlying error was: "
  ...
  public boolean applyWorkflowToContent(String[] folderIds) {
    ...
    if (folderIds != null && folderIds.length == 0) return didStartThread;
    ...
    try {
      for (String folderId : folderIds) {
        try {
          ...
          log.error(
              "There was an error applying the assigned workflow to content in the folder '"
                  + folderId
                  + "'. The underlying error was: "
                  + e.getMessage());
        }
          ...
          new Runnable() {
            ...
            public void run() {
              try {
                ...
                log.error(
                    "There was an error applying the assigned workflow to content in the specified folders. "
                        + "The underlying error was: {}",
      ...
      if (!didStartThread && didIncrementCount) decrementAssignmentOperationCount();
    }
  }
  ...
  private List<String> getDefaultWorkflowFolderIds() {
    ...
    try {
      ...
      log.error(
          "Error getting all paths using the default workflow.  The underlying error was: "
              + e.getMessage());
      idList.clear();
  ...
  /**
   * Recursively get all sub-folder ids using the default workflow implicitly (no workflow assigned)
   * for the supplied path .
   *
   * @param path the path to process, each subfolder of the path is evaluated and possibly added to
   *     the results, then the method recurses for each subfolder path. The supplied path is not
   *     evaluated.
  ...
  private List<String> getDefaultWorfklowIdsFromPath(String path) throws Exception {
    ...
    for (PSPathItem pathItem : subPaths) {
      ...
      if (StringUtils.isEmpty(folderItem.getWorkflowName())) ids.add(folderItem.getId());
  ...
  private List<String> getDefaultWorfklowIds(String id) throws Exception {
    ...
    for (PSPathItem pathItem : subPaths) {
      PSFolderItem folderItem = getFolderItem(pathItem);
      if (StringUtils.isEmpty(folderItem.getWorkflowName())) paths.add(pathItem.getId());
   ...
   *
   * @param path the path to get the children from. Assumed not <code>null</code>, or empty.
   * @return a list of {@link PSPathItem} objects. May be empty, but never <code>null</code>
   */
  private List<PSPathItem> getSubfolders(String path)
      throws IPSPathService.PSPathServiceException, PSDataServiceException {
    ...
    for (PSPathItem item : children) {
      if (item.isFolder() || PSPathItem.TYPE_SITE.equals(item.getType())) subfolders.add(item);
    }
  ...
  private List<PSPathItem> getSubFoldersWithoutPath(String id) {
    ...
    for (PSItemSummary sum : itemSums) {
      if (sum.getObjectType().equals(ObjectTypeEnum.ITEM)) continue;
      if (sum.getName().equals(IPSSiteSectionMetaDataService.SECTION_SYSTEM_FOLDER_NAME)) continue;
  ...
  /**
   * Given a {@link PSFolderItem} object, this method recursively finds all of the children and adds
   * them into that object, building the hierarchy of folders under the given root object.
   *
   * <p>If the <code>includeFoldersWithDifferentWorkflow</code> is set to <code>true</code>, the
   * hierarchy will hold every child folder, even those associated with another workflow.
   *
   * <p>If set to <code>false</code> the folders that are not associated with the given workflow,
   * and have no descendants associated either will not be included. But it will include those
   * folders that are not associated with the workflow but have at least one descendant associated.
   *
   ...
   * @param includeFoldersWithDifferentWorkflow include or not folders that are not associated with
   *     the given workflow.
   * @return a {@link PSFolderTreeInfo} object, holding the corresponding {@link PSFolderItem}
   *     object and a boolean value that indicates if there is a node in this branch of the tree
   *     associated with the given workflow.
   * @throws Exception if an error occurs when getting the childs of the path.
   */
  private PSFolderTreeInfo getFullFolderTree(
      PSFolderItem folderItem, String workflowName, Boolean includeFoldersWithDifferentWorkflow)
      throws Exception {
    PSThreadUtils.checkForInterrupt();
    ...
    for (PSPathItem folderProperties : foldersProperties) {
      ...
      if (!StringUtils.equalsIgnoreCase(
              childFolderItemWithChildren.folderItem.getWorkflowName(), workflowName)
          || (childFolderItemWithChildren.folderItem.getAllChildrenAssociatedWithWorkflow() != null
              && Boolean.FALSE.equals(
                  childFolderItemWithChildren.folderItem.getAllChildrenAssociatedWithWorkflow())))
    ...
    // workflow.
    branchAssociatedWithWorkflow =
        StringUtils.equalsIgnoreCase(folderItem.getWorkflowName(), workflowName)
    ...
    if (folderProperties.getWorkflowId() > 0) {
      PSWorkflow workflow =
          workflowService.loadWorkflow(
              PSGuidUtils.makeGuid(folderProperties.getWorkflowId(), PSTypeEnum.WORKFLOW));

      if (workflow != null) folder.setWorkflowName(workflow.getName());
      else
        log.debug(
  ...
  public PSWorkflow validateWorkflow(String workflowName) throws PSWorkflowNotFoundException {
    ...
    if (workflow == null) {
      throw new PSWorkflowNotFoundException(
  ...
  /**
   * Class used when generating the folder tree. Saves a {@link PSFolderItem} object and a {@link
   * Boolean} value, that indicates that at least one of its descendants is associated with the
   * given workflow.
   *
  ...
  private static class PSFolderTreeInfo {
     ...
     * @param branchAssociatedWithWorkflow the boolean value to use in the instance.
     */
    ...
    /**
     * Given a list of PSFolderTreeInfo objects, it iterates over them and builds a list that
     * contains only PSFolderItem objects.
     *
     * @param children the list of PSFolderTreeInfo objects. May be <code>null</code> or empty.
     * @return a list of PSFolderItem objects. May be empty but never <code>null</code>
     */
  ...
  public List<PSLightWeightObject> getPagesFromFolder(String folderId)
    ...
    try {
      ...
      log.error(e);
      throw new PSPagesNotFoundException(
          "Error occurred retrieving pages for the supplied folder, please se log for more details.");
    }
    List<PSItemSummary> itemSums =
        contentWs.findFolderChildren(idMapper.getGuid(folderProperties.getId()), false);
    for (PSItemSummary sum : itemSums) {
