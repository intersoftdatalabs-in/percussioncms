...
public class PSFolderService implements IPSFolderService {
  private static final String ASSIGNMENT_IN_PROGRESS =
      "We are unable to process your request at this time.  A similar request has already been"
          + " submitted and is still processing in the background.  Please try again later.";
  ...
  private void assignFoldersToWorkflow(
    ...
    for (var id : assignedFolderIds) {
      try {
        ...
        log.error(
            "There was an error assigning the workflow '{}' to the folder with id '{}'. The"
    ...
    for (var id : unassignedFolderIds) {
      try {
        ...
        log.error(
            "There was an error unassigning the workflow '{}' to the folder '{}'. The underlying"
                + " error was: {}",
            workflow.getName(),
            id,
            e.getMessage());
    ...
    try {
      for (var folderId : folderIds) {
        try {
          ...
          var workflowsList =
              workflowsMap.computeIfAbsent(folderWorkflowId, k -> new ArrayList<>());
          workflowsList.add(folderContentId);
        } catch (Exception e) {
          log.error(
              "There was an error applying the assigned workflow to content in the folder '{}'. The"
                  + " underlying error was: {}",
              folderId,
              e.getMessage());
        }
      ...
      Runnable worker =
          () -> {
            try {
              ...
              log.error(
                  "There was an error applying the assigned workflow to content in the specified"
                      + " folders. The underlying error was: {}",
  ...
  private PSFolderTreeInfo getFullFolderTree(
    ...
    for (var folderProperties : foldersProperties) {
      var childFolderItemWithChildren =
          getFullFolderTree(
              getFolderItem(folderProperties), workflowName, includeFoldersWithDifferentWorkflow);
  ...
  private PSFolderItem getFolderItem(PSPathItem pathItem) throws Exception {
    ...
    if (folderProperties.getWorkflowId() > 0) {
      ...
      if (workflow != null) folder.setWorkflowName(workflow.getName());
        ...
        log.debug(
            "The workflow ID associated with the folder '{}' is invalid and will be ignored",
            pathItem.getPath());
    }
    return folder;
  }
  ...
  public List<PSLightWeightObject> getPagesFromFolder(String folderId)
    ...
    try {
      ...
      throw new PSPagesNotFoundException(
          "Error occurred retrieving pages for the supplied folder, please see log for more"
              + " details.");
    }
