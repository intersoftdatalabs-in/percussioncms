...
public class PSWidgetBuilderService implements IPSWidgetBuilderService {
    ...
    try {
      ...
      if (StringUtils.isNotBlank(definition.getWidgetTrayCustomizedIconPath())) {
        definition.setWidgetTrayCustomizedIconPath(
            definition.getWidgetTrayCustomizedIconPath().replaceAll("\\\\", "/"));
        // validate the path
        File imagePath =
            new File(PSServer.getRxDir(), definition.getWidgetTrayCustomizedIconPath());
        if (!imagePath.exists()) {
  ...
  public void deployWidget(@PathParam("definitionId") final long definitionId) {
    ...
    if (!tgtDir.exists()) {
      try {
        ...
        throw new RuntimeException(
            "Unable to create target widget directory: " + tgtDir.getAbsoluteFile(), e);
      }
    }
    ...
    if (!tmpDir.exists()) {
      try {
        ...
        throw new RuntimeException(
            "Unable to create temp widget directory: " + tmpDir.getAbsoluteFile(), e);
      }
    }
    ...
    try {
      ...
      if (result.exists()) {
        ...
        throw new RuntimeException(
            "Failed to generate package for widget definition: " + Long.toString(definitionId));
      }
    } catch (Exception e) {
   ...
   *
   * @param ppkg Package file to be copied. Null value will result in a <code>RuntimeException
   *     </code> thrown
   */
    ...
    if (PSServer.getServerProps() != null
      ...
      if (!mutableWidgetDir.exists()) {
        try {
          ...
          log.error(
              "Unable to create mutable widget directory: {}, Error: {} ",
              mutableWidgetDir.getAbsolutePath(),
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
          throw new RuntimeException(
      ...
      if (!mutableObjectStoreDir.exists()) {
        try {
          ...
          log.error(
              "Unable to create mutable object store directory: {}. Error: {} ",
