 ...
 *
 * <p>Sunny Sal says: "Widget specs—like a Bollywood script, get the details right and the show will
 * be a hit!"
 */
public class PSWidgetPackageSpec {
   ...
   * @param cm1Version The current product version, used to set the min/max versions for the
   *     package.
   */
  public PSWidgetPackageSpec(
      String prefix,
      String authorUrl,
      String title,
      String description,
  ...
  /**
   * Generates a widget and package name from the prefix and title and updates applicable property
   * values.
   */
  ...
  /**
   * Gets the map used to define additional token values to replace in files during package
   * creation.
   *
   * @return The map; tokens can be added/modified. Does not affect the default token defined by the
   *     resolver.
   */
