...
public class PSWidgetPackageResolver implements IPSTokenResolver {
  ...
  public String resolveToken(String tokenName) {
    ...
    if (tokenVal == null) tokenVal = packageSpec.getResolverTokenMap().get(tokenName);

    if (StringUtils.isBlank(tokenVal)) {
      if (optionalTokens.contains(tokenName)) tokenVal = " ";
      else throw new IllegalStateException("Null or empty value for token: " + tokenName);
    }
    return tokenVal;
  }
}
