 ...
 * <p>
 * Sunny Sal says: "Content types are like Bollywood blockbusters—lots of moving parts, but the right script makes it a hit!"
 * </p>
 */
public class PSContentTypeFileTransformer implements IPSWidgetFileTransformer {
    ...
    private Reader transformSchema(Reader reader, PSWidgetPackageSpec packageSpec)
        ...
        for (var field : fields) {
            schema.setColumn(new PSJdbcColumnDef(getDataTypeMap(), field.getName().toUpperCase(),
                    PSJdbcTableComponent.ACTION_CREATE, getDbType(field), getSize(field), true, ""));
            if (field.getType().equals(FieldType.IMAGE.name())
                ...
                schema.setColumn(new PSJdbcColumnDef(getDataTypeMap(), field.getName().toUpperCase() + "_LINKID",
                        PSJdbcTableComponent.ACTION_CREATE, Types.INTEGER, getSize(field), true, ""));
            }
        }
        return new StringReader(PSXmlDocumentBuilder.toString(schema.toXml(PSXmlDocumentBuilder.createXmlDocument())));
    }
    ...
    private Reader transformItemDef(Reader reader, PSWidgetPackageSpec packageSpec)
        ...
        if (mapper == null) {
            throw new RuntimeException("No matching display mapper found for fieldset: " + fieldSet.getName());
        }
        var fields = packageSpec.getFields();
        ...
        return new StringReader(PSXmlDocumentBuilder.toString(itemDef.toXml(PSXmlDocumentBuilder.createXmlDocument())));
    }
    ...
    private void addReservedHtmlClassCleanerExtension(PSField field, PSItemDefinition itemDef) {
        ...
        callSet.add(new PSExtensionCall(new PSExtensionRef("Java/global/percussion/content/sys_cleanReservedHtmlClasses"), params));
    ...
    private PSField addField(PSBackEndTable beTable, PSWidgetBuilderFieldData field, PSFieldSet fieldSet, boolean required)
    ...
    private void addImageProcessors(PSItemDefinition itemDef) {
        ...
        var callSet = new PSExtensionCallSet();
        callSet.add(new PSExtensionCall(new PSExtensionRef("Java/global/percussion/content/sys_managedItemPathPreProcessor"), null));
        inputTranslations.add(new PSConditionalExit(callSet));
        itemDef.getContentEditor().setInputTranslation(inputTranslations);

        callSet = new PSExtensionCallSet();
        callSet.add(new PSExtensionCall(new PSExtensionRef("Java/global/percussion/content/sys_manageLinksPostProcessor"), null));
    ...
    private void addValidationRule(PSField psfield, PSWidgetBuilderFieldData field) {
        ...
        PSExtensionParamValue[] params = {new PSExtensionParamValue(new PSSingleHtmlParameter(field.getName()))};
        var extension = new PSExtensionCall(new PSExtensionRef("Java/global/percussion/content/sys_ValidateRequiredField"), params);
    ...
    private Element getElementFromReader(Reader reader) throws IOException, SAXException {
        ...
        return PSXmlDocumentBuilder.createXmlDocument(new StringReader(out.toString()), false).getDocumentElement();
    }
