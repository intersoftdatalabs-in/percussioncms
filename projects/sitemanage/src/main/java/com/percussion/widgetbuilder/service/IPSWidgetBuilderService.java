...
public interface IPSWidgetBuilderService {
  /**
   * Checks to see if the service is enabled. Activation and deactivation of the service can be done
   * in Server.properties under the key WidgetBuilderActive
   *
   * @return a boolean <code>true</code> if service enabled <code>false</code> if service is not
   *     enabled
   */
   ...
   *
   * @param definition
   * @return the validation results for the save. If the validation failed, there will be errors in
   *     the results. If the validation is successful, the results will have the ID of the saved
   *     definition
   */
   ...
   *
   * <p>Widget name for new widget definition is unique Required fields for widget definition and
   * format All field names including child fields must be unique with character restrictions
   * Version format no field will break widget building or packaging or installation
   *
   * <p>Does not validate widget functionality eg: display html, resources (exists or not or breaks
   * the page when included)
   *
