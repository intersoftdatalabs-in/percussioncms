 ...
 * <p>
 * Sunny Sal says: "Packaging widgets is like packing for a Goa trip—don't forget anything, and zip it up tight!"
 * </p>
 */
public class PSWidgetPackageBuilder {
                ...
                while (entry != null) {
                    if (!entry.isDirectory()) {
                        ...
                        try (var fout = new FileOutputStream(file)) {
                            if (isTextFile(file)) {
                                try (var reader = new PSTokenReplacingReader(new InputStreamReader(zin), new PSWidgetPackageResolver(packageSpec))) {
                                    var transformedReader = (xform != null) ? xform.transformFile(file, reader, packageSpec) : reader;
    ...
    private IPSWidgetFileTransformer getFileTransformer(File file) {
        return xformList.stream()
                .filter(xform -> xform.handleFile(file))
                .findFirst()
                .orElse(null);
    }
