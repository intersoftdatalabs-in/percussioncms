...
/**
 * Represents an abstract object that is bound to the the rendering of a {@link PSPage}, {@link
...
public abstract class PSAbstractAssemblyContext implements Cloneable {
  ...
  /**
   * The type of item that this context was created for. If the rendering is for a page than {@link
   * RootRenderType#PAGE} will be returned. If the rendering is for a template than the return value
   * will be {@link RootRenderType#TEMPLATE} returned.
   *
   * @return never <code>null</code>.
   */
  ...
  /**
   * The type of the item this context is created for while the item is been edited. This value is
   * only useful if {@link #isEditMode()} is <code>true</code>. It is default to {@link
   * EditType#PAGE}.
   *
  ...
  /**
   * This flag is provided to indicate whether the requester plans on using the assembled document
   * in an editor. Widgets can use this flag to change their behavior. For example, if they don't
   * have an asset, they should render some sample content such as 'Add image here' for an image
   * widget. Other possibilities would be to build an in-line editor to re-order a manual list.
   *
  ...
  /**
   * This flag is provided to indicate whether the requester plans on using the assembled document
   * with javascript code stripped or not. Some customer javascript code causes conflicts with our
   * code, this flag can be used to determine whether to render the javascript code or strip it.
   *
