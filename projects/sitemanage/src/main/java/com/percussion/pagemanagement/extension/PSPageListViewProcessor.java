...
public class PSPageListViewProcessor implements IPSListViewProcessor {
  ...
  @Autowired
  public PSPageListViewProcessor(
      IPSPageDaoHelper pageDaoHelper, IPSTemplateDao templateDao, IPSIdMapper idMapper) {
  ...
  public void process(PSDisplayPropertiesCriteria criteria) {
    ...
    Map<String, String> pageToTemplateIdMap =
        pageDaoHelper.findTemplateUsedByCurrentRevisionOfPages(contentIds);
    Map<String, String> linkTextMap =
        pageDaoHelper.findLinkTextForCurrentRevisionOfPages(contentIds);
