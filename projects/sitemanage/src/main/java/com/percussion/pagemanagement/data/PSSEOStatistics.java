 ...
 * have been detected for the Page as well as an indication of the overall severity.
 */
...
public class PSSEOStatistics {
  ...
  /** See {@link #getIssues()}. */
  @NotNull private Set<SEO_ISSUE> issues = new HashSet<>();
   ...
   *
   * @param pageSummary the summary for which seo statistics will be generated, may not be <code>
   *     null</code>.
  ...
  /**
   * Percentage of seo issues discovered for the Page based on the issues defined by {@link
   * SEO_ISSUE}.
   *
   * @return the severity
   */
  ...
  /**
   * @return <code>true</code> if the keyword is present in the page summary (description, link,
   *     title), <code>false</code> otherwise.
   */
  ...
  /**
   * @param keywordPresent <code>true</code> if the keyword is present in the page summary
   *     (description, link, title), <code>false</code> otherwise.
   */
   ...
   * and a severity is computed based on the number of issues found versus the total number of
   * possible issues.
   */
  ...
  /**
   * Calculates the number of possible issues for a Page if it has not been set. Used in {@link
   * #analyze()} to compute the severity level.
   *
  ...
  private double getTotalIssueCount() {
    if (ms_totalIssueCount == -1) {
      ...
      ms_totalIssueCount =
          SEO_ISSUE.values().length - singleWeightIssueCount + ms_singleWeightIssues.size();
    }
  ...
  /**
   * Analyzes the page summary for the correct usage of the keyword. Currently checks the page
   * description, link, and title to ensure that if the keyword exists in one of the fields, it
   * exists in all fields, otherwise, an issue is added for each field from which the keyword is
   * missing. For page links, the keyword is searched exact as well as with spaces replaced with
    ...
    if (StringUtils.isNotBlank(keyword)) {
      ...
      boolean inLink =
          containsKeyword(sLink)
              || containsKeyword(sLink, keyword.replaceAll(" ", "_"), '_')
  ...
  /**
   * Convenience method that calls {@link #containsKeyword(String, String, char)} as <code>
   * containsKeyword(str, keyword, ' ')</code>.
   */
  ...
  /**
   * Determines if the specified string contains the keyword. This is a whole word comparison,
   * case-insensitive.
   *
  ...
  private boolean containsKeyword(String str, String key, char separator) {
    ...
    return (lowerStr.equals(lowerKey)
        || lowerStr.startsWith(lowerKey + separator)
        || lowerStr.endsWith(separator + lowerKey)
        || lowerStr.indexOf(separator + lowerKey + separator) != -1);
  }
