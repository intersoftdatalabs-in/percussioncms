...
public class PSTemplateInfo extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      if (ret != null) resp.getWriter().write(ret);
      else throw new IOException("Unable to export template");
    } catch (Exception ex) {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (isMultipart) {
      try {
        ...
        } else throw new IOException("Unexpected error while importing template.");
      } catch (Exception e) {
