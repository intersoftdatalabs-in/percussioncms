...
/**
 * A legacy location scheme generator that uses resource defintions. This is mainly used for Inline
...
public class PSResourceAssemblyLocation extends PSAbstractAssemblyLocationAdapter {
  ...
  private ItemAndContext getItemAndContext(PSAssemblyLocationRequest locationRequest)
      throws IPSAssetService.PSAssetServiceException, IPSDataService.DataServiceLoadException,
    ...
    PSSiteSummary siteSummary =
        siteDataService.findByLegacySiteId(idMapper.getString(siteGuid), false);
  ...
  /**
   * A holder object to hold both the link context and item.
   *
   * @author adamgent
   */
