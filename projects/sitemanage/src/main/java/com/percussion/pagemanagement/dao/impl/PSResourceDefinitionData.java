...
public class PSResourceDefinitionData {
  private Map<PSResourceDefinitionUniqueId, PSResourceDefinition> resourceDefinitions =
      new HashMap<>();
  ...
  public void add(PSResourceDefinitionGroup group) throws PSDataServiceException {
    ...
    for (PSResourceDefinition rd : rds) {
      PSResourceDefinitionUniqueId uid =
          new PSResourceDefinitionUniqueId(group.getId(), rd.getId());
  ...
  /**
   * Legacy templates associated to a resources. Key is the legacy template name and value is a set
   * of assets with that legacy template.
   *
