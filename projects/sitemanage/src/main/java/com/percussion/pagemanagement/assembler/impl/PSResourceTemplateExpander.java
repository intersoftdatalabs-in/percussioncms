...
public class PSResourceTemplateExpander
  ...
  protected List<PSContentListItem> expandContentListItem(
    ...
    if (isPageType(contentTypeName)) {
      ...
      if (contentListItem.getFolderId() == null) {
        log.error(
            "Not filing this page item for content list: "
      ...
      if (contentListItem.getFolderId() == null) {
        log.error(
            "Not filing this item for content list: "
                + contentListItem.getItemId()
                + " as it has no affiliated folder.");
        return emptyList();
      }
   ...
   * @return the resource ID. It may be <code>null</code> if the parameters does not contain
   *     resource ID.
   */
  ...
  private void setLocation(PSContentListItem contentListItem, Map<String, String> parameters) {
    try {
      PSResourceLocation loc =
          assemblyItemBridge.getResourceLocation(contentListItem, getResourceId(parameters));
