...
public class PSPageChangeHandler implements IPSPageChangeListener {
    ...
    private final IPSCmsObjectMgrInternal cmsObjectMgr = (IPSCmsObjectMgrInternal) PSCmsObjectMgrLocator.getObjectManager();
    ...
    public void pageChanged(PSPageChangeEvent pageChangeEvent) {
        ...
        if ((type == PSPageChangeEventType.ITEM_ADDED || type == PSPageChangeEventType.ITEM_SAVED
                || type == PSPageChangeEventType.ITEM_REMOVED) && StringUtils.isBlank(itemId)) {
        ...
        try {
            ...
            log.error("Error while finding the Page with the pageId {} in pageChanged Event Handler.", pageId, e);
            return;
        }
        ...
        if ((type == PSPageChangeEventType.ITEM_ADDED || type == PSPageChangeEventType.ITEM_SAVED) && asset != null) {
    ...
    private void updateBlogPostWidgetTitle(PSContentItem page) {
        try {
            ...
            var workFlowService = (PSItemWorkflowService) getWebApplicationContext().getBean("workflowRestService");
    ...
    private void updateSummary(PSContentItem page) {
        try {
            ...
            if (pageFields.containsKey(PAGE_SUMMARY_FIELD_NAME)) {
                ...
                if (page.getFields() != null && page.getFields().get("sys_contentpostdate") == null && postDate != null) {
