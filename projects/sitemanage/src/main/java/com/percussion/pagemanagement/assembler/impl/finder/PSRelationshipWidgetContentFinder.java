...
public class PSRelationshipWidgetContentFinder extends PSWidgetContentFinder {
  ...
  /**
   * Finds the relationships where the owner is the given ID and the category of the relationships
   * is active assembly.
   *
  ...
  private class ContentItemOrder implements Comparator<ContentItem> {
    ...
    public int compare(ContentItem s1, ContentItem s2) {
      ...
      if (isBlank(m_criteria.widgetName)) return compareUnnamed(s1, s2);

      if (isBlank(s1.getWidgetName()) && isNotBlank(s2.getWidgetName())) return 1;

      if (isNotBlank(s1.getWidgetName()) && isBlank(s2.getWidgetName())) return -1;
    ...
    private int compareUnnamed(ContentItem s1, ContentItem s2) {
      if (s1.getSortrank() != s2.getSortrank()) return s1.getSortrank() - s2.getSortrank();
  ...
  /**
   * Widget criteria is typically used by the widget content finder, where we need to filter the
   * relationships by the specified criteria.
   */
  public static class WidgetCriteria {
    ...
    public WidgetCriteria(PSWidgetInstance widget) throws PSNotFoundException {
      ...
      try {
        if (StringUtils.isEmpty(ctType)) contentTypeId = -1L;
        else contentTypeId = PSItemDefManager.getInstance().contentTypeNameToId(ctType);
  ...
  /**
   * Inherit the shared relationship finder utility class, but override the {@link
   * #queryRelationships(IPSAssemblyItem, IPSGuid)}.
   *
   * @author YuBingChen
   */
  private class ContentFinderUtils extends PSRelationshipFinderUtils<WidgetCriteria> {
    ...
    protected boolean isTargetRelationship(
      ...
      if (!equalsIgnoreCase(criteria.widgetName, relWidgetName)) return false;

      return matchContentType(rel, criteria);
    }

    /**
     * Determines if the finding method is matching by widget / asset name or not. When rendering
     * template assets, it always matches the widget ID to the slot id of the relationship. However,
     * when rendering a page asset, it will try to match the named widget to named page asset(s).
     *
     * @param params the parameters passed to the finder, it may be <code>null</code> or empty.
     * @return <code>true</code> if matching by name (while rendering with page assets); otherwise
     *     matching by widget ID (while rendering with template assets).
     */
  ...
  private IPSCmsObjectMgr getCmsMgr() {
    if (m_cmsMgr != null) return m_cmsMgr;
  ...
  /**
   * The finder utility object, never <code>null</code>. It is initialized by {@link
   * #init(IPSExtensionDef, File)}, and never modified after that.
   */
