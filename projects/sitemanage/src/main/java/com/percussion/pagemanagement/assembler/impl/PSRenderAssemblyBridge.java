...
public class PSRenderAssemblyBridge implements IPSRenderAssemblyBridge {
  ...
  public String renderPage(PSPage page, boolean editMode, boolean scriptsOff)
    ...
    notEmpty(page.getId());
    IPSAssemblyItem ai =
        getWorkItemForPreview(page.getId(), null, page, true, scriptsOff, EditType.PAGE);
  ...
  private String assemble(IPSAssemblyItem work)
      throws RepositoryException, PSTemplateNotImplementedException, PSAssemblyException,
          PSFilterException, IOException {
  ...
  /**
   * Calls then adds the supplied template and page to the result if they are not <code>null</code>.
   *
  ...
  public IPSAssemblyItem getWorkItemForPreview(
    ...
    try {
      ...
      throw new PSRenderAssemblyBridgeException("Failed to get site for page: " + id.toString(), e);
    }
