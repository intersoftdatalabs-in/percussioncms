...
public class PSRegionTree extends PSRegionWidgetAssociations {
   ...
   * empty, then the 1st element of the returned list is the most outer region and there is only one
   * outer region for a valid region tree.
   *
   * @return the list of descendant regions. It may be empty if there is no child regions, never
   *     <code>null</code>.
   * @exception IllegalStateException if there is more than one direct child region node under the
   *     root node.
   */
