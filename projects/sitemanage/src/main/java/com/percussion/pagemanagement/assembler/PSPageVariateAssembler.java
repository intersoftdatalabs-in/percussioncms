...
/**
 * This is used to assemble a page. Similar with {@link PSPageAssembler}, but this will use a
 * predefined template to render the page, it does not use the template that is registered to the
 * page. The template ID can be specified by {@link
 * com.percussion.system.utils.IPSHtmlParameters#SYS_VARIANTID} or {@link
 * com.percussion.system.utils.IPSHtmlParameters#SYS_TEMPLATE}
 *
 * <p>This assembler is typically used to render a page in non-HTML format, such as XML.
 *
 * @author YuBingChen
 */
public class PSPageVariateAssembler extends PSPageAssembler {
   ...
   * boolean) PSPageAssemblyContextFactory.createContext(IPSAssemblyItem, TemplateAndPage, false)}
   */
  @Override
  ...
  private IPSAssemblyTemplate getAssemblyTemplate(IPSAssemblyItem assemblyItem) {
    ...
    if (isBlank(variantid) && isBlank(sys_template)) {
      throw new IllegalStateException(
          "Cannot find template ID from the parameter of \""
    ...
    try {
      ...
      final int id =
          variantid != null ? Integer.parseInt(variantid) : Integer.parseInt(sys_template);
