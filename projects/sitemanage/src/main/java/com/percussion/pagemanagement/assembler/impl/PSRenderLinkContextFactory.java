...
public class PSRenderLinkContextFactory implements IPSRenderLinkContextFactory {
  ...
  public PSAssemblyRenderLinkContext create(PSContentListItem listItem, IPSLinkableItem item)
    ...
    linkContext.setDeliveryContext(
        true); // content list item only need to get the publish location, never linked url/location
  ...
  /**
   * Gets the site from the specified site ID if not <code>null</code>; otherwise gets the site from
   * the specified linkable item.
   *
   * @param siteId the site ID, may be <code>null</code>.
   * @param item the linkable item, assumed not <code>null</code>.
   * @return the site, which may be {@link PSNullSiteSummary} if the site is unknown from both
  ...
  private PSSiteSummary getSite(IPSGuid siteId, IPSLinkableItem item)
    ...
    if (rvalue == null) rvalue = PSNullSiteSummary.getInstance();
    return rvalue;
  }
}
