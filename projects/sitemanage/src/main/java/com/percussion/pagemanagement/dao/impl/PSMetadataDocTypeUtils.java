 ...
 * from the client before saving it in DB. Also provides a default doc type to set when adding a new
 * template.
 *
...
public class PSMetadataDocTypeUtils {
  ...
  /**
   * Convert the doc type value stored in the DB and generate a PSMetadataDocType object, that will
   * be returned to client. So when PERC_XHTML -> xhmtl, PERC_HTML5 -> html5. Any other value is
   * treated as custom value, and in that case, it sets the selected value to custom.
   *
   * @param docTypeValue String value that it is stored in DB for a given template, assumed not
   *     <code>null</code>.
   */
  ...
  /**
   * Provide the doc type to store in the DB. It is used when a new template is added in the system.
   * For newly templates, the default doc type is HTML5.
   */
  ...
  /**
   * Provide the default doc type to set when a new template is created. For newly templates, the
