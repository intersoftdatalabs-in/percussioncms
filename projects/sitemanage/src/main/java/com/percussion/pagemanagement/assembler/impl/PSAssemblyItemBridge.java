...
/**
 * Manipulates and converts assembly items to resource objects. <strong>This class is private and
 * should not be used outside of legacy assembly.</strong>
 *
 * <p>This is very loosely based on the bridge pattern as we are separating the legacy AssemblyItem
 * from our new domain objects.
 *
...
public class PSAssemblyItemBridge {
  ...
  protected PSAssemblyItemBridge(
      ...
      IPSResourceLinkAndLocationService resourceLinkandLocationService,
      IPSRenderLinkService renderLinkService,
      IPSFolderHelper folderHelper,
      IPSAssetService assetService,
      IPSIdMapper idMapper,
      PSItemIdResolver itemIdResolver,
  ...
  /**
   * Creates a resource instance from a content list item for generating locations during content
   * list generating
   *
   * @param item never <code>null</code>.
   * @param resourceId the unique resource ID. It may be blank or <code>null</code> if the resource
   *     is determined by the content type of the item.
   * @return never <code>null</code>.
   */
  public PSResourceInstance createResourceInstance(PSContentListItem item, String resourceId)
    ...
    if (isBlank(resourceId)) {
      PSAssetResource resourceDef =
          resourceDefinitionService.findDefaultAssetResourceForType(linkableItem.getType());
  ...
  /**
   * Convenience method that gets the first resource location from an assembly item. If the resource
   * is paginated this would only return the first page.
   *
   * @param listItem never <code>null</code>.
   * @param resourceId the resource ID. It may be blank if the resource is determined by the content
   *     type of the item.
   * @return never <code>null</code>.
   */
  public PSResourceLocation getResourceLocation(PSContentListItem listItem, String resourceId)
    ...
    List<PSResourceLinkAndLocation> linkAndLocations =
        resourceLinkandLocationService.resolveLinkAndLocations(resource);
  ...
  public PSResourceInstance createResourceInstance(IPSAssemblyItem assemblyItem)
    ...
    try {
      return resourceLinkandLocationService.createResourceInstance(
          context, linkableItem, resourceDefinitionId);
  ...
  public PSResourceLocation getResourceLocation(IPSAssemblyItem assemblyItem)
      throws PSAssemblyException, IPSAssetService.PSAssetServiceException {
   ...
   *
   * <p><em>Notice that the return value maybe null</em>
   *
   * @param item never <code>null</code>.
  ...
  /**
   * Tells the assembly item to use the resource assembly result expander during publishing.
   *
   * @param item
  ...
  public void setAssemblyResultExpander(IPSAssemblyItem item) {
    item.setParameterValue(
        IPSAssemblyResultExpander.ASSEMBLY_RESULT_EXPANDER_PARAM,
  ...
  /**
   * Finds the resources definitions associated with the given assembly items <em>content type</em>.
   * For page content type, only return the primary resource.
   *
   * @param assemblyItem never <code>null</code>.
  ...
  public List<PSAssetResource> getResourceDefinitions(IPSAssemblyItem assemblyItem)
    ...
    if (IPSPageService.PAGE_CONTENT_TYPE.equals(contentType)) {
      PSAssetResource pageResource =
          resourceDefinitionService.findDefaultAssetResourceForType(contentType);
  ...
  protected PSRenderLinkContext getRenderLinkContext(
      IPSAssemblyItem assemblyItem, IPSLinkableItem item) throws PSAssemblyException {
    try {
  ...
  public static void setDeliveryLocation(IPSAssemblyItem workitem, String location) {
    ...
    String oldContext = workitem.getParameterValue(IPSHtmlParameters.SYS_CONTEXT, null);

    workitem.setParameterValue(
        IPSHtmlParameters.SYS_CONTEXT, String.valueOf(workitem.getDeliveryContext()));

    workitem.setDeliveryPath(location);

    workitem.setParameterValue(IPSHtmlParameters.SYS_CONTEXT, oldContext);
  }

  /**
   * Gets the page and template from an assembly item.
   *
   * <p>If the assembly item is a template then a {@link PSEmptyPage} will be used for the page. The
   * cached results of this method will be stored in the assembly item so there is no performance
   * penatly for call this method multiple times on an assembly item.
   *
   * @param item never <code>null</code>.
   * @return never <code>null</code>.
   * @throws RepositoryException
   */
  public TemplateAndPage getTemplateAndPage(IPSAssemblyItem item)
      throws RepositoryException, PSDataServiceException {
    String contentType =
        ((IPSNodeDefinition) ((IPSNode) item.getNode()).getDefinition()).getInternalName();
    log.debug(
        "Getting template and page for assembly item: "
            + item.getId()
            + " of type: "
            + contentType);
  ...
  /**
   * Container that holds the template and page corresponding to a rendering request (assembly
   * item).
   *
   * @author adamgent
   */
  public static class TemplateAndPage {
     ...
     *
     * <p>If its a {@link ItemType#TEMPLATE} then {@link #getPage()} will be a {@link PSEmptyPage}.
     *
     * @return never <code>null</code>.
     */
