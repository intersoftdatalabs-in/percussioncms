...
public interface IPSRenderService {
   ...
   * Sets the edit mode flag. See {@link #renderPageForEdit(String)} for details of this flag.
   *
   ...
   *     regionId</code> is set on the returned object.
  ...
  /**
   * Similar to the {@link #renderTemplate(String)}, except sets a scriptsOff variable as true to
   * the context. So that the macros can render the template by stripping the script tags.
   *
   * @param id, the string format of template item guid.
   * @return The rendered template, typically an (x)html document. Never <code>null</code> or empty.
   * @throws PSRenderServiceException
   */
   ...
   *
   * @param id The unique identifier of the page to be rendered. Not blank. (TODO: ph - as a user of
   *     the API, where do I get one of these ids?)
   * @return The rendered page, typically an (x)html document. Never <code>null</code> or empty.
   * @throws PSRenderServiceException
   */
  public String renderPageForEdit(String id, String editType) throws PSRenderServiceException;

  /**
   * Similar to {@link #renderPageForEdit(String)}, this method sets scripts off flag to the $perc
   * context. So that the velocity macros can strip the scripts if the flag is set to false.
   *
   * @param id, the string format of the guid of the page.
   * @return The rendered page, typically an (x)html document. Never <code>null</code> or empty.
   * @throws PSRenderServiceException
   */
  public String renderPageForEditScriptsOff(String id, String editType)
      throws PSRenderServiceException;
