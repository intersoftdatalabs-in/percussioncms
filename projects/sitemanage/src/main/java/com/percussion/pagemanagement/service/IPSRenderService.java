...
public interface IPSRenderService {
     ...
     * Sets the edit mode flag. See {@link #renderPageForEdit(String, String)} for details of this flag.
     *
     * @param page     The page to assemble. Not {@code null}.
     * @param regionId The name of the region within the supplied page. Not blank.
     * @return Just the rendered region as a string within the returned object. The supplied {@code regionId} is set on the returned object.
    ...
    /**
     * Similar to {@link #renderTemplate(String)}, except sets a scriptsOff variable as true to the context.
    ...
    /**
     * Similar to {@link #renderPage(String)}, except sets a flag that can be used by widgets if they want or need to render their output differently
     * when a page is being edited. For example, if a widget has no content, it may render some sample content or it may allow in-line editing of its content.
     * Each widget is responsible for what is rendered in this situation.
     *
    ...
    /**
     * Similar to {@link #renderPageForEdit(String, String)}, this method sets scripts off flag to the $perc context.
     * So that the velocity macros can strip the scripts if the flag is set to false.
     *
