...
/**
 * This result expander expands an assembly result into multiple assembly items based on the
 * resource definitions associated with the assembly item's content type.
 *
 * <p>For example an assembly result for an image content type might have two resource definitions:
 * full and thumbnail. Thus full and thumbnail would be expanded to two assembly items with the full
 * and thumbnail resource definition ids added to each assembly item respectivley.
 *
 * <p>This expander will also use the newer link/location services to generate the delivery location
...
public class PSResourceAssemblyResultExpander extends PSAbstractAssemblyResultExpander
  ...
  public List<IPSAssemblyItem> expand(IPSAssemblyResult result) throws Exception {
    List<PSAssetResource> resources =
        new ArrayList<>(getAssemblyItemBridge().getResourceDefinitions(result));
    ...
    Comparator<PSAssetResource> resourceComparator =
        new Comparator<PSAssetResource>() {
          @Override
          public int compare(PSAssetResource s1, PSAssetResource s2) {
    ...
    for (PSAssetResource r : resources) {
      ...
      if (isPaginated) assemblyItem.setPage(i);
