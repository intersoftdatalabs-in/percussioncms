...
/**
 * An adapter to the Template expander extension point in cm system which makes it easier to
 * implement template expanders.
 *
 * <p>{@link #createTemplateCache()}.
 *
 * @author adamgent
...
public abstract class PSAbstractTemplateExpanderAdapter<CACHE> implements IPSTemplateExpander {
  ...
  /**
   * Return a template guid or <code>null</code>. If <code>null</code> the item will not be expanded
   * which means will not be run and the item will be skipped.
   *
  ...
  /**
   * Creates a new template cache that will be used for the current publishing job. Called only once
   * per content list.
   *
  ...
  /**
   * Should return a list that is inclusive of the original item passed if they want the item passed
   * to be published. To create new content list items from the passed item use {@link
   * #clone(PSContentListItem)}.
   *
   ...
   */
  protected abstract List<PSContentListItem> expandContentListItem(
      ...
      while (riter.hasNext()) {
        ...
        if (templateId != null) {
          PSContentListItem item =
              createContentListItem(contentId, folderId, templateId, siteId, context);
  ...
  protected PSContentListItem createContentListItem(
      IPSGuid contentId, IPSGuid folderId, IPSGuid templateId, IPSGuid siteId, Integer context) {
  ...
  protected IPSGuid getContentItemGuid(Row r, Map<Integer, PSComponentSummary> summaryMap)
