...
/**
 * Low level api that works directly with Rhythmyx Assembly engine. All render methods assume
...
public interface IPSRenderAssemblyBridge {
  ...
  /**
   * Just like {@link #renderPage(String, boolean)}, except an object is supplied rather than the id
   * of an existing object. TODO: ph - does the page need to exist in this case?
   *
   * @param page never <code>null</code>.
   * @param editMode
   * @param scriptsOff, a boolean scripts off flag that is bind to the context so that it is
   *     available to the macros and macros then make use of this variable to strip the script tags.
   ...
   *
   * @param id not blank. TODO - ph - what format is expected (string format of a {@link IPSGuid}?)
   * @param editMode If <code>true</code> and a widget on the page has no content, it can optionally
   *     add some content to aid the viewer; or it may render in such a way as to allow in-line
   *     editing. Whether a widget will add sample content is up to the widget developer.
   * @param scriptsOff, a boolean scripts off flag that is bind to the context so that it is
   *     available to the macros and macros then make use of this variable to strip the script tags.
   * @return never <code>null</code>.
   */
  ...
  /**
   * This is the same as {@link #renderPage(String, boolean, boolean)}, in addition, it provides an
   * option to specify the edited item type.
   *
   ...
   * @param editType the edited item type. This is only useful if {@param editMode} is <code>true
   *     </code>.
