...
@Lazy
public class PSPageService extends PSAbstractDataService<PSPage, PSPage, String>
    implements IPSPageService {

  private final IPSPageDao pageDao;

  /** The page dao helper. Initialized by constructor, never <code>null</code> after that. */
  ...
  /** The publishing webservice, initialized by constructor, never <code>null</code> after that. */
  ...
  public PSPageService(
      ...
      @Qualifier("templateDao") IPSTemplateDao templateDao,
      IPSiteDao siteDao,
  ...
  public void delete(String id) throws PSValidationException {
    ...
    if ((sites != null) && (sites.size() > 1)) {
      PSSiteCopyUtils.throwCopySiteMessageIfNotAllowed(
          sites.get(0).getName(), "delete", PSSiteCopyUtils.CAN_NOT_DELETE_PAGE);
    }
    delete(id, false);
  }
  ...
  public String copy(String id, String targetFolder, boolean addToRecent)
      throws PSDataServiceException, IPSPathService.PSPathNotFoundServiceException {
    PSPage page;
    page = load(id);
    ...
    if (targetFolder != null) path = targetFolder;
    ...
    save(newPage);
    if (addToRecent) recentService.addRecentItem(newPage.getId());
    ...
    widgetAssetRelationshipService.removeAssetWidgetRelationships(
        newPageId, Collections.emptyList());
  ...
  public void delete(String id, boolean force, boolean purgeItem) throws PSValidationException {
    ...
    try {
      ...
      if (purgeItem) {
        ...
        // CMS-9013 : if the site has been deleted and the page is still in recycle bin somehow
        // (might be bad data due to copy site).
        // folder path is empty so do not process the folder-path. Also, the purgeItemPaths variable
        // is only used for logging purpose.
        if (summ.getFolderPaths() != null && !summ.getFolderPaths().isEmpty()) {
          purgeItemPaths.append(
              summ.getFolderPaths().get(0).replaceFirst("//Folders/\\$System\\$", ""));
          purgeItemPaths.append("/");
          purgeItemPaths.append(summ.getName());
        } else {
          log.debug(
              "FolderPath not found for Page: '{}'. Seems Site for Page : '{}' has been deleted. Page being sent for purging by user: {}",
              summ.getName(),
              summ.getName(),
              currentUser);
        }
  ...
  public PSPagedItemList findPagesByTemplate(
    ...
    for (IPSItemEntry pageEntry : pagedItemEntries) {
      ...
      pathItem.setTypeProperty(
          "contentMigrationVersion", page.getTemplateContentMigrationVersion());
    ...
    PSPagedItemList pagesByTemplatePagedList =
        new PSPagedItemList(itemsInPage, allPageEntries.size(), resultingStartIndex);

    if (!allPageEntries.isEmpty()) {
      IPSGuid myGuid =
          PSGuidUtils.makeGuid(allPageEntries.get(0).getContentId(), PSTypeEnum.LEGACY_CONTENT);
   ...
   *
   * @param sitename {@link String} with the name of the site. Must not be <code>null</code> nor
   *     empty.
  ...
  /**
   * Finds the startIndex (first index of the paging group) that contains the page with the id sent
   *
   * @param allItems The list of items to search the corresponding item never <code>null</code>.
   * @param maxResults The results page size , never <code>null</code> and equal or greater than 1.
   * @param childPageId The page item id to find in the list to get the correct page, never <code>
   *     null</code>.
   * @return Integer the start index of the page group that contains the item
   */
  private Integer getStartIndexForItem(
      List<IPSItemEntry> allItems, Integer maxResults, Integer childPageId) {
  ...
  /**
   * Finds the index of the item in the list. It compares using the contentId for the page
   *
   * @param childPageId The page item id to find, never <code>null</code>.
   * @param list The list of items to search the corresponding item, never <code>null</code>.
  ...
  private Integer getChildIndex(int childPageId, List<IPSItemEntry> list) {
    ...
    for (index = 0; index < list.size(); index++) {
      ...
      if (item.getContentId() == childPageId) break;
    }
  ...
  public PSPage save(PSPage page) throws PSDataServiceException {
    ...
    if (template == null)
      throw new RuntimeException(
          "The template you have selected doesn't exist in the system. Please refresh and try again.");
  ...
  public String getPageEditUrl(String id) {
    ...
    String url =
        contentDesignWs.getItemEditUrl(
            idMapper.getGuid(id), PAGE_CONTENT_TYPE, IPSConstants.SYS_HIDDEN_FIELDS_VIEW_NAME);
    return fixUrl(url);
  }
  ...
  public String getPageViewUrl(String id) {
    ...
    String url =
        contentDesignWs.getItemViewUrl(
            idMapper.getGuid(id), PAGE_CONTENT_TYPE, IPSConstants.SYS_HIDDEN_FIELDS_VIEW_NAME);
  ...
  public PSNoContent validateDelete(String id) throws PSValidationException {
    ...
    try {
      ...
      log.debug(
          "Page: {} not found for delete validation. Error: {}",
          id,
  ...
  public List<PSSEOStatistics> findNonSEOPages(PSNonSEOPagesRequest request)
    ...
    List<PSPageSummary> sums =
        pageDao.findPagesBySiteAndWf(
            PSPathUtils.getFolderPath(request.getPath()), workflowId, stateId);
    for (PSPageSummary sum : sums) {
      PSSEOStatistics stats =
          new PSSEOStatistics(
              sum, PSPathUtils.getFinderPath(sum.getFolderPath()) + '/' + sum.getName(), keyword);
  ...
  /**
   * Trims the specified keyword such that multiple spaces within the keyword are condensed into
   * one.
   *
  ...
  /**
   * This is used to validate a specified {@link PSPage} object to make sure the object can be used
   * to create a page if the ID is <code>null</code> or such page exists in the specified folder if
   * the ID is not <code>null</code>.
   */
  private class PSPageValidator extends PSAbstractBeanValidator<PSPage> {

    private List<String> reservedPageNames =
        Arrays.asList(".htdocs", ".htaccess", "pspagemetadatascanner.sfv");
    ...
    protected void doValidation(PSPage obj, PSBeanValidationException e) {
      ...
      if (!validatePageReservedName(name)) {
        String msg =
            "Cannot create or rename page \""
                + name
                + "\" to \""
                + name
                + "\" because that name is a reserved page name.";
        log.debug(msg);
      ...
      if (!validatePageName(name)) {
        String msg =
            "Cannot create or rename page \""
                ...
                + "\" to \""
                + name
                + "\" because that name is an invalid page name.";
        log.debug(msg);
      ...
      if (existingId != null) {
        String msg =
            "Cannot create a page with name \""
                + name
                + "\" because a page or folder named \""
                + name
      ...
      if (!validateMimeType(name)) {
        String msg =
            "Cannot create a page with name \""
                + name
                + "\" because it has an unknown mime type as file extension.";
        log.debug(msg);
   ...
   */
  private void validateForDelete(PSPage page, PSValidationErrorsBuilder builder)
    ...
    if (!itemWorkflowService.isModifiableByUser(id)) {
      builder.reject(
          "page.deleteNotAuthorized",
          "The current user is not authorized to delete this " + "page");
      builder.throwIfInvalid();
    }

    if (widgetAssetRelationshipService.isUsedByTemplate(id)) {
      builder.reject(
          "page.deleteTemplates",
  ...
  /**
   * Changes url from beginning with ../ to /Rhythmyx/ This is need because ../ now puts you in /cm/
   * and some requests still need to go to /Rhythmyx/
   *
  ...
  /**
   * Validator to validate the page before {@link #save(PSPage)} and on {@link #validate(PSPage)},
  ...
  private PSRegionsValidator<PSPage> regionsValidator =
      new PSRegionsValidator<PSPage>() {
  ...
  public static class PSPageRegionWidgetValidator
      extends PSRegionWidgetAssociationsValidator<PSPage> {
    ...
    public PSRegionWidgetAssociations getWidgetAssociations(
        PSPage wa, PSBeanValidationException e) {
  ...
  public IPSPageCatalogService getCatalogService() {
    if (catalogService == null) {
      catalogService =
          (IPSPageCatalogService) getWebApplicationContext().getBean("pageCatalogService");
    }
  ...
  private class PSUnassignedPagesDataHelper {
    ...
    public PSUnassignedResults getUnassignedResults(String siteName, int startIndex, int maxResults)
        throws Exception {
      // get mock data
      ...
      return buildUnassignedResults(
          catalogedIds, importedIds, importingIds, startIndex, maxResults);
    }

    private PSUnassignedResults createMockResponse(
        String sitename, int startIndex, int maxResults) {
    ...
    /**
     * Builds the result with the given list of ids. Paginates the results according to the given
     * start index, and max results parameters.
     *
     * @param catalogedIds {@link List}<{@link Integer}> list of the cataloged pages ids. Assumed
     *     not <code>null</code>.
     * @param importedIds {@link List}<{@link Integer}> list of the imported pages ids. Assumed not
     *     <code>null</code>.
     * @param importingIds {@link Integer} may be <code>null</code>.
     * @param startIndex {@link Integer} assumed not <code>null</code> and greater than 0.
     * @param maxResults {@link Integer} assumed not <code>null</code> and greater than 0.
     * @return {@link PSUnassignedResults} with the results, never <code>null</code>
     */
    private PSUnassignedResults buildUnassignedResults(
        List<Integer> catalogedIds,
        List<Integer> importedIds,
        List<Integer> importingIds,
        Integer startIndex,
        Integer maxResults)
      ...
      // build Item list
      PSPagedObjectList<Integer> pageGroup =
          PSPagedObjectList.getPage(pageIds, startIndex, maxResults);
      ...
      UnassignedItemList itemList =
          new UnassignedItemList(pageGroup.getStartIndex(), pageGroup.getChildrenCount(), items);
   ...
   *
   * @param id {@link Integer} assumed not <code>null</code> and greater than 0.
   * @param importingIds {@link Integer} may be <code>null</code>.
   * @param importedIds {@link List}<{@link Integer}> list of the imported pages ids. Assumed not
   *     <code>null</code>.
   * @return {@link UnassignedItem} the unassigned item. Never <code>null</code>, or empty.
   */
  private UnassignedItem getUnassignedPage(
      Integer id, List<Integer> importingIds, List<Integer> importedIds) throws PSPageException {
    try {
   ...
   *
   * @param path {@link String} the path to the target item, the assumed not <code>null</code> and
   *     greater than 0.
   * @param isImported {@link Boolean} flag to indicate if the item was already imported.
   * @return {@link String} the link title for the item. Never <code>null</code>, or empty.
   */
  private String getLinkTitleForUnassignedPage(String path, boolean isImported)
      throws PSPageException {
    String pagePath = path;

    if (isImported) pagePath = getCatalogService().convertToImportedFolderPath(path);
   ...
   *
   * @param id {@link String} assumed not <code>null</code> and greater than 0.
   * @param importingIds {@link String} assumed not <code>null</code> and greater than 0.
   * @param isImported {@link Boolean} flag to indicate if the item was already imported.
  ...
  /**
   * Generates a name for a new page, using site-wide naming conventions. It avoids collision with
   * existing pages appending a numerical value to the end if necessary (Example: New-Page-2)
   *
   * @param pageName The initial name for the page. It will be kept unchanged if no such page exists
   *     in the folder with folderPath. Otherwise, a numerical suffix will be added to avoid naming
   *     collision.
   * @param folderPath The path of the folder where the page will be created. Used to check for
   *     existing pages that might have the same name.
   * @return String The automatically generated name for the page. Never null or empty.
   */
  ...
  @Override
  public List<PSPageReportLine> findAllPages(String sitePath)
