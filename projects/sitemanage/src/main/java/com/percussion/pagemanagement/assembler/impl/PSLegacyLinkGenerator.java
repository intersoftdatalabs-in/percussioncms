...
/**
 * A wrapper around {@link PSLocationUtils}. This is the bridge between the legacy location scheme
 * generator and the new render link service.
 *
 * <p><em>This component only generates preview links</em>
 *
 * @author adamgent
 */
...
public class PSLegacyLinkGenerator {
  ...
  /**
   * Generates logical link using the legacy location generator. <strong>This should only be used
   * for preview links</strong>
   *
   ...
   */
  @SuppressWarnings("deprecation")
  public void addLegacyDataToInlineLink(
      PSInlineRenderLink renLink, PSInlineLinkRequest request, PSAsset asset) {
    notNull(asset, "asset");
    notNull(renLink, "renLink");
    if (IMAGE_ASSET_CONTENTTYPE.equals(asset.getType())
        && isNotBlank(renLink.getThumbResourceDefinitionId())) {
  ...
  private void fillInlineLinkHelper(PSInlineRenderLink renLink, String targetId) {
    notNull(renLink.getResourceDefinition(), "The resource definition should be loaded by now.");
      PSAssetResource resourceDef = (PSAssetResource) renLink.getResourceDefinition().orElse(null);
  ...
  private void fillInlineImageLink(PSInlineRenderLink renLink, PSInlineLinkRequest request) {
      PSAssetResource resourceDef = (PSAssetResource) renLink.getResourceDefinition().orElse(null);
    PSAssetResource thumbResourceDef = (PSAssetResource) renLink.getThumbResourceDefinition().orElse(null);
   ...
   *
   * <p>This should be called after the link object is created.
   *
  ...
  public void fillLegacyLink(
      ...
      PSLegacyLink link)
      throws ValidationException, PSException {
    notNull(legacyContext);
    notNull(link);
    notNull(resourceDefinition);
    notNull(item);
    isTrue(
        legacyContext instanceof PSAssemblyRenderLinkContext,
  ...
  public static class PSLegacyLink {
    ...
    @NotNull @NotBlank private String template;

    @NotNull @NotBlank private String legacyTemplate;

    @NotNull private Node node;

    @NotNull @NotBlank private String folderPath;

    @NotNull @NotBlank private String filter;

    @NotNegative @NotNull private Number siteId;

    @NotNegative @NotNull private Number context;

    @NotNull @NotBlank private String resourceDefinitionId;
