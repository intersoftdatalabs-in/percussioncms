...
/**
 * Retrieves {@link PSWidgetDefinition} and validates {@link PSWidgetItem} against their
 * corresponding {@link PSWidgetDefinition}.
 *
 * @author adamgent
 */
public interface IPSWidgetService
    extends IPSDataService<PSWidgetDefinition, PSWidgetSummary, String> {

  /** Enum of widgets that must be disabled for SaaS Install */
  public static enum SAAS_WIDGETS {
    LOGIN("percLogin"),
  ...
  /**
   * @param baseTemplate The name of the base template for the widgets, never <code>null</code>.
   *     Assigned through spring property.
   */
   ...
   *
   * @param type The type of the widgets, widget types are set to one of the PSWidgetTypeEnum.
   * @param filterDisabledWidgets if the value is equals case in sensitive "yes", then disabled
   *     widgets are filtered from returned list
   * @return List of widget summaries never <code>null</code> may be empty.
   */
   ...
   *
   * @param request A list of package names, not <code>null</code>, may be empty in which case an
   *     empty list is returned.
   * @return A list of results for each widget that package info was found. If package info for a
   *     widget is not found, not result for that widget is included in the results.
   */
