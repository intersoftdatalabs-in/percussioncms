 ...
 *
 * <p>Its responsibilities are to creates resource instances and manage executing them (getting
 * links and one day output).
 *
...
public class PSResourceInstanceHelper {
  ...
  /**
   * Executes a resource instance with the given script. The script should return either a list of
   * {@link PSResourceLinkAndLocation} or a single {@link PSResourceLinkAndLocation}. Mulptile
   * {@link PSResourceLinkAndLocation}s will indicate pagination.
   *
  ...
  /**
   * Create the resource script evaluator context. Usually this object is bound to <code>$perc
   * </code>
   *
  ...
  private PSSiteSummary resolveSite(PSResourceInstance r)
    ...
    // thus siteDataService.findByPath throws DataServiceNotFoundException which causes publishing
    // of resources to fail
  ...
  public PSResourceInstance createResourceInstance(
      ...
      throws IPSAssetService.PSAssetServiceException, DataServiceNotFoundException,
  ...
  /**
   * Loads the specified asset, which includes summary properties only. It does not include fields
   * relate to Clob or Blob columns. The returned asset is typically used to create links, but
   * should not be used to render the asset.
   *
  ...
  /**
   * Gets links from the resource instance by executing its link and location script.
   *
   * @param r never <code>null</code>.
   * @return never <code>null</code>, maybe empty.
   */
  public List<PSResourceLinkAndLocation> getLinkAndLocations(PSResourceInstance r)
    ...
    log.warn(
        "Resource definition: " + r.getResourceDefinition() + " does not generate any links. ");
    return emptyList();
  }
  ...
  /**
   * If the site is relative (not cross site) we extract the path from the base url.
   *
   * <p>That is the host information is removed.
   *
  ...
  public PSSiteSummary getSiteForPageId(String pageId)
      throws IPSDataService.DataServiceLoadException, DataServiceNotFoundException,
          PSValidationException {
    /*
