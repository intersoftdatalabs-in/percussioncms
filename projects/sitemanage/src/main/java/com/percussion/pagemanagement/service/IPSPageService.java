...
public interface IPSPageService extends IPSDataService<PSPage, PSPage, String> {
   ...
   *
   * @param fullPath folderPath + path, never <code>null</code>, empty, or blank.
   * @return A {@link PSPage} object. May be <code>null</code> if the page could not be found.
   * @throws PSPageException if an error occurs finding the page.
   */
  PSPage findPageByPath(String fullPath) throws PSPageException, PSValidationException;

  /**
   * Find all the pages that use a certain template and return the results paged.
   *
   * @param templateId The id of the template, from the UI, never <code>null</code>.
   * @param startIndex The starting index of the pagination. The resulting page of data will contain
   *     pages from startIndex to startIndex+maxResults, never <code>null</code>.
   * @param maxResults The results page size that the client paginator will handle, never <code>null
   *     </code> and equal or greater than 1.
   * @param sortColumn The results will be ordered by this attribute before being paged. If not
   *     specified, name will be used as default.
   * @param sortOrder Asc, or Desc order. If not specified, asc will be set as default.
   * @param pageId The page item id to find in the list to get the correct page, may be <code>null
   *     </code>
   * @return PSPagedItemList a paged item list of pathItems. These pathItems contain name and full
   *     path for the pages that use the template.
   ...
   *
   * @param id the ID of the page, never <code>null</code> or empty.
   * @param force <code>true</code> to delete the page even if it is being edited by another user,
   ...
   * @param force <code>true</code> to delete the page even if it is being edited by another user,
   *     <code>false</code>
   * @param purgeItem <code>true</code> if the item should be purged. <code>false</code> if it
   *     should be recycled. otherwise.
   */
  ...
  /**
   * Determines if the supplied item is one of the pages or its content type is {@link
   * #PAGE_CONTENT_TYPE}.
   *
  ...
  /**
   * At present the page meta-data saving happens through the content editor. Adding a dummy service
   * to get notified on page meta-data save.
   *
   * <p>TODO: - The page meta-data save needs to be rerouted from this method to the content editor,
   * at that time depending on the implementation the signature needs to be updated.
   *
