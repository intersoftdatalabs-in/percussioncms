...
public class PSPageDaoHelper implements IPSPageDaoHelper {
    ...
    public Collection<Integer> findPageIdsByTemplate(String templateId) {
        var sess = getSession();
        try {
            var sql = "select distinct CONTENTID from " + qualifyTableName(PAGE_TABLE) + " where TEMPLATEID = :template";
    ...
    public Collection<Integer> findPageIdsByTemplateInRecentRevision(String deletedTemplate) {
        ...
        var sess = getSession();
        try {
            var sql = "select distinct P.CONTENTID " +
            ...
            log.error("Failed to get the fully qualified table name for {}. Error: {}", PAGE_TABLE, PSExceptionUtils.getMessageForLog(e));
    ...
    /**
     * Update old page revisions that used the deleted template to use the one
     * that is being used in the current revision of the page.
     */
    @Transactional
    ...
    public Map<String, String> findTemplateUsedByCurrentRevision(List<Integer> pages) {
        ...
        var sess = getSession();
        try {
            var sql = "SELECT P.CONTENTID, P.TEMPLATEID " +
    ...
    public Collection<Integer> findPageIdsByTemplateAndImportedPageIds(String templateId, List<Integer> pages) {
        var sess = getSession();
        try {
            var sql = "select distinct CONTENTID from " + qualifyTableName(PAGE_TABLE) + " where TEMPLATEID ='" +
    ...
    public Map<String, String> findLinkTextForCurrentRevision(List<Integer> pages) {
        ...
        var sess = getSession();
        try {
            var sql = "SELECT P.CONTENTID, P.RESOURCE_LINK_TITLE " +
                    "FROM " + qualifyTableName(PAGE_TABLE) +
                    " AS P INNER JOIN " + qualifyTableName(CONTENT_TABLE) +
                    " AS CS ON P.CONTENTID = CS.CONTENTID " +
                    "WHERE P.CONTENTID IN (" + join(pages, ",") + ") " +
