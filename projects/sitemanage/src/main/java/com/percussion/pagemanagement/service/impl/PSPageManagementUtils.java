...
public class PSPageManagementUtils {
   ...
   * @return The new name, consisting of initial name, a separator and a count modifier. Example:
   *     "New_Template-3".
   */
  ...
  /**
   * Create a Raw HTML widget item, and its corresponding asset, with the given widget slot id (or
   * widget id). The asset is saved into the system.
   *
  ...
  /**
   * Extracts scripts after body start and before body end from the document body in memory and sets
   * them to afterBodyStart and beforeBodyClose in pageContent. Removes title from the head of
   * imported document in memory and updates headContent in pageContent.
   *
   * @param pageContent Current page content in memory to process
   * @param logger {@link IPSSiteImportLogger} to log the commented tags. Assumed not <code>null
   *     </code>.
   */
   ...
   *
   * @param bodyElems {@link Elements} with the elements that belong to the body. Assumed not <code>
   *     null</code>.
   * @param logger {@link IPSSiteImportLogger} to use. Assumed not <code>null</code>.
   * @return {@link StringBuilder}, never <code>null</code> but may be empty.
   */
  private static StringBuilder extractBeforeBodyClose(
      Elements bodyElems, IPSSiteImportLogger logger) {
    Elements beforeBodyCloseElems = new Elements();
    for (int i = bodyElems.size(); i > 0; i--) {
      Element element = bodyElems.get(i - 1);
      if (!element.tagName().equalsIgnoreCase("script")) break;
    ...
    for (int j = beforeBodyCloseElems.size(); j > 0; j--) {
      ...
      if (PSManagedTagsUtils.isManagedJSReference(element)) {
        ...
        logger.appendLogMessage(
            ...
            IPSImportHelper.COMMENTED_JS_REFERENCE_FROM_BODY,
            element.toString());
      } else {
   ...
   * @return {@link StringBuilder}, never <code>null</code> but may be empty.
   */
  private static StringBuilder extractAfterBodyStartContent(
      Elements bodyElems, IPSSiteImportLogger logger) {
  ...
  /**
   * Process the header element to comment out those tags that are managed by CMS.
   *
   * @param pageContent The page content being modified.. Assumed not <code>null</code>.
   * @param logger {@link IPSSiteImportLogger} to log the commented tags. Assumed not <code>null
   *     </code>.
   * @return {@link String} with the html code from the header element. Never <code>null</code> but
   *     may be empty.
   */
  private static String commentOutManagedTags(
      PSPageContent pageContent, IPSSiteImportLogger logger) {
    Element docHead = pageContent.getSourceDocument().head();

    // first comment the title tag
    for (Element title : docHead.select("title")) {
      logger.appendLogMessage(
          PSLogEntryType.STATUS, IPSImportHelper.COMMENTED_OUT_ELEMENT, title.toString());
  ...
  /**
   * Comments out the managed js referenced from the element passed as parameter.
   *
   * @param element {@link Element} to comment the references from. Assumed not <code>null</code>.
  ...
  private static void commentManagedJSReferences(Element element, IPSSiteImportLogger logger) {
    ...
    for (Element scriptTag : scriptTags) {
      if (PSManagedTagsUtils.isManagedJSReference(scriptTag)) {
        logger.appendLogMessage(
            PSLogEntryType.STATUS,
            IPSImportHelper.COMMENTED_JS_REFERENCE_FROM_HEAD,
            scriptTag.toString());
   ...
   *
   * @param pageContent The page content being modified, assumed not <code>null</code>.
   * @param docHead {@link Element} to comment the references from. Assumed not <code>null</code>.
   * @param logger {@link IPSSiteImportLogger} to append the message. Assumed not <code>null</code>.
   */
  private static void commentMetadataTags(
      PSPageContent pageContent, Element docHead, IPSSiteImportLogger logger) {
    ...
    for (Element metaTag : metaTags) {
      if (PSManagedTagsUtils.isManagedMetadataTag(metaTag)) {
        ...
        logger.appendLogMessage(
            PSLogEntryType.STATUS, IPSImportHelper.COMMENTED_OUT_ELEMENT, metaTag.toString());
