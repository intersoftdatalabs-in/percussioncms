...
public class PSTemplateService implements IPSTemplateService {
  ...
  /**
   * Instance of site section service, this service is not autowired by the spring during the
  ...
  public PSTemplate save(PSTemplate object, String siteId, String pageId)
    ...
    Map<String, PSPair<String, String>> changedWidgets =
        getWidgetNamesChanged(template, savedTemplate);
  ...
  /**
   * Update the revision of the supplied template to save with the value in the current template,
   * optionally incrementing the version.
   *
   * @param current The current version of the template object, assumed not <code>null</code>.
   * @param toSave The new version of the template object that will be saved, assumed not <code>null
   *     </code>.
   * @param increment <code>true</code> to increment the version, <code>false</code> to just copy
   *     it.
   */
  ...
  /**
   * Validates the supplied id is a valid id, that a page with that id exists, and that the page is
  ...
  /**
   * Compares the lists of widgets from the old template and the saved one to see if there has been
   * a change in the widget name. A change in a widget name means that the same widget (the same
   * slot id) in both templates has different name. It only takes into the comparison those widgets
   * that are present in the old template and the updated one.
   *
   * @param template {@link PSTemplate} the template object as it was before the update.
   * @param savedTemplate {@link PSTemplate} the template object as after the update.
   * @return {@link Map}<{@link String}, {@link PSPair}<{@link String}, {@link String}>> The first
   *     element is the old name of the widget, and the second value is the new name. Never <code>
  ...
  /**
   * Iterates over the set of region widgets and builds a map, from the widget id (slot id in the
   * relationships) to the widget name.
   *
   * @param list {@link List}<{@link PSWidgetItem}> containing the widget items for a given
  ...
  /**
   * Checks if there are two widgets with the same name, in which case, it throws a {@link
   * DataServiceSaveException}.
   *
   * @param region {@link PSRegionTree} object representing the root region of the template.
   * @throws DataServiceSaveException in case more than one widget have the same name.
   */
  ...
  public static class PSTemplateSorter extends PSAbstractTemplateSorter {
    ...
    /**
     * {@inheritDoc} We use the string of characters after the last <code>.</code> for readonly
     * templates. This will remove our default prefix of:
     *
     * <pre>
     * perc.base
     * </pre>
     *
     * from readonly templates. So a read only template named:
     *
     * <pre>
     * perc.base.Box
     * </pre>
     *
     * Will have a name: <code>Box</code> for sorting. Non-readonly templates (user templates) are
     * sorted by there unalterted name.
     */
    @Override
  ...
  /**
   * It creates a new template with specified templateName for the site with siteId, using
   * baseTemplateName as the base template.
   *
   * @param baseTemplateName The name of the template from which to create the new one. Example:
   *     "perc.base.plain" for plain template in base package.
   * @param templateName The name to use for naming the template, will be displayed in the UI.
   * @param siteId The id of the site to which the template is going to belong. Notice this is not
   *     the legacy long id, it's the String id.
   * @return PSTemplateSummary Class that holds information of the created template.
   */
