...
public class PSPage extends PSPageSummary implements IPSLinkableItem, IPSHtmlMetadata {
   ...
   * template.
   *
   * @param template if not {@code null} excludes widgets from page regions that also have the
  ...
  public List<PSWidgetItem> getWidgets(PSTemplate template) {
    ...
    for (var r : getRegionBranches().getRegions()) {
      // Don't return widgets if template has widgets in a region and page also has widgets in the
      // same region.
