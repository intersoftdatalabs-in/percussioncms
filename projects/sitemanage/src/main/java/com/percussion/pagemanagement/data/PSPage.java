...
public class PSPage extends PSPageSummary implements IPSLinkableItem, IPSHtmlMetadata {
  ...
  public int hashCode() {
    return Objects.hash(
        ...
        getAfterBodyStartContent(),
        getBeforeBodyCloseContent(),
        getProtectedRegion(),
        getProtectedRegionText(),
        getRegionBranches(),
        getSummary(),
        getWorkflowId(),
        getDocType(),
  ...
  /**
   * Gets all widgets contained in this page, which does not include widgets in the underlying
   * template.
   *
   ...
   * template.
   *
   * @param template if not <code>null</code> excludes widgets from page regions that also have the
   *     widgets on template region.
   * @return the widgets, never <code>null</code>, may be empty.
   */
  public List<PSWidgetItem> getWidgets(PSTemplate template) {
    ...
    for (PSRegion r : getRegionBranches().getRegions()) {
      // don't return widgets if template has widgets in a region and page also have widgets in the
      // same region
      // As the page widgets will not get rendered. Also don't return page widgets if the region
      // doesn't exist
  ...
  /**
   * Regions of the template that will be overrided. The overrided region is a list of widgets or
   * regions, never <code>null</code>, maybe empty.
   */
  @NotNull @AssertValid private PSRegionBranches regionBranches = new PSRegionBranches();
  ...
  /**
   * If set to true, the newly created item and its template and folder will be added to the recent
   * list.
   */
