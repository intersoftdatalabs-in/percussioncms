...
public interface IPSTemplateDao extends IPSGenericDao<PSTemplate, String> {
   ...
   * @param siteId the ID of the site. It may be <code>null</code> if there is no need to attach the
   *     template to the site, assumed the template has already been attached to a site.
   ...
   *
   * @param name the name of the specified template, assumed not <code>null</code>.
   * @return the template, never <code>null</code>.
   */
   ...
   *
   * @param type The type of the readonly or base templates, tries to find all the templates that
   *     start with perc.<type>. where type is the supplied type.
   ...
   *
   * @return the template summaries, never <code>null</code>, may be empty. Results will be sorted
   ...
   *
   * @return all template summaries, never <code>null</code>, may be empty. Results will be sorted
   *     alphabetically by name.
  ...
  /**
   * Finds the user template with the specified name. This is used by unit test only. It cannot be
   * used by production code
   *
   * @param name never blank.
   * @return the template, never <code>null</code>.
   * @deprecated this method does not make sense any more because the same template can be used for
   *     different site. This is only used by unit test for now and should be removed along with
   *     related unit test.
   */
  ...
  public PSTemplate generateTemplateFromSource(PSTemplate template, String siteId)
      throws IPSTemplateService.PSTemplateException, IPSPathService.PSPathNotFoundServiceException;

  /**
   * Gets the thumbnail path for a template given the specific site name. This method was refactored
   * to be able to get the thumbnail path as a service method. The template Summary must be a valid
   * summary in order to get the name of the template, the method assembles the path of the
   * thumbnail using the template name and the site name
   *
   * @param summary - the template object to retrieve the information
   * @param siteName - the corresponding site name from the template - may be <code>null</code>
   * @return String - the path to the thumbnail for that template - may return <code>null</code> if
   *     siteName is <code>empty</code> or <code>null</code>
   */
  public String getTemplateThumbPath(PSTemplateSummary summary, String siteName);

  /**
   * Retrieves the list of user templates (not base templates) by type. If type param is null, then
   * normal type and untyped templates are returned. Else the corresponding typed templates are
   * returned.
   *
   * @author federicoromanelli
   * @param type - the type of template: NORMAL or UNASSIGNED. May be <code>null</code>
   * @return the list of templates that correspond to that type. Never <code>null</code>. May be
   *     <code>empty</code>
   */
  ...
  /**
   * Retrieves the list of summaries that correspond to user templates (not base templates) filtered
   * by type. See com.percussion.pagemanagement.dao.findUserTemplatesByType(PSTemplateTypeEnum) for
   * details.
   *
   * @author federicoromanelli
   * @param type - the type of template: NORMAL or UNASSIGNED. May be <code>null</code>
