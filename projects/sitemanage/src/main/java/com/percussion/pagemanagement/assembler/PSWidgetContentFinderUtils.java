...
/**
 * Helper class used for retrieving page/template asset relationships. It uses the same API to
 * retrieve and sort relationships as the content finder does. The content finder is used during
...
public class PSWidgetContentFinderUtils {
  /**
   * Retrieves associated relationships where the specified page/template is the owner and the
   * dependents are local and shared assets.
   *
   * @param id the ID of the specified page or template, not <code>null</code> or empty.
   * @return the list of relationships, sorted by "sort-rank" property, never <code>null</code>, but
   *     may be empty.
   */
  ...
  /**
   * This is the same as {@link #getLocalSharedAssetRelationships(String)}, except the dependents
   * are shared assets only.
   *
   * @param id the ID of the specified page or template, not <code>null</code> or empty.
   * @return the list of relationships, sorted by "sort-rank" property, never <code>null</code>, but
   *     may be empty.
  ...
  /**
   * Retrieves associated relationships where the specified page/template is the owner and the
   * dependents are local and shared assets. In addition, the relationship category is
   * active-assembly.
   *
   * @param id the ID of the specified page or template, not <code>null</code>.
   * @param relationshipName the name of the returned relationship. It may be <code>null</code> if
   *     the dependent can be either share & local assets.
   * @return the list of relationships, sorted by "sort-rank" property, never <code>null</code>, but
   *     may be empty.
   */
  ...
  /**
   * Gets a list of relationships (from the given source relationships) that matches one of the
   * supplied widgets. The matched relationships are the ones that will be used during (page)
   * rendering.
   *
   * @param srcRels the source relationships, never <code>null</code>.
   * @param widgets the widget instances (that may be used on a page/template to render the source
   *     relationships/assets). Not <code>null</code>.
   * @return the matching relationships, never <code>null</code>, but may be empty.
   */
  public static Collection<PSRelationship> getMatchRelationships(
      Collection<PSRelationship> srcRels, Collection<PSWidgetItem> widgets) {
   ...
   */
  public static PSRelationship getMatchRelationship(
      Collection<PSRelationship> srcRels, PSWidgetItem widget) {
    try {
      ...
      log.error(
          "Error getting Widget Definition for: {} Error: {}",
  ...
  /**
   * Comparator to order widget/slot relationships. Note, this comparator must be compatible or
   * behaves the same as the comparator defined in {@link PSRelationshipWidgetContentFinder}.
   */
  private static class RelationshipOrder implements Comparator<PSRelationship> {
     ...
     * @return positive number for increasing order, negative for decreasing order, zero for no
     *     change
     */
