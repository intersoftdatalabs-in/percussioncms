...
/**
 * Represents the merging of {@link PSRegionTree} with {@link PSRegionBranches} as one unified tree.
 *
 * <p>The root nodes of the page region branches will replace the matching nodes of the tree
 * branches if {@link #chooseTemplateOrPageRegion(PSAbstractRegion, PSAbstractRegion,
 * PSMergedRegion)} == {@link PSMergedRegionOwner#PAGE}.
 *
 * <p>Implementations may also choose whether or not to expand sub regions through {@link
 * #isLeaf(PSMergedRegion)}.
 *
 * @author adamgent
 */
public abstract class PSAbstractMergedRegionTree {
   ...
   *
   * <p>Will never be <code>null</code> after {@link #merge(PSRegionTree, PSRegionBranches)} is
   * called. Maybe empty.
   */
  ...
  /**
   * Creates the merged region tree for the given tree and branches. <em>This should only be run
   * once as reuse of the object through this method is undefined. Its recommended a new object
   * should be be used for different inputs.</em>
   *
  ...
  private void createPageRegionMap(Collection<? extends PSAbstractRegion> pageRegionBranches) {
    ...
    if (pageRegionBranches == null) return;
  ...
  private PSMergedRegion mergeTree(PSAbstractRegion regionNode, PSMergedRegion parent)
    ...
    if (parent != null) notNull(parent.getOwner());
    ...
    if (isLeaf(rvalue)) {
      if (log.isDebugEnabled()) log.debug("Region is leaf: " + rvalue);
    } else {
   ...
   */
  protected abstract PSMergedRegionOwner chooseTemplateOrPageRegion(
  ...
  /**
   * <strong>Indicates</strong> whether or not merged region should have children. <em>The {@link
   * PSMergedRegion#getOriginalRegion() original region} may actually have sub regions but
   * implementations may choose to indicate not to expand this region</em>
   *
   * @param mr never <code>null</code>.
   * @return never <code>null</code>. If <code>true</code> will stop recursively merging
   *     sub-regions.
   */
   ...
   * @return never <code>null</code>.
   */
  protected PSMergedRegion createNode(
      PSAbstractRegion region,
   ...
   *
   * @return maybe <code>null</code> if {@link #merge(PSRegionTree, PSRegionBranches)} has not been
   *     called.
   */
  ...
  public Collection<PSMergedRegion> getWidgetRegions() {
    ...
    for (String id : mergedWidgetItems.keySet()) {
      ...
      if (r != null) regions.add(mergedRegionMap.get(id));
      else
        log.error(
            format(
                "Widgets associated to non existant region. " + "Widgets: {0}, RegionId: {1}",
                mergedWidgetItems.get(id), id));
    }
    return regions;
  }
