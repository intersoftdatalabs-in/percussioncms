 ...
 *
 * <p>Implementations may also choose whether or not to expand subregions through {@link
...
public abstract class PSAbstractMergedRegionTree {
  ...
  /**
   * The widgets associated to a page. Will never be {@code null} after {@link #merge(PSRegionTree,
   * PSRegionBranches)} is called. May be empty.
   */
  protected Map<String, List<PSWidgetItem>> pageWidgetItems;

  /**
   * The widgets associated to a template. Will never be {@code null} after {@link
   * #merge(PSRegionTree, PSRegionBranches)} is called. May be empty.
   */
   ...
   * once as reuse of the object through this method is undefined. It is recommended a new object
   * should be used for different inputs.</em>
   *
  ...
  private PSMergedRegion mergeTree(PSAbstractRegion regionNode, PSMergedRegion parent)
    ...
    if (isLeaf(rvalue)) {
      ...
      var mergedSubRegions =
          getChildRegions(rvalue.getOriginalRegion()).stream()
              .map(
                  subRegion -> {
                    try {
  ...
  /**
   * Indicates whether or not merged region should have children. The {@link
   * PSMergedRegion#getOriginalRegion() original region} may actually have subregions but
   * implementations may choose to indicate not to expand this region.
   *
   ...
   *
   * @return may be {@code null} if {@link #merge(PSRegionTree, PSRegionBranches)} has not been
   *     called.
   */
  ...
  public Collection<PSMergedRegion> getWidgetRegions() {
    ...
    for (var id : mergedWidgetItems.keySet()) {
      ...
      if (r != null) {
        ...
        log.error(
            format(
                "Widgets associated to non-existent region. Widgets: {0}, RegionId: {1}",
