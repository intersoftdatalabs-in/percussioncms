...
public class PSPageRestService {
  ...
  public PSPagedItemList findPagesByTemplate(
      ...
      throws PSValidationException {
    try {
      return pageService.findPagesByTemplate(
          templateId, startIndex, maxResults, sortColumn, sortOrder, pageId);
    } catch (PSValidationException ex) {
      log.error(ex.getMessage());
      log.debug(ex.getMessage(), ex);
      throw ex;
  ...
  public PSPage save(PSPage page) throws PSBeanValidationException {
    try {
      ...
      return pageService.save(page); // codeql[java/xss] justification: JSON/XML DTO via Jackson/JAXB; not HTML body (alert #748)
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSNoContent changeTemplate(
      @PathParam("pageId") String pageId, @PathParam("templateId") String templateId)
      throws PSValidationException {
    try {
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSNoContent restorePage(@PathParam("pageId") String pageId)
    ...
    if (hasErrors || !isValidForRecycle) {
      throw new PSPageException(
          "Problem restoring page.  There may be a complication due to a folder with the same name"
              + " being in the destination location.  Restoring the entire folder should restore"
              + " the folder to the target location.");
    }

    if (isValidForRecycle) recycleService.restoreItem(pageId);
  ...
  public PSPagedItemList search(PSSearchCriteria criteria)
    ...
    List<Integer> contentIdsAllowedForSite =
        searchService.getContentIdsForFetchingByStatus(criteria);
