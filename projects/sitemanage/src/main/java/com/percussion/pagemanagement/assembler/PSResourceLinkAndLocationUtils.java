...
public class PSResourceLinkAndLocationUtils {
  ...
  /**
   * URL escapes a path string while keeping the slashes of the path. The slashes ('<code>/</code>')
   * will not be escaped.
   *
   * <p><strong>This is url escaping for links not encoding for forms.</strong> In other words it is
   * not <code>application/x-www-form-urlencoded</code> but rather URI escaping.
   *
   ...
   * @param fileName never <code>null</code> or empty and should not contain <code>'/'</code>
   * @return never <code>null</code>.
   */
  ...
  /**
   * Creates a default link and location use the items title. It also append the "suffix" of the
   * template if the context is delivery, which is the publish location.
   *
   ...
   * linux.
   *
   * <p>Mainly checks to see if there are any bad characters that in the folder path. The bad
   * characters that are not allowed come from the windows operating system as it is more
   * restrictive than unix.
   *
   * <p>The following characters ar bad:
   *
   * <pre>
   * \ | < > ? " : *
   * </pre>
