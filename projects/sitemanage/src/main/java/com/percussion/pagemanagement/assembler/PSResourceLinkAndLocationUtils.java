...
public class PSResourceLinkAndLocationUtils {
    ...
    /**
     * URL-escapes a path string while keeping the slashes of the path.
     * The slashes ('/') will not be escaped.
     * <p>
     * This is URL escaping for links, not encoding for forms.
     * </p>
     *
                ...
                for (int i = 0; i < analyticsIdHalves.length; i++) {
                    try {
                        if (analyticsId.contains("=")) {
                            ...
                            var encodedValue = URLEncoder.encode(individualParam[0], UTF_8) + "="
    ...
    /**
     * Creates a default link and location using the item's title.
     * It also appends the "suffix" of the template if the context is delivery (publish location).
     *
    ...
    /**
     * Validates that the path is safe to use as a physical filesystem folder path on Windows and Linux.
     * Mainly checks to see if there are any bad characters in the folder path.
     * The bad characters that are not allowed come from the Windows operating system as it is more restrictive than Unix.
     * <p>
     * The following characters are bad: \ | < > ? " : *
     * </p>
     *
