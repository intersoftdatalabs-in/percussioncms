...
public class PSRecentService implements IPSRecentService {
      ...
      try {
        ...
        if (itemProps != null) {
          ...
          if (ignoreArchivedItems
              && (itemProps.getStatus().equals(PSWorkflowHelper.WF_STATE_ARCHIVE)
  ...
  public List<PSPathItem> findRecentSiteFolder(String siteName) {
    ...
    for (String entry : recentEntries) {
      ...
      try {
        ...
        if (pathItem != null) pathItems.add(pathItem);
        else log.debug("Removing recent siteFolder entry find returned null : {}", entry);
      } catch (Exception e) {
        log.debug(
      ...
      if (pathItem == null) toDelete.add(entry);
    }
    if (!toDelete.isEmpty())
  ...
  public List<PSPathItem> findRecentAssetFolder() {
    ...
    for (String entry : recentEntries) {
      ...
      try {
        pathItem = pathService.find(entry);
        if (pathItem != null) pathItems.add(pathItem);
        else log.debug("Removing recent assetFolder entry find returned null :{}", entry);
      } catch (Exception e) {
        log.debug(
  ...
  public void addRecentSiteFolder(String value) {
    ...
    if (StringUtils.isBlank(value)
        || !(StringUtils.startsWith(value, "//") || StringUtils.startsWith(value, "/"))) return;
  ...
  public void addRecentAssetFolder(String value) {
    ...
    if (pos >= 0 && pos <= 2) value = "/" + value.substring(pos);
    else return;
