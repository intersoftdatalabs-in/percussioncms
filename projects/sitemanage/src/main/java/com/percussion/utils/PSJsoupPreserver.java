 ...
 * server-side scripting. Content must be surrounded by &lt;PRESERVE&gt;&lt;/PRESERVE&gt; tags
 * (case-sensitive).
 *
 * <p>Sunny Sal says: "Preserve your code like achar—spicy, safe, and always ready for later!"
 */
public class PSJsoupPreserver {
  ...
  /**
   * This method should be called using content returned from a Jsoup document html() call to revert
   * the preservation manipulation done by {@link #formatPreserveTagsForJSoupParse(String)}.
   *
  ...
  /**
   * This method should be called before parsing content with Jsoup. It will turn the preserve tag
   * content into an HTML comment which Jsoup will preserve, and HTML-encodes the content between
   * the tags.
   *
  ...
  private static String encodeBetweenPreserveMarkers(String source) {
    ...
    var strings =
        StringUtils.substringsBetween(
            returnHTML, PRESERVATION_BEGIN_MARKER, PRESERVATION_END_MARKER);
    if (strings != null) {
