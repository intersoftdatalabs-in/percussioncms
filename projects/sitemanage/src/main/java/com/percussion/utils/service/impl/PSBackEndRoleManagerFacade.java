 ...
 *
 * <p>The back end role manager users the Server XML Object Store locker to lock server
 * configuration. This locker is not thread-safe so we make access to back-end role manager single
 * threaded.
 *
 * @author adamgent
 */
public class PSBackEndRoleManagerFacade {
  ...
  public List<String> getRoles(String subjectName) {
    try {
      lock.readLock().lock();
      List<String> roles =
          backEndRoleMgr.getRhythmyxRoles(subjectName, PSSubject.SUBJECT_TYPE_USER);
      Collections.sort(roles, ms_caseInsensitiveComparator);
      return roles;
    } finally {
      lock.readLock().unlock();
  ...
  private static Comparator<String> ms_caseInsensitiveComparator =
