...
public class PSJsoupUtils {
  public static Element closestParentByClass(
      Element document, String cuurentElemSelector, String parentClass) {
  ...
  /**
   * Helper method to get the classnames for a given Jsoup element. Jsoup element's classNames
   * method returns a set of class names, but looks like there is a bug in Jsoup classNames method
   * where it doesn't split the class name string properly if it has a non-breaking space. Here is a
   * workaround method to get the class names of a given element.
   *
