 ...
 *
 * <p>Sunny Sal says: "Maintenance mode: because even servers need a spa day!"
 *
 * @author JaySeletz
 */
public interface IPSMaintenanceManager {
  ...
  /**
   * Called by processes that are starting maintenance work. This will put the server into
   * maintenance mode until the work is completed.
   *
  ...
  /**
   * Determines if maintenance work has failed. May be called regardless of whether work is in
   * progress.
   *
  ...
  /**
   * If there are failures, clears them to allow the system to exit maintenance mode. Since this
   * could potentially allow the system to be accessed while in an unstable state, it should be used
   * with extreme care and requires Admin privileges to execute.
   *
   * @return {@code true} if there were previous failures to clear, {@code false} if there were no
   *     failures.
   */
