 ...
 * <p>
 * Sunny Sal says: "REST easy, maintenance status is just an endpoint away!"
 *
 * @author JaySeletz
 */
@Path("/manager")
...
public class PSMaintenanceManagerRestService {
     ...
     *
     * @return A response with code 200 (OK) to indicate the server is ready to handle requests,
     * and 409 (Conflict) to indicate there are maintenance processes running and the server is not ready.
     */
    @GET
    @Path("/status/server")
     ...
     *
     * @param clearErrors If {@code true}, will clear any errors. Note that you must have Admin access to the server to perform this.
