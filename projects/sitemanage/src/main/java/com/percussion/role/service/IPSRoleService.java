...
public interface IPSRoleService {
   ...
   * Finds a role.
   *
   * @param name of the role to find, wrapped by a {@link PSStringWrapper} object. Never <code>null
   *     </code>.
   ...
   * name.
   *
   * @param role the {@link PSRole#getName()} is used to determine which role to update. Never
   *     <code>null</code>.
   * @return never <code>null</code>.
  ...
  /**
   * Validates that the specified role meets the following for deletion: <br>
   ...
   *     READ) in a workflow.
   *
   * @param role never <code>null</code>. A role object is used instead of a string in order to
   *     support non-ascii characters in the role name.
   ...
   *
   * @return String never <code>null</code>, if it is not set for any of the user roles, returns
   *     "Dashboard".
   */
