...
public class PSRoleService implements IPSRoleService {
    ...
    public static final List<String> DEFAULT_IMPORTED_USER_ROLES = List.of("Default", CONTRIBUTOR_ROLE);
    ...
    public PSRole update(PSRole role) throws PSDataServiceException {
        ...
        if (!users.equals(existingUsers)) {
            var toAdd = users.stream().filter(u -> !existingUsers.contains(u)).collect(Collectors.toList());
            if (!toAdd.isEmpty()) {
                addUsersToRole(name, toAdd);
            }
            var toRemove = existingUsers.stream().filter(u -> !users.contains(u)).collect(Collectors.toList());
            if (!toRemove.isEmpty()) {
    ...
    public PSUserList getAvailableUsers(PSRole role) throws PSDataServiceException {
        ...
        var availableUsers = users.stream()
                .filter(u -> !role.getUsers().contains(u))
                .collect(Collectors.toList());
    ...
    public void validateDeleteUsersFromRole(PSUserList userList) throws PSDataServiceException {
        ...
        if (!singleRoleUsers.isEmpty()) {
            var emsg = "The following users will be unable to login if they are removed from this role: "
        ...
        protected void doValidation(PSRole role, PSBeanValidationException e) {
            ...
            for (var rl : role.getUsers()) {
                if (!PSCollectionUtils.containsIgnoringCase(allUsers, rl)) {
                    var msg = "Cannot add user \"" + rl + "\" because user named \"" + rl + "\" does not exist.";
            ...
            if (isCreateRole) {
                ...
                if (beRole != null) {
                    ...
                    log.debug("Cannot create role \"{}\" because a role named \"{}\" already exists.", newName, beRole.getName());
    ...
    private void setHomepage(String roleName, String homepage) throws IPSGenericDao.LoadException, IPSGenericDao.SaveException {
        ...
        if (StringUtils.isBlank(homepage) || !(homepage.equals(HOMEPAGE_TYPE_DASHBOARD)
