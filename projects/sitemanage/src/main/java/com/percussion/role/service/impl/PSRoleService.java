...
public class PSRoleService implements IPSRoleService {
  ...
  /** A list of pre-defined roles by the system, which should not be exposed to end user. */
  ...
  public void validateForDelete(PSRole role) throws PSDataServiceException {
    ...
    if (StringUtils.isNotEmpty(emsg)) {
      PSValidationErrorsBuilder builder =
          new PSValidationErrorsBuilder(PSRole.class.getCanonicalName());
  ...
  /**
   * Validates the specified role. It validates the role object according to its annotation and
   * invokes {@link PSRoleValidator#doValidation(PSRole, PSBeanValidationException)} for additional
   * validation.
   *
  ...
  /**
   * This is used to validate a {@link PSRole} object before updating an existing role or create a
   * new one.
   *
   * <p>This invocation of {@link #doValidation(PSRole, PSBeanValidationException)} is indirectly
  ...
  protected class PSRoleValidator extends PSAbstractBeanValidator<PSRole> {
    /** It is <code>true</code> if validating {@link PSRole} object for creating a role. */
  ...
  /**
   * Finds all users assigned to the specified role which are only assigned to one role (includes
   * system roles).
   *
   ...
   * @param homepage the homepage to set if <code>null</code> or not a valid home page, then set to
   *     Dashboard.
   */
  ...
  /**
   * Finds all workflows which are using the specified role. A workflow is using a role if the role
   * has been added to a state of the workflow as anything other than {@link
