...
public abstract class PSBaseMatchingMigrationRule implements IPSContentMigrationRule {
   ...
   * Document)}, that method handles basic contract validation of parameters.
   */
  protected abstract String matchOnRule(String widgetId, Document sourceDoc, Document targetDoc);

  /**
   * Given a widget id, finds the region element that is the immediate parent wrapping the widget,
   * which is the element rules should use for matching in the reference/source document.
   *
  ...
  /**
   * Given a set of class names, filters out the names used by the system as we don't want to match
   * on those.
   *
