...
public class PSClassNameMatcher {
  ...
  /**
   * Get the element this matcher is currently referencing, may be the element supplied during
   * construction or one of its parent elements.
   *
  ...
  /**
   * Walk the parent elements of the current element and for the first element with comparable class
   * names, return them, setting the current element reference to the matched element (see {@link
   * #getCurrentElement()})
   *
   * @return The set of class names, may be empty if none found in any element in the list, not
   *     <code>null</code>.
   */
  ...
  /**
   * Resets the current element reference to the first parent element of the current element, and
   * checks it for any class name matches with the supplied set.
   *
   * @param classNames The set to match, may not be <code>null</code>.
   * @return <code>true</code> if the parent has a match, <code>false</code> if not, or if the
   *     current element has no parent elements.
   */
