...
public class PSContentMigrationService implements IPSContentMigrationService {
  ...
  public void migrateContent(
      String siteName, String templateId, String refPageId, List<String> pageIds)
  ...
  @Override
  public void migrateContentOnTemplateChange(
  ...
  public void migrateSameTemplateChanges(String templateId, List<String> pageIds)
    ...
    for (String pageId : pageIds) {
      ...
      if (template.getContentMigrationVersion().equals(page.getTemplateContentMigrationVersion())) {
        log.info(
            "Both template {} and page {} have same version skipping content migration.",
            templateId,
            pageId);
      }
      if (itemWorkflowService.isCheckedOutToSomeoneElse(pageId)) {
      ...
      if (!applicableWidgets.isEmpty()) {
        ...
        log.debug(
            "Could not find any applicable widgets skipping migration process for page {}", pageId);
   ...
   */
  private void findMatchingContent(
      PSPage page, PSTemplate template, Document refDoc, List<ApplicableWidget> applicableWidgets)
    ...
    for (Document unusedDocument : unusedDocuments) {
      ...
      // @TODO optimize if all the widgets are filled then no need to loop through it, it doesn't
      // hurt much as findMatchingContent avoids the widgets if they are filled.
    }
  }

  /**
   * Returns the Jsoup document of assembled page if the reference page is not blank otherwise
   * assembled template.
   *
   ...
   */
  private Document getReferenceDocument(String templateId, String refPageId)
  ...
  private List<ApplicableWidget> findEmptyWidgets(String templateId, String pageId)
      throws PSDataServiceException {
    List<PSAssetDropCriteria> tplAssetDropCriteria =
        assetService.getWidgetAssetCriteria(templateId, false);
    ...
    List<PSAssetDropCriteria> pageAssetDropCriteria =
        assetService.getWidgetAssetCriteria(pageId, true);
    for (PSAssetDropCriteria adc : pageAssetDropCriteria) {
      ...
      if (!adc.getExistingAsset()
          && converterMap.size() == wc.size() + adc.getSupportedCtypes().size()) {
        applicableWidgets.add(
            createApplicableWidget(adc, null, !tplWidgetIds.contains(adc.getWidgetId())));
      }
    }
   ...
   */
  private ApplicableWidget createApplicableWidget(
      PSAssetDropCriteria adc, Document refDoc, boolean isPageWidget) {
    ApplicableWidget applicableWidget = new ApplicableWidget();
    applicableWidget.widgetId = adc.getWidgetId();
    if (refDoc != null) {
      Element regElem =
          PSJsoupUtils.closestParentByClass(
              refDoc,
              PSJsoupUtils.generateAttributeSelector(
                  IPSContentMigrationRule.ATTR_WIDGET_ID, adc.getWidgetId()),
  ...
  /**
   * Finds the matching content based on the registered rules then converts the content using the
   * content converter which results in a fields map. Sets the fields map on to applicable widget.
   * If the content is not found then the fields will be <code>null</code>.
   *
   * @param refDoc assumed not <code>null</code>.
   ...
   */
  private void findMatchingContent(
      Document refDoc, Document targetPageDoc, List<ApplicableWidget> applicableWidgets) {
  ...
  /**
   * Changes the page template and for each applicable widget creates local asset and associates it
   * to the page.
   *
   * @param pageId assumed not <code>null</code>.
   * @param templateId assumed not <code>null</code>.
  ...
  private void updatePage(
      String pageId, String templateId, List<ApplicableWidget> applicableWidgets)
  ...
  /**
   * Creates a local asset and adds it to the page through the supplied widget. Uses local content
   * workflow and generated name for local content.
   *
   * @param pageId assumed not <code>null</code>.
  ...
  /**
   * Data structure object to hold applicable widget information. All the members are filled in
   * findApplicableWidgets
   */
