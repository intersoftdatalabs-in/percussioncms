...
public class PSSiteimprove {
    ...
    private static final PSSiteImproveProviderService providerService = new PSSiteImproveProviderService();
     ...
     * Also called if the refresh token method is called from the 'advanced' button
     * within the gadget.
    ...
    public Response storeSiteImproveConfiguration(PSSiteImproveSiteConfigurations publishSettings) {
        try {
            ...
            publishSettingsJSON.put("doAssetsScanExclude", publishSettings.getDoAssetsScanExclude().orElse(false));
            publishSettingsJSON.put("doPreview", publishSettings.getDoPreview().orElse(false));
            publishSettingsJSON.put("isSiteImproveEnabled", publishSettings.getIsSiteImproveEnabled().orElse(false));
            metadataService.save(new PSMetadata(siteConfigKey, publishSettingsJSON.toString()));
            // CMS-8189: Upgraded jquery unable to parse if response is empty string. Advisable to return "204 - No content" to avoid jquery parser error for json.
            return Response.noContent().build();
        } catch (Exception e) {
            var message = "Failed to save configuration settings for " + publishSettings.getSiteName() + " Exception is " + e.getMessage();
            logger.error(message, e);
            return Response.serverError().entity(message).build();
        }
    }
    ...
    private boolean validateConfiguration(PSSiteImproveSiteConfigurations configuration) {
        var validSiteName = configuration.getSiteName() != null && !configuration.getSiteName().isEmpty();
    ...
    private void addSiteImproveTaskToPreExistingEditions(String siteName) {
        ...
        for (var edition : editions) {
            ...
            if (!hasSiteimproveTask(tasks)) {
                ...
                logger.info("Adding the {} post editiontask on the edition that doesn't have it already", EXT_NAME);
            ...
            if (hasSiteimproveTask(tasks)) {
                for (var task : tasks) {
                    if (EXT_NAME.equals(task.getExtensionName())) {
                        logger.info("Removing the {} post editiontask from the edition that have it.", EXT_NAME);
