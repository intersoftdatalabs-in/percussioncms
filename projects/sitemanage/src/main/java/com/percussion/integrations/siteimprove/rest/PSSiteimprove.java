...
public class PSSiteimprove {
   ...
   * applied to all sites that enable Siteimprove. Also called if the refresh token methd is called
   ...
   *
   * <p>* @param publishSettings Preview, Staging, Production settings for the site given.
   *
  ...
  private Boolean validateConfiguration(PSSiteImproveSiteConfigurations configuration) {
    Boolean validSiteName =
        configuration.getSiteName() != null && !configuration.getSiteName().isEmpty();
    return validSiteName
        && configuration.getDoPreview() != null
        && configuration.getDoStaging() != null
        && configuration.getDoAssetsScanExclude() != null
        && configuration.getDoProduction() != null;
  }
    ...
    for (IPSEdition edition : editions) {
      ...
      if (!hasSiteimproveTask(tasks)) {
        ...
        logger.info(
            "Adding the "
                + EXT_NAME
                + " post editiontask on the edition that doesn't have it already");
      ...
      if (hasSiteimproveTask(tasks)) {
        ...
        for (IPSEditionTaskDef task : tasks) {
          if (task.getExtensionName().equals(EXT_NAME)) {
            logger.info(
                "Removing the " + EXT_NAME + " post editiontask from the edition that have it.");
  ...
  private boolean hasSiteimproveTask(Collection<IPSEditionTaskDef> tasks) {
    for (IPSEditionTaskDef task : tasks) {
      if (task.getExtensionName().equals(EXT_NAME)) return true;
    }
    return false;
  }
}
