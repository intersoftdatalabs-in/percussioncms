 ...
 *
 * <p>The task alerts site improve that pages have been updated, or to crawl the whole site.
 */
public class PSSiteimproveEditionTask implements IPSEditionTask {
  ...
  private static IPSIntegrationProviderService siteimproveService =
  ...
  public void perform(
    ...
    PSSiteImproveSiteConfigurations siteConfigurations =
        obtainSiteConfiguration(siteConfiguration.getData());
  ...
  private void handleProductionOrStagingEnabled(
    ...
    if (isProductionEnabled(siteConfigurations, publishServerInfo)
      ...
      logger.info(
          "No production or staging settings are configured for Site improve, publishing no URLs to Site improve.");
    }
  }
  ...
  /**
   * Alert Site improve that we have published new pages for them to spider, or do a full crawl of
   * the site.
   *
  ...
  private boolean isProductionEnabled(
    ...
    boolean isEnabled =
        PSPubServer.PRODUCTION.equalsIgnoreCase(publishServerInfo.getServerType())
            && siteConfigurations.getDoProduction();

    if (isEnabled) {
      logger.debug(
          "Production configuration is enabled for this site for site improve, alerting site improve to update indices.");
    }
  ...
  private boolean isStagingEnabled(
    ...
    boolean isEnabled =
        PSPubServer.STAGING.equalsIgnoreCase(publishServerInfo.getServerType())
            && siteConfigurations.getDoStaging();

    if (isEnabled) {
      logger.debug(
          "Staging configuration is enabled for this site for site improve, alerting site improve to update indices.");
    }
  ...
  private boolean isAssetsScanExcludeEnabled(PSSiteImproveSiteConfigurations siteConfigurations) {
    ...
    if (isEnabled) {
      logger.debug(
          "Assets scan exclude is enabled for this site for site improve, alerting site improve to update indices.");
    }
  ...
  /**
   * Convert an url that is http to https. For example: http://www.percussion.com/ ->
  ...
  private PSSiteImproveSiteConfigurations obtainSiteConfiguration(String siteConfigurationData)
    ...
    if (!siteConfigurationJson.has(DO_ASSETS_SCAN_EXCLUDE)) {
      String message =
          "Site improve configuration details were missing the assets scan exclude setting";
      logger.error(message);
      throw new Exception(message);
    }
    ...
    if (!siteConfigurationJson.has(IS_SITEIMPROVE_ENABLED)) {
      String message =
          "Site improve configuration details were missing the Site improve enabled setting";
      logger.error(message);
      throw new Exception(message);
    }
  ...
  private void alertSiteImproveUpdatePageInfo(
    ...
    if (jobPage.getStatus().equals(IPSSiteItem.Status.SUCCESS)) {
      ...
      logger.debug(
          "Did not submit page:{} to site improve because of page's status of {}",
