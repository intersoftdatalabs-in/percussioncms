...
/**
 * A file system service which receives a root folder path and manages file system operations over
 * it. It uses paths similar to the ones used in the Finder. All these paths are relative to the
 * root folder path.
 *
...
public interface IPSFileSystemService {
  ...
  /**
   * Given a path, it returns a File object pointing to that file, relative to the root folder path.
   * It doesn't check whether the file exists or not.
   *
   * <p>Note that the paths handled by implementations of this interfaces are not supposed to handle
   * internal name mappings. So paths must directly point to the file system.
   *
  ...
  /**
   * Given a File object, it returns its name. It takes care of cases where the File object was
   * created with a name like "/", then it automatically returns the name of the parent File.
   *
  ...
  /**
   * Renames a folder. It receives the full folder path that will be renamed, and the new folder
   * name.
   *
   * <p>The implementation class should check at least three things when renaming a folder, and
   * return the following exceptions when the condition is met:
   *
   * <ul>
   *   <li>Max folder name length: {@link PSFolderNameLengthLimitException}
   *   <li>Invalid folder name: {@link PSInvalidFolderNameException}
   *   <li>Destination folder already exists: {@link PSExistingFolderException}
   * </ul>
   *
  ...
  /**
   * Handles the deletion of a file. It deletes the given file from the filesystem.
   *
  ...
  /**
   * Handles the deletion of a folder. It deletes the given folder from the filesystem.
   *
   ...
   * @return <code>true</code> if none of the strings in the list is equal to the given name. <code>
   *     false</code> otherwise
   */
  boolean foldernameAvailable(String name, String[] files);

  /**
   * Gets the new folder name according to those files or folders in the given path. Iterates over
   * all of the files and folders and compares its names with the prefix. If it finds the given
   * prefix, it adds a number as the suffix.
   *
   * @param filesAndFolders a list of strings representing the name of each file or folder from
   *     which we want to get the new folder name.
   * @return a String, never <code>null</code>
   */
  String getNewFolderName(String[] filesAndFolders);

  /**
   * Validates that a file with the given path can be uploaded. If there is another file with the
   * same name, it will return an error response, so the UI can show a message. If there is no file
   * named the same, it return success.
   *
  ...
  /**
   * Checks if the string passed as a parameter is a reserved filename, no matter the case it has.
   *
   * @param name String that holds the name we want to check.
   * @return <code>true</code> if the name is a reserved filename. <code>false</code>
   */
  boolean isReservedFilename(String name);

  /**
   * Handles the upload of a file. Checks if the path exists and saves the file. If another file
   * with the same name already exists, it overwrites it. If the file name is a reserved name, or
