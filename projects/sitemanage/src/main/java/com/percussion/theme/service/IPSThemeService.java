 ...
 * @author adamgent
 */
public interface IPSThemeService
    extends IPSCatalogService<PSThemeSummary, String>, IPSReadOnlyDataService<PSTheme, String> {
  /**
   ...
   */
  PSTheme load(String name)
      throws DataServiceLoadException, DataServiceNotFoundException, PSValidationException;

  /**
   * Creates a new theme from an existing theme. The newly created theme will be a copy of the
   * existing theme.
   *
   ...
   *     </code>.
   * @throws DataServiceLoadException if the new theme does not have a CSS file or multiple css
   *     files.
   ...
   */
  PSThemeSummary create(String newTheme, String existingTheme)
   ...
   * @throws DataServiceSaveException if the new theme could not be saved.
   */
  PSThemeSummary createFromDefault(String newTheme)
      throws DataServiceLoadException, DataServiceNotFoundException, DataServiceSaveException;
  ...
  void delete(String name)
      throws DataServiceNotFoundException, IPSDataService.DataServiceDeleteException;

  /**
   * Gets the root URL of the specified theme.
   *
   * @param themeName the theme name, may not be <code>null</code> or empty.
   * @return the URL that is relative to any site that uses this theme. It never <code>null</code>
   *     or empty.
   */
  ...
  /**
   * Prepare for editing the region CSS file. It copies the file from the location of the theme to a
   * temporary location. This should be called before modifying the "Layout" of the template.
   *
   * @param theme the theme name, not blank
   * @param templatename the template name, not blank
   */
  ...
  /**
   * Clear the temporary cached region CSS file if exist. This file is created by calling {@link
   * #prepareForEditRegionCSS(String, String)}. This should be called when canceling the changes of
   * template "Layout" and should be called when entering the edit mode of the template (to make
   * sure to clear any previously cached file if there is any).
   *
   * @param theme the theme name, not blank
   * @param templatename the template name, not blank
   */
  ...
  /**
   * Returns the list of custom styles that are stored in a property file, checks for the file
   * modification time and loads the properties if the file is modified from last read.
   *
