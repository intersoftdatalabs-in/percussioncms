 ...
 * on the Analytics provider handler used. I.e Google Analytics.
 *
 * @author erikserating
 */
public interface IPSAnalyticsProviderService {
   ...
   *
   * @param config The analytics <code>PSAnalyticsProviderConfig</code> object. Cannot be <code>null
   *     </code>.
   */
   ...
   *
   * @param encrypted if <code>true</code> then the password will be encrypted in the returned
   *     config object.
   ...
   *
   * @param uid the user id for access to the provider. May be <code>null</code> or empty. If so it
   *     will attempt to use stored uid or error if it does not find one.
   * @param password the password for access to the provider. May be <code>null</code> or empty. If
   *     so it will attempt to use stored password or error if it does not find one.
   * @return a map of strings, with the key being the profile|webpropertyId value and the value
   *     being the profile display value. Never <code>null</code>, may be empty.
  ...
  public void testConnection(String uid, String password)
      throws PSAnalyticsProviderException, IPSGenericDao.LoadException, IPSGenericDao.SaveException,
