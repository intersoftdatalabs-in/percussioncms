...
public class PSAnalyticsQueryResult implements IPSAnalyticsQueryResult {
  ...
  public float getFloat(String key) {
    ...
    key = key.toLowerCase();
    DataType type = getDataType(key);
    if (!hasValue(key)) return -1;
    if (type == null)
      throw new PSAnalyticsQueryResultException("No data type defined for specified field.");
    if (type == DataType.STRING || type == DataType.DATE)
      throw new PSAnalyticsQueryResultException("Type cannot be converted to a Float");
    if (type == DataType.INT) return ((Integer) values.get(key)).floatValue();
    if (type == DataType.LONG) return ((Long) values.get(key)).floatValue();
  ...
  public int getInt(String key) {
    ...
    if (type == DataType.FLOAT) return ((Float) values.get(key)).intValue();
    if (type == DataType.LONG) return ((Long) values.get(key)).intValue();
  ...
  public long getLong(String key) {
    ...
    if (type == DataType.DATE) return ((Date) values.get(key)).getTime();
    if (type == DataType.INT) return ((Integer) values.get(key)).longValue();
    if (type == DataType.FLOAT) return ((Float) values.get(key)).longValue();
  ...
  public String getString(String key) {
    ...
    if (type == DataType.DATE) {
      ...
      return df.getDateTimeInstance(DateFormat.FULL, DateFormat.FULL)
          .format((Date) values.get(key));
    }
   ...
   *
   * @param values map of key value pairs, cannot be <code>null</code>, may be empty.
   */
  public void putAll(Map<String, Object> vals) {
    if (vals == null) throw new IllegalArgumentException("values cannot be null or empty.");
  ...
  public void put(String key, Object value) {
    ...
    if (value.getClass() == String.class) {
      ...
      throw new PSAnalyticsQueryResultException("Class type is not supported.");
    }
  }
  ...
  private Map<String, DataType> types = new java.util.HashMap<>();
}
