...
public class PSGoogleAnalyticsProviderQueryHandler implements IPSAnalyticsProviderQueryHandler {
    ...
    private List<IPSAnalyticsQueryResult> getResultsForVisitsViewsBySite(String sitename, Report report)
            throws PSAnalyticsProviderException {
        var results = new ArrayList<IPSAnalyticsQueryResult>();
        var header = report.getColumnHeader();
        var dimensionHeaders = header.getDimensions();
    ...
    private synchronized Report executeGoogleQuery(ReportRequest requestQuery, String pid, String uid, String pwd)
        ...
        try {
            var analyticsReporting = PSGoogleAnalyticsProviderHelper.getInstance()
                    .initializeAnalyticsReporting(uid, pwd);
