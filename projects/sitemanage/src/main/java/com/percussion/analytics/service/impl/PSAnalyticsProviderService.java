...
public class PSAnalyticsProviderService implements IPSAnalyticsProviderService {
   ...
   *
   * @param sitename the name of the site, assumed not blank.
   * @return the profile of the site. It may be <code>null</code> if it is not configured for the
   *     site.
   */
  ...
  /**
   * The provider handler. Right now this is hard coded to use the Google Analytics handler. When a
   * new provider is implemented we will need to have spring load the appropriate handler.
   */
  private final IPSAnalyticsProviderHandler handler = new PSGoogleAnalyticsProviderHandler();

  /**
   * The provider error message handler. Right now this is hard coded to use the Google Analytics
   * message handler. When a new provider is implemented we will need to have spring load the
   * appropriate handler.
   */
  private final IPSAnalyticsErrorMessageHandler messageHandler =
      new PSGoogleAnalyticsErrorMessageHandler();

  /** The metadata service. Initialized via the constructor. Never <code>null</code> after that. */
