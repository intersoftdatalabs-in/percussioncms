...
public interface IPSCookieConsentService {
   ...
   * @param siteName - the name of the site. Used to concatenate with CSV file name.
  ...
  public String exportCookieConsentData(
      @PathParam("siteName") String siteName, @PathParam("csvFile") String csvFileName);

  /**
   * Returns the total number of cookie consent entries per site.
   *
   * @return A map with each site as the key and the total number of entries as the value.
   */
  public String getAllCookieConsentTotals();

  /**
   * Gets the total number of cookie consent entries per service for each site. Returns a map of all
   * cookies saved for the selected site.
   *
   * @param siteName - the name of the site to get the entries for.
   * @return A string response in Map<String, Integer> format. Key is site name value is total
   *     number of cookie entries saved per site.
   */
