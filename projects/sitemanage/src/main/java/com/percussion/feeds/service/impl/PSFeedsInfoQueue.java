 ...
 * delivery tier. The queue processor runs in a separate thread.
 *
 * @author erikserating
 */
@Deprecated // TODO: Refactor the feeds nonsense.  It should just be publishing feeds at publish
// time - not queuing and using a background thread.
public class PSFeedsInfoQueue implements InitializingBean {
  /** The metadata service, initialized in the ctor, never <code>null</code> after that. */
  private final IPSMetadataService metadataService;

  /** The delivery info service, initialized in the ctor, never <code>null</code> after that. */
   ...
   * @param descriptors the descriptors json object string, cannot be <code>null</code> or empty.
   */
  ...
  class QueueProcessor extends Thread {
    ...
    public void run() {
      ...
      PSDeliveryInfo prodService =
          deliveryInfoService.findByService("perc-metadata-services", "PRODUCTION");
      PSDeliveryInfo stagService =
          deliveryInfoService.findByService("perc-metadata-services", "STAGING");
      ...
      try {
        while (true) // Main process loop that never ends
        {
          ...
          Collection<PSMetadata> stagResults =
              metadataService.findByPrefix(META_KEY_STAGING_PREFIX);
