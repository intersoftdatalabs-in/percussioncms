 ...
 * PSSearchIndexEventQueue in the future, so that there is no copy and paste code.
 *
 * @author YuBingChen
...
public abstract class PSAbstractEventQueue<T> {
   ...
   * (hence the {@link #getNextEvent()} return null), then this will wait for a limited time before
   * returning, but will not process any new events that are ready after waiting. In this case, the
   * caller in different thread may call {@link #notifyEventQueue()} to wake up this and return
   * null.
   *
   * @param timeOut The number of milliseconds to wait before returning. Pass 0 to wait
   *     indefinitely, negative is the same as 0.
   * @return The next set of events to process, may be null if there are no events in the queue or
   *     if we waited.
