...
/**
 * A generic base class used for managing a queue. The queue can be either in memory (light weight)
 * or persistent (heavy weight) queue.
 *
 * <p>This class is inspired by PSSearchIndexEventQueue. It is possible to reuse this code with
 * PSSearchIndexEventQueue in the future, so that there is no copy and past code.
 *
 * @author YuBingChen
...
public abstract class PSAbstractEventQueue<T> {
  ...
  /**
   * Get next event which is returned by {@link #getNextEvent()}. If there is nothing in the queue
   * (hence the {@link #getNextEvent()} return <code>null</code>, then this will for a limited time
   * before returning, but will not process any new events that are ready after waiting. In this
   * case, the caller in different thread may call {@link #notifyEventQueue()} to wait up this and
   * return <code>null</code>.
   *
   * @param timeOut The number of milliseconds to wait before returning. Pass <code>0</code> to wait
   *     indefinitely, negative is the same as <code>0</code>.
   ...
   */
  protected PSPair<PSSiteQueue, Integer> getNextQueueEvent(long timeOut)
  ...
  /**
   * Monitor object to provide synchronization of queue access. Never <code>null</code> or modified.
   */
  ...
  /**
   * Thread to handle processing of events. Initialized by {@link #start()}, not <code>null</code>
   * or modified until {@link #shutdown()} is called.
   */
   ...
   * Value should only be modified by the {@link #shutdown()} method, where access is synchronized
   * appropriately.
   */
