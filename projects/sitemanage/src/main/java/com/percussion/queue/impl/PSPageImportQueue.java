...
public class PSPageImportQueue extends PSAbstractEventQueue<PSSiteQueue>
  ...
  public void addCatalogedPageIds(PSSite s, String userAgent, List<Integer> ids) {
  ...
  protected boolean doRun() {
    try {
      ...
      try {
        ...
        m_pageImporter.performPageImport(site, id, importingSite.getUserAgent());
   ...
   *
   * @param site the site of the page, assumed not <code>null</code>.
   * @param id the ID of the page, assumed not <code>null</code>.
  ...
  public void performPageImport(PSSite site, Integer id, String userAgent)
    ...
    m_importService.importCatalogedPage(site, pageId, userAgent, siteImportContext);
  }
  ...
  private PSSiteQueue createSiteQueue(Long siteId) {
    ...
    try {
      ...
      List<String> importedPages = m_pageCatalogService.findImportedPageIds(siteName);
      siteQueue.addImportedIds(getContentIds(importedPages));

      List<String> catalogedPages = m_pageCatalogService.findCatalogPages(siteName);
   ...
   *
   * @param ids {@link List}<{@link String}> with the string representation of ids. Assumed not
   *     <code>null</code>.
   * @return {@link List}<{@link Integer}> with the content ids. Never <code>null</code>, but may be
   *     empty.
   */
  private List<Integer> getContentIds(List<String> ids) {
    List<Integer> contentIds = new ArrayList<>();
    for (String id : ids) {
      contentIds.add(((PSLegacyGuid) m_idMapper.getGuid(id)).getContentId());
    }
    return contentIds;
  }
}
