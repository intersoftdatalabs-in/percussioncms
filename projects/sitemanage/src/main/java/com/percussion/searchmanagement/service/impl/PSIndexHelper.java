 ...
 *     <p>This class is meant to assist in loading locators into the search index queue. It has its
 *     own thread to handle the work so when large amounts of items need to be indexed due to a
 *     change of an item then we are not holding up the UI and the user can continue his or her
 *     task. The processing becomes a background process.
 */
...
public class PSIndexHelper implements Runnable {
  ...
  /**
   * Add items to the concurrent data structure so that they can be processed by the background
   * process.
   *
