...
public class PSSearchService implements IPSSearchService {
  ...
  private List<Integer> getContentIdsForSearchByStatus(PSSearchCriteria criteria) {
    ...
    try {
      ...
      List<Integer> contentIdList =
          searchHandler.searchAndGetContentIdsForSearchByStatus(request, search);
      List<Integer> newContentIdList = new ArrayList<>();

      for (Integer contentID : contentIdList) {

        if (!recycleService.isInRecycler(contentID.toString())) newContentIdList.add(contentID);
      }
  ...
  public PSPagedItemPropertiesList getExtendedSearchResults(PSSearchCriteria criteria)
    ...
    // Paginate
    PSPagedObjectList<IPSItemEntry> result =
        PSPagedObjectList.getPage(allItemEntries, startIndex, criteria.getMaxResults());
  ...
  public PSPagedItemList searchByStatus(PSSearchCriteria criteria, List<Integer> contentIdList)
    ...
    List<Integer> finalContentIdList =
        (List<Integer>) ipsPageDaoHelper.getContentIdsForFetchingByStatus(criteria, contentIdList);
    ...
    for (IPSItemEntry itemEntry : pagedItemEntries) {
      ...
      if (folderHelper.getParentFolderId(myGuid, false) == null) {
        log.debug(
            "Item (id = "
                + itemEntry.getContentId()
                + ") is not in a folder.  It will not be "
  ...
  /**
   * Gets the display format object ({@link PSSimpleDisplayFormat} according to the given display
   * format id. It returns the default value if the display format id passed in is null.
   *
  ...
  private PSSimpleDisplayFormat getDisplayFormat(Integer displayFormatId) {
    ...
    if (displayFormatId != null) id = displayFormatId;

    return uiService.getDisplayFormat(id);
  }

  /**
   * @param query query that to be escaped for the first character for the special characters
   *     supported for Lucene.
  ...
  private String excludeLocalWorkflow(String query, PSWSSearchParams searchParams)
    ...
    if (!StringUtils.isBlank(query)) query += EXCLUDE_WORKFLOW + localId;
    else {
      // if no query, can't pass "NOT" alone, get back nothing, so add workflow condition if not in
      // params
   ...
   * @return <code>true</code> if the the params contain the workflow id field, <code>false</code>
   *     otherwise.
   */
  private boolean hasWorkflowParam(PSWSSearchParams searchParams) {
    ...
    for (PSWSSearchField field : fields) {
      if (field.getName().equalsIgnoreCase(WORKFLOW_ID)) return true;
    }
  ...
  private List<Integer> getSearchableWorkflowIds()
    ...
    for (PSObjectSummary summary : workflowSums) {
      ...
      if (wfId == localWorkflowId) continue;
      wfIds.add(wfId);
    }
  ...
  /**
   * Query to include shared workflows using ranges, {0} should be localWFId - 1, {1} localWFId + 1,
   * {2} max value possible
   */
  private static final String SHARED_WORKFLOW_RANGE =
      "(" + WORKFLOW_ID + ":[0 TO {0}] OR " + WORKFLOW_ID + ":[{1} TO {2}])";
  ...
  public PSSearchCriteria validateSearchCriteria(PSSearchCriteria criteria) {
    ...
    if (systemService.isMySQL()) type = SecureStringUtils.DatabaseType.MYSQL;
    else if (systemService.isOracle()) type = SecureStringUtils.DatabaseType.ORACLE;
    else if (systemService.isDB2()) type = SecureStringUtils.DatabaseType.DB2;
    else if (systemService.isMsSQL()) type = SecureStringUtils.DatabaseType.MSSQL;
      ...
      for (Map.Entry<String, String> field : fields.entrySet()) {
        ...
        if (f != null) {
          if (f.getDataType().equalsIgnoreCase(PSField.DT_INTEGER)
            ...
            // Unsure on data type so just make sure there is no SQL injection possible DT_TEXT is
            // covered here.
