 ...
 * Currently changes the last modified date/user to match the last user's changes if the item was
 * actually last modified by the system during a transition from Pending to Live
 *
 * @author JaySeletz
 */
...
public class PSSearchIndexFieldValueModifier
  ...
  public void modifyFields(Map<String, Object> itemFragment) {
    try {
      ...
      if (wfId <= 0 || StringUtils.isBlank(stateName)) return;
      ...
      if (StringUtils.isBlank(userName) || StringUtils.isBlank(strDate)) return;
      ...
      for (PSState test : states) {
        if (test.getName().equals(stateName)) state = test;
      }
      ...
      PSPair<String, String> lastModInfo =
