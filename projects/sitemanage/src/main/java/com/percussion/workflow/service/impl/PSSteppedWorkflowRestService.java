...
public class PSSteppedWorkflowRestService {
   ...
   *     add to that workflow. Must not be emtpy or <code>null</code>
   * @return a <code>PSUiWorkflow</code> object maybe empty never <code>null</code>
   * @throws PSWorkflowEditorServiceException, if the supplied object is invalid.
   */
  @POST
   ...
   *     </code>
   * @throws PSWorkflowEditorServiceException if the supplied workflow is not found in the system or
   *     there are items associated with the workflow.
   */
  @DELETE
  @Path("/{workflowName:.*}")
   ...
   * @param stepName the name of the step to be updated that step (previous and actual step name).
  ...
  /**
   * Gets the list of states that belong to the workflow passed as a parameter, and builds a <code>
   * PSEnumVals</code> object with them, so they can be presented as choices in the screen. If the
   * workflow passed as parameter doesn't exist, the system selects the first workflow from the list
   * returned from service.getWorkflowList().
   *
  ...
  /**
   * Gets a list of workflow states, builds another list with their names, and then returns that
   * list ordered alphabetically in ascending order.
   *
