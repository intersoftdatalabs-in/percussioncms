...
public interface IPSItemService {
  ...
  /**
   * Sets the start and end date for a given page or asset resource. Dates must be in the following
   ...
   * @param name of the folder to which the contents of the source folder will be copied, may not be
   *     <code>null</code> or empty.
   */
  public Map<String, String> copyFolder(String srcFolder, String destFolder, String name)
      throws PSItemServiceException, IPSItemWorkflowService.PSItemWorkflowServiceException,
          PSErrorResultsException, PSDataServiceException;

  /**
   * Calculates the site impact of an asset and returns its result as a String representation of a
   * JSONObject, the json object will have three arrays. { "pages":[PSItemProperties],
   * "templates":[{"template":PSTemplateSummary, "site":sitename}], } pages is an array of
   * PSItemProperties objects of all the pages that the asset is related to. templates is an array
   * of objects that consist of PSTemplateSummary object and site name of all the templates the
   * asset is related to. If the asset is related to another asset(case of inline images and file
   * links) then it's parent page or template is returned as impacted item. If the parent asset is
   * not on any page or template then it is ignored. If the asset is not used on any page or
   * template then empty arrays are returned for pages and templates.
   *
   ...
   * exists logs a warning ignores the request.
   *
