...
public class PSBulkApprovalJob extends PSAsyncJob {
  ...
  private void approvePage(PSApprovableItem item) {
    try {
      ...
      if (isInApprovedState(item)
          || isCheckedOutByOthers(item)
          || !hasFolderAccess(item)
          || !hasApproveTransition(item)) {
        processedItems.add(item);
        return;
      }

      PSItemTransitionResults transitionResult =
          itemWorkflowService.performApproveTransition(item.getId(), false, null);
      ...
      if (transitionResult.getFailedAssets().isEmpty()) {
        ...
        String msg =
            "You cannot approve this page as one or more related asset(s) could not be approved.";
  ...
  /**
   * Helper method to check whether user has at least write permission to the parent folder. If user
   * doesn't have the access then updates the item status and error message.
   *
   * @param item assumed not <code>null</code>.
  ...
  private boolean hasFolderAccess(PSApprovableItem item) throws PSValidationException {
    ...
    boolean result = false;
    if (access.equals(PSFolderPermission.Access.ADMIN)
        || access.equals(PSFolderPermission.Access.WRITE)) {
      result = true;
    } else {
      String msg = "You do not have permission to approve this page.";
      handleError(item, msg);
  ...
  /**
   * Helper method to check whether the item is already in an approved state or not if it is in,
   * then updates the supplied item and returns true otherwise false.
   *
   * @param item assumed not <code>null</code>
