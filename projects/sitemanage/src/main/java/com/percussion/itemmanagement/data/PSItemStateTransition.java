...
/**
 * The class contains the workflow state and all possible transitions for an item. The transition
 * trigger names (along with the item ID) can be used to transition the item into different state
 * upon request.
 *
 * @author yubingchen
 */
...
public class PSItemStateTransition extends PSAbstractDataObject {
  ...
  /**
   * Gets the ID of the workflow that contains the specified state see {@link #getStateId()}.
   *
   ...
   *
   * @return all possible transition triggers, never <code>null</code>, but may be empty.
   */
   ...
   *
   * @param triggers the new set of trigger names, may be <code>null</code> or empty, <code>null
   *     </code> value is the same as empty list.
   */
  ...
  /** A list of all possible transition trigger names for the specified workflow state. */
