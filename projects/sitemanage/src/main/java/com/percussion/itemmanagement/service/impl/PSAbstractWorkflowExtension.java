...
/**
 * An abstract extension that allows you to workflow items into the live state on publish (post
 * edition task extension) and lock local content when the page is moved to pending state (workflow
 * action extension).
 *
 * @author adamgent
 */
public abstract class PSAbstractWorkflowExtension implements IPSExtension {
  ...
  /** Parameter name for what state to workflow from. Parameters come from the extension manager. */
  ...
  /**
   * Holds transient data that only applies to a single call to the method "perform".
   *
   * <p>This is needed because extensions are singletons.
   *
   * @author adamgent
   * @see PSWorkflowEditionTask#perform(IPSEdition, IPSSite, Date, Date, long, long, boolean, Map,
   *     IPSEditionTaskStatusCallback)
   */
  public class WorkflowItemWorker {
    ...
    public boolean processItem(IPSPubItemStatus item, IPSSite site, boolean isDefaultPubServer) {
      ...
      if (o == Operation.PUBLISH && s == Status.SUCCESS) {
        ...
        if (s != Status.SUCCESS) {
          log.warn(
              format(
                  "Not workflowing item:{0} because Message:{3} Operation:{1}, Status{2}",
    ...
    /**
     * Process the workflow context. Called from a workflow action.
     *
    ...
    /**
     * Workflows the item if possible. Exceptions will be logged but not thrown. An exception should
     * never be thrown from this method or else the other items in this publish job will not be
     * workflowed.
     *
     * @param item never <code>null</code>.
     */
    protected boolean workflowItemIfPossible(
        IPSPubItemStatus item, IPSSite site, boolean isDefaultPubServer) {
      notNull(item, "item");
      try {
        var wfItem = getWorkflowItem(item);
        // Skip the workflow transition if the item published does not match the current revision to
        // cover scheduled publish case.
        ...
        if (wfItem.assetType == AssetType.PAGE || wfItem.assetType == AssetType.RESOURCE) {
     ...
     * @param pageWfItem - the workflow item for the page. This is used to perform the allowed sites
     *     check. May be <code> null </code> if the workflow item passed is the page itself and not
     *     its shared assets.
     * @return true if all the assets where correctly transitioned, false if at least one of the was
     *     not correctly transitioned.
     */
    protected boolean transitionIfPossible(
        List<WorkflowItem> items,
        WorkflowItem pageWfItem,
        IPSSite site,
        boolean isDefaultPubServer) {
      boolean success = true;
      for (WorkflowItem wfItem : items) {
        try {
          if (isAlreadyWorkflowed(wfItem)) {
            ...
            if (log.isDebugEnabled()) {
              log.debug(
                  "Did not workflow item: " + wfItem + " because its already been workflowed.");
            }
            return success;
          }
          ...
          if (isValidWorkflowState(wfItem)) {
            if (pageWfItem != null && !isAllowedSitePublishable(wfItem, pageWfItem)) {
              ...
              if (log.isDebugEnabled()) {
                log.debug(
                    "Did not workflow item: "
                        + wfItem
                        + " because its not allowed for the current site.");
              }
            ...
            log.debug(
                "Did not workflow item: " + wfItem.guid + " because its not in a valid state");
            wfItem.status = ItemStatus.FAILED;
            success = false;
          }

          if (site != null && isDefaultPubServer) {
            contentChangeService.deleteChangeEvents(
                site.getSiteId(),
                wfItem.itemSummary.getContentId(),
    ...
    /**
     * Checks if the asset workflow item's root level folder has the current site allowed to
     * publish. If it's not then the asset shouldn't be transitioned to live (since it was not
     * included in the page's code)
     *
     * @author federicoromanelli
     * @param item - the asset workflow item to perform the check. Never <code> null </code>
     * @param pageWfItem - the page workflow item used to retrieve the site id. Never <code> null
     *     </code>
     * @return true if the asset could be transitioned (meaning its root level folder has the site
     *     included as allowed sites)
     */
    private boolean isAllowedSitePublishable(WorkflowItem item, WorkflowItem pageWfItem) {
      ...
      String rootLevelFolderAllowedSites =
          folderHelper.getRootLevelFolderAllowedSitesPropertyValue(item.guid.toString());
    ...
    private int getPublishRevision(
        PSComponentSummary summary, boolean isPublishable, boolean isStaging) {
      if (isStaging) {
    ...
    protected List<WorkflowItem> getSharedAssetWorkflowItems(WorkflowItem page)
      ...
      // if the templateId hasn't been seen, we check to see if the template is an owner of any
      // shared assets
     ...
     *
     * <p>This method is safe to override by subclasses.
     *
     * @param item Never <code>null</code>
     */
    ...
    /**
     * The name of the state that the item must be in if it is to be workflowed.
     *
     * @return never <code>null</code>.
     */
    ...
    /**
     * The trigger name used to transition the item if its in the right workflow state.
     *
     * @return never <code>null</code>.
     */
