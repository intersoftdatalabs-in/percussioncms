...
public class PSItemService implements IPSItemService {
  private static int MAX_PAGES_SITEIMPACT =
      25; // Stop looking for relationships after we hit this # of Pages.

  static final Map<String, String> UNIQUE_FIELDS =
  ...
  public PSItemService(
      ...
      IPSItemWorkflowService itemWfService,
      IPSFolderHelper folderHelper,
      @Qualifier("contentItemDao") IPSContentItemDao contentItemDao,
      IPSAssetDao assetDao,
      IPSTemplateService templateService,
      IPSUserItemsDao userItemDao,
      IPSNotificationService notificationService,
  ...
  public PSRevisionsSummary getRevisions(@PathParam("id") String id) throws PSItemServiceException {
    ...
    try {
      ...
      List<PSContentStatusHistory> _revisions =
          systemService.findContentStatusHistory(idMapper.getGuid(guid));
      ...
      try {
        ...
        throw new PSItemServiceException(
            "The page you are trying to act on no longer exists in the system.", e);
      }
      ...
      if (_revisions.isEmpty()) {
        ...
        // The following code adds an entry from the item summary if the item is checked out to the
        // current user
  ...
  @Path("pubhistory/{id}")
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public List<PSItemPublishingHistory> getPublishingHistory(@PathParam("id") String id)
  ...
  public String getLastComment(@PathParam("id") String id) throws PSItemServiceException {
    String lastComment = "";
    List<PSContentStatusHistory> historyEntries =
        systemService.findContentStatusHistory(idMapper.getGuid(id));
  ...
  private List<PSComment> createCommentsFromHistory(List<PSContentStatusHistory> historyEntries) {
    ...
    for (PSContentStatusHistory entry : historyEntries) {
      if (StringUtils.isNotBlank(entry.getTransitionComment())) {
        comments.add(
            new PSComment(
                entry.getTransitionComment(),
                entry.getActor(),
                entry.getTransitionLabel(),
                entry.getEventTime()));
      }
    }
  ...
  public PSNoContent restoreRevision(@PathParam("id") String id) {
    try {
      ...
      try {
        ...
        throw new WebApplicationException(
            "An unexpected error occurred while restoring the prior revision, "
                + "see log for details.",
            e);
      }
  ...
  public List<PSAssetSiteImpact> getAssetSiteImpact(List<String> assetIds) {
    ...
    for (String id : assetIds) {
      ...
      try {
        ...
        log.error(
            "Error processing Site Impact for owner Pages and Templates for Asset id: {} Error: {}",
            id,
            PSExceptionUtils.getMessageForLog(e));
        log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        // continue
      ...
      for (String page : ownerPages) {
        try {
          ...
          log.error(
              "Error processing Site Impact with owner Page {} for Asset id: {} Error: {}",
              page,
              id,
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        ...
        try {
          ...
          log.error(
              "Error processing Site Impact detecting owner Site for Page {} for Asset id: {} Error: {}",
              page,
              id,
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      ...
      for (String templateId : ownerTemplates) {
        ...
        try {
          ...
          log.error(
              "Error processing Site Impact for Template " + templateId + " for Asset id:" + id, e);
        }
        ...
        try {
          ...
          log.error(
              "Error processing Site Impact detecting owner Site for Template "
                  + templateId
                  + " for Asset id:"
                  + id,
              e);
    ...
    try {
      ...
      for (String page : ownerPages) {
        ...
        try {
          ...
          log.error(
              "An error occurred while processing Asset Impact checking item properties for Page: {} Error: {}",
              page,
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        }
      ...
      for (String templateId : ownerTemplates) {
        ...
        if (sites != null && !sites.isEmpty()) {
          ...
          if (sites != null) {
            if (sites.get(0) != null) templateItem.put("site", sites.get(0).getName());
          }
          ...
          if (template != null)
            log.warn(
                "template {} with id {} is not associated to any sites",
                template.getName(),
                template.getId());
        }
      }
  ...
  private void fillOwners(String assetId, Set<String> ownerPages, Set<String> ownerTemplates)
    ...
    if (owners != null) {
      for (String owner : owners) {
        ...
        if (typeEnum.equals(IPSWorkflowHelper.PSItemTypeEnum.PAGE)) ownerPages.add(owner);
    ...
    for (PSManagedLink link : links) {
      ...
      if (limit <= 0 || ownerPages.size() <= limit) {
        if (link.getParentId() > 0) {
          try {
            PSContentItem parent =
                contentItemDao.find(idMapper.getGuidFromContentId(link.getParentId()).toString());
          ...
          log.warn(
              "Managed Link with a parentId of -1 detected for Link Id: {} . Skipping link in Site Impact.",
              link.getLinkId());
   ...
   * @return a map where the key is a revision number and the value is a map (sorted descending) of
   *     content status history id's to content status history objects.
   */
  private Map<Integer, Map<Long, PSContentStatusHistory>> buildRevisionMap(
      List<PSContentStatusHistory> history) {
    Map<Integer, Map<Long, PSContentStatusHistory>> revMap = new HashMap<>();
  ...
  /**
   * Used to get the latest meaningful revisions from the content status history of an item. We are
   * not concerned with Quick Edit revisions unless this is all that is available.
   *
   * @param history content status history list, assumed not <code>null</code>.
  ...
  private List<PSRevision> getRevisions(List<PSContentStatusHistory> history) {
    ...
    for (Integer rev : revMap.keySet()) {
      ...
      for (Long historyId : rMap.keySet()) {
        ...
        if (prevStateName.equalsIgnoreCase("Quick Edit")
            && currentStateName.equalsIgnoreCase("Quick Edit")) {
        ...
        String lastModifierName =
            currentStateName.equalsIgnoreCase("Live")
                ? getLastModifierFromCheckOut(rMap, historyId)
                : hist.getLastModifierName();
        psRevision =
            new PSRevision(
                hist.getRevision(),
        ...
        if (!transitionLabel.equalsIgnoreCase("CheckOut")
            && !transitionLabel.equalsIgnoreCase("Edit")) {
  ...
  /**
   * Finds the last modifier name of the first occurrence of a checkout transition, starting from
   * the specified id.
   *
   * @param rMap map of history id to content status history, ordered descending. Assumed not <code>
   *     null</code>.
  ...
  private PSRevision getRevision(PSComponentSummary sum) {
    ...
    return new PSRevision(
        sum.getHeadLocator().getRevision(),
        PSDateUtils.getDateToString(sum.getContentLastModifiedDate()),
        sum.getContentLastModifier(),
        stateName);
  }

  /**
   * Comparator used for sorting the content status history map for each revision. Entries are
  ...
  /**
   * Converts the given from save to the display format and from display format to save format
   * depending on the supplied flag.
   *
   * @param inputDate date String expected to be in MM/dd/yyyy HH:mm format if the isForSave is true
   *     otherwise yyyy-MM-dd HH:mm:ss.S, may be <code>null</code> or empty.
   * @return date String in the following format yyyy-MM-dd HH:mm:ss.S if isForSave is true
   *     otherwise MM/dd/yyyy HH:mm, returns empty string if the inputDate is blank.
  ...
  private String convertDate(String inputDate, boolean isForSave) throws ParseException {
    ...
    FastDateFormat format1 =
        FastDateFormat.getInstance(isForSave ? "MM/dd/yyyy hh:mm a" : "yyyy-MM-dd HH:mm:ss.S");
    FastDateFormat format2 =
        FastDateFormat.getInstance(isForSave ? "yyyy-MM-dd HH:mm:ss.S" : "MM/dd/yyyy hh:mm a");
  ...
  /**
   * Prepares the supplied item for restoring a prior revision. Assumes that the item is valid for
   * restoring. Make sure to call {@link #validateItemRestorable(String)} before calling this
   * method. Checks in the item if it is checked out to the current user. Transitions to Quick edit
   * state if the item is in Live or Pending state. Uses the head locator to checkin and transition.
   *
   ...
   */
  private void validateItemRestorable(String id)
      throws PSItemServiceException, PSValidationException, PSNotFoundException {
      ...
      String invalidMessage =
          new String(
              MessageFormat.format(
                      "An unexpected error occurred while promoting this {0}"
                          + ", see log for details.",
                      args)
    ...
    if (asmtType == PSAssignmentTypeEnum.READER || asmtType == PSAssignmentTypeEnum.NONE) {
      ...
      String invalidMessage =
          new String(
              MessageFormat.format("You are not authorized to restore this {0}.", args)
                  .getBytes(StandardCharsets.UTF_8));
      validationResponse.setOperation(invalidMessage);
      validateParameters(opName).reject(opName, invalidMessage).throwIfInvalid();
    }
    ...
    if (access == PSFolderPermission.Access.READ) {
      ...
      String invalidMessage =
          new String(
              MessageFormat.format("You do not have permission to restore this {0}.", args)
                  .getBytes(StandardCharsets.UTF_8));
      validationResponse.setOperation(invalidMessage);
      validateParameters(opName).reject(opName, invalidMessage).throwIfInvalid();
    }
    ...
    if (StringUtils.isNotBlank(sum.getCheckoutUserName())
      ...
      String invalidMessage =
          new String(
              MessageFormat.format(
                      "User {1} is editing this {0}. You cannot restore " + "earlier revisions.",
                      args)
                  .getBytes(StandardCharsets.UTF_8));
      validationResponse.setOperation(invalidMessage);
      validateParameters(opName).reject(opName, invalidMessage).throwIfInvalid();
  ...
  /**
   * Check the validity of the date againt startdate with current date and end date with current
   * date if req.startdate >= currentdate or req.enddate >=currentdate then that is valid. other
  ...
  private PSNoContent dateValidation(PSItemDates req) {
    ...
    try {
      ...
      FastDateFormat formatter = FastDateFormat.getInstance("MM/dd/yyyy hh:mm a");
      ...
      if (!StringUtils.isBlank(req.getStartDate())) {
        ...
        if (!(startDate.getTime() >= currentDateFormat.getTime())) {
          ...
          invalidMessage =
              invalidMessage.concat(
                  "Publish date or time precedes CM1 date of " + currentDateFormat + ".");
          validationResponse.setOperation(invalidMessage);
          validateParameters(opName).reject(opName, invalidMessage).throwIfInvalid();
        }
      }
      if (!StringUtils.isBlank(req.getEndDate())) {
        ...
        if (!(endDate.getTime() >= currentDateFormat.getTime())) {
          ...
          invalidMessage =
              invalidMessage.concat(
                  "Removal date or time precedes CM1 date of " + currentDateFormat + ".");
          validationResponse.setOperation(invalidMessage);
          validateParameters(opName).reject(opName, invalidMessage).throwIfInvalid();
        }
      }
  ...
  public PSItemDates getItemDates(@PathParam("id") String id) throws PSItemServiceException {
    ...
    try {
      ...
      String startDateField =
          fields.get(START_DATE) != null ? fields.get(START_DATE).toString() : "";
  ...
  public Map<String, String> copyFolder(String srcPath, String destFolder, String name)
      throws IPSItemWorkflowService.PSItemWorkflowServiceException, PSErrorResultsException,
  ...
  private List<PSPageLinkedToItem> getPageLinkItem(
      PSLocator locator, String defaultString, PSRelationship relationship) throws Exception {
    ...
    final List<PSItemSummary> owners =
        contentWs.findOwners(idMapper.getGuid(locator), filter, false);
    for (PSItemSummary owner : owners) {
      // In case RichTextEditor is associated with the current page and is having a link to
      // someother page
      ...
      if (pagePath != null) {
        if (relationship == null) {
          PSPageLinkedToItem item =
              new PSPageLinkedToItem(owner.getGUID().toString(), pagePath, null);
          items.add(item);
        } else {
          PSPageLinkedToItem item =
              new PSPageLinkedToItem(
                  owner.getGUID().toString(), pagePath, relationship.getGuid().toString());
          items.add(item);
        }
      }
    }
    ...
    try {
      ...
      if (relationships != null && relationships.size() > 0) {
        for (PSRelationship relationship : relationships) {
          ...
          // Trying to avoid returning multiple of same owner or if Relationship is deleted
          // (dependent revision = -1)
          if (!owners.contains(Integer.valueOf(ownerId))
              && relationship.getDependent().getRevision() != -1) {
      ...
      // here it may be better not to throw exception, just proceed with the 'remove from site' as
  ...
  /**
   * Copy folder method is passed assetMap to store current state of copied assets while recursing
   * to allow for rollback on exception.
   *
   * @param assetMap
   * @param srcPath
   * @param destFolder
  ...
  private void copyFolder(
      Map<String, String> assetMap, String srcPath, String destFolder, String name)
      throws Exception {
    ...
    for (IPSItemSummary sum : sums) {
      ...
      if (sum.isFolder()) {
        ...
        // copy the item (currently the item is assumed to be an asset), eventually this should
        // delegate to
        // the copy methods of the page service (for site sub-folder copy) and asset service (when
        // copy
        ...
        List<PSCoreItem> items =
            contentWs.newCopies(
                Collections.singletonList(idMapper.getGuid(id)),
        ...
        if (type.equals(PSAssetRestService.FORM_CONTENT_TYPE)) {
          ...
          renderedForm =
              StringUtils.replaceOnce(
                  renderedForm, "{jQuery('#" + sumName + "')", "{jQuery('#" + copyName + "')");
          renderedForm =
              StringUtils.replaceOnce(
                  renderedForm,
                  "<form name=\"" + sumName + "\"",
          ...
          formData =
              StringUtils.replaceOnce(
                  formData, "{\"name\":\"" + sumName + "\"", "{\"name\":\"" + copyName + "\"");
          ...
          int i = 2;
          String unique_name =
              StringUtils.defaultString((String) fieldMap.get(uniqueField), sumName);
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public PSNoContent addToMyPages(
      @PathParam("userName") String userName, @PathParam("pageId") String pageId) {
    try {
   ...
   *
   * @param source assumed to be username if the isUser is <code>true</code> otherwise assumed to be
   *     content item guid.
   * @param isUser set to <code>true</code> if the source is user name, <code>false</code> if the
   *     source is content item guid
   */
   ...
   */
  @Override
  public void rollBackCopiedFolder(Map<String, String> itemMap, String folderName)
      throws PSItemServiceException {
    ...
    if (folderName != null) {
      ...
      try {
        ...
        log.warn(
            "cannot find copied folder during rollback "
                + folderName
  ...
  public class SecureKeyRotationListener implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
      ...
      } catch (PSReportFailedToRunException
          | PSValidationException
          | PSNotFoundException
          | IPSGenericDao.LoadException e) {
        log.error(
            "Secure Key Rotation Failed to get Asset Pages with Encryption. ERROR: {}",
            PSExceptionUtils.getMessageForLog(e));
      }
    }
  ...
  private static final String[] WORKFLOW_TRANSITION_IGNORE_LIST = {
    "CheckOut", "CheckIn", "Edit", "Live"
  };
