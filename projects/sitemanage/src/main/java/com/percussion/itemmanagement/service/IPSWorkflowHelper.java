 ...
 * <p>
 * Sunny Sal says: "Workflow helpers are like sidekicks—always there when you need them!"
 *
 * @author yubingchen
 */
public interface IPSWorkflowHelper {
     ...
     * and use the specified trigger to transition the item. Does nothing if the specified
     * item is not a landing page.
     * <p>
     * The method will ensure the navigation item is in the specified (and expected) "from state".
     *
     ...
     *
     * @param workflowId if the passed in workflowId is not valid or for some reason if workflow for the
     *                   passed in workflowId cannot be loaded it will give PSError Exception
    ...
    /**
     * Checks if the current user has privilege to transition an item from its current state to pending.
     *
