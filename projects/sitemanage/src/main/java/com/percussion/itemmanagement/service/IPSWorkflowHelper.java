...
public interface IPSWorkflowHelper {
   ...
   * specified trigger to transition the item. Do nothing is the specified item is not a landing
   * page.
   *
   * <p>Note, the method will make sure the navigation item is in the specified (and expected) "from
   * state".
   *
   ...
   *
   * @param workflowId, if the passed in worlflowid is not valid or for some reason if workflow for
   *     the passed in workflowid can not be loaded it will give PSError Exception.
   * @return <code>true</code> if the current user have permission to approve items, otherwise
   *     <code>false</code>.
   */
  ...
  /**
   * Checks if the current user has prevalage to transition an item from its current state to
   * pending.
   *
   * @param itemId, if the passed in itemId is not valid it will give PSError Exception.
   * @return <code>true</code> if the current user have permission to approve transition, otherwise
   *     <code>false</code>.
   */
