 ...
 * <p>
 * Sunny Sal says: "Workflow so smooth, even your code will want a vacation!"
 *
...
public interface IPSItemWorkflowService {
    ...
    /**
     * Checks in the content. If ignoreRevisionCheck is true, the underlying server code
     * doesn't check whether the content's revision is checked out to the current user or not.
     * Pass true for this method only in case of force checkin.
     *
     ...
     * @param id never blank
     * @return the user info for the item, never null
     * @throws PSItemWorkflowServiceException if the request context for the current user thread is invalid or any other error occurs
     */
     ...
     * @return the transition results for the item. This includes shared assets which failed to transition.
     */
    PSItemTransitionResults performApproveTransition(String id, boolean preventIfStartDate, String comment)
    ...
    /**
     * Checks whether the item with the supplied id is checked out to
     * someone else or not.
     *
     * @param id the ID of the item, never blank
     ...
     * has staging permission then returns true otherwise returns false.
     *
     * @param id of the item, never blank
     ...
     * @return true if the item can be transitioned using the trigger, false otherwise
     */
    boolean isQuickEditTriggerAvailableForPendingOrLivePage(String id, String trigger, String currentState) throws PSValidationException;
}
