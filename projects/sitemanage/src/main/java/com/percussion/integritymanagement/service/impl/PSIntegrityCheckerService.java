...
public class PSIntegrityCheckerService implements IPSIntegrityCheckerService {
  ...
  @Autowired private IPSIntegrityCheckerDao integrityDao;

  @Autowired private IPSDTSStatusProvider dtsStatusProvider;

  @Autowired private IPSAssetService assetService;

  @Autowired private IPSItemWorkflowService itemWorkflowService;
   ...
   * @return Map<String, PSPair<TaskStatus, String>> map of task name and pair of status and
   *     message.
   */
  private Map<String, PSPair<TaskStatus, String>> runImageTasks(String token) {
    ...
    try (InputStream in =
      ...
      PSPair<TaskStatus, String> imgCreate =
          new PSPair<>(TaskStatus.FAILED, PSExceptionUtils.getMessageForLog(e));
      result.put(IMAGE_CREATE_TASK, imgCreate);
    ...
    try {
      ...
      PSPair<TaskStatus, String> imgApprove =
          new PSPair<>(TaskStatus.FAILED, PSExceptionUtils.getMessageForLog(e));
      result.put(IMAGE_APPROVE_TASK, imgApprove);
    }
    ...
    try {
      ...
      PSPair<TaskStatus, String> imgDelete =
          new PSPair<>(TaskStatus.FAILED, PSExceptionUtils.getMessageForLog(e));
      result.put(IMAGE_DELETE_TASK, imgDelete);
    }
  ...
  private void validateUsage() throws PSDataServiceException {
    if (!userService.isAdminUser(userService.getCurrentUser().getName())) {
      throw new RuntimeException(
          "You are not authorized to use " + PSIntegrityCheckerService.class.getName() + " API.");
    }
    if (!utilityService.isSaaSEnvironment())
      throw new RuntimeException(
          "The "
              + PSIntegrityCheckerService.class.getName()
              + " API is not supported in your environment.");
  }
    ...
    try {
      ...
      for (IPSSite site : sites) {
        ...
        try {
          ...
          PSIntegrityTask task =
              createErrorTask(
                  IMAGE_PUBLISH_TASK + ":" + site.getName(),
                  token,
                  IntegrityTaskType.cm1.toString(),
                  e);
          task.setTaskProperties(taskProps);
          result.add(task);
        }
      ...
      PSIntegrityTask task =
          createErrorTask(IMAGE_PUBLISH_TASK, token, IntegrityTaskType.cm1.toString(), e);
      result.add(task);
    }
    // call
    return result;
