...
public class PSIntegrityCheckerService implements IPSIntegrityCheckerService {
  ...
  private void runTasks(IntegrityTaskType type) throws IPSGenericDao.SaveException {
    ...
    if (status != null) {
      ...
      // Roll up the failure status, if any of the tasks fail then we should mark the top status as
      // failed.
  ...
  private Map<String, PSPair<TaskStatus, String>> runImageTasks(String token) {
    ...
    try (InputStream in =
      ...
      result.put(
          IMAGE_CREATE_TASK, new PSPair<>(TaskStatus.FAILED, PSExceptionUtils.getMessageForLog(e)));
      return result;
    }
    ...
    try {
      ...
      result.put(
          IMAGE_DELETE_TASK, new PSPair<>(TaskStatus.FAILED, PSExceptionUtils.getMessageForLog(e)));
    }
