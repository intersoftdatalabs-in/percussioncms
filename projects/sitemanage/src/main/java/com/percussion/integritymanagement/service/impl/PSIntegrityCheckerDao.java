...
public class PSIntegrityCheckerDao
  ...
  public PSIntegrityStatus find(String token) {
    ...
    if (token != null) criteria.where(builder.equal(critRoot.get("token"), token));
    criteria.orderBy(builder.desc(critRoot.get("startTime")));
    List<PSIntegrityStatus> results = entityManager.createQuery(criteria).getResultList();
  ...
  public List<PSIntegrityStatus> find(Status status) {
    ...
    if (status != null) criteria.where(builder.equal(critRoot.get("status"), status));
   ...
   *
   * @param status the publish server in question, assumed not <code>null</code>.
   */
