...
/**
 * Field input transformer to process/update item paths in a JSON payload.
 * Expects a JSON object with an array named percJSONConfig as input, calls the managed link service to manage any links in the payload,
 * and returns the updated payload with resulting link ids.
 * For new items, PSManagedItemPathPreProcessor should be used to initialize new item links.
 * @author NateChadwick
 */
public class PSManagedJSONPayloadPathInputTransformer extends PSDefaultExtension implements IPSFieldInputTransformer {
    ...
    public Object processUdf(Object[] params, IPSRequestContext request) throws PSConversionException {
        ...
        try {
            ...
            try {
                ...
                log.debug("Found {} array.", IPSManagedLinkService.PERC_CONFIG);
            ...
            for (var i = 0; i < objectArray.length(); i++) {
                ...
                // Images
                if (entry.has(IPSManagedLinkService.PERC_IMAGEPATH) && entry.has(IPSManagedLinkService.PERC_IMAGEPATH_LINKID)) {
                    var newLinkId = manageLinks(request, entry.getString(IPSManagedLinkService.PERC_IMAGEPATH), entry.getString(IPSManagedLinkService.PERC_IMAGEPATH_LINKID));
                    entry.put(IPSManagedLinkService.PERC_IMAGEPATH_LINKID, newLinkId);
                    objectArray.put(i, entry);
                }

                // Files
                if (entry.has(IPSManagedLinkService.PERC_FILEPATH) && entry.has(IPSManagedLinkService.PERC_FILEPATH_LINKID)) {
                    var newLinkId = manageLinks(request, entry.getString(IPSManagedLinkService.PERC_FILEPATH), entry.getString(IPSManagedLinkService.PERC_FILEPATH_LINKID));
                    entry.put(IPSManagedLinkService.PERC_FILEPATH_LINKID, newLinkId);
                    objectArray.put(i, entry);
                }

                // Pages
                if (entry.has(IPSManagedLinkService.PERC_PAGEPATH) && entry.has(IPSManagedLinkService.PERC_PAGEPATH_LINKID)) {
                    var newLinkId = manageLinks(request, entry.getString(IPSManagedLinkService.PERC_PAGEPATH), entry.getString(IPSManagedLinkService.PERC_PAGEPATH_LINKID));
    ...
    private String manageLinks(IPSRequestContext request, String path, String linkId) {
        ...
        if (StringUtils.isBlank(cid) || !StringUtils.isNumeric(cid)) {
            ...
            var result = service.manageItemPath(null, path, linkId);
            ...
            return service.manageItemPath(cid, path, linkId);
        }
    }
