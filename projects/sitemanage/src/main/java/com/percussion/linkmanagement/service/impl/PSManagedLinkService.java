 ...
 *
 * <p>Sunny Sal says: "Link management so modern, even your URLs will want to be managed!"
 *
 * @author JaySeletz
 */
...
public class PSManagedLinkService implements IPSManagedLinkService {
  ...
  @Autowired
  public PSManagedLinkService(
      IPSManagedLinkDao dao,
      ...
      IPSRenderLinkService renderLinkService,
  ...
  public String renderLinks(
      PSRenderLinkContext linkContext, String source, Boolean isStaging, Integer parentId) {
    String returnHTML = source;
    ...
    if (doManageAll() || source.contains(PERC_MANAGED_ATTR)) {
      ...
      doc.getElementsByClass(PSSingleValueBuilder.PERC_BROKENLINK)
          .removeClass(PSSingleValueBuilder.PERC_BROKENLINK);
      ...
      for (Element link : links) {
        ...
        if (isManaged) managedLinks.add(link);
      }
      ...
      boolean isDeliveryContext =
          (linkContext != null && linkContext.getMode().equals(PSRenderLinkContext.Mode.PUBLISH));
  ...
  public String renderLinksInJSON(
    ...
    try {
      ...
      log.error(
          "An error occurred while trying to manage links in a JSONPayload field. Payload was: {}"
              + " Error was: {}",
          jsonPayload,
  ...
  public void updateNewItemLinks(String parentId) {
    ...
    if (linkIds == null) {
      log.warn(
          "newLInkIds not initialized for current thread, no links will be updated for parent Id:"
              + " {}",
          parentId);
      return;
    }
    ...
    for (Long linkId : linkIds) {
      ...
      if (link != null) {
        try {
          ...
          log.error(
              "Unable to update manage link: {} for parent id: {}. Error: {}",
              link,
              parentId,
   ...
   */
  public void updateCopyAssetsLinks(
  ...
  public int getDependent(Element elem) {
    ...
    String path =
        "img".equalsIgnoreCase(elem.tagName()) ? elem.attr(SRC_ATTR) : elem.attr(HREF_ATTR);
  ...
  private void updateCopyAssetLinks(
      int assetId, String origSiteRoot, String copySiteRoot, Map<Integer, Integer> assetIdMap) {
  ...
  private void updateCopyAssetLink(
      PSManagedLink link,
      String origSiteRoot,
      String copySiteRoot,
      Map<Integer, Integer> assetIdMap)
    ...
    if (guid == null) {
      log.warn(
          "Found a page from original site ( {}{} ), but cannot find the copied page ( {} ). Skip"
              + " updating managed link: {}",
          origSiteRoot,
          relativePath,
          path,
          link);
  ...
  /**
   * Update the href attribute of the supplied link element with the correct rendering based on the
   * supplied context. If the link id of the element is not found, or doesn't resolve to a dependent
   * page or asset, the href resolves to "#".
   *
   * @param linkContext The context to use, may be <code>null</code>.
   * @param link The element to update, assumed not <code>null</code>.
   * @return <code>true</code> if the link was successfully rendered for the given context, <code>
   *     false</code> if not.
   */
  private boolean renderLink(PSRenderLinkContext linkContext, Element link, Boolean isStaging) {
    ...
    try {
      ...
      if (href != null) link.attr(HREF_ATTR, href);
      else isBrokenLink = true;
      if (isDeliveryContext) {
        removeManagedAttributes(link);
      ...
      log.error(
          "Unable to render managed link: {} Error: {}",
          link,
          PSExceptionUtils.getMessageForLog(e));
      if (isDeliveryContext) isBrokenLink = true;
    }
  ...
  /**
   * Given a link element, checks to see if the target is blank and if it is, updates the link to
   * include the
   *
   * @param link
   */
  ...
  /**
   * Update the src attribute of the supplied image link element with the correct rendering based on
   * the supplied context. If the link id of the element is not found, or doesn't resolve to a
   * dependent asset, the src resolves to "#".
   *
   * @param linkContext The context to use, may be <code>null</code>.
   * @param link The element to update, assumed not <code>null</code>.
   * @return <code>true</code> if the link was successfully rendered for the given context, <code>
   *     false</code> if not.
   */
  private boolean renderImageLink(
      PSRenderLinkContext linkContext, Element link, Boolean isStaging) {
    boolean result = true;
    boolean isDeliveryContext =
        (linkContext != null && linkContext.getMode().equals(PSRenderLinkContext.Mode.PUBLISH));
    ...
    try {
      ...
      try {
        ...
        log.warn(
            "Exception getting Asset Node for link: {}. The link is likely broken. Error: {}",
            link,
            PSExceptionUtils.getMessageForLog(e));
      }
      ...
      if (src != null) link.attr(SRC_ATTR, src);
      else {
        result = false;
      }
  ...
  private PSManagedLink createTempManagedLink(Element link, String path) throws Exception {
    ...
    if (path.startsWith("/Sites/")
      ...
      if (dependentId != -1) {
        log.debug("Fixed link for path {} to id {}", path, dependentId);
  ...
  /**
   * Get the corrected href for dependent item specified by the supplied link using the supplied
   * context.
   *
   * @param link
   * @param linkContext
  ...
  private String renderHref(
      PSManagedLink link, PSRenderLinkContext linkContext, Boolean isStaging) {
    String href = null;
    ...
    try {
      ...
      log.debug(
          "Unable to render href for managed link with ID {} Error: {}",
          link.getLinkId(),
          PSExceptionUtils.getMessageForLog(e));
    }
  ...
  private String createHref(
      PSManagedLink link, PSRenderLinkContext linkContext, Boolean isStaging, String href)
    ...
    if (linkItem != null) {
      ...
      if (linkContext.getMode().equals(Mode.PUBLISH) && linkItem instanceof PSRenderAsset) {
        ...
        try {
          ...
          log.error(
              "Failed to retrieve property for Asset: {}. Error: {}",
              myasset.getId(),
              PSExceptionUtils.getMessageForLog(e));
        } catch (RepositoryException e) {
          log.error(
              "Failed to find property in object's repository for Asset: {} Error: {}",
              myasset.getId(),
              PSExceptionUtils.getMessageForLog(e));
        } catch (UnsupportedEncodingException e) {
          log.error(
              "Failed to encode url: {}. Error: {}", href, PSExceptionUtils.getMessageForLog(e));
        }
      }
    }
   ...
   */
  private String appendAnalyticsId(String href, String analyticsId)
    ...
    if (URLValidator.isValid("http://localhost" + href + analyticsId)) {
      ...
      log.warn(
          "The link to asset: {} with encoded analytics id of {} failed to form a valid URL.",
          href,
          analyticsId);
    }
   ...
   */
  private String encodeAnalyticsId(String analyticsId) throws UnsupportedEncodingException {
    ...
    if (analyticsId.contains("=")) {
      ...
      analyticsId =
          "?"
              + URLEncoder.encode(analyticsIdHalves[0], "UTF-8")
              + "="
              + URLEncoder.encode(analyticsIdHalves[1], "UTF-8");
    } else {
  ...
  /**
   * Manages an image link if it is not already managed, unmanages the link if the link's target is
   * invalid. If there are any unexpected errors, they are logged and the method silently returns
   *
  ...
  private void manageImageLink(String parentId, Element link) {
    try {
      ...
      log.error("Unable to manage link: {} Error: {}", link, PSExceptionUtils.getMessageForLog(e));
    }
  }
  ...
  private void unmanageImageLink(Element link) {
    try {
      ...
      log.error(
          "Unable to unmanage link: {} Error: {}", link, PSExceptionUtils.getMessageForLog(e));
    }
  }

  /**
   * Manages a link if it is not already managed, unmanages the link if the link's target is
   * invalid. If there are any unexpected errors, they are logged and the method silently returns
   *
   * @param link The link element to manage, assumed not <code>null</code>.
   */
  ...
  /**
   * Ensure the dependent id referenced by this link exists, update the href if it does.
   *
   * @param parentId The ID of the parent
  ...
  private boolean validateLink(String parentId, long linkId, Element linkEl) throws Exception {
    ...
    if (link != null) {
      ...
      if (itemPath != null) {
        ...
        if (String.valueOf(UNASSIGNED_PARENT_ID).equals(parentId)) return false;
  ...
  /**
   * Ensure the dependent id referenced by this image link exists, update the src if it does.
   *
   * @param parentId The ID of the parent
   * @param linkId The link ID to validate
   ...
   * @return The path, never empty, <code>null</code> if the item is not found.
   */
    ...
    if (parentId.equals(String.valueOf(UNASSIGNED_PARENT_ID))) {
      if (newIds == null) {
        throw new IllegalStateException(
            "processing new item link, but newLinkIds not initialized on current thread");
      }
  ...
  private int getDependentId(String path) throws Exception {
    ...
    if (!(path.startsWith("/Sites/")
        || path.startsWith("/Assets/")
        || path.startsWith("//Sites/")
        || path.startsWith("//Assets/"))) return dependentId;
    String pathMod = path;
    String anchor = PSReplacementFilter.getAnchor(pathMod);
    if (anchor != null && !anchor.isEmpty()) pathMod = pathMod.replace(anchor, "");
    ...
    if (guid == null) {
      ...
      if (!importedPath.equals(folderPath)) guid = contentWs.getIdByPath(importedPath);
    }

    if (guid != null) {
      String id = guid.toString();
      if (workflowHelper.isPage(id) || workflowHelper.isAsset(id)) dependentId = guid.getUUID();
      else {
        String indexPath =
            pageCatalogService.convertToImportedFolderPath(concatPath(folderPath, "index.html"));
  ...
  public long getLinkId(Element link) {
    ...
    } else if (!link.attr(PERC_MANAGED_OLD_ATTR).equals("")
        || !link.attr(PERC_MANAGED_ATTR).equals("")) {
  ...
  /**
   * Check for delete events and if an asset, delete any relationships for which it is the parent
   */
  private void itemDeleted(String assetId) {
    ...
    for (PSManagedLink link : links) {
      try {
        ...
        log.error(
            "Unable to delete manage link: {} Error: {}",
            link,
            PSExceptionUtils.getMessageForLog(e));
      }
    }
  ...
  public void cleanupDeletedLinks(List<Element> validLinks, Integer parentId) {
    ...
    for (PSManagedLink currentLink : currentLinks) {
      ...
      for (Element validLink : validLinks) {
        if (validLink.attr(PERC_LINKID_ATTR).equals(Long.toString(currentLink.getLinkId()))) {
      ...
      if (!linkMatched) {
        log.debug("Adding link id {} to list of managed links to delete.", currentLink.getLinkId());
  ...
  public String manageItemPath(String ownerId, String path, String linkId) {
    try {
      // see if new item
      if (ownerId == null) ownerId = getNewItemParentId();

      // see if already managed
      long linkIdVal = NumberUtils.toLong(linkId, -1);
      if (linkIdVal != -1) {
        // ensure it's valid, remove linkid if not
        if (validateLink(ownerId, linkIdVal, path)) return linkId;
      }
      ...
      log.error("Unable to manage path: {} Error: {}", path, PSExceptionUtils.getMessageForLog(e));
    }
  ...
  public String renderItemPath(PSRenderLinkContext linkContext, String linkId, Boolean isStaging) {
    ...
    try {
      ...
      log.error(
          "Unable to render managed link for linkId: {} Error: {}",
          linkId,
          PSExceptionUtils.getMessageForLog(e));
    }
   ...
   *
   * @param linkContext The link context to use to determine the revision to use, assumed not <code>
   *     null</code>.
   * @param childId The id, assumed not <code>null</code>.
   * @return The link item for the current revision, <code>null</code> if no page or asset found for
   *     the supplied id.
   */
  private IPSLinkableItem getLinkItem(
      PSRenderLinkContext linkContext, int childId, Boolean isStaging)
    ...
    if (workflowHelper.isPage(id)) {
      ...
      try {
        ...
        log.error(
            "Unable to locate asset with id {} Error: {}",
            id,
            PSExceptionUtils.getMessageForLog(e));
      }
    }
  ...
  private IPSGuid getCorrectRevisionGuid(
      int id, PSRenderLinkContext linkContext, Boolean isStaging) {
    ...
    try {
      ...
      log.warn(
          "Unable to locate dependent item with id {}, possible broken link. Error: {}",
          id,
          PSExceptionUtils.getMessageForLog(e));
      ...
      try {
        ...
        log.error("Cannot cleanup orphan links. Error: {}", PSExceptionUtils.getMessageForLog(ex));
      }
