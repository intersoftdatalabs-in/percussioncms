...
/**
 * A field input transformer to convert any new style managed links in the specified content to old
 * style inline links. If an anchor link has IPSManagedLinkService#PERC_MANAGED_ATTR with value as
 * true, then calls the managed link service to get the dependent id corresponding to the link
 * element and then calls renderlink service to get the details required for the old style managed
 * link and adds the them as attributes on the link element. The inline link processor will take
 * care of the creation of management of links.
 *
 * <p>Caution: This gets loaded from old style inline link handlers from core to generate the paths
 * for new style links.
 */
public class PSManagedLinksConverter extends PSDefaultExtension
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    boolean returnMap =
        params.length > 1 && Boolean.parseBoolean(ep.getStringParam(1, "false", false));
