 ...
 *
 * <p>Sunny Sal says: "Link management so smooth, even your URLs will want to be managed!"
 *
 * @author JaySeletz
 */
public interface IPSManagedLinkService {
  ...
  /**
   * Attempts to cleanup any managed links referencing content ids that no longer exist. Fixes
   * database for old code that left these around.
   */
  void cleanupOrphanedLinks();

  /**
   * Update the href of any managed links found in the supplied source based on the supplied link
   * context.
   *
   ...
   * @param linkContext The link context to use, may be null to use edit path for the link.
   * @param source The source html to update, not null.
   * @param isStaging if true then staging filter is used otherwise public filter is used to filter
   *     the links
   * @param parentId The parent id for the links.
   * @return The updated html, never null.
   */
  ...
  /**
   * Update the path of any managed links found in the supplied JSON string based on the supplied
   * link context.
   *
   * @param linkContext The link context to use, may be null to use edit path for the link.
  ...
  /**
   * Manage links for a new item, where the parent id is not yet known. Once the item has been
   * persisted and the id is known, {@link #updateNewItemLinks(String)} must be called to update the
   * link with the correct id. Both calls must be made from the same thread. This call clears any
   * stored new link data for the current thread.
   *
   * @param source The source html to check for links, not null.
   * @return The updated html, never null.
   */
  ...
  /**
   * Updates the parent id for new managed links created by the previous call to {@link
   * #manageNewItemLinks(String)} on the same thread, clears the new link data for the thread. Noop
   * if no new link data is present for the thread.
   *
  ...
  /**
   * Updates the managed links contained in the specified assets. The assets were created during
   * copy a site.
   *
   ...
   * @param assetMap the shared asset IDs that are created during copy above (original) site, not
   *     null, may be empty.
   */
  void updateCopyAssetsLinks(
  ...
  /**
   * Finds the dependent and returns it. If the supplied element has perc-linkid attribute, loads
   * the managed link with that id and returns the dependent, if the load fails or link id doesn't
   * exist, finds the dependent based on the href attribute value. If not found then returns -1.
   *
  ...
  /**
   * Initialize the thread local storage of new item links. To be called before managing one or more
   * links for a new owner, after which {@link #updateNewItemLinks(String)} must be called to set
   * the newly generated owner id on the collected new item links.
   */
   ...
   *
   * @param ownerId The parent item content id, may be null or empty, in which case a new owner item
   *     is assumed. Note that {@link #initNewItemLinks()} should have been called once per owner
   *     item save prior to calling this method for new owner items.
   * @param path The path to the dependent item, if null or empty, no managed link is created.
   * @param linkId The current link Id, may be null or empty. If supplied, if no matching managed
   *     link is located, the item is re-managed and a new link id is returned.
  ...
  /**
   * Get the updated path to the dependent item identified by the supplied link id based on the
  ...
  /**
   * Checks a server property called {@link #SERVER_PROPERTY_AUTO_MANAGE_LOCAL_PATHS} is available
