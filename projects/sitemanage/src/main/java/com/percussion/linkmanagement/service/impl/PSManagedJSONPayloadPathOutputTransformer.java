...
/**
 * Field output transformer to update managed item paths in JSON payloads on edit.
            ...
            for (var i = 0; i < objectArray.length(); i++) {
                ...
                if (entry.has(IPSManagedLinkService.PERC_IMAGEPATH) && entry.has(IPSManagedLinkService.PERC_IMAGEPATH_LINKID)) {
                    if (!StringUtils.isBlank(entry.getString(IPSManagedLinkService.PERC_IMAGEPATH_LINKID))
                        ...
                        var newPath = renderItemPath(entry.getString(IPSManagedLinkService.PERC_IMAGEPATH_LINKID));
                ...
                if (entry.has(IPSManagedLinkService.PERC_FILEPATH) && entry.has(IPSManagedLinkService.PERC_FILEPATH_LINKID)) {
                    if (!StringUtils.isBlank(entry.getString(IPSManagedLinkService.PERC_FILEPATH_LINKID))
                            && (!StringUtils.isBlank(cid) || !StringUtils.isNumeric(cid))) {
                        var newPath = renderItemPath(entry.getString(IPSManagedLinkService.PERC_FILEPATH_LINKID));
                ...
                if (entry.has(IPSManagedLinkService.PERC_PAGEPATH) && entry.has(IPSManagedLinkService.PERC_PAGEPATH_LINKID)) {
                    if (!StringUtils.isBlank(entry.getString(IPSManagedLinkService.PERC_PAGEPATH_LINKID))
                            && (!StringUtils.isBlank(cid) || !StringUtils.isNumeric(cid))) {
                        var newPath = renderItemPath(entry.getString(IPSManagedLinkService.PERC_PAGEPATH_LINKID));
