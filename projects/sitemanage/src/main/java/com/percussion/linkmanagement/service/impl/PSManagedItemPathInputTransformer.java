 ...
 * the managedlink service to manage the link, the resulting link id is returned. If no link is
 * created, an empty string is returned. Also takes an optional link id as an second input, if
 * supplied, it is passed to the managed link service to validate. For new items the item id is not
 * yet created, so PSManagedItemPathPreProcessor should be added to initialized new item links in
 * the service to track that and set a request private object as a marker. The managed link post
 * processor exit checks the private object and if it is there and if its value is true then updates
 * the parent id.
 *
 * @author JaySeletz
 */
public class PSManagedItemPathInputTransformer extends PSDefaultExtension
