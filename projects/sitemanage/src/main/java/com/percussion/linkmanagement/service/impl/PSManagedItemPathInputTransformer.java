 ...
 * managed link service to manage the link, and returns the resulting link id. If no link is
 * created, returns an empty string. Optionally takes a link id as a second input for validation.
 * For new items, the item id is not yet created, so PSManagedItemPathPreProcessor should be used to
 * initialize new item links. The managed link post processor checks a request private object and
 * updates the parent id if needed.
 *
 * @author JaySeletz
 */
