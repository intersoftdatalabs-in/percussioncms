...
public abstract class PSPathItemService implements IPSPathService {
  ...
  public PSPathItem find(String path)
      throws PSPathServiceException, IPSDataService.DataServiceNotFoundException,
          PSValidationException, IPSDataService.DataServiceLoadException {
    log.debug("Find root of path: {}", path);
    if ("/".equals(path)) return findRoot();

    return findItem(path);
  }

  public PSItemProperties findItemProperties(String path)
      throws PSPathNotFoundServiceException, IPSDataService.DataServiceNotFoundException,
  ...
  public List<PSItemProperties> findItemProperties(PSItemByWfStateRequest request)
    ...
    // build the jcr query
    String jcrQuery =
        "select rx:sys_title, jcr:path from "
            + getQuerySelectFrom()
            + " where jcr:path like '"
  ...
  public PSNoContent moveItem(PSMoveFolderItem request)
    ...
    if (relativePath.charAt(relativePath.length() - 1) != '/') relativePath += "/";
  ...
  /**
   * Validates whether user has access to source item and target folder. If the user is designer or
   * admin or if the source is folder then skips the check. For the source item checks whether item
   * is modifiable by the user, for target folder check whether user has access level of more than
   * reader.
   *
  ...
  public PSPathItem renameFolder(PSRenameFolderItem item)
      throws PSValidationException, PSPathServiceException,
    ...
    try {
      ...
      errors.rejectValue(
          ...
          "Cannot rename folder '<old_name>' to '<new_name>' because that name is a reserved folder name");
      throw errors;
      ...
      if (iter.hasNext()) {
        ...
        if (msg.contains("This field cannot be empty and must be unique within the folder"))
          msg =
              "An asset or folder with the name you specified already exists. Specify a different name.";
      }
  ...
  protected abstract String getFullFolderPath(String path)
      throws IPSDataService.DataServiceNotFoundException, PSPathNotFoundServiceException,
          PSValidationException;
  ...
  private int deleteFolder(String path, List<String> inUseItems, PSDeleteFolderCriteria criteria)
      throws IPSDataService.DataServiceNotFoundException, PSPathServiceException,
          PSValidationException, IPSDataService.DataServiceLoadException {
