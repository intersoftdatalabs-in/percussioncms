...
public abstract class PSFileSystemPathItemService implements IPSPathService {
  ...
  /**
   * Given a folder path, it translates the path to an internal file system representation, gets the
   * children and returns a List of File objects.
   *
  ...
  protected PSPathItem findItem(String path) throws PSPathNotFoundServiceException {
    ...
    if ("/".equals(path)) parentPath = "/";
      ...
      else parentPath = "/" + FilenameUtils.getPath(path);
    }
    ...
    if (!item.getPath().endsWith("/")) item.setPath(item.getPath() + "/");
  ...
  private PSPathItem getPathItemFromFile(String parentPath, File child)
    ...
    item.setFolderPaths(
        Arrays.asList(FilenameUtils.getFullPathNoEndSeparator(item.getFolderPath())));
  ...
  /**
   * Given a File object, it returns the associated extension. It also checks if the File object
   * points to a folder, in that case it returns the correct icon.
   *
  ...
  private String getIconPath(String fileExtension, boolean isSys) {
    ...
    if (iconPath == null) return StringUtils.EMPTY;
  ...
  public int deleteFolder(PSDeleteFolderCriteria criteria) throws PSPathServiceException {
    try {
      ...
      throw new PSPathServiceException(
          "An error ocurred when deleting the folder '"
   ...
   *
   * @return A {@link PSPathItem} object representing the root element. Never <code>null</code>.
   */
