...
public class PSPathUtils {
  ...
  public static String stripSitesRoot(String path) {
    ...
    if (path.startsWith(SITE_ROOT)) return path.substring(SITE_ROOT.length());

    return path;
  }

  /**
   * Given an internal folder path, this will return the corresponding path seen in the finder.
   * Also, paths starting with '//' will be normalized to start with '/'.
   *
  ...
  /**
   * Indicates when the given sortColumn and sortOrder variables has valid sorting information. Sort
   * is specified if and only if both sortColumn and sortOrder are not blank.
   *
   ...
   * @return <code>true</code> if sorting is specified. <code>false</code> otherwise.
   */
  ...
  /**
   * Gets the page of {@link PSPathItem} objects which has a PSPathItem object with a name equals to
   * the specified child. maxResults is necessary here to correctly create the pages.
   *
   * @param allPathItems A List of PSPathItem objects. Cannot be <code>null</code>.
   * @param maxResults The maximum amount of results. Cannot be <code>null</code> and must be
   *     greater or equals to 1.
   * @param child The child name to look for, for example: page1. Cannot be <code>null</code> nor
   *     empty.
   * @return An Object array with two components:
   *     <ol>
   *       <li>If child is found in the given PSPathItem list, the sublist of objects (page) which
   *           contains the specified child is returned. If not found, then the first page is
   *           returned.. The start index value.
   *     </ol>
   */
  public static PSPagedItemList getPage(
      List<PSPathItem> allPathItems, Integer maxResults, String child) {
    ...
    PSPagedObjectList<PSPathItem> results =
        PSPagedObjectList.getPage(allPathItems, startIndex, maxResults);
  ...
  /**
   * Validates a path. It cannot be null nor empty, and it must start and end with a slash "/".
   *
   ...
   * 'Assets/uploads/www.erau.edu/import/assets/images/ignite-sm.png', supplied as parameter. This
   * path is converted into a system folder path, so system is able to perform the search. If
   * already exists, then returns a true value.
   *
   * @param destinationPath the finder path of the downloaded image. Including file name. Cannot be
   *     <code>null</code>.
   * @return A boolean value to indicate if the item exists in the system, <code>true</code> if item
   *     exists.
   */
  ...
  /**
   * Checks if the given list has a {@link PSPathItem} with its name equals to 'child'.
   *
   * @param child The name of the PSPathItem that is being searched. Cannot be <code>null</code>.
   * @param list The list where the search should be done. Cannot be <code>null</code>, maybe empty.
   * @return 'true' if any PSPathItem with its name equals to 'child' is found. 'false' otherwise.
   */
  private static Integer getChildIndex(String child, List<PSPathItem> list) {
    ...
    for (index = 0; index < list.size(); index++) {
      ...
      if (StringUtils.equalsIgnoreCase(item.getName(), child)) break;
    }
   ...
   * @param sortColumn The sort column name to sort by. For example: sys_title. According to its
   *     value and {@link #isSortSpecified(String, String)}, sorting may not be performed.
   * @param sortOrder The sort order ("asc" or "desc"). According to its value and {@link
   *     #isSortSpecified(String, String)}, sorting may not be performed.
   */
  public static void sort(
      List<PSPathItem> pathItemList, final String sortColumn, String sortOrder) {
    Validate.notNull(pathItemList, "searchResults cannot be null");

    if (!PSPathUtils.isSortSpecified(sortColumn, sortOrder)) return;
    ...
    Collections.sort(
        ...
        new Comparator<PSPathItem>() {
          public int compare(PSPathItem o1, PSPathItem o2) {
            ...
            //                String o1type =
            // o1.getDisplayProperties().get(PSUiHelper.CONTENTTYPE_NAME);
            //                String o2type =
            // o2.getDisplayProperties().get(PSUiHelper.CONTENTTYPE_NAME);
            //
            ...
            int compareResult = 1;
            try {
              compareResult = new CompareToBuilder().append(prop1, prop2).toComparison();
            } catch (Exception e) {
          ...
          private Object getRealDataType(String propValue) {
            ...
            if (propValue != null) return propValue.toLowerCase();
  ...
  /**
   * * Given a path, will correct the path so that it is pointed at the Site Folder instead of the
   * Site Name
   *
   * @return
   */
  ...
  /**
   * Assumes the folder path is in the format "//Sites/sitename[/otherpath]" or
   * "/Sites/sitename[/otherpath]", or "Sites/sitename[/otherpath]" if it is then returns the
   * sitename otherwise returns null.
   *
  ...
  public static String getSiteFromPath(String folderPath) {
    ...
    folderPath =
        folderPath.startsWith("//Sites/")
            ? folderPath.replace("//Sites/", "")
            : (folderPath.startsWith("/Sites/")
                ? folderPath.replace("/Sites/", "")
  ...
  private static int getSortOrderNumber(String sortOrder) {
    if (StringUtils.equalsIgnoreCase(sortOrder, "desc")) return -1;
