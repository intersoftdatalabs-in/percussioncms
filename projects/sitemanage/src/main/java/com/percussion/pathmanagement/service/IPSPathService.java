...
/**
 * Retrieves {@link PSPathItem}s and their children for given paths. Used for UI tree controls like
 * a file browser.
 *
 * <p>The {@link PSPathItem}s returned by the implementation must have the property {@link
 * PSPathItem#getPath()} never {@code null}, empty, or blank.
 *
 * @author adamgent
 */
public interface IPSPathService {

  String FOLDER_TYPE_FILESYSTEM = "FSFOLDER";
   ...
   *
   * @return a list of user roles allowed to access the service, or {@code null} if all roles are
   *     allowed
   */
   ...
   *
   * @param path never {@code null}, empty, or blank
   ...
   * @throws PSPathNotFoundServiceException if the path is not found
   * @throws PSPathServiceException if a system error occurs
   */
   ...
   *
   * @param request contains both the target folder and the item to be moved
   ...
   *
   * @param criteria never {@code null}
   * @return the number of items that cannot be deleted
   ...
   * @return last existing path, never {@code null} or empty. This is a relative path (no
  ...
  class PSPathServiceException extends Exception {
  ...
  class PSPathNotFoundServiceException extends PSPathServiceException
  ...
  class PSReservedNameServiceException extends PSPathServiceException {
