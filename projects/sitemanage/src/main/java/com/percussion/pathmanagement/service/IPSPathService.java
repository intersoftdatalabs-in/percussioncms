...
/**
 * Retrieves {@link PSPathItem}s and their children for given paths.
 * Used for UI tree controls like a file browser.
 * <p>
 * The {@link PSPathItem}s returned by the implementation must
 * have the property {@link PSPathItem#getPath()} never {@code null}, empty, or blank.
 *
 * @author adamgent
 */
public interface IPSPathService {
     ...
     *
     * @return a list of user roles allowed to access the service, or {@code null} if all roles are allowed
     */
     ...
     * Should only work for paths with {@link PSPathItem#isLeaf()} set to {@code false}.
     *
     * @param path never {@code null}, empty, or blank
     ...
     * The folder will be named 'New Folder.' Additional new folders will be named 'New Folder (n)' with n starting at 2.
     * The first name 'New Folder (n)' will be used with the first available n value >= 2.
     *
     * @param path never {@code null}, empty, or blank
     * @return never {@code null}
     * @throws PSPathNotFoundServiceException if the created folder item could not be found
     ...
     * @return last existing path, never {@code null} or empty.
     *         This is a relative path (no leading/trailing forward slashes) or empty for root paths ("/Assets" or "/Sites").
     */
     ...
     */
    class PSPathNotFoundServiceException extends PSPathServiceException implements IPSNotFoundException {
