...
/**
 * Retrieves {@link PSPathItem}s and {@link PSPathItem} children for given paths. Used for UI tree
 * controls like a file browser.
 *
 * <p>The {@link PSPathItem}s returned by the implementation must have the property {@link
 * PSPathItem#getPath()} never <code>null</code>, empty, or blank.
 *
 * @author adamgent
 */
public interface IPSPathService {
  ...
  /**
   * Which user roles are allowed to reach this service. If the service is not allowed for a
   * specific role, then it's not shown in the UI nor reachable.
   *
   * @return A list of user roles allowed to access the service. May be <code>null</code>, in that
   *     case all user roles are allowed.
   */
   ...
   * @throws PSPathNotFoundServiceException If an item cannot be found for the given path or the
   *     path is invalid.
   * @throws PSPathServiceException thrown when a path item cannot be created or other system
   *     failure.
   */
  PSPathItem find(String path)
      throws PSPathNotFoundServiceException, PSPathServiceException, PSDataServiceException;

  /**
   * Finds the children of a given path. Should only work for paths with {@link PSPathItem#isLeaf()}
   ...
   */
  List<PSPathItem> findChildren(String path)
      throws PSPathNotFoundServiceException, PSPathServiceException, PSDataServiceException;
   ...
   *
   * @param request the request to find the item properties by workflow state, never <code>null
   *     </code>.
   ...
   * @throws PSPathServiceException If the item properties or workflow could not be found, or other
   *     system failure.
   */
  List<PSItemProperties> findItemProperties(PSItemByWfStateRequest request)
   ...
   */
  PSPathItem addFolder(String path)
      throws PSPathNotFoundServiceException, PSPathServiceException,
          IPSDataService.DataServiceNotFoundException, PSValidationException,
          IPSDataService.DataServiceLoadException;

  /**
   * Adds a new folder {@link PSPathItem} as a child of the given folder path or as a sibling of the
   * given item path. The folder will be named 'New Folder.' Additional new folders will be named
   * 'New Folder (n)' with n starting at 2. The first name 'New Folder (n)' will be used with the
   * first available n value >= 2.
   *
   * @param path never <code>null</code>, empty, or blank.
   * @return never <code>null</code>.
   * @throws PSPathNotFoundServiceException If the created folder item could not be found.
   * @throws PSPathServiceException If the folder could not be created or other system failure.
   */
  PSPathItem addNewFolder(String path)
      throws PSPathNotFoundServiceException, PSPathServiceException, PSDataServiceException;
   ...
   */
  PSPathItem renameFolder(PSRenameFolderItem item)
   ...
   *
   * @param request it contains both the target folder and the to be moved item.
   * @return no content object where operation = "moveItem" if the item is successfully moved.
   */
   ...
   */
  int deleteFolder(PSDeleteFolderCriteria criteria)
   ...
   * @return string response indicating the reason why all items cannot be deleted or success. Never
   *     blank.
   ...
   */
  String validateFolderDelete(String path)
      throws PSPathNotFoundServiceException, PSPathServiceException,
          IPSDataService.DataServiceNotFoundException, PSValidationException,
          IPSItemWorkflowService.PSItemWorkflowServiceException,
  ...
  /**
   * Checks for the existence of a path and finds the last portion of the path which exists. Never
   * blank.
   *
   * @param path the folder or item path, never blank.
   * @return last existing path, never <code>null</code> or empty. This is a relative path (no
   *     leading/trailing forward slashes) or empty for root paths ("/Assets" or "/Sites").
   */
   ...
   * @author adamgent
   */
  public static class PSPathNotFoundServiceException extends PSPathServiceException
