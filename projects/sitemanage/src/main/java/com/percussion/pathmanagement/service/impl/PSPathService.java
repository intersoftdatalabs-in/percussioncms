...
public class PSPathService extends PSDispatchingPathService
  ...
  public PSPathItem findById(@PathParam("id") String id)
      throws PSPathServiceException, PSParametersValidationException {
    try {
      String guid = PSLegacyExtensionUtils.getGUID(id);
      if (!SecureStringUtils.isValidGuidId(guid)) throw new PSPathServiceException("Invalid id.");
   ...
   * 'startIndex' and 'maxResults' are used to ask for a particular page of data. 'child' and
   * 'maxResults' are used to return the page of data where 'child' is contained.
   *
   * <p>According to the given parameters, different functions are called, in this order:
   *
   * <ul>
   *   <li>If 'child' and 'maxResults' are not null, then those two parameters are used to look for
   *       a page containing the 'child' entry.
   *   <li>If 'startIndex' is not null, then it's used to look for the page starting with that
   *       entry. If 'maxResults' was specified, then the page will have, at maximum, 'maxResults'
   *       entries. If it wasn't, all entries starting from 'startIndex' are returned.
   *   <li>In this case, no paging is done. All children for the specified path are returned.
   * </ul>
   *
   * @param path The path to look for its children.
   * @param startIndex The starting index. It can be <code>null</code>, in that case it won't be
   *     used.
   * @param maxResults The maximum amount of results. It can be <code>null</code>, in that case it
   *     won't be used.
   * @param sortColumn The column name to sort the path items by. For example: sys_title,
   *     sys_contentcreatedby. See {@link PSCm1ListViewHelper} for all supported column names. May
   *     be empty <code>null</code> or empty, in that case no sorting is done (both sortColumn and
   *     sortOrder are necessary to carry out any sorting).
   * @param sortOrder The order to sort the path items by. It accepts two possible values: 'asc'
   *     (ascendant) and 'desc' (descendant); case is ignored. May be empty <code>null</code> or
   *     empty, in that case no sorting is done (both sortColumn and sortOrder are necessary to
   *     carry out any sorting).
   * @param child The child name to look for, for example: page1. It can be <code>null</code>, in
   *     that case it won't be used.
   * @param displayFormatId The display format id to use to format the results, may be <code>null
   *     </code>.
   * @param category Optional category to filter the resulting items by, the string value of one or
   *     more of the {@link IPSItemSummary.Category} values, may be <code>null</code>,
   *     comma-separated list if multiple are supplied. Only used if startIndex is supplied.
   * @param type Optional type to filter the resulting items by, one or more of the possible values
   *     returned by {@link IPSItemSummary#getType()}, may be <code>null</code>, comma-separated
   *     list if multiple are supplied. Only used if startIndex is supplied.
   ...
   *     {@link PSPathItem#hasFolderChildren()}, {@link PSPathItem#hasItemChildren()}, and {@link
   *     PSPathItem#hasSectionChildren()} - they will all return <code>false</code> irrespective of
   *     actual data.
  ...
  public PSPagedItemList findChildren(
      ...
      @QueryParam("mustExist") boolean mustExist)
      throws PSPathNotFoundServiceException, PSPathServiceException {
    ...
    try {
      ...
      if (child != null && maxResults != null)
        ...
        List<PSPathItem> allItems =
            super.findChildren(path, displayFormatId, sortColumn, sortOrder);
        extendedPathItem = new PSPagedItemList(allItems, allItems.size(), 1);
      }

      extendedPathItem.setChildrenInPage(
          folderHelper.setFolderAccessLevel(extendedPathItem.getChildrenInPage()));
  ...
  public PSPathItem renameFolder(PSRenameFolderItem item)
      throws PSPathNotFoundServiceException, PSPathServiceException, PSBeanValidationException {

    try {
      log.debug("Attempting to rename folder: {} to: {}", item.getPath(), item.getName());
      PSSiteCopyUtils.throwCopySiteMessageIfNotAllowed(
          getSiteNameFromFolderPath(item.getPath()),
          "renameFolder",
  ...
  public PSNoContent moveItem(PSMoveFolderItem request) {
    try {
      PSSiteCopyUtils.throwCopySiteMessageIfNotAllowed(
          getSiteNameFromFolderPath(request.getItemPath()),
          "moveItem",
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML, MediaType.TEXT_PLAIN})
  @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public String deleteFolderService(PSDeleteFolderCriteria criteria)
      throws PSValidationException, PSPathServiceException, PSDataServiceException,
          PSNotFoundException {
    ...
    if (item != null
        && item.getType() != null
        && !item.getType().equalsIgnoreCase(IPSPathService.FOLDER_TYPE_FILESYSTEM)) {
      int iGuid = 0;
  ...
  public PSNoContent restoreFolder(@PathParam("id") String id) throws PSPathServiceException {
    ...
    if (!SecureStringUtils.isValidGuidId(id)) throw new PSPathServiceException("Invalid id.");
  ...
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML, MediaType.TEXT_PLAIN})
  public String validateFolderDelete(@PathParam("path") String path)
      throws PSValidationException, PSPathNotFoundServiceException, PSPathServiceException {
    try {
    ...
    } catch (IPSDataService.DataServiceNotFoundException
  ...
  public List<PSItemProperties> findItemProperties(PSItemByWfStateRequest request)
    ...
    try {
      log.debug(
          "Attempting to find item properties for: "
              + request.getPath()
              + ", "
              + request.getWorkflow()
              + ", "
  ...
  /**
   * The "path"s of the request are assumed from the "path" property of the {@link
   * com.percussion.pathmanagement.data.PSPathItem} object. The path looks like:
   * "/site/<site-name>/..." for a site item/folder "/Assets/<folder-name>/... for an item/folder
   * under assets However, the path-service is expecting path without leading "/"
   * "site/<site-name>/..." for a site item/folder "Assets/<folder-name>/... for an item/folder
   * under assets
   *
   * @param request
  ...
  private void stripLeadingSlashForPaths(PSMoveFolderItem request) {
    ...
    if (path.length() > 0 && path.charAt(0) == '/') request.setTargetFolderPath(path.substring(1));

    path = request.getItemPath();
    if (path.length() > 0 && path.charAt(0) == '/') request.setItemPath(path.substring(1));
  }
