...
public interface IPSPubServerService {
  ...
  /**
   * Load the server information based on the site name and server name as the parameters.
   *
   * @param siteId
   * @param serverId
   * @return a <code>PSPublishServerInfo</code> object maybe empty never <code>null</code>
  ...
  /**
   * Retrieves a list of all publish server existing including their information.
   *
   * @return a list of <code>PSPublishServerInfo</code>, never empty or <code>null</code>
   ...
   * @param pubServerInfo the <code>PSPubServer</code> object containing the server information to
   *     create that server. Must not be empty or <code>null</code>
   * @return a <code>PSPublishServerInfo</code> object maybe empty never <code>null</code>
   ...
   * @param pubServerInfo the <code>PSPubServer</code> object containing the server information to
   *     update that publish server. Must not be empty or <code>null</code>
   * @return a <code>PSPublishServerInfo</code> object never empty or <code>null</code>
   * @throws PSPubServerServiceException, if the supplied object is invalid.
   */
   ...
   *
   * @return a <code>Map<String, Boolean></code> object never empty or <code>null</code> at least we
   *     always will get drivers for ORACLE and MSSQL.
   */
  Map<String, Boolean> getAvailableDrivers();

  /**
   * Determine if the default publish server that belongs to a site is modified or not.
   *
   ...
   *
   * @param siteId the name of the site
   * @return a <code>PSPubServer</code> object never empty or <code>null</code>
   */
   ...
   * @return a <code>PSPubServer</code> object may be <code>null</code> if a staging server has not
   *     been created.
   */
   ...
   * @return the {@link PSPubServer publish server} object maybe <code>null</code> if it cannot be
   *     created
   * @throws PSPubServerServiceException
   */
   ...
   * @return <code>true</code> if the folder location was changed for any pub server, <code>false
   *     </code> if not
   */
