...
public class PSPubServerService implements IPSPubServerService {
  ...
  private Map<String, Object> generatePubServerHandlerMap() throws PSNotFoundException {
    ...
    handlerMap.put("ftp", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("ftp_only");
    handlerMap.put("ftp_only", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("ftps");
    handlerMap.put("ftps", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("ftps_only");
    handlerMap.put("ftps_only", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("sftp");
    handlerMap.put("sftp", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("sftp_only");
    handlerMap.put("sftp_only", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("filesystem");
    handlerMap.put("filesystem", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("filesystem_only");
    handlerMap.put("filesystem_only", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("database");
    handlerMap.put("database", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("amazon_s3");
    handlerMap.put("amazon_s3", PSBaseServiceLocator.getBean(tmp.getBeanName()));

    tmp = pubsvc.loadDeliveryType("amazon_s3_only");
    handlerMap.put("amazon_s3_only", PSBaseServiceLocator.getBean(tmp.getBeanName()));
  ...
  public List<PSPublishServerInfo> deleteServer(String siteId, String serverId)
    ...
    try {
      ...
      if (pubServer == null) {
        throw new PSPubServerServiceException(
            "The Server you have selected doesn't exist in the system. Please refresh and try again.");
      }

      if (site == null) {
        throw new PSPubServerServiceException(
            "The Site you have selected doesn't exist in the system. Please refresh and try again.");
      }
  ...
  private void setProperties(PSPubServer server, PSPublishServerInfo pubServerInfo, IPSSite site)
    ...
    PSPubServerProperty oldPasswordProperty =
        server.getProperty(IPSPubServerDao.PUBLISH_PASSWORD_PROPERTY);
    ...
    if (site.isPageBased()) {
      serverInfo.setCanIncrementalPublish(
          serverInfo.getIsDefault()
              || PSPubServer.STAGING.equalsIgnoreCase(pubServer.getServerType()));
  ...
  private void setPasswordFlags(IPSPubServer pubServer, PSPublishServerInfo serverInfo) {
    ...
    PSPubServerProperty privateKeyProperty =
        pubServer.getProperty(IPSPubServerDao.PUBLISH_PRIVATE_KEY_PROPERTY);
    String privatekeyValue = EMPTY;
    if (privateKeyProperty != null) {
      privatekeyValue =
          pubServer.getProperty(IPSPubServerDao.PUBLISH_PRIVATE_KEY_PROPERTY).getValue();
    }
  ...
  private void setFolderFlags(
    ...
    Boolean isOwnServer =
        Boolean.parseBoolean(
                pubServer.getProperty(IPSPubServerDao.PUBLISH_OWN_SERVER_PROPERTY).getValue())
            ? true
            : false;
  ...
  private void convertPasswordFromBase64(PSPublishServerInfo pubServerInfo) {
    ...
    if (encodedPassword != null) {
      ...
      try {
        ...
        log.warn(
            "Unable to decode stored Publishing Server password, proceeding as if it is clear text.  Try editing the Publishing server to clear this Warning.");
  ...
  public PSPubInfo getS3PubInfo(IPSGuid siteId)
    ...
    PSPubServerProperty buckProp =
        pubServer.getProperty(IPSPubServerDao.PUBLISH_AS3_BUCKET_PROPERTY);
    ...
    PSPubServerProperty accessKeyProp =
        pubServer.getProperty(IPSPubServerDao.PUBLISH_AS3_ACCESSKEY_PROPERTY);
    ...
    PSPubServerProperty securityKeyProp =
        pubServer.getProperty(IPSPubServerDao.PUBLISH_AS3_SECURITYKEY_PROPERTY);
    ...
    PSPubServerProperty useAssumeRoleProp =
        pubServer.getProperty(IPSPubServerDao.PUBLISH_AS3_USE_ASSUME_ROLE);
