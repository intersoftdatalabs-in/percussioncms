...
public class FolderAdaptor implements IFolderAdaptor {
  ...
  @Autowired private IPSSiteSectionService sectionService;
  ...
  @Autowired private IPSManagedNavService navSrv;

  @Autowired private IPSPageDaoHelper pageDaoHelper;

  @Autowired private IPSPageDao pageDao;

  @Autowired private IPSUserService userService;

  @Autowired private IPSContentWs contentService;

  @Autowired private IPSRedirectService redirectService;

  @Autowired private IPSSiteDataService siteDataService;

  @Autowired private IPSRecentService recentService;
  ...
  public Folder getFolder(URI baseUri, String site, String path, String folderName)
    ...
    try {
      ...
      log.warn("Folder {} not found. Error: {}", folderName, PSExceptionUtils.getMessageForLog(e));
      throw (e);
  ...
  private Folder getFolder(
      URI baseUri, String folderGuid, String site, String path, String folderName)
    ...
    if (StringUtils.isNotEmpty(folderGuid) && site == null && path == null && folderName == null) {
      ...
      try {
        ...
        String extLinkPath =
            generateFullSitePath(site, path, EXTERNAL_SECTION_NAME_PREFIX + folderName);
  ...
  private void extractSubPagesFolders(
      URI baseUri, PSPathItem pathItem, String pathUtilsPath, Folder folder)
      ...
      for (PSPathItem child : children) {
        if (child.isFolder()) {
          ...
          if (!existingFolders.contains(name)) {
            ...
            URI folderUri =
                Folder.getFolderURI(baseUri, parts.getSite(), parts.getPath(), parts.getName());
          ...
          URI assetUri =
              Page.getPageUri(baseUri, parts.getSite(), parts.getPath(), parts.getName());
  ...
  private void extractSectionInfo(
      URI baseUri, String fullPath, Folder folder, PSSectionTypeEnum sectionType)
      ...
      if (sectionType == PSSectionTypeEnum.section) {
        ...
        if (!childSections.isEmpty()) {
          ...
          for (PSSiteSection subSection : childSections) {
            ...
            sectionLink.setName(
                StringUtils.remove(
                    StringUtils.substringAfterLast(subSection.getFolderPath(), "/"),
            ...
            sectionLink.setHref(
                StringUtils.remove(folderUri.toASCIIString(), EXTERNAL_SECTION_NAME_PREFIX));
          }
        }
  ...
  private Folder modifyExistingFolder(URI baseUri, Folder existingFolder, Folder folder)
    ...
    if (existingFolder.getSectionInfo() != null
        && existingFolder.getSectionInfo().getType() != null
        && existingFolder
            .getSectionInfo()
            .getType()
            .equalsIgnoreCase(PSSectionTypeEnum.externallink.name())) {
    ...
    if (pathUtilsPath == null && CollectionUtils.isNotEmpty(pathItem.getFolderPaths())) {
      ...
      if (pathUtilsPath != null && !pathUtilsPath.startsWith("//")) fullPath = "/" + pathUtilsPath;
      else fullPath = pathUtilsPath;
    }
  ...
  private void updateRecentUsers(String folderPath, List<String> userNames) {
    ...
    for (String u : userNames) {
      try {
        ...
        log.warn("Error adding " + "/Sites/{} to the Recent list for user {}", folderPath, u);
      }
    }
  }
  ...
  private void updateLandingPage(Folder folder, Folder existingFolder, String folderPath)
    ...
    if (reqSectionInfo != null
        && currSectionInfo != null
        && reqSectionInfo.getLandingPage() != null
      ...
      if (!reqName.equals(existingName)) {
        ...
        if (foundPage) {
          ...
          PSPathItem pathItem =
              pathService.find(StringUtils.substring(folderPath + "/" + reqName, 1));
          navSrv.addLandingPageToNavnode(
              idMapper.getGuid(pathItem.getId()), sectionId, asmBridge.getDispatchTemplate());
  ...
  private void convertFolderToSection(
      String folderPath, String folderName, String siteName, SectionInfo sectionInfo)
  ...
  private String getTemplateIdForName(String templateName, String siteName)
      throws BackendException, IPSDataService.DataServiceLoadException, PSValidationException {
  ...
  private void reorderSiteSection(URI baseUri, String id, Folder folder, Folder existingFolder)
    ...
    if (folder.getSubsections() != null) {
      ...
      for (SectionLinkRef subsection : folder.getSubsections()) {
        ...
        else nameCheck = subsection.getName();
  ...
  private String createNewFolder(URI baseUri, Folder folder)
    ...
    // Create a regular folder
    {
      ...
      newFolder = pathService.addFolder(fullUrl.getUrl());
    }
    ...
    // create subsections
    if (!folder.getSiteName().equals("Assets")) createSubSections(baseUri, folder, folder);
  ...
  private void updateFolderProperties(Folder folder, String folderId, Folder existingFolder)
    ...
    if (changedProperties) folderHelper.saveFolderProperties(folderProperties);
  }
  ...
  private void createSubSections(URI baseUri, Folder existingFolder, Folder folder)
    ...
    if (folder.getSubsections() != null) {
      ...
      for (SectionLinkRef subSection : folder.getSubsections()) {
        ...
        createSubSection(
            baseUri, existingFolder, folder, name, subSection.getType(), subSection.getHref());
      }
    }
  }

  private void createSubSection(
      URI baseUri, Folder existingFolder, Folder folder, String name, String type, String extUrl)
      throws PSDataServiceException, BackendException, IPSPathService.PSPathServiceException {
    notNull(folder);
    ...
    if (!type.equalsIgnoreCase(PSSectionTypeEnum.sectionlink.name())) {
      ...
      if (folder.getWorkflow() != null) subFolder.setWorkflow(folder.getWorkflow());
      else subFolder.setWorkflow(existingFolder.getWorkflow());
  ...
  private PSPathItem createNewSection(Folder folder, UrlParts parentUrl)
    ...
    if (section.getType() != null
        && section.getType().equalsIgnoreCase(PSSectionTypeEnum.externallink.name())) {
      PSCreateExternalLinkSection req = new PSCreateExternalLinkSection();
      ...
      newSection = sectionService.createExternalLinkSection(req);
        ...
        || section.getType().equalsIgnoreCase(PSSectionTypeEnum.section.name())) {
  ...
  private boolean updatePermissions(
      Folder folder, PSFolderProperties folderProperties, Folder existingFolder) {
    ...
    if (editUsers != null) {
      ...
      if (writePrincipals == null) writePrincipals = new ArrayList<>();

      if (writePrincipals.size() > 0) {
        for (Principal principal : writePrincipals) {
          if (principal.getType().equals(PrincipalType.USER)) serverUsers.add(principal.getName());
        }
      }
      if (!CollectionUtils.isEqualCollection(editUsers, serverUsers)) {
        ...
        // Clear any Read principals as CM1 doesn't support them in the UI, remove this if that
  ...
  protected String generateFullSitePath(String siteName, String path, String name) {
    ...
    if (StringUtils.isNotEmpty(path)) sb.append("/").append(path);
    if (StringUtils.isNotEmpty(name)) sb.append("/").append(name);
    return sb.toString();
  }
