 ...
 * execute via {@link IPSPipelineRuntimeService}, Admin start/stop via {@link PSServer}, and Admin
 * validation/problems via {@link CollectingApplicationValidator}.
 *
 ...
 * Validation peers {@code PSValidatorAdapter#validateApplication}.
 */
@PSSiteManageBean
public class PipelinesAdaptor implements IPipelinesAdaptor {
  ...
  PipelinesAdaptor(
    ...
    this(summaryLoader, runtimeSupplier, null, null, null, null);
  }
  ...
  PipelinesAdaptor(
    ...
    this(summaryLoader, runtimeSupplier, adminChecker, lifecycleOps, detailLoader, null);
  }

  /**
   * Package-visible for unit tests covering Admin validation with injected collaborators (no object
   * store / validator singletons).
   */
  PipelinesAdaptor(
      Function<PSSecurityToken, PSApplicationSummary[]> summaryLoader,
      Supplier<IPSPipelineRuntimeService> runtimeSupplier,
  ...
  static List<String> defaultDesignGaps() {
    ...
    gaps.add("Pipe graph edit / IR write not supported via this API");
    return gaps;
  }
