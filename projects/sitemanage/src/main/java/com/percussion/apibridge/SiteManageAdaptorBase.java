...
public class SiteManageAdaptorBase {
  ...
  @Autowired
  public SiteManageAdaptorBase(
      IPSUserService userService, IPSItemWorkflowService itemWorkflowService) {
  ...
  protected String setWorkflowState(
      String itemId, String requestedWorkflowState, List<String> stateChanges) {
    ...
    if (stateChanges.contains(currentState)) {
      throw new RuntimeException(
          "Loop detected trying to get item into state "
              + testWorkflowState
              + ". Tried changing through following states "
              + stateChanges);
    }
    ...
    if (!testWorkflowState.equals(testCurrentState)) {
      ...
      if (requiredTrigger == null) {
        throw new RuntimeException(
            "Cannot find trigger to get to "
                + testWorkflowState
                + " tried "
                + Arrays.toString(testTriggers)
  ...
  protected static final ArrayList<String> APPROVED_STATES =
      new ArrayList<>(
          Arrays.asList(new String[] {DefaultWorkflowStates.live, DefaultWorkflowStates.pending}));
}
