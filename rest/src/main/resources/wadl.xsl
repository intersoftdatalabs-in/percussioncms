...
<xsl:stylesheet
	...
	<xsl:template name="getFullResourcePath">
		...
		<xsl:choose>
			<xsl:when
				test="substring($base, string-length($base)) = '/'">
	...
	<xsl:template name="getParamBlock">
		...
		<xsl:if
			test="ancestor-or-self::wadl:*/wadl:param[@style=$style]">
			<div class="h7">
		...
		<xsl:if test="wadl:representation">
			...
			<table>
				<xsl:for-each select="wadl:representation">
					...
					<xsl:call-template
						name="getRepresentationParamBlock">
