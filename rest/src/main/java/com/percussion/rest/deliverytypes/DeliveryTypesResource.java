...
public class DeliveryTypesResource {
  ...
  @Autowired private IDeliveryTypeAdaptor deliveryTypeAdaptor;
  ...
  @Operation(
      summary = "Get a DeliveryType by id. ID is expected to be in Guid untyped String form",
      responses = {
  ...
  public DeliveryType getDeliveryTypeById(@PathParam("id") String id) {
    try {
      return deliveryTypeAdaptor.getDeliveryTypeById(uriInfo.getBaseUri(), id);
    } catch (Exception e) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "404",
            description = "DeliveryType not found",
  ...
  public Status deleteDeliveryType(
      @Parameter(description = "The untyped Guid string of the DeliveryType to delete", name = "id")
          @PathParam("id")
          String id) {
    try {
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Operation(
      summary = "Create or update a delivery type",
      responses = {
        @ApiResponse(responseCode = "404", description = "No DeliveryType found"),
        @ApiResponse(responseCode = "500", description = "Error searching for DeliveryType"),
        @ApiResponse(
  ...
  public DeliveryType updateDeliveryType(
      @Parameter(description = "The body containing a JSON payload", name = "body")
          DeliveryType deliveryType) {
    try {
      return deliveryTypeAdaptor.updateDeliveryType(uriInfo.getBaseUri(), deliveryType);
    } catch (Exception e) {
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @Operation(
      summary = "List all Delivery Types available on the system.",
      responses = {
                        ...
                        @ExampleObject(
                            value =
                                "{\n"
                                    ...
                                    + "      \"description\": \"Publish content to the"
                                    + " filesystem\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 1,\n"
                                    ...
                                    + " doesn't include other delivery handlers like metadata"
                                    + " indexer.\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 5,\n"
                                    ...
                                    + "    {\n"
                                    + "      \"beanName\": \"sys_ftp\",\n"
                                    + "      \"description\": \"Publish content using ftp doesn't"
                                    + " include other delivery handlers like metadata indexer.\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 6,\n"
                                    + "        \"stringValue\": \"0-112-6\",\n"
                                    ...
                                    + "    {\n"
                                    + "      \"beanName\": \"sys_sftp\",\n"
                                    + "      \"description\": \"Publish content using sftp doesn't"
                                    + " include other delivery handlers like metadata indexer.\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 7,\n"
                                    ...
                                    + "      \"description\": \"Publish content to amazon s3"
                                    + " bucket\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 8,\n"
                                    ...
                                    + "      \"description\": \"Publish content amazon s3 bucket"
                                    + " does not include other delivery handlers like metadata"
                                    + " indexer.\",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
                                    + "        \"longValue\": 9,\n"
                                    ...
                                    + "    {\n"
                                    + "      \"beanName\": \"sys_ftps\",\n"
                                    + "      \"description\": \"Publish content using ftps doesn't"
                                    + " include other delivery handlers like metadata\\n"
                                    + "                indexer.\\n"
                                    + "            \",\n"
                                    + "      \"id\": {\n"
                                    + "        \"hostId\": 0,\n"
  ...
  public List<DeliveryType> getDeliveryTypes() {
    try {
      var ret = deliveryTypeAdaptor.getDeliveryTypes(uriInfo.getBaseUri());
      if (ret.isEmpty()) {
