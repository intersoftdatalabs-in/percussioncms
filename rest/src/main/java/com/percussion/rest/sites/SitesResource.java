  ...
  @Operation(
      ...
      description =
          "Persists virtual.* properties. Validation aligns with PSVirtualSiteHelper:"
              ...
              + " openapi-yaml, asyncapi-yaml, graphql-sdl, json-schema), required non-blank rootPath for"
              + " sql-database, http-json, object-storage, rss-atom, icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, asyncapi-yaml, graphql-sdl, and json-schema; for"
              ...
              + " icalendar, sitemap-xml, robots-txt, llms-txt, openapi-yaml, asyncapi-yaml, graphql-sdl, and json-schema reject remoteUrl — no secrets on this envelope), safe"
              ...
              + " only (portable-safe local rootPath; leftover remoteUrl, credentials, and cloud URL"
              + " rootPath return 400; no live spec fetch). REST Build/Preview/Publish for openapi-yaml"
              + " use that local fixture (Publish copies last-build HTML to IPSSite.root). asyncapi-yaml persist is a local AsyncAPI 2/3 YAML fixture"
              + " only (portable-safe local rootPath; leftover remoteUrl, credentials, and cloud URL"
              + " rootPath return 400; no live spec fetch). REST Build/Preview/Publish for"
              + " asyncapi-yaml use that local fixture (Publish copies last-build HTML to"
              ...
              + " REST Build/Preview/Publish for graphql-sdl use that local fixture (Publish copies"
              ...
              + " Build/Preview/Publish for json-schema use that local fixture (Publish copies"
              + " last-build HTML to IPSSite.root)."
              + " GET after PUT"
  ...
  @Operation(
      ...
      description =
          "Runs the Virtual Site static build (always using the default output root; unlike POST"
              ...
              + " and does not invent pages), or openapi-yaml (local openapi.yaml / openapi.file"
              + " fixture; leftover virtual.remoteUrl, credentials, and cloud URL rootPath are"
              + " 400; no live spec fetch; missing assemble is 400 and does not invent pages),"
              + " or asyncapi-yaml (local asyncapi.yaml / asyncapi.file fixture; leftover"
              + " virtual.remoteUrl, credentials, and cloud URL rootPath are 400; no live spec"
              + " fetch; missing assemble is 400 and does not invent pages), or graphql-sdl (local"
              + " schema.graphql / graphql.file fixture; leftover virtual.remoteUrl, credentials,"
              + " cloud URL rootPath, and graphql.url are 400; no live GraphQL HTTP or introspection;"
              + " missing assemble is 400 and does not invent pages), or json-schema (local"
              + " schema.json / jsonschema.file fixture; leftover virtual.remoteUrl, credentials,"
              + " cloud URL rootPath, jsonschema.url, and remote $ref/$id HTTP are 400; no live HTTP"
              + " schema fetch; missing assemble is 400 and does not invent pages),"
              + " then copies assembled HTML/assets to the"
