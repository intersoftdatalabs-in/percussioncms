 ...
 * loopback). REST {@code GET …/virtual/preview} streams last-build HTML for {@code
 * object-storage} after a successful assemble (missing build is {@code available=false}, HTTP
 * 200). REST {@code POST …/virtual/publish} copies that last-build HTML to {@code
 * IPSSite.root} for git, CSV, SQL, and {@code http-json}.
 *
