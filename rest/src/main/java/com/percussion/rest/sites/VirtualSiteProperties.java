 ...
 * {@code sitemap-xml}. Optional {@code remoteUrl} + {@code branch} apply to {@code git-filesystem}
 * only (fetch/clone into a contained work directory before discover); blank remote keeps local
 * {@code rootPath}. {@code csv-filesystem}, {@code sql-database}, {@code http-json}, {@code
 * object-storage}, {@code rss-atom}, {@code icalendar}, and {@code sitemap-xml} reject {@code
 * remoteUrl} (no secrets on this envelope). {@code sql-database} connection fields (JDBC URL,
 * user, query) live in {@code _config.yaml} under {@code rootPath} — never put passwords on this
 * envelope or in logs. {@code http-json} catalog URL/file live in {@code _config.yaml} ({@code
 * http.url} / {@code http.file}); REST persists a safe {@code rootPath} JSON fixture directory.
 * {@code object-storage}, {@code rss-atom}, {@code icalendar}, and {@code sitemap-xml} persist a
