 ...
 * {@code sitemap-xml}, {@code robots-txt}. Optional {@code remoteUrl} + {@code branch} apply to
 * {@code git-filesystem} only (fetch/clone into a contained work directory before discover);
 * blank remote keeps local {@code rootPath}. {@code csv-filesystem}, {@code sql-database}, {@code
 * http-json}, {@code object-storage}, {@code rss-atom}, {@code icalendar}, {@code sitemap-xml},
 * and {@code robots-txt} reject {@code remoteUrl} (no secrets on this envelope). {@code
 ...
 * icalendar}, {@code sitemap-xml}, and {@code robots-txt} persist a portable-safe local {@code
 * rootPath} (NIO Path; no remaining {@code ..}); cloud URLs and credential properties are 400
 * ({@code rss-atom} is local/loopback only; no live feed credentials; {@code icalendar} is a
 * local RFC 5545 fixture only — no CalDAV; {@code sitemap-xml} is a local sitemap.xml fixture
  ...
  @Schema(
      description =
          "Adapter wire name. Allow-list: git-filesystem, csv-filesystem, sql-database, http-json,"
              + " object-storage, rss-atom, icalendar, sitemap-xml, robots-txt. rss-atom persist/build/preview/publish is"
