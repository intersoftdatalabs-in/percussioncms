...
public interface ISiteAdaptor {
   ...
   * object-storage} / {@code rss-atom} / {@code icalendar} / {@code sitemap-xml}, required root path
   * when virtual and remote is blank, optional remoteUrl/branch for git-filesystem only, safe path /
   * config file name). {@code sql-database} JDBC settings stay in {@code _config.yaml} (never on
   * this envelope). {@code object-storage}, {@code rss-atom}, {@code icalendar}, and {@code
   * sitemap-xml} require a portable-safe local {@code rootPath}; leftover {@code remoteUrl}, cloud
   ...
   * sitemap.xml fixture only — no live crawl). GET after PUT round-trips the stored {@code
   * sourceKind}.
   *
   * @param nameOrId site name or GUID string, not blank
