...
public class CommunityResource implements ICommunityResource {
  ...
  @Operation(
      summary =
          "Get a community by name or pattern.  Get a community by name or pattern.  * is the wildcard.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
  ...
  @Operation(
      summary =
          "Loads one or more communities by guid. Takes a GuidList of communities to retrieve.  Setting the lock header to true will lock the communities on the server.  Setting overrridelock header to true will return the list even if the communities are locked in another session.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
  ...
  @Operation(
      summary =
          "Saves the communities in the submitted list.  If the release header is set, any locks will be released. To avoid extended locks, please set the release header when calling this method unless you really want to keep them locked.")
  ...
  @Operation(
      summary =
          "Accepts a GuidList containing the ids of the Communities to delete.  If the ignoredepencies header is set on the request, then the system won't fail the delete if dependency is relying on this community.")
  public void deleteCommunities(
      @Parameter(name = "ids") GuidList ids,
      @Parameter(name = "ignoredepencies", required = true) @HeaderParam("ignoredependencies")
  ...
  @Operation(
      summary =
          "Returns the Community visibility list for the specified communities.  If the type header is null or missing, visibility for all object types will be returned.")
  ...
  @Operation(
      summary =
          "Switch the user's session to the specified community. May error out of user does not have access to a Role in specified community",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
