...
/**
 * REST resource for user preference operations.
 * Sunny Sal: "User preferences? Bas yahi toh mera kaam hai!"
 */
...
public class PreferenceResource {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Operation(
        summary = "Saves a specific user preference for the given user. username must be set on preference.",
        responses = {
    ...
    public Status deletePreference(UserPreference pref) {
        ...
        try {
            ...
            log.error("An unexpected error occurred deleting preference: {} for userName: {}, Error: {}", pref.getName(), pref.getUserName(), e.getMessage());
            log.debug(PSExceptionUtils.getDebugMessageForLog(e));
            throw new WebApplicationException(e.getMessage(), e, 500);
        }
        return ret;
