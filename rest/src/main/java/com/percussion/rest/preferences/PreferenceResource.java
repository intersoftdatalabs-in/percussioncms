...
public class PreferenceResource {
  ...
  @Autowired IPreferenceAdaptor adaptor;
  ...
  @Produces(value = MediaType.APPLICATION_JSON)
  @Operation(
      summary = "Get a specific user preferences for the logged in user",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
  ...
  public UserPreference savePreference(UserPreference pref) {
    try {
      if (StringUtils.isEmpty(pref.getCategory())) pref.setCategory("sys_preferences");

      if (StringUtils.isEmpty(pref.getContext())) pref.setContext("private");
  ...
  @Consumes(value = MediaType.APPLICATION_JSON)
  @Operation(
      summary = "Delete a specific user preferences for the given user",
      responses = {
