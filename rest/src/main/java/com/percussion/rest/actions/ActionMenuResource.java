...
public class ActionMenuResource {
  ...
  public List<ActionMenu> findActions(
    ...
    if (StringUtils.isEmpty(label)) label = null;
  ...
  @Operation(
      description =
          "Accepts an object with an array of contentid's and assignment types and returns the allowed menus")
  ...
  @Operation(
      ...
      description =
          "Returns a list of Action Menus that matches the criteria. ActionId should be ignored for these menus.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "OK",
  ...
  public ActionMenuList getAllowedContentTypeMenus(AllowedContentTypeMenusRequest request) {
    return new ActionMenuList(
        adaptor.findAllowedContentTypes(
            Arrays.stream(request.getContentIds()).boxed().toArray(Integer[]::new)));
  }
