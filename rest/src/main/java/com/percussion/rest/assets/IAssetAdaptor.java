...
public interface IAssetAdaptor {
  ...
  /**
   * Creates or updates a shared asset based on its path. The path in the representation and the URL
   * should match. TODO PXA If not, do we want to throw an error?
   *
   ...
   *
   * @param path, the asset path to obtain binary if filename starts with thumb_ then thumbnail will
   *     be returned
  ...
  public Asset renameSharedAsset(
      URI baseURI, String Site, String folder, String name, String newName) throws BackendException;
  ...
  /**
   * * Returns a CSV file containing a complete list of non compliant image Assets in the Asset
   * repository.
   *
   * @param baseUri
   * @return
  ...
  /**
   * * Returns a CSV file containing a complete list of non compliant File assets in the Asset
   * repository.
   *
   * @param baseUri
   * @return
  ...
  /**
   * * Using the CSV format provided in the non ADA compliant Images report,bulk update Asset fields
   * based on the contents of the CSV.
   *
   * @param baseUri
   * @param inputStream
   */
  ...
  /**
   * * Using the CSV format provided in the non ADA Compliant Files report, bulk update Asset fields
   * based on the contents of the CSV.
   *
   * @param baseUri
   * @param inputStream
  ...
  /**
   * * Using the CSV format provided by the All Images report, bulk update Assets based on current
   * values in the CSV.
   *
   * @param baseUri
   * @param inputStream
  ...
  /**
   * * Previews the Assets that would be imported given the passed in options. Useful for
   * determining the impact of a bulk import.
   *
   * @param baseUri
   * @param osFolder A valid Operating System folder on the server running Percussion that contains
   *     data to import.
   * @param assetFolder A valid Assets folder
   * @param replace When true, Assets will be replaced if they are detected.
   * @param onlyIfDifferent When true, Assets should only be replaced if they are different,
   *     requires replace to be true or has no effect
