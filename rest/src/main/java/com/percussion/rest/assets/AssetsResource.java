...
public class AssetsResource {
    ...
    @Operation(summary = "Previews an import option with the supplied options.  A CSV file is generated listing the Assets that would be imported.",
            description = "Useful to run before running an import of Assets to determine the impact of the import. The report will also indicate the Asset type that files would be imported to, currently Image, File, or Flash assets are supported.",
            responses = {
    ...
    public Response importPreview(@QueryParam("osPath") String osPath,
        ...
        try {
            var rows = assetAdaptor.previewAssetImport(uriInfo.getBaseUri(), osPath, assetPath, replace, onlyIfDifferent, autoApprove);
            var out = new PSCSVStreamingOutput(rows);
            var r = Response.ok(out)
                    .header("Content-Type", "application/csv")
                    .header("Content-Disposition", "attachment; filename=" + APIUtilities.getReportFileName("asset-import-preview", "csv"));
            return r.build();
        } catch (Exception e) {
    ...
    public Asset uploadBinaryToAsset(
        ...
        if (body == null || body.isEmpty())
            throw new WebApplicationException("No file sent", 400);
        ...
        try (var tis = TikaInputStream.get(att.getObject(InputStream.class))) {
            ...
            return assetAdaptor.uploadBinary(uriInfo.getBaseUri(), path, assetType, tis, uploadFilename, fileMimeType, forceCheckOut);
    ...
    @Operation(summary = "Retrieve a binary file.",
            description = "Get the binary for an image, flash or file asset. Returns a javax.ws.rs.core.Response object.",
            responses = {
    ...
    @Operation(summary = "Sends an email with a report in CSV format listing all Images in the system, to current users's email Id",
            description = "NOTE:  This report can take a very long time to run, on a system with a lot of images.  Be sure to set timeouts accordingly if requesting programmatically.",
            responses = {
                    @ApiResponse(responseCode = "404", description = "Path not found"),
                    @ApiResponse(responseCode = "500", description = "Error"),
                    @ApiResponse(responseCode = "202", description = "Request Accepted", content = @Content(
