  ...
  $.PercMembershipGadget = {
    load: function () {
      ...
      $.PercMembershipService.getAccounts(siteName, function (status, data) {
        if (status == $.PercServiceUtils.STATUS_SUCCESS) {
          var accounts =
            typeof data.getUsersResponse.userSummaries != "undefined"
              ? data.getUsersResponse.userSummaries
              : [];
          $.PercMembershipGadget.updateMembershipTable(
            $.perc_utils.convertCXFArray(accounts)
          );
        } else {
          $.PercMembershipGadget.updateMembershipTable(
            [],
            I18N.message("perc.ui.gadgets.membership@No Accounts Found Long")
          );
        }
      });
    },
    updateMembershipTable: function (data, message) {
      ...
      for (var b = 0; b < data.length; b++) {
        ...
        var row = {
          ...
          rowContent: [
            ...
            [{ content: account.groups, title: account.groups }],
        ...
        if (account.status == "Blocked") {
          var percItemMenu = {
            ...
            items: [
              {
                label: I18N.message("perc.ui.gadgets.membership@Edit User"),
                callback: editUser,
              },
              {
                label: I18N.message("perc.ui.gadgets.membership@Activate"),
                callback: activateUser,
              },
              {
                label: I18N.message("perc.ui.gadgets.membership@Delete"),
                callback: deleteUser,
              },
            ],
          };
          ...
          var percItemMenu = {
            ...
            items: [
              ...
              {
                label: I18N.message("perc.ui.gadgets.membership@Block"),
                callback: blockUser,
              },
              {
                label: I18N.message("perc.ui.gadgets.membership@Delete"),
      ...
      var config = {
        ...
        percHeaders: [
          I18N.message("perc.ui.gadgets.membership@Login Name"),
          I18N.message("perc.ui.gadgets.membership@Created"),
          I18N.message("perc.ui.gadgets.membership@Groups"),
          I18N.message("perc.ui.gadgets.membership@Status"),
        ],
        percData: percData,
        ...
        oLanguage: {
          sZeroRecords:
            message != null
              ? message
              : I18N.message(
                  "perc.ui.gadgets.membership@No Accounts Found Short"
                ),
        },
        percMenus: percMenu,
      };
      ...
      function activateUser(event) {
        var data = event.data;
        var userObj = {
          AccountSummary: { email: data.account, action: "Activate" },
        };
        $.PercMembershipService.activateUser(
          siteName,
          userObj,
      ...
      function blockUser(event) {
        var data = event.data;
        var userObj = {
          AccountSummary: { email: data.account, action: "Block" },
        };
        $.PercMembershipService.activateUser(
          siteName,
          userObj,
      ...
      function deleteUser(event) {
        ...
        var settings = {
          ...
          type: "YES_NO",
          title: I18N.message(
            "perc.ui.gadgets.membership@Confirm User Deletion"
          ),
          question:
            I18N.message(
              "perc.ui.gadgets.membership@You are about to delete the user"
            ) +
            ": '" +
            userEmail +
            "'<br /><br />" +
            I18N.message(
              "perc.ui.gadgets.membership@Delete user confirmation"
            ) +
            "?",
          success: function () {
            $.PercMembershipService.deleteUser(
              siteName,
              userEmail,
              function (status, data) {
                if (status == $.PercServiceUtils.STATUS_SUCCESS) {
