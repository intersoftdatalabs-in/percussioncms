...
(function ($) {
  ...
  function showConfigurationWindow(callback) {
    $.PercLicenseService.getModuleLicense("REDIRECT", function (status, data) {
      if (status) {
        ...
        var url =
          data.moduleLicense.uiProvider +
          "/configure/?auth=" +
          JSON.stringify(clientIdentity);

        var dialogContent = "<div>";
        dialogContent +=
          "  <iframe id='perc-website-config-frame' src='" +
          url +
        ...
        var widget = percJQuery(".perc-website-config-gadget-frame").dialog(
          "widget"
        );
  ...
  function messageEventHandler(evt) {
    ...
    if (_dialog && evt.data.percussion && evt.data.percussion.redirect) {
