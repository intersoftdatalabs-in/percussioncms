...
(function ($) {
  ...
  $.fn.PercWorkflowGadget = function (
    site,
    template,
    workflow,
    state,
  ...
  function loadGadget(criteria) {
    ...
    searchCriteriaObj = {
      query: "",
      folderPath: criteria.site == "@all" ? "" : "//Sites/" + criteria.site,
      sortColumn: "sys_title",
      sortOrder: "asc",
      formatId: -1,
    };
    ...
    if (criteria.lasteditedby != "@all")
      searchFields.push({
        key: "sys_contentlastmodifier",
        value: criteria.lasteditedby,
      });
    ...
    PercSearchService.getAsyncSearchResult(
      { SearchCriteria: searchCriteriaObj },
      function (status, data) {
        if (status == true) {
          tableData = perc_utils.convertCXFArray(
  ...
  function createStatusTable(data) {
    ...
    $.each(itemProperties, function (index, item) {
      var valuesMap = nameValueObjectArrayToMap(
      ...
      var lastModifiedDateParts = perc_utils.splitDateTime(
        valuesMap["sys_contentlastmodifieddate"]
      );
      ...
      var lastModifiedTimeAndWho =
        lastModifiedDateTime == ""
          ? ""
          : lastModifiedDateTime + " (" + lastModifier + ")";
      var lastModifiedDateAndTime =
        lastModifiedDateDate + " " + lastModifiedDateTime; //Combine the last modified date & time
      //var lastPublishedDateParts = perc_utils.splitDateTime(item.lastPublishedDate);

      var lastPublishedDateParts =
        typeof valuesMap["sys_postdate"] == "undefined"
          ? ""
          : perc_utils.splitDateTime(valuesMap["sys_postdate"]);
      var lastPublishedDateDate =
        typeof valuesMap["sys_postdate"] == "undefined" ||
        valuesMap["sys_postdate"] == ""
          ? ""
          : lastPublishedDateParts.date;
      var lastPublishedDateTime =
        typeof valuesMap["sys_postdate"] == "undefined" ||
        valuesMap["sys_postdate"] == ""
          ? ""
          : lastPublishedDateParts.time;
      var lastPublishedDateAndTime =
        lastPublishedDateDate + " " + lastPublishedDateTime; //Combine the published date & time
      var pageName =
        valuesMap["linkText"] != null
          ? valuesMap["linkText"]
          : valuesMap["sys_title"];
      var pageId = item.id;
      ...
      if (item.folderPaths) {
        if (Array.isArray(item.folderPaths)) {
          pagePath =
            item.folderPaths[0].substring(1, item.folderPaths[0].length) +
            "/" +
            item.name;
        } else {
          pagePath =
            item.folderPaths.substring(1, item.folderPaths.length) +
            "/" +
            item.name;
        }
      }
      ...
      var checkBoxContent =
        "<input class='perc-table-row-checkbox' type = 'checkbox' perc-id='" +
        pageId +
        "'/>";
      var previewColContent =
        "<div class ='perc-preview-col perc-datatable-columnrow' style = 'display:none;'><img src='/cm/gadgets/repository/perc_workflow_status_gadget/images/previewIcon.png'/></div><span></span>";
      var previewTitle =
        I18N.message("perc.ui.gadgets.workflowStatus@Preview") +
        "'" +
        pageName +
        "'" +
        I18N.message("perc.ui.gadgets.workflowStatus@Page");
      var statusContent =
        "<span role='status' class='perc-table-row-status' perc-id='" +
        pageId +
        "'>" +
        itemStatus +
        "</span><span class='perc-table-row-status-icon'></span>";
      var row = {
        rowData: { pageId: pageId, pagePath: pagePath },
        rowContent: [
          [{ content: checkBoxContent, callback: function () {} }],
          [{ content: pageName, title: pagePath, callback: $.PercOpenPage }],
          [
            {
              content: previewColContent,
              title: previewTitle,
              callback: $.PercPreviewPage,
            },
          ],
    ...
    var chkBoxContent =
      "<input class = 'perc-header-checkbox' type = 'checkbox' />";

    var headers = [
      chkBoxContent,
      I18N.message("perc.ui.gadgets.workflowStatus@Title"),
      "&nbsp;",
      I18N.message("perc.ui.gadgets.workflowStatus@Status"),
      I18N.message("perc.ui.gadgets.workflowStatus@Last Modified By"),
      I18N.message("perc.ui.gadgets.workflowStatus@Template"),
      I18N.message("perc.ui.gadgets.workflowStatus@Modified"),
      I18N.message("perc.ui.gadgets.workflowStatus@Published"),
    ];
    var headerClasses = [
      "CheckBox",
      "Title",
    ...
    if (!isLargeColumn) percVisibleColumns = [0, 1, 2, 3, 4];
    ...
    var config = {
      ...
      percData: percData,
      percHeaders: headers,
      percHeaderClasses: headerClasses,
      aoColumns: aoColumns,
      showPreviewBtnOnHover: showPreviewBtnOnHover,
      additionalIframeHeight: 45,
    };
    ...
    tableDiv.find(".paginate_button,.paginate_active").on("click", function () {
      tableDiv
        .find(".perc-header-checkbox,.perc-table-row-checkbox")
        .prop("checked", false);
    });
    ...
    var criteriaContentFields = tableDiv
      .parents("body")
      .find("#perc-search-criteria-panel-content-fields");
    var iframe = $(percJQuery.find("#remote_iframe_" + moduleID));
    iframe.height(
      iframe.height() +
        (criteriaContentFields.length > 0
          ? criteriaContentFields.height() + 15
          : 0)
    );
  }
  ...
  function bindRefreshEvent() {
    tableDiv
      .parents("body")
      .find("#perc-search-criteria-panel-content-refresh-search")
      .removeClass("perc-disabled")
      .off("click")
      .on("click", function () {
        clearErrorMessage();
  ...
  function bindSelectFiltersEvent() {
    tableDiv
      .parents("body")
      .find("#perc-search-criteria-panel-content-select-filters")
      .off("click")
      .css("color", "#0099CC")
      .on("click", function () {
        clearErrorMessage();
  ...
  function approveItems(evt) {
    ...
    $.each(tableData, function () {
      ...
      var isItemChecked = tableDiv
        .find(".perc-table-row-checkbox[perc-id='" + this.id + "']")
        .is(":checked");
      if (isItemChecked)
        tableDiv
          .find(".perc-table-row-status[perc-id='" + this.id + "']")
          .next(".perc-table-row-status-icon")
          .addClass("perc-table-row-status-tobeprocessed")
          .html(statusImgHtml_tobe);
      $.extend(approvableItem, { approve: isItemChecked });
      aitems.push(approvableItem);
    });
    tableDiv
      .find(".perc-table-row-status-tobeprocessed")
      .first()
      .addClass("perc-table-row-status-processing")
      .removeClass("perc-table-row-status-tobeprocessed")
      .html(statusImgHtml_proc);
    percWorkflowService.bulkApproveItems(
      dataObject,
  ...
  function getStatus(isFull) {
    percWorkflowService.getBulkApproveStatus(
      jobId,
      isFull,
      function (status, results) {
        if (status == PercServiceUtils.STATUS_ERROR) {
  ...
  function updateStatus(approvalStatus) {
    ...
    if (processedItems) {
      ...
      $.each(processedItems, function () {
        ...
        var statusClass = "";
        var temp = tableDiv
          .find(".perc-table-row-status[perc-id='" + itemId + "']")
          .css("font-weight", "bold")
          .text(this.status);
        if (this.approvalStatus == "Failed") {
          var imgHtml = $(statusImgHtml_error).attr(
            "title",
            this.approvalMessage
          );
          temp
            .next(".perc-table-row-status-icon")
            .addClass("perc-table-row-status-error")
            .removeClass("perc-table-row-status-tobeprocessed")
            .html(imgHtml);
        } else {
          temp
    ...
    switch (approvalStatus.status) {
      ...
      case "FAILED":
        percJQuery.perc_utils.alert_dialog({
          title: I18N.message("perc.ui.gadgets.workflowStatus@Error"),
          content: I18N.message(
            "perc.ui.gadgets.workflowStatus@Unexpected Error Occurred While Approving"
          ),
          id: "perc-no-items-selected-message",
        });
      ...
      case "COMPLETED":
        if (
          approvalStatus.items.errors &&
          !jQuery.isEmptyObject(approvalStatus.items.errors)
        ) {
  ...
  function showErrorDialog(errors) {
    var mapEntries = [];
    if (!Array.isArray(errors.entry)) mapEntries.push(errors.entry);
    else mapEntries = errors.entry;
    ...
    var dialogHTML =
      "<div id='perc-ba-error-main-wrapper'>" +
      "<div>" +
      I18N.message("perc.ui.gadgets.workflowStatus@Pages Not Approved") +
      "</div>" +
    ...
    dialog = percJQuery(dialogHTML).perc_dialog({
      resizable: false,
      title: I18N.message(
        "perc.ui.gadgets.workflowStatus@Approval Errors Title"
      ),
      modal: true,
      closeOnEscape: true,
      percButtons: {
  ...
  var statusImgHtml_proc =
    "<img style='margin-left:5px;margin-bottom:-1px;' src='/cm/gadgets/repository/perc_workflow_status_gadget/images/processing.gif'/>";
  var statusImgHtml_tobe =
    "<img style='margin-left:5px;margin-bottom:-1px;' src='/cm/gadgets/repository/perc_workflow_status_gadget/images/tobeprocessed.png'/>";
  var statusImgHtml_error =
    "<img style='margin-left:5px;margin-bottom:-1px;' src='/cm/gadgets/repository/perc_workflow_status_gadget/images/errorIcon.gif'/>";

  //Search filter code
  $(document).ready(function () {
    var searchConfig;
    var labels = {
      site: I18N.message("perc.ui.gadgets.workflowStatus@Site"),
      template: I18N.message("perc.ui.gadgets.workflowStatus@Template"),
      workflow: I18N.message("perc.ui.gadgets.workflowStatus@Workflow"),
      state: I18N.message("perc.ui.gadgets.workflowStatus@Status"),
      modifiedby: I18N.message(
        "perc.ui.gadgets.workflowStatus@Last Modified By"
      ),
    };
    var defaultWorkflow;
    document.getElementById("perc-bulk-approve-button").innerText =
      I18N.message("perc.ui.workflow.status.gadget@Approve");
      ...
      var searchConfig = {
        ...
        workflow:
          defaultWorkflow != null
            ...
            : {
                name: I18N.message(
                  "perc.ui.workflow.status.gadget@Default Workflow"
                ),
                value: "6",
              },
        state: {
          name: I18N.message("perc.ui.workflow.status.gadget@Pending"),
          value: "4",
        },
    ...
    function setDefaultWorkflow(callback) {
      percJQuery
        .PercWorkflowService()
        .getDefaultWorkflow(function (status, result) {
          if (status) {
    ...
    function openSearchCriteriaDialog() {
      ...
      $.perc_gadgets_search_criteria_dialog.openSearchCriteriaDialog(
        searchConfig,
        function (resultObj) {
          percJQuery.PercMetadataService.save(
            "perc.user." +
              percJQuery.PercNavigationManager.getUserName() +
              ".dash.page.0.mid." +
              moduleId +
              ".prefs.search_criteria",
            ...
            function () {
              ...
              var config = {
                ...
                refreshSearchCallback: function () {
                  loadTable(searchConfig);
                },
              };
              $.perc_gadgets_search_criteria_panel.buildSearchInfoPanel(config);
              var criteriaContentFields = $(
                "#perc-search-criteria-panel-content-fields"
              );
              var iframe = $(percJQuery.find("#remote_iframe_" + moduleId));
              iframe.height(
                iframe.height() +
    ...
    function searchConfigCallback(resultObj) {
      searchConfig = resultObj;
      if ($.isEmptyObject(searchConfig)) searchConfig = getDefaultConfig();
    ...
    setDefaultWorkflow(function () {
      $.perc_gadgets_search_criteria_dialog.getSearchConfig(
        "perc.user." +
          percJQuery.PercNavigationManager.getUserName() +
          ".dash.page.0.mid." +
          moduleId +
        ...
        function (resultObj) {
          ...
          if ($.isEmptyObject(searchConfig)) {
            percJQuery.PercMetadataService.find(
              "perc.user." +
                percJQuery.PercNavigationManager.getUserName() +
                ".dash.page.0.prefs",
              function (status, data) {
                  ...
                  if (data != null) {
                    ...
                    if (dataObj.userprefs["mid_" + moduleId] != null) {
                      ...
                      if (metaPrefsObj.site != null) {
                        newSearchCriteria["site"] = {
                          name: metaPrefsObj.site,
                          value: metaPrefsObj.site,
                        };
                      }
                      percJQuery.PercReusableSearchService.getWorkflows(
                        function (status, result) {
                          if (
                            metaPrefsObj.ssworkflow != null &&
                            metaPrefsObj.ssworkflow == "@all"
                          ) {
                            newSearchCriteria["workflow"] = {
                              name: I18N.message(
                                "perc.ui.gadgets.workflowStatus@All"
                              ),
                              value: "@all",
                            };
                          } else {
                          ...
                          if (
                            newSearchCriteria["workflow"] != null &&
                            metaPrefsObj.status != null &&
                            metaPrefsObj.status != "@all"
                          ) {
                            percJQuery.PercReusableSearchService.getStates(
                              newSearchCriteria["workflow"].name,
                              function (status, result) {
                                for (var i = 0; i < result.length; i++) {
                                  var value = result[i].displayValue;
                                  var name = result[i].value;
                                  if (
                                    metaPrefsObj.status != null &&
                                    metaPrefsObj.status == name
                                  ) {
                                    newSearchCriteria["state"] = {
                                      name: name,
                                      value: value,
                                    };
                                  }
    ...
    function loadTable(searchConfig) {
      $("#perc-search-criteria-panel-content-refresh-search")
        .addClass("perc-disabled")
        .off();
      $("#perc-bulk-approve-button").off().addClass("perc-disabled");
      $("#perc-search-criteria-panel-content-select-filters")
        .off()
        .css("color", "#FFFFFF");
      loadingMsg = miniMsg.createStaticMessage("Loading...");
      site = searchConfig.site.value === "All" ? "" : searchConfig.site.value;
      pathService.getLastExistingPath(
        "/Sites/" + site,
        function (status, result) {
          ...
          var site_title =
            site == ""
              ? I18N.message("perc.ui.gadgets.workflowStatus@All Sites")
              : site;
          var workflowName =
            searchConfig.workflow["name"] == "" ||
            searchConfig.workflow["name"] == "All"
              ? "All Workflows"
              : searchConfig.workflow["name"];
          var stateName =
            searchConfig.state["name"] == "" ||
            searchConfig.state["name"] ==
              I18N.message("perc.ui.gadgets.workflowStatus@All")
              ? I18N.message("perc.ui.gadgets.workflowStatus@All States")
              : searchConfig.state["name"];
          var title =
            I18N.message("perc.ui.gadgets.workflowStatus@PAGES BY STATUS") +
            ": " +
            workflowName +
            " - " +
            stateName +
          ...
          $("#perc-pagesbystatus-gadget").PercWorkflowGadget(
            site,
            template,
            workflow,
            state,
