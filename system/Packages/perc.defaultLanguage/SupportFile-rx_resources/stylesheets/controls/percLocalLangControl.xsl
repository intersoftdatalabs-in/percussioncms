...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="/" />
	<!-- percLocalLanguage -->
	<psxctl:ControlMeta name="percLocalLangControl"
	...
	<xsl:template
		match="Control[@name='percLocalLangControl']" mode="psxcontrol">
		...
		<div class="perc-local-lang" id="{@paramName}">
			...
			<div class="perc-help">
				This widget is used to augment the behavior of the Default Language
				widget which scans your sites to automatically index and add
				alternate language page link references to your page.
				This widget
				will allow you to manually select alternate language pages
				that may
				not be automatically detected.
				<br />
				<br />
				Edit this widget at the page level to select specific alternate
				language versions of this page.
				Add a reference to the table below
				for each Page, Language, and Region
				combination. Language is always
				required.
				<br />
				<br />
				When the Region or Country is selected, search engines will only
				return results from the international site to users speaking that
				language in that region.
				Otherwise UK English content might be
				returned before US English content on
				US search engines and vice
				versa.
				<br />
				<br />
				Tips:
				...
				<table class="perc-table">
					<tbody>
						...
						<tr class="hide perc-local-lang-row" role="presentation">
							...
							<td>
								<div id="perc-page-select" class="perc-page-select">
									<input type="text" name="pageSelections"
										id="perc-content-page-selections-0"
										class="perc-content-edit-page-selections" data-page-id="false"
										title="{Value}" value="{Value}" readonly="readonly"
										style="background-color:#E6E6E9;overflow: hidden;text-overflow: ellipsis;">
										...
										<xsl:call-template
											name="parametersToAttributes">
											<xsl:with-param name="controlClassName"
												select="'sys_PagePath'" />
											<xsl:with-param name="controlNode" select="." />
										</xsl:call-template>
									</input>
									<input type="button" for="perc-content-page-selections-0"
										class="perc-page-field-select-button" value="Browse" />
								</div>
							</td>
							<td>
	...
	<xsl:template
		match="Control[@name='percLocalLangControl' and @isReadOnly='yes']"
