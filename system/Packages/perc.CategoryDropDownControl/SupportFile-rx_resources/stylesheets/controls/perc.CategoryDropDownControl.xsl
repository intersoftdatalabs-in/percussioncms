...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta
		name="perc.CategoryDropDownControl" dimension="single"
		choiceset="none">
		...
		<psxctl:ParamList>
			...
			<psxctl:Param name="size" datatype="Number"
				paramtype="generic">
				<psxctl:Description>If the element is presented as a scrolled list
					box, This parameter specifies the number of rows in the list that
					should be visible at the same time.</psxctl:Description>
			</psxctl:Param>
			<psxctl:Param name="multiple" datatype="String"
				paramtype="generic">
				<psxctl:Description>If set, this boolean attribute allows multiple
					selections. If not set, the element only permits single selections.</psxctl:Description>
			</psxctl:Param>
			...
			<psxctl:Param name="disabled" datatype="String"
				paramtype="generic">
				<psxctl:Description>If set, this boolean attribute disables the
					control for user input.</psxctl:Description>
			</psxctl:Param>
			...
			<psxctl:Param name="aarenderer" datatype="String"
				...
				<psxctl:Description>This parameter specifies whether the field
					...
					other value is treated as POPUP. The recommended values are MODAL
					and POPUP.</psxctl:Description>
				<psxctl:DefaultValue>MODAL</psxctl:DefaultValue>
			</psxctl:Param>
			<psxctl:Param name="parentCategory" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies a parent category name.
					Every field that specifies this group name will be updated when any
					of the other fields in the group are modified.</psxctl:Description>
				<psxctl:DefaultValue>root</psxctl:DefaultValue>
			</psxctl:Param>
			<psxctl:Param name="autoGenerate" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies if the cascaded drop
					downs are to be auto generated or not. Value will be yes to auto
					generate no to not auto generate.</psxctl:Description>
				<psxctl:DefaultValue>yes</psxctl:DefaultValue>
			</psxctl:Param>
			<psxctl:Param name="categoryxmlurl" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies the url location of a
					lookup application conforming to the sys_Lookup.dtd. The current
					form values are sent to the query and are available for use in
					filtering the results.</psxctl:Description>
				<psxctl:DefaultValue>/percCategoryDropDown/sys_Lookup</psxctl:DefaultValue>
			</psxctl:Param>
			<psxctl:Param name="sortOrder" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies the if the options are
					to be displayed by a specific sort order or not. Values to this
					parameter can be ascending or descending.</psxctl:Description>
			</psxctl:Param>
		</psxctl:ParamList>
		<psxctl:AssociatedFileList>
			<psxctl:FileDescriptor
				name="CategoryDropdown.css" type="css" mimetype="text/css">
				<psxctl:FileLocation>/rx_resources/widgets/categoryDropDown/css/CategoryDropdown.css</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="categoryDropdown.js" type="script" mimetype="text/javascript">
	...
	<xsl:template
		match="Control[@name='perc.CategoryDropDownControl']"
		mode="psxcontrol">
		<xsl:variable name="parentCategory">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='parentCategory']">
					<xsl:value-of
						select="ParamList/Param[@name='parentCategory']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='perc.CategoryDropDownControl']/psxctl:ParamList/psxctl:Param[@name='parentCategory']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="categoryxmlurl">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='categoryxmlurl']">
					<xsl:value-of
						select="ParamList/Param[@name='categoryxmlurl']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='perc.CategoryDropDownControl']/psxctl:ParamList/psxctl:Param[@name='categoryxmlurl']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="autoGenerate">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='autoGenerate']">
					<xsl:value-of
						select="ParamList/Param[@name='autoGenerate']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='perc.CategoryDropDownControl']/psxctl:ParamList/psxctl:Param[@name='autoGenerate']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="multiple">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='multiple']">
					<xsl:value-of
						select="ParamList/Param[@name='multiple']" />
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="sortOrder">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='sortOrder']">
					<xsl:value-of
						select="ParamList/Param[@name='sortOrder']" />
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		...
		<div id="maindiv-{@paramName}">
			<input type="hidden" name="{@paramName}"
				id="perc-categories-json-{@paramName}" value="{Value}" />
			<select id="parent-categories-{@paramName}"
				name="{@paramName}-Categories">
				...
				<xsl:if test="@accessKey!=''">
					<xsl:attribute name="accesskey">
						...
						select="preceding-sibling::DisplayLabel" />
                            <xsl:with-param
						name="sourceType"
						select="preceding-sibling::DisplayLabel/@sourceType" />
                            <xsl:with-param
						name="paramName" select="@paramName" />
                            <xsl:with-param
						name="accessKey" select="@accessKey" />
                        </xsl:call-template>
                    </xsl:attribute>
				</xsl:if>
				<xsl:call-template name="parametersToAttributes">
					<xsl:with-param name="controlClassName"
						select="'perc.CategoryDropDownControl'" />
