...
(function ($) {
  ...
  $.fn.PercBlogsGadget = function (siteName, rows, selectedBlogId, gadgetID) {
    ...
    $("#perc-add-post-button").on("click", function (event) {
      openNewPostDialog(event);
    });
    $("#perc-add-post-button").on("keydown", function (event) {
      if (event.code == "Enter" || event.code == "Space") {
        document.activeElement.click();
      }
    });

    $("#perc-add-blog-button").on("click", function (event) {
      $.PercBlogsGadget.showNewBlogDialog(event);
    });
    $("#perc-add-blog-button").on("keydown", function (event) {
      if (event.code == "Enter" || event.code == "Space") {
        document.activeElement.click();
      }
    });

    $("#perc-add-blog-button").attr(
      "title",
      I18N.message("perc.ui.blogs.Gadget.title@text")
    );
  ...
  $.PercBlogsGadget = {
    ...
    load: function (siteName, selectedBlogId) {
      if (selectedBlogId != "@null") {
        $.PercBlogService.getPostsForBlog(
          selectedBlogId,
    ...
    updatePostsTable: function (posts) {
      ...
      if (postsInfo != undefined) {
        ...
        for (var b = 0; b < postsInfo.length; b++) {
          ...
          if (post.tags != undefined && post.tags.length > 0) {
            ...
            tagsLine =
              "<span class='tagsClass perc-ellipsis'>Tags: " + tags + "</span>";
          }
          ...
          if (post.author != undefined && post.author != "") {
            htmlTitleLine +=
              '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style="color:grey; font-weight:bold;"> Author: ' +
              post.author +
              "</span>";
          }

          var commentLine =
            "<span role='button' title='Comments' class='perc-comments-commentsIcon'>&nbsp;</span> Comments (" +
            post.commentsCount +
            ") " +
            (post.newCommentsCount != undefined && post.newCommentsCount != 0
              ? "<span class='perc-comments-newCount'>" +
                post.newCommentsCount +
                " New</span>"
              : "");
          var row = {
            ...
            rowContent: [
              ...
              [{ content: formattedDate }, { content: formattedTime }],
            ],
          };
      ...
      var config = {
        ...
        oLanguage: {
          sZeroRecords:
            "No Posts Found. Click on the button above to create a new post",
        },
      };
    ...
    updateBlogsTable: function (blogs) {
      ...
      for (var b = 0; b < blogs.length; b++) {
        ...
        if (blog.lastPublishDate != "") {
          var lastPublishedDateParts = $.perc_utils.splitDateTime(
            blog.lastPublishDate
          );
        ...
        var row = {
          ...
          rowContent: [
            [
              ...
              {
                content:
                  "<span role='button' title='Open Blog' class='perc-blog-post-link' style='font-weight: normal; cursor: pointer;' for='" +
                  blog.title +
                  "'>Open Blog</span>",
                title: "",
                callback: function (event) {
      ...
      var config = {
        ...
        iDisplayLength: itemsPerPage,
        percHeaders: [
          I18N.message("perc.ui.blogs.Gadget@Blog Title"),
          I18N.message("perc.ui.gadgets.workflowStatus@Published"),
        ],
        percData: percData,
        percMenus: menus,
        oLanguage: {
          sZeroRecords: I18N.message("perc.ui.blogs.Gadget@No Blogs Found"),
        },
      };
      miniMsg.dismissMessage(loadingMsg);
  ...
  $.PercPostMenuActions = {
    title: "",
    menuItemsAlign: "left",
    stayInsideOf: ".dataTables_wrapper",
    items: [
  ...
  function viewPostComments(event) {
    ...
    var siteNameBlog = event.data.pagePath
      .replace(/\/Sites\//, "")
      .replace(/\/.+/, "");
  ...
  function openNewPostDialog() {
    ...
    percJQuery
      .PercFolderHelper()
      .getAccessLevelByPath(sendFolderPath, false, function (status, result) {
        if (
          status == percJQuery.PercFolderHelper().PERMISSION_ERROR ||
          result == percJQuery.PercFolderHelper().PERMISSION_READ
        ) {
          percJQuery.perc_utils.alert_dialog({
            title: "Warning",
            content:
              "You do not have permission to create a post in this blog.",
          });
          return;
        } else {
            ...
            function (status, result) {
              if (status == percJQuery.PercServiceUtils.STATUS_ERROR) {
                ...
                percJQuery.perc_utils.alert_dialog({
                  title: "Warning",
                  content:
                    "You are not authorized to create a blog post in this blog.",
                });
                hasWritePermission = false;
                return;
              } else {
                var templateId = postsTemplateId;
                this.window.parent.jQuery
                  .PercNewPageDialog()
                  .openDialog(sendFolderPath, templateId);
                return;
              }
            },
            sendFolderPath
