...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta name="percCalendarWidgetControl"
	...
	<xsl:template
		match="Control[@name='percCalendarWidgetControl']" mode="psxcontrol">
		<xsl:if
			test="//DisplayError/Details/FieldError[@submitName='calendar_unique_name']">
			<input type="hidden" id="calendar_unique_name_error">
				<xsl:attribute name="value"><xsl:value-of
					select="//DisplayError/Details/FieldError[@submitName='calendar_unique_name']" /></xsl:attribute>
			</input>
		</xsl:if>
		<label class="perc-required-field" for="display_sys_title">Name in finder:</label>
		<br />
		<input class="datadisplay" size="50" type="text"
			name="display_sys_title" id="display_sys_title" />
		<br />
		<xsl:if
			test="/ContentEditor/DisplayError/Details/FieldError[@submitName='sys_title']">
			<label class="perc_field_error">
				<xsl:value-of
					select="/ContentEditor/DisplayError/Details/FieldError[@submitName='sys_title']" />
			</label>
			<br />
			<br />
		</xsl:if>
		<label for="display_full_calendar">Full calendar page: (for mini calendar setup only)</label>
		<br />
		<input class="datadisplay" size="50" type="text"
			name="full_calendar_page" id="full_calendar_page" value="{Value}" />
		...
		<script>
			...
			sys_title, the error message needs to be shown
			below display_sys_title
			field
			...
			//handle
			the uniqueness of the form name here.
			if($("#calendar_unique_name_error").length)
			{
			if(!$('div.perc_field_error').length){
			var errorText =
			$("#calendar_unique_name_error").val();
			$("label[for =
			'display_full_calendar']").before('
			<div class="perc_unique_name perc_field_error" />
			');
			...
			//Text auto fill and
			filter settings for form fields
			{
			var calendarTitle =
			$('#perc-content-edit-calendar_title');
			var calendarName =
			$('#display_sys_title');
			$.perc_textAutoFill(calendarTitle,
			calendarName,
			$.perc_autoFillTextFilters.URL, null, 49);
			}

			//Handle the
			results page browse button click
			$("#perc_querylist_resultspage_browse").on("click",function(){
			var
			dlgTitle = "Select Full Calendar Page"
			var inputElemId =
			"full_calendar_page";
			...
			pagePath.indexOf("/");
			pagePath =
			pagePath.substring(chopStartPosition);
			$("#" +
			...
			asset_name: I18N.message(
			"perc.ui.saveasdialog.label@Selected Page:" ),
			title: dlgTitle,
			save_class: 'perc-save'
			});
			}

			// A helper method called on form
	...
	<xsl:template
		match="Control[@name='percCalendarWidgetControl' and @isReadOnly='yes']"
		priority='10' mode="psxcontrol">
		<label for="display_sys_title">Name in finder:</label>
		<br />
		<div id="perc-readonly-calendar_name" class="datadisplay"></div>
		<input style="display:none" class="datadisplay" size="50"
			type="text" name="display_sys_title" id="display_sys_title" />
		<br />
		<label for="display_full_calendar">Full calendar page: (for mini calendar setup only)</label>
		<br />
		<div id="perc-readonly-full_calendar_page" class="datadisplay"></div>
		<input style="display:none" class="datadisplay" size="50"
			type="text" name="full_calendar_page" id="full_calendar_page"
			value="{Value}" />
		<script>
			(function($) {
			$(document).ready(function() {
