...
$(function () {
  ...
  var loadingMsg = miniMsg.createStaticMessage(
    I18N.message("perc.ui.gadgets.cookieConsent@Loading")
  );
  ...
  function initButtons() {
    ...
    $exportButton.on("click", function (evt) {
      exportCookieCSV(
        SELECTED_SITES_CSV_URL + "/" + site + "/cookie_consent_log.csv"
      );
    });
  ...
  function confirmDelete(siteName) {
    ...
    var cookieQuestion =
      typeof siteName === "object"
        ? I18N.message("perc.ui.deletecookiesdialog.warning@Confirm")
        : I18N.message("perc.ui.deletesitecookiesdialog.warning@Confirm");
    var tag = "";
    ...
    if (typeof siteName === "object") {
      ...
      tag =
        I18N.message("perc.ui.deletesitecookiesdialog.tag@Cookies") +
        " " +
        siteName;
    }
    ...
    var options = {
      ...
      question:
        ...
        cookieQuestion +
        "<br/><br/>" +
        I18N.message("perc.ui.common.question@Continue") +
        warnCloseSpan,
      success: function () {
        if (typeof siteName === "object") {
          deleteAllEntries();
  ...
  function updateGadgetTable(stats) {
    ...
    Object.keys(stats).forEach(function (key) {
      var elem = $("<tr/>")
        .append(
          $("<td/>").append(
            $("<button/>").addClass("perc-cookie-button-site").text(key)
          )
        )
        .append($("<td/>").text(stats[key]));
    ...
    tableSelector.dataTable({
      ...
      oLanguage: {
        sZeroRecords: I18N.message(
          "perc.ui.gadgets.cookieConsent@No Entries Found"
        ),
        oPaginate: {
          sFirst: "&lt;&lt;",
          sPrevious: "&lt;",
  ...
  function displaySiteStatistics(siteName) {
    ...
    loadingMsg = miniMsg.createStaticMessage(
      I18N.message("perc.ui.gadgets.cookieConsent@Loading")
    );
    ...
    var siteURL =
      LOG_ENDPOINT + "/" + siteName + "/" + siteName + "_consent_log.csv";

    // update export entries button
    $exportSiteButton = $("#export-site-link");
    $exportSiteButton.text(
      I18N.message("perc.ui.gadgets.cookieConsent@Export consent entries for") +
        " " +
        siteName
    );
    ...
    $("#advanced-site p").text(
      I18N.message("perc.ui.gadgets.cookieConsent@Delete entries") +
        ": " +
        siteName
    );
  ...
  function getAllConsentEntries(site) {
    PercCookieConsentService.getAllCookieConsentEntries(
      site,
      function (status, result, message) {
        if (status == PercServiceUtils.STATUS_SUCCESS) {
          ...
          PercUtils.alert_dialog({
            title: I18N.message("perc.ui.gadgets.cookieConsent@Error"),
            content: message,
          });
          $("#perc-cookie-table").text(message);
          console.error(
            I18N.message(
              "perc.ui.gadgets.cookieConsent@Error populating entries"
            )
          );
        }
      }
  ...
  function getCookieConsentTotalsPerSite(siteName) {
    PercCookieConsentService.getCookieConsentEntriesPerSite(
      siteName,
      function (status, result, message) {
        if (status === PercServiceUtils.STATUS_SUCCESS) {
          ...
          console.error(
            I18N.message(
              "perc.ui.gadgets.cookieConsent@Error retrieving entries"
            ) +
              ": " +
              siteName
          );
  ...
  function exportCookieCSV(url) {
    PercCookieConsentService.exportCookieCSV(url, function (status, result) {
      if (status == PercServiceUtils.STATUS_SUCCESS) {
        ...
        var errMessage = I18N.message(
          "perc.ui.gadgets.cookieConsent@Error exporting entries"
        );
        PercUtils.alert_dialog({
          title: I18N.message("perc.ui.gadgets.cookieConsent@Error"),
          content: errMessage,
  ...
  function deleteAllEntries() {
    PercCookieConsentService.deleteAllCookieConsentEntries(
      LOG_ENDPOINT + "/" + site,
      function () {
        window.location.reload();
        console.log(
          I18N.message("perc.ui.gadgets.cookieConsent@Success deleting entries")
        );
      },
      function (results) {
        var errMessage = I18N.message(
          "perc.ui.gadgets.cookieConsent@Error deleting entries"
        );
        PercUtils.alert_dialog({
          title: I18N.message("perc.ui.gadgets.cookieConsent@Error"),
          content: errMessage,
        ...
        var defaultMessage =
          PercServiceUtils.extractDefaultErrorMessage(results);
  ...
  function deleteAllEntriesForSite(siteName) {
    ...
    PercCookieConsentService.deleteAllCookieConsentEntriesForSite(
      finalUrl,
      function () {
        window.location.reload();
        console.log(
          I18N.message(
            "perc.ui.gadgets.cookieConsent@Success deleting entries for site"
          ) +
            ": " +
            siteName
        );
      ...
      function (results) {
        ...
        var errMessage =
          I18N.message(
            "perc.ui.gadgets.cookieConsent@Error deleting entries for site"
          ) +
          ": " +
          siteName;
        PercUtils.alert_dialog({
