...
(function ($) {
  ...
  $.renderProcessMonitor = function (percJQuery, isGadget) {
    $("head")
      .append(
        ' <link href="/cm/jslib/profiles/3x/libraries/fontawesome/css/all.css" rel="stylesheet" type="text/css" />'
      )
      .append(
        '<link href="/cm/gadgets/repository/PercProcessorMonitorGadget/PercProcessorMonitorGadget.css" rel="stylesheet" type="text/css" />'
      );
    $("body").empty();
    var initialHtml =
      '<div id="perc-process-monitor-gadget">' +
      '<div id="perc-process-monitor-actions"><span id="perc-process-monitor-status-refresh" tabindex="0" role="button" title="' +
      I18N.message(
        "perc.ui.gadgets.processmonitor@Refresh Background Process"
      ) +
      '">&nbsp;</span><span id="perc-process-monitor-popout"><i ' +
      'class="icon-external-link"></i></span></div>' +
      '<div id="perc-process-monitor-table-container">' +
      '<table id="perc-process-monitor-table" width="100%">' +
      '<tr for="header">' +
      '<th scope="col" align="left" width="25%">' +
      I18N.message("perc.ui.gadgets.processmonitor@Process") +
      "</th>" +
      '<th scope="col" align="left" width="75%">' +
      I18N.message("perc.ui.gadgets.processmonitor@Status") +
      "</th>" +
      "</tr>" +
      "</table>" +
      "</div>" +
      '<div id="perc-pm-refresh-status"><b>' +
      I18N.message("perc.ui.gadgets.processmonitor@Last Refreshed") +
      ': </b><span id="perc-pm-refresh-time"></span></div>' +
      "</div>";
    ...
    if (isGadget) {
      $("#perc-process-monitor-popout").on("click", function () {
        ...
        var PercProcessMonitorWindow = window.open(
          "/cm/app/?view=popup&popuppage=PercProcessMonitor.jsp",
          "PercProcessMonitor",
    ...
    $("#perc-process-monitor-status-refresh")
    ...
    function renderStatusRows() {
      ...
      serviceUtils.makeJsonRequest(
        percJQuery.perc_paths.PROCESS_STATUS_ALL,
        "GET",
        false,
        function (status, results) {
          ...
          if (status != serviceUtils.STATUS_SUCCESS || !results.data) {
            var additonalMsg = "";
            if (!isGadget) {
              additonalMsg = I18N.message(
                "perc.ui.gadgets.processmonitor@Please Close Window"
              );
            }
            statusRows =
              '<tr for="data">' +
              '<td scope="row" for="error" colspan="2">' +
              I18N.message(
                "perc.ui.gadgets.processmonitor@Error Fetching Processors"
              ) +
              " " +
              additonalMsg +
              "</td>" +
          ...
          $.each(results.data.psMonitorList.monitor, function () {
            ...
            $.each(entries, function () {
              if (this.key === "name")
                name = I18N.message(
                  "perc.ui.gadgets.processmonitor@" + this.value
                );
              if (this.key === "message")
          ...
          $("#perc-process-monitor-status-refresh").removeClass(
            "perc-disabled"
          );
          $("#perc-pm-refresh-time").text(
            $.datepicker.formatDate("M d, yy", new Date()) +
              " " +
              percUtils.formatTimeFromDate(new Date(), true)
          );
