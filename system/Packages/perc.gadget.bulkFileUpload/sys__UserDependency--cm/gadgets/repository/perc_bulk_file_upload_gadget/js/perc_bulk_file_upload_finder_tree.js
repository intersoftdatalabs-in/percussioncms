...
function handleChecks(pathItem) {
  $("#perc-bulk-target-folder").text(pathItem.path).data("pathItem", pathItem);
...
function _handleApprovalCheckboxState(folderPath) {
  $.PercWorkflowService().isApproveAvailableToCurrentUser(
    folderPath,
...
function _handleButtonEnableState() {
  ...
  if (
    percAssetType === "html" ||
    percAssetType === "richtext" ||
    percAssetType === "simpletext"
  ) {
