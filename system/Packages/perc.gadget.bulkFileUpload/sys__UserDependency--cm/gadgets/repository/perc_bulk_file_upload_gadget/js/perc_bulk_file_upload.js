    ...
    add: function (e, data) {
      ...
      $("#perc-upload-trigger").on("click", function () {
        ...
        if (data.files.length > 0) {
          ...
          $("#perc-global-progress").removeClass("fade").show();
...
generateButtonHTML = function (data) {
  ...
  var buttonHtml = $(
    '<button class="btn perc-button" aria-label="remove item from queue">' +
  ...
  html.appendTo($("#perc-added-files")).hide().fadeIn("slow");

  return html;
};

markCompleted = function () {
  $("#perc-bulk-status").text(
    "Successful Uploads: " + NUM_COMPLETED + ", Failed uploads: " + NUM_FAILED
  );
  ...
  $("#perc-global-progress").addClass("fade").hide();
};
...
triggerSubmit = function () {
  if (shouldPromptForFileType()) {
    ...
    var options = {
      title: title,
      question:
        "You have added images for upload but have not selected the 'Asset Type' of 'image.'  Are you sure this is correct?",
      cancel: function () {
        return;
      },
      success: function () {
