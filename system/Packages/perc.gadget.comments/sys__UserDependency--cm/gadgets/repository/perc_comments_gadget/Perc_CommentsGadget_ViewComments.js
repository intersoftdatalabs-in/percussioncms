...
(function ($) {
  ...
  function createDataTable(data, site, pagePath) {
    ...
    statusTable = $(
      "<table id='perc-gadget-comments-viewComments-table' cellspacing='0'>"
    );
    tableDiv.append(statusTable);

    var percData = [];

    $.PercCommentsGadgetService().getArticleDescription(
      site,
      pagePath,
      function (article, callbackOptions) {
        var articleDesc = $("<div />")
          .append(
            $("<div />")
              .attr("id", "perc-gadget-comments-viewComments-articleSummary")
              .append(
                $("<span />").text($("<div />").append(article.title).text())
              )
              .append(
                $("<span />").text($("<div />").append(article.summary).text())
              )
          )
          .append(
            $("<div />")
        ...
        $.each(data, function () {
          ...
          var commentTitle = $("<h2 />")
            .addClass("perc-gadget-comments-viewComments-commentTitle")
            .text($("<div />").append(commentData.title).text());
          ...
          var commentDate = $("<span />")
            ...
            .append(
              $("<span />").text(", " + dateParts.date + ", " + dateParts.time)
            );
          var commentAuthor = $("<span />")
            .addClass("perc-gadget-comments-viewComments-commentAuthor")
            .text(
              $("<div />")
                .append(
                  commentData.username ? commentData.username : "Anonymous"
                )
                .text() + " said..."
            );
          var commentText = $("<div />")
            .addClass("perc-gadget-comments-viewComments-commentText")
            .text($("<div />").append(commentData.text).text());

          var commentAggregate = $("<div />")
            .addClass("perc-datatable-firstrow")
            .append(commentDate)
            .append(commentAuthor)
            .append(commentText);

          var commentId = commentData.id;
          var rejectApproveActions = newRejectApproveActions(
            commentData.approvalState,
            commentId,
            site
          );
          ...
          var row = {
            rowContent: [
              [{ content: commentAggregate.prop("outerHTML") }],
        ...
        var dataTypeCols0 = [
          {
            bSortable: false,
            sClass:
              "perc-datatable-cell-string perc-gadget-comments-viewComments-comment",
          },
          {
            bSortable: false,
            sClass:
              "perc-datatable-cell-action perc-gadget-comments-viewComments-action",
          },
        ];
        var dataTypeCols1 = [
          {
        ...
        var tableConfig = {
          ...
          percHeaderClasses: [
            "perc-datatable-cell-string perc-gadget-comments-viewComments-comment",
            "perc-datatable-cell-string perc-gadget-comments-viewComments-action",
          ],
          percHeaders: ["<span>Title</span>", "<span>Action</span>"],
          percData: percData,
          oLanguage: {
            sZeroRecords: "No Comments Found",
            sInfo: "Showing _START_ to _END_ of _TOTAL_ total results",
        ...
        statusTable.on(
          "click",
          "div.perc-gadget-comments-rejectAction-all",
          rejectAll
        );
        statusTable.on(
          "click",
          "div.perc-gadget-comments-approveAction-all",
          approveAll
        );
        statusTable.on(
          "click",
          "div.perc-gadget-comments-deleteAction-all",
          deleteAll
        );

        statusTable.on(
          "click",
          "div.perc-gadget-comments-rejectAction",
          reject
        );
        statusTable.on(
          "click",
          "div.perc-gadget-comments-approveAction",
          approve
        );
        statusTable.on(
          "click",
          "div.perc-gadget-comments-deleteAction",
          deleteComment
        );
        ...
        if (lastRowNewHeight > lastRowHeight) lastRow.height(lastRowNewHeight);
  ...
  function newRejectApproveAllActions() {
    var approvalActionMenu = $(
      "<div class='perc-gadget-comments-actionMenu-all'>"
    );
    var approvalActionRejectAction = $(
      "<div class='perc-gadget-comments-rejectAction-all'  title='Reject All Comments' />"
    );
    var approvalActionApproveAction = $(
      "<div class='perc-gadget-comments-approveAction-all' title='Approve All Comments' />"
    );
    var approvalActionDeleteAction = $(
      "<div class='perc-gadget-comments-deleteAction-all' title='Delete All Comments' />"
    );

    var approvalActionAllLabel = $(
      "<div class='perc-gadget-comments-label-all'>ALL</div></div>"
    );
  ...
  function newRejectApproveActions(approvalState, commentId, site) {
    var approvalStateClass =
      approvalState == "APPROVED"
        ? "perc-gadget-comments-approved"
        : "perc-gadget-comments-rejected";
    var approvalStateClassOriginal = approvalStateClass;

    var approvalActionMenu = $(
      "<div class='perc-gadget-comments-actionMenu' site='" +
        site +
        "' commentId='" +
        commentId +
        "'>"
    ...
    var approvalActionRejectAction = $(
      "<div class='perc-gadget-comments-rejectAction'  title='Reject Comment'>"
    );
    var approvalActionApproveAction = $(
      "<div class='perc-gadget-comments-approveAction' title='Approve Comment'>"
    );
    var approvalActionDeleteAction = $(
      "<div class='perc-gadget-comments-deleteAction' title='Delete Comment'>"
    );
  ...
  $(document).ready(function () {
    ...
    window.parent
      ...
      .parent()
      .css("padding", "0");
    $.PercCommentsGadgetService().getCommentsOnPage(
      site,
      pagePath,
      function (comments, callbackOptions) {
