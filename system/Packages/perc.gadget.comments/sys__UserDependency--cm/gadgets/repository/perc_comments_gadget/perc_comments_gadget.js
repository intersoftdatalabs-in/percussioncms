...
(function ($) {
  ...
  var PAGE_NOT_FOUND_LABEL = I18N.message(
    "perc.ui.gadgets.comments@Page not found in CM1"
  );
  var PAGE_NOT_FOUND_TOOLTIP = I18N.message(
    "perc.ui.gadgets.comments@Associated page does not exist in CM1"
  );
  ...
  $.fn.PercCommentsGadget = function (site, rows) {
    ...
    defaultModerationLink = $(this)
      .find("#perc-set-default-moderation-link")
    ...
    var tableDiv = $(this);
    tableDiv.append(
      '<div id="perc-gadget-comments-table-header">Comments</div>'
    );
    if (site == null) site = "";
  ...
  function loadGadget(site, tableDiv) {
    ...
    var callbackOptions = {};
    $.PercCommentsGadgetService().getAllCommentTotals(
      site,
  ...
  function createDataTable(data, siteName, tableDiv, message) {
    ...
    tableDiv.append(
      $('<div id="perc-comments-table-wrapper" />').append(statusTable)
    );
    ...
    var CM1Menu = {
      ...
      items: [
        {
          label: I18N.message("perc.ui.gadgets.comments@Open Page"),
          callback: editPage,
        },
        {
          label: I18N.message("perc.ui.gadgets.comments@Preview Page"),
          callback: previewPage,
        },
        {
          label: I18N.message("perc.ui.gadgets.comments@View Comments"),
    ...
    $.each(data, function () {
      ...
      if (pageSummary.commentCount > 0) {
        if (pageSummary.newCount > 0) {
          commentLine =
            "<span class='perc-comments-commentsIcon'>&nbsp;</span> Comments (" +
            pageSummary.commentCount +
            ") <span class='perc-comments-newCount'>" +
            pageSummary.newCount +
            " New</span>";
        } else {
          commentLine =
            "<span class='perc-comments-commentsIcon'>&nbsp;</span> Comments (" +
      ...
      var row = {
        rowContent: [
          [
            { content: linkTitle, title: path, callback: previewCallback },
            { content: truncateSummary(summary), title: summary },
            { content: commentLine, title: "", callback: viewComments },
          ],
          [postedDate, postedTime],
        ],
        rowData: {
          pageId: pageSummary.id,
          pagePath: pageSummary.pagePath,
          path: path,
          siteName: siteName,
        },
      };
    ...
    var dataTypeCols = [
      {
        sSortDataType: "dom-ptext",
        sType: "html",
        sClass: "perc-datatable-cell-string perc-comments-linktext",
      },
      {
        sSortDataType: "dom-pdate",
        sType: "html",
        sClass: "perc-datatable-cell-date perc-comments-publishDate",
      },
    ];
    ...
    var tableConfig = {
      ...
      percHeaderClasses: [
        "perc-datatable-cell-string perc-comments-linktext",
        "perc-datatable-cell-date perc-comments-publishDate",
      ],
      percHeaders: [
        I18N.message("perc.ui.gadgets.comments@Title"),
        I18N.message("perc.ui.gadgets.comments@Posted"),
      ],
      percData: percData,
      oLanguage: {
        sZeroRecords:
          message != null
            ? message
            : I18N.message("perc.ui.gadgets.comments@No comments found"),
      },
      percMenus: menus,
  ...
  function displayDefaultModerationDialog(evt) {
    ...
    $.PercCommentsGadgetService().getDefaultCommentModeration(
      site,
      function (status, defaultCommentsModeration) {
        if (status === $.PercServiceUtils.STATUS_ERROR) {
          percJQuery.perc_utils.alert_dialog({
            title: I18N.message("perc.ui.gadgets.comments@Error"),
            content: defaultCommentsModeration,
          });
          return;
        }
        var url =
          $.PercCommentsGadgetService().constants.URLS
            .SET_DEFAULT_COMMENT_MODERATION_DIALOG_JSP +
          "?" +
          "site=" +
          escape(site) +
          "&" +
          "state=" +
          escape(defaultCommentsModeration);
        var dlgHtml =
          "<div id='perc-gadget-comments-default-moderation-dialog' style='padding:0px'>" +
        ...
        percDialogObject.percButtons["Save"] = {
          click: function () {
            ...
            $.PercCommentsGadgetService().setDefaultCommentModeration(
              moderation,
              function (status, defaultMsg) {
                if (status === $.PercServiceUtils.STATUS_ERROR) {
                  percJQuery.perc_utils.alert_dialog({
                    title: I18N.message("perc.ui.gadgets.comments@Error"),
                    content: defaultMsg,
    ...
    function parseDefaultModerationFromDialog(dialog, site) {
      ...
      var moderation = {
        defaultModerationState: { site: site, state: "APPROVED" },
      };
