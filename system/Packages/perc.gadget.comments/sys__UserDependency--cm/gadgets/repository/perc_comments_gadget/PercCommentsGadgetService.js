  ...
  var PercCommentsGadgetService = function () {
    ...
    var constants = {
      URLS: {
        ...
        VIEW_COMMENTS_JSP:
          "/cm/app/dialogs/Perc_CommentsGadget_ViewComments.jsp",
      },
    };
    /**
    ...
    var exportedConstants = {
      URLS: {
        ...
        SET_DEFAULT_COMMENT_MODERATION_DIALOG_JSP:
          "/cm/app/dialogs/PercCommentsGadgetSetDefaultCommentModeration.jsp",
      },
    };
    ...
    var getAllCommentTotals = function (site, callback, callbackOptions) {
      ...
      $.PercServiceUtils.makeJsonRequest(
        constants.URLS.ALL_COMMENT_TOTALS + "/" + site,
        $.PercServiceUtils.TYPE_GET,
        false,
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            callback(
              result.data.commentsSummary,
              callbackOptions,
            ...
            callback(
              ...
              callbackOptions,
              I18N.message(
                "perc.ui.gadgets.comments@Delivery service may be unavailable"
              )
            );
          }
        }
    ...
    var getCommentsOnPage = function (
      site,
      pagePath,
      callback,
      callbackOptions
      ...
      $.PercServiceUtils.makeJsonRequest(
        constants.URLS.COMMENTS_ON_PAGE + "/" + site + pagePath,
        $.PercServiceUtils.TYPE_GET,
        false,
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
    ...
    var getArticleDescription = function (
      site,
      pagePath,
      callback,
      callbackOptions
      ...
      // Note, pagePath should start with a single /
      $.PercServiceUtils.makeJsonRequest(
        constants.URLS.ARTICLE_DESC + "/" + site + pagePath,
        $.PercServiceUtils.TYPE_GET,
        false,
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
    ...
    var setCommentModeration = function (site, moderations, callback) {
      $.PercServiceUtils.makeJsonRequest(
        constants.URLS.SET_MODERATIONS + "/" + site,
    ...
    var setDefaultCommentModeration = function (moderation, callback) {
      $.PercServiceUtils.makeJsonRequest(
        constants.URLS.DEFAULT_MODERATION,
        $.PercServiceUtils.TYPE_PUT,
        false,
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
        ...
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            callback(
              ...
              result.data.defaultModerationState
            );
          } else {
            var defaultMsg = $.PercServiceUtils.extractDefaultErrorMessage(
              result.request
