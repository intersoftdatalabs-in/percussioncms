...
(function ($) {
  ...
  var trafficScale = window.parent.trafficScale
    ? window.parent.trafficScale
    : 30;
  ...
  var VISITS_SERIES = 0,
    UPDATES_SERIES = 1,
    LIVEPAGES_SERIES = 2;
  var INITIAL_FRAME_HEIGHT = 350;
  var INITIAL_COLLAPSED_FRAME_HEIGHT = 360;
  ...
  var chart1, chart2;
  var tooltip = $(
    '<table class="jqplot-highlighter jqplot-highlighter-tooltip" style="display : none">' +
  ...
  function loadGadget(
    ...
    var title =
      "TRAFFIC: " + dateFromStr + " - " + dateToStr + " (" + site_title + ")";
    ...
    if (selectedGranularity === "DAY") {
      ...
      additionalDataPoints = calculateAdditionalDatapointCount(
        selectedDataPointCount
      );
      dateTo = selectedEndDate;
      if (additionalDataPoints !== 0)
        dateTo = getDateAfterRange(
          dateTo,
          selectedGranularity,
      ...
      dateFromStr = $.datepicker.formatDate(
        SIMPLE_DATE_FORMAT,
        getStartOfWeekDate(selectedStartDate)
      ); // I think I already have this as a string
      ...
      var months =
        (selectedEndDate.getFullYear() - selectedStartDate.getFullYear()) * 12;
      selectedDataPointCount =
        1 + selectedEndDate.getMonth() - selectedStartDate.getMonth() + months;
      additionalDataPoints = calculateAdditionalDatapointCount(
        selectedDataPointCount
      );
      dateFromStr = $.datepicker.formatDate(
        SIMPLE_DATE_FORMAT,
        getFirstDateInMonth(selectedStartDate)
      );
      ...
      selectedDataPointCount =
        1 + selectedEndDate.getFullYear() - selectedStartDate.getFullYear();
      additionalDataPoints = calculateAdditionalDatapointCount(
        selectedDataPointCount
      );
      dateFromStr = $.datepicker.formatDate(
        SIMPLE_DATE_FORMAT,
        getFirstDateInYear(selectedStartDate)
      );
    ...
    if (!fakeData) {
      PercActivityService.getContentTraffic(
        "//Sites/" + site,
        dateFromStr,
        dateToStr,
        selectedGranularity,
        ...
        function (status, data) {
          if (
            status == PercServiceUtils.STATUS_SUCCESS &&
            data.ContentTraffic != ""
          ) {
            draw(data);
            graphLabels.show();
            ...
            chart1.html(
              "<div class='perc-gadget-errormessage'>" + data + "</div>"
            );
            graphLabels.hide();
      ...
      var visits = createRandomSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
      var newPages = createRandomSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
      var pageUpdates = createRandomSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
      takeDowns = createRandomSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
      ...
      var lives = createRandomSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
      var dates = createDatesSeriesForDates(
        dateFromStr,
        dateToStr,
        selectedGranularity
      );
  ...
  function draw(data) {
    ...
    if (showNewPageUpdates || showTakedownUpdates || showUpdates) {
      if (showUpdates) totalUpdates = pageUpdates;
    ...
    var updatesSeries = createSeriesForDates(
      dates,
      totalUpdates,
      newPages,
      pageUpdates,
    ...
    $.jqplot.eventListenerHooks.push([
      "jqplotMouseDown",
      startDrilldownSelection,
    ]);
    $.jqplot.eventListenerHooks.push([
      "jqplotMouseUp",
      setEndDrilldownSelection,
    ]);
  ...
  function drillDownChart(selectedStartDateDrill, selectedEndDateDrill) {
    ...
    if (selectedGranularity == "DAY") {
      if (isSameDay(selectedStartDateDrill, selectedEndDateDrill)) {
        ...
        selectedStartDateDrill = getDateAfterRange(
    ...
    startDateLabel.html(
      $.datepicker.formatDate(TOOLTIP_DATE_FORMAT, selectedStartDateDrill)
    );
    endDateLabel.html(
      $.datepicker.formatDate(TOOLTIP_DATE_FORMAT, selectedEndDateDrill)
    );

    selectedStartDateDrill = $.datepicker.formatDate(
      SIMPLE_DATE_FORMAT,
      selectedStartDateDrill
    );
    selectedEndDateDrill = $.datepicker.formatDate(
      SIMPLE_DATE_FORMAT,
  ...
  function getActivityForDateRange(
    ...
    busy = true;
    PercActivityService.getActivityForDateRange(
      "//Sites/" + selectedSite,
      selectedStartDateDrill,
      selectedEndDateDrill,
      selectedUsage,
  ...
  function showDetailsPane() {
    ...
    hideDetailsLink.html("Hide details").css("color", "#0099CC");
    detailsPane.show();
    if (expandedFrameHeight) parentFrame.height(expandedFrameHeight);
  ...
  function hideDetailsPane() {
    ...
    if (isDetailsVisible) expandedFrameHeight = parentFrame.height();
    detailsPane.hide();
  ...
  function updateTooltip(ev, gridpos, datapos, neighbor, plot) {
    ...
    if (neighbor != null) {
      ...
      if (selectedGranularity === "DAY") {
        toolTipDateString = $.datepicker.formatDate(
          TOOLTIP_DATE_FORMAT_DAY,
          date
        );
      } else if (selectedGranularity === "WEEK") {
        toolTipDateString = $.datepicker.formatDate(
          TOOLTIP_DATE_FORMAT_WEEK,
          date
        );
        var endOfWeek = getEndOfWeekDate(date);
        toolTipDateString +=
          " - " + $.datepicker.formatDate(TOOLTIP_DATE_FORMAT_WEEK, endOfWeek);
      } else if (selectedGranularity === "MONTH") {
        toolTipDateString = $.datepicker.formatDate(
          TOOLTIP_DATE_FORMAT_MONTH,
          date
        );
      } else if (selectedGranularity === "YEAR") {
        toolTipDateString = $.datepicker.formatDate(
          TOOLTIP_DATE_FORMAT_YEAR,
          date
        );
      }
      ...
      if (tTop < gTop) tTop = gTop;
      if (tTop + tHeight > gTop + gHeight) tTop = gTop + gHeight - tHeight;

      if (tLeft < gLeft) tLeft += tWidth + 50;

      tTop += "px";
      tLeft += "px";

      tooltip.css("top", tTop).css("left", tLeft).show();
    } else {
      tooltip.hide();
    }
  }
  ...
  function createItemsTable(data) {
    ...
    for (i in itemProperties) {
      ...
      var summary = "&nbsp;";
      if (property.summary) summary = $(property.summary).text();

      var namePath = [
        { content: property.name, title: property.path },
        summary,
      ]; //"<div class='perc-datatable-firstrow perc-ellipsis'>" + property.name + "</div><div class='perc-datatable-secondrow perc-ellipsis'>" + property.path + "</div>";
      ...
      if (
        ...
        lastModifiedDateDate = $.datepicker.formatDate(
          "M d, yy",
          lastModifiedDate
        );
        lastModifiedDateTime = perc_utils.formatTimeFromDate(lastModifiedDate);
      }
      var lastModifiedDateTimeAndWho =
        lastModifiedDateTime + " (" + property.lastModifier + ")";
      ...
      var percRow = {
        rowContent: [
          ...
          templateName,
          lastModified,
          property.visitsDelta,
          property.visits,
        ],
        rowData: { pageId: property.id, pagePath: property.path },
      };
    ...
    if (!isLargeColumn) percVisibleColumns = [0, 2];

    var sZeroRecords =
      errorMessage == "" ? DEFAULT_TABLE_ERROR_MESSAGE : errorMessage;
    ...
    tableConfig = {
      ...
      percStayBelow: "#perc-traffic-heading",
      percHeaders: percHeaders,
      aoColumns: aoColumns,
      percData: percData,
      percVisibleColumns: percVisibleColumns,
      iDisplayLength: selectedItemsPerPage,
    };
    ...
    var gadgetTitlebarShadowHeight = percJQuery(
      ".perc-gadget-titlebar-shadow"
    ).height();
  ...
  function createSeriesForDates(dates, values, values2, values3, values4) {
    ...
    for (d = 0; d < dates.length; d++) {
      ...
      if (values2) dataPoint.push(values2[d]);
      if (values3) dataPoint.push(values3[d]);
      if (values4) dataPoint.push(values4[d]);
  ...
  function createDatesSeriesForDates(
    dateFromStr,
    dateToStr,
    selectedGranularity
  ) {
  ...
  function calculateAdditionalDatapointCount(selectedDataPointCount) {
    ...
    var additionalDataPointCount =
      calculatedDataPointCount - selectedDataPointCount;
  ...
  function fixXaxis(datesLength) {
    ...
    if (selectedGranularity == "DAY") {
      ...
      tickDateLbl = $.datepicker.formatDate(
        TOOLTIP_DATE_FORMAT_DAY_TICK,
        tickDate
      );
    } else if (selectedGranularity == "WEEK") {
      tickDate = getStartOfWeekDate(selectedStartDate);
      tickDateLbl = $.datepicker.formatDate(
        TOOLTIP_DATE_FORMAT_WEEK_TICK,
        tickDate
      );
    } else if (selectedGranularity == "MONTH") {
      tickDate = getFirstDateInMonth(selectedStartDate);
      tickDateLbl = $.datepicker.formatDate(
        TOOLTIP_DATE_FORMAT_MONTH_TICK,
        tickDate
      );
    } else if (selectedGranularity == "YEAR") {
    ...
    firstTick.html(tickDateLbl).css("left", "0");

    // increment and fix the rest of the labels
    $.each(ticks, function () {
      var tick = $(ticks[tickCount]);
      tickCount++;
      if (tickCount > ticksLength) return 0;
  ...
  function newMenu(pageId, pagePath) {
    ...
    menu.percSimpleMenu({
      ...
      menuTitleExpanded:
        "<img src='../images/images/perc-menu-dropdown-active.png'>",
      menuTitleCollapsed:
        "<img src='../images/images/perc-menu-dropdown-inactive.png'>",
