...
$(function () {
  $(document).ready(function () {
    document.getElementById("perc-site-improve-welcome-text").innerText =
      I18N.message("perc.ui.site.improve.gadget@Welcome Text");
    document.getElementById("perc-site-improve-explanation").innerText =
      I18N.message("perc.ui.site.improve.gadget@Explanation");
    document.getElementById("perc-existing-site-improve-btn").innerText =
      I18N.message("perc.ui.site.improve.gadget@Existing Site Improve");
    document.getElementById("perc-try-site-improve-btn").innerText =
      I18N.message("perc.ui.site.improve.gadget@Try Site Improve");
    gadgets.window.setTitle(
      I18N.message("perc.ui.site.improve.gadget@SITEIMPROVE")
    );
  });
  ...
  function validateSiteConfig(parsedMetadata) {
    return (
      parsedMetadata.hasOwnProperty("doPreview") &&
      parsedMetadata.hasOwnProperty("doProduction") &&
      parsedMetadata.hasOwnProperty("doStaging") &&
      parsedMetadata.hasOwnProperty("doAssetsScanExclude")
    );
  }
    ...
    getAllSISiteConfig(function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        ...
        for (var metadatakey in results.metadata) {
          ...
          var sitename = results.metadata[metadatakey].key.replace(
            regexRemove,
            ""
          );
          ...
          if (validateSiteConfig(parsedMetadata) && sitename in siteInfos) {
            siteInfos[sitename].siteSettings.doPreview =
              parsedMetadata.doPreview;
            siteInfos[sitename].siteSettings.doProduction =
              parsedMetadata.doProduction;
            siteInfos[sitename].siteSettings.doStaging =
              parsedMetadata.doStaging;
            siteInfos[sitename].siteSettings.doAssetsScanExclude =
              parsedMetadata.doAssetsScanExclude;
            siteInfos[sitename].siteSettings.isSiteImproveEnabled =
              parsedMetadata.isSiteImproveEnabled;
            ...
            if (selectedSite === sitename) {
              ...
              $("#perc-si-exclude-assets").prop(
                "checked",
                parsedMetadata.doAssetsScanExclude
              );
              $("#perc-si-production").prop(
                "checked",
                parsedMetadata.doProduction
              );
              $("#enableSiteimprove").prop(
                "checked",
                parsedMetadata.isSiteImproveEnabled
              );
            }
  ...
  function saveSiteimproveConfig() {
    ...
    saveSISiteConfig(metadataName, siteSettings, function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        ...
        $("#save-config-status").addClass(
          "fa fa-times fa-3x site-improve-config-status-failure"
        );
        var failMessage =
          I18N.message("perc.ui.gadgets.siteimprove@Failure Save Site Config") +
          " " +
          siteName;
        ...
        // TODO:i18n
        alertDialog(
          I18N.message("perc.ui.gadgets.siteimprove@Action Required"),
          I18N.message("perc.ui.gadgets.siteimprove@Confirm Registration")
        );
      }
    });
  }
  ...
  function setSuccessIndicators() {
    ...
    $("#siteimprove-status-wrapper").attr("role", "alert");
    $("#save-config-status").addClass(
      "fa fa-check-square fa-3x site-improve-config-status-success"
    );
    $("#inactive-save-settings-site-improve-img").css("display", "none");
    $("#perc-site-improve-back-btn").prop("disabled", false);
    $("#perc-site-improve-save-btn").prop("disabled", false);
    ...
    $("#save-config-status").attr(
      "title",
      I18N.message("perc.ui.gadgets.siteimprove@Success Save Site Config") +
        " " +
        siteName
    );
  }
  ...
  function validateReturnedCredentials(parsedResults) {
    return (
      parsedResults.hasOwnProperty("sitename") &&
      parsedResults.hasOwnProperty("token") &&
      parsedResults.hasOwnProperty("canonicalDist") &&
      parsedResults.hasOwnProperty("defaultDocument") &&
      parsedResults.hasOwnProperty("siteProtocol") &&
      parsedResults.siteProtocol &&
      parsedResults.defaultDocument &&
      parsedResults.sitename in siteInfos &&
      parsedResults.token &&
      parsedResults.canonicalDist
    );
  }
  ...
  function retrieveCredentials() {
    getAllSISiteCredentials(function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        console.error(
          I18N.message("perc.ui.gadgets.siteimprove@Error Getting Credentials")
        );
      } else {
        var siteName = $("#mySites").val();
        ...
        for (var result in results.metadata) {
          ...
          if (validateReturnedCredentials(parsedResults)) {
            siteInfos[parsedResults.sitename].siteCredentials.token =
              parsedResults.token;
          }
        }
      }
    ...
    PercSISiteService.getSites(function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        ...
        for (var result in results.SiteSummary) {
          ...
          $("#mySites").append(
            "<option id='perc-si-" +
              results.SiteSummary[result].name +
              "'>" +
              results.SiteSummary[result].name +
              "</option>"
          );
  ...
  function storeCredentials(siteName, token) {
    ...
    document.cookie =
      SITEIMPROVE_COOKIE_NAME +
      "=" +
      token +
      ";path=" +
      SITEIMPROVE_COOKIE_PATH +
      "; SameSite=Lax;" +
      secure;
    ...
    saveSICredentials(
      "perc.siteimprove.credentials." + siteName,
      credentials,
      function (status, results) {
        if (status === PercSIServiceUtils.STATUS_ERROR) {
          ...
          var failMessage = I18N.message(
            "perc.ui.gadgets.siteimprove@Failed To Save"
          );
          $("#save-config-status-failure-message").removeAttr("style");
          $("#save-config-status-failure-message").text(failMessage);
          $("#inactive-save-settings-site-improve-img").css("display", "none");
  ...
  function deleteSiteImproveConfig() {
    ...
    deleteSISiteConfig(siteName, function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        ...
        // TODO:i18n
        alertDialog(
          I18N.message("perc.ui.gadgets.siteimprove@Siteimprove Disabled"),
          I18N.message(
            "perc.ui.gadgets.siteimprove@Successfully Removed Siteimprove Configuration"
          ) +
            " " +
            $("#mySites").val() +
            ". " +
            I18N.message(
              "perc.ui.gadgets.siteimprove@Please Remember To Unregister"
            )
        );
      }
    });
  ...
  function save() {
    ...
    if (existing_token === "" || IS_REFRESH_TOKEN) {
      getSIToken(siteName, function (status, results) {
        if (status === PercSIServiceUtils.STATUS_ERROR) {
          console.log(
            I18N.message("perc.ui.gadgets.siteimprove@Error Getting Token") +
              ": " +
              siteName
          );
        } else {
  ...
  function getNewToken() {
    ...
    getNewSiteImproveToken(function (status, results) {
      if (status === PercSIServiceUtils.STATUS_ERROR) {
        console.log(
          I18N.message("perc.ui.gadgets.siteimprove@Error Getting New Token")
        );
      } else {
        token = results.token;
        if (IS_REFRESH_TOKEN) {
  ...
  function refreshToken() {
    ...
    setSuccessIndicators();
    alertDialog(
      I18N.message("perc.ui.gadgets.siteimprove@Token Refreshed"),
      I18N.message("perc.ui.gadgets.siteimprove@Token Successfully Refreshed")
    );
  }
  ...
  function getSiteImproveCookie() {
    var result = document.cookie.match(
      new RegExp(SITEIMPROVE_COOKIE_NAME + "=([^;]+)")
    );
  ...
  function siteDropdown() {
    ...
    $mySiteDropdown.on("change", function () {
      ...
      if (key in siteInfos) {
        ...
        $("#perc-si-preview").prop(
          "checked",
          siteInfos[key].siteSettings.doPreview
        );
        $("#perc-si-staging").prop(
          "checked",
          siteInfos[key].siteSettings.doStaging
        );
        $("#perc-si-staging").prop(
          "checked",
          siteInfos[key].siteSettings.doAssetsScanExclude
        );
        $("#perc-si-production").prop(
          "checked",
          siteInfos[key].siteSettings.doProduction
        );
        $("#enableSiteimprove").prop(
          "checked",
          siteInfos[key].siteSettings.isSiteImproveEnabled
        );
      } else {
...
function saveSICredentials(metadataName, data, callback) {
  PercSIServiceUtils.makeJsonRequest(
    SITEIMPROVE_ROOT + TOKEN_URL,
    ...
    function (status, result) {
      if (status === PercSIServiceUtils.STATUS_SUCCESS) {
        ...
        var defaultMsg = PercSIServiceUtils.extractDefaultErrorMessage(
          result.request
        );
...
function getSIToken(siteName, callback) {
  PercSIServiceUtils.makeJsonRequest(
    SITEIMPROVE_ROOT + TOKEN_URL + "/" + siteName,
...
function getNewSiteImproveToken(callback) {
  PercSIServiceUtils.makeJsonRequest(
    SITEIMPROVE_ROOT + NEW_TOKEN_URL,
    PercSIServiceUtils.TYPE_GET,
    false,
    function (status, result) {
      if (status === PercSIServiceUtils.STATUS_SUCCESS) {
...
function saveSISiteConfig(metadataName, data, callback) {
  PercSIServiceUtils.makeJsonRequest(
    SITEIMPROVE_ROOT + PUBLISH_CONFIG,
    PercSIServiceUtils.TYPE_PUT,
    false,
    function (status, result) {
      if (status === PercSIServiceUtils.STATUS_SUCCESS) {
...
function deleteSISiteConfig(siteName, callback) {
  PercSIServiceUtils.makeDeleteRequest(
    SITEIMPROVE_ROOT + DELETE_CONFIG + siteName,
    false,
    function (status, result) {
      if (status == PercSIServiceUtils.STATUS_SUCCESS) {
        callback(PercSIServiceUtils.STATUS_SUCCESS, result.data);
