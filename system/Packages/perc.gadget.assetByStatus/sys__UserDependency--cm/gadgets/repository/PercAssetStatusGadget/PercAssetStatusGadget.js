...
(function ($) {
  ...
  function createStatusTable(data) {
    ...
    $.each(itemProperties, function (index, item) {
      ...
      var pageName =
        valuesMap.linkText != null ? valuesMap.linkText : valuesMap.sys_title;
      var assetId = item.id;

      var assetPath = !Array.isArray(item.folderPaths)
        ? item.folderPaths.replace("//Folders/$System$", "") + "/" + item.name
        : "";
      ...
      var checkBoxContent =
        "<input class='perc-table-row-checkbox' type = 'checkbox' data-perc-id='" +
        assetId +
        "'/>";
      ...
      var statusContent =
        "<span role='status' class='perc-table-row-status' data-perc-id='" +
        assetId +
        "'>" +
        itemStatus +
        "</span><span class='perc-table-row-status-icon'></span>";
      var row = {
        rowData: { assetId: assetId, assetPath: assetPath },
        rowContent: [
    ...
    var headers = [
      chkBoxContent,
      I18N.message("perc.ui.publish.title@Title"),
      "&nbsp;",
      I18N.message("perc.ui.publish.title@Status"),
      I18N.message("perc.ui.publish.title@Last Modified By"),
      I18N.message("perc.ui.asset.status.gadget@Asset Type"),
    ...
    var oLanguage = {
      sZeroRecords: I18N.message("perc.ui.asset.status.gadget@No Assets Found"),
      oPaginate: {
        sFirst: "&lt;&lt;",
        sPrevious: "&lt;",
        sNext: "&gt;",
  ...
  function approveItems(evt) {
    ...
    $.each(tableData, function () {
      ...
      var isItemChecked = tableDiv
        .find(".perc-table-row-checkbox[data-perc-id='" + this.id + "']")
        .is(":checked");
      if (isItemChecked)
        tableDiv
          .find(".perc-table-row-status[data-perc-id='" + this.id + "']")
  ...
  function updateStatus(approvalStatus) {
    ...
    if (processedItems) {
      ...
      $.each(processedItems, function () {
        ...
        var statusClass = "";
        var temp = tableDiv
          .find(".perc-table-row-status[data-perc-id='" + itemId + "']")
    ...
    switch (approvalStatus.status) {
      ...
      case "FAILED":
        perc_utils.alert_dialog({
          title: I18N.message("perc.ui.labels@Error"),
          content:
            "Unexpected error occured while approving the selected items.",
          id: "perc-no-items-selected-message",
        });
  ...
  $(document).ready(function () {
    var searchConfig;
    var labels = {
      assetType: I18N.message("perc.ui.asset.status.gadget@Asset Type"),
      workflow: I18N.message("perc.ui.gadgets.workflowStatus@Workflow"),
      state: I18N.message("perc.ui.gadgets.workflowStatus@Status"),
      modifiedby: I18N.message("perc.ui.asset.status.gadget@Last Edited by"),
    };
    var defaultWorkflow;
      ...
      var searchConfig = {
        ...
        workflow:
          defaultWorkflow != null
            ...
            : {
                name: I18N.message(
                  "perc.ui.asset.status.gadget@Default Workflow"
                ),
                value: "6",
              },
        state: {
                  ...
                  if (data != null && typeof data !== "undefined") {
                    ...
                    if (dataObj.userprefs["mid_" + moduleId] != null) {
                      ...
                      if (metaPrefsObj.assetType != null) {
                        newSearchCriteria.assetType = {
                          name: metaPrefsObj.assetType,
                          value: metaPrefsObj.assetType,
                        };
                      }
    ...
    function loadTable(searchConfig) {
      ...
      var assetName =
        assetType === "" || assetType === "@all"
          ? I18N.message("perc.ui.asset.status.gadget@All Assets")
          : WidgetLabelByContentTypeId[assetType]
          ? "(" + WidgetLabelByContentTypeId[assetType] + ")"
          : "";
      var workflowName =
        searchConfig.workflow.name === "" ||
        searchConfig.workflow.name === "All"
          ? I18N.message("perc.ui.asset.status.gadget@All Workflows")
          : searchConfig.workflow.name;
      var stateName =
        searchConfig.state.name === "" || searchConfig.state.name === "All"
          ? I18N.message("perc.ui.asset.status.gadget@All States")
