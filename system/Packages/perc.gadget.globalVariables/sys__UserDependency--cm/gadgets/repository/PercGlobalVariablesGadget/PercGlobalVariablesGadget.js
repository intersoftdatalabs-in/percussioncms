...
(function ($) {
  ...
  function _editVariable(event) {
    ...
    var readOnlyStyle =
      origName == ""
        ? ""
        : "background-color:#E6E6E9; border: none; font-weight:bold; overflow:hidden; text-overflow:ellipsis;";
    var dialogHtml =
      '<div><div id="perc-name-label"><label for="perc_variable_name">Name:</label></div>' +
      '<div><input type="text" style="width: 300px;' +
      readOnlyStyle +
      '" id="perc_variable_name" maxlength="110" name="perc_variable_name" value="' +
      rowData.varName +
      '"' +
      readOnly +
      "></div>" +
    ...
    var dialog = percJQuery(dialogHtml).perc_dialog({
      ...
      open: function () {
        percJQuery(this).find("#perc_variable_value").val(rowData.varValue);
      },
    ...
    percJQuery.perc_filterField(
      dialog.find("#perc_variable_name"),
      perc_filters.ID_WITH_SPACE
    );
  }
  ...
  function _saveVariable(origName, varName, varValue, dialog) {
    if (varName.trim() == "") {
      displayErrorMessage(
        dialog,
        I18N.message("perc.ui.global.variables.gadget@No Blank Name"),
        "#perc_variable_name"
      );
      return;
    }
    ...
    if (varValue.trim() == "") {
      displayErrorMessage(
        dialog,
        I18N.message("perc.ui.global.variables.gadget@No Blank Value"),
        "#perc_variable_value"
      );
      return;
    }
      ...
      if (origName === "") {
        if (varData[varName]) {
          displayErrorMessage(
            dialog,
            I18N.message(
              "perc.ui.global.variables.gadget@Select Different Name"
            ),
            "#perc_variable_name"
          );
          return;
    ...
    mdService.saveGlobalVariables(
      ...
      function (status, result) {
        if (status == PercServiceUtils.STATUS_SUCCESS) {
          ...
          alertDialog(
            "Error",
            I18N.message("perc.ui.global.variables.gadget@Variable error")
          );
        }
      }
    );
  ...
  function _deleteVariable(event) {
    ...
    var options = {
      title: I18N.message(
        "perc.ui.global.variables.gadget@Delete Global Variable"
      ),
      question:
          ...
          function (status, result) {
            if (status === PercServiceUtils.STATUS_SUCCESS) {
              ...
              alertDialog(
                "Error",
                I18N.message(
                  "perc.ui.global.variables.gadget@Error Delete Variable"
                )
              );
            }
          }
  ...
  function _renderGlobalVariablesTable(data) {
    $.PercGlobalVariableActions = {
      title: "",
      menuItemsAlign: "left",
      stayInsideOf: ".dataTables_wrapper",
      items: [
        {
          label: I18N.message("perc.ui.gadgets.licenseMonitor@Edit"),
          callback: _editVariable,
        },
        {
          label: I18N.message("perc.ui.dashboard@Delete"),
          callback: _deleteVariable,
        },
      ],
    };
    var menus = [];
    ...
    if (data) {
      ...
      $.each(varData, function (name, value) {
        ...
        var row = {
          rowData: { varName: name, varValue: value },
          rowContent: dataRow,
        };
        dataRows.push(row);
    ...
    var config = {
      ...
      percHeaders: [
        I18N.message("perc.ui.global.variables.gadget@Variable name"),
        I18N.message("perc.ui.global.variables.gadget@Variable value"),
      ],
      aoColumns: [
        {
          sType: "string",
  ...
  function displayErrorMessage(dialog, message, inputField) {
    ...
    var errorLabelHtml =
      "<label style='display: block;' class='perc_field_error'>" +
      message +
      "</label>";
