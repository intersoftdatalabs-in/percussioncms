...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta
		name="directoryWidgetDropDownControl" dimension="single"
		choiceset="none">
		...
		<psxctl:ParamList>
			...
			<psxctl:Param name="organizationxmlurl"
				datatype="String" paramtype="generic">
			...
			<psxctl:Param name="departmentparentxmlurl"
				datatype="String" paramtype="generic">
				<psxctl:Description>This parameter specifies the url location of a
					lookup application conforming to the sys_Lookup.dtd. The current
					form values are sent to the query and are available for use in
			...
			<psxctl:Param name="departmentxmlurl" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies the url location of a
					lookup application conforming to the sys_Lookup.dtd. The current
					form values are sent to the query and are available for use in
		...
		<psxctl:AssociatedFileList>
			<psxctl:FileDescriptor
				name="directoryDropdown.css" type="css" mimetype="text/css">
				<psxctl:FileLocation>../rx_resources/widgets/percDirectory/css/directoryDropdown.css</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="directoryDropdown.js" type="script" mimetype="text/javascript">
	...
	<xsl:template
		match="Control[@name='directoryWidgetDropDownControl']"
		mode="psxcontrol">
		<xsl:variable name="organizationxmlurl">
			<xsl:choose>
				<xsl:when
					test="ParamList/Param[@name='organizationxmlurl']">
					<xsl:value-of
						select="ParamList/Param[@name='organizationxmlurl']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='directoryWidgetDropDownControl']/psxctl:ParamList/psxctl:Param[@name='organizationxmlurl']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="departmentparentxmlurl">
			<xsl:choose>
				<xsl:when
					test="ParamList/Param[@name='departmentparentxmlurl']">
					<xsl:value-of
						select="ParamList/Param[@name='departmentparentxmlurl']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='directoryWidgetDropDownControl']/psxctl:ParamList/psxctl:Param[@name='departmentparentxmlurl']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="departmentxmlurl">
			<xsl:choose>
				<xsl:when test="ParamList/Param[@name='departmentxmlurl']">
					<xsl:value-of
						select="ParamList/Param[@name='departmentxmlurl']" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="document('')/*/psxctl:ControlMeta[@name='directoryWidgetDropDownControl']/psxctl:ParamList/psxctl:Param[@name='departmentxmlurl']/psxctl:DefaultValue" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
        ...
        // ]]>
			...
			<xsl:call-template name="jsVar">
				<xsl:with-param name="name"
					select="'organizationxmlurl'" />
				<xsl:with-param name="value"
					select="$organizationxmlurl" />
			</xsl:call-template>
			<xsl:call-template name="jsVar">
				<xsl:with-param name="name"
					select="'departmentparentxmlurl'" />
				<xsl:with-param name="value"
					select="$departmentparentxmlurl" />
			</xsl:call-template>
			<xsl:call-template name="jsVar">
				<xsl:with-param name="name"
					select="'departmentxmlurl'" />
				<xsl:with-param name="value"
					select="$departmentxmlurl" />
			</xsl:call-template>
			<xsl:call-template name="jsVar">
		...
		<div id="maindiv-{@paramName}">
			<select id="{@paramName}" class="datadisplay"
				...
				<xsl:call-template name="parametersToAttributes">
					<xsl:with-param name="controlClassName"
						select="'directoryWidgetDropDownControl'" />
	...
	<xsl:template
		match="Control[@name='directoryWidgetDropDownControl' and @isReadOnly='yes']"
		...
		<div id="maindiv-{@paramName}">
			<div id="{@paramName}" class="datadisplay" name="{@paramName}"
				value="{Value}">
				<xsl:if test="@accessKey!=''">
					<xsl:attribute name="accesskey">
                        <xsl:call-template
