...
$(document).ready(function () {
  ...
  gadgets.window.setTitle(
    I18N.message("perc.ui.gadgets.sitewideFramework@Sitewide Framework")
  );
});
  ...
  getSitesDeferred.done(function (sitesObject) {
    ...
    Promise.all(getSitePropertiesDeferred).then(function () {
      ...
      for (var i = 0; i < siteProperties.length; i++) {
        var index = siteProperties.findIndex(
          (v) => typeof v.name === "undefined" || v.name === null
        );
        if (index > -1) {
      ...
      siteProperties.sort((a, b) =>
        (a.name != null ? a.name.toLowerCase() : "") >
        (b.name != null ? b.name.toLowerCase() : "")
          ? 1
          : -1
      );
...
function renderFrameworkSettings(sitePropertiesData) {
  ...
  $(sitePropertiesData).each(function (index, site) {
    ...
    sitePropertiesJsonString = sitePropertiesJsonString.replace(
      /[\"&'\/<>]/g,
      function (a) {
        return {
          '"': "&quot;",
    ...
    gadgetHtml = `<div style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-row">
                        <div style="font-family: 'Open Sans',sans-serif" id="perc-site-framework-container-${
                          site.name
                        }" class="perc-site-framework-container">
                          <h4 style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-site-name">${
                            site.name
                          }</h4>
                          <div style="font-family: 'Open Sans',sans-serif" data-perc-site-properties='${sitePropertiesJsonString}' class="perc-framework-contents">
                            <div  style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-checkbox perc-site-framework-foundation-override">
                              <input type="checkbox" id="perc-foundation-override-${
                                site.name
                              }" name="foundation-override-${site.name}" ${
      site.overrideSystemFoundation === true ? "checked" : ""
    }>
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-foundation-override-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@Override Percussion Foundation"
    )}</label>
                            </div>
                            <div style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-checkbox perc-site-framework-jquery-override">
                              <input type="checkbox" id="perc-jquery-override-${
                                site.name
                              }" name="jquery-override-${site.name}" ${
      site.overrideSystemJQuery === true ? "checked" : ""
    }>
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-jquery-override-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@Override Percussion jQuery"
    )}</label>
                            </div>
                            <div  style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-checkbox perc-site-framework-jqueryui-override">
                              <input type="checkbox" id="perc-jqueryui-override-${
                                site.name
                              }" name="jqueryui-override-${site.name}" ${
      site.overrideSystemJQueryUI === true ? "checked" : ""
    }>
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-jqueryui-override-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@Override Percussion jQuery UI"
    )}</label>
                            </div>
                            <div  style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-textarea-container perc-site-framework-additional-head-content-container">
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-site-framework-additional-head-content-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@Additional Head Content"
    )}</label>
                              <textarea class="perc-site-framework-textarea perc-site-framework-additional-head-content">${
                                site.siteAdditionalHeadContent == null
                                  ? ""
                                  : site.siteAdditionalHeadContent
                              }</textarea>
                            </div>
                            <div style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-textarea-container perc-site-framework-after-body-open-content-container">
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-site-framework-after-body-open-content-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@After Body Open Content"
    )}</label>
                              <textarea class="perc-site-framework-textarea perc-site-framework-after-body-open-content">${
                                site.siteAfterBodyOpenContent == null
                                  ? ""
                                  : site.siteAfterBodyOpenContent
                              }</textarea>
                            </div>
                            <div style="font-family: 'Open Sans',sans-serif" class="perc-site-framework-textarea-container perc-site-framework-before-body-close-content-container">
                              <label style="font-family: 'Open Sans',sans-serif" for="perc-site-framework-before-body-close-content-${
                                site.name
                              }">${I18N.message(
      "perc.ui.gadgets.sitewideFramework@Before Body Close Content"
    )}</label>
                              <textarea class="perc-site-framework-textarea perc-site-framework-before-body-close-content">${
                                site.siteBeforeBodyCloseContent == null
                                  ? ""
                                  : site.siteBeforeBodyCloseContent
                              }</textarea>
                            </div>
                          </div>
                        </div>
...
function renderSubmit() {
  submitHtml = `<div style="font-family: 'Open Sans',sans-serif" class="perc-sitewide-framework-submit-container">
                    <button id="percSitewideFrameworkSubmit" class="perc-submit-sitewide-framework btn btn-primary">${I18N.message(
                      "perc.ui.common.label@Submit"
                    )}</button>
                  </div>`;
...
function showSitewideWarningDialog(event) {
  errorDialogHtml = `<div><h4>${I18N.message(
    "perc.ui.gadgets.sitewideFramework@Notfication detail"
  )}</h4><br></div>`;
...
function showSitewideErrorDialog(errorList) {
  errorDialogHtml = `<div><h4>${I18N.message(
    "perc.ui.gadgets.sitewideFramework@Error detail"
  )}</h4><br>`;
...
function collectFrameworkUpdates() {
  ...
  $(".perc-framework-contents").each(function () {
    ...
    frameworkObject.SiteProperties.overrideSystemFoundation = $(
      `#perc-foundation-override-` + siteName1
    ).prop("checked")
      ? true
      : false;
    frameworkObject.SiteProperties.overrideSystemJQuery = $(
      `#perc-jquery-override-` + siteName1
    ).prop("checked")
      ? true
      : false;
    frameworkObject.SiteProperties.overrideSystemJQueryUI = $(
      `#perc-jqueryui-override-` + siteName1
    ).prop("checked")
      ? true
      : false;
    frameworkObject.SiteProperties.siteAdditionalHeadContent =
      $this.find(".perc-site-framework-additional-head-content").val() == ""
        ? null
        : $this.find(".perc-site-framework-additional-head-content").val();
    frameworkObject.SiteProperties.siteAfterBodyOpenContent =
      $this.find(".perc-site-framework-after-body-open-content").val() == ""
        ? null
        : $this.find(".perc-site-framework-after-body-open-content").val();
    frameworkObject.SiteProperties.siteBeforeBodyCloseContent =
      $this.find(".perc-site-framework-before-body-close-content").val() == ""
        ? null
        : $this.find(".perc-site-framework-before-body-close-content").val();
  ...
  $(frameworkUpdates).each(function (index, siteProps) {
    ...
    $.PercSiteService.updateSiteProperties(
      ...
      function (status, result) {
        completedUpdatesDeferrred[index].resolve({
          status: status,
          result: result,
        });
      }
