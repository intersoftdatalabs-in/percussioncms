  ...
  function createIndicator(elem, label) {
    ...
    function update(usage, limit) {
      ...
      function updateUsageLabel() {
        ...
        indicator
          .find(".usage-numbers")
          .html(usage.toString() + " of " + limitLabel);
      }

      // Updates and re-render the corresponding usage bar
      function updateUsageBar() {
        var excessBar, excessBarWidth;
        ...
        if (isLimited) {
          ...
          usageBar = $(
            '<div class="usage ' +
              cssClassNameOfPercentage(percentage) +
              '"></div>'
          );
          ...
          if (percentage > 100) {
            ...
            indicator
              .find(".bar-container")
              .append(excessBar)
              .append('<div class="bar-exceeded-end"></div>');
    ...
    if (label === undefined) {
      var indicatorPlaceholderMarkup =
      ...
      indicator.addClass("empty").html(indicatorPlaceholderMarkup);
    } else {
      var indicatorNormalMarkup =
        label + ': <span class="usage-numbers">0</span>';
