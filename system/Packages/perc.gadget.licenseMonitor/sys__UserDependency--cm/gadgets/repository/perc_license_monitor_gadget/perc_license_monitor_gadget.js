...
(function ($) {
  ...
  function refreshGadgetStatus(event) {
    ...
    var refreshButton = $(".perc-lmg-refresh").addClass("disabled"),
      loadingMsg = $(".perc-lmg-refresh-info")
        .find(".loading")
    ...
    function onAbortGetStatus(status) {
      ...
      percJQuery.perc_utils.alert_dialog({
        ...
        content: I18N.message(
          "perc.ui.gadgets.licenseMonitor@License Could Not Be Refreshed"
        ),
      });
      beforeRefresh();
    }
  ...
  function createRefreshButton() {
    ...
    var refreshButton = $(
      `<a href="#" class="perc-lmg-button perc-lmg-refresh hidden" alt="${I18N.message(
        "perc.ui.gadgets.licenseMonitor@Refresh"
      )}">${I18N.message("perc.ui.gadgets.licenseMonitor@Refresh")}</a>`
    );
  ...
  function updateGadgetInfo(licenseStatus) {
    ...
    function updateModuleLicenseInformation() {
      $.perc_module_license_manager.getModuleLicensesInfo(function (
        status,
        data
      ) {
        if (!status) {
          $("#perc-lmg-module-license-info").append(
            $("<div class='perc-module-license-error'/>").text(data)
          );
          return;
        }
        $("#perc-lmg-module-license-info")
          .empty()
          .append(
            $.perc_module_license_manager.generateLicenseView(data.combinedInfo)
          );

        $("#perc-lmg-module-license-info").addClass(
          "perc-module-licenses-refreshed"
        );
        if (isUserAdmin) {
          showModuleLicenseButton(
    ...
    function updateRefreshInformation() {
      ...
      if (
        licenseStatus[constants.FIELDNAMES.LASTREFRESHDATE] !== undefined &&
        licenseStatus[constants.FIELDNAMES.LASTREFRESHDATE] !== ""
      ) {
        // Parse and format the date
        var lastRefreshDate = $.perc_utils.splitDateTime(
          licenseStatus[constants.FIELDNAMES.LASTREFRESHDATE]
        );

        refreshDateInfo
          .removeClass("nodisplay")
          .find("span.last-refresh-date-info-span")
          .html(lastRefreshDate.date + " " + lastRefreshDate.time);
