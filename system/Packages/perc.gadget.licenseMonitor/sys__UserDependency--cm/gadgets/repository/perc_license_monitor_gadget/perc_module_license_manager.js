...
(function ($) {
  ...
  $.perc_module_license_manager = {
    ...
    SERVICE_NOT_AVAILABLE_MSG: I18N.message(
      "perc.ui.gadgets.licenseMonitor@Licensing Service Not Available"
    ),
    AJAX_TIMEOUT: 10000,
  };
  ...
  function activateModuleLicense(type, key, callback) {
    var handshakeUrl =
      $.perc_module_license_manager.LICENSING_BASE_URL +
      $.perc_module_license_manager.HANDSHAKE_URL;
    ...
    $.ajax({
      ...
      .fail(function (jqXhr, textstatus, errorMessage) {
        errorMessage = errorMessage
          ? errorMessage
          : I18N.message(
              "perc.ui.gadgets.licenseMonitor@Failed to activate the license"
            );
  ...
  function _getModuleLicenses(callback) {
    ...
    $.PercLicenseService.getAllModuleLicenses(function (status, data) {
      if (!status) {
        ...
        $.perc_module_license_manager.LICENSING_BASE_URL =
          data.moduleLicenses.licenseServiceUrl;
  ...
  function _getModuleLicenseTypes(callback) {
    var typesUrl =
      $.perc_module_license_manager.LICENSING_BASE_URL +
      $.perc_module_license_manager.TYPES_URL;
    $.ajax({
      type: "GET",
      url: typesUrl,
      ...
      .fail(function (jqXhr, textstatus, errorMessage) {
        errorMessage = errorMessage
          ? errorMessage
          : I18N.message(
              "perc.ui.gadgets.licenseMonitor@Failed to get available modules"
            );
        callback(false, errorMessage);
      });
  }
  ...
  function _validateModuleLicenses(payLoad, callback) {
    var validityUrl =
      $.perc_module_license_manager.LICENSING_BASE_URL +
      $.perc_module_license_manager.VALIDITY_URL;
    $.ajax({
      type: "POST",
      url: validityUrl,
      contentType: "application/json",
      ...
      .fail(function (jqXhr, textstatus, errorMessage) {
        errorMessage = errorMessage
          ? errorMessage
          : I18N.message(
              "perc.ui.gadgets.licenseMonitor@Failed to get validated modules"
            );
        callback(false, errorMessage);
      });
  }
  ...
  function generateLicenseView(licenseInfo, newLicenseInfo) {
    ...
    $.each(licenseInfo, function () {
      ...
      if (curLI.label != "Page Optimizer") {
        var licInfoElem = $("<div/>");
        $("<span/>")
          .addClass("perc-lmg-modulelicense-label")
          .text(curLI.label)
          .appendTo(licInfoElem);
        var msg = curLI.isValid
          ? " : " + I18N.message("perc.ui.gadgets.licenseMonitor@Active")
          : " : " + curLI.message + " ";
        $("<span/>")
          .addClass("perc-lmg-modulelicense-message")
          .html(msg)
          .appendTo(licInfoElem);
        var iconClass = curLI.isValid ? "status-active" : "status-warning";
        $("<span/>", { class: "perc-lmg-icon-status" })
          .addClass(iconClass)
          .attr("for", curLI.name)
          .appendTo(licInfoElem);
  ...
  function generateLicenseActivatorView(
    licenseTypes,
    activationSuccessCallback
  ) {
    var licActElemCont = $("<div/>").addClass("perc-lmg-modulelicense-act-top");
    var typeComboBox = $("<select/>", {
      id: "perc-lmg-modulelicense-type-list",
    });
    $.each(licenseTypes, function () {
      ...
      if (this.id !== "PAGE_OPTIMIZER") {
        $("<option/>")
          .attr("value", this.id)
          .text(this.label)
          .appendTo(typeComboBox);
      }
    });

    licActElemCont
      .append(typeComboBox)
      .append(`<input name='perc-lmg-modulelicense-key'/>`)
      .append(
        `<button aria-label="Activate License" id="activate-license-button" class="btn btn-primary advanced">${I18N.message(
          "perc.ui.gadgets.licenseMonitor@Activate"
        )}</button>`
      );
    licActElemCont.append(
      "<div id='perc-lmg-modulelicense-key-error' style='display:none; color:red'/>"
    );
    ...
    licActElemCont.find("#activate-license-button").on("click", function () {
      ...
      if (key.length < 1) {
        errElem
          .text(
            I18N.message(
              "perc.ui.gadgets.licenseMonitor@Please enter a valid key"
            )
          )
          .show();
        return;
      ...
      activateModuleLicense(type, key, function (status, result) {
        ...
        if (!handshake.valid) {
          var message = handshake.message
            ? handshake.message
            : I18N.message(
                "perc.ui.gadgets.licenseMonitor@License activation failed"
              );
        ...
        saveModuleLicense(result, function (status, result) {
          ...
          if (!status) {
            var message = handshake.message
              ? handshake.message
              : I18N.message(
                  "perc.ui.gadgets.licenseMonitor@Failed to save the activated license"
                );
            errElem.text(message).show();
            return;
          }
