...
(function ($) {
  ...
  function createActivateButton(
    ...
    var activateButton = percJQuery(
      `<a href="#" class="perc-lmg-activate" alt="${I18N.message(
        "perc.ui.gadgets.licenseMonitor@Activate"
      )}">${I18N.message("perc.ui.gadgets.licenseMonitor@Activate")}</a>`
    ).on("click", function (event) {
      event.preventDefault();
      _createActivationDialog(
        _onDialogCloseCallback,
        _onSuccessCallback,
        _onErrorCallback,
        false
  ...
  function createEditButton(
    ...
    var editButton = percJQuery(
      `<a class="perc-lmg-edit" alt="${I18N.message(
        "perc.ui.gadgets.licenseMonitor@Edit"
      )}" href="#">${I18N.message("perc.ui.gadgets.licenseMonitor@Edit")}</a>`
    ).on("click", function (event) {
      event.preventDefault();
      _createActivationDialog(
        _onDialogCloseCallback,
   ...
   * @param callback function to invoke when the edit is erroneous
   */
  function createModuleLicenseEditButton(
    onDialogCloseCallback,
    cloudInfo,
    combinedInfo
  ) {
    var editButton = percJQuery(
      ...
      event.preventDefault();
      _createModuleLicenseActivationDialog(
        onDialogCloseCallback,
        cloudInfo,
        combinedInfo
      );
   ...
   */
  function _createActivationDialog(
    _onDialogCloseCallback,
    _onSuccessCallback,
    _onErrorCallback,
    isEdit
    ...
    onSuccessCallback = _onSuccessCallback;
    onErrorCallback = _onErrorCallback;
    var infoText = I18N.message(
      "perc.ui.gadgets.licenseMonitor@Activation Dialog"
    );
    ...
    dialogContent +=
      '<div style="margin-bottom:20px;"><label>' + infoText + "</label></div>";
    dialogContent += '<form id="perc-lmg-activation-form" action="">';
    dialogContent += "<div>";
    dialogContent +=
      '<label for="perc-lmg-activation-code">' +
      I18N.message("perc.ui.gadgets.licenseMonitor@Activation code") +
      ":" +
      "</label><br/>";
    dialogContent +=
      '<input type="text" name="perc-lmg-activation-code" id="perc-lmg-activation-code" maxlength="255" /><br/>';
    ...
    var dlgTitle = isEdit
      ? I18N.message("perc.ui.gadgets.licenseMonitor@Edit Activated License")
      : I18N.message("perc.ui.gadgets.licenseMonitor@Activate License");
  ...
  function _createModuleLicenseActivationDialog(
    _onDialogCloseCallback,
    cloudInfo,
    combinedInfo
  ) {
    ...
    var dialog;
    var infoText = I18N.message(
      "perc.ui.gadgets.licenseMonitor@Module License Activation Dialog"
    );
    // Set the basic markup for the dialog and assign it to the module's dialog variable
    var dialogContent;
    var content;
    ...
    content
      .find("#perc-module-license-display-container")
      .append($.perc_module_license_manager.generateLicenseView(combinedInfo));
    content
      .find("#perc-module-license-activate-container")
      .append(
        $.perc_module_license_manager.generateLicenseActivatorView(
          cloudInfo.licenseTypes,
          activationSuccessCallback
        )
      );
    buttons = {
    ...
    var dlgTitle = I18N.message(
      "perc.ui.gadgets.licenseMonitor@Edit Module Licenses"
    );
    ...
    function activationSuccessCallback(newCombinedInfo) {
      ...
      content
        ...
        .empty()
        .append(
          $.perc_module_license_manager.generateLicenseView(combinedInfo)
        );
    }
  }
  /**
  ...
  function submitActivationCode(event) {
     ...
     * Netsuite).
     */
    function onSuccessfulActivate(data) {
    ...
    function onAbortedActivate(status) {
      percJQuery.perc_utils.alert_dialog({
        ...
        content: I18N.message(
          "perc.ui.gadgets.licenseMonitor@License Could Not Be Activated"
        ),
      });
      beforeActivate();
    }
    ...
    activationCode = activationForm
      .find("#perc-lmg-activation-code")
      .val()
      .trim();
    ...
    if (activationCode === "") {
      errorLabel
        ...
        .html(
          I18N.message(
            "perc.ui.gadgets.licenseMonitor@Activation code field is required"
          )
        );
      return false;
    }
  ...
  function invokeLicenseService(
    ...
    $.PercLicenseService[serviceName](function (status, data) {
      ...
      if (commonCallback !== undefined) commonCallback(data);
  ...
  function createResolveButton() {
    var resolveDialogTitle = I18N.message(
        "perc.ui.gadgets.licenseMonitor@Resolve Alerts"
      ),
      resolveDialogMessage,
      resolveButton;

    resolveDialogMessage = I18N.message(
      "perc.ui.gadgets.licenseMonitor@Resolve Dialog"
    );

    resolveButton = percJQuery(
      `<a class="perc-lmg-button perc-lmg-resolve hidden" alt="${I18N.message(
        "perc.ui.gadgets.licenseMonitor@Resolve"
      )}" href="#">${I18N.message(
        "perc.ui.gadgets.licenseMonitor@Resolve"
      )}</a>`
    ).on("click", function (event) {
