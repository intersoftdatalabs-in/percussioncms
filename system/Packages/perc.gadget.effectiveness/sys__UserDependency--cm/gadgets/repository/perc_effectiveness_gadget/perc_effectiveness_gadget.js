...
(function ($) {
  ...
  function renderGraph(site, durationType, durationValue, usage, threshold) {
    ...
    PercActivityService.getContentEffectiveness(
      ...
      function (status, data) {
        if (status === PercServiceUtils.STATUS_SUCCESS) {
          ...
          $("#perc-chart-error").append(
            '<div  class = "effectiveness-error-message" >' + data + "</div>"
          );
        }
    ...
    if (graphData) {
      ...
      plot1 = $.jqplot("perc-chart", [line1, line2], {
        ...
        highlighter: {
          ...
          tooltipAxes: "x",
          formatString:
            '<table class="jqplot-highlighter"><tr><td>Goal:</td><td>%s</td></tr></table>',
        },
      ...
      if (!errorMessagePresence) {
        $("#perc-chart-error").append(
          '<div class = "effectiveness-error-message">No Data Found</div>'
        );
      }
    }
  ...
  function generateBreadcrumbs(path) {
    ...
    if (siteName == "@all") {
      $("#perc-chart-breadcrumb-list").append(
        ' <li atitle = "" ><span id = "perc-breadcrumb-items" atitle = "">All Sites</span></li><span class = "perc-breadcrumb-separator" >&nbsp;>&nbsp;</span>'
      );
    }
    ...
    $("#perc-chart-breadcrumb-list").append(
      '<li atitle = "' +
        breadcrumbItems[0] +
        '" ><span id = "perc-breadcrumb-items" atitle = "' +
        breadcrumbItems[0] +
        '">' +
        breadcrumbItems[0] +
        "</span></li>"
  ...
  $(document).on("click", "#perc-breadcrumb-items", function () {
    var parentFolder =
      path.indexOf("/") === path.length - 1 ? path : path + "/";
