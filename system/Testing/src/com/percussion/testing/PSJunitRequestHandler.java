...
/**
 * This loadable handler instantiantes a JUnit class and execute the test suite in the context of
 * the Rx server.
 * 
 * The class can use it's own class loader to instance requested classes so no stopping/starting of
 * the server is needed when a change to the 'testing' class is made. The class loader is given a
 * root path, and root directory of classes to load fresh each time. This is controlled w/ the
 * useLocalClassLoader html parameter. By default, this is disable. Set it to true to enable this
 * feature.
 * 
...
public class PSJunitRequestHandler implements IPSLoadableRequestHandler {
   /**
    * see {@link IPSLoadableRequestHandler} interface for description.
    */
   public void init(Collection requestRoots, InputStream cfgFileIn) throws PSServerException {
      ...
      PSConsole.printMsg(HANDLER, "JUnit testing handler initialized.");
   }

   /**
    * See {@link IPSLoadableRequestHandler} interface for description.
    * 
    * This method will instantiate a class specified in the request and execute the specified
    * TestCase by invoking the <code>suite</code> method.
    * <p>
    * The following parameters are expected.
    * 
    * <table border="0">
    * <tr>
    * <td>The class to run:</td>
    * <td><code>HTML_PARAM_EXE</code></td>
    * </tr>
    * <tr>
    * <td>Class Loader will load any classes in or below:</td>
    * <td><code>HTML_PARAM_DIR</code></td>
    * </tr>
    * <tr>
    * <td>Classes directory:</td>
    * <td><code>HTML_PARAM_ROOT</code></td>
    * </tr>
    * <tr>
    * <td></td>
    * </tr>
   ...
   public void processRequest(PSRequest request) {
      ...
      Element root = PSXmlDocumentBuilder.createRoot(m_responseDoc, "Response");
      ...
      try {
         ...
         boolean useLocalClassLoader =
               request.getParameter(HTML_PARAM_LOCAL_LOADER, "").equalsIgnoreCase("true");

         if (strClassName.trim().length() == 0)
            throw new IllegalArgumentException(HTML_PARAM_EXE + " html param must be supplied");

         if (useLocalClassLoader && strRootDev.trim().length() == 0)
            throw new IllegalArgumentException(HTML_PARAM_ROOT + " html param must be supplied");
         ...
         try {
            if (strClassName.trim().length() > 0) {
               ...
               Class junitTest = Class.forName(strClassName, true, classLoader);
               ...
               Element testResultEl =
                     createTestResultResponse(m_responseDoc, result, strClassName, outputText);
               root.appendChild(testResultEl);
               System.out.println(outputText);
               PSXmlDocumentBuilder.toString(m_responseDoc);
            } else {
               PSXmlDocumentBuilder.addElement(m_responseDoc, root, CLASSNOTFOUNDOREMPTY,
            ...
            /*
             * In order to re-load a class that uses a dll, the original class loader must be
             * removed from memory because the class maintains a list of already loaded dlls and
             * throws an exception if you try to load one again in another instance of the class.
             */
            if (useLocalClassLoader) {
               classLoader = null;
               System.gc();
   ...
   /**
    * Creates a class loader that is responsible for loading all requested classes.
    * 
    * @param strDir directory string. Never <code>null</code> or empty.
    * 
    * @param strRootName directory of classes that may be requested of the class loader to create.
    *        Never <code>null</code> or empty.
    * 
    * @return class loader that loads a class fresh each time it is requested if it appears in or
    *         below <code>strDir</code>. Never <code>null</code>.
    */
   private RefreshClassLoader createClassLoader(String strDir, String strRootName) {
      // Threshold
      if (strDir == null || strRootName == null)
         throw new IllegalArgumentException("strDir and strRootName must not be null");

      // Threshold
      if (strDir.trim().length() == 0 || strRootName.trim().length() == 0)
         throw new IllegalArgumentException("strDir and strRootName must not be empty");
   ...
   /**
    * Recursively loads all files and children into a list provided.
    * 
   ...
   private void addAllFiles(File f, List<File> l) {
      if (f == null || l == null)
         throw new IllegalArgumentException("Both f and l must not be null");
   ...
   /**
    * Based on a {@link junit.framework.TestResult} it creates a xml node.
    * 
    * @param doc xml document. Never <code>null</code>.
    * 
    * @param tr Junit test result. Never <code>null</code>.
    * 
    * @param strMethod the method that was run. Never <code>null</code> or empty.
    * 
    * @todo define any xml node names that may be needed in definition ...
    */
   private Element createTestResultResponse(Document doc, TestResult tr, String strMethod,
         String consoleOutput) {
      // Thresholds
      if (doc == null || tr == null || strMethod == null)
         throw new IllegalArgumentException("doc, tr and strMethod must not be null");

      if (strMethod.trim().length() == 0)
         throw new IllegalArgumentException("strMethod must not be empty");
      ...
      if (tr.wasSuccessful()) {
         ...
         while (errors.hasMoreElements()) {
            Element errorEl = PSXmlDocumentBuilder.addEmptyElement(doc, testResultEl, TESTERROR);
            ...
            if (o instanceof TestFailure) {
               ...
               PSXmlDocumentBuilder.addElement(doc, errorEl, "Message", t.getMessage());

               Writer stackTrace = new StringWriter();
               t.printStackTrace(new PrintWriter(stackTrace));
               PSXmlDocumentBuilder.addElement(doc, errorEl, "Trace", stackTrace.toString());
            }
         }
         ...
         while (failures.hasMoreElements()) {
            Element failureEl =
                  PSXmlDocumentBuilder.addEmptyElement(doc, testResultEl, TESTFAILURE);
            ...
            if (o instanceof TestFailure) {
               ...
               Throwable t = (Throwable) tf.thrownException();
               PSXmlDocumentBuilder.addElement(doc, failureEl, "Message", t.getMessage());

               Writer stackTrace = new StringWriter();
               t.printStackTrace(new PrintWriter(stackTrace));
               PSXmlDocumentBuilder.addElement(doc, failureEl, "Trace", stackTrace.toString());
            }
         }
      }
      /*
       * The purpose of this line is to extract the time from the beginning of the console output.
       * We don't want the rest of the output because what is printed is equivalent to what we are
   ...
   /**
    * see {@link com.percussion.server.PSLoadableRequestHandler} interface for description.
    */
   ...
   /**
    * Inner class loader class appender to listen for log events and embed them in an xml response
    * doc.
    */
   private class XmlDocAppender extends AppenderSkeleton {
      ...
      public void append(LoggingEvent event) {
         ...
         /*
          * // Threshold if (m_responseDoc == null) return;
          * 
          * Object o = event.getMessage();
          * 
          * // Threshold if (o == null) return;
          * 
          * Element parent = (Element) m_responseDoc.getFirstChild();
          * 
          * if (parent == null) { parent = PSXmlDocumentBuilder.createRoot(m_responseDoc,
          * "Response"); }
          * 
          * String strMsg = ""; Element aLogEl = null;
          * 
          * if (o instanceof Exception) { Exception e = (Exception) o; ByteArrayOutputStream bStream
          * = new ByteArrayOutputStream(); PrintStream pw = new PrintStream(bStream);
          * e.printStackTrace(pw); strMsg = bStream.toString(); aLogEl =
          * PSXmlDocumentBuilder.addElement(m_responseDoc, parent, "LogMessageException", strMsg);
          * aLogEl.setAttribute("message",PSExceptionUtils.getMessageForLog(e)); } else { strMsg =
          * o.toString(); aLogEl = PSXmlDocumentBuilder.addElement(m_responseDoc, parent,
          * "LogMessage", strMsg); }
          * 
          * aLogEl.setAttribute("level", event.getLevel().toString()); aLogEl.setAttribute("name",
          * event.getLoggerName()); aLogEl.setAttribute("thread", event.getThreadName());
          * 
          */
      }
   ...
   /**
    * Inner class loader class to instantiate a specific class.
    */
   private class RefreshClassLoader extends ClassLoader {
      /**
       * Ctor that takes the root directory of classes and a list of file objects such that any file
       * requested in this list will be 're'-loaded.
       * 
       * @param root. Directory of classes. Never <code>null</code> or empty.
       * 
      ...
      public RefreshClassLoader(String root, List<File> dirs) {
         if (root == null || dirs == null)
            throw new IllegalArgumentException("root and dirs must not be null");
       ...
       * 
       * @throws IOException if filename doesn't specify a valid file or the data can't be read from
       *         the file.
       */
      ...
      /**
       * List of files that should be reloaded. Initialized in definition, never <code>null</code>.
       */
   ...
   /**
    * This parameter is used to force the handler to load test classes for the request. Useful when
    * debugging and test cases are changing frequently (i.e. the server doesn't need to be restarted
    * each time).
    */
