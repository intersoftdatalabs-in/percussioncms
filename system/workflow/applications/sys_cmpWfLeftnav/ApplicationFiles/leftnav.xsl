...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="userroles1"
		select="document(/*/userrolesurl)/UserStatus" />
	<xsl:variable name="componentcontext1"
	...
	<xsl:template match="/">
		<html>
			...
			<body>        <!-- psx-docalias="newcontent" psx-docref="psx-newcontenturl" -->
				...
				<table height="100%" width="150" cellpadding="4"
					cellspacing="0" border="0">
					<xsl:attribute name="id">
            <xsl:value-of select="*/x" />
          </xsl:attribute>
					<xsl:apply-templates select="*" mode="mode0">
						<xsl:with-param name="componentcontext1"
							select="$componentcontext1" />
						<xsl:with-param name="userroles1"
							select="$userroles1" />
					</xsl:apply-templates>
			...
			<tr>
              ...
              <xsl:value-of select="." />
						...
						<tr>
							<td valign="top">
								<xsl:for-each
									select="document($relatedlinks)/*/component">
						...
						<xsl:if test="not(workflowid='')">
							<tr class="datacell1">
								<td valign="middle" class="datacell1" height="21"
									align="center">
									<a valign="middle">
								...
								<xsl:text>', 'previewworkflow', 'width=860,height=500,location=0;0,toolbar=no,menubar=no,scrollbars=yes,resizable=yes')</xsl:text>
										...
										<img src="../sys_resources/images/preview_button.gif"
											width="145" height="20" border="0" alt="Preview Workflow"
						...
						<tr class="outerboxcell">
							<td class="datacell1font" align="center">
								<br id="XSpLit" />
								(c) Percussion Software 1999-@COPYRIGHTYEAR@
							</td>
						</tr>
					</table>
				</td>
