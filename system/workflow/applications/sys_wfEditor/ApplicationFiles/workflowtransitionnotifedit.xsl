...
<xsl:stylesheet version="1.1"
				...
				<table width="100%" height="100%" cellpadding="0"
					...
					<tr>
						...
						<td align="middle" width="100%" valign="top" height="100%"
							...
							<!-- start main body slot -->
							<form name="UpdateTransitionNotif"
								action="UpdateTransitionNotif.htm" method="get"
	...
	<xsl:template match="*" mode="mode0">
		<xsl:variable name="transitioninfo"
			select="document(/*/transitioninfo)/*" />
		<xsl:for-each select=".">
			<tr>
					...
					<span class="outerboxcellfont">
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$transitioninfo/workflowslink" /></xsl:attribute>
							Workflows
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$transitioninfo//workflowlink" /></xsl:attribute>
							<xsl:apply-templates
								select="$transitioninfo/@workflowname" />
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$transitioninfo/statelink" /></xsl:attribute>
							<xsl:apply-templates
								select="$transitioninfo/@statename" />
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$transitioninfo/transitionlink" /></xsl:attribute>
							<xsl:apply-templates
								select="$transitioninfo/@transitionlabel" />
						</a>
			...
			<tr>
				<td valign="top">
					...
					<input name="transitionnotifid" type="hidden">
						<xsl:attribute name="value"><xsl:value-of
							select="transitionnotifid" /></xsl:attribute>
					</input>
					...
					<table width="100%" cellpadding="0" cellspacing="1"
						...
						<xsl:for-each select="transitionnotif">
							...
							<tr class="datacell2">
								...
								<td width="100%" align="left" class="datacell1font">
									...
									<select name="notificationid">
										...
										<xsl:for-each
											select="document(/*/extstatenotifsurl)/*/statenotif">
							...
							<tr class="datacell1">
								<td align="left" class="datacell1font">
									State Role Recipients Type:
									<img src="/sys_resources/images/invis.gif" height="1"
										width="100" border="0" />
								</td>
								<td width="100%" align="left" class="datacell1font">
									<xsl:variable name="recipienttype"
										select="transitionnotifreciptype" />
									<select name="recipienttype">
										<xsl:for-each
											select="document(/*/rolerecipientsurl)/*/item">
											<option value="{value}">
