...
<xsl:stylesheet version="1.1"
				...
				<table width="100%" height="100%" cellpadding="0"
					...
					<tr>
						...
						<td align="middle" width="100%" valign="top" height="100%"
							...
							<!-- start main body slot -->
							<form name="UpdateRoles" action="UpdateRoles.htm"
								method="get" onsubmit="return save_onclick(false)">
								<!-- BEGIN Banner and Login Details -->
	...
	<xsl:template match="*" mode="mode0">
		<xsl:variable name="workflowinfo"
			select="document(/*/workflowinfourl)/*" />
		<xsl:for-each select=".">
			<tr>
					...
					<span class="outerboxcellfont">
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$workflowinfo/workflowslink" /></xsl:attribute>
							Workflows
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$workflowinfo/workflowlink" /></xsl:attribute>
							<xsl:apply-templates
								select="$workflowinfo/@workflowname" />
						</a>
						&gt;
						<xsl:choose>
							<xsl:when test="workflowRoleNew/Name=''">
								New Role
							</xsl:when>
							<xsl:otherwise>
								<xsl:apply-templates
									select="workflowRoleNew/Name" />
							</xsl:otherwise>
						</xsl:choose>
					</span>
				</td>
	...
	<xsl:template match="workflowRoleNew" mode="mode7">
		<xsl:variable name="serverroles"
			select="document(//serverrolesurl)/*" />
		<xsl:variable name="excluderoles"
			select="document(//excluderoles)/*" />
		<xsl:variable name="addableroles"
			select="$serverroles/role[not(@name=$excluderoles/role/name)]" />
		<xsl:for-each select=".">
					...
					<tr class="datacell1">
						...
						<td width="80%" align="left" class="datacell1font">
							<select name="requiredname">
								<!-- when adding a new role get all available server based roles,
					...
					<tr class="datacell1">
						<td colspan="2" align="left" class="datacell1font">
							...
							<input type="button" class="nav_body" name="DBActionType"
								value="Save" onclick="save_onclick(true);" />&nbsp;
