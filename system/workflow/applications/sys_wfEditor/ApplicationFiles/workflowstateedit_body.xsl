...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="publishableValues"
		select="document(/*/PublishableLookupUrl)/*" />
	<xsl:template mode="workflowstateeditbody" match="*">
		<xsl:variable name="workflowinfo"
			select="document(/*/workflowlinfourl)/*" />
		<table width="100%" height="100%" cellpadding="0"
			cellspacing="3" border="0">
			<form name="UpdateState" action="UpdateState.htm" method="get"
				onsubmit="return save_onclick()">            <!-- BEGIN Banner and Login Details -->
				<tr>
						...
						<table width="100%" height="100%" cellpadding="0"
							...
							<tr class="outerboxcell">
							 ...
							 <xsl:value-of select="$workflowinfo/workflowlink" />
										...
										&gt;
										<xsl:choose>
											<xsl:when test="workflowStateEdit/Name=''">
												New State
											</xsl:when>
											<xsl:otherwise>
												<xsl:apply-templates
													select="workflowStateEdit/Name" />
											</xsl:otherwise>
										</xsl:choose>
									</span>
								</td>
						 ...
						 <xsl:value-of select="componentname" />
							 ...
							 <xsl:value-of select="stateid" />
									...
									<table width="100%" height="100%" cellpadding="0"
										...
										<xsl:apply-templates
											select="workflowStateEdit" mode="mode16" />
									</table>
								...
								<tr>
									<td class="headercell">
										<table width="100%" cellpadding="0" cellspacing="1"
											...
											<xsl:apply-templates
												select="document($exttransitions)" mode="transitionlist" />
										</table>
									</td>
								</tr>
								<tr>
									<td class="headercell">
										<table width="100%" cellpadding="0" cellspacing="1"
											...
											<xsl:apply-templates
												select="document($extagingtransitions)"
												mode="agingtransitionlist" />
										</table>
									</td>
								</tr>
								<tr>
									<td class="headercell">
										<table width="100%" cellpadding="0" cellspacing="1"
											...
											<xsl:apply-templates
												select="document($extstateroles)" mode="staterolelist" />
										</table>
									</td>
								</tr>
								<!-- <tr> <td class="headercell"> <table width="100%" cellpadding="0"
									cellspacing="1" border="0"> <tr> <td colspan="10" class="outerboxcell" align="right">
									<span class="headercellfont">Notifications</span> </td> </tr> <xsl:apply-templates
									select="document($extstatenotifs)" mode="statenotiflist"/> </table> </td>
									</tr> -->
							</xsl:if>
						</table>
					</td>
			...
			<tr class="datacell2">
				<td colspan="2" align="left" class="datacell1font">
					...
					<input type="button" class="nav_body" name="DBActionType"
						value="Save" onclick="save_onclick();" />&nbsp;
