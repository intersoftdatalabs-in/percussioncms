...
<xsl:stylesheet version="1.1"
	...
	<xsl:import
		href="file:sys_resources/stylesheets/sys_bannerTemplate.xsl" />
	<xsl:include
		href="file:sys_wfLookups/workflowactionlistbox.xsl" />
	<xsl:include
		href="file:sys_wfLookups/exttransitionnotifs.xsl" />
	<xsl:include
		href="file:sys_wfLookups/exttransitionroles.xsl" />
	...
	<xsl:variable name="bannerinclude"
		select="/*/bannerincludeurl" />
	<xsl:variable name="userstatusinclude"
		select="/*/userstatusincludeurl" />
	<xsl:variable name="exttransitionnotifs"
		select="/*/URL/exttransitionnotifs" />
	<xsl:variable name="exttransitionroles"
		select="/*/URL/exttransitionroles" />
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<link href="../sys_resources/css/tabs.css" rel="stylesheet"
					type="text/css" />
				<script language="JavaScript"
					src="../sys_resources/js/checkrequired.js"><![CDATA[
]]></script>
				<script language="JavaScript"
					src="../sys_resources/js/delconfirm.js"><![CDATA[
]]></script>
				<script language="JavaScript"
					src="../sys_resources/js/formValidation.js"><![CDATA[
			]]></script>
			...
			<body class="backgroundcolour" leftmargin="0" topmargin="0"
				...
				<table width="100%" height="100%" cellpadding="0"
					...
					<tr>
						<td align="middle" valign="top" width="150" height="100%"
							class="outerboxcell">
							<!-- start left nav slot -->
							<!-- start left nav slot -->
							<xsl:for-each
								select="document($relatedlinks)/*/component[@slotname='slt_wf_nav']">
						...
						<td align="middle" width="100%" valign="top" height="100%"
							...
							<!-- start main body slot -->
							<form name="UpdateTransition" action="UpdateTransition.htm"
								method="get" onsubmit="return save_onclick()">
								<!-- BEGIN Banner and Login Details -->
					...
					<span class="outerboxcellfont">
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$stateinfo/workflowslink" /></xsl:attribute>
							Workflows
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$stateinfo/workflowlink" /></xsl:attribute>
							<xsl:apply-templates
								select="$stateinfo/@workflowname" />
						</a>
						&gt;
						<a>
							<xsl:attribute name="href"><xsl:value-of
								select="$stateinfo/statelink" /></xsl:attribute>
							<xsl:apply-templates
								select="$stateinfo/@statename" />
						</a>
						&gt;
						<xsl:choose>
							<xsl:when test="transition/label=''">
								New
								<xsl:if test="isaging='1'">
									Aging
								</xsl:if>
								Transition
							</xsl:when>
					...
					<select name="tostate" size="1">
						...
						<xsl:for-each select="state">
							<option>
								...
								<xsl:if
									test="$this/workflowtransitionedit/transition/tostateid=$value">
									<xsl:attribute name="selected" />
								</xsl:if>
								<xsl:attribute name="value"><xsl:value-of
									select="stateid" /></xsl:attribute>
	...
	<xsl:template match="*" mode="mode19">
		<xsl:for-each select="role">
			<option>
				...
				<xsl:if
					test="$this/workflowtransitionedit/transition/roles=$value">
					<xsl:attribute name="selected" />
				</xsl:if>
				<xsl:attribute name="value"><xsl:value-of
			...
			<tr>
				<td valign="top">
					...
					<input name="sys_componentname" type="hidden">
						<xsl:attribute name="value"><xsl:value-of
							select="componentname" /></xsl:attribute>
					</input>
					<input name="workflowid" type="hidden">
						<xsl:attribute name="value"><xsl:value-of
							select="workflowid" /></xsl:attribute>
					</input>
					...
					<input name="transitionid" type="hidden">
						<xsl:attribute name="value"><xsl:value-of
							select="transitionid" /></xsl:attribute>
					</input>
					<input name="transitiontype" type="hidden">
						<xsl:attribute name="value"><xsl:value-of
							select="isaging" /></xsl:attribute>
					</input>
					...
					<table width="100%" cellpadding="0" cellspacing="1"
						...
						<xsl:for-each select="transition">
							...
							<tr class="datacell1">
								...
								<td width="100%" align="left" class="datacell1font">
									...
									<span>
										<xsl:value-of
											select="document($extstates)/*/state[stateid=$stateidvar]/name" />
									</span>
									<span>&nbsp;(
									</span>
									<span>
							...
							<xsl:apply-templates
								select="document($extstates)" mode="mode14" />
									...
									<td width="100%" align="left" class="datacell1font">
										<input type="text" name="requiredapprovals" size="6">
											<xsl:attribute name="value">
												...
												select="approvals" /></xsl:otherwise>
												</xsl:choose>
											</xsl:attribute>
										</input>&nbsp;(Required
										if Approval Type is set to "Specified Number")
									</td>
								</tr>
								<tr class="datacell2">
									<td align="left" class="datacell1font">
									...
									<td width="100%" align="left" class="datacell1font">
										<select name="commentrequired">
											<xsl:attribute name="selected"><xsl:value-of
												select="commentrequired" /></xsl:attribute>
											<option value="n">
								...
								<tr class="datacell2">
									...
									<td width="100%" align="left" class="datacell1font">
										<select name="defaulttransition">
											<xsl:attribute name="selected"><xsl:value-of
												select="defaulttransition" /></xsl:attribute>
											<option value="n">
							...
							<tr class="datacell1">
								...
								<td width="100%" align="left" class="datacell1font">
									<!-- begin XSL -->
									<xsl:apply-templates
										select="/workflowtransitionedit/workflowactionlist"
										mode="workflowactions">
										<xsl:with-param name="selectedaction">
											<xsl:value-of
												select="/workflowtransitionedit/transition/actions" />
										</xsl:with-param>
									</xsl:apply-templates>&nbsp;
									<!-- end XSL -->
								</td>
								...
								<tr class="datacell2">
									...
									<td width="100%" align="left" class="datacell1font">
										...
										<select name="agingtype">
											<xsl:for-each
												select="document(/*/agingtypesurl)/*/item">
												<option value="{value}">
								...
								<tr class="datacell1">
									...
									<td width="100%" align="left" class="datacell1font">
										<input type="text" name="aginginterval" size="6">
											<xsl:attribute name="value"><xsl:value-of
												select="aginginterval" /></xsl:attribute>
										</input>&nbsp;(Required
										for Absolute and Repeated Aging Types)
									</td>
								</tr>
								<tr class="datacell2">
									<td align="left" class="datacell1font">
									...
									<td width="100%" align="left" class="datacell1font">
										<xsl:variable name="systemfield"
											select="systemfield" />
										<select name="systemfield">
											<option>&nbsp;
											</option>
											<xsl:for-each
												select="document(/*/systemfieldurl)/*/item">
												<option value="{value}">
						...
						<tr class="datacell1">
							<td colspan="2" align="left" class="datacell1font">
								<input type="hidden" class="nav_body" name="DBActionType"
									value="UPDATE" />
								<input type="button" class="nav_body" name="DBActionType"
									value="Save" onclick="javascript:save_onclick();" />&nbsp;
								<input type="button" value="Cancel" name="cancel"
									language="javascript" onclick="cancelFunc();" />
							</td>
						</tr>
					</table>
				</td>
					...
					<tr>
						<td class="headercell">
							<table width="100%" cellpadding="0" cellspacing="1"
								...
								<xsl:apply-templates
									select="document($exttransitionroles)"
									mode="exttransitionroles" />
							</table>
						</td>
					</tr>
				</xsl:if>
					...
					<tr>
						<td class="headercell">
							<table width="100%" cellpadding="0" cellspacing="1"
								...
								<xsl:apply-templates
									select="document($exttransitionnotifs)"
									mode="exttransitionnotifs" />
							</table>
						</td>
					</tr>
				</xsl:if>
