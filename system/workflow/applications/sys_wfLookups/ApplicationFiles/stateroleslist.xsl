		...
		<xsl:for-each select="staterole">
			<xsl:if test="roleid != ''">
                ...
                <xsl:value-of select="link" />
							...
							<xsl:apply-templates select="roleid" />)&nbsp;
						</a>
					</td>
					<td class="datacell1font" height="20" align="left">
						...
						<xsl:apply-templates
							select="document(assigntypesurl)/*/item[value=$assignmenttype]/display" />&nbsp;
					</td>
					<td class="datacell1font" height="20" align="left">
						<xsl:variable name="adhoctype">
							<xsl:value-of select="adhoc" />
						</xsl:variable>
						<xsl:apply-templates
							select="document(adhoctypeurl)/*/item[value=$adhoctype]/display" />&nbsp;
					</td>
				</tr>
			</xsl:if>
