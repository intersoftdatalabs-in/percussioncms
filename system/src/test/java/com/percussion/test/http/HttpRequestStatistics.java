...
public class HttpRequestStatistics implements Cloneable {
  ...
  /**
   * Record that the timing started at the specified time, expressed in milliseconds since the
   * epoch.
   */
  ...
  public void add(HttpRequestStatistics stats) {
    {
      ...
      if (stats.m_connectMinMS < m_connectMinMS) m_connectMinMS = stats.m_connectMinMS;

      if (stats.m_connectMaxMS > m_connectMaxMS) m_connectMaxMS = stats.m_connectMaxMS;
...

    {
      ...
      if (stats.m_reqSendMinMS < m_reqSendMinMS) m_reqSendMinMS = stats.m_reqSendMinMS;

      // FB: SA_FIELD_SELF_COMPARISON NC 1-17-16
      if (stats.m_reqSendMaxMS > m_reqSendMaxMS) m_reqSendMaxMS = stats.m_reqSendMaxMS;
...

    {
      ...
      if (stats.m_hdrReadMinMS < m_hdrReadMinMS) m_hdrReadMinMS = stats.m_hdrReadMinMS;

      if (stats.m_hdrReadMaxMS > m_hdrReadMaxMS) m_hdrReadMaxMS = stats.m_hdrReadMaxMS;
...

    {
      ...
      if (stats.m_cntReadMinMS < m_cntReadMinMS) m_cntReadMinMS = stats.m_cntReadMinMS;

      // FB: SA_FIELD_SELF_COMPARISON NC 1-17-16
      if (stats.m_cntReadMaxMS > m_cntReadMaxMS) m_cntReadMaxMS = stats.m_cntReadMaxMS;
...

    {
      ...
      if (stats.m_rndTripMinMS < m_rndTripMinMS) m_rndTripMinMS = stats.m_rndTripMinMS;

      // FB:SA_FIELD_SELF_COMPARISON 1-17-16
      if (stats.m_rndTripMaxMS > m_rndTripMaxMS) m_rndTripMaxMS = stats.m_rndTripMaxMS;
...

    {
      ...
      if (stats.m_hdrBytesMin < m_hdrBytesMin) m_hdrBytesMin = stats.m_hdrBytesMin;

      if (stats.m_hdrBytesMax > m_hdrBytesMax) m_hdrBytesMax = stats.m_hdrBytesMax;
...

    {
      ...
      if (stats.m_cntBytesMin < m_cntBytesMin) m_cntBytesMin = stats.m_cntBytesMin;

      // FB:SA_FIELD_SELF_COMPARISON NC 1-17-16
      if (stats.m_cntBytesMax > m_cntBytesMax) m_cntBytesMax = stats.m_cntBytesMax;
