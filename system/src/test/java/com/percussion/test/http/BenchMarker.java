...
/**
 * The BenchMarker class will run a given test series and collect statistics on the performance of
 * the server.
 */
public class BenchMarker implements Runnable, LogSink {
  ...
  public void setTimes(Date startTime, Date endTime) throws IllegalArgumentException {
    ...
    if (duration < 0) {
      throw new IllegalArgumentException("Error: start time is after end time");
    }
  ...
  public void addURL(URL url) {
    if (url == null) throw new IllegalArgumentException("Null URL specified");
    m_urls.add(url);
  }
