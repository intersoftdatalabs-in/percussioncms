...
/**
 * Encapsulates an HTTP GET or POST request and the results of the request. This class will not
 * follow the redirect URL, which is acomplished by {@link #getFollowRedirects()}.
 *
 * <p>This class is a replacement for {@link HttpRequest}. It implements all methods from {@link
 * HttpRequest}, but is implemented with {@link PSRemoteRequester}.
 */
public class PSHttpRequest extends PSRemoteRequester {

  /**
   * Construct a request to the given URL with the given method. If request content is non-null,
   * then the request content will be after the request and the request headers.
   *
   * <p>If you happen to know the length (in bytes) of the content, you may want to set the
   * "Content-length" request header to that value. We will not do it automatically, and we will
   * <B>not</B> consider the setting of this header when reading the content stream.
   *
   * <p>Also, if you happen to know the character encoding that the content uses, you may want to
   * set the "Char-encoding" request header. Again, we will not consider this value when sending the
   * content.
   *
   * @param URL
   * @param reqMethod
  ...
  private void init(String httpUrl, String reqMethod, InputStream reqContent)
    ...
    if (m_reqURL.getProtocol().equalsIgnoreCase("https")) props.put("useSSL", "true");
    else props.put("useSSL", "false");
   ...
   *
   * @throws IOException
   * @throws HttpConnectException if all atempts failed to establish an HTTP connection.
   */
  public void sendRequest() throws IOException, ModuleException {
    if (m_reqMethod.equalsIgnoreCase("GET")) {
      ...
      if (data.length > 0) {
        headers =
            new NVPair[] {new NVPair("Content-Type", "text/xml; charset=" + PSCharSets.rxStdEnc())};
      }
  ...
  /**
   * This is called by super class. It combines the default headers from the super class and the
   * headers of this objects.
   *
  ...
  protected NVPair[] getDefaultHeaders() {
    ...
    if (httpHeaders.length == 0) {
      ...
      for (int i = 0; i < superHeaders.length; i++) defHeaders[i] = superHeaders[i];
  ...
  /**
   * This is called by super class. We don't want the request to follow the redirect URL since we
   * need to get the info from the redirect URL.
   *
  ...
  /**
   * @return <code>false</code>, let this object to process the status code of the response.
   */
   ...
   *
   * @param con the connection used to set credential, assumed not <code>null</code>.
  ...
  private NVPair[] getHeaders() {
    ...
    if (httpHeader.length > 0) headers.toArray(httpHeader);

    return httpHeader;
  }

  /**
   * @return the request content in byte array, never <code>null</code>, may be empty.
   * @throws IOException
   */
  private byte[] getContentBytes() throws IOException {
    if (m_reqContent == null) return new byte[0];
  ...
  /**
   * The number of bytes sent in a request. Calculated during the processing of <code>sendRequest
   * </code>. 0 until this method called at least once. Use {@link #getBytesSent} to retrieve.
   */
