...
public class HttpRequest {
   ...
   */
  public HttpRequest(String URL, String reqMethod, InputStream reqContent) {
   ...
   * @throws HttpConnectException if all atempts failed to establish an HTTP connection.
   */
  private void sendRequest(HttpRequestTimings timings) throws IOException, HttpConnectException {
    try {
  ...
  protected void connect(String host, int port) throws IOException, HttpConnectException {
    log("Connecting to " + host + ":" + port + "...");

    // todo: remove this once we support keeping connections alive
    if (m_sock != null)
      throw new HttpConnectException("Socket must be closed befor creating a new connection!");
    ...
    while (tryCount++ < MAX_CONNECT_ATTEMPTS) {
      try {
        ...
        try {
          if (tryCount < MAX_CONNECT_ATTEMPTS) Thread.sleep(100);
          else throw new HttpConnectException("Connection failed: " + reason);
        } catch (InterruptedException ie) {
          Thread.currentThread().interrupt();
        }
      } catch (Throwable t) {
        throw new HttpConnectException("Connection failed: " + t.getLocalizedMessage());
      }
    }
  }
  ...
  /**
   * Returns the approximate number of bytes sent when the request was made. Until {@link
   * #sendRequest} is called once, 0 is returned. The bytes sent is recalculated each time a request
   * is sent.
   *
   * @return A value >= 0.
   */
