...
public class CreateWorkflows {
  /**
   * Usage: java testdata.CreateWorkflows [server=localhost] [port=9992] [uid=admin1] [pw=demo]
   * [basename=workflow] [count=1]
   *
   * <p>param names are case-insensitive, all are optional, order doesn't matter, default values
  ...
  public static void main(String[] args) throws Exception {
    Map<String, String> userParams =
        extractArgs(
            args,
            "CreateWorkflows [server=localhost] [port=9992] [uid=admin1] [pw=demo]"
                + " [basename=workflow] [count=1]");
    PSRemoteRequester req =
        new PSRemoteRequester(
            userParams.get("server"), Integer.parseInt(userParams.get("port")), -1);
  ...
  public static int createWorkflow(PSRemoteRequester req, String wfName) throws Exception {
  ...
  public static void createState(PSRemoteRequester req, int wfId, int counter) throws Exception {
  ...
  public static void createTransition(
      PSRemoteRequester req, int wfId, int fromStateId, int toStateId) throws Exception {
  ...
  static Document makeRequest(PSRemoteRequester req, String resource, String[] paramArray)
      throws Exception {
  ...
  /**
   * Fills a map with all default values, then overwrites them with values found in the supplied
   * args. The rules outlined in the usage are followed.
   *
   * @param args Assumed not <code>null</code>.
   * @param usage If any problems w/ args, an exception is thrown with this text as the exception
   *     text.
   * @return Never <code>null</code>, always has all parameters as specified in usage.
   */
