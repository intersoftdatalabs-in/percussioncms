...
public class PSUrlUtilsTest {
  ...
  public void testSSLProtocol() throws Exception {
    ...
    createdURL =
        PSUrlUtils.createUrl(
            null, null, "test/base.htm", map.entrySet().iterator(), anchor, reqContextMock, true);
    assertEquals("http://originalhost:9992/test/base.htm?p2=v2#a", createdURL.toString());

    createdURL =
        PSUrlUtils.createUrl(
            null, null, "test/base.htm", map.entrySet().iterator(), anchor, reqContextMock, false);
    assertEquals("http://originalhost:9992/test/base.htm?p2=v2#a", createdURL.toString());
