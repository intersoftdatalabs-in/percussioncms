...
/**
 * This class retrieves the exact time and date according to a server implementing a NIST-style
 * atomic clock service.
 *
 * <p>Since there is no way to set the date from Java, you should use this class to obtain the
 * offset between the Java time and the official time.
 */
public class ClockSync {
  ...
  /** Construct a clock sync that connects to the specified server and port. */
  ...
  public static void syncTimeDOS() throws IOException {
    ...
    if (os.equals("Windows NT")) timeCmd = "cmd";
    ...
    if (offsetSec < 0) offsetSec = -offsetSec;

    System.out.println("Your system clock was " + offsetSec + " seconds too " + suffix + ".");
  }

  /**
   * Measures the time difference between this machine's clock and the server clock.
   *
   * <p>The algorithm takes several samples, taking the average of each sample's difference.
   */
  ...
  /**
   * Gets the difference between the server clock and this computer's clock. If this value is
   * negative, the computer's clock is too fast. If this value is positive, this computer's clock is
   * too slow.
   */
  ...
  /**
   * Get the msec delta between this computer's time and the server time. Don't worry about time
   * zones, because we correct for this later.
   */
  ...
  /**
   * The measured difference between this computer's clock and the server's clock, in milliseconds.
   */
