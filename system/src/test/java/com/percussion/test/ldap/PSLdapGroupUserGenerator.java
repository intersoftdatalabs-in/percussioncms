...
/**
 * This class was created to generate an LDIF file containing a set of groups and users. The caller
 * specifies how many groups and users to create. The following structure is created:
 *
 * <pre>
 * com
 *    percussion
 ...
 *
 * <p>The number of users added to each group is a random # between 0 and 199 (or the # of users
 * specified-1 if less than 200,) inclusive. It is guaranteed that there will be 1 group w/ all
 * users, 1 group w/ 1 user (if there are at least 3 groups) and 1 group with 0 users (if there are
 * at least 4 groups.)
 *
 * <p>A random number is used to find the beginning of a block of users within the group of all
 * users.
 *
 * <p>An example output is as follows:
 *
 * <pre>
 * version: 1
 *
 ...
 * </pre>
 *
 * Note that the generated output contains object classes and attributes that are somewhat specific
 * to our test environment.
 *
 * <p>See {@link PSLdapGroupUserGenerator#main(String[])} for usage.
 *
 * @author paulhoward
 */
public class PSLdapGroupUserGenerator {
   ...
   *
   * <p>Groups must be >= 2. Users must be > 0;
   *
   * <p>Hasn't been thoroughly tested w/ all allowed input variations.
   *
   * @param args
   */
  public static void main(String[] args) throws IOException {
    ...
    if (groupCount < 0) throw new IllegalArgumentException("Groups must be >= 2");
    int userCount = Integer.parseInt(args[1]);
    if (userCount < 0) throw new IllegalArgumentException("Users must be >= 0");
    System.out.println(
  ...
  public void generateOutput(Writer output, int groupCount, int userCount) throws IOException {
    writeHeader(output);
  ...
  private void createGroups(int groupCount, int userCount, Writer output) throws IOException {
    if (groupCount < 2) throw new IllegalArgumentException("Must have 2 or more groups.");
    ...
    for (int ct = 0; ct < userCount; ct++) group.addMember(ct);
    writeNode(group, output);

    for (int i = 1; i < groupCount; i++) {
      group = new GroupLdapNode(i);
      int nextUserId = r.nextInt(userCount - idDistribution[i]);
      for (int ct = idDistribution[i]; ct > 0; ct--) group.addMember(nextUserId++);
      writeNode(group, output);
    }
  }

  private void createUsers(int userCount, Writer output) throws IOException {
  ...
  /**
   * Creates the container nodes that form the hierarchy and will contain the user and group nodes.
   * The names of the nodes for the groups and users are are stored in {@link #groupDn} and {@link
   * #userDn}.
   *
   * @param output The ldif data will be written to this object. Assumed not <code>null</code>.
   * @throws IOException
   */
  private void createContainers(Writer output) throws IOException {
    String rootNodeName = "ou=group_?performance_test,dc=percussion,dc=com";
    LdapNode parentNode =
        new OuLdapNode(
            rootNodeName,
            "Directory structure used for testing performance when there are a large quantity of"
                + " groups.");
    writeNode(parentNode, output);
    LdapNode groupsNode =
        new OuLdapNode("ou=groups," + rootNodeName, "node that contains all groups");
    writeNode(groupsNode, output);
    groupDn = groupsNode.dn;
    LdapNode usersNode = new OuLdapNode("ou=users," + rootNodeName, "node that contains all users");
    writeNode(usersNode, output);
    userDn = usersNode.dn;
  }

  private void writeNode(LdapNode node, Writer output) throws IOException {
    output.write("\r\n# entry-id: " + (entryId++) + "\r\n");
    node.write(output);
  }

  /**
   * The next number to use for entry ids. When a value is used, the user must increment it so it is
   * ready for use next time.
   */
  private int entryId = 10;

  /**
   * The fully qualified name (dn) of the node that contains all the group nodes. Set in <code>
   * createContainers</code> then never empty or modified further.
   */
  private String groupDn;

  /**
   * The fully qualified name (dn) of the node that contains all the user nodes. Set in <code>
   * createContainers</code> then never empty or modified further.
   */
  private String userDn;

  /**
   * The root of the name for all generated groups. A unique id is appended to this name for each
   * unique group.
   */
  private static final String GROUP_BASE_NAME = "group";

  /**
   * The root of the name for all generated users. A unique id is appended to this name for each
   * unique user.
   */
  ...
  private static List<String> tokenize(String value) {
    ...
    while (toker.hasMoreTokens()) results.add(toker.nextToken().trim());
    return results;
  }
  ...
  private abstract class LdapNode {
    ...
    /**
     * Format the data in a way needed for an ldif file and write it to the supplied stream.
     *
     * @param stream Never <code>null</code>.
     */
    public void write(Writer stream) throws IOException {
      stream.write("dn: " + dn);
      stream.write("\r\n");
  ...
  /**
   * Contains all the information needed for a group node in an ldif file. All that is needed to
   * create one is the name of the group, w/o the dc components.
   */
  private class GroupLdapNode extends LdapNode {
    ...
    /**
     * @param userId The id of the user to add as a member of this group. Assumed to be in the range
     *     of user ids present in the system.
     */
  ...
  /**
   * Contains all the information needed for a user node in an ldif file. All that is needed to
   * create one is a unique number that is appended to several of the names within the object.
   */
  private class UserLdapNode extends LdapNode {
    ...
    public void write(Writer stream) throws IOException {
      ...
      stream.write("\r\n");
      // pw is demo
      stream.write("userPassword: {SSHA}c7ODY/Rpeb06gLee4icS+XxqTQjoPBNYIQrFYw==");
      stream.write("\r\n");
    }
