...
public class PSSqlTest {
  ...
  /**
   * Create a new connection. The connection information is hard coded in this method.
   *
   * @return the connection created, never <code>null</code>. The caller is responsible to close it.
  ...
  private Connection getConnection() throws SQLException, ClassNotFoundException {
    ...
    Properties props = PSSqlHelper.makeConnectProperties(connStr, m_database, m_user, m_password);
    ...
    System.out.println(
        "Driver version= " + driver.getMajorVersion() + "." + driver.getMinorVersion());

    Connection conn = DriverManager.getConnection(connStr, props);
    if (m_verbose) {
      System.out.println("\nConnection properties:");
      System.out.println("\tautocommit= " + (conn.getAutoCommit() ? "true" : "false"));
   ...
   *
   * @param conn the connection used to make to lookup, assumed not <code>null</code>.
   * @param database the database name from which to make the lookup, assumed not <code>null</code>
   *     or empty.
   * @param schema the schema for which to make the lookup, assumed not <code>null</code> or empty.
   * @return a list with all user table names as <code>String</code>, never <code>null</code>, may
   *     be empty.
   * @throws SQLException for any error making the lookup.
   */
  private List getUserTables(Connection conn, String database, String schema) throws SQLException {
    ResultSet tables = null;
    try {
      List tableNames = new ArrayList();

      DatabaseMetaData metaData = conn.getMetaData();
      tables = metaData.getTables(database, schema, "%", new String[] {"TABLE"});
      if (tables != null) {
  ...
  /**
   * Query all user tables, then get all foreign key meta data for each table and measure the time
   * used for the foreign key lookup.
   *
   * @param conn the database connection to use, assumed not <code>null</code>.
   * @param tables a list of table names for which to perform the meta data test, assumed not <code>
  ...
  private void metaDataTest(Connection conn, List tables) {
    ...
    try {
      ...
      if (m_verbose) {
        System.out.println(
            "All Procedures callable= " + (metaData.allProceduresAreCallable() ? "yes" : "no"));
        System.out.println(
            "All Tables selectable= " + (metaData.allTablesAreSelectable() ? "yes" : "no"));
      ...
      while (walker.hasNext()) {
        ...
        if (m_verbose) System.out.println("Executing table: " + tableName);
        else {
          if (margin < MARGIN) {
            System.out.print(".");
            margin++;
        ...
        if (keys != null) {
          while (keys.next()) {
            ...
            if (m_verbose) System.out.println("\tForeign key= " + colDef.toString());
            foundForeignKeys = true;
          }
        }

        if (m_verbose && !foundForeignKeys) System.out.println("\tForeign key= ");

        if (m_verbose) System.out.println("\t" + stopWatch.toString());
      }
      stopWatch.stop();
      System.out.println(
  ...
  private void queryDataTest(
      Connection conn, List tables, String database, String schema, String driver) {
    PSStopwatch stopWatch = new PSStopwatch();
    ...
    try {
      ...
      while (count < m_queryCount) {
        ...
        while (walker.hasNext() && count < m_queryCount) {
          String tableName =
              PSSqlHelper.qualifyTableName((String) walker.next(), database, schema, driver);

          String query = queryString + tableName;

          if (m_verbose) System.out.println("Executing query: " + query);
          else {
            if (margin < MARGIN) {
              System.out.print(".");
              margin++;
      ...
      stopWatch.stop();
      System.out.println("\nTotal time to read " + count + " rows:" + stopWatch.toString());
    } catch (Exception e) {
      log.error(PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    } finally {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "\tjava com.percussion.testing.PSSqlTest driver "
            + "host database schema user password [queryCount] [verbose]");
    System.out.println(
        "\te.g.: java com.percussion.testing.PSSqlTest "
    ...
    try {
      ...
      if (conn != null) {
        try {
          ...
          if (warning != null) System.out.println("Warnings:");
          while (warning != null) {
