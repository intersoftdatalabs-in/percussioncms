...
/**
 * This class was created to provide functions that make it easy to create folder structures and
 * duplicate content. Typically, this is useful when stress testing or performance testing is being
 * done.
 *
 * <p>It was initially written for SFP use. It could use further enhancement for generic use.
 *
 * <p>When used as an exit, it performs the following steps:
 *
 * <ol>
 *   <li>Create a folder hierarchy starting at a specified location.
 *   <li>Duplicate a specified set of items a specified number of times.
 *   <li>Randomly distribute those items inside the created folder struct.
 *   <li>Transition a specified % of them using the DirecttoPublic transition.
 *   <li>Optionally create a matching managed navigation structure, transitioning all items created
 *       for navigation to public.
 * </ol>
 *
 * @author paulhoward
 */
public class PSContentHelper implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (params.length < 4) throw new PSParameterMismatchException(4, params.length);
    String path = params[0].toString();
    if (path.trim().length() == 0) {
      throw new PSParameterMismatchException("Must provide the root of the hierarchy.");
    }
    float distFactor = -1f;
    try {
    ...
    if (distFactor < 0f || distFactor > 1.0f) {
      throw new PSParameterMismatchException(
          "Invalid value for distributionFactor ("
              + params[1]
    ...
    if (maxDepth < 1) {
      throw new PSParameterMismatchException(
          "Invalid value for maxDepth (" + params[3] + "). Must be >= 1.");
    }
    ...
    if (count < maxDepth) {
      throw new PSParameterMismatchException(
          "Invalid value for totalFolders (" + params[2] + "). Must be >= maxDepth.");
    }
    ...
    if (params.length > 4 && params[4] != null && params[4].toString().trim().length() > 0) {
      try {
      ...
      if (lowId < 1) {
        throw new PSParameterMismatchException(
            "Invalid value for lowId (" + params[4] + "). Must be an int &gt; 0.");
      }
      ...
      if (highId < lowId) {
        throw new PSParameterMismatchException(
            "Invalid value for highId (" + params[5] + "). Must be an int &gt;= lowId.");
      }
      ...
      if (dupes < 1) {
        throw new PSParameterMismatchException(
            "Invalid value for dupes (" + params[6] + "). Must be an int &gt; 0.");
      }
      ...
      if (transitionPercent < 0 || transitionPercent > 100) {
        throw new PSParameterMismatchException(
            "Invalid value for transitionPercent ("
                + params[7]
    ...
    if (params.length >= 9) {
      createNavigation = (params[8] != null && "y".equalsIgnoreCase(params[8].toString()));
    }

    try {
      IPSRelationshipProcessor rproc = PSRelationshipProcessor.getInstance();
      IPSComponentProcessor cproc =
          new PSComponentProcessorProxy(PSProcessorProxy.PROCTYPE_SERVERLOCAL, request);
      List folders =
          createFolderTree(
              count, distFactor, maxDepth, path, rproc, cproc, request, createNavigation);
      ...
      if (folders.size() > 0 && dupeIds.size() > 0) {
        ...
        while (items.hasNext()) {
          ...
          if (random > transitionPercent) continue;
  ...
  /**
   * Creates a tree of <code>count</code> folders under <code>rootPath</code>. The minimum and
   * maximum depth of this tree is guaranteed to be <code>
   * maxDepth</code>. Folders are created using the definition found in a file called
   * "folderTemplate.xml" located in the root directory of the Rhythmyx server.
   *
   * <p>The folder generation algorithm is as follows: Subtract <code>maxDepth</code> from <code>
   * count</code>. Divide this value by maxDepth to get the number of first level folders. For each
   * additional level, take what is left and multiply by <code>
   * distribution</code> to get the # of folders for each level. Passing a value of .5 for the
   * distribution will use up the folders in about 6 levels. Passing .33 uses them up in about 8
   * levels. The distribution value must be 1 or less. If all folders haven't been used by the last
   * level, they will be added to the last level. At each level the folders are distributed randomly
   * among the parents.
   *
   * <p>It is guaranteed that there will be at least 1 path that is <code>
   * maxDepth</code> deep.
   *
   * @param count Total number of folders to create. Must be greater than or equal to <code>maxDepth
   *     </code>.
   * @param distribution A factor used to control what the tree will look like. Should be &gt; 0 and
   *     &lt;=1.
   * @param maxDepth How 'deep' should the tree be. Must be &gt;= 1.
   * @param rootPath The full path of the parent to receive the folders. Of the form
   *     //Sites/foo/bar. This folder must exist.
   * @param rproc Used to create the folder relationships
   * @param cproc Used to create the folder on the server
   * @param req The request context, used to locate and create navigation items.
   * @param createNavigation <code>true</code> to create a navtree in the root folder if a navtree
   *     or navon is not present, <code>false</code> otherwise. If <code>true</code>, then a file
   *     named navtestprops.properties is loaded from the rxroot directory. This file contains meta
   *     data for creating the nav structure.
   * @return A new list with 0 or more <code>PSLocator</code>s added, one for every folder created
   *     by this method. Never <code>null</code>.
   * @throws SAXException if there is an error loading the folder template xml file.
   ...
   * @throws PSUnknownNodeTypeException If there is an error loading a folder component summary.
   * @throws PSNavException If there is an error locating navons.
   * @throws PSInvalidContentTypeException if the landing page content type cannot be loaded.
   */
  public static List createFolderTree(
      int count,
      float distribution,
      int maxDepth,
      String rootPath,
    ...
    PSComponentSummary parentSummary =
        rproc.getSummaryByPath(
            ...
            rootPath,
            PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    if (null == parentSummary) throw new RuntimeException("Can't find folder named: " + rootPath);
    ...
    createFolderLevel(0, levelCounts, parentIds, rproc, cproc, folder, addedFolders);
    if (createNavigation) {
      if (navtreeloc != null) addedFolders.add(parentSummary.getCurrentLocator());
  ...
  /**
   * Checks for a navon in each folder in the supplied <code>folderList</code>, and if found,
   * creates a landing page in the folder and relates it to the navon.
   *
   * @param req The current request to use to create landing pages, assumed not <code>null</code>.
   * @param rproc The processor to use to add folder content, assumed not <code>null</code>.
   * @param cproc The processor to use to load folder component summaries, assumed not <code>null
   *     </code>.
   * @param folderList The list of folders to which landing pages are to be added, assumed not
   *     <code>null</code>, may be empty.
   * @param navprops The properties to use to create landing pages, assumed not <code>null</code>.
   * @throws PSNavException if there are any errors.
   */
  private static void createLandingPages(
      IPSRequestContext req,
      IPSRelationshipProcessor rproc,
      IPSComponentProcessor cproc,
      List folderList,
    ...
    try {
      PSKey[] locators = (PSKey[]) folderList.toArray(new PSKey[folderList.size()]);
      Element[] sumEls =
          cproc.load(PSDbComponent.getComponentType(PSComponentSummaries.class), locators);
      ...
      if (variantId == -1) {
        System.out.println("Slot " + landingSlot.getSlotName() + " has no variants ");
        return;
      }

      // Load all variants definitions and pick the one we need
      Element[] variantElems =
          cproc.load(PSDbComponent.getComponentType(PSContentTypeVariantSet.class), new PSKey[0]);
      PSContentTypeVariantSet variants = new PSContentTypeVariantSet(variantElems);
      PSContentTypeTemplate slotVariant = variants.getContentVariantById(variantId);
      PSContentTypeTemplate defaultVariant =
          variants.getContentVariantByName(navprops.getProperty("landing.defaultVariant"));
      ...
      for (int i = 0; i < sumEls.length; i++) {
        ...
        PSComponentSummary navon = PSNavFolderUtils.getChildNavonSummary(req, sum);
        if (navon == null) continue;
        ...
        IPSFieldValue titleValue = new PSTextValue(navon.getName() + "LandingPage");
        ...
        setFieldValue(item, bodyField, new PSTextValue(navon.getName() + " body"));
        if (defaultVariant != null) {
          String variantField = navprops.getProperty("ctype.defaultVariant");
          setFieldValue(
              item, variantField, new PSTextValue(String.valueOf(defaultVariant.getVariantId())));
        }
        ...
        rproc.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, children, sum.getCurrentLocator());

        // relate it to the navon
        PSLocator childLoc = (PSLocator) item.getKey().clone();
        PSAaRelationship aaRel =
            new PSAaRelationship(
                navon.getCurrentLocator(), childLoc, landingSlot, slotVariant, aaConfig);
  ...
  /**
   * Checks for a nav tree in the specified folder, and if not found, creates one with propagate set
   * to <code>false</code>, then transitions it to the Public state.
   *
   * @param req The request context, used to locate and create navigation items.
   * @param rproc The processor to use to add folder content, assumed not <code>null</code>.
   * @param folderSum The summary of the folder to check, assumed not <code>null</code>.
   * @param navprops Specifies user defined values to use in navigation creation, assumed not <code>
   *     null</code>.
   * @return The locator of the navtree if one is created, otherwise <code>null</code>.
   * @throws PSNavException if there are any errors.
   */
  private static PSLocator ensureNavigation(
      IPSRequestContext req,
      IPSRelationshipProcessor rproc,
      PSComponentSummary folderSum,
      Properties navprops)
      throws PSNavException {
    PSLocator result = null;
    // check for a navtree or navon in the folder
    PSComponentSummary navSum = PSNavFolderUtils.getChildNavonSummary(req, folderSum);
    if (navSum == null) {
      // need to create a navtree
      System.out.println("adding new navtree to folder " + folderSum.getName());
      PSItemDefManager defMgr = PSItemDefManager.getInstance();
      PSNavConfig config = PSNavConfig.getInstance(req);
      try {
        // create the navtree
        int communityId = req.getSecurityToken().getCommunityId();
        PSItemDefinition navonDef =
            defMgr.getItemDef(config.getNavTreeTypeIds().get(0), communityId);
        if (navonDef == null) {
        ...
        PSServerItem navon =
            new PSServerItem(navonDef, null, req.getSecurityToken()); // load default values
        ...
        setFieldValue(navon, themeField, new PSTextValue(navprops.getProperty("nav.theme")));

        String varField = config.getNavonVariableName();
        setFieldValue(navon, varField, new PSTextValue(navprops.getProperty("nav.variable")));
        ...
        rproc.add(
            PSRelationshipConfig.TYPE_FOLDER_CONTENT, children, folderSum.getCurrentLocator());
   ...
   * @param fieldName name of the field to set, assumed not <code>null</code> or empty.
  ...
  /**
   * Method used to recursively create some number of folders as children of the supplied <code>
   * parentFolderId</code>.
   *
   * @param curDepth Used as index into the <code>levelCounts</code> param. The value found at this
   *     index determines how many folders to create.
   * @param levelCounts How many folders to create at each depth of the folder tree. The size of
   *     this value is equal to the maximum depth of the tree. Assumed never <code>null</code> and
   *     <code>levelCounts</code>[<code>curDepth</code>] is valid.
   * @param parentFolderIds The ids of the folders to which the created folders will be added as
   *     children. Assumed not <code>null</code>. Which parents the created folders are added to are
   *     determined by calling {@link #randomizeFolderChildren(int, int) randomizeFolderChildren}.
   * @param rproc Used to perform all relationship operations. Assumed not <code>null</code>.
   * @param cproc Used to perform all component operations. Assumed not <code>null</code>.
   * @param folder Used as the template for creating all folders. All properties except the name
   *     will be 'inherited' by all created folders. The name will take the form "Lx_foldery", where
   *     x is the depth (1 based) and y is the folder number w/in a group of folders (1 based).
   * @param addedFolders Every folder added by this method is added to the supplied collection as a
   *     <code>PSLocator</code>. Assumed not <code>
   * null</code>.
   */
  private static void createFolderLevel(
    ...
    if (curDepth == 0) {
      ...
      folderDist = randomizeFolderChildren(levelCounts[curDepth - 1], levelCounts[curDepth]);
    }
    ...
    for (int i = 0; i < parentFolderIds.length; i++) {
      for (int j = 1; j <= folderDist[i]; j++) {
        String folderName = "L" + (curDepth + 1) + "-" + parentFolderIds[i].getId() + "_folder" + j;
        folder.setName(folderName);
        PSLocator id = createFolder(parentFolderIds[i], folder, rproc, cproc);
        parentIds[k++] = id;
        addedFolders.add(id);
      }
    }
    ...
    if (nextDepth < levelCounts.length) {
      createFolderLevel(nextDepth, levelCounts, parentIds, rproc, cproc, folder, addedFolders);
    }
  }

  /**
   * Randomly distributes the children among the parents and returns the # of children for each
   * parent.
   *
   ...
   * @return An array whose size is equal to <code>parentCount</code>. The sum of all values in this
   *     array is equal to <code>childCount</code>. Each value ranges from 0 to <code>childCount
   *     </code>.
   */
  ...
  /**
   * Calculates how many folders should be present at each level of a hierarchy given the provided
   * constraints. See for a description of the algorithm.
   *
   * @param count Total # of folders to exist in hierarchy. Assumed &gt;= <code>maxDepth</code>.
   * @param distribution A geometric weighting factor that determines how the files are distributed
   *     between the levels. Assumed &lt;= 1.
   * @param maxDepth The number of levels in the hierarchy at the deepest point. Assumed &gt;= 1.
   * @return An array of size <code>maxDepth</code>, where each entry in the array indicates how
   *     many folders to create at that level of the hierarchy. Never <code>null</code> or length
   *     &lt; 1 if assumptions are met.
   */
  private static int[] getFolderDistribution(int count, float distribution, int maxDepth) {
    ...
    for (int i = 1; i < maxDepth; i++) {
      ...
      if (foldersLeft < 0) foldersLeft = 0;
    }
    // Add remaining folders to last level (they could be randomly distributed)
    if (foldersLeft > 0) folderDist[maxDepth - 1] += foldersLeft;
    return folderDist;
  }

  /**
   * Duplicates all items whose content id is between <code>lowId</code> and <code>highId</code>,
   * inclusive. A dupe is made of the tip revision. The clones are made by asking the server to
   * clone each item.
   *
   ...
   * @return A set of <code>PSLocator</code>s for every clone created by this method. Never <code>
   *     null</code> or empty.
   */
  public static Collection dupeContent(PSLocator lowId, PSLocator highId, int count)
      throws PSException {
    int low = lowId.getId();
    ...
    for (int i = low; i <= high; i++) {
      for (int j = 0; j < count; j++) {
         ...
         *    Copy (3) of Itemtitle
         */
        req.setParameter(IPSHtmlParameters.SYS_CONTENTID, Integer.toString(i));
        req.setParameter(IPSHtmlParameters.SYS_REVISION, Integer.toString(1));
        dh.newCopy(req);
        results.add(
            new PSLocator(
  ...
  /**
   * Transitions the item identified by the supplied locator using the specified transition name.
   *
   * @param request The request context to use, assumed not <code>null</code>.
   * @param loc The locator of the item to transition, assumed not <code>null</code>.
   * @param wfAction The workflow action to perform, which is the transition name, assumed not
   *     <code>null</code> or empty.
   * @throws PSException if there are any errors.
   */
  private static void performTransition(IPSRequestContext request, PSLocator loc, String wfAction)
      throws PSException {
    PSItemDefManager defMgr = PSItemDefManager.getInstance();
    Map extraParams = new HashMap();
    extraParams.put(IPSHtmlParameters.SYS_COMMAND, PSWorkflowCommandHandler.COMMAND_NAME);
    extraParams.put("WFAction", wfAction);

    extraParams.put(IPSHtmlParameters.SYS_CONTENTID, Integer.toString(loc.getId()));
    extraParams.put(IPSHtmlParameters.SYS_REVISION, Integer.toString(loc.getRevision()));
    String url = defMgr.getTypeEditorUrl(defMgr.getItemContentType(loc));
    IPSInternalRequest ireq = request.getInternalRequest(url, extraParams, false);
    ireq.performUpdate();
  }

  /**
   * Saves the supplied folder to the system and adds the appropriate link to the parent folder..
   *
   * @param parentLocator The locator of the parent folder, assume not <code>null</code>.
  ...
  private static PSLocator createFolder(
    ...
    PSSaveResults results = cproc.save(new IPSDbComponent[] {folder});
    folder = (PSFolder) results.getResults()[0];
    PSLocator locator = (PSLocator) folder.getLocator();

    List locatorList = new ArrayList();
    locatorList.add(locator);
    rproc.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, parentLocator);
