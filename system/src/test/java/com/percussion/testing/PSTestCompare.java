...
public class PSTestCompare {
   ...
   */
  public static void assertEqualURLs(String a, String b) throws MalformedURLException {
  ...
  /**
   * Makes sure two objects are equal and have the same hash code. Used for testing hashCode() and
   * equals() behavior.
   */
  ...
  /**
   * Compare two values, throwing an assertion if the two values are not equal. First the values are
   * trimmed. Then they are modified with all extra whitespace turned into single space characters.
  ...
  public static void assertEqualIgnoringWhitespace(String exp, String act) {
    if (exp != null) assertNotNull(act, "actual value must not be null");
    if (act != null) assertNotNull(exp, "expected value must not be null");

    if (exp == null) assertNull(act, "actual value must be null");
    if (act == null) assertNull(exp, "expected value must be null");
