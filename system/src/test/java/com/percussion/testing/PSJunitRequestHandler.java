 ...
 * the Rx server.
 *
 * <p>The class can use it's own class loader to instance requested classes so no stopping/starting
 * of the server is needed when a change to the 'testing' class is made. The class loader is given a
 ...
 * feature.
 *
 * <p>The following should be added to the requestHandlers.xml config file.
 *
 * <pre>
 * &lt;RequestHandlerDef
...
public class PSJunitRequestHandler implements IPSLoadableRequestHandler {
   ...
   * See {@link IPSLoadableRequestHandler} interface for description.
   *
   * <p>This method will instantiate a class specified in the request and execute the specified
  ...
  public void processRequest(PSRequest request) {
    ...
    try {
      ...
      if (strDirName.trim().length() == 0) strDirName = strRootDev;

      ClassLoader classLoader = null;
      if (useLocalClassLoader) classLoader = createClassLoader(strDirName, strRootDev);
      ...
      try {
        if (strClassName.trim().length() > 0) {
          ...
          try {
             ...
             */
            System.out.println(
                "No suite() method; attempting JUnit Platform run for: " + strClassName);
                ...
                new TestExecutionListener() {
                  ...
                  public void executionStarted(
                    ...
                    if (testIdentifier.isTest()) {
                      ...
                      junit.framework.TestCase fakeTest =
                          new junit.framework.TestCase(display) {
                            @Override
                  ...
                  public void executionFinished(
                    ...
                    if (status == TestExecutionResult.Status.FAILED) {
                      Throwable ex =
                          testExecutionResult
                              .getThrowable()
                              .orElse(new AssertionError("Test failed"));
                      // Treat as a JUnit failure
                      result[0].addFailure(
                          fakeTest, new junit.framework.AssertionFailedError(ex.getMessage()));
                    } else if (status == TestExecutionResult.Status.ABORTED) {
                      Throwable ex =
                          testExecutionResult
                              .getThrowable()
                              .orElse(new RuntimeException("Test aborted"));
                      // Treat as an error
          ...
          if (result[0] == null) result[0] = new TestResult();
          Element testResultEl =
              createTestResultResponse(m_responseDoc, result[0], strClassName, outputText);
        ...
        PSXmlDocumentBuilder.addElement(m_responseDoc, root, CLASSNOTFOUNDOREMPTY, e.getMessage());
      } finally {
  ...
  private RefreshClassLoader createClassLoader(String strDir, String strRootName) {
    ...
    if (strDir.indexOf(strRootName) >= 0) f = new File(strDir);
    else f = new File(strRootName, strDir);
  ...
  private void addAllFiles(File f, List<File> l) {
    if (f == null || l == null) throw new IllegalArgumentException("Both f and l must not be null");

    if (f.getName().endsWith("class")) l.add(f);
   ...
   */
  private Element createTestResultResponse(
      Document doc, TestResult tr, String strMethod, String consoleOutput) {
    // Thresholds
    ...
    if (tr.wasSuccessful()) {
      ...
      while (failures.hasMoreElements()) {
        Element failureEl = PSXmlDocumentBuilder.addEmptyElement(doc, testResultEl, TESTFAILURE);
    ...
    PSXmlDocumentBuilder.addElement(
        doc, testResultEl, "ConsoleOutput", consoleOutput.substring(0, len));
