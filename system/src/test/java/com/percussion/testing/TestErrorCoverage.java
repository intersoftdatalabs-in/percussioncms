...
public class TestErrorCoverage {
   ...
   *
   * <p>First arg: filename of a properties file containing settings for this test run.
   *
   * @author chad loder
  ...
  public static void main(String[] args) {
    if (args.length < 1) printUsage(System.out);
  ...
  /**
   * Initializes and loads the properties from the file with the given name.
   *
   * @author chad loder
   * @version 1.0 1999/7/21
   * @param filename
  ...
  private static void runTests()
    ...
    if (ms_props == null) throw new IllegalStateException("properties have not been initialized");
  ...
  private static void testCoverage(String errorCodesClassName, Properties props)
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (message == null) {
        System.out.println(
            "No msg for\t" + errorCodesClassName + "." + field.getName() + "\t(" + code + ")");
      }
    }
  }
