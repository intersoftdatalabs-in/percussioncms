...
/**
 * This is a mock object to allow the running of a subset of code that requires a {@link
 * com.percussion.server.IPSRequestContext}. Note that right now only some of the methods provide
 * useful information. More may be added, along with set methods over time.
 *
 * @author dougrand
 */
public class PSMockRequestContext implements IPSRequestContext {
  ...
  public String getParameter(String name) {
    if (parameters.containsKey(name)) return parameters.get(name).toString();
    else return null;
  }
