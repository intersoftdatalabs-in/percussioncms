...
/**
 * Run the given test on the server using an HttpRequest. The server must be running the {@link
 * com.percussion.testing.PSJunitRequestHandler} loadable handler.
 *
 * <p>This class does not support the <code>useLocalClassLoader</code> property of the handler.
 */
public class PSRequestHandlerTestSuite extends TestSuite {
  ...
  */
   ...
   *
   * @param clazz The class to use when constructing the test suite. {@link TestSuite} uses
   *     reflection to find all the methods that start with "test" to construct a suite. Must never
   *     be <code>null</code>.
   */
  ...
  /**
   * Ctor that can be used if the {@link TestSuite#addTest(junit.framework.Test) addTest} methods
   * are needed.
   */
  ...
  /**
   * Invoke runs the test inside of Rhythmyx. It does this by using a special loadable request
   * handler. The request handler is available on the specific application sys_junitTestHandler. The
   * results are returned from Rhythmyx and this code then parses the results and reflects the
   * results onto the testResults object that this is called with.
   *
   * @param testResults A never <code>null</code> instance of {@link TestResult} that is supplied by
   *     junit and modified using the results from the run.
   */
  private void invoke(TestResult testResults) throws Exception {
    Properties connInfo =
        m_configHelper.getConnectionProps(IPSClientBasedJunitTest.CONN_TYPE_RXSERVER);
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (el.getNodeName().equals("ConsoleOutput")) {
        ...
        AssertionFailedError ex = new AssertionFailedError(message + " stack " + trace);
  ...
  /**
   * Look through the test list and return the correct test for the given name.
   *
   ...
   *
   * @return <code>true</code> if the code is running inside of the Rhythmyx server.
   */
  ...
  /**
   * Record the test class to be passed to the server. Initialized in the constructor and never
   * modified afterward. Never <code>null</code>.
   */
  private Class m_testClass;

  /** Config helper test case - a properties factory. */
  private PSConfigHelperTestCase m_configHelper = new PSConfigHelperTestCase("props");
}
