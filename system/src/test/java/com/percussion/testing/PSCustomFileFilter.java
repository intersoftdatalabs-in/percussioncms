...
/**
 * Object representation of the XML that defines an Ant custom filter using the <code>
 * PSJunitFileSelector</code> class. Allows selection of test classes using include/exclude
 * patterns. See {@link #toXml(Document)} for format of the XML.
 */
public class PSCustomFileFilter {
  ...
  /**
   * Construct a file filter from its Xml representation. See {@link #toXml(Document)} for format of
   * the XML.
   *
   * @param src The root element of the Xml representation, may not be <code>null</code>.
   */
  public PSCustomFileFilter(Element src) {
    ...
    Element paramEl = tree.getNextElement(PARAM_EL, tree.GET_NEXT_ALLOW_CHILDREN);
    while (paramEl != null) {
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(CUST_CLASS_NAME_ATTR, "com.percussion.ant.PSJunitFileSelector");
    ...
    if (m_filterClassPath != null)
      appendParams(
          doc,
          root,
          PSIteratorUtils.iterator(m_filterClassPath),
          FILTER_CLASS_PATH_ATTR,
          TYPE_PATH);

    // add package filters
    appendParams(doc, root, m_pkgIncludes.iterator(), PACKAGE_FILTER_ATTR, TYPE_INCLUDE);
    appendParams(doc, root, m_pkgExcludes.iterator(), PACKAGE_FILTER_ATTR, TYPE_EXCLUDE);

    // add class name filters
    appendParams(doc, root, m_classIncludes.iterator(), CLASS_NAME_FILTER_ATTR, TYPE_INCLUDE);
    appendParams(doc, root, m_classExcludes.iterator(), CLASS_NAME_FILTER_ATTR, TYPE_EXCLUDE);

    // add class instance filters
    appendParams(doc, root, m_instanceIncludes.iterator(), CLASS_IMPL_FILTER_ATTR, TYPE_INCLUDE);
    appendParams(doc, root, m_instanceExcludes.iterator(), CLASS_IMPL_FILTER_ATTR, TYPE_EXCLUDE);
   ...
   *
   * @param values The list of values to append, assumed not <code>null</code>, may be empty.
   ...
   */
  private void appendParams(Document doc, Element root, Iterator values, String name, String type) {
    while (values.hasNext()) {
      String filter = (String) values.next();
      Element param = PSXmlDocumentBuilder.addEmptyElement(doc, root, PARAM_EL);
  ...
  /**
   * Extracts a parameter from the supplied element and adds it to the appropriate list.
   *
   * @param paramEl The element from which the parameter is extracted. See {@link #toXml(Document)}
   *     for a description of the <code>param</code> element format expected. Assumed not <code>null
   *     </code>.
   */
  private void extractParam(Element paramEl) {
    ...
    if (TYPE_PATH.equalsIgnoreCase(type)) {
      if (name.equalsIgnoreCase(FILTER_CLASS_PATH_ATTR)) isPath = true;
      else throw new RuntimeException("Unsupported param name (" + name + ") for type: " + type);
  ...
  /**
   * Add a package filter. See {@link #toXml(Document)} for a description of how filters are used.
   *
   * @param filter The value of the filter to add, may not be <code>null</code> or empty.
   * @param isInclude <code>true</code> to specify an include filter, <code>false</code> to specify
   *     an exclude.
   */
  public void addPackageFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_pkgIncludes.add(filter);
    else m_pkgExcludes.add(filter);
  }

  /**
   * Add a class name filter. See {@link #toXml(Document)} for a description of how filters are
   * used.
   *
   * @param filter The value of the filter to add, may not be <code>null</code> or empty.
   * @param isInclude <code>true</code> to specify an include filter, <code>false</code> to specify
  ...
  public void addClassNameFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_classIncludes.add(filter);
    else m_classExcludes.add(filter);
  }

  /**
   * Add a class impl filter. See {@link #toXml(Document)} for a description of how filters are
   * used.
   *
   * @param filter The value of the filter to add, may not be <code>null</code> or empty.
   * @param isInclude <code>true</code> to specify an include filter, <code>false</code> to specify
  ...
  public void addInstanceFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_instanceIncludes.add(filter);
    else m_instanceExcludes.add(filter);
  }

  /**
   * Get the defined package include filters.
   *
   * @return An iterator over zero or more <code>String</code> objects, never <code>null</code>.
   */
  ...
  /**
   * List of package include filters, never null, may be empty. Packages are added by {@link
   * #PSCustomFileFilter(Element)}, and the list is modified by {@link #addPackageFilter(String,
   * boolean)} and {@link #clearAll()}.
   */
  ...
  /**
   * Specifies the classpath to provide as a param in the filter Xml output. May be <code>null
   * </code> if constructed using {@link #PSCustomFileFilter(Element)} and no path param is
   * specified. If constructed using {@link #PSCustomFileFilter()}, then it is initialized to {@link
   * #FILTER_CLASS_PATH_VALUE}. Never emtpy, immutable after construction.
   */
