...
public class PSQueryJoinerTest {
  ...
  public void testNormalJoin(int dataType) throws Exception {
    ...
    if (dataType == Types.BIT) matchingRows = 8400;
    else if (dataType == Types.NULL) matchingRows = 14400;
    else matchingRows = 3600;
    ...
    PSBackEndColumn leftCol = new PSBackEndColumn(leftTab, "leftCol");

    PSBackEndColumn rightCol = new PSBackEndColumn(rightTab, "rightCol");

    PSBackEndJoin join = new PSBackEndJoin(leftCol, rightCol);

    PSQueryJoiner joiner =
        new PSSortedResultJoiner(
            null, join, new String[] {"leftCol"}, null, new String[] {"rightCol"}, null, 100);
  ...
  /**
   * Test the joining on a random result set, making sure nothing blows up and that the data has the
  ...
  public void smokeTestJoin(int joinType) throws Exception {
    ...
    for (int i = 0; i < columnNames.length; i++) {
      schema.addColumnMetaData(new PSResultSetColumnMetaData(columnNames[i], columnTypes[i], 1));
    }
    ...
    PSBackEndColumn leftCol = new PSBackEndColumn(leftTab, columnNames[whichCol]);

    PSBackEndColumn rightCol = new PSBackEndColumn(rightTab, columnNames[whichCol]);
    ...
    PSQueryJoiner joiner =
        new PSSortedResultJoiner(
            null, join, columnNames, null, columnNames, null, leftCard * rightCard);
    ...
    try {
      if (joinType == PSBackEndJoin.BEJ_TYPE_LEFT_OUTER) {
        assertTrue(
            leftCard <= joinedRs.getNumRows(), "Left outer must be at least as large as left side");
      } else if (joinType == PSBackEndJoin.BEJ_TYPE_RIGHT_OUTER) {
        assertTrue(
      ...
      } else { // inner - ? what's the check here?!
      }
    } finally {
      leftRs.close();
      rightRs.close();
  ...
  /**
   * Creates a result set with the given number of rows, that conforms to the given schema. The data
   * will be more or less randomly generated.
   *
   * @author chad loder
   * @version 1.0 1999/8/1
   * @param schema
   ...
   * @return PSResultSet
   */
  protected static PSResultSet createResultSet(
      PSResultSetMetaData schema, int numRows, boolean sorted) throws SQLException {
  ...
  /**
   * Creates a random object of the corresponding SQL type. Not all types are supported; mostly
   * basic types.
   *
   * @author chad loder
   * @version 1.0 1999/8/1
