...
public class PSDataComparisonTest {
  ...
  public void testNumericComparisons() {
    ...
    try {
      ...
      if (ret != 0) fail("i1/s1 Integer/String comparison failed, expected equal: got " + ret);

      ret = PSDataConverter.compare(i1, s4);
      if (ret != 0) fail("i1/s4 Integer/String comparison failed, expected equal: got " + ret);

      ret = PSDataConverter.compare(i2, s2);
      if (ret != 0) fail("i2/s2 Integer/String comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(l1, s1);
      if (ret != 0) fail("l1/s1 Long/String comparison failed, expected equal: got " + ret);

      ret = PSDataConverter.compare(l2, s2);
      if (ret != 0) fail("l2/s2 Long/String comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(d1, s1);
      if (ret != 0) fail("d1/s1 Double/String comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(d2, s2);
      if (ret != 0) fail("d2/s2 Double/String comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(d3, s3);
      if (ret != 0) fail("d3/s3 Double/String comparison failed, expected equal: got " + ret);

      ret = PSDataConverter.compare(d1, i1);
      if (ret != 0) fail("d1/i1 Double/Integer comparison failed, expected equal: got " + ret);

      ret = PSDataConverter.compare(d2, i2);
      if (ret != 0) fail("d2/d2 Double/Integer comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(d1, l1);
      if (ret != 0) fail("d1/l1 Double/Long comparison failed, expected equal: got" + ret);

      ret = PSDataConverter.compare(d2, l2);
      if (ret != 0) fail("d2/l2 Double/Long comparison failed, expected equal: got" + ret);
