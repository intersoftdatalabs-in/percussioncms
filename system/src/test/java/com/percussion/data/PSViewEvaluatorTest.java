...
public class PSViewEvaluatorTest {
  /**
   * Tests view evaluator construction with valid and invalid data, checking for fields in views,
   * and getNextView
   *
  ...
  public void testViewEvaluator() throws Exception {
    ...
    Map<String, String> cgiVars = Collections.singletonMap(IPSHtmlParameters.SYS_VIEW, "VieW1");

    PSRequest req =
        PSRequestTest.makeRequest(
            "Rhythmyx/foo.html", "foo.html", params, cgiVars, null, null, null);
    ...
    viewSet.addView(new PSView(IPSConstants.DEFAULT_VIEW_NAME, fieldList.iterator()));
    ...
    assertFalse(eval.isFieldVisible("foobar", data), "field not visible");
    assertEquals(
        IPSConstants.DEFAULT_VIEW_NAME, eval.getNextView(data, 0), "getnextview: sys_Default");
    assertEquals(
        IPSConstants.DEFAULT_VIEW_NAME, eval.getNextView(data, 1), "getnextview: sys_Default");
