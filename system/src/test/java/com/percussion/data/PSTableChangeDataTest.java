...
public class PSTableChangeDataTest {
  ...
  /**
   * Tests the communication of table change data object with the listeners set with that object.
   * <br>
   * Tests the following cases with 2 listeners interested in different tables and actions.
   *
   * <ol>
   *   <li>The expectsColumn and addColumnValue methods should fire IllegalStateException before
   *       actioType is set on data object.
   *   <li>Test the expectsColumn method to return <code>true</code> only if the column of the table
   *       is in interest of any of the listeners for that particular action type.
   *   <li>Test for insert event by adding columns of the tables that are interested by two
   *       different listeners and make sure that the corresponding event is fired to the listener.
   *   <li>Test for update event and make sure that both listeners are notified as they are
   *       interested in the update event of the columns of particular table.
   *   <li>Test for delete event and make sure that only listener1 is notified as listener2 is not
   *       interested in the delete event of the particular table.
   * </ol>
   *
   * @throws Exception
   */
  ...
  public void testListenerComm() throws Exception {
    ...
    interestedChanges1.add(
        new TableChanges(PSTableChangeEvent.ACTION_INSERT, "CONTENTSTATUS", list));

    List list1 = new ArrayList();
    list1.add("STATEID");
    list1.add("CONTENTID");
    interestedChanges1.add(
        new TableChanges(PSTableChangeEvent.ACTION_UPDATE, "CONTENTAPPROVALS", list1));

    List list2 = new ArrayList();
    list2.add("USERNAME");
    interestedChanges1.add(
        new TableChanges(PSTableChangeEvent.ACTION_INSERT, "CONTENTADHOCUSERS", list2));
    interestedChanges1.add(
        new TableChanges(PSTableChangeEvent.ACTION_DELETE, "CONTENTADHOCUSERS", new ArrayList()));

    TableChangeListener listener1 = new TableChangeListener(interestedChanges1.iterator());

    List interestedChanges2 = new ArrayList();
    List list3 = new ArrayList();
    list3.add("DISPLAYTITLE");
    list3.add("AUTHORNAME");
    interestedChanges2.add(new TableChanges(PSTableChangeEvent.ACTION_INSERT, "RXARTICLE", list3));
    interestedChanges2.add(
        new TableChanges(PSTableChangeEvent.ACTION_UPDATE, "CONTENTAPPROVALS", list1));

    TableChangeListener listener2 = new TableChangeListener(interestedChanges2.iterator());
  ...
  private class TableChangeListener implements IPSTableChangeListener {
    ...
    public TableChangeListener(Iterator interestedChanges) {
      if (interestedChanges == null || !interestedChanges.hasNext())
        throw new IllegalArgumentException("interestedChanges may not be null or empty");
      m_interestedChanges = new ArrayList();
      while (interestedChanges.hasNext()) m_interestedChanges.add(interestedChanges.next());
    }
    ...
    public Iterator getColumns(String tableName, int actionType) {
      ...
      while (interestedChanges.hasNext()) {
        TableChanges change = (TableChanges) interestedChanges.next();
        if (change.getActionType() == actionType && change.getTableName().equals(tableName)) {
    ...
    /**
     * Make sures the correct event is fired and adds the table that is changed to its own list.
     * This is useful for further checking by test class to make sure only the table changed events
     * that are in interest of this listener got fired.
     *
    ...
    /**
     * Determines whether this listener is interested in supplied table and action.
     *
     ...
     * @return <code>true</code> if this listener is interested, otherwise <code>false</code>
     */
    ...
    /**
     * @return the list of changed tables, never <code>null</code> may be empty.
     */
    ...
    /**
     * The list of interested <code>TableChanges</code>, initialized in ctor and never <code>null
     ...
     * tableChanged</code> events are fired. Initialized to an empty list and never <code>null
     * </code> after that.
     */
  ...
  private class TableChanges {
     ...
     * @param columns The list of column names listener interested in, may not be <code>null</code>,
     *     but can be empty.
     */
    public TableChanges(int actionType, String tableName, List columns) {
      if (!PSTableChangeEvent.isValidAction(actionType)) {
        throw new IllegalArgumentException("Invalid actionType");
      }
      ...
      if (columns == null) throw new IllegalArgumentException("columns can not be null");
