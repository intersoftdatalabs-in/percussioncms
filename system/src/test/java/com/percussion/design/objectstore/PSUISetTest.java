...
public class PSUISetTest {
  ...
  /**
   * Tests that a cloned instance is equal to cloning instance. Assumes that <code>equal()</code> is
  ...
  /**
   * Test that merging a partial uiset with another and then diffing them produces the same result.
   *
   * @throws Exception if the test fails.
   */
  @Test
  public void testMergeAndDiff() throws Exception {
    ...
    uiSet.setControl(new PSControlRef("myControl"));
    uiSet.setCustomActionGroup(
        new PSCustomActionGroup(
            new PSLocation(PSLocation.PAGE_ROW_EDIT, PSLocation.TYPE_ROW),
            new PSActionLinkList(new PSActionLink(new PSDisplayText("linkText")))));
    uiSet.setDefaultSet("default");
    uiSet.setErrorLabel(new PSDisplayText("errorLabel"));
    uiSet.setLabel(new PSDisplayText("Author Age:"));
    uiSet.setName("set_1");
