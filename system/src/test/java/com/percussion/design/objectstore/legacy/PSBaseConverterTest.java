...
public abstract class PSBaseConverterTest extends PSBaseXmlConfigTest {
  ...
  public static final String TEST_LOGIN_CFG_FILE = "/com/percussion/utils/jboss/login-config.xml";

  /** Constant for the test ds config file. */
  public static final String TEST_DATASOURCE_FILE = "/com/percussion/utils/jboss/rx-ds.xml";
   ...
   */
  protected List<IPSDatasourceConfig> getDatasourceConfigs(PSConfigurationCtx ctx)
   ...
   * @throws PSInvalidXmlException If a configuration is bad.
   */
  protected PSDatasourceResolver getResolver(PSConfigurationCtx ctx) throws PSInvalidXmlException {
    ...
    PSDatasourceResolver resolver =
        (PSDatasourceResolver) springConfig.getBean(PSSpringConfiguration.DS_RESOLVER_NAME);
  ...
  protected IPSConfigFileLocator initFileLocator() throws Exception {
    ...
    return initFileLocator(
        PSTestResourceUtils.getFile(
                PSBaseConverterTest.class, PSLegacyServerConfigTest.TEST_LEGACY_CONFIG_FILE, null)
            .getAbsolutePath(),
        PSTestResourceUtils.getFile(
                PSBaseConverterTest.class, PSSpringConfigurationTest.TEST_BEANS_FILE, null)
            .getAbsolutePath(),
        PSTestResourceUtils.getFile(PSBaseConverterTest.class, TEST_DATASOURCE_FILE, null)
            .getAbsolutePath(),
        PSTestResourceUtils.getFile(PSBaseConverterTest.class, TEST_LOGIN_CFG_FILE, null)
            .getAbsolutePath());
  }
   ...
   * @throws Exception if there are any errors
   */
  protected IPSConfigFileLocator initFileLocator(
      String serverFile, String springFile, String jndiFile, String loginFile) throws Exception {
    if (serverFile == null) throw new IllegalArgumentException("serverFile may not be null");
    if (springFile == null) throw new IllegalArgumentException("springFile may not be null");
    if (jndiFile == null) throw new IllegalArgumentException("jndiFile may not be null");
    if (loginFile == null) throw new IllegalArgumentException("loginFile may not be null");
    ...
    // create the config
    return new PSConfigFileLocator(
        srcServerConfig, srcSpringConfig, srcJndiDSConfig, srcLoginConfig);
  }
  ...
  class PSConfigFileLocator implements IPSConfigFileLocator {
    ...
    private PSConfigFileLocator(
        File serverConfig, File springConfig, File jndiConfig, File loginConfig) {
