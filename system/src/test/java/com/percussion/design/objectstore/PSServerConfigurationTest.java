...
public class PSServerConfigurationTest {
  ...
  /**
   * Make sure that toXml() and fromXml() are consistent with equals. This means that if you
   * construct an object A, then call A.toXml (producing a document ADoc), and then you create a new
   * object B and call B.fromXml(ADoc) on it with the document, A.equals(B) should return true.
   *
   * <p>This means also that fromXml() should NOT create instances of child objects unless their
   * elements exist in the document. Likewise, toXml() should NOT put elements in the document for
  ...
  public void testToFromXml() throws Exception {
    ...
    List<PSJdbcDriverConfig> driverConfigs = new ArrayList<PSJdbcDriverConfig>();
    a.setJdbcDriverConfigs(driverConfigs);
    xmlEq(a, b);
    assertEquals(driverConfigs, a.getJdbcDriverConfigs());

    driverConfigs.add(new PSJdbcDriverConfig("driver1", "class1", "typeMap1"));

    a.setJdbcDriverConfigs(driverConfigs);
    xmlEq(a, b);
    assertEquals(driverConfigs, a.getJdbcDriverConfigs());

    driverConfigs.add(new PSJdbcDriverConfig("driver2", "class2", "typeMap2"));
    driverConfigs.add(new PSJdbcDriverConfig("driver3", "class3", "typeMap3"));
  ...
  /**
   * Tests to be sure security providers and group providers are acurately stored and serialized to
   * and from Xml.
   *
  ...
  public void testSecurityProviders() throws Exception {
    ...
    PSCollection secProviders = new PSCollection(PSSecurityProviderInstance.class);
    PSSecurityProviderInstance secProv1 =
        new PSSecurityProviderInstance("test1", PSSecurityProvider.SP_TYPE_DIRCONN);
    ...
    props1.setProperty(PSJndiProvider.PROPS_PROVIDER_CLASS_NAME, "pkg.myClass1");
    secProv1.setProperties(props1);
    PSReference ref1 = new PSReference("ref1", PSDirectorySet.class.getName());
    PSProvider directoryProvider =
        new PSProvider(
            PSBackEndTableDirectoryCataloger.class.getName(), PSProvider.TYPE_DIRECTORY, ref1);
  ...
  public void testAuthentications() throws Exception {
    ...
    PSAuthentication auth3 =
        new PSAuthentication(
            "auth3", PSAuthentication.SCHEME_SIMPLE, "user", "userAttr", "pw", "filter");

    PSAuthentication auth2replacement =
        new PSAuthentication(
            "auth2", PSAuthentication.SCHEME_SIMPLE, "user", "userAttr", "pw", null);
  ...
  public void testDirectories() throws Exception {
    ...
    PSGroupProviderInstance group1 =
        new PSJndiGroupProviderInstance("Group1", PSSecurityProvider.SP_TYPE_DIRCONN);
    PSGroupProviderInstance group2 =
        new PSJndiGroupProviderInstance("Group2", PSSecurityProvider.SP_TYPE_DIRCONN);
    ...
    PSGroupProviderInstance group3 =
        new PSJndiGroupProviderInstance("Group3", PSSecurityProvider.SP_TYPE_DIRCONN);
  ...
  public void testRoleProviders() throws Exception {
    ...
    PSRoleProvider prov3 =
        new PSRoleProvider("prov3", PSRoleProvider.TYPE_DIRECTORY, "directoryRef");

    PSRoleProvider prov2replacement =
  ...
  protected void xmlEq(PSServerConfiguration a, PSServerConfiguration b) throws Exception {
