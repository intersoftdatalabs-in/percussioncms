...
public class PSComponentTest {
  /**
   * Tests that all known subclasses of <code>PSComponent</code> have overridden the equals()
   * method.
   */
  @Test
  ...
  /**
   * Tests that the specified class has overridden Object's equal() method, using reflection.
   *
  ...
  private void testEqualsIsOverridden(String className) throws Exception {
    ...
    if (Modifier.isAbstract(theClass.getModifiers())) return;

    Method equalsMethod = theClass.getMethod("equals", new Class[] {Object.class});
