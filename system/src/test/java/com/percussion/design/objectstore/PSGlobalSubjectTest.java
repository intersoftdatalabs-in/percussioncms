...
/**
 * Basic global subject testing, including simple to/from Xml checking w/ simple global subject
...
public class PSGlobalSubjectTest {
   ...
   */
  public static boolean testGlobalSubjectEquals(PSGlobalSubject sub1, PSGlobalSubject sub2) {
   ...
   */
  public void assertGlobalSubjectEquals(PSGlobalSubject sub1, PSGlobalSubject sub2)
      throws Exception {
  ...
  /**
   * Test if two newly constructed global subjects (name, type, and sp ctor) are equal. Test
   * construction with invalid parameters.
   *
   * @throws Exception If any exceptions occur or assertions fail.
   */
  public void testNameTypeConstructor() throws Exception {
    PSGlobalSubject sub = new PSGlobalSubject("foo", PSSubject.SUBJECT_TYPE_USER, null);

    assertEquals(sub.getName(), "foo");

    PSGlobalSubject otherSub = new PSGlobalSubject("foo", PSSubject.SUBJECT_TYPE_USER, null);
    ...
    try {
      sub = new PSGlobalSubject(null, PSSubject.SUBJECT_TYPE_USER, null);
    ...
    try {
      ...
      sub = new PSGlobalSubject(name.toString(), PSSubject.SUBJECT_TYPE_USER, null);
    } catch (IllegalArgumentException e) {
      didThrow = true;
    }
    assertTrue(didThrow);
  ...
  /**
   * Test using accessors to set illegal values. Ensure they throw exceptions as appropriate.
   *
   * @throws Exception If any exceptions occur or assertions fail.
   */
