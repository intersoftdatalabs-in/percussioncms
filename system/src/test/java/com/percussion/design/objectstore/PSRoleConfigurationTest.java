...
/**
 * Basic role confiuration testing, including simple to/from Xml checking w/ basic
 * roleconfigurations.
 */
public class PSRoleConfigurationTest {
   ...
   * @return <code>true</code> if the supplied role configurations are equal, otherwise <code>false
   *     </code>.
   */
  public static boolean testRoleConfigurationEquals(
      PSRoleConfiguration rc1, PSRoleConfiguration rc2) {
    if ((rc1 == null) || (rc2 == null)) {
      if ((rc2 != null) || (rc1 != null)) return false;
    ...
    while (i1.hasNext()) {
      ...
      PSGlobalSubject gs2 = rc2.getGlobalSubject(gs1.makeRelativeSubject(), false);

      if (!PSGlobalSubjectTest.testGlobalSubjectEquals(gs1, gs2)) return false;
    }

    if (rc1.getRoles().size() != rc2.getRoles().size()) return false;
    ...
    while (i1.hasNext()) {
      ...
      while (i2.hasNext()) {
        ...
        if (role1.getName().equals(role2.getName())) {
          found = true;
          if (!PSRoleTest.testRoleEquals(role1, role2)) return false;
          break;
        }
      }
      if (!found) {
   ...
   */
  public void assertRoleConfigurationEquals(PSRoleConfiguration rc1, PSRoleConfiguration rc2)
      throws Exception {
  ...
  public void testXml() throws Exception {
    ...
    PSRelativeSubject sub = new PSRelativeSubject("sub1", PSSubject.SUBJECT_TYPE_USER, null);
    ...
    PSRelativeSubject sub2 = new PSRelativeSubject("sub2", PSSubject.SUBJECT_TYPE_USER, null);
    ...
    for (int i = gs.getAttributes().size() - 1; i >= 0; i--) gs.getAttributes().removeElementAt(i);
