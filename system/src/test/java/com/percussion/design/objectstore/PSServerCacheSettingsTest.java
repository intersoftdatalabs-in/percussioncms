...
public class PSServerCacheSettingsTest {
  ...
  /**
   * Performs basic tests with the constructors. Makes sure ctors provide default values to the
   * members of the object and rejects invalid parameters.
   */
  @Test
  public void testCtors() throws Exception {
    ...
    cacheSettings =
        new PSServerCacheSettings(true, true, 10 * 1024 * 1024, 100 * 1024 * 1024, 100 * 1024, 300);
  ...
  /**
   * Creates a <code>PSServerCacheSettings</code> object from the supplied values and asserts that
   * the constructor fails by throwing an <code>
   ...
   */
  private void testFailCtor(
      boolean enabled,
      boolean folderCacheEnabled,
      long memUsage,
      long diskUsage,
      long pageSize,
      long agingTime) {
    ...
    try {
      PSServerCacheSettings cacheSettings =
          new PSServerCacheSettings(
              enabled, folderCacheEnabled, memUsage, diskUsage, pageSize, agingTime);
   ...
   * PSServerCacheSettings</code> object from an xml element. <br>
   * List of test cases:
   *
   * <ol>
   *   <li>From xml, all members got initialized properly
   *   <li>Test Equals method, compare the one created from xml with the one created using
   *       parameters
   *   <li>Tests for the attributes whose values are numbers throws exception for non-number values
   *   <li>Tests for missing required attributes
   * </ol>
   *
   * @throws Exception
   */
  ...
  public void testXml() throws Exception {
    ...
    Element cacheEl =
        getXMLNode(
            ...
            "300");
    PSServerCacheSettings cacheSettings = new PSServerCacheSettings(cacheEl, null, null);
    assertTrue(cacheSettings.isEnabled());
    ...
    PSServerCacheSettings otherCacheSet =
        new PSServerCacheSettings(true, true, 10 * 1024 * 1024, 100 * 1024 * 1024, 100 * 1024, 300);
    assertEquals(cacheSettings, otherCacheSet);

    // maximum memory usage can not be a string
    testFailXml(
        true, true, "test", String.valueOf(100 * 1024 * 1024), String.valueOf(100 * 1024), "300");

    // maximum disk space usage can not be a string
    testFailXml(
        true, true, String.valueOf(10 * 1024 * 1024), "test", String.valueOf(100 * 1024), "300");
  ...
  /**
   * Creates a <code>PSServerCacheSettings</code> object from supplied source element and asserts
   * that the constructor fails by throwing a <code>
   * PSUnknownNodeTypeException</code>.
   *
   * @param source the source element to construct the object from, assumed not to be <code>null
   *     </code>
   */
  private void testFailXml(Element source) {
    boolean didThrow = false;
    try {
      PSServerCacheSettings cacheSettings = new PSServerCacheSettings(source, null, null);
  ...
  /**
   * Creates an xml element from the supplied values and calls {@link #testFailXml(Element)} to
   * assert that constructing {@link PSServerCacheSettings#PSServerCacheSettings(Element)
  ...
  private void testFailXml(
      ...
      String pageSize,
      String agingTime) {
    testFailXml(getXMLNode(enabled, folderCacheEnabled, memUsage, diskUsage, pageSize, agingTime));
  }

  /**
   * Creates a <code>PSXServerCacheSettings</code> element with supplied values as its attribute
   * values. Please see {@link PSServerCacheSettings#toXml} for the format of the element
   *
   * @param enabled the attribute value required for creating the element.
   * @param folderCacheEnabled the attribute value to enable/disable folder cache.
  ...
  private Element getXMLNode(
    ...
    if (enabled) root.setAttribute("enabled", "yes");
    else root.setAttribute("enabled", "no");
    if (folderCacheEnabled) root.setAttribute("folderCacheEnabled", "yes");
    else root.setAttribute("folderCacheEnabled", "no");
