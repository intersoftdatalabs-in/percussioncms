...
/**
 * Unit tests for the PSLogManager class. These are private and are not to be shipped with the
 * product.
 */
@TestInstance(Lifecycle.PER_CLASS)
public class PSLogManagerTest extends PSConfigHelperTestCase implements IPSServerBasedJunitTest {
  /*
  ...
  */
  ...
  /** Init should throw if you pass a null properties and a null location */
  @Test
  ...
  /**
   * For the first version of the server, we only support file logging with a file:/// type (local
   * file) URL. Make sure that init throws if the URL is of any other type.
   */
  public void testInitWithFilePropsRemoteURL() throws Exception {
    boolean didThrow = false;
    try {
      java.util.Properties props = (java.util.Properties) m_validFileProps.clone();
