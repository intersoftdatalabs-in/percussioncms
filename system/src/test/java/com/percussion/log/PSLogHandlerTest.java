...
public class PSLogHandlerTest {
  ...
  public void TestOptionsInitiallyOff() {
    ...
    assertFalse(
        handler.isDetailedUserActivityLoggingEnabled(), "Detailed user activity logging off");
  ...
  public void TestOptionsEnabling() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      else assertFalse(handler.isErrorLoggingEnabled(), "Error logging off");
      if (0 != (options & SERVER_STARTSTOP))
        assertTrue(handler.isServerStartStopLoggingEnabled(), "Server start/stop logging on");
      else assertFalse(handler.isServerStartStopLoggingEnabled(), "Server start/stop logging off");
      ...
      if (0 != (options & DETAILED_USER))
        assertTrue(
            handler.isDetailedUserActivityLoggingEnabled(), "Detailed user activity logging on");
      else
        assertFalse(
            handler.isDetailedUserActivityLoggingEnabled(), "Detailed user activity logging off");
      if (0 != (options & MULTIPLE_HANDLER))
        assertTrue(handler.isMultipleHandlerLoggingEnabled(), "Multiple handler logging on");
      else assertFalse(handler.isMultipleHandlerLoggingEnabled(), "Multiple handler logging off");
    } // end for
  }
}
