...
public class PSLogManagerThreadTest extends PSConfigHelperTestCase
  ...
  */
  ...
  /**
   * A private class for use in hammer() -- is a thread that periodically sleeps for a random
   * millisecond interval between 0 and waitLimit, then writes a message to the log manager and then
   * sleeps again, and so on...until it gets interrupted.
   */
    ...
    public void run() {
      ...
      try {
        while (System.currentTimeMillis() < endTime) {
          ...
          if (sleepFor < 0) sleepFor = -sleepFor;
          ...
          try {
            PSLogManager.write(new PSLogApplicationStart(0, m_messageData));
            flushPrint(
                "*** \tTestThread " + Thread.currentThread().toString() + " queued one message.");
            ...
            if (PSLogManager.isOpen()) throw e;
  ...
  private long hammer(int numThreads, int msecHammerFor) throws Exception {
    ...
    ThreadGroup threadGroup = new ThreadGroup("Hammer Threads");
    try {
  ...
  /**
   * Initialize with valid file properties and then spawn a bunch of threads that all write to the
   * log manager at random intervals.
   */
  ...
  /**
   * Initialize with valid DBMS properties and then spawn a bunch of threads that all write to the
   * log manager at random intervals.
   */
  ...
  private class DataTestFilter extends PSOutputLogReaderFilter {
    public DataTestFilter(
        PSLogManagerThreadTest parent,
  ...
  /**
   * Initialize with valid properties and then hammer. Then use the log reader to read messages and
   * make sure we only get the ones back that we wrote, and that the text is equal to what we put
   * in. Test that we get back *every* message we successfully wrote in the test period (by having
   * hammer() return the number of succesfully written messages and then putting a test in
   * DataTestFilter.processMessage() that compares logEntryNum with the return value from hammer()
   * when it gets a null (final) entry.
   */
    ...
    try {
      ...
      PSLogManager.read(
          new DataTestFilter(
              ...
              numWritten,
              System.err,
              PSOutputLogReaderFilter.OUTPUT_RAW,
              startTime,
              endTime,
              null));
    } catch (Exception e) {
  ...
  /**
   * Initialize with valid file properties and then hammer. Then use the log reader to read messages
   * and make sure we only get the ones back that we wrote, and that the text is equal to what we
   * put in. Test that we get back *every* message we successfully wrote in the test period (by
   * having hammer() return the number of succesfully written messages and then putting a test in
   * DataTestFilter.processMessage() that compares logEntryNum with the return value from hammer()
   * when it gets a null (final) entry.
   */
