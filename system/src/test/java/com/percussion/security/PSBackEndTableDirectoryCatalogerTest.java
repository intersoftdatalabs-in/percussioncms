...
public class PSBackEndTableDirectoryCatalogerTest {
  ...
  public void testCataloger() throws Exception {
    ...
    PSBackEndTableDirectoryCataloger cat = new PSBackEndTableDirectoryCataloger(props);

    // test user w/ single attribute
    assertEquals(
        DEFAULT_PASSWORD, cat.getAttribute("admin1", testAttr), "test user w/ single attribute");
    ...
    assertEquals(
        DEFAULT_PASSWORD,
        sub.getAttributes().getAttribute(testAttr).getValues().get(0),
        "test subject and attr list");
    ...
    cond =
        new PSConditional(
            new PSTextLiteral(cat.getObjectAttributeName()),
            PSConditional.OPTYPE_EQUALS,
            new PSTextLiteral("admin1"));
    ...
    cond =
        new PSConditional(
            new PSTextLiteral(testAttr),
            PSConditional.OPTYPE_EQUALS,
            new PSTextLiteral(DEFAULT_PASSWORD));
    subs = cat.findUsers(new PSConditional[] {cond}, attrs);
    ...
    cond =
        new PSConditional(
            ...
            new PSTextLiteral(DEFAULT_PARTIAL_PASSWORD + "%"));
    subs = cat.findUsers(new PSConditional[] {cond}, attrs);
