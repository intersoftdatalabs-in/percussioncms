...
public class PSContentItemDependencyTreeTest {
  ...
  public void testPublicInterface() throws Exception {
    ...
    try {
      InputStream is = new FileInputStream(new File(RESOURCE_PATH + "relatedcontentitemset.xml"));
      ...
      PSRelationshipConfig config =
          PSRelationshipTest.getConfigs()
              .getConfigByNameOrCategory(PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY);
      ...
      while (node != null) {
        PSRelationship relationship =
            new PSRelationship((Element) tree.getCurrent(), null, null, config);
  ...
  private static final String RESOURCE_PATH = "/com/percussion/server/cache/";

  /** Expected test results. */
  private static final String result_1 =
      "Dependencies(key= 7 value= dependency=(7, 6, 3, 1, 100), )key= 6 value= dependency=(6, 4, 3,"
          + " 1, 100), )key= 5 value= dependency=(5, 1, 4, 1, 100), )key= 4 value= dependency=(4,"
          + " 2, 2, 1, 100), )key= 3 value= dependency=(3, 5, 1, 1, 100), )key= 2 value="
          + " dependency=(2, 3, 1, 1, 100), )key= 1 value= dependency=(1, 7, 7, 1, 100), )";
