...
public class PSCacheStatisticsTest {
  ...
  /**
   * Tests that the cache statistics thread does not start unless the caching is enabled.
   *
   * @throws Exception
   */
  public void testCacheDisabled() throws Exception {
    ...
    // fire some events
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_ADDED, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_ADDED, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_STORED_TO_DISK, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_ACCESSED_FROM_MEMORY, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_ACCESSED_FROM_DISK, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_NOT_FOUND, id, test, keys, size);
  ...
  /**
   * Tests the <code>PSCacheStatistics</code> for statistics snapshot by firing different events.
   *
   * @throws Exception
   */
  public void testStatistics() throws Exception {
    ...
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_REMOVED, id, test, keys, size);
    fireEvent(statistics, PSCacheEvent.CACHE_ITEM_REMOVED, id, test, keys, size);
   ...
   * </code> object.
   *
   * @param statistics the object that needs to be notified, assumed not to be <code>null</code>
  ...
  private void fireEvent(
      PSCacheStatistics statistics, int action, int id, Object obj, Object[] keys, long size) {
    PSCacheEvent event = new PSCacheEvent(action, new PSCacheItem(id, obj, keys, size));
   ...
   *
   * @param statistics the object from which to get the snapshot, assumed not to be <code>null
   *     </code>.
   ...
   * @param diskHitRate the disk hit rate to access an item, assumed not to be < 0.
   * @param hitRate the cache hit rate to access an item, assumed not to be < 0.
   */
  private void assertSnapshotValues(
      PSCacheStatistics statistics,
      long memUsage,
      long totalHits,
      long diskUsage,
      long totalRequests,
