...
public class PSMultiLevelCacheTest {
  ...
  public void testAccess() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        for (int k = 0; k < 3; k++, x++) {
          addAndRetrieve(cache, new Object[] {i + "", j + "", k + ""}, Integer.valueOf(x), x + 1);
        }
      }
    }
  ...
  public void testFlush() throws Exception {
    ...
    assertNotNull(
        ...
        "retrieve item after flushing another");
    assertNotNull(
    ...
    assertNull(
        cache.retrieveItem(new Object[] {"0", "2", "0"}, CACHE_ITEM_TYPE),
        "retreive item in flushed branch returns null");
    assertNull(
        cache.retrieveItem(new Object[] {"0", "2", "1"}, CACHE_ITEM_TYPE),
        "retreive item in flushed branch returns null");
    assertNull(
    ...
    assertNull(
        cache.retrieveItem(new Object[] {"0", "0", "0"}, CACHE_ITEM_TYPE),
        "retrieve item after flush returns null");
    assertNull(
        cache.retrieveItem(new Object[] {"1", "0", "1"}, CACHE_ITEM_TYPE),
        "retrieve item after flush returns null");
    assertNull(
  ...
  public void testListeners() throws Exception {
    ...
    assertEquals(
        Integer.valueOf(PSCacheEvent.CACHE_ITEM_ADDED), m_modifyActions.get(0), "add event action");
    ...
    assertEquals(
        Integer.valueOf(PSCacheEvent.CACHE_ITEM_REMOVED),
        m_modifyActions.get(0),
    ...
    assertEquals(
        Integer.valueOf(PSCacheEvent.CACHE_ITEM_ADDED),
        m_modifyActions.get(1),
        "replace event action");
    assertEquals(
        "test2", ((PSCacheItem) m_modifyObjects.get(1)).getObject(), "replace event object");
  ...
  /**
   * Add and retrieves the supplied object using the supplied keys, comparing the result to be sure
   ...
   */
  private void addAndRetrieve(PSMultiLevelCache cache, Object[] keys, Object item, long size)
      throws Exception {
   ...
   * @throws Exception if the test fails or anything goes wrong.
   */
  private void addBadItem(PSMultiLevelCache cache, Object[] keys, Object item, long size)
      throws Exception {
   ...
   * @throws Exception if the test fails or anything goes wrong.
   */
  private void getBadItem(PSMultiLevelCache cache, Object[] keys) throws Exception {
    boolean didThrow = false;
    try {
