...
public class PSActionSetRequestHandlerTest {
  ...
  /**
   * Tests that <code>PSActionSetRequestHandler</code> can be constructed and that it will not
   * process requests until inited.
   *
   * <p>Unfortunately, init'ing the handler and processing requests require a running server, and
   * are therefore tested as an AutoTest instead of a UnitTest.
   */
  @Test
  public void testCtor() throws Exception {
    ...
    PSActionSetRequestHandler rh = new PSActionSetRequestHandler(osh, extMgr);

    // makes sure the requests cannot be proceed when handler not init
    boolean didThrow = false;
    try {
      rh.processRequest(PSRequestTest.makeRequest(null, null, null, null, null, null, null));
  ...
  private static final String RESOURCE_PATH = "/com/percussion/server/actions/";
}
