...
public class PSDatabaseAssemblerTest extends PSBaseXmlConfigTest {
  private static File ms_test =
      new File("UnitTestResources/com/percussion/tablefactory/multichild.xml");

  private static File ms_test_out =
      new File("UnitTestResources/com/percussion/tablefactory/multichild_out.xml");
  ...
  private void processTable(Node el, int index, PSJexlEvaluator eval) {
    ...
    if (index < 0) eval.bind("$db.parent", name);
    else eval.bind("$db.child[" + index + "]", name);
        ...
        for (int j = 0; j < colcount; j++) {
          ...
          if (colelname.equals("column")) {
            ...
            if (index < 0) {
              ...
              eval.bind("$child[" + index + "]." + columnname + "[" + rowindex + "]", colvalue);
            }
