...
public class PSAclEntryImplIteratorTest {
  ...
  public void testIterator() throws Exception {
    ...
    checkPerms(
        ...
        comms,
        new PSPermissions[] {
          PSPermissions.OWNER, PSPermissions.READ, PSPermissions.RUNTIME_VISIBLE
        });

    user = createUser("admin1", new String[] {"Group2"}, new String[] {"Admin"});
    checkPerms(
        testAcl,
        user,
        comms,
    ...
    user = createUser("admin2", new String[] {"Group2"}, new String[] {"Admin"});
    checkPerms(
        testAcl,
        user,
        comms,
    ...
    user = createUser("admin3", new String[] {"Group2"}, new String[] {"Admin"});
    checkPerms(
        testAcl,
        user,
        comms,
    ...
    user = createUser("admin3", new String[] {"Group2"}, new String[] {"Admin", "QA"});
    checkPerms(
        testAcl,
        user,
        comms,
    ...
    checkPerms(
        ...
        comms,
        new PSPermissions[] {
          PSPermissions.READ, PSPermissions.UPDATE, PSPermissions.RUNTIME_VISIBLE
        });
    ...
    comms.add("Test");
    checkPerms(
        testAcl, user, comms, new PSPermissions[] {PSPermissions.READ, PSPermissions.DELETE});
    ...
    checkPerms(
        ...
        user,
        comms,
        new PSPermissions[] {PSPermissions.READ, PSPermissions.UPDATE, PSPermissions.DELETE});
  ...
  /**
   * Check that the supplied user has the expected permissions from the given acl.
   *
  ...
  private void checkPerms(
      PSAclImpl acl, PSUserEntry user, Set<String> comms, PSPermissions[] permissions)
      throws Exception {
    ...
    Iterator<IPSAclEntry> iterator = new PSAclEntryImplIterator(acl, user, comms);
  ...
  private PSUserEntry createUser(String name, String[] groupNames, String[] roleNames) {
    ...
    PSUserEntry user = new PSUserEntry(name, 0, groups, roles, new PSUserAttributes(), "");
  ...
  private PSAclImpl createTestAcl() {
    PSAclEntryImpl owner = new PSAclEntryImpl(new PSTypedPrincipal("admin1", PrincipalTypes.USER));
