...
/**
 * Test most of query result, but not the node iterator (at least directly) since that requires a
 * servlet test case. That will be tested as part of the general query test.
 *
 * @author dougrand
 */
public class PSQueryResultTest {
  private static final String[] columns = {"rx:able", "rx:baker", "rx:charlie"};
  ...
  /**
   * Test that we are getting an appropriate result back, includes checking for count, and correct
   * behavior when incorrect properties are requested
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * As the comment says, test the sort ordering built into the query result implementation
   *
  ...
  /**
   * Make sure that something is coming back that seems correct for the nodes. A better test would
