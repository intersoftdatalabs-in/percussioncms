...
public class PSContentTypeMgrTest implements IPSCustomJunitTest {
  static IPSContentMgr mgr = PSContentMgrLocator.getContentMgr();

  /**
   * deserialize a node and delete,add a template association and save this is what msm does for a
   * node def
   */
  @Test
  ...
  public void testFindMultipleDefs() throws Exception {
    ...
    assertEquals(
        "A generic HTML page; includes local text plus Snippets of other Content Items.",
        def.getDescription());
  }
  ...
  /**
   * Verifies that every entry in the supplied list only occurs once within the list (using the
