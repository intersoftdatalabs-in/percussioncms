...
public class PSQueryParserTest {
  static String example =
      "select * from rx:press_release " + "where rx:type <> 3 order by rx:title asc";

  public void testSqlSimpleExpression() throws TokenStreamException, RecognitionException {
    ...
    assertEquals("rx:press_release", q.getTypeConstraints().iterator().next().getName());
    assertEquals("rx:title", q.getSortFields().iterator().next().getFirst().getName());
    assertEquals(PSQuery.SortOrder.ASC, q.getSortFields().iterator().next().getSecond());
  ...
  static String example2 =
      "select rx:title, rx:count, rx:url from rx:press_release " + "where rx:count > 3";

  public void testSqlComplexProjection() throws RecognitionException, TokenStreamException {
  ...
  static String example3 =
      "select * from rx:generic,rx:press_release,rx:brief "
          + "where rx:contentStartDate < '10/3/99' and rx:title like 'john%smith'";

  public void testSqlMoreStuff() throws RecognitionException, TokenStreamException {
  ...
  static String example4 =
      "select * from nt:root " + "where rx:a > 3 and rx:b < 4.5 or rx:c = -1 and rx:d < -.5";
  ...
  public void testSqlNumeric() throws Exception {
    ...
    PSTestCompare.assertEqualIgnoringWhitespace(
        "{ { rx:a GT 3 AND rx:b LT 4.5 } OR { rx:c EQ -1 AND rx:d LT -0.5 } } ", pp);
  }

  static String example5 =
      "select * from nt:root " + "where not rx:a > :avar and jcr:contains(rx:b,'foo')";
  ...
  static String example7 = "select * from nt:root " + "where not rx:a is null and rx:b is not null";
  ...
  public void testSqlNestedExpressions() throws Exception {
    ...
    PSTestCompare.assertEqualIgnoringWhitespace(
        "{ { { NOT  rx:a EQ <<null>> } AND  rx:b NE <<null>> } AND"
            + " { rx:c LT 3 OR { rx:d GT 5 AND  rx:e EQ 10 } } }",
        pp);
  }

  static String example9 =
      "select rx:displaytitle from rx:generic "
  ...
  public void testSqlSpecialCharacters() throws RecognitionException, TokenStreamException {
  ...
  public void testXpathAttrs() throws Exception {
    ...
    assertNotNull(q.getWhere());
    String pp = PSQueryNodePrinter.prettyPrint(q.getWhere());
    PSTestCompare.assertEqualIgnoringWhitespace("{ jcr:path LIKE //foo/bar AND rx:a GT :var }", pp);
  }
  ...
  public void testXpathSortOrder() throws Exception {
    ...
    assertEquals("rx:b", q.getSortFields().iterator().next().getFirst().getName());
    assertEquals(PSQuery.SortOrder.DSC, q.getSortFields().iterator().next().getSecond());
  }
  ...
  static String xpath7 =
      "[(@rx:x < 7 and @rx:y >= 9) or @rx:z = 3 and (@rx:a = @rx:b or @rx:a = @rx:z)]";
  ...
  public void testXpathLongerProjection() throws Exception {
    ...
    PSTestCompare.assertEqualIgnoringWhitespace("rx:x LT 6\n", pp);
    String proj = q.getProjection().toString();
    PSTestCompare.assertEqualIgnoringWhitespace("[id(rx:a), id(rx:b), id(rx:c)]", proj);
  }

  static String xpath10 =
      "/sites/enterpriseinvestments/element(*,rx:press_release)"
