...
public class PSDesignObjectAuditServiceTest {
  ...
  public void fixMetestConfig() throws Exception {
    IPSDesignObjectAuditService svc = PSDesignObjectAuditServiceLocator.getAuditService();
  ...
  public void fixMetestAuditEntries() throws Exception {
    ...
    // test create
    Date date = new Date();
    PSAuditLogEntry entry =
        createEntry(svc, date, new PSGuid(PSTypeEnum.ACL, 301), "admin1", AuditTypes.SAVE);
    ...
    entrySet1.add(
        createEntry(
            svc,
            cal.getTime(),
            new PSGuid(PSTypeEnum.CONTENT_LIST, 1002),
            "editor1",
            AuditTypes.SAVE));
    ...
    entrySet1.add(
        createEntry(
            svc, nextDate, new PSGuid(PSTypeEnum.CONTEXT, 1003), "editor2", AuditTypes.SAVE));

    cal.add(Calendar.DATE, 1);
    nextDate = cal.getTime();
    entrySet1.add(
        createEntry(
            svc, nextDate, new PSGuid(PSTypeEnum.EDITION, 1004), "author1", AuditTypes.DELETE));

    svc.saveAuditLogEntries(entrySet1);
    Set<PSAuditLogEntry> entrySet2 = new HashSet<PSAuditLogEntry>(svc.findAuditLogEntries());
  ...
  /**
   * Creates an entry, tests that is was assigned a guid, fills in the supplied values.
   *
   ...
   * @param user The name of the user to set, assumed not <code>null</code> or empty.
   * @param type The action type to set, assumed not <code>null</code>.
   * @return The entry, never <code>null</code>.
   * @throws Exception If there is an error.
   */
  private PSAuditLogEntry createEntry(
      IPSDesignObjectAuditService svc, Date date, IPSGuid guid, String user, AuditTypes type)
      throws Exception {
