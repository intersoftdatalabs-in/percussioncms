...
/**
 * The tests in this class assume the content and relationships in the fast forward sample content.
 * The exact ids may need to be updated over time.
 *
 * @author dougrand
 */
...
public class PSPublisherTouchItemsTest {
  ...
  static com.percussion.services.publisher.impl.PSPublisherService ps =
      (com.percussion.services.publisher.impl.PSPublisherService)
  ...
  /**
   * To find candidates for this test, run this. Note that this relies on slot ids < 300 being
   * inline. Change if needed. In the current FF implementation, this doesn't return anything out of
   * the box.
   *
   * <p>SELECT R.RID, C.RID, R.CONFIG_ID, R.SLOT_ID, R.OWNER_ID, R.OWNER_REVISION, CS.EDITREVISION,
   * R.DEPENDENT_ID, R.DEPENDENT_REVISION, C.DEPENDENT_ID GRANDCHILD FROM PSX_OBJECTRELATIONSHIP R
   * INNER JOIN PSX_OBJECTRELATIONSHIP C ON C.OWNER_ID = R.DEPENDENT_ID INNER JOIN CONTENTSTATUS CS
   * ON CS.CONTENTID = C.OWNER_ID WHERE (R.CONFIG_ID IN (1,2)) AND cs.CONTENTID = R.OWNER_ID AND
   * ((cs.EDITREVISION > 0 and r.OWNER_REVISION = cs.EDITREVISION) or (R.OWNER_REVISION =
   * CS.CURRENTREVISION)) AND (R.SLOT_ID < 300) AND (R.DEPENDENT_ID IN (SELECT P.OWNER_ID FROM
   ...
   *
   * <p>SELECT * FROM PSX_OBJECTRELATIONSHIP WHERE (CONFIG_ID IN (1,2)) AND (VARIANT_ID IN (SELECT
   * DISTINCT VARIANTID FROM RXVARIANTSLOTTYPE)) AND (DEPENDENT_ID IN (SELECT OWNER_ID FROM
   * PSX_OBJECTRELATIONSHIP AS PSX_OBJECTRELATIONSHIP_1)) ORDER BY DEPENDENT_ID
   *
   * @throws Exception
   */
  @Test
  ...
  public void testPerformance() throws Exception {
    ...
    ps.touchContentItems(items);
    sw.stop();
    ms_log.info("Touch multiple items took " + sw + " for " + items.size() + " items");
