...
public class PSObjectLockServiceTest {
  ...
  public void testLockManager() throws Exception {
    ...
    IPSSystemService systemService = PSSystemServiceLocator.getSystemService();
    ...
    for (PSSharedProperty property : properties) propertyIds.add(property.getGUID());
    ...
    List<PSObjectLock> createdLocks = service.findLocksByObjectIds(propertyIds, session, locker);
    assertTrue(createdLocks != null);
    assertTrue(createdLocks.size() == 3);

    // update lock 1 so that it expires in 1000ms
    service.extendLock(
        properties.get(1).getGUID(), session, locker, properties.get(1).getVersion(), 1000);
    ...
    List<PSObjectLock> remainingLocks = service.findLocksByObjectIds(propertyIds, session, locker);
    assertTrue(remainingLocks != null);
    assertTrue(remainingLocks.size() == 2);

    // update lock 2 so that it expires in 1000ms
    service.extendLock(
        properties.get(2).getGUID(), session, locker, properties.get(1).getVersion(), 1000);
    ...
    service.createLock(
        properties.get(2).getGUID(), "session2", "locker2", properties.get(0).getVersion(), false);
   ...
   * @param session the current session, assumed not <code>null</code> or empty.
   * @param locker the user for which to lock, assumed not <code>null</code> or empty.
   * @return all created locks, never <code>null</code>, may be empty.
   * @throws Exception for any error locking the supplied properties.
   */
  private List<PSObjectLock> createLocks(
      List<PSSharedProperty> properties, String session, String locker) throws Exception {
    IPSObjectLockService service = PSObjectLockServiceLocator.getLockingService();
    ...
    locks.add(
        service.createLock(
            properties.get(0).getGUID(), session, locker, properties.get(0).getVersion(), false));
    locks.add(
        service.createLock(
            properties.get(1).getGUID(), session, locker, properties.get(1).getVersion(), false));
    locks.add(
        service.createLock(
            properties.get(2).getGUID(), session, locker, properties.get(2).getVersion(), false));
