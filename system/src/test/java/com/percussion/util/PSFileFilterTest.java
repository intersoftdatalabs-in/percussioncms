...
/** Unit tests for the PSFileFilter class TODO: date and size testing */
public class PSFileFilterTest {
  ...
  public void testAttributes() {
    ...
    filt =
        new PSFileFilter(PSFileFilter.IS_HIDDEN | PSFileFilter.IS_FILE | PSFileFilter.IS_DIRECTORY);
    files = m_testDir.listFiles((java.io.FileFilter) filt);
    assertEquals(m_fileNames.length + m_dirNames.length, files.length);
  }
  ...
  public void testNamePatterns() {
    ...
    PSFileFilter filt = new PSFileFilter(PSPatternMatcher.FileWildcardMatcher("*a*"));
    File[] files = m_testDir.listFiles((java.io.FileFilter) filt);
    assertEquals(m_fileNames.length + m_dirNames.length, files.length);

    // only allow files/dirs that start witg 'a' (there are 9 of them)
    filt = new PSFileFilter(PSPatternMatcher.FileWildcardMatcher("a*"));
  ...
  public void setUp() {
    ...
    try {
      ...
      m_dirNames = new String[] {"ad", "dda", "dad", "dada", "daad", "da", "adad"};
