...
public class PSSqlHelperTest {
  /**
   * This method tests PSSqlHelper.parseSqlName by supplying the various possibilities of qname and
   * verifying that it is parsed correctly.
   */
  ...
  public void testJdbcUrlGen() {
    assertEquals(
        "jdbc:oracle:thin:serverName", PSSqlHelper.getJdbcUrl("oracle:thin", "serverName"));
  }

  /**
   * The generic test cases for parsing each qualified table name. It verifies that <code>null
   * </code> buffers is OK, then it checks that the qname was parsed into the expected pieces. All
   * params are assumed not <code>null
   * </code>.
   *
   * @param driver The JDBC sub-protocol (e.g. oracle:thin, odbc).
   * @param qName The possibly qualified name to be tested. The format is driver dependent. Each
   *     name part in the qname should be represented by the corresponding expected type param.
   * @param expectedTable This is the table name part in the supplied qname. May be empty.
   * @param expectedOrigin This is the schema or owner part in the supplied qname. May be empty.
   * @param expectedCatalog This is the database part in the supplied qname. May be empty.
   */
  private void testQName(
      String driver,
      String qName,
    ...
    assertTrue(
        ...
        driver + " couldn't parse table from " + qName + ".");
    ...
    assertTrue(
        ...
        driver
            + " incorrectly parsed the origin. Expected '"
            + expectedOrigin
            + "' got '"
            + origin
            + ".");
    ...
    assertTrue(
        ...
        driver
            + " incorrectly parsed the catalog. Expected '"
            + expectedCatalog
            + "' got '"
            + cat
            + ".");
