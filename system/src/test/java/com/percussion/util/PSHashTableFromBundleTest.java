...
/**
 * Unit tests for the PSHashTableFromBundle class. This class is private and is not meant to be
 * shipped with the product.
 */
public class PSHashTableFromBundleTest {

  /**
   * Test with a valid resource bundle file using the default locale. Make sure the returned
   * hashtable is equal to the known valid hashtable that is contained in that .properties file.
   */
  public void testConstructorWithValidBundleAndDefaultLocale() {
    PSHashTableFromBundle table = new PSHashTableFromBundle(m_validBundleName, Locale.getDefault());
  ...
  /**
   * Test with a valid resource bundle file using no locale. Make sure the returned hashtable is
   * equal to the known valid hashtable that is contained in that .properties file.
   */
  public void testConstructorWithValidBundleAndNoLocale() {
    PSHashTableFromBundle table = new PSHashTableFromBundle(m_validBundleName);
  ...
  /**
   * Test with an existing but invalid bundle (one that has keys which cannot be represented as
   * integers). Make sure it returns null.
   */
  public void testConstructorWithInvalidBundleAndDefaultLocale() {
    boolean didThrow = false;
    try {
      PSHashTableFromBundle table =
          new PSHashTableFromBundle(m_invalidBundleName, Locale.getDefault());
  ...
  public void testConstructorWithInvalidBundleAndNoLocale() {
    boolean didThrow = false;
    try {
      PSHashTableFromBundle table = new PSHashTableFromBundle(m_invalidBundleName);
    } catch (NumberFormatException nfe) {
      didThrow = true;
    }
  ...
  /** Test with a non existent bundle and make sure it throws MissingResourceException */
  public void testConstructorWithNonExistentBundleAndDefaultLocale() {
    boolean didThrow = false;
    try {
      PSHashTableFromBundle table =
          new PSHashTableFromBundle(m_nonExistentBundleName, Locale.getDefault());
  ...
  public void testConstructorWithNonExistentBundleAndNoLocale() {
    boolean didThrow = false;
    try {
      PSHashTableFromBundle table = new PSHashTableFromBundle(m_nonExistentBundleName);
    } catch (MissingResourceException e) {
      didThrow = true;
    }
