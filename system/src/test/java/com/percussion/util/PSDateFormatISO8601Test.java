...
public class PSDateFormatISO8601Test {
  ...
  /**
   * Midnight is a weird case -- there could be two different representations (i.e., July 3rd,
   * 24:00:00 is the same time as July 4th, 00:00:00). We should prefer the more standard 00:00:00
   * notation (that is, the next day's midnight), so we test that we use the next day (and if on New
   * Year's, exactly when the big apple falls in Times Square, use the next year).
   */
  ...
  /**
   * Test the time formatting for the current time, breaking the formatted string up into substrings
   * and then reparsing them and comparing them to the current time's numeric values.
   */
