...
/**
 * The PSFormatVersionTest is the unit test for the PSFormatVersion class which handles build
 * version string formatting operations.
 */
public class PSFormatVersionTest {
  ...
  public void testCreateFromXml() throws Exception {
    Document withDisplayDoc =
        PSXmlDocumentBuilder.createXmlDocument(new StringReader(FORMAT_VERSION_DISPLAY), false);

    PSFormatVersion fv = PSFormatVersion.createFromXml(withDisplayDoc.getDocumentElement());
    ...
    Document noDisplayDoc =
        PSXmlDocumentBuilder.createXmlDocument(new StringReader(FORMAT_VERSION_NO_DISPLAY), false);

    PSFormatVersion fv2 = PSFormatVersion.createFromXml(noDisplayDoc.getDocumentElement());
  ...
  /**
   * Ensures that the following properties are equal for two <code>PSFormatVersion</code> objects:
   *
   * <p>buildid, buildnumber, displayversion, optionalid, versionstring, interfaceversion,
   ...
   * @throws Exception
   */
  private void checkEqualProperties(PSFormatVersion fv1, PSFormatVersion fv2) throws Exception {
