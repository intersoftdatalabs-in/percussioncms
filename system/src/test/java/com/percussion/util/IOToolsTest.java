...
public class IOToolsTest {
  ...
  /** Ensures the directories contain the same number of files and the files have the same name. */
  private void checkEqualDirs(final File dir1, final File dir2) {
    ...
    for (int i = 0; i < dir1.listFiles().length; i++) {
      assertTrue(dir1.listFiles()[i].getName().equals(dir2.listFiles()[i].getName()));
    }
  }
