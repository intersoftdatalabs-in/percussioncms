...
/**
 * This is a unit test for {@link PSWebServiceAgent}. It can also be used to perform some SOAP
 * operations as a sample program.
 */
public class PSWebServiceAgentTest {
   ...
   */
  public PSWebServiceAgentTest(Properties props) throws PSUnknownNodeTypeException {
    m_props = props;
    ...
    m_wsAgent = new PSWebServiceAgent(protocol, host, port, username, password);
   ...
   *
   * @throws PSUnknownNodeTypeException if error occurs.
   */
  private void login() throws PSUnknownNodeTypeException {
    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    Element msg = doc.createElement(LOGIN_REQUEST);
    Element data = sendMessage(LOGIN_OPERATION, XML_NS_MISC, msg, LOGIN_RESPONSE);

    if (data != null) {
      Element loginDataEl = PSXMLDomUtil.getFirstElementChild(data, "LoginData");
   ...
   *
   * @param locator the locator of the item, assume not <code>null</code>.
   * @param transitionId the transition id.
   * @param transitionName the name of the transition, assume not <code>null</code> or empty.
   * @throws PSUnknownNodeTypeException if error occurs.
   */
  private void doTransition(PSLocator locator, String transitionId, String transitionName)
    ...
    Element data =
        sendMessage(TRANSITIONITEM_OPERATION, XML_NS_WORKFLOW, msg, TRANSITIONITEM_RESPONSE);

    Element resultEl = PSXMLDomUtil.getFirstElementChild(data, "ResultResponse");
    String type = resultEl.getAttribute("type");
    if (!type.equalsIgnoreCase("success")) {
      String errormsg = "FAILURE: transition -> " + transitionName + "(" + transitionId + ")";
      writeToLog(errormsg);
  ...
  /**
   * Performs a number of transitions that is specified in the properties of <code>m_prop</code>.
   *
   * @throws PSUnknownNodeTypeException
   */
  private void performTransitions() throws PSUnknownNodeTypeException {
    ...
    int iteration = Integer.parseInt(m_props.getProperty("numberOfTransitionCycles"));

    PSLocator locator = new PSLocator(Integer.parseInt(contentId), Integer.parseInt(revision));
   ...
   *
   * @param operation the SOAP method to call
   * @param nameSpace the namespace where the method lives, ex. design, search
   * @param message the XML body message to send
   * @param respNodeName the response XML node for the operation
  ...
  private Element sendMessage(
    ...
    try {
      return m_wsAgent.sendSoapBody(
          m_wsAgent.getSoapBodyForParams(operation, nameSpace, message), respNodeName);
    } catch (Exception ex) {
      writeToLog("Send SOAP Message Error - " + "'" + operation + "' - " + ex.getMessage());
      return null;
    }
  }
  ...
  private static Properties loadProperties() {
    ...
    try {
      in = PSWebServiceAgentTest.class.getResourceAsStream(DEFAULT_PROPERTIES_FILE);
      props.load(in);
  ...
  /**
   * The parameter properties, which is loaded from <code>DEFAULT_PROPERTIES_FILE</code>. Init by
   * ctor, never <code>null</code> after that.
   */
  private Properties m_props = new Properties();

  /** The webservice agent object, init by ctor, never <code>null</code> after that. */
  private PSWebServiceAgent m_wsAgent = null;

  /** The Rhythmyx location, init by the first login, never <code>null</code> after that. */
  private String m_RxLocation = null;

  /** The session id, init by the first login, never <code>null</code> after that. */
  ...
  private static final String XML_NS_CONTENTDATA = XML_NS_PREFIX + "contentdata";
