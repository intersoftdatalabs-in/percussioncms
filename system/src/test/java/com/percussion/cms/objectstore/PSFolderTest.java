...
public class PSFolderTest {
  ...
  public void testAll() throws Exception {
    ...
    PSFolder emptyFolder =
        new PSFolder("f1", 10, -1, PSObjectPermissions.ACCESS_ADMIN, "description");
    doc = PSXmlDocumentBuilder.createXmlDocument();
  ...
  public void testMerge() throws Exception {
    ...
    PSObjectAclEntry entry =
        new PSObjectAclEntry(
            1, PSObjectAclEntry.ACL_ENTRY_TYPE_USER, "qa1", PSObjectAclEntry.ACCESS_READ);
    acl.add(entry);
    entry =
        new PSObjectAclEntry(
            1, PSObjectAclEntry.ACL_ENTRY_TYPE_USER, "admin1", PSObjectAclEntry.ACCESS_ADMIN);
    acl.add(entry);
    ...
    entry =
        new PSObjectAclEntry(
            PSObjectAclEntry.ACL_ENTRY_TYPE_USER, "qa1", PSObjectAclEntry.ACCESS_WRITE);
    acl.add(entry);
    entry =
        new PSObjectAclEntry(
            PSObjectAclEntry.ACL_ENTRY_TYPE_USER, "admin2", PSObjectAclEntry.ACCESS_ADMIN);
    acl.add(entry);
  ...
  public void testComponentSummary() throws Exception {
    PSComponentSummary compSummary =
  ...
  /**
   * Test if folders with only different locators have different hashcodes, as they should. Also
   * tests the use case of putting several folders in a Set.
   *
   * @throws Exception on error
   */
  @Test
  public void testFolderHashCodeAndEquals() throws Exception {
    PSFolder folder1 = new PSFolder("f1", 10, -1, PSObjectPermissions.ACCESS_ADMIN, "description");
    folder1.setProperty("p1", "value1");
    folder1.setProperty("p2", "value2", "desc2");
    folder1.setLocator(new PSLocator(1, -1));
    PSFolder folder2 = new PSFolder("f1", 10, -1, PSObjectPermissions.ACCESS_ADMIN, "description");
    folder2.setProperty("p1", "value1");
    folder2.setProperty("p2", "value2", "desc2");
    folder2.setLocator(new PSLocator(2, -1));
    PSFolder folder3 = new PSFolder("f1", 10, -1, PSObjectPermissions.ACCESS_ADMIN, "description");
