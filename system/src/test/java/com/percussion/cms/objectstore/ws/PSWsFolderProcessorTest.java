...
public class PSWsFolderProcessorTest extends PSClientTestCase {
  ...
  public static void checkRemoteServerAvailable() {
    try {
      ...
      org.junit.jupiter.api.Assumptions.assumeTrue(
          false, "Remote Rhythmyx server not available - skipping PSWsFolderProcessorTest");
    }
  }
   ...
   * @throws Exception
   */
  private static PSComponentProcessorProxy getRemoteComponentProxy() throws Exception {
    PSRemoteRequester requester = getRemoteRequester();

    PSComponentProcessorProxy proxy =
        new PSComponentProcessorProxy(PSComponentProcessorProxy.PROCTYPE_REMOTE, requester);
   ...
   * @throws Exception
   */
  private static PSRelationshipProcessorProxy getRemoteRelationshipProxy() throws Exception {
    PSRemoteRequester requester = getRemoteRequester();
   ...
   */
  public static void testCopyFolderChildrenItem(PSLocator itemLocator) throws Exception {
    ...
    int communityId = -1;
    PSFolder folder =
        new PSFolder("folder1", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder1");
    folder.setProperty("p1", "v1");
    folder.setProperty("p2", "v2");

    PSFolder folder2 =
        new PSFolder("folder2", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder 2");
    PSFolder folder3 =
        new PSFolder("folder3", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder 3");
    ...
    PSSaveResults results = compProxy.save(new IPSDbComponent[] {folder, folder2, folder3});
    ...
    relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator);
    ...
    relProxy.copy(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator2);
  ...
  /**
   * Get a remote request object, which can be used to communicate to remote Rhythmyx.
   *
  ...
  public void testSearch() throws Exception {
    ...
    IPSExecutableSearch search =
        PSExecutableSearchFactory.createExecutableSearch(requester, names, ids);
  ...
  public void testRelationship() throws Exception {
    ...
    assertTrue(
        summaries.length == 1 && summaries[0].getName().equalsIgnoreCase(PSFolder.ROOT_TITLE));
  }
  ...
  public void fixme_testFolderProcessor() throws Exception {
    ...
    PSComponentSummary summary =
        relProxy.getSummaryByPath(folderType, "/", PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    assertTrue(summary != null && summary.getName().equalsIgnoreCase(PSFolder.ROOT_TITLE));

    summary =
        relProxy.getSummaryByPath(folderType, "//Sites", PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    ...
    String[] paths =
        relProxy.getRelationshipOwnerPaths(
            folderType, summary.getCurrentLocator(), PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    assertTrue(paths.length == 1 && paths[0].equalsIgnoreCase("//Sites/EnterpriseInvestments"));

    paths =
        relProxy.getRelationshipOwnerPaths(
            folderType, new PSLocator(301, 1), PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    assertTrue(paths.length == 1 && paths[0].equalsIgnoreCase("//Sites"));

    paths =
        relProxy.getRelationshipOwnerPaths(
            folderType, new PSLocator(2, 1), PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    assertTrue(paths.length == 1 && paths[0].equalsIgnoreCase("/"));

    paths =
        relProxy.getRelationshipOwnerPaths(
            folderType, new PSLocator(1, 1), PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    ...
    PSFolder folder3 =
        new PSFolder("folder3", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder 3");
    PSFolder folder4 =
        new PSFolder("folder4", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder 4");
    PSFolder folder5 =
        new PSFolder("folder5", communityId, PSObjectPermissions.ACCESS_ADMIN, "this is folder 5");
    ...
    try {
      summaries =
          relProxy.getParents(folderType, PSServerFolderProcessor.FOLDER_RELATE_TYPE, rootLocator);
    ...
    summaries =
        relProxy.getParents(folderType, PSRelationshipConfig.TYPE_FOLDER_CONTENT, sitesLocator);
    ...
    PSSaveResults results =
        compProxy.save(new IPSDbComponent[] {folder, folder2, folder3, folder4, folder5});
    ms_log.info("create folder successful");
    ...
    relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator4);
    ...
    relProxy.copy(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator);
    ms_log.info("copy folder children successful");
    ...
    locatorList.clear();
    locatorList.add(locator3);
    relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator5);
    ...
    locatorList.clear();
    locatorList.add(locator5);
    relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, locator3);
  ...
  public void performCreateFolder() throws Exception {
    ...
    createSiteFolder(
        ...
        "newFolderAdmin2.txt",
        355,
        5403); // there are 5049 items, from 355 to 5403
  }

  /**
   * Creates folders for the given parameters.
   *
   * @param parentPath The path of the parent folder, assume not <code>null</code> or empty.
   * @param siteName The name of the site, assume not <code>null</code> or empty.
   * @param dataFile The name of the data file that is used to create folders, assume not <code>null
   *     </code> or empty.
   * @throws Exception if error occurs.
   */
  private void createSiteFolder(
      String parentPath, String siteName, String dataFile, int firstContentId, int lastContentId)
      throws Exception {
    FolderInfo info = new FolderInfo(siteName, dataFile, firstContentId, lastContentId);
    PSRelationshipProcessorProxy relProxy = getRemoteRelationshipProxy();
    ...
    for (int i = 1; i <= 25; i++) {
      ...
      if (i >= 6 && i <= 15) {
        ...
        for (int j = 1; j <= 5; j++) createFolder(locator, 2, info);
      }
      // create L1 folder, 16-25 => 10 sub-folder
      else if (i >= 16 && i <= 25) {
        for (int j = 16; j <= 25; j++) createFolder(locator, 2, info);
      }
    }
    ...
    for (int i = 76; i <= 100; i++) {
      for (int j = 0; j < 3; j++) createFolder(info.m_l3Locators[i - 1], 4, info);
    }
    ...
    for (int i = 91; i <= 100; i++) {
      for (int j = 0; j < 3; j++) createFolder(info.m_l4Locators[i - 1], 5, info);
    }
   ...
   * @throws Exception
   */
  private void assignItemToFolder(FolderInfo info) throws Exception {
    ms_log.info(
        "Processing L1 folders, next available content id: " + info.m_nextContentId + " ...");
    ...
    ms_log.info(
        "Processing L2 folders, next available content id: " + info.m_nextContentId + " ...");
    ...
    ms_log.info(
        "Processing L3 folders, next available content id: " + info.m_nextContentId + " ...");
    ...
    ms_log.info(
        "Processing L4 folders, next available content id: " + info.m_nextContentId + " ...");
    ...
    ms_log.info(
        "Processing L5 folders, next available content id: " + info.m_nextContentId + " ...");
   ...
   * @param begin The first index of the <code>parents</code>, assume greater than zero, less than
   *     <code>end</code>.
   * @param end The last index of the <code>parents</code>, assume greater than <code>begin</code>.
   * @param itemCount The number of items need to be added to each folder.
   * @param info The folder info, assume not <code>null</code>.
   * @throws Exception
   */
  private void addItemsToFolder(
      PSLocator parents[], int begin, int end, int itemCount, FolderInfo info) throws Exception {
    PSRelationshipProcessorProxy relProxy = getRemoteRelationshipProxy();
    ...
    for (int i = begin; i <= end; i++) {
      ...
      relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, childLocators, parents[i]);
    }
  }
   ...
   * @throws Exception if error occurs.
   */
  private PSLocator createFolder(PSLocator parentLocator, int level, FolderInfo info)
      throws Exception {
    String name =
        "F"
   ...
   *
   * @param parentLocator The locator of the parent folder, assume not <code>null</code>.
   * @param folderName The name of the created folder, assume it contains "_L1_", "_L2_", "_L3_",
   *     "_L4_", or "_L5_".
   * @param info The folder info, assume not <code>null</code>.
   * @return The locator of the created folder, never <code>null</code>.
   * @throws Exception if error occurs.
   */
  private PSLocator createFolder(PSLocator parentLocator, String folderName, FolderInfo info)
      throws Exception {
    ...
    relProxy.add(PSRelationshipConfig.TYPE_FOLDER_CONTENT, locatorList, parentLocator);
  ...
  /**
   * An container class used to hold the information that is needed to create (site) folders. This
   * is used for creating folders for the benchmark database.
   */
  private class FolderInfo {
    /**
     * Constructs an object from the given parameters.
     *
     * @param siteName The name of the site folder, assume not <code>null</code> or empty.
     * @param dataFile The data file that is used to create folder, assume not <code>null</code> or
     *     empty.
     */
    private FolderInfo(String siteName, String dataFile, int firstContentId, int maxContentId) {
      m_siteName = siteName;
      m_dataFile = dataFile;
      ...
      for (int i = 0; i < 5; i++) m_nextCount[i] = 1;
    }

    /**
     * Get the next available content id.
     *
     * @return the content id, which will next greater than <code>m_maxContentId</code>.
    ...
    /**
     * The next content id, which is used to be a child of a folder. It will incremented for each
     * usage, but it never greater than <code>m_lastContentId</code>. It is initialized by ctor.
     */
    int m_nextContentId;

    /** The maximum content id of the <code>m_nextContentId</code>. It is initialized by ctor. */
    int m_maxContentId;

    /** The name of the site, init by ctor, never <code>null</code> or empty after that. */
    private String m_siteName;

    /**
     * The data file used to create folders, init by ctor, never <code>null</code> or empty after
     * that.
     */
