...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="row" mode="copy">
		<xsl:variable name="siteid"
			select="column[@name='SITEID']" />
		<xsl:variable name="contextid"
			select="column[@name='CONTEXTID']" />
		<xsl:variable name="propid"
			select="column[@name='PROPERTYID']" />
		<xsl:variable name="propname"
			select="column[@name='PROPERTYNAME']" />
		<xsl:variable name="propvalue"
			select="column[@name='PROPERTYVALUE']" />
		<xsl:if
			test="not(preceding-sibling::*[column[@name='SITEID']=$siteid and column[@name='CONTEXTID']=$contextid and column[@name='PROPERTYID']=$propid and column[@name='PROPERTYNAME']=$propname and column[@name='PROPERTYVALUE']=$propvalue])">
