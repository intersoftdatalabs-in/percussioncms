...
<xsl:stylesheet version="1.1"
	...
	<xsl:template
		match="PSXRelationshipConfig[@name='New Copy']/PSXCloneOverrideFieldList/PSXCloneOverrideField[@name='sys_communityid']"
		mode="copy">
	...
	<xsl:template
		match="PSXRelationshipConfig[@name='New Copy']/PSXCloneOverrideFieldList/PSXCloneOverrideField[@name='sys_workflowid']"
		mode="copy">
		<PSXCloneOverrideField id="0"
			name="sys_workflowid">
			<value>
	...
	<xsl:template
		match="PSXRelationshipConfig[@name='Promotable Version']/EffectSet"
		mode="copy">
		<EffectSet>
			<xsl:apply-templates select="*" mode="copy" />
			<xsl:if
				test="count(PSXConditionalEffect[@activationEndpoint='owner']/PSXExtensionCall[name='Java/global/percussion/relationship/effect/sys_AddCloneToFolder']) = 0">
	...
	<!-- This template replaces the sys_ValidateFolder with sys_TouchParentFolderEffect
		for Folder Content relationship -->
	<xsl:template
		match="/PSXRelationshipConfigSet/PSXRelationshipConfig/EffectSet/PSXConditionalEffect/PSXExtensionCall/name[text()='Java/global/percussion/relationship/effect/sys_ValidateFolder']"
		mode="copy">
