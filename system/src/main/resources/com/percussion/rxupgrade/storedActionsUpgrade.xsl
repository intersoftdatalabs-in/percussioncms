...
<!-- Stylesheet for upgrading the "rxconfig/Server/requestHandlers/storedActions.xml"
	file. Currently it adds the "checkin" and "checkout" actions if the corresponding
	"ActionSet" element is missing. -->
<xsl:stylesheet version="1.1"
		...
		<xsl:copy>
			...
			<xsl:if test="not(ActionSet[@name='checkoutedit'])">
				<ActionSet name="checkoutedit">
					<!-- checks out, then edits a content item. will not fail if item is
						already checked out -->
					<Action name="checkout" ignoreError="yes">
						<PSXParam name="sys_command">
							<DataLocator>
			...
			<xsl:if test="not(ActionSet[@name='checkintransition'])">
				<ActionSet name="checkintransition">
					<!-- checks in, then transitions a content item using the transition
						specified in the parameter "WFAction" -->
					<Action name="checkin" ignoreError="yes">
						<PSXParam name="sys_command">
							<DataLocator>
								<PSXTextLiteral id="0">
			...
			<xsl:if test="not(ActionSet[@name='transitcheckoutedit'])">
				<!-- An action set introduced for quick edit. The current item will be
					transitioned using the workflow transition specified in parameter 'WFAction',
					then it is checked-out and opened in edit mode. WFAction - the transition
					(the trigger name) used to transit the item. sys_contentid - the content
					id of the item to act on. sys_revision - the revision of the item to act
					on. sys_view - the view used to open the item in edit mode. -->
			...
			<xsl:if test="not(ActionSet[@name='checkoutaapage'])">
				<!-- An action set introduced for new style of active assembly for pages
					in that we actually checkout the item if not checked out and then open the
					page for active assembly. The trick is that we always have to open the tip
					revision instead of current revision. As we now checkout the page implicitly
					before active assembly we have to resolve the revision dynamically and then
					modify the active assembly page url. This is what exactly this action set
					does. The parameters: sys_command is hardcoded to be 'workflow' WFAction
					- hardcoded to 'checkout' We generate the redirect url based in the following
					parameters sys_assemblyurl - Base url for the document assembly, caller sends
					this as HTML parameter. sys_contentid - the content id of the item for active
					assembly, caller sends this as HTML parameter. sys_revision - the revision
					of the item for active assembly. By now workflow exit would already have
					put the right revision value for this parameter. sys_variantid - variantid
					of the page for active assembly, caller sends this as HTML parameter. sys_context
					- context hardcoded to preview context, i.e. 0. sys_authtype - authtype hardcoded
					to all content, i.e. 0. -->
			...
			<xsl:if test="not(ActionSet[@name='checkoutaadoc'])">
				<!-- An action set introduced for new style of active assembly for documents
					in that we actually checkout the item if not checked out and then open the
					page for active assembly. The trick is that we alway have to open the tip
					revision instead of current revision. As we now checkout the document implicitly
					before active assembly we have to resolve the revision dynamically and then
					modify the active assembly document url. This is what exactly this action
					set does. The parameters: sys_command is hardcoded to be 'workflow' WFAction
					- hardcoded to 'checkout' We generate the redirect url based in the following
					parameters sys_assemblyurl - Base url for the document assembly, caller sends
					this as HTML parameter. sys_contentid - the content id of the item for active
					assembly, caller sends this as HTML parameter. sys_revision - the revision
					of the item for active assembly. By now workflow exit would already have
					put the right revision value for this parameter. sys_variantid - variantid
					of the page for active assembly, caller sends this as HTML parameter. -->
	...
	<xsl:template
		match="ActionSet[@name='checkoutedit']/PSXUrlRequest/PSXExtensionCall | ActionSet[@name='transitcheckoutedit']/PSXUrlRequest/PSXExtensionCall"
		mode="copy">
		<xsl:copy>
			<xsl:apply-templates select="@*" mode="copy" />
			<xsl:apply-templates mode="copy" />
			<xsl:if
				test="not(PSXExtensionParamValue/value/PSXTextLiteral[text='refreshHint'])">
				<PSXExtensionParamValue id="7">
					<value>
						<PSXTextLiteral id="0">
							<text>refreshHint</text>
	...
	<xsl:template
		match="ActionSet[@name='checkoutaapage']/PSXUrlRequest/PSXExtensionCall"
		mode="copy">
		<xsl:copy>
			<xsl:apply-templates select="@*" mode="copy" />
			<xsl:apply-templates mode="copy" />
			<xsl:if
				test="not(PSXExtensionParamValue/value/PSXTextLiteral[text='sys_siteid'])">
				<PSXExtensionParamValue id="20">
					<value>
						<PSXTextLiteral id="21">
							<text>sys_siteid</text>
			...
			<xsl:if
				test="not(PSXExtensionParamValue/value/PSXTextLiteral[text='parentPage'])">
				<PSXExtensionParamValue id="24">
					<value>
						<PSXTextLiteral id="25">
							<text>parentPage</text>
			...
			<xsl:if
				test="not(PSXExtensionParamValue/value/PSXTextLiteral[text='sys_folderid'])">
				<PSXExtensionParamValue id="20">
					<value>
						<PSXTextLiteral id="21">
							<text>sys_folderid</text>
