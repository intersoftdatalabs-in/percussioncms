...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="row" mode="findexists">
		...
		<xsl:choose>
			<xsl:when test="val=''">
				<xsl:if
					test="(column[@name='TYPE']=$type and column[@name='CATEGORY']=$cat)">
					<xsl:value-of select="'found'" />
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:if
					test="(column[@name='TYPE']=$type and column[@name='CATEGORY']=$cat and column[@name='NAME']=$val)">
					<xsl:value-of select="'found'" />
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
			...
			<xsl:variable name="var1">
				<xsl:apply-templates select="row"
					...
					<xsl:with-param name="cat"
						select="'SYS_ATTRIBUTENAME'" />
					<xsl:with-param name="val"
						select="'sys_defaultCommunity'" />
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:if test="$var1!='found'">
				<row>
					<column name="ID">
						<xsl:value-of select="($maxid + 1)" />
					</column>
			...
			<xsl:variable name="var2">
				<xsl:apply-templates select="row"
					...
					<xsl:with-param name="cat"
						select="'SYS_ATTRIBUTENAME'" />
					<xsl:with-param name="val"
						select="'sys_defaultHomepageURL'" />
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:if test="$var2!='found'">
				<row>
					<column name="ID">
						<xsl:value-of select="($maxid + 2)" />
					</column>
					<column name="TYPE">role</column>
					<column name="CATEGORY">SYS_ATTRIBUTENAME</column>
