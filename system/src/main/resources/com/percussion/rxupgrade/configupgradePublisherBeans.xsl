...
<xsl:stylesheet version="1.1"
	...
	xmlns:foo="http://www.springframework.org/schema/beans"
	...
	<xsl:template
		match="foo:bean[@id='sys_metadataDeliveryHandler']" mode="copy">
		<xsl:copy>
			<xsl:apply-templates select="@*" mode="copy" />
			<xsl:if
				test="not(foo:property[@name='supportedContentTypes'])"><!-- if it already exists don't add it -->
