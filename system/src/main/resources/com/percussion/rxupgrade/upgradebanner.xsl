...
<xsl:stylesheet version="1.1"
	...
	<xsl:output method="xml" version="1.0" encoding="UTF-8"
		indent="yes" />
	<!-- Stylesheet to upgrade 5.7 or lower rx_bannerTemplate.xsl to work with
		6.0 which uses JSPs for other components -->
	<xsl:template match="/">
	...
	<xsl:template
		match="xsl:template[@name='bannerAndUserStatus']" mode="copy">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:choose>
	...
	<!-- Replace the old table cell with known attribute valuess to the new
		one -->
	<xsl:template
		match="td[@width='205' and @class='outerboxcell' and @valign='middle']"
