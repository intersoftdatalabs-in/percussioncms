...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="Server">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<!-- quick test to see if the upgrade has added this listener classname
				already, don't add new nodes if they already exist. -->
			<xsl:if
				test="not(//Listener[@className='org.apache.catalina.mbeans.ServerLifecycleListener'])">
				<xsl:comment>
	...
	<!-- change the HTTP 1.1 connector class names from catalina to coyote,
		add 2 new attributes, if default values of other attributes are detected,
		modify to new default attributes otherwise copy unchanged -->
	<xsl:template match="//Connector">
		<xsl:choose>
			<xsl:when
				test="./@className='org.apache.catalina.connector.http.HttpConnector'">
				<xsl:copy>
					<xsl:copy-of select="@*" />
			...
			<xsl:when
				test="./@className='org.apache.ajp.tomcat4.Ajp13Connector'">
				<xsl:copy>
					<xsl:copy-of select="@*" />
					<xsl:attribute name="className">org.apache.coyote.tomcat4.CoyoteConnector</xsl:attribute>
					<xsl:apply-templates select="node()" />
	...
	<xsl:template match="//Connector/Factory">
		<xsl:choose>
			<xsl:when
				test="./@className='org.apache.catalina.net.SSLServerSocketFactory'">
				<xsl:copy>
					<xsl:copy-of select="@*" />
	...
	<xsl:template match="//Realm">
		<Realm>
			<xsl:choose>
				<xsl:when
					test="./@className='org.apache.catalina.realm.MemoryRealm'">
	...
	<!-- change user to username and driverName to url for all Resource Parameters
		of type javax.sql.DataSource -->
	<xsl:template match="//ResourceParams">
		<xsl:choose>
			<xsl:when
				test="./@name=../Resource[@type='javax.sql.DataSource']/@name">
				<xsl:copy>
					<xsl:copy-of select="@*" />
