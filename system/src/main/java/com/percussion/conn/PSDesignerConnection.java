...
public class PSDesignerConnection {
  ...
  public PSDesignerConnection(java.util.Properties connInfo)
    ...
    if (connInfo == null) throw new IllegalArgumentException("conn props required");
    ...
    if (m_useSSL) {
      try {
        ...
        if (handlers == null || handlers.indexOf(HANDLER_VAL) == -1) {
          if (handlers != null && handlers.trim().length() > 0) handlers += "|";
          else handlers = "";
    ...
    try {
      ...
      else m_port = Integer.parseInt(portStr);
    } catch (Exception e) {
      throw new PSInvalidPortException(portStr);
    }

    m_host = (String) connInfo.get(PROPERTY_HOST);
    if (m_host == null) throw new IllegalArgumentException("host address required");
    ...
    */
    ...
    if (StringUtils.isBlank(encoding)) encoding = PSCharSets.rxJavaEnc();
    m_httpAuthorization = "Basic " + PSBase64Encoder.encode(m_userName + ":" + m_loginPw, encoding);

    String useJavaUrl = (String) connInfo.get(PROPERTY_USE_JAVA_URL);
    m_useJavaUrl = ((useJavaUrl != null) && useJavaUrl.equalsIgnoreCase(USE_JAVA_URL_ENABLED));
  ...
  public void connect()
      throws PSServerException, PSAuthorizationException, PSAuthenticationFailedException,
          PSAuthenticationRequiredException, java.io.IOException {
    ...
    if (isConnected()) PSXmlDocumentBuilder.addElement(reqDoc, root, "sessid", m_sessId);
    ...
    if (m_serverVersion != null) System.out.println("Version = " + m_serverVersion.getVersion());
  }
  ...
  private void reconnect()
      throws IOException, PSServerException, PSAuthorizationException,
  ...
  private Document makeRequest(Document req)
      throws PSServerException, PSAuthorizationException, PSAuthenticationFailedException,
          IOException {
    if (m_useJavaUrl) return makeUrlRequest(req);
  ...
  private Socket createSocket(String host, int port, boolean useSSL) throws IOException {
    ...
    if (!useSSL) socket = new Socket(host, port);
    else {
      // create SSL socket
  ...
  private Document makeSocketRequest(Document req)
    ...
    String url = m_protocol + "://" + m_host + ":" + m_port + "/Rhythmyx/Designer HTTP/1.0\r\n";
    ...
    try (Socket sock = createSocket(m_host, m_port, m_useSSL)) {
      ...
      try (OutputStream out = sock.getOutputStream()) {
        try (InputStream in = sock.getInputStream()) {
          ...
          writeHeaderLine(
              out, "Content-Type", IPSMimeContentTypes.MIME_TYPE_TEXT_XML + ";charset=" + stdEnc);
          ...
          if (m_reqType != null) writeHeaderLine(out, "PS-Request-Type", m_reqType);
          if (m_reqVersion != null) writeHeaderLine(out, "PS-Request-Version", m_reqVersion);
          if (m_reqApplication != null) writeHeaderLine(out, "PS-Application", m_reqApplication);
          if (m_reqDataSet != null) writeHeaderLine(out, "PS-DataSet", m_reqDataSet);

          if (m_sessId != null) {
            writeHeaderLine(
                out, "Cookie", com.percussion.server.PSUserSession.SESSION_COOKIE + "=" + m_sessId);
          }
          ...
          try (PSInputStreamReader reader = new PSInputStreamReader(in)) {
            ...
            try (BufferedReader contentReader =
                new BufferedReader(new InputStreamReader(reader, charSet))) {
                ...
                if ("PSXError".equalsIgnoreCase(root.getTagName())) {
                  ...
                  if (eClass.startsWith("com.percussion.")) {
                    try {
                      ...
                      int argCount = 0;
                      while (w.getNextElement("arg", true, false) != null) argCount++;
                      w.setCurrent(cur);

                      Object[] newArgs = new Object[argCount];

                      // second pass to load them
                      for (int i = 0; w.getNextElement("arg", true, false) != null; i++) {
                  ...
                  if (eClass != null) {
                    if ("com.percussion.security.PSAuthorizationException".equals(eClass)) {
                      final PSAuthorizationException e =
                          new PSAuthorizationException(errorCode, args);
                      e.setOverridingMessage(eMessage);
                      throw e;
                    } else if ("com.percussion.design.objectstore.PSLockedException"
                        .equals(eClass)) {
                      final PSAuthorizationException e =
                          new PSAuthorizationException(errorCode, args);
                      e.setOverridingMessage(eMessage);
                      throw e;
                    } else if ("com.percussion.security.PSAuthenticationRequiredException"
                        .equals(eClass)) {
                      final PSAuthenticationRequiredException e =
                          new PSAuthenticationRequiredException(errorCode, args);
                      e.setOverridingMessage(eMessage);
                      throw e;
                    } else if ("com.percussion.security.PSAuthenticationFailedException"
                            .equals(eClass)
                        || "com.percussion.security.PSAuthenticationFailedExException"
                            .equals(eClass)) {
                      final PSAuthenticationFailedException e =
                          new PSAuthenticationFailedException(errorCode, args);
                      e.setOverridingMessage(eMessage);
                      throw e;
                    } else {
  ...
  private Document makeUrlRequest(Document req)
    ...
    // write the headers
    conn.setRequestProperty("Content-Type", IPSMimeContentTypes.MIME_TYPE_TEXT_XML);

    if (m_reqType != null) conn.setRequestProperty("PS-Request-Type", m_reqType);
    if (m_reqVersion != null) conn.setRequestProperty("PS-Request-Version", m_reqVersion);
    if (m_reqApplication != null) conn.setRequestProperty("PS-Application", m_reqApplication);
    if (m_reqDataSet != null) conn.setRequestProperty("PS-DataSet", m_reqDataSet);

    if (m_sessId != null) {
      conn.setRequestProperty(
          "Cookie", com.percussion.server.PSUserSession.SESSION_COOKIE + "=" + m_sessId);
    }
  ...
  public static PSException createExceptionFromXml(Element root) {
    ...
    if (!"PSXError".equalsIgnoreCase(root.getTagName())) return null;
    ...
    if (eClass != null) {
      ...
      else e = new PSServerException(eClass, errorCode, args);
    } else e = new PSServerException(errorCode, args);
  ...
  private static void writeHeaderLine(OutputStream out, String name, String value)
      throws IOException {

    StringBuilder buf = new StringBuilder(name.length() + 4 + (value == null ? 0 : value.length()));
  ...
  private String makeLasagna(String str) {
    ...
    try {
      ...
      return PSEncryptor.encryptString(
          PathUtils.getRxDir(null).getAbsolutePath().concat(PSEncryptor.SECURE_DIR), str);
  ...
  public boolean checkVersionCompatibility(PSFormatVersion client) {
    ...
    return ((m_serverVersion.getMajorVersion() >= 6)
        && (m_serverVersion.getInterfaceVersion() <= client.getInterfaceVersion()));
  }
