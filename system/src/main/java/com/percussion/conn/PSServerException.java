...
/**
 * PSServerException is thrown to indicate the server cannot be reached. This may be due to the
 * server being down, a configuration error or a path may not exist between the client and the
 * server.
 *
...
public class PSServerException extends PSException {
  ...
  /**
   * Construct an exception for the specified class which uses the specified message code and
   * arguments. This class is used primarily on the client side when the server sends an exception.
   * If it's one of our exceptions, it is wrapped into this class. The recipient can then determine
   * if the exception is of a type it is interested in reconstructing and throwing. If so, it can do
   * so by using the class name, msg code and msg args.
   *
  ...
  /**
   * Get the underlying PSException class associated with this exception. If the exception is not
   * one of ours, null is returned.
   */
  public PSException getOriginatingException() {
    if (m_className == null) return null;

    try {
      Class excClass = Class.forName(m_className);
      Class[] classParams = {Integer.TYPE, Class.forName("[Ljava.lang.Object;")};
      Constructor constr = excClass.getConstructor(classParams);
      PSException e = (PSException) constr.newInstance(new Object[] {new Integer(m_code), m_args});
      return e;
    } catch (Exception e) {
  ...
  /**
   * Get the underlying PSException class name associated with this exception. If the exception is
   * not one of ours, null is returned.
   */
