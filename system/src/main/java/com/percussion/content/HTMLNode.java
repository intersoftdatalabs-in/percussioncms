...
public abstract class HTMLNode implements Node {
  ...
  public HTMLElement getFirstElementChild() {
    ...
    for (n = getFirstChild(); n != null; n = n.getNextSibling()) {
      if (n.getNodeType() == ELEMENT_NODE) break;
    }
  ...
  public Node insertBefore(Node newChild, Node refChild) throws DOMException {
    ...
    if (prevSib != null) prevSib.setNextSibling(nextSib);

    if (nextSib != null) nextSib.setPreviousSibling(prevSib);
    ...
    if (idx < 0) throw new HTMLException(DOMException.NOT_FOUND_ERR, "refChild not found");
  ...
  public Node removeChild(Node oldChild) throws DOMException {
    ...
    if (oPrev != null) oPrev.setNextSibling(oNext);

    if (oNext != null) oNext.setPreviousSibling(oPrev);

    int idx = m_children.indexOf(oChild);
    if (idx < 0) throw new HTMLException(DOMException.NOT_FOUND_ERR, "oldChild not found");
  ...
  public Node cloneNode(boolean deep) {
    // TODO: implement
    if (true) throw new RuntimeException("cloneNode not supported");
    return null;
  }
