...
public class PSFileLogReader implements IPSLogReader {
  ...
  PSFileLogReader(RandomAccessFile file) throws IOException, SAXException {
    super();
    com.percussion.server.PSConsole.printMsg("LogReader", "Opening log file.");
  ...
  synchronized void read(IPSLogReaderFilter filter, boolean reparse)
    ...
    if (reparse) {
      try {
        ...
        com.percussion.server.PSConsole.printMsg(
            "LogReader", "Could not read log file", new String[] {e.toString()});
      }
    }
    ...
    for (Element msg = walker.getNextElement("PSXLogMessage", firstFlags);
      ...
      if ((startTime == null || msgTime.compareTo(startTime) >= 0)
        ...
        if (appIdString == null) continue; // skip this invalid entry
        int appId;
        try {
        ...
        if (appIdMatches) {
          ...
          for (Element subMsg = walker.getNextElement("data", firstFlags);
            ...
            if (subMsgTypeString == null) continue;
            int subMsgType;
            try {
            ...
            subMessages.add(
                new PSLogSubMessage(
                    subMsgType, walker.getElementData((Element) walker.getCurrent())));
          }
          PSLogSubMessage[] subMsgArray = new PSLogSubMessage[subMessages.size()];
          subMessages.toArray(subMsgArray);
          PSLogEntry logInfo = new PSLogEntry(msgType, appId, msgTime, subMsgArray);
          subMessages.clear();
