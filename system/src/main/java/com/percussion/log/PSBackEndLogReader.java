...
public class PSBackEndLogReader implements IPSLogReader {
  ...
  PSBackEndLogReader() throws SQLException, NamingException {
    ...
    try {
      ...
      PSConsole.printMsg(
          ms_subsystem, "Back end log reader connecting with CMS Repository datasource");

      internalOpen();
    } catch (SQLWarning w) {
      PSConsole.printMsg(ms_subsystem, "SQL warning", new String[] {w.toString()});
      ...
      if (m_connDetails != null
        ...
        PSLogManager.printSqlException(
            ms_subsystem, "Back end log reader has unexpectedly gone down.", e);
  ...
  public synchronized boolean isOpen() {
    try {
      if (m_con != null) return !m_con.isClosed();
    } catch (Exception e) {
    }
    return false;
  }
  ...
  public synchronized void read(IPSLogReaderFilter filter) throws IllegalStateException {
    ...
    if (startTime != null) startMillis = startTime.getTime();
    else startMillis = 0;

    if (endTime != null) endMillis = endTime.getTime();
    else endMillis = Long.MAX_VALUE;
    ...
    queryString.append(
        m_joinFormatter.getLeftTablePrefix(joinType)
            + PSSqlHelper.qualifyTableName(
                PSLogDatabase.TABLE_PSLOG,
                m_connDetails.getDatabase(),
                m_connDetails.getOrigin(),
                m_connDetails.getDriver())
            + " L"
            + m_joinFormatter.getLeftTableSuffix(joinType)
            + m_joinFormatter.getRightTablePrefix(joinType)
            + PSSqlHelper.qualifyTableName(
                PSLogDatabase.TABLE_PSLOGDAT,
                m_connDetails.getDatabase(),
                m_connDetails.getOrigin(),
                m_connDetails.getDriver())
            + " D"
    ...
    if (m_joinFormatter.usesOnClauseInFrom())
      queryString.append(" ON " + highIdJoin + " AND " + lowIdJoin);
    else {
    ...
    try {
      ...
      try {
        ...
        PSConsole.printMsg(ms_subsystem, "Finished query.");
        ...
        for (boolean haveData = true; haveData; numRows++) {
          ...
          if ((minutesSinceEpoch == lastMinutesSinceEpoch)
            ...
            if (sequence != lastSequence) {
              ...
              subMessages.add(new PSLogSubMessage(lastSubType, dataBuffer.toString()));
            ...
            if (doWrite) {
              ...
              long msgMsecSinceEpoch =
                  (long) (microsSinceMinute / 1000L + minutesSinceEpoch * 60000L);
              ...
              filter.processMessage(new PSLogEntry(lastType, lastAppl, newDate, messages), true);
