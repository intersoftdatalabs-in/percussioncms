...
public class PSLogHandler {
  ...
  public void write(PSLogInformation msg, boolean forceLogging) throws IllegalStateException {
    ...
    m_combinedRules = m_rules;
    if (m_combinedRules == null) m_combinedRules = serverRules;
    else m_combinedRules.or(serverRules);
    ...
    if (write) PSLogManager.write(msg);
  }
  ...
  public void logBasicUserActivity(PSRequest request, boolean forceLogging) {
    ...
    if (!forceLogging
        && !isBasicUserActivityLoggingEnabled()
        && !isDetailedUserActivityLoggingEnabled()
        && !isFullUserActivityLoggingEnabled()) return;
    ...
    try {
      ...
      if (sess != null) {
        ...
        com.percussion.security.PSUserEntry[] entries = sess.getAuthenticatedUserEntries();
        if (entries != null) {
      ...
      info.put("url", request.getServletRequest().getRequestURL().toString());
    ...
    try {
      ...
      if ((params != null) && (params.size() != 0)) {
        ...
        for (int i = 0; entries.hasNext(); i++) {
          ...
          if (i != 0) /* write the param separator */ pw.write("&");
  ...
  public void logFullUserActivityAction(
    ...
    if (!forceLogging && !isFullUserActivityLoggingEnabled()) return;
    ...
    try {
      ...
      PSLogFullUserActivity logMsg = new PSLogFullUserActivity(applId, msgCode, msgArgs);
