...
/**
 * The PSLogExecutionPlan class is used to log more informative information concerning the execution
 * plan we choose for an application. This information is not necessarily structured and is anything
 * deemed relevent by the developer for debug purposes.
 *
...
public class PSLogExecutionPlan extends PSLogInformation {
   ...
   *
   * <p>Be sure to store the arguments in the correct order in the array, where {0} in the string is
   * array element 0, etc.
   *
   * @param applId the id of the application associated with this execution plan
  ...
  public PSLogExecutionPlan(int applId, int msgCode, Object[] arrayArgs) {
    ...
    m_subs[0] = new PSLogSubMessage(msgCode, PSErrorManager.createMessage(msgCode, arrayArgs));
  }

  /**
   * Get the sub-messages (type and text). A sub-message is created for each piece of information
   * reported when this object was created. See the {@link #PSLogExecutionPlan constructor} for more
   * details.
   *
