...
/**
 * The PSLogMultipleHandlers class is used to log requests matching the request criteria of multiple
 * applications/datasets.
 *
 * <p>When processing requests, the first data set matching the request criteria handles it. If
 * other data sets exist which are also interested in the request, they will be ignored. This may
...
public class PSLogMultipleHandlers extends PSLogInformation {
  ...
  /**
   * Construct a log message identifying that multiple handlers attempted to act upon the same
   * request.
   *
   * <p>The following list contains the supported keys and values:
   *
   * <ul>
   *   <li>sessionId - (subtype 1) the session id of the user making this request. This can be used
   *       to map back to the request when logging detailed user activity is enabled.
   *   <li>dataSetNames - (subtype 2) a comma delimited String containing the name of each dataset
   ...
   * reported when this object was created. See the {@link #PSLogMultipleHandlers(int,
   * java.util.concurrent.ConcurrentHashMap) constructor} for more details.
   *
