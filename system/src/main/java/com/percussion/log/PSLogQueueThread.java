...
class PSLogQueueThread extends java.lang.Thread {
  ...
  PSLogQueueThread(PSDoubleList logQueue, long msecBetweenFlushing, int flushLimit) {
    super();
    setDaemon(true);
    if (logQueue == null) throw new IllegalArgumentException("logQueue == null");
    if (msecBetweenFlushing < 0) throw new IllegalArgumentException("msecBetweenFlushing < 0");
    if (flushLimit < 0) flushLimit = 0;
    m_queue = logQueue;
  ...
  public void run() {
    ...
    conOut("Started log queue thread: " + Thread.currentThread().toString());
      ...
      synchronized (m_queue) {
        try {
          if (m_queue.size() == 0) {
            ...
            if (m_truncateDays > 0) m_queue.wait(ms_millisecInOneDay - truncatePeriod);
            else m_queue.wait();
          } else {
  ...
  public void flushQueue() {
    ...
    synchronized (m_queue) {
      ...
      if (msgs != null) {
        ...
        if (numMsgs >= ms_consoleThreshold) conOut("Flushing " + numMsgs + " log messages...");
        ...
        if (numMsgs >= ms_consoleThreshold) conOut("Finished flushing messages");
      }
