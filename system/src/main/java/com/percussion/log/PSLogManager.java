...
public class PSLogManager {
  ...
  public static synchronized void init(Properties props, Locale loc)
      throws IllegalArgumentException, IllegalStateException, IOException, ClassNotFoundException,
          SQLException, SAXException, NamingException {
    ...
    if (logTo == null) throw new IllegalArgumentException("logTo not specified");
    ...
    if (file != null) {
      try {
        ...
        if (strFile.startsWith("/")) m_logFile = new File(fileURL.getFile()); // absolute path
        else m_logFile = new File(PSServer.getRxDir(), fileURL.getFile());
    ...
    if (logTo.equalsIgnoreCase("DBMS")) {
      try {
        ...
        for (int i = 0; (i < 5 && e != null); i++) {
          try {
            ...
            if (m_logWriter == null) m_logWriter = new PSBackEndLogWriter();
            m_logReader = new PSBackEndLogReader();
      ...
      if (m_logFile != null) {
        ...
        try {
          PSFileLogReaderWriterPair pair = new PSFileLogReaderWriterPair(m_logFile);
          ...
          conOut(
              "Caught SAXException. Secondary logfile is corrupted. "
                  + "Failsafe will not be available.",
              new String[] {sax.toString()});
        }
      } else
        conOut("No secondary logging mechanism specified. " + "Failsafe will not be available.");
    } else if (logTo.equalsIgnoreCase("FILE")) {
      if (m_logFile == null) throw new IllegalArgumentException();
      ...
      try {
        ...
        conOut(
            "Caught SAXException. Primary logfile is corrupted. "
                + "Logging will not be available.",
            new String[] {sax.toString()});
        throw sax;
      }
    } else throw new IllegalArgumentException("logTo must be FILE or DBMS");

    try {
      m_queueChecker = new PSLogQueueThread(m_queue, 60000, 15);
    ...
    m_messageStrings = ResourceBundle.getBundle(PSLogStringBundle.class.getName(), loc);
  ...
  public static synchronized void close() {
    ...
    if (null != m_queueChecker) {
      ...
      try {
        ...
        conOut("Interrupted while waiting for log queue thread to terminate.");
        Thread.currentThread().interrupt();
      }
    }
    ...
    if (m_secondaryLogWriter != null) m_secondaryLogWriter.close();
    if (m_secondaryLogReader != null) m_secondaryLogReader.close();
    if (m_logReader != null) m_logReader.close();
    if (m_logWriter != null) m_logWriter.close();
  }
  ...
  public static void write(PSLogInformation msg) throws IllegalStateException {
    ...
    if (!isOpen()) {
      conOut(
          "Discarded message because log manager is closed:",
  ...
  protected static void writeThrough(PSLogInformation msg) {
    ...
    if (!m_useSecondaryLogMechanism) {
      if (m_logWriter.isOpen()) writer = m_logWriter;
      else {
        conOut("The primary logging mechanism has gone down." + " Switching to failsafe mode.");
      ...
      if (System.currentTimeMillis() - m_msecLastReOpenAttempt >= m_msecBetweenReconnect) {
        ...
        if (m_logWriter != null && m_logWriter.open()) {
          ...
          PSServerLogReaderFilter filter =
              new PSServerLogReaderFilter(null, m_exitFailsafeTime) {
                ...
                public void processMessage(PSLogEntry logMsg, boolean filterWasApplied) {
    ...
    if (!writer.isOpen()) {
      conOut("Discarded message because all writers are closed:", new String[] {msg.toString()});
      return;
    }
  ...
  public static void read(IPSLogReaderFilter filter) {
    ...
    flushQueue();
    if (!isOpen()) throw new IllegalStateException("cannot read when the log manager is closed");
    if (filter == null) throw new IllegalArgumentException("cannot read with a null filter");
    if (m_logReader == null) throw new IllegalStateException("logic error: log reader is null");
  ...
  public static void truncateLog(int days) {
    ...
    java.util.Date beforeDate =
        new java.util.Date(
            System.currentTimeMillis() - ((long) days * ms_minutesInOneDay * MILLIS_IN_MINUTE));

    if (m_logWriter != null) {
      m_logWriter.truncateLog(beforeDate);
    }
    if (m_secondaryLogWriter != null) m_secondaryLogWriter.truncateLog(beforeDate);
  }
  ...
  private static PSLogQueueThread m_queueChecker;

  /** True if we are currently in failsafe mode. */
  private static boolean m_useSecondaryLogMechanism = false;
