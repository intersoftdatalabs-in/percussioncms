...
/**
 * IPSLogReaderFilter defines an interface for filtering log messages read by an IPSLogReader
 * implementation. When calling the IPSLogReader's {@link com.percussion.log.IPSLogReader#read read}
 * method, the filter must be passed in. The filter may define the conditions it wants to filter, in
 * which case the IPSLogReader implementation may be able to optimize its search. If this cannot be
 * done, then the filter will be called to check each log message manually.
 *
 * <p>Log entries can be filtered using one of the following mechanisms:
 *
 * <ul>
 *   <li>within a specified time range
 *   <li>by application id
 *   <li>by application id and time range
 * </ul>
 *
