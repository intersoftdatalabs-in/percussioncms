...
public class PSFileLogWriter implements IPSLogWriter {
  ...
  PSFileLogWriter(RandomAccessFile file) throws IOException {
    super();
    com.percussion.server.PSConsole.printMsg("LogWriter", "Opening log file.");
    try {
      m_file = file;
    ...
    if (!m_file.getFD().valid()) throw new IOException("File descriptor is not valid");
  }
  ...
  private synchronized void init() throws IOException {
    ...
    if (i <= 0) throw new IOException("invalid log file format");
    else m_appendPos = i;
  ...
  public synchronized void write(PSLogInformation msg) throws java.lang.IllegalStateException {
    ...
    if (!isOpen()) throw new IllegalStateException("cannot write to a closed log writer");
    ...
    try {
      ...
      com.percussion.server.PSConsole.printMsg(
          "LogWriter", "Caught exception while writing to log file. ", new String[] {e.toString()});
      close();
    }
  ...
  public synchronized boolean isOpen() {
    if (m_isOpen) {
      try {
        if (m_file.getFD().valid()) return true; // early out
      } catch (IOException e) {
  ...
  public synchronized void truncateLog(java.util.Date allBefore) {
    ...
    com.percussion.server.PSConsole.printMsg(
        "FileLogWriter", "Truncating log entries prior to " + allBefore);

    com.percussion.util.PSDateFormatISO8601 fmt = new com.percussion.util.PSDateFormatISO8601();

    if (!isOpen()) throw new IllegalStateException("cannot access a closed log writer.");
    ...
    try {
      ...
      while (msg != null) {
        ...
        if (time == null) continue; // skip this invalid entry
        time = time.trim();
        java.util.Date msgTime = fmt.parse(time, new java.text.ParsePosition(0));

        if (msgTime == null) continue; // skip this invalid entry
      ...
      com.percussion.server.PSConsole.printMsg(
          "PSFileLogWriter", "Caught IOException while truncating", new String[] {e1.toString()});
      close();
      ...
      try {
        java.io.File tempFile =
            java.io.File.createTempFile(prefix, suffix, new File(".").getCanonicalFile());
      ...
      */
      ...
      com.percussion.server.PSConsole.printMsg(
          "PSFileLogWriter, truncate method", "Caught DOMException", new String[] {e3.toString()});
      close();
    }
  }
