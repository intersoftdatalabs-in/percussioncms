...
/**
 * The PSApplicationLogReaderFilter class implements log message filtering for applications.
 * Construct a filter with the appropriate attributes, then calling the IPSLogReader's {@link
 * com.percussion.log.IPSLogReader#read read} method. The filter will cause only messages generated
 * by the application to be read. A time range for messages can optionally be applied.
 *
...
public class PSApplicationLogReaderFilter implements IPSLogReaderFilter {
  ...
  /**
   * Construct an application log filter to retrieve all log entries for an application.
   *
   ...
   */
  public PSApplicationLogReaderFilter(int applId, java.io.OutputStream out, boolean rawText) {
    this(applId, out, rawText, null, null);
  }

  /**
   * Construct an application log filter to retrieve log entries within the specified time range for
   * an application.
   *
   * @param applId the application
   * @param out the output stream to write the log entries to
   ...
   */
  public PSApplicationLogReaderFilter(
      int applId,
   ...
   * entries (server or application). Return an empty array to retrieve only server entries.
   *
