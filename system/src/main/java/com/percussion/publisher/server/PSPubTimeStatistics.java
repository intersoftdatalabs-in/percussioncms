  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    if (params != null && params.length > 0 && params[0] != null) {
      ...
      try {
        ...
        ms_log.warn(
            "Failed to get the maxRows from value of '"
                + sMaxRows
                + "', the maxRows is default to "
                + maxRows);
      }
    }
    ...
    IPSInternalRequest ir = request.getInternalRequest(RESOURCE_NAME, null, true);
    ...
    try {
      ...
      while (rs.next() && rowCount < maxRows) {
        ...
        PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_PUB_STATUS_ID, pubstatusId);
        PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_CT_NAME, contentTypeName);
        PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_VARIANT, variant);
        PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_PUBSTATUS, pubstatus);

        numberEl =
            PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_AVG, String.valueOf(avgElapse));
        numberEl.setAttribute(ATTR_TYPE, NUM_VALUE);

        numberEl = PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_MIN, minElapse);
        numberEl.setAttribute(ATTR_TYPE, NUM_VALUE);

        numberEl = PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_MAX, maxElapse);
        numberEl.setAttribute(ATTR_TYPE, NUM_VALUE);

        numberEl = PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_SUM, sumElapse);
        numberEl.setAttribute(ATTR_TYPE, NUM_VALUE);

        numberEl = PSXmlDocumentBuilder.addElement(resDoc, statisticEl, EL_NUMITEMS, numItems);
        numberEl.setAttribute(ATTR_TYPE, NUM_VALUE);
