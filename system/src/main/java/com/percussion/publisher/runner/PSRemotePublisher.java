...
public class PSRemotePublisher {
  ...
  private static String publish(String server, String editionid, String userid, String password)
    ...
    Client client =
        ClientBuilder.newBuilder().newClient().register(new PSBasicAuthenticator(userid, password));
  ...
  public static void main(String[] args) {
    if (args.length < 4) {
      System.err.println(
          "Usage: java PSRemotePublisher <http(s)://server>:<port>/ "
              + "<editionid> [cmsuserid] [password]");
      System.err.println();
      System.err.println("where");
      System.err.println(
          "  server is url of the"
              + "CMS server to initiate publish from e.g. https://127.0.0.1:8443/");
      System.err.println("  editionid  is the numeric id of the edition " + "to be published");
      System.err.println("  cmsuserid  is the UserID to access " + "CMS applications, optional");
      System.err.println("  password   is the Password to access " + "CMS applications");
      System.exit(1);
    }
    ...
    try {
      System.out.println(publish(server, editionid, userid, password));
    } catch (Exception e) {
