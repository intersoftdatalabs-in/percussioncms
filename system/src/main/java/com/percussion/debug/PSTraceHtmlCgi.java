...
public class PSTraceHtmlCgi extends PSTraceMessage {
  ...
  protected String getMessageBody(Object source) {
    ...
    if (m_typeFlag != PSTraceMessageFactory.POST_EXIT_CGI_FLAG) {
      ...
      buf.append(NEW_LINE);
      if (params != null) logMap(buf, params);
    }
    ...
    if (cgiVars != null) logMap(buf, cgiVars);
    ...
    while (entries.hasNext()) {
      ...
      if (!(o instanceof File)) {
        if (o instanceof ArrayList) {
          ...
          if (tmpList.size() > 0 && tmpList.get(0) instanceof File) continue;
        }
        key = entry.getKey().toString();
        if (ms_hiddenVars.contains(key)) value = "**********";
        else value = (null == o) ? "" : o.toString();
        buf.append(key);
        buf.append(" = ");
        buf.append(value);
        buf.append(NEW_LINE);
