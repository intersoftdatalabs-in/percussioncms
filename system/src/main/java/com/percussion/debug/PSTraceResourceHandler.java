...
/**
 * Used to generate trace messages for the Resource Handler trace message type (0x0040). Includes
 * validation processing, and for each step in the plan, what it is and what the results are.
 *
 * <p>For a Query: All data associated with the result set returned by the current query. Binary
 * data is displayed in hex.
 *
 * <p>For an Update: For each row - the action taken (Insert, update, etc). If not skipped, the
 * values bound to each column.
 */
...
public class PSTraceResourceHandler extends PSTraceMessage {
   ...
   * source object.
   *
   * @param source an object containing the information required for the trace message. For most
   *     calls, this will be an object array where the last object in the array is a string
   *     specifying a message format, and all objects before that constitute the parameters for the
