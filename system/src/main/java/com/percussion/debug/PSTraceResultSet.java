...
public class PSTraceResultSet extends PSTraceMessage {
  ...
  private String printResultSet(Object[] args) {
    ...
    for (int i = 0; i < cols.length; i++) {
      String col = cols[i];
      if ((col == null) || (col.length() == 0)) col = "col" + String.valueOf(i + 1);
      else col = col.toLowerCase();
    ...
    while (rows.hasNext()) {
      ...
      for (int i = 0; i < colData.length; i++) {
        if (colData[i] != null) {
          ...
          } else width = colData[i].toString().length();
      ...
      for (int i = 0; i < colData.length; i++) {
        if (colData[i] != null) {
          ...
          if (colData[i] instanceof PSBinaryData) {
            ...
            int size = ((MAX_COL_WIDTH - HEX_PREFIX.length()) / BYTE2HEX_FACTOR);
          ...
          } else buf.append(padLeft(colData[i].toString(), ' ', maxLen[i]));
        } else buf.append(padLeft("", ' ', maxLen[i]));
        buf.append(COL_SPACER);
      }
      buf.append(NEW_LINE);
    }
  ...
  private void addChars(StringBuilder buf, char addChar, int len) {
    for (int i = 0; i < len; i++) buf.append(addChar);
  }
