...
public class PSTraceMapper extends PSTraceMessage {
  ...
  protected String getMessageBody(Object source) {
    ...
    if (args.length == 1) {
      ...
      String msg;
      if (((Boolean) args[0]).booleanValue()) msg = "traceMapper_useMapping";
      else msg = "traceMapper_skipMapping";

      buf.append(ms_bundle.getString(msg));
    } else if (args.length == 3) {
      // udf execution - print out name and result
      buf.append(MessageFormat.format(ms_bundle.getString("traceMapper_Udf"), args));
      ...
      buf.append(MessageFormat.format(ms_bundle.getString("traceMapper_dispMapping"), args));
    } else throw new IllegalArgumentException("Invalid number of arguments in source Object array");
