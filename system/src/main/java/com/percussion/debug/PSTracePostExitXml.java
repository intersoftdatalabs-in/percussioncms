...
public class PSTracePostExitXml extends PSTraceMessage {
  ...
  protected String getMessageBody(Object source) {
    ...
    if (args.length == 1) {
      ...
      buf.append(MessageFormat.format(ms_bundle.getString("tracePostExitXml_ExitDoc"), args));
      buf.append(NEW_LINE);
    } else throw new IllegalArgumentException("invalid source args");

    // XML doc may be null
    if (args[0] != null) buf.append(PSXmlDocumentBuilder.toString((Document) args[0]));
    else buf.append(ms_bundle.getString("tracePostExitXml_NullDoc"));
