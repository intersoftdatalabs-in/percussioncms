...
public class PSDebugLogHandler extends PSLogHandler implements IPSTraceStateListener {
  ...
  public PSDebugLogHandler(PSLogger logger, PSTraceInfo traceInfo, PSApplication app) {
    super(logger);
    if (traceInfo == null) throw new IllegalArgumentException("traceInfo may not be null");
  ...
  public void printTrace(int type, Object object) {
    ...
    if (!m_traceInfo.isTraceEnabled(type)) return;
  ...
  public void traceStopped(PSTraceInfo traceInfo) {
    try {
      if (m_traceWriter != null) {
        ...
        writeTraceHeader(
            MessageFormat.format(traceMessage, msgParams), m_traceInfo.getColumnWidth());
  ...
  public void traceStarted(PSTraceInfo traceInfo) {
    try {
      ...
      Object[] msgParams = {
        m_appName, flag.toString(0), flag.toString(1), flag.toString(2), flag.toString(3)
      };
      ...
      writeTraceHeader(MessageFormat.format(traceMessage, msgParams), m_traceInfo.getColumnWidth());
  ...
  public void shutdownTrace() {
    ...
    if (m_traceInfo.isTraceEnabled()) m_traceInfo.setTraceEnabled(false);
  ...
  private ResourceBundle getBundle() {
    if (m_resourceBundle == null)
      m_resourceBundle = ResourceBundle.getBundle("com.percussion.server.PSStringResources");
   ...
   */
  private void writeTraceHeader(String msg, int width) throws IOException {
    ...
    while (breaker.hasNext()) {
      ...
      if (mid.length() < msgLen) addChars(buf, ' ', msgLen - mid.length());
   ...
   * @throws java.io.IOException if there is a problem writing to the buffer
   */
  private void addChars(BufferedWriter buf, char addChar, int len) throws java.io.IOException {
    for (int i = 0; i < len; i++) buf.write(addChar);
  }
  ...
  private String getFormattedDate() {
    String date = null;
    try {
      if (m_format == null) m_format = FastDateFormat.getInstance("yyyy/MM/dd hh:mm:ss");
  ...
  private void logIOServerWarning(String msg) {
    ...
    Object[] dumpParams = {
      MessageFormat.format(getBundle().getString("traceIOErrorMsg"), msgParams)
    };
