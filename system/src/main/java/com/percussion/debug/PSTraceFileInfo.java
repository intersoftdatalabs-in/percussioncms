...
public class PSTraceFileInfo extends PSTraceMessage {
  ...
  protected String getMessageBody(java.lang.Object source) {
    ...
    if (params != null) {
      ...
      if (size == 0) buf.append(ms_bundle.getString("traceFileInfo_nofiles"));
      else if (size == 1) buf.append(ms_bundle.getString("traceFileInfo_onefile"));
      else {
        Object[] args = {Integer.toString(size)};
        buf.append(MessageFormat.format(ms_bundle.getString("traceFileInfo_files"), args));
      }
      buf.append(NEW_LINE);
      ...
      while (files.hasNext()) {
        ...
        if (file.getCharacterSetEncoding() != null) {
          ...
          buf.append(MessageFormat.format(ms_bundle.getString("traceFileInfo_charencoding"), args));
          buf.append(", ");
        }

        // length
        args[0] = Long.toString(file.length());
        buf.append(MessageFormat.format(ms_bundle.getString("traceFileInfo_bytes"), args));
        buf.append(", ");

        // content type
        args[0] = file.getSourceContentType();
        buf.append(MessageFormat.format(ms_bundle.getString("traceFileInfo_contenttype"), args));
      }
    }
    ...
    if (doc != null) {
      ...
      try {
        ...
        msg = "Exception occurred while trying to print doc: " + ioe.getLocalizedMessage();
      }
      buf.append(NEW_LINE);
      buf.append(msg);
    }
