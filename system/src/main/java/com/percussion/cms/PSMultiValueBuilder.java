...
public class PSMultiValueBuilder extends PSDisplayFieldBuilder {
    ...
    try {
      ...
      if (data != null && !isNewDoc) {
        try {
          ...
          rows = new ArrayList();
          while (data.readRow()) rows.add(data.getCurrentResultRowData()[0].toString());
        } finally {
  ...
  public boolean addChoiceElement(
    ...
    boolean addedElem =
        PSChoiceBuilder.addChoiceElement(doc, parent, choices, data, isNewDoc, true);

    List rows = (List) m_rows.get();
    if (!isNewDoc && rows != null) PSDisplayFieldElementBuilder.selectChoices(parent, rows);
