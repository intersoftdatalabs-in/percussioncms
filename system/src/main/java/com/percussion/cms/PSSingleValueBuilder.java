...
// REFACTORED: CP-JAVA11
{
  ...
  public static final List<String> REMOVE_EMPTY_ATTRIBUTES =
      Arrays.asList(
          ATTR_INLINESLOT,
          INLINE_TYPE,
          IPSHtmlParameters.SYS_DEPENDENTID,
  ...
  public PSSingleValueBuilder(
      PSField field, PSUISet ui, IPSReplacementValue value, PSEditorDocumentBuilder parentBuilder)
   ...
   */
  public PSSingleValueBuilder(
      PSField field, PSUISet ui, IPSBackEndMapping locator, PSEditorDocumentBuilder parentBuilder)
      throws PSExtensionException, PSNotFoundException, PSSystemValidationException {
    super(field, ui, parentBuilder);
  ...
  private IPSDataExtractor createExtractor(IPSReplacementValue value) {
    ...
    try {
      if (null != value) {
        extractor = PSDataExtractorFactory.createReplacementValueExtractor(value);
      }
  ...
  protected boolean addDataElement(
    ...
    String content = null;
    boolean isBECol = null != m_extractor && m_extractor.getSource()[0] instanceof PSBackEndColumn;
    if (isNewDoc && isBECol) {
    ...
    if (null != content && content.length() > 0) {
      ...
      if (content.contains(ATTR_INLINESLOT)) {
        IPSRequestContext request = new PSRequestContext(data.getRequest());
        ...
        if (contentDoc != null) {
          content =
              PSXmlDocumentBuilder.toString(
                  contentDoc,
                  PSXmlDocumentBuilder.FLAG_NO_INDENT | PSXmlDocumentBuilder.FLAG_OMIT_XML_DECL);
          if (isModified) {
            content = PSInlineLinkField.replaceString(content, PSInlineLinkField.ELEM_FILLER, "");
          }
        }
      }
  ...
  public static void processVariant(
    ...
    if (request == null) throw new IllegalArgumentException(NULL_REQUEST);

    if (inlineLinkRelData == null)
      throw new IllegalArgumentException("inlineLinkRelData must not be null");

    boolean processed = false;
    String inlineType = elem.getAttribute(PSInlineLinkField.RX_INLINETYPE);
    if (inlineType.equals(RX_INLINETYPE_RXVARIANT)
        || inlineType.equals(RX_INLINETYPE_HYPERLINK)
        || inlineType.equals(RX_INLINETYPE_IMAGE)) {

      Element newVar = replaceVariant(elem, request, inlineLinkRelData);
    ...
    if (!processed && (elem.hasChildNodes())) {
      ...
      for (int i = 0; i < children.getLength(); i++) {
        ...
        if (test.getNodeType() == Node.ELEMENT_NODE)
          processVariant((Element) test, request, inlineLinkRelData, controlName);
      }
    }
  }
  ...
  private static void updateLegacyVariant(Element elem) {
    ...
    if (parent instanceof Element) {
      ...
      parentIsEditLiveDivWrapper =
          parentName.equalsIgnoreCase("div") && parentClassAttr.equals("rx_ephox_inlinevariant");
    }
  ...
  public static PSRelationshipData buildRelationshipData(
    ...
    if (parent == null) {
      String parentContentId = request.getParameter(IPSHtmlParameters.SYS_CONTENTID);
      String parentRevision = request.getParameter(IPSHtmlParameters.SYS_REVISION);
    ...
    if (rels.isEmpty()) {
      ...
      if (summary.getName().startsWith("LocalContent") && summary.getContentPathName() == null
          || "".equals(summary.getContentPathName())) {
    ...
    for (PSRelationship rel : rels) {
      if (!StringUtils.isEmpty(rel.getProperty(PSRelationshipConfig.PDU_INLINERELATIONSHIP))) {
        ...
        inlineLinkRelData.m_relIdContentIdMap.put(rel.getId(), dependentid);
  ...
  public static void cleanupEmptyAttributes(Element elem) {
    ...
    for (int i = 0; i < numAttrs; i++) {
      ...
      if (StringUtils.isEmpty(attr.getValue()) && REMOVE_EMPTY_ATTRIBUTES.contains(name)) {
        if (removeAttrs == null) removeAttrs = new ArrayList<>();
        removeAttrs.add(name);
      }
    }

    if (removeAttrs != null) {
      for (String att : removeAttrs) elem.removeAttribute(att);
    }
  }
  ...
  private static void modifyLink(
    ...
    String variantid = elem.getAttribute(IPSHtmlParameters.SYS_DEPENDENTVARIANTID);
    ...
    Map<String, String> relMap = getSiteAndFolderFromRelationship(relid, contentid, request);
    // If they exist update the site and folder ids
    siteid =
        StringUtils.isNotBlank(relMap.get(IPSHtmlParameters.SYS_SITEID))
            ? relMap.get(IPSHtmlParameters.SYS_SITEID)
            : siteid;
    folderid =
        StringUtils.isNotBlank(relMap.get(IPSHtmlParameters.SYS_FOLDERID))
            ? relMap.get(IPSHtmlParameters.SYS_FOLDERID)
            : folderid;

    // siteid or folderid might have been modified from relationship set them
    // back on element as attributes.
    if (StringUtils.isNotBlank(siteid)) elem.setAttribute(IPSHtmlParameters.SYS_SITEID, siteid);
    ...
    if (siteid.length() < 1) siteid = request.getParameter(IPSHtmlParameters.SYS_SITEID);

    String originalSiteId = request.getParameter(IPSHtmlParameters.SYS_ORIGINALSITEID);
    String newContentId = getCorrectedContentId(request, contentid, relid, inlineLinkRelData, null);
    /*
    ...
    if (!newContentId.equals(contentid)) {
      elem.setAttribute(IPSHtmlParameters.SYS_DEPENDENTID, newContentId);
    }

    if (getLinkState(elem, Integer.parseInt(newContentId))) return;
    ...
    if (url.length() > 0) {
      ...
      if (index > -1) anchor = href.substring(index + 1);
      ...
      if (!authtype.equals(AUTHTYPE_ALL)) {
        ...
        PSException ex = null;
        try {
          fixedUrl = PSMacroUtils.fixLinkUrlRevisionForFlags(request, url, "i".toCharArray());
        } catch (PSInternalRequestCallException
            | PSNotFoundException
            | PSRequestParsingException e) {
          ex = e;
        }
        if (ex != null) {
          throw new PSCmsException(ex);
        }
        if (fixedUrl == null) url = "";
        else url = fixedUrl;
      }
  ...
  public static String getValidFlag(int contentid) {
    ...
    if (PSFolderRelationshipCache.getInstance() != null
        && PSFolderRelationshipCache.getInstance()
                .getParentPaths(loc, PSRelationshipConfig.TYPE_RECYCLED_CONTENT)
                .length
            > 0) {
      return "u";
    }
    ...
    PSComponentSummary componentSummary =
        PSCmsObjectMgrLocator.getObjectManager().loadComponentSummary(contentid);
    PSState state =
        svc.loadWorkflowState(
            new PSGuid(PSTypeEnum.WORKFLOW_STATE, componentSummary.getContentStateId()),
            new PSGuid(PSTypeEnum.WORKFLOW, componentSummary.getWorkflowAppId()));
  ...
  public static void addClass(Element elem, String className) {
    ...
    if (StringUtils.isBlank(classAttrib)) elem.setAttribute(CLASS, className);
  ...
  public static void removeClass(Element elem, String className) {
    ...
    if (StringUtils.contains(classAttrib, className)) {
      ...
      if (classAttrib.length() > 0) elem.setAttribute(CLASS, classAttrib.trim());
  ...
  private static String getAaProperty(
    ...
    if (StringUtils.isBlank(propertyValue) || propertyValue.equals("-1")) return defaultValue;
  ...
  public static String replaceVariant(
    ...
    if (pssessionid == null || pssessionid.trim().length() == 0)
      throw new IllegalArgumentException("pssessionid may not be null or empty");
    if (sys_dependentid == null || sys_dependentid.trim().length() == 0)
      throw new IllegalArgumentException("sys_dependentid may not be null or empty");
    if (sys_dependentvariantid == null || sys_dependentvariantid.trim().length() == 0)
      throw new IllegalArgumentException("sys_dependentvariantid may not be null or empty");
    if (rx_inlineslot == null || rx_inlineslot.trim().length() == 0)
      throw new IllegalArgumentException("rx_inlineslot may not be null or empty");
    if (rx_inlinetype == null || rx_inlinetype.trim().length() == 0)
      throw new IllegalArgumentException("rx_inlinetype may not be null or empty");
    if (sys_relationshipid == null || sys_relationshipid.trim().length() == 0)
      throw new IllegalArgumentException("sys_relationshipid may not be null or empty");
    if (sys_authtype == null || sys_authtype.trim().length() == 0)
      throw new IllegalArgumentException("sys_authtype may not be null or empty");
    if (sys_context == null || sys_context.trim().length() == 0)
      throw new IllegalArgumentException("sys_context may not be null or empty");
  ...
  public static Element replaceVariant(
    ...
    if (inlineLinkRelData == null) throw new IllegalArgumentException("in must not be null");

    String contentid = elem.getAttribute(IPSHtmlParameters.SYS_DEPENDENTID);
    String variantid = elem.getAttribute(IPSHtmlParameters.SYS_DEPENDENTVARIANTID);
    String rxselectedtext = elem.getAttribute(PSInlineLinkField.RX_SELECTEDTEXT);
    String rxinlineslot = elem.getAttribute(PSInlineLinkField.RX_INLINESLOT);
    String inlinetype = elem.getAttribute(PSInlineLinkField.RX_INLINETYPE);
    String relationshipid = elem.getAttribute(IPSHtmlParameters.SYS_RELATIONSHIPID).trim();
    String siteid = elem.getAttribute(IPSHtmlParameters.SYS_SITEID).trim();
    String folderid = elem.getAttribute(IPSHtmlParameters.SYS_FOLDERID).trim();
    // Get the siteid and folder id from relationship
    Map<String, String> relMap =
        getSiteAndFolderFromRelationship(relationshipid, contentid, request);
    // If they exist update the site and folder ids
    siteid =
        StringUtils.isNotBlank(relMap.get(IPSHtmlParameters.SYS_SITEID))
            ? relMap.get(IPSHtmlParameters.SYS_SITEID)
    ...
    contentid = getCorrectedContentId(request, contentid, relationshipid, inlineLinkRelData, null);
  ...
  public static String getCorrectedContentId(
    ...
    if (relationshipid == null || relationshipid.length() < 1) relationshipid = "-1";
    ...
    if (!inlineLinkRelData.m_contentIdPathMap.containsKey(cid)) {
      ...
      Integer newCid = inlineLinkRelData.m_relIdContentIdMap.get(rid);
      if (newCid != null) {
        contentid = newCid.toString();
      } else {
        boolean found = false;
        for (Integer entry : inlineLinkRelData.m_contentIdPathMap.keySet()) {
          List<Integer> locatorList = inlineLinkRelData.m_contentIdPathMap.get(entry);
    ...
    if (contentid == null) {
      try {
        ...
        List<PSRelationship> rels =
            PSRelationshipDbProcessor.getInstance().getRelationshipList(filter);
        if (!rels.isEmpty()) {
  ...
  private static List<PSLocator> getPromotedIdList(Integer contentid, IPSRequestContext request)
      throws PSCmsException {
    if (contentid == null) throw new IllegalArgumentException("contentid must not be null");
    if (request == null) throw new IllegalArgumentException(NULL_REQUEST);
    ...
    while (configs.hasNext()) {
      ...
      if (!cfg.getCategory().equals(PSRelationshipConfig.CATEGORY_PROMOTABLE)) continue;
  ...
  private static Element replaceVariant(
      ...
      IPSRequestContext request)
      throws PSCmsException {
    if (rxselectedtext == null) rxselectedtext = "";
    if (contentid == null
        || contentid.trim().length() < 1
        || rxinlineslot == null
        || rxinlineslot.trim().length() < 1
        || variantid == null
        || variantid.trim().length() < 1
        || inlinetype == null
        || inlinetype.trim().length() < 1) {
    ...
    if (command != null) paramMap.put(IPSHtmlParameters.SYS_COMMAND, command);
    ...
    try {
      ...
      variantUrlReq = request.getInternalRequest(VARIANTURL, paramMap, false);
      if (variantUrlReq == null) {
         ...
         *Just print the trace message.*/
        request.printTraceMessage(
            "Unable to make internal request to "
                + VARIANTURL
      ...
      if (relatedDoc == null) {
        ...
        request.printTraceMessage(
            "Internal request to "
                + VARIANTURL
                + " resulted in null document."
                + " Continuing without fixing the variants on opening the document.");
        return null;
      }
      variantUrl = relatedDoc.getDocumentElement().getAttribute(ATTR_PREVEWURL);
    ...
    if (!authtype.equals(AUTHTYPE_ALL)) {
      PSException ex = null;
      try {
        fixedUrl = PSMacroUtils.fixLinkUrlRevisionForFlags(request, variantUrl, "i".toCharArray());
    ...
    try {
      result = PSAssemblyServiceUtils.getAssembledDocumentResult(fixedUrl, paramMap);
    } catch (Exception e) {
    ...
    if (!result.isSuccess()) {
      ...
      request.printTraceMessage(
          "Inline template expanded to non-html value: " + result.getMimeType());
      return null;
    } else {
    ...
    if (nl != null && nl.getLength() > 0) {
      ...
      if (varBody == null) {
         ...
         */
        request.printTraceMessage(
            "The output of variant with url "
                + variantUrl
                + " produced a tree without a child element under"
                + " body element. Continuing without "
                + "fixing this variant on opening the document.");
        return null;
      }
      varBody.setAttribute(IPSHtmlParameters.SYS_DEPENDENTID, contentid);
      varBody.setAttribute(IPSHtmlParameters.SYS_DEPENDENTVARIANTID, variantid);
      varBody.setAttribute(PSInlineLinkField.RX_INLINESLOT, rxinlineslot);
      varBody.setAttribute(PSInlineLinkField.RX_SELECTEDTEXT, rxselectedtext);
      varBody.setAttribute(PSInlineLinkField.RX_INLINETYPE, inlinetype);
      varBody.setAttribute(IPSHtmlParameters.SYS_RELATIONSHIPID, relationshipid);
  ...
  private static void addReadOnlyAttributes(Element elem) {
    if (elem != null) {
      // Add attributes
      if (!ms_noReadOnlyAttribs.contains(elem.getNodeName().toLowerCase())) {
        elem.setAttribute(CONTENT_EDITABLE, CONTENT_EDITABLE_FALSE);
        elem.setAttribute(UN_SELECTABLE, UN_SELECTABLE_ON);
      }
