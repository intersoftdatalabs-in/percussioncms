...
public class PSEditorDocumentContext {
  ...
  public PSEditorDocumentContext(
      PSContentEditorHandler ceh, PSApplication app, PSContentEditor ce) {
    if (null == ceh || null == ce)
  ...
  public String getInitParam(String paramName) {
    ...
    String result = "";
    IPSReplacementValue param = m_ceHandler.getInitParam(m_commandName, paramName);
    if (param instanceof PSLiteral) result = ((Object) param).toString();
    return result;
  }
  ...
  public int getParentPageId(int pageId) throws PSNotFoundException {
    ...
    while (entries.hasNext()) {
      ...
      while (pageIds.hasNext()) {
        ...
        if (idObj.intValue() == pageId) parentIds.add(entry);
      }
    }
  ...
  public Iterator getPageInfoList(int childId) {
    ...
    while (entries.hasNext()) {
      Map.Entry entry = (Map.Entry) entries.next();
      PSPageInfo info = (PSPageInfo) entry.getValue();
      if (info.getChildId() == childId) results.add(entry);
    }
