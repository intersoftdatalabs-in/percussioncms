...
public abstract class PSModifyPlanBuilder {
  ...
  public static PSBackEndTable getMapperTable(PSDisplayMapper dispMapper, PSFieldSet fieldSet) {
    ...
    while (mappings.hasNext()) {
      ...
      if (beMapping instanceof PSBackEndColumn) {
        ...
        if (!colTable
            .getAlias()
            .equalsIgnoreCase(PSContentEditorSystemDef.CONTENT_STATUS_TABLE_ALIAS)) {
          table = colTable;
          break;
        }
      }
  ...
  /**
   * Walks the mapper and builds a list of all tables that the display mappings reference. For any
   * mappings that reference a field not found in the provided fieldSet, will check any immediate
   * child fieldsets of the type {@link PSFieldSet#TYPE_MULTI_PROPERTY_SIMPLE_CHILD} for that field,
   * and if found also adds that field's table to the list that is returned.
   *
   * @param dispMapper The mapper to get the table for. Not <code>null</code>.
   * @param fieldSet The fieldSet that the mapper uses. Not <code>null</code>.
    ...
    while (mappings.hasNext()) {
      ...
      if (beMapping instanceof PSBackEndColumn) {
        ...
        if (!colTableAlias.equalsIgnoreCase(PSContentEditorSystemDef.CONTENT_STATUS_TABLE_ALIAS)) {
          ...
          if (!tables.containsKey(colTableAlias)) tables.put(colTableAlias, colTable);
        }
      }
    }
   ...
   * </code> if not.
   */
  public static boolean allowMultipleInserts(PSDisplayMapper mapper, PSFieldSet fieldSet) {
   ...
   */
  protected IPSModifyStep createRevisionValidationStep() throws PSSystemValidationException {
    try {
      ...
      PSBackEndColumn contentIdCol = new PSBackEndColumn(beTable, IPSConstants.ITEM_PKEY_CONTENTID);

      PSDataMapping dataMapping = new PSDataMapping(contentIdParam, contentIdCol);
      dataMapper.add(dataMapping);

      // Add the EDITREVISION column
      String editRevParam = m_ceHandler.getParamName(PSContentEditorHandler.REVISION_ID_PARAM_NAME);
      PSBackEndColumn revCol = new PSBackEndColumn(beTable, IPSConstants.ITEM_EDITREVISION);
      ...
      selectionKeys.put(contentIdCol.getColumn(), contentIdParam);

      String resourceName =
          PSApplicationBuilder.createQueryDataset(
              m_internalApp, dataMapper, selectionKeys.entrySet().iterator(), null);

      // now create the validations
      Map validations = new HashMap();
      validations.put(IPSConstants.ITEM_EDITREVISION, new PSSingleHtmlParameter(editRevParam));

      // create the step
      IPSModifyStep valStep =
          new PSValidateModifyStep(
              resourceName,
              m_internalApp.getRequestTypeHtmlParamName(),
              m_internalApp.getRequestTypeValueQuery(),
              validations);
  ...
  protected PSDataMapper getSystemUpdateMapper(PSDisplayMapper mapper, PSFieldSet fieldSet) {
    ...
    ArrayList updateSysMappings = PSApplicationBuilder.getSystemUpdateMappings(m_ceHandler, m_ce);
    updateSysMappings = addTableKeys(updateSysMappings, mapper, fieldSet);
   ...
   * @return A data mapper with the required mappings, never <code>null</code>.
   */
  protected ArrayList getSystemDeleteMappings(PSDisplayMapper mapper, PSFieldSet fieldSet) {
    if (mapper == null || fieldSet == null)
      throw new IllegalArgumentException("One or more params is null");
    ...
    m_ceHandler.getSystemDef().populateSystemTableInfo(sysTableRef, sysCred, sysTable);

    // add contentId = contentId html param
    PSSystemMapping sysMapping =
        new PSSystemMapping(
            sysTable,
            "CONTENTID",
            new PSHtmlParameter(
   ...
   * @return A data mapper with the required mappings, never <code>null</code>.
   */
  protected List getDeleteChildMappings(PSDisplayMapper mapper, PSFieldSet fieldSet) {
    ...
    while (mappings.hasNext()) {
      ...
      if (fieldSet.getType() == PSFieldSet.TYPE_COMPLEX_CHILD
        ...
        deleteChildMappings = addTableKeysForDelete(deleteChildMappings, childMapper, fieldSet);
      }
    }
  ...
  private List getBinaryFields(PSFieldSet fieldSet, boolean checkChildren)
    ...
    if (m_binaryFields.containsKey(fieldSet)) return ((List) m_binaryFields.get(fieldSet));

    PSContentEditorPipe pipe = (PSContentEditorPipe) m_ce.getPipe();

    PSCollection tableSetCol = new PSCollection(pipe.getLocator().getTableSets());
    ...
    while (fields.hasNext()) {
      ...
      if (o instanceof PSFieldSet) {
        ...
        if (childFieldSet.getType() == PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD) {
      ...
      if (field.isForceBinary()) {
        ...
        while (tableSets.hasNext()) {
          ...
          if (foundIt) {
            if (PSMetaDataCache.isBinaryBackendColumn(tableSet, (PSBackEndColumn) beMapping)) {
              binFields.add(field);
            }
          }
        }
   ...
   */
  protected ArrayList addTableKeys(
      ArrayList mappings, PSDisplayMapper dispMapper, PSFieldSet fieldSet) {
   ...
   */
  protected ArrayList addTableKeys(
      ArrayList mappings, PSDisplayMapper dispMapper, PSFieldSet fieldSet, boolean updateSequence) {
      ...
      if (updateSequence && fieldSet.isSequencingSupported()) {
        sysMapping =
            new PSSystemMapping(
                ...
                new PSHtmlParameter(
                    m_ceHandler.getParamName(PSContentEditorHandler.SORT_RANK_PARAM_NAME)));
        resultMappings.add(sysMapping);
      }
    }
    ...
    for (int i = 0; i < tableList.length; i++) addCommonTableKeys(tableList[i], resultMappings);
   ...
   *     not modified. Never <code>null</code>.
   */
  protected List addTableKeysForDelete(
      List mappings, PSDisplayMapper dispMapper, PSFieldSet fieldSet) {
    if (mappings == null || dispMapper == null || fieldSet == null)
      throw new IllegalArgumentException("One or more params is null");
    ...
    for (int i = 0; i < tableList.length; i++) {
      PSSystemMapping perTableSysMapping =
          new PSSystemMapping(
              tableList[i],
  ...
  private static boolean containsChildType(
    ...
    while (mappings.hasNext()) {
      ...
      if (o == null || o instanceof PSField) continue;

      PSFieldSet childFieldSet = (PSFieldSet) o;
      if (childFieldSet.getType() == childType) result = true;
