...
public class PSDisplayFieldBuilder implements IPSBuildStep {
  ...
  public PSDisplayFieldBuilder(PSField field, PSUISet ui, PSEditorDocumentBuilder parentBuilder)
    ...
    initField(field, ui, PSDisplayFieldElementBuilder.DIMENSION_SINGLE, parentBuilder);
  }
  ...
  public PSDisplayFieldBuilder(
    ...
    if (null != ui.getLabel()) {
      ...
      if (null == m_errLabel || m_errLabel.trim().length() == 0) m_errLabel = m_label;
  ...
  private void initField(
      ...
      throws PSExtensionException, PSNotFoundException, PSSystemValidationException {
    if (null == dataDimension
        || !(dataDimension.equals(PSDisplayFieldElementBuilder.DIMENSION_SINGLE)
      ...
      throw new IllegalArgumentException("Invalid type for dataDimension (" + dataDimension + ")");
    }
    ...
    m_isRequired = field.getOccurrenceDimension(null) == PSField.OCCURRENCE_DIMENSION_REQUIRED;
    m_paramExtractors = prepareCustomParams(ui.getControl().getParameters());
    Iterator roRules = ui.getReadOnlyRules();
    m_isReadOnly = !parentBuilder.getDocContext().isEditMode();
    if (roRules.hasNext()) m_readOnlyHandler = new PSRuleListEvaluator(roRules);

    if (null == field.getVisibilityRules()) m_omit = false;
    else m_omit = field.getVisibilityRules().getDataHiding() == PSVisibilityRules.DATA_HIDING_XML;
  ...
  public PSDisplayFieldBuilder(
    ...
    if (null == controlName
        ...
        || null == submitName
        || submitName.trim().length() == 0
        || null == parentBuilder) {
      throw new IllegalArgumentException("One or more of the params was null or empty.");
    }
  ...
  public void execute(IPSBuildContext ctx, PSExecutionData data, boolean isNewDoc)
    ...
    if (null == ctx.getResultDocument()) {
      throw new IllegalArgumentException("Context document cannot be null.");
    }

    Element dispNode = createNode(ctx.getResultDocument(), data, isNewDoc, false);
    if (null != dispNode) {
      if (PSDisplayFieldElementBuilder.isHiddenDisplayField(dispNode))
        ctx.addHiddenField(dispNode, m_controlName);
      else ctx.addVisibleField(dispNode, m_controlName);
    }
  }
  ...
  private String getDataType(PSFieldSet fs) {
    ...
    while (fieldNames.hasNext()) {
      ...
      if (o instanceof PSField) dataType = getDataTypeText(((PSField) o).getType());
    }
  ...
  private Element createNode(
    ...
    if (!showField(data)) return null;

    boolean isVisible = m_isVisible;
    Object objSkipVisibilityRule =
        data.getRequest().getPrivateObject(IPSConstants.SKIP_FIELD_VISIBILITY_RULES);
    boolean skipVisibilityRule =
        (objSkipVisibilityRule instanceof Boolean)
            && ((Boolean) objSkipVisibilityRule).booleanValue();

    if (null != m_visibilityHandler) {
      if (skipVisibilityRule) isVisible = true;
      else isVisible = m_visibilityHandler.isMatch(data);
    }

    if (!isVisible && m_omit) return null;

    // if still visible, check view
    if (isVisible) isVisible = m_viewEvaluator.isFieldVisible(m_submitName, data);

    String displayType = getDisplayTypeText(!isVisible, isErrorDoc);

    String label = null;
    if (null != m_label) label = isErrorDoc ? m_errLabel : m_label;
  ...
  public Element createControlNode(Document doc, PSExecutionData data, boolean isNewDoc)
    ...
    Element control =
        PSDisplayFieldElementBuilder.createControlElement(
            doc,
            m_controlName,
            m_submitName,
            m_dataType,
    ...
    if (m_choices != null) addChoiceElement(doc, control, m_choices, data, isNewDoc);
  ...
  protected boolean addParamElement(
    ...
    while (params.hasNext()) {
      ...
      PSMapPair newPair = new PSMapPair(pair.getKey(), new Object[] {o, pairValues[1]});
      newParamList.add(newPair);
    }

    // create params el
    PSDisplayFieldElementBuilder.addParamListElement(doc, parent, newParamList.iterator());
  ...
  public boolean addChoiceElement(
    ...
    return PSChoiceBuilder.addChoiceElement(doc, parent, choices, data, isNewDoc, false);
  }
  ...
  private List prepareCustomParams(Iterator paramSet) {
    ...
    while (paramSet.hasNext()) {
      ...
      Object[] array = {extractor, param.getSourceType()};
  ...
  private String getDisplayTypeText(boolean isHidden, boolean isError) {
    String value;
    if (isHidden) value = PSDisplayFieldElementBuilder.DISPLAY_TYPE_HIDDEN;
    else if (isError) value = PSDisplayFieldElementBuilder.DISPLAY_TYPE_ERROR;
    else value = PSDisplayFieldElementBuilder.DISPLAY_TYPE_NORMAL;
    return value;
  }
  ...
  private boolean isReadOnly(PSExecutionData data) {
    ...
    else if (null == m_readOnlyHandler) return false;
    else return m_readOnlyHandler.isMatch(data);
  }
