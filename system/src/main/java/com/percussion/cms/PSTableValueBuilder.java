...
public class PSTableValueBuilder extends PSDisplayFieldBuilder {
  ...
  public PSTableValueBuilder(
    ...
    while (pages.hasNext()) {
      ...
      if (info.isSummaryEditor()) {
        m_parentId = ctx.getParentPageId(((Integer) entry.getKey()).intValue());
        break;
      }
    }
    ...
    while (mappings.hasNext()) {
      ...
      if (null != mapping.getUISet().getLabel()) label = mapping.getUISet().getLabel().getText();
      ...
      if (!m_showAllFields
          && ((!field.isShowInPreview() && !ctx.isEditMode()) || !field.isShowInSummary())) {
        continue;
      }
      ...
      if (parentBuilder.isBinaryField(field)) {
        builder = new PSDisplayFieldBuilder(field, mapping.getUISet(), parentBuilder);
      } else {
        builder = new PSSingleValueBuilder(field, mapping.getUISet(), locator, parentBuilder);
      }
    ...
    String[][] paramInfo = {
      {ctx.getSystemParam(PSContentEditorHandler.CONTENT_ID_PARAM_NAME), CONTENT_ID_EXTRACTOR_KEY},
      {
        ctx.getSystemParam(PSContentEditorHandler.REVISION_ID_PARAM_NAME), REVISION_ID_EXTRACTOR_KEY
      },
    ...
    String controlName = ctx.getInitParam(IPSConstants.HIDDEN_CONTROL_PARAM_NAME);
    while (params.hasNext()) {
      ...
      m_paramExtractors.put(paramPair[1], (Map.Entry) tmp.entrySet().iterator().next());
    }
    ...
    try {
      PSBackEndColumn col = new PSBackEndColumn(column.getTable(), IPSConstants.CHILD_ITEM_PKEY);
      Map tmp = new HashMap();
      tmp.put(
      ...
      m_paramExtractors.put(ROW_ID_EXTRACTOR_KEY, (Map.Entry) tmp.entrySet().iterator().next());
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(e.getLocalizedMessage());
    }
    ...
    for (int i = 0; i < vals.length; i++) {
      ...
      while (paramIter.hasNext() && null == location) {
        ...
        if (p.getName().equalsIgnoreCase((String) vals[i][1])) location = p.toString();
        if (null != location && location.trim().length() == 0) location = null;
      }
  ...
  protected boolean addDataElement(
    ...
    if (null == doc || null == parent) {
      throw new IllegalArgumentException("Document or parent element was null.");
    }
    ...
    try {
      ...
      if (headers.hasNext()) {
        ...
        while (headers.hasNext()) {
          ...
          colElem.appendChild(doc.createTextNode(headers.next().toString()));
      ...
      if (null != data && !isNewDoc) {
        try {
          ...
          while (data.readRow()) {
            addedRow = true;
            Map.Entry entry = (Map.Entry) m_paramExtractors.get(ROW_ID_EXTRACTOR_KEY);
            ...
            while (builders.hasNext()) {
              PSDisplayFieldBuilder builder = (PSDisplayFieldBuilder) builders.next();
              Element colElem = doc.createElement(COLUMN_NAME);
              rowElem.appendChild(colElem);
              Element control = builder.createControlNode(doc, data, isNewDoc);
            ...
            if (m_parentBuilder.getDocContext().isSummaryEditor()) {
              ...
              addRowEditActions(doc, actionsElem, data, rowId.toString());
            }
          }
        } finally {
  ...
  private boolean addHiddenFields(
    ...
    while (keys.hasNext()) {
      ...
      if (null == entry) {
        ...
        throw new IllegalArgumentException("extractor not found for: " + key);
      }
      IPSDataExtractor extractor = (IPSDataExtractor) entry.getValue();
  ...
  private boolean addRowEditActions(
    ...
    try {
      ...
      String commandParamName =
          docContext.getSystemParam(PSContentEditorHandler.COMMAND_PARAM_NAME);
      String rowIdParamName =
          docContext.getSystemParam(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME);
      String dbActionType = docContext.getRequestTypeHtmlParamName();
      String childIdParamName =
          docContext.getSystemParam(PSContentEditorHandler.CHILD_ID_PARAM_NAME);
      String pageIdParamName = docContext.getSystemParam(PSContentEditorHandler.PAGE_ID_PARAM_NAME);
      ...
      String view = getViewEvaluator().getNextView(data, m_parentBuilder.getPageId());
      ...
      params.add(new PSMapPair(dbActionType, docContext.getRequestTypeValueDelete()));
      params.add(new PSMapPair(IPSHtmlParameters.SYS_VIEW, view));
      Element elem = m_parentBuilder.createActionElement(doc, "Del", params.iterator(), true);
      ...
      if (m_supportsSequencing) {
        ...
        params.add(new PSMapPair(commandParamName, modifyCmdName));
        params.add(
            new PSMapPair(dbActionType, PSContentEditorHandler.DB_ACTION_SEQUENCE_DECREMENT));
        params.add(new PSMapPair(rowIdParamName, rowId));
        params.add(new PSMapPair(pageIdParamName, pageId));
        params.add(new PSMapPair(childIdParamName, childId));
        params.add(new PSMapPair(IPSHtmlParameters.SYS_VIEW, view));
        elem = m_parentBuilder.createActionElement(doc, "Up", params.iterator(), true);
        parent.appendChild(elem);
        if (m_imageLocations[MOVEUP_INDEX] != null) {
          elem.setAttribute(IMAGEREF_ATTRIB, m_imageLocations[MOVEUP_INDEX]);
        }
        ...
        params.add(new PSMapPair(commandParamName, modifyCmdName));
        params.add(
            new PSMapPair(dbActionType, PSContentEditorHandler.DB_ACTION_SEQUENCE_INCREMENT));
        params.add(new PSMapPair(rowIdParamName, rowId));
        params.add(new PSMapPair(pageIdParamName, pageId));
        params.add(new PSMapPair(childIdParamName, childId));
        params.add(new PSMapPair(IPSHtmlParameters.SYS_VIEW, view));
        elem = m_parentBuilder.createActionElement(doc, "Dn", params.iterator(), true);
        parent.appendChild(elem);
        if (m_imageLocations[MOVEDOWN_INDEX] != null) {
          elem.setAttribute(IMAGEREF_ATTRIB, m_imageLocations[MOVEDOWN_INDEX]);
        }
      }
      return true;
    } catch (PSNotFoundException nfe) {
      throw new PSDataExtractionException(nfe.getErrorCode(), nfe.getErrorArguments());
    }
  }
  ...
  private boolean addSummaryViewActions(
    ...
    while (possibleTargets.hasNext()) {
      ...
      if (!found && ((info.isRowEditor() && !hasData) || (info.isSummaryEditor() && hasData))) {
        found = true;
    ...
    boolean isNewDoc =
        m_parentBuilder.isNewDocument(data, m_parentBuilder.getDocContext().isRowEditor());
    ...
    if (!hasData && null == summaryBuilder) {
      ...
      if (!actionSet.hasNext()) return false;
      ...
      unwantedParams.put(
          docContext.getSystemParam(PSContentEditorHandler.COMMAND_PARAM_NAME), null);
      unwantedParams.put(docContext.getSystemParam(PSEditorDocumentBuilder.FORMACTION_NAME), null);
      // the following params will be re-added by the addLocalParams method
      unwantedParams.put(
          docContext.getSystemParam(PSContentEditorHandler.CHILD_ID_PARAM_NAME), null);
      unwantedParams.put(
          docContext.getSystemParam(PSContentEditorHandler.PAGE_ID_PARAM_NAME), null);
      unwantedParams.put(docContext.getSystemParam(IPSHtmlParameters.SYS_VIEW), null);
      ...
      while (actionSet.hasNext()) {
        ...
        while (extractors.hasNext()) {
          ...
          if (!unwantedParams.containsKey(name)) {
            ...
            params.add(new PSMapPair(name, extractor.extract(data).toString()));
          }
        }
        ...
        if (label.equalsIgnoreCase(summaryBuilder.ADDITEM_ACTION_LABEL)) {
          label =
              PSI18nUtils.getString(
                  PSI18nUtils.PSX_CE_ACTION
                      + PSI18nUtils.LOOKUP_KEY_SEPARATOR_LAST
                      + summaryBuilder.ADDITEM_ACTION_LABEL,
                  lang);
        }
        parent.appendChild(
            PSEditorDocumentBuilder.createActionElement(doc, label, params.iterator(), true));
    ...
    String actionLabel =
        PSI18nUtils.getString(
            PSI18nUtils.PSX_CE_ACTION + PSI18nUtils.LOOKUP_KEY_SEPARATOR_LAST + FORMSUBMIT_LABEL,
            lang);

    parent.appendChild(
        PSEditorDocumentBuilder.createActionElement(doc, actionLabel, params.iterator(), true));
    return true;
  }
  ...
  private void addLocalParams(
      boolean isNewDoc,
      int pageId,
      PSEditorDocumentContext docContext,
      PSExecutionData data,
      List params)
      throws PSDataExtractionException {
    params.add(
        new PSMapPair(
            docContext.getSystemParam(PSContentEditorHandler.PAGE_ID_PARAM_NAME), "" + pageId));

    String actionType =
        isNewDoc ? docContext.getRequestTypeValueInsert() : docContext.getRequestTypeValueUpdate();
    params.add(new PSMapPair(docContext.getRequestTypeHtmlParamName(), actionType));
    ...
    Object o = m_parentBuilder.getExtractor(m_parentBuilder.CONTENT_ID_EXTRACTOR_KEY).extract(data);
    if (null != o && o.toString().trim().length() > 0) {
      params.add(
          new PSMapPair(
              docContext.getSystemParam(PSContentEditorHandler.CONTENT_ID_PARAM_NAME),
              o.toString()));
    }

    o = m_parentBuilder.getExtractor(m_parentBuilder.REVISION_ID_EXTRACTOR_KEY).extract(data);
    if (null != o && o.toString().trim().length() > 0) {
      params.add(
          new PSMapPair(
              docContext.getSystemParam(PSContentEditorHandler.REVISION_ID_PARAM_NAME),
              o.toString()));
    }
