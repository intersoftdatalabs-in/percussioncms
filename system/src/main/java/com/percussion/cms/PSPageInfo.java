...
public class PSPageInfo {
  ...
  public PSPageInfo(int type, int childId, List queryHandlers, List pageMap) {
    ...
    m_type = type;
    m_childId = childId;
    if (null != queryHandlers) m_handlers.addAll(queryHandlers);
    if (null != pageMap) m_pageMap.addAll(pageMap);
  }
  ...
  public void setBuilder(PSEditorDocumentBuilder builder) {
    if (null == builder) throw new IllegalArgumentException("builder cannot be null");
    m_builder = builder;
  }
