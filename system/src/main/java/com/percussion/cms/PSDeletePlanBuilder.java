...
public class PSDeletePlanBuilder extends PSModifyPlanBuilder {
  ...
  public PSModifyPlan createModifyPlan(PSDisplayMapper mapper, PSFieldSet fieldSet)
    ...
    deleteMappings =
        addTableKeysForDelete(getSystemDeleteMappings(mapper, fieldSet), mapper, fieldSet);
    ...
    deleteSystemMapper = PSApplicationBuilder.createSystemMappings(deleteMappings.iterator());

    // create delete resource
    String deleteRequestName = "Delete" + mapper.getId();
    PSApplicationBuilder.createDeleteDataset(
        m_internalApp, deleteRequestName, m_ce, null, null, deleteSystemMapper);
