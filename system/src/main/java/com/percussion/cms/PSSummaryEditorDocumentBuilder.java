...
public class PSSummaryEditorDocumentBuilder extends PSModifyDocumentBuilder {
   ...
   */
  public PSSummaryEditorDocumentBuilder(
      PSContentEditor ce,
    ...
    if (null == mapping) throw new IllegalArgumentException("mapping can't be null");
    ...
    String contentIdParam = ctx.getSystemParam(PSContentEditorHandler.CONTENT_ID_PARAM_NAME);
    String revisionIdParam = ctx.getSystemParam(PSContentEditorHandler.REVISION_ID_PARAM_NAME);
    Object[][] hiddenParamSet = {
      {contentIdParam, new PSHtmlParameter(contentIdParam)},
      {revisionIdParam, new PSHtmlParameter(revisionIdParam)},
    };
    ...
    PSPageInfo info = (PSPageInfo) m_docContext.getPageInfoMap().get(new Integer(getPageId()));
    Iterator childrenPageIds = info.getPageIdList();
    String rowEditorPageId = childrenPageIds.next().toString();

    PSCustomActionGroup group =
        ce.getCustomActionGroup(
            PSLocation.PAGE_SUMMARY_VIEW, PSLocation.TYPE_FORM, mapping.getFieldRef());
    m_submitActions =
        createActionLinkList(
            group,
            "" + mapping.getDisplayMapper().getId(),
            rowEditorPageId,
            "" + getPageId(),
            ctx.getRequestUrl());
  }
  ...
  protected Iterator getActionLinks(Document doc, PSExecutionData data)
    ...
    while (actionSet.hasNext()) {
      ...
      while (extractors.hasNext()) {
        ...
        if (name != FORMACTION_NAME) value = URLEncoder.encode(value);
        ...
        label =
            PSI18nUtils.getString(
                PSI18nUtils.PSX_CE_ACTION
                    ...
                    + ADDITEM_ACTION_LABEL,
                lang);
      }

      actions.add(createActionElement(doc, label, params.iterator(), true));
   ...
   */
  private List createActionLinkList(
      PSCustomActionGroup customActions,
      String mapperId,
      String rowPageId,
      String thisPageId,
      String url) {
    try {
      ...
      if (null != customActions) {
        ...
        if (sequence < 0) sequence = 1000; // arbitrarily large
        ...
        while (links.hasNext()) {
          ...
          pair =
              new PSMapPair(
                  m_docContext.getSystemParam(PSContentEditorHandler.PAGE_ID_PARAM_NAME),
                  PSDataExtractorFactory.createReplacementValueExtractor(
          ...
          pair =
              new PSMapPair(
                  m_docContext.getSystemParam(IPSHtmlParameters.SYS_VIEW),
                  new PSViewExtractor(getViewEvaluator(), getPageId()));
          extractors.add(pair);

          // move the position of the standard button, if necessary
          if (sequence >= 0 && sequence <= stdPos) stdPos++;

          addAction(actionList, link.getDisplayText().getText(), extractors, sequence++);
        }
        ...
        while (removeActions.hasNext()) {
          ...
          if (actionName.equalsIgnoreCase(ADD_ITEM_ACTION_NAME)) addStandard = false;
        }
      }
  ...
  private void addAction(List actions, String label, List paramExtractors, int pos) {
    ...
    if (pos <= 0) actions.add(0, entry);
    else if (pos > actions.size()) actions.add(entry);
    else actions.add(pos, entry);
  }
