...
public class PSChoiceBuilder {
  ...
  public static boolean addChoiceElement(
    ...
    return addChoiceElement(doc, parent, choices, data, isNewDoc, ignoreNullEntry, true);
  }
  ...
  public static Iterator getGlobalLookupEntries(String key, PSRequest request)
    ...
    String lang =
        (String)
            request.getUserSession().getPrivateObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);
    if (lang == null) lang = PSI18nUtils.DEFAULT_LANG;

    HashMap paramMap = new HashMap(1);
    paramMap.put("key", key);

    PSInternalRequest intReq =
        PSServer.getInternalRequest(IPSConstants.GLOBAL_LOOKUP, request, paramMap, false);
  ...
  public static boolean addChoiceElement(
    ...
    if (!ignoreFilter) filter = choices.getChoiceFilter();

    switch (choices.getType()) {
      case PSChoices.TYPE_GLOBAL:
        result =
            addGlobalChoiceElement(doc, parent, choices, data, isNewDoc, ignoreNullEntry, filter);
        break;

      case PSChoices.TYPE_LOCAL:
        result = addLocalChoiceElement(doc, parent, choices, isNewDoc, ignoreNullEntry, filter);
        break;
      ...
      case PSChoices.TYPE_INTERNAL_LOOKUP:
        result =
            addInternalLookupChoiceElement(
                doc, parent, choices, data, isNewDoc, ignoreNullEntry, filter);
        break;

      case PSChoices.TYPE_TABLE_INFO:
        result =
            addTableChoiceElement(doc, parent, choices, data, isNewDoc, ignoreNullEntry, filter);
        break;
  ...
  private static boolean addTableChoiceElement(
      Document doc,
      Element parent,
      PSChoices choices,
      PSExecutionData data,
    ...
    if (choices.getType() != PSChoices.TYPE_TABLE_INFO || choices.getTableInfo() == null)
      return addedElem;
    ...
    try {
      addedElem =
          addChoiceElement(
              ...
              parent,
              entries,
              choices.getNullEntry(),
              choices.getDefaultSelected(),
              choices.getSortOrder(),
              isNewDoc,
              ignoreNullEntry,
              filter);
    } catch (PSException e) {
  ...
  private static void setDynamicWorkflows(Element control, PSRequest req) {
    ...
    if (!control
        .getAttribute(PSDisplayFieldElementBuilder.PARAMNAME_NAME)
        .equals(IPSHtmlParameters.SYS_WORKFLOWID)) {
      return;
    }
    ...
    for (int i = 0; i < datasets.size(); i++) {
      ...
      if (dataset instanceof PSContentEditor) ce = (PSContentEditor) dataset;
    }
    if (ce == null) return;
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (id.equals(defaultWorkflowId)) {
        workflow.setAttribute(SELECTED_ATTRIBUTE_NAME, ATTRIB_BOOLEAN_TRUE);
        break;
      }
    }
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (wfInfo.isExclusionary()) {
         ...
         * remove it.
         */
        if (found) workflowNodesToRemove.add(workflow);
  ...
  private static boolean addGlobalChoiceElement(
      ...
      PSChoiceFilter filter)
      throws PSDataExtractionException {
    if (doc == null || parent == null || choices == null || execData == null)
      throw new IllegalArgumentException(PARAMETER_NOT_NULL);

    boolean addedElem = false;
    if (choices.getType() != PSChoices.TYPE_GLOBAL) return addedElem;

    PSRequest request = execData.getRequest();
    try {
      Iterator entries = getGlobalLookupEntries(Integer.toString(choices.getGlobal()), request);
  ...
  private static boolean addLocalChoiceElement(
    ...
    if (choices.getType() != PSChoices.TYPE_LOCAL) return false;

    return addChoiceElement(
        doc,
        parent,
        choices.getLocal(),
        choices.getNullEntry(),
  ...
  private static boolean addLookupChoiceElement(
    ...
    if (choices.getType() != PSChoices.TYPE_LOOKUP) return false;
    ...
    displayChoices.setAttribute(HREF_ATTRIBUTE_NAME, url.toExternalForm());
  ...
  private static boolean addInternalLookupChoiceElement(
    ...
    if (choices.getType() != PSChoices.TYPE_INTERNAL_LOOKUP) return false;
    ...
    try {
      ...
      paramMap.put(IPSHtmlParameters.SYS_SESSIONID, requestContext.getUserSessionId());

      PSInternalRequest irequest =
          PSServer.getInternalRequest(url.getPath(), request, paramMap, false);
      ...
      if (null == irequest) {
        ...
        entries.add(new PSEntry("invalid", new PSDisplayText("Error! Invalid Choice Resource")));
      } else {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        IPSDataExtractor extractor =
            PSDataExtractorFactory.createReplacementValueExtractor(param.getValue());
  ...
  private static URL getLookupUrl(
    ...
    String lang =
        (String)
            request.getUserSession().getSessionObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);
    if (lang == null) lang = PSI18nUtils.DEFAULT_LANG;
    try {
      PSRequestContext requestContext = new PSRequestContext(request);
      ...
      if (converter != null) {
        try {
          ...
          if (result instanceof URL) url = (URL) result;
          else {
            throw new PSDataExtractionException(
                lang,
        ...
        if (addParams) {
          ...
          params.put(IPSHtmlParameters.SYS_SESSIONID, requestContext.getUserSessionId());
        ...
        if (isInternalRequest
            && href.startsWith(RELATIVE_URL_PREFIX)
        ...
        url =
            PSUrlUtils.createUrl(
                requestContext.getServerHostAddress(),
  ...
  private static boolean addChoiceElement(
      ...
      Element parent,
      Iterator choices,
      PSNullEntry nullEntry,
      Iterator defaultSelected,
      int sortOrder,
      boolean isNewDoc,
      boolean ignoreNullEntry,
      PSChoiceFilter filter)
    ...
    try {
      ...
      Element valEl = PSXMLDomUtil.getFirstElementChild(parent, DISPLAYVALUE_NAME);
      if (valEl != null) selectedValue = PSXMLDomUtil.getElementData(valEl);
    ...
    while (choices.hasNext()) {
      ...
      if (values.contains(value)
          || (!ignoreNullEntry && nullEntry != null && value.equals(nullEntry.getValue()))) {
      ...
      if (mapEntry == null) mapEntry = new ArrayList();
    ...
    while (keys.hasNext()) {
      ...
      if (key.compareTo(noSequence) == 0) noSequenceValue = value;
      else entries.addAll(value);
    }
    if (noSequenceValue != null) entries.addAll(noSequenceValue);
    ...
    if (!ignoreNullEntry)
      addedNullEntry = addNullEntry(doc, displayChoices, nullEntry, isNewDoc, selectedValue);
      ...
      while (defaultSelected.hasNext()) {
        ...
        switch (ds.getType()) {
          case PSDefaultSelected.TYPE_NULL_ENTRY:
            if (nullEntry != null) select(displayChoices, nullEntry.getLabel().getText());
            break;
    ...
    if (filter != null) displayChoices.appendChild(filter.toXml(doc));

    if (addedElem || addedNullEntry) parent.appendChild(displayChoices);
    return addedElem;
  }
  ...
  public static void select(Element displayChoices, String value) {
    if (displayChoices == null || value == null)
      throw new IllegalArgumentException("displayChoices and value cannot be null");

    NodeList entries = displayChoices.getElementsByTagName(DISPLAYENTRY_NAME);
    int i = 0;
    Node node;
  ...
  private static void sortEntries(ArrayList entries, int sortOrder) {
    if (!entries.isEmpty()) {
      ...
      entries.clear();
      for (int i = 0; i < array.length; i++) entries.add(array[i]);
    }
  }
  ...
  private static boolean addNullEntry(
    ...
    if (nullEntry == null) return addedElem;
    ...
    if (nullElement != null) {
      switch (nullEntry.getSortOrder()) {
        ...
        case PSNullEntry.SORT_ORDER_SORTED:
          ...
          parent.insertBefore(nullElement, nodes.item(nullEntry.getSequence()));
          addedElem = true;
          break;
  ...
  private static Element createDisplayEntry(Document doc, String value, String label, String type) {
    ...
    displayEntry.setAttribute(SELECTED_ATTRIBUTE_NAME, ATTRIB_BOOLEAN_FALSE);

    if (type != null)
      displayEntry.setAttribute(PSDisplayFieldElementBuilder.SOURCE_TYPE_NAME, type);
  ...
  private static class ElementComparatorAscending implements Comparator {
    ...
    public int compare(Object left, Object right) {
      ...
      String rightValue = getFirstTagText((Element) right, DISPLAYLABEL_NAME);
  ...
  public static String getFirstTagText(Element elem, String tag) {
    ...
    if (node == null) {
      throw new IllegalStateException("tagname " + tag + " not found in provided element");
    } else {
      ...
      while (text != null) {
         ...
         * immediate children
         */
        if (text instanceof Text) labelText += ((Text) text).getNodeValue();
        else break;
  ...
  public static List loadDisplayChoices(Element src) throws PSUnknownNodeTypeException {
    ...
    Element entryEl =
        tree.getNextElement(DISPLAYENTRY_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (entryEl != null) {
      String val = null;
      Element valEl =
          tree.getNextElement(DISPLAYVALUE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      ...
      String label = null;
      Element labelEl =
          tree.getNextElement(DISPLAYLABEL_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      ...
      entryEl = tree.getNextElement(DISPLAYENTRY_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  ...
  public static Element createDisplayChoices(Document doc, Iterator choices) {
    ...
    while (choices.hasNext()) {
      PSEntry entry = (PSEntry) choices.next();
      dispChoicesEl.appendChild(
          createDisplayEntry(doc, entry.getValue(), entry.getLabel().getText(), null));
    }
