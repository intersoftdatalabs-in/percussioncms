...
public class PSDisplayChoices implements Cloneable {
  ...
  public PSDisplayChoices(Element src) throws PSUnknownNodeTypeException {
    ...
    // default to true
    boolean choicesLoaded = !ATTR_VAL_FALSE.equalsIgnoreCase(src.getAttribute(CHOICES_LOADED_ATTR));
      ...
      while (entryEl != null) {
        ...
        boolean emptyEntry =
            (val == null
                ...
                || label.trim().length() == 0);
        // Skip empty entries
        if (!emptyEntry) m_choices.add(new PSEntry(val, new PSDisplayText(label)));
    ...
    tree.setCurrent(src);
    Element filterEl =
        tree.getNextElement(PSChoiceFilter.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (filterEl != null) m_filter = new PSChoiceFilter(filterEl, null, null);
  }
  ...
  public Element toXml(Document doc) {
    ...
    dispChoicesEl.setAttribute(CHOICES_LOADED_ATTR, choicesLoaded ? ATTR_VAL_TRUE : ATTR_VAL_FALSE);
    ...
    if (choicesLoaded) {
      ...
      while (choices.hasNext()) {
        ...
        dispChoicesEl.appendChild(
            createDisplayEntry(doc, entry.getValue(), entry.getLabel().getText()));
      }
    }

    if (m_filter != null) dispChoicesEl.appendChild(m_filter.toXml(doc));
