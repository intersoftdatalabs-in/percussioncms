...
public class PSActionVisibilityChecker {
  ...
  public PSActionVisibilityChecker(
    ...
    if (ctx != null && !ctx.isEmpty()) m_contexts = ctx;
  }
  ...
  public boolean isVisible(
    ...
    if (objStates != null && objStates.size() > 1 && !supportsMultipleObjects()) return false;
    if (m_contexts == null) return true;

    boolean visible = true;
    if (globalState != null) visible = checkGlobalState(globalState);
    if (objStates != null && visible) visible = checkObjectState(objStates);
    return visible;
  }

  private boolean checkObjectState(Collection<PSActionVisibilityObjectState> objStates) {
    for (PSActionVisibilityObjectState os : objStates) {
      ...
      while (ctxIter.hasNext()) {
        ...
        while (valIter.hasNext()) {
          ...
          if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_FOLDER_SECURITY)) {
            int access = PSObjectPermissions.translateAccess(value);

            int higherAccessLevel = PSObjectPermissions.getHigherAccessLevel(access);

            PSObjectPermissions perm = os.getFolderPermissions();

            if (perm != null && !perm.hasAccess(higherAccessLevel)) return false;
          } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_CHECKOUT_STATUS)) {
          ...
          } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_PUBLISHABLE_TYPE)) {
            if (value.equalsIgnoreCase(os.getPublishableType())) return false;
          } else {
            if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_ASSIGNMENT_TYPE)) {
              if (convert(value) == os.getAssignmentType()) return false;
            } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_CONTENT_TYPE)) {
              if (convert(value) == os.getContentTypeUuid()) return false;
            } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_OBJECT_TYPE)) {
              if (convert(value) == os.getObjectType()) return false;
            } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_WORKFLOWS_TYPE)) {
              if (convert(value) == os.getWorkflowAppUuid()) return false;
            }
          }
        }
      }
  ...
  private int convert(String num) {
    int ctxValue = -1;
    if (StringUtils.isNotBlank(num)) ctxValue = Integer.parseInt(num);
    return ctxValue;
  }
  ...
  private boolean checkGlobalState(PSActionVisibilityGlobalState globalState) {
    ...
    while (iter.hasNext()) {
      ...
      if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_COMMUNITY)) {
        Iterator valIter = vc.iterator();
        String communityId = String.valueOf(globalState.getCommunityUuid());
        while (valIter.hasNext()) {
          String value = (String) valIter.next();
      ...
      } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_ROLES_TYPE)) {
        ...
        if (valueList.containsAll(roleList)) return false;
      } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_LOCALES_TYPE)) {
        Iterator valIter = vc.iterator();
        String locale = globalState.getLocale();
        while (valIter.hasNext()) {
          String value = (String) valIter.next();
          if (value.equalsIgnoreCase(locale)) return false;
        }
      } else if (context.equals(PSActionVisibilityContext.VIS_CONTEXT_CLIENT_CONTEXT)) {
        // drag drop, multi-select, etc..
        Iterator valIter = vc.iterator();
        while (valIter.hasNext()) {
          String value = (String) valIter.next();
          if (value.equalsIgnoreCase(globalState.getClientContext())) return false;
        }
      }
    }
    return true;
   ...
   */
  public boolean isVisible(
      PSActionVisibilityGlobalState globalState, PSActionVisibilityObjectState ctxInstance) {
