...
public class PSChildInsertPlanBuilder extends PSModifyPlanBuilder {
  ...
  /**
   * Creates a plan that will perform an insert of the complex child item specified by the supplied
  ...
  public PSModifyPlan createModifyPlan(PSDisplayMapper mapper, PSFieldSet fieldSet)
     ...
     * inserted.
     */
    PSDataMapper childInsertSystemMapper = getSystemUpdateMapper(mapper, fieldSet);
    ...
    PSDataMapper insertSystemMapper =
        PSApplicationBuilder.createSystemMappings(
            addTableKeys(insertSysMappings, mapper, fieldSet, true).iterator());
    ...
    if (allowMultipleInserts(mapper, fieldSet)) allowMultiRowInsert = true;
    ...
    PSApplicationBuilder.createUpdateDataset(
        m_internalApp, sysReqName, m_ce, null, null, childInsertSystemMapper, false);
    plan.addModifyStep(
        new PSUpdateStep(
            sysReqName,
            m_internalApp.getRequestTypeHtmlParamName(),
