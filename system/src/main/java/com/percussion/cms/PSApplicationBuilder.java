...
public class PSApplicationBuilder {
  ...
  public static PSApplication createApplication(String appName, PSApplication srcApp) {
    ...
    if (srcApp == null) throw new IllegalArgumentException("srcApp may not be null");
    ...
    try {
      ...
      app.setRequestTypeHtmlParamName(srcApp.getRequestTypeHtmlParamName());
  ...
  public static void createInsertDataset(
      ...
      throws PSSystemValidationException {
    createUpdateDataset(
        app, requestName, ce, mapper, xmlActionTypeField, systemMappings, true, FLAG_ALLOW_INSERTS);
  }
  ...
  public static void createDeleteDataset(
      ...
      throws PSSystemValidationException {
    createUpdateDataset(
        app, requestName, ce, mapper, xmlActionTypeField, systemMappings, true, FLAG_ALLOW_DELETES);
  }
  ...
  public static void createUpdateDataset(
      ...
      PSDtdBuilder dtd)
      throws PSSystemValidationException {
    if (null == app || null == requestName || null == ce || requestName.length() == 0) {
    ...
    try {
      ...
      if (buildDtd) {
        ...
        dtdRoot = dtdFile.getName().substring(0, dtdFile.getName().lastIndexOf('.'));
      ...
      if (xmlActionTypeField != null) page.setActionTypeXmlField(xmlActionTypeField);
      ds.setPageDataTank(page);

      // create pipe
      PSUpdatePipe pipe = new PSUpdatePipe(PSUniqueObjectGenerator.makeUniqueName("UpdatePipe"));
      ...
      if (systemMappings != null) {
        ...
        while (sysMappings.hasNext()) {
          ...
          if (beMapping instanceof PSBackEndColumn) {
            PSBackEndTable sysTable = ((PSBackEndColumn) beMapping).getTable();
            String tableRef = sysTable.getAlias();

            PSDataMapper dataMapper = (PSDataMapper) dataMappers.get(tableRef.toLowerCase());
            if (dataMapper == null) {
      ...
      boolean allowInserts = ((allowActions & FLAG_ALLOW_INSERTS) == FLAG_ALLOW_INSERTS);
      boolean allowUpdates = ((allowActions & FLAG_ALLOW_UPDATES) == FLAG_ALLOW_UPDATES);
      boolean allowDeletes = ((allowActions & FLAG_ALLOW_DELETES) == FLAG_ALLOW_DELETES);
      ...
      if (mapper != null) {
        ...
        if (mapperFieldSetRef.equals(mainFieldSet.getName())) mapperFieldSet = mainFieldSet;
        else mapperFieldSet = ceMapper.getFieldSet(mapperFieldSetRef);
        ...
        while (mappings.hasNext()) {
          ...
          if (o == null) {
            ...
            o = mapperFieldSet.getChildField(fieldRef, PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
            if (o == null) {
          ...
          if (beMapping instanceof PSBackEndColumn) {
             ...
             */
            if (isReservedColumn(column.getColumn())) continue;
             ...
             */
            if (!includeBinary && field.isForceBinary()) continue;
             ...
             * column.
             */
            PSBackEndColumn newColumn = new PSBackEndColumn(column.getTable(), column.getColumn());
            ...
            if (buildDtd) {
              dtd.addElement(submitName, PSDtdBuilder.OCCURS_ONCE, dtdRoot);
            }
            ...
            PSDataMapping dataMapping =
                new PSDataMapping(new PSHtmlParameter(submitName), beMapping);
      ...
      while (tableRefs.hasNext()) {
        ...
        while (dataMappings.hasNext()) {
          ...
          PSBackEndColumn beCol = (PSBackEndColumn) dataMapping.getBackEndMapping();

          // if not including binary, check metadata now
          if (!includeBinary
              && PSMetaDataCache.isBinaryBackendColumn(
                  (PSTableSet) tableSetMap.get(beCol.getTable().getAlias().toLowerCase()), beCol)) {
            continue;
          }
      ...
      if (datasets == null) datasets = new PSCollection(PSDataSet.class);
      datasets.add(ds);
  ...
  public static String createQueryDataset(
      ...
      Iterator auxMappings,
      Iterator sortColumns)
      throws PSSystemValidationException {
    if (null == app
        || null == ce
        || null == dispMapper
    ...
    try {
      PSQueryPipe pipe = new PSQueryPipe(PSUniqueObjectGenerator.makeUniqueName("QueryPipe"));
      PSDataSet ds = new PSDataSet(PSUniqueObjectGenerator.makeUniqueName("DataSet"));
      ...
      URL dtdUrl = new URL("file:" + dtdFile.getName());
      PSPageDataTank page = new PSPageDataTank(dtdUrl);
      String dtdRoot = dtdFile.getName().substring(0, dtdFile.getName().lastIndexOf('.'));
      ...
      PSFieldSet mainFieldSet = ceMapper.getFieldSet();
      PSFieldSet fieldSet;
      if (mainFieldSet.getName().equals(dispMapper.getFieldSetRef())) fieldSet = mainFieldSet;
      else fieldSet = (PSFieldSet) mainFieldSet.get(dispMapper.getFieldSetRef());
      ...
      while (dispMappings.hasNext()) {
        ...
        if (null == o) {
          o = fieldSet.getChildField(fieldRef, PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
          if (null == o) {
          ...
          switch (field.getType()) {
            ...
            case PSField.TYPE_LOCAL:
              if (!sdmpField) {
                ...
                sdmpTables.put(col.getTable().getAlias().toLowerCase(), col.getTable());
              }
              break;
          }
        } else {
          List nonCol = (List) colMap.get("nbe");
          if (null == nonCol) nonCol = new ArrayList();
      ...
      if (!sdmpTables.isEmpty()) {
        ...
        while (sdmpTablesIter.hasNext()) {
          ...
          table = (PSBackEndTable) tableMap.get(table.getAlias().toLowerCase());
      ...
      while (colSets.hasNext()) {
        ...
        while (dispMappings.hasNext()) {
          ...
          if (null == field) {
            ...
            field = fieldSet.getChildField(fieldRef, PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
          }

          IPSBackEndMapping beMapping = field.getLocator();
          if (beMapping instanceof PSBackEndColumn) {
            PSBackEndColumn beCol = (PSBackEndColumn) beMapping;
            table = (PSBackEndTable) tableMap.get(beCol.getTable().getAlias().toLowerCase());
            beCol.setTable(table);
            if (PSMetaDataCache.isBinaryBackendColumn(
                (PSTableSet) tableSetMap.get(beCol.getTable().getAlias()), beCol)) {
              continue;
            }
          }
          PSDataMapping mapping =
              new PSDataMapping(dtdRoot + "/" + field.getSubmitName(), beMapping);
      ...
      while (auxMappings.hasNext()) {
        ...
        PSDataMapping dm = new PSDataMapping(dtdRoot + "/" + (String) entry.getKey(), col);
        Integer groupIdObj = (Integer) groupMap.get(col.getTable().getAlias().toLowerCase());
        if (null == groupIdObj) {
          // this should never happen
          throw new RuntimeException("Local table " + localTable + " not found in group map");
        }
      ...
      if (null != systemTable) {
        // todo: dbg
        PSBackEndColumn leftCol = new PSBackEndColumn(localTable, IPSConstants.ITEM_PKEY_CONTENTID);
      ...
      while (sdmpTableIter.hasNext()) {
        ...
        // todo: dbg
        PSBackEndColumn leftCol = new PSBackEndColumn(localTable, IPSConstants.ITEM_PKEY_CONTENTID);
        PSBackEndColumn rightCol = new PSBackEndColumn(table, IPSConstants.ITEM_PKEY_CONTENTID);
        PSBackEndJoin join = new PSBackEndJoin(leftCol, rightCol);
        if (null == joins) joins = new PSCollection(join.getClass());
        joins.add(join);
        leftCol = new PSBackEndColumn(localTable, IPSConstants.ITEM_PKEY_REVISIONID);
        rightCol = new PSBackEndColumn(table, IPSConstants.ITEM_PKEY_REVISIONID);
        join = new PSBackEndJoin(leftCol, rightCol);
        joins.add(join);
      }
      if (null != joins) tank.setJoins(joins);
      pipe.setBackEndDataTank(tank);
      ...
      while (selectionKeys.hasNext()) {
        ...
        IPSReplacementValue value = new PSSingleHtmlParameter((String) entry.getValue());
        PSWhereClause clause =
            new PSWhereClause(beColumn, PSWhereClause.OPTYPE_EQUALS, value, true);

        if (null == clauses) clauses = new PSCollection(clause.getClass());
        clauses.add(clause);
      }
      ...
      if (sortColumns.hasNext()) {
        while (sortColumns.hasNext()) {
          ...
          if (null == sortedCols) sortedCols = new PSCollection(sortedCol.getClass());
  ...
  public static String createQueryDataset(
      ...
      throws PSSystemValidationException {
    return createQueryDataset(app, dataMapper, selectionKeys, sortCols, null, false);
  }
  ...
  public static String createQueryDataset(
      PSApplication app,
      PSDataMapper dataMapper,
      Iterator selectionKeys,
      Iterator sortCols,
      PSDtdBuilder dtd,
      boolean isBinary)
      throws PSSystemValidationException {
    if (null == app || null == dataMapper || null == selectionKeys) {
      throw new IllegalArgumentException("One or more params was null.");
    }
    ...
    try {
      ...
      while (mappings.hasNext()) {
        ...
        if (buildDtd) dtd.addElement(dataMapping.getXmlField(), PSDtdBuilder.OCCURS_ONCE, dtdRoot);
      ...
      if (tableMap.size() != 1) {
        throw new IllegalArgumentException("dataMapper must result in exactly one table");
      }
      ...
      while (selectionKeys.hasNext()) {
        ...
        IPSReplacementValue value = new PSSingleHtmlParameter((String) entry.getValue());
        PSWhereClause clause = new PSWhereClause(name, PSWhereClause.OPTYPE_EQUALS, value, true);
        if (null == clauses) clauses = new PSCollection(clause.getClass());
        clauses.add(clause);
      }
      selector.setWhereClauses(clauses);
  ...
  private static void addMapping(PSDataMapper mapper, PSDataMapping mapping) {
    ...
    if (!mappingAdded) mapper.add(mapping);
  }
  ...
  public static ArrayList<PSSystemMapping> getSystemInsertMappings(
      PSContentEditorHandler ceHandler, PSContentEditor ce) {
    ...
    try {
      ...
      String sysTableRef = PSContentEditorSystemDef.CONTENT_STATUS_TABLE_ALIAS;
      PSBackEndTable sysTable = new PSBackEndTable(sysTableRef);
      PSBackEndCredential sysCred = new PSBackEndCredential("cred");
      ceHandler.getSystemDef().populateSystemTableInfo(sysTableRef, sysCred, sysTable);
      ...
      mapping =
          new PSSystemMapping(
              ...
              "CURRENTREVISION",
              new PSHtmlParameter(
                  ceHandler.getParamName(PSContentEditorHandler.REVISION_ID_PARAM_NAME)));
      ...
      mapping = new PSSystemMapping(sysTable, "CONTENTCHECKOUTUSERNAME", value);
      mappings.add(mapping);

      // add contentCreatedBy = username
      mapping = new PSSystemMapping(sysTable, "CONTENTCREATEDBY", new PSUserContext("User/Name"));
      mappings.add(mapping);

      // Create UDF's to put in a literal "1" and today's date
      // TODO: at some point we don't want hard coded udfs here
      PSExtensionRef literalRef =
          new PSExtensionRef("Java", "global/percussion/generic/", "sys_Literal");

      // add contentCreatedDate = current datetime
      mapping = new PSSystemMapping(sysTable, "CONTENTCREATEDDATE", getTodaysDateUdf());
      mappings.add(mapping);

      // make a UDF for literal "1"
      PSExtensionParamValue literalOne = new PSExtensionParamValue(new PSTextLiteral("1"));
      PSExtensionParamValue[] literalOneParams = {literalOne};
      PSExtensionCall literalOneUdf = new PSExtensionCall(literalRef, literalOneParams);

      // add contentTypeId = type specified in content editor def
      PSExtensionParamValue literalType =
          new PSExtensionParamValue(new PSTextLiteral(String.valueOf(ce.getContentType())));
      PSExtensionParamValue[] literalTypeParams = {literalType};
      PSExtensionCall literalTypeUdf = new PSExtensionCall(literalRef, literalTypeParams);

      mapping = new PSSystemMapping(sysTable, "CONTENTTYPEID", literalTypeUdf);
      mappings.add(mapping);

      // add workflowAppId = id from the request
      if (ceHandler != null
          && ceHandler.getCmsObject() != null
          && ceHandler.getCmsObject().isWorkflowable()) {
        mapping =
            new PSSystemMapping(
                sysTable,
                "WORKFLOWAPPID",
        ...
        // UDF prepare start
        PSExtensionRef wfInitStateLookupRef =
            new PSExtensionRef("Java", "global/percussion/cms/", "sys_CmsObjectNameLookup");

        PSExtensionParamValue paramLookupResource =
            new PSExtensionParamValue(new PSTextLiteral("WorkflowInitialState"));

        PSExtensionParamValue paramWorkflowid =
            new PSExtensionParamValue(new PSSingleHtmlParameter(IPSHtmlParameters.SYS_WORKFLOWID));

        PSExtensionParamValue[] params = {paramLookupResource, paramWorkflowid};
        PSExtensionCall wfInitStateLookupUdf = new PSExtensionCall(wfInitStateLookupRef, params);
        // UDF prepare end

        mapping = new PSSystemMapping(sysTable, "CONTENTSTATEID", wfInitStateLookupUdf);
        mappings.add(mapping);
      ...
      // add EDITREVISION = 1
      mapping = new PSSystemMapping(sysTable, IPSConstants.ITEM_EDITREVISION, literalOneUdf);
      mappings.add(mapping);

      // add TIPREVISION = 1
      mapping = new PSSystemMapping(sysTable, IPSConstants.ITEM_TIPREVISION, literalOneUdf);
      mappings.add(mapping);
    ...
    try {
      ...
      mapping =
          new PSSystemMapping(
              ...
              "CONTENTID",
              new PSHtmlParameter(
                  ceHandler.getParamName(PSContentEditorHandler.CONTENT_ID_PARAM_NAME)));
      mappings.add(mapping);

      // add contentLastModified = username
      mapping =
          new PSSystemMapping(sysTable, "CONTENTLASTMODIFIER", new PSUserContext("User/Name"));
      mappings.add(mapping);

      // add contentLastModifiedDate = current datetime
      mapping = new PSSystemMapping(sysTable, "CONTENTLASTMODIFIEDDATE", getTodaysDateUdf());
      mappings.add(mapping);
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(e.getLocalizedMessage());
    }
  ...
  private static PSExtensionCall getTodaysDateUdf() {
    // TODO: at some point we don't want hard coded udfs here
    PSExtensionRef dateFormatRef =
        new PSExtensionRef("Java", "global/percussion/generic/", "sys_DateFormat");

    // pass empty params to get current datetime at runtime
    PSExtensionParamValue first = new PSExtensionParamValue(new PSTextLiteral(""));
    PSExtensionParamValue second = new PSExtensionParamValue(new PSTextLiteral(""));
    PSExtensionParamValue[] todaysDateParams = {first, second};
    PSExtensionCall todaysDateUdf = new PSExtensionCall(dateFormatRef, todaysDateParams);
  ...
  public static PSDataMapper createSystemMappings(Iterator mappings) {
    ...
    try {
      ...
      while (mappings.hasNext()) {
        ...
        column = new PSBackEndColumn(sysMapping.getTable(), sysMapping.getColumnName());
  ...
  public static boolean isReservedColumn(String columnName) {
    ...
    if (columnName.equalsIgnoreCase(IPSConstants.CHILD_SORT_KEY) || isKeyColumn(columnName)) {
      isReserved = true;
    }
