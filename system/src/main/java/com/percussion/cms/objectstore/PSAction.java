...
public class PSAction extends PSVersionableDbComponent implements IPSCatalogSummary, IPSCloneTuner {
   ...
   */
  public PSAction(
      String name, String label, String type, String url, String handler, int sortrank) {
    ...
    if (!(HANDLER_CLIENT.equals(handler) || HANDLER_SERVER.equals(handler))) {
      throw new IllegalArgumentException(
          "handler must be one of the following:" + HANDLER_CLIENT + "," + HANDLER_SERVER);
    }
    ...
    m_props.setProperty(PSAction.PROP_REFRESH_HINT, RefreshHint.NONE.getValue());
  }
  ...
  public static PSKey createKey(String value) {
    ...
    if (null == value || value.trim().length() == 0) key = new PSSimpleKey(PRIMARY_KEY);
    else key = new PSSimpleKey(PRIMARY_KEY, value);
  ...
  public static int getIdFromGuid(IPSGuid guid) {
    if (guid.getType() != PSTypeEnum.ACTION.getOrdinal())
      throw new IllegalArgumentException("guid must be PSTypeEnum.ACTION type.");
  ...
  public void setModeUIContexts(PSDbComponentCollection modeCtxs) {
    if (modeCtxs == null) throw new IllegalArgumentException("modeCtxs may not be null.");
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_description = "";
    Element descEl = PSComponentUtils.getChildElement(sourceNode, "Description", false);
    if (descEl != null) {
      Node descText = descEl.getFirstChild();
      if (descText instanceof Text) m_description = ((Text) descText).getData();
    }

    Element propsEl =
        PSComponentUtils.getChildElement(sourceNode, PSActionProperties.XML_NODE_NAME, false);
    if (propsEl != null) m_props.fromXml(propsEl);
    else m_props.clear();

    Element paramsEl =
        PSComponentUtils.getChildElement(sourceNode, PSActionParameters.XML_NODE_NAME, false);
    if (paramsEl != null) m_params.fromXml(paramsEl);
    else m_params.clear();

    Element visCtxEl =
        PSComponentUtils.getChildElement(
            sourceNode, PSActionVisibilityContexts.XML_NODE_NAME, false);
    if (visCtxEl != null) m_visContexts.fromXml(visCtxEl);
    else m_visContexts.clear();

    Element modeCtxEl =
        PSComponentUtils.getChildElement(sourceNode, PSDbComponentCollection.XML_NODE_NAME, false);
    if (modeCtxEl != null) m_modeUiContexts.fromXml(modeCtxEl);
    else {
  ...
  public void setName(String name) {
    ...
    if (!name.equals(m_name)) setDirty();
  ...
  public void setLabel(String label) {
    ...
    if (!label.equals(m_label)) setDirty();
  ...
  public void setDescription(String description) {
    ...
    if (!description.equals(m_description)) setDirty();
  ...
  public void setURL(String url) {
    ...
    if (!url.equals(m_actionURL)) setDirty();
    m_actionURL = url;
  }
  ...
  private boolean validateType(String type) {
    ...
    for (int i = 0; i < ms_menuTypes.size(); i++) {
      if (type.equals(ms_menuTypes.get(i))) return true;
    }
  ...
  public void setMenuType(String type) {
    ...
    if (!type.equals(m_type)) setDirty();
  ...
  protected Element toXml(Document doc, boolean includeChildComps) {
    ...
    if (includeChildComps) {
      if (!m_props.isEmpty()) root.appendChild(m_props.toXml(doc));

      if (!m_params.isEmpty()) root.appendChild(m_params.toXml(doc));

      if (!m_visContexts.isEmpty()) root.appendChild(m_visContexts.toXml(doc));

      if (!m_modeUiContexts.isEmpty()) root.appendChild(m_modeUiContexts.toXml(doc));

      if (!m_children.isEmpty()) root.appendChild(m_children.toXml(doc));
    }
  ...
  public void toDbXml(Document doc, Element root, IPSKeyGenerator keyGen, PSKey parent)
    ...
    if (m_children.getState() != DBSTATE_UNMODIFIED) m_children.toDbXml(doc, root, keyGen, key);
    if (m_params.getState() != DBSTATE_UNMODIFIED) m_params.toDbXml(doc, root, keyGen, key);
    if (m_props.getState() != DBSTATE_UNMODIFIED) m_props.toDbXml(doc, root, keyGen, key);
  ...
  public Object clone() {
    ...
    copy.m_modeUiContexts = (PSDbComponentCollection) m_modeUiContexts.clone();
  ...
  public boolean equals(Object obj) {
    ...
    boolean equals = true;
    if (!(obj instanceof PSAction)) equals = false;
    else {
      ...
      else if (!m_description.equals(other.m_description)) equals = false;
      else if (!m_type.equals(other.m_type)) equals = false;
      else if (!m_actionURL.equals(other.m_actionURL)) equals = false;
      else if (!m_handler.equals(other.m_handler)) equals = false;
      else if (m_sortrank != other.m_sortrank) equals = false;
      else if (!m_props.equals(other.m_props)) equals = false;
      else if (!m_params.equals(other.m_params)) equals = false;
      else if (!m_visContexts.equals(other.m_visContexts)) equals = false;
      else if (!m_children.equals(other.m_children)) equals = false;
      else if (!m_modeUiContexts.equals(other.m_modeUiContexts)) equals = false;
      else if (!m_version.equals(other.m_version)) equals = false;
    }
  ...
  public int getState() {
    ...
    if (state != DBSTATE_UNMODIFIED) return state;
    ...
    for (int i = 0; i < states.length; i++) {
      if (states[i] != DBSTATE_UNMODIFIED) return DBSTATE_MODIFIED;
    }
  ...
  public void setMenuDynamic(boolean flag) {
    ...
    if (!flag && !StringUtils.isBlank(getURL())) setURL("");
  }
  ...
  public static void main(String[] args) {
    try {
      ...
      PSActionVisibilityContext vis = new PSActionVisibilityContext("visctxname1", "visctxval1");
