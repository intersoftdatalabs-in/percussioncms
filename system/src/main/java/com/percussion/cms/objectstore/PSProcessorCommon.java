...
public abstract class PSProcessorCommon implements IPSComponentProcessor, IPSKeyGenerator {
  ...
  private Collection<PSDbComponentCollection> groupComponents(IPSDbComponent[] comps) {
    try {
      ...
      from a Class object */
      throw new RuntimeException("Unexpected ClassNotFoundException: " + cnf.getLocalizedMessage());
    }
  }
  ...
  private Map<String, String[]> prepareForModify(String componentType, PSKey[] locators) {
    ...
    if (null != locators) {
      for (int k = 0; k < locators.length; k++) {
        if (null == locators[k]) throw new IllegalArgumentException("Null keys not allowed.");
      }

      // verify all keys are same type
      for (int k = 1; k < locators.length; k++) {
        if (!locators[0].isSameType(locators[k])) {
          throw new IllegalArgumentException("All keys must be same type.");
        }
      }
    }
  ...
  public int delete(IPSDbComponent[] comps) throws PSCmsException {
    for (int i = 0; i < comps.length; i++) {
      if (null != comps[i]) comps[i].markForDeletion();
    }
    ...
    if (null != cachedIds) {
      for (int i = 0; i < cachedIds.length; i++) {
        if (cachedIds[i] != Integer.MIN_VALUE) {
          id = cachedIds[i];
          if (i == cachedIds.length - 1) m_cachedIds.remove(lookup);
          else cachedIds[i] = Integer.MIN_VALUE;
        }
      }
    } else {
  ...
  private void updateDbComponentVersions(PSDbComponentCollection comps) {
    ...
    while (iter.hasNext()) {
      ...
      if (component instanceof PSDbComponentCollection
        ...
        if (compIter != null) updateDbComponentVersions(compIter);
      } else if (component instanceof PSVersionableDbComponent) {
        PSVersionableDbComponent dbComponent = (PSVersionableDbComponent) component;
