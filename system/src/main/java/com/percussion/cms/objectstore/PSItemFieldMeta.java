...
public class PSItemFieldMeta extends PSItemComponent {
  ...
  public int getBackendDataType() {
    ...
    if (m_fieldDef.getDataType().length() == 0) datatype = DATATYPE_TEXT;

    if (m_fieldDef.getDataType().equals(PSField.DT_BINARY)) datatype = DATATYPE_BINARY;

    if (m_fieldDef.getDataType().equals(PSField.DT_TEXT)) datatype = DATATYPE_TEXT;

    if (m_fieldDef.getDataType().equals(PSField.DT_DATE)) datatype = DATATYPE_DATE;

    if (m_fieldDef.getDataType().equals(PSField.DT_TIME)) datatype = DATATYPE_DATE;

    if (m_fieldDef.getDataType().equals(PSField.DT_DATETIME)) datatype = DATATYPE_DATE;

    if (m_fieldDef.getDataType().equals(PSField.DT_INTEGER)) datatype = DATATYPE_NUMERIC;

    if (m_fieldDef.getDataType().equals(PSField.DT_BOOLEAN)) datatype = DATATYPE_TEXT;

    if (m_fieldDef.getDataType().equals(PSField.DT_FLOAT)) datatype = DATATYPE_NUMERIC;
  ...
  protected Element toXml(Document doc, PSAcceptElements acceptElements) {
    ...
    if (getFieldValueType() != FIELD_VALUE_TYPE_UNKNOWN)
      root.setAttribute(ATTR_FIELD_VALUE_TYPE, PSField.FIELD_VALUE_TYPE_ENUM[getFieldValueType()]);

    if (m_mimeType != null) root.setAttribute(ATTR_MIME_TYPE, m_mimeType);

    if (getSourceType() != SOURCE_TYPE_UNKNOWN)
      root.setAttribute(ATTR_SOURCE_TYPE, PSField.TYPE_ENUM[getSourceType()]);

    root.setAttribute(ATTR_TRANSFER_ENCODING, ENCODING_TYPE_ENUM[m_transferEncoding]);

    if (getDisplayName() != null) root.setAttribute(ATTR_DISPLAY_NAME, getDisplayName());

    root.setAttribute(ATTR_SHOW_IN_PREVIEW, (showInPreview() == true ? "true" : "false"));

    if (!m_AllowedValuesOptionMap.isEmpty()) root.appendChild(createOptionElements(doc));
  ...
  public boolean equals(Object obj) {
    ...
    // checking references:
    if (!compare(m_AllowedValuesOptionMap, compMeta.m_AllowedValuesOptionMap)) return false;
    if (!compare(m_fieldDef, compMeta.m_fieldDef)) return false;
    if (!compare(m_uiDef, compMeta.m_uiDef)) return false;

    // check values:
    if (!compare(getDisplayName(), compMeta.getDisplayName())) return false;
    if (!compare(m_mimeType, compMeta.m_mimeType)) return false;
    if (!compare(getName(), compMeta.getName())) return false;

    // check primitives:
    if (getBackendDataType() != compMeta.getBackendDataType()) return false;
    if (hasOptions() != compMeta.hasOptions()) return false;
    if (m_isMultiValueField != compMeta.m_isMultiValueField) return false;
    if (showInPreview() != compMeta.showInPreview()) return false;
    if (getFieldValueType() != compMeta.getFieldValueType()) return false;
    if (getSourceType() != compMeta.getSourceType()) return false;
    if (m_transferEncoding != compMeta.m_transferEncoding) return false;
  ...
  public void setTransferEncoding(int encodingType) {
    if (encodingType != ENCODING_TYPE_BASE64 || encodingType != ENCODING_TYPE_NONE)
  ...
  public String getDisplayName() {
    ...
    if (m_uiDef.getLabel() != null && m_uiDef.getLabel().getText().trim().length() != 0)
  ...
  /** field value types */
  public static final int FIELD_VALUE_TYPE_UNKNOWN = PSField.FIELD_VALUE_TYPE_UNKNOWN;

  public static final int FIELD_VALUE_TYPE_CONTENT = PSField.FIELD_VALUE_TYPE_CONTENT;

  public static final int FIELD_VALUE_TYPE_META = PSField.FIELD_VALUE_TYPE_META;
