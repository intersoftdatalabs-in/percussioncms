...
public final class PSItemRelatedItem extends PSItemComponent {
  ...
  private boolean equalXmlElements(Element a, Element b) {
    ...
    if ((a != null) && (b == null)) return false;
    if ((a != null) && (b != null)) {
      if (!PSXmlDocumentBuilder.toString(a).equals(PSXmlDocumentBuilder.toString(b))) return false;
    }
  ...
  void loadXmlData(Element sourceNode, boolean clearValues) throws PSUnknownNodeTypeException {
    ...
    setRelatedType(PSXMLDomUtil.checkAttribute(sourceNode, ATTR_RELATED_TYPE, true));
    setRelationshipId(PSXMLDomUtil.checkAttributeInt(sourceNode, ATTR_ID, false));
    setAction(PSXMLDomUtil.checkAttribute(sourceNode, ATTR_ACTION, false));

    // get the item to be related
    Element el = PSXMLDomUtil.getFirstElementChild(sourceNode, PSServerItem.EL_ITEM);
    Element contentKey = PSXMLDomUtil.getFirstElementChild(el, PSServerItem.EL_CONTENT_KEY);
    int contentId = PSXMLDomUtil.checkAttributeInt(contentKey, PSServerItem.ATTR_CONTENT_ID, false);

    setDependentId(contentId);
    setRelatedItemData(el);

    el = PSXMLDomUtil.getNextElementSibling(el);
    while (el != null && PSXMLDomUtil.getUnqualifiedNodeName(el).equals(EL_PROPERTY)) {
    ...
    if (el != null && PSXMLDomUtil.getUnqualifiedNodeName(el).equals(EL_KEY_FIELDS)) {
      el = PSXMLDomUtil.getFirstElementChild(el);
      while (el != null && PSXMLDomUtil.getUnqualifiedNodeName(el).equals(EL_SEARCH_FIELD)) {
        String name = PSXMLDomUtil.checkAttribute(el, ATTR_NAME, true);
