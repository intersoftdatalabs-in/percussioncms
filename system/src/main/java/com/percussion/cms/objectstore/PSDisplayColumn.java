...
public class PSDisplayColumn extends PSDbComponent implements IPSSequencedComponent {
  ...
  public PSDisplayColumn(Element src) throws PSUnknownNodeTypeException {
    this();
    fromXml(src);
  }
   ...
   *
   * @param sourceId the source id, may be <code>null</code> or empty if creating an empty key.
   * @param displayId the (parent) display id.
   * @param persisted <code>true</code> if the object is persisted in the repository.
   */
  public static PSKey createKey(String sourceId, long displayId, boolean persisted) {
    PSKey key = null;
  ...
  public PSDisplayColumn(
    ...
    if (strSource.length() > SOURCE_LENGTH)
      throw new IllegalArgumentException("source must not exceed " + SOURCE_LENGTH + " characters");

    // Handle defaults
    if (strLabel == null || strLabel.trim().length() == 0) strLabel = strSource;
  ...
  public void fromXml(Element e) throws PSUnknownNodeTypeException {
    ...
    if (label.length() == 0) label = m_strSource;
    ...
    for (; groupingType < GROUPING_ENUM.length; groupingType++) {
      if (GROUPING_ENUM[groupingType].equalsIgnoreCase(grouping)) break;
    }
    if (groupingType == GROUPING_ENUM.length) groupingType = GROUPING_FLAT;
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(
        SORTORDER_ATTR, isAscendingSort() ? SORTORDER_VALUE_ASC : SORTORDER_VALUE_DESC);
    ...
    PSXmlDocumentBuilder.addElement(doc, root, XML_NODE_RENDERTYPE, getRenderType());

    return root;
  }

  /**
   * This value allows the UI engine to determine how the associated data should be displayed.
   *
  ...
  public void setRenderType(String str) {
    ...
    if (str == null || str.trim().length() == 0) str = DATATYPE_TEXT;

    // Validate length
    if (str.length() > RENDERTYPE_LENGTH)
      throw new IllegalArgumentException(
          "render type must not exceed " + RENDERTYPE_LENGTH + " characters");

    // Threshold
    if (m_strRenderType.equalsIgnoreCase(str)) return;

    String[] container =
        new String[] {DATATYPE_TEXT, DATATYPE_NUMBER, DATATYPE_DATE, DATATYPE_IMAGE};
    ...
    if (!bFound)
      throw new IllegalArgumentException(str + " is not a valid render type for a PSDisplayColumn");
  ...
  public void setDisplayName(String str) {
    ...
    if (str.length() > DISPLAYNAME_LENGTH)
      throw new IllegalArgumentException(
          "display name must not exceed " + DISPLAYNAME_LENGTH + " characters");

    // Threshold
    if (str.equalsIgnoreCase(m_strDisplayName)) return;
  ...
  public void setDescription(String str) {
    ...
    if (str.length() > DESCRIPTION_LENGTH)
      throw new IllegalArgumentException(
          "description must not exceed " + DESCRIPTION_LENGTH + " characters");

    // Threshold
    if (str.equalsIgnoreCase(m_strDescription)) return;
  ...
  /**
   * A column can either be 'flat' or categorized. A categorized column allows rows w/ the same
   * value for this property to be grouped together. This is usually represented by a 'virtual'
   * folder in a UI. Non-categorized columns are used in the list view of the UI.
   *
   * @return <code>true</code> if this column has been defined to be displayed as a category, <code>
   *     false</code> if it is a list header.
   */
  ...
  /**
   * The position of this column relative to other columns being displayed. Columns are sequenced
   * from left to right, with the first index being 0. Defaults to 0. The order of columns that have
   * the same sequence value is implmenentation dependent.
   *
   * @return A value >= 0.
   */
  ...
  public void setPosition(int pos) {
    if (pos < 0) pos = 0;
    if (m_headerSequence == pos) return;
   ...
   * ordering should be. Defaults to <code>true</code>.
   *
   * @return <code>true</code> if ascending, otherwise <code>false</code>
   */
   ...
   *
   * @return <code>true</code> if descending, otherwise <code>false</code>
   */
  ...
  /**
   * See {@link #isCategorized()} for details. Determines whether this col is categorized or flat.
   *
  ...
  public void setGroupingType(int groupingType) {
    ...
    // Threshold
    if (m_groupingType == groupingType) return;
  ...
  public void setSortOrder(boolean isAscending) {
    // Threshold
    if (m_isAscendingSort == isAscending) return;
   ...
   *
   * @return The width, greater than zero if specified, -1 if no width has been specified.
   */
  public int getWidth() {
    return m_width;
  }
   ...
   *
   * @param width The width to use, must be greater than zero, or -1 to clear the width.
   */
  public void setWidth(int width) {
    if (width <= 0 && width != -1) throw new IllegalArgumentException("invalid column width");
  ...
  public void copyFrom(IPSDbComponent src) {
    ...
    if (null == src || !getClass().isInstance(src))
      throw new IllegalArgumentException("src must be a " + getClass().getName());
  ...
  public boolean equals(Object obj) {
    ...
    return m_strDescription.equalsIgnoreCase(col2.m_strDescription)
        ...
        && (getPosition() == col2.getPosition() && (m_width == col2.m_width));
  }
  ...
  /**
   * Names for each of the GROUPING_xxx numeric types. The type is used as an index into this array
   * to find its name.
   */
  public static final String[] GROUPING_ENUM = {GROUPING_FLAT_NAME, GROUPING_CATEGORY_NAME};

  public static final String XML_NODE_DESCRIPTION = "Description";
  public static final String SEQUENCE_ATTR = "sequence";
  private static final String WIDTH_ATTR = "width";

  /** Description of column, initialized in definition, never <code>null</code> but may be empty. */
  private String m_strDescription = "";

  /** See {@link #getPosition()} for details. Always >= 0. */
  private int m_headerSequence = 0;

  /** Display name of column, initialized in definition, never <code>null</code> and never empty. */
  ...
  /**
   * Source name, initialized in ctor, not modified after that, part of the primary key. Never
   * <code>null</code> or empty
   */
  private String m_strSource;

  /**
   * See {@link #isCategorized()} for details. One of the GROUPING_xxx values. Defaults to
   * GROUPING_FLAT.
   */
  private int m_groupingType = GROUPING_FLAT;

  /**
   * Width the ui should use to display this column. See {@link #getWidth()} for details. Initially
   * -1, modified by {@link #setWidth(int)}.
   */
  private int m_width = -1;

  // NOTE: All render types must be less than RENDERTYPE_LENGTH in char count
  /** A hint to the UI that the data associated with this column is a plain text string. */
  public static final String DATATYPE_TEXT = PSSearchField.TYPE_TEXT;

  /**
   * A hint to the UI that the data associated with this column is numeric and may need to be
   * rendered differently depending on the locale.
   */
  public static final String DATATYPE_NUMBER = PSSearchField.TYPE_NUMBER;

  /**
   * A hint to the UI that the data associated with this column is a date, and may need to be
   * rendered differently depending on the locale. The date is always returned from this object in
   * ISO8601 format, i.e., yyyyMMdd HH:mm:ss, where HH is in 24 hour time. The time part is only
   * returned if it is not all 0's. See SimpleDataFormat for more details on format pattern.
   */
  public static final String DATATYPE_DATE = PSSearchField.TYPE_DATE;

  /**
   * When this data type is specified, the engine expects to find a url as the value. It will query
   * the data from the url and display as an image within the column. Only small images should be
   * supplied for this data type.
   */
