...
public class PSSlotType extends PSDbComponent {
  ...
  public PSSlotType(IPSTemplateSlot slot) {
    super(
        new PSKey(new String[] {KEY_SLOTID}, new int[] {(int) slot.getGUID().longValue()}, false));

    if (slot.getSlottypeEnum() == IPSTemplateSlot.SlotType.INLINE) m_slotType = 1;
    else m_slotType = 0;
    ...
    Collection<PSPair<IPSGuid, IPSGuid>> associations = slot.getSlotAssociations();
    ...
    for (PSPair<IPSGuid, IPSGuid> pair : associations) {
      ...
      PSSlotTypeContentTypeVariant sv =
          PSSlotTypeContentTypeVariant.create(slotId, contentTypeId, templateId);
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(XML_ATTR_allowedRelationshipName, "" + m_allowedRelationshipTypeName);

    PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_SlotName, m_slotName);

    PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_SlotDesc, m_slotDesc);

    if (m_slotVariants != null) m_slotVariants.toXml(doc);
  ...
  private void loadFieldsFromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_allowedRelationshipTypeName =
        PSXMLDomUtil.checkAttribute(sourceNode, XML_ATTR_allowedRelationshipName, false);
    if (m_allowedRelationshipTypeName.trim().length() < 1)
      m_allowedRelationshipTypeName = PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY;

    PSXmlTreeWalker walker = new PSXmlTreeWalker(sourceNode);

    Element el = walker.getNextElement(XML_ELEM_SlotName, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_slotName = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_SlotDesc, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_slotDesc = PSXmlTreeWalker.getElementData(el);
    ...
    el =
        walker.getNextElement(
            PSSlotTypeContentTypeVariantSet.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);

    if (m_slotVariants == null) m_slotVariants = new PSSlotTypeContentTypeVariantSet();
    if (el != null) m_slotVariants.fromXml(el);
  }
  ...
  public static PSKey[] createKeys(int[] slotIds) {
    ...
    for (int i = 0; i < slotIds.length; i++) {
      if (slotIds[i] < 1) throw new IllegalArgumentException("slotIds may not be < 0");
  ...
  public boolean equals(Object o) {
    ...
    if (!m_allowedRelationshipTypeName.equalsIgnoreCase(type.m_allowedRelationshipTypeName))
      return false;
    if (!m_slotDesc.equalsIgnoreCase(type.m_slotDesc)) return false;
    if (!m_slotName.equalsIgnoreCase(type.m_slotName)) return false;
    if (m_slotType != type.m_slotType) return false;
    if (m_systemSlot != type.m_systemSlot) return false;
    if (!m_slotVariants.equals(type.m_slotVariants)) return false;
  ...
  private static final String XML_ATTR_systemSlot = "systemSlot";
  private static final String XML_ATTR_slotType = "slotType";
  private static final String XML_ATTR_allowedRelationshipName = "allowedRelationship";

  private static final String XML_ELEM_SlotName = "SlotName";
  private static final String XML_ELEM_SlotDesc = "SlotDesc";
}
