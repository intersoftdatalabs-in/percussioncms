...
public class PSChildActions extends PSDbComponentCollection {
  ...
  public void add(PSMenuChild child) {
    if (null == child) {
      throw new IllegalArgumentException("Supplied action reference can't be null.");
    }
    PSMenuChild existing = getChild(child);
    if (null == existing) super.add(child);
  }
  ...
  public void add(IPSDbComponent comp) {
    if (null == comp || !((comp instanceof PSAction) || comp instanceof PSMenuChild)) {
      throw new IllegalArgumentException("Can only add PSAction or PSMenuChild components");
    }

    if (comp instanceof PSAction) add((PSAction) comp);
    if (comp instanceof PSMenuChild) add((PSMenuChild) comp);
  }
  ...
  private PSMenuChild getChild(PSMenuChild child) {
    ...
    while (it.hasNext()) {
      ...
      if (child.equals(tmp)) return tmp;
    }
    return null;
  }
