...
public final class PSCloningOptions extends PSComponent {
  ...
  public PSCloningOptions(
    ...
    if (folderName.length() == 0) throw new IllegalArgumentException("folderName cannot be empty");
    m_folderName = folderName;

    if (!isValid(copyOption, ms_copyOptionsEnum))
      throw new IllegalArgumentException("copyOption must be one of COPY_XXX");
    m_copyOption = copyOption;

    if (!isValid(copyContentOption, ms_copyContentOptionsEnum))
      throw new IllegalArgumentException("copyContentOption must be one of COPYCONTENT_XXX");
    m_copyContentOption = copyContentOption;

    m_communityMappings.clear();
    if (communityMappings != null) m_communityMappings.putAll(communityMappings);
  }
  ...
  public int hashCode() {
    int hash =
        m_type + m_siteToCopy == null
            ? 0
            : m_siteToCopy.hashCode() + m_siteName == null
                ? 0
                : m_siteName.hashCode()
  ...
  public boolean equals(Object o) {
    ...
    if (t.m_type != m_type) return false;
    if (!compare(t.m_siteToCopy, m_siteToCopy)) return false;
    if (!compare(t.m_siteName, m_siteName)) return false;
    if (!compare(t.m_folderName, m_folderName)) return false;
    if (t.m_copyOption != m_copyOption) return false;
    if (t.m_copyContentOption != m_copyContentOption) return false;
    if (!t.m_communityMappings.equals(m_communityMappings)) return false;
    if (!t.m_siteMappings.equals(m_siteMappings)) return false;
  ...
  public void setSiteToCopy(String siteToCopy) {
    ...
    if (siteToCopy != null && siteToCopy.trim().length() > 0) m_siteToCopy = siteToCopy.trim();
  }
  ...
  public void setSiteName(String siteName) {
    ...
    if (siteName != null && siteName.trim().length() > 0) m_siteName = siteName.trim();
  }
  ...
  public void fromXml(Element source, IPSDocument parent, List parentComponents)
    ...
    m_copyOption =
        toInteger(source.getAttribute(COPY_OPTION_ATTR), ms_copyOptionNames, COPY_OPTION_ATTR);

    m_copyContentOption =
        toInteger(
            source.getAttribute(COPYCONTENT_OPTION_ATTR),
            ms_copyContentOptionNames,
            COPYCONTENT_OPTION_ATTR);

    PSXmlTreeWalker walker = new PSXmlTreeWalker(source);

    walker.setCurrent(source);
    m_communityMappings.clear();
    Element communityMappings = walker.getNextElement(COMMUNITY_MAPPINGS_ELEM, true);
  ...
  public Element toXml(Document doc) {
    ...
    if (m_siteToCopy != null) root.setAttribute(SITE_TO_COPY_ATTR, m_siteToCopy);
    if (m_siteName != null) root.setAttribute(SITE_NAME_ATTR, m_siteName);
    root.setAttribute(FOLDER_NAME_ATTR, m_folderName);
    root.setAttribute(COPY_OPTION_ATTR, ms_copyOptionNames[m_copyOption]);
    root.setAttribute(COPYCONTENT_OPTION_ATTR, ms_copyContentOptionNames[m_copyContentOption]);
  ...
  private boolean isValid(int value, int[] validValues) {
    ...
    for (int i = 0; i < validValues.length && !isValid; i++) isValid = validValues[i] == value;
