...
public class PSCoreItem extends PSItemComponent implements IPSItemAccessor {
  ...
  public void accept(IPSVisitor visitor) {
    ...
    if (visitor.getObject() instanceof PSItemField) addField((PSItemField) visitor.getObject());
  ...
  public Element toMinXml(
    ...
    if (m_currentrevision > 0) root.setAttribute(ATTR_CURRENT_REVISION, "" + m_currentrevision);

    if (m_editrevision > 0) root.setAttribute(ATTR_EDIT_REVISION, "" + m_editrevision);

    if (m_editrevision > 0) root.setAttribute(ATTR_REVISION_COUNT, "" + m_editrevision);

    // not counting on JRE to create attribute value, done explicitely:
    root.setAttribute(ATTR_REVISION_LOCK, (m_revisionLock == true ? "true" : "false"));

    if (m_systemLocale != null) root.setAttribute(ATTR_SYSTEM_LOCALE, "" + m_systemLocale);

    if (m_dataLocale != null) root.setAttribute(ATTR_DATA_LOCALE, "" + m_dataLocale);

    // just check for null, if empty, then set empty:
    if (m_checkedOutBy.trim().length() != 0) root.setAttribute(ATTR_CHECKED_OUT_BY, m_checkedOutBy);

    Element e = createStandardItemElement(doc, EL_CONTENT_KEY);

    if (m_contentId > 0) e.setAttribute(ATTR_CONTENT_ID, "" + m_contentId);

    if (m_revision > 0) e.setAttribute(ATTR_REVISION, "" + m_revision);

    root.appendChild(e);

    // build accept elements rules:
    PSAcceptElements acceptElements =
        new PSAcceptElements(includeFields, includeChilds, includeRelated, includeBinary);

    // fields:
    if (includeFields && !m_fieldNameFieldMap.isEmpty())
      toXmlCollection(root, doc, m_fieldNameFieldMap.values(), acceptElements);

    // children:
    if (includeChilds && !m_childNameChildMap.isEmpty())
      toXmlCollection(root, doc, m_childNameChildMap.values(), acceptElements);

    // related items:
    if (includeRelated && !m_relatedItemsMap.isEmpty()) {
      Element relatedItems = createStandardItemElement(doc, EL_RELATED_ITEMS);
      root.appendChild(relatedItems);
      toXmlCollection(relatedItems, doc, m_relatedItemsMap.values(), acceptElements);
    }
  ...
  public Object clone() {
    ...
    if (m_dataLocale != null) copy.m_dataLocale = (Locale) m_dataLocale.clone();

    if (m_systemLocale != null) copy.m_systemLocale = (Locale) m_systemLocale.clone();
  ...
  public boolean equals(Object obj) {
    ...
    if (!compare(m_itemDefinition, comp.m_itemDefinition)) return false;
    if (!compare(m_checkedOutBy, comp.m_checkedOutBy)) return false;
    if (!compare(m_childNameChildMap, comp.m_childNameChildMap)) return false;
    if (m_contentId != comp.m_contentId) return false;
    if (getContentTypeId() != comp.getContentTypeId()) return false;
    if (m_currentrevision != comp.m_currentrevision) return false;
    if (!compare(m_dataLocale, comp.m_dataLocale)) return false;
    if (!compare(m_dataVariant, comp.m_dataVariant)) return false;
    if (m_editrevision != comp.m_editrevision) return false;
    if (!compare(m_fieldNameFieldMap, comp.m_fieldNameFieldMap)) return false;
    if (!compare(m_relatedItemsMap, comp.m_relatedItemsMap)) return false;
    if (m_requestedrevision != comp.m_requestedrevision) return false;
    if (m_revision != comp.m_revision) return false;
    if (m_revisionLock != comp.m_revisionLock) return false;
    if (!compare(m_systemLocale, comp.m_systemLocale)) return false;
  ...
  void loadXmlData(Element sourceNode, boolean clearValues) throws PSUnknownNodeTypeException {
    ...
    int currentRevision = PSXMLDomUtil.checkAttributeInt(sourceNode, ATTR_CURRENT_REVISION, false);
    if (currentRevision > 0) setCurrentRevision(currentRevision);

    int editRevision = PSXMLDomUtil.checkAttributeInt(sourceNode, ATTR_EDIT_REVISION, false);
    if (editRevision > 0) setEditRevision(editRevision);

    int requestedRevision =
        PSXMLDomUtil.checkAttributeInt(sourceNode, ATTR_REQUESTED_REVISION, false);
    if (requestedRevision > 0) setRequestedRevision(requestedRevision);

    String systemLocale = PSXMLDomUtil.checkAttribute(sourceNode, ATTR_SYSTEM_LOCALE, false);
    if (!StringUtils.isBlank(systemLocale)) setSystemLocale(new Locale(systemLocale));

    String dataLocale = PSXMLDomUtil.checkAttribute(sourceNode, ATTR_DATA_LOCALE, false);
    if (!StringUtils.isBlank(dataLocale)) setDataLocale(new Locale(dataLocale));

    String checkedOutBy = PSXMLDomUtil.checkAttribute(sourceNode, ATTR_CHECKED_OUT_BY, false);
    if (!StringUtils.isBlank(checkedOutBy)) setCheckedOutByName(checkedOutBy);

    // content key element (required)
    Element el = PSXMLDomUtil.getFirstElementChild(sourceNode, EL_CONTENT_KEY);
    ...
    int contentId = PSXMLDomUtil.checkAttributeInt(el, ATTR_CONTENT_ID, false);
    if (contentId > 0) setContentId(contentId);

    int revision = PSXMLDomUtil.checkAttributeInt(el, ATTR_REVISION, false);
    if (revision > 0) setRevision(revision);
    ...
    while (el != null) {
      ...
      if (name.equals(PSItemField.EL_FIELD)) {
        ...
        Element relEl = PSXMLDomUtil.getFirstElementChild(el, EL_RELATED_ITEM);
  ...
  public void loadData(PSCoreItem item) {
    ...
    while (fieldIter.hasNext()) {
      ...
      if (field == null) {
        // should not happen
        throw new RuntimeException("Field '" + itemFieldName + "' could not be found.");
      }
      field.clearValues();
    ...
    while (childIter.hasNext()) {
      ...
      if (child == null) {
        // should not happen
        throw new RuntimeException("Child '" + itemChildName + "' could not be found.");
      }
  ...
  public Iterator<PSItemChild> getAllChildren() {
    ...
    return Collections.unmodifiableCollection(m_childNameChildMap.values()).iterator();
  }
  ...
  public Iterator<PSItemRelatedItem> getAllRelatedItems() {
    ...
    return Collections.unmodifiableCollection(m_relatedItemsMap.values()).iterator();
  }
  ...
  private void resetLocator() {
    if (m_contentId == INVALID_CONTENTID) {
      ...
      if (m_revision == INVALID_REVISION) setLocator(new PSLocator(m_contentId));
      else setLocator(new PSLocator(m_contentId, m_revision));
    }
  }
  ...
  public void setDataLocale(Locale dataLocale) {
    if (dataLocale == null) throw new IllegalArgumentException("dataLocale cannot be null");
