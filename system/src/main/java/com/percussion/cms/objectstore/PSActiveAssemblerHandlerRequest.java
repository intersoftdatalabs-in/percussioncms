...
public final class PSActiveAssemblerHandlerRequest extends PSCmsComponent {
  ...
  public boolean equals(Object o) {
    ...
    PSActiveAssemblerHandlerRequest request = (PSActiveAssemblerHandlerRequest) o;

    if (!m_owner.equals(request.m_owner)) return false;
    if (!m_dependents.equals(request.m_dependents)) return false;
    if (m_index != request.m_index) return false;
  ...
  public Element toXml(Document doc) {
    if (doc == null) throw new IllegalArgumentException("doc must be supplied");
  ...
  public void fromXml(Element source, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    setOwner(new PSLocator(walker.getNextElement(PSLocator.XML_NODE_NAME)));
    setDependents(
        new PSDependentSet(walker.getNextElement(PSDependentSet.getNodeName()), null, null));
  }
   ...
   */
  public int hashCode() {
    return m_owner.hashCode() + m_dependents.hashCode() + Integer.toString(m_index).hashCode();
  }
