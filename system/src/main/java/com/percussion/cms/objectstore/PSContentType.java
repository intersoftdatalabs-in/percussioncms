...
public class PSContentType extends PSDbComponent {
  ...
  public PSContentType(
    ...
    if (null == editorUrl || editorUrl.trim().length() == 0) {
      throw new IllegalArgumentException("non-empty editorUrl must be supplied");
    } else {
      // verify url format matches what we say below
      if (!verifyUrlFormat(editorUrl)) {
        throw new IllegalArgumentException(
            "incorrect editorUrl format, expected ../appname/resource, got " + editorUrl);
      }
    }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_newRequest != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_NewRequest, m_newRequest);

    if (m_queryRequest != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_QueryRequest, m_queryRequest);

    if (m_updateRequest != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_UpdateRequest, m_updateRequest);
  ...
  private void loadFieldsFromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_label =
        sourceNode.hasAttribute(XML_ATTR_label)
            ? PSXMLDomUtil.checkAttribute(sourceNode, XML_ATTR_label, false)
            : m_name; // if no label attribute then label will be the
    // same as the name.

    m_hideFromMenu = PSXMLDomUtil.checkAttributeInt(sourceNode, XML_ATTR_hideFromMenu, true) == 1;

    m_objectType = PSXMLDomUtil.checkAttributeInt(sourceNode, XML_ATTR_objectType, true);

    PSXmlTreeWalker walker = new PSXmlTreeWalker(sourceNode);

    Element el =
        walker.getNextElement(XML_ELEM_NewRequest, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_newRequest = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_QueryRequest, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    ...
    el = walker.getNextElement(XML_ELEM_UpdateRequest, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_updateRequest = PSXmlTreeWalker.getElementData(el);
    ...
    el =
        walker.getNextElement(
            PSContentTypeVariantSet.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSContentType)) return false;

    PSContentType type = (PSContentType) o;

    if (!m_queryRequest.equals(type.m_queryRequest)) return false;
    if (!m_description.equals(type.m_description)) return false;
    if (!m_name.equalsIgnoreCase(type.m_name)) return false;
  ...
  @Override
  public int hashCode() {
    return (m_description + m_queryRequest + m_hideFromMenu + getTypeId() + m_name.toLowerCase())
        .hashCode();
  }
  ...
  private boolean verifyUrlFormat(String editorUrl) {
    ...
    if (!normalized.startsWith(CE_URL_PREFIX)) return false;
  ...
  public static String getAppName(String requestUrl) {
    String appName = StringUtils.substringAfter(requestUrl.replace('\\', '/'), CE_URL_PREFIX);
    appName = StringUtils.substringBefore(appName, "/");

    if (StringUtils.isBlank(appName))
      throw new IllegalArgumentException("invalid content editor url: " + requestUrl);
  ...
  public static String getAppNameFromRequestUrl(String url) {
    ...
    if (secondSlash > firstSlash) return url.substring(firstSlash + 1, secondSlash);
    return null;
  }
   ...
   */
  private static final String XML_ELEM_NewRequest = "NewRequest";
  private static final String XML_ELEM_QueryRequest = "QueryRequest";
  private static final String XML_ELEM_UpdateRequest = "UpdateRequest";
  private static final String XML_ELEM_Description = "Description";
  private static final String XML_ATTR_hideFromMenu = "hideFromMenu";
  private static final String XML_ATTR_objectType = "objectType";
