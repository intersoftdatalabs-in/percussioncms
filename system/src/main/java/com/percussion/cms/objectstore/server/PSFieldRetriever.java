...
public class PSFieldRetriever {
  ...
  private String getRequestUrl(PSRequest req, PSLocator itemId, String fieldName, int childRowId)
    ...
    PSSecurityToken tok;
    if (req != null) tok = req.getSecurityToken();
    else tok = PSRequest.getContextForRequest().getSecurityToken();
  ...
  protected boolean isLocatorValid(PSLocator locator) {
    ...
    if (locator.getId() < 1 || locator.getRevision() < 1) isValid = false;
  ...
  public Object clone() {
    ...
    try {
      ...
      throw new RuntimeException(
          "PSFieldRetriever.clone() caught " + "exception: \n" + e.toString());
    }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof PSFieldRetriever)) isEqual = false;
    else {
      PSFieldRetriever other = (PSFieldRetriever) o;
      if (m_contentTypeId != other.m_contentTypeId) isEqual = false;
    }
  ...
  private String getUrlToApp(PSSecurityToken tok)
    ...
    String url = null;
    PSItemDefSummary ids = mgr.getSummary(m_contentTypeId, tok);
  ...
  private byte[] requestData(PSRequest request, String requestUrl, Map<String, Object> params)
    ...
    byte[] data = null;
    try {
      PSMimeContentResult mimeContent = requestMimeContent(request, requestUrl, params);
  ...
  private PSMimeContentResult requestMimeContent(
    ...
    try {
      ...
      IPSInternalResultHandler rh = (IPSInternalResultHandler) ir.getInternalRequestHandler();
