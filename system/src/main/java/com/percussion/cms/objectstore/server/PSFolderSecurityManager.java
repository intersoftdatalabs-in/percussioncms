...
public class PSFolderSecurityManager {
  ...
  public static PSObjectPermissions getPermissions(int contentId) throws PSCmsException {
    ...
    if ((contextVal != null) && (contextVal == thread)) {
      m_permissions = new PSFolderPermissions(PSFolderPermissions.ACCESS_ALL);
      process = false;
    }
    ...
    if (process) {
      ...
      if (req != null) {
        Object requestVal = req.getPrivateObject(CHECK_USER_FOLDER_PERMISSION);
    ...
    if (m_permissions == null)
      m_permissions = new PSFolderPermissions(PSFolderPermissions.ACCESS_DENY);
  ...
  public static void setCheckFolderPermissions(boolean checkPermission) {
    ...
    if (checkPermission) {
      ...
      request.setPrivateObject(CHECK_USER_FOLDER_PERMISSION, Thread.currentThread());
    }
  }
