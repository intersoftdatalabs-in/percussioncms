...
public class PSLoadChildDataExit extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (params.length == 0 || params[0] == null || params[0].toString().trim().length() == 0) {
      throw new PSParameterMismatchException("Base element not specified");
    }

    if (params.length < 2 || params[1] == null || params[1].toString().trim().length() == 0) {
      throw new PSParameterMismatchException("child element not specified");
    }

    if (params.length < 3 || params[2] == null || params[2].toString().trim().length() == 0) {
      throw new PSParameterMismatchException("query attribute not specified");
    }
