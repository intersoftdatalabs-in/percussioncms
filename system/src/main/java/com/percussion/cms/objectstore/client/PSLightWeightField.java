...
public class PSLightWeightField implements Comparable<PSLightWeightField> {
  ...
  public PSLightWeightField(
      String internalName, String dataType, String displayName, String mnemonic) {
    ...
    if (mnemonic.length() > 1) throw new IllegalArgumentException("mnemonic must be <= 1");
    m_mnemonic = mnemonic;
  }
  ...
  private String normalizeDataType(String type) {
    ...
    else if (type.equalsIgnoreCase(PSField.DT_DATETIME)) return PSSearchField.TYPE_DATE;
    else if (type.equalsIgnoreCase(PSField.DT_TEXT)) return PSSearchField.TYPE_TEXT;
    else return type;
  }
