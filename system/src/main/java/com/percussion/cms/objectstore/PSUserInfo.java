...
public final class PSUserInfo implements IPSCmsComponent {
  ...
  public PSUserInfo(IPSRequestContext request) throws PSCmsException {
    m_RoleList.clear();
    try {
      m_UserName = request.getUserContextInformation("User/Name", "unknown").toString();
      ...
      if (obj != null) {
        if (obj instanceof PSCollection) {
          ...
          while (it.hasNext()) m_RoleList.add(it.next().toString());
      ...
      obj = request.getSessionPrivateObject(IPSHtmlParameters.SYS_COMMUNITY);
      if (obj != null) m_CommunityId = Integer.parseInt(obj.toString());
      // get Locale from session objects
      obj = request.getSessionPrivateObject(IPSHtmlParameters.SYS_LANG);
      if (obj != null) m_Locale = obj.toString();

      m_SessionTimeout = PSServer.getServerConfiguration().getUserSessionTimeout();
    } catch (Exception e) {
      throw new PSCmsException(1000, PSExceptionUtils.getMessageForLog(e));
    }
  }
  ...
  public Object clone() {
    ...
    try {
      ...
      for (int i = 0; i < m_RoleList.size(); i++) clone.m_RoleList.add(m_RoleList.get(i));
  ...
  public int hashCode() {
    return Objects.hash(
        m_SessionId, m_UserName, m_CommunityId, m_Locale, m_RoleList, m_SessionTimeout);
  }
  ...
  public void fromXml(Element elemRoot) throws PSUnknownNodeTypeException {
    ...
    while (elChild != null && elChild.getNodeName().equals(XML_ELEM_ROLE)) {
      ...
      if (value != null) m_RoleList.add(value);
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, elem, XML_ELEM_SESSIONID, m_SessionId);
    PSXmlDocumentBuilder.addElement(doc, elem, XML_ELEM_USERNAME, m_UserName);
    PSXmlDocumentBuilder.addElement(doc, elem, XML_ELEM_COMMUNITYID, String.valueOf(m_CommunityId));
    PSXmlDocumentBuilder.addElement(doc, elem, XML_ELEM_LOCALE, m_Locale);

    Element rolesElem = PSXmlDocumentBuilder.addElement(doc, elem, XML_ELEM_ROLES, null);
    for (int i = 0; i < m_RoleList.size(); i++) {
      PSXmlDocumentBuilder.addElement(doc, rolesElem, XML_ELEM_ROLE, m_RoleList.get(i).toString());
    }
    PSXmlDocumentBuilder.addElement(
        doc, elem, XML_ELEM_SESSIONTIMEOUT, String.valueOf(m_SessionTimeout));
    return elem;
  }
  ...
  public static void main(String[] args) {
    try {
      Document doc =
          PSXmlDocumentBuilder.createXmlDocument(new FileReader("c:/PSXUserInfo.xml"), false);
  ...
  public static final String XML_ELEM_LOCALE = "Locale";
  public static final String XML_ELEM_ROLES = "Roles";
  public static final String XML_ELEM_ROLE = "Role";
  public static final String XML_ELEM_SESSIONID = "SessionId";
  public static final String XML_ELEM_SESSIONTIMEOUT = "SessionTimeOut";
}
