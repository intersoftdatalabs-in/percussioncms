...
public class PSItemChildEntry extends PSItemComponent implements IPSItemAccessor {
  ...
  public void setAction(String childAction) {
    if (childAction == null || childAction.length() == 0)
      throw new IllegalArgumentException("childAction must not be null or empty");
    ...
    if (!childAction.equals(CHILD_ACTION_DELETE)
      ...
      throw new IllegalArgumentException("childAction must be a valid action");
  ...
  protected Element toXml(Document doc, PSAcceptElements acceptElements) {
    ...
    if (m_childRowId >= 0) root.setAttribute(ATTR_CHILD_ROW_ID, "" + m_childRowId);
  ...
  public boolean equals(Object obj) {
    ...
    if (m_childRowId != comp.m_childRowId) return false;
    if (!compare(m_action, comp.m_action)) return false;
  ...
  void loadXmlData(Element sourceNode, boolean clearValues) throws PSUnknownNodeTypeException {
    ...
    // set row id if exist
    int rowId = PSXMLDomUtil.checkAttributeInt(sourceNode, ATTR_CHILD_ROW_ID, false);
    ...
    if (rowId != -1 && m_childRowId == -1) m_childRowId = rowId;
    ...
    while (el != null && PSXMLDomUtil.getUnqualifiedNodeName(el).equals(PSItemField.EL_FIELD)) {
      Element fieldMeta = PSXMLDomUtil.getFirstElementChild(el, PSItemFieldMeta.EL_FIELD_META);
  ...
  public static final String CHILD_ACTION_IGNORE = "ignore";

  public static final String CHILD_ACTION_UPDATE = "update";
