...
/**
 * This class wraps a set of results returned by a processor. The object is logically immutable
 * (meaning you can't change it directly, but you could get the objects it manages and change
 * those).
 *
 * @author Paul Howard
 * @version 1.0
...
public final class PSSaveResults implements IPSCmsComponent {
  ...
  public PSSaveResults(IPSDbComponent[] comps, PSProcessingStatistics stats) {
    ...
    if (null == stats) throw new IllegalArgumentException("Statistics object cannot be null");
   ...
   *    </code>.
   */
  public PSSaveResults(Element src) throws PSUnknownNodeTypeException {
   ...
   * </code></pre>
   *
   * <p>There will be a component list for each group of components that are the same. Note that the
   * array from a re-serialized object does not necessarily have the same sequence of components as
   * the original if the compnent types were intermingled.
   */
  public Element toXml(Document doc) {
    ...
    for (int i = 0; i < m_comps.length; i++) {
      PSDbComponentList l = (PSDbComponentList) compGroups.get(m_comps[i].getComponentType());
      if (null == l) {
    ...
    while (it.hasNext()) root.appendChild(((PSDbComponentList) it.next()).toXml(doc));
    return root;
  }
