...
public final class PSProcessingStatistics implements IPSCmsComponent {
   ...
   * @param skips How many elements were not processed (probably because they didn't hava a dbaction
   *     parameter). >= 0.
   * @param errors How many errors occurred. If the transaction level is set to row level, a
   *     non-zero value could be set here. >= 0.
   */
  public PSProcessingStatistics(int inserts, int updates, int deletes, int skips, int errors) {
    if (inserts < 0 || updates < 0 || deletes < 0 || skips < 0 || errors < 0) {
      throw new IllegalArgumentException("Invalid value supplied for statistic, must be >= 0");
    }
  ...
  /**
   * Creates stats from the PSXExecStatistics element returned by the server as the result of an
   * update request.
   *
   * @param doc the XML representation of the object. Never <code>null</code>.
   */
  public PSProcessingStatistics(Document doc) throws PSUnknownNodeTypeException {
    if (null == doc) throw new IllegalArgumentException("Source document cannot be null.");
   ...
   */
  public PSProcessingStatistics(Element src) throws PSUnknownNodeTypeException {
  ...
  /**
   * Convenience method that calls {@link #PSProcessingStatistics(int,int,int, int,int) this(0, 0,
   * deletes, 0, errors)}.
   */
  ...
  /**
   * The number of rows inserted during the processing of this request. A row is considered inserted
   * if a new row is created in the db.
   *
   * @return >= 0
   */
  ...
  /**
   * The number of rows updated during the processing of this request. A row is considered updated
   * if it was already in the db.
   *
   * @return >= 0
   */
  ...
  /**
   * The number of elements skipped during the processing of this request. An element may be skipped
   * if the db action for that element was not set.
   *
   * @return >= 0
   */
  ...
  /**
   * See {@link IPSCmsComponent#toXml(Document) interface} for description. The document generated
   * by this class conforms to the following dtd:
   *
   * <pre><code>
  ...
  public Element toXml(Document doc) {
    if (null == doc) throw new IllegalArgumentException("Document cannot be null.");
   ...
   */
  public void fromXml(Document doc) throws PSUnknownNodeTypeException {
    if (doc == null) throw new IllegalArgumentException("Source document cannot be null.");
  ...
  /** The number of rows inserted into the db by the resource that generated these results. */
  private int m_inserts = 0;

  /** The number of rows updated in the db by the resource that generated these results. */
  private int m_updates = 0;

  /** The number of rows removed from the db by the resource that generated these results. */
  private int m_deletes = 0;

  /**
   * The number of elements skipped while walking the supplied xml document during processing. This
   * can happen if the db action isn't set properly.
   */
  private int m_skips = 0;

  /**
   * The number of errors that occurred. This may be non-zero if the transaction level is set to
   * row-level on the resource.
   */
  ...
  public static final String XML_NODE_NAME =
      PSProcessingStatistics.class
          .getName()
          .substring(PSProcessingStatistics.class.getName().lastIndexOf('.') + 1);
}
