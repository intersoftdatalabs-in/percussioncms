...
public class PSRelationshipEffectProcessor {
  ...
  public PSRelationshipEffectProcessor(
    ...
    m_execData = data;
    m_executionContext = new PSExecutionContext(executionContext, data, m_relationshipsProcessed);
   ...
   */
  public void process()
      throws PSNotFoundException, PSExtensionException, PSIllegalArgumentException,
          PSDataExtractionException, PSExtensionProcessingException, PSParameterMismatchException,
          PSCmsException {
  ...
  private Iterator getAllRelationships(PSLocator locator) throws PSCmsException {
    ...
    for (PSRelationshipConfig config : configs) allConfigNames.add(config.getName());
    ...
    Collection<String> ignoreOwnerRevConfigNames = getConfigsWhichIgnoreOwnerRevision(configs);
  ...
  private void runTests(PSRelationship currentRel, boolean isOwner)
    ...
    try {
      if (currentRel.getConfig().useServerId()) m_execData.setRequest(m_serverRequest);
      else m_execData.setRequest(m_userRequest);
      ...
      if (m_executionContext.isPreUpdate()) {
        ...
        if (sourceRel != null)
          log.debug(
              "source owner is {} dest owner is {} ",
              sourceRel.getOwner().getId(),
      ...
      while (effects.hasNext()) {
        ...
        if (extension instanceof IPSEffect) {
          ...
          result = (PSTestResult) runner.testEffect(m_execData, m_executionContext);
      ...
      while (iter.hasNext()) {
        ...
        PSLocator next = null;
        if (isOwner) next = rel.getDependent();
        else next = rel.getOwner();
        runTests(next);
        break;
      }
    } finally {
  ...
  private PSAttemptResult runAttempts()
    ...
    try {
      while (keys.hasNext()) {
        ...
        if (relationship.getConfig().useServerId()) m_execData.setRequest(m_serverRequest);
        else m_execData.setRequest(m_userRequest);
        ...
        while (iter.hasNext()) {
          ...
          if (extension instanceof IPSEffect) {
            runner = PSExtensionRunner.createRunner(call, extension);
            attemptResult = (PSAttemptResult) runner.attemptEffect(m_execData, m_executionContext);
