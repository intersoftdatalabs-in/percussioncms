...
public class PSActiveAssemblerProcessor extends PSRelationshipProcessor {
  ...
  public void insert(PSLocator owner, PSDependentSet dependents, int index) throws PSCmsException {
    ...
    String slotId = checkPropertyValue(IPSHtmlParameters.SYS_SLOTID, owner.getId(), dependents);
    ...
    PSRelationshipSet relationships =
        getDependents(config.getName(), owner, IPSHtmlParameters.SYS_SLOTID, slotId);
  ...
  public static PSRelationshipConfig getConfigForSlot(IPSRequestContext request, String slotid)
    ...
    try {
      ...
      if (slots.getLength() > 0) {
        ...
        if (relName.length() < 1) relName = PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY;
      }
      PSRelationshipConfig cfg = PSRelationshipCommandHandler.getRelationshipConfig(relName);
      return cfg;
    } catch (PSException e) {
  ...
  public void reorder(PSLocator owner, PSDependentSet dependents, int index) throws PSCmsException {
    ...
    for (int i = 0; i < relationships.size(); i++) {
      ...
      int sortrank = Integer.parseInt(relationship.getProperty(IPSHtmlParameters.SYS_SORTRANK));
      ...
      PSDependent dependent = contains(dependents.iterator(), relationship);
      if (dependent == null) resultSet.add(relationship);
    }

    // need to append the list if its not been inserted to the results set yet
    if (!inserted) resultSet.addAll(reorderSet);
  ...
  private PSRelationshipSet getSortedRelationships(PSLocator owner, String slotId)
    ...
    PSRelationshipSet relationships =
        getDependents(
            getConfigForSlot(m_request, slotId).getName(),
            owner,
  ...
  private void updateProperties(PSRelationship relationship, PSDependent dependent) {
    ...
    while (properties.hasNext()) {
      ...
      if (value != null) relationship.setProperty(property.getName(), value.toString());
    }
  }
  ...
  private PSDependent contains(Iterator<?> list, PSRelationship relationship)
    ...
    while (list.hasNext()) {
      ...
      if (test == relationship.getId()) return dependent;
    }
    return null;
  }
  ...
  private PSRelationshipSet getDependents(
    ...
    for (int i = 0; i < relationships.size(); i++) {
      PSRelationship relationship = (PSRelationship) relationships.get(i);
      if (propVal.equals(relationship.getProperty(propName))) dependents.add(relationship);
    }
