...
public final class PSRemoteAgent {
  ...
  public PSEntry login() throws PSRemoteException {
    ...
    try {
      ...
      Element loginDataEl = PSXMLDomUtil.getFirstElementChild(responseData, "LoginData");
      String communityId = PSXMLDomUtil.checkAttribute(loginDataEl, "defaultCommunityId", true);
      community = new PSEntry(communityId, new PSDisplayText("Unknown"));

      Element sessionIdEl = PSXMLDomUtil.getFirstElementChild(loginDataEl, "SessionId");
      Element communitiesEl = PSXMLDomUtil.getNextElementSibling(sessionIdEl, "Communities");

      Element communityEl = PSXMLDomUtil.getFirstElementChild(communitiesEl, "Community");
      while (communityEl != null) {
        ...
        communityEl = PSXMLDomUtil.getNextElementSibling(communityEl, "Community");
      }
  ...
  public List<PSEntry> getCommunities() throws PSRemoteException {
    ...
    try {
      ...
      for (int i = 0; i < communityNodes.getLength(); i++) {
        ...
        Element nameEl = PSXMLDomUtil.getFirstElementChild(communityEl, "communityname");
        Element idEl = PSXMLDomUtil.getNextElementSibling(nameEl, "communityid");
  ...
  public List<PSEntry> getTransitions(PSEntry workflow) throws PSRemoteException {
    ...
    try {
      ...
      Document doc =
          m_requester
              .getRemoteRequester()
              .getDocument("sys_wfLookups/getAllTransitionNames.xml", params);
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
        ...
        String trigger = PSXMLDomUtil.checkAttribute(entryEl, "actiontrigger", true);
  ...
  public String getLastTransitionComment(int contentId) throws PSRemoteException {
    try {
      ...
      Document doc =
          m_requester
              .getRemoteRequester()
              .getDocument("sys_psxWorkflowCataloger/getLastTransitionHistory.xml", params);
  ...
  public List<PSEntry> getContextVariables() throws PSRemoteException {
    ...
    try {
      Document doc =
          m_requester.getRemoteRequester().getDocument("sys_pubVariables/variableslist.xml", null);
      ...
      for (int i = 0; i < categoryNodes.getLength(); i++) {
        ...
        Element nameEl = PSXMLDomUtil.getFirstElementChild(categoryEl, "propname");
        ...
        for (int j = 0; j < varlistNodes.getLength(); j++) {
          ...
          String contextid = PSXMLDomUtil.checkAttribute(valueEl, "contextid", true);
          if (contextid.equals("0")) {
            String name = PSXMLDomUtil.getElementData(nameEl);
            String value = PSXMLDomUtil.getElementData(valueEl);
            // strip the leading "../" in "value"
            if (value.startsWith("../") && value.length() > 3) value = value.substring(3);
  ...
  public boolean transitionItem(PSLocator locator, String transId, String comment)
    ...
    Element responseData = sendRequest(TRANSITEM_ACTION, WS_WORKFLOW, params, TRANSITEM_RESPONSE);
  ...
  public PSClientItem newItemDefault(String contentTypeId) throws PSRemoteException {
    ...
    Element respData = sendRequest(NEWITEM_ACTION, WS_CONTENTDATA, params, NEWITEM_RESPONSE);
  ...
  public PSLocator updateItem(Element itemEl, boolean checkin) throws PSRemoteException {
    if (itemEl == null) throw new IllegalArgumentException("itemEl may not be null");

    Element params = getUpdateItemParams(itemEl, checkin);
    Element responseData =
        sendRequest(UPDATEITEM_ACTION, WS_CONTENTDATA, params, UPDATEITEM_RESPONSE);
  ...
  public PSLocator updateItem(PSClientItem item, boolean checkin) throws PSRemoteException {
    if (item == null) throw new IllegalArgumentException("Client item cannot be null.");
      ...
      try {
        if (meta.isBinary()) {
          ...
          if (fieldVal != null && fieldVal instanceof PSBinaryValueEx) {
            ...
            // this binary field
            if (binVal.getFilename() == null || binVal.getFilename().trim().length() == 0) continue;

            files.add(
                new PSBinaryFileData(
                    data, field.getName(), binVal.getFilename(), binVal.getContentType()));
          }
        } else {
          // Non-Binary field
          String value = field.getValue() != null ? field.getValue().getValueAsString() : "";
    ...
    if (item.getContentId() != -1) {
      ...
      params.put("DBActionType", IPSDatabaseComponent.DATABASE_ACTION_UPDATE);
    } else {
      params.put("DBActionType", IPSDatabaseComponent.DATABASE_ACTION_INSERT);
    }
    params.put(IPSHtmlParameters.SYS_CONTENTTYPEID, Long.toString(item.getContentTypeId()));
    ...
    // Checkin if needed
    if (checkin && locator != null) checkInItem(locator);
    return locator;
  }
  ...
  private String getAppResource(PSClientItem item) {
    ...
    if (app_resource == null || app_resource.trim().length() == 0) return null;
    app_resource = app_resource.substring(0, app_resource.lastIndexOf('.'));
    int aPos = app_resource.lastIndexOf('/');
    if (aPos == -1)
      throw new IllegalStateException("The application resource in the client item is invalid.");
    int bPos = app_resource.lastIndexOf('/', aPos - 1);
    bPos = (bPos == -1) ? 0 : bPos;

    return app_resource.substring(bPos + 1, aPos) + app_resource.substring(aPos) + ".xml";
  }
  ...
  public boolean checkOutItem(PSLocator locator) throws PSRemoteException {
    ...
    Element responseEl = sendRequest(CHECKOUT_ACTION, WS_MISC, params, CHECKOUT_RESPONSE);
  ...
  public boolean checkInItem(PSLocator locator) throws PSRemoteException {
    ...
    Element responseEl = sendRequest(CHECKIN_ACTION, WS_MISC, params, CHECKIN_RESPONSE);
  ...
  public boolean purgeItems(PSLocator[] items) throws PSRemoteException {
    ...
    Element response = sendRequest(PURGEITEMS_ACTION, WS_CONTENTDATA, params, PURGEITEMS_RESPONSE);
  ...
  public boolean purgeTree(PSComponentSummary summary) throws PSRemoteException {
    ...
    if (summary.isFolder()) {
      ...
      return isSuccessResponse(PSXMLDomUtil.getFirstElementChild(response));
    } else if (summary.isItem()) return purgeItem(summary.getCurrentLocator());
  ...
  private PSComponentProcessorProxy getRemoteComponentProxy() throws PSCmsException {
    ...
    PSComponentProcessorProxy proxy =
        new PSComponentProcessorProxy(
            PSComponentProcessorProxy.PROCTYPE_REMOTE, m_requester.getRemoteRequester());
    return proxy;
  }
   ...
   */
  public Element newCopyItem(PSLocator locator)
      throws PSRemoteException, PSUnknownNodeTypeException {
    if (locator == null) throw new IllegalArgumentException("locator may not be null");
    ...
    Element respData = sendRequest(NEWCOPY_ACTION, WS_CONTENTDATA, params, NEWCOPY_RESPONSE);
  ...
  public PSClientItem openItem(PSLocator locator, boolean includeData, boolean checkOut)
      throws PSRemoteException {
    return openItem(locator, includeData, includeData, includeData, checkOut);
  }
  ...
  public PSClientItem openItem(
      ...
      boolean checkOut)
      throws PSRemoteException {
    return openItem(locator, includeChildren, includeRelated, includeBinary, checkOut, null);
  }
  ...
  public PSClientItem openItem(
    ...
    Element params = getOpenItemParams(locator, includeChildren, includeRelated, false, checkOut);

    Element respData = sendRequest(OPENITEM_ACTION, WS_CONTENTDATA, params, OPENITEM_RESPONSE);
  ...
  public PSItemDefinition getTypeDef(String contentTypeId) throws PSRemoteException {
    ...
    try {
      Element respData = sendRequest(CONTENTTYPE_ACTION, WS_DESIGN, message, CONTENTTYPE_RESPONSE);

      Element el = PSXMLDomUtil.getFirstElementChild(respData);
      String itemDefStr = PSXMLDomUtil.getElementData(el);

      Document itemDefDoc =
          PSXmlDocumentBuilder.createXmlDocument(new StringReader(itemDefStr), false);
  ...
  private Element getPurgeFolderParams(PSLocator locator) {
    ...
    Element params = doc.createElement(PSRemoteFolderProcessor.PURGE_FOLDER_REQUEST);

    Element folderIdEl =
        PSXmlDocumentBuilder.addElement(
            doc, params, PSRemoteFolderProcessor.FOLDER_ID_EL, "" + locator.getId());
  ...
  private Element getContentKeyElement(Document bodyDoc, PSLocator locator) {
    ...
    contentKeyEl.setAttribute(XML_ATTR_CONTENT_ID, Integer.toString(locator.getId()));
    contentKeyEl.setAttribute(XML_ATTR_REVISION, Integer.toString(locator.getRevision()));
  ...
  private Element createPurgeKeyElement(Document doc, PSLocator locator) {
    ...
    purgeKeyEl.setAttribute(XML_ATTR_CONTENT_ID, Integer.toString(locator.getId()));
  ...
  private static final String WS_DESIGN = "Design";
  private static final String WS_CONTENTDATA = "ContentData";
  private static final String WS_WORKFLOW = "Workflow";
