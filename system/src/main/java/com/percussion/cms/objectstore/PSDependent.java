...
public final class PSDependent extends PSCmsComponent {
  ...
  public boolean equals(Object o) {
    ...
    if (m_relationshipId != dependent.m_relationshipId) return false;
    if (!m_locator.equals(dependent.m_locator)) return false;
    if (m_properties.equals(dependent.m_properties)) return false;
  ...
  public Element toXml(Document doc) {
    ...
    if (!getProperties().isEmpty()) root.appendChild(getProperties().toXml(doc));
  ...
  public void fromXml(Element source, IPSDocument parentDoc, List parentComponents)
    ...
    setRelationshipId(PSXMLDomUtil.checkAttributeInt(source, "relationshipId", true));

    setLocator(new PSLocator(walker.getNextElement(PSLocator.XML_NODE_NAME)));
