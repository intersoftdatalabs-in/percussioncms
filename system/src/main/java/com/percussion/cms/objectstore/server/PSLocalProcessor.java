...
public class PSLocalProcessor extends PSProcessorCommon {
  ...
  protected Document doLoad(String resourceName, Map<String, String[]> ids) throws PSCmsException {
    ...
    try {
      ...
      IPSInternalResultHandler irh = (IPSInternalResultHandler) ireq.getInternalRequestHandler();
      ...
      if (resourceName.toLowerCase().endsWith("xml")) doc = ireq.getResultDoc();
      else {
        ByteArrayOutputStream os = ireq.getMergedResult();
        doc =
            PSXmlDocumentBuilder.createXmlDocument(
                new StringReader(new String(os.toByteArray(), PSCharSets.rxJavaEnc())), false);
      }
      ...
      if (doc != null) {
        ...
        if (root != null) {
          ...
          if (child != null) resultDoc = doc;
        }
      }

      return resultDoc;
    } catch (PSInternalRequestCallException irce) {
      throw new PSCmsException(irce.getErrorCode(), irce.getErrorArguments());
  ...
  protected int doDelete(String resourceName, Map<String, String[]> ids) throws PSCmsException {
    ...
    try {
      ...
      throw new PSCmsException(ae.getErrorCode(), ae.getErrorArguments());
    } catch (PSAuthenticationFailedException afe) {
      throw new PSCmsException(afe.getErrorCode(), afe.getErrorArguments());
    }
  }
  ...
  private PSInternalRequest getRequest(
    ...
    if (null != m_req) {
      ...
      if (req != null) req.getRequest().setInputDocument(input);
    } else {
      req = PSServer.getInternalRequest(resourceName, m_ctx, params, true, input);
    }
    if (null == req) {
