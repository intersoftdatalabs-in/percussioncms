...
public abstract class PSProcessorProxy {
  ...
  private ProcessorConfig processComponentConfig(
    ...
    Element generalProps =
        ewalker.getNextElement(
            "Properties",
    ...
    while (null != processorEl) {
      String foundProcType = PSXMLDomUtil.checkAttribute(processorEl, "type", true);
      Node curProc = ewalker.getCurrent();
      if (procType.equalsIgnoreCase(foundProcType)) {
        String procClass = PSXMLDomUtil.checkAttribute(processorEl, "className", true);
        foundProc = true;
        ...
        Element propertyEl =
            ewalker.getNextElement(PROPERTY_NODENAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
        while (null != propertyEl) {
          Node curProp = ewalker.getCurrent();
          String propName = PSXMLDomUtil.checkAttribute(propertyEl, "name", true).toLowerCase();
          ...
          } else processorProps.put(propName, propValue);

          ewalker.setCurrent(curProp);
          propertyEl =
              ewalker.getNextElement(PROPERTY_NODENAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
        }
      ...
      processorEl =
          ewalker.getNextElement(PROCESSOR_NODENAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
    return cfg;
  }
  ...
  public void assignKey(IPSDbComponent comps[], PSKey parentKey) throws PSCmsException {
    ...
    for (int i = 0; i < comps.length; i++) {
      if (comps[i] == null) {
        throw new IllegalArgumentException("Component entry cannot be null.");
      }
    }
  ...
  protected class ProcessorConfig {
    ...
    public Object getProcessor(IPSDbComponent comp) throws PSCmsException {
      ...
      // member type
      if (compType.equals(PSDbComponent.getComponentType(PSDbComponentCollection.class))) {
        PSDbComponentCollection compColl = (PSDbComponentCollection) comp;
        compType = compColl.getMemberComponentType();
      } else if (compType.equals(PSDbComponent.getComponentType(PSDbComponentList.class))) {
