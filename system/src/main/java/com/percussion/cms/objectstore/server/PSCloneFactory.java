...
public class PSCloneFactory {
  ...
  public static PSLocator createClone(
    ...
    try {
      ...
      if (cache != null) item = cache.getItem(source.getId());

      if (item != null) {
        if (item.isFolder()) clone = createFolderClone(request, source);
        else
          clone =
              createItemClone(
                  request,
                  source,
                  PSCms.getNewRequestResource(new PSRequestContext(request), source),
                  childRowMappings);
      } else {
        ...
        else clone = createItemClone(request, source, contentType.getNewURL(), childRowMappings);
      }
   ...
   */
  private static PSLocator createFolderClone(PSRequest request, PSLocator source)
      throws PSCmsException {
    ...
    for (Map.Entry<String, String> entry : overrides.entrySet()) {
      ...
      if (value != null) {
        ...
        if (value.trim().length() > 0) {
          ...
          } else if (name.equals(IPSHtmlParameters.SYS_LANG)) tgt.setLocale(value);
        }
      }
    }
  ...
  private static PSLocator createItemClone(
    ...
    try {
      ...
      cloneParams.put(PSCloneCommandHandler.SYS_WFACTION, PSCloneCommandHandler.WF_ACTION_CHECKIN);
      ...
      return new PSLocator(
          ir.getRequest().getParameter(IPSHtmlParameters.SYS_CONTENTID),
          ir.getRequest().getParameter(IPSHtmlParameters.SYS_REVISION));
    } catch (PSException e) {
      throw new PSCmsException(e);
    }
  }
