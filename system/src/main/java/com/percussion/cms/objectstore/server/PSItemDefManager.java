...
public class PSItemDefManager {
  ...
  public String[] getContentTypeNames(PSSecurityToken securityToken) {
    ...
    for (int i = 0; i < ids.length; i++) names[i] = getItemDefSummary(ids[i]).getName();
    return names;
  }
  ...
  public long contentTypeNameToId(String contentTypeName) throws PSInvalidContentTypeException {
    ...
    while (defs.hasNext() && !found) {
      PSItemDefSummary summary = (PSItemDefSummary) ((List<?>) defs.next()).get(1);
      if (summary.getName().equalsIgnoreCase(contentTypeName)) {
        id = summary.getTypeId();
        found = true;
      }
    }
    if (!found) throw new PSInvalidContentTypeException(contentTypeName);
  ...
  public PSItemDefSummary getSummary(long contentTypeId, PSSecurityToken securityToken)
    ...
    if (summary == null) {
      throw new RuntimeException(
          "Content type with id="
              + contentTypeId
              + " is not visible to the community with id="
              + communityId);
    }
    return summary;
  }
  ...
  public long getItemContentType(PSLocator itemId) {
    ...
    if (cache != null) {
      ...
      if (entry != null) return entry.getContentTypeId();
    }
  ...
  public long registerDef(String appName, PSContentEditor editorDef)
      throws PSInvalidContentTypeException {
    if (null == editorDef) throw new IllegalArgumentException("Missing editor definition");
    ...
    PSSecurityToken internalToken = PSRequest.getContextForRequest().getSecurityToken();
    ...
    try {
      nodeDef =
          cml.loadNodeDefinitions(Collections.singletonList(new PSGuid(PSTypeEnum.NODEDEF, typeId)))
              .stream()
              .findFirst()
              .orElse(null);
    ...
    PSItemDefSummary summary =
        new PSItemDefSummary(
            typeName,
            typeDef.getLabel(),
            typeDef.getTypeId(),
            typeDef.getQueryRequest(),
            typeDef.getDescription(),
            typeDef.isHiddenFromMenu());

    PSCmsObject cmsObject = PSServer.getCmsObjectRequired(typeDef.getObjectType());
  ...
  public void registerDef(PSItemDefinition def, PSCmsObject cmsObject) {
    ...
    if (cmsObject == null) throw new IllegalArgumentException("cmsObject cannot be null");
  ...
  public String getTypeEditorUrl(long contentTypeId) throws PSException {
    PSItemDefSummary summary = getSummary(contentTypeId, PSItemDefManager.COMMUNITY_ANY);
  ...
  public String getAssemblerUrl(IPSRequestContext request, int variantId)
    ...
    IPSInternalRequest ir =
        request.getInternalRequest("sys_casSupport/AssemblyUrl.xml", params, false);
  ...
  private PSContentType getContentTypeDef(PSSecurityToken token, long contentTypeId) {
    ...
    try {
      elems = getProxy(token).load(PSDbComponent.getComponentType(PSContentTypeSet.class), keys);
  ...
  private Element load(String resource, Map<String, Object> params) {
    try {
      ...
      PSInternalRequest ir = PSServer.getInternalRequest(path, req, params, true);
      if (null == ir) {
        throw new RuntimeException("System application '" + path + "' missing or not running.");
      }
  ...
  private boolean isRegisteredItemType(long contentTypeId) {
    ...
    for (Long key : m_itemDefMap.keySet()) {
      ...
      if (list != null) {
        ...
        if (cmsObj != null && cmsObj.getTypeId() != PSCmsObject.TYPE_ITEM) continue;
      }

      long id = key;
      if (contentTypeId == id) return true;
    }
    return false;
  }
  ...
  private long[] getVisibleContentTypes(
    ...
    for (Entry<Long, List<Object>> entry : m_itemDefMap.entrySet()) {
      ...
      if (isUIVisible || forContentExplorer) {
        ...
        if (list != null) {
          if (isUIVisible) {
            // skip Folder, which should not be visible from UI or MENU
            PSCmsObject cmsObj = (PSCmsObject) list.get(3);
            if (cmsObj != null && cmsObj.getTypeId() == PSCmsObject.TYPE_FOLDER) continue;
          }
      ...
      if (id == m_currentRegisteredId) continue;
  ...
  public Map<String, List<String>> getFieldsWithControlProp(
      String propertyName, String propertyValue) throws PSInvalidContentTypeException {
    ...
    for (long typeId : getAllContentTypeIds(COMMUNITY_ANY)) {
      ...
      if (!fieldList.isEmpty()) retMap.put(typeName, fieldList);
    }
    return retMap;
  }
  ...
  public void unRegisterDef(PSContentEditor editorDef) throws Exception {
    ...
    if (editorDef == getContentEditorDef(typeId)) {
      ...
      synchronized (m_itemDefListeners) {
        for (Iterator<IPSItemDefChangeListener> iter = m_itemDefListeners.iterator();
            iter.hasNext(); ) {
          IPSItemDefChangeListener listener = iter.next();
  ...
  public synchronized void commitUpdateNotifications() {
    if (m_deferringNotifications && m_deferredNotifications.size() > 0) {
      for (Iterator<PSItemDefinition> iter = m_deferredNotifications.iterator(); iter.hasNext(); ) {
  ...
  public Map<PSLocator, String> getContentTypeIconPaths(List<PSLocator> itemLoc) {
    ...
    for (PSLocator locator : itemLoc) {
      ...
      if (ctypeid == -1) {
        log.warn(
            "Failed to get the content type for locator {} . Setting the icon path to null",
            locator.toString());
      ...
      if (typeDef == null) {
        try {
          ...
          log.warn(
              "Failed to get the item def for contenttype id {} . Setting the icon path to null for"
                  + " locator {}",
              ctypeid,
              locator.toString());
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
  ...
  private Map<PSLocator, String> getFileIconPaths(Map<PSLocator, String> fileItems) {
    ...
    for (PSLocator loc : fileItems.keySet()) {
      List<Node> nodes;
      try {
        nodes = cmgr.findItemsByGUID(Collections.singletonList(guidMgr.makeGuid(loc)), config);
        Node node = nodes.get(0);
        if (node == null) {
          icPaths.put(loc, null);
          log.warn(
              "Failed to load item for locator {} Setting the icon path to null.", loc.toString());
          continue;
        }
        ...
        log.warn(
            "Failed to load item for locator {}  Setting the icon path to null. Error : {}",
            loc.toString(),
            e);
        log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        icPaths.put(loc, null);
    ...
    File file =
        new File(
            PSServer.getRxDir()
                ...
                + FILE_ICONS_FOLDER
                + "/"
                + FILE_ICONS_PROPERTIES);
    if (!file.exists()) return rxProps;
    if (ms_rxFileIconProperties == null) {
      loadProps = true;
    } else {
      Iterator<Long> iter = ms_rxFileIconProperties.keySet().iterator();
      Long dt = iter.next();
      if (file.lastModified() > dt || ms_rxFileIconProperties.get(dt) == null) loadProps = true;
      else rxProps = ms_rxFileIconProperties.get(dt);
    }
    if (loadProps) {
  ...
  public synchronized Properties getSysFileIconProperties() {
    ...
    if (ms_sysFileIconProperties == null) {
      ...
      File sfile =
          new File(
              PSServer.getRxFile(
                  SYS_ICON_FOLDER + FILE_ICONS_FOLDER + "/" + FILE_ICONS_PROPERTIES));
      try {
        ms_sysFileIconProperties.load(new FileInputStream(sfile));
      } catch (IOException e) {
        log.warn(
            "Error getting the sys resources file icon details. Error: {}",
