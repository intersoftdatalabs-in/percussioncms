...
public final class PSMenuModeContextMapping extends PSDbComponent {
  ...
  public void fromXml(Element source) throws PSUnknownNodeTypeException {
    ...
    m_contextId = PSXMLDomUtil.checkAttribute(source, XML_ATTR_CONTEXTID, true);
    m_modeName = PSXMLDomUtil.checkAttribute(source, XML_ATTR_MODENAME, false);
    m_contextName = PSXMLDomUtil.checkAttribute(source, XML_ATTR_CONTEXTNAME, false);
    ...
    if (key.isAssigned()) {
      if (!m_modeId.equals(key.getPart(MODEID_COLNAME))
        ...
        String[] args = {
          ...
          key.getPart(MODEID_COLNAME) + ":" + key.getPart(CONTEXTID_COLNAME)
        };
  ...
  public boolean equals(Object obj) {
    ...
    if (!m_modeId.equalsIgnoreCase(other.m_modeId)) return false;
    else if (!m_contextId.equalsIgnoreCase(other.m_contextId)) return false;
  ...
  private static PSKey createKey(String modeId, String ctxId, String actId) {
    String[] keyDef = new String[] {MODEID_COLNAME, CONTEXTID_COLNAME, "ACTIONID"};
