...
public class PSDbComponentList extends PSDbComponent {
  ...
  public PSDbComponentList(Class compClass, String compType) {
    this();
    if (null == compClass) throw new IllegalArgumentException("Supplied class cannot be null.");
   ...
   */
  public PSDbComponentList(Element[] items, Class compClass) throws PSUnknownNodeTypeException {
  ...
  void fromXml(Element src, String nodeName) throws PSUnknownNodeTypeException {
    ...
    try {
      ...
      if (null != seqFlag && seqFlag.equalsIgnoreCase(XML_FALSE)) m_ignoreSequencing = true;
      else m_ignoreSequencing = false;
  ...
  public void setPersisted() throws PSCmsException {
    ...
    while (it.hasNext()) {
       ...
       * minimizing changes.
       */
      else if (!(m_markedForDelete && c.getState() == DBSTATE_NEW)) c.setPersisted();
    }
  ...
  public void toDbXml(Document doc, Element root, IPSKeyGenerator keyGen, PSKey parent)
    ...
    while (iter.hasNext()) {
      ...
      if (!m_ignoreSequencing) ((IPSSequencedComponent) c).setPosition(index++);

      // Check that this component is marked for delete
      if (m_markedForDelete) {
        if (c.isPersisted()) c.markForDeletion();
        else continue; // skip new components since we're deleting
      }
  ...
  public int getState() {
    ...
    if (m_list.isEmpty()) return DBSTATE_UNMODIFIED;
  ...
  public void set(int index, IPSDbComponent comp) {
    ...
    if (index < 0 || index >= size())
      throw new IndexOutOfBoundsException(index + " out of bounds. Existing size: " + size());

    m_deleteList.add(get(index));
    m_list.set(index, comp);

    if (!m_ignoreSequencing) ((IPSSequencedComponent) comp).setPosition(index);
  }
  ...
  public void add(int index, IPSDbComponent comp) {
    ...
    if (index == size()) m_list.add(comp);
    else m_list.add(index, comp);
  ...
  public void move(int fromIndex, int toIndex) {
    if (fromIndex < 0 || fromIndex > size())
      throw new IndexOutOfBoundsException(fromIndex + " out of bounds. Existing size: " + size());
    ...
    if (toIndex == size()) add(moveMe);
    else add(toIndex, moveMe);
  }
  ...
  private boolean checkType(IPSDbComponent comp, boolean shouldThrow) {
    ...
    for (int i = 0; i < interfaces.length; i++) {
      if (interfaces[i].getName().equals(IPSSequencedComponent.class.getName())) {
        interfaceOk = true;
      }
    }
    ...
    if (!m_ignoreSequencing && !interfaceOk) {
      if (shouldThrow) {
        throw new ClassCastException(
            comp.getClass().getName() + " must be a " + IPSSequencedComponent.class);
      }
      isValid = false;
    }
    return isValid;
  ...
  public boolean contains(IPSDbComponent comp) {
    ...
    while (iter.hasNext()) {
      ...
      if (aComp.equals(comp)) return true;
    }
  ...
  public IPSDbComponent get(int index) {
    if (index >= size() || index < 0)
      throw new IndexOutOfBoundsException(
          "Requested component at index " + index + ", but list only has " + size() + " entries.");
  ...
  public int indexOf(IPSDbComponent comp) {
    if (!checkType(comp, false)) return -1;
  ...
  private int getIndex(IPSDbComponent comp) {
    ...
    Iterator it = m_list.iterator();
    while (it.hasNext()) {
      if (((IPSDbComponent) it.next()).equals(comp)) return i;
      i++;
    }
    return -1;
  }
