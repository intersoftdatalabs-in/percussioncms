...
/**
 * The class represents a collection of ACL entries. Each ACL entries represents a level of access
 * for a particular user, role or virtual entry (Folder Community, Everyone) on a specified
 * securable object such as a "folder". This class acts as a Set even though it does not implement
 * the <code>Set</code> interface. A <code>PSObjectAclEntry</code> object is considered a duplicate
 * if it represents permissions for the same user, role, or virtual entry as another existing object
 * in this collection. Adding a duplicate <code>PSObjectAclEntry</code> returns <code>false</code>
 * from the <code>add()</code> method and the collection does not change. Adding a collection of
 * <code>PSObjectAclEntry</code> objects using <code>addAll()</code> method returns <code>true
 * </code> even if the collection being added contains objects already existing in this collection.
 * However the resulting collection does not contain any duplicate entries.
 *
 * <p>This class needs to extend <code>PSDbComponentSet</code> to use the functionality of storing
 * state (new, modified, unmodified, markedfordelete) of ACL entries. The state of ACL entries is
...
public class PSObjectAcl extends PSDbComponentSet<PSObjectAclEntry> {
   ...
   */
  public PSObjectAcl(Element element) throws PSUnknownNodeTypeException {
    super(element);
  }

  /**
   * Constructs this object from the supplied element. See {@link #toXml(Document)} for the expected
   * form of xml.
   *
   ...
   *
   * @param name the name of the specified ACL entry. It may not be <code>null</code> or emtpy. The
   *     comparison is case insensitive.
   * @param type the specified ACL type.
   * @return the specified ACL entry. It may be <code>null</code> if the specified ACL entry does
   *     not exist.
   */
  ...
  /**
   * Produce XML representation of this object. The xml format is:
   *
   * <p>&lt;!ELEMENT PSXObjectAcl (PSXObjectAclEntry+)><br>
   *
   * <p>See {@link com.percussion.cms.objectstore.PSObjectAclEntry#toXml(Document)} for the DTD of
   * PSXObjectAclEntry element.
   *
   ...
   *
   * @param obj the object which should be compared with this object, may not be <code>null</code>
   * @return <code>true</code> if the specified object is an instance of this class and contains
   *     same number of entries and for the same users, role and virtual entries.
   * @throws IllegalArgumentException if <code>obj</code> is <code>null</code>
   */
  public boolean equals(Object obj) {
    boolean equals = super.equals(obj);
    if (equals) {
      if (!(obj instanceof PSObjectAcl)) equals = false;
    }
    return equals;
  }
