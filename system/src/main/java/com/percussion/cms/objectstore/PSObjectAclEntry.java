...
public final class PSObjectAclEntry extends PSDbComponent {
  ...
  public String toString() {
    return "name=" + m_name + ", type=" + m_type + ", permissions=" + m_permissions;
  }
  ...
  public void setAccess(int level, boolean grant) {
    ...
    if (grant) {
      if (level == ACCESS_DENY) m_permissions = ACCESS_DENY;
      else m_permissions = m_permissions | level;
    } else m_permissions = m_permissions & ~level;
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    // permissions
    temp = PSComponentUtils.getRequiredAttribute(sourceNode, XML_ATTR_PERMISSIONS);
    try {
  ...
  public boolean equals(Object obj) {
    ...
    boolean equals = true;
    if (!(obj instanceof PSObjectAclEntry)) equals = false;
    else {
      PSObjectAclEntry other = (PSObjectAclEntry) obj;

      if (m_type != other.m_type) equals = false;
      else if (!m_name.equalsIgnoreCase(other.m_name)) equals = false;
    }
    return equals;
  }
  ...
  public int hashCodeFull() {
    return super.hashCodeFull() + m_type + m_name.toLowerCase().hashCode() + m_permissions;
  }
  ...
  private int validateType(String type) {
    for (int i = 0; i < ACL_ENTRY_TYPES.length; i++) {
      if (type.equalsIgnoreCase(ACL_ENTRY_TYPES[i])) return i;
    }
    throw new IllegalArgumentException("Invalid ACL entry type");
  }
