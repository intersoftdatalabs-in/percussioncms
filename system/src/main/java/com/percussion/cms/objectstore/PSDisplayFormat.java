...
public class PSDisplayFormat extends PSVersionableDbComponent
  ...
  public boolean isValidForRelatedContent() {
    ...
    while (columns.hasNext() && (!ctTypeExists || !variantExists)) {
      ...
      if (!ctTypeExists
          ...
          && (column.getSource().equals(COL_VARIANTID)
              || column.getSource().equals(COL_VARIANTNAME))) variantExists = true;
    }
  ...
  public boolean isColumnSorted(String columnName) {
    ...
    if (sortedColumnName != null) isSorted = sortedColumnName.equals(columnName);
  ...
  public String getInvalidFolderFieldNames() {
    ...
    for (int i = 0; i < ms_invalidFolderFields.length; i++) {
      ...
      if (i < ms_invalidFolderFields.length - 1) names.append(", ");
    }
  ...
  public void removeInvalidFolderColums() {
    ...
    for (IPSDbComponent delete : deletes) m_columns.remove(delete);
  }
  ...
  public void setDescription(String description) {
    ...
    if (description.equalsIgnoreCase(m_strDescription)) return;
  ...
  public void toDbXml(Document doc, Element root, IPSKeyGenerator keyGen, PSKey parent)
    ...
    if (m_columns.getState() != DBSTATE_UNMODIFIED) m_columns.toDbXml(doc, root, keyGen, key);
  ...
  protected Element toXml(Document doc, boolean includeChildComps) {
    ...
    if (elDisName == null)
      throw new IllegalStateException("Unable to create " + XML_NODE_DISPLAYNAME + " element.");

    Element elInternalName =
        PSXmlDocumentBuilder.addElement(doc, root, XML_NODE_INTERNALNAME, m_strInternalName);

    if (elInternalName == null)
      throw new IllegalStateException("Unable to create " + XML_NODE_INTERNALNAME + " element.");

    Element elVersion =
        PSXmlDocumentBuilder.addElement(doc, root, XML_NODE_VERSION, String.valueOf(m_version));

    if (elVersion == null)
      throw new IllegalStateException("Unable to create " + XML_NODE_VERSION + " element.");
    ...
    if (m_strDescription.trim().length() > 0) {
      ...
      if (elDescName == null)
        throw new IllegalStateException("Unable to create " + XML_NODE_DESCRIPTION + " element.");
    }
  ...
  public void fromXml(Element e) throws PSUnknownNodeTypeException {
    ...
    if (elDis == null) throw new IllegalStateException(XML_NODE_DISPLAYNAME + " must exist");
    ...
    if (elInternal == null) throw new IllegalStateException(XML_NODE_INTERNALNAME + " must exist");
    ...
    if (elVersion == null) throw new IllegalStateException(XML_NODE_VERSION + " must exist");
    ...
    if (elDesc != null) m_strDescription = tree.getElementData(elDesc);

    Element aEl =
        tree.getNextElement(
            PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS | PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
  ...
  private void addSystemTitle(PSDFColumns columns) {
    ...
    if (!found) {
      PSDisplayColumn titleColumn =
          new PSDisplayColumn(
              SYS_TITLE, SYS_TITEL_LABEL, PSDisplayColumn.GROUPING_FLAT, null, null, true);
  ...
  public void addCommunity(String communityId) {
    ...
    if (doesPropertyHaveValue(PROP_COMMUNITY, communityId)) return;
  ...
  public void setInternalName(String str) {
    ...
    if (str.equalsIgnoreCase(m_strInternalName)) return;
    if (str.length() > INTERNALNAME_LENGTH)
      throw new IllegalArgumentException(
          "internal name must not exceed " + INTERNALNAME_LENGTH + " characeters");
  ...
  public static final String XML_NODE_COLUMNS = PSDFColumns.XML_NODE_NAME;
  public static final String XML_NODE_PROPERTIES = PSDFProperties.XML_NODE_NAME;
