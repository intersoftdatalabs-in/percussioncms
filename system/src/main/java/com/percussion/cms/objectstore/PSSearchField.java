...
public class PSSearchField extends PSDbComponent implements IPSSequencedComponent {
  ...
  public static PSKey createKey(String fieldName, int searchId) {
    ...
    if (null == fieldName || fieldName.trim().length() == 0) key = new PSKey(keys);
    else key = new PSKey(keys, values, true);
  ...
  public PSSearchField(
      String strName, String strLabel, String mnemonic, String strType, String strDesc) {
    this();
    ...
    if (strName.length() > FIELDNAME_LENGTH)
      throw new IllegalArgumentException(
          "field name must not exceed " + FIELDNAME_LENGTH + "characters");

    if (strType == null || strType.trim().length() == 0)
      throw new IllegalArgumentException("field type must not be null or empty");

    if (strLabel == null || strLabel.trim().length() == 0) strLabel = strName;
  ...
  public void fromXml(Element e) throws PSUnknownNodeTypeException {
    ...
    if (elChoices != null) m_choices = new PSDisplayChoices(elChoices);
    tree.setCurrent(e);
  ...
  private String normalizeOperator(String op) {
    ...
    if (op != null && op.trim().length() > 0) {
      ...
      for (int i = 0; i < ops.length && normalOp == null; i++) {
        if (ops[i].equalsIgnoreCase(op)) normalOp = ops[i];
      }
    }
  ...
  public void setOperator(String str) {
    ...
    // Threshold
    if (str.equalsIgnoreCase(m_strOperator)) return;

    String normalOp = normalizeOperator(str);
    if (normalOp == null)
      throw new IllegalArgumentException("Invalid operator specified for search field");

    if (str.length() > OPERATOR_LENGTH)
      throw new IllegalArgumentException(
          "operator must not exceed " + OPERATOR_LENGTH + "characters.");
  ...
  public void setFieldType(String str) {
    ...
    // Threshold
    if (m_strFieldType.equalsIgnoreCase(str)) return;
  ...
  public void setFieldValues(String op, List values) {
    ...
    if (setValues(op, null, values)) setDirty();
  }
  ...
  public void setFieldValue(String op, String value) {
    ...
    if (value.length() > FIELDVALUE_LENGTH)
      throw new IllegalArgumentException(
          "field value must not exceed " + FIELDVALUE_LENGTH + "characters");
  ...
  public void setExternalFieldValues(String extOp, List values) {
    ...
    if (setValues(null, extOp, values)) setDirty();
  }
  ...
  public void setDisplayName(String str) {
    if (str == null) throw new IllegalArgumentException("display name must not be null");
  ...
  public void setMnemonic(String mnemonic) {
    ...
    if (mnemonic.length() > 1) throw new IllegalArgumentException("mnemonic size must be <= 1");
  ...
  public char getMnemonicChar() {
    if (m_mnemonic.length() == 0) return 0;
  ...
  public void copyFrom(IPSDbComponent src) {
    ...
    setValues(other.getOperator(), other.getExternalOperator(), other.m_values);
  ...
  public int hashCode() {
    ...
    StringBuilder buf =
        new StringBuilder(
            m_strDescription.toLowerCase()
                + m_strFieldType
                + m_strFieldName.toLowerCase()
                + m_strOperator
                + m_sequence
                + m_extOperator);
    Iterator iter = m_values.iterator();
    ...
    return nHash + buf.toString().hashCode() + (m_choices != null ? m_choices.hashCode() : 0);
  }
  ...
  private boolean setValues(String op, String extOp, List values) {
    ...
    if (op.trim().length() == 0 && extOp.trim().length() == 0) {
      throw new IllegalArgumentException("either op or extOp must not be null or emtpy");
    }
    ...
    if (!m_strOperator.equalsIgnoreCase(op) || !m_extOperator.equals(extOp)) {
      ...
      if (op.trim().length() > 0) m_strOperator = op;
      m_extOperator = extOp;
    }
    ...
    if (extOp.trim().length() > 0) {
      ...
      if (op.equalsIgnoreCase(OP_BETWEEN) && values.size() < 2) {
        values.add("");
        if (values.size() < 2) values.add("");
      } else if (values.size() < 1) values.add("");

      Iterator iter = values.iterator();
      boolean finished = false;
      if (!(op.equalsIgnoreCase(OP_ISNULL) || op.equalsIgnoreCase(OP_ISNOTNULL))) {
        while (iter.hasNext() && !finished) {
          ...
          if (op.equalsIgnoreCase(OP_BETWEEN)) {
            if (newValues.size() == 2) finished = true;
          }
        }
      }
    }
    ...
    optimization */
    if (!dirty && newValues.equals(getFieldValues())) dirty = false;
    else {
      m_values = newValues;
      dirty = true;
    }
