...
public class PSContentTypeTemplate extends PSDbComponent {
  ...
  public PSContentTypeTemplate(IPSAssemblyTemplate template) {
    super(
        new PSKey(
            new String[] {KEY_VARIANTID}, new int[] {(int) template.getGUID().longValue()}, false));
    if (template == null) {
    ...
    m_variantSlots = new PSVariantSlotTypeSet(template.getGUID(), template.getSlots());
    IPSContentMgr mgr = PSContentMgrLocator.getContentMgr();
    try {
      List<IPSNodeDefinition> defs = mgr.findNodeDefinitionsByTemplate(template.getGUID());
  ...
  public int getContentTypeId() {
    if (m_contentTypes == null || m_contentTypes.size() == 0) {
      ...
      return getKeyPartInt(KEY_CONTENTTYPEID, (int) (long) m_contentTypes.get(0));
    }
  }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_assembyUrl != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_AssemblyUrl, m_assembyUrl);

    if (m_styleSheetName != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_StyleSheetName, m_styleSheetName);

    if (m_name != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_VariantDescription, m_name);

    if (m_description != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_Description, m_description);

    if (m_publishWhen != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_PublishWhen, m_publishWhen);

    if (m_locationPrefix != null)
      PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_LocationPrefix, m_locationPrefix);

    if (m_variantSlots != null) m_variantSlots.toXml(doc);
  ...
  private void loadFieldsFromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    Element el =
        walker.getNextElement(XML_ELEM_AssemblyUrl, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_assembyUrl = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_StyleSheetName, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_styleSheetName = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el =
        walker.getNextElement(XML_ELEM_VariantDescription, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_name = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_Description, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_description = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_PublishWhen, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_publishWhen = PSXmlTreeWalker.getElementData(el);

    walker.setCurrent(sourceNode);

    el = walker.getNextElement(XML_ELEM_LocationPrefix, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (el != null) m_locationPrefix = PSXmlTreeWalker.getElementData(el);
    ...
    el =
        walker.getNextElement(
            PSVariantSlotTypeSet.XML_NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);

    if (m_variantSlots == null) m_variantSlots = new PSVariantSlotTypeSet();

    if (el != null) m_variantSlots.fromXml(el);
  }
  ...
  public boolean equals(Object o) {
    ...
    if (m_aaType != type.m_aaType) return false;
    if (!compare(m_assembyUrl, type.m_assembyUrl)) return false;
    if (!compare(m_description, type.m_description)) return false;
    if (!compare(m_publishWhen, type.m_publishWhen)) return false;
    if (!compare(m_locationPrefix, type.m_locationPrefix)) return false;
    if (!compare(m_name, type.m_name)) return false;
    if (m_outputFormat != type.m_outputFormat) return false;
    if (!compare(m_styleSheetName, type.m_styleSheetName)) return false;
  ...
  public static PSKey[] createKeys(int[] variantIds) {
    ...
    for (int i = 0; i < variantIds.length; i++) {
      if (variantIds[i] < 1) throw new IllegalArgumentException("variantIds may not be < 0");
  ...
  private static final String XML_ATTR_outputFormat = "outputFormat";
  private static final String XML_ATTR_aaType = "aaType";

  private static final String XML_ELEM_VariantDescription = "VariantDescription";
  private static final String XML_ELEM_StyleSheetName = "StyleSheetName";
  private static final String XML_ELEM_AssemblyUrl = "AssemblyUrl";
  private static final String XML_ELEM_Description = "Description";
  private static final String XML_ELEM_LocationPrefix = "LocationPrefix";
  private static final String XML_ELEM_PublishWhen = "PublishWhen";
}
