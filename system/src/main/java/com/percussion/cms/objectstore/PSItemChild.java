...
public class PSItemChild extends PSItemComponent {
  ...
  public PSItemChild(PSFieldSet fieldSet, PSDisplayMapping displayMapping) {
    ...
    if (displayMapping.getDisplayMapper() == null)
      throw new IllegalArgumentException("displayMapping must contian a child mapper");
  ...
  public PSItemChildEntry getChildEntryByRowId(int childRowId) {
    if (childRowId < 0) throw new IllegalArgumentException("childRowId must be >= 0");
    ...
    while (childEntryIter.hasNext()) {
      ...
      if (childEntry.getChildRowId() == childRowId) return childEntry;
    }
    return null;
  }
  ...
  public PSItemChildEntry createChildEntry() {
    ...
    } else return (PSItemChildEntry) m_template_entry.clone();
  }
  ...
  public void move(PSItemChildEntry childEntry, int upDown) {
    ...
    // can we move:
    if (currentPos + upDown == -1 || currentPos + upDown >= m_entryList.size())
  ...
  public void moveToPosition(PSItemChildEntry childEntry, int position) {
    ...
    int currentPos = m_entryList.indexOf(childEntry);
    if (currentPos == -1) throw new IllegalArgumentException("childEntry not found in list");
  ...
  public void addEntry(PSItemChildEntry childEntry) {
    ...
    if (!validateEntry(childEntry))
      throw new IllegalArgumentException("childEntry must have a valid PSItemField objects");
  ...
  public void replaceEntry(PSItemChildEntry childEntry) {
    ...
    if (!validateEntry(childEntry))
      throw new IllegalArgumentException("childEntry must have valid PSItemField objects");
    ...
    if (curChild == null) throw new IllegalArgumentException("matching childEntry not found");
  }
  ...
  private boolean validateEntry(PSItemChildEntry childEntry) {
    ...
    while (entryFields.hasNext()) {
      ...
      if (!entryField.getItemFieldMeta().equals(templateFieldMeta)) return false;
    }
    return true;
  }
  ...
  protected Element toXml(Document doc, PSAcceptElements acceptElements) {
    ...
    // childentry:
    if (!m_entryList.isEmpty()) toXmlCollection(root, doc, m_entryList, acceptElements);
  ...
  public boolean equals(Object obj) {
    ...
    if (!compare(m_fieldSetDef, comp.m_fieldSetDef)) return false;
    if (!compare(m_uiDef, comp.m_uiDef)) return false;
    if (!compare(m_entryList, comp.m_entryList)) return false;
    if (!compare(m_template_entry, comp.m_template_entry)) return false;

    if (isSequenced() != comp.isSequenced()) return false;
    if (!compare(getName(), comp.getName())) return false;
    if (!compare(getDisplayName(), comp.getDisplayName())) return false;
    if (m_childId != comp.m_childId) return false;
  ...
  void loadXmlData(Element sourceNode, boolean clearValues) throws PSUnknownNodeTypeException {
    ...
    if (clearValues) m_entryList.clear();

    // get all the child entries
    Element el = PSXMLDomUtil.getFirstElementChild(sourceNode);
    while (el != null
        && PSXMLDomUtil.getUnqualifiedNodeName(el).equals(PSItemChildEntry.EL_CHILD_ENTRY)) {
      if (clearValues) {
        ...
        if (id < 0) {
          ...
          if (itemChildEntry == null) itemChildEntry = createAndAddChildEntry();
        }
        itemChildEntry.loadXmlData(el, clearValues);
      }
