...
public final class PSSite extends PSComponent {
  ...
  public boolean equals(Object o) {
    ...
    return isPageBased() == psSite.isPageBased()
        && m_name.equals(psSite.m_name)
        && Objects.equals(m_description, psSite.m_description)
        && Objects.equals(m_baseUrl, psSite.m_baseUrl)
        && Objects.equals(m_root, psSite.m_root)
        && Objects.equals(m_ipAddress, psSite.m_ipAddress)
        && Objects.equals(m_port, psSite.m_port)
        && Objects.equals(m_userId, psSite.m_userId)
        && Objects.equals(m_password, psSite.m_password)
        && Objects.equals(m_state, psSite.m_state)
        && Objects.equals(m_folderRoot, psSite.m_folderRoot)
        && Objects.equals(m_navTheme, psSite.m_navTheme)
        && Objects.equals(m_globalTemplate, psSite.m_globalTemplate);
  }
  ...
  public void fromXml(Element source, IPSDocument parent, List<IPSComponent> parentComponents)
    ...
    if (m_state == null || m_state.trim().length() == 0) m_state = String.valueOf(STATE_ACTIVE);
  ...
  public Element toXml(Document doc) {
    ...
    if (m_description != null) root.setAttribute(DESCRIPTION_ATTR, m_description);
    if (m_baseUrl != null) root.setAttribute(BASEURL_ATTR, m_baseUrl);
    if (m_ipAddress != null) root.setAttribute(IPADDRESS_ATTR, m_ipAddress);
    if (m_userId != null) root.setAttribute(USERID_ATTR, m_userId);
    if (m_password != null) root.setAttribute(PASSWORD_ATTR, m_password);
    if (m_state != null) root.setAttribute(STATE_ATTR, m_state);
    if (m_folderRoot != null) root.setAttribute(FOLDERROOT_ATTR, m_folderRoot);
    if (m_navTheme != null) root.setAttribute(NAVTHEME_ATTR, m_navTheme);
    if (m_globalTemplate != null) root.setAttribute(GLOBALTEMPLATE_ATTR, m_globalTemplate);
  ...
  public String getGlobalTemplateName() {
    if (m_globalTemplate != null) {
      ...
      return pos != -1 ? m_globalTemplate.substring(0, pos) : m_globalTemplate;
    }
