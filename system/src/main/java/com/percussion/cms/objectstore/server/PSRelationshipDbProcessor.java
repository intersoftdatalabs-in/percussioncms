...
public final class PSRelationshipDbProcessor {
  ...
  public List<PSRelationship> getRelationshipList(PSRelationshipFilter filter)
    ...
    int doNotFilter =
        filter.isCommunityFilteringEnabled() ? 0 : PSRelationshipConfig.FILTER_TYPE_COMMUNITY;
  ...
  private List<PSRelationship> getRelationshipList(
      PSRelationshipFilter filter, int doNotApplyFilters) throws PSCmsException {
    if ((!filter.isCommunityFilteringEnabled()) && doNotApplyFilters == 0)
      throw new IllegalArgumentException(
          "Community filter is disabled, but doNotApplyFilters == 0");
    ...
    try {
      ...
      IPSFolderRelationshipCache folderCache = canProcessedByFolderCache(filter, params);

      String useAaRelationshipCacheString =
          StringUtils.defaultString(
              (String) PSServer.getServerProps().get("useAaRelationshipCache"), "true");
      ...
      if (folderCache != null) {
        ...
        IPSFolderRelationshipCache assemblyCache = canProcessedByAssemblyCache(filter, params);
      ...
      if (testResult != null) {
        ...
        if (result != null && result.size() != testResult.size()) {
          log.error(
              "AA cache returned {} service returned {} items", testResult.size(), result.size());
      ...
      if (result != null && !result.isEmpty()) {
        ...
        if ((doNotApplyFilters & PSRelationshipConfig.FILTER_TYPE_COMMUNITY)
          ...
          if ((doNotApplyFilters & commFilterFlags) == commFilterFlags) doCommFilter = false;
        }

        int filterFlags = 0;
        if (doCommFilter && communityId != -1) {
          if ((doNotApplyFilters & PSRelationshipConfig.FILTER_TYPE_FOLDER_COMMUNITY)
              == PSRelationshipConfig.FILTER_TYPE_FOLDER_COMMUNITY) {
            filterFlags |= FILTER_ITEM;
          }
          if ((doNotApplyFilters & PSRelationshipConfig.FILTER_TYPE_ITEM_COMMUNITY)
              == PSRelationshipConfig.FILTER_TYPE_ITEM_COMMUNITY) {
            filterFlags |= FILTER_FOLDER;
          }

          if (filterFlags == 0) filterFlags |= (FILTER_FOLDER | FILTER_ITEM);

          result = filterByCommunity(result, communityId, filterFlags);
        }

        // only skip if user is rxserver user
        if (!((doNotApplyFilters & PSRelationshipConfig.FILTER_TYPE_FOLDER_PERMISSIONS)
        ...
        if (StringUtils.isNotEmpty(filter.getName())
            && StringUtils.isNotEmpty(filter.getCategory())) {
  ...
  private List<PSRelationship> getRelationshipsFromAaCache(
      IPSFolderRelationshipCache relCache,
      PSRelationshipFilter filter,
      QueryRelationshipCriteria params)
      throws PSCmsException {
    ...
    if (result != null && (crossSite || filterNames)) {
      rels.removeIf(
          rel ->
              (crossSite && (rel.getLegacySiteId() == null && rel.getLegacyFolderId() == null))
    ...
    } else if (result == null && rels != null) // need to filter the relationships
    {
      ...
      for (PSRelationship rel : rels) {
        if (filterRelNames != null
            && !filterRelNames.isEmpty()
            && !filterRelNames.contains(rel.getConfig().getName())) continue;
        ...
        if (params.m_relationshipId != -1 && params.m_relationshipId != rel.getId()) continue;

        // testing owner
        if (params.m_ownerId != -1 && params.m_ownerId != ownerId) continue;

        // testing dependent(s)
        if (params.m_dependentIds != null) {
          if (!params.m_dependentIds.contains(dependentId)) continue;
        }

        if (params.m_dependentContentTypeIds != null
            && (!params.m_dependentContentTypeIds.contains((long) dependent.getContentTypeId())))
          continue;

        if (params.m_dependentObjectType != -1
            && params.m_dependentObjectType != dependent.getObjectType()) continue;
  ...
  private void setExtraDependentProperties(Collection<PSRelationship> rels) throws PSCmsException {
    ...
    if (folderCache == null) {
      ...
      for (PSRelationship rel : rels) locators.add(rel.getDependent());

      PSServerFolderProcessor folderProcessor = PSServerFolderProcessor.getInstance();
      PSComponentSummaries sums =
          folderProcessor.getComponentSummaries(locators.iterator(), null, false);
  ...
  public static PSRelationshipFilter getFilterFromParameters(Map<String, Object> params) {
    ...
    if (rid != -1) filter.setRelationshipId(rid);

    String name = (String) params.get(IPSHtmlParameters.SYS_RELATIONSHIPTYPE);
    if (name != null && name.trim().length() > 0) filter.setName(name);
    ...
    if (dependentId != -1) filter.setDependent(new PSLocator(dependentId, -1));

    int slotId = getParameterInt(IPSHtmlParameters.SYS_SLOTID, params);
    if (slotId != -1) filter.setProperty(IPSHtmlParameters.SYS_SLOTID, slotId + "");

    String pname = (String) params.get("sys_propname");
    String pvalue = (String) params.get("sys_propvalue");
    if (pname != null && pvalue != null) filter.setProperty(pname, pvalue);
    ...
    if (ownerJoin != null && ownerJoin.equals(OWNER_JOINED)) {
      ...
      if (ctId != -1) filter.setOwnerContentTypeId(ctId);

      int objType = getParameterInt(IPSHtmlParameters.SYS_OBJECTTYPE, params);
      if (objType != -1) filter.setOwnerObjectType(objType);
    } else {
      int ctId = getParameterInt(IPSHtmlParameters.SYS_CONTENTTYPEID, params);
      if (ctId != -1) filter.setDependentContentTypeId(ctId);

      int objType = getParameterInt(IPSHtmlParameters.SYS_OBJECTTYPE, params);
      if (objType != -1) filter.setDependentObjectType(objType);
    }
  ...
  public PSRelationshipSet queryRelationships(
      PSRelationshipConfig type, PSLocator owner, int doNotApplyFilters) throws PSCmsException {
   ...
   */
  public PSRelationshipSet queryRelationships(
      PSRelationshipConfig type, PSLocator locator, boolean lookupDependents, int doNotApplyFilters)
      throws PSCmsException {
    ...
    if (lookupDependents) {
      if (type.useDependentRevision() && hasRevision) filter.setDependent(locator);
      else filter.setDependentId(locator.getId());
    } else {
      if (type.useOwnerRevision() && hasRevision) {
        filter.setOwner(locator);
        filter.limitToOwnerRevision(true);
      } else filter.setOwnerId(locator.getId());
    }
    List<PSRelationship> result = getRelationshipList(filter, doNotApplyFilters);
    PSRelationshipSet resultSet = new PSRelationshipSet(result.size());
    resultSet.addAll(result);
    return resultSet;
  }
  ...
  private boolean notFilterByCommunity(int doNotApplyFilters) {
    ...
    return (notByItemCom && notByFolderCom) || notByCom;
  }
  ...
  private List<PSRelationship> getRelationshipsFromFolderCache(
    ...
    if (params.m_ownerContentTypeId != -1
        && params.m_ownerContentTypeId != PSFolder.FOLDER_CONTENT_TYPE_ID) return rels;

    // owner must be folder object type if specified
    if (params.m_ownerObjectType != -1
        && params.m_ownerObjectType != PSComponentSummary.TYPE_FOLDER) return rels;
  ...
  private IPSFolderRelationshipCache canProcessedByFolderCache(
    ...
    if (filter.getOwner() != null) criteria.m_ownerId = filter.getOwner().getId();

    if (filter.getOwnerContentTypeId() != -1)
      criteria.m_ownerContentTypeId = filter.getOwnerContentTypeId();

    if (filter.getOwnerObjectType() != -1) criteria.m_ownerObjectType = filter.getOwnerObjectType();

    if (filter.getDependent() != null) {
      List<Integer> ids = new ArrayList<>();
      for (PSLocator loc : filter.getDependents()) ids.add(loc.getId());
    ...
    if ((criteria.m_relationshipId == -1)
        && (criteria.m_ownerId == -1)
        && (criteria.m_dependentIds == null)) {
  ...
  private IPSFolderRelationshipCache canProcessedByAssemblyCache(
    ...
    if (!(StringUtils.equals(filter.getName(), PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY)
        || StringUtils.equals(filter.getCategory(), PSRelationshipConfig.CATEGORY_ACTIVE_ASSEMBLY)))
      return null;
    ...
    if (owner != null) {
      ...
      if (item.getObjectType() != PSCmsObject.TYPE_FOLDER
          && (item.getCurrentRevision() != revision
              && item.getTipRevision() != revision
              && item.getPublicRevision() != revision)) return null;
    }
    ...
    if (filter.getDependent() != null) {
      ...
      if (!(publicRev || tip || current)) {
        log.debug(
            "Using dependent and not specifying owner limit will not use Aa Cache : {}", filter);
        return null;
      }
    }
  ...
  private List<PSRelationship> filterByCommunity(
    ...
    List<PSRelationship> filteredSet = new ArrayList<>(rels.size());
    for (PSRelationship rel : rels) {
      ...
      if (rel.getConfig().useCommunityFilter()
        ...
        if ((filterItems && rel.getDependentObjectType() == PSCmsObject.TYPE_ITEM)
            || (filterFolders && rel.getDependentObjectType() == PSCmsObject.TYPE_FOLDER)) {
          continue;
        }
      }
      filteredSet.add(rel);
  ...
  private List<PSRelationship> filterByFolderPermissions(List<PSRelationship> rels)
    ...
    List<PSRelationship> filteredSet = new ArrayList<>(rels.size());
    for (PSRelationship rel : rels) {
      if (rel.getConfig().getName().equalsIgnoreCase(PSRelationshipConfig.TYPE_FOLDER_CONTENT)) {
        if (!checkFolderPermission(rel, PSObjectPermissions.ACCESS_READ, false)) {
          continue;
        }
      }
      filteredSet.add(rel);
  ...
  private boolean checkFolderPermission(PSRelationship rel, int accessLevel, boolean throwException)
      throws PSCmsException {
    if (!thread_context && m_context == null) return true;

    if (rel.getDependentObjectType() != PSCmsObject.TYPE_FOLDER) return true;

    PSRelationshipConfig type = rel.getConfig();
    String configType = type.getName();
    if (!configType.equalsIgnoreCase(PSRelationshipConfig.TYPE_FOLDER_CONTENT)) return true;
  ...
  public void modifyRelationships(PSRelationshipSet relationships) throws PSCmsException {
    ...
    if (!relationships.isEmpty()) processModifyPlan(createModifyPlan(relationships));
  }
   ...
   * @throws PSCmsException if anything goes wrong processing the request.
   */
  public void deleteRelationship(PSRelationship relationship) throws PSCmsException {
    if (relationship == null) throw new IllegalArgumentException("relationship cannot be null");
  ...
  public void deleteRelationships(PSRelationshipSet relationships) throws PSCmsException {
    ...
    if (!relationships.isEmpty() && log.isDebugEnabled()) {
      ...
      Exception e =
          new Exception(
              "The following trace is for tracking what code performed the deletions, no exception"
                  + " actually occurred.");
      log.debug(
          "The following {}  relationship(s) have been deleted: {} \n",
          relationships.size(),
          buf.toString(),
          e);
    }
    ...
    try {
      if (!relationships.isEmpty()) {
        ...
        for (PSRelationship relationship : (Iterable<PSRelationship>) relationships) {
          ...
          effectProc = new PSRelationshipEffectProcessor(rels, data, execContext);
          effectProc.process();
  ...
  private void updateCaches(ActionEnum action, PSRelationshipSet relationships) {
    ...
    PSRelationshipChangeEvent event =
        new PSRelationshipChangeEvent(action.ordinalValue(), relationships);
    PSNotificationEvent notifyEvent =
        new PSNotificationEvent(EventType.RELATIONSHIP_CHANGED, event);
    ...
    if (!(handler instanceof PSAssemblerCacheHandler)) return;
  ...
  private Map<String, PSRelationshipSet> createModifyPlan(PSRelationshipSet relationships)
    ...
    for (PSRelationship psRelationship : (Iterable<PSRelationship>) relationships) {
      ...
      if (!relsExisting.contains(relationship.getId())) {
        ...
        PSRelationship existingRel =
            relationshipProcessor.checkIfRelationshipAlreadyExists(relationship);
        if (existingRel != null) {
        ...
        if (!relationship.isPersisted()) {
          relationship.setPersisted(true);
          log.debug(
              "Oops! persisted-flag=FALSE for an existing relationship id: {} ",
              relationship.getId());
        }
  ...
  private Collection<Integer> getExisting(PSRelationshipSet relationships) {
    ...
    for (PSRelationship relationship : (Iterable<PSRelationship>) relationships) {
      if (relationship.getId() > 0) relIds.add(relationship.getId());
    }
  ...
  private int getCommunity() {
    ...
    try {
      String userName = req.getUserContextInformation("User/Name", "").toString();
    ...
    int communityId = 1;
    Object obj = req.getSessionPrivateObject(IPSHtmlParameters.SYS_COMMUNITY);
    if (obj != null) {
      try {
  ...
  public List<PSLocator> getOwnerLocators(PSLocator object, String relationshipTypeName)
    ...
    if (relationshipTypeName == null || relationshipTypeName.length() < 1)
      throw new IllegalArgumentException("relationshipTypeName must not be null or empty");

    IPSFolderRelationshipCache cache = getFolderCache(relationshipTypeName);
    if (cache != null
        && (relationshipTypeName.equalsIgnoreCase(PSRelationshipConfig.TYPE_FOLDER_CONTENT)
  ...
  private List<PSLocator> getOwnerLocatorsFromRepository(
      PSLocator object, String relationshipTypeName) throws PSCmsException {
    List<PSRelationship> rels = getRelationshipsFromDependent(object, relationshipTypeName);
    List<PSLocator> owners = new ArrayList<>();

    for (PSRelationship rel : rels) {
      getAncestorLocatorsFromRepository(rel.getOwner(), owners, relationshipTypeName);
    }
    return owners;
  }
  ...
  public int getIdByPath(String path, String relationshipTypeName) throws PSCmsException {
    if (!(relationshipTypeName.equalsIgnoreCase(PSRelationshipConfig.TYPE_FOLDER_CONTENT)
      ...
      throw new UnsupportedOperationException(
          "Relationship, " + relationshipTypeName + ", is not supported by getIdByPath()");

    if (StringUtils.isBlank(path)) return -1;

    if (path.equals("/")) return PSFolder.ROOT_ID; // root path

    final String start = "//";
    final String separator = "/";
    if (!path.startsWith(start))
      throw new IllegalArgumentException("Path: " + path + " - must start with '" + start + "'.");
  ...
  private int getChildIdFromName(int parentId, String childName) throws PSCmsException {
    ...
    List<PSRelationship> rels =
        getRelationshipList(filter, PSRelationshipConfig.FILTER_TYPE_COMMUNITY);

    for (PSRelationship rel : rels) {
      PSComponentSummary summary = cms.loadComponentSummary(rel.getDependent().getId());

      if (summary.getName().equalsIgnoreCase(childName)) return rel.getDependent().getId();
    }
  ...
  public String[] getOwnerPaths(PSLocator object, String relationshipTypeName)
    ...
    if (!PSRelationshipConfig.TYPE_FOLDER_CONTENT.equalsIgnoreCase(relationshipTypeName)
        && !PSRelationshipConfig.TYPE_RECYCLED_CONTENT.equalsIgnoreCase(relationshipTypeName))
      throw new UnsupportedOperationException(
          "getOwnerPaths for relationship '" + relationshipTypeName + "' is not supported.");

    IPSFolderRelationshipCache cache = getFolderCache(relationshipTypeName);
    if (cache != null) return cache.getParentPaths(object, relationshipTypeName);
    else return getOwnerPathsFromRepository(object);
  }
  ...
  private List<PSRelationship> getRelationshipsFromDependent(
      PSLocator dependent, String relationshipTypeName) throws PSCmsException {
    PSRelationshipFilter filter = new PSRelationshipFilter();
    filter.setDependent(dependent);
    filter.setName(relationshipTypeName);
    filter.setCommunityFiltering(false);

    return getRelationshipList(filter, PSRelationshipConfig.FILTER_TYPE_COMMUNITY);
  }
  ...
  private String[] getOwnerPathsFromRepository(PSLocator object)
    ...
    List<PSRelationship> rels =
        getRelationshipsFromDependent(object, PSRelationshipConfig.TYPE_FOLDER_CONTENT);
    ...
    for (int i = 0; i < paths.length; i++) {
      ...
      getAncestorLocatorsFromRepository(parent, parents, PSRelationshipConfig.TYPE_FOLDER_CONTENT);
  ...
  private void getAncestorLocatorsFromRepository(
      PSLocator dependent, List<PSLocator> locators, String relationshipTypeName)
      throws PSCmsException {
    locators.add(dependent);

    List<PSRelationship> rels = getRelationshipsFromDependent(dependent, relationshipTypeName);

    for (PSRelationship rel : rels) {
      PSLocator plocator = rel.getOwner();
      getAncestorLocatorsFromRepository(plocator, locators, relationshipTypeName);
    }
  }
