...
public abstract class PSObjectPermissions implements java.io.Serializable {
  ...
  protected void init(PSObjectAcl objectAcl)
    ...
    if (objectAcl == null) throw new IllegalArgumentException("objectAcl may not be null");
    m_objectAcl = objectAcl;

    if (!processAcl()) return;
    ...
    if (processLastAclEntry) processAclEntry(null);
  }
   ...
   * @throws PSCmsException
   */
  protected abstract boolean processAcl()
      throws PSAuthenticationRequiredException, PSAuthorizationException, PSCmsException;
   ...
   * @throws PSAuthenticationRequiredException
   */
  protected abstract boolean processAclEntry(PSObjectAclEntry aclEntry)
  ...
  public boolean equals(Object obj) {
    ...
    boolean equals = true;
    if (!(obj instanceof PSObjectPermissions)) equals = false;
    else {
      PSObjectPermissions other = (PSObjectPermissions) obj;

      if (m_permissions != other.m_permissions) equals = false;
    }
    return equals;
  }
  ...
  public static int translateAccess(String accessLevel) {
    if ((accessLevel == null) || (accessLevel.trim().length() < 1))
      throw new IllegalArgumentException("accessLevel may not be null or empty");

    int iAccess;
    if (accessLevel.equalsIgnoreCase(ACCESS_DENY_STR)) iAccess = ACCESS_DENY;
    else if (accessLevel.equalsIgnoreCase(ACCESS_READ_STR)) iAccess = ACCESS_READ;
    else if (accessLevel.equalsIgnoreCase(ACCESS_WRITE_STR)) iAccess = ACCESS_WRITE;
    else if (accessLevel.equalsIgnoreCase(ACCESS_ADMIN_STR)) iAccess = ACCESS_ADMIN;
    else if (accessLevel.equalsIgnoreCase(ACCESS_SERVER_ADMIN_STR)) iAccess = ACCESS_SERVER_ADMIN;
    else throw new IllegalArgumentException("Invalid access level");
