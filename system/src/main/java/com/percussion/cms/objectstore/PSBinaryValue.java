...
public class PSBinaryValue extends PSFieldValue {
  ...
  public Object clone() {
    ...
    if (m_locator != null) copy.m_locator = (IPSBinaryLocator) m_locator.clone();
  ...
  public boolean equals(Object obj) {
    ...
    if (obj == null || !(getClass().isInstance(obj))) isEqual = false;
    else {
      PSBinaryValue comp = (PSBinaryValue) obj;
      if (!compare(m_locator, comp.m_locator)) isEqual = false;
      else if (m_isDataLoaded != comp.m_isDataLoaded) isEqual = false;
      else if (!compare(m_data, comp.m_data)) isEqual = false;
    }
  ...
  private void applyDataStream(InputStream content) throws IOException {
    ...
    try {
      ByteArrayOutputStream out = new ByteArrayOutputStream(content.available());
    ...
    if (m_locator != null) {
      try {
        ...
        throw new RuntimeException(
            "Failed to load binary data from the server: " + e.getLocalizedMessage());
      }
    }
