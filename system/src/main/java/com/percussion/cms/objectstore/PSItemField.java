...
public class PSItemField extends PSItemComponent {
  ...
  public void clearValue(IPSFieldValue fieldValue) {
    if (fieldValue == null) throw new IllegalArgumentException("fieldValue must not be null");
        ...
        case PSItemFieldMeta.DATATYPE_TEXT:
          if ((!(content instanceof PSTextValue)
              ...
              && !(m_fieldMeta
                  .getFieldDef()
                  .getSubmitName()
                  .endsWith(IPSHashFileInfoExtension.HASH_PARAM_SUFFIX))))
  ...
  public IPSFieldValue createFieldValue(String stringValue) {
    ...
    if (m_fieldMeta.isBinary()) {
      try {
        ...
        ByteArrayInputStream iBuf =
            new ByteArrayInputStream(stringValue.getBytes(PSCharSets.rxJavaEnc()));
    ...
    // numeric must be PSTextValue (no need to wrap an integer or float):
    else if (m_fieldMeta.getBackendDataType() == PSItemFieldMeta.DATATYPE_NUMERIC) {
      fieldValue = new PSTextValue(stringValue);
    }
  ...
  public IPSFieldValue getValue() {
    int i = 0;
    if (isMultiValue()) i = m_fieldValues.size() - 1;

    if (i < 0 || i >= m_fieldValues.size()) return null;
  ...
  protected Element toXml(Document doc, PSAcceptElements acceptElements) {
    ...
    if (!m_fieldValues.isEmpty()) {
      ...
      if (getItemFieldMeta().isBinary()
          && acceptElements != null
          && !acceptElements.includeBinary()) return root;
  ...
  public boolean equals(Object obj) {
    ...
    if (!compare(m_fieldMeta, comp.m_fieldMeta)) return false;
    if (!compare(m_fieldValueHref, comp.m_fieldValueHref)) return false;
    if (!compare(m_fieldValues, comp.m_fieldValues)) return false;
  ...
  public void loadXmlData(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    Element fieldMeta =
        PSXMLDomUtil.getFirstElementChild(sourceNode, PSItemFieldMeta.EL_FIELD_META);
