...
public class PSWebServiceAgent {
   ...
   */
  private Element getOperationElement(Document doc, String operationName, String wsdlPortName) {
   ...
   */
  public Body getSoapBodyForParams(String operationName, String wsdlPortName, Element params) {
   ...
   * @param responseElName The expected element name from the responsed msg. It may not be <code>
   *     null</code> or empty.
   ...
   * @param respElName The expected XML node name from the responsed envelope. It may not be <code>
   *     null</code> or empty.
   ...
   * @return <code>true</code> if the response element is caused by an expired session-id; <code>
   *     false</code> otherwise.
   */
  private boolean isSessionExpireResponse(Element responseEl) {
    boolean isSessionExpire = false;

    if ((m_sessionId != null) && (responseEl.getNodeName().equals("SOAP-ENV:Fault"))) {
      PSXmlTreeWalker tree = new PSXmlTreeWalker(responseEl);
      Element faultCodeEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      if (faultCodeEl != null && faultCodeEl.getNodeName().equals("faultcode")) {
        String faultCode = tree.getElementData(faultCodeEl);

        Element faultStringEl = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
        if ((faultStringEl != null) && faultStringEl.getNodeName().equals("faultstring")) {
   ...
   */
  private Element doSendEnvelope(Envelope msgEnv) throws PSCmsException {
    ...
    try {
      ...
      Envelope respEnv = Envelope.unmarshall(respDoc.getDocumentElement(), respContext);
  ...
  /**
   * The protocol of the web service server, initialized by constructor, it is either <code>
   * PROTOCOL_HTTP</code> or <code>PROTOCOL_HTTPS</code> and never modified after that.
   */
  private String m_protocol;

  /**
   * The server name of the web services, initialized by constructor, never modified, <code>null
  ...
  /**
   * The user name of the Rhythmyx server, initialized by constructor, never modified, <code>null
   * </code> or empty after that.
   */
