...
public final class PSSlot extends PSCmsComponent {
  ...
  public void addRelatedItem(PSLocator item, int pos) {
    ...
    if (pos < 0) pos = 0;
    if (pos >= m_relatedItems.size()) m_relatedItems.add(item.clone());
    else m_relatedItems.add(pos, item.clone());
  }
  ...
  public boolean equals(Object o) {
    ...
    if (m_slotId != slot.m_slotId) return false;
    if (!m_name.equalsIgnoreCase(slot.m_name)) return false;
    if (!m_description.equals(slot.m_description)) return false;
    if (!m_relatedItems.equals(slot.m_relatedItems)) return false;
    if (!m_allowedVariants.equals(slot.m_allowedVariants)) return false;
    return true;
  }
  ...
  public int hashCode() {
    return (m_name.toLowerCase() + m_description + m_slotId).hashCode()
        + m_relatedItems.hashCode()
        + m_allowedVariants.hashCode();
  }
