...
public class PSGetFolderPermissions extends PSDefaultExtension
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      if ((params.length < EXPECTED_NUMBER_OF_PARAMS)
        ...
        throw new PSParameterMismatchException(EXPECTED_NUMBER_OF_PARAMS, params.length);
      }
      ...
      if (contentId == -1) {
        ...
        */
        ...
        Element el = walker.getNextElement(elementName, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);

        while (el != null) {
          if (el.getNodeName().equalsIgnoreCase(elementName)) {
            // get contentid
            int contentid2 = convertContentId(el.getAttribute(contentIdAttrName));

            if (contentid2 == -1) {
              throw new PSParameterMismatchException(ms_className + ": Invalid content id: -1");
            }
          ...
          el = walker.getNextElement(elementName, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
        }
  ...
  private void setFolderPermissions(
      IPSRequestContext request, Element element, int contentId, String permAttrName)
      throws PSCmsException {
    ...
    request.printTraceMessage(ms_className + ": Folder Permissions = " + permissions);
