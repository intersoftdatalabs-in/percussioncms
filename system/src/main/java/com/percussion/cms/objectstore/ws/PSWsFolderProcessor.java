...
/**
 * This class handles the folder operations on the client side. It uses webservices to send request
 * to the server and get the result from the server's response.
 */
public class PSWsFolderProcessor extends PSRemoteFolderProcessor {

  /**
   * Construct a folder processor from a WebService agent object. This will allow several webservice
   * clients to share the same object, which is perfered, so that they can share the same session id
   * if there is one.
   *
   * @param wsAgent The WebService agent object, it may not be <code>null</code>.
   */
  public PSWsFolderProcessor(PSWebServiceAgent wsAgent) {
    if (wsAgent == null) throw new IllegalArgumentException("wsAgent may not be null");
  ...
  /**
   * Set the webservice agent object. This object may be shared among all webservices operations in
   * a client
   *
  ...
  protected Element sendMessage(String operation, Element message, String respNodeName)
    ...
    Body msgBody = m_wsAgent.getSoapBodyForParams(operation, XML_NS_FOLDER, message);

    return m_wsAgent.sendSoapBody(msgBody, respNodeName);
  }

  /**
   * It is used to handle webservices related operations. Initialized by the constructor, never
