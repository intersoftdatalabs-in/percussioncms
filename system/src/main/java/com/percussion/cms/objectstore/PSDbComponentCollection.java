 ...
 * would insert, update and delete all slots as necessary to make the database represent the current
 * state of the saved collection.
 *
 * <p>Although it doesn't implement the Collection interface, it is meant to behave like one. In
 * fact, it is more restrictive because it limits the allowed object types to a type specified at
 * creation.
 *
 * <p>It implements many, but not all of the methods of the Collection interface, except it uses
 * appropriate types rather than Object. Collection was not implemented because many of the methods
 * don't make sense in the context of how this collection will be used.
 *
 * @author Paul Howard
 * @version 1.0
 */
public class PSDbComponentCollection extends PSDbComponent {
  /**
   * Convenience method that calls {@link #PSDbComponentCollection(String, String)
   * PSDbComponentCollection(className, null)}.
   */
  public PSDbComponentCollection(String className) throws ClassNotFoundException {
    this(className, null);
  }

  /**
   * Creates an empty collection that limits the objects that can be added to the type specified. If
   * an interface is supplied, an exception is thrown.
   *
  ...
  /**
   * Just like {@link #PSDbComponentCollection(String, String)} except it takes a component class as
   * the parameter.
   *
   ...
   */
  public PSDbComponentCollection(Element src) throws PSUnknownNodeTypeException {
  ...
  /**
   * See {@link IPSDbComponent#toXml(Document) interface} for description. This class is implemented
   * in terms of a PSDbComponentList and returns an element that matches the dtd of the at
   * component.
   */
  @Override
  ...
  /**
   * For use by classes using this class as part of its implementation. Allows the calling class to
   * set the tag name of the element returned by this method.
   *
   * @param doc Never <code>null</code>.
   * @param nodeName The name to use for the element returned by this method. If <code>null</code>
   *     or empty, the node name of this class is used.
  ...
  void fromXml(Element src, String nodeName) throws PSUnknownNodeTypeException {
    if (null == nodeName || nodeName.trim().length() == 0) nodeName = getNodeName();
    if (m_list == null) m_list = new PSDbComponentList(src, nodeName);
    else m_list.fromXml(src, nodeName);
  }
   ...
   * delete list when performing the compare. The order of objects within the collection is also
   * irrelevant.
   */
  @Override
  public boolean equals(Object obj) {
    ...
    if (!equalsIgnoreOrder(c.iterator(), iterator())) return false;
   ...
   * delete list when performing the calculation. The hash code is order invariant.
   */
  @Override
  public int hashCode() {
   ...
   * will be as described in {@link IPSDbComponent#clone()}.
   */
  @Override
  public Object clone() {
   ...
   *
   * @param comp The datatype must be the same as the type of this list or a ClassCastException will
   *     be thrown.
   * @throws ClassCastException if the supplied component doesn't have the same type as the type
   *     supplied during construction.
   * @throws IllegalStateException If the type of the supplied component doesn't match the type
  ...
  /**
   * Checks whether the components in thisIter and otherIter are equal ignoring sequencing.
   *
   * @param thisIter Assumed not <code>null</code> and that no entries are <code>null</code> and
   *     that the number of entries in both iterators is equal.
   * @param otherIter Assumed not <code>null</code> and that no entries are <code>null</code>.
  ...
  /**
   * Calculates the hashcode of all components in the supplied iterator such that the same value
   * will be returned regardless of the order of the comps in the iterator. This is done by adding
   * all the hashcodes of the comps. A constant value is added for <code>null</code> entries.
   *
  ...
  static int hashCodeIgnoresOrder(Iterator<?> comps) {
    ...
    while (comps.hasNext()) {
      Object o = comps.next();
      if (null == o) hash += 31; // arbitrary, non-zero value
      else hash += o.hashCode();
    }
    return hash;
  }
  ...
  /**
   * Proxy implementation of sequenced list also representing the data store for this unordered
   * collection of objects. Initialized in ctor(s), never <code>null</code> after that.
   */
