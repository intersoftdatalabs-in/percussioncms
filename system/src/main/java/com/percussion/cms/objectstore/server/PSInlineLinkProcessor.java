...
public class PSInlineLinkProcessor {
  ...
  private static List<PSInlineLinkField> getInlineLinkFields(
    ...
    if (inlineFields == null) inlineFields = new ArrayList<>();

    Iterator<PSComponent> fields = fieldSet.getAll();
    while (fields.hasNext()) {
      PSComponent testFieldSet = fields.next();
      if (testFieldSet instanceof PSFieldSet) {
        getInlineLinkFields((PSFieldSet) testFieldSet, inlineFields);
      } else {
        PSField field = (PSField) testFieldSet;
        if (field.mayHaveInlineLinks()) inlineFields.add(new PSInlineLinkField(field));
      }
    }
    ...
    try {
      ...
      if (bCheckout) {
        request.setParameter(
            IPSHtmlParameters.SYS_CHECKOUT_SAME_REVISION, IPSConstants.BOOLEAN_TRUE);
      ...
      while (fields.hasNext()) {
        ...
        } else processInlineLinkField(itemField, relationshipMap);
      }
       ...
       */
      try {
        request.setParameter(IPSHtmlParameters.SYS_INLINELINK_DATA_UPDATE, "yes");
        item.save(request, -1);
      } finally {
        request.removeParameter(IPSHtmlParameters.SYS_INLINELINK_DATA_UPDATE);
      }
    } finally {
      if (bCheckout) request.removeParameter(IPSHtmlParameters.SYS_CHECKOUT_SAME_REVISION);
      if (checkin) ws.executeCheckInOut(request, IPSConstants.TRIGGER_CHECKIN);
    }
  }
  ...
  private static void processInlineLinkField(PSItemField itemField, Map<?, ?> relationshipMap)
    ...
    try {
      ...
      PSInlineLinkField.modifyField(fieldDoc.getDocumentElement(), relationshipMap);
      ...
      try {
        ...
        logger.warn(
            "IOException occurred while writing the inline link field {} ERROR: {}.",
            itemField.getName(),
      ...
      logger.error(
          "Error happened while parsing Field : {}, the inline link field value ERROR: {}.",
          itemField.getName(),
