...
public class PSAuthTypes implements Observer {
  ...
  public static synchronized PSAuthTypes getInstance() {
    if (ms_this == null) ms_this = new PSAuthTypes();
    return ms_this;
  }
  ...
  public String getResourceForAuthtype(String authType) {
    if (authType == null || authType.length() == 0) {
      throw new IllegalArgumentException("authType must not be null or empty");
    }
   ...
   */
  public void update(
      @SuppressWarnings("unused") Observable o, @SuppressWarnings("unused") Object arg) {
    try {
      loadProperties(ms_configFile);
    } catch (IOException e) {
      handleException(e);
