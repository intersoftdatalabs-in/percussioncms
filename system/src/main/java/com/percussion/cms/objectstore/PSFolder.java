...
public final class PSFolder extends PSDbComponent implements java.io.Serializable {
  ...
  public PSFolder(String name, int id, int communityId, int permissions, String description) {
    ...
    if (id <= 0) throw new IllegalArgumentException("id may not be <= 0");
  }
  ...
  public String getPubFileName() {
    ...
    if (prop != null) {
      ...
      if (pubFileName.length() != 0) return pubFileName;
      else return getName();
    } else {
      return getName();
    }
  ...
  private void init(String name, int communityId, int permissions, String description) {
    ...
    if (description != null) m_description = description;
    else m_description = "";
  ...
  public void setLocale(String locale) {
    if (locale == null || locale.length() < 0) {
      throw new IllegalArgumentException("locale for the folder cannot be null or empty");
    }
    m_locale = locale;
  }
  ...
  public boolean isPublishOnlyInSpecialEdition() {
    ...
    if (property != null) return property.getValue().equalsIgnoreCase(Boolean.TRUE.toString());
  ...
  public void setPublishOnlyInSpecialEdition(boolean isFolderPublishProperty) {
    ...
    else setProperty(IPSConstants.SYS_PUBLISH_FOLDER_WITH_SITE, Boolean.TRUE.toString());
  }
  ...
  public static boolean isFolderPublishProperty(PSFolderProperty property) {
    ...
    return (property.getName().equalsIgnoreCase(IPSConstants.SYS_PUBLISH_FOLDER_WITH_SITE));
  }
  ...
  public int getDisplayFormatId() {
    ...
    if (StringUtils.isBlank(idString)) return -1;
  ...
  public void setGlobalTemplateProperty(String value) {
    ...
    else setProperty(PROPERTY_GLOBALTEMPLATE, value.trim());
  }
  ...
  public void setAcl(PSObjectAcl acl) {
    if (acl == null) throw new IllegalArgumentException("acl may not be null");
    m_acl = acl;
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSFolder)) return false;
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, XML_NODE_DESCRIPTION, m_description);

    PSXmlDocumentBuilder.addElement(doc, root, XML_NODE_LOCALE, m_locale);

    if (m_properties.size() > 0) root.appendChild(m_properties.toXml(doc));
  ...
  private void loadFieldsFromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    int permissions = PSXMLDomUtil.checkAttributeInt(sourceNode, XML_ATTR_PERMISSIONS, true);
    m_permissions = new PSFolderPermissions(permissions);
  ...
  public void copyFrom(IPSDbComponent obj) {
    if (!(obj instanceof PSFolder))
      throw new IllegalArgumentException("obj must be an instance of PSFolder");
  ...
  public void setProperty(PSFolderProperty property) {
    if (property == null) throw new IllegalArgumentException("property may not be null");
    setProperty(property.getName(), property.getValue(), property.getDescription());
  }
  ...
  public void addProperty(PSFolderProperty prop) {
    ...
    if (getProperty(prop.getName()) != null) {
      logger.warn("Warning: property \"" + prop.getName() + "\" must be a new one");
    } else {
      m_properties.add(prop);
    }
  }
  ...
  public String getPropertyDesciption(String name) {
    PSFolderProperty prop = getProperty(name);
    if (prop == null) return null;
    else return prop.getDescription();
  }
  ...
  public void deleteProperty(String name) {
    PSFolderProperty prop = getProperty(name);
    if (prop != null) m_properties.remove(prop);
  }
  ...
  public void setDisplayFormatName(String displayFormatName) {
    if (StringUtils.isBlank(displayFormatName))
      throw new IllegalArgumentException("displayFormatName must not be null or empty.");
  ...
  private static final String XML_NODE_LOCALE = "Locale";
}
