...
public class PSActionProperties extends PSDbComponentCollection {
  ...
  /** Convenience method that calls {@link #getProperty(String,String) getProperty(name, null)}. */
  public String getProperty(String name) {
    return getProperty(name, null);
  }

  /**
   * Gets the value of the specified property. Uses case-insensitive comparison to get the property.
   ...
   * @param defaultValue Returned if the property does not exist in the list, may be <code>null
   *     </code> or empty.
   ...
   * be replaced. The property name is compared case-insensitive.
   *
   * @param name name of the property, may not be <code>null</code> or empty.
   * @param value value of the property, may be <code>null</code> or empty. If <code>null</code>
   *     supplied, "" is stored.
   */
  public void setProperty(String name, String value) {
    if (null == value) value = "";
    PSActionProperty prop = getPropertyObject(name);
    if (null == prop) super.add(new PSActionProperty(name, value));
    else prop.setValue(value);
  }

  /**
   * This method is overridden to guarantee that the names of all members forms a set.
   *
  ...
  public void add(IPSDbComponent comp) {
    if (null == comp || !(comp instanceof PSActionProperty)) {
      throw new IllegalArgumentException("Can only add PSActionProperty components");
    }

    PSActionProperty newProp = (PSActionProperty) comp;
    PSActionProperty prop = getPropertyObject(newProp.getName());
    if (null != prop) prop.setValue(newProp.getValue());
    else super.add(comp);
  }
