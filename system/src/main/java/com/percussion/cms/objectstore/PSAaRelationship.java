...
public final class PSAaRelationship extends PSRelationship {
  ...
  public PSAaRelationship(
      PSRelationship relationship, IPSTemplateSlot slot, IPSAssemblyTemplate template) {
    this(relationship, new PSSlotType(slot), new PSContentTypeTemplate(template));
  }
  ...
  @SuppressWarnings("deprecation")
  public PSAaRelationship(
      PSLocator owner, PSLocator dependent, IPSTemplateSlot slot, IPSAssemblyTemplate template) {
    super(-1, owner, dependent);

    if (slot == null) throw new IllegalArgumentException("slot may not be null");
    if (template == null) throw new IllegalArgumentException("template may not be null");

    PSRelationshipConfig config =
        PSRelationshipCommandHandler.getRelationshipConfig(slot.getRelationshipName());
    setConfig(config);
  ...
  public PSAaRelationship(PSRelationship source) {
    ...
    if (StringUtils.isBlank(slotId))
      throw new IllegalArgumentException("sys_slotid property must not be null or empty.");
    String templetId = getProperty(IPSHtmlParameters.SYS_VARIANTID);
    if (StringUtils.isBlank(templetId))
      throw new IllegalArgumentException("sys_variantid property must not be null or empty.");
  }
  ...
  public PSSlotType getSlot() {
    if (m_slot == null) throw new IllegalStateException("The slot object has not been set.");
  ...
  public PSContentTypeTemplate getVariant() {
    if (m_variant == null)
      throw new IllegalStateException("The variant object has not been set yet.");
  ...
  public void setSiteId(IPSGuid siteId) {
    ...
    setProperty(IPSHtmlParameters.SYS_SITEID, String.valueOf(siteId.longValue()));
  }
  ...
  public IPSGuid getSiteId() {
    ...
    if (siteId == null || siteId.trim().length() == 0) {
      ...
      if (id > 0) return new PSGuid(PSTypeEnum.SITE, id);
      else return null;
    }
  }
  ...
  public List<PSRelationshipPropertyData> getAllAaUserProperties() {
    List<PSRelationshipPropertyData> result = new ArrayList<>(super.getAllUserProperties().size());
    for (PSRelationshipPropertyData prop : super.getAllUserProperties()) {
      if (!isKnownProperty(prop.getName())) result.add(prop);
    }
    return result;
  }
  ...
  private boolean isKnownProperty(String pname) {
    for (String name : ms_knownUserProps) {
      if (name.equalsIgnoreCase(pname)) return true;
    }
