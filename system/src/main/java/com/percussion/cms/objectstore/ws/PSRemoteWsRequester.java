...
/**
 * This class is used to send request to the webservice handle on the remote Rhythmyx Server. The
 * current client can be either applet or application.
 */
public class PSRemoteWsRequester {
  ...
  /**
   * Convenient method, it calls {@link sendRequest(String, String, Element, Map, String)
   * sendRequest(action,wsdlPort,message,null,responseNodeName)}
   */
  public Element sendRequest(
      String action, String wsdlPort, Element message, String responseNodeName)
   ...
   *     empty.
   * @param wsdlPort The wsdl port as one of the catagory of the webservices. It may not be <code>
   *     null</code> or empty.
   * @param message The to be send message. It may not be <code>null</code>.
   * @param extraParams The extra or additional parameters for this request. It is a map with both
   *     key and value as <code>String</code> objects. It may be <code>null</code>.
   ...
   */
  public Element sendRequest(
      String action, String wsdlPort, Element message, Map extraParams, String responseNodeName)
      throws PSCmsException {
    ...
    if (extraParams != null) paramsMap.putAll(extraParams);
  ...
  /**
   * The requester used to communicate with Rhythmyx server. Initialized by constructor, never
