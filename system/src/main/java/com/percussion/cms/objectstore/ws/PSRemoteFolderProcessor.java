...
public class PSRemoteFolderProcessor extends PSProcessorCommon
  ...
  public PSComponentSummary[] getChildSummaries(PSLocator folderId) throws PSCmsException {
    return getChildren(PSRelationshipProcessorProxy.RELATIONSHIP_COMPTYPE, folderId);
  }
  ...
  public void removeChildren(PSLocator sourceFolderId, List children, boolean force)
      throws PSCmsException {
    if (children == null) throw new IllegalArgumentException("children cannot be empty");
    ...
    PSXmlDocumentBuilder.addElement(
        doc, params, PARENT_ID_EL, sourceFolderId.getPart(PSLocator.KEY_ID));
    ...
    Element result =
        sendMessage(REMOVE_FOLDERCHILDREN_OPERATION, params, REMOVE_FOLDERCHILDREN_RESPONSE);
  ...
  public void moveChildren(
    ...
    else if (children.size() == 0) return; // nothing to move from
    ...
    PSXmlDocumentBuilder.addElement(
        doc, params, SOURCE_PARENT_ID_EL, sourceFolderId.getPart(PSLocator.KEY_ID));
    params.appendChild(getChildIdsElement(children, doc));
    PSXmlDocumentBuilder.addElement(
        doc, params, TARGET_PARENT_ID_EL, targetFolderId.getPart(PSLocator.KEY_ID));
    PSXmlDocumentBuilder.addElement(doc, params, FORCE, force ? "yes" : "no");

    // send the prepared request parameters to the server, get the response
    Element result =
        sendMessage(MOVE_FOLDERCHILDREN_OPERATION, params, MOVE_FOLDERCHILDREN_RESPONSE);
  ...
  public void moveChildren(PSLocator sourceFolderId, List children, PSLocator targetFolderId)
      throws PSCmsException {
    move(PSRelationshipConfig.TYPE_FOLDER_CONTENT, sourceFolderId, children, targetFolderId);
  }
  ...
  public PSLocator[] getDescendentFolderLocatorsWithoutFilter(PSLocator folderId)
      throws PSCmsException {
    Element msg =
        getSingleFolderIdMsg(GET_DESCENDENTSLOCATORS_WITHOUTFILTER_REQUEST, PARENT_ID_EL, folderId);
    ...
    int len = 0;
    if (nl == null || (len = nl.getLength()) == 0) return new PSLocator[] {};

    try {
      for (int i = 0; i < len; i++) locators.add(new PSLocator((Element) nl.item(i)));
  ...
  public void add(String componentType, String relationshipType, List children, PSKey targetParent)
    ...
    if (!relationshipType.equals(PSRelationshipConfig.TYPE_FOLDER_CONTENT))
      throw new UnsupportedOperationException(
          "relationshipType must be " + PSRelationshipConfig.TYPE_FOLDER_CONTENT);
  ...
  public Element[] load(String componentType, PSKey[] locators) throws PSCmsException {
    ...
    for (int i = 0; i < locators.length; i++) {
      Element msg = getSingleFolderIdMsg(OPEN_FOLDER_REQUEST, FOLDER_ID_EL, locators[i]);

      Element response = sendMessage(OPEN_FOLDER_OPERATION, msg, OPEN_FOLDER_RESPONSE);
  ...
  public PSSaveResults save(IPSDbComponent[] components) throws PSCmsException {
    ...
    PSProcessingStatistics statistics = new PSProcessingStatistics(inserts, updates, 0, 0, 0);
  ...
  public int delete(String componentType, PSKey[] locators) throws PSCmsException {
    // TODO: enhance web services, to handle bulk operation
    for (int i = 0; i < locators.length; i++) {
      Element msg = getSingleFolderIdMsg(DELETE_FOLDER_REQUEST, FOLDER_ID_EL, locators[i]);
      Element result = sendMessage(DELETE_FOLDER_OPERATION, msg, DELETE_FOLDER_RESPONSE);
  ...
  public void add(String relationshipType, List children, PSKey targetParent)
    ...
    PSXmlDocumentBuilder.addElement(
        doc, params, TARGET_PARENT_ID_EL, targetParent.getPart(PSLocator.KEY_ID));
    params.appendChild(getChildIdsElement(children, doc));

    // send the prepared request parameters to the server, get the response
    Element result = sendMessage(ADD_FOLDERCHILDREN_OPERATION, params, ADD_FOLDERCHILDREN_RESPONSE);
  ...
  public void copy(String relationshipType, List children, PSKey parent) throws PSCmsException {
    if (children == null) throw new IllegalArgumentException("children cannot be null");
    else if (children.size() == 0) return; // nothing to copy from
    ...
    PSXmlDocumentBuilder.addElement(
        doc, params, TARGET_PARENT_ID_EL, parent.getPart(PSLocator.KEY_ID));
    params.appendChild(getChildIdsElement(children, doc));
    PSXmlDocumentBuilder.addElement(doc, params, RECURSIVE_EL, XML_TRUE);

    // send the prepared request parameters to the server, get the response
    Element result =
        sendMessage(COPY_FOLDERCHILDREN_OPERATION, params, COPY_FOLDERCHILDREN_RESPONSE);
  ...
  public PSComponentSummary[] getChildren(String type, PSKey parent) throws PSCmsException {
    ...
    Element data = sendMessage(GET_FOLDERCHILDREN_OPERATION, msg, GET_FOLDERCHILDREN_RESPONSE);
  ...
  private Element getChildIdsElement(List childIds, Document doc) {
    ...
    for (int i = 0; i < childIds.size(); i++) {
      ...
      Element node =
          PSXmlDocumentBuilder.addElement(
              doc, childIdsEl, CHILD_ID_EL, key.getPart(PSLocator.KEY_ID));
  ...
  private Element getSingleFolderIdMsg(String requestEl, String idElName, PSKey locator) {
    ...
    Element folderIdEl =
        PSXmlDocumentBuilder.addElement(doc, root, idElName, locator.getPart(PSLocator.KEY_ID));
  ...
  private Element getSourceTargetFolderIdMsg(
    ...
    Element srcFolderIdEl =
        PSXmlDocumentBuilder.addElement(
            doc, root, srcIdElName, srcLocator.getPart(PSLocator.KEY_ID));

    root.appendChild(srcFolderIdEl);

    Element tgtFolderIdEl =
        PSXmlDocumentBuilder.addElement(
            doc, root, tgtIdElName, tgtLocator.getPart(PSLocator.KEY_ID));
  ...
  private void checkResultResponse(String operation, Element rootEl) throws PSCmsException {
    ...
    if (!result.equals("success")) {
      ...
      if (resultNode == null) {
        ...
        throw new PSCmsException(IPSCmsErrors.FOLDER_OPERATION_FAILED, operation);
      } else {
  ...
  public PSComponentSummary[] getParents(String type, String relationshipType, PSKey locator)
    ...
    Element data = sendMessage(GET_PARENTFOLDER_OPERATION, msg, GET_PARENTFOLDER_RESPONSE);
  ...
  public PSComponentSummary getSummaryByPath(
    ...
    Element pathEl = PSXmlDocumentBuilder.addElement(doc, root, PATH_EL, path);
    root.appendChild(pathEl);

    // send the request
    Element data = sendMessage(GET_SUMMARYBYPATH_OPERATION, root, GET_SUMMARYBYPATH_RESPONSE);
    ...
    try {
      ...
      if (!summaryEl.getNodeName().equals(NULL_RESULT)) summary = new PSComponentSummary(summaryEl);
      return summary;
    } catch (PSException e) {
      throw new PSCmsException(e);
    }
  ...
  public String[] getRelationshipOwnerPaths(
    ...
    if (null == relationshipTypeName) {
      throw new IllegalArgumentException("relationshipTypeName cannot be null");
    }
    ...
    PSXmlDocumentBuilder.addElement(doc, params, CHILD_ID_EL, locator.getPart(PSLocator.KEY_ID));

    // send the prepared request parameters to the server, get the response
    Element response = sendMessage(GET_FOLDER_PATH_OPERATION, params, GET_FOLDER_PATH_RESPONSE);
  ...
  public boolean isDescendent(
      ...
      throws PSCmsException {
    throw new UnsupportedOperationException(
        "isDescendent() is not implemented in : " + this.getClass().getName());
  }
  ...
  public PSKey[] getDescendentsLocators(String type, String relationshipType, PSKey parent)
    ...
    Element msg = getSingleFolderIdMsg(GET_DESCENDENTSLOCATORS_REQUEST, PARENT_ID_EL, parent);

    Element data =
        sendMessage(GET_DESCENDENTSLOCATORS_OPERATION, msg, GET_DESCENDENTSLOCATORS_RESPONSE);

    List locators = new ArrayList();
    NodeList nl = data.getElementsByTagName("PSXLocator");
    int len = 0;
    if (nl == null || (len = nl.getLength()) == 0) return new PSKey[] {};
  ...
  public String copyFolder(PSLocator source, PSLocator target, PSCloningOptions options)
    ...
    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    PSCloneSiteFolderRequest request = new PSCloneSiteFolderRequest(source, target, options);

    Element data =
        sendMessage(CLONE_SITEFOLDER_OPERATION, request.toXml(doc), CLONE_SITEFOLDER_RESPONSE);

    Element results = PSXMLDomUtil.getFirstElementChild(data);
    String errorLog = results.getAttribute("errorLog");
    if (errorLog != null && errorLog.trim().length() > 0) return errorLog;
  ...
  public void copyFolderSecurity(PSLocator source, PSLocator target) throws PSCmsException {
    ...
    Element msg =
        getSourceTargetFolderIdMsg(
            COPY_FOLDERSECURITY_REQUEST, SOURCE_FOLDER_ID_EL, TARGET_FOLDER_ID_EL, source, target);

    Element result = sendMessage(COPY_FOLDERSECURITY_OPERATION, msg, COPY_FOLDERSECURITY_RESPONSE);
  ...
  public Set getFolderCommunities(PSLocator source) throws PSCmsException {
    ...
    Element msg = getSingleFolderIdMsg(GET_FOLDERCOMMUNITIES_REQUEST, FOLDER_ID_EL, source);

    Element data =
        sendMessage(GET_FOLDERCOMMUNITIES_OPERATION, msg, GET_FOLDERCOMMUNITIES_RESPONSE);

    Set communities = new HashSet();
    NodeList communityElements =
        PSXMLDomUtil.getFirstElementChild(data).getElementsByTagName("Community");
  ...
  public void purgeFolderAndChildItems(List<PSLocator> items) throws PSCmsException {
    ...
    // send the prepared request parameters to the server, get the response
    Element result =
        sendMessage(PURGE_FOLDERCHILDREN_OPERATION, params, PURGE_FOLDERCHILDREN_RESPONSE);
  ...
  public void purgeFolderAndChildItems(PSLocator sourceFolderId, List<PSLocator> items)
      throws PSCmsException {
    if (items == null) throw new IllegalArgumentException("items cannot be empty");
  ...
  public void purgeFolderNavigation(PSLocator folder) throws PSCmsException {
    if (folder == null) throw new IllegalArgumentException("items cannot be empty");
  ...
  public static final String GET_FOLDER_PATH_RESPONSE = "GetFolderPathsResponse";
  ...
  public static final String ADD_FOLDERCHILDREN_OPERATION = "addFolderChildren";
  public static final String ADD_FOLDERCHILDREN_REQUEST = "AddFolderChildrenRequest";
  public static final String ADD_FOLDERCHILDREN_RESPONSE = "AddFolderChildrenResponse";

  public static final String COPY_FOLDERCHILDREN_OPERATION = "copyFolderChildren";
  public static final String COPY_FOLDERCHILDREN_REQUEST = "CopyFolderChildrenRequest";
  public static final String COPY_FOLDERCHILDREN_RESPONSE = "CopyFolderChildrenResponse";

  public static final String COPY_FOLDERSECURITY_OPERATION = "copyFolderSecurity";
  public static final String COPY_FOLDERSECURITY_REQUEST = "CopyFolderSecurityRequest";
  public static final String COPY_FOLDERSECURITY_RESPONSE = "CopyFolderSecurityResponse";

  public static final String GET_FOLDERCHILDREN_OPERATION = "getFolderChildren";
  public static final String GET_FOLDERCHILDREN_REQUEST = "GetFolderChildrenRequest";
  public static final String GET_FOLDERCHILDREN_RESPONSE = "GetFolderChildrenResponse";

  public static final String GET_FOLDERCOMMUNITIES_OPERATION = "getFolderCommunities";
  public static final String GET_FOLDERCOMMUNITIES_REQUEST = "GetFolderCommunitiesRequest";
  public static final String GET_FOLDERCOMMUNITIES_RESPONSE = "GetFolderCommunitiesResponse";

  public static final String CLONE_SITEFOLDER_OPERATION = "cloneSiteFolder";
  public static final String CLONE_SITEFOLDER_REQUEST = "CloneSiteFolderRequest";
  public static final String CLONE_SITEFOLDER_RESPONSE = "CloneSiteFolderResponse";

  public static final String GET_PARENTFOLDER_OPERATION = "getParentFolder";
  public static final String GET_PARENTFOLDER_REQUEST = "GetParentFolderRequest";
  public static final String GET_PARENTFOLDER_RESPONSE = "GetParentFolderResponse";

  public static final String MOVE_FOLDERCHILDREN_OPERATION = "moveFolderChildren";
  public static final String MOVE_FOLDERCHILDREN_REQUEST = "MoveFolderChildrenRequest";
  public static final String MOVE_FOLDERCHILDREN_RESPONSE = "MoveFolderChildrenResponse";

  public static final String REMOVE_FOLDERCHILDREN_OPERATION = "removeFolderChildren";
  public static final String REMOVE_FOLDERCHILDREN_REQUEST = "RemoveFolderChildrenRequest";
  public static final String REMOVE_FOLDERCHILDREN_RESPONSE = "RemoveFolderChildrenResponse";

  public static final String GET_SUMMARYBYPATH_OPERATION = "getSummaryByPath";
  public static final String GET_SUMMARYBYPATH_REQUEST = "GetSummaryByPathRequest";
  public static final String GET_SUMMARYBYPATH_RESPONSE = "GetSummaryByPathResponse";

  public static final String GET_DESCENDENTSLOCATORS_OPERATION = "getDescendentsLocators";
  public static final String GET_DESCENDENTSLOCATORS_REQUEST = "GetDescendentsLocatorsRequest";
  public static final String GET_DESCENDENTSLOCATORS_RESPONSE = "GetDescendentsLocatorsResponse";
