...
public class PSAaRelationshipList extends PSCollectionComponent {
    ...
    try {
      ...
      while (node != null) {
        PSRelationship relationship =
            new PSRelationship((Element) tree.getCurrent(), parentDoc, parentComponents);
  ...
  /**
   * Override the base class version to set the sort rank of the relationship just beforeaccessing.
   *
   ...
   * @exception ArrayIndexOutOfBoundsException index is out of range (index &lt; 0 || index &gt;=
   *     size()).
