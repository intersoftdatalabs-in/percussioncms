...
public class PSActionVisibilityContexts extends PSDbComponentCollection {
  ...
  public void add(IPSDbComponent comp) {
    if (null == comp || !(comp instanceof PSActionVisibilityContext)) {
      throw new IllegalArgumentException("Can only add PSActionVisibilityContext components");
    }
    ...
    if (null != ctx) {
      if (ctx == newCtx) return; // tried to add self
      ctx.clear();
  ...
  private PSActionVisibilityContext getContextObject(String name) {
    ...
    while (it.hasNext()) {
      ...
      if (ctx.getName().equalsIgnoreCase(name)) return ctx;
    }
    return null;
  }
