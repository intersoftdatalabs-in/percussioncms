...
public class PSUpdatePlanBuilder extends PSModifyPlanBuilder {
  ...
  public PSModifyPlan createModifyPlan(PSDisplayMapper mapper, PSFieldSet fieldSet)
    ...
    int insertUpdateFlag =
        PSApplicationBuilder.FLAG_ALLOW_INSERTS | PSApplicationBuilder.FLAG_ALLOW_UPDATES;
    ...
    while (binFields.hasNext()) {
      ...
      PSDisplayMapping mapping = getMapping(mapper, binField.getSubmitName());
      if (mapping == null) continue;

      // create copy of display mapper with only the one binary field
      PSDisplayMapper binMapper = new PSDisplayMapper(mapper.getFieldSetRef());
      binMapper.add(new PSDisplayMapping(mapping.getFieldRef(), mapping.getUISet()));

      // create the dataset
      updateSystemMapper = getSystemUpdateMapper(mapper, fieldSet);
      String binReqName = PSUniqueObjectGenerator.makeUniqueName(baseReqName);
      PSApplicationBuilder.createUpdateDataset(
          m_internalApp,
          binReqName,
          m_ce,
      ...
      PSCollection clearConditions = createBinaryClearConditional(binField);
      if (clearConditions != null) {
        // need to map the backend column to something that will be null
        PSDataMapping clearMapping = null;
        try {
          clearMapping = new PSDataMapping(new PSTextLiteral(""), binField.getLocator());
        ...
        // create dataset
        String binClearReqName = PSUniqueObjectGenerator.makeUniqueName(baseReqName);
        PSApplicationBuilder.createUpdateDataset(
            m_internalApp,
            binClearReqName,
            m_ce,
  ...
  private PSDisplayMapping getMapping(PSDisplayMapper mapper, String fieldName) {
    ...
    while (mappings.hasNext()) {
      ...
      if (mapping.getFieldRef().equalsIgnoreCase(fieldName)) return mapping;
    }
      ...
      if (clearParamName != null) {
        cond =
            new PSConditional(
                ...
                new PSTextLiteral(CLEAR_PARAM_TRUE),
                PSConditional.OPBOOL_AND);
        conditionals.add(cond);
      }
    } catch (IllegalArgumentException e) {
