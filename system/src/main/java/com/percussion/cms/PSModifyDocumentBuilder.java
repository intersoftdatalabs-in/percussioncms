...
public abstract class PSModifyDocumentBuilder extends PSEditorDocumentBuilder {
  ...
  protected Node createWorkflowInfo(Document doc, PSExecutionData data)
    ...
    if (null != o) contentId = o.toString();
    if (null != contentId) {
      try {
    ...
    try {
      while (handlers.hasNext()) {
        IPSInternalResultHandler rh = (IPSInternalResultHandler) handlers.next();
        ...
        if (null == fragmentDoc || null == fragmentDoc.getDocumentElement()) continue;
        Node importNode = doc.importNode(fragmentDoc.getDocumentElement().getFirstChild(), true);
      ...
      if (null != reqData) reqData.release();
    }
  ...
  private List<?> getWorkflowResources() throws PSDataExtractionException {
    String[] datasetNames = {HISTORY_DATASET_NAME, CONTENTSTATUS_DATATSET_NAME};

    List<Object> handlers = new ArrayList<>();

    Iterator<?> names = PSIteratorUtils.iterator(datasetNames);
    while (names.hasNext()) {
      String reqName = IPSConstants.EDITOR_SUPPORT_APPNAME + "/" + (String) names.next();
