...
public class PSContentEditorWalker {
  ...
  public static Element getControlElement(Document doc, String name) {
    ...
    while ((node = controls.item(i++)) != null) {
      ...
      if (paramName != null && paramName.equalsIgnoreCase(name)) return control;
    }
  ...
  public static Object getDisplayFieldValue(Document doc, String name) {
    ...
    if (control != null) {
      ...
      if (values.size() > 0) {
         ...
         * is a 'Value' element.
         */
        if (elem.getTagName().equals(PSDisplayFieldElementBuilder.DATA_NAME)) {
          Object o = values.get(0);
          if (o != null) return values.get(0).toString();
          else return "";
        }
  ...
  public static List getValues(Element elem) {
    ...
    NodeList values = elem.getElementsByTagName(PSDisplayFieldElementBuilder.DATA_NAME);
    for (int i = 0; i < values.getLength(); i++) {
      Element value = (Element) values.item(i);
      Text text = (Text) value.getFirstChild();
      if (text != null) list.add(text.getNodeValue());
    }
  ...
  public static void replaceOrAddValue(Document doc, Element container, Object value) {
    ...
    NodeList values = container.getElementsByTagName(PSDisplayFieldElementBuilder.DATA_NAME);

    Node oldValue = values.item(0);
    Element newValue = createValue(doc, value.toString());

    if (oldValue == null) container.appendChild(newValue);
    else container.replaceChild(newValue, (Element) oldValue);
  }
