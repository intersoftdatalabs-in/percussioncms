...
/**
 * This file contains various constants that are used across 2 or more editors. It serves as a
 * registry. This includes such things as:
 *
 * <ul>
 *   <li>The names of xml document elements and attributes
 *   <li>The names of table columns for system tables
 *   <li>The names of system fields
 * </ul>
 */
public interface IPSConstants {
  ...
  /** This is part of the primary key for the main content item table. Never empty. */
  ...
  /**
   * This is the foreign key to the parent for any table that is a child of a child. Never empty.
   */
  String CHILD_OF_CHILD_PKEY = "PARENTID";

  /**
   * The (optional) column that stores the list position an item within all the items for the
   * contentid.
   */
  ...
  String PSX_RELATIONSHIPPROPERTIES = "PSX_OBJECTRELATIONSHIPPROP";
  ...
  /** The HTML parameter name that specifies the submit name for the requested binary field. */
  String SUBMITNAME_PARAM_NAME = "sys_submitname";

  /** This is the last modifier column of the main content item table. Never empty. */
  String ITEM_CONTENTLASTMODIFIER = "CONTENTLASTMODIFIER";

  /** This is the last modified date column of the main content item table. Never empty. */
  String ITEM_CONTENTLASTMODIFIEDDATE = "CONTENTLASTMODIFIEDDATE";

  /** This is the edit revision column of the main content item table. Never empty. */
  String ITEM_EDITREVISION = "EDITREVISION";

  /** This is the tip revision column of the main content item table. Never empty. */
  String ITEM_TIPREVISION = "TIPREVISION";

  /** This is the current revision column of the main content item table. Never empty. */
  String ITEM_CURRENTREVISION = "CURRENTREVISION";

  /** This is the content state id column of the main content item table. Never empty. */
  String ITEM_CONTENTSTATEID = "CONTENTSTATEID";

  /** This is the author column of the main content item table. Never empty. */
  String ITEM_CONTENTCREATEDBY = "CONTENTCREATEDBY";

  /** This is the checked out username of the main content item table. Never empty. */
  String ITEM_CONTENTCHECKOUTUSERNAME = "CONTENTCHECKOUTUSERNAME";

  /** This is the title column of the main content item table. May be empty; */
  String ITEM_TITLE = "TITLE";

  /* Init params have names of the form 'com.percussion.<name>'. */
  /**
   * The name of the InitParam in the system def that contains the name of the control to use when a
   * hidden control is needed. The specified control can be overridden by the end purchaser. Never
   * empty.
   */
  String HIDDEN_CONTROL_PARAM_NAME = "com.percussion.hidden_control_name";

  /** Name of the main content item table. */
  String CONTENT_STATUS_TABLE = "CONTENTSTATUS";

  /**
   * Name of the column in the related content table that identifies the content id to which an item
   * is related.
   */
  ...
  /** Name of the column in the contentstatus table that identifies the content type of that row. */
  String CONTENTTYPEID_COLUMN = "CONTENTTYPEID";

  /** Name of the column in the content variants table that contains the id of that variant. */
  String VARIANTID_COLUMN = "TEMPLATE_ID";

  /**
   * Name of the column in the content variants table that contains the url used to query a new
   * editor page.
   */
  ...
  /**
   * Name of the column in the ASSEMBLERPROPERTIES table that holds a value of the context variable.
   */
  String PROPERTYVALUE_COLUMN = "PROPERTYVALUE";

  /**
   * The name of the (hidden) Rx application that contains the resources used by the crow editors.
   */
  ...
  /**
   * Revision value used when there is no corresponding actual revision. Used for the edit revison
   * of a content item that is not checked out, or for the tip revision before it is given a
   * meaninful value.
   */
  int NO_CORRESPONDING_REVISION_VALUE = -1;

  /**
   * Workflow id used when there is no workflow id exist for an content item. For example, if the
   * object is not workflowable, its workflow id should be this value.
   */
  int INVALID_WORKFLOW_ID = -1;

  /** This is the value text to use for document attributes that have a boolean value of true. */
  String BOOLEAN_TRUE = "yes";

  String TRUE = "true";

  /** This is the value text to use for document attributes that have a boolean value of false. */
  String BOOLEAN_FALSE = "no";

  String FALSE = "false";

  /**
   * The name of the default view to use if one has not been specified. Currently this is the <code>
   * sys_Default</code> view. This is the main content editor view, includes all fields, and is
   * linked to a stylesheet through the system def that displays the full CE UI.
   */
  String DEFAULT_VIEW_NAME = "sys_Default";

  /**
   * The name of the system view that includes all fields, but is linked through the system def with
   * a stylesheet that only displays the the fields, and no other parts of the CE UI.
   */
  ...
  /**
   * The name of the system view that includes a list of hidden fields. The displayed fields are all
   * fields (from {@link #SYS_ALL_VIEW_NAME}) minus the fields specified by this view. The list of
   * hidden fields are comma delimited, such as, "sys_HiddenFields:XXX,YYY,ZZZ". The list of fields
   * may be empty, in this case, all fields (from {@link #SYS_ALL_VIEW_NAME}) will be displayed.
   */
  String SYS_HIDDEN_FIELDS_VIEW_NAME = "sys_HiddenFields:";

  /**
   * Name of the system field that denotes the relevancy ranking of a search result, currently
   * "sys_relevancy".
   */
  String SYS_RELEVANCY = "sys_relevancy";

  /**
   * The name of the system field that acts as a flag to indicate that a folder should be published
   * with a site.(Site folder Publishing)
   */
  String SYS_PUBLISH_FOLDER_WITH_SITE = "sys_publishFolderWithSite";

  /**
   * The name of query attribute which determines how SFP should handle publish flagged site
   * folders.
   */
  String INCLUDE_FOLDERS = "IncludeFolders";

  /**
   * Name of the system field that denotes the current revision of an item, currently
   * "sys_currentrevision".
   */
  ...
  /** Checkout status indicating it is checked out to somebody other than the current user. */
  String CHECKOUT_STATUS_SOMEONEELSE = "Checked Out";

  /**
   * Constant for security provider type name for internal user. This is redefined here since
   * security provider classes are obfuscated.
   *
  ...
  String SP_TYPE_RXINTERNAL =
      PSSecurityProvider.getSecurityProviderTypeString(PSSecurityProvider.SP_TYPE_RXINTERNAL);

  /**
   * Constant for internal user name. This is redefined here since security provider classes are
   * obfuscated.
   *
   * @see PSSecurityProvider#INTERNAL_USER_NAME
   */
  String INTERNAL_USER_NAME = PSSecurityProvider.INTERNAL_USER_NAME;

  /** Constant for the file object referencing the properties file that defines authtypes. */
  String AUTHTYPE_PROP_FILE =
      "rxconfig" + File.separator + "Server" + File.separator + "authtypes.properties";

  /**
   * Prefix for authtype properties used to build the key in the {@link #AUTHTYPE_PROP_FILE}. Key
   * format is <code>AUTHTYPE_PREFIX</code> + authtype.
   */
  String AUTHTYPE_PREFIX = "authtype.";

  /**
   * Name of the request private object to use to disable field visibility rules processing by the
   * content editor. If value is a {@link Boolean} object with a <code>true</code> value, visibility
   * rules will be disabled, otherwise it has no effect.
   */
  String SKIP_FIELD_VISIBILITY_RULES = "sys_skipFieldVisibilityRules";

  /**
   * Name of the request private object to use to indicate that an item is being loaded in order to
   * index content for search. A value that is a {@link Boolean} object with a <code>true</code>
   * value indicates that this is the case, otherwise not. Intended to allow for conditional
  ...
  /**
   * When set to true will send output through a compressor based on the mime type of the assembly
   * result.
   */
