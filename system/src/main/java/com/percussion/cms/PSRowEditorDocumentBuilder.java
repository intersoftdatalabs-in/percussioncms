...
public class PSRowEditorDocumentBuilder extends PSModifyDocumentBuilder {
  ...
  public PSRowEditorDocumentBuilder(
    ...
    while (mappings.hasNext()) {
      ...
      if (null == o) {
        o = fields.getChildField(fieldName, fields.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
      }
      if (null == o) {
      ...
      if (o instanceof PSFieldSet) {
        ...
        if (fieldSet.getType() == PSFieldSet.TYPE_SIMPLE_CHILD) {
          addBuildStep(new PSMultiValueBuilder(fieldSet, mapping.getUISet(), this));
        } else {
          addBuildStep(new PSTableValueBuilder(fieldSet, mapping.getUISet(), childMapper, this));
        }
      } else {
        PSField field = (PSField) o;
        if (showField(field)) {
          if (isBinaryField(field)) {
            addBuildStep(new PSDisplayFieldBuilder(field, mapping.getUISet(), this));
          } else {
            addBuildStep(
                new PSSingleValueBuilder(field, mapping.getUISet(), field.getLocator(), this));
          }
        }
      }
    }
    ...
    for (int i = 0; i < hiddenParamSet.length; i++) {
      addBuildStep(
          new PSSingleValueBuilder(
              controlName,
              (String) hiddenParamSet[i][0],
              (IPSReplacementValue) hiddenParamSet[i][1],
              this));
    }
  }
  ...
  protected Iterator getActionLinks(Document doc, PSExecutionData data)
    ...
    try {
      ...
      params.add(
          new PSMapPair(
              m_docContext.getSystemParam(PSContentEditorHandler.PAGE_ID_PARAM_NAME), "" + pageId));

      params.add(
          new PSMapPair(IPSHtmlParameters.SYS_VIEW, getViewEvaluator().getNextView(data, pageId)));
      ...
      params.add(new PSMapPair(m_docContext.getRequestTypeHtmlParamName(), actionType));

      params.add(
          new PSMapPair(
              m_docContext.getSystemParam(PSContentEditorHandler.CHILD_ID_PARAM_NAME),
      ...
      if (null != o && o.toString().trim().length() > 0) {
        params.add(
            new PSMapPair(
                m_docContext.getSystemParam(PSContentEditorHandler.CONTENT_ID_PARAM_NAME),
                o.toString()));
      }
      ...
      if (null != o && o.toString().trim().length() > 0) {
        params.add(
            new PSMapPair(
                m_docContext.getSystemParam(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME),
                o.toString()));
      }
      ...
      if (null != o && o.toString().trim().length() > 0) {
        params.add(
            new PSMapPair(
                m_docContext.getSystemParam(PSContentEditorHandler.REVISION_ID_PARAM_NAME),
                o.toString()));
      }
      ...
      String label =
          isNewDoc
              ? PSI18nUtils.getString(
                  PSI18nUtils.PSX_CE_ACTION + PSI18nUtils.LOOKUP_KEY_SEPARATOR_LAST + "Insert",
                  lang)
              : PSI18nUtils.getString(
                  PSI18nUtils.PSX_CE_ACTION + PSI18nUtils.LOOKUP_KEY_SEPARATOR_LAST + "Update",
                  lang);

      String accesskey =
          isNewDoc
              ? PSI18nUtils.getMnemonic(
                  PSI18nUtils.PSX_CE_ACTION + PSI18nUtils.LOOKUP_KEY_SEPARATOR + "Insert",
                  lang,
                  'I')
              : PSI18nUtils.getMnemonic(
                  PSI18nUtils.PSX_CE_ACTION + PSI18nUtils.LOOKUP_KEY_SEPARATOR + "Update",
                  lang,
                  'U');

      actions.add(createActionElement(doc, label, null, params.iterator(), true, accesskey));
  ...
  public void prepareExecutionData(PSExecutionData data)
    ...
    if (isNewDocument(data, true)) return;
  ...
  private List createSdmpExtractors(PSFieldSet fs, PSDisplayMapper mapper)
    ...
    while (mappingsIter.hasNext()) {
      ...
      PSField sdmpField =
          fs.getChildField(mapping.getFieldRef(), fs.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
      if (null != sdmpField) {
        PSFieldSet sdmpFieldSet =
            fs.getChildsFieldSet(sdmpField.getSubmitName(), fs.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
        ...
        if (o instanceof PSField) {
          PSField field = (PSField) o;
          if (field.getLocator() instanceof PSBackEndColumn) parentMappings.add(mapping);
        }
      }
    }
    if (0 == sdmpFieldSets.size()) return null;

    List results = new ArrayList();
    // create the extractors for the parent
    List parentExtractors = new ArrayList();
    String tableName = createExtractors(fs, parentMappings, parentExtractors);
    ...
    while (sdmpIter.hasNext()) {
      ...
      if (tableName != null) results.add(new PSMapPair(tableName, extractors));
    }
  ...
  private String createExtractors(PSFieldSet fs, List mappings, List results)
    ...
    while (mappingIter.hasNext()) {
      ...
      if (isBinaryField(field)) continue;
      IPSReplacementValue value = field.getLocator();
      if (!(value instanceof PSBackEndColumn)) continue;
      if (null == tableName) {
      ...
      try {
        results.add(PSDataExtractorFactory.createReplacementValueExtractor(value));
    ...
    if (null != m_sdmpExtractors) {
      ...
      for (int i = 0; i < mergeCount; i++) {
        ...
        while (extractorIter.hasNext()) {
          ...
          if (hasData) value = ext.extract(data);
      ...
      ResultSet rs =
          new PSResultSet(
              finalData, colToIndex, new PSResultSetMetaData(new MergedMetaData(metaDataList)));
  ...
  private class MergedMetaData implements ResultSetMetaData {
    ...
    public MergedMetaData(List metaDataInfo) throws SQLException {
      ...
      while (iter.hasNext()) {
        ...
        if (!colIter.hasNext()) {
          throw new IllegalArgumentException("missing columns for metadata");
        }
        while (colIter.hasNext()) {
          PSBackEndColumn col = (PSBackEndColumn) colIter.next();
          String alias = col.getAlias();
          String colName;
          if (null != alias && alias.length() > 0) colName = alias;
          else colName = col.getColumn();
          PSBackEndTable table = col.getTable();
          PSMetaDataCache.loadConnectionDetail(table);
          int idx =
              getColumnIndex(
                  md,
                  col.getTable().getTable(),
                  colName,
    ...
    public String getCatalogName(int index) throws SQLException {
      return getOriginalMetaData(index).getCatalogName(getOriginalIndex(index));
    }

    // see interface for desc
    public String getColumnClassName(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnClassName(getOriginalIndex(index));
    }
    ...
    public int getColumnDisplaySize(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnDisplaySize(getOriginalIndex(index));
    }

    // see interface for desc
    public String getColumnLabel(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnLabel(getOriginalIndex(index));
    }

    // see interface for desc
    public String getColumnName(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnName(getOriginalIndex(index));
    }

    // see interface for desc
    public int getColumnType(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnType(getOriginalIndex(index));
    }

    // see interface for desc
    public String getColumnTypeName(int index) throws SQLException {
      return getOriginalMetaData(index).getColumnTypeName(getOriginalIndex(index));
    }
    ...
    public int getScale(int index) throws SQLException {
      return getOriginalMetaData(index).getScale(getOriginalIndex(index));
    }

    // see interface for desc
    public String getSchemaName(int index) throws SQLException {
      return getOriginalMetaData(index).getSchemaName(getOriginalIndex(index));
    }
    ...
    public String getTableName(int index) throws SQLException {
      String tableName = null;
      try {
        tableName = getOriginalMetaData(index).getTableName(getOriginalIndex(index));
    ...
    public boolean isAutoIncrement(int index) throws SQLException {
      return getOriginalMetaData(index).isAutoIncrement(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isCaseSensitive(int index) throws SQLException {
      return getOriginalMetaData(index).isCaseSensitive(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isCurrency(int index) throws SQLException {
      return getOriginalMetaData(index).isCurrency(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isDefinitelyWritable(int index) throws SQLException {
      return getOriginalMetaData(index).isDefinitelyWritable(getOriginalIndex(index));
    }

    // see interface for desc
    public int isNullable(int index) throws SQLException {
      return getOriginalMetaData(index).isNullable(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isReadOnly(int index) throws SQLException {
      return getOriginalMetaData(index).isReadOnly(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isSearchable(int index) throws SQLException {
      return getOriginalMetaData(index).isSearchable(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isSigned(int index) throws SQLException {
      return getOriginalMetaData(index).isSigned(getOriginalIndex(index));
    }

    // see interface for desc
    public boolean isWritable(int index) throws SQLException {
      return getOriginalMetaData(index).isWritable(getOriginalIndex(index));
    }
    ...
    private int getColumnIndex(
        ResultSetMetaData md, String tableName, String colName, String origin, String driver)
        throws SQLException {
      ...
      for (int i = 1; i <= count && colIndex < 0; i++) {
        ...
        if (null != tmpTableName && tmpTableName.trim().length() > 0) {
          ...
          tmpTableName = PSSqlHelper.parseTableName(driver, tmpTableName, originBuf, null);
          if (originBuf.length() > 0) tmpOrigin = originBuf.toString();
          if (null != tmpOrigin && tmpOrigin.trim().length() == 0) tmpOrigin = null;
        }
      ...
      if (colIndex < 0) throw new SQLException("Column not found in result set");
      return colIndex;
    }
