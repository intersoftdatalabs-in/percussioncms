...
public abstract class PSCloneHandler implements IPSCloneHandler {
  ...
  public PSCloneHandler(IPSCopyHandler copyHandler, String config)
      throws PSServerConfigException, PSUnknownNodeTypeException {
    if (copyHandler == null) throw new IllegalArgumentException("copyHandler cannot be null");

    if (config == null || config.trim().length() == 0) config = "standard";

    m_copyHandler = copyHandler;

    // load clone handler configuration
    PSCloneHandlerConfigSet configs =
        new PSCloneHandlerConfigSet(
            PSConfigManager.getInstance()
                .getXMLConfig(PSConfigurationFactory.CLONE_HANDLERS_CFG)
                .getDocumentElement(),
            null,
            null);
  ...
  public static boolean doProcessCheck(
      PSProcessCheck check, PSExecutionData data, boolean defaultValue)
      throws PSNotFoundException, PSExtensionException {
    if (data == null) throw new IllegalArgumentException("execution data cannot be null");

    if (check == null) return defaultValue;

    PSRuleListEvaluator evaluator = new PSRuleListEvaluator(check.getConditions());
