...
public abstract class PSQueryCommandHandler extends PSCommandHandler
  ...
  private static final String XSL_PARSER_ERROR_STRING =
  ...
  public PSQueryCommandHandler(
    ...
    List<PSConditionalUrlEvaluator> ssEvaluators = new ArrayList<>(4);

    while (stylesheets.hasNext()) {
      PSConditionalStylesheet ss = (PSConditionalStylesheet) stylesheets.next();
      PSConditionalUrlEvaluator ssEval =
          new PSConditionalUrlEvaluator(ss.getConditions(), ss.getRequest());
      ssEvaluators.add(ssEval);
    }

    // System.out.println( "found " + ssEvaluators.size() + " stylesheet evals" );
    m_stylesheetEvaluators = new PSConditionalUrlEvaluator[ssEvaluators.size()];
    ssEvaluators.toArray(m_stylesheetEvaluators);

    PSSingleHtmlParameter pageId =
        new PSSingleHtmlParameter(ceh.getParamName(PSContentEditorHandler.PAGE_ID_PARAM_NAME));
    m_pageIdExtractor = PSDataExtractorFactory.createReplacementValueExtractor(pageId);
  }
  ...
  private void executeQueryRequest(
      ...
      throws PSUnsupportedConversionException, PSErrorException, PSDataExtractionException,
          PSRequestValidationException, PSNotFoundException, PSInternalRequestCallException,
          PSAuthorizationException, PSAuthenticationFailedException, PSConversionException,
          PSExtensionProcessingException, PSParameterMismatchException, SQLException {
    ...
    Iterator apps =
        getAppList(
            getPageId(data).intValue(),
            data,
            builder.isNewDocument(data, builder.getDocContext().isRowEditor()));
    Stack tmpStack = new Stack();
    while (apps.hasNext()) {
      IPSInternalResultHandler appHandler = (IPSInternalResultHandler) apps.next();
    ...
    while (!tmpStack.empty()) rsStack.push(tmpStack.pop());
  }
   ...
   */
  private Document produceResultDocument(PSExecutionData data, PSEditorDocumentBuilder builder)
      throws PSUnsupportedConversionException, PSErrorException, PSDataExtractionException,
          PSRequestValidationException, PSNotFoundException, PSInternalRequestCallException,
          PSAuthorizationException, PSAuthenticationFailedException, PSConversionException,
          PSExtensionProcessingException, PSParameterMismatchException, SQLException {
  ...
  public void processRequest(PSRequest req) {
    ...
    try {
      ...
      if (cacheId == null) {
        executeQueryRequest(req, data, execDataCleanupList, resultSetCleanupList);
      ...
      if (PSRequest.PAGE_TYPE_TEXT == req.getRequestPageType()) {
        ...
        mergeStylesheet = applyStylesheetConditions(data, resultDoc, stylesheet, true);
      }
      ...
      if (PSRequest.PAGE_TYPE_HTML == req.getRequestPageType()) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
          ...
          try {
            ...
            if (urlPath.startsWith("../")) urlPath = urlPath.substring(3);
          ...
          if (out != null) {
            try (ByteArrayInputStream in = new ByteArrayInputStream(out.toByteArray())) {
              ...
              if (null == contentHeader) {
                ...
                contentHeader = PSBaseHttpUtils.constructContentTypeHeader(mimeType, null);
              }
  ...
  public PSExecutionData makeInternalRequest(PSRequest request)
    ...
    try {
      executeQueryRequest(request, data, execDataCleanupList, resultSetCleanupList);
  ...
  public ResultSet getResultSet(PSExecutionData data) throws PSInternalRequestCallException {
    ...
    if (data != null) {
      ...
      if (stack.size() > 0) rs = (ResultSet) stack.pop();
    } else throw new IllegalArgumentException("Execution data must not be null.");
  ...
  protected Map createPageMap(Map pageInfo) {
    ...
    while (pi.hasNext()) {
      ...
      while (childIds.hasNext()) childIdList.add(childIds.next());
  ...
  private URL applyStylesheetConditions(
      PSExecutionData data, Document doc, String stylesheet, boolean fixupURL)
    ...
    } else stylesheetURL = new URL(stylesheet);
  ...
  private void processFieldTranslations(Document doc, PSExecutionData data)
    ...
    while (names.hasNext()) {
      ...
      Element control = PSContentEditorWalker.getControlElement(doc, name);
      ...
      try {
        ...
        Object val;
        if (values.isEmpty()) val = "";
        else if (values.size() == 1) val = values.get(0);
        else val = values;

        req.setParameter(name, val);

        Object result = runner.processUdfCallExtractor(data);
        if (result != null) PSContentEditorWalker.replaceOrAddValue(doc, control, result);
      } catch (Exception e) {
