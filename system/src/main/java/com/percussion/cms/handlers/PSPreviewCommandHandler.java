...
/**
 * This class handles the 'preview' command. It responds by building a page of data that can be used
 * to generate the preview of the content item. The output document looks like the document
 * generated by the 'edit' command handler, except it only includes fields that have the
 * 'showInPreview' flag set to <code>true</code> and it also includes additional information used by
 * the stylesheet for generating links to actual previews of the data in supported formats.
 *
...
public class PSPreviewCommandHandler extends PSQueryCommandHandler {
  ...
  /**
   * Creates a handler to process requests for previewing existing content items.
   *
   ...
   * of the params and exceptions that aren't listed below.
   *
   * @param eh The handler that will actually process the edit requests. This handler uses resources
   *     for the 'root' page when processing the preview request. Never <code>null</code>.
   */
  public PSPreviewCommandHandler(
      PSApplicationHandler ah,
      PSContentEditorHandler ceh,
      PSContentEditor ce,
      PSApplication dynamicApp,
    ...
    m_previewBuilderEx =
        new PSPreviewDocumentBuilderEx(ce, ctx, dispMapper, ROOT_PARENT_PAGE_ID, false);
  }
  ...
  protected PSEditorDocumentBuilder getDocumentBuilder(int id, PSExecutionData data) {
    ...
    if (id != ROOT_PARENT_PAGE_ID) return null;

    PSRequest req = data.getRequest();

    String showAllField = req.getParameter(IPSHtmlParameters.SYS_SHOW_IN_PREVIEW);

    if (showAllField != null && showAllField.equalsIgnoreCase(IPSConstants.BOOLEAN_TRUE))
      return m_previewBuilderEx;
    else return m_previewBuilder;
  }

  /**
   * The builder of the preview output page. The actual editor is a <code>PSPreviewDocumentBuilder
   * </code>. Never <code>null</code> or modified after construction.
   */
  private PSEditorDocumentBuilder m_previewBuilder;

  /**
   * The builder of the preview output page. The actual editor is a <code>PSPreviewDocumentBuilderEx
   * </code>. Use this to override the "showInPreview" of the fields to "yes". Never <code>null
   * </code> or modified after construction.
   */
  private PSEditorDocumentBuilder m_previewBuilderEx;

  /**
   * The handler for the edit pages of the editor associated with this preview page. Never <code>
   * null</code> or modified after construction.
   */
  private PSEditCommandHandler m_editHandler;

  /**
   * Every editor works on a particular type of content. The identifier of this content type in the
   * db is this value. We use it when making the variant lookup. Set in ctor, then never modified.
   */
