...
public class PSEditCommandHandler extends PSQueryCommandHandler {
  ...
  public PSEditCommandHandler(
      ...
      throws PSNotFoundException, PSIllegalArgumentException, PSExtensionException,
          PSServerException, PSSystemValidationException {
    ...
    initializeRequestResources(
        app, ce, dispMapper, 0, PSQueryCommandHandler.ROOT_PARENT_PAGE_ID, m_pageInfo, result);

    m_appName = app.getName();

    // init builders
    PSEditorDocumentContext rowCtx =
        new PSEditorDocumentContext(ceh, ah.getApplicationDefinition(), ce);
    ...
    try {
      ...
      if (PSServer.getProperty("requestBehindProxy", "false").equalsIgnoreCase("true")) {
        url =
            new URL(
                PSServer.getProperty("proxyScheme", "http"),
                PSServer.getProperty("publicCmsHostname", "127.0.0.1"),
                Integer.parseInt(
                    PSServer.getProperty("proxyPort", "" + PSServer.getListenerPort())),
                ah.getFullRequestRoot() + "/" + ce.getRequestor().getRequestPage() + ".html");
      } else {
        url =
            new URL(
                "http",
                PSServer.getServerName().toLowerCase(),
    ...
    PSEditorDocumentContext summaryCtx =
        new PSEditorDocumentContext(ceh, ah.getApplicationDefinition(), ce);
    summaryCtx.setPageInfoMap(m_pageInfo);
    summaryCtx.setEditorType(PSEditorDocumentContext.EDITOR_TYPE_SUMMARY_EDITOR);
    summaryCtx.setRequestUrl(url.toString());

    initializeEditorBuilders(
        ce, dispMapper, PSQueryCommandHandler.ROOT_PARENT_PAGE_ID, rowCtx, summaryCtx);
  }
  ...
  public void preProcessRequest(PSRequest req)
    ...
    String contentId = req.getParameter(PSContentEditorHandler.CONTENT_ID_PARAM_NAME);
    String condition;
  ...
  protected Iterator getAppList(int id, PSExecutionData data, boolean isNewDoc)
    ...
    if (isNewDoc) return PSIteratorUtils.emptyIterator();
    ...
    while (datasetNames.hasNext()) {
      ...
      if (null == rh) {
        throw new RuntimeException("Unexpected: Couldn't find handler for " + reqName);
      }
      handlers.add(rh);
    }
  ...
  protected PSEditorDocumentBuilder getDocumentBuilder(int id, PSExecutionData data) {
  ...
  private Set getQueryKeys(int tableDepth) {
    if (tableDepth < 0) throw new IllegalArgumentException("table depth can't be negative");
  ...
  private static List buildSdmpChildMappers(PSFieldSet fields, PSDisplayMapper dispMapper) {
    ...
    while (mappings.hasNext()) {
      ...
      PSField sdmpField =
          fields.getChildField(mapping.getFieldRef(), PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
      if (null != sdmpField) {
        PSFieldSet sdmpFieldSet =
            fields.getChildsFieldSet(
                sdmpField.getSubmitName(), PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
  ...
  private int initializeEditorBuilders(
    ...
    updatePageInfo(
        pageInfoMap, pageId, new PSRowEditorDocumentBuilder(ce, rowCtx, dispMapper, pageId, false));
    pageId++;
      ...
      if (o instanceof PSFieldSet) {
        ...
        if (PSFieldSet.TYPE_COMPLEX_CHILD == fs.getType()) {
          ...
          updatePageInfo(
              pageInfoMap,
              pageId,
              new PSSummaryEditorDocumentBuilder(ce, summaryCtx, mapping, pageId, false));

          // Hack alert: see COMPLETE_CHILD_PAGEID_OFFSET for details
          updatePageInfo(
              pageInfoMap,
              pageId + COMPLETE_CHILD_PAGEID_OFFSET,
              new PSCompleteChildDocumentBuilder(
                  ce, summaryCtx, mapping, pageId + COMPLETE_CHILD_PAGEID_OFFSET, false));

          pageId++;
          pageId = initializeEditorBuilders(ce, childMapper, pageId, rowCtx, summaryCtx);
        }
      }
    }
    return pageId;
  ...
  private int initializeRequestResources(
      ...
      int pageId,
      HashMap pageInfoMap,
      StringBuilder resultDatasetName)
      throws PSSystemValidationException {
    PSContentEditorPipe pipe = (PSContentEditorPipe) ce.getPipe();
    PSFieldSet fields = pipe.getMapper().getFieldSet(dispMapper.getFieldSetRef());
    ...
    String datasetName =
        PSApplicationBuilder.createQueryDataset(
            app, ce, dispMapper, keys.iterator(), auxMappings, sortedCols);
    ...
    while (sdmpMapperIter.hasNext()) {
      String resourceName =
          PSApplicationBuilder.createQueryDataset(
              app,
              ce,
              (PSDisplayMapper) sdmpMapperIter.next(),
              keys.iterator(),
              auxMappings,
      ...
      if (resourceName != null) datasetNames.add(resourceName);
    }
    ...
    while (mappings.hasNext()) {
      ...
      if (o instanceof PSFieldSet) {
        ...
        if (PSFieldSet.TYPE_COMPLEX_CHILD == fs.getType()) {
          ...
          nextPageId =
              initializeRequestResources(
                  app, ce, childMapper, recursionDepth + 1, nextPageId, pageInfoMap, resultName);
          ...
          pageInfoMap.put(
              summaryPageId,
              new PSPageInfo(
                  PSPageInfo.TYPE_SUMMARY_EDITOR,
                  childMapper.getId(),
                  summaryDSName,
                  summaryPageMap));
          ...
          datasetNames.add(
              PSApplicationBuilder.createQueryDataset(
                  app, ce, childMapper, keys.iterator(), auxMappings, sortedCols));
        }
      }
    }
    pageInfoMap.put(
        pageId,
        new PSPageInfo(PSPageInfo.TYPE_ROW_EDITOR, dispMapper.getId(), datasetNames, pageMap));
