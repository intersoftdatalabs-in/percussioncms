...
public class PSCloneCommandHandler extends PSCommandHandler {
  ...
  public PSCloneCommandHandler(
      ...
      throws PSIllegalArgumentException, PSExtensionException, PSNotFoundException,
          PSSystemValidationException {
    super(ah, ceh, ce, app);

    // save the param names we use
    m_contentIdParamName = ceh.getParamName(PSContentEditorHandler.CONTENT_ID_PARAM_NAME);
    m_revisionIdParamName = ceh.getParamName(PSContentEditorHandler.REVISION_ID_PARAM_NAME);
  ...
  private int executeCloneRequest(PSRequest request, PSExecutionData execData)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          ...
          PSNotFoundException, IOException, PSUnknownNodeTypeException, PSCmsException {
    // Extract the params
    ...
    boolean checkin = (wfAction != null && wfAction.equalsIgnoreCase(WF_ACTION_CHECKIN));

    // make the copy
    m_copyHandler.createCopy(
        new PSLocator(contentId, revisionId),
        new PSLocator(newContentId, newRevisionId, false),
        execData,
        checkin);
  ...
  public PSExecutionData makeInternalRequest(PSRequest request)
    ...
    PSExecutionData execData = new PSExecutionData(m_appHandler, this, request);
  ...
  public static Map copyRelatedContent(
    ...
    PSRelationshipCommandHandler rsh =
    ...
    if (!relationships.hasNext()) return inlineRelationships;
    ...
    while (relationships.hasNext()) {
      ...
      tempParams.put(
          IPSHtmlParameters.SYS_DEPENDENTID, Integer.toString(relationship.getDependent().getId()));
      tempParams.putAll(relationship.getUserProperties());
      tempParams.put(IPSHtmlParameters.SYS_RELATIONSHIPTYPE, relationship.getConfig().getName());

      if (relationship.getConfig().useDependentRevision()) {
        tempParams.put(
            IPSHtmlParameters.SYS_DEPENDENTREVISION, relationship.getDependent().getRevision());
      }
      ...
      if (relationship.isInlineRelationship()) {
        ...
        while (clones.hasNext()) lastClone = (PSRelationship) clones.next();

        if (lastClone != null) inlineRelationships.put(key, lastClone);
      }
    }
