...
public class PSWorkflowCommandHandler extends PSCommandHandler {
  ...
  public PSWorkflowCommandHandler(
      ...
      throws PSNotFoundException, PSIllegalArgumentException, PSExtensionException,
  ...
  public void processRequest(PSRequest req) {
    if (req == null) throw new IllegalArgumentException("req cannot be null");
    ...
    try {
      ...
      req.setParameter(IPSHtmlParameters.SYS_WORKFLOWID, extractWorkflowId(execData));

      // determine if this is a checkout or checkin action
      boolean isCheckin = false;
      boolean isCheckout = false;
      String wfAction = req.getParameter(IPSConstants.DEFAULT_ACTION_TRIGGER_NAME);
      if (wfAction != null && wfAction.trim().equalsIgnoreCase(IPSConstants.TRIGGER_CHECKOUT))
        isCheckout = true;
      else if (wfAction != null && wfAction.trim().equalsIgnoreCase(IPSConstants.TRIGGER_CHECKIN))
        isCheckin = true;
      else if (wfAction != null
          && wfAction.trim().equalsIgnoreCase(IPSConstants.TRIGGER_FORCE_CHECKIN)) isCheckin = true;
      ...
      if (exception != null) ms_logger.warn("Possible problem in workflow transition: ", exception);
  ...
  private boolean isItemValidationNeeded(PSRequest req) throws PSCmsException {
    String prop =
        PSServer.getServerProps().getProperty("allowItemValidationOnPublicItems", "false");
   ...
   */
  private void normalizeTransitionIdParameter(PSRequest req) throws Exception {
    if (req.getParameter(IPSHtmlParameters.SYS_TRANSITIONID) == null) {
      ...
      req.setParameter(IPSHtmlParameters.SYS_TRANSITIONID, Integer.toString(transition_id));
    }
  }
  ...
  public PSExecutionData makeInternalRequest(PSRequest req)
    ...
    try {
      ...
      PSExecutionData execData = new PSExecutionData(m_appHandler, this, req);
      ...
      if (!isCheckin && !isCheckout) {
        // skip validation if currently in a public state
        if (!PSCms.isPublishable(new Object[] {"y"}, new PSRequestContext(req))) {
          // Each request needs a new item validator
          ItemValidator itemValidator = new ItemValidator(execData, m_itemEvaluator);
           ...
           */
          if (!validate(itemValidator)) {
            recordErrorToRequest(execData, itemValidator.getErrorCollector(), true);
      ...
      throw new PSInternalRequestCallException(e.getErrorCode(), e, e.getErrorArguments());
    } catch (Exception e) {
  ...
  private PSExecutionData executeWorkflowRequest(
      PSRequest req, PSExecutionData execData, boolean isCheckin, boolean isCheckout)
      throws PSErrorException, PSRequestValidationException, PSInternalRequestCallException,
          PSExtensionProcessingException, PSAuthorizationException, PSAuthenticationFailedException,
          PSDataExtractionException, PSParameterMismatchException, PSNotFoundException,
          SQLException, PSSystemValidationException, IOException, PSUnknownNodeTypeException,
          PSCmsException, PSRelationshipProcessorException, PSRejectTransition,
    ...
    try {
      ...
      int level;
      if (isCheckout) level = IPSConstants.ASSIGNMENT_TYPE_READER;
      ...
      if (!isCheckin && !isCheckout) {
        PSRelationshipEffectProcessor effectProcessor =
            new PSRelationshipEffectProcessor(execData, IPSExecutionContext.RS_PRE_WORKFLOW);
      ...
      if (isCheckin) {
        ...
        PSRelationshipEffectProcessor effectProcessor =
            new PSRelationshipEffectProcessor(execData, IPSExecutionContext.RS_PRE_CHECKIN);
        effectProcessor.process();
      ...
      } else condition = IPSConstants.CHECKINOUT_CONDITION_CHECKIN;
      ...
      if (isCheckout) {
        ...
        PSRelationshipEffectProcessor effectProcessor =
            new PSRelationshipEffectProcessor(execData, IPSExecutionContext.RS_POST_CHECKOUT);
        effectProcessor.process();
        ...
        req.setParameter(
            m_internalApp.getRequestTypeHtmlParamName(), m_internalApp.getRequestTypeValueUpdate());
        String reqName = createRequestName(m_internalApp.getName(), m_updateStatusResource);
      ...
      if (isCheckin) action = PSEditorChangeEvent.ACTION_CHECKIN;
      else if (isCheckout) action = PSEditorChangeEvent.ACTION_CHECKOUT;
      ...
      if (action != PSEditorChangeEvent.ACTION_UNDEFINED) {
        Object wfActionPerformed = req.getPrivateObject(IPSConstants.WF_ACTION_PERFORMED);
        if (wfActionPerformed == null
            || !wfActionPerformed.toString().equalsIgnoreCase(IPSConstants.BOOLEAN_FALSE)) {
      ...
      if (!isCheckin && !isCheckout) {
        PSRelationshipEffectProcessor effectProcessor =
            new PSRelationshipEffectProcessor(execData, IPSExecutionContext.RS_POST_WORKFLOW);
        effectProcessor.process();
      }

      PSComponentSummary summary =
          PSWebserviceUtils.getItemSummary(Integer.parseInt(req.getParameter("sys_contentid")));
   ...
   */
  private void prepareStatusUpdate() throws PSSystemValidationException {
    ...
    PSDataMapper updateSystemMapper =
        PSApplicationBuilder.createSystemMappings(updateSysMappings.iterator());

    // create resource name
    m_updateStatusResource = PSUniqueObjectGenerator.makeUniqueName(CONTENTSTATUS_UPDATE);
  ...
  private boolean createCheckedOutRevision(PSExecutionData data)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          PSAuthenticationFailedException, PSNotFoundException, SQLException,
          PSSystemValidationException, IOException, PSUnknownNodeTypeException, PSCmsException {
    ...
    IPSWorkFlowContext wfCtx =
        (IPSWorkFlowContext)
            req.getPrivateObject(IPSWorkFlowContext.WORKFLOW_CONTEXT_PRIVATE_OBJECT);
    ...
    if (wfCtx != null) {
      ...
      if (baseRevision != curRevision) {
        m_copyHandler.createCopy(contentId, baseRevision, contentId, curRevision, data);
        Map<Integer, PSRelationship> inlineRelationships =
            PSCloneCommandHandler.copyRelatedContent(
                contentId, baseRevision, contentId, curRevision, data, m_ceHandler);
        ...
        try {
          ...
          PSInlineLinkProcessor.processInlineLinkItem(
              data.getRequest(), item, inlineRelationships, -1, false, false);
        } catch (PSException e) {
  ...
  private void recordErrorToRequest(
      PSExecutionData data, PSErrorCollector errorCollector, boolean redirect)
    ...
    try {
      ...
      IPSReplacementValue itemErrorStylesheet =
          m_ceHandler.getInitParam(COMMAND_NAME, "com.percussion.defaultItemError");
      if (itemErrorStylesheet != null) stylesheet = itemErrorStylesheet.getValueText();
      ...
      if (stylesheet != null) params.put(PSContentEditorHandler.USE_STYLESHEET, stylesheet);
      URL completeUrl =
          PSUrlUtils.createUrl(null, null, url, params.entrySet().iterator(), null, context);
  ...
  private class ItemValidator {
    ...
    private void initValidation(PSExecutionData data)
      ...
      for (int i = 0; i < controls.getLength(); i++) {
        ...
        String dimension = control.getAttribute(PSDisplayFieldElementBuilder.DIMENSION_NAME);
        // check for complex child
        if (dimension.equals(PSDisplayFieldElementBuilder.DIMENSION_TABLE)) {
          NodeList tableActionLink =
              control.getElementsByTagName(PSEditorDocumentBuilder.ACTION_NAME);
          int childPageId = 0;
          ...
          for (int j = 0; j < tableActionLink.getLength(); j++) {
            ...
            NodeList paramList = action.getElementsByTagName(PSEditorDocumentBuilder.PARAM_NAME);
            for (int k = 0; j < paramList.getLength(); k++) {
              ...
              if (name.equals(PSContentEditorHandler.PAGE_ID_PARAM_NAME)) {
                ...
                childPageId = Integer.parseInt(test.trim());
                // if child page Id is odd + 1 or else
                if (childPageId % 2 != 0) childPageId++;
                break;
              }
            }
          }
          ...
          for (int l = 0; l < rows.getLength(); l++) {
            ...
            String rownum = row.getAttribute(PSEditorDocumentBuilder.CHILDKEY_ATTRIB);
            params.put(PSContentEditorHandler.PAGE_ID_PARAM_NAME, Integer.toString(childPageId));
            params.put(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME, rownum);
      ...
      mi_errorCollector = new PSErrorCollector(PSErrorCollector.TYPE_ITEM, maxErrorsToStop);
    ...
    private Document createItemDocument() {
      ...
      Element controlNameSet = item.createElement(PSEditorDocumentBuilder.CONTROLNAMES_NAME);
      Element itemContent = item.createElement(PSEditorDocumentBuilder.ITEM_NAME);
      ...
      while (pages.hasNext()) {
        ...
        while (pageMapper.hasNext()) {
          ...
          NodeList cns = page.getElementsByTagName(PSEditorDocumentBuilder.CONTROLNAME_NAME);
          ...
          NodeList dvs = page.getElementsByTagName(PSDisplayFieldElementBuilder.DISPLAYFIELD_NAME);
          ...
          if (userStatus == null) {
            NodeList uss = page.getElementsByTagName(PSEditorDocumentBuilder.USERSTATUS_NAME);
            if (uss.getLength() > 0) userStatus = (Element) (item.importNode(uss.item(0), true));
          }

          // get the first workflow info found
          if (workflow == null) {
            NodeList wfs = page.getElementsByTagName(PSModifyDocumentBuilder.WORKFLOW_NAME);
            if (wfs.getLength() > 0) workflow = (Element) (item.importNode(wfs.item(0), true));
          }
        }
      }
      ...
      if (userStatus != null) contentEditor.appendChild(userStatus);
      if (workflow != null) contentEditor.appendChild(workflow);
