...
class PSCopyHandler implements IPSCopyHandler {
  ...
  public PSCopyHandler(
      ...
      while (tableSets.hasNext()) {
        ...
        while (tableRefs.hasNext()) {
          ...
          PSBackEndTable table =
              new PSBackEndTable(PSUniqueObjectGenerator.makeUniqueName("table"));
    ...
    m_contentIdParamName = m_ceHandler.getParamName(PSContentEditorHandler.CONTENT_ID_PARAM_NAME);

    m_revisionIdParamName = m_ceHandler.getParamName(PSContentEditorHandler.REVISION_ID_PARAM_NAME);
  ...
  public void createCopy(PSLocator source, PSLocator target, PSExecutionData data, boolean checkin)
  ...
  public void createCopy(
      int contentId, int revisionId, int newContentId, int newRevisionId, PSExecutionData data)
      throws PSAuthorizationException,
          PSInternalRequestCallException,
          PSAuthenticationFailedException,
          PSNotFoundException,
    ...
    try {
      ...
      request.setParameter(PSContentEditorHandler.CONTENT_ID_PARAM_NAME, String.valueOf(contentId));
      request.setParameter(
          PSContentEditorHandler.REVISION_ID_PARAM_NAME, String.valueOf(revisionId));
      request.setParameter(m_app.getRequestTypeHtmlParamName(), m_app.getRequestTypeValueQuery());

      // run each query, add all elements within root to the input doc
      Document inputDoc = executeQueries(data, contentId, revisionId, isNewItem, binaryTempFiles);

      // create new child id's for any complex children before inserting
      Map childRowMappings = fixupChildIds(inputDoc);
      request.setPrivateObject(PSCloneFactory.CHILD_ROW_MAPPINGS_PRIVATE_OBJECT, childRowMappings);
      ...
      if (isNewItem) {
        ...
        request.setParameter(IPSHtmlParameters.SYS_CLONEDPARENTID, String.valueOf(contentId));
        ...
        if (obj != null) {
          ...
          if (obj != null && obj instanceof Text) defWorkflowId = ((Text) obj).getData();
          request.setParameter(IPSHtmlParameters.SYS_WORKFLOWID, defWorkflowId);
        }

        // Override the clone fields
        Map fields =
            (Map)
                request.getParameterObject(PSCloneCommandHandler.SYS_CLONE_OVERRIDE_FIELDSET, null);
        if (fields != null) {
            ...
            if (fieldElemMap.containsKey(fieldName)) {
              if (value.length() < 1) {
                ...
                if (fieldName.equals(IPSHtmlParameters.SYS_WORKFLOWID)
                    ...
                    || fieldName.equals(IPSHtmlParameters.SYS_COMMUNITYID)) continue;
              }
      ...
      request.setParameter(m_app.getRequestTypeHtmlParamName(), m_app.getRequestTypeValueInsert());
      request.setParameter(m_contentIdParamName, String.valueOf(newContentId));
      request.setParameter(m_revisionIdParamName, String.valueOf(newRevisionId));

      // make the correct update
      String updateResourceName = null;
      if (isNewItem) updateResourceName = (String) m_updateResourceNames.get(INSERT_ITEM_RESOURCE);
      else updateResourceName = (String) m_updateResourceNames.get(INSERT_REVISION_RESOURCE);
  ...
  private void validateCommunity(int contentId, int revision, String communityId)
    ...
    try {
      boolean isVisible =
          mgr.isVisibleToCommunity(m_ce.getContentType(), Integer.parseInt(communityId));
      if (!isVisible) {
        Object[] args =
            new Object[] {
  ...
  private void createDataSets(
      ...
      PSDataMapper updateMapper,
      PSDtdBuilder updateDtd,
      boolean skipRelationships)
      throws PSSystemValidationException {
    if (mapper == null || pipe == null)
      throw new IllegalArgumentException("mapper or pipe may not be null");

    boolean firstCall = false;
    if (updateMapper == null) {
      updateMapper = new PSDataMapper();
      updateDtd = new PSDtdBuilder(PSUniqueObjectGenerator.makeUniqueName("CopyDTD"));
    ...
    while (mappings.hasNext()) {
      ...
      if (childMapper != null) {
        if (!(skipRelationships
            && childMapper.getFieldSetRef().equals(IPSHtmlParameters.RELATED_CONTENT)))
      ...
      if (o == null) {
        ...
        PSFieldSet sdmpFieldSet =
            fieldSet.getChildsFieldSet(fieldRef, PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
      ...
      if (o instanceof PSFieldSet) continue;
    ...
    // create the dataset
    createDataSet(dataMappings.entrySet().iterator(), fieldSet, updateMapper, updateDtd, false);

    // process system mappings
    if (systemMappings.size() > 0) {
      PSContentEditorSystemDef sysDef = m_ceHandler.getSystemDef();
      createDataSet(
          systemMappings.entrySet().iterator(),
          sysDef.getFieldSet(),
          updateMapper,
          updateDtd,
          true);
    ...
    while (sdmpMappings.hasNext()) {
      ...
      createDataSet(
          sdmpDataMappings.entrySet().iterator(), sdmpFieldSet, updateMapper, updateDtd, false);
    }
    ...
    while (children.hasNext())
      createDataSets(
          (PSDisplayMapper) children.next(), pipe, updateMapper, updateDtd, skipRelationships);

    // if first call, create update dataset
    if (firstCall) createUpdateDatasets(updateMapper, updateDtd);
  }
  ...
  private void createDataSet(
    ...
    try {
       ...
       */
      PSDtdBuilder queryDtd = new PSDtdBuilder(PSUniqueObjectGenerator.makeUniqueName("CopyDTD"));
      String parentElement = fieldSet.getName();
      int occurence =
          fieldSet.getType() == PSFieldSet.TYPE_PARENT
              ? PSDtdBuilder.OCCURS_ONCE
              : PSDtdBuilder.OCCURS_ANY;
      queryDtd.addElement(parentElement, occurence, queryDtd.getRootName());
      // build update dtd as we go
      updateDtd.addElement(parentElement, occurence, updateDtd.getRootName());

      // Create context
      PSMappingContext ctx =
          new PSMappingContext(dataMapper, updateMapper, queryDtd, updateDtd, fieldSet);
      ...
      while (mappings.hasNext()) {
        ...
        if (!(beMapping instanceof PSBackEndColumn)) continue;
        PSBackEndColumn beCol = (PSBackEndColumn) beMapping;

        if (PSApplicationBuilder.isKeyColumn(beCol.getColumn())) continue;
        ...
        if (!tables.contains(beTable)) tables.add(beTable);

        if (PSMetaDataCache.isBinaryBackendColumn(
            (PSTableSet) m_tableSets.get(tableAlias.toLowerCase()), beCol)) {
          // Create binary queries as we go
          m_binaryQueryHandlers.add(
              new PSBinaryQueryHandler(ctx, fieldSet, field.getSubmitName(), beCol, isSystemTable));
          continue;
        }
      ...
      if (tables.size() != 1) {
        Object[] args = {
          fieldSet.getName(),
          ((PSBackEndTable) tables.get(0)).getAlias(),
      ...
      String resourceName =
          PSApplicationBuilder.createQueryDataset(
              m_app, dataMapper, selKeys.entrySet().iterator(), null, queryDtd, false);

      m_queryResources.add(
          new PSQueryResourceContext(resourceName, fieldSet, localTable, isSystemTable));
    } catch (IllegalArgumentException e) {
      throw new IllegalArgumentException(e.getLocalizedMessage());
  ...
  private void createUpdateDatasets(PSDataMapper updateMapper, PSDtdBuilder updateDtd)
    ...
    String resNameRevisionInsert = PSUniqueObjectGenerator.makeUniqueName("insertRevision");
    ...
    PSApplicationBuilder.createUpdateDataset(
        ...
        updateMapper,
        true,
        PSApplicationBuilder.FLAG_ALLOW_INSERTS,
        updateDtd);

    m_updateResourceNames.put(INSERT_REVISION_RESOURCE, resNameRevisionInsert);
    ...
    PSDataMapper insertSystemMapper =
        PSApplicationBuilder.createSystemMappings(insertSysMappings.iterator());

    insertSystemMapper.addAll(updateMapper);

    // create item insert dataset
    String resNameItemInsert = PSUniqueObjectGenerator.makeUniqueName("insertItem");
  ...
  private Document executeQueries(
      PSExecutionData data, int contentId, int revisionId, boolean isNewItem, List binaryTmpFiles)
      throws PSAuthorizationException,
          PSInternalRequestCallException,
          PSAuthenticationFailedException,
          PSNotFoundException,
    ...
    Element root = PSXmlDocumentBuilder.createRoot(resultDoc, m_updateDtdRoot);

    // run each query, add all elements within root to the input doc
    boolean parentRowSelected = false;
    Iterator resources = m_queryResources.iterator();
    while (resources.hasNext()) {
      PSQueryResourceContext queryCtx = (PSQueryResourceContext) resources.next();

      // don't query system table if only making a new revision
      if (!isNewItem && queryCtx.isSystemTable()) continue;

      int numRows =
          executeQueryResource(
              data,
              root,
              PSCommandHandler.createRequestName(
                  m_app.getRequestRoot(), queryCtx.getResourceName()));
      queryCtx.setRowCount(numRows);
      if (queryCtx.getFieldSet().getType() == PSFieldSet.TYPE_PARENT && numRows > 0) {
  ...
  private int executeQueryResource(PSExecutionData data, Element root, String resourceName)
    ...
    try {
      ...
      IPSInternalResultHandler rh =
          (IPSInternalResultHandler) PSServer.getInternalRequestHandler(resourceName);
      queryData = rh.makeInternalRequest(data.getRequest());

      if (queryData == null) {
        // bug!
        throw new RuntimeException("no query handler found for " + resourceName);
      }
      ...
      Element tableRow = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      ...
      while (tableRow != null) {
        ...
        tableRow = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);

        Node importNode = root.getOwnerDocument().importNode(savedRow, true);
  ...
  private void executeUpdate(String resourceName, PSExecutionData data)
    ...
    IPSInternalRequestHandler rh =
        PSServer.getInternalRequestHandler(
            PSCommandHandler.createRequestName(m_app.getRequestRoot(), resourceName));

    if (rh == null) {
      // bug!
      throw new RuntimeException("no update handler found for " + resourceName);
    }
    ...
    try {
      ...
      if (resultData != null) resultData.release();
    }
  }
  ...
  private Map fixupChildIds(Document resultDoc) throws SQLException {
    ...
    while (queries.hasNext()) {
      ...
      if (ctx.getFieldSet().getType() != PSFieldSet.TYPE_COMPLEX_CHILD) continue;

      // determine size of id block to reserve
      int numIds = ctx.getRowCount();

      // make sure we have results to fix up
      if (numIds == 0) continue;

      // get the id's
      int[] idBlock = m_cmdHandler.getNextIdBlock(ctx.getTable().getTable(), numIds);
      ...
      while (row != null) {
        if (curRow >= numIds) {
          ...
          throw new RuntimeException(
              "More rows found in query results than reported for dataset" + ctx.getResourceName());
        }

        // fixup the row's sysid element
        Element sysIdEl =
            walker.getNextElement(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME, firstFlag);
        if (sysIdEl == null) {
            ...
            while (fields.hasNext()) {
              ...
              if (test instanceof PSField) {
                ...
                if (field.mayHaveInlineLinks())
                  replacedIds.put(
                      PSInlineLinkField.makeInlineRelationshipId(field.getSubmitName(), oldSysId),
                      PSInlineLinkField.makeInlineRelationshipId(field.getSubmitName(), newSysId));
              }
            }
  ...
  private void fixWorkflowId(Document doc, PSRequest request)
    ...
    if (strWorkflowId == null || strWorkflowId.trim().length() == 0) {
      strWorkflowId =
          PSChoiceBuilder.getFirstTagText(
    ...
    boolean communitiesEnabled =
        PSServer.getServerProps().getProperty("communities_enabled").trim().equalsIgnoreCase("yes");
    ...
    if (null != communityId && communitiesEnabled) {
      ...
      PSInternalRequest rh = PSServer.getInternalRequest(resourceName, request, null, true);

      if (rh == null) {
        // bug!
        throw new RuntimeException("No handler found for " + resourceName);
      }
      ...
      if (null != communities) {
        ...
        Element el = walker.getNextElement(nodeName, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
        while (el != null) {
          commWorkflowIds.add(el.getAttribute("workflowid"));
          el = walker.getNextElement(nodeName, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
        }
      }
      ...
      boolean isInclusionary =
          null != m_workflowIds
              ? m_ce.getWorkflowInfo().getType().equals(PSWorkflowInfo.TYPE_INCLUSIONARY)
              : true;
      while (iter.hasNext()) {
        Object id = iter.next();
        if (null == m_workflowIds) resultWorkflows.add(id);
        else if (("" + m_ce.getWorkflowId()).equals((String) id)) resultWorkflows.add(id);
        else if (isInclusionary) {
          if (m_workflowIds.contains(id)) resultWorkflows.add(id);
        } else {
          if (!m_workflowIds.contains(id)) resultWorkflows.add(id);
        }
      }
      ...
      if (suppliedWorkflowId) {
        ...
        while (!found && testIds.hasNext()) {
          ...
          if (testId != null && strWorkflowId.equals(testId)) found = true;
        }

        if (!found) {
          errMsg =
              "The supplied workflow id ("
                  + strWorkflowId
                  + ") is not allowed for community id ("
                  + communityId
                  + ").";
        }
      } else {
        String NON_UNIQUE_MSG =
            "Couldn't find unique workflow for "
                + "community id '"
                + communityId
                + "' and content type id '"
                + m_ce.getContentType()
        ...
        switch (resultWorkflows.size()) {
          ...
          default:
            ...
            if (el != null) docCommunityId = walker.getElementData(el);
      ...
      if (null != errMsg) throw new RuntimeException(errMsg);
    }
  ...
  private String getRequestorsCommunity(PSRequest request) {
    ...
    if (obj != null) {
      ...
      if (null == communityId || communityId.trim().length() == 0) communityId = null;
    }
    return communityId;
  }
  ...
  private void fixLangId(Document doc, PSRequest request) {
    ...
    if (langId == null || langId.trim().length() == 0) return;

    PSXmlTreeWalker walker = new PSXmlTreeWalker(doc);
    Element elem = walker.getNextElement(IPSHtmlParameters.SYS_LANG);
    if (elem != null) {
      Node childNode = elem.getFirstChild();
      if (childNode instanceof Text) ((Text) childNode).setData(langId);
    }
  }
  ...
  private class PSMappingContext {
    ...
    public void addColumnMapping(
      ...
      try {
        ...
        PSDataMapping dataMapping = new PSDataMapping(parentElement + "/" + xmlFieldName, beCol);
        ...
        if (updateDocMapping == null)
          dataMapping =
              new PSDataMapping(
                  m_updateDtdBuilder.getRootName() + "/" + parentElement + "/" + xmlFieldName,
                  beCol);
        else dataMapping = new PSDataMapping(updateDocMapping, beCol);

        m_updateMapper.add(dataMapping);

        // add to both dtd's
        m_queryDtdBuilder.addElement(xmlFieldName, PSDtdBuilder.OCCURS_ONCE, parentElement);
        m_updateDtdBuilder.addElement(xmlFieldName, PSDtdBuilder.OCCURS_ONCE, parentElement);
      } catch (IllegalArgumentException e) {
        throw new IllegalArgumentException(e.getLocalizedMessage());
      }
    }
    ...
    public void addSystemColumns(PSBackEndTable beTable, Map selectionKeys, boolean isSystemTable) {
      ...
      addKeyColumn(beTable, IPSConstants.ITEM_PKEY_CONTENTID, m_contentIdParamName);

      if (!isSystemTable) {
        addKeyColumn(beTable, IPSConstants.ITEM_PKEY_REVISIONID, m_revisionIdParamName);
        ...
        if (m_fieldSet.getType() == PSFieldSet.TYPE_COMPLEX_CHILD) {
          ...
          if (m_fieldSet.isSequencingSupported()) {
            addKeyColumn(beTable, IPSConstants.CHILD_SORT_KEY, IPSConstants.CHILD_SORT_KEY, null);
          }
        }
      }
    ...
    private void addKeyColumn(
        ...
        String columnName,
        String xmlFieldName,
        String updateHtmlParamName) {
      try {
        PSBackEndColumn beCol = new PSBackEndColumn(beTable, columnName);
        PSHtmlParameter updateParam = null;
        if (updateHtmlParamName != null) updateParam = new PSHtmlParameter(updateHtmlParamName);
    ...
    public void addSelectionKeys(
      ...
      selectionKeys.put(IPSConstants.ITEM_PKEY_CONTENTID, m_contentIdParamName);

      if (!isSystemTable) {
        selectionKeys.put(IPSConstants.ITEM_PKEY_REVISIONID, m_revisionIdParamName);

        if (includeChildKeys)
          selectionKeys.put(
              IPSConstants.CHILD_ITEM_PKEY, PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME);
      }
    }
  ...
  private class PSBinaryQueryHandler {
    ...
    public PSBinaryQueryHandler(
      ...
      if (ctx == null
          || xmlFieldName == null
          || xmlFieldName.trim().length() == 0
          || fieldSet == null
          || beCol == null) {
        throw new IllegalArgumentException("One or more params is null or empty");
      }
      ...
      PSDtdBuilder binQueryDtd =
          new PSDtdBuilder(PSUniqueObjectGenerator.makeUniqueName("CopyDTD"));
      binQueryDtd.addElement(
          ctx.getParentElement(), PSDtdBuilder.OCCURS_ONCE, binQueryDtd.getRootName());

      // make new ctx using current update stuff
      PSMappingContext binCtx =
          new PSMappingContext(
              binQueryMapper, ctx.getUpdateMapper(), binQueryDtd, ctx.getUpdateDtd(), fieldSet);
      ...
      Map selKeys = new HashMap();
      binCtx.addSelectionKeys(
          selKeys, isSystemTable, (fieldSet.getType() == PSFieldSet.TYPE_COMPLEX_CHILD));

      // create the dataset and store its name
      m_resourceName =
          PSApplicationBuilder.createQueryDataset(
              m_app, binQueryMapper, selKeys.entrySet().iterator(), null, binQueryDtd, true);
    }
    ...
    public List execute(PSExecutionData data, Document resultDoc)
      ...
      String requestName = PSCommandHandler.createRequestName(m_app.getName(), m_resourceName);
      IPSInternalResultHandler rh =
          (IPSInternalResultHandler) PSServer.getInternalRequestHandler(requestName);

      if (rh == null) {
        // bug!
        throw new RuntimeException("No binary query handler found for " + requestName);
      }
      ...
      Element row =
          walker.getNextElement(m_fieldSet.getName(), PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);

      while (row != null) {
        // Extract the keys and add them as params to the request
        String strKey = walker.getElementData(m_contentIdParamName, false);
        request.setParameter(m_contentIdParamName, strKey);

        strKey = walker.getElementData(m_revisionIdParamName, false);
        request.setParameter(m_revisionIdParamName, strKey);

        if (m_fieldSet.getType() == PSFieldSet.TYPE_COMPLEX_CHILD) {
          strKey = walker.getElementData(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME, false);
          request.setParameter(PSContentEditorHandler.CHILD_ROW_ID_PARAM_NAME, strKey);
        }

        // execute the query - this will add the data to the row
        PSPurgableTempFile result = executeQuery(rh, data, resultDoc, row);
        if (result != null) fileList.add(result);

        // get the next row
        row = walker.getNextElement(m_fieldSet.getName(), PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
      }
    ...
    private PSPurgableTempFile executeQuery(
      ...
      try {
        ...
        if (result != null) {
          ...
          Element fileEl = PSXmlDocumentBuilder.addEmptyElement(doc, row, m_xmlFieldName);
          fileEl.setAttribute(PSXmlFieldExtractor.XML_URL_REFERENCE_ATTRIBUTE, u.toExternalForm());
        }
