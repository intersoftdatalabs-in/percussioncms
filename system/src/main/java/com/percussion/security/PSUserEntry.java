...
public class PSUserEntry extends PSEntry {
  ...
  public boolean isMatch(PSEntry entry) {
     ...
     */
    if (this.getClass().isInstance(entry)) return super.isMatch(entry); // do the default comparison
    else if (entry.isFilter()) return entry.isMatch(this); // let the filter do the check
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof com.percussion.security.PSUserEntry)) return false;
    ...
    if ((srcSize != 0) && !ArrayUtils.isEquals(m_groups, dest.m_groups)) return false;

    // also need to consider the user attributes!
    if (m_attributes == null ^ dest.m_attributes == null) return false;
    else if (m_attributes != null && !ArrayUtils.isEquals(m_attributes, dest.m_attributes)) {
      return false;
    }

    if (m_roles == null ^ dest.m_roles == null) return false;
    else if (m_roles != null && !ArrayUtils.isEquals(m_roles, dest.m_roles)) {
      return false;
    }
  ...
  public int hashCode() {
    return super.hashCode()
        + (m_groups == null ? 0 : m_groups.hashCode())
        + (m_roles == null ? 0 : m_roles.hashCode())
  ...
  public boolean isSignatureMatch(PSEntry entry) {
    ...
    if (!(entry instanceof PSUserEntry)) return false;

    PSUserEntry userEntry = (PSUserEntry) entry;
    if ((m_signature == null) && (userEntry.m_signature == null)) return true;
  ...
  public Element getUserEntryStatus(Document doc) {
    ...
    entry.setAttribute("signature", (m_signature == null) ? "null" : m_signature);

    String name = getName();

    entry.setAttribute("userName", (getName() == null) ? "null" : name);
