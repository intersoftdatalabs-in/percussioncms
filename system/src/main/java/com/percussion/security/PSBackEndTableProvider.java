...
public class PSBackEndTableProvider extends PSSecurityProvider {
  ...
  PSBackEndTableProvider(Properties props, String providerInstance) {
    ...
    if (props == null) throw new IllegalArgumentException("Null properties not allowed!");

    if (providerInstance == null)
      throw new IllegalArgumentException("Null provider instance not allowed!");

    m_backendConnection = new PSBackEndConnection(props);
    m_defaultDirectoryProvider =
        new PSProvider(
            PSBackEndTableDirectoryCataloger.class.getName(), PSProvider.TYPE_DIRECTORY, null);
  ...
  public PSUserEntry authenticate(String uid, String pw, CallbackHandler callbackHandler)
    ...
    try {
      ...
      if (!result.next()) throw new PSAuthenticationFailedException(SP_NAME, m_spInstance, uid);

      // get the password from the current row
      String password = result.getString(m_backendConnection.getPasswordColumn());
      ...
      if (filter != null) {
        ...
        if (!authenticationValid) {
          ...
          if (!encodedPw.equals(password)) {
            ...
            log.info(
                "Security Update: Re-encrypting password for database user: {} from legacy"
                    + " algorithm {} to current algorithm {}",
                uid,
            ...
            auditlogUserActivity(
                ...
                String.format(
                    ...
                    uid, filter.getLegacyAlgorithm(), filter.getAlgorithm()));
          }
        }
      }
          ...
          if (filter != null) {
            ...
            auditlogUserActivity(
                ...
                String.format(
                    "Security Update: Re-encrypting password for database user: {%s} from legacy"
                        + " algorithm: {%s} to current algorithm: {%s}",
                    uid, "Plain Text", filter.getAlgorithm()));
          }
        }
      }
      ...
      return new PSUserEntry(uid, 0, null, attributeValues, PSUserEntry.createSignature(uid, pw));
    } catch (SQLException | PSEncryptionException e) {
      throw new PSAuthenticationFailedException(SP_NAME, m_spInstance, uid, e.toString());
    } finally {
      if (result != null)
        try {
          result.close();
  ...
  public IPSSecurityProviderMetaData getMetaData() {
    if (m_metaData == null) {
      ...
      m_metaData =
          new PSBackEndTableProviderMetaData(
              this, m_backendConnection.getUserColumn(), m_backendConnection.getTable(), attrNames);
    }
