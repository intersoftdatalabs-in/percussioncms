...
/**
 * The PSFilterEntry class defines the implementation of a filter entry within E2. Filters are used
 * to define security provider specific user or group matching strings. For instance, an LDAP
 * compliant filter string to check the user's organizational unit (ou) is:
 *
 * <pre><code>
 *      (ou=Engineering)
 * </code></pre>
...
public abstract class PSFilterEntry extends PSEntry {
   ...
   * attributes, etc.
   *
   * @return <code>true</code> is always returned for the PSFilterEntry class
   */
