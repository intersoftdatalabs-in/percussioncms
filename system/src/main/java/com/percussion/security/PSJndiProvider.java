...
public abstract class PSJndiProvider extends PSSecurityProvider {
  ...
  PSJndiProvider(
    ...
    if (!isDirectoryProvider) {
       ...
       * supplied properties.
       */
      m_providerClassName = PSJndiUtils.getProperty(providerAttrs, PROPS_PROVIDER_CLASS_NAME, true);

      m_providerURL = PSJndiUtils.getProperty(providerAttrs, PROPS_PROVIDER_URL, true);

      // auth scheme describes how we process passwords before comparison
      m_authenticationScheme = PSJndiUtils.getProperty(providerAttrs, PROPS_AUTH_SCHEME, false);

      m_authenticationPrincipal =
          PSJndiUtils.getProperty(providerAttrs, PROPS_AUTH_PRINCIPAL, false);

      m_authenticationCredential = PSJndiUtils.getProperty(providerAttrs, PROPS_AUTH_CRED, false);
      if (null == m_authenticationCredential) m_authenticationCredential = "";

      m_principalAttribute = PSJndiUtils.getProperty(providerAttrs, PROPS_ATTR_PRINCIPAL, false);

      m_credentialAttribute = PSJndiUtils.getProperty(providerAttrs, PROPS_ATTR_CRED, false);

      // get the class name of the password filter, if provided
      String filterClass = PSJndiUtils.getProperty(providerAttrs, PROPS_CRED_FILTER, false);

      if (filterClass != null) m_credentialFilter = PSJndiUtils.initPasswordFilter(filterClass);
      ...
      if ((m_authenticationScheme == null) || (m_authenticationScheme.length() == 0))
  ...
  public void setProviderProperties(PSDirectory directory, PSAuthentication authentication) {
    ...
    m_authenticationPrincipal = authentication.getUser(directory.getProviderUrl());
  ...
  protected Object filterPassword(String pw) {
    ...
    Object ret = pw;
    if (m_credentialFilter != null) ret = m_credentialFilter.encrypt(pw);
  ...
  public synchronized InitialDirContext createInitialContext(String url) throws NamingException {
    ...
    if (m_authenticationPrincipal != null) {
      String prefix = "";
      if (m_principalAttribute != null) prefix = m_principalAttribute + "=";
      env.put(Context.SECURITY_PRINCIPAL, prefix + m_authenticationPrincipal);
  ...
  Properties getConnectionProperties() {
    ...
    if (m_providerURL != null) props.setProperty(PROPS_PROVIDER_URL, m_providerURL);
    ...
    if (m_principalAttribute != null) props.setProperty(PROPS_ATTR_PRINCIPAL, m_principalAttribute);
    if (m_credentialAttribute != null) props.setProperty(PROPS_ATTR_CRED, m_credentialAttribute);
    if (m_credentialFilter != null)
      props.setProperty(PROPS_CRED_FILTER, m_credentialFilter.getClass().getName());
