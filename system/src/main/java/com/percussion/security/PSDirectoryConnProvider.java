...
public class PSDirectoryConnProvider extends PSJndiProvider {
   ...
   */
  public PSDirectoryConnProvider(
      Properties providerAttrs, String providerInstance, boolean prepCataloging)
      throws NamingException {
  ...
  public PSUserEntry authenticate(String uid, String pw, CallbackHandler callbackHandler)
    ...
    // fail if null uid
    if (uid == null) {
      throw new PSAuthenticationFailedException(SP_NAME, m_spInstance, "null");
    }
    ...
    if (provider == null)
      throw new IllegalStateException("must set a directory provider before authenticating");

    PSServerConfiguration config = PSServer.getServerConfiguration();
    PSDirectorySet directorySet = config.getDirectorySet(provider.getReference().getName());
    ...
    while (references.hasNext()) {
      try {
        PSDirectory directory = config.getDirectory(((PSReference) references.next()).getName());
        if (directory == null) {
        ...
        try {
          ...
          env.put(Context.PROVIDER_URL, PSJndiUtils.getEncodedProviderUrl(getProviderURL()));
          env.put(Context.SECURITY_AUTHENTICATION, getAuthenticationScheme());

          String objectAttributeName =
              directorySet.getRequiredAttributeName(PSDirectorySet.OBJECT_ATTRIBUTE_KEY);
          String fullUserName =
              PSJndiUtils.getFullUserName(uid, objectAttributeName, getBaseContext());
          env.put(Context.SECURITY_PRINCIPAL, fullUserName);
          env.put(
              Context.SECURITY_CREDENTIALS,
              PSJndiUtils.getFullPassword(encryptedPassword, getCredentialAttribute()));
          ...
          return new PSUserEntry(
              uid, 0, groupEntries, attributeValues, PSUserEntry.createSignature(uid, pw));
        } catch (NamingException e) {
  ...
  private DirContext authenticate(
    ...
    Hashtable<String, Object> environment =
        getEnvironment(user, userAttributeName, password, env, scope, authentication, directory);
  ...
  private Hashtable<String, Object> getEnvironment(
    ...
    env.put(Context.SECURITY_PRINCIPAL, authentication.getPrincipal(directory.getProviderUrl()));
    ...
    try {
      ...
      if (results.hasMore()) {
         ...
         */
        if (resName.startsWith("\"") && resName.endsWith("\"") && resName.length() > 1) {
      ...
      if (objectAttributeName != null && objectAttributeUser != null) {
        ...
        String baseContext = "";
        if (isRelative) {
          if (objectAttributePath != null) baseContext += objectAttributePath + ",";

          baseContext += getBaseContext();
        } else if (objectAttributePath != null) baseContext += objectAttributePath;

        env.put(
            Context.SECURITY_PRINCIPAL,
            PSJndiUtils.getFullUserName(objectAttributeUser, objectAttributeName, baseContext));
        env.put(
            Context.SECURITY_CREDENTIALS,
  ...
  public void setDirectoryProvider(
    ...
    if (provider != null) {
      if (!provider.isProviderFor(PSDirectoryServerCataloger.class.getName()))
        throw new IllegalArgumentException("provider must be for PSDirectoryServerCataloger");

      props = new Properties();
      props.put(PSSecurityProvider.PROVIDER_NAME, provider.getReference().getName());
    }
  ...
  private void setDirectoryCataloger(
    ...
    if (provider == null) m_directoryCataloger = null;
    else if (config == null)
      m_directoryCataloger = (IPSDirectoryCataloger) instantiateProvider(provider, properties);
    else
