...
public class PSJndiUtils {
  ...
  public static String getProperty(Properties properties, String name, boolean required) {
    ...
    if (property == null && !required) return property;
  ...
  public static IPSPasswordFilter initPasswordFilter(String extensionName) {
    ...
    try {
      ...
      IPSExtension ext = PSServer.getExtensionManager(null).prepareExtension(ref, null);
      if (!(ext instanceof IPSPasswordFilter))
        throw new IllegalArgumentException("Supplied extension is not a password filter.");
  ...
  private static void appendFilterCond(StringBuilder filter, String attrName, String value) {
    if (value == null || value.trim().length() == 0) value = WILDCARD_SEARCH;
  ...
  public static DirContext createContext(PSDirectoryDefinition directoryDef, String url)
      throws NamingException {
    if (directoryDef == null) throw new IllegalArgumentException("directoryDef cannot be null");

    PSAuthentication auth = directoryDef.getAuthentication();
    PSDirectory dir = directoryDef.getDirectory();

    if (StringUtils.isBlank(url)) url = dir.getProviderUrl();
    ...
    env.put(Context.SECURITY_PRINCIPAL, auth.getPrincipal(dir.getProviderUrl()));
  ...
  public static void addConnectionPooling(Map<String, Object> env) {
    ...
    String securityProtocol =
        StringUtils.defaultString((String) env.get(Context.SECURITY_PROTOCOL));
    if (securityProtocol.equals("ssl")) {
      String sslFactory =
          StringUtils.defaultString((String) env.get(PSServerConfiguration.SECURE_SOCKET_FACTORY));
  ...
  public static String getFullUserName(String uid, String principleAttribute, String baseContext) {
    ...
    uid = uid.trim();
    if (uid.length() == 0) throw new IllegalArgumentException("uid cannot be empty");

    if (principleAttribute != null) {
      principleAttribute = principleAttribute.trim();
      if (principleAttribute.length() == 0)
        throw new IllegalArgumentException("principleAttribute cannot be empty");
    }
  ...
  public static String getFullPassword(String pw, String credentialAttribute) {
    ...
    if (credentialAttribute != null) {
      ...
      if (credentialAttribute.length() == 0)
        throw new IllegalArgumentException("credentialAttribute cannot be empty");
    }
  ...
  public static String escapeDnComponent(String name) {
    ...
    for (int i = 0; i < name.length(); i++) {
      ...
      if (isDnEscapeChar(test)) {
        ...
        if (test == DN_ESCAPE_CHAR && i < name.length() - 1 && isDnEscapeChar(name.charAt(i + 1))) {
  ...
  public static String unEscapeDnComponent(String name) {
    ...
    for (int i = 0; i < name.length(); i++) {
      ...
      if (name.charAt(i) == DN_ESCAPE_CHAR
          && (i < name.length() - 1 && isDnEscapeChar(name.charAt(i + 1)))) {
  ...
  public static String getEncodedProviderUrl(String providerURL) throws NamingException {
    if (providerURL == null || providerURL.trim().length() == 0)
      throw new IllegalArgumentException("providerURL may not be null or empty");
  ...
  public static String getProtocol(String url) {
    ...
    url = url.trim();
    if (url.length() == 0) throw new IllegalArgumentException("url cannot be empty");

    String protocol = "";
    int pos = url.indexOf("://");
    if (pos != -1) protocol = url.substring(0, pos);
  ...
  public static URL getURL(String url) throws MalformedURLException {
    ...
    if (!StringUtils.isBlank(protocol)) url = "http" + url.substring(protocol.length());
