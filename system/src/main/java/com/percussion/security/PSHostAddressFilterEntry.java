...
public class PSHostAddressFilterEntry extends PSFilterEntry {
  ...
  public PSHostAddressFilterEntry(java.lang.String filter, int accessLevel)
    ...
    if (filter.startsWith("*")) {
      ...
      throw new IllegalArgumentException(PSErrorManager.createMessage(NOT_FILTER_ERROR, filter));
    }
  }
  ...
  public boolean passesFilter(PSEntry entry) {
     ...
     */
    if (m_type == TYPE_ALL) return true;
    else if (m_type == TYPE_FILTER_ADDRESS) return isAddressMatch(entry.getName());
