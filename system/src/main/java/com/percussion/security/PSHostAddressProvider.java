...
public class PSHostAddressProvider extends Object {
  ...
  public PSUserEntry[] getAuthenticatedUserEntries(PSRequest req) {
    ...
    if ((addr != null) && (name != null)) {
      if (name.equals(addr)) name = null;
    } else if ((name == null) && (addr == null)) return null;
    ...
    if (addr != null) {
      ...
      if (!isUserEntryDefined(req, entry)) sess.addAuthenticatedUserEntry(entry);

      for (pos = addr.indexOf('.'); pos != -1; pos = addr.indexOf('.', pos + 1)) {
        entry = new PSUserEntry(addr.substring(0, pos) + ".*", 0, null, null, null);
        entries.add(entry);
    ...
    if (name != null) {
      ...
      for (pos = name.lastIndexOf('.'); pos != -1; pos = name.lastIndexOf('.', pos - 1)) {
        entry = new PSUserEntry("*." + name.substring(pos + 1), 0, null, null, null);
        entries.add(entry);
