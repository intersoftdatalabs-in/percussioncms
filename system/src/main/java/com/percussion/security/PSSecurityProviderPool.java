...
public class PSSecurityProviderPool {
  ...
  public static synchronized void init(PSServerConfiguration config) {
    ...
    if ((pInstances == null) || (pInstances.size() == 0)) return;
  ...
  private static IPSDirectoryCataloger initDirectoryCataloger(
    ...
    PSReference directorySetReference =
        new PSReference(dirSet.getName(), PSDirectorySet.class.getName());
    PSProvider directoryProvider =
        new PSProvider(
            PSDirectoryServerCataloger.class.getName(),
            PSProvider.TYPE_DIRECTORY,
            directorySetReference);
    Properties props = new Properties();
    props.setProperty(PSSecurityProvider.PROVIDER_NAME, dirSet.getName());
    IPSDirectoryCataloger dirCat =
        (IPSDirectoryCataloger)
            PSSecurityProvider.instantiateProvider(directoryProvider, props, config);
    if (dirCat != null) dirCat.setName(dirSet.getName());
  ...
  private static IPSInternalRoleCataloger initRoleCataloger(
    ...
    PSReference roleProviderReference =
        new PSReference(roleProvider.getName(), PSRoleProvider.class.getName());
    PSProvider provider =
        new PSProvider(
            PSRoleCataloger.class.getName(), PSProvider.TYPE_ROLE, roleProviderReference);

    Properties props = new Properties();
    props.put(PSSecurityProvider.PROVIDER_NAME, roleProvider.getName());
    IPSInternalRoleCataloger roleCat =
        (IPSInternalRoleCataloger) PSSecurityProvider.instantiateProvider(provider, props, config);
  ...
  public static synchronized IPSSecurityProviderMetaData getProviderMetaData(String providerType) {
    if (providerType == null) throw new IllegalArgumentException("providerType cannot be null");
   ...
   */
  public static synchronized List<IPSDirectoryCataloger> getAllDirectoryCatalogers() {
   ...
   * @return The list, never <code>null</code>, may be empty.
   */
  public static synchronized List<IPSInternalRoleCataloger> getAllRoleCatalogers() {
   ...
   * @return The cataloger, never <code>null</code>.
   */
  public static synchronized IPSInternalRoleCataloger getDefaultRoleCataloger() {
   ...
   * @return The cataloger, never <code>null</code>.
   */
  public static synchronized IPSDirectoryCataloger getDefaultDirectoryCataloger() {
  ...
  private static synchronized void addInstance(
    ...
    switch (spType) {
      ...
      case PSSecurityProvider.SP_TYPE_DIRCONN:
        try {
          ...
          dirConn.setDirectoryProvider(spInst.getDirectoryProvider(), providerProps, config);
          break;
        } catch (Exception e) {
          String spName = PSDirectoryConnProvider.SP_NAME;

          Object[] args = {spName, spInstName, PSException.getStackTraceAsString(e)};
    ...
    if (provider != null) {
      IPSDirectoryCataloger dirCat =
          (IPSDirectoryCataloger) PSSecurityProvider.instantiateProvider(provider, props, config);
      if (dirCat != null) {
