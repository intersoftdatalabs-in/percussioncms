...
public class PSThreadRequestUtils {
  ...
  public static PSRequest initServerThreadRequest() {
    ...
    if (PSRequestInfo.isInited()) {
      log.debug(
          "Thread request already initialized method should run only once per thread",
          new Throwable());
  ...
  public static boolean initUserThreadRequestByToken(PSSecurityToken token) {
    ...
    if (!PSRequestInfo.isInited()) PSRequestInfo.initRequestInfo(new HashMap<String, Object>());
  ...
  public static PSRequest changeToInternalRequest(PSRequest req, boolean systemUser) {
    ...
    if (systemUser) req = PSRequest.getContextForRequest();
    else req = req.cloneRequest();

    PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_PSREQUEST, req);
    PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_ORIG_MAP, currentRequestMap);

    log.debug(
        "Changing to internal request for user "
            + currentUser
            + "on thread "
  ...
  public static PSRequest restoreOriginalRequest() {
    ...
    log.debug(
        "Restoring original request on thread "
            + Thread.currentThread().getName()
            + " id "
            + Thread.currentThread().getId());

    Map<String, Object> origRequestMap =
        (Map<String, Object>) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_ORIG_MAP);
    PSRequest currentRequest =
  ...
  public static void updateHttpSession(HttpServletRequest req, PSUserSession sess) {
    ...
    if (PSRequestInfo.isInited()) PSRequestInfo.setRequestInfo(PSRequestInfo.SUBJECT, subject);
  ...
  private static void updateRequestInfo(PSRequest req) {
    ...
    if (req.hasUserSession()) {
      ...
      if (locale != null) PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_LOCALE, locale);
    }
  ...
  public static IPSRequestContext getReqCtx() {
    IPSRequestContext requestContext =
        (IPSRequestContext) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUESTCONTEXT);
    if (requestContext == null) {
      ...
      PSRequestInfo.setRequestInfo(
          PSRequestInfo.KEY_PSREQUESTCONTEXT, new PSRequestContext(request));
    }
  ...
  public static PSUserInfo getUserInfo() throws PSCmsException {
    ...
    if (userInfo == null) userInfo = new PSUserInfo(getReqCtx());
  ...
  public static boolean isInternalUser() throws PSDataExtractionException {

    boolean isInternalUser = false;

    String userCtx = "User/Name";
    PSLiteralSet userSet =
        (PSLiteralSet)
