...
public class PSWebServerProvider extends PSSecurityProvider {
  ...
  public PSWebServerProvider(Properties props, String providerInstance) {
    ...
    String roleListHeaderNameProp = m_properties.getProperty(USER_ROLE_LIST_HEADER);
    String roleListDelimiterProp = m_properties.getProperty(ROLE_LIST_DELIMITER);
  ...
  public PSUserEntry authenticate(String uid, String pw, CallbackHandler callbackHandler)
      throws PSAuthenticationFailedException, IOException, UnsupportedCallbackException {
    ...
    callbackHandler.handle(new Callback[] {headersCallback, remoteUserCallBack});
  ...
  PSUserEntry authenticate(String remoteUserName, Map<String, String> headers)
    ...
    if (var != null) {
      ...
      if (var != null) parseCertificateString("ca/", var, attribs);
    ...
    if (StringUtils.isBlank(authType)) {
      throw new PSAuthenticationFailedException(SP_NAME, m_spInstance, remoteUserName);
    }

    // determine if we have custom headers defined
    if (m_properties != null) {
      String userHeaderName = m_properties.getProperty(AUTHENTICATED_USER_HEADER);

      String roleListHeaderName = m_properties.getProperty(USER_ROLE_LIST_HEADER);

      String user = null;
      String roles = null;
      if (userHeaderName != null && roleListHeaderName != null) {
        user = PSSecurityFilter.getNormalizedHeader(userHeaderName, headers);
        roles = PSSecurityFilter.getNormalizedHeader(roleListHeaderName, headers);
      }

      PSRoleEntry[] roleEntries = null;
      if (roles != null) {
        String delim = m_properties.getProperty(ROLE_LIST_DELIMITER);
        if (StringUtils.isBlank(delim)) delim = DELIMITER;
      ...
      String name =
          PSSecurityFilter.getNormalizedHeader(IPSCgiVariables.CGI_AUTH_USER_NAME, headers);
      if (name == null) {
        name = remoteUserName;
      }

      if (name != null) entry = new PSUserEntry(name, 0, null, null, null);
    }
  ...
  public static void parseCertificateString(
    ...
    if (attribHeader == null) attribHeader = "";
    else attribHeader = attribHeader.toLowerCase();
