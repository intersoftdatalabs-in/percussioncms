  ...
  public Document processResultDocument(
    ...
    try {
      ...
      if (StringUtils.isBlank(objectId)) {
        request.printTraceMessage("No object id found. Skipping ACL creation.");
        return resultDoc;
      }

      // fixme this needs to be modified when the ACL stuff is reworked
      IPSGuidManager guidMgr = PSGuidManagerLocator.getGuidMgr();
      objectGuid = guidMgr.makeGuid(Long.parseLong(objectId), objectTypeEnum);
      ...
      PSCommunity comm =
          roleMgr.loadCommunity(guidMgr.makeGuid(Long.parseLong(commId), PSTypeEnum.COMMUNITY_DEF));

      IPSAclEntry communityEntry =
          acl.createEntry(
              new PSTypedPrincipal(comm.getName(), PrincipalTypes.COMMUNITY),
