...
public class PSJndiGroupProvider implements IPSGroupProvider {
  ...
  public PSJndiGroupProvider(
      ...
      PSDirectoryDefinition directoryDef,
      String userObjectAttrName)
      throws NamingException {
    if (groupProviderDef == null)
      throw new IllegalArgumentException("groupProviderDef may not be null");

    if (directoryDef == null) throw new IllegalArgumentException("directoryDef may not be null");

    if (StringUtils.isBlank(userObjectAttrName))
      throw new IllegalArgumentException("userObjectAttrName may not be null or empty");
  ...
  public boolean isMember(String user, String group) throws PSSecurityException {
    ...
    if (!isGroupSupported(group))
      throw new IllegalArgumentException("group is not supported by this provider");
    ...
    try {
      ...
      if (results.hasMore()) {
        ...
        if (result.isRelative()) {
          fullName = PSJndiUtils.getCompoundName(fullName, m_dirBaseCtx).toString();
        }
      }
  ...
  // see IPSGroupProvider interface
  public Collection<IPSTypedPrincipal> getGroupMembers(Collection<? extends Principal> groups) {
    ...
    for (Principal group : groupList) {
      ...
      if (!isGroupSupported(groupName)) continue;
  ...
  private List<IPSTypedPrincipal> getGroupMembers(String groupName, int level) {
    ...
    try {
      ...
      System.arraycopy(memberListAttributes, 0, memberAttrs, 1, size);
       ...
       */
      for (String staticMember : staticMemberList) {
        List<IPSTypedPrincipal> staticGroupMembers = getGroupMembers(staticMember, level);

        // if no "members" returned, just add it to the return list as a
        // member
        if (staticGroupMembers.isEmpty()) members.add(dnToPrincipal(staticMember));
        else members.addAll(staticGroupMembers);
      }
  ...
  private IPSTypedPrincipal dnToPrincipal(String dn, boolean treatAsGroup) throws NamingException {
    String member = PSJndiUtils.unEscapeDnComponent(PSJndiUtils.getCompoundName(dn).get(0));
    IPSTypedPrincipal principal;
    if (!treatAsGroup) {
      String user = StringUtils.substringAfter(member, "=");
      if (!StringUtils.isBlank(member)) principal = PSTypedPrincipal.createSubject(user);
      else principal = PSTypedPrincipal.createUndefined(member);
    } else principal = PSTypedPrincipal.createGroup(member);
  ...
  private Collection<IPSTypedPrincipal> filterMemberList(
      ...
      List<String> staticMemberList,
      List<String> dynamicMemberList)
      throws NamingException {
      ...
      if (foundOcs.contains(defOc.toLowerCase())) {
        ...
        while (members.hasNext()) {
          ...
          if (memberAttrType == PSJndiObjectClass.MEMBER_ATTR_STATIC) {
            ...
            if (shouldTreatAsGroup(member, false)) staticMemberList.add(member);
            else groupMembers.add(dnToPrincipal(member, false));
          } else {
  ...
  public boolean shouldTreatAsGroup(String dn, boolean doSearch) throws NamingException {
    ...
    if (!m_userObjectAttrName.equalsIgnoreCase(PRINCIPAL_GROUP_ATTR)) {
      ...
      if (dn.startsWith(PRINCIPAL_GROUP_ATTR) && isGroupSupported(dn)) treatAsGroup = true;
      else treatAsGroup = false;
    } else if (!isGroupSupported(dn)) treatAsGroup = false;
    else if (doSearch) {
      ...
      try {
        ...
        results = ctx.search("", filter, ctls);
        if (!results.hasMore()) treatAsGroup = false;
      } finally {
        if (results != null)
          try {
            results.close();
  ...
  public boolean isGroupSupported(String group) {
    ...
    try {
      ...
      while (nodes.hasNext() && !isValid) {
        if (groupName.endsWith(nodes.next().mi_groupLocation)) isValid = true;
      }
  ...
  private boolean isMember(String user, String group, int level) throws PSSecurityException {
    ...
    try {
      ...
      while (results.hasMore()) {
        ...
        if (isUserInMemberList(user, attrMap, staticMemberList, dynamicMemberList)) {
          return true;
        }
      }
      results.close();
      ...
      if (foundOcs.contains(defOc.toLowerCase())) {
        ...
        while (members.hasNext()) {
          ...
          if (memberAttrType == PSJndiObjectClass.MEMBER_ATTR_STATIC) {
            ...
            CompoundName userName = PSJndiUtils.getCompoundName(user);
            if (memberName.equals(userName)) return true;

            // not a match, add to the list
            checkedStaticMembers.add(member);
          } else {
            // dynamic list - need to search to verify
            if (isUserInDynamicList(user, member)) return true;
  ...
  private boolean isUserInDynamicList(String user, String searchUrl) throws NamingException {
    ...
    try {
      ...
      while (results.hasMoreElements() && !hasMatch) {
        ...
        if (cn.equals(memberName)) hasMatch = true;
      }
  ...
  public void setProviderUrl(String providerUrl) {
    if (providerUrl == null) {
      ...
      try {
        ...
        throw new IllegalArgumentException("providerUrl must be a valid URL");
      }
    }
  }
  ...
  private static String parseSearchUrl(String url, StringBuilder baseBuffer) {
    ...
    if (iDelim == -1) iDelim = url.length();
    else filter = url.substring(iDelim + delim.length());
    ...
    if (iPrefix != -1) {
      if (base.length() == iPrefix + 1) base = "";
      else base = base.substring(iPrefix + 1);
    }
    ...
    public GroupLocationInfo(String entry) throws InvalidNameException {
      try {
        ...
        if (entry.indexOf('?') > 0) {
          ...
          if (filter.length() > 0 && filter.charAt(0) != '(') filter = "(" + filter + ")";
        } else node = entry;
        mi_groupLocation = PSJndiUtils.getCompoundName(URLDecoder.decode(node, "UTF8"));
