...
public abstract class PSGenericCharacterSet implements PluggableCharacterSet {
  ...
  protected PSGenericCharacterSet(String encodingName, String rscsName) throws IOException {
    if (null == encodingName || 0 == encodingName.trim().length())
      throw new IllegalArgumentException("encodingName must not be null or empty");

    if (null == rscsName || 0 == rscsName.trim().length())
      throw new IllegalArgumentException("rscsName must not be null or empty");
  ...
  private void loadMappingDocument(InputStream encodingFile) throws IOException, SAXException {
    ...
    for (; null != el; el = tree.getNextElement(MAP_ELEMENT_NAME, nextFlags)) {
      String characterCodeString = tree.getElementData(CODE_ATTRIBUTE_NAME, false);
      try {
  ...
  public final boolean inCharset(int i) {
    if (i < 0) throw new IllegalArgumentException("Character code is less than zero.");

    if (i >= MAP_SIZE) return false;
    else return m_characterInEncoding[i];
  }
