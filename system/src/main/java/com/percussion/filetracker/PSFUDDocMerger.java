...
public class PSFUDDocMerger {
  ...
  public PSFUDDocMerger(Document snapshotDoc, Document remoteDoc)
    ...
    if (null == remoteDoc) throw new PSFUDNullDocumentsException("Remote document cannot be null");
  ...
  public void merge(PSFUDApplication app) throws PSFUDMergeDocumentsException {
    ...
    if (null == oldElem) {
      throw new PSFUDMergeDocumentsException(
          MainFrame.getRes().getString("errorRootElementNullSnapShot"));
    }
    if (null == newElem) {
      throw new PSFUDMergeDocumentsException(
          MainFrame.getRes().getString("errorRootElementNullRemote"));
    }
  ...
  private boolean cleanup(IPSFUDNode current) {
    ...
    if (current instanceof PSFUDFileNode) {
      ...
      if (!fileNode.isRemoteExists() && !fileNode.isLocalCopy()) {
        current.getElement().getParentNode().removeChild(current.getElement());
        return true;
      }
    } else {
      Object[] childs = current.getChildren();
      if (null == childs) return false;

      int count = childs.length;
      int removed = 0;
      for (int i = 0; i < count; i++) {
        if (cleanup((IPSFUDNode) childs[i])) removed++;
        childs[i] = null;
      }
      if (removed == count && !current.isRemoteExists() && !(current instanceof PSFUDAppNode)) {
  ...
  private void merge(Element elemOld, Element elemNew) throws PSFUDMergeDocumentsException {
    ...
    {
      throw new PSFUDMergeDocumentsException(
          MainFrame.getRes().getString("errorElementNullSnapshot"));
    }
    if (null == elemNew) // never happens
    {
      throw new PSFUDMergeDocumentsException(
          MainFrame.getRes().getString("errorElementNullRemote"));
    }
    ...
    if (!elemTag.equals(elemNew.getTagName())) {
      throw new PSFUDMergeDocumentsException(
          MainFrame.getRes().getString("errorElementTagsConfilct"));
    }
    ...
    if (elemTag.equals(PSFUDApplication.ELEM_APPLICATION)) {
      mapOldElem =
          getMap(elemOld, PSFUDApplication.ELEM_CATEGORY, PSFUDApplication.ATTRIB_CATEGORYID);
      mapNewElem =
          getMap(elemNew, PSFUDApplication.ELEM_CATEGORY, PSFUDApplication.ATTRIB_CATEGORYID);
      merge(mapOldElem, mapNewElem, elemOld);
    } else if (elemTag.equals(PSFUDApplication.ELEM_CATEGORY)) {
      mapOldElem =
          getMap(elemOld, PSFUDApplication.ELEM_CONTENTITEM, PSFUDApplication.ATTRIB_CONTENTID);
      mapNewElem =
          getMap(elemNew, PSFUDApplication.ELEM_CONTENTITEM, PSFUDApplication.ATTRIB_CONTENTID);
      merge(mapOldElem, mapNewElem, elemOld);
    } else if (elemTag.equals(PSFUDApplication.ELEM_CONTENTITEM)) {
      mapOldElem = getMap(elemOld, PSFUDApplication.ELEM_FILE, PSFUDApplication.ATTRIB_NAME);
      mapNewElem = getMap(elemNew, PSFUDApplication.ELEM_FILE, PSFUDApplication.ATTRIB_NAME);
      merge(mapOldElem, mapNewElem, elemOld);
  ...
  private HashMap getMap(Element elemParent, String child, String attrib) {
    ...
    for (int i = 0; (nl != null && i < nl.getLength()); i++) {
      ...
      if (null != id && id.trim().length() > 0) map.put(id, elem);
    }
    return map;
  }
  ...
  private void mergeFileElements(Element oldElem, Element newElem) {
    ...
    Element elemOld = getChildElement(oldElem, PSFUDApplication.ELEM_DOWNLOADURL);

    Element elemNew = getChildElement(newElem, PSFUDApplication.ELEM_DOWNLOADURL);

    if (null != elemNew) {
      if (null != elemOld) oldElem.replaceChild(elemNew.cloneNode(true), elemOld);
      else oldElem.appendChild(elemNew.cloneNode(true));
    }
  }
  ...
  private void setStatus(Element parent, int nCode, String msg) {
    ...
    Element status = PSFUDDocMerger.createChildElement(parent, IPSFUDNode.ELEM_STATUS);
  ...
  public static Element getChildElement(Element parent, String elemName) {
    ...
    if (null == parent || null == elemName) return elem;

    NodeList nl = parent.getChildNodes();
    if (null == nl || nl.getLength() < 1) return elem;
  ...
  public static Element createChildElement(Element parent, String elemName) {
    ...
    elem = (Element) parent.getOwnerDocument().createElement(elemName);
