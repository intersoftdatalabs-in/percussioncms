...
/**
 * This class wraps the content item node in the application and extends the abstract class
 * PSFUDAbstractNode. A Content Item node can have two states, viz. normal remote absent.
 *
...
public class PSFUDContentItemNode extends PSFUDAbstractNode {
   ...
   */
  public PSFUDContentItemNode(IPSFUDNode parent, Element elem) throws PSFUDNullElementException {
    super(parent, elem);
  }
   ...
   *
   * @return child nodes as an array Objects, can be <code>null</code> though a rare possibility.
   */
    ...
    if (null != nl) {
      ...
      for (int i = 0; i < nl.getLength(); i++) {
        try {
          ...
          if (null == temp || temp.trim().length() < 1) continue;
   ...
   *
   * @return string representation of the node for displaying in the application, never <code>null
   *     </code> or empty.
   */
  public String toString() {
    String value =
        m_Element.getAttribute(PSFUDApplication.ATTRIB_TITLE).trim()
            + " (ID:"
            + m_Element.getAttribute(PSFUDApplication.ATTRIB_CONTENTID).trim()
            + ")";
    return value;
  }
}
