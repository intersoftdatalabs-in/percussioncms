...
public class MainFrame extends JFrame {
  ...
  private void init() {
    ...
    KeyStroke ks = KeyStroke.getKeyStroke(KeyEvent.VK_N, Event.CTRL_MASK, true);
    url = getClass().getResource(getRes().getString("gif_fileConnect"));
    ImageIcon image = null;
    if (url != null) image = new ImageIcon(url);
    PSAction action =
        new PSAction("Connect", 'N', ks, image) {
          public void actionPerformed(ActionEvent e) {
            fileConnect();
          }
        };

    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("fileConnectDesc"));
    ...
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Exit", 'x', ks, image) {
          public void actionPerformed(ActionEvent e) {
            fileExit();
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("fileExitDesc"));
    ...
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Refresh", 'f', ks, image) {
          public void actionPerformed(ActionEvent e) {
            viewRefresh();
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("viewRefreshDesc"));
    ...
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Options", 'O', ks, image) {
          public void actionPerformed(ActionEvent e) {
            viewOptions();
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("viewOptionsDesc"));
    ...
    ks = KeyStroke.getKeyStroke(KeyEvent.VK_D, Event.CTRL_MASK, true);
    url = getClass().getResource(getRes().getString("gif_actionDownload"));
    image = null;
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Download", 'd', ks, image) {
          public void actionPerformed(ActionEvent e) {
            performAction(DOWNLOAD);
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("actionDownloadDesc"));
    m_actions.put(DOWNLOAD, action);

    ks = KeyStroke.getKeyStroke(KeyEvent.VK_U, Event.CTRL_MASK, true);
    url = getClass().getResource(getRes().getString("gif_actionUpload"));
    image = null;
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Upload", 'U', ks, image) {
          public void actionPerformed(ActionEvent e) {
            performAction(UPLOAD);
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("actionUploadDesc"));
    ...
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Launch", 'l', ks, image) {
          public void actionPerformed(ActionEvent e) {
            performAction(LAUNCH);
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("actionLaunchDesc"));
    ...
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("Purge local", 'g', ks, image) {
          public void actionPerformed(ActionEvent e) {
            performAction(PURGELOCAL);
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("actionPurgeLocalDesc"));
    ...
    ks = KeyStroke.getKeyStroke(KeyEvent.VK_B, Event.CTRL_MASK, true);
    url = getClass().getResource(getRes().getString("gif_helpAbout"));
    image = null;
    if (url != null) image = new ImageIcon(url);
    action =
        new PSAction("About", 'b', ks, image) {
          public void actionPerformed(ActionEvent e) {
            helpAbout();
          }
        };
    action.putValue(Action.SHORT_DESCRIPTION, getRes().getString("helpAboutDesc"));
  ...
  private void treeInit() throws PSFUDNullElementException {
    ...
    m_TreeTable = new PSFUDJTreeTable(new PSFUDNodeModel(new PSFUDAppNode(m_fudApplication)));
  ...
  private void fileConnect() {
    ...
    try {
      dlg = new PSFUDConnectDlg(this, APPTITLE + ": " + getRes().getString("ConnectString"));
    ...
    do {
      ...
      if (PSFUDConnectDlg.OK == nReturn) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(MainFrame.this, getRes().getString("errorAuthentication"));
          bRepeat = true;
      ...
      int option =
          JOptionPane.showConfirmDialog(
              ...
              MessageFormat.format(
                  getRes().getString("errorSaveExit"), new String[] {e.getMessage()}),
              ERROR,
  ...
  private void viewRefresh() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          MainFrame.this, getRes().getString("authenticationErrorReconnect"));
    } catch (PSFUDServerException e) {
      JOptionPane.showMessageDialog(MainFrame.this, PSExceptionUtils.getMessageForLog(e));
    } catch (PSFUDNullDocumentsException e) {
      JOptionPane.showMessageDialog(MainFrame.this, PSExceptionUtils.getMessageForLog(e));
      ...
      try {
        ...
        if (action.equals(LAUNCH)) {
          ...
          */
          ...
          if (!fileNode.isLocalCopy() && fileNode.STATUS_CODE_ABSENT != fileNode.getStatusCode()) {
            performAction(DOWNLOAD);
          }
          // launch now.
          fileNode.launch();
        } else if (action.equals(PURGELOCAL)) {
          if (fileNode.isLocalCopy() && getConfig().getIsPromptBeforePurge()) {
            int option =
                JOptionPane.showOptionDialog(
                    MainFrame.this,
                    MessageFormat.format(
                        getRes().getString("warningPurge"), new String[] {fileNode.getFileName()}),
                    WARNING,
                    JOptionPane.YES_NO_OPTION,
            ...
            if (JOptionPane.NO_OPTION == option) {
              System.out.println("File '" + fileNode.getLocalPath() + "' is not being purged...");
              continue;
            }
          }

          if (false == fileNode.purgeLocal()) throw new Exception(getRes().getString("errorPurge"));
          // purge local changes snapshot doc, set the flag to save
          bSaveSnapshot = true;
        } else if (action.equals(DOWNLOAD)) {
          if ((fileNode.isLocalCopy() && getConfig().getIsPromptBeforeOverwrite())
              || IPSFUDNode.STATUS_CODE_LOCALNEW == fileNode.getStatusCode()) {
            int option =
                JOptionPane.showOptionDialog(
                    MainFrame.this,
                    MessageFormat.format(
                        getRes().getString("warningOverwrite"),
            ...
            if (JOptionPane.NO_OPTION == option) {
              // Debug purpose
              System.out.println(
                  "File '" + fileNode.getLocalPath() + "' is skipped for downloading...");
              continue;
            }
          }
          setStatus(
              MessageFormat.format(
                  getRes().getString("statusDownloadingFile"),
                  new String[] {fileNode.getFileName()}));
          ...
          setStatus(
              MessageFormat.format(
                  getRes().getString("statusUploadingFile"),
                  new String[] {fileNode.getFileName()}));
          fileNode.upload();
          // purge local copy if option is set so
          if (getConfig().getIsPurgeLocalCopy()) fileNode.purgeLocal();
        ...
        JOptionPane.showMessageDialog(
            ...
            MessageFormat.format(
                ...
                new String[] {fileNode.getFileName(), action, e.getMessage()}));
      }
    }

    // save the snapshot if required
    try {
      if (bSaveSnapshot) m_fudApplication.save();
    } catch (Exception e) {
      JOptionPane.showMessageDialog(
          MainFrame.this, fileNode.getFileName() + " : " + e.getMessage());
    }
    m_TreeTable.repaint();
    setCursor(null);
    setStatus(null);
  ...
  public void updateMenu() {
    ...
    for (int i = 0; i < fileNodes.size(); i++) {
      ...
      if (!bLaunchable && (fileNode.isRemoteExists() || fileNode.isLocalCopy())) {
        bLaunchable = true;
      }
      ...
      if (!bUploadable && fileNode.isRemoteExists() && fileNode.isLocalCopy()) {
        bUploadable = true;
      }
    }
    ...
    try {
      ms_res =
          ResourceBundle.getBundle(
              "com.percussion.filetracker.MainFrame" + "Resources", Locale.getDefault());
    ...
    if (ms_res == null) {
      JOptionPane.showMessageDialog(
          ...
          "Resource file could not be loaded. Make sure you have right"
              + " build or contact Percussion Technical Support.",
          "Fatal Error",
  ...
  private class MouseHandler extends MouseAdapter {
    ...
    public void mouseReleased(MouseEvent event) {
      if (m_contextMenu != null && event.isPopupTrigger()) {
        updateMenu();
        m_contextMenu.show(event.getComponent(), event.getX(), event.getY());
      }
      setStatus(null);
    }
  ...
  public static String getVersionString() {
    ResourceBundle res =
        ResourceBundle.getBundle("com.percussion.filetracker.Version", Locale.getDefault());
    if (null == res) return "";

    String strVersion = res.getString("versionString");

    String tmp = res.getString("majorVersion") + "." + res.getString("minorVersion");
    if (null != tmp && tmp.length() > 0) strVersion += " " + tmp;
