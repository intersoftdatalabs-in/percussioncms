...
class PSStatusRenderer extends DefaultTreeCellRenderer {
  ...
  public PSStatusRenderer() {
    try {
      ...
      url = getClass().getResource(MainFrame.getRes().getString("iconAppNodeAbsent"));
      ms_IconAppAbsent = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconContentTypeNode"));
      ms_IconContentType = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconContentTypeNodeAbsent"));
      ms_IconContentTypeAbsent = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconContentItemNode"));
      ms_IconContentItem = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconContentItemNodeAbsent"));
      ms_IconContentItemAbsent = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconFileNode"));
      ms_IconFile = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconFileNodeAbsent"));
      ms_IconFileAbsent = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconFileNodeInSync"));
      ms_IconFileInSync = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconFileNodeRemoteNew"));
      ms_IconFileRemoteNew = new ImageIcon(url);

      url = getClass().getResource(MainFrame.getRes().getString("iconFileNodeLocalNew"));
  ...
  private void setIconAndToolTipText(IPSFUDNode node, boolean sel) {
    ...
    if (node instanceof PSFUDAppNode) {
      ...
      if (((PSFUDFileNode) node).isLocalCopy()) {
        if (sel) setForeground(Color.lightGray);
  ...
  public Component getTreeCellRendererComponent(
    ...
    if (!(value instanceof IPSFUDNode)) return this;
