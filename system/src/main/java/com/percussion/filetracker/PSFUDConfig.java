...
/**
 * This class wraps the configuration document (rxftconfig.xml). Has required set and get methods
 * for parameters such as userid, password etc. No parameter is stored locally and obtioned walking
 * through the XML document on demand except the password. Password is never stored part of the
 * document. Also has save() method that saves the config document, again password is never saved as
 * part of the file.
 */
public class PSFUDConfig {
  /**
   * Default and constructor. Configuration file is loaded first. If load succeeds, current element
   * is obtained walking throw the socument. Current element is created if one does not exist.
   * Current element is the one that stores the last connected server alias and userid.
   *
   * @throws PSFUDConfigFileNotFoundException if the configuartion file not found.
   * @throws SAXException if the loaded XML document is not parsable.
   * @throws IOException if load fails for other reason.
   */
  public PSFUDConfig() throws PSFUDConfigFileNotFoundException, IOException, SAXException {
    File file = new File(FUDCONFIGFILE);
    if (!file.exists()) throw new PSFUDConfigFileNotFoundException();
    ...
    try (FileInputStream fi = new FileInputStream(file)) {
      ...
      if (m_ConfigDoc != null) {
        m_ElementCurrent =
            PSFUDDocMerger.createChildElement(m_ConfigDoc.getDocumentElement(), ELEM_CURRENT);
      }
    }
  }
  ...
  /**
   * Get method for user path. User path is calculated as $current/serveralias/userid, where
   * $current is the current working directory. The snapshot file is always saved to user path.
   *
   ...
   *
   * @return protocol as Stirng, never <code>null</code> default is DEFAULT_PROTOCOL.
  ...
  /**
   * Get method to get a list of all available server aliases in the configuration file
   *
   * @return server alias list as an array of strings, never be empty or <code>null</code>.
   * @throws PSFUDInvalidConfigFileException if the there are no server aliases defined in the
   *     document.
   */
  public Object[] getServerAliases() throws PSFUDInvalidConfigFileException {
  ...
  /**
   * Set method for user's unencrypted password. This will never be saved part of the configuration
   * document
   *
  ...
  public void setServerAlias(String serverAlias)
    ...
    if (null == serverAlias || serverAlias.trim().length() < 1)
      throw new PSFUDEmptyServerAliasException(MainFrame.getRes().getString(ERROR_SERVER_ALIAS));
  ...
  private void setCurrentURL(String serverAlias)
    ...
    NodeList nl = m_ConfigDoc.getElementsByTagName(ELEM_URL);
    if (null == nl || nl.getLength() < 1) {
      throw new PSFUDInvalidConfigFileException(MainFrame.getRes().getString(ERROR_SERVER_ALIAS));
    }
   ...
   *
   * @param userid as String, set to empty string if <code>null</code> is specified.
   */
   ...
   *
   * @return true if prompt required beofre purging else false. Default is true.
   */
  ...
  /**
   * Method to get the prompt before overwriting file option. The default is true.
   *
  ...
  /**
   * General helper function to get an option value. For now it is meant for only booleans, but can
  ...
  private Object getOption(int option) {
    ...
    if (null == nl || nl.getLength() < 1) return Boolean.FALSE;
  ...
  /**
   * General helper function to set an option value. For now it is meant for only booleans, but can
   * be generalized in future.
   *
  ...
  private void setOption(int option, Object value) {
    ...
    if (value instanceof Boolean) {
      if (((Boolean) value).booleanValue()) temp = OPTION_YES;
      else temp = OPTION_NO;
    }
    NodeList nl = m_ConfigDoc.getElementsByTagName(tagname);
    Element elem = null;
  ...
  /**
   * Method to get enctrypted userid-password combo required for HTTP authentication.
   *
   * @return Base64 encrypted userdid-password combo as required for the header in the request for
   *     HTTP authentication.
   */
  public String getEncryptedUseridPassword() {
    return Base64.encode(convertStringToByteArray(getUserid() + ":" + getPassword()));
  }

  /**
   * Method to see if authentication is required for Rx app.
   *
   * @return <code>true</code> if authentication was required to access the Rx app, else <code>false
   *     </code>.
   */
  ...
  /**
   * Cheap method to convert a stringto byte array. Be aware that this does not use any encoding.
   * This is used in password encryption process and is sufficient.
   */
  ...
  /** Configuration XML DOM Document, never null after this class is instantiated successfully. */
  private Document m_ConfigDoc = null;

  /**
   * Current element that stores current user and server alias, never null after this class is
   * instantiated successfully.
   */
  private Element m_ElementCurrent = null;

  /**
   * URL to get Content List for the current server alias, never null after this class is
   * instantiated successfully.
   */
  ...
  private static final String ELEM_PURGELOCALOPTION = "purgelocalcopyafterupload";
  ...
  /**
   * This variable is set when application connects to the server and gets content item list. This
   * is required since upload requires (under the current server bahavior) the userid and password
   * supplied during the first post request itself. We will have problems if the Rx application
   * requires authentication versus not. This does happen with a query request. So when we get the
   * content item list we request first without password. If request fails with Authentication
   * error, we supply the credentials and then make a request. Essentially, this variable tells us
   * whether authentication was actually required when content item list was obtained. If so, we
   * supply the credentials for upload, otherwise not. Default is <code>flase</code>.
   */
