...
public class PrintNode {
  ...
  /**
   * Prints the supplied node to the supplied writer. Empty elements are written using the empty
   * element tag format. See the method by the same name that takes more parameters for a more
  ...
  /**
   * Writes the supplied node and all its children as a nicely formatted XML document to the
   * supplied writer.
   *
   * @param node The node to print. To print a whole document, pass in the document. If null,
   *     nothing is done.
   * @param indent A String containing white space. Comments and elements are prepended with this
   *     string when they are being written. Children are automatically indented from their parents.
   *     <p>When passing in a document, this should typically be the empty string. This may be null
   *     to mean the empty string.
   * @param out The target for the output. If null, nothing is done.
   * @param useEmptyElementTag If <code>true</code>, when an element has no data, the empty element
   *     tag form is used (example: <foo/>. If <code>false
   * </code>, a start and end tag is used, even when no content is present (example: <foo></foo>).
   *     This flag has no effect if an element has content.
  ...
  public static void printNode(Node node, String indent, Writer out, boolean useEmptyElementTag)
    ...
    switch (node.getNodeType()) {
      case Node.ATTRIBUTE_NODE:
        out.write(" " + ((Attr) node).getName() + "=\"" + ((Attr) node).getValue() + "\"");
        break;
