...
public class PSDisplayOptions {
  ...
  private String checkDigits(
    ...
    String returnString = "";
    if (s.length() == 1) returnString = "0" + s;
    else returnString = s;
  ...
  private void setDefinitionOption(PSOptions definition) {
    if (definition == null || (!definition.getCategory().equals(DISPLAY_OPTIONS_CATEGORY)))
  ...
  private Color getTheColor(String optionName, String defaultColor) {
    if (getIsUseOSSettings()) return getSystemColor(optionName);

    String theValue = defaultColor;
    PSOption option = getDefinitionOption().getOption(optionName);

    if (option != null) theValue = (String) option.getOptionValue();
  ...
  private void setTheColor(String context, String optionName, Color color) {
    ...
    if (option == null) {
      getDefinitionOption().addOption(context, optionName, getColorString(color));
    } else option.setOptionValue(getColorString(color));
  }
  ...
  public boolean getIsUseOSSettings() {
    ...
    if (option != null) {
      ...
      if (temp.equalsIgnoreCase("yes") || temp.equalsIgnoreCase("true")) theValue = true;
    }
    return theValue;
  }
  ...
  public void setIsUseOSSettings(boolean useOSSettings) {
    ...
    if (option == null) {
      getDefinitionOption().addOption(GENERAL_CONTEXT, USE_OS_SETTINGS, theValue);
    } else option.setOptionValue(theValue);
  }
  ...
  private void setTheFont(String context, String optionName, Font font) {
    ...
    } else throw new IllegalStateException("invalid object for " + optionName);
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!PSOptionManagerConstants.compare(m_defOptions, comp.m_defOptions)) return false;
