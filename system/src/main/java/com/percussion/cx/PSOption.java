...
public class PSOption implements IPSClientObjects {
  ...
  public void fromXml(Element sourceNode) throws PSContentExplorerException {
    ...
    try {
      ...
      setContext(PSXMLDomUtil.checkAttribute(sourceNode, ATTR_CONTEXT, true));

      // get optionid attribute (required)
      setOptionId(PSXMLDomUtil.checkAttribute(sourceNode, ATTR_OPTIONID, true));

      // if has children and is one of ours try to instantiate class:
      if (PSXMLDomUtil.getFirstElementChild(sourceNode) != null
          && PSXMLDomUtil.getFirstElementChild(sourceNode)
              .getNodeName()
              .toLowerCase()
              .startsWith("psx")) {
        Element childEl = PSXMLDomUtil.getFirstElementChild(sourceNode);

        String className = PSXMLDomUtil.checkAttribute(sourceNode, ATTR_CLASSNAME, false);

        if (className == null || className.trim().length() == 0) className = getOptionId();
  ...
  private IPSClientObjects makeObject(Element source, String className)
    ...
    try {
      ...
      if (ctor == null) throw new ClassNotFoundException("not a valid class");

      Element[] e = {source};
      Object obj = ctor.newInstance(e);
      if (!(obj instanceof IPSClientObjects)) throw new ClassNotFoundException("not a valid class");
  ...
  public boolean equals(Object obj) {
    ...
    if (!PSOptionManagerConstants.compare(m_optionValue, comp.m_optionValue)) return false;
    if (!m_optionId.equals(comp.m_optionId)) return false;
    if (!m_context.equals(comp.m_context)) return false;
