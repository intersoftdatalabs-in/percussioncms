...
public class PSActionEvent {
  ...
  public PSActionEvent(String refreshHint) {
    if (refreshHint == null || refreshHint.trim().length() == 0)
      throw new IllegalArgumentException("refreshHint may not be null or empty.");

    m_refreshHint = refreshHint;
  }

  /**
   * Gets the refresh hint that describes the action to take by the reciever of this event object.
   *
   * @return the hint, never <code>null</code> or empty, may be one of the REFRESH_xxx values.
   */
   ...
   * refresh hint is <code>REFRESH_NODES</code>. The first one in the list is the default selection
   * for the listener.
   *
   ...
   *
   * @return the nodes, may be <code>null</code> if the refresh hint is not <code>REFRESH_NODES
   *     </code>.
   */
   ...
   *
   * @param isFull <code>true</code> if a full refresh is to be performed, <code>false</code> if a
   *     refresh of only dirty nodes is to be performed.
   */
   ...
   * more info.
   *
   * @return <code>true</code> if a full refresh is to be performed, <code>false</code> if not.
   */
  ...
  /**
   * The hint that describes the action that needs to be performed by the reciever, initialized in
  ...
  /**
   * Determines if a full vs partial refresh is required. Is <code>true</code> if a full refresh is
   * to be performed, <code>false</code> if a refresh of only dirty nodes is to be performed.
   */
