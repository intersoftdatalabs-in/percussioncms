...
/**
 * This class provides a way of constructing the Java Font object from the XML document and to get
 * XML document from the Java Font object. The DTD used by this class is defined in the file
 * sys_Font.dtd.
 */
public class PSFont implements IPSClientObjects {
   ...
   * reflection. {@link #fromXml(Element) fromXml(Element)} should be called immediately after
  ...
  public boolean equals(Object obj) {
    ...
    if (m_style != comp.m_style) return false;
    if (m_size != comp.m_size) return false;
    if (!m_face.equals(comp.m_face)) return false;
   ...
   *
   * @param the style to use. Must be one of the values in the {@link #getStyle() getStyle()}
   *     description
   */
  private void setStyle(int style) {
    if (style == Font.PLAIN
      ...
      m_style = style;
    } else throw new IllegalArgumentException("style is not valid");
  }
   ...
   * @return one of the possible style values. Defaults to {@link #TEXT_STYLE_PLAIN
   *     TEXT_STYLE_PLAIN}
   * @see #getStyle()
   */
  private int getStyle(String styleAsString) {
    ...
    if (styleAsString.equalsIgnoreCase(TEXT_STYLE_BOLD)) theStyle = Font.BOLD;

    if (styleAsString.equalsIgnoreCase(TEXT_STYLE_ITALIC)) theStyle = Font.ITALIC;

    if (styleAsString.equalsIgnoreCase(TEXT_STYLE_BOLDITALIC)) theStyle = Font.BOLD | Font.ITALIC;
  ...
  private String getStyleAsString(int style) {
    ...
    if (style == Font.BOLD) theStyle = TEXT_STYLE_BOLD;

    if (style == Font.ITALIC) theStyle = TEXT_STYLE_ITALIC;

    if (style == (Font.BOLD | Font.ITALIC)) theStyle = TEXT_STYLE_BOLDITALIC;
  ...
  /**
   * Creates a <code>java.awt.Font</code> object from the values in this class. This value is not
   * owned by this object and any modifications to it will not be referenced by this class.
   *
   ...
   *
   * @param the name of the font-face to use. Must not be <code>null</code> or empty.
   */
  private void setFace(String face) {
    if (face == null || face.trim().length() == 0)
      throw new IllegalArgumentException("face must not be null or empty.");

    m_face = face;
  }

  /**
   * The style to be used with this font, initialized by <code>setStyle(int)</code>, may be changed.
   */
  private int m_style = 0;

  /** The size of this font, initialized by <code>setSize(int)</code>, may be changed. */
  private int m_size = -1;

  /**
   * The face of this font, initialized by <code>setFace(String)</code>, may be changed, never
   * <code>null</code>.
   */
