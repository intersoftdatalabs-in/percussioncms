...
public class PSContentExplorerConstants {
  ...
  /**
   * The suffix to use with icon constants to put or get expanded version of an icon. The constant
   * used for an icon of any node is the node type. See PSNode for more information on nodes.
   */
  ...
  /**
   * The name of the parameter that gives the RestrictSearchFieldsToUserCommunity property value in
   * server.properties.
   */
  public static final String PARAM_RESTRICTSEARCHFIELDSTOUSERCOMMUNITY =
      "RestrictSearchFieldsToUserCommunity";

  /**
   * The name of the parameter that gives the <code>CacheSearchableFields</code> property value in
   * server.proerpties.
   *
  ...
  /**
   * The name of the parameter that indicate whether the managed navigation is used or not. The
   * value of the parameter is <code>yes</code> if the managed navigation is used by the server;
   * otherwise the managed navigation is not used.
   */
  public static final String PARAM_IS_MANAGEDNAV_USED = "isManagedNavUsed";

  /**
   * The name of the parameter that indicate whether to use the session keeper functionality. This
   * makes the appet ping the server to keep the session alive while the applet is running. If the
   * new session warning and logout functionality this may not be wanted as it may be required for
   * this functionality to log out the user
   */
  ...
  /**
   * RELATIONSHIP_NAME property that is set based on user selection made in the dropdown box of the
   * dependency viewer window, defaults to empty string.
   */
  public static final String PARAM_RELATIONSHIP_NAME = "RELATIONSHIP_NAME";

  /** The parameter that provides the path to select by default when the applet is loaded. */
  ...
  /** The constant used as key to store the display options in UIManager defaults. */
  public static final String DISPLAY_OPTIONS = "DisplayOptions";

  /**
   * The parameter which defines the helpset url to be used to display help for this application
   * using JavaHelp viewer.
   */
  public static final String PARAM_HELPSET = "helpset_file";

  /**
   * The constant for the parameter that specifies if selective view refresh is enabled. Value is
   * "yes" if enabled, "no" if not.
   */
  public static final String PARAM_SELECTIVE_VIEW_REFRESH = "sys_useSelectiveViewRefresh";

  /**
   * The constant for the parameter to define the default selection path for CX or IA view. This
   * path represents internal names of nodes.
   */
  public static final String PARAM_CXINTERNALPATH = "sys_cxinternalpath";

  /**
   * The constant for the parameter to define the default selection path for CX or IA view. This
   * path represents display names of nodes.
   */
  static final String PARAM_CXDISPLAYPATH = "sys_cxdisplaypath";

  /** The application resource used to retreive the list of folders with the publish flag set */
  static final String APP_RESOURCE_FLAGGED_FOLDERS =
      "/sys_psxObjectSupport/getAllFolderPublishFlags.xml";

  /**
   * The constant for the parameter that specifies if an external search engine is available. Value
   * is "yes" if available, "no" if not.
   */
  ...
  /** The following represents different variable names for known paths correspondingly. */
  ...
  /**
   * The following represents the paths of specific nodes in navigation tree of content explorer
   * view.
   */
  ...
  /** String constant for the default value for action property value for ACTION_PROPERTY_TARGET */
  ...
  /**
   * Integer constant for the maximum likely length of the url that can be passed to ShowWindow
   * function.
   */
  public static final int MAX_SHOWWINDOW_URL_LENGTH = 256;

  /**
   * Integer constant for the maximum likely length for url in get method on most of the browsers.
   */
  ...
  /**
   * Contstant for the name of resource to save the server action url. This resource has a pre-exit
   * which sets a session variable called SYS_SERVERACTIONURL with the value from the htmlparameter
   * SYS_SERVERACTIONURL.
   */
  public static final String SAVE_SERVERACTIONURL = "/sys_cxSupport/saveserveractionurl.html";

  /**
   * Default Actions for various types of nodes. These are invoked on mouse double click. The
   * default actions are assigned in the order of priority so that first action that is found in the
   * possible actions for the mode and selection is executed. For example, for a content item, we
   * need to specify two default actions "Edit" and "View" so that if user cannot edit he may be
   * able to view.
   */
