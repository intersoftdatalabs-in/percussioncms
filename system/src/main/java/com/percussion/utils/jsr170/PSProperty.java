...
/**
 * Represents a single property for a node. The actual data is held in loaded instances where each
 * instance holds multiple values. The mappings between the fields and the instances is part of the
 * {@link com.percussion.services.contentmgr.impl.legacy.PSTypeConfiguration}.
 */
public class PSProperty extends PSPropertyWrapper implements IPSProperty, IPSJcrCacheItem {
  ...
  public synchronized void init() {
    ...
    if (beanPropertyName != null) value = getPropertyValue(beanPropertyName);
    else {
   ...
   * @throws RepositoryException if a backend error occurs
   */
  public PSProperty(String pname, Node parent, Object value) throws RepositoryException {
    super(parent);
    ...
    if (value == null) return "";
      ...
      if (strValue == null) {
        try {
          ...
          throw new RepositoryException("Could not extract string for property " + name, e);
        }
      }
      return strValue;
  ...
  public InputStream getStream() throws RepositoryException {
    ...
    try {
      if (value instanceof byte[]) {
        ...
        return new PSReaderInputStream(((Clob) value).getCharacterStream());
      } else if (value instanceof Blob) {
  ...
  public Calendar getDate() throws RepositoryException {
    ...
    else if (value instanceof Calendar) return (Calendar) value;
  ...
  public PropertyDefinition getDefinition() throws RepositoryException {
    ...
    throw new PathNotFoundException("Can't find definition for property: " + name);
  }
  ...
  public Item getAncestor(int index) throws RepositoryException {
    if (depth < index) {
      throw new ItemNotFoundException("Can't have an ancestor below this node's depth");
    }
    if (depth == index) {
      return this;
    } else {
  ...
  public String toString() {
    return new ToStringBuilder(this)
        .append("name", name)
        .append("value", value)
        .append("initialized", m_initialized)
        .toString();
  }
  ...
  @Override
  public int hashCode() {
    return new HashCodeBuilder().append(depth).append(name).append(value).toHashCode();
  }
  ...
  public long getSizeInBytes() throws RepositoryException {
    try {
      if (value == null) return 0;
      else if (value instanceof byte[]) return ((byte[]) value).length;
      else if (value instanceof String) return ((String) value).length() * 4L;
      else if (value instanceof Clob) return ((Clob) value).length() * 4L;
      else if (value instanceof Blob) return ((Blob) value).length();
      else return 0;
    } catch (SQLException e) {
