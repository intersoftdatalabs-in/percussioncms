...
/** The base class for all effect execution context objects. This is the default implementation. */
public class PSExecutionContext implements IPSExecutionContext {
  /**
   * Convenience ctor that calls {@link #PSExecutionContext(int, PSExecutionData, Map) this(context,
   * data, null)}.
   */
   ...
   * @param data refernce to PSExecutionData object to extract relationship information, must not be
   *     <code>null</code>.
   * @param processedRelationships A dynamic set of entries that tracks which relationships have
   *     already been processed. Only the values are read and they are expected to be a <code>
   *     PSRelationshipEffectTestResult</code>. May be <code>null</code> if this information is not
   *     tracked. This class treats the supplied object as read-only and does not take ownership so
   *     it can be modified by the caller. [NB Although this may seem odd, it was done this way
   ...
   * @throws IllegalArgumentException if the parameter is not one of the ACTIVATION_ENDPOINT_XXXX
   *     values.
   */
  public void setActivationEndPoint(String activationEndPoint) {
    m_activationEndPoint = getActivationEndPointFromString(activationEndPoint);
  }

  /**
   * Helper method to set owner or dependent as activation end point for processing the current
   * effect.
   *
  ...
  public void setActivationEndPoint(boolean isOwner) {
    if (isOwner) m_activationEndPoint = RS_ENDPOINT_OWNER;
    else m_activationEndPoint = RS_ENDPOINT_DEPENDENT;
  }

  /**
   * helper method that evaluates the activation endpoint type constants from the activation
   * endpoint string.
   *
   * @param activationEndPoint must be one of the ACTIVATION_ENDPOINT_XXXX values defined in {@link
   *     PSRelationshipConfig}.
   * @return one of the RS_ENDPOINT_xxxx values ORed. Default is RS_ENDPOINT_OWNER.
   * @throws IllegalArgumentException if the parameter is not one of the ACTIVATION_ENDPOINT_XXXX
   *     values.
   */
  public static int getActivationEndPointFromString(String activationEndPoint) {
    ...
    if (activationEndPoint.equalsIgnoreCase(PSRelationshipConfig.ACTIVATION_ENDPOINT_DEPENDENT)) {
   ...
   * @return <code>true</code> if the given context is valid, <code>false</code> otherwise.
   */
  public static boolean isContextValid(int context) {
  ...
  /**
   * Reference to the executiondata to access the most current relationship chain. Set in the
   * constructor, never <code>null</code> after that.
   */
  private PSExecutionData m_executionData = null;

  /**
   * Activation endpoint for the current effect. One of the RS_ENDPOINT_xxxx values. Default is
   * RS_ENDPOINT_OWNER. Must have been set just before processing the effect by the command handler.
   */
