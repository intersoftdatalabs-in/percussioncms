...
public class PSCloneAlreadyExistsException extends RuntimeException {
   ...
   * canb be accessible later when geenrating the message.
   *
   * @param owner owner of the existing relationship, must not be <code>null</code>.
   * @param dependent dependent of the existing relationship, must not be <code>null</code>.
   * @param relType name of the relationship, must not be <code>null</code> or empty.
   */
  ...
  /**
   * Constructor that takes the existing relationship that can be accessible later when geenrating
   * the message.
   *
  ...
  public String getRelationshipType() {
    if (m_relationship != null) return m_relationship.getConfig().getName();
    return m_relType;
  }
  ...
  public PSLocator getOwner() {
    if (m_relationship != null) return m_relationship.getOwner();
    return m_owner;
  }
  ...
  public PSLocator getDependent() {
    if (m_relationship != null) return m_relationship.getDependent();
    return m_dependent;
  }
   ...
   *
   * @return the existing relationship because of which this exception was thrown, may be<code>null
   *     </code>.
   */
