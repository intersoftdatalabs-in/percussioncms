...
/**
 * This is an empty interface to define the execution context needed for relationship effects.
 * Depending on where relationships are processed, the execution context will be different. All
 * context classes that can be used to process relationships must implement this interface. See
 * <code>com.percussion.design.objectstore.PSRelationship</code> and PSXRelationshipSet.dtd for a
...
public interface IPSExecutionContext {
   ...
   *    null</code> if no relationship is available yet. If the current context is <code>
   *     RS_CONSTRUCTION</code>, then this is a newly created relationship that has not been
   *     persisted. Any changes to this object will affect what is persisted.
   */
   ...
   * While procesing the topmost relationship, this method returns an empty list.
   *
   * @return Never <code>null</code>, may be empty. Although each entry is the actual relationship,
   *     any changes will not affect the persisted value. However, they could have an affect on
   *     other effects that call this method. Therefore, these should be treated as read-only. They
   *     are not cloned to save processing time.
   */
   ...
   *
   * @return Never <code>null</code>, but may be empty if this information is not tracked. The
   *     caller takes ownership of the returned set. Each entry is a <code>PSRelationship</code>.
   *     The caller should not modify the members, treat them as read-only.
   */
  ...
  /** The context type used to run effects before a workflow transition is executed. */
  public static final int RS_PRE_WORKFLOW = 3;

  /** The context type used to run effects after a workflow transition is executed. */
  ...
  /**
   * The context type used to run effects before persisting an updated relationship instance in the
   * backend repository.
   */
  ...
  /**
   * The context type used to run effects just before creating a clone of an existing item is
   * createed.
   */
  ...
  /**
   * Maximu valid context value. Make sure it is maximum of all RS_XXXX values. Needs to be changed
   * when we add a new context value.
   */
