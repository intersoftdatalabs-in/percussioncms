...
public abstract class PSFolderEffect extends PSEffect {
  ...
  public void test(
    ...
    if (!context.isPreConstruction() && !context.isPreDestruction() && !context.isPreUpdate()) {
      result.setWarning(
          "Illegal Context, expected to be "
              + "RS_PRE_CONSTRUCTION, RS_PRE_DESTRUCTION or RS_PRE_UPDATE.");
      return;
    }
    if (!isFolderRelationship(context, result)) return;
  ...
  private boolean isFolderRelationship(IPSExecutionContext context, PSEffectResult result) {
    ...
    if (currentRel == null
        || !currentRel.getConfig().getName().equals(PSRelationshipConfig.TYPE_FOLDER_CONTENT)) {
      result.setWarning(
          "The current relationship is not of type '"
  ...
  protected boolean isValidOriginatingRelationship(
      IPSRequestContext reqCtx, PSEffectResult result) {
    ...
    if (relationship.getConfig().getName().equals(PSRelationshipConfig.TYPE_FOLDER_CONTENT)) {
      return true;
    }
  ...
  protected PSRequest getRequest(IPSRequestContext reqCtx) {
    if (!(reqCtx instanceof PSRequestContext))
      throw new IllegalArgumentException(
          "request context must be an instance of PSRequestContext.");
   ...
   */
  protected abstract void onFolderChange(
      Object[] params,
      IPSRequestContext request,
      IPSExecutionContext context,
      PSEffectResult result);
