...
public abstract class PSPublishUnpublishMandatory extends PSEffect {
  ...
  private void validateParams(IPSRequestContext request, Object[] params)
    ...
    if (params != null) suppliedParams = params.length;
    if (suppliedParams == 0) {
      throw new PSParameterMismatchException(request.getUserLocale(), 2, suppliedParams);
    }
  ...
  public void test(
    ...
    if (params.length > 1 && params[1] != null) trNameOwner = params[1].toString();
    if (params.length > 2 && params[2] != null) trNameDependent = params[2].toString();

    String wfAction = request.getParameter(IPSConstants.DEFAULT_ACTION_TRIGGER_NAME, "").trim();
    ...
    boolean isOwner = (context.getActivationEndPoint() == IPSExecutionContext.RS_ENDPOINT_OWNER);
    ...
    try {
       ...
       */
      if (!isCorrectStateContext(request, result) || !isEndpointValidRevision(locator, isOwner)) {
      ...
      Document workflowStatus = PSEffectUtils.getWorkflowState(request, locator.getId(), m_name);
      ...
      String realTransitionName = getNamedTransition(request, workflowStatus, transitionName);
  ...
  private boolean isCorrectStateContext(IPSRequestContext request, PSEffectResult result)
    ...
    boolean isCurrentlyPublic = PSWorkFlowUtils.isInPublicState(request);

    boolean isToPublic = PSWorkFlowUtils.toPublicState(request);

    boolean isToOutOfPublic = (isCurrentlyPublic && !isToPublic);

    return isTransitioningIntoTriggerState(
        request, isCurrentlyPublic, isToPublic, isToOutOfPublic, result);
  }
  ...
  private boolean isEndpointValidRevision(PSLocator endpoint, boolean isEndpointActive) {
    if (!isEndpointActive && getModeName().equals(MODE_UNPUBLISH)) {
      ...
      String lastPublicRev = PSMacroUtils.getLastPublicRevision(contentid);
  ...
  private String getNamedTransition(
    ...
    if (elem != null) {
      ...
      transition = getForceTransition(request, wfStateId, workflowId, transitionName);
    }
  ...
  private void transit(
    ...
    try {
      ...
      if (forceDependent)
        params.put(IPSHtmlParameters.SYS_FORCEDEPENDENT, String.valueOf(forceDependent));
      ...
      String validationErrMsg =
          ir.getRequestContext().getParameter(IPSHtmlParameters.SYS_VALIDATION_ERROR);
  ...
  private String getForceTransition(
      IPSRequestContext request, String stateId, String workflowId, String wfAction)
    ...
    if (ir != null) {
      ...
      if (wfAction == null || wfAction.trim().length() < 1) lookForNamedTransition = false;
      ...
      for (int i = 0; transitions != null && transitions.item(i) != null; i++) {
        ...
        if (lookForNamedTransition) {
          if (transitionName.equals(wfAction)) return transitionName;
        } else {
          if (transition.getAttribute("isDefault").equalsIgnoreCase("y")) return transitionName;
        }
      }
    } else {
      String[] args = {
