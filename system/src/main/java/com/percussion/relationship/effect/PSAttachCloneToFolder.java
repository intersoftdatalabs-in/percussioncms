 ...
 *       com.percussion.util.IPSHtmlParameters#SYS_FOLDERID} must exist in the request. If more than
 *       one value exists the clone will be addedd to all target folders
...
public class PSAttachCloneToFolder extends PSEffect {
  ...
  public void test(
    ...
    if (!originatingRel.getConfig().getName().equals(currentRel.getConfig().getName())
  ...
  public void attempt(
      ...
      throws PSExtensionProcessingException, PSParameterMismatchException {
    PSRelationship currentRel = context.getCurrentRelationship();
    Object orig = request.getParameterObject(IPSHtmlParameters.SYS_ORIGINALFOLDERID);
    Object obj = request.getParameterObject(IPSHtmlParameters.SYS_FOLDERID);
