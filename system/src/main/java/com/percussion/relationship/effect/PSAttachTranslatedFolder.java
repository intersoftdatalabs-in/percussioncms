...
public class PSAttachTranslatedFolder extends PSEffect {
  ...
  public void test(
    ...
    if (!context.isPreConstruction()) {
      result.setWarning("This effect is active only during relationship construction");
      return;
    }
    ...
    if (!currentRel.getConfig().getCategory().equals(PSRelationshipConfig.CATEGORY_TRANSLATION)) {
      result.setWarning(
    ...
    try {
      ...
      PSComponentSummary summary = cms.loadComponentSummary(origOwner.getId());

      if (summary.getType() == PSCmsObject.TYPE_FOLDER
          && (originatingRel.getOwner().getId() != currentRel.getOwner().getId())) {
        result.setWarning(
            "This effect is active only if the originating "
      ...
      PSComponentSummaries folderParents = relProxy.getSummaries(filter, true);
  ...
  public void attempt(
    ...
    try {
      ...
      PSLocator depLocator = currentRel.getDependent();
      IPSCmsObjectMgr cms = PSCmsObjectMgrLocator.getObjectManager();
      PSComponentSummary depSummary = cms.loadComponentSummary(depLocator.getId());
      ...
      while (walker.hasNext()) {
        ...
        if (updateNewParents(relProxy.getSummaries(filter, false).iterator(), locale, newParents))
        ...
        filter.setName(currentRel.getConfig().getName());
        if (updateNewParents(relProxy.getSummaries(filter, true).iterator(), locale, newParents))
          foundTranslatedParent = true;

        if (!foundTranslatedParent) originalParents.add((PSLocator) summary.getLocator());
      }
      ...
      for (int i = 0; i < newParents.size(); i++) {
        ...
        folderProc.addChildren(list, newParent);
      }
