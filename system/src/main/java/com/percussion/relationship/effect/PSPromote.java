...
public class PSPromote extends PSEffect {
  ...
  public void attempt(
    ...
    if (params != null && params.length > 0) transitionName = params[0].toString();
    PSRelationship relationship = context.getCurrentRelationship();
    try {
      PSLocator ownerLocator = relationship.getOwner();
      ...
      transitionItem(
          ...
          ownerLocator,
          resource,
          getTransition(request, transitionName, ownerLocator),
          false);
  ...
  private void transitionItem(
    ...
    try {
      ...
      ir.performUpdate();
      String errMsg = ir.getRequestContext().getParameter(IPSHtmlParameters.SYS_VALIDATION_ERROR);
  ...
  private String getTransition(
      IPSRequestContext request, String suppliedTransitionName, PSLocator locator)
    ...
    try {
      ...
      if (ir != null) {
        ...
        for (int i = 0; transitions != null && !found && (transitions.item(i) != null); i++) {
  ...
  private void repointRelationships(IPSRequestContext request, PSRelationship relationship)
    ...
    while (itOutboundRel.hasNext()) {
      ...
      if (!rs.getConfig().isCloningAllowed()) {
        ...
        rs.setOwner(new PSLocator(pvDependentItem.getId(), pvDependentItem.getRevision()));
    ...
    while (itInboundRel.hasNext()) {
      ...
      rs.setDependent(new PSLocator(pvDependentItem.getId(), pvDependentItem.getRevision()));
  ...
  PSRelationshipSet removeRedundentRelationships(
      ...
      PSRelationshipSet relationshipsToModify)
      throws PSCmsException {
  ...
  public static PSComponentSummary getItemSummary(IPSRequestContext request, PSLocator locator)
    ...
    if (summary == null) {
      throw new PSCmsException(0, "Could not get locator for contentid= " + locator.getId());
    }
