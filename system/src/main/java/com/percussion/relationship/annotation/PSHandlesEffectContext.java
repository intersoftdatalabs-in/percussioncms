...
public @interface PSHandlesEffectContext {
  /**
   * Required contexts will automatically added to relationship configuration when the effect is
   * added to a relationship type.
   *
  ...
  /**
   * Optional contexts will be allowed to be manually added to relationship configuration when the
   * effect is added to a type, contexts that are not in optional, or required will be removed. This
   * defaults to ALL which allows any user assigned contexts
   *
   * @return an array of {@link PSEffectContext}, never <code>null</code>, defaults to
   *     PSEffectContext.ALL
   */
  PSEffectContext[] optional() default {PSEffectContext.ALL};

  /**
   * The endpoint is only applicable to workflow contexts that are not directly related to effect
   * construction, destruction and update, e.g. {@link PSEffectContext#PRE_WORKFLOW}, {@link
   * PSEffectContext#POST_WORKFLOW} {@link PSEffectContext#PRE_CHECKIN} and {@link
   * PSEffectContext#POST_CHECKOUT}.
   *
   * <p>This defines which relationships of the item to process, e.g. whether the current item is
