...
public class PSPreUpgradePluginCustomProperty implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    m_config
        .getLogStream()
        .println(
            "Checking views, searches, display formats, " + "and actions for custom property use.");

    // Initialize non-custom properties
    for (int i = 0; i < m_viewSearchProps.length; i++) m_nonCustomProps.add(m_viewSearchProps[i]);
    ...
    try {
      ...
      respType = PSPluginResponse.EXCEPTION;
      respMessage =
          "Failed to check Server Objects for custom property use, "
              + "see the \""
              + log
              + "\" located in "
    ...
    config.getLogStream().println("Finished process() of the plugin Custom Property...");
  ...
  private Set<String> checkCustomProps(
    ...
    String qualTableName =
        PSSqlHelper.qualifyTableName(
            propTable, dbmsDef.getDataBase(), dbmsDef.getSchema(), dbmsDef.getDriver());
    ...
    while (rs.next()) {
      ...
      if (!m_nonCustomProps.contains(propName)) {
        ...
        m_config.getLogStream().println("Custom property found [" + propName + "]");
      }
    }
    ...
    for (Integer idObj : ids) {
      ...
      if (rs.next()) names.add(rs.getString(nameColumn));
    }
