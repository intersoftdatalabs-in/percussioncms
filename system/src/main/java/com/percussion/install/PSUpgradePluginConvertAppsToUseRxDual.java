...
public class PSUpgradePluginConvertAppsToUseRxDual implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    config.getLogStream().println("Converting application files to use " + "RXDUAL table.");
    ...
    try {
      for (int iApp = 0; iApp < appFiles.length; ++iApp) {
        ...
        if (appFile.isFile()
            && ((appFile.getName().indexOf(".xml") + 4) == appFile.getName().length())) {
          ...
          doc =
              PSXmlDocumentBuilder.createXmlDocument(
                  new InputSource(appFile.toURL().toString()), false);
          ...
          if (modified1 == 1 || modified2 == 1 || modified3 == 1) {
            config.getLogStream().println("Saving the transformed file: " + targetFile + "...");
            pw = new PrintWriter(new FileOutputStream(targetFile));
            pw.print(PSXmlDocumentBuilder.toString(doc));
            pw.flush();
            config.getLogStream().println("Saved the transformed file: " + targetFile + "...");
          }
        }
      }
    } catch (Exception e) {
    ...
    config.getLogStream().println("Finished process() of the plugin DUAL To RXDUAL...");
    return null;
  }
