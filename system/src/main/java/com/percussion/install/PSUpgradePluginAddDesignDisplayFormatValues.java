...
public class PSUpgradePluginAddDesignDisplayFormatValues implements IPSUpgradePlugin {
  ...
  private void fixDisplayFormat(Connection conn) throws SQLException {
    ...
    String query =
        "Select DISPLAYID FROM " + displayFormatTable + " WHERE INTERNALNAME LIKE '%CM1_Design%'";
    ...
    if (!results.next()) {
      ...
      if (resultsLastId.next()) {
        ...
        logger.println(
            "Display Format not found, inserting corresponding row with id " + displayFormatId);
        String insertDisplayFormatQuery =
            "INSERT INTO "
                + displayFormatTable
                + "(DISPLAYID, INTERNALNAME, DISPLAYNAME, DESCRIPTION, VERSION)"
                + " VALUES(?,?,?,?,?)";
  ...
  private void fixDisplayFormatColumns(Connection conn) throws SQLException {
    ...
    String query = "Select SOURCE FROM " + displayFormatColumnsTable + " WHERE DISPLAYID = ?";
    PreparedStatement ps = conn.prepareStatement(query);
    ...
    if (!results.next()) {
      logger.println(
          "Display Format Columns not found, inserting corresponding rows for fields"
              + " sys_title, sys_size, sys_contentlastmodifieddate");
      String insertDisplayFormatColumnQuery =
          "INSERT INTO "
              + displayFormatColumnsTable
              + "(DISPLAYID, SOURCE, DISPLAYNAME, TYPE, RENDERTYPE, SORTORDER, SEQUENCE,"
              + " DESCRIPTION, WIDTH) VALUES(?,?,?,?,?,?,?,?,?)";
  ...
  private void fixDefaultDisplayFormatWidths(Connection conn) throws SQLException {
    ...
    String query =
        "Select DISPLAYID FROM " + displayFormatTable + " WHERE INTERNALNAME LIKE '%CM1_Default%'";
    PreparedStatement ps = conn.prepareStatement(query);
    ResultSet results = ps.executeQuery();
    boolean updated = false;
    if (results.next()) {
