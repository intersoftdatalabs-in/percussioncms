...
public class PSUpgradePluginPublishing implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule module, Element elemData) {
    ...
    try {
      ...
      logger.println("leaving the process() of the Update Publishing " + "plugin.");
    }
   ...
   */
  private String getOldPubStatusTable() throws FileNotFoundException, IOException {
   ...
   * @throws IOException, FileNotFoundException if an error occurs
   */
  private String getNewPubStatusTable() throws FileNotFoundException, IOException {
   ...
   * @throws IOException, FileNotFoundException if an error occurs
   */
  private String getOldPubDocsTable() throws FileNotFoundException, IOException {
   ...
   * @throws IOException, FileNotFoundException if an error occurs
   */
  private String getNewPubDocsTable() throws FileNotFoundException, IOException {
   ...
   * @throws IOException, FileNotFoundException if an error occurs
   */
  private String getContentListTable() throws FileNotFoundException, IOException {
   ...
   * @throws IOException, FileNotFoundException if an error occurs
   */
  private String getCLAssociationTable() throws FileNotFoundException, IOException {
  ...
  private int executeUpdate(PrintStream logger, Connection conn, String sqlStmt, Object[] bindValue)
    ...
    try {
      ...
      if (logger != null) logger.println("Successful execute SQL[" + ms_sqlCount + "].");
      return rowCount;
    } finally {
      if (stmt != null) {
        try {
  ...
  private void migratePubStatus(PrintStream logger, Connection conn) throws Exception {
    ...
    try {
      logger.println(
          "Migrating publication status data from "
              + oldPubStatusTable
              + " to "
              + newPubStatusTable);

      String sqlStmt = "SELECT DISTINCT PUBLICATIONID FROM " + oldPubStatusTable;
      ...
      while (rs1.next()) {
        int id = rs1.getInt(1);
        sqlStmt =
            "SELECT STARTDATE, ENDDATE, OPSTATUS, INSERTED, "
        ...
        IPSPubStatus.EndingState endingStatus = IPSPubStatus.EndingState.COMPLETED;
        int delivered = 0;
        int removed = 0;
        int failed = 0;
        ...
        while (rs2.next()) {
          ...
          if (start == null || (s != null && s.before(start))) start = s;

          Timestamp e = rs2.getTimestamp(2);
          if (end == null || (e != null && e.after(end))) end = e;
          ...
          if (editionid == 0) editionid = rs2.getInt(8);

          if (opstatus != null && !opstatus.toUpperCase().equals(STATUS_SUCCESS)) {
        ...
        if (endingStatus.equals(IPSPubStatus.EndingState.COMPLETED) && failed > 0) {
          endingStatus = IPSPubStatus.EndingState.COMPLETED_W_FAILURE;
        }
      ...
      logger.println("Successfully inserted " + rowCount + " row(s) in " + newPubStatusTable);
    } finally {
      connObj2.close();
      connObj1.close();
    }
  ...
  private void migratePubDocs(PrintStream logger, Connection conn) throws Exception {
    ...
    ResultSet rs2 = null;
    int rowCount = 0;
    Map<Integer, PubStatusValue> pubStatusMap = new HashMap<>();
    PSConnectionObject connObj1 = new PSConnectionObject();
    PSConnectionObject connObj2 = new PSConnectionObject();

    try {
      logger.println(
          "Migrating publication docs from " + oldPubDocsTable + " to " + newPubDocsTable);
      ...
      String sqlStmt =
          "SELECT PUBSTATUSID, CONTENTID, VARIANTID, "
      ...
      sqlStmt = "SELECT PUBSTATUSID, PUBLICATIONID, SITEID " + "FROM " + getOldPubStatusTable();
      stmt2 = PSPreparedStatement.getPreparedStatement(conn, sqlStmt);
      rs2 = stmt2.executeQuery();
      connObj2.setStatement(stmt2);
      connObj2.setResultSet(rs2);
      ...
      while (rs2.next()) {
        pubStatusMap.put(rs2.getInt(1), new PubStatusValue(rs2.getInt(2), rs2.getInt(3)));
      }
      ...
      while (rs1.next()) {
        ...
        if (pubstatusval != null) {
          Long referenceid = new Long(PSGuidHelper.generateNextLong(PSTypeEnum.PUB_REFERENCE_ID));
          ...
          if (pubstatus != null && !pubstatus.toUpperCase().equals(STATUS_SUCCESS)) {
          ...
          logger.println(
              "Could not find publication id for publication " + "status " + pubstatusid);
        }
      }

      logger.println("Successfully inserted " + rowCount + " row(s) in " + newPubDocsTable);
    } finally {
      connObj1.close();
      connObj2.close();
    }
  ...
  private void migrateAssemblyContext(PrintStream logger, Connection conn) throws Exception {
    ...
    try {
      logger.println(
          "Migrating assembly context data from " + contentListTable + " to " + clAssociationTable);
      ...
      while (rs.next()) {
        ...
        if (url == null) {
          logger.println("Url does not exist for content list id " + contentlistid);
          continue;
        }
        ...
        if (!params.containsKey(param)) {
          logger.println(
              "Assembly context parameter does not exist in "
                  + "url '"
                  + url
                  + "' for content list id "
                  + contentlistid);
        ...
        try {
          ...
          logger.println(
              "Invalid assembly context parameter value '"
                  + value
                  + "' in url '"
                  + url
                  + "' for content list "
      ...
      logger.println("Successfully updated " + rowCount + " row(s) in " + clAssociationTable);
      ...
      logger.println("Successfully updated " + rowCount + " row(s) in " + contentListTable);
    } finally {
      connObj.close();
    }
  }
