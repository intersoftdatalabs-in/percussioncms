...
public class PSUpgradePluginCreateServerAndServerProperties implements IPSUpgradePlugin {
    ...
    try {
      ...
      if (conn != null) {
        try {
          ...
          return new PSPluginResponse(PSPluginResponse.EXCEPTION, sqle.getLocalizedMessage());
        }
      }
    }
  ...
  private void createServerAndServerPropertiesTables() throws SQLException {
    ...
    while (sites.next()) {
      logger.println("Getting publish type for site " + sites.getInt("SITEID") + ".");
      String publishType = getPublishType(sites.getString("SITENAME"));

      // Insert properties into servers table
      logger.println("Updating server table for site " + sites.getInt("SITEID") + ".");
      int newServerId =
          updateServersTable(sites.getInt("SITEID"), sites.getString("SITENAME"), publishType);

      logger.println("Updating server properties table for site " + sites.getInt("SITEID") + ".");
      for (Map.Entry<String, String> entry : propertiesMap.entrySet()) {
        Object property =
            (sites.getObject(entry.getKey()) != null) ? sites.getObject(entry.getKey()) : "";
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private Integer updateServersTable(int siteId, String siteName, String publishType)
      throws SQLException {
    String query =
        "INSERT INTO "
            + serversTable
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void updateServerPropertiesTable(int serverId, String propertyName, Object propertyValue)
      throws SQLException {
    String query =
        "INSERT INTO "
    ...
    int nextServerPropertiesId = getNextId("SERVERPROPERTYID", serverPropertiesTable);
   ...
   * @throws SQLException
   */
  private void addAdditionalServerProperties(String publishType, int newServerId)
      throws SQLException {
   ...
   * @throws SQLException
   */
  private void updateEditionsTable(int siteId, int serverId) throws SQLException {
    String query = "UPDATE " + editionsTable + " " + "SET PUBSERVER=? " + "WHERE DESTSITE=? ";
  ...
  private void updateSitesTable(int siteId, int serverId) throws SQLException {
    String query = "UPDATE " + sitesTable + " " + "SET DEFAULT_PUBSERVERID=? " + "WHERE SITEID=? ";
  ...
  private void updatePubStatusTable(int serverId) throws SQLException {
    ...
    if (editions != null) {
      ...
      for (Integer edition : editions) {
        String query =
            "UPDATE " + pubStatusTable + " " + "SET PUBSERVERID=? " + "WHERE EDITION_ID=?";
  ...
  private void updateDeliveryTypeTable() throws SQLException {
    addDeliveryType(
        "filesystem_only",
        "Publish content to the filesystem doesn't include other delivery handlers like metadata"
            + " indexer.",
        "sys_filesystem",
        0);
    addDeliveryType(
  ...
  private void addDeliveryType(
      String name, String description, String beanName, int unpubRequiresAssembly)
      throws SQLException {
    String query =
        "INSERT INTO "
            + deliveryTypeTable
  ...
  private void updatePublicationSiteItem(int siteId, int serverId) throws SQLException {
    String query = "UPDATE " + publicationSiteItem + " " + "SET SERVER_ID=? " + "WHERE SITE_ID=? ";
  ...
  private List<Integer> getEditions(int serverId) throws SQLException {
    String query =
        "SELECT DISTINCT EDITIONID FROM "
            + editionsTable
            + " et, "
            + pubStatusTable
            + " st "
  ...
  private String getPublishType(String siteName) throws SQLException {
    ...
    if (!siteName.equals("Enterprise_Investments") && !siteName.equals("Corporate_Investments")) {
      name = siteName.toUpperCase() + "_FULL_SITE";
    } else {
      if (siteName.equals("Enterprise_Investments")) name = "rffEiPublishNow";

      if (siteName.equals("Corporate_Investments")) name = "rffCiPublishNow";
    }

    String query = "SELECT URL " + "FROM " + contentListTable + " " + "WHERE NAME=?";

    PreparedStatement ps = conn.prepareStatement(query);
    ps.setString(1, name);
    ResultSet result = ps.executeQuery();

    if (result.next()) url = result.getString("URL");
