...
public class PSUpgradePluginSpaceToUnderscore extends PSUpgradePluginModifyColumnBase {
  ...
  public boolean modifyColumnValues(
    ...
    log("Attempting to replace space with '_' in " + column + " column from " + table + " table.");

    final String valuesQuery = constructValuesQuery(dbmsDef, table, column, idcolumn);
    ...
    try {
      ...
      try {
        final ResultSet rs =
            statement.executeQuery(constructUpdateValuesQuery(conn, dbmsDef, valuesQuery));
        while (rs.next()) {
          final String value = rs.getString(column);
          final String newValue = InstallUtil.convertSpaceToUnderscore(value);
