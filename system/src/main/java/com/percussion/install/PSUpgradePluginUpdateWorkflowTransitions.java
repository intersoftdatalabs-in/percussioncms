...
public class PSUpgradePluginUpdateWorkflowTransitions implements IPSUpgradePlugin {
  ...
  private void updateTransitionNames(Connection conn) throws SQLException {
    ...
    updateTransitionStateName(
        transitionTable, workflowId, approvedStateId, "Take Down", "Archive", conn);
    ...
    updateTransitionStateName(
        transitionTable, workflowId, archieveStateId, "Submit", "Resubmit", conn);
    ...
    updateFlowTransition(
        transitionTable, workflowId, archieveStateId, reviewStateId, draftStateId, conn);
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private int getStateId(String stateTable, String state, Connection conn) throws SQLException {
    int stateId = 0;
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void updateTransitionStateName(
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void updateFlowTransition(
      String transitionTable,
      int workflowId,
      int stateId,
      int resubmitStateId,
    ...
    String query =
        "UPDATE "
            + transitionTable
            + " SET TRANSITIONTOSTATEID = ?"
            + " WHERE WORKFLOWAPPID = ?"
            + " and TRANSITIONFROMSTATEID = ?"
            + " and TRANSITIONTOSTATEID = ?";
