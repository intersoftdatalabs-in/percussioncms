...
public class PSUpgradePluginMigrateControlDependencies implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    if (appFiles == null) {
      log("Error accessing objectstore directory " + objDir.getAbsolutePath());
      return response;
    }
    ...
    for (int i = 0; i < appFiles.length; i++) {
      ...
      if (RxUpgrade.isContentEditorApp(appFile, m_config.getLogStream())) {
        ...
        try {
          ...
          log("Migrating field control dependencies in " + "application file " + appFileName);
          ...
          log("Error performing control dependency " + "migration on application " + appFileName);
          e.printStackTrace(m_config.getLogStream());
        } finally {
          try {
            if (bak != null) bak.close();
  ...
  private void migrateFieldControlDependencies(Document doc) {
    try {
      ...
      for (int i = 0; i < userPropNodes.getLength(); i++) {
        ...
        if (!parent.getNodeName().equals("PSXApplication")) continue;
      ...
      if (!controlDepends.isEmpty()) {
        for (int i = 0; i < contentEditorPipes.getLength(); i++) {
          ...
          NodeList userPropsNodes =
              cePipe.getElementsByTagName(PSContentEditorPipe.USER_PROPERTIES);
          Element userPropsEl;
          ...
          for (Element controlDep : controlDepends) {
            ...
            Element propEl =
                PSXmlDocumentBuilder.addElement(
                    doc, userPropsEl, PSContentEditorPipe.USER_PROPERTY, value);
