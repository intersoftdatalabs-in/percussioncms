...
public class PSUpgradePluginFixFormUrl implements IPSUpgradePlugin {
  ...
  private void fixContentFormUrl(Connection conn) throws SQLException {
    ...
    String query =
        "Select CONTENTID, REVISIONID, NAME, RENDEREDFORM FROM "
            + formsTable
    ...
    while (results.next()) {
      ...
      renderedForm =
          renderedForm.replaceAll("/perc-form-processor/form", "/perc-form-processor/forms/form");

      logger.println(
          "updating 'RENDEREDFORM' column in 'CT_PERCFORMASSET' for contentid = "
              + contentid
              + " and revision = "
              + revisionid
              + " and name = "
