...
public class PSUpgradePluginNextNumberFixup implements IPSUpgradePlugin {
   ...
   */
  private static int findLargestLocaleId(Connection conn, PSJdbcDbmsDef dbmsDef) throws Exception {
    String qualTableName =
        PSSqlHelper.qualifyTableName(
            PSLocaleHandler.LOCALE_TABLE,
            dbmsDef.getDataBase(),
            dbmsDef.getSchema(),
            dbmsDef.getDriver());

    String queryStmt = "SELECT MAX(" + qualTableName + ".LOCALEID) " + "FROM " + qualTableName;
      ...
      if (largestLocaleId >= nextLocaleId) {
        ...
        log(
            "Updated "
                ...
                + "localeid = "
                + nextLocaleId);
      } else log(PSContentEditorSystemDef.NEXT_ID_TABLE_ALIAS + " table update not required");
    } else {
      nextLocaleId = largestLocaleId + 1;
