...
public class PSUpgradePluginMoveConfigFiles implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      logger.println(
          "Moving the delivery-servers.xml to the destination directory " + deliveryDestDir);
      boolean result = moveConfigFile(deliverySrcDir, deliveryDestDir, deliveryConfigFileName);
      if (!result) {
        logger.println(
            "Failed to move " + deliveryConfigFileName + " to the new location " + deliveryDestDir);
      } else {
        logger.println(
            "Moved the delivery-servers.xml to the destination directory " + deliveryDestDir);
      }
      ...
      if (result) {
        ...
        logger.println(
            "Failed to move " + deliveryConfigFileName + " to the new location " + ldapDestDir);
      }
    } catch (Exception e) {
  ...
  private boolean moveConfigFile(File srcDir, File destinationDir, String configFileName) {
    ...
    if (new File(srcDir.getPath(), configFileName).exists()) {
      ...
      result =
          new File(srcDir.getPath(), configFileName)
              .renameTo(new File(destinationDir.getPath(), configFileName));
    }
    return result;
  }
