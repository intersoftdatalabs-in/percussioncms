...
public class PSCreateMenuVisibilityAcls extends PSSpringUpgradePluginBase {
  ...
  /**
   * Communties' id name map, initialized in the {@link #process(IPSUpgradeModule, Element)}. Never
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      List comms = PSRoleMgrLocator.getBackEndRoleManager().findCommunitiesByName(null);
      for (int i = 0; i < comms.size(); i++) {
        PSCommunity community = (PSCommunity) comms.get(i);
        m_communityNames.put(new Integer(community.getGUID().getUUID()), community.getName());
      }
      //
