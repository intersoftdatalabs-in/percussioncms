...
public class PSUpgradePluginPublisherBeans implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      if (upgradeConfig(target, backup)) {
        ...
        return new PSPluginResponse(PSPluginResponse.WARNING, "Failed update");
      }
    } catch (Exception e) {
  ...
  private boolean upgradeConfig(File target, File backup) throws Exception {
    ...
    try {
      ...
      if (maxTimeout > 0 || enablePassiveMode || umaskval != null) {
        ...
        if (maxTimeout > 0) updateFtpHandlersTimeout(doc, maxTimeout);

        if (enablePassiveMode) updateFtpHandlerPassiveMode(doc, enablePassiveMode);

        if (umaskval != null) updateSftpHandlerUmask(doc, umaskval);
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (id.equals(FTP_HANDLER_NAME) || id.equals(SFTP_HANDLER_NAME)) {
        if (!propertyExists(node, TIMEOUT_VALUE)) {
          Element mtProp = PSXmlDocumentBuilder.addEmptyElement(doc, node, PROPERTY_TAGNAME);
          mtProp.setAttribute(NAME_ATTR, TIMEOUT_VALUE);
          mtProp.setAttribute(VAL_ATTR, String.valueOf(newTimeout));
        }
      }
    }
  }
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (id.equals(FTP_HANDLER_NAME)) {
        if (!propertyExists(node, PASSIVE_MODE_VALUE)) {
          Element epmProp = PSXmlDocumentBuilder.addEmptyElement(doc, node, PROPERTY_TAGNAME);
          epmProp.setAttribute(NAME_ATTR, PASSIVE_MODE_VALUE);
          epmProp.setAttribute(VAL_ATTR, String.valueOf(enablePassiveMode));
          break;
        }
      }
    }
  ...
  private boolean propertyExists(Element elem, String name) {
    ...
    for (int i = 0; i < propElems.getLength(); i++) {
      ...
      if (attr.trim().length() > 0 && attr.equals(name)) {
        propExists = true;
        break;
      }
    }
  ...
  private int findMaxTimeout(Connection conn) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (timeout != 0 && timeout > maxTimeout) maxTimeout = timeout;
      ...
      if (maxTimeout > 0 && timeoutvals > 1) {
        ms_logger.println(
            "WARNING: Multiple publisher timeout values found. " + " Using " + maxTimeout + "ms");
      }
    } catch (Exception e) {
      ms_logger.println("ERROR: encountered while determining maximum " + "timeout value.");
  ...
  private boolean isPassiveModeEnabled(Connection conn) {
    ...
    try {
      ...
      ms_logger.println("ERROR: encountered while determining if passive " + "mode is enabled.");
      e.printStackTrace(ms_logger);
    } finally {
      connObj.close();
    }
  ...
  private String findUmask(Connection conn) {
    ...
    try {
      String sqlStmt =
          "SELECT PARAMVALUE FROM " + getPublisherConfigTable() + " WHERE PARAMNAME='umask'";
      stmt = PSPreparedStatement.getPreparedStatement(conn, sqlStmt);
      rs = stmt.executeQuery();
      connObj.setStatement(stmt);
      connObj.setResultSet(rs);
      ...
      while (rs.next()) {
        ...
        if (umaskval == null) umaskval = value;

        umaskvals++;
      }

      if (umaskvals > 1) {
        ms_logger.println("WARNING: Multiple umask values found.  Using " + umaskval);
      }
    } catch (Exception e) {
