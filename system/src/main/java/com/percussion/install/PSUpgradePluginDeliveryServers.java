...
public class PSUpgradePluginDeliveryServers extends PSUpgradePluginDeliveryServersBase {
  protected void upgradeDeliveryServer(Document doc, Element deliveryServer) {
    ...
    if (user.getTextContent().equals(OLD_USER_VALUE)
        && password.getTextContent().equals(OLD_PASSWORD_VALUE)) {
    ...
    PSXmlDocumentBuilder.addElement(
        doc, deliveryServer, ADMIN_CONNECTION_URL_TAGNAME, getAdminConnectionUrl(connUrlStr));

    logMsg("Adding allow self signed certificate element.");
    PSXmlDocumentBuilder.addElement(
        doc, deliveryServer, ALLOW_SELF_SIGNED_CERT_TAGNAME, TRUE_VALUE);

    logMsg("Adding available services element.");
    Element availableServices =
        PSXmlDocumentBuilder.addEmptyElement(doc, deliveryServer, AVAILABLE_SERVICES_TAGNAME);
    PSXmlDocumentBuilder.addElement(doc, availableServices, SERVICE_TAGNAME, FORM_SERVICE_VALUE);
    PSXmlDocumentBuilder.addElement(doc, availableServices, SERVICE_TAGNAME, FEEDS_SERVICE_VALUE);
    PSXmlDocumentBuilder.addElement(
        doc, availableServices, SERVICE_TAGNAME, COMMENTS_SERVICE_VALUE);
    PSXmlDocumentBuilder.addElement(
        doc, availableServices, SERVICE_TAGNAME, METADATA_SERVICE_VALUE);
  }
