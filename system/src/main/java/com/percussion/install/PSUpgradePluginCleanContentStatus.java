...
public class PSUpgradePluginCleanContentStatus implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      csSchema =
          PSJdbcTableFactory.catalogTable(conn, dbmsDef, dataTypeMap, CONTENT_STATUS_TABLE, true);

      if (csSchema == null) {
        log("null value for tableSchema " + CONTENT_STATUS_TABLE + " table");
      ...
      if (csSchema.getColumn("TITLE") == null || csSchema.getColumn("CONTENTID") == null) {
  ...
  private boolean checkForNullsNonTextAndModify(
    ...
    csModifiedRows.insert(0, "<table name=\"" + CONTENT_STATUS_TABLE + "\">\n");
    csModifiedRows.insert(
        0,
        "\n\nThe following rows with null or non-text TITLE column values" + " were modified:\n\n");
