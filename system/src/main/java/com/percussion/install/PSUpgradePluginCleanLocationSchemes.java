...
public class PSUpgradePluginCleanLocationSchemes implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      lsSchema =
          PSJdbcTableFactory.catalogTable(conn, dbmsDef, dataTypeMap, LOCATION_SCHEME_TABLE, true);

      if (lsSchema == null) {
        log("null value for tableSchema " + LOCATION_SCHEME_TABLE + " table");
      ...
      if ((lsSchema.getColumn("SCHEMEID") == null)
      ...
      if (paramsSchema == null) {
        log("null value for tableSchema " + LOCATION_SCHEME_PARAMS_TABLE + " table");
        log("Table clean-up aborted");
  ...
  private boolean checkForDupsAndModify(
      ...
      final PSJdbcDbmsDef dbmsDef,
      PSJdbcTableSchema lsSchema,
        ...
        for (Map.Entry<String, PSJdbcRowData> keyEntry : paramsKeys.entrySet()) {
          ...
          if ((value != null) && value.equals(schemeId)) {
            paramsDeletedRows.append(
                PSXmlDocumentBuilder.toString(
                    theRow.toXml(doc), PSXmlDocumentBuilder.FLAG_OMIT_XML_DECL));
            cols.add(new PSJdbcColumnData("SCHEMEPARAMID", schemeparamid));
            paramsDeleteData.add(new PSJdbcRowData(cols.iterator(), PSJdbcRowData.ACTION_DELETE));
          }
        }
      } else {
        schemeKeys.add(sk);
    ...
    if (schemeIds.isEmpty()) {
      ...
      try {
        ...
        if (!paramsDeleteData.isEmpty()) {
          tData = new PSJdbcTableData(LOCATION_SCHEME_PARAMS_TABLE, paramsDeleteData.iterator());
          tData.setOnCreateOnly(false);
          paramsSchema.setTableData(tData);
          PSJdbcTableFactory.processTable(conn, dbmsDef, paramsSchema, System.out, false);
        }

        log("Removing non-unique location schemes.");
        tData = new PSJdbcTableData(LOCATION_SCHEME_TABLE, lsDeleteData.iterator());
        tData.setOnCreateOnly(false);
        lsSchema.setTableData(tData);

        PSJdbcTableFactory.processTable(conn, dbmsDef, lsSchema, System.out, false);

        // Write a log of the rows removed so that they can
        // be recreated if necessary
        lsDeletedRows.insert(0, "<table name=\"" + LOCATION_SCHEME_TABLE + "\">\n");
        lsDeletedRows.insert(
            0, "\n\nThe following non-unique location scheme" + " rows were deleted:\n\n");
        lsDeletedRows.append("</table>\n\n");

        if (paramsDeletedRows.length() > 0) {
          paramsDeletedRows.insert(0, "<table name=\"" + LOCATION_SCHEME_PARAMS_TABLE + "\">\n");
          paramsDeletedRows.append("</table>\n\n");
          lsDeletedRows.append(paramsDeletedRows.toString());
        }

        String path = RxUpgrade.getRxRoot() + File.separator + LOG_FILE_NAME;
  ...
  private static final String LOCATION_SCHEME_PARAMS_TABLE = "RXLOCATIONSCHEMEPARAMS";
