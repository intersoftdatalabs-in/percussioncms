...
public class PSJdbcTransitionRoles implements IPSJdbcTableDataHandler {
  ...
  public void init(
    ...
    m_transTblFilter.doAND(
        new PSJdbcSelectFilter(
            COL_TRANSITIONROLES, PSJdbcSelectFilter.IS_NOT_NULL, "", Types.VARCHAR));
    m_transTblFilter.doAND(
        new PSJdbcSelectFilter(
            COL_TRANSITIONROLES, PSJdbcSelectFilter.NOT_EQUALS, COL_VALUE_ALL, Types.VARCHAR));
    m_transTblFilter.doAND(
        new PSJdbcSelectFilter(
            COL_TRANSITIONROLES,
            PSJdbcSelectFilter.NOT_EQUALS,
            COL_VALUE_SPECIFIED,
            Types.VARCHAR));
  }
  ...
  public PSJdbcRowData execute(Connection conn, PSJdbcRowData nullRow)
    ...
    PSJdbcTableSchema transTblSchema =
        PSJdbcTableFactory.catalogTable(conn, m_dbmsDef, dataTypeMap, TBL_TRANSITIONS, false);
    ...
    PSJdbcTableSchema rolesTblSchema =
        PSJdbcTableFactory.catalogTable(conn, m_dbmsDef, dataTypeMap, TBL_ROLES, false);
    ...
    // (TRANSITIONROLES != '*Specified*'))
    String[] transTblColumns =
        new String[] {COL_TRANSITIONID, COL_WORKFLOWAPPID, COL_TRANSITIONROLES};
    ...
    PSJdbcColumnData transColData = new PSJdbcColumnData(COL_TRANSITIONROLES, COL_VALUE_SPECIFIED);
    ...
    try {
      ...
      while (row != null) {
        strTransitionId = getRequiredColumnValue(row, TBL_TRANSITIONS, COL_TRANSITIONID);
        strWorkflowAppId = getRequiredColumnValue(row, TBL_TRANSITIONS, COL_WORKFLOWAPPID);
        strTransitionRoles = getRequiredColumnValue(row, TBL_TRANSITIONS, COL_TRANSITIONROLES);
        ...
        PSJdbcExecutionStep updateStep =
            PSJdbcStatementFactory.getUpdateStatement(m_dbmsDef, transTblSchema, row);
        ...
        if (rolesTblSchema != null) {
          ...
          rolesTblFilter.doAND(
              new PSJdbcSelectFilter(
                  COL_WORKFLOWAPPID, PSJdbcSelectFilter.EQUALS, strWorkflowAppId, Types.INTEGER));
          rolesTblFilter.doAND(
              new PSJdbcSelectFilter(
                  COL_ROLENAME, PSJdbcSelectFilter.EQUALS, strTransitionRoles, Types.VARCHAR));

          PSJdbcTableData roleTblData =
          ...
          if (roleTblData != null) {
            ...
            if (it.hasNext()) {
              ...
              PSJdbcColumnData transRolesColData =
                  new PSJdbcColumnData(COL_TRANSITIONROLEID, roleId);

              row.removeColumn(COL_TRANSITIONROLES);
              row.addColumn(transRolesColData);

              PSJdbcExecutionStep insertStep =
                  PSJdbcStatementFactory.getInsertStatement(m_dbmsDef, m_transRolesTblSchema, row);
