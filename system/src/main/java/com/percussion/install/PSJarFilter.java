...
public class PSJarFilter {
   ...
   */
  public static void filter(File inputJar, File outputJar, List filesToFilter, Map environment)
      throws IOException {
    if (inputJar == null) {
  ...
  private static void copyFilterBytes(
      JarEntry entry, InputStream inputStream, JarOutputStream outputJarStream, Map environment)
      throws IOException {
   ...
   */
  private static String expand(String line, Map environment) throws IOException {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (ch == '$' && i < lenm1 && line.charAt(i + 1) == '{') {
        ...
        if (end < 0) {
          throw new IOException("Invalid replacement variable found on line: " + line);
        }
