...
public class PSUpgradePluginCheckDuplicateKeysInTables implements IPSUpgradePlugin {
  ...
  private void processTable() {
    if (getTableName() == null || getTableColumn() == null) {
      log("PSUpgradePluginCheckDuplicateKeysInTables: No tables to " + "check, but is ok");
      return;
    }
    ...
    try (FileInputStream in =
      ...
      try (Connection conn = RxLogTables.createConnection(props)) {
        ...
        cvSchema =
            PSJdbcTableFactory.catalogTable(conn, dbmsDef, dataTypeMap, getTableName(), true);

        if (cvSchema == null) {
          log("null value for tableSchema " + getTableName() + " table");
        ...
        if (cvSchema.getColumn(getTableColumn()) == null) {
          log("Required column{" + getTableColumn() + "} does not exist, nothing to check");
          return;
        }
  ...
  private boolean checkForDups(
    ...
    log("Checking for primary key constraint violations in " + getTableName() + " table.");
  ...
  private void displayErrorMessage(Set<String> dupKeys) {
    ...
    log(msg);
    log("======================================");
    msg += "Please check the log file in the upgrade directory: " + m_config.getLogFile();
    setUpgradeChecksErrorMsg(msg);

    JOptionPane.showMessageDialog(
        null, msg, "Unique constraints violation", JOptionPane.ERROR_MESSAGE);

    try {
      InstallUtil.restoreVersionPropertyFile(RxUpgrade.getRxRoot());
    } catch (Exception e) {
      log(
          "PSUpgradePluginCheckDuplicateKeysInTables: Unable to restore "
