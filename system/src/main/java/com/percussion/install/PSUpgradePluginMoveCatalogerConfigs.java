...
public class PSUpgradePluginMoveCatalogerConfigs implements IPSUpgradePlugin {
  ...
  protected void moveConfigs(File rxRoot) {
    ...
    File configDir = new File(servletDir, PSServletUtils.getSpringConfigPath());
    File serverBeansFile = new File(configDir, PSServletUtils.SERVER_BEANS_FILE_NAME);
    File catalogerBeansFile = new File(configDir, PSServletUtils.CATALOGER_BEANS_FILE_NAME);

    try {
      // load the server beans xml file
      PSSpringConfiguration serverBeans = new PSSpringConfiguration(serverBeansFile);

      // find the role mgr bean
      Element roleMgrEl = serverBeans.getBeanXml(PSServletUtils.ROLE_MGR_BEAN_NAME);
      if (roleMgrEl == null) {
        logMessage(
            "No definition found for bean with id "
      ...
      // copy subject catalogers to cataloger beans
      Element roleCatEl =
          PSSpringBeanUtils.getNextPropertyElement(roleMgrEl, subCatEl, "roleCatalogers");
      configs.addAll(getCatalogerList(roleCatEl, ConfigTypes.ROLE));

      PSCatalogerConfigurations.saveCatalogerConfigs(configs, catalogerBeansFile);
      ...
      logMessage(
          "Failed to migrate cataloger configs from "
              + serverBeansFile
              + " to "
              + catalogerBeansFile
              + ": "
  ...
  private Collection<PSCatalogerConfig> getCatalogerList(Element source, ConfigTypes type)
    ...
    while (catEl != null) {
      ...
      catEl = PSSpringBeanUtils.getNextPropertyListElement(source, catEl);
    }
  ...
  public static void main(String[] args) {
    ...
    PSUpgradePluginMoveCatalogerConfigs plugin = new PSUpgradePluginMoveCatalogerConfigs();
