...
public class PSPreUpgradePluginDeprecatedSysApps implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    config.getLogStream().println("Scanning files for " + "deprecated system applications");
    ...
    for (int i = 0; i < appFiles.length; i++) {
      ...
      if (appFile.isDirectory() || !appFileName.endsWith(".xml")) continue;
      else {
        if (appFileName.startsWith("sys_")) {
          if (!ms_sysApps.contains(appFileName)) {
            ...
            config.getLogStream().println("Application " + appFileName + " has been deprecated");
          }
        }
      }
    }

    if (deprecatedApps) respType = PSPluginResponse.WARNING;

    response = new PSPluginResponse(respType, respMessage);

    config.getLogStream().println("Finished process() of the plugin Deprecated Sys Apps...");
    return response;
  }
