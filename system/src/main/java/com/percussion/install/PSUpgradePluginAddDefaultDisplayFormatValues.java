...
public class PSUpgradePluginAddDefaultDisplayFormatValues implements IPSUpgradePlugin {
  ...
  private void fixDisplayFormatColumns(Connection conn) throws SQLException {
    ...
    if (!results.next()) {
      logger.println(
          "Display Format Columns not found, inserting corresponding rows for fields sys_title,"
              + " sys_contenttypename, sys_statename, sys_contentlastmodifieddate, sys_postdate,"
