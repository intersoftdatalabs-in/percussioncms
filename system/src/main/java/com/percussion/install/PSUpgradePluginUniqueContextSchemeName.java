...
public class PSUpgradePluginUniqueContextSchemeName implements IPSUpgradePlugin {
  ...
  private List<PSPair<Long, String>> getDedupPairs(List<PSPair<Long, String>> src) {
    ...
    for (PSPair<Long, String> p : src) {
      ...
      PSPair<Long, String> pcopy = new PSPair<>(p.getFirst(), p.getSecond());
      ...
      if (names.contains(pcopy.getSecond())) {
        ...
        if (addToResult) result.add(pcopy);
      }
    }
    return result;
  }
  ...
  private String getUniqueName(String base, Set<String> names) {
    ...
    throw new RuntimeException("Cannot find unique name for Location Scheme: " + base);
  }
  ...
  private void updateDedupNames(
      PrintStream logger, List<PSPair<Long, String>> pairs, Connection conn) throws Exception {
  ...
  private Map<Long, List<PSPair<Long, String>>> getContextSchemeMap(Connection conn)
    ...
    try {
      String sqlStmt = "SELECT CONTEXTID, SCHEMEID, SCHEMENAME FROM " + getSchemeTable();
      PreparedStatement stmt = PSPreparedStatement.getPreparedStatement(conn, sqlStmt);
