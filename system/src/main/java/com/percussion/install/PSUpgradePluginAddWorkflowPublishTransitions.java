...
public class PSUpgradePluginAddWorkflowPublishTransitions implements IPSUpgradePlugin {
  ...
  private void updateWorkflow() throws SQLException {
    logger.println(
        "Running upgrade plugin to add the Publish transition to system and custom steps.");
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void addPublishTransitionAccordingApproveTransition() throws SQLException {
    ...
    List<Integer> publishRolesTransitionId =
        getDifference(
  ...
  private void addRemoveTransition() throws SQLException {
    ...
    String query =
        "INSERT INTO "
            + transitionTable
            + "(TRANSITIONID, TRANSITIONTYPE, TRANSITIONLABEL, TRANSITIONFROMSTATEID,"
            + " TRANSITIONTOSTATEID, TRANSITIONACTIONTRIGGER, TRANSITIONAPPROVALSREQUIRED,"
            + " TRANSITIONDESC, WORKFLOWAPPID, TRANSITIONROLES, TRANSITIONCOMMENTREQUIRED,"
            + " DEFAULTTRANSITION) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void addRoleTransition(int transitionId, int transitionRoleId) throws SQLException {
    // Add a role transition
    logger.println(
        "Adding role ID: " + transitionRoleId + " for publish transition ID: " + transitionId);
    String query =
        "INSERT INTO "
            + transitionRoleTable
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private void changeAssignmentTypeRole(int stateId, int roleId, int assignmentType)
      throws SQLException {
    // Update a state role
    logger.println("Updating role ID: " + roleId + " for state ID: " + stateId);
  ...
  private int getStateId(String state) throws SQLException {
    ...
    String query = "SELECT STATEID FROM " + stateTable + " WHERE STATENAME = ?";
  ...
  private int getRoleId(String role) throws SQLException {
    ...
    String query = "SELECT ROLEID FROM " + roleTable + " WHERE WORKFLOWAPPID = ? AND ROLENAME = ?";
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private List<Integer> getTransitionRoleId(List<Integer> transitionsId) throws SQLException {
    List<Integer> ids = new ArrayList<>();

    for (Integer transitionId : transitionsId) {
      logger.println("Finding all transitions roles associated to approve transition.");
      String query =
          "SELECT DISTINCT TRANSITIONROLEID FROM "
              + transitionRoleTable
   ...
   * @throws SQLException if any error occurs during DB access.
   */
  private boolean existTransition(Integer transition, Integer transitionRole) throws SQLException {
    Boolean exist = false;
