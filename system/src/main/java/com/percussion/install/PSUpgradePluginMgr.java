...
public class PSUpgradePluginMgr {
  ...
  public PSUpgradePluginMgr(IPSUpgradeModule config, Element elemPlugin) {
    ...
    m_class = InstallUtil.getElemValue(elemPlugin, IPSUpgradeModule.ELEM_CLASS);
    m_elemData = InstallUtil.getElement(elemPlugin, IPSUpgradeModule.ELEM_DATA);
    m_config = config;
  }
    ...
    try {
      ...
      if (obj instanceof IPSUpgradePlugin) {
        ...
        if (RxUpgrade.ms_bPreUpgrade) {
          if (response != null) RxUpgrade.addResponse(response);
        }
      }
    } catch (Exception e) {
