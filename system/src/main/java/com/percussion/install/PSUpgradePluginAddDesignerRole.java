...
public class PSUpgradePluginAddDesignerRole extends PSSpringUpgradePluginBase {
  private static final String DESIGNER_ROLE_DESC =
      "Designers have the same abilities as Admins, except that they do not have the ability to"
          + " manage workflows, users, roles, and folders, and cannot create, delete, or copy"
          + " sites.";
    ...
    for (PSState state : states) {
      ...
      for (PSAssignedRole assignedRole : assignedRoles) {
        ...
        if (asRoleId == roleId.getUUID()) {
          logger.println(
              "Setting " + DESIGNER_ROLE_NAME + " role as assignee for state: " + state.getName());
      ...
      if (!found) {
        logger.println(
            "Error: "
                + DESIGNER_ROLE_NAME
                + " role not found as assigned role for state: "
                + state.getName());
        return;
      }
