...
public class PSPreUpgradePluginRelationship implements IPSUpgradePlugin {
    ...
    try (PrintStream logger = module.getLogStream()) {
      ...
      try {
        ...
        try (Connection conn = RxUpgrade.getJdbcConnection()) {
          ...
          if (!validateEmptyExpiretime(logger, configs)) successValidated = false;

          if (!validateRelationshipName(logger, configs, conn, util)) successValidated = false;
          ...
          if (successValidated) {
            ...
            respMessage =
                "Finished validating relationship data, see the "
                    + "\""
                    + log
                    + "\" located in "
                    + RxUpgrade.getPreLogFileDir()
        ...
        respMessage =
            "<<<ERROR>>>: Failed to validate relationship data, see "
                + "the \""
                + log
                + "\" located in "
                + RxUpgrade.getPreLogFileDir()
        ...
        if (successValidated)
          logger.println(
              "leaving the process() of the Validating Relationships plugin, successfully validated"
                  + " without warning or error.");
        else
          logger.println(
              "leaving the process() of the Validating Relationships plugin with WARNINGs.");
      }
  ...
  private boolean validateUnknownRelationshipProperties(
    ...
    String sqlStmt =
        "select distinct p.PROPERTYNAME from "
            + util.getOldRelPropTable()
            + " p, "
            + util.getOldRelMainTable()
            + " r "
            + "where r.RID = p.RID and r.CONFIG = '"
            + config.getName()
            + "' "
            + "and p.PROPERTYNAME not in ("
            + buf.toString()
            + ")";
    ...
    if ((!unknownNames.isEmpty())
        && config.getCategory().equalsIgnoreCase(PSRelationshipConfig.CATEGORY_ACTIVE_ASSEMBLY)) {
      unknownNames.removeAll(PSRelationshipConfig.getPreDefinedUserPropertyNames());
    }
  ...
  private boolean validateExtensionSet(PrintStream logger, Document doc) {
    ...
    NodeList extensions = doc.getDocumentElement().getElementsByTagName("ExtensionSet");
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (ext != null) {
        logWarningMsg(
            logger,
            "Found an unknown relationship extension, '"
  ...
  private PSConditionalExtension getUnknownExit(PrintStream logger, Element exitSetNode) {
    ...
    if (chidNodes == null || chidNodes.getLength() == 0) return null;
    ...
    while (childEl != null) {
      ...
      if (extension.getExtension().getName().equalsIgnoreCase(KNOWN_EXIT)) {
        logger.println(
            "Found the known relationship extension, '"
                + KNOWN_EXIT
                + "', which will be removed, and will be replaced by 'sys_isCloneExists' effect"
  ...
  private boolean validateRelationshipName(
    ...
    for (String name : rs) {
      if (configs.getConfig(name) == null) {
        logWarningMsg(
            logger,
            "Found undefined relationship config name, '"
                + name
                + "'. This warning may be ignored if the relationship "
