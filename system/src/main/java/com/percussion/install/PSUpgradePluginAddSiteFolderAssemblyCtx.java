...
public class PSUpgradePluginAddSiteFolderAssemblyCtx implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule module, Element elemData) {
    ...
    try {
      ...
      logger.println("leaving the process() of the Add Site Folder " + "Assembly Context plugin.");
    }
   ...
   */
  private String getContextTable() throws FileNotFoundException, IOException {
  ...
  private Long findContext(String name, PrintStream logger, Connection conn) throws Exception {
    ...
    try {
      String sqlStmt =
          "SELECT CONTEXTID FROM " + getContextTable() + " WHERE CONTEXTNAME='" + name + "'";
  ...
  private void insertSiteFolderAssemblyCtx(PrintStream logger, Connection conn) throws Exception {
    ...
    String insertStmt = "INSERT INTO " + contextTable + " VALUES(?, ?, ?, ?, NULL)";

    PreparedStatement stmt = null;
    try {
      Long contextid = Long.valueOf(PSGuidHelper.generateNextLong(PSTypeEnum.CONTEXT));
    ...
    logger.println("Successfully inserted " + rowCount + " row(s) in " + contextTable);
  }
  ...
  private void updateSiteFolderAssemblyCtx(long ctxId, PrintStream logger, Connection conn)
    ...
    String updateStmt =
        "UPDATE " + contextTable + " SET CONTEXTNAME='" + NEW_CONTEXT_NAME + "' WHERE CONTEXTID=?";
