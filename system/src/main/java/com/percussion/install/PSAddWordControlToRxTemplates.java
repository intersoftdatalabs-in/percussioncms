...
/** This plugin has been written to add the sys_FileWord control to rx_Templates.xsl file. */
public class PSAddWordControlToRxTemplates implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    config.getLogStream().println("Adding the word control to " + "rx_Templates.xsl...");
    ...
    try {
      rxfile = new File(RxUpgrade.getRxRoot() + "rx_resources/stylesheets/rx_Templates.xsl");
      ...
      upgfile = new File(RxUpgrade.getRxRoot() + "sys_resources/stylesheets/sys_Templates.xsl");

      upgfis = new FileInputStream(upgfile);
      Document tempdoc = PSXmlDocumentBuilder.createXmlDocument(upgfis, false);
      ...
      if (cnl == null || cnl.getLength() < 1) {
        config
            .getLogStream()
            .println(
                "ControlMeta elements are missing in \n"
      ...
      for (int i = 0; i < cnl.getLength(); i++) {
        ...
        if (wcelem.getAttribute("name").equalsIgnoreCase("sys_FileWord")) break;
      }
      if (wcelem == null) {
        config
            .getLogStream()
            .println(
                "WordControl element is missing in \n"
                    + upgfile.getAbsolutePath()
                    + "Skipped adding WordControl. Leaving the plugin...");
        return null;
      }
      ...
      String result = rxstr.substring(0, nLoc) + upgstr + rxstr.substring(nLoc);
      writer = new OutputStreamWriter(new FileOutputStream(rxfile), "UTF8");
      writer.write(result);
      writer.flush();
    } catch (IOException e) {
