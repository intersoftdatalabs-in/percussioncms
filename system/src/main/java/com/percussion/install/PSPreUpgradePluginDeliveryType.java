  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (url == null) {
          config.getLogStream().println("Url does not exist for content " + "list '" + name + "'");
          continue;
        }
        ...
        if (!ms_deliveryTypes.contains(value)) customTypes.add(value);
      ...
      if (customTypes.size() > 0) {
        ...
        respMessage += RxInstallerProperties.getString("customDeliveryTypes") + "\n\n" + types;
      }
      ...
      if (ftpTypesFound) {
        ...
        respMessage += RxInstallerProperties.getString("ftpDeliveryTypes");
      }
    } catch (Exception e) {
      e.printStackTrace(config.getLogStream());
      respType = PSPluginResponse.EXCEPTION;
      respMessage =
          "Failed to check delivery types, see the \""
              + config.getLogFile()
              + "\" located in "
    ...
    config.getLogStream().println("Finished process() of the plugin Delivery Types...");
    return response;
  }
