...
public class PSUpgradePluginAddLicenseMonitorToDashboard implements IPSUpgradePlugin {
  ...
  public static final String NEW_GADGET_URL =
      "/cm/gadgets/repository/perc_license_monitor_gadget/perc_license_monitor_gadget.xml";
  ...
  public PSPluginResponse process(
      IPSUpgradeModule module, @SuppressWarnings("unused") Element elemData) {
  ...
  private void addLicenseMonitorToDashboards(Connection conn) throws Exception {
    try {
      ...
      for (String admin : admins) {
        ...
        if (!results.next()) {
          logger.println(
              "Admin '" + admin + "' does not have a Dashboard configuration (skipping).");
          continue;
        }

        // Process the DATA value and get it as string
        // logger.println("Updating " + this.UPDATED_COLUMN + " in " + this.UPDATED_TABLE + ", row
        // with: " + results.getRow());
        logger.println(
            "Updating the dashboard representation ('perc.user."
                + admin
                + ".dash.page.0') for admin: "
                + admin);
        ...
        String updateDataValueQuery =
            "UPDATE "
                + this.UPDATED_TABLE
                + " SET METAKEY = ?, DATA = ? WHERE METAKEY = '"
                + results.getString("METAKEY")
                + "'";
  ...
  private String processDashboardMetadata(String dashboardMetadata) {
    ...
    String delimiter = "]}}";
    proccessedDashboardMetadata =
        proccessedDashboardMetadata.replaceFirst("]}}", licenseMonitorGadget);
  ...
  private String modifyCurrentGadgetsPositions(String dashboardMetadata) {
    ...
    while (matcher.find()) {
      matcher.appendReplacement(
          result, "\"col\":1,\"row\":" + (Integer.parseInt(matcher.group(1)) + 1));
    }
