...
public class PSPreUpgradePluginLocalCreds implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    config
        .getLogStream()
        .println("Scanning application files for " + "local back-end credentials.");
    ...
    try {
      for (int i = 0; i < appFiles.length; i++) {
        ...
        if (appFile.isDirectory() || !appFileName.endsWith(".xml") || isSystemApp(appFileName))
          continue;
        else {
          if (hasLocalBECreds(appFile)) {
            ...
            config
                .getLogStream()
                .println("Application " + appFileName + " contains local back-end credentials");
          }
        }
      }

      if (localCreds) respType = PSPluginResponse.EXCEPTION;
    } catch (Exception e) {
      e.printStackTrace(config.getLogStream());
      respType = PSPluginResponse.EXCEPTION;
      respMessage =
          "Failed to validate existing applications, see the "
              + "\""
              + log
              + "\" located in "
    ...
    config.getLogStream().println("Finished process() of the plugin Local Back-end Credentials...");
