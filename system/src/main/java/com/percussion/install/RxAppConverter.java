...
public class RxAppConverter {
  ...
  public static void updateRxApp(
    ...
    if (strUpdateSecProv != null && strUpdateSecProv.length() > 0) {
    ...
    if (bUpdateNativeStatement) {
      ...
      if (backend.equalsIgnoreCase("ORACLE")) nativeStatement = NATIVE_STATEMENT_ORACLE;
      ...
      if (null != nl) {
        ...
        } else text.setData(nativeStatement);
      }
    }
  ...
  public static void modifyPort(String sPort, Document doc) {
    ...
    if (null != sPort) // ignore if port is null
    {
      ...
      if (!sPort.equals("80")) replacePort = new String(":" + sPort);
      ...
      for (int i = 0; (null != nl) && i < nl.getLength(); i++) {
        ...
        if (null == node || !(node instanceof Text)) continue;
  ...
  private static Element setElementData(Element parent, String elemName, String elemValue) {
    NodeList childs = parent.getElementsByTagName(elemName);
    if (null == childs || childs.getLength() < 1) return null;
  ...
  public static void main(String[] args) {
    if (args.length < 3) {
      ...
      System.out.println(
          " RxAppConverter <Properties file name> "
    ...
    if (rxRoot.isDirectory()) PSEntityResolver.getInstance().setResolutionHome(rxRoot);
    ...
    while (tokenizer.hasMoreElements()) {
      ...
      try {
        ...
        System.out.println("Failed to covert application '" + temp + "' Error: " + e.getMessage());
      }
    }
  ...
  public static void loadProps(String[] args, Properties props) {
    try {
      ...
      System.out.println(
          "Failed to load properties file '" + args[0] + "' Error: " + ioe.getMessage());
      System.out.println("Press ENTER to continue...");
      try {
        System.in.read();
      } catch (Exception e) {
