...
public class PSUpgradePluginLocationSchemeTable implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    String bkptablename = "RXLOCATIONSCHEMEPARAMS" + PSJdbcPlanBuilder.BACKUP_SUFFIX;
    try {
      in =
          new FileInputStream(
      ...
      if (paramsDoc == null) {
        config.getLogStream().println("Could not extract data out of " + bkptablename + " table");
        config.getLogStream().println("Table upgrade aborted");
        return null;
      }

      Document schemesDoc =
          PSUpgradePluginGeneralTables.getTableDataDoc(
              conn, dbmsDef, dataTypeMap, "RXLOCATIONSCHEME");
      if (schemesDoc == null) {
        config.getLogStream().println("Could not extract data out of RXLOCATIONSCHEME table");
        config.getLogStream().println("Table upgrade aborted");
        return null;
      }
      ...
      tableSchema =
          PSJdbcTableFactory.catalogTable(
              conn, dbmsDef, dataTypeMap, "RXLOCATIONSCHEMEPARAMS", true);
      if (tableSchema == null) {
        config.getLogStream().println("nulll value for tableSchema RXLOCATIONSCHEMEPARAMS table");
        config.getLogStream().println("Table upgrade aborted");
        return null;
      }

      paramsDoc.getDocumentElement().setAttribute("onCreateOnly", "no");
      PSJdbcTableData tableData = new PSJdbcTableData(paramsDoc.getDocumentElement());
