...
public class PSJdbcTableMapper implements IPSJdbcTableDataHandler {
  ...
  public PSJdbcRowData execute(Connection conn, PSJdbcRowData row)
    ...
    if (m_tableMapEl == null) throw new IllegalStateException("row mappings not defined");
    ...
    PSJdbcTableSchema srcSchema =
        PSJdbcTableFactory.catalogTable(conn, m_dbmsDef, dataTypeMap, m_srcTable, false);
    boolean srcTableExists = (srcSchema != null);
    if (!srcTableExists) return null;

    // check if the destination table exists
    PSJdbcTableSchema destSchema =
        PSJdbcTableFactory.catalogTable(conn, m_dbmsDef, dataTypeMap, m_destTable, false);
    boolean destTableExists = (destSchema != null);
    if (!destTableExists) return null;

    PSJdbcTableMapping tableMapping = new PSJdbcTableMapping(m_dbmsDef, destSchema);
   ...
   */
  private void applyTableMapping(
      Connection conn,
      PSJdbcTableMapping tableMapping,
      PSJdbcTableSchema srcSchema,
      PSJdbcTableSchema destSchema)
      throws PSJdbcTableFactoryException {
    PSJdbcResultSetIteratorStep step =
        PSJdbcStatementFactory.getResultSetIteratorStatement(
            m_dbmsDef, srcSchema, null, null, PSJdbcRowData.ACTION_INSERT);
  ...
  public void fromXml(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    Element tableMapEl = walker.getNextElement(PSJdbcTableMapping.NODE_NAME, firstFlags);
