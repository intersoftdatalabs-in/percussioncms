...
public class PSPreUpgradePluginCheckOracleVersion implements IPSUpgradePlugin {
  ...
  public PSPluginResponse process(IPSUpgradeModule config, Element elemData) {
    ...
    try {
      ...
      if (conn != null) {
        ...
        if (dbName.indexOf(m_oracleName.toLowerCase()) != -1) {
          ...
          if (PSSqlHelper.compareVersions(conn, PSSqlHelper.MIN_VERSION_ORACLE) >= 0)
      ...
      respType = PSPluginResponse.EXCEPTION;
      respMessage =
          "Failed to check oracle version for rhythmyx repository, "
              + "see the \""
              + log
              + "\" located in "
    ...
    config.getLogStream().println("Finished process() of the plugin Check Oracle Version...");
