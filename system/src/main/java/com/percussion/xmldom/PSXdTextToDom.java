...
/**
 * A Rhythmyx extension used to translate a text field into a temporary XML DOM document.
 *
 ...
 * </table>
 *
 * <p>When used as a pre-exit, the source text field is always an HTML parameter. This exit will
 * automatically detect if the uploaded field is an attached file or an HTML field, and process it
 * accordingly. In a post-exit, the source always a single XML node.
 *
 * <p>The resulting XML document is stored in a temporary object. The default name of this object is
 * <code>XMLDOM</code>. In a pre-exit, this object name may also be <code>InputDocument</code>. In
 * this case, the XML document is passed to the Rhythmyx server for mapping to fields.
 *
 * <p>The Tidy properties file is optional. If it is provided, the text will be run through the HTML
 * Tidy program before parsing. See the <a href="http://www.w3.org/People/Raggett/tidy"
 * target="_blank"> W3C HTML Tidy</a> page for details and properties file formats. Note that this
 * implementation uses the Java version of Tidy, which is not exactly identical to the C
 * implementation. The pathname provided must be relative to the Rhythmyx server root.
 *
 * <p>The ServerPageTags.xml file is used to elmininate certain non-parsible text generated by some
 * editting programs. For details see {@link ProcessServerPageTags ProcessServerPageTags}. The
 * pathname is relative to the server root.
 *
 * <p>Rhythmyx provides default files for Tidy (rxW2Ktidy.properties) and ServerPageTags
...
public class PSXdTextToDom extends PSDefaultExtension
   ...
   *     PSXdTextToDom} for parameter details.
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (params.length < 1 || null == params[0] || 0 == params[0].toString().trim().length()) {
      throw new PSParameterMismatchException(params.length, 1);
    }
    String fileParamName = PSXmlDomUtils.getParameter(params, 0, null);

    String XMLPrivateObjectName =
        PSXmlDomUtils.getParameter(params, 1, PSXmlDomUtils.DEFAULT_PRIVATE_OBJECT);
    ...
    try {
      ...
      if (inputSourceObj instanceof PSPurgableTempFile) {
        ...
        PSPurgableTempFile inputSourceFile = (PSPurgableTempFile) inputSourceObj;
        request.printTraceMessage("Loading file " + inputSourceFile.getSourceFileName());

        encoding =
            PSXmlDomUtils.determineCharacterEncoding(contxt, inputSourceFile, encodingDefault);
        if (encoding == null) {
          tempXMLDocument = PSXmlDomUtils.loadXmlDocument(contxt, (File) inputSourceObj);
        } else {
          tempXMLDocument = PSXmlDomUtils.loadXmlDocument(contxt, (File) inputSourceObj, encoding);
        }
      } else {
        // inputSourceObj must be a String
        tempXMLDocument = PSXmlDomUtils.loadXmlDocument(contxt, inputSourceObj.toString());
      }

      if (XMLPrivateObjectName.equals("InputDocument")) request.setInputDocument(tempXMLDocument);
      else request.setPrivateObject(XMLPrivateObjectName, tempXMLDocument);
    } catch (Exception e) {
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      String textSource = sourceWalker.getElementData(textSourceName, true);
  ...
  /**
   * This method performs the common parameter checking and parsing. These parameters are the same
   * in both pre-exits and post-exits.
   */
    ...
    if (params.length >= 3 && null != params[2] && params[2].toString().trim().length() > 0) {
      try {
        ...
        contxt.printTraceMessage(
            "Tidy Properties file " + params[2].toString().trim() + " not found ");
