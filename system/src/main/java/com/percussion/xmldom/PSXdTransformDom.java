...
/**
 * A Rhythmyx extension that applies an XSL Stylesheet to a temporary XML document.
 *
 ...
 * </table>
 *
 * <p>The XSL Stylesheet must reside in the current application directory. The easiest way to
 * accomplish this is to attach it to a query in the current application. Through a combination of
 * selection parameters, it is possible to prevent this stylesheet from being used except for as
 * part of this exit.
 *
 * <p>When used as a pre-exit, the special XML document name <code>InputDocument</code> may be used.
 * This name refers to the input XML document. Usually, this document is provided by the
 * PSXmlUploader.
 *
 * <p>When used as a post-exit, the name <code>ResultDocument</code> may be used as the source
 * and/or destination object. This name refers to the document generated by the query or update
 * resource to which this exit is attached; this is the document that will be returned to the
 * requestor, usually after being transformed into HTML by an attached stylesheet.
 *
 * <p>This extension parses the output of the XSL processor and stores it as an XML document. For
 * this reason, the output must be well-formed. The best way to insure this is to use <code>
 * &lt;xsl:output method="xml"&gt;</code>
 *
...
public class PSXdTransformDom extends PSDefaultExtension
   ...
   *     PSXdTransformDom} for parameter details.
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    try {
      Document sourceDoc;
      if (sourceObjectName.equals("InputDocument")) sourceDoc = request.getInputDocument();
      else sourceDoc = (Document) request.getPrivateObject(sourceObjectName);
      ...
      Document outDoc =
          processXmlDoc(
              cx,
              sourceDoc,
              PSXmlDomUtils.getFullStyleName(stylesheetName, request.getCurrentApplicationName()));

      if (destObjectName.equals("InputDocument")) request.setInputDocument(outDoc);
      else request.setPrivateObject(destObjectName, outDoc);
    } catch (Exception e) {
  ...
  public Document processResultDocument(
    ...
    try {
      Document sourceDoc;
      if (sourceObjectName.equals("ResultDocument")) sourceDoc = resultDoc;
      else sourceDoc = (Document) request.getPrivateObject(sourceObjectName);
      ...
      if (destObjectName.equals("ResultDocument")) resultDoc = outDoc;
      else request.setPrivateObject(destObjectName, outDoc);
    } catch (Exception e) {
      cx.handleException(e);
    }
  ...
  /**
   * This method handles the common parts of the pre-exit and the post-exit. The stylesheet is
   * applied to the source and the result is parsed back into XML.
   *
   ...
   * @throws IOException
   */
  private static Document processXmlDoc(PSXmlDomContext cx, Document xmlDoc, String stylesheetName)
      throws PSExtensionProcessingException, TransformerException, SAXException, IOException {
    ...
    PSCachedStylesheet styleCached = PSStylesheetCacheManager.getStyleSheetFromCache(styleFile);
    ...
    try {
      ...
      DOMResult res = new DOMResult((Node) PSXmlDocumentBuilder.createXmlDocument());
      ...
      if (cx.isLogging() && (errorListener.numErrors() + errorListener.numFatalErrors() > 0)) {
      ...
      if (cx.isLogging()) {
        ...
        try (FileOutputStream parsedOutput = new FileOutputStream("xmldocxformout.doc")) {

          PSXmlTreeWalker walk = new PSXmlTreeWalker(outputDoc);
          walk.write(new BufferedWriter(new OutputStreamWriter(parsedOutput, "UTF-8")), true);
        }
      }
      ...
      errorMsg.append("\r\n");
      PSStylesheetCacheManager.appendErrorMessages(errorMsg, styleCached.getErrorListener());
