...
/**
 * A Rhythmyx extension that applies an XSL Stylesheet to a temporary XML document and stores the
 ...
 * object. This name refers to the document generated by the query or update resource to which this
 * exit is attached; this is the document that will be returned to the requestor, usually after
 * being transformed into HTML by an attached stylesheet.
 *
 * <p>This extension stores the result of the XSL processor as a text string. In a pre-exit, this
 * value is always an HTML parameter. In a post-exit, the result is stored in an node in the XML
 * result document. The name of this name node may contain more than one level (e.g. <code>
 * category/fieldname</code>)/
 *
 * <p>Because the output is not parsd, it does not have to be well-formed. The stylesheet may create
 * XML, HTML or even plain Text without any problems.
 *
...
public class PSXdTransformDomToText extends PSDefaultExtension
   ...
   * @throws IOException
   */
  private static String processXmlDoc(PSXmlDomContext cx, Document xmlDoc, String stylesheetName)
