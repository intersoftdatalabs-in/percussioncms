...
public class PSRemoteCommandHandler implements IPSCommandHandler {
  ...
  public PSProcessRequestResult executeProcess(
    ...
    try {
      ...
      PSXmlDocumentBuilder.replaceRoot(
      ...
      resultDoc =
          PSXmlDocumentBuilder.createXmlDocument(new StringReader(result.toString()), false);
  ...
  public PSProcessRequestResult waitOnProcess(int handle, int wait) throws PSProcessException {
    ...
    try {
      ...
      PSProcessDaemon.sendCommand(
          m_server, m_port, PSProcessDaemon.CMD_WAIT_FOR_PROCESS, params, result);
      resultDoc =
          PSXmlDocumentBuilder.createXmlDocument(new StringReader(result.toString()), false);
      return new PSProcessRequestResult(resultDoc.getDocumentElement());
    } catch (Exception e) {
  ...
  public boolean fileSystemObjectExists(File path) throws PSProcessException {
    ...
    try {
      ...
      int resultCode =
          PSProcessDaemon.sendCommand(
              m_server, m_port, PSProcessDaemon.CMD_FS_OBJ_EXISTS, params, result);
      if (resultCode != 0) throw new PSProcessException(result.toString());
  ...
  public void removeFileSystemObject(File path) throws PSProcessException {
    ...
    try {
      ...
      int resultCode =
          PSProcessDaemon.sendCommand(
              m_server, m_port, PSProcessDaemon.CMD_REMOVE_FS_OBJ, params, result);
      if (resultCode != 0) throw new PSProcessException(result.toString());
    } catch (IOException e) {
      throw new PSProcessException(e.getLocalizedMessage());
    }
  ...
  public void makeDirectories(File path) throws PSProcessException {
    ...
    try {
      ...
      int resultCode =
          PSProcessDaemon.sendCommand(
              m_server, m_port, PSProcessDaemon.CMD_MAKE_DIRS, params, result);
      if (resultCode != 0) throw new PSProcessException(result.toString());
    } catch (IOException e) {
      throw new PSProcessException(e.getLocalizedMessage());
    }
  ...
  public void saveTextFile(File path, String content) throws IOException {
    ...
    int resultCode =
        PSProcessDaemon.sendCommand(
            m_server, m_port, PSProcessDaemon.CMD_SAVE_FILE, params, result);
    if (resultCode != 0) throw new IOException(result.toString());
  }
  ...
  public void saveBinaryFile(File path, InputStream src) throws IOException {
    ...
    int resultCode =
        PSProcessDaemon.sendCommand(
            m_server, m_port, PSProcessDaemon.CMD_SAVE_BINARY_FILE, params, result);
    if (resultCode != 0) throw new IOException(result.toString());
  }
  ...
  public String getTextFile(File path) throws IOException {
    ...
    params.add(virtualPath);
    int resultCode =
        PSProcessDaemon.sendCommand(m_server, m_port, PSProcessDaemon.CMD_GET_FILE, params, result);
    if (resultCode != 0) throw new IOException(result.toString());
