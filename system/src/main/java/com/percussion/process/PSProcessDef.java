...
public class PSProcessDef {
  ...
  private void fromXml(Element el) throws PSProcessException {
    ...
    if (!(tagName.equals(NODE_NAME)))
      throw new IllegalArgumentException(
          "Invalid node element specified: (" + el.getTagName() + ") Expected (" + NODE_NAME + ")");
      ...
      if (node instanceof Element) {
        ...
        if (childTagName.equals(EL_DIR)) {
          ...
          throw new PSProcessException(
              "Invalid child element: (" + childTagName + ") of element (" + NODE_NAME + ")");
        }
      }
    }
  }
  ...
  public String[] getCommandParams(Map<String, ?> ctx)
    ...
    for (int i = 0; i < m_cmdParams.length; i++) {
      ...
      if (def.getIfDefinedName() != null && !ctx.containsKey(def.getIfDefinedName())) {
        ...
        if (def.isBeginGroup()) isGroup = true;
        if (def.isEndGroup()) {
          isGroup = false;
          cmdList.add(param);
          param = "";
      ...
      if (name != null && def.getSeparator() != null) {
        ...
        if (isGroup) param += pair; // accumulate params that belong to same group
        else cmdList.add(pair); // single param
      } else {
        // add as separate params
        if (name != null) {
          if (isGroup) param += name;
          else cmdList.add(name); // add single name
        }

        if (val.trim().length() > 0) {
          if (isGroup) param += (((name != null) ? " " : "") + val);
          else cmdList.add(val); // add single value
        }
      }
