...
public class PSProcessAction implements Runnable {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String[] params = new String[] {"SetupConsole", "-console"};
  ...
  public static void getNextInput(StringBuilder buf) throws Exception {
    boolean done = false;
    ...
    while (!done) {
      ...
      local.append(frag);
      Thread.sleep(1000);
      if (local.toString().toLowerCase().indexOf("press") > 0) done = true;
      log.info("X");
    }
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (m_process.getInputStream() != null) {
        m_stdOutputReader =
            new OutputReader(
                m_process.getInputStream(), m_stdOutBuf, m_cmdArray[0] + " Std Output Reader");
      }
      if (m_process.getErrorStream() != null) {
        m_stdErrorReader =
            new OutputReader(
                m_process.getErrorStream(), m_stdOutBuf, m_cmdArray[0] + " Error Output Reader");
      }
  ...
  public String toString() {
    ...
    for (int i = 0; i < m_cmdArray.length; i++) {
      ...
      if (i < m_cmdArray.length - 1) buf.append(" ");
    }
    return buf.toString();
  }
...
class OutputReader extends Thread {
  ...
  public OutputReader(InputStream source, StringBuilder buf, String name) {
    ...
    if (null == name || name.trim().length() == 0) name = "Input Reader";
        ...
        if (count > 0) {
          ...
          synchronized (m_lock) {
            if ((m_buffer.length() + count) > BUFFERING_COUNT) {
              ...
              m_buffer = new StringBuilder(m_buffer.substring(x, m_buffer.length()));
            }
