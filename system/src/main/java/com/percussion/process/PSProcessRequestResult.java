...
public class PSProcessRequestResult {
  ...
  public PSProcessRequestResult(
    ...
    if (status < 0 || status >= STATUS_VALUES.length) {
      throw new IllegalArgumentException("status must be one of the STATUS_xxx values");
    }
    if (status != STATUS_STARTED || actionHandle < 1) actionHandle = -1;
  ...
  public PSProcessRequestResult(Element src) throws Exception {
    ...
    m_status = PSXMLDomUtil.checkAttributeEnumerated(src, STATUS_ATTR, STATUS_VALUES, false);
    String resultCode = PSXMLDomUtil.checkAttribute(src, RESULT_CODE_ATTR, true);
    m_resultCode = Integer.parseInt(resultCode);
    if (m_status == STATUS_STARTED) {
      String actionHandle = PSXMLDomUtil.checkAttribute(src, ACTION_HANDLE_ATTR, false);
      m_actionHandle = actionHandle.trim().length() == 0 ? -1 : Integer.parseInt(actionHandle);
    }
  ...
  private void setResultText(String text) {
    ...
    StringBuilder buf = new StringBuilder(m_resultText);
    StringBuilder cleanBuf = new StringBuilder(m_resultText.length());
    for (int i = 0; i < buf.length(); i++) {
      char c = buf.charAt(i);
      if (!Character.isISOControl(c) || Character.isWhitespace(c)) cleanBuf.append(c);
    }
  ...
  private String[] STATUS_VALUES = {
    ...
    "finished", "error", "terminated", "started"
  };
