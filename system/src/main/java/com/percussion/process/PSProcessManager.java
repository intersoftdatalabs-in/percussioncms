...
public class PSProcessManager {
  ...
  public PSProcessManager(InputStream stream) throws IOException, SAXException, PSProcessException {
    ...
    try {
      ...
      if ((root == null) || (!root.getTagName().equals(NODE_NAME)))
        throw new PSProcessException("Invalid root element of process definitions.");
      ...
      for (int i = 0; i < nl.getLength(); i++) {
        ...
        if (node instanceof Element) {
          ...
          IPSProcess process = (IPSProcess) Class.forName(processType).newInstance();
          process.fromXml(el);
      ...
      throw new PSProcessException("Class not found: " + cls.getLocalizedMessage());
    } catch (IllegalAccessException iae) {
      throw new PSProcessException("Illegal Access Exception: " + iae.getLocalizedMessage());
    } catch (InstantiationException ins) {
      throw new PSProcessException("Instantiation Exception: " + ins.getLocalizedMessage());
    } finally {
      if (stream != null) {
        try {
          stream.close();
  ...
  private static void setOS() {
    ...
    if (osName.indexOf("sunos") != -1) {
      ...
      throw new RuntimeException("Unable to determine the operating system.");
    }
  }
  ...
  public static int getOSType(String os) throws PSProcessException {
    ...
    for (int i = 0; i < OS_TYPES.length; i++) {
      if (OS_TYPES[i].equalsIgnoreCase(os)) return i;
    }
