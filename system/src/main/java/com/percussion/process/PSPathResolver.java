...
/**
 * PSPathResolver class resolves file path. If a path relative to the Rhythmyx root directory is
 * specified as value, then it resolves it to an absolute path with appropriate path separator. When
 * specifying the value for this resolver, always use Unix file separator. The returned path
 * contains separators appropriate for the OS.
 *
 * <p>Example: If the path is specified as: &lt;param
 * resolver="com.percussion.process.PSPathResolver"
 * value="./sys_search/rware70/rx/config/rware.cfg"/>
 *
 * <p>then <code>getValue</code> method returns the following value:
 *
 * <p>On Windows (assuming "C:\Rhythmyx" is the Rhythmyx root directory) :
 * "C:\Rhythmyx\sys_search\rware70\rx\config\rware.cfg"
 *
 * <p>On Unix (assuming "/home/Rhythmyx" is the Rhythmyx root directory) :
