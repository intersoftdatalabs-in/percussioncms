...
public class PSArchiveFiles {
   ...
   */
  public static ZipFile openArchive(String archiveName, String type) throws IOException {
    // validate input
    if (archiveName == null) throw new IllegalArgumentException("arvhiveName may not be null");

    if (type == null) type = ZIP_FILE_TYPE;

    // open the zip file
    File archiveFile = new File(archiveName);

    if (type.equalsIgnoreCase(JAR_FILE_TYPE)) return new JarFile(archiveFile);
    else return new ZipFile(archiveFile);
  }
  ...
  public static InputStream getFile(ZipFile archiveFile, String name)
    ...
    // validate input
    if (archiveFile == null)
      throw new IllegalArgumentException("Archive file to read may not be null.");

    if (name == null) throw new IllegalArgumentException("File entry name may not be null.");
    ...
    if (entry == null) {
      ...
      throw new IOException(
          "Archive File Entry " + name + " not found in " + archiveFile.getName());
    }
   ...
   * @throws IllegalStateException if archiveFile has been closed.
   */
  public static int getFileSize(ZipFile archiveFile, String name) throws IOException, ZipException {
    if (archiveFile == null)
      throw new IllegalArgumentException("Archive file to read may not be null.");
   ...
   * @throws IllegalArgumentException if archive is <code>null</code>. stream from it.
   */
  public static ZipOutputStream createArchive(File archive, String type, Manifest manifest)
      throws IOException {
    ...
    if (type.equalsIgnoreCase(JAR_FILE_TYPE)) {
      if (manifest != null) return new JarOutputStream(fout, manifest);
      else return new JarOutputStream(fout);
    } else return new ZipOutputStream(fout);
  }
  ...
  public static void addFilesToArchive(
    ...
    if (file == null)
      throw new IllegalArgumentException("The file or directory to be added may not be null.");
    ...
    if (file.isDirectory()) {
      File[] files;
      if (filter == null) files = file.listFiles();
      else files = file.listFiles(filter);

      if (files.length > 0) {
        for (int i = 0; i < files.length; i++) {
          addFilesToArchive(archiveOutputStream, files[i], rootPath, filter, out);
        }
      } else {
        if (pw != null) pw.println(filePath);
  ...
  public static void archiveFile(
    ...
    if (fileEntryPath == null)
      throw new IllegalArgumentException("The path for this file entry may not be null.");

    if (sourceFile == null) throw new IllegalArgumentException("The source file may not be null.");

    // Add entry to output stream.
    ZipEntry entry;
    if (archiveOutputStream instanceof JarOutputStream) entry = new JarEntry(fileEntryPath);
    else entry = new ZipEntry(fileEntryPath);
    ...
    try {
      ...
      if (sourceFile.isFile()) {
        ...
        int len;
        while ((len = in.read(buf)) > 0) archiveOutputStream.write(buf, 0, len);

        in.close();
      }
    } catch (ZipException e) {
      if (e.getMessage().contains("duplicate entry")) {
        log.warn("Skipping duplicate entry: {} in package archive.", fileEntryPath);
      } else {
        throw e;
      }
    }
   ...
   */
  public static void archiveFile(
      ZipOutputStream archiveOutputStream, String fileEntryPath, File sourceFile)
      throws IOException, ZipException {
  ...
  public static String extractFilesFromArchive(
    ...
    if (archiveFile == null)
      throw new IllegalArgumentException("Archive file to extract from can not be null.");
  ...
  public static byte[] getExtra(ZipFile archiveFile, String archiveEntryPath) {
    ...
    if ((archiveEntryPath == null) || (archiveEntryPath.trim().length() < 1))
      throw new IllegalArgumentException("archiveEntryPath may not be null or empty.");

    ZipEntry entry = archiveFile.getEntry(archiveEntryPath);
    if (entry != null) return entry.getExtra();
    return null;
  }
  ...
  public static void setExtra(ZipFile archiveFile, String archiveEntryPath, byte[] extra) {
    ...
    if (extra == null) throw new IllegalArgumentException("extra may not be null");
