...
public class PSRelationshipUtils {
   ...
   *
   * @param request the request to operate on, not <code>null</code>. The request uses the <code>
   *     IPSHtmlParameters.SYS_CONTENTID</code> and <code>IPSHtmlParameters.SYS_LANG</code> HTML
   *     parameters.
   * @return <code>null</code> if no translation exists, the dependent locator if there is a
   *     translation.
   ...
   * @throws PSUnknownNodeTypeException if the supplied document does not contain a valid
   *     relationship set.
   * @throws IllegalArgumentException if the sullpied request is <code>null</code>.
   */
  public static PSLocator getExistingTranslation(IPSRequestContext request)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          ...
          PSNotFoundException, IOException, PSUnknownNodeTypeException {
    ...
    if (ir != null) {
      ...
      if (elem != null) {
        PSRelationshipSet relationships = new PSRelationshipSet(elem, null, null);
   ...
   *
   * @param request the request to check, not <code>null</code>. The request uses the <code>
   *     IPSHtmlParameters.SYS_CONTENTID</code> and <code>IPSHtmlParameters.SYS_LANG</code> HTML
   *     parameters.
   * @return <code>true</code> if the requested language already exists for the provided content id,
  ...
  public static boolean doesTranslationExist(IPSRequestContext request)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          PSSystemValidationException, SQLException, PSAuthenticationFailedException,
          PSNotFoundException, IOException {
    if (request == null) throw new IllegalArgumentException("request cannot be null");
    ...
    if (ir != null) {
      ...
      if (doc.getDocumentElement() != null) {
        Element elem = doc.getDocumentElement();
        return elem.getFirstChild().getNodeName().equals(PSRelationship.XML_NODE_NAME);
      }
  ...
  /**
   * Filters out all relationships that have the supplied category from the provided relationship
   * set and returns the filtered set. The passed in relationship set is not changed.
   *
   * @param relationships the relationship set from which to filter out relationships that have the
   *     suppleid category, assumed not <code>null</code>, may be empty.
   * @param category the category to filter out, assumed not <code>null</code>.
   * @return a relationship set with all relationships filter out that have the supplied category.
   */
  public static PSRelationshipSet filterCategory(PSRelationshipSet relationships, String category) {
    ...
    for (int i = 0; i < relationships.size(); i++) {
      ...
      if (currentCategory == null || !currentCategory.equals(category)) filtered.add(relationship);
    }
  ...
  public static final String SYS_PSXRELATIONSHIPSUPPORT = "sys_psxRelationshipSupport";

  /** The name of the translations query resource. */
  public static final String GET_TRANSLATIONS = "getTranslations";

  /**
   * The name of the query resource to get the contenttype information for the requested contentid
   * and revision.
   */
  ...
  /** The name of the query resource to get the requested transition information. */
  public static final String GET_REQUESTEDTRANSITION = "getRequestedTransition";
