...
public class PSCms {
  ...
  public static boolean isRelatedItemPublishable(
      Element linkurl, IPSRequestContext request, String publishableTokens) throws PSCmsException {
    if (linkurl == null) throw new IllegalArgumentException("Param linkurl must not be null");

    if (request == null) throw new IllegalArgumentException("Param request must not be null");
    ...
    if (nodes.getLength() > 0) {
      ...
      StringTokenizer tokens = new StringTokenizer(url.getAttribute("current"), "?&");
      while (tokens.hasMoreTokens() && contentid == null) {
        String token = tokens.nextToken();
        if (token.startsWith(IPSHtmlParameters.SYS_CONTENTID))
          contentid = token.substring(token.indexOf(equalSign) + equalSign.length());
      }

      Object[] params = {publishableTokens, contentid};
  ...
  public static boolean canWriteToFolders(IPSRequestContext request) throws PSCmsException {
    ...
    if (folderId == null || StringUtils.isBlank(folderId.toString())) {
      log.error(
          "canWriteToFolders: Missing sys_folderid parameter on the request, unable to validate"
              + " user permissions for folder.");
      throw new IllegalArgumentException(
          "request context must contain valid " + IPSHtmlParameters.SYS_FOLDERID + " parameter");
    }
      ...
      for (PSFolderAcl acl : acls) {
        ...
        try {
          ...
          if (!folderPerms.hasWriteAccess()) {
            log.debug(
                "Folder does not have write access. Folder id: {} ACL: {}",
                acl.getContentId(),
                folderPerms);
            return false;
          }
          ...
          log.error(
              "An Authorization exception was thrown by PSFolderPermissions. {}",
  ...
  public static boolean isFolderSecurityOverridesWorkflowSecurity() {
    ...
    String prop =
        PSServer.getServerProps().getProperty("folderSecurityOverridesWorkflowSecurity", "false");
  ...
  public static boolean checkFolderPermission(int contentid, int folderPermAccess)
    ...
    if ("true"
        .equalsIgnoreCase(
            PSServer.getServerProps().getProperty("disableFolderItemSecurity", "false"))) {
      return true;
    }

    try {
      String userName =
          (PSUserContextExtractor.getUserContextInformation(
                  "User/Name", PSThreadRequestUtils.getPSRequest(), new PSLiteralSet(String.class)))
              .toString();

      if (userName.equals(IPSConstants.INTERNAL_USER_NAME)) return true;
    ...
    try {
      ...
      for (int i = 0; i < folderids.length; i++) {
        ...
        if (locs.isEmpty()) continue; // shouldn't happen
      ...
      if (acls.length < folderids.length) return true;
  ...
  public static boolean isPublishable(Object[] params, IPSRequestContext request)
    ...
    if (params != null) {
      if (params.length > 0 && params[0] != null && params[0].toString().trim().length() > 0)
        validTokens = params[0].toString();

      if (params.length > 1 && params[1] != null) contentid = params[1].toString();

      if (params.length > 2 && params[2] != null) revision = params[2].toString();
    }

    // get the content id from the request if not supplied
    if (contentid == null) contentid = request.getParameter(IPSHtmlParameters.SYS_CONTENTID);

    if (contentid == null || contentid.trim().length() == 0)
      throw new PSCmsException(0, "Missing required parameter " + IPSHtmlParameters.SYS_CONTENTID);
    ...
    try {
      ...
      IPSInternalRequest ir = request.getInternalRequest(resource, htmlParams, false);
      if (ir == null) throw new PSCmsException(0, "Required resource not found: " + resource);
      ...
      if (testDoc != null) {
        ...
        if (nodes != null && nodes.getLength() > 0) {
          ...
          while (tokens.hasMoreTokens()) {
            ...
            if (publishable.equalsIgnoreCase(token)) return true;
          }
        }
      }
   ...
   */
  public static String getNewRequestResource(IPSRequestContext request, PSLocator source)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          PSSystemValidationException, PSAuthenticationFailedException, PSNotFoundException {
   ...
   * @throws PSNotFoundException for any file not found.
   */
  public static PSContentType getContentType(IPSRequestContext request, PSLocator source)
      throws PSRequestValidationException, PSAuthorizationException, PSInternalRequestCallException,
          PSSystemValidationException, PSAuthenticationFailedException, PSNotFoundException {
    if (request == null) throw new IllegalArgumentException("request cannot be null");
    ...
    String resource =
        PSRelationshipUtils.SYS_PSXRELATIONSHIPSUPPORT
            + "/"
            + PSRelationshipUtils.GET_CONTENTTYPEINFO;

    Map<String, Object> params = new HashMap<>();
    params.put(IPSHtmlParameters.SYS_CONTENTID, Integer.toString(source.getId()));
    params.put(IPSHtmlParameters.SYS_REVISION, Integer.toString(source.getRevision()));

    IPSInternalRequest ir = request.getInternalRequest(resource, params, false);
    if (ir != null) {
      try {
        Document doc = ir.getResultDoc();
        return new PSContentType((Element) doc.getDocumentElement().getFirstChild());
  ...
  public static String getPublicationUrl(
      IPSRequestContext request, String contentid, String variantid, String context)
      throws PSCmsException {
    ...
    if (context == null || context.length() < 1) context = "0";
    ...
    if (siteid != null) params.put(IPSHtmlParameters.SYS_SITEID, siteid);
   ...
   */
  public static String getPublicationUrl(
      IPSRequestContext request, Map<String, Object> assemblyParamMap) throws PSCmsException {
    if (assemblyParamMap == null) {
      throw new IllegalArgumentException("assemblyParamMap must not be null");
    }
    ...
    if (command != null && command.equals(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY)) {
      params.put(IPSHtmlParameters.SYS_COMMAND, command);
      params.put(IPSHtmlParameters.SYS_ACTIVEITEMID, "");
    }

    IPSInternalRequest iReq =
        request.getInternalRequest("sys_casSupport/PublicationUrl", params, false);
    Document doc;
    try {
    ...
    if (doc != null) return doc.getDocumentElement().getAttribute("current");
  ...
  public static int getDefaultWorkflowId(PSRequest request, PSContentEditor ce, String communityId)
    ...
    String theWorkflowIdStr =
        request.getParameter(IPSHtmlParameters.SYS_WORKFLOWID, defaultWorkflowId);
    int theWorkflowId;
    ...
    if (null != communityId && communitiesEnabled) {
      ...
      boolean haveWorkflowInfo = info != null && info.getWorkflowIds() != null;
      boolean isInclusionary =
          !haveWorkflowInfo || info.getType().equals(PSWorkflowInfo.TYPE_INCLUSIONARY);
      ...
      while (it.hasNext()) {
        ...
        if (!haveWorkflowInfo) {
          ...
          if (info.getWorkflowIds().contains(workflowId)) resultWorkflows.add(workflowId);
        } else {
          if (!info.getWorkflowIds().contains(workflowId)) resultWorkflows.add(workflowId);
        }
      }
      ...
      if (resultWorkflows.contains(theWorkflowId)) return theWorkflowId;
      ...
      if (resultWorkflows.size() == 1) {
        theWorkflowId = resultWorkflows.iterator().next();
      } else {
   ...
   * @param workflowId
   */
  public static boolean isAllowedWorkflow(PSContentEditor ce, String communityId, int workflowId) {

    if (isCommunitiesEnabled()) {
      List<String> workflowIds = findCommunityWorkflows(communityId);
      // FB: GC_UNRELATED_TYPES NC 1-17-16
      if (!workflowIds.contains(Integer.toString(workflowId))) return false;
  ...
  private static boolean isCommunitiesEnabled() {
    return PSServer.getServerProps()
        .getProperty("communities_enabled")
        .trim()
  ...
  private static List<String> findCommunityWorkflows(String communityId) {
    try {
      ...
      throw new RuntimeException(
          "Failed to determine workflows for "
              + "community "
              + communityId
              + ": "
