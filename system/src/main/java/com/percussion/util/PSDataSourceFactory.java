...
public class PSDataSourceFactory extends BasicDataSourceFactory {
  ...
  public static DataSource createDataSource(Properties properties) throws Exception {
    ...
    if (url == null) {
      throw new IllegalArgumentException("properties must contain " + URL_PROP_NAME);
    }

    String username = properties.getProperty(USER_PROP_NAME);
    if (username == null) {
      throw new IllegalArgumentException("properties must contain " + USER_PROP_NAME);
    }

    String password = properties.getProperty(PWD_PROP_NAME);
    if (password == null) {
      throw new IllegalArgumentException("properties must contain " + PWD_PROP_NAME);
    }

    String driverClass = properties.getProperty(DRIVER_CLASS_PROP_NAME);
    if (driverClass == null) {
      throw new IllegalArgumentException("properties must contain " + DRIVER_CLASS_PROP_NAME);
    }

    String driverLocation = properties.getProperty(DRIVER_LOC_PROP_NAME);
    if (driverLocation == null) {
      throw new IllegalArgumentException("properties must contain " + DRIVER_LOC_PROP_NAME);
    }
