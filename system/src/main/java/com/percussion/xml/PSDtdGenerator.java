...
public class PSDtdGenerator {
        ...
        while (tok.hasMoreTokens()) {
          Document doc =
              PSXmlDocumentBuilder.createXmlDocument(
                  new FileInputStream(new File(tok.nextToken())), false);
          documents.add(doc);
        }
  ...
  public static PSDtdTree generate(Document[] exemplars) {
    ...
    try {
      ...
      XMLInputSource src =
          new XMLInputSource(
              null, null, null, new ByteArrayInputStream(ba), /*PSCharSets.rxStdEnc()*/ null);
    ...
    for (int i = 0; i < exemplars.length; i++) {
      ...
      if (!root.getNodeName().equals(rootName))
        throw new IllegalArgumentException(
            "Root element " + root.getNodeName() + " should be " + rootName);
          ...
          if (elDef.isSequenced()) {
            ...
            if (!child.getName().equals("#PCDATA")) {
              if (child.isRepeatable()) {
                if (child.isOptional()) writer.write('*');
                else writer.write('+');
              } else {
                if (child.isOptional()) writer.write('?');
            ...
            if (j < sizeMinusOne) writer.write(", ");
          } else {
            if (j < sizeMinusOne) writer.write(" | ");
          }
        }
  ...
  protected void handleNode(
      Node node,
      int seqInParent,
      boolean firstSiblingWithThisName,
      int groupNumber,
      int numberInGroup) {
    ...
    if (seqInParent != -1) // not root?
    {
      ...
      } else // we have seen this element under this parent before
      {
        ...
        if (elSeqDef.getPosition() != seqInParent) {
          if (numberInGroup < 1) parentElDef.setSequenced(false);
        }
      }
