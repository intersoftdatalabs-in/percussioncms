...
public class PSDtdNode implements Serializable {
  ...
  public void setOccurrences(int occurrence_type) {
    if ((occurrence_type < MIN_OCCURRENCE) || (occurrence_type > MAX_OCCURRENCE))
      throw new IllegalArgumentException(
          "occurrence_type must be between " + MIN_OCCURRENCE + " and " + MAX_OCCURRENCE);
    else m_occurrenceType = occurrence_type;
  }
  ...
  protected int getMaxMergedOccurrenceSetting(int occurrenceType) {
    if (occurrenceType == OCCURS_UNKNOWN || occurrenceType == OCCURS_ANY) return occurrenceType;

    int myOccurrence = m_occurrenceType;

    if (myOccurrence == OCCURS_UNKNOWN || myOccurrence == OCCURS_ANY) return myOccurrence;

    if (myOccurrence != occurrenceType) {

      if ((this instanceof PSDtdNodeList)
          && (((PSDtdNodeList) this).getType() == PSDtdNodeList.OPTIONLIST)) {
        if (occurrenceType == OCCURS_ATLEASTONCE) return OCCURS_ANY;
        else if (myOccurrence == OCCURS_ATLEASTONCE) return OCCURS_ANY;
      }

      /* Merge myOccurrence (modified for optionlist) and occurrenceType */
      if (occurrenceType == OCCURS_ATLEASTONCE) {
        if (myOccurrence == OCCURS_OPTIONAL) return OCCURS_ANY;
        else occurrenceType = OCCURS_ATLEASTONCE;
      } else if (myOccurrence == OCCURS_ATLEASTONCE) {
        if (occurrenceType == OCCURS_OPTIONAL) return OCCURS_ANY;
        else occurrenceType = OCCURS_ATLEASTONCE;
      } else {
    ...
    if ((parent instanceof PSDtdElementEntry) || (parent == null)) return occurrenceType;
  ...
  public static final int MIN_OCCURRENCE = 0;
  public static final int OCCURS_ONCE = 0; /* default - required */
  public static final int OCCURS_OPTIONAL = 1; /* 0 or 1 */
  public static final int OCCURS_ANY = 3; /* 0 or more */
  public static final int OCCURS_ATLEASTONCE = 2; /* 1 or more */
  public static final int MAX_OCCURRENCE = 3;
  public static final int OCCURS_UNKNOWN = 4;
}
