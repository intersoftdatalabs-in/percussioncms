...
public class PSXmlElementDecl extends XMLElementDecl {
  ...
  public PSXmlElementDecl(XMLElementDecl xmlElemDecl) {
    super();
    if (xmlElemDecl == null) throw new IllegalArgumentException("xmlElemDecl may not be null");
    setValues(
        xmlElemDecl.name,
        xmlElemDecl.scope,
        xmlElemDecl.type,
        xmlElemDecl.contentModelValidator,
        xmlElemDecl.simpleType);
  }

  /**
   * Returns the name of the encapsulated element
   *
   * @return the name of the encapsulated element, never <code>null</code> or empty
   */
