...
public class PSNodePrinter {
  ...
  public void printNode(Node node) throws IOException {
    ...
    switch (node.getNodeType()) {
      case Node.ATTRIBUTE_NODE:
        ...
        ent = PSXmlTreeWalker.convertToXmlEntities(((Attr) node).getValue());
        m_out.write(ent);
        m_out.write("\"");
        break;
      ...
      case Node.CDATA_SECTION_NODE:
        m_out.write("<![CDATA[");
        ent = PSXmlTreeWalker.convertToXmlEntities(((CDATASection) node).getData());
        m_out.write(ent);
        m_out.write("]]>");
        break;
      ...
      case Node.DOCUMENT_NODE:
        ...
        if (kid != null) {
          if (kid.getNodeType() == Node.ELEMENT_NODE) printElement((Element) kid);
          else printNode(kid);
      ...
      case Node.PROCESSING_INSTRUCTION_NODE:
        ...
        if (tmp.endsWith("?")) m_out.write(">");
        else m_out.write("?>");
        break;

      case Node.TEXT_NODE:
        String data = ((Text) node).getData();
        if (data != null) ent = PSXmlTreeWalker.convertToXmlEntities(data);
        m_out.write(ent);
        break;
    }
  }
   ...
   * @throws IOException when it fails to print a node
   */
  void printElement(Element eNode) throws IOException {
    m_out.write("<");
    ...
    if (eNode.hasChildNodes()) {
      ...
      if (tagName.equalsIgnoreCase("script")) {
        ...
        if (name.toLowerCase().startsWith("xsl:")) printNode(fChild);
  ...
  private String cleanupJavaScript(String script) {
    ...
    if (script.startsWith(COMMENTOPEN)) script = script.substring(COMMENTOPEN.length()).trim();
    // Chop the XML comment close if present
    if (script.endsWith(COMMENTCLOSE)) {
      script = script.substring(0, script.length() - COMMENTCLOSE.length()).trim();
    }

    // Ektron puts "//" at the end of javacript chop that
    if (script.endsWith("//")) script = script.substring(0, script.length() - 2).trim();
    ...
    if (!script.startsWith(JSCRIPTCOMMENTOPEN + CDATAOPEN + JSCRIPTCOMMENTCLOSE)) {
      ...
      script = JSCRIPTCOMMENTOPEN + CDATAOPEN + JSCRIPTCOMMENTCLOSE + nline + script;
    }
    ...
    if (!script.endsWith(JSCRIPTCOMMENTOPEN + CDATACLOSE + JSCRIPTCOMMENTCLOSE)) {
      ...
      script = script + nline + JSCRIPTCOMMENTOPEN + CDATACLOSE + JSCRIPTCOMMENTCLOSE;
    }
