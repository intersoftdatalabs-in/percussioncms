...
public class PSDtd extends DTDGrammar {
  ...
  public static void main(String argv[]) {
    ...
    try {
      if (cmd.equalsIgnoreCase("-d")) dtdParser.parseDtd(filePath);
      else if (cmd.equalsIgnoreCase("-x")) dtdParser.parseXmlForDtd(filePath, true);
    } catch (Exception e) {
  ...
  public Enumeration getAttributeDeclarations(String elementName) {
    ...
    if (elementDeclIndex != -1) {
      ...
      while (attributeDeclIndex != -1) {
        ...
        if (getAttributeDecl(attributeDeclIndex, attributeDecl)) attrList.addElement(attributeDecl);
  ...
  public String getName() {
    if (m_dtdRoot != null) return m_dtdRoot;
    ...
    do {
      ...
      if (gramDesc != null) {
        if (gramDesc instanceof XMLDTDDescription) {
          ...
          if (!((root == null) || (root.trim().length() == 0))) break;
        }
      }
      ...
      if ((systemId != null) && (systemId.trim().length() > 0)) {
        ...
        if (index != -1) dtdFileName = dtdFileName.substring(0, index);
      }
        ...
        for (int i = 0; i < possibleRoots.size(); i++) {
          ...
          if (contentModel != null) {
            ...
            while (st.hasMoreTokens()) {
              ...
              if (roots.contains(token)) roots.remove(token);
            }
          }
        }
      }
      ...
      if (index != -1) {
        ...
        if (elemDecl != null) root = elemDecl.getName();
      }
    } while (false);
  ...
  public Enumeration getElementDeclarations() {
    ...
    while (index != -1) {
      PSXmlElementDecl elemDecl = getElementDeclaration(index);
      if (elemDecl != null) elementList.addElement(elemDecl);
  ...
  public String getContentModelAsString(String elementName) {
    if ((elementName == null) || (elementName.trim().length() == 0))
      throw new IllegalArgumentException("elementName may not be null or empty");
    int elementDeclIndex = getElementDeclIndex(elementName);
    if (elementDeclIndex != -1) return getContentSpecAsString(elementDeclIndex);
    return null;
  }
  ...
  protected PSXmlElementDecl getElementDeclaration(int elementDeclIndex) {
    ...
    if (getElementDecl(elementDeclIndex, elemDecl)) return elemDecl;
    return null;
  }
  ...
  protected CMNode buildSyntaxTree(int contentSpecIndex, XMLContentSpec contentSpec) {
    ...
    CMNode nodeRet = null;
    if (!(getContentSpec(contentSpecIndex, contentSpec))) return null;
    // do a binary AND with 0x0f to obtain the type of node
    if ((contentSpec.type & 0x0f) == XMLContentSpec.CONTENTSPECNODE_ANY) {
      nodeRet = new CMAny(contentSpec.type, (String) contentSpec.otherValue, m_leafCount++);
  ...
  public void elementDecl(String name, String contentModel, Augmentations augs)
      throws XNIException {
    if (m_bAddPossibleRootElement) m_possibleRootElements.add(name);
