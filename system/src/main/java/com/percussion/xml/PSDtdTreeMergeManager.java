...
public class PSDtdTreeMergeManager {
  ...
  public PSDtdTreeMergeManager(PSDtdTree masterTree) {
    if (null == masterTree) throw new IllegalArgumentException("masterTree cannot be null!");
  ...
  public PSDtdTree updateTreeForUserMod(PSDtdTree master, PSDtdTree slave) {
    ...
    try {
      ...
      /* Ignore, shouldn't happen unless tree stops cloning */
    }
    ...
    // valid root (very bad)
    if (null == paths) return workingTree;
  ...
  private int mergeAttributeOccurrence(int master, int slave) {
    ...
    else if (PSDtdAttribute.IMPLIED == master) result = slave;
  ...
  private PSDtdElement mergeElement(String key) {
    ...
    if (null == sElement) return null;

    PSDtdNode mContent = null;

    if (null != mElement) mContent = mElement.getContent();
    ...
    if (null == mContent) {
      if (sContent instanceof PSDtdDataElement) {
        ...
        if (null == tempElement) tempElement = mergeElement(elementName);
        ...
        PSDtdNodeList tempList =
            new PSDtdNodeList(nodeList.getType(), nodeList.getOccurrenceType());
      ...
      if (sContent instanceof PSDtdDataElement) {
        ...
        PSDtdNodeList nodeList =
            new PSDtdNodeList(PSDtdNodeList.OPTIONLIST, PSDtdNodeList.OCCURS_ANY);
        entry.setElement(tempElement);
      ...
      if (sContent instanceof PSDtdDataElement) {
        ...
        if (elementName.equals(mEntry.getElement().getName())) {
          ...
          if (!(null == mContent || null == sNode)) {
            int occur =
                mergeElementOccurrence(mContent.getOccurrenceType(), sContent.getOccurrenceType());
        ...
        // NodeList
          ...
          PSDtdNodeList tempList =
              new PSDtdNodeList(PSDtdNodeList.SEQUENCELIST, PSDtdNode.OCCURS_ONCE);
          tempList.add(mContent);
          tempList.add(sEntry);
        ...
        PSDtdNodeList sNodeList = (PSDtdNodeList) sContent;
        PSDtdNodeList tempList =
            new PSDtdNodeList(sNodeList.getType(), sNodeList.getOccurrenceType());
          ...
          if (innerNode instanceof PSDtdDataElement) {
            ...
            if (elementName.equals(mEntry.getElement().getName())) {
              ...
              if (!(null == mNode || null == sNode)) {
                int occur =
                    mergeElementOccurrence(mEntry.getOccurrenceType(), sEntry.getOccurrenceType());
                sEntry.setOccurrences(occur);
              }
              m_bNodeInList = true;
            } else {
              ...
              // recurse down slaveTree until a copy-able element CM is reached
              if (null == element) element = mergeElement(elementName);
            }
        ...
        if (!m_bNodeInList) {
          PSDtdNodeList newList =
              new PSDtdNodeList(sNodeList.getType(), sNodeList.getOccurrenceType());
          newList.add(mContent);
          for (int i = 0; i < tempList.getNumberOfNodes(); i++) newList.add(tempList.getNode(i));

          tempList = newList;
        }

        // mContent = fixMixedModelNodeList( tempList );
        if (bNeedFix) mContent = fixMixedModelNodeList(tempList);
        else mContent = tempList;
      }
      ...
      if (sContent instanceof PSDtdDataElement) {
        ...
        PSDtdNodeList mNodeList = (PSDtdNodeList) mContent;
        PSDtdNodeList tempList =
            new PSDtdNodeList(mNodeList.getType(), mNodeList.getOccurrenceType());
        ...
        if (!m_bNodeInList) {
          ...
          if (null == element) element = mergeElement(entryName);
    ...
    if (null == mElement) mElement = new PSDtdElement(key);
   ...
   */
  private boolean mergeInnerNodes(PSDtdNodeList innerList, PSDtdNodeList ownerList) {
    ...
    for (int i = 0; i < innerList.getNumberOfNodes(); i++) {
      ...
      if (innerNode instanceof PSDtdElementEntry) {
        ...
        // recurse down slaveTree until a copy-able element CM is reached
        if (null == tempElement) tempElement = mergeElement(innerNodeName);
  ...
  private PSDtdNodeList fixMixedModelNodeList(PSDtdNodeList list) {
    if (null == list) throw new IllegalArgumentException("list parameter is null!");

    PSDtdNodeList newList = new PSDtdNodeList(PSDtdNodeList.OPTIONLIST, PSDtdNodeList.OCCURS_ANY);
