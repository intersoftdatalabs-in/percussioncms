...
public class PSUrlUtils {
  ...
  public static String createUrl(String base, Iterator queryParams, String anchor) {
    if (null == base || base.trim().length() == 0)
      throw new IllegalArgumentException("Base of URL can't be null or empty.");
    base = base.trim();
    ...
    if (parsedFile != null) base = parsedFile;

    int anchorPos = base.lastIndexOf(ANCHOR_SEP);
    String suppliedAnchor = null;
    if (anchor != null) suppliedAnchor = PSURLEncoder.encodeQuery(anchor);
    else if (anchorPos >= 0) suppliedAnchor = base.substring(anchorPos + 1, base.length());
    if (suppliedAnchor != null && suppliedAnchor.trim().length() == 0) suppliedAnchor = null;

    int queryPos = base.indexOf(QUERY_SEP);
    if (base.length() == 0 || queryPos == 0 || anchorPos == 0 || anchorPos == queryPos + 1)
      throw new IllegalArgumentException("Base must include root part.");

    String suppliedParams = null;
    if (queryPos >= 0) {
      if (anchorPos >= 0) suppliedParams = base.substring(queryPos + 1, anchorPos);
      else if (queryPos < base.length() - 1) suppliedParams = base.substring(queryPos + 1);
    }

    String baseUrl = base;
    if (queryPos >= 0) baseUrl = baseUrl.substring(0, queryPos);
    else if (anchorPos >= 0) baseUrl = baseUrl.substring(0, anchorPos);
    ...
    if (suppliedParams != null || (queryParams != null && queryParams.hasNext()))
      finalUrl.append(QUERY_SEP);

    if (suppliedParams != null) {
      int strLength = suppliedParams.length();
      if (suppliedParams.endsWith(PARAM_SEP)) --strLength;
    ...
    if (null != queryParams) {
      String delim = "";
      if (queryParams.hasNext() && suppliedParams != null) delim = PARAM_SEP;
      ...
      while (queryParams.hasNext()) {
        ...
        if (null != param) {
          ...
          if (null != value) finalUrl.append(PSURLEncoder.encodePath(value));
          delim = PARAM_SEP;
        }
      }
    }
  ...
  public static URL createUrl(
    ...
    return createUrl(host, port, partialUrl, queryParams, anchor, request, true);
  }
  ...
  public static URL createUrl(
    ...
    if (partialUrl != null && partialUrl.trim().length() > 0) {
      ...
      if (colonPos > 0) {
        ...
        int firstBadChar = ((slashPos != -1) && (slashPos < queryPos)) ? slashPos : queryPos;
        if (colonPos < firstBadChar || firstBadChar == -1) {
          try {
            parsedUrl = new URL(partialUrl);
            if (!DEFAULT_PROTOCOL.equalsIgnoreCase(parsedUrl.getProtocol())
                && !SECURE_PROTOCOL.equalsIgnoreCase(parsedUrl.getProtocol())) return parsedUrl;
          } catch (MalformedURLException e) {
          } // ignore, we don't have a fully qualified URL
        }
      }
    ...
    if (origProtocol.equalsIgnoreCase("https")) wasHttps = true;
    ...
    if (pos == 0) {
      ...
      StringTokenizer tok = new StringTokenizer(request.getRequestRoot(), PATH_SEP);
      ...
      for (int i = 0; i < numEl; i++) {
        if (partialUrl.substring(pos, relLen).equals(RELATIVE_PREFIX)) {
          pos += relLen;
        }
        if ((pos + relLen) > partialUrl.length()) break;
      }

      // in case we were just passed "../"
      if (pos >= partialUrl.length()) partialUrl = "";
      else partialUrl = partialUrl.substring(pos);
    }
    ...
    if (parsedUrl != null) {
      ...
      if (p != -1) parsedPort = String.valueOf(p);
      parsedFile = parsedUrl.getFile();
      /*
      ...
      if (index != -1) parsedFile = parsedFile.substring(0, index);
    } else {
      if (partialUrl.length() == 0) {
        ...
        if (!hasAppRoot) {
          ...
          parsedFile = serverConfig.getRequestRoot() + PATH_SEP + partialUrl;
        }
      }
    }
    ...
    if (parsedProtocol != null && parsedProtocol.equalsIgnoreCase(SECURE_PROTOCOL)) {
      if (allowHttps) {
         ...
         */
        isSecure = false;
        if (!wasHttps) parsedPort = String.valueOf(request.getOriginalPort());
        else parsedPort = String.valueOf(PSServer.getListenerPort());
      }
    }

    if (isSecure) finalUrl.append(SECURE_PROTOCOL);
    else finalUrl.append(DEFAULT_PROTOCOL);

    finalUrl.append("://");

    // add host
    if (passedHost || parsedHost == null) finalUrl.append(host);
    else finalUrl.append(parsedHost);
    ...
    if (!(parsedFile.length() > 0 && parsedFile.charAt(0) == PATH_SEP.charAt(0))) {
  ...
  public static void main(String[] args) {
    ...
    map.put("p3", null);
    map.put(null, "v1");
    System.out.println("...createUrl= " + createUrl(base, map.entrySet().iterator(), anchor));
    System.out.println("\n\n");
    ...
    base = "file:/test/base.htm";
    System.out.println("...createUrl= " + createUrl(base, PSIteratorUtils.emptyIterator(), null));
    System.out.println("\n\n");
  ...
  public static String getUrlParameterValue(String url, String name) {
    ...
    if (pos != -1) {
      ...
      if (startPos != -1) {
        ...
        if (endPos == -1) return url.substring(startPos);
        else return url.substring(startPos, endPos);
      }
    }
  ...
  public static String replaceUrlParameterValue(String url, String name, String value) {
    ...
    if (pos != -1) {
      ...
      if (startPos != -1) {
        startPos += PARAM_VALUE_SEP.length();
        int endPos = url.indexOf(PARAM_SEP, startPos);
        if (endPos == -1) return url.substring(0, startPos) + value;
        else return url.substring(0, startPos) + value + url.substring(endPos);
      }
    }
