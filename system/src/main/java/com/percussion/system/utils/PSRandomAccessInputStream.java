...
/**
 * This class provides a wrapper on top of a RandomAccessFile object to treat it as an input stream.
 *
...
public class PSRandomAccessInputStream extends InputStream {
   ...
   *
   * <p>If <code>b</code> is <code>null</code>, a <code>NullPointerException</code> is thrown. If
   * the length of <code>b</code> is zero, then no bytes are read and <code>0</code> is returned;
   ...
   *
   * <p>If the first byte cannot be read for any reason other than end of file, then an <code>
  ...
  public long skip(long n) throws IOException {
    ...
    if (skip > 0) m_file.seek(m_file.getFilePointer() + skip);
  ...
  /**
   * This method is not implemented. The underlying RandomAccessFile object must be closed directly
   * to close the file.
   *
   * @exception IOException if an I/O error occurs.
   */
   ...
   *
   * @return <code>true</code> if this true type supports the mark and reset method; <code>false
   *     </code> otherwise.
   * @see java.io.InputStream#mark(int)
   * @see java.io.InputStream#reset()
   */
