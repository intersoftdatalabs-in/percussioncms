...
public class RemoteConsole {
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 4) {
      ...
      // get port
      String portID = null;
      if (args.length >= 2) portID = args[1];
      else {
        portID =
            remoteCon.getPortID(
                "rxconfig" + File.separator + "Server" + File.separator + "server.properties",
                "bindPort");
      }
      ...
      // get userid
      String userID = null;
      if (args.length >= 3) userID = args[2];
      else userID = System.getProperty("user.name");
      ...
      do {
        System.out.println("\nExecuting console command with the " + "following parameters");
    ...
    // if all 4 parameters are provided
      ...
      if (args.length > logArg) {
        logOutput = args[logArg].trim().toUpperCase().startsWith("Y") ? true : false;
      }
    }
  ...
  private void getNewParams(Properties connProps) {
    ...
    if (newCmd.length() > 0) connProps.put(CMD_PROP, newCmd);
  }
  ...
  private static int parseCommand(String[] args, int start, StringBuilder cmd) {
    ...
    for (int i = start; i < args.length; i++) {
      ...
      if (i == start) {
        ...
        if (arg.startsWith("\"") && arg.length() > 1) cmd.append(arg.substring(1));
  ...
  private static void showUsage() {
    ...
    System.out.println(
        "java com.percussion.util.RemoteConsole <server> "
            + "[<port> <userid> <password> <command> <log?>] ");
    System.out.println("For example, to shutdown the server in interactive mode:");
    ...
    System.out.println("java com.percussion.util.RemoteConsole localhost 9992 admin1 demo");
    System.out.println();
    System.out.println("To run in non-interactive mode, with logging, specifying a command:");
    System.out.println(
        "java com.percussion.util.RemoteConsole localhost "
