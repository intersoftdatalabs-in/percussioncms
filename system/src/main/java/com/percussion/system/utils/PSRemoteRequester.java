...
public class PSRemoteRequester implements IPSRemoteRequesterEx {
  ...
  protected void init(Properties connInfo) {
    ...
    if (ssl.length() > 0 && ssl.equalsIgnoreCase("true")) m_useSSL = true;

    if (m_useSSL) m_sslPort = port;
    else m_Port = port;

    String uid = getString(connInfo, "loginId", false);
    String pw = getString(connInfo, "loginPw", false);
    if (uid.length() > 0) setCredentials(uid, pw);

    String serverRoot = getString(connInfo, "serverRoot", false);
    if (serverRoot.length() > 0) m_serverRoot = serverRoot;
  ...
  public PSRemoteRequester(String server, int port, int sslPort) {
    ...
    if (sslPort > 0) m_sslPort = sslPort;
  }
  ...
  private String getString(Properties props, String propName, boolean required) {
    ...
    if (required && value.trim().length() == 0) {
      throw new IllegalArgumentException("A required property was missing or empty: " + propName);
    }
    return value;
  }
    ...
    try {
      ...
      if (required) {
        throw new IllegalArgumentException(
            "An invalid number was supplied for the property: " + propName + ".");
      } else return -1;
    }
  }
  ...
  public Document getDocument(String resource, Map params) throws IOException, SAXException {
    ...
    final byte[] data = Codecs.mpFormDataEncode(opts, PSCharSets.rxJavaEnc(), files, hdrs);
  ...
  public Document sendUpdate(String resource, Document doc) throws IOException, SAXException {
    ...
    IPSPoster p =
        new IPSPoster() {
          public HTTPResponse sendRequest(HTTPConnection con) throws IOException, ModuleException {
            NVPair[] headers = {
              new NVPair("Content-Type", "text/xml; charset=" + PSCharSets.rxStdEnc())
            };
    ...
    try {
      ...
      if (getProcessStatusCode() && nStatus != PSHttpRequest.HTTP_STATUS_OK) {
        ...
        try {
          ...
          if (null == msg) msg = "No response text";
  ...
  private HTTPConnection createConnection(String protocol, String server, int port)
    ...
    if (!getFollowRedirects()) con.removeModule(RedirectionModule.class);
  ...
  private HTTPConnection getConnection() {
    try {
      if ((m_useSSL && null == m_sslConnection) || (!m_useSSL && null == m_connection)) {
        HTTPConnection con;
        if (m_useSSL) {
        ...
        if (getJSessionCookie() != null) CookieModule.addCookie(getJSessionCookie());
      }
    } catch (ProtocolNotSuppException pnse) {
      // should never happen because we use 2 stds
      throw new RuntimeException("Unexpected exception: " + pnse.getLocalizedMessage());
    }
  ...
  private void closeConnection() {
    ...
    if (getJSessionCookie() != null) CookieModule.removeCookie(getJSessionCookie());
  }
  ...
  public static URL createUrl(String url, String host, int port, int sslPort)
    ...
    if (url == null || url.trim().length() == 0 || host == null || host.trim().length() == 0)
      throw new IllegalArgumentException("url and host cannot be null or empty");
    ...
    if (sslPort > 0) {
      resultUrl = new URL(PSHttpRequest.HTTPS_PROTOCOL, hostSpec, sslPort, fileSpec);
    } else if (port > 0 && port != 80) {
      resultUrl = new URL(PSHttpRequest.DEFAULT_PROTOCOL, hostSpec, port, fileSpec);
    } else {
      resultUrl = new URL(PSHttpRequest.DEFAULT_PROTOCOL, hostSpec, fileSpec);
    }
  ...
  protected byte[] postRequest(String resource, Map params, byte[] pdata, NVPair[] headers)
    ...
    final NVPair[] hdrs = (headers == null || headers.length == 0) ? new NVPair[1] : headers;
    NVPair[] files = new NVPair[1];
    final byte[] data;
    if (params != null) {
      NVPair[] opts = getParams(params);
      data = Codecs.mpFormDataEncode(opts, PSCharSets.rxJavaEnc(), files, hdrs);
    } else {
      data = pdata;
    }
  ...
  public PSLocator updateBinary(
    ...
    ByteArrayInputStream xmlresp = new ByteArrayInputStream(sendByteReturnRequest(p));
  ...
  private Cookie getJSessionCookie() {
    ...
    else return new Cookie("JSESSIONID", m_jsessionId, m_Server, "/" + m_serverRoot, null, false);
  }
  ...
  public static void main(String[] args) {
    try {
      ...
      if (null != doc) PSXmlDocumentBuilder.write(doc, System.out);
      else log.info("No result doc");
