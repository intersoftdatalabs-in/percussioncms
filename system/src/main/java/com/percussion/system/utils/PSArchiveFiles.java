...
public class PSArchiveFiles {
  ...
  public static ZipFile openArchive(String archiveName, String type) throws IOException {
    ...
    if (archiveType.equalsIgnoreCase(JAR_FILE_TYPE)) return new JarFile(archiveFile);
  ...
  public static ZipOutputStream createArchive(File archive, String type, Manifest manifest)
    ...
    if (archiveType.equalsIgnoreCase(JAR_FILE_TYPE)) {
      return manifest != null ? new JarOutputStream(fout, manifest) : new JarOutputStream(fout);
    } else {
  ...
  public static void addFilesToArchive(
    ...
    Objects.requireNonNull(
        archiveOutputStream, "The archive stream to which files should be added may not be null");
    Objects.requireNonNull(file, "The file or directory to be added may not be null");
    ...
    if (file.isDirectory()) {
      ...
      if (files != null && files.length > 0) {
        for (var subFile : files) {
          addFilesToArchive(archiveOutputStream, subFile, rootPath, filter, out);
        }
      } else {
  ...
  public static void archiveFile(
      ...
      throws IOException, ZipException {
    Objects.requireNonNull(
        archiveOutputStream, "The archive stream to which file should be added may not be null");
    Objects.requireNonNull(fileEntryPath, "The path for this file entry may not be null");
    Objects.requireNonNull(sourceFile, "The source file may not be null");

    // Add entry to output stream.
    ZipEntry entry =
        archiveOutputStream instanceof JarOutputStream
            ? new JarEntry(fileEntryPath)
            : new ZipEntry(fileEntryPath);
