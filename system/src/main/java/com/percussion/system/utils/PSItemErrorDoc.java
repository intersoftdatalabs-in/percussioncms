...
public class PSItemErrorDoc {
  ...
  public static void addError(
    ...
    if (submitName == null || submitName.trim().length() == 0)
      throw new IllegalArgumentException("submitName cannot be null or empty");
  ...
  public static void addError(
    ...
    if (submitNames.length != displayNames.length)
      throw new IllegalArgumentException("submitNames and displayNames must have the same length");
    ...
    for (int i = 0; i < submitNames.length; i++) {
      ...
      errorField.setAttribute(SUBMIT_NAME_ATTR, submitNames[i].toString());
      errorField.setAttribute(DISPLAY_NAME_ATTR, displayNames[i].toString());
