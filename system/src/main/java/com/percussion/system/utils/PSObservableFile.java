...
public class PSObservableFile extends Observable {
  ...
  public PSObservableFile(File file, int observeInterval) {
    ...
    if (observeInterval < 1)
      throw new IllegalArgumentException("observeInterval must be greater than 0 milli seconds");
    m_file = file;
  ...
  /**
   * Record the last modifed date to compare with actual modified date of the file to be observed
   * during polling.
   */
  private long m_lastModified;

  /**
   * The file to be observed. Initialized in the ctor {@link #PSObservableFile(String)}and never
