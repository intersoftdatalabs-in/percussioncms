...
public class PSStringFilter {
  ...
  public PSStringFilter(
    ...
    if ((filterPatterns != null) && (filterPatterns.length > 0)) {
      PSPatternMatcher[] matchers = new PSPatternMatcher[filterPatterns.length];
      for (int i = 0; i < filterPatterns.length; i++) {
        matchers[i] =
            new PSPatternMatcher(matchOne, matchZeroOrMore, filterPatterns[i], caseSensitive);
      }
      m_matchers = matchers;
    }
  }
  ...
  public boolean accept(String text) {
    ...
    if (m_matchers != null) {
      for (int i = 0; i < m_matchers.length; i++)
        if (m_matchers[i].doesMatchPattern(text)) return true;
  ...
  public static String[] parseDelimitedList(String delimitedList, char delim, char delimEscape) {
    ...
    while (pos >= 0) {
      ...
      if (pos == 0) {
        ...
        delimitedList = delimitedList.substring(0, pos - 1) + delimitedList.substring(pos);
        offset = pos;
      }
