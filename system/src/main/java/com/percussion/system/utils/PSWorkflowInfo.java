...
public class PSWorkflowInfo {
  ...
  public boolean isValidState(int workflowId, int stateId, String validTokens) {
    ...
    return isValidState(String.valueOf(workflowId), String.valueOf(stateId), validTokens);
  }
  ...
  public String getTransitionId(
    ...
    List transitions = getAutoTransitionId(workflowId, fromStateId, toValidTokens, useDefaults);
  ...
  private boolean checkValidFlag(char publishable, String validTokens) {
    ...
    while (tokens.hasMoreTokens()) {
      ...
      if (publishable == token.charAt(0)) return true;
    }
    return false;
  }
  ...
  private void fromXML(Element el) {
    ...
    if (snodes != null && snodes.getLength() > 0) {
      for (int i = 0; i < snodes.getLength(); i++) {
        ...
        String cv = element.getAttribute(ATTR_VALID_FLAG);
        if ((cv == null)
            || (cv.trim().length() == 0)
            || ((id == null) || id.trim().length() == 0)
            || ((sortOrder == null) || sortOrder.trim().length() == 0)
            || ((workflow == null) || workflow.trim().length() == 0))
          m_logger.error("Cannot create PSTStateInfo recieved empty or null data");
        else {
          StateInfo s = new StateInfo(cv.charAt(0), id, sortOrder, workflow);
          allStates.add(s);
        }
      }
    }
    ...
    if (tnodes != null && tnodes.getLength() > 0) {
      for (int i = 0; i < tnodes.getLength(); i++) {
        ...
        if (flag == null || flag.trim().length() == 0) flag = "n";
        if (((to == null) || to.trim().length() == 0)
            || ((id == null) || id.trim().length() == 0)
            || ((from == null) || from.trim().length() == 0)
            || ((workflow == null) || workflow.trim().length() == 0))
          m_logger.error("Cannot create PSTransitionInfo recieved empty or null data");
        else {
          TransitionInfo t = new TransitionInfo(flag.charAt(0), id, to, from, workflow);
          allTrans.add(t);
        }
      }
    }
  ...
  private class SortOrderTransition {
    ...
    private SortOrderTransition(StateInfo state, TransitionInfo transition) {
      if (state.getSortOrder() == null)
        throw new IllegalArgumentException(
            "The sort order of state id \"" + state.m_stateId + "\" cannot be null");
    ...
    public SortOrderTransition leastSortOrder(SortOrderTransition dat) {
      if (dat == null) return this;

      if (getSortOrder() < dat.getSortOrder()) return this;
      else return dat;
    }
  ...
  private class TransitionInfo implements IPSWorkflowId {
    ...
    public TransitionInfo(
      ...
      if ((transitionId == null)
          || (transitionFrom == null)
          || (transitionTo == null)
          || (workflowId == null)) throw new IllegalArgumentException("Arguments may not be null");
