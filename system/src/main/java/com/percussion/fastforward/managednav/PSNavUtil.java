...
public class PSNavUtil {
   ...
   */
  public static PSComponentSummary getItemSummary(IPSRequestContext request, PSLocator locator)
    ...
    try {
      ...
      if (summary == null) {
        throw new PSNavException("Could not get locator for contentid= " + locator.getId());
      }
      return summary;
  ...
  public static PSContentTypeTemplate loadVariantInfo(
    ...
    log.debug(
        "Search for Variant {} in ContentType {} failed!",
        variantName,
  ...
  public static PSContentTypeTemplate loadVariantInfo(
    ...
    log.warn(
        "Search for Variant ID {} in ContentType {} failed!",
   ...
   */
  public static PSSlotType findSlotByName(IPSRequestContext req, PSKey parent, String slotName)
      throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
   ...
   * @throws PSNavException
   */
  public static PSSlotTypeSet loadAllSlots(IPSRequestContext req) throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
    try {
      ...
      Element[] slotElems =
          compProxy.load(PSSlotTypeSet.getComponentType(PSSlotTypeSet.class), new PSKey[0]);
  ...
  public static void checkActiveAssembly(IPSRequestContext req, Map target) {
    ...
    if (sysCommand != null && sysCommand.equalsIgnoreCase("editrc")) {
      ...
      target.put("relateditemid", req.getParameter(IPSHtmlParameters.SYS_CONTENTID));
    }
  }
   ...
   */
  public static Document getNavonDocument(IPSRequestContext req, PSComponentSummary navon)
      throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
   ...
   * @throws PSNavException if process fails for any reason.
   */
  public static Document getNavonDocument(IPSRequestContext req, PSNavComponentSummary navon)
      throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
    ...
    if (config.getNavonTypeIds().contains(contentTypeId)) {
      ...
      throw new PSNavException("This summary is not a Nav Type" + navon.getName());
    }
   ...
   * @throws PSNavException if process fails for any reason.
   */
  public static Document getVariantDocument(
      IPSRequestContext req, PSContentTypeTemplate variant, PSLocator loc) throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
    if (variant == null) {
    ...
    try {
      ...
      IPSInternalRequest ir = req.getInternalRequest(variant.getAssemblyUrl(), pmap, false);
      if (ir == null) {
  ...
  public static String getFieldValueFromXML(Document doc, String fieldName) {
    ...
    Element resElem = walk.getNextElement(fieldName, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (resElem == null) {
   ...
   * @throws PSNavException
   */
  public static PSAaRelationshipList getSlotContents(
      IPSRequestContext req, PSLocator parentLoc, String slotName) throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
    if (parentLoc == null) {
  ...
  public static PSAaRelationshipList getSlotContents(
    ...
    PSAaRelationshipList rel = contents.getSlotContents(parentLoc, slot.getSlotId());
  ...
  public static PSAaRelationshipList getSlotContentsOld(
    ...
    try {
      slotContents = aaProxy.getSlotRelationships(parentLoc, slot, PSNavUtil.getAuthType(req));
    } catch (PSCmsException e) {
      throw new PSNavException(PSNavUtil.class.getName(), e);
    }
