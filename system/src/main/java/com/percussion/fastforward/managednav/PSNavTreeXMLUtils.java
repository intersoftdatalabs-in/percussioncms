...
/**
 * Consists of a set of utility methods to build and mainipulate the NavTree XML document.
 *
 * @author DavidBenua
 */
public class PSNavTreeXMLUtils {
  ...
  /**
   * Sets the value of the Nav specific variable. This is a context variable whose value is copied
   * from the context variable specified in the Nav Tree. <br>
   * The name of the source variable will taken from the lowest level ancestor node with a non-null
   * value of the selector. This happens during tree building, and the value at the root of the tree
   * must be valid before we get to this method. <br>
   * This method relies on the PSNavTree.XML_ATTR_VARIABLE name which must be specified in
   * Navigation.properties. The XML document must contain an &lt;AssemblerProperties&gt; node, and
   * this node must contain the source variable. If either of these conditions is not met, this
   * method returns without modifying the result document.
   *
   ...
   */
  public static void setNavVariable(IPSRequestContext req, Document resultDoc, Document navonDoc)
      throws PSNavException {
    log.debug("Setting Nav Variables");
    PSNavConfig config = PSNavConfig.getInstance(req);
    String varName = navonDoc.getDocumentElement().getAttribute(PSNavTree.XML_ATTR_VARIABLE);
    if (varName.trim().length() == 0) { // nothing to do here
      log.debug("No variable name specified");
      return;
    }
    PSXmlTreeWalker walker = new PSXmlTreeWalker(resultDoc.getDocumentElement());
    Element assemblerInfo =
        walker.getNextElement(XML_ELEM_ASSEMBLERINFO, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (assemblerInfo == null) { // no sys_AssemblerInfo element, can't go forward
      log.warn("No Assembler Info Element, cannot add Assembler Properties");
      return;
    }
    Element assemblerProps =
        walker.getNextElement(
            XML_ELEM_ASSEMBLERPROPERTIES, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
  ...
  /**
   * This method copies an assembler property. The new copy of the assembler property (aka Context)
   * variable will have the name specified in the PSNavConfig.NAVTREE_VARIABLE as specified in
   ...
   * @throws PSNavException
   */
  private static void copyAssemblerProperty(
      Element assemblerProps, String srcName, PSNavConfig config) throws PSNavException {
    ...
    if (newProp == null) {
      ...
      Element sValue =
          nwalk.getNextElement(XML_ELEM_VALUE, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      if (sValue == null) {
        throw new PSNavException("invalid document structure, no Value node");
      }
  ...
  /**
   * gets the assembler property that matches the supplied name. Returns <code>null</code> if no
   * property with that name exists.
   *
   * @param assemblerProps the <code>&lt;AssemblerProperties&gt;</code> element.
   * @param name the name of the element find.
   * @return the <code>&lt;Property&gt;</code> element that matches the name, or <code>null</code>
   */
  private static Element getPropertyByName(Element assemblerProps, String name) {
    ...
    Element property =
        walk.getNextElement(XML_ELEM_PROPERTY, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (property != null) {
      String pName = property.getAttribute(XML_ATTR_NAME);
      if (pName.equals(name)) { // we found our property
        return property;
      }
      property = walk.getNextElement(XML_ELEM_PROPERTY, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
    return null;
  }
   ...
   *
   * @param relatedContent DOM element representing the related content, must not be <code>null
   *     </code>.
   * @param link Nav link object from which the URL is obtained, must not be <code>null</code>.
   * @param slot Nav slot object to set the slotid and slot name attributes the linkurl element
   *     being created, must not be <code>null</code>.
   * @param doc the parent DOM document, must not be <code>null</code>.
   * @param context context value to set the rxcontext atribute of the linkurl element being
   *     created, must not be <code>null</code>, may be empty..
   */
  public static void addLinkUrl(
      Element relatedContent, PSNavLink link, PSNavSlot slot, Document doc, String context) {
    ...
    linkElem.setAttribute(XML_ATTR_VARIANT_ID, String.valueOf(link.getVariantId()));
  ...
  /**
   * Make an internal request and get the XML document for the assmebler properties for siteid and
   * context from the request context. Then add the assmbeler properties XML eleemnt from that
   * document to the result document.
   *
   * @param req request context object, must not be <code>null</code>.
   * @param resultDoc result XML document wo which the assembler properties element needs to be
   *     added, must not be <code>null</code> and must have the root element.
   * @throws PSNavException
   */
  public static void AddAssemblerProperties(IPSRequestContext req, Document resultDoc)
      throws PSNavException {
    if (req == null) {
      throw new IllegalArgumentException("req must not be null");
    }
    if (resultDoc == null || resultDoc.getDocumentElement() == null) {
      throw new IllegalArgumentException("resultDoc must not be null and must have a root eleemnt");
    }
    ...
    IPSInternalRequest irq = req.getInternalRequest(QUERY_ASSEMBLER_PROPERTIES, parm, false);
    if (irq == null) {
  ...
  private static final String QUERY_ASSEMBLER_PROPERTIES = "sys_casSupport/AssemblerProperties.xml";

  /** Error message to indicate that no assembler properties exist for a gven site and context. */
  ...
  public static final String XML_ELEM_ASSEMBLERPROPERTIES = "AssemblerProperties";
