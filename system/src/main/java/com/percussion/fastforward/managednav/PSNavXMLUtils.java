...
public class PSNavXMLUtils {
   ...
   */
  public static void setRootAttributes(IPSRequestContext req, Document doc, PSNavonStack stack)
      throws PSNavException {
   ...
   * @throws PSNavException
   */
  public static void setRootAttributes(
      IPSRequestContext req, Element docElem, PSNavonStack stack, Map extraAttribs)
      throws PSNavException {
  ...
  public static void processXMLTree(
      IPSRequestContext req, Document doc, PSNavonStack stack, Map landingPageParams)
      throws PSNavException {
    Element docElem = doc.getDocumentElement();
    ...
    Element rootNavon = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (rootNavon == null) {
      log.debug("the navigation tree is empty");
      return;
    }
    rootNavon.setAttribute(PSNavon.XML_ATTR_REL_LEVEL, String.valueOf(stack.getRelLevel()));
    if (stack.getRelLevel() == 0) {
      rootNavon.setAttribute(PSNavon.XML_ATTR_TYPE, PSNavonType.TYPENAME_SELF);
      log.debug("Root node is type {}", PSNavonType.TYPENAME_SELF);
    } else {
      rootNavon.setAttribute(PSNavon.XML_ATTR_TYPE, PSNavonType.TYPENAME_ROOT);
    ...
    processChildXML(req, doc, rootNavon, stack, stack.getRelLevel(), 0, landingPageParams);
  }
  ...
  private static void processChildXML(
    ...
    Element childElem =
        walker.getNextElement(PSNavon.XML_ELEMENT_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    ...
    while (childElem != null) {
      ...
      if (compareFlag) {
        String contentId = childElem.getAttribute(PSNavon.XML_ATTR_CONTENTID);
      ...
      processChildXML(req, doc, childElem, stack, rLevel + 1, aLevel + 1, landingPageParams);
      childElem =
          walker.getNextElement(PSNavon.XML_ELEMENT_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  }
   ...
   * @throws PSNavException
   */
  private static void buildLandingPage(
      IPSRequestContext req, Element navonElem, Map landingPageParams) throws PSNavException {
    PSXmlTreeWalker lpWalker = new PSXmlTreeWalker(navonElem);
    Element lpElement =
        lpWalker.getNextElement(
            PSNavon.XML_ELEMENT_LANDINGPAGE, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (lpElement != null) {
      log.debug("setting landing page attributes");
      setAttributes(lpElement, landingPageParams);

      String variantIdStr = lpElement.getAttribute(IPSHtmlParameters.SYS_VARIANTID);
      int variantid = Integer.parseInt(variantIdStr);
      PSContentTypeTemplate variant =
          PSNavConfig.getInstance(req).getAllVariants().getContentVariantById(variantid);
      log.debug("found variant {}", variant.getName());
      Map<String, String> linkParams = new HashMap<>();
      String contentId = lpElement.getAttribute(IPSHtmlParameters.SYS_CONTENTID);
      linkParams.put(IPSHtmlParameters.SYS_CONTENTID, contentId);
      String revision = lpElement.getAttribute(IPSHtmlParameters.SYS_REVISION);
      linkParams.put(IPSHtmlParameters.SYS_REVISION, revision);

      String folderId = lpElement.getAttribute(IPSHtmlParameters.SYS_FOLDERID);
      ...
      linkParams.put(IPSHtmlParameters.SYS_SESSIONID, req.getSecurityToken().getUserSessionId());
  ...
  private static void fixInfoLink(
      IPSRequestContext req, Element navonElem, Map<String, String> landingPageParams)
      throws PSNavException {
    log.debug("fixing Info Link");
    PSXmlTreeWalker infoWalker = new PSXmlTreeWalker(navonElem);
    Element infoElem =
        infoWalker.getNextElement(
            PSNavon.XML_ELEMENT_INFOLINK, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (infoElem != null) {
      try {
   ...
   * @throws PSNavException
   */
  private static void fixImageLinks(IPSRequestContext req, Element navonElem, Map landingPageParams)
      throws PSNavException {
    log.debug("fixing Images Link");
    PSXmlTreeWalker imageListWalker = new PSXmlTreeWalker(navonElem);
    Element imageListElem =
        imageListWalker.getNextElement(
            PSNavon.XML_ELEMENT_IMAGELIST, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (imageListElem != null) {
      PSXmlTreeWalker imageWalker = new PSXmlTreeWalker(imageListElem);
      Element imageElement =
          imageWalker.getNextElement(
              PSNavImageLink.XML_ELEM_IMAGELINK, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      while (imageElement != null) {
        ...
        imageElement =
            imageWalker.getNextElement(
                PSNavImageLink.XML_ELEM_IMAGELINK, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
      }
    } else {
   ...
   * @throws PSNavException
   */
  public static void overrideTheme(IPSRequestContext req, Document doc) throws PSNavException {
   ...
   * @throws PSNavException
   */
  private static String getThemeFromQuery(IPSRequestContext req) throws PSNavException {
    log.debug("Getting theme from query");
    try {
      IPSInternalRequest irq = req.getInternalRequest(NAV_THEME_QUERY, new HashMap(), true);
      if (irq == null) {
  ...
  public static Map<String, String> getLandingPageMap(IPSRequestContext req) {
    ...
    PSNavUtil.copyParam(req, landingPageParams, IPSHtmlParameters.SYS_CONTEXT);
    PSNavUtil.copyParam(req, landingPageParams, IPSHtmlParameters.SYS_SITEID);
    PSNavUtil.copyParam(req, landingPageParams, IPSHtmlParameters.SYS_AUTHTYPE);
    landingPageParams.put(
        IPSHtmlParameters.SYS_SESSIONID, req.getSecurityToken().getUserSessionId());
    ...
    if (cmd != null) {
      ...
      if (cmd.equalsIgnoreCase("editrc")) {
        ...
        landingPageParams.put("relateditemid", req.getParameter(IPSHtmlParameters.SYS_CONTENTID));
      }
    }
