...
public class PSNavTree {
  ...
  public PSNavTree(IPSRequestContext req) throws PSNavException {
    ...
    PSLocator selfLoc = new PSLocator(req.getParameter(IPSHtmlParameters.SYS_CONTENTID));
    PSComponentSummary self = PSNavUtil.getItemSummary(req, selfLoc);
    log.debug("Self node is {}", self.getName());
    if (ms_config.getNavTreeTypeIds().contains(self.getContentTypeId())) { // this is the root
      ...
      loadNavTreeData(req, self.getCurrentLocator());
        ...
        .getNavonTypeIds()
        .contains(self.getContentTypeId())) { // this is a navon somewhere in the middle of the tree
      ...
      while (top != null) {
        ...
        if (next.getType().getType() == PSNavonType.TYPE_ROOT) { // we found a NavTree content item
          this.m_root = next;
   ...
   * @throws PSNavException
   */
  private void loadNavTreeData(IPSRequestContext req, PSLocator loc) throws PSNavException {
    try {
      ...
      this.m_theme = PSNavUtil.getFieldValueFromXML(tdoc, ms_config.getNavThemeParamName());

      this.m_variableSelector =
          PSNavUtil.getFieldValueFromXML(tdoc, ms_config.getNavonVariableName());

      this.m_imageSelector =
          PSNavUtil.getFieldValueFromXML(tdoc, ms_config.getNavonSelectorField());
  ...
  public Document toXml(IPSRequestContext req) {
    ...
    Element rootElement = PSXmlDocumentBuilder.createRoot(navtreeDoc, XML_NODE_NAME);
    ...
    if (this.m_variableSelector != null && this.m_variableSelector.trim().length() > 0) {
    ...
    // templates
    Element shared = PSXmlDocumentBuilder.addElement(navtreeDoc, rootElement, "shared", null);
    Element displaytitle =
        PSXmlDocumentBuilder.addElement(navtreeDoc, rootElement, "displaytitle", m_root.getName());
