...
/**
 * It provides various operations to access the managed navigation objects, navtree, navon and its
 * configuration.
 *
 * <p>A site may contain one navigation system. A navigation system contains a navtree and 0 or more
 * navon (navigation node).
 *
 * <p>A typical navtree contains (or links with) one home page and 0 or more navons. A navon may
 * contain 0 or more navon.
 *
 * <p>Note, the caller must make sure {@link #isManagedNavUsed()} returns <code>true</code> before
 * calling any other methods. This is to insure the managed navigation is configured properly;
 * otherwise the implementation may throw unchecked exception.
 *
 * @author YuBingChen
 */
public interface IPSManagedNavService {
  ...
  /**
   * Adds a Navon to a child folder. The Navon should always be added in the community of the parent
   * Navon (which is in the parent folder) even if the user is in a different community.
   *
   ...
   * @return the ID of added navon. It may be <code>null</code> if there is no navon in the parent
   *     folder.
   */
  IPSGuid addNavonToFolder(
  ...
  /**
   * Calls {@link #addNavonToFolder(IPSGuid, IPSGuid, String, String, int)} supplying -1 for the
   * workflow Id
   */
  IPSGuid addNavonToFolder(
   ...
   *
   * <p>Note, the navigation node/item must be checked out before this call.
   *
   ...
   * @param templateName the name of the template, which is used to create the relationship between
   *     the nav-node (owner) and the landing page (dependent), never <code>null</code>.
   */
   ...
   * @param index the target location of the source node. The source node will be placed on the top
   *     if it is <code>0</code>. The child node will be placed at the bottom if it is <code>-1
   *     </code> or a number greater than the number of child nodes under the parent node. If the
   *     source is already under the target node, then the index is the location of the source node
   *     after it is moved.
   */
   ...
   * @param parentNavonId The parent navon on which the relationships are replaced, must not be
   *     null.
   */
  ...
  /**
   * Gets the landing page for a given navigation node (navon). It is the responsibility of the
   * caller to ensure the specified navon id has a revision > -1.
   *
   * @param nodeId the ID of the navigation node, not <code>null</code>.
   * @return the ID of the landing page, may be <code>null</code> if not found. It is the
   *     responsibility of the caller to get the correct revision of the page as the revision part
   *     of this ID will be -1.
   */
   ...
   * @return <code>true</code> if the item is a landing page; otherwise return <code>false</code>.
   */
   ...
   *
   * @param folderId the folder ID that may contain a navtree or navon, not <code>null</code>.
   * @return the summary of a navtree or navon under the specified folder. It may be <code>null
   *     </code> if there is no navtree or navon item under the folder.
   */
   ...
   * Finds a navtree or navon item under the specified folder.
   *
   * @param folderPath the folder path that may contain a navtree or navon, not <code>null</code>.
   * @return the summary of a navtree or navon under the specified folder. It may be <code>null
   *     </code> if there is no navtree or navon item under the folder.
   */
   ...
   *
   * @param folderPath the folder that may contain a navigation item, not blank.
   * @return the ID of the navigation item. It may be <code>null</code> if the folder does not
   *     contain a navigation item. The revision of the returned ID is undetermined.
   */
  ...
  /**
   * Finds the navigation node ID that is related to the specified page if the specified page is a
   * landing page.
   *
   * <p>Assumed a landing page exists under one folder only.
   *
   * @param id the ID of the specified page, not <code>null</code>.
   * @return the related navigation node ID if the given page is a landing page. It may be <code>
   *     null</code> if the specified page is not a landing page.
   */
   ...
   * Finds a navigation node/item under a specified folder.
   *
   * @param folderId the ID of the folder that may contain a navigation item, not blank.
   * @return the ID of the navigation item. It may be <code>null</code> if the folder does not
   *     contain a navigation item. The revision of the returned ID is undetermined.
   */
  ...
  /**
   * Sets the display title for the specified navigation node. The workflow state of the navigation
   * item/node will remain to be the same afterwards.
   *
   * @param nodeId the ID of the navigation node, not <code>null</code>.
   ...
   * @return the list of IDs of the child navigation nodes, never <code>null</code>, but may be
   *     empty.
   */
  List<IPSGuid> findChildNavonIds(IPSGuid nodeId);

  /**
   * Finds IDs of all the descendant navigation nodes for the specified node. The descendant nodes
   * are the immediate child nodes, the grand child nodes (the child nodes of the immediate child
   * nodes), ..., until reach to the leave level nodes.
   *
   * @param nodeId the ID of the navigation node, never <code>null</code>.
   * @return the list of IDs of the descendant navigation nodes, never <code>null</code>, but may be
   *     empty.
   */
   ...
   *
   * @return <code>true</code> if the navigation properties file exists; otherwise return <code>
   *     false</code>.
   */
  ...
  /**
   * Sets the supplied property values on the supplied navon. If the navon is in publishable state,
   * moves to quick edit state and leaves it there checked in.
   *
   * @param nodeId the ID of the navigation node, never <code>null</code>.
   * @param propertyMap of name and value pairs. If the name is not field of navon type throws
   *     exception.
   */
  public void setNavonProperties(IPSGuid nodeId, Map<String, String> propertyMap);

  /**
   * Moves a specified source navigation node to a specified target navigation node at the specified
   * location.
   *
   * <p>Note, if the source parent is specified, this will only move the specified node from the
   * source parent to the target node; otherwise the side effect of the move operation will remove
   * the source node from all its parent nodes then add it to the target node.
   *
   * @param sourceId the ID of the source node, not <code>null</code>.
   * @param srcParentId the parent ID of the source node. It may be <code>null</code> if the caller
   *     wants to have the side effect after the move operation (described above).
  ...
  /**
   * Checks whether the supplied navonId is a section link or not. If the folder corresponding to
   * the supplied navon is a child of the supplied parent navon's folder, then returns <code>true
  ...
  /**
   * Loads the item corresponding to the UI and verifies if it is the nav tree object, or a navon
   * object.
   *
   * @param guid {@link IPSGuid} with the target guid. Must not <code>null</code>.
   * @return <code>true</code> if the guid is the navTree object. <code>false</code> otherwise.
   */
  ...
  /**
   * Finds the ancestor's IDs for the specified navon node. Note, the returned ancestor IDs do not
   * include the parent that links to the specified node as a 'section link'.
   *
