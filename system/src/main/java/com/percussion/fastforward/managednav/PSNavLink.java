...
public class PSNavLink {
  ...
  public void createLinkToDocument(
      IPSRequestContext req, PSNavComponentSummary summary, int variantId) throws PSNavException {
    PSContentTypeTemplate variant =
        PSNavUtil.loadVariantInfo(req, summary.getContentTypeId(), variantId);
    if (variant == null) {
   ...
   */
  public void createLinkToDocument(
      IPSRequestContext req, PSNavComponentSummary summary, PSContentTypeTemplate variant)
      throws PSNavException {
  ...
  public void createLinkToDocument(
    ...
    String folderid = (String) extraParams.get(IPSHtmlParameters.SYS_FOLDERID);
  ...
  public void createLinkFromSnippet(
    ...
    m_variantId = variant.getVariantId();
    String folderid = (String) extraParams.get(IPSHtmlParameters.SYS_FOLDERID);
    if (folderid != null && folderid.trim().length() > 0) m_folderId = Integer.parseInt(folderid);
    ...
    try {
      ...
      Map params = buildLinkParams(summary, variant, req);
      if (extraParams != null && extraParams.size() > 0) params.putAll(extraParams);
      if (log.isDebugEnabled()) {
        PSNavUtil.logMap(params, "internal link params", log);
      }

      IPSInternalRequest ir = req.getInternalRequest(variant.getAssemblyUrl(), params, false);
      if (ir == null) {
      ...
      Document doc =
          PSXmlDocumentBuilder.createXmlDocument(new ByteArrayInputStream(snippetData), false);
      ...
      if (links.getLength() > 0) {
        ...
        if (m_uri == null) {
          log.warn(
              "snippet without @href attribute found  \n {} ", PSXmlDocumentBuilder.toString(doc));
        }
  ...
  private Map buildLinkParams(
      PSComponentSummary summary, PSContentTypeTemplate variant, IPSRequestContext req) {
    PSNavComponentSummary navsum = new PSNavComponentSummary(summary);
  ...
  public Element toXML(Element parentElem) {
    parentElem.setAttribute(XML_ATTR_CONTENTID, String.valueOf(m_contentId));
    parentElem.setAttribute(XML_ATTR_VARIANTID, String.valueOf(m_variantId));
