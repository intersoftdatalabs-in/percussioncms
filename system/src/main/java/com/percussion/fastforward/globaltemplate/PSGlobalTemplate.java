...
/**
 * This class is the Java representation of the "Global" or "Outer" template introduced in Rhythmyx
 * Simple (code name "Zho" or Fast Forward). A global template is created off of a marked up HTML
 * source. The markups indicate the location of the navigation slots and the body of the page. The
 * body comes from the local template which is a variant for the actual item. The global template,
 * by definition is common for several pages on one or more sites. This provides place for various
 * navigation bars for the page and encloses the actual content specific to the page which we call
 * body of the page.
 *
 ...
 *       <ol>
 *         <li>Modify the &gt;title&gt;element to make the title dynamic
 *         <li>Add an XSL block to insert a global_head template
 *         <li>Modify all &lt;link&gt; tags that point to CSS files to point to dynamic location via
 *             context variable
 *         <li>Modify all &lt;script&gt; tags that point to JavaScript files to point to dynamic
 *             location via context variable
 *       </ol>
 *   <li>Modify all &lt;img&gt; element to change the attribute "src" to point to dynamic location
 *       via the context variable for images
 *   <li>Insert the XSL blocks navigation slots with appropriate syntax based on the markups in the
 *       HTML source
 *   <li>Insert the page body spacer which calls the local template to render the body part of the
 *       page
 *   <li>Run XSpLit to split this prepared HTML document to generate the XSL file, and
 *   <li>Post process the XSL document to perform renamng the template names appropriately and
 *       remove any &lt;xsl:import&gt;s added by XSpLit
 * </ol>
 *
 * <p>Please refer to the global template documentation for more details of the above steps.
 *
 * <p>The file name for the XSL document which is the final outcome of this class is auto generated
 * by replacing ".htm*" with ".xsl". The saved file always goes a predefined location which is
 * {@link com.percussion.fastforward.globaltemplate.PSRxGlobals#RX_GLOBALS_PATH}.
 *
 * <p>Please note that all parsing and saving of the input and output content uses the UTF-8
 * encoding}
 */
public class PSGlobalTemplate extends PSLoggable {
  /**
   * Constructor. All the steps described in the class description are performed here.
   *
   * @param rxRootDir The Rhythmyx root directory, may not be <code>null</code> or empty.
   * @param appDir the location of the markedup HTML source file relative to Rhythmyx root. Must not
   *     be <code>null</code> or empty.
   * @param srcHtmlFile name of the marked up HTML source file for the global template. Must not be
   *     <code>null</code> or empty.
   * @param logger {@link org.apache.logging.log4j.Logger logger} to log the inoformation, warnings
   *     or errors. May be <code>null</code> in which case a logger with default named category will
   *     be ceated.
  ...
  public PSGlobalTemplate(String rxRootDir, String appDir, String srcHtmlFile, Logger logger)
    ...
    if (loc != -1) m_name = m_name.substring(0, loc);
  ...
  /**
   * Fix the HTML document to make sure that there are no empty &lt;script&gt; elements. HTML
   * document with empty script elements does not render in IE for example. Adds ";" as the value of
   * such elements.
   */
   ...
   *
   * <ol>
   *   <li>Remove &lt;xsl:import&gt;s added by the XSpLit. Global template does not need these
   *   <li>Rename the name "xsplit_body" of the "xsl:call-template" to the name of the template
   *       (m_name) appended with "_body".
   *   <li>Rename the name "placeholder_xsplit_body" of the "xsl:call-template" to "xsplit_body"
   *   <li>Rename the root template name to m_name + "_root" and the template with name
   *       "xsplit_body" to m_name + "_body"
   * </ol>
   */
  private void postProcess() {
    ...
    logInfo("Renaming call to template named \"xsplit_body\" to " + getName() + "_body");

    NodeList callTemplateElems = m_xslDoc.getElementsByTagName("xsl:call-template");
    ...
    // to "xsplit_body".
    logInfo("Renaming call to template named \"placeholder_xsplit_body\"" + "to \"xsplit_body\"");
  ...
  /**
   * Parse element recursively for all navigation slots and body place holder and then insert
   * appropriate slot blocks recognizable by XSpLit.
   *
   * @param elem the element to look for the slot or body markup and process, must not be <code>null
   *     </code>
   */
  ...
  /**
   * Processes the marked up element for body container to insert an XSL block that ultimately
   * renders the body part of the page. The markup attribute will be removed after the XSL block is
   * inserted.
   *
   * @param bodyElem the element that is market up as the place holder for the body. Assumes that
   *     this elemen has the required markup attribute {@link #ATTR_PSXBODY}.
   */
  private void processBodyTag(Element bodyElem) {
    if (bodyElem == null) throw new IllegalArgumentException("bodyElem must not be null");

    String bodymarkup = bodyElem.getAttribute(ATTR_PSXBODY);
    if (isEmpty(bodymarkup)) { // FB: RV_EXCEPTION_NOT_THROWN NC 1-17-16
      throw new IllegalArgumentException("Element body must have " + ATTR_PSXBODY + " attribute");
    }

    logInfo("Processing element " + bodyElem.getTagName() + " for local template/body...");

    Node startLocalTemplateComment = m_tidiedHtmlDoc.createComment(" start Local Template ");
    ...
    Element xslCallTemplate = m_tidiedHtmlDoc.createElement("xsl:call-template");
    ...
    Node endLocalTemplateComment = m_tidiedHtmlDoc.createComment(" end Local Template ");

    NodeList children = bodyElem.getChildNodes();
    for (int i = children.getLength() - 1; i >= 0; i--) bodyElem.removeChild(children.item(i));
    ...
    logInfo("End processing element " + bodyElem.getTagName() + " for local template/body...");
  }

  /**
   * Processes the element marked up for a navigation via the attribute {@link #ATTR_PSXSLOTNAME}
   * slot in that it inserts the slot makup consumable by the XSpLit. Assumes that this elemen has
   * the required markup attribute {@link #ATTR_PSXSLOTNAME}. Removes the attribute after slot XSL
   * markup.
   *
   * @param slotElem element with slot markup, must not be <code>null</code> and must have the
   *     attribute {@link #ATTR_PSXSLOTNAME}.
   */
  private void processSlotTag(Element slotElem) {
    if (slotElem == null) throw new IllegalArgumentException("slotElem must not be null");

    String slotName = slotElem.getAttribute(ATTR_PSXSLOTNAME);

    if (isEmpty(slotName))
      throw new IllegalArgumentException("slotElem must have " + ATTR_PSXSLOTNAME + " attribute");

    logInfo("Processing element " + slotElem.getTagName() + " for slots and local template...");

    Node startSlotComment = m_tidiedHtmlDoc.createComment(" start slot " + slotName + " ");
    Node startSnippetWrapperComment = m_tidiedHtmlDoc.createComment(" start snippet wrapper ");
    ...
    Node endSnippetWrapperComment = m_tidiedHtmlDoc.createComment(" end snippet wrapper ");
    Node endSlotComment = m_tidiedHtmlDoc.createComment(" end slot ");

    NodeList children = slotElem.getChildNodes();
    for (int i = children.getLength() - 1; i >= 0; i--) slotElem.removeChild(children.item(i));
    ...
    logInfo("End processing element " + slotElem.getTagName() + " for slots and local template...");
  }

  /**
   * Modify all image (&lt;img&gt;) tags and input tags of type image (&lt;input type='image' ...
   * &lt;/input&gt;) and any other tag that has the attribute 'background' to add the dynamic
   * locations for the "src" or "background" attribute in the tidied HTML document. Those tags that
   * already have dynamic location specified will be ignored. The markup follows the XSpLit
   * conventions.
   */
  ...
  /**
   * Process a single element that has an attribute that specifies image location path.
   *
   * @param imgElem DOM element that has an attribute that can hold an image path which needs to be
   *     made dynamic. Assumed not <code>null</code>.
   * @param attrName Name of the attribute that holds the path, assumed not <code>null</code>.
   */
  private void processImgElement(Element imgElem, String attrName) {
    String psxsrc = imgElem.getAttribute("psx-" + attrName);
    if (!isEmpty(psxsrc)) return;
    String src = imgElem.getAttribute(attrName);
    int loc = src.lastIndexOf('/');
    if (loc != -1) src = src.substring(loc + 1);
    src = "$rxs_img/" + src;
    imgElem.setAttribute("psx-" + attrName, src);
  }

  /**
   * Modify the HTML &lt;head&gt; element in the tidied document to achieve the following:
   *
   * <ol>
   *   <li>Modify the &gt;title&gt;element to make the title dynamic
   *   <li>Add an XSL block to insert a global_head template
  ...
  private void modifyHeader() {
    ...
    Element xslCallTemplateElem = m_tidiedHtmlDoc.createElement("xsl:call-template");
  ...
  /**
   * Access method for the name of the template. The name is built by stripping the file extension
   * from the source file.
   *
   * @return name of the global template as explained above. Never <code>null</code> or empty.
   */
  ...
  /**
   * Save the files associated with this template. First, saves the tidied and pre-processed HTML
   * document to the application directory specified during construction. The file name will be
   * constructed as template name appended with "_prepared.html". Then saves the global template XSL
   * document. The name of this file is constructed by appending ".xsl" to the template name.
   *
  ...
  /**
   * Saves the outer template XSL document. The path of the file is constructed by concatinating:
   * {@link PSRxGlobals#ABS_GLOBAL_TEMPLATES_PATH global template path}, {@link #getName() template
   * name} and ".xsl".
   *
   * @throws IOException if save fails for any reason.
   */
  private void saveXsl() throws IOException {
    String fileName = PSRxGlobals.ABS_GLOBAL_TEMPLATES_PATH + File.separator + getName() + ".xsl";
    logInfo("Saving the global template XSL to " + fileName);
    saveDoc(m_xslDoc, fileName, true);
  }

  /**
   * Saves the specifed XML document to specified file path. Always uses {@ link
   * PSCharSets#getInternalEncoding() encoding} while saving. Optionally adds the standard HTML
   * entity refernces.
   *
   * @param doc XML document to save to the file, must not be <code>null</code>.
   * @param filePath path and name of the file to save the document to. It does not create any non
   *     existing directories in the path. Must not be <code>null</code> or empty.
   * @param addHtmlEntRef <code>true</code> to add the standard HTML entity references to the file
   *     being saved, <code>false</code> otherwise.
   * @throws IOException in case of any error writng to file.
   */
  private void saveDoc(Document doc, String filePath, boolean addHtmlEntRef) throws IOException {
  ...
  /**
   * Constant for the suffix that is appended to the name of the global template file and used as
   * the template name
   */
  public static final String GLOBAL_TEMPLATE_NAME_SUFFIX = "_root";

  /**
   * Name of the template. Built in the constructor by stripping the extension from the source HTML
   * file. Never <code>null</code> or empty.
   */
  private String m_name = null;

  /**
   * Tidied HTML document. Constructed during tidying process in the constructor. Never <code>null
   * </code> after that.
   */
  private Document m_tidiedHtmlDoc = null;

  /**
   * XSL outer templaet which is the final result desired from this object. Built by running XSpLit
   * on the tidied source in the constructor, never <code>null</code> after that.
   */
  private Document m_xslDoc = null;

  /**
   * Folder location of the template source file. Iniialized in the constructor, never <code>null
   * </code> after that.
   */
  private String m_appDir = null;

  /**
   * Name of the attribute of an HTML element to indicate that all the child content is to be
   * replaced or tagged as dynamic body or local template.
   */
  ...
  /**
   * Name of the attribute of an HTML element attribute to indicate that the slot is editable or
   * not.
   */
  ...
  public static String getDefaultEntities(String serverRoot) {
    ...
    if (root == null || root.equals("")) root = ms_defaultServerRoot;

    return "\t<!ENTITY % HTMLlat1 SYSTEM \"/"
        + root
        + "/DTD/HTMLlat1x.ent\">"
        + NEWLINE
        + "\t\t%HTMLlat1;"
