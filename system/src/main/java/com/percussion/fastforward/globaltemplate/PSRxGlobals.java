...
public class PSRxGlobals extends PSLoggable {
    ...
    try {
      writer =
          new OutputStreamWriter(
              new FileOutputStream(m_globalTemplateFilePath), StandardCharsets.UTF_8);
  ...
  private void updateDispatchingTemplate() {
    ...
    for (int i = 0; dispatchingTemplate == null && i < templates.getLength(); i++) {
      ...
      if (name.equals(DISPATCHING_TEMPLATE_NAME)) dispatchingTemplate = template;
    }
    ...
    if (dispatchingTemplate == null) {
      ...
      NodeList chooseElements = dispatchingTemplate.getElementsByTagName("xsl:choose");
      choose = (Element) chooseElements.item(0);

      // there is always maximum one otherwise element
      NodeList otherwiseElements = dispatchingTemplate.getElementsByTagName("xsl:otherwise");
      if (otherwiseElements.getLength() > 0) otherwise = (Element) otherwiseElements.item(0);
    }
    ...
    while (keys.hasNext()) {
      ...
      String test = "/*/sys_AssemblerInfo[@psxglobaltemplate='" + name + "']";
      ...
      if (!found) {
        ...
        if (otherwise == null) choose.appendChild(when);
        else choose.insertBefore(when, otherwise);
    ...
    for (int i = existingWhens.getLength() - 1; i >= 0; i--) {
      ...
      if (!found) {
        logInfo("Removing dispatch case for: " + existingWhen.getAttribute("test"));
  ...
  private void saveTemplateFiles() throws IOException {
    ...
    while (keys.hasNext()) {
      ...
      PSGlobalTemplate template = (PSGlobalTemplate) m_GlobalTemplates.get(key);
      template.save();
    }
  }
  ...
  private void cleanupImports() {
    ...
    while (keys.hasNext()) {
      ...
      m_doc.getDocumentElement().insertBefore(elem, m_doc.getDocumentElement().getFirstChild());
    }
