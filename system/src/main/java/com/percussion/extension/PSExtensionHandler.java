...
public abstract class PSExtensionHandler implements IPSExtensionHandler {
  ...
  public synchronized void install(IPSExtensionDef def, Iterator<?> resources)
    ...
    if (!(def instanceof PSExtensionDef)) {
      // TODO: i18n and code
      throw new PSExtensionException(0, "def is not the right type for this handler");
    }
    ...
    try {
      ...
      ((PSExtensionDef) def).setInitParameter(INIT_PARAM_VERSION, Integer.toString(version));
      ...
      PSExtensionManager emgr = (PSExtensionManager) PSServer.getExtensionManager(null);
      if (emgr != null) {
  ...
  public synchronized void update(IPSExtensionDef def, Iterator<?> resources)
    ...
    ((PSExtensionDef) def).setInitParameter(INIT_PARAM_VERSION, Integer.toString(newVer));
  ...
  private ArrayList<URL> catalogResources(File location, File codeRoot) throws MalformedURLException {
    ...
    if (!location.isDirectory())
      throw new IllegalArgumentException("location must be a valid directory.");

    if (codeRoot == null) throw new IllegalArgumentException("codeRoot may not be null.");
  ...
  public File getCodeBase(IPSExtensionDef def) {
    ...
    return new File(
        m_rootDir, ref.getContext() + ref.getExtensionName() + "/" + def.getVersion() + "/");
  }
  ...
  protected void checkState() {
    if (!m_isInited) {
      throw new IllegalStateException("Extension handler has not been initialized.");
    }
  }
  ...
  private boolean isWithinRoot(File f) throws IOException {
    ...
    while (f != null) {
      if (f.equals(m_rootDir)) return true;
  ...
  protected void logMessage(int errorCode, Object[] args) {
    ...
    if (m_logHandler == null) {
      ...
      for (int i = 0; i < messages.length; i++) strMessage.append(messages[i].getText());
      System.out.println(strMessage.toString());
    } else m_logHandler.write(warning);
  }
