...
public class PSModifyXmlHierarchyExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    try {
       ...
       */
      String relativeResponseKey = PSXmlTreeWalker.getRelativeFieldName(responseNode, responseKey);
      if (relativeResponseKey == null) { // no path?!
        throw new IllegalArgumentException(
            "no path found between " + responseNode + " and " + responseKey);
      }

      String relativeParentKey = PSXmlTreeWalker.getRelativeFieldName(parentNode, parentKey);
      if (relativeParentKey == null) { // no path?!
        throw new IllegalArgumentException(
      ...
      for (int i = 0; i < size; i++) {
        ...
        if ((parent != null)
            && (parent != el)) { // we found our parent, move ourselves under the parent
