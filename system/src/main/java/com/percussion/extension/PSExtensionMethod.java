...
public class PSExtensionMethod implements Serializable {
  ...
  /** The method description, never <code>null</code> after construction, may be empty. */
  private String m_description = null;

  /**
   * The method parameters, never <code>null</code>, may be empty. The order of the parameters is
   * important for the way a method is called.
   */
  private List<PSExtensionMethodParam> m_parameters = new ArrayList<>();

  /** The method return type, never <code>null</code> or empty after construction. */
   ...
   * @param description the method description, may be <code>null</code> or empty.
   */
   ...
   *
   * @param source the XML element from which to construct this method, not <code>null</code>.
  ...
  /**
   * Add a new method parameter. The caller must make sure that the parameters are added in
   * execution order.
   *
   * @param parameter the new method parameter, not <code>null</code>. If a parameter with the same
   *     name already exists it will be replaced with the one supplied, otherwise the supplied
   ...
   *
   * @param name the name or the parameter to be removed, not <code>null</code> or empty. Does
   *     nothing if the identified parameter does not exist.
   */
  public void removeParameter(String name) {
    ...
    if (index != -1) m_parameters.remove(index);
  }
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(m_name, that.m_name)
        ...
        && Objects.equals(m_returnType, that.m_returnType);
  }
   ...
   *
   * @param source the xml element from which to construct this object, nor <code>null</code>.
  ...
  public void fromXML(Element source) throws PSExtensionException {
    ...
    if (!source.getTagName().equals(XML_NAME))
      throw new PSExtensionException(
          IPSExtensionErrors.INVALID_XML_ELEMENT, new Object[] {source.getTagName(), XML_NAME});

    String test = source.getAttribute(NAME_ATTR);
    if (StringUtils.isBlank(test))
      throw new PSExtensionException(IPSExtensionErrors.MISSING_REQUIRED_ATTRIBUTE, NAME_ATTR);
    setName(test);
    ...
    NodeList params = source.getElementsByTagName(PSExtensionMethodParam.XML_NAME);
  ...
  public Element toXML(Document doc) {
    ...
    while (params.hasNext()) parameters.appendChild(params.next().toXML(doc));
