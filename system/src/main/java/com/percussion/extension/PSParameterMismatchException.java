...
/**
 * PSParameterMismatchException is thrown to indicate that an extension or extension was initialized
 * with the incorrect parameter values.
 *
 * <p>Extensions provide their parameter definitions through the {@link
 * IPSExtensionDef#getRuntimeParameter IPSExtensionDef.getRuntimeParameter} method. The corrsponding
 * run-time values must then be set via an unspecified mechanism (usually a sub-interface will
 * define how params are to be set).
 *
...
public class PSParameterMismatchException extends PSException {
  /**
   * Constructs a parameter mismatch exception when an incorrect number of values was specified.
   *
