...
public class PSExtensionDefFactory implements IPSExtensionDefFactory {
  ...
  public Element toXml(Element root, IPSExtensionDef def, boolean excludeMethods) {
    ...
    Element e = PSXmlDocumentBuilder.addEmptyElement(doc, root, "Extension");
    ...
    if (!excludeMethods) {
      ...
      if (methods.hasNext()) {
        Element methodsElem = PSXmlDocumentBuilder.addEmptyElement(doc, e, METHODS_ELEM);
        while (methods.hasNext()) methodsElem.appendChild(methods.next().toXML(doc));
      }
    }
  ...
  public IPSExtensionDef fromXml(Element defElement) throws PSExtensionException {
    ...
    try {
      ref =
          new PSExtensionRef(
              defElement.getAttribute("categorystring"),
    ...
    String strRestore = defElement.getAttribute("restoreRequestParamsOnError");
    ...
    while (paramEl != null) {
      ...
      if (paramName == null) {
        // TODO: i18n and code
        throw new PSExtensionException(0, "init param name cannot be null");
      }
    ...
    while (interfaceEl != null) {
      ...
      if (ifaceName == null) {
        // TODO: i18n and code
        throw new PSExtensionException(0, "interface name cannot be null");
      }
    ...
    while (rParamEl != null) {
      try {
        ...
        rParamEl = tree.getNextElement(PSExtensionParamDef.ms_NodeType, nextFlag);
    ...
    // reset so we don't have to worry about order
    tree.setCurrent(defElement);
    Element appsEl = tree.getNextElement("requiredApplications", firstFlag);
    if (appsEl != null) {
    ...
    if (methodsElem != null) {
      Element methodElem = tree.getNextElement(PSExtensionMethod.XML_NAME, firstFlag);
      while (methodElem != null) {
        def.addExtensionMethod(new PSExtensionMethod(methodElem));
        methodElem = tree.getNextElement(PSExtensionMethod.XML_NAME, nextFlag);
      }
    }
