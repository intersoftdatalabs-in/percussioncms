...
public class PSValidateCharacters implements IPSFieldValidator {
  ...
  protected String getParameter(Object[] params, int i, String defaultValue, boolean notBlank) {
    ...
    if (notBlank) notEmpty(rvalue, "parameter: " + paramName + " cannot be empty or blank");
    return rvalue;
  }
   ...
   */
  public final void init(IPSExtensionDef def, File codeRoot) throws PSExtensionException {
    String[] paramNames =
        (String[]) IteratorUtils.toArray(def.getRuntimeParameterNames(), String.class);
