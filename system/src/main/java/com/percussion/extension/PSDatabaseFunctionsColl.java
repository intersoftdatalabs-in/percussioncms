...
public class PSDatabaseFunctionsColl {
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    m_dbFuncs.clear();
    Element el = walker.getNextElement(PSDatabaseFunction.getNodeName(), firstFlags);
    while (el != null) {
      PSDatabaseFunction dbFunc = new PSDatabaseFunction(m_type, el);
      m_dbFuncs.put(dbFunc.getName().toLowerCase(), dbFunc);
      el = walker.getNextElement(PSDatabaseFunction.getNodeName(), nextFlags);
    }
  }
  ...
  public boolean equals(Object obj) {
    ...
    boolean equals = true;
    if (!(obj instanceof PSDatabaseFunctionsColl)) equals = false;
    else {
      PSDatabaseFunctionsColl other = (PSDatabaseFunctionsColl) obj;
      if (m_type != other.m_type) equals = false;
      else if (!(compareDatabaseFunctions(other, false))) equals = false;
    }
    return equals;
  }
  ...
  public boolean equalsFull(Object obj) {
    ...
    if (!(obj instanceof PSDatabaseFunctionsColl)) equals = false;
      ...
      PSDatabaseFunctionsColl other = (PSDatabaseFunctionsColl) obj;
      if (m_type != other.m_type) equals = false;
      else if (!(compareDatabaseFunctions(other, true))) equals = false;
    }
    return equals;
  }
  ...
  private boolean compareDatabaseFunctions(PSDatabaseFunctionsColl dbFuncsColl, boolean full) {
    ...
    while (it.hasNext()) {
      ...
      PSDatabaseFunction dbFunc = (PSDatabaseFunction) item.getValue();
      PSDatabaseFunction dbFuncOther = dbFuncsColl.getDatabaseFunction(dbFuncName);
      if (dbFuncOther == null) equals = false;
      else if (full && (!(dbFunc.equalsFull(dbFuncOther)))) equals = false;
      else if ((!full) && (!(dbFunc.equals(dbFuncOther)))) equals = false;
  ...
  private int getDatabaseFunctionDefsHashCode(boolean full) {
    ...
    while (it.hasNext()) {
      Map.Entry item = (Map.Entry) it.next();
      PSDatabaseFunction dbFunc = (PSDatabaseFunction) item.getValue();
      if (full) code += dbFunc.hashCodeFull();
      else code += dbFunc.hashCode();
    }
    return code;
  }
  ...
  public PSDatabaseFunction getDatabaseFunction(String dbFuncName) {
    if ((dbFuncName == null) || (dbFuncName.trim().length() < 1))
      throw new IllegalArgumentException("database function name may not be null or empty");
  ...
  public PSDatabaseFunctionDef getDatabaseFunctionDef(String dbFuncName, String driver) {
    ...
    if (dbFunc != null) dbFuncDef = dbFunc.getDatabaseFunctionDef(driver);
    return dbFuncDef;
  }
  ...
  public Iterator<PSDatabaseFunctionDef> getDatabaseFunctionsDef(String driver) {
    ...
    while (it.hasNext()) {
      ...
      if (dbFuncDef != null) funcList.add(dbFuncDef);
    }
  ...
  public boolean contains(PSDatabaseFunctionDef dbFuncDef) {
    ...
    if (dbFuncDef.getType() == m_type) {
      PSDatabaseFunction dbFunc = getDatabaseFunction(dbFuncDef.getName());
      if (dbFunc != null) contains = dbFunc.contains(dbFuncDef);
    }
    return contains;
  }
  ...
  public boolean remove(PSDatabaseFunctionDef dbFuncDef) {
    ...
    boolean remove = false;
    PSDatabaseFunction dbFunc = getDatabaseFunction(dbFuncDef.getName());
    if (dbFunc != null) remove = dbFunc.remove(dbFuncDef);
    return remove;
  }
