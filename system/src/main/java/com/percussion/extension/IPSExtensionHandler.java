...
/**
 * An extension handler is directly responsible for loading, storing, and managing extensions of a
 * particular type.
 *
 * <p>The implementation of an extension handler may actually be responsible for executing the
 * extensions, or it may instead create IPSExtension objects which can execute on their own.
 *
 * <p>Implementations are not required to be thread-safe with respect to installing, cataloging,
 * removing, or otherwise managing individual extensions. However, implementations must be aware
 * that multiple IPSExtension objects returned from prepare() may be executed in different threads.
 * That is, although multiple threads will not call prepare() simultaneously, prepare() may be
 * called several times for a single extension, and the returned IPSExtension object(s) may be used
...
public interface IPSExtensionHandler extends IPSExtension {
  ...
  public static final String INIT_PARAM_CONFIG_FILENAME = "com.percussion.extension.configFile";

  /**
   * TODO: add this doc elsewhere
   *
   * <p>Initializes this extension handler. The extension manager will call this method after
   ...
   *
   * @param def The extension def, containing configuration info and init params.
   ...
   *
   * @param context the context in which to search. Must not be <CODE>null</CODE>.
   ...
   * @param ref The name of an installed extension. Must not be <CODE>null</CODE>.
   * @throws PSNotFoundException If the extension does not exist.
   * @throws PSExtensionException If an error occurred while removing the extension.
   * @throws IllegalArgumentException If any param is invalid.
   */
  public void remove(PSExtensionRef ref) throws PSNotFoundException, PSExtensionException;
   ...
   *     was not well-formed.
   */
  public File getCodeBase(IPSExtensionDef def) throws PSExtensionException;
}
