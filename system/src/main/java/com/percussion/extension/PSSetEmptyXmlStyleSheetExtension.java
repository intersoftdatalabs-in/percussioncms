  ...
  public Document processResultDocument(Object[] params, IPSRequestContext rc, Document resultDoc)
    ...
    if ((resultDoc != null)
        && (resultDoc.getDocumentElement()
            != null)) { // if this has a root node, there's nothing more to do
    ...
    if (resultDoc == null) resultDoc = PSXmlDocumentBuilder.createXmlDocument();

    try {
      String styleSheet = params[0].toString();
      if (styleSheet == null) {
        throw new PSExtensionProcessingException(getClass().getName(), new NullPointerException());
      }
      ...
      try {
        ...
        if (rc.getRequestPageType() == IPSRequestContext.PAGE_TYPE_HTML)
          urlText =
              PSApplicationHandler.getLocalizedURL(rc.getCurrentApplicationName(), url)
                  .toExternalForm();
        else
          urlText =
              PSApplicationHandler.getExternalURLString(
                  rc.getCurrentApplicationName(), rc.getRequestRoot(), url);
