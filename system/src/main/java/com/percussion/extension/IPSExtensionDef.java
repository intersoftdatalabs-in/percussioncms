...
public interface IPSExtensionDef {
   ...
   *
   * @return An Iterator over one or more non-{@code null} Strings.
   * Never {@code null}.
   */
   ...
   *
   * @return {@code true} if the supplied interface is supported by
   * this extension, {@code false} otherwise.
   */
   ...
   * mean anything.
   *
   * @return An Iterator over zero or more non-{@code null} Strings.
   * Never {@code null}.
   */
   ...
   * parameter of the extension, or {@code null} if the parameter
   ...
   * it does not exist. This method will never return {@code null}
   ...
   * or {@code null} if no parameter by that name is used.
   *
   ...
   * or libraries, etc.) as well as any other resources used by the
   * extension.
   *
   * @return An Iterator over zero or more non-{@code null}
   ...
   * {@code null} if no resources have been supplied.
   */
  Iterator<java.net.URL> getSuppliedResources();

  /**
   * Sets the supplied resources for this extension.
   * @param resources A non-{@code null} but possibly empty Iterator of
   * URL objects.
   */
  void setSuppliedResources(Iterator<java.net.URL> resources);

  /**
   * Sets the list of extension applications that are required by this
   * extension. Every time this extension is loaded, all of the required
   * applications will be loaded as well.
   * <p>
   * This method may be called multiple times. Each call replaces the
   * previous data.
   * <p>
   * The applications are loaded in the order they are supplied. If an
   * application can't be loaded, a warning is logged and the process
   * continues. If an application is already loaded, it is skipped.
   * <p>
   * Circular dependencies are not detected. If they exist, the behavior
   * is undefined.
   * <p>
   * Applications are not automatically unloaded when this extension is
   * unloaded.
   *
   * @param apps An iterator over 0 or more PSExtensionRef objects. If
   * {@code null}, any existing settings are cleared. May be an empty
   * iterator.
   */
  void setRequiredApplications(Iterator<PSExtensionRef> apps);

  /**
   * Returns the list of applications that are required by this extension.
   * See {@link #setRequiredApplications(Iterator)} for more info.
   *
   * @return An iterator over 0 or more PSExtensionRef objects.
   * Never {@code null}, but may be empty.
   */
  Iterator<PSExtensionRef> getRequiredApplications();

  /**
   * Returns extension methods implemented by this extension.
   * This is used in jexl method extensions.
   *
   * @return An iterator over 0 or more PSExtensionMethod objects.
   * Never {@code null}, but may be empty.
   */
