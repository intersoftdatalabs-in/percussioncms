...
public interface IPSExtensionDef {
  ...
  public static final String INIT_PARAM_CLASSNAME = "className";

  /** The javascript version. */
  public static final String INIT_PARAM_JAVASCRIPT_VERSION = "version";

  /** The extension version. */
  public static final String INIT_PARAM_VERSION = "com.percussion.extension.version";
   ...
   */
  public static final String INIT_PARAM_FACTORY = "com.percussion.extension.factoryClassName";
   ...
   */
  public static final String INIT_PARAM_REENTRANT = "com.percussion.extension.reentrant";
   ...
   */
  public static final String INIT_PARAM_SCRIPTABLE = "com.percussion.user.scriptable";
   ...
   */
  public static final String INIT_PARAM_DESCRIPTION = "com.percussion.user.description";
  ...
  public static final String INIT_PARAM_SCRIPT_BODY = "scriptBody";
   ...
   *
   * @return An Iterator over one or more non-<CODE>null</CODE> Strings. Never <CODE>null</CODE>.
   */
   ...
   * @return <code>true</code> if the supplied interface is supported by this extension, <code>false
   *     </code> otherwise.
   */
   ...
   *
   * @return An Iterator over zero or more non-<CODE>null</CODE> Strings. Never <CODE>null</CODE>.
   */
  public Iterator getInitParameterNames();

  /**
   * Returns a String containing the value of the named initialization parameter of the extension,
   * or <CODE>null</CODE> if the parameter does not exist. All parameters beginning with
   ...
   * @return The value of the named parameter, or <CODE>null</CODE> if it does not exist. This
   *     method will never return <CODE>null</CODE> when called with a String value from the
   ...
   *
   * @return An Iterator over zero or more non-<CODE>null</CODE> String param names. The order of
   *     the parameter names is important and should be preserved.
   */
  public Iterator getRuntimeParameterNames();

  /**
   * Gets the parameter definition for the named runtime parameter, or <CODE>null</CODE> if no
   * parameter by that name is used.
   *
   ...
   *
   * <p><STRONG>Note</STRONG>: Depending on the type of extension defined, not all resources need to
   * be specified explicitly. For instance, some unspecified resources may be contained within
   * resources which are explicitly specified (such as files and subdirectories which are contained
   * within an explicitly specified directory, or Java .class files which are contained within an
   * explicitly specified .jar file, and so on).
   *
   * <p>In the current version of the product, each resource location must be a relative <CODE>file:
   * </CODE> URL whose parent directory will be determined at runtime by the extension handler.
   *
   * <p>In future versions, some extensions may be allowed to use absolute <CODE>file:</CODE> URLs
   * and/or other protocols (such as http). Handler implementations should verify that they
   * understand the locations of each specified resource before installing the extension.
   *
   * @return An Iterator over zero or more non-<CODE>null</CODE> URLs. Never <CODE>null</CODE>.
   */
  public Iterator getResourceLocations();

  /**
   * Returns the list of URL objects that point to the files supplied with this def when it was
   * installed or last updated.
   *
   * @return An Iterator over zero or more URL objects. May be <code>null</code> if this def does
   *     not contain such a list (backward compatibility.
   */
  public Iterator getSuppliedResources();

  /**
   * Sets the list of supplied resource files installed with this extension so their names and
   * locations will be saved with this definition.
   *
   * @param resources A non-<code>null</code> but possibly empty Iterator of URL objects.
   */
   ...
   *
   * @param isDeprecated If <code>true</code>, then this extension is flagged as having been
   *     deprecated. If <code>false</code>, then this extension is not deprecated.
   */
  ...
  /**
   * @return <code>true</code> indicates that if this extension call is followed by an error
   *     condition then the request parameters modified by this extension should be explicitly
   *     restored by calling PSRequest.restoreParams(), <code>false</code> otherwise.
   */
  public boolean isRestoreRequestParamsOnError();

  /**
   * Sets the list of the applications required by the implementation of this extension.
   *
   * @param apps An iterator over zero or more application names as <code>String</code> objects, may
   *     not be <code>null</code>.
   */
  public void setRequiredApplications(Iterator apps);

  /**
   * Get the list of required applications. See {@link #setRequiredApplications(Iterator)} of more
   * info.
   *
   * @return An iterator over zero or more application names as <code>String</code> objects, never
   *     <code>null</code>.
   */
   ...
   *
   * @param method the new method to add, not <code>null</code>. If a method with the same name
   *     already exists, it will be overridden. Otherwise the new method is added.
   */
   ...
   *
   * @param name the name or the extension method to be removed, not <code>null</code> or empty.
   *     Does nothing if no method exists for the supplied name.
   */
  ...
  /**
   * Returns an integer form of the "rhythmyx.extension.version" init property. If the property is
   * not specified or invalid, it will return 1 (one).
   *
