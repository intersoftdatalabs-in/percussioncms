...
public class PSExtensionClassLoader extends URLClassLoader {
  ...
  public void addURL(URL url) {
    if (!url.getProtocol().equals("file")) {
      throw new IllegalArgumentException("only file URLs are currently supported");
    }
  ...
  protected PermissionCollection getPermissions(CodeSource codeSource) {
    ...
    if (m_codeSource.implies(codeSource)) {
      perms.add(
          new FilePermission(m_rootDir.toString() + File.separator + "-", "read,write,delete"));
    }
   ...
   */
  private void initCodeSource(File file) throws MalformedURLException {
