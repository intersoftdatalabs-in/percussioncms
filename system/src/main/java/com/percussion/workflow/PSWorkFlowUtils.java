...
public class PSWorkFlowUtils {
  ...
  static {
    encryptProps = new String[] {"SMTP_PASSWORD"};
  ...
  private static synchronized void loadProperties() {
    try {
      ...
      File confFile =
          PSProperties.getConfig(ENTRY_NAME, FILE_PROPERTIES, PSServer.getRxFile(WORKFLOW_DIR));
  ...
  public static void printWorkflowMessage(IPSRequestContext request, String message) {
    ...
    printWorkflowMessage(request, message, useSysOut, usePSConsole);
  }
  ...
  public static void printWorkflowException(IPSRequestContext request, Throwable throwable) {
    ...
    printWorkflowException(request, throwable, useSysOut, usePSConsole);
  }
    ...
    while (sTokenizer.hasMoreElements()) {
      ...
      for (int i = 0; i < roleList.size(); i++) {
        ...
        if (sRole.trim().equalsIgnoreCase(sToken)) {
          if (!bPresent) bPresent = true;
          index = assignmentTypeList.get(i);
          if (index > nAssignmentType) nAssignmentType = index;
        }
      }
    }
  ...
  public static int modifyAssignmentType(
    ...
    if ((itemCommunity != usersCommunity) && (assignmentType > ASSIGNMENT_TYPE_READER))
  ...
  public static void setTransitionCommentInHTMLParams(String comment, Map<String, Object> params) {
    ...
    if (comment != null) params.put(getTransitionCommentParamName(), comment);
  }
    ...
    if (null == m_ResourceBundle)
      m_ResourceBundle =
          ResourceBundle.getBundle(
              "com.percussion.workflow." + "PSWorkFlowUtilsResources", Locale.getDefault());
  ...
  public static String getContentItemURL(int contentID, int revisionID, IPSRequestContext request)
      throws PSAuthenticationFailedException, PSInternalRequestCallException,
   ...
   */
  public static String getContentItemURL(
      int contentID, int revisionID, IPSRequestContext request, boolean overrideCommunity)
      throws PSAuthenticationFailedException, PSInternalRequestCallException,
          PSAuthorizationException {
    if (request == null) {
      throw new IllegalArgumentException("Request object must not be empty");
    }
    ...
    try {
      ...
      throw new IllegalArgumentException("Invalid contentID value supplied");
    }

    try {
      revisionid = Integer.toString(revisionID);
    } catch (Exception e) {
      throw new IllegalArgumentException("Invalid revisionID value supplied");
    }

    host = PSWorkFlowUtils.properties.getProperty("RX_SERVER_HOST_NOTIFICATION");
    ...
    portString = PSWorkFlowUtils.properties.getProperty("RX_SERVER_PORT_NOTIFICATION");
    ...
    String linkUrlComponent =
        PSWorkFlowUtils.properties.getProperty("MAIL_NOTIFY_LINK_COMPONENT_NAME");
    if (linkUrlComponent != null) {
      linkUrlComponent = linkUrlComponent.trim();
      if (linkUrlComponent.length() < 1) linkUrlComponent = null;
    }
    ...
    try {
      ...
      if (overrideCommunity) {
        ...
        String resource = IPSConstants.EDITOR_SUPPORT_APPNAME + "/contentstatus";
        iReq = request.getInternalRequest(resource, newHtmlParams, false);
        if (iReq != null) {
          ...
          if (rootStatusEl != null) {
            ...
            if (statusEl != null) communityId = statusEl.getAttribute("communityId");
          }
        }
      }
      ...
      if (communityId != null)
        newHtmlParams.put(IPSHtmlParameters.SYS_OVERRIDE_COMMUNITYID, communityId);

      iReq =
          request.getInternalRequest("sys_ComponentSupport/componentabslink", newHtmlParams, false);
      ...
      if (iReq != null) {
        ...
        if (PSServer.isRequestBehindProxy(null)) {
          ...
          editURL = new URL(proxyScheme, editURL.getHost(), editURL.getPort(), editURL.getFile());

        } else {
          if (isSSLEnabledForNotification()) {
            editURL = new URL("https", editURL.getHost(), editURL.getPort(), editURL.getFile());
          }
        }
  ...
  public static void sendMailWithAttachment(
    ...
    try {
      ...
      throw new EmailException("Failed to send email with attachment", e.fillInStackTrace());
    }
  }
  ...
  public static String trimmedOrNullString(String string) {
    ...
    return ((null == trimmedString) || trimmedString.length() == 0) ? null : trimmedString;
  }
  ...
  public static String listToDelimitedString(
    ...
    if (null == list || list.isEmpty()) {
      throw new IllegalArgumentException("List to be delimited may not be null or empty.");
    }
    ...
    while (iter.hasNext()) {
      delimitedStringBuilder
          .append(delimiter)
          .append(toStringHandleNull(iter.next(), stringForNull));
    }
  ...
  public static String emailListToDelimitedString(
    ...
    while (iter.hasNext()) {
      delimitedStringBuilder
          .append(delimiter)
          .append(toStringHandleNull(iter.next().getEmail(), stringForNull));
    }
    return delimitedStringBuilder.toString();
  }
  ...
  public static boolean isPublicState(IPSRequestContext request, Map params, boolean inherit)
    ...
    IPSInternalRequest ir = request.getInternalRequest(resource, params, inherit);
    if (ir != null) {
  ...
  public static boolean isInPublicState(IPSRequestContext request)
    ...
    String resource =
        PSRelationshipUtils.SYS_PSXRELATIONSHIPSUPPORT + "/" + PSRelationshipUtils.GET_CURRENTSTATE;

    IPSInternalRequest ir = request.getInternalRequest(resource, null, true);
  ...
  public static boolean isInPublicState(IPSRequestContext request, int contentid)
    ...
    if (contentid < 1) throw new IllegalArgumentException("contentid must be valid");
    ...
    IPSInternalRequest ir = request.getInternalRequest(resource, extraParams, false);

    if (ir != null) {
      Document doc = ir.getResultDoc();
      Element elem = doc.getDocumentElement();
      if (elem != null) return elem.getAttribute("isPublic").equalsIgnoreCase("y");
  ...
  public static boolean toPublicState(IPSRequestContext request)
    ...
    if (ir != null) {
      ...
      if (elem != null) {
        ...
        params.put(IPSHtmlParameters.SYS_WORKFLOWID, elem.getAttribute("wfId"));
        params.put(IPSConstants.DEFAULT_NEWSTATEID_NAME, elem.getAttribute("toStateId"));
  ...
  private static synchronized void storeProperty() {
    try {
      ...
      printWorkflowMessage(
          null, "Error saving the workflow properties file. Cannot found the file.");
    } catch (IOException e) {
  ...
  public static boolean isSSLEnabledForNotification() {
    return PSWorkFlowUtils.properties
        .getProperty("RX_SERVER_IS_SSLLINK_NOTIFICATION", "")
  ...
  private static PSWorkflow loadDefaultWorkflow(String wfName)
      throws com.percussion.services.error.PSNotFoundException {
  ...
  private static void encryptWorkflowProps() {
    ...
    File workflowProps =
        new File(PathUtils.getRxDir(null), WORKFLOW_PROPS_PATH + "/" + WORKFLOW_PROPS_FILE_NAME);
  ...
  private static boolean hasRolesActed(
    ...
    StringTokenizer tkn = new StringTokenizer(roleList, PSWorkFlowUtils.ROLE_DELIMITER);

    while (tkn.hasMoreTokens()) {
      String role = tkn.nextToken().trim();
      int roleId = PSWorkFlowUtils.getRoleIdFromMap(tc.getTransitionRoleNameIdMap(), role);
      ...
      if (!cac.hasRoleActed(roleId)) return false;
    }
    return true;
  }
  ...
  public static synchronized void logTestRunNotifications(IPSMailMessageContext messageContext) {
    try (BufferedWriter writer =
        new BufferedWriter(
            new FileWriter(
                PSServer.getRxDir().getAbsolutePath() + TEST_RUN_MODE_LOG_LOCATION, true))) {
