...
public class PSContentStatusContext implements IPSContentStatusContext {
  public PSContentStatusContext(Connection connection, int contentID)
    ...
    try {
      ...
      m_sCheckOutUserName =
          PSWorkFlowUtils.trimmedOrNullString(m_Rs.getString("CONTENTCHECKOUTUSERNAME"));
      m_sLastModifierName =
          PSWorkFlowUtils.trimmedOrEmptyString(m_Rs.getString("CONTENTLASTMODIFIER"));
      ...
      m_LastModifiedDate = PSWorkFlowUtils.sqlDateFromTimestamp(tmpTimestamp);
      tmpTimestamp = m_Rs.getTimestamp("LASTTRANSITIONDATE");
      m_LastTransitionDate = PSWorkFlowUtils.sqlDateFromTimestamp(tmpTimestamp);
      ...
      m_sCreatedByName = PSWorkFlowUtils.trimmedOrEmptyString(m_Rs.getString("CONTENTCREATEDBY"));
      ...
      if (m_Rs.wasNull()) {
        m_nCurrentRevision = IPSConstants.NO_CORRESPONDING_REVISION_VALUE;
      }
      ...
      String sYes = PSWorkFlowUtils.trimmedOrEmptyString(m_Rs.getString("REVISIONLOCK"));
      if (!m_Rs.wasNull() && sYes.trim().equalsIgnoreCase("Y")) {
        m_bRevisionLocked = true;
      }

      tmpTimestamp = m_Rs.getTimestamp("STATEENTEREDDATE");
      m_StateEnteredDate = PSWorkFlowUtils.sqlDateFromTimestamp(tmpTimestamp);
      ...
      m_RepeatedAgingTransitionStartDate = PSWorkFlowUtils.sqlDateFromTimestamp(tmpTimestamp);
  ...
  public String getContentCheckedOutUserName() {
    if (null == m_sCheckOutUserName) return "";
  ...
  private void setInt(
      PreparedStatement stmt, int index, int value, String columnName, Map<String, String> columns)
      throws SQLException {
  ...
  private void setDate(
      PreparedStatement stmt, int index, Date value, String columnName, Map<String, String> columns)
      throws SQLException {
  ...
  private void notifyUpdateItem(Map<String, String> columns) {
    PSTableChangeEvent event =
        new PSTableChangeEvent(
            IPSConstants.CONTENT_STATUS_TABLE, PSTableChangeEvent.ACTION_UPDATE, columns);
  ...
  public void commit(Connection conn) throws SQLException {
    ...
    try {
      ...
      setDate(
          stmt, ++nLoc, m_RepeatedAgingTransitionStartDate, REPEATEDAGINGTRANSSTARTDATE, columns);
  ...
  public String toString(boolean agingOnly) {
    ...
    try {
      ...
      if (null == getRepeatedAgingTransitionStartDate()) {
        ...
        buf.append(new Timestamp(getRepeatedAgingTransitionStartDate().getTime()));
      }
      ...
      return "PSContentStatusContext toString: Caught exception: " + e;
    }
    return buf.toString();
  }
