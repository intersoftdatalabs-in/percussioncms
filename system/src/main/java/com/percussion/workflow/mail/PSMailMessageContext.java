...
public class PSMailMessageContext implements IPSMailMessageContext, Serializable {
  ...
  public PSMailMessageContext(
      ...
      String smtpHost) {
    this(
        from, to, cc, subject, body, url, mailDomain, smtpHost, null, null, null, null, null, null);
  }
  ...
  public PSMailMessageContext(
      ...
      String smtpSSLPortNumber,
      String smtpBounceAddr) {
    if (to == null || to.trim().length() < 1) {
      throw new IllegalArgumentException(
          "Recipient's address must not be " + "empty or null in message context");
    }
    this.from = from;
    this.to = to;
    this.cc = cc;
