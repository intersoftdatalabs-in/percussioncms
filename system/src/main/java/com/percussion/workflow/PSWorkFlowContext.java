...
/**
 * This class provides a convenient way to bundle together information needed to process a workflow
 * content item.
 */
public class PSWorkFlowContext implements IPSWorkFlowContext {
   ...
   * @param revisionNum current revision number or revision number of item being checked out (must
   *     be > 0)
  ...
  public PSWorkFlowContext(
      int workflowID, int contentID, int revisionNum, int transitionID, int stateID) {
    if (workflowID > 0) {
      m_workflowID = workflowID;
    } else {
      throw new IllegalArgumentException(
          "PSWorkFlowContext: workflowID value " + workflowID + " is not valid\n");
    }
    ...
    if (contentID > 0) {
      ...
      throw new IllegalArgumentException(
          "PSWorkFlowContext: contentID value " + contentID + " is not valid\n");
    }
    ...
    if (revisionNum > 0) {
      ...
      throw new IllegalArgumentException(
          "PSWorkFlowContext: revisionNum value " + revisionNum + " is not valid\n");
    }

    if (transitionID >= 0 || IPSConstants.TRANSITIONID_NO_ACTION_TAKEN == transitionID) {
      m_transitionID = transitionID;
    } else {
      throw new IllegalArgumentException(
          "PSWorkFlowContext: transitionID value " + transitionID + " is not valid\n");
    }
    ...
    if (stateID >= 0) {
      ...
      throw new IllegalArgumentException(
          "PSWorkFlowContext: stateID value " + stateID + " is not valid\n");
    }
  ...
  private int m_workflowID = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;

  /** database ID of content item (must be > 0) */
  private int m_contentID = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;

  /**
   * database ID of content revision current revision number or revision number of item being
   * checked out (must be > 0)
   */
  private int m_revisionNum = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;

  /**
   * database ID of transition (must be > 0) or = IPSConstants.TRANSITIONID_CHECKINOUT or =
   * IPSConstants.TRANSITIONID_NO_ACTION_TAKEN
   */
  private int m_transitionID = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;

  /**
   * database ID of current/new content state (must be >= 0) (new state if a transition was
   * performed) = 0 for a checkin or checkout
   */
  private int m_stateID = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;

  /** database ID of content status history entry for this transition */
  private int m_historyID = IPSWorkFlowContext.WORKFLOW_CONTEXT_INITIAL_INTEGER_VALUE;
}
