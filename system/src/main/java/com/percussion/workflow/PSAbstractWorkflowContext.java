...
public abstract class PSAbstractWorkflowContext {
  ...
  public void getDataFromDataBase() throws SQLException {
    ...
    m_Statement = PSPreparedStatement.getPreparedStatement(m_Connection, m_sQueryString);
    ...
    if (m_Rs.next()) {
      throw new SQLException(
          "Multiple database entries found, when " + "the context should uniquely specify entry");
    }
  }
  ...
  protected void reinitializeDataMembers() {
    throw new UnsupportedOperationException(
        "This context does not support refreshing the context " + "from the database");
  ...
  protected static void setDate(PreparedStatement stmt, int index, Date date) throws SQLException {
    Timestamp tmpTimeStamp = null;
    tmpTimeStamp = PSWorkFlowUtils.timestampFromDate(date);
