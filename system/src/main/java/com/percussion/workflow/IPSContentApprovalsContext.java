...
/**
 * An interface that defines methods for the content approvals context, which provides methods for
 * accessing, adding and removing users that have approved content transitions .
 *
...
public interface IPSContentApprovalsContext {
   ...
   * @author Ram
   * @version 1.0
   * @return a list of names of the users approved that have approved the transition of this content
   *     item
   */
  ...
  /**
   * Checks whether a given user has any pending transition approvals on this content item from this
   * state.
   *
   * @author Aaron Brandes
   * @version 1.0
   * @param userName name of user to check for pending transition approvals
   * @return <CODE>true</CODE> if has a pending approval, else <CODE>false</CODE>
   */
  ...
  /**
   * Deletes all entries for approvals of this content item for transitions with the same "from
   * state" as the transition that is being approved. This method must be invoked once the
   * transition is done.
   *
   * @author Ram
   * @version 1.0
