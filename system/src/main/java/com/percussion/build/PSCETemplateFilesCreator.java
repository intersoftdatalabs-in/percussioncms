...
/**
 * This is a utility class to create the template files from all content editor applications. This
 * is meant to be used before manufacturing. It will go through all apps in the specified directory
 * and create the template xml files.
 */
public class PSCETemplateFilesCreator {
  ...
  /**
   * Creates the template files for all content editor application files exist in the application
   * subdirectories of specified <code>strAppDirectory
   * </code> directory. Assumes the application as a content editor application if the application
   * name has the string 'rx_ce'. The templates are created in a directory 'sys_psxTemplates' under
   * the specified <code>strAppDirectory
   * </code> directory. The name of the template file is genereated by appending the name after
   * 'rx_ce' to 'sys_'. <br>
   * For example if the application file name is 'rx_ceArticle', then the template file is
   * 'sys_Article'. <br>
   * Uses {@link com.percussion.tools.simple.PSCETemplateGenerator#createTemplate createTEmplate}
   * for creating templates. Uses the specified dtd file for validating the created template file.
   *
   * @param strAppDirectory Must not be <code>null</code> and must be a directory not a file. The
   *     applications directory which the editor applications reside. This directory needs to have
   *     subdirectories each having an editor application.
