...
public class HttpURLConnection extends java.net.HttpURLConnection
{
    ...
    * @exception ProtocolNotSuppException if the protocol is not supported
    */
   protected HTTPConnection getConnection(URL urlparam)
         throws ProtocolNotSuppException
   {
      // try the cache, using the host name

      String php = urlparam.getProtocol()
            + ":"
            + urlparam.getHost()
            + ":"
            + ((urlparam.getPort() != -1) ? urlparam.getPort() : URI.defaultPort(urlparam
                  .getProtocol()));
      php = php.toLowerCase();
   ...
   public synchronized void connect() throws IOException
   {
      ...
      synchronized (m_con)
      {
         ...
         if (do_redir)
            m_con.addModule(redir_mod, 2);
         else
            m_con.removeModule(redir_mod);

         try
         {
            if (output_stream instanceof ByteArrayOutputStream)
               resp = m_con.ExtensionMethod(method, resource,
