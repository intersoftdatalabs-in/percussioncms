 ...
 *
 * con.setDoOutput(true); con.setRequestProperty("Content-Length", ...);
...
public class HttpURLConnection extends java.net.HttpURLConnection {
  protected static ThreadLocal ms_connectionMap = new ThreadLocal();

  /** the current connection */
  protected HTTPConnection m_con;
  ...
  static {
    ...
    // false.
    try {
      if (Boolean.getBoolean("com.percussion.HTTPClient.HttpURLConnection.AllowUI"))
    ...
    try {
      redir_mod = Class.forName("com.percussion.HTTPClient.RedirectionModule");
    ...
    try {
      ...
      if (agent != null) setDefaultRequestProperty("User-Agent", agent);
    } catch (SecurityException se) {
    }
  }
  ...
  private Map getConnectionMap() {
    Map rval = (Map) ms_connectionMap.get();
    if (rval == null) {
      rval = new HashMap();
      ms_connectionMap.set(rval);
    }
    return rval;
  }
  ...
  public HttpURLConnection(URL url) throws ProtocolNotSuppException, IOException {
    ...
    try {
      ...
      if (!hosts.equalsIgnoreCase(non_proxy_hosts)) {
        ...
        for (int idx = 0; idx < list.length; idx++) HTTPConnection.dontProxyFor(list[idx]);
      }
   ...
   * @exception ProtocolNotSuppException if the protocol is not supported
   */
  protected HTTPConnection getConnection(URL urlparam) throws ProtocolNotSuppException {
    // try the cache, using the host name

    String php =
        urlparam.getProtocol()
            + ":"
            + urlparam.getHost()
            + ":"
            + ((urlparam.getPort() != -1)
                ? urlparam.getPort()
                : URI.defaultPort(urlparam.getProtocol()));
    php = php.toLowerCase();

    HTTPConnection con = (HTTPConnection) getConnectionMap().get(php);
    if (con != null) return con;

    // Not in cache, so create new one and cache it

    con = new HTTPConnection(urlparam);
    getConnectionMap().put(php, con);
  ...
  public String getHeaderFieldKey(int n) {
    if (hdr_keys == null) fill_hdr_arrays();

    if (n >= 0 && n < hdr_keys.length) return hdr_keys[n];
    else return null;
  }
  ...
  public String getHeaderField(int n) {
    if (hdr_values == null) fill_hdr_arrays();

    if (n >= 0 && n < hdr_values.length) return hdr_values[n];
    else return null;
  }
  ...
  public InputStream getInputStream() throws IOException {
    if (!doInput) throw new ProtocolException("Input not enabled! (use setDoInput(true))");
  ...
  public InputStream getErrorStream() {
    try {
      if (!doInput
          ...
          || resp.getHeaderAsInt("Content-length") <= 0) return null;
  ...
  public synchronized OutputStream getOutputStream() throws IOException {
    ...
    if (!doOutput) throw new ProtocolException("Output not enabled! (use setDoOutput(true))");
    if (!method_set) method = "POST";
    ...
    if (output_stream == null) {
      ...
      if (cl != null) output_stream = new HttpOutputStream(Integer.parseInt(cl.trim()));
      else {
        ...
        else output_stream = new HttpOutputStream();
      }

      if (output_stream instanceof HttpOutputStream) connect();
    }
  ...
  public void setRequestProperty(String name, String value) {
    Log.write(
        Log.URLC, "URLC:  (" + urlString + ") Setting request property: " + name + " : " + value);

    int idx;
    for (idx = 0; idx < headers.length; idx++) {
      if (headers[idx].getName().equalsIgnoreCase(name)) break;
    }

    if (idx == headers.length) headers = Util.resizeArray(headers, idx + 1);
  ...
  public String getRequestProperty(String name) {
    for (int idx = 0; idx < headers.length; idx++) {
      if (headers[idx].getName().equalsIgnoreCase(name)) return headers[idx].getValue();
    }
  ...
  public static void setDefaultRequestProperty(String name, String value) {
    ...
    if (idx == default_headers.length) default_headers = Util.resizeArray(default_headers, idx + 1);
  ...
  public synchronized void connect() throws IOException {
    ...
    synchronized (m_con) {
      m_con.setAllowUserInteraction(allowUserInteraction);
      if (do_redir) m_con.addModule(redir_mod, 2);
      else m_con.removeModule(redir_mod);

      try {
        if (output_stream instanceof ByteArrayOutputStream)
          resp =
              m_con.ExtensionMethod(
                  method, resource, ((ByteArrayOutputStream) output_stream).toByteArray(), headers);
        else
          resp = m_con.ExtensionMethod(method, resource, (HttpOutputStream) output_stream, headers);
      } catch (ModuleException e) {
        throw new IOException(e.toString());
      }
    }
  ...
  public boolean usingProxy() {
    return (m_con.getProxyHost() != null);
  }
