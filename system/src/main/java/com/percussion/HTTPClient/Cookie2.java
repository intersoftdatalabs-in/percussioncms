...
public class Cookie2 extends Cookie {
  ...
  protected static Cookie[] parse(String set_cookie, RoRequest req) throws ProtocolException {
    ...
    for (int idx = 0; idx < cookie_arr.length; idx++) {
      ...
      for (int idx2 = 0; idx2 < params.length; idx2++) {
        ...
        } else if (name.equals("domain")) // Domain
        {
          ...
          if (d.charAt(0) != '.' && !d.equals(curr.domain)) curr.domain = "." + d;
          else curr.domain = d;
      ...
      if (!Util.getPath(req.getRequestURI()).startsWith(curr.path)) {
        Log.write(
            Log.COOKI,
            "Cook2: Bad Set-Cookie2 header: "
                + set_cookie
                + "\n       path `"
                + curr.path
                + "' is not a prefix of the "
                + "request uri `"
                + req.getRequestURI()
                + "'");
        continue;
    ...
    if (cidx < cookie_arr.length) cookie_arr = Util.resizeArray(cookie_arr, cidx);
  ...
  protected String toExternalForm() {
    ...
    } else throw new Error("Internal Error: unknown version " + version);
  ...
  public String toString() {
    ...
    if (version == 1) {
      ...
      if (port_set) {
        ...
        for (int idx = 1; idx < port_list.length; idx++) res.append(',').append(port_list[idx]);
        res.append('\"');
      }
      if (expires != null)
      ...
      if (comment_url != null) res.append("; CommentURL=\"").append(comment_url).append('\"');
    } else throw new Error("Internal Error: unknown version " + version);
