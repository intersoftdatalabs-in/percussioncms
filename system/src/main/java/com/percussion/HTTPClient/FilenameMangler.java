...
public interface FilenameMangler {
  /**
   * This is invoked by {@link Codecs#mpFormDataDecode(byte[], java.lang.String, java.lang.String,
   * HTTPClient.FilenameMangler) Codecs.mpFormDataDecode} for each file found in the data, just
   * before the file is created and written. If null is returned then the file is not created or
   * written. This allows you to control which files are written and the names of the resulting
   * files.
   *
   * <p>For the mpFormDataEncode method, this is also invoked on each filename, allowing you to
   * control the actual name used in the <var>filename</var> attribute of the Content-Disposition
   * header. This does not change the name of the file actually read. If null is returned then the
   * file is ignored.
   *
