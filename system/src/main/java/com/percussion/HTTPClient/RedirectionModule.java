    ...
    switch (sts) {
      case 302: // General (temporary) Redirection (handle like 303)
        ...
        if (req.getMethod().equals("POST") || req.getMethod().equals("PUT")) {
          Log.write(
              Log.MODS,
              "RdirM: Received status: " + sts + " " + resp.getReasonLine() + " - treating as 303");
      ...
      case 306: // Switch Proxy
        ...
        if (level >= 15 || resp.getHeader("Location") == null) {
          if (level >= 15)
            Log.write(
                Log.MODS, "RdirM: not redirected because " + "of too many levels of redirection");
          else
            Log.write(
                Log.MODS, "RdirM: not redirected because " + "no Location header was present");
          ...
          if (sts == 303) {
            ...
            if (req.getStream() != null) {
              if (!HTTPConnection.deferStreamed) {
                Log.write(
                    Log.MODS,
                    "RdirM: status " + sts + " not handled - request " + "has an output stream");
                return RSP_CONTINUE;
              }
            ...
            if (sts == 301) {
              ...
              try {
                ...
                throw new Error(
                    "HTTPClient Internal Error: " + "unexpected exception '" + pe + "'");
              }
            }
          }
