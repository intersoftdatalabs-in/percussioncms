 ...
 *
 * <p>There are 7 fields which make up this class: host, port, scheme, realm, cookie, params, and
 * extra_info. The host and port select which server the info will be sent to. The realm is server
 * specified string which groups various URLs under a given server together and which is used to
 * select the correct info when a server issues an auth challenge; for schemes which don't use a
 * realm (such as "NTLM", "PEM", and "Kerberos") the realm must be the empty string (""). The scheme
 * is the authorization scheme used (such as "Basic" or "Digest").
 *
 * <p>There are basically two formats used for the Authorization header, the one used by the "Basic"
 * scheme and derivatives, and the one used by the "Digest" scheme and derivatives. The first form
 * contains just the the scheme and a "cookie":
 *
 * <PRE>    Authorization: Basic aGVsbG86d29ybGQ=</PRE>
 *
 * The second form contains the scheme followed by a number of parameters in the form of name=value
 * pairs:
 *
 ...
 * HREF="#toString()">toString()</A> is used by the AuthorizationModule when generating the
 * Authorization header and will format the info accordingly. Note that "cookie" and "params" are
 * mutually exclusive: if the cookie field is non-null then toString() will generate the first form;
 * otherwise it will generate the second form.
 *
 * <p>In some schemes "extra" information needs to be kept which doesn't appear directly in the
 * Authorization header. An example of this are the A1 and A2 strings in the Digest scheme. Since
 * all elements in the params field will appear in the Authorization header this field can't be used
 * for storing such info. This is what the extra_info field is for. It is an arbitrary object which
 * can be manipulated by the corresponding setExtraInfo() and getExtraInfo() methods, but which will
 * not be printed by toString().
 *
 * <p>The addXXXAuthorization(), removeXXXAuthorization(), and getAuthorization() methods manipulate
 * and query an internal list of AuthorizationInfo instances. There can be only one instance per
 * host, port, scheme, and realm combination (see <A HREF="#equals">equals()</A>).
 *
...
public class AuthorizationInfo implements Cloneable {
  ...
  /**
   * the scheme. (e.g. "Basic") Note: don't lowercase because some buggy servers use a
  ...
  /**
   * the string used for the "Basic", "NTLM", and other authorization schemes which don't use
   * parameters
   */
  ...
  /**
   * Creates a new info structure for the specified host and port with the specified scheme, realm,
   * params. The cookie is set to null.
   *
   * @param host the host
   * @param port the port
  ...
  public AuthorizationInfo(
      String host, int port, String scheme, String realm, NVPair params[], Object info) {
  ...
  /**
   * Creates a new info structure for the specified host and port with the specified scheme, realm
   * and cookie. The params is set to a zero-length array, and the extra_info is set to null.
   *
   * @param host the host
   * @param port the port
   * @param scheme the scheme
   * @param realm the realm
   * @param cookie for the "Basic" scheme this is the base64-encoded username/password; for the
   *     "NTLM" scheme this is the base64-encoded username/password message.
   */
  public AuthorizationInfo(String host, int port, String scheme, String realm, String cookie) {
    ...
    this.port = port;
    this.realm = realm;
    if (cookie != null) this.cookie = cookie.trim();
  ...
  /**
   * Set's the authorization handler. This handler is called whenever the server requests
   * authorization and no entry for the requested scheme and realm can be found in the list. The
   * handler must implement the AuthorizationHandler interface.
   *
   * <p>If no handler is set then a {@link DefaultAuthHandler default handler} is used. This handler
   * currently only handles the "Basic" and "Digest" schemes and brings up a popup which prompts for
   * the username and password.
   *
   * <p>The default handler can be disabled by setting the auth handler to <var>null</var>.
   *
  ...
  /**
   * Searches for the authorization info using the given host, port, scheme and realm. The context
  ...
  /**
   * Searches for the authorization info in the given context using the given host, port, scheme and
   * realm.
   *
   * @param host the host
   * @param port the port
   ...
   *
   * @param auth_info any info needed by the AuthHandler; at a minimum the host, scheme and realm
   *     should be set.
   * @param req the request which initiated this query
   * @param resp the full response
   * @return a structure containing the requested info, or null if either no AuthHandler is set or
   *     the user canceled the request.
   * @exception AuthSchemeNotImplException if this is thrown by the AuthHandler.
   */
  static AuthorizationInfo queryAuthHandler(
  ...
  /**
   * Searches for the authorization info using the host, port, scheme and realm from the given info
   * struct. If not found it queries the AuthHandler (if set).
   *
  ...
  /**
   * Searches for the authorization info given a host, port, scheme and realm. Queries the
   * AuthHandler if not found in list.
   *
   * @param host the host
   * @param port the port
   * @param scheme the scheme
   ...
   * @exception AuthSchemeNotImplException If thrown by the AuthHandler.
   */
  static AuthorizationInfo getAuthorization(
      String host,
      int port,
      String scheme,
      String realm,
  ...
  /**
   * Adds an authorization entry to the list using the default context. If an entry for the
   * specified scheme and realm already exists then its cookie and params are replaced with the new
   * data.
   *
  ...
  /**
   * Adds an authorization entry to the list. If an entry for the specified scheme and realm already
   * exists then its cookie and params are replaced with the new data.
   *
   * @param auth_info the AuthorizationInfo to add
  ...
  public static synchronized void addAuthorization(AuthorizationInfo auth_info, Object context) {
    ...
    if (old_info != null) {
      ...
      if (al == 0) auth_info.paths = old_info.paths;
      else {
  ...
  public static void addAuthorization(
      ...
      Object info) {
    addAuthorization(
        host, port, scheme, realm, cookie, params, info, HTTPConnection.getDefaultContext());
  }
  ...
  /**
   * Adds an authorization entry for the "Basic" authorization scheme to the list using the default
   * context. If an entry already exists for the "Basic" scheme and the specified realm then it is
   * overwritten.
   *
   * @param host the host
   * @param port the port
  ...
  public static void addBasicAuthorization(
      String host, int port, String realm, String user, String passwd) {
    addAuthorization(
        host,
        port,
        "Basic",
   ...
   */
  public static void addBasicAuthorization(
      String host, int port, String realm, String user, String passwd, Object context) {
    addAuthorization(
        host,
        port,
        "Basic",
  ...
  /**
   * Adds an authorization entry for the "Digest" authorization scheme to the list using the default
   * context. If an entry already exists for the "Digest" scheme and the specified realm then it is
   * overwritten.
   *
   * @param host the host
   * @param port the port
   ...
   * @param passwd the password
   */
  public static void addDigestAuthorization(
      String host, int port, String realm, String user, String passwd) {
  ...
  /**
   * Removes an authorization entry from the list using the default context. If no entry for the
  ...
  /**
   * Removes an authorization entry from the list. If no entry for the specified host, port, scheme
   * and realm exists then this does nothing.
   *
   * @param auth_info the AuthorizationInfo to remove
   * @param context the context this info is associated with
   */
  ...
  public static void removeAuthorization(
      String host, int port, String scheme, String realm, Object context) {
    removeAuthorization(
        new AuthorizationInfo(host, port, scheme, realm, (NVPair[]) null, null), context);
  }

  /**
   * Tries to find the candidate in the current list of auth info for the given request. The paths
   * associated with each auth info are examined, and the one with either the nearest direct parent
   * or child is chosen. This is used for preemptively sending auth info.
   *
   * @param req the Request
   * @return an AuthorizationInfo containing the info for the best match, or null if none found.
   */
  static synchronized AuthorizationInfo findBest(RoRequest req) {
    ...
    while (list.hasMoreElements()) {
      ...
      for (int idx = 0; idx < paths.length; idx++) {
        if (path.equals(paths[idx])) return info;
      }
    }
    ...
    while (list.hasMoreElements()) {
      ...
      for (int idx = 0; idx < paths.length; idx++) {
        ...
        if (base.equals(ibase)) return info;
  ...
  /**
   * Adds the path from the given resource to our path list. The path list is used for deciding when
   * to preemptively send auth info.
   *
  ...
  public synchronized void addPath(String resource) {
    ...
    for (int idx = 0; idx < paths.length; idx++) if (paths[idx].equals(path)) return;
  ...
  /**
   * Parses the authentication challenge(s) into an array of new info structures for the specified
   * host and port.
   *
   * @param challenge a string containing authentication info. This must have the same format as
   *     value part of a WWW-authenticate response header field, and may contain multiple
      ...
      if (sts == 401)
        ...
        curr =
            new AuthorizationInfo(
                req.getConnection().getProxyHost(), req.getConnection().getProxyPort());
  ...
  private static final Vector<NVPair> parseParams(
    ...
    while (true) {
      ...
      if (first
          && (end == len
              || buf[end] == '=' && (end + 1 == len || (buf[end + 1] == '=' && end + 2 == len)))) {
      ...
      if (beg < len && buf[beg] == '=') // we have a value
      {
        ...
        } else // it's a quoted-string
        {
          ...
          do end = challenge.indexOf('"', end + 1);
      ...
      if (param_name.equalsIgnoreCase("realm")) curr.realm = param_value;
      else params.addElement(new NVPair(param_name, param_value));
  ...
  /**
   * Constructs a string containing the authorization info. The format is that of the http
  ...
  /**
   * Produces a hash code based on host, scheme and realm. Port is not included for simplicity (and
   * because it probably won't make much difference). Used in the AuthorizationInfo.AuthList hash
   * table.
   *
  ...
  /**
   * Two AuthorizationInfos are considered equal if their host, port, scheme and realm match. Used
   * in the AuthorizationInfo.AuthList hash table.
   *
   * @param obj another AuthorizationInfo against which this one is to be compared.
  ...
  public boolean equals(Object obj) {
    if ((obj != null) && (obj instanceof AuthorizationInfo)) {
      ...
      if (host.equals(auth.host)
          ...
          && realm.equals(auth.realm)) return true;
    }
    return false;
  }
