...
class BufferedInputStream extends FilterInputStream {
  ...
  public int read(byte[] buf, int off, int len) throws IOException {
    ...
    if (pos >= end && len >= lr_thrshld && mark_pos < 0) return in.read(buf, off, len);
  ...
  private final void fillBuff() throws IOException {
    ...
    } else if (mark_pos == 0 && end < buffer.length) ; // pos == end, so we just fill what's left
    else pos = 0; // try to fill complete buffer

    // make sure our state is consistent even if read() throws InterruptedIOException
    end = pos;

    int got = in.read(buffer, pos, buffer.length - pos);
    if (got > 0) end = pos + got;
  }
  ...
  public int available() throws IOException {
    int avail = end - pos;
    if (avail == 0) return in.available();
  ...
  int pastEnd(byte[] search, int[] search_cmp) {
    ...
    if (idx == -1) mark_pos = (pos > search.length) ? pos - search.length : 0;
    else {
