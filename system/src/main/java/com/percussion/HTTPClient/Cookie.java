...
public class Cookie implements Serializable {
   ...
   */
  public Cookie(
      String name, String value, String domain, String path, Date expires, boolean secure) {
    ...
    while (true) // get all cookies
    {
      ...
      while (true) // parse all parts
      {
        ...
        if (name.equalsIgnoreCase("expires")) {
           ...
           * actually send that...
           */
          if (set_cookie.charAt(beg) == '\"') beg = Util.skipSpace(buf, beg + 1);
          ...
          int pos = beg;
          while (pos < len
              && (buf[pos] >= 'a' && buf[pos] <= 'z' || buf[pos] >= 'A' && buf[pos] <= 'Z')) pos++;
          pos = Util.skipSpace(buf, pos);
          if (pos < len && buf[pos] == ',' && pos > beg) beg = pos + 1;
        }
      ...
      } else Log.write(Log.COOKI, "Cooki: Ignoring cookie: " + curr);
    }
  ...
  private static boolean setAttribute(Cookie cookie, String name, String value, String set_cookie)
    ...
    } else if (name.equals("max-age")) // from rfc-2109
    {
      ...
      try {
        ...
        throw new ProtocolException(
            "Bad Set-Cookie header: " + set_cookie + "\nMax-Age '" + value + "' not a number");
      }
      ...
      // contain any dots
      String top = null;
      if (value.length() > 3) top = value.substring(value.length() - 4);
      if (top == null
      ...
      cookie.domain = value;
    } else if (name.equalsIgnoreCase("path")) cookie.path = value;
