...
/**
 * This is a simple logger for the HTTPClient. It defines a number of "facilities", each
 * representing one or more logically connected classes. Logging can be enabled or disabled on a per
 * facility basis. Furthermore, the logging output can be redirected to any desired place, such as a
 * file or a buffer; by default all logging goes to <var>System.err</var>.
 *
 * <p>All log entries are preceded by the name of the currently executing thread, enclosed in {}'s,
 * and the current time in hours, minutes, seconds, and milliseconds, enclosed in []'s. Example:
 * <tt>{Thread-5} [20:14:03.244] Conn: Sending Request</tt>
 *
 * <p>When the class is loaded, two java system properties are read: <var>HTTPClient.log.file</var>
 * and <var>HTTPClient.log.mask</var>. The first one, if set, causes all logging to be redirected to
 * the file with the given name. The second one, if set, is used for setting which facilities are
 * enabled; the value must be the bitwise OR ('|') of the values of the desired enabled facilities.
 * E.g. a value of 3 would enable logging for the HTTPConnection and HTTPResponse, a value of 16
 * would enable cookie related logging, and a value of 8 would enable authorization related logging;
 * a value of -1 would enable logging for all facilities. By default logging is disabled.
 *
...
public class Log {
  ...
  /**
   * Write the stack trace of the given exception to the current log if logging for the given
   * facility is enabled.
   *
   * @param facility the facility which is logging the message
  ...
  /**
   * Write the contents of the given buffer to the current log if logging for the given facility is
   * enabled.
   *
   * @param facility the facility which is logging the message
   * @param prefix the string with which to prefix the buffer contents; may be null
   ...
   *
   * @param facilities the facilities for which to enable or disable logging. This is bitwise OR
   *     ('|') of all the desired facilities; use {@link #ALL ALL} to affect all facilities
   * @param enable if true, enable logging for the chosen facilities; if false, disable logging for
   *     them.
   */
  @Deprecated
  ...
  /**
   * Set the writer to which to log. By default, things are logged to <var>System.err</var>.
   *
   * @param log the writer to log to; if null, nothing is changed
   * @param closeWhenDone if true, close this stream when a new stream is set again
   */
  @Deprecated
