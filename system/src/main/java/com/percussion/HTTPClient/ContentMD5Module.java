...
class ContentMD5Module implements HTTPClientModule {
  ...
  public void responsePhase3Handler(Response resp, RoRequest req)
    ...
    try {
      if (trailer != null) md5_tok = Util.hasToken(trailer, "Content-MD5");
    } catch (ParseException pe) {
      throw new ModuleException(pe.toString());
    }

    if ((md5_digest == null && !md5_tok) || resp.getHeader("Transfer-Encoding") != null) return;

    if (md5_digest != null)
      Log.write(Log.MODS, "CMD5M: Received digest: " + md5_digest + " - pushing md5-check-stream");
    else Log.write(Log.MODS, "CMD5M: Expecting digest in trailer " + " - pushing md5-check-stream");
...
class VerifyMD5 implements HashVerifier {
  ...
  public void verifyHash(byte[] hash, long len) throws IOException {
    String hdr;
    try {
      if ((hdr = resp.getHeader("Content-MD5")) == null) hdr = resp.getTrailer("Content-MD5");
    ...
    for (int idx = 0; idx < hash.length; idx++) {
      if (hash[idx] != ContMD5[idx])
        throw new IOException(
            "MD5-Digest mismatch: expected " + hex(ContMD5) + " but calculated " + hex(hash));
    }
