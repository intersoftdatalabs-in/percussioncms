...
public class Util {
      ...
      if (search[c1l] == str[beg + c1l]) {
        ...
        *  so here is the replacement: */
        if (search[c1f] == str[beg + c1f]) {
          ...
          if (same) break Find; // we found it
        }

        beg += d1;
      } else if (search[c2l] == str[beg + c2l]) beg += d2;
      else if (search[c3l] == str[beg + c3l]) beg += d3;
      else beg++;
    }

    if (beg + search.length > end) return -1;
    else return beg;
  }
  ...
  public static final String quoteString(String str, String qlist) {
    ...
    int idx;
    for (idx = 0; idx < list.length; idx++) if (str.indexOf(list[idx]) != -1) break;
    ...
    while (pos < len) {
      if (qlist.indexOf(buf[pos], 0) != -1) {
        if (len == buf.length) buf = Util.resizeArray(buf, len + str.length());
  ...
  public static final Vector<HttpHeaderElement> parseHeader(String header, boolean dequote)
    ...
    while (true) {
      ...
      while (end < len
          && !Character.isWhitespace(buf[end])
          && buf[end] != '='
          && buf[end] != ','
          && buf[end] != ';') end++;
    ...
    if (beg < len && buf[beg] == '"') // it's a quoted-string
    {
      ...
      while (end < len && buf[end] != '"') {
        if (buf[end] == '\\') {
          ...
          {
            if (deq_buf == null) deq_buf = new char[buf.length];
            System.arraycopy(buf, lst_pos, deq_buf, deq_pos, end - lst_pos);
            deq_pos += end - lst_pos;
            lst_pos = ++end;
          } else end++; // skip quoted char
        }
      ...
      } else value = new String(buf, beg, end - beg);
      end++;
  ...
  public static final boolean hasToken(String header, String token) throws ParseException {
    ...
    else return parseHeader(header).contains(new HttpHeaderElement(token));
  }
  ...
  public static final boolean sameHttpURL(URL url1, URL url2) {
    if (!url1.getProtocol().equalsIgnoreCase(url2.getProtocol())) return false;

    if (!url1.getHost().equalsIgnoreCase(url2.getHost())) return false;
  ...
  static final Date parseHttpDate(String dstr) {
    synchronized (http_parse_lock) {
      if (parse_1123 == null) setupParsers();
    }
  ...
  private static final void setupParsers() {
    parse_1123 =
        FastDateFormat.getInstance(
            "EEE, dd MMM yyyy HH:mm:ss 'GMT'", new SimpleTimeZone(0, "GMT"), Locale.US);
    parse_850 =
        FastDateFormat.getInstance(
            "EEEE, dd-MMM-yy HH:mm:ss 'GMT'", new SimpleTimeZone(0, "GMT"), Locale.US);
    parse_asctime =
        FastDateFormat.getInstance(
            "EEE MMM d HH:mm:ss yyyy", new SimpleTimeZone(0, "GMT"), Locale.US);
  }
  ...
  public static final String httpDate(Date date) {
    synchronized (http_format_lock) {
      if (http_format == null) setupFormatter();
    }
  ...
  static final String escapeUnsafeChars(String path) {
    ...
    if (dst > len) return new String(buf, 0, dst);
    else return path;
  }
  ...
  public static final String getParams(String resource) {
    ...
    return null;
    if (q == -1 && f == -1) return resource.substring(beg + 1);
    if (f == -1 || (q != -1 && q < f)) return resource.substring(beg + 1, q);
    else return resource.substring(beg + 1, f);
  }
  ...
  public static final String getQuery(String resource) {
    ...
    if (f == -1) return resource.substring(beg + 1); // no fragment
    else return resource.substring(beg + 1, f); // strip fragment
  }
  ...
  public static final String getFragment(String resource) {
    ...
    return null;
    else return resource.substring(beg + 1);
  }
  ...
  private static final boolean wildcardMatch(
      String pattern, String name, int ppos, int npos, int plen, int nlen) {
    // find wildcard
    ...
    // match prefix
    if (!pattern.regionMatches(ppos, name, npos, star - ppos)) return false;

    // match suffix
    if (star == plen - 1) return true;
    while (!wildcardMatch(pattern, name, star + 1, npos, plen, nlen) && npos < nlen) npos++;
    return (npos < nlen);
  }
}
