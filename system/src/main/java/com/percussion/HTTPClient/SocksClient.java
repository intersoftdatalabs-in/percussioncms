...
class SocksClient {
  ...
  /** socks commands */
  private static final byte CONNECT = 1, BIND = 2, UDP_ASS = 3;

  /** socks version 5 authentication methods */
  private static final byte NO_AUTH = 0, GSSAPI = 1, USERPWD = 2, NO_ACC = (byte) 0xFF;

  /** socks version 5 address types */
  private static final byte IP_V4 = 1, DMNAME = 3, IP_V6 = 4;
  ...
  private static final Socket connect(String host, int port, InetAddress localAddr, int localPort)
    ...
    for (int idx = 0; idx < addr_list.length; idx++) {
      try {
        if (localAddr == null) return new Socket(addr_list[idx], port);
        else return new Socket(addr_list[idx], port, localAddr, localPort);
      } catch (SocketException se) {
        if (idx < addr_list.length - 1) continue; // try next IP address
        else throw se; // none of them worked
      }
    }
  ...
  private void v4ProtExchg(InputStream inp, OutputStream out, String host, int port)
    ...
    if (!v4A) {
      ...
      if (v4A) Log.write(Log.SOCKS, "Socks: Switching to version 4A");
    }
    ...
    Log.write(
        Log.SOCKS,
        "Socks: Sending connect request for user " + new String(user, 0, user.length - 1));
    ...
    if (version == -1) throw new SocksException("Connection refused by server");
  ...
  private void v5ProtExchg(InputStream inp, OutputStream out, String host, int port)
    ...
    Log.write(
        Log.SOCKS,
        "Socks: Sending authentication request; methods" + " No-Authentication, Username/Password");
    ...
    switch (method) {
      ...
      case NO_ACC:
        throw new SocksException(
            "Server unwilling to accept any " + "standard authentication methods");
      default:
  ...
  private void negotiate_userpwd(InputStream inp, OutputStream out)
    ...
    Log.write(
        Log.SOCKS, "Socks: Entering authorization subnegotiation" + "; method: Username/Password");
    ...
    try {
      auth_info =
          AuthorizationInfo.getAuthorization(
              socks_host, socks_port, "SOCKS5", "USER/PASS", null, null, true);
    ...
    if (auth_info == null)
      throw new SocksException(
          "No Authorization info for SOCKS found " + "(server requested username/password).");
    ...
    if (version != 1)
      throw new SocksException(
          "Wrong version received in username/"
