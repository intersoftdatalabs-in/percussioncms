...
class StreamDemultiplexor implements GlobalConstants {
  ...
  RespInputStream getStream(Response resp) {
    ...
    if (resph != null) return resph.stream;
    else return null;
  }
  ...
  int read(byte[] b, int off, int len, ResponseHandler resph, int timeout) throws IOException {
    ...
    while ((head = (ResponseHandler) RespHandlerList.getFirst()) != null && head != resph) {
      try {
        ...
        if (ioe instanceof InterruptedIOException) throw ioe;
        else {
          resph.exception.fillInStackTrace();
          throw resph.exception;
        }
      ...
      try {
        ...
        switch (resph.resp.cd_type) {
          ...
          case CD_CONTLEN:
            ...
            if (resph.stream.count + rcvd == cl) close(resph);
          ...
          default:
            throw new Error(
                "Internal Error in StreamDemultiplexor: "
                    + "Invalid cd_type "
                    + resph.resp.cd_type);
        }
  ...
  synchronized int available(ResponseHandler resph) throws IOException {
    ...
    if (resph != null && resph.eof) return 0;
    ...
    switch (resph.resp.cd_type) {
      ...
      default:
        throw new Error(
            "Internal Error in StreamDemultiplexor: " + "Invalid cd_type " + resph.resp.cd_type);
    }
  }
  ...
  private void close(ResponseHandler resph) {
    synchronized (RespHandlerList) {
      if (resph != (ResponseHandler) RespHandlerList.getFirst()) return;
    ...
    if (resph == MarkedForClose) close(new IOException("Premature end of Keep-Alive"), false);
    else closeSocketIfAllStreamsClosed();
  }
  ...
  synchronized void markForClose(Response resp) {
    synchronized (RespHandlerList) {
      ...
      for (resph = (ResponseHandler) RespHandlerList.enumerate();
        ...
        if (MarkedForClose == resph) return; // already marked for closing after an earlier resp
  ...
  void abort() {
    ...
    synchronized (RespHandlerList) {
      for (ResponseHandler resph = (ResponseHandler) RespHandlerList.enumerate();
          resph != null;
          resph = (ResponseHandler) RespHandlerList.next()) {
        if (resph.resp.http_resp != null) resph.resp.http_resp.markAborted();
        if (resph.exception == null) resph.exception = new IOException("Request aborted by user");
      }
  ...
  public void run() {
    ...
    while (alive) {
      ...
      synchronized (time_list) {
        ...
        current++;
        if (current >= time_list.length) current = 0;
