...
public class URI {
  ...
  static {
    ...
    defaultPorts.put("shttp", Integer.valueOf(80));
    defaultPorts.put("http-ng", Integer.valueOf(80));
    defaultPorts.put("coffee", Integer.valueOf(80));
    defaultPorts.put("https", Integer.valueOf(443));
    defaultPorts.put("ftp", Integer.valueOf(21));
    defaultPorts.put("telnet", Integer.valueOf(23));
    defaultPorts.put("nntp", Integer.valueOf(119));
    defaultPorts.put("news", Integer.valueOf(119));
    defaultPorts.put("snews", Integer.valueOf(563));
    defaultPorts.put("hnews", Integer.valueOf(80));
    defaultPorts.put("smtp", Integer.valueOf(25));
    defaultPorts.put("gopher", Integer.valueOf(70));
    defaultPorts.put("wais", Integer.valueOf(210));
    defaultPorts.put("whois", Integer.valueOf(43));
    defaultPorts.put("whois++", Integer.valueOf(63));
    defaultPorts.put("rwhois", Integer.valueOf(4321));
    defaultPorts.put("imap", Integer.valueOf(143));
    defaultPorts.put("pop", Integer.valueOf(110));
    defaultPorts.put("prospero", Integer.valueOf(1525));
    defaultPorts.put("irc", Integer.valueOf(194));
    defaultPorts.put("ldap", Integer.valueOf(389));
    defaultPorts.put("nfs", Integer.valueOf(2049));
    defaultPorts.put("z39.50r", Integer.valueOf(210));
    defaultPorts.put("z39.50s", Integer.valueOf(210));
    defaultPorts.put("vemmi", Integer.valueOf(575));
    defaultPorts.put("videotex", Integer.valueOf(516));
    defaultPorts.put("cmp", Integer.valueOf(829));
  ...
  public URI(URI base, String rel_uri) throws ParseException {
    ...
    if (scheme == null) {
      if (base == null) throw new ParseException("No scheme found");
    ...
    type =
        usesGenericSyntax(final_scheme)
            ? GENERIC
    ...
    if (type == OPAQUE) {
      ...
      if (opaque.length() > 0 && opaque.charAt(0) == '/') opaque = "%2F" + opaque.substring(1);
      return;
    }
    ...
    if (pos + 1 < len && uri[pos] == '/' && uri[pos + 1] == '/') {
      pos += 2;
      idx = pos;
      while (idx < len && uri[idx] != '/' && uri[idx] != '?' && uri[idx] != '#') idx++;
    ...
    if (type == SEMI_GENERIC) {
      ...
      idx = pos;
      while (idx < len && uri[idx] != '?' && uri[idx] != '#') idx++;
    ...
    if (base != null) {
      if (scheme != null
          && // resolve scheme
          !(scheme.equals(base.scheme) && ENABLE_BACKWARDS_COMPATIBILITY)) return;
      scheme = base.scheme;
      ...
      if (path.length() == 0 || path.charAt(0) != '/') // relative path
      {
        ...
        if (idx < 0) path = '/' + path;
        else path = base.path.substring(0, idx + 1) + path;
  ...
  private void parse_authority(String authority, String scheme) throws ParseException {
    ...
    if (pos < (len - 1) && uri[pos] == ':') {
      ...
      if (p == defaultPort(scheme)) this.port = -1;
      else this.port = p;
    }
  }
  ...
  public URI(
    ...
    if (scheme == null) throw new ParseException("missing scheme");
    this.scheme = escape(scheme.trim().toLowerCase(), schemeChar, true);
    if (userinfo != null) this.userinfo = escape(userinfo.trim(), userinfoChar, true);
    if (host != null) {
      host = host.trim();
      this.host = isIPV6Addr(host) ? host : escape(host, hostChar, true);
    }
    if (port != defaultPort(scheme)) this.port = port;
    if (path != null) this.path = escape(path.trim(), escpdPathChar, true); // ???
    if (query != null) this.query = escape(query.trim(), escpdQueryChar, true);
    if (fragment != null) this.fragment = escape(fragment.trim(), escpdFragChar, true);
  ...
  public URL toURL() throws MalformedURLException {
    ...
    if (opaque != null) return (url = new URL(scheme + ":" + opaque));

    String hostinfo;
    if (userinfo != null && host != null) hostinfo = userinfo + "@" + host;
    else if (userinfo != null) hostinfo = userinfo + "@";
    else hostinfo = host;
  ...
  private final void assemblePath(
      StringBuilder buf, boolean printEmpty, boolean incFragment, boolean unescape) {
    if ((path == null || path.length() == 0) && printEmpty) buf.append('/');

    if (path != null) buf.append(unescape ? unescapeNoPE(path, resvdPathChar) : path);
  ...
  private final String stringify(boolean unescape) {
    ...
    if (host != null) {
      if (host.indexOf(':') < 0) uri.append(unescape ? unescapeNoPE(host, resvdHostChar) : host);
      else uri.append('[').append(host).append(']');
    }
  ...
  public boolean equals(Object other) {
    ...
    if (other instanceof URL) {
      ...
      if (userinfo != null) h = userinfo + "@" + host;
      else h = host;
  ...
  private static final boolean pathsEqual(String p1, String p2) {
    ...
    while (pos1 < end1 && pos2 < end2) {
      ...
      char ch;
      while (pos1 < end1 && (ch = p1.charAt(pos1)) != '/' && ch != ';') pos1++;
      while (pos2 < end2 && (ch = p2.charAt(pos2)) != '/' && ch != ';') pos2++;

      if (pos1 == end1 && pos2 < end2
          || pos2 == end2 && pos1 < end1
          || pos1 < end1 && pos2 < end2 && p1.charAt(pos1) != p2.charAt(pos2)) return false;

      if ((!p1.regionMatches(start1, p2, start2, pos1 - start1)
              || (pos1 - start1) != (pos2 - start2))
          && !unescapeNoPE(p1.substring(start1, pos1), null)
              .equals(unescapeNoPE(p2.substring(start2, pos2), null))) return false;
  ...
  public static char[] escape(char[] elem, BitSet allowed_char, boolean utf8) {
    ...
    for (int idx = 0; idx < elem.length; idx++) {
      if (!allowed_char.get(elem[idx])) {
        cnt += 2;
        if (utf8) {
          if (elem[idx] >= 0x0080) cnt += 3;
          if (elem[idx] >= 0x00800) cnt += 3;
          if ((elem[idx] & 0xFC00) == 0xD800
              && idx + 1 < elem.length
              && (elem[idx + 1] & 0xFC00) == 0xDC00) cnt -= 6;
        }
      }
    }
    ...
    for (int idx = 0, pos = 0; idx < elem.length; idx++) {
      char c = elem[idx];
      if (allowed_char.get(c)) tmp[pos++] = c;
      else if (utf8) {
  ...
  public static final String unescape(String str, BitSet reserved) throws ParseException {
    ...
    for (int sidx = 0; sidx < buf.length; sidx++) {
      if (buf[sidx] == '%') {
        int ch;
        try {
          if (sidx + 3 > buf.length) throw new NumberFormatException();
        ...
        if (utf_len > 0) {
          ...
          {
            if ((utf[0] & 0xE0) == 0xC0) ch = (utf[0] & 0x1F) << 6 | (ch & 0x3F);
            ...
            else if (utf_len < 4) res[didx++] = (char) ch;
            else {
              ch -= 0x10000;
        ...
        // check if this is the start of a utf-char
          ...
          if ((ch & 0xE0) == 0xC0) utf_len = 2;
          else if ((ch & 0xF0) == 0xE0) utf_len = 3;
          else utf_len = 4;
          utf[0] = (char) ch;
          utf_idx = 1;
        }
  ...
  private static final int copyBuf(
      char[] utf, int utf_idx, int ch, char[] res, int didx, BitSet reserved, boolean escapeAll) {
    if (ch >= 0) utf[utf_idx++] = (char) ch;

    for (int idx = 0; idx < utf_idx; idx++) {
      if (reserved != null && reserved.get(utf[idx]) || escapeAll) didx = enc(res, didx, utf[idx]);
      else res[didx++] = utf[idx];
    }
  ...
  public static void main(String args[]) throws Exception {
    ...
    testParser(base, "g#s/../x", "http://a/b/c/g#s/../x");
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:g", "http://a/b/c/g");
    else testParser(base, "http:g", "http:g");
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:", "http://a/b/c/d;p?q");
    else testParser(base, "http:", "http:");
    testParser(base, "./g:h", "http://a/b/c/g:h");
    ...
    testParser(base, "ftp:/h", "ftp:/h");
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "http:/h", "http://s/h");
    else testParser(base, "http:/h", "http:/h");
    ...
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:", "ldap://s");
    else testParser(base, "ldap:", "ldap:");
    testParser(base, "ldap://a", "ldap://a");
    testParser(base, "ldap://a/b", "ldap://a/b");
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:/b", "ldap://s/b");
    else testParser(base, "ldap:/b", "ldap:/b");
    ...
    testParser(base, "ftp:/h", "ftp:/h");
    if (ENABLE_BACKWARDS_COMPATIBILITY) testParser(base, "ldap:/h", "ldap://s/h");
    else testParser(base, "ldap:/h", "ldap:/h");
  ...
  private static void testEqual(String one, String two) throws Exception {
    ...
    if (u1.hashCode() != u2.hashCode()) {
      throw new Exception(
          "Test failed: " + nl + "  hashCode <" + one + "> != hashCode <" + two + ">");
    }
  }
