...
class RetryException extends IOException {
  ...
  /**
   * Constructs an RetryException with no detail message. A detail message is a String that
  ...
  /**
   * Constructs an RetryException class with the specified detail message. A detail message is a
  ...
  void addToListAfter(RetryException re) {
    ...
    if (re.next != null) this.next = re.next;
    re.next = this;
  }
}
