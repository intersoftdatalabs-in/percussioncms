...
final class RespInputStream extends InputStream implements GlobalConstants {
  ...
  public synchronized int read(byte[] b, int off, int len) throws IOException {
    ...
    if (buffer != null && !(left == 0 && interrupted)) {
      ...
      else rcvd = demux.read(b, off, len, resph, resph.resp.timeout);
      if (rcvd != -1 && resph.resp.got_headers) count += rcvd;
  ...
  public synchronized int available() throws IOException {
    ...
    if (buffer != null && !(end - offset == 0 && interrupted)) return end - offset;
    else return demux.available(resph);
  }
  ...
  public synchronized void close() throws IOException {
    if (!closed) {
      ...
      if (dont_truncate && (buffer == null || interrupted)) readAll(resph.resp.timeout);
