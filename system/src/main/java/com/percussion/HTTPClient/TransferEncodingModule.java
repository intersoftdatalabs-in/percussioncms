...
class TransferEncodingModule implements HTTPClientModule {
  ...
  public int requestHandler(Request req, Response[] resp) throws ModuleException {
    ...
    if (all != null) {
      ...
      try {
        ...
        throw new ModuleException(
            "Invalid q value for \"*\" in TE " + "header: " + nfe.getMessage());
      }
    }
    ...
    if (!pte.contains(new HttpHeaderElement("gzip"))) pte.addElement(new HttpHeaderElement("gzip"));
  ...
  public void responsePhase3Handler(Response resp, RoRequest req)
    ...
    if (pte.size() > 0) resp.setHeader("Transfer-Encoding", Util.assembleHeader(pte));
