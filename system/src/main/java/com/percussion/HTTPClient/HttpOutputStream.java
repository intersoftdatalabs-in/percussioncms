...
/**
 * This class provides an output stream for requests. The stream must first be associated with a
 * request before it may be used; this is done by passing it to one of the request methods in
 ...
 * </PRE>
 *
 * <p>There are two constructors for this class, one taking a length parameter, and one without any
 * parameters. If the stream is created with a length then the request will be sent with the
 * corresponding Content-length header and anything written to the stream will be written on the
 * socket immediately. This is the preferred way. If the stream is created without a length then one
 * of two things will happen: if, at the time of the request, the server is known to understand
 * HTTP/1.1 then each write() will send the data immediately using the chunked encoding. If,
 * however, either the server version is unknown (because this is first request to that server) or
 * the server only understands HTTP/1.0 then all data will be written to a buffer first, and only
 * when the stream is closed will the request be sent.
 *
 * <p>Another reason that using the <var>HttpOutputStream(length)</var> constructor is recommended
 * over the <var>HttpOutputStream()</var> one is that some HTTP/1.1 servers do not allow the chunked
 * transfer encoding to be used when POSTing to a cgi script. This is because the way the cgi API is
 * defined the cgi script expects a Content-length environment variable. If the data is sent using
 * the chunked transfer encoding however, then the server would have to buffer all the data before
 * invoking the cgi so that this variable could be set correctly. Not all servers are willing to do
 * this.
 *
 * <p>If you cannot use the <var>HttpOutputStream(length)</var> constructor and are having problems
 * sending requests (usually a 411 response) then you can try setting the system property
 * <var>HTTPClient.dontChunkRequests</var> to <var>true</var> (this needs to be done either on the
 * command line or somewhere in the code before the HTTPConnection is first accessed). This will
 * prevent the client from using the chunked encoding in this case and will cause the
 * HttpOutputStream to buffer all the data instead, sending it only when close() is invoked.
 *
 * <p>The behaviour of a request sent with an output stream may differ from that of a request sent
 * with a data parameter. The reason for this is that the various modules cannot resend a request
 * which used an output stream. Therefore such things as authorization and retrying of requests
 * won't be done by the HTTPClient for such requests. But see {@link HTTPResponse#retryRequest()
...
public class HttpOutputStream extends OutputStream {
  ...
  /**
   * Creates an output stream of unspecified length. Note that it is <strong>highly</strong>
   * recommended that this constructor be avoided where possible and <code>HttpOutputStream(int)
   * </code> used instead.
   *
  ...
  /**
   * This creates an output stream which will take <var>length</var> bytes of data.
   *
   * @param length the number of bytes which will be sent over this stream
   */
  public HttpOutputStream(int length) {
    if (length < 0) throw new IllegalArgumentException("Length must be greater equal 0");
    this.length = length;
  }
  ...
  /**
   * Setup this stream to dump the data to the great bit-bucket in the sky. This is needed for when
   * a module handles the request directly.
   *
   * @param req the request this stream is to be associated with
   */
  ...
  /**
   * Return the response we got from sendRequest(). This waits until the request has actually been
   * sent.
   *
  ...
  /**
   * Sets the trailers to be sent if the output is sent with the chunked transfer encoding. These
   * must be set before the output stream is closed for them to be sent.
   *
   * <p>Any trailers set here <strong>should</strong> be mentioned in a <var>Trailer</var> header in
   * the request (see section 14.40 of draft-ietf-http-v11-spec-rev-06.txt).
   *
   * <p>This method (and its related <code>getTrailers()</code>)) are in this class and not in
   * <var>Request</var> because setting trailers is something an application may want to do, not
   * only modules.
   *
   * @param trailers an array of header fields
   */
  public void setTrailers(NVPair[] trailers) {
    if (trailers != null) this.trailers = trailers;
    else this.trailers = empty;
  }

  /**
   * Reset this output stream, so it may be reused in a retried request. This method may only be
   * invoked by modules, and <strong>must</strong> never be invoked by an application.
   */
  public void reset() {
    rcvd = 0;
    req = null;
  ...
  /**
   * Writes a single byte on the stream. It is subject to the same rules as <code>
   * write(byte[], int, int)</code>.
   *
