...
class AuthorizationModule implements HTTPClientModule {
  ...
  public int requestHandler(Request req, Response[] resp)
    ...
    if (out != null && deferred_auth_list.get(out) != null) {
      ...
      if (auth_sent != null)
        ...
        Log.write(
            Log.AUTH, "AuthM: Sending request with " + "Proxy-Authorization '" + prxy_sent + "'");
      ...
      if (auth_sent != null && auth_from_4xx) {
        try {
          AuthorizationInfo authInfo =
              AuthorizationInfo.getAuthorization(auth_sent, req, resp, false);
          if (authInfo != null) authInfo.addPath(req.getRequestURI());
        } catch (AuthSchemeNotImplException asnie) {
          /* shouldn't happen */
        }
      }
  ...
  public int responsePhase2Handler(Response resp, Request req)
    ...
    if (h != null) h.handleAuthHeaders(resp, req, auth_sent, prxy_sent);
      ...
      case 407: // Proxy Authentication Required
        ...
        if (req.getStream() != null) {
          if (!HTTPConnection.deferStreamed) {
            Log.write(
                Log.AUTH,
                "AuthM: status " + sts + " not handled - request has " + "an output stream");
            return RSP_CONTINUE;
          }
        ...
        if (auth_sent != null || prxy_sent != null) {
          ...
          if (auth_sent != null)
            Log.write(
                Log.AUTH, "AuthM: Resending request " + "with Authorization '" + auth_sent + "'");
          else
            Log.write(
                Log.AUTH,
                "AuthM: Resending request " + "with Proxy-Authorization '" + prxy_sent + "'");
        ...
        if (req.getStream() != null)
          Log.write(
              Log.AUTH,
              "AuthM: status " + sts + " not " + "handled - request has an output " + "stream");
        else Log.write(Log.AUTH, "AuthM: No Auth Info found - " + "status " + sts + " not handled");
  ...
  private void handle_auth_challenge(Request req, Response resp)
    ...
    int[] idx_arr = {
      auth_lst_idx, // hack to pass by ref
      auth_scm_idx
    };
    auth_sent =
        setAuthHeaders(
            resp.getHeader("WWW-Authenticate"), req, resp, "Authorization", idx_arr, auth_sent);
    if (auth_sent != null) {
      auth_from_4xx = true;
    ...
    if (prxy_sent != null) {
      HTTPConnection con = req.getConnection();
      Util.getList(proxy_cntxt_list, con.getContext())
          .put(con.getProxyHost() + ":" + con.getProxyPort(), prxy_sent);
    }
    ...
    if (auth_sent == null
      ...
      else throw new ProtocolException("Missing Proxy-Authenticate header");
    }
  }
   ...
   */
  private AuthorizationInfo setAuthHeaders(
      String auth_str,
      Request req,
      RoResponse resp,
      String header,
    ...
    for (auth_idx = 0; auth_idx < hdrs.length; auth_idx++) {
      if (hdrs[auth_idx].getName().equalsIgnoreCase(header)) break;
    }
