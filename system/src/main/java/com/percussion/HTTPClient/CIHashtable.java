...
final class CIHashtableEnumeration implements Enumeration<Object> {
  ...
  public Object nextElement() {
    ...
    if (tmp instanceof CIString) return ((CIString) tmp).getString();
...
final class CIString {
  ...
  private static final int calcHashCode(String str) {
    ...
    for (int idx = 0; idx < len; idx++) hash = 31 * hash + llc[str.charAt(idx)];
  ...
  public boolean equals(Object obj) {
    if (obj != null) {
      ...
      if (obj instanceof String) return string.equalsIgnoreCase((String) obj);
    }
  ...
  static {
    // just ISO-8859-1
    for (char idx = 0; idx < 256; idx++) lc[idx] = Character.toLowerCase(idx);
  }
}
