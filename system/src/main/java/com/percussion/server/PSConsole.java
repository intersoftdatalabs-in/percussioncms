...
public class PSConsole /* extends Thread */ {
  ...
  public static void printInfoMsg(
    ...
    getLogger(subsystem).info(PSErrorManager.createMessage(errorCode, errorArgs));
  }
  ...
  public static void printMsg(String subsystem, String message, String[] subMessages, Level level) {
    ...
    if (subMessages == null) l.info("{}", message);
    else {
  ...
  public static void printMsg(String subsystem, Throwable t) {
    ...
    if (errorMessage == null || errorMessage.trim().length() == 0) errorMessage = t.getMessage();
  ...
  public static void printMsg(String subsystem, Throwable t, String message) {
    ...
    Logger l = getLogger(subsystem);
    if (ms_printCallStacks) l.error("{}", logError);
    else l.error(message);
  }
  ...
  public static String getUserInput(boolean noWait) {
    try {
      if (!noWait || m_inBuf.ready()) return m_inBuf.readLine();
    } catch (IOException e) {
  ...
  public static void setOutput(PrintStream out) throws IllegalArgumentException {
    if (out == null) throw new IllegalArgumentException("PSConsole output must be a valid stream");
  ...
  private static final Logger getLogger(String subsystem) {
    ...
    if (!subsystem.startsWith("com.percussion.")) subsystem = "com.percussion." + subsystem;
  ...
  class PSConsoleCommandRequest extends PSRequest {
    PSConsoleCommandRequest() {
      super();
      throw new RuntimeException("Commands are not supported within an" + " application server");
    }
  }
