...
public class PSRequestPageMap {
  ...
  public PSRequestPageMap(PSDataSet dataSet, IPSRequestHandler rh)
    ...
    if ((conditionals == null) || (conditionals.size() == 0)) m_SelectionCriteriaChecker = null;
    else m_SelectionCriteriaChecker = new PSConditionalEvaluator(conditionals);
    ...
    if (mimeMap != null) {
      ...
      while (i.hasNext()) m_extensionsSupported.add(i.next());
    }
        ...
        for (int i = 0; i < pageCollection.size(); i++) {
          ...
          if (pageExtensions != null) {
            ...
            while (pageIt.hasNext()) {
              ...
              if (!m_extensionsSupported.contains(o)) m_extensionsSupported.add(o);
            }
          }
        }
      }
  ...
  public boolean isMatch(PSApplicationHandler ah, PSRequest request) {
    if (!m_reqPage.equalsIgnoreCase(request.getRequestPage())
        && !m_reqPage.equalsIgnoreCase(request.getRequestPage(false))) return false;
    ...
    if (m_SelectionCriteriaChecker == null) return true;

    return m_SelectionCriteriaChecker.isMatch(new PSExecutionData(ah, null, request));
  }
