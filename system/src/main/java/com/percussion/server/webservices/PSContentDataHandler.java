...
public class PSContentDataHandler extends PSSearchHandler implements IPSContentDataHandler {
  ...
  void openItemAction(PSRequest request, Document parent) throws PSException {
    ...
    if (checkOut) executeCheckInOut(request, IPSConstants.TRIGGER_CHECKOUT);
    ...
    respEl.appendChild(
        theItem.toMinXml(parent, true, includeChildren, includeRelated, includeBinary));
  }
  ...
  private PSServerItem openItem(PSRequest request, boolean includeBinary) throws PSException {
    ...
    PSItemDefinition itemDef = mgr.getItemDef(contentTypeId, request.getSecurityToken());

    String id = SecureStringUtils.srp(request.getParameter(IPSHtmlParameters.SYS_CONTENTID));

    String revision = SecureStringUtils.srp(request.getParameter(IPSHtmlParameters.SYS_REVISION));

    if (!NumberUtils.isCreatable(id) || !NumberUtils.isCreatable(revision)) {
      throw new PSException("Not a valid id.");
    }

    PSLocator loc = new PSLocator(id, revision);
  ...
  public void newCopy(PSRequest request) throws PSException {
    ...
    request.setParameter(
        IPSHtmlParameters.SYS_CONTENTID, newReq.getParameter(IPSHtmlParameters.SYS_CONTENTID));
    request.setParameter(
  ...
  public static void purgeItems(PSRequest request) throws PSException, PSValidationException {
    ...
    if (idList != null) {
      for (Object idObj : idList) {
        ...
        if (id > 0) locList.add(new PSLocator(id));
      }
    }
  ...
  public static void purgeItems(PSRequest request, List<String> itemIds)
      throws PSException, PSValidationException {
    if (request == null) throw new IllegalArgumentException("request may not be null.");
    if (itemIds == null) throw new IllegalArgumentException("itemIds may not be null.");

    if (itemIds.isEmpty()) return; // do nothing

    IPSSqlPurgeHelper purgeHelper = PSSqlPurgeHelperLocator.getPurgeHelper();
    List<PSLocator> locatorList = new ArrayList<>();
    for (String item : itemIds) {
      int id = NumberUtils.toInt(item);
      if (id > 0) locatorList.add(new PSLocator(id));
    }
  ...
  void updateItemAction(PSRequest request, Document parent) throws PSException {
    ...
    request.setParameter(IPSHtmlParameters.SYS_CONTENTID, "" + newLoc.getId());
    request.setParameter(IPSHtmlParameters.SYS_REVISION, "" + newLoc.getRevision());

    if (checkIn) executeCheckInOut(request, IPSConstants.TRIGGER_CHECKIN);
  }
   ...
   */
  @Override
  public PSServerItem updateItem(PSRequest request, Element item, PSLocator loc, long typeId)
      throws PSException {
  ...
  private void processInsertItemAction(PSRequest request, Document parent) throws PSException {
    ...
    String contentType = PSXMLDomUtil.checkAttribute(item, ATTR_CONTENTTYPE, true);
