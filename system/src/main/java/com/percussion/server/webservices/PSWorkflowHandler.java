...
class PSWorkflowHandler extends PSWebServicesBaseHandler {
  ...
  void transitionItemAction(PSRequest request, Document parent) throws PSException {
    ...
    while (el != null) {
      ...
      if (name.equals(EL_COMMENT)) comment = PSXMLDomUtil.getElementData(el);

      if (name.equals(EL_ADHOCUSERS)) adhocList += ";" + PSXMLDomUtil.getElementData(el);

      el = PSXMLDomUtil.getNextElementSibling(el);
    }

    // skip the initial ';'
    if (adhocList.trim().length() > 1) adhocList = adhocList.substring(1);
