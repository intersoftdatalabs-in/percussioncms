...
public interface IPSSearchHandler extends IPSPortActionHandler {
  PSWSSearchResponse search(PSRequest request, PSWSSearchRequest searchRequest) throws PSException;

  List<Integer> searchAndGetContentIds(PSRequest request, PSWSSearchRequest searchReq)
      throws PSException;

  List<Integer> searchAndGetContentIdsForSearchByStatus(
  ...
  public static class PSExternalInValuesIterator implements Iterator<String> {
    ...
    private void getNext() {
      ...
      while (mi_toker.hasMoreTokens()) {
        ...
        if (token.trim().equalsIgnoreCase("or")) continue;
        next = token;
        break;
      }
      mi_next = next;
