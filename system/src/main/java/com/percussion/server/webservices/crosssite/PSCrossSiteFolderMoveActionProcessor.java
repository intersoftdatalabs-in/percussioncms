...
public class PSCrossSiteFolderMoveActionProcessor extends PSCrossSiteFolderActionProcessor {
  ...
  public PSCrossSiteFolderMoveActionProcessor(
    ...
    if (sites.length == 0) theData.setTargetSiteId(null);
    else {
      if (sites.length > 1) {
        log.warn(
            "folder with folderid = {}",
            theData.getTargetFolderId().getId() + "resolves to multiple sites: {}",
            sites);
    ...
    log.debug(
        "The move action category based on the data is: " + theData.getActionCategory().toString());
    buildDescendents();
  }
  ...
  protected boolean skipFolder(PSLocator folder, PSRelationshipSet rs, int depth) {
    if (getData().getActionCategory() == PSMoveActionCategoryEnum.ACTION_CATEGORY_REORGANIZE) {
      return true;
    }
    return false;
  }
  ...
  public void modifyLinks(PSAaFolderDependent depItem) throws PSCmsException {
    log.debug(
        "Modifying AA relationships for the dependent item: " + depItem.getItem().getId() + "...");
    ...
    PSLocator targetFolderid =
        depItem.isGrandChild() ? depItem.getSrcFolder() : data.getTargetFolderId();
    //
    while (iter.hasNext()) {
        ...
        case ACTION_CATEGORY_REORGANIZE:
          switch (getCrossSiteLinkType(rel)) {
            ...
            case CROSSSITE_LINK_FOLDER_ONLY:
              setFolderId(rel, depItem, String.valueOf(targetFolderid.getId()));
              break;
            default:
              break;
          }
  ...
  private boolean setSiteId(PSRelationship rel) {
    ...
    String srcSiteId = (data.getSourceSiteId() == null) ? "" : data.getSourceSiteId().toString();
  ...
  private PSMoveActionCategoryEnum evaluateActionCategory() {
    ...
    if (data.getSourceSiteId() == null) return PSMoveActionCategoryEnum.ACTION_CATEGORY_DEPLOY;
