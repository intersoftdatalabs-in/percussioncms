...
public abstract class PSCrossSiteFolderActionProcessor {
   ...
   */
  protected PSCrossSiteFolderActionProcessor(PSLocator sourceFolderId, List<PSLocator> children)
    ...
    for (int i = 0; i < children.size(); i++) log.debug(children.get(i).getId());
    //
    m_folderProc = PSServerFolderProcessor.getInstance();
    ...
    if (sites.length == 0) theData.setSourceSiteId(null);
    else {
      if (sites.length > 1) {
        log.warn(
            "folder with folderid = {}, resolves to multiple sites: {}",
            theData.getSourceFolderId().getId(),
            sites);
  ...
  private void fillAaRels() throws PSCmsException {
    ...
    log.debug(
        "Total number of child items including folders is: {}", data.getDependentItems().size());
  ...
  private void fillAaRels(PSAaFolderDependent depItem) throws PSCmsException {
    log.debug(
        "Collecting cross site AA relationships for the dependent item with id: {}",
    ...
    while (iter.hasNext()) {
      ...
      if (getCrossSiteLinkType(aaRel).getOrdinal()
          == PSCrossSiteLinkTypeEnum.CROSSSITE_LINK_NONE.getOrdinal()) {
        iter.remove();
      }
    }
  ...
  private Integer[] getSiteIdsFromPaths(String[] paths) {
    ...
    for (int i = 0; i < paths.length; i++) {
      if (!paths[i].endsWith("/")) newPaths[i] = paths[i] + "/";
      else newPaths[i] = paths[i];
    }
    List<IPSSite> allSites = getAllSites();
    for (int i = 0; i < allSites.size(); i++) {
      IPSSite site = allSites.get(i);
      String siteRootPath = site.getFolderRoot();
      if (!siteRootPath.endsWith("/")) siteRootPath = siteRootPath + "/";
      for (int j = 0; j < newPaths.length; j++) {
        if (newPaths[j].startsWith(siteRootPath)) siteIds.add(site.getGUID().getUUID());
      }
    }
  ...
  private void getDescendents(
      ...
      List<PSAaFolderDependent> allFolderChildren,
      int depth)
      throws PSCmsException {
    for (int i = 0; i < children.size(); i++) {
      PSLocator loc = children.get(i);
      if (!m_folderProc.isItemFolder(loc)) {
        PSAaFolderDependent depItem = new PSAaFolderDependent(loc, folder);
        ...
        PSRelationshipSet rs =
            processor.getDependents(
                ...
                loc,
                PSRelationshipConfig.FILTER_TYPE_NONE);
        if (rs.isEmpty() || skipFolder(loc, rs, depth)) continue;
  ...
  public PSCrossSiteLinkTypeEnum getCrossSiteLinkType(PSRelationship rel) {
    boolean isSiteid = !StringUtils.isBlank(rel.getProperty(IPSHtmlParameters.SYS_SITEID));
    boolean isFolderid = !StringUtils.isBlank(rel.getProperty(IPSHtmlParameters.SYS_FOLDERID));

    if (isSiteid && isFolderid) return PSCrossSiteLinkTypeEnum.CROSSSITE_LINK_BOTH;

    if (isSiteid && !isFolderid) return PSCrossSiteLinkTypeEnum.CROSSSITE_LINK_SITE_ONLY;

    if (!isSiteid && isFolderid) return PSCrossSiteLinkTypeEnum.CROSSSITE_LINK_FOLDER_ONLY;
  ...
  public void saveLinks() throws PSCmsException {
    ...
    if (data.getProcessorStatus().getOrdinal()
        < ProcessorStatusEnum.PROCESSOR_STATUS_PROCESSED.getOrdinal()) {
      throw new RuntimeException("Cannot be saved before processing the links");
    }
    ...
    while (itemIter.hasNext()) {
      ...
      if (!depItem.isActionSuccess()) {
        if (log.isDebugEnabled()) {
          log.debug(
              "Skipping saving relationships for the item with id: {}, as the action failed",
              depItem.getItem().getId());
        }
        continue;
      }
  ...
  private void markUnsuccessfulItems() throws PSCmsException {
    ...
    boolean move = getActionName().equals(PSCrossSiteFolderMoveActionProcessor.ACTION_NAME);
    ...
    for (int i = list.size() - 1; i >= 0; i--) {
      ...
      if (!move || !item.isGrandChild()) {
        PSComponentSummary[] summaries = m_folderProc.getParentSummaries(item.getItem());
        for (int j = 0; j < summaries.length; j++) {
          PSComponentSummary summary = summaries[j];
          if (summary.getCurrentLocator().getId() == item.getSrcFolder().getId()) {
  ...
  public Document getProcessReport() {
    ...
    Element root = PSXmlDocumentBuilder.createRoot(doc, "sys_crossSiteFolderAnalysis");
    root.setAttribute("processStatus", data.getProcessorStatus().getName());
    root.setAttribute("hasCrossSiteLinks", data.isHasCrossSiteLinks() ? "yes" : "no");
    ...
    Element depItems = PSXmlDocumentBuilder.addEmptyElement(doc, root, "dependentItems");
    for (int i = 0; i < data.getDependentItems().size(); i++) {
      PSAaFolderDependent depItem = data.getDependentItems().get(i);
      if (depItem.getAaRelationships().isEmpty()) continue;
  ...
  public List<IPSSite> getAllSites() {
    if (m_allSites == null) {
      ...
      if (log.isDebugEnabled()) log.debug("Getting all registered sites from the server...");

      m_allSites = sm.findAllSites();
      for (int i = m_allSites.size() - 1; i >= 0; i--) {
        IPSSite site = m_allSites.get(i);
        if (StringUtils.isBlank(site.getFolderRoot())) m_allSites.remove(i);
      }
      Collections.sort(
          m_allSites,
          new Comparator<IPSSite>() {
            public int compare(IPSSite o1, IPSSite o2) {
              ...
              int c = a.compareTo(b);
              if (c != 0) return c;
              return (o1.getFolderRoot().length() - o1.getFolderRoot().length());
            }
          });
      if (log.isDebugEnabled()) {
