 ...
 * item being processed basis. Each category is identified by a unique ordinal value and name.
 */
public enum PSRemoveActionCategoryEnum {
  ...
  /** Folder remove action category -> item exists on the same site but multiple folders */
  ACTION_CATEGORY_SAMESITE_MULTIPLE_FOLDERS(2, "same site multpile folders"),

  /**
   * Folder remove action category -> item exists only one folder in one site but also exists in
   * other sites.
   */
  ACTION_CATEGORY_MULTIPLE_SITES(3, "multiple sites"),

  /**
   * Folder remove action category -> item being processed is not under any site folder, meaning it
   * is from a non-site folder.
   */
