...
public class PSFolderHandler extends PSWebServicesBaseHandler {
  /**
   * Processing the create folder request. The parent document upon completion will contain the
   * created folder object in XML. The root element is <code>CreateFolderResponse</code> which is
   * specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>CreateFolderRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   ...
   */
  void createFolderAction(PSRequest request, Document parent) throws PSException {
  ...
  /**
   * Processing the open folder request. The parent document upon completion will contain the folder
   * object in XML. The root element is <code>OpenFolderResponse</code> which is specified in <code>
   * sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>OpenFolderRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   ...
   *
   * @param request The request that contains the input paramters, the <code>DeleteFolderRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void deleteFolderAction(PSRequest request, Document parent) throws PSException {
    Document inputDoc = request.getInputDocument();
    ...
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
    proxy.delete(FOLDER_PROXY_TYPE, new PSKey[] {new PSLocator(folderId, 1)});

    addResultResponseXml("success", 0, null, parent);
  }

  /**
   * Processing the purge folder request. This action first deletes all folders recursivly and if
   * that operation is successful, it purges all content items found in all deleted folders.
   *
   * @param request the request that contains the input paramters, the <code>PurgeFolderRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assumed not <code>null
   *     </code>.
   * @param parent the parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void purgeFolderAction(PSRequest request, Document parent)
   ...
   * @throws PSException if an error occurs while purging the specified folder and its child folders
   *     and items.
   */
  ...
  /**
   * Get a list of content id's for all content items for the supplied folder recursivly.
   *
   * @param folder the folder for which to get a list of content id's, assumed not <code>null</code>
   *     .
   * @param itemIds a list into which all content id's are collected. This is a list of content id's
   *     as <code>String</code> objects for all content items found in the supplied folder
   *     recursivly.
   * @throws PSCmsException for any relationship processor error.
   */
  private static void getFolderContent(PSLocator folder, List<String> itemIds)
   ...
   *
   * @param locator the locator of the folder for which to get all children, assumed not <code>null
   *     </code>.
   * @return a list of <code>PSComponentSummary</code> objects with all direct children found for
   *     the supplied folder locator, never <code>null</code>, may be empty.
   * @throws PSCmsException for any error making the lookup request.
   */
  private static List<PSComponentSummary> getFolderSummaries(PSLocator locator)
      throws PSCmsException {
  ...
  /**
   * Processing the update folder request. The parent document upon completion will contain the
   * updated folder object in XML. The root element is <code>UpdateFolderResponse</code> which is
   * specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>UpdateFolderResponse
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void updateFolderAction(PSRequest request, Document parent) throws PSException {
    Document inputDoc = request.getInputDocument();
    ...
    PSSaveResults results =
        PSServerFolderProcessor.getInstance().save(new IPSDbComponent[] {folder});
   ...
   *
   * @param request The request that contains the input paramters, the <code>
   *     AddFolderChildrenRequest</code> element, that is specified in sys_FolderParameters.xsd.
   *     Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void addFolderChildrenAction(PSRequest request, Document parent) throws PSException {
   ...
   *
   * @param request The request that contains the input paramters, the <code>
   *     CopyFolderChildrenRequest</code> element, that is specified in sys_FolderParameters.xsd.
   *     Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void copyFolderChildrenAction(PSRequest request, Document parent) throws PSException {
    ParentChildIds pcIds =
        new ParentChildIds(
            request.getInputDocument(),
  ...
  /**
   * Processing the get folder children request. The parent document upon completion will contain
   * the updated folder object in XML. The root element is <code>GetFolderChildrenResponse</code>
   * which is specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>
   *     GetFolderChildrenRequest</code> element, that is specified in sys_FolderParameters.xsd.
   *     Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getFolderChildrenAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
   ...
   *
   * @param request the request that contains the input paramters, the <code>
   *     GetFolderCommunitiesRequest</code> element, that is specified in <code>
   *     sys_FolderParameters.xsd</code>, assumed not <code>null</code>.
   * @param parent the parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getFolderCommunitiesAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
   ...
   *
   * @param request the request that contains the input paramters, the <code>CloneSiteFolderRequest
   *     </code> element, that is specified in <code>sys_FolderParameters.xsd</code>, assumed not
   *     <code>null</code>.
   * @param parent the parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void cloneSiteFolderAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();

    PSCloneSiteFolderRequest msg =
        new PSCloneSiteFolderRequest(request.getInputDocument().getDocumentElement(), null, null);

    String errorLog = proxy.copyFolder(msg.getSource(), msg.getTarget(), msg.getOptions());
   ...
   *
   * @param request The request that contains the input paramters, the <code>
   *     MoveFolderChildrenRequest</code> element, that is specified in sys_FolderParameters.xsd.
   *     Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void moveFolderChildrenAction(PSRequest request, Document parent) throws PSException {
    ParentChildIds pcIds =
        new ParentChildIds(
            request.getInputDocument(),
   ...
   *
   * @param request The request that contains the input parameters (source and target folder id),
   *     the <code>CopyFolderSecurityRequest</code> element. Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void copyFolderSecurityAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    ...
    Element el = PSXMLDomUtil.getFirstElementChild(root, PSWsFolderProcessor.SOURCE_FOLDER_ID_EL);

    int srcFolderId = getContentId(el, PSWsFolderProcessor.COPY_FOLDERSECURITY_OPERATION);

    el = PSXMLDomUtil.getNextElementSibling(el, PSWsFolderProcessor.TARGET_FOLDER_ID_EL);

    int tgtFolderId = getContentId(el, PSWsFolderProcessor.COPY_FOLDERSECURITY_OPERATION);

    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
    proxy.copyFolderSecurity(new PSLocator(srcFolderId, 1), new PSLocator(tgtFolderId, 1));
   ...
   *
   * @param request The request that contains the input paramters, the <code>
   *     RemoveFolderChildrenRequest</code> element, that is specified in sys_FolderParameters.xsd.
   *     Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void removeFolderChildrenAction(PSRequest request, Document parent) throws PSException {
    ParentChildIds pcIds =
        new ParentChildIds(
            request.getInputDocument(),
  ...
  /**
   * Given a folder or item id, retrieves the fully qualified path. Either a valid path is returned
   * or a ResultResponse w/ an error is returned. The result may contain more than 1 path.
   *
   ...
   * @param response The parent document to add the response element to, never <code>null</code> and
   *     it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getFolderPathsAction(PSRequest request, Document response) throws PSException {
    if (null == request) {
    ...
    Element parent =
        PSXmlDocumentBuilder.addEmptyElement(
            response, response.getDocumentElement(), PSWsFolderProcessor.FOLDER_PATHS_EL);
    if (paths.length >= 1 && paths[0] != null && paths[0].trim().length() > 0) {
      // return the valid path
      for (int i = 0; i < paths.length; i++) {
        PSXmlDocumentBuilder.addElement(response, parent, PSWsFolderProcessor.PATH_EL, paths[i]);
      }
    } else {
      // validating the content id
      PSItemSummaryCache cache = PSItemSummaryCache.getInstance();
      boolean validateId = false;
      if (cache != null && cache.getItem(id) != null) { // validating from cache
        validateId = true;
  ...
  /**
   * Processing the get parent folder request. The parent document upon completion will contain the
   * updated folder object in XML. The root element is <code>GetFolderParentResponse</code> which is
   * specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>GetFolderParentRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getParentFolderAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
    PSComponentSummary[] summaryArray = null;
  ...
  /**
   * Processing the get descendents Locators request. The parent document upon completion will
   * contain the array of locators in XML. The root element is <code>GetDescendentsLocatorsResponse
   * </code> which is specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param request The request that contains the input paramters, the <code>
   *     GetDescendentsLocatorsRequest</code> element, that is specified in
   *     sys_FolderParameters.xsd. Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getDescendentsLocatorsAction(PSRequest request, Document parent) throws PSException {
    // get the input data from request
    PSServerFolderProcessor proxy = PSServerFolderProcessor.getInstance();
  ...
  /**
   * Processing the get descendants Locators without filter request. The parent document upon
   * completion will contain the array of locators in XML.
   *
   * @param request The request that contains the input param3ters, the <code>
   *     GetDescendentsLocatorsWithoutFilterRequest</code> element. Assume not <code>null</code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  ...
  /**
   * Processing the getSummaryByPath request. The parent document upon completion will contain the
   * updated folder object in XML. The root element is <code>GetSummaryByPathResponse</code> which
   * is specified in <code>sys_FolderParameters.xsd</code>.
   *
   * @param req The request that contains the input paramters, the <code>GetSummaryByPathRequest
   *     </code> element, that is specified in sys_FolderParameters.xsd. Assume not <code>null
   *     </code>.
   * @param parent The parent document to add the response element to, assumed not <code>null</code>
   *     and it will already contain the correct base element for the response.
   * @throws PSException if any error occurs.
   */
  void getSummaryByPathAction(PSRequest req, Document parent) throws PSException {
    PSFolderProcessorProxy proxy = getFolderProcessorProxy(req);
    String path =
        getRequestValueFromDoc(
    ...
    Element respRoot = parent.getDocumentElement();
    if (summary == null) {
      Element nullEl = parent.createElement(PSRemoteFolderProcessor.NULL_RESULT);
  ...
  private String getRequestValueFromDoc(Document inputDoc, String parentEl, String childEl)
   ...
   *
   * @param request the request to be used to construct the proxy object, assume not <code>null
   *     </code>.
   * @return the created proxy object, never <code>null</code>.
   * @throws PSCmsException if any error occurs.
   */
  private static PSFolderProcessorProxy getFolderProcessorProxy(PSRequest request)
      throws PSCmsException {
    return new PSFolderProcessorProxy(PSComponentProcessorProxy.PROCTYPE_SERVERLOCAL, request);
  }
   ...
   * </pre>
   *
   * where <code>parentEl</code> and <code>idElName</code> are determined by the caller of the
   * method.
   *
   * @param inputDoc the input document, assume not <code>null</code>.
   * @param parentEl the name of the expected root node, assume not <code>null</code>.
   * @param idElName the expected node name that contains the id, assume not <code>null</code>.
   * @param action the name of the action for which to extract the folder id, assumed not <code>null
   *     </code> or empty. Used only for exception info if the content id is not a valid number.
   * @return the extracted folder id.
   * @throws PSException if the XML is malformed or the folder id is not a parsable integer.
   */
  private int getFolderIdFromDoc(Document inputDoc, String parentEl, String idElName, String action)
      throws PSException {
    Element root = inputDoc.getDocumentElement();
    PSXMLDomUtil.checkNode(root, parentEl);
   ...
   *
   * @param idElem the element of the folder id, assume not <code>null</code>, but may be empty.
   * @param action the action of the current request. Used only for exception info if the content id
  ...
  /**
   * If the request is for createFolder, an empty PSLocator is invalid instead set some sane
   * defaults.
   *
   * @param sourceNode
   * @throws PSUnknownNodeTypeException
  ...
  protected static Element validateCreateFolderRequest(Element sourceNode)
    ...
    for (int i = 0; i < sz; i++) {
      ...
      if (psl.getId() == -1 && psl.getRevision() == -1) {
        el.setAttribute(PSKey.XML_ATTR_NEED_GEN_ID, PSKey.XML_TRUE);
        el.setAttribute(PSKey.XML_ATTR_IS_PERSISTED, PSKey.XML_FALSE);
      }
    }
    return sourceNode;
  }
  ...
  /**
   * This is a helper class, it handles reading XML document which contains parent and child ids, as
   * input parameters.
   */
  private class ParentChildIds {
    /**
     * Constructs the object. It will retrieve one parent id and a list of child ids from a input
     * document.
     *
     * @param inputDoc The input document, assume not <code>null</code>.
     * @param rootName The root name of the document, assume not <code>null</code>.
     ...
     */
    private ParentChildIds(Document inputDoc, String rootName, String parentIdName, String action)
        throws PSException {
      processDoc(inputDoc, rootName, parentIdName, null, null, false, action);
    }

    /**
     * Constructs the object. It will retrieve one parent id, a list of child ids and recursively
     * flag from a input document.
     *
     * @param inputDoc The input document, assume not <code>null</code>.
     * @param rootName The root name of the document, assume not <code>null</code>.
     * @param parentIdName The parent id name, assume not <code>null</code>.
    ...
    private ParentChildIds(
        Document inputDoc, String rootName, String parentIdName, boolean recursive, String action)
        throws PSException {
      processDoc(inputDoc, rootName, parentIdName, null, null, recursive, action);
    }

    /**
     * Constructs the object. It will retrieve two parent ids and a list of child ids from a input
     * document.
     *
     * @param inputDoc The input document, assume not <code>null</code>.
     * @param rootName The root name of the document, assume not <code>null</code>.
     * @param parentIdName The parent id name, assume not <code>null</code>.
     * @param parentIdName2 The 2nd parent id name, assume not <code>null</code>.
    ...
    private ParentChildIds(
        ...
        String action)
        throws PSException {
      processDoc(inputDoc, rootName, parentIdName, parentIdName2, forceName, false, action);
    }
    ...
    private ParentChildIds(
        ...
        String action)
        throws PSException {
      processDoc(inputDoc, rootName, parentIdName, parentIdName2, null, false, action);
    }

    /**
     * Extracts the parent and child ids from a input document. The XML formate is:
     *
     * <pre>
     ...
     * </pre>
     *
     * where <code>rootName</code>, <code>parentIdName</code> and <code>parentIdName2</code> is
     * determined by caller.
     *
     * @param inputDoc The input document, assume not <code>null</code>.
     * @param rootName The root name of the document, assume not <code>null</code>.
     * @param parentIdName The parent id name, assume not <code>null</code>.
     * @param parentIdName2 The 2nd parent id name. If it is <code>null</code>, then not expecting
     *     the 2nd parent id element at the end of the XML document.
     * @param forceName name of the element for force flag, assumed not <code>null</code>
     * @param hasRecursive <code>true</code> if has <code>Recursive</code> element.
     * @param action the current request action, assume not <code>null</code>.
    ...
    private void processDoc(
      ...
      Element parentIdEl = PSXMLDomUtil.getFirstElementChild(root, parentIdName);
      ...
      if (parentIdName2 != null) {
        parentId2El = PSXMLDomUtil.getNextElementSibling(childIdsEl, parentIdName2);
        m_parentId2 = getContentId(parentId2El, action);
      }
      Element prev = childIdsEl;
      if (parentId2El != null) prev = parentId2El;
      if (forceName != null) {
        Element forceEl = PSXMLDomUtil.getNextElementSibling(prev, forceName);
      ...
      if (hasRecursive) {
        ...
        Element recursiveEl =
            PSXMLDomUtil.getNextElementSibling(childIdsEl, PSWsFolderProcessor.RECURSIVE_EL);
     ...
     *     null</code>
     * @param action the action or operation of the current request. Assume it is not <code>null
     *     </code> or empty.
     * @return A list of locators in <code>PSLocator</code> or <code>PSLocatorWithName</code>
    ...
    /**
     * Force move in case if any of the children are involved in cross site linking. Initialized by
     * ctor. Default is <code>false</code>.
     *
    ...
    /** The 2nd optional parent id of the object. It is <code>-1</code> if not exist. */
    int m_parentId2 = -1;

    /** <code>true</code> if this object has a recursive flag. Initialized by constrcutor. */
    private boolean m_hasRecursive = false;

    /**
     * If <code>m_hasRecursive</code> is <code>true</code> then this value is valid. Initialized by
     * constrcutor.
     */
