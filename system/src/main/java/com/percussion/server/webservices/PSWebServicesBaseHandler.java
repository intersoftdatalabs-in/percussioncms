...
public abstract class PSWebServicesBaseHandler implements IPSPortActionHandler {
  ...
  public void processAction(String port, String action, PSRequest request, Document parent)
    ...
    try {
      ...
      method =
          getClass()
              .getDeclaredMethod(action + "Action", new Class[] {PSRequest.class, Document.class});
      ...
      l.error(
          "WSBaseHandler InvocationTargetException,"
                  ...
                  == null
              ? "null"
              : request.getRequestFileURL() + " doc: " + parent == null
                  ? "null"
                  : PSXmlDocumentBuilder.toString(parent) + " method: " + method == null
                      ? "null"
                      : method.toString(),
  ...
  public Element validateContentKey(
    ...
    int id = PSXMLDomUtil.checkAttributeInt(el, ATTR_CONTENTID, contentIdRequired);
    int rev = PSXMLDomUtil.checkAttributeInt(el, ATTR_REVISION, revisionRequired);
  ...
  public void executeCheckInOut(PSRequest request, String trigger) throws PSException {
    ...
    request.setParameter(
        IPSHtmlParameters.SYS_REVISION, newReq.getParameter(IPSHtmlParameters.SYS_REVISION));
  }
   ...
   *    null</code>.
   */
  public static void addResultResponseXml(String type, int code, String message, Document doc) {
    if (type == null || type.trim().length() == 0)
      throw new IllegalArgumentException("type may not be null or empty");

    Element resultResponseEl;
    Element root = doc.getDocumentElement();
    resultResponseEl = doc.createElementNS(NS_COMMON, NS_COMMON_PREFIX + EL_RESULTRESPONSE);
    ...
    if (root == null) doc.appendChild(resultResponseEl);
    else root.appendChild(resultResponseEl);

    Element resultEl = doc.createElementNS(NS_COMMON, NS_COMMON_PREFIX + EL_RESULT);

    resultEl.setAttribute("type", type);
    resultEl.setAttribute("errorCode", "" + code);
    if (message != null) resultEl.appendChild(doc.createTextNode(message));
  ...
  protected long lookupContentTypeId(PSRequest request) throws PSException {
    ...
    String strContentId = request.getParameter(IPSHtmlParameters.SYS_CONTENTID);
  ...
  protected void processInternalRequest(PSRequest request, String path, Document parent)
    ...
    if (doc == null) addResultResponseXml("success", 0, null, parent);
    else
  ...
  protected Document processInternalRequestEx(
      PSRequest request, String path, boolean isHandleOverrideCommunity) throws PSException {
    ...
    if (isHandleOverrideCommunity) handleOverrideCommunity(request);

    if (rh != null && ((rh instanceof PSQueryHandler) || (rh instanceof PSContentEditorHandler))) {
   ...
   */
  protected Document processInternalRequest(PSRequest request, String path) throws PSException {
   ...
   */
  protected static void handleOverrideCommunity(PSRequest request) throws PSException {
    if (null == request) throw new IllegalArgumentException("Request cannot be null.");

    String usersComm =
        (String) request.getUserSession().getPrivateObject(IPSHtmlParameters.SYS_COMMUNITY);
    if (usersComm == null) usersComm = "";
    ...
    if (commOverride != null
        && commOverride.trim().length() > 0
