...
public class PSJobException extends Exception {
   ...
   */
  public PSJobException(int msgCode) {
    this(msgCode, null);
  }
   ...
   *
   * <p>{@code <!ELEMENT PSXJobException (Arg*) <!ATTLIST PSXJobException msgCode CDATA #REQUIRED
   * exceptionClass CDATA #IMPLIED > <!ELEMENT Arg (#PCDATA)> }
   *
  ...
  private static ResourceBundle getErrorStringBundle(Locale loc) throws MissingResourceException {
    if (ms_bundle == null) {
      ms_bundle = ResourceBundle.getBundle("com.percussion.server.job.PSJobErrorStringBundle", loc);
    }
