...
public class PSJobHandlerConfiguration {
   ...
   */
  public String getJobClassName(String category, String jobType) throws PSJobException {
   ...
   * @throws PSJobException if The job definition cannot be located.
   */
  public Properties getJobInitParams(String category, String jobType) throws PSJobException {
    if (category == null || category.trim().length() == 0)
      throw new IllegalArgumentException("category may not be null or empty");
  ...
  private Properties loadInitParams(Element el, Properties defaults)
    ...
    Properties props;
    if (defaults != null) props = new Properties(defaults);
  ...
  private String getContext(String cat, String jobType) {
    if ((cat == null) && (jobType != null))
      throw new IllegalArgumentException("cat may not be null if jobType is not also null");

    String ctx = null;

    if (cat == null) ctx = "handler";
    else if (jobType == null) ctx = "handler/" + cat;
    else ctx = "handler/" + cat + "/" + jobType;
