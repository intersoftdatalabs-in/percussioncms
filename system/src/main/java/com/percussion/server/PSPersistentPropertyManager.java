...
public class PSPersistentPropertyManager {
  ...
  public List<PSPersistentPropertyMeta> savePersistentPropertyMeta(
  ...
  public void deletePersistantPropertyMeta(
      List<PSPersistentPropertyMeta> list, PSUserSession user) {
    IPSCmsObjectMgr mgr = PSCmsObjectMgrLocator.getObjectManager();
  ...
  public PSPersistentPropertyMeta savePersistentPropertyMeta(
  ...
  public static String getUserName(PSUserSession userSession) {
    ...
    PSUserEntry[] pUsr = userSession.getAuthenticatedUserEntries();
    if (pUsr == null || pUsr.length == 0) return "";
    ...
    try {
      ...
      while (itr.hasNext()) {
        ...
        if (action.equals(INSERT) || action.equals(UPDATE) || action.equals(DELETE)) {
          ...
          if (category != null) {
            PSPersistentPropertyMeta metaObj = getFromMetaCache(category, userName, name);
    ...
    if (action.equals(INSERT)) {
      ...
      if (getPersistedProperty(prop.getCategory(), session, propName).isEmpty()) {
        try {
          ...
          // TODO: Look at this code and make sure that an IP or Session is used to make properties
          // unique.
          ms_log.warn(
              "Unable to save persistent property ["
                  + propName
                  + "] for user session "
                  + prop.getUserName());
        }
        ...
        // update instead
        ms_log.warn(
            "Attempting to insert existing persistent property ["
                + propName
  ...
  private void setPropertyCache(PSPersistentProperty ps) {
    ...
    synchronized (ps) {
      ...
      if (action.equals(PSPersistentPropertyManager.INSERT)) {
        ...
        if (obj == null) objMap.put(propName, ps);
        else obj.setValue(ps.getValue());
        // reset the action
        ps.setExtraParam("");
  ...
  public static final String[] METAELEMENTS = {
    PSPersistentPropertyMeta.ROOT_ELEM, PSPersistentPropertyMeta.PROPERTYNAME_ELEM
  };
  public static final String[] PROPERTYELEMENTS = {
    PSPersistentProperty.PERSISTEDPROPERTYVALUES_ELEM, PSPersistentPropertyMeta.PROPERTYNAME_ELEM
  };
}
