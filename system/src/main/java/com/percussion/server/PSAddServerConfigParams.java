 ...
 */
public class PSAddServerConfigParams implements IPSResultDocumentProcessor {
  /**
  ...
  public Document processResultDocument(
    ...
    String elemName = "";
    if (params.length > 0 && params[0] != null) elemName = params[0].toString().trim();

    Element elem = null;
    if (elemName.length() > 1) {
      NodeList nl = resultDoc.getElementsByTagName(elemName);
      if (nl != null && nl.getLength() > 0) elem = (Element) nl.item(0);
    }
    ...
    Element child = (Element) elem.appendChild(resultDoc.createElement(ELEM_SESSIONTIMEOUT));
