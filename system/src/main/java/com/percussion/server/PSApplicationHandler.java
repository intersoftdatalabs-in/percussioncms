...
/**
 * The PSApplicationHandler class is used to handle requests for a given application.
 *
 * <p>The handler supports multi-threaded use. The server contains one application handler for each
 * application. Many user threads can access the handler simultaneously to process an application
 * specific request.
 *
 * <p>The application handler makes use of the following {@link
 * com.percussion.design.objectstore.PSApplication PSApplication} info:
 *
 * <ul>
 *   <li>the application's access control list (ACL)
 *   <li>the application's data encryption settings
 *   <li>the maximum amount of time to spend servicing a request
 *   <li>the request type parameter name/values
 *   <li>the log settings for this application (=> PSLogHandler)
 *   <li>the user session management info
 *   <li>the web page used to login to this application
 *   <li>the web pages being return on error for this application (=> PSErrorHandler)
 *   <li>the data sets defined for accessing data through this application
 *   <li>the default back-end credentials to use when accessing data
 *   <li>the private roles defined for use exclusively by this application
 *   <li>the mail notification settings associated with the application
 *   <li>the user defined functions (UDFs) associated with the application
 * </ul>
 *
 * For each data set ({@link com.percussion.design.objectstore.PSDataSet PSDataSet}), the following
 * information is also used:
 *
 * <ul>
 *   <li>the name of the data set
 *   <li>the type of transaction support
 *   <li>the data set's data encryption settings
 ...
 *   <li>the request definition for this data set (the URL and input parameter settings
 *   <li>the definition of the results being generated by this data set
 *   <li>the exits which act upon the entire data set
 * </ul>
 *
...
public class PSApplicationHandler implements IPSRootedHandler {
  /**
   * Creates an application handler for the specified application. The application handler creates
   * the query and update handlers associated with each data set.
   *
   * @param app The app to create an application handler for. Must not be <CODE>null</CODE>.
   * @param objectStoreHandler The objectstore handler to use. Must not be <CODE>null</CODE>.
   * @param extMgr The extension manager to use. Must not be <CODE>null</CODE>.
   * @throws IllegalArgumentException If any param is invalid.
   * @throws PSSystemValidationException If an error occurs validating the contents of the data
   *     sets.
   */
  public PSApplicationHandler(
      PSApplication app, IPSObjectStoreHandler objectStoreHandler, IPSExtensionManager extMgr)
    ...
    if (extMgr == null) throw new IllegalArgumentException("extMgr cannot be null");
   ...
   *
   * @param request The name of the request to retrieve (the name of the dataset). Never <code>null
   *     </code>.
   ...
   * @throws PSNotFoundException if the target data set does not exist in this application
   */
   ...
   * @throws PSSystemValidationException if an error occurs validating the contents of the supplied
   *     data set.
   */
  public synchronized void addDataHandler(PSDataSet ds) throws PSSystemValidationException {
    if (ds == null) throw new IllegalArgumentException("ds cannot be null");
    ...
    if (dsPipe instanceof PSQueryPipe) {
      ...
      try {
        ...
        if (e.getErrorCode() == 0) {
          throw new PSSystemValidationException(
              "Error in initialization application : "
          ...
          for (int i = 0; i < maps.length; i++) {
            if (maps[i].getRequestHandler() == rh) {
              if (maps.length == 1) {
                ...
                PSRequestPageMap[] newMaps = new PSRequestPageMap[maps.length - 1];
                System.arraycopy(maps, 0, newMaps, 0, i);
                System.arraycopy(maps, i + 1, newMaps, i, maps.length - i);
  ...
  /**
   * Get the id assigned to this application. This is most commonly used when constructing a
   * PSLogError subclass to report an error through the error handler.
   *
  ...
  /**
   * Get the application's request root. This is the url-request root following the server root and
   * preceeding the request page.
   *
  ...
  /**
   * Get the full request root for this application. This includes the server's request root as well
   * as the application's request root.
   *
   ...
   *
   * @return the max number of user threads we can consume servicing requests
   */
  ...
  /**
   * Determine the request type from the information defined in the request. This can only be used
   * for HTML requests. XML requests may contain many types as the tree may contain several objects,
  ...
  public int getRequestType(PSRequest request) {
    ...
    if (sReqType != null) return getRequestType(sReqType);
  ...
  public int getRequestType(String actionString) {
    ...
    if (actionString != null) {
      if (actionString.equalsIgnoreCase(m_requestTypeValueDelete)) iReqType = REQUEST_TYPE_DELETE;
  ...
  /**
   * Convert the URL to use a format which can be acccessed externally. If this is a file URL, it
   * will be converted to the format /ServerRoot/ApplicationRoot/FileName. If this is any other type
   * of URL (eg, HTTP), the name will be left untouched.
   */
  ...
  public static String getExternalURLString(String requestRoot, String fullRequestRoot, URL url) {
    ...
    if (!url.getProtocol().equalsIgnoreCase("file")) return url.toExternalForm();
    ...
    StringBuilder buf = new StringBuilder(newURL.length() + fullRequestRoot.length());
    ...
    if (pos != -1) {
       ...
       */
      if (pos > 0) buf.append(newURL.substring(pos));
      else buf.append(newURL);
      ...
      if (!newURL.startsWith("/")) buf.append('/');
      buf.append(newURL);
    }
  ...
  /**
   * Convert the URL to use a format which can be acccessed from the local system. If this is a file
   * URL, and does not reference the rhythmyx DTD directory, it will be converted to the format
   * ApplicationRoot/FileName. If this is any other type of URL (eg, HTTP), the name will be left
   * untouched.
   */
  public URL getLocalizedURL(URL url) throws java.net.MalformedURLException {
  ...
  public static URL getLocalizedURL(String requestRoot, URL url)
    ...
    if (!url.getProtocol().equalsIgnoreCase("file")) return url;

    /**
     * If the URL points to the Rhythmyx base directory's DTD directory, then return without
     * modification
     */
    ...
    StringBuilder buf =
        new StringBuilder(newURL.length() + requestRoot.length() + "file:".length());
    buf.append("file:");
   ...
   * break security by going outside of the app root
   */
  private static void checkForParentPath(String url) throws java.net.MalformedURLException {
    /* make sure their URL does not include .. We don't want them to
     * break security by going outside of the app root
     */
    if (url.equals("..") || (url.indexOf("../") != -1) || (url.indexOf("/..") != -1)) {
      throw new java.net.MalformedURLException("parent traversal is not permitted in paths");
    }
  }

  /**
   * Initialize the request for use with this application handler, tracing any initial request
   * values, if need be.
   *
  ...
  private void initializeRequestForAppHandler(PSRequest request) {
    ...
    if (m_LogHandler.isTraceEnabled()) {
      m_LogHandler.printTrace(PSTraceMessageFactory.BASIC_REQUEST_INFO_FLAG, request);
      m_LogHandler.printTrace(PSTraceMessageFactory.INIT_HTTP_VAR_FLAG, request);
      m_LogHandler.printTrace(PSTraceMessageFactory.FILE_INFO_FLAG, request);

      // trace if session enabled and if exists
      if (m_LogHandler.isTraceEnabled(PSTraceMessageFactory.SESSION_INFO_FLAG)) {
        Object[] args = {false, request};
        m_LogHandler.printTrace(PSTraceMessageFactory.SESSION_INFO_FLAG, args);
      }
    }
  }
   ...
   * @param irh The internal request handler used to process the current request. Never <code>null
   *     </code>.
   * @throws PSAuthorizationException if the user is not authorized to perform the requested action
   *     or access the requested application
   * @throws PSAuthenticationFailedException if the user failed to authenticate or doesn't have the
   *     appropriate access
   * @throws IllegalArgumentException if any parameter is invalid
   */
  public void initInternalRequestAppSecurity(PSRequest request, IPSInternalRequestHandler irh)
    ...
    if (irh != null) {
      ...
      if (irh instanceof PSUpdateHandler) {
         ...
         */
        hasAccess = true;
      } else hasAccess = hasAccess(request, REQUEST_TYPE_QUERY, false);

      if (!hasAccess) {
        throw new PSAuthorizationException(
            getRequestTypeName(getRequestType(request)),
  ...
  /**
   * Process a request for this application. This includes locating the appropriate data set,
   * checking the ACL and running the request through the appopriate data handler.
   *
   * <p>The following steps are performed to process the request:
   *
   * <ol>
   *   <li>if the data set is not set in the request, locate the appropriate data set for this
   *       request
   *   <li>determine the request type from the selection parameters defined in the PSRequestor
   *   <li>execute the validations defined on the data set. These can be accessed through the
   *       PSRequestor.
   *   <li>verify the user has the appropriate access for what they are trying to do
   *   <li>get the handler for the specified request type from the hash
   *   <li>call the handler if it was found, app error if not
   * </ol>
   *
  ...
  public void processRequest(PSRequest request) {
     ...
     * ***** NOTE ***** */
    synchronized (this) {
      if (m_requestsInProcessing < m_maxUserThreads) m_requestsInProcessing++;
      else {
        ...
        try {
          ...
          PSConsole.printMsg(m_name, "reportError failed: " + e.toString(), null);
        }
    ...
    try {
      if (PSDataEncryptionHandler.checkEncryption(request, m_encryptor)) {
        ...
        // overriding
        String commOverride = request.getParameter(IPSHtmlParameters.SYS_OVERRIDE_COMMUNITYID);
        ...
        if (rh != null) {
          ...
          if (rh instanceof com.percussion.data.PSUpdateHandler) {
             ...
             */
            hasAccess = true;
          } else hasAccess = hasAccess(request, REQUEST_TYPE_QUERY);
          ...
          if (hasAccess) {
            ...
            if (rh instanceof PSDataHandler) {
              cacheContext = new PSCacheContext(request, ((PSDataHandler) rh).getDataSet(), this);
              cacheHandler = PSCacheManager.getInstance().getCacheHandler(cacheContext);
            }
            ...
            try {
              if (!resp.isErrorResponse()) {
                ...
                if (storeResponse) cacheHandler.storeResponse(cacheContext, cachedResp);
              }
            } catch (IOException e) {
              PSLogError respErr =
      ...
      try {
        ...
        PSConsole.printMsg(m_name, "reportError failed: " + t2.toString(), null);
        PSConsole.printMsg(m_name, "original exception: " + t.toString(), null);
      }
    }
  ...
  /**
   * Calls {@link IPSRequestHandler#shutdown()} method on any datahandler that has been successfully
   * created.
   */
  ...
  /**
   * Looks for an internal request handler capable of processing the supplied request by first
   * searching the request page names, then searching the dataset names.
   *
   * @param request request to be handled, not <code>null</code>.
   * @return A handler for the specified request, or <code>null</code> if no handler could be found.
  ...
  public IPSInternalRequestHandler getInternalRequestHandler(PSRequest request) {
    ...
    if (rh != null) return (IPSInternalRequestHandler) rh;
    else {
   ...
   * @param respondWithError determines behavior when no handler is found. If <code>true</code>,
   *     responds to the request with an error message. If <code>false</code>, no reponse is
   *     generated.
   * @return the appropriate request handler or <code>null</code> if a handler is not found.
   */
  IPSRequestHandler getRequestHandler(PSRequest request, boolean respondWithError) {
    ...
    String dataSetName = request.getCgiVariable(IPSCgiVariables.CGI_PS_DATA_SET_NAME);
    ...
    if (dataSetName != null) {
       ...
       * the extension
       */
      if ((maps == null) && (request.getRequestPageType() != PSRequest.PAGE_TYPE_UNKNOWN)) {
      ...
      if ((pageName != null)
        ...
        Object[] args = {pageName, getName(), "traceAppHandlerProc_resourcename"};
        m_LogHandler.printTrace(traceFlagApp, args);
      }
      ...
      if (maps != null) {
        ...
        if (m_LogHandler.isTraceEnabled(traceFlagApp) && (maps.length > 1)) {
          Object[] args = {pageName, getName(), "traceAppHandlerProc_multipleResources"};
          m_LogHandler.printTrace(traceFlagApp, args);
        }

        String dataSetNames = null;
        int matchingHandlers = 0;
        final boolean logMultipleHandlers = m_LogHandler.isMultipleHandlerLoggingEnabled();

        for (int i = 0; i < maps.length; i++) {
          String dsname = null;
          if (maps[i].isMatch(this, request)) {
            if (rh == null) rh = maps[i].getRequestHandler();
            matchingHandlers++;

            if (rh instanceof PSDataHandler) dsname = ((PSDataHandler) rh).getDataSetName();
            else {
              PSContentEditor ce = ((PSContentEditorHandler) rh).getContentEditor();
              dsname = ce.getName();
            }

            // trace test dataset succeeded
            if (m_LogHandler.isTraceEnabled(traceFlagApp)) {
              Object[] args = {pageName, dsname, "traceAppHandlerProc_testhandlerSucceed"};
              m_LogHandler.printTrace(traceFlagApp, args);
            }

            if (!logMultipleHandlers) break;

            if (dataSetNames == null) dataSetNames = dsname;
            else dataSetNames += ", " + dsname;
          } else {
            // trace test dataset failed
            if (m_LogHandler.isTraceEnabled(traceFlagApp)) {
              if (maps[i].getRequestHandler() instanceof PSDataHandler)
                dsname = ((PSDataHandler) maps[i].getRequestHandler()).getDataSetName();
              else {
                PSContentEditor ce =
                    ((PSContentEditorHandler) maps[i].getRequestHandler()).getContentEditor();
                dsname = ce.getName();
              }

              Object[] args = {pageName, dsname, "traceAppHandlerProc_testhandlerFail"};
              m_LogHandler.printTrace(traceFlagApp, args);
            }
          }
        }
      ...
      if ((pageName == null) || (pageName.length() == 0)) {
        if (null != m_defaultPageRedirectHandler) {
          ...
          if (m_LogHandler.isTraceEnabled(traceFlagApp)) {
            ...
            Object[] args = {requestFileURL, "traceAppHandlerProc_defaultPageRedirectUse"};
            m_LogHandler.printTrace(traceFlagApp, args);
          }
        }
      }
    ...
    if (rh == null) { // last resort, is this a file?!
      if (m_fileRequestHandler.isValidFile(request)) {
        rh = m_fileRequestHandler;

        // trace file used
        if (m_LogHandler.isTraceEnabled(traceFlagApp)) {
          String requestFileURL = m_requestRoot + "/" + request.getRequestPage();
          Object[] args = {requestFileURL, "traceAppHandlerProc_filehandlerUse"};
          m_LogHandler.printTrace(traceFlagApp, args);
        }
      } else { // a standalone static page
        if ((maps == null) && (!StringUtils.isEmpty(pageName))) {
          ...
          if (m_fileRequestHandler.isValidFile(request)) {
            rh = m_fileRequestHandler;
            if (m_LogHandler.isTraceEnabled(traceFlagApp)) {
              Object[] args = {requestFileURL, "traceAppHandlerProc_staticfileUse"};
              m_LogHandler.printTrace(traceFlagApp, args);
            }
          }
        }
    ...
    if (rh == null && respondWithError) {
      if (dataSetName != null)
        PSServerLogHandler.handleDataSetHandlerNotFound(
            request, m_id, m_name, dataSetName, getRequestTypeName(requestType));
      else PSServerLogHandler.handleDataSetNotFound(request, m_id, m_name);
    }
  ...
  /**
   * Check if the user associated with the specified request has the specified level of access. If
   * the appropriate access level is not held, an error response will be generated for the request.
   *
   * @param request the request context
   * @param requestType the REQUEST_TYPE_xxx flag specifying the type of request to check access for
  ...
  /**
   * Check if the user associated with the specified request has the specified level of access.
   *
   * @param request the request context
   * @param requestType the REQUEST_TYPE_xxx flag specifying the type of request to check access for
   * @param replyOnError <code>true</code> to send an authorization error to the caller if the
   *     appropriate access is not held
   * @return <code>true</code> if access is permitted
   */
  public boolean hasAccess(PSRequest request, int requestType, boolean replyOnError) {
    ...
    switch (requestType) {
      case REQUEST_TYPE_QUERY:
        if ((accessLevel & PSAclEntry.AACE_DATA_QUERY) == PSAclEntry.AACE_DATA_QUERY)
          hasAccess = true;
        reqiredAcessLevel = PSAclEntry.AACE_DATA_QUERY;
        break;

      case REQUEST_TYPE_INSERT:
        if ((accessLevel & PSAclEntry.AACE_DATA_CREATE) == PSAclEntry.AACE_DATA_CREATE)
          hasAccess = true;
        reqiredAcessLevel = PSAclEntry.AACE_DATA_CREATE;
        break;

      case REQUEST_TYPE_UPDATE:
        if ((accessLevel & PSAclEntry.AACE_DATA_UPDATE) == PSAclEntry.AACE_DATA_UPDATE)
          hasAccess = true;
        reqiredAcessLevel = PSAclEntry.AACE_DATA_UPDATE;
        break;

      case REQUEST_TYPE_DELETE:
        if ((accessLevel & PSAclEntry.AACE_DATA_DELETE) == PSAclEntry.AACE_DATA_DELETE)
          hasAccess = true;
   ...
   *
   * @param request the request context
   * @throws PSAuthorizationException if the user does not have access to this application
   * @throws PSAuthenticationRequiredException if the user has not logged in to this application
   */
  ...
  /**
   * Used by lower level components (like PSQuery/Update/DataHandler) to prepare extensions for use
   * in this application.
   *
   * @param ref The extension ref. Must not be <CODE>null</CODE>.
   * @throws PSExtensionException If an error occurred in preparing the named extension.
   * @throws PSNotFoundException If the extension does not exist.
   * @throws IllegalArgumentException If any param is invalid.
   */
  public IPSRequestPreProcessor prepareRequestPreProcessor(PSExtensionRef ref)
   ...
   * @throws IllegalArgumentException If any param is invalid.
   */
  public IPSResultDocumentProcessor prepareResultDocumentProcessor(PSExtensionRef ref)
      throws PSExtensionException, PSNotFoundException {
   ...
   * @return <CODE>true</CODE> iff the referenced extension implements <CODE>interfaceName</CODE>.
   */
   ...
   * @param e The exception indicating the type of access failure. The error message of this
   *     exception is used as part of the error response text
   */
  ...
  /**
   * Used to obtain the ConcurrentHashMap containing all cached stylesheets used by the app.
   *
   * @return a ConcurrentHashMap containing stylesheets cached using a URL object that identifies
   *     the stylesheet as a key.
   */
  ...
  /**
   * Send the login page once a PSAuthenticationRequiredException or PSAuthorizationException has
   * been thrown.
   *
   * @param request the request context
  ...
  private void sendLoginPage(PSRequest request, PSException e) {
    if (m_loginPage == null) {
      ...
      if (!m_loginPage.isSecure()
          || PSDataEncryptionHandler.checkSecureChannel(request, 40, loginFile)) {
        ...
        try {
          if (loginFile == null) { // signifies we should use the default login page
            ...
            buf.write(
                "        <tr><td colspan=\"2\">Please enter your user id and password to access"
                    + " this application.</td></tr>\r\n");
            buf.write(
            ...
            if (!file.exists()) { // do we need to root this from the app directory?
          ...
          resp.setContent(fileContent, fileLength, IPSMimeContentTypes.MIME_TYPE_TEXT_HTML);
   ...
   *
   * @param app the app definition for which to initialize an application handler, assumed not
   *     <code>null</code>.
   * @throws PSSystemValidationException if an error occurs validating the contents of the supplied
   *     application.
   */
  private void init(PSApplication app) throws PSSystemValidationException {
    ...
    try {
      ...
      if (appDefaultRequestPage != null && appDefaultRequestPage.length() > 0) {
        m_defaultPageRedirectHandler =
            new PSDefaultPageRedirectHandler(m_fullRequestRoot, appDefaultRequestPage);
      }
      ...
      if (m_maxRequestsQueued < 0) m_maxRequestsQueued = Integer.MAX_VALUE;

      // the maximum number of threads the app may consume
      m_maxUserThreads = app.getMaxThreads();
      if (m_maxUserThreads <= 0) m_maxUserThreads = Integer.MAX_VALUE;
      ...
      if (dataSets != null) {
        for (int i = 0; i < dataSets.size(); i++) addDataHandler((PSDataSet) dataSets.get(i));
      }
  ...
  /**
   * Initialize the security components for this application. This should only be called by the init
   * method.
   *
   * @param app the app definition to use
   * @throws PSSystemValidationException if an error occurs validating the contents of the data sets
   */
  private void initSecurity(PSApplication app) throws PSSystemValidationException {
  ...
  private void addMap(PSDataSet ds, IPSRequestHandler rh) throws PSSystemValidationException {
    try {
      ...
      if (curMaps != null) System.arraycopy(curMaps, 0, newMaps, 0, len - 1);
  ...
  /**
   * The request root (path) used for this application. This does not include the server request
   * root.
   */
  private String m_requestRoot;

  /**
   * The full request root (path) used for this application. This includes the server request root.
   */
  ...
  /**
   * Holds the reference to the handler that will process redirects to the application's default
   * page. Will be <code>null</code> if the application has not defined a default page.
   */
  ...
  /**
   * Keep track of the object store we were started with so we can tell it to remove our listener at
   * shutdown.
   */
  ...
  /**
   * Caches PSCachedStylesheet for use in merging. This reference is used directly by other objects
   * to maintain the cache. It is stored in this class to provide a cache on an app by app basis so
   * that when the application is restarted, the cache is cleared and any new version of underlying
   * stylesheets are then used.
   */
  ...
  class PSAppExtensionChangeListener implements IPSExtensionListener {
    ...
    /**
     * Shuts down this application and then re-initializes it. This is usually done after an in-use
     * extension has been updated or removed.
     */
   ...
   *
   * <ul>
   *   <li>Each '/./' is replaced with '/'
   *   <li>Each '/name/../' is replaced with '/'
   * </ul>
   *
   * Paths that are invalid (e.g. empty path part) are returned w/o modification. Relative paths
   * with leading ../'s are ok. If any invalid paths result, an exception is thrown.
   *
   * @param url A url of the form protocol//server:port/path?query, where most pieces are optional
   *     (the part is optional if leaving it out results in a valid url).
   * @return A url normalized to point to the same location as the original url. Never <code>null
   *     </code> unless <code>null</code> was passed. If the 'fixed' url results in an invalid url,
  ...
  private static String fixupDots(String url) {
    ...
    if (url.startsWith("/" + dotdotPath) || url.indexOf("//../") >= 0) {
      return url;
    }
    ...
    while (!done) {
      ...
      if (pos >= 0) {
        String pre = "";
        String post = "";
        if (pos > 0) pre = normalizedUrl.substring(0, pos + 1);
    ...
    while (true) {
      ...
      if (leadingPos >= 0) pre = normalizedUrl.substring(0, leadingPos + 1);
      int trailingPos = pos + dotdotPath.length();
      if (trailingPos < normalizedUrl.length()) post = normalizedUrl.substring(trailingPos);
      normalizedUrl = pre + post;
    }
  ...
  class PSAppHandlerAppChangeListener implements IPSApplicationListener {
    ...
    public void applicationRenamed(PSApplication app, String oldName, String newName) {
      if (m_name.equalsIgnoreCase(oldName)) m_name = newName;
    }
