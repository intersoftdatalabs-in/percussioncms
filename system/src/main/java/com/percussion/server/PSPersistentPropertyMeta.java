...
/**
 * This class defines the metadata for system properties. A system property having a meta data entry
 * is a candidate for persistence.
 *
 * <p>Instances of this class are immutable after construction, so they are thread-safe.
 */
@Entity
@Cache(usage = CacheConcurrencyStrategy.READ_WRITE, region = "PSPersistentPropertyMeta")
...
public class PSPersistentPropertyMeta {
   ...
   */
  public PSPersistentPropertyMeta(
      String userName,
      String propertyName,
      String category,
      int propertySaveType,
      boolean overridable,
      boolean enabled,
      String className) {
   ...
   * @param userName Username for the new object. If <code>null</code> is passed in, the value will
   *     be taken from the original object.
   */
  public PSPersistentPropertyMeta(PSPersistentPropertyMeta metaObj, String userName) {
    this(
        (userName == null ? metaObj.getUserName() : userName),
        metaObj.getPropertyName(),
        metaObj.getCategory(),
        metaObj.getPropertySaveType(),
        metaObj.isOverridable(),
        metaObj.isEnabled(),
        metaObj.getClassName());
  }
   ...
   * Sets the category.
   *
   * @param category The category; if <code>null</code>, an empty string is used.
   */
  ...
  private void fromXML(Element elem) {
    ...
    int iOverridable = Integer.parseInt(lpsxml.getElementData(OVERRIDE_ELEM));
    int iEnabled = Integer.parseInt(lpsxml.getElementData(ENABLE_ELEM));
    ...
    propertySaveType = Integer.parseInt(lpsxml.getElementData(SAVETYPE_ELEM));
  ...
  public Element toXML(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, PROPERTYNAME_ELEM, propertyName);
  ...
  public boolean equals(Object obj) {
    ...
    return Objects.equals(
        this.userName + "," + this.propertyName, other.userName + "," + other.propertyName);
  }
  ...
  /** A value corresponding to default (0), always (1), first time (2) or never (3). */
  @Basic
  @Column(name = "PROPERTYSAVETYPE")
  private int propertySaveType;

  /**
   * If it is <code>1</code>, then the property is persisted (based on the PROPERTYSAVETYPE) and
   * restored; otherwise, the property is not persisted or restored. Defaults to <code>1</code>.
   */
  @Basic
  ...
  /**
   * A flag to indicate whether the default attributes can be overridden by user specific
   * attributes. It is <code>1</code> if overridden is enabled. Defaults to <code>1</code>.
   */
  @Basic
  ...
  /**
   * The fully qualified name of the extension used to serialize/deserialize the value object of
   * this property if it is not a String.
   *
   * @todo Remove this, or implement a use for it. It doesn't seem to get used or set anywhere.
   *     (dbreslau)
   */
  @Basic
  ...
  /**
   * Default entries are against this user name which are later merged with user specified values
