...
public class PSHookRequestHandler implements IPSRequestHandler {
  ...
  public PSHookRequestHandler() {
    ...
    try {
      ...
      m_requestHandlerMethods.put(
          "hook-requestroots-load", myClass.getMethod("loadRequestRoots", xmlClass));
    } catch (Exception e) {
  ...
  public Document loadRequestRoots(Document inDoc)
    ...
    try {
      ...
      PSXmlDocumentBuilder.addElement(respDoc, root, "ServerRoot", PSServer.makeRequestRoot(null));

      // get all of the rooted apps
      for (Iterator<?> i = rootedAppHandlers.iterator(); i.hasNext(); ) {
        Element rootedAppElement =
            PSXmlDocumentBuilder.addEmptyElement(respDoc, root, "RootedApplication");

        Object handler = i.next();
        if (!(handler instanceof IPSRootedHandler)) continue;
        ...
        while (roots.hasNext()) {
          ...
          PSXmlDocumentBuilder.addElement(respDoc, rootedAppElement, "root", reqRoot);
          PSXmlDocumentBuilder.addElement(respDoc, rootedAppElement, "name", name);
        }
      }
    } catch (Exception e) {
      com.percussion.server.PSConsole.printMsg("PSHookRequestHandler", e.toString());
