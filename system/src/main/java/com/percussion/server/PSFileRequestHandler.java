...
public class PSFileRequestHandler implements IPSRequestHandler, IPSInternalRequestHandler {
  ...
  public PSFileRequestHandler(String baseDirectory) {
    ...
    if (baseDirectory != null) m_baseDirectory = new File(root, baseDirectory);
    else m_baseDirectory = root;
  }
  ...
  public boolean isValidFile(PSRequest request) {
    ...
    if (!f.exists()) f = new File(m_baseDirectory, request.getRequestPage(false));
    return f.exists();
  }
  ...
  public void processRequest(PSRequest request) {
    ...
    try {
      ...
      InputStream is;
      if (request.getServletRequest().getMethod().equalsIgnoreCase("head")) {
        byte[] buf = new byte[0];
      ...
      resp.setHeader(
          PSResponse.EHDR_LAST_MOD, PSResponse.formatDateForHeader(new Date(f.lastModified())));
