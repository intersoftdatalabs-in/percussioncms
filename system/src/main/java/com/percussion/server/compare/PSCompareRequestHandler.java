...
public class PSCompareRequestHandler implements IPSLoadableRequestHandler {
  ...
  public void init(Collection<String> requestRoots, InputStream cfgFileIn) {
    ...
    if (configDoc == null) {
      PSConsole.printMsg(HANDLER, "configDoc is null, using default options");
    }

    PSConsole.printMsg(HANDLER, "Compare request handler initialization completed.");
  }
  ...
  public void processRequest(PSRequest request) {
    ...
    response.setContent(new ByteArrayInputStream(bytes), bytes.length, "text/html");
  }
  ...
  protected static String getComparisionResult(String newStr, String oldStr, String lang) {
    ...
    classpath =
        PSServer.getRxFile(jarDirPath + "/docucomp.jar")
            + File.pathSeparator
            + PSServer.getRxFile(jarDirPath + "/rxdocucomp.jar");

    configUri =
        new File(PSServer.getRxFile("rxconfig/Server/requestHandlers/compare.xml"))
            .toURI()
            .toString();
