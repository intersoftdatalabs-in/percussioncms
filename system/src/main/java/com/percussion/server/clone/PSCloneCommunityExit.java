...
public class PSCloneCommunityExit extends PSCloneBase {
  ...
  public Document processResultDocument(
    ...
    if (targetCommunityId.length() < 0) {
      request.printTraceMessage(
          "Error: Missing communityid html parameter, "
    ...
    if (sourceCommunityId > 0) {
      ...
      if (roleId.length() > 0) {
        ...
        request.printTraceMessage("Skipping role community relation as " + "roleid is empty");
      }
    }
    return resultDoc;
  }
  ...
  private PSRole createRole(IPSRequestContext request, String roleName)
    ...
    if (roleName.length() > 0) {
      ...
      try {
        ...
        String userName = request.getUserContextInformation("User/Name", "").toString();
        lockId = new PSXmlObjectStoreLockerId(userName, true, tok.getUserSessionId());
  ...
  private PSRole getRole(PSRoleConfiguration roleCfg, String roleName) {
    ...
    while (roles.hasNext() && role == null) {
      ...
      if (test.getName().equalsIgnoreCase(roleName)) role = test;
    }
