...
public class PSResponse extends PSBaseResponse {
  ...
  public void setStatus(int statusCode, String statusMessage) {
    ...
    // and the content
    byte[] ba = (new String("<HTML>" + statusMessage + "</HTML>")).getBytes();
      ...
      m_statusLine =
          new String(
                  HTTP_VERSION
                      ...
                      + " "
                      + PSErrorManager.getErrorText(statusCode, false, m_locale))
              .getBytes("US-ASCII");
  ...
  public void setCookie(
    ...
    if (path != null) cookieValue.append("[;path=" + path + "]");
    if (domain != null) cookieValue.append("[;domain=" + domain + "]");
    if (expires != null) {
  ...
  public void setContent(Document doc, String type) {
    ...
    if (doc != null) {
      try {
        ...
        Object[] args = {
          "PSXmlDocumentBuilder/write", com.percussion.error.PSException.getStackTraceAsString(e)
        };
  ...
  private void setWifxUploadRedirect(String url) throws IOException {
    ...
    ByteArrayInputStream bis =
        new ByteArrayInputStream(
            ms_wifxResponse.toString().trim().getBytes(PSCharSetsConstants.rxJavaEnc()));
    setStatus(200);
  ...
  static {
    String delayString =
        PSServer.getServerProps().getProperty(PSServer.PROP_RESP_CLOSE_DELAY, "10");
    try {
      ms_delay = Integer.parseInt(delayString);
      if (ms_delay < 10) ms_delay = 10;
