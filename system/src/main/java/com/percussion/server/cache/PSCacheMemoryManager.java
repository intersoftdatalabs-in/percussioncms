...
class PSCacheMemoryManager extends Thread
  ...
  public PSCacheMemoryManager(PSServerCacheSettings cacheSettings) {
    if (cacheSettings == null) throw new IllegalArgumentException("cacheSettings cannot be null");
  ...
  public void setCache(PSMultiLevelCache cache) {
    if (cache == null) throw new IllegalArgumentException("cache may not be null");
  ...
  private long reachedLimit(long max, long current) {
    if (max < -1) throw new IllegalArgumentException("max must be greater or equal to -1");

    if (current < 0) throw new IllegalArgumentException("current must be greater or equal to 0");
    ...
    while (!m_itemsInMemory.isEmpty() && m_usedMemorySpace > marker) {
      ...
      if (item != null) {
        ...
        if (itemFits(item, m_maxDiskUsage)) {
          ...
          if (PSCacheManager.getInstance().isDebugLoggingEnabled())
            log(
                "Item did not fit into disk. The size was: "
                    + item.getSize()
                    + " Available disk was: "
                    + m_maxDiskUsage
                    + "bytes Flushing it instead.");
  ...
  private void handleEvent(PSCacheEvent event) {
    Object o = event.getObject();
    if (!(o instanceof PSCacheItem)) return;
  ...
  private void addCacheItem(PSCacheItem item, long maxMemoryUsage, long maxDiskUsage) {
    if (item.isInMemory()) {
      if (itemFits(item, maxMemoryUsage)) {
        ...
        if (PSCacheManager.getInstance().isDebugLoggingEnabled())
          log(
              "Item did not fit into memory. The size was: "
                  + item.getSize()
  ...
  static boolean delete(File dir) {
    ...
    if (dir.isDirectory()) {
      ...
      for (int i = 0; i < children.length; i++) delete(new File(dir, children[i]));
    }
    return dir.delete();
  }
