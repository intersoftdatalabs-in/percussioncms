...
public class PSResourceCacheHandler extends PSCacheHandler implements IPSTableChangeListener {
    ...
    svc.addListener(
        ...
        new IPSNotificationListener() {
          ...
          public void notifyEvent(PSNotificationEvent notification) {
            ...
            String table = ms_tableTypeMap.get(PSTypeEnum.valueOf(g.getType()));

            if (table != null) {
              tableChanged(
                  new PSTableChangeEvent(
                      table, PSTableChangeEvent.ACTION_UPDATE, new ConcurrentHashMap<>()));
            }
          }
        });
  }
  ...
  public boolean isRequestCacheable(PSCacheContext context) {
    ...
    result = (m_depTree.getSettings(context.getAppName(), context.getDataSetName()) != null);

    if (result) {
      // see if the resource cache is off for this request
      String isCacheOffParam =
          context.getRequest().getParameter(IPSHtmlParameters.SYS_IS_RESOURCE_CACHE_OFF);
  ...
  protected Object[] getKeys(PSCacheContext context) throws PSCacheException {
    ...
    for (Map.Entry<?, ?> entry : sortedParams.entrySet()) {
      ...
      if (entry.getValue() != null) buf.append(entry.getValue());
      buf.append(KEY_SEP);
    }
  ...
  private class PSDataSetKey {
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof PSDataSetKey)) isEqual = false;
      else {
        PSDataSetKey other = (PSDataSetKey) obj;
        if (!m_appName.equals(other.m_appName)) isEqual = false;
        else if (!m_dsName.equals(other.m_dsName)) isEqual = false;
      }
  ...
  private class PSResourceDependencyTree {
    ...
    public void addResource(String appName, PSDataSet ds) {
      ...
      String resourceName = getResourceName(appName, ds.getRequestor().getRequestPage());
      m_resourceMap.put(dsKey, resourceName);

      PSPipe pipe = ds.getPipe();
      if (pipe instanceof PSQueryPipe) // should always be true
      {
        PSResourceCacheSettings settings = ((PSQueryPipe) ds.getPipe()).getCacheSettings();
        if (settings.isCachingEnabled()) {
          ...
          while (children.hasNext()) {
            ...
            List<PSDataSetKey> parents =
                m_parentMap.computeIfAbsent(childName, k -> new ArrayList<>());
            parents.add(dsKey);
          }
        }
      }
    ...
    private Collection<String> expandViews(String tableName) {
      ...
      Collection<String> viewTables = ms_viewMap.get(tableName.toUpperCase());
      if (viewTables == null || viewTables.isEmpty()) result.add(tableName);
      else result.addAll(viewTables);
    ...
    public void removeResource(String appName, String datasetName) {
      ...
      // remove as child
      if (resourceName != null) m_parentMap.remove(resourceName);
    ...
    public Iterator<PSDataSetKey> getDatasetKeys(String tableName) {
      ...
      if (resources != null) dsList.addAll(resources);
      ...
      for (PSDataSetKey dsKey : dsList) {
        ...
        if (m_settingsMap.containsKey(dsKey)) result.add(dsKey);
    ...
    public PSResourceCacheSettings getSettings(String appName, String dsName) {
      ...
      return m_settingsMap.get(new PSDataSetKey(appName, dsName));
    }
    ...
    public Iterator<IPSDataExtractor> getExtractors(String appName, String dsName) {
      ...
      if (intList != null) extractors.addAll(intList);
    ...
    private void initExtractors(PSResourceCacheSettings settings, PSDataSetKey dsKey) {
      ...
      while (extraKeys.hasNext()) {
        PSNamedReplacementValue val = (PSNamedReplacementValue) extraKeys.next();

        try {
          extractors.add(PSDataExtractorFactory.createReplacementValueExtractor(val));
