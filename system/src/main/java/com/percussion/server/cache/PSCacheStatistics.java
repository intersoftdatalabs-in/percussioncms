...
class PSCacheStatistics extends Thread
  ...
  public PSCacheStatistics(PSServerCacheSettings cacheSettings) {
    ...
    if (cacheSettings.isEnabled()) start();
    else m_run = false;
  }
  ...
  public PSCacheStatisticsSnapshot getSnapshot() {
    ...
    synchronized (m_cacheMonitor) {
      snapshot =
          new PSCacheStatisticsSnapshot(
              m_diskHits, m_diskItems, m_diskUsage, m_memItems, m_memUsage, m_misses, m_totalHits);
    }
  ...
  public void run() {
    while (m_run) {
      try {
        ...
        PSCacheEvent event;
        synchronized (m_cachedEvents) {
          if (m_cachedEvents.size() == 0) m_cachedEvents.wait();
