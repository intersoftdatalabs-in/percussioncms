...
/**
 * This class is used to 'plot' graphs for a specific relationship type, so that we can retrieve the
 * parent and/or child graph paths of a specified item.
 *
 * <p>Note: it is necessary to plot all relationships, otherwise it only contains partial graths.
 * The current implementation is for folder relationship only.
 */
class PSRelationshipGraph {
  ...
  public void addRelationship(
      Integer relationshipId, PSLocator parent, PSLocator child, int sortRank) {
    if (relationshipId == null) throw new IllegalArgumentException("relatedObj may not be null");
  ...
  public void removeRelationship(Integer relatedObj, PSLocator parent, PSLocator child) {
    if (relatedObj == null) throw new IllegalArgumentException("relatedObj may not be null");
    if (parent == null) throw new IllegalArgumentException("parent may not be null");
    if (child == null) throw new IllegalArgumentException("child may not be null");
   ...
   *
   * <p>Note: this is implemented for folder relationships. It needs to be overridden for other
   * relationships.
   *
   * @param child the child object, never <code>null</code>.
   * @return a 2 dimension array. The 1st dimension is a list of paths; the 2nd dimension contains
   *     the actual paths, which is an array of <code>PSGraphEntry[]</code> objects. Within each
   *     path, the 1st element is the root, followed by its immediate child, ...etc. Never <code>
   *     null</code>, but may be empty if the supplied object is the root, which does not have
   *     parent.
   */
  ...
  /**
   * This is the same as , but it returns a list instead of array.
   *
   ...
   *
   * @param path the folder path that is returned to the caller. It is a list of {@link
   *     PSGraphEntry}objects. Assume never <code>null</code>, but may be empty.
   * @param childFolder the child folder object. Assume not <code>null</code>.
   */
  private void getFolderPath(
      List<PSGraphEntry> path, PSGraphEntry childFolder, String relationshipTypeName) {
    ...
    Map<Integer, PSGraphEntry> parents = m_childMapToParent.get(childFolder.getValue());
   ...
   *
   * @return a set of <code>Object</code>, which is the <code>parent</code> object that was passed
   *     in , never <code>null</code>, may be empty. It cannot be modified by the caller.
   */
  ...
  /**
   * It maps the child object to its parent. The map keys are child in <code>Object</code>; the map
   * values are collections of parents in <code>Collection</code> objects. Each value (the <code>
   * Collection</code> object) is a collection of {@link PSGraphEntry} object.
   *
   * <p>It may be empty, but never <code>null</code>.
   */
  private Map<PSLocator, Map<Integer, PSGraphEntry>> m_childMapToParent = new ConcurrentHashMap<>();

  /**
   * It maps the parent object to its children. The map keys are parents in <code>Object</code>
   * objects; the map values are lists of child in <code>Collection</code> objects. Each value (the
   * <code>Collection</code> object) is a collection of {@link PSGraphEntry} object.
   *
   * <p>It may be empty, but never <code>null</code>.
   */
  private Map<PSLocator, Map<Integer, PSGraphEntry>> m_parentMapToChildren =
