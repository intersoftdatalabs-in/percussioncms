...
public class PSCacheStatisticsSnapshot {
  ...
  public PSCacheStatisticsSnapshot(
      ...
      long misses,
      long totalHits) {
    if (diskHits < 0) throw new IllegalArgumentException("diskHits may not be < 0");

    if (diskItems < 0) throw new IllegalArgumentException("diskItems may not be < 0");

    if (diskUsage < 0) throw new IllegalArgumentException("diskUage may not be < 0");

    if (memItems < 0) throw new IllegalArgumentException("memItems may not be < 0");

    if (memUsage < 0) throw new IllegalArgumentException("memUsage may not be < 0");

    if (misses < 0) throw new IllegalArgumentException("misses may not be < 0");

    if (totalHits < 0) throw new IllegalArgumentException("totalHits may not be < 0");
  ...
  public Element toXml(Document doc) {
    ...
    if (!StringUtils.isBlank(m_name)) el.setAttribute("name", m_name);

    PSXmlDocumentBuilder.addElement(doc, el, "totalHits", String.valueOf(getTotalHits()));
    PSXmlDocumentBuilder.addElement(doc, el, "totalRequests", String.valueOf(getTotalRequests()));
    PSXmlDocumentBuilder.addElement(doc, el, "hitRate", String.valueOf(getHitRate()) + "%");
    PSXmlDocumentBuilder.addElement(doc, el, "diskHitRate", String.valueOf(getDiskHitRate()) + "%");
    PSXmlDocumentBuilder.addElement(doc, el, "memoryUsage", getProperUnitInBytes(getMemoryUsage()));
    PSXmlDocumentBuilder.addElement(doc, el, "diskUsage", getProperUnitInBytes(getDiskUsage()));
    PSXmlDocumentBuilder.addElement(doc, el, "totalItems", String.valueOf(getTotalItems()));
    PSXmlDocumentBuilder.addElement(
        doc, el, "AverageItemSize", getProperUnitInBytes(getAvgItemSize()));
  ...
  private static String getRoundedValue(double size) {
    return (new BigDecimal(size)).setScale(1, BigDecimal.ROUND_HALF_UP).toString();
  }
