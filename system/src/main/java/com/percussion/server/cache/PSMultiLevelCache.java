...
/**
 * Provides caching of items using a key set. The key mechanism is hierarchical in nature. A fixed
 * number of keys must be supplied, defined when creating the cache, and the item will be stored
 * using the key set as a unique identifier. Items may only be retrieved by providing a full key
 * (all keys supplied), but may be flushed using partial keys (<code>null</code> values in the
 * keyset array). Any subset of the keyset may be used to identify a group of common items to be
 * flushed. Any omitted keys will cause all objects at the level to be included. For example, say a
 * set of items are stored using a key that is composed of the following:
 *
 * <ol>
 *   <li>server
 *   <li>appname
 ...
 * </ol>
 *
 * When flushing, if only server and sessionid are supplied, then that will select all items on the
 * specified server with the specified sessionid, regardless of the appname and resource. The order
 * of the keys is critical and must be maintained when adding, accessing, and removing items from
 * the cache. To omit a key value, supply a <code>null</code> value in that position in the keyset
 * array. So for the above example, the keyset provided for the flush command would be <code>
 * {"myServer", null, null,
 * "25c346b1e7b4e0ace4a1ea39f1adc6b8dabae267"}</code>. <br>
 * Cache is multi-level in that it caches items both in memory and on disk, depending on intial
 * settings, resource usage, and item usage. Handles automatically removing items from the cache
 * that have aged or exceed memory restrictions (based on an LRU algorithm), and manages the
 * swapping items from memory to disk and back to maintain memory restrictions. Items are initially
 * stored in memory, and the items accessed least recently are moved to disk when memory storage is
 * full. When disk storage has reached its limitations, least recently used items on disk are
 * flushed to free resources.
 */
public class PSMultiLevelCache {
   ...
   * @param agingTime The amount of time in minutes to allow an object to be idle in the cache
   *     before it is flushed. Provide <code>-1</code> to allow unlimited time (never expires), or
   *     else a number greater than <code>0</code>.
  ...
  public PSMultiLevelCache(int keySize, long agingTime) {
    ...
    if (agingTime < -1 || agingTime == 0) throw new IllegalArgumentException("invalid aging time");
    ...
    for (int i = 0; i < m_keySize; i++) m_flushAllKeys[i] = null;
   ...
   *
   * @return The id. Each instance of this class will return a value that is unique among all
   ...
   *
   * @return The size of the array that must be used to specify a set of keys that uniquely
   *     identifies an item in the cache. Will be greater than <code>0</code>.
   */
   ...
   *
   * @param keys An array of keys that will uniquely identify this item, may not be <code>null
   *     </code>, may not contain any <code>null</code> entries, and must contain the number of
   *     entries specified when the cache was constructed. If an item identified by the keys is
   *     already found in the cache, it will be replaced.
   * @param item The item to cache, may not be <code>null</code>
   * @param size The reported size (in bytes) the item will use in the cache. It is up to the caller
   *     to provide an accurate value in order to properly maintain the storage limitations of the
   *     cache. Actual storage will exceed these values by a small amount due to a small overhead
   *     incurred by storing additional metadata for each item cached. May not be less than <code>0
   *     </code>.
   * @param type The type of item to retrieve, may not be <code>null</code> or empty.
   * @throws IllegalArgumentException if any param is invalid.
   */
        ...
        if (i == keys.length - 1) {
          synchronized (cacheMap) {
            ...
            if (oldItem != null) {
              ...
              oldItem.release();
              notifyModifiedListeners(PSCacheEvent.CACHE_ITEM_REMOVED, oldItem);
            }

            // notify listeners of add event
            notifyModifiedListeners(PSCacheEvent.CACHE_ITEM_ADDED, cacheItem);
          }
        } else {
   ...
   *
   * @param keys An array of keys that will uniquely identify the item, may not be <code>null</code>
   *     , may not contain any <code>null</code> entries, and must contain the number of entries
   *     specified when the cache was constructed.
   * @param type The type of item to retrieve, may not be <code>null</code> or empty.
   * @return The object matching the keys, or <code>null</code> if the object is not found in the
   *     cache.
   * @throws IllegalArgumentException if any param is invalid.
   * @throws PSCacheException if there are any errors retrieving the item
   */
  public Object retrieveItem(Object[] keys, String type) throws PSCacheException {
    validateKeys(keys, false);
    ...
    if (cacheMap != null) {
      ...
      if (item == null) notifyAccessListeners(PSCacheEvent.CACHE_ITEM_NOT_FOUND, cacheItem);
    }
   ...
   *
   * @param keys An array of keys that will identify the set of items to flush. Any subset of the
   *     keys may be provided, but <code>null</code> values must be used as placeholders for the
   *     omitted keys, and it must contain the number of entries specified when the cache was
   *     constructed. Passing in all null keys is equiv to calling flush().
   * @throws IllegalArgumentException if <code>keys</code> is <code>null</code> or the wrong size.
   */
  ...
  /**
   * Recursively walks the tree of keys and flushes based on the provided keyset.
   *
   * @param cacheMap The Map representing the current branch of the key tree. Assumed not <code>null
   *     </code>.
   * @param keys The array of keys, assumed not <code>null</code> and of the correct length.
  ...
  /**
   * Flushes the entry in the provided map using the specified key. If the entry is an item, it is
   * flushed, otherwise if it is another map, it is recursively flushed using the provided keyset
   * and level.
   *
   * @param cacheMap The map that contains the specified entry. Assumed not <code>null</code>.
   * @param keys The keyset being used to identify the particular item to flush.
   * @param key The key identifying the specific item or "branch" to flush. Assumed not <code>null
   *     </code>.
   * @param level The index into the <code>keys</code> array of the key that is currently being
   *     processed.
   */
  ...
  /**
   * Add's the supplied listener to this objects notifier list so that it may be notified each time
   * an attempt to access an item results in a miss.
   *
  ...
  /**
   * Adds the supplied listener to this object's notifier list so that it may be notified each time
   * an item is added or removed from the cache.
   *
   * @param listener The listener to notify, may not be <code>null</code>.
   * @throws IllegalArgumentException if listener is <code>null</code>.
   */
   ...
   * @param allowNulls <code>true</code> if <code>keys</code> may contain <code>null</code> values,
   *     <code>false</code> if not.
   * @throws IllegalArgumentException if keys is <code>null</code>, if it is not the same size as
   *     specified by the value passed to the ctor, or it contains <code>null</code> values and
   *     <code>allowNulls</code> is <code>false</code>.
   */
  private void validateKeys(Object[] keys, boolean allowNulls) {
    ...
    if (!allowNulls) {
      for (int i = 0; i < keys.length; i++) {
        if (keys[i] == null) throw new IllegalArgumentException("keys may not contain nulls");
      }
    }
  }

  /**
   * Notifies all <code>IPSCacheAccessedListener</code>s with the supplied action and item.
   *
   * @param action Assumed to be one of the <code>PSCacheEvent.ACTION_xxx</code> types.
  ...
  /**
   * Notifies all <code>IPSCacheModifiedListener</code>s with the supplied event.
   *
   * @param action Assumed to be one of the <code>PSCacheEvent.ACTION_xxx</code> types.
   ...
   *
   * @return The id to use. Each call to this method will return a value that is unique among all
   *     current instances of this class.
   */
  ...
  /**
   * Contains the next cacheId to use. Should only be modified through calls to {@link
   * #createCacheId()} to ensure uniqueness.
   */
  private static int ms_nextCacheId = 0;

  /**
   * Indicates the size of the keyset arrays this cache will use. All keyset arrays passed must be
   * this size. Set during ctor, never modified after that.
   */
  private int m_keySize;

  /** The id that uniquely identifies this cache. Set during the ctor, never modified after that. */
  private int m_cacheId;

  /**
   * Top level Map that holds the entire cache. Key is the first key object provided by the keyset
   * of a stored item. If keysize is <code>1</code>, then value is the <code>PSCacheItem</code>
  ...
  /**
   * Keys to use to flush all items from the map. Will be the correct length for this cache and
   * contain all <code>null</code> values. Intialized during construction, never <code>null</code>
   * or modified after that.
   */
  private Object[] m_flushAllKeys;

  /**
   * Object to use to synchronize cache access while flushing the entire cache. Never <code>null
   * </code>.
   */
  private Object m_cacheMonitor = new Object();

  /**
   * Monitors cache for expired items and flushes them. Initialized in ctor, never <code>null</code>
   * after that.
   */
