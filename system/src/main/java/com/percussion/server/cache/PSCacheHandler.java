...
public abstract class PSCacheHandler implements IPSCacheHandler {
  ...
  public boolean isResponseCacheable(PSBaseResponse response) {
    if (response == null) throw new IllegalArgumentException("response may not be null");
  ...
  public PSCachedResponse retrieveResponse(PSCacheContext context) throws PSCacheException {
    ...
    if (keys != null) {
      ...
      if (cache != null) {
        response = (PSCachedResponse) cache.retrieveItem(keys, ITEM_TYPE_RESPONSE);
        if (response != null) {
          logDebugMessage("retrieved response for item with keys: ", keys);
        } else {
          logDebugMessage("failed to located response for item with keys: ", keys);
        }
      }
    }
    return response;
  ...
  public void storeResponse(PSCacheContext context, PSCachedResponse response)
    ...
    if (keys != null) {
      ...
      if (!isResponseCacheable(response)) {
        logDebugMessage("response not cacheable for item with keys: ", keys);
        return;
      }

      PSMultiLevelCache cache = getCache();
      if (cache != null) {
        cache.addItem(keys, response, response.getContentLength(), ITEM_TYPE_RESPONSE);
      ...
      if (cache != null) {
        ...
        if (bytes != null) {
          try {
            ...
            if (bytes.length == 0) doc = PSXmlDocumentBuilder.createXmlDocument();
            else
          ...
          logDebugMessage("failed to located document for item with keys: ", keys);
        }
      }
    }
    return doc;
  ...
  public void storeDocument(PSCacheContext context, Document doc) throws PSCacheException {
    ...
    if (keys != null) {
      ...
      try {
         ...
         */
        if (doc.getDocumentElement() == null) data = new byte[0];
        else data = PSXmlDocumentBuilder.toString(doc).getBytes(PSCharSetsConstants.rxStdEnc());

        // if doc is not cacheable, just return
        if (!isDataCacheable(data.length)) {
          logDebugMessage("document not cacheable for item with keys: ", keys);
          return;
        }
        ...
        // would never happen
        throw new RuntimeException("Cannot store doc in cache: " + e.getLocalizedMessage());
      }
    }
  }
  ...
  public PSCachedResultPage retrieveMergedResults(PSCacheContext context) throws PSCacheException {
    ...
    if (keys != null) {
      ...
      if (cache != null) {
        ...
        if (data != null) {
          logDebugMessage("retrieved merged result for item with keys: ", keys);
        } else {
          logDebugMessage("failed to located merged results for item with keys: ", keys);
        }
      }
    }
    return data;
  ...
  public void storeMergedResults(PSCacheContext context, PSCachedResultPage data)
    ...
    if (keys != null) {
      ...
      if (!isDataCacheable(data.getLength())) {
        logDebugMessage("merged results not cacheable for item with keys: ", keys);
        return;
      }
      ...
      if (cache != null) {
        ...
        logDebugMessage("storing merged results for item with keys: ", keys);
      }
    }
  }
  ...
  private boolean isDataCacheable(long size) {
    ...
    if (maxPageSize != -1 && size > maxPageSize) isCacheable = false;
  ...
  protected void logDebugMessage(String message, Object[] keys) {
    ...
    if (PSCacheManager.getInstance().isDebugLoggingEnabled()) {
      if (keys != null) message += PSCacheItem.toString(keys);
