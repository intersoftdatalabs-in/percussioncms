...
public class PSAssemblerCacheHandler extends PSCacheHandler
  ...
  public PSAssemblerCacheHandler(PSServerCacheSettings cacheSettings) {
    ...
    // load resource file
    try (InputStream stream = PSKeyRules.class.getResourceAsStream(KEY_RULES_FILENAME)) {
      if (null == stream)
        throw new RuntimeException("Cannot locate resource: " + KEY_RULES_FILENAME);
      ...
      throw new RuntimeException(
          "Error loading key rules resource file: " + e.getLocalizedMessage());
    }
  }
  ...
  public boolean isRequestCacheable(PSCacheContext context) {
    ...
    if (!request.getUserSession().isAnonymous()) {
      ...
      isCacheable =
          getVariantsMap()
              .containsValue(request.getAppName() + URL_SEP + request.getRequestPage(false));

      if (isCacheable) {
        // see if the assembler cache is off for this request
        String isCacheOffParam = request.getParameter(IPSHtmlParameters.SYS_IS_ASSEMBLER_CACHE_OFF);
  ...
  void flushApplication(String appName) {
    ...
    if (!PSServer.isCaseSensitiveURL()) appName = appName.toLowerCase();
  ...
  public void tableChanged(PSTableChangeEvent e) {
    if (e == null) throw new IllegalArgumentException("event may not be null");
    ...
    if (table.equals(IPSConstants.CONTENT_STATUS_TABLE)) handleContentStatusChange(cache, e);
  ...
  public void editorChanged(PSEditorChangeEvent e) {
    ...
    switch (e.getActionType()) {
      ...
      case PSEditorChangeEvent.ACTION_DELETE:
        ...
        cache.flush(keys);
        flushDependencies(cache, m_dependencyTree.getDependentItems(contentid, revisionid, -1));
        break;
      ...
      case PSEditorChangeEvent.ACTION_CHECKIN:
        ...
        flushDependencies(cache, m_dependencyTree.getDependentItems(contentid, -1, -1));
        break;
    }
  }
        ...
        case PSRelationshipChangeEvent.ACTION_MODIFY:
          {
            ...
            try {
              variantid =
                  Integer.parseInt(relationship.getProperty(IPSHtmlParameters.SYS_VARIANTID));
            ...
            if (event.getAction() == PSRelationshipChangeEvent.ACTION_ADD)
              depKeys =
                  m_dependencyTree.addDependency(
                      rid, ownerid, ownerrevision, dependentid, variantid, done);
            else
              depKeys =
                  m_dependencyTree.updateDependency(
                      rid, ownerid, ownerrevision, dependentid, variantid, done);
  ...
  private void handleContentStatusChange(PSMultiLevelCache cache, PSTableChangeEvent e) {
    if (e.getActionType() == PSTableChangeEvent.ACTION_DELETE) {
      // get the content id
      String strContentId = (String) e.getColumns().get(IPSConstants.ITEM_PKEY_CONTENTID);
    ...
    if (strVariantId != null) {
      ...
      switch (e.getActionType()) {
        ...
        case PSTableChangeEvent.ACTION_UPDATE:
          ...
          String assemblyUrl = (String) columns.get(IPSConstants.ASSEMBLYURL_COLUMN);
          if (assemblyUrl != null) {
            String reqPage = getRequestPage(assemblyUrl);
            if (reqPage != null) getVariantsMap().put(strVariantId, reqPage);
          }
          break;
      ...
      if (changed) {
        ...
        try {
          ...
          flushDependencies(cache, m_dependencyTree.getDependentItems(-1, -1, variantid));
        } catch (NumberFormatException n) {
          // ignore
        }
      }
  ...
  protected Object[] getKeys(PSCacheContext context) throws PSCacheException {
    ...
    keys[KEY_CONTENTID_INDEX] = request.getParameter(IPSHtmlParameters.SYS_CONTENTID);
    keys[KEY_REVISIONID_INDEX] = request.getParameter(IPSHtmlParameters.SYS_REVISION);
    keys[KEY_VARIANTID_INDEX] = request.getParameter(IPSHtmlParameters.SYS_VARIANTID);
    ...
    else keys[KEY_SESSIONID_INDEX] = NO_SESSION_ID;
  ...
  private void loadContentVariants(Map<String, String> variantMap) throws PSCacheException {
    ...
    for (int i = 0; i < variants.getLength(); i++) {
      ...
      String reqPage = getRequestPage(variant.getAttribute(ASSEMBLY_URL_ATTR));
      if (reqPage != null) variantMap.put(variant.getAttribute(VARIANT_ID_ATTR), reqPage);
    }
  }
  ...
  private String getRequestPage(String url) {
    ...
    if (url.trim().length() > 0) {
      ...
      if ((token.equals(PSServer.getRequestRoot()) || token.equals("..")) && tok.hasMoreTokens()) {
        // next token is the appname
        if (!PSServer.isCaseSensitiveURL()) requestPage.append(tok.nextToken().toLowerCase());
        else requestPage.append(tok.nextToken());
        ...
        while (tok.hasMoreTokens()) {
          ...
          if (dotPos != -1) pageName = pageName.substring(0, dotPos);
