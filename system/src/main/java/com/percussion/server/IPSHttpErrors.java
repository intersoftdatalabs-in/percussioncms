...
/**
 * The IPSHttpErrors inteface is provided as a convenient mechanism for accessing the various HTTP
 * related error codes. The HTTP error code ranges are:
 *
 * <UL>
...
public interface IPSHttpErrors {
  ...
  /**
   * The client may continue with its request. This interim response is used to inform the client
   * that the initial part of the request has been received and has not yet been rejected by the
   * server. The client SHOULD continue by sending the remainder of the request or, if the request
   * has already been completed, ignore this response. The server MUST send a final response after
   ...
   * message header field (section 14.41), for a change in the application protocol being used on
   * this connection. The server will switch protocols to those defined by the response's Upgrade
   * header field immediately after the empty line which terminates the 101 response. The protocol
   * should only be switched when it is advantageous to do so. For example, switching to a newer
   * version of HTTP is advantageous over older versions, and switching to a real-time, synchronous
   * protocol may be advantageous when delivering resources that use such features.
   */
  public static final int HTTP_SWITCHING_PROTOCOLS = 101;

  /**
   * The request has succeeded. The information returned with the response is dependent on the
   * method used in the request, for example: GET an entity corresponding to the requested resource
   * is sent in the response; HEAD the entity-header fields corresponding to the requested resource
   * are sent in the response without any message-body; POST an entity describing or containing the
   * result of the action; TRACE an entity containing the request message as received by the end
   * server.
   */
   ...
   * specific URL for the resource given by a Location header field. The origin server MUST create
   * the resource before returning the 201 status code. If the action cannot be carried out
   ...
   * operation such as this. The 202 response is intentionally non-committal. Its purpose is to
   * allow a server to accept a request for some other process (perhaps a batch-oriented process
   * that is only run once per day) without requiring that the user agent's connection to the server
   * persist until the process is completed. The entity returned with this response SHOULD include
   * an indication of the request's current status and either a pointer to a status monitor or some
   * estimate of when the user can expect the request to be fulfilled.
   */
  public static final int HTTP_ACCEPTED = 202;

  /**
   * The returned metainformation in the entity-header is not the definitive set as available from
   * the origin server, but is gathered from a local or a third-party copy. The set presented MAY be
   * a subset or superset of the original version. For example, including local annotation
   * information about the resource MAY result in a superset of the metainformation known by the
   ...
   * include new metainformation in the form of entity-headers, which SHOULD apply to the document
   * currently in the user agent's active view. The 204 response MUST NOT include a message-body,
   ...
   * that the user can easily initiate another input action. The response MUST NOT include an
   * entity.
   */
   ...
   * included a Range header field (section 14.36) indicating the desired range. The response MUST
   * include either a Content-Range header field (section 14.17) indicating the range included with
   * this response, or a multipart/byteranges Content-Type including Content-Range fields for each
   * part. If multipart/byteranges is not used, the Content-Length header field in the response MUST
   * match the actual number of OCTETs transmitted in the message-body. A cache that does not
   * support the Range and Content-Range headers MUST NOT cache 206 (Partial) responses.
   */
   ...
   * that location. Unless it was a HEAD request, the response SHOULD include an entity containing a
   ...
   * specification does not define any standard for such automatic selection. If the server has a
   * preferred choice of representation, it SHOULD include the specific URL for that representation
   * in the Location field; user agents MAY use the Location field value for automatic redirection.
   ...
   * resource SHOULD be done using one of the returned URIs. Clients with link editing capabilities
   * SHOULD automatically re-link references to the Request-URI to one or more of the new references
   * returned by the server, where possible. This response is cachable unless indicated otherwise.
   ...
   * note with a hyperlink to the new URI(s). If the 301 status code is received in response to a
   * request other than GET or HEAD, the user agent MUST NOT automatically redirect the request
   * unless it can be confirmed by the user, since this might change the conditions under which the
   * request was issued. Note: When automatically redirecting a POST request after receiving a 301
   * status code, some existing HTTP/1.0 user agents will erroneously change it into a GET request.
   */
   ...
   * This response is only cachable if indicated by a Cache-Control or Expires header field. If the
   * new URI is a location, its URL SHOULD be given by the Location field in the response. Unless
   * the request method was HEAD, the entity of the response SHOULD contain a short hypertext note
   * with a hyperlink to the new URI(s). If the 302 status code is received in response to a request
   * other than GET or HEAD, the user agent MUST NOT automatically redirect the request unless it
   * can be confirmed by the user, since this might change the conditions under which the request
   ...
   * substitute reference for the originally requested resource. The 303 response is not cachable,
   * but the response to the second (redirected) request MAY be cachable. If the new URI is a
   * location, its URL SHOULD be given by the Location field in the response. Unless the request
   ...
   * has not been modified, the server SHOULD respond with this status code. The response MUST NOT
   * contain a message-body. The response MUST include the following header fields: o Date o ETag
   * and/or Content-Location, if the header would have been sent in a 200 response to the same
   * request o Expires, Cache-Control, and/or Vary, if the field-value might differ from that sent
   * in any previous response for the same variant If the conditional GET used a strong cache
   * validator (see section 13.3.3), the response SHOULD NOT include other entity-headers. Otherwise
   * (i.e., the conditional GET used a weak validator), the response MUST NOT include other
   * entity-headers; this prevents inconsistencies between cached entity-bodies and updated headers.
   * If a 304 response indicates an entity not currently cached, then the cache MUST disregard the
   * response and repeat the request without the conditional. If a cache uses a received 304
   * response to update a cache entry, the cache MUST update the entry to reflect any new field
   * values given in the response. The 304 response MUST NOT include a message-body, and thus is
   * always terminated by the first empty line after the header fields.
   */
  public static final int HTTP_NOT_MODIFIED = 304;

  /**
   * The requested resource MUST be accessed through the proxy given by the Location field. The
   * Location field gives the URL of the proxy. The recipient is expected to repeat the request via
   * the proxy.
   */
   ...
   * field (section 14.46) containing a challenge applicable to the requested resource. The client
   ...
   * that entity MAY include relevant diagnostic information. HTTP access authentication is
   ...
   * the refusal in the entity. This status code is commonly used when the server does not wish to
   ...
   * status code SHOULD be used if the server knows, through some internally configurable mechanism,
   ...
   * request. Unless it was a HEAD request, the response SHOULD include an entity containing a list
   * of available entity characteristics and location(s) from which the user or user agent can
   * choose the one most appropriate. The entity format is specified by the media type given in the
   * Content-Type header field. Depending upon the format and the capabilities of the user agent,
   * selection of the most appropriate choice may be performed automatically. However, this
   * specification does not define any standard for such automatic selection. Note: HTTP/1.1 servers
   * are allowed to return responses which are not acceptable according to the accept headers sent
   * in the request. In some cases, this may even be preferable to sending a 406 response. User
   * agents are encouraged to inspect the headers of an incoming response to determine if it is
   * acceptable. If the response could be unacceptable, a user agent SHOULD temporarily stop receipt
   ...
   * may not be possible and is not required. Conflicts are most likely to occur in response to a
   * PUT request. If versioning is being used and the entity being PUT includes changes to a
   * resource which conflict with those made by an earlier (third-party) request, the server MAY use
   * the 409 response to indicate that it can't complete the request. In this case, the response
   * entity SHOULD contain a list of the differences between the two versions in a format defined by
   ...
   * delete references to the Request-URI after user approval. If the server does not know, or has
   * no facility to determine, whether or not the condition is permanent, the status code 404 (Not
   * Found) SHOULD be used instead. This response is cachable unless indicated otherwise. The 410
   ...
   * After header field to indicate that it is temporary and after what time the client may try
   * again.
   */
   ...
   * client has descended into a URL "black hole" of redirection (e.g., a redirected URL prefix that
   ...
   * would for a 500 response. Note: The existence of the 503 status code does not imply that a
   * server must use it when becoming overloaded. Some servers may wish to simply refuse the
   * connection.
   */
   ...
   * not supported and what other protocols are supported by that server.11 Access Authentication
   */
