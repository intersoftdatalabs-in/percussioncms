...
/** An abstract class used for all HTTP responses as a base. Container for generic members used. */
public abstract class PSBaseResponse implements Serializable {
   ...
   *
   * @param keepAlive string that specifies whether or not to keep connections alive. Keep alive is
   *     currently not supported and this parameter will be ignored.
   * @param loc the locale to use, if <code>null</code> is provided we will use the default locale.
   */
  public PSBaseResponse(String keepAlive, Locale loc) {
    if (loc == null) m_locale = Locale.getDefault();
    else m_locale = loc;

    /**
     * DO NOT HONOR KEEP-ALIVE UNLESS WE QUEUE CONNECTIONS IN THE LISTENER!!! if (keepAlive != null)
     * m_keepAlive = keepAlive.equalsIgnoreCase(CONN_KEEP_ALIVE); else m_keepAlive = false;
     */
  }

  /**
   * Constructs an new response for the supplied keep alive option and the default locale.
   *
   * @param keepAlive string that specifies whether or not to keep connections alive. Keep alive is
   *     currently not supported and this parameter will be ignored.
   */
   ...
   * @throws IllegalArgumentException if the supplied object is <code>null</code>.
   */
  ...
  /**
   * Create a copy from the supplied object. A shallow copy of all header maps will be made. Cookies
   * are not copied.
   *
  ...
  public void clear() {
    if (m_generalHeaders != null) m_generalHeaders.clear();

    if (m_responseHeaders != null) m_responseHeaders.clear();
   ...
   *
   * @return The length, may be <code>0</code> if no content has been set on this response.
   */
  ...
  /**
   * Format a date for use as a header. HTTP requires dates in the following format:
   *
   * <p>Wed, 15 Nov 1995 04:58:08 GMT
   *
   ...
   * </code>, the response content is maintained in a <code>Document</code>, otherwise its
   * maintained in an <code>InputStream</code>.
   */
  ...
  /**
   * The content if the <code>m_isContentDoc</code> is <code>false</code>, might be <code>null
   * </code>. This is transient because serialization has to be special for this.
   */
  protected transient InputStream m_contentStream = null;

  /**
   * The content if the <code>m_isContentDoc</code> is <code>true</code>, might be <code>null</code>
   * . This is transient because serialization has to be special for this.
   */
  protected transient Document m_contentDoc = null;

  /**
   * A flag specifying whether or not to keep connections alive. Keep alive is currently not
   * supported and this will always be set to <code>false</code>.
   */
  protected boolean m_keepAlive = false;

  /** The locale to use. Initialized in the constructor, never <code>null</code> after that. */
  protected Locale m_locale = null;

  /**
   * The status line used for successful requests. Initialized while constructed, never changed
   * after that.
   */
  protected static byte[] HTTP_OK_STATUS_LINE = null;

  /**
   * A constant for carriage return / line feed. Initialized once while constructed, never changed
   * after that.
   */
  protected static byte[] CR_LF = null;

  /**
   * A constant used as header break. Initialized once while consturcetd, never changed after that.
   */
  ...
  /**
   * The following constants define all general header fields. For more information consult the HTTP
   * 1.1 specification.
   */
  ...
  /**
   * The following constants define all response header fields. For more information consult the
  ...
  /**
   * The following constants define all entity header fields. For more information consult the HTTP
   * 1.1 specification.
   */
  ...
  static {
    try {
      HTTP_OK_STATUS_LINE = (new String(HTTP_VERSION + " 200 OK")).getBytes("US-ASCII");
      CR_LF = (new String("\r\n")).getBytes("US-ASCII");
