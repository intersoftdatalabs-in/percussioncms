...
public class PSFormContentParser extends PSContentParser {
  ...
  public void parse(
    ...
    if (newLength < length) {
      Object[] args = {
        request.getUserSessionId(), contentType, String.valueOf(length), String.valueOf(newLength)
      };
      PSLogManager.write(
     ...
     */
    parseParameterString(request.getParameters(), createUrlDecodedString(buf, charset));
  }
  ...
  public static void parseParameterString(Map<String, Object> params, String paramString)
    ...
    if (params == null) throw new IllegalArgumentException("Parameter map must be specified.");

    if (paramString == null) throw new IllegalArgumentException("paramString can not be null");

    try {
      paramString =
          createUrlDecodedString(
    ...
    PSParamContext paramContext = ms_innerBuilder.new PSParamContext(params);
    ...
    StringTokenizer tok =
        new StringTokenizer(
            paramString, STR_URLENCODING_VALUE_TOKEN + STR_URLENCODING_PARAM_TOKEN, true);
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (curTok.equals(STR_URLENCODING_PARAM_TOKEN)) {
        ...
        else curValue = urlDecode(curTok);
      }
    }
  ...
  private boolean isMultipartContentType(String contentType) {
    ...
    return contentType
        .toLowerCase()
        .startsWith(IPSMimeContentTypes.MIME_TYPE_MULTIPART_FORM.toLowerCase());
  }
  ...
  private static void parseMultipartForm(
      ...
      throws IOException, PSRequestParsingException {
    PSParamContext paramContext = ms_innerBuilder.new PSParamContext(request.getParameters());
    ...
    if (line != null) {
      ...
      do {
        ...
        more =
            parseNextBlock(
                reader, boundary, name, paramContext, request, originalPageCharSet, xmlDocFlags);
      } while (more);
    }
  }
  ...
  private static boolean parseNextBlock(
    ...
    try {
      ...
      if (!"form-data".equalsIgnoreCase(curTok)) {
        if ("attachment".equalsIgnoreCase(curTok)) isAttach = true;
        else contentDispError(line);
      }
      curTok = tok.nextToken(); /* skip the ; delimiter */
      curTok = tok.nextToken().trim();
      if (!isAttach) {
        if ("name=".equalsIgnoreCase(curTok)) {
          ...
          if ("\"".equals(curTok)) {
            name = "";
            while (!"\"".equals(curTok = tok.nextToken())) name += curTok;
          } else name = curTok;
        } else contentDispError(line);
      ...
      while (tok.hasMoreTokens()) {
        ...
        if ("filename=".equalsIgnoreCase(curTok)) {
          ...
          if ("\"".equals(curTok)) {
            filename = "";
            while (!"\"".equals(curTok = tok.nextToken())) filename += curTok;
          } else filename = curTok;
        }
      }
      ...
      if (mimeType.startsWith("text/xml") || mimeType.startsWith("application/xml")) {
        ...
        if (xmlDocFlags != null) {
          if (!xmlDocFlags.isEmpty()) {
            String xmlDocValue = xmlDocFlags.remove(xmlDocFlags.size() - 1);
            if (xmlDocValue.equalsIgnoreCase(PSRequest.XML_DOC_AS_TEXT)) {
              isXml = false;
            } else if (xmlDocValue.equalsIgnoreCase(PSRequest.XML_DOC_VALIDATE)) {
              validateXML = true;
            }
          }
        }
      } else if (!mimeType.startsWith("text/")) isText = false;
    ...
    if (isText == false) {
      if (mimeType.startsWith("multipart/mixed")) {
        ...
        encType = null;
      } /* end block of "multipart/mixed" content-type case */ else { // other content-types such as
        // octet/stream, image, audio,
        // etc.
        ...
        if (headers.size() > 1) {
          for (i = 1; i < headers.size(); i++) {
            line = headers.get(i);
            line = line.toLowerCase();
            if (line.startsWith("content-transfer-encoding")) break;
          }
        }
    ...
    */
    ...
    if (!isXml && filename != null) isText = false;
    ...
    if (isText) {
      ...
      if (isXml) {
        File f = new PSPurgableTempFile("psx", ".xml", null, filename, contentType, encType);
        sourceValue = f;
      ...
      if ((filename != null) || isXml) value = sourceValue;
      ...
      Object sourceValue = null;
      if (filename != null) {
        File f = new PSPurgableTempFile("psx", ".bin", null, filename, contentType, encType);
        sourceValue = f;
      ...
      byte[] readBuf = (innerBoundary == null) ? boundary.getBytes() : innerBoundary.getBytes();
      ...
      for (int curRead = reader.read(readBuf); curRead >= 0; curRead = reader.read(readBuf)) {
        for (i = 0; i < curRead; ++i) {
          c = readBuf[i];
      ...
      if (filename != null) value = sourceValue;
      else // this is a bytearray otherwise
     ...
     */
    if (isXml) {
      Document doc = PSXmlContentParser.getXMLDocument((File) value, null, validateXML);
  ...
  private static String urlDecode(String str) throws PSRequestParsingException {
    ...
    while (iSrc < str.length()) {
      if ((ch = str.charAt(iSrc++)) == URLENCODING_SPACE_TOKEN) ch = URLENCODING_SPACE_REAL;
  ...
  private static String encTypeOfXml(String charsetName, String line) {
    ...
    if (xmlTok.hasMoreTokens()) {
      ...
      if ("{BOM}<?xml".equalsIgnoreCase(curTok)) bomFlag = 1;
    }
    while (xmlTok.hasMoreTokens()) {
      ...
      if ("encoding=".equalsIgnoreCase(curTok)) {
        curTok = xmlTok.nextToken();
        if ("\"".equals(curTok)) {
          while (!"\"".equals(curTok = xmlTok.nextToken())) encoding += curTok;
        } else encoding = curTok;
      }
    }

    if (charsetName != null) encType = charsetName;
    else {
      if (bomFlag == 1) encType = "UTF-16";
      else {
        if (encoding == null) encType = "UTF-8";
        else encType = encoding;
      }
    }
  ...
  public static String createUrlDecodedString(byte[] buf, String charset) throws IOException {
      ...
      if (b == 37) { // 37 is the decimal representation of the % char
        ...
        if ((readFrom + 2) < len) {
          ...
          if ((decodedByte == (byte) URLENCODING_VALUE_TOKEN)
              ...
              || (decodedByte
                  == (byte)
                      URLENCODING_SPACE_TOKEN)) { // we can't convert these as they'll break our
            // parser
            ...
            != writeTo) { // since we've decoded an escape char, we need to move in any
          // subsequent bytes
    ...
    return new String(buf, 0, writeTo, PSCharSets.getJavaName(charset));
  }
  ...
  public static byte getByteFromEscapedHex(byte hexh, byte hexl) {
    ...
    if ((hexh - '0') < 10) b = (byte) (hexh - '0');
    else if ((hexh >= 'a') && (hexh <= 'f')) b = (byte) (10 + (hexh - 'a'));
    else b = (byte) (10 + (hexh - 'A'));
    b <<= 4; // shift over to the high nibble

    if ((hexl - '0') < 10) b += (byte) (hexl - '0');
    else if ((hexl >= 'a') && (hexl <= 'f')) b += (byte) (10 + (hexl - 'a'));
    else b += (byte) (10 + (hexl - 'A'));

    return b;
  }

  private static final String[] ARRAY_SUPPORTED_TYPES = {
    IPSMimeContentTypes.MIME_TYPE_URLENCODED_FORM, IPSMimeContentTypes.MIME_TYPE_MULTIPART_FORM
  };
