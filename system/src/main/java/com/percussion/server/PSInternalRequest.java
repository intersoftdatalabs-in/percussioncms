...
public class PSInternalRequest {
  ...
  public PSInternalRequest(PSRequest req, IPSInternalRequestHandler rh) {
    if (req == null || rh == null) throw new IllegalArgumentException("req or rh may not be null");
  ...
  public void makeRequest()
    ...
    if (m_madeRequest) throw new IllegalStateException("This method may only be called once");
  ...
  public ByteArrayOutputStream getMergedResult() throws PSInternalRequestCallException {
    if (m_madeRequest) {
      throw new IllegalStateException(
          "This method cannot be called if" + " makeRequest() has already been called.");
    } else {
      try {
        ...
        if (os == null) {
          ...
          if (m_internalRequestHandler instanceof PSDataHandler) {
            ...
            m_computedMimeType =
                PSResultSetXmlConverter.getMimeTypeForRequestor(requestor, ext, m_execData);
  ...
  // see IPSInternalResultHandler
  public ByteArrayOutputStream getContent() throws PSInternalRequestCallException {
    if (m_madeRequest) {
      throw new IllegalStateException(
          "This method cannot be called if" + " makeRequest() has already been called.");
    } else {
      try {
        ByteArrayOutputStream os = null;
        m_execData = m_internalRequestHandler.makeInternalRequest(m_request);
        m_madeRequest = true; // required by getResultHandler()
      ...
      try {
        ...
        if (cacheCtx != null) {
          ...
          if (cacheHandler != null) {
            ...
            if (result == null) storeInCache = true;
          }
        }

        // if no cached doc, process the request
        if (result == null) result = getResultDocument(m_request, m_internalRequestHandler);
  ...
  private PSCacheContext getCacheContext() {
    ...
    if (m_internalRequestHandler instanceof PSDataHandler && appHandler != null) {
      ctx =
          new PSCacheContext(
              m_request, ((PSDataHandler) m_internalRequestHandler).getDataSet(), appHandler);
    }
  ...
  public ResultSet getResultSet() throws PSInternalRequestCallException {
    if (m_madeRequest) {
      ...
      if (m_execData != null)
        throw new IllegalStateException("Must cleanUp() this request before reusing it.");
  ...
  private IPSInternalResultHandler getResultHandler() {
    ...
    if (!m_madeRequest) throw new IllegalStateException("makeRequest must be called first.");

    if (m_gotResults) throw new IllegalStateException("results have already been retrieved.");

    IPSInternalResultHandler irh = (IPSInternalResultHandler) m_internalRequestHandler;
   ...
   * @throws PSInternalRequestCallException
   */
  private PSExecutionData makeInternalRequest(PSRequest req, IPSInternalRequestHandler rh)
      throws PSAuthorizationException,
          PSAuthenticationFailedException,
   ...
   * @throws PSAuthenticationFailedException
   */
  private Document getResultDocument(PSRequest req, IPSInternalRequestHandler rh)
      throws PSInternalRequestCallException,
          PSAuthorizationException,
          PSAuthenticationFailedException {
    Document data = null;
   ...
   * @throws PSAuthorizationException
   */
  private ByteArrayOutputStream getMergedResult(PSExecutionData data, IPSInternalResultHandler rh)
      throws PSInternalRequestCallException,
          PSAuthorizationException,
          PSAuthenticationFailedException {
  ...
  private class PSInternalRequestInfo {
    ...
    public void replaceRequestInfo() {
      ...
      mi_actualReq = (PSRequest) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUEST);
      if (PSRequestInfo.isInited()) {
        if (mi_actualReq == null) mi_isActualNull = true;
        PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_PSREQUEST, mi_req);
      }
    }
    ...
    public void reset() {
      if (PSRequestInfo.isInited()) {
        ...
        if (mi_isActualNull) PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_PSREQUEST, null);
        else PSRequestInfo.setRequestInfo(PSRequestInfo.KEY_PSREQUEST, mi_actualReq);
      }
