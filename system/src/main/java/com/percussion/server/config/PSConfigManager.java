...
public class PSConfigManager {
  ...
  public Document getRxConfiguration(Document inDoc, String reqUser)
    ...
    if (root == null) throw new IllegalArgumentException("inDoc must have the document element");
    ...
    boolean overrideSame = XML_BOOL_TRUE.equalsIgnoreCase(root.getAttribute("overrideSameUser"));
    boolean overrideDifferent =
        XML_BOOL_TRUE.equalsIgnoreCase(root.getAttribute("overrideDifferent"));

    Document cfgDoc = getRxConfiguration(name, lock, overrideSame, overrideDifferent, reqUser);
  ...
  public void saveRxConfiguration(Document inDoc, String reqUser)
      throws PSServerConfigException, PSNotLockedException, IOException, SAXException {
    if (inDoc == null) throw new IllegalArgumentException("inDoc may not be null.");
    ...
    String name = root.getAttribute("name");
    boolean releaseLock = XML_BOOL_TRUE.equalsIgnoreCase(root.getAttribute("releaseLock"));
   ...
   * @throws SAXException if an error occurs parsing the configuration.
   */
  public void saveRxConfiguration(String name, String config, boolean releaseLock, String reqUser)
      throws PSServerConfigException, PSNotLockedException, IOException, SAXException {
    if (name == null || name.trim().length() == 0)
      throw new IllegalArgumentException("name may not be null or empty");
    if (reqUser == null || reqUser.trim().length() == 0)
    ...
    if (releaseLock) cfg.releaseLock();

    // set new configuration
    if (config != null) cfg.setConfig(config);
