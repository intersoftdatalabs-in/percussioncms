...
public class PSCachedResponse extends PSBaseResponse implements Serializable {
  ...
  public PSCachedResponse(PSResponse response) throws IOException {
    ...
    try {
      if (m_isContentDoc) {
        if (m_contentDoc != null) PSXmlDocumentBuilder.write(m_contentDoc, os);
      } else {
        if (m_contentStream != null) IOTools.copyStream(m_contentStream, os);
      }

      m_content = os.toByteArray();
      if (!m_isContentDoc) { // reset the content stream, since it has been used by
