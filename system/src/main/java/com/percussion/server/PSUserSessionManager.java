...
public class PSUserSessionManager extends Thread implements IPSServerConfigurationListener {
  ...
  public static synchronized void init(PSServerConfiguration config, IPSObjectStoreHandler os)
    ...
    if (ms_SessionManager != null)
      throw new IllegalStateException("This object can only be instantiated once.");
  ...
  private void validateSessions() {
    ...
    try {
      ...
      boolean collectReleases = ms_Sessions.size() > ms_maxOpenUserSessions;
      ...
      while (sessionIter.hasNext()) {
        ...
        if ((idledTimeMS > DESIGNER_TIMEOUT && sess.isDesignerSession())
            || (idledTimeMS > requestTimeout && !sess.isDesignerSession())) {
          ...
          if (user != null) userPreserveSet.add(user);

          sleepTimeMS = shortestSleep - idledTimeMS;

          if (sleepTimeMS < shortestSleep) shortestSleep = sleepTimeMS;

          if (collectReleases) {
            if (sess.hasAuthenticatedUserEntries()) {
              if (!sess.isDesignerSession()) authenticated.put(idledAt, sess);
            } else anonymous.put(idledAt, sess);
          }
        }
      } // end of while
      ...
      if (collectReleases) {
        ...
        while (!anonymous.isEmpty() && ms_Sessions.size() > maxSessions) {
          ...
          releaseUserSession((PSUserSession) anonymous.remove(anonymous.firstKey()));
        }

        while (!authenticated.isEmpty() && ms_Sessions.size() > maxSessions) {
          log.warn("Too many open sessions releasing authenticated");
          releaseUserSession((PSUserSession) authenticated.remove(authenticated.firstKey()));
        }
      }

      if (shortestSleep < minimumSleep) shortestSleep = minimumSleep;

      if (shortestSleep > ms_maximumSleep) shortestSleep = ms_maximumSleep;

      // Add sessions removed by log out last to use these session ids instead of an expired
      // sessionid for force checkin
      ...
      // After we have calculated user sessions that may be removed.  We save any users that have
  ...
  public static synchronized PSUserSession getUserSession(PSRequest request) {
    ...
    if (sessId != null) sess = getUserSession(sessId);
    ...
    if (sess == null) {
      ...
      */
      ...
      if ((sessId != null) && (areSessionsEnabled() || isDesignerRequest(request)))
  ...
  public static boolean isDesignerRequest(PSRequest request) {
    if (request != null) {
      String type = request.getCgiVariable(IPSCgiVariables.CGI_PS_REQUEST_TYPE);
      if (type != null) {
        type = type.toLowerCase();

        if (type.startsWith("admin") || type.startsWith("design")) return true;
      }
    }
  ...
  public static boolean isDesignerConnectRequest(PSRequest request) {
    if (request != null) {
      String type = request.getCgiVariable(IPSCgiVariables.CGI_PS_REQUEST_TYPE);
      if (type != null) {
        if (type.equalsIgnoreCase("design-open")) return true;
      }
    }
  ...
  public static void releaseUserSession(PSUserSession sess) {
    if (sess != null) {
      ...
      cleanUpSession(sess);
      String user = sess.getRealAuthenticatedUserEntry();
      if (user != null) releasedUsers.add(sess);
  ...
  private static void fromServerConfiguration(PSServerConfiguration config) {
    if (config == null) {
      ...
      if (config.isUserSessionEnabled()) ms_flags = F_ENABLED;
      else ms_flags = F_NONE;
      ...
      ms_maximumSleep =
          ms_maxOpenUserSessions / PSServerConfiguration.MINIMAL_REQUIRED_OPEN_SESSIONS * 1000;
    }
  }
    ...
    while (sessionIter.hasNext()) {
      ...
      if (sess.hasAuthenticatedUserEntries()) {
        if (sess.isDesignerSession()) {
          ++designerCount;
          if (full) designer.appendChild(sess.getUserSessionStatus(doc));
        } else {
          ++authenticatedCount;
          if (full) authenticated.appendChild(sess.getUserSessionStatus(doc));
        }
      } else {
        ++anonymousCount;
  ...
  private static ConcurrentHashMap<String, PSUserSession> ms_Sessions =
      new ConcurrentHashMap<String, PSUserSession>();

  private static ConcurrentLinkedQueue<PSUserSession> releasedUsers =
      new ConcurrentLinkedQueue<PSUserSession>();
