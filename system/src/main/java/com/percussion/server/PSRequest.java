...
public class PSRequest {
  ...
  public PSRequest(
    ...
    if (!StringUtils.isBlank(charset)) setCharacterSet(charset);
  ...
  public PSRequest cloneRequest() {
    ...
    req.m_cloneParentRequest = isOriginalRequest() ? this : m_cloneParentRequest;
  ...
  public void addClone(int originalId, PSLocator clone) {
    if (originalId < 0) throw new IllegalArgumentException("originalId cannot be < 0");

    if (m_clones == null) m_clones = new HashMap<>();
  ...
  public void addRelationships(PSRelationshipSet relationships) {
    ...
    if (m_relationships == null) m_relationships = new ArrayList<>();
  ...
  public synchronized PSUserSession getUserSession() {
    ...
    if (m_session == null) m_session = PSUserSessionManager.getUserSession(this);
  ...
  public void setRequestFileURL(String reqFileURL) {
    ...
    if (serverRoot == null) {
      ...
      if (pagePos == -1) {
        ...
        } else pagePos = serverRoot.length(); /* obviously no app root */
      }
    }
    ...
    if (pagePos == -1) {
      m_reqRoot = "";
      if (extPos == -1) m_reqPage = reqFileURL;
      else m_reqPage = reqFileURL.substring(0, extPos);
    } else {
      m_reqRoot = reqFileURL.substring(0, pagePos);
      if (pagePos == reqFileURL.length()) m_reqPage = "";
      else if (extPos < pagePos) m_reqPage = reqFileURL.substring(pagePos + 1);
    ...
    if (extPos != -1) {
      ...
      if (reqExtension.equalsIgnoreCase("XML")) m_reqPageType = PAGE_TYPE_XML;
  ...
  public synchronized String getParameter(String name, String defValue) {
    ...
    if (m_params.isEmpty()) return defValue;
    ...
    if (o instanceof char[]) {
      ...
      if (!((List<?>) o).isEmpty()) return String.valueOf(((List<?>) o).get(0));
  ...
  public synchronized Object getParameterObject(String name, Object defValue) {
    ...
    if (m_params.isEmpty()) retValue = defValue;
    else {
      Object o = m_params.get(name);
      if (o == null) retValue = defValue;
      else retValue = getBalancedValue(o, getMaxListValuesSize());
    }
  ...
  private synchronized Object getBalancedValue(Object value, int size) {
    ...
    if (value instanceof List) {
      ...
      {
        if (listSize > 0) retValue = valList.get(0);
        else retValue = null; // not likely
      } else // requires a list
      {
        ...
        if (listSize >= size) {
          ...
          if (listSize > 0) {
            ...
            balanceList.addAll(createList(valList.get(listSize - 1), size - valList.size()));
          } else balanceList = createList(null, size); // not likely
  ...
  public synchronized Object getSingleParameterObject(String name, Object defValue) {
    ...
    if (o instanceof List) // false if it is null
    {
      ...
      if (!sourceList.isEmpty()) o = sourceList.get(0);
      else o = null;
    }
  ...
  public synchronized void appendParameter(String name, Object value) {
    ...
    if (paramValue instanceof List) {
      ...
      if (!paramValue.getClass().isAssignableFrom(value.getClass()))
        throw new IllegalStateException("The value of parameter to append is not of correct type.");
  ...
  public boolean isOnlyMultiValueParam(String name) {
    ...
    if (o instanceof List) {
      for (String key : m_params.keySet()) {
        ...
        if (m_params.get(key) instanceof List) return false;
      }
      return true;
    }
    return false;
  ...
  public PSPurgableTempFile getTempFileResource(String key) {
    ...
    if (m_tempFileResources == null) return null;
    else return (PSPurgableTempFile) m_tempFileResources.get(key);
  }
  ...
  public void addTempFileResource(PSPurgableTempFile f, String key) {
    ...
    if (f == null) throw new IllegalArgumentException("Must supply a file resource.");

    if (m_tempFileResources == null) m_tempFileResources = new HashMap<>();
  ...
  public Map<String, Object> getTruncatedParameters() {
    ...
    if (m_params.isEmpty()) result = new HashMap<>();
    else result = getBalancedParameters(1);
  ...
  public void setParameters(Iterator<? extends Map.Entry<String, Object>> params) {
    ...
    if (m_serverRequest != null && m_serverRequest != this) m_serverRequest.setParameters(params);
  }
  ...
  public Iterator<?> getParametersIterator() {
    return PSIteratorUtils.protectedIterator(m_params.entrySet().iterator());
  }
  ...
  public Map<String, Object> getBalancedParameters(String controlParam) {
    if (controlParam == null || controlParam.trim().length() == 0)
      throw new IllegalArgumentException("controlParam can not be null or empty.");
    ...
    if (m_params.containsKey(controlParam)) {
      ...
      else return getBalancedParameters(1);
    } else
      throw new IllegalStateException(
  ...
  public int getMaxListValuesSize() {
    ...
    for (Object value : m_params.values()) {
      if (value instanceof List) {
        ...
        if (max < listValue.size()) max = listValue.size();
      }
    }
  ...
  PSResponse getResponse(boolean inError) {
    if (m_response == null) {
      ...
      m_response.setGeneralHeader(
          PSResponse.GHDR_DATE, PSDateFormatter.formatDateForHttp(new java.util.Date()));
  ...
  public PSSecurityToken getSecurityToken() {
    ...
    if (!hasUserSession()) throw new IllegalStateException("User session does not exist");
  ...
  public String getAppName() {
    ...
    if (!PSServer.isCaseSensitiveURL()) reqRoot = reqRoot.toLowerCase();

    // will be lowercase if not case sensitive
    String rxRoot = PSServer.getRequestRoot();
    // string leading "/"
    if (rxRoot.startsWith(URL_SEP) && rxRoot.length() > 1) rxRoot = rxRoot.substring(1);
    String appName = null;
  ...
  public static PSRequest getContextForRequest(boolean forceLocal, boolean useInternalSession) {
    ...
    if (forceLocal) {
      ...
      // proxy request
      String proxyPort =
          PSServer.getProperty("proxyPort", Integer.toString(servletReq.getServerPort()));

      String proxyScheme = PSServer.getProperty("proxyScheme", servletReq.getScheme());
    ...
    if (useInternalSession) {
      ...
      if (key != null) {
        sess = PSUserSessionManager.getUserSession(key);
      }
      if (sess == null) {
        ...
        synchronized (PSRequest.class) {
          ...
          } else sess = PSUserSessionManager.getUserSession(newKey);
        }
      }
  ...
  private void deletePurgableTempFile(Iterator<Object> objList) {
    while (objList.hasNext()) {
      ...
      if (value instanceof PSPurgableTempFile) ((PSPurgableTempFile) value).release();
    }
  }
  ...
  public Document getContentItemStatus(String contentid) {
    ...
    if (m_contentItemStatusCache == null) return null;
  ...
  private static PSUserEntry createInternalUserEntry() {
    ...
    try {
      ...
      Set<String> roles =
          roleMgr.getUserRoles(
              PSTypedPrincipal.createSubject(PSSecurityProvider.INTERNAL_USER_NAME));
      ...
      log.error(
          "Failed to load roles for internal server user. Error: {}",
  ...
  public PSSubject getOriginalSubject() {
    ...
    PSUserEntry[] users =
        originalRequest.getSecurityToken().getUserSession().getAuthenticatedUserEntries();

    // there is always only one entry
    PSUserEntry user = users[0];

    PSAttributeList attributes = new PSAttributeList(user.getAttributes());
    return new PSGlobalSubject(user.getName(), PSSubject.SUBJECT_TYPE_USER, attributes);
  }
  ...
  public PSRequest getOriginalRequest() {
    return isOriginalRequest() ? this : m_cloneParentRequest.getOriginalRequest();
  }
  ...
  public void setOriginatingRelationship(PSRelationship relationship) {
    ...
    if (isOriginalRequest()) {
      if (m_originatingRelationship == null) m_originatingRelationship = relationship;
  ...
  public PSRequest getServerRequest() {
    ...
    if (m_serverRequest != null) return m_serverRequest;
    ...
    if (PSSecurityProvider.INTERNAL_USER_NAME.equals(
      ...
      PSSecurityFilter.updateHttpSession(m_serverRequest.getServletRequest(), newSession);
    }
  ...
  public String getProtocol() {
    if (getServletRequest().isSecure()) return PSUserSession.PROTOCOL_HTTPS;
    else return PSUserSession.PROTOCOL_HTTP;
  }
  ...
  private List<Object> createList(Object value, int size) {
    ...
    for (int i = 0; i < size; i++) newEntry.add(value);
  ...
  public void parseBody() throws PSRequestParsingException, IOException {
    ...
    PSInputStreamReader reader =
        new PSInputStreamReader(in, false, PSContentParser.MIN_PUSHBACK_BUF_SIZE);
