...
public class PSServerLockManager {
  ...
  static synchronized PSServerLockManager createInstance() {
    if (ms_lockMgr != null) throw new IllegalStateException("Server lock manager already created");
  ...
  public static synchronized PSServerLockManager getInstance() {
    if (ms_lockMgr == null) throw new IllegalStateException("Server lock manager not yet created");
  ...
  public Element getServerLockStatus(Document doc) {
    ...
    while (locks.hasNext()) {
      ...
      lockEl.setAttribute("created", m_dateFormat.format(lock.getLockTime()));

      Element resourcesEl = PSXmlDocumentBuilder.addEmptyElement(doc, lockEl, "Resources");
      int[] resources = lock.getLockedResources();
      for (int i = 0; i < resources.length; i++) {
        PSXmlDocumentBuilder.addElement(
            doc, resourcesEl, "Resource", getResourceName(resources[i]));
      }
    }
