...
/**
 * Tracks the results of the execution of a single action set. Each action in the set has its status
 * tracked: success, failure, or skipped. The results can be reported as XML.
 *
 * @see PSActionSet
 */
public class PSActionSetResult {
  /**
   * Constructs a new <code>PSActionSetResult</code> to track the execution of the provided action
   * set. Initializes each action's status to {@link #SKIPPED_STATUS}.
   *
   * @param actionSet set whose results will be tracked, not <code>null</code>.
   * @param contentEditorUrl the URL of the content editor receiving the actions, not <code>null
   *     </code> or empty.
   * @throws IllegalArgumentException if <code>actionSet</code> is <code>null
   * </code>, or if <code>contentEditorUrl</code> is <code>null</code> or empty.
   */
  public PSActionSetResult(PSActionSet actionSet, String contentEditorUrl) {
    if (actionSet == null) throw new IllegalArgumentException("actionSet may not be null");
    if (contentEditorUrl == null || contentEditorUrl.trim().length() == 0)
      throw new IllegalArgumentException("contentEditorUrl may not be null or empty");
  ...
  public Document toXml() {
    ...
    for (String actionName : m_actionNames) {
      ...
      if (resultObject instanceof Exception) {
        ...
        PSXmlDocumentBuilder.addElement(doc, errorEl, "Description", e.getMessage());
        StringWriter callstack = new StringWriter();
        e.printStackTrace(new PrintWriter(callstack));
        PSXmlDocumentBuilder.addElement(doc, errorEl, "Callstack", callstack.toString());
  ...
  /**
   * @return the XML representation of these results as a string, may be <code>null</code> if any IO
   *     exception occurs
   * @see #toXml()
   */
   ...
   *
   * @param actionName name of the action that failed, not <code>null</code> or empty.
   * @param error the error generated by the failed action, may be <code>null</code> if unknown.
   * @throws IllegalArgumentException if actionName is <code>null</code> or empty.
   */
   ...
   *
   * @param actionName name of the action was skipped, not <code>null</code> or empty.
   * @throws IllegalArgumentException if actionName is <code>null</code> or empty.
   */
   ...
   *
   * @param actionName name of the action that succeeded, not <code>null</code> or empty.
   * @param statistics XML describing the number of rows inserted, updated, etc. May be <code>null
   *     </code> if not supported.
   * @throws IllegalArgumentException if actionName is <code>null</code> or empty.
   */
  ...
  /**
   * Gets the result assigned to the specified action. Package protection so this method can be
   * called by the unit test.
   *
   * @param actionName name of the action, not <code>null</code> or empty.
   * @return the results associated with the specified action, not <code>null</code>.
   * @throws IllegalArgumentException if actionName is <code>null</code> or empty or if no results
   *     are registered for that name.
   */
  ...
  /**
   * Inner-class to be used as the value of the action results Map since two values need to be
   * rememebered -- a status flag (success, skipped, or failed) and an object associated with that
   * status (an Exception if failed, for example).
   */
  public class ActionResult {
    /**
     * Construct a new result object to track the specified status and object.
     *
     * @param status the type of result, one of the <code>{SUCCESS,SKIPPED,FAILED}_STATUS</code>
     *     values.
     * @param result an object that elaborates on the status, an <code>Exception</code> if the
     *     status is failed, or a <code>Document</code> if the status is success. May be <code>null
     *     </code>.
     * @throws IllegalArgumentException if <code>status</code> is not one of the <code>
     *     {SUCCESS,SKIPPED,FAILED}_STATUS</code> values.
     */
    public ActionResult(int status, Object result) {
      if (status != SUCCESS_STATUS && status != FAILED_STATUS && status != SKIPPED_STATUS)
        throw new IllegalArgumentException("must use one of the _STATUS codes");
     ...
     * </code> if the status is failed, <code>null</code> if the status is skipped, and a <code>
     *     Document</code> if the status is success. May be <code>null</code> if no object has been
     *     assigned.
     */
     ...
     *
     * @return the status assigned to this result, one of the <code>{SUCCESS,SKIPPED,FAILED}_STATUS
     *     </code> values.
     */
    ...
    /**
     * Gets the string representation of the status flag assigned to this result.
     *
     * @return the string representation of the status flag assigned to this result, never <code>
     *     null</code> or empty.
     */
    ...
    /**
     * Result could be PSExecStatistics (success) or Exception (failure) or <code>null</code>
     * (skipped).
     */
  ...
  /**
   * Name of the action set these results are for. Used in generating the result XML. Assigned in
   * the ctor, and never <code>null</code> after.
   */
  private String m_actionSetName;

  /**
   * The URL of the content editor to which the action set is being applied. Assigned in the ctor,
   * and never <code>null</code> or empty after.
   */
  private String m_originalHref;

  /**
   * Maintains the result for each action, keyed by the action's name (<code>String</code>). The
   * value is the inner-class <code>ActionResult
