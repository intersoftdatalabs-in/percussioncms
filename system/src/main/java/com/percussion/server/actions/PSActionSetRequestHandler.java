...
public class PSActionSetRequestHandler implements IPSLoadableRequestHandler {
  ...
  public void init(Collection requestRoots, InputStream cfgFileIn) throws PSServerException {
    ...
    if (cfgFileIn != null) {
      try {
            ...
            if (child instanceof Element)
              try {
                ...
                if (this.actionSets.containsKey(actionSet.getName()))
                  ...
                  PSConsole.printMsg(HANDLER, "Initialization completed: " + actionSet.getName());
        ...
        PSConsole.printMsg(HANDLER, "Action sets request handler initialization completed.");
        ...
        PSConsole.printMsg(
            HANDLER, "Failed to initialize action sets request handler: " + e.getMessage());
      }
    }
  }
  ...
  private String getContentEditorURL(PSRequest request)
    ...
    PSInternalRequest ir =
        PSServer.getInternalRequest(catalogerAppRsrcName, request, params, false);
    if (ir != null) {
      try {
  ...
  public void processRequest(PSRequest request) {
    ...
    if (actionSets == null)
      throw new IllegalStateException("Must call the init method before processing requests");
    ...
    try {
      ...
      PSLogError err =
          new PSUnknownProcessingError(
              0, request.getUserSessionId(), e.getErrorCode(), e.getErrorArguments());
      errorHandler.reportError(response, err);
    } catch (Exception e) {
      // wrap the exception in a PSLogError so it can be returned
      PSLogError err =
