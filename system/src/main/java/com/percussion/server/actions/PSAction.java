...
public class PSAction {
  ...
  private void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    if (ignoreErrorAttrValue != null && ignoreErrorAttrValue.equals("yes")) m_ignoreError = true;

    int searchChildrenFlags =
        PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN | PSXmlTreeWalker.GET_NEXT_RESET_CURRENT;
    int searchSiblingsFlags =
    ...
    if (elem != null
        && !elem.getNodeName().equals(PSParam.XML_NODE_NAME)
    ...
    if (elem != null && elem.getNodeName().equals(PSExtensionCallSet.ms_NodeType))
  ...
  public String getUrl(String ceHref, PSExecutionData execData) throws PSDataExtractionException {
    if (ceHref == null || ceHref.trim().length() == 0)
      throw new IllegalArgumentException("content editor href may not be null or empty");
