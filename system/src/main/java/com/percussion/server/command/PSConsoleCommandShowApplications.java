...
/**
 * The PSConsoleCommandShowApplications class implements processing of the "show applications"
 * console command.
 *
...
public class PSConsoleCommandShowApplications extends PSConsoleCommand {
   ...
   */
  public PSConsoleCommandShowApplications(String cmdArgs) throws PSIllegalArgumentException {
    super(null);
    ...
    if ((cmdArgs == null) || (cmdArgs.length() == 0))
      ...
      if ("all".equals(cmdArgs)) m_statusCodesToShow = SHOW_STATUS_ALL;
      else if ("active".equals(cmdArgs)) m_statusCodesToShow = SHOW_STATUS_ACTIVE;
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    ...
    root = PSXmlDocumentBuilder.addEmptyElement(respDoc, root, "Applications");

    Element node;
    PSApplicationSummary[] apps = PSServer.getApplicationSummaries(request);

    for (int i = 0; i < apps.length; i++) {
      PSApplicationSummary sum = apps[i];
      // make sure we're allowed to log this type
      if (!sum.isActive()) {
        if ((m_statusCodesToShow & SHOW_STATUS_DISABLED) == 0) continue;
      } else if ((m_statusCodesToShow & SHOW_STATUS_ACTIVE) == 0) continue;

      node = PSXmlDocumentBuilder.addEmptyElement(respDoc, root, "Application");
      node.setAttribute("id", "" + sum.getId());
      node.setAttribute("active", sum.isActive() ? "yes" : "no");
      node.setAttribute("enabled", sum.isEnabled() ? "yes" : "no");
      PSXmlDocumentBuilder.addElement(respDoc, node, "name", sum.getName());
    }
  ...
  private static final int SHOW_STATUS_ALL = SHOW_STATUS_ACTIVE + SHOW_STATUS_DISABLED;

  private int m_statusCodesToShow;

  /** allow package members to see our command name */
  static final String ms_cmdName = "show applications";
}
