...
public class PSConsoleCommandShowStatusSearch extends PSConsoleCommand {
  ...
  /**
   * Queries the search engine for its status and appends the results to the standard doc. The
   * modified dtd is as follows:
   *
   * <pre><code>
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    try {
      ...
      throw new PSConsoleCommandException(e.getErrorCode(), e.getErrorArguments());
    }
  }

  /**
   * Checks if the full text search engine is enabled and if it isn't, builds a document w/
   * appropriate messages.
   *
   * <p>Note: this is made static so it is available to other search console command handlers.
   *
   ...
   * null</code> or empty.
   * @return <code>null</code> if full text search is enabled, otherwise a document suitable for
   *     return from an <code>execute</code> method.
   */
  static Document getDisabledDoc(PSRequest req, String commandName) {
    ...
    Locale loc;
    if (req != null) loc = req.getPreferredLocale();
    else loc = Locale.getDefault();

    String termMsg = PSErrorManager.createMessage(resultCode, null, loc);
   ...
   */
  private static final String COMMAND_NAME = "status search";
}
