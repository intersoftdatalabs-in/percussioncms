...
public class PSConsoleCommandDebugSearch extends PSConsoleCommand {
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    Document doc = PSConsoleCommandShowStatusSearch.getDisabledDoc(request, COMMAND_NAME);
    ...
    PSServer.getServerConfiguration().getSearchConfig().setTraceEnabled(enabled);
    String[] args = {
      enabled ? "on" : "off",
    };
