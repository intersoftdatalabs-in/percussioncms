...
public class PSConsoleCommandTrace extends PSConsoleCommand {
  ...
  public PSConsoleCommandTrace(String cmdArgs) throws PSIllegalArgumentException {
    ...
    if ((cmdArgs == null) || (cmdArgs.length() == 0)) m_help = true;
    else {
      StringTokenizer tok = new StringTokenizer(cmdArgs, " ");

      // check first arg
      if (!tok.hasMoreTokens()) m_help = true;
      else {
        first = tok.nextToken();
        if (first.toLowerCase().equals("default")) m_default = true;
        else if (first.toLowerCase().equals("help")) m_help = true;
      }

      // handle each case
      if (m_default) {
        // next arg should be app name
        if (!tok.hasMoreTokens()) isValid = false;
        else {
          m_appname = tok.nextToken();
          if (tok.hasMoreTokens()) isValid = false; // should not be any more
        }
      } else if (!m_help) // don't need to do anything more if help
      {
        ...
        if (first.toLowerCase().startsWith("0x") || first.equals("0")) {
          ...
          do {
            try {
              if (next.toLowerCase().startsWith("0x")) flag = next.substring(2);
              else flag = next;
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    ...
    try {
      ...
      // case appname and no flags
      {
        ...
        root = PSXmlDocumentBuilder.addEmptyElement(respDoc, root, "PSXTraceFlag");
