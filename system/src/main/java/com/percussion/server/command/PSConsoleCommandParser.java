...
public class PSConsoleCommandParser {
  ...
  public static IPSConsoleCommand parse(String command) throws PSIllegalArgumentException {
    ...
    while (true) {
      ...
      if (cmdBase.length() == 0) cmdObject = ms_cmdSet.get(cmdName);
      else cmdObject = ms_cmdSet.get(cmdBase + " " + cmdName);
      if (cmdObject == null) {
  ...
  static {
    ...
    ms_cmdSet.put(
        "",
        "start, restart, stop, show, log, trace, flush, "
    ...
    ms_cmdSet.put(
        "start application", com.percussion.server.command.PSConsoleCommandStartApplication.class);

    ms_cmdSet.put("start cache", com.percussion.server.command.PSConsoleCommandStartCache.class);
    ...
    ms_cmdSet.put("reload i18nresources", PSConsoleCommandReloadI18nResources.class);

    ms_cmdSet.put("debug i18n", PSConsoleCommandDebugI18n.class);

    ms_cmdSet.put("debug search", PSConsoleCommandDebugSearch.class);
    ...
    ms_cmdSet.put(
        "restart cache", com.percussion.server.command.PSConsoleCommandRestartCache.class);

    // build the sub-command list for stop
    ms_cmdSet.put("stop", "server, application, cache");

    // build the actual command handlers for stop
    ms_cmdSet.put("stop server", com.percussion.server.command.PSConsoleCommandStopServer.class);

    ms_cmdSet.put(
        "stop application", com.percussion.server.command.PSConsoleCommandStopApplication.class);

    ms_cmdSet.put("stop cache", com.percussion.server.command.PSConsoleCommandStopCache.class);
    ...
    ms_cmdSet.put(
        "show status server", com.percussion.server.command.PSConsoleCommandShowStatusServer.class);

    ms_cmdSet.put(
        "show status objectstore",
    ...
    ms_cmdSet.put("show status search", PSConsoleCommandShowStatusSearch.class);

    ms_cmdSet.put(
        "show applications", com.percussion.server.command.PSConsoleCommandShowApplications.class);

    ms_cmdSet.put("show version", com.percussion.server.command.PSConsoleCommandShowVersion.class);
    ...
    ms_cmdSet.put("log flush", com.percussion.server.command.PSConsoleCommandLogFlush.class);

    ms_cmdSet.put("log dump", com.percussion.server.command.PSConsoleCommandLogDump.class);

    // build the sub-command list for trace
    ms_cmdSet.put("trace", com.percussion.server.command.PSConsoleCommandTrace.class);
    ...
    ms_cmdSet.put("dump resources", PSConsoleCommandDumpServerResources.class);
    ms_cmdSet.put("dump usersessions", PSConsoleCommandDumpUserSessions.class);
    ms_cmdSet.put("dump handlers", PSConsoleCommandDumpHandlers.class);
    ms_cmdSet.put("dump cache", PSConsoleCommandDumpCache.class);
    ms_cmdSet.put(
        PSConsoleCommandDumpFolderRelationshipCache.ms_cmdName,
        PSConsoleCommandDumpFolderRelationshipCache.class);
    ms_cmdSet.put(
        PSConsoleCommandDumpItemSummaryCache.ms_cmdName,
        PSConsoleCommandDumpItemSummaryCache.class);
    ms_cmdSet.put(
        PSConsoleCommandDumpDatasources.ms_cmdName, PSConsoleCommandDumpDatasources.class);
    ...
    ms_cmdSet.put("search index recreate", PSConsoleCommandSearchIndexRecreate.class);

    ms_cmdSet.put("search queue", "clear");
    ms_cmdSet.put("search queue clear", PSConsoleCommandSearchQueueClear.class);

    ms_cmdSet.put("search queue", "pause");
    ms_cmdSet.put("search queue pause", PSConsoleCommandSearchQueuePause.class);
