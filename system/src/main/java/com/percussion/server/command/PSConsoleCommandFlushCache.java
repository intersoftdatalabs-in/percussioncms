...
public class PSConsoleCommandFlushCache extends PSConsoleCommandCache {
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    ...
    if (m_cmdArgs != null && m_cmdArgs.length() != 0) {
      ...
      String cmdArgs = "";
      StringTokenizer st = new StringTokenizer(m_cmdArgs, CACHE_ARG_SEPARATOR, false);
      ...
      if (st.hasMoreTokens()) {
        ...
        if (handlerType.equalsIgnoreCase("hibernate")) {
          ...
          for (int i = 0; i < requiredCacheKeys; i++) keys.put(keyNames[i], null);
          ...
          while (st.hasMoreTokens() && iKey < requiredCacheKeys) {
            ...
            if (token.equals(CACHE_KEY_SEPARATOR)) iKey++; // will skip over empty values as well
            else keys.put(keyNames[iKey], token);
          }
