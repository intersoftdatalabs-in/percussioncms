...
public abstract class PSConsoleCommandSearchIndex extends PSConsoleCommand {
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    Document doc = PSConsoleCommandShowStatusSearch.getDisabledDoc(request, getCommandName());
    ...
    doc = getResultsDocument(request, getCommandName(), code, resultArgs);
    return doc;
  }
  ...
  protected PSKey[] getIds(String type) throws PSConsoleCommandException {
    ...
    try {
      ...
      if (typeId < 0 && type.length() == 0) {
        ...
        ids = PSItemDefManager.getInstance().getAllContentTypeIds(PSItemDefManager.COMMUNITY_ANY);
      } else {
        ids = new long[1];
        if (typeId < 0) {
          ids[0] = PSItemDefManager.getInstance().contentTypeNameToId(type);
        } else {
