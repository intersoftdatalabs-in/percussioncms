 ...
 * @author paulhoward
 */
public class PSConsoleCommandSearchIndexType extends PSConsoleCommandSearchIndex {
  /**
  ...
  // see base class
  @Override
  public PSKey[] doExecute(@SuppressWarnings("unused") PSRequest request, PSKey[] ctypeIds)
    ...
    for (int i = 0; i < tot; i++) {
      int result =
          PSSearchIndexEventQueue.getInstance().indexContentType(ctypeIds[i].getPartAsInt());
      if (result == -1) {
        hadBadType = true;
        tmpResults[cTypeCount++] = ctypeIds[i];
      } else m_indexCount += result;
    }
  ...
  protected String[] getResultArgs(PSKey[] successes) {
    ...
    if (hadBadType) args[1] = super.getResultArgs(successes)[0];
