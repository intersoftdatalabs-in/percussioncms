...
public class PSConsoleCommandFlushDbmd extends PSConsoleCommand {
  ...
  public PSConsoleCommandFlushDbmd(String cmdArgs) throws PSIllegalArgumentException {
    ...
    if (!isError) {
      ...
      if (m_origin != null && (m_table == null || m_table.trim().length() == 0)) {
        isError = true;
        badArg = cmdArgs;
      }
    }
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    ...
    if (m_help) {
      ...
      if (m_table != null) {
        ...
        PSDatabaseMetaData dbmd = PSMetaDataCache.getCachedDatabaseMetaData(connInfo);
        ...
        if (flushed) msg = "Table cache has been flushed";
        else msg = "No table meta data located to flush";
      } else {
        // flush the whole db
        boolean flushed = PSMetaDataCache.flushDatabaseMetaData(m_datasource);

        if (flushed) msg = "Database cache has been flushed";
        else msg = "No database meta data located to flush";
      }
    }
