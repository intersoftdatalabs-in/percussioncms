...
public class PSConsoleCommandDumpDatasources extends PSConsoleCommand {
  // see base class
  public PSConsoleCommandDumpDatasources(String cmdArgs) throws PSIllegalArgumentException {
    super(cmdArgs);
  ...
  public Document execute(PSRequest request) throws PSConsoleCommandException {
    ...
    Element datasources = PSXmlDocumentBuilder.addEmptyElement(doc, root, "datasources");
    ...
    try {
      for (String dsName : dsMgr.getDatasources()) {
        PSConnectionDetail detail = dsMgr.getConnectionDetail(new PSConnectionInfo(dsName));

        // detail should not be null since the mgr gave us the name
        if (detail == null) {
          throw new RuntimeException("No conn detail found for: " + dsName);
        }

        Element dsEl = PSXmlDocumentBuilder.addEmptyElement(doc, datasources, "datasource");

        dsEl.setAttribute("name", dsName);
        dsEl.setAttribute("isCmsRepository", dsName.equals(defaultDs) ? "yes" : "no");
        PSXmlDocumentBuilder.addElement(
            doc, dsEl, "jndiDatasourceName", detail.getDatasourceName());
        PSXmlDocumentBuilder.addElement(doc, dsEl, "jdbcUrl", detail.getJdbcUrl());
        PSXmlDocumentBuilder.addElement(doc, dsEl, "database", detail.getDatabase());
        PSXmlDocumentBuilder.addElement(doc, dsEl, "schema", detail.getOrigin());
      }
