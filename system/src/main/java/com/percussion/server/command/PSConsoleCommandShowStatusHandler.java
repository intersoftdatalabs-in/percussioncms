...
public abstract class PSConsoleCommandShowStatusHandler extends PSConsoleCommand {
  ...
  public void createStatistics(PSHandlerStatistics stats, Document doc, Element root) {
    if (stats != null) {
      Element node = PSXmlDocumentBuilder.addEmptyElement(doc, root, "PSXStatistics");

      node = PSXmlDocumentBuilder.addEmptyElement(doc, root, "ElapsedTime");
      long timer = (new Date().getTime()) - stats.getStartTime().getTime();
      addTimeElement(doc, node, timer);

      node = PSXmlDocumentBuilder.addEmptyElement(doc, root, "Counters");
      PSXmlDocumentBuilder.addElement(
          doc, node, "eventsProcessed", String.valueOf(stats.getSuccessfulEventCount()));
      PSXmlDocumentBuilder.addElement(
          doc, node, "eventsFailed", String.valueOf(stats.getFailedEventCount()));
      PSXmlDocumentBuilder.addElement(
          doc, node, "eventsPending", String.valueOf(stats.getPendingEventCount()));
      PSXmlDocumentBuilder.addElement(doc, node, "cacheHits", String.valueOf(stats.getCacheHits()));
      PSXmlDocumentBuilder.addElement(
          doc, node, "cacheMisses", String.valueOf(stats.getCacheMisses()));

      node = PSXmlDocumentBuilder.addEmptyElement(doc, root, "Timers");
      addTimeElement(
          doc,
  ...
  private void addTimeElement(Document doc, Element node, int timer) {
    ...
    if ((timer == Integer.MIN_VALUE) || (timer == Integer.MAX_VALUE)) lTimer = 0;
    else lTimer = (long) timer;

    addTimeElement(doc, node, lTimer);
  }

  private void addTimeElement(Document doc, Element node, long timer) {
    PSXmlDocumentBuilder.addElement(doc, node, "days", String.valueOf(timer / MILLIS_IN_DAY));
    timer = timer % MILLIS_IN_DAY;
    PSXmlDocumentBuilder.addElement(doc, node, "hours", String.valueOf(timer / MILLIS_IN_HOUR));
    timer = timer % MILLIS_IN_HOUR;
    PSXmlDocumentBuilder.addElement(doc, node, "minutes", String.valueOf(timer / MILLIS_IN_MIN));
    timer = timer % MILLIS_IN_MIN;
    PSXmlDocumentBuilder.addElement(doc, node, "seconds", String.valueOf(timer / MILLIS_IN_SEC));
    timer = timer % MILLIS_IN_SEC;
    PSXmlDocumentBuilder.addElement(doc, node, "milliseconds", String.valueOf(timer));
  }
