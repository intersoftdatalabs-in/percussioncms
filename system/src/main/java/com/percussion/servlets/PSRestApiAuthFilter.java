...
public class PSRestApiAuthFilter implements ContainerRequestFilter {
  ...
  public void filter(ContainerRequestContext requestContext) throws IOException {
    ...
    if (!auth || !allowedURLParam) {
      ...
      try {
        ...
        log.error("Unable to set the default error URI: {}", PSExceptionUtils.getMessageForLog(e));
  ...
  String get404Content(URI uri) {
    ...
    try (InputStream is =
        Thread.currentThread().getContextClassLoader().getResourceAsStream("/default-error.html")) {
      if (is != null) {
