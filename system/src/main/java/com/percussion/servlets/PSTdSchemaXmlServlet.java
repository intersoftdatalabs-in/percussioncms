...
public class PSTdSchemaXmlServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    boolean allowChanges =
        StringUtils.defaultString(request.getParameter("allowchanges"), "false")
  ...
  private boolean checkTablesAreValid(String[] tables) {
    for (String t : tables) {
      if (!SecureStringUtils.isValidTableOrColumnName(t)) return false;
    }
    return true;
  }
  ...
  private String createTableSchemaXml(
    ...
    try {
      ...
      PSJdbcDataTypeMap dataTypeMap =
          new PSJdbcDataTypeMap(dbmsDef.getBackEndDB(), dbmsDef.getDriver(), null);
  ...
  private List getReferencedTables(Element node) {
    ...
    if (foreignKeys != null) {
      for (int i = 0; i < foreignKeys.getLength(); i++) {
        ...
        NodeList externalTables = foreignKey.getElementsByTagName("externalTable");
        if (externalTables != null) {
          for (int j = 0; j < externalTables.getLength(); j++) {
            String externalTableName = PSXmlTreeWalker.getElementData(externalTables.item(j));
  ...
  private void pushResponse(
    ...
    if (respCode == 500) httpResponse.sendError(respCode, resp);
