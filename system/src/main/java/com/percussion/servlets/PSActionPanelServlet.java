...
public class PSActionPanelServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      PSCommunity com =
          rmgr.loadCommunity(new PSGuid(PSTypeEnum.COMMUNITY_DEF, sum.getCommunityId()));
    ...
    if (!accessible) {
      ...
      RequestDispatcher disp = request.getRequestDispatcher("/ui/actionpage/noaccess.jsp");
    ...
    if (siteinfo.size() == 0) {
      ...
      if (siteinfo.size() == 1) {
        ...
        fid = (Integer) session.getAttribute(RXAP + contentid + "_folderid");
    ...
    if (showpanel) {
      if (targetType == TargetType.ACTIVE_ASSEMBLY && sid != null && fid != null) {
      ...
      RequestDispatcher disp = request.getRequestDispatcher("/ui/actionpage/panel.jsp");
      disp.forward(request, response);
    } else {
      request.setAttribute("siteinfo", siteinfo);
      RequestDispatcher disp = request.getRequestDispatcher("/ui/actionpage/choose.jsp");
      disp.forward(request, response);
    }
  }
  ...
  private IPSGuid calculateDefaultTemplate(long ctypeId, long siteId) {
    ...
    IPSGuid tguid = null;
    try {
      String ctypeName = PSItemDefManager.getInstance().contentTypeIdToName(ctypeId);
      List<PSAssemblyTemplateWs> templates = mgr.loadAssemblyTemplates(null, ctypeName);
      List<PSAssemblyTemplateWs> siteTemplates = new ArrayList<>();
      IPSGuid siteGuid = PSGuidManagerLocator.getGuidMgr().makeGuid(siteId, PSTypeEnum.SITE);
      ...
      for (PSAssemblyTemplateWs t : siteTemplates) {
        if (t.getTemplate().getPublishWhen() == IPSAssemblyTemplate.PublishWhen.Default) {
            ...
            findTemplate(
                ...
                new Filter() {
                  ...
                  public boolean accept(IPSAssemblyTemplate t) {
                    return t.getPublishWhen() == IPSAssemblyTemplate.PublishWhen.Always;
                  }
                });
      }
      if (tguid == null) {
        tguid =
            findTemplate(
                ...
                new Filter() {
                  @Override
                  public boolean accept(IPSAssemblyTemplate t) {
                    return t.getOutputFormat() == IPSAssemblyTemplate.OutputFormat.Page;
                  }
                });
      }
      if (tguid == null) {
  ...
  private void sort(List<IPSAssemblyTemplate> templates) {
    Collections.sort(
        templates,
        new Comparator<IPSAssemblyTemplate>() {
  ...
  private String getTargetUrl(TargetType targetType, String contentid) {
    ...
    if (targetType == TargetType.ACTIVE_ASSEMBLY) {
      url.append("assembler/render?sys_authtype=0&sys_command=editrc&sys_context=0");
    } else url.append("sys_ActionPage/Entries.html");
  ...
  private TargetType getDispatchTargetType() {
    ...
    if (result == null) result = TargetType.getDefault();
    return result;
  }
