...
public class PSSecurityFilter implements Filter {
  ...
  public void init(FilterConfig config) throws ServletException {
    ...
    if (!StringUtils.isEmpty(p)) ms_policy = p;
    if (!StringUtils.isEmpty(l)) ms_loginForm = l;
  ...
  void initSecurityConfiguration(String rootDir) throws ServletException {
    // locate config files
    File configDir =
        new File(new File(rootDir, PSServletUtils.WEB_INF), PSServletUtils.SYS_CONFIG_DIR);
    File userConfigDir = new File(configDir, PSServletUtils.USER_CONFIG_DIR);
    m_systemSecurityConfig = new File(configDir, "security/system-security-conf.xml");
    m_userSecurityConfig = new File(userConfigDir, "security/user-security-conf.xml");
    if (!m_userSecurityConfig.exists())
      ms_log.info("User security config file not found: {}", m_userSecurityConfig);
    loadConfigs();
  }
  ...
  static boolean loadConfig(File securityConfig, boolean isUser, List<SecurityEntry> configs)
    ...
    try (InputStream in = new FileInputStream(securityConfig)) {
      ...
      boolean forceSecure = "yes".equals(root.getAttribute("forceSecureLogin"));
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (StringUtils.isEmpty(pathVal)) continue;
        String name = path.getNodeName();
        String type = path.getAttribute("authType");
        SecurityEntry entry =
            new SecurityEntry(
                isUser, name.equals("path"), AuthType.valueOf(type.toUpperCase()), pathVal);
        configs.add(entry);
      }
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try {
      ...
      if (request instanceof HttpServletRequest) {
        ...
        if (httpReq.getServletPath().equals("/__validateSession__")) ssorequest = true;
      }
      if (!ssorequest) {
      ...
      if (needsReset) PSRequestInfoBase.resetRequestInfo();
    }
  }
  ...
  private boolean httpsRequired() {
    ...
    if (serverProps != null) {
      ...
      if (StringUtils.isNotBlank(httpsProperty)
          && (httpsProperty.equalsIgnoreCase("true") || httpsProperty.equalsIgnoreCase("yes"))) {
   ...
   */
  private HttpServletRequest authorize(HttpServletRequest request, HttpServletResponse response)
      throws IOException, ServletException {
    ms_log.debug("enter authorize on thread {}", Thread.currentThread().getName());
    ...
    synchronized (mutex) {
      ...
      if (PSServer.getServerProps()
              .getProperty("disableCrossSiteRequestForgeryCheck", "false")
              .equalsIgnoreCase("true")
          || !checkForCrossSiteRequestForgery(request)
          || isBehindProxy) {

        if (handleSessionCheck(request, response)) return null;
        ...
        if (!StringUtils.isBlank(sessId)) {
          HttpServletRequest authReq = handleExistingUserSession(request, response, sessId);
          if (authReq == null) sendAuthenticationError(request, response);
      ...
      if (authType.equals(AuthType.ANONYMOUS)) {
        ...
        if (sub == null) sess.setAttribute(SUBJECT, new Subject());
  ...
  private String getSessionIdFromRequest(HttpServletRequest request) {
    ...
    if (StringUtils.isEmpty(sessionId)) {
      sessionId =
          SecureStringUtils.srp((String) request.getAttribute(IPSHtmlParameters.SYS_SESSIONID));
    }
  ...
  private boolean handleSessionCheck(HttpServletRequest request, HttpServletResponse response)
    ...
    if (checkString == null) checkString = request.getServletPath();
    ...
    if (statusRequest || statusSupportFiles) {
      ...
      long lastActivity =
          NumberUtils.toLong(
              SecureStringUtils.stripAllLineBreaks(request.getParameter("lastActivity")), -1);
      ...
      if (statusRequest) {
        ...
        if (lastActivity >= 0) userSession.setIdleOffset(lastActivity);
        ...
        long warning =
            warning_s > 0 ? PSServer.getServerConfiguration().getUserSessionWarning() * 1000L : -1;
        ...
        try {
          ...
          ms_log.error(
              "Invalid json object for sessioncheck. Error: {}",
  ...
  private boolean checkForCrossSiteRequestForgery(HttpServletRequest request) {
    ...
    try {
      ...
      ms_log.warn(
          "Invalid URL detected in request: URI: {} Error: {}. NOTE: These warnings may be"
              + " generated by an active application security scan.",
          reqURL,
          PSExceptionUtils.getMessageForLog(e));
      return true;
    }

    if (reqURI.getPath().equals(request.getContextPath() + "/login")) return false;

    // requested login from the CMS-API
    if (!(reqURI.getPath().equals(request.getContextPath() + "/v8/security/session"))
        && request.getServletPath().startsWith(ms_externalApiRequests.get(0))) return false;

    // If Referer exists and does not match request then go to login.
    if (!requestHeaderMatchesRequest(reqURI, request.getHeader("Referer"))) return true;
   ...
   */
  AuthType calculateAuthType(HttpServletRequest request) throws ServletException {
    // see if a login request
    if (isLoginRequest(request)) return AuthType.BASIC;
    ...
    if (request.getPathInfo() != null) url = url + request.getPathInfo();
    if (url.startsWith("/Rhythmyx")) url = StringUtils.substringAfter(url, "/Rhythmyx");
  ...
  private HttpServletRequest handleExistingUserSession(
    ...
    ms_log.debug(
        "{}{} found user session id specified by html param: {}",
        THREAD,
    ...
    try {
      ...
      ms_log.debug(
          "{}{} invalid user session id specified by html param: {}",
          THREAD,
          Thread.currentThread().getName(),
          sessId);
      return null;
  ...
  private void sendAuthenticationError(HttpServletRequest request, HttpServletResponse response)
    ...
    redirectToErrorPage(request, response, errorCode, errorType, errorMessages);
  }
  ...
  public static void redirectToErrorPage(
    ...
    if (response == null) throw new IllegalArgumentException(ERROR_RESPONSE_NULL);
    if (StringUtils.isBlank(errorType))
      throw new IllegalArgumentException("errorType may not be null or empty");
  ...
  private HttpServletRequest handleExternalAuth(
    ...
    try {
      HttpServletRequest authReq = authenticate(request, response, null, null);
      ms_log.debug(
          "Successfully authenticated subject on thread {} using external authentication"
              + " information",
          Thread.currentThread().getName());
      return authReq;
    } catch (LoginException e) {
      response.sendError(401, "Authentication Failed: " + e.getLocalizedMessage());
      return null;
    }
  }
  ...
  private boolean isExternallyAuthenticated(HttpServletRequest request) {
    ...
    return !StringUtils.isBlank(getCertAuth(headers)) || !StringUtils.isBlank(getAuthType(headers));
  }
  ...
  public static String getAuthType(Map<String, String> headers) {
    if (headers == null) throw new IllegalArgumentException(ERROR_HEADER_NULL);
  ...
  public static String getNormalizedHeader(String name, Map<String, String> headers) {
    ...
    for (Map.Entry<String, String> header : headers.entrySet()) {
      if (normalizedName.equals(normalizeHeaderName(header.getKey()))) return header.getValue();
    }
  ...
  private void handleFormAuth(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      ms_log.error(
          "IllegalState on session: {} Error: {}",
          sess.getId(),
          PSExceptionUtils.getMessageForLog(e));
    } catch (NullPointerException e) {
      ms_log.error(
          "Error on session: {} Error: {}", sess.getId(), PSExceptionUtils.getMessageForLog(e));
    }
  }
  ...
  private boolean isExternalApiRequest(HttpServletRequest request) throws ServletException {
    return (matching(request.getServletPath(), ms_externalApiRequests)
        && !request.getRequestURI().equals("/v8/security/session"));
  }
  ...
  private static boolean hasRhythmyxRole(PSServletRequestWrapper request) {
    ...
    List<String> rxRoles = PSRoleMgrLocator.getBackEndRoleManager().getRhythmyxRoles();
      ...
      if (space > 0) {
        ...
        if (colonIndex > 0) {
          ...
          try {
            authReq =
                authenticate(
                    request,
                    response,
                    decodedAuth.substring(0, colonIndex),
                    decodedAuth.substring(colonIndex + 1));
          } catch (LoginException e) {
          ...
          if (isLoginRequest(request)) {
            ...
            ms_log.debug(
                "Doing Basic.Return 200 response on thread {}", Thread.currentThread().getName());
            return null;
          }

          ms_log.debug(
              "Doing Basic: Successfully authenticated subject on thread {}",
              Thread.currentThread().getName());
          return authReq;
        }
      }
      ...
      if (ms_secureloginForm == null) {
        try {
          ...
          if (!url.getProtocol().equalsIgnoreCase(secureProtocol)) {
            ...
            if (sslPort != 0) {
              URL newUrl = new URL(secureProtocol, url.getHost(), sslPort, ms_loginForm);
              ms_secureloginForm = newUrl.toExternalForm();
            }
          }
        } catch (MalformedURLException e) {
          ms_log.error(
              "Failed to determine url to force secure login. Error: {}",
              PSExceptionUtils.getMessageForLog(e));
        }
      }
  ...
  private boolean isSystemRequest(String url) {
    ...
    if (url.startsWith("/user")) {
      ...
      String appRoot = url;
      if (url.startsWith("/")) appRoot = StringUtils.substringAfter(url, "/");
      ...
      if (ah != null) {
        ...
        if (!(appName.startsWith("/sys_")
            ...
            || appName.startsWith("/docs")
            || appName.startsWith("/dtd"))) {
          isSystemApp = false;
        }
      }
    }
  ...
  public static HttpServletRequest authenticate(
    ...
    sub = lc.getSubject();
    PSServletRequestWrapper tmpReq = new PSServletRequestWrapper(request, sub);
    if (!hasRhythmyxRole(tmpReq)) {
      String msg =
          "Authorization failed.  User \"" + userId + "\" is not a " + "member of a valid role.";
      ms_log.info(msg);
  ...
  public static void authenticate(HttpServletRequest request, HttpServletResponse response)
    ...
    if (rxSession == null)
      throw new LoginException("The supplied request does not contain a valid rhythmyx session.");
  ...
  public static void authenticate(HttpServletRequest request, String sessionId)
    ...
    if (rxSession == null) throw new LoginException("The supplied rhythmyx session is invalid.");

    updateHttpSession(request, rxSession);

    // now update the request
    PSRequest rxReq = getCurrentRequest();
    if (rxReq == null) throw new IllegalStateException("Rx request must be initialized");
  ...
  public static void logout(HttpServletRequest request, String sessionId) {
    ...
    PSUserSession rxSession = PSUserSessionManager.getUserSession(sessionId);
    if (rxSession != null) PSUserSessionManager.releaseUserSession(rxSession);
   ...
   */
  public static void updateHttpSession(HttpServletRequest req, PSUserSession sess) {
    if (req == null) throw new IllegalArgumentException(ERROR_REQUEST_NULL);
    if (sess == null) throw new IllegalArgumentException("sess may not be null");
    ...
    if (req instanceof PSServletRequestWrapper) ((PSServletRequestWrapper) req).setSubject(subject);
  ...
  public static boolean connectSession(PSSecurityToken token) {
    ...
    PSUserSession sess = PSUserSessionManager.getUserSession(token.getUserSessionId());

    if (sess == null) return false;

    if (!PSRequestInfoBase.isInited()) PSRequestInfoBase.initRequestInfo(new HashMap<>());

    PSRequest psreq = getCurrentRequest();
    if (psreq != null && psreq.hasUserSession() && psreq.getUserSessionId().equals(sess.getId())) {
      // same session already setup on this thread, nothing to do
      return true;
    }

    psreq = new PSRequest(token);
    psreq.setServletRequest(new PSServletRequestWrapper(psreq.getServletRequest(), null));
  ...
  public static void setRequest(PSRequest req) {
    ...
    PSRequest curReq =
        (PSRequest) PSRequestInfoBase.getRequestInfo(PSRequestInfoBase.KEY_PSREQUEST);
  ...
  public static PSRequest initRequest(HttpServletRequest req, HttpServletResponse res) {
    if (req == null) throw new IllegalArgumentException("req may not be null");
    if (res == null) throw new IllegalArgumentException("res may not be null");
  ...
  public static PSRequest getCurrentRequest() {
    ...
    return (PSRequest) PSRequestInfoBase.getRequestInfo(PSRequestInfoBase.KEY_PSREQUEST);
  }
  ...
  private static void updateUserSession(
    ...
    if (locale != null) PSRequestInfoBase.setRequestInfo(PSRequestInfoBase.KEY_LOCALE, locale);
  ...
  private static void updateRequestInfo(PSRequest req) {
    ...
    if (req.hasUserSession()) {
      ...
      String locale = (String) sess.getPrivateObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);

      String httpSessionLocale =
          (String)
              req.getServletRequest()
                  .getSession()
                  .getAttribute(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);
      if (httpSessionLocale != null && (locale == null || !locale.equals(httpSessionLocale))) {
        // Check to see if the locale is on the underlying HTTP session in the event that this is a
        // new User session
        locale = httpSessionLocale;
        sess.setPrivateObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG, locale);
      }
   ...
   */
  private static LoginContext authenticate(
      final HttpServletRequest req, final String user, final String pass) throws LoginException {
  ...
  static boolean matching(String url, List<String> patterns) throws ServletException {
    ...
    for (String pattern : patterns) {
      if (ms_matcher.doesMatchPattern(pattern, url)) return true;
    }
  ...
  protected List<String> getAllowedOriginHosts() {
    ...
    String origins =
        PSServer.getProperty(
            IPSConstants.SERVER_PROP_ALLOWED_ORIGINS,
            IPSConstants.SERVER_PROP_ALLOWED_ORIGINS_DEFAULT);

    if (origins.equalsIgnoreCase(IPSConstants.SERVER_PROP_ALLOWED_ORIGINS_DEFAULT)
        || origins.isEmpty()) {
      //
    ...
    if (!publicHost.isEmpty()) ret.add(publicHost);
  ...
  private String getHostFromURLString(String s) {
    ...
    if (s.startsWith("http")) {
      try {
        ...
        // handle bad uri
        ms_log.warn(
            "Invalid url detected in {} server.property. Skipping origin {}. Error: {}",
            IPSConstants.SERVER_PROP_ALLOWED_ORIGINS,
            s,
            PSExceptionUtils.getMessageForLog(e));
      }
