...
public class PSBrowserDetect implements Filter {
    ...
    try {
      ...
      if (request instanceof HttpServletRequest) {
        ...
        if (userAgentString != null
            && !userAgentString.equals("Jakarta Commons-HttpClient/3.1")
            && !userAgentString.startsWith("Java/")
            && !userAgentString.contains("nagios")
            && !userAgentString.contains("LogicMonitor")) {
          ...
          if (!(userAgentString.contains("rv:11") && userAgentString.contains("Windows NT 6.1"))) {
            ...
            if (showWarningMessage) {
              log.info(
                  "PSBrowserDetect: Support level for {} is not supported.",
                  userAgent.getBrowser());
            }
          }
        }
      ...
      log.error(
          "Failure parsing browser User-Agent {}, Error: {}",
  ...
  private void forwardToWarning(ServletRequest request, ServletResponse response)
