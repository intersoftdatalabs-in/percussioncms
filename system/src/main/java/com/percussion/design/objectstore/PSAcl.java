...
public class PSAcl extends PSComponent {
  ...
  private static IllegalArgumentException validateEntries(PSCollection entries) {
    if (null == entries) return new IllegalArgumentException("acl entrylist is null");

    if (0 == entries.size()) return new IllegalArgumentException("acl entrylist is empty");

    if (!com.percussion.design.objectstore.PSAclEntry.class.isAssignableFrom(
        entries.getMemberClassType())) {
      return new IllegalArgumentException(
          "coll bad content type, ACL Entry: " + entries.getMemberClassName());
    }
    ...
    for (int i = 0; i < entries.size(); i++) {
      ...
      if (null != entryHash.put(entry.getName(), Boolean.TRUE)) {
        // there was a duplicated entry
        return new IllegalArgumentException("acl entry list duplicate: " + entry.getName());
      }
    }
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, "multiMembershipBehavior", sTemp);
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      // Read merge flag
      if (hasLooped) firstFlags = nextFlags;
      else tree.setCurrent(cur);

      if (tree.getNextElement("multiMembershipBehavior", firstFlags) != null) {
        sTemp = tree.getElementData((Element) tree.getCurrent());
        if (sTemp.equals(XML_FLAG_USE_MAX)) m_mergeAccess = MULTI_ACE_GETS_MAX;
        else if (sTemp.equals(XML_FLAG_USE_MIN)) m_mergeAccess = MULTI_ACE_GETS_MIN;
        else if (sTemp.equals(XML_FLAG_MERGE_MAX)) m_mergeAccess = MULTI_ACE_GETS_MERGED_MAX;
        else if (sTemp.equals(XML_FLAG_MERGE_MIN)) m_mergeAccess = MULTI_ACE_GETS_MERGED_MIN;
      }
    } finally {
  ...
  public boolean equals(Object o) {
    ...
    PSAcl psAcl = (PSAcl) o;
    return m_mergeAccess == psAcl.m_mergeAccess && Objects.equals(m_entries, psAcl.m_entries);
  }
