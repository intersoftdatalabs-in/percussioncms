...
public class PSSecurityProviderConverter extends PSBaseDSConverter {
   ...
   */
  public void convert() throws PSUnknownNodeTypeException, PSIllegalArgumentException {
    ...
    for (int i = 0; i < legacySecProvs.size(); i++) {
      ...
      if (iterGP.hasNext()) {
        ...
        PSDirectorySet dirSet = legacyServerConfig.getDirectorySet(dirSetName);
    ...
    while (iterDS.hasNext()) {
      ...
      String emailAttrName = drSet.getRequiredAttributeName(PSDirectorySet.EMAIL_ATTRIBUTE_KEY);
      String roleAttrName = drSet.getRequiredAttributeName(PSDirectorySet.ROLE_ATTRIBUTE_KEY);
      ...
      for (int k = 0; k < drNames.length; k++) {
        ...
        if (drAttrs == null) drAttrs = new PSCollection(String.class);

        if (emailAttrName != null
            && emailAttrName.trim().length() > 0
            && !drAttrs.contains(emailAttrName)) drAttrs.add(emailAttrName);
        if (roleAttrName != null
            && roleAttrName.trim().length() > 0
            && !drAttrs.contains(roleAttrName)) drAttrs.add(roleAttrName);

        // only set with new attributes if they exist
        if (drAttrs.size() > 0) dr.setAttributes(drAttrs);
      }
    }
  }
  ...
  private String resolveToDatasource(PSLegacySecurityProviderInstance oldProvider)
    ...
    getLogger()
        .info(
            "Attempting to locate matching datasource for \""
                + oldProvider.getName()
                + "\" security provider");
