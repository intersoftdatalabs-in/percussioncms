...
public class PSJndiObjectClass {
  ...
  public PSJndiObjectClass(String objectClass, String memberAttr, int attrType) {
    if (objectClass == null || objectClass.trim().length() == 0)
      throw new IllegalArgumentException("objectClass may not be null or emtpy");

    if (memberAttr == null || memberAttr.trim().length() == 0)
      throw new IllegalArgumentException("memberAttr may not be null or emtpy");

    if (attrType != MEMBER_ATTR_DYNAMIC && attrType != MEMBER_ATTR_STATIC)
      throw new IllegalArgumentException("invalid attrType");
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSJndiObjectClass)) isMatch = false;
    else {
      PSJndiObjectClass other = (PSJndiObjectClass) obj;
      if (!this.m_objectClass.equals(other.m_objectClass)) isMatch = false;
      else if (!this.m_memberAttribute.equals(other.m_memberAttribute)) isMatch = false;
      else if (this.m_attributeType != other.m_attributeType) isMatch = false;
    }
