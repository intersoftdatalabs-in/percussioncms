...
public class PSProperty extends PSComponent {
   ...
   * true</code> to lock the property, otherwise <code>false</code>. Locked properties can not be
   *     overridden at runtime.
   * @param description the description of the property, may be <code>null
   * </code> or empty.
   * @throws IllegalArgumentException if any parameter is invalid.
   */
  public PSProperty(String name, int type, Object value, boolean lock, String description) {
    setName(name);
    setType(type);
    setValue(value);
    setLock(lock);
  ...
  /**
   * Convenience constructor for PSProperty(name, TYPE_STRING, null, false, null)}. See the link for
   * more description.
   */
   ...
   *
   * @param typeName name of the data type, may not be <code>null</code> or empty and must be one of
   *     the <code>TYPE_ENUM</code> elements.
   */
  public void setTypeName(String typeName) {
    ...
    if (type == -1) throw new IllegalArgumentException("typeName is not a valid type");
   ...
   *
   * @return <code>true</code> if the property is locked (does not allow to override in runtime),
  ...
  /**
   * Constructs this property object from its xml representation. See {@link #toXml(Document)} for
   * expected xml format.
   *
  ...
  public PSProperty(Element propertyEl) throws PSUnknownNodeTypeException {
    if (propertyEl == null) throw new IllegalArgumentException("propertyEl may not be null.");
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    m_lock = XML_BOOL_YES.equalsIgnoreCase(sourceNode.getAttribute(XML_ATTR_LOCKED));
    ...
    if (descEl != null) m_description = tree.getElementData(descEl);
  }
   ...
   * </code></pre>
   *
   * See <code>IPSComponent</code> interface for description and parameters of this method.
   */
  public Element toXml(Document doc) {
    ...
    if (m_value == null) value = "";
    else if (m_type == TYPE_BOOLEAN) {
      if (((Boolean) m_value).booleanValue()) value = XML_BOOL_YES;
      else value = XML_BOOL_NO;
    } else value = m_value.toString();

    Element valueEl = PSXmlDocumentBuilder.addElement(doc, root, XML_VALUE_NODE, value);
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSProperty)) return false;

    PSProperty t = (PSProperty) o;

    boolean equal = true;
    if (!compare(m_description, t.getDescription())) equal = false;
    else if (m_lock != t.isLocked()) equal = false;
    else if (!compare(m_name, t.getName())) equal = false;
    else if (m_type != t.getType()) equal = false;
    else if (!compare(m_value, t.getValue())) equal = false;
  ...
  /**
   * Name of the property, initialized in the constructor and may be modified through a call to
   * <code>setName(String)</code>, never <code>null</code> or empty after construction.
   */
  private String m_name;

  /**
   * The data type of the property value, initialized in the constructor and may be modified through
   * a call to <code>setType(int)</code>, will be one of TYPE_xxx values.
   */
   ...
   * through a call to <code>setLock(boolean)</code>.
   */
  private boolean m_lock = false;

  /**
   * The value of the property, initialized in the constructor and may be modified through a call to
   * <code>setDescription(String)</code>, may be <code>null</code> or empty.
   */
  ...
  /**
   * The constant to indicate 'File' data type for property value. This type represents a file path
   * as <code>String</code> relative to the server root.
   */
  public static final int TYPE_FILE = 3;

  /**
   * Enumeration that represents the allowed data types for property values. The indices represent
   * the TYPE_xxx constants correspondingly.
   */
