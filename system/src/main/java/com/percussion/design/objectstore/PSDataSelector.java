...
public final class PSDataSelector extends PSComponent {
  ...
  public PSDataSelector() {
    ...
    m_whereClauses = new PSCollection(com.percussion.design.objectstore.PSWhereClause.class);
    m_sortedColumns = new PSCollection(com.percussion.design.objectstore.PSSortedColumn.class);
  }
  ...
  public void setSelectUnique(boolean enable) {
    ...
    else if ((m_selector & DS_DISTINCT) == DS_DISTINCT) m_selector ^= DS_DISTINCT;
  }
    ...
    if (clauses != null) {
      if (!com.percussion.design.objectstore.PSWhereClause.class.isAssignableFrom(
        ...
        throw new IllegalArgumentException(
            "coll bad content type, WHERE clause: " + clauses.getMemberClassName());
      }
    }
  ...
  public void setSortedColumns(com.percussion.util.PSCollection cols) {
    if (cols != null) {
      ...
      if (!com.percussion.design.objectstore.PSSortedColumn.class.isAssignableFrom(
        ...
        throw new IllegalArgumentException(
            "coll bad content type, Sorted Column: " + cols.getMemberClassName());
      }
    }
  ...
  private IllegalArgumentException validateNativeStatement(String text) {
    if (isSelectByNativeStatement() && ((text == null) || text.length() == 0))
  ...
  public Element toXml(Document doc) {
    ...
    if (isSelectByNativeStatement()) root.setAttribute("method", XML_FLAG_METHOD_NATIVE);
    else root.setAttribute("method", XML_FLAG_METHOD_WHERE);
    root.setAttribute("unique", isSelectUnique() ? "yes" : "no");

    // private          PSCollection    m_whereClauses = null;
    if (m_whereClauses != null) {
      Element whereNode = PSXmlDocumentBuilder.addEmptyElement(doc, root, "WhereClauses");
    ...
    if (m_sortedColumns != null) {
      Element sortNode = PSXmlDocumentBuilder.addEmptyElement(doc, root, "Sorting");
    ...
    PSXmlDocumentBuilder.addElement(doc, root, "nativeStatement", m_nativeStatement);

    // private          boolean         m_caching = false;  //caching on or off
    // private          int             m_cacheType = DS_CACHE_TYPE_INTERVAL;   //cache type
    // DS_CACHE_TYPE_XXXX
    ...
    if (m_cacheAgeTime != null)
      PSXmlDocumentBuilder.addElement(
          doc, cacheNode, "ageTime", m_DateFormatter.format(m_cacheAgeTime));

    // private          int             m_cacheAgeInterval = 0;
    PSXmlDocumentBuilder.addElement(
        doc, cacheNode, "ageInterval", String.valueOf(m_cacheAgeInterval));
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_selector = DS_DISTINCT;
      else m_selector = 0;
      ...
      if (tree.getNextElement("WhereClauses", firstFlags) != null) {
        if (tree.getNextElement(curNodeType, firstFlags) != null) {
          ...
          do {
            whereClause =
                new PSWhereClause((Element) tree.getCurrent(), parentDoc, parentComponents);
      ...
      if (tree.getNextElement("Sorting", firstFlags) != null) {
        ...
        if (tree.getNextElement(curNodeType, firstFlags) != null) {
          PSSortedColumn col;
          do {
            col = new PSSortedColumn((Element) tree.getCurrent(), parentDoc, parentComponents);
      ...
      try { // private          String          m_nativeStatement = "";
        ...
        throw new PSUnknownNodeTypeException(
            ms_NodeType, "nativeStatement", new PSException(e.getLocalizedMessage()));
      }
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    // validate type of query
    {
      ...
      final int all_flags_compliment = ~(DS_BY_WHERE_CLAUSE | DS_BY_NATIVE_STATEMENT | DS_DISTINCT);
  ...
  private String getCacheTypeString() {
    String sRet = "";
    if (m_cacheType == DS_CACHE_TYPE_INTERVAL) sRet = XML_FLAG_CACHE_TYPE_INTERVAL;
    else if (m_cacheType == DS_CACHE_TYPE_TIME) sRet = XML_FLAG_CACHE_TYPE_TIME;
    else if (m_cacheType == DS_CACHE_TYPE_TIME_INTERVAL) sRet = XML_FLAG_CACHE_TYPE_BOTH;
    return sRet;
  }
