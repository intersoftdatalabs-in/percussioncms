...
public final class PSFieldSet extends PSComponent {
  ...
  public PSFieldSet getChildsFieldSet(String name, int type) {
    ...
    while (fields.hasNext()) {
      ...
      if (fieldSet.getType() != type) continue;
  ...
  public PSField findFieldByName(String name, boolean systemModOnly) {
    ...
    while (iter.hasNext()) {
      ...
      if (o instanceof PSFieldSet) {
        PSField field = ((PSFieldSet) o).findFieldByName(name, systemModOnly);
        if (field != null) {
          return field;
        }
      } else {
  ...
  public PSField getSimpleChildField() throws PSSystemValidationException {
    if (getType() != PSFieldSet.TYPE_SIMPLE_CHILD)
      throw new IllegalStateException("This fieldset not a Simple Child");
  ...
  public final Object add(PSField field) {
    ...
    if (name == null || name.length() == 0)
      throw new IllegalArgumentException("the provided field must have a valid name");
  ...
  public final Object add(PSFieldSet fieldSet) {
    ...
    if (name == null || name.length() == 0)
      throw new IllegalArgumentException("the provided field set must have a valid name");
  ...
  public void setRepeatability(int repeatability) {
    if (repeatability != REPEATABILITY_ZERO_OR_MORE && repeatability != REPEATABILITY_ONE_OR_MORE)
  ...
  public PSField[] getAllFields(boolean readOnly) {
    Collection<PSField> fields = getAllFields(new ArrayList<>(), readOnly);
  ...
  public PSField getFieldByName(String fieldName) {
    ...
    while (fields.hasNext() && null == match) {
      ...
      if (o instanceof PSFieldSet) {
        ...
        PSField f = (PSField) o;
        if (f.getSubmitName().equalsIgnoreCase(fieldName)) match = f;
      }
    }
    return match;
  }
  ...
  private Collection<PSField> getAllFields(Collection<PSField> c, boolean systemModOnly) {
    ...
    while (fields.hasNext()) {
      ...
      if (o instanceof PSFieldSet) {
        ...
        PSField f = (PSField) o;
        if (isSystemModOnly(f, systemModOnly)) c.add(f);
      }
    }
    return c;
  }
  ...
  public final void copyFrom(PSFieldSet c) {
    ...
    if (c.getCount() <= 0) setRepeatability(c.getRepeatability());
    else setCount(c.getCount());
    m_fields.clear();
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSFieldSet)) return false;

    PSFieldSet t = (PSFieldSet) o;

    boolean equal = true;
    if (!compare(m_fields, t.m_fields)) equal = false;
    else if (!equalSettings(t)) equal = false;
  ...
  public boolean equalSettings(PSFieldSet set) {
    boolean equal = true;
    if (!compare(m_name, set.m_name)) equal = false;
    else if (m_repeatability != set.m_repeatability) equal = false;
    else if (m_repeatability == REPEATABILITY_COUNT && m_count != set.m_count) equal = false;
    else if (m_supportsSequencing != set.m_supportsSequencing) equal = false;
    else if (isUserSearchable() != set.isUserSearchable()) equal = false;
    else if (m_type != set.m_type) equal = false;
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (data != null)
        m_supportsSequencing = data.equalsIgnoreCase(BOOLEAN_ENUM[0]) ? true : false;

      m_userSearchable = PSXMLDomUtil.checkAttributeBool(sourceNode, USER_SEARCHABLE_ATTR);
  ...
  public void fixSourceTypes(
      ...
      List sysFieldExcludes,
      List sharedGroupIncludes,
      List sharedFieldExcludes) {
    if (systemDef == null) throw new IllegalArgumentException("systemDef may not be null");
    if (sharedDef == null) throw new IllegalArgumentException("sharedDef may not be null.");
    if (sysFieldExcludes == null)
      throw new IllegalArgumentException("sysFieldExcludes may not be null.");
    if (sharedGroupIncludes == null)
      throw new IllegalArgumentException("sharedGroupIncludes may not be null.");
    if (sharedFieldExcludes == null)
      throw new IllegalArgumentException("sharedFieldExcludes may not be null.");
    ...
    if (sharedDef.isSharedFieldSet(this, sharedGroupIncludes)) {
      ...
      while (fields.hasNext()) {
        ...
        if (test instanceof PSField) {
          PSField field = (PSField) test;
          if (field.getType() != PSField.TYPE_UNKNOWN) continue;

          if (systemDef.isSystemField(field, sysFieldExcludes)) field.setType(PSField.TYPE_SYSTEM);
          else if (sharedDef.isSharedField(field, sharedGroupIncludes, sharedFieldExcludes)) {
            field.setType(PSField.TYPE_SHARED);
          } else field.setType(PSField.TYPE_LOCAL);
        } else if (test instanceof PSFieldSet) {
          ((PSFieldSet) test)
              .fixSourceTypes(
                  systemDef, sharedDef, sysFieldExcludes, sharedGroupIncludes, sharedFieldExcludes);
        }
      }
    }
  }
  ...
  public boolean hasMandatorySystemFields() {
    ...
    while (fields.hasNext()) {
      ...
      if (test instanceof PSField) {
        PSField field = (PSField) test;
        if (field.isSystemMandatory()) return true;
      }
    }
  ...
  public Element toXml(Document doc) {
     ...
     */
    root.setAttribute(SUPPORTS_SEQUENCING_ATTRIBUTE, BOOLEAN_ENUM[m_supportsSequencing ? 0 : 1]);
    root.setAttribute(USER_SEARCHABLE_ATTR, BOOLEAN_ENUM[m_userSearchable ? 0 : 1]);
  ...
  public PSFieldSet merge(PSFieldSet source) throws PSSystemValidationException {
    ...
    while (i.hasNext()) {
      ...
      if (contains(name)) {
        if (get(name).getClass().isInstance(o) || o.getClass().isInstance(get(name)))
          hasMatch = true;
        else {
          // throw error
          throw new PSSystemValidationException(
    ...
    while (i.hasNext()) {
      ...
      if (hasMatch) {
        ...
        if (o instanceof PSFieldSet) {
          ...
          if (demergedChildSet.getAll().hasNext() || !demergedChildSet.equalSettings(childSource))
            demergedSet.add(demergedChildSet);
          else demergedSet.remove(demergedChildSet.getName());
        } else // demerge the field
        {
          ...
          } else demergedSet.add(demergedField);
        }
      }
    }
    return demergedSet;
  ...
  public PSFieldSet removeFields(List fieldExcludes, boolean validate)
    ...
    while (fields.hasNext()) {
      ...
      if (fieldExcludes.contains(name)) resultFields.remove(name);
    }
  ...
  public void fixupBackEndColumns(Map tables) throws PSSystemValidationException {
    try {
      ...
      while (iter.hasNext()) {
        Object o = iter.next();
        if (o instanceof PSFieldSet) ((PSFieldSet) o).fixupBackEndColumns(tables);
        else {
          PSField field = (PSField) o;
