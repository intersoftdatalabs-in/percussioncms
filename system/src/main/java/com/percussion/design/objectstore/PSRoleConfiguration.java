...
public final class PSRoleConfiguration implements IPSDocument {
  ...
  public PSRoleConfiguration() {
    ...
    m_roles =
        new PSDatabaseComponentCollection(role.getClass(), role.getDatabaseAppQueryDatasetName());
  }
  ...
  public PSRoleConfiguration(Document xmlDef)
    ...
    this();
    if (xmlDef == null) throw new IllegalArgumentException("xmlDef must be supplied.");
    fromXml(xmlDef);
  }
  ...
  public PSRoleConfiguration(PSDatabaseComponentLoader loader)
    ...
    this();
    if (loader == null) throw new IllegalArgumentException("Loader must be supplied.");
    fromDb(loader);
  }
  ...
  private void updateGlobalSubjects() {
    ...
    for (int i = m_subjects.size() - 1; i >= 0; i--) {
      ...
      if (!found) m_subjects.removeElementAt(i);
    }

    // add any needed global subjects
    for (int i = 0; i < m_roles.size(); i++) {
      PSDatabaseComponentCollection roleSubjects = ((PSRole) m_roles.get(i)).getSubjects();
  ...
  private PSGlobalSubject getCorrespondingGlobalSubject(PSRelativeSubject sub) {
    for (Iterator i = m_subjects.iterator(); i.hasNext(); ) {
      ...
      if (gSub.isMatch(sub)) return gSub;
    }
    ...
    throw new NullPointerException("No matching global subject found for " + sub.getName());
  }
  ...
  public synchronized PSGlobalSubject getGlobalSubject(
      PSRelativeSubject roleSubj, boolean createsubject) {
    if (roleSubj == null) {
      throw new IllegalArgumentException("Subject must be specified for lookup.");
    }
