...
public class PSField extends PSComponent {
  ...
  private void setBooleanProperty(boolean enable, String name) {
    ...
    PSProperty property = (PSProperty) m_properties.get(name);
    if (property == null) {
      property = new PSProperty(name, PSProperty.TYPE_BOOLEAN, value, true, null);

      m_properties.put(name, property);
    } else {
      if (property.getType() != PSProperty.TYPE_BOOLEAN)
        throw new ClassCastException("the property is not of type PSProperty.TYPE_BOOLEAN");
  ...
  public String[] getDeclaredNamespaces() {
    ...
    if (StringUtils.isBlank(allowedstr)) return new String[] {};
  ...
  public void setDeclaredNamespaces(String[] namespaces) {
    ...
    if (StringUtils.isBlank(val)) {
      ...
      if (prop == null) {
        prop = new PSProperty(DECLARED_NAMESPACES, PSProperty.TYPE_STRING, val, false, "");
      } else {
        prop.setValue(val);
      }
    }
    if (prop != null) setProperty(prop);
  }
  ...
  public String getCleanupPropertiesFile() {
    ...
    if (property != null) return (String) property.getValue();
  ...
  public void setCleanupPropertiesFile(String file) {
    ...
    if (test.exists()) throw new IllegalArgumentException("file must exist");
    ...
    if (property == null) {
      property =
          new PSProperty(CLEANUP_PROPERTIES_PROPERTY, PSProperty.TYPE_STRING, file, true, null);

      m_properties.put(CLEANUP_PROPERTIES_PROPERTY, property);
    } else property.setValue(file);
  }
  ...
  public void setCleanupServerPageTagFile(String file) {
    ...
    if (property == null) {
      property =
          new PSProperty(CLEANUP_SERVERPAGETAGS_PROPERTY, PSProperty.TYPE_STRING, file, true, null);
  ...
  public void setCleanupEncoding(String encoding) {
    ...
    if (property == null) {
      property =
          new PSProperty(CLEANUP_ENCODING_PROPERTY, PSProperty.TYPE_STRING, encoding, true, null);

      m_properties.put(CLEANUP_ENCODING_PROPERTY, property);
    } else property.setValue(encoding);
  }
  ...
  public PSPropertySet getProperties() {
    ...
    while (properties.hasNext()) propertySet.add(properties.next());
  ...
  public PSPropertySet getProperties(PSPropertySet defaults) {
    ...
    while (properties.hasNext()) {
      ...
      if (exists != null) propertySet.add(exists);
      else
        propertySet.add(
            new PSProperty(
                property.getName(),
  ...
  public PSProperty getProperty(String name) {
    if (name != null) return (PSProperty) m_properties.get(name);
  ...
  public void setDataType(String dataType) {
    ...
    if (dataType.length() == 0
      ...
      String msg =
          "Converting the deprecated data type \""
              + dataType
              + "\" to \""
              + DT_INTEGER
              + "\" for field '"
      ...
      throw new IllegalArgumentException(
          "the data type (" + dataType + ") doesn't match one of the known DT_xxx types");
    }
  }
  ...
  public boolean fixupDataType(
    ...
    switch (jdbcType) {
      ...
      case Types.TIMESTAMP:
        if (changed && (currentType.equals(DT_DATE) || currentType.equals(DT_TIME))) {
          changed = false;
        }
        break;
      ...
      case Types.CLOB:
        if (searchEnabled) {
           ...
           */
          if (!m_searchProps.isFromXml()) m_searchProps.setEnableTransformation(true);
        } else
          // this is the pre version 5.5 behavior
          m_searchProps.setUserSearchable(false);
      default:
    ...
    if (getDataType().equals(DT_TEXT) || getDataType().equals(DT_BINARY)) {
      ...
      if (!MAX_FORMAT.equalsIgnoreCase(getDataFormat())
          && !precisionStr.equals(getDataFormat())
          && precision > 0) {
  ...
  public void setSearchProperties(PSSearchProperties props) {
    if (null == props) props = new PSSearchProperties();
    else
    ...
    if (locked != props.isEnableTransformationLocked()) {
      if (locked) props.setEnableTransformation(enableTransform);
  ...
  public void setOccurrenceDimension(int occurrenceDimension, Integer transitionId)
    ...
    (getOccurrenceSetting(transitionId, true)).setOccurrenceDimension(occurrenceDimension);
  }
  ...
  public void setOccurrenceMultiValuedType(int occurrenceMultiValuedType, Integer transitionId)
      throws PSSystemValidationException {
    (getOccurrenceSetting(transitionId, true))
        .setOccurrenceMultiValuedType(occurrenceMultiValuedType);
  }
  ...
  public void setOccurrenceDelimiter(String occurrenceDelimiter, Integer transitionId) {
    (getOccurrenceSetting(transitionId, true)).setOccurrenceDelimiter(occurrenceDelimiter);
  }
  ...
  public String getClearBinaryParam() {
    if (m_enableClearBinaryParam) return getSubmitName() + CLEAR_BINARY_PARAM_SUFFIX;
    else return null;
  }
  ...
  public boolean equalMetaData(Object o) {
    ...
    boolean equal = true;
    if (!compare(m_dataType, t.m_dataType)) equal = false;
    if (!compare(m_dataFormat, t.m_dataFormat)) equal = false;
    else if (!compare(m_locator, t.m_locator)) equal = false;
    else if (m_systemMandatory != t.m_systemMandatory) equal = false;
    else if (m_systemInternal != t.m_systemInternal) equal = false;
    else if (!compare(m_submitName, t.m_submitName)) equal = false;
    else if (!compare(m_mimeType, t.m_mimeType)) equal = false;
    else if (m_type != t.m_type) equal = false;
    else if (m_fieldValueType != t.m_fieldValueType) equal = false;
    // the isCleanupNamespaces() property is used while creating hibernate
    // session factory for Content Types.
    else if (isCleanupNamespaces() != t.isCleanupNamespaces()) equal = false;
  ...
  public int hashCode() {
    return Objects.hash(
        ...
        m_inputTranslation,
        m_outputTranslation,
        m_enableClearBinaryParam,
        m_occurrenceSettings,
        m_choices,
        m_properties,
        m_searchProps,
        m_systemInternal);
    ...
    try {
      ...
      if (data != null) {
        for (int i = 0; i < MODTYPE_ATTR_VALUES.length; i++) {
          if (data.equalsIgnoreCase(MODTYPE_ATTR_VALUES[i])) m_modificationType = i;
        }
      }

      // OPTIONAL: get the field forceBinary attribute
      data = tree.getElementData(FORCE_BINARY_ATTR);
      if (data != null) m_forceBinary = data.equalsIgnoreCase(BOOLEAN_ENUM[0]) ? true : false;

      data = tree.getElementData(EXPORT_ATTR);
      if (data != null) m_exportable = data.equalsIgnoreCase(BOOLEAN_ENUM[0]) ? true : false;

      // OPTIONAL: get the field clearBinaryParam attribute
      data = tree.getElementData(CLEAR_BINARY_PARAM_ATTR);
      if (data != null && !(data.equalsIgnoreCase("no"))) setClearBinaryParam(true);
      ...
      while (node != null) {
        ...
        if (elementName.equals(DATA_LOCATOR_ELEM)) {
          ...
          m_locator =
              (IPSBackEndMapping)
                  PSReplacementValueFactory.getReplacementValueFromXml(
                      parentDoc, parentComponents, node, XML_NODE_NAME, DATA_LOCATOR_ELEM);
          ...
          if (m_dataFormat != null && m_dataFormat.trim().length() <= 0) m_dataFormat = null;
          ...
          PSOccurrenceSetting occurrenceSetting = new PSOccurrenceSetting();
          ...
          if (occurrenceSetting.getOccurrenceDimension() == OCCURRENCE_DIMENSION_COUNT) {
            data = PSXmlTreeWalker.getElementData(node);
            if (data != null && data.trim().length() > 0) {
              try {
                occurrenceSetting.setOccurrenceCount(Integer.parseInt(data));
          ...
          if (data != null && data.trim().length() != 0) {
            ...
            for (int i = 0; i < OCCURRENCE_MULTI_VALUED_TYPE_ENUM.length; i++) {
              if (OCCURRENCE_MULTI_VALUED_TYPE_ENUM[i].equalsIgnoreCase(data)) {
                try {
          ...
          while (node != null) {
            ...
            if (elementName.equals(PSFieldValidationRules.XML_NODE_NAME))
              m_validationRules = new PSFieldValidationRules(node, parentDoc, parentComponents);
            else if (elementName.equals(PSVisibilityRules.XML_NODE_NAME))
              m_visibilityRules = new PSVisibilityRules(node, parentDoc, parentComponents);
              ...
              while (node != null) {
                ...
                if (elementName.equals(PSFieldTranslation.XML_NODE_NAME))
                  m_inputTranslation = new PSFieldTranslation(node, parentDoc, parentComponents);
              ...
              while (node != null) {
                elementName = node.getTagName();
                if (elementName.equals(PSFieldTranslation.XML_NODE_NAME))
                  m_outputTranslation = new PSFieldTranslation(node, parentDoc, parentComponents);
      ...
      if (null == m_searchProps) {
        ...
        data = sourceNode.getAttribute(PSSearchProperties.USER_SEARCHABLE_ATTR);
        if (data != null && data.trim().length() > 0) {
          m_searchProps.setUserSearchable(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
        }

        data = sourceNode.getAttribute(PSSearchProperties.USER_CUSTOMIZABLE_ATTR);
        if (data != null && data.trim().length() > 0) {
          m_searchProps.setUserCustomizable(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
        }

        // OPTIONAL
        m_searchProps.setDefaultSearchLabel(
            sourceNode.getAttribute(PSSearchProperties.DEF_SEARCH_LABEL_ATTR));
      }
    } finally {
      correctVisibilityRules();
  ...
  private void correctVisibilityRules() {
    if (m_systemMandatory) {
      ...
      if (rules != null) rules.setDataHiding(PSVisibilityRules.DATA_HIDING_XSL);
    }
  }
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(SHOW_IN_SUMMARY_ATTR, m_showInSummary ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
    root.setAttribute(SHOW_IN_PREVIEW_ATTR, m_showInPreview ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
    root.setAttribute(MODIFICATION_TYPE_ATTR, MODTYPE_ATTR_VALUES[m_modificationType]);
    root.setAttribute(FORCE_BINARY_ATTR, m_forceBinary ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
    root.setAttribute(EXPORT_ATTR, m_exportable ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
    if (null != m_mimeType) root.setAttribute(MIME_TYPE_ATTR, m_mimeType);
    if (m_enableClearBinaryParam) root.setAttribute(CLEAR_BINARY_PARAM_ATTR, BOOLEAN_ENUM[0]);
    if (m_type != TYPE_UNKNOWN) root.setAttribute(FIELD_TYPE_ATTR, TYPE_ENUM[m_type]);
    if (m_fieldValueType != FIELD_VALUE_TYPE_UNKNOWN)
      root.setAttribute(FIELD_VALUE_TYPE_ATTR, FIELD_VALUE_TYPE_ENUM[m_fieldValueType]);
    if (isSystemMandatory()) root.setAttribute(SYSTEM_MANDATORY_ATTR, BOOLEAN_ENUM[0]);
    if (m_systemInternal) root.setAttribute(SYSTEM_INTERNAL_ATTR, BOOLEAN_ENUM[0]);
    ...
    while (settings.hasNext()) {
      ...
      if (setting.getOccurrenceDimension() == OCCURRENCE_DIMENSION_COUNT) {
        elem =
            PSXmlDocumentBuilder.addElement(
                doc, root, OCCURRENCE_ELEM, Integer.toString(setting.getOccurrenceCount()));
      } else {
        elem = PSXmlDocumentBuilder.addEmptyElement(doc, root, OCCURRENCE_ELEM);
      }

      elem.setAttribute(
          OCCURRENCE_DIMENSION_ATTR, OCCURRENCE_DIMENSION_ENUM[setting.getOccurrenceDimension()]);
      elem.setAttribute(
          OCCURRENCE_MULTI_VALUED_TYPE_ATTR,
          OCCURRENCE_MULTI_VALUED_TYPE_ENUM[setting.getOccurrenceMultiValuedType()]);
      elem.setAttribute(OCCURRENCE_DELIMITER_ATTR, setting.getOccurrenceDelimiter());

      if (transId != null) elem.setAttribute(TRANSITION_ID_ATTR, transId.toString());
    }

    // create the field rules
    if (m_validationRules != null
        || m_visibilityRules != null
        || m_inputTranslation != null
        || m_outputTranslation != null) {
      elem = doc.createElement(FIELD_RULES_ELEM);
      if (m_visibilityRules != null) elem.appendChild(m_visibilityRules.toXml(doc));
      if (m_validationRules != null) elem.appendChild(m_validationRules.toXml(doc));

      // create the field input translations
      if (m_inputTranslation != null) {
        Element el = doc.createElement(FIELD_INPUT_TRANSLATIONS_ELEM);
        if (m_inputTranslation != null) el.appendChild(m_inputTranslation.toXml(doc));
      ...
      if (m_outputTranslation != null) {
        ...
        if (m_outputTranslation != null) el.appendChild(m_outputTranslation.toXml(doc));
    ...
    // create choices
    if (m_choices != null) root.appendChild(m_choices.toXml(doc));
    ...
    if (isFeatureSupported("FullTextSearch", 1)) {
      ...
      root.setAttribute(
          ...
          m_searchProps.isUserSearchable() ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
      root.setAttribute(
          PSSearchProperties.USER_CUSTOMIZABLE_ATTR,
          m_searchProps.isUserCustomizable() ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
      root.setAttribute(
          PSSearchProperties.DEF_SEARCH_LABEL_ATTR, m_searchProps.getDefaultSearchLabel());
    }
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      ...
      context.validationError(this, e.getErrorCode(), e.getErrorArguments());
    }
    ...
    try {
      ...
      if (m_default instanceof IPSComponent) ((IPSComponent) m_default).validate(context);
      if (m_inputTranslation != null) m_inputTranslation.validate(context);
      if (m_outputTranslation != null) m_outputTranslation.validate(context);
      if (m_validationRules != null) m_validationRules.validate(context);
      if (m_visibilityRules != null) m_visibilityRules.validate(context);
      if (m_choices != null) m_choices.validate(context);
    } finally {
      context.popParent();
    }
  }
  ...
  public PSField merge(PSField source) throws PSSystemValidationException {
    ...
    if (!source.getSubmitName().equals(getSubmitName()))
      throw new IllegalArgumentException("The fields to be merged must have same name.");
     ...
     */
    boolean valOverride = (source.hasValidationRules() && hasValidationRules());
    boolean dtOverride =
        source.getDataType().length() > 0
            && getDataType().length() > 0
            && !source.getDataType().equals(getDataType());
    boolean fbOverride = (source.isForceBinary() != isForceBinary());

    if (valOverride || dtOverride || fbOverride) {
      String setting = "";
      if (valOverride) setting = "validation rules";
      else if (dtOverride) setting = "data type";
     ...
     */
    if (source.getDataType().length() > 0) newField.setDataType(source.getDataType());

    if (source.getDataFormat() != null) newField.setDataFormat(source.getDataFormat());

    if (getLocator() == null) newField.setLocator(source.getLocator());

    if (getInputTranslation() == null) newField.setInputTranslation(source.getInputTranslation());

    if (getOutputTranslation() == null)
      newField.setOutputTranslation(source.getOutputTranslation());

    if (getValidationRules() == null) newField.setValidationRules(source.getValidationRules());

    if (getVisibilityRules() == null) newField.setVisibilityRules(source.getVisibilityRules());

    if (getDefault() == null) newField.setDefault(source.getDefault());

    if (!hasOccurrenceSettings()) newField.replaceOccurrenceSettings(source);

    if (getChoices() == null) newField.setChoices(source.getChoices());

    if (m_properties.isEmpty()) newField.replaceProperties(source);
  ...
  public PSField demerge(PSField source) {
    ...
    if (m_inputTranslation != null && !compare(m_inputTranslation, source.m_inputTranslation)) {
      newField.m_inputTranslation = m_inputTranslation;
      hasOverrides = true;
    }

    if (m_outputTranslation != null && !compare(m_outputTranslation, source.m_outputTranslation)) {
      newField.m_outputTranslation = m_outputTranslation;
      hasOverrides = true;
    }

    if (hasValidationRules() && !compare(m_validationRules, source.m_validationRules)) {
      newField.m_validationRules = m_validationRules;
      hasOverrides = true;
    }

    if (m_visibilityRules != null && !compare(m_visibilityRules, source.m_visibilityRules)) {
    ...
    if (source.m_showInSummary != m_showInSummary) hasOverrides = true;

    newField.m_showInPreview = m_showInPreview;
    if (source.m_showInPreview != m_showInPreview) hasOverrides = true;

    newField.setSearchProperties(getSearchProperties());
    if (!source.m_searchProps.equals(m_searchProps)) hasOverrides = true;

    newField.m_forceBinary = m_forceBinary;
    if (source.m_forceBinary != m_forceBinary) hasOverrides = true;

    newField.m_enableClearBinaryParam = m_enableClearBinaryParam;
    if (source.m_enableClearBinaryParam != m_enableClearBinaryParam) hasOverrides = true;
  ...
  private PSOccurrenceSetting getOccurrenceSetting(Integer transitionId, boolean addNew) {
    PSOccurrenceSetting setting = (PSOccurrenceSetting) m_occurrenceSettings.get(transitionId);
    ...
    if (setting == null) {
      ...
      if (transitionId != null) setting = (PSOccurrenceSetting) m_occurrenceSettings.get(null);
      if (setting == null) {
        // no default setting specified, get our defaults.
        setting = new PSOccurrenceSetting();
      }

      if (!bFound && addNew) m_occurrenceSettings.put(transitionId, setting);
    }
  ...
  public void setUserProperty(String name, Object value) {
    ...
    if (value == null) m_userProps.remove(name);
    else m_userProps.put(name, value);
  }
  ...
  public PSMimeTypeModeEnum getMimeTypeMode() {
    ...
    return PSMimeTypeModeEnum.valueOf(Integer.parseInt((String) prop.getValue()));
  }
  ...
  public void setMimeType(PSMimeTypeModeEnum modeEnum, String value) {
    ...
    if (StringUtils.isBlank(value))
      throw new IllegalArgumentException("mime type value must not be null or empty.");
  ...
  public enum PSMimeTypeModeEnum {
    ...
    public static PSMimeTypeModeEnum getModeEnum(String displayName) {
      ...
      for (PSMimeTypeModeEnum modeEnum : values())
        if (modeEnum.getDisplayName().equals(displayName)) modeE = modeEnum;
    ...
    public static PSMimeTypeModeEnum valueOf(int mode) {
      PSMimeTypeModeEnum modeE = null;
      for (PSMimeTypeModeEnum modeEnum : values()) if (modeEnum.getMode() == mode) modeE = modeEnum;
  ...
  private static final String FIELD_INPUT_TRANSLATIONS_ELEM = "FieldInputTranslation";
  private static final String FIELD_OUTPUT_TRANSLATIONS_ELEM = "FieldOutputTranslation";
  ...
  private static final String OCCURRENCE_MULTI_VALUED_TYPE_ATTR = "multiValuedType";
  ...
  public enum PSDimensionEnum {
    ...
    public static PSDimensionEnum valueOf(int ordinal) {
      for (PSDimensionEnum dimension : values())
        if (dimension.getOrdinal() == ordinal) return dimension;

      throw new IllegalArgumentException("No dimension is defined for the supplied ordinal.");
    }
  ...
  private class PSOccurrenceSetting implements Serializable {
    ...
    public int hashCode() {
      return Objects.hash(
          m_occurrenceDimension,
          m_occurrenceCount,
          m_occurrenceMultiValuedType,
