...
public class PSContentTypeWorkflowsUpdater implements IPSComponentUpdater {
  ...
  public void updateComponent(PSComponent comp) {
    ...
    PSCmsObject cmsObject = PSServer.getCmsObjectRequired(editor.getObjectType());
    if (cmsObject == null || !cmsObject.isWorkflowable()) return;
    ...
    if (wfGuids.isEmpty()) {
      ...
      if (wfInfo != null && !wfInfo.isExclusionary()) return;
  ...
  private void setWorkflowInfo(PSContentEditor editor, Set<Integer> wfInts) {
    ...
    if (wfInfo == null) {
      if (!wfInts.isEmpty()) {
        wfInfo =
            new PSWorkflowInfo(PSWorkflowInfo.TYPE_INCLUSIONARY, new ArrayList<Integer>(wfInts));
   ...
   */
  private List<IPSGuid> getContentTypeWorkflows(PSContentEditor editor) throws RepositoryException {
    List<IPSGuid> wfGuids = new ArrayList<>();
    IPSContentMgr mgr = PSContentMgrLocator.getContentMgr();
    long cTypeId = editor.getContentType();
    IPSGuid ctypeGuid = new PSGuid(PSTypeEnum.NODEDEF, cTypeId);
    List<PSContentTypeWorkflow> ctwfs = mgr.findContentTypeWorkflowAssociations(ctypeGuid);
  ...
  private void resetDefaultWorkflow(PSContentEditor editor) {
    ...
    if (wfInts.isEmpty()) {
      ...
      Logger log = LogManager.getLogger(this.getClass());
      log.warn(
          "Reset the default workflow id to \""
              + smallestWfId
              + "\" (name="
              + smallestWfId
              + ") for Content Type (id="
