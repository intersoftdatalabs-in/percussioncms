...
public abstract class PSSubject extends PSDatabaseComponent {
  ...
  public boolean equals(Object obj) {
    if (obj instanceof PSSubject) {
      ...
      if (!m_name.equals(subject.m_name)) return false;
      if (m_type != subject.m_type) return false;
      if (!m_attributes.equals(subject.m_attributes)) return false;
  ...
  protected Element toXml(Document doc, boolean includeCompChildren) {
    ...
    if (includeCompChildren) root.appendChild(m_attributes.toXml(doc));
  ...
  private void fromXmlBase(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    Element attListElement = tree.getNextElement(PSAttributeList.ms_NodeType, firstFlags);
  ...
  public String getSubjectTypeName() {
    String sRet = "";
    if (m_type == SUBJECT_TYPE_USER) sRet = SUBJECT_TYPE_USER_STRING;
    else if (m_type == SUBJECT_TYPE_GROUP) sRet = SUBJECT_TYPE_GROUP_STRING;
