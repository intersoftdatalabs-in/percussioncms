...
public class PSTableLocator extends PSComponent {
  ...
  public PSTableLocator(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    if (converter != null && converter.isForcedConversion()) {
      PSTableLocator convertedLocator = (PSTableLocator) converter.convertComponent(sourceNode);
   ...
   * Specifically it compares the backend credential's datasource.
   *
   * @param locator The locator to compare to. May not be <code>null</code>.
   * @return <code>true</code> if it is the same location, <code>false</code> if not..
   */
  public boolean isSameLocation(PSTableLocator locator) {
    ...
    // check database
    if (((targetCred.getDataSource() == null ^ thisCred.getDataSource() == null)
        || (targetCred.getDataSource() != null
            && !targetCred.getDataSource().equalsIgnoreCase(thisCred.getDataSource())))) {
      equal = false;
    }
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (data != null && data.trim().length() > 0) m_alias = data;
  ...
  public Element toXml(Document doc) {
    ...
    if (m_alias != null) root.setAttribute(ALIAS_ATTR, m_alias);

    // REQUIRED: create PSBackEndCredential or alias
    if (m_credentials == null) PSXmlDocumentBuilder.addElement(doc, root, ALIAS_ELEM, m_aliasRef);
    else root.appendChild(m_credentials.toXml(doc));
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      if (m_credentials != null) m_credentials.validate(context);
    } finally {
      context.popParent();
    }
  }
