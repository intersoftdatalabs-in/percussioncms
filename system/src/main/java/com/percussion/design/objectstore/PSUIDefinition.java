...
public class PSUIDefinition extends PSComponent {
  ...
  public void setDefaultUI(PSCollection defaultUI) {
    ...
    if (defaultUI != null
        && m_defaultUI != null
        && !defaultUI.getMemberClassName().equals(m_defaultUI.getMemberClassName())) {
      throw new IllegalArgumentException("PSUISet collection expected");
    }

    if (defaultUI == null) m_defaultUI = null;
    else {
      if (m_defaultUI == null) m_defaultUI = new PSCollection(new PSUISet().getClass());
      else m_defaultUI.clear();
  ...
  public void setDisplayMapper(PSDisplayMapper displayMapper) {
    if (displayMapper == null) throw new IllegalArgumentException("mapper cannot be null");
  ...
  public int appendMapping(PSDisplayMapper mapper, PSDisplayMapping mapping) {
    if (mapper == null || mapping == null)
      throw new IllegalArgumentException("mapper and/or mapping cannot be null");
  ...
  public void replaceMapping(String fieldRef, PSDisplayMapping mapping) {
    ...
    throw new IllegalArgumentException("could not find the field, nothing replaced");
  }
  ...
  public PSDisplayMapping getMapping(String fieldRef) {
    ...
    while (it.hasNext()) {
      ...
      if (mapping.getFieldRef().equals(fieldRef)) return mapping;
    }
  ...
  public PSDisplayMapper getDisplayMapper(String fieldSetRef) {
    PSDisplayMapper mapper = getDisplayMapper();
    if (mapper.getFieldSetRef().equals(fieldSetRef)) return mapper;
  ...
  private PSDisplayMapper getDisplayMapper(String fieldSetRef, PSDisplayMapper mapper) {
    ...
    while (it.hasNext() && found == null) {
      PSDisplayMapping mapping = (PSDisplayMapping) it.next();
      if (mapper != null && mapper.getFieldSetRef().equals(fieldSetRef)) found = mapper;
      else found = getDisplayMapper(fieldSetRef, mapping.getDisplayMapper());
    }
  ...
  public PSDisplayMapper getDisplayMapper(int id) {
    PSDisplayMapper mapper = getDisplayMapper();
    if (mapper.getId() == id) return mapper;
  ...
  private PSDisplayMapper getDisplayMapper(int id, PSDisplayMapper mapper) {
    ...
    while (it.hasNext() && found == null) {
      ...
      if (mapper != null && mapper.getId() == id) found = mapper;
      else found = getDisplayMapper(id, test);
    }
  ...
  public PSUIDefinition merge(
      PSUIDefinition sourceUIDef, boolean mergeChild, List fieldExcludes, boolean mergeDefault)
      throws PSSystemValidationException {
    if (sourceUIDef == null) throw new IllegalArgumentException("sourceUIDef may not be null.");
    ...
    localMapper =
        localMapper.merge(
            mergeDefault,
            getDefaultUI(),
            sourceUIDef.getDisplayMapper(),
            sourceUIDef.getDefaultUI(),
            mergeChild,
            fieldExcludes);

    PSUIDefinition uiDef = null;
    if (getDefaultUI() == null) uiDef = new PSUIDefinition(localMapper);
    else {
      uiDef = new PSUIDefinition(localMapper, new PSCollection(getDefaultUI()));
    }
    return uiDef;
  }
  ...
  public PSUIDefinition demerge(
      PSUIDefinition sourceUIDef, boolean demergeChild, List fieldExcludes, boolean demergeDefault)
      throws PSSystemValidationException {
    if (sourceUIDef == null) throw new IllegalArgumentException("sourceUIDef may not be null.");
