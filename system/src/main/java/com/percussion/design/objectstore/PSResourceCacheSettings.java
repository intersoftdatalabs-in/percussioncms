...
public class PSResourceCacheSettings extends PSComponent {
  ...
  public void setDependencies(Iterator dependencies) {
    ...
    while (dependencies.hasNext()) {
      ...
      if (!(depObj instanceof String))
        throw new IllegalArgumentException("dependencies may only contain non-null Strings");

      String dep = (String) depObj;
      if (dep.trim().length() == 0)
        throw new IllegalArgumentException("dependencies may not contain empty String");
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(ENABLED_ATTR, m_isEnabled ? TRUE_ATTR_VAL : FALSE_ATTR_VAL);

    Element keysEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, KEYS_XML_EL);
    Iterator keys = m_extraKeys.iterator();
    while (keys.hasNext()) {
      Element keyEl = PSXmlDocumentBuilder.addEmptyElement(doc, keysEl, KEY_XML_EL);
      PSNamedReplacementValue val = (PSNamedReplacementValue) keys.next();
      keyEl.appendChild(val.toXml(doc));
    }

    Element depsEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, DEPS_XML_EL);
    Iterator deps = m_dependencies.iterator();
    while (deps.hasNext()) {
      PSXmlDocumentBuilder.addElement(doc, depsEl, DEP_XML_EL, (String) deps.next());
    }
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      m_isEnabled = TRUE_ATTR_VAL.equals(sourceNode.getAttribute(ENABLED_ATTR));
      ...
      while (key != null) {
        ...
        IPSReplacementValue replVal =
            PSReplacementValueFactory.getReplacementValueFromXml(
                parentDoc, parentComponents, child, XML_NODE_NAME, KEY_XML_EL);
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSResourceCacheSettings)) isEqual = false;
    else {
      PSResourceCacheSettings other = (PSResourceCacheSettings) obj;
      if (m_id != other.m_id) isEqual = false;
      if (m_isEnabled != other.m_isEnabled) isEqual = false;
      else if (!m_extraKeys.equals(other.m_extraKeys)) isEqual = false;
      else if (!m_dependencies.equals(other.m_dependencies)) isEqual = false;
    }
