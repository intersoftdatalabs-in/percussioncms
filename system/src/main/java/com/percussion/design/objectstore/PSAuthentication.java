...
public class PSAuthentication extends PSComponent {
  ...
  public void setScheme(String scheme) {
    ...
    if (scheme.length() == 0) throw new IllegalArgumentException("scheme cannot be empty");
    ...
    if (!valid) throw new IllegalArgumentException("scheme must be one of SCHEME_ENUM");
  ...
  public String getUser(String providerUrl) {
    ...
    if (shouldAppendBaseDn()) {
      try {
        ...
        if (baseDn.length() > 0) user += "," + baseDn;
  ...
  public void setUserAttr(String userAttr) {
    ...
    if (userAttr != null && userAttr.length() == 0) userAttr = null;
  ...
  public String getPrincipal() {
    ...
    if (attrName != null) return attrName + "=" + getUser();
  ...
  public String getPrincipal(String providerUrl) {
    ...
    if (shouldAppendBaseDn()) {
      try {
        String baseDn = getBaseDn(providerUrl);
        if (baseDn.length() > 0) principal += "," + baseDn;
      } catch (MalformedURLException e) {
        // this should never happen
      }
    }
  ...
  private String getBaseDn(String providerUrl) throws MalformedURLException {
    if (providerUrl == null) throw new IllegalArgumentException("providrurl cannot be null");

    providerUrl = providerUrl.trim();
    if (providerUrl.length() == 0) throw new IllegalArgumentException("providrurl cannot be empty");
    ...
    if (pos != -1) baseDn = baseDn.substring(pos + delimiter.length());
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    setScheme(getEnumeratedAttribute(tree, XML_ATTR_SCHEME, SCHEME_ENUM, true));
    ...
    isPasswordEncrypted =
        (encryptedValue != null && encryptedValue.trim().equalsIgnoreCase(XML_ATTRVALUE_YES));
      ...
      try {
        data =
            PSEncryptor.decryptString(
                PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR), encData);
      } catch (PSEncryptionException e) {
        String userStr = getUser();
        String key =
            userStr.trim().length() == 0
    ...
    Element filterExtension = tree.getNextElement(XML_ELEM_FILTER_EXTENSION_NAME);
  ...
  public Element toXml(Document doc, boolean encPwd) {
    ...
    Element credentials = PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_ELEM_CREDENTIALS);

    String userStr = getUser();
    Element user = PSXmlDocumentBuilder.addElement(doc, credentials, XML_ELEM_USER, userStr);
    user.setAttribute(
        XML_ATTR_APPENDBASEDN, shouldAppendBaseDn() ? XML_ATTRVALUE_YES : XML_ATTRVALUE_NO);
    if (getUserAttr() != null) user.setAttribute(XML_ATTR_ATTRIBUTE_NAME, getUserAttr());
    ...
    Element password = PSXmlDocumentBuilder.addElement(doc, credentials, XML_ELEM_PASSWORD, pw);
    ...
    if (getFilterExtension().length() != 0)
      PSXmlDocumentBuilder.addElement(
          doc, root, XML_ELEM_FILTER_EXTENSION_NAME, getFilterExtension());
  ...
  public int hashCode() {
    return Objects.hash(
        super.hashCode(), m_name, m_scheme, m_user, m_appendBaseDn, m_userAttr, m_filterExtension);
  }
  ...
  private static final String XML_ELEM_FILTER_EXTENSION_NAME = "FilterExtensionName";
