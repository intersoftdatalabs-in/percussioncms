...
public class PSActionLink extends PSComponent {
  ...
  public void setDisplayText(PSDisplayText displayText) {
    if (displayText == null) throw new IllegalArgumentException("displayText cannot be null");
  ...
  public void setParameters(PSCollection parameters) {
    ...
    if (!parameters.getMemberClassName().equals(m_parameters.getMemberClassName()))
  ...
  public int hashCode() {
    return Objects.hash(
        super.hashCode(), m_displayText, m_parameters, m_isDisabled, m_isTransition);
  }
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (data != null) m_isDisabled = data.equalsIgnoreCase(BOOLEAN_ENUM[0]) ? true : false;

      // OPTIONAL: get the isTransition attribute
      data = tree.getElementData(IS_TRANSITION_ATTR);
      if (data != null) m_isTransition = data.equalsIgnoreCase(BOOLEAN_ENUM[0]) ? true : false;
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(IS_DISABLED_ATTR, m_isDisabled ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
    root.setAttribute(IS_TRANSITION_ATTR, m_isTransition ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
