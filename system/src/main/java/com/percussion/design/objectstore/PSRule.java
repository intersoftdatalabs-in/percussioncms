...
public class PSRule extends PSComponent {
  ...
  public int getOperator() {
    if (m_extensionRules == null) ((PSConditional) m_conditionalRules.lastElement()).getBoolean();
  ...
  private void setConditionalRules(PSCollection conditionalRules) {
    ...
    if (conditionalRules != null) m_conditionalRules.addAll(conditionalRules);
  }
  ...
  private void setExtensionRules(PSExtensionCallSet extensionRules) {
    if (extensionRules == null) throw new IllegalArgumentException("extensionRules cannot be null");
  ...
  public boolean equals(Object o) {
    ...
    boolean equal = true;
    if (!compare(m_conditionalRules, t.m_conditionalRules)) equal = false;
    else if (!compare(m_extensionRules, t.m_extensionRules)) equal = false;
    else if (getOperator() != t.getOperator()) equal = false;
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (node != null)
        m_extensionRules = new PSExtensionCallSet(node, parentDoc, parentComponents);
      else {
        node = tree.getNextElement(PSConditional.ms_NodeType, firstFlags);
        if (node == null) {
          Object[] args = {
            XML_NODE_NAME,
            PSExtensionCallSet.ms_NodeType + " and " + PSConditional.ms_NodeType,
            "null"
          };
        ...
        while (node != null) {
          ...
          node = tree.getNextElement(PSConditional.ms_NodeType, nextFlags);
        }
      }
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      if (m_extensionRules != null) m_extensionRules.validate(context);
