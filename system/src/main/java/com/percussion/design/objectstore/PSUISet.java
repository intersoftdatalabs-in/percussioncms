...
public final class PSUISet extends PSComponent {
  ...
  public Object clone() {
    ...
    if (m_choices != null) copy.m_choices = (PSChoices) m_choices.clone();
    if (m_control != null) copy.m_control = (PSControlRef) m_control.clone();
    if (m_customActionGroup != null)
      copy.m_customActionGroup = (PSCustomActionGroup) m_customActionGroup.clone();
    if (m_errorLabel != null) copy.m_errorLabel = (PSDisplayText) m_errorLabel.clone();
    if (m_label != null) copy.m_label = (PSDisplayText) m_label.clone();
    if (m_labelSourceType != null) copy.m_labelSourceType = new String(m_labelSourceType);
  ...
  public PSUISet merge(PSUISet source) {
    ...
    if (getChoices() == null) mergedSet.setChoices(source.getChoices());
    if (getControl() == null) mergedSet.setControl(source.getControl());
    if (getDefaultSet() == null) mergedSet.setDefaultSet(source.getDefaultSet());
    if (getErrorLabel() == null) mergedSet.setErrorLabel(source.getErrorLabel());
    if (getLabel() == null) mergedSet.setLabel(source.getLabel());
    if (getAccessKey() == null) mergedSet.setAccessKey(source.getAccessKey());
    if (m_readOnlyRules.isEmpty()) mergedSet.setReadOnlyRules(source.m_readOnlyRules);
    if (getCustomActionGroup() == null)
      mergedSet.setCustomActionGroup(source.getCustomActionGroup());
    if (getLabelSourceType() == null) mergedSet.setLabelSourceType(source.getLabelSourceType());
    if (m_name == null || m_name.trim().length() == 0) mergedSet.setName(source.getName());
  ...
  public PSUISet demerge(PSUISet source) {
    ...
    if (m_labelSourceType != null && !compare(m_labelSourceType, source.m_labelSourceType))
      diffSet.m_labelSourceType = m_labelSourceType;
    if (m_choices != null && !compare(m_choices, source.m_choices)) diffSet.m_choices = m_choices;
    if (m_control != null && !compare(m_control, source.m_control)) diffSet.m_control = m_control;
    ...
    if (m_label != null && !compare(m_label, source.m_label)) diffSet.m_label = m_label;
    if (!compare(m_accessKey, source.m_accessKey)) diffSet.m_accessKey = m_accessKey;
    if (m_name != null && !compare(m_name, source.m_name)) diffSet.m_name = m_name;
    if (!m_readOnlyRules.isEmpty() && !compare(m_readOnlyRules, source.m_readOnlyRules)) {
      diffSet.m_readOnlyRules = m_readOnlyRules;
    }
    if (m_customActionGroup != null && !compare(m_customActionGroup, source.m_customActionGroup)) {
  ...
  public void setAccessKey(String accessKey) {
    if (accessKey != null) {
      ...
      if (accessKey.length() > 1) throw new IllegalArgumentException("accesskey cannot be > 1");
    } else accessKey = "";
  ...
  public void setReadOnlyRules(PSCollection readOnlyRules) {
    if (readOnlyRules != null
        && !readOnlyRules.getMemberClassName().equals(m_readOnlyRules.getMemberClassName()))
      throw new IllegalArgumentException("PSRule collection expected");

    m_readOnlyRules.clear();
    if (readOnlyRules != null) m_readOnlyRules.addAll(readOnlyRules);
  }
  ...
  public boolean equals(Object o) {
    ...
    boolean equal = true;
    if (!compare(m_labelSourceType, t.m_labelSourceType)) equal = false;
    if (!compare(m_choices, t.m_choices)) equal = false;
    else if (!compare(m_control, t.m_control)) equal = false;
    else if (!compare(m_defaultSet, t.m_defaultSet)) equal = false;
    else if (!compare(m_errorLabel, t.m_errorLabel)) equal = false;
    else if (!compare(m_label, t.m_label)) equal = false;
    else if (!compare(m_accessKey, t.m_accessKey)) equal = false;
    else if (!compare(m_name, t.m_name)) equal = false;
    else if (!compare(m_readOnlyRules, t.m_readOnlyRules)) equal = false;
    else if (!compare(m_customActionGroup, t.m_customActionGroup)) equal = false;
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (data != null && data.trim().length() != 0) m_name = data;
      ...
      if (data != null && data.trim().length() != 0) m_defaultSet = data;
      ...
      while (node != null) {
        ...
        if (elementName.equals(LABEL_ELEM)) {
          ...
          node = tree.getNextElement(PSDisplayText.XML_NODE_NAME, firstFlags);
          m_errorLabel = new PSDisplayText(node, parentDoc, parentComponents);
          ...
          while (node != null) {
            m_readOnlyRules.add(new PSRule(node, parentDoc, parentComponents));
          ...
          m_customActionGroup = new PSCustomActionGroup(node, parentDoc, parentComponents);
        }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_name != null) root.setAttribute(NAME_ATTR, m_name);
    if (m_defaultSet != null) root.setAttribute(DEFAULT_SET_ATTR, m_defaultSet);

    if (m_accessKey.length() > 0) root.setAttribute(ACCESSKEY_ATTR, m_accessKey);
    ...
    // create the control
    if (m_control != null) root.appendChild(m_control.toXml(doc));
    ...
    if (it.hasNext()) {
      ...
      while (it.hasNext()) elem.appendChild(((IPSComponent) it.next()).toXml(doc));

      root.appendChild(elem);
    }

    // create the custom action group
    if (m_customActionGroup != null) root.appendChild(m_customActionGroup.toXml(doc));
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      ...
      if (m_control != null) m_control.validate(context);

      if (m_label != null) m_label.validate(context);

      if (m_errorLabel != null) m_errorLabel.validate(context);
