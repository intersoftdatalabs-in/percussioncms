...
public class PSRelationship extends PSComponent {
   ...
   */
  public PSRelationship(int id, PSLocator owner, PSLocator dependent, PSRelationshipConfig config) {
    ...
    if (m_config.getUserProperties().isEmpty()) {
      ...
      for (Map.Entry<String, String> entry : entries) {
        PSRelationshipPropertyData prop =
            new PSRelationshipPropertyData(entry.getKey(), entry.getValue());
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSRelationship)) return false;
  ...
  public Object clone() {
    ...
    relationship.m_userProperties = new ArrayList<>(m_userProperties);
  ...
  public void setGUID(IPSGuid newguid) {
    ...
    if (PSTypeEnum.valueOf(newguid.getType()) != PSTypeEnum.RELATIONSHIP) {
      throw new IllegalArgumentException("The type of newguid must be PSTypeEnum.RELATIONSHIP");
    }
    if (newguid.longValue() < 0L) {
      throw new IllegalArgumentException("The value of newguid must be > 0");
    }
    if (getId() > 0) {
  ...
  public void setDependent(PSLocator dependent) {
    if (dependent == null) throw new IllegalArgumentException("dependent may not be null");
  ...
  public final void setProperty(String name, String value) {
    ...
    if (m_config.getSysProperty(name) != null)
      throw new IllegalArgumentException("Cannot set the value for system property '" + name + "'");

    PSRelationshipPropertyData prop = findUserProperty(name);
    if (prop == null)
      throw new IllegalArgumentException("Unknown user property name: '" + name + "'");
  ...
  public Map<String, String> getUserProperties() {
    ...
    for (PSRelationshipPropertyData prop : m_userProperties) {
      value = prop.getValue();
      if (value != null && value.trim().length() > 0) userProps.put(prop.getName(), value);
    }
  ...
  public PSRelationshipPropertyData getUserProperty(String propertyName) {
    for (PSRelationshipPropertyData prop : m_userProperties) {
      if (prop.getName().equalsIgnoreCase(propertyName)) return prop;
    }
  ...
  public void setUserProperty(PSRelationshipPropertyData srcProperty) {
    if (srcProperty == null) throw new IllegalArgumentException("srcProperty may not be null.");
  ...
  public void fromXml(
    ...
    try {
      ...
      if (data != null && data.equals(XML_TRUE)) m_isPersisted = true;
      else m_isPersisted = false;
      ...
      try {
        setDependentObjectType(PSXMLDomUtil.checkAttributeInt(node, ATTR_OBJECTTYPE, false));
      } catch (NumberFormatException ex) {
        /* ignore already set */
      }
  ...
  public String toString() {
    return PSXmlDocumentBuilder.toString(toXml(PSXmlDocumentBuilder.createXmlDocument()));
  }
  ...
  private static final String ATTR_ISPERSISTED = "persisted";
