...
public final class PSQueryPipe extends PSPipe {
  ...
  public Element toXml(Document doc) {
    ...
    if (m_backEndDataTank != null) root.appendChild(m_backEndDataTank.toXml(doc));

    // PSDataMapper object
    if (m_dataMapper != null) root.appendChild(m_dataMapper.toXml(doc));

    // private PSDataSelector m_dataSelector = null;
    if (m_dataSelector != null) root.appendChild(m_dataSelector.toXml(doc));
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (tree.getNextElement(PSBackEndDataTank.ms_NodeType, firstFlags) != null) {
        m_backEndDataTank =
            new PSBackEndDataTank((Element) tree.getCurrent(), parentDoc, parentComponents);
      }
      ...
      if (tree.getNextElement(PSDataMapper.ms_NodeType, firstFlags) != null) {
        m_dataMapper = new PSDataMapper((Element) tree.getCurrent(), parentDoc, parentComponents);
      }

      tree.setCurrent(cur);

      // PSXDataSelector object
      if (tree.getNextElement(PSDataSelector.ms_NodeType, firstFlags) != null) {
        m_dataSelector =
            new PSDataSelector((Element) tree.getCurrent(), parentDoc, parentComponents);
      }
      ...
      if (tree.getNextElement(PSResourceCacheSettings.XML_NODE_NAME, firstFlags) != null) {
        m_cacheSettings = new PSResourceCacheSettings((Element) tree.getCurrent());
      } else // use defaults
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    if (m_backEndDataTank != null) {
      int joinSize =
          (m_backEndDataTank.getJoins() == null) ? 0 : m_backEndDataTank.getJoins().size();
      int tabSize =
          (m_backEndDataTank.getTables() == null) ? 0 : m_backEndDataTank.getTables().size();
  ...
  public boolean equals(Object o) {
    ...
    if (!super.equals(o)) bEqual = false;

    if (bEqual && o instanceof PSQueryPipe) {
      PSQueryPipe other = (PSQueryPipe) o;

      if (!compare(m_dataSelector, other.m_dataSelector)) bEqual = false;
      else if (!compare(m_cacheSettings, other.m_cacheSettings)) bEqual = false;
    }
    return bEqual;
  }
