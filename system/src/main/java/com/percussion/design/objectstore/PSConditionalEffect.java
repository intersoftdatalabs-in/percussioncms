...
public class PSConditionalEffect extends PSComponent {
  ...
  public PSConditionalEffect(PSExtensionCall effect) {
    if (effect == null) throw new IllegalArgumentException("effect may not be null.");
  ...
  public void setConditions(Iterator conds) {
    ...
    m_conditions.clear();
    while (conds.hasNext()) m_conditions.add(conds.next());
  }
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      m_activationEndPoint = sourceNode.getAttribute(XML_ATTR_ACTIVATIONENDPOINT);
      if (!PSRelationshipConfig.isActivationEndPointValid(m_activationEndPoint))
        m_activationEndPoint = PSRelationshipConfig.ACTIVATION_ENDPOINT_OWNER;

      // REQUIRED: the PSXExtensionCall element
      node = PSXMLDomUtil.getFirstElementChild(sourceNode, PSExtensionCall.ms_NodeType);
      ...
      if (optionalNode != null && optionalNode.getNodeName().equalsIgnoreCase(ELEM_CONDITIONS)) {
        hasConditions = true;
        node = PSXMLDomUtil.getFirstElementChild(optionalNode);
        while (node != null && node.getNodeName().equalsIgnoreCase(PSRule.XML_NODE_NAME)) {
      ...
      if (hasConditions) exeCtxNode = PSXMLDomUtil.getNextElementSibling(optionalNode);
      else exeCtxNode = optionalNode;
      if (exeCtxNode != null && exeCtxNode.getNodeName().equalsIgnoreCase(ELEM_EXE_CONTEXT_SET)) {
        node = PSXMLDomUtil.getFirstElementChild(exeCtxNode);
        while (node != null && node.getNodeName().equalsIgnoreCase(ELEM_EXE_CONTEXT)) {
          String name = node.getAttribute(ATTR_TYPE);
          Integer id = ms_ContextNameIdMapper.get(name.toLowerCase());
          if (id != null) m_exeContexts.add(id);
          else log.warn("Ignore invalid Execution Context, ' {} ,'.", name);
  ...
  public Element toXml(Document doc) {
    ...
    if (conditions.hasNext()) {
      ...
      while (conditions.hasNext()) elem.appendChild(((IPSComponent) conditions.next()).toXml(doc));
   ...
   */
  public Object clone() {
    PSConditionalEffect clone = new PSConditionalEffect((PSExtensionCall) m_effect.clone());
    ...
    while (iter.hasNext()) clone.m_conditions.add(((PSRule) iter.next()).clone());
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSConditionalEffect)) return false;

    PSConditionalEffect other = (PSConditionalEffect) obj;

    return new EqualsBuilder()
        .append(m_activationEndPoint, other.m_activationEndPoint)
        .append(m_conditions, other.m_conditions)
        .append(m_exeContexts, other.m_exeContexts)
        .append(m_effect, other.m_effect)
        .isEquals();
  }
   ...
   */
  public int hashCode() {
    return new HashCodeBuilder(32, 2)
        .appendSuper(super.hashCode())
        .append(m_activationEndPoint)
        .append(m_conditions)
        .append(m_exeContexts)
        .append(m_effect)
        .toHashCode();
  }
  ...
  public boolean hasExecutionContext(int testCtx) {
    ...
    for (Integer ctx : m_exeContexts) {
      if (ctx.intValue() == testCtx) return true;
    }
    return false;
  }
  ...
  public static Integer getExecutionContextValueForName(String executionContextName) {
    Integer ctxVal = null;
    if (StringUtils.isBlank(executionContextName)) return ctxVal;
  ...
  public static String getExecutionContextNameForValue(
    ...
    String ctxName = null;
    if (executionContextValue == null) return ctxName;
  ...
  static {
    ...
    ms_ContextNameIdMapper.put(
        RS_PRE_CONSTRUCTION.toLowerCase(), IPSExecutionContext.RS_PRE_CONSTRUCTION);
    ms_ContextNameIdMapper.put(
        RS_PRE_DESTRUCTION.toLowerCase(), IPSExecutionContext.RS_PRE_DESTRUCTION);
    ms_ContextNameIdMapper.put(RS_PRE_WORKFLOW.toLowerCase(), IPSExecutionContext.RS_PRE_WORKFLOW);
    ms_ContextNameIdMapper.put(
        RS_POST_WORKFLOW.toLowerCase(), IPSExecutionContext.RS_POST_WORKFLOW);
    ms_ContextNameIdMapper.put(RS_PRE_CLONE.toLowerCase(), IPSExecutionContext.RS_PRE_CLONE);
    ms_ContextNameIdMapper.put(RS_PRE_CHECKIN.toLowerCase(), IPSExecutionContext.RS_PRE_CHECKIN);
    ms_ContextNameIdMapper.put(RS_PRE_CHECKOUT.toLowerCase(), IPSExecutionContext.RS_POST_CHECKOUT);
    ms_ContextNameIdMapper.put(RS_PRE_UPDATE.toLowerCase(), IPSExecutionContext.RS_PRE_UPDATE);

    // init ms_ContextIdNameMapper
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_CONSTRUCTION, RS_PRE_CONSTRUCTION);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_DESTRUCTION, RS_PRE_DESTRUCTION);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_WORKFLOW, RS_PRE_WORKFLOW);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_POST_WORKFLOW, RS_POST_WORKFLOW);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_CLONE, RS_PRE_CLONE);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_CHECKIN, RS_PRE_CHECKIN);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_POST_CHECKOUT, RS_PRE_CHECKOUT);
    ms_ContextIdNameMapper.put(IPSExecutionContext.RS_PRE_UPDATE, RS_PRE_UPDATE);
  }
}
