...
/**
 * This class implements all of the IPSValidationContext methods with reasonable implementations.
 * startValidation always returns <CODE>true</CODE>.
 */
public class PSValidatorAdapter implements IPSValidationContext {
  ...
  public IPSComponent peekParent() {
    if (m_parentList.size() == 0) return null;
  ...
  public void validationWarning(IPSComponent component, int errorCode, Object[] args)
    ...
    if (m_treatWarningsAsErrors) validationError(component, errorCode, args);
  }
  ...
  public void validationError(IPSComponent component, int errorCode, Object[] args)
    ...
    if (m_logHandler != null) {
      ...
      if (cont instanceof PSApplication) app = (PSApplication) cont;
      ...
      PSValidationError err =
          new PSValidationError(appId, sessionId, ex.getErrorCode(), ex.getErrorArguments(), null);

      m_logHandler.write(err);
    }
    if (m_throwOnErrors) throw ex;
  }
  ...
  public boolean startValidation(IPSComponent component, String message) {
    trace("Validating " + component.getClass().getName() + ": message = " + message);

    return true;
  }

  protected void trace(String message) {
    if (m_traceOut != null) m_traceOut.println(message);
  }
