...
/**
 * This class is used to track relationships while copying a site or site subfolder. The collected
 * information will be used for post processes.
 */
public class PSRelationshipTracker {
  /**
   * Adds a source to target item mapping. This also looks up all relationships of related content
   * and stores it in its own map.
   *
   ...
   * @param target the target item copied from the source, not <code>null</code>.
   * @throws PSCmsException for any error.
   */
  public void addItemMapping(PSRequest request, PSLocator source, PSLocator target)
   ...
   * @param target the target folder copied from the source, not <code>null</code>.
   */
  ...
  /**
   * Convenience method that calls {@link #getFolderTarget(PSLocator) getFolderTarget(new
   * PSLocator(source)}.
   */
   ...
   *
   * @param source the source locator for which to get the locator of the copied folder, not <code>
   *     null</code>.
   * @return the value supplied as the target in the {@link #addFolderMapping( PSLocator, PSLocator)
   *     addFolderMapping(source, target)} method in which the locator supplied to this method was
   *     the locator supplied as the <code>source</code> of the {@link #addFolderMapping(PSLocator,
   ...
   *
   * @return an iterator over all source id's tracked as <code>Integer</code> objects, never <code>
   ...
   *
   * @param sourceId the source id for which we want the relationships, not <code>null</code>.
   * @return an iterator over all relationships as <code>PSRelationship</code> objects for the
   *     supplied source id, never <code>null</code>, may be empty.
   */
  ...
  /**
   * Maps source item id's as <code>Integer</code> to target item id's as <code>Integer</code>.
   * Initialized during construction, never <code>null</code> after that, updated through {@link
   * #addItemMapping(PSRequest, PSLocator, PSLocator)}.
   */
  private Map m_sourceTargetItemMap = new HashMap();

  /**
   * Maps source folder id as <code>Integer</code> to target folder as <code>PSLocator</code>.
   * Initialized during construction, never <code>null</code> after that, updated through {@link
   * #addFolderMapping(PSLocator, PSLocator)}.
   */
  private Map m_sourceTargetFolderMap = new HashMap();

  /**
   * Maps source item id's as <code>Integer</code> to source related content relationships as <code>
   * PSRelationshipSet</code>. Initialized during construction, never <code>null</code> after that,
   * updated through {@link #addItemMapping(PSRequest, PSLocator, PSLocator)}.
   */
