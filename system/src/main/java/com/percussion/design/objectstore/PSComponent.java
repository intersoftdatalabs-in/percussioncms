...
public abstract class PSComponent implements IPSComponent, Serializable {
  ...
  public static synchronized void setFeatureSet(PSFeatureSet fs) {
    if (null != ms_features) throw new IllegalStateException("Can only set feature set once.");
    ...
    while (features.hasNext()) {
      ...
      while (versions.hasNext()) {
        ...
        if (ver.getNumber() > maxVer) maxVer = ver.getNumber();
      }
   ...
   */
  protected static String getEnumeratedAttribute(
      PSXmlTreeWalker tree, String attrName, String[] legalValues, boolean ignoreCase)
      throws PSUnknownNodeTypeException {
    if (null == tree) throw new IllegalArgumentException("tree cannot be null");
    ...
    if (null == data || data.trim().length() == 0)
      ...
      for (int i = 0; !found && i < legalValues.length; i++) {
        if (legalValues[i] != null)
          found = ignoreCase ? legalValues[i].equalsIgnoreCase(data) : legalValues[i].equals(data);
      }
  ...
  protected static String getRequiredElement(
    ...
    if (elemName == null) throw new IllegalArgumentException("elemName cannot be null");
