...
/**
 * The IPSComponent interface defines required methods for all component level object store objects.
 * Component objects are contained within document objects, such as PSApplication. Components should
 * be derived from the PSComponent or PSCollectionComponent class rather than implement this
 * interface directly.
 *
 * @see PSComponent
...
public interface IPSComponent extends Cloneable {
   ...
   *
   * <p>NOTE: Currently, this method has been overridden only in a subset of the classes that
   * require it. Only those classes recursively aggregated by <code>PSDisplayMapper</code> are
   * correct.
   *
