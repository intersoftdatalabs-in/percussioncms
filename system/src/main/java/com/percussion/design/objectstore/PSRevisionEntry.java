...
public class PSRevisionEntry extends PSComponent {
  ...
  public Element toXml(Document doc) {
    ...
    // time
    PSXmlDocumentBuilder.addElement(doc, root, "time", new PSDateFormatISO8601().format(m_time));
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    if (time == null) m_time = new Date();
    else m_time = df.parse(time, new ParsePosition(0));
  }
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    if (m_majorVersion < 1) cxt.validationError(this, 0, "Invalid version: " + getVersion());

    if (m_minorVersion < 0) cxt.validationError(this, 0, "Invalid version: " + getVersion());

    if (m_agent == null) cxt.validationError(this, 0, "Revision entry agent is null");

    if (m_desc == null) cxt.validationError(this, 0, "Revision entry description is null.");

    if (m_time == null) cxt.validationError(this, 0, "Revision entry time is null.");
  }
