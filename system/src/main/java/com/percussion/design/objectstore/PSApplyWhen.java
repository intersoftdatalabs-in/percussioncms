...
public class PSApplyWhen extends PSCollectionComponent {
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (ifFieldEmpty != null) m_ifFieldEmpty = ifFieldEmpty.equalsIgnoreCase(BOOLEAN_ENUM[0]);

      // OPTIONAL: get all rules
      Element node = tree.getNextElement(PSRule.XML_NODE_NAME, firstFlags);
      while (node != null) {
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(IS_FIELD_EMPTY_ATTR, ifFieldEmpty() ? BOOLEAN_ENUM[0] : BOOLEAN_ENUM[1]);
