...
public abstract class PSPipe extends PSComponent {
  ...
  private static IllegalArgumentException validateName(String name) {
    if ((null == name) || (name.length() == 0))
      ...
      return new IllegalArgumentException(
          "pipe name is too big" + MAX_PIPE_NAME_LEN + " " + name.length());
    }
  ...
  private static IllegalArgumentException validateDescription(String description) {
    if ((description != null) && (description.length() > MAX_DESC_LEN)) {
      return new IllegalArgumentException(
          "pipe desc is too big" + MAX_DESC_LEN + " " + description.length());
    }
  ...
  private static IllegalArgumentException validateBackEndDataTank(PSBackEndDataTank dt) {
    if (dt == null) return new IllegalArgumentException("No backend data tank found");
  ...
  private static IllegalArgumentException validateDataMapper(PSDataMapper mapper) {
    if (mapper != null) {
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSPipe)) return false;
    PSPipe pipe = (PSPipe) o;
    boolean bEqual = true;
    if (!compare(m_name, pipe.m_name)) bEqual = false;
    else if (!compare(m_description, pipe.m_description)) bEqual = false;
    else if (!compare(m_backEndDataTank, pipe.m_backEndDataTank)) bEqual = false;
    else if (!compare(m_dataMapper, pipe.m_dataMapper)) bEqual = false;
    else if (!compare(m_resultDataExtensions, pipe.m_resultDataExtensions)) bEqual = false;
    else if (!compare(m_inputDataExtensions, pipe.m_inputDataExtensions)) bEqual = false;
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    cxt.pushParent(this);
    try {
      if (m_backEndDataTank != null) m_backEndDataTank.validate(cxt);

      if (m_dataMapper != null) m_dataMapper.validate(cxt);

      if (m_inputDataExtensions != null) m_inputDataExtensions.validate(cxt);

      if (m_resultDataExtensions != null) m_resultDataExtensions.validate(cxt);
    } finally {
      cxt.popParent();
    }
  }
  ...
  public Object clone() {
    ...
    if (m_backEndDataTank != null)
      copy.m_backEndDataTank = (PSBackEndDataTank) m_backEndDataTank.clone();
    if (m_dataMapper != null) copy.m_dataMapper = (PSDataMapper) m_dataMapper.clone();
    if (m_resultDataExtensions != null)
      copy.m_resultDataExtensions = (PSExtensionCallSet) m_resultDataExtensions.clone();
    if (m_inputDataExtensions != null)
      copy.m_inputDataExtensions = (PSExtensionCallSet) m_inputDataExtensions.clone();
    return copy;
  }
