...
public class PSRelationshipConfigSet extends PSCollectionComponent implements IPSConfig {
  ...
  public PSRelationshipConfig getConfig(IPSGuid id) {
    if (id == null) throw new IllegalArgumentException("GUID (id) cannot be null or empty");

    Iterator configs = iterator();
    while (configs.hasNext()) {
      PSRelationshipConfig config = (PSRelationshipConfig) configs.next();
      if (config.getGUID().longValue() == id.longValue()) return config;
    }
  ...
  public PSRelationshipConfig getConfigByCategory(String category) {
    ...
    while (configs.hasNext()) {
      ...
      if (value != null && value.equals(category)) return config;
    }
    return null;
  }
  ...
  public List<PSRelationshipConfig> getConfigsByType(String type) {
    if ((!PSRelationshipConfig.RS_TYPE_SYSTEM.equals(type))
      ...
      throw new IllegalArgumentException(
          "type must be either PSRelationshipConfig.RS_TYPE_SYSTEM or"
    ...
    while (configs.hasNext()) {
      PSRelationshipConfig config = (PSRelationshipConfig) configs.next();
      if (config.getType().equals(type)) list.add(config);
    }
    return list;
  }
  ...
  public List<PSRelationshipConfig> getConfigList() {
    List<PSRelationshipConfig> list = new ArrayList<>();
    Iterator configs = iterator();
    while (configs.hasNext()) list.add((PSRelationshipConfig) configs.next());
  ...
  public List<PSRelationshipConfig> getConfigListByCategory(String category) {
    ...
    while (configs.hasNext()) {
      PSRelationshipConfig config = (PSRelationshipConfig) configs.next();
      value = config.getCategory();
      if (value != null && value.equals(category)) list.add(config);
    }
    return list;
  }
  ...
  public PSRelationshipConfig getConfigByNameOrCategory(String nameOrCategory) {
    ...
    if (config == null) config = getConfigByCategory(nameOrCategory);
    return config;
  }
  ...
  public PSRelationshipConfig addConfig(String name, String type) {
    ...
    if (getConfig(name) != null) {
      throw new IllegalArgumentException(
          "can not add config with name " + name + ", reason:duplicate");
    }
  ...
  public void deleteConfig(String name) {
    ...
    if (config != null) {
      if (config.isUser()) remove(config);
      else
        throw new IllegalArgumentException("name represents system configuration, can not delete.");
    }
  }
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      node = tree.getNextElement(PSRelationshipConfig.XML_NODE_NAME, firstFlags);
      while (node != null) {
        ...
        PSRelationshipConfig config =
            createMemberObject((Element) tree.getCurrent(), parentDoc, parentComponents);
        ...
        node = tree.getNextElement(PSRelationshipConfig.XML_NODE_NAME, nextFlags);
      }
    } finally {
