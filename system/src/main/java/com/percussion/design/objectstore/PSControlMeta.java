...
public class PSControlMeta extends PSComponent {
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    m_deprecated = getEnumeratedAttribute(tree, XML_DEPRECATE_ATTR, XML_DEPRECATE_ENUM);
    ...
    m_dimension = getEnumeratedAttribute(tree, XML_DIMENSION_ATTR, XML_DIMENSION_ENUM);
    m_choiceSet = getEnumeratedAttribute(tree, XML_CHOICESET_ATTR, XML_CHOICESET_ENUM);
    m_params.clear();
    ...
    while (child != null) {
      ...
      if (childName.equals(XELEM_DESCRIPTION)) setDescription(tree.getElementData(child));
  ...
  public Element toXml(Document doc) {
    ...
    if (m_displayName.length() > 0) root.setAttribute(XML_DISPLAYNAME_ATTR, m_displayName);
    root.setAttribute(XML_DIMENSION_ATTR, m_dimension);
    root.setAttribute(XML_CHOICESET_ATTR, m_choiceSet);
    if (m_description.length() > 0)
      PSXmlDocumentBuilder.addElement(doc, root, XELEM_DESCRIPTION, m_description);
  ...
  public static final String TABLE_DIMENSION = "table";

  /** Constant for controls that expect one-dimensional data */
  public static final String ARRAY_DIMENSION = "array";
