...
public final class PSContentEditorSharedDef extends PSComponent implements IPSDocument {
  ...
  public PSCollection lookupFieldGroupByFileName(String fileName) {
    ...
    while (iter.hasNext()) {
      ...
      if (g.getFilename().equals(fileName)) fieldGroups.add(g);
    }
    return fieldGroups;
  }
  ...
  public void setFieldGroupsByFileName(Iterator group) {
    ...
    while (group.hasNext()) m_fieldGroups.add(group.next());
  }
  ...
  public final void setFieldGroups(PSCollection fieldGroups) {
    ...
    if (!fieldGroups.getMemberClassName().equals(m_fieldGroups.getMemberClassName()))
      throw new IllegalArgumentException("PSSharedFieldGroup collection expected");
  ...
  public PSSharedFieldGroup getSharedGroup(String groupName) {
    if (groupName == null || groupName.trim().length() == 0) {
      throw new IllegalArgumentException("groupName may not be null or " + "empty");
    }
    ...
    while (groups.hasNext() && group == null) {
      ...
      if (groupName.equals(test.getName())) group = test;
    }
  ...
  public final void fromXml(
    ...
    try {
      ...
      node = tree.getNextElement(PSSharedFieldGroup.XML_NODE_NAME, firstFlags);
      while (node != null) {
        m_fieldGroups.add(new PSSharedFieldGroup(node, parentDoc, parentComponents));
        node = tree.getNextElement(PSSharedFieldGroup.XML_NODE_NAME, nextFlags);
      }
  ...
  public Element toXml(Document doc) {
    ...
    if (getStylesheetSet() != null) root.appendChild(getStylesheetSet().toXml(doc));

    // OPTIONAL: application flow
    if (getApplicationFlow() != null) root.appendChild(getApplicationFlow().toXml(doc));
  ...
  public boolean isSharedField(PSField field, List groupIncludes, List fieldExcludes) {
    ...
    if (groupIncludes == null) throw new IllegalArgumentException("groupIncludes may not be null");
  ...
  public boolean isSharedFieldSet(PSFieldSet set, List groupIncludes) {
    ...
    if (groupIncludes == null) throw new IllegalArgumentException("groupIncludes cannot be null");

    Iterator groups = getFieldGroups();
    while (groups.hasNext()) {
      PSSharedFieldGroup group = (PSSharedFieldGroup) groups.next();
      if (groupIncludes.contains(group.getName())
          && group.getFieldSet().getName().equals(set.getName())) return true;
    }
    return false;
  }
