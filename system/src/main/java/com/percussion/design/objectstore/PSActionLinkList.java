...
public class PSActionLinkList extends PSCollectionComponent {
  ...
  public PSActionLinkList(PSActionLink actionLink) {
    ...
    if (actionLink == null) throw new IllegalArgumentException("actionLink cannot be null");
    add(actionLink);
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSActionLinkList)) return false;
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      Element node = tree.getNextElement(PSActionLink.XML_NODE_NAME, firstFlags);
      while (node != null) {
        add(new PSActionLink(node, parentDoc, parentComponents));

        node = tree.getNextElement(PSActionLink.XML_NODE_NAME, nextFlags);
      }
    } finally {
