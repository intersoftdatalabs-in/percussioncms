...
public class PSDisplayMapper extends PSCollectionComponent {
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSDisplayMapper)) return false;

    PSDisplayMapper t = (PSDisplayMapper) o;

    boolean equal = true;
    if (!PSComponent.compare(m_fieldSetRef, t.m_fieldSetRef)) equal = false;

    if (size() != t.size()) equal = false;
    else {
      for (int i = 0; i < size() && equal; i++) {
        IPSComponent c1 = (IPSComponent) get(i);
        IPSComponent c2 = (IPSComponent) t.get(i);
        if (!PSComponent.compare(c1, c2)) equal = false;
      }
    }
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      node = tree.getNextElement(PSDisplayMapping.XML_NODE_NAME, firstFlags);

      // get all mappings
      while (node != null) {
        add(new PSDisplayMapping(node, parentDoc, parentComponents));

        node = tree.getNextElement(PSDisplayMapping.XML_NODE_NAME, nextFlags);
      }
    } finally {
  ...
  public Element toXml(Document doc) {
    ...
    while (it.hasNext()) root.appendChild(((IPSComponent) it.next()).toXml(doc));
  ...
  public PSPair<PSDisplayMapping, Integer> getMappingAndSequence(String fieldRef) {
    if (fieldRef == null || fieldRef.trim().length() == 0)
      throw new IllegalArgumentException("fieldRef may not be null or empty.");
    ...
    while (it.hasNext() && matchedMapping == null) {
      ...
      if (mapping.getFieldRef().equals(fieldRef)) matchedMapping = mapping;
    ...
    if (matchedMapping == null) return null;
    else return new PSPair<>(matchedMapping, sequence);
  }
  ...
  public void replaceMapping(PSDisplayMapping mapping) {
    ...
    throw new IllegalArgumentException("could not find the matching mapping, nothing replaced");
  }
  ...
  public PSDisplayMapper merge(
      ...
      Iterator defaultUISet,
      PSDisplayMapper sourceMapper,
      Iterator sourceDefaultUISet,
      boolean mergeChild,
      List fieldExcludes)
      throws PSSystemValidationException {
    if (sourceMapper == null) throw new IllegalArgumentException("sourceMapper may not be null.");
    ...
    PSDisplayMapper mapper = removeExcludedMappings(sourceMapper, fieldExcludes);
    ...
    if (mergeChild) {
      ...
      while (mappings.hasNext()) {
        ...
        String fieldRef = mapping.getFieldRef();
        if (!usedMappings.contains(fieldRef)) addMapping(mergedMapper, sourceDefaultUISet, mapping);
      }
    }
  ...
  public PSDisplayMapper demerge(
      ...
      Iterator defaultUISet,
      PSDisplayMapper sourceMapper,
      Iterator sourceDefaultUI,
      boolean demergeChild,
      List fieldExcludes)
      throws PSSystemValidationException {
    if (sourceMapper == null) throw new IllegalArgumentException("sourceMapper may not be null.");
  ...
  private void mergeMapper(
      PSDisplayMapper targetDisplayMapper,
      boolean mergeDefault,
      Iterator targetDefaultUISet,
      List fieldExcludes,
      PSDisplayMapper sourceMapper,
      Iterator sourceDefaultUISet,
      boolean mergeChild,
    ...
    while (mappings.hasNext()) {
      ...
      if (fieldExcludes.contains(fieldRef)) continue;

      // get the matching source mapping
      PSDisplayMapping sourceMapping = null;
      if (sourceMapper != null) {
        PSDisplayMapper childMapper = removeExcludedMappings(sourceMapper, fieldExcludes);

        if (fieldRef.equalsIgnoreCase(childMapper.getFieldSetRef()) && mergeChild) {
          // add the group's mapper as a child mapper to this mapping
          PSDisplayMapping tmpMapping = new PSDisplayMapping(fieldRef, mapping.getUISet());

          if (mapping.getDisplayMapper() != null) {
            tmpMapping.setDisplayMapper(
                getMergedChildMapper(
                    childMapper,
                    sourceDefaultUISet,
                    mapping.getDisplayMapper(),
                    mergeDefault,
                    targetDefaultUISet));
          } else addUIMapper(tmpMapping, sourceDefaultUISet, childMapper);
          ...
          PSDisplayMapping tmpMapping =
              mergeMapping(
                  sourceDefaultUISet, sourceMapping, mergeDefault, targetDefaultUISet, mapping);
  ...
  private PSDisplayMapper removeExcludedMappings(PSDisplayMapper mapper, List excludes) {
    ...
    while (mappings.hasNext()) {
      PSDisplayMapping mapping = (PSDisplayMapping) mappings.next();
      if (excludes.contains(mapping.getFieldRef())) continue;
   ...
   * @throws PSSystemValidationException if there are any errors.
   */
  private void addUIMapper(
      PSDisplayMapping targetMapping, Iterator sourceDefaultUI, PSDisplayMapper sourceMapper)
      throws PSSystemValidationException {
    ...
    while (i.hasNext()) {
      ...
      PSDisplayMapping tmpMapping = addMapping(targetMapper, sourceDefaultUI, mapping);

      // do the same for any child mappers
      PSDisplayMapper childMapper = mapping.getDisplayMapper();
      if (childMapper != null) addUIMapper(tmpMapping, sourceDefaultUI, childMapper);
    }
  ...
  private PSDisplayMapping addMapping(
      PSDisplayMapper targetMapper, Iterator sourceDefaultUI, PSDisplayMapping sourceMapping)
      throws PSSystemValidationException {
    // create new mapping and merge default values
    String fieldRef = sourceMapping.getFieldRef();
    PSDisplayMapping tmpMapping =
        new PSDisplayMapping(
            fieldRef, mergeDefaultUISet(sourceDefaultUI, sourceMapping.getUISet()));
  ...
  private PSDisplayMapping mergeMapping(
      ...
      PSDisplayMapping targetMapping)
      throws PSSystemValidationException {
    PSDisplayMapping resultMapping = null;

    // merge source with it's default
    PSUISet sourceUISet = mergeDefaultUISet(sourceDefaultUI, sourceMapping.getUISet());
    ...
    if (mergeDefault) {
      tmpUISet = mergeDefaultUISet(targetDefaultUI, targetMapping.getUISet());
    } else tmpUISet = targetMapping.getUISet();

    // now merge target with source
    tmpUISet = tmpUISet.merge(sourceUISet);

    // create our result mapping
    resultMapping = new PSDisplayMapping(targetMapping.getFieldRef(), tmpUISet);
  ...
  private PSDisplayMapper getMergedChildMapper(
      PSDisplayMapper sourceChildMapper,
      Iterator sourceDefaultUI,
      PSDisplayMapper targetChildMapper,
      boolean mergeDefault,
      Iterator targetDefaultUI)
      throws PSSystemValidationException {
    ...
    while (mappings.hasNext()) {
      PSDisplayMapping mapping = (PSDisplayMapping) mappings.next();
      String fieldRef = mapping.getFieldRef();
      PSDisplayMapping sourceChildMapping = (PSDisplayMapping) sourceMappings.get(fieldRef);
      ...
      if (sourceChildMapping != null) {
        ...
        PSDisplayMapping tmpMapping =
            mergeMapping(
                sourceDefaultUI, sourceChildMapping, mergeDefault, targetDefaultUI, mapping);
    ...
    PSDisplayMapper resultMapper = new PSDisplayMapper(targetChildMapper.getFieldSetRef());
      ...
      if (defaultUISet != null) {
        while (defaultUISet.hasNext()) {
          ...
          if (defUI.getName().equals(defaultName)) {
            ...
            result = ((PSUISet) uiSet.clone()).merge((PSUISet) defUI.clone());
            isMatch = true;
            break;
          }
        }
  ...
  private void demergeMapper(
    ...
    while (mappings.hasNext()) {
      ...
      if (sourceMapper != null) {
        ...
        if (fieldRef.equalsIgnoreCase(childMapper.getFieldSetRef()) && demergeChild) {
          ...
          PSDisplayMapping tmpMapping =
              demergeMapping(sourceDefaultUI, sourceMapping, demergeDefault, defaultUISet, mapping);
  ...
  private PSDisplayMapper getDemergedChildMapper(
    ...
    while (mappings.hasNext()) {
      ...
      if (sourceChildMapping != null) {
        ...
        PSDisplayMapping tmpMapping =
            demergeMapping(
                sourceDefaultUI, sourceChildMapping, demergeDefault, targetDefaultUI, mapping);
      ...
      if (defaultUISet != null) {
        while (defaultUISet.hasNext()) {
          ...
          if (defUI.getName().equals(defaultName)) {
            ...
            // original versions in anyway.
            result = ((PSUISet) uiSet.clone()).demerge((PSUISet) defUI.clone());
            isMatch = true;
            break;
          }
        }
