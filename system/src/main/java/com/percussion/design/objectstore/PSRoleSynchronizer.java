...
public class PSRoleSynchronizer {
  ...
  public PSRoleSynchronizer(Properties connInfo)
    ...
    if (null == connInfo) throw new IllegalArgumentException("properties can't be null");
    try {
      if (null == connInfo.getProperty("hostName")) connInfo.setProperty("hostName", "localhost");
      if (null == connInfo.getProperty("port")) connInfo.setProperty("port", "9992");
  ...
  public void saveRoleConfiguration(PSRoleConfiguration roles, boolean releaseLock)
      throws PSServerException,
          ...
          PSLockedException {
    if (null == roles) {
      throw new IllegalArgumentException("a valid role config must be supplied");
    }
