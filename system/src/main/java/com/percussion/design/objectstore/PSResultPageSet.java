...
/**
 * The PSResultPageSet class defines the results being generated by a data set. One data set may
 * produce HTML or XML results. It may also associate a different stylesheet with the output,
...
public final class PSResultPageSet extends PSComponent implements IPSResults {
  ...
  /**
   * Get the result pages defined as output from this data set. Result pages are used to specify the
   * style sheet to use for returned pages. Conditionals can be associated with the result pages
   * allowing different results to be generated based upon the request context.
   *
   * @return a collection containing the result pages (PSResultPage objects) (may be null)
   */
  ...
  /**
   * Overwrite the result pages associated with the specified collection. If you only want to modify
   * certain pages, add a new page, etc. use getResultPages to get the existing collection and
   * modify the returned collection directly.
   *
   * <p>Result pages are used to specify the style sheet to use for returned pages. Conditionals can
   * be associated with the result pages allowing different results to be generated based upon the
   * request context.
   *
   * <p>The PSCollection object supplied to this method will be stored with the PSResultPageSet
   * object. Any subsequent changes made to the object by the caller will also effect the results
   * object.
   *
  ...
  private static IllegalArgumentException validateResultPages(PSCollection pages) {
    if (pages != null) {
      ...
      if (!com.percussion.design.objectstore.PSResultPage.class.isAssignableFrom(
        ...
        return new IllegalArgumentException(
            "coll bad content type, Result Page: " + pages.getMemberClassName());
      }
    }
  ...
  /**
   * This method is called to create a PSXResultPageSet XML element node containing the data
  ...
  public Element toXml(Document doc) {
    ...
    if (m_resultPages != null) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (null != page) root.appendChild(page.toXml(doc));
      }
    }
  ...
  /**
   * This method is called to populate a PSResultPageSet Java object from a PSXResultPageSet XML
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (tree.getNextElement(curNodeType, firstFlags) != null) {
        PSResultPage page;
        do {
          page = new PSResultPage((Element) tree.getCurrent(), parentDoc, parentComponents);
    ...
    try {
      if (m_resultPages != null) {
        for (int i = 0; i < m_resultPages.size(); i++) {
          ...
          if (o == null) cxt.validationError(this, 0, "result page " + i + " == null");
