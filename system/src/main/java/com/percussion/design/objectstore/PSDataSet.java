...
public class PSDataSet extends PSComponent {
  ...
  private static IllegalArgumentException validateName(String name) {
    if ((null == name) || (name.length() == 0))
      ...
      return new IllegalArgumentException(
          "dataset name is too big" + MAX_DATASET_NAME_LEN + " " + name.length());
    }
  ...
  public void copyFrom(PSDataSet ds) {
    ...
    if (ds.isTransactionDisabled()) setTransactionDisabled();
    else if (ds.isTransactionForAllRows()) setTransactionForAllRows();
    else if (ds.isTransactionForRow()) setTransactionForRow();
  ...
  public Element toXml(Document doc) {
    ...
    if (m_transactionType == DS_TRANSACTION_ROW)
      PSXmlDocumentBuilder.addElement(doc, root, "transactionType", XML_FLAG_XACT_ROW);
    else if (m_transactionType == DS_TRANSACTION_ALL_ROWS)
      PSXmlDocumentBuilder.addElement(doc, root, "transactionType", XML_FLAG_XACT_ALL_ROWS);
    else PSXmlDocumentBuilder.addElement(doc, root, "transactionType", XML_FLAG_XACT_NONE);
    ...
    if (m_pipe != null) root.appendChild(m_pipe.toXml(doc));

    // private          PSPageDataTank  m_pageDataTank = null;
    if (m_pageDataTank != null) root.appendChild(m_pageDataTank.toXml(doc));

    // private          PSRequestor     m_requestor = null;
    if (m_requestor != null) root.appendChild(m_requestor.toXml(doc));

    // private          IPSResults      m_results;
    if (m_results != null) root.appendChild(((IPSComponent) m_results).toXml(doc));

    // private         PSResultPager      m_resultPager;
    if (m_resultPager != null) root.appendChild(m_resultPager.toXml(doc));
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    if (m_pageDataTank == null && (m_requestor != null && !m_requestor.isDirectDataStream()))
    ...
    if (m_results != null) {
      if (m_results instanceof PSResultPageSet) {
        PSCollection resultPages = ((PSResultPageSet) m_results).getResultPages();
        HashMap fieldLinks = new HashMap();
        int size = 0;

        if (resultPages != null) size = resultPages.size();
        ...
        for (int i = 0; i < size; i++) {
          ...
          for (int j = 0; j < requestLinks.size(); j++) {
            PSRequestLink requestLink = (PSRequestLink) requestLinks.get(j);
    ...
    try {
      ...
      if (m_pipe != null) m_pipe.validate(cxt);

      if (m_pageDataTank != null) m_pageDataTank.validate(cxt);

      if (m_requestor != null) m_requestor.validate(cxt);

      if (m_resultPager != null) m_resultPager.validate(cxt);
  ...
  public Object clone() {
    ...
    if (m_pipe != null) copy.m_pipe = (PSPipe) m_pipe.clone();
    if (m_dataEncryptor != null) copy.m_dataEncryptor = (PSDataEncryptor) m_dataEncryptor.clone();
    if (m_pageDataTank != null) copy.m_pageDataTank = (PSPageDataTank) m_pageDataTank.clone();
    if (m_requestor != null) copy.m_requestor = (PSRequestor) m_requestor.clone();
    if (m_resultPager != null) copy.m_resultPager = (PSResultPager) m_resultPager.clone();
    if (m_results != null) copy.m_results = (IPSResults) m_results.clone();
    return copy;
  }
