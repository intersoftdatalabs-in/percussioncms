...
public class PSChoiceTableInfo extends PSComponent {
  ...
  public Iterator<PSEntry> getChoiceEntries() {
    ...
    if (StringUtils.isBlank(valueColumn)) valueColumn = m_lableColumn;
    Connection con = null;
    try {
      ...
      String tableName =
          PSSqlHelper.qualifyTableName(
              m_tableName, detail.getDatabase(), detail.getOrigin(), detail.getDriver());

      PreparedStatement stmt = null;
      String sql = "SELECT " + m_lableColumn + "," + valueColumn + " FROM " + tableName;
      ...
      if (con != null) {
        try {
          ...
          ms_log.error("Unable to close the SQL connection while getting the choice entries.", e);
        }
      }
    }
  ...
  private PSEntry createInvalidPSEntryChoice() {
    return new PSEntry("invalid", new PSDisplayText("Error! Invalid Table Info"));
  }
  ...
  public void setLableColumn(String lableColumn) {
    ...
    else m_lableColumn = lableColumn;
  }
