...
public class PSAttribute extends PSDatabaseComponentCollection {
  ...
  public void setValues(List list) {
    ...
    if (list != null)
      for (int i = 0; i < list.size(); i++)
        if (list.get(i) != null) add(new PSAttributeValue(list.get(i).toString()));
  }
  ...
  public void merge(PSAttribute attr) {
    if (attr == null) throw new IllegalArgumentException("attr may not be null");
    ...
    for (int i = 0; i < attr.size(); i++) {
      ...
      if (!curVals.contains(text)) curVals.add(text);
    }
    setValues(curVals);
  }
