...
/**
 * The PSFile abstract class represents the file data of a request to load, save, or remove a file.
 * It contains an input stream from which the raw file data can be read.
 *
 * <p>This object does <B>not</B> encapsulate a request to load, save, or remove a file. It is the
 * result of, or an argument to, such a request.
 *
 * @author Jian Huang
 * @version 1.1
 * @since 1.1
...
public abstract class PSFile extends PSComponent {
  ...
  /**
   * Constructor with a file, for sending requests to load or remove a file. All returned paths for
   * this file will be normalized.
   *
  ...
  protected PSFile(File fileName) {
    try {
      ...
      throw new IllegalArgumentException("invalid file name: PSFile/constructor");
    }
  }

  /**
   * Construct a file object. All returned paths for this file will be normalized.
   *
   * @param stream an input stream positioned at the start of the file data. If this object is to be
   *     converted to XML and/or serialized in the form of a request, then this stream must remain
   *     open and unchanged until the conversion process is complete. After the conversion, the
   *     input stream will be closed by this object. So consider the input stream as belonging only
   *     to this object.
   * @param fileName the file name.
   * @param lastModified the modification date of the file, or 0L if not known or undefined.
   * @deprecated Use {@link #PSFile(InputStream, URL, long) PSFile(InputSream, URL, long)} instead
   */
  protected PSFile(InputStream stream, File fileName, long lastModified) {
    if (stream == null) {
      throw new IllegalArgumentException("" + "no input stream: PSFile/constructor");
    }
  ...
  /**
   * Construct a file object from MIME content. Ususally one does this in order to save this content
   * to the server.
   *
   * @param content MIME content whose stream is positioned at the start of the data. If this object
   *     is to be converted to XML and/or serialized in the form of a request, then this content
   *     must remain open and unchanged until this conversion process is complete. After the
   *     conversion, the content will be closed by this object. So consider the content as belonging
   *     only to this object.
   * @param fileName the file name.
   * @param lastModified the modification date of the file, or 0L if not known or undefined.
   * @deprecated Use {@link #PSFile(IPSMimeContent, URL, long) PSFile( IPSMimeContent, URL, long)}
   *     instead
   */
  protected PSFile(IPSMimeContent content, File fileName, long lastModified) {
    if (content == null) {
      throw new IllegalArgumentException("no input content: PSFile/constructor");
    }
  ...
  /**
   * Construct a file object from MIME content. Ususally one does this in order to save this content
   * to the server. All returned paths for this file will be normalized.
   *
   * @param content MIME content whose stream is positioned at the start of the data. If this object
   *     is to be converted to XML and/or serialized in the form of a request, then this content
   *     must remain open and unchanged until this conversion process is complete. After the
  ...
  private static IllegalArgumentException validateFileName(File fileName) {
    if (fileName == null || fileName.getPath().trim().length() == 0) {
      throw new IllegalArgumentException("file name does not exist: validateFileName");
    }
   ...
   *
   * @return long value representing the time the file was last modified, measured in milliseconds
   *     since the epoch (00:00:00 GMT, January 1, 1970), or 0L if the last modified time is not
   *     defined or not available. This time will reflect the clock of the Java VM on the server if
   *     this file is the result of a load file request.
   */
  ...
  /**
   * Gets the file name of this object, understood to be relative to the extension on the server.
   * The extension is not known to this object -- it is supplied by other means when a request is
   * made.
   *
  ...
  /**
   * Gets the MIME content from the object and transfers to the caller the responsibility for
   * releasing the stream. This method can only be called once because this object releases all
   * references to the content.
   *
   * @return an IPSMimeContent object
   * @exception PSIllegalStateException if the input stream has already been retrieved from this
   *     object (the ownership of the stream is transferred to the first caller of this method).
   *     This is to avoid situations where multiple objects are reading from the stream
   *     simultaneously. This exception can also be thrown if the {@link #toXml(Document) toXml}
  ...
  /**
   * Create an XML element node with the appropriate format for the given object. An element node
   * may contain a hierarchical structure, including child objects. The element node can also be a
   * child of another element node.
   *
  ...
  protected Element toXml(Document doc, Element root) {
    ...
    if (m_content != null) {
      ...
      Element contentEl = PSXmlDocumentBuilder.addElement(doc, root, CONTENT, content);
  ...
  /**
   * Populate an object from an XML element node. An element node may contain a hierarchical
   * structure, including child objects. The element node can also be a child of another element
   * node.
   *
  ...
  /**
   * This method is called by subclasses' <code>fromXml(Element, IPSDocument, ArrayList)</code>
   * method.
   *
   * @param sourceNode the XML element node to construct this object from, (may not be <code>null
   *     </code>)
  ...
  protected void fromXml(
    ...
    try {
      ...
      m_url = new URL("file", "", getRequiredElementValue(walker, FILE_NAME, nodeType));
      ...
      if (contentEl != null) {
        ...
        long len = -1L;
        {
          ...
          if (length != null && length.length() > 0) len = Long.parseLong(length, ms_radix);
        }
        ...
        PSMimeContentAdapter mimeContent =
            new PSMimeContentAdapter(in, mimeType, xferEnc, charEnc, len);
  ...
  private static String getRequiredElementValue(
      PSXmlTreeWalker walker, String elementName, String nodeType)
      throws PSUnknownNodeTypeException {
   ...
   * </CODE> should not catch it unless it is to be re-thrown).
   *
   * @param cxt the validation context.
   * @exception PSSystemValidationException depends on the implementation of the validation context
   ...
   * @return <CODE>true</CODE> if the given file object represents the same abstract filename
   */
  @Override
  public boolean equals(Object o) {
    ...
    if (!compare(m_url, other.m_url)) return false;
  ...
  /**
   * Fixes up a filename's separators to be safe on Unix systems. If the current system path
   * separator is a back-slash then any back- slashes found in the filename will be converted to
   * forward-slashes. If the current system path separator is a forward-slash, the filename is
  ...
  /**
   * The file name, relative to the file root as a URL. May be null if constructed with no
   * arguments.
   */
  URL m_url = null;
