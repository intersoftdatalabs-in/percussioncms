...
public class PSContentEditor extends PSDataSet {
   ...
   */
  public PSContentEditor(
      String name, long contentType, int workflowId, PSCommandHandlerStylesheets stylesheetSet) {
  ...
  public Object clone() {
    ...
    copy.m_inputTranslations = (PSInputTranslations) m_inputTranslations.clone(); // TODO
    copy.m_outputTranslations = (PSOutputTranslations) m_outputTranslations.clone(); // TODO
    ...
    copy.m_stylesheetSet = (PSCommandHandlerStylesheets) m_stylesheetSet.clone(); // TODO
    copy.m_validationRules = (PSValidationRules) m_validationRules.clone(); // TODO

    if (m_workflowInfo != null) copy.m_workflowInfo = (PSWorkflowInfo) m_workflowInfo.clone();
  ...
  public void setSectionLinkList(PSCollection sectionList) {
    if (sectionList != null
        && !sectionList.getMemberClassName().equals(m_sectionLinkList.getMemberClassName()))
      throw new IllegalArgumentException("PSUrlRequest collection expected");

    m_sectionLinkList.clear();
    if (sectionList != null) m_sectionLinkList.addAll(sectionList);
  }
  ...
  public void setStylesheetSet(PSCommandHandlerStylesheets stylesheetSet) {
    if (stylesheetSet == null) throw new IllegalArgumentException("stylesheetSet cannot be null");
  ...
  public void setValidationRules(PSValidationRules groupValidations) {
    ...
    if (groupValidations != null) {
      ...
      m_validationRules.setMaxErrorsToStop(groupValidations.getMaxErrorsToStop());
    }
  }
  ...
  public void setInputTranslation(PSInputTranslations groupInputTranslations) {
    ...
    if (groupInputTranslations != null) m_inputTranslations.addAll(groupInputTranslations);
  }
  ...
  public void setOutputTranslation(PSOutputTranslations groupOutputTranslations) {
    ...
    if (groupOutputTranslations != null) m_outputTranslations.addAll(groupOutputTranslations);
  }
  ...
  public PSCustomActionGroup getCustomActionGroup(int pageType, int pageLocation, String fieldRef) {
    ...
    while (groups.hasNext()) {
      ...
      if (group.getLocation().hasCustomActions(pageType, pageLocation, fieldRef)) {
        matchGroup = group;
        break;
      }
    }
  ...
  public void addCustomActionGroup(PSCustomActionGroup customActionGroup) {
    if (null == customActionGroup) throw new IllegalArgumentException("action cannot be null");
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (data != null) m_enableRelatedContent = data.toString().trim().equalsIgnoreCase("yes");
      else m_enableRelatedContent = false;

      // OPTIONAL: get the producesResource flag, defaults to false
      data = tree.getElementData(PRODUCES_RESOURCE_ATTR);
      if (data != null) m_producesResource = data.toString().trim().equalsIgnoreCase("yes");
      else m_producesResource = false;
      ...
      if (data != null) {
        // if supplied, it must be valid
        try {
          if (StringUtils.isBlank(data) || !ArrayUtils.contains(ms_iconSources, data))
            throw new RuntimeException("");
          m_iconSource = data;
        } catch (Exception e) {
      ...
      node = tree.getNextElement(PSCommandHandlerStylesheets.XML_NODE_NAME, nextFlags);
      if (node != null) {
        m_stylesheetSet = new PSCommandHandlerStylesheets(node, parentDoc, parentComponents);
      }
      ...
      while (node != null) {
        ...
        if (elementName.equals(PSApplicationFlow.XML_NODE_NAME)) {
          m_applicationFlow = new PSApplicationFlow(node, parentDoc, parentComponents);
          ...
          while (node != null) {
            urlRequest = new PSUrlRequest(node, parentDoc, parentComponents);
          ...
          m_validationRules = new PSValidationRules(node, parentDoc, parentComponents);
        } else if (elementName.equals(PSInputTranslations.XML_NODE_NAME)) {
          m_inputTranslations = new PSInputTranslations(node, parentDoc, parentComponents);
        } else if (elementName.equals(PSOutputTranslations.XML_NODE_NAME)) {
          m_outputTranslations = new PSOutputTranslations(node, parentDoc, parentComponents);
        } else if (elementName.equals(PSCustomActionGroup.XML_NODE_NAME)) {
          m_customActionGroups.add(new PSCustomActionGroup(node, parentDoc, parentComponents));
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(ENABLE_RELATED_CONTENT_ATTR, m_enableRelatedContent ? "yes" : "no");
    root.setAttribute(PRODUCES_RESOURCE_ATTR, m_producesResource ? "yes" : "no");
    root.setAttribute(OBJECT_TYPE_ATTR, Integer.toString(m_objectType));
    root.setAttribute(ICON_SOURCE_ATTR, m_iconSource);
    root.setAttribute(ICON_VALUE_ATTR, StringUtils.defaultString(m_iconValue));
    // store the base class info
    root.appendChild(super.toXml(doc));

    // create the stylesheet set
    if (m_stylesheetSet != null && m_stylesheetSet.getCommandHandlerNames().hasNext())
      root.appendChild(m_stylesheetSet.toXml(doc));

    // create application flow
    if (m_applicationFlow != null) root.appendChild(m_applicationFlow.toXml(doc));
    ...
    if (m_validationRules != null) root.appendChild(m_validationRules.toXml(doc));

    // create input translations
    if (m_inputTranslations != null) root.appendChild(m_inputTranslations.toXml(doc));

    // create output translations
    if (m_outputTranslations != null) root.appendChild(m_outputTranslations.toXml(doc));
    ...
    if (m_workflowInfo != null) root.appendChild(m_workflowInfo.toXml(doc));
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    IPSInternalRequestHandler rh = PSServer.getInternalRequestHandler("sys_psxCms/contentTypes");
    if (rh != null) {
    ...
    try {
      if (m_stylesheetSet != null) m_stylesheetSet.validate(context);

      if (m_applicationFlow != null) m_applicationFlow.validate(context);

      if (m_sectionLinkList != null) {
        Iterator it = this.getSectionLinkList();
        while (it.hasNext()) ((PSUrlRequest) it.next()).validate(context);
      }

      if (m_validationRules != null) m_validationRules.validate(context);

      if (m_inputTranslations != null) m_inputTranslations.validate(context);

      if (m_outputTranslations != null) m_outputTranslations.validate(context);

      if (getWorkflowInfo() != null) getWorkflowInfo().validate(context);
