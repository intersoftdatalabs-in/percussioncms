...
public class PSBackEndConnection extends PSComponent {
   ...
   */
  public PSBackEndConnection(String driverName, String className, String serverName) {
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof PSBackEndConnection)) return false;
    if (!super.equals(o)) return false;
    PSBackEndConnection that = (PSBackEndConnection) o;
  ...
  public int hashCode() {
    return Objects.hash(
        ...
        m_connMin,
        m_connMax,
        m_idleTimeoutSeconds,
        m_refreshPeriodSeconds);
  }
  ...
  private static IllegalArgumentException validateServer(String server) {
    if (server != null && server.length() > MAX_SERVER_NAME_LEN) {
      return new IllegalArgumentException(
          "back-end server is too big" + MAX_SERVER_NAME_LEN + " " + server.length());
    }
  ...
  public IllegalArgumentException validateConnectionMax(int max) {
    if (max == 0) {
      return new IllegalArgumentException(
          "back-end connection maxconn invalid" + m_jdbcDriverName + " " + max);
    }
  ...
  private IllegalArgumentException validateConnectionMin(int min) {
    if (min < 0) {
      return new IllegalArgumentException(
          "back-end connection minconn invalid" + m_jdbcDriverName + " " + min);
    }
  ...
  public void setRefreshPeriodSeconds(int seconds) {
    if (seconds != -1 && seconds < 1)
      throw new IllegalArgumentException("Illegal value for refresh period: " + seconds);
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, "jdbcDriverName", m_jdbcDriverName);

    // store the class name
    PSXmlDocumentBuilder.addElement(doc, root, "jdbcClassName", m_jdbcClassName);

    // store the server name
    PSXmlDocumentBuilder.addElement(doc, root, "serverName", m_server);

    // store min connection count
    PSXmlDocumentBuilder.addElement(doc, root, "connectionMin", String.valueOf(m_connMin));

    // store max connection count
    PSXmlDocumentBuilder.addElement(doc, root, "connectionMax", String.valueOf(m_connMax));

    // store idle connection timeout (in seconds)
    PSXmlDocumentBuilder.addElement(
        doc, root, "connectionIdleTimeout", String.valueOf(m_idleTimeoutSeconds));

    PSXmlDocumentBuilder.addElement(
        doc, root, "connectionRefreshPeriod", String.valueOf(m_refreshPeriodSeconds));
    ...
    try {
      ...
      try {
        ...
        throw new PSUnknownNodeTypeException(
            ms_NodeType, "serverName", new PSException(e.getLocalizedMessage()));
      }
  ...
  public void copyFrom(PSComponent c) {
    PSBackEndConnection conn = (PSBackEndConnection) c;
  ...
  public static final String ms_NodeType = "PSXBackEndConnection";
}
