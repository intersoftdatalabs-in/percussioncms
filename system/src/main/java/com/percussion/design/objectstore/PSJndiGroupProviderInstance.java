...
public class PSJndiGroupProviderInstance extends PSGroupProviderInstance {
  ...
  protected Element toXmlEx(Document doc) {
    ...
    // add objectClasses
    Element objectClasses = PSXmlDocumentBuilder.addEmptyElement(doc, root, OBJECT_CLASSES_ELEMENT);
    Iterator ocs = m_objectClasses.iterator();
    while (ocs.hasNext()) {
      PSJndiObjectClass oc = (PSJndiObjectClass) ocs.next();
      Element objectClass =
          PSXmlDocumentBuilder.addEmptyElement(doc, objectClasses, OBJECT_CLASS_ELEMENT);
      objectClass.setAttribute(NAME_ATTR, oc.getObjectClassName());
      objectClass.setAttribute(MEMBER_ATTR, oc.getMemberAttribute());
      objectClass.setAttribute(
          TYPE_ATTR, PSJndiObjectClass.MEMBER_ATTR_TYPE_ENUM[oc.getAttributeType()]);
    }

    // add groupNodes
    Element groupNodes = PSXmlDocumentBuilder.addEmptyElement(doc, root, GROUP_NODES_ELEMENT);
    Iterator nodes = m_groupNodes.iterator();
    while (nodes.hasNext()) {
      String node = (String) nodes.next();
      PSXmlDocumentBuilder.addElement(doc, groupNodes, GROUP_NODE_ELEMENT, node);
    }
  ...
  protected void fromXmlEx(Element source) throws PSUnknownNodeTypeException {
    ...
    while (objectClass != null) {
      ...
      if (attrType == null || attrType.trim().length() == 0) type = 0;
      else {
  ...
  public Iterator getObjectClasses() {
    ...
    Iterator ocs = m_objectClasses.iterator();
    while (ocs.hasNext()) classes.add(ocs.next());
  ...
  public String getMemberAttribute(String objectClass) {
    ...
    PSJndiObjectClass oc = (PSJndiObjectClass) m_objectClassMap.get(objectClass.toLowerCase());

    if (oc == null) throw new IllegalArgumentException("objectClass not supported");
  ...
  public void addObjectClass(String objectClass, String memberAttribute, int attributeType) {
    ...
    PSJndiObjectClass oc = new PSJndiObjectClass(objectClass, memberAttribute, attributeType);
  ...
  public void addGroupNode(String groupNode) {
    if (groupNode == null || groupNode.trim().length() == 0)
      throw new IllegalArgumentException("groupNode may not be null or empty");
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSJndiGroupProviderInstance)) isMatch = false;
    else {
      PSJndiGroupProviderInstance other = (PSJndiGroupProviderInstance) obj;
      if (!super.equals(other)) isMatch = false;
      else if (!this.m_objectClasses.equals(other.m_objectClasses)) isMatch = false;
      else if (!this.m_groupNodes.equals(other.m_groupNodes)) isMatch = false;
    }
  ...
  public Object clone() {
    ...
    PSJndiGroupProviderInstance copy = (PSJndiGroupProviderInstance) super.clone();
    ...
    while (ocs.hasNext()) {
      PSJndiObjectClass oc = (PSJndiObjectClass) ocs.next();
      PSJndiObjectClass ocClone =
          new PSJndiObjectClass(
              oc.getObjectClassName(), oc.getMemberAttribute(), oc.getAttributeType());
      copy.m_objectClasses.add(ocClone);
      copy.m_objectClassMap.put(ocClone.getObjectClassName().toLowerCase(), ocClone);
    }
