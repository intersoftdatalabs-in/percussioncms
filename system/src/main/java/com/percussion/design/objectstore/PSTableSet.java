...
public class PSTableSet extends PSComponent {
  ...
  public final void setTableRefs(PSCollection tableRefs) {
    ...
    if (!tableRefs.getMemberClassName().equals(m_tableRefs.getMemberClassName()))
      throw new IllegalArgumentException("PSTableRef collection expected");
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (data != null && data.trim().length() > 0) m_href = data;
  ...
  public Element toXml(Document doc) {
    ...
    if (m_href != null) root.setAttribute(HREF_ATTR, m_href);
