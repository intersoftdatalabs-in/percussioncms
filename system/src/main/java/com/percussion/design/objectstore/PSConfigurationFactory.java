...
public class PSConfigurationFactory {
  ...
  public static IPSConfig getConfiguration(String configName, Document configDoc)
    ...
    if (configName == null || configName.trim().length() == 0)
      throw new IllegalArgumentException("configName may not be null or empty.");

    if (configDoc == null) throw new IllegalArgumentException("configDoc may not be null.");

    if (configDoc.getDocumentElement() == null)
      throw new IllegalArgumentException("the document element may not be null.");
    ...
    if (configName.equals(RELATIONSHIPS_CFG)) {
      rxConfig = new PSRelationshipConfigSet(configDoc.getDocumentElement(), null, null);
    } else if (configName.equals(CLONE_HANDLERS_CFG)) {
      rxConfig = new PSCloneHandlerConfigSet(configDoc.getDocumentElement(), null, null);
    } else
      throw new IllegalArgumentException(
          "configName must be one of the <"
              + RELATIONSHIPS_CFG
              + ">, <"
              + CLONE_HANDLERS_CFG
              + ">");
