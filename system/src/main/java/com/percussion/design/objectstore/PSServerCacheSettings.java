...
public class PSServerCacheSettings extends PSComponent {
  ...
  public Element toXml(Document doc) {
    ...
    if (isEnabled()) root.setAttribute(ENABLED_ATTR, "yes");
    else root.setAttribute(ENABLED_ATTR, "no");

    if (isFolderCacheEnabled()) root.setAttribute(FOLDER_CACHE_ENABLED_ATTR, "yes");
    else root.setAttribute(FOLDER_CACHE_ENABLED_ATTR, "no");
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (data == null) m_isFolderCacheEnabled = true;
      else m_isFolderCacheEnabled = data.equalsIgnoreCase("yes");
  ...
  private void setMaxMemoryUsage(long memUsage) {
    if (memUsage < -1) {
      throw new IllegalArgumentException("Max Memory Usage can not be less than -1.");
    }
    if (memUsage == 0 && m_maxDiskUsage == 0) {
      throw new IllegalArgumentException("Both memory and disk usage can not be 0.");
    }
  ...
  private void setMaxDiskUsage(long diskUsage) {
    if (diskUsage < -1) {
      throw new IllegalArgumentException("Max Disk Usage can not be less than -1.");
    }
  ...
  private void setMaxPageSize(long pageSize) {
    if (pageSize == 0 || pageSize < -1) {
      throw new IllegalArgumentException("Max Page Size can not be less than -1 or equal to 0.");
    }
  ...
  private void setAgingTime(long agingTime) {
    if (agingTime == 0 || agingTime < -1) {
      throw new IllegalArgumentException("Aging time can not be equal to 0 or less than -1.");
    }
  ...
  public static final String MAX_MEMORY_ATTR = "maxMemory";
  public static final String MAX_DISK_ATTR = "maxDiskSpace";
  public static final String MAX_PAGE_ATTR = "maxPageSize";
  public static final String AGING_TIME_ATTR = "agingTime";
}
