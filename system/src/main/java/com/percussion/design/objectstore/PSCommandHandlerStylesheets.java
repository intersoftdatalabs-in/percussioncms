...
public class PSCommandHandlerStylesheets extends PSComponent {
  ...
  public final void setDefaultStylesheet(String name, PSStylesheet stylesheet) {
    ...
    try {
      ...
      rules.add(rule);
      PSConditionalStylesheet condStylesheet = new PSConditionalStylesheet(stylesheet, rules);
      ...
      if (c.isEmpty()) c.add(condStylesheet);
      else c.set(c.size() - 1, condStylesheet);
  ...
  public final void addConditionalStylesheets(String name, PSCollection stylesheets) {
    ...
    PSCollection c = new PSCollection((new PSConditionalStylesheet()).getClass());
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      while (node != null) {
        ...
        PSCollection stylesheets = new PSCollection((new PSConditionalStylesheet()).getClass());
        node = tree.getNextElement(PSConditionalStylesheet.XML_NODE_NAME, firstFlags);
        while (node != null) {
          stylesheets.add(new PSConditionalStylesheet(node, parentDoc, parentComponents));
          node = tree.getNextElement(PSConditionalStylesheet.XML_NODE_NAME, nextFlags);
        }

        // REQUIRED: the default stylesheet
        if (stylesheets.size() > 0)
          node = tree.getNextElement(PSStylesheet.XML_NODE_NAME, nextFlags);
        else node = tree.getNextElement(PSStylesheet.XML_NODE_NAME, firstFlags);
        if (node == null) {
        ...
        node =
            tree.getNextElement(
                COMMAND_HANDLER_ELEM, nextFlags | PSXmlTreeWalker.GET_NEXT_ALLOW_PARENTS);
      }
    } finally {
      ...
      while (names.hasNext()) {
        ...
        if (stylesheets != null && !stylesheets.isEmpty()) {
          ...
          for (int i = 0; i < stylesheets.size() - 1; i++)
            elem.appendChild(((IPSComponent) stylesheets.get(i)).toXml(doc));
          ...
          elem.appendChild((new PSStylesheet(defaultStylesheet.getRequest())).toXml(doc));
        }
  ...
  public void validate(PSCollection stylesheets) {
    ...
    if (!stylesheets
        .getMemberClassName()
        .equals("com.percussion.design.objectstore.PSConditionalStylesheet"))
      throw new IllegalArgumentException("PSConditionalStylesheet collection expected");
  }
