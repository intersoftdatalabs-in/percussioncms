...
public final class PSRoleProvider extends PSComponent {
  ...
  private void setType(String type) {
    ...
    if (!valid) throw new IllegalArgumentException("type must be one of TYPE_ENUM");
  ...
  public void setDirectoryRef(String directoryRef) {
    if (!isBackendRoleProvider()) {
      if (directoryRef == null) throw new IllegalArgumentException("directoryRef cannot be null");

      directoryRef = directoryRef.trim();
      if (directoryRef.length() == 0)
        throw new IllegalArgumentException("directoryRef cannot be null");

      setDirectoryRef(new PSReference(directoryRef, PSDirectorySet.class.getName()));
    }
  }
  ...
  public void setDirectoryRef(PSReference directoryRef) {
    if (!isBackendRoleProvider()) {
      ...
      if (!directoryRef.getType().equals(PSDirectorySet.class.getName()))
        throw new IllegalArgumentException("must be a PSDirectorySet reference");
  ...
  public Element toXml(Document doc) {
    ...
    if (m_directoryRef != null) root.appendChild(m_directoryRef.toXml(doc));
