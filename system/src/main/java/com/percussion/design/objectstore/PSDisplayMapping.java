...
public class PSDisplayMapping extends PSComponent {
  ...
  public Object clone() {
    ...
    if (m_displayMapper != null) copy.m_displayMapper = (PSDisplayMapper) m_displayMapper.clone();
    return copy;
  }
  ...
  public void setFieldRef(String fieldRef) {
    if (fieldRef == null || fieldRef.trim().length() == 0)
      throw new IllegalArgumentException("the fieldRef cannot be null or empty");
  ...
  public void setUISet(PSUISet uiSet) {
    if (uiSet == null) throw new IllegalArgumentException("the uiSet cannot be null");
  ...
  public boolean equals(Object o) {
    ...
    if (!compare(m_displayMapper, t.m_displayMapper)) equal = false;
    else if (!compare(m_fieldRef, t.m_fieldRef)) equal = false;
    else if (!compare(m_uiSet, t.m_uiSet)) equal = false;
  ...
  public Element toXml(Document doc) {
    ...
    if (m_displayMapper != null) root.appendChild(m_displayMapper.toXml(doc));
