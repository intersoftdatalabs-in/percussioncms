...
public class PSAttributeList extends PSDatabaseComponentCollection implements IPSComponent {
    ...
    if (attributes != null) {
      ...
      while (keys.hasNext()) {
        ...
        if (value instanceof List) {
          ...
          throw new IllegalArgumentException("map values must be of type string or list");
      }
    }
  }
  ...
  public void mergeAttributes(PSAttributeList attributes) {
    ...
    while (attrs.hasNext()) {
      ...
      if (match == null) add(attr);
      else match.merge(attr);
    }
  }
  ...
  public boolean equals(Object obj) {
    if (obj instanceof PSAttributeList) {
      ...
      if (size() != attributeList.size()) return false;

      Iterator attributes = iterator();
      while (attributes.hasNext()) {
        PSAttribute attribute1 = (PSAttribute) attributes.next();
        PSAttribute attribute2 = attributeList.getAttribute(attribute1.getName());
        if (!attribute1.equals(attribute2)) return false;
      }
