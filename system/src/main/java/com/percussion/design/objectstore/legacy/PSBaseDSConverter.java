...
public abstract class PSBaseDSConverter {
  ...
  public PSBaseDSConverter(
    ...
    if (configCtx == null) throw new IllegalArgumentException("configCtx may not be null");
  ...
  protected PSJdbcDriverConfig getDriver(String driver) {
    ...
    if (driverConfig == null) {
      getLogger().error("Failed to locate driver config for driver: " + driver);
    }
  ...
  protected String getDatasourceName(IPSJndiDatasource ds, String database, String origin) {
    ...
    List<IPSDatasourceConfig> configs =
        containerUtils.getDatasourceResolver().getDatasourceConfigurations();
    ...
    if (m_updateConfig) {
      ...
      PSDatasourceConfig newConfig =
          new PSDatasourceConfig(configName, ds.getName(), origin, database);
      ...
      containerUtils
          ...
          .addDatasourceConfig(configName, ds.getName(), origin, database);

      PSContainerUtilsFactory.getConfigurationContextInstance().save();

      getLogger().info("Created datatsource configuration with name: " + configName);
    }
  ...
  protected IPSJndiDatasource getJndiDatasource(PSJdbcDriverConfig driver, String server) {
    ...
    if (datasource == null && m_updateConfig) {
      ...
      if (cred != null) {
        ...
        String jndiName = "jdbc/" + StringUtils.deleteWhitespace(cred.getAlias());
        jndiName = ensureUniqueName(jndiName, dsNames);
        String uid = cred.getUserId();
        String pwd = PSCryptographer.decrypt(m_configCtx.getSecretKey(), uid, cred.getPassword());
        datasource =
            new PSJndiDatasourceImpl(
                jndiName, driver.getDriverName(), driver.getClassName(), server, uid, pwd);
    ...
    if (datasource == null) {
      getLogger()
          .info(
              "No JNDI datasource found for driver \""
                  + driver
                  + "\" and server \""
                  + server
                  + "\"");
  ...
  private String ensureUniqueName(String name, Set<String> curNames) {
    ...
    while (uniqueName == null) {
      ...
      if (!curNames.contains(test)) uniqueName = test;
      else {
