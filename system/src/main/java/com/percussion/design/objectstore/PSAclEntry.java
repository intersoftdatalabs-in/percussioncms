...
public class PSAclEntry extends PSComponent {
  ...
  private static IllegalArgumentException validateName(String name) {
    if ((null == name) || (name.trim().length() == 0)) {
      ...
      return new IllegalArgumentException(
          "acl entry name is too big" + " " + ACE_MAX_NAME_LEN + " " + name.length());
    return null;
  }
  ...
  public final void setAccessLevel(int level) {
    ...
    if (isServer && isApplication) {
      throw new IllegalArgumentException("acl security level invalid" + " " + level);
    }

    if (isServer) m_isServerAcl = true;
    else if (isApplication) m_isServerAcl = false;
  ...
  public Element toXml(Document doc) {
    ...
    Element node;
    if (m_isServerAcl) { // create serverAccessLevel element
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    if (tree.getNextElement("serverAccessLevel", firstFlags) != null) {
      ...
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= SACE_ACCESS_DATA;

      sTemp = tree.getElementData("designAccess", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= SACE_ACCESS_DESIGN;
      ...
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= SACE_ADMINISTER_SERVER;
    } else if (tree.getNextElement("applicationAccessLevel", firstFlags) != null) {
      m_isServerAcl = false;

      sTemp = tree.getElementData("dataQuery", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DATA_QUERY;

      sTemp = tree.getElementData("dataCreate", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DATA_CREATE;

      sTemp = tree.getElementData("dataUpdate", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DATA_UPDATE;

      sTemp = tree.getElementData("dataDelete", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DATA_DELETE;

      sTemp = tree.getElementData("designRead", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DESIGN_READ;

      sTemp = tree.getElementData("designUpdate", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DESIGN_UPDATE;

      sTemp = tree.getElementData("designDelete", false);
      if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_accessLevel |= AACE_DESIGN_DELETE;
    ...
    if (m_isServerAcl) {
      ...
      final int all_flags_compliment =
          ~0
              & (SACE_ACCESS_DATA
                  | SACE_ACCESS_DESIGN
                  | SACE_CREATE_APPLICATIONS
                  | SACE_DELETE_APPLICATIONS
                  | SACE_ADMINISTER_SERVER);
  ...
  private String getEntryTypeName() {
    String sRet = "";
    if (m_type == ACE_TYPE_USER) sRet = XML_FLAG_TYPE_USER;
    else if (m_type == ACE_TYPE_GROUP) sRet = XML_FLAG_TYPE_GROUP;
    else if (m_type == ACE_TYPE_ROLE) sRet = XML_FLAG_TYPE_ROLE;
