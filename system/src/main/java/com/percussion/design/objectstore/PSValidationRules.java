...
public class PSValidationRules extends PSCollectionComponent {
  ...
  public boolean equals(Object o) {
    ...
    else if (getMaxErrorsToStop() != t.getMaxErrorsToStop()) equal = false;
    else {
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      Element node = tree.getNextElement(PSConditionalExit.XML_NODE_NAME, firstFlags);
      while (node != null) {
        add(new PSConditionalExit(node, parentDoc, parentComponents));

        node = tree.getNextElement(PSConditionalExit.XML_NODE_NAME, nextFlags);
      }
    } finally {
