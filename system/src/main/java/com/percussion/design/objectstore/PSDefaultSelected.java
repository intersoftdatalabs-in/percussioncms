...
public class PSDefaultSelected extends PSComponent {
  ...
  public PSDefaultSelected(int sequence) {
    if (sequence < 0) throw new IllegalArgumentException("sequence cannot be lower than 0");
  ...
  public Element toXml(Document doc) {
    ...
    String value = null;
    if (m_type == TYPE_SEQUENCE) value = Integer.toString(m_sequence);
    else if (m_type == TYPE_TEXT) value = m_text;

    if (value != null) root.appendChild(doc.createTextNode(value));
