...
public class PSContentEditorDependencyMacroResolver {
      ...
      if (o instanceof IPSParameter) {
        ...
        if (macroName != null) {
          ...
          if (value instanceof IPSMutatableReplacementValue) {
            ...
            if (macroValue != null) ((IPSMutatableReplacementValue) value).setValueText(macroValue);
          } else {
            throw new IllegalStateException("macro supplied on immutable replacement value");
          }
        }
      } else throw new IllegalArgumentException("Only IPSParameters may be supplied as parameters");
    }
  }
  ...
  public static void replaceValueWithMacro(
    ...
    while (templateParameters.hasNext() && instanceParameters.hasNext()) {
      ...
      if (templateObj instanceof IPSParameter && instanceObj instanceof IPSParameter) {
        ...
        if (macroName != null) {
          ...
          if (instanceValue instanceof IPSMutatableReplacementValue && macroValue != null) {
            ...
            if (mutatableValue.getValueText().equals(macroValue))
              mutatableValue.setValueText(MACRO_PREFIX + macroName + MACRO_SUFFIX);
          }
        }
      } else throw new IllegalArgumentException("Only IPSParameters may be supplied as parameters");
    }
  ...
  private static String extractMacroName(IPSReplacementValue value) {
    ...
    if (valueText != null && valueText.startsWith(MACRO_PREFIX) && valueText.endsWith(MACRO_SUFFIX))
      macroName =
          valueText.substring(MACRO_PREFIX.length(), valueText.length() - MACRO_SUFFIX.length());
    return macroName;
  }
  ...
  private static String resolveMacro(PSDisplayMapping context, String macroName) {
    ...
    if (macroName.equals("fieldName") || macroName.equals("simpleChildFieldSet")) {
