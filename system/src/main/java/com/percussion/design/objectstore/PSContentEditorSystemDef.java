...
public final class PSContentEditorSystemDef implements IPSDocument {
  ...
  public PSContentEditorSystemDef(Document sourceDoc)
    ...
    if (sourceDoc == null) throw new IllegalArgumentException("sourceDoc may not be null");
  ...
  public Iterator getInputDataExits(String commandName) {
    ...
    PSExtensionCallSet exits = (PSExtensionCallSet) m_inputDataExits.get(commandName);

    if (exits != null) results = exits.iterator();
    else {
  ...
  public Iterator getResultDataExits(String commandName) {
    ...
    PSExtensionCallSet exits = (PSExtensionCallSet) m_resultDataExits.get(commandName);
  ...
  public Iterator getValidationRules() {
    ...
    if (m_groupValidations != null) results = m_groupValidations.iterator();
    else {
      // create an empty iterator to return
      results = PSIteratorUtils.emptyIterator();
    }
  ...
  public Iterator getInputTranslations() {
    ...
    if (m_inputTranslations != null) results = m_inputTranslations.iterator();
    else {
      // create an empty iterator to return
      results = PSIteratorUtils.emptyIterator();
    }
  ...
  public Iterator getOutputTranslations() {
    ...
    if (m_outputTranslations != null) results = m_outputTranslations.iterator();
    else {
      // create an empty iterator to return
      results = PSIteratorUtils.emptyIterator();
    }
  ...
  public Document toXml() {
    ...
    while (iter.hasNext()) {
      ...
      inputExits.appendChild(((PSExtensionCallSet) m_inputDataExits.get(cmdName)).toXml(doc));
      cmdExits.appendChild(inputExits);
      Element resultExits = doc.createElement(ELEMENT_RESULT_DATA_EXITS);
      resultExits.appendChild(((PSExtensionCallSet) m_resultDataExits.get(cmdName)).toXml(doc));
    ...
    if (m_paramNames.size() > 0) {
      ...
      while (iter.hasNext()) {
        ...
        PSParam param =
            new PSParam(entry.getKey().toString(), (IPSReplacementValue) entry.getValue());
    ...
    while (cmdIter.hasNext()) {
      ...
      PSXmlDocumentBuilder.addElement(doc, initParams, ELEMENT_COMMAND_NAME, cmdName);
    ...
    if (((null == getContainerLocator()) != (null == getFieldSet()))
      ...
      throw new IllegalStateException(
          "PSXContainerLocator, PSXFieldSet, "
  ...
  public final void fromXml(Document doc) throws PSUnknownNodeTypeException, PSUnknownDocTypeException {
    ...
    int firstFlags =
        PSXmlTreeWalker.GET_NEXT_RESET_CURRENT | PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN;
    ...
    // load StylesheetSet
    Element styleSheets = tree.getNextElement(PSCommandHandlerStylesheets.XML_NODE_NAME, nextFlags);
    ...
    Element appFlow = tree.getNextElement(PSApplicationFlow.XML_NODE_NAME, nextFlags);
    ...
    tree.setCurrent(sourceNode);
    Element sectionLinkList = tree.getNextElement(ELEMENT_SECTION_LINK_LIST, firstFlags);
    if (sectionLinkList != null) loadSectionLinkList(sectionLinkList);

    // load CommandHandlerExits if found
    tree.setCurrent(sourceNode);
    Element exits = tree.getNextElement(ELEMENT_COMMAND_HANDLER_EXITS, firstFlags);
    while (exits != null) {
      loadExits(exits);
    ...
    tree.setCurrent(sourceNode);
    Element paramNames = tree.getNextElement(ELEMENT_SYSTEM_PARAM_NAMES, firstFlags);

    if (paramNames != null) loadParamNames(paramNames);
    ...
    if (m_fieldSet != null) {
      ...
      Element validations = tree.getNextElement(PSValidationRules.XML_NODE_NAME, nextFlags);
      if (validations != null) {
      ...
      Element inputTranslations = tree.getNextElement(PSInputTranslations.XML_NODE_NAME, nextFlags);
      ...
      Element outputTranslations =
          tree.getNextElement(PSOutputTranslations.XML_NODE_NAME, nextFlags);
  ...
  public void populateSystemTableInfo(
    ...
    if (tableRef == null) throw new IllegalArgumentException("tableRef may not be null");

    if (cred == null) throw new IllegalArgumentException("cred may not be null");
  ...
  private void loadSectionLinkList(Element sectionLinkList) throws PSUnknownNodeTypeException {
    ...
    Element node = tree.getNextElement(PSUrlRequest.XML_NODE_NAME, firstFlags);
    while (node != null) {
      urlRequest = new PSUrlRequest(node, null, null);
  ...
  private void loadExits(Element exits) throws PSUnknownNodeTypeException {
    ...
    Element inputDataExits = tree.getNextElement(ELEMENT_INPUT_DATA_EXITS, firstFlags);
    ...
    PSExtensionCallSet inputCallSet = new PSExtensionCallSet(inputCallSetEl, null, null);

    // get the result data exits
    tree.setCurrent(exits);
    Element resultDataExits = tree.getNextElement(ELEMENT_RESULT_DATA_EXITS, firstFlags);
    ...
    PSExtensionCallSet resultCallSet = new PSExtensionCallSet(resultCallSetEl, null, null);
  ...
  private void loadInitParams(Element initParams) throws PSUnknownNodeTypeException {
    if (initParams == null) throw new IllegalArgumentException("initParams may not be null");
  ...
  private Element loadFieldSet(Element sibling) throws PSUnknownNodeTypeException {
     ...
     */
    Element containerLocator = tree.getNextElement(PSContainerLocator.XML_NODE_NAME, nextFlags);

    Element fieldSet = tree.getNextElement(PSFieldSet.XML_NODE_NAME, nextFlags);

    Element uiDef = tree.getNextElement(PSUIDefinition.XML_NODE_NAME, nextFlags);
    ...
    if (!((containerLocator == null) ^ (fieldSet == null))
      ...
      String badEl;
      if ((containerLocator == null)) badEl = PSContainerLocator.XML_NODE_NAME;
  ...
  private void loadSystemLocator(Element systemLocator) throws PSUnknownNodeTypeException {
    ...
    Element containerLocator = tree.getNextElement(PSContainerLocator.XML_NODE_NAME, firstFlags);
  ...
  public boolean isSystemField(PSField field, List fieldExcludes) {
    ...
    if (fieldExcludes == null) throw new IllegalArgumentException("fieldExcludes may not be null.");
  ...
  public boolean equals(Object o) {
    ...
    return m_cacheTimeout == that.m_cacheTimeout
        && Objects.equals(m_inputDataExits, that.m_inputDataExits)
        && Objects.equals(m_resultDataExits, that.m_resultDataExits)
        && Objects.equals(m_paramNames, that.m_paramNames)
        && Objects.equals(m_containerLocator, that.m_containerLocator)
        && Objects.equals(m_fieldSet, that.m_fieldSet)
        && Objects.equals(m_uiDef, that.m_uiDef)
        && Objects.equals(m_systemLocator, that.m_systemLocator)
        && Objects.equals(m_appFlow, that.m_appFlow)
        && Objects.equals(m_styleSheets, that.m_styleSheets)
        && Objects.equals(m_inputTranslations, that.m_inputTranslations)
        && Objects.equals(m_outputTranslations, that.m_outputTranslations)
        && Objects.equals(m_groupValidations, that.m_groupValidations)
