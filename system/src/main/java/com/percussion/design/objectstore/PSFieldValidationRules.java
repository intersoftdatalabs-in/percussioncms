...
public final class PSFieldValidationRules extends PSComponent {
  ...
  public void setMaxErrorsToStop(int maxErrorsToStop) {
    if (maxErrorsToStop <= 0)
      throw new IllegalArgumentException("mxaErrorsToStop must be greater than 0");
  ...
  public void setRules(PSCollection rules) {
    if (rules != null && !rules.getMemberClassName().equals(m_rules.getMemberClassName()))
      throw new IllegalArgumentException("PSRule collection expected");

    m_rules.clear();
    if (rules != null) m_rules.addAll(rules);
  }
  ...
  public void setRuleReferences(PSCollection ruleReferences) {
    ...
    if (ruleReferences != null) m_ruleReferences.addAll(ruleReferences);
  }
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      while (node != null) {
        m_rules.add(new PSRule(node, parentDoc, parentComponents));
      ...
      while (node != null) {
        ...
        if (ref != null && ref.trim().length() > 0) m_ruleReferences.add(ref);
      ...
      if (node != null) {
        m_applyWhen = new PSApplyWhen(node, parentDoc, parentComponents);
      }
      ...
      if (node != null) {
        ...
        m_errorMessage = new PSDisplayText(node, parentDoc, parentComponents);
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(MAX_ERRORS_TO_STOP_ATTR, Integer.toString(m_maxErrorsToStop));
    ...
    while (it.hasNext())
      PSXmlDocumentBuilder.addElement(doc, root, RULE_REFERENCE_ELEM, (String) it.next());

    // OPTIONAL: create the apply when
    if (m_applyWhen != null) root.appendChild(m_applyWhen.toXml(doc));
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      ...
      if (m_applyWhen != null) m_applyWhen.validate(context);
    } finally {
      context.popParent();
    }
  }
