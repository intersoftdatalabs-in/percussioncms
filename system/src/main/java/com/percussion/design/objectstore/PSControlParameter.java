...
public class PSControlParameter extends PSComponent {
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    setRequired(getEnumeratedAttribute(tree, XML_REQUIRED_ATTR, XML_REQUIRED_ENUM));
  ...
  public Element toXml(Document doc) {
    ...
    if (m_description.length() > 0)
      PSXmlDocumentBuilder.addElement(doc, root, XML_DESCRIPTION_NODE, m_description);
    // TODO: psxctl:ChoiceList
    if (m_defaultValue.length() > 0)
      PSXmlDocumentBuilder.addElement(doc, root, XML_DEFAULTVALUE_NODE, m_defaultValue);
    return root;
  }
  ...
  public void copyFrom(PSComponent object) {
    if (!(object instanceof PSControlParameter))
      throw new IllegalArgumentException("Must provide a non-null PSControlParameter");
  ...
  private void setDefaultValue(String defaultValue) {
    ...
    else m_defaultValue = defaultValue;
  }
  ...
  public class Entry {
    ...
    public void setInternalName(String internalName) {
      // force empty to null
      if (internalName != null && internalName.trim().length() == 0) internalName = null;
