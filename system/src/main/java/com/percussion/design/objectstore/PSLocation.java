...
public class PSLocation extends PSComponent {
  ...
  public boolean hasCustomActions(int pageType, int pageLocation, String fieldRef) {
    ...
    if (!fieldRefs.hasNext()) fieldMatch = true;

    while (fieldRefs.hasNext() && !fieldMatch) {
      String fieldName = (String) fieldRefs.next();
      if (fieldName.equalsIgnoreCase(fieldRef)) fieldMatch = true;
    }
  ...
  public void setPage(int page) {
    if (page != PAGE_SUMMARY_VIEW && page != PAGE_ROW_EDIT && page != PAGE_CHILD_ROW_EDIT)
  ...
  public void setType(int type) {
    if (type != TYPE_FORM
        ...
        && type != TYPE_FIELD
        && type != TYPE_WF_ACTION
        && type != TYPE_WF_TRANSITION) throw new IllegalArgumentException("Unknown type");
  ...
  public void setFieldRefs(Iterator refs) {
    if (null == refs) throw new IllegalArgumentException("refs can't be null");

    m_fieldRefs.clear();
    while (refs.hasNext()) m_fieldRefs.add(refs.next().toString());
  }
  ...
  public boolean equals(Object o) {
    ...
    boolean equal = true;
    if (m_page != t.m_page) equal = false;
    else if (m_type != t.m_type) equal = false;
    else if (m_sequence != t.m_sequence) equal = false;
    else if (!compare(m_fieldRefs, t.m_fieldRefs)) equal = false;
  ...
  public Element toXml(Document doc) {
    ...
    if (m_sequence > 0) root.setAttribute(SEQUENCE_ATTR, "" + m_sequence);
