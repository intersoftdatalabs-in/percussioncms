...
public class PSSecurityProviderInstance extends PSComponent {
  ...
  public int hashCode() {
    return Objects.hash(
        super.hashCode(), m_providerType, m_instanceName, m_instanceProps, m_directoryProvider);
  }
   ...
   */
  public void setProperties(Properties props) throws PSIllegalArgumentException {
  ...
  public static String prepareCredentials(String uid, String pw) {
    try {
      return PSEncryptor.encryptString(
          PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR), pw);
  ...
  public Element toXml(Document doc) {
    ...
    String type = PSSecurityProvider.getSecurityProviderTypeString(m_providerType);
    if (type != null) root.setAttribute("type", type);

    // store the instance name
    PSXmlDocumentBuilder.addElement(doc, root, "name", m_instanceName);

    // store the appropriate config object
    Element configNode = PSXmlDocumentBuilder.addEmptyElement(doc, root, "Properties");
    ...
    if ((configNode != null) && (m_instanceProps != null)) {
      ...
      while (keys.hasMoreElements()) {
        PSXmlDocumentBuilder.addElement(
            doc, configNode, (String) keys.nextElement(), (String) values.nextElement());
      }
    }

    // store directory provider
    if (m_directoryProvider != null) {
      Element directoryProvider =
          PSXmlDocumentBuilder.addEmptyElement(doc, root, DIRECTORY_PROVIDER_ELEMENT);
  ...
  private void fromXmlBase(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    Element directoryProvider = tree.getNextElement(DIRECTORY_PROVIDER_ELEMENT, firstFlags);
    if (directoryProvider != null) {
      Element provider = tree.getNextElement(PSProvider.XML_NODE_NAME, firstFlags);

      m_directoryProvider = new PSProvider(provider, parentDoc, parentComponents);
    }
  }
