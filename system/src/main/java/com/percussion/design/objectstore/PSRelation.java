...
public class PSRelation extends PSDatabaseComponent implements Cloneable {
  ...
  public Object clone() {
    PSRelation copy =
        new PSRelation(
            new PSCollection(m_keyNames.iterator()), new PSCollection(m_keyValues.iterator()));
  ...
  public boolean isMatch(PSRelation relationContext) {
    ...
    if (m_keyNames.size() == (relationContext.m_keyNames.size() + 1)) {
      for (int i = 0; i < relationContext.m_keyNames.size(); i++) {
        if (!m_keyNames.get(i).equals(relationContext.m_keyNames.get(i))) return false;
        if (!m_keyValues.get(i).equals(relationContext.m_keyValues.get(i))) return false;
      }
    } else {
      return false;
    }
  ...
  public Element toXml(Document doc) {
    ...
    for (int i = 0; i < m_keyNames.size(); i++) {
      PSXmlDocumentBuilder.addElement(
          doc, root, m_keyNames.get(i).toString(), m_keyValues.get(i).toString());
    }
  ...
  public String getDatabaseAppQueryDatasetName() {
    ...
    String resourceName = "";
    for (int i = 0; i < m_keyNames.size(); i++) resourceName += m_keyNames.get(i);
  ...
  public String getDatabaseAppQueryDatasetName(String componentName) {
    ...
    for (int i = 0; i < m_keyNames.size(); i++) datasetName.append(m_keyNames.get(i));
