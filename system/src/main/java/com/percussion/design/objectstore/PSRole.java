...
public class PSRole extends PSDatabaseComponent implements Comparable, IPSCatalogSummary {
  ...
  public boolean containsCorrespondingSubject(PSSubject sub) {
    if (sub == null) {
      throw new IllegalArgumentException("subject to check for must be specified.");
    } else {
      for (int i = 0; i < m_subjects.size(); i++)
        if (sub.isMatch((PSSubject) m_subjects.get(i))) return true;
    }
    return false;
  }
  ...
  public PSRole() {
    ...
    m_subjects =
        new PSDatabaseComponentCollection(subj.getClass(), subj.getDatabaseAppQueryDatasetName());
  }
  ...
  public void fromDatabaseXml(Element e, PSDatabaseComponentLoader cl, PSRelation relationContext)
    ...
    if (e == null || cl == null) throw new IllegalArgumentException("one or more params is null");
    ...
    if (null == relationContext) myCtx = new PSRelation();
    else myCtx = (PSRelation) relationContext.clone();
    myCtx.addKey(getComponentType(), m_databaseComponentId);

    PSRelativeSubject rs = new PSRelativeSubject();
    IPSDatabaseComponent[] subjects =
        cl.getRelatedDatabaseComponents(myCtx, PSRelativeSubject.class, rs.getComponentType());
    ...
    IPSDatabaseComponent[] attributes =
        cl.getRelatedDatabaseComponents(myCtx, PSAttribute.class, attr.getComponentType());
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    Element curNode = tree.getNextElement(PSDatabaseComponentCollection.ms_NodeType, firstFlags);
    if (curNode != null) {
      m_subjects.fromXml(curNode, parentDoc, parentComponents);
    }

    // Get role attributes here.
    Element attListElement = tree.getNextElement(PSAttributeList.ms_NodeType, nextFlags);
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSRole)) return false;
    ...
    return new EqualsBuilder()
        ...
        .append(m_subjects, role.m_subjects)
        .isEquals();
  }
