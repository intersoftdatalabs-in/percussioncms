...
public final class PSSearchConfig extends PSComponent {
  ...
  public Element toXml(Document doc) {
    ...
    search.setAttribute(TRACE_ENABLED_ATTR, BOOLEAN_VALUE[m_traceEnabled ? 0 : 1]);
    search.setAttribute(SEARCH_ENABLED_ATTR, BOOLEAN_VALUE[m_ftsEnabled ? 0 : 1]);
    search.setAttribute(ADMIN_MASTER_ATTR, BOOLEAN_VALUE[m_adminMaster ? 0 : 1]);
    search.setAttribute(MAX_SEARCH_RESULT_ATTR, Integer.toString(m_maxSearchResult));
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    if (searchEnabled != null && searchEnabled.trim().equalsIgnoreCase(BOOLEAN_VALUE[0])) {
      m_ftsEnabled = true;
    } else {
      m_ftsEnabled = false;
    }
    ...
    String nsMaxSearchResult = sourceNode.getAttribute(MAX_SEARCH_RESULT_ATTR);

    if (adminMaster != null && adminMaster.trim().equalsIgnoreCase(BOOLEAN_VALUE[0])) {
      m_adminMaster = true;
    } else {
      m_adminMaster = false;
    }
    if (traceEnabled != null && traceEnabled.trim().equalsIgnoreCase(BOOLEAN_VALUE[0])) {
      m_traceEnabled = true;
    } else {
    ...
    Element resultProcessingExitsEl = walker.getNextElement(RESULTPROCESSINGEXITS_ELEM);
    if (null != resultProcessingExitsEl) {
      Element exSetEl = walker.getNextElement(PSExtensionCallSet.ms_NodeType);
      if (null != exSetEl) m_resultProcessingExitSet.fromXml(exSetEl, null, null);
    }
    ...
    if (analyzersEl != null) {
      ...
      do {
        if (mtypeEl != null) {
          Element ext = aWalker.getNextElement(PSExtensionCall.ms_NodeType);
          m_analyzers.put(mtypeEl.getAttribute(NAME_ATTR), new PSExtensionCall(ext, null, null));
        }
    ...
    if (textconvertersEl != null) {
      ...
      do {
        if (localeEl != null) {
          Element ext = tWalker.getNextElement(PSExtensionCall.ms_NodeType);
          m_textConverters.put(
              localeEl.getAttribute(NAME_ATTR), new PSExtensionCall(ext, null, null));
        }
  ...
  public boolean equals(Object o) {
    ...
    if (m_ftsEnabled != test.m_ftsEnabled) return false;

    if (m_adminMaster != test.m_adminMaster) return false;
    ...
    if (!m_analyzers.equals(test.m_analyzers)) return false;

    if (!m_textConverters.equals(test.m_textConverters)) return false;
  ...
  public void setMaxSearchResult(int maxSearchResult) {
    if (m_maxSearchResult < 0) m_maxSearchResult = -1;
    else m_maxSearchResult = maxSearchResult;
  }
  ...
  public String getIndexDirectory() {
    ...
    if (irDir == null) return null;
    // normalize the path
    irDir = irDir.replace('\\', '/');
    File irDirFile = new File(irDir);
    if (!irDirFile.isAbsolute()) {
      if (irDir.startsWith("/")) irDir = irDir.substring(1);
      String rxroot = PSServer.getRxDir().getAbsolutePath().replace('\\', '/');
      if (!rxroot.endsWith("/")) rxroot += "/";
      irDir = rxroot + irDir;
    }
    // Remove the trailing slashes.
    if (!irDir.endsWith("/")) irDir += "/";
    return irDir;
  }
  ...
  public boolean isSynonymExpansionRequired() {
    ...
    if (prop != null && (prop.toLowerCase().equals("yes") || prop.toLowerCase().equals("true")))
      return true;
    return false;
  }
  ...
  private static final String RESULTPROCESSINGEXITS_ELEM = "ResultProcessingExits";
