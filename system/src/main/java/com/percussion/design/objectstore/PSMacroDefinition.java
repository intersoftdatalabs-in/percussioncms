...
public final class PSMacroDefinition extends PSComponent {
  ...
  public void setClassName(String className) {
    ...
    if (className.length() == 0) throw new IllegalArgumentException("className cannot be empty");
   ...
   */
  public final void fromXml(Element source, IPSDocument parent, List<IPSComponent> parentComponents)
      throws PSUnknownNodeTypeException {
  ...
  public Element toXml(Document doc) {
    ...
    if (getDescription().trim().length() > 0)
      PSXmlDocumentBuilder.addElement(doc, root, ELEM_DESCRIPTION, getDescription());
  ...
  public boolean equals(Object o) {
    if (!(o instanceof PSMacroDefinition)) return false;

    PSMacroDefinition t = (PSMacroDefinition) o;
    if (!getName().equals(t.getName())) return false;
    if (!getClassName().equals(t.getClassName())) return false;
    if (!getDescription().equals(t.getDescription())) return false;
