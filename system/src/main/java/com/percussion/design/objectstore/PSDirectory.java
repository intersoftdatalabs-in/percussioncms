...
public class PSDirectory extends PSComponent {
  ...
  public void setCatalogOption(String catalogOption) {
    ...
    if (!valid) throw new IllegalArgumentException("catalogOption must be one of CATALOG_ENUM");
  ...
  public void setFactory(String factory) {
    ...
    if (factory.length() == 0) throw new IllegalArgumentException("factory cannot be empty");
  ...
  public void setAuthenticationRef(String authenticationRef) {
    ...
    m_authenticationRef = new PSReference(authenticationRef, PSAuthentication.class.getName());
  }
  ...
  public String getBaseContext() {
    ...
    try {
      ...
      if (baseCtx.startsWith("/")) {
        if (baseCtx.length() > 1) baseCtx = baseCtx.substring(1);
        else baseCtx = "";
      }
  ...
  public void setAttributes(PSCollection attributes) {
    if (attributes == null) m_attributes = null;
      ...
      if (attributes.isEmpty()) throw new IllegalArgumentException("attributes cannot be empty");

      if (!attributes.getMemberClassName().equals(String.class.getName()))
        throw new IllegalArgumentException("attributes mus be a collection of String objects");
  ...
  public void setGroupProviderNames(Iterator<String> providers) {
    if (providers == null) throw new IllegalArgumentException("providers may not be null");

    m_groupProviderNames.clear();
    while (providers.hasNext()) {
      String provider = providers.next();
      if (provider.trim().length() == 0)
        throw new IllegalArgumentException("providers may not contain empty Strings");
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    setCatalogOption(getEnumeratedAttribute(tree, XML_ATTR_CATALOG, CATALOG_ENUM));
    ...
    m_groupProviderNames.clear();
    tree.setCurrent(current);
    Element groupProviders = tree.getNextElement(GROUP_PROVIDERS_ELEMENT, firstFlags);
    if (groupProviders != null) {
      Element groupName = tree.getNextElement(GROUP_NAME_ELEMENT, firstFlags);
  ...
  public Element toXml(Document doc) {
    ...
    if (isDebug()) root.setAttribute(XML_ATTR_DEBUG, DEBUG_YES);
    else root.setAttribute(XML_ATTR_DEBUG, DEBUG_NO);

    root.setAttribute(XML_ATTR_TIMEOUT, Long.toString(getTimeout()));

    PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_FACTORY, getFactory());

    Element authentication =
        PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_ELEM_AUTHENTICATION);
    authentication.appendChild(m_authenticationRef.toXml(doc));

    PSXmlDocumentBuilder.addElement(doc, root, XML_ELEM_PROVIDER_URL, getProviderUrl());
    ...
    if (m_attributes != null) {
      ...
      while (attrs.hasNext()) {
        Element attr = PSXmlDocumentBuilder.addEmptyElement(doc, attributes, XML_ELEM_ATTRIBUTE);
    ...
    if (!m_groupProviderNames.isEmpty()) {
      Element groupProviders =
          PSXmlDocumentBuilder.addEmptyElement(doc, root, GROUP_PROVIDERS_ELEMENT);
      Iterator<String> gps = m_groupProviderNames.iterator();
      while (gps.hasNext()) {
        PSXmlDocumentBuilder.addElement(doc, groupProviders, GROUP_NAME_ELEMENT, gps.next());
      }
    }
  ...
  private long timeout =
      59000; // default to 59 seconds - most load balancers timeout at 60 by default
