...
public class PSBackEndJoin extends PSComponent {
  ...
  public Object clone() {
    ...
    if (m_leftColumn != null) copy.m_leftColumn = (PSBackEndColumn) m_leftColumn.clone();
    if (m_rightColumn != null) copy.m_rightColumn = (PSBackEndColumn) m_rightColumn.clone();
    if (m_translator != null) copy.m_translator = (PSExtensionCall) m_translator.clone();
    return copy;
  }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_joinType == BEJ_TYPE_FULL_OUTER) root.setAttribute("joinType", XML_FLAG_FULL_OUTER);
    else if (m_joinType == BEJ_TYPE_LEFT_OUTER) root.setAttribute("joinType", XML_FLAG_LEFT_OUTER);
    else if (m_joinType == BEJ_TYPE_RIGHT_OUTER)
      root.setAttribute("joinType", XML_FLAG_RIGHT_OUTER);
    else root.setAttribute("joinType", XML_FLAG_INNER);
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (sTemp != null) {
        if (sTemp.equals(XML_FLAG_INNER)) m_joinType = BEJ_TYPE_INNER;
        else if (sTemp.equals(XML_FLAG_FULL_OUTER)) m_joinType = BEJ_TYPE_FULL_OUTER;
        else if (sTemp.equals(XML_FLAG_LEFT_OUTER)) m_joinType = BEJ_TYPE_LEFT_OUTER;
        else if (sTemp.equals(XML_FLAG_RIGHT_OUTER)) m_joinType = BEJ_TYPE_RIGHT_OUTER;
      }
      ...
      if (tree.getNextElement("translator", firstFlags) != null) {
        for (Node child = tree.getCurrent().getFirstChild();
          ...
          if (child instanceof Element) {
            m_translator = new PSExtensionCall((Element) child, parentDoc, parentComponents);
            break;
          }
        }
      }
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    try {
      ...
      if (m_translator != null) m_translator.validate(cxt);
    } finally {
      cxt.popParent();
    }
  }
