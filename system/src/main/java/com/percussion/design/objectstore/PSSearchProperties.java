...
public class PSSearchProperties extends PSComponent {
  ...
  public void setEnableTransformation(boolean enableTransformation) {
    ...
    if (enableTransformation) setVisibleToGlobalQuery(true);
  }
  ...
  public void setDefaultSearchLabel(String label) {
    if (null == label || label.trim().length() == 0) m_defaultSearchLabel = "";
    else m_defaultSearchLabel = label.trim();
  }
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(USER_SEARCHABLE_ATTR, BOOLEAN_ENUM[isUserSearchable() ? 0 : 1]);
    root.setAttribute(GLOBAL_QUERY_VISIBLE_ATTR, BOOLEAN_ENUM[isVisibleToGlobalQuery() ? 0 : 1]);
    root.setAttribute(SEARCH_TOKEN_ATTR, BOOLEAN_ENUM[isTokenizeSearchContent() ? 0 : 1]);
    root.setAttribute(ENABLE_TRANSFORMATION_ATTR, BOOLEAN_ENUM[isEnableTransformation() ? 0 : 1]);
    root.setAttribute(USER_CUSTOMIZABLE_ATTR, BOOLEAN_ENUM[isUserCustomizable() ? 0 : 1]);
    String label = getDefaultSearchLabel();
    if (label.length() > 0) root.setAttribute(DEF_SEARCH_LABEL_ATTR, label);
    return root;
  }
  ...
  public final void fromXml(Element source, IPSDocument parentDoc, List parentComponents)
    ...
    try {
      ...
      if (data != null) setUserSearchable(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
      else
      ...
      if (data != null) setVisibleToGlobalQuery(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
      else
        // default value in case loading into a non-clean field
      ...
      if (data != null) setTokenizeSearchContent(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
      else
        // default value in case loading into a non-clean field
      ...
      if (data != null) setEnableTransformation(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
      else
        // default value in case loading into a non-clean field
      ...
      if (data != null) setUserCustomizable(data.equalsIgnoreCase(BOOLEAN_ENUM[0]));
      else
        // default value in case loading into a non-clean field
  ...
  public int hashCode() {
    return Objects.hash(
        super.hashCode(),
        m_userSearchable,
        m_visibleToGlobalQuery,
        m_tokenizeSearchContent,
        m_enableTransformation,
        m_userCustomizable,
  ...
  private static final String ENABLE_TRANSFORMATION_ATTR = "enableTransformation";
  private static final String GLOBAL_QUERY_VISIBLE_ATTR = "visibleToGlobalQuery";
