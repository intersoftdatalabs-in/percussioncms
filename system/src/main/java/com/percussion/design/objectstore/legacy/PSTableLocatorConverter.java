...
public class PSTableLocatorConverter extends PSBaseComponentConverter {
  ...
  public Object convertComponent(Element source) throws PSUnknownNodeTypeException {
    ...
    if (isForcedConversion()) {
      ...
      if (ex != null) {
        ...
        if (oldTableLoc.getCredentials() == null) newTableLoc = createTableLoc(oldTableLoc);
        else {
  ...
  private PSTableLocator createTableLoc(PSLegacyTableLocator oldTableLoc) {
    PSTableLocator newTableLoc = new PSTableLocator(oldTableLoc.getAliasRef());
    copyFrom(newTableLoc, oldTableLoc);
  ...
  private String resolveToDatasource(PSLegacyTableLocator oldTableLoc)
    ...
    getLogger()
        .info(
            "Attempting to locate matching datasource for table "
                + "locator with credentials alias \""
                + oldCreds.getAlias()
                + "\" "
                + getContextLogMessage());
    ...
    return resolveToDatasource(
        oldCreds.getDriver(),
        oldCreds.getServer(),
        oldTableLoc.getDatabase(),
        oldTableLoc.getOrigin());
  }
   ...
   */
  private void copyFrom(PSTableLocator convertedObj, PSLegacyTableLocator oldTableLoc) {
    convertedObj.setAlias(oldTableLoc.getAlias());
    convertedObj.setId(oldTableLoc.getId());
    if (oldTableLoc.getCredentials() != null) {
      convertedObj.getCredentials().setAlias(oldTableLoc.getCredentials().getAlias());
      convertedObj.getCredentials().setComment(oldTableLoc.getCredentials().getComment());
      convertedObj.getCredentials().setConditionals(oldTableLoc.getCredentials().getConditionals());
      convertedObj.getCredentials().setId(oldTableLoc.getCredentials().getId());
    }
  }
}
