...
public class PSConditionalExit extends PSComponent {
  ...
  public Object clone() {
    ...
    if (null != m_condition) ce.m_condition = (PSApplyWhen) m_condition.clone();
    if (null != m_rules) ce.m_rules = (PSExtensionCallSet) m_rules.clone();
    return ce;
  }
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (data != null) m_maxErrorsToStop = Integer.parseInt(data);
      ...
      if (node != null) m_condition = new PSApplyWhen(node, parentDoc, parentComponents);
    } finally {
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(MAX_ERRORS_TO_STOP_ATTR, Integer.toString(getMaxErrorsToStop()));

    // REQUIRED: the extension call set
    root.appendChild(m_rules.toXml(doc));

    // create the apply when condition
    if (m_condition != null) root.appendChild(m_condition.toXml(doc));
