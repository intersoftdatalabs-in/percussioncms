...
public class PSDependency extends PSComponent {
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    if (status.equals("readyToGo")) m_status = READY_STATUS;
    else if (status.equals("setupRequired")) m_status = SETUP_REQUIRED_STATUS;
    ...
    if (occurrence.equals("single")) m_occurrence = SINGLE_OCCURRENCE;
    ...
    // default
    Element defaultChildElement = tree.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    if (defaultChildElement != null) {
       ...
       */
      IPSReplacementValue dependent =
          PSReplacementValueFactory.getReplacementValueFromXml(
              parentDoc, parentComponents, child, XML_NODE_NAME, XELEM_DEFAULT);
  ...
  public void setDependent(IPSDependentObject dependent) {
    if (null == dependent) throw new IllegalArgumentException("dependent may not be null");
  ...
  public boolean hasSameDependent(PSDependency testObj) {
    if (null == testObj) throw new IllegalArgumentException("testObj may not be null");
    // should the number of parameters be compared?
    return (getType().equals(testObj.getType())) && (getName().equals(testObj.getName()));
  }
