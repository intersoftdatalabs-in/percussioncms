...
public class PSDatabaseComponentCollection extends PSDatabaseComponent implements Serializable {
  ...
  PSDatabaseComponentCollection(Class c, String datasetName) {
    if (c == null || datasetName == null || datasetName.length() < 1)
      throw new IllegalArgumentException("Class and resource name must be specified.");
  ...
  void addCollectionComponents(Element collectionRoot, Document doc) {
    ...
    if (doc == null) throw new IllegalArgumentException("Document must be specified.");
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      for (Element e = tree.getNextElement(firstFlags);
          e != null;
      ...
      throw new UnknownError("Can't inst collection component!  Reason: " + instE.toString());
    } catch (IllegalAccessException accE) {
      // Should never happen here, fail fast.
      throw new UnknownError("Can't access constructor!  Reason: " + accE.toString());
    }
  }
  ...
  public void fromDatabaseXml(
      Element source, PSDatabaseComponentLoader cl, PSRelation relationContext)
    ...
    try {
      ...
      for (Element e = tree.getNextElement(firstFlags);
        ...
        if (null != relationContext) myCtx = (PSRelation) relationContext.clone();
        IPSDatabaseComponent component = (IPSDatabaseComponent) c.newInstance();
  ...
  void addFromDb(PSDatabaseComponent c) {
    if (c == null) throw new IllegalArgumentException("Component must be supplied.");
  ...
  public void toDatabaseXml(Document doc, Element actionRoot, PSRelation relationContext)
    ...
    while (i.hasNext()) {
      ...
      component.toDatabaseXml(doc, actionRoot, (PSRelation) relationContext.clone());
    }
    i = m_deletes.iterator();
    while (i.hasNext()) {
      IPSDatabaseComponent component = (IPSDatabaseComponent) i.next();
  ...
  public void add(PSDatabaseComponent component) {
    ...
    if (!component.isInsert())
      throw new IllegalArgumentException("Added components must be newly created!");
  ...
  public void removeElementAt(int index) {
    PSDatabaseComponent component = (PSDatabaseComponent) m_components.remove(index);
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof PSDatabaseComponentCollection)) return false;
    ...
    return new EqualsBuilder()
        ...
        .append(m_resourceName, t.m_resourceName)
        .isEquals();
  }
