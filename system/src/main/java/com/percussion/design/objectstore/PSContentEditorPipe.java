...
public final class PSContentEditorPipe extends PSPipe {
   ...
   */
  public PSContentEditorPipe(
      String name, PSContainerLocator locator, PSContentEditorMapper mapper) {
    try {
      setName(name);
  ...
  public PSExtensionCallSet getInputDataExtensions() {
    ...
    if (super.getInputDataExtensions() != null) set.addAll(super.getInputDataExtensions());
    else if (set.isEmpty()) return null;
  ...
  public boolean equals(Object o) {
    ...
    boolean equal = true;
    if (!compare(m_locator, t.m_locator)) equal = false;
    else if (!compare(m_mapper, t.m_mapper)) equal = false;
    else if (!compare(m_controlDepMap, t.m_controlDepMap)) equal = false;
    else equal = super.equals(o);
  ...
  public PSBackEndDataTank getBackEndDataTank() {
    try {
      ...
      backEndDataTank.setTables(
          new PSCollection(getLocator().getBackEndTables().values().iterator()));
    ...
    try {
      ...
      try {
        ...
        throw new PSUnknownNodeTypeException(
            XML_NODE_NAME, PSPipe.NAME_ELEM, new PSException(e.getLocalizedMessage()));
      }
      ...
      try {
        ...
        throw new PSUnknownNodeTypeException(
            XML_NODE_NAME, PSPipe.DESCRIPTION_ELEM, new PSException(e.getLocalizedMessage()));
      }
      ...
      if (node != null) {
        ...
        if (tree.getNextElement(PSExtensionCallSet.ms_NodeType, firstFlags) != null) {
          ...
          m_inputDataExtensions =
              new PSExtensionCallSet((Element) tree.getCurrent(), parentDoc, parentComponents);
        }
      }
      ...
      node = tree.getNextElement(PSContentEditorMapper.XML_NODE_NAME, nextFlags);
      if (node != null) {
      ...
      if (node != null) {
        ...
        m_controlDepMap = new PSControlDependencyMap(userProps, m_inputDataExtensions);
      }
    } finally {
  ...
  public Element toXml(Document doc) {
    ...
    // defined in base class PSPipe
    PSXmlDocumentBuilder.addElement(doc, root, PSPipe.DESCRIPTION_ELEM, m_description);
    ...
    Map<String, String> userProps = m_controlDepMap.generateUserProperties(inputDataExts);
    // defined in base class PSPipe
    ...
    if (!inputDataExts.isEmpty()) {
      Element parent =
          PSXmlDocumentBuilder.addEmptyElement(doc, root, PSPipe.INPUT_DATA_EXITS_ELEM);
    ...
    if (m_resultDataExtensions != null) {
      Element parent =
          PSXmlDocumentBuilder.addEmptyElement(doc, root, PSPipe.RESULT_DATA_EXITS_ELEM);
    ...
    if (!userProps.isEmpty()) {
      Element userPropsEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, USER_PROPERTIES);
      for (Map.Entry<String, String> entry : userProps.entrySet()) {
        Element propEl =
            PSXmlDocumentBuilder.addElement(doc, userPropsEl, USER_PROPERTY, entry.getValue());
  ...
  static Map<String, String> loadUserProps(Element node) {
    ...
    Element propEl = tree.getNextElement(USER_PROPERTY, tree.GET_NEXT_ALLOW_CHILDREN);
    while (propEl != null) {
      String name = propEl.getAttribute(USER_PROP_NAME_ATTR);
      if (StringUtils.isBlank(name)) continue;
      String value = tree.getElementData();
      props.put(name, value);
      propEl = tree.getNextElement(USER_PROPERTY, tree.GET_NEXT_ALLOW_SIBLINGS);
    }
