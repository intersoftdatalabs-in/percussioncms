...
import com.percussion.system.utils.PSDocVersionConverter;
...
public final class PSApplication implements IPSDocument {
  ...
  private class RequestKey {
    ...
    public RequestKey(PSDataSet set) {
      if (set == null) {
        throw new IllegalArgumentException("Passed dataset must not be null");
      }
    ...
    public boolean equals(Object compareTo) {
       ...
       */
      if (sourceSet.isEmpty() || compareSet.isEmpty()) return true;
      else return sourceSet.equals(compareSet);
    }
  ...
  private static IllegalArgumentException validateName(String name) {
    ...
    if (length < 1) return new IllegalArgumentException("app name is empty");
    else if (length > APP_MAX_NAME_LEN) {
      return new IllegalArgumentException(
          "app name is too big: " + APP_MAX_NAME_LEN + " " + length);
    }
  ...
  private static IllegalArgumentException validateRequestRoot(String requestRoot) {
    ...
    if (length > APP_MAX_REQ_ROOT_LEN) {
      return new IllegalArgumentException(
          "app root too big: " + length + " " + APP_MAX_REQ_ROOT_LEN);
    }
    return null;
  }
  ...
  private static IllegalArgumentException validateAcl(PSAcl acl) {
    if (null == acl) return new IllegalArgumentException("app acl is null");

    PSCollection entries = acl.getEntries();
    int size = entries.size();
    if (0 == size) return new IllegalArgumentException("app acl is empty");

    int fullDesignAccess =
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (names.put(key, ace) != null) /* we've seen this already */
        return new IllegalArgumentException("acl entry list is duplicate: " + ace.getName());

      level = ace.getAccessLevel();
      if ((level & fullDesignAccess) == fullDesignAccess) hasFullDesign = true;
    }

    if (!hasFullDesign) return new IllegalArgumentException("app acl no manager");
  ...
  private static IllegalArgumentException validateRequestTypeHtmlParamName(String param) {
    if ((param != null) && (param.length() > MAX_HTML_REQ_PARAM_NAME_LEN)) {
      return new IllegalArgumentException(
          "app req param html too big:" + MAX_HTML_REQ_PARAM_NAME_LEN + " " + param.length());
    }
  ...
  private static IllegalArgumentException validateRequestTypeValueQuery(String value) {
    if ((value != null) && (value.length() > MAX_REQ_TYPE_VALUE_LEN)) {
      return new IllegalArgumentException(
          "app req type value too big:" + MAX_REQ_TYPE_VALUE_LEN + " " + value.length());
    }
  ...
  private static IllegalArgumentException validateDataSets(PSCollection dataSets) {
    if (dataSets != null) {
      if (!com.percussion.design.objectstore.PSDataSet.class.isAssignableFrom(
          dataSets.getMemberClassType())) {
        return new IllegalArgumentException(
            "coll bad content type: Data Set" + dataSets.getMemberClassName());
      }
    }
  ...
  private static IllegalArgumentException validateRoles(PSCollection roles) {
    if (roles != null) {
      if (!com.percussion.design.objectstore.PSRole.class.isAssignableFrom(
        ...
        return new IllegalArgumentException(
            "coll bad content type: Role" + roles.getMemberClassName());
      }
    }
  ...
  private static IllegalArgumentException validateApplicationExtensions(PSCollection extensions) {
    if (extensions != null) {
      if (!PSExtensionRef.class.isAssignableFrom(extensions.getMemberClassType())) {
        return new IllegalArgumentException(
            "coll bad content type: Application Extensions" + extensions.getMemberClassName());
      }
    }
  ...
  public boolean equals(Object o) {
    ...
    if (m_beLoginPassthru != other.m_beLoginPassthru) return false;

    if (m_enabled != other.m_enabled) return false;

    if (m_hidden != other.m_hidden) return false;

    if (m_startPriority != other.m_startPriority) return false;

    if (m_active != other.m_active) return false;

    if (m_maxThreads != other.m_maxThreads) return false;

    if (m_maxRequestTime != other.m_maxRequestTime) return false;

    if (m_maxRequestsInQueue != other.m_maxRequestsInQueue) return false;

    if (m_userSessions != other.m_userSessions) return false;

    if (m_sessionTimeout != other.m_sessionTimeout) return false;

    if (!compare(m_name, other.m_name)) return false;

    if (!compare(m_description, other.m_description)) return false;

    if (!compare(m_requestRoot, other.m_requestRoot)) return false;

    if (!compare(m_defaultRequestPage, other.m_defaultRequestPage)) return false;

    if (!compare(m_context, other.m_context)) return false;

    if (!compare(m_acl, other.m_acl)) return false;

    if (!compare(m_dataEncryptor, other.m_dataEncryptor)) return false;

    if (!compare(m_dataSets, other.m_dataSets)) return false;

    if (!compare(m_logger, other.m_logger)) return false;

    if (!compare(m_loginWebPage, other.m_loginWebPage)) return false;

    if (!compare(m_errorWebPages, other.m_errorWebPages)) return false;

    if (!compare(m_roles, other.m_roles)) return false;

    if (!compare(m_notifier, other.m_notifier)) return false;

    if (!compare(m_userProperties, other.m_userProperties)) return false;

    // can't use standard equals on XML documents
    if (m_propertyTree == null || other.m_propertyTree == null) {
      if (m_propertyTree != null || other.m_propertyTree != null) return false;
    }
    ...
    // do a deep comparison if both are not null
      ...
      if ((m_propertyTree instanceof ParentNode) && (other.m_propertyTree instanceof ParentNode)) {
        ParentNode pn = (ParentNode) m_propertyTree;
        ParentNode pnOther = (ParentNode) other.m_propertyTree;
        if (!(pn.isEqualNode(pnOther))) return false;
      } else {
        return false;
      }
    }

    if (!compare(m_htmlParamName, other.m_htmlParamName)) return false;

    if (!compare(m_requestTypeValueQuery, other.m_requestTypeValueQuery)) return false;

    if (!compare(m_requestTypeValueInsert, other.m_requestTypeValueInsert)) return false;

    if (!compare(m_requestTypeValueUpdate, other.m_requestTypeValueUpdate)) return false;

    if (!compare(m_requestTypeValueDelete, other.m_requestTypeValueDelete)) return false;

    if (!compare(m_version, other.m_version)) return false;

    if (!compare(m_history, other.m_history)) return false;
  ...
  public Document toXml() {
    ...
    String appType = "";
    if (m_appType != null) appType = m_appType.toString();
    ...
    PSXmlDocumentBuilder.addElement(doc, root, "defaultRequestPage", m_defaultRequestPage);

    if (null != m_context)
      PSXmlDocumentBuilder.addElement(doc, root, "appExtensionContext", m_context);

    // create revision history element
    if (m_history != null) root.appendChild(m_history.toXml(doc));

    // create PSAcl elements
    if (m_acl != null) root.appendChild(m_acl.toXml(doc));

    // create PSDataEncryptor elements
    if (m_dataEncryptor != null) root.appendChild(m_dataEncryptor.toXml(doc));

    // create maxThreads element
    PSXmlDocumentBuilder.addElement(doc, root, "maxThreads", String.valueOf(m_maxThreads));

    // create maxRequestTime element
    PSXmlDocumentBuilder.addElement(doc, root, "maxRequestTime", String.valueOf(m_maxRequestTime));

    // create maxRequestInQueue element
    PSXmlDocumentBuilder.addElement(
        doc, root, "maxRequestsInQueue", String.valueOf(m_maxRequestsInQueue));

    // create userSessionEnabled element
    PSXmlDocumentBuilder.addElement(
        doc, root, "userSessionEnabled", (m_userSessions ? "yes" : "no"));

    // create user sessions time out
    PSXmlDocumentBuilder.addElement(
        doc, root, "userSessionTimeout", String.valueOf(m_sessionTimeout));

    // create requestTypeHtmlParamName element
    PSXmlDocumentBuilder.addElement(doc, root, "requestTypeHtmlParamName", m_htmlParamName);

    // create requestTypeValueQuery element
    PSXmlDocumentBuilder.addElement(doc, root, "requestTypeValueQuery", m_requestTypeValueQuery);

    // create requestTypeValueInsert element
    PSXmlDocumentBuilder.addElement(doc, root, "requestTypeValueInsert", m_requestTypeValueInsert);

    // create requestTypeValueUpdate element
    PSXmlDocumentBuilder.addElement(doc, root, "requestTypeValueUpdate", m_requestTypeValueUpdate);

    // create requestTypeValueDelete element
    PSXmlDocumentBuilder.addElement(doc, root, "requestTypeValueDelete", m_requestTypeValueDelete);

    // create the data sets now
    PSCollectionComponent.appendCollectionToXml(doc, root, m_dataSets);

    // create PSLogger elements
    if (m_logger != null) root.appendChild(m_logger.toXml(doc));
    ...
    if (m_errorWebPages != null) root.appendChild(m_errorWebPages.toXml(doc));

    // create backEndLoginPassthru element
    PSXmlDocumentBuilder.addElement(
        doc, root, "backEndLoginPassthru", (m_beLoginPassthru ? "yes" : "no"));

    // create m_roles elements
    PSCollectionComponent.appendCollectionToXml(doc, root, m_roles);

    // create PSNotifier elements
    if (m_notifier != null) root.appendChild(m_notifier.toXml(doc));
    ...
    Element propRoot = (m_propertyTree == null) ? null : m_propertyTree.getDocumentElement();
    if (propRoot != null) {
      // in one op, we can copy the tree as a child of our tree root
      PSXmlDocumentBuilder.copyTree(
          doc, PSXmlDocumentBuilder.addEmptyElement(doc, root, "PropertyTree"), propRoot);
    }
  ...
  private void fromXmlBase(Document sourceDoc)
    ...
    if (sTemp != null) m_active = sTemp.equalsIgnoreCase("yes");

    sTemp = tree.getElementData("enabled");
    if (sTemp != null) m_enabled = sTemp.equalsIgnoreCase("yes");

    sTemp = tree.getElementData("hidden");
    if (sTemp != null) m_hidden = sTemp.equalsIgnoreCase("yes");
    ...
    for (IPSComponentConverter converter : PSComponent.getComponentConverters()) {
    ...
    if (tree.getNextElement(PSRevisionHistory.ms_nodeType, walkerFlags) != null) {
      if (m_history == null) m_history = new PSRevisionHistory();

      m_history.fromXml((Element) tree.getCurrent(), this, parentComponents);
    }

    // construct new   PSAcl from xml
    if (tree.getNextElement(PSAcl.ms_NodeType, walkerFlags) != null) {
      m_acl = new PSAcl((Element) tree.getCurrent(), this, parentComponents);
    }

    // create PSDataEncryptor object from XML node
    if (tree.getNextElement(PSDataEncryptor.ms_NodeType, walkerFlags) != null) {
      m_dataEncryptor = new PSDataEncryptor((Element) tree.getCurrent(), this, parentComponents);
    }

    // create maxThreads element
    sTemp = tree.getElementData("maxThreads");
    if (sTemp == null) m_maxThreads = 0;
    else {
      try {
    ...
    if (sTemp == null) m_maxRequestTime = 0;
    else {
      try {
    ...
    if (sTemp == null) m_maxRequestsInQueue = 0;
    else {
      try {
    ...
    if (sTemp != null) m_userSessions = sTemp.equalsIgnoreCase("yes");

    // create user sessions time out
    sTemp = tree.getElementData("userSessionTimeout");
    if (sTemp == null) m_sessionTimeout = 0;
    else {
      try {
    ...
    // create requestTypeHtmlParamName element
    try {
      setRequestTypeHtmlParamName(tree.getElementData("requestTypeHtmlParamName"));
    } catch (IllegalArgumentException e) {
      throw new PSUnknownDocTypeException(
          ms_NodeType, "requestTypeHtmlParamName", new PSException(e.getLocalizedMessage()));
    }

    // create requestTypeValueQuery element
    try {
      setRequestTypeValueQuery(tree.getElementData("requestTypeValueQuery"));
    } catch (IllegalArgumentException e) {
      throw new PSUnknownDocTypeException(
          ms_NodeType, "requestTypeValueQuery", new PSException(e.getLocalizedMessage()));
    }

    // create requestTypeValueInsert element
    try {
      setRequestTypeValueInsert(tree.getElementData("requestTypeValueInsert"));
    } catch (IllegalArgumentException e) {
      throw new PSUnknownDocTypeException(
          ms_NodeType, "requestTypeValueInsert", new PSException(e.getLocalizedMessage()));
    }

    // create requestTypeValueUpdate element
    try {
      setRequestTypeValueUpdate(tree.getElementData("requestTypeValueUpdate"));
    } catch (IllegalArgumentException e) {
      throw new PSUnknownDocTypeException(
          ms_NodeType, "requestTypeValueUpdate", new PSException(e.getLocalizedMessage()));
    }

    // create requestTypeValueDelete element
    try {
      setRequestTypeValueDelete(tree.getElementData("requestTypeValueDelete"));
    } catch (IllegalArgumentException e) {
      throw new PSUnknownDocTypeException(
          ms_NodeType, "requestTypeValueDelete", new PSException(e.getLocalizedMessage()));
    }

    // create m_dataSets from XML nodes
    if (null != m_dataSets) m_dataSets.clear();
    ...
    if (tree.getNextElement(PSDataSet.ms_NodeType, walkerFlags) != null) {
      PSDataSet dataSet;
      if (null == m_dataSets) {
        m_dataSets = new PSCollection(com.percussion.design.objectstore.PSDataSet.class);
      }
    ...
    if (tree.getNextElement(PSContentEditor.XML_NODE_NAME, walkerFlags) != null) {
      PSContentEditor dataSet;
      if (null == m_dataSets) m_dataSets = new PSCollection(PSDataSet.class);

      do {
        dataSet = new PSContentEditor((Element) tree.getCurrent(), this, parentComponents);
        m_dataSets.add(dataSet);
      } while (tree.getNextElement(PSContentEditor.XML_NODE_NAME, walkerFlags) != null);
    }

    // get PSLogger object from XML
    m_logger = null;
    if (tree.getNextElement(PSLogger.ms_NodeType, walkerFlags) != null) {
      m_logger = new PSLogger((Element) tree.getCurrent(), this, parentComponents);
    } else m_logger = new PSLogger();

    // get the PSTraceInfo object from XML
    m_traceInfo = null;
    if (tree.getNextElement(PSTraceInfo.ms_NodeType, walkerFlags) != null) {
      m_traceInfo = new PSTraceInfo((Element) tree.getCurrent(), this, parentComponents);
    } else m_traceInfo = new PSTraceInfo();

    // handle old PSLoginWebPage definition
    m_loginWebPage = null;
    if (tree.getNextElement(PSLoginWebPage.ms_NodeType, walkerFlags) != null) {
      PSLoginWebPage loginWebPage =
          new PSLoginWebPage((Element) tree.getCurrent(), this, parentComponents);
    ...
    if (tree.getNextElement(PSErrorWebPages.ms_NodeType, walkerFlags) != null) {
      m_errorWebPages = new PSErrorWebPages((Element) tree.getCurrent(), this, null);
    }
    ...
    if (sTemp != null && sTemp.equalsIgnoreCase("yes")) m_beLoginPassthru = true;
    ...
    if (tree.getNextElement("PSXRole", walkerFlags) != null) {
      String roleNames = "";
      do {
        if (roleNames.length() > 0) roleNames += ",";
    ...
    if (tree.getNextElement(PSNotifier.ms_NodeType, walkerFlags) != null) {
      if (null == m_notifier) m_notifier = new PSNotifier();

      m_notifier.fromXml((Element) tree.getCurrent(), this, parentComponents);
    }
    ...
    if (tree.getNextElement("userProperty", walkerFlags) != null) {
      ...
      do {
        m_userProperties.put(
            tree.getElementData("name", false), tree.getElementData((Element) tree.getCurrent()));
    ...
    if (propRoot != null) {
      ...
      if (propRoot != null) {
        ...
        PSXmlDocumentBuilder.copyTree(m_propertyTree, m_propertyTree, propRoot);
      }
    }
    // Guess the app type if not defined
    if (m_appType == null) m_appType = guessAppType();
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    if (ex != null) cxt.validationError(null, 0, ex.getLocalizedMessage());
    ...
    if (m_dataEncryptor != null) m_dataEncryptor.validate(cxt);
    ...
    if (m_loginWebPage != null) m_loginWebPage.validate(cxt);
    ...
    if (m_notifier != null) m_notifier.validate(cxt);

    if (m_history != null) m_history.validate(cxt);
  }
  ...
  public PSTraceInfo getTraceInfo() {
    // see if we have one
    if (m_traceInfo == null) m_traceInfo = new PSTraceInfo();
