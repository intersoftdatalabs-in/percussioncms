...
public class PSTraceInfo extends PSComponent {
  ...
  public void setTraceOptionsFlag(PSTraceFlag optionsFlag, boolean updateEnabled) {
    ...
    if (updateEnabled) setTraceEnabled(isEnabled);
  }
  ...
  public String getOptionDescription(int traceFlag) {
    ...
    if (!found) throw new IllegalArgumentException("invalid traceflag: " + traceFlag);
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute("traceTimestampOnlyEnabled", (m_timeStampOnly ? "yes" : "no"));

    // now set the trace option attributes
    Iterator i = ms_options.iterator();
    while (i.hasNext()) {
      PSTraceOption o = (PSTraceOption) i.next();
      root.setAttribute(o.getName(), (isTraceEnabled(o.getFlag()) ? "yes" : "no"));
    }
  ...
  public void addTraceStateListener(IPSTraceStateListener traceStateListener) {
    ...
    if (m_enabled) traceStateListener.traceStarted(this);
  }
  ...
  public int hashCode() {
    return Objects.hash(
        super.hashCode(),
        m_enabled,
        m_traceStateListeners.hashCode(),
        m_timeStampOnly,
        m_columnWidth,
        m_traceFlag);
  }
  ...
  public void setTraceEnabled(boolean isEnabled) {
    ...
    if (isEnabled && !m_enabled) notifyStart();
    else if (!isEnabled && m_enabled) notifyStop();
    else if (isEnabled && m_enabled) notifyRestart();
  ...
  public void setTraceEnabled(int traceFlag, boolean isEnabled) {
    ...
    if (isEnabled) m_traceFlag.setBit(traceFlag);
    else m_traceFlag.clearBit(traceFlag);
  }
  ...
  private boolean isValidOption(int flag) {
    ...
    while (list.hasNext()) {
      if (((PSTraceOption) list.next()).getFlag() == flag) return true;
    }
    return false;
  }
