...
public class PSDatabaseComponentLoader {
  ...
  public PSDatabaseComponentLoader(PSRequest req) {
    if (req == null) throw new IllegalArgumentException("Request must be specified.");
  ...
  public void actualizeCollectionComponent(
      PSDatabaseComponentCollection component, PSRelation relationContext)
      throws PSUnknownNodeTypeException, PSDatabaseComponentException {
    if (component == null) throw new IllegalArgumentException("Component must be specified");
    ...
    if (e != null) component.fromDatabaseXml(e, this, relationContext);
  ...
  public IPSDatabaseComponent[] getRelatedDatabaseComponents(
    ...
    if (relationContext == null)
      throw new IllegalArgumentException("Relation context must be supplied.");

    if (componentType == null || componentType.length() < 1)
      throw new IllegalArgumentException("Component type must be specified.");

    String resourceName = relationContext.getDatabaseAppQueryDatasetName(componentType);

    List relationList = (ArrayList) m_relationListMap.get(resourceName);

    if (relationList == null) {
      // load the relation list
      Element relationListElement = lookupDatabaseXmlElement(resourceName, null, false);

      if (relationListElement == null) return new IPSDatabaseComponent[0];

      PSXmlTreeWalker walker = new PSXmlTreeWalker(relationListElement);

      relationList = new ArrayList();

      Element relationElement = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      while (relationElement != null) {
        ...
        relationElement = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
      }
    ...
    while (i.hasNext()) {
      ...
      if (relation.isMatch(relationContext)) {
        Element relatedComponentElement =
            lookupDatabaseXmlElement(
        ...
        IPSDatabaseComponent component =
            PSDatabaseComponent.newComponentInstance(
                c, relatedComponentElement, (PSRelation) relationContext.clone(), this);
    ...
    try {
      if (cacheMap || key != null) {
        ...
        if (elementMap == null) {
          ...
          if (cacheMap) m_elementsMap.put(resourceName, elementMap);
        }
        return (Element) elementMap.get(key);
      } else { // No cache and no key, just return the root of the doc.
        return getComponentDoc(resourceName).getDocumentElement();
      }
    } catch (PSException e) {
      if (e instanceof PSUnknownNodeTypeException) throw (PSUnknownNodeTypeException) e;
  ...
  private Map loadElementMap(String resourceName)
      throws PSInternalRequestCallException, PSAuthorizationException,
          PSAuthenticationFailedException, PSUnknownNodeTypeException {
    ...
    Element e = walker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
  ...
  private Document getComponentDoc(String resourceName)
    ...
    IPSInternalRequestHandler irh = PSServer.getInternalRequestHandler(request);
