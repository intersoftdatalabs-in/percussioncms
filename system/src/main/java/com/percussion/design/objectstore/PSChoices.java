...
public final class PSChoices extends PSComponent {
  ...
  public Object clone() {
    ...
    for (int i = 0; i < m_defaultSelected.size(); i++) {
      PSDefaultSelected defaultSelected = (PSDefaultSelected) m_defaultSelected.elementAt(i);
    ...
    if (m_lookup != null) copy.m_lookup = (PSUrlRequest) m_lookup.clone();
    if (m_nullEntry != null) copy.m_nullEntry = (PSNullEntry) m_nullEntry.clone();

    if (m_choiceFilter != null) copy.m_choiceFilter = (PSChoiceFilter) m_choiceFilter.clone();
  ...
  public final void setLocal(PSCollection local) {
    ...
    if (!local.getMemberClassName().equals(m_local.getMemberClassName()))
      throw new IllegalArgumentException("PSEntry collection expected");
  ...
  public final void setDefaultSelected(PSCollection defaultSelected) {
    ...
    if (defaultSelected != null) m_defaultSelected.addAll(defaultSelected);
  }
  ...
  public final void fromXml(Element sourceNode, IPSDocument parentDoc, List<IPSComponent> parentComponents)
    ...
    try {
      ...
      if (m_type == TYPE_GLOBAL) {
        ...
        if (node != null) key = PSXmlTreeWalker.getElementData(node);
      ...
      if (node != null) m_nullEntry = new PSNullEntry(node, parentDoc, parentComponents);

      // OPTIONAL: get the defaultSelected
      node = tree.getNextElement(DEFAULT_SELECTED_ELEM, nextFlags);
      if (node != null) {
        node = tree.getNextElement(PSDefaultSelected.XML_NODE_NAME, firstFlags);
        while (node != null) {
          m_defaultSelected.add(new PSDefaultSelected(node, parentDoc, parentComponents));
          node = tree.getNextElement(PSDefaultSelected.XML_NODE_NAME, nextFlags);
        }
      }
      ...
      if (node != null) {
        m_choiceFilter = new PSChoiceFilter(node, parentDoc, parentComponents);
      }
    } finally {
  ...
  public Element toXml(Document doc) {
    ...
    if (m_type == TYPE_GLOBAL) {
      PSXmlDocumentBuilder.addElement(doc, root, CHOICE_LIST_KEY_ELEM, Integer.toString(m_global));
    } else if (m_type == TYPE_LOCAL) {
    ...
    if (m_nullEntry != null) root.appendChild(m_nullEntry.toXml(doc));
    ...
    if (m_choiceFilter != null) root.appendChild(m_choiceFilter.toXml(doc));
  ...
  public void validate(IPSValidationContext context) throws PSSystemValidationException {
    ...
    try {
      ...
      if (m_nullEntry != null) m_nullEntry.validate(context);

      Iterator it = getDefaultSelected();
      while (it.hasNext()) ((PSEntry) it.next()).validate(context);

      // OPTIONAL choice filter
      if (m_choiceFilter != null) m_choiceFilter.validate(context);
    } finally {
      context.popParent();
    }
  }
