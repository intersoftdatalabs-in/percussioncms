...
public class PSRecipient extends PSComponent {
  ...
  public void setSendEnabled(boolean enable) {
    ...
    else m_options &= ~MN_SEND_ENABLED;
  }
  ...
  public void setErrorThresholdByCount(boolean enable) {
    if (enable) m_options |= MN_ERROR_THRESHOLD_BY_COUNT;
    else m_options &= ~MN_ERROR_THRESHOLD_BY_COUNT;
  }
  ...
  public void setErrorThresholdByInterval(boolean enable) {
    if (enable) m_options |= MN_ERROR_THRESHOLD_BY_INTERVAL;
    else m_options &= ~MN_ERROR_THRESHOLD_BY_INTERVAL;
  }
  ...
  public void setAppAuthorizationFailureEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_AUTH_FAILURE;
  }
  ...
  public void setAppDesignErrorEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_DESIGN_ERROR;
  }
  ...
  public void setAppValidationErrorEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_VALIDATION_ERROR;
  }
  ...
  public void setAppXmlErrorEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_XML_ERROR;
  }
  ...
  public void setAppHtmlErrorEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_HTML_ERROR;
  }
  ...
  public void setAppResponseTimeEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_RESPONSE_TIME;
  }
  ...
  public void setAppRequestQueueLargeEnabled(boolean enable) {
    ...
    else m_options &= ~MN_APP_REQUEST_QUEUE;
  }
  ...
  public void setBackEndAuthorizationFailureEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_AUTH_FAILURE;
  }
  ...
  public void setBackEndServerDownFailureEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_SERVER_DOWN_FAILURE;
  }
  ...
  public void setBackEndDataConversionErrorEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_DATA_CONVERSION_FAILURE;
  }
  ...
  public void setBackEndQueryFailureEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_QUERY_FAILURE;
  }
  ...
  public void setBackEndUpdateFailureEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_UPDATE_FAILURE;
  }
  ...
  public void setBackEndRequestQueueLargeEnabled(boolean enable) {
    ...
    else m_options &= ~MN_BACKEND_REQUEST_QUEUE;
  }
  ...
  public Element toXml(Document doc) {
    ...
    if ((m_options & MN_SEND_ENABLED) == MN_SEND_ENABLED) root.setAttribute("enabled", "yes");
    else root.setAttribute("enabled", "no");
    ...
    else parent.setAttribute("type", "none");

    // private          int         m_errorThresholdCount = 0;
    PSXmlDocumentBuilder.addElement(doc, parent, "count", String.valueOf(m_errorThresholdCount));

    // private          int         m_errorThresholdInterval = 0;
    PSXmlDocumentBuilder.addElement(
        doc, parent, "interval", String.valueOf(m_errorThresholdInterval));
    ...
    Element event =
        PSXmlDocumentBuilder.addElement(
            doc, parent, "authorizationFailure", String.valueOf(m_appAuthFailureCount));
    if ((m_options & MN_APP_AUTH_FAILURE) == MN_APP_AUTH_FAILURE)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "designError");
    if ((m_options & MN_APP_DESIGN_ERROR) == MN_APP_DESIGN_ERROR)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "validationError");
    if ((m_options & MN_APP_VALIDATION_ERROR) == MN_APP_VALIDATION_ERROR)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "xmlError");
    if ((m_options & MN_APP_XML_ERROR) == MN_APP_XML_ERROR) event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "htmlError");
    if ((m_options & MN_APP_HTML_ERROR) == MN_APP_HTML_ERROR) event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event =
        PSXmlDocumentBuilder.addElement(
            doc, parent, "poorResponseTime", String.valueOf(m_appResponseTime));
    ...
    event =
        PSXmlDocumentBuilder.addElement(
            doc, parent, "largeRequestQueue", String.valueOf(m_appRequestQueue));
    ...
    event =
        PSXmlDocumentBuilder.addElement(
            doc, parent, "authorizationFailure", String.valueOf(m_backendAuthFailureCount));
    if ((m_options & MN_BACKEND_AUTH_FAILURE) == MN_BACKEND_AUTH_FAILURE)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "serverDown");
    if ((m_options & MN_BACKEND_SERVER_DOWN_FAILURE) == MN_BACKEND_SERVER_DOWN_FAILURE)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "dataConversionError");
    if ((m_options & MN_BACKEND_DATA_CONVERSION_FAILURE) == MN_BACKEND_DATA_CONVERSION_FAILURE)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "queryError");
    if ((m_options & MN_BACKEND_QUERY_FAILURE) == MN_BACKEND_QUERY_FAILURE)
      event.setAttribute("enabled", "yes");
    else event.setAttribute("enabled", "no");

    event = PSXmlDocumentBuilder.addEmptyElement(doc, parent, "updateError");
    ...
    event =
        PSXmlDocumentBuilder.addElement(
            doc, parent, "largeRequestQueue", String.valueOf(m_backendRequestQueue));
  ...
  public void fromXml(Element sourceNode, IPSDocument parentDoc, List parentComponents)
    ...
    if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_SEND_ENABLED;
    ...
    if (tree.getNextElement("Threshold", firstFlags) != null) {
      ...
      if (sTemp != null) {
        if (sTemp.equalsIgnoreCase("exceedsCount")) m_options |= MN_ERROR_THRESHOLD_BY_COUNT;
    ...
    if (tree.getNextElement("ApplicationEvents", firstFlags) != null) {
      ...
      if (tree.getNextElement("authorizationFailure", firstFlags) != null) {
        ...
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_AUTH_FAILURE;
      ...
      if (tree.getNextElement("designError", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_DESIGN_ERROR;
      }

      tree.setCurrent(saveCur);
      if (tree.getNextElement("validationError", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_VALIDATION_ERROR;
      }

      tree.setCurrent(saveCur);
      if (tree.getNextElement("xmlError", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_XML_ERROR;
      }

      tree.setCurrent(saveCur);
      if (tree.getNextElement("htmlError", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_HTML_ERROR;
      }

      tree.setCurrent(saveCur);
      if (tree.getNextElement("poorResponseTime", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_RESPONSE_TIME;
      ...
      if (tree.getNextElement("largeRequestQueue", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_APP_REQUEST_QUEUE;
    ...
    if (tree.getNextElement("BackEndEvents", firstFlags) != null) {
      ...
      if (tree.getNextElement("authorizationFailure", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_BACKEND_AUTH_FAILURE;
      ...
      if (tree.getNextElement("queryError", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_BACKEND_QUERY_FAILURE;
      }
      ...
      if (tree.getNextElement("largeRequestQueue", firstFlags) != null) {
        sTemp = tree.getElementData("enabled", false);
        if ((sTemp != null) && sTemp.equalsIgnoreCase("yes")) m_options |= MN_BACKEND_REQUEST_QUEUE;
  ...
  public void validate(IPSValidationContext cxt) throws PSSystemValidationException {
    ...
    final int all_flags_compliment =
        ~(MN_SEND_ENABLED
            | MN_ERROR_THRESHOLD_BY_COUNT
            | MN_ERROR_THRESHOLD_BY_INTERVAL
            | MN_APP_AUTH_FAILURE
            | MN_APP_DESIGN_ERROR
            | MN_APP_VALIDATION_ERROR
            | MN_APP_XML_ERROR
            | MN_APP_HTML_ERROR
            | MN_APP_RESPONSE_TIME
            | MN_APP_REQUEST_QUEUE
            | MN_BACKEND_AUTH_FAILURE
            | MN_BACKEND_SERVER_DOWN_FAILURE
            | MN_BACKEND_DATA_CONVERSION_FAILURE
            | MN_BACKEND_QUERY_FAILURE
            | MN_BACKEND_UPDATE_FAILURE
            | MN_BACKEND_REQUEST_QUEUE);

    if (0 != (m_options & all_flags_compliment)) {
      cxt.validationError(this, 0, "Invalid flags in recipient options: " + m_options);
    }
    ...
    if (m_errorThresholdCount < 0)
      cxt.validationError(this, 0, "Invalid error threshold count: " + m_errorThresholdCount);

    if (m_errorThresholdInterval < 0)
      cxt.validationError(this, 0, "Invalid error Threshold Interval: " + m_errorThresholdInterval);

    if (m_appAuthFailureCount < 0)
      cxt.validationError(this, 0, "Invalid app Auth Failure Count: " + m_appAuthFailureCount);

    if (m_appResponseTime < 0)
      cxt.validationError(this, 0, "Invalid app Response Time: " + m_appResponseTime);

    if (m_appRequestQueue < 0)
      cxt.validationError(this, 0, "Invalid app Request Queue: " + m_appRequestQueue);

    if (m_backendAuthFailureCount < 0)
      cxt.validationError(
          this, 0, "Invalid back end Auth Failure Count: " + m_backendAuthFailureCount);

    if (m_backendRequestQueue < 0)
      cxt.validationError(this, 0, "Invalid back end Request Queue: " + m_backendRequestQueue);
  }
  ...
  public int hashCode() {
    return Objects.hash(
        ...
        m_options,
        m_name,
        m_errorThresholdCount,
        m_errorThresholdInterval,
        m_appAuthFailureCount,
        m_appResponseTime,
        m_appRequestQueue,
        m_backendAuthFailureCount,
        m_backendRequestQueue);
  }
