...
public class PSObjectTypesCatalogHandler extends PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    if (catalogerType != null)
      PSXmlDocumentBuilder.addElement(retDoc, root, "catalogerName", catalogerType);

    try {
      // this will throw if non-existant cataloger
      IPSRoleMgr roleMgr = PSRoleMgrLocator.getRoleManager();
      boolean supportsGroups = roleMgr.supportsGroups(catalogerName, catalogerType);

      // all catalogers support users
      Element node;
      node = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "ObjectType");
