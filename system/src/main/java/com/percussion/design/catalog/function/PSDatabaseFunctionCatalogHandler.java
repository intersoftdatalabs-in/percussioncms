...
public class PSDatabaseFunctionCatalogHandler implements IPSCatalogHandler {
  ...
  public Document formatRequest(Properties req) {
    ...
    if ((sTemp == null) || (!(REQ_CATEGORY_VALUE.equalsIgnoreCase(sTemp)))) {
      throw new IllegalArgumentException("req category invalid: null or REQ_CATEGORY_VALUE");
    }

    sTemp = (String) req.get(REQ_TYPE_KEY);
    if ((sTemp == null) || (!(REQ_TYPE_VALUE.equalsIgnoreCase(sTemp)))) {
      throw new IllegalArgumentException("req type invalid: null or REQ_TYPE_VALUE");
    }
    ...
    if (datasource != null)
      PSXmlDocumentBuilder.addElement(reqDoc, root, EL_DATASOURCE, datasource);
  ...
  public static PSDatabaseFunctionDef[] getDatabaseFunctions(
      ...
      throws PSServerException, PSAuthenticationFailedException, PSAuthorizationException,
          IOException {
    ...
    if (root != null) {
      ...
      while (el != null) {
        try {
          list.add(
              new PSDatabaseFunctionDef(PSDatabaseFunctionManager.FUNCTION_TYPE_SYSTEM, el, true));
