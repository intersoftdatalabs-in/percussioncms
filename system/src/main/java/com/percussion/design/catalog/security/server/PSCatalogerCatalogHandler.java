...
public class PSCatalogerCatalogHandler extends PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    for (IPSDirectoryCataloger dirCat : roleMgr.getDirectoryCatalogers()) {
      if (roleMgr.isDefaultCataloger(dirCat)) continue;
      addCatalogerDefinition(
          doc,
          root,
          dirCat.getName(),
          dirCat.getCatalogerType(),
          dirCat.getName() + "/" + dirCat.getCatalogerDisplayType(),
          "");
    }
    ...
    for (IPSSubjectCataloger subCat : roleMgr.getSubjectCatalogers()) {
      addCatalogerDefinition(
          ...
          root,
          subCat.getName(),
          IPSRoleMgr.SUBJECT_CATALOGER_TYPE,
          subCat.getName() + "/" + "Subject Cataloger",
  ...
  private void addCatalogerDefinition(
    ...
    Element node = PSXmlDocumentBuilder.addEmptyElement(doc, root, "cataloger");
