...
public class PSTableCatalogHandler extends com.percussion.design.catalog.PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    if (filter != null) PSXmlDocumentBuilder.addElement(retDoc, root, "filter", filter);

    if (tableTypes != null) {
      for (int i = 0; i < tableTypes.length; i++)
        PSXmlDocumentBuilder.addElement(retDoc, root, "tableType", tableTypes[i]);
    }
    ...
    String searchTable = "%";
    if ((filter != null) && (filter.length() != 0)) searchTable = filter;
    ...
    try {
      ...
      ResultSet rs =
          meta.getTables(detail.getDatabase(), detail.getOrigin(), searchTable, tableTypes);
      if (rs != null) {
        while (rs.next()) {
          ...
          tableNode = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Table");
          tableNode.setAttribute("type", retType);

          PSXmlDocumentBuilder.addElement(retDoc, tableNode, "name", retName);
        }
