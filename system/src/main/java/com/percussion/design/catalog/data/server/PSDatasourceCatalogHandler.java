...
/**
 * This class is the server-side implementation to handle requests generated by the {@link
 * com.percussion.design.catalog.data.PSDatasourceCatalogHandler}. See that class for more info.
 */
public class PSDatasourceCatalogHandler extends PSCatalogRequestHandler
   ...
   * See {@link com.percussion.design.catalog.data.PSDatasourceCatalogHandler} for the format.
   *
  ...
  public void processRequest(PSRequest request) {
    ...
    boolean inclDbPubSources = tAttrib != null && tAttrib.equalsIgnoreCase("true");
    ...
    try {
      ...
      for (String dsName : dsList) {
        ...
        try {
          ...
          logger.warn("Error occurred loading connection info for: " + dsName, e);
          continue;
        }
        ...
        boolean isDbPubOnly = PSJdbcUtils.getDbPubOnlyDrivers().contains(detail.getDriver());
        if (isDbPubOnly && !inclDbPubSources) continue;

        Element dsEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, "datasource");

        PSXmlDocumentBuilder.addElement(doc, dsEl, "name", dsName);
        PSXmlDocumentBuilder.addElement(doc, dsEl, "jndiDatasource", detail.getDatasourceName());
        ...
        PSXmlDocumentBuilder.addElement(doc, dsEl, "origin", detail.getOrigin());
        PSXmlDocumentBuilder.addElement(
            doc, dsEl, "isRepository", dsName.equals(defaultDs) ? "yes" : "no");
        PSXmlDocumentBuilder.addElement(doc, dsEl, "driver", detail.getDriver());
        PSXmlDocumentBuilder.addElement(doc, dsEl, "isDbPubOnly", isDbPubOnly ? "yes" : "no");
      }
  ...
  /** Constant for the root element name of the catalog request handled by this cataloger. */
