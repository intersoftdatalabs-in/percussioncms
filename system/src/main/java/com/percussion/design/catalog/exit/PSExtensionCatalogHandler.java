 ...
 *
 * <p>Extension handler catalog requests are sent to the server using the PSXExtensionCatalog XML
 ...
 * </pre>
 *
 * The PSXExtensionCatalogResults XML document is sent as the response. Its definition is as
 * follows:
 *
 * <pre>
...
public class PSExtensionCatalogHandler implements IPSCatalogHandler {
  ...
  public org.w3c.dom.Document formatRequest(Properties req) {
    ...
    if ((sTemp == null) || (sTemp.length() == 0)) {
      throw new IllegalArgumentException(
          "reqd prop not specified: ExtensionHandlerPattern or null");
    }
  ...
  public static IPSExtensionDef[] getCatalog(
    ...
    if (null != context) req.put("Context", context);
    if (null != interfacePattern) req.put("InterfacePattern", interfacePattern);
    ...
    } else { // create an empty one - no extension defs!
