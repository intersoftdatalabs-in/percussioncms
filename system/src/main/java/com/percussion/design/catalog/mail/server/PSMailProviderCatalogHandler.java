...
public class PSMailProviderCatalogHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    Element node = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Provider");

    // at this time, we only have the SMTP mail provider
    com.percussion.mail.PSSmtpMailProvider provider = new com.percussion.mail.PSSmtpMailProvider();

    PSXmlDocumentBuilder.addElement(retDoc, node, "name", provider.getName());

    PSXmlDocumentBuilder.addElement(retDoc, node, "fullName", provider.getFullName());

    PSXmlDocumentBuilder.addElement(retDoc, node, "description", provider.getDescription());

    node = PSXmlDocumentBuilder.addEmptyElement(retDoc, node, "Properties");
    ...
    if (props != null) {
      ...
      while (iterator.hasNext()) {
        ...
        Element propNode = PSXmlDocumentBuilder.addEmptyElement(retDoc, node, "Property");

        // now add in the name/description
        PSXmlDocumentBuilder.addElement(retDoc, propNode, "name", (String) entry.getKey());
        PSXmlDocumentBuilder.addElement(retDoc, propNode, "description", (String) entry.getValue());
      }
    }
