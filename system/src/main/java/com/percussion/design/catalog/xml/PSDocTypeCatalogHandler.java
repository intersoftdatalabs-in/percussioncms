...
/**
 * The PSDocTypeCatalogHandler class implements cataloging of XML document types supported
 * internally by the E2 server. XML documents are used by E2 for storing various pieces of
 * information. They are also used as a transport mechanism (such as catalog request handling). The
 * following categories of documents are used in E2:
 *
 * <ul>
 *   <li>catalog - DTDs used for sending and receiving catalog requests and results
 *   <li>context - DTDs used to provide context information during the processing of user requests
 *       by an application
 *   <li>error - DTDs used for defining how various errors will be packaged for processing be a
 *       style sheet
 * </ul>
 *
 * DocType catalog requests are sent to the server using the PSXDocTypeCatalog XML document. Its
 ...
 * </pre>
 *
 * The PSXDocTypeCatalogResults XML document is sent as the response. Its definition is as follows:
 *
 * <pre>
