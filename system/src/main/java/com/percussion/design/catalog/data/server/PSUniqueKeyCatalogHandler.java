...
public class PSUniqueKeyCatalogHandler extends com.percussion.design.catalog.PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    root = PSXmlDocumentBuilder.createRoot(retDoc, "PSXUniqueKeyCatalogResults");
    ...
    try {
      ...
      try {
        ...
        rs = meta.getPrimaryKeys(detail.getDatabase(), detail.getOrigin(), table);
        if (rs != null) {
          while (rs.next()) {
            if (bFirstRow) {
              keyNode = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "UniqueKey");
              keyNode.setAttribute("type", "primaryKey");
              bFirstRow = false;
            }

            PSXmlDocumentBuilder.addElement(
                retDoc, keyNode, "name", rs.getString(COLNO_PKEY_COL_NAME));
          }
          rs.close();
        }
      ...
      try {
        ...
        rs = meta.getIndexInfo(detail.getDatabase(), detail.getOrigin(), table, true, true);
        if (rs != null) {
          while (rs.next()) {
            ...
            PSXmlDocumentBuilder.addElement(retDoc, keyNode, "name", colName);
          }
          rs.close();
        }
      } catch (java.sql.SQLException e) {
