...
public class PSObjectCatalogHandler extends PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    while (tree.getNextElement("objectType", true, false) != null) {
      ...
      typeList.add(type);
      if (IPSSecurityProviderMetaData.OBJECT_TYPE_USER.equals(type)) findUsers = true;
      else if (IPSSecurityProviderMetaData.OBJECT_TYPE_GROUP.equals(type)) findGroups = true;
    }
    ...
    if (catalogerName != null) {
      PSXmlDocumentBuilder.addElement(retDoc, root, "catalogerName", catalogerName);
    }

    if (catalogerType != null) {
      PSXmlDocumentBuilder.addElement(retDoc, root, "catalogerType", catalogerName);
    }
    ...
    if (typeList != null) {
      for (String objectType : typeList) {
        PSXmlDocumentBuilder.addElement(retDoc, root, "objectType", objectType);
      }
    }

    try {
      List<String> names = null;
      if (filterPattern != null) names = Arrays.asList(filterPattern.split(FILTER_PATTERN_DELIM));

      if (findUsers) addUsers(retDoc, root, names, catalogerName, catalogerType);

      if (findGroups) addGroups(retDoc, root, names, catalogerName, catalogerType);
  ...
  private void addUsers(
      Document retDoc, Element root, List<String> names, String catalogerName, String catalogerType)
      throws PSSecurityCatalogException {
    IPSRoleMgr roleMgr = PSRoleMgrLocator.getRoleManager();
    for (Subject subject : roleMgr.findUsers(names, catalogerName, catalogerType)) {
      Element node = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Object");

      node.setAttribute("type", IPSSecurityProviderMetaData.OBJECT_TYPE_USER);

      PSXmlDocumentBuilder.addElement(
          retDoc, node, "name", PSJaasUtils.subjectToPrincipal(subject).getName());
    }
  }
   ...
   */
  private void addGroups(
      Document retDoc, Element root, String pattern, String catalogerName, String catalogerType)
      throws Exception {
    IPSRoleMgr roleMgr = PSRoleMgrLocator.getRoleManager();
    for (Principal principal : roleMgr.findGroups(pattern, catalogerName, catalogerType)) {
      Element node = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Object");

      node.setAttribute("type", IPSSecurityProviderMetaData.OBJECT_TYPE_GROUP);

      PSXmlDocumentBuilder.addElement(retDoc, node, "name", principal.getName());
    }
  }
