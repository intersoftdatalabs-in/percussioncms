...
public class PSColumnCatalogHandler extends com.percussion.design.catalog.PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    if (driver != null) PSXmlDocumentBuilder.addElement(retDoc, root, "driverName", driver);

    if (server != null) PSXmlDocumentBuilder.addElement(retDoc, root, "serverName", server);

    if (database != null) PSXmlDocumentBuilder.addElement(retDoc, root, "databaseName", database);

    if (schema != null) PSXmlDocumentBuilder.addElement(retDoc, root, "schemaName", schema);

    if (table != null) PSXmlDocumentBuilder.addElement(retDoc, root, "tableName", table);

    Element colNode;
    String colName, colBeType, colJdbcType, colSize, colFraction, colNull;

    String searchSchema = "%";
    if ((schema != null) && (schema.length() != 0)) searchSchema = schema;
    ...
    try {
      ...
      if (rs != null) {
        while (rs.next()) {
          ...
          colNode = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Column");

          PSXmlDocumentBuilder.addElement(retDoc, colNode, "name", colName);

          PSXmlDocumentBuilder.addElement(retDoc, colNode, "backEndDataType", colBeType);

          PSXmlDocumentBuilder.addElement(retDoc, colNode, "jdbcDataType", colJdbcType);

          if ((colFraction != null) && !colFraction.equals("0")) colSize += "." + colFraction;
          PSXmlDocumentBuilder.addElement(retDoc, colNode, "size", colSize);

          colNull = colNull.toLowerCase();
          if (!colNull.equals("yes") && !colNull.equals("no")) colNull = "unknown";
          PSXmlDocumentBuilder.addElement(retDoc, colNode, "allowsNull", colNull);
        }
   ...
   */
  private Connection makeConnection(String driver, String server, String database)
      throws Exception {
    String className = ms_driverMap.get(driver);
    if (className == null)
      throw new RuntimeException("Invalid driver for file system catalog:" + driver);
  ...
  private static Map<String, String> ms_driverMap = new HashMap<>();

  static {
    ms_driverMap.put("psxml", "com.percussion.data.jdbc.PSXmlDriver");
    ms_driverMap.put("psfilesystem", "com.percussion.data.jdbc.PSFileSystemDriver");
  }
}
