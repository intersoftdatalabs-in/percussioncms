...
public abstract class PSCatalogRequestHandler implements IPSRequestHandler, IPSValidateSession {
  ...
  public void processRequest(PSRequest request) {
    try {
       ...
       */
      try {
        com.percussion.server.PSServer.checkAccessLevel(request, PSAclEntry.SACE_ACCESS_DESIGN);
      } catch (Exception e) {
        // don't catch this one, it will generate the access error
        com.percussion.server.PSServer.checkAccessLevel(request, PSAclEntry.SACE_ADMINISTER_SERVER);
      }
      ...
      IPSCatalogRequestHandler rh =
          (IPSCatalogRequestHandler) m_catalogHandlers.get(root.getTagName());
      if (rh == null) {
  ...
  protected static void createErrorResponse(PSRequest request, Throwable t) {
    ...
    if (lh != null) {
      ...
      String reqCategory = request.getCgiVariable(IPSCgiVariables.CGI_PS_REQUEST_TYPE, "");
      ...
      if (reqDoc != null) {
        Element root = reqDoc.getDocumentElement();
        if (root != null) reqType = root.getNodeName();
      }
      ...
      com.percussion.error.PSCatalogRequestError err =
          new com.percussion.error.PSCatalogRequestError(
              request.getUserSessionId(), reqCategory, reqType, errorCode, args);
      lh.write(err);
    }
  ...
  protected static void sendXmlData(PSRequest request, Document doc) {
    try {
      ...
      /* log this */
      Object[] args = {
        request.getUserSessionId(), com.percussion.error.PSException.getStackTraceAsString(t)
      };
