...
public class PSIndexCatalogHandler extends com.percussion.design.catalog.PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    PSXmlDocumentBuilder.addElement(
        retDoc, root, "datasource", datasource == null ? "" : datasource);
    ...
    try {
      ...
      ResultSet rs =
          meta.getIndexInfo(detail.getDatabase(), detail.getOrigin(), table, false, true);
      if (rs != null) {
        while (rs.next()) {
          ...
          if (!lastIndex.equals(indexName)) {
            indexNode = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Index");
            lastIndex = indexName;
            indexNode.setAttribute("enforceUniqueness", (indexUnique ? "yes" : "no"));
            PSXmlDocumentBuilder.addElement(retDoc, indexNode, "name", indexName);
          }

          node = PSXmlDocumentBuilder.addElement(retDoc, indexNode, "indexColumn", colName);

          node.setAttribute("position", String.valueOf(colPos));

          if (colSort == null) node.setAttribute("sorting", "unk");
          else if (colSort.equals("A")) node.setAttribute("sorting", "asc");
          else if (colSort.equals("D")) node.setAttribute("sorting", "desc");
          else node.setAttribute("sorting", "unk");
        }
