...
public class PSServerCatalogHandler extends com.percussion.design.catalog.PSCatalogRequestHandler
  ...
  public void processRequest(PSRequest request) {
    ...
    if (provider != null) PSXmlDocumentBuilder.addElement(retDoc, root, "providerName", provider);

    ResultSet rs = null;
    try {
      IPSSecurityProviderMetaData meta = PSSecurityProviderPool.getProviderMetaData(provider);
      if (meta != null) {
        rs = meta.getServers();
        while (rs.next()) {
          String serverName = rs.getString(1);

          Element node = PSXmlDocumentBuilder.addEmptyElement(retDoc, root, "Server");
