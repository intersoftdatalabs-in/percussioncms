...
public class PSCataloger {
  ...
  public PSCataloger(PSDesignerConnection conn) {
    ...
    if (conn == null) throw new IllegalArgumentException("conn obj null");
  ...
  private IPSCatalogHandler getHandler(java.lang.String reqCategory, java.lang.String reqType) {
    IPSCatalogHandler handler = null;

    if (m_Handlers != null) {
      handler = m_Handlers.get(reqCategory + "-" + reqType);
    } else {
    ...
    if (handler == null) {
      try {
        ...
        Class hClass =
            Class.forName(
                "com.percussion.design.catalog."
                    + reqCategory
                    + ".PS"
                    + reqType
                    + "CatalogHandler");
        handler = (IPSCatalogHandler) hClass.getConstructor(noClass).newInstance(noObjects);
        m_Handlers.put(reqCategory + "-" + reqType, handler);
      } catch (Exception e) {
        throw new IllegalArgumentException(
            "req handler exception: " + reqCategory + " " + reqType + " " + e.toString());
      }
    }
  ...
  public String prepareCredentials(String uid, String pw) {
    try {
      return PSEncryptor.encryptString(
          PathUtils.getRxDir(null).getAbsolutePath().concat(PSEncryptor.SECURE_DIR), pw);
