...
public class PSMacroCatalogHandler implements IPSCatalogHandler {
  ...
  public Document formatRequest(Properties req) {
    ...
    if ((test == null) || (!(REQ_CATEGORY_VALUE.equalsIgnoreCase(test))))
      throw new IllegalArgumentException("req category invalid: null or invalid");

    test = (String) req.get(REQ_TYPE_KEY);
    if ((test == null) || (!(REQ_TYPE_VALUE.equalsIgnoreCase(test))))
      throw new IllegalArgumentException("req type invalid: null or invalid");
  ...
  public static PSMacroDefinitionSet getMacros(PSCataloger cataloger)
      throws PSServerException, PSAuthenticationFailedException, PSAuthorizationException,
          IOException, PSUnknownNodeTypeException {
    ...
    NodeList nodes = doc.getElementsByTagName(PSMacroDefinitionSet.XML_NODE_NAME);
    if (nodes != null && nodes.getLength() > 0) macros.fromXml((Element) nodes.item(0), null, null);
