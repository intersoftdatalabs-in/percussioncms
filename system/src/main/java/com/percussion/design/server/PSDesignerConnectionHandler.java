...
public class PSDesignerConnectionHandler implements IPSRequestHandler {
  ...
  public void processRequest(PSRequest request) {
    ...
    try {
      ...
      if (ms_openRoot.equals(reqType)) respDoc = openSession(request);
  ...
  private Document openSession(PSRequest request)
    ...
    PSXmlTreeWalker w = new PSXmlTreeWalker(request.getInputDocument().getDocumentElement());
    ...
    if (loginPw == null || loginPw.length() == 0) loginPw = "";
    else if (isEncrypted) { // fix bug #Rx-99-12-0016
      try {
        loginPw =
            PSEncryptor.decryptString(
                PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR), loginPw);
    ...
    try {
      try {
        ...
        PSServer.checkAccessLevel(request, PSAclEntry.SACE_ACCESS_DESIGN);
        freeSession = false;
      ...
      if (freeSession)
        com.percussion.server.PSUserSessionManager.releaseUserSession(request.getUserSession());
    }

    // if we've made it here, we have the appropriate access
    if (null != locale && locale.length() > 0)
      request.getUserSession().setPrivateObject(IPSHtmlParameters.SYS_LANG, locale);

    String jsessionid = (String) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_JSESSIONID);

    Document respDoc = PSXmlDocumentBuilder.createXmlDocument();
    Element root = PSXmlDocumentBuilder.createRoot(respDoc, ms_openRoot + "Response");
  ...
  private Document closeSession(PSRequest request) { // an empty response doc means success
    PSUserSession sess = request.getUserSession();
    if (sess != null) com.percussion.server.PSUserSessionManager.releaseUserSession(sess);
  ...
  static com.percussion.system.utils.PSFormatVersion ms_version =
      new com.percussion.system.utils.PSFormatVersion(UTIL_PKG_NAME);
