...
public class PSMailMessage {
  ...
  public PSMailMessage() {
    ...
    try {
      ...
      if (pos != -1) m_from = "e2@" + host.substring(pos + 1);
  ...
  private void appendWrappedBodyText(final char[] chars, final int offset, final int len) {
    ...
    while (true) {
      ...
      switch (c) {
        case '\r':
          ...
          break;
        case '\n':
          if (startNormal != i) writeWithBreaks(chars, startNormal, i - startNormal);
  ...
  public String[] getRecipients() {
    ...
    for (int i = 0; i < toSize; i++) recipients[onEntry++] = (String) m_sendTo.get(i);

    for (int i = 0; i < ccSize; i++) recipients[onEntry++] = (String) m_copyTo.get(i);

    for (int i = 0; i < bccSize; i++) recipients[onEntry++] = (String) m_blindCopyTo.get(i);
  ...
  public void write(Writer writer) throws java.io.IOException {
    ...
    if (m_toHeaderForDisplay != null) {
      ...
      if (!addedUsers) writer.write("BCC:");
    }
    ...
    if (m_bodyIn != null) {
      ...
      int n;
      while ((n = rdr.read(buf, 0, buf.length))
          != -1) { // todo: this really needs to deal with 998 char line limit
  ...
  private void sendMimeHeaders(Writer out) throws java.io.IOException {
    ...
    out.write(
        "Content-Type: TEXT/PLAIN; charset=" + PSCharSets.getStdName(m_charEncoding) + "\r\n");
