 ...
 * processing. An alternative plan can be followed to handle the request, but we will log the
 * condition so we can look into it (and know it occurred). This may expose a flaw in our logic (eg,
 * occurs often when we thought it never should).
 *
...
public class PSNonFatalError extends PSLogError {
  /**
   * Report a non-fatal error. The error code and parameters should clearly define where the error
