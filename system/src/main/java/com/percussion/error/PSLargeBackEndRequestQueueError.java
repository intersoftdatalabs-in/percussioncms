...
/**
 * The PSLargeBackEndRequestQueueError class is used to report large requests queues for a back-end
 * (database) driver. This may signify that the number of connections permitted is insufficient to
 * handle the load, or the amount of time to process a request is taking too long.
 *
...
public class PSLargeBackEndRequestQueueError extends PSLargeRequestQueueError {
   ...
   * @param driver the back-end driver with the large request queue
   * @param server the back-end server with the large request queue
