  ...
  public static synchronized int getHttpCode(PSLogInformation err, Locale loc) {
    ...
    if (i == null) {
      com.percussion.server.PSConsole.printMsg(
          "Error", "ERROR: no default HTTP error code for class " + err.getClass().toString());
      return 500;
    }
    return i.intValue();
  }
  ...
  private static void initErrorHttpCodeMaps(Locale loc)
    ...
    synchronized (ms_codes) {
      if (!ms_areCodesLoaded) {
        ...
        ResourceBundle bun =
            ResourceBundle.getBundle("com.percussion.error.PSErrorHttpCodeBundle", loc);

        String key = null;
        for (Enumeration<String> e = bun.getKeys(); e.hasMoreElements(); ) {
          key = e.nextElement();
          try {
            ms_codes.addReplaceMapping(Class.forName(key), Integer.valueOf(bun.getString(key)));
