...
public class PSExecutableSearchFactory {
  ...
  public static IPSExecutableSearch createExecutableSearch(
    ...
    if (context instanceof PSRequest || context instanceof IPSRequestContext) {
      ...
      if (context instanceof IPSRequestContext) {
        req = new PSRequest(((IPSRequestContext) context).getSecurityToken());
      } else req = (PSRequest) context;

      exSearch = new PSLocalExecutableSearch(req, colNameList, search);
    } else if (context instanceof IPSRemoteRequester) {
      exSearch = new PSWSExecutableSearch((IPSRemoteRequester) context, colNameList, search);
    } else {
  ...
  public static IPSExecutableSearch createExecutableSearch(
    ...
    if (context instanceof PSRequest || context instanceof IPSRequestContext) {
      ...
      exSearch = new PSLocalExecutableSearch(req, colNameList, contentIdList);
    } else if (context instanceof IPSRemoteRequester) {
      exSearch = new PSWSExecutableSearch((IPSRemoteRequester) context, colNameList, contentIdList);
    } else {
      throw new IllegalArgumentException("Invalid context");
    }
