...
public class PSSearchFieldOperators {
  ...
  public static Object[] getOperators(
    ...
    for (int i = 0; i < results.length; i++) {
      ...
      if (translator == null) {
        ...
        else label = PSI18nUtils.getString(operator, locale);
      } else label = translator.getTranslationValue(operator);
  ...
  public static PSEntry getOperator(
    ...
    if (entry != null) {
      ...
      if (translator == null) {
        ...
        else label = PSI18nUtils.getString(entry.getValue(), locale);
      } else label = translator.getTranslationValue(entry.getValue());
  ...
  public static String getInputOperator(PSSearchField field) {
    ...
    if (field.isDateValue()) {
      if (operator.equals(PSSearchField.OP_GREATERTHAN)) result = PSCommonSearchUtils.OP_AFTER;
      else if (operator.equals(PSSearchField.OP_LESSTHAN)) result = PSCommonSearchUtils.OP_BEFORE;
      else if (operator.equals(PSSearchField.OP_BETWEEN)) result = PSCommonSearchUtils.OP_BETWEEN;
      else if (operator.equals(PSSearchField.OP_EQUALS)) result = PSCommonSearchUtils.OP_ON;
    } else if (field.isNumberValue()) {
      if (operator.equals(PSSearchField.OP_BETWEEN)) result = PSCommonSearchUtils.OP_BETWEEN;
      else if (operator.equals(PSSearchField.OP_EQUALS)) result = PSCommonSearchUtils.OP_EQUALS;
      ...
      if (operator.length() > 0 && !operator.equals(PSSearchField.OP_EQUALS)) {
        if (value.startsWith("%") && value.endsWith("%")) result = PSCommonSearchUtils.OP_CONTAINS;
        else if (value.startsWith("%")) result = PSCommonSearchUtils.OP_ENDS_WITH;
        else if (value.endsWith("%")) result = PSCommonSearchUtils.OP_STARTS_WITH;
      } else if (operator.equals(PSSearchField.OP_EQUALS)) result = PSCommonSearchUtils.OP_EXACT;
    }
  ...
  public static String getOutputOperator(PSSearchField field, String operator) {
    ...
    if (operator.length() == 0) throw new IllegalArgumentException("operator cannot be empty");

    String result = PSSearchField.OP_EQUALS;

    if (field.isDateValue()) {
      if (operator.equals(PSCommonSearchUtils.OP_AFTER)) result = PSSearchField.OP_GREATERTHAN;
      else if (operator.equals(PSCommonSearchUtils.OP_BEFORE)) result = PSSearchField.OP_LESSTHAN;
      else if (operator.equals(PSCommonSearchUtils.OP_BETWEEN)) result = PSSearchField.OP_BETWEEN;
      else if (operator.equals(PSCommonSearchUtils.OP_ON)) result = PSSearchField.OP_EQUALS;
    } else if (field.isNumberValue()) {
      if (operator.equals(PSCommonSearchUtils.OP_BETWEEN)) result = PSSearchField.OP_BETWEEN;
      else if (operator.equals(PSCommonSearchUtils.OP_EQUALS)) result = PSSearchField.OP_EQUALS;
      ...
      if (operator.equals(PSCommonSearchUtils.OP_EXACT)) result = PSSearchField.OP_EQUALS;
    }
    ...
    if (!field.usesExternalOperator()
      ...
      for (String val : field.getFieldValues()) {
        ...
        try {
          ...
          String key = PSSearchFieldOperators.class.getName() + "@NonNumericFieldValueException";
          if (translator == null) {
            if (locale != null && locale.trim().length() > 0)
              msg = PSI18nUtils.getString(key, locale);
            else msg = PSI18nUtils.getString(key);
          } else msg = translator.getTranslationValue(key);
  ...
  private static List<String> getOperatorsList(PSSearchField field) {
    ...
    else if (field.isTextValue()) operators = ms_textOperators;
