...
public class PSAddThumbnailURL implements IPSSearchResultsProcessor {
  ...
  public List<Object> processRows(Object[] params, List<Object> rows, IPSRequestContext request)
    ...
    if (params.length > 0
        ...
        && ((String) params[0]).trim().length() > 0) m_thumbnail = (String) params[0];
    else m_thumbnail = THUMBNAIL;
    ...
    loadVariantMappings();
    if (ms_variantMappings == null) return rows; // No variant mappings defined, nothing to do

    while (it.hasNext()) {
      IPSSearchResultRow row = (IPSSearchResultRow) it.next();
      if (row.getColumnValue(m_thumbnail) == null) break; // If No thumbnail column then break out
      ...
      if (variant == null) continue; // Could not find a mapped variant for this
      // content type, so go to the next row.
      String assemblyUrl = getVariantAssemblyURL(contentTypeId, variant);
      if (assemblyUrl == null) continue;
      String url = createUrl(assemblyUrl, contentId, contentTypeId, variant, revision);
  ...
  private String getVariantAssemblyURL(String contenttype, String variant)
      throws PSExtensionProcessingException {
    if (!m_variantUrls.containsKey(contenttype)) loadVariantsByContentType(contenttype);
  ...
  private void loadVariantMappings() throws PSExtensionProcessingException {
    ...
    try {
      ...
      throw new PSExtensionProcessingException("Error fetching variant mapping properties", ioe);
    } finally {
      try {
        if (in != null) in.close();
   ...
   */
  private void loadVariantsByContentType(String contenttype) throws PSExtensionProcessingException {
    ...
    try {
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (variant != null && url != null) variantMap.put(variant, url);
      }
  ...
  private String getElementValue(Element parent, String name) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (child.getNodeType() == Node.TEXT_NODE) return ((Text) child).getData();
    }
    return null;
  }
  ...
  private String createUrl(
      String assemblyUrl, String contentid, String contenttype, String variant, String revision) {
