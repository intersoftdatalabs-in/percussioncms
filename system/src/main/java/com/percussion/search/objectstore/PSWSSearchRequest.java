...
/**
 * Object representation of the request document submitted to the web services search handler. See
 * the {@link #toXml(Document)} method for more info.
 */
public class PSWSSearchRequest {
   ...
   *
   * @param searchParams Specify the search criteria, may not be <code>null</code>.
   */
  public PSWSSearchRequest(PSWSSearchParams searchParams) {
    if (searchParams == null) throw new IllegalArgumentException("searchParams may not be null");
   ...
   *
   * @param searchName The name of an internal search, may not be <code>null</code> or empty. Used
   *     as the name of the request page to make a request to the internal search application.
   * @param params Parameters to pass with the request to the internal search application. May be
   *     <code>null</code>. Key is the parameter name and value is the parameter value, both as
   *     <code>String</code> objects. A shallow copy of the map is stored in this object.
   */
  public PSWSSearchRequest(String searchName, Map<String, String> params) {
    if (searchName == null || searchName.trim().length() == 0)
      throw new IllegalArgumentException("searchName may not be null or empty");

    m_internalSearchName = searchName;
    if (params != null) {
      m_internalSearchParams = new HashMap<>();
      m_internalSearchParams.putAll(params);
    } else m_internalSearchParams = null;

    m_searchParams = null;
  }

  /**
   * Construct a search request from its XML representation. See {@link #toXml(Document)} for
   * details.
   *
   * @param src The root element of the search request, may not be <code>null</code>.
   * @throws PSUnknownNodeTypeException if the <code>src</code> element does not conform the
   *     required format.
   */
   ...
   *
   * @return The params, <code>null</code> if this is object was constructed as an internal search.
   */
  ...
  /**
   * Get the name of the internal search to perform provided during construction.
   *
   * @return The name, <code>null</code> if object was not constructed as an internal search, never
   *     empty.
   */
  ...
  /**
   * Get a read-only copy of the internal search params provided during construction.
   *
   ...
   *
   * @param caseInsensitive <code>true</code> to perform a case-insensitive search, <code>false
   *     </code> to use the database default. If the database is case-insensitive by default, then
   ...
   *
   * @return <code>true</code> if it is case-insensitive, <code>false</code> if not.
   */
   ...
   *
   * @param useExternalSearchEngine <code>true</code> to use the external search engine, <code>false
   *     </code> otherwise.
   */
   ...
   *
   * @return <code>true</code> if the external search engine is used, <code>false</code> otherwise.
   */
   ...
   * details and the required format.
   *
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(ATTR_USE_DATABASE_CASE, m_caseInsensitiveSearch ? XML_FALSE : XML_TRUE);

    root.setAttribute(
        ATTR_USE_EXTERNAL_SEARCH_ENGINE, m_useExternalSearchEngine ? XML_TRUE : XML_FALSE);
  ...
  private void fromXml(Element src) throws PSUnknownNodeTypeException {
    ...
    m_caseInsensitiveSearch =
        !PSXMLDomUtil.getBooleanData(src.getAttribute(ATTR_USE_DATABASE_CASE));

    m_useExternalSearchEngine = false;
    String test = src.getAttribute(ATTR_USE_EXTERNAL_SEARCH_ENGINE);
    if (test != null) m_useExternalSearchEngine = PSXMLDomUtil.getBooleanData(test);
    ...
    else if (PSXMLDomUtil.getUnqualifiedNodeName(child).equals(PSWSSearchParams.XML_NODE_NAME)) {
  ...
  public int hashCode() {
    ...
    if (m_internalSearchName != null) {
      ...
      if (m_internalSearchParams != null) hash += m_internalSearchParams.hashCode();
    } else {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSWSSearchRequest)) isEqual = false;
    else if (this != obj) {
      PSWSSearchRequest other = (PSWSSearchRequest) obj;
      if (m_internalSearchName == null ^ other.m_internalSearchName == null) isEqual = false;
      else if (m_internalSearchName != null
          && !m_internalSearchName.equals(other.m_internalSearchName)) {
        isEqual = false;
      } else if (m_internalSearchParams != null
          && !m_internalSearchParams.equals(other.m_internalSearchParams)) {
        isEqual = false;
      } else if (m_searchParams != null && !m_searchParams.equals(other.m_searchParams)) {
        isEqual = false;
      }
    }
  ...
  /**
   * Search criteria for a standard search, set during ctor, <code>null</code> only if {@link
   * #m_internalSearchName} is not, never modified after construction.
   */
  private PSWSSearchParams m_searchParams;

  /**
   * Name of internal named search to perform, set by ctor, <code>null</code> only if {@link
   * #m_searchParams} is not, never empty, never modified after construction.
   */
  private String m_internalSearchName;

  /**
   * Optional parameters to supply with request to internal named search, may be <code>null</code>,
   * possibly supplied by ctor only if {@link #m_internalSearchName} was supplied, never modified
   * after that.
   */
  private Map<String, String> m_internalSearchParams;

  /**
   * Determines if search is to be case-insentive or if dbms defaults will be used. See {@link
   * #setCaseInsensitiveSearch(boolean)} for details.
   */
  private boolean m_caseInsensitiveSearch = false;

  /**
   * A flag to specify whether to use the external search engine or not. Defaults to <code>false
   * </code>, changed through {@link #setUseExternalSearchEngine(boolean)}.
   */
  ...
  private static final String ATTR_USE_EXTERNAL_SEARCH_ENGINE = "useExternalSearchEngine";
