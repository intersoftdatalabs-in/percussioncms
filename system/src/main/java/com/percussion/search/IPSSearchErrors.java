...
public interface IPSSearchErrors {
  /**
   * Unimplemented operation. This problem is raised if an operation is executed that is not
   * implemented in the server's current mode. This error takes no arguments.
   */
  ...
  /**
   * The search terms for the query are missing. This error does not take arguments. TODO - this
   * should take at least 1 param - the name of the method, and maybe a second param that is all the
   * supplied args (or how many were supplied, or something to give one a hint of what is going on)
   */
  ...
  /** Bad or missing parameters in API call. This error takes no arguments. */
  public static final int SEARCH_ENGINE_BAD_PARAMETERS = 16005;

  /** The search engine ran out of wildcards. This error takes no arguments. */
  public static final int SEARCH_ENGINE_WILDCARD_LIMIT = 16006;

  /**
   * The search engine hit an error that wasn't mapped. Ask the user to call tech support so we can
   * augment the mapping list.
   *
  ...
  /**
   * The object used to manage the search engine configuration is currently being used by someone
   * else.
   *
   * <p>This message has no arguments.
   */
  public static final int ADMIN_HANDLER_LOCKED = 16009;

  /**
   * One or more objects that require releaseXXX to be called are still allocated.
   *
  ...
  /**
   * A search request has been made against the external search engine, but one is not available.
   *
   * <p>This message has no arguments.
   */
  ...
  /**
   * An index event is processed for an item whose content type is not currently valid for indexing.
   *
   * <p>The arguments passed in for this message are:
   *
   * <TABLE BORDER="1">
  ...
  /** Authentication failed. This error takes no arguments. */
