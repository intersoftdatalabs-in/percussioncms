...
public class PSSearchAdminImpl extends PSSearchAdmin {
  ...
  protected void doDelete(PSKey key) throws PSSearchException {
    ...
    String ctypeDirPath = PSSearchEngineImpl.getLuceneIndexRootPath() + key.getPart();
    File ctypeDir = new File(ctypeDirPath);
    if (!ctypeDir.exists()) {
      // The directory does not exist and there is nothing to delete
      // Write some debug information
      String msg = "Skipping deletion of indexes for content type id({0})." + " No folder exists.";
    ...
    if (!ctypeDir.isDirectory()) {
      ...
      String msg =
          "Skipping deletion of indexes for content type id({0})."
    ...
    try {
      ...
      if (!success) {
        String msg = "Failed to delete the index directory for content " + "type id ({0}).";
        log.error(MessageFormat.format(msg, key.getPart()));
      }
      String msg = "Successfully deleted the index directory for content " + "type id ({0}).";
      log.debug(MessageFormat.format(msg, key.getPart()));
    } catch (Exception e) {
      String msg = "Failed to delete the index directory for content " + "type id ({0}).";
      throw new PSSearchException(MessageFormat.format(msg, key.getPart()), e);
    }
  }
  ...
  public PSKey[] optimizeIndexes(PSKey[] contentTypes) throws PSSearchException {
    ...
    for (PSKey key : contentTypes) {
      ...
      IndexWriter iw = iws.get(ctypeId);
      if (iw != null) {
        String msg = "Failed to optimize index for content type with id" + key.getPartAsInt();
        try {
          iw.forceMerge(1);
          successList.add(key);
          String msg1 = "Successfully optimized indexes for content " + "type id ({0}).";
  ...
  public PSKey[] rebuildIndexes(PSKey[] contentTypes) throws PSSearchException {
    ...
    for (PSKey key : contentTypes) {
      ...
      boolean success =
          (new File(PSSearchEngineImpl.getLuceneIndexRootPath() + idAsString)).mkdirs();
      if (success) {

        PSSearchIndexEventQueue.getInstance().indexContentType(key.getPartAsInt());
        successList.add(key);
        String msg = "Successfully queued items of content type id ({0}) " + "for reindexing.";
        log.debug(MessageFormat.format(msg, key.getPart()));
      } else {
        String msg = "Failed to create an index directory for content " + "type id ({0})";
  ...
  private void validateKeyArray(PSKey[] types) {
    if (null == types || types.length < 1) {
      throw new IllegalArgumentException("contentTypes must have at least 1 entry");
    }

    for (PSKey type : types) {
      if (type == null) throw new IllegalArgumentException("No null entries in array.");
    }
  }
  ...
  public void verify(Set<Long> knownContentTypes) throws PSSearchException {
    ...
    if (rootDir.exists()) {
      ...
      if (indexDirs != null) {
        for (File indexDir : indexDirs) {
          ...
          if (id != null) currentIndexes.add(id);
        }
      }
    }
    ...
    for (Long id : currentIndexesCopy) {
      String msg =
          "A directory with name {0} exists under lucene indexes "
              + "folder {1} without a known content type.";
      Object[] args = {id.toString(), PSSearchEngineImpl.getLuceneIndexRootPath()};
      log.info(MessageFormat.format(msg, args));
    }
    // Create directories
    for (Long id : knownContentTypesCopy) {
      String msg =
          "Created an index directory for content type id ({0}, submitting the type for"
              + " indexing...)";
        ...
        for (File indexDir : indexDirs) {
          ...
          if (lockedFiles != null) {
            for (String lf : lockedFiles) {
              File file = new File(indexDir.getAbsolutePath() + File.separator + lf);

              try {
                Files.deleteIfExists(Paths.get(file.getAbsolutePath()));
              } catch (IOException e) {
                log.warn(
                    "Unable to delete lock file: {} Error: {}",
