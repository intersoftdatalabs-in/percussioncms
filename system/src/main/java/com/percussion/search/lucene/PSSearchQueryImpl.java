...
public class PSSearchQueryImpl extends PSSearchQuery implements Closeable {
    ...
    try (MultiReader mr = prepareMultiSearcher(ctypeIds)) {
      ...
      if (count > 0) {
        ...
        for (ScoreDoc sd : docs.scoreDocs) {
           ...
           * (Fix for RX-16244)
           */
          if (cid == null || processedIds.contains(cid)) continue;
  ...
  private Query prepareSearchQuery(
    ...
    if (StringUtils.isNotBlank(globalQuery)) {
      ...
      boolean defaultSynExp =
          PSServer.getServerConfiguration().getSearchConfig().isSynonymExpansionRequired();
      boolean synExpansion = getBoolProp(props, QUERYPROP_SYNONYM_EXPANSION, defaultSynExp);
      boolean synExpanded = false;
      if (synExpansion) {}
      // If we have not expanded the query with synonyms search on original
      // query.
      if (!synExpanded) {
        QueryParser qp = new QueryParser(IPSLuceneConstants.ALL_CONTENT_FIELD_NAME, an);
    ...
    if (addGlobalBooleanQR) builder.add(new BooleanClause(qr, Occur.MUST));
  ...
  private MultiReader prepareMultiSearcher(Collection<? extends PSKey> ctypeIds)
    ...
    try {
      ...
      for (PSKey ctype : cTypeKeys) {
        String msg =
            "Failed to create IndexSearcher for content "
  ...
  private boolean getBoolProp(Map<String, String> props, String keyName, boolean defaultValue) {
    ...
    if (null != o) {
      ...
      if (val.length() > 0) {
        ...
        if (val.equalsIgnoreCase("yes") || val.equalsIgnoreCase("true")) result = true;
        else result = false;
      }
    }
    return result;
