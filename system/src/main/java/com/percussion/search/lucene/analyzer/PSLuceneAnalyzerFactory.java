...
public class PSLuceneAnalyzerFactory {
   ...
   */
  public static synchronized PSLuceneAnalyzerFactory getInstance() {
    if (ms_instance == null) {
  ...
  private void init() {
    ...
    while (extensionIterator.hasNext()) {
      ...
      try {
        if (!isValidLangString(language)) {
          ...
          String msg = "Registered locale " + language + " is not currently supported by the CMS";
          log.error(msg);
        }
        IPSExtension ext = manager.prepareExtension(extensionCall.getExtensionRef(), null);
        if (ext instanceof IPSLuceneAnalyzer) {
          IPSLuceneAnalyzer analyzer = (IPSLuceneAnalyzer) ext;
          m_analyzers.put(language, analyzer.getAnalyzer(language));
        } else {
        ...
        String msg =
            "Error loading registered extension "
                + extensionCall.getName()
                + " with locale "
                + language
        ...
        String msg =
            "Failed to get the Analyzer object for registered "
                + "extension "
                + extensionCall.getName()
                + " with locale "
                + language
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        if (m_analyzers.get(ls) != null) continue;
        Analyzer an = getDefaultAnalyzer(ls);
        if (an instanceof WhitespaceAnalyzer) {
          String msg =
              "Could not find a user registered or system "
                  + "analyzer for locale \""
                  + ls
  ...
  private boolean isValidLangString(String langstr) throws PSLocaleException {
    ...
    while (iter.hasNext()) {
      PSLocale loc = iter.next();
      String ls = loc.getLanguageString();
      if (ls.equalsIgnoreCase(langstr)) return true;
    }
    return false;
  }
  ...
  private Analyzer getDefaultAnalyzer(String languageString) {
    ...
    try {
      ...
      log.warn(
          "Unable to detect Analyzer for Locale: {} , defaulting to Whitespace Analyzer.",
          languageString);
    }

    if (al == null) al = new WhitespaceAnalyzer();
  ...
  public Analyzer getAnalyzer(String languageString) {
    ...
    if (al == null) al = getDefaultAnalyzer(languageString);
    return al;
  }
