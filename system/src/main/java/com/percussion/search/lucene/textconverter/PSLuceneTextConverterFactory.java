...
public class PSLuceneTextConverterFactory {
   ...
   */
  public static synchronized PSLuceneTextConverterFactory getInstance() {
    if (ms_instance == null) {
  ...
  private void init() {
    ...
    while (extiter.hasNext()) {
      ...
      try {
        IPSExtension ext = extmgr.prepareExtension(extcall.getExtensionRef(), null);
        if (ext instanceof IPSLuceneTextConverter) {
          m_converters.put(StringUtils.lowerCase(mtype), (IPSLuceneTextConverter) ext);
        } else {
  ...
  private void buildSystemConverterMap() {
    ...
    m_systemConverters.put(IPSLuceneConstants.MIME_TYPE_TEXT_BY_HTML, new PSTextConverterHtml());
  }
