...
public class PSSearchEngineImpl extends PSSearchEngine {
  ...
  public PSSearchEngineImpl() throws PSSearchException {
    ...
    // @TODO we need to make it as property, as I am just fixing the bug for a patch I did not make
    // it as a property.
  ...
  protected PSSearchAdmin doGetSearchAdmin(boolean wantLocked) {
    if (wantLocked) return m_searchAdmin;
    else return m_searchAdminReadOnly;
  }

  @Override
  protected synchronized PSSearchIndexer doGetSearchIndexer() throws PSSearchException {
  ...
  @Override
  protected Element doGetStatus(Document doc, Element root) throws PSSearchException {
  ...
  @Override
  protected void doReleaseSearchIndexer(PSSearchIndexer si) throws PSSearchException {
  ...
  @Override
  protected void doReleaseSearchQuery(PSSearchQuery sq) throws PSSearchException {
  ...
  private void closeIndexWriters() throws PSSearchException {
    ...
    while (iter.hasNext()) {
      Long ct = iter.next();
      String ermsg =
          "Failed to close the index writer for contenttype " + "id {0} during server shutdown.";
  ...
  protected void setStateCode(int code) {
    if (code != STATUS_INITIALIZED
        && code != STATUS_RUNNING
        && code != STATUS_TERMINATED
        && code != STATUS_TERMINATING
        && code != STATUS_PAUSED) {
