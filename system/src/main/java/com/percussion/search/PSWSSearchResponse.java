...
public class PSWSSearchResponse {
  ...
  public PSWSSearchResponse(Document doc, String curUser) throws PSUnknownNodeTypeException {
    ...
    Element row = tree.getNextElement(PSSearchResultRow.XML_NODE_NAME, firstFlag);
    while (row != null) {
  ...
  public IPSSearchResultRow createRow(Element rowElem, String curUser)
    ...
    if (curUser != null && curUser.trim().length() > 0) {
      ...
      String checkOutUser = resRow.getColumnValue("sys_contentcheckoutusername");

      String sysRev;
      if (checkOutUser.trim().length() == 0 || checkOutUser.equalsIgnoreCase(curUser)) {
        sysRev = tipRev;
      } else sysRev = curRev;
      ...
      if (resRow.hasColumn(IPSHtmlParameters.SYS_REVISION)) {
        ...
        PSSearchResultColumn field =
            new PSSearchResultColumn(IPSHtmlParameters.SYS_REVISION, sysRev, sysRev);
  ...
  public List<IPSSearchResultRow> getRowList() {
    List<IPSSearchResultRow> list = new ArrayList<IPSSearchResultRow>(m_rowList.size());
  ...
  public void setRows(List<IPSSearchResultRow> rows) {
    ...
    try {
      ...
      throw new IllegalArgumentException("rows must contain PSSearchResultColumn objects");
    }
  ...
  public void addColumn(String columnName, String defaultValue) {
    ...
    for (IPSSearchResultRow row : m_rowList) {
      ...
      if (!rr.hasColumn(columnName)) {
        PSSearchResultColumn col = new PSSearchResultColumn(columnName, null, defaultValue);
        rr.addColumn(col);
      }
    }
  }
