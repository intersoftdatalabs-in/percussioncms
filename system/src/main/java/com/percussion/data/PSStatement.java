...
public abstract class PSStatement implements IPSExecutionStep {
  ...
  protected PSStatement(int connKey, IPSStatementBlock[] blocks, boolean buildSql)
    ...
    if (buildSql) m_sqlString = buildSqlString(null); // build the sql statement once
    else m_sqlString = null; // must be building it on the fly each time
  }
  ...
  public java.lang.String buildSqlString(PSExecutionData data, boolean forceRebuild)
    ...
    if ((m_sqlString != null) && !forceRebuild) s = m_sqlString;
    else {
  ...
  protected void joinSqlExceptions(
      Connection conn, PreparedStatement stmt, java.sql.SQLException e) {
    SQLWarning wConn = null;
    SQLWarning wStmt = null;

    if (conn != null) {
      try {
        wConn = conn.getWarnings();
  ...
  protected void logPreparedStatement(PSExecutionData data, String statementText) {
  ...
  public abstract void execute(PSExecutionData data)
      throws java.sql.SQLException, com.percussion.error.PSIllegalArgumentException,
          com.percussion.data.PSDataExtractionException, com.percussion.error.PSErrorException;
