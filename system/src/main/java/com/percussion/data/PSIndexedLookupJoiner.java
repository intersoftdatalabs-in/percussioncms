...
public class PSIndexedLookupJoiner extends PSQueryJoiner {
  ...
  private void processData(PSExecutionData execData, PSJoinedRowDataBuffer rowData)
    ...
    while (rowData.readLeftRow()) {
       ...
       */
      if (m_translator != null) keyData = m_translator.extract(execData);
      else keyData = curRow[m_leftColumnIndex];

      if (keyData == null) params.remove(paramName);
      else {
        try {
          keyData = PSDataConverter.convert(keyData, PSDataConverter.DATATYPE_TEXT);
      ...
      try {
        ...
        // no data
        if (stackDepth == stack.size()) continue;
