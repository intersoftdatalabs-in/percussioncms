...
public class PSConditionalExitEvaluator extends PSRuleListEvaluator {
   ...
   */
  public PSConditionalExitEvaluator(
      PSConditionalExit exit, PSApplicationHandler appHandler, String className)
      throws PSExtensionException, PSNotFoundException {
    super(exit != null ? exit.getCondition() : null);
    if (exit == null || appHandler == null)
      throw new IllegalArgumentException("exit and appHandler may not be null");
    if (className == null || className.trim().length() == 0)
      throw new IllegalArgumentException("className cannot be null orempty");

    try {
      m_preparedExits = new ArrayList<>();
      PSDataHandler.loadExtensions(appHandler, exit.getRules(), className, m_preparedExits);
