...
public class PSUpdateOptimizer extends PSOptimizer {
  ...
  public static IPSExecutionStep[] createExecutionPlan(
    ...
    if (planType == PLAN_TYPE_INSERT) {
      if (!dataSync.isInsertingAllowed()) return null;
    } else if (planType == PLAN_TYPE_UPDATE) {
      if (!dataSync.isUpdatingAllowed()) return null;
    } else if (planType == PLAN_TYPE_DELETE) {
      if (!dataSync.isDeletingAllowed()) return null;
    } else {
    ...
    for (int i = 0; i < tableCount; i++) {
      ...
      for (int j = 0; j < builderCount; j++) {
        ...
        if ((planType == PLAN_TYPE_UPDATE) && !curBuilder.hasUpdateColumns()) {
          ...
          Object[] args = {ah.getName(), ds.getName(), curTable.getAlias()};
          ah.getLogHandler()
              .write(
     ...
     */
    execSteps = reorderExecutionSteps(planType, beTables, execSteps, builderMaps);
  ...
  private static PSUpdatePipe getUpdatePipe(PSApplicationHandler ah, PSDataSet ds)
    ...
    if (pipe instanceof com.percussion.design.objectstore.PSUpdatePipe) return (PSUpdatePipe) pipe;
  ...
  private static java.util.List reorderExecutionSteps(
    ...
    if ((planType == PLAN_TYPE_INSERT) || (planType == PLAN_TYPE_UPDATE)) pkeyTableFirst = true;
      ...
      if (dependencyMap.size() == 0) {
        ...
        for (int i = 0; i < tableCount; ) {
          ...
          while (++i < tableCount) {
            ...
            if (builderMaps.get(nextTable) != null) break; // or continue???? (dbreslau 12/19/02)
          }
      ...
      for (int i = 0; i < tableCount; i++) {
        ...
        for (int j = 0; j < builderCount; j++) {
          ...
          if (crossDependentTable != null) {
             ...
             */
            if (pkeyTableFirst) newSteps.add(tableName);
            else newSteps.add(0, tableName);
          } else {
            ...
            for (int k = 0; k < dependentTables.size(); k++) {
              ...
              if (pos != -1) {
                if (pkeyTableFirst) {
                  if (pos < insertPos) insertPos = pos;
                } else if (pos > insertPos) insertPos = pos;
              }
            }
