...
/**
 * The PSSetStyleSheetEvaluator class implements exit handling for the setStyleSheet simple action
 * exit. This exit can be used against context information and the XML document. If the conditional
 * evaluates to true, the style sheet associated with this exit will be used.
 *
...
public class PSSetStyleSheetEvaluator extends PSConditionalEvaluator {
  /**
   * Build an evaluator which will check the specified conditions and set the style sheet if they
   * conditions are met.
   *
   ...
   */
  public PSSetStyleSheetEvaluator(
      URL styleSheet, PSResultPage resultPage, PSCollection conditionals) {
    super(conditionals);
   ...
   *
   * <p>Override conditional exit handler's isMatch() method to check if the request URL's extension
   * is supported by the result page associated with this exit handler.
   *
   * @param data The execution data associated with the current request.
   * @throws PSEvaluationException if an error occurs doing the underlying isMatch().
   */
