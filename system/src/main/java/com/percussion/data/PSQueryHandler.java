...
/**
 * The PSQueryHandler class is used internally by the Rhythmyx server to process queries. Once an
 * application handler determines a request which is for a query, it hands the request off to the
 * query handler. The query handler must then prepare the request for the given back-end and prepare
 * the resulting data for returning to the requestor.
 *
...
public class PSQueryHandler extends PSDataHandler implements IPSInternalResultHandler {
  /**
   * Construct a query handler to manage the querying for the specified data set.
   *
   * <p>The steps performed during construction:
   *
   * <ol>
   *   <li>call the query optimizer to build the execution plan
   *   <li>go through the execution plan, and for each PSStatement object, add its column mapper to
   *       the proper place
   *   <li>set the result set converter to be used (m_converter)
   * </ol>
   *
   ...
   * @param ds the data set containing the query pipe(s) this object will handle
  ...
  public PSQueryHandler(PSApplicationHandler app, PSDataSet ds)
      throws PSInvalidRequestTypeException, java.sql.SQLException, PSIllegalArgumentException,
          PSNotFoundException, PSCatalogException, PSSystemValidationException,
    ...
    try {
      if (requestor.isDirectDataStream()) m_converter = new PSResultSetMimeConverter(app, ds);
      else m_converter = new PSResultSetHtmlConverter(app, ds);
    } catch (RuntimeException e) {
      // for unknown exceptions, it's useful to log the stack trace
      Object[] args = {app.getName(), com.percussion.error.PSException.getStackTraceAsString(e)};
      throw new PSSystemValidationException(
  ...
  /**
   * Return a reference to the pager for the query. This is used to tune the Statement and
   * ResultSet.
   *
   * @return May be <code>null</code> if the dataset associated with this handler doesn't have a
   *     pager.
   */
  ...
  /**
   * Process a query request using the input context information and data. Execute the request,
   * running any pre-processing extensions.
   *
   * @param request The request context.
   * @return The execution data, which can be used to retrieve a result set.
   * @throws PSErrorException if a lower level call throws this
   * @throws PSIllegalArgumentException if a lower level call throws this
   * @throws PSDataExtractionException if an error occurs extracting a data value
   * @throws SQLException if a sql error occurs
   */
  public PSExecutionData executeQueryRequest(PSRequest request)
      throws SQLException, PSIllegalArgumentException, PSErrorException, PSDataExtractionException {
    ...
    try {
      ...
      boolean isTraceEnabled = dh.isTraceEnabled(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG);
      ...
      if (!success && execData != null) execData.release();
    }
  ...
  /**
   * This filter is used during document construction. It is passed to the converter, which will
   * pass all rows in the result set through it. Only those rows that are accepted will be included
   * in the result document.
   *
   * @param filter May be <code>null</code>. If <code>null</code>, no filtering will occur.
   */
   ...
   *
   * @return The filter set by {@link #setRowFilter(IPSResultSetDataFilter) setRowFilter}. Default
   *     is <code>null</code>.
   */
  ...
  /**
   * Process a query request using the input context information and data. The results will be
   * written to the specified output stream.
   *
   * <p>The following steps are performed to handle the request:
   *
   * <ol>
   *   <li>fill query statement(s) with selection variables.
   *   <li>run the execution plan (IPSExecutionStep[])
   *   <li>convert the data to the appropriate output format (XML or HTML). This is done by calling
   *       the result set converter (m_converter) with the request infomation. The converter will
   *       then attempt to generate the appropriate output. If this cannot be done, an exception
   *       will be shown.
   * </ol>
   *
      ...
      if (m_converter instanceof PSResultSetMimeConverter) {
        ...
        try {
          return ((PSResultSetXmlConverter) m_converter)
              .createXmlDocument(execData, m_rowFilter, true);
  ...
  public ByteArrayOutputStream getMergedResult(PSExecutionData data)
      throws PSInternalRequestCallException, PSAuthorizationException,
          PSAuthenticationFailedException {
    if (data == null) throw new IllegalArgumentException("Executeion data cannot be null.");
    ...
    try {
      if (m_converter instanceof PSResultSetMimeConverter) {
        ...
        try {
          Document resultDoc =
              ((PSResultSetXmlConverter) m_converter).createXmlDocument(data, m_rowFilter, false);
  ...
  /**
   * A special version of the <code>getMergedResult</code> method that will run the namespace
   * cleanup on the merged result if the stylesheet's namespace cleanup flag is set. See {@link
   * IPSInternalResultHandler#getMergedResult(PSExecutionData)} for more details.
   *
   * @param invokeNamespaceCleanup flag indicating that namespace cleanup should run if the flag is
   *     set on the stylesheet.
   */
  public ByteArrayOutputStream getMergedResult(PSExecutionData data, boolean invokeNamespaceCleanup)
      throws PSInternalRequestCallException, PSAuthorizationException,
          PSAuthenticationFailedException {
    ByteArrayOutputStream result = getMergedResult(data);
    if (invokeNamespaceCleanup) {
      PSResultSetXmlConverter converter = ((PSResultSetXmlConverter) m_converter);
      int index = converter.getResultPageIndex(data);
      String encoding = converter.getEncodingForRequestPage(index);
      if (encoding == null) encoding = converter.getRequestorCharacterEncoding();
  ...
  public Document getResultDoc(PSExecutionData data) throws PSInternalRequestCallException {
    if (m_converter instanceof PSResultSetMimeConverter) {
      ...
      try {
        if (data != null)
          return ((PSResultSetXmlConverter) m_converter).createXmlDocument(data, m_rowFilter, true);
  ...
  public PSMimeContentResult getMimeContent(PSExecutionData data, boolean setResponse)
    ...
    if (data == null) throw new IllegalArgumentException("Execution data must not be null.");

    if (m_converter instanceof PSResultSetMimeConverter) {
      try {
        ((PSResultSetMimeConverter) m_converter).processMimeContent(data, setResponse);
  ...
  /**
   * Get the result set based on the execution data created when the internal request was made.
   * Since there is no conversion necessary, none of the resultset converters need be called. The
   * caller of this method is not responsible for closing the result set!
   *
   * @return the result set, or <code>null</code> if the result set stack is empty or the execution
   *     data has not been supplied.
   * @throws PSInternalRequestCallException If there are multiple result sets to return.
   */
  public ResultSet getResultSet(PSExecutionData data) throws PSInternalRequestCallException {
    ...
    if (data != null) {
      ...
      if (stack.size() > 1) {
        throw new PSInternalRequestCallException(
            IPSDataErrors.CANNOT_RETURN_MULTIPLE_RESULT_SETS, new Integer(stack.size()));
      } else {
  ...
  /**
   * This array contains the query execution plan. It contains the steps that must be performed, in
   * the appropriate order. It may contain SELECT statements (PSQueryStatement), joins
   * (PSQueryJoiner), and extension runners (???).
   */
  ...
  /**
   * See {@link #setRowFilter(IPSResultSetDataFilter)} for details. Defaults to <code>null</code>.
   */
