...
public class PSStatementBlock implements IPSStatementBlock {
   ...
   */
  public void addReplacementField(
  ...
  public int setColumnData(PSExecutionData data, PreparedStatement stmt, int bindStart)
    ...
    if (shouldThisBeOmitted(data)) return bindStart;

    for (Object cur : m_blocks) {
      if (cur instanceof PSStatementColumn) {
        bindStart = ((PSStatementColumn) cur).setData(data, stmt, bindStart);
      }
    }
  ...
  public void releaseColumnData() {
    for (Object cur : m_blocks) {
      if (cur instanceof PSStatementColumn) ((PSStatementColumn) cur).releaseData();
    }
  }
  ...
  public void buildStatement(StringBuilder buf, PSExecutionData data)
    ...
    // if we're doing omit when null, check if we have any null values
    if (shouldThisBeOmitted(data)) return;

    for (Object cur : m_blocks) {
      if (cur instanceof PSStatementColumn)
        buf.append(((PSStatementColumn) cur).getPlaceHolder(data));
      else buf.append((String) cur);
    }
  }
  ...
  public List<IPSDataExtractor> getReplacementValueExtractors() {
    ...
    for (Object m_block : m_blocks) {
      if ((cur = m_block) instanceof PSStatementColumn) {
        retList.add(((PSStatementColumn) cur).getReplacementValueExtractor());
      }
    }
    return retList;
  }
  ...
  public void addReplacementField(IPSReplacementValue value, Object[] params) {
    ...
    try {
      ...
      throw new IllegalArgumentException("Invalid jdbc type specified." + ex.getLocalizedMessage());
    }
  ...
  protected boolean shouldThisBeOmitted(PSExecutionData data)
    ...
    if (!m_isStatic) {
      for (Object cur : m_blocks) {
        if ((cur instanceof PSStatementColumn) && ((PSStatementColumn) cur).isNull(data)) {
          return true;
        }
      }
    }
