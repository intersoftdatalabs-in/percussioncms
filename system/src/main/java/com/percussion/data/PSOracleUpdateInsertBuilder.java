...
public class PSOracleUpdateInsertBuilder extends PSOracleUpdateBuilder {
  ...
  PSUpdateStatement generate(List<PSBackEndLogin> logins, ConcurrentHashMap<?, Integer> connKeys)
    ...
    PSSqlBuilderContext insertContext = getInsertContext(table, login, dtHash, columnList);

    // Build the UPDATE context
    PSSqlBuilderContext updateContext = getSingleRowidUpdateContext(table, login, dtHash);

    try {
      return new PSOracleUpdateInsertStatement(
          iConnKey,
          updateContext.getBlocks(),
          insertContext.getBlocks(),
