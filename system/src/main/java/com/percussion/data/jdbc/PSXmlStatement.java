...
/**
 * The PSXmlStatement class extends the File System driver's statement handling for XML file
 * support.
 *
...
public class PSXmlStatement extends PSFileSystemStatement {
  ...
  /**
   * Package access no-arg constructor. Generally, the only way to get a PSXmlStatement object is to
   * get one from a PSXmlConnection object.
   *
   ...
   *
   * <p>Limited search capabilities are supported against XML files using SQL constructs. In
   ...
   *
   * <ul>
   *   <li>FieldList - one or more comma delimited fields (elements or attributes) defined in the
   *       XML document. Use the full object hierarchy when selecting columns. For instance, <code>
   *       SELECT Products/manufacturer, Products/partnumber</code>
   *   <li>XmlFile - the name of the XML file(s) to search. Use the XML file's URL as the name of
   *       the file. Only file based URLs (file://) are currently supported.
   * </ul>
   *
  ...
  /**
   * @author chadloder
   *     <p>Private utility method to execute a parsed statement.
   * @param statement The root of the parse tree.
   * @since 1.2 1999/5/7
   */
  private ResultSet doXmlFileQuery(SimpleNode node) throws java.io.IOException, SQLException {
