...
/**
 * ************************************************************************** This class runs SQL
 * queries, given a SQL statement parse tree. The main() method allows interactive debugging of the
 * class.
 *
 * <p>DEBUGGING NOTE: To enable trace, follow these steps:
 *
 * <p>1) Replace all occurrences of the string "//--+trace(" with "trace(".
 *
 * <p>2) When you construct a PSXmlDocumentQuery object, before calling run() on it, call
 * enableTrace(System.out) on it.
 *
 * <p>Now trace messages should be sent to the console when you run this.
 *
 * <p>To disable trace, follow the steps in reverse, but be careful when replacing all occurrences
 * of "trace(" with "//--+trace". You must only replace occurrences that are preceded by whitespace,
 * otherwise you will end up changing "enableTrace(" to "enable//--+trace". You can do this in
 * Developer Studio by replacing with regular expressions: replace "\(\:b+\)trace(" with
 * "\1//--+trace(" (check the Regular Expression checkbox in the Replace dialog).
 *
 * <p>All this crap is necessary because there is no Java preprocessor.
 *
 * <p>*************************************************************************
 */
public class PSXmlDocumentQuery implements SQLParserVisitor {
  ...
  public Object visit(ASTSelectList node, Object data) {
    ...
    if (node.isAllColumns()) {
      ...
      for (int i = 0; i < node.jjtGetNumChildren(); i++) {
        ...
        IPSReplacementValue val = (IPSReplacementValue) derivedCol.jjtAccept(this, data);
  ...
  public Object visit(ASTFromClause node, Object data) {
    ...
    try {
      ...
      for (int i = 0; i < node.jjtGetNumChildren(); i++) {
        ...
        if (!xmlFile.canRead() || !xmlFile.isFile()) {
          throw new UncheckedSQLException("The file " + fs.getValue() + " does not exist.");
        }
      ...
      for (int i = 0; i < con.getNumDocuments(); i++) {
        ...
        if (docType == null) {
          if (dtdPresent) throw new UncheckedSQLException("Documents do not share a common DTD.");
        } else {
          dtdPresent = true;
          if (sharedDocType != null) {
            if (!docType.equals(sharedDocType)) {
              throw new UncheckedSQLException(
                  "Documents do not share a common DTD: "
                      + docType.getName()
                      + " != "
                      + sharedDocType.getName());
            }
          }
      ...
      if (dtdPresent) {
        ...
        if (doc != null) dtdParser.parseXmlForDtd(doc, false);
      }
      // no DTD present, we should guess at one
        ...
        ByteArrayInputStream bias = new ByteArrayInputStream(bout.toByteArray());
        dtdParser.parseDtd(bias, null);
      }
      // create a DTD tree from the DTD in the parser
      dtd = dtdParser.getDtd();
      if (dtd != null) dtdTree = new PSDtdTree(dtd);
    } catch (PSCatalogException e) {
      // --+trace(e);
      throw new UncheckedSQLException("Error getting DTD: " + e.getMessage());
    ...
    if (dtdTree != null) dtdRootEntry = dtdTree.getRoot();
  ...
  public Object visit(ASTComparisonPredicate node, Object data) {
    ...
    IPSReplacementValue leftVal = (IPSReplacementValue) leftDerivedCol.jjtAccept(this, data);

    ASTDerivedColumn rightDerivedCol = (ASTDerivedColumn) node.jjtGetChild(1);
    IPSReplacementValue rightVal = (IPSReplacementValue) rightDerivedCol.jjtAccept(this, data);
  ...
  private void flattenDocument(QueryContext con) {
    ...
    if (mapper.getNumTables() < 1) throw new UncheckedSQLException("Error: no tables");
  ...
  private void processElement(
    ...
    // --+trace("seekToFirstElement(" + tableName + ") positioned us on " +
    // firstSibling.getNodeName());
      ...
      try {
        ...
        for (int i = 1; i <= table.getNumColumns(); i++) // 1 based
        {
          ...
          for (int j = 0; j < col.getNumPkeyReferences(); j++) {
            PSDtdRelationalMapper.ColumnDef referringCol = col.getPkeyReference(j);

            PSDtdRelationalMapper.TableDef referringTable = referringCol.getTable();

            // --+trace("Chasing pkey ref " + referringTable.getName() + "." +
            // referringCol.getName() + "(->" + col.getName() + ")");

            PSResultSet referringTableRS = con.getResultSet(referringTable.getName());
  ...
  private void processRow(
    ...
    try {
      ...
      for (int i = 1; i <= table.getNumColumns(); i++) // 1 based
      {
        ...
        if (unIDCol != null) {
          if (i == unIDCol.getOrdinal()) continue;
        }
        ...
        if (colVal == null) rs.updateNull(i);
        else rs.updateString(i, colVal);
      }
    } catch (SQLException e) {
      // --+trace(e);
      throw new UncheckedSQLException(e.getMessage());
    ...
    if (!el.getNodeName().equals(name)) {
      ...
      for (int i = 0; i < children.getLength(); i++) {
        ...
        if (n.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
          if (n.getNodeName().equals(name)) return (Element) n;
        } else continue;
      }
  ...
  public java.lang.String getElementData(org.w3c.dom.Element node) {
    ...
    if (node != null) {
      for (text = node.getFirstChild(); text != null; text = text.getNextSibling()) {
         ...
         */
        if (text instanceof org.w3c.dom.Text) ret += text.getNodeValue();
        else break; // assumes normalized document
      }
    }
  ...
  PSBackEndColumn qualifyColumnName(ASTColumnReference col, QueryContext con) {
    ...
    if (startTable == null) startTable = con.getRelationalMapper().getTable(1); // 1-based
    ...
    if (warnText.length() > 0) {
      warnText =
          "Column ref "
              + colName
              + " is assumed to mean "
              + qualifiedCol.getName()
              + " in table "
              + qualifiedCol.getTable().getName()
              + ", but: "
              + warnText;
  ...
  void findColumnName(
    ...
    for (int i = 1; i <= tabDef.getNumColumns(); i++) // 1 based
    {
      ...
      for (int j = 0; j < col.getNumPkeyReferences(); j++) {
        ...
        findColumnName(occurrences, prefix + tabDef.getName() + "/", referringTable, colName);
      }
    }
  }
  ...
  private class QueryContext {
    ...
    public void addSelectColumn(IPSReplacementValue dCol, String asName) {
      ...
      if (dCol instanceof PSBackEndColumn) {
        ...
        if (tabOrd < 1) throw new UncheckedSQLException("Unknown table: " + tabName);
    ...
    public void addWhereConditional(PSConditional cond) {
      ...
      if (right instanceof PSBackEndColumn) {
        ...
        if (tableName == null) {
          ...
          PSDtdRelationalMapper.TableDef leftTab = m_dtdMapper.getTable(tableName);
          PSDtdRelationalMapper.TableDef rightTab = m_dtdMapper.getTable(rightTableName);
          ...
          if (leftTab.getName().equals(rightTab.getName())) {
            ...
            if (leftTab.dependsOn(rightTab)) {
              ...
              throw new UncheckedSQLException(
                  "Cannot relate table " + leftTab.getName() + " with " + rightTab.getName());
            }
      ...
      if (tableName == null) {
        ...
        if (tabOrd < 1) throw new UncheckedSQLException("Unknown table: " + tableName);
        ...
        for (int i = highTableOrdinal + 1; i <= lowTableOrdinal; i++) // 1-based
        {
          ...
          for (int j = 1; j <= tab.getNumColumns(); j++) // 1-based
          {
            ...
            if (parentCol != null) {
              ...
              parentJoinCol = parentTab.getBackEndColumn(parentCol.getOrdinal());
    ...
    public class ConditionalApplier implements IPSExecutionStep {
      ...
      public void execute(PSExecutionData data) throws java.sql.SQLException {
        ...
        try {
          ...
          PSRowDataBuffer rowData = new PSRowDataBuffer(in);
