...
public class PSXmlDriver extends PSFileSystemDriver {
   ...
   * @param info
   * @exception SQLException if this is an appropriate URL for this driver, but the XML file or
  ...
  public java.sql.Connection connect(java.lang.String url, java.util.Properties info)
    ...
    try {
      ...
      if (!url.toLowerCase().startsWith("jdbc:psxml")) return null;
    } catch (Exception e) {
      throw new SQLException(e.toString());
    }

    PSRequest currentReq = (PSRequest) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUEST);
  ...
  private static final String ms_driverName = "Percussion Software XML Driver";

  static {
    try {
      java.sql.DriverManager.registerDriver(new PSXmlDriver());
