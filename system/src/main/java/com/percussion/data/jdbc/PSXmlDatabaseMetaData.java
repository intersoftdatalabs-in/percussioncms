...
public class PSXmlDatabaseMetaData extends PSFileSystemDatabaseMetaData {
  ...
  public PSXmlDatabaseMetaData(PSXmlConnection conn) throws SQLException {
  ...
  public java.sql.ResultSet getTables(
      ...
      throws SQLException {
    if (null == catalog) throw new SQLException("catalog == null");
    ...
    if (types == null) {
      types = new String[] {"HTML", "XML", "DTD", "XSL", "DIRECTORY"};
    }
    ...
    for (int i = 0; i < types.length; i++) {
      ...
      if (type.equalsIgnoreCase("html")) {
        if (filter == null) filter = new PSFileFilter();
        ...
        if (dirFilter == null) dirFilter = new PSFileFilter();
    ...
    if (filter != null) filter.setAllowableAttributes(PSFileFilter.IS_FILE);

    if (dirFilter != null) dirFilter.setAllowableAttributes(PSFileFilter.IS_DIRECTORY);

    // bind the catalog directory to a real physical path
    File catalogDir = null;
    try {
      catalogDir =
          PSXmlDriver.getPhysicalPath(
              catalog, // the catalog
    ...
    if (!catalogDir.isDirectory()) {
      ...
      File[] files = null;
      if (filter != null) files = catalogDir.listFiles((java.io.FileFilter) filter);

      File[] dirs = null;
      if (dirFilter != null) dirs = catalogDir.listFiles((java.io.FileFilter) dirFilter);

      int filesSize = 0;
      int dirsSize = 0;
      if (files != null) filesSize = files.length;
      if (dirs != null) dirsSize = dirs.length;
    ...
    for (int i = 0; i < finalList.length; i++) {
      ...
      String type;
      if (periodPos == name.length() - 1 || -1 == periodPos) type = "";
      else {
  ...
  public java.sql.ResultSet getColumns(
      ...
      throws SQLException {
    if (null == tableNamePattern) tableNamePattern = "%";
    ...
    try {
      ...
      // get the column names
        ...
        // pattern
        PSPatternMatcher patMat = PSPatternMatcher.SQLPatternMatcher(tableNamePattern);
