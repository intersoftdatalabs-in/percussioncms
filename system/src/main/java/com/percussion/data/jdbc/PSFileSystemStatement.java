...
public class PSFileSystemStatement implements Statement {
  ...
  protected SimpleNode parse(String sql) throws ParseException {
    ...
    if (m_parser == null) m_parser = new SQLParser(stream);
    else m_parser.ReInit(stream);
  ...
  private void doFileQuery(Node node) throws java.io.IOException, SQLException {
    ...
    ASTDirectSQLDataStatement statement = (ASTDirectSQLDataStatement) root.jjtGetChild(0);

    ASTDirectSelectStatementMultipleRows select =
        (ASTDirectSelectStatementMultipleRows) statement.jjtGetChild(0);

    ASTQuerySpecification query = (ASTQuerySpecification) select.jjtGetChild(0);

    ASTSelectList selectList = (ASTSelectList) query.jjtGetChild(0);
    ...
    for (int i = 0; i < columnIndexes.length; i++) {
      ...
      m_metaData.addColumnMetaData(
          new PSResultSetColumnMetaData(colName, getColumnType(colConst), getColumnSize(colConst)));
    }

    ASTTableExpression tableEx = (ASTTableExpression) query.jjtGetChild(1);
    ...
    ASTFromClause from = (ASTFromClause) tableEx.jjtGetChild(0);
    ...
    FileList fileList = new FileList(files, null, true, fileSpec.isRecursive(), columnIndexes);
  ...
  private static int getColumnConstant(String name) throws SQLException {
    ...
    } else throw new SQLException("Column name invalid: " + name);
  ...
  class FileList {
    ...
    public FileList(
      ...
      for (int i = 0; i < m_fileColumns.length; i++) m_fileColumns[i].trimToSize();
    }

    private void doList(File rootDir, FilenameFilter fileFilter, boolean addDirs, boolean recursive)
        throws java.io.IOException {
      File[] files = rootDir.listFiles(fileFilter);
      for (int i = 0; i < files.length; i++) {
        if (files[i].isFile()) addFileData(files[i]);
        else if (files[i].isDirectory()) {
          if (addDirs) addFileData(files[i]);
          if (recursive) doList(files[i], fileFilter, addDirs, recursive);
        }
      }
    }
      ...
      for (int i = 0; i < m_columnConstants.length; i++) {
        switch (m_columnConstants[i]) {
          ...
          case FileList.MODIFIED:
            ...
            buf = m_df.format(new Date(f.lastModified()), buf, new java.text.FieldPosition(0));
