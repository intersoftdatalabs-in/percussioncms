...
public class PSFileSystemDriver extends PSJdbcDriver {
  ...
  public java.sql.Connection connect(java.lang.String url, java.util.Properties info)
    ...
    try {
      ...
      if (!url.toLowerCase().startsWith("jdbc:psfilesystem")) return null;
    } catch (Exception e) {
      throw new SQLException(e.toString());
    }
    PSRequest currentReq =
  ...
  public static void addVirtualDirectory(IPSVirtualDirectory vdir) {
    printMsg(
        "Adding virtual directory: "
            + vdir.getVirtualDirectory()
            + " -> "
            + vdir.getPhysicalPath(new File(".")).toString());
  ...
  static File getPhysicalPath(String catalog, PSUserSession session, int requiredAccess)
    ...
    if (catalog == null) throw new IllegalArgumentException("vfs convert path error" + catalog);
  ...
  private static final java.lang.String ms_driverName = "Percussion Software File System Driver";

  static {
    try {
      java.sql.DriverManager.registerDriver(
          new PSFileSystemDriver(ms_driverName, ms_majorVersion, ms_minorVersion));
