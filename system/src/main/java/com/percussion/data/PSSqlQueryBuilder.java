...
public class PSSqlQueryBuilder extends PSSqlBuilder {
  ...
  private void buildMultiTableFrom(
    ...
    for (int j = 0; j < joinCount; j++) {
      ...
      if (curJoin.getTranslator() != null) {
        throw new IllegalArgumentException(
            "sql builder xlator unsupported in homogeneous"
                + lTable.getAlias()
                + " "
                + rTable.getAlias());
      }
      ...
      if (curJoin.isInnerJoin()) {
        addInnerJoinCondition(login, context, dtHash, knownTables, curJoin);
        continue;
      }
  ...
  private void buildSelectColumnList(PSSqlBuilderContext context) {
    ...
    if (size == 0) {
      if (m_isUnique) context.addText("SELECT DISTINCT * ");
      else context.addText("SELECT * ");
      return;
    }
  ...
  private List<PSSortedColumn> getSortedColumnsSelectList() {
    ...
    while (it.hasNext()) {
      ...
      while (selCols.hasNext() && !found) {
        ...
        if (bakCol.isSameColumn(col)) found = true;
      }
      if (!found) sortedCols.add(col);
    }
    return sortedCols;
  }
  ...
  private void addSelectColumnToText(
    ...
    for (int i = 0; i < size; i++) {
      ...
      String alias = col.getAlias();
      if ((alias != null) && (alias.length() > 0) && !alias.equals(col.getColumn())) {
        if (supportsColumnAliases) {
          ...
          throw new IllegalArgumentException(
              "sql builder alias unsupported" + t.getDataSource() == null
                  ? ""
                  : t.getDataSource() + " " + t.getTable() + " " + col.getColumn());
        }
      }
    }
  }
  ...
  private void addJoinsToWhereClause(PSSqlBuilderContext context) {
    ...
    if (!m_joinFormatter.usesOnClauseInFrom()) context.newGroupBlock(true, " AND ");
    else
      for (int j = 0; j < joinCount; j++) {
    ...
    for (int j = 0; j < joinCount; j++) {
      ...
      if (m_joinFormatter.usesOnClauseInFrom() && !curJoin.isInnerJoin()) continue;

      PSBackEndColumn lCol = curJoin.getLeftColumn();
      PSBackEndColumn rCol = curJoin.getRightColumn();

      if (needAnd) context.addText(" AND ");
      else needAnd = true;
  ...
  protected void buildWhereClauses(
    ...
    if (m_Wheres == null) return;

    final int size = m_Wheres.size();
    final int joinCount = m_Joins.size();

    if ((size == 0)
        && ((joinCount == 0) || (m_joinFormatter.usesOnClauseInFrom() && !hasInnerJoins()))) {
    ...
    if (joinCount != 0) addJoinsToWhereClause(context);
      ...
      if (replValue instanceof PSBackEndColumn) {
        ...
        if (isIn) {
          ...
          if (litText.toUpperCase().indexOf(" AND ") == -1) {
            ...
            throw new IllegalArgumentException(
                "sql builder litset reqd for op" + opCode + " " + litText);
          }
        ...
        if (isBetween && (literalCount != 2)) {
          throw new IllegalArgumentException(
              "sql builder litset wrong size" + opCode + 2 + " " + literalCount);
        ...
        for (int l = 0; l < literalCount; l++) {
          if (l > 0) {
            ...
            if (isBetween) context.addText(" AND ");
  ...
  protected void addLiteralText(PSSqlBuilderContext context, PSLiteral replValue, int jdbcType) {
    ...
    if (replValue instanceof PSDateLiteral) {
      ...
      if (jdbcType == Types.DATE) {
        o = new java.sql.Date(((PSDateLiteral) replValue).getDate().getTime());
        typeString = "d";
      } else if (jdbcType == Types.TIME) {
        o = new java.sql.Time(((PSDateLiteral) replValue).getDate().getTime());
        typeString = "t";
      } else // assume it's Types.TIMESTAMP
      {
        o = new java.sql.Timestamp(((PSDateLiteral) replValue).getDate().getTime());
        typeString = "ts";
      }
      ...
      if (strText.startsWith("'") && strText.endsWith("'")) {
        ...
        for (int pos = strText.indexOf('\''); ; pos = strText.indexOf('\'', lastPos)) {
          if (pos == -1) {
            if (lastPos == 0) context.addText(strText);
            else context.addText(strText.substring(lastPos));
            break;
          }
