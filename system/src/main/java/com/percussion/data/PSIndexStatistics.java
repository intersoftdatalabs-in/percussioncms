...
public class PSIndexStatistics {
  private PSIndexStatistics(
      PSBackEndTable table, String indexName, short indexType, boolean isUnique, int cardinality) {
    super();
    m_table = table;
  ...
  public String[] getSortedColumns() {
    if (m_columns == null) return new String[0];
   ...
   */
  public static boolean isTableView(PSBackEndTable table, DatabaseMetaData md) throws SQLException {

    if (null != table) {
      PSMetaDataCache.loadConnectionDetail(table);
      ResultSet rs =
          md.getTables(
              table.getConnectionDetail().getDatabase(),
  ...
  public static PSIndexStatistics[] getStatistics(PSBackEndTable table, DatabaseMetaData md)
    ...
    try {
      ...
      if (isTableView(table, md)) return null;
      ...
      rs = md.getIndexInfo(databaseName, schemaName, tableName, false, false);
      ...
      while (rs.next()) {
        ...
        if (!indexName.equals(lastIndexName) || (indexType != lastIndexType)) {
          stats = new PSIndexStatistics(table, indexName, indexType, isUnique, cardinality);
        ...
        if (colName != null) stats.m_columns.add(colName);
      }
    } finally {
      try {
        if (rs != null) rs.close();
