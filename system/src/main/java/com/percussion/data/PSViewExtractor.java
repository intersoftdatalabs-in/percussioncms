...
/**
 * This class allows the {@link PSViewEvaluator} to be used as a data extractor for a particular
 * pageId.
 */
public class PSViewExtractor implements IPSDataExtractor {
   ...
   *
   * @param viewEvaluator The view evaluator constructed for the set of views that are being used.
  ...
  public PSViewExtractor(PSViewEvaluator viewEvaluator, int pageId) {
    if (viewEvaluator == null) throw new IllegalArgumentException("viewEvaluator may not be null");

    m_viewEval = viewEvaluator;
    m_pageId = pageId;
  }

  /** Convenience method calls {@link #extract(PSExecutionData, Object) extract(data, null)}. */
  public Object extract(PSExecutionData data) throws PSDataExtractionException {
    return extract(data, null);
  }

  /**
   * Extracts the next view to use by calling {@link PSViewEvaluator#getNextView(PSExecutionData,
   * int)} using the supplied <code>data</code> and the <code>pageId</code> supplied during
   * construction. Since this method is currently documented as never returning <code>null</code> or
   * empty, the <code>defValue</code> parameter will not ever be utilized.
   *
   * @param data The execution data associated with this request.
   * @param defValue The default value to use if a value is not found. Currently has no effect.
  ...
  /**
   * The pageId to use when retrieving the next view. Supplied during construction, immutable after
   * that.
   */
  private int m_pageId;

  /**
   * The view evaluator to use to retrieve the next view. Intialized during construction, never
