...
public class PSSqlUpdateBuilderFactory {
  ...
  public static PSSqlUpdateBuilder getSqlUpdateBuilder(
    ...
    if (table == null) throw new IllegalArgumentException("Table must be supplied");

    try {
      if (type == PSUpdateOptimizer.PLAN_TYPE_UPDATE
          || type == PSUpdateOptimizer.PLAN_TYPE_INSERT) {
        if (login != null) {
          String driver = PSMetaDataCache.getConnectionDetail(login).getDriver();
          if (driver.toUpperCase().indexOf("ORACLE") > -1) {
            // Return an Oracle-specific builder!
            if (type == PSUpdateOptimizer.PLAN_TYPE_UPDATE)
              if (allowInserts) builder = new PSOracleUpdateInsertBuilder(table);
              else builder = new PSOracleUpdateBuilder(table);
            else // type is insert
      ...
      if (builder == null) {
        if (type == PSUpdateOptimizer.PLAN_TYPE_UPDATE)
          if (allowInserts) return new PSSqlUpdateInsertBuilder(table);
          else return new PSSqlUpdateBuilder(table);
        else if (type == PSUpdateOptimizer.PLAN_TYPE_INSERT) return new PSSqlInsertBuilder(table);
        else
