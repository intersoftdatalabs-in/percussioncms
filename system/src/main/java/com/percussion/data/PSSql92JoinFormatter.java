...
public class PSSql92JoinFormatter extends PSJoinFormatter {
  ...
  public void addRightTablePrefix(
      PSSqlBuilderContext context, PSBackEndJoin join, boolean inverseColumnOrder) {
    if (join.isFullOuterJoin()) context.addText(" FULL OUTER JOIN ");
    else if (inverseColumnOrder) {
      if (join.isLeftOuterJoin()) context.addText(" RIGHT OUTER JOIN ");
      else if (join.isRightOuterJoin()) context.addText(" LEFT OUTER JOIN ");
      else context.addText(", ");
    } else {
      if (join.isLeftOuterJoin()) context.addText(" LEFT OUTER JOIN ");
      else if (join.isRightOuterJoin()) context.addText(" RIGHT OUTER JOIN ");
      else context.addText(", ");
    }
  }
  ...
  public String getRightColumnPrefix(int joinType) {
    return "";
  }

  public String getLeftColumnPrefix(int joinType) {
    return "";
  }

  public String getRightColumnSuffix(int joinType) {
    return "";
  }

  public String getLeftColumnSuffix(int joinType) {
    return "";
  }

  public String getLeftTablePrefix(int joinType) {
    return "";
  }

  public String getRightTablePrefix(int joinType) {
    if (joinType == JOIN_TYPE_FULL_OUTER) return " FULL OUTER JOIN ";
    else if (joinType == JOIN_TYPE_LEFT_OUTER) return " LEFT OUTER JOIN ";
    else if (joinType == JOIN_TYPE_RIGHT_OUTER) return " RIGHT OUTER JOIN ";
    else return ", ";
  }

  public String getLeftTableSuffix(int joinType) {
    return "";
  }

  public String getRightTableSuffix(int joinType) {
    return "";
  }
}
