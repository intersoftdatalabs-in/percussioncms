...
/**
 * The PSQueryStatement class defines a query (SELECT) statement which can be executed as part of a
...
public class PSQueryStatement extends PSStatement {
  /**
   * Construct a query statement which can be executed as part of the query execution plan. The
   ...
   * @param cursorName the cursor name to use when performing a positioned UPDATE based on this
   *     SELECT
   */
  public PSQueryStatement(int connKey, IPSStatementBlock[] blocks, String cursorName)
   ...
   * @param blocks the statement blocks comprising this statement
   */
  public PSQueryStatement(int connKey, IPSStatementBlock[] blocks)
      throws com.percussion.data.PSDataExtractionException {
    this(connKey, blocks, null);
  }

  /**
   * Determines whether a static sql can be constructed from the specified blocks. If <code>blocks
   * </code> is <code>null</code> or empty array then <code>true</code> is returned.
   *
   * @param blocks statement blocks, may be <code>null</code> or empty array
   * @return <code>true</code> if a static sql can be constructed from the specified blocks, <code>
  ...
  /**
   * Execute the query statement as a step in the execution plan. The result set generated by
   * executing this query will be added to the execution data.
   *
   * <p>The following steps are performed during execution:
   *
   * <ol>
   *   <li>PSQueryHandler filled in the statement before calling execute. We can now check the cache
   *       to see if the statement already exists in the cache (if caching is enabled for this
   *       pipe).
   *   <li>if the statement was in cache, we can add its results to the execution data and return
   *   <li>otherwise, we need to execute the statement through the db pool and add its results to
   *       the execution data
   *   <li>if caching is enabled, we also need to add its results to the cache
   * </ol>
   *
  ...
  public void execute(PSExecutionData data)
    ...
    try {
      ...
      String sqlString;
      if (m_storeSqlStatement) sqlString = m_sqlString;
      else sqlString = buildSqlString(data, true);
      ...
      if (m_cursorName != null) stmt.setCursorName(m_cursorName);
      ...
      try {
        ...
        // as an error
        if (!PSSqlException.hasFeatureNotSupported(se, connDetail.getDriver())) {
          throw se;
        }
      }
