...
/**
 * The PSUdfCallExtractor class is used to extract data from various sources, which are then
 * massaged through the UDF call.
 *
 * <p>Within an application, users may define UDFs. They can then define many instances of the UDF,
 * each taking its own set of parameters. This class has been defined to provide the components of
 * E2 an easy access mechanism into the UDFs. The {@link
 * com.percussion.design.objectstore.PSExtensionCall PSExtensionCall} object defined in the
 * application is used to create an instance of this class. It then builds the appropriate extension
 * handler. When the UDF is executed, it loads the parameters from the appropriate places (XML, CGI
 * vars, etc.). It then returns the result the UDF supplies.
 *
...
public class PSUdfCallExtractor extends PSDataExtractor {
  ...
  public PSUdfCallExtractor(PSExtensionCall call) throws PSExtensionException, PSNotFoundException {
    ...
    m_udfProcessor =
        (IPSUdfProcessor)
            PSServer.getExtensionManager(null).prepareExtension(call.getExtensionRef(), null);
  ...
  /**
   * Set the base from which any XML field sources used by this extractor will be extracted.
   *
