...
public class PSCssStyleSheetMerger extends PSStyleSheetMerger {
  ...
  public void merge(
    ...
    String sURL;
    if (!styleFile.getProtocol().equalsIgnoreCase("file")) sURL = styleFile.toExternalForm();
    else {
      sURL = styleFile.getFile();
      if (sURL == null) /* some FILE URLs use the file as host ?! */ sURL = styleFile.getHost();

      sURL = com.percussion.server.PSServer.makeRequestRoot(sURL);
    }

    try {
      BufferedWriter buf = new BufferedWriter(new OutputStreamWriter(out, PSCharSets.rxJavaEnc()));
