...
public class PSOracleUpdateBuilder extends PSSqlUpdateBuilder {
  ...
  protected boolean buildColumnAndPlaceholderList(
    ...
    return super.buildColumnAndPlaceholderList(
        context,
        table,
        dataTypes,
        columnList,
        delimiter,
        ignoreAutoIncrements,
        m_lobColumnInitializer);
  }
  ...
  protected void buildColumnList(
    ...
    super.buildColumnList(
        context, table, datatypes, usePlaceHolder, columns, m_lobColumnInitializer);
  }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (jdbcType != null) {
        ...
        if (colType == java.sql.Types.BLOB || colType == java.sql.Types.CLOB) {
          if (colCount > 0) context.addText(delimiter);
  ...
  PSSqlBuilderContext getRowRetrievalByRowidContext(
    ...
    PSSqlBuilderContext selectRowForLobUpdateContext = new PSSqlBuilderContext();

    selectRowForLobUpdateContext.addText("SELECT ");

    // build column list of lob columns...
    buildLobColumnList(selectRowForLobUpdateContext, table, dtHash, m_Columns, ", ");
  ...
  PSSqlBuilderContext getInsertContext(
    ...
    buildColumnList(context, table, dtHash, false, /* use colname */ columnList);

    context.addText(") VALUES (");

    buildColumnList(context, table, dtHash, true, /* use placeholder */ columnList);
  ...
  PSUpdateStatement generate(
    ...
    if (m_lobColumnInitializer == null) {
      /* Normal Update */
      return super.generateUpdate(dtHash, iConnKey, login);
    }
