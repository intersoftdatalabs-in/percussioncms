 ...
 */
public class PSFunctionBlock extends PSStatementBlock implements IPSStatementBlock {
  /**
  ...
  public boolean hasStaticSql() {
    ...
    for (int i = 0; (i < m_blocks.size()) && staticSql; i++) {
      ...
      if (cur instanceof PSFunctionCall) staticSql = ((PSFunctionCall) cur).hasStaticParamsOnly();
    }
    return staticSql;
  }
  ...
  private void addDynamicBindParams(PSFunctionCall funcCall) {
    ...
    while (itDef.hasNext() && itVal.hasNext()) {
      ...
      if (!paramDef.isStaticBind()) {
        ...
        PSStatementColumn stmtCol =
            new PSStatementColumn(paramVal.getValue(), jdbcType, null, null);
  ...
  public void buildStatement(StringBuilder buf, PSExecutionData data)
    ...
    for (int i = 0; i < m_blocks.size(); i++) {
      ...
      if (cur instanceof PSStatementColumn)
        ...
        String res = getFunctionCallReplacementText((PSFunctionCall) cur, data);
  ...
  private String getFunctionCallReplacementText(PSFunctionCall funcCall, PSExecutionData data)
    ...
    PSFunctionCallExtractor funcCallEx = new PSFunctionCallExtractor(funcCall);
