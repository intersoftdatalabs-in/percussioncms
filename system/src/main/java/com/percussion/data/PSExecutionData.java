...
public class PSExecutionData implements AutoCloseable {
  ...
  public PSLogHandler getLogHandler() {
    if (m_LogHandler == null) {
      if (m_AppHandler != null) m_LogHandler = m_AppHandler.getLogHandler();

      if (m_LogHandler == null) m_LogHandler = PSServer.getLogHandler();
    }
    return m_LogHandler;
  }
  ...
  public PSErrorHandler getErrorHandler() {
    if (m_ErrorHandler == null) {
      if (m_AppHandler != null) m_ErrorHandler = m_AppHandler.getErrorHandler();

      if (m_ErrorHandler == null) m_ErrorHandler = PSServer.getErrorHandler();
    }
   ...
   */
  public void restoreResultSetContext() throws IllegalStateException {
    if (null == m_rsContext) throw new IllegalStateException("No context to restore.");
  ...
  public boolean readRow() throws IllegalStateException, SQLException {
    if (null == m_rowBuffer) throw new IllegalStateException("No row buffer available.");
  ...
  public int getFirstRowNumber() {
    int pagerFirstRow = 0;
    String firstIndex =
        m_Request.getParameter(PSResultSetXmlConverter.FIRST_QUERY_INDEX_PARAMETER_NAME);
  ...
  public int getMaxRows() {
    ...
    if (maxPerPage <= 0) return -1;

    int maxForwardLinks = pager.getMaxPageLinks();
    if (maxForwardLinks <= 0) return -1;
  ...
  public void setCurrentRelationship(PSRelationship currentRel) {
    if (currentRel == null) throw new IllegalArgumentException("currentRel must not be empty");
