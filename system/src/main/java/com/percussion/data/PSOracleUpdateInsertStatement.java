...
public class PSOracleUpdateInsertStatement extends PSOracleUpdateStatement {
  ...
  public PSOracleUpdateInsertStatement(
    ...
    super(connKey, updateBlocks, lobUpdateBlocks, rowIdQueryBlocks, TYPE_UPDATE);

    m_insertStatement =
        new PSOracleUpdateStatement(connKey, insertBlocks, lobUpdateBlocks, null, TYPE_INSERT);
  }
  ...
  public void execute(PSExecutionData data)
    ...
    if (rowsUpdated == stats.getRowsUpdated()) {
      ...
      if ((level & PSAclEntry.AACE_DATA_CREATE)
          == PSAclEntry.AACE_DATA_CREATE) { // no new updates and we're allowed to do inserts.
