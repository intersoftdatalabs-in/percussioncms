...
/**
 * A ResultSet provides access to a table of data. A ResultSet object is usually generated by
 ...
 *
 * <p>The getXXX methods retrieve column values for the current row. You can retrieve values using
 * either the index number of the column or the name of the column. In general, using the column
 * index will be more efficient. Columns are numbered from 1.
 *
 * <p>For maximum portability, ResultSet columns within each row should be read in left-to-right
 * order and each column should be read only once.
 *
 * <p>For the getXXX methods, the JDBC driver attempts to convert the underlying data to the
 * specified Java type and returns a suitable Java value. See the JDBC specification for allowable
 * mappings from SQL types to Java types with the ResultSet.getXXX methods.
 *
 * <p>Column names used as input to getXXX methods are case insensitive. When performing a getXXX
 * using a column name, if several columns have the same name, then the value of the first matching
 * column will be returned. The column name option is designed to be used when column names are used
 * in the SQL query. For columns that are NOT explicitly named in the query, it is best to use
 * column numbers. If column names are used, there is no way for the programmer to guarantee that
 * they actually refer to the intended columns.
 *
 * <p>A ResultSet is automatically closed by the Statement that generated it when that Statement is
 * closed, re-executed, or used to retrieve the next result from a sequence of multiple results.
 *
 * <p>The number, types and properties of a ResultSet's columns are provided by the ResulSetMetaData
 * object returned by the getMetaData method.
 *
 * <p><B>Note:</B> This wrapper has not been created with synchronization issues in mind, to force
 * thread safety, you should synchronize on this object.
 *
...
public class PSResultSetWrapper implements ResultSet {
  ...
  /**
   * The Connection associated with this result set. (Created when constructed, can be <code>null
   * </code>). Will be closed along with this result set.
   */
  private Connection m_conn = null;

  /**
   * The Statement associated with this result set. (Created when constructed, can be <code>null
   * </code>). Will be closed along with this result set.
   */
  ...
  /**
   * Create an instance of the ResultSetWrapper. This is used to facilitate statement and connection
   * closing by mimicking a standard result set, but overriding the <code>close()</code> method to
   * close the associated statement and connection, if they are supplied.
   *
   * @param rs The result set on which to act using the ResultSet implementation. Can't be <code>
   *     null</code>. Can't be a subclass of this wrapper!
   * @param stmt The statement to close when through with this result set. Can be <code>null</code>.
   * @param conn The connection to close when through with this result set. Can be <code>null</code>
   *     .
  ...
  /**
   * Check to ensure the wrapped resultset is available. All of the ResultSet implementation calls
   * will call this method first, to validate that the result set has not been previously closed.
   *
   * @throws SQLException if the resultset has been closed (set to <code>null</code>
   */
  private void checkClosed() throws SQLException {
    if (m_rs == null) throw new SQLException("Operation not allowed, ResultSet has been closed!");
  }
   ...
   *
   * <p><B>Note:</B> This wrapper class allows the close method to also close the connection and
   * statement that can be optionally supplied to the constructor.
   *
   * <p><B>Note:</B> A ResultSet is automatically closed by the Statement that generated it when
   * that Statement is closed, re-executed, or is used to retrieve the next result from a sequence
   * of multiple results. A ResultSet is also automatically closed when it is garbage collected.
   *
