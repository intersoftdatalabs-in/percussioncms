...
public class PSBackEndColumnExtractor extends PSDataExtractor {
  ...
  public PSBackEndColumnExtractor(com.percussion.design.objectstore.PSBackEndColumn source) {
    ...
    if (alias != null && alias.length() > 0) m_columnName = alias;
    m_columnIndex = -1;
  }
  ...
  public Object extract(PSExecutionData data, Object defValue) throws PSDataExtractionException {
    ...
    if (data.forceNullRowForNullResultSet()) return defValue;
    ...
    try {
      if (m_columnIndex >= 0 && m_columnIndex < rowData.length) value = rowData[m_columnIndex];
      else
        throw new IllegalStateException(
            "m_columnIndex ("
                + m_columnIndex
                + ") is out of the range of rowData["
                + rowData.length
                + "]");
    } catch (Throwable t) { // most likely the index was not found (out of bounds)
