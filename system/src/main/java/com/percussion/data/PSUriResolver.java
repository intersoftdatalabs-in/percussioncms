...
public class PSUriResolver implements URIResolver {
  ...
  public Source resolve(String href, String base) throws TransformerException {
    ...
    if (PSServer.isCaseSensitiveURL()) {
      if (href.startsWith(serverRoot)) return new StreamSource(FILE_PROTOCOL + postFix);
    } else {
      String hrefRoot = href.substring(0, serverRoot.length());
      if (hrefRoot.equalsIgnoreCase(serverRoot)) return new StreamSource(FILE_PROTOCOL + postFix);
    }
