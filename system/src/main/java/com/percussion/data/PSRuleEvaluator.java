...
public class PSRuleEvaluator extends PSConditionalEvaluator {
   ...
   */
  public PSRuleEvaluator(PSRule rule) throws PSNotFoundException, PSExtensionException {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (extensionImplementsInterface(ref, interfaceName)) {
          ...
          if (interfaceName == null)
            throw new IllegalArgumentException("Interface name must be supplied.");
          else
            throw new IllegalArgumentException(
                ref.getExtensionName() + " does not implement " + interfaceName);
        }
      }
  ...
  public boolean isMatch(PSExecutionData data) {
    if (m_extensions != null) {
      try {
        ...
        throw new PSEvaluationException(extractE.getErrorCode(), extractE.getErrorArguments());
      } catch (PSConversionException convE) {
        throw new PSEvaluationException(convE.getErrorCode(), convE.getErrorArguments());
      }
    } else {
