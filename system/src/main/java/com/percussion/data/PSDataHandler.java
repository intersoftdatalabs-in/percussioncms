...
public abstract class PSDataHandler implements IPSRequestHandler, IPSInternalRequestHandler {
  ...
  protected PSDataHandler(PSApplicationHandler app, PSDataSet ds)
    ...
    if (null != requestor) m_validationRules = requestor.getValidationRules();

    if ((m_validationRules == null) || (m_validationRules.size() == 0)) {
      m_validationRules = null;
      m_validationHandler = null;
    } else m_validationHandler = new PSConditionalEvaluator(m_validationRules);
  ...
  public void runPreProcessingExtensions(PSExecutionData data, Iterator<? extends PSExtensionRunner> extensionRunners)
    ...
    if (extensionRunners.hasNext()) {
      ...
      try {
        ...
        if (dh.isTraceEnabled(PSTraceMessageFactory.POST_PREPROC_HTTP_VAR_FLAG)) {
          dh.printTrace(PSTraceMessageFactory.POST_PREPROC_HTTP_VAR_FLAG, data.getRequest());
        }
        ...
        PSRequestPreProcessingError err =
            new PSRequestPreProcessingError(
                null, // ip address?
                e.getErrorCode(),
                e.getErrorArguments());
  ...
  public Document runPostProcessingExtensions(PSExecutionData data, Document doc)
    ...
    Document retDoc = runPostProcessingExtensions(data, doc, m_preparedPostProcExts.iterator());
    return retDoc;
  }
  ...
  public void validate(PSExecutionData data) throws PSErrorException {
    ...
    if (!isMatch) {
      ...
      PSRequestValidationException exc = new PSRequestValidationException(m_validationRules);
  ...
  protected final void setPreProcExits(PSExtensionCallSet exits)
    ...
    loadExtensions(
        m_appHandler, exits, IPSRequestPreProcessor.class.getName(), m_preparedPreProcExts);
  }
  ...
  protected final void setResultDocExits(PSExtensionCallSet exits)
    ...
    loadExtensions(
        m_appHandler, exits, IPSResultDocumentProcessor.class.getName(), m_preparedPostProcExts);
  }
  ...
  protected void reportError(PSRequest request, Throwable th) {
    ...
    for (String pname : pmap.keySet()) {
      ...
      } else params.append(value == null ? "" : value.toString());
      firstParam = false;
    }
    String message =
