...
public class PSFieldValidationRulesEvaluator {
  ...
  public boolean isValid(Integer pageId, PSExecutionData data, PSErrorCollector errorCollector) {
    ...
    try {
      if (evaluateApplyWhen(data)) {
        ...
        if (!isValid) errorCollector.add(pageId, this);
  ...
  public boolean isValidOccurrence(
      ...
      String lang) {
    if (data == null || pageId == null || page == null || errorCollector == null)
      throw new IllegalArgumentException("parameters cannot be null");
    ...
    if (null != m_uiSet.getLabel()) label = m_uiSet.getLabel().getText();
        ...
        if (valueCount > 0) {
          ...
          try {
            ...
            if (irh instanceof IPSInternalCommandRequestHandler) {
              ...
              Map htmlParams = PSHtmlParameters.createStandardParams(backupHtmlParams);
              htmlParams.put(IPSConstants.SUBMITNAME_PARAM_NAME, m_field.getSubmitName());
              request.setParameters((HashMap) htmlParams);

              Document doc = rh.makeInternalRequest(request, PSBinaryCommandHandler.COMMAND_NAME);
  ...
  private boolean evaluateApplyWhen(PSExecutionData data)
    ...
    if (applyWhen == null) return true;

    // check field empty rule
    if (!applyWhen.ifFieldEmpty()) {
      if (getValueCount(m_field.getSubmitName(), data) == 0) return false;
    }

    PSRuleListEvaluator eval = new PSRuleListEvaluator(applyWhen.iterator());
  ...
  private int getValueCount(String paramName, PSExecutionData data) {
    PSHtmlParameterExtractor extractor =
        new PSHtmlParameterExtractor(new PSHtmlParameter(m_field.getSubmitName()));

    Object o = extractor.extract(data);
    if (o instanceof String) {
      String param = (String) o;
      if (param == null || param.trim().length() == 0) return 0;
  ...
  private int getValueCount(String paramName, Document page) {
    ...
    for (int i = 0; i < controls.getLength(); i++) {
      ...
      String pn = control.getAttribute(PSDisplayFieldElementBuilder.PARAMNAME_NAME);
        ...
        if (dimension.equals(PSDisplayFieldElementBuilder.DIMENSION_SINGLE)) {
          ...
          for (int j = 0; nl != null && j < nl.getLength(); j++) {
            if (nl.item(j).getNodeType() == Node.ELEMENT_NODE
                && nl.item(j).getNodeName().equals(PSDisplayFieldElementBuilder.DATA_NAME)) {
              hasValue = true;
              break;
            }
          }
          ...
          values = control.getElementsByTagName(PSChoiceBuilder.DISPLAYENTRY_NAME);
          if (values != null) {
            for (int j = 0; j < values.getLength(); j++) {
              Element value = (Element) values.item(j);
              String selected = value.getAttribute(PSChoiceBuilder.SELECTED_ATTRIBUTE_NAME);
              if (selected.equals(PSChoiceBuilder.ATTRIB_BOOLEAN_TRUE)) counter++;
            }
          }
        } else if (dimension.equals(PSDisplayFieldElementBuilder.DIMENSION_TABLE)) {
          values = control.getElementsByTagName(PSTableValueBuilder.ROWSET_NAME);

          if (values != null) counter += values.getLength();
        }
      }
    }
