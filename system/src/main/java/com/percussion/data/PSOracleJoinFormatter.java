...
public class PSOracleJoinFormatter extends PSJoinFormatter {
  ...
  public void addLeftColumnSuffix(
    ...
    if (join.isFullOuterJoin()
        ...
        || (!inverseColumnOrder && join.isRightOuterJoin())) context.addText(" (+) ");
  ...
  public String getRightColumnSuffix(int joinType) {
    // if we're doing a left or full outer join, we need to add the marker
    if (joinType == JOIN_TYPE_FULL_OUTER || joinType == JOIN_TYPE_LEFT_OUTER) return " (+) ";
  ...
  public String getLeftColumnSuffix(int joinType) {
    // if we're doing a right or full outer join, we need to add the marker
    if (joinType == JOIN_TYPE_FULL_OUTER || joinType == JOIN_TYPE_RIGHT_OUTER) return " (+) ";
    else return " ";
  }
