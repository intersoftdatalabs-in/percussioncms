...
public class PSTransactionSet implements IPSExecutionStep {
  ...
  public PSTransactionSet(PSApplicationHandler ah, PSDataSet ds)
      throws PSInvalidRequestTypeException, PSIllegalArgumentException, java.sql.SQLException,
          PSSystemValidationException {
    super();

    m_appHandler = ah;

    if (ds.isTransactionForRow()) m_type = XACT_EACH_ROW;
    else if (ds.isTransactionForAllRows()) m_type = XACT_ALL_ROWS;
  ...
  public void execute(PSExecutionData data)
    ...
    if (inDoc != null) {
      ...
      if (rowIterator != null) {
        if ((curNode != null)
            && !rowIterator.equals(
                curNode
                    .getNodeName())) { // if the iterator is not the root node, we need to move to
    ...
    rows. */
    if (null == m_actionTypeXmlField) requestType = m_appHandler.getRequestType(request);
    ...
    for (;
      ...
      for (curExec = 0; curExec < curPlan.length; curExec++) {
        ...
        if (walker != null) walker.setCurrent(curNode); // reset for next block

        // break out of the current execution plan on error
        if (encounteredException != null) break;
    ...
    if (m_type != XACT_NONE) {
      for (int i = 0; i < m_logins.length; i++) {
        try {
          ...
          if (encounteredException == null) PSSqlHelper.commit(conn); // no error, commit
          else {
            ...
            PSDebugLogHandler dh = m_appHandler.getLogHandler();
            if (dh.isTraceEnabled(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG)) {
              Object[] args = {"traceResourceHandler_rollbackAll"};
              dh.printTrace(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG, args);
            }
          }
          conn.setAutoCommit(true); // reset this for other users
        } catch (Exception e) {
          if (encounteredException == null) encounteredException = e;
          else if ((encounteredException instanceof SQLException) && (e instanceof SQLException)) {
            ((SQLException) encounteredException).setNextException((SQLException) e);
          }
    ...
    if (beError != null) throw new PSErrorException(beError);
  }

  private void fixupExecutionPlans(
      PSDataSet ds,
      IPSExecutionStep[] insertPlan,
      IPSExecutionStep[] updatePlan,
      IPSExecutionStep[] deletePlan)
      throws PSInvalidRequestTypeException {
    if ((updatePlan != null) && (updatePlan.length == 0)) updatePlan = null;
    if ((insertPlan != null) && (insertPlan.length == 0)) insertPlan = null;
    if ((deletePlan != null) && (deletePlan.length == 0)) deletePlan = null;

    IPSExecutionStep[] loginSource =
        ((updatePlan != null) ? updatePlan : ((insertPlan != null) ? insertPlan : deletePlan));
    ...
    if (updatePlan != null) m_updatePlan = removeLoginsFromPlan(updatePlan);

    if (insertPlan != null) m_insertPlan = removeLoginsFromPlan(insertPlan);

    if (deletePlan != null) m_deletePlan = removeLoginsFromPlan(deletePlan);
  }

  private int getLoginCount(IPSExecutionStep[] plan) {
    int loginCount = 0;
    for (int i = 0; i < plan.length; i++) {
      if (plan[i] instanceof com.percussion.data.PSBackEndLogin) loginCount++;
    }
  ...
  private IPSExecutionStep[] removeLoginsFromPlan(IPSExecutionStep[] plan) {
    ...
    int index = 0;
    for (int i = 0; i < plan.length; i++) {
      if (!(plan[i] instanceof com.percussion.data.PSBackEndLogin)) retPlan[index++] = plan[i];
    }

    return retPlan;
  }

  private void checkpoint(PSExecutionData data, int rowsProcessed) throws java.sql.SQLException {
    // if no transactions, we've dealt with this through auto-commit
    if (m_type == XACT_NONE) return;

    // one transaction set, the whole set will be handled at the end of
    // data processing (no checkpoint)
    if (m_type == XACT_ALL_ROWS) return;

    // we're either doing it for each row or based on a set of rows
    if ((m_type != XACT_EACH_ROW) && ((rowsProcessed % m_type) != 0))
  ...
  private void commit(PSExecutionData data) throws java.sql.SQLException {
    // if no transactions, we've dealt with this through auto-commit
    if (m_type == XACT_NONE) return;
  ...
  private void rollback(PSExecutionData data) throws java.sql.SQLException {
    // if no transactions, we've dealt with this through auto-commit
    if (m_type == XACT_NONE) return;
  ...
  private PSBackEndUpdateProcessingError recordFailure(
    ...
    PSBackEndUpdateProcessingError newErr =
        new PSBackEndUpdateProcessingError(
            m_appHandler.getId(), sessId, errorCode, errorArgs, sqlSource, curInDocNode);

    // then we'll append the error to our previous error
    if (err != null) err.setNext(newErr);
    else err = newErr;
  ...
  private void prepareDataWalker(PSDataSet ds) throws PSSystemValidationException {
    ...
    if (pageTank != null) m_actionTypeXmlField = pageTank.getActionTypeXmlField();

    /* if we have no action type XML field, we'll use the HTML action
     * field. This is also required to fix bug id TGIS-4BUPSU
     */
    if (!m_isXmlDataSource
        && m_actionTypeXmlField
            == null) { // in this case, the XML action field is really the HTML param
    ...
    if (m_isXmlDataSource) {
      if (m_actionTypeXmlField != null) {
        m_actionTypeXmlField =
            PSXmlTreeWalker.getRelativeFieldName(m_rootElement, m_actionTypeXmlField);
      }
    } else { // in this case, the XML action field is really the HTML param
      PSApplication app = m_appHandler.getApplicationDefinition();
      m_actionTypeXmlField = app.getRequestTypeHtmlParamName();
    }
    ...
    for (int i = 0; i < plan.length; i++) {
      ...
      if (step instanceof PSStatement) {
        ...
        if (tempElements.size() > 0) {
          ...
          String baseElement = PSXmlTreeWalker.getLowestLevelElement(tempElements);
          ...
          if (atPos > 0) {
            ...
            else baseElement = baseElement.substring(0, atPos);
          }
          ...
          for (int j = 0; j < size; j++) {
            ...
            if (extr instanceof PSXmlFieldExtractor) {
              ((PSXmlFieldExtractor) extr).setXmlFieldBase(baseElement);
                ...
                instanceof PSUdfCallExtractor) { // taking UDFs into consideration as they may also
          ...
          if (m_rootElement == null) {
            ...
            m_rootElement = PSXmlTreeWalker.getBaseElement(m_rootElement, baseElement);
          }
        }
      }
    }
  ...
  private void addXmlFieldNames(java.util.List xmlList, IPSDataExtractor extr) {
    ...
    for (int k = 0; k < values.length; k++) {
      ...
      if (rv instanceof PSXmlField) {
        ...
        if (!xmlList.contains(paramName)) xmlList.add(paramName);
      } else if (rv instanceof PSExtensionCall) { // taking UDFs into consideration as they may also
        // contain XML fields (bug id TGIS-4BTW25)
  ...
  private void convertToXmlDataSource(Object obj, PSDataMapping mapping)
    ...
    if (obj instanceof PSHtmlParameter) {
       ...
       */
      mapping.setDocumentMapping(makeHtmlParamXmlField((PSHtmlParameter) obj));
    } else if (obj instanceof PSExtensionCall) {
      ...
      for (int i = 0; i < arrayLength; i++) {
        ...
        if (repValue instanceof PSHtmlParameter) {
           ...
           */
          paramValues[i].setValue(makeHtmlParamXmlField((PSHtmlParameter) repValue));
        } else if (repValue instanceof PSExtensionCall) {
  ...
  private void traceRequestType(String type) {
    PSDebugLogHandler dh = m_appHandler.getLogHandler();
    if (dh.isTraceEnabled(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG)) {
      Object[] args = {type, "traceResourceHandler_reqType"};
      dh.printTrace(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG, args);
    }
  }
