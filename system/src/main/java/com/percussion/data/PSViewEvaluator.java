...
public class PSViewEvaluator {
  ...
  public PSViewEvaluator(PSViewSet viewSet) {
    ...
    while (condViewNames.hasNext()) {
      ...
      if (!evals.isEmpty()) m_viewEvaluators.put(viewName, evals);
    }
  }
  ...
  public String getNextView(PSExecutionData data, int pageId) throws PSDataExtractionException {
    ...
    if (PSModifyCommandHandler.COMMAND_NAME.equals(
            data.getRequest().getParameter(PSContentEditorHandler.COMMAND_PARAM_NAME))
  ...
  private String getCurrentViewName(PSExecutionData data) {
    ...
    String viewName =
        request.getParameter(IPSHtmlParameters.SYS_VIEW, IPSConstants.DEFAULT_VIEW_NAME);
  ...
  private boolean isFieldVisible(String fieldName, Iterator<?> fields) {
    ...
    while (fields.hasNext() && !isVisible) {
      if (fields.next().toString().equalsIgnoreCase(fieldName)) isVisible = true;
    }
