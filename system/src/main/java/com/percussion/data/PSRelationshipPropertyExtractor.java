...
public class PSRelationshipPropertyExtractor extends PSDataExtractor {
  ...
  public Object extract(PSExecutionData data, Object defValue) {
    ...
    final String dependentPrefix = PSRelationshipConfig.XML_ATTR_DEPENDENT + "/";
    ...
    if (name.equalsIgnoreCase(PSRelationshipConfig.XML_ATTR_NAME))
      ...
      if (depLocator == null) return defValue;

      String subName = name.substring(ownerPrefix.length());

      if (subName.equals(IPSHtmlParameters.SYS_CONTENTID)) return "" + depLocator.getId();
      else if (subName.equals(IPSHtmlParameters.SYS_REVISION)) return "" + depLocator.getRevision();
