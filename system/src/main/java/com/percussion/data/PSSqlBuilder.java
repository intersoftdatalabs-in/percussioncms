...
public abstract class PSSqlBuilder {
  ...
  protected static String getTableName(
    ...
    if ((tableName == null) || (tableName.length() == 0)) {
      ...
      if (tableName == null) tableName = builder.getTableOwner(table);
  ...
  protected String getTableOwner(PSBackEndTable table) {
    ...
    if ((tableName == null) || (tableName.length() == 0)) tableName = table.getTable();
    else tableName += '.' + table.getTable();
  ...
  protected String getExpandedColumnName(
    ...
    if (useAlias) tableName = table.getAlias();

    if ((tableName == null) || (tableName.length() == 0)) {
      if (supportsSchemaInName) tableName = getTableOwner(table);
      else tableName = table.getTable();
    }
  ...
  protected void loadDataTypes(
      PSBackEndLogin login, HashMap<String, Integer> dtHash, PSBackEndTable table) {
    try {
