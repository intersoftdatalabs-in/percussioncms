...
public class PSSqlParser {
  ...
  public static IPSExecutionStep[] prepare(int connKey, java.lang.String sql, boolean bIsNative)
    ...
    if (bIsNative) ret = parseNativeStatement(connKey, sql);
    else ret = parseStatement(connKey, sql);
  ...
  public static IPSReplacementValue[] getReplacementValues(String sql) {
    if (sql == null) throw new IllegalArgumentException("sql may not be null");
    ...
    for (start = 0; (end = sql.indexOf(PARAM_PREFIX_CHAR, start)) != -1; ) {
      ...
      if (sql.length() > end + 1) {
        if (sql.charAt(end + 1) == PARAM_PREFIX_CHAR) {
          ...
          if (block != null) block.addText(sql.substring(start, end + 1));
      ...
      if (block != null) block.addText(sql.substring(start, end));
      ...
      if (!PSReplacementValueFactory.isValidFieldName(field)) {
        throw new IllegalArgumentException(
            "Field \"" + field + "\" cannot be used in this statement");
      }

      value = PSReplacementValueFactory.getReplacementValueFromXmlFieldName(field);

      replValList.add(value);

      if (block != null) block.addReplacementField(value, dataType, null);
    }
