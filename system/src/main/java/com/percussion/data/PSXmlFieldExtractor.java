...
public class PSXmlFieldExtractor extends PSDataExtractor {
  ...
  public Object extract(PSExecutionData data, Object defValue) throws PSDataExtractionException {
    ...
    if (walker != null) {
      ...
      if (((strVal == null) || (strVal.length() == 0)) && !(base.indexOf("@") >= 0)) {
        ...
        if (ele != null) urlRef = ele.getAttribute(XML_URL_REFERENCE_ATTRIBUTE);

        if ((urlRef != null) && (urlRef.length() > 0)) {
          PSPurgableTempFile f = data.getRequest().getTempFileResource(urlRef);
  ...
  public void setXmlFieldBase(String base) {
    ...
    if (base == null) {
      ...
      if (!m_source.startsWith("/")) m_source = "/" + m_source;
    } else {
      String source;
      if (m_source.startsWith("/")) source = m_source.substring(1);
      else source = m_source;

      m_sourceFromBase = PSXmlTreeWalker.getRelativeFieldName(base, source);
      if (m_sourceFromBase != null) m_source = source;
    }
  }
