...
public class PSQueryCacher {
  ...
  public PSQueryCacher(
    ...
    if (ms_shouldCreateCacheDir) ms_shouldCreateCacheDir = (!ms_CacheDirectory.mkdirs());
    ...
    if (m_cacheType == CACHE_TYPE_NONE) return;
    ...
    for (int i = 0; i < plan.length; i++) {
      ...
      if (plan[i] instanceof com.percussion.data.PSStatement) {
        m_keysForResultSet.addAll(((PSStatement) plan[i]).getReplacementValueExtractors());
      }
    }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (map.getBackEndMapping() instanceof com.percussion.design.objectstore.PSExtensionCall) {
    ...
    for (int i = 0; i < size; i++) {
       ...
       * list
       */
      addUdfExtractors(m_keysForXmlDocument, ((PSBackEndJoin) joins.get(i)).getTranslator());
    }
    ...
    if (ds.isOutputResultPages()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < condSize; j++) {
          ...
          if (!cond.isUnary()) addReplacementValueExtractors(styleKeys, cond.getValue());
        }
  ...
  public static boolean isCacheReadPermitted(PSRequest request) {
    ...
    String pragmaHeader = request.getCgiVariable(IPSCgiVariables.CGI_HTTP_PRAGMA, "").toLowerCase();

    // check the Cache-Control header for HTTP/1.1
    String cacheHeader =
        request.getCgiVariable(IPSCgiVariables.CGI_HTTP_CACHE_CONTROL, "").toLowerCase();
  ...
  public void addResultPage(PSExecutionData data, java.io.InputStream in, int length) {
    ...
    try {
      cacheKey = getCachedResultPageKey(data);
      PSCachedEntry entry = new PSCachedEntry(data.getRequest(), in, length);
  ...
  public void addXmlDocument(PSExecutionData data, Document doc) {
    ...
    try {
      cacheKey = getCachedXmlDocumentKey(data);
      PSCachedEntry entry = new PSCachedEntry(data.getRequest(), doc);
  ...
  public static java.util.Date generateTimeIntervalExpirationTime(
     ...
     */
    cal.add(java.util.Calendar.MINUTE, intervalMinutes - (timeOfDay % intervalMinutes));
  ...
  public static java.util.Date generateTimeExpirationTime(GregorianCalendar cal, int ageTimeOfDay) {
    int timeOfDay = getTimeOfDay(cal);
    if (timeOfDay >= ageTimeOfDay) {
  ...
  protected java.util.Date generateExpirationTime() {
    ...
    else if (m_cacheType == CACHE_TYPE_TIME) return generateTimeExpirationTime(cal, m_ageTimeOfDay);
    else if (m_cacheType == CACHE_TYPE_TIME_INTERVAL)
      return generateTimeIntervalExpirationTime(cal, m_ageTimeOfDay, m_intervalMinutes);
  ...
  private void addReplacementValueExtractors(
    ...
    if (!(value instanceof com.percussion.design.objectstore.PSBackEndColumn)
      ...
      list.add(PSDataExtractorFactory.createReplacementValueExtractor(value));
    }
  }
  ...
  private PSCachedEntry getEntryFromCache(
    ...
    if (entry != null) {
      ...
      if (entry == null) throw new PSCacheEntryAgedException();
    ...
    synchronized (m_cacheSizeSync) {
      if (wouldExceedSize(entrySize)) {
        ...
        if (wouldExceedSize(entrySize)) {
          Object[] args = {
            entryKey,
            String.valueOf(m_cacheSizeMax),
            String.valueOf(m_cacheSizeCur),
  ...
  private void logCacheEntryException(String cacheKey, int errorCode, Exception e) {
    Object[] args = {((cacheKey == null) ? "" : cacheKey), e.toString()};
    com.percussion.log.PSLogManager.write(
  ...
  class PSCachedEntry implements java.io.Serializable, Comparable<PSCachedEntry> {
    ...
    private PSCachedEntry(PSRequest request) throws java.io.IOException {
      super();
      ...
      m_cachedFile =
          File.createTempFile("psx", request.getRequestPageExtension(), ms_CacheDirectory);
    ...
    PSCachedEntry(PSRequest request, Document doc) throws java.io.IOException {
      this(request);
    ...
    InputStream getContent() throws java.io.FileNotFoundException {
