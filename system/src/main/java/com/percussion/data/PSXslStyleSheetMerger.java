...
public class PSXslStyleSheetMerger extends PSStyleSheetMerger {
  ...
  public void merge(
      ...
      throws PSConversionException {
    if (doc == null)
      throw new IllegalArgumentException("The Source document to merge should not be null");

    if (out == null)
      throw new IllegalArgumentException(
          "The output stream to which results" + " will be written should not be null");

    if (styleFile == null)
      throw new IllegalArgumentException("The Style Sheet to merge should not be null");
    ...
    if (req != null) {
      ...
      if (ah != null) ssCache = ah.getStylesheetCache();
    }
    ...
    try {
      ...
      try {
        ...
        if (listener instanceof PSTransformErrorListener)
          errorMsg.append(getErrorListenerMessage((PSTransformErrorListener) listener));
      ...
      try {
        ...
        else if (se instanceof TransformerException) errorMsg.append(getExceptionContextData(se));
        ...
        errorMsg.append(" ");
        errorMsg.append(
            "Exception happened while reading error messages of"
                + " style sheet loading, "
                + ioe.getLocalizedMessage());
    ...
    try {
      ...
      if (req != null) req.getRequestTimer().pause();
      ...
      // Traverse the nodes of document to delete nodes with Null Text, else transformer fails with
      // NPE.
      ...
      if (watch != null) {
        watch.stop();
        l.debug("Transforming stylesheet {} {}", styleFile, watch);
      }
      if (req != null) req.getRequestTimer().cont();

      if (hasParams) transformer.clearParameters();

      try {
        // we assigned the listener to the transformer, so it is safe to cast
        String errorMessage =
            getErrorListenerMessage((PSTransformErrorListener) transformer.getErrorListener());
        if (errorMessage.length() > 0) {
          String message =
              "Error transforming the xml document with " + "stylesheet " + errorMessage;
          if (ssCache != null) {
            // Removing cached style sheet from cache due to errors
            ssCache.remove(styleFile);
          }
        ...
        throwConversionException(
            ...
            "Exception happened while reading error messages of document "
                + "transformation with style sheet, "
                + ioe.getLocalizedMessage());
      }
      ...
      if (e instanceof TransformerConfigurationException)
        ...
        errorMsg.append("Error while transforming the XML file with stylesheet.");
        try {
          // we assigned the listener to the transformer, so it is safe to cast
          if (transformer != null) {
            String errorMessage =
          ...
          errorMsg.append(
              " Exception happened while reading error "
  ...
  public static String getErrorListenerMessage(PSTransformErrorListener listener)
      throws IOException {
    if (listener == null) throw new IllegalArgumentException("ErrorListener may not be null");

    StringBuilder errorMsg = new StringBuilder();

    if (listener.numErrors() > 0) errorMsg.append(getErrorMessages(listener.errors()));
    if (listener.numFatalErrors() > 0) errorMsg.append(getErrorMessages(listener.fatalErrors()));
  ...
  private static String getExceptionContextData(Exception e) throws IOException {
    if (e == null) throw new IllegalArgumentException("Exception may not be null");

    String resource = "";

    if (e instanceof TransformerException) {
      TransformerException te = (TransformerException) e;
      if (te.getLocator() != null) resource = te.getLocator().getSystemId();
    } else if (e instanceof SAXParseException) {
      SAXParseException se = (SAXParseException) e;
      resource = se.getSystemId();
    }

    if (resource == null || resource.equals("")) return "";

    URL url;
    try {
      if (resource.startsWith("file:") || resource.startsWith("http:")) url = new URL(resource);
      else url = new URL("file:" + resource);
  ...
  private static String getExceptionContextData(Exception e, URL url) throws IOException {
    ...
    if (e instanceof TransformerException) {
      TransformerException te = (TransformerException) e;
      if (te.getLocator() != null) errorLine = te.getLocator().getLineNumber();
    } else if (e instanceof SAXParseException) errorLine = ((SAXParseException) e).getLineNumber();
  ...
  private static void getExceptionContextData(
      StringBuilder buf, BufferedReader source, int errorLine) throws IOException {
    if (errorLine > 0) {
      String curLine;
      ...
      for (int i = 1; i <= errorLine + 1; i++) {
        ...
        if (i >= (errorLine - 1)) buf.append(curLine);
      }
    }
  }
  ...
  public static void setStyleSheet(PSRequest request, Document doc, URL styleSheetURL)
    ...
    if (request == null)
      throw new IllegalArgumentException("The context for this request can not be null.");

    if (doc == null)
      throw new IllegalArgumentException("The XML document to modify can not be null.");

    if (styleSheetURL == null)
      throw new IllegalArgumentException("The URL of style sheet to use can not be null.");
    ...
    if (ah != null) {
      ...
      else urlText = ah.getExternalURLString(styleSheetURL);
    } else {
    ...
    if (root != null) doc.insertBefore(pi, root);
    else doc.appendChild(pi);
  }
}
