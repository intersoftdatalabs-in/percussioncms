...
public class PSFolderPermissionsDataFilter implements IPSResultSetDataFilter {
  ...
  public PSFolderPermissionsDataFilter(String folderIdColName) {
    if (StringUtils.isBlank(folderIdColName))
      throw new IllegalArgumentException("folderIdColName may not be null or empty");
  ...
  public boolean accept(PSExecutionData data, Object[] vals) {
    ...
    if (vals == null) throw new IllegalArgumentException("vals may not be null");

    if (vals.length != 1 || !(vals[0] instanceof String))
      throw new IllegalArgumentException("vals must contain a single string");
    ...
    try {
      List<PSLocator> locList = processor.getAncestorLocators(new PSLocator(contentId));
      ...
      for (PSFolderAcl acl : acls) {
        ...
        if (!folderPerms.hasReadAccess()) return false;
      }
