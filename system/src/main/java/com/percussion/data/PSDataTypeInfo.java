...
public class PSDataTypeInfo {
  ...
  PSDataTypeInfo(
    ...
    if (nullable == DatabaseMetaData.typeNoNulls) m_flags |= FLAG_NO_NULLS;
    else if (nullable == DatabaseMetaData.typeNullable) m_flags |= FLAG_NULLABLE;
    else m_flags |= FLAG_NULL_UNKNOWN;

    if (autoIncrement) m_flags |= FLAG_AUTO_INCREMENT;

    if (searchable == DatabaseMetaData.typeSearchable) m_flags |= FLAG_SEARCH_ALL;
    else if (searchable == DatabaseMetaData.typePredChar) m_flags |= FLAG_SEARCH_CHAR;
    else if (searchable == DatabaseMetaData.typePredBasic) m_flags |= FLAG_SEARCH_BASIC;
    else m_flags |= FLAG_SEARCH_NONE;
  }
  ...
  public short getNullableSetting() {
    ...
    if ((m_flags & FLAG_NULLABLE) == FLAG_NULLABLE) return DatabaseMetaData.typeNullable;
  ...
  public short getSearchableSetting() {
    ...
    if ((m_flags & FLAG_SEARCH_ALL) == FLAG_SEARCH_CHAR) return DatabaseMetaData.typePredChar;

    if ((m_flags & FLAG_SEARCH_ALL) == FLAG_SEARCH_BASIC) return DatabaseMetaData.typePredBasic;
