...
public class PSUpdateHandler extends PSDataHandler {
  ...
  public PSUpdateHandler(PSApplicationHandler app, PSDataSet ds)
      throws PSInvalidRequestTypeException, PSIllegalArgumentException, java.sql.SQLException,
          PSSystemValidationException, PSNotFoundException, PSExtensionException {
    super(app, ds);
    ...
    if (!ds.isOutputResultPages()) {
      ...
      if (link != null) {
        try {
          ...
          throw new PSSystemValidationException(
              e.getErrorCode(), e.getErrorArguments(), app.getApplicationDefinition(), ds);
        }
      }
    } else {
      PSCollection pageSet = null;
      PSResultPageSet rs = ds.getOutputResultPages();
      if (rs != null) pageSet = rs.getResultPages();
      int size = (pageSet == null) ? 0 : pageSet.size();
      if (size != 0) m_styleSheetEvaluators = new PSSetStyleSheetEvaluator[size];
        ...
        try {
          m_styleSheetEvaluators[i] =
              new PSSetStyleSheetEvaluator(
                  app.getLocalizedURL(page.getStyleSheet()), page, page.getConditionals());
  ...
  public PSException makeRequest(PSRequest request, boolean sendResponse) {
     ...
     */
    int startingStats = stats.getRowsDeleted() + stats.getRowsInserted() + stats.getRowsUpdated();
    ...
    try {
      ...
      if (!m_listeners.isEmpty()) {
        PSTableChangeData tableChangeData = new PSTableChangeData(m_listeners.iterator());
      ...
      int currentStats = stats.getRowsDeleted() + stats.getRowsInserted() + stats.getRowsUpdated();
      ...
      if (null != updErr && currentStats == startingStats) {
        ...
        if (sendResponse) m_appHandler.handleAuthorizationException(request, e);
        else return e;
      } else {
        if (sendResponse)
      ...
      if (t instanceof PSException) {
        ...
        if (!sendResponse) e = new PSException(errorCode, errorArgs);
      }
      ...
      if (execData != null) {
        ...
        if (tableChangeData != null) tableChangeData.notifyListeners();
      }
    }
  ...
  public PSExecutionData makeInternalRequest(PSRequest originalRequest)
    ...
    if (originalRequest == null) throw new IllegalArgumentException("Request must be supplied.");

    PSRequest req = originalRequest;
    if (originalRequest.allowsCloning()) req = originalRequest.cloneRequest();
    checkInternalRequestAuthorization(req);

    String sendUpdateResponse = req.getParameter(IPSHtmlParameters.SYS_SENDUPDATERESPONSE);
    boolean sendResponse =
        sendUpdateResponse != null
  ...
  private java.net.URL applyStylesheetConditions(PSExecutionData data, Document doc)
    ...
    if (m_styleSheetEvaluators == null) return null;
    ...
    try {
      if (doc != null) data.setInputDocumentWalker(new PSXmlTreeWalker(doc));
  ...
  private Document processResultDocExtensions(PSExecutionData data, Document doc)
    ...
    if (m_resultDocProcessors != null) {
      ...
      for (int i = 0; i < size; i++) {
        PSExtensionRunner proc = (PSExtensionRunner) m_resultDocProcessors.elementAt(i);
    ...
    if (dh.isTraceEnabled(PSTraceMessageFactory.POST_EXIT_CGI_FLAG)) {
      dh.printTrace(PSTraceMessageFactory.POST_EXIT_CGI_FLAG, data.getRequest());
    }
