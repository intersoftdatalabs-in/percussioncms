...
public class PSStylesheetCleanupUtils {
  ...
  public static String namespaceCleanup(String htmlText, PSStylesheetCleanupFilter filter) {
    if (filter == null)
      throw new IllegalArgumentException("Stylesheet cleanup filter cannot be null.");
    if (htmlText == null || htmlText.trim().length() == 0) return htmlText;
    ...
    for (StartTag tag : startTags) {
      ...
      if (!filter.isNSElementAllowed(qname[0], qname[1])) {
        ...
        if (!sTag.isEmptyElementTag()) {
          ...
          if (eTag != null) outDoc.add(new StringOutputSegment(eTag, ""));
        }
        continue;
      }
