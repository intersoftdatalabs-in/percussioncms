...
/**
 * The IPSStatementBlock interface defines a block of text which will be used to construct a
 * statement. These blocks can be strung together to get the full text of the statement. Blocks can
 * be static blocks or replaceable blocks. Static blocks are always used when building the statement
 * text. Replaceable blocks are used only if all the XML fields they contain are not <code>NULL
 * </code>.
 *
...
public interface IPSStatementBlock {
  ...
  /**
   * Releases all resources that were used by the column's data. This should be called after the
   ...
   *
   * @return The list of columns. Never <code>null</code>. Can be empty.
   */
  ...
  /**
   * Determines whether this block generates sql that does not depend upon the runtime data.
   *
   * @return If <code>true</code> the sql is not dependent on runtime data, <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Add a replacement field to the block. The value of the replacement field will be used, at
   * run-time, when the statement to execute is constructed. Replacement fields are often XML
   * fields, HTML parameters or CGI variables.
   *
   ...
   * @param params implementation specific parameters, may be <code>null</code> or empty if the
   *     implementation does not require these parameters
   */
