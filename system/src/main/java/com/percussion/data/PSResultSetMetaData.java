...
public class PSResultSetMetaData implements java.sql.ResultSetMetaData {
  ...
  public void addResultSetMetaData(ResultSetMetaData meta) throws SQLException {
    ...
    try {
      ...
      getColumnClassNameMethod =
          cl.getDeclaredMethod("getColumnClassName", new Class[] {Integer.TYPE});
    } catch (Throwable t) {
    ...
    for (int colNo = 1; colNo <= colCount; colNo++) {
      ...
      if (type != Types.CLOB && type != Types.BLOB) col.setPrecision(meta.getPrecision(colNo));
  ...
  public boolean isAutoIncrement(int column) throws SQLException {
    if (column > m_columnCount)
      throw new SQLException(
          "Specified column number (" + column + ") is out of range (" + m_columnCount + ").");
