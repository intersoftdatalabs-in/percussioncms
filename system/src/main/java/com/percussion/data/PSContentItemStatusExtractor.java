...
public class PSContentItemStatusExtractor extends PSDataExtractor {
  ...
  private Document getContentStatus(PSExecutionData data) throws PSDataExtractionException {
    try {
      ...
      if (contentid == null || contentid.trim().length() == 0) return null;

      Document doc = (Document) req.getContentItemStatus(contentid);
      if (doc != null) return doc; // has already been cached

      Map params = new HashMap();
      params.put(IPSHtmlParameters.SYS_CONTENTID, contentid);

      PSInternalRequest rh =
          PSServer.getInternalRequest(GET_CONTENTITEMSTATUS_RESOURCE, req, params, false);
