 ...
 * <code>
 * PSFilterResultSetWrapper</code> class calls "accept" each time the result set advances using the
 * <code> next</code> method. The accept method will move the cursor position foward until the end
 * or the current row passes the accept method.
 */
public interface IPSResultSetDataFilter {
   ...
   * false</code>, the calling class will move the current cursor of the result set forward until
   ...
   *     implementing class
   * @return <code>true</code> if the result row is to be accepted in the creation of the xml data,
