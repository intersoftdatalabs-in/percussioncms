...
public class PSStatementColumn {
  ...
  public PSStatementColumn(
    ...
    if (type == Types.BLOB || type == Types.CLOB) m_lobColInitializer = lci;

    m_dataExtractor = PSDataExtractorFactory.createReplacementValueExtractor(value);
    m_dataType = type;
    m_backEndColumn = col;

    /* Check to see if our driver/server is tweaked by the builder
     */
    PSBackEndTable table = (m_backEndColumn == null) ? null : m_backEndColumn.getTable();

    if (table != null) {
      String datasource = table.getDataSource();

      m_untweakNulls = PSDatabaseMetaData.unTweakDriverNulls(datasource);
    } else {
  ...
  public int setData(PSExecutionData data, PreparedStatement stmt, int bindStart)
    ...
    if (dh.isTraceEnabled(PSTraceMessageFactory.RESOURCE_HANDLER_FLAG)) {
      ...
      if (traceValue.length() > 25) traceValue = traceValue.substring(0, 25) + "...";
    ...
    if (value instanceof String) {
      ...
      if (isNull) PSSqlHelper.setNullParameter(stmt, bindStart, dt);
      else
        try {
      ...
      if (value instanceof byte[]) {
        ...
        m_inputStream = PSSqlHelper.setDataFromFile(stmt, bindStart, (File) value, dt, charSet);
    ...
    if (tableChangeData != null) {
      ...
      if (value instanceof String || value instanceof Number) {
        ...
        if (tableChangeData.expectsColumn(tableName, columnName)) {
          ...
          if (value instanceof String) myString = (String) value;
          else if (value instanceof Number) myString = String.valueOf(value);

          if (myString != null) tableChangeData.addColumnValue(tableName, columnName, myString);
        }
      }
    }
  ...
  private int getAppropriateDataType(
      PSExecutionData data, PreparedStatement stmt, int bindStart, Object value, boolean isNull)
      throws java.sql.SQLException {
    int dt = m_dataType;

    if (dt == UNKNOWN_JDBC_TYPE) {
      // try to get the type from the result set's meta-data
      if (dt == UNKNOWN_JDBC_TYPE) dt = determineColumnTypeFromMetaData();

      // if still unknown, do it based on value object
      if (dt == UNKNOWN_JDBC_TYPE) dt = resolveUnkownDataType(stmt, bindStart, value);
    } else {
      if (isNull) {
  ...
  public String getPlaceHolder(PSExecutionData data)
    ...
    if (Types.NULL == m_dataType) return getValue(data).toString();
  ...
  private static boolean isPreparedObjectNull(Object o) {
    ...
    else if (o instanceof byte[]) return (((byte[]) o).length == 0);
    else if (o instanceof java.io.File) return (((java.io.File) o).length() == 0);
  ...
  public Object getPreparedValue(PSExecutionData data, Object defaultValue)
    ...
    if (o instanceof PSLiteral) o = o.toString();
    else if (o == null) o = defaultValue;
  ...
  private int resolveUnkownDataType(PreparedStatement stmt, int bindStart, Object value)
      throws java.sql.SQLException {
    ...
    if (value instanceof String) dt = Types.VARCHAR;
    else if (value instanceof byte[]) dt = Types.VARBINARY;
    else if (value instanceof java.io.File)
      dt = Types.VARBINARY; // todo: examine the file contents for better guess?
    else if (value instanceof Boolean) dt = Types.BIT;
    else if (value instanceof Byte) dt = Types.TINYINT;
    else if (value instanceof Short) dt = Types.SMALLINT;
    else if (value instanceof Integer) dt = Types.INTEGER;
    else if (value instanceof Long) dt = Types.BIGINT;
    else if (value instanceof BigInteger) dt = Types.BIGINT;
    else if (value instanceof Float) dt = Types.FLOAT;
    else if (value instanceof Double) dt = Types.DOUBLE;
    else if (value instanceof BigDecimal) dt = Types.NUMERIC;
    else if ((value instanceof java.util.Date) || (value instanceof java.sql.Timestamp))
      dt = Types.TIMESTAMP;
    else if (value instanceof java.sql.Date) dt = Types.DATE;
    else if (value instanceof java.sql.Time) dt = Types.TIME;
  ...
  int determineColumnTypeFromMetaData() {
    if (m_nativeDataType == UNKNOWN_JDBC_TYPE) {
      ...
      try {
        PSDatabaseMetaData dbMeta =
            (table == null) ? null : PSOptimizer.getCachedDatabaseMetaData(table);

        PSTableMetaData tabMeta =
            (dbMeta == null)
                ? null
                : dbMeta.getTableMetaData(
                    dbMeta.getConnectionDetail().getOrigin(), table.getTable());
