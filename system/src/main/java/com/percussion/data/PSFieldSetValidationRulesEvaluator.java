...
public class PSFieldSetValidationRulesEvaluator {
  ...
  public boolean isValidRepeatability(
    ...
    switch (m_fieldSet.getRepeatability()) {
      case PSFieldSet.REPEATABILITY_ONE_OR_MORE:
        if (getRowCount(m_fieldSet.getName(), page) >= 1) return true;
      ...
      case PSFieldSet.REPEATABILITY_COUNT:
        ...
        if (rowCount == m_fieldSet.getCount()) return true;
  ...
  private int getRowCount(String paramName, Document page) {
    ...
    for (int i = 0; i < controls.getLength(); i++) {
      ...
      if (paramName.equals(pn)) {
        NodeList rows = control.getElementsByTagName(PSTableValueBuilder.ROW_NAME);
   ...
   * object.
   *
   * @return the UI set being evaluated, never <code>null</code>.
   */
