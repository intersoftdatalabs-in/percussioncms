...
/**
 * Encapsulates the information about an action performed by an update resource in a Rhythmyx
 * application.
 */
public class PSTableChangeEvent {
   ...
   * @param actionType The action performed, must be one of the ACTION_xxx types.
   * @param columns The column data used to change the specified table. See {@link #getColumns()}
  ...
  /**
   * Gets the data that was used to update the table. The columns returned are specified when the
   * listener of this event is registered with the publisher of this event (see
   * IPSTableChangeListener#getColumns()}).
   *
   * @return A copy of the data map where each entry represents a column data where the key is the
   *     name of the column as a <code>String</code> and the value is the data that was used while
   *     performing the event converted to a <code>String</code>. Will contain all columns that were
   *     specified by the listener when it was registered, but if any of these columns were updated
   *     with a value other than a <code>String</code> or numeric type, they will be excluded from
   *     the results. The <code>Map</code> may contain additional columns not specified by the
   ...
   * @return <code>true</code> if the supplied action is one of the ACTION_xxx types, otherwise
   *     <code>false</code>
   */
  ...
  /**
   * The name of the table that was modified by this event. Set during ctor, never <code>null</code>
   * , empty, or modified after that.
   */
  private String m_tableName;

  /**
   * The action that was taken when the table was modified. One of the <code>ACTION_xxx</code>
   * values, set during ctor, never <code>null</code> or modified after that.
   */
  private int m_actionType;

  /**
   * The data used to modify a row in the the table, where the key is the column name as a <code>
