 ...
 * @author dougrand
 */
public class PSHibernateEvictionTableUpdateHandler extends PSTableUpdateHandlerBase {
  ...
  public PSHibernateEvictionTableUpdateHandler(
    ...
    if (persistenceClasses == null) {
      throw new IllegalArgumentException("persistenceClasses may not be null");
    }
    if (tables.length != pkcolumns.length || pkcolumns.length != persistenceClasses.length) {
  ...
  public Iterator<String> getColumns(String tableName, int actionType) {
    ...
    if (m_keys[i] != null) rval.add(m_keys[i]);
