...
public class PSSqlLockedUpdateBuilder extends PSSqlUpdateBuilder {
  ...
  PSUpdateStatement generate(
    ...
    // result set
    if (supportsLocking) context.addText(" WHERE CURRENT OF ");
    else buildWhereClauseFromKeys(context, table, dtHash);
    ...
    if (supportsLocking) queryContext.addText(" FOR UPDATE ");
