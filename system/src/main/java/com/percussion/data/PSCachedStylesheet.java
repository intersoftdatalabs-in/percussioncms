...
public class PSCachedStylesheet {
  ...
  public PSCachedStylesheet(URL ssUrl) throws IOException, TransformerFactoryConfigurationError {
    if (ssUrl == null) throw new IllegalArgumentException("Url to cache can not be null");
    ...
    if ("file".equalsIgnoreCase(m_ssUrl.getProtocol())) m_ssFile = new File(m_ssUrl.getFile());
    else m_ssFile = null;
  }
        ...
        if (m_ssTemplate == null) {
          ...
          try {
            ...
            try (InputStream urlStream = m_ssUrl.openStream()) {
              PSXmlDocumentBuilder.setInternalRequestURIResolver(
                  new PSInternalRequestURIResolver());
              doc = PSXmlDocumentBuilder.createXmlDocument(new InputSource(urlStream), false);
            }
            ...
            m_ssTemplate = m_transformFactory.newTemplates(new DOMSource(doc, m_ssUrl.toString()));
          } catch (SAXException | IOException | TransformerConfigurationException e) {
            l.error("Error loading: {} Error: {}", m_ssUrl, PSExceptionUtils.getMessageForLog(e));
            throw e;
          }
        }
      }
    ...
    for (int i = 0; i < nodeCount; i++) {
      ...
      if (attr != null && attr.equalsIgnoreCase("content-type")) {
        ...
        if (content != null && !content.equals("")) {
          ...
          if (found) {
            ...
            StringTokenizer st = new StringTokenizer(elem.getAttribute("content"), "; ", true);
            String token = null;
      ...
      */
      if (!found) {
        ...
        if (attr != null && !attr.equals("")) elem.setAttribute("charset", encoding);
      }
    }
  }
