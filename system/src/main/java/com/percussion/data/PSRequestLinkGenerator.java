...
public class PSRequestLinkGenerator extends PSDataExtractor {
  ...
  public PSRequestLinkGenerator(PSApplicationHandler app, PSDataSet sourceDS, PSRequestLink link)
    ...
    this(
        app,
        sourceDS,
        app.getDataSetDefinition(link.getTargetDataSet()),
        link.getXmlField(),
        link.getType());
  }
  ...
  protected PSRequestLinkGenerator(
    ...
    if (null == sourceDS) {
      throw new IllegalArgumentException(
          "req link source ds null " + app.getName() + " " + targetDS);
    }
    if (null == targetDS) {
      throw new IllegalArgumentException(
          "req link target ds null " + app.getName() + " " + sourceDS);
    }
    ...
    switch (type) {
      case PSRequestLink.RL_TYPE_QUERY:
        addQueryParameters(app, sourceDS, targetDS, type, params, lookupVals);
        break;
      ...
      case PSRequestLink.RL_TYPE_DELETE:
        ...
        addUpdateRequestType(app, type);
        addUpdateParameters(app, sourceDS, targetDS, type, params, lookupVals);
        break;
    ...
    addSelectionParameters(app, sourceDS, targetDS, type, params, lookupVals);

    // now build up our internal string arrays
    if (lookupVals.size() == 0) m_lookupParams = null;
    else {
      m_lookupParams = new IPSDataExtractor[lookupVals.size()];
      lookupVals.toArray(m_lookupParams);
    }

    if (params.size() == 0) m_parameterNames = null;
    else {
  ...
  public Object extract(PSExecutionData data, Object defValue)
    ...
    if (data.getRequest() == null) return defValue;
  ...
  protected String generateURL(PSExecutionData data, String moreParams)
    ...
    String reqExtension;
    if (m_useExtension) reqExtension = request.getRequestPageExtension();
    else reqExtension = "";
  ...
  protected void addSelectionParameters(
    ...
    for (int j = 0; j < condsCount; j++) {
      ...
      if (PSHtmlParameter.VALUE_TYPE.equals(value.getValueType())) {
        addParameterSource((PSHtmlParameter) value, cond.getValue(), sourceDS, params, extractors);
      } else {
        ...
        if (PSHtmlParameter.VALUE_TYPE.equals(value.getValueType())) {
          addParameterSource(
              (PSHtmlParameter) value, cond.getVariable(), sourceDS, params, extractors);
        }
      }
    }
  }
      ...
      if (selector.isSelectByWhereClause()) {
        ...
        if ((clauses != null) && (clauses.size() != 0)) {
          for (int j = 0; j < clauses.size(); j++) {
            ...
            if (PSHtmlParameter.VALUE_TYPE.equals(value.getValueType())) {
              addParameterSource(
                  (PSHtmlParameter) value, wClause.getVariable(), sourceDS, params, extractors);
            }
        ...
        IPSReplacementValue[] replValues =
            PSSqlParser.getReplacementValues(selector.getNativeStatement());
        for (int j = 0; j < replValues.length; j++) {
          IPSReplacementValue value = replValues[j];
          // only add HTML params to the list
          if (PSHtmlParameter.VALUE_TYPE.equals(value.getValueType())) {
            addParameterSource((PSHtmlParameter) value, null, sourceDS, params, extractors);
          }
          // else: must be a literal or CGI var
          //   either case, we're not interested
        }
  ...
  protected void addUpdateParameters(
    ...
    if (pipe instanceof PSUpdatePipe) {
      ...
      if (sync == null) {
        throw new IllegalArgumentException(
            "update link target sync required "
                + targetDS.getName()
                + " "
                + getLinkTypeString(type));
      }
            ...
            if (docMaps != null) {
              ...
              for (int k = 0; k < docMaps.length; k++) {
                ...
                if (val instanceof PSHtmlParameter) {
                  addParameterSource(
                      (PSHtmlParameter) val, col.getColumn(), sourceDS, params, extractors);
  ...
  protected boolean addParameterSource(
    ...
    if ((expectedSource != null)
      ...
      addedExtractor = addColumnSource((PSBackEndColumn) expectedSource, sourceDS, extractors);
    } else if (expectedSource != null) { // this must be a CGI variable, literal, etc.
      // use the value directly
      extractor = PSDataExtractorFactory.createReplacementValueExtractor(expectedSource);
    }
    ...
    if (!addedExtractor) {
      if (extractor == null)
        extractor = PSDataExtractorFactory.createReplacementValueExtractor(param);
  ...
  protected void addUpdateRequestType(PSApplicationHandler ah, int reqType) {
    ...
    if (strParam == null) return; // no param name, nothing to do
    strParam = PSURLEncoder.encodeQuery(strParam); // URL encode this

    String strValue;
    if (reqType == PSRequestLink.RL_TYPE_INSERT) strValue = app.getRequestTypeValueInsert();
    else if (reqType == PSRequestLink.RL_TYPE_UPDATE) strValue = app.getRequestTypeValueUpdate();
    else if (reqType == PSRequestLink.RL_TYPE_DELETE) strValue = app.getRequestTypeValueDelete();
    else return; // not a supported type, nothing to do
    strValue = PSURLEncoder.encodeQuery(strValue); // URL encode this

    if (m_staticParams == null) m_staticParams = strParam + "=" + strValue;
    else m_staticParams += "&" + strParam + "=" + strValue;
  }

  private static IPSReplacementValue getMatch(PSBackEndColumn col, PSDataSet ds) {
        ...
        if (col != null) {
          for (int condi = 0; ((condi < conditionals.size()) && (source == null)); condi++) {
            ...
            if (cond.getValue() instanceof IPSBackEndMapping) {
              bec = getMatchingColumn((IPSBackEndMapping) cond.getValue(), col);
              if (bec == col) {
                source = col;
                break;
              } else if (bec != null) {
            ...
            if (cond.getVariable() instanceof IPSBackEndMapping) {
              bec = getMatchingColumn((IPSBackEndMapping) cond.getVariable(), col);
              if (bec == col) {
                source = col;
                break;
              } else if (bec != null) {
  ...
  private static PSBackEndColumn getMatchingColumn(IPSBackEndMapping beMap, PSBackEndColumn col) {
    ...
    if (beMap instanceof PSBackEndColumn) {
      if (col.equals(beMap)) {
        return col;
      } else if (col.getColumn().equalsIgnoreCase(((PSBackEndColumn) beMap).getColumn())) {
        possibleMatch = (PSBackEndColumn) beMap;
      }
    } else if (beMap instanceof PSExtensionCall) { // UDF, which may contain the col in a param
      PSExtensionParamValue[] pValues = ((PSExtensionCall) beMap).getParamValues();
  ...
  private static IPSDocumentMapping[] getDocMappings(PSBackEndColumn col, PSDataSet ds) {
  ...
  protected boolean addColumnSource(
    ...
    if (source != null) {
      extractors.add(PSDataExtractorFactory.createReplacementValueExtractor(source));
      return true;
    }
