...
public class PSSortedResultJoiner extends PSQueryJoiner {
  ...
  public void execute(PSExecutionData data)
    ...
    try {
      PSJoinedRowDataBuffer rowData =
          new PSJoinedRowDataBuffer(
              lRS,
              rRS,
              m_columnHash,
              m_omitColumns,
  ...
  private void processData(PSExecutionData execData, PSJoinedRowDataBuffer rowData)
    ...
    while (readLeft || readRight) {
      ...
      if ((!readLeft) && (m_joinType == LEFT_OUTER_JOIN)) break;
      else if ((!readRight) && (m_joinType == RIGHT_OUTER_JOIN)) break;
      else if ((!readLeft || !readRight) && (m_joinType == INNER_JOIN)) break;
      ...
      if (comparator == 0) {
        ...
        if ((m_joinType == LEFT_OUTER_JOIN) || (m_joinType == FULL_OUTER_JOIN)) {
          rowData.addRowNullRight(curRow); // add the row we're on
        }
      } else { // comparator > 0
        // if the right side is smaller
        // see if we're doing an outer join on it
        if ((m_joinType == RIGHT_OUTER_JOIN) || (m_joinType == FULL_OUTER_JOIN)) {
       ...
       */
      if (readLeft && (comparator <= 0)) readLeft = rowData.readLeftRow();
      if (readRight && (comparator >= 0)) readRight = rowData.readRightRow();
    }
  ...
  private int readWhat(PSExecutionData execData, PSJoinedRowDataBuffer buf)
      throws com.percussion.data.PSDataExtractionException, java.sql.SQLException {
    ...
    if (rHasNext) {
      if (m_translator != null) lValue = m_translator.extract(execData);
      else lValue = curRow[m_leftColumnIndex];
    ...
    if (lHasNext) {
      ...
      } else lValue = nextRow[m_leftColumnIndex];
     ...
     * smallest value
     */
    if (lHasNext && rHasNext) comparator = compareKeyValues(lValue, nextRow[m_rightColumnIndex]);
    else comparator = 0; // move both forward
   ...
   */
  private int processManyToMany(PSExecutionData execData, PSJoinedRowDataBuffer buf)
    ...
    while ((comparator == 0) && hasNext) {
      if (m_translator != null) {
        ...
        execData.setCurrentResultRowData(curRow);
      } else nextLValue = nextRow[m_leftColumnIndex];

      comparator = compareKeyValues(lValue, nextLValue);
      if (comparator == 0) { // we're still in the many condition, process the next row
        leftMatches++;
        buf.readLeftRow();
