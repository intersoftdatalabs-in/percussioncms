...
/**
 * The PSSortedResultJoiner class is used internally by the E2 server to merge the results of
 * heterogenous queries into a single, joined result set.
 *
...
public class PSSortedResultJoiner extends PSQueryJoiner {
  ...
  /**
   * Execute the join of two or more result sets as a step in the execution plan. Each join may act
   * on a subset of the result sets defined in the execution data. It will then remove the result
   * sets and store the merged result set.
   *
   * @param data execution data is a container for the input data as well as a collection of result
   *     sets generated by queries.
  ...
  /**
   * compare the peek ahead rows with the current rows to determine which side should be
   * repositioned. If we need to read the next left side row, -1 is returned; right side row 1 is
   * returned; both rows 0.
   */
  ...
  /**
   * after a match has been found, we check if we're in a many:many case so we can set the mark on
   * the result set appropriately.
   *
