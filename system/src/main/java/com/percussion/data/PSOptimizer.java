...
public abstract class PSOptimizer {
   ...
   */
  public static PSDatabaseMetaData getCachedDatabaseMetaData(PSBackEndTable table)
      throws SQLException {
    if (table == null) {
      throw new IllegalArgumentException("table may not be null");
    }

    PSDatabaseMetaData dbmd = PSMetaDataCache.getCachedDatabaseMetaData(table.getDataSource());
  ...
  protected static void logExecutionPlan(
    ...
    for (int i = 0; i < steps.length; i++) {
      if (steps[i] != null) {
        ...
        Object[] args = {
          ah.getName(), ds.getName(), String.valueOf(i + 1), className, steps[i].toString()
        };
      ...
      for (int i = 0; i < joins.size(); i++) {
        ...
        if (j.getTranslator() != null) {
          ...
          if (left.getTable().isSameDatasource(right.getTable())) {
            ...
            if (!beTableDoubles.containsKey(leftServerKey)) beTableDoubles.put(leftServerKey, null);
          }
        }
      }
    }
  ...
  protected static void printIndexStatistics(PSBackEndTable table, PSIndexStatistics[] stats) {
    if ((stats != null) && (stats.length > 0)) {
      ...
      for (int indNo = 0; indNo < stats.length; indNo++) {
        ...
        log.info("  Type: ");
        if (stats[indNo].getIndexType() == DatabaseMetaData.tableIndexStatistic)
          log.info("table statistics");
        else if (stats[indNo].getIndexType() == DatabaseMetaData.tableIndexClustered)
          log.info("clustered index");
        else if (stats[indNo].getIndexType() == DatabaseMetaData.tableIndexHashed)
