 ...
 * with each other.
 *
 * <p>For example, suppose we have the following data: java.math.BigDecimal num = 10; String
 * oneNumberInString = 20; If we were asked to determine mathematically whether a number 10 is less
 * than the other number 20, then what we can do here is to convert oneNumberInString from type
 * String to type java.math.BigDecimal. Once this is done, since both num and oneNumberInString are
 * of the same type, mathematical operation can be applied.
 *
 * <p>Currently, the convertable data types are BigDecimal, PSNumericLiteral, Date, PSDateLiteral,
...
public abstract class PSDataConverter {
   ...
   *
   * <p>With default format being activated, here are some working cases: (1) "1999-08-12
   * 00:00:00.123"; (2) "1999.08.12"; (3) "1999.08.12 AD"; (4) "1999.08.12 AD at 14:04:24"; (5)
   * "1999.08.12 at 01:01:01 PDT".
   *
  ...
  public static Object convert(Object data, int dstType, FastDateFormat dateFormat) {
    ...
    switch (srcType) {
      case DATATYPE_NUMERIC:
        if (data instanceof PSNumericLiteral) data = ((PSNumericLiteral) data).getNumber();

        switch (dstType) {
          case DATATYPE_DATE:
            if (data != null) return new Date(((java.lang.Number) data).longValue());
            break;
          case DATATYPE_TEXT:
          ...
          case DATATYPE_DOUBLE:
            throw new IllegalArgumentException(
                "data invalid conversion" + getTypeString(srcType) + " " + getTypeString(dstType));
          default:
        }
        break;
      ...
      case DATATYPE_DATE:
        if (data instanceof PSDateLiteral) data = ((PSDateLiteral) data).getDate();
        ...
        switch (dstType) {
          case DATATYPE_NUMERIC:
            try {
              ...
              throw new IllegalArgumentException(
                  "cannot convert with reason"
                      + getTypeString(srcType)
                      + " "
   ...
   *
   * <p>notes: If none of the above then returns an unconverted object. It returns an object of
   * PSLiteralSet with elements of PSDateLiteral if all tokens of the data is in the format of Date.
  ...
  public static Object convertToSet(Object data) {
    if (data == null) throw new IllegalArgumentException("data to convert can not be null");
    ...
    if (getDataType(data) == DATATYPE_TEXT) {
      ...
      if (valueSet.isEmpty()) valueSet.add(data.toString()); // a set can have only one item
      ...
      while (isDate && itValueSet.hasNext()) {
        ...
        try {
          ...
          Date date = PSDataTypeConverter.parseStringToDate(itValueSet.next(), formatBuf);

          if (date != null) {
            day = new PSDateLiteral(date, FastDateFormat.getInstance(formatBuf.toString()));
  ...
  /**
   * Convert data from one type to another type. This is the same as calling method convert(data,
   * dstType, null). Any default date format will not be activated unless conversion between
   * String/PSTextLiteral and Date/PSDateLiteral is performed. If this is the case, then the String
   * representing for a date have to be in one default date format in order to perform conversion
   * successfully.
   *
   * <p>Example: String oneDateInString = "1999.08.12 AD at 14:04:56"; which means August 12, 1999
   * after domino, at the fourteenth hour, the fourth minitue, and the fifty sixth second. If
   * oneDateInString is not in any given date format pattern, then an exception will be thrown. A
   * typical case is when data is a String/PSTextLiteral, srcType is DATATYPE_TEXT, and dstType is
   * DATATYPE_DATE.
   */
  ...
  public static int getDataType(Object data) {
    if (data == null) return DATATYPE_NULL;
    else if ((data instanceof PSDateLiteral) || (data instanceof java.util.Date))
      return DATATYPE_DATE;
    else if ((data instanceof PSNumericLiteral)
        || (data instanceof java.math.BigInteger)
        || (data instanceof java.math.BigDecimal)) return DATATYPE_NUMERIC;
    else if ((data instanceof java.lang.Byte)
        || (data instanceof java.lang.Short)
        || (data instanceof java.lang.Integer)) return DATATYPE_INT;
    else if ((data instanceof java.lang.Float) || (data instanceof java.lang.Double))
      return DATATYPE_DOUBLE;
    else if (data instanceof java.lang.Long) return DATATYPE_LONG;
    else if (data instanceof PSTextLiteral) return DATATYPE_TEXT;
      ...
      if (((String) data).equals("")) return DATATYPE_NULLTEXT;
      else return DATATYPE_TEXT;
    } else if (data instanceof byte[]) return DATATYPE_BINARY;
    else if (data instanceof PSLiteralSet) {
      PSLiteralSet ls = (PSLiteralSet) data;

      if (ls.isEmpty()) return DATATYPE_NULL;
  ...
  public static int getBestComparisonType(int leftType, int rightType) {
    ...
    BugId: Rx-99-10-0122 */
    if ((leftType == DATATYPE_NULLTEXT) || (rightType == DATATYPE_NULLTEXT)) {
      if ((leftType == DATATYPE_FILE) || (rightType == DATATYPE_FILE)) {
    ...
    switch (leftType) {
      ...
      case DATATYPE_NULL:
        if (rightType == DATATYPE_FILE) return DATATYPE_NULL;
   ...
   * @return -1 if left is less than right; 0 if they are equal; 1 if left is greater than right
   */
  public static int compare(Object left, Object right) {
    ...
    if (((leftType & DATATYPE_SET_FLAG) != 0) && ((rightType & DATATYPE_SET_FLAG) != 0)) {
      throw new IllegalArgumentException(
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "unsupported conversion: "
              ...
              + " "
              + PSDataConverter.getTypeString(bestType)
              + " "
              + left);
    }
    ...
    if (leftType == DATATYPE_FILE) f = (File) left;

    if (rightType == DATATYPE_FILE) f = (File) right;
    ...
    switch (bestType) {
      ...
      case DATATYPE_NULLTEXT:
        if (f != null) {
          if (leftType == DATATYPE_FILE) return 1;
          else if (rightType == DATATYPE_FILE) return -1;
        } else return 0; // if they're both NULL, they're obviously equal
   ...
   * @deprecated Use {@link com.percussion.util.PSDataTypeConverter#parseStringToDate(String)}
   *     instead.
   */
  public static Date parseStringToDate(String myText) throws ParseException {
    if (myText == null) throw new IllegalArgumentException("myText may not be null");

    Date result = PSDataTypeConverter.parseStringToDate(myText);
    if (result == null) throw new ParseException("Text is not in date pattern", 0);
