...
/**
 * The PSResultSetMimeConverter class implements the IPSResultSetConverter interface, providing
 * conversion to the specified MIME output type.
 *
...
public class PSResultSetMimeConverter implements IPSResultSetConverter {
   ...
   * </ol>
   *
   * @param filter This parameter is ignored by this converter as this converter only processes a
   *     single row. In the future, we could be smarter about this.
   */
  ...
  /**
   * Retrieve a column mime-data and optionally its mime type from the back end database. Sets the
   * data on the response if indicated.
   *
   * @param data The execution data for this query. Never <code>null</code>.
   * @param setResponse Should we set the result on response? <code>true</code> indicates to set the
   *     result on the response, while <code>false</code> will cause the temporary file and mime
   *     type information to be stored in the execution data to be retrieved by the caller.
   * @throws PSConversionException if the mime content fails to be prepared for return to the user
   *     (or internal caller).
   * @throws PSUnsupportedConversionException if any lower level call throws this exception
  ...
  public void processMimeContent(PSExecutionData data, boolean setResponse)
      throws PSConversionException, PSUnsupportedConversionException {
    if (data == null) throw new IllegalArgumentException("Execution data must be supplied.");
    ...
    try {
      ...
      while (rs.next()) {
         ...
         * that is, a call to available is not necessarily the amount of data pending! 4. when
         * writing the data, HTTP required Content-Length be set before sending any data
         *
         * <p>So instead, I'm reading the data a chunk at a time into an output stream. The last
         * chunk of data read will be less than our stream size, which is our end condition.
         *
         * <p>To avoid allocating huge chunks of memory, we'll write the retrieved data to disk and
         * serve the file up.
         */
        ...
        switch (mimeDataColType) {
          ...
          case Types.CLOB:
            Clob c = rs.getClob(1);
            if (c != null) reader = c.getCharacterStream();
            break;
        ...
        if (rowData != null || reader != null) {
          ...
          // we're going
          PSPurgableTempFile f = new PSPurgableTempFile("psm", "tmp", null);
          ...
          try {
            ...
            } else // we have a reader
            {
              ...
              if (encoding != null && encoding.length() > 0)
                osw = new OutputStreamWriter(fout, PSCharSets.getJavaName(encoding));
              else osw = new OutputStreamWriter(fout);
            ...
            if (!setResponse) {
              ...
              data.setTempFileResource(new PSMimeContentResult(f, mimeType));
            } else {
              // the temp file will be delete after closing the stream
              fin = new PSPurgableFileInputStream(f);

              String contentHeader = PSBaseHttpUtils.constructContentTypeHeader(mimeType, encoding);
  ...
  /**
   * The requestor associated with this converter, to be used to determine the mime type and
