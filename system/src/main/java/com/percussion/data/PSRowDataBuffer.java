...
public class PSRowDataBuffer {
  ...
  public boolean readRow() throws java.sql.SQLException {
    ...
    if ((m_hasData = m_resultSet.next())) {
      for (int i = 0; i < m_columnCount; i++) {
        ...
        if (m_curRow[i] instanceof byte[]) m_curRow[i] = new PSBinaryData((byte[]) m_curRow[i]);

        if (m_resultSet.wasNull()) m_curRow[i] = null;
      }
    } else {
      for (int i = 0; i < m_columnCount; i++) {
        m_curRow[i] = null;
  ...
  static Object getResultColumn(ResultSet rs, int colType, int index) throws java.sql.SQLException {
    Object colData = null;
    ...
    if (rs instanceof PSResultSet
      ...
      if (colType == Types.CLOB) colType = Types.LONGVARCHAR;
      if (colType == Types.BLOB) colType = Types.LONGVARBINARY;
    }
    ...
    switch (colType) {
      ...
      case Types.LONGVARBINARY:
        ...
        if (null == rowData) {
          ...
          try {
            ...
            if (iRead < 0) return null;
            else bout.write(aData, 0, iRead);
      ...
      case Types.CLOB:
        ...
        if (c == null) {
          ...
          try {
            colData = PSSqlHelper.getClobColumnData(c.getCharacterStream());
   ...
   */
  private static String fixupUtf16SqlServerBug(String data) throws SQLException {
    if (null == data || data.trim().length() == 0) return data;
