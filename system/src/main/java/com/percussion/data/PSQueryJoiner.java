...
public abstract class PSQueryJoiner implements IPSExecutionStep {
  ...
  protected PSQueryJoiner(
    ...
    for (int i = 0; i < m_columnCount; i++) m_omitColumns[i] = false; // default to keep the col
    ...
    for (; i < lCols.length; i++) {
      if (!m_omitColumns[i]) m_columnNames[iOnCol++] = lCols[i];
    }
    for (; i < m_columnCount; i++) {
      if (!m_omitColumns[i]) m_columnNames[iOnCol++] = rCols[i - lCols.length];
    }
    ...
    if (join.isFullOuterJoin()) m_joinType = FULL_OUTER_JOIN;
    else if (join.isLeftOuterJoin()) m_joinType = LEFT_OUTER_JOIN;
    else if (join.isRightOuterJoin()) m_joinType = RIGHT_OUTER_JOIN;
    else m_joinType = INNER_JOIN;
  }
  ...
  protected int compareKeys(
    ...
    if (!leftHasData) return 1; // right wins
    else if (!rightHasData) return -1; // left wins
    ...
    else lValue = rowData[m_leftColumnIndex];
  ...
  protected int compareKeyValues(Object lKey, Object rKey)
      throws com.percussion.data.PSDataExtractionException {
    if (lKey == null) return (rKey == null) ? 0 : -1;
    else if (rKey == null) return (lKey == null) ? 0 : 1;
    else if (lKey.equals(rKey)) return 0;
    ...
    if (lKey.getClass().isInstance(rKey)) {
      if (lKey instanceof Comparable) return ((Comparable) lKey).compareTo(rKey);
      else if (lKey.equals(rKey)) return 0;
    } else if (rKey.getClass().isInstance(lKey)) {
      if (rKey instanceof Comparable) return ((Comparable) rKey).compareTo(lKey);
      else if (rKey.equals(lKey)) return 0;
    }
