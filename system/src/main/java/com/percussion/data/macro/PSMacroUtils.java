...
public class PSMacroUtils {
   ...
   */
  public static String correctRevisionForFlags(
      IPSRequestContext request, String contentid, char[] contentValidFlags, String defaultRevision)
   ...
   */
  public static String fixLinkUrlRevisionForFlags(
      IPSRequestContext request, String linkUrl, char[] contentValidFlags)
      throws PSInternalRequestCallException, PSNotFoundException, PSRequestParsingException {
    if (request == null) {
    ...
    String lastPublicRevision =
        correctRevisionForFlags(request, contentid, contentValidFlags, revision);
    if (lastPublicRevision.equals("-1")) return null;
    ...
    if (end != -1) fixedUrl += linkUrl.substring(end);
  ...
  public static String getLastPublicRevision(String contentid) {
    ...
    if (contentid.length() == 0) throw new IllegalArgumentException("contentid cannot be empty");
  ...
  public static Map<Integer, Integer> getLastPublicRevisions(List<Integer> contentIds)
    ...
    if (contentIds == null || contentIds.isEmpty())
      throw new IllegalArgumentException("contentIds cannot be null or empty");
    ...
    if (counter > 0) result.putAll(getLastPublicRevisionsPerGroup(group));
   ...
   * @throws PSNotFoundException if the lookup resource was not found.
   */
  public static boolean isItemStateFlag(
      IPSRequestContext request, String contentid, char[] validFags)
      throws PSInternalRequestCallException, PSNotFoundException {
    ...
    if (doc != null) {
      ...
      if (root != null) {
        String contentValid = root.getAttribute("contentvalid").toLowerCase();
        if (contentValid.length() < 1) return false;
        char valid = contentValid.charAt(0);
        for (int i = 0; i < validFags.length; i++) {
          if (valid == Character.toLowerCase(validFags[i])) return true;
        }
      }
    }
    return false;
  ...
  public static String extractContentId(PSExecutionData data) throws PSDataExtractionException {
    ...
    PSBackEndColumn column = new PSBackEndColumn(table, IPSConstants.ITEM_PKEY_CONTENTID);
    PSBackEndColumnExtractor cidExtractor = new PSBackEndColumnExtractor(column);
    try {
  ...
  public static String extractCheckoutUser(String contentid, PSExecutionData data)
    ...
    return extractContentItemStatus(contentid, data, IPSConstants.ITEM_CONTENTCHECKOUTUSERNAME);
  }
  ...
  public static String extractCurrentRevision(String contentid, PSExecutionData data)
    ...
    return extractContentItemStatus(contentid, data, IPSConstants.ITEM_CURRENTREVISION);
  }
  ...
  public static String extractTipRevision(String contentid, PSExecutionData data)
    ...
    return extractContentItemStatus(contentid, data, IPSConstants.ITEM_TIPREVISION);
  }
  ...
  private static String extractContentItemStatus(
      String contentid, PSExecutionData data, String column) throws PSDataExtractionException {
    PSRequest request = data.getRequest();
    ...
    try {
      PSContentItemStatus source =
          new PSContentItemStatus(IPSConstants.CONTENT_STATUS_TABLE, column);

      PSContentItemStatusExtractor extractor = new PSContentItemStatusExtractor(source);
