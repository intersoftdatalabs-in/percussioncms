  ...
  public Object extract(PSExecutionData data) throws PSDataExtractionException {
    try {
      ...
      if (authtype.equals(AUTHTYPE_ALL)) {
        String checkoutUser = PSMacroUtils.extractCheckoutUser(contentid, data);

        PSUserEntry[] userEntries = request.getUserSession().getAuthenticatedUserEntries();

        // there should never be more then 1 user entry
        PSUserEntry user = null;
        if (userEntries != null) user = userEntries[0];

        if (checkoutUser != null && user != null) {
          if (checkoutUser.equals(user.getName())) {
            String tipRevision = PSMacroUtils.extractTipRevision(contentid, data);
            if (tipRevision == null || tipRevision.trim().length() == 0) {
              // this should never happen
              throw new IllegalStateException("processed item is in an invalid state");
            }
        ...
        String currentRevision = PSMacroUtils.extractCurrentRevision(contentid, data);
      ...
      String lastPublicRevision = PSMacroUtils.getLastPublicRevision(contentid);
