...
public class PSQueryOptimizer extends PSOptimizer {
  ...
  public static IPSExecutionStep[] createExecutionPlan(PSApplicationHandler ah, PSDataSet ds)
    ...
    if (ah == null) throw new IllegalArgumentException("exec plan app handler is null");

    // make sure the data set is not null
    if (ds == null) {
      throw new IllegalArgumentException("exec plan data set is null: " + ah.getName());
    }
  ...
  private static IPSExecutionStep[] buildExecutionPlan(
    ...
    if ((beTables == null) || (beTables.size() == 0)) {
      throw new IllegalArgumentException(
          "exec plan no beta tables in pipe: "
              + ah.getName()
              + " "
              + ds.getName()
              + " "
    ...
    loginCount +=
        createLoginPlan(ah, beTables, logins, connKeys, pipe.getBackEndDataTank().getJoins());

    // and create the statement to select data for this pipe
    if (sel.isSelectByNativeStatement()) {
      // the last login is the one we need
      curSteps = PSSqlParser.prepare(loginCount - 1, sel.getNativeStatement(), true);
    } else {
      curSteps =
          createStatements(
              ah, ds, logins, connKeys, pipe.getBackEndDataTank(), pipe.getDataMapper(), sel);
    }

    for (int s = 0; s < curSteps.length; s++) steps.add(curSteps[s]);
  ...
  private static IPSExecutionStep[] createStatements(
    ...
    if (backEnds == null) throw new IllegalArgumentException("sql builder no back ends");
    ...
    if (numTables == 1) {
      ...
      for (int i = 0; i < cols.size(); i++) {
        ...
        if (curMap instanceof PSExtensionCall) {
          PSExtensionParamValue[] params = ((PSExtensionCall) curMap).getParamValues();
        ...
        if ((conds != null) && (conds.size() != 0)) {
          ...
          for (int j = 0; j < conds.size(); j++) {
            ...
            if (val instanceof PSBackEndColumn) {
              ...
              PSFunctionParamValue[] params = ((PSFunctionCall) val).getParamValues();
    ...
    {
      return createJoinPlan(ah, ds, logins, connKeys, backEnds, tables, cols, sel);
    }
  ...
  private static IPSExecutionStep[] createJoinPlan(
      ...
      PSDataMapper cols,
      PSDataSelector sel)
      throws java.sql.SQLException, PSExtensionException, PSNotFoundException {
    /* **********
     * VALIDATION
     * ********** */
    ...
    int joinCount = 0;
    if (joins != null) joinCount = joins.size();
    else joinCount = 0;
    ...
    for (int i = 0; i < numTables; i++) {
      ...
      if (null != tmdMap.get(curTable.getAlias())) continue;
      ...
      PSTableMetaData tmdCur =
          dmdCur.getTableMetaData(curTable.getConnectionDetail().getOrigin(), curTable.getTable());
    ...
    int clausesSize;
    if (clauses == null) clausesSize = 0;
    else clausesSize = clauses.size();
    for (int i = 0; i < clausesSize; i++) {
      ...
      if (!((repl instanceof PSBackEndColumn) || (repl instanceof PSFunctionCall))) {
        throw new IllegalArgumentException(
            "where var must be backend col" + repl.getValueDisplayText());
      }

      // now add the WHERE to the driver mapping
      if (repl instanceof PSBackEndColumn) {
        curBuilder = (PSSqlQueryBuilder) builderMaps.get(((PSBackEndColumn) repl).getTable());
        curBuilder.addWhereClause(curWhere);
      } else if (repl instanceof PSFunctionCall) {
        PSFunctionParamValue[] params = ((PSFunctionCall) repl).getParamValues();

        for (int k = 0; k < params.length; k++) {
          if ((null != params[k]) && (params[k].isBackEndColumn())) {
            PSBackEndColumn bcol = (PSBackEndColumn) params[k].getValue();
            curBuilder = (PSSqlQueryBuilder) builderMaps.get(bcol.getTable());
            curBuilder.addWhereClause(curWhere);
            break;
          }
        }
    ...
    if (joinSize > 0) { // got some heterogeneous joins goin' on over here
      ...
      for (int i = 0; i < joinSize; i++) {
         ...
         */
        if (i == 0) {
          curBuilder = (PSSqlQueryBuilder) builderMaps.get(curJoin.getLeftColumn().getTable());
        ...
        curBuilder = (PSSqlQueryBuilder) builderMaps.get(curJoin.getRightColumn().getTable());
   ...
   */
  private static void addSelectColumn(PSBackEndColumn curCol, HashMap taken, Map builderMaps) {
    ...
    // find the builder for this column and add it to the select list
    PSSqlQueryBuilder curBuilder = (PSSqlQueryBuilder) builderMaps.get(curCol.getTable());
   ...
   *     sql query builder (<code>PSSqlQueryBuilder</code>) as value, assumed not <code>null</code>
   */
  private static void addSelectColumn(PSFunctionCall fnCall, HashMap taken, Map builderMaps) {
   ...
   *     sql query builder (<code>PSSqlQueryBuilder</code>) as value, assumed not <code>null</code>
   */
  private static void addSelectColumn(PSExtensionCall extCall, HashMap taken, Map builderMaps) {
   ...
   */
  private static java.util.Map createBuilderMaps(PSCollection joins, PSJoinTree jTree) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!hasTranslator && lTab.getServerKey().equals(rTab.getServerKey())) {
        ...
        if (hasTranslator) {
          ...
          // separate them
          if ((lBuilder != null) && (rBuilder != null) && (lBuilder == rBuilder)) {
            // leave the left builder alone, mark the right side
            rBuilder = null;
          }
        }

        if (lBuilder == null) builderMaps.put(lTab, new PSSqlQueryBuilder());

        if (rBuilder == null) builderMaps.put(rTab, new PSSqlQueryBuilder());
      }
    }
  ...
  private static boolean isIndexedLookupOptimal(
    ...
    PSTableMetaData rightTableMetaData = (PSTableMetaData) tmdMap.get(rightTable.getAlias());
    ...
    for (int i = 0; i < rightIndexStats.length && !hasUsefulIndex && !hasRowCount; i++) {
    ...
    PSTableMetaData leftTableMetaData = (PSTableMetaData) tmdMap.get(leftTable.getAlias());
  ...
  private static int estimateJoinCardinality(
    ...
    try {
      ...
      return estimateJoinCardinality(ah, ds, join, whereClauses, leftTableStats, rightTableStats);
    } catch (Throwable t) {
      ah.getLogHandler()
          .write(
              new com.percussion.log.PSLogExecutionPlan(
  ...
  private static int estimateJoinCardinality(
        ...
        .write(
            new com.percussion.log.PSLogExecutionPlan(
                ...
                new Object[] {
                  ...
                  ds.getName(),
                  rightTable.getAlias(),
                  String.valueOf(rightCardinality)
                }));

    int leftUniqueCount = leftStats.getDistinctRows(leftColName);
    int rightUniqueCount = rightStats.getDistinctRows(rightColName);

    if (join.isFullOuterJoin()) return leftCardinality * rightCardinality;
    if (join.isLeftOuterJoin()) return leftCardinality;
    if (join.isRightOuterJoin()) return rightCardinality;
        ...
        .write(
            new com.percussion.log.PSLogExecutionPlan(
                ...
                new Object[] {
                  ...
                  ds.getName(),
                  rightTable.getAlias(),
                  String.valueOf(rightUniqueCount)
                }));
    ...
    if (leftUniqueCount == 0 || rightUniqueCount == 0) {
      ...
      double selectivity =
          estimateJoinedSelectivity(ah, ds, leftCol, rightCol, whereClauses, leftStats, rightStats);
  ...
  private static double estimateJoinedSelectivity(
    ...
    try {
      joinClause = new PSWhereClause(leftCol, PSConditional.OPTYPE_EQUALS, rightCol, false);
      if (whereClauses == null) whereClauses = new PSCollection(joinClause.getClass());
          ...
          .write(
              new com.percussion.log.PSLogExecutionPlan(
                  ...
                  new Object[] {
                    ...
                    rightCol.getTable().getAlias(),
                    e.getLocalizedMessage()
                  }));
    }

    int clausesSize;
    if (whereClauses == null) clausesSize = 0;
    else clausesSize = whereClauses.size();
    ...
    for (int i = 0; i < clausesSize; i++) {
      ...
      if (op.equalsIgnoreCase(PSConditional.OPTYPE_EQUALS)
        ...
        selectivity *= 1.0 - (1.0 / (double) Math.max(rowCount, valueCardinality));
          ...
          || op.equalsIgnoreCase(PSConditional.OPTYPE_GREATERTHANOREQUALS)) selectivity *= 0.33;
      else if (op.equalsIgnoreCase(PSConditional.OPTYPE_BETWEEN)) selectivity *= 0.25;
      else if (op.equalsIgnoreCase(PSConditional.OPTYPE_NOTBETWEEN)) selectivity *= 0.75;
      else if (op.equalsIgnoreCase(PSConditional.OPTYPE_LIKE)) selectivity *= 0.1;
      else if (op.equalsIgnoreCase(PSConditional.OPTYPE_NOTLIKE)) selectivity *= 0.9;
    }

    if (joinClause != null) whereClauses.remove(joinClause);
  ...
  public static PSCollection getSortColumnsForXmlCollapsing(PSApplicationHandler ah, PSDataSet ds) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "page tank bad schema URL" + ah.getName() + " " + ds.getName());
    }

    boolean haveNonCollapsed = false;
    PSCollection collapseNodes =
        new PSCollection(com.percussion.design.objectstore.PSSortedColumn.class);
    ...
    for (int j = 0; j < maps.size(); j++) {
      ...
      String parent = (nodeParts.length >= 2) ? nodeParts[nodeParts.length - 2] : "";
      ...
      if ((occurs == PSDtdNode.OCCURS_OPTIONAL) || (occurs == PSDtdNode.OCCURS_ONCE)) {
        ...
        if (colMap instanceof com.percussion.design.objectstore.PSBackEndColumn) {
          addBackEndColumnAsSortedColumn(collapseNodes, (PSBackEndColumn) colMap);
          ...
          for (int i = 0; i < size; i++) {
            if (values[i].isBackEndColumn())
              addBackEndColumnAsSortedColumn(collapseNodes, (PSBackEndColumn) values[i].getValue());
          }
        }
  ...
  private static void addBackEndColumnAsSortedColumn(
    ...
    if (!sortedColumns.contains(sortedCol)) sortedColumns.add(sortedCol);
  }
  ...
  private static String[] parseXmlNodeName(String xmlField) {
    ...
    for (; (pos = xmlField.indexOf('/', lastPos)) != -1; ) {
      ...
      if (pos != 0) {
        ...
        if (!xmlStruct.contains(name)) xmlStruct.add(name);
      }
  ...
  static PSJoinTree validateJoins(java.util.List tables, java.util.List joins) {
    ...
    for (int i = 1; i < tableCount; i++) {
      ...
      if (!jTree.hasRoute(firstTable, tab)) {
        throw new IllegalArgumentException(
            "no join path between tables" + firstTable.getAlias() + " " + tab.getAlias());
      }
    }
