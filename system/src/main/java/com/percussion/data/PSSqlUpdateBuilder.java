...
public class PSSqlUpdateBuilder extends PSSqlBuilder {
  ...
  void addUpdateColumn(PSBackEndColumn col) throws PSIllegalArgumentException {
    ...
    if (!m_Columns.contains(col)) m_Columns.add(col);
  }
  ...
  void addKeyColumn(PSBackEndColumn col) throws PSIllegalArgumentException {
    ...
    if (!m_Keys.contains(col)) m_Keys.add(col);
  }
  ...
  void addColumnToXmlMapping(PSDataMapping map) throws PSIllegalArgumentException {
    ...
    if (!(col
        ...
        com.percussion.design.objectstore
            .PSBackEndColumn)) { // UDF's are not yet supported on the SQL side of an UPDATE
    ...
    if (m_Mappings.get(colName) == null) m_Mappings.put(colName, map);
  }
  ...
  PSUpdateStatement generateUpdate(
    ...
    try {
      return new PSUpdateStatement(iConnKey, context.getBlocks(), PSUpdateStatement.TYPE_UPDATE);
  ...
  protected void buildTableName(
      PSBackEndLogin login, PSSqlBuilderContext context, PSBackEndTable table) {
  ...
  protected void buildWhereClauseFromKeys(
    ...
    if (m_Keys != null) {
      ...
      if (!buildColumnAndPlaceholderList(context, table, datatypes, m_Keys, " AND ", false)) {
        /* updates without WHERE clauses tend to be dangerous, so
         * we'll warn about this, but not give an error as it may
         * be intentional.
         */
  ...
  protected boolean buildColumnAndPlaceholderList(
      ...
      throws PSIllegalArgumentException {
    return buildColumnAndPlaceholderList(
        context, table, datatypes, columnList, delimiter, ignoreAutoIncrements, null);
  }
  ...
  protected boolean buildColumnAndPlaceholderList(
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!(beMap
          instanceof
          com.percussion.design.objectstore
              .PSBackEndColumn)) { // UDF's are not yet supported in UPDATEs
       ...
       */
      if (ignoreAutoIncrements && isAutoIncrement(table, columnName)) continue;

      colCount++;
      if (colCount > 1) context.addText(delimiter);
  ...
  protected PSTableMetaData getTableMetaData(PSBackEndTable table) throws SQLException {
    ...
    return (dbMeta == null)
        ? null
        : dbMeta.getTableMetaData(table.getConnectionDetail().getOrigin(), table.getTable());
  }
  ...
  protected boolean isAutoIncrement(PSBackEndTable table, String columnName) {
    ...
    if (m_autoIncrCols == null) {
      try {
        ...
        Object[] args = {
          table.getDataSource(), "", "", "getAutoUpdateColumns", PSSqlException.toString(e)
        };
        com.percussion.log.PSLogManager.write(
            new com.percussion.log.PSLogServerWarning(
  ...
  protected void buildColumnList(
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (table.equals(col.getTable())) {
         ...
         */
        if (isAutoIncrement(table, columnName)) continue;

        colCount++;
        if (colCount > 1) context.addText(", ");
  ...
  PSUpdateStatement generateInsert(
    ...
    try {
      return new PSUpdateStatement(iConnKey, context.getBlocks(), PSUpdateStatement.TYPE_INSERT);
    } catch (PSDataExtractionException e) {
      throw new PSIllegalArgumentException(e.getErrorCode(), e.getErrorArguments());
    }
  }
  ...
  PSUpdateStatement generateUpdateInsert(
    ...
    buildColumnList(insertContext, table, dtHash, false, /* use colname */ columnList);
    insertContext.addText(") VALUES (");
    buildColumnList(insertContext, table, dtHash, true, /* use placeholder */ columnList);
    insertContext.addText(")");
    insertContext.closeTextRun();

    try {
      return new PSUpdateInsertStatement(iConnKey, context.getBlocks(), insertContext.getBlocks());
    } catch (PSDataExtractionException e) {
      throw new PSIllegalArgumentException(e.getErrorCode(), e.getErrorArguments());
    }
  }
