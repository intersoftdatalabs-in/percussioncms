...
/**
 * The PSConditionalEvaluator class is used to perform evaluate conditions, returning <code>true
 * </code> if the conditions are met or <code>false</code> otherwise.
 *
...
public class PSConditionalEvaluator {
  ...
  public boolean isMatch(PSExecutionData data) throws PSEvaluationException {
    ...
    if (handler instanceof PSDebugLogHandler) dh = (PSDebugLogHandler) handler;
    ...
    for (int i = 0; i < m_tokens.size(); i++) {
      ...
      if (!popFlag) {
        if (pushIn == null) myStack.push(nullId);
        else myStack.push(pushIn);
      } else {
        ...
        if ((opCode == OPCODE_ISNULL) || (opCode == OPCODE_ISNOTNULL)) {
          ...
          if (variable instanceof List) {
            ...
            try {
              variable = PSDataConverter.convert(variable, PSDataConverter.DATATYPE_NULL);
          ...
          if ((opCode != OPCODE_AND) && (opCode != OPCODE_OR)) {
            ...
            // trace the result
            if (dh != null && dh.isTraceEnabled(traceFlag)) {
              Object[] args = {variable, getStringOperator(opCode), value, result};
              dh.printTrace(traceFlag, args);
            }
            ...
            if (opCode == OPCODE_AND) result = (popBoolOne && popBoolTwo);
            else result = (popBoolOne || popBoolTwo);

            myStack.push(String.valueOf(result));

            // trace the result
            if (dh != null && dh.isTraceEnabled(traceFlag)) {
              Object[] args = {popStringOne, getStringOperator(opCode), popStringTwo, result};
              dh.printTrace(traceFlag, args);
            }
          }
        }
  ...
  /**
   * Set the data extractors which can be used to retrieve the parameter values at run-time. For
   * each instance of the evaluator, a different set of parameter values may be used.
   *
   * <p>IPSDataExtractors provide a transparent mechanism for accessing data. The PSExecutionData
   * object passed in to the evaluator's execute method is used with the extractor to load the
   * runtime value.
   *
   * @param extractors an array of IPSDataExtractor objects defining the parameter values for this
   *     instance
   */
  public void setParamValues(IPSDataExtractor[] extractors) throws PSParameterMismatchException {}
  ...
  public String[] getBackEndColumnList(PSRequest request) {
    if (m_backEndColNames == null) return new String[0];
   ...
   *
   * <p>The logic we want to follow here is that all terms ANDed together are evaluated first. There
   * results can then be checked with all the ORs. For instance, when the following conditions
   * exist:
   *
   * <p>
   *
   ...
   * </TABLE>
   *
   * <p>In the above example, it is important that the first two conditions be evaluated, then the
   * last two conditions and finally the OR between the two computed values. To accomplish this
   * task, we will act as a boolean prefix calculator. The following set of instructions will be
   * stored as the execution steps:
   *
   * <p>
   *
   * <TABLE BORDER="1">
   ...
   * </TABLE>
   *
   * <p>Let's assume we have t1.c1 = 0 and t1.c2 = Y. The following execution would occur:
   *
   * <p>
   *
   * <TABLE BORDER="1">
   ...
   */
  private void tokenize(PSCollection conds) throws PSEvaluationException {
    if ((conds == null) || (conds.size() == 0)) { // no conditions means always match!
      m_tokens = null;
      return;
    }

    if (m_backEndColNames == null) m_backEndColNames = new ArrayList();
    ...
    if (!com.percussion.design.objectstore.PSConditional.class.isAssignableFrom(
      ...
      throw new IllegalArgumentException(
          "coll bad content type, Conditional Handler: " + conds.getMemberClassName());
    }
    ...
    for (int i = 0; i < conds.size(); i++) {
      ...
      if (!cur.isUnary()) {
        if (cur.getValue() == null) {
          String arg = "null";
          throw new PSEvaluationException(IPSDataErrors.UNKNOWN_OPCODE_LOAD_TYPE, arg);
        }
      ...
      if (preBoolType == OPCODE_AND) {
        ...
        else { // opBool.equals(cur.OPBOOL_OR) || opBool.equals(cur.OPBOOL_AND)
          orCount += 1;
          preBoolType = intOpBool(opBool);
        }
      }
  ...
  private int intOpBool(String opType) {
    ...
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_NOTEQUALS)) return OPCODE_NOTEQUALS;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_LESSTHAN)) return OPCODE_LESSTHAN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_LESSTHANOREQUALS))
      return OPCODE_LESSTHANOREQUALS;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_GREATERTHAN)) return OPCODE_GREATERTHAN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_GREATERTHANOREQUALS))
      return OPCODE_GREATERTHANOREQUALS;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_ISNULL)) return OPCODE_ISNULL;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_ISNOTNULL)) return OPCODE_ISNOTNULL;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_BETWEEN)) return OPCODE_BETWEEN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_NOTBETWEEN)) return OPCODE_NOTBETWEEN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_IN)) return OPCODE_IN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_NOTIN)) return OPCODE_NOTIN;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_LIKE)) return OPCODE_LIKE;
    else if (opType.equalsIgnoreCase(PSConditional.OPTYPE_NOTLIKE)) return OPCODE_NOTLIKE;
    else if (opType.equalsIgnoreCase(PSConditional.OPBOOL_AND)) return OPCODE_AND;
    else if (opType.equalsIgnoreCase(PSConditional.OPBOOL_OR)) return OPCODE_OR;
    else throw new IllegalArgumentException("Illegal op type: " + opType);
  }
   ...
   */
  private static boolean makeComparable2Lists(List leftList, List rightList, int opCode)
      throws PSEvaluationException {
    ...
    switch (opCode) {
      case OPCODE_EQUALS:
        {
          if (leftList.isEmpty() && rightList.isEmpty()) return true;

          if (leftListSize != rightListSize) return false; // they can't be equal
      ...
      case OPCODE_NOTEQUALS:
        {
          if (leftList.isEmpty() && rightList.isEmpty()) return false; // empty is equal to empty
      ...
      case OPCODE_GREATERTHANOREQUALS:
        {
          ...
          while (itLeft.hasNext()) {
            ...
            while (itRight.hasNext()) {
              if (!makeComparable2Obj(leftObj, itRight.next(), opCode)) return false;
            }
          }
      ...
      case OPCODE_IN:
        {
          if (leftList.isEmpty() && rightList.isEmpty()) return true; // empty is IN empty
      ...
      case OPCODE_NOTIN:
        {
          if (leftList.isEmpty() && rightList.isEmpty()) return false; // empty is IN empty
  ...
  private static boolean makeComparable2Obj(Object left, Object right, int opCode)
    ...
    */
    if ((opCode == OPCODE_IN)
        || (opCode == OPCODE_NOTIN)
        || (opCode == OPCODE_BETWEEN)
        || (opCode == OPCODE_NOTBETWEEN)) {
      try {
        ...
        Object[] args = {PSDataConverter.getTypeString(rightType), getStringOperator(opCode)};
    ...
    if ((opCode == OPCODE_BETWEEN) || (opCode == OPCODE_NOTBETWEEN)) {
      if (((rightType & PSDataConverter.DATATYPE_SET_FLAG) == 0) || ((List) right).size() != 2) {
        Object[] args = {PSDataConverter.getTypeString(rightType), getStringOperator(opCode)};
        throw new PSEvaluationException(IPSDataErrors.OPERATOR_INVALID_FOR_TYPE, args);
      }
    }
    ...
    if (left instanceof PSDateLiteral) dateFormat = ((PSDateLiteral) left).getDateFormat();
    else if (right instanceof PSDateLiteral) dateFormat = ((PSDateLiteral) right).getDateFormat();
    else if (right instanceof PSLiteralSet) dateFormat = getDateFormat((PSLiteralSet) right);
    ...
    */
    ...
    try {
      ...
      Object[] args = {
        PSDataConverter.getTypeString(rightType), PSDataConverter.getTypeString(bestType), right
      };
    ...
    switch (bestType) {
      ...
      case PSDataConverter.DATATYPE_DATE:
        if (right instanceof PSLiteralSet) {
          ...
          try {
            result = compareNumber(opCode, leftDate.getTime(), rightDate.getTime());
          } catch (PSEvaluationException e) {
  ...
  private static FastDateFormat getDateFormat(PSLiteralSet literalSet) {
    ...
    if (literalSet == null) return dateFormat;
   ...
   */
  private static boolean compareWithNumericSet(
      int opCode, Object left, Object right, FastDateFormat dateFormat)
      throws PSEvaluationException {
    String leftName = left.getClass().getName();
    String rightName = right.getClass().getName();

    if ((opCode != OPCODE_BETWEEN)
        && (opCode != OPCODE_NOTBETWEEN)
        && (opCode != OPCODE_IN)
        && (opCode != OPCODE_NOTIN)) {
      ...
      for (int i = 0; i < numSet.size(); i++) {
        try {
          ...
          if (convertTextLiteralToDate) {
            ...
            if (PSDataConverter.compare(left, obj) == 0) return (opCode == OPCODE_IN);

          } else {
            if (PSDataConverter.compare(left, numSet.get(i)) == 0) return (opCode == OPCODE_IN);
          }
    ...
    switch (opCode) {
      ...
      case OPCODE_NOTIN:
        ...
        for (int i = 0; i < setSize; i++) {
          if (isTextLiteralSet == true) rightString = ((PSTextLiteral) (numSet.get(i))).getText();
        ...
        if (opCode == OPCODE_NOTIN) result = !result;
