...
public class PSResultSetXmlConverter implements IPSResultSetConverter {
  ...
  public PSResultSetXmlConverter(PSApplicationHandler app, PSDataSet def)
    ...
    for (int j = 0; j < maps.size(); j++) addXmlNode((PSDataMapping) maps.get(j));
    ...
    if (pager != null) {
      if (pager.getMaxRowsPerPage() < 1) {
        ...
        */
        m_prevLink =
            new PSPagedRequestLinkGenerator(app, def, PSPagedRequestLinkGenerator.RPL_TYPE_PREV);

        m_nextLink =
            new PSPagedRequestLinkGenerator(app, def, PSPagedRequestLinkGenerator.RPL_TYPE_NEXT);

        m_indexLink =
            new PSPagedRequestLinkGenerator(app, def, PSPagedRequestLinkGenerator.RPL_TYPE_INDEXED);
      }
    }
    ...
    if (resPageSet != null) m_resultPageSet = resPageSet;
    PSCollection pages;
    if (((pages = m_resultPageSet.getResultPages()) != null) && (pages.size() != 0)) {
      /* we're not using a style sheet! */
      PSResultPage page;
      PSCollection links;
      PSRequestLink link;
      ...
      for (int i = 0; i < pages.size(); i++) {
        ...
        try {
          m_StyleSheetEvaluators[i] =
              new PSSetStyleSheetEvaluator(
                  m_appHandler.getLocalizedURL(page.getStyleSheet()), page, page.getConditionals());
        } catch (java.net.MalformedURLException e) {
          throw new IllegalArgumentException(
              "style sheet bad URL"
                  + m_appHandler.getName()
  ...
  public void convert(PSExecutionData data, IPSResultSetDataFilter filter)
    ...
    if (doc == null) {
      ...
      if (contentHeader == null) {
        ...
        else mimeType = IPSMimeContentTypes.MIME_TYPE_TEXT_XML;
  ...
  protected boolean isSupported(String reqPageURL) {
    ...
    if (reqPageURL.endsWith(".xml") || reqPageURL.endsWith(".txt")) return true;
  ...
  protected Document createXmlDocument(
    ...
    try {
      ...
      for (int i = 0; i < cols.length; i++) {
        ...
        if ((col == null) || (col.length() == 0)) cols[i] = "col" + String.valueOf(i + 1);
        else cols[i] = col.toLowerCase();
      }
      ...
      if (m_maxRowsPerPage > 0) {
        ...
        for (int i = 1; i < pagerFirstRow; i++) {
          if (rs.next()) curRowNumber++;
          else break;
        }
      }
      PSFilterResultSetWrapper frsw = new PSFilterResultSetWrapper(data, rs, filter);
      ...
      while (dataBuf.readRow()) {
        ...
        if ((dh.isTraceEnabled(traceResultSetFlag)) && (curRow != null)) {
          ...
          for (int i = 0; i < curRow.length; i++) traceRow[i] = curRow[i];
        ...
        if (m_maxRowsPerPage > 0) {
          ...
          if (rowsSelected >= m_maxRowsPerPage) break;
          if ((m_maxPages > 0) && (curRowNumber >= m_maxRowsPerPage * m_maxPages)) break;
        }
      }
      ...
      */
      ...
      if (m_maxRowsPerPage > 0) {
        ...
        int curPage = ((pagerFirstRow + m_maxRowsPerPage) - 1) / m_maxRowsPerPage;
        ...
        if (m_maxPageLinks > 0 || m_maxPageLinks < 0) {
          ...
          if (m_maxPageLinks > 0) maxPageLinks = m_maxPageLinks;
          ...
          if (m_maxPages == 1) maxPagesAhead = 0;
          else if (m_maxPages > 1) maxPagesAhead = m_maxPages - curPage;

          maxRowsAhead = (maxPagesAhead * m_maxRowsPerPage) - 1;

          if (m_maxRowsPerPage == 1) maxRowsAhead += 1;
        ...
        /* Partial page? */
        if (curRowNumber % m_maxRowsPerPage > 0) numPages++;
        ...
        if (numPages > 1) {
          ...
          end = (start + m_maxPageLinks) < numPages ? (start + m_maxPageLinks) - 1 : numPages - 1;
      ...
      if (request != null) {
        ...
        if (stats != null) stats.incrementRowsSelected(rowsSelected);
      }
    } catch (Throwable t) {
    ...
    if (dh.isTraceEnabled(PSTraceMessageFactory.POST_EXIT_XML_FLAG)) {
      ...
      dh.printTrace(PSTraceMessageFactory.POST_EXIT_XML_FLAG, traceArgs);
    }
    ...
    if (dh.isTraceEnabled(PSTraceMessageFactory.POST_EXIT_CGI_FLAG))
      m_appHandler.getLogHandler().printTrace(PSTraceMessageFactory.POST_EXIT_CGI_FLAG, request);
  ...
  private boolean processNodes(Node node) {
    ...
    if (isNodeStyleSheetProcessingInstruction(node)) return true;
    else {
      if (node.hasChildNodes()) {
        ...
        while ((kid = kids.item(i++)) != null) {
          ...
          if (kidHasStyleSheet) return true;
        }
      }
    }
    return false;
  ...
  private void applyStylesheetConditions(PSExecutionData data, Document doc, boolean fixupURL) {
     ...
     */
    if (m_requestor.getMimeType("txt") == null && reqExt.equals(".txt")) return;
    ...
    if (styleSheetURL != null) {
      ...
      if (fixupURL) urlText = m_appHandler.getExternalURLString(styleSheetURL);
      else urlText = styleSheetURL.toExternalForm();

      int extPos = urlText.lastIndexOf('.');
      if (extPos == -1) ssType = "xsl"; // assume it's XSL by default
      else ssType = urlText.substring(extPos + 1).toLowerCase();
  ...
  private PSXmlNode addXmlNode(PSDataMapping map) {
    ...
    dataExtract =
        PSDataExtractorFactory.createReplacementValueExtractor((IPSReplacementValue) beMap);

    return addXmlNode(xmlField, dataExtract, map.getConditionals(), map.getTextFormatter());
  }

  private PSXmlNode addXmlNode(
      String xmlField,
      IPSDataExtractor dataExtract,
    ...
    for (; (pos = xmlField.indexOf('/', lastPos)) != -1; ) {
       ...
       */
      if (pos != 0) names.add(xmlField.substring(lastPos, pos));
    ...
    for (pos = 0; pos < nodeCount; pos++) {
          ...
          if (conditionals != null && conditionals.size() > 0) {
            xmlNode.addConditionalNode(
                new PSXmlNode(
                    ...
                    textFormatter,
                    false));
          } else xmlNode.setDataSource(dataExtract);
        }
      ...
      if (pos == 0) { // this is the root, set it as such
        if (m_xmlRoot != null) {
  ...
  private void storeRowData(
      PSXmlNode node, PSExecutionData data, Document doc, PSXmlNodeData[] dataStruct)
      throws com.percussion.data.PSDataExtractionException {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (xNode.isForData()) xNode.setValue(data, doc, dataStruct);
  ...
  public String getMimeTypeForRequestPage(int index, PSExecutionData data) {
    ...
    try {
      IPSDataExtractor extractor = PSDataExtractorFactory.createReplacementValueExtractor(val);

      if (extractor != null) {
        Object mimeType = extractor.extract(data);
        if (mimeType != null) return mimeType.toString();
      }
  ...
  public PSResultPage getResultPage(int index) {
    ...
    if (m_resultPageSet == null) return null;
  ...
  public static String getMimeTypeForRequestor(
      PSRequestor requestor, String extension, PSExecutionData data) {
    try {
      ...
      if (extractor != null) obj = extractor.extract(data);
  ...
  class PSXmlNodeSorter implements PSDtdTreeVisitor {
    public PSXmlNodeSorter(PSXmlNode xmlRoot, PSDtdTree dtdTree) {
      ...
      if ((m_xmlRoot != null) && (m_dtdTree != null)) {
        ...
        if (root != null) root.acceptVisitor(this, null);
      }
    }
    ...
    public Object visit(PSDtdElementEntry node, Object data) {
      ...
      if (parentNode == null) { // not much to do here, we're sorted just fine
  ...
  class PSXmlNode {
    ...
    PSXmlNode(
      ...
      if ((conditionals != null) && (conditionals.size() != 0)) {
        try {
          m_conditionChecker = new PSConditionalEvaluator(conditionals);
        } catch (IllegalArgumentException e) {
          throw new IllegalArgumentException(e.getLocalizedMessage());
        }
      } else m_conditionChecker = null;
      ...
      if ((m_DTDTree != null) && (m_parent != null)) {
        ...
        m_maxOccurrences = PSDtdNode.OCCURS_ONCE;
      } else if ((m_flags & FLAG_IS_ATTRIBUTE)
          == FLAG_IS_ATTRIBUTE) { // safe to assume the attribute is 0 or 1
        m_maxOccurrences = PSDtdNode.OCCURS_OPTIONAL;
      } else { // for anything else, we really have no clue
      ...
      if (m_children == null) return false;
        ...
        for (int i = 0; i < m_children.size(); i++) {
          ...
          if ((curChild.m_maxOccurrences != PSDtdNode.OCCURS_ONCE)
            ...
            if (curChild.doesXmlNodeContainMultipleOccurrenceElement()) return true;
          }
        }
    ...
    boolean requestCollapsePrivilege(PSXmlNode node) {
      if (node == null) { // if this is null, there's not much to do
        return false;
      } else if (node.getParent()
          == null) { // the root is non-repeating, which is what null parent implies
        return false;
      } else if ((m_flags & FLAG_CHECKED_NODEKIDS)
          == FLAG_CHECKED_NODEKIDS) { // We've already checked, return result
        return ((m_flags & FLAG_COLLAPSEABLE_NODE) == FLAG_COLLAPSEABLE_NODE);
      } else if ((m_maxOccurrences == PSDtdNode.OCCURS_ONCE)
          || (m_maxOccurrences
              == PSDtdNode.OCCURS_OPTIONAL)) { // if we're non-repeating, the node can't collapse us
        return false;
      } else {
    ...
    private boolean setConditionalValue(
        PSExecutionData data, Document doc, PSXmlNodeData[] dataStructs)
        throws PSDataExtractionException {
      Element elementNode;
      PSXmlNode parent;

      PSDebugLogHandler dh = (PSDebugLogHandler) data.getLogHandler();
      boolean traceEnabled = dh.isTraceEnabled(PSTraceMessageFactory.MAPPER_FLAG);
      ...
      if (traceEnabled && m_dataSource != null) {
        ...
        if (values != null && values.length > 0) dataName = values[0].getValueDisplayText();
        else dataName = "??"; // there should always be at least one
      ...
      PSXmlNodeData parentData = (m_parent == null) ? null : dataStructs[m_parent.m_nodeId];
      ...
      if ((m_flags & FLAG_IS_ATTRIBUTE) == FLAG_IS_ATTRIBUTE) {
        ...
        if (m_dataSource != null) {
          ...
          if ((nodeData.m_curValue != null)
            ...
            if (nodeData.m_curValue.equals(curValue.toString())) return true;
          }
        }
        ...
        if (curValue instanceof Element) {
          ...
          if (!elementNode.getTagName().equals(m_nodeName))
            throw new PSDataExtractionException(
                0, "The value element name must match the mapped element name.");

          // if this has no parent, it must be the root element
          if (m_parent == null) {
            doc.appendChild(doc.importNode(elementNode, true));
          } else {
            if (parentData.m_nodeElement == null) m_parent.setValue(data, doc, dataStructs);

            parentData.m_nodeElement.appendChild(doc.importNode(elementNode, true));
          }
        } else {
          ...
          if (m_dataSource != null) {
            ...
            Text StringNode =
                doc.createTextNode(PSXmlDocumentBuilder.normalize(curValue.toString()));
    ...
    void clearElementData(PSXmlNodeData[] dataStructs) {
      ...
      for (PSXmlNode cur = this; cur != null; cur = cur.m_parent) {
    ...
    Object computeValue(PSExecutionData data) throws com.percussion.data.PSDataExtractionException {
      ...
      if (o == null) return "";
        ...
        if (o instanceof Document) return ((Document) o).getDocumentElement();
    ...
    boolean hasChanged(PSXmlNodeData[] dataStructs, PSExecutionData data)
      ...
      if ((m_conditionChecker != null) && !m_conditionChecker.isMatch(data)) return false;

      Object newValue = computeValue(data);
      Object oldValue = getValue(dataStructs);
      if (newValue instanceof Element) {
        if (!(oldValue instanceof Element)) return true;
    ...
    boolean areAllParentsSet(PSXmlNodeData[] dataStructs) {
      ...
      if (dataStructs[m_parent.m_nodeId].m_nodeElement == null) return false;
    ...
    void setCollapseKey(boolean enable) {
      if (enable) m_flags |= FLAG_IS_COLLAPSE_KEY;
      else m_flags &= ~FLAG_IS_COLLAPSE_KEY;
    }
    ...
    String getFullName() {
      if (m_parent == null) return m_nodeName;
  ...
  class PSXmlCollapseNode {
    ...
    void clear(PSXmlNodeData[] dataStructs) {
      if (m_clearNode != null) m_clearNode.clearChildElementData(dataStructs);
    }

    boolean needsClear(PSXmlNodeData[] dataStructs, PSExecutionData data)
        throws com.percussion.data.PSDataExtractionException {
      if (m_collapseNode != null) return m_collapseNode.hasChanged(dataStructs, data);
      else return false;
    }
