...
public class PSRequestRedirector implements IPSResultGenerator {
  ...
  public void generateResults(PSExecutionData data) {
    ...
    if (m_appendURLExtension) url += request.getRequestPageExtension();
    ...
    if (m_useHttpRedirectResponse) sendHttpRedirectResponse(request, url);
    else {
  ...
  public void sendHttpRedirectResponse(PSRequest request, String url) {
    ...
    try {
      ...
      String errText = "Failed to send redirection response, exception text:" + e.toString();

      Object[] args = {request.getUserSessionId(), errText};

      PSResponseSendError err =
