...
/**
 * The IPSStyleSheetMerger interface must be implemented by all processors capable of merging an XML
 * document with a type of style sheet to generate HTML output.
 *
 * <p>The style sheet implementations must be named PS{Type}StyleSheetMerger where {Type} represents
...
public interface IPSStyleSheetMerger {
  /**
   * Merge the style sheet defined in the XML document to generate HTML output. The <code>stylesheet
   * </code> processing instruction must exist in the XML document and define the style sheet to
   * use.
   *
   * @param req the request document which may provide additional context information required by
   *     the processor
