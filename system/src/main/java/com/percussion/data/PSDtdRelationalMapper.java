...
/**
 * ************************************************************************** The
 * PSDtdRelationalMapper class is used to map from an XML DTD to a collection of relational table
 * schemata. These tables have implicit, invisible, unique ID columns that are used with joining to
 * represent the link between parent and child in the data source.
 * *************************************************************************
 */
public class PSDtdRelationalMapper implements PSDtdTreeVisitor {
  ...
  public void printTableDefs(PrintStream out) {
    for (Iterator i = m_tablesInOrder.iterator(); i.hasNext(); ) {
      ...
      for (int j = 1; j <= table.getNumColumns(); j++) {
        ...
        if (fkey != null) {
          out.print(" (->" + fkey.getTable().getName() + "." + fkey.getName() + ")");
        }
        out.print(" ");
      }
      out.println("\r\n\r\n");
  ...
  public Object visit(PSDtdElementEntry node, Object data) {
    ...
    occ == PSDtdNode.OCCURS_ATLEASTONCE || occ == PSDtdNode.OCCURS_ANY) {
      ...
      //          {
      //             throw new RuntimeException("Error: " + el.getName() + " has no parent
      // element.");
      //          }
      ...
      if (parentIDColumn == null) {
        ...
        parentTable.setUniqueIDColumn(new ColumnDef(makeIdColName(parentTable.getName())));
        parentIDColumn = parentTable.getUniqueIDColumn();
      }

      ColumnDef parentJoin = new ColumnDef(makeParentIdColName(el.getName()), parentIDColumn);
      ...
      if (hasCharData) {
        ...
        else parentTable.addColumn(new ColumnDef(el.getName()));
      }
      ...
      for (int i = 0; i < el.getNumAttributes(); i++) {
        ...
        if (m_currentColumnPrefix.length() > 0)
          parentTable.addColumn(
              new ColumnDef(m_currentColumnPrefix + "/" + el.getName() + "/@" + attr.getName()));
        else parentTable.addColumn(new ColumnDef(el.getName() + "/@" + attr.getName()));
      }

      String oldPrefix = m_currentColumnPrefix;
      if (m_currentColumnPrefix.length() > 0)
        m_currentColumnPrefix = m_currentColumnPrefix + "/" + el.getName();
      else m_currentColumnPrefix = el.getName();
  ...
  public class TableDef {
    ...
    public PSBackEndColumn getBackEndColumn(String colName) {
      ...
      PSBackEndColumn backEndCol = new PSBackEndColumn(m_backEndTable, colDef.getName());
      return backEndCol;
    }
    ...
    public boolean dependsOn(TableDef possDep) {
      ...
      for (int i = 1; i <= this.getNumColumns(); i++) // 1-based
      {
        ...
        if (parentTable.getName().equals(possDep.getName())) return true;
    ...
    public void initResultSetMetaData(PSResultSetMetaData meta) throws SQLException {
      for (int i = 1; i <= this.getNumColumns(); i++) // 1-based
      {
