...
public class PSUserContextExtractor extends PSDataExtractor {
  ...
  public PSUserContextExtractor(PSUserContext source) {
    ...
    if (m_sourceType == TOK_USER_ATTRIBUTE) {
      ...
      m_sourceName = m_sourceName.substring(PREFIX_USER_SESSION_OBJECT.length());
    }
  }
  ...
  private static int getType(String contextFieldName) {
    ...
    if ("sessionid".equals(contextFieldName)) sourceType = TOK_SESSION_ID;
    else if ("sessioncreatetime".equals(contextFieldName)) sourceType = TOK_SESSION_CREATE_TIME;
    else if ("user/name".equals(contextFieldName)) sourceType = TOK_USER_NAME;
    else if ("user/securityprovider".equals(contextFieldName)) sourceType = TOK_SEC_PROVIDER;
      ...
      sourceType = TOK_USER_SESSIONOBJECT;
        ...
        .equals(contextFieldName)) // fixed bug id Rx-99-10-0208 (was using role/rolename)
    sourceType = TOK_ROLE_NAME;
    else if ("roles/rolenameinclause".equals(contextFieldName)) sourceType = TOK_ROLE_NAME_INCLAUSE;
    else if ("dataaccessrights/query".equals(contextFieldName)) sourceType = TOK_DATA_ACC_QUERY;
    else if ("dataaccessrights/insert".equals(contextFieldName)) sourceType = TOK_DATA_ACC_INSERT;
    else if ("dataaccessrights/update".equals(contextFieldName)) sourceType = TOK_DATA_ACC_UPDATE;
    else if ("dataaccessrights/delete".equals(contextFieldName)) sourceType = TOK_DATA_ACC_DELETE;
    ...
    if (sess != null) {
      switch (sourceType) {
        ...
        case TOK_USER_ATTRIBUTE:
          {
            ...
            if (size != 0) {
              ...
              for (int i = 0; i < size; i++) {
                if (sourceType == TOK_USER_NAME) valueArray[i] = users[i].getName();
                else if (sourceType == TOK_SEC_PROVIDER) valueArray[i] = "";
                else if (sourceType == TOK_SEC_PROVIDER_TYPE_ID) valueArray[i] = "";
                else if (sourceType == TOK_SEC_PROVIDER_INST) valueArray[i] = "";
                  ...
                  try {
                    ...
                    if (pos > 0) {
                       ...
                       */
                      values = getSubjectAttributeValues(users[i], sourceName);

                      if (values.isEmpty()) {
                        // get security provider specific attributes
                        PSUserAttributes attribs = users[i].getAttributes();
                        if (attribs != null) {
                    ...
                    if (valueArray[i] == null) valueArray[i] = defValue;
        ...
        case TOK_ROLE_NAME_INCLAUSE:
          try {
            ...
            if (null == users || users.length == 0) {
              ...
              if (buf.length() > 0) inclause = buf.toString();
            }
            value = inclause;
          } catch (PSSecurityException e) {
            throw new PSDataExtractionException(e.getErrorCode(), e.getErrorArguments());
        ...
        case TOK_DATA_ACC_QUERY:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DATA_QUERY) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DATA_ACC_INSERT:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DATA_CREATE) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DATA_ACC_UPDATE:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DATA_UPDATE) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DATA_ACC_DELETE:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DATA_DELETE) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DESIGN_ACC_MOD_ACL:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DESIGN_MODIFY_ACL)
              != 0) {
            value = "1";
          } else value = "0";
          break;
        ...
        case TOK_DESIGN_ACC_READ:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DESIGN_READ) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DESIGN_ACC_UPDATE:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DESIGN_UPDATE) != 0) {
            value = "1";
          } else value = "0";
          break;

        case TOK_DESIGN_ACC_DELETE:
          if ((request.getCurrentApplicationAccessLevel() & PSAclEntry.AACE_DESIGN_DELETE) != 0) {
            value = "1";
          } else value = "0";
          break;
        ...
        case TOK_SERVER_ADMIN_ACCESS:
          {
            ...
            try {
              ...
              throw new PSDataExtractionException(e1.getErrorCode(), e1.getErrorArguments());
            } catch (PSAuthorizationException e1) {
              throw new PSDataExtractionException(e1.getErrorCode(), e1.getErrorArguments());
            }
            switch (sourceType) {
              case TOK_SERVER_DATA_ACCESS:
                if ((accessLevel & PSAclEntry.SACE_ACCESS_DATA) != 0) value = "1";
                else value = "0";
                break;
              case TOK_SERVER_DESIGN_ACCESS:
                if ((accessLevel & PSAclEntry.SACE_ACCESS_DESIGN) != 0) value = "1";
                else value = "0";
                break;
              case TOK_SERVER_CREATEAPPLICATIONS_ACCESS:
                if ((accessLevel & PSAclEntry.SACE_CREATE_APPLICATIONS) != 0) value = "1";
                else value = "0";
                break;
              case TOK_SERVER_DELETEAPPLICATIONS_ACCESS:
                if ((accessLevel & PSAclEntry.SACE_DELETE_APPLICATIONS) != 0) value = "1";
                else value = "0";
                break;
              case TOK_SERVER_ADMIN_ACCESS:
                if ((accessLevel & PSAclEntry.SACE_ADMINISTER_SERVER) != 0) value = "1";
                else value = "0";
                break;
            }
          }
  ...
  public static Element toXml(PSRequest request) {
    ...
    PSXmlDocumentBuilder.addElement(
        doc, root, "SessionCreateTime", sdf.format(sess.getCreateTimeStamp()));
    ...
    for (int i = 0; i < users.length; i++) {
      ...
      if (attributes != null) {
        ...
        while (keys.hasNext()) {
          ...
          PSXmlDocumentBuilder.addElement(doc, user, "Value", attributes.getString(key));
        }
      }
    ...
    try {
      ...
      roles.appendChild(
          doc.createTextNode(
              "Unable to obtain roles for the following reason: " + e.getLocalizedMessage()));
    }
    ...
    try {
      ...
      rights.setAttribute(
          "query", (String) getUserContextInfo(TOK_DATA_ACC_QUERY, null, request, null));
      rights.setAttribute(
          "insert", (String) getUserContextInfo(TOK_DATA_ACC_INSERT, null, request, null));
      rights.setAttribute(
          "update", (String) getUserContextInfo(TOK_DATA_ACC_UPDATE, null, request, null));
      rights.setAttribute(
          "delete", (String) getUserContextInfo(TOK_DATA_ACC_DELETE, null, request, null));

      // add DesignAccessRights node
      rights = PSXmlDocumentBuilder.addEmptyElement(doc, root, "DesignAccessRights");
      rights.setAttribute(
          "modifyAcl", (String) getUserContextInfo(TOK_DESIGN_ACC_MOD_ACL, null, request, null));
      rights.setAttribute(
          "readDesign", (String) getUserContextInfo(TOK_DESIGN_ACC_READ, null, request, null));
      rights.setAttribute(
          "updateDesign", (String) getUserContextInfo(TOK_DESIGN_ACC_UPDATE, null, request, null));
      rights.setAttribute(
          "deleteDesign", (String) getUserContextInfo(TOK_DESIGN_ACC_DELETE, null, request, null));

      // add ServerAccessRights node
      rights = PSXmlDocumentBuilder.addEmptyElement(doc, root, "ServerAccessRights");
      rights.setAttribute(
          "data", (String) getUserContextInfo(TOK_SERVER_DATA_ACCESS, null, request, null));
      rights.setAttribute(
          "design", (String) getUserContextInfo(TOK_SERVER_DESIGN_ACCESS, null, request, null));
      rights.setAttribute(
          "createApplications",
          (String) getUserContextInfo(TOK_SERVER_CREATEAPPLICATIONS_ACCESS, null, request, null));
      rights.setAttribute(
          "deleteApplications",
          (String) getUserContextInfo(TOK_SERVER_DELETEAPPLICATIONS_ACCESS, null, request, null));
      rights.setAttribute(
          "admin", (String) getUserContextInfo(TOK_SERVER_ADMIN_ACCESS, null, request, null));
  ...
  private static PSLiteralSet convertToLiteralSet(Object[] objArray) {
    ...
    PSLiteralSet literalSet =
        new PSLiteralSet(com.percussion.design.objectstore.PSTextLiteral.class);
  ...
  public static Object getUserContextInformation(
    ...
    if (type == TOK_USER_ATTRIBUTE)
      ...
      field = userContextField.substring(PREFIX_USER_SESSION_OBJECT.length());
