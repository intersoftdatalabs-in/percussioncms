...
public class PSTableMetaData implements IPSConnectionInfo {
  ...
  public String[] getColumns(String columnNamePattern) {
    ...
    if (ret.length > 0) cols.toArray(ret);
    return ret;
  }
  ...
  public String[] getPrimaryKeyColumns() {
    ...
    if (ret.length > 0) m_primaryKeyColumns.toArray(ret);
    return ret;
  }
  ...
  public String[] getForeignKeyColumns() {
    ...
    if (ret.length > 0) m_foreignKeyColumns.toArray(ret);
    return ret;
  }
  ...
  public String[] getAutoUpdateColumns() {
    ...
    if (ret.length > 0) m_autoUpdateColumns.toArray(ret);
    return ret;
  }
  ...
  private void loadKeyInformation(DatabaseMetaData md) throws SQLException {
    ...
    try {
       ...
       */
      Object[] args = {
        m_dataSource, m_schema, m_tableName, "getPrimaryKeys", PSSqlException.toString(e)
      };
      com.percussion.log.PSLogManager.write(
          new PSLogServerWarning(
    ...
    try {
       ...
       */
      Object[] args = {
        m_dataSource, m_schema, m_tableName, "getImportedKeys", PSSqlException.toString(e)
      };
      com.percussion.log.PSLogManager.write(
          new PSLogServerWarning(
              BackEndErrorCodes.LOAD_META_DATA_EXCEPTION.numericCode(), args, false, "PSTableMetaData"));
  ...
  private void loadStatsAndIndexInformation(DatabaseMetaData md) throws SQLException {
    ...
    if (isEstimateSatisticsEnabled()) {
      ...
      if (estimateRow != null) {
        m_tableStats = new PSTableStatistics(m_login, table, estimateRow.intValue());
        ms_logger.debug(
            "estimate table: " + table.getTable() + " value = " + estimateRow.intValue());
      } else {
  ...
  boolean isEstimateSatisticsEnabled() {
    String enableEstimate =
        PSServer.getServerProps().getProperty("enableEstimateStatistics", "true");

    if ((StringUtils.equals(enableEstimate, "false"))) return false;
