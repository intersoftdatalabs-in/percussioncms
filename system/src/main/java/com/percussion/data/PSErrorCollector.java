...
public class PSErrorCollector {
  ...
  public Document getErrorDocument(PSRequest request) {
    if (request == null) throw new IllegalArgumentException("parameters cannot be null");
    ...
    try {
      ...
      while (errors.hasNext()) {
        ...
        params.put(
            ...
            PSHtmlParameters.get(IPSHtmlParameters.SYS_CONTENTID, request.getParameters()));
        params.put(
            IPSHtmlParameters.SYS_REVISION,
            PSHtmlParameters.get(IPSHtmlParameters.SYS_REVISION, request.getParameters()));
        URL completeUrl =
            PSUrlUtils.createUrl(
                null, null, url, params.entrySet().iterator(), null, new PSRequestContext(request));
        ...
        for (int i = 0; i < pageErrors.size(); i++) {
          ...
          for (int j = 0; j < submitNames.size(); j++) {
            ...
            errorField.setAttribute(SUBMIT_NAME_ATTR, submitNames.get(j).toString());
    ...
    while (pages.hasNext()) {
      ...
      while (iter.hasNext()) {
        ...
        for (int i = 0; i < controls.getLength(); i++) {
          ...
          String paramName = control.getAttribute(PSDisplayFieldElementBuilder.PARAMNAME_NAME);
          if (paramName.equals(submitName)) return pageId;
        }
      }
    }
  ...
  public void mergeErrors(Integer pageId, Document doc) {
    if (pageId == null || doc == null)
      throw new IllegalArgumentException("pageid and document cannot be null");
  ...
  private Element createDisplayError(Document doc) {
    ...
    displayError.setAttribute(ERROR_COUNT_ATTR, Integer.toString(getErrorCount()));
  ...
  private void mergeFieldErrors(Integer pageId, Document doc) {
    ...
    while (evals.hasNext()) {
      ...
      PSDisplayText errorMessage = field.getValidationRules().getErrorMessage();
    ...
    contentEditor.setAttribute(
        PSEditorDocumentBuilder.DOCTYPE_NAME, PSEditorDocumentBuilder.DOCTYPE_ERROR);
    Node first = contentEditor.getFirstChild();
    if (first != null) contentEditor.insertBefore(displayError, first);
    else contentEditor.appendChild(displayError);
    ...
    for (int i = 0; i < controls.getLength(); i++) {
      ...
      if (paramName != null) {
        ...
        if (errorLabel != null) {
          ...
          Element errorElement = doc.createElement(PSDisplayFieldElementBuilder.LABEL_NAME);
          Text errorText = doc.createTextNode(errorLabel.getText());
          errorElement.appendChild(errorText);

          String labelSrcType =
              ((Element) displayField.getFirstChild())
                  .getAttribute(PSDisplayFieldElementBuilder.SOURCE_TYPE_NAME);
          errorElement.setAttribute(PSDisplayFieldElementBuilder.SOURCE_TYPE_NAME, labelSrcType);
