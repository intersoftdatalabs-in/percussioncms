...
/** This class is used to provide context info to our PSSqlXXXBuilder classes. */
class PSSqlBuilderContext {
  /** Construct a new builder context using a PSStatementBlock as the initial block. */
  PSSqlBuilderContext() {
    super();
    m_curGroup = null;
   ...
   *
   * @param str the text to add to the current block, may be <code>null</code> or empty. If <code>
   *     null</code> then "null" text is added.
   * @param closeRun <code>true</code> if no more text is to be added to the current statement
   *     block, <code>false</code> otherwise.
   */
  void addText(String str, boolean closeRun) {
    m_text.print(str);
    if (closeRun) closeTextRun();
  }

  /** Convenience method that calls {@link #addText(String, boolean) addText(str, false)} */
   ...
   *
   * @param type the type of block to create, should be one of the <code>BLOCK_TYPE_XXX</code>
   *     vallues
   * @param isStaticBlock <code>true</code> if this block should be omitted if <code>null</code>,
   *     <code>false</code> otherwise
   * @param addToList if this block should be added to the internal list of blocks, <code>false
   *     </code> otherwise. If <code>true</code> then this block will be contained in the array
   *     returned by <code>getBlocks()</code> method, otherwise not
   */
  private void newBlock(int type, boolean isStaticBlock, boolean addToList) {
    ...
    if (addToList) m_blocks.add(m_curBlock);
  }

  /**
   * Convenience method that calls {@link #newBlock(int, boolean, boolean)
  ...
  /**
   * Convenience method that calls {@link #newBlock(int, boolean, boolean) newBlock(type,
   * isStaticBlock, true)}
   */
  ...
  /**
   * Convenience method that calls {@link #newBlock(int, boolean) newBlock(BLOCK_TYPE_STATEMENT,
   * isStaticBlock)}
   */
  ...
  /** Convenience method that calls {@link #newBlock(int, boolean) newBlock(type, true)} */
  void newBlock(int type) {
    newBlock(type, true);
  }

  /** Convenience method that calls {@link #newBlock(int) newBlock(BLOCK_TYPE_STATEMENT)} */
  void newBlock() {
    newBlock(BLOCK_TYPE_STATEMENT);
  }

  /**
   * Creates a new statement group (<code>PSStatementGroup</code>). A statement group can contain
   * other statements or statement groups.
   *
   * @param prefix the prefix to be added before the string form of the statement group (for example
   *     - "WHERE"), may be <code>null</code> or empty if no string is to be prepended to the
   *     statement group
   */
   ...
   * @param isStaticBlock <code>true</code> if this block should be omitted if <code>null</code>,
   *     <code>false</code> otherwise
   * @param separator the statement block sepator (for example, AND, OR etc), may be <code>null
   *     </code> or empty
   */
  void newGroupBlock(int type, boolean isStaticBlock, String separator) {
    ...
    if (m_curGroup.getLeftBlock() == null) {
      ...
      // separators, etc.
      PSStatementGroup grp = new PSStatementGroup(null, m_curBlock, separator, null);
  ...
  /**
   * Convenience method that calls {@link #newGroupBlock(int, boolean, String)
  ...
  /**
   * Closes the current group and current block. Once closed, no more text or replacement value can
   * be added to the block. And no more statement block or group can be added to the group.
   */
  void closeGroup() {
    closeTextRun(); // make sure there's no stray data
    m_curGroup = null;
  ...
  /**
   * Used to flush the text string created using the <code>addText()</code> methods to the current
   * statement block.
   */
  void closeTextRun() {
    m_text.flush();
    if (m_buf.size() > 0) {
      if (m_curBlock != null) m_curBlock.addText(m_buf.toString());
      m_buf.reset();
    }
  }
   ...
   * @param col The back end column associated with this replacement field.
   * @param lci The Lob column initializer to be used when dealing with LOB-based column types. Can
   *     be <code>null</code>.
   ...
   *
   * @param value the replacement value to add to the block, may not be <code>null</code>.
   * @param params current block specific implementation parameters, may be <code>null</code> or
   *     empty if the implementation does not require these parameters
   * @throws IllegalStateException if this context is in an invalid state such that the current
   *     block is <code>null</code>
   */
   ...
   *
   * @return the statement blocks contained in this context, never <code>null</code>, may be empty
   */
  ...
  public void verifyBlockType(int blockType) {
    if (!((blockType == BLOCK_TYPE_STATEMENT) || (blockType == BLOCK_TYPE_FUNCTION))) {
