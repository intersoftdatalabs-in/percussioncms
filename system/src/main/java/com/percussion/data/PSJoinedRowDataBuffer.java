...
class PSJoinedRowDataBuffer {
  ...
  PSJoinedRowDataBuffer(
    ...
    for (int i = 0; i < m_omitColumns.length; i++) {
      if (m_omitColumns[i]) m_omitColumnCount++;
    }
  ...
  void setRightSideResultSet(ResultSet rRS) throws java.sql.SQLException {
    ...
    if (m_rCols != 0) {
      ...
      for (int i = m_lCols; i < m_joinedColumnCount; i++) newBuf[i] = null;
      m_curRow = newBuf;
  ...
  boolean readLeftRow() throws java.sql.SQLException {
    ...
    if (m_lHasData && (m_lHasData = m_lRS.next())) {
      for (int i = 0; i < m_lCols; i++) {
        ...
        if (m_lRS.wasNull()) m_nextRow[i] = null;
      }
    } else {
      m_lHasNext = false;
      for (int i = 0; i < m_lCols; i++) {
  ...
  boolean readRightRow() throws java.sql.SQLException {
    ...
    if (m_rHasData && (m_rHasData = m_rRS.next())) {
      for (int i = 0; i < m_rCols; i++) {
        ...
        if (m_rRS.wasNull()) m_nextRow[m_lCols + i] = null;
      }
    } else {
      m_rHasNext = false;
      for (int i = 0; i < m_rCols; i++) {
  ...
  PSResultSet getJoinedResultSet() {
    if (!m_adjustedResultSetMeta) { // this had better happen only once
      for (int i = m_omitColumns.length; i > 0; i--) {
        ...
        if (i > m_joinedColumnCount) continue;
  ...
  void reprocessMarkedRows() {
    ...
    if (m_rowEndMark == NO_MARK) m_rowEndMark = m_joinedRSData[0].size();
  ...
  private Object readColumnData(boolean isLeft, int index) throws java.sql.SQLException {
    Object colData = null;
    ResultSet rs;
    int colType;
