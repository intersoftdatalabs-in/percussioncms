...
/**
 * Java 11 refactored: Utilities for working with CM System folder paths.
 * <p>
 * Many methods mirror Commons FilenameUtils, but are adapted for Percussion CMS.
 * <p>
 * All methods are static, pure, and thread-safe. Uses modern Java 11 features and Google Java Style.
 *
 * @author adamgent
 */
public class PSFolderPathUtils {
    ...
    public static boolean hasExtension(String path) {
        var pathIndex = lastIndexOfPathSeparator(path);
        var extIndex = lastIndexOfExtSeparator(path);
        /*
    ...
    public static String resolveFolderPath(
        ...
        // Use getTags() instead of getFolderPaths() as per IPSItemSummary API
        if (item.getTags() == null || item.getTags().isEmpty())
            return null;

        var matchingPaths = new ArrayList<String>();
        for (IPSFolderPath p : paths) {
            if (p == null || isBlank(p.getFolderPath()))
                continue;
            if (item.getTags().contains(p.getFolderPath()))
                return p.getFolderPath();
            matchingPaths.addAll(matchingDescedentPaths(p.getFolderPath(), item.getTags()));
        }
    ...
    public static boolean isDescedentPath(String path, String ancestorPath) {
        var normalizedSitePath = removeEnd(ancestorPath, PATH_SEPARATOR) + PATH_SEPARATOR;
    ...
    public static String parentPath(String path) {
        ...
        var parentPath = substringBeforeLast(path, PATH_SEPARATOR);
