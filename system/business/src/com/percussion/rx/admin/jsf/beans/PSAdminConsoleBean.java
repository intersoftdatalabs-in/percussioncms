...
public class PSAdminConsoleBean extends PSLockableNode {
     ...
     * Named as a getter to make it work with JSF.
     *
     * @return the results of the command, optionally concatenated with the
     * previous commands results. Never {@code null}, can be empty.
     */
    public String getResult() {
        ...
        resetResults();
        m_result.append("Command: ").append(getCommand()).append("\n");
     ...
     */
    private String executeInConsoleHandler() throws PSIllegalArgumentException, PSConsoleCommandException, IOException {
        var cout = new StringWriter();
        var parsedCmd = PSConsoleCommandParser.parse(getCommand());
        var req = (PSRequest) PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUEST);
        var respDoc = parsedCmd.execute(req);
    ...
    public void setCommand(String command) {
        if (command == null) {
            m_command = null;
        } else if (command.equalsIgnoreCase(QUIT_CMD) || command.equalsIgnoreCase(EXIT_CMD)) {
