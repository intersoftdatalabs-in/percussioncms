...
public class PSTaskContainerNode extends PSEditableNodeContainer {
     ...
     */
    private static final Logger ms_log = LogManager.getLogger(PSTaskContainerNode.class);

    /**
     * Constructor.
     */
    public PSTaskContainerNode() {
        super("The scheduled tasks.", ADMIN_NOTIFICATION_VIEWS, "Scheduled Tasks");
    }
    ...
    public String createEvent() throws PSNotFoundException {
        var event = getSchedulingService().createSchedule();
    ...
    private void initChildrenNodes() {
        ...
        try {
            notifications = new ArrayList<>(getSchedulingService().findAllSchedules());
        } catch (PSSchedulingException e) {
            ms_log.error("Failure to load all the timed events", e);
            throw new RuntimeException(e);
        }
        notifications.sort(new ByLabelComparator());
        notifications.forEach(notification -> addNode(new PSTaskNode(notification)));
    }
