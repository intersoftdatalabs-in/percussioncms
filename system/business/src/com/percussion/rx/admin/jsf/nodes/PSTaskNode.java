...
public class PSTaskNode extends PSEditableNode {
     ...
     *
     * @param event the notification template. Not <code>null</code>.
     */
    ...
    public String copy() throws PSNotFoundException {
        var copy = getSchedulingService().createSchedule();
        var id = copy.getId();
        copy.apply(getEvent());
        copy.setId(id);
        var label = getContainer().getUniqueName(getEvent().getName(), true);
        copy.setName(label);
    ...
    private boolean isValidSchedule() {
        return StringUtils.isBlank(getNotifyWarning()) && StringUtils.isBlank(getNotifyTemplateWarning());
    }
    ...
    private void setEventFromInput() {
        ...
        if (m_notifyTemplateId.equals(NOTIFY_TEMPLATE_NULL_ID)) {
            ...
            var id = gmgr.makeGuid(m_notifyTemplateId, PSTypeEnum.SCHEDULE_NOTIFICATION_TEMPLATE);
    ...
    private boolean doSave() {
        ...
        try {
            ...
            ms_log.warn("Failure to save the schedule {}", getEvent().getName(), e);
    ...
    public void runNow() {
        ...
        try {
            ...
            ms_log.warn("Failure to execute the schedule {}", getEvent().getName(), e);
        }
    }
     ...
     * before the super constructor is called. to request the label from. If
     * <code>null</code> the method throws <code>IllegalArgumentException</code>.
     *
    ...
    public List<SelectItem> getNotifyWhenChoices() {
        var choices = new ArrayList<SelectItem>();
        for (var when : PSNotifyWhen.values()) {
     ...
     *
     * @return the choices - names for roles and subjects. Never <code>null</code> or
     *         empty.
     */
    ...
    public String getNotifyTemplate() {
        if (m_notifyTemplateId == null) {
            if (getEvent().getNotificationTemplateId() == null) {
                ...
                m_notifyTemplateId = getEvent().getNotificationTemplateId().toString();
            }
        }
    ...
    public List<SelectItem> getNotificationTemplateChoices() {
        var choices = new ArrayList<SelectItem>();
        var ntList = new ArrayList<>(getSchedulingService().findAllNotificationTemplates());
        ntList.sort(new ByLabelComparator());
    ...
    private void setupParameters() {
        m_params = PSExtensionHelper.setupParameters(getEvent().getExtensionName(), getEvent().getParameters(), m_params);
    }
    ...
    public String getNotifyWarning() {
        if (isNotifyRequired()) {
            if (StringUtils.isBlank(getEvent().getNotify()) && StringUtils.isBlank(getEvent().getEmailAddresses())) {
    ...
    private boolean isNotifyRequired() {
        var notifyLabel = getEvent().getNotifyWhen().getLabel();
        var isNever = PSNotifyWhen.NEVER.getLabel().equalsIgnoreCase(notifyLabel);
        return !isNever;
    }
