...
public class PSTaskLogNode extends PSLockableNode {
    ...
    public class EventLog {
        ...
        public EventLog(PSScheduledTaskLog log, String taskName) {
            ...
            if (StringUtils.isBlank(taskName)) throw new IllegalArgumentException("taskName may not be null or empty.");
            mi_log = log;
         ...
         */
        public IPSGuid getId() { return mi_log.getId(); }
        /**
         * Get the name of the execution task
         * @return the task name, never <code>null</code> or empty.
         */
        public String getTaskName() { return mi_taskName; }
        /**
         * It is the same as {@link PSScheduledTaskLog#getStartTime()}
         */
        public String getStartTime() { return PSPublishingStatusHelper.getDatetime(mi_log.getStartTime()); }
        /**
         * It is the same as {@link PSScheduledTaskLog#getElapsed()}
         */
        public String getElapsed() { return PSPublishingStatusHelper.convertMilliSecondToHhMmSs(mi_log.getElapsed()); }
        /**
    ...
    public EventLog getDetailLog() {
        if (m_detailLog == null) throw new IllegalStateException("m_detailLog must be set first.");
        return m_detailLog;
    }
    ...
    private Map<IPSGuid, String> getTaskIdNameMap() throws PSNotFoundException {
        ...
        for (var n : parent.getChildren()) {
            if (n instanceof PSTaskContainerNode) containerNode = (PSTaskContainerNode) n;
        }
        if (containerNode == null) throw new IllegalStateException("Cannot find Event Container Node.");
        for (var n : containerNode.getChildren()) {
            var event = (PSTaskNode) n;
    ...
    public List<EventLog> getEventLogs() throws PSNotFoundException {
        var idnameMap = getTaskIdNameMap();
        var logs = getService().findAllTaskLogs(8640);
        var eventLogs = new ArrayList<EventLog>();
        for (var log : logs) {
            var name = idnameMap.get(log.getTaskId());
            if (name == null) name = log.getTaskId().toString();
            var elog = new EventLog(log, name);
            eventLogs.add(elog);
        }
    ...
    public String deleteLogs() throws PSNotFoundException {
        if (m_eventLogs == null) getEventLogs();
        var ids = new ArrayList<IPSGuid>();
        for (var log : m_eventLogs) {
            if (log.getSelected()) ids.add(log.getId());
        }
        if (ids.isEmpty()) return "admin-no-log-selection-warning";
    ...
    private void setPageRows(List<EventLog> entries) throws PSNotFoundException {
        if (m_pageRows != null) return;
        if (entries == null) entries = getEventLogs();
    ...
    private IPSSchedulingService getService() { return PSSchedulingServiceLocator.getSchedulingService(); }
