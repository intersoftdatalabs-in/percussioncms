...
public class PSSiteRootBrowser extends PSContentBrowser
{
   ...
   /**
    * Finishes browsing folder root.
    * @return the outcome of the Site Editor, never null or empty.
    */
   public String done() {
      if (m_site == null) return null;
    ...
    */
   /**
    * Gets the help file name for the Site Root Browser page.
    ...
    */
   /**
    * Returns the outcome of browsing the Site Root Path.
    */
   @Override
    ...
    */
   /**
    * Gets all child folders for the current folder.
    * @return list of child folders, never null, may be empty
    */
   @Override
   protected List<ChildItem> getChildItems() throws Exception {
      ...
      var sums = getFolderSrv().getSummaries(filter, false);
      var folders = new ArrayList<ChildItem>();
      var it = sums.getSummaries();
      while (it.hasNext()) {
         var sum = (PSComponentSummary) it.next();
         var child = new ChildItem(mgr.makeGuid(sum.getCurrentLocator()), sum.getName(), sum.isFolder());
         folders.add(child);
      }
      return folders;
   }
