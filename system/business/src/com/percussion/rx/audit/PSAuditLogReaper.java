...
/**
 * Java 11 refactored: Automatically deletes old audit log entries on a daily basis as configured by
...
public class PSAuditLogReaper extends Thread {
    ...
    */
    public synchronized void setService(IPSDesignObjectAuditService service) {
        ...
        m_service = service;
        var config = m_service.getConfig();
        m_logRetentionDays = config.getLogRetentionDays();
        if (config.isEnabled()) {
            ms_log.info("Design object audit Logging is enabled");
            if (m_logRetentionDays > 0) {
                ms_log.info("Design object audit log pruning is enabled, log entries will be deleted after {} days.", m_logRetentionDays);
                start();
            } else {
                ms_log.info("Design object audit log pruning is disabled, log entries will be saved indefinitely");
            }
        } else {
   ...
   @Override
    public void run() {
        ...
        try {
            while (true) {
                deleteLogEntries();
                var sleepTime = m_sleepIntervalMins * MINS_TO_MILLIS;
                ms_log.debug("Next audit log pruning will be in {} minutes", m_sleepIntervalMins);
                sleep(sleepTime);
            }
