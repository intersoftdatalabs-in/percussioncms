...
public class ContentTestCase extends PSContentTestBase {
   ...
   /**
    * Moves the specified item to the public state if it is in Quick-Edit state. Assumes the given
    * item is either in public or quick-edit state.
    *
    ...
    */
   private void moveItemInPublicState(ContentSOAPStub binding, int contentId) throws Exception {
      PSItemStatus item = checkoutItem(binding, contentId);
      if (item.isDidCheckout() == false && item.isDidTransition() == false) {
   ...
   private PSItemStatus checkoutItem(ContentSOAPStub binding, int contentId) throws Exception {
      long[] guidId = new long[] {getLegacyGuid(contentId)};
      return binding.prepareForEdit(guidId)[0];
   }

   private void checkinItem(ContentSOAPStub binding, PSItemStatus item) throws Exception {
   ...
   public void fix_testItemStatus() throws Exception {
      ...
      try {
         ...
         try {
            ...
            verifyErrorResultsFault(e, ids.length - 1, PSItemStatus.class.getName());
            statuses[0] = e.getServiceCall(0).getResult().getPSItemStatus();
         }

         // transition from public to quick-edit and checkout
         statuses[1] = binding.prepareForEdit(new long[] {getLegacyGuid(490)})[0];
         ...
         throw new junit.framework.AssertionFailedError("RemoteException Exception caught: " + e);
      }
   }
   ...
   public void fix_testFolder() throws Exception {
      ...
      try {
         ...
         String[] paths = new String[] {"//Sites", "//Folders", "//Sites/EnterpriseInvestments",
         ...
         areq.setName("testContentSQAPFolder" + java.lang.System.currentTimeMillis());
         AddFolderResponse aresp = binding.addFolder(areq);
         PSFolder folder = aresp.getPSFolder();

         // validate the folder property
         long displayId_0 = new PSDesignGuid(PSTypeEnum.DISPLAY_FORMAT, 0).getValue();
         ...
         long displayId_1 = new PSDesignGuid(PSTypeEnum.DISPLAY_FORMAT, 1).getValue();
         ...
         PSFolderPropertiesProperty prop =
               new PSFolderPropertiesProperty("AdamProperty", "PropValue", "Description");
         propsNew[propLength - 1] = prop;
         folder.setProperties(propsNew);
         long[] savedFolderIds = binding.saveFolders(new PSFolder[] {folder}).getIds();
         ...
         PSItemSummary[] children = findFolderChildren(binding, folders[0].getId(), null, true);
         assertTrue(children.length == 1);
         assertTrue(children[0].getObjectType().getValue().equals(ObjectType._folder));
         ...
         children = findFolderChildren(binding, null, folders[0].getPath(), false);
         assertTrue(children.length == 1);
         ...
         for (PSItemSummary child : children) {
            assertTrue(child.getObjectType().getValue().equals(ObjectType._item));
   ...
   public void testAddFolderTree() throws Exception {
      ...
      AddFolderTreeRequest atreq = new AddFolderTreeRequest();
      String path =
            "//Folders/TestFolder_1_" + java.lang.System.currentTimeMillis() + "/TestFolder_2";
      atreq.setPath(path);
   ...
   public void fix_testMoveFolderChildren() throws Exception {
      ...
      try {
         ...
         // validate the move
         PSItemSummary[] children = findFolderChildren(binding, srcFolder.getId(), null, false);
         assertTrue(children.length == 0);
   ...
   /**
    * Load Folder Children by either parent folder id (if specified) or by parent folder path.
    *
    * @param binding the object used to communicate with server, assumed not <code>null</code>.
    * @param parentId the parent folder id, may be <code>null</code> if wanting to request by folder
    *        path.
    * @param parentPath the path of the parent folder, may be <code>null</code> if wanting to
    *        request by folder id.
    * @param isLoadOperations it is <code>true</code> if load the allowed operations for the
    *        returned summaries; otherwise, the allowed operations in the returned summaries will be
    *        <code>null</code> or empty.
    *
    * @return the child summaries of the specified folder, never <code>null</code>, may be empty.
    *
    * @throws Exception if any error occurs.
    */
   private PSItemSummary[] findFolderChildren(ContentSOAPStub binding, Long parentId,
         String parentPath, boolean isLoadOperations) throws Exception {
   ...
   public void testGetAssemblyUrl() throws Exception {
      ...
      try {
         ...
         // set the folder path
         req.setFolderPath("//Sites/EnterpriseInvestments/AboutEnterpriseInvestments");
         urls = binding.getAssemblyUrls(req).getUrls();
         assertTrue(urls.length == 1);
   ...
   public void fix_testAaRelationship() throws Exception {
      ...
      try {
         ...
         PSContentTypeSummary[] sums = loadContentTypeSummaries("rffGeneric", m_session);
         ...
         assertTrue(rels.length > 0);
         long rid1479 = new PSDesignGuid(PSTypeEnum.RELATIONSHIP, 1479).getValue();
         long rid1482 = new PSDesignGuid(PSTypeEnum.RELATIONSHIP, 1482).getValue();
         long rid1483 = new PSDesignGuid(PSTypeEnum.RELATIONSHIP, 1483).getValue();
         ...
         PSAaRelationship rel1479 = loadAaRelationshipsByRid(binding, rid1479, true);
         ...
         addContentRelationships(binding, createdRids, PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY,
               1000);
         // append 2 new relationships
         ...
         PSAaRelationship rel = loadAaRelationshipsByRid(binding, createdRids.get(0), true);
         long templateId = new PSDesignGuid(PSTypeEnum.TEMPLATE, 503).getValue();
         ...
         aaFilter.setConfigurations(new String[] {PSRelationshipConfig.TYPE_ACTIVE_ASSEMBLY,
         ...
         try {
            ...
            // relationship name
            aaFilter.setConfigurations(new String[] {PSRelationshipConfig.TYPE_NEW_COPY});
    ...
    *
    * @param rels the to be reversed relationships; assumed not <code>null</code>.
    *
    ...
    *
    * @param binding the binding object; assumed not <code>null</code>.
    * @param rels the to be reordered relationships; assumed not <code>null</code>.
    * @param index the new location of the reordered relationships.
    *
    * @throws Exception if any error occurs.
    */
   @Test
   private void reorderContentRelationships(ContentSOAPStub binding, PSAaRelationship[] rels,
    ...
    *
    * @param binding the binding object used to communicate with the server; assumed not
    *        <code>null</code>.
    * @param createdRids the object used to collect the newly created relationships.
    * @param index the index of the request.
    *
    * @throws Exception if an error occurs.
    */
   @Test
   private void addContentRelationships(ContentSOAPStub binding, List<Long> createdRids,
         String relationshipName, int index) throws Exception {
      PSAaRelationship[] existRels = loadAaRelationshipsByOwnerSlot(binding, 335, SLOT_NAME);
      ...
      PSAaRelationship rel = loadAaRelationshipsByRid(binding, rels[0].getId(), true);
   ...
   @Test
   private PSAaRelationship loadAaRelationshipsByRid(ContentSOAPStub binding, long rid,
   ...
   private PSAaRelationship[] loadAaRelationshipsByOwner(ContentSOAPStub binding, int contentId,
         Reference ownerCT) throws Exception {
      PSAaRelationshipFilter filter = new PSAaRelationshipFilter();
   ...
   private PSAaRelationship[] loadAaRelationshipsByOwnerSlot(ContentSOAPStub binding, int contentId,
   ...
   public void testLoadLocales() throws Exception {
      ...
      try {
         ...
         for (PSLocale locale : localesToFind) {
            ...
            locales = loadLocales(locale.getCode(), locale.getLabel(), m_session);
            assertTrue(locales.length == 1);
            assertEquals(locale, locales[0]);
         }
      } catch (com.percussion.webservices.faults.PSInvalidSessionFault e1) {
         throw new AssertionFailedError("InvalidSessionFault Exception caught: " + e1);
      } catch (Exception e2) {
   ...
   /**
    * Validates that the supplied array of locales contains the expected locales to be found.
    *
    ...
    */
   @Test
   private void validateLocaleSummaries(PSLocale[] locales, List<PSLocale> localesToFind) {
      ...
      try {
         ...
         if (!saved) {
            PSDesignGuid guid = new PSDesignGuid(
                  com.percussion.services.content.data.PSAutoTranslation.getAutoTranslationsGUID());
   ...
   /**
    * Tests the loadContenTypes web service. Creates test content types and deletes them when
    * finished, so this test relies on some of the CRUD design services.
    *
    * @throws Exception If the test fails.
    */
   @Test
   ...
   /**
    * Validates that the summaries are in the expected types, and that the fields match.
    *
    * @param sums The list of summaries to validate, assumed not <code>null</code>.
    * @param typesToFind The list of expected types, assumed not <code>null</code> or empty.
    */
   ...
   private void validateContentTypeSummaries(PSContentTypeSummary[] sums,
      ...
      for (PSContentType ctype : typesToFind) {
         ...
         Iterator<Object> mappings = ((PSContentEditorPipe) ce.getPipe()).getMapper()
               .getUIDefinition().getDisplayMapper().iterator();
         while (mappings.hasNext()) {
            PSDisplayMapping mapping = (PSDisplayMapping) mappings.next();
            if (!mapping.getFieldRef().equals("dummy")) {
               PSFieldDescription fieldDesc = fieldMap.get(mapping.getFieldRef());
    ...
    *
    * @param folderCT the folder content type in question; assumed not <code>null</code>.
    */
   @Test
   private void validateFolderContentType(PSContentTypeSummary folderCT) {
      ...
      for (PSContentTypeSummaryChild child : children) {
         assertTrue(child.getName().equals("properties") || child.getName().equals("acl"));
   ...
   public void testContentSOAPChildItems() throws Exception {
      ...
      try {
         ...
         PSTestUtils.setSessionHeader(binding, m_session);
         List<PSItem> items = createTestItems(contentType.getName(), 1, true, true, false, null,
               m_session, binding);
         ...
         PSChildEntry[] entries =
               doTestCreateChildItems(contentId, childName, count, "field_12", "1", binding);
         ...
         try {
            ...
            java.lang.System.out
                  .println("Failed to cleanup test content item: " + e.getLocalizedMessage());
         }
    ...
    * @param childName The child field name, may be <code>null</code> or empty to test contract
    *        enforcement
    * @param count The number of entries to create, may be <=0 to test contract enforcemnt
    * @param fieldName The name of the field to check for a default value, assumed not
    *        <code>null</code> or empty.
    * @param fieldVal The expected value of the field, assumed not <code>null</code>.
    * @param binding the stub used, assumed not <code>null</code>.
    * @return An array of entries, never <code>null</code> and will have the specified count.
    *
    * @throws Exception
    */
   @Test
   private PSChildEntry[] doTestCreateChildItems(long contentId, String childName, int count,
         String fieldName, String fieldVal, ContentSOAPStub binding) throws Exception {
      // test no session
      try {
      ...
      PSChildEntry[] entries = createChildEntries(contentId, childName, count, m_session);
   ...
   /**
    * Populates field values in the supplied entries, also validating the default field value if
    * specified.
    *
    * @param entries The entries to populate, assumed not <code>null</code>.
    * @param fieldName The name of the field to validate, may be <code>null</code> or empty if
    *        validation is not required.
    * @param fieldVal The expected value, ignored if no field name is specified.
    * @param attachBinaries <code>true</code> to create attachments for binary fields,
    *        <code>false</code> to use base64 encoded data.
    * @param updateId an update identifier used to differentiate contents between updates.
    * @param binding the stub used, assumed not <code>null</code>.
    */
   @Test
   private void populateChildRows(PSChildEntry[] entries, String fieldName, String fieldVal,
         boolean attachBinaries, int updateId, ContentSOAPStub binding) {
      if (updateId <= 0)
      ...
      for (int i = 0; i < entries.length; i++) {
         ...
         for (int j = 0; j < fields.length; j++) {
            ...
            if (vals == null) {
               ...
               if (dt.equals(PSFieldDataType.text) || dt.equals(PSFieldDataType.number)
                     || dt.equals(PSFieldDataType.date) || dt.equals(PSFieldDataType.binary)) {
               ...
               if (!StringUtils.isBlank(fieldName) && field.getName().equals(fieldName)) {
            ...
            if (dt.equals(PSFieldDataType.text)) {
               ...
               vals[0].setRawData(updateId + String.valueOf(i) + String.valueOf(j));
               ...
               if (attachBinaries) {
                  File file = new File(RESOURCE_BASE + "attachments/", "child.jpg");
    ...
    * @param entries The entries to save, assumed not <code>null</code> or empty.
    * @param binding the stub used, assumed not <code>null</code>.
    * @return The loaded entries that were saved, never <code>null</code> or empty.
    *
    * @throws Exception if there are any errors.
    */
   @Test
   private PSChildEntry[] doTestSaveChildItems(long id, String name, PSChildEntry[] entries,
      ...
      if (field.getDataType().getValue().equals(PSFieldDataType._text)) {
         ...
         String newVal =
               StringUtils.isBlank(oldVal) ? "1" : String.valueOf(Integer.parseInt(oldVal) + 1);
    ...
    * @param id The content item id.
    * @param name The child field name, assumed not <code>null</code> or empty.
    * @param entries The entries to reorder, assumed not <code>null</code> or empty.
    *
    * @return The reordered entries that were saved, never <code>null</code> or empty.
    *
    * @throws Exception if there are any errors.
    */
   @Test
   private PSChildEntry[] doTestReorderChildItems(long id, String name, PSChildEntry[] entries)
         throws Exception {
      ...
      java.lang.System.arraycopy(childIds, remainder, partialIds, 0, partialSize);
      java.lang.System.arraycopy(childIds, 0, childIds, childIds.length - remainder, remainder);
    ...
    * @param id The content item id.
    * @param name The child field name, assumed not <code>null</code> or empty.
    * @param entries The entries to delete, assumed not <code>null</code> or empty.
    *
    * @throws Exception if there are any errors.
    */
   @Test
   private void doTestDeleteChildItems(long id, String name, PSChildEntry[] entries)
         throws Exception {
      long[] childIds = new long[entries.length];
      for (int i = 0; i < childIds.length; i++) {
         childIds[i] = entries[i].getId();
      ...
      compareChildEntries(remainder, loadChildEntries(id, name, false, m_session), true);
    ...
    */
   @Test
   private PSChildEntry[] doTestLoadChildItems(long id, String name, PSChildEntry[] entries)
         throws Exception {
      // test no session
      try {
      ...
      compareChildEntries(entries, loadChildEntries(id, name, true, m_session), true);
    ...
    * @param compareIds <code>true</code> to compare ids, <code>false</code> to compare only other
    *        values (used to compare a new entry with what is loaded after it is saved).
    */
   @Test
   private void compareChildEntries(PSChildEntry[] srcEntries, PSChildEntry[] tgtEntries,
         boolean compareIds) {
    ...
    * @param name The child field name, may be <code>null</code> or empty to test contract
    *        enforcement
    * @param count The number of entries to create, may be <=0 to test contract enforcemnt
    * @param session The session, may be <code>null</code> or empty to test authorization
    *
    * @return The entries, never <code>null</code>.
    *
    * @throws Exception If there are any errors.
    */
   private PSChildEntry[] createChildEntries(long id, String name, int count, String session)
         throws Exception {
      ContentSOAPStub binding = getBinding(null);
    ...
    * @param name The child field name, may be <code>null</code> or empty to test contract
    *        enforcement
    * @param entries The entires to save, assumed not <code>null</code> or empty.
    * @param session The session, may be <code>null</code> or empty to test authorization
    *
    * @throws Exception if there are any errors.
    */
   private void saveChildEntries(long id, String name, PSChildEntry[] entries, String session)
         throws Exception {
    ...
    * @param name The child field name, may be <code>null</code> or empty to test contract
    *        enforcement.
    * @param includeBinary <code>true</code> to load binary fields, <code>false</code> otherwise.
    * @param session The session, may be <code>null</code> or empty to test authorization
    *
    ...
    * @throws Exception If there are any errors.
    */
   private PSChildEntry[] loadChildEntries(long id, String name, boolean includeBinary,
    ...
    * @param name The child field name, may be <code>null</code> or empty to test contract
    *        enforcement.
    * @param childIds The new order of child ids, may only specify a subset of ids, see
    *        {@link ReorderChildEntriesRequest#setChildId(long[])} for more info.
    * @param session the session to use, may be <code>null</code> or empty to test authorization.
    *
    * @throws Exception If there are any errors.
    */
   private void reorderChildEntries(long id, String name, long[] childIds, String session)
         throws Exception {
      ContentSOAPStub binding = getBinding(null);
    ...
    * @throws Exception If there are any errors.
    */
   private void deleteChildEntries(long id, String name, long[] childIds, String session)
         throws Exception {
      ContentSOAPStub binding = getBinding(null);
   ...
   public void testCreateContentItems() throws Exception {
      ...
      try {
         ...
         throw new junit.framework.AssertionFailedError("Unknown content type: " + e);
      } catch (RemoteException e) {
         throw new junit.framework.AssertionFailedError("Unexpected error: " + e);
      } finally {
         // cleanup
   ...
   public void FIXME_testSaveContentItems() throws Exception {
      ...
      try {
         ...
         PSTestUtils.setSessionHeader(binding, session);
         int rows = 3;
         items.addAll(createTestItems(contentType.getName(), rows, true, false, false, null,
               session, binding));
         ...
         populateTestFields(items, -1, updateId++, binaryFile, session, binding);
         request = new SaveItemsRequest();
         request.setPSItem(toArray(items));
         ...
         childRows = 3;
         populateTestFields(items, childRows, updateId++, binaryFile, session, binding);
         request = new SaveItemsRequest();
         request.setPSItem(toArray(items));
         ids = saveItems(request, binding);
         assertTrue(ids != null && ids.length == items.size());
          ...
          *
          * // add all items to these folders String[] folderPaths = {
          * "//Folders/Tests/SaveItems_1"}; setupFolderPaths(items, folderPaths); request = new
          * SaveItemsRequest(); request.setPSItem(toArray(items)); ids = saveItems(request,
          * binding); cleanupFolders = true; assertTrue(ids != null && ids.length == items.size());
          * items = validateSave(items, folderPaths, true, binding);
          * 
          * // add all items to an additional folder
          * 
          * String[] folderPaths2 = { "//Folders/Tests/SaveItems_1", "//Folders/Tests/SaveItems_2"};
          * setupFolderPaths(items, folderPaths2); request = new SaveItemsRequest();
          * request.setPSItem(toArray(items)); ids = saveItems(request, binding); assertTrue(ids !=
          * null && ids.length == items.size());
          * 
          * // remove all items from a folder String[] folderPaths3 = {
          * "//Folders/Tests/SaveItems_1"}; setupFolderPaths(items, folderPaths3); request = new
          * SaveItemsRequest(); request.setPSItem(toArray(items)); ids = saveItems(request,
          * binding); assertTrue(ids != null && ids.length == items.size());
          */
         ...
         try {
            ...
            ConversionException expectedException =
                  new ConversionException(PSWebserviceErrors.createErrorMessage(
                        IPSWebserviceErrors.UNKNOWN_FIELD_NAME, guid.toString(), unknownFieldName));
            assertTrue(
                  e.getLocalizedMessage().indexOf(expectedException.getLocalizedMessage()) >= 0);
         }
   ...
   /**
    * Add the supplied attachment to the provided stub and return the attachment id.
    *
    * @param binding the stub to which to add the attachement, assumed not <code>null</code>.
    * @param attachment the file to attach, assumed not <code>null</code>.
    * @return the attachment id needed on the server to map this to the appropriate field, never
    *         <code>null</code> or empty.
    */
   ...
   /**
    * Setup the folder paths for all supplied items. All supplied folders will be added, updated or
    * removed. Supply <code>null</code> to ignore folder processing. Supply empty to remove all
    * folders.
    *
    * @param items the items for which to seup the folder paths, assumed not <code>null</code>.
   ...
   /**
    * Reloads all supplied items with everything included. Then this verifies that the reloaded
    * items contain only the suppliedd folder paths.
    *
    * @param items the items to reload and verify, assumed not <code>null</code>.
    * @param paths the expected patrhs to verify all items for, may be <code>null</code> or empty.
    * @param attachedBinaries <code>true</code> to validate binaries as attachments,
    *        <code>false</code> to validate them base64 encoded.
    * @param binding the stub used, assumed not <code>null</code>.
    * @return a list with all reloaded items, never <code>null</code>, may be empty.
    * @throws RemoteException for any error.
    */
   @Test
   private List<PSItem> validateSave(List<PSItem> items, String[] paths, boolean attachedBinaries,
         ContentSOAPStub binding) throws RemoteException {
      // reload the items
      LoadItemsRequest loadRequest = new LoadItemsRequest(toItemIds(items), null, true,
            attachedBinaries, true, null, true, null, true);
   ...
   /**
    * Reloads all supplied items with everything included. Then the supplied items are verified
    * against the reloaded ones to make sure that a previous save was successful.
    *
    * @param items the items to reload and validate, assumed not <code>null</code>, may be empty.
    * @param rows the number of expected items, -1 to ignore this parameter.
    * @param childRows the number of expected child entries, -1 to ignore this parameter.
    * @param attachedBinaries <code>true</code> to validate binaries as attachments,
    *        <code>false</code> to validate them base64 encoded.
    * @param binding the stub used, assumed not <code>null</code>.
    * @return a list with all reloaded items, never <code>null</code>, may be empty.
    ...
    */
   @Test
   private List<PSItem> validateSave(List<PSItem> items, int rows, int childRows,
   ...
   /**
    * Compares the two item lists. If the size of the supplied lists is different, the only the
    * number of items for the size of the smaller list will be compared.
    *
    * @param a the first list to compare, assumed not <code>null</code>.
    * @param b the second list to compare, assumed not <code>null</code> and of the same order as
    *        the first list.
    * @param binding the stub used, assumed not <code>null</code>.
    * @return <code>true</code> if all local fields and all childrens rows are equal,
    *         <code>false</code> otherwise.
    */
   private boolean compareTestItems(List<PSItem> a, List<PSItem> b, ContentSOAPStub binding) {
      int itemCount = a.size() > b.size() ? b.size() : a.size();

      /*
       * IMPORTANT: getAttachments clears all attachments so that they can only be retrieved once!
       */
         ...
         if (compareObjects(itemA, itemB) && itemA != null) {
            ...
            for (int j = 0; j < itemA.getChildren().length; j++) {
               ...
               if (compareObjects(childA, childB) && childA != null) {
                  ...
                  if (compareObjects(childEntriesA, childEntriesB) && childEntriesA != null) {
                     // only compare fields for the smaller or the 2 arrays
                     int childEntryCount =
                           childEntriesA.length > childEntriesB.length ? childEntriesB.length
                     ...
                     for (int n = 0; n < childEntryCount; n++) {
                        ...
                        for (int m = 0; m < fieldsA.length; m++) {
                           if (!compareTestFields(fieldsA[m], fieldsB[m], attachments))
                              return false;
                        }
                     }
                  }
    ...
    * @param b value b to compare, assumed not <code>null</code> and that the value arrays of both
    *        fields have the same length.
    * @param attachments an array with all response attachments, assumed not <code>null</code>, may
    *        be empty.
    * @return <code>true</code> if the contents or the 2 fields are equal, <code>false</code>
    *         otherwise.
    */
      ...
      if (compareObjects(valuesA, valuesB) && valuesA != null) {
         for (int i = 0; i < valuesA.length; i++) {
            if (!StringUtils.isBlank(valuesA[i].getAttachmentId())) {
               ...
               if (!StringUtils.equals(valuesA[i].getRawData(), valuesB[i].getRawData()))
                  return false;
            }
         }
      }
   ...
   /**
    * Compares the two supplied objects to test if both are <code>null</code> or not.
    *
    * @param a the first test object, may be <code>null</code>.
    * @param b the second test object, may be <code>null</code>.
    * @return <code>true</code> if both object are <code>null</code> or both are not
   ...
   public void testLoadContentItems() throws Exception {
      ...
      try {
         ...
         // create 3 test items for the new content type
         PSTestUtils.setSessionHeader(binding, session);
         items.addAll(createTestItems(contentType.getName(), 3, true, true, true,
               "//Folders/TestLoadContentItems", session, binding));
         ...
         validateBinaryField(loadedItems, "field_2", false, BinaryFiles.SKIP, binding);
         ...
         assertTrue(loadedItems != null && loadedItems.length == ids.length);
         validateBinaryField(loadedItems, "field_2", true, BinaryFiles.SKIP, binding);
         validateRequestParts(loadedItems, false, false, false);
         ...
         loadedItems = binding.loadItems(request);
         assertTrue(loadedItems != null && loadedItems.length == ids.length);
         validateBinaryField(loadedItems, "field_2", true, BinaryFiles.SMALL, binding);
         validateRequestParts(loadedItems, false, false, false);
   ...
   /**
    * Validate if the supplied items contain / miss the requested / not requested parts.
    *
    * @param items the items to verify, assumed not <code>null</code>.
    * @param includeChildren <code>true</code> if children were requested, <code>false</code>
    *        otherwise.
    * @param includeSlots <code>true</code> if slots were requested, <code>false</code> otherwise.
    * @param includeFolders <code>true</code> if folders were requested, <code>false</code>
    *        otherwise.
    */
   @Test
   private void validateRequestParts(PSItem[] items, boolean includeChildren, boolean includeSlots,
         boolean includeFolders) {
      for (PSItem item : items) {
         if (includeChildren)
            assertTrue("children requested but not returned", item.getChildren() != null);
         else
            assertTrue("children not requested but returned", item.getChildren() == null);

         if (includeSlots)
            assertTrue("slots requested but not returned", item.getSlots() == null);
         else
            assertTrue("slots not requested but returned", item.getSlots() == null);

         if (includeFolders)
            assertTrue("folders requested but not returned", item.getFolders() != null);
         else
            assertTrue("folders not requested but returned", item.getFolders() == null);
      }
   }
    ...
    *
    * @param items the items for which to validate the identified binary field, assumed not
    *        <code>null</code>.
    * @param fieldName the name of the binary field to validate, assumed not <code>null</code> or
    *        empty.
    * @param loaded <code>true</code> if the identified binary field must be loaded,
    *        <code>false</code> otherwise.
    * @param binaryFile <code>LARGE</code> or <code>SMALL</code> if the identified binary field is
    *        expected as attachment, <code>SKIP</code> if it is expected base64 encoded.
    * @param binding the binding used, assumed not <code>null</code>.
    * @throws Exception for any error.
    */
   @Test
   private void validateBinaryField(PSItem[] items, String fieldName, boolean loaded,
         BinaryFiles binaryFile, ContentSOAPStub binding) throws Exception {
      /*
       * IMPORTANT: getAttachments clears all attachments so that they can only be retrieved once!
       */
      Object[] attachments = binding.getAttachments();
                  ...
                  if (binaryFile != BinaryFiles.SKIP) {
                     ...
                     for (Object attachment : attachments) {
                        ...
                        if (part.getContentId().equals(value.getAttachmentId())) {
                           /*
                            * Compare the loaded binary attachment with the originally attached
                            * file.
                            */
                           InputStreamReader contentReader =
                                 new InputStreamReader((InputStream) part.getContent());
                           FileReader originalReader =
                                 new FileReader(getAttachmentFile(itemIndex, binaryFile));

                           assertTrue(IOTools.compareReaders(contentReader, originalReader));
   ...
   public void testViewContentItems() throws Exception {
      ...
      try {
         ...
         PSTestUtils.setSessionHeader(binding, session);
         items.addAll(createTestItems(contentType.getName(), itemCount, true, true, true, null,
               session, binding));
         long[] ids = toItemIds(items);
         ...
         int index = 0;
         for (PSRevisions itemRevisions : revisions)
            revisionIds[index++] = itemRevisions.getRevisions()[viewRevision - 1].getId();
         request = new ViewItemsRequest();
   ...
   /**
    * Verifies that the supplied items are all for the revision supplied. Assumes that the
    * <code>field_1</code> fiels all represent the items revosion.
    *
    * @param items all items to verify, assumed not <code>null</code>, may be empty.
    * @param revision the revision to verify.
    * @throws Exception for any error.
    */
   @Test
   private void verifyRevisions(List<PSItem> items, int revision) throws Exception {
      for (PSItem item : items) {
         ...
         for (PSField field : fields) {
            if (field.getName().equalsIgnoreCase("field_1")) {
               ...
               for (PSFieldValue value : values)
                  assertTrue(value.getRawData().equalsIgnoreCase(String.valueOf(revision)));
            }
         }
      }
   }
   ...
   public void testDeleteContentItems() throws Exception {
      ...
      try {
         ...
         // create 3 test items for the new content type
         PSTestUtils.setSessionHeader(binding, session);
         items.addAll(createTestItems(contentType.getName(), 3, true, true, false, null, session,
               binding));
         long[] ids = toItemIds(items);
   ...
   public void testFindContentItems() throws Exception {
      ...
      try {
         ...
         // create 3 test items for the new content type
         PSTestUtils.setSessionHeader(binding, session);
         items.addAll(createTestItems(contentType.getName(), 3, true, true, false,
               "//folders/testFindContentItems", session, binding));
         ...
         PSSearchResultField resfields[] = {new PSSearchResultField("sys_workflowname"),
         ...
         Map<String, PSSearchResultsFields> fieldMap = new HashMap<String, PSSearchResultsFields>();
         ...
         for (PSSearchResultField resField : resfields) {
            ...
            assertTrue("expected field value is empty", !StringUtils.isBlank(retField.get_value()));
         }
   ...
   public void testDeleteFolderItem() throws Exception {
      ...
      try {
         ...
         // create 3 test items for the new content type
         items.addAll(createTestItems(contentType.getName(), 1, true, true, false, null, session,
               binding));

         // create a new folder
         AddFolderRequest areq = new AddFolderRequest();
         areq.setPath("//Sites/EnterpriseInvestments/Files");
         areq.setName("testContentSQAPFolder_1_" + java.lang.System.currentTimeMillis());
         AddFolderResponse aresp = binding.addFolder(areq);
         PSFolder folder_1 = aresp.getPSFolder();

         areq.setName("testContentSQAPFolder_2_" + java.lang.System.currentTimeMillis());
         aresp = binding.addFolder(areq);
   ...
   public void testCreateNewCopies() throws Exception {
      ...
      try {
         ...
         long[] ids = toItemIds(items);
         String[] paths =
               {"//Folders/Tests/Copy_1", "//Folders/Tests/Copy_2", "//Folders/Tests/Copy_3"};
         String[] nullPaths = {null, null, null};
         String[] invalidPaths = {"//Folders/Tests/Copy_1", "//Folders/Tests/Copy_3"};
         String relationshipType = PSRelationshipConfig.TYPE_NEW_COPY;
         String[] onePaths = {"//Folders/Tests/Copy_One"};
         String[] one3Paths =
               {"//Folders/Tests/Copy_One", "//Folders/Tests/Copy_One", "//Folders/Tests/Copy_One"};
   ...
   public void testCreateNewPromotableVersions() throws Exception {
      ...
      try {
         ...
         String[] paths = {"//Folders/Tests/NewPromotableVersion_1",
               "//Folders/Tests/NewPromotableVersion_2", "//Folders/Tests/NewPromotableVersion_3"};
         String[] nullPaths = {null, null, null};
         String[] invalidPaths =
               {"//Folders/Tests/NewPromotableVersion_1", "//Folders/Tests/NewPromotableVersion_3"};
         ...
         String[] one3Paths = {"//Folders/Tests/NewPromotableVersion_1",
               "//Folders/Tests/NewPromotableVersion_1", "//Folders/Tests/NewPromotableVersion_1"};
    ...
    * @throws Exception if any error occurs.
    */
   private void removeItems(ContentSOAPStub binding, PSItem[] items) throws Exception {
   ...
   /**
    * Attempts to create a translation of an item twice. Verify that no item was created the 2nd
    * time and that the original translation was not modified.
    * <p>
    * This test depends on the presence of a specific FastForward item, #335. This item must not
    * have any existing translations.
    *
    * @throws Exception
    */
   @Test
   public void FIXME_testCreateTranslationAgain() throws Exception {
      ...
      try {
         ...
         {
            List<PSAutoTranslation> autoTranslations = createTestAutoTranslations(locales);
            long[] ids = new long[1];
            ...
            assertTrue("Item 335 must not contain any translations for this test to run.",
                  rels.length == 0);

            PSAaRelationshipFilter relFilter = new PSAaRelationshipFilter();
            relFilter.setOwner(
                  new PSLegacyGuid(originalItemGuid.getUUID(), getCurrentRevision(originalItemGuid))
                        .longValue());
            relFilter.setLimitToOwnerRevisions(true);
            PSAaRelationship[] origAaRels = loadAaRelationships(binding, relFilter);

            final NewTranslationsRequest request =
                  createTranslationRequest(ids, new PSAutoTranslation[] {autoTranslations.get(0)});
            ...
            PSAaRelationship[] transAaRels = loadAaRelationships(binding, relFilter);
            ...
            PSRelationship[] transRels2 = loadRelationships(sysBinding, rFilter);
            assertTrue(newCopies[0].getId() == newCopies2[0].getId());
            assertTrue(transRels2.length == 1);
            assertTrue(transRels[0].getDependentId() == transRels2[0].getDependentId());

            relFilter.setOwner(newCopies2[0].getId());
            PSAaRelationship[] transAaRels2 = loadAaRelationships(binding, relFilter);
   ...
   /**
    * If <code>original</code> is <code>true</code>, looks for a translation relationship on the
    * item identified by the supplied guid. If found, the dependent item is processed recursively.
    * If <code>original</code> is <code>false</code>, all AA rels are followed and the dependent
    * items are processed recursively. If the item's lang is not en-us, it is purged.
    *
    * @param binding Assumed not <code>null</code>.
    *
    * @param itemGuid The id of the item to process. This item is deleted after processing all
    *        relationships unless the <code>original</code> flag is <code>true</code>. Assumed not
    *        <code>null</code>.
    *
    * @param original <code>true</code> if <code>itemGuid</code> is for the untranslated item,
    *        <code>false</code> if it is one of the translated items in the translation graph.
    *
    * @param idsToDelete Supply an empty set on the first call. Used for recursive processing.
    */
   private void purgeTranslations(ContentSOAPStub binding, SystemSOAPStub sysBinding,
         IPSGuid itemGuid, boolean original, List<Long> idsToDelete) throws Exception {
      if (original) {
         PSRelationshipFilter rFilter = new PSRelationshipFilter();
         rFilter.setCategory(PSRelationshipFilterCategory.translation);
         ...
         for (PSRelationship r : rels) {
            purgeTranslations(binding, sysBinding, new PSLegacyGuid(r.getDependentId()), false,
                  idsToDelete);
         }
   ...
   /**
    *
    * param g Assumed not null and that it is of type {@link PSTypeEnum#LEGACY_CONTENT}.
    *
    * @return -1 if no item is found with the supplied id, otherwise, the current revision of that
    *         item.
    */
   ...
   /**
    * Attempt to translate the EI site tree logged in as the admin1 user in the EI Admin community.
    * Verify that the cloned site looks like the original site, with all folders and items
    * translated, regardless of object community.
    *
    * @throws Exception
    */
   ...
   /*
    * left here for testing purposes, easy way to disable all tests except what is needed when
    * creating new tests or fixing existing
    */
   ...
   public void testCreateNewTranslations() throws Exception {
      ...
      try {
         ...
         final PSAutoTranslation[] autoTranslations =
               createTestAutoTranslations(locales).toArray(new PSAutoTranslation[0]);

         // create 3 test items for the new content type
         PSTestUtils.setSessionHeader(binding, m_session);
         final List<PSItem> items = createTestItems(contentType.getName(), 3, true, true, false,
         ...
         binding.clearHeaders();
         try {
            binding.newTranslations(createTranslationRequest(ids, autoTranslations));
         ...
         // try with invalid ids
         try {
            binding.newTranslations(createTranslationRequest(null, autoTranslations));
            fail("Should have thrown exception");
         ...
         // try with invalid ids
         try {
            binding.newTranslations(createTranslationRequest(new long[0], autoTranslations));
            fail("Should have thrown exception");
         } catch (PSContractViolationFault expected) {
         }
         ...
         // try with invalid relationship type
         try {
            final NewTranslationsRequest request = createTranslationRequest(ids, autoTranslations);
            request.setType("sometype");
         ...
         try {
            ...
            final NewTranslationsRequest request =
                  createTranslationRequest(ids, new PSAutoTranslation[] {autoTranslation});
         ...
         {
            final NewTranslationsRequest request =
                  createTranslationRequest(ids, new PSAutoTranslation[] {autoTranslations[0]});
         ...
         try {
            final NewTranslationsRequest request =
                  createTranslationRequest(new long[] {allCopies.get(0).getId()},
         ...
         {
            final NewTranslationsRequest request =
                  createTranslationRequest(ids, new PSAutoTranslation[] {autoTranslations[1]});
            request.setEnableRevisions(true);
            final PSItem[] newCopies = binding.newTranslations(request);
            assertEquals(newCopies.length, ids.length);
            verifyRevisionLocks(newCopies, true);
         ...
         // recreate the translations for all test locales
         {
            final NewTranslationsRequest request = createTranslationRequest(ids, autoTranslations);
            final PSItem[] newCopies = binding.newTranslations(request);
            assertEquals(newCopies.length, ids.length * autoTranslations.length);
         ...
         {
            final NewTranslationsRequest request = createTranslationRequest(ids, null);
            final PSItem[] newCopies = binding.newTranslations(request);
    ...
    * @param autoTranslations the translations storing the locales to translate to. Can be
    *        <code>null</code>.
    * @return the new translation request of the translation type, with enable revisions is set to
    *         "false".
    */
   private NewTranslationsRequest createTranslationRequest(final long[] ids,
         PSAutoTranslation[] autoTranslations) {
    ...
    *
    * @param items the items to verify, assumed not <code>null</code>, may be empty.
    * @param enabled <code>true</code> if the revision locks should be enabled, <code>false</code>
    *        otherwise.
    */
   @Test
   ...
   /**
    * Verifies that the supplied items are attached to the paths specified in the same order.
    *
    * @param items the items to verify, assumed not <code>null</code>, may be empty.
    * @param paths the folder paths to verify, assumed not <code>null</code> and of same length as
    *        items.
    * @param none <code>true</code> to specify that none of the items should be attached to a
    *        folder, <code>false</code> otherwise.
    * @param binding the stub to do the lookup, assumed not <code>null</code>.
    * @throws Exception for any error.
    */
   @Test
   private void verifyFolders(PSItem[] items, String[] paths, boolean none, ContentSOAPStub binding)
         throws Exception {
   ...
   /**
    * Test the findRevisions service. Requires that revisions exist for fixed items (currently 360
    * and 471).
    *
    * @throws Exception if the test fails
    */
   ...
   public void testContentSOAPFindRevisions() throws Exception {
      ...
      for (PSRevisions rev : revisionsList) {
         assertTrue(rev.getRevisions() == null || rev.getRevisions().length == 0);
      }
   ...
   /**
    * Test promoteRevisions web service. Requires a revision exists for fixed item (currently 335).
    *
    * @throws Exception If the test fails
    */
   @Test
   public void fix_testPromoteRevision() throws Exception {
      ...
      modField.setPSFieldValue(new PSFieldValue[] {new PSFieldValue("test: " + new Date(), null)});
      ...
      loadReq.setId(new long[] {new PSLegacyGuid(contentId, numRevs).longValue()});
      ...
      promoteReq = new long[] {promoteId};
      binding.promoteRevisions(promoteReq);
      binding.releaseFromEdit(new ReleaseFromEditRequest(new PSItemStatus[] {itemStatus}, false));
   ...
   /**
    * Test checkin and checkout of items. Requires a revision exists for fixed item (currently 335).
    *
    * @throws Exception if the test fails.
    */
   @Test
   public void fix_testContentSOAPCheckInOut() throws Exception {
      ...
      try {
         ...
         try {
            ...
            assertFalse("should not have thrown: " + e.getLocalizedMessage(), true);
         }
   ...
   /**
    * Use the audit trail to determine if the item was checked out, assumes item has been public.
    *
    * @param sysBinding The binding to use, assumed not <code>null</code> and to have a session set
    *        on it.
    * @param ctId The item content id.
    * @param user The actor, assumed not <code>null</code> or empty.
    * @param comment Optional comment, may be <code>null</code> or empty, used to check if the audit
    *        trail comment matches.
    *
    * @return <code>true</code> if the last audit trail entry indicates the item was checked out by
    *         the specified user with the specified comment, <code>false</code> if not.
    *
    * @throws Exception If there are any errors.
    */
   @Test
   private boolean didCheckOut(SystemSOAPStub sysBinding, long ctId, String user, String comment)
         throws Exception {
      ...
      boolean isCheckout = audit.getActor().equals(user) && audit.getTransitionId() == 0;
   ...
   /**
    * Use the audit trail to determine if the item was checked in, assumes item has been public.
    *
    * @param sysBinding The binding to use, assumed not <code>null</code> and to have a session set
    *        on it.
    * @param ctId The item content id.
    ...
    *        trail comment matches.
    *
    * @return <code>true</code> if the last audit trail entry indicates the item was checked in by
    *         the specified user with the specified comment, <code>false</code> if not.
    *
    * @throws Exception If there are any errors.
    */
   @Test
   private boolean didCheckIn(SystemSOAPStub sysBinding, long ctId, String user, String comment)
         throws Exception {
      PSAuditTrail[] trails = sysBinding.loadAuditTrails(new long[] {ctId});
      assertTrue(trails.length == 1);
      PSAuditTrail trail = trails[0];
      ...
      boolean isCheckin = audit.getActor().equals(user) && audit.getTransitionId() == 0;
   ...
   /**
    * Save the items supplied with the request and update the items ids after a successful save.
    *
    * @param request the save request with all items and options to save, assumed not
    *        <code>null</code>.
    * @param binding the binding to use for the save operation, assumed not <code>null</code>.
    * @return the new ids of the saved items, never <code>null</code> or empty.
    * @throws Exception fof any error.
    */
   private long[] saveItems(SaveItemsRequest request, ContentSOAPStub binding) throws Exception {
   ...
   /**
    * Populate the field values for all required fields to make sure we are able to save the items.
    *
    * @param items all items for which to fill the values of all required fields, assumed not
    *        <code>null</code>, may be empty.
    * @param communityId the community id to fill the <code>sys_communityid</code> field with,
    *        assumed not <code>null</code> or empty.
    * @param workflowId the workflow id to ffill the <code>sys_workflowid</code> field with, assumed
    *        not <code>null</code> or empty.
    * @param locale the locale to fill the <code>sys_lang</code> field with, assumed not
    *        <code>null</code> or empty.
    */
   private void populateRequiredFieldValues(List<PSItem> items, String communityId,
         String workflowId, String locale) {
         ...
         for (PSField field : item.getFields()) {
            ...
            if (dimension.equals(PSFieldDimension.required)
               ...
               if (field.getName().equals("sys_communityid")) {
                  ...
                  String value = "item_" + itemIndex + ".field_" + textFieldIndex++;
                  ...
                  updateBinaryField(item, field.getName(), "Some binary !@#$%^&", null, null);
               } else {
   ...
   /**
    * Populates the local parent fields and all children for the test content type
    * <code>test3</code>.
    *
    * @param items the items to populate, not <code>null</code>, may be empty.
    * @param rowCount the number of rows to create for each child, <= 0 to leave all children empty.
    * @param updateId an update identifier used to differentiate contents between updates.
    * @param binaryFile <code>LARGE</code> or <code>SMALL</code> if the identified binary field is
    *        expected as attachment, <code>SKIP</code> if it is expected base64 encoded.
    ...
    * @throws Exception for any error.
    */
   private void populateTestFields(List<PSItem> items, int rowCount, int updateId,
         BinaryFiles binaryFile, String session, ContentSOAPStub binding) throws Exception {
      int itemIndex = 0;
      for (PSItem item : items) {
         // local parent fields
         ...
         updateBinaryField(item, "field_2",
               itemIndex + updateId + ": some binary data !@#$%^^&&*(*_)+", file, binding);
         ...
         if (rowCount > 0) {
            ...
            for (PSItemChildren child : children) {
               ...
               if (childEntries == null) {
                  ...
                  childEntries =
                        createChildEntries(item.getId(), child.getName(), rowCount, session);
                  child.setPSChildEntry(childEntries);
               } else if (childEntries.length < rowCount) {
                  // add child entries
                  int newCount = rowCount - childEntries.length;
                  PSChildEntry[] newEntries =
                        createChildEntries(item.getId(), child.getName(), newCount, session);

                  PSChildEntry[] newChildEntries = new PSChildEntry[rowCount];

                  // copy existing
                  java.lang.System.arraycopy(childEntries, 0, newChildEntries, 0,
                        childEntries.length);

                  // copy new
                  java.lang.System.arraycopy(newEntries, 0, newChildEntries, childEntries.length,
                        newEntries.length);
   ...
   /**
    * Creates the specified number of content items for the specified content type, populates all
    * required field values and saves the items as requested.
    *
    * @param contentType the content type of the new items to create, not <code>null</code> or
    *        empty.
    * @param count the number of items to create, must be > 0.
    * @param populateRequiredFields <code>true</code> to populate all required fields values,
    *        <code>false</code> otherwise.
    * @param saveItems <code>true</code> to save the newly created items to the repository,
    *        <code>false</code> to return them unsaved.
    * @param enableRevisions <code>true</code> to enable revisions for all saved items,
    *        <code>false</code> otherwise.
    * @param path the folder path to which to attach the created items, may be <code>null</code> or
    *        empty.
    * @param session the user session, not <code>null</code> or empty.
    * @param binding the stub used for the item creation, not <code>null</code>.
    * @return a list with all new items created, never <code>null</code> or empty. The returned
    *         items are not persisted yet.
    * @throws Exception for any error.
    */
   @Test
   public List<PSItem> createTestItems(String contentType, int count,
         boolean populateRequiredFields, boolean saveItems, boolean enableRevisions, String path,
         String session, ContentSOAPStub binding) throws Exception {
      ...
      if (populateRequiredFields) {
         ...
         com.percussion.webservices.security.data.PSLocale defaultLocale = getDefaultLocale();
         String locale = defaultLocale.getCode();

         List<PSWorkflow> workflows = catalogWorkflows(session);
         assertFalse("Need at least one workflow definition.", workflows.isEmpty());
         long workflowId = workflows.get(0).getId();

         populateRequiredFieldValues(items, String.valueOf(communityId), String.valueOf(workflowId),
               locale);
      }
   ...
   /**
    * Creates the specified number of revisions for the supplied items, updates the
    * <code>field_1</code> for each revision. Assumes that revisions are enabled for all items
    * already.
    *
    * @param items the items to create revisions for, assumed not <code>null</code>, may be empty.
    * @param count the number of revisions to create, assumed >= 0.
    * @param binding the stub used to save the items, assumed not <code>null</code>.
    * @return the updated items, never <code>null</code>, may be empty. All items will be checked
    *         out on return.
    * @throws Exception for any error.
    */
   @Test
   private List<PSItem> createItemRevisions(List<PSItem> items, int count, ContentSOAPStub binding)
         throws Exception {
   ...
   /**
    * Convenience method to update a binary field. Base64 encodes the supplied fieldValue and calls
    * {@link #updateField(PSItem, String, String)}.
    *
    * @param attachment an attachment to update the binary field with, may be <code>null</code> in
    *        which case the supplied fieldValue will be used and base64 encoded.
    * @param binding the stub to add the attachment to, assumed not <code>null</code> if attached is
    *        <code>true</code>.
    */
   private void updateBinaryField(PSItem item, String fieldName, String fieldValue, File attachment,
   ...
   /**
    * Convenience method to update a date field. Converts the supplied date to a string and then
    * calls {@link #updateField(PSItem, String, String)}.
    *
    * @param dayOfYear the day of the year to set for the date field, assumed > 0 and < 365.
    */
   private void updateDateField(PSItem item, String fieldName, Date fieldValue, int dayOfYear) {
      Calendar calendar = Calendar.getInstance();
   ...
   /**
    * Convenience method to update a number field. Converts the supplied integer to a string and
    * then calls {@link #updateField(PSItem, String, String)}.
    */
    ...
    *
    * @param item the items to update, assumed not <code>null</code>, may be empty.
    * @param fieldName the name of the field to update, assumed not <code>null</code> or empty.
    ...
    *
    * @param searchResults the search results to convert, assumed not <code>null</code>, may be
    *        empty.
    * @return an array with all items ids, never <code>null</code>, may be empty.
    */
   ...
   /**
    * Create the specified content type used for testing and sets up runtime access for the current
    * logins default community.
    *
    * @param name the name of the content type to create, not <code>null</code> or empty, the
    *        content editor definition must be available under <code>UnitTestResources</code>.
    * @param session a valid session, not <code>null</code> or empty.
    * @param acls a container to return all created runtime acl's for the created content type, not
    *        <code>null</code>, may be empty.
    * @return the new content type, never <code>null</code>.
    * @throws Exception for any error.
    */
   public PSContentType createContentType(String name, String session, List<PSAclImpl> acls)
         throws Exception {
      ...
      PSCommunity defaultCommunity = getDefaultCommunity();
      acls.addAll(setupCommunityAcls(contentType.getId(), defaultCommunity.getName(), session));
    ...
    *
    * @param session the user session, not <code>null</code> or empty.
    * @param contentTypes the content types to remove, may be <code>null</code> or empty.
    * @param acls the acls to remove, may be <code>null</code> or empty.
    * @throws Exception for any error.
    */
   public void cleanUpContentTypes(String session, List<PSContentType> contentTypes,
   ...
   /**
    * Convenience method that calls {@link #cleanUpContentTypes(String, List, List)}.
    */
   public void cleanUpContentType(String session, PSContentType contentType, List<PSAclImpl> acls)
         throws Exception {
    ...
    *
    * @param session the user session, not <code>null</code> or empty.
    * @param contentTypeName the name of the content types to remove, not <code>null</code> or
    *        empty.
    * @throws Exception for any error.
    */
   public void cleanUpContentType(String session, String contentTypeName) throws Exception {
      if (StringUtils.isBlank(session))
         throw new IllegalArgumentException("session may not be null or empty.");
      if (StringUtils.isBlank(contentTypeName))
         throw new IllegalArgumentException("contentTypeName may not be null or empty.");
    ...
    * @return a list with all test locales, never <code>null</code> or empty.
    * @throws Exception for any error.
    */
    ...
    *
    * @param locales a list with all locales for which to create auto translations, assumed not
    *        <code>null</code>, may be empty.
    * @return a list with all created auto translations of the same size as the locales list, never
    *         <code>null</code>. These auto translations are not initialized except the locale
    *         field.
    */
   private List<PSAutoTranslation> createTestAutoTranslations(List<PSLocale> locales) {
