	...
	<xsl:template match="/">
		<PSXParam>
			<ArticleTitle>
				<xsl:choose>
					<xsl:when
						test="string(//html:body[1]//html:p[@class='ArticleDisplayTitle'])">
						<xsl:apply-templates
			...
			<ArticleAbstract>
				<xsl:choose>
					<xsl:when
						test="string(//html:body[1]//html:p[@class='ArticleAbstract'])">
						<xsl:apply-templates
							select="//html:body[1]//html:p[@class='ArticleAbstract']" />
					</xsl:when>
					<xsl:otherwise>
			...
			<ArticleAuthor>
				<xsl:choose>
					<xsl:when
						test="string(//html:body[1]//html:p[@class='ArticleAuthor'])">
						<xsl:apply-templates
							select="//html:body[1]//html:p[@class='ArticleAuthor']" />
					</xsl:when>
					<xsl:otherwise>
