...
<xsl:stylesheet version="1.1"
	...
	<xsl:template
		match="rxslot[@template='Zigzag Links' and not(linkurl)]"
		mode="rxslot">
		<xsl:comment>
			Empty ZigZag Links Slot
		</xsl:comment>
	</xsl:template>

	<xsl:template
		match="rxslot[@template='Zigzag Links' and linkurl]" mode="rxslot">
		<xsl:comment>
			Start ZigZag Links Template
		</xsl:comment>
		<table width="300" border="1">
			<xsl:for-each select="linkurl[(position() mod 2) = 1]">
				<tr>
					<td align="left">
						<xsl:copy-of
							select="document(Value/@current)/*/body/*" />
					</td>
				</tr>
				<xsl:apply-templates
					select="following-sibling::linkurl[1]" mode="rightlink" />
			</xsl:for-each>
		</table>
		<xsl:comment>
			End ZigZag Links Template
		</xsl:comment>
	</xsl:template>
	...
	<xsl:template
		match="rxslot[@template=&apos;Related Articles&apos;]" mode="rxslot">
		<xsl:if test="linkurl">
			<!-- start snippet wrapper -->

			<xsl:apply-templates mode="rxcas-5"
				select="linkurl" />
			<!-- end snippet wrapper -->
	...
	<xsl:template match="linkurl" mode="rxcas-5">
		<span id="psx-RelatedItems/sys_AssemblerInfo" psxeditslot="yes"
			slotname="Related Articles" template="SimpleTableSlot">Related Articles</span>
	</xsl:template>
				...
				<td>
					<table width="100%" border="0" cellspacing="0"
						...
						<xsl:apply-templates select="displaytitle"
							mode="mode0" />
						<xsl:apply-templates select="abstractcontent"
							mode="mode1" />
						<tr>
							<td>
								...
								<xsl:variable name="rxslot-6">
									<rxslot psxeditslot="yes" slotname="Related Articles"
										template="SimpleTableSlot">
										<xsl:copy-of
											select="$related/linkurl[@slotname=&apos;Related Articles&apos;]" />
									</rxslot>

								</xsl:variable>

								<xsl:apply-templates mode="rxslot"
									select="$rxslot-6" />
	...
	<xsl:template
		match="rxslot[@template=&apos;Related Links&apos;]" mode="rxslot">
		<xsl:if test="linkurl">
			<!-- start snippet wrapper -->

			<xsl:apply-templates mode="rxcas-7"
				select="linkurl" />
			<!-- end snippet wrapper -->
	...
	<xsl:template match="linkurl" mode="rxcas-7">
		<span id="psx-RelatedItems/sys_AssemblerInfo" psxeditslot="yes"
			slotname="Related Links" template="Zigzag Links">Related Links</span>
	</xsl:template>
			...
			<tr>
				...
				<td valign="top" width="20%">
					...
					<xsl:variable name="rxslot-8">
						<rxslot psxeditslot="yes" slotname="Related Links"
							template="Zigzag Links">
							<xsl:copy-of
								select="$related/linkurl[@slotname=&apos;Related Links&apos;]" />
						</rxslot>
