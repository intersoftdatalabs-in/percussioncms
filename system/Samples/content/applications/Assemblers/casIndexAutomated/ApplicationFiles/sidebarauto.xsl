...
<xsl:stylesheet version="1.1"
		...
		<html>
			...
			<body>
				<div>
					...
					<xsl:variable name="related">
						<xsl:copy-of
							select="document(/*/@AutoRelatedContentURL)/*/RelatedContent/*" />
					</xsl:variable>
					<!-- end XSL -->
					...
					<xsl:variable name="rxslot-2">
						<rxslot psxeditslot="no" slotname="System"
							template="System">
							<xsl:copy-of
								select="$related/linkurl[@slotname=&apos;System&apos;]" />
						</rxslot>
	...
	<xsl:template match="rxslot[@template=&apos;System&apos;]"
		mode="rxslot">
		<xsl:if test="linkurl">
