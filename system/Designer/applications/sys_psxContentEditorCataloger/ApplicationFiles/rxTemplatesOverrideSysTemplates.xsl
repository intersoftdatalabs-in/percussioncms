...
<xsl:stylesheet version="1.1"
	...
	extension-element-prefixes="exslt" exclude-result-prefixes="exslt">
	<xsl:output method="xml" indent="yes" version="1.0"
		encoding="UTF-8" standalone="yes" />
	<xsl:variable name="rxroot" select="/" />
	<xsl:variable name="systemp"
		select="document('file:sys_resources/stylesheets/sys_Templates.xsl')" />
	<xsl:template match="/">
		<xsl:variable name="controls">
			<!-- copy all psxctl:ControlMeta nodes from rx_Templates.xsl into controls
				variable -->
			<xsl:apply-templates select="*" mode="copyrx" />
			<!-- copy only those psxctl:ControlMeta nodes from sys_Templates.xsl which
				are NOT found in the rx_Templates.xsl into controls variable -->
			<xsl:apply-templates select="$systemp/*"
				mode="copysys" />
		</xsl:variable>
		<!-- copy and sort a merged set of psxctl:ControlMeta nodes into the final
			XML document -->
		<Controls>
			<xsl:copy-of select="$systemp//psxctl:ControlMeta" />
			<xsl:apply-templates
				select="exslt:node-set($controls)//*" mode="copysorted">
	...
	<xsl:template match="psxctl:ControlMeta" mode="copysys">
		...
		<xsl:variable name="rxctrl"
			select="$rxroot//psxctl:ControlMeta[@name=$sys_ctrlname]" />
		<xsl:choose>
	...
	<!-- copy and alpha sort a merged collection of psxctl:ControlMeta nodes
