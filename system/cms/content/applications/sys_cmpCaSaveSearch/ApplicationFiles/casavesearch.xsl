...
<xsl:stylesheet version="1.1"
				...
				<title>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_cmpCaSaveSearch.casavesearch@Save Searche Component'" />
	...
	<xsl:template match="casavesearch">
		<form name="savesearch" method="get">
			<xsl:attribute name="action"><xsl:value-of
				select="savesearchurl" /></xsl:attribute>
			...
			<input type="hidden" name="username"
				value="{document(userrolesurl)/UserStatus/UserName}" />
			<br />
			<input type="button" value="Save Search"
				onClick="savesearchfunction();">
				<xsl:attribute name="value"><xsl:call-template
					name="getLocaleString"><xsl:with-param name="key"
					select="'psx.sys_cmpCaSaveSearch.casavesearch@Save Search'" /><xsl:with-param
	...
	<psxi18n:lookupkeys>
		<key
			name="psx.sys_cmpCaSaveSearch.casavesearch@Save Searche Component">Title for the Save Search Component.</key>
