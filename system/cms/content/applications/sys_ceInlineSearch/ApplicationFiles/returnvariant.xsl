...
<xsl:stylesheet version="1.1"
			...
			<head>
				...
				<title>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceInlineSearch.returnvariant@Rhythmyx - Inline Content Search Variant Output'" />
				...
				<xsl:if test="//@inlinetype='rxvariant'">
					...
					<xsl:variable name="tmp1">
						<xsl:apply-templates select="$variantbody"
							mode="copy" />
					</xsl:variable>
	      ...
	      return result;
			...
			<body onload="javascript:returnResult();">
				<form name="returnvariant">
					<input type="hidden" name="inlinetext"
						value="{//@inlinetext}" />
					<input type="hidden" name="dependentvariantid"
						value="{//@dependentvariantid}" />
					<input type="hidden" name="dependentid"
						value="{//@dependentid}" />
					...
					<input type="hidden" name="inlineslotid"
						value="{//@inlineslotid}" />
					<input type="hidden" name="inlinetype"
						value="{//@inlinetype}" />
				...
				<xsl:element name="script">
					...
					var
					conid = document.returnvariant.dependentid.value;
					var siteid =
					document.returnvariant.sys_siteid.value;
					var folderid =
					document.returnvariant.sys_folderid.value;
					var inlineslotid =
					document.returnvariant.inlineslotid.value;
					var inlinetype =
					document.returnvariant.inlinetype.value;
					var inlinetext =
					document.returnvariant.inlinetext.value;
					var urlstring =
					document.returnvariant.urlstring.value;
					var height =
					document.returnvariant.height.value;
					var width =
					document.returnvariant.width.value;
					var linktitle =
					document.returnvariant.linktitle.value;
				</xsl:element>
				<table width="100%" height="100%" cellpadding="0"
					cellspacing="0" border="0" class="headercell">
					<tr class="outerboxcell">
						<td align="center" valign="middle" class="outerboxcellfont">
							<xsl:call-template name="getLocaleString">
								<xsl:with-param name="key"
									select="'psx.generic@Your request is being processed'" />
								<xsl:with-param name="lang" select="$lang" />
							</xsl:call-template>
							.
							<xsl:call-template name="getLocaleString">
								<xsl:with-param name="key"
									select="'psx.generic@Please wait a moment'" />
								<xsl:with-param name="lang" select="$lang" />
							</xsl:call-template>
							...
						</td>
	...
	<psxi18n:lookupkeys>
		<key
			name="psx.sys_ceInlineSearch.returnvariant@Rhythmyx - Inline Content Search Variant Output">Title for inline content search variant output page.</key>
	</psxi18n:lookupkeys>
</xsl:stylesheet>
