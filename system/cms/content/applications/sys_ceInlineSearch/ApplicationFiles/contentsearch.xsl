...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="CreateContentlookup"
		select="/*/contenttypesurl" />
	<xsl:variable name="CreateStatuslookup"
		select="/*/statusurl" />
	<xsl:variable name="contentlookup"
		select="document(//contenttypesurl)" />
		...
		<xsl:choose>
			<xsl:when test="//logincommunity!=0">
				<xsl:for-each
					...
					<xsl:if
						test="contenttype[displayname=$contentlookup//item/display]">
						1
					</xsl:if>
				</xsl:for-each>
			</xsl:when>
			<xsl:otherwise>
				<xsl:if test="not($contentlookup//item/display='')">
					1
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
		...
		<html>
			<head>
               ...
               function community(id, name, contenttypes)
               {
                   ...
                   this.contenttypes = contenttypes;
					...
					<xsl:element name="script">
						...
						<xsl:for-each
							...
							<xsl:for-each
								select="contenttype[displayname=$contentlookup//item/display]">
				...
				<title>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceInlineSearch.contentsearch@Rhythmyx - Inline Content Search'" />
				...
				<script src="../sys_ceInlineSearch/searchinit.js"
					language="javascript">;</script>
			...
			<body onload="javascript:self.focus();"
					...
					<table align="center" width="75%" border="0" cellspacing="1"
						...
						<tr class="headercell">
							<td colspan="2" align="left" class="headercellfont">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'psx.sys_ceInlineSearch.contentsearch@Search Parameters'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
						</tr>
							...
							<xsl:when test="$checkContentTypes=''">
								<tr class="datacell1">
									<td align="left" class="datacell1font">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@No variants are available for the Inline Slot. Please consult your Rhythmyx Administrator.'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
									</td>
								</tr>
								<tr class="datacell2">
									<td colspan="2" align="center">
										...
										<input type="button" name="Cancel" value="Close"
											language="javascript" onclick="return Cancel_onclick()">
							...
							<xsl:otherwise>
								<tr class="datacell1">
									<td align="left" class="datacell1font">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Title'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
										:
									</td>
									<td class="datacell1font">
										<input size="15" type="text" name="sys_contenttitle">
											<xsl:attribute name="value"><xsl:value-of
												select="/*/contenttitle" /></xsl:attribute>
										</input>
									</td>
								</tr>
								<tr class="datacell1">
									<td align="left" class="datacell1font">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@ID'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
										:
									</td>
									<td class="datacell1font">
										<input size="15" type="text" name="sys_contentid">
											<xsl:attribute name="value"><xsl:value-of
												select="/*/contentid" /></xsl:attribute>
										</input>
									</td>
								</tr>
								<xsl:choose>
									<xsl:when test="//logincommunity!=0">
										<tr class="datacell1" border="0">
											<td align="left" class="datacell1font">
												<xsl:call-template name="getLocaleString">
													<xsl:with-param name="key"
														select="'psx.sys_ceInlineSearch.contentsearch@Community'" />
													<xsl:with-param name="lang" select="$lang" />
												</xsl:call-template>
												:
											</td>
											<td align="left" class="datacell1font">
												<select name="sys_communityid"
										...
										<tr class="datacell1" border="0">
											<td align="left" class="datacell1font">
												<xsl:call-template name="getLocaleString">
													<xsl:with-param name="key"
														select="'psx.sys_ceInlineSearch.contentsearch@Content Type'" />
													<xsl:with-param name="lang" select="$lang" />
												</xsl:call-template>
												:
											</td>
									...
									<xsl:otherwise>
										<xsl:apply-templates
											select="document($CreateContentlookup)/*" mode="mode2">
								...
								<xsl:apply-templates
									select="document($CreateStatuslookup)/*" mode="mode3">
								...
								<tr class="datacell2">
									<td align="left" class="datacell1font">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Author'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
										:
									</td>
								...
								<tr class="datacell2">
									<td class="datacell1font" align="left">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Start Date Before'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
										:
									</td>
									<td class="datacell1font">
										<input type="text" name="sys_enddate" size="12">
											<xsl:attribute name="value"><xsl:value-of
												select="/*/enddate" /></xsl:attribute>
										</input>&nbsp;
										<a href="#"
											onclick="showCalendar(document.forms[0].sys_enddate);">
											<img border="0" height="20" width="20"
												src="../sys_resources/images/cal.gif">
												<xsl:attribute name="alt"><xsl:call-template
													name="getLocaleString"><xsl:with-param name="key"
													select="'psx.sys_ceInlineSearch.contentsearch.alt@Calendar Pop-up'" /><xsl:with-param
													name="lang" select="$lang" /></xsl:call-template></xsl:attribute>
											</img>
										</a>
									</td>
								...
								<tr class="datacell1">
									<td align="left" class="datacell1font">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Start Date After'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
										:
									</td>
									<td class="datacell1font">
										<input type="text" name="sys_startdate" size="12">
											<xsl:attribute name="value"><xsl:value-of
												select="/*/startdate" /></xsl:attribute>
										</input>&nbsp;
										<a href="#"
											onclick="showCalendar(document.forms[0].sys_startdate);">
											<img border="0" height="20" width="20"
												src="../sys_resources/images/cal.gif">
												<xsl:attribute name="alt"><xsl:call-template
													name="getLocaleString"><xsl:with-param name="key"
								...
								<tr class="datacell2">
									<td colspan="2" align="center">
										...
										<input type="button" name="Search" value="Search"
											language="javascript" onclick="return Search_onclick()">
											<xsl:attribute name="value"><xsl:call-template
												name="getLocaleString"><xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Search'" /><xsl:with-param
												name="lang" select="$lang" /></xsl:call-template></xsl:attribute>
										</input>&nbsp;
										<input type="button" name="Reset" value="Reset"
											language="javascript" onclick="return Reset_onclick()">
											<xsl:attribute name="value"><xsl:call-template
												name="getLocaleString"><xsl:with-param name="key"
												select="'psx.sys_ceInlineSearch.contentsearch@Reset'" /><xsl:with-param
												name="lang" select="$lang" /></xsl:call-template></xsl:attribute>
										</input>&nbsp;
										<input type="button" name="Cancel" value="Close"
											language="javascript" onclick="return Cancel_onclick()">
				...
				<xsl:if
					test="//logincommunity!=0 and not($checkContentTypes='')">
					<script>
	...
	<xsl:template match="*" mode="mode3">
		...
		<tr class="datacell1">
			<td class="datacell1font" align="left">
				<xsl:call-template name="getLocaleString">
					<xsl:with-param name="key"
						select="'psx.sys_ceInlineSearch.contentsearch@Status'" />
					<xsl:with-param name="lang" select="$lang" />
				</xsl:call-template>
				:
			</td>
	...
	<psxi18n:lookupkeys>
		<key
			name="psx.sys_ceInlineSearch.contentsearch@Rhythmyx - Inline Content Search">Title for the Inline Content Search dialog box.</key>
		...
		<key name="psx.sys_ceInlineSearch.contentsearch@Start Date Before">Start Date Before label in Inline Content Search dialog
			box.</key>
		<key name="psx.sys_ceInlineSearch.contentsearch@Start Date After">Start Date After label in Inline Content Search dialog
			box.</key>
		<key
			name="psx.sys_ceInlineSearch.contentsearch.alt@Calendar Pop-up">Alt text for calender image in Inline Content Search dialog box.</key>
		...
		<key
			name="psx.sys_ceInlineSearch.contentsearch@No variants are available for the Inline Slot. Please consult your Rhythmyx Administrator.">Message appears when no variants are available for the selected
			inline slot.</key>
	</psxi18n:lookupkeys>
</xsl:stylesheet>
