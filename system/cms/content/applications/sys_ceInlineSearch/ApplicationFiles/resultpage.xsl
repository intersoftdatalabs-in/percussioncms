...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="/">
		<html>
				...
				<title>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceInlineSearch.resultpage@Rhythmyx - Inline Content Search Results'" />
			...
			<body onload="javascript:self.focus();">
				<form name="updaterelateditems" method="post"
					action="../sys_ceInlineSearch/returnvariant.html">
					<input type="button" name="searchagain" value="Search Again"
						onclick="javascript:onClickSearchAgain()">
						<xsl:attribute name="value"><xsl:call-template
							name="getLocaleString"><xsl:with-param name="key"
							select="'psx.sys_ceInlineSearch.resultpage@Search Again'" /><xsl:with-param
							name="lang" select="$lang" /></xsl:call-template></xsl:attribute>
					</input>&nbsp;
					<input type="button" name="cancel" value="Close"
						onclick="javascript:onClickCancel()">
						<xsl:attribute name="value"><xsl:call-template
							name="getLocaleString"><xsl:with-param name="key"
					...
					<div align="center">
						<table width="100%" cellpadding="0" cellspacing="3"
							...
							<tr class="headercell">
								<td align="center">
									<span class="headercellfont">Link Title: </span>
									<input name="linktitle" value="" type="text" size="40" />
								</td>
							</tr>
							<tr class="headercell">
								<td align="center">
					...
					<table width="100%" border="0" cellspacing="1"
						...
						<tr class="headercell2">
							<td class="headercell2font" align="center">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'psx.sys_ceInlineSearch.resultpage@Content Title (ID)'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>&nbsp;&nbsp;&nbsp;
							</td>
							<td class="headercell2font" align="center">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'psx.sys_ceInlineSearch.resultpage@Content Type'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="headercell2font" align="center">&nbsp;
	...
	<xsl:template match="item" mode="item">
		...
		<tr class="datacell1">
			...
			<td align="left" class="datacell1font">
				<a href="#">
					<xsl:attribute name="onclick">javascript:onClickUpdate('<xsl:value-of
						select="$varid" />','<xsl:value-of select="contentid" />','<xsl:value-of
			...
			<td align="left" class="datacell1font">
				<a>
					...
					<xsl:attribute name="onclick">javascript:window.open(&quot;<xsl:value-of
						select="previewurl" />&quot;,&quot;preview&quot;, &quot;toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=400,height=300,z-lock=1&quot;)</xsl:attribute>
					<img src="/sys_resources/images/preview.gif" alt="Preview"
						align="top" border="0">
						<xsl:attribute name="alt"><xsl:call-template
							name="getLocaleString"><xsl:with-param name="key"
							select="'psx.sys_ceInlineSearch.resultpage.alt@Preview'" /><xsl:with-param
							name="lang" select="$lang" /></xsl:call-template></xsl:attribute>
					</img>
				</a>
			</td>
	...
	<xsl:template match="*" mode="paging">
		<table>
			<tr>
				<td>
					<xsl:apply-templates select="PSXPrevPage"
						mode="paging" />
					<xsl:apply-templates select="PSXIndexPage"
						mode="paging" />
					<xsl:apply-templates select="PSXNextPage"
						mode="paging" />
				</td>
			</tr>
		</table>
	...
	<xsl:template match="PSXIndexPage" mode="paging">
		<xsl:choose>
			<xsl:when test="not(/*/currentpsfirst='')">
				<xsl:choose>
					<xsl:when
						test="substring-after(.,'psfirst=')=/*/currentpsfirst">
	...
	<psxi18n:lookupkeys>
		<key
			name="psx.sys_ceInlineSearch.resultpage@Rhythmyx - Inline Content Search Results">Title for inline content search result page.</key>
		<key name="psx.sys_ceInlineSearch.resultpage@Search Again">Label for Search Again button in inline content search
			result page.</key>
		<key name="psx.sys_ceInlineSearch.resultpage@Content Title (ID)">Second column header in inline content search result
			page.</key>
		<key name="psx.sys_ceInlineSearch.resultpage@Content Type">Third column header in inline content search result
			page.</key>
		<key name="psx.sys_ceInlineSearch.resultpage.alt@Preview">Alt text for preview image in inline content search
			result page.</key>
	</psxi18n:lookupkeys>
</xsl:stylesheet>
