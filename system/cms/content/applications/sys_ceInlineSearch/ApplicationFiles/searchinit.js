function onFormLoad() {
  ...
  for (i = 1; i <= communities.length; i++) {
    document.updatesearch.sys_communityid.options[i].value =
      communities[i - 1].id;
    document.updatesearch.sys_communityid.options[i].text =
      communities[i - 1].name;
...
function community_onchange() {
  ...
  if (index == 0) {
    document.updatesearch.sys_contenttype.options.length =
      allcontenttypes.length + 1;
    ...
    for (j = 1; j <= allcontenttypes.length; j++) {
      document.updatesearch.sys_contenttype.options[j].value =
        allcontentids[j - 1];
      document.updatesearch.sys_contenttype.options[j].text =
        allcontenttypes[j - 1];
    }
  } else {
    ...
    for (i = 1; i <= contenttypes.length; i++) {
      document.updatesearch.sys_contenttype.options[i].value =
        contenttypes[i - 1].id;
      document.updatesearch.sys_contenttype.options[i].text =
        contenttypes[i - 1].name;
    }
  }
