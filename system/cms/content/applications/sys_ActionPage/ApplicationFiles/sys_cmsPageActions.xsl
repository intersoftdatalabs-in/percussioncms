...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="Action[@name='Item_ViewDependents']"
		mode="table">
	...
	<xsl:template
		match="Action[contains(@name,'_Preview') or contains(@name,'_ActiveAssembly')]"
		mode="rightmenu">
		<xsl:param name="contentid" />
		...
		<xsl:variable name="jsfunction">
			<xsl:choose>
				<xsl:when test="contains(@name,'_Preview')">
					PSPreviewItem
				</xsl:when>
				<xsl:otherwise>
					PSActiveAssemblyItem
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:call-template name="subtable">
			...
			<xsl:with-param name="icon">
				<xsl:choose>
					<xsl:when test="contains(@name,'_ActiveAssembly')">
						Active_Assembly.gif
					</xsl:when>
					<xsl:when test="contains(@name,'_Preview')">
						Prev_Intra_and_Pub.gif
					</xsl:when>
				</xsl:choose>
			</xsl:with-param>
			<xsl:with-param name="content">
				<xsl:apply-templates
					select="document(concat(substring-before(//@wfurlint, 'Rhythmyx'), 'Rhythmyx/sys_rcSupport/variantlist.xml?sys_contentid=', $contentid, '&amp;pssessionid=',$sessionid,'&amp;sys_folderid=',$folderid,'&amp;sys_siteid=',$siteid))/*"
					mode="list">
					<xsl:with-param name="urlParams"
						select="document(concat(substring-before(//@wfurlint, 'Rhythmyx'), 'Rhythmyx/sys_cxSupport/Params.xml?sys_actionid=', @actionid, '&amp;pssessionid=',$sessionid))//Params" />
					<xsl:with-param name="contentid" select="$contentid" />
					<xsl:with-param name="revision" select="$revision" />
					<xsl:with-param name="jsfunction"
						select="$jsfunction" />
	...
	<xsl:template match="*" mode="variant-copy">
		...
		<xsl:param name="level" />
		<xsl:param name="siteid"
			select="$urlParams/Param[@name='sys_siteid']" />

		<xsl:variable name="url">
			<xsl:choose>
				<xsl:when test="$jsfunction = 'PSActiveAssemblyItem'">
					../sys_action/checkoutaapage.xml
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="AssemblyUrlPlain" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="absurl"
			select="psxlink:get-abs-link($url)" />
		...
		<xsl:call-template name="item">
			...
			<xsl:with-param name="level" select="$level" />
			<xsl:with-param name="action">
				<!-- use a different assembly url which is a relative URL of the variant
					as registered -->
				<xsl:value-of select="$jsfunction" />
				('
				...
				','
				<xsl:value-of select="$assemblyUrl" />
				')
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
