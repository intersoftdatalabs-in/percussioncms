...
<xsl:stylesheet version="1.1"
	...
	<xsl:template mode="editparentcomponent_mainbody"
		match="*">
			...
			<tr class="headercell">
				<td>
					<table width="100%" cellpadding="0" cellspacing="1"
						border="0">
						<form name="editparentcomponent" method="post"
							action="updateparent.html">
							...
							<input name="parentcomponentid" type="hidden"
								...
								<td class="datacell1font">
									<xsl:choose>
										<xsl:when test="parentcomponentid=''">
											<xsl:apply-templates
												select="document(componentlookup)/*" mode="complookup">
												<xsl:with-param name="parentcompid"
													select="parentcomponentid" />
											</xsl:apply-templates>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of
												select="document(componentlookup)/*/item/componentname" />
										</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
