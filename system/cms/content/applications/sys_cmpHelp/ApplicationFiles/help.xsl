			...
			<head>
				...
				<xsl:call-template name="getLocaleString">
					<xsl:with-param name="key"
						select="'psx.sys_cmpHelp.help@Help'" />
	...
	<xsl:template match="*" mode="mode0">
		<tr class="outerboxcell">
			<td align="center">
				...
				<xsl:variable name="helpIcon"
					select="concat('/rx_resources/images/',$lang,'/help_icon.gif')" />
				<xsl:variable name="helpAlt">
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_cmpHelp.help.alt@Help'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
				</xsl:variable>
				...
				<script language="JavaScript1.2">
					...
					var helpSetFile =
					"../../Docs/Business_Users/Content_Explorer_Help.hs";
					var helpId = "
					<xsl:value-of select="$helpId" />
					";
					var helpIcon = "
					<xsl:value-of select="$helpIcon" />
					";
					var helpAlt = "
					<xsl:value-of select="$helpAlt" />
					";

				</script>
				<script language="JavaScript1.2"><![CDATA[<!--

	        var _codebase = "]]><xsl:value-of
					select="//@codebase" /><![CDATA[";
	        var _classid = "]]><xsl:value-of
					select="//@classid" /><![CDATA[";
	        var _type = "]]><xsl:value-of
					select="concat('application/x-java-applet;',//@version_type,'=',//@implementation_version)" /><![CDATA[";
	        var _pluginpage = "]]><xsl:value-of
					select="concat('http://java.sun.com/products/plugin/',//@implementation_version,'/plugin-install.html')" /><![CDATA[";
	        var _appletcodebase = "]]><xsl:value-of
					select="//@appletcodebase" /><![CDATA[";
				...
				<a href="javascript:void(0)">
                 ...
                 <xsl:choose>
                   ...
                   <xsl:otherwise>
					...
					<img align="absmiddle" alt="{$helpAlt}" border="0"
						src="{$helpIcon}" />
				</a>
			</td>
		</tr>
