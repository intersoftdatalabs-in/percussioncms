...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="variantlist"
		select="document(//variantlisturl)" />
	<xsl:template match="/">
		<html>
			<head>
	...
	<xsl:template match="itemdetails">
		<form name="concat('itemdetails',//itemnumber)">
			<input type="hidden" name="revurl"
				value="concat(//selectrevurl,'&amp;itemnumber=',//itemnumber)" />
			<table width="220" align="center" border="0" cellpadding="0"
				...
				<xsl:for-each select="item">
					...
					<tr class="datacell1">
						...
						<td align="left" class="datacell1font">
							<img src="../sys_resources/images/singlecomment.gif"
								alt="{comment}" />
						</td>
					</tr>
				</xsl:for-each>
				<xsl:apply-templates
					select="$variantlist//VariantList" mode="variantlist" />
				<tr class="datacell2">
					<td width="100%" align="center" valign="middle" colspan="2"
						...
						<input type="button" name="change" value="Select Revision">
							<xsl:attribute name="onclick">javascript:openSelectRev("<xsl:value-of
								select="//selectrevurl" />");</xsl:attribute>
						</input>
					</td>
				</tr>
			</table>
	...
	<xsl:template match="VariantList" mode="variantlist">
		<tr class="datacell1">
			<td colspan="2" align="left" class="datacell1font" height="20">Select
				Template</td>
		</tr>
		<tr class="datacell2">
