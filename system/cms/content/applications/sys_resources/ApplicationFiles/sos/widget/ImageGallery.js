...
dojo.declare("sos.widget.ImageGallery", dojo.widget.HtmlWidget, {
  ...
  updateNavControls: function () {
    ...
    var lastCell =
      this.thumbsTableRow.cells[this.thumbsTableRow.cells.length - 1];
  ...
  loadNextThumbnail: function () {
    ...
    dojo.event.browser.addListener(img, "onload", function () {
      ...
      if (
        pos >= thisWidget.thumbIndex &&
        pos < thisWidget.thumbIndex + thisWidget.numberThumbs
      ) {
        thisWidget.showThumbs();
      }
  ...
  loadImage: function (pos, callbackFn) {
    ...
    if (
      thumbNotifier &&
      !dojo.html.hasClass(thumbNotifier, "image-gallery-loading")
    ) {
  ...
  showImage: function (index, /*optional*/ callback) {
    ...
    var showOrLoadIt = function () {
      if (thisWidget.images[index]) {
        ...
        dojo.lfx.html
          .fadeIn(thisWidget.images[index], 300, null, callback)
          .play();
      } else {
    ...
    if (current && current.length > 0) {
      dojo.lfx.html
        .fadeOut(current[0], 300, null, function () {
...
dojo.declare("sos.widget.FlickrImageGallery", sos.widget.ImageGallery, {
  ...
  addFlickrData: function (flickrData) {
    ...
    for (var i = 0; i < flickrData.length; i++) {
      ...
      var link =
        "http://www.flickr.com/photos/" + owner + "/" + flickrData[i]["id"];
