...
sos.charting.events = {
      ...
      if (!plots[i].hasListener[type]) {
        ...
        dojo.event.browser.addListener(plots[i], type, function (evt) {
          ...
          if (evt.target.tagName == "circle") {
            packet["title"] =
              evt.target.parentNode.firstChild.getAttribute("title");
            packet.value =
              yConvert.baseDisplay +
              (yConvert.base - evt.target.cy.baseVal.value) / yConvert.ratio;
          }
          packet.event = evt;
  ...
  _getPlotAreas: function (chart) {
    ...
    while (child) {
      if (
        child.tagName == "g" &&
        child.id &&
        child.id.indexOf("-plots") == child.id.length - 6
      ) {
  ...
  _getChartAxisConversion: function (chart) {
    ...
    var nodes =
      chart.plotAreas[0].plotArea.plots[0].axisY.nodes.labels.childNodes;
    ...
    chart["YCONVERT"] = {
      ratio: totalYVal / totalDisp,
      base: lowest,
      baseDisplay: lowestDisplay,
    };
