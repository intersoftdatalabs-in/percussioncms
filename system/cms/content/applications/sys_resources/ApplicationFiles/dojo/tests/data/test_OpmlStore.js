...
function showResultViaTreeV3(result) {
  ...
  var tree = dojo.widget.createWidget("TreeV3", {
    listeners: [controller.widgetId],
  });
  var treeDiv = dojo.byId("treeDiv");
  treeDiv.appendChild(tree.domNode);

  var rootTreeNode = dojo.widget.createWidget("TreeNodeV3", {
    title: result.store._opmlFileUrl,
    tree: tree.widgetId,
  });
...
function showItemAsTreeNode(store, item, tree, parentTreeNode) {
  ...
  var treeNode = dojo.widget.createWidget("TreeNodeV3", {
    title: treeNodeTitle,
    tree: tree.widgetId,
  });
