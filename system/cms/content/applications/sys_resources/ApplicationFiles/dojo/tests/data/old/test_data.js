...
function test_data_provider_json_sample1() {
  ...
  while (!iterator.atEnd) {
    ...
    jum.assertTrue(
      "209",
      stateItem.get("abbr") == stateItem.get(abbrAttribute)
    );
    jum.assertTrue(
      "210",
      stateItem.get("population") == stateItem.get(popAttribute)
    );
    jum.assertTrue(
      "211",
      stateItem.get("name") == stateItem.get(nameAttribute)
    );
...
function test_data_provider_json_sample3() {
  ...
  jum.assertTrue(
    "500",
    queryResultSet.getLength() == arrayOfJsonStates.length - 1
  );
  showQueryResultSet(queryResultSet);
}
...
function BROKEN_test_data_provider_json_file() {
  var dataProvider = new dojo.data.old.provider.FlatFile({
    url: "states_with_keywords.json",
  });
  var queryResultSet = dataProvider.fetchResultSet();
  showQueryResultSet(queryResultSet);

  dataProvider = new dojo.data.old.provider.FlatFile({
    url: "states_with_header_row.json",
  });
