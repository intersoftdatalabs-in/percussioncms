...
var rdfTestData = {
  testJson:
    '{"_:1": {"http://rx4rdf.sf.net/ns/wiki#name" : [{"type":"bnode","value":"1"}, {"type":"bnode","value":"2"}]}' +
...
function _load_rdf_data(rhizomeStore, sync, kwArgs) {
  tests.MockXMLHttpRequest.wrap(
    rhizomeStore,
    "find",
    rdfTestData.testJson,
    200
  );
...
function _test_rdf_data(sync) {
  ...
  var callback = function (result) {
    ...
    jum.assertTrue(
      "212",
      rhizomeStore.get(rdfTestData.testId, rdfTestData.testAttr) ==
        rdfTestData.testValue
    );
...
function test_data_rdf_revert() {
  ...
  jum.assertTrue(
    "310",
    rhizomeStore.get(testId, testAttr) == rdfTestData.testValue
  );
