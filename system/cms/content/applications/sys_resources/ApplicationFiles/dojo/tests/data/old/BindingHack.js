...
ChartBindingHack.prototype.getHtmlString = function () {
  ...
  var arrayOfStrings = [];
  arrayOfStrings.push(
    '<div dojoType="chart" style="border:1px solid black;width:420px;background-color:#ededde;">'
  );
  ...
  for (var i in arrayOfItems) {
    ...
    for (var j in map.plots) {
      ...
      if (plotType == "bubble") {
        rowString +=
          '<td size="' + item.get(size) + '">' + item.get(y) + "</td>";
      } else {
...
TableBindingHack.prototype.getHtmlString = function () {
  ...
  var arrayOfStrings = [];
  arrayOfStrings.push(
    '<table dojoType="SortableTable" enableAlternateRows="true" cellpadding="0" cellspacing="0">'
  );
  arrayOfStrings.push("	<thead>");
  arrayOfStrings.push("		<tr>");
  var map = this._attributeMapping;
