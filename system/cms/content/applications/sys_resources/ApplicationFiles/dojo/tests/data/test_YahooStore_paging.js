...
gui = {
  ...
  displayPage: function () {
    ...
    this.textArray = [];
    this.textArray.push(
      "<p>Showing results " + min + " to " + max + " of " + total + "</p>"
    );
    this.textArray.push(
      '<table style="background-color:#EEEEEE; border-collapse:collapse">'
    );
    ...
    (this.result.onnext = this.displayItem), (this.result.oncompleted = null);
  ...
  displayItem: function (item, result) {
    ...
    this.textArray.push(
      '<td width="25%"><a href="' + url + '">' + title + "</a></td>"
    );
    ...
    if (!this.tailEndText) {
      ...
      this.tailEndText +=
        '<input type="button" value="Next Page" onclick="gui.showNextPage();">';
      this.tailEndText += "</td>";
      this.tailEndText += "</tr>";
