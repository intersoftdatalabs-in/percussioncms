...
dojo.declare(
  ...
  },
  {
    ...
    parseData: function (table) {
      ...
      if (!(thead && tbody))
        dojo.raise(
          "dojo.widget.Chart: supplied table must define a head and a body."
        );

      //	set up the series.
      var columns = thead
        .getElementsByTagName("tr")[0]
        .getElementsByTagName("th"); //	should be <tr><..>
  ...
  },
  {
    add: function (v) {
      if (v.x == null || v.value == null) {
        dojo.raise(
          "dojo.widget.Chart.DataSeries.add: v must have both an 'x' and 'value' property."
        );
      }
      this.values.push(v);
    },
...
dojo.requireIf(
  !dojo.render.svg.capable && dojo.render.vml.capable,
