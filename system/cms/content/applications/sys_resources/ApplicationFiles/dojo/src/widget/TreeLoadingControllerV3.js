  ...
  dojo.widget.TreeBasicControllerV3,
  {
    ...
    checkValidRpcResponse: function (type, obj) {
      ...
      if (typeof obj != "object") {
        return new dojo.FormatError(
          "Wrong server answer format " +
            (obj && obj.toSource ? obj.toSource() : obj) +
            " type " +
            typeof obj,
          obj
        );
    ...
    getRpcUrl: function (action) {
      ...
      if (this.RpcUrl == "local") {
        var dir = document.location.href.substr(
          0,
    ...
    loadProcessResponse: function (node, result) {
      ...
      if (!dojo.lang.isArray(result)) {
        throw new dojo.FormatError(
          "loadProcessResponse: Not array loaded: " + result
        );
      }
    ...
    expandToLevel: function (nodeOrTree, level, sync) {
      return this.recurseToLevel(
        nodeOrTree,
        nodeOrTree.isTree ? level + 1 : level,
        this.expand,
        this,
        nodeOrTree.isTree,
        sync
    ...
    startProcessing: function (nodesArray) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        if (nodes[i].isLocked()) {
          deferred.errback(
            new dojo.LockedError("item locked " + nodes[i], nodes[i])
          );
    ...
    refreshChildren: function (nodeOrTree, sync) {
      return this.runStages(
        null,
        this.prepareRefreshChildren,
        this.doRefreshChildren,
        this.finalizeRefreshChildren,
