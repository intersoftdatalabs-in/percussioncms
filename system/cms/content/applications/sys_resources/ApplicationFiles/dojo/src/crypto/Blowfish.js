...
dojo.crypto.Blowfish = new (function () {
  ...
  function init(key) {
    ...
    if (typeof k == "string") {
      var a = [];
      for (var i = 0; i < k.length; i++) a.push(k.charCodeAt(i) & 0xff);
      k = a;
    }
    //	init the boxes
  ...
  this.getIV = function (/* dojo.crypto.outputTypes? */ outputType) {
    ...
    switch (out) {
      case dojo.crypto.outputTypes.Hex: {
        var s = [];
        for (var i = 0; i < iv.length; i++) s.push(iv[i].toString(16));
  ...
  this.setIV = function (
    /* string */ data,
    /* dojo.crypto.outputTypes? */ inputType
  ) {
    //	summary
  ...
  this.encrypt = function (
    ...
    switch (out) {
      case dojo.crypto.outputTypes.Hex: {
        var s = [];
        for (var i = 0; i < cipher.length; i++) s.push(cipher[i].toString(16));
        return s.join(""); //	string
      }
      case dojo.crypto.outputTypes.String: {
  ...
  this.decrypt = function (
    ...
    //	convert to string
    for (var i = 0; i < pt.length; i++) pt[i] = String.fromCharCode(pt[i]);
