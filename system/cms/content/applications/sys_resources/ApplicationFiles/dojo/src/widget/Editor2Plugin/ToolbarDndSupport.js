...
dojo.event.topic.subscribe(
  "dojo.widget.Editor2::preLoadingToolbar",
  function (editor) {
...
dojo.declare("dojo.widget.Editor2Plugin.ToolbarDndSupport", null, {
  ...
  setup: function () {
    ...
    var nodes = dojo.html.getElementsByClass(
      ...
      tbw.domNode,
      null,
      dojo.html.classMatchType.ContainsAny
    );
    if (!nodes) {
      dojo.debug(
        "dojo.widget.Editor2Plugin.ToolbarDndSupport: No dom node with class in " +
          this.lookForClass
      );
      return;
    }
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (droptarget) {
        new dojo.dnd.HtmlDropTarget(node, [
          droptarget + tbw.widgetId,
        ]).vertical = true;
      }
