      ...
      if (ref) {
        ...
        switch (gradient.tagName.toLowerCase()) {
          ...
          case "pattern":
            ...
            fillStyle.src = gradient.firstChild.getAttributeNS(
              dojo.svg.xmlns.xlink,
              "href"
            );
            break;
        }
    ...
    for (var i = 0; i < gradient.childNodes.length; ++i) {
      fillStyle.colors.push({
        ...
        color: new dojo.gfx.color.Color(
          gradient.childNodes[i].getAttribute("stop-color")
        ),
      });
    }
    return fillStyle;
...
dojo.declare("dojo.gfx.Rect", dojo.gfx.shape.Rect, {
  ...
  attachShape: function (rawNode) {
    ...
    if (rawNode) {
      ...
      shape.r = Math.min(
        rawNode.getAttribute("rx"),
        rawNode.getAttribute("ry")
      );
    }
