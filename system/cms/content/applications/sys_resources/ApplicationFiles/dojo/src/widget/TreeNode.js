...
dojo.widget.defineWidget(
  ...
  },
  {
    ...
    templateString: (
      ...
      "</div>"
    ).replace(/(>|<)\s+/g, "$1"), // strip whitespaces between nodes

    childIconSrc: "",
    childIconFolderSrc: dojo.uri.moduleUri(
      "dojo.widget",
      "templates/images/Tree/closed.gif"
    ), // for under root parent item child icon,
    childIconDocumentSrc: dojo.uri.moduleUri(
      "dojo.widget",
      "templates/images/Tree/document.gif"
    ), // for under root parent item child icon,
    ...
    isLastChild: function () {
      return this.getParentIndex() == this.parent.children.length - 1
        ? true
        : false;
    },
    ...
    actionIsDisabled: function (action) {
      ...
      if (
        this.tree.strictFolders &&
        action == this.actions.ADDCHILD &&
        !this.isFolder
      ) {
        disabled = true;
      }
    ...
    setFolder: function () {
      ...
      this.expandIcon.src = this.isExpanded
        ? this.tree.expandIconSrcMinus
        : this.tree.expandIconSrcPlus;
      this.isFolder = true;
    },
    ...
    createDOMNode: function (tree, depth) {
      ...
      dojo.event.topic.publish(this.tree.eventNames.createDOMNode, {
        source: this,
      });
    ...
    onTreeClick: function (e) {
      dojo.event.topic.publish(this.tree.eventNames.treeClick, {
        source: this,
        event: e,
      });
    },
    ...
    updateExpandGrid: function () {
      if (this.tree.showGrid) {
        if (this.depth) {
          this.setGridImage(
            -2,
            this.isLastChild() ? this.tree.gridIconSrcL : this.tree.gridIconSrcT
          );
        } else {
          if (this.isFirstChild()) {
            this.setGridImage(
              -2,
              this.isLastChild()
                ? this.tree.gridIconSrcX
                : this.tree.gridIconSrcY
            );
          } else {
            this.setGridImage(
              -2,
              this.isLastChild()
                ? this.tree.gridIconSrcL
                : this.tree.gridIconSrcT
            );
          }
        }
      } else {
    ...
    updateChildGrid: function () {
      if ((this.depth || this.tree.showRootGrid) && this.tree.showGrid) {
        this.setGridImage(
          -1,
          this.children.length && this.isExpanded
            ? this.tree.gridIconSrcP
            : this.tree.gridIconSrcC
        );
      } else {
        if (this.tree.showGrid && !this.tree.showRootGrid) {
          this.setGridImage(
            -1,
            this.children.length && this.isExpanded
              ? this.tree.gridIconSrcZ
              : this.tree.blankIconSrc
          );
        } else {
    ...
    updateParentGrid: function () {
      ...
      for (var i = 0; i < this.depth; i++) {
        ...
        var img =
          this.tree.showGrid && !parent.isLastChild()
            ? this.tree.gridIconSrcV
            : this.tree.blankIconSrc;
    ...
    hideChildren: function () {
      this.tree.toggleObj.hide(
        ...
        this.toggleDuration,
        this.explodeSrc,
        dojo.lang.hitch(this, "onHide")
      );

      /* if dnd is in action, recalculate changed coordinates */
      if (
        dojo.exists(dojo, "dnd.dragManager.dragObjects") &&
        dojo.dnd.dragManager.dragObjects.length
      ) {
