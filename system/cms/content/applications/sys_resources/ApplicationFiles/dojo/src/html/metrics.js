...
dojo.html.measureFragment = function (
  /* HTMLElement */ node,
  /* string */ html,
  /* string? */ boxType
) {
...
dojo.html.getFittedFragment = function (
  /* HTMLElement */ node,
  /* string */ html
) {
  //	summary
  ...
  function cl(node) {
    ...
    for (var j = 0; j < node.attributes.length; j++) {
      if (node.attributes[j].specified) {
        if (
          node.attributes[j].nodeName.toLowerCase() != "style" &&
          ...
          element.setAttribute(
            node.attributes[j].nodeName.toLowerCase(),
...
dojo.html.fitToElement = function (/* HTMLElement */ node, /* string */ html) {
  ...
  if (str.substr(0, i) != str) {
    ...
    if (lastLess <= lastGreater && lastNewLine == i - 1) i = i;
      ...
      lastSpace != -1 &&
      lastSpace > lastGreater &&
      lastGreater > lastLess
    )
      i = lastSpace + 1;
