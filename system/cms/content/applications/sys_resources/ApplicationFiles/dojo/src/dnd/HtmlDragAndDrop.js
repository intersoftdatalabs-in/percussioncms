      ...
      if (isTr || ltn == "tbody") {
        ...
        for (var i = 0; i < domTds.length; i++) {
          if (cloneTds[i] && cloneTds[i].style) {
            cloneTds[i].style.width =
              dojo.html.getContentBox(domTds[i]).width + "px";
          }
        }
        node = table;
      }
    ...
    placeIndicator: function (e, dragObjects, boxIndex, before) {
      ...
      if (boxIndex < 0) {
        if (this.childBoxes.length) {
          ...
          this.dropIndicator.style[targetProperty] =
      ...
      if (child) {
        this.dropIndicator.style[targetProperty] =
          (before
            ? child[targetProperty]
            : child[this.vertical ? "right" : "bottom"]) + "px";
        if (this.vertical) {
