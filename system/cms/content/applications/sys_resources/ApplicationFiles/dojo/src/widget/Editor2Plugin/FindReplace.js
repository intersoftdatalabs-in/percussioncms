...
dojo.declare(
  "dojo.widget.Editor2Plugin.FindCommand",
  dojo.widget.Editor2DialogCommand,
  {
    ...
    find: function (text, option) {
      ...
      if (window.find) {
      ...
      } else if (dojo.body().createTextRange) {
        //IE
        var range = this._editor.document.body.createTextRange();
        var found = range.findText(
          text,
          option & this.SearchOption.SearchBackwards ? 1 : -1,
          option
        );
        if (found) {
        ...
        alert(
          "No idea how to search in this browser. Please submit patch if you know."
        );
      }
    },
    getText: function () {
  ...
  getCommand: function (editor, name) {
    ...
    if (name == "find") {
      ...
      command = new dojo.widget.Editor2DialogCommand(editor, "replace", {
        ...
        href: dojo.uri.cache.allow(
          dojo.uri.moduleUri(
            "dojo.widget",
            "templates/Editor2/Dialog/replace.html"
          )
        ),
        title: "Replace",
        width: "350px",
...
dojo.widget.Editor2Manager.registerHandler(
  dojo.widget.Editor2Plugin.FindReplace.getCommand
);
dojo.widget.Editor2ToolbarItemManager.registerHandler(
  dojo.widget.Editor2Plugin.FindReplace.getToolbarItem
);
