...
dojo.widget.defineWidget(
  ...
  },
  {
    ...
    _focusNextVisible: function (nodeWidget) {
      ...
      if (
        nodeWidget.isFolder &&
        nodeWidget.isExpanded &&
        nodeWidget.children.length > 0
      ) {
        ...
        if (nodeWidget.isTreeNode) {
          var returnWidget =
            nodeWidget.parent.children[nodeWidget.getParentIndex() + 1];
        }
      }
      ...
      try {
        ...
        if (nodeWidget && nodeWidget.isTreeNode) {
          if (treeWidget.lastFocused && treeWidget.lastFocused.isTreeNode) {
            // onClick could have chosen a non-root node
            nodeWidget = treeWidget.lastFocused;
          }
    ...
    expandToLevel: function (nodeOrTree, level) {
      ...
      var iterator = new dojo.widget.TreeTimeoutIterator(
        nodeOrTree,
        callFunc,
        this
      );
    ...
    doEditLabelFinish: function (save, server_data) {
      ...
      if (save) {
        ...
        if (node.isPhantom) {
          ...
          var title =
            server_data && server_data.title ? server_data.title : editorTitle;
...
dojo.lang.extend(dojo.widget.TreeBasicControllerV3, {
  ...
  canMove: function (child, newParent) {
    ...
    if (
      ...
      newParent.actionIsDisabledNow(newParent.actions.ADDCHILD)
    ) {
      return false;
    }
