...
dojo.validate.check = function (/*HTMLFormElement*/ form, /*Object*/ profile) {
    ...
    for (var i = 0; i < profile.required.length; i++) {
      ...
      // case 2: elem is a select box
        ...
        for (var j = 0; j < elem.options.length; j++) {
          if (
            elem.options[j].selected &&
            !/^\s*$/.test(elem.options[j].value)
          ) {
            selected++;
          }
        }
  ...
  if (
    dojo.lang.isObject(profile.dependencies) ||
    dojo.lang.isObject(profile.dependancies)
  ) {
    if (profile["dependancies"]) {
      dojo.deprecated(
        "dojo.validate.check",
        "profile 'dependancies' is deprecated, please use " + "'dependencies'",
        "0.5"
      );
      ...
      if (dojo.lang.isFunction(profile.constraints[name])) {
        ...
        if (dojo.lang.isArray(profile.constraints[name][0])) {
          for (var i = 0; i < profile.constraints[name].length; i++) {
            isValid = dojo.validate.evaluateConstraint(
              profile,
              profile.constraints[name][i],
              name,
              elem
            );
            if (!isValid) {
...
dojo.validate.evaluateConstraint = function (
  profile,
  /*Array*/ constraint,
  fieldName,
  elem
