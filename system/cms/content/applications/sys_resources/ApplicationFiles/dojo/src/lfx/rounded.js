...
dojo.lfx.rounded.applyCorners = function (
  /* object */ options,
  /* HTMLElement */ node
) {
  //	summary
  ...
  var borderWidth = parseInt(
    dojo.html.getComputedStyle(node, "border-top-width")
  );
  ...
  for (var i = 0; i < corners.length; i++) {
    ...
    if (options.radii[cc] == 0) {
      ...
      if ((cc.charAt(0) == "t" && top) || (cc.charAt(0) == "b" && bottom)) {
        ...
        switch (cc) {
          case "tl": {
            ...
            corner.style.marginRight =
              options.radii[cc] - format.borderWidth * 2 + "px";
          ...
          case "tr": {
            ...
            corner.style.borderTop = format.borderString;
            corner.style.backgroundPosition =
              "-" + (topHeight - format.borderWidth) + "px 0px";
            corner.style.left = format.borderWidth + "px";
            break;
          }
          case "bl": {
            ...
            corner.style.backgroundPosition =
              ...
              format.borderWidth +
              "px -" +
              (format.height + (bottomHeight + format.borderWidth)) +
              "px";
            break;
          }
          case "br": {
            ...
            corner.style.backgroundPosition =
              "-" +
              (bottomHeight + format.borderWidth) +
              "px -" +
              (format.height + (bottomHeight + format.borderWidth)) +
              "px";
            break;
      ...
      for (var x = 0, j = options.radii[cc]; x < j; x++) {
        ...
        var y1 =
          Math.floor(
            Math.sqrt(Math.pow(borderRadius, 2) - Math.pow(x + 1, 2))
          ) - 1;
        if (x + 1 >= borderRadius) {
          var y1 = -1;
        }
        var y2 = Math.ceil(
          Math.sqrt(Math.pow(borderRadius, 2) - Math.pow(x, 2))
        );
        if (x >= borderRadius) {
          y2 = -1;
        }
        ...
        if (options.antiAlias) {
          ...
          for (var y = y3 + 1; y < y4; y++) {
            fns.draw(
              ...
              y,
              format.borderColor,
              fns.fraction(x, y, j) * 100,
              1,
              corner,
              format.borderWidth > 0 ? 0 : -1,
              options.radii[cc],
              topHeight,
              format
            );
        ...
        for (var t = 0, k = corner.childNodes.length; t < k; t++) {
          ...
          if (cc == "tr") {
            ...
            bar.style.backgroundPosition =
              "-" +
              Math.abs(
                format.width - options.radii[cc] + format.borderWidth + barLeft
              ) +
              "px -" +
              Math.abs(
                options.radii[cc] - barHeight - barTop - format.borderWidth
              ) +
              "px";
          } else if (cc == "tl") {
            bar.style.top = options.radii[cc] - barHeight - barTop + "px";
            bar.style.backgroundPosition =
              "-" +
              Math.abs(options.radii[cc] - barLeft - 1 - format.borderWidth) +
              "px -" +
              Math.abs(
                options.radii[cc] - barHeight - barTop - format.borderWidth
              ) +
            ...
            bar.style.backgroundPosition =
              "-" +
              Math.abs(options.radii[cc] + barLeft + format.borderWidth) +
              "px -" +
              Math.abs(
                format.height + options.radii[cc] + barTop - format.borderWidth
              ) +
              "px";
          }
        }
  ...
  for (var z in diff) {
    var smaller =
      options.radii[z + "l"] < options.radii[z + "r"] ? z + "l" : z + "r";
    ...
    if (z == "t") {
      if (top) {
        if (options.radii.tl && options.radii.tr) {
          ...
          fillBar.style.marginLeft =
            options.radii.tl - format.borderWidth + "px";
          fillBar.style.marginRight =
            options.radii.tr - format.borderWidth + "px";
      ...
      if (bottom) {
        if (options.radii.bl && options.radii.br) {
          ...
          fillBar.style.marginLeft =
            options.radii.bl - format.borderWidth + "px";
          fillBar.style.marginRight =
            options.radii.br - format.borderWidth + "px";
          ...
          if (format.bgImage != "") {
            fillBar.style.backgroundPosition =
              ...
              (bottomHeight + format.borderWidth) +
              "px -" +
              (format.height + (topHeight + format.borderWidth)) +
              "px";
          }
        }
  ...
  if (options.autoPad && format.padding > 0) {
    ...
    if (bottomHeight < format.padding) {
      content.style.paddingBottom =
        Math.abs(bottomHeight - format.padding) + "px";
    }
...
dojo.lfx.rounded._fns = {
  ...
  fraction: function (x, y, r) {
    ...
    switch (whatsides) {
      case "LeftRight":
        return (
          Math.min(yval[0], yval[1]) +
          (Math.max(yval[0], yval[1]) - Math.min(yval[0], yval[1])) / 2
        );
      case "TopRight":
        return 1 - ((1 - xval[0]) * (1 - yval[1])) / 2;
      case "TopBottom":
        return (
          Math.min(xval[0], xval[1]) +
          (Math.max(xval[0], xval[1]) - Math.min(xval[0], xval[1])) / 2
        );
      case "LeftBottom":
  ...
  draw: function (
    ...
    if (image == -1 && format.bgImage != "") {
      ...
      px.style.backgroundPosition =
        "-" +
        (format.width - (radius - x) + format.borderWidth) +
        "px -" +
        (format.height + top + y - format.borderWidth) +
        "px";
    } else {
