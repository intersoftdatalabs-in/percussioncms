...
dojo.widget.defineWidget(
  ...
  },
  {
    ...
    postCreate: function () {
      ...
      for (var i = 0, child; (child = this.children[i]); i++) {
        if (child.widgetType == "ShowSlide") {
          ...
          this.option.parentNode.insertBefore(
            this.option.cloneNode(true),
            this.option
          );
        }
      }
    ...
    stopEvent: function (/*Event*/ ev) {
      ...
      if (
        ev.type == "click" &&
        (this._slides[this._slide].noClick || this.noClick)
      ) {
        return false;
      }
      ...
      if (!dojo.dom.isDescendantOf(target, this.nav)) {
        while (target && target != this.domNode) {
          ...
          if (
            ...
            typeof target.onkeypress == "function"
          ) {
            return false;
          }
          target = target.parentNode;
