...
dojo.widget.defineWidget("dojo.widget.DatePicker", dojo.widget.HtmlWidget, {
  ...
  fillInTemplate: function (args, frag) {
    ...
    var dayLabels = dojo.lang.unnest(
  ...
  _preInitUI: function (dateObj, initFirst, initUI) {
    ...
    if (dateObj < this.startDate || dateObj > this.endDate) {
      dateObj = new Date(
        dateObj < this.startDate ? this.startDate : this.endDate
      );
    }
    ...
    if (
      ...
      if (
        dojo.date.diff(nextDate, this.endDate, dojo.date.dateParts.DAY) > days
      ) {
      ...
      this.curMonth.setDate(1);
      var curClass =
        nextDate.getMonth() == this.curMonth.getMonth()
          ? "current"
          : "previous";
    }
    if (initUI) {
  ...
  _initUI: function (days) {
    ...
    var calendarNodes =
      this.calendarDatesContainerNode.getElementsByTagName("td");
    var calendarRows =
      this.calendarDatesContainerNode.getElementsByTagName("tr");
    ...
    for (i = 0; i < days; i++) {
      ...
      var curClass =
        nextDate.getMonth() != this.curMonth.getMonth() &&
        Number(nextDate) < Number(this.curMonth)
          ? "previous"
          : nextDate.getMonth() == this.curMonth.getMonth()
          ? "current"
          : "next";
      ...
      dojo.html.setClass(
        currentCalendarNode,
        this._getDateClassName(nextDate, mappedClass)
      );
      if (
        dojo.html.hasClass(currentCalendarNode, this.classNames.selectedDate)
      ) {
  ...
  _handleUiClick: function (/*Event*/ evt) {
    ...
    if (
      dojo.html.hasClass(eventTarget, this.classNames["disabledPrevious"]) ||
      dojo.html.hasClass(eventTarget, this.classNames["disabledCurrent"]) ||
      dojo.html.hasClass(eventTarget, this.classNames["disabledNext"])
    ) {
      return; //this date is disabled... ignore it
    }
    this.clickedNode = eventTarget;
    this.setDate(
      new Date(Number(dojo.html.getAttribute(eventTarget, "djDateValue")))
    );
  },
