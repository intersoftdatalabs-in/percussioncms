...
dojo.lang.extend(dojo.gfx.Shape, {
      ...
      switch (fill.type) {
        case "linear":
          ...
          for (var i = 0; i < f.colors.length; ++i) {
            ...
            s +=
              f.colors[i].offset.toFixed(8) +
              " " +
              f.colors[i].color.toHex() +
              ";";
          }
          ...
          fo.type = "gradient";
          fo.angle =
            (dojo.math.radToDeg(Math.atan2(f.x2 - f.x1, f.y2 - f.y1)) + 180) %
            360;
          fo.on = true;
          break;
        case "radial":
          ...
          for (; i >= 0; --i) {
            f.colors[i].color = dojo.gfx.normalizeColor(f.colors[i].color);
            s +=
              (1 - c * f.colors[i].offset).toFixed(8) +
              " " +
              f.colors[i].color.toHex() +
              ";";
          }
          ...
          if (isNaN(w) || isNaN(h)) {
            ...
            fo.focusposition =
              (f.cx / w).toFixed(8) + " " + (f.cy / h).toFixed(8);
          }
          fo.focussize = "0 0";
          fo.on = true;
          break;
  ...
  _applyTransform: function () {
    ...
    if (skew) {
      ...
      var mt =
        ...
        " 0 0";
      var offset =
        Math.floor(matrix.dx).toFixed() +
        "px " +
        Math.floor(matrix.dy).toFixed() +
        "px";
  ...
  attachFill: function (rawNode) {
    ...
    if (rawNode) {
      if (fo.on && fo.type == "gradient") {
        var fillStyle = dojo.lang.shallowCopy(
          dojo.gfx.defaultLinearGradient,
          true
        );
  ...
  attachStroke: function (rawNode) {
    ...
    if (rawNode && rawNode.stroked) {
      ...
      dojo.debug(
        "We are expecting an .75pt here, instead of strokeweight = " +
          rawNode.strokeweight
      );
      strokeStyle.width = dojo.gfx.vml.normalizedLength(
        rawNode.strokeweight + ""
      );
      strokeStyle.color.a = rawNode.stroke.opacity;
      strokeStyle.cap = this._translate(
        this._capMapReversed,
        rawNode.stroke.endcap
      );
      strokeStyle.join =
        rawNode.stroke.joinstyle == "miter"
          ? rawNode.stroke.miterlimit
          : rawNode.stroke.joinstyle;
    } else {
      return null;
    }
...
dojo.declare("dojo.gfx.Rect", dojo.gfx.shape.Rect, {
  ...
  attachShape: function (rawNode) {
    ...
    arcsize =
      arcsize.indexOf("%") >= 0
        ? parseFloat(arcsize) / 100
        : dojo.gfx.vml._parseFloat(arcsize);
  ...
  setShape: function (newShape) {
    ...
    style.width =
      typeof shape.width == "string" && shape.width.indexOf("%") >= 0
        ? shape.width
        : shape.width.toFixed();
    style.height =
      typeof shape.width == "string" && shape.height.indexOf("%") >= 0
        ? shape.height
        : shape.height.toFixed();
    var r = Math.min(
      1,
      shape.r / Math.min(parseFloat(shape.width), parseFloat(shape.height))
    ).toFixed(8);
    // a workaround for the VML's arcsize bug: cannot read arcsize of an instantiated node
...
dojo.declare(
  ...
  },
  {
    ...
    _calcArcTo: function (path, last, rx, ry, xRotg, large, cw, x, y) {
      ...
      var rx2 = rx * rx;
      var ry2 = ry * ry;
      var pa = m.multiplyPoint(m.rotate(-xRot), {
        x: (last.x - x) / 2,
        y: (last.y - y) / 2,
      });
      var pax2 = pa.x * pa.x;
