...
dojo.html.overElement = function (/* HTMLElement */ element, /* DOMEvent */ e) {
  ...
  var absolute = dojo.html.getAbsolutePosition(
    element,
    true,
...
dojo.html.placeOnScreen = function (
  ...
  for (var cidex = 0; cidex < corners.length; ++cidex) {
    ...
    var tryX =
      desiredX -
      (corner.charAt(1) == "L" ? 0 : w) +
      padding[0] * (corner.charAt(1) == "L" ? 1 : -1);
    var tryY =
      desiredY -
      (corner.charAt(0) == "T" ? 0 : h) +
      padding[1] * (corner.charAt(0) == "T" ? 1 : -1);
    if (hasScroll) {
      tryX -= scroll.x;
      tryY -= scroll.y;
    }
    ...
    if (match) {
      ...
      var dist =
        Math.pow(x - tryX - scroll.x, 2) + Math.pow(y - tryY - scroll.y, 2);
...
dojo.html.placeOnScreenAroundElement = function (
  ...
  var aroundNodePos = dojo.html.getAbsolutePosition(
    aroundNode,
    true,
    aroundType
  );
  ...
  for (var nodeCorner in aroundCorners) {
    ...
    desiredX =
      aroundNodePos.x + (nodeCorner.charAt(1) == "L" ? 0 : aroundNodeW);
    desiredY =
      aroundNodePos.y + (nodeCorner.charAt(0) == "T" ? 0 : aroundNodeH);
