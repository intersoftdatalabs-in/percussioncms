...
dojo.deprecated(
  "dojo.widget.Editor",
  "is replaced by dojo.widget.Editor2",
  "0.5"
);
...
dojo.lang.extend(dojo.widget.Editor, {
  ...
  buildRendering: function (args, frag) {
    ...
    var trt = dojo.widget.createWidget(
      this._richTextType,
      {
  ...
  setRichText: function (richText) {
    ...
    if (this._richText && !this._richText.isClosed) {
      dojo.debug(
        "You are switching richTexts yet you haven't closed the current one. Losing reference!"
      );
    }
  ...
  initToolbar: function () {
    ...
    this._toolbarContainer = dojo.widget.createWidget(
      this._toolbarContainerType
    );
  ...
  addItem: function (item, tb, dontValidate) {
    ...
    var cmd =
      item && !dojo.lang.isUndefined(item["getValue"])
        ? (cmd = item["getValue"]())
        : item;
      ...
      if (dontValidate || this.isSupportedCommand(cmd)) {
        ...
        if (cmd == "formatblock") {
          ...
          dojo.event.connect(
            select,
            "onSetValue",
            dojo.lang.hitch(this, function (item, value) {
          ...
          var btn = tb.addChild(
            dojo.widget.createWidget(
              "ToolbarColorDialog",
              this.getItemProperties(cmd)
            )
          );
          dojo.event.connect(btn, "onSetValue", this, "_setValue");
        } else {
          var btn = tb.addChild(
            this.getCommandImage(cmd),
            null,
            this.getItemProperties(cmd)
          );
          if (cmd == "save") {
  ...
  getCommandImage: function (cmd) {
    if (cmd == "|") {
      ...
      return dojo.uri.moduleUri(
        "dojo.widget",
        "templates/buttons/" + cmd + ".gif"
      );
    }
  },
  ...
  onAction: function (cmd, value) {
    switch (cmd) {
      ...
      case "insertimage":
        if (
          !(value = prompt("Please enter the URL of the image:", "http://"))
        ) {
          return;
        }
        break;
  ...
  getHtml: function () {
    this._richText.contentFilters = this._richText.contentFilters.concat(
      this.contentFilters
    );
