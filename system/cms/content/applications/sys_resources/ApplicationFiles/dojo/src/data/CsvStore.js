...
dojo.declare("dojo.data.CsvStore", dojo.data.core.RemoteStore, {
   ...
   */
  _setupQueryRequest: function (
    /* dojo.data.core.Result */ result,
    /* object */ requestKw
  ) {
  ...
  _resultToQueryData: function (/* varies */ serverResponseData) {
    ...
    var arrayOfArrays =
      this._getArrayOfArraysFromCsvFileContents(csvFileContentString);
    var arrayOfObjects =
      this._getArrayOfObjectsFromArrayOfArrays(arrayOfArrays);
    var remoteStoreData =
      this._getRemoteStoreDataFromArrayOfObjects(arrayOfObjects);
    return remoteStoreData;
  },

  _setupSaveRequest: function (
    /* object */ saveKeywordArgs,
    /* object */ requestKw
  ) {
  ...
  // -------------------------------------------------------------------
  // Private methods
  _getArrayOfArraysFromCsvFileContents: function (
    ...
    for (var i in arrayOfInputLines) {
      ...
      if (singleLine.length > 0) {
        ...
        while (j < listOfFields.length) {
          ...
          var field_space = space_field_space.replace(
  ...
  _getRemoteStoreDataFromArrayOfObjects: function (
