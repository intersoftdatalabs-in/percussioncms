...
if (dojo.render.svg.capable) {
  ...
  dojo.mixin(dojo.charting.Plotters, {
    ...
    StackedArea: function (
      ...
      for (var i = n - 1; i >= 0; i--) {
        ...
        if (i == 0) {
          cmd.push("L");
          cmd.push(
            x + "," + plot.axisY.getCoord(plot.axisX.origin, plotarea, plot)
          );
          cmd.push("L");
          cmd.push(
            plot.axisX.getCoord(data[0][0].x, plotarea, plot) +
              "," +
              plot.axisY.getCoord(plot.axisX.origin, plotarea, plot)
          );
          cmd.push("Z");
        } else {
          var values = data[i - 1];
    ...
    Area: function (
      ...
      cmd.push("L");
      cmd.push(
        plot.axisX.getCoord(data[0].x, plotarea, plot) +
          "," +
          plot.axisY.getCoord(plot.axisX.origin, plotarea, plot)
      );
      cmd.push("Z");
    ...
    HighLowClose: function (
      ...
      for (var i = 0; i < n; i++) {
        ...
        line.setAttribute(
          ...
          "stroke:" +
            data[i].series.color +
            ";stroke-width:1px;stroke-opacity:0.6;"
        );
        g.appendChild(line);
