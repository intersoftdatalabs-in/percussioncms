...
dojo.widget.defineWidget(
  ...
  function () {
    ...
    this.image = dojo.uri.moduleUri(
      "dojo.widget",
      "templates/images/clock.png"
    );
    this.surface = null;
    this.labelNode = null;
    ...
    this.draw = function () {
      ...
      self.topLabelNode.innerHTML =
        self.date.getHours() + self.timeZoneOffset > 11 ? "PM" : "AM";
    };
  ...
  },
  {
    ...
    _placeHand: function (shape, angle, shift) {
      var move = {
        dx: this.center.x + (shift ? shift.dx : 0),
        dy: this.center.y + (shift ? shift.dy : 0),
      };
    ...
    placeMinute: function (/* Number */ m, /* Number */ s) {
      ...
      this._placeHand(
        this.shadows.minute.shadow,
        angle,
        this.shadows.minute.shift
      );
    },
    placeSecond: function (/* Number */ s) {
      ...
      this._placeHand(
        this.shadows.second.shadow,
        angle,
        this.shadows.second.shift
      );
    },
    ...
    init: function () {
      ...
      this.shadows.hour.shadow = this._initPoly(this.surface, hP).setFill([
        0, 0, 0, 0.1,
      ]);
      this.hands.hour = this._initPoly(this.surface, hP)
        ...
        .setFill({
          ...
          colors: [
            { offset: 0, color: "#fff" },
            { offset: 0.33, color: this.handColor },
          ],
        });
      this.shadows.minute.shadow = this._initPoly(this.surface, mP).setFill([
        0, 0, 0, 0.1,
      ]);
      ...
      this.shadows.second.shadow = this._initPoly(this.surface, sP).setFill([
        0, 0, 0, 0.1,
      ]);
      this.hands.second = this._initPoly(this.surface, sP).setFill(
      ...
      this.topLabelNode.innerHTML =
        this.date.getHours() + this.timeZoneOffset > 11 ? "PM" : "AM";
