...
dojo.crypto.MD5 = new (function () {
  ...
  function toHex(wa) {
    ...
    for (var i = 0; i < wa.length * 4; i++) {
      s.push(
        h.charAt((wa[i >> 2] >> ((i % 4) * 8 + 4)) & 0xf) +
          h.charAt((wa[i >> 2] >> ((i % 4) * 8)) & 0xf)
      );
    }
    return s.join("");
  }
  function toBase64(wa) {
    ...
    for (var i = 0; i < wa.length * 4; i += 3) {
      var t =
        (((wa[i >> 2] >> (8 * (i % 4))) & 0xff) << 16) |
        (((wa[(i + 1) >> 2] >> (8 * ((i + 1) % 4))) & 0xff) << 8) |
        ((wa[(i + 2) >> 2] >> (8 * ((i + 2) % 4))) & 0xff);
      for (var j = 0; j < 4; j++) {
  ...
  this.compute = function (
    /* string */ data,
    /* dojo.crypto.outputTypes */ outputType
  ) {
    //	summary
