    ...
    for (var i = 0, len = this.domNode.childNodes.length; i < len; i++) {
      ...
      for (var k = 0; k < elems.length; k++) {
        ...
        for (var j = 0, len2 = list.length; j < len2; j++) {
          ...
          var newName = dojo.string.substituteParams(this.pattern, {
            index: "" + i,
          });
    ...
    for (var k = 0; k < elems.length; k++) {
      ...
      for (var i = 0, len = list.length; i < len; i++) {
        ...
        if (child.getAttribute("rowFunction") != null) {
          if (
            typeof this.myObject[child.getAttribute("rowFunction")] ==
            "undefined"
          ) {
            dojo.debug(
              "Function " + child.getAttribute("rowFunction") + " not found"
            );
          } else {
          ...
          if (child.getAttribute("rowAction") == "delete") {
            child.name = dojo.string.substituteParams(this.pattern, {
