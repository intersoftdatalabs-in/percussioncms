...
dojo.widget.html.loader = new (function () {
  ...
  this._log = function (widget, data) {
    ...
    if (widget.trackHistory) {
      ...
      dojo.undo.browser.addToHistory(
        new trackerObj(widget.widgetId, dojo.lang.shallowCopy(data, true))
      );
    }
  };
  ...
  this.htmlContentBasicFix = function (
    /*string*/ s,
    /*string||dojo.uri.Uri*/ url
  ) {
    // summary:
  ...
  this.htmlContentAdjustPaths = function (
    /*string*/ s,
    /*string||dojo.uri.Uri*/ url
  ) {
    // summary:
  ...
  this.htmlContentScripts = function (
    /*string*/ s,
    /*boolean*/ collectScripts
  ) {
    // summary:
    ...
    var regexRequires =
      /dojo\.(?:(?:require(?:After)?(?:If)?)|(?:widget\.(?:manager\.)?registerWidgetPackage)|(?:(?:hostenv\.)?setModulePrefix)|defineNamespace)\((['"]).*?\1\)\s*;?/;
    ...
    while ((match = regex.exec(s))) {
      ...
      if (match[2]) {
        ...
        while ((tmp = regexRequires.exec(sc))) {
          requires.push(tmp[0]);
          sc =
            sc.substring(0, tmp.index) + sc.substr(tmp.index + tmp[0].length);
        }
        if (collectScripts) {
          scripts.push(sc);
        }
  ...
  this.splitAndFixPaths = function (/*object*/ args) {
    ...
    if (args.content) {
      // make sure we don't run regexes on empty content
      var tmp = null,
      ...
      if (args.collectRequires || args.collectScripts) {
        tmp = _loader.htmlContentScripts.call(
          this,
          content,
          args.collectScripts
        );
  ...
  this.hookUp = function (/*object*/ args) {
    ...
    if (dojo.lang.isString(widget)) {
      if (args.mixin) {
        dojo.raise(
          this.toString() + ", cant use mixin when widget is a string"
        );
      }
    ...
    if (!widget || !(widget instanceof dojo.widget.HtmlWidget)) {
      dojo.raise(
        this.toString() + " Widget isn't defined or isn't a HtmlWidget instance"
      );
    }
    ...
    dojo.mixin(widgetProto, {
      ...
      adjustPaths: undef(widgetProto.adjustPaths)
        ? true
        : widgetProto.adjustPaths,
      // only usable on construction, use setUrl or setContent after that
      href: undef(widgetProto.href) ? "" : widgetProto.href,
      // extract visible content from inside of <body> .... </body>
      extractContent: undef(widgetProto.extractContent)
        ? true
        : widgetProto.extractContent,
      // construct all widgets that is in content
      // FIXME: rename to parseWidgets?
      parseContent: undef(widgetProto.parseContent)
        ? true
        : widgetProto.parseContent,
      // use io binds javascript cache, or if false, prevent browsercache
      cacheContent: undef(widgetProto.cacheContent)
        ? true
        : widgetProto.cacheContent,
      ...
      refreshOnShow: undef(widgetProto.refreshOnShow)
        ? false
        : widgetProto.refreshOnShow,
    ...
    widgetProto.postCreate = (function (postCreate) {
      return function () {
        ...
        if (this.isShowing() || this.preload) {
          this.loadContents();
          if (!this.href) {
            // back/forward save initial state
    ...
    widgetProto.destroy = (function (destroy) {
      return function (destroy) {
        ...
        this.loader.unHook();
        if (
          widgetProto.constructor.superclass.destroy != destroy &&
          isFunc(destroy)
        ) {
      ...
      widgetProto.onSetContent = function (cont) {
        ...
        try {
          if (typeof cont != "string") {
            ...
            try {
              // hack to deal with domfaults, ie. appending div to tablenodes
              node.innerHTML = cont;
            } catch (e) {
              var tmp;
      ...
      widgetProto.setContent = function (data, dontLog) {
        ...
        if (!data || dojo.html.isNode(data)) {
          ...
          for (var i = 0, styles = data.styles; i < styles.length; i++) {
            if (styles[i].path) {
              this.loader.styleNodes.push(
                dojo.html.insertCssFile(styles[i].path)
              );
            } else {
          ...
          if (this.parseContent) {
            for (
              var i = 0, requires = data.requires;
              i < requires.length;
              i++
            ) {
              try {
                ...
                e._text =
                  "dojo.widget.html.loader.hookUp: error in package loading calls, " +
                  (e.description || e);
    ...
    if (!widgetProto.setHandler) {
      widgetProto.setHandler = function (handler) {
        ...
        if (!isFunc(fcn)) {
          ...
          handleDefaults.call(
            this,
            "Unable to set handler, '" + handler + "' not a function.",
            "onExecError",
            true
          );
          return;
