  ...
  dojo.widget.TreeBasicController,
  {
    ...
    loadProcessResponse: function (node, result, callObj, callFunc) {
      ...
      for (var i = 0; i < newChildren.length; i++) {
        ...
        newChildren[i] = dojo.widget.createWidget(
          node.widgetType,
          newChildren[i]
        );
      ...
      if (dojo.lang.isFunction(callFunc)) {
        callFunc.apply(dojo.lang.isUndefined(callObj) ? this : callObj, [
          node,
          newChildren,
        ]);
      }
    ...
    doMove: function (child, newParent, index) {
      ...
      if (
        newParent.isTreeNode &&
        newParent.state == newParent.loadStates.UNCHECKED
      ) {
        this.loadRemote(newParent, true);
      }

      return dojo.widget.TreeBasicController.prototype.doMove.apply(
        this,
        arguments
      );
    },
    ...
    doCreateChild: function (parent, index, data, callObj, callFunc) {
      ...
      return dojo.widget.TreeBasicController.prototype.doCreateChild.apply(
        this,
        arguments
      );
    },
