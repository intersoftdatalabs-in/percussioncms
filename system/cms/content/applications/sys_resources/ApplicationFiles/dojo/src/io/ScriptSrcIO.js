...
dojo.io.ScriptSrcTransport = new (function () {
  ...
  this.watchInFlight = function () {
    ...
    for (var param in this._state) {
      ...
      if (currentState.isDone) {
        ...
        try {
          if (
            currentState.checkString &&
            eval("typeof(" + currentState.checkString + ") != 'undefined'")
          ) {
            ...
            if (
              currentState.startTime + listener.timeoutSeconds * 1000 <
              new Date().getTime()
            ) {
              currentState.isFinishing = true;
          ...
          currentState.isFinishing = true;
          this._finish(currentState, "error", {
  ...
  this.bind = function (/*dojo.io.Request*/ kwArgs) {
    ...
    if (kwArgs.sendTransport || jsonpName) {
      ...
      if (jsonpName) {
        content[jsonpName] =
          "dojo.io.ScriptSrcTransport._state." + id + ".jsonpCall";
      }
    }
    ...
    if (kwArgs.postContent) {
      ...
      query +=
        (query ? "&" : "") +
        dojo.io.argsFromMap(content, kwArgs.encoding, jsonpName);
    }
    ...
    state.constantParams =
      kwArgs["constantParams"] == null ? "" : kwArgs["constantParams"];
  ...
  this._multiAttach = function (state, part) {
    ...
    if (isDone) {
      ...
      if (ampEnd > eqEnd || eqEnd == queryMax - 1) {
        ...
        var queryName = currentQuery.substring(
          ampEnd == -1 ? 0 : ampEnd + 1,
          eqEnd
        );
        state.query =
          queryName + "=" + state.query.substring(queryMax, state.query.length);
      }
    }
    ...
    if (
      ...
      switch (callback) {
        case "load":
          ...
          ]("load", response, event, state.kwArgs);
          state.isDone = true;
          break;
        case "partOk":
        ...
        default:
          state.kwArgs[
            typeof state.kwArgs[callback] == "function" ? callback : "handle"
          ](callback, event, event, state.kwArgs);
          state.isDone = true;
      }
    }
  };
  ...
  if (transport._state[event.id]) {
    ...
    if (state == null) {
      ...
      for (var i = 0; scripts && i < scripts.length; i++) {
        var scriptTag = scripts[i];
        if (
          scriptTag.getAttribute("class") == "ScriptSrcTransport" &&
