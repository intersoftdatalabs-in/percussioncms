...
dojo.io.XhrIframeProxy = {
  ...
  xipClientUrl:
    djConfig["xipClientUrl"] ||
    dojo.uri.moduleUri("dojo.io", "xip_client.html"),
  ...
  send: function (facade) {
    ...
    var frameUrl =
      ...
      stateId +
      "&server=" +
      encodeURIComponent(facade._ifpServerUrl) +
      "&fr=false";
    if (this.needFrameRecursion()) {
      ...
      if ((this.xipClientUrl + "").charAt(0) == "/") {
        ...
        fullClientUrl = fullClientUrl.substring(
          0,
          fullClientUrl.lastIndexOf("/") + 1
        );
      }
      fullClientUrl += this.xipClientUrl;

      var serverUrl =
        facade._ifpServerUrl +
        (facade._ifpServerUrl.indexOf("?") == -1 ? "?" : "&") +
        "dojo.fr=1";
      ...
      frameUrl =
        ...
        stateId +
        "&client=" +
        encodeURIComponent(fullClientUrl) +
        "&fr=" +
        this.needFrameRecursion(); //fr is for Frame Recursion
    }
  ...
  receive: function (/*String*/ stateId, /*String*/ urlEncodedData) {
    ...
    for (var i = 0; i < nvPairs.length; i++) {
      if (nvPairs[i]) {
        ...
        response[decodeURIComponent(nameValue[0])] = decodeURIComponent(
          nameValue[1]
        );
      }
    }
    ...
    if (response.responseText) {
      ...
      if (
        ...
        facade.responseXML = dojo.dom.createDocumentFromText(
  ...
  createFacade: function () {
    if (arguments && arguments[0] && arguments[0]["iframeProxyUrl"]) {
      ...
      return dojo.io.XhrIframeProxy.oldGetXmlhttpObject.apply(
        dojo.hostenv,
        arguments
      );
    }
