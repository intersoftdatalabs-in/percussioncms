...
dojo.io.createIFrame = function (
  ...
  if (!turi) {
    if (djConfig["useXDomain"] && !djConfig["dojoIframeHistoryUrl"]) {
      dojo.debug(
        "dojo.io.createIFrame: When using cross-domain Dojo builds," +
    ...
    turi =
      (djConfig["dojoIframeHistoryUrl"] ||
        dojo.uri.moduleUri("dojo", "../iframe_history.html")) + "#noInit=true";
  }
  var ifrstr =
    r.ie && dojo.render.os.win
      ? '<iframe name="' +
        fname +
        '" src="' +
        turi +
...
dojo.io.IframeTransport = new (function () {
  ...
  this.setUpIframe = function () {
    ...
    this.iframe = dojo.io.createIFrame(
      this.iframeName,
