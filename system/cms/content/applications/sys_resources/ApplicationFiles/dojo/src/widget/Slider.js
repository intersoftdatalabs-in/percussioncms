...
dojo.widget.defineWidget("dojo.widget.Slider", dojo.widget.HtmlWidget, {
  // summary
  //	Slider Widget.
  //
  ...
  bottomButtonSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider_down_arrow.png"
  ),

  // topButtonSrc: String
  //	up arrow graphic URL
  topButtonSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider_up_arrow.png"
  ),

  // leftButtonSrc: String
  //	left arrow graphic URL
  leftButtonSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider_left_arrow.png"
  ),

  // rightButtonSrc: String
  //	right arrow graphic URL
  rightButtonSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider_right_arrow.png"
  ),

  // backgroundSrc: String
  //	slider background graphic URL
  backgroundSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/blank.gif"
  ),

  // progressBackgroundSrc: String
  //	slider background graphic URL to overlay the normal background to show progress
  progressBackgroundSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/blank.gif"
  ),
  ...
  handleSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider-button.png"
  ),
  ...
  _buttonPressed: function (/*Event*/ evt, /*DomNode*/ buttonNode) {
    ...
    if (buttonNode == this.leftButtonNode && this.isEnableX) {
      this._snapX(
        dojo.html.getPixelValue(this.sliderHandleNode, "left") -
          this._valueSizeX
      );
      ...
      this._snapY(
        dojo.html.getPixelValue(this.sliderHandleNode, "top") - this._valueSizeY
      );
    ...
    this._typamaticTimer = dojo.lang.setTimeout(
      this,
      "_buttonPressed",
    ...
    this._currentTimeout = Math.round(
      this._currentTimeout * this.timeoutChangeRate
    );
    return false;
  },
  ...
  fillInTemplate: function (/*Object*/ args, /*Object*/ frag) {
    ...
    if (
      dojo.lang.isString(padding) &&
      padding != "" &&
      padding != "0px" &&
      padding != "0px 0px 0px 0px"
    ) {
      this.topBorderNode.style.padding = this.bottomBorderNode.style.padding =
        padding;
    ...
    if (
      dojo.lang.isString(borderWidth) &&
      borderWidth != "" &&
      borderWidth != "0px" &&
      ...
      this.rightBorderNode.style.borderRightWidth =
        this.domNode.style.borderRightWidth;
      this.leftBorderNode.style.borderLeftWidth =
        this.domNode.style.borderLeftWidth;
    ...
    this._handleMove = new dojo.widget._SliderDragMoveSource(
      this.sliderHandleNode
    );
    this._handleMove.setParent(this);

    if (this.clickSelect) {
      dojo.event.connect(
        this.constrainingContainerNode,
        "onmousedown",
        this,
        "_onClick"
      );
    ...
    if (this.focusNode.addEventListener) {
      ...
      this.focusNode.addEventListener(
        "DOMMouseScroll",
        dojo.lang.hitch(this, "_mouseWheeled"),
        false
  ...
  _snapX: function (/*Number*/ x) {
    ...
    this.progressBackgroundNode.style.clip =
      "rect(" +
      this._clipTop +
      "px," +
      this._clipRight +
  ...
  _calc_valueSizeX: function () {
    var constrainingCtrBox = dojo.html.getContentBox(
      this.constrainingContainerNode
    );
    var sliderHandleBox = dojo.html.getContentBox(this.sliderHandleNode);
    if (
      isNaN(constrainingCtrBox.width) ||
      isNaN(sliderHandleBox.width) ||
      constrainingCtrBox.width <= 0 ||
      sliderHandleBox.width <= 0
    ) {
      return false;
    }

    this._constraintWidth =
      constrainingCtrBox.width +
  ...
  setValueX: function (/*Number*/ value) {
    ...
    var pixelPercent =
      (value - this.minimumX) / (this.maximumX - this.minimumX);
    if (this.flipX) {
  ...
  getValueX: function () {
    ...
    return (
      Math.round(pixelPercent * (this.snapValuesX - 1)) *
        ((this.maximumX - this.minimumX) / (this.snapValuesX - 1)) +
      this.minimumX
    );
  },
  ...
  _calc_valueSizeY: function () {
    ...
    var sliderHandleBox = dojo.html.getContentBox(this.sliderHandleNode);
    if (
      isNaN(constrainingCtrBox.height) ||
      isNaN(sliderHandleBox.height) ||
      constrainingCtrBox.height <= 0 ||
      sliderHandleBox.height <= 0
    ) {
      return false;
    }

    this._constraintHeight =
      constrainingCtrBox.height +
  ...
  setValueY: function (/*Number*/ value) {
    ...
    var pixelPercent =
      (value - this.minimumY) / (this.maximumY - this.minimumY);
    if (this.flipY) {
      pixelPercent = 1.0 - pixelPercent;
    }
  ...
  getValueY: function () {
    var pixelPercent =
      dojo.html.getPixelValue(this.sliderHandleNode, "top") /
      this._constraintHeight;
    if (this.flipY) {
      pixelPercent = 1.0 - pixelPercent;
    }
    return (
      Math.round(pixelPercent * (this.snapValuesY - 1)) *
        ((this.maximumY - this.minimumY) / (this.snapValuesY - 1)) +
      this.minimumY
    );
  },
  ...
  _onClick: function (/*Event*/ evt) {
    ...
    var parent = dojo.html.getAbsolutePosition(
      this.constrainingContainerNode,
      true,
...
dojo.widget.defineWidget("dojo.widget.SliderHorizontal", dojo.widget.Slider, {
  // summary
  ...
  backgroundSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider-bg.gif"
  ),
  ...
  postMixInProperties: function () {
    dojo.widget.SliderHorizontal.superclass.postMixInProperties.apply(
      this,
      arguments
    );
...
dojo.widget.defineWidget("dojo.widget.SliderVertical", dojo.widget.Slider, {
  // summary
  ...
  backgroundSrc: dojo.uri.moduleUri(
    "dojo.widget",
    "templates/images/slider-bg-vert.gif"
  ),
  ...
  postMixInProperties: function () {
    dojo.widget.SliderVertical.superclass.postMixInProperties.apply(
      this,
      arguments
    );
    if (!isNaN(parseFloat(this.initialValue))) {
 ...
 */
dojo.declare("dojo.widget._SliderDragMoveSource", dojo.dnd.HtmlDragMoveSource, {
  slider: null,
  ...
  createDragMoveObject: function () {
    ...
    var dragObj = new dojo.widget._SliderDragMoveObject(
      this.dragObject,
      this.type
    );
 ...
 * features for the slider handle.
 */
dojo.declare("dojo.widget._SliderDragMoveObject", dojo.dnd.HtmlDragMoveObject, {
