...
dojo.hostenv.createXdPackage = function (
  /*String*/ contents,
  /*String*/ resourceName,
  /*String=*/ resourcePath
) {
  ...
  var deps = [];
  var depRegExp =
    /dojo.(requireLocalization|require|requireIf|requireAll|provide|requireAfterIf|requireAfter|kwCompoundRequire|conditionalRequire|hostenv\.conditionalLoadModule|.hostenv\.loadModule|hostenv\.moduleLoaded)\(([\w\W]*?)\)/gm;
  var match;
...
dojo.hostenv.loadPath = function (
  ...
  if (colonIndex > 0 && colonIndex < slashIndex) {
    ...
    if (
      ...
      (!location.host || uri.indexOf("http://" + location.host) != 0)
    ) {
      this.isXDomain = currentIsXDomain = true;
    }
  }
  ...
  try {
    return !module || this.isXDomain
      ? this.loadUri(uri, cb, currentIsXDomain, module)
      : this.loadUriAndCheck(uri, module, cb); //boolean
  } catch (e) {
    dojo.debug(e);
    ...
    for (var i = 0; i < deps.length; i++) {
      ...
      if (dep[0] == "provide" || dep[0] == "hostenv.moduleLoaded") {
        ...
        if (unpackedDeps.requiresAfter) {
          requireAfterList = requireAfterList.concat(
            unpackedDeps.requiresAfter
          );
        }
      }
...
dojo.hostenv.xdLoadFlattenedBundle = function (
  /*String*/ moduleName,
  /*String*/ bundleName,
  /*String?*/ locale,
  /*Object*/ bundleData
...
dojo.xdRequireLocalization = function (
  /*String*/ moduleName,
  /*String*/ bundleName,
  /*String?*/ locale,
  ...
  var bundlePackage = dojo.evalObjPath(
    [moduleName, "nls", bundleName].join(".")
  );
  ...
  switch (dep[0]) {
    ...
    case "conditionalRequire":
      ...
      if (
        dep[1] === true ||
        dep[1] == "common" ||
        (dep[1] && dojo.render[dep[1]].capable)
      ) {
    ...
    case "hostenv.conditionalLoadModule":
      ...
      var newDeps = modMap[dojo.hostenv.name_]
        ? common.concat(modMap[dojo.hostenv.name_] || [])
        : common.concat(modMap["default"] || []);
      dojo.hostenv.flattenRequireArray(newDeps);
      break;
    case "require":
    case "requireAfter":
...
dojo.hostenv.callLoaded = function () {
  ...
  if (
    this.xdHasCalledPreload ||
    dojo.hostenv.getModulePrefix("dojo") == "src" ||
    !this.localesGenerated
  ) {
    ...
    if (this.localesGenerated) {
      this.registerNlsPrefix = function () {
        ...
        dojo.registerModulePath(
          "nls",
          dojo.hostenv.getModulePrefix("dojo") + "/../nls"
        );
      };
