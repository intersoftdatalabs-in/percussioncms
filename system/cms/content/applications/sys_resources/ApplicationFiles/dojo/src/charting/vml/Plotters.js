  ...
  dojo.mixin(dojo.charting.Plotters, {
      ...
      for (var i = n - 1; i >= 0; i--) {
        ...
        if (i == 0) {
          ...
          cmd.push(
            x +
              "," +
              Math.round(plot.axisY.getCoord(plot.axisX.origin, plotarea, plot))
          );
          cmd.push("l");
          cmd.push(
            Math.round(plot.axisX.getCoord(data[0][0].x, plotarea, plot)) +
              "," +
              Math.round(plot.axisY.getCoord(plot.axisX.origin, plotarea, plot))
          );
        } else {
          ...
          cmd.push(
            x +
              "," +
              Math.round(
                plot.axisY.getCoord(values[values.length - 1].y, plotarea, plot)
              )
          );
          for (var j = values.length - 2; j >= 0; j--) {
            var x = Math.round(
              plot.axisX.getCoord(values[j].x, plotarea, plot)
            );
            var y = Math.round(
              plot.axisY.getCoord(values[j].y, plotarea, plot)
            );
      ...
      for (var i = n - 1; i >= 0; i--) {
        ...
        for (var j = 0; j < data[i].length; j++) {
          ...
          if (j == 0) {
            ...
            var lastx = Math.round(
              plot.axisX.getCoord(values[j - 1].x, plotarea, plot)
            );
            var lasty = Math.round(
              plot.axisY.getCoord(values[j - 1].y, plotarea, plot)
            );
            var dx = x - lastx;
            var dy = y - lasty;
    ...
    DataBar: function (
      ...
      var w =
        ...
        (plot.axisX.range.upper - plot.axisX.range.lower); //	the width of each group.
      var yOrigin = plot.axisY.getCoord(plot.axisX.origin, plotarea, plot);
    ...
    CurvedLine: function (
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (i == 0) {
          ...
          var lastx = Math.round(
            plot.axisX.getCoord(data[i - 1].x, plotarea, plot)
          );
          var lasty = Math.round(
            plot.axisY.getCoord(data[i - 1].y, plotarea, plot)
          );
          var dx = x - lastx;
          var dy = y - lasty;
    ...
    Area: function (
      ...
      cmd.push("l");
      cmd.push(
        Math.round(plot.axisX.getCoord(data[0].x, plotarea, plot)) +
          "," +
          Math.round(plot.axisY.getCoord(plot.axisX.origin, plotarea, plot))
      );
      path.setAttribute("path", cmd.join(" ") + " x e");
    ...
    HighLow: function (
      ...
      var part =
        (area.right - area.left) /
        (plot.axisX.range.upper - plot.axisX.range.lower) /
        4;
      var w = part * 2;
