...
dojo.data.RdfDatatypeSerializer = function (
  /* JavaScript type */ type,
  /* function */ convertFunc,
  /* RDF datatype URI */ uri
) {
  /* summary:
...
dojo.declare("dojo.data.RdfStore", dojo.data.core.RemoteStore, {
  ...
  _toRDFValue: function (value) {
    ...
    if (value.id) {
      ...
      if (value.lang) rdfvalue["xml:lang"] = value.lang;
    } else {
      if (typeof value == "number") value = new Number(value);
      else if (typeof value == "boolean") value = new Boolean(value);
...
dojo.declare("dojo.data.RhizomeStore", dojo.data.RdfStore, {
  ...
  initializer: function (kwArgs) {
    this._serverQueryUrl =
      kwArgs.baseUrl + "search?view=json&searchType=RxPath&search=";
  ...
  _setupSaveRequest: function (saveKeywordArgs, requestKw) {
    ...
    for (key in this._changed) {
      if (!this._added[key]) {
        //don't put new resources in this list
        resources.push(key);
      }
