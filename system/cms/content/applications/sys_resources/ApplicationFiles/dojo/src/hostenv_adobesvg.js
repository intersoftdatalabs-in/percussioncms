...
dojo.debug = function () {
  ...
  if (typeof dojo.hostenv.println != "function") {
    dojo.raise(
      "attempt to call dojo.debug when there is no dojo.hostenv println implementation (yet?)"
    );
  }
  ...
  if (isJUM) {
    // this seems to be the only way to get JUM to "play nice"
    jum.debug(s);
  } else {
...
dojo.hostenv.getText = function (uri, async_cb, fail_ok) {
  ...
  try {
    if (async_cb) {
      ...
      return dojo.raise(
        "No synchronous XMLHTTP implementation available, for uri " + uri
      );
    }
  } catch (e) {
 ...
 */
dojo.hostenv.postText = function (
  uri,
  async_cb,
  text,
  fail_ok,
  ...
  var async_callback = function (httpResponse) {
    if (!httpResponse.success) {
      dojo.raise(
        "Request for uri '" + uri + "' resulted in " + httpResponse.status
      );
    }
  ...
  try {
    if (async_cb) {
      ...
      return dojo.raise(
        "No synchronous XMLHTTP post implementation available, for uri " + uri
      );
    }
  } catch (e) {
    return dojo.raise(
      "No XMLHTTP post implementation available, for uri " + uri
    );
  }
};
...
function dj_last_script_src() {
  ...
  if (scripts.length < 1) {
    dojo.raise(
      "No script elements in window.document, so can't figure out my script src"
    );
  }
  ...
  if (!src) {
    dojo.raise(
      "Last script element (out of " + scripts.length + ") has no src"
    );
  }
  return src;
}
