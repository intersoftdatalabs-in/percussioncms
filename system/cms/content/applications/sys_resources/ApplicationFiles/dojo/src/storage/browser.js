...
dojo.lang.extend(dojo.storage.browser.FileStorageProvider, {
  ...
  showSettingsUI: function () {
    dojo.raise(
      this.getType() + " does not support a storage settings user-interface"
    );
  },
  ...
  _saveFileXPCOM: function (filename, value) {
    ...
    try {
      ...
      var ouputStream = Components.classes[
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._saveFileXPCOM(): " + msg
      );
    }
  },
  ...
  _loadFileXPCOM: function (filename) {
    ...
    try {
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._loadFileXPCOM(): " + msg
      );
    }
  ...
  _saveFileActiveX: function (filename, value) {
    ...
    try {
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._saveFileActiveX(): " + msg
      );
    }
  },
  ...
  _loadFileActiveX: function (filename) {
    ...
    try {
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._loadFileActiveX(): " + msg
      );
    }
  },
  ...
  _saveFileJava: function (filename, value) {
    ...
    try {
      var applet = dojo.byId(
        dojo.storage.browser.FileStorageProvider._APPLET_ID
      );
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._saveFileJava(): " + msg
      );
    }
  },
  ...
  _loadFileJava: function (filename) {
    ...
    try {
      ...
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider._loadFileJava(): " + msg
      );
    }
  },
  ...
  _getPagePath: function () {
    ...
    if (/^\/?[a-z]+\:/i.test(path)) {
      // Windows disk name, such as a:
      // Mozilla adds a leading slash
      path = path.replace(/^\/?/, "");
      // convert slashes to Windows back slashes
      path = path.replace(/\//g, "\\");
    } else if (/^[\/\\]{2,3}[^\/]/.test(path)) {
      // SMB share, such as \\foobar\someshare\dir
  ...
  _removeXPCOM: function (filename) {
    ...
    try {
      ...
      // dojo.storage.remove()?
      dojo.raise(
        "dojo.storage.browser.FileStorageProvider.remove(): " + e.toString()
      );
    }
  },
  ...
  _writeApplet: function () {
    var archive = dojo.uri
      .moduleUri("dojo", "../DojoFileStorageProvider.jar")
      .toString();
    var tag =
      "<applet " +
      "id='" +
...
dojo.lang.extend(dojo.storage.browser.FlashStorageProvider, {
  ...
  initialize: function () {
    ...
    var swfloc6 = dojo.uri
      .moduleUri("dojo", "../Storage_version6.swf")
      .toString();
    var swfloc8 = dojo.uri
      .moduleUri("dojo", "../Storage_version8.swf")
      .toString();
  ...
  hideSettingsUI: function () {
    ...
    // now hidden
    if (
      dojo.storage.onHideSettingsUI != null &&
...
dojo.storage.manager.register(
  "dojo.storage.browser.FileStorageProvider",
  new dojo.storage.browser.FileStorageProvider()
);
dojo.storage.manager.register(
  "dojo.storage.browser.WhatWGStorageProvider",
