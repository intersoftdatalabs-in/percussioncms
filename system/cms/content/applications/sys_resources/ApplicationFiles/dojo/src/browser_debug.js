...
function removeComments(contents) {
  ...
  return contents.replace(/__DOJONEWLINE/gm, "\n");
}
...
dojo.hostenv.getRequiresAndProvides = function (contents) {
  ...
  RegExp.lastIndex = 0;
  var testExp =
    /dojo.(hostenv.loadModule|hostenv.require|require|requireIf|kwCompoundRequire|hostenv.conditionalLoadModule|hostenv.startPackage|provide)\([\w\W]*?\)/gm;
...
dojo.hostenv.getDelayRequiresAndProvides = function (contents) {
  ...
  var tmp;
  RegExp.lastIndex = 0;
  var testExp = /dojo.(requireAfterIf)\([\w\W]*?\)/gm;
  while ((tmp = testExp.exec(contents)) != null) {
    deps.push(tmp[0]);
  }
  return deps;
...
dojo.hostenv.writeIncludes = function (willCallAgain) {
  ...
  for (var x = dojo.hostenv._loadedUrisListStart; x < depList.length; x++) {
    document.write(
      "<script type='text/javascript' src='" + depList[x] + "'></script>"
    );
  }
  document.write(
