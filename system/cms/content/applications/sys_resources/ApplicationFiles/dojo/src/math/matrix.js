...
dojo.math.matrix.multiply = function (a, b) {
  ...
  if (ax != by) {
    dojo.debug(
      "Can't multiply matricies of sizes " +
        ax +
        "," +
        ay +
        " and " +
...
dojo.math.matrix.sum = function () {
  ...
  for (var i = 1; i < arguments.length; ++i) {
    ...
    if (arg.length != rows || arg[0].length != cols) {
      dojo.debug(
        "can't add matrices of different dimensions: first dimensions were " +
          rows +
          "x" +
          cols +
...
dojo.math.matrix.upperTriangle = function (m) {
  ...
  for (var col = 0; col < tms - 1; col++) {
    if (typeof m[col][col] != "number") {
      dojo.debug(
        "non-numeric entry found in a numeric matrix: m[" +
          col +
          "][" +
          col +
          "]=" +
...
dojo.math.matrix.adjoint = function (a) {
  ...
  if (tms <= 1) {
    dojo.debug(
      "Can't find the adjoint of a matrix with a dimension less than 2"
    );
    return [[0]];
  }
  ...
  for (var i = 0; i < tms; i++) {
    for (var j = 0; j < tms; j++) {
      ia = 0;
      for (ii = 0; ii < tms; ii++) {
        // create a temporary matrix for determinant calc
        if (ii == i) {
