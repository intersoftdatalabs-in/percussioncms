...
dojo.io.repubsub = new (function () {
  ...
  this.tunnelCloseCallback = function () {
    ...
    dojo.io.setIFrameSrc(
      this.rcvNode,
      this.initDoc + "?callback=repubsub.rcvNodeReady&domain=" + document.domain
    );
  };
  ...
  this.receiveEventFromTunnel = function (evt, srcWindow) {
    ...
    for (var i = 0; i < evt.elements.length; i++) {
      ...
      this.log(
        "[event]: " + (ee.name || ee.nameU) + ": " + e[ee.name || ee.nameU]
      );
    }
  ...
  this.openTunnel = function () {
    ...
    dojo.io.setIFrameSrc(
      this.sndNode,
      this.initDoc + "?callback=repubsub.sndNodeReady&domain=" + document.domain
    );
  };
  ...
  this.rcvNodeReady = function () {
    ...
    var initURIArr = [
      this.serverBaseURL,
      "/kn?kn_from=",
      escape(this.tunnelURI),
      "&kn_id=",
      escape(this.tunnelID),
      "&kn_status_from=",
      escape(statusURI),
    ];
  ...
  this.subscribe = function (
    topic /* kn_from in the old terminilogy */,
    toObj,
    toFunc,
    dontTellServer
  ) {
  ...
  this.dequeueEvent = function (force) {
    ...
    if (this.canSnd || force) {
      dojo.io.setIFrameSrc(
        this.sndNode,
        this.backlog.shift() + "&callback=repubsub.sndNodeReady"
      );
      this.canSnd = false;
    } else {
...
dojo.io.repubsubEvent.prototype.properties = [
  ["from", "kn_from"],
  ["to", "kn_to"],
  ["method", "do_method"],
  ["id", "kn_id"],
