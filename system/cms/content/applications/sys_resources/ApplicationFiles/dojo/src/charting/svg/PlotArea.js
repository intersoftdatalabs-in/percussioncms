...
if (dojo.render.svg.capable) {
  ...
  dojo.extend(dojo.charting.PlotArea, {
    resize: function () {
      ...
      this.nodes.plots.setAttribute(
        "style",
        "clip-path:url(#" + this.getId() + "-clip);"
      );
      ...
      for (var p in axes) {
        var obj = axes[p];
        this.nodes.axes.appendChild(
          obj.axis.initialize(this, obj.plot, obj.drawAgainst, obj.plane)
        );
      }
    },
    ...
    initialize: function () {
      ...
      this.nodes.background = document.createElementNS(
        dojo.svg.xmlns.svg,
        "rect"
      );
