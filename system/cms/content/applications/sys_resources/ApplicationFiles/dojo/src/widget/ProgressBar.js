...
dojo.widget.defineWidget("dojo.widget.ProgressBar", dojo.widget.HtmlWidget, {
  // summary:
  ...
  _setupAnimation: function () {
    ...
    this._animation = dojo.lfx.html.slideTo(
      this.internalProgress,
      {
        top: 0,
        left:
          parseInt(this.width) - parseInt(this.internalProgress.style.width),
      },
      parseInt(this.duration),
      null,
      function () {
        var _backAnim = dojo.lfx.html.slideTo(
          _self.internalProgress,
          { top: 0, left: 0 },
  ...
  setProgressValue: function (value, noRender) {
    ...
    if (dojo.string.endsWith(_value, "%", false)) {
      ...
      _value = Math.min(
        _floatValue * this._unitPercentRatio,
        this.maxProgressValue
      );
      _pixelValue = Math.min(
        _floatValue * this._pixelPercentRatio,
      ...
      _pixelValue = Math.min(
        _floatValue / this._unitPixelRatio,
        eval("this." + this._dimension)
      );
    }
  ...
  _showRemoteProgress: function () {
    var _self = this;
    if (
      this.getMaxProgressValue() == this.getProgressValue() &&
      this._oInterval
    ) {
  ...
  _setLabelPosition: function () {
    ...
    var _bottomFront =
      (parseInt(this.height) - parseInt(_heightFront)) / 2 + "px";
    var _leftBack = (parseInt(this.width) - _widthBack) / 2 + "px";
    var _bottomBack =
      (parseInt(this.height) - parseInt(_heightBack)) / 2 + "px";
