...
if (dojo.render.svg.capable) {
  dojo.extend(dojo.charting.Axis, {
    renderLines: function (
      ...
      var g = (this.nodes.lines = document.createElementNS(
        dojo.svg.xmlns.svg,
        "g"
      ));
      ...
      for (var i = 0; i < this._labels.length; i++) {
        ...
        l.setAttribute(
          "style",
          "stroke:#999;stroke-width:1px;stroke-dasharray:1,4;"
        );
        if (plane == "x") {
    ...
    renderTicks: function (
      ...
      var g = (this.nodes.ticks = document.createElementNS(
        dojo.svg.xmlns.svg,
        "g"
      ));
    ...
    renderLabels: function (
      ...
      var g = (this.nodes.labels = document.createElementNS(
        dojo.svg.xmlns.svg,
        "g"
      ));
      ...
      for (var i = 0; i < this._labels.length; i++) {
        ...
        if (plane == "x") {
          g.appendChild(
            createLabel(this._labels[i].label, v, coord, textSize, anchor)
          );
        } else if (plane == "y") {
          g.appendChild(
            createLabel(this._labels[i].label, coord, v, textSize, anchor)
          );
        }
      }
      return g; //	SVGGelement
    ...
    render: function (
      ...
      var line = (this.nodes.axis = document.createElementNS(
        dojo.svg.xmlns.svg,
        "line"
      ));
      if (plane == "x") {
        ...
        if (this.showLabels) {
          g.appendChild(
            this.renderLabels(plotArea, plot, plane, y, textSize, "middle")
          );
        }
        if (this.showLabel && this.label) {
          ...
          text.setAttribute(
            "style",
            "text-anchor:middle;font-family:sans-serif;font-weight:bold;font-size:" +
              (textSize + 2) +
              "px;fill:#000;"
          );
        ...
        if (this.showLabels) {
          g.appendChild(
            this.renderLabels(plotArea, plot, plane, x, textSize, anchor)
          );
        }
        if (this.showLabel && this.label) {
          var x = isMax
            ? coord + textSize * 2 + textSize / 2
            : coord - textSize * 4;
