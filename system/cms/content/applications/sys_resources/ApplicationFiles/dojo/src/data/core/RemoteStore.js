...
dojo.lang.declare(
  "dojo.data.core.RemoteStore",
  [dojo.data.core.Read, dojo.data.core.Write],
  {
    _datatypeMap: {
    ...
    _queryToQueryKey: function (query) {
      ...
      else return dojo.json.serialize(query);
    },

    _assertIsItem: function (/* item */ item) {
      if (!this.isItem(item)) {
        throw new Error(
          "dojo.data.RemoteStore: a function was passed an item argument that was not an item"
        );
      }
    },
    ...
    getAttributes: function (/* item */ item) {
      ...
      var itemIdentity = this.getIdentity(item);
      if (!itemIdentity) return undefined; //todo: raise exception
      ...
      var bindfunc = function (type, data, evt) {
        if (type == "load") {
          ...
          for (key in self._added) {
            if (!self._data[key]) self._data[key] = [{}, 1];
          }
    ...
    forgetResults: function (query) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (existingValue[1] <= 1) {
          ...
          removed++;
        } else existingValue[1] = --existingValue[1];
      }
