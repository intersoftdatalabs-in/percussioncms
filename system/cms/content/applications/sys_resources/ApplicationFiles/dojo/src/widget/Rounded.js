...
dojo.deprecated(
  "dojo.widget.Rounded will be removed in version 0.5; you can now apply rounded corners to any block element using dojo.lfx.rounded.",
  "0.5"
);
...
dojo.widget.defineWidget("dojo.widget.Rounded", dojo.widget.ContentPane, {
  ...
  curvyCorners: function (settings) {
    ...
    this.boxHeight = parseInt(
      boxHeight != "" && boxHeight != "auto" && boxHeight.indexOf("%") == -1
        ? boxHeight.substring(0, boxHeight.indexOf("px"))
        : this.box.scrollHeight
    );
    this.boxWidth = parseInt(
      boxWidth != "" && boxWidth != "auto" && boxWidth.indexOf("%") == -1
        ? boxWidth.substring(0, boxWidth.indexOf("px"))
        : this.box.scrollWidth
    );
    this.borderWidth = parseInt(
      borderWidth != "" && borderWidth.indexOf("px") !== -1
        ? borderWidth.slice(0, borderWidth.indexOf("px"))
        : 0
    );
    ...
    this.boxColour =
      boxColour != "" && boxColour != "transparent"
        ? boxColour.substr(0, 3) == "rgb"
          ? this.rgb2Hex(boxColour)
          : boxColour
        : "#ffffff";
    this.borderColour =
      borderColour != "" &&
      ...
      this.borderWidth > 0
        ? borderColour.substr(0, 3) == "rgb"
          ? this.rgb2Hex(borderColour)
          : borderColour
        : this.boxColour;
    ...
    this.applyCorners = function () {
      ...
      for (var i in corners) {
        ...
        if (!this.settings[cc]) {
          // No
          if (
            ((cc == "tr" || cc == "tl") && this.topContainer != null) ||
          ...
          if (this.masterCorners[this.settings[cc].radius]) {
            ...
            for (var intx = 0, j = this.settings[cc].radius; intx < j; intx++) {
              ...
              if (intx + 1 >= borderRadius) {
                ...
                var y1 =
                  Math.floor(
                    Math.sqrt(Math.pow(borderRadius, 2) - Math.pow(intx + 1, 2))
                  ) - 1;
              }
              ...
              if (borderRadius != j) {
                if (intx >= borderRadius) {
                  ...
                  var y2 = Math.ceil(
                    Math.sqrt(Math.pow(borderRadius, 2) - Math.pow(intx, 2))
                  );
                }
                ...
                if (intx + 1 >= j) {
                  ...
                  var y3 =
                    Math.floor(
                      Math.sqrt(Math.pow(j, 2) - Math.pow(intx + 1, 2))
                    ) - 1;
                }
              }
              ...
              if (intx >= j) {
                ...
                var y4 = Math.ceil(
                  Math.sqrt(Math.pow(j, 2) - Math.pow(intx, 2))
                );
              }
              ...
              if (borderRadius != j) {
                ...
                if (this.antiAlias) {
                  ...
                  for (var inty = y1 + 1; inty < y2; inty++) {
                    ...
                    if (this.backgroundImage != "") {
                      var borderFract =
                        this.pixelFraction(intx, inty, borderRadius) * 100;
                      ...
                      var pixelcolour = dojo.gfx.color.blend(
                        this.boxColour,
                        this.borderColour,
                        this.pixelFraction(intx, inty, borderRadius)
                      );
                      this.drawPixel(
                        intx,
                        inty,
              ...
              if (this.antiAlias) {
                ...
                for (var inty = y3 + 1; inty < y4; inty++) {
                  ...
                  this.drawPixel(
                    ...
                    inty,
                    outsideColour,
                    this.pixelFraction(intx, inty, j) * 100,
                    1,
                    newCorner,
                    this.borderWidth > 0 ? 0 : -1,
                    this.settings[cc].radius
                  );
                }
              }
          ...
          if (cc != "br") {
            ...
            for (var t = 0, k = newCorner.childNodes.length; t < k; t++) {
              ...
              var pixelBarTop = parseInt(
                pixelBar.style.top.substring(
                  0,
                  pixelBar.style.top.indexOf("px")
                )
              );
              var pixelBarLeft = parseInt(
                pixelBar.style.left.substring(
                  0,
                  pixelBar.style.left.indexOf("px")
                )
              );
              var pixelBarHeight = parseInt(
                pixelBar.style.height.substring(
                  0,
      ...
      var radiusDiff = [];
      radiusDiff["t"] =
        this.settings.tl.enabled && this.settings.tr.enabled
          ? Math.abs(this.settings.tl.radius - this.settings.tr.radius)
          : 0;
      radiusDiff["b"] =
        this.settings.bl.enabled && this.settings.br.enabled
          ? Math.abs(this.settings.bl.radius - this.settings.br.radius)
          : 0;

      for (var z in radiusDiff) {
        if (radiusDiff[z]) {
          // Get the type of corner that is the smaller one
          var smallerCornerType =
            this.settings[z + "l"].radius < this.settings[z + "r"].radius
              ? z + "l"
              : z + "r";
    ...
    this.drawPixel = function (
      ...
      if (image == -1 && this.backgroundImage != "") {
        ...
        pixel.style.backgroundPosition =
          "-" +
          (this.boxWidth - (cornerRadius - intx) + this.borderWidth) +
          "px -" +
          (this.boxHeight + cornerRadius + inty - this.borderWidth) +
          "px";
      } else {
  ...
  pixelFraction: function (x, y, r) {
    ...
    switch (whatsides) {
      case "LeftRight":
        pixelfraction =
          Math.min(yvalues[0], yvalues[1]) +
          (Math.max(yvalues[0], yvalues[1]) -
            Math.min(yvalues[0], yvalues[1])) /
            2;
        break;
      ...
      case "TopBottom":
        pixelfraction =
          Math.min(xvalues[0], xvalues[1]) +
          (Math.max(xvalues[0], xvalues[1]) -
            Math.min(xvalues[0], xvalues[1])) /
            2;
        break;
  ...
  rgb2Hex: function (rgbColour) {
    try {
      ...
      var hexColour =
        "#" + this.intToHex(red) + this.intToHex(green) + this.intToHex(blue);
    } catch (e) {
      alert(
        "There was an error converting the RGB value to Hexadecimal in function rgb2Hex"
      );
    }
    return hexColour;
  },
