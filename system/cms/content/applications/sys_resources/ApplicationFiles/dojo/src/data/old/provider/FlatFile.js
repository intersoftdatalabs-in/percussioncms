...
dojo.data.old.provider.FlatFile = function (/* keywords */ keywordParameters) {
  ...
  if (keywordParameters) {
    ...
    if (jsonObjects) {
      dojo.data.old.format.Json.loadDataProviderFromArrayOfJsonData(
        this,
        jsonObjects
      );
    }
    if (jsonString) {
      dojo.data.old.format.Json.loadDataProviderFromFileContents(
        this,
        jsonString
      );
    }
    if (fileUrl) {
      ...
      if (formatParser) {
        ...
        dojo.lang.assert(
          false,
          "new dojo.data.old.provider.FlatFile({url: }) was passed a file without a .csv or .json suffix"
        );
      }
    }
  }
...
// -------------------------------------------------------------------
dojo.data.old.provider.FlatFile.prototype.getProviderCapabilities = function (
  /* string */ keyword
) {
...
dojo.data.old.provider.FlatFile.prototype.registerAttribute = function (
...
dojo.data.old.provider.FlatFile.prototype.getAttribute = function (
  /* string or dojo.data.old.Attribute */ attributeId
) {
...
dojo.data.old.provider.FlatFile.prototype._newAttribute = function (
  /* String */ attributeId
) {
