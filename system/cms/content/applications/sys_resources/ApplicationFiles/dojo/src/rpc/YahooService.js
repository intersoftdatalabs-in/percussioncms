...
dojo.rpc.YahooService = function (appId) {
  ...
  if (!appId) {
    ...
    dojo.debug(
      "please initialize the YahooService class with your own",
  ...
  if (djConfig["useXDomain"] && !djConfig["yahooServiceSmdUrl"]) {
    dojo.debug(
      "dojo.rpc.YahooService: When using cross-domain Dojo builds," +
  ...
  this.connect(
    djConfig["yahooServiceSmdUrl"] ||
      dojo.uri.moduleUri("dojo.rpc", "yahoo.smd")
  );
