...
dojo.extend(dojo.logging.Logger, {
  ...
  filter: function (/*dojo.logging.Record*/ rec) {
    ...
    for (var x = 0; x < this.filters.length; x++) {
      if (
        (this.filters[x]["filter"] && !this.filters[x].filter(rec)) ||
...
dojo.lang.extend(dojo.logging.LogHandler, {
  ...
  handleError: function () {
    ...
    dojo.deprecated(
      "dojo.logging.LogHandler.handleError",
      "use handle()",
      "0.6"
    );
  },
...
void (function () {
  // begin globals protection closure
  var names = [
    "setLevel",
    "addFilter",
    "removeFilterByIndex",
...
dojo.declare("dojo.logging.MemoryLogHandler", dojo.logging.LogHandler, {
  initializer: function (level, recordsToKeep, postType, postInterval) {
    ...
    this.numRecords =
      typeof djConfig["loggingNumRecords"] != "undefined"
        ? djConfig["loggingNumRecords"]
        : recordsToKeep
        ? recordsToKeep
        : -1;
    // 0=count, 1=time, -1=don't post TODO: move this to a better location for prefs
    this.postType =
      typeof djConfig["loggingPostType"] != "undefined"
        ? djConfig["loggingPostType"]
        : postType || -1;
    // milliseconds for time, interger for number of records, -1 for non-posting,
    this.postInterval =
      typeof djConfig["loggingPostInterval"] != "undefined"
        ? djConfig["loggingPostInterval"]
        : postType || -1;
  },
  emit: function (record) {
    ...
    var logStr =
      String(
        dojo.log.getLevelName(record.level) +
          ": " +
...
dojo.logging.logQueueHandler = new dojo.logging.MemoryLogHandler(
  0,
  50,
  0,
  10000
