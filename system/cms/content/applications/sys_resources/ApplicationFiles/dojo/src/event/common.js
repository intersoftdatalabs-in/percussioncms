...
dojo.event = new (function () {
  ...
  this._kwConnectImpl = function (kwArgs, disconnect) {
    ...
    if (typeof kwArgs["adviceFunc"] == "function") {
      ...
      var tmpName = dojo.lang.nameAnonFunc(
        kwArgs.adviceFunc,
        kwArgs.adviceObj,
        true
      );
...
dojo.event.MethodInvocation = function (
  /*dojo.event.MethodJoinPoint*/ join_point,
  /*Object*/ obj,
  /*Array*/ args
) {
  // summary:
...
dojo.event.MethodJoinPoint.getForMethod = function (
  /*Object*/ obj,
  /*String*/ funcName
) {
  // summary:
  ...
  if (!joinpoint) {
    ...
    if (dojo.event["browser"]) {
      if (obj["attachEvent"] || obj["nodeType"] || obj["addEventListener"]) {
        isNode = true;
        dojo.event.browser.addClobberNodeAttrs(obj, [
          jpname,
          jpfuncname,
          funcName,
        ]);
...
dojo.lang.extend(dojo.event.MethodJoinPoint, {
  ...
  run: function () {
    ...
    var unrollAdvice = function (marr) {
      ...
      if (!callObj[callFunc]) {
        dojo.raise(
          'function "' + callFunc + '" does not exist on "' + callObj + '"'
        );
      }
      ...
      var hasDelay =
        !isNaN(delay) && marr[4] !== null && typeof marr[4] != "undefined";
      if (marr[5]) {
      ...
      if (aroundFunc) {
        ...
        if (hasDelay && (dojo.render.html || dojo.render.svg)) {
          // FIXME: the render checks are grotty!
          ...
          // many environments can't support delay!
          if (msg) {
            callObj[callFunc].call(callObj, to);
          } else {
            callObj[callFunc].apply(callObj, args);
  ...
  hasAdvice: function (thisAdviceObj, thisAdvice, adviceType, arr) {
    ...
    for (var x = 0; x < arr.length; x++) {
      var aao =
        typeof thisAdvice == "object"
          ? new String(thisAdvice).toString()
          : thisAdvice;
      var a1o =
