...
if (!djConfig["useXDomain"] || djConfig["allowXdRichTextSave"]) {
  if (dojo.hostenv.post_load_) {
    (function () {
      ...
      savetextarea.style =
        "display:none;position:absolute;top:-100px;left:-100px;height:3px;width:3px;overflow:hidden;";
    ...
    try {
      dojo
        .doc()
        .write(
          '<textarea id="dojo.widget.RichText.savedContent" ' +
  ...
  },
  {
    ...
    open: function (/*DomNode, optional*/ element) {
      ...
      if (
        ...
        var html = this._preFilterContent(
          dojo.string.trim(this.domNode.innerHTML)
        );
      }
      if (html == "") {
        html = "&nbsp;";
      ...
      this._firstChildContributingMargin = this._getContributingMargin(
        this.domNode,
        "top"
      );
      this._lastChildContributingMargin = this._getContributingMargin(
        this.domNode,
        "bottom"
      );
      ...
      if (
        ...
        var saveTextarea = dojo
          .doc()
          .getElementById("dojo.widget.RichText.savedContent");
      ...
      if (h.ie70 && this.useActiveX) {
        dojo.debug(
          "activeX in ie70 is not currently supported, useActiveX is ignored for now."
        );
      ...
      if (this.useActiveX && h.ie) {
      ...
      } else if (h.ie || this._safariIsLeopard() || h.opera) {
        ...
        this.document.open();
        this.document.write(
          "<html><head><style>body{margin:0;padding:0;border:0;overflow:hidden;}</style></head><body><div></div></body></html>"
        );
        this.document.close();
        ...
        for (var i in formats) {
          if (formats[i].charAt(1) != "l") {
            localhtml +=
              "<" + formats[i] + "><span>content</span></" + formats[i] + ">";
          } else {
            localhtml +=
              "<" + formats[i] + "><li>content</li></" + formats[i] + ">";
          }
        }
        ...
        while (node) {
          dojo.withGlobal(this.window, "selectElement", dojo.html.selection, [
            node.firstChild,
          ]);
    ...
    _hasCollapseableMargin: function (/*DomNode*/ element, /*String*/ side) {
      ...
      if (
        dojo.html.getPixelValue(element, "border-" + side + "-width", false)
      ) {
        return false;
    ...
    _getContributingMargin: function (
      /*DomNode*/ element,
      /*String*/ topOrBottom
    ) {
      // summary:
      ...
      function isSignificantNode(element) {
        ...
        return (
          !(element.nodeType == 3 && dojo.string.isBlank(element.data)) &&
      ...
      while (child) {
        ...
        childMargin = Math.max(
          childMargin,
          dojo.html.getPixelValue(child, marginProp, false)
        );
      ...
      while (sibling) {
        if (isSignificantNode(sibling)) {
          contextMargin = dojo.html.getPixelValue(
            sibling,
            siblingMarginProp,
            false
          );
      ...
      if (!sibling) {
        // no sibling, look at parent's margin instead
        contextMargin = dojo.html.getPixelValue(
          element.parentNode,
          marginProp,
          false
      ...
      if (childMargin > elementMargin) {
        return parseInt(
          Math.max(childMargin - elementMargin - contextMargin, 0)
        );
      } else {
        return 0;
      }
    ...
    _drawIframe: function (/*String*/ html) {
      ...
      var oldMoz = Boolean(
        dojo.render.html.moz && typeof window.XML == "undefined"
      );
      ...
      if (djConfig["useXDomain"] && !djConfig["dojoRichTextFrameUrl"]) {
        dojo.debug(
          "dojo.widget.RichText: When using cross-domain Dojo builds," +
      ...
      this.iframe.src =
        (djConfig["dojoRichTextFrameUrl"] ||
          dojo.uri.moduleUri("dojo.widget", "templates/richtextframe.html")) +
      ...
      if (this.relativeImageUrls) {
        ...
        for (var i = 0; i < imgs.length; i++) {
          imgs[i].src = new dojo.uri.Uri(
            dojo.global().location,
            imgs[i].src
          ).toString();
        }
      ...
      var ifrFunc = dojo.lang.hitch(this, function () {
        ...
        if (!this.editNode) {
          ...
          var lineHeight = "1.0";
          var lineHeightStyle = dojo.html.getUnitValue(
            this.domNode,
            "line-height"
          );
          ...
          dojo.html.insertCssText(
            "body,html{background:transparent;padding:0;margin:0;}" +
              ...
              "body > *:first-child{padding-top:0 !important;margin-top:" +
              this._firstChildContributingMargin +
              "px !important;}" + // FIXME: test firstChild nodeType
              "body > *:last-child{padding-bottom:0 !important;margin-bottom:" +
              this._lastChildContributingMargin +
              "px !important;}" +
    ...
    addStyleSheet: function (/*dojo.uri.Uri*/ uri) {
      ...
      if (dojo.lang.find(this.editingAreaStyleSheets, url) > -1) {
        dojo.debug(
          "dojo.widget.RichText.addStyleSheet: Style sheet " +
            url +
    ...
    removeStyleSheet: function (/*dojo.uri.Uri*/ uri) {
      ...
      if (index == -1) {
        dojo.debug(
          "dojo.widget.RichText.removeStyleSheet: Style sheet " +
            url +
      ...
      for (var i = 0; i < links.length; i++) {
        if (links[i].href == url) {
          if (dojo.render.html.ie) {
            //we need to empty the href first, to get IE to remove the rendered styles
            links[i].href = "";
          }
    ...
    _drawObject: function (/*String*/ html) {
      ...
      this.object.attachEvent(
        "DocumentComplete",
        dojo.lang.hitch(this, "onLoad")
      );
    ...
    _cacheLocalBlockFormatNames: function () {
      ...
      if (!this._native2LocalFormatNames["p"]) {
        ...
        try {
          var oNamesParm = new ActiveXObject(
            "DEGetBlockFmtNamesParam.DEGetBlockFmtNamesParam"
          );
          obj.ExecCommand(
            this._activeX.command["getblockformatnames"],
            0,
            oNamesParm
          );
    ...
    onLoad: function (e) {
      ...
      if (this.object) {
        ...
        this.editingArea.style.height = this.height
          ? this.height
          : this.minHeight;
        if (!this.height) {
        ...
        if (dojo.render.html.safari) {
          ...
          this.interval = setInterval(
            dojo.lang.hitch(this, "onDisplayChanged"),
            750
          );
          // dojo.raise("onload");
    ...
    onKeyPress: function (e) {
      ...
      if (this._keyHandlers[e.key]) {
        ...
        var handlers = this._keyHandlers[e.key],
          i = 0,
          handler;
    ...
    queryCommandAvailable: function (/*String*/ command) {
      ...
      return (
        (dojo.render.html.ie && supportedBy.ie) ||
    ...
    execCommand: function (/*String*/ command, argument) {
      ...
      if (this.object) {
        ...
        if (command == "inserthtml") {
          ...
          returnValue = this.object.ExecCommand(
    ...
    queryCommandEnabled: function (/*String*/ command) {
      ...
      if (this.object) {
        ...
        return (
          status != this._activeX.status.notsupported &&
    ...
    queryCommandState: function (command) {
      ...
      if (this.object) {
        ...
        var status = this.object.QueryStatus(this._activeX.command[command]);
        return (
          status == this._activeX.status.latched ||
    ...
    replaceEditorContent: function (/*String*/ html) {
      ...
      if (this.isClosed) {
        ...
        (this.document && this.document.selection)
      ) {
        // Moz/IE
        this.execCommand("selectall");
    ...
    _saveContent: function (e) {
      ...
      var saveTextarea = dojo
        ...
        .getElementById("dojo.widget.RichText.savedContent");
      saveTextarea.value +=
        this._SEPARATOR + this.saveName + ":" + this.getEditorContent();
    },
    ...
    getEditorContent: function () {
      ...
      if (this.relativeImageUrls) {
        ...
        var siteBase =
          dojo.global().location.protocol + "//" + dojo.global().location.host;
    ...
    disconnectAllWithRoot: function (targetObj) {
      dojo.deprecated(
        "disconnectAllWithRoot",
        "is deprecated. No need to disconnect manually",
        "0.5"
      );
    },
