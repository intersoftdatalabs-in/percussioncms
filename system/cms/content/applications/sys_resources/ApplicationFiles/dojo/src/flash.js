...
dojo.flash = {
  ...
  useFlash6: function () {
    ...
    if (this.flash6_version == null) {
      ...
      this.flash6_version != null &&
      dojo.flash.info.commVersion == 6
    ) {
...
dojo.flash.Info.prototype = {
  ...
  isVersionOrAbove: function (
    ...
    if (
      this.versionMajor >= reqMajorVer &&
      this.versionMinor >= reqMinorVer &&
  ...
  _detectCommunicationVersion: function () {
    ...
    if (
      typeof djConfig["forceFlashComm"] != "undefined" &&
...
dojo.flash.Communicator.prototype = {
  ...
  _handleFSCommand: function (command, args) {
    ...
    if (command == "addCallback") {
      // add Flash method for JavaScript callback
      this._fscommandAddCallback(command, args);
    } else if (command == "call") {
      // Flash to JavaScript method call
      this._fscommandCall(command, args);
    } else if (command == "fscommandReady") {
      // see if fscommands are ready
  ...
  _chunkReturnData: function () {
    ...
    for (var i = 0; i < numSegments; i++) {
      // directly use the underlying CallFunction method used by
      // ExternalInterface, which is vastly faster for large strings
      var piece = plugin.CallFunction(
        '<invoke name="chunkReturnData" ' +
          'returntype="javascript">' +
          "<arguments>" +
          "<number>" +
          i +
...
dojo.flash.Install.prototype = {
  ...
  install: function () {
    ...
    } else if (dojo.flash.info.isVersionOrAbove(6, 0, 65)) {
      // Express Install
  ...
  _onInstallStatus: function (msg) {
    if (msg == "Download.Complete") {
      ...
      window.location.href =
        dojo.flash.Embed.protocol() + "://www.macromedia.com/go/getflashplayer";
