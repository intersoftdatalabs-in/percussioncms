...
dojo.math.curves = {
    ...
    this.getValue = function (/* float */ step) {
      ...
      for (var j = 0; j < this.p[0].length; j++) {
        ...
        for (var i = 0; i < this.p.length; i++) {
          C +=
            this.p[i][j] *
            this.p[this.p.length - 1][0] *
            dojo.math.bernstein(step, this.p.length, i);
        }
        for (var l = 0; l < this.p.length; l++) {
          D +=
            this.p[this.p.length - 1][0] *
            dojo.math.bernstein(step, this.p.length, l);
        }
        retVal[j] = C / D;
      }
      return retVal; //	array
  ...
  Arc: function (/* array */ start, /* array */ end, /* boolean? */ ccw) {
    ...
    var sides = dojo.math.points.translate(
      dojo.math.points.invert(center),
      start
    );
    ...
    dojo.math.curves.CenteredArc.call(
      this,
      center,
      rad,
      theta,
