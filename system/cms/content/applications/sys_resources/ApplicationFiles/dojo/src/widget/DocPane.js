...
dojo.widget.defineWidget("dojo.widget.DocPane", dojo.widget.HtmlWidget, {
  // summary
  ...
  _loggedIn: function () {
    ...
    this.dialog.hide();
    this.pkgEditor = dojo.widget.createWidget(
      "editor2",
      {
        toolbarAlwaysVisible: true,
      },
  ...
  onPkgResult: function (/*Object*/ results) {
    ...
    for (var env in requires) {
      ...
      for (var i = 0, require; (require = requires[env][i]); i++) {
        ...
        var rRow2 = this.rRow2.parentNode.insertBefore(
          this.rRow2.cloneNode(true),
          this.rRow2
        );
        rRow2.style.display = "";
        appends.push(rRow2);
      }
      var rRow = this.rRow.parentNode.insertBefore(
        this.rRow.cloneNode(true),
        this.rRow
      );
      rRow.style.display = "";
      appends.push(rRow);
    ...
    if (results.size) {
      for (var i = 0, method; (method = methods[i]); i++) {
        ...
        var mRow = this.mRow.parentNode.insertBefore(
          this.mRow.cloneNode(true),
          this.mRow
        );
        mRow.style.display = "";
        appends.push(mRow);
    ...
    for (var i = 0, a; (a = as[i]); i++) {
      if (a.className == "docMLink") {
        ...
        dojo.event.connect(
          a,
          "onclick",
          makeSelect("package", requireLinks[i])
        );
      }
    }
  },
