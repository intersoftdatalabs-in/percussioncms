...
dojo.widget.TreeDemo = {
  reportIfDefered: function (res) {
    if (res instanceof dojo.Deferred) {
      res.addCallbacks(
        function (res) {
          /* dojo.debug("OK " + (res ? res: '')); */ return res;
        },
        function (err) {
          dojo.debug("Error");
  ...
  makeRandomChildren: function (title) {
    ...
    dojo.lang.forEach(children, function (child) {
      ...
      if (
        _this.randomChildrenCount < _this.randomChildrenMaxCount &&
        ((_this.randomChildrenDepth == 1 && child === children[0]) ||
          (_this.randomChildrenDepth < 5 && Math.random() > 0.3))
      ) {
