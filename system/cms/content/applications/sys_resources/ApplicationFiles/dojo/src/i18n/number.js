 ...
 **/
dojo.i18n.number.format = function (
  value,
  flags /*optional*/,
  locale /*optional*/
) {
  ...
  var formatData = dojo.i18n.number._mapToLocalizedFormatData(
  ...
  var whole = String(
    (flags.places > 0 || !flags.round ? Math.floor : Math.round)(value)
  );
  ...
  if (flags.groupSize2 && whole.length > flags.groupSize) {
    var groups = splitSubstrings(
      whole.substr(0, whole.length - flags.groupSize),
      flags.groupSize2
    );
    ...
    output =
      output + splitSubstrings(whole, flags.groupSize).join(flags.separator);
  } else {
    output = output + whole;
  }
  ...
  if (flags.places > 0) {
    ...
    fract = (flags.round ? Math.round : Math.floor)(
      fract * Math.pow(10, flags.places)
    );
 ...
 **/
dojo.i18n.number.parse = function (
  value,
  locale /*optional*/,
  flags /*optional*/
) {
  ...
  var whole = Number(
    numbers[0].replace(new RegExp("\\" + flags.separator, "g"), "")
  );
  var fract =
    numbers.length == 1
      ? 0
      : Number(numbers[1]) / Math.pow(10, String(numbers[1]).length); // could also do Number(whole + "." + numbers[1]) if whole != NaN
...
*/
dojo.i18n.number.isInteger = function (
  value,
  locale /*optional*/,
  flags /*optional*/
) {
  ...
  if (typeof flags.separator == "undefined") {
    ...
    dojo.lang.isArray(flags.separator) &&
    flags.separator.length === 0
  ) {
    flags.separator = [formatData[1], ""];
  }
  if (typeof flags.groupSize == "undefined") {
...
/**
  ...
  @return  true or false.
*/
dojo.i18n.number.isReal = function (
  value,
  locale /*optional*/,
  flags /*optional*/
) {
