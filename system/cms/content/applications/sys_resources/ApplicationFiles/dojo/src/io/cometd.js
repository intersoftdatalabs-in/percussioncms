...
cometd = new (function () {
  ...
  this.finishInit = function (type, data, evt, request) {
    ...
    if (data.version < this.minimumVersion) {
      dojo.debug(
        "cometd protocol version mismatch. We wanted",
        this.minimumVersion,
        "but got",
        data.version
      );
    ...
    this.tunnelCollapse = dojo.lang.hitch(
  ...
  this._deliver = function (message) {
      ...
      switch (message.channel) {
        case "/meta/subscribe":
          if (!message.successful) {
            dojo.debug(
              "cometd subscription error for channel",
              message.channel,
              ":",
              message.error
            );
        ...
        case "/meta/unsubscribe":
          if (!message.successful) {
            dojo.debug(
              "cometd unsubscription error for channel",
              message.channel,
              ":",
              message.error
            );
    ...
    if (message.data) {
      ...
      var tname = this.globalTopicChannels[message.channel]
        ? message.channel
        : "/cometd" + message.channel;
  ...
  this.subscribed = function (/*string*/ channel, /*obj*/ message) {
    dojo.debug(channel);
  ...
  this.unsubscribed = function (/*string*/ channel, /*obj*/ message) {
    dojo.debug(channel);
    dojo.debugShallow(message);
  };
...
cometd.iframeTransport = new (function () {
  ...
  this.check = function (types, version, xdomain) {
    return (
      !xdomain && !dojo.render.html.safari && dojo.lang.inArray(types, "iframe")
    );
  };
  ...
  this.postToIframe = function (content, url) {
    if (!this.phonyForm) {
      if (dojo.render.html.ie) {
        this.phonyForm = document.createElement(
          "<form enctype='application/x-www-form-urlencoded' method='POST' style='display: none;'>"
        );
    ...
    for (var x in content) {
      var tn;
      if (dojo.render.html.ie) {
        tn = document.createElement(
          "<input type='hidden' name='" + x + "' value='" + content[x] + "'>"
        );
  ...
  this.openTunnelWith = function (content, url) {
    ...
    if (dojo.render.html.mozilla) {
      this.xhr.addEventListener(
        "load",
        dojo.lang.hitch(this, "handleOnLoad"),
        false
      );
...
cometd.connectionTypes.register(
  "mime-message-block",
  cometd.mimeReplaceTransport.check,
  cometd.mimeReplaceTransport
);
cometd.connectionTypes.register(
  "long-polling",
  cometd.longPollTransport.check,
  cometd.longPollTransport
);
cometd.connectionTypes.register(
  "callback-polling",
  cometd.callbackPollTransport.check,
  cometd.callbackPollTransport
);
cometd.connectionTypes.register(
  "iframe",
  cometd.iframeTransport.check,
  cometd.iframeTransport
);
