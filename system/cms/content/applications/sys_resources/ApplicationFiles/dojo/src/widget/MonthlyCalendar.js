...
dojo.widget.defineWidget(
  ...
  dojo.widget.DatePicker,
  {
    ...
    templatePath: dojo.uri.moduleUri(
      "dojo.widget",
      "templates/MonthlyCalendar.html"
    ),
    templateCssPath: dojo.uri.moduleUri(
      "dojo.widget",
      "templates/MonthlyCalendar.css"
    ),
    ...
    initUI: function () {
      var dayLabels = dojo.date.getNames(
        "days",
        this.dayWidth,
        "standAlone",
        this.lang
      ...
      var nextDate = new Date(
        this.firstSaturday.year,
        this.firstSaturday.month,
        this.firstSaturday.date,
        8
      );
      var lastDay = new Date(
        this.firstSaturday.year,
        this.firstSaturday.month,
        this.firstSaturday.date + 42,
        8
      );
      ...
      if (this.firstSaturday.date < 7) {
        ...
        for (var i = this.firstSaturday.date; i > 0; i--) {
          ...
          dojo.html.setClass(
            currentCalendarNode,
            this.getDateClassName(nextDate, "current")
          );
          dayInWeek--;
        ...
        for (var i = dayInWeek; i > -1; i--) {
          ...
          dojo.html.setClass(
            currentCalendarNode,
            this.getDateClassName(nextDate, "previous")
          );
          previousDate = nextDate;
          nextDate = this.incrementDate(nextDate, false);
        }
      ...
      while (count < 42) {
        ...
        dojo.html.setClass(
          currentCalendarNode,
          this.getDateClassName(nextDate, "next")
        );
        currentCalendarNode = calendarNodes.item(++count);
        previousDate = nextDate;
        nextDate = this.incrementDate(nextDate, true);
...
dojo.widget.MonthlyCalendar.util = new (function () {
  ...
  this.fromRfcDate = function (rfcDate) {
    ...
    return new Date(
      parseInt(tempDate[0]),
      parseInt(tempDate[1], 10) - 1,
      parseInt(tempDate[2].substr(0, 2), 10)
    );
  };
