      ...
      switch (this.whitePoint) {
        ...
        default:
          dojo.debug(
            "White point " +
              this.whitePoint +
              " isn't defined for Std. Observer " +
              this.strObserver
          );
      }
      break;
...
dojo.gfx.Colorspace.prototype.getPrimaries = function () {
  ...
  switch (this.RGBWorkingSpace) {
    ...
    default:
      dojo.debug(
        "RGB working space " + this.RGBWorkingSpace + " isn't defined"
      );
  }
  ...
  if (p.wp != this.whitePoint) {
    var r = this.XYZ_to_xyY(
      this.chromaticAdaptation(
        this.xyY_to_XYZ([p.xr, p.yr, p.Yr]),
        p.wp,
        this.whitePoint
      )
    );
    var g = this.XYZ_to_xyY(
      this.chromaticAdaptation(
        this.xyY_to_XYZ([p.xg, p.yg, p.Yg]),
        p.wp,
        this.whitePoint
      )
    );
    var b = this.XYZ_to_xyY(
      this.chromaticAdaptation(
        this.xyY_to_XYZ([p.xb, p.yb, p.Yb]),
        p.wp,
        this.whitePoint
      )
    );
...
dojo.gfx.Colorspace.prototype.RGB_to_XYZ = function () {
  ...
  if (this.RGBWorkingSpace == "s_rgb") {
    var r =
      src.R > 0.04045 ? Math.pow((src.R + 0.055) / 1.055, 2.4) : src.R / 12.92;
    var g =
      src.G > 0.04045 ? Math.pow((src.G + 0.055) / 1.055, 2.4) : src.G / 12.92;
    var b =
      src.B > 0.04045 ? Math.pow((src.B + 0.055) / 1.055, 2.4) : src.B / 12.92;
  } else {
...
dojo.gfx.Colorspace.prototype.XYZ_to_Lab = function () {
  ...
  var fx =
    xr > this.epsilon() ? Math.pow(xr, 1 / 3) : (this.kappa() * xr + 16) / 116;
  var fy =
    yr > this.epsilon() ? Math.pow(yr, 1 / 3) : (this.kappa() * yr + 16) / 116;
  var fz =
    zr > this.epsilon() ? Math.pow(zr, 1 / 3) : (this.kappa() * zr + 16) / 116;
...
dojo.gfx.Colorspace.prototype.Lab_to_XYZ = function () {
  ...
  var wp = this.getWhitePoint();
  var yr =
    src.L > this.kappa() * this.epsilon()
      ? Math.pow((src.L + 16) / 116, 3)
      : src.L / this.kappa();

  var fy =
    yr > this.epsilon() ? (src.L + 16) / 116 : (this.kappa() * yr + 16) / 116;
  var fx = src.a / 500 + fy;
  var fz = fy - src.b / 200;
...
dojo.gfx.Colorspace.prototype.chromaticAdaptation = function (
  col,
  src_w,
  dst_w
) {
  ...
  switch (this.chromaticAdaptationAlg) {
    ...
    case "von_kries":
      ...
      var mai = [
        [1.859936, 0.361191, 0.0],
        [-1.129382, 0.638812, 0.0],
    ...
    default:
      dojo.debug(
        "The " +
          this.chromaticAdaptationAlg +
          " chromatic adaptation algorithm matricies are not defined"
      );
  }

  var domain_src = dojo.math.matrix.multiply(
    [[wp_src.x, wp_src.y, wp_src.z]],
    ma
  );
  var domain_dst = dojo.math.matrix.multiply(
    [[wp_dst.x, wp_dst.y, wp_dst.z]],
    ma
  );
...
dojo.gfx.Colorspace.prototype.XYZ_to_Luv = function () {
  ...
  var L =
    yr > this.epsilon() ? 116 * Math.pow(yr, 1 / 3) - 16 : this.kappa() * yr;
  var u = 13 * L * (ud - udr);
  var v = 13 * L * (vd - vdr);
