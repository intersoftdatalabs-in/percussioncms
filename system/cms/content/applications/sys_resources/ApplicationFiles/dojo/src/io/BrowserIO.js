...
if (!dj_undef("window")) {
  ...
  dojo.io.updateNode = function (
    /*DOMNode*/ node,
    /*String or Object*/ urlOrArgs
  ) {
  ...
  dojo.io.XMLHTTPTransport = new (function () {
    ...
    function doLoad(kwArgs, http, url, query, useCache) {
      if (
        (http.status >= 200 && http.status < 300) || // allow any 2XX response code
        ...
        if (kwArgs.method.toLowerCase() == "head") {
          ...
          kwArgs.mimetype.substr(0, 9) == "text/json" ||
          kwArgs.mimetype.substr(0, 16) == "application/json"
        ) {
          try {
        ...
        kwArgs[typeof kwArgs.load == "function" ? "load" : "handle"](
          "load",
          ret,
          http,
          kwArgs
        ...
        var errObj = new dojo.io.Error(
          "XMLHttpTransport Error: " + http.status + " " + http.statusText
        );
        kwArgs[typeof kwArgs.error == "function" ? "error" : "handle"](
          "error",
          errObj,
          http,
          kwArgs
    ...
    this.bind = function (/*dojo.io.Request*/ kwArgs) {
      ...
      if (kwArgs.method.toLowerCase() == "post") {
        ...
        if (preventCache) {
          tmpUrl +=
            (dojo.string.endsWithAny(tmpUrl, "?", "&")
              ? ""
              : tmpUrl.indexOf("?") > -1
              ? "&"
              : "?") +
      ...
      kwArgs.abort = function () {
        try {
          // khtml doesent reset readyState on abort, need this workaround
          http._aborted = true;
        } catch (e) {
          /*squelsh*/
        }
