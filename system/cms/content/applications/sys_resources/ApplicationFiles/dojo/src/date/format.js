...
(function () {
  dojo.date.format = function (/*Date*/ dateObject, /*Object?*/ options) {
    ...
    if (options.selector != "timeOnly") {
      var datePattern =
        options.datePattern || info["dateFormat-" + formatLength];
      if (datePattern) {
    ...
    if (options.selector != "dateOnly") {
      var timePattern =
        options.timePattern || info["timeFormat-" + formatLength];
      if (timePattern) {
  ...
  dojo.date.parse = function (/*String*/ value, /*Object?*/ options) {
    ...
    var groups = [];
    var dateREString = _processPattern(
      pattern,
      dojo.lang.curry(this, _buildDateTimeRE, groups, info, options)
    );
    ...
    switch (switchCase) {
      ...
      case "swap": // Upper to lower, and versey-vicea
        ...
        while (j < property.length) {
          ...
          swapString +=
            ch == compareString.charAt(j) ? ch.toUpperCase() : ch.toLowerCase();
          j++;
        }
        property = swapString;
        break;
...
(function () {
  var _customFormats = [];
  dojo.date.addCustomFormats = function (
    /*String*/ packageName,
    /*String*/ bundleName
  ) {
    //
...
dojo.date.getDayName = function (/*Date*/ dateObject, /*String?*/ locale) {
  ...
  return dojo.date.getNames("days", "wide", "format", locale)[
    dateObject.getDay()
  ]; /*String*/
};
...
dojo.date.getDayShortName = function (/*Date*/ dateObject, /*String?*/ locale) {
  ...
  return dojo.date.getNames("days", "abbr", "format", locale)[
    dateObject.getDay()
  ]; /*String*/
};
...
dojo.date.getMonthName = function (/*Date*/ dateObject, /*String?*/ locale) {
  ...
  return dojo.date.getNames("months", "wide", "format", locale)[
    dateObject.getMonth()
  ]; /*String*/
};

dojo.date.getMonthShortName = function (
  /*Date*/ dateObject,
  /*String?*/ locale
) {
  // summary: gets the abbreviated localized month name corresponding to the date object
  return dojo.date.getNames("months", "abbr", "format", locale)[
    dateObject.getMonth()
  ]; /*String*/
};
