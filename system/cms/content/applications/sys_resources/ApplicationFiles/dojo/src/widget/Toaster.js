...
dojo.widget.defineWidget("dojo.widget.Toaster", dojo.widget.HtmlWidget, {
  // summary
  ...
  postCreate: function () {
    ...
    if (
      ...
      !dojo.lang.inArray(this.positionDirectionTypes, this.positionDirection)
    ) {
  ...
  setContent: function (msg, messageType, delay) {
    ...
    if (!msg) {
      dojo.debug(
        this.widgetId + ".setContent() incoming content was null, ignoring."
      );
      return;
    }
    if (
      ...
      dojo.raise(
        this.widgetId +
          ".positionDirection is an invalid value: " +
          this.positionDirection
      );
    }
  ...
  _placeClip: function () {
    ...
    if (this.positionDirection.match(/^t/)) {
      ...
      this.clipNode.style.top =
        view.height - nodeSize.height - 2 + scroll.top + "px";
    }
    if (this.positionDirection.match(/^[tb]r-/)) {
      this.clipNode.style.left =
        view.width - nodeSize.width - 1 - scroll.left + "px";
    } else if (this.positionDirection.match(/^[tb]l-/)) {
      this.clipNode.style.left = 0 + "px";
    }

    this.clipNode.style.clip =
      "rect(0px, " + nodeSize.width + "px, " + nodeSize.height + "px, 0px)";
