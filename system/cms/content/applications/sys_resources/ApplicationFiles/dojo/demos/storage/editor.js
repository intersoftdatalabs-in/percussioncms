...
if (
  dojo.render.html.ie70 == true &&
  window.location.protocol.indexOf("file") != -1
) {
  alert(
    "Moxie on Internet Explorer 7.0 is not yet supported when loaded from the file:// URL"
  );
}
...
var Moxie = {
  ...
  _save: function (key, value) {
    ...
    var saveHandler = function (status, keyName) {
      ...
      if (status == dojo.storage.FAILED) {
        alert(
          "You do not have permission to store data for this web site. " +
  ...
  _printStatus: function (message) {
    ...
    for (var i = 0; i < top.childNodes.length; i++) {
      ...
      if (
        currentNode.nodeType == dojo.dom.ELEMENT_NODE &&
