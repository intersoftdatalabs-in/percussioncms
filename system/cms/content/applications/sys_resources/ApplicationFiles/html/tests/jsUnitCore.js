...
function _assert(comment, booleanValue, failureMessage) {
  if (!booleanValue) throw new JsUnitException(comment, failureMessage);
}
...
function assertEvaluatesToTrue() {
  ...
  if (!value) fail(commentArg(1, arguments));
}
...
function warn() {
  if (top.tracer != null) top.tracer.warn(arguments[0], arguments[1]);
}

function inform() {
  if (top.tracer != null) top.tracer.inform(arguments[0], arguments[1]);
}
...
function debug() {
  if (top.tracer != null) top.tracer.debug(arguments[0], arguments[1]);
}
...
function trim(str) {
  ...
  while (str.substring(endingIndex, endingIndex + 1) == " ") endingIndex--;
...
function jsUnitSetOnLoad(windowRef, onloadHandler) {
  ...
  if (typeof windowRef.attachEvent != "undefined") {
    ...
    typeof windowRef.document.addEventListener != "undefined" &&
    !isKonqueror
  ) {
    // DOM 2 Events
