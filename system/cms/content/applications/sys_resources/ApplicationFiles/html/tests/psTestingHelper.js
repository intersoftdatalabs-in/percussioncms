function getTestFunctionNames() {
  ...
  if (typeof document.scripts != "undefined" && document.scripts.length > 0) {
    // IE5 and up
    var scripts = document.scripts;

    for (i = 0; i < scripts.length; i++) {
      var someNames = _extractTestFunctionNamesFromScript(scripts[i]);
      if (someNames) testFunctionNames = testFunctionNames.concat(someNames);
    }
  } else {
    for (i in self) {
...
function ___psRunTest() {
  ...
  for (i = 0; i < tests.length; i++) {
    ...
    try {
      ...
      var msg = "";
      if (e.comment != null) msg += e.comment + "\n\n";
      msg += e.jsUnitMessage + "\n\n";
      msg += "StackTrace:\n";
      msg += e.stackTrace + "\n";
