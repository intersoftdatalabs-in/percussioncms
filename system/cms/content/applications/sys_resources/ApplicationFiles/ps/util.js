...
ps.util.setDialogSize = function (dialog, width, height) {
  ...
  if (pHeight >= scrSize.height) height = scrSize.height * 0.96;
  if (pWidth >= scrSize.width) width = scrSize.width * 0.96;
...
ps.util.CreatePromptDialog = function (options) {
  ...
  ps.event.connect(this.wgtDlg, "onLoad", function () {
    ...
    _this.wgtButtonOk.onClick = function () {
      if (
        _this.textRequired &&
        ps.util.trim(_this.wgtPromptText.value).length < 1
      ) {
    ...
    if (rules.length > 0) {
      for (var j = 0; j < rules.length; j++) {
        ...
        if (
          ...
          elemStyles =
            elemStyles == "" ? s.cssText : elemStyles + ";" + s.cssText;
          if (clrStyle) s.cssText = "";
        }
      }
    }
  }
