...
ps.DivActionHelper.handleDivHover = function (div, isHovering) {
  ...
  if (objId.isFieldNode()) ps.util.handleIE6FieldDivHover(div, isHovering);
  if (ps.DivActionHelper.mouseOverInfo.dndOccurring) return;
  if (___sys_aamode == 1) {
...
ps.DivActionHelper._getPageDiv = function () {
  ...
  for (i = 0; i < divs.length; i++) {
    if (divs[i].className == "PsAaPage") return divs[i];
  }
  return null;
};
...
ps.DivActionHelper.reset = function () {
  if (
    ps.DivActionHelper.mouseOverInfo.mouseOnFloat ||
    !ps.DivActionHelper.mouseOverInfo.fIconsActive
  )
    return;
...
ps.DivActionHelper._copyArray = function (arr) {
  var nArr = new Array();
  for (i = 0; i < arr.length; i++) nArr[i] = arr[i];
  return nArr;
};
