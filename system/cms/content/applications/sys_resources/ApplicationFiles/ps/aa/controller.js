...
ps.aa.controller = new (function () {
  ...
  ((this.init = function () {
    ...
    (this.updateBodyStyles = function () {
      ...
      else if (ilStyles.length > 0) newStyle = ilStyles;
      else if (elemStyles.length > 0) newStyle = elemStyles;

      //Move the new style to div element
      if (newStyle.length > 0) pageDivElem.style.cssText = newStyle;
  ...
  this._refreshX = function (id, newId, onlyContent, actionObject, action) {
    ...
    if (response.isSuccess()) {
      ...
      if (onlyContent) {
        if (!id.equals(newId)) this._replaceDomId(id, newId);
    ...
    if (___sys_aamode == 1) ps.DivActionHelper.reset();
    return notChangedId;
  };
  ...
  this.addSnippet = function (source) {
    ...
    switch (source) {
      ...
      case ps.aa.Menu.INSERT_FROM_SNIPPET:
        ...
        this.snippetPickerDlg.open(
          function (slotId, relId, position) {
            _this._openBrowseDlg(slotId, relId, position);
          },
          function () {},
          snSlotId,
          1,
  ...
  this.openRemoveSnippetsDlg = function () {
    ...
    var _this = this;
    this.snippetPickerDlg.open(
      function (slotId, relIds) {
        _this._handleRemoveSnippets(slotId, relIds);
      },
      function () {},
      this.activeId,
      0,
  ...
  this.createItem = function (source) {
    ...
    switch (source) {
      ...
      case ps.aa.Menu.COPY_FROM_CONTENT: //Copy from Page
        ...
        var newData = {
          sys_contenttypeid: itemId.getContentTypeId(),
          sys_templateid: itemId.getTemplateId(),
          folderPath: fpath,
          itemPath: ipath,
          itemTitle: "",
        };
  ...
  this.handleCreateItem = function (slotId, itemId, position, newData) {
    var response = ps.io.Actions.createItem(
  ...
  this.postCreateItem = function (slotId, itemId, position, newData, cid) {
    if (slotId == null) {
      ...
      var response = ps.io.Actions.addSnippet(snippetId, slotId, null, null);
      ps.io.Actions.maybeReportActionError(response);
      if (!response.isSuccess()) {
        return;
      }
  ...
  this.logout = function () {
    ...
    var url = ps.util.addParamToUrl(urlPath, "sys_redirecturl", escape(aaUrl));
    window.location = url;
  };
  ...
  this._getSizedStyle = function (style, width, height) {
    if (style == null) style = this.BASIC_WINDOW_STYLE;
    if (width == null || width < 0) width = 800;
  ...
  this.editAll = function (cachedUrl) {
    ...
    //whether he wants to open this item and cancel editing of the other item?
      ...
      if (!confirm(this.INPLACE_EDITOROPEN_MSG)) return false;
  ...
  this._resetTreeLabel = function (newtitle) {
    ...
    if (this.editObjectId == null) return;
  ...
  this._getAffectedObjectIds = function (contentId, fieldName) {
    ...
    for (var i = 0; i < allObjIds.count; i++) {
      ...
      if (!allContentIds.contains(cid)) allContentIds.add(cid);
    }
  ...
  this._onBeforeDomChange = function (id) {
    ...
    if (___sys_aamode != 1) ps.aa.Menu.unBindContextMenu(ids);
  };
  ...
  this._onDomChanged = function (id) {
    ...
    for (var i = 0; i < ids.count; i++) this.refreshImageForObject(ids.item(i));

    // bind the context menu from the to be replaced DOM elements
    if (___sys_aamode != 1) ps.aa.Menu.bindContextMenu(ids);
  };
  ...
  this.refreshOpener = function (contentId) {
    ...
    if (wopener == null || wopener.closed) return;
