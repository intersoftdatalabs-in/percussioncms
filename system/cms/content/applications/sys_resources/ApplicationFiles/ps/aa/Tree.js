...
ps.aa.Tree = function () {
  ...
  this._getParentPageSnippet = function (childNode, lastParent) {
    ...
    while (parentNode) {
      if (
        (parentNode.isPageNode() || parentNode.isSnippetNode()) &&
  ...
  this._getAllIdsByContentId = function (contentId, node, result) {
    if (contentId == null) result.add(node.objId);
    if (node.objId.getContentId() == contentId) result.add(node.objId);
  ...
  this._getFieldIdsByContentIdName = function (
    ...
    for (var i = 0; i < pnode.childNodes.count; i++) {
      ...
      if (
        ...
        this._getFieldIdsByContentIdName(contentId, fieldName, node, result);
      }
    }
  };
  ...
  this.removeNode = function (id) {
    ...
    if (pnode != null) pnode.childNodes.remove(node);
  ...
  this.toString = function (node) {
    if (typeof node === "undefined") {
      ...
      if (node.isLeafNode()) return text;
