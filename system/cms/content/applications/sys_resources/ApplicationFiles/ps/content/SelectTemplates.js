...
ps.content.SelectTemplates = function () {
  ...
  this.loadPreviewPane = function (snippetId) {
    ...
    if (response.isSuccess()) {
      ...
      for (i = 0; i < res.length; i++) {
        if (res[i].variantid == tempId) {
          if (res[i].outputformat == "Binary") isBinary = true;
          break;
        }
      }
    }
    ...
    if (this.parentMode == ps.util.BROWSE_MODE_RTE_INLINE_IMAGE) {
      ...
      this.parentMode == ps.util.BROWSE_MODE_RTE_INLINE_LINK &&
      isBinary
    ) {
      var isImage = false;
      ...
      if (response.isSuccess()) {
        if (isImage) {
          ...
          content +=
            "<a id ='downloadAnchor' onclick='webviewAction.showFileInBrowser();' target=\"_new\" href=\"" +
