ps.declare(
  ...
  },
  {
    ...
    _onOk: function () {
      ...
      } else var inclsites = this.includeSitesCheckbox.checked ? "yes" : "no";
      ...
      if (!this.isSearchSimple) sf.sys_searchMode.value = "advanced";

      if (!this._validateQueryForSynonymExp()) return;
    ...
    _onCancel: function () {
      ...
      if (typeof psSearch != "undefined") psSearch = null;
    ...
    _validateQuery: function () {
      if (!document.searchQuery.sys_fulltextquery) return true;
    ...
    _validateQueryForSynonymExp: function () {
      if (!document.searchQuery.sys_synonymexpansion) return true;
      ...
      if (synonymExp) {
        ...
        for (var i = 0; i < specialChars.length; i++) {
          ...
          if (searchForValue.indexOf(spChar) != -1) {
            if (spChars.length == 0) spChars = spChar;
            else spChars += ", " + spChar;
          }
        }
