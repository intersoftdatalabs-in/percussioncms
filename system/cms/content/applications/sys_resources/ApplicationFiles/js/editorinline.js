...
function inlineCallback(objectId, callback, type) {
  ...
  var buff = "";
  var attribs = "";
  var allowTrueInlineTemplates =
    (___serverProperties.allowTrueInlineTemplates != undefined
      ? ___serverProperties.allowTrueInlineTemplates
      : "true"
    ).toLowerCase() == "true";
  ...
  if (type == "rxhyperlink" || type == "rximage") {
    ...
    if (response2.isSuccess()) {
      ...
      attribs +=
        ' contenteditable="false" class="rx_inlinevariant mceNonEditable"';

      attribs +=
        ' rxselectedtext="' + encodeURIComponent(___selectedContent) + '"';
...
function isBlockTag(tagname) {
  ...
  for (i = 0; i < len; i++) {
    if (___blockTags[i].toLowerCase() == tagname.toLowerCase()) return true;
  }
  return false;
}

function createInlineSearchBox(
  type,
  selectedText,
  inlineslotid,
  ctypeid,
