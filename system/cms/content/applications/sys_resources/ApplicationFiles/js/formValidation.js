        ...
        if (theform[i].name == parampair[0]) {
          if (theform[i].type == "select-one") {
            for (k = 0; k < theform[i].options.length; k++) {
              if (
                theform[i].options[k].value == replace(parampair[1], "+", " ")
              ) {
...
function reqFileExtinURL(fieldValue, fieldName, fileExt) {
  if (
    fieldValue
      .toString()
      .toUpperCase()
      .indexOf("." + fileExt.toString().toUpperCase()) == -1
  ) {
...
function dateValidate(dateValue) {
  ...
  if (dateValue !== "") {
    ...
    if (
      ...
      datearr[1] == "" ||
      datearr[2] == "" ||
      parseInt(datearr[0], 10) != datearr[0] ||
      parseInt(datearr[1], 10) != datearr[1] ||
      parseInt(datearr[2], 10) != datearr[2]
    ) {
      alert(ermsg);
      return false;
    }
...
function showUserSearch(datafield, fromRoles, contentid) {
  if (fromRoles == null) fromRoles = "";
  userDataField = datafield;
  window.open(
    "../sys_ServerUserRoleSearch/rolelist.html?sys_command=GetRoles&fromRoles=" +
      fromRoles +
      "&sys_contentid=" +
      contentid,
    "rolesearch",
