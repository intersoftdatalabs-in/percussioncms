...
function ps_setUpdateFlag() {
  if (window.opener != null) window.opener.ps_updateFlag = true;
}
...
function isOpenerActiveAssembly() {
  if (window.opener != null && !window.opener.closed) {
    ...
    if (openerUrl.indexOf("sys_command=editrc") !== -1) return true;
  }
  return false;
}
...
function ps_getAllFieldChecksums(theForm, isInitial) {
  ...
  if (ps_theForm != null) {
      ...
      if (currentEl.type !== "hidden") {
        if (currentEl.type === "checkbox" || currentEl.type === "radio") {
          if (currentEl.length === null) {
            ...
            if (currentEl.checked === true) out += "CK" + i;
          } else {
            //Handle checkbox and radio button groups
            for (y = 0; y < currentEl.length; y++) {
              if (currentEl[y].checked === true) out += "CK" + i;
            }
          }
        } else if (
          currentEl.type === "select-one" ||
    ...
    if (
      !isInitial &&
      typeof psCustomControlIsDirty === "function" &&
      psCustomControlIsDirty()
    ) {
      out += psCustomControlIsDirty();
    }
  }
...
function ps_getInitialChecksum(theForm) {
  // Make sure we set our editLive session as clean upon first load

  if (typeof _editLiveInstance != "undefined") {
...
function hex_hmac_md5(key, data) {
  return binl2hex(str_hmac_md5(key, data));
}
function b64_hmac_md5(key, data) {
  return binl2b64(str_hmac_md5(key, data));
}
