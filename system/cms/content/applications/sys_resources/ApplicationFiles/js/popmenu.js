...
function PSCreateMenu(id, menu, parentID, parentItemID) {
  ...
  if (is_safari) PSCreateMenuSafari(menu, menuIndex);
  else PSCreateMenuIE(menu, menuIndex);
...
function PSCreateMenuIE(menu, menuIndex) {
  ...
  for (var j = 0; j < menu.items.length; j++) {
    ...
    if (subItems) PSCreateMenu(null, item, menuIndex, j);
    ...
    var controlBlock =
      ' height="' +
      h +
      '" onmouseover="PSEnterItemIE(this,' +
      menuIndex +
      ", " +
      j +
      ');"' +
      ' onmouseout="PSExitItemIE(this,' +
      menuIndex +
      ", " +
      j +
      ');" ';
  ...
  var menuHeader =
    '<div class="PSpopupmenu" id="PSXMenu' +
    menuIndex +
    '" onmouseover = "PSEnterMenu(' +
    menuIndex +
    ');"' +
    ' onmouseout  = "PSExitMenu(' +
    menuIndex +
    ');"' +
    ' style="position: absolute; z-index:' +
    z +
    '">' +
    ' <table class="PSmenuitem" border="0" cellpadding="0" cellspacing="0" width="' +
    menu.width +
    '">';
...
function PSCreateMenuSafari(menu, menuIndex) {
  var h = PSitemHeight;
  var spacerHTML =
    '<img align=absmiddle src="' + PS_SPACER_IMG + '" width="21" height="19"/>';
  var menuBody = "";
  var noEntries = menu.items.length == 0;
  ...
  var menuHeader =
    ...
    z +
    '">' +
    ' <table class="PSmenuitem" border="0" cellpadding="0" cellspaing="0" width="' +
    menu.width +
    '">';
...
function PSRealScrollTop() {
  ...
  else if (window.pageYOffset) s = window.pageYOffset;
...
function PSRealScrollLeft() {
  ...
  else if (window.pageXOffset) s = window.pageXOffset;
...
function PSCleanUpLeftOverObj() {
  if (PSlastEnteredObj == null) return;
...
function PSEnterMenu(ID) {
  ...
  if (parentID == null) PSClearHideTimer(ID);
  else PSReleaseMenuTree(ID);
}
...
function PSReleaseMenuTree(ID) {
  PSClearHideTimer(ID);
  var parentID = PSmenuElement[ID].parentID;
  if (parentID != null) PSReleaseMenuTree(parentID);
}

function PSTimeoutMenuTree(ID) {
  PSDelayHidePopMenu(ID);
  var parentID = PSmenuElement[ID].parentID;
  if (parentID != null) PSTimeoutMenuTree(parentID);
}
...
function PSCalcMenuHeight(menu) {
  ...
  for (var j = 0; j < menu.items.length; j++) {
    var item = menu.items[j];
    if (item.label == "-")
      // spacer is less height
      menu.height += 2 + 2;
    else menu.height += h + 2;
  }
}
...
function PSMovePopMenu(ID, x, y) {
  ...
  var clientHeight =
    (is_ie ? document.body.clientHeight : window.innerHeight) - 6;
...
function PSActivateParent() {
  var x = PSParentURL();
  if (x.indexOf("parentPage") == -1) x += "&parentPage=yes";
  window.location.href = x;
}
...
function PSActivateItem(id) {
  ...
  if (h.sys_trail == null) h.sys_trail = id;
  else h.sys_trail += ":" + id;
  ...
  if (y.indexOf("?parentPage=yes&amp;") != -1)
    ...
    tmp = y.split("&amp;parentPage=yes");
  else if (y.indexOf("&parentPage=yes") != -1) tmp = y.split("&parentPage=yes");
  var x = y;
  if (tmp != null) {
    x = tmp[0];
    if (tmp.length > 1) x += tmp[1];
  }
...
function PSBreadCrumbParentItemURL() {
  ...
  if (trail.length - 1 < 0) url = PSParentURL();
  else {
    // update the breadcrumb trail
...
function PSMoveItem(command, url, sysid) {
  ...
  if (command == "delete") redir = PSBreadCrumbParentItemURL();
...
function PSAddItem(url, slotname) {
  ...
  if (activeitemid == null) activeitemid = 0;
  x +=
    "sys_slotname=" +
    slotname +
    "&sys_contentid=" +
...
function PSEditContent(url, sysconid, sysrevid) {
  ...
  var p =
    "location=1,directories=0,status=0,menubar=0,scrollbars=1,resizable=1";
  if (is_ie) p += ",toolbar=0";
  else p += ",toolbar=1";
...
function PSViewContent(url, sysconid, sysrevid) {
  ...
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_pageid=0&sys_command=preview&sys_view=sys_All";
...
function PSViewMeta(url, sysconid, sysrevid) {
  ...
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_pageid=0&sys_command=preview&sys_view=sys_ItemMeta";
  var w = window.open(
    x,
    "viewmeta",
    "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=600,height=400",
...
function PSNewVersion(url, sysconid, sysrevid) {
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_pageid=0&sys_command=relate&sys_relationshiptype=NewCopy&sys_view=sys_All";
  var w = window.open(
    x,
    "newversion",
    "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=600,height=400",
...
function PSPromotableVersion(url, sysconid, sysrevid) {
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_pageid=0&sys_command=relate&sys_relationshiptype=PromotableVersion&sys_view=sys_All";
  var w = window.open(
    x,
    "promotableversion",
    "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=600,height=400",
...
function PSActiveAssemblyItem(
  ...
  x +=
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_authtype=0&sys_context=0&sys_command=editrc&parentPage=yes";
  if (assemblyUrl) {
...
function PSAuditTrail(url, sysconid, sysrevid) {
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_command=preview&sys_userview=sys_audittrail";
  var w = window.open(
    x,
    "audittrail",
...
function PSShowRevisions(url, sysconid, sysrevid) {
  var x =
    ...
    "&sys_revision=" +
    sysrevid +
    "&sys_command=preview&sys_userview=sys_Revisions";
  var w = window.open(
    x,
    "showrevisions",
    "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=600,height=200",
...
function PSBuildWFAction(
  url,
  checkintransitionurl,
  transitionname,
  commentrequired,
  showadhoc,
  sysconid,
...
function PSCheckinCheckout(
  ...
  if (confirmAction) window.location.href = x;
}
...
function PSFieldEditSpan(start) {
  if (!is_nav4) {
    if (start) document.write('<span style="position:absolute">');
...
function PSTranslateItem(url, sysconid, revision) {
  var x =
    ...
    "&sys_revision=" +
    revision +
    "&sys_relationshiptype=Translation";
  var w = window.open(
    x,
    "translateitem",
...
function PSPurgeItem(url, sessionid, sysconid) {
  var x =
    appendQuestionOrAmpersandtoURL(url) +
    "sys_contentid=" +
    sysconid +
    "&pssessionid=" +
    sessionid;
