  ...
  $.fn.perc_checkboxTree = function (opts) {
    ...
    return this.each(function () {
      ...
      function doNode(node, parentItem, selected) {
        ...
        if (readonly) resultItem.unselectable = true;
        if (children.length) {
      ...
      function resizeTreeWidthToFitContent() {
        ...
        if (biggestWidth > parentWidth) container.width(biggestWidth);
      }
      ...
      getTreeXml(treeSrcUrl, function (xml) {
        ...
        if (children) rootItem.children = children;
        var tree = [rootItem];
        _testTree = tree;
        if (tree[0].children.length > 0) {
          displayTree(tree);
        } else {
          $("#page_categories_tree-tree")
            .html("")
