...
(function ($) {
  $.fn.perc_checkboxTreeReadonly = function (opts) {
    ...
    function findSelectedTitle(selected, tree, returnString, categoryCount) {
      ...
      if (returnString == null) returnString = "/" + selectedArray[1];

      for (i = 0; i < tree.length; i++) {
        if (returnString.split("/").length == selectedArray.length) break;
        if (tree[i].nodeType == 1) {
          if (
            ...
            returnString =
              returnString +
              "/" +
              tree[i].attributes.getNamedItem("label").value;

            if (selectedArray.length > categoryCount) {
              returnString = findSelectedTitle(
                selected,
                tree[i].childNodes,
                returnString,
                categoryCount + 1,
