...
var AjaxSwingProxy = (function () {
  ...
  return {
    ...
    refresh: function (hint, contentids, revisionids) {
      window.document.getElementById("maincontent").src =
        hostBase +
        "?refresh=true&hint=" +
        hint +
        "&contentids=" +
...
function _showHelp() {
  if (is_safari || (is_ie == true && is_mac == true))
    if (arguments.length == 1) document.applets.help.showHelp(arguments[0]);
    else document.applets.help.showHelp();
  else if (arguments.length == 1) document.help.showHelp(arguments[0]);
  else document.help.showHelp();
}
...
function AppletCaller_addParam(name, value) {
  var type = "embed";
  if (is_safari || (is_ie == true && is_mac == true)) type = "applet";
  else if (is_ie == true) type = "object";
  if (isTagAttrib(type, name)) {
    if (type == "object") {
      if (name.toLowerCase() == "codebaseattr") name = "codebase";
      if (name.toLowerCase() == "typeattr") name = "type";
    }
    if (type == "embed") {
      if (
        name.toLowerCase() === "codebaseattr" ||
    ...
    for (i = 0; i < this.attribs.length; i++) {
      if (this.attribs[i].name.toLowerCase() == name.toLowerCase()) idx = i;
    }
    ...
    for (i = 0; i < this.params.length; i++) {
      if (this.params[i].name.toLowerCase() == name.toLowerCase()) idx1 = i;
    }
...
function isTagAttrib(type, name) {
  ...
  if (type.toLowerCase() == "applet") {
    ...
    for (i = 0; i < attribs.length; i++) {
      if (name.toLowerCase() == attribs[i]) return true;
    }
...
function fixArchiveValues(list) {
  if (pssessionid == undefined || pssessionid.length == 0) return list;
  var results = [];
  ...
  for (i = 0; i < list.length; i++) {
    ...
    if (nm == pssessionidKey) pssessionidParamExists = true;
...
function focusBannerFrame() {
  if (parent.frames.length > 0) parent.frames[0].focus();
}
...
function stopAllApplets() {
  if (is_firefox) {
    ...
    for (i = 0; i < len; i++) {
      if (embeds[i] != null) embeds[i].stop();
    }
  }
}
...
function _ignoreMultipleSubmit() {
  if (window.opener !== null && window.opener.ps_CloseMe) return true;
  if (_formAlreadySubmitted == true) return false;
...
function saveFile(binaryURL, fileType, fileName) {
  if (is_dce) {
    ...
    else window.open(binaryURL, "_blank", "width=700,height=500,resizable=yes");
  } else window.open(binaryURL, "_blank", "width=700,height=500,resizable=yes");
}
