(function ($) {
  ...
  function tax_add_tag(theTreeDivId, id, name, skip_do_checkbox) {
    ...
    if (not_in_array || not_in_tags) {
      ...
      var del_link =
        ...
        db_id +
        "'" +
        ',false);return false;"><b>X</b></a>';
      var s =
        ' <span class="taxtag" id="' +
        visual_tag_id +
        '">' +
        name +
        " " +
    ...
    if (not_in_array && not_in_tags) {
      $.get("/Rhythmyx/taxonomy/xmlprovider.htm", {
        ids: db_id,
        action: "setInUse",
      });
    }
  ...
  function tax_autocomplete_rebuild(theTreeDivId) {
    var the_url =
      "/Rhythmyx/taxonomy/xmlprovider.htm?action=getAutocompleteSearch&langID=1&taxID=" +
      tax_current_tax_id(theTreeDivId) +
      "&prefix=treenode_&exclude_ids=" +
      tax_get_init_ids_as_string(theTreeDivId);
      ...
      .jstree({
        xml_data: {
          ajax: {
            ...
            async: true,
            url:
              "/Rhythmyx/taxonomy/xmlprovider.htm?action=getXMLJSTreeLazy&exclude_disabled=true&prefix=treenode_&langID=1&taxID=" +
              tax_id +
              "&already_picked_node_ids=" +
              tax_get_init_ids_as_string(theTreeDivId),
            data: function (n) {
              return {
        ...
        search: {
          ...
          ajax: {
            cache: false,
            url:
              "/Rhythmyx/taxonomy/xmlprovider.htm?action=getJSTreeSearch&prefix=treenode_&exclude_disabled=true&langID=1&taxID=" +
              tax_id,
            data: function (str) {
              return {
                q: str,
  ...
  var methods = {
    tax_create_accordion: function (theTreeDivId, tax_id) {
      tax_create_accordion(theTreeDivId, tax_id);
    },
    tax_create_tree: function (theTreeDivId, tax_id) {
      tax_create_tree(theTreeDivId, tax_id);
    },
    tax_tree_search: function (theTreeDivId) {
      tax_tree_search(theTreeDivId);
    },
    tax_remove_tag: function (theTreeDivId, id, skip_do_checkbox) {
      tax_remove_tag(theTreeDivId, id, skip_do_checkbox);
    },
  };
