...
function createWorkflowDropDown() {
  ...
  var url =
    "/Rhythmyx/contentui/aa?action=GetWorkFlowActions" +
    '&objectId=["2","' +
    contentId +
    ...
    success: function (data, status, xhr) {
      ...
      for (i = 0; i < data.length; i++) {
        ...
        entry.on("mouseover", function () {
          $(this).addClass("perc-entry-over");
        });
        entry.on("mouseout", function () {
          $(this).removeClass("perc-entry-over");
        });
...
function doWorkflowAction(action) {
  ...
  if (hasComment || hasAdhoc) {
    ...
    doWorkflowActionCall(action, null, null, function () {
      self.close();
    });
  }
}
...
function doWorkflowActionCall(action, comment, adhocusers, callback) {
  ...
  if (adhocusers != null) data.adHocUsers = adhocusers;
  ...
  if (typeof data.operation != "undefined" && data.operation.length > 0) {
    $.ajax({
      ...
      error: function (jqXHR, status, error) {
        alert("An error occurred while attempting workflow action.");
      },
    });
  }
}
