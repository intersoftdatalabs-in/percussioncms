...
function addFormRedirect(myform) {
  if (myform.sys_currentview) {
    if (
      (myform.sys_currentview.value == "sys_All" ||
        myform.sys_currentview.value == "sys_Content" ||
        myform.sys_currentview.value == "sys_ItemMeta" ||
        myform.sys_currentview.value.indexOf("sys_SingleField") != -1) &&
      myform.sys_pageid.value == 0
    ) {
      ...
      if (
        myform.psredirect.value == "" &&
        (currenturl.indexOf("rc_createnew=yes") != -1 ||
          currenturl.indexOf("sys_folderid=") != -1)
      ) {
        var sysContentId = "";
        var sysRevision = "";
...
function buildRedirectURL(conid, rev) {
  ...
  if (currenturl.indexOf("rc_createnew=yes") != -1) {
    ...
    redirecturl =
      "../sys_rcSupport/updaterelateditems.html?" +
      "sys_activeitemid=" +
      sys_activeitemid +
      "&sys_contentid=" +
      sys_contentid +
    ...
    redirecturl =
      "../sys_cxSupport/redirectrefresh.html?" +
      "sys_folderid=" +
      sys_folderid +
      "&closeWindow=no&refreshHint=Selected";
  } else {
    redirecturl =
      "../sys_uiSupport/redirect.html?sys_revision=" +
      rev +
      "&sys_contentid=" +
      conid;
  }
...
function parseParam(param, href) {
  var value = "";
  if (param == null || param == "") return value;
