...
TrRequestQueue.prototype._addRequestToQueue = function (
  ...
  try {
    var dtsRequestEvent = new TrXMLRequestEvent(
      ...
      null
    ); // no xmlhttp object at this time
...
TrRequestQueue.prototype._isMultipartForm = function (actionForm) {
  ...
  if (actionForm.enctype.toLowerCase() != "multipart/form-data") return false;
      ...
      if (input.name && !input.disabled) {
        if (input.options) {
          ...
          for (var j = 0; j < input.options.length; j++) {
            ...
            if (option.selected) {
              var optionValue =
                option.value === null ? option.text : option.value;
    ...
    if (paramValue != null) {
      ...
      if (paramValue.join) {
        ...
        for (var i = 0; i < array.length; i++) {
          content = TrRequestQueue._appendUrlFormEncoded(
            content,
            key,
            array[i]
          );
...
TrRequestQueue._appendUrlFormEncoded = function (buffer, key, value) {
  if (buffer.length > 0) {
    buffer = buffer + "&";
  }
...
TrRequestQueue.prototype.sendMultipartRequest = function (
  ...
  this._addRequestToQueue(
    TrRequestQueue._MULTIPART_TYPE,
    privateContext,
    null,
    actionURL
  );
...
TrRequestQueue.prototype._doXmlHttpRequest = function (requestItem) {
  ...
  if (headerParams != null) {
    for (var headerName in headerParams) {
      ...
      if (currHeader["join"]) currHeader = currHeader.join(",");
...
TrRequestQueue.prototype._doRequestThroughIframe = function (requestItem) {
  ...
  var hiddenFrame = domDocument.getElementById(frameName),
    iframeDoc;
...
TrRequestQueue.prototype._appendParamNode = function (
  domDocument,
  form,
  name,
  value
...
TrRequestQueue.prototype._handleIFrameLoad = function () {
  ...
  try {
    if (
      ...
      !iframeDoc.documentElement.firstChild ||
      (agentIsIE && iframeDoc.readyState != "complete")
    ) {
      this._window.setTimeout(this._iframeLoadCallback, 50);
    } else {
      this._onIFrameLoadComplete(
        iframeDoc,
        this._dtsContext,
...
TrRequestQueue.prototype._onIFrameLoadComplete = function (
  ...
  try {
    var dtsRequestEvent = new TrIFrameXMLRequestEvent(iframeDoc);
    ...
    //cleanup
    if (iframeDoc.firstChild) iframeDoc.removeChild(iframeDoc.firstChild);
    this._htmlForm.action = this._savedActionUrl;
    this._htmlForm.target = this._savedTarget;
...
TrRequestQueue.prototype._handleRequestCallback = function (xmlHttp) {
  var httpState = xmlHttp.getCompletionState();

  if (httpState != TrXMLRequest.COMPLETED) return;
 ...
 */
TrRequestQueue.prototype.addStateChangeListener = function (
  listener,
  instance
) {
 ...
 * @param {object} instance to pass as this when calling function
 */
TrRequestQueue.prototype.removeStateChangeListener = function (
  listener,
  instance
) {
  // assertFunction(listener);
  ...
  if (stateChangeListeners) {
    ...
    for (var i = 0; i < listenerCount; i++) {
      try {
        ...
        if (currInstance != null) currListener.call(currInstance, state);
        else currListener(state);
      } catch (e) {
...
TrRequestQueue._alertError = function () {
  // TODO: get translated connection information
  var failedConnectionText = TrRequestQueue._getFailedConnectionText();
  if (failedConnectionText != null) alert(failedConnectionText);
};
