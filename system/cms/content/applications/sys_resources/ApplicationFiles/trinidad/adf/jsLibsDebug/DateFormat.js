      ...
      case "G": // era designator
        {
          ...
          appendString =
            time.getTime() < _getADEra().getTime() ? eras[0] : eras[1];
        }
        break;
      ...
      case "K": // hour in am/pm (0-11)
        {
          ...
          if (hour != null) {
            if (formatType == "h" && hour == 12) hour = 0;
            if (formatType == "k" && hour == 24) hour = 0;
      ...
      case "y": // year
        {
          ...
          if (year != null) {
            if (enteredChars > 2 && charCount <= 2 && year <= 999) {
              ...
              if (enteredChars == 3) return false;
              if (enteredChars <= 2) year = _fix2DYear(year);
            }
      ...
      case "z": // GMT timezone - "GMT Sign Hours : Minutes"
        {
          ...
          if (parseContext.parseString.length - parseContext.currIndex > 0) {
            ...
            if (
              parseContext.parseString.length - parseContext.currIndex < 2 ||
...
function _matchText(parseContext, text) {
  ...
  if (parseMatch != textMatch) return false;
...
function _accumulateNumber(parseContext, maxLength) {
  ...
  if (parseLength > currIndex + maxLength) parseLength = currIndex + maxLength;
...
function _getPaddedNumber(number, minDigits, maxDigits) {
  ...
  if (maxDigits != null) {
    ...
    if (extraDigits > 0) {
      stringNumber = stringNumber.substring(
        extraDigits,
 ...
 */
function TrDateTimeConverter(pattern, locale, exampleString, type, messages) {
  // for debugging
...
TrDateTimeConverter.prototype.getFormatHint = function () {
  ...
  if (this._messages && this._messages["hint"]) {
    ...
    var key =
      ...
      this._type +
      "_HINT";
    return TrMessageFactory.createMessage(key, "" + this._exampleString);
  }
};
...
TrDateTimeConverter.prototype.getAsString = function (formatTime) {
  ...
  if (typeof pattern != "string") pattern = pattern[0];
...
TrDateTimeConverter.prototype.getAsObject = function (parseString, label) {
  ...
  if (parseString.length == 0) return null;
  ...
  if (typeof pattern == "string") {
    return this._simpleDateParseImpl(
      parseString,
      pattern,
      this._localeSymbols,
      facesMessage
    ...
    for (i = 0; i < pattern.length; i++) {
      try {
        var date = this._simpleDateParseImpl(
          parseString,
          pattern[i],
          this._localeSymbols,
          facesMessage
        ...
        if (i == pattern.length - 1) throw e;
      }
    }
  }
};

TrDateTimeConverter.prototype._initPatterns = function (pattern) {
  ...
  if (pattern) patterns = patterns.concat(pattern);
...
TrDateTimeConverter.prototype._simpleDateParseImpl = function (
  ...
  if (
    ...
    if (month != null) parsedTime.setMonth(month);

    // Set the parsed day-of-month, if any
    var date = parseContext.parsedDate;
    if (date != null) parsedTime.setDate(date);
    ...
    if (minutes != null) parsedTime.setMinutes(minutes);

    // Set the parsed seconds, if any
    var seconds = parseContext.parsedSeconds;
    if (seconds != null) parsedTime.setSeconds(seconds);

    // Set the parsed milliseconds, if any
    var milliseconds = parseContext.parsedMilliseconds;
    if (milliseconds != null) parsedTime.setMilliseconds(milliseconds);
