function TrIFrameXMLRequestEvent(a0) {
  this._iframeDoc = a0;
}
...
TrIFrameXMLRequestEvent.prototype.getResponseXML = function () {
  ...
  if (a1 && a2.XMLDocument) return a2.XMLDocument;
  else return a2;
};
TrIFrameXMLRequestEvent.prototype.getResponseText = function () {
  ...
  var a4 = this._iframeDoc,
    xmlDocument = null;
  if (a3 && a4.XMLDocument) xmlDocument = a4.XMLDocument;
  else if (window.XMLDocument && a4 instanceof XMLDocument) xmlDocument = a4;
  if (xmlDocument) return AdfAgent.AGENT.getNodeXml(xmlDocument);
  else return a4.documentElement.innerHTML;
};
TrIFrameXMLRequestEvent.prototype._isResponseValidXML = function () {
  var a5 = _agent.isIE;
  var a6 = this._iframeDoc;
  if (a5 && a6.XMLDocument) return true;
  else if (window.XMLDocument && a6 instanceof XMLDocument) return true;
  else return false;
};
...
TrIFrameXMLRequestEvent.prototype.isPprResponse = function () {
  ...
  if (a7 && a8.XMLDocument) {
    var a10 = a8.XMLDocument,
      childNodes = a10.childNodes;
    if (
      childNodes.length >= 2 &&
