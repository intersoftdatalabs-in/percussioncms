  ...
  if (swatch != null) {
    ...
    if (value != null) {
      if (value.alpha == 0) {
        ...
        swatch.style.backgroundColor = new TrColorConverter(
          "#RRGGBB"
        ).getAsString(value);
    ...
    // on Mozilla
    if (_agent.isGecko) swatch.title = swatch.alt;
  }
}

function _returnColorPickerValue(closingWindow, event) {
  ...
  if (closingWindow.isApplicable) _cfUpdate(colorField, newColor);
}

/*
 * ColorField Bus Select (used by ColorPalette)
 */
function _cfbs(event) {
  _cfUpdate(_cfBus[event.source], event.params.value);
}

function _cfUpdate(colorField, newColor) {
  if (colorField != null) {
    //
    ...
    if (newValue == _cfTrans && !cff._allowsTransparent) return;
    ...
    if (newValue != colorField.value) {
      ...
      if (colorField.onchange != null) {
        ...
        if (_agent.isIE) {
          ...
          if (!_agent.isNav) _cfsw(colorField);
 ...
 */
function _lcp(formName, nameInForm, destination) {
  ...
  if (colorField.value != "") {
    ...
    try {
      ...
      if (color != null) {
        ...
        if (color.alpha == 0) destination += escape(_cfTrans);
        else
          destination += escape(
            new TrColorConverter(pattern).getAsString(color)
          );
      }
    } catch (e) {
      // no-op
