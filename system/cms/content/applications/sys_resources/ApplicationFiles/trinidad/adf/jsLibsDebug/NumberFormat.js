...
function TrNumberFormat(type) {
  if (!type) alert("type for TrNumberFormat not defined!");
  this._type = type;
...
TrNumberFormat.prototype.format = function (number) {
  if (this._type == "percent") return this.percentageToString(number);
  else if (this._type == "currency") return this.currencyToString(number);
  else if (this._type == "number") return this.numberToString(number);
};
...
TrNumberFormat.prototype.parse = function (string) {
  if (this._type == "percent") return this.stringToPercentage(string);
  else if (this._type == "currency") return this.stringToCurrency(string);
};
...
TrNumberFormat.prototype.stringToCurrency = function (numberString) {
  ...
  if (negP != -1) {
    ...
    if (negS != -1) {
      numberString = numberString.substr(
        0,
        numberString.length - this._nSuf.length
      );
    ...
    if (posP != -1) {
      ...
      if (posS != -1) {
        numberString = numberString.substr(
          0,
          numberString.length - this._pSuf.length
        );
...
TrNumberFormat.prototype.numberToString = function (number) {
  ...
  if (fracs != "") numberString = ints + decimalSeparator + fracs;
  else numberString = ints;

  if (negative) numberString = "-" + numberString;
 ...
 */
function TrParseException(message) {
