...
TrPanelPopup.showPopup = function (contentId, triggerId, props, event) {
  ...
  if (!popup) popup = TrPanelPopup.POPUP = new TrPanelPopup();
  else popup.hide();
  ...
  if (props) {
    if (props["modal"]) popup.setModal(true);

    if (props["center"]) popup.setCentered(true);
...
TrPanelPopup.prototype._handleEvent = function (event) {
  ...
  while (currElement) {
    ...
    if (currElement == this.getContent() || currElement == this.getTrigger()) {
      break;
    }
