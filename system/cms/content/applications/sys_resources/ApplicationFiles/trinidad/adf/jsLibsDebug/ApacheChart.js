...
ApacheChartEvent.prototype.marshall = function () {
  ...
  if (this._xValues) value.push("xValues\t" + this._xValues.join("\t"));
...
ApacheChart.createSVG = function (
  containerId,
  svgEmbedId,
  sourceUrl,
  inlineStyle,
  ...
  if (agent && agent.isIE) {
    ...
    if (semiColIndex != -1) sourceUrl = sourceUrl.substr(0, semiColIndex);
  }
...
ApacheChart.prototype.Init = function (
  type,
  model,
  svgEmbedId,
  isPerspective,
...
ApacheChart.prototype.ComputeMinMaxValues = function () {
  var model = this._model,
    ...
    minYValue = model.getMinYValue(),
    minXValue = model.getMinXValue(),
  ...
  if (yValues != null && (maxYValue == null || minYValue == null)) {
    ...
    if (maxYValue == null) model.setMaxYValue(minMax.max);
    if (minYValue == null) model.setMinYValue(minMax.min);
  }
  if (xValues != null && (maxXValue == null || minXValue == null)) {
    var minMax = this._computeAxisMinMaxValues(xValues, seriesLabels.length);
    if (maxXValue == null) model.setMaxXValue(minMax.max);
    if (minXValue == null) model.setMinXValue(minMax.min);
  }
};
...
ApacheChart.prototype._computeAxisMinMaxValues = function (values, seriesSize) {
  ...
  for (var i = 0; i < groupsCount; ++i) {
    ...
    for (var j = 0; j < seriesSize; ++j) {
      value = values[i][j];
      if (isStacked) stackedTotal += value;
      else {
...
ApacheChart.createChart = function (
  ...
  if (
    type == this.TYPE_VBAR ||
    type == this.TYPE_VBAR_STACKED ||
    type == this.TYPE_BAR_LINE_COMBO
  ) {
    chart = new ApacheBarChart(
      type,
      model,
      svgEmbedId,
      isPerspective,
    ...
    chart = new ApacheScatterPlotChart(
      type,
      model,
      svgEmbedId,
      isPerspective,
    ...
    chart = new ApacheSemiGaugeChart(
      type,
      model,
      svgEmbedId,
      isPerspective,
...
ApacheChart.prototype.SetDataAnimStep = function (ratio) {
  var animElems = this._dataElems,
    animCount = animElems.length;
  ...
  if (animHorizontal) {
    ...
    for (var i = 0; i < animCount; ++i) {
      ...
      animElems[i].setAttribute(
        "transform",
        "translate(" + tx + ",0) scale(" + ratio + ",1)"
      );
    }
  } else {
    var marginBottom = margins.bottom,
      cy = this._height - marginBottom;

    for (var i = 0; i < animCount; ++i) {
      var ty = (1 - ratio) * cy;
      animElems[i].setAttribute(
        "transform",
        "translate(0," + ty + ") scale(1," + ratio + ")"
      );
    }
  }
};
...
ApacheChart.prototype.SetLabelsAnimStep = function (ratio) {
  var animElems = this._labelElems,
    animCount = animElems.length;
...
ApacheChart.prototype.InitializeGradients = function () {
  ...
  ApacheChartObj.Assert(
    gradients.childNodes.length > 1,
    "No Gradient Template in the SVG document"
  );
  var gradientTemplate = gradients.childNodes[0],
    gradientElement;
...
ApacheChart.prototype._getLighterColor = function (color) {
  if (color.indexOf("#") >= 0) {
    ...
    var rVal = color.substr(0, 2),
      gVal = color.substr(2, 2),
      bVal = color.substr(4);
    color =
      "#" +
      this._getLighterNumberStr(rVal) +
    ...
    color =
      "#" +
      this._getLighterNumberStr(arr[0]) +
      this._getLighterNumberStr(arr[1]) +
...
ApacheChart.prototype.DrawBorder = function () {
  ...
  var borderSize = ApacheChart._BORDER_SIZE,
    stroke = borderSize / 2;
...
ApacheChart.prototype._drawTitleElem = function (template, title, isFooter) {
  var svgDoc = this._svgDoc,
    rootElem = this._rootElement;
  var margins = this._margins,
    gridWidth = this._width - margins.left - margins.right;
  var labelElems = this._labelElems,
...
ApacheChart.prototype.LayoutGroupLabels = function () {
  ...
  var groupLabels = model.getGroupLabels(),
    vLineCount = groupLabels.length;
  var labelElem,
    groupWidth = gridWidth / (isCenterAligned ? vLineCount : vLineCount - 1);
  var dx = 0,
    dy =
      this._height -
      margins.bottom +
      container.getBBox().height +
  ...
  for (var i = 0; i < vLineCount; ++i) {
    ...
    if (isCenterAligned) {
      if (groupWidth > textWidth) dx = (groupWidth - textWidth) / 2;
      else dx = 2;
    } else {
      dx = -textWidth / 2;
      if (this._isPerspective) dx -= ApacheChart._XOFFSET_PERSPECTIVE;
    }
...
ApacheChart.prototype.AdjustMarginsForGroupLabels = function () {
  ...
  if (container && container.childNodes.length > 0) {
    this._margins.bottom +=
      container.getBBox().height + ApacheChart._TEXT_MARGIN;
...
ApacheChart.prototype.DrawLegend = function () {
  ...
  var dx = 0,
    dy = 0,
    tx = marginLeft,
    ty = this._height - margins.bottom;
  var drawSideWays =
    legendPosition == ApacheChart.LEGEND_LOCATION_START ||
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    if (drawSideWays) dx = 0;

    rectElem = rectElem.cloneNode(false);
    rectElem.setAttribute("x", dx);
    rectElem.setAttribute("y", dy - legendRectHeight);
    if (gradientsUsed) rectElem.setAttribute("fill", "url(#gradient" + i + ")");
    else rectElem.setAttribute("fill", seriesColors[i]);
  ...
  if (!drawSideWays && gridWidth > dx) tx = (gridWidth - dx) / 2 + marginLeft;
  ...
  if (drawSideWays) {
    ...
    if (gridHeight > dy) ty = (gridHeight - lBBox.height) / 2 + marginTop;
    else ty = gridHeight + marginTop - lBBox.height;
  }
...
ApacheChart.prototype.Draw2DGrid = function () {
  ...
  if (gradientsUsed) rectElem.setAttribute("fill", "url(#gridGradient)");
  ...
  for (var i = 0; i < hLineCount - 1; ++i) {
    sb.append("M")
      .append(marginLeft)
      .append(",")
      .append(((i + 1) * gridHeight) / hLineCount + marginTop);
  ...
  for (var i = 0; i < vLineCount - 1; ++i) {
    sb.append("M")
      .append(marginLeft + ((i + 1) * gridWidth) / vLineCount)
      .append(",")
      .append(marginTop);
...
ApacheChart.prototype.GetVLineCount = function () {
  ...
  if (xMajorCount >= 0) return xMajorCount;
...
ApacheChart.prototype.DrawPerspectiveGrid = function () {
  ...
  var gridElems = this._gridElems,
    animate = this._animDuration > 0;
  var xOffset = ApacheChart._XOFFSET_PERSPECTIVE,
  ...
  var sb = new ApacheChartBuffer();
  var pathElem = svgDoc
    .getElementById("gridPath3dRectPrototype")
    .cloneNode(false);
  sb.append("M")
    .append(marginLeft + xOffset)
    .append(",")
    .append(marginTop);
  ...
  sb.append("l").append(-xOffset).append(",").append(yOffset);
  sb.append("h").append(-gridWidth);
  if (gradientsUsed) pathElem.setAttribute("fill", "url(#gridGradient)");
  pathElem.setAttribute("d", sb.toString());
  pathElem.removeAttribute("id");
  this._rootElement.appendChild(pathElem);
  ...
  var vLineCount = this.GetVLineCount(),
    hLineCount = this.GetHLineCount();
  ...
  for (var i = 0; i < hLineCount - 1; ++i) {
    sb.append("M")
      .append(marginLeft)
      .append(",")
      .append(((i + 1) * gridHeight) / hLineCount + marginTop + yOffset);
    sb.append("l").append(xOffset).append(",").append(-yOffset);
    sb.append("h").append(gridWidth);
  }
  ...
  for (var i = 0; i < vLineCount - 1; ++i) {
    sb.append("M")
      .append(marginLeft + xOffset + ((i + 1) * gridWidth) / vLineCount)
      .append(",")
      .append(marginTop);
    sb.append("v").append(gridHeight);
    sb.append("l").append(-xOffset).append(",").append(yOffset);
...
ApacheChart.prototype.DrawYValueLabels = function () {
  ...
  var minValue = model.getMinYValue(),
...
ApacheChart.prototype.LayoutYValueLabels = function () {
  ...
  var container = this._vLabelContainer,
    childNodes = container.childNodes;
  var gridHeight = this._height - marginTop - margins.bottom;
  if (this._isPerspective) gridHeight -= ApacheChart._YOFFSET_PERSPECTIVE;
...
ApacheChart.prototype.SetVerticalLabelAt = function (
  ...
  if (this._isPerspective) y += ApacheChart._YOFFSET_PERSPECTIVE;
...
ApacheChart.prototype.DrawGroupLabelTitle = function (
  ...
  container.appendChild(labelElem);
  var textWidth = labelElem.getComputedTextLength();
  if (quadWidth > textWidth) dx += (quadWidth - textWidth) / 2;
  else dx += 2;
  labelElem.setAttribute("x", dx);

  if (animate) labelElems.push(labelElem);
...
ApacheChart.prototype.FillToolTipData = function (
  boundingRectElem,
  circleElem,
  e
) {
  ...
  var model = this._model,
    groupLabels = model.getGroupLabels(),
    seriesLabels = model.getSeriesLabels(),
  ...
  var xMargin = ApacheChart._TEXT_MARGIN,
    dx = xMargin;
  if (labelWidth > dataWidth) dx = (labelWidth - dataWidth) / 2 + xMargin;
...
ApacheChart.prototype.HideToolTip = function (e) {
  var toolTip = this.getToolTip();
  if (toolTip) toolTip.style.setProperty("visibility", "hidden", "");
  this._toolTipVisible = false;
};
...
ApacheBarChart.prototype.DrawChartData = function () {
  ...
  if (isPerspective) this._drawPerspectiveBars(isCombo);
  else this._drawBars(isCombo);
...
ApacheBarChart.prototype._drawBars = function (isCombo) {
  ...
  var groupLabels = model.getGroupLabels(),
    ...
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var seriesColors = model.getSeriesColors(),
    yValues = model.getYValues();
  var minValue = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var barDivider = isStacked
    ? 1
    : isCombo
    ? Math.ceil(seriesCount / 2)
    : seriesCount;
  var yValueCount = yValues.length;
  var barWidth =
    (gridWidth / Math.max(yValueCount, groupCount) - 2 * barItemPadding) /
    barDivider;
  var dx = marginLeft,
    dy,
    barHeight,
  ...
  for (var i = 0; i < yValueCount; ++i) {
    ...
    for (var j = 0; j < seriesCount; ++j) {
      ...
      rectElem.setAttribute("x", dx);
      barHeight =
        (gridHeight * (yValues[i][j] - stackBase)) / (maxValue - minValue);
      if (isStacked) dy -= barHeight;
      else dy = gridHeight + marginTop - barHeight;
      ...
      else rectElem.setAttribute("fill", seriesColors[j]);
...
ApacheBarChart.prototype._drawPerspectiveBars = function (isCombo) {
  ...
  var minValue = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var seriesBars = isCombo ? Math.ceil(seriesCount / 2) : seriesCount,
    barWidth;
  var yValueCount = yValues.length;
  if (isStacked)
    barWidth =
    ...
    barWidth =
      (gridWidth / Math.max(yValueCount, groupCount) -
        2 * barItemPadding -
        seriesBars * barItemPadding) /
      seriesBars;
  var dx = marginLeft,
    dy,
    barHeight,
  ...
  for (var i = 0; i < yValueCount; ++i) {
    ...
    for (var j = 0; j < seriesCount; ++j) {
      ...
      barHeight =
        (gridHeight * (yValues[i][j] - stackBase)) / (maxValue - minValue);
      if (isStacked) dy -= barHeight;
      else dy = gridHeight + yOffset + marginTop - barHeight;
      ...
      else pathElem.setAttribute("fill", seriesColors[j]);
      pathElem.setAttribute("d", sb.toString());
...
ApacheHBarChart.prototype.LayoutYValueLabels = function () {
  ...
  for (var i = 0; i < hLineCount; ++i) {
    ...
    this.SetVerticalLabelAt(
      labelElem,
      ((hLineCount - i) * gridHeight) / hLineCount +
        marginTop -
        gridHeight / (2 * hLineCount),
      marginLeft,
      textHeight
    );
  }
...
ApacheHBarChart.prototype.DrawGroupLabels = function () {
  ...
  for (var i = 0; i < vLineCount + 1; ++i) {
    ...
    else if (i == vLineCount) value = maxValue;
    else value = ((maxValue - minValue) * i) / vLineCount + minValue;
    labelElem.firstChild.data = this._formatValue(value);
    container.appendChild(labelElem);
  }
  rootElem.appendChild(container);
...
ApacheHBarChart.prototype.LayoutGroupLabels = function () {
  ...
  if (this._isPerspective) gridWidth -= ApacheChart._XOFFSET_PERSPECTIVE;

  var vLineCount = this._yMajorGridCount;
  var labelElem,
    yValWidth = gridWidth / vLineCount;

  var bBox = container.getBBox();
  var dx = 0,
    dy = this._height - margins.bottom + bBox.height + ApacheChart._TEXT_MARGIN;
  var labelElems = this._labelElems,
    animate = this._animDuration > 0;
  for (var i = 0; i < vLineCount + 1; ++i) {
    // draw the horizontal label
...
ApacheHBarChart.prototype._drawBars = function () {
  ...
  var minValue = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var barDivider = isStacked ? 1 : seriesCount,
    stackBase = minValue;
  var yValueCount = yValues.length;
  var barHeight =
    (gridHeight / Math.max(yValueCount, groupCount) - 2 * barItemPadding) /
    barDivider;
  var dx = marginLeft,
    dy = gridHeight + marginTop,
    barWidth;
  ...
  for (var i = 0; i < yValueCount; ++i) {
    ...
    for (var j = 0; j < seriesCount; ++j) {
      ...
      rectElem.setAttribute("x", dx);
      barWidth =
        (gridWidth * (yValues[i][j] - stackBase)) / (maxValue - minValue);
      if (isStacked) dx += barWidth;
...
ApacheHBarChart.prototype._drawPerspectiveBars = function () {
  ...
  if (isStacked)
    ...
    barHeight =
      (gridHeight / Math.max(yValueCount, groupCount) -
        2 * barItemPadding -
        seriesCount * barItemPadding) /
      seriesCount;
  var dx = marginLeft,
    dy = gridHeight + marginTop + yOffset,
    barWidth;
...
ApachePieChart.prototype.Init = function (
  ...
  ApachePieChart.superclass.Init.call(
    this,
    type,
    model,
    svgEmbedId,
...
ApachePieChart.prototype.DrawChartData = function () {
  ...
  var nCols = Math.ceil(Math.sqrt(yValueCount)),
  ...
  var animate = this._animDuration > 0,
    isPerspective = this._isPerspective;
  var pieAnimRadii,
    vGap = 2 * ApacheChart._TEXT_MARGIN;
  var quadHeight =
    (this._height - margins.top - margins.bottom - (nRows - 1) * vGap) / nRows;
  if (animate) {
    this._pieAnimAngles = [];
  ...
  for (var i = 0; i < nRows; ++i) {
    for (var j = 0; j < nCols; ++j) {
      ...
      var newHeight = this.DrawGroupLabelTitle(
        groupLabel,
        rootElem,
        labelElem.cloneNode(true),
        dx,
        dy,
        quadWidth,
        quadHeight
      ...
      var cx = dx + quadWidth / 2 + ApacheChart._TEXT_MARGIN,
        cy = dy + newHeight / 2;
...
ApachePieChart.prototype.SetDataAnimStep = function (ratio) {
  var pieAnimRadii = this._pieAnimRadii,
    pieAnimAngles = this._pieAnimAngles,
    isPerspective = this._isPerspective,
    agleIndex = 0,
    elemIndex = 0;
  var animElems = this._dataElems,
    chartCount = pieAnimRadii.length;
  var model = this._model,
    yValues = model.getYValues();
  ...
  for (var i = 0; i < chartCount; ++i) {
    ...
    for (var j = 0; j < nPies; ++j) {
      ...
      animElems[elemIndex++].setAttribute(
        "transform",
        "translate(" + tx + "," + ty + ")"
      );
      if (isPerspective) {
...
ApachePieChart.prototype._drawPies = function (
  ...
  var pathElem = svgDoc.getElementById("piePathPrototype"),
    pieStart = 0,
    animAngleStart = 0;
  var pieElems = new Array(nPies),
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var gradientsUsed = this._gradientsUsed;
  var defaultTransform = "translate(-10000, -10000)",
    pieAnimAngles = this._pieAnimAngles;
  for (var i = 0; i < nPies; ++i) {
    pathElem = pathElem.cloneNode(false);
    ...
    if (valueRatio >= 0.5) {
      ...
      sb.append(",").append(
        pieSize * Math.sin((pieStart + valueRatio) * Math.PI * 2)
      );
    } else {
      sb.append("A")
        .append(pieSize)
    ...
    pathElem.setAttribute("d", sb.toString());
    if (gradientsUsed) pathElem.setAttribute("fill", "url(#gradient" + i + ")");
    else pathElem.setAttribute("fill", seriesColors[i]);
...
ApachePieChart.prototype._draw3DPies = function (
  ...
  var perspectiveHeight = pieSize / 4,
    pieElems = new Array(nPies),
    ringElems = new Array(nPies),
    edgeElems = new Array(nPies);
  var dataElems = this._dataElems,
    animate = this._animDuration > 0;
  ...
  for (var i = 0; i < nPies; ++i) {
    ...
    if (valueRatio >= 0.5) {
      ...
      sb.append(arcBeginX)
        .append(",")
        .append(arcBeginY + perspectiveHeight);
    } else {
      sb.append("A")
        .append(pieSize)
        .append(" ")
  ...
  var totalRatio = 0;
  for (var i = 0; i < nPies; ++i) {
    if (totalRatio <= 0.5) pieContainer.appendChild(ringElems[i]);
    totalRatio += yValues[iGroup][i] / pieTotal;
  }
  totalRatio = 0;
  for (var i = 0; i < nPies; ++i) {
    if (totalRatio <= 0.5) pieContainer.appendChild(edgeElems[i]);
    totalRatio += yValues[iGroup][i] / pieTotal;
  }
...
ApachePieChart.prototype.GetToolTipLocation = function (e, ttBBox) {
  ...
  return {
    x: ctm.e + targetBBox.x + targetBBox.width / 2,
...
ApacheAreaChart.prototype.Init = function (
  ...
  ApacheAreaChart.superclass.Init.call(
    this,
    type,
    model,
    svgEmbedId,
...
ApacheAreaChart.prototype.SetDataAnimStep = function (ratio) {
  ...
  var newRatio = ratio * seriesCount,
    animSeriesIndex = 0;
  if (newRatio > 1) {
  ...
  for (var j = 0; j < animPathCount; ++j) {
    ...
    animElems[i * animPathCount + j].setAttribute(
      "transform",
      "translate(0," + ty + ") scale(1," + newRatio + ")"
    );
    if (i > 0) {
      animElems[(i - 1) * animPathCount + j].setAttribute(
        "transform",
        "scale(1,1)"
      );
    }
...
ApacheAreaChart.prototype._drawAreas = function () {
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    var sb = new ApacheChartBuffer();
    pathElem = pathElem.cloneNode(false);
    if (i == 0 || !isStacked) sb.append("M").append(dx).append(",").append(dy);
    else if (isStacked) sb.append("M").append(dx).append(",").append(cumYs[0]);
    ...
    for (var j = 0; j < yValueCount; ++j) {
      if (isStacked) {
        if (null == cumYs[j]) cumYs[j] = gridHeight + marginTop;

        dy = cumYs[j] -=
          (gridHeight * (yValues[j][i] - stackBase)) / (maxValue - minValue);
      } else
        dy =
          gridHeight +
          marginTop -
      ...
      if (j != yValueCount - 1) dx += barWidth;
    }
    if (animate) {
      dataElems.push(pathElem);
      pathElem.setAttribute("transform", defaultTransform);
...
ApacheAreaChart.prototype._drawPerspectiveAreas = function () {
  ...
  var barWidth = gridWidth / (Math.max(yValueCount, groupCount) - 1),
    stackBase;
  var gridBottom = gridHeight + marginTop + yOffset,
    dx,
    dy,
    cumYs = [];
  var gradientsUsed = this._gradientsUsed;
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    for (var j = 0; j < yValueCount; ++j) {
      if (isStacked) {
        if (null == cumYs[j]) cumYs[j] = gridBottom;
      ...
      if (j != yValueCount - 1) {
        ...
        var nextdy,
          nextdx = dx + barWidth;
        if (isStacked) {
          if (null == cumYs[j + 1]) cumYs[j + 1] = gridBottom;

          nextdy =
            cumYs[j + 1] -
            (gridHeight * (yValues[j + 1][i] - stackBase)) /
              (maxValue - minValue);
        } else
          nextdy =
            gridBottom -
            (gridHeight * (yValues[j + 1][i] - minValue)) /
              (maxValue - minValue);
        sb.append("L")
          .append(nextdx + xOffset)
          .append(",")
          .append(nextdy - yOffset);
        sb.append("l").append(-xOffset).append(",").append(yOffset);
        sb.append("L").append(dx).append(",").append(dy);
        ...
        if (i == 0 || !isStacked) {
          ...
          sb.append("L")
            ...
            .append(
              cumYs[j] +
                (gridHeight * (yValues[j][i] - stackBase)) /
                  (maxValue - minValue)
            );
        }
        sb.append("L").append(dx).append(",").append(dy);
      ...
      if (clientX > dx && clientX < dx + barWidth) {
        if (isStacked) {
          ...
          if (clientY >= dy) {
            value =
              yValues[j][i] +
              ((yValues[j + 1][i] - yValues[j][i]) * (clientX - dx)) / barWidth;
          ...
          dy =
            dy1 -
            (((gridHeight * (yValues[j + 1][i] - yValues[j][i])) /
              (maxValue - minValue)) *
              (clientX - dx)) /
              barWidth;
...
ApacheAreaChart.prototype.ShowToolTip = function (e) {
  ...
  this._displayToolTips(
    chartEvent.getYValues(),
    chartEvent.getSeriesIndices(),
    seriesYs,
    e
  );
};
...
ApacheAreaChart.prototype._displayToolTips = function (
  ...
  var model = this._model,
    ...
    seriesCount = seriesLabels.length,
    seriesColors = model.getSeriesColors();
  var tooltipCount = seriesIndices.length,
...
ApacheLineChart.prototype.DrawChartData = function (isCombo) {
  if (this._isPerspective) this.__drawPerspectiveLines(isCombo);
  else this.__drawLines(isCombo);
};
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    for (var j = 0; j < yValueCount; ++j) {
      dy =
        ...
        marginTop -
        (gridHeight * (yValues[j][i] - minValue)) / (maxValue - minValue);
      if (j == 0) sb.append("M").append(dx).append(",").append(dy);
      else sb.append("L").append(dx).append(",").append(dy);
      ...
      else lineDot.setAttribute("fill", seriesColors[i]);
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    for (var j = 0; j < yValueCount; ++j) {
      ...
      if (j != yValueCount - 1) {
        ...
        var nextdy =
          gridBottom -
          (gridHeight * (yValues[j + 1][i] - minValue)) / (maxValue - minValue);
...
ApacheLineChart.prototype.FillToolTipData = function (
  boundingRectElem,
  circleElem,
  e
) {
  ...
  var i = chartEvent.getSeriesIndices()[0],
    value = chartEvent.getYValues()[0];
  var seriesLabels = this._model.getSeriesLabels();

  var textElem = boundingRectElem.nextSibling.nextSibling;
  textElem.firstChild.data = seriesLabels[i] + ": " + this._formatValue(value);
...
ApacheScatterPlotChart.prototype.Init = function (
  ...
  ApacheScatterPlotChart.superclass.Init.call(
    this,
    type,
    model,
    svgEmbedId,
...
ApacheScatterPlotChart.prototype.SetDataAnimStep = function (ratio) {
  ...
  var gridHeight = this._height - marginTop - margins.bottom;
  var animElems = this._dataElems,
    animCount = cxs.length,
    elemIndex = 0;

  if (isPerspective) marginLeft += ApacheChart._XOFFSET_PERSPECTIVE;
...
ApacheScatterPlotChart.prototype._drawPoints = function () {
  ...
  var minYValue = model.getMinYValue(),
    maxYValue = model.getMaxYValue();
  var minXValue = model.getMinXValue(),
    maxXValue = model.getMaxXValue();
  ...
  for (var i = 0; i < seriesCount; ++i) {
    for (var j = 0; j < nValues; ++j) {
      dy =
        gridHeight +
        marginTop -
        (gridHeight * (yValues[j][i] - minYValue)) / (maxYValue - minYValue);
      dx =
        marginLeft +
        (gridWidth * (xValues[j][i] - minXValue)) / (maxXValue - minXValue);
      dotElem = dotElem.cloneNode(false);
      if (gradientsUsed)
        dotElem.setAttribute("fill", "url(#gradient" + i + ")");
      else dotElem.setAttribute("fill", seriesColors[i]);
...
ApacheScatterPlotChart.prototype.FillToolTipData = function (
  boundingRectElem,
  circleElem,
  e
) {
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    for (var j = 0; j < nValues; ++j) {
      ...
      if (j != nValues - 1) {
        ...
        nextdx =
          marginLeft +
          (gridWidth * (xValues[j + 1][i] - minXValue)) /
            (maxXValue - minXValue);
        nextdy =
          gridBottom -
          (gridHeight * (yValues[j + 1][i] - minYValue)) /
            (maxYValue - minYValue);
        sb.append("L")
          .append(nextdx + xOffset)
          .append(",")
...
ApacheXYLineChart.prototype.GetChartEvent = function (e) {
  ...
  for (var j = 0; j < nValues; ++j) {
    if (j != nValues - 1) {
      ...
      nextdx =
        marginLeft +
        (gridWidth * (xValues[j + 1][i] - minXValue)) / (maxXValue - minXValue);
      ...
      if (clientX > dx && clientX < dx + nextdx) {
        ...
        yValue =
          yValues[j][i] +
          ((yValues[j + 1][i] - yValues[j][i]) * (clientY - dy)) /
            (nextdy - dy);
        xValue =
          xValues[j][i] +
          ((xValues[j + 1][i] - xValues[j][i]) * (clientX - dx)) /
            (nextdx - dx);
        break;
      }
    }
...
ApacheXYLineChart.prototype.FillToolTipData = function (
  boundingRectElem,
  circleElem,
  e
) {
...
ApacheRadarChart.prototype.Init = function (
  ...
  ApacheRadarChart.superclass.Init.call(
    this,
    type,
    model,
    svgEmbedId,
...
ApacheRadarChart.prototype.SetDataAnimStep = function (ratio) {
  ...
  var tx = (1 - newRatio) * cx,
    ty = (1 - newRatio) * cy;
  var transform =
    "translate(" + tx + "," + ty + ") scale(" + newRatio + "," + newRatio + ")";
...
ApacheRadarChart.prototype.LayoutGroupLabels = function () {
  ...
  var groupLabels = model.getGroupLabels(),
    vLineCount = groupLabels.length;
  var labelElem,
    groupWidth = gridWidth / vLineCount;
  var container = this._hLabelContainer,
    childNodes = container.childNodes;
  ...
  for (var i = 0; i < vLineCount; ++i) {
    ...
    if (!firstLabel) {
      ...
      cy = marginTop + gridHeight / 2;
      radius =
        Math.min(gridWidth, gridHeight) / 2 +
        rect.height -
        ApacheChart._TEXT_MARGIN / 2;
      firstLabel = true;
    }
...
ApacheRadarChart.prototype.Draw2DGrid = function () {
  ...
  if (gradientsUsed) circle.setAttribute("fill", "url(#gridGradient)");
  if (animate) {
  ...
  var sb = new ApacheChartBuffer();
  var pathElem = svgDoc
    .getElementById("radarGridPathPrototype")
    .cloneNode(false);
...
ApacheRadarChart.prototype.LayoutYValueLabels = function () {
  ...
  var labelElem = svgDoc.getElementById("yLabelPrototype");
  labelElem = labelElem.cloneNode(true);
  var textHeight = this._addRadarYLabelAt(
    rootElem,
    labelElem,
    circleCount - 1,
    cx,
...
ApacheRadarChart.prototype._drawRadar = function () {
  ...
  var lineDot,
    pathElem = svgDoc.getElementById(protoName);
  var seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var seriesColors = model.getSeriesColors(),
    yValues = model.getYValues();
  ...
  if (!isRadarArea) lineDot = svgDoc.getElementById("lineDotPrototype");
  ...
  for (var i = 0; i < seriesCount; ++i) {
    ...
    for (var j = 0; j < yValueCount; ++j) {
      var yPoint =
        (radius * (yValues[j][i] - minValue)) / (maxValue - minValue);
...
ApacheRadarChart.prototype.GetChartEvent = function (e, seriesXs, seriesYs) {
  ...
  var dx1,
    ...
    seriesValues = [];
  for (var i = 0; i < seriesCount; ++i) {
    if (!isRadarArea && seriesIndex != i) continue;
    ...
    for (var j = 0; j < yValueCount; ++j) {
      ...
      if (
        this.isPointInPolygon([cx, dx1, dx2], [cy, dy1, dy2], clientX, clientY)
      ) {
        ...
        else mousePtAngle = (3 * Math.PI) / 2 + (Math.PI - mousePtAngle);
...
ApacheGaugeChart.prototype.Init = function (
  ...
  ApacheGaugeChart.superclass.Init.call(
    this,
    type,
    model,
    svgEmbedId,
...
ApacheGaugeChart.prototype.SetIndicatorPosition = function (
  yValue,
  indicator,
  ratio
) {
...
ApacheGaugeChart.prototype.DrawChartData = function () {
  ...
  for (var i = 0; i < nRows; ++i) {
    for (var j = 0; j < nCols; ++j) {
      ...
      if (groupLabel) labelElem = labelElem.cloneNode(true);
      var newHeight = this.DrawGroupLabelTitle(
        groupLabel,
        rootElem,
        labelElem,
      ...
      if (groupLabel) {
        var gBBox = gaugeContainer.getBBox(),
          gHeight = gBBox.height;
...
ApacheGaugeChart.prototype.DrawDial = function (
  ...
  var gaugeBBox = gauge.getBBox(),
    gaugeWidth = gaugeBBox.width,
    gaugeR = gaugeWidth / 2;
...
ApacheGaugeChart.prototype.CreateTextMarkerGroup = function (gauge, gaugeR) {
  ...
  for (
    var i = 0, theta = Math.PI / 6;
    i <= majorMarkerCount;
    ++i, theta += (5 * Math.PI) / 3 / majorMarkerCount
  ) {
    var adjustedTheta;
    if (theta < Math.PI / 2) adjustedTheta = theta + (3 * Math.PI) / 2;
    else adjustedTheta = theta - Math.PI / 2;
    ...
    if (theta >= (5 * Math.PI) / 6 && theta <= (7 * Math.PI) / 6) {
      ...
      y += textBBox.height / 2;
      if (theta < Math.PI) x += 2 * ApacheChart._TEXT_MARGIN;
      else x -= textBBox.width + 2 * ApacheChart._TEXT_MARGIN;
    }
  ...
  for (
    ...
    if (i % minorMarkerCount == 0) continue;
    var adjustedTheta;
    if (theta < Math.PI / 2) adjustedTheta = theta + (3 * Math.PI) / 2;
    else adjustedTheta = theta - Math.PI / 2;
...
ApacheGaugeChart.prototype.ScaleGauge = function (
  gauge,
  quadWidth,
  quadHeight,
  gaugeWidth,
...
ApacheGaugeChart.prototype.FillToolTipData = function (
  boundingRectElem,
  circleElem,
  e
) {
  var chartEvent = this.GetChartEvent(e);
  if (chartEvent == null) return;
...
ApacheSemiGaugeChart.prototype.SetIndicatorPosition = function (
  yValue,
  indicator,
  ratio
) {
...
ApacheSemiGaugeChart.prototype.CreateTextMarkerGroup = function (
  gauge,
  gaugeR
) {
  var svgDoc = this._svgDoc,
  ...
  for (var i = 0; i <= majorMarkerCount; ++i) {
    ...
    if (theta >= Math.PI / 3 && theta <= (2 * Math.PI) / 3) {
      ...
      y += textBBox.height / 2;
      if (theta < Math.PI / 2) x += 2 * ApacheChart._TEXT_MARGIN;
      else x -= textBBox.width + 2 * ApacheChart._TEXT_MARGIN;
    }
...
ApacheSemiGaugeChart.prototype.ScaleGauge = function (
  ...
  var sx = quadWidth / gaugeWidth,
    sy = quadHeight / gaugeHeight;
