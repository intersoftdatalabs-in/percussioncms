TrPanelPopup.showPopup = function (a0, a1, a2, a3) {
  ...
  if (!a4) a4 = TrPanelPopup.POPUP = new TrPanelPopup();
  else a4.hide();
  ...
  if (a2) {
    if (a2["modal"]) a4.setModal(true);
    if (a2["center"]) a4.setCentered(true);
    if (a2["width"] && a2["height"]) a4.setSize(a2["width"], a2["height"]);
  }
  a4.show(a3);
};
...
function TrPanelPopup() {
  ...
  this._modal = false;
  this._visible = false;
  this._eventCallbackFunction = TrUIUtils.createCallback(
    this,
    this._handleEvent
  );
}
...
TrPanelPopup.prototype.setContent = function (a0) {
  ...
  if (this._content) {
    this._content.style.cssText =
      "position: absolute; z-index: 201; top: 0px; left: 0px; visibility:hidden; padding: 0px; overflow:auto;";
  }
};
...
TrPanelPopup.prototype._handleEvent = function (a7) {
  if (!this.isVisible() || this.isModal()) return;
  var a8 = false;
  if (_agent.isIE) {
    a8 = a7.srcElement;
  } else {
  ...
  while (a8) {
    if (a8 == this.getContent() || a8 == this.getTrigger()) {
      break;
    }
    a8 = a8.parentNode;
  }
...
TrPanelPopup._showMask = function () {
  if (!TrPanelPopup._mask) {
    ...
    TrPanelPopup._mask.style.cssText =
      "display:none;position: absolute; z-index: 200;top: 0px;left: 0px;cursor: not-allowed; background-color: transparent;";
    TrPanelPopup._mask.innerHTML = "&nbsp;";
    TrPanelPopup._addEvent(
      TrPanelPopup._mask,
      "click",
      TrPanelPopup._consumeMaskEvent
    );
...
TrPanelPopup.prototype._calcPosition = function (a9) {
  ...
  if (!this.isModal()) TrPanelPopup._resizeIeIframe(a10, a11, a19, a20);
};
...
TrPanelPopup._initIeIframe = function () {
  if (!TrPanelPopup._maskIframe) {
    ...
    TrPanelPopup._maskIframe.style.cssText =
      "display: none; left: 0px; position: absolute; top: 0px; z-index: 199;";
    TrPanelPopup._maskIframe.style.filter =
