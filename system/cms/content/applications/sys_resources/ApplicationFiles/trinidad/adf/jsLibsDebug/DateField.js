...
function _dfsv(dateField, newValue) {
  // Make sure we have valid values
  if (dateField == void 0 || newValue == void 0) return;
...
function _returnCalendarValue(closingWindow, event) {
...
function _returnPopupCalendarValue(props, value) {
 ...
 */
function _ldp(formName, nameInForm, usePopup, minValue, maxValue, destination) {
  ...
  if (!destination) {
    ...
    if (usePopup)
      ...
      destination += "_t=cd";
    else destination += "_t=fred&_red=cd";
  } else {
  ...
  if (usePopup) {
    ...
    TrPopupDialog._launchDialog(destination, {}, _returnPopupCalendarValue, {
      formName: formName,
      fieldName: nameInForm,
    });
  } else {
    // Open the window;  we used to name it "calendar", but
    // that's a common enough name that we hit bug 2807778
    var calWindow = openWindow(
      self,
      destination,
      "uix_2807778",
...
function _calsd(source, value) {
  if (window._calActiveDateFields != void 0) {
    ...
    if (dateField) _dfsv(dateField, value);
  }
