...
ApacheChart.prototype.Init = function (a17, a18, a19, a20, a21) {
  this._type = a17;
  this._model = a18;
  ...
  this._isIE = false;
  if (window._agent) this._isIE = _agent.isIE;
  if (this._isIE)
    this._errorHtml =
    ...
    this._errorHtml =
      "<H4>This component needs an SVG enabled browser like Internet Explorer, Firefox 1.5+ or Opera 9.0+<H4>";
...
ApacheChart.prototype.ComputeMinMaxValues = function () {
  var a34 = this._model,
    yValues = a34.getYValues(),
    xValues = a34.getXValues(),
    maxYValue = a34.getMaxYValue(),
    maxXValue = a34.getMaxXValue(),
    minYValue = a34.getMinYValue(),
    minXValue = a34.getMinXValue(),
  ...
  if (yValues != null && (maxYValue == null || minYValue == null)) {
    ...
    if (maxYValue == null) a34.setMaxYValue(a35.max);
    if (minYValue == null) a34.setMinYValue(a35.min);
  }
  if (xValues != null && (maxXValue == null || minXValue == null)) {
    var a35 = this._computeAxisMinMaxValues(xValues, seriesLabels.length);
    if (maxXValue == null) a34.setMaxXValue(a35.max);
    if (minXValue == null) a34.setMinXValue(a35.min);
  }
};
ApacheChart.prototype._computeAxisMinMaxValues = function (a36, a37) {
  var a38,
    ...
    type = this._type,
    isStacked = false,
    groupsCount = a36.length;
  if (
    type == ApacheChart.TYPE_VBAR_STACKED ||
    type == ApacheChart.TYPE_HBAR_STACKED ||
  ...
  var a41 =
      maxValue > 0 ? ApacheChart._MAX_MULTIPLIER : ApacheChart._MIN_MULTIPLIER,
    minMult =
      minValue > 0 ? ApacheChart._MIN_MULTIPLIER : ApacheChart._MAX_MULTIPLIER;
...
ApacheChart.createChart = function (a42, a43, a44, a45, a46) {
  var a47 = null;
  if (
    a42 == this.TYPE_VBAR ||
    a42 == this.TYPE_VBAR_STACKED ||
    a42 == this.TYPE_BAR_LINE_COMBO
  ) {
    a47 = new ApacheBarChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_HBAR || a42 == this.TYPE_HBAR_STACKED) {
    a47 = new ApacheHBarChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_PIE) {
    a47 = new ApachePieChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_AREA || a42 == this.TYPE_AREA_STACKED) {
    a47 = new ApacheAreaChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_LINE) {
    a47 = new ApacheLineChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_SCATTER_PLOT) {
    a47 = new ApacheScatterPlotChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_XYLINE) {
    a47 = new ApacheXYLineChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_RADAR || a42 == this.TYPE_RADAR_AREA) {
    a47 = new ApacheRadarChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.TYPE_FUNNEL) {
    a47 = new ApacheFunnelChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.SEMI_CIRCULAR_GAUGE) {
    a47 = new ApacheSemiGaugeChart(a42, a43, a44, a45, a46);
  } else if (a42 == this.CIRCULAR_GAUGE) {
    a47 = new ApacheGaugeChart(a42, a43, a44, a45, a46);
  }
  return a47;
};
...
ApacheChart.prototype._initDocument = function () {
  ...
  try {
    ...
    if (!this._rootElement) throw "not yet loaded";
    this._svgDoc = a53;
...
ApacheChart.prototype.SetDataAnimStep = function (a62) {
  var a63 = this._dataElems,
    animCount = a63.length;
  var a64 = this._margins,
    animHorizontal = this.AnimAlongXAxis();
  if (animHorizontal) {
    var a65 = a64.left;
    for (var a66 = 0; a66 < animCount; ++a66) {
      var a67 = (1 - a62) * a65;
      a63[a66].setAttribute(
        "transform",
        "translate(" + a67 + ",0) scale(" + a62 + ",1)"
      );
    }
  } else {
    var a68 = a64.bottom,
...
ApacheChart.prototype.SetGridAnimStep = function (a74) {
  ...
  if (animHorizontal) {
    ...
    for (var a78 = 0; a78 < animCount; ++a78) {
      var a79 = (1 - a74) * cy;
      a75[a78].setAttribute(
        "transform",
        "translate(0," + a79 + ") scale(1," + a74 + ")"
      );
    }
  } else {
    var a80 = a76.left;
...
ApacheChart.prototype.InitializeGradients = function () {
  ...
  ApacheChartObj.Assert(
    a83.childNodes.length > 1,
    "No Gradient Template in the SVG document"
  );
  var a85 = a83.childNodes[0],
    gradientElement;
...
ApacheChart.prototype._getLighterColor = function (a91) {
  if (a91.indexOf("#") >= 0) {
    a91 = a91.substr(1);
    var a92 = a91.substr(0, 2),
      gVal = a91.substr(2, 2),
      bVal = a91.substr(4);
    a91 =
      "#" +
      this._getLighterNumberStr(a92) +
      this._getLighterNumberStr(gVal) +
    ...
    a91 =
      "#" +
      this._getLighterNumberStr(a93[0]) +
      this._getLighterNumberStr(a93[1]) +
...
ApacheChart.prototype.DrawBorder = function () {
  var a102 = this._svgDoc,
    rootElem = this._rootElement;
  var a103 = a102.getElementById("borderPrototype").cloneNode(false);
  var a104 = ApacheChart._BORDER_SIZE,
    stroke = a104 / 2;
...
ApacheChart.prototype.DrawTitles = function () {
  var a106 = this._model,
    title = a106.getTitle(),
    subTitle = a106.getSubTitle(),
    footNote = a106.getFootNote();
  if (title) this._drawTitleElem("titleTextPrototype", title, false);
  if (subTitle) this._drawTitleElem("subTitleTextPrototype", subTitle, false);
  if (footNote) this._drawTitleElem("footNoteTextPrototype", footNote, true);
};
ApacheChart.prototype._drawTitleElem = function (a107, a108, a109) {
  var a110 = this._svgDoc,
    rootElem = this._rootElement;
  var a111 = this._margins,
    gridWidth = this._width - a111.left - a111.right;
  var a112 = this._labelElems,
    animate = this._animDuration > 0;
  ...
  var a114 = a113.getBBox(),
    textWidth = a114.width,
    dx = a111.left;
...
ApacheChart.prototype.DrawGroupLabels = function () {
  var a115 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model;
  var a116 = a115.createElementNS("http://www.w3.org/2000/svg", "g");
  this._hLabelContainer = a116;
  var a117 = model.getGroupLabels(),
    vLineCount = a117.length;
  var a118,
    labelElems = this._labelElems,
    animate = this._animDuration > 0;
  var a119,
    gLabelElems = this._groupLabelElems;
...
ApacheChart.prototype.LayoutGroupLabels = function () {
  var a121 = this._model,
    margins = this._margins,
    marginLeft = margins.left;
  var a122 = this._hLabelContainer,
    childNodes = a122.childNodes;
  if (childNodes.length == 0) return;
  if (this._isPerspective) marginLeft += ApacheChart._XOFFSET_PERSPECTIVE;
  var a123 = this._width - marginLeft - margins.right;
  var a124 = this.IsGroupLabelCentered();
  var a125 = a121.getGroupLabels(),
    vLineCount = a125.length;
  var a126,
    groupWidth = a123 / (a124 ? vLineCount : vLineCount - 1);
  var a127 = 0,
    dy =
      this._height -
      margins.bottom +
      a122.getBBox().height +
      ApacheChart._TEXT_MARGIN;
  ...
  for (var a129 = 0; a129 < vLineCount; ++a129) {
    ...
    if (a124) {
      if (groupWidth > a130) a127 = (groupWidth - a130) / 2;
      else a127 = 2;
    } else {
      a127 = -a130 / 2;
      if (this._isPerspective) a127 -= ApacheChart._XOFFSET_PERSPECTIVE;
    }
...
ApacheChart.prototype.AdjustMarginsForGroupLabels = function () {
  ...
  if (a131 && a131.childNodes.length > 0) {
    ...
    if (!a132) {
      ...
      if (a133 / 2 > this._margins.right) this._margins.right = a133 / 2;
    }
  }
};
ApacheChart.prototype.DrawLegend = function () {
  ...
  var a135 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model;
  var a136 = this._gradientsUsed;
  var a137 = model.getSeriesLabels(),
    seriesCount = a137.length,
    seriesColors = model.getSeriesColors();
  var a138,
    rectElem,
    legendRectHeight,
    legendGroup = a135.createElementNS("http://www.w3.org/2000/svg", "g");
  var a139 = this._margins,
    marginLeft = a139.left,
    marginTop = a139.top;
  var a140 = this._labelElems,
    animate = this._animDuration > 0;
  ...
  var a142 = 0,
    dy = 0,
    tx = marginLeft,
    ty = this._height - a139.bottom;
  var a143 =
    a134 == ApacheChart.LEGEND_LOCATION_START ||
  ...
  for (var a144 = 0; a144 < seriesCount; ++a144) {
    ...
    if (a136) rectElem.setAttribute("fill", "url(#gradient" + a144 + ")");
    else rectElem.setAttribute("fill", seriesColors[a144]);
    ...
    if (!a143) a142 += a138.getComputedTextLength() + legendRectHeight;
    else dy += 1.5 * legendRectHeight;
    if (a144 == 0 && !a143) {
  ...
  if (!a143 && a141 > a142) tx = (a141 - a142) / 2 + marginLeft;
  if (a143) {
    ...
    if (gridHeight > dy) ty = (gridHeight - a146.height) / 2 + marginTop;
    else ty = gridHeight + marginTop - a146.height;
  }
...
ApacheChart.prototype.DrawGrid = function () {
  if (this._isPerspective) this.DrawPerspectiveGrid();
  else this.Draw2DGrid();
};
ApacheChart.prototype.Draw2DGrid = function () {
  var a153 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a154 = this._gridElems,
    animate = this._animDuration > 0;
  var a155 = margins.left,
  ...
  if (a158) a159.setAttribute("fill", "url(#gridGradient)");
  ...
  var a161 = new ApacheChartBuffer(),
    vLineCount = this.GetVLineCount(),
    hLineCount = this.GetHLineCount();
  for (var a162 = 0; a162 < hLineCount - 1; ++a162) {
    a161
      .append("M")
      .append(a155)
      .append(",")
      .append(((a162 + 1) * a157) / hLineCount + marginTop);
    a161.append("h").append(a156);
  }
  for (var a162 = 0; a162 < vLineCount - 1; ++a162) {
    a161
      .append("M")
      .append(a155 + ((a162 + 1) * a156) / vLineCount)
      .append(",")
      .append(marginTop);
...
ApacheChart.prototype.GetVLineCount = function () {
  ...
  if (a163 >= 0) return a163;
  else return this._model.getGroupLabels().length;
};
...
ApacheChart.prototype.DrawPerspectiveGrid = function () {
  var a164 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a165 = this._gridElems,
    animate = this._animDuration > 0;
  var a166 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a167 = margins.left,
    marginTop = margins.top;
  ...
  if (a171) a170.setAttribute("fill", "url(#gridGradient)");
  ...
  if (a171) a173.setAttribute("fill", "url(#gridGradient)");
  ...
  for (var a175 = 0; a175 < hLineCount - 1; ++a175) {
    a172
      ...
      .append(a167)
      .append(",")
      .append(((a175 + 1) * a169) / hLineCount + marginTop + yOffset);
    a172.append("l").append(a166).append(",").append(-yOffset);
    a172.append("h").append(a168);
  }
  for (var a175 = 0; a175 < a174 - 1; ++a175) {
    a172
      .append("M")
      .append(a167 + a166 + ((a175 + 1) * a168) / a174)
      .append(",")
      .append(marginTop);
    a172.append("v").append(a169);
    a172.append("l").append(-a166).append(",").append(yOffset);
...
ApacheChart.prototype.DrawYValueLabels = function () {
  var a176 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model;
  var a177 = a176.createElementNS("http://www.w3.org/2000/svg", "g");
  this._vLabelContainer = a177;
  var a178 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a179 = this._labelElems,
    animate = this._animDuration > 0;
...
ApacheChart.prototype.LayoutYValueLabels = function () {
  var a188 = this._model,
    margins = this._margins;
  var a189 = margins.left,
    marginTop = margins.top;
  var a190 = this._vLabelContainer,
    childNodes = a190.childNodes;
  var a191 = this._height - marginTop - margins.bottom;
  if (this._isPerspective) a191 -= ApacheChart._YOFFSET_PERSPECTIVE;
  var a192 = a190.getBBox(),
    textHeight = a192.height;
  this.SetVerticalLabelAt(
    childNodes.item(0),
    a191 + marginTop,
    a189,
    textHeight
  );
  this.SetVerticalLabelAt(childNodes.item(1), marginTop, a189, textHeight);
  var a193 = this._yMajorGridCount;
  for (var a194 = 0; a194 < a193 - 1; ++a194) {
    this.SetVerticalLabelAt(
      childNodes.item(a194 + 2),
...
ApacheChart.prototype.SetVerticalLabelAt = function (a195, a196, a197, a198) {
  if (this._isPerspective) a196 += ApacheChart._YOFFSET_PERSPECTIVE;
  var a199 = ApacheChart._TEXT_MARGIN,
    textLength = a195.getComputedTextLength(),
    dx = a199;
  if (a197 > textLength + a199) dx = a197 - textLength - a199;
...
ApacheChart.prototype.ShowToolTip = function (a210) {
  ...
  var a211 = this._model,
    seriesColors = a211.getSeriesColors();
...
ApacheChart.prototype.GetChartEvent = function (a221) {
  ...
  var a224 = this._model,
    yValues = a224.getYValues();
...
ApacheChart.prototype.FillToolTipData = function (a225, a226, a227) {
  ...
  var a230 = this._model,
    groupLabels = a230.getGroupLabels(),
  ...
  var a234 = ApacheChart._TEXT_MARGIN,
    dx = a234;
  if (a232 > a233) dx = (a232 - a233) / 2 + a234;
...
ApacheChart.prototype.HideToolTip = function (a236) {
  ...
  if (a237) a237.style.setProperty("visibility", "hidden", "");
  this._toolTipVisible = false;
};
...
function ApacheBarChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
...
ApacheBarChart.prototype.DrawChartData = function () {
  ...
  if (a6) this._drawPerspectiveBars(a5);
  else this._drawBars(a5);
  if (a5) {
    if (a6)
      this.__drawPerspectiveLines =
        ApacheLineChart.prototype.__drawPerspectiveLines;
    else this.__drawLines = ApacheLineChart.prototype.__drawLines;
...
ApacheBarChart.prototype._drawBars = function (a7) {
  ...
  var a9 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a10 = margins.left,
    marginTop = margins.top;
  ...
  var a16 = model.getGroupLabels(),
    groupCount = a16.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a17 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a18 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  ...
  for (var a25 = 0; a25 < a20; ++a25) {
    ...
    for (var a26 = 0; a26 < seriesCount; ++a26) {
      if (a7 && a26 % 2 > 0) continue;
      if (a15) stackBase = a26 == 0 ? a18 : 0;
      ...
      if (a15) dy -= barHeight;
      else dy = a12 + marginTop - barHeight;
      a13.setAttribute("y", dy);
      a13.setAttribute("width", a21);
      a13.setAttribute("height", barHeight);
      if (a23) a13.setAttribute("fill", "url(#gradient" + a26 + ")");
      else a13.setAttribute("fill", a17[a26]);
...
ApacheBarChart.prototype._drawPerspectiveBars = function (a27) {
  var a28 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a29 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a30 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a31 = margins.left,
    marginTop = margins.top;
  ...
  var a37 = model.getGroupLabels(),
    groupCount = a37.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a38 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a39 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a40 = a27 ? Math.ceil(seriesCount / 2) : seriesCount,
    barWidth;
  var a41 = yValues.length;
  if (a36) barWidth = a32 / Math.max(a41, groupCount) - 2 * a35;
  else barWidth = (a32 / Math.max(a41, groupCount) - 2 * a35 - a40 * a35) / a40;
  var a42 = a31,
    dy,
    barHeight,
    stackBase = a39;
  ...
  for (var a45 = 0; a45 < a41; ++a45) {
    ...
    for (var a46 = 0; a46 < seriesCount; ++a46) {
      if (a27 && a46 % 2 > 0) continue;
      if (a36) stackBase = a46 == 0 ? a39 : 0;
      barHeight = (a33 * (yValues[a45][a46] - stackBase)) / (maxValue - a39);
      if (a36) dy -= barHeight;
      else dy = a33 + yOffset + marginTop - barHeight;
      ...
      if (a43) a34.setAttribute("fill", "url(#gradient" + a46 + ")");
      else a34.setAttribute("fill", a38[a46]);
...
ApacheHBarChart.prototype.DrawYValueLabels = function () {
  ...
  var a8 = this._labelElems,
    animate = this._animDuration > 0;
  var a9 = model.getGroupLabels(),
    hLineCount = a9.length;
  var a10,
    gLabelElems = this._groupLabelElems;
...
ApacheHBarChart.prototype.LayoutYValueLabels = function () {
  ...
  if (this._isPerspective) a13 -= ApacheChart._YOFFSET_PERSPECTIVE;
  var a14 = this._vLabelContainer,
    childNodes = a14.childNodes;
  if (childNodes.length == 0) return;
  var a15,
    bBox = a14.getBBox(),
    textHeight = bBox.height;
  var a16 = a12.getGroupLabels(),
    hLineCount = a16.length;
...
ApacheHBarChart.prototype.DrawGroupLabels = function () {
  ...
  for (var a25 = 0; a25 < a20 + 1; ++a25) {
    ...
    if (a25 == 0) a24 = a22;
    else if (a25 == a20) a24 = maxValue;
    else a24 = ((maxValue - a22) * a25) / a20 + a22;
...
ApacheHBarChart.prototype.LayoutGroupLabels = function () {
  ...
  var a28 = this._hLabelContainer,
    childNodes = a28.childNodes;
  if (this._isPerspective) a27 -= ApacheChart._XOFFSET_PERSPECTIVE;
  var a29 = this._yMajorGridCount;
  var a30,
    yValWidth = a27 / a29;
  var a31 = a28.getBBox();
  var a32 = 0,
    dy = this._height - margins.bottom + a31.height + ApacheChart._TEXT_MARGIN;
  var a33 = this._labelElems,
    animate = this._animDuration > 0;
...
ApacheHBarChart.prototype._drawBars = function () {
  var a37 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a38 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a39 = margins.left,
    marginTop = margins.top;
  ...
  var a45 = model.getGroupLabels(),
    groupCount = a45.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a46 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a47 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a48 = a44 ? 1 : seriesCount,
    stackBase = a47;
  ...
  for (var a54 = 0; a54 < a49; ++a54) {
    ...
    for (var a55 = 0; a55 < seriesCount; ++a55) {
      ...
      if (a52) a42.setAttribute("fill", "url(#gradient" + a55 + ")");
      else a42.setAttribute("fill", a46[a55]);
...
ApacheHBarChart.prototype._drawPerspectiveBars = function () {
  var a56 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a57 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a58 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a59 = margins.left,
    marginTop = margins.top;
  ...
  var a65 = model.getGroupLabels(),
    groupCount = a65.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a66 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a67 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a68 = yValues.length;
  var a69,
    stackBase = a67;
  if (a64) a69 = a61 / Math.max(a68, groupCount) - 2 * a63;
  else
    a69 =
      (a61 / Math.max(a68, groupCount) - 2 * a63 - seriesCount * a63) /
      seriesCount;
  var a70 = a59,
    dy = a61 + marginTop + yOffset,
    barWidth;
  ...
  for (var a73 = 0; a73 < a68; ++a73) {
    ...
    for (var a74 = 0; a74 < seriesCount; ++a74) {
      ...
      if (a71) a62.setAttribute("fill", "url(#gradient" + a74 + ")");
      else a62.setAttribute("fill", a66[a74]);
...
function ApachePieChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApachePieChart);
ApachePieChart.prototype.Init = function (a5, a6, a7, a8, a9) {
  ApachePieChart.superclass.Init.call(this, a5, a6, a7, a8, a9);
};
ApachePieChart.prototype.DrawChartData = function () {
  var a10 = this._rootElement;
  var a11 = this._model,
    yValues = a11.getYValues(),
    yValueCount = yValues.length;
  var a12 = a11.getGroupLabels(),
    groupCount = a12 ? a12.length : 1;
  var a13 = Math.ceil(Math.sqrt(yValueCount)),
    nRows = Math.round(Math.sqrt(yValueCount));
  var a14 = this._svgDoc.getElementById("groupLabelPrototype");
  var a15 = this._margins,
    dx = a15.left,
    dy = a15.top;
  var a16 = (this._width - a15.left - a15.right) / a13;
  var a17 = this._animDuration > 0,
    isPerspective = this._isPerspective;
  var a18,
    vGap = 2 * ApacheChart._TEXT_MARGIN;
  ...
  for (var a20 = 0; a20 < nRows; ++a20) {
    for (var a21 = 0; a21 < a13; ++a21) {
      ...
      if (a22 >= yValueCount) break;
      var a23 = a22 == -1 ? null : a12[a22];
      var a24 = a10.cloneNode(false);
      a10.appendChild(a24);
      var a25 = this.DrawGroupLabelTitle(
        a23,
        a10,
        a14.cloneNode(true),
        dx,
      ...
      if (isPerspective) {
        ...
        a24.setAttribute("transform", "translate(" + a27 + "," + cy + ")");
      }
      dx += a16;
    }
    dx = a15.left;
...
ApachePieChart.prototype.SetDataAnimStep = function (a28) {
  var a29 = this._pieAnimRadii,
    pieAnimAngles = this._pieAnimAngles,
    isPerspective = this._isPerspective,
    agleIndex = 0,
    elemIndex = 0;
  var a30 = this._dataElems,
    chartCount = a29.length;
  var a31 = this._model,
    yValues = a31.getYValues();
...
ApachePieChart.prototype._drawPies = function (a38, a39, a40, a41) {
  var a42 = this._svgDoc,
    model = this._model,
    yValues = model.getYValues();
  var a43 = model.getGroupLabels(),
    seriesColors = model.getSeriesColors();
  ...
  var a48 = a42.getElementById("piePathPrototype"),
    pieStart = 0,
    animAngleStart = 0;
  var a49 = new Array(a45),
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a50 = this._gradientsUsed;
  var a51 = "translate(-10000, -10000)",
    pieAnimAngles = this._pieAnimAngles;
  for (var a47 = 0; a47 < a45; ++a47) {
    ...
    if (a50) a48.setAttribute("fill", "url(#gradient" + a47 + ")");
    else a48.setAttribute("fill", seriesColors[a47]);
...
ApachePieChart.prototype._draw3DPies = function (a55, a56, a57, a58) {
  var a59 = this._svgDoc,
    model = this._model,
    yValues = model.getYValues();
  var a60 = model.getGroupLabels(),
    seriesColors = model.getSeriesColors();
  ...
  var a65 = a61 / 4,
    pieElems = new Array(a63),
    ringElems = new Array(a63),
    edgeElems = new Array(a63);
  var a66 = this._dataElems,
    animate = this._animDuration > 0;
  if (a65 > ApachePieChart._MAX_PERSPECTIVE_HEIGHT)
    a65 = ApachePieChart._MAX_PERSPECTIVE_HEIGHT;
  var a67 = a59.getElementById("piePathPrototype"),
    pieStart = 0;
  var a68 = this._gradientsUsed;
  var a69 = "translate(-10000, -10000)",
    pieAnimAngles = this._pieAnimAngles;
  for (var a64 = 0; a64 < a63; ++a64) {
    ...
    a72.append("z");
    if (a68) a67.setAttribute("fill", "url(#gradient" + a64 + ")");
    else a67.setAttribute("fill", seriesColors[a64]);
  ...
  var a75 = 0;
  for (var a64 = 0; a64 < a63; ++a64) {
    if (a75 <= 0.5) a55.appendChild(ringElems[a64]);
    a75 += yValues[a58][a64] / a62;
  }
  a75 = 0;
  for (var a64 = 0; a64 < a63; ++a64) {
    if (a75 <= 0.5) a55.appendChild(edgeElems[a64]);
    a75 += yValues[a58][a64] / a62;
  }
  a75 = 0;
  for (var a64 = 0; a64 < a63; ++a64) {
...
ApachePieChart.prototype.GetToolTipLocation = function (a76, a77) {
  ...
  return {
    x: a80.e + a79.x + a79.width / 2,
    y: a80.f + a79.y + a79.height / 2 - a77.height,
  };
};
ApachePieChart._MAX_PERSPECTIVE_HEIGHT = 30;
function ApacheAreaChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheAreaChart);
ApacheAreaChart.prototype.Init = function (a5, a6, a7, a8, a9) {
  ApacheAreaChart.superclass.Init.call(this, a5, a6, a7, a8, a9);
  this._toolTips = [];
};
...
ApacheAreaChart.prototype.DrawChartData = function () {
  ...
  if (this._isPerspective) this._drawPerspectiveAreas();
  else this._drawAreas();
};
ApacheAreaChart.prototype.SetDataAnimStep = function (a12) {
  var a13 = this._model,
    seriesLabels = a13.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a14 = a13.getYValues(),
    yValueCount = a14.length;
  var a15 = this._dataElems,
    animPathCount = this._isPerspective ? yValueCount - 1 : 1;
  var a16 = this._margins,
    marginBottom = a16.bottom;
  var a17 = this._height - marginBottom;
  var a18 = a12 * seriesCount,
    animSeriesIndex = 0;
  if (a18 > 1) {
    animSeriesIndex = Math.floor(a18);
    if (animSeriesIndex >= seriesCount) animSeriesIndex = seriesCount - 1;
    a18 = a18 - Math.floor(a18);
  }
  var a19 = animSeriesIndex;
  for (var a20 = 0; a20 < animPathCount; ++a20) {
    var a21 = (1 - a18) * a17;
    a15[a19 * animPathCount + a20].setAttribute(
      "transform",
      "translate(0," + a21 + ") scale(1," + a18 + ")"
    );
    if (a19 > 0) {
      a15[(a19 - 1) * animPathCount + a20].setAttribute(
        "transform",
        "scale(1,1)"
      );
    }
...
ApacheAreaChart.prototype._drawAreas = function () {
  var a23 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a24 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a25 = margins.left,
    marginTop = margins.top;
  ...
  var a29 = model.getGroupLabels(),
    groupCount = a29.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a30 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a31 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  ...
  for (var a38 = 0; a38 < seriesCount; ++a38) {
    ...
    if (a38 == 0 || !a32) a39.append("M").append(a37).append(",").append(dy);
    else if (a32) a39.append("M").append(a37).append(",").append(cumYs[0]);
    stackBase = a38 == 0 ? a31 : 0;
    for (var a40 = 0; a40 < a33; ++a40) {
      if (a32) {
        if (null == cumYs[a40]) cumYs[a40] = a27 + marginTop;
        dy = cumYs[a40] -=
          (a27 * (yValues[a40][a38] - stackBase)) / (maxValue - a31);
      } else
        dy =
          a27 +
          marginTop -
          (a27 * (yValues[a40][a38] - a31)) / (maxValue - a31);
      a39.append("L").append(a37).append(",").append(dy);
      if (a40 != a33 - 1) a37 += a34;
    }
    if (animate) {
      dataElems.push(a28);
      a28.setAttribute("transform", a36);
    }
    if (a35) a28.setAttribute("fill", "url(#gradient" + a38 + ")");
    else a28.setAttribute("fill", a30[a38]);
...
ApacheAreaChart.prototype._drawPerspectiveAreas = function () {
  var a42 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a43 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a44 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a45 = margins.left,
    marginTop = margins.top;
  ...
  var a49 = model.getGroupLabels(),
    groupCount = a49.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a50 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a51 = this._type == ApacheChart.TYPE_AREA_STACKED;
  var a52 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a53 = yValues.length;
  var a54 = a46 / (Math.max(a53, groupCount) - 1),
    stackBase;
  var a55 = a47 + marginTop + yOffset,
    dx,
    dy,
    cumYs = [];
  ...
  for (var a58 = 0; a58 < seriesCount; ++a58) {
    ...
    for (var a60 = 0; a60 < a53; ++a60) {
      if (a51) {
        if (null == cumYs[a60]) cumYs[a60] = a55;
        dy = cumYs[a60] -=
          (a47 * (yValues[a60][a58] - stackBase)) / (maxValue - a52);
      } else dy = a55 - (a47 * (yValues[a60][a58] - a52)) / (maxValue - a52);
      if (a60 != a53 - 1) {
        a48 = a48.cloneNode(false);
        ...
        var a61,
          nextdx = dx + a54;
        if (a51) {
          if (null == cumYs[a60 + 1]) cumYs[a60 + 1] = a55;
          a61 =
            cumYs[a60 + 1] -
            (a47 * (yValues[a60 + 1][a58] - stackBase)) / (maxValue - a52);
        } else
        ...
        if (a58 == 0 || !a51) {
          a59
            ...
            .append(nextdx + a44)
            .append(",")
            .append(a47 + marginTop);
        } else {
          a59
            .append("L")
            .append(nextdx + a44)
            .append(",")
            .append(cumYs[a60 + 1] - yOffset);
        }
        a59.append("l").append(-a44).append(",").append(yOffset);
        ...
        a59.append("L").append(dx).append(",").append(dy);
        if (a56) a48.setAttribute("fill", "url(#gradient" + a58 + ")");
        else a48.setAttribute("fill", a50[a58]);
...
ApacheAreaChart.prototype.GetChartEvent = function (a63, a64) {
  var a65 = a63.clientX,
    clientY = a63.clientY,
    evtTarget = a63.target;
  var a66 = this._type == ApacheChart.TYPE_AREA_STACKED;
  var a67 = this._isPerspective;
  var a68 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a69 = this._model,
    yValues = a69.getYValues();
  var a70 = a69.getGroupLabels(),
    groupCount = a70.length;
  var a71 = a69.getMinYValue(),
    maxValue = a69.getMaxYValue();
  var a72 = this._margins,
    marginLeft = a72.left,
    marginTop = a72.top;
  ...
  var a79 = a69.getSeriesLabels().length,
    stackBase,
    insideStacked = false;
  if (!a64) a64 = [];
  for (var a80 = 0; a80 < a79 && !insideStacked; ++a80) {
    ...
    for (var a81 = 0; a81 < a75; ++a81) {
      if (a66) {
        if (null == cumYs[a81]) cumYs[a81] = a78;
        if (null == cumYs[a81 + 1] && a81 != a75 - 1) cumYs[a81 + 1] = a78;
        cumYs[a81] -=
          (a74 * (yValues[a81][a80] - stackBase)) / (maxValue - a71);
      }
      if (a81 == a75 - 1) continue;
      if (a65 > a77 && a65 < a77 + a76) {
        if (a66) {
          dy1 = cumYs[a81];
          dy2 =
            cumYs[a81 + 1] -
            (a74 * (yValues[a81 + 1][a80] - stackBase)) / (maxValue - a71);
          dy = dy1 - ((dy1 - dy2) * (a65 - a77)) / a76;
          if (clientY >= dy) {
            value =
              yValues[a81][a80] +
              ((yValues[a81 + 1][a80] - yValues[a81][a80]) * (a65 - a77)) / a76;
          ...
          dy1 = a78 - (a74 * (yValues[a81][a80] - a71)) / (maxValue - a71);
          dy =
            dy1 -
            (((a74 * (yValues[a81 + 1][a80] - yValues[a81][a80])) /
              (maxValue - a71)) *
              (a65 - a77)) /
              a76;
          if (dy <= clientY) {
...
ApacheAreaChart.prototype._displayToolTips = function (a85, a86, a87, a88) {
  var a89 = this._svgDoc,
    rootElem = this._rootElement;
  var a90 = this._model,
    seriesLabels = a90.getSeriesLabels(),
    seriesCount = seriesLabels.length,
...
ApacheAreaChart.prototype.HideToolTip = function (a107) {
  var a108 = this._toolTips,
    tooltipCount = a108.length;
  for (var a109 = 0; a109 < tooltipCount; ++a109) {
    var a110 = a108[a109];
    if (a110) a110.style.setProperty("visibility", "hidden", "");
  }
};
function ApacheLineChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheLineChart);
ApacheLineChart.prototype.DrawChartData = function (a5) {
  if (this._isPerspective) this.__drawPerspectiveLines(a5);
  else this.__drawLines(a5);
};
...
ApacheLineChart.prototype.__drawLines = function (a6) {
  ...
  var a13 = model.getGroupLabels(),
    groupCount = a13.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a14 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a15 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  ...
  for (var a21 = 0; a21 < seriesCount; ++a21) {
    if (a6 && a21 % 2 == 0) continue;
    a18 = a9 + a17 / 2;
    var a22 = new ApacheChartBuffer();
    pathElem = pathElem.cloneNode(false);
    for (var a23 = 0; a23 < a16; ++a23) {
      dy =
        a11 + marginTop - (a11 * (yValues[a23][a21] - a15)) / (maxValue - a15);
      if (a23 == 0) a22.append("M").append(a18).append(",").append(dy);
      else a22.append("L").append(a18).append(",").append(dy);
      a12 = a12.cloneNode(false);
      a12.setAttribute("cx", a18);
      a12.setAttribute("cy", dy);
      if (a19) a12.setAttribute("fill", "url(#gradient" + a21 + ")");
      else a12.setAttribute("fill", a14[a21]);
...
ApacheLineChart.prototype.__drawPerspectiveLines = function (a24) {
  var a25 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a26 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a27 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a28 = margins.left,
    marginTop = margins.top;
  ...
  var a32 = model.getGroupLabels(),
    groupCount = a32.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a33 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a34 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a35 = yValues.length;
  var a36 = a29 / Math.max(a35, groupCount);
  var a37 = a30 + marginTop + yOffset,
    dx,
    dy;
  var a38 = this._gradientsUsed;
  var a39 = "scale(0.00001,1)";
  for (var a40 = 0; a40 < seriesCount; ++a40) {
    if (a24 && a40 % 2 == 0) continue;
    dx = a28 + a36 / 2;
    ...
    for (var a42 = 0; a42 < a35; ++a42) {
      ...
      if (a42 != a35 - 1) {
        ...
        var a43 =
          a37 - (a30 * (yValues[a42 + 1][a40] - a34)) / (maxValue - a34);
        var a44 = dx + a36;
        a41
          .append("L")
          .append(a44 + a27)
        ...
        if (a38) a31.setAttribute("fill", "url(#gradient" + a40 + ")");
        else a31.setAttribute("fill", a33[a40]);
...
ApacheLineChart.prototype.GetChartEvent = function (a48) {
  ...
  var a54 = this._model,
    yValues = a54.getYValues();
  var a55 = a54.getGroupLabels(),
    groupCount = a55.length;
  var a56 = this._margins,
    marginLeft = a56.left,
    marginTop = a56.top;
  ...
  var a62 = marginLeft + a60 / 2,
    value = 0.0;
  for (var a63 = 0; a63 < a59; ++a63) {
    if (a63 == a59 - 1) continue;
    if (a51 > a62 && a51 < a62 + a60) {
      value =
        yValues[a63][a50] +
        ((yValues[a63 + 1][a50] - yValues[a63][a50]) * (a51 - a62)) / a60;
      break;
    }
    a62 += a60;
  }
...
ApacheLineChart.prototype.FillToolTipData = function (a64, a65, a66) {
  ...
  var a68 = a67.getSeriesIndices()[0],
    value = a67.getYValues()[0];
  var a69 = this._model.getSeriesLabels();
  var a70 = a64.nextSibling.nextSibling;
  a70.firstChild.data = a69[a68] + ": " + this._formatValue(value);
...
function ApacheScatterPlotChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheScatterPlotChart);
ApacheScatterPlotChart.prototype.Init = function (a5, a6, a7, a8, a9) {
  ApacheScatterPlotChart.superclass.Init.call(this, a5, a6, a7, a8, a9);
};
ApacheScatterPlotChart.prototype.DrawChartData = function () {
  if (this._isPerspective) this._drawPerspectivePoints();
  else this._drawPoints();
};
ApacheScatterPlotChart.prototype.SetDataAnimStep = function (a10) {
  ...
  var a12 = this._cxs,
    ...
    gridCx,
    gridCy;
  var a13 = this._margins,
    marginLeft = a13.left,
    marginTop = a13.top;
  var a14 = this._width - marginLeft - a13.right;
  var a15 = this._height - marginTop - a13.bottom;
  var a16 = this._dataElems,
    animCount = a12.length,
    elemIndex = 0;
  if (a11) marginLeft += ApacheChart._XOFFSET_PERSPECTIVE;
...
ApacheScatterPlotChart.prototype.SetGridAnimStep = function (a21) {
  var a22 = this._gridElems,
    animCount = a22.length;
...
ApacheScatterPlotChart.prototype._drawPoints = function () {
  ...
  var a31 = model.getGroupLabels(),
    groupCount = a31.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a32 = model.getSeriesColors(),
    xValues = model.getXValues(),
    yValues = model.getYValues(),
    nValues = yValues.length;
  var a33 = "translate(0,100000)";
  var a34 = model.getMinYValue(),
    maxYValue = model.getMaxYValue();
  var a35 = model.getMinXValue(),
    maxXValue = model.getMaxXValue();
  ...
  for (var a39 = 0; a39 < seriesCount; ++a39) {
    for (var a40 = 0; a40 < nValues; ++a40) {
      dy =
        a29 + marginTop - (a29 * (yValues[a40][a39] - a34)) / (maxYValue - a34);
      dx = a27 + (a28 * (xValues[a40][a39] - a35)) / (maxXValue - a35);
      a30 = a30.cloneNode(false);
      if (a38) a30.setAttribute("fill", "url(#gradient" + a39 + ")");
      else a30.setAttribute("fill", a32[a39]);
...
ApacheScatterPlotChart.prototype._drawPerspectivePoints = function () {
  var a41 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a42 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a43 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a44 = margins.left,
    marginTop = margins.top;
  ...
  var a48 = model.getGroupLabels(),
    groupCount = a48.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a49 = model.getSeriesColors(),
    xValues = model.getXValues(),
    yValues = model.getYValues(),
    nValues = yValues.length;
  var a50 = model.getMinYValue(),
    maxYValue = model.getMaxYValue();
  var a51 = model.getMinXValue(),
    maxXValue = model.getMaxXValue();
  var a52 = a45 / (groupCount - 1);
  var a53 = a46 + marginTop + yOffset,
    cxs,
    cys,
    dx,
    dy,
  ...
  for (var a55 = 0; a55 < seriesCount; ++a55) {
    for (var a56 = 0; a56 < nValues; ++a56) {
      ...
      if (a54) a47.setAttribute("fill", "url(#gradient" + a55 + ")");
      else a47.setAttribute("fill", a49[a55]);
...
ApacheScatterPlotChart.prototype.FillToolTipData = function (a62, a63, a64) {
  ...
  var a67 = this._model,
    seriesLabels = a67.getSeriesLabels();
...
function ApacheXYLineChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
...
ApacheXYLineChart.prototype.DrawChartData = function () {
  if (this._isPerspective) this._drawPerspectiveXYValues();
  else this._drawXYValues();
};
...
ApacheXYLineChart.prototype._drawXYValues = function () {
  ...
  var a11, pathElem;
  var a12 = model.getGroupLabels(),
    groupCount = a12.length,
    seriesLabels = model.getSeriesLabels(),
    seriesCount = seriesLabels.length;
  var a13 = model.getSeriesColors(),
    xValues = model.getXValues(),
    yValues = model.getYValues(),
    nValues = yValues.length;
  ...
  for (var a19 = 0; a19 < seriesCount; ++a19) {
    ...
    for (var a21 = 0; a21 < nValues; ++a21) {
      dy =
        a10 + marginTop - (a10 * (yValues[a21][a19] - a15)) / (maxYValue - a15);
      a18 = a8 + (a9 * (xValues[a21][a19] - a16)) / (maxXValue - a16);
      if (a21 == 0) a20.append("M").append(a18).append(",").append(dy);
      else a20.append("L").append(a18).append(",").append(dy);
    }
...
ApacheXYLineChart.prototype._drawPerspectiveXYValues = function () {
  var a22 = this._svgDoc,
    model = this._model,
    margins = this._margins;
  var a23 = this._rootElement,
    dataElems = this._dataElems,
    animate = this._animDuration > 0;
  var a24 = ApacheChart._XOFFSET_PERSPECTIVE,
    yOffset = ApacheChart._YOFFSET_PERSPECTIVE;
  var a25 = margins.left,
    marginTop = margins.top;
  ...
  var a32 = model.getMinYValue(),
    maxYValue = model.getMaxYValue();
  var a33 = model.getMinXValue(),
    maxXValue = model.getMaxXValue();
  var a34 = a27 + marginTop + yOffset,
    dx,
    dy;
  var a35 = this._gradientsUsed;
  ...
  for (var a36 = 0; a36 < seriesCount; ++a36) {
    ...
    for (var a38 = 0; a38 < nValues; ++a38) {
      ...
      if (a38 != nValues - 1) {
        ...
        var a39, nextdx;
        nextdx =
          a25 + (a26 * (xValues[a38 + 1][a36] - a33)) / (maxXValue - a33);
        a39 = a34 - (a27 * (yValues[a38 + 1][a36] - a32)) / (maxYValue - a32);
        a37
          .append("L")
          .append(nextdx + a24)
          .append(",")
    ...
    if (a35) pathElem.setAttribute("fill", "url(#gradient" + a36 + ")");
    else pathElem.setAttribute("fill", a30[a36]);
...
ApacheXYLineChart.prototype.GetChartEvent = function (a43) {
  ...
  var a49 = this._model,
    xValues = a49.getXValues(),
    yValues = a49.getYValues(),
    nValues = yValues.length;
  var a50 = a49.getMinYValue(),
    maxYValue = a49.getMaxYValue();
  var a51 = a49.getMinXValue(),
    maxXValue = a49.getMaxXValue();
  var a52 = this._margins,
    marginLeft = a52.left,
    marginTop = a52.top;
  ...
  for (var a58 = 0; a58 < nValues; ++a58) {
    if (a58 != nValues - 1) {
      ...
      nextdx =
        marginLeft + (a53 * (xValues[a58 + 1][a45] - a51)) / (maxXValue - a51);
      if (a46 > a56 && a46 < a56 + nextdx) {
        dy = a55 - (a54 * (yValues[a58][a45] - a50)) / (maxYValue - a50);
        a57 = a55 - (a54 * (yValues[a58 + 1][a45] - a50)) / (maxYValue - a50);
        yValue =
          yValues[a58][a45] +
          ((yValues[a58 + 1][a45] - yValues[a58][a45]) * (clientY - dy)) /
            (a57 - dy);
        xValue =
          xValues[a58][a45] +
          ((xValues[a58 + 1][a45] - xValues[a58][a45]) * (a46 - a56)) /
            (nextdx - a56);
        break;
      }
    }
...
ApacheXYLineChart.prototype.FillToolTipData = function (a59, a60, a61) {
  ...
  var a64 = this._model,
    seriesLabels = a64.getSeriesLabels();
...
function ApacheRadarChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheRadarChart);
ApacheRadarChart.prototype.Init = function (a5, a6, a7, a8, a9) {
  ApacheRadarChart.superclass.Init.call(this, a5, a6, a7, a8, a9);
  this._toolTips = [];
};
...
ApacheRadarChart.prototype.SetGridAnimStep = function (a10) {
  var a11 = this._gridElems,
    animCount = a11.length;
...
ApacheRadarChart.prototype._setRadarSeriesAnimStep = function (
  a25,
  a26,
  a27,
  a28
...
ApacheRadarChart.prototype.DrawGroupLabels = function () {
  var a43 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model;
  var a44 = a43.createElementNS("http://www.w3.org/2000/svg", "g");
  this._hLabelContainer = a44;
  var a45 = this._labelElems,
    animate = this._animDuration > 0;
  var a46 = model.getGroupLabels(),
    vLineCount = a46.length;
...
ApacheRadarChart.prototype.LayoutGroupLabels = function () {
  var a53 = this._model,
    margins = this._margins;
  var a54 = margins.left,
    marginTop = margins.top;
  var a55 = this._width - a54 - margins.right;
  var a56, cy;
  var a57 = a54 + a55 / 2,
    radius;
  var a58 = a53.getGroupLabels(),
    vLineCount = a58.length;
  var a59,
    groupWidth = a55 / vLineCount;
  var a60 = this._hLabelContainer,
    childNodes = a60.childNodes;
  var a61 = false,
    gLabelElems = this._groupLabelElems;
  if (childNodes.length == 0) return;
  for (var a62 = 0; a62 < vLineCount; ++a62) {
    ...
    if (!a61) {
      ...
      cy = marginTop + a56 / 2;
      radius =
        Math.min(a55, a56) / 2 + a63.height - ApacheChart._TEXT_MARGIN / 2;
      a61 = true;
    }
...
ApacheRadarChart.prototype.Draw2DGrid = function () {
  var a67 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model,
    margins = this._margins;
  var a68 = this._gridElems,
    animate = this._animDuration > 0;
  var a69 = margins.left,
    marginTop = margins.top;
  ...
  if (a74) a75.setAttribute("fill", "url(#gridGradient)");
  if (animate) {
    a68.push(a75);
    a75.setAttribute("fill-opacity", "0");
  }
  rootElem.appendChild(a75);
  var a76 = this.GetVLineCount(),
    circleCount = this.GetHLineCount();
...
ApacheRadarChart.prototype._drawRadar = function () {
  var a106 = this._svgDoc,
    rootElem = this._rootElement,
    model = this._model,
    margins = this._margins;
  var a107 = this._dataElems,
    animate = this._animDuration > 0,
    dotElems;
  var a108 = margins.left,
    marginTop = margins.top;
  var a109 = this._width - a108 - margins.right;
  var a110 = this._height - marginTop - margins.bottom;
  var a111 = a108 + a109 / 2,
    cy = marginTop + a110 / 2;
  ...
  var a115,
    pathElem = a106.getElementById(a114);
  var a116 = model.getSeriesLabels(),
    seriesCount = a116.length;
  var a117 = model.getSeriesColors(),
    yValues = model.getYValues();
  var a118 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a119 = "scale(0.00001,0.00001)";
  var a120 = this._gradientsUsed;
  var a121 = yValues.length;
  var a122, dy;
  if (!a113) a115 = a106.getElementById("lineDotPrototype");
  for (var a123 = 0; a123 < seriesCount; ++a123) {
    ...
    for (var a125 = 0; a125 < a121; ++a125) {
      ...
      if (!a113) {
        ...
        if (a120) a115.setAttribute("fill", "url(#gradient" + a123 + ")");
        else a115.setAttribute("fill", a117[a123]);
    ...
    a124.append("Z");
    if (a113) {
      if (a120) pathElem.setAttribute("fill", "url(#gradient" + a123 + ")");
      else pathElem.setAttribute("fill", a117[a123]);
    } else {
...
ApacheRadarChart.prototype.isPointInPolygon = function (
  a127,
  a128,
  a129,
  a130
  ...
  for (a131 = 0, j = npol - 1; a131 < npol; j = a131++) {
    if (
      ((a128[a131] <= a130 && a130 < a128[j]) ||
        (a128[j] <= a130 && a130 < a128[a131])) &&
      a129 <
        ((a127[j] - a127[a131]) * (a130 - a128[a131])) /
          (a128[j] - a128[a131]) +
          a127[a131]
    )
      inside = !inside;
...
ApacheRadarChart.prototype.GetChartEvent = function (a132, a133, a134) {
  var a135 = a132.clientX,
    clientY = a132.clientY,
    evtTarget = a132.target;
  var a136 = this._type == ApacheChart.TYPE_RADAR_AREA;
  var a137 = this._model,
    yValues = a137.getYValues();
  var a138 = a137.getMinYValue(),
    maxValue = a137.getMaxYValue();
  var a139 = this._margins,
    marginLeft = a139.left,
    marginTop = a139.top;
  var a140 = this._width - marginLeft - a139.right;
  var a141 = this._height - marginTop - a139.bottom;
  var a142 = marginLeft + a140 / 2,
    cy = marginTop + a141 / 2;
  var a143 = Math.min(a140, a141) / 2;
  var a144 = a137.getSeriesLabels().length;
  var a145 = a137.getYValues(),
    seriesIndex;
  ...
  if (!a136) seriesIndex = parseInt(evtTarget.getAttribute("seriesIndex"));
  if (!a133) a133 = [];
  if (!a134) a134 = [];
  var a147,
    dy1,
  ...
  for (var a148 = 0; a148 < a144; ++a148) {
    if (!a136 && seriesIndex != a148) continue;
    for (var a149 = 0; a149 < a146; ++a149) {
      ...
      if (
        this.isPointInPolygon([a142, a147, dx2], [cy, dy1, dy2], a135, clientY)
      ) {
        var a154 = Math.atan2(cy - clientY, a135 - a142);
        if (a154 <= Math.PI / 2) a154 = Math.PI / 2 - a154;
        else a154 = (3 * Math.PI) / 2 + (Math.PI - a154);
...
ApacheRadarChart.prototype.ShowToolTip = function (a156) {
  ...
  this._displayToolTips(
    a158.getYValues(),
    a158.getSeriesIndices(),
    seriesYs,
    a157,
    a156
  );
...
ApacheRadarChart.prototype._displayToolTips = function (
  ...
  var a164 = this._svgDoc,
    rootElem = this._rootElement;
  var a165 = this._model,
    seriesLabels = a165.getSeriesLabels(),
    seriesCount = seriesLabels.length,
    seriesColors = a165.getSeriesColors();
  var a166 = a160.length,
    toolTips = this._toolTips;
  var a167, dy;
...
ApacheRadarChart.prototype.HideToolTip = function (a181) {
  var a182 = this._toolTips,
    tooltipCount = a182.length;
  for (var a183 = 0; a183 < tooltipCount; ++a183) {
    var a184 = a182[a183];
    if (a184) a184.style.setProperty("visibility", "hidden", "");
  }
};
function ApacheFunnelChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheFunnelChart);
ApacheFunnelChart.prototype.SetDataAnimStep = function (a5) {
  var a6 = this._dataElems,
    animCount = a6.length;
  var a7 = "scale(" + a5 + "," + a5 + ")";
...
ApacheFunnelChart.prototype.DrawChartData = function () {
  var a9 = this._rootElement;
  var a10 = this._model,
    yValues = a10.getYValues(),
    yValueCount = yValues.length;
  var a11 = a10.getGroupLabels(),
    groupCount = a11 ? a11.length : 1;
  var a12 = Math.ceil(Math.sqrt(yValueCount)),
    nRows = Math.round(Math.sqrt(yValueCount));
  var a13 = this._margins,
    dx = a13.left,
    dy = a13.top;
  ...
  for (var a18 = 0; a18 < nRows; ++a18) {
    for (var a19 = 0; a19 < a12; ++a19) {
      ...
      if (a20 >= yValueCount) break;
      var a21 = a20 == -1 ? null : a11[a20];
      var a22 = a9.cloneNode(false);
      a9.appendChild(a22);
      var a23 = this.DrawGroupLabelTitle(
        a21,
        a9,
        a17.cloneNode(true),
        dx,
...
ApacheFunnelChart.prototype._drawFunnel = function (a25, a26, a27, a28) {
  var a29 = this._svgDoc,
    model = this._model,
    yValues = model.getYValues();
  var a30 = model.getGroupLabels(),
    seriesColors = model.getSeriesColors();
  if (a28 == -1) a28 = 0;
  ...
  for (var a33 = a31 - 1; a33 >= 0; --a33) {
    ...
    if (a33 != 0) {
      ...
      var a41 =
        dy / 3 <= ApacheFunnelChart._MAX_FUNNEL_TIP
          ? y + (dy - dy / 3)
    ...
    if (a36) a35.setAttribute("fill", "url(#gradient" + a33 + ")");
    else a35.setAttribute("fill", seriesColors[a33]);
...
ApacheFunnelChart.prototype._drawPerspectiveFunnel = function (
  ...
  var a46 = this._svgDoc,
    model = this._model,
    yValues = model.getYValues();
  var a47 = model.getGroupLabels(),
    seriesColors = model.getSeriesColors();
  if (a45 == -1) a45 = 0;
  ...
  for (var a50 = a48 - 1; a50 >= 0; --a50) {
    ...
    if (a53) a52.setAttribute("fill", "url(#gradient" + a50 + ")");
    else a52.setAttribute("fill", seriesColors[a50]);
...
ApacheFunnelChart.prototype.GetToolTipLocation = function (a60, a61) {
  ...
  return {
    x: a64.e + a63.x + a63.width / 2,
    y: a64.f + a63.y + a63.height / 2 - a61.height,
  };
};
ApacheFunnelChart._MAX_FUNNEL_TIP = 16;
function ApacheGaugeChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
ApacheChartObj.Inherit(ApacheChart, ApacheGaugeChart);
ApacheGaugeChart.prototype.Init = function (a5, a6, a7, a8, a9) {
  ApacheGaugeChart.superclass.Init.call(this, a5, a6, a7, a8, a9);
};
ApacheGaugeChart.prototype.SetDataAnimStep = function (a10) {
  var a11 = this._dataElems,
    animCount = a11.length;
  var a12 = this._model,
    yValues = a12.getYValues();
...
ApacheGaugeChart.prototype.DrawChartData = function () {
  ...
  for (var a30 = 0; a30 < nRows; ++a30) {
    for (var a31 = 0; a31 < a24; ++a31) {
      ...
      if (a32 >= yValueCount) break;
      var a33 = a32 == -1 ? null : a23[a32];
      var a34 = a21.cloneNode(false);
      a21.appendChild(a34);
      if (a33) a29 = a29.cloneNode(true);
      var a35 = this.DrawGroupLabelTitle(a33, a21, a29, dx, dy, a26, a28);
...
ApacheGaugeChart.prototype.DrawDial = function (a39, a40, a41, a42) {
  var a43 = this._svgDoc,
    model = this._model;
  if (a42 == -1) a42 = 0;
  var a44 = this._dataElems,
    animate = this._animDuration > 0;
  var a45 = this.GetGaugeTemplateName();
  var a46 = a43.getElementById(a45).cloneNode(true);
  a39.appendChild(a46);
  var a47 = a46.getBBox(),
    gaugeWidth = a47.width,
    gaugeR = gaugeWidth / 2;
...
ApacheGaugeChart.prototype.CreateTextMarkerGroup = function (a51, a52) {
  ...
  var a58 = model.getMinYValue(),
    maxValue = model.getMaxYValue();
  var a59, y, angle, textMargin;
  for (
    var a60 = 0, theta = Math.PI / 6;
    a60 <= majorMarkerCount;
    ++a60, theta += (5 * Math.PI) / 3 / majorMarkerCount
  ) {
    var a61;
    if (theta < Math.PI / 2) a61 = theta + (3 * Math.PI) / 2;
    else a61 = theta - Math.PI / 2;
    ...
    if (theta >= (5 * Math.PI) / 6 && theta <= (7 * Math.PI) / 6) {
      ...
      y += a65.height / 2;
      if (theta < Math.PI) a59 += 2 * ApacheChart._TEXT_MARGIN;
      else a59 -= a65.width + 2 * ApacheChart._TEXT_MARGIN;
    }
  ...
  for (
    var a60 = minorMarkerCount + 1,
      theta =
        Math.PI / 6 + (5 * Math.PI) / 3 / (majorMarkerCount * minorMarkerCount);
    a60 <= (majorMarkerCount + 1) * minorMarkerCount;
    ++a60, theta += (5 * Math.PI) / 3 / (majorMarkerCount * minorMarkerCount)
  ) {
    if (a60 % minorMarkerCount == 0) continue;
    var a61;
    if (theta < Math.PI / 2) a61 = theta + (3 * Math.PI) / 2;
    else a61 = theta - Math.PI / 2;
...
ApacheGaugeChart.prototype.ScaleGauge = function (a67, a68, a69, a70, a71) {
  ...
  a67.setAttribute(
    "transform",
    "translate(" + a73 + "," + ty + ") scale(" + a74 + "," + a74 + ")"
  );
};
ApacheGaugeChart.prototype.GetChartEvent = function (a75) {
  var a76 = a75.target;
  while (a76 != null && a76.tagName != "g") a76 = a76.parentNode;
  if (a76 == null) return null;
  var a77 = parseInt(a76.getAttribute("yValueIndex"));
  var a78 = this._model,
    yValues = a78.getYValues();
...
function ApacheSemiGaugeChart(a0, a1, a2, a3, a4) {
  this.Init(a0, a1, a2, a3, a4);
}
...
ApacheSemiGaugeChart.prototype.SetIndicatorPosition = function (a5, a6, a7) {
  var a8,
    ...
    cy = this._animCy;
  var a9 = this._model,
    minValue = a9.getMinYValue(),
    maxValue = a9.getMaxYValue();
...
ApacheSemiGaugeChart.prototype.CreateTextMarkerGroup = function (a11, a12) {
  ...
  for (var a19 = 0; a19 <= majorMarkerCount; ++a19) {
    ...
    if (a20 >= Math.PI / 3 && a20 <= (2 * Math.PI) / 3) {
      ...
      y += a24.height / 2;
      if (a20 < Math.PI / 2) a18 += 2 * ApacheChart._TEXT_MARGIN;
      else a18 -= a24.width + 2 * ApacheChart._TEXT_MARGIN;
    }
    textElem.setAttribute(
      "transform",
  ...
  for (var a19 = 1; a19 <= majorMarkerCount * minorMarkerCount; ++a19) {
    if (a19 % minorMarkerCount == 0) continue;
...
ApacheSemiGaugeChart.prototype.ScaleGauge = function (a26, a27, a28, a29, a30) {
  var a31 = a27 / a29,
    sy = a28 / a30;
  ...
  a26.setAttribute(
    "transform",
    "translate(" + a33 + "," + ty + ") scale(" + a32 + "," + a32 + ")"
  );
};
