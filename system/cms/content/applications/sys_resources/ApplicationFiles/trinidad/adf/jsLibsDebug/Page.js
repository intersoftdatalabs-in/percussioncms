...
TrPage.getInstance = function () {
  if (TrPage._INSTANCE == null) TrPage._INSTANCE = new TrPage();
...
TrPage.prototype._handlePprResponse = function (documentElement) {
  ...
  if (rootNodeName == "content") {
    ...
    // message
    if (nodeText == null) nodeText = "Unknown error during PPR";
    alert(nodeText);
...
TrPage._getNodeName = function (element) {
  ...
  if (!nodeName) nodeName = element.tagName;
  return nodeName;
};
...
TrPage._getFirstElementWithId = function (domNode) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (childNode.nodeType == 1) {
      if (childNode.id) return childNode;
...
TrPage.prototype._loadScript = function (source) {
  ...
  if (loadedLibraries[source]) return;
  ...
  if (xmlHttp.getStatus() == 200) {
    ...
    if (responseText) {
      if (_agent.isIE) window.execScript(responseText);
      else window.eval(responseText);
    }
  }
...
TrPage.prototype._handlePprResponseScript = function (scriptNode) {
  ...
  if (source) {
    ...
    if (nodeText) {
      if (_agent.isIE) window.execScript(nodeText);
      else window.eval(nodeText);
    }
  }
};
...
TrPage._getTextContent = function (element) {
  if (_agent.isIE) {
    ...
    if (textContent == undefined) textContent = element.text;
  ...
  if (_agent.isSafari) {
    ...
    while (currChild) {
      ...
      if (nodeType == 3 || nodeType == 4) text = text + currChild.data;
...
TrPage._collectLoadedLibraries = function () {
  ...
  for (var i = 0; i < scripts.length; i++) {
    ...
    if (src) loadedLibraries[src] = true;
  }
...
TrPage.prototype.addStyleClassMap = function (styleClassMap) {
  if (!styleClassMap) return;

  if (!this._styleClassMap) this._styleClassMap = new Object();

  // Copy key:styleClass pairs to internal map
  for (var key in styleClassMap) this._styleClassMap[key] = styleClassMap[key];
};
