...
function TrMessageBox(a0) {
  ...
  TrMessageBox._LIST_STYLE =
    TrPage.getInstance().getStyleClass("af|messages::list");
  TrMessageBox._LIST_SINGLE_STYLE = TrPage.getInstance().getStyleClass(
...
TrMessageBox.prototype.addMessage = function (a1, a2, a3) {
  ...
  if (a4.hasChildNodes()) {
    ...
    if (a8.length == 1) a4.className = TrMessageBox._LIST_SINGLE_STYLE;
    else a4.className = TrMessageBox._LIST_STYLE;
  }
  this._showMessageBox();
};
TrMessageBox.prototype.removeMessages = function (a9) {
  var a10 = this._getMessageList();
  if (!a10.hasChildNodes()) return;
  ...
  if (a12.length == 0) this._hideMessageBox();
  else if (a12.length == 1) a10.className = TrMessageBox._LIST_SINGLE_STYLE;
  else a10.className = TrMessageBox._LIST_STYLE;
};
TrMessageBox.prototype._getMessageBox = function () {
  if (this._messageBoxId == null) return null;
...
TrMessageBox.prototype._getMessageNameForInput = function (a17) {
  if (!this._messageBoxId || !a17) return null;
