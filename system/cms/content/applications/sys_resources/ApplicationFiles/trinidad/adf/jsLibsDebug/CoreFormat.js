...
TrIntegerConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrIntegerConverter.prototype.getAsObject = function (numberString, label) {
...
function TrLongConverter(message, maxPrecision, maxScale, maxValue, minValue) {
  this._message = message;
  this._maxPrecision = maxPrecision;
  this._maxScale = maxScale;
  this._maxValue = maxValue;
...
TrLongConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrLongConverter.prototype.getAsObject = function (numberString, label) {
  return _decimalParse(
    numberString,
    this._message,
...
function TrShortConverter(message, maxPrecision, maxScale, maxValue, minValue) {
  this._message = message;
  this._maxPrecision = maxPrecision;
  this._maxScale = maxScale;
  this._maxValue = maxValue;
...
TrShortConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrShortConverter.prototype.getAsObject = function (numberString, label) {
  return _decimalParse(
    numberString,
    this._message,
...
function TrByteConverter(message, maxPrecision, maxScale, maxValue, minValue) {
  this._message = message;
  this._maxPrecision = maxPrecision;
  this._maxScale = maxScale;
  this._maxValue = maxValue;
...
TrByteConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrByteConverter.prototype.getAsObject = function (numberString, label) {
  return _decimalParse(
    numberString,
    this._message,
...
TrDoubleConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrDoubleConverter.prototype.getAsObject = function (numberString, label) {
  return _decimalParse(
    numberString,
    this._message,
...
function TrFloatConverter(message, maxPrecision, maxScale, maxValue, minValue) {
  this._message = message;
  this._maxPrecision = maxPrecision;
  this._maxScale = maxScale;
  this._maxValue = maxValue;
...
TrFloatConverter.prototype.getAsString = function (number, label) {
  return "" + number;
};

TrFloatConverter.prototype.getAsObject = function (numberString, label) {
  return _decimalParse(
    numberString,
    this._message,
...
function TrRangeValidator(maxValue, minValue, messages) {
  this._maxValue = maxValue;
  this._minValue = minValue;
...
TrRangeValidator.prototype.getHints = function (converter) {
...
TrRangeValidator.prototype.validate = function (value, label, converter) {
  string = "" + value;
...
function TrLengthValidator(maxValue, minValue, messages) {
  this._maxValue = maxValue;
  this._minValue = minValue;
  this._messages = messages;
...
TrLengthValidator.prototype.getHints = function (converter) {
  return _returnRangeHints(
    this._messages,
    this._maxValue,
    this._minValue,
...
TrLengthValidator.prototype.validate = function (value, label, converter) {
  var string = "" + value;
  var length = string.length;

  // If validation succeeds, return
  if (
    length >= this._minValue &&
...
function TrDateTimeRangeValidator(maxValue, minValue, messages) {
  this._maxValue = maxValue;
  this._minValue = minValue;
  this._messages = messages;
  // for debugging
...
TrDateTimeRangeValidator.prototype.getHints = function (converter) {
  var max = null;
  var min = null;
  if (this._maxValue) max = converter.getAsString(new Date(this._maxValue));
  if (this._minValue) min = converter.getAsString(new Date(this._minValue));
...
function TrDateRestrictionValidator(weekdaysValue, monthValue, messages) {
...
TrDateRestrictionValidator.prototype.getHints = function (converter) {
...
function _decimalParse(
  ...
  if (symbols) {
    ...
    if (
      numberString.indexOf(grouping) == 0 ||
  ...
  if (
    ...
    numberString.indexOf("E") < 0 &&
    (numberString * numberString == 0 || numberString / numberString == 1)
  ) {
    var result = null;
    var floater = false;
    if (parsefloat != null) {
    ...
    if (!floater && !isNaN(result)) {
      ...
      if (messageKey != null) {
        ...
        if (messages == null || messages[messageKey] == null)
        ...
        // default error format
          ...
          facesMessage = _createFacesMessage(messageKey, label, numberString);
          throw new TrConverterException(facesMessage);
        }
      }
      return result;
  ...
  if (standardKey.indexOf("NumberConverter") == -1) {
    ...
    if (message && message["number"]) {
      facesMessage = _createCustomFacesMessage(
        TrMessageFactory.getSummaryString(usedKey),
        message["number"],
        label,
        numberString
      );
  ...
  if (!custom) {
    facesMessage = _createFacesMessage(usedKey, label, numberString);
  }

  throw new TrConverterException(facesMessage);
}

function TrRegExpValidator(pattern, messages) {
...
TrRegExpValidator.prototype.getHints = function (converter) {
  var hints = null;
...
function _returnRangeHints(
  ...
  if (max && min) {
    ...
    if (messages && messages[rangeHint]) {
      ...
      hints.push(TrMessageFactory.createMessage(rangeKey, "" + min, "" + max));
    }
    return hints;
  }

  return _returnHints(messages, max, min, maxKey, minKey, maxHint, minHint);
}
function _returnHints(messages, max, min, maxKey, minKey, maxHint, minHint) {
  var hints;
  if (max) {
    hints = new Array();
    if (messages && messages[maxHint]) {
      hints.push(
        TrMessageFactory.createCustomMessage(messages[maxHint], "" + max)
      );
    } else {
      hints.push(TrMessageFactory.createMessage(maxKey, "" + max));
    }
  }
  if (min) {
    if (!hints) {
    ...
    if (messages && messages[minHint]) {
      hints.push(
        TrMessageFactory.createCustomMessage(messages[minHint], "" + min)
      );
    } else {
      hints.push(TrMessageFactory.createMessage(minKey, "" + min));
    }
  }
  return hints;
}
