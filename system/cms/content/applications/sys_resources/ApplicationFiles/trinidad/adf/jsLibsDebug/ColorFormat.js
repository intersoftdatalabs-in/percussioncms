...
TrColorConverter.prototype.getAsString = function (formatColor) {
  //pu: Return undefined string for an undefined Color.
  if (formatColor == null) return null;

  // return localized transparent text for transparent color
  if (formatColor.alpha == 0) return _cfTrans;
  ...
  _cfoDoClumping(pattern, _cfoSubformat, formatColor, stringHolder);
 ...
 */
TrColorConverter.prototype.getAsObject = function (parseString, label) {
  ...
  if (typeof pattern == "string") {
    return this._rgbColorParseImpl(parseString, pattern, facesMessage);
  } else {
    var i;
    for (i = 0; i < pattern.length; i++) {
      try {
        var color = this._rgbColorParseImpl(
          parseString,
          pattern[i],
          facesMessage
        );
...
TrColorConverter.prototype._rgbColorParseImpl = function (
  ...
  // parse the color
  if (_cfoDoClumping(parsePattern, _cfoSubParse, parseContext, parsedColor)) {
...
function TrColor(red, green, blue, alpha) {
  // for debugging
  this._class = "TrColor";

  if (alpha == null) alpha = 0xff;
 ...
 * succeeded.
 */
function _cfoDoClumping(formatPattern, subFunction, param, outValue) {
  ...
  if (
    ...
    switch (formatType) {
      ...
      case "R": // hex red component (0x00-0xff)
        appendString = _cfoGetPaddedNumber(
          color.red,
          charCount,
          2,
          16
...
function _cfoSubParse(
  ...
  if (
    ...
    switch (formatType) {
      ...
      case "g": // decimal green component (0-255)
        parsedColor.green = _cfoAccumulateNumber(
          parseContext,
          charCount,
          3,
          10
      ...
      case "a": // decimal alpha component (0-255)
        parsedColor.alpha = _cfoAccumulateNumber(
          parseContext,
          charCount,
          3,
          10
 ...
 */
function _cfoMatchText(parseContext, text) {
 ...
 * there is no number.
 */
function _cfoAccumulateNumber(parseContext, minDigits, maxDigits, base) {
  var startIndex = parseContext.currIndex;
  var currIndex = startIndex;
  var parseString = parseContext.parseString;
  var parseLength = parseString.length;
  if (parseLength > currIndex + maxDigits) parseLength = currIndex + maxDigits;
  ...
  if (startIndex != currIndex && currIndex - startIndex >= minDigits) {
    // update the current parseContext
 ...
 */
function _cfoGetPaddedNumber(number, minDigits, maxDigits, base) {
