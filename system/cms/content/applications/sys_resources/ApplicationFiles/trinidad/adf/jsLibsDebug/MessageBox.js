...
function TrMessageBox(messageBoxId) {
  if (messageBoxId == undefined) return;
...
TrMessageBox.prototype.addMessage = function (inputId, label, facesMessage) {
  ...
  if (inputId) {
    ...
    if (!label) label = facesMessage.getSummary();
    ...
    var textNode = document.createTextNode(
      facesMessage.getSummary() + " - " + facesMessage.getDetail()
    );
    line.appendChild(textNode);
  }
  ...
  if (listElement.hasChildNodes()) {
    ...
    else listElement.className = TrMessageBox._LIST_STYLE;
  }
...
TrMessageBox.prototype.removeMessages = function (inputId) {
  ...
  if (!listElement.hasChildNodes()) return;
  ...
  if (children.length == 0) this._hideMessageBox();
...
TrMessageBox.prototype._getMessageNameForInput = function (inputId) {
  if (!this._messageBoxId || !inputId) return null;
