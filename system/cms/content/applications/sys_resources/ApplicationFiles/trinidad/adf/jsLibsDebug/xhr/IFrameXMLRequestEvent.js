 ...
 */
function TrIFrameXMLRequestEvent(iframeDoc) {
...
TrIFrameXMLRequestEvent.prototype.getResponseXML = function () {
  ...
  if (agentIsIE && iframeDoc.XMLDocument) return iframeDoc.XMLDocument;
  else return iframeDoc;
};
...
TrIFrameXMLRequestEvent.prototype.getResponseText = function () {
  ...
  if (agentIsIE && iframeDoc.XMLDocument) xmlDocument = iframeDoc.XMLDocument;
  else if (window.XMLDocument && iframeDoc instanceof XMLDocument)
    xmlDocument = iframeDoc;

  if (xmlDocument) return AdfAgent.AGENT.getNodeXml(xmlDocument);
  else return iframeDoc.documentElement.innerHTML;
};
...
TrIFrameXMLRequestEvent.prototype._isResponseValidXML = function () {
  ...
  if (agentIsIE && iframeDoc.XMLDocument) return true;
  else if (window.XMLDocument && iframeDoc instanceof XMLDocument) return true;
  else return false;
};
...
TrIFrameXMLRequestEvent.prototype.isPprResponse = function () {
  ...
  if (agentIsIE && iframeDoc.XMLDocument) {
    var xmlDocument = iframeDoc.XMLDocument,
      childNodes = xmlDocument.childNodes;
    // In IE the xml PI is the first node
    if (
      childNodes.length >= 2 &&
      childNodes[1].nodeName == "Tr-XHR-Response-Type"
    )
      isRichReponse = true;
  } else {
    if (
      iframeDoc.firstChild &&
      iframeDoc.firstChild.nodeName == "Tr-XHR-Response-Type"
    )
      isRichReponse = true;
  }
...
TrIFrameXMLRequestEvent.prototype.getResponseContentType = function () {
  if (this._isResponseValidXML()) return "text/xml";
