...
(function (rb) {
  ...
  var d = document,
    ...
    setPosition = function (wh, t, cl) {
      ...
      var width = wh == "auto" ? w.innerWidth : wh[0],
        ...
        style =
          "width:" + width + "px;height:" + height + "px;margin-top:20px;";
  ...
  readyElement("percwrapper", function () {
    ...
    if (
      window.chrome ||
      (window.getComputedStyle && !window.globalStorage && !window.opera)
    ) {
    }
    [].forEach.call(document.querySelectorAll("#percdevices a"), function (el) {
      el.addEventListener("click", function (e) {
        [].forEach.call(
          document.querySelectorAll("#percdevices a"),
          function (el) {
        ...
        var self = this;
        if (
          (self.className.match("perc-auto") && isResized === false) ||
          isAnimated === true
        )
          return false;
        isAnimated = true;
    ...
    close.addEventListener(
      ...
      function (e) {
        if (
          ...
          d.getElementById("percexpand").className =
            "icon-double-angle-down icon-2x fas fa-angle-double-down fa-2x";
          d.getElementById("percexpand").title =
            "Click to collapse mobile preview toolbar";
          ...
          d.getElementById("perccollapse").className =
            "icon-double-angle-up icon-2x fas fa-angle-double-up fa-2x";
          d.getElementById("perccollapse").title =
            "Click to expand mobile preview toolbar";
...
window.mobilePreviewFrameOnload = function () {
  try {
    window.frames["percmobilepreviewframe"].document.body.style.overflowX =
      "hidden";
    ...
    for (var i = 0; i < alllinks.length; i++) {
      if (
        alllinks[i].target == "" ||
        alllinks[i].target.toLowerCase() == "_self"
      )
      ...
      if (hrefattr.indexOf("/Sites") != -1 && !isAnchor) {
        ...
        if (prevmode) h["percpreviewmode"] = prevmode;
        // replace() removes absolute ref from start of url
        alllinks[i].href = PSHash2Href(h, hrefattr).replace(
          /^.*\/\/[^\/]+/,
          "",
