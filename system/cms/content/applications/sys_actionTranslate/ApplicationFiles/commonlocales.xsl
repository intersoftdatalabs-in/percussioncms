...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="childlocalesurl"
		select="concat(//@childlocalesurl,$childids)" />
	<xsl:variable name="childlocales"
		select="$childlocalesurl" />
	...
	<xsl:value-of select="$transitionedlocales">
		<xsl:for-each select="//item">
			...
			<item contentid="{@contentid}">
				...
				<xsl:for-each
					select="document($childlocales)//item[@contentid=$contentid]/locale">
					<locale name="{@name}" />
				</xsl:for-each>
			</item>
		</xsl:for-each>
	...
	<xsl:value-of select="$untransitionedlocales">
		<xsl:for-each
			select="document($transitionedlocales)//item">

			<item contentid="{@contentid}">
				<xsl:for-each
					select="document($locales)//PSXEntry[not(Value=$trlocales/@name)]">
	...
	<xsl:value-of select="$commonuntransitionedlocales">
		<xsl:variable name="itemcount"
			select="count($untransitionedlocales//item)" />
		<xsl:for-each
			select="document($untransitionedlocales)//item[position()=1]/locale">
			<xsl:variable name="name" select="@name" />
			<xsl:variable name="displaytext" select="@displaytext" />
			<xsl:if
				test="count($untransitionedlocales//item[locale[@name=$name]])=$itemcount">
	...
	<xsl:template match="/">
		<commonlocales>
			<xsl:apply-templates
				select="document($commonuntransitionedlocales)" mode="copy" />
		</commonlocales>
	</xsl:template>
