...
<xsl:stylesheet version="1.1"
	...
	<xsl:template mode="editvariantslot_mainbody" match="*">
		...
		<xsl:variable name="contvariantid"
			select="/*/contvariantidurl" />
			...
			<tr class="headercell">
				<td>
					<table width="100%" cellpadding="0" cellspacing="1"
						border="0">
						<xsl:apply-templates
							select="document($contvariantid)" mode="mode5" />
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <xsl:value-of select="'datacell2'" />
								...
								<xsl:choose>
									<xsl:when
										test="not(string-length(./slottypeid)) and position()=1">
							...
							<xsl:value-of select="slottypeid" />
											...
											<xsl:if test="string-length(./slottypeid)">
												<a href="{./sloturl}">
													<img height="17" alt="Add"
														src="../sys_resources/images/new.gif" width="17"
														border="0" />
												</a>
											</xsl:if>
										</td>
       ...
       <xsl:value-of select="variantid" />
			...
			<tr class="headercell">        <!-- Repeats once per category -->
				<td valign="top" align="left" class="headercellfont" colspan="2">
					Edit&nbsp;
					<xsl:apply-templates select="contenttype" />&nbsp;Variant
					Slot
				</td>
			</tr>
			<tr class="headercell">        <!-- Repeats once per category -->
				<td valign="top" align="left" class="headercellfont" colspan="2">
					Variant Name:&nbsp;
					<xsl:apply-templates select="varianttype" />&nbsp;(
