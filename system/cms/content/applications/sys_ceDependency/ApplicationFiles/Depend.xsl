...
<xsl:stylesheet version="1.1"
	...
	<xsl:variable name="searchurl"
		select="document(//searchurl)//url" />
	<xsl:variable name="showRelationshiptype">
		<xsl:value-of
			select="//Relationships/@showRelationshiptype" />
	</xsl:variable>
	<!-- begin XSL -->
	<xsl:template match="Item[string-length(@contentid)=0]"
		mode="childitem" priority="10" />
			...
			<tr class="datacell1">
				<td class="datacell1font">
					<a href="javascript:void(0);">
						<xsl:attribute name="onclick">javascript:openCA("<xsl:value-of
							select="concat($searchurl,'&amp;statusid=',@contentid)" />")</xsl:attribute>
	...
	<xsl:template match="Workflow[@stateValid='y']"
		mode="childitem">
		<b>
			<xsl:value-of select="State" />
		</b>
	...
	<!-- end XSL -->
	<xsl:include
		href="file:sys_resources/stylesheets/UserStatusA.xsl" />
	<xsl:include
		href="file:sys_resources/stylesheets/Banner.xsl" />
	<xsl:variable name="this" select="/" />
	<xsl:variable name="createcontentlookup"
		select="/*/createcontentlookupurl" />
		...
		<html>
			<head>
				...
				<title>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceDependency.Depend@Rhythmyx - Content Editor - Dependency Viewer'" />
				...
				<script>
					...
					*
					* @param url the page to refresh, not
					<code>null</code>
					.
			...
			<body leftmargin="5" rightmargin="5" topmargin="5">
				<xsl:attribute name="sessionid"><xsl:value-of
					select="*/sessionid" /></xsl:attribute>
		...
		<xsl:for-each select=".">
			<tr class="datacell1">
				<td valign="top" align="left" class="outerboxcellfont">
					...
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceDependency.Depend@Parent Items of'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
					:&nbsp;
			...
			<tr class="datacell1">
				<td valign="top" align="left" class="headercellfont">
					<em>&nbsp;*
						<xsl:call-template name="getLocaleString">
							<xsl:with-param name="key"
								select="'psx.sys_ceDependency.Depend@indicates branch already expanded elsewhere'" />
							<xsl:with-param name="lang" select="$lang" />
						</xsl:call-template>
					</em>
				</td>
		...
		<xsl:for-each select=".">
			<tr class="datacell1">
				...
				<td valign="top" align="left" class="outerboxcellfont">
					...
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'psx.sys_ceDependency.Depend@Child Items of'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>:&nbsp;
					<xsl:apply-templates select="title" />
					(
	...
	<!-- This template displays a drop down control with all relationship types
		available in the system. By default 'Related Content' is selected. If the
		selection is changed, the window will be refreshed for the new selected relationship
		type. -->
	<xsl:template match="Relationships">
		<select
			onchange="javascript:refresh(window.location, this.value)"
			name="sys_relationshiptype" class="datadisplay">
			<xsl:for-each select="Relationship">
				...
				<option value="{$id}">
					...
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="concat(psx.relationship.type, $id)" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
				</option>
			</xsl:for-each>
		...
		<xsl:for-each select=".">
						...
						<tr>
							...
							<td align="right" valign="top">
								<span class="outerboxcellfont">
									<xsl:call-template name="getLocaleString">
										<xsl:with-param name="key"
											select="'psx.sys_ceDependency.Depend@Dependency Viewer'" />
										<xsl:with-param name="lang" select="$lang" />
									</xsl:call-template>
								</span>
							</td>
			...
			<tr class="datacell1">
				<td height="100%" align="center">
					<input type="button" name="gotoparent" value="Close"
	...
	<psxi18n:lookupkeys>
		<key
			name="psx.sys_ceDependency.Depend@Rhythmyx - Content Editor - Dependency Viewer">Title for Dependency viewer.</key>
		<key name="psx.sys_ceDependency.Depend@Parent Items of">Header for parent items.</key>
		<key
			name="psx.sys_ceDependency.Depend@indicates branch already expanded elsewhere">Note explaining branch already expanded elsewhere.</key>
