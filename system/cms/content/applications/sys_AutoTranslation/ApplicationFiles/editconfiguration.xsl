...
<xsl:stylesheet version="1.1"
	...
	<xsl:include
		href="file:sys_AutoTranslation/editconfiguration_body.xsl" />
	<xsl:include
		href="file:sys_resources/stylesheets/viewpaging.xsl" />
	<xsl:variable name="this" select="/" />
	<xsl:variable name="relatedlinks" select="/*/relatedlinks" />
	<xsl:variable name="configparamsdoc"
		select="document(/*/configparamsurl)" />
	<xsl:template match="/">
		<html>
			<head>
               ...
               function community(id, name, contenttypes)
               {
				...
				<script>
					function cancelFunc() {
					document.location.href=document.editconfiguration.doccancelurl.value+"?
					sys_componentname="+document.editconfiguration.sys_componentname.value+"&amp;sys_pagename="+document.editconfiguration.sys_pagename.value;
					}
				...
				<table width="100%" cellpadding="0" cellspacing="1" border="0">
					<tr>
						...
						<td align="middle" width="100%" valign="top" height="100%"
							...
							<xsl:apply-templates
								mode="editconfiguration_mainbody" />
							<!-- end main body slot -->
						</td>
					</tr>
				</table>
