...
<xsl:stylesheet version="1.1"
	...
	<xsl:template mode="editconfiguration_mainbody" match="*">
		<form name="editconfiguration" method="post"
			action="updateconfiguration.html">
					...
					<td>
						<table width="100%" cellpadding="0" cellspacing="1"
							...
							<tr class="headercell2">
								<td width="30%" align="left" class="headercell2font">Content&nbsp;Type
									:&nbsp;Locale
								</td>
								<td width="90%" align="left" class="headercell2font">&nbsp;
									<xsl:value-of select="//contenttypename" />
									<xsl:if test="//contenttypename!=''">
										:
									</xsl:if>
							...
							<tr class="datacell1" border="0">
								...
								<td align="left" class="datacell1font">
									<select name="sys_community"
										onchange="return community_onchange('no')">
								...
								<td align="left" class="datacell2font">
									<xsl:choose>
										<xsl:when test="//contenttypeid!=''">
											<xsl:value-of select="//contenttypename" />
											<input type="hidden" name="sys_contenttype"
												value="{//contenttypeid}" />
										</xsl:when>
										<xsl:otherwise>
											<select name="sys_contenttype"
												onchange="return contenttype_onchange('no')">
												<option>Leave this for NS</option>
												<option>1</option>
												<option>2</option>
												<option>3</option>
								...
								<td align="left" class="datacell1font">
									<xsl:choose>
										<xsl:when test="//locale!=''">
											<xsl:value-of select="//localename" />
											<input type="hidden" name="sys_locale"
												value="{//locale}" />
										</xsl:when>
										<xsl:otherwise>
							...
							<tr class="datacell1">
								<td colspan="2">
									<input type="button" value="Save" class="nav_body"
										name="save"
										onclick="javascript:document.editconfiguration.submit()" />&nbsp;
