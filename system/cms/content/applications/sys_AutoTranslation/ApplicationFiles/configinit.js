function onFormLoad(init) {
  ...
  for (i = 0; i < communities.length; i++) {
    document.editconfiguration.sys_community.options[i].value =
      communities[i].id;
    document.editconfiguration.sys_community.options[i].text =
      communities[i].name;
    if (
      init == "yes" &&
      communities[i].id == document.editconfiguration.communityid.value
    ) {
...
function community_onchange(init) {
  ...
  if (document.editconfiguration.sys_contenttype.type == "select-one") {
    document.editconfiguration.sys_contenttype.options.length =
      contenttypes.length;
    for (i = 0; i < contenttypes.length; i++) {
      document.editconfiguration.sys_contenttype.options[i].value =
        contenttypes[i].id;
      document.editconfiguration.sys_contenttype.options[i].text =
        contenttypes[i].name;
      if (
        init == "yes" &&
        contenttypes[i].id == document.editconfiguration.contenttypeid.value
      ) {
...
function contenttype_onchange(init) {
  ...
  if (document.editconfiguration.sys_contenttype.type == "select-one") {
    ...
    for (i = 0; i < contenttypes.length; i++) {
      if (
        contenttypes[i].id == document.editconfiguration.sys_contenttype.value
      ) {
        index = i;
        break;
      }
  ...
  for (i = 0; i < workflows.length; i++) {
    ...
    if (
      init == "yes" &&
      workflows[i].id == document.editconfiguration.workflowid.value
    ) {
      document.editconfiguration.sys_workflow.options[i].selected = true;
    }
  }
  if (init != "yes") document.editconfiguration.sys_workflow.selectedIndex = 0;
  if (document.editconfiguration.sys_locale.type == "select-one") {
    ...
    for (i = 0; i < locales.length; i++) {
      ...
      if (
        init == "yes" &&
        locales[i].id == document.editconfiguration.locale.value
      ) {
        document.editconfiguration.sys_locale.options[i].selected = true;
      }
    }
    if (init != "yes") document.editconfiguration.sys_locale.selectedIndex = 0;
  }
}
