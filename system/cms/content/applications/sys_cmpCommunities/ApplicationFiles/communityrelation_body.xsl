			...
			<tr class="outerboxcell">
				<td class="outerboxcell" align="right" valign="top">
					<span class="outerboxcellfont">
						<xsl:value-of select="reltypename" />s&nbsp;for
						<a href="{communityurl}">
							<xsl:value-of select="communityname" />
						</a>
						(
						<xsl:value-of select="communityid" />)&nbsp;Community&nbsp;
					</span>
				</td>
			</tr>
			<tr class="headercell">
				<td align="right" valign="top">
					<span class="headercellfont">
						<a href="{addurl}">Add&nbsp;
			...
			<tr class="headercell">
				<td>
					<table width="100%" cellpadding="0" cellspacing="1"
						border="0">
						<form name="editcommunity" method="post"
							action="updatecommunity.html">
							...
							<xsl:choose>
								<xsl:when test="reltypename='Variant'">
									<!-- for variants we want to show what content type they represent,
										so the xml and output layout are different -->
									<xsl:choose>
										<xsl:when test="contenttype/list/id=''">
											<xsl:call-template name="NoEntries" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:apply-templates mode="ShowVariants"
												select="contenttype" />
										</xsl:otherwise>
									</xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										...
										<xsl:otherwise>
											...
											<xsl:apply-templates mode="ShowOthers"
												select="list" />
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
