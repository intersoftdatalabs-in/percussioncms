...
<xsl:stylesheet version="1.1"
			...
			<tr class="outerboxcell">
				<td class="outerboxcell" align="right" valign="top" colspan="2">
					<span class="outerboxcellfont">
						Add
						<xsl:value-of select="reltypename" />&nbsp;for&nbsp;
						<a href="{communityurl}">
					...
					<table width="100%" cellpadding="0" cellspacing="1"
						...
						<form name="addcommrelation" method="post">
							<xsl:attribute name="action"><xsl:value-of
								select="updateurl" /></xsl:attribute>
							...
							<input name="communityid" type="hidden">
								<xsl:attribute name="value"><xsl:value-of
									select="communityid" /></xsl:attribute>
							</input>
							<input name="communityname" type="hidden">
								<xsl:attribute name="value"><xsl:value-of
									select="communityname" /></xsl:attribute>
							</input>
							<xsl:choose>
								<xsl:when test="reltypename='Variants'">
									<!-- for variants we want to show what content type they represent,
										so the xml and html output are different -->
									<xsl:choose>
										<xsl:when test="contenttype/list/id=''">
											<xsl:call-template name="NoEntries" />
										</xsl:when>
										<xsl:otherwise>
											<xsl:apply-templates mode="AddVariants"
												select="contenttype" />
								...
								<xsl:otherwise>
									<xsl:choose>
										...
										<xsl:otherwise>
											...
											<xsl:apply-templates mode="AddOthers"
												select="list" />
											<xsl:call-template name="ShowButtons" />
										</xsl:otherwise>
									</xsl:choose>
							...
							<tr>
								<td align="center">
									<xsl:attribute name="colspan"><xsl:choose><xsl:when
										test="list/desc">3</xsl:when><xsl:otherwise>2</xsl:otherwise></xsl:choose></xsl:attribute>
		...
		<tr class="headercell2">
			...
			<xsl:choose>
				<xsl:when test="list/desc">
					...
					<td width="50%" align="left" class="headercell2font">Description&nbsp;
					</td>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="commrelationlookup" mode="rellookup">
		...
		<table width="100%" cellpadding="0" cellspacing="1" border="0">
				...
				<td colspan="2">
					<table width="100%" cellpadding="0" cellspacing="1"
						...
						<td class="outerboxcell" align="left" valign="top" width="50%">
							<span class="outerboxcellfont">
								<xsl:value-of select="reltypename" />&nbsp;&nbsp;
							</span>
						</td>
						<td valign="top" align="right" width="50%">
							<a>
								<xsl:attribute name="href"><xsl:value-of
									select="Addurl" /></xsl:attribute>
								<b>Add</b>
							</a>
					&nbsp;&nbsp;&nbsp;
						</td>
			...
			<tr class="headercell2">
				...
				<td width="95%" align="left" class="headercell2font">Name&nbsp;(ID)&nbsp;&nbsp;&nbsp;
				</td>
			</tr>
			<xsl:for-each select="list">
				<tr class="datacell1">
	...
	<xsl:template name="ShowListHeader">
		<tr class="headercell2">
			...
			<xsl:choose>
				<xsl:when test="list/desc">
					<td width="45%" align="left" class="headercell2font">Name&nbsp;(ID)&nbsp;&nbsp;&nbsp;
					</td>
					<td width="50%" align="left" class="headercell2font">Description&nbsp;
					</td>
				</xsl:when>
	...
	<xsl:template name="ShowButtons">
		<tr class="datacell1">
			<td colspan="2">
				...
				<input type="button" value="Save" class="nav_body" name="save"
				...
				<input type="button" value="Select All"
					onclick="javascript:setChecked(1);" name="B3" />&nbsp;
				<input type="button" value="Clear All"
					onclick="javascript:setChecked(0)" name="ResetButton" />
			</td>
		</tr>
	</xsl:template>
</xsl:stylesheet>
