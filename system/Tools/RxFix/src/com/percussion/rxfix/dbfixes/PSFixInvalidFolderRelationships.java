...
public class PSFixInvalidFolderRelationships extends PSFixDBBase implements IPSFix
{
   ...
   PSStringTemplate ms_findBrokenRelationshipQuery = new PSStringTemplate(
         "SELECT RID FROM {schema}." + IPSConstants.PSX_RELATIONSHIPS
               + " WHERE (CONFIG_ID=3 OR CONFIG_ID=8) AND OWNER_ID NOT IN (SELECT CONTENTID "
               + "FROM {schema}.CONTENTSTATUS where OBJECTTYPE=2 )");
    ...
    */
   public PSFixInvalidFolderRelationships() throws NamingException, SQLException {
      super();
   }
   ...
   {
      return "Remove folder relationships where owner is not a folder";
   }
}
