...
public class ImageReaderTest {
   ...
   private void testImage(String resourceFileName) throws IOException {
      ...
      BufferedImage bufferedImage =
            ImageIO.read(getClass().getClassLoader().getResourceAsStream(resourcePath));
      testResize(resourcePath);
      Assert.assertNotNull("Buffered image is null after read:" + resourcePath, bufferedImage);
   }
   ...
   private boolean testResize(String resourcePath) {
      ...
      try {
         ...
         ImageData resizedImage = resizeManager
               .generateImage(getClass().getClassLoader().getResourceAsStream(resourcePath));

         Assert.assertTrue("Invalid ImageData for generateImage(InputStream input)",
   ...
   private BufferedImage readImage(byte[] imageBytes) {
      ...
      try {
         ...
         System.out.print("Image Read Time: " + (System.currentTimeMillis() - startTime) + " ms\n");
   ...
   private byte[] readBytesForImageResource(String resourceLocation) {
      InputStream inputStream = getClass().getClassLoader().getResourceAsStream(resourceLocation);
