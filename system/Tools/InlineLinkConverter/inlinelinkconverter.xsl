...
<xsl:stylesheet version="1.1"
	...
	<xsl:template
		match="html:a[@sys_contentid] | a[@sys_contentid]" mode="rxbodyfield"
		priority="5">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="sys_dependentid"><xsl:value-of
				select="@sys_contentid" /></xsl:attribute>
			<xsl:attribute name="sys_dependentvariantid"><xsl:value-of
				select="@sys_variantid" /></xsl:attribute>
	...
	<xsl:template
		match="html:img[@sys_contentid] | img[@sys_contentid]"
		mode="rxbodyfield" priority="5">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="sys_dependentid"><xsl:value-of
				select="@sys_contentid" /></xsl:attribute>
	...
	<xsl:template match="*[@rxinlineslot]" mode="rxbodyfield"
		priority="10">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:apply-templates mode="rxbodyfield" />
