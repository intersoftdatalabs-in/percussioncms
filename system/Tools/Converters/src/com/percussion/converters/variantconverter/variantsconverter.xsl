...
<xsl:stylesheet version="1.1"
	...
	<!-- Converter tool fills the variant map varible with the values from properties
		file. -->
	...
	<xsl:template match="@sys_dependentvariantid"
		...
		<xsl:choose>
			<xsl:when test="$oldVar=$variantMap//variant/@old">
				<xsl:attribute name="sys_dependentvariantid"><xsl:value-of
					select="$variantMap//variant[@old = $oldVar]/@new" /></xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy>
					<xsl:copy-of select="@*" />
	...
	<xsl:template match="@sys_variantid" mode="rxbodyfield"
		priority="100">
		<xsl:variable name="oldVar" select="." />
		<xsl:choose>
			<xsl:when test="$oldVar=$variantMap//variant/@old">
				<xsl:attribute name="sys_variantid"><xsl:value-of
					select="$variantMap//variant[@old = $oldVar]/@new" /></xsl:attribute>
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy>
