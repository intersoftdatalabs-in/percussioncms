...
public class PSMembershipAuthProvider extends AbstractUserDetailsAuthenticationProvider {
  ...
  /**
   * Get the current thread's session id for the authenticated subject, calling this method clears
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    ...
    // Check what kind of request is this. Membership or ldap secure membership? Then proceed
    // accordingly.

    if (membershipConfig != null
        && membershipConfig.getUseLdap() != null
        && membershipConfig.getUseLdap().equalsIgnoreCase("yes")) {
  ...
  protected UserDetails retrieveUser(
    ...
    if (StringUtils.isEmpty(groups)) return new User(username, "", authorities);
    ...
    for (int i = 0; i < groupList.length; i++) {
      ...
      if (groupsFromFile != null
          ...
          && groupsFromFile.contains("'" + group.toUpperCase() + "'"))
        authorities.add(new SimpleGrantedAuthority(group));
    }

    if (!authorities.isEmpty()) return new User(username, "", authorities);
  ...
  private String authenticateMember(String userId, String password) throws JSONException {
    ...
    if (!"SUCCESS".equals(status)) {
      if ("AUTH_FAILED".equals(status)) throw new BadCredentialsException("");
   ...
   * @return The list of groups as a comma-delimited string, upper-cased to support
   *     case-insensitivity.
   * @throws JSONException If there is an error parsing a REST response
   */
