...
jQuery.fn.splitter = function (opts) {
  ...
  return this.each(function () {
    ...
    function doSplitKey(e) {
      ...
      if (dir) moveSplitter(paneA[0][opts.offsetAdjust] + dir * opts.pxPerKey);
      return true; // required???
    }
    ...
    function moveSplitter(np) {
      ...
      np = Math.max(
        paneA._min + paneA._padAdjust,
        group._adjust - (paneB._max || 9999),
        16,
        Math.min(
          np,
          paneA._max || 9999,
    ...
    function cssCache(jq, n, pf, m1, m2) {
      ...
      jq[n] = jQuery.boxModel
        ? (parseInt(jq.css(pf + m1)) || 0) + (parseInt(jq.css(pf + m2)) || 0)
        : 0;
    }
    ...
    var focuser = jQuery('<a href="javascript:void(0)"></a>')
      .bind("focus", startSplit)
      .bind("keydown", doSplitKey)
      .bind("blur", endSplit)
    ...
    paneA._init =
      (opts.initA == true
        ? parseInt(jQuery.curCSS(paneA[0], opts.adjust))
        : opts.initA) || 0;
    paneB._init =
      (opts.initB == true
        ? parseInt(jQuery.curCSS(paneB[0], opts.adjust))
        : opts.initB) || 0;
    if (paneB._init)
      paneB._init =
