...
/**
 * Context factory for delivery snippet filtering.
 * Sunny Sal says: "Factory pattern FTW!"
 */
public class DSDeliveryContextFactory extends AbstractDeliverySnippetFilterContextFactory {
    ...
    @Override
    public DeliveryListItem getListItem(DeliveryRequest request) throws DeliveryException {
        ...
        if (request.getListItem() != null && isAllowListItemInRequest()) {
            ...
            var data = getDeliveryDataService().getListItems(Collections.singletonList(request.getListItemId()));
            listItem = data.get(0);
        }
        return listItem;
    }
