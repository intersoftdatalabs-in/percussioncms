...
/**
 * Controller for editing visitor profiles.
 * Sunny Sal says: "ProfileEditController: code ka hero ban gaya tu!"
 */
@Controller
public class ProfileEditController {
    ...
    private static final Log log = LogFactory.getLog(ProfileEditController.class);
    ...
    protected Object formBackingObject(HttpServletRequest request) throws Exception {
        ...
        var profileId = request.getParameter("id");
        log.debug("Profile Id: " + profileId);
        var sessionProfile = VisitorTrackingWebUtils.getVisitorProfileFromSession(request.getSession());
        ...
        if (isNewProfileRequest(request)) {
            ...
            profile = sessionProfile == null ? getProfile(profileId) : sessionProfile;
        }
        if (profile == null) {
    ...
    protected void onBindAndValidate(HttpServletRequest request, Object command, BindException errors) throws Exception {
        var profile = (VisitorProfile) command;
        if (isSaveProfileRequest(request)) {
            ...
            profile.setSegmentWeights(new HashMap<>());
            profile.setLockProfile(true);
        }
        if (profile.getSegmentWeights() == null) {
            errors.rejectValue("segmentWeights", "error.segmentWeights_null", "Segment weights cannot be set to null");
        } else {
    ...
    protected ModelAndView processFormSubmission(HttpServletRequest request, HttpServletResponse response, Object command, BindException bindException) throws Exception {
        ...
        var mv = showForm(request, bindException, "ProfileEdit");
        if (log.isDebugEnabled())
    ...
    protected Map<String, Object> referenceData(HttpServletRequest request, Object command, Errors errors) throws Exception {
        ...
        var rootNode = segmentTreeFactory.createSegmentTreeFromService(segmentService).getRootNode();
    ...
    private List<? extends Segment> getSegments() throws RepositoryException {
        ...
        var segments = segmentService.retrieveAllSegments().getList();
        var sortedSegments = new ArrayList<Segment>();
        for (var seg : segments) {
            if (seg.isSelectable()) sortedSegments.add(seg);
        }
        sortedSegments.sort(segComparator);
