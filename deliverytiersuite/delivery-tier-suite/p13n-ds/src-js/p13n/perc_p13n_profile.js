...
perc_p13n.getLogger = function (loggerName) {
  if (perc_p13n._loggers[loggerName] != null)
    return perc_p13n._loggers[loggerName];
...
P13NProfileEditor.prototype.refreshProfileEditorWithTracking = function (
  ...
  this.loadProfileEditor(null, function () {
    ...
    var message =
      data && data.visitorProfile && data.visitorProfile.lockProfile
...
P13NProfileEditor.prototype.handleNodeHighLighting = function (jqNode, fwt) {
  if (jqNode == null) return;
  else if (jqNode.css("font-weight") == fwt) return;
  else if (jqNode.is(".SegmentTree")) return;
  else {
