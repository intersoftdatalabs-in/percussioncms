...
@ContextHierarchy({
  @ContextConfiguration(
      loader = PSNonValidatingGenericXMLContextLoader.class,
      locations = {"file:src/test/webapp/WEB-INF/test-beans.xml"})
})
@TestPropertySource(
...
public class PSFormServiceTest extends PSBaseFormServiceTest {
  ...
  public void testSave() {
    ...
    String[] multipleValuesField =
        new String[] {
          "multipleValuesField",
          StringUtils.join(new String[] {"first", "second", "third"}, FIELD_VALUES_SEPARATOR)
        };
    ...
    IPSFormData formData =
        generateFormData(
            ...
            fieldValue2[0],
            fieldValue2[1],
            multipleValuesField[0],
            multipleValuesField[1]);
      ...
      for (IPSFormData form : allForms) {
        if (form.getName().startsWith(INVALID_FORM_NAME_PREFIX))
          fail(
              "Found invalid form "
                  + form.getName()
                  + " that should have been filtered by service");
      }
  ...
  public void testDelete_NonExistingForm() {
    ...
    IPSFormData formData =
        generateFormData(
            "testform1", fieldValue1[0], fieldValue1[1], fieldValue2[0], fieldValue2[1]);
  ...
  public void testMarkAsExported() {
    ...
    for (int i = 0; i < 10; i++) {
      IPSFormData formData =
          generateFormData(
              "testform" + i, fieldValue1[0], fieldValue1[1], fieldValue2[0], fieldValue2[1]);
  ...
  public void testMarkAsExported_NonExistingForms() {
    ...
    for (int i = 0; i < 10; i++) {
      PSFormData formData =
          generateFormData(
              "testform1" + i, fieldValue1[0], fieldValue1[1], fieldValue2[0], fieldValue2[1]);
  ...
  public void testFindFormsByName_WithFormNameArgument_CheckOrder() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      IPSFormData formData =
          generateFormData(
              "testform", fieldValue1[0], fieldValue1[1], fieldValue2[0], fieldValue2[1]);
  ...
  private void checkFindFormsByNameResult(
      String[] fieldValue1, String[] fieldValue2, String testFormName, List<IPSFormData> forms) {
    ...
    for (IPSFormData aForm : forms) {
      ...
      assertEquals(
          "form field 1 value", fieldValue1[1], filedValue1.replace(FIELD_VALUES_SEPARATOR, ""));
      assertTrue("form field 2", aForm.getFields().containsKey(fieldValue1[0]));
      String filedValue2 = aForm.getFields().get(fieldValue2[0]);
      assertEquals(
          "form field 2 value", fieldValue2[1], filedValue2.replace(FIELD_VALUES_SEPARATOR, ""));
  ...
  public void testFindDistinctFormNames() {
    ...
    String[] fieldValue2 = new String[] {"field2", "value2"};
    String[] formNames =
        new String[] {"testform", "TESTFORM", "testForm", "anotherForm", "lastForm"};

    for (int i = 0; i < 10; i++) {

      IPSFormData formData =
          generateFormData(
              formNames[i % 5], fieldValue1[0], fieldValue1[1], fieldValue2[0], fieldValue2[1]);
  ...
  public void testSendFormEmail() throws Exception {
    ...
    IPSFormData formData =
        generateFormData(
            "testform", fieldValue2[0], fieldValue2[1], fieldValue1[0], fieldValue1[1]);
