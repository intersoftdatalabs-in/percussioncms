...
public class PSFormRestService extends PSAbstractRestService implements IPSFormRestService {
  ...
  @Autowired private IPSFormService formService;

  private String enabledCiphers;

  /**
   * The form data joiner initialized by constructor. Used to merge forms data and generate a CSV
  ...
  public void delete(@PathParam("formName") String formName) {
    try {
      ...
      log.error(
          "Exception occurred while deleting form, Error: {}",
  ...
  public void create(
    ...
    Form form2 =
        (Form) containerRequest.getProperty(InternalServerProperties.FORM_DECODED_PROPERTY);
    ...
    try {
      ...
      if (formNameValues == null
          || formNameValues[0] == null
          || formNameValues[0].trim().length() == 0) {
        log.error("Supplied form missing {} field.", FORM_NAME_KEY);
        WebApplicationException webEx =
            new WebApplicationException(
                new IllegalArgumentException("Supplied form missing " + FORM_NAME_KEY + " field."),
      ...
      if (emailNotifToVals != null
          && emailNotifToVals[0] != null
          && emailNotifToVals[0].trim().length() > 0) {
        try {
          emailNotifTo =
      ...
      if (emailNotifSubjectVals != null
          && emailNotifSubjectVals[0] != null
          && emailNotifSubjectVals[0].trim().length() > 0
          && !isFormEmail) {
        try {
          emailNotifSubject =
              PSEncryptor.decryptString(
                  PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR),
                  emailNotifSubjectVals[0]);
        } catch (PSEncryptionException | java.lang.IllegalArgumentException e) {
          emailNotifSubject =
              PSLegacyEncrypter.getInstance(
                      PathUtils.getRxDir(null).getAbsolutePath().concat(PSEncryptor.SECURE_DIR))
      ...
      if (isFormEmail) {
        ...
        if (emailNotifSubject == null)
          log.error(
              "Form  is configured as an email form but is missing email-subject field {}",
              formName);
        if (emailNotifToVals == null)
          log.error(
              "Form {} is configured as an email form but is missing perc_EmailFormTo field to send email to.",
              formName);
      ...
      if (formService.getRecaptchaService().isCaptchaOn() && !isSpamBot) {
        ...
        if (captchaResponse == null) {
          ...
          if (formName == null) formName = "<not set>";

          log.error(
              "recaptcha.on=true in configured properties, but form post is missing reCaptcha response. Post will be treated as a bot, verify that reCaptcha field is present on form: "
                  + formName);
        }
      ...
      if (isSpamBot) {
        log.error(
            "Blocking post from {}, was detected as a SPAM bot.  Consider blocking this IP in firewall rules.",
            httpServletRequest.getRemoteAddr());
        WebApplicationException webEx =
            new WebApplicationException(
                new IllegalArgumentException("Post detected as a Bot.  Form submission rejected."),
                Response.serverError().build());
        handleError(header, resp, webEx, hostRedirect, errorRedirect, encryptExist);
        return;
      }
      ...
      if ((processorType != null) && !processorType.equalsIgnoreCase("LocalServer")) {
        ...
        if (percFields.get(FORM_PROCESSORURL) != null) {
          ...
          log.error(
              "{} was not specified but Form is configured to POST to a remote service.",
              FORM_PROCESSORURL);
          WebApplicationException webEx =
              new WebApplicationException(
                  new IllegalArgumentException("Invalid form submitted"),
                  Response.serverError().build());
          handleError(header, resp, webEx, hostRedirect, errorRedirect, encryptExist);
          return;
        }
        ...
        try {
          ...
          log.error(
              "Exception occurred while saving a form, Error: {}",
              PSExceptionUtils.getMessageForLog(e));
          log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        ...
        if (emailNotifTo != null && emailNotifSubject != null) {
          try {
            ...
            log.error(
                "An error occurred sending the form notification email to: {} for form {}, Error: {}",
                emailNotifTo,
                formName,
                e.getMessage());
            log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      ...
      WebApplicationException webEx =
          new WebApplicationException(ex, Response.serverError().build());
      handleError(header, resp, webEx, hostRedirect, errorRedirect, encryptExist);
    }
  }
  ...
  private void sendFormDataEmail(String emailNotifTo, String emailNotifSubject, IPSFormData form) {
    try {
      ...
      log.error(
          "Cannot email form data, unexpected error, Error: {}",
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
    }
  }
  ...
  public PSFormSummaries get(@PathParam("formName") String formName) {
    if (!formService.isValidFormName(formName)) {
      ...
      throw new WebApplicationException(
          new IllegalArgumentException("Invalid formName"), Response.serverError().build());
    }
    try {
      ...
      log.error(
          "Exception occurred while getting form summaries, Error: {}",
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw new WebApplicationException(e, Response.serverError().build());
    }
  ...
  public PSFormSummaries get() {
    try {
      ...
      log.error(
          "Exception occurred while getting all form summaries, Error: {}",
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw new WebApplicationException(e, Response.serverError().build());
    }
  ...
  public Response export(
    ...
    try {
      ...
      log.error(
          "Exception occurred while exporting the form, Error: {}",
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      throw new WebApplicationException(e, Response.serverError().build());
    }
  ...
  /**
   * Handle the exception by trying to redirect to the error page if one exists, else go to default
   * error page, else throw a ServletException if all else fails.
   *
   * @param header
   * @param ex
   * @param redirect
    ...
    if (redirect != null) {
      if (isEncrypted) {
        try {
          urlErrorPage =
              PSEncryptor.decryptString(
                  PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR), redirect);
        } catch (PSEncryptionException e) {
          urlErrorPage =
              PSLegacyEncrypter.getInstance(
                      PathUtils.getRxDir(null).getAbsolutePath().concat(PSEncryptor.SECURE_DIR))
