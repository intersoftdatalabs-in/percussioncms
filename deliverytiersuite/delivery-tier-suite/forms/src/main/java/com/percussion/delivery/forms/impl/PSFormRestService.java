  ...
  public void create(
    ...
    try {
      ...
      if (emailNotifToVals != null
          ...
          && !emailNotifToVals[0].trim().isEmpty()) {
        try {
          emailNotifTo =
      ...
      if (emailNotifSubjectVals != null
          ...
          && !emailNotifSubjectVals[0].trim().isEmpty()
          && !isFormEmail) {
        try {
          emailNotifSubject =
              PSEncryptor.decryptString(
      ...
      if (isFormEmail) {
        ...
        if (emailNotifToVals == null)
          log.error(
              "Form {} is configured as an email form but is missing perc_EmailFormTo field to send"
                  + " email to.",
              formName);
      ...
      if (formService.getRecaptchaService().isCaptchaOn() && !isSpamBot) {
        ...
        if (captchaResponse == null) {
          ...
          if (formName == null || formName.isEmpty()) formName = "<not set>";

          log.error(
              "recaptcha.on=true in configured properties, but form post is missing reCaptcha"
                  + " response. Post will be treated as a bot, verify that reCaptcha field is"
                  + " present on form: {}",
              formName);
        }
      ...
      if (isSpamBot) {
        log.error(
            "Blocking post from {}, was detected as a SPAM bot.  Consider blocking this IP in"
                + " firewall rules.",
