...
/**
 * Merges the content of 0 or more PSFormData objects into a single CSV file, with a single header
 * and a row for every form data object. Handles the case where different forms have different
 * fields by putting empty data for each row's cell if that row did not originally have that field's
 * data. The output rows are in the same order as the input rows.
 *
...
public class PSFormDataJoiner {
   ...
   *
   * @param columnNames A Set object where form headers will be added. Assumed not <code>null</code>
   *     .
   * @param formHeader Array with form's header (column names). Assumed no <code>null</code>.
   */
  ...
  /**
   * Takes a PSFormData object and returns a Map with all its keys (field names) normalized, using
   * the CaselessString class. It also adds the 'form name' and 'create date' columns.
   *
   * @param formData A PSFormData object to process. Assumed not <code>null</code>.
   * @return A new Map with the same information as currently present in 'formData', but using a
   *     CaselessString object as the key (field names). Never <code>null</code>, never empty.
   */
  private Map<CaselessString, String> processCsvRow(IPSFormData formData) {
    ...
    processedFormDataFields.put(
        new CaselessString(CREATE_DATE_FIELD), formData.getCreateDate().toString());

    return processedFormDataFields;
  }

  /**
   * Takes a list of PSFormData objects and process them. It creates a unique set of fields and
   * returns the processing result.
   *
   * @param formsList List of PSFormData objects to be processed. Assumed not <code>null</code>.
   * @return Parsing result. It has the final header in alpha order and the data as a list of maps.
  ...
  private FormDataProcessingResult parseCSVData(List<IPSFormData> formsList) throws IOException {
    ...
    for (Map<CaselessString, String> aCsvRow : currentDataList) {
      ...
      for (CaselessString aHeader : finalHeader) {
        ...
        else aFinalCsvRow.put(aHeader.toString(), StringUtils.EMPTY);
      }
  ...
  /**
   * Joins an array with forms data given as PSFormData objects and returns a merged CSV.
   *
  ...
  public String generateEmailBody(IPSFormData form) {
    ...
    for (CaselessString cString : finalHeader) {
      ...
      if (cString.string.equals("topyenoh")) continue;
  ...
  final class CaselessString implements Comparable<CaselessString> {
    ...
    public boolean equals(Object obj) {
      if (!(obj instanceof CaselessString)) return false;
  ...
  /**
   * Represents the processing result of the forms data. It has the final header and the data.
   *
   * @author miltonpividori
   */
  class FormDataProcessingResult {
     ...
     * @param data Data list of parsing result. It's a list of Maps. Each key/value pair represents
     *     a CSV row, with the field as the key and its value as the Map's value.
     */
    public FormDataProcessingResult(List<CaselessString> header, List<Map<String, String>> data) {
      this.header = new String[header.size()];
      for (int i = 0; i < header.size(); i++) this.header[i] = header.get(i).toString();
    ...
    /**
     * @return Data list of parsing result. It's a list of Maps. Each key/value pair represents a
     *     CSV row, with the field as the key and its value as the Map's value.
     */
     ...
     *
     * @return Returns true if the ParsingResult object has no header and no data. False otherwise.
     */
