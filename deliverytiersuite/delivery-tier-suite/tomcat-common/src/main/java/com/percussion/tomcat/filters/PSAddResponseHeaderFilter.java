...
public class PSAddResponseHeaderFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (response instanceof HttpServletResponse) {
      ...
      if (cachingAgeTimeValue == null || cachingAgeTimeUnit == null) {
        ...
        String contextCachingAgeTimeVal =
            contextProps.getProperty(CACHING_MAX_AGE_VALUE_PROPERTY_KEY);
        ...
        String contextCachingAgeUnitVal =
            contextProps.getProperty(CACHING_MAX_AGE_UNIT_PROPERTY_KEY);
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    if (tomcatBase != null) {
      try (InputStream in = new FileInputStream(tomcatBase + PERC_SECURITY_PROPS_ROOT)) {
        props.load(in);
      } catch (IOException e) {
