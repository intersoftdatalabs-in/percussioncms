...
/**
 * Provides a basic data structure for indexing the routine table used by the version redirector
 * valve.
 *
 * @author natechadwick
 */
public class PSVersionRoutingTable {
  ...
  public void addServiceContext(String context) {
    if (serviceContexts == null) serviceContexts = new HashMap<>();
  ...
  public String determineRoute(String context, String requestVer) {
    ...
    try {
      ...
      if (routes.containsKey(requestVer)) {
        ...
        if (routes.containsKey("")) ret = routes.get("");
      ...
      log.error(
          String.format(
              "Unable to determine route for Context: %s and Version: %s", context, requestVer));
    }
    return ret;
  }
}
