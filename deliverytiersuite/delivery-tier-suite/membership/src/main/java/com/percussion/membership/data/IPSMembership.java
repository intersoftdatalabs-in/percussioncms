...
public interface IPSMembership {
   ...
   *
   * @return The account id, never <code>null</code> or empty, "0" if this membership has not been
   *     persisted.
   */
  public String getId();
  ...
  /**
   * Get the date and time when this membership account was last accessed, may be used for
   * determining if a session has timed out.
   *
   ...
   *
   * @return The session id, never empty, may be <code>null</code> if no session has been created or
   *     if the previously used session has been expired and thus cleared.
   */
   ...
   *
   * @return The status, an {@link PSMemberStatus} object, never empty or <code>null</code>.
   */
   ...
   *
   * @param status The status, an {@link PSMemberStatus} object, never empty or <code>null</code>.
   */
