...
public class PSMembershipDao implements IPSMembershipDao {
    ...
    if (!result.isEmpty()) {
      if (result.size() > 1) {
        ...
        throw new IllegalStateException(
            "More than one membership entry found for sessionID: " + sessionId);
      }
  ...
  public IPSMembership findMemberByPwdResetKey(String pwdResetKey) {
    ...
    if (!result.isEmpty()) {
      if (result.size() > 1) {
        // this would be a bug
        throw new IllegalStateException(
            "More than one membership entry found for pwdResetKey: " + pwdResetKey);
      }
  ...
  @Transactional
  public IPSMembership createMember(String userId, String password, PSMemberStatus status)
      throws Exception {
  ...
  public IPSMembership findMember(String userId, Session session) {
    ...
    if (!result.isEmpty()) {
      if (result.size() > 1) {
        // this would be a bug
        throw new IllegalStateException(
            "More than one membership entry found for userId: " + userId);
      }
