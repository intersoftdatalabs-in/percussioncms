...
public interface IPSMembershipService {
  ...
  /**
   * Creates a membership account for the specified user, and also creates a valid session for the
   * user by calling the authentication provider.
   *
   * @param email Used as the user id for the account, may not be <code>null</code> or empty.
   * @param password The password for the account, may not be <code>null</code> or empty. <code>null
   *     </code> or empty if they were not registering while trying to view a protected resource.
   * @param confirmationRequired Indicates if the activation through email is required for the
   *     account to create. May not be <code>null</code> or empty.
   * @param confirmationPage The confirmation page to include in the email sent to the user, if the
   *     confirmation is required. May not be <code>null</code> or empty.
  ...
  /**
   * Set a reset key for the supplied email account. The reset key is only valid for 24 hours.
   *
   * @param email Used as the user id for the account, may not be <code>null</code> or empty.
   * @param linkUrl The URL link to the reset page to be included in the email sent to the user, not
   *     <code>null</code> or empty.
   * @return The email for the user, not <code>null</code> or empty.
   * @throws PSAuthenticationFailedException if the member cannot be authenticated.
   * @throws Exception If there are any unexpected errors.
   */
  public String setResetKey(String email, String linkUrl)
      throws PSAuthenticationFailedException, Exception;
