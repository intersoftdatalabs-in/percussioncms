...
public class PSMetadataQueryService implements IPSMetadataQueryService {
  ...
  public List<Object[]> executeCategoryQuery(PSMetadataQuery query)
    ...
    /**
     * Following this Example Query Writing the query String below SELECT count(p4.ENTRY_ID),
     * p4.name ,p4.stringvalue from PERC_PAGE_METADATA_PROPERTIES p4 where p4.ENTRY_ID in( select
     * p.ENTRY_ID from PERC_PAGE_METADATA_PROPERTIES p where p.ENTRY_ID in( select p.ENTRY_ID from
     * PERC_PAGE_METADATA_PROPERTIES p where ((p.name = 'dcterms:title' and p.stringvalue LIKE '%n%'
     * ) or (p.name = 'dcterms:source' and p.stringvalue = 'CaldTemp' ) ) and p.ENTRY_ID in( select
     * e.pagePathHash from PERC_PAGE_METADATA e where e.type = 'page' AND e.site = 'Callidus' AND
     * e.folder LIKE '%/News-Releases/%') )) AND p4.name = 'perc:category' GROUP BY p4.name,
     * p4.stringvalue ORDER BY p4.stringvalueRDER BY p4.stringvalue
     *
     * <p>Q4 = select e.pagePathHash from PERC_PAGE_METADATA e where e.type = 'page' AND e.site =
     * 'Callidus' AND e.folder LIKE '%/News-Releases/%')
     *
     * <p>Q3 = select p.ENTRY_ID from PERC_PAGE_METADATA_PROPERTIES p where ((p.name =
     * 'dcterms:title' and p.stringvalue LIKE '%n%' ) or (p.name = 'dcterms:source' and
     * p.stringvalue = 'CaldTemp' ) ) and p.ENTRY_ID in( Q4 ) Q2 = select p.ENTRY_ID from
     * PERC_PAGE_METADATA_PROPERTIES p where p.ENTRY_ID in(Q3) Q1 = SELECT count(p4.ENTRY_ID),
     * p4.name ,p4.stringvalue from PERC_PAGE_METADATA_PROPERTIES p4 where p4.ENTRY_ID in(Q2) AND
     * p4.name = 'perc:category' GROUP BY p4.name, p4.stringvalue ORDER BY p4.stringvalue
     */
    ...
    Map<String, PSCriteriaElement.OPERATION_TYPE> paramOps =
        new HashMap<String, PSCriteriaElement.OPERATION_TYPE>();
    for (PSCriteriaElement ce : entryCrit) {
      ...
      Q4WhereClause.append(
          MessageFormat.format(clauseTemplate, ce.getName(), ce.getOperation(), replParam));
    ...
    for (PSCriteriaElement ce : propsCrit) {
      ...
      if ((valueColumn.equals(PROP_STRINGVALUE_COLUMN_NAME)
        ...
        Q3WhereCaluse.append(
            MessageFormat.format(
                clauseTemplate,
                PROP_VALUEHASH_COLUMN_NAME,
                ce.getOperation(),
                valueParam,
                nameParam));
      } else {
        Q3WhereCaluse.append(
            MessageFormat.format(
                clauseTemplate, valueColumn, ce.getOperation(), valueParam, nameParam));
      }
    ...
    StringBuilder Q1 =
        new StringBuilder(
                "SELECT distinct count(p4.entry.id), p4.name ,p4.stringvalue from PSDbMetadataProperty p4"
                    + " where p4.entry.id in (")
            .append(Q2)
            .append(" )")
            .append(
                "AND p4.name = 'perc:category'  GROUP BY p4.name, p4.stringvalue  ORDER BY p4.stringvalue");
    ...
    try (Session session = getSession()) {
      ...
      if (!isPagingSupported(query)) {
        throw new UnsupportedOperationException(
            "Pagination is not supported for requested sort property");
      } else {
        // Get the Count of entries only when the StartIndex is 0, for later pages client should be
        // having the
  ...
  /**
   * Helper method to build the Hibernate query based on the raw query passed in. There are four
   * possible ways to create the query based on the passed in criteria 1)Query for the count of
   * total entries for the given criteria Ex:select count(*) from PSDbMetadataEntry as me where
   * me.type = :pagePropValue0 and me.site = :pagePropValue1 and me.folder LIKE :pagePropValue2 and
   * me.name != :pagePropValue3
   *
   * <p>2)Query for Results set for passed in criteria and order by is a property from the
   * properties table Ex:select distinct me, lower(prop.stringvalue) as sort1 from PSDbMetadataEntry
   * as me join me.properties as prop where prop.id.name = 'dcterms:title' and me.site =
   * :pagePropValue0 order by sort1 desc
   *
   * <p>3)Query for Results set for passed in criteria and ordeby is a column name from the parent
   * table Ex:select distinct me from PSDbMetadataEntry as me where me.site=:pagePropValue0 order by
   * lower(me.pagepath) asc
   *
   * <p>4)Query for results set for passed in criteria but there no orderby Ex:
   *
   * @param sess
   * @param rawQuery isCount if it is true then query will be created to get the count of entries
  ...
  private PSPair<Query, SORTTYPE> buildHibernateQuery(
    ...
    for (PSCriteriaElement ce : entryCrit) {
      if (needConjunction) queryBuf.append(" and");
      else needConjunction = true;
      String replParam = "pagePropValue" + paramIndex++;
    ...
    for (PSCriteriaElement ce : propsCrit) {
      ...
      if ((valueColumn.equals(PROP_STRINGVALUE_COLUMN_NAME)
        ...
        queryBuf.append(
            MessageFormat.format(
                useClause, i++, valueColumn, ce.getOperation(), valueParam, nameParam));
      }
    ...
    // If the method is getting called only for the entry count, in that case order by doesn't need
    // to be included
    ...
    if (rawQuery.getTotalMaxResults() > 0 && rawQuery.getTotalMaxResults() < queryLimit) {
      log.debug(
          "Setting max query limit to client provided value :{}", rawQuery.getTotalMaxResults());
    ...
    if (!isCount && rawQuery.getMaxResults() > 0 && rawQuery.getMaxResults() <= useLimit) {
      if (rawQuery.getMaxResults() > 0) q.setMaxResults(rawQuery.getMaxResults());
      if (rawQuery.getStartIndex() >= 0) q.setFirstResult(rawQuery.getStartIndex());
    }
    for (String key : paramValues.keySet()) {
      Object value = paramValues.get(key);
      PSCriteriaElement.OPERATION_TYPE opType = paramOps.get(key);
  ...
  private String escapeSpecialCharacters(String value) {
    ...
    if (endsWithWildcard) escapedString = escapedString.substring(0, escapedString.length() - 1);
    ...
    if (startsWithWildcard) escapedString = "%" + escapedString;

    if (endsWithWildcard) escapedString = escapedString + "%";
  ...
  /**
   * Method to obtain the URL string of the JDBC connection. Used to check the DB provider for
   * delivery server.
   *
   * @return URL string of the JDBC connection. Can be empty if there was a problem retrieving the
  ...
  /**
   * Based on the order By option, get the additional fields to add to the order by sentence in the
   * query * Ex: orderBy = "dcterms:created desc, linktitle desc" and the method returns linktitle
   * desc
   *
   * @param orderBy cannot be <code>null</code> or empty
   * @return sortColums the map of additional fields to be added, may be empty never<code>null
   *     </code>
   */
  ...
  private boolean isPagingSupported(PSMetadataQuery query) {
    ...
    if (StringUtils.isNotBlank(orderBy)) {
      isSupported =
          !PSMetadataQueryServiceHelper.getDatatype(
                  PSMetadataQueryServiceHelper.getSortPropertyName(orderBy), datatypeMappings)
              .toString()
              .equals(PROP_TEXTVALUE_COLUMN_NAME);
    }
    return isSupported;
  }
