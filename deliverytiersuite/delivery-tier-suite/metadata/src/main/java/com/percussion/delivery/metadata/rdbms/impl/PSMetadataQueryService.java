...
public class PSMetadataQueryService implements IPSMetadataQueryService {
  ...
  public List<Object[]> executeCategoryQuery(PSMetadataQuery query)
    ...
    StringBuilder Q1 =
        new StringBuilder(
                "SELECT distinct count(p4.entry.id), p4.name ,p4.stringvalue from"
                    + " PSDbMetadataProperty p4 where p4.entry.id in (")
            .append(Q2)
            .append(" )")
            .append(
                "AND p4.name = 'perc:category'  GROUP BY p4.name, p4.stringvalue  ORDER BY"
                    + " p4.stringvalue");
