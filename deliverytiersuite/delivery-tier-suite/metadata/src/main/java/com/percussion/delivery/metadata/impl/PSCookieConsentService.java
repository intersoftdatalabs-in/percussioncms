...
public class PSCookieConsentService implements IPSCookieConsentService {
  ...
  /**
   * Takes a new PSCookieConsentQuery from JavaScript request and maps 1 or more approved cookie
   * services to unique PSDbCookieConsent objects. This will allow for creating unique rows in the
   * DB for each service/cookie approved.
   *
   * @param consentQueries - Collection containing PSCookieConsentQuery objects - siteName,
   *     services, etc. approved for cookies.
   * @return A collection of unique PSDbCookieConsent objects corresponding to the approved cookies.
   *     Never <code>null</code>, may be empty.
   */
  private Collection<PSDbCookieConsent> convertToDbCookieConsents(
      Collection<PSCookieConsentQuery> consentQueries) {
  ...
  public void updateOldSiteName(String oldSiteName, String newSiteName) {
    try {
      ...
      log.error(
          "Error updating site name in cookie consent service for old site: {} Error: {}",
          oldSiteName,
