...
/**
 * Merges the contents of a PSCookieConsent object into a .CSV file with each PSCookieConsent obj as
 * a single row.
 *
 * @author chriswright
 */
public class PSCookieConsentCSVWriter {
  ...
  private static final Logger MS_LOG =
      LogManager.getLogger(PSCookieConsentCSVWriter.class.getName());
  ...
  /**
   * This method loops through each IPSCookieConsent entry and adds its values to a new line in a
   * .CSV file.
   *
  ...
  public String writeCSVFile() {
    ...
    for (IPSCookieConsent consent : entries) {
      try {
        ...
        MS_LOG.error(
            "Error writing cookie consent entry to .CSV file. "
                + "Check for NULL entries in the DTS database.",
            e);
        continue;
      }
    }
