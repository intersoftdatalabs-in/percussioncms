...
public class PSProfanityFilter {
  ...
  /***
   * Initializes the Profanity filter from a file. Will first try to load it from
   * conf/perc. If it is not found, will load it from the old webapps location. If
   * that is not found, will load it from the class path resource.
   */
  ...
  private synchronized void setProfanity() {
    ...
    try {
      if (profanityFile.exists()) {
        log.info(
            "Initializing the Comment Profanity Filter from file: {}",
        ...
        profanityWords =
            new Scanner(PSProfanityFilter.class.getResourceAsStream(PROFANITY_FILE_CP))
                .useDelimiter("\\A")
                .next();
      }
    } catch (IOException e) {
      log.error(
          "Error initializing the Comment Profanity Filter. Error: {}",
