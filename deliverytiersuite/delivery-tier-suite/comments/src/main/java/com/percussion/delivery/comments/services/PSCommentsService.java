...
public class PSCommentsService implements IPSCommentsService {
  ...
  public IPSComment addComment(IPSComment comment) {
    ...
    try {
      ...
      log.error("Error in adding a new comment: {}", PSExceptionUtils.getMessageForLog(ex));
      log.debug(ex);
  ...
  public PSComments getComments(PSCommentCriteria criteria, boolean isModerator) {
    ...
    try {
      ...
      log.error("Error in getting comments by criteria: {}", PSExceptionUtils.getMessageForLog(ex));
      throw new RuntimeException(ex);
    }
  ...
  public APPROVAL_STATE getDefaultModerationState(String site) {
    try {
      ...
      log.error(
          "Error getting default moderation state: {}", PSExceptionUtils.getMessageForLog(ex));
      throw new RuntimeException(ex);
    }
  }
  ...
  public void setDefaultModerationState(String sitename, APPROVAL_STATE dflt) {
    ...
    try {
      ...
      log.error(
          "Error setting default moderation state: {}", PSExceptionUtils.getMessageForLog(ex));
      log.debug(ex);
      throw new RuntimeException(ex);
    }
  }
  ...
  private List<PSPageSummary> createPageSummaries(List<PSPageInfo> pagePathSummaryQuery) {
    ...
    for (PSPageInfo tmp : pagePathSummaryQuery) {
      ...
      else pagepathWithCommentCount.unapprovedCount += count;
      if (!viewed) pagepathWithCommentCount.newComments += count;
    }
    ...
    for (String pagePath : pagepathAndCommentsCountMap.keySet()) {
      ...
      pageSummaries.add(
          new PSPageSummary(
              pagepathWithCommentCount.pagepath, commentCount, approvedCount, newCommentsCount));
    }

    // Sort the list by pagepath in ascending order
    Collections.sort(
        pageSummaries,
        new Comparator<PSPageSummary>() {
    ...
    try {
      ...
      for (IPSComment comment : comments) {
        ...
        try {
          ...
          log.error(
              "Error updating comment with id: {} An administrator should attempt to update the comment manually. Error: {}",
              comment.getId(),
      ...
      log.error(
          "Error finding comments for site: {} Error: {}",
          prevSiteName,
          PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
      return false;
