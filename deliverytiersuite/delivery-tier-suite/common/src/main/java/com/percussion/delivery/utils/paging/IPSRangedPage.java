...
public interface IPSRangedPage {
   ...
   *
   * @return A Map of backend fields being sorted on, paired with their Sort direction, may be
   *     empty, never null.
   */
  ...
  /**
   * * Determines the fields that are being used to apply the range filter and the last value used
   * to fetch the backend page.
   *
   * <p>The return value will be a map containing each field, paired with the value for that field
   * that was at the end of the last page if the direction is forward, and at the beginning of the
   * last page if the direction is backward.
   *
   * <p>Field values may be null when indicating the beginning or end of a dataset.
   *
