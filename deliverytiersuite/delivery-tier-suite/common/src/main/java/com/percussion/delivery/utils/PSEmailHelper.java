...
public class PSEmailHelper implements IPSEmailHelper {
  ...
  public String sendMail(IPSEmailRequest emailRequest)
    ...
    if (commonsMultiPartEmail == null) throw new PSEmailServiceNotInitializedException();
    ...
    if (StringUtils.isNotBlank(emailRequest.getToList())) {
      ...
      for (String email : emails) {
        try {
          ...
          log.error(
              "Error adding address: {} to To: for email message. Error: {}",
              email,
              e.getMessage());
        }
      }
    ...
    if (StringUtils.isNotBlank(emailRequest.getCCList())) {
      ...
      for (String email : emails) {
        try {
          ...
          log.error(
              "Error adding address: {} to CC: for email message. Error: {}",
              email,
              e.getMessage());
        }
      }
    ...
    if (StringUtils.isNotBlank(emailRequest.getBCCList())) {
      ...
      for (String email : emails) {
        try {
          ...
          log.error(
              "Error adding address: {} to BCC: for email message. Error: {}",
              email,
              e.getMessage());
        }
      }
    ...
    try {
      ...
      log.error(
          "Error setting the body: {} for email message. Error: {}",
    ...
    try {
      ...
      log.error(
          "Error sending email message. Error: {} Cause: {}",
          e.getMessage(),
  ...
  private MultiPartEmail createMultiPartEmail() {
    ...
    try {
      ...
      if (!StringUtils.isBlank((String) emailProps.get(EMAIL_PROPS_SMTP_USERNAME))) {
        commonsMultiPartEmail.setAuthenticator(
      ...
      if (!emailProps.containsKey(EMAIL_PROPS_FROMNAME)
          || StringUtils.isBlank((String) emailProps.get(EMAIL_PROPS_FROMNAME)))
        commonsMultiPartEmail.setFrom((String) fromAddrProp);
      else
        commonsMultiPartEmail.setFrom(
            (String) fromAddrProp, (String) emailProps.get(EMAIL_PROPS_FROMNAME));
      ...
      log.error(
          "Invalid properties supplied for email client: {}", PSExceptionUtils.getMessageForLog(e));
    }
