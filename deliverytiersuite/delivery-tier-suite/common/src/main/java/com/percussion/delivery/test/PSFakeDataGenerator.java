...
public class PSFakeDataGenerator {
   ...
   *
   * @param count The number of registrations to return, 0 for all available data. Be careful as
   *     test datasets can be large.
  ...
  public static List<FakeRegistrant> getFakeRegistrations(int count) {
    ...
    try {
      while ((line = br.readLine()) != null) {
        ...
        if (count > 0) if (lineNumber > count) break;
