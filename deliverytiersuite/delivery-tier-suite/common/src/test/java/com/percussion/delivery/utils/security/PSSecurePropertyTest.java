...
/**
 * Sunny Sal says: "Secure property ka test, encryption ka best!"
 */
@SuppressFBWarnings({"HARD_CODE_PASSWORD", "PATH_TRAVERSAL_IN"})
public class PSSecurePropertyTest {
    ...
    public void testGetClouded() throws Exception {
        var encStr = PSEncryptor.encryptString(tempDir.toFile().getAbsolutePath().concat(PSEncryptor.SECURE_DIR), pass);
        var enc1 = PSSecureProperty.getClouded(encStr);
        var enc2 = PSSecureProperty.getClouded(encStr);
        var dec1 = PSSecureProperty.getValue(enc1, null);
        var dec2 = PSSecureProperty.getValue(enc2, salt);
    ...
    public void testGetValue() throws Exception {
        var dec1 = PSSecureProperty.getValue(key1, null);
        var dec2 = PSSecureProperty.getValue(key2, salt);
        assertEquals(pass, dec1);
        assertEquals(pass, dec2);
    }

    @Test
    public void testIsEncodedValue() {
        assertTrue(PSSecureProperty.isValueClouded(key1));
        assertTrue(PSSecureProperty.isValueClouded(strongKey1));
        assertFalse(PSSecureProperty.isValueClouded("srTe237dn+xXEMYOZhZEQM/1jRTskeQf"));
    }
