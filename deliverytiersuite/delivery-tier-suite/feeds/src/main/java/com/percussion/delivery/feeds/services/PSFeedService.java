...
feedDao = Objects.requireNonNull(dao, "dao cannot be null");
    ...
    public void saveDescriptors(PSFeedDescriptors descriptors)
    {
        ...
        List<IPSFeedDescriptor> deletes = existing.stream()
            .filter(d -> descriptors.getDescriptors().stream()
                .noneMatch(nd -> nd.getName().equals(d.getName()) && nd.getSite().equals(d.getSite())))
     ...
     */
    private String generateFeed(IPSFeedDescriptor desc, Optional<String> hostName, HttpServletRequest httpRequest) throws FeedException {
        ...
        try
        {
            ...
            String host = hostName.filter(StringUtils::isNotBlank).orElse(PSFeedGenerator.getHost(desc.getLink()));
