...
| `RESULT:FAIL … DETAIL:timeout after Ns (last_http=… health=…)` | `qa-health` | Probe never became ready (HTTP and/or Health not green); if logs later show context fail, re-run `qa-health` or `docker logs perc-matrix-cms-h2`. Bare `qa-health` uses the **running** cell publish (`docker port perc-matrix-cms-h2`), then last `qa-up` log, not a fresh preferred `:9993` (#4597). Timeout RESULT may include `HINT:cell_publishes:<port>` when the probe port is dead but the cell is mapped elsewhere. |
