...
- Build/test with `./mvnw` (JDK 21 on `development`)
...
- [x] T002 Confirm branch baseline: run `git status` clean on `002-jdbc-drivers-cleanup` based on `development`; confirm JDK 21 via `./mvnw -v`
- [x] T003 [P] Confirm `mvnw` baseline build works: `cd modules/perc-distribution-tree && ../../mvnw -o -q clean verify` succeeds before any edits (record baseline `mvn verify` output excerpt for the PR description)
...
- [x] T009 Verify foundational phase: run `cd modules/perc-distribution-tree && ../../mvnw -o test -Dtest=NoSuchTest` (or `test-compile`) and confirm the test source tree compiles against the new junit-jupiter dependency; resolve any version-mismatch or surefire errors before proceeding
...
**Independent Test**: After `cd modules/perc-distribution-tree && ../../mvnw clean package`, `unzip -l target/perc-distribution-tree.jar | grep '_jdbc-stage'` returns zero matches; `unzip -l target/perc-distribution-tree.jar | awk '/jetty\/base\/lib\/jdbc/ && /\.jar$/'` lists exactly the 7 curated drivers from data-model.md (E1).
...
- [x] T014 [US1] Run the US1 tests: `cd modules/perc-distribution-tree && ../../mvnw -o test -Dtest='StagingCleanupAntScriptTest'`. Both `@Test` methods must pass.
- [x] T015 [US1] Run the full module build end-to-end: `cd modules/perc-distribution-tree && ../../mvnw -o clean verify`. Confirm: (a) BUILD SUCCESS; (b) `verify-jdbc-drivers.sh` reports `OK: 7 JDBC driver JAR(s) verified`; (c) `target/perc-distribution-tree.jar` exists and has no `_jdbc-stage` entries (manual `unzip -l` check from quickstart Scenario 1).
...
- [x] T022 [US2] Run the US2 JUnit tests: `cd modules/perc-distribution-tree && ../../mvnw -o test -Dtest='InstallXmlDeleteSetTest'`. All 4 methods must pass.

- [x] T023 [US2] Run the full module build: `cd modules/perc-distribution-tree && ../../mvnw -o clean verify`. Both `verify-jdbc-drivers` and the new `check-no-glob-deletes` executions must pass.
...
- [x] T028 [US3] Sanity-check the documented example by running it verbatim against the freshly built artifact: `cd modules/perc-distribution-tree && ../../mvnw -o clean package && ./scripts/verify-jdbc-drivers.sh --artifact target/perc-distribution-tree.jar --expected-driver-glob 'mariadb-java-client-*.jar,derby-*.jar,derbyclient-*.jar,derbynet-*.jar,mssql-jdbc-*.jar,jtds-*.jar,ojdbc17-*.jar'`. Expect exit 0 and `OK: 7 JDBC driver JAR(s) verified under jetty/base/lib/jdbc/`. This implements FR-006 and SC-004.
...
- [x] T030 [US3] Run the full module build: `cd modules/perc-distribution-tree && ../../mvnw -o clean verify`. Both `verify` executions must pass: `verify-jdbc-drivers` and `check-no-glob-deletes`, plus the JUnit suite.
...
- [x] T034 [P] Run the full module test+verify suite one last time: `cd modules/perc-distribution-tree && ../../mvnw -o clean verify`. Capture the final `verify` output excerpt for the PR description.
...
- Build/test invocations always use `../../mvnw` (or `mvnw.cmd` on Windows) per the module `AGENTS.md`
