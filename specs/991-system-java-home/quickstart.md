...
- **Layer 3 — behavioral tests** (`ResolveJavaHomeBehaviorTest`) actually invoke `resolve-java-home.sh` on Linux + macOS against fixture scenarios (env set/unset, `<InstallDir>/JRE` present/absent, `PATH` present/absent, Java 8/17/22 version-mismatch rejections) using the fake-`java` fixture at `src/test/resources/fixtures/fake-java-home/jre/bin/`. 7 scenarios × major versions = 7 tests (annotated with `@EnabledOnOs({LINUX, MAC})`). Windows coverage is provided structurally by `DtsJavaHomeScriptTest` and `ResolveJavaHomeScriptTest` until a real PE-binary launcher fixture is available.
