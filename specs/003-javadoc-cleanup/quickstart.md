...
- JDK 21 reachable via `./mvnw` (the script sets `JAVA_HOME=JAVA_HOME_21`).
- Clean working tree on branch `003-javadoc-cleanup` (or `development` if running a
  pre-cleanup baseline).
- Ability to run `./mvnw` (network for first-time dep download is the same as any
...
```bash
./mvnw -pl modules/DesktopContentExplorer javadoc:javadoc -DskipTests 2> \
...
- Exit code from `mvnw` is `0` (note: even at baseline, exit is `0` because the
...
| **SC-003** (full module build succeeds) | `./mvnw -pl modules/DesktopContentExplorer -am verify -DskipTests` exits `0`.                                                                    |
