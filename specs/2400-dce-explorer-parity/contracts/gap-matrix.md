...
### 2026-08-20 Explorer Copy Folder product-route proof (#3647 / #3102)

**This slice:** [#3647](https://github.com/intersoftdatalabs-in/percussioncms/issues/3647)

H2 Playwright on `spa.jsp?entry=explorer` (no `rxFolderMutations=1`) for ReducedActions **Copy** of a selected folder: public REST `POST /rest/folders/copy/folder` HTTP 200 and destination tree/list refresh when a Sites/Assets parent exists. **Sites/folders tree stays Partial.** Do not flip Present from this agent proof. Out of scope: Subfolder Copy wizard (#2792), clipboard paste (#2408).

### 2026-08-20 Explorer Create Folder product-route proof (#3640 / #3102)

**This slice:** [#3640](https://github.com/intersoftdatalabs-in/percussioncms/issues/3640)

H2 Playwright on `spa.jsp?entry=explorer` (no `rxFolderMutations=1`) for ReducedActions **Create Folder**: pathmanagement `addNewFolder` HTTP 200 and tree/list refresh when a Sites/Assets parent exists. **Sites/folders tree stays Partial.** Do not flip Present from this agent proof.
