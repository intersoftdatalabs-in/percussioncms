...
- **Cross-module artifacts**: When a removal affects `modules/perc-distribution-tree` or `modules/perc-ant` (packaging), rebuild the installer locally with `./mvnw -pl modules/perc-distribution-tree -am package` and inspect the produced `.ppkg` archive listing to confirm the removed file is absent.
