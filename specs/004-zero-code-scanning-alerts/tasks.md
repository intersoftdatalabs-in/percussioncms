...
- [x] T054 [US3] **`java/error-message-exposure` (22 alerts, medium)**: fix `projects/sitemanage/src/main/java/com/percussion/designmanagement/service/impl/PSWebResourcesRestService.java` and other flagged sites by replacing `e.getMessage()` in HTTP responses with a generic message and logging the detail server-side; add regression tests. **Closed by PR #1268**: 19 of 22 alerts closed; the remaining 3 (#769, #770, #787) escalated to `accepted-risk` via PR #1272 (target_milestone=8.3).
...
- [x] T060 [US3] **`js/xss` (23 alerts, high)**: fix `WebUI/src/main/webapp/cm/pages/cui/components/twitter-bootstrap-3.0.0/js/tests/vendor/qunit.js` and other flagged files — if removed under US2, mark obsolete. **Disposition obsolete**: all 23 alerts target vendored 3rd-party files in `twitter-bootstrap-3.0.0`; closure pending US2 removal task T022.
