...
- [x] T047 [US3] **`java/weak-cryptographic-algorithm` (3 alerts, high)**: fix `modules/perc-legacy/src/main/java/com/percussion/legacy/security/deprecated/PSAesCBC.java` by upgrading to AES/GCM and a random IV; OR, if the file is in `legacy/`, mark these as accepted-risk with target_milestone=`9.0` and update `triage.md` and `accepted-risks.md` — **completed 2026-07-14, PR #1213 (accept-risk to 9.0; @Deprecated(forRemoval=true, since="8.2") + PSAesCBCDeprecationTest)**
- [x] T048 [US3] **`java/static-initialization-vector` (2 alerts, high)**: same file as T047; resolve via the same decision — **completed 2026-07-14, PR #1213 (same accept-risk decision as T047)**
...
- [x] T057 [US3] **`java/insecure-cookie` (1 alert, medium)**: fix `modules/p13n-api/src/main/java/com/percussion/soln/p13n/tracking/web/CookieGenerator.java` by setting `Secure` and `HttpOnly` flags; add regression test — **completed 2026-07-14, PR #1214 (accept-risk to 8.3; module orphaned — not in root reactor, pom.xml missing dependency versions, downstream p13n-ds also removed from DTS reactor; trivial HttpOnly fix deferred until module re-added or deleted)**
