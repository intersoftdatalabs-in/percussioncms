...
- [x] T002 Confirm branch is based on JDK 21 `development` tooling and `./mvnw -v` (or equivalent) reports a usable JDK 21 toolchain
...
- [x] T006 Run initial compile of primary modules via `./mvnw -pl modules/utils,system -am compile -DskipTests` and capture full output to `tmp/jcr-compile-phase1.log`
...
**Independent Test**: `./mvnw -DskipTests compile` succeeds; `dependency:tree` shows only jcr 2.0; PR opened with compile scope only (no full deprecation cleanup required).  
...
- [x] T028 [US2] Run `./mvnw -DskipTests compile` to BUILD SUCCESS and re-run US2 unit tests (`modules/utils`, contentmgr tests)
- [x] T029 [US2] Verify `./mvnw -pl system -am dependency:tree -Dincludes=javax.jcr:jcr` shows only version 2.0
...
- [x] T042 [US1] Run designated automated tests: `./mvnw -pl modules/utils,system,projects/sitemanage -am test` (adjust modules if failures are pre-existing and documented)
...
- [x] T057 [US3] Run `./mvnw -pl system -am dependency:tree -Dincludes=javax.jcr:jcr` (and broader tree if needed) and record evidence of **2.0 only** in `specs/987-jcr-2-0-api-migration/dependency-tree.txt`
...
- [x] T062 [P] Run Spotless (or project format check) on all touched modules via `./mvnw` Spotless goals used by the repo
...
- [x] T065 Run full designated automated suite: `./mvnw -pl modules/utils,system,projects/sitemanage,modules/perc-toolkit -am test` (expand if CI requires)
