...
- **Dev runtime (NEW, 2026-07-19)**: A docker dev runtime is now operational — `docker compose -f docker-compose.yml up` brings up the cms-dts container against a host-side install at `/opt/Percussion`. Install is run once on host via `scripts/install-cms-dev.sh`; container is service-only. Default DB is **Derby** (MySQL mode is currently blocked by [issue #1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388) — collation bug at install — UI development does **not** require MySQL). The CMS is reachable at `http://localhost:9992/Rhythmyx/login` with Admin/Editor/Contributor credentials auto-discovered from `/opt/Percussion/var/config/generated/passwords`. This runtime enables Playwright E2E tests against a live CMS in CI/dev.
...
- **Dev runtime is the docker compose stack** (`docker-compose.yml` → cms-dts container) against a host-side install at `/opt/Percussion` (default). Install is run once on host via `scripts/install-cms-dev.sh`; container is service-only. Default DB is **Derby** for dev/test; MySQL mode is deferred until the install bug ([issue #1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388)) is fixed. UI development does **not** require MySQL.
