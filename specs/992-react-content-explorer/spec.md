...
- **Primary module(s)**: `WebUI/` (Finder includes, Web Management shell, modern React/TypeScript frontend under `WebUI/src/main/ts` and `WebUI/src/main/frontend`); `modules/DesktopContentExplorer/` (source of UX and capability requirements; retirement target); **`modules/perc-qa-automation/` (Playwright + TestNG end-to-end test module — primary automated test surface for this feature, replaces prior "scripted UAT" for SC-001..SC-011)**
...
- **AGENTS files to apply**: root `AGENTS.md`, `WebUI/AGENTS.md` (if present), `modules/perc-qa-automation/AGENTS.md` (Playwright QA agent profile), module AGENTS under Desktop Content Explorer / sitemanage when changing those modules
- **Dev runtime (NEW, 2026-07-19)**: A docker dev runtime is now operational — `docker compose -f docker-compose.yml up` brings up the cms-dts container against a host-side install at `/opt/Percussion`. Install is run once on host via `scripts/install-cms-dev.py`; container is service-only. Default DB is **Derby** (MySQL mode is currently blocked by [issue #1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388) — collation bug at install — UI development does **not** require MySQL). The CMS is reachable at `http://localhost:9992/Rhythmyx/login` with Admin/Editor/Contributor credentials auto-discovered from `/opt/Percussion/var/config/generated/passwords`. This runtime enables Playwright E2E tests against a live CMS in CI/dev.
...
### Session 2026-07-19 (late — docker dev runtime + Playwright)

- Q: How are "scripted UAT" scenarios for SC-001..SC-011 now executed? → A: **Per-story Playwright E2E specs** in `modules/perc-qa-automation/frontend/tests/`, runnable against the live docker dev CMS at `http://localhost:9992`. Each spec logs in as Admin/Editor/Contributor (auto-discovered credentials), navigates the surface under test, asserts state, and records evidence. Manual UAT remains the fallback when the docker runtime is unavailable. Vitest + Testing Library continue to cover component-level logic (reduced actions, filters, helpers).
- Q: What happens when the upstream REST endpoint has a bug (e.g. [issue #1387 FolderAdaptor ClassCastException](https://github.com/intersoftdatalabs-in/percussioncms/issues/1387))? → A: Playwright specs document the failure with `test.skip(...)` and a `BUG:` note. Component tests (Vitest) mock the API and pass independently of the upstream bug. When the upstream bug is fixed, flipping `test.skip` → `test(...)` makes the spec run; that PR is also the SC-008 evidence.
- Q: Is the MySQL/collation install bug ([#1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388)) a blocker? → A: **No.** The modern UI exercises CMS REST endpoints; the backend DB type (Derby vs MySQL) does not affect REST shape or UI behavior. The dev runtime defaults to Derby. MySQL mode is deferred until the connector + collation fix lands.
- Q: What changed in the problem statement regarding Desktop CE's stack? → A: **Correction** — `modules/DesktopContentExplorer/` is a **pure JavaFX 21** application (eclipse-temurin JRE 21, `perc-content-explorer-8.1.6-SNAPSHOT.jar` with `com.percussion.cx.PSContentExplorerApplication`). The earlier "Swing/AWT + JavaFX WebView" description in this spec is updated to reflect reality; the change does not affect the feature scope (Desktop CE is still retired as a content admin runtime).
...
| **Desktop Content Explorer**                    | JavaFX 21 (eclipse-temurin JRE 21 + `com.percussion.cx.PSContentExplorerApplication`); SOAP + HTTP | Explorer tree + detail list; powerful **server-driven action menus**; folder properties/security (ACL); search; clipboard; mature content admin workflows | JavaFX desktop runtime; separate from modern web shell; hard to support; not usable inside web dialogs                                                  |
...
> **Testing framework** (effective 2026-07-19): Each SC below is exercised by a **Playwright E2E spec** in `modules/perc-qa-automation/frontend/tests/` running against the live docker dev CMS. Vitest + Testing Library cover component-level logic (filter rules, ReducedAction confirm, lockout warning). Manual scenarios in `quickstart.md` remain the fallback when the docker runtime is unavailable. CI green = Vitest green + Playwright green + axe-core a11y gate (SC-009).

- **SC-001**: In automated Playwright UAT (`tests/us1-core-explorer.spec.js`) for the **Finder primary-nav hard cut**, the modern explorer completes navigate → open item → create folder → move or copy → delete (where permitted) **entirely via the modern web explorer** with **100%** of checklist steps passed and **zero** steps requiring miller-column Finder. Full CE menus, ACL UI, and full search are **not** required for this gate.
- **SC-002**: In automated Playwright UAT (one spec per in-scope host: `host-asset-picker.spec.js`, `host-page-picker.spec.js`, `host-aa-contentbrowser-dialog.spec.js`, `host-folder-picker.spec.js`, optional `host-home-library.spec.js`) for **each browser host hard-cut phase**, that host’s dialog flow completes navigate or search → select → confirm using the **reusable content browser**, returning a valid `SelectionResult` to the host, **100%** of checklist steps passed (other hosts may still be unmigrated).
- **SC-003**: In automated Playwright UAT (`tests/us3-menus.spec.js`) for a checklist of high-value CE actions (minimum **10** actions spanning open/edit, folder ops, and at least two workflow or properties actions; enumerated in `contracts/capability-matrix.md` P-Menu), each action is available from the modern UI for an authorized user and completes successfully (**100%** of list)—gate for **full menu / CE-parity phase**, not for Finder primary-nav hard cut.
- **SC-004**: In automated Playwright UAT (`tests/us4-acl.spec.js`), authorized admin completes view → edit → save of folder ACL/permissions on a test folder in the modern UI; a second user session reflects access changes on refresh (**pass/fail** scripted scenario)—gate for **ACL phase**, not Finder primary-nav hard cut.
- **SC-005**: Explorer remains usable on a folder with a large child set used in UAT (product-agreed fixture, target **≥ 500** children, created via `scripts/create-large-folder-fixture.sh`): user can scroll/page and open an item within **10 seconds** of selecting the folder on a standard office network in the test environment (applies at Finder primary-nav hard cut). Asserted by Playwright spec `tests/us1-perf-sc005.spec.js` against the live CMS.
- **SC-006**: After Finder primary-navigation hard cut, production Web Management content entry loads **zero** miller-column Finder chrome and offers **no** production classic fallback; cutover inventory items for that phase are **100%** signed off; SC-001 and SC-005 pass; `tests/us6-hard-cut.spec.js` green.
- **SC-007**: After Desktop CE retirement for in-scope workflows, Playwright UAT and docs confirm core-navigate content admin workflows are completed on modern web only; CE not required (**pass** on retirement checklist)—same core-navigate bar as SC-001/SC-006; full menus/ACL/search not required for this gate.
- **SC-008**: CI is green for automated coverage required by the phase under test: Vitest (component) + Playwright (E2E) + axe-core a11y gate (SC-009). No permanent skip of replaced legacy-only tests for retired surfaces — flipped `test.skip` → `test(...)` in `perc-qa-automation` is the SC-008 evidence for upstream bug fixes (see [issue #1387](https://github.com/intersoftdatalabs-in/percussioncms/issues/1387)).
- **SC-009**: Accessibility spot-check for surfaces in the phase under test (at Finder hard cut: primary tree and list at minimum) are keyboard-completable; **automated** via `axe-core` (run in Playwright specs and Vitest component tests) with zero serious/critical violations; supplemented by manual checklist for surface-only flows (e.g. embedded iframe scenarios).
- **SC-010**: Prefer-CE usability signal: in structured feedback from at least **5** internal users familiar with both UIs, majority rate modern explorer as **clearer for daily folder navigation** than miller-column Finder (document scores in `checklists/sc010-usability-notes.md`)—measured at or before Finder primary-nav hard cut. **This SC is the one criterion that is intentionally manual**; quantitative scores from internal users cannot be substituted by automated tests.
- **SC-011**: Capability matrix lists advanced CE items (clipboard, site/subfolder copy wizards, dependency viewer, IA/relationship views) with phase labels; **100%** of in-scope matrix rows (including advanced) are **Done** with acceptance met before 8.2 GA (no unlabeled omit; “scheduled for post-8.2” is **not** allowed). Each P-Adv row has a Playwright spec assertion in `tests/us7-advanced.spec.js`.
- **SC-012**: **8.2 release gate** — 8.2 GA is blocked until functional parity is proven: (1) Finder and Desktop CE retired for in-scope workflows, (2) in-scope content-browser hosts hard-cut, (3) full menus + ACL UI + search complete per matrix, (4) advanced CE matrix rows Done, (5) Playwright + Vitest suites green for the 8.2 build, (6) SC-001–SC-011 applicable checks pass. A build missing any of the above MUST NOT be labeled or shipped as 8.2 GA.
...
- **Dev runtime is the docker compose stack** (`docker-compose.yml` → cms-dts container) against a host-side install at `/opt/Percussion` (default). Install is run once on host via `scripts/install-cms-dev.py`; container is service-only. Default DB is **Derby** for dev/test; MySQL mode is deferred until the install bug ([issue #1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388)) is fixed. UI development does **not** require MySQL.
- **Backend DB type is not a UI implementation concern** ([issue #1388](https://github.com/intersoftdatalabs-in/percussioncms/issues/1388) is **not** a blocker). REST contract shape, auth, CSRF, and behavior are DB-agnostic. Tests run against Derby for dev/test simplicity.
- **Automated UAT via Playwright** (`modules/perc-qa-automation`) is the primary acceptance surface for SC-001..SC-011. Manual scenarios in `quickstart.md` remain the fallback for environments without the docker runtime.
- **Known upstream REST bug** ([issue #1387 FolderAdaptor ClassCastException](https://github.com/intersoftdatalabs-in/percussioncms/issues/1387)) affects `/rest/folders/by-path/...` and `/rest/items/...` endpoints with **500** responses even with valid Admin auth. Work that depends on those endpoints can:
  1. Mock the API in Vitest (component-level coverage proceeds independently).
  2. Mark the Playwright spec `test.skip` with a `BUG:` note; flip to `test(...)` when the fix lands.
  3. If a Phase 3+ task cannot proceed past this bug, **escalate the task into this feature scope** (per user's directive) so the fix is part of the 8.2 delivery.
