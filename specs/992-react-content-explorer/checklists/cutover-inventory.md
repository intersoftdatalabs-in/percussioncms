...
|---------|----------------|--------------------------|---------------|----------------|---------------|--------|
| `host-asset-picker` | Finder asset picker (uses `$.perc_finder().launchAssetPreview` and `perc_finder().refresh()`) | `WebUI/src/main/webapp/cm/widgets/perc_delete_page_button.js:21`, `…/widgets/PercActionDataTable/PercActionDataTable.js:121,131`, `…/views/PercPageView.js:1222` | `ContentBrowser` (P-Adv-1) | P-Host-1 | T045a | **Pilot complete (2026-07-19)**: new modern entry point `WebUI/src/main/webapp/cm/app/assetPickerModern.jsp` mounts `ContentBrowser` in select mode with `allowedTypes: ['page','asset']` and emits a `SelectionResult` on confirm. The 3 legacy `launchAssetPreview` / `perc_finder().refresh()` call sites in the existing jQuery view can be migrated to call `window.PercModernUI.mount("…", "ContentBrowser", { onConfirm: … })` from the existing handlers; that migration is the per-host follow-up (out of scope for the pilot commit). The pilot establishes the host contract. |
