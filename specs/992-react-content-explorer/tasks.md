...
- [x] T036 [US6] Commit US6 hard-cut PR; resolve review threads; **do not** claim 8.2 GA (SC-012 still open) *(done 2026-07-19 — PR #1390 merged; 8 primary-nav shells hard-cut to mount `ContentExplorerShell` via `PercModernUI` bridge; US6 review-thread resolution per-thread IDs in the cross-PR summary at `docs/ai-generated/code-reviews/992-react-content-explorer-pr-review-summary.md`; PR-babysit last_cycle.json confirms 0 open threads. The "do not claim 8.2 GA (SC-012 still open)" clause was honoured: T090 was deferred to the release manager per the 2026-07-21 user directive; FR-029 parity is a spec-internal QA gate, not an 8.2 GA gate, per the post-US8 SC-012 packet at `docs/ai-generated/release/992-8.2-parity-evidence.md` §9. Doc-drift close-out 2026-07-21.)*
...
- [x] T046b [US2] Run all per-host Playwright specs (`tests/host-*.spec.js`) against the live CMS and fix failures (asserts SC-002 for each host) *(done 2026-07-20 — 3 per-host Playwright specs authored and passing on the live docker dev CMS at `http://localhost:9992`: `tests/host-asset-picker.spec.js` (T045a-pw, 4 tests, 16.1 s); `tests/host-page-picker.spec.js` (T045b-pw, 4 tests); `tests/host-folder-picker.spec.js` (T045d-pw, 4 tests). Per-host SC-002 evidence recorded in `specs/992-react-content-explorer/checklists/cutover-inventory.md` P-Host section. PRs #1391 (US2 pilot) + #1394 (T045a-pw + doc drift fix). The 2 deferred host Playwright specs (`host-aa-contentbrowser-dialog.spec.js` / `T045c-pw`, `host-home-library.spec.js` / `T045e-pw`) are explicitly OUT for spec 992 per `checklists/cutover-inventory.md` P-Host OUT rows. Doc-drift close-out 2026-07-21.)*
- [x] T047 [US2] Commit US2 + host migrations PR(s); per-PR constitution IX review-thread resolution (inline reply with commit hash + `gh api graphql resolveReviewThread`) *(done 2026-07-20 — US2 PR train merged: PR #1386 (US1 core explorer, pre-US2 prerequisite), PR #1391 (US2 ContentBrowser pilot — host-asset-picker migration), PR #1394 (T045a-pw + doc drift fix). Per-PR review-thread resolution per constitution IX; PR-babysit last_cycle.json confirms 0 open threads. Phase 5 sign-off recorded in `checklists/cutover-inventory.md` P-Host section + cross-PR summary at `docs/ai-generated/code-reviews/992-react-content-explorer-pr-review-summary.md`. The US2 commit + thread-resolution gate is fully satisfied; the deferred host migrations (AA dialog + home-library) are explicitly OUT for spec 992 per the P-Host OUT rows. Doc-drift close-out 2026-07-21.)*
...
- [ ] T052a [US3] If a new façade or REST endpoint is added per T052, write a **service-contract integration test** in `projects/sitemanage` or `rest` per constitution III/IV — happy path + AuthZ negative + CSRF negative; run via `./mvn-env.sh -pl <module> -am test` on JDK 21 *(GATED by T052 outcome — T052 decided NO new façade; T052a is therefore N/A for this PR; not applicable)*
- [ ] T052b [US3] If a new façade or REST endpoint is added per T052, add a **threat-model note** for the new endpoint (zip-slip / XXE / CSRF / server-side AuthZ) per constitution VI in PR evidence; no secrets logged *(GATED by T052 — N/A for this PR; not applicable)*
...
- [ ] T057 [US3] Run Vitest for action menu tests; commit US3 PR; per-PR constitution IX review-thread resolution (inline reply with commit hash + `gh api graphql resolveReviewThread`) *(pending — Vitest green (23/23); Playwright green (5/5); Erlang review pending; commit + PR open pending)*
...
- [ ] T092 [P] [US8] Vitest unit suite for `relationshipsApi.ts` wire shapes + DTO mapping under `WebUI/src/test/ts/contentExplorer/relationshipsApi.test.ts` (happy path, empty summary default `count: 0`, defensive `null` guards, wire envelope for outgoing / incoming / taxonomy / local / reverse / summary). ≥6 tests.
- [ ] T093 [P] [US8] Vitest component tests for `DependencyViewer.test.tsx` updated to mock the new typed API + assert **all 6 dimensions populated**, no `clientSidePreview` banner when summary is authoritative. ≥3 tests.
- [ ] T094 [P] [US8] Vitest component tests for `RelationshipsView.test.tsx` updated analogously. ≥3 tests.
- [ ] T095 [P] [US8] **Playwright spec `tests/us8-dependency.spec.js`** (NEW): mount `cm/app/us7AdvancedModern.jsp`, log in as Admin, click into a node, drive a server round-trip to `/Rhythmyx/rest/content-explorer/relationships/<itemId>/summary` via the React component, assert all 6 dimension rows show non-zero or "0 (no links)" (NOT "—"). ≥3 tests.

### Implementation — server (Java, sitemanage + rest modules)

- [ ] T096 [US8] Define the Java DTOs at `rest/src/main/java/com/percussion/share/relationship/`: `PSRelationshipSummary` (used by outgoing / incoming / reverse), `PSTaxonomySummary` (taxonomy), `PSLocalDependencySummary` (local), `PSNodeRelationshipSummary` (the consolidated #7 wrapper). All five mirror existing server DTOs (no invented fields per constitution II).
- [ ] T097 [US8] New sitemanage service in `projects/sitemanage/src/main/java/com/percussion/share/relationship/`: `IPSRelationshipSummaryService` interface + `PSRelationshipSummaryService` impl wrapping `IPSRelationshipCataloger` (outgoing / incoming / reverse), `IPSNodeService` (taxonomy), and `IPSWidgetAssetRelationshipService` plus page/widget DAO layer (local). AuthZ: server-side ACL check on `itemId` per the existing `accessLevel` model.
- [ ] T098 [US8] JAX-RS resource at `rest/src/main/java/com/percussion/share/relationship/RelationshipSummaryResource.java`: 5 GET endpoints + 1 consolidated summary endpoint. Path base `/Rhythmyx/rest/content-explorer/relationships`. Wire envelopes: `{"PSRelationshipSummary": ...}` / `{"PSTaxonomySummary": ...}` / `{"PSLocalDependencySummary": ...}` / `{"PSNodeRelationshipSummary": ...}`. No request body; no CSRF (GET). Returns 403 on AuthZ denial (with shape `{ "error": { "code": "...", "message": "..." } }`).
- [ ] T099 [US8] Service-contract integration test at `rest/src/test/java/com/percussion/share/relationship/RelationshipSummaryResourceTest.java`: happy path (counts returned for the canonical Admin case), AuthZ negative (View-only caller gets 403), JSON wire envelope round-trip. ≥3 tests. Pattern matches the existing `rest/src/test/java/com/percussion/rest/actions/` convention.

### Implementation — client (WebUI TypeScript)

- [ ] T100 [US8] TypeScript types in `WebUI/src/main/ts/api/contentExplorer/relationship.ts` (new file): mirror of the 4 server DTOs. Strict enums for the dimension keys `outgoing / incoming / aa / taxonomy / local / reverse`.
- [ ] T101 [US8] Typed fetch client `WebUI/src/main/ts/api/contentExplorer/relationshipsApi.ts`: exports `fetchOutgoing(itemId)`, `fetchIncoming(itemId)`, `fetchTaxonomy(itemId)`, `fetchLocal(itemId)`, `fetchReverse(itemId)`, `fetchRelationshipSummary(itemId)` (= #7). Each function calls the matching rest endpoint, parses the wire envelope, returns the typed DTO.
- [ ] T102 [US8] Update `WebUI/src/main/ts/contentExplorer/views/dependencyModel.ts`: replace the morning "unknown + clientSidePreview" branch with `await fetchRelationshipSummary(itemId)` (or accept the summary as a prop when the host pre-fetches). `synthesiseRelationshipSummary(...)` is retained only as the unit-test fallback (no longer used in production paths).
- [ ] T103 [US8] Wire the new fetch into `DependencyViewer.tsx` + `RelationshipsView.tsx`: load on mount (`useEffect`); show a transient loading skeleton until the summary arrives, then render the 6 dimensions with the supplied counts. Drop the `clientSidePreview` banner when at least one server dimension is authoritative.
- [ ] T104 [US8] US8 PR(s) opened + per-PR constitution IX review-thread resolution (inline reply with commit hash + `gh api graphql resolveReviewThread`); Erlang pre-push review report at `docs/ai-generated/code-reviews/992-react-content-explorer-us8-{rest,sitemanage,webui}-erlang.md`. **P-Adv matrix row flips from Partial to Implemented.**
