...
- [x] T002 Confirm JDK 21 branch baseline and that WebUI modern tests run via `./mvnw -pl WebUI` (and/or frontend npm/vitest per `WebUI` docs)
...
- [x] T029b [P] [US6 → Phase 10] **CI-gate artifact-grep** for FR-019a: ensure the production-built **WebUI WAR** contains zero `finder*.jsp` navigation-entry references in `cm/app/webmgt.jsp` post-US6. Implementation: `scripts/verify-no-finder-jsp-references.sh` (+ `.bat` Windows counterpart) with regex matched against the **navigation-entry forms** (`<jsp:include page="includes/finder.jsp">`, `<%@include file="includes/finder.jsp">`), explicitly carving out (a) the `finder_js.jsp` shared-library include (required for non-Finder functionality: `PercComponentWrapper`, `PercViewReadyManager`, `PercPathService`, etc.) and (b) `cm/pages/app/webmgt.jsp` Track A path (deferred to the Track A migration workstream per `WebUI/AGENTS.md`). Multi-line JSP comment stripping via `perl -0777 -pe 's/<%--.*?--%>//gs'` so explanatory comments at the cutover boundary don't false-positive. Paired shell self-test `scripts/test-verify-no-finder-jsp-references.sh` exercises 4 cases: PASS on current tree, FAIL on re-introduced `<jsp:include>`, FAIL on `<%@include>`, PASS on `finder_js.jsp` shared-lib include (carve-out). Vitest spec `WebUI/src/test/ts/scripts/verify-no-finder-jsp-references.test.ts` is the load-bearing CI assertion that fires on every `npx vitest run` (2 tests passing: script file present, PASS on current tree). *(done 2026-07-21 — 2/2 Vitest tests passing; 4/4 shell self-test cases passing. `scripts/README.md` entry added under "Other scripts in this directory" with purpose + Spec ref T029b.)*
...
- [x] T092e [P] [Edge Cases #11] **Network-failure-mid-action test**: Vitest + Playwright scenario where the user starts a move / copy / paste action, the network drops mid-flight, and the action is then retried. Assert: (a) no data corruption in the destination folder; (b) the UI surfaces a recoverable error rather than a hard fail; (c) re-auth + retry works without a hard refresh. 4 new Vitest tests in `WebUI/src/test/ts/contentExplorer/clipboardApi.test.ts` (T092e describe block): network-drop TypeError surfaces with no status field (transport-level failure, not HTTP); 401 session-expired surfaces with `status: 401` and recoverable re-auth-required message; re-auth + retry succeeds (second `pasteClipboardItems` call is independent of the first's failure); per-item boundary preserved on mixed success+failure (Promise.allSettled keeps per-item outcomes, no silent overwrite). Playwright spec `modules/perc-qa-automation/frontend/tests/us8-edge-cases-network-failure.spec.js` (NEW) uses `page.route` to abort the moveItem endpoint and assert the modern explorer mounts cleanly under the simulated drop. *(done 2026-07-21 — 12/12 clipboardApi tests passing (5 original + 3 T092c + 4 T092e). Existing `pasteClipboardItems` Promise.allSettled + per-item message contract satisfies the recoverable-error + no-data-corruption requirement.)*
