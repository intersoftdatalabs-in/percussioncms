...
- [x] **VII. Build & Dependency Hygiene** — Vite + Maven frontend; JDK 21 via mvn-env; **`modules/perc-qa-automation/pom.xml` adds `frontend-maven-plugin` to install Node + Playwright**, runs `npm ci` in `generate-resources`; testng 6.9.6 explicit (parent pom doesn't manage it on `development`)
