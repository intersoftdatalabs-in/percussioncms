...
(function ($) {
  ...
  function initializeForm() {
    ...
    var formDataStr = $("input[name='" + formFieldName + "']")
      .val()
      .trim();
    var formData = null;
    if (formDataStr.length > 0) formData = JSON.parse(formDataStr);
    ...
    $(document).on("click", ".toggle-editor", function () {
      ...
      if (extEditorElem.length) deactivateEditor(extEditorElem);
    ...
    $(document).on("click", ".delete-field", function () {
      if (
        tinymce.EditorManager.get("elm1") &&
        $(this).parent().parent().children("#elm1").length > 0
      ) {
    ...
    $(".form-radio-buttons-label").on("click", function () {
      var newElem =
        $.PercFormController().getNewFieldEditor("PercRadioControl");
      addEvents(newElem);
      $("#perc-form-dnd-fields").append(newElem);
      newElem.find(".toggle-editor").trigger("click");
      $("iframe").scrollTop(20000);
    ...
    if (
      $("div[type='sys_error']").find("input[name='sys_title']").length > 0 ||
      $("div[type='sys_error']").find("input[name='formuniquenamevalidator']")
        .length
    ) {
      var errorLabelHtml =
        '<br/><label style="display: block;" generated="true" for="sys_title" class="perc_field_error">This field cannot be empty, must be unique within the system.</label>';
  ...
  function addConfig(elem) {
    if (
      $.PercFormController()
        .getControl($(elem).children().eq(0))()
        .getAvailablePrefs().length > 0
    )
      configButtonHtml =
        "<span class='toggle-configure'><img src='../rx_resources/widgets/form/images/configure.png' alt='Configure'></span>";
    else
      configButtonHtml =
        "<span class='toggle-configure-disabled'><img src='../rx_resources/widgets/form/images/configure2.png' alt='Configure'></span>";
    elem
      .find(".field-editor")
      .append(
  ...
  function _showFieldErrorMessage(reqFieldIds) {
    ...
    var errorLabelHtml =
      '<label style="display: block;margin-left: 10px;" generated="true" for="formerrorpage" class="perc_field_error">This is a required field.</label>';
    ...
    if (
      isFieldError &&
      $("#perc-form-edit-metadata-link").hasClass("perc-form-spacer")
    ) {
  ...
  function updateFormFields() {
    ...
    var reqFieldIds = [
      "perc-formbuild-success-url",
      "perc-formbuild-error-url",
    ];
    ...
    if (_showFieldErrorMessage(reqFieldIds)) return false;
    ...
    var script =
      "<script id='form-script'>window.addEventListener('DOMContentLoaded', function() {jQuery(document).ready(function(){jQuery('#" +
      formConfig.name +
      "').validate()});});</script>";
    ...
    $("#perc-content-edit-renderedform").val(
      script +
        $("<div/>")
  ...
  function openPrefsDialog(control, preferences, values) {
    ...
    if (typeof preferenceVals != "object") preferenceVals = {};
    ...
    $("#perc-field-prefs-apply")
      .off("click")
      .on("click", function () {
        var isValid = true;
        for (i = 0; i < preferences.length; i++) {
          isValid = isValid
            ? preferences[i].pref.validate(preferences[i].defaults)
            : isValid;
        }
        if (!isValid) return;
        ...
        for (i = 0; i < preferences.length && onApplySuccessful; i++) {
          preferencesAfterOnApply[preferences[i].pref.name] = preferences[
            i
          ].pref.onApply(preferenceVals, preferences[i].defaults);
          // Check that the onApply function was succesfull comparing with null
          onApplySuccessful =
            preferencesAfterOnApply[preferences[i].pref.name] != null;
        }
