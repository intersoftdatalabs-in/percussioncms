  ...
  $.socialButtonsSetup = function () {
    ...
    function renderEditor() {
      ...
      if (data !== null) {
        ...
        if (data.buttonType === "page") {
          $("#perc-social-button-type").val("page").trigger("change");
        } else {
        ...
        $(".perc-social-button-row")
          .sort(sort_elements) // sort elements
    ...
    function _attachEvents() {
      ...
      function moveRowUp(thisObj) {
        if (
          thisObj
            .parents(".perc-social-button-row")
            .prev()
            .hasClass("perc-social-header") === false
        ) {
          thisObj
            .parents(".perc-social-button-row")
            .insertBefore(thisObj.parents(".perc-social-button-row").prev());
          if (
            thisObj.parents(".perc-social-button-row").next().css("display") ==
            "none"
          ) {
            moveRowUp(thisObj);
          }
      ...
      function moveRowDown(thisObj) {
        if (
          thisObj.parents(".perc-social-button-row").is(":last-child") === false
        ) {
          thisObj
            .parents(".perc-social-button-row")
            .insertAfter(thisObj.parents(".perc-social-button-row").next());
          if (
            thisObj.parents(".perc-social-button-row").prev().css("display") ==
            "none"
          ) {
            moveRowDown(thisObj);
          }
    ...
    function validateForm() {
      $.each(config, function (key, value) {
        ...
        if (
          data.buttonType == "page" &&
          value.enableButton === true &&
          urlValid === false
        ) {
          invalid = true;
          return false;
