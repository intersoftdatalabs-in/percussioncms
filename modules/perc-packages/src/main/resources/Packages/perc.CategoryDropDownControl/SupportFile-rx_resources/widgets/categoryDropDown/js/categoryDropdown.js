(function ($) {
  $.fn.dropDownControl = function (opts) {
    ...
    function showCategories(container, xml) {
      ...
      if (sortorder != null) sortOptions(container, sortorder);

      if (readonly === "yes") containerSelect[0].disabled = "disabled";
    ...
    containerSelect.on("change", function () {
      ...
      if (
        $(parentContainer + " :selected").val() !== null &&
        $(parentContainer + " :selected").val() !== ""
      ) {
        if (autoGenerate === "yes") {
          var value = {
            id: $(parentContainer + " :selected").val(),
          ...
          $(parentContainer + " :selected").each(function (i, selected) {
            if ($(selected).val() != null) {
              value = { id: $(selected).val(), title: $(selected).text() };
              setVal.push(value);
            }
          });
    ...
    $(document).on(
      "change",
      'select[id^="subCategory-' + paramName + '"]',
      function () {
    ...
    function setSubSelectsValues(subSelectId, setValue) {
      ...
      for (var i = 0; i < subSelectId; i++) {
        var value = {
          id: $("#" + subSelect[i].id + " :selected").val(),
              ...
              if (x[i].childNodes.length > 0) {
                ...
                if (
                  !$("#subCategory-" + paramName + "-" + subSelectIdVal)
                    .length ||
                  $("#subCategory-" + paramName + "-" + subSelectIdVal).length <
                    1
                ) {
    ...
    function getChildNodes(node, parentId) {
      ...
      if (parentId == null) childNodeList = node.childNodes;
      else {
