...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="/" />
	<!-- percDefaultLangControl -->
	<psxctl:ControlMeta name="percDefaultLangControl"
		...
		<psxctl:AssociatedFileList>
			...
			<psxctl:FileDescriptor
				name="percDefaultLang.css" type="css" mimetype="text/css">
				<psxctl:FileLocation>/rx_resources/widgets/percDefaultLang/css/percDefaultLang.css</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="percDefaultLang.js" type="script" mimetype="text/javascript">
				<psxctl:FileLocation>/rx_resources/widgets/percDefaultLang/js/percDefaultLang.js</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
	...
	<xsl:template
		match="Control[@name='percDefaultLangControl']" mode="psxcontrol">
		<input type="hidden" name="{@paramName}"
			id="perc-content-edit-{@paramName}" value="{Value}" />
		<div class="perc-default-lang" id="{@paramName}">
			<h2>International SEO - Default Language &amp; Alternates</h2>
			...
			<div class="perc-help">
				To ensure that search engines index your international sites
				correctly, add this Widget to all Templates on International sites.
				Add a reference to the table below for each Site, Language, and
				Region combination. Language is always required.
				When the Region or
				Country is selected, search engines will only return
				results from the
				international site to users speaking that language
				in that region.
				Otherwise UK English content might be returned before US English
				content on
				US search engines and vice versa.

				Tips:
				<ul>
					<li>International sites should follow identical site folder layouts
						or this widget can't generate the alternate url.</li>
					<li>Always choose language and country when you want search engines
						to return search results only in that region.</li>
					<li>Save this Widget as a Shared Asset and re-use that asset on all
						sites.</li>
					...
					<li>
						...
						<a
							...
							target="_blank" rel="noopener noreferrer">rel=alternate href-lang=x</a>
					</li>
				</ul>
			</div>
			<div id="perc-default-lang-editor"
				class="perc-default-lang-editor">
				...
				<table class="perc-table">
					<tbody>
						...
						<tr class="hide perc-defaultlang-row" role="presentation">
							<td>
								<select id="perc-lang-protocol"
									class="perc-lang-protocol">
									<option value="http" title="http">http</option>
									<option value="https" title="https">https</option>
								</select>
							</td>
							<td>
								<select id="perc-site-list" class="perc-site-list"><!-- Sites go Here -->
								</select>
							</td>
							<td>
								<select id="perc-lang-list" class="perc-lang-list">
									...
									<option value="kl"
										title="kalaallisut, kalaallit oqaasii">Kalaallisut, Greenlandic</option>
									...
									<option value="li" title="Limburgs">Limburgish, Limburgan,
										Limburger</option>
									...
									<option value="cu" title="ѩзыкъ словѣньскъ">Old Church Slavonic, Church
										Slavonic, Old Bulgarian</option>
									...
									<option value="bo" title="བོད་ཡིག">Tibetan Standard, Tibetan,
										Central</option>
							...
							<td>
								<select id="perc-country-list" class="perc-country-list">
									<option value="">None</option>
									<option value="AF">Afghanistan</option>
									<option value="AX">&Aring;land
										Islands
									</option>
	...
	<xsl:template
		match="Control[@name='percDefaultLangControl' and @isReadOnly='yes']"
		priority="10" mode="psxcontrol">
		<input type="hidden" name="{@paramName}"
			id="perc-content-edit-{@paramName}" value="{Value}" />
		<div class="perc-default-lang-readonly" id="{@paramName}">
