      ...
      onSelect: function (value, date) {
        ...
        if (p_start_date > p_end_date) {
          $.perc_utils.alert_dialog({
            title: "Error",
            content:
              "First published on or after date must be less than First published before date.",
            okCallBack: function () {
              setDisplayDate(
                $('[name="start_date"]').val(),
                "display_start_date",
      ...
      onSelect: function (value, date) {
        ...
        if (p_start_date >= p_end_date) {
          $.perc_utils.alert_dialog({
            title: "Error",
            content:
              "First published before date must be greater than First published on or after date.",
            okCallBack: function () {
      ...
      $.PercServiceUtils.makeJsonRequest(
        ...
        function (status, result) {
          if (status === $.PercServiceUtils.STATUS_SUCCESS) {
            ...
            var tempsArray = [];
            var tempIds =
              $("#perc_template_list").val() !== ""
                ? $("#perc_template_list").val().split(",")
                : "";
