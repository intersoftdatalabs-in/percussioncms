...
/**
 *    jquery.categoryList.js
 *
 *    Java Script code for the control categoryListWidgetControl used by the CategoryListWidget
 *    This control used to build the JCR query for the Category List widget, based on
...
function getPageTemplates() {
  ...
  $.each($(".perc-pagetemplates-chkbox"), function () {
    if (this.checked) pageTpls.push(this.value);
  });
 ...
 */
function buildQuery() {
  var p_pageresults = $('[name="category_page_result"]').val();
 ...
 */
(function ($) {
  $.fn.categoryListControl = function (settings) {
    var config = {};
    ...
    function showPageTemplates() {
      ...
      if (typeof $('[name="page_templates_list"]').val() != "undefined") {
        pageTemplatesOptionsArray = $('[name="page_templates_list"]')
          .val()
          .split(",");
      }
    ...
    function populateTemplateTypes(siteName, pageTemplatesOptionsArray) {
      // has to inject the HTML category here because $('.ui-perc-categorylist-pagetypes').empty();
      var tplPath = $.perc_paths.TEMPLATES_USER;
      if (siteName) {
        tplPath = $.perc_paths.TEMPLATES_BY_SITE + "/" + siteName;
      }
      $(".ui-perc-categorylist-pagetypes")
        .empty()
        .append($('<div id="perc-pagetemplates-container"/>'));
      $.getJSON(tplPath, function (data) {
        var tpls = data.TemplateSummary;
        for (let i = 0; i < tpls.length; i++) {
          var tpl = tpls[i];
          var checked =
            $.inArray(tpl.id, pageTemplatesOptionsArray) === -1
              ? ""
              : " checked ='true' ";
          $("#perc-pagetemplates-container").append(
            $(
              "<div class='perc-pagetemplates-entry'><input type='checkbox' class='perc-pagetemplates-chkbox'" +
                checked +
                " value='" +
                tpl.id +
                "'/><span title='" +
    ...
    function showSites() {
      $(document).ready(function () {
        ...
        function setPath(pathItem) {
          ...
          if (
            oldPath != pathItem.folderPath &&
            getSiteFromFolderPath(oldPath) !=
  ...
  function handleBrowseButtonClick(dlgTitle, inputElemId) {
    $.perc_browser({
      on_save: function (spec, closer, show_error) {
        ...
        $('[name="category_page_result"]').val(pagePath);
        closer();
      },
