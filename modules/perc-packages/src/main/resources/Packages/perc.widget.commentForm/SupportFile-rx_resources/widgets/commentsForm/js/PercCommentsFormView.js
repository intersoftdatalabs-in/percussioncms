...
(function ($) {
  ...
  function attachActionButtons(editorHtml) {
    editorHtml.find(".field-editor").each(function () {
      ...
      if (
        fieldType === "PercTextareaFieldControl" ||
  ...
  function deactivateEditor(extEditorElem) {
    ...
    var newElem =
      $.PercCommentsFormController().toggleFieldEditor(checkEditorExt);
    if (
      fieldType === "PercTextareaFieldControl" ||
      fieldType === "PercSubmitButtonControl"
    ) {
  ...
  function addConfig(elem, isComment) {
    ...
    if (
      $.PercCommentsFormController()
        .getControl($(elem).children().eq(0))()
        .getAvailablePrefs().length > 0
    )
      configButtonHtml =
        "<span class='toggle-configure'><img alt=''  src='../rx_resources/widgets/commentsForm/images/configure.png' ></span>";
    else
      configButtonHtml =
        "<span class='toggle-configure-disabled'><img alt='' src='../rx_resources/widgets/commentsForm/images/configure2.png' ></span>";
    if (isComment) {
      elem
        .find(".field-editor")
        .append(
  ...
  function updateFormFields() {
    ...
    var script =
      "<script id='form-script' type='text/javascript'>window.addEventListener('DOMContentLoaded', function() {jQuery(document).ready(function(){jQuery('form[name=commentForm]').validate({errorClass:'form-error-msg'})});});</script>";
