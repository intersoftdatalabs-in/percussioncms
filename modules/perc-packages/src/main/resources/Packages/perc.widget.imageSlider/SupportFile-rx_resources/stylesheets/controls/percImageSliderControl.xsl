...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta name="percImageSliderControl"
		...
		<psxctl:ParamList>
			...
			<psxctl:Param name="columncount" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies the number of column(s)
					displayed.</psxctl:Description>
				<psxctl:DefaultValue>1</psxctl:DefaultValue>
			</psxctl:Param>
			<psxctl:Param name="columnwidth" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter specifies the width of the column
					in pixels or percentage.</psxctl:Description>
		...
		<psxctl:AssociatedFileList>
			...
			<psxctl:FileDescriptor
				name="PercSiteService.js" type="script" mimetype="text/javascript">
	...
	<xsl:template
		match="Control[@name='percImageSliderControl']" mode="psxcontrol">
		<xsl:variable name="lang" select="//@lang" />
		<input type="hidden" name="{@paramName}"
			id="perc-content-edit-{@paramName}" value="{Value}" />
		<label for="perc-image-slider-setup-toggle"></label>
		<div class="perc-image-slider" id="{@paramName}">
			<h2>
				<xsl:call-template name="getLocaleString">
					<xsl:with-param name="key"
						select="'perc.ui.control.imageSlider@Add Slide'" />
					<xsl:with-param name="lang" select="$lang" />
				</xsl:call-template>
			</h2>
			<h4 id="slideConfigurationAlert"
				class="hidden perc-slider-warning">
				<xsl:call-template name="getLocaleString">
					<xsl:with-param name="key"
						select="'perc.ui.control.imageSlider@Slider Warning'" />
					<xsl:with-param name="lang" select="$lang" />
				</xsl:call-template>
			</h4>
			<!--<span id="perc-help-toggle" class="perc-toggle-help fa fa-question-circle"
				title="Click to toggle help"></span><label for="perc-help-toggle">Click to
				toggle help.</label> <div class="perc-help"> Image Slider Help Section </div> -->
			<div id="perc-image-slider-setup-editor"
				class="perc-image-slider-setup-editor">
				<span class="perc-table-add fa fa-plus-square"></span>
				<table class="perc-table">
					<tbody>
						<tr class="perc-header-row">
							<td class="perc-input-header perc-slider-image-preview-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Preview'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-preview-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Size'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-path-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Image'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-caption-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Slide Text'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-caption-2-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Slide Text'" />
									<xsl:with-param name="lang" select="$lang" />
							...
							<td class="perc-input-header perc-slider-image-link-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Internal Link'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-link-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@External Link'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td class="perc-input-header perc-slider-image-link-td">
								<xsl:call-template name="getLocaleString">
									<xsl:with-param name="key"
										select="'perc.ui.control.imageSlider@Link Type'" />
									<xsl:with-param name="lang" select="$lang" />
								</xsl:call-template>
							</td>
							<td></td>
						...
						<tr class="hide perc-image-slider-row" role="presentation">
							<td id="perc-content-image-thumbnail"
								class="perc-thumbnail-td">
								&nbsp;
							</td>
							...
							<td class="perc-input-td perc-slider-image-path-td">
								<input readonly="readonly"
									id="perc-content-edit-imageInputField"
									data-perc-widget-event-source="imageSlider"
									data-perc-image-content-id="" data-perc-image-path-link-id=""
									type="text" name="imageInputField"
									class="datadisplay perc-slider-image-path"></input>
								<input alt="Browse Images"
									id="perc-image-slider-browse-button"
									class="fa fa-search perc-select-button perc-image-field-select-button"
									type="button" for="perc-content-edit-imageInputField"
									value="&#xf002;"></input>
							</td>
							<td class="perc-input-td perc-slider-image-caption-td">
								<input name="imageCaption"
									class="datadisplay perc-slider-image-caption"
									style="height: 15px;padding-top: 3px;padding-bottom: 5px;"></input>
							</td>
							<td>
								<input name="imageCaption2"
									class="datadisplay perc-slider-image-caption-2"
									style="height: 15px;padding-top: 3px;padding-bottom: 5px;"></input>
							</td>
							<td class="perc-input-td perc-slider-image-link-td">
								<input readonly="readonly"
									id="perc-content-edit-imageLinkField"
									data-perc-page-content-id="" data-perc-page-path-link-id=""
									type="text" name="imageLinkField"
									class="datadisplay perc-slider-image-link"></input>
								<input alt="Browse Internal Pages"
									id="perc-image-slider-browse-button"
									class="fa fa-search perc-select-button perc-page-field-select-button"
									type="button" for="perc-content-edit-imageLinkField"
									value="&#xf002;"></input>
							</td>
							<td class="perc-input-td perc-slider-image-external-link-td">
								<input name="imageExternalLinkField"
									class="datadisplay perc-slider-image-external-link"
									style="height: 15px;padding-top: 3px;padding-bottom: 5px;"
									placeholder="https://www.percussion.com"></input>
							</td>
							<td class="perc-input-td perc-slider-image-link-setting-td">
								<select id="perc-slider-image-link-setting"
									class="perc-slider-image-link-setting-select"
									name="imageLinkSettingField">
									<option value="none">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'perc.ui.control.imageSlider@None'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
									</option>
									<option value="internal">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'perc.ui.control.imageSlider@Internal'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
									</option>
									<option value="external">
										<xsl:call-template name="getLocaleString">
											<xsl:with-param name="key"
												select="'perc.ui.control.imageSlider@External'" />
											<xsl:with-param name="lang" select="$lang" />
										</xsl:call-template>
									</option>
								</select>
							...
							<td class="perc-move-button-container">
								<div role="button" tabindex="0" id="move-up"
									class="perc-move-button perc-move-button-up">
									<i class="fa fa-caret-up" aria-hidden="true"></i>
								</div>
								<div role="button" tabindex="0" id="move-down"
									class="perc-move-button perc-move-button-down">
			...
			<div id="slideConfigurationAlertDetails" class="hidden">
				<h4>
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'perc.ui.control.imageSlider@Slider Warning Details'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
				</h4>
				<p id="sliderWarningDetails" class="hidden">
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'perc.ui.control.imageSlider@Slider Warning Details Message'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
				</p>
				<p id="sliderErrorDetails" class="hidden">
					<xsl:call-template name="getLocaleString">
						<xsl:with-param name="key"
							select="'perc.ui.control.imageSlider@Slider Error Details'" />
						<xsl:with-param name="lang" select="$lang" />
					</xsl:call-template>
				</p>
			</div>
	...
	<xsl:template
		match="Control[@name='percImageSliderControl' and @isReadOnly='yes']"
	...
	<psxi18n:lookupkeys>
		...
		<key
			name="perc.ui.control.imageSlider@Slider Warning Details Message">Slider Warning Details Message</key>
