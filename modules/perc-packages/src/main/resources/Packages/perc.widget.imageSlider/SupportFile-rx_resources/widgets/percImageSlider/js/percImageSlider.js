...
(function ($) {
  ...
  $.percImageSliderSetup = function () {
    ...
    var sliderImagePathBaseId = $table
      .find("tr.hide td input.perc-slider-image-path")
      .attr("id");
    var sliderImageLinkBaseId = $table
      .find("tr.hide td input.perc-slider-image-link")
      .attr("id");
    imageCount = 0;
    ...
    function renderEditor() {
      ...
      var dataStr = $("input[name='" + dataFieldName + "']")
        .val()
        .trim();
      ...
      if (data !== null) {
        for (var i = 0; i < data.percJSONConfig.length; i++) {
          ...
          $clone
            ...
            .val(data.percJSONConfig[i].percImagePath);
          $clone
            .find(".perc-slider-image-caption")
            .val(data.percJSONConfig[i].imageCaption);
          $clone
            .find(".perc-slider-image-caption-2")
            .val(data.percJSONConfig[i].imageCaption2);
          $clone
            .find(".perc-slider-image-link")
            .val(data.percJSONConfig[i].percPagePath);
          $clone
            .find(".perc-slider-image-external-link")
            .val(data.percJSONConfig[i].externalLink);
          $clone
            .find("#perc-slider-image-link-setting")
            .val(data.percJSONConfig[i].linkSetting);
          $clone
            .find(".perc-slider-image-path")
            .attr(
    ...
    function _attachEvents() {
      ...
      function moveRowUp(thisObj) {
        if (
          thisObj.parents(".perc-image-slider-row").prev().hasClass("hide") ===
          false
        ) {
          thisObj
            .parents(".perc-image-slider-row")
            .insertBefore(thisObj.parents(".perc-image-slider-row").prev());
        }
      }
      function moveRowDown(thisObj) {
        if (
          thisObj.parents(".perc-image-slider-row").is(":last-child") === false
        ) {
          thisObj
            .parents(".perc-image-slider-row")
            .insertAfter(thisObj.parents(".perc-image-slider-row").next());
        }
      }
    }
    ...
    function getData() {
      ...
      $(".perc-image-slider-row").each(function () {
        if (!$(this).hasClass("hide")) {
          ...
          alt["imageCaption"] = $(this)
            .find(".perc-slider-image-caption")
            .val();
          alt["imageCaption2"] = $(this)
            .find(".perc-slider-image-caption-2")
            .val();
          alt["percPagePath"] = $(this).find(".perc-slider-image-link").val();
          alt["externalLink"] = $(this)
            .find(".perc-slider-image-external-link")
            .val();
          alt["linkSetting"] = $(this)
            .find("#perc-slider-image-link-setting :selected")
            .val();
          alt["percImageContentId"] = $(this)
            .find(".perc-slider-image-path")
            .attr("data-perc-image-content-id");
          alt["percPageContentId"] = $(this)
            .find(".perc-slider-image-link")
            .attr("data-perc-page-content-id");
          alt["percImagePathLinkId"] = $(this)
            .find(".perc-slider-image-path")
            .attr("data-perc-image-path-link-id");
          alt["percPagePathLinkId"] = $(this)
            .find(".perc-slider-image-link")
            .attr("data-perc-page-path-link-id");
  ...
  function validateAssetItems() {
    ...
    $(".perc-image-slider-row").each(function () {
      ...
      if (!$(this).hasClass("hide")) {
        currentElement = this;
        currentImagePath = $(currentElement)
          .find(".perc-slider-image-path")
          .val();
        currentInternalPagePath = $(currentElement)
          .find(".perc-slider-image-link")
          .val();
        currentLinkSetting = $(currentElement)
          .find("#perc-slider-image-link-setting :selected")
          .val();
        ...
        // If the image path is not valid, apply the error class
          ...
          $(currentElement)
            .find(imageTargetClass)
            .removeClass("perc-input-slider-warning");
          $(currentElement)
            .find(imageTargetClass)
            .addClass("perc-input-slider-error");
          sliderErrorCount++;
        }
        ...
        if (currentLinkSetting == "internal") {
          if (currentInternalPagePath != "#" && currentInternalPagePath != "") {
            ...
            url =
              ...
              "//" +
              window.location.host +
              "/Rhythmyx/rest/pages/by-path" +
              currentInternalPagePath.replace("/Sites/", "/");
          ...
          $(currentElement)
            .find(internalPageTargetClass)
            .removeClass("perc-input-slider-error");
        }
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        if (item.workflow.state != "Live" && item.workflow.state != "Pending") {
          $(currentElement)
            .find(targetClass)
            .addClass("perc-input-slider-warning");
          $(currentElement)
            .find(targetClass)
            .removeClass("perc-input-slider-error");
          sliderWarningCount++;
