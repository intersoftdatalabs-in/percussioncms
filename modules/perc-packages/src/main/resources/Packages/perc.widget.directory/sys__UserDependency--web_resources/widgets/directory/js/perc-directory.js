...
$(document).ready(function () {
  ...
  if ($("#percDirectoryList").data("directory-results-size") > 0) {
    var percDirectorySearchAllOrgs =
      $("#percDirectoryList").data("search-all-orgs") === true;
    var percDirectorySearchAllDepts =
      $("#percDirectoryList").data("show-dpt-filter") === true;
    percDisplayFullDir =
      $("#percDirectoryList").data("display-full-directory") === true;
  ...
  function populateAlphaFilters(alphaFilterLetters) {
    alphaFilterLetters.forEach(function (letter) {
      if (typeof letter !== "undefined" && letter.trim() !== "") {
        $("#perc-directory-alphabet-sort #perc-alpha-sort-letters").append(
          '<a id="perc-alpha-sort-' +
            letter +
            '" class="perc-alpha-sort" aria-label="Filter by letter ' +
            letter.toUpperCase() +
            '" tabindex="0">' +
            letter.toUpperCase() +
            "</a>",
  ...
  $("#perc-org-filter").on("change", function () {
    if (DirectoryList) {
      ...
      if (orgName == "all") {
        ...
        DirectoryList.filter(function (item) {
          return (
            _.includes(item.values()["perc-person-org"], orgName) || !orgName
          );
        });
  ...
  $("#perc-dpt-filter").on("change", function () {
    ...
    if (DirectoryList) {
      ...
      if (dptName === "all") {
        ...
        DirectoryList.filter(function (item) {
          return (
            (_.includes(item.values()["perc-person-org"], orgName) ||
              !orgName) &&
  ...
  $("#perc-directory-alphabet-sort").on(
    "click",
    ".perc-alpha-sort",
    function (event) {
