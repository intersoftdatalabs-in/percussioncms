...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta name="percAssetFinderControl"
		dimension="single" choiceset="none">
		>
		<psxctl:Description>Provides ability to search through parent folders
			heirarchy for asset of a sleected content_type name.</psxctl:Description>
		<psxctl:ParamList>
			<psxctl:Param name="content_type" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter assigns the name of the asset
					type for the control to search for.</psxctl:Description>
			</psxctl:Param>
			<psxctl:Param name="field_to_display" datatype="String"
				paramtype="generic">
				<psxctl:Description>This parameter field from the asset type for the
					control to display after locating it. This will be the Column name
					from the Content Type Table.</psxctl:Description>
			</psxctl:Param>
		...
		<psxctl:AssociatedFileList>
			...
			<psxctl:FileDescriptor
				name="PercFolderService.js" type="script" mimetype="text/javascript">
				<psxctl:FileLocation>/cm/services/PercFolderService.js</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="PercPathService.js" type="script" mimetype="text/javascript">
	...
	<xsl:template
		match="Control[@name='percAssetFinderControl']" mode="psxcontrol">
		<xsl:variable name="controlName"
			select="'percAssetFinderControl'" />
		...
		<input type="hidden" name="{@paramName}"
			id="perc-content-value-{@paramName}" value="{Value}" />
		<div class="perc-asset-finder" id="perc-asset-finder-{@paramName}"
			data-paramName="{@paramName}">
			<div class="perc-asset-finder-editor">
				<div class="perc-asset-display-container">
					<div class="perc-asset-finder-data">
						<input type="text" name="{@paramName}"
							id="perc-content-display-{@paramName}" title="#" value="#"
							readonly=""
							style="background-color: rgb(230, 230, 233); overflow: hidden; text-overflow: ellipsis; height: 15px; padding-top: 3px; padding-bottom: 5px;"
							class="datadisplay" size="50" dlg_width="450" dlg_height="160"
							aarenderer="INPLACE_TEXT" />
					</div>
	...
	<xsl:template
		match="Control[@name='percAssetFinderControl' and @isReadOnly='yes']"
		...
		<input type="hidden" name="{@paramName}"
			id="perc-content-value-{@paramName}" value="{Value}" />
		<div class="perc-asset-finder-readonly"
			id="perc-asset-finder-{@paramName}" data-paramName="{@paramName}">
			<div class="perc-asset-finder-editor">
				<div class="perc-asset-display-container">
					<div class="perc-asset-finder-data">
						<input type="text" name="{@paramName}"
