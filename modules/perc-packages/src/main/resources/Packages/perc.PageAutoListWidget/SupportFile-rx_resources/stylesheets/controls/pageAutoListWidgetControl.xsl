...
<xsl:stylesheet version="1.1"
	...
	<psxctl:ControlMeta name="pageAutoListWidgetControl"
		dimension="single" choiceset="none">
		<psxctl:Description>The control for building the JCR query for the
			Page Auto List Widget:</psxctl:Description>
		<psxctl:ParamList>
		</psxctl:ParamList>
		...
		<!-- CSS perc_webmgt.packed.css contains all css for jquery datepicker
			and Site/Save As control -->
		<psxctl:AssociatedFileList>
			<psxctl:FileDescriptor
				name="PercDatetimePicker.css" type="css" mimetype="text/css">
				<psxctl:FileLocation>/cm/css/PercDatetimePicker.css</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="jquery-ui-1.8.9.custom.css     " type="css"
				mimetype="text/css">
				<psxctl:FileLocation>/cm/themes/smoothness/jquery-ui-1.8.9.custom.css     </psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor name="ui.datepicker.css"
			...
			<psxctl:FileDescriptor
				name="perc_webmgt.packed.css" type="css" mimetype="text/css">
			...
			<psxctl:FileDescriptor name="tree.css"
				type="css" mimetype="text/css">
			...
			<!-- JavaScript -->
			<psxctl:FileDescriptor name="jquery-ui.js"
			...
			<psxctl:FileDescriptor
				name="jquery.percutils.js" type="script" mimetype="text/javascript">
			...
			<psxctl:FileDescriptor
				name="tools.scrollable.mousewheel-1.0.1.js" type="script"
				mimetype="text/javascript">
			...
			<psxctl:FileDescriptor
				name="jquery.jeditable.js" type="script" mimetype="text/javascript">
			...
			<psxctl:FileDescriptor name="PercFinderTree.js"
				type="script" mimetype="text/javascript">
				<psxctl:FileLocation>/cm/widgets/PercFinderTree.js</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="PercExtendUiDialog.js" type="script"
				mimetype="text/javascript">
			...
			<psxctl:FileDescriptor
				name="jquery.pageAutoListControl.js" type="script"
				mimetype="text/javascript">
	...
	<xsl:template
		match="Control[@name='pageAutoListWidgetControl']" mode="psxcontrol">
		...
		<script>
			...
			//
			*********************************************************************
			//This will be set from the widget config $date_format (set by
			velocity)
			//
			*********************************************************************
			$(this).trigger("focus");
			...
			'/rx_resources/widgets/pageAutoList/images/calendar.gif',
			buttonImageOnly: true, altFormat: 'yy-mm-dd', buttonText: ''
			});
		...
		<script>
			...
			$("#autolist-title").toggleClass("autolist-expand-image
			autolist-close-image");
			});
			});
		</script>
		<div id="perc-autolist-wrapper">
			...
			<div id="criteria_for_list">
				<div class="ui-perc-ctl_daterange">
					<label for="display_title_contains">
						Title contains:
						<br />
					</label>
					<input size="50" type="text" class="datadisplay"
						name="display_title_contains" id="display_title_contains" />
					<table style="line-height:17px;">
						<tr>
							<td>
								<label for="display_start_date">
									First published on or after:
									<br />
								</label>
								<input size="16" type="text" name="display_start_date"
									id="display_start_date" class="datadisplay date-pick" />
							</td>
							<td width="18px"></td>
							<td>
								<label for="display_end_date">
									First published before:
									<br />
								</label>
								<input size="16" type="text" name="display_end_date"
									id="display_end_date" class="datadisplay date-pick" />
							</td>
						</tr>
						<tr>
							<td colspan="3">
								<input id="perc-pageautolist-hide-past-results"
									name="hidePastResults" type="checkbox" />
								<label id="perc-pageautolist-hide-past-results-label"
									for="hidePastResults">Hide historical results by</label>
								<input type="radio"
									id="perc-pal-hide-past-results-button1"
									name="perc-pageautolist-hide-past-results-date-type"
									value="rx:sys_contentpostdate" />
								<label for="perc-pal-hide-past-results-button1">Content publish date</label>
								<input type="radio"
									id="perc-pal-hide-past-results-button2"
									name="perc-pageautolist-hide-past-results-date-type"
									value="rx:page_end_date" />
				...
				<div id="perc-pageautolist-detail">
					<table style="line-height:17px;">
						<tr>
							<td>
								<span class="autolistlabel">
									<label for="perc-folder-selector"
										style="font-weight:bold">Website location:</label>
								</span>
							</td>
							<td>
								<span class="autolistlabel">
									<label for="display_pagetemplates_list"
										style="font-weight:bold">Template:</label>
								</span>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td>
								<span class="autolistlabel">
									<label for="perc-category-selector"
										style="font-weight:bold">Categories:</label>
								</span>
							</td>
							<td>
								<span class="autolistlabel">
									<label for="perc-tags-selector"
										style="display:none;font-weight:bold">Tags:</label>
								</span>
							</td>
						</tr>
						<tr>
							<td height="265px" valign="top">
								<div id="perc-category-selector"
									style="width:300px; height:250px; overflow:auto;"
									class="tree-datadisplay" role="tree">
								</div>
							</td>
							<td valign="top">
				...
				<input type="hidden" aarenderer="NONE" class="datadisplay"
					id="perc-content-edit-query" name="query" />
			</div>
		</div>

	</xsl:template>
	<xsl:template
		match="Control[@name='pageAutoListWidgetControl' and @isReadOnly='yes']"
    ...
    <![CDATA[
		...
		<div id="perc-autolist-wrapper">
			...
			<div id="criteria_for_list">
				...
				<br />
				<div class="datadisplay" id="perc_display_title_contains">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='title_contains']/Value" />
				</div>
				<br />
				<label for="display_start_date">Posted on or after:</label>
				<br />
				<div class="datadisplay" id="perc_display_start_date">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='start_date']/Value" />
				</div>
				<br />
				<label for="display_end_date">Posted before:</label>
				<br />
				<div class="datadisplay" id="perc_display_end_date">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='end_date']/Value" />
				</div>
				<br />
				...
				<input type="hidden" id="perc_site_path">
					<xsl:attribute name="value"><xsl:value-of
						select="//DisplayField/Control[@paramName='site_path']/Value" /></xsl:attribute>
				</input>
