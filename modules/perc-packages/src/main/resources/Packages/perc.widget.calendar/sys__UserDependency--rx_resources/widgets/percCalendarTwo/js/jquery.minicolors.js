...
(function (factory) {
    ...
    if (
      ...
      switch (settings.control) {
        case "wheel":
          ...
          slider.css(
            "backgroundColor",
            hsb2hex({ h: hue, s: saturation, b: 100 }),
          );
          break;
        ...
        case "saturation":
          // Calculate hue, saturation, and brightness
          hue = keepWithin(
            parseInt(gridPos.x * (360 / grid.width()), 10),
            0,
            360,
          );
          ...
          brightness = keepWithin(
            100 - Math.floor(gridPos.y * (100 / grid.height())),
            0,
            100,
          );
          hex = hsb2hex({
          ...
          slider.css(
            "backgroundColor",
            hsb2hex({ h: hue, s: 100, b: brightness }),
          );
          minicolors
            .find(".minicolors-grid-inner")
            .css("opacity", saturation / 100);
          break;
        ...
        case "brightness":
          ...
          minicolors
            .find(".minicolors-grid-inner")
            .css("opacity", 1 - brightness / 100);
          break;

        default:
          // Calculate hue, saturation, and brightness
          hue = keepWithin(
            360 - parseInt(sliderPos.y * (360 / slider.height()), 10),
            0,
            360,
          );
          saturation = keepWithin(
            Math.floor(gridPos.x * (100 / grid.width())),
            0,
            100,
          );
          brightness = keepWithin(
      ...
      // Handle opacity
      if (settings.opacity) {
        opacity = parseFloat(1 - opacityPos.y / opacitySlider.height()).toFixed(
          2,
        );
      } else {
        opacity = 1;
  ...
  function updateFromInput(input, preserveInputValue) {
    ...
    if (!hex) {
      hex = convertCase(
        parseInput(settings.defaultValue, true),
        settings.letterCase,
      );
    }
    hsb = hex2hsb(hex);
    ...
    switch (settings.control) {
      ...
      case "saturation":
        ...
        // Set slider position
        y = keepWithin(
          slider.height() - hsb.s * (slider.height() / 100),
          0,
          slider.height(),
        );
        sliderPicker.css("top", y + "px");
      ...
      case "brightness":
        ...
        // Set slider position
        y = keepWithin(
          slider.height() - hsb.b * (slider.height() / 100),
          0,
          slider.height(),
        );
        sliderPicker.css("top", y + "px");
        ...
        minicolors
          .find(".minicolors-grid-inner")
          .css("opacity", 1 - hsb.b / 100);
        break;

      default:
        // Set grid position
        x = keepWithin(
          Math.ceil(hsb.s / (100 / grid.width())),
          0,
          grid.width(),
        );
        y = keepWithin(
        ...
        // Set slider position
        y = keepWithin(
          slider.height() - hsb.h / (360 / slider.height()),
          0,
          slider.height(),
        );
        sliderPicker.css("top", y + "px");
  ...
  function doChange(input, value, opacity) {
    ...
    if (
      ...
      if (settings.swatches && settings.swatches.length !== 0) {
        ...
        for (i = 0; i < settings.swatches.length; ++i) {
          if (
            obj.r === settings.swatches[i].r &&
            obj.g === settings.swatches[i].g &&
            obj.b === settings.swatches[i].b &&
  ...
  function parseHex(string, expand) {
    ...
    if (!string.match(/^[A-F0-9]{3,6}/gi)) return "";
