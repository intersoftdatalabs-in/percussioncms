...
(function ($) {
  $.fn.imageAssetControl = function (settings) {
    var config = {
      ...
      maxDisplayWidth: 600,
      filenameFields: ["sys_title", "filename"],
      debug: false,
    };
    ...
    function imageUploadResponse(data, statusText) {
      ...
      for (var i = 0; i < config.filenameFields.length; i++) {
        form
          .find("input[name=" + config.filenameFields[i] + "]")
          .val(image.filename);
      }
    ...
    function imageRequest(imageKey, imageInfo) {
      $.get(
        "/Rhythmyx/user/apps/imageWidget/requestImage.do",
        {
          imageKey: imageKey,
        },
    ...
    function setDefaultSizes(imageInfo) {
      var defaultWidth = imageInfo.page
        .find(".image_asset_default_width")
        .val();
      var resize = false;
    ...
    function updateImageForDisplay(imageInfo) {
      ...
      if (
        imageInfo.image.width <= config.maxDisplayWidth &&
        imageInfo.image.height <= config.maxDisplayHeight
      ) {
        scale.html("");
        ...
        var result = $.post(
          "/Rhythmyx/user/apps/imageWidget/resizeImage.do",
          request,
          function (data) {
    ...
    function renderImage(selector, j) {
      $(selector).html(
        '<img src="/Rhythmyx/user/apps/imageWidget/image/img' +
          j.imageKey +
          "." +
          j.ext +
          '"' +
    ...
    function resizePage(imageInfo) {
      ...
      if (
        imageInfo.image.height != newHeight ||
        imageInfo.image.width != newWidth
      ) {
    ...
    function rotateImage(imageInfo, direction) {
      ...
      var result = $.post(
        ...
        function (data) {
          ...
          if (imageInfo.rotation === 4) {
            imageInfo.rotation = 0;
          }
          if (imageInfo.rotation === -1) {
            imageInfo.rotation = 3;
          }
          updateImageForDisplay(imageInfo);
          imageInfo.dirty = true;
          setImageIdOnForm(imageInfo);
    ...
    function setImageIdOnForm(imageInfo) {
      ...
      if (
        ...
        imageInfo.image.imageKey !== undefined
      ) {
        imageKey = imageInfo.image.imageKey;
      }
    ...
    var updateFormActionUrl = function (data) {
      ...
      queryUrl +=
        "?sys_folderid=" + newFolderId + "&sys_asset_folderid=" + newFolderId;
    ...
    function _checkUserPermission(data) {
      $.topFrameJQuery
        .PercFolderHelper()
        .getAccessLevelByPath(
          data.folderPath.substring(data.folderPath.indexOf("/Assets")),
          true,
          function (status, result) {
            var error =
                status === $.topFrameJQuery.PercFolderHelper().PERMISSION_ERROR,
              onlyWrite =
                result === $.topFrameJQuery.PercFolderHelper().PERMISSION_READ;
            if (error || onlyWrite) {
      ...
      $.topFrameJQuery.PercUserService.getAccessLevel(
        ...
        function (status, result) {
          var error = status === $.PercServiceUtils.STATUS_ERROR,
            accessRead =
              result === $.topFrameJQuery.PercUserService.ACCESS_READ,
            accessNone =
              result === $.topFrameJQuery.PercUserService.ACCESS_NONE;
    ...
    function _showStep1Image(isHighLight) {
      ...
      else $("#perc-image-upload").css("background-position", "0px 0px");
    }
    ...
    function _showStep2Image(isHighLight) {
      ...
      else $("#perc-image-resize").css("background-position", "0px -152px");
    }
    ...
    function _showStep3Image(isHighLight) {
      ...
      else $("#perc-image-thumbnail").css("background-position", "0px -76px");
    }
    ...
    function _enableClickStep1(enable) {
      if (enable)
        $("#perc-image-upload")
          ...
          .css("cursor", "pointer");
      else $("#perc-image-upload").off("click").css("cursor", "default");
    }
    ...
    function _enableClickStep2(enable) {
      if (enable)
        $("#perc-image-resize")
          ...
          .css("cursor", "pointer");
      else $("#perc-image-resize").off("click").css("cursor", "default");
    }
    ...
    function _enableClickStep3(enable) {
      if (enable)
        $("#perc-image-thumbnail")
          .off("click")
          .on("click", function (evt) {
            _step3ClickHandler(evt);
          })
          .css("cursor", "pointer");
      else $("#perc-image-thumbnail").off("click").css("cursor", "default");
    }
