...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="/" />
	<!-- percTagListControl -->
	<psxctl:ControlMeta name="percTagListControl"
		dimension="array" choiceset="required">
		<psxctl:Description>The tag list control.</psxctl:Description>
		<psxctl:ParamList>
			<psxctl:Param name="width" datatype="Number"
				paramtype="generic">
				<psxctl:Description>Defines the width of the input field</psxctl:Description>
			</psxctl:Param>
			<psxctl:Param name="maxchars" datatype="Number"
				paramtype="generic">
				<psxctl:Description>The field defines the maximum number of
					characters that are allowed</psxctl:Description>
			</psxctl:Param>
		</psxctl:ParamList>
		<psxctl:AssociatedFileList>
			<psxctl:FileDescriptor
				name="percTagListControl.css" type="css" mimetype="text/css">
				<psxctl:FileLocation>/rx_resources/css/percTagListControl.css</psxctl:FileLocation>
				<psxctl:Timestamp />
			</psxctl:FileDescriptor>
			<psxctl:FileDescriptor
				name="jquery.autocomplete.css" type="css" mimetype="text/css">
			...
			<psxctl:FileDescriptor
				name="jquery.autocomplete.js" type="script"
			...
			<psxctl:FileDescriptor
				name="percTagListControl.js" type="script"
				mimetype="text/javascript">
	...
	<xsl:template match="Control[@name='percTagListControl']"
		mode="psxcontrol">
		...
		<select type="text" style="display:none" name="{@paramName}"
			id="perc-content-edit-{@paramName}" multiple="1">
		</select>
	</xsl:template>
