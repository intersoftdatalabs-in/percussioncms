...
<xsl:stylesheet version="1.1"
	...
	<xsl:template match="/" />
	<!-- percQueryControl -->
	<psxctl:ControlMeta name="percQueryControl"
		dimension="single" choiceset="none">
		<psxctl:Description>The control for building the JCR query for the any
			List Widget:</psxctl:Description>
		<psxctl:ParamList>
		</psxctl:ParamList>
		...
		<psxctl:AssociatedFileList>
			...
			<psxctl:FileDescriptor
				name="percQueryControl.css" type="css" mimetype="text/css">
			...
			<psxctl:FileDescriptor
				name="PercContentBrowserWidget.js" type="script"
				mimetype="text/javascript">
			...
			<psxctl:FileDescriptor name="percQueryControl"
				type="script" mimetype="text/javascript">
	...
	<xsl:template match="Control[@name='percQueryControl']"
		mode="psxcontrol">
		...
		<script>
			...
			*********************************************************************
			$(this).trigger("focus");
			var p_query_published_after = new
			Date(value);
			var p_query_published_before = new
			Date($('[name="query_published_before"]').val());
			...
			'/rx_resources/controls/percQueryControl/images/calendar.gif',
			...
			$(this).trigger('focus');
			var
			p_query_published_after = new
			Date($('[name="query_published_after"]').val());
			var
			p_query_published_before = new Date(value);
			<xsl:text disable-output-escaping="yes">
				if(p_query_published_after >= p_query_published_before)
                {
		...
		<script>
			...
			$("#querylist-title").toggleClass("querylist-expand-image
			querylist-close-image");
			});
			});
		</script>
		<div id="perc-querylist-wrapper">
			...
			<div id="criteria_for_list">
				<div class="ui-perc-ctl_daterange">
					<table style="line-height:17px;">
						<tr>
							...
							<td width="30px"></td>
							<td>
								<label for="display_query_published_after">
									First published on or after:
									<br />
								</label>
								<input size="16" type="text"
									name="display_query_published_after"
									id="display_query_published_after" class="date-pick" />
							</td>
							<td width="10px"></td>
							<td>
								<label for="display_query_published_before">
									First published before:
									<br />
								</label>
								<input size="16" type="text"
									name="display_query_published_before"
									id="display_query_published_before" class="date-pick" />
							</td>
						</tr>
					</table>
				</div>
				...
				<input type="hidden" aarenderer="NONE" class="datadisplay"
					id="perc-content-edit-query" name="query_string" />

				<div>
					<span class="querylistlabel">
						<label for='perc_querylist_resultspage'
							style="font-weight:bold">Results page:</label>
					</span>
					<div style="margin-top: 5px;">
						<input type='text' name='display_query_page_result'
							id='display_query_page_result' />
						<span id='perc_querylist_resultspage_browse'>Browse</span>
						<input type='hidden'
							id='perc-formbuild-success-url-paired-unencrypted' />
						<input type='hidden'
							id='perc-formbuild-success-url-paired-encrypted' />
					</div>
				</div>
	...
	<xsl:template
		match="Control[@name='percQueryControl' and @isReadOnly='yes']"
		...
		<div id="perc-querylist-wrapper">
			...
			<div id="criteria_for_list">
				...
				<div class="datadisplay" id="perc_display_title_contains">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='query_title_contains']/Value" />
				</div>
				<br />
				<label for="display_query_published_after">Posted on or after:</label>
				<br />
				<div class="datadisplay" id="perc_display_query_published_after">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='query_published_after']/Value" />
				</div>
				<br />
				<label for="display_query_published_before">Posted before:</label>
				<br />
				<div class="datadisplay"
					id="perc_display_query_published_before">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='query_published_before']/Value" />
				</div>
				<br />
				...
				<input type="hidden" id="perc_site_path">
					<xsl:attribute name="value"><xsl:value-of
						select="//DisplayField/Control[@paramName='query_site_path']/Value" /></xsl:attribute>
				</input>
				...
				<input type="hidden" id="perc_template_list">
					<xsl:attribute name="value"><xsl:value-of
						select="//DisplayField/Control[@paramName='query_template_list']/Value" /></xsl:attribute>
				</input>
				...
				<br />
				<div class="datadisplay" id="perc_display_query_page_result">
					<xsl:value-of
						select="//DisplayField/Control[@paramName='archive_page_result']/Value" />
				</div>
			</div>
		</div>
	</xsl:template>
