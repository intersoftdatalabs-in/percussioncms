 ...
 * committed install Widget XML (ship-exit #2883 / #2884 / #2885) materialize {@code
 * sys__UserDependency--rxconfig/Widgets/*.xml} before reorganize so deployer / {@code PSWidgetDao}
 * still receive the legacy install wire format. Dual-ship packages that still commit Widget XML are
 * left unchanged.
 *
...
public final class PSPackageBuilder {
  ...
  private void buildPackage(Path packageDir, String dirName) throws IOException {
    ...
    try {
      ...
      // Step 1a: Modern-only widget packages → install Widget XML (#2883/#2884/#2885 ship-exit)
   ...
   * definition XML (ship-exit #2883 / #2884 / #2885).
   */
