...
/**
 * PublishingShell Design — save delivery type (#4677 / parent #4531).
 *
 * Surface filter (H2 QA / agent path):
 *   cd modules/perc-qa-automation/frontend
 *   npm run test:surface -- --path tests/publishing/designDeliveryTypeSave.spec.js
 *
...
test.describe("PublishingShell Design delivery-type save", () => {
  test.beforeEach(async ({ page }) => {
    await loginAsAdmin(page);
  });

  test("opens Design delivery-type editor and saves with unique name", async ({
    page,
  }) => {
    const jsErrors = [];
    ...
    const name = `NightDt-${Date.now()}`;
    await page.locator("#dt-name").fill(name);
    await page.locator("#dt-bean").fill("sys_fileDeliveryType");
  ...
  test("surfaces HTTP 409 on duplicate delivery-type save", async ({ page }) => {
    await page.route(
      ...
      (route) => {
        if (route.request().method() === "POST") {
          return route.fulfill({
            ...
            body: JSON.stringify({
              message: "Delivery type name already exists",
            }),
          });
        }
    ...
    await page.getByRole("tab", { name: /Delivery types/i }).click();
    await page.getByTestId("design-add-delivery-type").click();
    await page.locator("#dt-name").fill("DupFromQa");
    await page.locator("#dt-bean").fill("sys_fileDeliveryType");
    await page.getByTestId("delivery-type-save").click();
