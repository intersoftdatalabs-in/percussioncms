...
/**
 * EditorHost open a related item (#4889).
 *
 * <p>Tags: {@code @explorer-content-editor} {@code @editor}</p>
 *
 * <p>Run (QA mode after {@code perc-devctl qa-up}):
 * {@code npm run test:surface -- --path tests/editor-related-open.spec.js}</p>
 */
...
async function stubEditorApis(page) {
  await page.route("**/itemmanagement/workflow/**", async (route) => {
    await route.fulfill({
      status: 200,
  ...
  await page.route("**/assembly/slot-relationships**", async (route) => {
    if (route.request().url().includes("/canvas")) {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
...
test.describe("EditorHost open related item", () => {
  ...
  test(
    "opens the related item in edit from an edit host",
    ...
    async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="editor-related-item-id"]')).toHaveCount(2);
      await expect(page.locator('[data-testid="editor-related-open"]')).toHaveCount(1);
      const popupPromise = page.waitForEvent("popup");
      await page.locator('[data-testid="editor-related-open"]').click();
      const popup = await popupPromise;
      await expect(popup).toHaveURL(/spa\.jsp\?.*entry=editor/);
      await expect(popup).toHaveURL(/contentId=55/);
      await expect(popup).toHaveURL(/mode=edit/);
      await popup.close();
  ...
  test(
    "opens the related item in view from a view host",
    { tag: ["@explorer-content-editor", "@editor"] },
    async ({ page }) => {
      const { pageErrors, consoleErrors } = watchErrors(page);
      await stubEditorApis(page);
      ...
      const popup = await popupPromise;
      await expect(popup).toHaveURL(/contentId=55/);
      await expect(popup).toHaveURL(/mode=view/);
      expect(popup.url()).not.toMatch(/mode=edit/);
      await popup.close();
      await assertConsoleClean(page, pageErrors, consoleErrors);
    },
  );
