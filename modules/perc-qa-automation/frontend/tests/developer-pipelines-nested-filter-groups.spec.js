...
/**
 * Developer Pipelines — nested AND/OR filter groups (#4394 / parent #1690).
 *
 * Admin opens Developer → Pipelines detail, rejects a malformed group,
 * saves nested boolean groups, saves HTTP tank (bundled fixture), then
 * Test invoke returns SKU-1 (matching nested predicate) and not SKU-2.
 *
 * Surface-filtered QA:
 * <pre>
 *   perc-devctl qa-up
 ...
 *     npm run test:surface -- --path tests/developer-pipelines-nested-filter-groups.spec.js
 *   perc-devctl qa-down
 * </pre>
 */
...
async function openPipelineDetail(page, name) {
  ...
  await expect(page.locator('[data-testid="developer-pipe-filter"]')).toBeVisible({
    timeout: 15_000,
  });
}
...
test.describe("Developer Pipelines nested filter groups (#4394 / Slice C leftover)", () => {
  test(
    "Admin saves nested AND/OR groups; Test invoke returns matching fixture rows",
    { tag: ["@developer", "@pipelines", "@filter-groups"] },
    async ({ page }) => {
      ...
      const predLeft = page.locator('[data-testid="developer-pipe-filter-pred-0-left"]');
      await expect(predLeft).toBeVisible();
      await predLeft.fill("");
      await page.locator('[data-testid="developer-pipe-filter-save"]').click();
      await expect(page.locator('[data-testid="developer-pipe-filter-error"]')).toBeVisible({
        timeout: 15_000,
      });
      await expect(page.locator('[data-testid="developer-pipe-filter-error"]')).toContainText(
        /column|invalid|required/i,
      );

      await predLeft.fill("sku");
      await page.locator('[data-testid="developer-pipe-filter-pred-0-right"]').fill("SKU-1");
      await page.locator('[data-testid="developer-pipe-filter-nested-0-left"]').fill("qty");
      await page.locator('[data-testid="developer-pipe-filter-nested-0-right"]').fill("3");
      await page.locator('[data-testid="developer-pipe-filter-nested-1-left"]').fill("qty");
      await page.locator('[data-testid="developer-pipe-filter-nested-1-right"]').fill("99");

      const saveWait = page.waitForResponse(
        (r) =>
          /\/services\/pipelines\/[^/?#]+\/resources\/[^/?#]+\/filterGroup(?:\?|$)/i.test(
            r.url(),
      ...
      expect(saveResp.status(), `filter group save status ${saveResp.status()}`).toBe(200);
      await expect(page.locator('[data-testid="developer-pipe-filter-notice"]')).toBeVisible({
        timeout: 15_000,
      });
