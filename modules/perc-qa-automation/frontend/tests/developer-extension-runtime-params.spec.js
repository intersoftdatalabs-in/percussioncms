...
/**
 * Developer Extensions runtime-parameter editor (#4607 / parent #1690 slice 22).
 *
 * Admin add / update / clear of Extension.runtimeParameters on
 * Developer → Extensions. Closes the last EXTENSION_DESIGN_GAPS entry
 * (Workbench parameter-dialog parity on the wire DTO).
 *
 * Surface-filtered QA:
 * <pre>
 *   perc-devctl qa-up
 ...
 *     npm run test:surface -- --path tests/developer-extension-runtime-params.spec.js
 *   perc-devctl qa-down
 * </pre>
 */
...
test.describe("Developer extension runtime parameters (#4607)", () => {
  test("Admin add, GET round-trip, and clear runtime parameters", async ({ page }) => {
    test.setTimeout(180_000);
    const { pageErrors, consoleErrors } = attachConsoleGuards(page);
    await loginAsAdmin(page);
    await openExtensionsCatalog(page);

    const extName = uniqueExtensionName("qa4607");
    ...
    await page.locator('[data-testid="developer-ex-rtparam-add"]').click();
    await page.locator('[data-testid="developer-ex-rtparam-name-0"]').fill("htmlParams");
    await page.locator('[data-testid="developer-ex-rtparam-type-0"]').fill("java.util.Map");
    await page.locator('[data-testid="developer-ex-rtparam-desc-0"]').fill("html params");
    ...
    if (await saveError.isVisible()) {
      throw new Error(`Create with runtime params failed: ${(await saveError.innerText()).trim()}`);
    }
    ...
    expect(raw, `GET detail missing runtimeParameters: ${raw.slice(0, 800)}`).toMatch(
      /htmlParams|"runtimeParameters"/i,
    );
    await expect(page.locator('[data-testid="developer-ex-detail"]')).toBeVisible();
    await expect(page.locator('[data-testid="developer-ex-detail-loading"]')).toHaveCount(0, {
      timeout: 20_000,
    });
    await expect(page.locator('[data-testid="developer-ex-rtparam-name-0"]')).toHaveValue(
      "htmlParams",
    );

    await page.locator('[data-testid="developer-ex-rtparam-remove-0"]').click();
    await expect(page.locator('[data-testid="developer-ex-params-empty"]')).toBeVisible();
    await page.locator('[data-testid="developer-ex-save"]').click();
    await expect(notice.or(saveError).first()).toBeVisible({ timeout: 20_000 });
    if (await saveError.isVisible()) {
      throw new Error(`Clear runtime params failed: ${(await saveError.innerText()).trim()}`);
    }
    await expect(page.locator('[data-testid="developer-ex-params-empty"]')).toBeVisible();
