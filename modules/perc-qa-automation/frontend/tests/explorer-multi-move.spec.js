...
/**
 * Playwright surface: #4856 / parent #4530 — Content → Move selected to folder.
 *
 * One confirm moves checked pages/assets into one folder, names skipped
 * folders, and does not report a partial HTTP failure as full success.
 *
 * Run (QA mode after perc-devctl qa-up):
 * npm run test:surface -- --path tests/explorer-multi-move.spec.js
 */
...
const TAGS = ["@explorer-multi-move", "@explorer", "@item", "@smoke"];
...
test.describe("Explorer multi-select move to one folder (#4856)", () => {
  test(
    "UI: one confirm moves pages, names skipped folders, partial failure stays visible",
    { tag: TAGS },
    async ({ page, request }) => {
      ...
      const moveItemUrls = [];
      /** @type {string[]} */
      const moveFolderUrls = [];
      page.on("request", (req) => {
        const url = req.url();
        if (req.method() === "OPTIONS") return;
        if (isFoldersMoveItemUrl(url)) moveItemUrls.push(url);
        if (isFoldersMoveFolderUrl(url)) moveFolderUrls.push(url);
      });

      const stamp = Date.now();
      const sourceName = uniqueCopyItemName("qa4856src", stamp);
      const destName = uniqueCopyItemName("qa4856dst", stamp);
      const folderName = uniqueCopyItemName("qa4856fld", stamp);
      const okName = uniqueCopyItemName("qa4856ok", stamp);
      const missName = uniqueCopyItemName("qa4856miss", stamp);
      ...
      try {
        ...
        let itemMoves = 0;
        await page.route(/\/folders\/move\/item/, async (route) => {
          itemMoves += 1;
          const post = route.request().postData() || "";
          if (itemMoves >= 2 || post.includes(missName)) {
            await route.fulfill({
              status: 404,
        ...
        await page.locator('[data-testid="explorer-multi-move"]').click();
        ...
        const result = page.locator('[data-testid="explorer-multi-move-result"]');
        ...
        expect(moveFolderUrls, "folders must not be posted to move/folder").toEqual(
          [],
        );
        expect(moveItemUrls.length).toBeGreaterThanOrEqual(1);
