...
/**
 * Developer Display Formats create / delete chrome (#4086 UI-05) and REST DELETE
 * persist (#4091 / parent #4086 / #1690).
 *
 * SPA tests create via New view chrome. REST persist creates a user format via
 * Admin POST, asserts the catalog lists it, DELETE returns 204, following GET
 * is 404, and the SPA catalog omits the row. Does not delete packaged system
 * formats.
 *
 * Surface-filtered QA:
 * <pre>
...
test.describe("Developer display format editor (#4086 / UI-05)", () => {
  test("Admin can create a display format and see it in the catalog", async ({ page }) => {
    ...
    const formatName = uniqueDisplayFormatName("qa4086");
...
test.describe("Developer display format editor (#4091 / UI-05 DELETE persist)", () => {
  test("Admin DELETE of a POST-created user format is 204, GET 404, catalog omits the row", async ({
    ...
    const formatName = uniqueDisplayFormatName("qa4091");
