 ...
 * (#2956 / #3020 / #3300 / #3687 / #3697 / #3699 / #3707 / #3735 / #3759 / #3778 / #3796 / epic #2678).
 ...
 * csv-filesystem, and sql-database (never repository or http-json this slice). Also intercepts build REST
 ...
 * persists sourceKind=sql-database and live Build/Publish/Preview complete after
 * save (in-memory H2 SELECT fixture; Preview #3768).
 *
...
test.describe("Developer Site Virtual Site source panel (#2956 / #3020)", () => {
  ...
  test("Sites detail shows Virtual Site source panel with repository default", async ({
    ...
    if (await page.locator('[data-testid="developer-site-virtual-form"]').isVisible()) {
      ...
      // Switch to http-json reveals root path only (no Git remotes, no Build/Preview/Publish)
  ...
  test("http-json save PUTs envelope and GET-roundtrip persists without Build chrome (#3796)", async ({
    page,
  }) => {
    const pageErrors = [];
    page.on("pageerror", (err) => pageErrors.push(String(err)));
