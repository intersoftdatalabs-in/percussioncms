 ...
 * uses a portable-safe local rootPath only (no live spec fetch credentials, no virtual.remoteUrl);
 * Build, Preview, and Publish chrome stay hidden for asyncapi-yaml (later slice). Also intercepts build REST
...
test.describe("Developer Site Virtual Site source panel (#2956 / #3020)", () => {
  ...
  test("Sites detail shows Virtual Site source panel with repository default", async ({
    ...
    if (await page.locator('[data-testid="developer-site-virtual-form"]').isVisible()) {
      ...
      // Switch to asyncapi-yaml reveals root path; Build / Preview / Publish stay hidden
      ...
      await expect(
        page.locator('[data-testid="developer-site-virtual-asyncapi-yaml-hint"]'),
      ).not.toContainText("then Build Virtual Site");
      await expect(page.locator('[data-testid="developer-site-virtual-remote-url"]')).toHaveCount(0);
      await expect(page.locator('[data-testid="developer-site-virtual-branch"]')).toHaveCount(0);
  ...
  test("asyncapi-yaml save PUTs envelope and GET-roundtrip persists with Build/Preview/Publish hidden (#4390)", async ({
    page,
  }) => {
    const pageErrors = [];
    page.on("pageerror", (err) => pageErrors.push(String(err)));
