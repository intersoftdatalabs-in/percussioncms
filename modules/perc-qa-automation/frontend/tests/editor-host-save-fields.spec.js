...
/**
 * React Content Editor host — save dirty fields + 409 stale (#4645).
 *
 * <p>Tags: {@code @explorer-content-editor} {@code @editor} {@code @save}</p>
 *
 * <p>Run (QA mode after {@code perc-devctl qa-up}):
 * {@code npm run test:surface -- --path tests/editor-host-save-fields.spec.js}</p>
 */
...
const {
  ...
  editorSpaUrl,
  isItemFieldsPutUrl,
} = require("./helpers/editor-host-save-fields");
  ...
  await page.route("**/services/itemmanagement/item/fields/**", async (route) => {
    if (route.request().method() === "PUT") {
      ...
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify(putBody ?? SAVED),
      });
      return;
    }
    await route.fulfill({
...
test.describe("React Content Editor save field values", () => {
  ...
  test(
    "saves dirty displaytitle and sends revision",
    { tag: ["@explorer-content-editor", "@editor", "@save"] },
    async ({ page }) => {
      const puts = [];
      const pageErrors = [];
      ...
      await page.locator(`[data-testid="${TEST_IDS.fieldDisplayTitle}"]`).fill("Updated");
      ...
      const display = (payload.fields || []).find((f) => f.name === "displaytitle");
      expect(display && display.value).toBe("Updated");
  ...
  test(
    "save HTTP 409 shows stale revision and is not success",
    { tag: ["@explorer-content-editor", "@editor", "@save"] },
    async ({ page }) => {
      const pageErrors = [];
      page.on("pageerror", (err) => pageErrors.push(String(err)));
      await stubEditorApis(page, {
        putStatus: 409,
        putBody: { Error: { message: "This item was saved with a newer revision." } },
      });
