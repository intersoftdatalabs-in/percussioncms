...
/**
 * Developer Pipelines Slice D — request tracing + last-trace view (#4396 / parent #1690).
 *
 * Admin enables tracing, saves a bundled HTTP tank, Test invoke with secret params,
 * then last-trace shows stages/timings and redacts password/token/Authorization.
 *
 * Surface-filtered QA:
 * <pre>
 *   perc-devctl qa-up
 ...
 *     npm run test:surface -- --path tests/developer-pipelines-request-tracing.spec.js
 *   perc-devctl qa-down
 * </pre>
 */
...
const SECRET_TOKEN = "s3cret-token-value";
const SECRET_AUTH = "Bearer s3cret-auth-value";
...
async function openPipelineDetail(page, name) {
  ...
  await expect(page.locator('[data-testid="developer-pipe-tracing"]')).toBeVisible({
    timeout: 15_000,
  });
}
...
test.describe("Developer Pipelines request tracing (#4396 / Slice D)", () => {
  test(
    "Admin enables tracing, Test invoke last-trace has stages and omits secrets",
    { tag: ["@developer", "@pipelines", "@request-tracing"] },
    async ({ page }) => {
      ...
      await ensureInvokeResource(page);

      const enabled = page.locator('[data-testid="developer-pipe-tracing-enabled"]');
      await expect(enabled).toBeVisible();
      await enabled.check();
      const tracingWait = page.waitForResponse(
        (r) =>
          /\/services\/pipelines\/[^/?#]+\/tracing(?:\?|$)/i.test(r.url()) &&
          r.request().method() === "PUT",
        { timeout: 45_000 },
      );
      await page.locator('[data-testid="developer-pipe-tracing-save"]').click();
      const tracingResp = await tracingWait;
      expect(tracingResp.status(), `tracing save status ${tracingResp.status()}`).toBe(200);
      await expect(page.locator('[data-testid="developer-pipe-tracing-notice"]')).toBeVisible({
        timeout: 15_000,
      });
      ...
      await page.locator('[data-testid="developer-pipe-invoke-body"]').fill(invokeBody);
      ...
      const lastTraceWait = page.waitForResponse(
        (r) =>
          /\/services\/pipelines\/[^/?#]+\/lastTrace(?:\?|$)/i.test(r.url()) &&
      ...
      const lastTraceResp = await lastTraceWait;
      expect(lastTraceResp.status(), `lastTrace status ${lastTraceResp.status()}`).toBe(200);

      const stages = page.locator('[data-testid="developer-pipe-last-trace-stages"]');
      await expect(stages).toBeVisible({ timeout: 20_000 });
      const stagesText = (await stages.innerText()).trim();
      expect(stagesText).toMatch(/adapter|preHooks|preWebhook/i);
      expect(stagesText).toMatch(/\d+/);

      const json = page.locator('[data-testid="developer-pipe-last-trace-json"]');
      await expect(json).toBeVisible();
      const jsonText = (await json.innerText()).trim();
      expect(jsonText.length).toBeGreaterThan(0);
      expect(jsonText).toMatch(/REDACTED/i);
      expect(jsonText).not.toContain(SECRET_PASSWORD);
      expect(jsonText).not.toContain(SECRET_TOKEN);
      expect(jsonText).not.toContain(SECRET_AUTH);
      expect(jsonText).not.toMatch(/s3cret/i);
