...
const LLMS_TXT_VIRTUAL_QA_ROOT = "/opt/Percussion/tmp/llms-txt-virtual-qa";
...
function llmsTxtVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "llms-txt-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`llms-txt Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `llms.txt`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link LLMS_TXT_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deployLlmsTxtVirtualFixtureToQaCell() {
  const hostDir = llmsTxtVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const llms = path.join(hostDir, "llms.txt");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, llms, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`llms-txt Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", LLMS_TXT_VIRTUAL_QA_ROOT]);
  dockerExec(container, ["mkdir", "-p", `${LLMS_TXT_VIRTUAL_QA_ROOT}/_theme`]);
  dockerCp(config, `${container}:${LLMS_TXT_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(llms, `${container}:${LLMS_TXT_VIRTUAL_QA_ROOT}/llms.txt`);
  dockerCp(theme, `${container}:${LLMS_TXT_VIRTUAL_QA_ROOT}/_theme/page.html`);
