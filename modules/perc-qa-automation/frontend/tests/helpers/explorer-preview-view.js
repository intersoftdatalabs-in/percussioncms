...
function cmsSitePathPreviewGetUrl(baseUrl, cmsPath) {
  ...
  if (!site) return "";
  const qIndex = site.indexOf("?");
  const pathPart = qIndex >= 0 ? site.slice(0, qIndex) : site;
  const query = qIndex >= 0 ? site.slice(qIndex + 1) : "";
  const encoded = pathPart
    .split("/")
    .map((seg, i) => (i === 0 ? "" : encodeURIComponent(seg)))
    .join("/");
  const base = String(baseUrl || "").replace(/\/+$/, "");
  return `${base}/Rhythmyx${encoded}${query ? `?${query}` : ""}`;
}
