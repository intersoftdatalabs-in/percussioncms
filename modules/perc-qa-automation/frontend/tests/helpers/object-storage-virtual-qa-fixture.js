...
function objectStorageVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "object-storage-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`object-storage Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `8.2/index.md`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link OBJECT_STORAGE_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deployObjectStorageVirtualFixtureToQaCell() {
  const hostDir = objectStorageVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const index = path.join(hostDir, "8.2", "index.md");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, index, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`object-storage Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", OBJECT_STORAGE_VIRTUAL_QA_ROOT]);
  dockerExec(container, [
    "mkdir",
    "-p",
    `${OBJECT_STORAGE_VIRTUAL_QA_ROOT}/8.2`,
    `${OBJECT_STORAGE_VIRTUAL_QA_ROOT}/_theme`,
  ]);
  dockerCp(config, `${container}:${OBJECT_STORAGE_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(index, `${container}:${OBJECT_STORAGE_VIRTUAL_QA_ROOT}/8.2/index.md`);
  dockerCp(theme, `${container}:${OBJECT_STORAGE_VIRTUAL_QA_ROOT}/_theme/page.html`);
