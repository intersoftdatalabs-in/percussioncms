...
const JSON_SCHEMA_VIRTUAL_QA_ROOT = "/opt/Percussion/tmp/json-schema-virtual-qa";
...
function jsonSchemaVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "json-schema-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`json-schema Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `schema.json`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link JSON_SCHEMA_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deployJsonSchemaVirtualFixtureToQaCell() {
  const hostDir = jsonSchemaVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const spec = path.join(hostDir, "schema.json");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, spec, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`json-schema Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", JSON_SCHEMA_VIRTUAL_QA_ROOT]);
  dockerExec(container, ["mkdir", "-p", `${JSON_SCHEMA_VIRTUAL_QA_ROOT}/_theme`]);
  dockerCp(config, `${container}:${JSON_SCHEMA_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(spec, `${container}:${JSON_SCHEMA_VIRTUAL_QA_ROOT}/schema.json`);
  dockerCp(theme, `${container}:${JSON_SCHEMA_VIRTUAL_QA_ROOT}/_theme/page.html`);
