...
const GRAPHQL_SDL_VIRTUAL_QA_ROOT = "/opt/Percussion/tmp/graphql-sdl-virtual-qa";
...
function graphQlSdlVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "graphql-sdl-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`graphql-sdl Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `schema.graphql`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link GRAPHQL_SDL_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deployGraphQlSdlVirtualFixtureToQaCell() {
  const hostDir = graphQlSdlVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const spec = path.join(hostDir, "schema.graphql");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, spec, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`graphql-sdl Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", GRAPHQL_SDL_VIRTUAL_QA_ROOT]);
  dockerExec(container, ["mkdir", "-p", `${GRAPHQL_SDL_VIRTUAL_QA_ROOT}/_theme`]);
  dockerCp(config, `${container}:${GRAPHQL_SDL_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(spec, `${container}:${GRAPHQL_SDL_VIRTUAL_QA_ROOT}/schema.graphql`);
  dockerCp(theme, `${container}:${GRAPHQL_SDL_VIRTUAL_QA_ROOT}/_theme/page.html`);
