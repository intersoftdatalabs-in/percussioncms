...
function httpJsonVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "http-json-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`HTTP JSON Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `pages.json`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link HTTP_JSON_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deployHttpJsonVirtualFixtureToQaCell() {
  const hostDir = httpJsonVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const catalog = path.join(hostDir, "pages.json");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, catalog, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`HTTP JSON Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", HTTP_JSON_VIRTUAL_QA_ROOT]);
  dockerExec(container, ["mkdir", "-p", `${HTTP_JSON_VIRTUAL_QA_ROOT}/_theme`]);
  dockerCp(config, `${container}:${HTTP_JSON_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(catalog, `${container}:${HTTP_JSON_VIRTUAL_QA_ROOT}/pages.json`);
  dockerCp(theme, `${container}:${HTTP_JSON_VIRTUAL_QA_ROOT}/_theme/page.html`);
