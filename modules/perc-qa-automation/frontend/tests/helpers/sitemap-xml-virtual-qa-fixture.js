...
function sitemapXmlVirtualFixtureHostDir() {
  return path.join(__dirname, "..", "fixtures", "sitemap-xml-virtual-site");
}

function dockerFailed(detail, err) {
  const msg = err && err.message ? err.message : String(err);
  return new Error(`sitemap-xml Virtual Site QA fixture Docker failed (${detail}): ${msg}`);
}
...
/**
 * Place `_config.yaml`, `sitemap.xml`, `pages/index.md`, and theme on the QA CMS host.
 * Replaces any previous tree at {@link SITEMAP_XML_VIRTUAL_QA_ROOT}.
 *
 * @returns {string} in-container root path to save as virtual.rootPath
 */
function deploySitemapXmlVirtualFixtureToQaCell() {
  const hostDir = sitemapXmlVirtualFixtureHostDir();
  const config = path.join(hostDir, "_config.yaml");
  const sitemap = path.join(hostDir, "sitemap.xml");
  const page = path.join(hostDir, "pages", "index.md");
  const theme = path.join(hostDir, "_theme", "page.html");
  for (const file of [config, sitemap, page, theme]) {
    if (!fs.existsSync(file)) {
      throw new Error(`sitemap-xml Virtual Site fixture missing: ${file}`);
    }
  }
  const container = qaCmsContainer();
  dockerExec(container, ["rm", "-rf", SITEMAP_XML_VIRTUAL_QA_ROOT]);
  dockerExec(container, ["mkdir", "-p", `${SITEMAP_XML_VIRTUAL_QA_ROOT}/_theme`]);
  dockerExec(container, ["mkdir", "-p", `${SITEMAP_XML_VIRTUAL_QA_ROOT}/pages`]);
  dockerCp(config, `${container}:${SITEMAP_XML_VIRTUAL_QA_ROOT}/_config.yaml`);
  dockerCp(sitemap, `${container}:${SITEMAP_XML_VIRTUAL_QA_ROOT}/sitemap.xml`);
  dockerCp(page, `${container}:${SITEMAP_XML_VIRTUAL_QA_ROOT}/pages/index.md`);
  dockerCp(theme, `${container}:${SITEMAP_XML_VIRTUAL_QA_ROOT}/_theme/page.html`);
