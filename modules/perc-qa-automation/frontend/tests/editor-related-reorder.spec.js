  ...
  await page.route("**/assembly/slot-relationships**", async (route) => {
    ...
    if (url.includes("/canvas")) {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(canvas(swapped)),
      });
      return;
    }
...
async function expectOrder(page, first, second) {
  const ids = page.locator('[data-testid="editor-related-item-id"]');
  await expect(ids).toHaveCount(2);
  await expect(ids.nth(0)).toHaveText(first);
  await expect(ids.nth(1)).toHaveText(second);
}
...
test.describe("EditorHost related content reorder", () => {
  ...
  test(
    "reorders a slot association and reloads the new order",
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await page.locator('[data-testid="editor-related-move-down"]').click();
      await expect(page.locator('[data-testid="editor-related-reorder-error"]')).toContainText(
