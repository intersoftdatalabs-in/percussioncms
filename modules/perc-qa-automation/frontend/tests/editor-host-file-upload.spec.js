...
/**
 * React Content Editor host — file field upload (#4678).
 *
 * <p>Tags: {@code @explorer-content-editor} {@code @editor} {@code @file}</p>
 *
 * <p>Run (QA mode after {@code perc-devctl qa-up}):
 * {@code npm run test:surface -- --path tests/editor-host-file-upload.spec.js}</p>
 */
...
test.describe("React Content Editor file field upload", () => {
  ...
  test(
    "uploads a file field on save for a checked-out item",
    ...
    async ({ page }) => {
      ...
      await page
        .locator(`[data-testid="${TEST_IDS.fileInput}"]`)
        .setInputFiles({
          name: "spec.pdf",
  ...
  test(
    "view mode keeps the file input read-only",
    { tag: ["@explorer-content-editor", "@editor", "@file"] },
    async ({ page }) => {
      await stubEditorApis(page);
  ...
  test(
    "maps HTTP 403 and 400 file uploads as errors, not success",
    { tag: ["@explorer-content-editor", "@editor", "@file"] },
    async ({ page }) => {
