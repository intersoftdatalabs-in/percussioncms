...
/**
 * Playwright surface: #4884 / parent #4530 — Content → Restore selected.
 *
 * One confirm restores checked Recycling pages/assets/folders. Cancel sends
 * no PUT. A 404/409 on one item is named and is not full success.
 *
 * Run (QA mode after perc-devctl qa-up):
 * npm run test:surface -- --path tests/explorer-multi-restore.spec.js
 */
...
const TAGS = ["@explorer-multi-restore", "@explorer", "@item", "@smoke"];
...
test.describe("Explorer multi-select restore (#4884)", () => {
  test(
    "UI: cancel restores nothing; one confirm restores recycling rows and names a partial failure",
    { tag: TAGS },
    async ({ page, request }) => {
      ...
      page.on("request", (req) => {
        ...
        if (/\/rest\/folders\/recycle\/restore\//.test(req.url())) {
          restoreUrls.push(req.url());
        }
      });

      const stamp = Date.now();
      const sourceName = uniqueCopyItemName("qa4884src", stamp);
      const folderName = uniqueCopyItemName("qa4884fld", stamp);
      const okName = uniqueCopyItemName("qa4884ok", stamp);
      const missName = uniqueCopyItemName("qa4884miss", stamp);
      ...
      try {
        ...
        let restoreCount = 0;
        await page.route(/\/rest\/folders\/recycle\/restore\//, async (route) => {
          if (route.request().method() !== "PUT") {
            await route.continue();
            return;
          }
          restoreCount += 1;
          const fail = restoreCount >= 2;
          await route.fulfill({
            status: fail ? 409 : 200,
        ...
        await page.locator('[data-testid="explorer-multi-restore"]').click();
        await page.locator('[data-testid="explorer-restore-cancel"]').click();
        expect(restoreUrls).toEqual([]);

        await page.locator('[data-testid="explorer-menu-content"]').click();
        await page.locator('[data-testid="explorer-multi-restore"]').click();
        await page.locator('[data-testid="explorer-restore-ok"]').click();
        ...
        await expect(result).not.toContainText("Restored 3");
        expect(restoreUrls.length).toBeGreaterThanOrEqual(2);
