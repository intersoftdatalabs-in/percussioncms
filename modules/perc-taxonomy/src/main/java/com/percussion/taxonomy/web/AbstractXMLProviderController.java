...
public class AbstractXMLProviderController extends AbstractControllerWithSecurityChecks {
  ...
  protected ModelAndView getXML(
      ...
      int request_type,
      int skip_id,
      Collection<Integer> already_picked_node_ids)
      throws Exception {
    ...
    if (request_type == NO_CHILDREN) {
      ...
      for (Integer id_we_must_have : already_picked_node_ids) {
        ...
        ids_we_must_have.addAll(
            children_and_sub_ids(id_we_must_have.intValue(), true)); // firstlevelonly
      }
    ...
    for (Object[] tripletPlusTwo : the_nodes) {
      ...
      String title =
          buildTitle(
              titles_ConcurrentHashMap.get(nodeID),
              attributes,
              langID,
              tripletPlusTwo,
              tp.getForJEXL(),
      ...
      Item item =
          new Item(
              html_id_prefix + nodeID,
              (tripletPlusTwo[1] == null) ? null : (html_id_prefix + tripletPlusTwo[1].toString()),
              title,
              node_name,
              xml_link,
              xml_onclick);
      if ((request_type == ONLY_CHILDREN
              || request_type == TOP_LEVEL_ONLY
              || request_type == MINIMAL)
          && nodes_with_children.contains(nodeID)) {
        item.state = "closed";
      }
  ...
  protected String buildTitle(
      ConcurrentHashMap<String, Collection<String>> title_pairs,
      ArrayList<Attr> attributes,
      int langID,
      Object[] tripletPlusTwo,
      boolean is_for_jexl,
      boolean exclude_disabled) {
