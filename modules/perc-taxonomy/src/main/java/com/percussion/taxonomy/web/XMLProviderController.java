...
public class XMLProviderController extends AbstractXMLProviderController {
  ...
  public ModelAndView getXMLJSTreeNav(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    String prefix = request.getParameter("prefix");
    String link =
        "taxoneditor.htm?action=changeNode&nodeID=[NODE_ID]&taxID="
            + tp.getTaxID()
            + "&langID="
            + tp.getLangID();
    ...
    if (request.getParameter("only_expand_children") != null
        && request.getParameter("only_expand_children").equals("true")
        && tp.hasNodeID()) {
      request_type = AbstractXMLProviderController.ONLY_CHILDREN;
    } else {
      request_type =
          tp.hasNodeID()
              ? AbstractXMLProviderController.MINIMAL
              : AbstractXMLProviderController.TOP_LEVEL_ONLY;
    }
    ...
    return getXML(
        request, response, prefix, link, false, request_type, -1, already_picked_node_ids);
  }

  public ModelAndView getXMLJSTreeMove(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    String prefix = request.getParameter("prefix");
    ...
    String link =
        "taxoneditor.htm?action=setNewParent&fromID="
            + fromID
            + "&parentID=[NODE_ID]&taxID="
            + tp.getTaxID()
            + "&langID="
            + tp.getLangID();
  ...
  public ModelAndView getXMLJSTree(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    String prefix = request.getParameter("prefix");
    return getXML(
        request, response, prefix, null, false, AbstractXMLProviderController.NORMAL, -1, null);
  }

  public ModelAndView getXMLJSTreeLazy(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    String prefix = request.getParameter("prefix");
    ...
    if (prefix.contains("related")) {
      ...
      if (request.getParameter("already_picked_node_ids").length() > 0) {
        for (String id_as_string :
            StringUtils.split(
                StringUtils.trimToEmpty(request.getParameter("already_picked_node_ids")), ",")) {
    ...
    return getXML(
        request, response, prefix, null, false, request_type, -1, already_picked_node_ids);
  }

  public ModelAndView getJSTreeSearch(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    ...
    boolean exclude_disabled =
        (request.getParameter("exclude_disabled") != null
            && request.getParameter("exclude_disabled").equals("true"));

    String prefix = request.getParameter("prefix");

    ArrayList<String> a = new ArrayList<>();

    // TODO in the future admins might be able to browse / search in a different language but for
    // now we assume editors are editing in English (even if they are entering data for different
    // language)
    // int langID = tp.getLangID();
    int langID = Language.DEFAUL_LANG;

    for (Node node :
        nodeService.getNodesFromSearch(
            tp.getTaxID(), langID, request.getParameter("q"), exclude_disabled)) {
  ...
  public ModelAndView setInUse(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    HashMap<String, Object> myModel = new HashMap<>();
    ...
    for (String id_string :
        StringUtils.split(StringUtils.trimToEmpty(request.getParameter("ids")), ',')) {
  ...
  public ModelAndView getIdsToString(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    HashMap<String, Object> myModel = new HashMap<>();
    ...
    if (ids.size() == 0) return new ModelAndView("getidstostring", "model", myModel);
  ...
  public ModelAndView getAutocompleteSearch(
    ...
    for (Node node :
        nodeService.getNodesFromSearch(tp.getTaxID(), langID, request.getParameter("term"), true)) {
      ...
      if (z <= 5 && !ArrayUtils.contains(exclude_id_strings, String.valueOf(node.getId()))) {
          ...
          for (Object[] obj : unfilted_nodes) {
            ...
            if (obj_id == node.getParent().getId()) {
              ...
              if (StringUtils.contains(parent_abbr, "(")
                  && StringUtils.contains(parent_abbr, ")")) {
        ...
        String title =
            buildTitle(
                titles_ConcurrentHashMap.get(node.getId()),
                null,
                langID,
                tripletPlusTwo,
                false,
        ...
        s +=
            "\"label\" : \""
                + StringUtils.replace(parent_abbr + tripletPlusTwo[2], "\"", "\\\"")
                + "\", ";
        s +=
            "\"value\" : \""
                + StringEscapeUtils.escapeHtml4(parent_abbr + tripletPlusTwo[2])
                + "\", ";
        s +=
            "\"title\" : \""
                + StringUtils.replace(StringEscapeUtils.escapeHtml4(title), "|", "&#13;&#10;")
                + "\"";
        s += "}";
