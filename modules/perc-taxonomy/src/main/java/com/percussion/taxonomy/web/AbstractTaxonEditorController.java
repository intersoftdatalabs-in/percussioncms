...
public abstract class AbstractTaxonEditorController extends AbstractControllerWithSecurityChecks {
  ...
  protected HashMap<Integer, Integer> getSimilarOrRelatedNodeIds(
      int nodeId, int relationship_type_id) {
  ...
  // Save Vals ---------------------------------------------------------
  public Map<String, String> saveNodeValues(
      HttpServletRequest request, Collection<Attribute> attributes, Node node, int langID)
      throws Exception {
    return valueService.saveValuesFromParams(
        request.getParameterMap(), attributes, node, langID, getUserName(request));
  }
  ...
  protected void set_new_permissions_for_role(
      String[] roleNames, Node this_node2, boolean do_children) {

    Node this_node =
        nodeService.getNode(
            this_node2.getId(), 1); // language doesn't matter so we pass 1 for english
