...
public class TaxonomyController extends AbstractControllerWithSecurityChecks {
  ...
  public ModelAndView new_taxonomy(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
  ...
  public ModelAndView create(HttpServletRequest request, HttpServletResponse response)
    ...
    boolean is_relate_taxon =
        StringUtils.isEmpty(relate_taxon) || relate_taxon.equalsIgnoreCase("yes");
    ...
    if (taxonomyService.doesTaxonomyExists(taxonomy_name)) {
      ...
      myModel.put(
          "errorMessage",
          "Cannot create an taxonomy with name '"
              + taxonomy_name
    ...
    return new ModelAndView(
        "redirect:taxonomy.htm?action=edit&taxID="
            + tax.getId()
            + "&langID="
            + tp.getLangID()
            + "&dup=0");
  ...
  public boolean isUniqueAttributeName(Taxonomy tax, String attr_name) {
    for (Attribute attr : tax.getAttributes()) {
      ...
      if (attr_name.equalsIgnoreCase(name)) return false;
    }

    return true;
  }

  public ModelAndView new_attribute(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonomySecurityHelper.raise_error_if_cannot_admin();
    String attr_name = request.getParameter("attr_name");
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    ...
    if (!isUniqueAttributeName(tax, attr_name)) {
      ...
      messages.put(
          "errorAttrName",
          "Cannot create an attribute with name '"
              + attr_name
    ...
    return new ModelAndView(
        "redirect:taxonomy.htm?action=edit&taxID=" + tax.getId() + "&langID=" + language.getId());
  }

  public ModelAndView set_visibility(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    ...
    Collection<Visibility> existing_visiblities =
        visibilityService.getAllVisibilitiesForTaxonomyId(tp.getTaxID());
  ...
  public ModelAndView generic_edit(
      HttpServletRequest request, HttpServletResponse response, Map<String, Object> messages)
      throws Exception {
    TaxonomySecurityHelper.raise_error_if_cannot_admin();
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    Map<String, Object> myModel = new HashMap<String, Object>();
    ...
    if (messages != null) myModel.put("messages", messages);
  ...
  public ModelAndView delete_attribute(HttpServletRequest request, HttpServletResponse response)
    ...
    return new ModelAndView(
        "redirect:taxonomy.htm?action=edit&taxID=" + tp.getTaxID() + "&langID=" + tp.getLangID());
  }

  public ModelAndView delete_taxonomy(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    ...
    if (StringUtils.isNotBlank(request.getParameter("taxID"))) {
      contentTypeToFields.putAll(
          defMgr.getFieldsWithControlProp("taxonomy_name", taxonomy.getName()));
    }
    contentTypeToFields.putAll(
        defMgr.getFieldsWithControlProp("taxonomy_id", String.valueOf(tp.getTaxID())));
