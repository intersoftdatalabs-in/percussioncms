...
public class TaxonEditorController extends AbstractTaxonEditorController {
  ...
  public ModelAndView index(HttpServletRequest request, HttpServletResponse response)
    ...
    TaxonParams tp =
        new TaxonParams(
            request, 1, Language.DEFAUL_LANG, taxonomyService); // TODO hard coded values
  ...
  // SECURITY: added
  public ModelAndView archiveNode(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
  ...
  public ModelAndView changeNode(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    return changeNodeGeneric(request, response, null);
  }

  // SECURITY: not needed
  private ModelAndView changeNodeWithError(
  ...
  // SECURITY: not needed
  private ModelAndView changeNodeGeneric(
      HttpServletRequest request, HttpServletResponse response, Map<String, String> errors)
      throws Exception {
    ...
    if (node.getTaxonomy().getId() != tp.getTaxID()) {
      // System.out.println("Taxon doesn't match:" + node.getTaxonomy().getId() + " vs. " +
      // tp.getTaxID());
      return index(request, response);
    }
    ...
    if (errors != null) {
      ...
      for (String e : errors.keySet()) {
        which_accordian_to_open =
            (which_accordian_to_open == 1 || e.startsWith("child")) ? 1 : which_accordian_to_open;
      }
    }
    ...
    if (tp.getParentID() != null) {
      myModel.put(
          "parentNode",
          getParentNodeName(tp.getNodeID(), tp.getLangID())); // TODO what the heck is this?
    }
    ...
    myModel.put(
        ...
        StringUtils.join(
            getSimilarOrRelatedNodeIds(tp.getNodeID(), Relationship_type.RELATED)
                .keySet()
                .toArray(),
            ","));
    myModel.put(
        "selectedSimilarNodeIds",
        StringUtils.join(
            getSimilarOrRelatedNodeIds(tp.getNodeID(), Relationship_type.SIMILAR)
                .keySet()
                .toArray(),
            ","));
    myModel.put("js_timestamp", String.valueOf(System.currentTimeMillis()));
    ...
    Collection<Related_node> referenced_by_relationships =
        nodeService.getRelatedNodeReferences(tp.getNodeID());
  ...
  public ModelAndView saveChild(HttpServletRequest request, HttpServletResponse response)
    ...
    Collection<Attribute> attributes =
        this.attributeService.getAllAttributes(tp.getTaxID(), tp.getLangID());
    ...
    if (errors == null) {
      return new ModelAndView(
          "redirect:taxoneditor.htm?action=changeNode&nodeID="
              + node.getId()
              + "&taxID="
              + tp.getTaxID()
              + "&langID="
  ...
  public ModelAndView saveSimilar(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    return saveSimilarOrRelated(request, response, Relationship_type.SIMILAR);
  }

  public ModelAndView saveRelated(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    return saveSimilarOrRelated(request, response, Relationship_type.RELATED);
  }

  // SECURITY: added
  private ModelAndView saveSimilarOrRelated(
      HttpServletRequest request, HttpServletResponse response, int relationship_type_id)
      throws Exception {
    ...
    if (relationship_type_id == Relationship_type.RELATED) {
      string_node_ids =
          string_node_ids = StringUtils.split(request.getParameter("relatedtree_ids"), ",");
    } else {
      string_node_ids =
          string_node_ids = StringUtils.split(request.getParameter("similartree_ids"), ",");
    }
  ...
  // SECURITY: added
  public ModelAndView setRoles(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    ...
    boolean do_children =
        (TaxonomySecurityHelper.amITaxonomyAdmin()
            && request.getParameter("apply_to_children") != null);
  ...
  // SECURITY: added
  public ModelAndView setNewParent(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    TaxonParams tp = new TaxonParams(request, taxonomyService);
    ...
    if (tp.getParentID() == 0) {
      // Check if the original parent of this node still has children? If no, then make that node as
      // a leaf node.
      if (node.getParent() != null) {
      ...
      // Find the original parent, check if this parent has any more children. If not, then make
      // this parent as a leaf node.
      if (node.getParent() != null) {
        orig_node_parent = nodeService.getNode(node.getParent().getId(), Language.DEFAUL_LANG);
