...
public class TaxonomyJexl extends PSJexlUtilBase implements IPSJexlExpression {
  ...
  @IPSJexlMethod(
      description =
          "returns a list(TaxAttMap) of taxon attributes given a Taxonomy Name or Taxonomy Id",
      params = {
        @IPSJexlParam(name = "taxonomy_name", description = "the taxonomy id or taxonomy name")
      })
  public TaxAttMap getTaxonomyAtrributes(Object taxonomy_name)
    ...
    try {
      ...
      throw new PSExtensionProcessingException(
          new PSExtensionProcessingException(e.getMessage(), e));
    } finally {
      try {
        session.close();
  ...
  public TaxNodeList getRelatedNodes(Object node_id, String relationship_type_name)
      throws PSErrorException, PSExtensionProcessingException {
  ...
  @IPSJexlMethod(
      description =
          "returns a list of taxonomy nodes (TaxNodeList) for a given taxonomy name or id",
      params = {@IPSJexlParam(name = "taxonomy_name", description = "the taxonomy name or id")})
  public TaxNodeList getTaxonomyNodes(Object taxonomy_name)
      throws PSErrorException, PSExtensionProcessingException {
    TaxNodeList ret = new TaxNodeList();
    List<Node> nodes = new ArrayList<Node>();
    String taxName = "";
