...
public class TaxonomyFieldVisibility implements IPSFieldVisibilityRule {
  ...
  private String SQL_STRING =
      "select v.community_id from Visibility v where v.taxonomy.id = :taxonomy_id";

  private static final Logger log = LogManager.getLogger(TaxonomyFieldVisibility.class);

  public Object processUdf(Object[] params, IPSRequestContext ipsRequestContext)
    ...
    if (visible_to_community_ids.size() > 0) {
      // there are community visibility restrictions in place so we will only return true if db
