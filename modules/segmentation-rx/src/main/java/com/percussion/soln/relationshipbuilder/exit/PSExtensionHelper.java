...
public class PSExtensionHelper
{
   ...
   private final transient IPSRelationshipBuilder relationshipBuilder;

   private final transient Map<String, String> parameters;

   private final transient IPSRequestContext requestContext;
   ...
   public void updateDisplayChoices(Document resultDoc, boolean selectAll) 
       ...
       log.debug("Starting Updating display choices with builder: " + 
               relationshipBuilder.getClass().getCanonicalName());
       ...
       if ( ! selectAll ) {
           ...
           try {
                ids = relationshipBuilder.retrieve(contentId);
   ...
   {
      IPSRequestContext request = requestContext;
       log.debug("Started retrieving target ids using exit: " + relationshipBuilder.getClass().getName());
      ...
      if (StringUtils.isNumeric(contentId))
      {
         ...
         try
         {
            final Collection<Integer> ids = relationshipBuilder.retrieve(cid);
      ...
      log.debug("Finished extracting ids from slot exit: " + relationshipBuilder.getClass().getName());
      return idsString;
   }
         ...
         throws PSParameterMismatchException, PSExtensionProcessingException
   {
      ...
      if (isRequestToBeProcessedForBuilding(requestContext))
      {
         log.debug("Starting building relationships exit with builder " + 
                 relationshipBuilder.getClass().getName());
         Map<String, String> paramMap = parameters;
         PSOExtensionParamsHelper extParamHelper = new PSOExtensionParamsHelper(paramMap, requestContext, log);
         ...
         if (StringUtils.isNumeric(contentId))
         {
            ...
            try
            {
                Object[] fieldValues = requestContext.getParameterList(fieldName);
                buildFromFieldValues(relationshipBuilder, cid, fieldName, fieldValues );
            }
            ...
            catch (PSException e)
            {
               ...
               log.debug("Finished processing exit with builder " + relationshipBuilder.getClass().getName());
            }
         }
      }
   }
    ...
    */
   private static final Log log = LogFactory.getLog(PSExtensionHelper.class);
