...
public class PSNavAutoSlotExtension extends PSDefaultExtension
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext req, Document resultDoc)
    ...
    try {
      ...
      if (recurse instanceof Boolean && Boolean.TRUE.equals(recurse)) {
        log.error(
            "Recursion violation in PSNavAutoSlot. Content Id= {} ",
            req.getParameter(IPSHtmlParameters.SYS_CONTENTID));
      ...
      if (navon != null) {
        Document navonDoc =
            PSNavTreeLinkExtension.getTreeVariantXMLRaw(req, navon.getCurrentLocator());
        if (navonDoc != null) {
   ...
   */
  private PSComponentSummary findNavonRx(IPSRequestContext req) throws PSNavException {
    ...
    if (rxfolder != null && rxfolder.length() > 0) {
      ...
      PSComponentSummary folderSummary = PSNavUtil.getItemSummary(req, folderLoc);
      ...
      PSNavFolder ourNav = PSNavFolderUtils.getNavParentFolder(req, folderSummary);
      if (ourNav == null) {
  ...
  private PSComponentSummary findNavon(IPSRequestContext req) throws PSNavException {
    ...
    PSLocator itemLoc = new PSLocator(req.getParameter(IPSHtmlParameters.SYS_CONTENTID));
    PSNavFolderSet allFolders = PSNavFolderUtils.buildNavFolders(req, itemLoc);
    if (allFolders.isEmpty()) { // no parent nav folders found
          ...
          if (siteRoot != null) {
            ...
            while (it.hasNext()) {
              ...
              if (relProxy.isDescendent(
                  cType, siteLoc, lFolder, PSRelationshipFilter.FILTER_NAME_FOLDER_CONTENT)) {
  ...
  private void processSlots(
      IPSRequestContext req, PSComponentSummary navon, Document resultDoc, Document navonDoc)
      throws PSNavException {
    ...
    int contentTypeId = -1;
    PSContentTypeTemplate variant = PSNavUtil.loadVariantInfo(req, contentTypeId, variantId);
    ...
    while (slots.hasNext()) {
      ...
      else { // this is not an error, we just skip over all other slots.
  ...
  private static Element findRelatedContentElement(Document doc) {
    ...
    Element relatedContent = walker.getNextElement(RELATED_CONTENT_ELEM, true);
