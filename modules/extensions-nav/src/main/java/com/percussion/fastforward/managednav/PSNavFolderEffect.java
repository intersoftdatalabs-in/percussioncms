...
public class PSNavFolderEffect extends PSNavAbstractEffect implements IPSEffect {
  ...
  public void test(
    ...
    if (excontext == null) throw new IllegalArgumentException("excontext cannot be null");
    ...
    try {
      ...
      try {
        ...
        m_log.warn(
            "Unable to load relationship info rid is {}. Error: {}",
            currRel.getId(),
      ...
      if (excontext.isPreConstruction()) handleTestNew(req, currentInfo, result);
  ...
  private void handleTestNew(
      IPSRequestContext req, PSNavRelationshipInfo currentInfo, PSEffectResult result)
    ...
    if (dependent.isItem() && PSNavUtil.isNavType(req, dependent)) {
      ...
      PSComponentSummary currentNavon = PSNavFolderUtils.getChildNavonSummary(req, owner);
      if (currentNavon != null) {
        ...
        String[] args = {folderName, currentItemName};
        ...
        return;
      } else m_log.debug("no child navon found in folder {}", owner.getName());
    } else {
   ...
   * @throws PSNavException when an error occurs.
   */
  private void handleAttemptNewFolder(
      IPSRequestContext req, PSNavRelationshipInfo currentInfo, PSEffectResult result)
      throws PSNavException {
    ...
    if (navon == null) result.setSuccess();
    else flushall();
  }
   ...
   * @throws PSNavException when an error occurs.
   */
  private void handleAttemptNewNavon(
      IPSRequestContext req, PSNavRelationshipInfo currentInfo, PSEffectResult result)
      throws PSNavException {
    ...
    PSComponentSummary myParent = PSNavFolderUtils.getParentFolder(req, folder);
    if (myParent != null) {
      PSNavFolder parentNavFolder = PSNavFolderUtils.getNavParentFolder(req, myParent, false);

      if (parentNavFolder != null) { // There is a Navon in the folder above this one
        m_log.debug("found parent folder with Navon");
        PSNavFolderUtils.addNavonSubmenu(
            req, parentNavFolder.getNavonSummary().getCurrentLocator(), navon.getCurrentLocator());
      }
    }
   ...
   * @throws PSNavException when an error occurs.
   */
  private void handleAttemptNewNavTree(
      IPSRequestContext req, PSNavRelationshipInfo currentInfo, PSEffectResult result)
      throws PSNavException {
    PSComponentSummary folder = currentInfo.getOwner();
    PSComponentSummary navon = currentInfo.getDependent();
  ...
  private void handleAttemptDestroy(IPSRequestContext req, PSNavRelationshipInfo currentInfo)
    ...
    if (dependent.getType() == PSComponentSummary.TYPE_FOLDER) {
      ...
      PSComponentSummary parentNavon = PSNavFolderUtils.getChildNavonSummary(req, parentFolder);
      if (parentNavon == null) {
        m_log.debug("parent folder has no Navon");
        return;
      } else m_log.debug("parent navon is {}", parentNavon.getName());

      PSComponentSummary navon = PSNavFolderUtils.getChildNavonSummary(req, dependent);
      if (navon == null) {
        m_log.debug("child Navon not found");
        return;
      } else m_log.debug("child Navon is {}", navon.getName());

      PSNavFolderUtils.removeNavonChild(
          req, parentNavon.getCurrentLocator(), navon.getCurrentLocator());
  ...
  private boolean isPropagate(PSLocator navonLoc) throws PSNavException {
    ...
    try {
      ...
      List<Node> nodes =
          mgr.findItemsByGUID(PSGuidUtils.toGuidList(new PSLegacyGuid(navonLoc)), conf);
      if (nodes.isEmpty()) {
        throw new RuntimeException("Failed to check propagation property of navon: " + navonLoc);
      }
