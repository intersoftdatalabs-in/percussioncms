...
public class PSNavTreeLinkExtension extends PSDefaultExtension
  ...
  public static Document getTreeVariantXMLClean(IPSRequestContext req, PSLocator loc)
    ...
    Document resDoc = getTreeVariantXMLRoot(req, stack.peek().getCurrentLocator());
  ...
  public static Document getTreeVariantXMLRoot(IPSRequestContext req, PSLocator loc)
    ...
    if (loc != null) {
      ...
      intParams.put(IPSHtmlParameters.SYS_CONTENTID, loc.getPart(PSLocator.KEY_ID));
      log.debug("Content Id {}", loc.getPart(PSLocator.KEY_ID));
      intParams.put(IPSHtmlParameters.SYS_REVISION, loc.getPart(PSLocator.KEY_REVISION));
    ...
    if (resDoc == null) {
      IPSInternalRequest intReq = req.getInternalRequest(treeURL, intParams, false);
      if (intReq == null) {
