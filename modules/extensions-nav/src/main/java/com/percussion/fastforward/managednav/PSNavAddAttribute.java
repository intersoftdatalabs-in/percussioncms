...
public class PSNavAddAttribute extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  private class Worker {
     ...
     * @throws SQLException
     */
    public void processDoc(Document doc) throws PSInternalRequestCallException, SQLException {
     ...
     * @throws SQLException
     */
    public void walkElement(Element navon) throws PSInternalRequestCallException, SQLException {
      ...
      if (value != null) {
        ...
        if (!(wholeTree
            || (relation.equals(PSNavonType.TYPENAME_ANCESTOR)
                || relation.equals(PSNavonType.TYPENAME_ROOT)
                || relation.equals(PSNavonType.TYPENAME_SELF)))) {
          logger.debug("Found attribute on non-Ancestor node, end of this branch.");
          return;
        }
      }
      ...
      while (next != null) {
        ...
        next =
            walker.getNextElement(
                PSNavon.XML_ELEMENT_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
      }
    }
    ...
    private void setLevelAttribute(Element navon, int level) {
      ...
      Element childNavon =
          childWalker.getNextElement(
              PSNavon.XML_ELEMENT_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      level += 1;
    ...
    private Object doQuery(PSLocator locator) throws PSInternalRequestCallException, SQLException {
      ...
      IPSInternalRequest irq = request.getInternalRequest(queryName, xtraParams, false);
      if (irq != null) {
        try {
