...
public class PSServiceWrapper {
  ...
  public static void main(String[] args) {
    ...
    StartWrapper stagingDtsWrapper =
        new DtsStartWrapper("Staging DTS", stagingDtsRoot, filteredArgs);
    ...
    if (dtsWrapper.isActive() && !isStagingOnly) {
      ...
      if (percArgs.isStopDTS()) dtsWrapper.stopServer(percArgs.isForce());
      if (percArgs.isStartDTS()) dtsWrapper.startServer();
    }

    if (stagingDtsWrapper.isActive()) {
      debug("Staging DTS Server Found");
      if (percArgs.isStopStagingDTS()) stagingDtsWrapper.stopServer(percArgs.isForce());
      if (percArgs.isStartStagingDTS()) stagingDtsWrapper.startServer();
    }
  }
}
