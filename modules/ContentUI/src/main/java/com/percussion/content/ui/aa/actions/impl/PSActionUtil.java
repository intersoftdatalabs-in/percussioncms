...
public abstract class PSActionUtil {
  ...
  protected static Map<String, String[]> getAssemblyParams(
    ...
    if (temp != null) params.put(IPSHtmlParameters.SYS_FOLDERID, new String[] {temp.toString()});

    temp = objectId.getAuthType();
    if (temp == null) temp = "0";
    params.put(IPSHtmlParameters.SYS_AUTHTYPE, new String[] {temp.toString()});

    temp = objectId.getContext();
    if (temp == null) temp = "0";
    params.put(IPSHtmlParameters.SYS_CONTEXT, new String[] {temp.toString()});

    temp = objectId.getParentId();
    if (temp != null)
      params.put(IPSHtmlParameters.SYS_CLONEDPARENTID, new String[] {temp.toString()});
    return params;
  }
  ...
  public static Map<String, IPSGuid> resolveSiteFolders(Object siteNameObj, Object folderPathObj)
    ...
    if (folderPath.startsWith("//") && !folderPath.startsWith("//Sites")) {
      throw new IllegalArgumentException("Absolte site folder path must start with '//Sites'");
    }
    if (!StringUtils.isBlank(folderPath) && !folderPath.startsWith("/")) {
      throw new IllegalArgumentException("Site folder path must start with '/'");
    }

    if (!folderPath.startsWith("//Sites") && folderPath.startsWith("/")) {
      StringBuffer absPath = new StringBuffer();
      IPSSite site = PSContentBrowser.computeAbsoluteFolderPath(folderPath, absPath);
      siteid = site.getGUID();
    ...
    if (!StringUtils.isBlank(folderPath)) {
      ...
      if (guids.size() < 1) {
        throw new IllegalArgumentException(
            "'" + folderPath + "' does not resolve to a folder in the system");
      }
      folderid = guids.get(guids.size() - 1);
    }
    // Explicit siteid is required
    if (siteNameObj != null) map.put(IPSHtmlParameters.SYS_SITEID, siteid);
    // Explicit fodlerid is required
    if (folderPathObj != null) map.put(IPSHtmlParameters.SYS_FOLDERID, folderid);
