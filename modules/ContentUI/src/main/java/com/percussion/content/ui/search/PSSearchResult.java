...
public class PSSearchResult {
  ...
  public String getSearchResults(IPSRequestContext request)
    ...
    if (search.useExternalSearch()) {
      ...
      if (searchMode.equals(PSSearch.SEARCH_MODE_ADVANCED)) {
        ...
        if (synExpParam != null) synExpVal = PSSearchAdvancedPanel.BOOL_YES;
        else synExpVal = PSSearchAdvancedPanel.BOOL_NO;

        search.setProperty(PSSearchAdvancedPanel.PROP_SYNONYM_EXPANSION, synExpVal);
      } else {
        // need to set these to the defaults
        search.removeProperty(PSSearchAdvancedPanel.PROP_SYNONYM_EXPANSION, null);
        ...
        search.setProperty(PSSearchAdvancedPanel.PROP_SYNONYM_EXPANSION, synExpansion);
      }
    ...
    if (inCludeSiteColumn) resultColumns.add(IPSHtmlParameters.SYS_SITEID);
    if (inCludeFolderColumn) resultColumns.add(IPSHtmlParameters.SYS_FOLDERID);
    ...
    if (searchProcessor instanceof PSWSSearchRequest)
      ((PSWSSearchRequest) searchProcessor).setUseExternalSearchEngine(search.useExternalSearch());
    PSWSSearchResponse searchResp = searchProcessor.executeSearch();
    Map<String, String> iconMap = getIconPathMap(searchResp, request.getUserName());
    ...
    for (IPSSearchResultRow row : list) {
      ...
      String ctypeId = row.getColumnValue(IPSHtmlParameters.SYS_CONTENTTYPEID);
      String ctypeName = row.getColumnDisplayValue(IPSHtmlParameters.SYS_CONTENTTYPEID);
      String title = row.getColumnValue(IPSHtmlParameters.SYS_TITLE);
      JSONObject jsonObj = new JSONObject();
      String colId = cid;
      jsonObj.put(PSContentBrowser.COLUMN_NAME, PSContentBrowser.getNameHtml(title));
      ...
      jsonObj.put(PSContentBrowser.COLUMN_ICON_PATH, StringUtils.defaultString(iconMap.get(cid)));
      if (inCludeSiteColumn) {
        String siteId = row.getColumnValue(IPSHtmlParameters.SYS_SITEID);
        colId += ":" + siteId;
        String siteName = row.getColumnDisplayValue(IPSHtmlParameters.SYS_SITEID);
        jsonObj.put(IPSHtmlParameters.SYS_SITEID, siteId);
        jsonObj.put(COLUMN_SITE, siteName);
      }
      if (inCludeFolderColumn) {
        String folderId = row.getColumnValue(IPSHtmlParameters.SYS_FOLDERID);
        colId += ":" + folderId;
        String folderPath = row.getColumnDisplayValue(IPSHtmlParameters.SYS_FOLDERID);
   ...
   */
  private Map<String, String> getIconPathMap(PSWSSearchResponse resp, String username) {
  ...
  private Map parseParameters(Iterator params, Map<String, SearchField> searchFields)
    ...
    if (searchFields == null) throw new IllegalArgumentException("searchFields cannpt be null");
      ...
      if (name.endsWith(OP_DELIMITER)) {
        ...
        if (paramNumber != null) {
          try {
            ...
            if (fieldName.equals(IPSHtmlParameters.SYS_CONTENTTYPEID)) ctypeFound = true;
            SearchField field = searchFields.get(fieldName);
            if (field == null) {
              field = new SearchField(fieldName);
              searchFields.put(fieldName, field);
    ...
    if (!ctypeFound) {
      SearchField field = new SearchField(IPSHtmlParameters.SYS_CONTENTTYPEID);
      String slotid = (String) parameters.get("slotId");
      if (!StringUtils.isBlank(slotid)) {
        List<String> values = new ArrayList<String>();
        List<IPSNodeDefinition> defs = PSActionUtil.getAllowedNodeDefsForSlot(slotid);
