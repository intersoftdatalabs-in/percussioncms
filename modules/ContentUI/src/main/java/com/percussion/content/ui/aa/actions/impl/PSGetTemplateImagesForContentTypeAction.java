...
public class PSGetTemplateImagesForContentTypeAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    try {
      ...
      // Get the templates
      Collection<IPSAssemblyTemplate> templates = new ArrayList<IPSAssemblyTemplate>();

      if (objectId.getNodeType() != null
          && (objectId.getNodeType().equals("" + PSAANodeType.AA_NODE_TYPE_SLOT.getOrdinal())
              || objectId
                  .getNodeType()
                  .equals("" + PSAANodeType.AA_NODE_TYPE_SNIPPET.getOrdinal()))) {
        templates =
            PSGetItemTemplatesForSlotAction.getAssociatedTemplates(cTypeId, objectId.getSlotId());
      } else {
      ...
      IPSSite site =
          sm.loadUnmodifiableSite(
              PSGuidManagerLocator.getGuidMgr().makeGuid(siteid, PSTypeEnum.SITE));
      ...
      for (IPSAssemblyTemplate template : templates) {
        ...
        jobj.append("thumbUrl", getImageUrl(template.getName(), siteName, "Thumb", fileNames));
        jobj.append("fullUrl", getImageUrl(template.getName(), siteName, "Full", fileNames));
        array.put(jobj);
      }
  ...
  private Collection<IPSAssemblyTemplate> filterTemplatesBySite(
    ...
    if (siteId == null) return templates;
    IPSSiteManager sm = PSSiteManagerLocator.getSiteManager();
    IPSSite site =
        sm.loadUnmodifiableSite(
            PSGuidManagerLocator.getGuidMgr().makeGuid(siteId, PSTypeEnum.SITE));
  ...
  private String getImageUrl(
    ...
    String imgUrl = type.equals("Thumb") ? sysDefaultThumbImage : sysDefaultFullImage;
    if (fileNames.isEmpty()) {
      return imgUrl;
    }
    String imgName = template + "_" + site + "_" + type;
    String iurl = fileNames.get(imgName);
    if (iurl == null) iurl = fileNames.get(template + "_" + WILD_CARD_ANY_SITE + "_" + type);
    if (iurl == null) iurl = fileNames.get(WILD_CARD_ANY_TEMPLATE + "_" + site + "_" + type);
    if (iurl == null)
      iurl = fileNames.get(WILD_CARD_ANY_TEMPLATE + "_" + WILD_CARD_ANY_SITE + "_" + type);
  ...
  private Map<String, String> getImageFileNames() {
    ...
    if (!imgFolder.exists() || !imgFolder.isDirectory()) {
      ms_log.info("Templage image folder " + imgFolderStr + " does not exist.");
      return fileNames;
    }
    ...
    for (File dir : dirs) {
      ...
      for (File file : files) {
        ...
        if (fileNames.get(imgFileKey) != null) {
          String msg =
              "More than one template image exists with same "
                  + "name '{0}' but different extension using '{1}'";
          Object[] args = {fnfull, fileNames.get(imgFileKey)};
