...
public class PSGetUrlAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    if (StringUtils.isBlank(actionname))
      throw new PSAAClientActionException("Missing required actionname parameter.");
    final String GENERIC_ACTION_PREFIX = "ACTION_";
    if (!actionname.startsWith(GENERIC_ACTION_PREFIX) && !ms_allowedTypes.contains(actionname)) {
    ...
    try {
      if (actionname.startsWith("CE_")) {
        Method method =
            getClass()
                .getDeclaredMethod(
                    "do" + actionname, new Class[] {String.class, PSAAObjectId.class});
        IPSGuid guid = new PSGuid(PSTypeEnum.NODEDEF, Long.parseLong(objectId.getContentTypeId()));
        PSNodeDefinition def = PSContentTypeHelper.findNodeDef(guid);
        result = (String) method.invoke(this, new Object[] {def.getQueryRequest(), objectId});
      } else if (actionname.startsWith(GENERIC_ACTION_PREFIX)) {
        String realActionName = actionname.substring(GENERIC_ACTION_PREFIX.length());
        result = doGenericAction(realActionName, objectId);
      } else {
        Method method =
            getClass().getDeclaredMethod("do" + actionname, new Class[] {PSAAObjectId.class});
        result = (String) method.invoke(this, new Object[] {objectId});
      }

    } catch (Exception e) {
      if (e instanceof PSAAClientActionException) throw (PSAAClientActionException) e;
  ...
  private String doCE_EDIT(String queryurl, PSAAObjectId objectId) throws JSONException {
    ...
    url.addParam(IPSHtmlParameters.SYS_REVISION, getCorrectRevision(objectId));
   ...
   */
  @SuppressWarnings("unused") // dynamically called
  private String doCE_VIEW_CONTENT(String queryurl, PSAAObjectId objectId) throws JSONException {
    SimpleURL url = createSimpleUrl(queryurl);
  ...
  private String doCE_VIEW_PROPERTIES(String queryurl, PSAAObjectId objectId) throws JSONException {
    SimpleURL url = createSimpleUrl(queryurl);
    url.addParam(IPSHtmlParameters.SYS_COMMAND, "preview");
  ...
  private String doCE_FIELDEDIT(String queryurl, PSAAObjectId objectId) throws Exception {
    SimpleURL url = createSimpleUrl(queryurl);
    url.addParam(IPSHtmlParameters.SYS_COMMAND, "edit");
    url.addParam(IPSHtmlParameters.SYS_VIEW, "sys_SingleField:" + objectId.getFieldName());
    url.addParam("refreshHint", "Selected");
    url.addParam(IPSHtmlParameters.SYS_CONTENTID, objectId.getContentId());
    url.addParam(IPSHtmlParameters.SYS_REVISION, getCorrectRevision(objectId));
    PSItemDefManager mgr = PSItemDefManager.getInstance();

    PSDisplayMapping dmapping =
        mgr.getDisplayMapping(Long.parseLong(objectId.getContentTypeId()), objectId.getFieldName());
    if (dmapping == null) {
      throw new PSAAClientActionException(
          "Error occured while preparing the field for edit, "
    ...
    if (control != null) {
      ...
      width = StringUtils.defaultIfEmpty(width, getControlParamValue(dmapping, DLG_WIDTH));
      height = StringUtils.defaultIfEmpty(height, getControlParamValue(dmapping, DLG_HEIGHT));
      aaRenderer =
          StringUtils.defaultIfEmpty(aaRenderer, getControlParamValue(dmapping, AA_RENDERER));
    }
  ...
  private String getControlParamValue(PSDisplayMapping mapping, String paramname) {
    ...
    try {
      ...
      List<PSControlParameter> params = (List<PSControlParameter>) controlMeta.getParams();
   ...
   */
  @SuppressWarnings("unused") // dynamically called
  private String doCE_VIEW_REVISIONS(String queryurl, PSAAObjectId objectId) throws JSONException {
    SimpleURL url = createSimpleUrl(queryurl);
    url.addParam(IPSHtmlParameters.SYS_COMMAND, "preview");
   ...
   */
  @SuppressWarnings("unused") // dynamically called
  private String doCE_VIEW_AUDIT_TRAIL(String queryurl, PSAAObjectId objectId)
  ...
  private String doPREVIEW_PAGE(PSAAObjectId objectId) throws JSONException {
    ...
    url.addParam(IPSHtmlParameters.SYS_CONTENTID, objectId.getContentId());
    url.addParam(IPSHtmlParameters.SYS_REVISION, getCurrentRevision(objectId));
  ...
  public String doCE_LINK(@SuppressWarnings("unused") String queryurl, PSAAObjectId objectId)
    ...
    try {
      assemblyUrl = mgr.getAssemblerUrl(req, Integer.parseInt(objectId.getVariantId()));
    ...
    url.addParam(IPSHtmlParameters.SYS_VARIANTID, objectId.getVariantId());
    url.addParam(
        IPSHtmlParameters.SYS_CONTEXT, req.getParameter(IPSHtmlParameters.SYS_CONTEXT, "0"));
    url.addParam(
        IPSHtmlParameters.SYS_AUTHTYPE, req.getParameter(IPSHtmlParameters.SYS_AUTHTYPE, "0"));
    url.addParam(
        IPSHtmlParameters.SYS_SITEID,
        StringUtils.isBlank(objectId.getSiteId()) ? "" : objectId.getSiteId());
    url.addParam(
        IPSHtmlParameters.SYS_FOLDERID,
        StringUtils.isBlank(objectId.getFolderId()) ? "" : objectId.getFolderId());
    return createJsonReturnString(url.toString(), null);
  }
  ...
  private String doTOOL_SHOW_AA_RELATIONSHIPS(PSAAObjectId objectId)
    ...
    url.addParam(IPSHtmlParameters.SYS_RELATIONSHIP_CATEGORY_FILTER, "Active Assembly");
    return createJsonReturnString(url.toString(), null);
  }
  ...
  private String doTOOL_PUBLISH_NOW(PSAAObjectId objectId)
    ...
    try {
      ...
      if (!isPublic) {
        getLogger()
            .info(
                "Attempting to checkin/transition item "
                    + itemGuid.getUUID()
        ...
        Map<String, String> transitionNames = sws.getAllowedTransitions(itemIds);
        ...
        for (String tname : transitionNames.keySet()) {
          ...
          IPSGuid toStateGuid = gmgr.makeGuid(targetStateId, PSTypeEnum.WORKFLOW_STATE);
          if (wfSvc.isPublic(toStateGuid, wfGuid)) {
            PSState targetState = wf.findState(toStateGuid);
            if (targetState.isAdhocEnabled()) adhocToPublic.add(tname);
            else transToPublic.add(tname);
          }
        }
        if (transToPublic.size() != 1) {
          ...
          if (transToPublic.size() > 1) {
            ...
            msg =
                "Only transitions to a public state using adhoc assignment were found."
                    + " The item must be transitioned manually before attempting this action again."
                    + " Action not performed.";
          } else {
            msg =
  ...
  private String doTOOL_LINK_TO_PAGE(PSAAObjectId objectId)
    ...
    try {
      ...
      if (associations.isEmpty()) {
        throw new PSAAClientActionException("This content type has no templates.");
      }
      ...
      if (!found) {
        ...
        for (PSContentTemplateDesc assoc : associations) {
          IPSAssemblyTemplate t = templateSvc.loadTemplate(assoc.getTemplateId(), false);
          if (t.getOutputFormat() == IPSAssemblyTemplate.OutputFormat.Page) {
            templateUuid = t.getGUID().getUUID();
            break;
          } else if (defaultSnippetId < 0
              && t.getOutputFormat() == IPSAssemblyTemplate.OutputFormat.Snippet) {
      ...
      url.addParam(IPSHtmlParameters.SYS_VARIANTID, String.valueOf(templateUuid));
  ...
  private String doMANAGE_NAVIGATION(PSAAObjectId objectId) throws JSONException {
    IPSManagedNavService managedNavService = PSManagedNavServiceLocator.getContentWebservice();
    ...
    if (title == null) title = sum.getName();
  ...
  private SimpleURL createSimpleUrl(String url) {
    ...
    String sys_aamode =
        getRequestContext().getParameter(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY_MODE);
    if (StringUtils.isNotBlank(sys_aamode)) {
      sUrl.addParam(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY_MODE, sys_aamode);
    }
    return sUrl;
  }
  ...
  private SimpleURL createUrlFromAction(String actionName, PSAAObjectId id)
    ...
    while (paramsIter.hasNext()) {
      ...
      if (value.startsWith("$")) {
        ...
        if (varName.equals(IPSHtmlParameters.SYS_CONTENTID)) value = id.getContentId();
        else if (varName.equals(IPSHtmlParameters.SYS_REVISION)) value = getCorrectRevision(id);
        else if (varName.equals(IPSHtmlParameters.SYS_FOLDERID)) value = id.getFolderId();
        else if (varName.equals(IPSHtmlParameters.SYS_SITEID)) value = id.getSiteId();
        else if (varName.equals(IPSHtmlParameters.SYS_VARIANTID)) value = id.getVariantId();
        else if (varName.equals(IPSHtmlParameters.SYS_CONTEXT)) value = id.getContext();
    ...
    if (StringUtils.isBlank(pairs.get(IPSHtmlParameters.SYS_FOLDERID))) {
      ...
      try {
        ...
        for (String path : paths) {
          if (path.startsWith("//Sites")) opaths.add(path);
        }
        if (!opaths.isEmpty()) Collections.sort(opaths);
        ...
        if (StringUtils.isNotBlank(s)) {
          ...
          if (StringUtils.isNotBlank(siteRoot)) {
            ...
            for (int i = opaths.size() - 1; i >= 0; i--) {
              if (!opaths.get(i).toLowerCase().startsWith(siteRoot)) opaths.remove(i);
            }
          }
        }
        if (opaths.size() > 0) {
        ...
        // ignore
        getLogger().error("Exception while trying to infer folder path, processing continues.", e);
    ...
    for (String name : pairs.keySet()) url.addParam(name, pairs.get(name));
    return url;
  }
  ...
  private String createJsonReturnString(String url, Map<String, String> params)
    ...
    if (params != null) {
      ...
      while (iter.hasNext()) {
        ...
        if (StringUtils.isNotBlank(value)) obj.append(key, value);
      }
    }
    return obj.toString();
  }
  ...
  private Map<String, PSControlMeta> getControls() throws Exception {
    if (m_controls == null) {
      ...
      Document controlXML;
      String resource = "sys_psxContentEditorCataloger/getControls.xml";
      ...
      try {
        ...
        if (controlXML != null) {
          ...
          NodeList controlNodes = controlXML.getElementsByTagName(PSControlMeta.XML_NODE_NAME);
  ...
  class SimpleURL {
    ...
    public SimpleURL(String urlbase) {
      ...
      if (urlbase.startsWith("..")) urlbase = urlbase.substring(2);
      ...
      if (!mi_params.isEmpty()) {
        for (String name : mi_params.keySet()) {
          ...
          try {
            ...
            if (StringUtils.isNotBlank(value)) sb.append(URLEncoder.encode(value, "UTF8"));
