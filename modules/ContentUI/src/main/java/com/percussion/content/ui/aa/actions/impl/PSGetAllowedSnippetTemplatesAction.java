...
public class PSGetAllowedSnippetTemplatesAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    return new PSActionResponse(object.toString(), PSActionResponse.RESPONSE_TYPE_JSON);
  }
  ...
  private String[] getSnippetTemplates(PSAAObjectId objectId) throws Exception {
    ...
    if (contentid == null && rid == null) {
      throw new IllegalArgumentException(
          "sys_contentid or sys_relationshid must be specified" + " to load item templates");
    }
    if (contentid == null) {
    ...
    PSActionUtil.addAssemblyParam(assemblyParams, IPSHtmlParameters.SYS_RELATIONSHIPID, rid);
    ...
    for (IPSAssemblyTemplate template : templates) {
      ...
      PSPair<IPSAssemblyItem, IPSAssemblyResult> result = PSActionUtil.assemble(assemblyParams);
    ...
    String[] resultArr =
        new String[] {
          decorateSnippets(slotObj, aItems, aResults), String.valueOf(templates.size())
        };
    return resultArr;
  }
  ...
  private String getContentIdFromRelId(String rid) throws PSCmsException {
    ...
    if (rels.size() == 0) throw new RuntimeException("no relationship found with rid=" + rid);
   ...
   * @throws Exception
   */
  private String decorateSnippets(
      IPSTemplateSlot slotObj, List<IPSAssemblyItem> ais, List<IPSAssemblyResult> results)
      throws Exception {
    StringBuffer sb = new StringBuffer();
    for (int i = 0; i < results.size(); i++) {
      IPSAssemblyResult result = results.get(i);
      IPSAssemblyItem item = ais.get(i);
      String id = PSAAUtils.getSnippetObjectId((IPSAssemblyResult) item, slotObj.getName());
      sb.append("<div class=\"aaSnippet\" id=");
      sb.append(id);
      sb.append(" onClick=" + "\"return ps.aa.controller._onSnippetTemplateSelected(this);\">");
