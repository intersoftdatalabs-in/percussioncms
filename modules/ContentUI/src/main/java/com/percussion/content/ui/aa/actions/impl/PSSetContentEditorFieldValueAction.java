...
public class PSSetContentEditorFieldValueAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    String fieldValue = StringUtils.defaultString((String) getParameter(params, "fieldValue"));
    try {
    ...
    return new PSActionResponse(dummyResult.toString(), PSActionResponse.RESPONSE_TYPE_JSON);
  }
  ...
  private PSActionResponse handleValidationException(PSErrorResultsException e)
    ...
    try {
      if (!StringUtils.isBlank(validationError)) errObj.append("validationError", validationError);
      else if (!StringUtils.isBlank(cmsError)) errObj.append("cmsError", cmsError);
    } catch (JSONException e1) {
      throw new PSAAClientActionException(e1);
    }
    return new PSActionResponse(errObj.toString(), PSActionResponse.RESPONSE_TYPE_JSON);
  }
}
