...
public class PSGetWorkFlowActionsAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    try {
      ...
      int userCommunity = -1;
      String usercomm = (String) reqCtx.getSessionPrivateObject(IPSHtmlParameters.SYS_COMMUNITY);
      ...
      List assignmentTypes =
          sysSvc.getContentAssignmentTypes(contentids, userName, userRoles, userCommunity);
      ...
      for (int i = 0; i < actions.size(); i++) {
        ...
        if (comment.equals(PSMenuAction.VAL_BOOLEAN_TRUE)) commentVal = "2"; // Required
        else if (comment.equals(PSMenuAction.VAL_HIDE)) commentVal = "0"; // Hide
        ...
        if (name.equals(PSMenuAction.CHECKIN_ACTION_NAME)) actionType = "0"; // Checkin
        else if (name.equals(PSMenuAction.FORCE_CHECKIN_ACTION_NAME))
          actionType = "1"; // Force Checkin
        else if (name.equals(PSMenuAction.CHECKOUT_ACTION_NAME)) actionType = "2"; // Checkout
        else if (wfAction.equals("Quick Edit")) actionType = "3"; // Transition and Checkout
        ...
        if (wfAction.equals("Quick Edit")) label = "Quick Edit Check-out";
