...
public class PSCreateItemAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    int ctypeId = getValidatedInt(params, IPSHtmlParameters.SYS_CONTENTTYPEID, true);
    ...
    try {
      ...
      if (newItem == -1) throw new PSAAClientActionException("Failed to create the content item.");
      ...
      if (ve != null) {
        ...
        return new PSActionResponse(obj.toString(), PSActionResponse.RESPONSE_TYPE_JSON);
      }
   ...
   */
  private PSFieldValidationException getValidationError(PSErrorResultsException e) {
    Map<IPSGuid, Object> er = e.getErrors();
    if (er.isEmpty()) return null;
   ...
   */
  private int addItemToFolder(int itemid, String folderPath) throws PSErrorException {
    IPSContentWs cws = PSContentWsLocator.getContentWebservice();
    IPSGuid fid = cws.getIdByPath(folderPath.toString());
    if (fid == null) return -1;
    cws.addFolderChildren(
        getItemGuid(fid.getUUID()), Collections.singletonList(getItemGuid(itemid)));
    return fid.getUUID();
  }
