...
public class PSGetSnippetPickerSlotContentAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    if (StringUtils.isBlank(showTitlesParam) || !showTitlesParam.equalsIgnoreCase("true")) {
      showTitles = false;
    }
    String output = null;
    try {
      output = showTitles ? getSnippetTitles(objectId) : getAssembledSnippets(objectId);
    } catch (Throwable e) {
  ...
  private String getAssembledSnippets(PSAAObjectId objectId) throws Throwable {
    ...
    List<IPSAssemblyResult> results = autils.assemble(pair.getFirst(), slot, null);
    ...
    for (int i = 0; i < results.size(); i++) {
      ...
      String begin =
          "<div class=\""
              + cssClass
              + "\" rid=\""
              + result.getParameterValue(IPSHtmlParameters.SYS_RELATIONSHIPID, "")
              + "\">";
      String end = "</div>";
      sb.append(begin);
  ...
  private String getSnippetTitles(PSAAObjectId objectId) throws PSCmsException {
    ...
    PSLocator ownerLocator = new PSLocator(objectId.getContentId(), getCorrectRevision(objectId));
    ...
    List<PSComponentSummary> summarylist = cms.loadComponentSummaries(idList);
    ...
    Comparator<RelatedItems> relObjectComparator =
        new Comparator<RelatedItems>() {
    ...
    for (RelatedItems obj : objects) {
      ...
      String begin = "<div class=\"" + cssClass + "\" rid=\"" + obj.relId + "\">";
      String end = "</div>";
      sb.append(begin);
