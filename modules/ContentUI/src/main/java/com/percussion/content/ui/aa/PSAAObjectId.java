...
/**
 * The object id class to recognize active assembly elements. It creates and parses a JSON Array
 * consisting of the values of following parameters in the order mentioned below.
 *
 * <p>0 - Active assembly node type as given in PSAANodeType
 *
 * <p>1 - Content id
 *
 * <p>2 - Template id
 ...
 * <p>7 - Contenttype Id
 *
 * <p>8 - Checkedout status The values will be 0 if it is not checked out to anyone 1 if checked out
 * by the login user 2 if it is checked out to someone other than current user
 *
 * <p>9 - Slot id
 *
 * <p>10 - Relationship id
 *
 * <p>11 - Field name
 *
 * <p>12 - Parent Content Id
 *
 * <p>13 - Field label
 *
 * <p>14 - Sort rank applicable to only snippet object tytpes
 */
public class PSAAObjectId {
  ...
  public static String createObjectString(
      int contentId, int templateId, long contentTypeId, int slotId) {
    JSONArray result = new JSONArray();
    try {
      result.put(
          m_indices.get(PARAM_NODE_TYPE).intValue(), PSAANodeType.AA_NODE_TYPE_SNIPPET.ordinal());
  ...
  /**
   * Constructor that takes a string representation of JSON array of values of the parameters
   ...
   * @param nodeType The type of the node must not be <code>null</code>.
   * @param item The assembly work item must not be <code>null</code>.
   * @param slotname The name of the slot or field, may be <code>null</code> for nodetype page but
   *     must not be null or empty for node types slot,snippet and field.
   * @param sortrank optional sort rank, may be <code>null</code> or empty.
   * @throws JSONException if fails to create JSON array from the assembly item parameters.
   * @throws PSMissingBeanConfigurationException is the assmebly service is not loaded because of
   *     incorrect service confgiuration.
   * @throws PSAssemblyException if assembly service throws errors while loading slot or template in
   *     the process of building the objectid.
   */
  public PSAAObjectId(PSAANodeType nodeType, IPSAssemblyItem item, String slotname, String sortrank)
      throws PSAssemblyException, PSMissingBeanConfigurationException, JSONException {
    if (item == null) {
    ...
    if ((nodeType.equals(PSAANodeType.AA_NODE_TYPE_SLOT)
            || nodeType.equals(PSAANodeType.AA_NODE_TYPE_SNIPPET)
            || nodeType.equals(PSAANodeType.AA_NODE_TYPE_FIELD))
   ...
   * @param sortrank sort rank (applicable to only {@link PSAANodeType#AA_NODE_TYPE_SNIPPET}) as
   *     string, may be <code>null</code> or empty.
   * @throws PSAssemblyException
   * @throws PSMissingBeanConfigurationException
   * @throws JSONException
   */
  private void addParamsToJsonArray(
      PSAANodeType nodeType, IPSAssemblyItem item, String name, String sortrank)
      throws PSAssemblyException, PSMissingBeanConfigurationException, JSONException {
    ...
    switch (nodeType) {
      case AA_NODE_TYPE_SLOT:
        // slotid required
        int slotid =
            PSAssemblyServiceLocator.getAssemblyService().findSlotByName(name).getGUID().getUUID();
        params.put(IPSHtmlParameters.SYS_SLOTID, String.valueOf(slotid));
        break;
      case AA_NODE_TYPE_SNIPPET:
        // relationshipid required
        temp = parseParam(allParams, IPSHtmlParameters.SYS_RELATIONSHIPID, null, true);
      ...
      case AA_NODE_TYPE_FIELD:
        ...
        try {
          IPSNodeDefinition def = (IPSNodeDefinition) item.getNode().getDefinition();
          PSItemDefManager idManager = PSItemDefManager.getInstance();
          String label = idManager.getFieldLabel(def.getGUID().longValue(), name);
  ...
  public void modifyParam(String paramname, String value) throws JSONException {
    ...
    if (index == null) throw new IllegalArgumentException("The param name is not valid.");
    if (value == null) m_objectId.put(index, JSONObject.NULL);
    else m_objectId.put(index, value);
  }
  ...
  /**
   * Helper method to find the value of check out status for the specified item.
   *
   * @param sum component summary of the item in question, assumed not <code>null</code>.
   * @return The String representation of value of:
   *     <ol>
   *       <li>0 if the item is not checked to by anyone
   *       <li>1 if it is checked out to the login user
   *       <li>2 if it is checked out to someone other than login user
   *     </ol>
   */
  private static String getCheckOutStatusValue(PSComponentSummary sum) {
    ...
    if (StringUtils.isBlank(chkuser)) return String.valueOf(NOT_CHECKED_OUT);
    else if (chkuser.equalsIgnoreCase(loginuser)) return String.valueOf(CHECKED_OUT_BYME);
  ...
  public PSComponentSummary getItemSummary()
      throws PSMissingBeanConfigurationException, NumberFormatException {
    IPSCmsObjectMgr objMgr = PSCmsObjectMgrLocator.getObjectManager();
    PSComponentSummary summary = objMgr.loadComponentSummary(Integer.parseInt(getContentId()));
    return summary;
  }

  /**
   * Helper method to get the parameters from the item, adds IPSHtmlParameters.SYS_VARIANTID
   * parameter to the list if there is one in the item.
   *
  ...
  private Map<String, String> getParams(IPSAssemblyItem item) {
    ...
    while (iter.hasNext()) {
      ...
      if (val != null && val.length > 0) params.put(key, val[0]);
    }

    if (item.getOriginalTemplateGuid() != null) {
      params.put(
          IPSHtmlParameters.SYS_VARIANTID,
          String.valueOf(item.getOriginalTemplateGuid().getUUID()));
    }
   ...
   * @throws PSAssemblyException
   */
  private static Map<String, String> parseCommonParams(IPSAssemblyItem item, Map params)
      throws PSAssemblyException, PSMissingBeanConfigurationException {
    Map<String, String> id = new HashMap<String, String>();
    // ContentId
    String temp = parseParam(params, IPSHtmlParameters.SYS_CONTENTID, null, true);
    ...
    temp = null;
    if (pItem == null) {
      temp = parseParam(params, IPSHtmlParameters.SYS_CLONEDPARENTID, null, false);
    } else {
    ...
    if (!StringUtils.isEmpty(temp)) id.put(IPSHtmlParameters.SYS_FOLDERID, temp.toString());
    ...
    id.put(IPSHtmlParameters.SYS_CONTENTTYPEID, String.valueOf(sum.getContentTypeId()));
  ...
  /**
   * Helper method to parse parameter with a given name in the supplied parameter map.
   *
   * @param params parameter map assumed not <code>null</code> or empty.
   * @param name nameof the parameter to parse, assumed not <code>null</code> or empty.
   * @param defValue defualt value to be used if the parameter does not exist in the supplied map,
   *     may be <code>null</code> or empty.
   * @param isRequired <code>true</code> if the parameter must have a non <code>null</code> or non
   *     empty value directly or via the default value.
   * @return parsed parameter, may be <code>null</code> or empty.
   * @throws IllegalArgumentException if the value is required and the value is resolved to <code>
   *     null</code> or empty via parameter map or defualt value.
   */
  private static String parseParam(Map params, String name, String defValue, boolean isRequired) {
    String val = defValue;
    Object temp = params.get(name);
    if (temp != null && !StringUtils.isEmpty(temp.toString())) val = temp.toString();
   ...
   *
   * @return The node type value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return A revisionless guid. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The content id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The sortrank id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The variant id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The site id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The folder id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The context value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The authtype value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The contenttype id value. Will be <code>null</code> if the property does not exist.
   */
  public String getContentTypeId() {
    return getValue(m_indices.get(IPSHtmlParameters.SYS_CONTENTTYPEID).intValue());
  }

  /**
   * Gets the check out status value from the JSON array object.
   *
   * @return The check out status value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The slot id value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The relationship id value. Will be <code>null</code> if the property does not exist.
   */
  public String getRelationshipId() {
    return getValue(m_indices.get(IPSHtmlParameters.SYS_RELATIONSHIPID).intValue());
  }

  /**
   * Gets the field name value from the JSON array object.
   *
   * @return The field name value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The field label value. Will be <code>null</code> if the property does not exist.
   */
   ...
   *
   * @return The content id of the parent value. Will be <code>null</code> if the property does not
   *     exist.
   */
  ...
  /**
   * JSON array object of the parameters mentioned in the class description. Initialized in the
  ...
  /** Constant value for checkout status indicating the item is not checked out to any one */
  private static final String NOT_CHECKED_OUT = "0";

  /** Constant value for checkout status indicating the item is checked to the login user */
  private static final String CHECKED_OUT_BYME = "1";

  /**
   * Constant value for checkout status indicating the item is checked to some one other than the
   * login user
   */
  private static final String CHECKED_OUT_BYOTHERS = "2";

  /** Constant map for to maintain the indices of the parameters in the JSON array object */
  ...
  public static void main(String[] args) {
    ...
    try {
      for (String s : jsonArrays.keySet()) {
        ...
        System.out.println(IPSHtmlParameters.SYS_CONTENTID + " : " + objid.getContentId());
        System.out.println(IPSHtmlParameters.SYS_VARIANTID + " : " + objid.getVariantId());
        System.out.println(IPSHtmlParameters.SYS_SITEID + " : " + objid.getSiteId());
        System.out.println(IPSHtmlParameters.SYS_FOLDERID + " : " + objid.getFolderId());
        System.out.println(IPSHtmlParameters.SYS_CONTEXT + " : " + objid.getContext());
        System.out.println(IPSHtmlParameters.SYS_AUTHTYPE + " : " + objid.getAuthType());
        System.out.println(IPSHtmlParameters.SYS_CONTENTTYPEID + " : " + objid.getContentTypeId());
        System.out.println(PARAM_CHECKEDOUT_STATUS + " : " + objid.getCheckoutStatus());
        System.out.println(IPSHtmlParameters.SYS_SLOTID + " : " + objid.getSlotId());
        System.out.println(
            IPSHtmlParameters.SYS_RELATIONSHIPID + " : " + objid.getRelationshipId());
