...
public class PSGetInlinelinkParentsAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    return new PSActionResponse(jarray.toString(), PSActionResponse.RESPONSE_TYPE_PLAIN);
  }
  ...
  private Set<Integer> getInlinelinkOwners(int dependentId) {
    ...
    try {
      List<PSAaRelationship> rels = service.loadContentRelations(filter, false);
      String p;
  ...
  private Set<Integer> getManagedIds(Map<String, Object> params) {
    ...
    if (StringUtils.isBlank(mids)) {
      throw new IllegalArgumentException(
          "Parameter '" + MANAGED_IDS + "' must not be null or empty.");
    }
