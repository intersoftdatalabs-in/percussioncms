...
public class PSContentBrowser {
  ...
  public static String getSites() throws JSONException, PSErrorException {
    ...
    for (IPSSite site : sites) {
      if (isBlank(site.getFolderRoot()) || !startsWith(site.getFolderRoot(), "//Sites")) continue;
    ...
    for (IPSSite site : filterSites) {
      if (StringUtils.isBlank(site.getFolderRoot())) continue; // skip site without root folder
  ...
  public static String getRootFolders(IPSRequestContext request)
    ...
    IPSContentWs cws = PSContentWsLocator.getContentWebservice();
    List<PSItemSummary> summaries = cws.findFolderChildren(new PSLegacyGuid(3), true);
  ...
  public static String getSiteFolderChildren(
    ...
    if (!StringUtils.isEmpty(contentTypeId)) {
      try {
        ...
        throw new IllegalArgumentException(
            "content type id specified as '" + contentTypeId + "' is invalid");
      }
    }
    ...
    try {
      ...
      throw new IllegalArgumentException("slot id specified as '" + slotId + "' is invalid");
    }
    ...
    List<PSItemSummary> summaries = getFolderChildrenSummaries(request, fullFolderPath);

    Map<String, String> iconPathMap = getIconPathMap(summaries, request.getUserName());
    JSONArray result = new JSONArray();
    Set<Integer> allowedContentTypeIds = null;
    for (PSItemSummary summary : summaries) {
      if (allowedContentTypeIds == null) {
        allowedContentTypeIds = new HashSet<Integer>();
        Collection<IPSGuid> allowedContentTypeGuid =
            PSActionUtil.getAllowedContentIdsForSlot(slotId);
        for (IPSGuid id : allowedContentTypeGuid) allowedContentTypeIds.add(id.getUUID());
      }
      ...
      if (t != PSFolder.FOLDER_CONTENT_TYPE_ID) {
        ...
        if (!allowedContentTypeIds.contains(t)) continue;
        // Apply content type filter
        if (ctypeid != -1 && t != ctypeid) continue;
      }
  ...
  private static Map<String, String> getIconPathMap(List<PSItemSummary> summaries, String user) {
    ...
    if (summaries == null || summaries.size() < 1) return icmap;
    ...
    for (PSComponentSummary csum : csums) {
      ...
      if (user.equalsIgnoreCase(csum.getCheckoutUserName())) loc = csum.getTipLocator();
      locs.add(loc);
    }
  ...
  public static String getFolderChildren(
    ...
    List<PSItemSummary> summaries = getFolderChildrenSummaries(request, folderPath);
    Map<String, String> iconPathMap = getIconPathMap(summaries, request.getUserName());
    ...
    for (PSItemSummary summary : summaries) {
      ...
      // is -1.
      int t = summary.getContentTypeId();
      if (t != PSFolder.FOLDER_CONTENT_TYPE_ID && ctypeid != -1 && t != ctypeid) continue;
      result.put(summaryToJsonObject(summary, iconPathMap));
    }
    return result.toString();
  }
  ...
  private static String validateFolderPath(String folderPath) throws IllegalArgumentException {
    if (folderPath == null || folderPath.length() == 0) {
      throw new IllegalArgumentException("absSiteFolderPath must not be null or empty");
    }

    if (!folderPath.startsWith("//Sites") && !folderPath.startsWith("//Folders")) {
  ...
  public static String createSiteFolder(
    ...
    if (folderName == null || folderName.length() == 0) {
      throw new IllegalArgumentException("folderName must not be null or empty");
    }
    StringBuffer tmp1 = new StringBuffer();
  ...
  public static boolean canCreateFolder(IPSRequestContext request, String folderId)
    ...
    if (StringUtils.isBlank(folderId))
      throw new IllegalArgumentException("parentFolderId must not be null or empty");
  ...
  public static boolean canCreateItem(
    ...
    if (!canCreateFolder(request, parentFolderId)) return false;
  ...
  public static IPSSite computeAbsoluteFolderPath(
    ...
    if (!absSiteFolderPath.startsWith("/")) {
      throw new IllegalArgumentException("absSiteFolderPath must start with '/'");
    }
    if (absFolderPath.length() > 0) absFolderPath.delete(0, absFolderPath.length() - 1);
    ...
    if (index == -1) {
      ...
      if (fullpath.length() > index) folderPath = fullpath.substring(index + 1);
    }
    if (StringUtils.isEmpty(siteName)) {
      throw new IllegalArgumentException("siteName part in the path must not be null or empty");
    }
    IPSSiteManager sm = PSSiteManagerLocator.getSiteManager();
    IPSSite site = sm.loadSite(siteName);
    String sfRoot = site.getFolderRoot();
    if (!sfRoot.endsWith("/")) sfRoot += "/";
  ...
  private static List<PSItemSummary> getFolderChildrenSummaries(
    ...
    IPSContentWs cws = PSContentWsLocator.getContentWebservice();
    List<PSItemSummary> summaries = cws.findFolderChildren(absFolderPath, true);
    return filterSummaries(summaries);
  }
  ...
  private static List<PSItemSummary> filterSummaries(List<PSItemSummary> items) {
    ...
    for (PSItemSummary s : items) {
      ...
      if (ops != null && !ops.isEmpty() && ops.iterator().next() != OperationEnum.NONE) {
        rvalue.add(s);
      }
    }
    return rvalue;
  ...
  static JSONObject summaryToJsonObject(PSItemSummary summary, Map<String, String> iconMap)
    ...
    String iconPath = StringUtils.defaultString(iconMap.get("" + summary.getGUID().getUUID()));
  ...
  public static String getNewItemUrlByFolderPath(String parentPath, String ctypeid)
    ...
    String url = PSUrlUtils.createUrl(urlString, params.entrySet().iterator(), null);
    return url;
  }
