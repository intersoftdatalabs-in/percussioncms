...
public abstract class PSAAActionBase implements IPSAAClientAction {
  ...
  protected PSAAObjectId getObjectId(Map<String, Object> params) throws PSAAClientActionException {
    ...
    if (StringUtils.isNotBlank(objectId)) {
      try {
        ...
        throw new PSAAClientActionException("Error parsing JSON array string.", e);
      }
    } else {
      throw new PSAAClientActionException("Required objectid does not exist.");
    }
  }
  ...
  protected int getValidatedInt(Map<String, Object> params, String paramname, boolean required)
    ...
    if (StringUtils.isNotBlank(param)) {
      try {
        ...
        throw new PSAAClientActionException(param + " is not a valid integer.");
      }
    } else if (required) {
      throw new PSAAClientActionException(paramname + " is a required parameter.");
    }
    return -1;
  }
  ...
  protected String getCorrectRevision(PSAAObjectId objectId) {
    ...
    PSComponentSummary summary = objectId.getItemSummary(Integer.parseInt(objectId.getContentId()));
  ...
  protected int getCurrentCommunityUuid() {
    ...
    if (o != null) {
      if (o instanceof Integer) commId = ((Integer) o).intValue();
      else commId = Integer.parseInt(o.toString());
    }
    return commId;
  }
  ...
  protected PSAAClientActionException createException(PSErrorException e) {
    ...
    PSAAClientActionException ex = new PSAAClientActionException(e.getLocalizedMessage());
    return ex;
  }
  ...
  protected PSAAClientActionException createException(PSErrorsException e) {
    ...
    if (errors.size() == 0) throw new RuntimeException("Unexpected empty error.");
