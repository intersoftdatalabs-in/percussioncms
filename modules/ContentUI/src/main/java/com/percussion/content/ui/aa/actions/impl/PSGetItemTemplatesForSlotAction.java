...
public class PSGetItemTemplatesForSlotAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    try {
      ...
      String filterTemplatesByComm =
          PSServer.getProperty("contentBrowserFilterTemplatesByComunity");
      if (filterTemplatesByComm != null && filterTemplatesByComm.equalsIgnoreCase("true")) {
      ...
      for (IPSAssemblyTemplate template : temps) {
        ...
        String vid = String.valueOf(template.getGUID().getUUID());
        obj.append("variantid", vid);
        obj.append("name", String.valueOf(template.getLabel()));
        obj.append("description", String.valueOf(template.getDescription()));
   ...
   */
  private void filteredTemplatesByCommunity(Collection<IPSAssemblyTemplate> temps)
      throws PSErrorResultsException, RemoteException, PSInternalRequestCallException {
    IPSSecurityDesignWs secDesWs = PSSecurityWsLocator.getSecurityDesignWebservice();

    int comid = PSWebserviceUtils.getUserCommunityId();
    IPSGuid communityGuid =
        PSGuidManagerLocator.getGuidMgr().makeGuid(comid, PSTypeEnum.COMMUNITY_DEF);
    List<PSCommunityVisibility> commVis =
        secDesWs.getVisibilityByCommunity(
            Collections.singletonList(communityGuid),
            PSTypeEnum.TEMPLATE,
    ...
    for (PSCommunityVisibility psCommunityVisibility : commVis) {
      Set<PSObjectSummary> objSumm = psCommunityVisibility.getVisibleObjects();
  ...
  protected static Collection<IPSAssemblyTemplate> getAssociatedTemplates(PSAAObjectId objectId)
    ...
    if (StringUtils.isBlank(objectId.getSlotId()))
      throw new IllegalArgumentException("The slotid in the objectId cannot be null or empty.");
    return getAssociatedTemplates(
        objectId.getItemSummary().getContentTypeId() + "", objectId.getSlotId());
  }
  ...
  protected static Collection<IPSAssemblyTemplate> getAssociatedTemplates(
    ...
    if (StringUtils.isBlank(ctypeid))
      throw new IllegalArgumentException("The ctypeid cannot be null or empty.");
    if (StringUtils.isBlank(slotid))
      throw new IllegalArgumentException("The slotid cannot be null or empty.");
    ...
    for (PSPair<IPSGuid, IPSGuid> pair : assoc) {
      if (pair.getFirst()
          .equals(PSGuidManagerLocator.getGuidMgr().makeGuid(ctypeid, PSTypeEnum.NODEDEF)))
