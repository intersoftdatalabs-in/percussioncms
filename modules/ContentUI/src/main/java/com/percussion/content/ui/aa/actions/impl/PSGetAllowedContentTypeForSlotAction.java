...
public class PSGetAllowedContentTypeForSlotAction extends PSAAActionBase {
  ...
  public PSActionResponse execute(Map<String, Object> params) throws PSAAClientActionException {
    ...
    try {
      ...
      if (StringUtils.isBlank(objectIdStr)) {
        ...
        Collection summ = defMgr.getSummaries(getRequestContext().getSecurityToken());
        ...
        for (Object sum : summLst) {
          ...
          array.put(
              getCtypeObject(
                  String.valueOf(def.getGUID().getUUID()),
                  String.valueOf(def.getLabel()),
                  String.valueOf(def.getDescription())));
        }
      } else {
   ...
   */
  private JSONObject getCtypeObject(String id, String name, String desc) throws JSONException {
