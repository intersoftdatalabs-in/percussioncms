...
function checkFile(fileName) {
  var file = fs.readFileSync(fileName, "utf8"),
    notAllowed;
  if (
    (notAllowed = file.match(/[\x00-\x08\x0b\x0c\x0e-\x19\uFEFF\t]|[ \t]\n/))
  ) {
    var msg;
    ...
    fail(msg + " at line " + info.line + ", column " + info.column, {
      source: fileName,
    });
  }

  if (!cBlob.test(file)) fail("Missing license blob", { source: fileName });
  ...
  var allowedGlobals = Object.create(topAllowedGlobals),
    m;
    ...
    if (!(glob in allowedGlobals))
      fail(
        "Access to global variable " +
          glob +
          ". Add a '// declare global: " +
          glob +
          "' comment or add this variable in test/lint/lint.js.",
        globalsSeen[glob]
      );
  ...
  for (var i = 0; i < scopes.length; ++i) {
    ...
    for (var name in scope.vars) {
      ...
      if (
        !info.used &&
        info.type != "catch clause" &&
        info.type != "function name" &&
        name.charAt(0) != "_"
      )
