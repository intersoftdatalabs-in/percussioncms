...
tinymce.PluginManager.add("percadvimage", function (editor, url) {
  ...
  function showDialog(buttonAPI) {
    ...
    function buildImageList() {
      ...
      tinymce.each(editor.options.get("image_list"), function (link) {
        linkImageItems.push({
    ...
    function setReadWriteMode() {
      ...
      if (imgElm && !cm1LinkData.title) {
        if (
          !imgElm.getAttribute("data-description-override") ||
          !imgElm.getAttribute("data-title-override")
        ) {
    ...
    function setReadWriteModeAlt() {
      ...
      var sys_dependentid = cm1LinkData.sys_dependentid;
      var datadescriptionoverride = formData.isDataDescriptionOverride;
      var datapreviousaltoverride =
        formData.dataPreviousAltOverride == null ||
        typeof formData.dataPreviousAltOverride === "undefined"
          ? ""
          : formData.dataPreviousAltOverride;
      var datadecorativeoverride = formData.isDataDecorativeOverride;
    ...
    function setReadWriteModeTitle() {
      ...
      var description = formData.alt;
      var sys_dependentid = cm1LinkData.sys_dependentid;
      var dataprevioustitleoverride =
        formData.dataPreviousTitleOverride == null ||
        typeof formData.dataPreviousTitleOverride === "undefined"
          ? ""
          : formData.dataPreviousTitleOverride;
      var datatitleoverride = formData.isDataTitleOverride;
      var datadecorativeoverride = formData.isDataDecorativeOverride;
    ...
    function getImageData(itemId, updateTitle, callback) {
      topFrJQ.PercPathService.getPathItemById(
        itemId,
        function (idstatus, pathItem) {
          if (!idstatus) {
            topFrJQ.perc_utils.info(pathItem);
            topFrJQ.perc_utils.alert_dialog({
              title: "Error",
              content:
                "Sorry for the inconvenience, could not get selected item details to insert image.",
            });
            return;
          }
          ...
          topFrJQ.PercPathService.getInlineRenderLink(
            ...
            function (status, data) {
              ...
              var previousoverride =
                imgElm !== null
                  ? imgElm.getAttribute("data-previous-alt-override")
                  : "";
              var previoustitleoverride =
                imgElm !== null
                  ? imgElm.getAttribute("data-previous-title-override")
                  : "";
              ...
              if (isUpgradeScenario) {
                if (
                  currentaltoverride !== renderLink.altText &&
                  currentaltoverride !== ""
                ) {
                ...
                if (
                  currenttitleoverride !== renderLink.title &&
                  currenttitleoverride !== ""
                ) {
                  formData.isTitleOverride = true;
                  imgElm.setAttribute("data-title-override", true);
                  if (
                    previoustitleoverride !== "undefined" &&
                    previoustitleoverride === "" &&
                    currenttitleoverride !== "undefined" &&
                    currenttitleoverride === ""
                  ) {
    ...
    formData.dataImgtype = isImage
      ? dom.getAttrib(imgElm, "data-imgtype")
      : "_full";
    formData.constrain = isImage
      ? dom.getAttrib(imgElm, "data-constrain") === "true"
      : false;
    formData.url = isImage ? dom.getAttrib(imgElm, "src") : "";
    formData.src = { value: formData.url };
    formData.alt = isImage ? dom.getAttrib(imgElm, "alt") : "";
    formData.isDataDescriptionOverride = isImage
      ? dom.getAttrib(imgElm, "data-description-override") === "true"
      : false;
    formData.dataPreviousAltOverride = isImage
      ? dom.getAttrib(imgElm, "data-previous-alt-override")
      : "";
    formData.dataPreviousTitleOverride = isImage
      ? dom.getAttrib(imgElm, "data-previous-title-override")
      : "";
    formData.isDataDecorativeOverride = isImage
      ? dom.getAttrib(imgElm, "data-decorative-override") === "true"
      : false;
    formData.isDataTitleOverride = isImage
      ? dom.getAttrib(imgElm, "data-title-override") === "true"
      : false;
    formData.title = isImage ? dom.getAttrib(imgElm, "title") : "";
    formData.align = isImage
      ? dom.getStyle(imgElm, "float") !== ""
        ? dom.getStyle(imgElm, "float")
        : dom.getStyle(imgElm, "vertical-align")
      : "";
    cm1LinkData.sys_dependentvariantid = isImage
      ? dom.getAttrib(imgElm, "sys_dependentvariantid")
      : "";
    cm1LinkData.rxinlineslot = isImage
      ? dom.getAttrib(imgElm, "rxinlineslot")
      : "";
    cm1LinkData.sys_dependentid = isImage
      ? dom.getAttrib(imgElm, "sys_dependentid")
      : "";
    ...
    win = editor.windowManager.open({
      ...
      body: {
        ...
        items: [
          ...
          {
            name: "isDataDecorativeOverride",
            type: "checkbox",
            label: "Decorative- Image (WAI)",
            ...
            items: [
              {
                ...
                label: "Image description",
                size: 26,
                classes: "perc-description",
              },
              {
                name: "isDataDescriptionOverride",
                type: "checkbox",
                label: "override",
              },
            ],
          ...
          },
          {
            ...
            maxWidth: 135,
            items: [
              { text: "Full", value: "_full" },
              { text: "Thumbnail", value: "_thumb" },
            ],
          },
          {
            label: "Alignment",
      ...
      onSubmit: function (e) {
        resetAriaBoxes();
        formData.dataPreviousAltOverride = formData.isDataDescriptionOverride
          ? formData.alt
          : formData.dataPreviousAltOverride;
        formData.dataPreviousTitleOverride = formData.isDataTitleOverride
          ? formData.title
          : formData.dataPreviousTitleOverride;
        if (!formData.isDataDecorativeOverride) {
          if (
            formData.isDataDescriptionOverride &&
            (!formData.alt || formData.alt.trim() === "")
          ) {
        ...
        var imgPath = dataSrc.trim(),
          imgPathLower = imgPath.toLowerCase();
        //Resolve manually entered internal links
        if (
          imgPathLower.match("^//sites/") ||
          imgPathLower.match("^//assets/") ||
          imgPathLower.match("^/sites/") ||
          imgPathLower.match("^/assets/") ||
          imgPathLower.match("^sites/") ||
          imgPathLower.match("^assets/")
          ...
          topFrJQ.PercPathService.getPathItemForPath(
            imgPath,
            function (status, result) {
              if (
                status === topFrJQ.PercServiceUtils.STATUS_ERROR ||
                result.PathItem.type !== "percImageAsset"
              ) {
                topFrJQ.perc_utils.alert_dialog({
                  title: "Error",
                  content:
                    "We were unable to create the image because this is an invalid URL. Please validate the URL and re-enter the image.",
                });
              } else {
                updateLinkData(
                  result.PathItem,
                  function (fullUrl, thumUrl, title) {
                    formData.url =
                      formData.dataimgtype === "_thumb" ? thumUrl : fullUrl;
                    formData.src = { value: formData.url };
