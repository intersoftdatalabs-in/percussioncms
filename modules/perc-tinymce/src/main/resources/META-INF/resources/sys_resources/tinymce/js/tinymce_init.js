...
function percTinyMceInitialized(tinymceEditor) {
  ...
  tinymceEditor.ui.registry.addContextMenu("useBrowserSpellcheck", {
    update: function (node) {
      return tinymceEditor.selection.isCollapsed()
          ...
          $.each(data, function (i, item) {
            if (
              (!item.hasOwnProperty("roles") ||
                $(options.userRoles).not(item.roles).length <
                  $(options.userRoles).length) &&
              (!item.hasOwnProperty("communityNames") ||
                $.inArray(options.community, item.communityNames) >= 0) &&
    ...
    var mergedBaseOptions = $.extend(
      {},
      {
        ...
        external_plugins: {
          ...
          percglobalvariables:
            "/sys_resources/tinymce/plugins/percglobalvariables/plugin.js",
        ...
        file_picker_callback: function (callback, value, meta) {
          ...
          if (meta.filetype == "file") {
            var pathSelectionOptions = {
              ...
              selectedItemValidator: function (pathItem) {
                return pathItem &&
                  (pathItem.type === "percPage" ||
                    pathItem.type === "percImageAsset" ||
                    pathItem.type === "percFileAsset")
                  ? null
                  : "Please select a page, file, or an image";
              },
              acceptableTypes:
                "percPage,percImageAsset,percFileAsset,site,Folder",
            };
          ...
          if (meta.filetype == "image") {
            ...
            var validator = function (pathItem) {
              return pathItem && pathItem.type === "percImageAsset"
                ? null
