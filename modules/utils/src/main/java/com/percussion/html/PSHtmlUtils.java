...
public class PSHtmlUtils {
  ...
  private static Document.OutputSettings getOutputSettings(Properties props, Charset encoding) {
    ...
    else settings.syntax(Document.OutputSettings.Syntax.html);
  ...
  public static Document createHTMLDocument(
      String fragment, Charset encoding, boolean cleanse, String configFile)
  ...
  public static String cleanseHTMLContent(String fragment, Charset encoding, String configFile) {
    ...
    Properties props;
    if (configFile != null) {
      // Catch any extensions or customizations that may be using the old system default and
      // autowire to the new default.
  ...
  protected static Safelist processAllowedAttributes(
      String fragment, String propVal, Safelist ret, String removeAttrConfigProp) {

    if (propVal == null || StringUtils.isEmpty(propVal.trim())) return ret;

    // Get the list of attributes to remove so that we don't whitelist an attr that shopuld be
    // removed.
  ...
  public static Properties getDefaultCleanerProperties() {
    ...
    try (InputStream is =
        PSHtmlUtils.class.getResourceAsStream("/com/percussion/html/html-cleaner.properties")) {
      props.load(is);
    } catch (IOException e) {
      log.error(
