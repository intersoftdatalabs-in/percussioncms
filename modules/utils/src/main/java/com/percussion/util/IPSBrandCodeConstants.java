  ...
  public enum ServerTypes {
    ...
    public static ServerTypes valueOf(int s) throws IllegalArgumentException {
      ...
      for (int i = 0; i < types.length; i++) {
        if (types[i].getValue() == s) return types[i];
      }
    ...
    public static List<ServerTypes> getSortedValues() {
      ...
      for (ServerTypes serverTypes : values()) sortedTypes.add(serverTypes);

      Collections.sort(
          sortedTypes,
          new Comparator<ServerTypes>() {

            public int compare(ServerTypes t1, ServerTypes t2) {
              return t1.mi_name.toLowerCase().compareTo(t2.mi_name.toLowerCase());
            }
          });
