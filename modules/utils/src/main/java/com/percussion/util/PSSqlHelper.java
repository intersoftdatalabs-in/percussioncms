...
public class PSSqlHelper {
  ...
  private static final String JBOSS_WRAPPED_CLASS =
  ...
  public static String qualifyTableName(String tableName) throws SQLException {
    ...
    return qualifyTableName(
        tableName, detail.getDatabase(), detail.getOrigin(), detail.getDriver());
  }
   ...
   */
  public static String qualifyTableName(String table, String db, String owner, String driver) {
    ...
    if (driver.equalsIgnoreCase(PSJdbcUtils.DB2)
      ...
      if ((owner == null) || (owner.trim().length() == 0)) return table;
      return owner + "." + table;
    }
    ...
    if (driver.equals(PSJdbcUtils.ODBC)
      ...
      if (db == null || db.length() == 0) {
        if (owner == null || owner.length() == 0) return table;
        else return owner + "." + table;
      } else {
        if (owner == null || owner.trim().length() == 0) return db + ".." + table;
        else return db + "." + owner + "." + table;
      }
    } else if (driver.startsWith(PSJdbcUtils.ORACLE_PRIMARY)) {

      if (owner == null || owner.trim().length() == 0) {
        if (db == null || db.trim().length() == 0) return table;
        else if (db.startsWith("@")) return table + db;
        else return db + "." + table;
      } else {
        if (db == null || db.trim().length() == 0) return owner + "." + table;
        else if (db.startsWith("@")) return owner + "." + table + db;
        else return owner + "." + db + "." + table;
      }
  ...
  public static String qualifyViewName(String view, String db, String owner, String driver) {
    ...
    if (driver.equalsIgnoreCase(PSJdbcUtils.DB2)
        || driver.equalsIgnoreCase(PSJdbcUtils.DERBY_DRIVER)) {
      // formats owner.table or table
      if ((owner == null) || (owner.trim().length() == 0)) return view;
      return owner + "." + view;
    }
    ...
    if (driver.equals(PSJdbcUtils.ODBC)
      ...
      if (owner == null || owner.length() == 0) return view;
      else return owner + "." + view;
    } else if (driver.startsWith(PSJdbcUtils.ORACLE_PRIMARY)) {

      if (owner == null || owner.trim().length() == 0) {
        if (db == null || db.trim().length() == 0) return view;
        else if (db.startsWith("@")) return view + db;
        else return db + "." + view;
      } else {
        if (db == null || db.trim().length() == 0) return owner + "." + view;
        else if (db.startsWith("@")) return owner + "." + view + db;
        else return owner + "." + db + "." + view;
      }
    }
   ...
   * @throws IllegalArgumentException for any illegal argument
   */
  public static String qualifyPrimaryKeyName(String key, String db, String owner, String driver) {
    if (driver == null || driver.length() == 0)
      throw new IllegalArgumentException("we need a valid driver type");
    ...
    if (driver.equalsIgnoreCase(PSJdbcUtils.DB2)) return key;
  ...
  public static String parseTableName(
    ...
    if (null == tableName || tableName.trim().length() == 0) return "";
    ...
    if (driver.equals(PSJdbcUtils.DB2) || driver.equals(PSJdbcUtils.DERBY_DRIVER)) {
      // format: db.table or table
      if (hasDot) {
        if (null != catalogBuf) catalogBuf.append(tableName.substring(0, dotPos));
      ...
      if (atPos > 0) {
        if (null != catalogBuf) catalogBuf.append(tableName.substring(atPos + 1));
        tableName = tableName.substring(0, atPos);
      }
      if (hasDot) {
        if (null != originBuf) originBuf.append(tableName.substring(0, dotPos));
        tableName = tableName.substring(dotPos + 1);
      }
    } else {
      ...
      if (hasDot) {
        ...
        if (null != originBuf) originBuf.append(tableName.substring(0, lastDotPos));
  ...
  public static void setDataFromString(
    ...
    switch (dataType) {
      ...
      case Types.BIT:
        value = value.trim();
        stmt.setBoolean(bindStart, (value.equals("1") || value.equalsIgnoreCase("true")));
        break;
      ...
      case Types.DATE:
        ...
        try {
          ...
          if (day != null) date = new Date(day.getTime());
        } catch (Exception e) {
        }

        if (date != null) stmt.setDate(bindStart, date);
      ...
      case Types.TIME:
        ...
        if (time != null) stmt.setTime(bindStart, time);
        else // finally, try for a driver string convert
        stmt.setObject(bindStart, value, dataType);
        break;
      ...
      case Types.TIMESTAMP:
        ...
        if (timestamp == null) { // next pass use Java Date
          try {
            ...
            if (stampDay != null) timestamp = new Timestamp(stampDay.getTime());
          } catch (Exception e) {
          }
        }
      ...
      case Types.CLOB:
        ...
        try {
          if (getOracleStatement(stmt) != null) isOracleStmt = true;
        ...
        if ((isOracleStmt) || (value.length() > 1800)) {
          stmt.setCharacterStream(bindStart, new StringReader(value), value.length());
        } else {
  ...
  public static void setDataFromByteArray(
    ...
    switch (dataType) {
      ...
      case Types.BLOB:
        ...
        if (value.length == 0) setNullParameter(stmt, bindStart, dataType);
        else {
  ...
  public static InputStream setDataFromFile(
      PreparedStatement stmt, int bindStart, File value, int dataType, String charset)
      throws SQLException {
    ...
    try {
      ...
      if (fileLength == 0) {
        ...
        buf = new BufferedInputStream(new FileInputStream(value));
        ...
        switch (dataType) {
          ...
          case Types.CLOB:
            ...
            if (charset == null || charset.length() == 0) {
              ...
              reader = new InputStreamReader(buf, PSCharSets.getJavaName(charset));
            }
  ...
  public static PreparedStatement getOracleStatement(PreparedStatement stmt) throws SQLException {
    ...
    if (stmt.getClass().getName().equals(JBOSS_WRAPPED_CLASS)) {
      ...
      try {
        ...
        if (PreparedStatement.class.isInstance(s)) stmt = (PreparedStatement) s;
    ...
    return stmt.getClass().getName().equals("oracle.jdbc.OraclePreparedStatement") ? stmt : null;
  }
  ...
  public static void setDataFromNumber(
    ...
    switch (dataType) {
      ...
      case Types.DECIMAL:
        {
          BigDecimal num = null;
          if (value instanceof BigDecimal) num = (BigDecimal) value;
          else if (value instanceof BigInteger) num = new BigDecimal((BigInteger) value);
          else num = new BigDecimal(value.doubleValue());
      ...
      case Types.TIMESTAMP:
        // this doesn't make much sense, but not much else we can do
        Timestamp timestamp = new Timestamp(value.longValue());
   ...
   * </code> or emtpy.
   */
  public static short convertNativeDataType(String dbmsName, String typeName, short jdbcType) {
    if (dbmsName == null || dbmsName.trim().length() == 0)
      throw new IllegalArgumentException("dbmsName may not be null or empty");
    ...
    if (dbmsName.equals("Microsoft SQL Server")) {
      if (sqlLowerType.equals("nvarchar")) { // this is really a JDBC varchar
        jdbcType = Types.VARCHAR;
      } else if (sqlLowerType.equals("nchar")) { // this is really a JDBC char
        jdbcType = Types.CHAR;
      } else if (sqlLowerType.equals("ntext")) { // this is really a JDBC CLOB
        jdbcType = Types.CLOB;
      } else if (sqlLowerType.equals("image")) { // this is really a JDBC BLOB
        jdbcType = Types.BLOB;
      } else if (sqlLowerType.equals("sysname")) { // this is really a JDBC varchar
        jdbcType = Types.VARCHAR;
      }
    }
     ...
     */
      ...
      if (sqlLowerType.equals("blob")) jdbcType = Types.BLOB;
      else if (sqlLowerType.equals("clob")) jdbcType = Types.CLOB;
      else if (sqlLowerType.equals("float")) jdbcType = Types.FLOAT;
    }
  ...
  public static short convertNativeDataType(short jdbcType, String nativeType, String driver) {
    if ((driver == null) || (driver.trim().length() == 0))
      throw new IllegalArgumentException("DBMS name and JDBC Driver both may not be null or empty");

    if ((nativeType == null) || (nativeType.trim().length() == 0))
      throw new IllegalArgumentException("native data type may not be null or empty");
    ...
    if (driver.equals(PSJdbcUtils.SPRINTA)) isSprinta = true;

    if (driver.equals(PSJdbcUtils.JTDS_DRIVER)) isJTdsDriver = true;

    if (driver.equals(PSJdbcUtils.MICROSOFT_DRIVER)) isMSDriver = true;

    if (driver.equals(PSJdbcUtils.ORACLE)) isOracle = true;

    String sqlLowerType = nativeType.toLowerCase();

    if (isSprinta || isMSDriver || isJTdsDriver) {
      if (sqlLowerType.equals("nvarchar")) jdbcType = Types.VARCHAR;
      else if (sqlLowerType.equals("nchar")) jdbcType = Types.CHAR;
      else if (sqlLowerType.equals("ntext")) jdbcType = Types.CLOB;
      else if (sqlLowerType.equals("image")) jdbcType = Types.BLOB;
      else if (sqlLowerType.equals("sysname")) jdbcType = Types.VARCHAR;
    } else if (isOracle) {
      if (sqlLowerType.equals("blob")) jdbcType = Types.BLOB;
      else if (sqlLowerType.equals("clob")) jdbcType = Types.CLOB;
      else if (sqlLowerType.equals("float")) jdbcType = Types.FLOAT;
      ...
      else if (sqlLowerType.equals("date") && jdbcType == Types.DATE) jdbcType = Types.TIMESTAMP;
    }
  ...
  public static String getNullColumnSpecifier(String driver) {
    ...
    if (driver.equals(PSJdbcUtils.DB2)
        ...
        || driver.equals(PSJdbcUtils.ORACLE)) {
      nullClause = "";
    } else {
      nullClause = " NULL";
    }
  ...
  public static int getMaxConstraintNameLength(String driver) {
    ...
    if (driver.equals(PSJdbcUtils.DB2)) maxLength = 18;
    else if (driver.startsWith(PSJdbcUtils.ORACLE_PRIMARY)) maxLength = 30;
    else if (driver.equals(PSJdbcUtils.SPRINTA)) maxLength = 128;
    else if (driver.equals(PSJdbcUtils.DERBY)) maxLength = 128;
  ...
  public static boolean supportsIdentityColumns(String driver) {
    ...
    if (driver.startsWith(PSJdbcUtils.ORACLE_PRIMARY) || driver.contains(PSJdbcUtils.MYSQL_DRIVER))
      supported = false;
  ...
  public static String getMetaDataQuery(
    ...
    if (driver.equalsIgnoreCase(PSJdbcUtils.DB2)) {
      if (isMetaQuery && isView) {
        ...
        if (isView) query = "SELECT 1 FROM SYSCAT.VIEWS WHERE VIEWNAME = '";
        else query = "SELECT 1 FROM SYSCAT.TABLES WHERE TABNAME = '";

        query += objectName;
        query += "'";

        if (!((schema == null) || (schema.trim().length() == 0))) {
          if (isView & !isMetaQuery) query += " AND VIEWSCHEMA = '";
          else query += " AND TABSCHEMA = '";
          query += schema.trim();
          query += "'";
        }
      }
  ...
  public static void setNullParameter(PreparedStatement stmt, int index, int jdbcDataType)
    ...
    String driverName = stmt.getConnection().getMetaData().getDatabaseProductName();
    boolean isASA =
        driver.equalsIgnoreCase(PSJdbcUtils.ODBC) && driverName.equals("Adaptive Server Anywhere");
    boolean isSQLServer =
        driver.equalsIgnoreCase(PSJdbcUtils.MSSQL)
    ...
    if (isASA || isSQLServer || isJTDS) {
      if (jdbcDataType == Types.CLOB) jdbcDataType = Types.LONGVARCHAR;
      else if (jdbcDataType == Types.BLOB) jdbcDataType = Types.LONGVARBINARY;
    }
  ...
  public static String getParameterMarker(String driver, String nativeType, String size) {
    ...
    if (driver.equalsIgnoreCase(PSJdbcUtils.DB2)) {
      if ((nativeType == null) || (nativeType.trim().length() < 1))
        throw new IllegalArgumentException("nativeType may not be null or empty");

      marker = "CAST(? AS " + nativeType;
      if ((size != null) && (size.trim().length() > 0)) marker += "(" + size.trim() + ")";
      marker += ")";
    }
    return marker;
  }
  ...
  public static String getDropIndexStatement(
      String driver,
      String table,
      @SuppressWarnings("unused") String db,
      String owner,
      String indexName) {
    if ((driver == null) || (driver.length() == 0))
      throw new IllegalArgumentException("driver may not be null or empty");
    ...
    if ((indexName == null) || (indexName.length() == 0))
      throw new IllegalArgumentException("indexName may not be null or empty");

    log.info("DropIndexStatement Driver : " + driver);

    String sql = "DROP INDEX ";

    if ((owner != null) && (owner.trim().length() > 0)) sql += owner + ".";
    if (driver.equals(PSJdbcUtils.SPRINTA)
        || driver.equals(PSJdbcUtils.MICROSOFT_DRIVER)
        || driver.equals(PSJdbcUtils.JTDS_DRIVER)) sql += table + ".";
  ...
  public static String getCreateOracleTypeSql(String typeName, String dataType, String schema) {
    ...
    if ((dataType == null) || (dataType.trim().length() < 1))
      throw new IllegalArgumentException("dataType may not be null or empty");

    String sql = "CREATE OR REPLACE TYPE ";

    if ((schema != null) && (schema.trim().length() > 0)) sql = sql + schema.trim() + ".";
  ...
  public static Properties makeConnectProperties(
      String url, String database, String userid, String password) {
  ...
  public static boolean isMsSql(String driverName) {
    return (driverName.toUpperCase().contains("MSSQL")
        || driverName.toUpperCase().contains("SQLSERVER")
        || driverName.toUpperCase().contains("JTDS"));
  }
  ...
  public static boolean handleBackingIndex(StringBuilder indexNameBuf, DatabaseMetaData md)
    ...
    if (StringUtils.isBlank(indexName)) {
      throw new IllegalArgumentException("indexName may not be empty, blanks, or null");
    }
    if (md == null) {
  ...
  public static boolean supportsUnicode(Connection conn, PSConnectionDetail connDetail)
    ...
    String qualifiedTableName =
        qualifyTableName(
            "RXDUAL", connDetail.getDatabase(), connDetail.getOrigin(), connDetail.getDriver());
  ...
  public static boolean supportsUnicode(Connection conn, String table, String column)
    ...
    try {
      ...
      for (char ch = 0x21; ch < 0x017F; ch += 15) {
        ...
        rs = null;
        st.close();
        if (db == null || !db.equals(input)) return false;
      }
      return true;
    } catch (SQLException e) {
      if (rs != null) rs.close();
   ...
   * @return 1 if conn is greater than version, -1 if conn is less than version, and 0 if equal
   * @throws SQLException if a database error occurs, the SQL state is returned in the exception
   * @throws NumberFormatException if the version format is invalid, e.g., non-numeric characters
   */
  public static int compareVersions(Connection conn, String version) throws SQLException {
    ...
    int result = 0;
    if (connVersionInt > minVersionInt) result = 1;
    if (connVersionInt < minVersionInt) result = -1;
  ...
  public static String parseDBVersion(String metaVer) {
    ...
    for (int i = 0; i < (cVer.length - 1); i++) {
      if (((Character.isSpaceChar(cVer[i])) || (Character.isWhitespace(cVer[i])))
  ...
  public static boolean isOracle() {
    if (ms_isOracle != null) return ms_isOracle;
    ...
    try {
      ...
      log.error(
          "Failed to determine database type, Error: {}", PSExceptionUtils.getMessageForLog(e));
      log.debug(PSExceptionUtils.getDebugMessageForLog(e));
  ...
  public static boolean isExistingCMSTableName(Connection conn, String t, boolean refresh) {
    ...
    if (existingCMStables.size() == 0 || refresh) {
      try {
        ...
        try (ResultSet rs =
            conn.getMetaData().getTables(conn.getCatalog(), conn.getSchema(), "%", types)) {
          while (rs.next()) {
