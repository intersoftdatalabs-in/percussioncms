...
public class PSDataTypeConverter {
  ...
  public static byte[] getBinaryFromString(String value) throws IOException {
    ...
    if (isHexData(value)) return getBinaryFromHex(value);
    else return getBinaryFromBase64(value);
  }
  ...
  public static byte[] getBinaryFromHex(String value) {
    ...
    for (int i = 0; i < size; ) {
      ...
      if ((c1 - '0') < 10) b = (byte) (c1 - '0');
      else if ((c1 - 'a') < 6) b = (byte) (10 + (c1 - 'a'));
      else b = (byte) (10 + (c1 - 'A'));
      b <<= 4; // shift over to the high nibble

      if ((c2 - '0') < 10) b += (byte) (c2 - '0');
      else if ((c2 - 'a') < 6) b += (byte) (10 + (c2 - 'a'));
      else b += (byte) (10 + (c2 - 'A'));
  ...
  public static String transformDateString(
    ...
    if (value instanceof Date || value instanceof Timestamp) parsedDate = (Date) value;
      ...
      try {
        ...
        if (throwEx) {
          throw new ParseException(
              e.getLocalizedMessage() + ": expected " + inputFormat, e.getErrorOffset());
        }
        parsedDate = null;
      }
    }
    ...
    if (StringUtils.isBlank(outputFormat)) {
      ...
      if (IntStream.of(Calendar.HOUR_OF_DAY, Calendar.MINUTE, Calendar.SECOND, Calendar.MILLISECOND)
          .allMatch(i -> cal.get(i) == 0)) {
        outputFormat = "yyyy-MM-dd";
      } else outputFormat = "yyyy-MM-dd HH:mm:ss.SSS";
    }
  ...
  public static Date parseStringToDate(String myText, StringBuilder patternUsed, Locale locale) {
    if (myText == null || myText.trim().length() < 1)
      throw new IllegalArgumentException("Date string to parse may not be null or empty");

    String[] datePatternArray =
        isYearFirstPattern(myText) ? ms_datePatternArray1 : ms_datePatternArray2;
    FastDateFormat dateFormat = null;
    Date day = null;
    for (String aDatePatternArray : datePatternArray) {
      if (locale == null) dateFormat = FastDateFormat.getInstance(aDatePatternArray);
      else
        /*
      ...
      try {
        ...
        if (patternUsed != null) patternUsed.append(aDatePatternArray);
        break;
    ...
    if (day == null) {
      ...
      try {
        day = dateFormat.parse(myText);
        if (patternUsed != null) patternUsed.append(dateFormat.getPattern());
      } catch (ParseException e) {
        day = null;
      }
    }
