...
public class PSBaseHttpUtils {
  ...
  public static String parseContentType(String contentType, Map<String, String> params) {
    ...
    if (semiPos < 0) semiPos = cType.indexOf(',');
    if (semiPos < 0) semiPos = cTypeLen;
  ...
  protected static int parseHttpParamsString(String paramStr, Map<String, String> params) {
    ...
    while (semiPos >= 0 && semiPos < strLen) {
      if (semiPos == 0) semiPos = -1; // special case for first param

      int nextSemiPos = str.indexOf(';', semiPos + 1);
      if (nextSemiPos < 0) nextSemiPos = strLen;
      ...
      if (param.length() < 3) {
        throw new IllegalArgumentException(
            "HTTP param length too short : " + param + " for string" + str);
      }

      int eqPos = param.indexOf('=');
      if (eqPos < 2 || eqPos == (param.length() - 1)) {
        throw new IllegalArgumentException(
            "HTTP param invalid equals position for param : " + param + " for string" + str);
      }
      ...
      while (start == '"' && end != '"') {
        ...
        if (quotePos < 0) {
          throw new IllegalArgumentException(
              "HTTP cannot find next quote in : " + str + " from position " + nextSemiPos + 1);
        }
  ...
  public static String constructContentTypeHeader(String mimeType, String encoding) {
    ...
    if (null == mimeType || 0 == mimeType.length()) contentHeader = "";
    else {
      if (null == encoding || 0 == encoding.length()) contentHeader = mimeType;
      else contentHeader = mimeType + "; charset=" + encoding;
    }
  ...
  public static Map<String, Object> parseQueryParamsString(
    ...
    try {
      ...
      if (queryOffset >= 0) query = query.substring(queryOffset + 1);
      ...
      while (toker1.hasMoreTokens()) {
        ...
        if (toker2.hasMoreTokens()) {
          ...
          throw new RuntimeException("Invalid query string: missing param name");
        }
        if (toker2.hasMoreTokens()) value = toker2.nextToken().trim();
        if (toker2.hasMoreTokens()) {
          throw new RuntimeException("Invalid query string: invalid name/value pair: " + pair);
        }

        if (StringUtils.isBlank(name)) {
          throw new RuntimeException("Invalid query string: missing name" + pair);
        }
        if (urlDecode) name = URLDecoder.decode(name, "UTF8");
        if (lowerCaseNames) name = name.toLowerCase();
        if (StringUtils.isBlank(value)) value = StringUtils.EMPTY;
  ...
  public static String removeQueryParam(String url, String paramName) {
    ...
    if (!params.containsKey(paramName.toLowerCase())) return url;
    ...
    String result =
        PSBaseHttpUtils.addQueryParams(PSBaseHttpUtils.parseHttpPath(url), params, false);
  ...
  private static void appendKeyValuePair(
      StringBuilder result, String param, String value, boolean urlEncode) {
    try {
      if (StringUtils.isBlank(param)) return;
      if (urlEncode) param = URLEncoder.encode(param, "UTF-8");
      result.append(param);
      result.append('=');
      if (value != null) {
        if (urlEncode) value = URLEncoder.encode(value, "UTF-8");
        result.append(value);
      }
    } catch (UnsupportedEncodingException e) {
