...
public class PSConcurrentListIterator<T> implements ListIterator<T> {
   ...
   *     {@code remove} or {@code add} have been called after the last call to {@code next} or
   *     {@code previous}
   */
  @Override
  public void remove() {
   ...
   *
   * @param t the element with which to replace the last element returned by {@code next} or {@code
   *     previous}
   ...
   * implicit cursor: a subsequent call to {@code next} would be unaffected, and a subsequent call
   * to {@code previous} would return the new element. (This call increases by one the value that
   * would be returned by a call to {@code nextIndex} or {@code previousIndex}.)
   *
