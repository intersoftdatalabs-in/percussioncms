...
public class PSStringTemplate {
  ...
  private String expand(IPSTemplateDictionary dict, int i) throws PSStringTemplateException {
    ...
    if (StringUtils.isBlank(m_template)) return rval.toString();
    ...
    while (pos < length) {
      ...
      if (ch == m_quoteCharacter) {
        ...
        pos++;
        if (pos >= length) {
          throw new PSStringTemplateException("Found quote character at end of template");
        }
  ...
  private int handleVariable(
    ...
    if (end < 0) {
      if (!m_ignoreUnmatchedSequence) {
        throw new PSStringTemplateException(
            "No end sequence for variable reference starting at: " + template.substring(pos));
      }
    ...
    if (var.startsWith("escape ")) {
      ...
      } else if ("mysql".equalsIgnoreCase(dict.lookup("driver"))
          && "schema".equals(var)
          && (val == null || "".equals(val))) {
        val = dict.lookup("db");
        if (val != null) {
