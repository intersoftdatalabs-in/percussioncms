...
public class PSPurgableTempFile extends File implements AutoCloseable {
  ...
  public PSPurgableTempFile(
      String prefix, String suffix, File dir, String sourceFile, String contentType, String encType)
      throws IOException {
    ...
    try {
      if (contentType != null)
        m_contentType = PSBaseHttpUtils.parseContentType(contentType, m_contentProperties);
   ...
   * @throws IOException
   */
  private static String calculatePath(String prefix, String suffix, File dir) throws IOException {
    if (dir == null) {
      ...
      synchronized (makeTempLock) {
        try {
          ...
          throw new IllegalArgumentException(
              "Cannot create tempfile in dir " + dir.getCanonicalPath(), ex);
        }
      }
    }
  ...
  public void release() {
    ...
    if (!m_isDeleted) {
      try {
        ...
        if (!m_isDeleted)
          log.debug(
              "Could not delete tempfile {}", this.getAbsolutePath(), new Throwable("STACKTRACE"));
  ...
  static {
    try {
      ...
      ms_psxTempDirectory =
          new File(temp.getParentFile(), "rxtemp" + "." + System.getProperty("user.name"));
      createTempFileDir();
