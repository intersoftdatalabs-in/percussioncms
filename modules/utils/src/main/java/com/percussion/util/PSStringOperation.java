...
public class PSStringOperation {
  ...
  public static String replace(String strSrc, String strSub, String strRep) {
    ...
    if ((strSub == null) || (strRep == null)) return strSrc;
    ...
    while ((index = text.indexOf(oldChr)) != -1) {
      ...
      if (index + 1 < length) {
        if (text.charAt(index + 1) == oldChr) // means it is escaped
        {
          ...
          if (index + 2 < length) text = text.substring(index + 2);
          else text = "";
        } else {
  ...
  public static String replace(String text, char oldChr, char newChr) {
    ...
    for (int i = 0; i < text.length(); i++) {
      ...
      if (ch == oldChr) newtext.append(newChr);
      else if (ch == newChr) {
  ...
  public static String append(List<String> values, String separator) {
    ...
    if (buffer.length() > 0) return buffer.substring(0, buffer.length() - 1);
  ...
  public static String toProperCase(String strVal) {
    ...
    while ((nLoc = strRight.indexOf(strSep)) != -1) {
      ...
      if (strRes.length() > 0) strRes += strSep + (strL + strR);
      else strRes = strL + strR;
    }
  ...
  public static String dateFormat(String strFormat, Date oneDate, Locale outputLocale) {
    if (oneDate == null) {
      throw new IllegalArgumentException("Date to format must not be null or empty");
    }
    ...
    if (strFormat == null || strFormat.trim().length() < 1) {
      formatter =
          FastDateFormat.getDateTimeInstance(DateFormat.DEFAULT, DateFormat.DEFAULT, locale);
    } else {
      /*
