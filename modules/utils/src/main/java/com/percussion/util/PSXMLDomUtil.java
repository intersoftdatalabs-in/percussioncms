...
public class PSXMLDomUtil {
  ...
  public static void checkNode(Element el, String name) throws PSUnknownNodeTypeException {
    ...
    String nodeName = (el.getNamespaceURI() != null) ? el.getLocalName() : el.getNodeName();
  ...
  public static int checkAttributeInt(Element el, String name, boolean required)
    ...
    return (val == null || val.trim().length() == 0) ? -1 : Integer.parseInt(val);
  }
  ...
  public static boolean checkAttributeBool(
    ...
    if (val.length() > 0) {
      if (yesValue == null || yesValue.trim().length() == 0) yesValue = "yes";
  ...
  public static long checkAttributeLong(Element el, String name, boolean required)
    ...
    return (val == null || val.trim().length() == 0) ? -1 : Long.parseLong(val);
  }
   ...
   */
  public static Date checkAttributeDate(Element el, String name, boolean required)
      throws PSUnknownNodeTypeException, NumberFormatException {
    if (el == null || name == null || name.trim().length() == 0)
      throw new IllegalArgumentException("arguments must not be null or empty");
  ...
  public static String makeXmlName(String str) {
    if (str == null) throw new IllegalArgumentException("makeXmlName, null is not allowed");
  ...
  public static boolean hasStyleSheetPiInDoc(Document doc) {
    ...
    NodeList nl = doc.getChildNodes();
    for (int i = 0; i < nl.getLength(); i++) {
      if (isNodeStyleSheetProcessingInstruction(nl.item(i))) return true;
    }
    return false;
  }
