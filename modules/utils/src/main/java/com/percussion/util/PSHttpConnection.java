...
public class PSHttpConnection {
  ...
  public String postData(URL url, Map<String, ?> paramMap) throws IOException, PSException {
    ...
    if (paramMap.isEmpty()) {
      ...
      return postDataWithPSSessionId(
          url,
          data.toString(),
  ...
  public String postXmlData(URL url, String XmlDocString) throws IOException, PSException {
    ...
    String prefix =
        "Content-Disposition: form-data; name=\"inputdoc\"; "
    ...
    return postDataWithPSSessionId(
        url,
        data.toString(),
        "multipart/form-data; charset=" + StandardCharsets.UTF_8.name() + "; boundary=" + boundary);
  }
  ...
  public Document postData(URL url, Document doc) throws IOException, PSException, SAXException {
    ...
    String resp =
        postDataWithPSSessionId(
            url, postData, "text/xml; charset=" + StandardCharsets.UTF_8.name());

    return PSXmlDocumentBuilder.createXmlDocument(new StringReader(resp), false);
  }
  ...
  private static String postData(URL url, String data, String sendContentType)
    ...
    try {
      ...
      try (OutputStream out = connection.getOutputStream()) {
        ...
        int contentLength = (sContentLength != null) ? Integer.parseInt(sContentLength) : -1;
        long readData;
  ...
  public URL appendPsSessionId(URL url) {
    ...
    if (sUrl.indexOf("pssessionid=") != -1) return url;

    if (sUrl.indexOf('?') == -1) sUrl = sUrl + "?pssessionid=" + getPsSessionId();
