 ...
 * will construct the XML document out of the response from the server.
 */
public interface IPSRemoteRequester {
  /**
   * Makes an http/s request to the specified resource, providing the key- value pairs in the params
   * map as html parameters. Expects that an xml document will be returned.
   *
   * @param resource Never <code>null</code> or empty. Must be a full path to the target resource
   *     without the root path, e.g. app/res.xml. (assume the full path including the root is,
