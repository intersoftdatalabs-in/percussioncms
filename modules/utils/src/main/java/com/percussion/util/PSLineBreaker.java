...
public class PSLineBreaker {
  ...
  public static String wrapString(String line, int maxLen, int threshold, String newLine) {
    ...
    if (null == newLine || newLine.length() == 0) newLine = "\r\n";
    ...
    space than needed.*/
    StringBuilder buf =
        new StringBuilder(line.length() + (line.length() / maxLen + 10) * newLine.length());

    while (breaker.hasNext()) {
      buf.append(breaker.next());
      if (breaker.hasNext()) buf.append(newLine);
    }
  ...
  public PSLineBreaker(String line, int maxLen, int threshold, String newLine) {
    ...
    if (maxLen <= 0) throw new IllegalArgumentException("maxLen must be greater than zero");
    ...
    while (line != null && (lineLen = line.length()) > 0) {
      ...
      if (lineLen <= maxLen) {
        ...
        */
        ...
        do {
          ...
          if (tmpNewLineInd >= 0) newLineInd = tmpNewLineInd; // remember new line index
        ...
        if (newLineInd >= 0) {
          ...
          if (spaceInd >= 0 && dashInd < 0 && (end - spaceInd) <= threshold) {
            ...
            if ((end - maxInd) <= threshold) end = maxInd + 1;
          }
        }
      }
      ...
      if (newLineInd >= 0) nextBeginIndex = substrLen + newLineLen; // skip new line
      else nextBeginIndex = substrLen;

      if (nextBeginIndex >= lineLen) break; // make sure we never go out of bounds
  ...
  public static void main(String[] args) {
    ...
    while (breaker.hasNext()) log.info("\"" + breaker.next() + "\"");
    log.info(" ");
