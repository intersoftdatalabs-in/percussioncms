...
/**
 * The PSURLEncoder class is a utility class which supports encoding URLs. The process of encoding
 * URLs involves take special characters and converting them to an escaped representation. The
 * escape syntax is %HH where HH is the hex representation of the byte. This even supports unicode
 * as the unicode character may end up being two or three escaped chars.
 *
...
public class PSURLEncoder {
  /**
   * This is used as a flag to indicate to encode the PATH part of a URL. This encoding process will
   * replace space characters (' ') to its hex representation ("%20").
   */
  private static final int SCHEME_PATH = 1;

  /**
   * This is used as a flag to indicate to encode the query string of a URL This encoding process
   * will replace space characters (' ') to plus characters ('+').
   */
   ...
   * @return The encoded string, with all reserved characters escaped. Unreserved characters will
   *     not be escaped.
   * @throws IllegalArgumentException if the provided string is <code>null</code>.
   */
   ...
   * See RFC 2396 for a more complete definition of the URI standard.
   *
   * <p>This is what happens:
   *
   * <ul>
   *   <li>The ASCII characters 'a' through 'z', 'A' through 'Z', and '0' through '9' remain the
   *       same.
   *   <li>The space character ' ' is converted into a plus sign '+'.
   *   <li>Unreserverd characters ("-", "_", ".", "!", "~", "*", "'", "(", ")") are not encoded.
   *   <li>All other ASCII characters are converted into the 3-character string "%xy", where xy is
   *       the two-digit hexadecimal representation of the character code
   *   <li>All non-ASCII characters are encoded in two steps: first to a sequence of 2 or 3 bytes,
   *       using the UTF-8 algorithm; secondly each of these bytes is encoded as "%xx".
   * </ul>
   *
   * <p>The algorithm used for converting non-ASCII characters is as follows:
   *
   * <ul>
   ...
   * </ul>
   *
   * The algorithm we use will handle USC-2 characters properly, but will not properly handle USC-4
   * characters nor will it properly handle UCS-2 values between D800 and DFFF, being actually UCS-4
   * characters transformed through UTF-16, which require special treatment. This is very unlikely
   * to be a problem for us at this time (see RFC-2044: "UTF-8, a transformation format of Unicode
   * and ISO 10646" for more info).
   *
  ...
  private static String encode(String data, int scheme) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if ('A' <= ch && ch <= 'Z') {
        ...
        if (SCHEME_PATH == scheme) sbuf.append("%20");
        else sbuf.append('+');
      } else if (ch == '-'
          || ch == '_'
          || ch == '.'
