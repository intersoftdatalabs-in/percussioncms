...
public class PSXmlValidator {
  ...
  public static boolean validateXmlAgainstSchema(
    ...
    if (XmlFile == null || !XmlFile.exists() || !XmlFile.isFile())
      throw new IllegalArgumentException("XmlFile cannot be null and must exist.");
    if (SchemaFile == null || !SchemaFile.exists() || !SchemaFile.isFile())
      throw new IllegalArgumentException("SchemaFile cannot be null and must exist.");
    ...
    try {
      factory.setAttribute(
          "http://java.sun.com/xml/jaxp/properties/schemaSource", SchemaFile.toURL().toString());
