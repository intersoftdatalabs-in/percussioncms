...
public class PSObjectSerializer {
    ...
    try {
      Document doc =
          PSXmlDocumentBuilder.createXmlDocument(
              new InputSource(classRegistryStreamSrc.getResourceAsStream(CLASSREGISTRY_XML)),
              false);
  ...
  public static PSObjectSerializer getInstance() {
    if (serializer == null) serializer = new PSObjectSerializer();
    return serializer;
  }
  ...
  public Object fromXmlString(String xmlString) throws PSObjectSerializerException {
    if (xmlString == null) throw new IllegalArgumentException("xmlString must not be null");

    xmlString = xmlString.trim();

    if (xmlString.length() == 0) throw new IllegalArgumentException("xmlString must not empty");
  ...
  public Element toXml(Object object)
    ...
    ByteArrayInputStream bais =
        new ByteArrayInputStream(toXmlString(object).getBytes(PSCharSetsConstants.rxStdEnc()));
    return PSXmlDocumentBuilder.createXmlDocument(bais, false).getDocumentElement();
  }
