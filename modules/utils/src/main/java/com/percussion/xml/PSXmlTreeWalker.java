...
public class PSXmlTreeWalker implements Serializable {
    ...
    if (cur != null) {
      try {
        if (name.equals(NODENAME_CURRENT)) {
          ...
          if (cur instanceof Element) return getElementData((Element) cur);
          else return null;
        } else if (name.equals(NODENAME_PARENT)) {
          // we want the data from the parent element; otherwise null
          cur = cur.getParentNode();
          if ((cur != null) && (cur instanceof Element)) return getElementData((Element) cur);
          else return null;
          ...
          return getElementData(
              name.substring(RELATIVE_NODENAME_PARENT.length()), false); // use the node we just set
        ...
        if (name.startsWith("@")) {
          ...
          if (childPos != -1) {
            ...
            m_cur = getNextElement(name.substring(0, childPos), GET_NEXT_ALLOW_CHILDREN);
  ...
  public Element getNextElement(String name, int flags) {
    ...
    Node next;
    Node stopNode;
    if (noSiblings) stopNode = m_cur;
    else if (noParents) stopNode = m_cur.getParentNode();
    else stopNode = m_root;

    if (name != null) {
      if (NODENAME_CURRENT.equals(name)) return (Element) m_cur;
    ...
    if (namePos != -1) {
      ...
      if (next != null) {
        ...
        next = getNextElement(name.substring(namePos + 1), GET_NEXT_ALLOW_CHILDREN);
      }
    } else {
       ...
       * siblings.
       */
      int nextRunFlags = flags | GET_NEXT_ALLOW_SIBLINGS | GET_NEXT_ALLOW_PARENTS;

      for (next = getNext(flags, stopNode); next != null; next = getNext(nextRunFlags, stopNode)) {
        String unqualified_name =
            isQualifiedName(name) ? next.getNodeName() : getUnqualifiedNodeName(next);

        if ((next instanceof org.w3c.dom.Element)
            && ((name == null)
                || name.equals(unqualified_name))) { // return the node we found, it's a match
          break;
        }
      }
    }
    ...
    if ((next == null) && resetCur) m_cur = saveCur;
  ...
  public Node getNext(int flags, Node stopNode) {
    ...
    if (next == null) {
      ...
      if ((next == null) && !noParents) {
        for (Node parent = m_cur.getParentNode(); next == null; parent = parent.getParentNode()) {
          if ((parent == stopNode) || (parent == null)) break;
    ...
    try {
      ...
      if (!omitDocumentType && m_doc != null) {
        ...
        if (type != null && type.getPublicId() != null)
          serializer.setOutputProperty(OutputKeys.DOCTYPE_PUBLIC, type.getPublicId());
        if (type != null && type.getSystemId() != null)
          serializer.setOutputProperty(OutputKeys.DOCTYPE_SYSTEM, type.getSystemId());
      }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (i == 0) { // no comparison to do here
        ...
        if (el.startsWith(baseElement)) {
          ...
          if (!baseWasStripped || (el.lastIndexOf('/') > baseElement.length())) {
  ...
  public static String getBaseElement(String currentBase, String xmlField) {
    if (currentBase == null || currentBase.trim().length() == 0) {
      if (xmlField != null) {
        ...
        if (pos != -1) return xmlField.substring(0, pos);
      }

      return xmlField;
    }

    if (xmlField == null || xmlField.trim().length() == 0) return currentBase;

    boolean currentBaseStartsWithDelimiter = currentBase.startsWith(XML_ELEMENT_DELIMITER);
    boolean xmlFieldStartsWithDelimiter = xmlField.startsWith(XML_ELEMENT_DELIMITER);
    if (currentBaseStartsWithDelimiter != xmlFieldStartsWithDelimiter) return null;

    StringTokenizer currentBaseTokenizer = new StringTokenizer(currentBase, XML_ELEMENT_DELIMITER);

    StringTokenizer xmlFieldTokenizer = new StringTokenizer(xmlField, XML_ELEMENT_DELIMITER);
    ...
    while (currentBaseTokenizer.hasMoreTokens()) {
      ...
      if (xmlFieldTokenizer.hasMoreTokens()) {
        ...
        if (currentBaseToken.equals(xmlFieldToken)) {
          if (baseElement == null) baseElement = currentBaseToken;
          else baseElement += XML_ELEMENT_DELIMITER + currentBaseToken;
        } else break;
      } else break;
    }
  ...
  public static String getRelativeFieldName(String base, String xmlField) {
    ...
    if (xmlField.equals(base)) {
      ...
      int len;
      if (base.endsWith("/")) len = base.length();
      else len = base.length() + 1;
      ...
      if (commonBase != null) {
        ...
        if (pos == xmlField.length()) { // the XML field is the same as the base element
          ret = ".";
        } else {
  ...
  private String getDataFromAttribute(Node cur, String name) {
    ...
    if (map != null) {
      ...
      if (attr != null) return ((Attr) attr).getValue();
    }
  ...
  private String getDataFromChildElement(Node cur, String name) {
    ...
    for (cur = cur.getFirstChild(); cur != null; cur = cur.getNextSibling()) {
      if ((ret = getNodeMatchValue(cur, name)) != null) break;
    }
  ...
  private String getNodeMatchValue(Node node, String name) {
    String nodeName = isQualifiedName(name) ? node.getNodeName() : getUnqualifiedNodeName(node);
