...
public class PSErrorManagerDefaultImpl implements IPSErrorManager {
  private static final Logger log = LogManager.getLogger(PSErrorManagerDefaultImpl.class);
  ...
  private static ResourceBundle getErrorStringBundle(Locale loc) throws MissingResourceException {
    ...
    if (strBundle != null) return strBundle;

    strBundle = ResourceBundle.getBundle("com.percussion.error.PSErrorStringBundle", loc);
  ...
  public URL getErrorURL(PSLogInformation error, Locale loc) {
    PSMapClassToObject pageMap = getErrorPageMaps(loc);
    if (pageMap != null) return (URL) pageMap.getMapping(error.getClass());
  ...
  public String getErrorText(int code, boolean nullNotFound, String language) {
    ...
    if (bundle == null) return Integer.toString(code);
