...
public class PSOrganizeProperties {
  ...
  public static void execute(File file) throws IOException {
    Map<String, List<String>> m_comments = new HashMap<>();
   ...
   */
  protected static Properties loadPropsFile(File file, Map<String, List<String>> comments_list)
      throws IOException {
   ...
   * @throws IOException
   */
  private static void parseComments(File file, Map<String, List<String>> comments_list)
      throws IOException {
    try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
      String line = null;
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (!okToParse) {
          if (trimmedLine.startsWith("#") || trimmedLine.length() == 0) {
            if (trimmedLine.startsWith("#### ")) okToParse = true;
          } else okToParse = true;
        } else {
          if (trimmedLine.startsWith("#") || trimmedLine.length() == 0) {
            if (trimmedLine.startsWith("#### ")) comments.clear(); // dump collected comments
            else comments.add(trimmedLine);
   ...
   * @throws IOException upon any error.
   */
  private static void savePropsFile(
      File file, Map<String, String> props, Map<String, List<String>> comments_list)
      throws IOException {
    ...
    for (String key : props.keySet()) {
      ...
      if (!lastPrefix.equals(prefix)) {
        if (lastPrefix.length() > 0) sb.append("\n");
        sb.append("#### ");
        sb.append(prefix);
        sb.append(" ####\n");
        lastPrefix = prefix;
  ...
  private static void addComments(
      StringBuilder sb, String key, Map<String, List<String>> comments) {
