...
public final class PSTomcatConnector extends PSAbstractConnector implements XMLEnabled {
  ...
  public PSTomcatConnector(
      Path connectorFileContext, String scheme, int port, Map<String, String> props) {
  ...
  public PSTomcatConnector(
      Path connectorFileContext, IPSConnector copyFrom, Map<String, String> props) {
    super(connectorFileContext);
    if (props != null) this.properties.putAll(props);
    copyFrom(copyFrom);
  }
  ...
  protected void updateFields() {
    ...
    updateProp(SCHEME_ATTR).ifPresent(this::setScheme);

    updateProp(PORT_ATTR).map(NumberUtils::toInt).ifPresent(this::setPort);

    updateProp(ADDRESS_ATTR).ifPresent(this::setHostAddress);
    ...
    updateProp(PROTOCOL_ATTR).ifPresent(this::setProtocol);
    ...
    updateProp(KEYSTORE_FILE_ATTR).map(getAbsolutePath()).ifPresent(this::setKeystoreFile);

    updateProp(KEYSTORE_PASS_ATTR).ifPresent(this::setKeystorePass);

    updateProp(TRUSTSTORE_FILE_ATTR).map(getAbsolutePath()).ifPresent(this::setTruststoreFile);

    updateProp(TRUSTSTORE_PASS_ATTR).ifPresent(this::setTruststorePass);
  }

  /**
   * Serialize this connector to its XML representation. See {@link #PSTomcatConnector(Path,
   * Element, Map)} more information.
   *
  ...
  protected void updateProperties() {
    ...
    Optional.ofNullable(this.getHostAddress()).ifPresent(setProp(ADDRESS_ATTR));

    Optional.ofNullable(this.getScheme()).ifPresent(setProp(SCHEME_ATTR));

    Optional.ofNullable(this.getKeystoreFile())
        .map(getRelativePathString())
        .ifPresent(setProp(KEYSTORE_FILE_ATTR));

    Optional.ofNullable(this.getKeystorePass()).ifPresent(setProp(KEYSTORE_PASS_ATTR));

    Optional.ofNullable(this.getTruststoreFile())
        .map(getRelativePathString())
        .ifPresent(setProp(TRUSTSTORE_FILE_ATTR));

    Optional.ofNullable(this.getTruststorePass()).ifPresent(setProp(TRUSTSTORE_PASS_ATTR));
  ...
  static final Map<String, String> DEFAULT_HTTP_ATTRIBUTES =
      new HashMap<String, String>() {
        {
          ...
          put(
              COMPRESSABLE_MIME_TYPE_ATTR,
              "text/html,text/xml,text/plain,text/css,text/javascript,application/javascript,application/json,application/xml");
          put(COMPRESSION_MIN_SIZE_ATTR, "256");
        }
      };

  static final Map<String, String> DEFAULT_HTTPS_ATTRIBUTES =
      new HashMap<String, String>() {
        {
          ...
          put(
              CIPHERS_ATTR,
              "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,TLS_RSA_WITH_AES_128_GCM_SHA256,TLS_RSA_WITH_AES_256_GCM_SHA384");
        }
      };
  ...
  public static boolean isHttpConnector(Element connEl) {
    if (connEl == null) throw new IllegalArgumentException("connEl may not be null");
    ...
    if (connEl.getNodeName().equals(CONNECTOR_NODE_NAME)) {
      ...
      isHttpConn =
          (StringUtils.isBlank(protocol) || protocol.toLowerCase().startsWith(SCHEME_HTTP));
    }
  ...
  @Override
  public Document toXml(Map<String, String> properties)
  ...
  @Override
  public void fromXml(Element source, Map<String, String> properties)
      throws IOException, SAXException, PSInvalidXmlException {

    if (source == null) throw new IllegalArgumentException("source may not be null");

    if (!source.getNodeName().equals(CONNECTOR_NODE_NAME))
      throw new IllegalArgumentException("source must be a Connector element");
