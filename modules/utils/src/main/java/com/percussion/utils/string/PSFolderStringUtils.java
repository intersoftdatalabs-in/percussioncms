...
public class PSFolderStringUtils {
   ...
   *
   * <p>This quoting strategy (introduced in T045 / PR #1295) prevents the unvalidated concatenation
   * of user-supplied folder paths into compiled regular expressions, addressing the CodeQL {@code
   * java/regex-injection} cluster (alerts #761, #765, #766, #1041, #1042, #1043). The previous
   * implementation hex-encoded all non-alphanumeric / whitespace / slash characters; that approach
   * was equivalent for matching but obscured the path semantics at debug time.
   *
