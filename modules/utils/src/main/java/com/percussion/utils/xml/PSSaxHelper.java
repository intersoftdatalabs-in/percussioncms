...
public class PSSaxHelper {
   ...
   * handler must implement {@link ContentHandler} and inherit from {@link DefaultHandler} or {@link
   * DefaultHandler2}. If it implements {@link LexicalHandler} then it will be set as the lexical
   * handler on the parser.
   *
   * <p>The first argument to the handler's constructor must be an {@link XMLStreamWriter}, which is
   * used to output the transformed source. The constructor is found by finding the first
   * constructor for the class that matches the length and has a first argument that takes a {@link
   * ContentHandler}.
   *
   * @param source the source document in the form of a string, never <code>null</code> or empty
   * @param handler the content handler (and possibly lexical handler) class, which takes an {@link
   *     XMLStreamWriter} as the first argument in the constructor, never <code>null</code>.
   * @param args further arguments for the constructor, may be <code>null</code>.
  ...
  public static String parseWithXMLWriter(
    ...
    try {
      ...
      InputSource is =
          new InputSource(
              new BOMInputStream(IOUtils.toInputStream(source, StandardCharsets.UTF_8), false));
  ...
  /**
   * Creates a new instance of a SAXParser with the specified handler.
   *
   * @param handler the handler of the parser. It should be an instance of {@link LexicalHandler} if
   *     the handler wants to handle comments and/or CDADA. It may be <code>null</code> if the
   *     handler is unknown.
  ...
  /**
   * Determines if the specified element can be a self closed HTML element or not. Some of the self
   * closed HTML element may cause problems in browser, Ephox or eKtron.
   *
   * @param name the name of the element, may be blank.
   * @return <code>true</code> if the element cannot be self closed HTML element; otherwise it can
   *     be a self closed HTML element.
   */
  ...
  /**
   * Array of empty elements that cause problems in browser, Ephox or eKtron when self (or smart)
   * closed by tidy or document builder. If you find a new one add the tag name to this array.
   */
  public static final ArrayList<String> ELEMENTS_NO_SELF_CLOSE_LIST =
      new ArrayList<>(
          Arrays.asList(
              "a",
              "div",
