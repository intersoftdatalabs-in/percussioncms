...
/**
 * Modern HTTP client wrapper that uses java.net.http.HttpClient.
 * This class provides a migration-friendly interface for replacing 
 * Apache Commons HttpClient usage.
 */
public class PSModernHttpClient {
    
    private static final Logger log = Logger.getLogger(PSModernHttpClient.class.getName());
    ...
    private HttpClient createHttpClient() {
        try {
            ...
            log.warning("Failed to create SSL context, using default: " + e.getMessage());
            return HttpClient.newBuilder()
                .connectTimeout(Duration.ofSeconds(10))
                .followRedirects(HttpClient.Redirect.NORMAL)
                .build();
    ...
    private void validateResponse(HttpResponse<?> response, String url) throws IOException {
        ...
        if (log.isLoggable(java.util.logging.Level.FINE)) {
            log.fine("HTTP " + response.request().method() + " " + url + " Status: " + statusCode);
        }
