...
public class PSDatasourceResolver implements IPSDatasourceResolver {
  ...
  public IPSDatasourceConfig resolveDatasource(IPSConnectionInfo info) {
    ...
    if (info != null && !StringUtils.isBlank(info.getDataSource())) dsName = info.getDataSource();
    ...
    while (dsConfigs.hasNext() && dsConfig == null) {
      ...
      if (test.getName().equalsIgnoreCase(dsName)) dsConfig = test;
    }
  ...
  public void fromXml(Element source) throws PSInvalidXmlException {
    ...
    try {
      ...
      PSSpringBeanUtils.validateBeanRootElement(
          DATASOURCE_RESOLVER_NAME_60, getClassName(), source);
    }
  ...
  public Properties getProperties() {
    ...
    for (IPSDatasourceConfig config : m_configs) {
      ...
      props.put(
          indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_DS_SUFFIX,
          config.getDataSource());
      props.put(
          indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_NAME_SUFFIX, config.getName());
      props.put(
          indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_SCHEMA_SUFFIX,
          config.getOrigin());
      props.put(
          indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_DB_SUFFIX, config.getDatabase());
  ...
  public void setProperties(Properties props) {
    ...
    String pattern =
        IPSJdbcJettyDbmsDefConstants.JETTY_CONN_PREFIX
            + "\\.(\\d+)\\."
    ...
    for (Entry<Object, Object> propEntry : props.entrySet()) {
      ...
      if (m.find()) {
        ...
        String dsName =
            (String) props.get(indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_DS_SUFFIX);
        String origin =
            (String)
                props.get(indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_SCHEMA_SUFFIX);
        String database =
            (String) props.get(indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_DB_SUFFIX);
        String isDefault =
            (String)
                props.get(indexedPrefix + IPSJdbcJettyDbmsDefConstants.JETTY_CONN_DEFAULT_SUFFIX);
