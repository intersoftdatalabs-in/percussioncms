...
public class PSSaxCopier extends DefaultHandler2 {
  ...
  public PSSaxCopier(
    ...
    m_writer = writer;
    if (renames != null) m_elementRenames = renames;
  ...
  public void characters(char[] ch, int start, int length) throws SAXException {
    ...
    try {
      if (m_inCData) m_writer.writeCData(new String(ch, start, length));
      else m_writer.writeCharacters(ch, start, length);
  ...
  public void endElement(String uri, String localName, String qName) throws SAXException {
    try {
      ...
      if ((!canBeSelfClosedElement(qName)) && m_addFillerTextToEmptyElements) {
  ...
  public void error(SAXParseException error) throws SAXException {
    ms_log.error(
        "Problem processing data at line "
            + error.getLineNumber()
            + " at column "
