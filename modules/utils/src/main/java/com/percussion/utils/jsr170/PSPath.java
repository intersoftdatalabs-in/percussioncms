...
public final class PSPath {
  private static class Element {
    ...
    public boolean equals(Object obj) {
      ...
      return mi_index == b.mi_index && mi_name.equals(b.mi_name);
    }
    ...
    for (int i = 0; i < (parts.length - start); i++) {
      ...
      if (bracket > 0) {
        ...
        if (endbracket <= (bracket + 1)) {
          throw new IllegalArgumentException("Bad index specification in path: " + parts[i]);
        }
        String ind = parts[i].substring(bracket + 1, endbracket);
        try {
          e.mi_index = Integer.parseInt(ind);
        } catch (NumberFormatException ex) {
          throw new IllegalArgumentException("Badly formated index: " + ind);
        }
      }
      e.mi_name = name;
      m_elements[i] = e;
  ...
  public boolean equals(Object obj) {
    ...
    PSPath b = (PSPath) obj;
    if (b.m_elements.length != m_elements.length) return false;
    for (int i = 0; i < m_elements.length; i++) {
      if (m_elements[i].equals(b.m_elements[i]) == false) return false;
    }
    return true;
  }
  ...
  public PSPath getAllButLast() {
    ...
    else return new PSPath(this, 0, getCount() - 1);
  }
  ...
  public PSPath getRest() {
    if (getCount() == 0) return this;
    else return new PSPath(this, 1, getCount());
  }
