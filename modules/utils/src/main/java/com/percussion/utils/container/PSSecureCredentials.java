...
public class PSSecureCredentials {
  ...
  public static boolean isMatch(Element node, String secDomain) {
    ...
    if (StringUtils.isBlank(secDomain))
      throw new IllegalArgumentException("secDomain may not be null or empty");
  ...
  public PSSecureCredentials(Element source) throws PSInvalidXmlException {
    ...
    if (!source.getNodeName().equals(APP_POLICY_NODE_NAME))
      throw new IllegalArgumentException("invalid source application policy element");
    ...
    while (moduleOptionEl != null) {
      ...
      if (USERNAME_ATTR_VAL.equals(nameAttr))
        m_userId = PSXmlUtils.getElementData(moduleOptionEl, MODULE_OPTION, true);
      else if (PASSWORD_ATTR_VAL.equals(nameAttr)) {
        foundPassword = true;
        m_password = PSXmlUtils.getElementData(moduleOptionEl, MODULE_OPTION, false);
      } else if (MANAGED_CONN_FACTORY_ATTR_VAL.equals(nameAttr)) {
        String factoryName = PSXmlUtils.getElementData(moduleOptionEl, MODULE_OPTION, true);
      ...
      moduleOptionEl = tree.getNextElement(MODULE_OPTION, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  ...
  public Element toXml(Document doc) {
    ...
    Element authEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, AUTHENTICATION);

    Element loginModuleEl = PSXmlDocumentBuilder.addEmptyElement(doc, authEl, LOGIN_MODULE);
    ...
    Element moduleEl;
    moduleEl = PSXmlDocumentBuilder.addElement(doc, loginModuleEl, MODULE_OPTION, m_userId);
    moduleEl.setAttribute(NAME_ATTR, USERNAME_ATTR_VAL);

    moduleEl = PSXmlDocumentBuilder.addElement(doc, loginModuleEl, MODULE_OPTION, m_password);
    ...
    moduleEl =
        PSXmlDocumentBuilder.addElement(
            ...
            loginModuleEl,
            MODULE_OPTION,
            "jboss.jca:service=LocalTxCM,name=" + m_datasourceName);
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(m_datasourceName, that.m_datasourceName)
        && Objects.equals(m_userId, that.m_userId)
        && Objects.equals(m_password, that.m_password)
        && Objects.equals(m_securityDomain, that.m_securityDomain);
  }
  ...
  private static final String MANAGED_CONN_FACTORY_ATTR_VAL = "managedConnectionFactoryName";
}
