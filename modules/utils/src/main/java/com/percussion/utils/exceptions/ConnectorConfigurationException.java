...
public class ConnectorConfigurationException extends Exception {
   ...
   *     null</code>.
   */
  public ConnectorConfigurationException(int msgCode, Object singleArg) {
   ...
   *     <code>null</code>, and may contain <code>null</code> elements.
   */
  public ConnectorConfigurationException(int msgCode, Object[] arrayArgs) {
    for (int i = 0; arrayArgs != null && i < arrayArgs.length; i++) {
      if (arrayArgs[i] == null) arrayArgs[i] = "";
    }
   ...
   */
  public ConnectorConfigurationException(int msgCode) {
    this(msgCode, null);
  }
   ...
   */
  public ConnectorConfigurationException(PSException ex) {
   ...
   *     by the class.
   */
  public ConnectorConfigurationException(Element source) throws PSUnknownNodeTypeException {
    ...
    if (sTemp != null && sTemp.trim().length() > 0) m_originalExceptionClass = sTemp;

    // get args
    List<String> argList = new ArrayList<String>();
    PSXmlTreeWalker tree = new PSXmlTreeWalker(source);
    Element arg = tree.getNextElement(XML_ELEMENT_ARG, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (arg != null) {
      argList.add(tree.getElementData());
      arg = tree.getNextElement(XML_ELEMENT_ARG, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  ...
  public Element toXml(Document doc) {
    ...
    for (int i = 0; m_args != null && i < m_args.length; i++) {
      if (m_args[i] == null) PSXmlDocumentBuilder.addEmptyElement(doc, root, XML_ELEMENT_ARG);
      else PSXmlDocumentBuilder.addElement(doc, root, XML_ELEMENT_ARG, m_args[i].toString());
    }
  ...
  private String createMessage(int msgCode, Object[] arrayArgs, Locale loc) {
    ...
    String msg = null;
    if (m_originalExceptionClass == null) msg = getErrorText(msgCode, true, loc);
    ...
    if (msg == null) {
      ...
      if (m_originalExceptionClass != null) msg = m_originalExceptionClass + ": ";
      else msg = "";
  ...
  public static String getErrorText(int code, boolean nullNotFound, Locale loc) {
    ...
    try {
      ...
      if (errList != null) return errList.getString(String.valueOf(code));
    ...
    if (ms_bundle == null) {
      ms_bundle =
          ResourceBundle.getBundle(
              "com.percussion.utils.exceptions.PSPortConfigurationResources.properties", loc);
    }
  ...
  public static final String XML_NODE_NAME = "PSXConnectorConfigurationException";
