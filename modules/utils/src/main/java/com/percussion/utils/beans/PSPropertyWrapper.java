...
public class PSPropertyWrapper {
  ...
  private static class Key {
    ...
    public boolean equals(Object b) {
      ...
      Key keyb = (Key) b;
      return eq.append(m_property, keyb.m_property)
          .append(m_class, keyb.m_class)
          .append(m_type, keyb.m_type)
          .isEquals();
    }
  ...
  public Object getPropertyValue(String pname) throws PSPropertyAccessException {
    if (pname == null || pname.trim().length() < 2) {
      throw new IllegalArgumentException("pname may not be null or contain less than 2 characters");
    }
    ...
    try {
      ...
      throw new PSPropertyAccessException("No matching get method found for property " + pname, e);
    }
  }
   ...
   */
  private synchronized Method findMethod(String pname, String type) throws NoSuchMethodException {
    ...
    if (m == null) {
      String str = type + Character.toTitleCase(pname.charAt(0)) + pname.substring(1);
  ...
  public synchronized void init() {
    ...
    if (m_loader == null) {
      if (m_wrappedObject == null) {
        throw new IllegalStateException("Invalid state, wrapped object is null and no loader");
      }
    } else {
  ...
  protected NodeType validateParent(Node parent) throws RepositoryException {
    if (parent == null
      ...
      throw new IllegalStateException("Missing parent, parent definition or nodetype information");
    }
