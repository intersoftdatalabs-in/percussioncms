...
public interface IPSJndiDatasource {
  ...
  /**
   * Set the name of this datasource. Must be unique among all datasources, case-insensitive.
   *
   ...
   *
   * @param password The password, may be <code>null</code> or empty, and must not be encrypted.
   */
   ...
   * {@link PSJndiDatasourceImpl#toXml(Document)} is called (if implemented), and will instead include a reference to the supplied
  ...
  /**
   * Get the minimum number of connections for this datasource. See {@link #setMinConnections(int)}
   * for more info.
   *
  ...
  /**
   * Set the Maximum numer of connections based on this datasource that the pool may hold. Defaults
   * to 100 if not set.
   *
   * @param max The maximum number, must be >= the value returned by {@link #getMinConnections()}
   */
  void setMaxConnections(int max);

  /**
   * Get the maximum number of connections. See {@link #setMaxConnections(int)} for more info.
   *
  ...
  /**
   * Set the the maximum time in minutes a connection may be idle before being closed. Defaults to
   * DEFAULT_IDLE_TIMEOUT if not set.
   *
