...
public class PSParseFragments {
  ...
  public static Map<String, String> parseContent(String raw) {
    ...
    for (String line : lines) {
      ...
      if (trimmedLine.startsWith(COMMENT_PREFIX)) continue;
      if (trimmedLine.startsWith(START_DELIMITER)) {
        String n = trimmedLine.substring(START_DELIMITER.length() + 1).trim();
        if (StringUtils.isBlank(n))
          throw new RuntimeException("Malformed fragment file. Start fragment missing name.");
        name = n.toUpperCase();
  ...
  public static String[] splitByNewlines(String rawText) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line != null && line.length() > 0) {
        if (line.charAt(line.length() - 1) == '\r') lines[i] = line.substring(0, line.length() - 1);
      }
    }
    return lines;
  }
