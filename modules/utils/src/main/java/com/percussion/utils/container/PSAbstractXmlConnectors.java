...
public abstract class PSAbstractXmlConnectors extends PSAbstractConnectors implements XMLEnabled {
  ...
  public Document toXml(Map<String, String> properties)
    ...
    for (IPSConnector connector : getConnectors()) {
      PSAbstractConnector newconnector =
          PSTomcatConnector.getBuilder()
              .setConnectorFileContext(getConnectorFileContext())
              .setCopyFrom(connector)
              .build();
    ...
    int nextFlag = PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS | PSXmlTreeWalker.GET_NEXT_RESET_CURRENT;

    Element testEl = tree.getNextElement(firstFlag);
    while (testEl != null) {
      // copy all but http connectors
      if (!(testEl.getNodeName().equals(PSTomcatConnector.CONNECTOR_NODE_NAME))) {
  ...
  public void fromXml(Element el, Map<String, String> properties)
      throws IOException, SAXException, PSInvalidXmlException {
    ...
    Element connEl = tree.getNextElement(CONNECTOR_NODE_NAME, firstFlag);
    while (connEl != null) {
      PSAbstractConnector newConnector =
      ...
      connEl = tree.getNextElement(CONNECTOR_NODE_NAME, nextFlag);
    }
