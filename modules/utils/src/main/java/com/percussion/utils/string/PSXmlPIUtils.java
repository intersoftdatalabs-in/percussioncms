...
public class PSXmlPIUtils {
  ...
  private static final String[] START_SEQ = {"<%", "<?php"};

  private static final String[] END_SEQ = {"%>", "?>"};

  private static final Action[] ACTION = {Action.QUOTE, Action.PHP};
  ...
  public static PSPair<Map<Integer, PSPair<Action, String>>, String> encodeTags(String input) {
        ...
        if (input.startsWith(START_SEQ[j], i)) {
          ...
          if (l < 0) {
            throw new IllegalStateException(
                "Delimeters did not match, start delim: " + START_SEQ[j]);
          }
    ...
    return new PSPair<Map<Integer, PSPair<Action, String>>, String>(keys, rval.toString());
  }
   ...
   */
  public static void substitutePIs(Document doc, Map<Integer, PSPair<Action, String>> keys) {
    if (doc == null) {
   ...
   */
  private static void doSubstitutePIs(
      Document doc, Node node, Map<Integer, PSPair<Action, String>> keys) {
      ...
      if (n.getNodeType() == Node.COMMENT_NODE) {
        ...
        if (text.startsWith(COMMENT_TAG)) {
          ...
          if (active.getFirst().equals(Action.QUOTE)) {
            pi = doc.createProcessingInstruction("psx-activetag", active.getSecond());

          } else if (active.getFirst().equals(Action.PHP)) {
            String code = stripPI(active.getSecond());
            pi = doc.createProcessingInstruction("php", code);
          } else {
            throw new IllegalStateException("Unknown action " + active.getFirst());
          }
   ...
   * pi, followed by the percent sign percent sign will have the pi stripped if the <% and %> are in the sequences.
   *
   * @param input input data string, never <code>null</code> or empty
  ...
  public static String removePI(String input) {
    ...
    for (int i = 0; i < input.length(); i++) {
      if (input.startsWith(PI_START, i)) {
        ...
        boolean match = false;
        for (int j = 0; j < START_SEQ.length; j++) {
          if (content.startsWith(START_SEQ[j]) && content.endsWith(END_SEQ[j])) {
            match = true;
            break;
          }
        }
