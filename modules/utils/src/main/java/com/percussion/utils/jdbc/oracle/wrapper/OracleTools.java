...
public class OracleTools {
  ...
  public static void setDataFromCollection(
    ...
    if (dataType != Types.ARRAY) throw new IllegalArgumentException("dataType must be Types.ARRAY");

    // has to use the original (Oracle) object to handle to collection data
    OraclePreparedStatement oraStmt =
        (OraclePreparedStatement) PSSqlHelper.getOracleStatement(stmt);
    if (oraStmt == null) {
      throw new IllegalArgumentException(
          "expected OraclePreparedStatement but got " + stmt.getClass().getName());
    }
    ...
    if (coll.isEmpty()) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Collection contains non-numeric object. " + nfe.getLocalizedMessage());
      }

      ArrayDescriptor desc =
          ArrayDescriptor.createDescriptor(NUMBER_TABLE_DESCRIPTOR_NAME, oraStmt.getConnection());
