...
public class PSStringUtils {
  ...
  public static Charset getCharsetFromMimeType(String mimeType) {
    ...
    if (cset < 0) return Charset.forName("UTF-8");
    else {
      int eq = mimeType.indexOf('=', cset);
      if (eq < 0) return Charset.forName("UTF-8");
  ...
  public static String listToString(List<String> input, String delim) {
    ...
    if (delim == null) throw new IllegalArgumentException("delim may not be null");
  ...
  public static String toCamelCase(String src) {
    ...
    String[] words = WordUtils.capitalizeFully(src, new char[] {'_'}).split("_");

    String result = "";
    for (String word : words) {
      if (result.length() == 0) result += WordUtils.uncapitalize(word);
      else result += word;
    }
   ...
   */
  public static String findMatchingLeftSubstring(String leftSubstring, String[] candidates) {
   ...
   */
  public static String abbreviatePath(String path, Dimension dimension, Font font) {
    ...
    if (middle == 0) return ELIPSIS + ppath.subpath(ppath.getCount() - 1);
    else
      return abbreviatePath(
          ppath.subpath(0, middle), ppath.subpath(middle + 1), dimension, font, ctx);
  }
   ...
   * @return the string
   */
  private static String abbreviatePath(
      PSPath startpath, PSPath endpath, Dimension dimension, Font font, FontRenderContext ctx) {
    StringBuilder b = new StringBuilder();
    ...
    if (startpath.getCount() >= endpath.getCount()) {
      ...
      if (startpath.getCount() < 3 /* just slashes */) startpath = null;
      else startpath = startpath.subpath(0, startpath.getCount() - 1);
    } else {
  ...
  public static int indexOfIgnoringCase(String src, String sequence, int start) {
    if (StringUtils.isBlank(sequence))
      throw new IllegalArgumentException("sequence may not be null or empty");
    if (start < 0)
  ...
  public static Character validate(String string, String invalidChars) {
    ...
    for (int i = 0; i < string.length(); i++)
      for (int j = 0; j < invalidChars.length(); j++)
        if (string.charAt(i) == invalidChars.charAt(j)) return invalidChars.charAt(j);
    return null;
  }
  ...
  public static Character validateContentTypeName(String name) {
    ...
    for (int i = 0; i < name.length(); i++) {
      ...
      if (isValidCharForContentTypeName(ch)) continue;
      return ch;
    }
  ...
  public static String makeValidContentTypeName(String name) {
    ...
    if (validateContentTypeName(name) == null) return name;

    StringBuilder nameBuff = new StringBuilder();
    for (int i = 0; i < name.length(); i++) {
      final char ch = name.charAt(i);
      if (isValidCharForContentTypeName(ch)) nameBuff.append(ch);
      else nameBuff.append('_');
    }
  ...
  public static void notBlank(String s, String message) {
    if (StringUtils.isBlank(s)) {
      final String m =
          StringUtils.isBlank(message) ? "The value should not be null, empty, or blank." : message;
  ...
  public static String[] getAllowedGroups(String allowAccessTo) {
    ...
    String allowed =
        StringUtils.replace(
            allowAccessTo,
            ALLOWED_GROUPS_ESCAPE_CHAR + ALLOWED_GROUPS_SPLIT_CHAR,
