...
public class PSRequestInfoBase {
  ...
  public static void initRequestInfo(Map<String, Object> initialData) {
    ...
    if (map != null) throw new IllegalStateException("Must call reset first");
  ...
  public static void setRequestInfo(String key, Object value) {
    ...
    Map<String, Object> map = mapThreadLocal.get();
    if (map == null) throw new IllegalStateException("Must call init first");
    if (value == null) {
      map.remove(key);
      if (key.equals(KEY_PSREQUEST)) map.remove(KEY_PSREQUESTCONTEXT);
  ...
  public static void resetRequestInfo() {
    Map<String, Object> map = mapThreadLocal.get();
    if (map == null) return; // do nothing there is nothing in the map.
