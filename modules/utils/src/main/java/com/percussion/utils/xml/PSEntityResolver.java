...
/**
 * @author dougrand
 *     <p>This entity resolver allows references files in the rhythmyx root directory by looking for
 *     the initial string /Rhythmyx/. It then resolves the rest of the string to a path in the
 *     resolution directory. If the passed reference is not appropriate for this resolver or the
 *     reference is not found, the default action will occur by the {@link #resolveEntity(String,
 *     String)} method returning <code>null</code>.
 * @deprecated
 */
@Deprecated
public class PSEntityResolver implements EntityResolver {
  ...
  /**
   * Debug helper that allows to trace who last set the root, never <code>null</code>. Initialized
   * by {@link #setResolutionHome(File)}.
   */
  ...
  /** Initialized in the {@link #getInstance()} method, never changed afterward. */
  private static PSEntityResolver singleton = null;

  /**
   * The directory to resolve references from. This is the current working directory by default, but
   * can be overridden.
   */
  private static volatile File resolutionHome = new File(System.getProperty("rxdeploydir", "."));

  /**
   * Map of public ids to local resource files containing the DTD, never <code>null</code>, may be
   * empty.
   */
  private static final Map<String, String> localResources = new HashMap<>();

  static {
    // load map from file
    Properties props = new Properties();
    try {
      props.load(PSEntityResolver.class.getResourceAsStream("PSLocalDTDResources.properties"));
      ...
      LogManager.getLogger(PSEntityResolver.class)
          .warn(
  ...
  /**
   * Hide constructor to force use of factory. There only needs to be a single instance of this
   * class
   */
  ...
  public InputSource resolveEntity(String publicid, String systemid) throws IOException {
    ...
    if (systemid.startsWith(RXROOT)) {
      ...
      entityFile = new File(resolutionHome, systemid.substring(RXROOT.length()));
    ...
    if (entityFile != null) {
      if (entityFile.exists()) {
        ...
        log.error(
            "entityFile doesn't exist! file: {} Error: {}",
   ...
   */
  private InputSource getInputSource(String publicid, String systemid, InputStream is) {
  ...
  /**
   * Get a local source for a public id. Used to avoid obtaining sources from an external url.
   * Public ids that are known to reference external URLs may be mapped to a local resource
   * location, in which case an input source to that resource is returned.
   *
   * @param publicid The public id, may be <code>null</code> or empty.
   * @param warnIfMissing if the public id isn't found and this is <code>true</code> then output a
   *     warning to the log
   * @return The input source to the local resource if one is configured for the supplied public id,
   *     or <code>null</code> if one is not configured or if the supplied public id is <code>null
   *     </code> or empty.
   */
  private InputSource getLocalSource(String publicid, String systemid, boolean warnIfMissing) {
    InputSource source = null;

    if (StringUtils.isBlank(publicid)) return source;
      ...
      if (in != null) {
        ...
        if (warnIfMissing) {
          LogManager.getLogger(this.getClass())
              .warn("Failed to load local resource for entity resolution: {}", localResource);
        }
      }
    } else {
      if (warnIfMissing) {
        LogManager.getLogger(this.getClass())
            .warn("Failed to locate matching local resource for public id: {}", publicid);
      }
    }
  ...
  /**
   * @return the current resolution home. This is used to resolve entity reference that the resolver
   *     recognizes.
   */
  ...
  /**
   * Modifies the resolution home. This call should never be required by a normal running server. It
   * is available for use by tools or the installer to allow variant configurations to be supported
   * by the resolver.
   *
   * @param dir a new resolution home, must never be <code>null</code> and must point to a valid
   *     directory.
   */
  public static synchronized void setResolutionHome(File dir) {
    ...
    if (!dir.exists()) {
      throw new IllegalArgumentException("dir must exist: " + dir.getAbsolutePath());
    }
    if (!dir.isDirectory()) {
      throw new IllegalArgumentException("dir must be a directory: " + dir.getAbsolutePath());
    }
    ...
    log.info("Entity Resolution home set to:  {} ", dir.getAbsolutePath());
