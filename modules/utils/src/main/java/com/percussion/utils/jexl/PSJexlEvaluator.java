...
public class PSJexlEvaluator {
  ...
  public String getStringValue(IPSScript var, String defaultval, boolean required) {
    ...
    if (val == null) {
      if (required) throw new IllegalStateException("No value for required expression " + var);
  ...
  public Object evaluate(IPSScript jexlexpression) {
    ...
    try {
      ...
      if (!e.isUndefined() || !jexlexpression.getSourceText().trim().startsWith("$")) throw e;
    } finally {
      sws.stop();
    }
    return null;
  ...
  public void bind(String var, Object value) throws IllegalStateException {
    ...
    for (int i = 0; i < components.length; i++) {
      ...
      if (square > 0) {
        ...
        if (parts.length < 2) throw new IllegalStateException("Insufficient parts for array deref");
        component = parts[0];
        try {
          index = Integer.parseInt(parts[1]);
        } catch (NumberFormatException nfe) {
          throw new IllegalStateException(
              "Found bad index expression: " + parts[1] + " for component " + parts[0]);
        }
      }
