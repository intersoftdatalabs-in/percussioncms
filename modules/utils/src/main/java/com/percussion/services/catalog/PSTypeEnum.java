 ...
 * these values is used when creating guids. Ordinal values <b>must</b> be unique.
 *
 * @author dougrand
 */
public enum PSTypeEnum {
  /**
   * Used for internal objects that require a unique id, but are not exposed through the service
   * interface.
   */
  INTERNAL(0),
  /**
   * Indicates a JSR-170 item is referenced. The object could be a node or a property. (Not used in
   * Rhino, consider this a placeholder)
   */
  ITEM(1),
  ...
  RELATIONSHIP_CONFIGNAME(20, "Relationship Type", "PSX_RELATIONSHIPCONFIGNAME"),
  ...
  /** Identifies a configuration that can be loaded, edited, and saved through the workbench. */
  ...
  /**
   * Identifies the scheduled tasks. It is only used by the task scheduler subsystem. The task
   * object is saved by Quartz (table prefixed with PSX_Q)
   */
  ...
  SCHEDULE_NOTIFICATION_TEMPLATE(38, "Task Notification Template", "PSX_SCH_NOTIF_TEMPLATE"),

  /** A locale object */
  LOCALE(100, "Locale", "localeid"),

  /** A content item from the legacy content repository. Will be deprecated in the future. */
  LEGACY_CONTENT(101, "Content Item/Folder"),

  /**
   * A content item's child from the legacy content repository. Will be deprecated in the future.
   */
  LEGACY_CHILD(102, "Child Item"),

  /** A child content type from the item def manager. Will be deprecated in the future. */
  LEGACY_CHILD_CONTENT_TYPE(104, "Content Type"),

  /** Rhythmyx extension. */
  EXTENSION(105, "Extension"),

  /** Design objects used only by the workbench for managing a user-defined folder structure. */
  WORKBENCH_FILE(106),
  ...
  /** The display name of the type, may be <code>null</code> or empty after construction. */
  ...
  /**
   * Returns the ordinal value for the enumeration. This ordinal is used as an part of the {@link
   * com.percussion.utils.guid.IPSGuid} id, and can be used as part of the cataloging process
   *
   * @return the ordinal
   */
  ...
  /**
   * For legacy types this enum stores the key from the next number table that is used to lookup the
   * next block of ids.
   *
  ...
  /**
   * Get the name of this type to use suitable for display to an end user. This value is not
   * necessarily unique among types and should not be used other than for display purposes.
   *
   ...
   * @param className The className associated with the ordinal, may be <code>null</code>
   * @param displayName The display name, if <code>null</code> or empty, the name is used.
   * @param keyvalue Optional key value to use for id generation, specifies a key for the NEXTNUMBER
   ...
   *
   * @param ord The ordinal value to use
   * @param clazz The class associated with the ordinal, may be <code>null</code>
   * @param displayName The display name, if <code>null</code> or empty, the name is used.
   * @param keyvalue Optional key value to use for id generation, specifies a key for the NEXTNUMBER
   *     table. If <code>null</code> or empty, the default guid manager key generation is used
   *     instead of the NEXTNUMBER table.
  ...
  /**
   * Lookup enum value by class. This will only return values for enum values that have an
   * associated class.
   *
