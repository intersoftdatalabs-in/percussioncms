...
/**
 * The PSLogInformation abstract class is used as the super-class of all classes used for defining a
 * message to be logged. The sub-classes provide a cleaner interface for their specific function.
 * They in turn call the constructor in this class to store their information in the appropriate
 * fashion. All the methods in this class are used by the log manager to access the data for writing
 * to the log.
 *
...
public abstract class PSLogInformation {
  /**
   * Construct a log message from the specified information. The message will be stamped with the
   ...
   *
   * @return the associated application id or 0 if an application did not generate the message
   */
  ...
  /**
   * Get the sub-messages (type and text). Most log messages contain only one sub-message, though
   * many are permitted.
   *
  ...
  public void toXMLString(StringBuilder buf) {
    com.percussion.util.PSDateFormatISO8601 dateFmt = new com.percussion.util.PSDateFormatISO8601();
   ...
   *
   * @return The submessages formatted as a String, never <code>null</code>, may be empty.
   */
  ...
  public org.w3c.dom.Element toXml(org.w3c.dom.Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, "time", dateFmt.format(m_time));
    PSXmlDocumentBuilder.addElement(doc, root, "applicationId", String.valueOf(m_applId));

    PSLogSubMessage[] subs = getSubMessages();
    for (int i = 0; i < subs.length; i++) {
      org.w3c.dom.Element data =
          PSXmlDocumentBuilder.addElement(doc, root, "data", subs[i].getText());
  ...
  /** Used when a session is not associated with the user that generated the message. */
