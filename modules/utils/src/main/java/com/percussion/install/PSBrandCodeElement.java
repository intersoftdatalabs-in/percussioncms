...
public final class PSBrandCodeElement {
    ...
    if (optionalAttrNames != null) {
      for (int i = 0; i < optionalAttrNames.length; i++) {
        ...
        if ((optionalAttrName == null) || (optionalAttrName.trim().length() < 1))
          throw new IllegalArgumentException("optionalAttrName may not be null or empty");
      }
    }
    fromXml(sourceNode);
  }
  ...
  public final void fromXml(Element sourceNode) throws CodeException {
    ...
    if (m_reqAttrNames != null) {
      for (int i = 0; i < m_reqAttrNames.length; i++) {
        String attrValue = PSBrandCodeUtil.getAttributeValue(sourceNode, m_reqAttrNames[i], true);
    ...
    if (m_optionalAttrNames != null) {
      for (int i = 0; i < m_optionalAttrNames.length; i++) {
        String attrValue =
            PSBrandCodeUtil.getAttributeValue(sourceNode, m_optionalAttrNames[i], false);
        if (attrValue != null) m_attrMap.put(m_optionalAttrNames[i], attrValue);
      }
    }
  }
  ...
  private boolean hasAttribute(String attrName) {
    ...
    if (m_attrMap.containsKey(attrName)) return true;
    return false;
  }
