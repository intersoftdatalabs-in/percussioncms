...
public class PSXmlDocumentBuilderTest extends TestCase {
  ...
  protected void internalTestBooks(boolean withDtd) throws IOException, org.xml.sax.SAXException {
    ...
    if (withDtd) buf.append(ms_bookListDtd);
    ...
    Document doc;
    if (withDtd) doc = PSXmlDocumentBuilder.createXmlDocument(docStringReader, true);
    else doc = PSXmlDocumentBuilder.createXmlDocument(docStringReader, false);
  ...
  public void assertEquals(Node a, Node b) {
    if (a instanceof Element) ((Element) a).normalize();

    if (b instanceof Element) ((Element) b).normalize();
