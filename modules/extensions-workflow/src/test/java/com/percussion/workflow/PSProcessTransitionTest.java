...
public class PSProcessTransitionTest extends PSAbstractWorkflowTest {
    ...
    try {
      ...
      do {
        if (0 == transitionID) {
          ...
          if (PSWorkFlowUtils.timeDiffSecs(m_now, nextAgingDate) < 0) {
          ...
          tc = new PSTransitionsContext(transitionID, m_nWorkflowID, connection);
          tc.close();
        ...
        m_now = PSWorkFlowUtils.incrementDate(m_now, m_nPollingInterval);
        nextAgingDate = csc.getNextAgingDate();
      } while ((null != nextAgingDate && (count < m_nMaxTransitions)));
      ...
      if (m_bIsVerbose) {
        log.info("\nEnd test of " + "PSExitPerformTransition.processTransition\n");
      }
  ...
  public void startInState(
    ...
    workingVarCal = PSWorkFlowUtils.incrementCalendar(workingVarCal, now, -60 * setBackHours);
    ...
    csc.setReminderDate(
        PSWorkFlowUtils.sqlDateFromCalendar(
            PSWorkFlowUtils.incrementCalendar(workingVarCal, startDate, 60 * reminderOffset)));

    csc.setExpiryDate(
        PSWorkFlowUtils.sqlDateFromCalendar(
            PSWorkFlowUtils.incrementCalendar(
                workingVarCal, startDate, 60 * (reminderOffset + setBackHours))));
    csc.commit(connection);
  }
