...
public abstract class PSAbstractWorkflowTest {
    ...
    try {
      ...
      if (null != throwable) {
        if ((java.sql.SQLException.class).isInstance(throwable)) {
          ...
          for (; null != sqlEx; sqlEx = sqlEx.getNextException()) {
  ...
  private static final String QUERY_REQUEST = "SELECT NEXTNR FROM NEXTNUMBER WHERE KEYNAME = ?";
  private static final String QUERY_UPDATE = "UPDATE NEXTNUMBER SET NEXTNR = ? WHERE KEYNAME = ?";
