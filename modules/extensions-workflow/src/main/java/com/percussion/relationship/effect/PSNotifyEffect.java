...
public class PSNotifyEffect extends PSEffect {
  ...
  public void attempt(
    ...
    for (int i = 0; i < REQUIRED_HTML_PARAMS.length; i++) {
      String parameter = request.getParameter(REQUIRED_HTML_PARAMS[i], "").trim();
    ...
    PSWorkFlowContext wfContext =
        new PSWorkFlowContext(
            getIntParameter("workflowid", (String) parameters.get("workflowid")),
            getIntParameter(
            ...
            getIntParameter("transitionid", (String) parameters.get("transitionid")),
            getIntParameter("stateid", (String) parameters.get("stateid")));
    ...
    request.setPrivateObject(PSWorkflowRoleInfo.WORKFLOW_ROLE_INFO_PRIVATE_OBJECT, wfRoleInfo);

    // call the notify assignees exit with an empty document
    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    Object[] notifyParams = {
      parameters.get(IPSHtmlParameters.SYS_CONTENTID), parameters.get("username")
    };
    try {
