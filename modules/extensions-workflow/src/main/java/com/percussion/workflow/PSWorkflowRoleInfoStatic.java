...
public class PSWorkflowRoleInfoStatic {
  ...
  public static List<String> roleIDListToRoleNameList(
    ...
    return PSWorkFlowUtils.applyMapList(roleIDList, src.getStateRoleNameMap());
  }
  ...
  public static List<Integer> getActorRoles(
    ...
    if (null == src) throw new IllegalArgumentException("State roles context may not be null.");

    if (null == userName) throw new IllegalArgumentException("User name may not be null.");

    if (cauc == null) throw new IllegalArgumentException("cauc may not be null");

    userName = userName.trim();

    if (0 == userName.length()) throw new IllegalArgumentException("User name may not be empty.");

    if (null == userRoleNames) return null;

    userRoleNames = userRoleNames.trim();
    if (0 == userRoleNames.length()) return null;

    userRoleNameList =
        PSWorkFlowUtils.tokenizeString(userRoleNames.toLowerCase(), PSWorkFlowUtils.ROLE_DELIMITER);

    if (userRoleNameList.isEmpty()) return null;
    ...
    stateAdhocAnonymousRoles = src.getAdhocAnonymousStateRoleIDs();
    stateAdhocNormalRoles = src.getAdhocNormalStateRoleIDs();
    ...
    emptyAdhocNormalRoles = cauc.getEmptyAdhocRoles(stateAdhocNormalRoles);
    emptyAdhocAnonymousRoles = cauc.getEmptyAdhocRoles(stateAdhocAnonymousRoles);
     ...
     *    state role context
     */
    if (null != nonAdhocStateRoleNameToRoleIDMap && !nonAdhocStateRoleNameToRoleIDMap.isEmpty()) {
      actorNonAdhocRoles =
          PSWorkFlowUtils.applyMapList(userRoleNameList, nonAdhocStateRoleNameToRoleIDMap);
      ...
      if (authUser) {
        ...
        if (null != adhocNormalStateRoleNameToRoleIDMap
          ...
          actorAdhocNormalRoles =
              PSWorkFlowUtils.applyMapList(userRoleNameList, adhocNormalStateRoleNameToRoleIDMap);
    ...
    if (null != emptyAdhocAnonymousRoles && !emptyAdhocAnonymousRoles.isEmpty()) {
      // for Adhoc anonymous the user need not belong to the role
      actorRoles.addAll(emptyAdhocAnonymousRoles);
    }

    PSAssignmentTypeHelper.filterAssignedRolesByCommunity(cauc.getContentId(), actorRoles);
    ...
    if (adhocUserNames != null && !adhocUserNames.isEmpty()) {
      ...
      for (String adhocUserName : adhocUserNames) {
        ...
        if (userName.equalsIgnoreCase(adhocUserName)) {
          ...
          if (resAdhocRoles != null && !resAdhocRoles.isEmpty()) actorRoles.addAll(resAdhocRoles);
      ...
      if (stateAdhocRoles != null && !stateAdhocRoles.isEmpty()) {
        ...
        if (!resAdhocRoles.isEmpty()) actorRoles.addAll(resAdhocRoles);
      }
    }
  }
  ...
  public static int getAssignmentType(PSStateRolesContext src, List<Integer> actorRoleList) {
    ...
    return getAssignmentType(src.getStateRoleAssignmentTypeMap(), actorRoleList);
  }
  ...
  static int getAssignmentType(
    ...
    if (null == assignmentTypeMap || assignmentTypeMap.isEmpty()) {
      throw new IllegalArgumentException("Assignment type map may not be null or empty.");
    }
    if (null == actorRoleList || actorRoleList.isEmpty()) {
      return assignmentType;
    }
  ...
  static List<Integer> findAdhocNormalRoles(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method findAdhocNormalRoles");
    ...
    adhocNormalStateRoleNameToRoleIDMap = src.getAdhocNormalStateRoleNameToRoleIDMap();
    if (null == adhocNormalStateRoleNameToRoleIDMap
        || adhocNormalStateRoleNameToRoleIDMap.isEmpty()) {
      return null;
    }
  ...
  static PSContentAdhocUsersContext classifyAdhocUsers(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method classifyAdhocUsers");
    if (null == src) {
      throw new IllegalArgumentException("State roles context may not be null.");
    }
    ...
    String userName;
    PSContentAdhocUsersContext cauc = new PSContentAdhocUsersContext(contentID);
    List<String> unassignedUserList = new ArrayList<>();
    List<Integer> anonymousStateRoleIDs = src.getAdhocAnonymousStateRoleIDs();
    boolean existsAnonymousStateRole = (!anonymousStateRoleIDs.isEmpty());
    ...
    List<String> adhocUserNameList =
        PSWorkFlowUtils.tokenizeString(adhocUserNames, PSWorkFlowUtils.ADHOC_USER_LIST_DELIMITER);
    ...
    for (String s : adhocUserNameList) {
      ...
      PSAssignmentTypeHelper.filterAssignedRolesByCommunity(contentID, adhocNormalRoles);
      ...
      if (adhocNormalRoles.isEmpty()) {
        ...
        cauc.addUserAdhocNormalRoleIDs(userName, adhocNormalRoles);
      }
    }
    ...
    if (!unassignedUserList.isEmpty()) {
      if (existsAnonymousStateRole) {
        ...
        cauc.setAdhocAnonymousUsersAndRoles(unassignedUserList, anonymousStateRoleIDs);
      } else {
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method classifyAdhocUsers");
    return cauc;
  }
  ...
  static List<String> getStateRoleNameNotificationList(PSStateRolesContext src, int contentId) {
    ...
    return PSWorkFlowUtils.applyMapList(
        getStateRoleIDNotificationList(src, contentId), src.getStateRoleNameMap());
  }
  ...
  static List<String> getStateAdhocActorNotificationList(
    ...
    return getStateAdhocActorNotificationList(cauc, src, contentId, request, true);
  }
  ...
  static List<String> getStateAdhocActorNotificationList(
      ...
      int contentId,
      IPSRequestContext request,
      boolean validateRoleMembership) {
    ...
    if (null == request && validateRoleMembership) {
      throw new IllegalArgumentException(
          "Request context may not be null if role membership" + "must be validated.");
    }
    ...
    if (null != adhocNormalUserNames) {
      ...
      stateNotificationEnabledAdhocNormalRoles.removeAll(notifiedAdhocRoleIds);
      unnotifiedAdhocRoleIds.addAll(stateNotificationEnabledAdhocNormalRoles);
    }

    // add all usernames for unnotified roles
    stateAdhocActorSet.addAll(getRoleSubjectNames(src, request, unnotifiedAdhocRoleIds));
  ...
  private static void filterNotifyEnabledMapByCommunity(
    ...
    Set<Integer> notifyRoleIds = new HashSet<>(notifyEnabledMap.keySet());
    PSAssignmentTypeHelper.filterAssignedRolesByCommunity(contentId, notifyRoleIds);
    Set<Integer> nonNotifyRoleIds = new HashSet<>(notifyEnabledMap.keySet());
  ...
  public static List<Integer> getSubjectRoleIDs(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method getSubjectRoleIDs");

    List<String> subjectRoleNames;
    List<Integer> subjectRoleIDs;

    if (null == subject || 0 == subject.length()) {
      throw new IllegalArgumentException("Subject name may not be null or empty.");
    }

    subject = subject.trim();

    if (0 == subject.length()) {
      throw new IllegalArgumentException("Subject name may not be empty after trimming.");
    }
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getSubjectRoleIDs");
  ...
  public static List<String> getSubjectRoles(String subject, IPSRequestContext request) {
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getSubjectRoles");
    return roleList;
  }
  ...
  public static List<PSSubject> getRoleSubjects(String roleName, IPSRequestContext request) {
    ...
    if (null == roleName || 0 == roleName.length()) {
      throw new IllegalArgumentException("Role name may not be null or empty.");
    }

    roleName = roleName.trim();

    if (0 == roleName.length()) {
      throw new IllegalArgumentException("Role name may not be empty after trimming.");
    }
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getRoleSubjects");
    return subjectList;
  }
  ...
  public static List<String> getSubjectAttributeValueTextList(
    ...
    if (0 == attributeName.length()) {
      throw new IllegalArgumentException("Attribute name may not be empty after trimming.");
    }
  ...
  public static List<PSSubject> getRoleSubjectsGlobalAttribute(
    ...
    if (0 == roleName.length()) {
      throw new IllegalArgumentException(
          "Role name may not be empty after trimming in " + "getRoleMemberEmailAddresses.");
    }
  ...
  public static List<PSSubject> getSubjectGlobalAttribute(
    ...
    if (0 == subjectName.length()) {
      throw new IllegalArgumentException(
          "Subject name may not be empty after trimming in " + "getSubjectMemberEmailAddresses.");
    }
  ...
  public static List<String> getRolesEmailAddresses(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method getRolesEmailAddresses");
    ...
    for (String s : roleList) {
      ...
      roleEmailAddresses = getRoleEmailAddresses(roleName, request, communityId);
      if (!roleEmailAddresses.isEmpty()) {
        membersEmail.addAll(roleEmailAddresses);
      }
    }

    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getRolesEmailAddresses");
  ...
  public static List<String> getRoleEmailAddresses(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method getRoleEmailAddresses");
    ...
    if (null == request) throw new IllegalArgumentException("Request context may not be null.");

    String emailAttributeName =
        PSWorkFlowUtils.properties.getProperty(
            PSWorkFlowUtils.USER_EMAIL_ATTRIBUTE_PROPERTY, PSWorkFlowUtils.USER_EMAIL_ATTRIBUTE);
    List<String> membersEmail =
        new ArrayList<String>(
            request.getRoleEmailAddresses(roleName, emailAttributeName, communityId));

    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getRoleEmailAddresses");
  ...
  protected static List<PSNotificationEmailAddress> getRoleEmailAddresses(
    ...
    return new ArrayList<>(
        PSRoleManager.getInstance()
            .getSubjectEmailAddresses(null, princes, emailAttributeName, null));
  }
  ...
  protected static Set<IPSTypedPrincipal> filterPrincipalsByCommunityAndFolderSecurity(
    ...
    Set<IPSTypedPrincipal> commFiltered =
        PSRoleManager.getInstance().filterByCommunity(princes, "" + itemCommunityId, true);
    if (!PSCms.isFolderSecurityOverridesWorkflowSecurity()) {
      return commFiltered;
    }
    final Set<IPSTypedPrincipal> filteredPrinces = new HashSet<>();

    PSRunAsUser<IPSTypedPrincipal> runAsUser =
        new PSRunAsUser<IPSTypedPrincipal>() {
          @Override
   ...
   * @return not null maybe empty.
   */
  protected static Set<IPSTypedPrincipal> getPrincipals(List<String> fromStateRoles) {
    try {
  ...
  public static Set<IPSTypedPrincipal> getPrincipalsFromNames(List<String> userNames) {
    ...
    for (String user : userNames) principals.add(PSTypedPrincipal.createSubject(user));
  ...
  private static int getCommunityId(int contentid) {
    ...
    final PSComponentSummary summary =
        PSCmsObjectMgrLocator.getObjectManager()
            .loadComponentSummaries(singletonList(contentid))
            .get(0);
  ...
  public static List<PSNotificationEmailAddress> getSubjectsEmailAddresses(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method getSubjectsEmailAddresses");
    ...
    for (String s : subjectList) {
      ...
      subjectsEmail.addAll(getSubjectEmailAddresses(subjectName, request, communityId));
    }

    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getSubjectsEmailAddresses");
    return subjectsEmail;
  }
  ...
  public static List<PSNotificationEmailAddress> getSubjectEmailAddresses(
    ...
    PSWorkFlowUtils.printWorkflowMessage(request, "    Entering Method getSubjectEmailAddresses");
    ...
    String emailAttributeName =
        PSWorkFlowUtils.properties.getProperty(
            PSWorkFlowUtils.USER_EMAIL_ATTRIBUTE_PROPERTY, PSWorkFlowUtils.USER_EMAIL_ATTRIBUTE);
    List<PSNotificationEmailAddress> subjectsEmail =
        new ArrayList<>(
            request.getSubjectEmailAddresses(subjectName, emailAttributeName, communityId));

    PSWorkFlowUtils.printWorkflowMessage(request, "    Exiting Method getSubjectEmailAddresses");
    return subjectsEmail;
  }
  ...
  public static List<String> getRoleSubjectNames(
    ...
    if (!roleIds.isEmpty()) {
      ...
      List<String> roleNameList = roleIDListToRoleNameList(roleIds, src);
