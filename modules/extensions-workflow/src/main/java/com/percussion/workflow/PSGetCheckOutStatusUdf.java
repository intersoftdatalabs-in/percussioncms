  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    try {
      ...
      if (null != params
        ...
        if (null != o) currentUser = o.toString().trim();
        if (null != currentUser && currentUser.equalsIgnoreCase(checkedOutUser)) {
          status = CHECKOUT_STATUS_CURRENT_USER;
        } else status = CHECKOUT_STATUS_OTHER;
      }
      return status;
    } catch (PSDataExtractionException e) {
      String lang = e.getLanguageString();
      if (lang == null) lang = PSI18nUtils.DEFAULT_LANG;
      throw new PSConversionException(lang, e.getErrorCode(), e.getErrorArguments());
    }
  }
