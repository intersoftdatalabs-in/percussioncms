...
public class PSGetWorkflowActionList implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    String lang =
        (String) requestContext.getSessionPrivateObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);
    if (lang == null) lang = PSI18nUtils.DEFAULT_LANG;
    PSWorkFlowUtils.printWorkflowMessage(
        requestContext, "Entering PSGetWorkflowActionList processResultDocument");
    int size = (params == null) ? 0 : params.length;
    if (size != 0) { // no parameters are required
      throw new PSParameterMismatchException(lang, size, 0);
    }

    if (null == requestContext) {
      throw new PSExtensionProcessingException(
          PSWorkFlowUtils.ERROR_INVALID_PARAMETER_TYPE, "The request context must not be null");
    }

    if (null == resultDoc) {
      throw new PSExtensionProcessingException(
          PSWorkFlowUtils.ERROR_INVALID_PARAMETER_TYPE, "The result document must not be null");
    }
    ...
    try {
      ...
      while (wfActionRefIter.hasNext()) {
        ...
        workflowAction.setAttribute("extensionhandlername", wfActionRef.getHandlerName());
        workflowAction.setAttribute("extensioncontext", wfActionRef.getContext());
        workflowAction.setAttribute("extensionname", wfActionRef.getExtensionName());
      }
      ...
      if (null != fatal) {
        ...
        if (fatal instanceof PSException) {
          ...
          throw new PSExtensionProcessingException(language, m_fullExtensionName, fatal);
        } else throw new PSExtensionProcessingException(m_fullExtensionName, fatal);
      }
      PSWorkFlowUtils.printWorkflowMessage(
          requestContext, "Exiting PSGetWorkflowActionList: processResultDocument");
    }
    return resultDoc;
  }
}
