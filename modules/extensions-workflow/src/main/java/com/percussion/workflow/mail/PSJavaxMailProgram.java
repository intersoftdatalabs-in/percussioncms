...
public class PSJavaxMailProgram implements IPSMailProgram {
  ...
  public void sendMessage(IPSMailMessageContext messageContext) throws PSMailException {
    ...
    try {
      ...
      msg.setRecipients(Message.RecipientType.TO, makeAddress(messageContext.getTo(), mailDomain));

      if ((null != mailCc) && mailCc.length() > 0) {
        msg.setRecipients(Message.RecipientType.CC, makeAddress(mailCc, mailDomain));
      }
  ...
  private InternetAddress[] makeAddress(String sUserList, String mailDomain)
    ...
    while (tokenizer.hasMoreElements()) {
      ...
      if (0 != sToken.length()) {
        ...
        InternetAddress userInetAddress = new InternetAddress(userAddressString);
