...
public class PSExitAuthenticateUser implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    try {
      ...
      if (null == params) return; // no parameters exit with peace!
      ...
      try {
        ...
        // Get action trigger
        if (htmlParams.containsKey(PSWorkFlowUtils.DEFAULT_ACTION_TRIGGER_NAME)) {
          localParams.m_actionTrigger =
              (String) htmlParams.get(PSWorkFlowUtils.DEFAULT_ACTION_TRIGGER_NAME);
        }
        ...
        } else if (PSWorkFlowUtils.ASSIGNMENT_TYPE_NOT_IN_WORKFLOW > requiredAccessLevel) {
          requiredAccessLevel = PSWorkFlowUtils.ASSIGNMENT_TYPE_NOT_IN_WORKFLOW;
        }
      ...
      try {
        ...
        if (e instanceof PSException) language = ((PSException) e).getLanguageString();
        if (language == null) language = PSI18nUtils.DEFAULT_LANG;
...
if (csc.getObjectType() == PSCmsObject.TYPE_FOLDER) {
    ...
    if (null != checkInOutCondition
        && !checkInOutCondition.equalsIgnoreCase(PSWorkFlowUtils.CHECKINOUT_CONDITION_IGNORE)) {
      //
    ...
    actorRoleNames = PSWorkflowRoleInfoStatic.roleIDListToRoleNameList(actorRoles, src);
...
private boolean canUserCreate(AuthParams localParams)
    ...
    String folderid = localParams.m_request.getParameter(IPSHtmlParameters.SYS_FOLDERID, null);
    ...
    String lang =
        (String)
            localParams.m_request.getSessionPrivateObject(PSI18nUtils.USER_SESSION_OBJECT_SYS_LANG);
    if (lang == null) lang = PSI18nUtils.DEFAULT_LANG;

    IPSCmsObjectMgr cms = PSCmsObjectMgrLocator.getObjectManager();
    Optional<IPSWorkflowAppsContext> wcxtOpt =
    ...
    Boolean communitiesEnabled =
        Boolean.parseBoolean(
            PSServer.getServerProps()
                .getProperty(IPSConstants.SERVER_PROP_COMMUNITIES_ENABLED, "no"));

    // Only filter by community if communities are enabled.
    if (communitiesEnabled) {
      // now filter roles by community
      PSAssignmentTypeHelper.filterAssignedRolesByCommunity(
          commId, localParams.m_workflowAppID, stateRoleList);
    }
    // now see if user has one of those roles
    canCreate = PSWorkFlowUtils.compareRoleList(stateRoleList, localParams.m_roleNameList);
    PSWorkFlowUtils.printWorkflowMessage(localParams.m_request, "    Exiting canUserCreate");
    return canCreate;
  }
}
