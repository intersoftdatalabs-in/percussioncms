...
public class PreviewWorkflow extends PSPostExitHandler {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    try {
      ...
      for (int i = 0; nl != null && i < nl.getLength(); i++) {
        ...
        transitionsReq = request.getInternalRequest("sys_wfPreviewWorkflow/transitionslookup");
        try {
  ...
  private Document processDocument(Document doc) {
    ...
    elemTransitions = (Element) elemStates.getParentNode().appendChild(elemTransitions);
    ...
    for (int i = 0; i < nlTransitions.getLength(); i++) {
      ...
      if (null == tmp || tmp.trim().length() < 1) continue;
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (!statesMap.containsKey(transition.getFrom())
        ...
        elemTransitions.replaceChild(
            transition.makeElement(elemTransitions, statesMap, height), elemTransition);
    }
