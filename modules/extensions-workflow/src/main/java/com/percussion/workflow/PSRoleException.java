...
/**
 * This error class is used to indicate that a an error has occurred when processing state role or
...
public class PSRoleException extends PSException {
   ...
   *     null</code>.
   */
  public PSRoleException(String language, int msgCode, Object singleArg) {
   ...
   *     <code>null</code>.
   */
  public PSRoleException(String language, int msgCode, Object[] arrayArgs) {
    super(language, msgCode, arrayArgs);
  }

  /**
   * Construct an exception for messages taking an array of arguments and message code. Be sure to
   * store the arguments in the correct order in the array, where {0} in the string is array element
   * 0, etc.
   *
   * @param msgCode the error string to load
   * @param arrayArgs the array of arguments to use as the arguments in the error message. Can be
