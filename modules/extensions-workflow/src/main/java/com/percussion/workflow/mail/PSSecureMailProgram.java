...
public class PSSecureMailProgram implements IPSMailProgram {
  ...
  public void sendMessage(IPSMailMessageContext messageContext) throws PSMailException {
    ...
    try {
      ...
      log.error(
          "Error sending workflow mail notification with message: {}",
  ...
  private MultiPartEmail createMultiPartEmail(IPSMailMessageContext messageContext)
    ...
    if (!StringUtils.isEmpty(messageContext.getUserName())
      ...
      String pwd =
          PSEncryptProperties.decryptProperty(
              ...
              PathUtils.getRxDir().getAbsolutePath().concat(PSEncryptor.SECURE_DIR),
              null);
      commonsMultiPartEmail.setAuthentication(messageContext.getUserName(), pwd);
    }
    ...
    if (!StringUtils.isEmpty(messageContext.getSSLPortNumber())) {
      commonsMultiPartEmail.setSslSmtpPort(messageContext.getSSLPortNumber());
    ...
    if (!StringUtils.isEmpty(messageContext.getBounceAddr())) {
      commonsMultiPartEmail.setBounceAddress(
          makeAddress(messageContext.getBounceAddr(), messageContext.getMailDomain()));
    ...
    commonsMultiPartEmail.setFrom(
        makeAddress(messageContext.getFrom(), messageContext.getMailDomain()));
