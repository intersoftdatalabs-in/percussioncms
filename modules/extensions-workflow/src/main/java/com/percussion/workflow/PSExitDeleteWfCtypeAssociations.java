...
public class PSExitDeleteWfCtypeAssociations implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (StringUtils.isBlank(wfid)) {
      String msg =
          "workflowid parameter is missing, skipping "
    ...
    try {
      List<IPSNodeDefinition> nodeDefs = mgr.findNodeDefinitionsByWorkflow(wfGuid);
      deleteWorkflowFromNodeDefs(wfGuid, nodeDefs);
    } catch (RepositoryException e) {
      String msg = "Failed to delete the workflow({0}) " + "association with the content types.";
   ...
   */
  private void deleteWorkflowFromNodeDefs(IPSGuid wfGuid, List<IPSNodeDefinition> nodeDefs) {
    ...
    for (IPSNodeDefinition nodeDef : nodeDefs) {
      ...
      try {
        ...
        for (PSContentTypeWorkflow ctwf : ctwfs) {
          if (!ctwf.getWorkflowId().equals(wfGuid)) wfguids.add(ctwf.getWorkflowId());
        }
        ...
        log.debug(PSExceptionUtils.getDebugMessageForLog(e));
        String msg =
            "Failed to delete the workflow({0}) " + "association with the content type ({1}).";
