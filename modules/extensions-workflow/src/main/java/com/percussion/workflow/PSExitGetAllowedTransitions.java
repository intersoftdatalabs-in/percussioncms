...
public class PSExitGetAllowedTransitions extends PSDefaultExtension
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      List<PSAssignmentTypeEnum> assignmentTypes = getAssignmentTypes(request, contentids);
      ...
      List<PSMenuAction> actions =
          svc.getAllWorkflowActions(contentids, assignmentTypes, userName, userRoles, locale);
  ...
  private List<IPSGuid> getContentIds(IPSRequestContext request) {
    ...
    if (ids == null || ids.length == 0) return contentids;

    for (Object val : ids) {
      if (val != null) contentids.add(guidMgr.makeGuid(new PSLocator(val.toString())));
    }
    return contentids;
  }
  ...
  private List<PSAssignmentTypeEnum> getAssignmentTypes(
    ...
    Object[] types = request.getParameterList(IPSHtmlParameters.SYS_ASSIGNMENTTYPEID);
    if (types == null || types.length == 0) {
      return calculateAssignmentTypes(request, contentIds);
    } else {
      if (types.length != contentIds.size())
        throw new IllegalArgumentException(
            "The supplied number of " + "contentids and assignment type ids must match");
    }

    for (Object val : types) {
      if (val != null)
        assignmentTypes.add(PSAssignmentTypeEnum.valueOf(Integer.parseInt(val.toString())));
    }
  ...
  private List<PSAssignmentTypeEnum> calculateAssignmentTypes(
    ...
    try {
      ...
      if (usercomm != null) userCommunity = Integer.parseInt(usercomm);
    } catch (Exception e) {
    }

    return sysSvc.getContentAssignmentTypes(
        contentIds, request.getUserName(), request.getSubjectRoles(), userCommunity);
  }
}
