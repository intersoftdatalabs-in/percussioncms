...
function enableCrop() {
  ...
  $(overlay).Resizable({
    ...
    onResize: function (size, position) {
      this.style.backgroundPosition =
        "-" +
        (position.left - imageContainerLeft) +
        "px -" +
        (position.top - imageContainerTop) +
        "px";
...
function getImageElement() {
  ...
  for (var i = 0; i < imageContainer.childNodes.length; i++) {
    if (
      (typeof imageContainer.childNodes[i].name != "undefined" &&
        imageContainer.childNodes[i].name.toLowerCase() == "image") ||
      (typeof imageContainer.childNodes[i].tagName != "undefined" &&
        (imageContainer.childNodes[i].tagName.toLowerCase() == "canvas" ||
          imageContainer.childNodes[i].tagName.toLowerCase() == "img"))
    ) {
