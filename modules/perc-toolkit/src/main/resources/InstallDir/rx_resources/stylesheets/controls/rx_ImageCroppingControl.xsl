...
<xsl:stylesheet version="1.1"
	...
	<xsl:template
		match="Control[ParamList/Param[@name='helptext'] and @isReadOnly='no']"
		priority="25" mode="psxcontrol">
		<!-- override all controls to display helptext, then return control to
			standard template -->
		...
		<xsl:variable name="control-stripped-of-help">
			<xsl:apply-templates select="."
				mode="copy-all-but-help-param" />
		</xsl:variable>

		<xsl:variable name="control-help"
			select="concat('help_', @paramName)" />

		<xsl:apply-templates
			select="exsl:node-set($control-stripped-of-help)" mode="psxcontrol" />

		<!-- display help text -->
		<img src="../rx_resources/images/help.gif"
			onmouseover="TagToTip('{$control-help}', CLOSEBTN, true, STICKY, true);"
			onmouseout="UnTip()" />
		<span id="{$control-help}">
			<xsl:value-of select="ParamList/Param[@name='helptext']" />
		</span>
	</xsl:template>
	...
	<xsl:template match="Param[@name='helptext']"
		mode="copy-all-but-help-param" />
	...
	<xsl:template
		match="Control[@name='rx_ImageCroppingControl']" mode="psxcontrol">
		<xsl:variable name="cropping_attributes">
			<xsl:value-of
				select="ParamList/Param[@name='cropping_attributes']" />
		</xsl:variable>
		<div class="image_cropper_inputs" data-param-name="{@paramName}">
			...
			<input type="text" name="{@paramName}_resize_h"></input>
			<br />
		</div>
		<div class="image_cropper" data-param-name="{@paramName}"
			id="{@paramName}">
	...
	<xsl:template
		match="Control[@name='rx_ImageCroppingControl' and @isReadOnly='yes']"
		priority="11" mode="psxcontrol">
		<xsl:variable name="cropping_attributes">
			<xsl:value-of
				select="ParamList/Param[@name='cropping_attributes']" />
		</xsl:variable>
