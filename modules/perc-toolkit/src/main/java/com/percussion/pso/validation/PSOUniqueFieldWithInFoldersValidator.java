...
/**
 * This is a field validator that checks whether or not a field is unique for all the folders that
 * the item resides (or to be created) in.
 *
 * <p>There are 3 parameters:
 *
 * <ul>
 *   <li>The field value - defaults to the current value of the field.
 *   <li>The field name - defaults to the name of the current field.
 *   <li>Exclude Promotable Versions flag -- specify <code>true</code> or <code>false</code>
 * </ul>
 *
...
public class PSOUniqueFieldWithInFoldersValidator implements IPSFieldValidator {
  ...
  public Boolean processUdf(Object[] params, IPSRequestContext request)
    ...
    PSOExtensionParamsHelper h =
        new PSOExtensionParamsHelper(getExtensionDef(), params, request, log);
    ...
    try {
      ...
      if (actionType.equals("UPDATE")) {
        rvalue =
            isFieldValueUniqueInFolderForExistingItem(
                contentId.intValue(), fieldName, fieldValue, typeList, checkPaths);
      } else {
        Number folderId = getFolderId(request);
        if (folderId != null)
          rvalue =
              isFieldValueUniqueInFolder(
                  folderId.intValue(), fieldName, fieldValue, typeList, checkPaths);
        else rvalue = false;
      }
      return rvalue;
    } catch (Exception e) {
      log.error(
          format(
              "An error happend while checking if "
  ...
  public boolean isFieldValueUniqueInFolderForExistingItem(
    ...
    return this.isFieldValueUniqueInFolderForExistingItem(
        contentId, fieldName, fieldValue, typeList, null);
  }
  ...
  public boolean isFieldValueUniqueInFolderForExistingItem(
    ...
    if (paths != null && paths.size() != 0) {
      for (String pathItem : paths) {
        if (unique) {
          String jcrQuery =
              getQueryForValueInFolders(contentId, fieldName, fieldValue, pathItem, typeList);
          log.trace(jcrQuery);
    ...
    String jcrQuery =
        format(
            "select rx:sys_contentid, rx:{0} "
                ...
                + "and "
                + "jcr:path like ''{3}''",
            fieldName, "" + contentId, fieldValue, path, typeList);
    return jcrQuery;
  }
