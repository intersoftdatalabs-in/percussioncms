...
public class ValidatingContentAssemblerMerge {
  ...
  public static IPSAssemblyResult merge(IPSExtensionDef def, IPSAssemblyResult result)
    ...
    configTidyPropertiesFile =
        PSServer.getRxFile(
            def.getInitParameter("com.percussion.extension.assembly.TidyPropertiesFile"));

    // When set we tell Tidy to ignore errors and force output.
    configForcePublish =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.ForcePublish"));

    // When set we tell Tidy to do it's thing, but we don't want it trying to do any real cleanup
    // for us
    configDontFixMarkup =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.DontFixMarkup"));

    // If Tidy finds errors, we'll tell the CMS to fail the publish job for this item.
    configFailPublishOnErrors =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.FailPublishOnErrors"));

    configFailPublishOnErrors =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.FailPublishOnWarnings"));

    configIsXMLContent =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.IsXMLContent"));

    // If set, we tell Tidy to treat the content as XHTML.
    configIsXHTMLContent =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.IsXHTMLContent"));

    // If set, we tell Tidy to treat the content as HTML.
    configIsHTMLContent =
        Boolean.parseBoolean(
            def.getInitParameter("com.percussion.extension.assembly.IsHTMLContent"));
    ...
    try {
      if ((parent != null) && (result.getId() != parent.getId())) {
        log.debug("Not processing. Item {} has parent Item id={}", result.getId(), parent.getId());
        isSnippet = true;
      } else {
      ...
      log.error(
          "Unexpected Error {} Snippet detection logic", PSExceptionUtils.getMessageForLog(ex));
    } finally {
