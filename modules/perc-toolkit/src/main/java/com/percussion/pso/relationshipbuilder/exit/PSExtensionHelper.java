...
/**
 * This class is shared code between the several exits that are provided for relationship building.
 *
 * @author Adam Gent
 * @since 6.0
 */
public class PSExtensionHelper {
  ...
  public void updateDisplayChoices(Document resultDoc, boolean selectAll)
    ...
    if (controlElement == null) {
      ...
      if (displayChoicesElement == null) {
        log.debug(
            "No DisplayChoice Elements. Checking for Value e.g. multi value no child table, ;"
                + " separated");
    ...
    log.debug(
        "Finished updating display choices builder: " + m_builder.getClass().getCanonicalName());
  }
  ...
  public Object retrieveIds() throws PSConversionException {
    ...
    final String contentId = request.getParameter(IPSHtmlParameters.SYS_CONTENTID);
  ...
  public void buildRelationships()
    ...
    if (isRequestToBeProcessedForBuilding(m_request)) {
      log.debug(
          "Starting building relationships exit with builder " + m_builder.getClass().getName());
      Map<String, String> paramMap = m_parameters;
      PSOExtensionParamsHelper extParamHelper =
          new PSOExtensionParamsHelper(paramMap, m_request, log);
  ...
  public static void logRequestCommand(IPSRequestContext request) {
    ...
    String processInlineLink = request.getParameter(IPSHtmlParameters.SYS_INLINELINK_DATA_UPDATE);
  ...
  public static boolean isRequestToBeProcessedForBuilding(IPSRequestContext request) {
    ...
    Number contentId = helper.getOptionalParameterAsNumber(IPSHtmlParameters.SYS_CONTENTID, "0");
    String command = request.getParameter(IPSHtmlParameters.SYS_COMMAND);
    String page = request.getParameter(PSContentEditorHandler.PAGE_ID_PARAM_NAME);
    String processInlineLink = request.getParameter(IPSHtmlParameters.SYS_INLINELINK_DATA_UPDATE);
   ...
   */
  public static Collection<Object> convert(Object[] inputIds, final Set<Integer> output) {
    ...
    if (inputIds != null) {
      ...
      for (int i = 0; i < inputIds.length; i++) {
        ...
        if (contentId instanceof Integer) {
          ...
          log.warn("\ttaking note of non-parsable element in array <" + contentId + ">");
