...
public class ImageEditorWizard {
  ...
  protected Map<String, SizedImageMetaData> cleanEmptySizedImages(
    ...
    for (Map.Entry<String, SizedImageMetaData> entry : inmap.entrySet()) {
      ...
      if (simd.getX() != 0
          ...
          || simd.getHeight() != 0
          || simd.getWidth() != 0
          || StringUtils.isNotBlank(simd.getImageKey())) {
        outmap.put(key, simd);
      }
    }
    return outmap;
  ...
  public void setupSizedImages(
    ...
    if (!StringUtils.isBlank(sizedImages)) {
      ...
      log.debug(
          "setupSizedImages: the bean has {} sized image(s) selected", sizedImagesArray.length);
      ...
      // Images have been selected and there were others stored previously
      {
        ...
        for (int x = 0; x < sizedImagesArray.length; x++) {
          ...
          if (previousSimds.containsKey(key)) {
            ...
            log.debug(
                "setupSizedImages: Image of type [{}] previously chosen, re-using existing one",
                key);
  ...
  public void setPagesDynamically(HttpServletRequest request, String sizedImages) {
    ...
    if (!StringUtils.isBlank(sizedImages)) {
      String[] sizedImagesArray = sizedImages.split(",");
      log.debug(
          "setPagesDynamically: the bean has {} sized image(s) selected", sizedImagesArray.length);
      ...
      int x;
      wizardPages[0] = MAIN_PAGE;
      for (x = 1; x <= sizedImagesArray.length; x++) wizardPages[x] = SIZE_PAGE;
    ...
    if (page == 0) {
      ...
      if (StringUtils.isBlank(ib.getSysTitle())) {
        errors.rejectValue(
            ...
            new String[] {"System Title"},
            "System Title can't be blank.");
      }
      if (StringUtils.isBlank(ib.getDisplayTitle())) {
        errors.rejectValue(
            "displayTitle",
            "blank.field.not.allowed",
            new String[] {"Display Title"},
            "Display Title can't be blank.");
      }
  ...
  protected void ValidateFieldLength(
    ...
    if (fieldValue.trim().length() > length) {
      errors.rejectValue(
          fieldName, "field.too.large", new Object[] {fieldName, length}, "field too large");
    }
  }
  ...
  protected Map<String, String> buildCropBox(SizedImageMetaData simd) {
    ...
    if (simd.getHeight() > 0 && simd.getWidth() > 0) {
      Rectangle rect = new Rectangle(simd.getX(), simd.getY(), simd.getWidth(), simd.getHeight());
