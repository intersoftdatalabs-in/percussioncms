...
public interface ImageResizeManager {
  ...
  /**
   * Generates a resized and cropped image. The input image must be JPEG or other type supported by
   * the java.imagio package. The crop box and image size parameters are optional.
   *
   * @param input the input image. Never <code>null</code>.
   * @param cropBox the crop box determines how the image is cropped. If this parameter is <code>
   *     null</code>, the image is not cropped.
