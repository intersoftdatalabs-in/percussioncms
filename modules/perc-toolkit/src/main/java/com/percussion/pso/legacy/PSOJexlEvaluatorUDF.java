...
/**
 * A UDF that evaluates a JEXL expression. This can be used in XML query resources and in Default
 * Values where no existing UDF is available to generate the required output.
 *
 * <p>There is only one parameter to this UDF, the JEXL expression script itself, which must not be
 * blank or empty. The JEXL expression may reference the following variables:
 *
 * <ul>
 *   <li>$rx - the list of system defined JEXL functions
 *   <li>$user - the list of user defined JEXL functions
 *   <li>$tools - the Velocity Tools
 *   <li>$params - the HTML parameters associated with the calling request
 * </ul>
 *
 * The return value of the extension is the result of the JEXL script.
 *
 * @author DavidBenua
 */
