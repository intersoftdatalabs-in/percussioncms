...
/**
 * This slot finder returns all of the Active Assembly parents of the current item. The results can
 * be limited by Slot name. The parameters may be supplied in the usual ways: via the slot finder
 * parameters in the slot, or vai the template.
 *
 * <ul>
 *   <li><code>max_results</code> limits the total number of items returned in the slot
 *   <li><code>order_by</code> sorts the results by a field (or property). Use <code>ASC</code> and
 *       <code>DESC</code> to control direction of the sort.
 *   <li><code>limit_public</code> limits the results to items where the public revision has a
 *       relationship to this item. Supply any non-blank value.
 ...
 * @author DavidBenua
 */
public class PSOReverseSlotContentFinder extends PSBaseSlotContentFinder
  ...
  protected Set<SlotItem> getSlotItems(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Starting Reverse Slot Content Finder. Template={},source slot={},order by {},public {}",
          template,
          sourceSlotName,
          orderBy,
          limitToPublic);
    ...
    try {
      PSRelationshipProcessorProxy proxy =
          new PSRelationshipProcessorProxy(PSRelationshipProcessorProxy.PROCTYPE_SERVERLOCAL, req);
