...
public class PSNavLandingPageGeneratorEffect extends PSNavAbstractEffect {
  ...
  public static String DEFAULT_LANDING_DISPLAYTITLE_FORMAT =
      "navon.landingpage.default.displaytitle.format";
  public static String DEFAULT_LANDING_REQUIRED_FIELD_NAMES =
  ...
  private void initParams(Object[] params) {
    ...
    if (params != null) {
      ...
      if (params[0] != null) this.setDefaultContentType(params[0].toString());

      if (params[1] != null) this.setDefaultLandingTitleTemplate(params[1].toString());

      if (params[2] != null) this.setDefaultLandingDisplayTitleField(params[2].toString());

      if (params[3] != null) this.setDefaultLandingDisplayTitleFormat(params[3].toString());

      if (params[4] != null) this.setDefaultLandingRequiredFields(getCSVList(params[4].toString()));

      if (params[5] != null) this.setDefaultLandingRequiredValues(getCSVList(params[5].toString()));

      if (params[6] != null) this.setDefaultLandingCommunityId(Integer.parseInt("0" + params[6]));
  ...
  private void addNewLandingPage(
      IPSRequestContext req, PSNavRelationshipInfo info, PSEffectResult result) {
    ...
    try {
      ...
      if (navConfig.getNavonTypeIds().contains(dep.getContentTypeId())) {
        ...
        PSAaRelationshipList slotContents =
            PSNavUtil.getSlotContents(req, info.getRelation().getDependent(), landingSlot);
        ...
        if (slotContents.isEmpty()) {
          ...
          try {
            lpLoc =
                createLandingPage(
                    req, info.getOwner(), info.getDependent(), m_defaultLandingCommunityId);
          } catch (Exception e) {
      ...
      log.error(
          "Unable to process NavOn slots for NavOn: {} Error: {}",
          dep.getContentId(),
          e.getMessage());
    ...
    try {
      ...
      if (communityId
          != req.getSecurityToken()
              .getCommunityId()) { // the user is in a different community from the parent navon
      ...
      IPSFieldValue titleValue =
          new PSTextValue(makeLPTitle(req, folder, m_defaultLandingTitleTemplate));
      log.debug("New Landing Page name is {}", titleValue.getValueAsString());
      setFieldValue(lp, "sys_title", titleValue);

      IPSFieldValue displaytitleValue =
          new PSTextValue(makeLPTitle(req, folder, m_defaultLandingDisplayTitleFormat));
      ...
      relProxy.add(
          ...
          Collections.singletonList(lpLoc),
  ...
  private void createNavOnLandingPageRelationship(
    ...
    try {
      ...
      log.debug(
          "An exception occurred while looking up the landing page slot: {}", landingSlotName, e1);
    ...
    if (slotTempsAndCTs == null || slotTempsAndCTs.isEmpty()) {
      throw new PSNavException(
          "Unable to locate a default template for the landing page" + landingSlotName);
    } else {
    ...
    try {
      ...
      throw new PSNavException(
          "Error adding Landing Page to NavOn slot for NavOn" + navon.getContentId(), e);
    }
  }

  private static String makeLPTitle(
      final IPSRequestContext req, final PSComponentSummary folder, String template) {
