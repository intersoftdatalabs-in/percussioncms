...
/**
 * An abstract base class for Template Expanders. Unlike the undocumented system class <code>
 * PSBaseTemplateExpander</code> this class allows selection of templates at the individual item
 * level. Subclasses must supply the {@link #findTemplates(IPSGuid, IPSGuid, IPSGuid, int,
 * PSComponentSummary, Node, Map)} method which generates the list of templates for the content
 * item.
 *
 * <p>In addition, there are 2 hook methods: {@link #preprocessResults(QueryResult, Map, Map)} and
 * {@link #postprocessContentList(List)}. These will be called before and after each content list.
 * They can be used by implementations that need to do caching or other processing at the content
 * list level. As with all <code>IPSExtension</code> implementations, there is only one instance of
 * each Template Expander. It is the implementers responsibility to use the hook methods in a
 * thread-safe manner.
 *
 * @author DavidBenua
 */
public abstract class AbstractTemplateExpander implements IPSTemplateExpander {
  ...
  public List<PSContentListItem> expand(
    ...
    int context = 0;
    if (deliveryctx != null) context = Integer.parseInt(deliveryctx);
    else if (ctx != null) context = Integer.parseInt(ctx);
    else throw new RuntimeException("Either sys_context or sys_delivery_context must be specified");
    try {
      ...
      while (riter.hasNext()) {
        ...
        List<IPSGuid> templateids =
            findTemplates(cguid, fid, siteg, context, sum, cnode, parameters);
   ...
   * @param contentNode the content node. Will be <code>null</code> if the <code>needsContentNode
   *     </code> property is <code>false</code>.
   ...
   */
  protected abstract List<IPSGuid> findTemplates(
      IPSGuid itemGuid,
      IPSGuid folderGuid,
      IPSGuid siteGuid,
      int context,
  ...
  /**
   * Preprocess the query results. This method will always be called before the first call to <code>
   * findTemplates</code>. It may be used to replace or add columns to the result set, or as a
   * general hook for preprocessing.
   *
   * @param results the query results. Never <code>null</code>
   * @param parameters the parameters passed to <code>expand</code>. Never <code>null</code> or
   *     <code>empty</code>
   * @param summaryMap the component summary map.
   * @return the query results. Must not be <code>null</code>
   */
  protected QueryResult preprocessResults(
      QueryResult results,
      Map<String, String> parameters,
      Map<Integer, PSComponentSummary> summaryMap) {
    return results;
  ...
  /**
   * PostProcess the content list. This method will always be called after the last call to <code>
   * findTemplates</code>. It may be used to modify the resulting list of content list items, or to
   * perform any cleanup required after the content list is completed.
   *
  ...
  /**
   * Builds the map of Node objects. The nodes may be used to access the properties of the content
  ...
  protected Map<IPSGuid, Node> buildNodeMap(
    ...
    //       }
    // however, the result set does not contain the revision, so we have to do something a bit more
    // complex:
