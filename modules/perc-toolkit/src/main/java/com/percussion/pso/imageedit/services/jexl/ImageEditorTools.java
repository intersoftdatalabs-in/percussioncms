...
public class ImageEditorTools extends PSJexlUtilBase implements IPSJexlExpression {
  ...
  /**
   * Gets an image size definition based on the code. Exposes underlying service functionality to
   * JEXL.
   *
   * @param code the size code
   * @return the size defintion or <code>null</code> if the size is not defined.
   */
  @IPSJexlMethod(
  ...
  /**
   * Gets a child node based on the size code. The child node name and size code property name are
   * found from the <code>ImageSizeDefinitionManager</code>
   *
  ...
  @IPSJexlMethod(
      ...
      params = {
        @IPSJexlParam(name = "itemNode", description = "item node"),
        @IPSJexlParam(
            name = "sizecode",
  ...
  public InputStream getFailureImageStream() throws Exception {
    ...
    try {
      ...
      log.error("Configuration error: Unable to find failure image {}", path, ex.getMessage());
