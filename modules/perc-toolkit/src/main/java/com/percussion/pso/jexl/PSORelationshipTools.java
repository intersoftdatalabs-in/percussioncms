...
public class PSORelationshipTools extends PSJexlUtilBase implements IPSJexlExpression {
  ...
  @IPSJexlMethod(
      description = "get the dependents of this item of a certain content type",
      params = {
        @IPSJexlParam(name = "itemId", description = "the item GUID"),
        @IPSJexlParam(
            name = "contenttypename",
            type = "String",
            description = "the name of the content type we are testing for"),
        @IPSJexlParam(
            name = "userName",
            type = "String",
  ...
  public List<PSItemSummary> getRelationships(
    ...
    try {
      ...
      dependents = cws.findDependents(itemId, filter, false);
    } catch (Exception e) {
  ...
  @IPSJexlMethod(
      description = "find all parent items",
      params = {
        @IPSJexlParam(name = "guid", description = "the current item guid"),
  ...
  @IPSJexlMethod(
      description = "find all parent items",
      params = {
        @IPSJexlParam(name = "contentid", description = "the content id of the current item"),
        @IPSJexlParam(name = "slotName", description = "the slot name")
      },
      returns = "a list of all parent GUIDs")
  /**
   ...
   * Navons in QuickEdit will not consider the relationships of the current revision, only the
   * relationships of the last public revision.
   *
  ...
  /**
   * Is this page referenced in the landing page slot in the current folder. Will return true if the
   * specified content id is referenced in a landing page slot of the navon in the current folder
   *
   * @param contentid the content id.
   ...
   */
  @IPSJexlMethod(
      description = "is this page referenced in the landing page slot in the current folder",
      params = {
        @IPSJexlParam(name = "contentid", description = "the content id of the current page"),
  ...
  public boolean isLandingPageInFolder(String contentid, String folderid) throws Exception {
    ...
    if (parents.isEmpty()) { // no parents at all
      return false;
    }
    ...
    for (PSLocator loc : parents) {
      if (navonId == loc.getId()) { // navon id matches parent id
  ...
  /**
   * Checks if an item has any non-public ancestors. Convenience method for {@link
   * #hasOnlyPublicAncestors(String, String, String)}
   *
  ...
  @IPSJexlMethod(
      description = "Does this item have any non-public ancestors",
      params = {
        @IPSJexlParam(name = "contentId", description = "content id for item"),
        @IPSJexlParam(name = "slotName", description = "slotName"),
        @IPSJexlParam(
            name = "validFlags",
