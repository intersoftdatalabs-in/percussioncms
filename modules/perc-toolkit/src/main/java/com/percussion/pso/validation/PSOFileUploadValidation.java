...
/**
 * A Percussion CMS post-exit to perform content field validation. This exit uses the same code as
 * in in the com.percussion.extensions.general.PSFileInfo class.
 *
 * @author Roy Kiesler
 * @version 1.0
 */
public class PSOFileUploadValidation implements IPSFieldValidator {
   ...
   *           sys_File control. Cannot be null.
   *       <li><code>maxSize</code> - the maximum allowed file size (in bytes); (-1) means
   *           unlimited.
   *       <li><code>mimeTypes</code> - comma-separated list of allowed Mime types, e.g., text/html,
   *           image/jpeg, etc.
   *     </ul>
   *
   * @param request the request context object
   * @return the result document, if no validation errors encountered; otherwise, an instance of
   *     PSItemErrorDoc.
   ...
   *     message code.
   *     <p>An example of usage in a content editor definition follows: <code>
   * &lt;FieldRules&gt;<br/>
    ...
    if (params.length >= 2 && params[1] != null) {
      ...
      if (StringUtils.isBlank(mxSizeParam)) {
        ...
        try {
          ...
          throw new PSConversionException(0, "Invalid file size parameter.");
        }
    }
   ...
   * @throws PSConversionException if an error occurred during data conversion. This exception takes
   *     two parameters, a message code and an argument. You should always pass in zero (0) for the
   *     message code. to the error document
   */
  private boolean doValidation(IPSRequestContext request) throws PSConversionException {
   ...
   *     message code.
   */
  private boolean doFileSizeValidation(IPSRequestContext request) throws PSConversionException {
    ...
    if (maxFileSize != (-1)) {
      ...
      if (clearFlag != null && clearFlag.trim().length() > 0) return true;
   ...
   *     message code.
   */
  private boolean doFileMimeValidation(IPSRequestContext request) throws PSConversionException {
