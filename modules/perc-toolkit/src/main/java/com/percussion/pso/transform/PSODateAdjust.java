...
/**
 * A field input transform for setting one date field based on another. This works like the standard
 * date adjust, except that it takes its source from a different field.
 *
 * <p>If the source date is blank (or not specified), then the current date and time are used.
 *
 * @author davidbenua
 */
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    if (StringUtils.isNotBlank(sourceFieldName)) {
      ...
      if (StringUtils.isNotBlank(sourceFieldValue)) {
        sourceDate = PSDataTypeConverter.parseStringToDate(sourceFieldValue);
