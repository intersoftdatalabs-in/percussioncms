...
/**
 * Support routines for processing Content Item XML documents. These documents are generated by the
 * server in response to an OpenItem or ViewItem. These routines are likely to be useful in
 * validation and post-exits.
 *
 * @author DavidBenua
 */
public class PSOItemXMLSupport {
   ...
   * @return the XML element that represents the field. Will be <code>null</code> if the field does
   *     not exist.
   */
  public Element getFieldElement(Document inputDoc, String fieldName) {
    ...
    Element field =
        fieldWalker.getNextElement("DisplayField", PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (field != null) {
      ...
      if (control != null) {
        ...
        if (StringUtils.isNotBlank(fld) && fld.equals(fieldName)) { // the field name matches
          return field;
        }
      }
   ...
   * @return the value. Will be <code>null</code> if there is no &lt;Value&gt; node in the field
   *     element.
   ...
   *
   * @param field the field element
   * @return the label. Will be <code>null</code> if there is no label or the field is <code>null
   *     </code>.
   */
  public String getFieldLabel(Element field) {
    if (field != null) {
      PSXmlTreeWalker w = new PSXmlTreeWalker(field);
      Element l = w.getNextElement("DisplayLabel", PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      if (l != null) {
  ...
  public List<String> getFieldValues(Element field) {
    ...
    Element entry =
        w.getNextElement(
            "Control/DisplayChoices/DisplayEntry", PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (entry != null) {
