...
/**
 * This class provides an API for reading and writing the contents of a slot as a List of
 * PSAaRelationships. The content Web services API provides methods for reading and writing
 * individual relationships, or all of the relationships for a given parent, but it does not provide
 * a way to view and modify the contents of a specific slot. This is accomplished by wrapping the
 * legacy Active Assembly Processor Proxy using objects that are consistent with the Java service
 * API. These methods should only be used when the Java Service API cannot be used for some reason.
 *
 * @author DavidBenua
 */
public class PSOSlotRelations {
  ...
  /**
   * Saves a list of relationships. These relationships have already been persisted at least once,
