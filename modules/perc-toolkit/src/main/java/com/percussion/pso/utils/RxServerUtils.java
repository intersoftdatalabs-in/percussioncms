...
public class RxServerUtils {
  ...
  /**
   * Wait for the server to be ready. When initializing or performing actions from a service on
   * startup, the Percussion CMS server may not be ready. This routine polls for the server run lock
   * file on 10 second intervals.
   *
   * <p>This routine should be called from a new thread only, it should not block the server
  ...
  public static String getRhythmyxHome() {
    ...
    if (env.containsKey(PERCUSSION_HOME)) return env.get(PERCUSSION_HOME);
    else return null;
  }
}
