...
public class PSOSwitchCommunityWorkflowAction implements IPSWorkflowAction {
  ...
  public static final String PARAM_BASE =
      "com.percussion.pso.workflow.PSOSwitchCommunityWorkflowAction";
  ...
  public void performAction(IPSWorkFlowContext wfCtx, IPSRequestContext request)
    ...
    if (targetCommunity == null) {
      log.warn(
          "No "
              ...
              + " property configured for the folder: "
              + folderid
              + " , skipping Community Switching action");
    ...
    if (comm == null) {
      log.warn(
          "Unable to load community {} , skipping Community Switching action", targetCommunity);
      return; // Do nothing further as there is no community configured.
    }
   ...
   */
  private boolean testCommunityVisibility(
      IPSWorkFlowContext wfCtx, IPSGuid communityId, String user, String session) {
    ...
    try {
      ...
      List<PSCommunityVisibility> v =
          secSvc.getVisibilityByCommunity(comms, PSTypeEnum.WORKFLOW, user, session);
  ...
  private String getDefaultCommunityProperty(int folder_id) {
    ...
    if (props.containsKey(DEFAULT_COMMUNITY_NAME)) {
      ...
      log.info(
          "Default community identified in parent folder {}. Community: {}", folder_id, community);
      return community;
    }
  ...
  private Map<String, String> getFolderProperties(int id) {
    ...
    try {
      ...
      log.error(
          "Error looking up path for folder: {} Error: {}",
          id,
