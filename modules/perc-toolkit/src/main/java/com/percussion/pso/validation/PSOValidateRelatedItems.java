...
/**
 * Purpose of this class is to validate the existance of items in a slot. Through an internal
 * resource pointing to a query resource this class retrieves a xml dom doc and locates the slot
 * names occuring therein. It then checks it against the parameters passed to the
 * processResultDocument.
 *
 * <p>Parameters must be passed in the following order:
 *
 * <ol>
 *   <li>url to query resource
 *   <li>transitionid against which to validate
 *   <li>variantid of the parent document to validate
 *   <li>slotname of required items
 *   <li>occurance parameters MUST be one of the following: "MORE_THAN" or "LESS_THAN" or "EXACTLY"
 *   <li>number to correspond to parameter 5
 * </ol>
 *
 * Read 3 ,4, 5 and 6 as follows: VARIANTID with SLOT_NAME occurs MORE_THAN 5
 *
 * <p>To accomodate more validations on the related item simple repeat 3, 4 , 5 and 6 arguments.
 *
 * <p>In the latest version, the URL and Variant id are not longer needed, as this version uses the
 * Relationship API rather than making an internal request.
 *
 * @author Scott M. Morales
 * @version 1.1 - added the ability to wild card the transitionid. It will check for a number and
 *     act on that transition only or a * and act on all.
 * @version 2.0 - rewritten to use relationship API instead of making internal requests. The URL and
 *     VariantId are no longer used.
...
public class PSOValidateRelatedItems extends PSDefaultExtension
  ...
  /**
   * Check to see if the slotName exists in the map. If it does, makes a quick check against the
   * slotName mustOccur numoftimes values.
   *
   * @param slotSize - the actual size of the slot.
  ...
  protected boolean isValid(int slotSize, String mustOccur, int numoftimes)
    ...
    if (mustOccur.equalsIgnoreCase(MORE_THAN)) return slotSize > numoftimes;
    else if (mustOccur.equalsIgnoreCase(LESS_THAN)) return slotSize < numoftimes;
    else if (mustOccur.equalsIgnoreCase(EXACTLY)) return slotSize == numoftimes;
    else {
  ...
  /**
   * Required method implemented by interface IPSResultDocumentProcessor This is the method which is
   * called by the rx server. It uses the Object[] params index 0 to get the location of a query
   * resource, then uses the IPSRequestContext to make an internal request to that resource which
   * returns a Document. It then uses that Document and compares it against what has been passed in
   * with the params Object[]. If the comparison returns false this method returns a error page to
   * the user.
   *
   * <p>The Document must have NODE_TO_CHECK node. Its NODE_TO_CHECK node must have the ATT_TO_CHECK
   * attribute.
   *
   * <p>It uses the params Object[] to determine the location of the query resource, transitionid on
   * which to act, the slot name to check, how it should check the slot name and for how many items.
   *
   * <p>EXAMPLE:<br>
   * Parameters coming in should look like this: sys_AssemblerInfo/sys_AssemblerInfo<br>
   * 1<br>
   * 312<br>
   * Behind_Scenes_Index_Slot<br>
   * MORE_THAN<br>
   * 3<br>
   * 312<br>
   * Behind_Scenes_Index_Slot<br>
   ...
   * 9<br>
   *
   * <p>The first param is the location to the query resource. Not used, included for backwards
   * compatibility.<br>
   * The second param is the transition id on which to act.(can use * for act on all transitions)
   * <br>
   * The third and the seventh param is the Variant which holds the slot to be checked. Not used,
   * included for backwards compatibility.<br>
   * The fourth and the eigth is the Slot Name to check.<br>
   * The fifth and ninth is how to check that slot.<br>
    ...
    if (matchTransitionId(request, transParam)) {
      ...
      try {
        ...
        for (int i = 2; i < params.length; i += 4) {
          ...
          if (slotName == null || mustOccur == null || numOfTimes == null) {
            logger.error(
                "Unable to validate parameters " + varId + slotName + mustOccur + numOfTimes);
            throw new PSParameterMismatchException("Incorrect Parameters for Slot Validation");
          }
          ...
          for (PSAaRelationship rel : relations) {
            if (slotName.equalsIgnoreCase(rel.getSlotName())) { // we found one
              slotSize++;
            }
          }

          logger.debug("Slot Size " + slotSize + " must be " + mustOccur + " number " + numTimes);
          if (!isValid(slotSize, mustOccur, numTimes)) {
            ...
            PSItemErrorDoc.addError(
                errDoc, "Related Content", "Related Content", MSG_SLOT_VALIDATION, msgargs);
  ...
  private static final String MSG_SLOT_VALIDATION =
      "Slot name {0} must have {1} {2} related items.";

  /**
   * Validates the parameters passed into the processResultDocument method. The sixth parameter must
   * be either AND or OR inorder to validate additional slots.
   *
   * @param args the params to be validated.
   */
  private void validateParams(Object[] args) throws PSParameterMismatchException {
    ...
    if (args.length < 6) {
      throw new PSParameterMismatchException("At least 6 arguments are required for this exit.");
    }
    ...
    if (transId != null && !transId.equals("*")) {
      try {
        ...
        throw new PSParameterMismatchException("Transition id must be a number.");
      }
    }
    ...
    for (int i = 2; i < args.length; i += 4) {
      ...
      if (numOfTimes != null) {
        try {
          ...
          throw new PSParameterMismatchException(
              "Required argument numOfTimes is not a number: Value is " + numOfTimes);
        }
      } else {
        throw new PSParameterMismatchException("Required argument numOfTimes is null");
      }
      if (varId == null) throw new PSParameterMismatchException("Required argument varId is null");
      if (slotName == null)
        throw new PSParameterMismatchException("Required argument slotName is null");
      if (mustOccur == null)
        throw new PSParameterMismatchException("Required argument mustOccur is null");
      if (numOfTimes == null)
        throw new PSParameterMismatchException("Required argument numOfTimes is null");
      ...
      if ((!mustOccur.equals(MORE_THAN))
        ...
        throw new PSParameterMismatchException(
            "Usage: MORE_THAN, LESS_THAN, EXACTLY;  What occured: " + mustOccur);
    }
  }

  /**
   * Checks if the current transition matches a specified value. This routine is intended for use in
   * validation exits where a specific transition id is provided as a parameter.
   *
   * <p>If the parameter supplied is an "*", then this method always returns true. Otherwise, the
   * parameter is compared with the <code>sys_transitionid</code> HTML parameter and then of the
   * <code>WFAction</code> HTML parameter. If either of these values match, then <code>true</code>
   * is returned.
   *
   * @param request the callers request context.
   * @param transParameter the match parameter to be compared. Use "*" to match any transition.
   * @throws IllegalArgumentException when the <code>transParameter</code> is <code>null</code>.
   * @return <code>true</code> if the transition matches, <code>false</code> otherwise.
   */
