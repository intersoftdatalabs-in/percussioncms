 ...
 * not removed from any current folders, only added to new ones.
 *
 * @author DavidBenua
 */
...
public class PSFolderOwnerSubfolderEffect extends PSAbstractFolderEffect {
  ...
  protected String findOrCreateChildFolder(String folderPath, String subfolderPath, String userName)
      throws Exception {
    if (StringUtils.isBlank(subfolderPath)) { // no sub path, just return the existing folder.
      return folderPath;
    }
  ...
  public void attempt(
    ...
    // TODO: Replace deprecated isPreConstruction()/isPreUpdate() with context type check when
    // available
