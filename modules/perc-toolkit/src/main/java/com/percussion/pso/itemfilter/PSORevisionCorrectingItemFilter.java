...
/**
 * Corrects revisions of items in Staging states where the current revision is desired.
 *
 * <p>This filter determines if an item is in one of the named states set within its parameters. If
 * the item is in one of the states, the <code>Current</code> revision will be used in place of the
 * <code>Public</code> revision.
 *
 * @author DavidBenua
 */
public class PSORevisionCorrectingItemFilter extends PSBaseFilter implements IPSItemFilterRule {
  ...
  public List<IPSFilterItem> filter(List<IPSFilterItem> items, Map<String, String> params)
    ...
    List<String> wfStates = SimplifyParameters.getValueAsList(params.get(WORKFLOW_STATES));
