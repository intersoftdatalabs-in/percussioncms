...
public class PSOCopyParameter implements IPSItemInputTransformer, IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    int fieldSize = -1;
    if (StringUtils.isNotBlank(fieldSizeString) && StringUtils.isNumeric(fieldSizeString)) {
    ...
    if (s != null && (!doNotCopyEmpty || s.length() > 0)) {
      if (!onlyIfEmpty.equals("true") || d == null || d.trim().length() == 0) {
        if (fieldSize > 0 && (s.length() > fieldSize)) {
          s = s.substring(0, fieldSize);
        }
        if (cleanup) {
          s =
              PathCleanupUtils.cleanupPathPart(
                  s, forceLower, includesExtension, stripExtension, prefix, suffix, forceExtension);
        }
