...
public interface IPSOWorkflowInfoFinder {
   ...
   * @param state the state id.
   * @return the state or <code>null</code> if the state does not exist.
   ...
   * @return the workflow state or <code>null</code> if the state does not exist.
   * @throws PSException when the content item is not found.
   * @throws IllegalArgumentException when the contentid is not numeric, or when the workflow id of
   *     the item is invalid.
   */
  ...
  /**
   * Finds a workflow transition from the workflow id and transition id. This does NOT including
   * aging transitions. Convenience method for {@link #findWorkflowTransition(PSWorkflow, int)}.
   *
  ...
  /**
   * Finds any workflow transition from the workflow id and transition id. This includes aging
   * transitions. Convenience method for {@link #findWorkflowAnyTransition(PSWorkflow, int)}.
   *
   * @param workflow the workflow id. Must be a valid workflow.
   * @param transid the transition id.
   * @return the transition or <code>null</code> if no transition for this id.
  ...
  /**
   * Is the workflow state of an item one of the valid ones. The content item state contains a
   * single valid flag. The method checks that this flag is one of the listed ones, comparing with a
