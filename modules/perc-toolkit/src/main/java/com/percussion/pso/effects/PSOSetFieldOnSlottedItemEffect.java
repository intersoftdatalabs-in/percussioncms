...
public class PSOSetFieldOnSlottedItemEffect implements IPSEffect {
  ...
  public void init(IPSExtensionDef def, File f) throws PSExtensionException {
    ...
    for (Iterator<String> paramsIter = def.getInitParameterNames(); paramsIter.hasNext(); ) {
    ...
    if (context.isPostConstruction() || context.isPostDestruction()) {
      ...
      if (rel.isActiveAssemblyRelationship()) {
        ...
        if (relParams.slotid == configParams.slotId) {
          ...
          IPSGuid guid = getGuidManager().makeGuid(rel.getOwner());

          List<IPSGuid> guids = Collections.singletonList(guid);
          try {
            // Load the owner item.
            List<PSCoreItem> items = getContentService().loadItems(guids, true, true, false, false);

            if (items != null && items.size() > 0) {
              // Should only be one item.
              PSCoreItem item = items.get(0);

              if (context.isPreDestruction()) log.debug("Pre-Destruction");
              else log.debug("Pre-Construction");
              ...
              if (context.isPostConstruction()) {
                ...
                // If this is destruction then we need to see if there are any more items in the
                // slot.
                boolean found = false;
                try {
                  ...
                  for (PSAaRelationship r : mCws.loadContentRelations(filter, true)) {
                    if (r.getId() != rel.getId()) {
                      if (r.getSlotName().equals(configParams.slotName)) {
                        updateField(
                            item, configParams.fieldName, configParams.valueIfNotEmpty + "");
                        found = true;
                        log.debug(
                            "Found item in slot, setting {} to {}",
                            configParams.fieldName,
                            configParams.valueIfNotEmpty);
                        break;
                      }
                    }
                  }
              ...
              log.error(
                  "Unable to locate owner for relationship. Content ID: {} not found!",
                  rel.getOwner().getId());
            }
  ...
  private IPSFieldValue getFieldValue(PSItemField field, Object value) {
    ...
    if (dataType.equals(PSField.DT_BOOLEAN)) {
      ...
      if (value.equals("true") || value.equals("yes") || value.equals("1")) {
        ...
        || (fieldMeta.getBackendDataType() == PSItemFieldMeta.DATATYPE_NUMERIC)) {
  ...
  private void saveItem(PSRequestContext request, PSLocator loc, PSCoreItem item)
