...
public class PSFolderFollowerEffect extends PSAbstractFolderEffect implements IPSEffect {
  ...
  protected void addMissing(PSLocator item, List<PSRelationship> missingList)
    ...
    for (PSRelationship missing : missingList) {
      ...
      sws.createRelationship(PSRelationshipFilter.FILTER_NAME_FOLDER_CONTENT, folderGuid, itemGuid);
    }
  }
  ...
  /**
   * Subtract a list of relationships from another list of relationships. The relationships are
   * subtracted if a relationship exists with the same owner content id.
   *
   ...
   */
  protected List<PSRelationship> subtractRels(
