...
/**
 * Implementation of the PSOWFActionService. This service is designed to be called from a workflow
 * action dispatcher. It will return the list of IPSWorkflowActions defined for a particular
 * workflow transition.
 *
 * <p><b>Note:</b> unlike the original PSOWFActionDispatcher which matched on workflow id and
 ...
 * </pre>
 *
 * <p>A sample file can be found at
...
public class PSOWFActionService implements IPSOWFActionService {
        ...
        new Thread() {
          ...
          public void run() {
            try {
              ...
              initServices();
            } catch (InterruptedException ex) { // should only happen if the server startup fails.
   ...
   */
  public void setTransitionActions(Map<String, Map<String, List<String>>> transitionActions) {
