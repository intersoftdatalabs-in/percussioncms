...
/**
 * The Image Persistence Manager is used for loading and saving images to/from the Rhythmyx server.
 *
 * <p>All of these methods MUST be called in the user request thread, and the user's Rhythmyx
 * session id and user name must be supplied. These values can be obtained from the <code>
 * RxRequestUtils</code> class in the PSOToolkit.
 *
 * @author DavidBenua
 */
public interface ImagePersistenceManager {
  ...
  /**
   * Opens an existing image. If necessary, the image will be checked out to the current user, and
   * the edit revision will be returned.
   *
   ...
   *
   * <p>The item will be restored to the previous state, according to the settings in the item
   * status parameter. If the item status is missing, the item will be checked in, but no further
   ...
   *
   * @param image the image metadata. Must not be <code>null</code>.
   * @param folderid the folder id where the item will be created. If this is <code>null</code> then
   *     the item will not be added to any folder.
   ...
   * @throws Exception when a server error occurs.
   */
  public String CreateImage(MasterImageMetaData image, String folderid, boolean checkin)
      throws Exception;
