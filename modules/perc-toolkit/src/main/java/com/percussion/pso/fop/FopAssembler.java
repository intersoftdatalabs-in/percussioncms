...
/**
 * A Rhythmyx Assembly plugin for FOP processing. This plugin extends the standard Velocity
 * assembler. The template produces XML in FO format, and then calls the FO renderer to produce a
 * PDF.
 *
 * <p>The Assembler finds the FOP Properties file in <code>
 * &lt;RhythmyxRoot&gt;/rxconfig/Server/fop.config</code>. This file contains the default settings
 * for the FOP processor.
 *
 * <p>Metadata can be added to the PDF file with mappings in the <code>$FOP</code> binding variable.
 ...
 * </ul>
 *
 * <p>The FOP Assembler can used to generate any of the formats that Apache FOP supports. If the
 * MIME type specified in the template is one of those supported by FOP, it will be generated.
 * Otherwise, the output will be in PDF format. Note that by default the Rhythmyx Workbench does not
 * support PDF and other such formats. These can be added to the <code>mimemapwb.properties</code>
 * file. Some additional configuration to the renderers found in <code>fop.config</code> may be
 * necessary for formats other than PDF.
 *
 * <p>For debugging purposes, users can preview the template adding the parameter "showxml" with any
 * non-blank value. This will cause the FOP Assembler to return the XML that would have otherwise
 * been passed to the Apache FO Processor.
 *
 * <p>See Instructions.pdf for further information.
 *
 * @author DavidBenua
 */
public class FopAssembler extends PSVelocityAssembler implements IPSAssembler {
  ...
  protected IPSAssemblyResult doAssembleSingle(IPSAssemblyItem item) throws Exception {
    ...
    if (StringUtils.isNotBlank(showXML)) {
      ...
      String xmlMime = "text/xml;charset=" + charSet.displayName();
  ...
  /**
   * Gets a new default User Agent. The values in the User Agent will be determined by the FOP
   * Properties used to initialize the FOP factory.
   *
  ...
  /**
   * Sets up the FOUserAgent metadata parameters based on the values included in the bindings. The
   * bindings in <code>$FOP</code> are evaluated and saved into the user agent
   *
  ...
  /**
   * Validate the MIME type of the calling template. If the type is one of the recognized list, it
   * will be returned unchanged. Otherwise, this function will return <code>application/pdf</code>
   *
  ...
  private static final IPSScript FOP_CREATOR =
      PSJexlEvaluator.createStaticExpression("$FOP.creator");
  private static final IPSScript FOP_PRODUCER =
      PSJexlEvaluator.createStaticExpression("$FOP.producer");
  private static final IPSScript FOP_KEYWORDS =
      PSJexlEvaluator.createStaticExpression("$FOP.keywords");
  private static final IPSScript FOP_TITLE = PSJexlEvaluator.createStaticExpression("$FOP.title");
  private static final IPSScript FOP_CREATIONDATE =
      PSJexlEvaluator.createStaticExpression("$FOP.creationDate");
  private static final IPSScript FOP_TARGETRESOLUTION =
      PSJexlEvaluator.createStaticExpression("$FOP.targetResolution");

  /**
   * Parameter value for showing XML. Specify any non-empty value for this HTML parameter, and the
   * assembler will return the XML that would have been passed to the FO Processor. Use this for
   * debugging templates.
   */
  public static final String SHOW_XML_PARAMETER = "showxml";

  /**
   * Extension Parameter for setting allowable MIME Types. Changing this parameters in the Extension
   * Registration allows the implementer to add other types supported by the FO Processor.
   */
  public static final String INIT_PARAMETER_MIME_TYPES =
      "com.percussion.pso.fop.SupportedMimeTypes";
}
