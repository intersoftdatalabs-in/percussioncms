...
public class PSOExtensionParamsHelper {
   ...
   * @see #getParameter(String)
   */
  public PSOExtensionParamsHelper(
      IPSExtensionDef def, Object[] params, IPSRequestContext request, Logger log) {
   ...
   */
  public PSOExtensionParamsHelper(
      Map<String, String> parameters, IPSRequestContext request, Logger log) {
  ...
  public String getParameter(String paramName) {
    ...
    if (slotSelectors != null && slotArguments != null) {
      ...
      if (val != null) {
        log.debug(
            "Got the parameter name = {} value = {} from the slot selectors.", paramName, val);
      } else {
        val = slotArguments.get(paramName);
        if (val != null) {
          log.debug(
              "Got the parameter name = {} value = {} from the slot arguments.", paramName, val);
        }
      }
    ...
    if (extensionParameters != null) {
      ...
      log.debug(
          "Got the parameter name = {} value = {} from the extension parameters.",
          paramName,
          value);
      return value;
    }
  ...
  public Number paramToNumber(String paramName, String param) {
    try {
      ...
      String message = "Parameter " + paramName + " is not a number. " + "Param value=" + param;
      log.error(message, e);
  ...
  private void doParameters() {
    ...
    if (params != null) {
      ...
      while (names.hasNext()) {
        ...
        } else extensionParameters.put(name, null);
