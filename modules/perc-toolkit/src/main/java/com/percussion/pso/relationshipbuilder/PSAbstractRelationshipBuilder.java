...
public abstract class PSAbstractRelationshipBuilder implements IPSRelationshipBuilder {
   ...
   */
  protected static Set<Integer> createComplement(
      final Set<Integer> retain, final Set<Integer> suppress) {
  ...
  public void synchronize(int sourceId, Set<Integer> targetIds)
    ...
    if (currentRelatedIds.isEmpty()) {
      ...
      // desired - current = add
      Set<Integer> idsToAdd = createComplement(targetIds, currentRelatedIds);
      log.debug("\tadd ids: {}", idsToAdd);
      add(sourceId, idsToAdd);
      // current - desired = remove
      Set<Integer> idsToRemove = createComplement(currentRelatedIds, targetIds);
  ...
  /**
   * Adds new relationships to sourceId. Relates sourceId to targetIds. Should ONLY ADD AND NOT
   * DELETE relationships. Override this class to get relationship synchronization working.
   *
  ...
  /**
   * Deletes relationships from sourceId to targetIds. Should ONLY DELETE AND NOT ADD relationships.
