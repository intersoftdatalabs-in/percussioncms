...
/**
 * Binding functions usefule for Navigation. These functions are intended to be called from JEXL.
 *
 * @author DavidBenua
 */
public class PSONavTools extends PSJexlUtilBase implements IPSJexlExpression {
  ...
  private static void initServices() {
    ...
    if (proxy == null) {
      try {
        ...
        log.error(
            "Unexpected Exception initializing proxy Error: {}",
   ...
   * <code>Root</code> node, there will be only one node in the list.
   *
   * <p>This function can be used to generate breadcrumbs. Here is a simple example:
   *
   * <pre>
  ...
  /**
   * Finds the navon or navtree in a given folder. The navon node is a regular content node, it is
   * not a nav proxy node: the extra nav attributes are missing.
   *
  ...
  /**
   * Finds the for a current navon. Convenience method for findParentNavonNode(PSLocator). Note that
   * the Node contains the content data only, the extra Navon proxy properties are not included in
   * this node.
   *
  ...
  /**
   * Finds the for a current navon. Note that the Node contains the content data only, the extra
   * Navon proxy properties are not included in this node.
   *
  ...
  /**
   * Gets the nearest property value for a named property. This method checks each navon for a value
   * which is non-null and not empty. If the value is defined on the current navon is it is
   * returned, and if not, the parent navon is examined, and so on until a non-empty value is
   * encountered. This method will return <code>null</code> if no values are defined.
   *
