...
public class ImagePersistenceManagerImpl extends ImageItemSupport
  ...
  public boolean validateSystemTitleUnique(String sysTitle, String folderId) throws Exception {
    ...
    if (StringUtils.isEmpty(folderId)) { // no folder id, no problem...
      return true;
    }
    ...
    for (PSItemSummary item : items) {
      if (sysTitle.equalsIgnoreCase(item.getName())) { // we found a matching title
        log.debug(
            "validateSystemTitleUnique: item name matches: "
  ...
  public OpenImageResult OpenImage(String contentid) throws Exception {
    ...
    for (PSItemChildEntry entry : getChildEntries(contentid)) {
      ...
      log.debug(
          "OpenImage: read the data into the sized image ["
              + sized.getSizeDefinition().getCode()
  ...
  public void UpdateImage(MasterImageMetaData image, String contentid, PSItemStatus itemStatus)
    ...
    try {
      ...
      log.debug(
          "UpdateImage: dealing with sized images - there are "
              + image.getSizedImages().size()
              + " image(s)");
      for (SizedImageMetaData sized : image.getSizedImages().values()) {
        String sizeCode = sized.getSizeDefinition().getCode();
        log.debug(
            "UpdateImage: in the for loop - trying to find child entry for sized: {}", sizeCode);
      ...
      for (Map.Entry<IPSGuid, Object> entry : errors.entrySet()) {
        log.error("Error for GUID " + entry.getKey() + " : " + entry.getValue());
      }
      throw ee;
    } catch (Exception ex) {
