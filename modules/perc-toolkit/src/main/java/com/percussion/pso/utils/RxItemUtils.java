...
public class RxItemUtils {
   ...
   * @return the value of the field. Never <code>null</code> may be <code>empty</code>
   ...
   * @param item the item or child to get the field from
   * @param fieldName the field name
   * @return the field value as a number. Will be <code>ZERO</code> if the field does not exist, has
   *     no values, or is not a number.
   * @throws PSCmsException when an error occurs
   */
  public static Number getFieldNumeric(IPSItemAccessor item, String fieldName)
      throws PSCmsException {
    PSItemField fld = item.getFieldByName(fieldName);
    if (fld == null) {
      log.debug("no such field " + fieldName);
