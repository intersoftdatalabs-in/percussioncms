...
public class PSOSlotTools extends PSJexlUtilBase implements IPSJexlExpression {
  ...
  public List<IPSAssemblyItem> getSlotContents(
    ...
    try {
      ...
      if (slot == null) {
        throw new IllegalArgumentException("slot not found, check template's slot reference");
      }
      ...
      ms_log.error(PSI18nUtils.getString("psx_assembly@Problem during assembly"), orig);
      throw e;
    }
  }

  /**
   * Gets all values for a named property across the contents of a slot. For each item in the slot,
   * the property is fetched, and if it exists on that item, its String value is appended to the
   * result.
   *
