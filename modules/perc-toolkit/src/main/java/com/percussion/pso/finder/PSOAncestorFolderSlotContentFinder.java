...
/**
 * Finds a single content item of a given content type in the current folder of the assembly item or
 * an ancestor folder.
 *
 * <p>The finder first looks in the current folder and if not found then proceeds up the folder tree
 * until an item with the given content type is found. If no item is found the slot will be empty.
 *
 * <p>The slot will always have either one or zero items.
 *
 * @author adamgent
 */
public class PSOAncestorFolderSlotContentFinder extends PSBaseSlotContentFinder {
   ...
   */
  public PSOAncestorFolderSlotContentFinder(IPSContentWs contentWs, PSOFolderTools folderTools) {
    super();
  ...
  public Set<SlotItem> getSlotItems(
    ...
    if (sum == null) {
      log.warn(
          "Did not find an ancestor item of type: {}  for item: {}",
          contentType,
          assemblyItem.getId());
      return new HashSet<>();
    } else if (log.isDebugEnabled()) {
      log.debug(
          "Found ancestor item of type: {} with id: {} for item: {}",
          contentType,
          sum.getGUID(),
          assemblyItem.getId());
    }
