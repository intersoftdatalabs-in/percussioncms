...
public class ListBuilderJexl implements IPSJexlExpression {
  ...
  private boolean hasFolderPaths(ListBuilderItem item) {
    ...
    return fp == null || fp.isEmpty() || (fp.size() == 1 && isBlank(fp.iterator().next()));
  }
  ...
  public JCRQueryBuilder getQueryBuilder(ListBuilderItem lbn) {
    ...
    if (contentTypes == null || contentTypes.isEmpty() && isNotBlank(contentType)) {
  ...
  public Map<String, String> getSlotParameters(ListBuilderItem lbn) {
    ...
    if (!MANUAL_SLOT_NAME.equals(lbn.getSlot())) params.put("template", lbn.getChildSnippet());
    if (lbn.getCount() != null) params.put("max_results", "" + lbn.getCount());
