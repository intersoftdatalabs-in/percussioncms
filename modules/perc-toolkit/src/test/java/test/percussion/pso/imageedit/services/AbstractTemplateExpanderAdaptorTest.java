...
public class AbstractTemplateExpanderAdaptorTest {
    ...
    public void testBuildNodeMap() throws Exception {
        ...
        nodelist.add(Mockito.mock(Node.class));
        Mockito.when(cmgr.findItemsByGUID(Mockito.anyList(), Mockito.any(PSContentMgrConfig.class))).thenReturn(nodelist);
    ...
    private QueryResult buildQueryResultExpectations() {
        var row1 = new PSRow(new HashMap<String, Object>() {{
        ...
        var row2 = new PSRow(new HashMap<String, Object>() {{
        ...
        var rows = new PSRowIterator(new ArrayList<PSRow>() {{
            add(row1);
            add(row2);
        }});
        ...
        var rowcomp = new PSRowComparator(new ArrayList<PSPair<String, Boolean>>() {{
            add(new PSPair<>(IPSContentPropertyConstants.RX_SYS_CONTENTID, true));
        }});
        var qr = new PSQueryResult(new String[] {
            ...
            IPSContentPropertyConstants.RX_SYS_FOLDERID }, rowcomp);
        qr.addRow(row1);
        qr.addRow(row2);
    ...
    private class TestableTemplateExpanderAdaptor extends AbstractTemplateExpanderAdaptor {
        @Override
        protected List<IPSGuid> findTemplates(IPSGuid itemGuid, IPSGuid folderGuid, IPSGuid siteGuid, int context,
                PSComponentSummary summary, Node contentNode, Map<String, String> parameters) {
            log.info("Item Guid " + itemGuid);
            return templateList;
        }
        @Override
        public Map<IPSGuid, Node> buildNodeMap(QueryResult result, Map<Integer, PSComponentSummary> summaryMap)
