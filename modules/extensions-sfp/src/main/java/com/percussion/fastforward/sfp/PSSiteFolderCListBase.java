...
public abstract class PSSiteFolderCListBase {
  ...
  public PSSiteFolderCListBase(
    ...
    if (publishableContentValidValues == null
      ...
      throw new IllegalArgumentException(
          "PSSiteFolderContentList's publishableContentValidValues " + "may not be null or empty");
    }
  ...
  private void initializeMembers(
    ...
    String includeMode = request.getParameter(IPSConstants.INCLUDE_FOLDERS);
    if (includeMode == null
        || includeMode.trim().length() == 0
  ...
  public synchronized Document buildContentList(
    ...
    String sys_publicationid = m_request.getParameter(IPSHtmlParameters.SYS_PUBLICATIONID);
    ...
    if (m_request != null && m_helper != null) {
      if (m_isIncremental)
        ...
        log.debug(
            "creating full content list with contentvalid= {}", m_publishableContentValidValues);
      ...
      if (folder != null) {
        ...
        generateContentItems(
            folderPath, folder.m_folderId, siteFolderPath, filenameContext, appendFolderName);
  ...
  private PublishFolder getPublishFolder(String publishFolder, String siteFolderPath)
    ...
    PSLocator rootLocator = m_helper.getFolderLocatorByPath(siteFolderPath);
    PSLocator folderLocator = null;
    if (siteFolderPath.equalsIgnoreCase(publishFolder)) folderLocator = rootLocator;
    else folderLocator = m_helper.getFolderLocatorByPath(publishFolder);
    ...
    // publish a sub folder of the site
      ...
      if (!m_helper.isFolderDescendent(rootLocator, folderLocator)) {
        log.warn(
            "The supplied folder path, '{}', is not a decendent of the site folder path, '{}'",
            publishFolder,
            siteFolderPath);
      } else {
        ...
        if (parentLocator.getId() == rootLocator.getId()) {
          return new PublishFolder(folderLocator.getId(), "/", rootLocator.getId());
        }
        ...
        String parentFolder = publishFolder.substring(0, publishFolder.lastIndexOf('/'));
        String relativeParentPath =
            parentFolder.substring(siteFolderPath.length(), parentFolder.length()) + "/";

        return new PublishFolder(folderLocator.getId(), relativeParentPath, rootLocator.getId());
      }
    }
  ...
  protected boolean doesIncludeQuickEditItems() {
    ...
    while (tokenizer.hasMoreTokens()) {
      ...
      if (element.length() > 0 && element.charAt(0) == PSContentListItem.CONTENTVALID_FLAG_I) {
        return true;
      }
    }
  ...
  private PSContentList getCachedContentList(String sys_publicationid) {
    return (PSContentList)
        m_request.getSessionPrivateObject(CONTENT_LIST_CACHE_KEY + sys_publicationid);
  }

  private void setCachedContentList(String sys_publicationid, PSContentList list) {
    log.debug("caching content list for publication id=" + sys_publicationid);
    m_request.setSessionPrivateObject(CONTENT_LIST_CACHE_KEY + sys_publicationid, list);
  }
  ...
  private void clearCachedContentList(String sys_publicationid) {
    ...
    m_request.setSessionPrivateObject(CONTENT_LIST_CACHE_KEY + sys_publicationid, null);
  }
  ...
  protected String getContentValidValues4SQL() {
    ...
    StringTokenizer tokenizer = new StringTokenizer(m_publishableContentValidValues, ",");
    boolean isFirst = true;
    while (tokenizer.hasMoreTokens()) {
      if (!isFirst) values.append(",");
      else isFirst = false;
  ...
  protected boolean isFolderExcluded(int folderId) {
    if (m_folderIncludeMode.equals(INCLUDE_MODE_ALL)) return false;
