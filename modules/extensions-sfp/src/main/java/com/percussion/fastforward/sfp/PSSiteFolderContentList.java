...
public class PSSiteFolderContentList extends PSSiteFolderCListBase {
  ...
  public PSSiteFolderContentList(
      ...
      int maxRowsPerPage) {
    super(request, isIncremental, publishableContentValidValues, maxRowsPerPage);
  }
  ...
  protected void generateContentItems(
      String parentPath,
      int folderId,
    ...
    PSComponentSummary root = m_helper.getComponentSummary(new PSLocator(folderId));

    appendFolderItems(parentPath, root, siteFolderPath, filenameContext, appendFolderName, false);
  }
  ...
  private void appendFolderItems(
    ...
    log.debug(
        "NEW FOLDER. Path={}, Folder={}, Site folder path={}, context={}",
        parentFolderPath,
        folder.getName(),
        siteFolderPath,
        filenameContext);
    ...
    if (appendFolderName) {
      ...
      folderPathBuf.append(PSSite.getFolderFileName(folder.getCurrentLocator()));
    ...
    boolean bExclude = !includeOverride && isFolderExcluded(folder.getContentId());
    boolean bOverrideInclude = false;
    if (bExclude) log.debug("Excluding: {}", folderPath);
    ...
    while (items.hasNext()) {
      ...
      if (item.getType() == PSComponentSummary.TYPE_ITEM) {
        ...
        appendFolderItems(
            folderPath, item, siteFolderPath, filenameContext, true, bOverrideInclude);
      }
    }
  ...
  private void appendItemVariants(
    ...
    try {
      isPublishable =
          PSCms.isPublishable(
              new Object[] {m_publishableContentValidValues, contentId, revision}, m_request);
    ...
    if (isPublishable) {
      ...
      if (variants.isEmpty()) {
        ...
        for (Iterator i = variants.iterator(); i.hasNext(); ) {
          ...
          if (m_isIncremental) {
            if (isValid(contentListItem)) m_contentList.addItem(contentListItem);
            else
              log.debug(
                  "this variant/location of {} does not need to be incrementally published",
                  contentId);
          } else {
  ...
  private boolean isValid(PSContentListItem contentItem) {
    ...
    String requestContext = m_request.getParameter(IPSHtmlParameters.SYS_CONTEXT);
    String requestSiteId = m_request.getParameter(IPSHtmlParameters.SYS_SITEID);
  ...
  private Set lookupVariantsForItem(
    ...
    if (folder != null) {
      ...
      lookupParams.put(IPSHtmlParameters.SYS_FOLDERID, String.valueOf(folderId));
    }

    IPSInternalRequest lookupRequest =
        m_request.getInternalRequest(m_contentResourceName, lookupParams, false);
    ...
    if (resultXml != null) {
      ...
      if (root != null) {
        ...
        for (int i = 0; (variant = (Element) variants.item(i)) != null; i++) {
          ...
          if (id != null && assembler != null) variantsSet.add(new Variant(id, assembler));
        }
      }
    }
    return variantsSet;
  ...
  private class Variant {
    ...
    public boolean equals(Object o) {
      if (o instanceof Variant) {
        Variant v = (Variant) o;
        return v.getId().equals(m_id) && v.getAssemblerBase().equals(m_assemblerBase);
      }
      return false;
    }
    ...
    public Variant(String variantid, String assemblerBase) {
      if (variantid == null || variantid.trim().length() == 0)
        throw new IllegalArgumentException("variant id may not be null or empty");
      if (assemblerBase == null || assemblerBase.trim().length() == 0)
        throw new IllegalArgumentException("variant assemblerBase may not be null or empty");
