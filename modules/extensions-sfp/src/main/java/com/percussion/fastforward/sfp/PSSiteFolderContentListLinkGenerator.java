...
public class PSSiteFolderContentListLinkGenerator {
  ...
  public static URL generateAssemblerLink(
    ...
    paramMap.put(
        IPSHtmlParameters.SYS_AUTHTYPE, request.getParameter(IPSHtmlParameters.SYS_AUTHTYPE, "0"));
    paramMap.put(
        IPSHtmlParameters.SYS_CONTEXT, request.getParameter(IPSHtmlParameters.SYS_CONTEXT, "0"));
    ...
    paramMap.put(IPSHtmlParameters.SYS_SITEID, request.getParameter(IPSHtmlParameters.SYS_SITEID));
    ...
    String navTheme = request.getParameter(PSSiteFolderContentList.NAV_THEME_PARAM_NAME);
    if (navTheme != null) paramMap.put(PSSiteFolderContentList.NAV_THEME_PARAM_NAME, navTheme);
    ...
    try {
      ...
      if (assembler.getProtocol().equals(protocol) == false) {
        assembler =
            new URL(protocol, assembler.getHost(), assembler.getPort(), assembler.getFile());
      }
      return assembler;
  ...
  public String generatePubLocation(
      ...
      String folderPath,
      IPSRequestContext request) {
    return generatePubLocation(contentId, revision, variantId, context, folderPath, null, request);
  }
  ...
  public String generatePubLocation(
    ...
    try {
      ...
      params[2] = revision;
      params[3] = context;
      if (folderId != null) params[5] = folderId;
  ...
  public URL generateNullLink(IPSRequestContext request) {
    try {
      ...
      URL assembler = PSUrlUtils.createUrl(null, null, "null.xml", null, null, request);
      return assembler;
    } catch (MalformedURLException e) {
      // this exception should never occur
      log.error(this.getClass().getName(), e);
  ...
  protected void initGeneratorUDF() {
    try {
      ...
      m_generatorUDF = (IPSUdfProcessor) extMgr.prepareExtension(extRef, null);
  ...
  public synchronized IPSUdfProcessor getGenerator() {
    if (m_generatorUDF == null) initGeneratorUDF();
