...
public class PSHolidays {
  ...
  protected Document executeCatalogerQuery(IPSRequestContext request) {
    ...
    IPSInternalRequest ir = request.getInternalRequest(HOLIDAY_CATALOGER, null, false);
    if (ir == null) {
      StringBuilder error = new StringBuilder();
      error.append("Catalog of holidays failed, beacause the query resource '");
      ...
      try {
        ...
        error.append("because an error occurred during the internal request: ");
  ...
  protected Set<Holiday> loadHolidays(IPSRequestContext request) {
    ...
    if (results != null) {
      ...
      for (int i = 0; i < numNodes; i++) {
        ...
        if (day == null) {
          PSUtils.printTraceMessage(request, "Could not parse day attribute: " + dayAttr);
        } else {
