...
public class PSAutoSiteItemFilter extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (params.length < MIN_REQD_PARAM_COUNT)
      throw new PSParameterMismatchException(
          ...
          "Invalid # of Parameters."
              + " Expected : "
              + MIN_REQD_PARAM_COUNT
              + " Passed : "
              + params.length);
    if (params[0] == null || params[0].toString().equals(""))
      throw new PSParameterMismatchException(0, "SiteFolderLookupURL cannot be null or empty");
    ...
    if (currSiteid.length() < 1) {
      request.printTraceMessage("Site Id is null or empty. No site item filtering is done");
      return resultDoc;
    }
    ...
    try {
      ...
      request.printTraceMessage("Parent's Folder Path = " + parentFolderPath);
      ...
      if (relatedInfoNode == null) {
        request.printTraceMessage("root node is null, returning empty list");
        return resultDoc;
      }
      PSXmlTreeWalker resultWalker = new PSXmlTreeWalker(relatedInfoNode);
      Element linknode =
          resultWalker.getNextElement("linkurl", PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      while (linknode != null) {
        if (isValid(linknode, request, parentFolderPath)) {
          isEmpty = false;
          request.printTraceMessage("Keeping Item :: " + linknode.getAttribute("relateditemid"));
          linknode =
              resultWalker.getNextElement("linkurl", PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);

        } else {
          Element nextRes =
              resultWalker.getNextElement("linkurl", PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
          request.printTraceMessage("Removing Item:: " + PSXmlDocumentBuilder.toString(linknode));
      ...
      if (isEmpty) {
        ...
        PSXmlDocumentBuilder.addEmptyElement(resultDoc, relatedInfoNode, "linkurl");
        return resultDoc;
      } else {
      ...
      request.printTraceMessage(PSInternalRequestCallException.getStackTraceAsString(irce));
  ...
  private boolean isValid(Element currElement, IPSRequestContext request, String parentFolderPath)
    ...
    if (folderpath != null) {
      for (int i = 0; i < folderpath.length; i++) {
        ...
        request.printTraceMessage("in isValid method, currPath = " + currPath);
        request.printTraceMessage("in isValid method, parentFolderPath = " + parentFolderPath);
        if (currPath.indexOf(parentFolderPath) >= 0) {
          request.printTraceMessage(currPath + " contains parent folder path" + parentFolderPath);
          return true;
        }
      }
    }
    request.printTraceMessage(" None of the item paths have parent folder path");
  ...
  private int computeRevision(Element linkurl, IPSRequestContext request) {
    ...
    if (startIndex < 0) {
      request.printTraceMessage("URL doesnot have sys_revision : " + currentURL);
      return -1;
    }
    int endIndex = currentURL.indexOf("&", startIndex);
    if (endIndex < 0) {
      request.printTraceMessage("sys_revision is probably the last parameter");
      endIndex = currentURL.length();
    }
    String rev = currentURL.substring(startIndex + revstring.length(), endIndex);
