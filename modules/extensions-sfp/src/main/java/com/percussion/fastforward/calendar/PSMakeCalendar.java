...
public class PSMakeCalendar implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object params[], IPSRequestContext request, Document doc)
    ...
    todayElem.setAttribute("day", Integer.toString(cal.get(Calendar.DAY_OF_MONTH)));
    todayElem.setAttribute("month", FastDateFormat.getInstance("MMMM").format(cal.getTime()));
    ...
    monthElem.setAttribute("name", FastDateFormat.getInstance("MMMM").format(cal.getTime()));
    ...
    for (int i = 0; nlWeeks != null && i < weeksInMonth; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        ...
        days[(i * 7) + j].setAttribute("dayOfWeek", Integer.toString(j + 1));
      ...
      try {
        ...
        try {
          ...
          for (j = 0; j < numberOfDays; j++) {
            ...
            int calendarDay = Integer.parseInt(dayElem.getAttribute("day"));
            if (calendarDay == linkDay) dayElem.appendChild(link);
          }
