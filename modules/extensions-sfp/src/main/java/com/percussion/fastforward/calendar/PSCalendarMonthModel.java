  ...
  @IPSJexlMethod(
      ...
      params = {
        @IPSJexlParam(
            name = "cal",
            type = "Calendar",
            description = "calendar to use for computing month information")
      })
  public PSCalendarMonthModel assign(Calendar cal) {
    if (cal == null) throw new IllegalArgumentException("cal may not be null");
  ...
  public void setEvents(List<IPSAssemblyResult> events) {
    if (events == null) {
      ...
      for (IPSAssemblyResult event : events) {
        ...
        if (event_node != null) {
          try {
            Property event_start_prop = event_node.getProperty(EVENT_START_PROP_NAME);
            if (event_start_prop != null) {
              Calendar event_start = event_start_prop.getDate();
              if (!(event_start.before(getStartDate()) || event_start.after(getEndDate()))) {
                Integer event_day = event_start.get(Calendar.DAY_OF_MONTH);
                model.m_eventsByDay.put(event_day, event);
              } else {
                ms_log.warn("event " + event.getId() + " does not fall within current month");
              }
            } else {
              ms_log.error("event " + event.getId() + " does not have rx:event_start property");
            }
