...
public class PSChildRelationshipBuilder extends PSChildRelationshipBase {
  ...
  public void build(int dependentId, Object[] ownerIds, String slotName, String templateName)
    ...
    IPSAssemblyTemplate template = findTemplate(templateName, item.getContentTypeGUID());
    ...
    if (!currentRelations.isEmpty()) {
      ...
      // desired - current = add
      List<Integer> idsToAdd = createComplement(desiredOwnerIds, currentOwnerIds);
      addRelationships(idsToAdd, child, slot, template);

      // current - desired = remove
      List<Integer> idsToRemove = createComplement(currentOwnerIds, desiredOwnerIds);
  ...
  private void addRelationships(
    ...
    for (PSLocator owner : owners) {
      PSAaRelationship newRelationship = new PSAaRelationship(owner, dependent, slot, template);
