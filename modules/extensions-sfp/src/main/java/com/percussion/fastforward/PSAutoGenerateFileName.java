...
public class PSAutoGenerateFileName extends PSDefaultExtension
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document doc)
    ...
    if (params.length < 3) {
      throw new PSParameterMismatchException(0, "Atleast 3 parameter required");
    }
    if (params[0] == null || params[0].toString().trim().equals(""))
      throw new PSParameterMismatchException(0, "Default Filename should not be empty");

    if (params[1] == null || params[1].toString().trim().equals("")) {
      request.printTraceMessage("Creating filename because it was not passed in ");
    } else {
      return doc;
    }

    if (params[2] == null || params[2].toString().trim().equals(""))
      throw new PSParameterMismatchException(0, "Update resource should not be empty");
    ...
    try {
      IPSInternalRequest irq = request.getInternalRequest(updateresource, paramMap, false);
      irq.performUpdate();
