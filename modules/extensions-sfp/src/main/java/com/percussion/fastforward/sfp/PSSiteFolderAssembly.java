...
/**
 * Extension that can be used a location scheme generator as well as a UDF to generate the publish
 * or assembly location path based on the location of the item in the site folder tree. The path is
 * generated by concatenating the names of all site folders in the tree starting from root to leaf.
 * The base for the path could differ based on whether the location being is generated for an item
 ...
 *       then the link generated will be relative link, e.g. /Marketing/page301.html
 *   <li><i>Two Independent Sites: </i>An item has link to an item from a section of a different
 *       site folder tree (originating siteid and current siteid are different) then the link
 *       generated will be absolute link, e.g. http://server:port/root/Marketing/page301.html
 *   <li><i>Multiple Sites with a Common Root: </i>An item has a link to an item from a section of a
 *       different site however, that site originated from the same site folder root, or the
 *       registered base URLs for both sites have common base then the link generated will be a
 *       relative link starting after the common part the base URLs of two sited, e.g. consider the
 *       original site is registered with a base URL of http://server:port/commonRoot/siteAroot and
 *       that for teh current site is http://server:port/commonRoot/siteBroot. The location
...
public class PSSiteFolderAssembly extends PSDefaultExtension
   ...
   * contentid}, {@link IPSHtmlParameters#SYS_REVISION revision}and {@link
   * IPSHtmlParameters#SYS_SITEID publishing siteid}HTML parameters.
   *
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    log.debug("Site Folder Assembly - process UDF " + contentId + " " + revision + " " + siteId);
  ...
  /**
   * Generates a publishing location by concatenating the names of all site folders that contain the
   * content item identified by the contentId and revision in the current request.
   *
   * @param params Each parameter will be concatenated to the end of the site folder path, to form
   *     the filename
   * @param request The current request, never null.
   * @return A publishing location consisting of all site folders' names that contain the specified
   *     item, plus any additional parameters to this exit. The location will be an empty string if
   *     the specified item is not contained by a site folder.
   * @throws PSExtensionException if the current request does not define a contentid and revision,
   *     or if any exception occurs working with the relationship processor or the database proxy.
   */
  public String createLocation(Object[] params, IPSRequestContext request)
    ...
    String revision = request.getParameter(IPSHtmlParameters.SYS_REVISION);
    if (contentId == null || revision == null) {
      throw new PSExtensionException(DEBUG_HDR, "request must supply content id and revision");
    }
    String siteId = request.getParameter(IPSHtmlParameters.SYS_SITEID, "");
    ...
    try {
      ...
      if (siteId.length() > 0 && originalSiteId.length() > 0 && !siteId.equals(originalSiteId)) {
        String urlCurrBase = PSUtils.getbaseUrl(siteId, request);
        String urlOrigBase = PSUtils.getbaseUrl(originalSiteId, request);
        // Normalize both to end with "/"
        if (!urlCurrBase.endsWith("/")) urlCurrBase += "/";
        if (!urlOrigBase.endsWith("/")) urlOrigBase += "/";
      ...
      path = (siteBaseUrl.endsWith("/") && path.startsWith("/")) ? path.substring(1) : path;
      siteFolderPath.append(siteBaseUrl + path);
    } catch (Exception e) {
      // the relationship API failed
      log.error(PSExceptionUtils.getMessageForLog(e));
      throw new PSExtensionException(this.getClass().getName(), e.getLocalizedMessage());
    }
  ...
  /**
   * Finds all folders that contain the specified item, pick one that is a desecendent of the site's
   * root folder, and build that folder's path.
   *
   * @param contentId
   * @param revision
   * @param siteId
   * @param request
   * @return the path name. Never <code>null</code>, may be <code>empty</code>
   * @throws PSUnknownNodeTypeException
   * @throws PSCmsException if an error occurs while fetching the folders that contain the specified
   *     item
   * @throws PSExtensionProcessingException
   */
  private String buildSiteFolderPath(
      String contentId, String revision, String siteId, IPSRequestContext request)
    ...
    if (siteId == null) {
      log.error(
          DEBUG_HDR + "a site id parameter is " + "required, so a location was not generated");
      return "";
    }
    ...
    if (rootpath == null || rootpath.trim().length() == 0) {
      log.error(
          DEBUG_HDR
              + "a site folder root for site id "
              + siteId
    ...
    if (siteFolderLoc != null) { // a folder specified in the parameters,
      ...
      path = PSSite.buildFolderPathList(fid, siteFolderLoc, true);

      if (path.isEmpty()) {
        log.error("Specified folder, id=" + siteFolderLoc.getId() + ", not in Site Tree ");
        return "";
      }
    } else {
  ...
  /**
   * get the locator for the specified folder from the request context. Looks for the folderid as
   * HTML parameter {@link IPSHtmlParameters#SYS_FOLDERID} first; if does not find, looks for the
   * parameter named "rx_folder" (for backward compatibility). If a non <code>null</code> and
   * non-empty folderid exists in the request a locator is constructed with revision=1 and returned.
   * Otherwise, returns <code>null</code>.
   *
   * @param req request context object, must not be <code>null</code>.
   * @return Loctaor for the folderid found in the request context object, will be <code>null</code>
   *     if a valid folderid not found.
   */
  private PSLocator getSpecifiedFolder(IPSRequestContext req) {
    ...
    if (rxFolder == null || rxFolder.trim().length() == 0 || rxFolder.equals("-1")) {
