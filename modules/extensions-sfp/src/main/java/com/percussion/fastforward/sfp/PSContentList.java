...
public class PSContentList {
  ...
  public PSContentList(
    ...
    m_context = context;
    if (m_context == null) m_context = "";
    m_deliveryType = deliveryType;
    if (m_deliveryType == null) m_deliveryType = "";
  ...
  public Document getPage(int indexOfFirstItem, IPSRequestContext request) {
    ...
    if (indexOfFirstItem < 0) {
      throw new IllegalArgumentException("indexOfFirstItem can not be negative");
    }

    Document listDoc = PSXmlDocumentBuilder.createXmlDocument();
    Element contentList =
        PSXmlDocumentBuilder.createRoot(listDoc, IPSDTDPublisherEdition.ELEM_CONTENTLIST);
    contentList.setAttribute(IPSDTDPublisherEdition.ATTR_CONTEXT, m_context);
    contentList.setAttribute(IPSDTDPublisherEdition.ATTR_DELIVERYTYPE, m_deliveryType);

    // determine if pagination is necessary
    if (m_maxRowsPerPage > -1) {
      // paginate when m_maxRowsPerPage is assigned
      log.debug(
          "outputting paged content list XML with "
              + m_maxRowsPerPage
              + " rows per page, "
              + "starting at item "
              + indexOfFirstItem);
  ...
  private void addResultPagerXml(
      int indexOfFirstItem, int indexOfLastItem, Document listDoc, IPSRequestContext request) {
    if (listDoc == null) {
    ...
    if (indexOfFirstItem > 1) {
      ...
      try {
        ...
        // values
        URL prevLink =
            PSUrlUtils.createUrl(null, null, "", paramMap.entrySet().iterator(), null, request);
        PSXmlDocumentBuilder.addElement(
            listDoc, contentList, RPL_XML_FIELDNAME_PREV, prevLink.toString());
    ...
    if (indexOfLastItem < size()) {
      ...
      try {
        ...
        URL nextLink =
            PSUrlUtils.createUrl(null, null, "", paramMap.entrySet().iterator(), null, request);
        PSXmlDocumentBuilder.addElement(
            listDoc, contentList, RPL_XML_FIELDNAME_NEXT, nextLink.toString());
      } catch (MalformedURLException e) {
        // this exception should never be thrown
        log.error(getClass().getName(), e);
        log.error(PSExceptionUtils.getMessageForLog(e));
