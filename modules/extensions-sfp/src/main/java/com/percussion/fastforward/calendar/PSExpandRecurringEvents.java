...
public class PSExpandRecurringEvents extends PSDefaultExtension
  ...
  private static Object[] parseParameters(Object[] params)
    ...
    if (params == null || params.length < 2) {
      throw new PSExtensionProcessingException(
          0, "params must not be null and must have at least two entries.");
    }
    /*
    ...
    Date calendarStart = PSDataTypeConverter.parseStringToDate(calendarStartString);
    ...
    return new Object[] {calendarStart, calendarEnd};
  }
  ...
  public Document processResultDocument(
    ...
    Element sysAssemblerInfo = newResultDoc.createElement("sys_AssemblerInfo");
    ...
    request.printTraceMessage(
        "Expanding recurring events between " + calendarStart + " and " + calendarEnd);

    Element root = resultDoc.getDocumentElement();
    if (root == null) return resultDoc;
    ...
    for (int i = 0; i < eventElems.getLength(); i++) {
      ...
      request.printTraceMessage(
          "Processing sys_contentid = " + eventElem.getAttribute("sys_contentid"));
        ...
        try {
          ...
          while ((c = eventIterator.next()) != null) {
            ...
            if (d.compareTo(calendarStart) >= 0 && !holidays.isHoliday(d)) {
              ...
              linkUrlClone.setAttribute(MONTH_ATTR, monthFormat.format(d));
              linkUrlClone.setAttribute(YEAR_ATTR, yearFormat.format(d));
              // add the recurrence date to the URL so it can be used
              // by snippet
              NodeList nlValue = linkUrlClone.getElementsByTagName("Value");
              if (nlValue.getLength() > 0) {
                Element value = (Element) nlValue.item(0);
                String currentUrl = value.getAttribute("current");
                if (currentUrl != null) {
                  value.setAttribute(
                      "current", currentUrl + "&recurrenceDate=" + fullFormat.format(d));
                }
              }
              relatedContent.appendChild(newResultDoc.importNode(linkUrlClone, true));
            }
          }
           ...
           */
          request.printTraceMessage("error with recurring event values: " + e);
