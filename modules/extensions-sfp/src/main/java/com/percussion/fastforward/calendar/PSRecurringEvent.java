...
public class PSRecurringEvent {
  ...
  public PSRecurringEvent(Element root) throws UnknownNodeTypeException, IllegalValueException {
    ...
    try {
      ...
      if (startDateString == null)
        throw new UnknownNodeTypeException("Missing required attribute: startDate");
      String endDateString = root.getAttribute("endDate");
      if (endDateString == null)
        throw new UnknownNodeTypeException("Missing required attribute: endDate");
      ...
      Element intervalElem =
          walker.getNextElement(INTERVAL_ELEM, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
      if (intervalElem == null)
        throw new UnknownNodeTypeException("Missing required element: " + INTERVAL_ELEM);
      try {
        setInterval(Integer.parseInt(PSXmlTreeWalker.getElementData(intervalElem)));
      } catch (NumberFormatException e) {
        throw new UnknownNodeTypeException("Could not parse " + INTERVAL_ELEM);
      } catch (IllegalArgumentException e) {
      ...
      if (dayOfWeek != null) {
        ...
        try {
          ...
          throw new UnknownNodeTypeException("Could not parse " + DAYOFWEEK_ELEM + ": " + data);
        }
        String occurrence = dayOfWeek.getAttribute(OCCURRENCE_ATTR);
        if (occurrence == null)
          throw new UnknownNodeTypeException("Missing required attribute: " + OCCURRENCE_ATTR);
        try {
          setDayOfWeekOccurrence(Integer.parseInt(occurrence));
        } catch (NumberFormatException e) {
          throw new UnknownNodeTypeException("Could not parse " + OCCURRENCE_ATTR);
        }
      }
      ...
      if (dayOfMonth != null) {
        ...
        try {
          ...
          throw new UnknownNodeTypeException("Could not parse " + DAYOFMONTH_ELEM + ": " + data);
        }
      }
      ...
      try {
        ...
        if (intervalType == null)
          throw new UnknownNodeTypeException("Missing required attribute: " + TYPE_ATTR);
        setIntervalType(intervalType);
      } catch (IllegalStateException e) {
  ...
  private void setStartDate(Date startDate) {
    if (startDate == null)
      throw new IllegalArgumentException("Illegal startDate (must not be null).");
  ...
  private void setEndDate(Date endDate) {
    if (endDate == null) throw new IllegalArgumentException("Illegal endDate (must not be null).");

    if (m_startDate != null && m_startDate.compareTo(endDate) > 0)
      throw new IllegalStateException(
          "endDate " + endDate + "may not come before startDate " + m_startDate);
  ...
  private void setInterval(int interval) {
    if (interval < 1) throw new IllegalArgumentException("Illegal interval (must be > 0).");
    m_interval = interval;
  }
  ...
  private void setIntervalType(int intervalType) {
    if (intervalType != DAILY_RECURRENCE
      ...
      throw new IllegalArgumentException("Illegal intervalType: " + intervalType);
    m_intervalType = intervalType;

    /* validate the other parameters based on the interval type */
    if (intervalType == MONTHLY_RECURRENCE_DAY && (m_dayOfMonth < 1 || m_dayOfMonth > 31))
      throw new IllegalStateException("When recurring by day of month, must provide a valid day.");
  ...
  private void setIntervalType(String intervalType) {
    ...
    if (intervalType.equals("d")) setIntervalType(DAILY_RECURRENCE);
    else if (intervalType.equals("dd")) setIntervalType(WEEKDAILY_RECURRENCE);
    else if (intervalType.equals("w")) setIntervalType(WEEKLY_RECURRENCE);
    else if (intervalType.equals("md")) setIntervalType(MONTHLY_RECURRENCE_DAY);
    else if (intervalType.equals("mw")) setIntervalType(MONTHLY_RECURRENCE_WEEK);
    else throw new IllegalArgumentException("Illegal intervalType: " + intervalType);
  }
  ...
  public Calendar getRecurrence(int recurrence) {
    ...
    if (calendar.getTime().compareTo(m_endDate) <= 0) return calendar;
    else return null;
  }
  ...
  private void assignMonthlyByWeekRecurrence(Calendar calendar, int recurrence) {
    ...
    if (m_dayOfWeekOccurrence == LAST_WEEK_OF_MONTH) {
      ...
      while (calendar.get(Calendar.DAY_OF_WEEK) != m_dayOfWeek) calendar.add(Calendar.DATE, -1);
    } else {
      /*
