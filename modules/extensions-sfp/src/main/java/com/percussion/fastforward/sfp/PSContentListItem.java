...
public class PSContentListItem implements Comparable<PSContentListItem> {
  ...
  private void fromXml(Element src) throws PSUnknownNodeTypeException {
    ...
    m_contentId = PSXMLDomUtil.checkAttribute(src, IPSDTDPublisherEdition.ATTR_CONTENTID, true);
    m_revision = PSXMLDomUtil.checkAttribute(src, IPSDTDPublisherEdition.ATTR_REVISION, true);
    m_variantId = PSXMLDomUtil.checkAttribute(src, IPSDTDPublisherEdition.ATTR_REVISION, false);
    m_unpublishValue =
        PSXMLDomUtil.checkAttribute(src, IPSDTDPublisherEdition.ATTR_UNPUBLISH, false);

    // the title is not stored in RXSITEITEMS, so this is option
    Element childElem = PSXMLDomUtil.getFirstElementChild(src);
    if (PSXMLDomUtil.getUnqualifiedNodeName(childElem)
        .equalsIgnoreCase(IPSDTDPublisherEdition.ELEM_CONTENTTITLE)) {
      m_contentTitle = PSXMLDomUtil.getElementData(childElem);
      childElem =
          PSXMLDomUtil.getNextElementSibling(childElem, IPSDTDPublisherEdition.ELEM_CONTENTURL);
    } else {
      m_contentTitle = "";
    }
    ...
    Element deliveryElem =
        PSXMLDomUtil.getNextElementSibling(childElem, IPSDTDPublisherEdition.ELEM_DELIVERY);
    Element locationElem =
        PSXMLDomUtil.getFirstElementChild(deliveryElem, IPSDTDPublisherEdition.ELEM_LOCATION);
    m_deliveryLocation = PSXMLDomUtil.getElementData(locationElem);

    Element modifyDateElem =
        PSXMLDomUtil.getNextElementSibling(deliveryElem, IPSDTDPublisherEdition.ELEM_MODIFYDATE);
    ...
    Element userElem =
        PSXMLDomUtil.getNextElementSibling(modifyDateElem, IPSDTDPublisherEdition.ELEM_MODIFYUSER);
    m_lastModifier = PSXMLDomUtil.getElementData(userElem);

    Element expireDateElem =
        PSXMLDomUtil.getNextElementSibling(userElem, IPSDTDPublisherEdition.ELEM_EXPIREDATE);
    ...
    Element contentTypeElem =
        PSXMLDomUtil.getNextElementSibling(expireDateElem, IPSDTDPublisherEdition.ELEM_CONTENTTYPE);
  ...
  public Element toXml(Document doc, IPSRequestContext request) {
    ...
    if (request == null) throw new IllegalArgumentException("request context may not be null");

    log.debug("generating contentitem XML for id=" + m_contentId);
    Element itemXml = doc.createElement(IPSDTDPublisherEdition.ELEM_CONTENTITEM);
    ...
    if (m_variantId != null)
      itemXml.setAttribute(IPSDTDPublisherEdition.ATTR_VARIANTID, m_variantId);
    else itemXml.setAttribute(IPSDTDPublisherEdition.ATTR_VARIANTID, "0");
    itemXml.setAttribute(IPSDTDPublisherEdition.ATTR_UNPUBLISH, m_unpublishValue);

    Element titleElem =
        PSXmlDocumentBuilder.addElement(
            doc, itemXml, IPSDTDPublisherEdition.ELEM_CONTENTTITLE, m_contentTitle);

    // add the "contenturl" element
    if (m_contenturl != null) // got it from #fromXml(), just use it.
    {
      PSXmlDocumentBuilder.addElement(
          doc, titleElem, IPSDTDPublisherEdition.ELEM_CONTENTURL, m_contenturl);
    }
    // generate content URL to the assembler resource
    else {
      Element contentUrlXml = doc.createElement(IPSDTDPublisherEdition.ELEM_CONTENTURL);
      ...
      if (m_variantId != null && m_variantId.trim().length() > 0) {
        ...
        try {
          ...
          if (m_getLastPubRev4QEState) {
            ...
            correctedRevision =
                PSMacroUtils.correctRevisionForFlags(request, m_contentId, validFlags, m_revision);
          }
          if (correctedRevision.equals("-1")) {
            revError =
                "This item was never public "
                    + "but in workflow state with contentvalid="
                    + CONTENTVALID_FLAG_I;
          } else {
            itemXml.setAttribute(IPSDTDPublisherEdition.ATTR_REVISION, correctedRevision);
          }
        } catch (PSException e) {
        ...
        if (revError != null) {
          ...
          URL assemblerURL =
              PSSiteFolderContentListLinkGenerator.generateAssemblerLink(
                  ...
                  m_port,
                  m_paramSetToPass);
          contentUrlXml.appendChild(doc.createTextNode(assemblerURL.toString()));
        }
      } else {
        // if variant is unknown, include a comment in the content list to aid
        // debugging
        String comment =
            "content URL suppressed because no publishable " + "variants for this item";
        makeNullContentUrl(doc, request, contentUrlXml, comment);
      }
    }

    Element deliveryXml = doc.createElement(IPSDTDPublisherEdition.ELEM_DELIVERY);
    ...
    {
      PSXmlDocumentBuilder.addElement(
          doc, deliveryXml, IPSDTDPublisherEdition.ELEM_LOCATION, m_deliveryLocation);
    } else // create the "location" element on the fly
    {
      Element locationXml = doc.createElement(IPSDTDPublisherEdition.ELEM_LOCATION);
      ...
      if (m_variantId == null) {
        ...
        locationXmlValue =
            doc.createComment("location suppressed because no publishable variants for this item");
      } else {
        ...
        if (filepath.length() > 0) {
          ...
          log.warn("location scheme generator errored or returned empty string");
          locationXmlValue =
              doc.createComment("location scheme generator errored or returned an empty string");
        }
      }
    ...
    if (m_lastModifiedDate != null)
      PSXmlDocumentBuilder.addElement(
          doc, itemXml, IPSDTDPublisherEdition.ELEM_MODIFYDATE, format.format(m_lastModifiedDate));

    PSXmlDocumentBuilder.addElement(
        doc, itemXml, IPSDTDPublisherEdition.ELEM_MODIFYUSER, m_lastModifier);

    if (m_expiryDate != null)
      PSXmlDocumentBuilder.addElement(
          doc, itemXml, IPSDTDPublisherEdition.ELEM_EXPIREDATE, format.format(m_expiryDate));

    PSXmlDocumentBuilder.addElement(
        doc, itemXml, IPSDTDPublisherEdition.ELEM_CONTENTTYPE, m_contentTypeId);
   ...
   */
  private void makeNullContentUrl(
      Document doc, IPSRequestContext request, Element contentUrlXml, String comment) {
  ...
  public boolean isContentUrlNull() {
    if (m_contenturl != null)
      throw new IllegalStateException(
          "This instance is created from XML, the m_nullUrl data is unknown.");
  ...
  private String getCompareKey() {
    if (m_compareKey == null) m_compareKey = m_contentId + "," + m_variantId;
    return m_compareKey;
  }
