...
public class PSJavaHelp {
  ...
  public void setHelpSet(String helpSetURL, String helpMappingResourceFile) {
    ...
    synchronized (ms_theInstance) {
      if (helpSetURL == null || helpSetURL.trim().length() == 0)
        throw new IllegalArgumentException("helpSetURL may not be null or empty");
      ...
      if (helpMappingResourceFile != null) {
        ...
        else throw new IllegalArgumentException("helpMappingResourceFile may not be empty.");
      }
    }
  }
   ...
   */
  public static String getHelpSetURL(String hsFilePath, boolean isApplet, String codeBase) {
    if (null == ms_theInstance) ms_theInstance = new PSJavaHelp();
    synchronized (ms_theInstance) {
      if (isApplet && (codeBase == null || codeBase.trim().length() == 0))
        throw new IllegalArgumentException("applet codeBase may not be null or empty.");
        ...
        if (hsFilePath.indexOf(".jar!/") == -1) {
          if (isApplet) {
            try {
              ...
              throw new IllegalArgumentException("malformed url for helpset");
            }
          } else helpSetURL = prefix + hsFilePath;
        } else helpSetURL = "jar:" + prefix + hsFilePath;
      }
  ...
  public static void launchHelp(String helpID, boolean isTopicID, Window window) {
    ...
    synchronized (ms_theInstance) {
      ...
      if (null == broker) {
        showErrorDialog(
            getResourceString("noHelpSet"),
            getResourceString("noHelpTitle"),
            JOptionPane.ERROR_MESSAGE);
        return;
      }

      if (null == helpID || 0 == helpID.trim().length()) helpID = MAIN_HELP_ID;
      ...
      try {
        ...
        if (helpInst.m_helpIDToFileMap == null || isTopicID) {
          ...
          if (null == helpTopicID) {
            ...
            helpTopicID = helpInst.m_helpIDToFileMap.getProperty(MAIN_HELP_ID);
          }
        }
  ...
  public boolean isHelpDisplayed() {
    ...
    synchronized (ms_theInstance) {
      ...
      if (broker != null) return broker.isDisplayed();
      else return false;
    }
  }
  ...
  private void loadHelpTopicMappings(String helpMappingResourceFile) {
    ...
    synchronized (ms_theInstance) {
      ...
      try {
        ...
        if (stream == null) throw new FileNotFoundException("helpMappingResourceFile");
  ...
  private void createHelpSet() {
    ...
    try {
      ...
      String text = MessageFormat.format(getResourceString("malformedHelpSetURL"), params);
      showErrorDialog(text, getResourceString("invalidHelpSetTitle"), JOptionPane.ERROR_MESSAGE);
    } catch (HelpSetException ee) {
      System.out.println("Unable to parse HelpSetURL: " + m_helpSetURL);
      ee.printStackTrace();

      Object[] params = {m_helpSetURL};
      String text = MessageFormat.format(getResourceString("invalidHelpSetFile"), params);
      showErrorDialog(text, getResourceString("invalidHelpSetTitle"), JOptionPane.ERROR_MESSAGE);
    }
  }
      ...
      if (null == m_res) {
        m_res =
            ResourceBundle.getBundle(
                "com.percussion.tools.help.PSJavaHelpErrorResources", Locale.getDefault());
      }
  ...
  private static String getResourceString(String key) {
    ...
    try {
      if (getResources() != null) resourceValue = getResources().getString(key);
    } catch (MissingResourceException e) {
   ...
   * </code> message types.
   */
  public static void showErrorDialog(String errorBody, String errorTitle, int type) {
