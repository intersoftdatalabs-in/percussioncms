...
public class PSHelpApplet extends JApplet {
  ...
  public void init() {
    ...
    String helpSetURL = PSJavaHelp.getHelpSetURL(helpFile, true, getCodeBase().toString());
    m_help = PSJavaHelp.getInstance();
    m_help.clearBroker();
    if (helpSetURL != null && helpSetURL.trim().length() != 0) m_help.setHelpSet(helpSetURL);
    ...
    if (iconHref != null && iconHref.length() > 0) {
      ...
      helpIcon = new ImageIcon(getImage(iconURL));
    }
    ...
    if (null == iconURL
      ...
      String altText = null;
      if (null != getParameter(HELPALTTEXT) && getParameter(HELPALTTEXT).length() > 0) {
  ...
  private URL getCorrectedURL(String imageURLString) {
    ...
    try {
      ...
      String context = base.getPath().substring(0, base.getPath().indexOf("/", 1));

      theURL =
          new URL("http://" + base.getHost() + ":" + base.getPort() + context + imageURLString);
