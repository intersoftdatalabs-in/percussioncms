...
public class PSJdbcExecutionPlanLogsContainer {
  ...
  public void addPlanLogData(PSJdbcExecutionPlanLog planLogData) {
    if (planLogData == null) throw new IllegalArgumentException("planLogData may not be null");
  ...
  public Element toXml(Document doc) {
    ...
    while (list.hasNext()) {
      PSJdbcExecutionPlanLog planLogData = list.next();
  ...
  public synchronized int getDbActionType() {
    Document doc = PSXmlDocumentBuilder.createXmlDocument();
    Element root = toXml(doc);
    NodeList queries = root.getElementsByTagName(PSJdbcExecutionStepLog.QUERY_NODE_NAME);
    NodeList updateCounts =
        root.getElementsByTagName(PSJdbcExecutionStepLog.UPDATE_COUNT_NODE_NAME);
    ...
    if (count > 0 && query != null) {
      ...
      if (queryData instanceof Text) {
        ...
        if (queryString.startsWith("insert")) action = INSERT;
        else if (queryString.startsWith("update")) action = UPDATE;
        else if (queryString.startsWith("delete")) action = DELETE;
      }
    }
