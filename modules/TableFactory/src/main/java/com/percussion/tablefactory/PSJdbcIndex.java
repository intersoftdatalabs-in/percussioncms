...
public class PSJdbcIndex extends PSJdbcKey {
  ...
  public Element toXml(Document doc) {
    ...
    root.setAttribute(ATTR_IS_UNIQUE, m_type == TYPE_UNIQUE ? XML_TRUE : XML_FALSE);
    return root;
  }
  ...
  protected void getComponentState(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    if (sTemp.trim().length() > 0)
      m_type = sTemp.trim().equalsIgnoreCase(XML_FALSE) ? TYPE_NON_UNIQUE : TYPE_UNIQUE;
  }
  ...
  public boolean canAlter() {
    ...
    switch (getAction()) {
      ...
      case ACTION_REPLACE:
        ...
        break;
      case ACTION_DELETE:
        if (getType() == TYPE_NON_UNIQUE) canAlter = true;
        break;
    }
    return canAlter;
  }
  ...
  public boolean isComponentEqual(PSJdbcIndex other) {
    ...
    else if (this.getIndexColumnNames().size() != other.getIndexColumnNames().size()) return false;
    else {
      if (other.getAction() != getAction()) {
        return false;
      }
      if (compareColumns(this.getIndexColumnNames(), other.getIndexColumnNames())
          == IS_GENERIC_MISMATCH) {
        return false;
      }
    }
