...
public class PSJdbcTableSchemaCollection extends PSCollection {
  ...
  public void setTableData(PSJdbcTableDataCollection tableDataCollection)
    ...
    if (tableDataCollection == null)
      throw new IllegalArgumentException("tableDataCollection may not be null");
      ...
      if (tableData != null) {
        if (tableSchema.isAlter()) {
          PSJdbcTableFactory.logMessage(
              "Skipping table data for table schema (" + tableSchema.getName() + ") with alter=y.");
          continue;
        }
  ...
  public static void main(String[] args) {
    if (args.length != 1) {
      ...
      System.out.println("where <tables> is an xml file containing the expected xml.");
      System.exit(1);
    }
    ...
    try {
      ...
      try (FileInputStream in = new java.io.FileInputStream(args[0])) {
        ...
        PSJdbcTableSchemaCollection coll = new PSJdbcTableSchemaCollection(doc, map);
