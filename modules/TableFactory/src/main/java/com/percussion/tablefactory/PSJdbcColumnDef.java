...
public class PSJdbcColumnDef extends PSJdbcTableComponent {
  ...
  public PSJdbcColumnDef(
      ...
      boolean allowsNull,
      String defaultValue) {
    this(dataTypeMap, name, action, jdbcType, size, null, allowsNull, defaultValue);
  }
  ...
  public PSJdbcColumnDef(PSJdbcColumnDef srcCol) {
    if (srcCol == null) throw new IllegalArgumentException("srcCol may not be null");
  ...
  public final void fromXml(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    m_limitSizeForIndex = "y".equalsIgnoreCase(sourceNode.getAttribute(LIMIT_SIZE_ATTR));
  ...
  public Element toXml(Document doc) {
    ...
    // add size parameter
    if (m_size != null) PSXmlDocumentBuilder.addElement(doc, root, SIZE_EL, m_size);

    // add allows null
    PSXmlDocumentBuilder.addElement(doc, root, ALLOWS_NULL_EL, m_allowsNull ? "yes" : "no");

    // add scale parameter
    if (m_scale != null) PSXmlDocumentBuilder.addElement(doc, root, SCALE_EL, m_scale);

    // add default if not null
    if (m_defaultValue != null)
      PSXmlDocumentBuilder.addElement(doc, root, DEFAULT_VALUE_EL, m_defaultValue);

    if (m_sequence != null) PSXmlDocumentBuilder.addElement(doc, root, SEQUENCE_EL, m_sequence);
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSJdbcColumnDef)) isMatch = false;
    else {
      PSJdbcColumnDef other = (PSJdbcColumnDef) obj;
      if (!super.equals(other)) isMatch = false;
      else if (m_allowsNull ^ other.m_allowsNull) isMatch = false;
      else if (m_jdbcType != other.m_jdbcType) isMatch = false;
      else if (m_size != null ^ other.m_size != null) isMatch = false;
      else if (m_size != null && !m_size.equals(other.m_size)) isMatch = false;
      else if (m_scale != null ^ other.m_scale != null) isMatch = false;
      else if (m_scale != null && !m_scale.equals(other.m_scale)) isMatch = false;
      else if (m_defaultValue == null ^ other.m_defaultValue == null) isMatch = false;
      else if (m_sequence != null && !m_sequence.equals(other.m_sequence)) isMatch = false;
      else if (m_defaultValue != null && !m_defaultValue.equals(other.m_defaultValue)) {
        isMatch = false;
      } else if (m_limitSizeForIndex ^ other.m_limitSizeForIndex) isMatch = false;
    }
  ...
  public int hashCode() {
    ...
    if (null != m_size) hash += m_size.hashCode();
    if (null != m_scale) hash += m_scale.hashCode();
    if (null != m_defaultValue) hash += m_defaultValue.hashCode();
    return hash;
  }
  ...
  public boolean isChanged(PSJdbcColumnDef oldCol) {
    ...
    if (oldCol == null) isChanged = true;
      ...
      if (!super.equals(oldCol)) isChanged = true;
        ...
        try {
          ...
          if (existingScale < desiredScale) isChanged = true;
        } catch (NumberFormatException e) {
          // shouldn't happen, but if it does, ignore it here and let
          // the create/alter attempt report the error
          isChanged = true;
        ...
        if (oldCol.m_nativeType != null) {
          ...
          if (!oldCol.m_nativeType.equalsIgnoreCase(nativeType)) isChanged = true;
        } else isChanged = true;
      }
    }
  ...
  public String getScale() {
    return m_scale == null ? m_dataTypeMap.getMapping(m_jdbcType).getDefaultScale() : m_scale;
  }
  ...
  public void setScale(String scale) {
    ...
    String defaultScale = m_dataTypeMap.getMapping(m_jdbcType).getDefaultScale();

    // if the default is null, force value to null
    if (defaultScale == null) scale = null;
    // if the provided value is the default, store null
    else if (defaultScale.equals(scale)) scale = null;
  ...
  public String getSize() {
    return m_size == null ? m_dataTypeMap.getMapping(m_jdbcType).getDefaultSize() : m_size;
  }
  ...
  public void setSize(String size) {
    if (!validateParams(size)) throw new IllegalArgumentException("size may be null but not empty");

    String defaultSize = m_dataTypeMap.getMapping(m_jdbcType).getDefaultSize();

    // if the default is null, force value to null
    if (defaultSize == null) size = null;
    // if the provided value is the default value, store null
    else if (defaultSize.equals(size)) size = null;
  ...
  public String getSqlDef(PSJdbcDbmsDef dbmsDef) {
    ...
    if (dbmsDefDriver != null
        && dataTypeMapDriver != null
    ...
    if (!(PSJdbcTableComponent.ACTION_REPLACE == getAction()
        && "clob".equalsIgnoreCase(nativeDataType)
        && dataTypeMapDriver.equals(PSJdbcUtils.ORACLE))) {
      ...
      if (size != null) {
        ...
        if (scale != null) buf.append(",").append(scale);
        buf.append(")");
      }
    }

    // add the suffix, if defined in the mapping
    if (dataType.getSuffix() != null) buf.append(" ").append(dataType.getSuffix());

    String nullClause;
    if (m_allowsNull) nullClause = PSSqlHelper.getNullColumnSpecifier(dbmsDef.getDriver());
    else nullClause = " NOT NULL";
  ...
  public boolean canAlter() {
    ...
    if (canAlter && (getAction() == ACTION_CREATE || getAction() == ACTION_REPLACE)) {
      if (isAllowedNullChanged) canAlter = false;
      else if (m_defaultValue != null && m_defaultValue.trim().length() > 0) {
        canAlter = false;
      }
    }
    return canAlter;
  ...
  private boolean validateParams(String params) {
    boolean valid = true;
    if (params != null && params.trim().length() == 0) valid = false;
