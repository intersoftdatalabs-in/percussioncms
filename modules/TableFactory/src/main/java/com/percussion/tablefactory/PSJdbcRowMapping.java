...
public class PSJdbcRowMapping implements IPSJdbcRowMapping {
  ...
  public PSJdbcRowData processRow(Connection conn, PSJdbcRowData srcRow)
    ...
    if (srcRow == null) throw new IllegalArgumentException("srcRow may not be null");
    ...
    while (rowIt.hasNext()) {
      ...
      if (destColData != null) colDataList.add(destColData);
    }
  ...
  private PSJdbcColumnData processColumn(PSJdbcColumnData srcColData) {
    ...
    if (m_colMapping.size() < 1) return null;

    String srcColName = srcColData.getName();
    if (!m_colMapping.containsKey(srcColName)) return null;

    String destColName = m_colMapping.get(srcColName);
    return new PSJdbcColumnData(destColName, srcColData.getValue(), srcColData.getEncoding());
  }
