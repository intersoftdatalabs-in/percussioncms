...
public class PSJdbcColumnData {
  ...
  public void setValue(String value, int encoding) {
    ...
    if (!validEncoding(encoding)) throw new IllegalArgumentException("invalid encoding");
    m_encoding = encoding;
  }
  ...
  public final void fromXml(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    m_encoding =
        PSJdbcTableComponent.getEnumeratedAttributeIndex(tree, ENCODING_ATTR, ENCODING_ENUM);
    ...
    if (m_value.trim().length() == 0) {
       ...
       */
      String isEmptyNull = PSJdbcTableComponent.getAttribute(tree, EMPTY_NULL_ATTR, false);
      if (isEmptyNull == null || isEmptyNull.trim().equalsIgnoreCase(EMPTY_NULL_TRUE)) {
        m_value = null;
  ...
  private String resolveExternalResourcePath(String parentNodeName, String external)
    ...
    if (external == null) throw new IllegalArgumentException("externalAttr may not be null");
    ...
    if (rootStart >= 0) {
      ...
      if (rootEnd > rootStart) {
        ...
        if (rootVal != null && rootVal.trim().length() != 0) root = rootVal;

        external = PSStringOperation.replace(external, rootKey, root);
      }
    }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_value == null) {
      ...
      root.appendChild(doc.createTextNode(PSXmlDocumentBuilder.normalize(m_value)));
      // if value is empty, set attribute to indicate it is not null
      if (m_value.trim().length() == 0) root.setAttribute(EMPTY_NULL_ATTR, EMPTY_NULL_FALSE);
    }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSJdbcColumnData)) isMatch = false;
    else {
      PSJdbcColumnData other = (PSJdbcColumnData) obj;
      if (!this.m_name.equals(other.m_name)) isMatch = false;
      else if (!this.m_value.equals(other.m_value)) isMatch = false;
      else if (this.m_encoding != other.m_encoding) isMatch = false;
    }
  ...
  public int hashCode() {
    ...
    if (null != m_value) hash += m_value.hashCode();
    return hash;
  }
