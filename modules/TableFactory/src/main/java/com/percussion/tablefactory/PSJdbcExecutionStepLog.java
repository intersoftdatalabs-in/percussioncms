...
public class PSJdbcExecutionStepLog {
  ...
  public void setDatabase(String name, String type) {
    if (!((name == null) || (name.trim().length() == 0))) m_dbName = name;

    if (!((type == null) || (type.trim().length() == 0))) m_dbType = type;
  }
  ...
  public void setTable(String table) {
    if (!((table == null) || (table.trim().length() == 0))) m_table = table;
  }
  ...
  public Element toXml(Document doc) {
    ...
    PSXmlDocumentBuilder.addElement(doc, root, CONNECTION_NODE_NAME, m_connString);

    PSXmlDocumentBuilder.addElement(doc, root, DB_SERVER_NODE_NAME, m_dbServer);

    Element db = PSXmlDocumentBuilder.addEmptyElement(doc, root, DATABASE_NODE_NAME);
    PSXmlDocumentBuilder.addElement(doc, db, DATABASE_NAME_NODE_NAME, m_dbName);
    PSXmlDocumentBuilder.addElement(doc, db, DATABASE_TYPE_NODE_NAME, m_dbType);

    PSXmlDocumentBuilder.addElement(doc, root, TABLE_NODE_NAME, m_table);

    PSXmlDocumentBuilder.addElement(doc, root, QUERY_NODE_NAME, m_sqlQuery);

    if (m_pkKeyRowData != null) {
      Iterator<PSJdbcColumnData> pkeyList = m_pkKeyRowData.getColumns();
      if (pkeyList.hasNext()) {
        Element pkey = PSXmlDocumentBuilder.addEmptyElement(doc, root, PRIMARY_KEY_NODE_NAME);

        while (pkeyList.hasNext()) {
          PSJdbcColumnData colData = pkeyList.next();
          Element pkeyCol =
              PSXmlDocumentBuilder.addEmptyElement(doc, pkey, PRIMARY_KEY_COL_NODE_NAME);
          PSXmlDocumentBuilder.addElement(
              doc, pkeyCol, PRIMARY_KEY_COL_NAME_NODE_NAME, colData.getName());
          String colVal = colData.getValue();
          if (colVal == null) colVal = "";
          PSXmlDocumentBuilder.addElement(doc, pkeyCol, PRIMARY_KEY_COL_VALUE_NODE_NAME, colVal);
        }
      }
    }

    PSXmlDocumentBuilder.addElement(
        doc, root, UPDATE_COUNT_NODE_NAME, String.valueOf(m_updateCount));

    String successVal = SUCCESS_YES_VALUE;
    if (!m_success) successVal = SUCCESS_NO_VALUE;
    PSXmlDocumentBuilder.addElement(doc, root, SUCCESS_NODE_NAME, successVal);

    PSXmlDocumentBuilder.addElement(doc, root, MESSAGE_NODE_NAME, m_errorMsg);
