...
public class PSJdbcTableMetaData {
   ...
   */
  public PSJdbcTableMetaData(
      DatabaseMetaData dmd, PSJdbcDbmsDef dbmsDef, PSJdbcDataTypeMap dataTypeMap, String tableName)
      throws PSJdbcTableFactoryException {
    if (dmd == null) throw new IllegalArgumentException("dmd may not be null");
    ...
    try {
      ...
      if (dmd.storesLowerCaseIdentifiers()) {
        ...
        if (m_schema != null) m_schema = m_schema.toLowerCase();
        if (m_database != null) m_database = m_database.toLowerCase();
      } else if (dmd.storesUpperCaseIdentifiers()) {
        m_tableName = m_tableName.toUpperCase();
        if (m_schema != null) m_schema = m_schema.toUpperCase();
        if (m_database != null) m_database = m_database.toUpperCase();
      }
  ...
  public Iterator<Map.Entry<String, List<String>>> getIndexes(int type) {
    ...
    for (PSJdbcIndex index : m_indexes) {
      if (index.isOfType(type))
        indexes.put(index.getName(), PSIteratorUtils.cloneList(index.getColumnNames()));
    }
  ...
  public Iterator<PSJdbcIndex> getIndexObjects(int type) {
    ...
    for (PSJdbcIndex index : m_indexes) {
      if (index.isOfType(type)) list.add(index);
    }
   ...
   */
  private void checkIsView(DatabaseMetaData md) throws SQLException {
    ResultSet rs = null;
    PSJdbcResultSetIteratorStep step = null;
    try {
      String driver = m_dbmsDef.getDriver();
      String sql =
          PSSqlHelper.getMetaDataQuery(driver, m_database, m_schema, m_tableName, true, false);
      if (sql != null) {
   ...
   * @param md The meta data to use, assumed not <code>null</code>.
   */
  private void checkTableExists(DatabaseMetaData md) throws SQLException {
    ResultSet rs = null;
    PSJdbcResultSetIteratorStep step = null;
    try {
      String driver = m_dbmsDef.getDriver();
      String sql =
          PSSqlHelper.getMetaDataQuery(driver, m_database, m_schema, m_tableName, false, false);
      if (sql != null) {
        Connection conn = md.getConnection();
        step = PSJdbcStatementFactory.getQueryStatement(m_dbmsDef, sql);
        step.execute(conn);
        ...
        // driver can return a null ResultSet even though the API doc implies
        // that they should not (for example, Informix)
        if (rs != null) m_tableExists = rs.next();
      }
    } finally {
      if (rs != null) {
        try {
  ...
  private void loadColumnInformation(ResultSetMetaData meta) throws SQLException {
    ...
    try {
      for (int i = 1; i <= meta.getColumnCount(); i++) {
        ...
        jdbcType = m_dataTypeMap.getJdbcType(nativeType, size, scale, jdbcType);
  ...
  private void loadColumnInformation(DatabaseMetaData md) throws SQLException {
    ...
    try {
      String driver = m_dbmsDef.getDriver();
      String sql =
          PSSqlHelper.getMetaDataQuery(driver, m_database, m_schema, m_tableName, isView(), true);
      ...
      if (sql != null) {
        ...
        if (rs != null) {
          while (rs.next()) {
            ...
            int type = PSSqlHelper.convertNativeDataType(driverType, typeName, driver);

            String nativeType = rs.getString(6);
            String size = String.valueOf(rs.getInt(7));
            String scale = String.valueOf(rs.getInt(9));
            type = m_dataTypeMap.getJdbcType(nativeType, size, scale, type);
  ...
  private String convertSize(int type, String size, String driver) {
    ...
    if (dataType == null || dataType.getDefaultSize() == null) return null;
    return size;
  }
    ...
    try {
      ...
      if (rs != null) {
        ...
        while (rs.next()) {
          ...
          if (keyName != null) m_primaryKeyName = keyName;
        }
      ...
      if (rs != null) {
        while (rs.next()) {
          ...
          if (fkName != null && fkName.trim().length() > 0) {
            List<String[]> keyCols =
                m_foreignKeyColumns.computeIfAbsent(fkName, k -> new ArrayList<>());
            keyCols.add(colDef);
          }
        }
      }
    ...
    try (ResultSet rs = md.getIndexInfo(m_database, m_schema, m_tableName, false, true)) {
      if (rs != null) {
        while (rs.next()) {
          ...
          if (name == null || colName == null) continue;

          // will get the primary key columns too, so skip them
          if (m_primaryKeyName != null && m_primaryKeyName.equals(name)) continue;
          ...
          if (PSSqlHelper.handleBackingIndex(nameBuffer, md)) continue;
      ...
      m_indexes.addAll(getIndexes(nonUniqueIndexMap, PSJdbcIndex.TYPE_NON_UNIQUE));
    } catch (PSJdbcTableFactoryException ex) {
  ...
  private List<PSJdbcIndex> getIndexes(Map<String, List<String>> indexMap, int type)
    ...
    for (Map.Entry<String, List<String>> item : indexMap.entrySet()) {
      ...
      PSJdbcIndex index =
          new PSJdbcIndex(name, columns.iterator(), PSJdbcTableComponent.ACTION_NONE, type);
      list.add(index);
    }
    return list;
  }
