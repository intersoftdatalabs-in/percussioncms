...
public class PSJdbcOracleSqlStatement extends PSJdbcPreparedSqlStatement {
  ...
  public PSJdbcOracleSqlStatement(
      ...
      PSCollection values,
      String selLobString,
      PSCollection keyValues,
      List<Integer> lobTypes,
      List<String> lobValues,
      List<Integer> lobValuesEncoding) {
    ...
    if ((selLobString == null) || (selLobString.trim().length() == 0))
      throw new IllegalArgumentException("selLobString may not be null or empty");
    m_selLobString = selLobString;

    if (keyValues != null) {
      if (keyValues.isEmpty())
        throw new IllegalArgumentException("keyValues must contain at least one object");

      if (!keyValues.getMemberClassType().getName().equals(PSJdbcStatementColumn.class.getName())) {
    ...
    if (lobTypes == null) throw new IllegalArgumentException("lobTypes may not be null");
    m_lobTypes = lobTypes;

    if (lobValues == null) throw new IllegalArgumentException("lobTypes may not be null");
    ...
    if ((lobTypes.size() != lobValues.size()) || (lobTypes.size() != lobValuesEncoding.size()))
      throw new IllegalArgumentException(
  ...
  private int executeInsert(Connection conn) throws SQLException {
    ...
    try {
      ...
      if (updateCount == 0) {
        stepLogData.setErrorMessage(PSJdbcExecutionStepLog.NO_ROWS_UPDATED_MSG);
      } else {
      ...
      String errMsg = PSJdbcTableFactoryException.formatSqlException(callStmtString, e);
      stepLogData.setErrorMessage(errMsg);
      handleSqlException(e);
    } finally {
      try {
        if (insRs != null) insRs.close();
    ...
    try {
      lobStmt =
          PSPreparedStatement.getPreparedStatement(
              conn, m_selLobString, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
      switch (m_statementType) {
        case ORACLE_UPDATE:
        case ORACLE_DUMMY_UPDATE:
          if (m_keyValues != null) bindValues(lobStmt, m_keyValues);
          break;
      ...
      int counter = 1;
      if (lobRs == null) return;
      if (!lobRs.next()) return;
            ...
            if (colValue != null) {
              switch (colValueEncoding) {
                ...
                case PSJdbcColumnData.ENC_BASE64:
                  try {
                    binData = PSDataTypeConverter.getBinaryFromString(colValue);
                  } catch (IOException ioe) {
                    binData = null;
                  }
                  if (binData != null) colValue = new String(binData);
                  break;
  ...
  private BLOB createTempBLOB(Connection conn, byte[] binData) throws SQLException, IOException {
    ...
    try {
      ...
      if (tempBlob != null) closeTempBLOB(tempBlob);
      throw ioe;
    } finally {
  ...
  private CLOB createTempCLOB(Connection conn, String colValue) throws SQLException, IOException {
    ...
    try {
      ...
      if (tempClob != null) closeTempCLOB(tempClob);
      throw ioe;
    } finally {
      // Flush and close the streams
