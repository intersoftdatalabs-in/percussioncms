...
public class PSJdbcPreparedSqlStatement extends PSJdbcSqlStatement {
  ...
  public final void setValues(PSCollection values) {
    if (values != null) {
      ...
      if (!values.getMemberClassType().getName().equals(PSJdbcStatementColumn.class.getName())) {
  ...
  public int execute(Connection conn) throws SQLException {
    ...
    try {
      ...
      if (m_values != null) bindValues(stmt, m_values);
      ...
      String errMsg = PSJdbcTableFactoryException.formatSqlException(m_statement, e);
        ...
        try {
          ...
          PSJdbcTableFactory.logDebugMessage(
              "error closing prepared statement: "
                  + PSJdbcTableFactoryException.formatSqlException(m_statement, e));
        }
      }
  ...
  protected void bindValues(PreparedStatement stmt, PSCollection values) throws SQLException {
    for (int i = 0; i < values.size(); i++) {
      ...
      try {
        ...
        PSJdbcTableFactory.logDebugMessage("bind value " + i + ": " + value.getValue());
