...
class RxTables {
      ...
      for (int iTargetCol = 0; iTargetCol < targetTable.vColumns.size(); ++iTargetCol) {
        ...
        if (targetCol != null && sourceCol != null) {
          if (targetCol.getColName().compareToIgnoreCase(sourceCol.getColName()) == 0) {
            ...
            if (sTargetName.compareToIgnoreCase(sSourceName) != 0
                || (sourceCol.getColLength() > 0
                    && targetCol.getColLength() != sourceCol.getColLength())) {
              System.out.println(
                  "found column diff for "
                      + sourceCol.getColName()
                      + ". Source type: "
                      + sourceCol.getNativeType()
                      + " Target type: "
                      + targetCol.getNativeType()
                      + " Source length: "
                      + Integer.valueOf(sourceCol.getColLength()).toString()
                      + " Target length: "
                      + Integer.valueOf(targetCol.getColLength()).toString());
  ...
  void setNewTableName(String pNewTableName) {
    newTableName = pNewTableName;
  }
  ...
  void setDataBase(String pDataBase) {
    dataBase = pDataBase;
  }

  void setdbLink(String pDbLink) {
    dbLink = pDbLink;
  }
  ...
  void setDataElement(Element eElement) {
    dataElement = eElement;
  }

  void setDtdElement(Element dElement) {
    dtdElement = dElement;
  }

  void setTableAction(int iTableAction) {
    tableAction = iTableAction;
  }

  void setDataAction(boolean bTableAction) {
    dataAction = bTableAction;
  }
