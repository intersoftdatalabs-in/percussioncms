...
/** Interface used by classes that wish to be process data when table schema change occurs. */
public interface IPSJdbcTableDataHandler {
  /**
   * Initialize this data handler. This method must be called before <code>execute()</code> method,
   * and <code>close()</code> method must be called after the <code>execute()</code> method. This
   * method does not close the database connection <code>conn</code>.
   *
   * <p>The value of <code>srcTableSchema</code> and <code>destTableSchema</code> depend upon the
   * schema handler which contains this data handler.
   *
   * <p>If the enclosing schema handler is of type <code>PSJdbcTableSchemaHandler.TYPE_INT_ON_CREATE
   * </code> then <code>srcTableSchema</code> is the schema of the table being created and <code>
   * destTableSchema</code> is <code>null</code>.
   *
   * <p>If the enclosing schema handler is one of the following types: <code>
   ...
   *     </code> and enclosing schema handler is one of the following types: <code>
   *     PSJdbcTableSchemaHandler.TYPE_INT_NO_ALTER_TABLE_STMT</code> or <code>
   *     PSJdbcTableSchemaHandler.TYPE_INT_TO_BACKUP</code> or <code>
  ...
  /**
   * Modifies the row data obtained from the souce table, and returns the modified row data. <code>
   * init()</code> method must be called before this method, and <code>close()</code> method must be
   * called after this method. This method does not close the database connection <code>conn</code>.
   *
   * @param conn the database connection to use, may not be <code>null</code>
  ...
  /**
   * Closes this data handler. <code>init()</code> and <code>execute()</code> methods must be called
   * after this method. This method should close any open database resources. This method does not
   * close the database connection <code>conn</code>. The database connection parameter <code>conn
   * </code> may be a different object from the connection parameter <code>conn</code> provided in
   * the <code>init()</code> and <code>execute()</code> methods.
   *
   * @param conn the database connection to use, may not be <code>null</code>
   * @throws IllegalStateException if <code>init</code> method has not been called previously on
   *     this data handler.
   * @throws PSJdbcTableFactoryException if there are any errors.
   */
  ...
  /**
   * Restore this object from an Xml representation conforming with the tabledef.dtd. The tag name
   * of <code>sourceNode</code> must equal the value of <code>NODE_NAME</code>. The child elements
   * and attributes of <code>sourceNode</code> depend upon the class implementing this interface.
   * Different implementations may have different attributes and child elements of <code>sourceNode
   * </code>.
   *
   * @param sourceNode The element from which to get this object's state. May not be <code>null
   *     </code>. The tag name of this node equals the value of <code>NODE_NAME</code>.
   * @throws IllegalArgumentException if sourceNode is <code>null</code>.
   * @throws PSJdbcTableFactoryException if there are any errors or if the tag name of <code>
   *     sourceNode</code> does not equal the value of <code>NODE_NAME</code>
   */
  public void fromXml(Element sourceNode) throws PSJdbcTableFactoryException;

  /**
   * Serializes this object's state to Xml conforming with the tabledef.dtd. The tag name of the
   * returned element must equal the value of <code>NODE_NAME</code>.
   *
