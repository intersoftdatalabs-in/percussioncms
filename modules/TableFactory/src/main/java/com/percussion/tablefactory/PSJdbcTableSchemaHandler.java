...
public class PSJdbcTableSchemaHandler {
  ...
  public Element toXml(Document doc) {
    ...
    Element dataHandlersEl = PSXmlDocumentBuilder.addEmptyElement(doc, root, DATA_HANDLERS_EL);
  ...
  public final void fromXml(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    Element dataHandlersEl = walker.getNextElement(DATA_HANDLERS_EL, firstFlags);

    if (dataHandlersEl != null) {
      walker = new PSXmlTreeWalker(dataHandlersEl);
      Element dataHandlerEl = walker.getNextElement(IPSJdbcTableDataHandler.NODE_NAME, firstFlags);
      while (dataHandlerEl != null) {
        ...
        dataHandlerEl = walker.getNextElement(IPSJdbcTableDataHandler.NODE_NAME, nextFlags);
      }
    }
  }
   ...
   * @throws PSJdbcTableFactoryException if there are any errors.
   */
  public void closeHandlers(Connection conn) throws PSJdbcTableFactoryException {
    if (!m_bHandlersInitialized) throw new IllegalStateException("data handlers not initialized");
    if (conn == null) throw new IllegalArgumentException("conn may not be null");
  ...
  public static int getSchemaHandlerType(String type) {
    if ((type == null) || (type.trim().length() < 1))
      throw new IllegalArgumentException("schema handler type may not be null or empty");

    type = type.trim();
    if (type.equalsIgnoreCase(TYPE_STR_NO_ALTER_TABLE_STMT)) return TYPE_INT_NO_ALTER_TABLE_STMT;
    else if (type.equalsIgnoreCase(TYPE_STR_TO_BACKUP)) return TYPE_INT_TO_BACKUP;
    else if (type.equalsIgnoreCase(TYPE_STR_FROM_BACKUP)) return TYPE_INT_FROM_BACKUP;
    else if (type.equalsIgnoreCase(TYPE_STR_ON_CREATE)) return TYPE_INT_ON_CREATE;
    else throw new IllegalArgumentException("invalid schema handler type");
  }
