...
public class PSJdbcTableSchemaHandlerStep extends PSJdbcExecutionStep {
  ...
  public PSJdbcTableSchemaHandlerStep(
    ...
    if (schemaHandler == null) throw new IllegalArgumentException("schemaHandler may not be null");
  ...
  public int execute(Connection conn) throws SQLException {
    ...
    try {
      m_schemaHandler.initHandlers(m_dbmsDef, conn, m_srcTableSchema, m_destTableSchema);
      ...
      String details = " source schema " + m_srcTableSchema + " dest schema" + m_destTableSchema;
      String errMsg = PSJdbcTableFactoryException.formatSqlException(details, e);
  ...
  private int copyTableData(Connection conn)
    ...
    PSJdbcResultSetIteratorStep step =
        PSJdbcStatementFactory.getResultSetIteratorStatement(
            m_dbmsDef, m_srcTableSchema, null, null, PSJdbcRowData.ACTION_INSERT);
      ...
      while (insertRow != null) {
        ...
        if (insertRow != null) {
          ...
          PSJdbcExecutionStep insertStep =
              PSJdbcStatementFactory.getInsertStatement(m_dbmsDef, m_destTableSchema, insertRow);
