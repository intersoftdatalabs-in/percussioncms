...
public class PSJdbcReplaceExecutionPlan {
  ...
  public void addStep(PSJdbcExecutionStep firstStep, PSJdbcExecutionStep secondStep) {
    if (firstStep == null) throw new IllegalArgumentException("first step may not be null");

    if (secondStep == null) throw new IllegalArgumentException("second step may not be null");
  ...
  public void execute(Connection conn, boolean forward) throws SQLException {
    ...
    int index = 0;
    if (!forward) index = m_firstSteps.size();
    ...
    if (forward) continueLoop = firstList.hasNext();
    else continueLoop = firstList.hasPrevious();
        ...
        if (errorStep != null) {
          ...
          try {
            ...
            PSJdbcTableFactory.logMessage(
                "error step failed: " + PSJdbcTableFactoryException.formatSqlException(e2));
            e.setNextException(e2);
          }
        }
        if (isFirstStep) {
          if (firstStep.stopOnError()) throw e;
        } else {
          if (secondStep.stopOnError()) throw e;
        }
      }
      if (forward) continueLoop = firstList.hasNext();
      else continueLoop = secondList.hasPrevious();
    }
  }
