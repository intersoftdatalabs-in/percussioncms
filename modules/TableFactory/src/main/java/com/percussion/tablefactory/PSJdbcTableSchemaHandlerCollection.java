...
public class PSJdbcTableSchemaHandlerCollection extends PSCollection {
   ...
   */
  public PSJdbcTableSchemaHandlerCollection(Element sourceNode) throws PSJdbcTableFactoryException {
    this();
    fromXml(sourceNode);
  }
  ...
  public final void fromXml(Element sourceNode) throws PSJdbcTableFactoryException {
    ...
    Element schemaHandler = walker.getNextElement(PSJdbcTableSchemaHandler.NODE_NAME, firstFlags);
    ...
    while (schemaHandler != null) {
      ...
      schemaHandler = walker.getNextElement(PSJdbcTableSchemaHandler.NODE_NAME, nextFlags);
    }
  }
  ...
  /**
   * Returns a table schema handler object of the specified type if it exists in this collection,
   * otherwise returns <code>null</code>.
   *
   * @param schemaHandlerType The type of the table schema handler to locate.
   * @return a table schema handler object of the specified type if it exists in this collection,
   *     otherwise returns <code>null</code>.
   * @throws IllegalArgumentException if <code>schemaHandlerType</code> is not one of the following
   *     values: <code>PSJdbcTableSchemaHandler.TYPE_INT_NO_ALTER_TABLE_STMT</code> <code>
   *     PSJdbcTableSchemaHandler.TYPE_INT_TO_BACKUP</code> <code>
   *     PSJdbcTableSchemaHandler.TYPE_INT_FROM_BACKUP</code> <code>
   *     PSJdbcTableSchemaHandler.TYPE_INT_NO_ALTER_TABLE_STMT</code>
   */
  public PSJdbcTableSchemaHandler getTableSchemaHandler(int schemaHandlerType) {
    ...
    for (Object o : this) {
      ...
      if (schemaHandler.getType() == schemaHandlerType) return schemaHandler;
    }
    return null;
  }
