...
public class PSJdbcPlanBuilder {
  ...
  public static int createSchemaPlan(
      ...
      PSJdbcExecutionPlan schemaPlan)
    ...
    if (schemaPlan == null) throw new IllegalArgumentException("schemaPlan may not be null");

    tableSchema.validateSchema();

    // create empty plan
    PSJdbcTableFactory.logMessage("");
    PSJdbcTableFactory.logMessage("Creating schema plan for table: " + tableSchema.getName());
    ...
    if (addListeners) {
      if (dbmsDef.getConnectionDetail() != null) {
        ...
        changeEvent =
            new PSJdbcTableChangeEvent(
                dbmsDef.getConnectionInfo(),
                tableSchema.getName(),
    ...
    PSJdbcTableSchema curSchema =
        PSJdbcTableFactory.catalogTable(
            conn, dbmsDef, tableSchema.getDataTypeMap(), tableSchema.getName(), false);
    ...
    if (!tableExists) {
      ...
      if (!tableChanges.hasChanges()) {
        ...
        if (tableSchema.isDelOldData() && hasRows) {
          ...
          schemaPlan.addStep(PSJdbcStatementFactory.getClearTableStatement(dbmsDef, tableSchema));
        }
      } else if ((tableChanges.canBeAltered())
          && (tableSchema.getTableSchemaHandler(
                  PSJdbcTableSchemaHandler.TYPE_INT_NO_ALTER_TABLE_STMT)
              == null)) {
  ...
  private static void createMissingTable(
      Connection conn,
      PSJdbcDbmsDef dbmsDef,
      PSJdbcTableSchema tableSchema,
      PSJdbcExecutionPlan schemaPlan) {
    // no table and allowed to create the table
    PSJdbcTableFactory.logMessage("Table doesn't exist and create=y, creating table");

    schemaPlan.addStep(PSJdbcStatementFactory.getCreateTableStatement(dbmsDef, tableSchema));

    // add step to create non-unique indexes
    schemaPlan.addStep(PSJdbcStatementFactory.getCreateIndexStatements(dbmsDef, tableSchema));

    // check if this table schema has a schema handler of type
    // "onCreate"
    PSJdbcTableSchemaHandler schemaHandler =
        tableSchema.getTableSchemaHandler(PSJdbcTableSchemaHandler.TYPE_INT_ON_CREATE);
    if (schemaHandler != null) {
      // add a step for processing the schema handler after this
      // table is created
      PSJdbcTableSchemaHandlerStep handlerStep =
          new PSJdbcTableSchemaHandlerStep(dbmsDef, tableSchema, null, schemaHandler);
      schemaPlan.addStep(handlerStep);
    }
  }

  private static void alterModifiedTable(
      Connection conn,
      PSJdbcDbmsDef dbmsDef,
      PSJdbcTableSchema tableSchema,
      PSJdbcExecutionPlan schemaPlan,
      ...
      PSJdbcTableChangeEvent changeEvent,
      List<IPSJdbcTableChangeListener> changeListeners) {
    ...
    PSJdbcExecutionStep step = PSJdbcStatementFactory.getAlterTableStatement(dbmsDef, tableChanges);

    // add change event to this step
    if (addListeners) step.addTableChangeEvent(changeEvent, changeListeners);

    schemaPlan.addStep(step);

    // add step to drop non-unique indexes
    schemaPlan.addStep(PSJdbcStatementFactory.getDropIndexStatements(dbmsDef, tableChanges));
    // add step to create non-unique indexes
    schemaPlan.addStep(PSJdbcStatementFactory.getCreateIndexStatements(dbmsDef, tableChanges));
  }

  private static void recreateExisitngTable(
      Connection conn,
      PSJdbcDbmsDef dbmsDef,
      PSJdbcTableSchema curSchema,
      PSJdbcTableSchema tableSchema,
     ...
     * want to create
     */
    PSJdbcTableSchema newSchema =
        new PSJdbcTableSchema(
            tableSchema.isAlter() ? addTableChanges(curSchema, tableSchema) : tableSchema);
    ...
    if (newSchema.isDelOldData() || !hasRows) {
      ...
      PSJdbcTableFactory.logMessage(
          "No data or else deleting " + "old data, backup, dropping and recreating");
    } else {
      // backup, drop, create, and restore
      PSJdbcTableFactory.logMessage("Table has data, backup, dropping, recreating, restore data");
      copyData = true;
    }
    ...
    try {
      ...
      String[] names = {"TABLE"};
      ResultSet rsTableNames = dmd.getTables(null, dbmsDef.getSchema(), "%", names);

      while (rsTableNames.next()) {
        String tableName = rsTableNames.getString("TABLE_NAME");

        // get FKs for a given table
        ResultSet rs = dmd.getImportedKeys(dbmsDef.getDataBase(), dbmsDef.getSchema(), tableName);
        if (rs != null) {
          while (rs.next()) {
            // get table name
            String fTableName = rs.getString(3);
            if (fTableName.compareToIgnoreCase(curSchema.getName()) != 0) // is it our child?
            continue;

            // attempt to catalog the table, no data.
            PSJdbcTableSchema curFKSchema =
                PSJdbcTableFactory.catalogTable(
                    conn, dbmsDef, tableSchema.getDataTypeMap(), tableName, false);
      ...
      Iterator<PSJdbcTableSchema> itTableSchemasFithFK = curSchema.getTablesWithForeignKey();
      ...
      while (itTableSchemasFithFK.hasNext()) {
        ...
        for (PSJdbcForeignKey fk : fks) {
          ...
          String dropFKSQL = PSJdbcStatementFactory.getDropFKContraint(dbmsDef, schemaFK, fk);
      ...
      String backupTableName = getBackupTableName(conn, newSchema.getName());
      boolean backupExists =
          PSJdbcTableFactory.catalogTable(
                  conn, dbmsDef, newSchema.getDataTypeMap(), backupTableName, false)
              != null;

      // don't create backup with any constraints
      PSJdbcTableSchema backupSchema = new PSJdbcTableSchema(curSchema);
      ...
      PSJdbcTableSchemaHandler schemaHandler =
          tableSchema.getTableSchemaHandler(PSJdbcTableSchemaHandler.TYPE_INT_TO_BACKUP);
      schemaPlan.addStep(
          getCopyTableStatement(
              dbmsDef, backupSchema, backupTableName, backupExists, true, schemaHandler));

      // drop it
      schemaPlan.addStep(
          PSJdbcStatementFactory.getDropTableStatement(dbmsDef, newSchema.getName()));

      // recreate
      PSJdbcExecutionStep createStep =
          PSJdbcStatementFactory.getCreateTableStatement(dbmsDef, newSchema);
      ...
      PSJdbcExecutionStep errorStep =
          getCopyTableStatement(dbmsDef, backupSchema, curSchema.getName(), false, false, null);
      createStep.setErrorStep(errorStep);
      schemaPlan.addStep(createStep);

      // add step to create non-unique indexes
      PSJdbcExecutionStep createIndexStep =
          PSJdbcStatementFactory.getCreateIndexStatements(dbmsDef, newSchema);
      ...
      if (copyData) {
        ...
        schemaHandler =
            tableSchema.getTableSchemaHandler(PSJdbcTableSchemaHandler.TYPE_INT_FROM_BACKUP);
        if (schemaHandler != null) {
          dataStep =
              new PSJdbcTableSchemaHandlerStep(dbmsDef, backupSchema, newSchema, schemaHandler);
        } else {
          dataStep =
              PSJdbcStatementFactory.getCopyTableDataStatement(dbmsDef, backupSchema, newSchema);
        }
        ...
        errorStep =
            getCopyTableStatement(dbmsDef, backupSchema, curSchema.getName(), true, false, null);
        dataStep.setErrorStep(errorStep);
      }

      // restore previously removed FKs
      itTableSchemasFithFK = curSchema.getTablesWithForeignKey();

      while (itTableSchemasFithFK.hasNext()) {
        PSJdbcTableSchema schema = itTableSchemasFithFK.next();

        String fullFKSQL =
            PSJdbcStatementFactory.getForeignKeyConstraint(dbmsDef, schema, newSchema);

        if (fullFKSQL == null) continue;
        ...
        String restoreFKStep = "ALTER TABLE " + fullTableName + " ADD " + fullFKSQL;
      ...
      if (addListeners && listenerStep != null) {
        listenerStep.addTableChangeEvent(changeEvent, changeListeners);
      }
  ...
  public static void getDataPlan(
    ...
    if (insertPlan == null) throw new IllegalArgumentException("insertPlan may not be null");

    if (deletePlan == null) throw new IllegalArgumentException("deletePlan may not be null");

    if (updatePlan == null) throw new IllegalArgumentException("updatePlan may not be null");

    if (replacePlan == null) throw new IllegalArgumentException("replacePlan may not be null");
    ...
    while (rows.hasNext()) {
      ...
      if (bOnTableCreateOnly && schemaAction != SCHEMA_ACTION_CREATE) continue;
      ...
      switch (rowAction) {
        case PSJdbcRowData.ACTION_INSERT:
          insertStep = PSJdbcStatementFactory.getInsertStatement(dbmsDef, tableSchema, row);
        ...
        case PSJdbcRowData.ACTION_INSERT_IF_NOT_EXIST:
          insertStep = PSJdbcStatementFactory.getInsertStatement(dbmsDef, tableSchema, row);
          insertPlan.addStep(insertStep);
          insertStep.setIgnoreSQLExceptions(PSSqlHelper.SQLSTATE_INTEGRITY_CONSTRAINT_VIOLATIONS);
          processChildTables(
              conn,
              dbmsDef,
              tableMetaMap,
        ...
        case PSJdbcRowData.ACTION_UPDATE:
          processChildTables(
              ...
              isChildTable,
              row);
          updateStep = PSJdbcStatementFactory.getUpdateStatement(dbmsDef, tableSchema, row);
        ...
        case PSJdbcRowData.ACTION_REPLACE:
          ...
          updateStep.setIgnoreSQLExceptions(PSSqlHelper.SQLSTATE_NO_UPDATE_ROWS);
          insertStep = PSJdbcStatementFactory.getInsertStatement(dbmsDef, tableSchema, row);
          insertStep.setIgnoreSQLExceptions(PSSqlHelper.SQLSTATE_INTEGRITY_CONSTRAINT_VIOLATIONS);
        ...
        case PSJdbcRowData.ACTION_DELETE:
          ...
          */
          ...
          deleteStep = PSJdbcStatementFactory.getDeleteStatement(dbmsDef, tableSchema, row);
  ...
  private static void processChildTables(
    ...
    while (childTables.hasNext()) {
      ...
      PSJdbcTableSchema childTableSchema =
          PSJdbcTableFactory.getTableSchemaCollection().getTableSchema(childTableData.getName());
      ...
      while (childTblIt.hasNext()) {
        PSJdbcTableSchema childTableSchema =
            PSJdbcTableFactory.getTableSchemaCollection()
                .getTableSchema((String) childTblIt.next());
        ...
        if (dbmsDef.getSchema() != null) {
          PSJdbcTableSchema testChildTblSchema =
              PSJdbcTableFactory.catalogTable(
                  conn, dbmsDef, tableMetaMap, dataTypeMap, childTableSchema.getName(), false);
          if (testChildTblSchema == null) continue;
        }
        ...
        for (PSJdbcForeignKey fkey : fkeys) {
          ...
          while (fkeyColIt.hasNext()) {
            ...
            PSJdbcColumnDef colDef = childTableSchema.getColumn(childTableColName);

            PSJdbcColumnData colData = row.getColumn(parentTableColName);
            if (colData == null) continue;
            if (colData.getValue() == null) continue;
        ...
        PSJdbcTableData childTblData =
            PSJdbcTableFactory.catalogTableData(
                conn, dbmsDef, childTableSchema, pkColumns, selFilter, PSJdbcRowData.ACTION_DELETE);
        ...
        PSJdbcTableData childTableData =
            doTableDataDiff(childTableSchema, childTblData, memChildTblData);
  ...
  private static boolean canUnpublishChildTables(
      ...
      PSJdbcTableSchema childTableSchema,
      PSJdbcRowData parentRowData) {
    if (childTableSchema == null) return false;
  ...
  private static PSJdbcTableData doTableDataDiff(
      ...
      PSJdbcTableData memChildTblData)
      throws PSJdbcTableFactoryException {
    if (tableSchema == null) throw new IllegalArgumentException("tableSchema may not be null");
  ...
  public static boolean hasSameKeyValues(
      PSJdbcTableSchema tableSchema, PSJdbcRowData rowData, PSJdbcRowData rowDataToCompare)
      throws PSJdbcTableFactoryException {
    if (rowData == null) throw new IllegalArgumentException("row data may not be null");
  ...
  private static void setLogData(
    ...
    if (pkColNames.hasNext()) {
      ...
      if (columns.size() > 0) {
        PSJdbcRowData pkRow = new PSJdbcRowData(columns.iterator(), PSJdbcRowData.ACTION_INSERT);
  ...
  private static void addChildTablesDeletePlan(
      PSJdbcDbmsDef dbmsDef,
      PSJdbcTableSchema parentTableSchema,
      PSJdbcTableSchema childTableSchema,
    ...
    if (parentTableSchema == null)
      throw new IllegalArgumentException("parentTableSchema may not be null");

    if (childTableSchema == null)
      throw new IllegalArgumentException("childTableSchema may not be null");
    ...
    for (PSJdbcForeignKey foreignKey : foreignKeys) {
      ...
      while (cols.hasNext()) {
        ...
        String childTableColName = col[0];
        String parentTableColName = col[2];
        PSJdbcColumnData parentColData = parentRow.getColumn(parentTableColName);
        if (parentColData != null) {
          PSJdbcColumnData childColData =
              new PSJdbcColumnData(
                  childTableColName, parentColData.getValue(), parentColData.getEncoding());
        ...
        while (grandChildTables.hasNext()) {
          ...
          while (childRows.hasNext()) {
            addChildTablesDeletePlan(
                dbmsDef,
                childTableSchema,
                grandChildTableSchema,
                (PSJdbcRowData) childRows.next(),
                deletePlan);
          }
        }
        deletePlan.addStep(
            PSJdbcStatementFactory.getDeleteStatement(dbmsDef, childTableSchema, childRow));
      }
    }
  }
  ...
  private static PSJdbcExecutionStep getCopyTableStatement(
      ...
      boolean failOnDataCopyError,
      PSJdbcTableSchemaHandler schemaHandler) {
    PSJdbcExecutionBlock block = new PSJdbcExecutionBlock();

    // drop current target if it exists
    if (targetExists)
      block.addStep(PSJdbcStatementFactory.getDropTableStatement(dbmsDef, targetTableName));
    ...
    // create the table
    block.addStep(PSJdbcStatementFactory.getCreateTableStatement(dbmsDef, targetSchema));

    // copy the data
    PSJdbcExecutionStep dataStep = null;
    if (schemaHandler != null) {
      dataStep =
          new PSJdbcTableSchemaHandlerStep(dbmsDef, tableSchema, targetSchema, schemaHandler);
    } else {
      dataStep =
          PSJdbcStatementFactory.getCopyTableDataStatement(dbmsDef, tableSchema, targetSchema);
    }
  ...
  private static PSJdbcTableSchema getTableChanges(
    ...
    PSJdbcTableSchema diffTableSchema =
        new PSJdbcTableSchema(newSchema.getName(), changedCols.iterator());
    diffTableSchema.setCreate(false);
    diffTableSchema.setAlter(true);

    // check primary key for changes
    int flags =
        PSJdbcTableComponent.COMPARE_IGNORE_NAME | PSJdbcTableComponent.COMPARE_IGNORE_ACTION;
    ...
    if (newPKey == null && oldPKey != null) {
      PSJdbcPrimaryKey difPKey =
          new PSJdbcPrimaryKey(oldPKey.getColumnNames(), PSJdbcTableComponent.ACTION_DELETE);
      ...
      if (oldPKey == null) {
        ...
        if (newPKey.compare(oldPKey, flags) >= PSJdbcTableComponent.IS_EXACT_MATCH) {
      ...
      PSJdbcPrimaryKey difPKey = new PSJdbcPrimaryKey(newPKey.getColumnNames(), pkAction);
      diffTableSchema.setPrimaryKey(difPKey);
    }
    ...
    for (PSJdbcForeignKey fk : oldFKeys) {
      ...
      if (idxFK == -1) {
        PSJdbcForeignKey difFKey =
            new PSJdbcForeignKey(fk.getName(), fk.getColumns(), PSJdbcTableComponent.ACTION_DELETE);
    ...
    for (PSJdbcForeignKey fk2 : newFKeys) {
      if (!updateKeys.contains(fk2)) {
        ...
        PSJdbcForeignKey difFKey =
            new PSJdbcForeignKey(
                fk2.getName(), fk2.getColumns(), PSJdbcTableComponent.ACTION_CREATE);
        updateKeys.add(difFKey);
      }
    }
    ...
    Iterator<PSJdbcIndex> newIndexes =
        newSchema.getIndexes(PSJdbcIndex.TYPE_UNIQUE | PSJdbcIndex.TYPE_NON_UNIQUE);

    // Get Old Indexes from existing schema
    Iterator<PSJdbcIndex> oldIndexes =
        oldSchema.getIndexes(PSJdbcIndex.TYPE_UNIQUE | PSJdbcIndex.TYPE_NON_UNIQUE);
    ...
    while (newIndexes.hasNext()) {
      ...
      if (oldIdx != null) {
        ...
        PSJdbcIndex difIndex =
            new PSJdbcIndex(
                newIndex.getName(), newIndex.getColumnNames(), indexAction, newIndex.getType());
    ...
    // Check old indexes that are not in new schema, and validate them, if are having valid column
    // names, else delete them
    for (PSJdbcIndex oldIdx : oldIdxList) {
      ...
      for (Iterator<String> it = colNames; it.hasNext(); ) {
        ...
        if (diffTableSchema.getColumn(colName) == null) {
          ...
          PSJdbcIndex difIndex =
              new PSJdbcIndex(
                  oldIdx.getName(), oldIdx.getColumnNames(), indexAction, oldIdx.getType());
          processedIdx.add(difIndex);
          break;
        }
      }
    ...
    if (newUKey != null) {
      ...
      diffTableSchema.setUpdateKey(new PSJdbcUpdateKey(newUKey.getColumnNames()));
    } else if (oldUKey != null) {
      // only defined in old schema, use it
      diffTableSchema.setUpdateKey(new PSJdbcUpdateKey(oldUKey.getColumnNames()));
    }
  ...
  private static PSJdbcTableSchema addTableChanges(
      PSJdbcTableSchema oldSchema, PSJdbcTableSchema newSchema) throws PSJdbcTableFactoryException {
    ...
    if (newPKey != null) {
      ...
      if (colAction == PSJdbcTableComponent.ACTION_DELETE) resultSchema.setPrimaryKey(null);
      else if (colAction != PSJdbcTableComponent.ACTION_NONE) resultSchema.setPrimaryKey(newPKey);
    }
    ...
    for (PSJdbcForeignKey newFKey : newFKeys) {
      ...
      if (colAction != PSJdbcTableComponent.ACTION_NONE
          && colAction != PSJdbcTableComponent.ACTION_DELETE) changedFKeys.add(newFKey);
    }
    ...
    while (indexes.hasNext()) {
      ...
      else if (action != PSJdbcTableComponent.ACTION_NONE) resultSchema.setIndex(newIndex);
    }

    // process update key - if defined in newSchema, use it
    PSJdbcUpdateKey newUKey = newSchema.getUpdateKey();
    if (newUKey != null) resultSchema.setUpdateKey(newUKey);
