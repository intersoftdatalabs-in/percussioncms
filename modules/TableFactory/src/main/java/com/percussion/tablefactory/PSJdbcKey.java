...
public abstract class PSJdbcKey extends PSJdbcTableComponent {
  ...
  protected PSJdbcKey(String name, int action, Iterator<String> names, String containerName)
    ...
    if (!names.hasNext()) throw new IllegalArgumentException("names may not be empty");

    if (containerName == null || containerName.trim().length() == 0)
      throw new IllegalArgumentException("containerName may not be null or empty");
  ...
  protected Element toXml(Document doc, String nodeName) {
    ...
    // now add each name
    for (String m_name : m_names) PSXmlDocumentBuilder.addElement(doc, root, NAME_EL, m_name);
  ...
  protected final void fromXml(Element sourceNode, String nodeName)
    ...
    Element nameEl = walker.getNextElement(NAME_EL, PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    ...
    do {
      ...
      nameEl = walker.getNextElement(NAME_EL, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    } while (nameEl != null);
  ...
  public int compare(Object obj, int flags) {
    ...
    do {
      ...
      if (!((match == IS_CASE_INSENSITIVE_MATCH) && (compareColumns == IS_EXACT_MATCH))) {
  ...
  protected int compareColumns(List<String> cols1, List<String> cols2) {
    ...
    do {
      ...
      // We need to match the columns in same order as some Indexes and keys are having same columns
      // but different odrder of indexing
