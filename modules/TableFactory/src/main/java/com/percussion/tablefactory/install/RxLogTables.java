...
public class RxLogTables {
   ...
   */
  public static void createLogTables(
      String driver, String server, String db, String uid, String pw, String schema)
      throws SQLException {
    ...
    if (conn == null) throw new SQLException("Could not connect to server: " + server);
    ...
    if (intTypeName == null) intTypeName = "INT";
    if (varcharTypeName == null) varcharTypeName = "VARCHAR2";

    if (intTypeName == null) throw new SQLException("INT type is not supported in target");
    else if (varcharTypeName == null)
      throw new SQLException("VARCHAR type is not supported in target");

    if (tinyintTypeName == null) tinyintTypeName = intTypeName; // promote the type, don't error

    Statement stmt = PSSQLStatement.getStatement(conn);
    if (!(hasTable(targetMeta, schema, PSLogDatabase.TABLE_PSLOG))) {
      StringBuilder buf = new StringBuilder(512);
      buf.append("CREATE TABLE ");
      buf.append(PSSqlHelper.qualifyTableName(PSLogDatabase.TABLE_PSLOG, db, schema, driver));
      buf.append(" (");
      ...
      buf.append(
          PSSqlHelper.qualifyPrimaryKeyName(PSLogDatabase.COL_PSLOG_PKEY, db, schema, driver));
      buf.append(" primary key ");

      buf.append("(" + PSLogDatabase.COL_LOG_ID_HIGH + ", " + PSLogDatabase.COL_LOG_ID_LOW + "))");
    ...
    if (!(hasTable(targetMeta, schema, PSLogDatabase.TABLE_PSLOGDAT))) {
      StringBuilder buf = new StringBuilder(512);
      buf.append("CREATE TABLE ");
      buf.append(PSSqlHelper.qualifyTableName(PSLogDatabase.TABLE_PSLOGDAT, db, schema, driver));
      buf.append(" (");
      buf.append(" " + PSLogDatabase.COL_LOG_ID_HIGH + " ");
      buf.append(intTypeName);
      buf.append(" NOT NULL ");
      ...
      buf.append(", constraint ");
      buf.append(
          PSSqlHelper.qualifyPrimaryKeyName(PSLogDatabase.COL_PSLOGDAT_PKEY, db, schema, driver));
      buf.append(" primary key ");

      buf.append(
          "("
              + PSLogDatabase.COL_LOG_ID_HIGH
              + ", "
              + PSLogDatabase.COL_LOG_ID_LOW
              + ", "
              + PSLogDatabase.COL_LOG_SEQ
              + ", "
  ...
  public static Connection createConnection(
    ...
    if (className == null) throw new SQLException("Cannot load driver of " + driver);
    try {
   ...
   * @throws IllegalArgumentException for any illegal argument
   */
  private static boolean hasTable(DatabaseMetaData md, String schema, String table)
      throws SQLException {
    if (md == null) throw new IllegalArgumentException("need a valid database meta data object");
