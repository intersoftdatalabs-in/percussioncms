...
public class PSJdbcTableSchema implements Comparable<PSJdbcTableSchema> {
   ...
   * @param name The name of this table, may not be <code>null</code> or empty.
   */
  public PSJdbcTableSchema(String name) throws PSJdbcTableFactoryException {
  ...
  public PSJdbcTableSchema(PSJdbcTableSchema source) {
    ...
    try {
      ...
      if (pk != null) m_primaryKey = new PSJdbcPrimaryKey(pk.getColumnNames(), pk.getAction());
      List<PSJdbcForeignKey> fks = source.getForeignKeys();
      if (fks != null) {
        List<PSJdbcForeignKey> newKeys = new ArrayList<>();
        for (PSJdbcForeignKey fk : fks) {
          newKeys.add(new PSJdbcForeignKey(fk.getColumns(), fk.getAction()));
        }
        m_foreignKeys = newKeys;
      }

      PSJdbcUpdateKey uk = source.getUpdateKey();
      if (uk != null) m_updateKey = new PSJdbcUpdateKey(uk.getColumnNames());

      Iterator<PSJdbcColumnDef> cols = source.getColumns();
      while (cols.hasNext()) m_columns.add(new PSJdbcColumnDef(cols.next()));

      Iterator<PSJdbcIndex> indexes =
          source.getIndexes(PSJdbcIndex.TYPE_UNIQUE | PSJdbcIndex.TYPE_NON_UNIQUE);

      while (indexes.hasNext()) {
        PSJdbcIndex index = indexes.next();
        m_indexes.add(
            new PSJdbcIndex(
                index.getName(), index.getColumnNames(), index.getAction(), index.getType()));
      }

    } catch (PSJdbcTableFactoryException e) {
      // cant' really happen!
      throw new IllegalArgumentException("invalid source schema: " + e);
    }
  }
  ...
  public PSJdbcColumnDef getColumn(String name) {
    ...
    if (index > -1) column = m_columns.get(index);
  ...
  public void resetIndexes(List<PSJdbcIndex> indexes) throws PSJdbcTableFactoryException {
    if (indexes == null) throw new IllegalArgumentException("indexes may not be null");
    m_indexes.clear();
  ...
  public PSJdbcIndex getIndex(PSJdbcIndex newIndex) {
    if (newIndex == null) throw new IllegalArgumentException("index may not be null");
  ...
  public Iterator<PSJdbcIndex> getIndexes(int type) {
    ...
    for (PSJdbcIndex index : m_indexes) {
      if (index.isOfType(type)) indexList.add(index);
    }
  ...
  public Element toXml(Document doc) {
    ...
    if (m_primaryKey != null) root.appendChild(m_primaryKey.toXml(doc));
    ...
    if (m_updateKey != null) root.appendChild(m_updateKey.toXml(doc));

    // add indexes if any defined
    if (!m_indexes.isEmpty()) {
      Element indexdefs = PSXmlDocumentBuilder.addEmptyElement(doc, root, INDEX_DEF_EL);
  ...
  public final void fromXml(Element sourceNode, PSJdbcDataTypeMap dataTypeMap)
    ...
    if (sTemp.trim().length() != 0) m_allowSchemaChanges = sTemp.equalsIgnoreCase(XML_TRUE);

    sTemp = sourceNode.getAttribute(IS_VIEW_ATTR);
    if (sTemp.trim().length() != 0) m_isView = sTemp.equalsIgnoreCase(XML_TRUE);

    sTemp = sourceNode.getAttribute(CREATE_ATTR);
    if (sTemp.trim().length() != 0) m_create = sTemp.equalsIgnoreCase(XML_TRUE);

    sTemp = sourceNode.getAttribute(DELOLDDATA_ATTR);
    if (sTemp.trim().length() != 0) m_delOldData = sTemp.equalsIgnoreCase(XML_TRUE);

    sTemp = sourceNode.getAttribute(ALTER_ATTR);
    if (sTemp.trim().length() != 0) m_alter = sTemp.equalsIgnoreCase(XML_TRUE);

    // load schema handlers
    walker.setCurrent(sourceNode);
    Element schemaHandlersEl =
        walker.getNextElement(PSJdbcTableSchemaHandlerCollection.NODE_NAME, firstFlags);
    if (schemaHandlersEl != null) {
      m_schemaHandlerCollection = new PSJdbcTableSchemaHandlerCollection(schemaHandlersEl);
    }
    ...
    walker.setCurrent(sourceNode);
    Element primaryKey = walker.getNextElement(PSJdbcPrimaryKey.NODE_NAME, firstFlags);
    if (primaryKey != null) {
    ...
    } else m_primaryKey = null;

    // load foreign key if defined
    walker.setCurrent(sourceNode);
    Element foreignKey = walker.getNextElement(PSJdbcForeignKey.NODE_NAME, firstFlags);
    m_foreignKeys.clear();
    while (foreignKey != null) {
      ...
      PSJdbcTableSchemaCollection tableSchemaColl = PSJdbcTableFactory.getTableSchemaCollection();
      ...
      foreignKey = walker.getNextElement(PSJdbcForeignKey.NODE_NAME, firstFlags);
    }

    // load update key if defined
    walker.setCurrent(sourceNode);
    Element updateKey = walker.getNextElement(PSJdbcUpdateKey.NODE_NAME, firstFlags);
    if (updateKey != null) {
      m_updateKey = new PSJdbcUpdateKey(updateKey);
    } else m_updateKey = null;
  ...
  private void createIndexesForForeignKey(PSJdbcDataTypeMap dataTypeMap)
      throws PSJdbcTableFactoryException {
    if (!(dataTypeMap.isCreateForeignKeyIndexes())) return;
    if (m_foreignKeys == null || m_foreignKeys.isEmpty()) return;
    if (doForeignKeyColumnsMatch(m_foreignKeys)) return;
   ...
   * @throws PSJdbcTableFactoryException if error occurs
   */
  private void processCreationOfIndexes(List<PSJdbcForeignKey> foreignKeys)
      throws PSJdbcTableFactoryException {
    for (PSJdbcForeignKey foreignKey : foreignKeys) {
  ...
  public void setSchemaAction(int schemaAction) {
    ...
    throw new IllegalArgumentException("invalid schema action : " + schemaAction);
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSJdbcTableSchema)) isMatch = false;
    else {
      PSJdbcTableSchema other = (PSJdbcTableSchema) obj;
      if (!this.m_columns.equals(other.m_columns)) isMatch = false;
      else if (this.m_primaryKey != null ^ other.m_primaryKey != null) isMatch = false;
      else if (this.m_primaryKey != null && !this.m_primaryKey.equals(other.m_primaryKey))
        isMatch = false;
      else if (this.m_updateKey != null ^ other.m_updateKey != null) isMatch = false;
      else if (this.m_updateKey != null && !this.m_updateKey.equals(other.m_updateKey))
        isMatch = false;
      else if (this.m_create != other.m_create) isMatch = false;
      else if (this.m_delOldData != other.m_delOldData) isMatch = false;
      else if (this.m_alter != other.m_alter) isMatch = false;
      else if (this.m_allowSchemaChanges != other.m_allowSchemaChanges) isMatch = false;
      else if (this.m_isView != other.m_isView) isMatch = false;
      else if (this.m_foreignKeys != null ^ other.m_foreignKeys != null) isMatch = false;
      else if (this.m_foreignKeys != null && !this.m_foreignKeys.equals(other.m_foreignKeys))
        isMatch = false;
      else if (!this.m_indexes.equals(other.m_indexes)) isMatch = false;
      else if (this.m_tableData != null ^ other.m_tableData != null) isMatch = false;
      else if (this.m_tableData != null && !this.m_tableData.equals(other.m_tableData))
        isMatch = false;
    }
  ...
  public int hashCode() {
    ...
    if (m_foreignKeys != null) hash += m_foreignKeys.hashCode();
    if (m_primaryKey != null) hash += m_primaryKey.hashCode();
    if (m_updateKey != null) hash += m_updateKey.hashCode();
    hash += m_indexes.hashCode();
    if (m_tableData != null) hash += m_tableData.hashCode();
  ...
  public boolean hasChanges() {
    ...
    if (!hasChanges && m_primaryKey != null) hasChanges = m_primaryKey.hasChanges();
  ...
  public boolean canBeAltered() {
    ...
    if (m_primaryKey != null && !m_primaryKey.canAlter()) return false;

    for (PSJdbcForeignKey foreignKey : m_foreignKeys) {
      if (!foreignKey.canAlter()) return false;
    }
  ...
  public List<String> getKeyColumns() {
    ...
    while (colNames != null && colNames.hasNext()) keyCols.add(colNames.next());
  ...
  public void addParentTable(String parentTableName) {
    if ((parentTableName == null) || (parentTableName.trim().length() == 0))
      throw new IllegalArgumentException("parentTableName may not be null or empty");

    if (!(m_parentTables.contains(parentTableName))) m_parentTables.add(parentTableName);
  }
  ...
  public void addChildTable(String childTableName) {
    if ((childTableName == null) || (childTableName.trim().length() == 0))
      throw new IllegalArgumentException("childTableName may not be null or empty");

    if (!(m_childTables.contains(childTableName))) m_childTables.add(childTableName);
  }
  ...
  public void addSchemaChangeListener(IPSJdbcTableChangeListener listener) {
    ...
    if (m_schemaChangeListeners == null) m_schemaChangeListeners = new ArrayList<>();
  ...
  private String checkColumns(Iterator<String> names) {
    ...
    while (names.hasNext()) {
      ...
      if (getColumnIndex(name) == -1) badNameList.add(name);
    }
  ...
  public int compareTo(PSJdbcTableSchema o) {
    ...
    int value =
        foreignKeyCompare(o.getName(), this.getForeignKeys())
            - foreignKeyCompare(this.getName(), o.getForeignKeys());
