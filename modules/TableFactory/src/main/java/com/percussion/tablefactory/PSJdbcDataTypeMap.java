...
public class PSJdbcDataTypeMap {
  ...
  public PSJdbcDataTypeMap(Document doc, String dbAlias, String driver, String os)
    ...
    if (dbAlias == null) dbAlias = "";
    if (driver == null) driver = "";
    if (driver.equals("") && dbAlias.equals(""))
      throw new IllegalArgumentException("either dbAlias or driver must not be null or empty");
  ...
  private void initData(Document doc, String dbAlias, String driver, String os)
    ...
    while (map != null) {
      ...
      if (!dbAlias.equals("") && !dbAlias.equalsIgnoreCase(map.getAttribute(DB_ATTR))) {
        isMatch = false;
      }
      ...
      if (isMatch && !driver.equals("")) {
        ...
        if (driverAttr.trim().length() == 0) isMatch = false;
        else {
          List<String> drivers = PSStringOperation.getSplittedList(driverAttr, ';');
          if (!drivers.contains(driver)) isMatch = false;
        }
      }
      ...
      if (isMatch && os != null && !os.equals("")) {
        ...
        if (osAttr.trim().length() == 0) isMatch = false;
        else {
          List<String> osList = PSStringOperation.getSplittedList(osAttr, ';');
          if (!osList.contains(os)) isMatch = false;
        }
      }
      ...
      map = tree.getNextElement(MAP_EL, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
    ...
    while (mapping != null) {
      ...
      mapping =
          tree.getNextElement(
              PSJdbcDataTypeMapping.NODE_NAME, PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
  ...
  public PSJdbcDataTypeMap(String dbAlias, String driver, String os)
    ...
    try (InputStream stream = PSJdbcDataTypeMap.class.getResourceAsStream(DEFAULT_MAP_FILE_NAME)) {
      // fix Rx-02-01-0003 by checking for null result
      if (null == stream) throw new IOException("Could not load default mapping XML document");
  ...
  public PSJdbcDataTypeMap(Map<String, PSJdbcDataTypeMapping> mappings)
    ...
    if (mappings == null || mappings.isEmpty())
      throw new IllegalArgumentException("mappings may not be null or empty");
  ...
  public PSJdbcDataTypeMapping getMapping(String jdbcString) {
    if (jdbcString == null || jdbcString.trim().length() == 0)
      throw new IllegalArgumentException("jdbcString may not be null or empty");
  ...
  public String getNativeString(String jdbcString) {
    ...
    PSJdbcDataTypeMapping dataType = m_strJdbc2NativeMap.get(jdbcString);
    if (dataType != null) nativeString = dataType.getNative();
    return nativeString;
  }
  ...
  public String getNativeString(int jdbcType) {
    String nativeString = null;
    PSJdbcDataTypeMapping dataType = m_intJdbc2NativeMap.get(jdbcType);
    if (dataType != null) nativeString = dataType.getNative();
    return nativeString;
  }
  ...
  public String getJdbcString(String nativeString) {
    if (nativeString == null || nativeString.trim().length() == 0)
      throw new IllegalArgumentException("nativeString may not be null or empty");
    ...
    if (mappingList != null) {
      ...
      for (PSJdbcDataTypeMapping test : mappingList) {
        if (dataTypeMapping == null) dataTypeMapping = test;
        else if (!dataTypeMapping.isNative2Jdbc() && test.isNative2Jdbc()) dataTypeMapping = test;
      }

      if (dataTypeMapping != null) jdbcString = dataTypeMapping.getJdbc();
    }
  ...
  public int getJdbcType(String nativeString, String size, String scale, int defaultJdbcType)
    ...
    if (mappingList != null) {
      PSJdbcDataTypeMapping dataTypeMapping = null;
      for (PSJdbcDataTypeMapping test : mappingList) {
        if (!test.isNative2Jdbc()) continue;
  ...
  private void processMappings(Map<String, PSJdbcDataTypeMapping> mappings)
    ...
    for (Map.Entry<String, PSJdbcDataTypeMapping> stringPSJdbcDataTypeMappingEntry :
        mappings.entrySet()) {
      Map.Entry<String, PSJdbcDataTypeMapping> entry = stringPSJdbcDataTypeMappingEntry;
      String jdbcStr = entry.getKey();
      PSJdbcDataTypeMapping dataType = entry.getValue();
      ...
      List<PSJdbcDataTypeMapping> dataTypeList =
          m_native2JdbcMap.computeIfAbsent(nativeType, k -> new ArrayList<>());
