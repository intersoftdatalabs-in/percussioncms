...
public class CSVAuditLogger extends AuditLogger {
  ...
  public void writeLog(Event auditEvent) {
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(auditEvent.getId())) writer.write("" + auditEvent.getId());
      writer.write(CSVSeperator);
      if (auditEvent.getEventTime() != null) writer.write(auditEvent.getEventTime());
      writer.write(CSVSeperator);
      if (StringUtils.isNotEmpty(auditEvent.getAction())) writer.write(auditEvent.getAction());
      writer.write(CSVSeperator);
      ...
      writer.write(CSVSeperator);
      if (auditEvent.getOutcome() != null) writer.write(auditEvent.getOutcome());
      writer.write(CSVSeperator);
      if (auditEvent.getMeasurements() != null && !auditEvent.getMeasurements().isEmpty()) {
        ...
        for (Measurement measurement : auditEvent.getMeasurements()) {
          if (measurement.getMetric() != null) {
            writer.write(
                measurement.getMetric().getMetricId()
                    + " - "
                    + measurement.getMetric().getName()
                    + " "
  ...
  private void writeFieldNames() {
    ...
    if (new File(outputFilePath).exists()) return;
