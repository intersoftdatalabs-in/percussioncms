...
<!-- This stylesheet is used to extract all translation units, but with only
	a set of language specific notes, properties, and variants within each unit. -->
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="header" mode="copy">
		<xsl:copy>
			<xsl:apply-templates select="@*" mode="copy" />
			<xsl:apply-templates select="prop[.=$reqlang]"
				mode="copy1" />
		</xsl:copy>
	</xsl:template>
