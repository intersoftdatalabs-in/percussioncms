...
# Usage: ./simple_translate.sh [source_file] [target_locale]
...
# Process the file line by line to translate content between <seg> tags
while IFS= read -r line; do
    ...
    if echo "$line" | grep -q "<seg>[^<]*</seg>" && ! echo "$line" | grep -q "<seg>{[0-9]}</seg>"; then
        ...
        CONTENT=$(echo "$line" | sed -n 's/.*<seg>\([^<]*\)<\/seg>.*/\1/p')
        
        # Skip if content is empty or contains only placeholders like {0}
        if [[ -n "$CONTENT" && ! "$CONTENT" =~ ^[[:space:]]*\{[0-9]+(\,[0-9]+)*\}[[:space:]]*$ ]]; then
            # Translate the content using trans command
            TRANSLATED_CONTENT=$(echo "$CONTENT" | trans -b :$TARGET_LOCALE 2>/dev/null)
            
            # Check if translation succeeded and we got content back
            if [ $? -eq 0 ] && [ -n "$TRANSLATED_CONTENT" ]; then
                # Remove any locale prefix that trans might add (like ":es")
                TRANSLATED_CONTENT=$(echo "$TRANSLATED_CONTENT" | sed -e 's/^:[a-z][a-z]\(-[A-Z][A-Z]\)\?//')
                # Replace the content with translated content
                line=$(echo "$line" | sed "s|$CONTENT|$TRANSLATED_CONTENT|")
            else
