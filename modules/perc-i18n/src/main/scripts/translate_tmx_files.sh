...
echo "Starting translation of TMX files to their respective locales..."
...
for tmx_file in "$I18N_DIR"/*.tmx; do
    ...
    # Skip ResourceBundle.tmx and backup files
    if [[ "$filename" = "ResourceBundle" || "$filename" = *backup* ]]; then
        echo "Skipping $filename.tmx (not a locale file or backup)"
        continue
    fi
