...
# Usage: ./translate_tmx.sh [source_file] [target_locale]
...
while IFS= read -r line; do
    ...
    if echo "$line" | grep -q "<seg>[^<]*</seg>" && ! echo "$line" | grep -q "<seg>{[0-9]}</seg>"; then
        ...
        if [[ -n "$CONTENT" && ! "$CONTENT" =~ ^[[:space:]]*\{[0-9]+(\,[0-9]+)*\}[[:space:]]*$ ]]; then
            # Translate the content using trans command with proper handling
            TRANSLATED_CONTENT=$(echo "$CONTENT" | trans -b :$TARGET_LOCALE 2>/dev/null)
            
            # Check if translation succeeded and got meaningful output
            if [ $? -eq 0 ] && [ -n "$TRANSLATED_CONTENT" ]; then
                # Check if the output looks like it has the locale prefix (like ":en_GB")
                if echo "$TRANSLATED_CONTENT" | grep -q "^:[a-z][a-z]-[A-Z][A-Z]"; then
                    # Remove the locale prefix
                    TRANSLATED_CONTENT=$(echo "$TRANSLATED_CONTENT" | sed 's/^:[a-z][a-z]-[A-Z][A-Z]//')
                fi
