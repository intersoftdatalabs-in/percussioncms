...
public class PSTmxBody extends PSTmxNode {
  ...
  PSTmxBody(IPSTmxDocument TMXDoc, boolean createDefault) {
    if (TMXDoc == null) {
      throw new IllegalArgumentException("TMXDoc must not be null in PSTmxBody constructor");
    }
    m_Parent = TMXDoc;
   ...
   * @throws IllegalArgumentException if supplied TMXDocument is <code>null</code>.
   */
  private Element getBodyElement(IPSTmxDocument TMXDocument) throws IllegalArgumentException {
    if (TMXDocument == null) {
      throw new IllegalArgumentException("TMXDocument must not be null in getBodyElement()");
    }
  ...
  private void processBodyElement(boolean createDefault) {
    NodeList nl =
        m_DOMElement.getElementsByTagName(IPSTmxNode.NODENAMEMAP[IPSTmxNode.TMXTRANSLATIONUNIT]);
    for (int i = 0; nl != null && i < nl.getLength(); i++) {
      PSTmxTranslationUnit tu =
          new PSTmxTranslationUnit(m_PSTmxDocument, (Element) nl.item(i), createDefault);
  ...
  protected void addTranslationUnit(IPSTmxTranslationUnit tu) throws PSTmxDomException {
    ...
    m_TransUnits.put(tu.getKey(), new PSTmxTranslationUnit(getTMXDocument(), newElemTu));
  }
