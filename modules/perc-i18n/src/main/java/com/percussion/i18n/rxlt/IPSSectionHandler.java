...
/**
 * This interface is implemented by all section handlers. Sections are part of the action {@link
 * IPSActionHandler#ACTIONID_GENERATE_TMX_RESOURCES}. This action is split into four logical
 * sections so that the user gets flexibility of generationg TMX resources for translation from only
 * sections that are known to be modified.
 */
public interface IPSSectionHandler {
  /**
   * This is the method that actually processes the section.
   *
   * @param cfgData DOM element holding the data for processing the section , must not be <code>null
   *     </code>.
   * @return The newly created TMX document object that subsequently merged with other sections when
   *     required, never <code>null</code>.
