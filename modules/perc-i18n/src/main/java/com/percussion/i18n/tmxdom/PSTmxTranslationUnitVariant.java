...
public class PSTmxTranslationUnitVariant extends PSTmxNode implements IPSTmxTranslationUnitVariant {
  ...
  PSTmxTranslationUnitVariant(IPSTmxDocument tmxdoc, Element tuv) {
    ...
    if (tuv == null) throw new IllegalArgumentException("tuv must not be null");

    m_DOMElement = tuv;
    m_PSTmxDocument = tmxdoc;

    m_Lang = m_DOMElement.getAttribute(IPSTmxDtdConstants.ATTR_XML_LANG);
    NodeList nl = m_DOMElement.getElementsByTagName(IPSTmxNode.NODENAMEMAP[IPSTmxNode.TMXSEGMENT]);
      ...
      Element newElemNote =
          (Element)
              PSXmlDocumentBuilder.copyTree(
                  m_DOMElement.getOwnerDocument(), m_DOMElement, note.getDOMElement(), true);
  ...
  public void addProperty(IPSTmxProperty property) {
    ...
    if (!m_Properties.containsKey(property.getType())) {
      Element newProperty =
          (Element)
              PSXmlDocumentBuilder.copyTree(
                  m_DOMElement.getOwnerDocument(), m_DOMElement, property.getDOMElement(), false);
      m_Properties.put(property.getType(), new PSTmxProperty(getTMXDocument(), newProperty));
    }
  }
  ...
  public void addSegment(IPSTmxSegment segment) {
    ...
    Element newSegment =
        (Element)
            PSXmlDocumentBuilder.copyTree(
                m_DOMElement.getOwnerDocument(), m_DOMElement, segment.getDOMElement(), false);
  ...
  private void removeSegment(IPSTmxSegment segment) {
    ...
    // as a whole.
    if (m_Segment.getValue().equals(segment.getValue())) m_Segment.setValue("");
  }
  ...
  public void merge(IPSTmxNode node) throws PSTmxDomException {
    if (!(node instanceof IPSTmxTranslationUnitVariant)) {
      throw new PSTmxDomException("onlyOneTypeAllowedToMerge", "IPSTmxTranslationUnitVariant");
    }
  ...
  private void mergeSegment(IPSTmxTranslationUnitVariant tuv) {
    ...
    PSTmxConfigParams options =
        getTMXDocument().getMergeConfig().getConfigParams(IPSTmxMergeConfig.MERGE_NODEID_SEGMENT);
    if (exists) {
      if (options
          .getParam(IPSTmxMergeConfig.MERGE_OPTION_DELETEIFEXISTS)
          .equalsIgnoreCase(IPSTmxMergeConfig.YES)) {
