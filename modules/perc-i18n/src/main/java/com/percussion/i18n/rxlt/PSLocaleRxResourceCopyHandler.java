...
public class PSLocaleRxResourceCopyHandler extends PSIdleDotter {
  ...
  public PSLocaleRxResourceCopyHandler(String rxroot, String languageString) {
    if (rxroot == null || rxroot.trim().length() < 1) {
      throw new IllegalArgumentException("Rhythmyx root directory must not be empty");
    }
    if (languageString == null || languageString.trim().length() < 1) {
      throw new IllegalArgumentException(
          "Language string must not be empty for making " + "copies of Rhythmyx resources");
    }
    m_rxroot = rxroot;
  ...
  public Iterator<File> getResourceFiles() {
    ...
    if (jsFile.exists()) files.add(jsFile);
  ...
  private File getJSFile(String languageString, boolean isSystem) {
    ...
    return new File(
        m_rxroot + File.separator + resourceFolder + File.separator + languageString,
  ...
  private File getImageFileDir(String languageString, boolean isSystem) {
    ...
    return new File(m_rxroot + File.separator + resourceFolder, languageString);
  }
  ...
  private void makeJSCopy() throws IOException {
    PSCommandLineProcessor.logMessage("copyingJavaScriptResourcesFor", m_languagestring);
    ...
    if (!srcFile.exists()) {
      PSCommandLineProcessor.logMessage("resourceFileNotExist", srcFile.getCanonicalPath());
      PSCommandLineProcessor.logMessage("resourceFileNotbeCreated", tgtFile.getCanonicalPath());
      return;
    }
    ...
    if (tgtFile.getParentFile() != null) tgtFile.getParentFile().mkdirs();
  ...
  private void makeCSSCopy() throws IOException {
    PSCommandLineProcessor.logMessage("copyingCSSResourcesFor", m_languagestring);
  ...
  private void makeXMLCopy() throws IOException {
    PSCommandLineProcessor.logMessage("copyingXMLResourcesFor", m_languagestring);
  ...
  private void makeImagesCopy() throws IOException {
    ...
    for (int i = 0; listFiles != null && i < listFiles.size(); i++) {
      ...
      try {
        ...
        PSCommandLineProcessor.logMessage("errorMessageException", e.getMessage());
      }
    }
  }
