...
/**
 * This interface is implemented by every action handler. User chooses one or more actions to
 * execute in the UI. Every action will have an entry in the configuration file to hold the option
 * parameters to process the action. The actionid in this interface will match with that in the
 * config file.
 */
public interface IPSActionHandler {
   ...
   *
   * @param cfgdata DOM element with configuration data for the current action. Must not be <code>
   *     null</code>
   * @throws PSActionProcessingException if any error occurs during processing
   */
  void process(Element cfgdata) throws PSActionProcessingException;

  /**
   * Action id to generate the TMX resourecs bundle from Rhythmyx Content Manager. This bundle
