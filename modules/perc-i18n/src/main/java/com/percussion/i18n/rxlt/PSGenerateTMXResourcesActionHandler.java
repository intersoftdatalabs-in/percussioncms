...
public class PSGenerateTMXResourcesActionHandler implements IPSActionHandler {
  ...
  public void process(Element cfgdata) throws PSActionProcessingException {
    ...
    PSCommandLineProcessor.logMessage(
        "processingAction", cfgdata.getAttribute(PSRxltConfigUtils.ATTR_NAME));
    ...
    try {
      ...
      try (InputStream is = getClass().getResourceAsStream(SECTION_MERGE_CONFIG_FILE)) {
        tmxDocSectionMaster.setMergeConfigDoc(PSXmlDocumentBuilder.createXmlDocument(is, false));
        //
      ...
      if (keepmissingonly.equalsIgnoreCase("yes")) {
        ...
        try (InputStream is =
            getClass().getResourceAsStream(MASTER_MERGE_CONFIG_FILE_MISSING_ONLY)) {
          mergeDoc = PSXmlDocumentBuilder.createXmlDocument(is, false);
        }
      } else {
        try (InputStream is = getClass().getResourceAsStream(MASTER_MERGE_CONFIG_FILE)) {
          mergeDoc = PSXmlDocumentBuilder.createXmlDocument(is, false);
        }
      }
      ...
      header.setProperty(IPSTmxHeader.PROP_CREATION_TOOL, PSRxltMain.PROGRAM_NAME);
      header.setProperty(
          IPSTmxHeader.PROP_CREATION_TOOL_VERSION, PSRxltMain.getVersionNumberString());
      ...
      PSCommandLineProcessor.logMessage("savingGeneratedTMXFile", file.getCanonicalPath());
