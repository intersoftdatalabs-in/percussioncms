 ...
 * built by the TableFactory classes. Thse list of tables is first read from the stylesheet. Table
 * factory classes use this list of tables to generate a single XML document for all the CMS tables
...
public class PSCmsTablesSectionHandler extends PSIdleDotter implements IPSSectionHandler {
   ...
   *
   * @throws PSSectionProcessingException in case it cannot parse the XSL file. This should not
   *     happen since the stylesheet file is packaged into the JAR file as we shipped.
   */
  public PSCmsTablesSectionHandler() throws PSSectionProcessingException {
    // Load the XSL stylesheet only once.
    if (ms_XslDoc != null) return;

    // This block is executed only once (like a static one).

    try (InputStream is = getClass().getResourceAsStream(CMSTABLEDATA_TRANSFORM_XSL)) {
      ...
      for (int i = 0; nl != null && i < nl.getLength(); i++) {
        ...
        if (table.trim().length() > 0) ms_Tables.add(table);
      }
    }
  ...
  public IPSTmxDocument process(Element cfgData) throws PSActionProcessingException {
    ...
    try {
      ...
      for (int i = 0; nl != null && i < nl.getLength(); i++) {
        ...
        desc = "";
        if (node instanceof Text) desc = ((Text) node).getData();
        tu = tmxDoc.createTranslationUnit(name, desc);
        tu.addProperty(
            tmxDoc.createProperty(
                IPSTmxDtdConstants.ATTR_VAL_SECTIONNAME, PSI18nUtils.DEFAULT_LANG, ms_SectionName));
  ...
  /**
   * Method that is specific to extract the configuration related keys from the table
   * PSX_RXCONFIGURATIONS. This table has the the configuration document (XML clob) as one of the
   * columns. This method extracts that parses into an XML document expands that table row element
   * with the root element of the config document. The table processing XSL has a template that
   * processes this table to extract the config related key names.
   *
   * @param dataDoc assumed not <code>null</code> and in the format generated by {@link
   *     #getTableDataDoc(Connection, PSJdbcDbmsDef, PSJdbcDataTypeMap)}
   */
  private void processRxConfigSets(Document dataDoc) {
    ...
    if (elemConfigTable == null) return; // no such table, do nothing
    ...
    for (int i = 0; i < count; i++) {
      ...
      for (int ii = 0; ii < nlCols.getLength(); ii++) {
        ...
        if (attr.equals("NAME")) name = ((Text) elem.getFirstChild()).getData();
        else if (attr.equals("TYPE")) type = ((Text) elem.getFirstChild()).getData();
        else if (attr.equals("CONFIGURATION")) elemCol = elem;
      }
      if (!type.equals("XML")) continue;
      PSCommandLineProcessor.logMessage("processingSection", name);
      if (elemCol == null) continue;
      else {
        try {
          String configData = ((Text) elemCol.getFirstChild()).getData();
          Document doc =
              PSXmlDocumentBuilder.createXmlDocument(new StringReader(configData), false);
          Element root = doc.getDocumentElement();
          root.setAttribute("name", name);
          // replace the row element with the root element of the config doc
          elemConfigRow
              .getParentNode()
              .replaceChild(elemConfigRow.getOwnerDocument().importNode(root, true), elemConfigRow);
        } catch (Exception e) {
   ...
   * @param dbmsDef Used to connect to the database and provides correct schema/origin, must not be
   *     <code>null</code>.
   * @param dataTypeMap The dataType map to use for this table's columns, must not be <code>null
   *     </code>
   * @return dataDoc table data document never <code>null</code>
   * @throws PSJdbcTableFactoryException if data could not be extracted for some reason.
   */
  private Document getTableDataDoc(
    ...
    for (int i = 0; i < ms_Tables.size(); i++) {
      PSCommandLineProcessor.logMessage("processingTable", ms_Tables.get(i).toString());

      tableSchema =
          PSJdbcTableFactory.catalogTable(
              conn, dbmsDef, dataTypeMap, ms_Tables.get(i).toString(), true);
    ...
    schemaDoc.appendChild(schemaDoc.importNode(collSchema.toXml(schemaDoc), true));
    dataDoc.appendChild(dataDoc.importNode(collData.toXml(dataDoc), true));
    return dataDoc;
  }

  /**
   * Helper method to transform and XML document to another XML Document using the given stylesheet
   * document.
   *
  ...
  public static Document transformXML(Document srcDoc, Document xslDoc)
    ...
    templates.newTransformer().transform(new DOMSource(srcDoc), new DOMResult(outNode));

    return outNode;
  }

  /**
   * Name of the stylesheet file that is used to transform the table fatcory generated XML doc to
   * another XML doc that has just keyset. The DTD for this document produces and XML document of
   * the form: &lt;keys&gt; &lt;key name='psx.workflow.1.3@Draft'&gt;Name of the first state in the
   * workflow&lt;/key&gt; &lt;key name='psx.keyword.101.3@Business'&gt;Name of the keyword for field
   * categories used in content editor&lt;/key&gt; ... &lt;/keys&gt;
   */
  ...
  /**
   * DOM Document for the XSL stylesheet that is run to produce required keys of the XML document
