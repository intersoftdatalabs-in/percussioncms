...
/**
 * This is the interface every node of the TMX Document must implement. Defines common methods to
 * all TMX nodes. Also defines the Node type constants and Node name constants.
 */
public interface IPSTmxNode {
  /**
   * Merge the supplied TMX node with this applying the merge rules for this particular type of
  ...
  /**
   * String representation of the TMX node which is the same as the string representation of the
   * underlying XML DOM node.
   *
   * @return XML string, never <code>null</code>
   * @throws PSTmxDomException if string conversion fails for some reason.
   */
  String toString() throws PSTmxDomException;

  /**
   * Method to return the TMX Document interface object this node is associated.
   *
   ...
   *
   * @return parent TMX node, never <code>null</code> except when the node is {@link
   *     IPSTmxDocument}.
   */
  IPSTmxNode getParent();

  /**
   * Access method for the XML DOM element this TMX wraps. Never modify the DOM element directly
   * which can leave the TMX node in a vague state. Use only TMX nodes to make any changes.
   *
  ...
  /**
   * Array of string representing the element names for each node type. These correspond to the node
   * type values above.
   *
