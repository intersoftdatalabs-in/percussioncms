...
public class PSTmxProperty extends PSTmxLeafNode implements IPSTmxProperty {
  ...
  PSTmxProperty(IPSTmxDocument tmxdoc, Element prop) {
    ...
    if (prop == null) throw new IllegalArgumentException("prop must not be null");
    ...
    if (node instanceof Text) m_Value = ((Text) node).getData();
  }
  ...
  public void merge(IPSTmxNode node) throws PSTmxDomException {
    if (node == null) {
      ...
      throw new PSTmxDomException("onlyOneTypeAllowedToMerge", "IPSTmxProperty");
    }
    ...
    PSTmxConfigParams options = getTMXDocument().getMergeConfig().getConfigParams(configType);
