...
/**
 * This interface defines specific methods to access and modify the translation unit of the TMX
 * document. As the name indicates transition variant is one unit of translation that has variants
 * for different languages. Translation Variant has a unique id to locate it in the TMX document.
 * Details can be seen in the TMX 1.4 DTD at:
 *
 * <p><a href="http://www.lisa.org/tmx/">Localisation Industry Standards Association</a>
 */
public interface IPSTmxTranslationUnit extends IPSTmxNode {
  /**
   * Method to get the translation unit key (attribute 'tuid' of &lt;tu&gt; element). This is the
   * uniques identifier for a translation unit.
   *
  ...
  /**
   * Method to get all note objects that are associated with the translation unit. Typically, there
   * will be at least one note in default language and optionally one each for other langauge(s).
   *
   ...
   *
   * @param lang a specific language name, must never be <code>null</code> or empty
   * @return a variant, or <code>null</code> if there is no variant for the given language.
   */
   ...
   *
   * @param tuv translation unit variant to be added to the tree, must not be <code>null</code>.
   * @param overwrite set to <code>true</code> in order to overwrite an existing translation unit
   *     variant if found, <code>false</code> otherwise.
   */
   ...
   * @param overwrite set to <code>true</code> in order to overwrite an existing note if found,
   ...
   *
   * @param prop tmx property object, must not be <code>null</code> or <code>empty</code>.
   */
