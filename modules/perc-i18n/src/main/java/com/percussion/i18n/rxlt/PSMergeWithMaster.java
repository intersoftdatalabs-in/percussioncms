...
public class PSMergeWithMaster extends PSIdleDotter implements IPSActionHandler {
  ...
  public PSMergeWithMaster() throws PSActionProcessingException {
    try {
      try (InputStream ir = getClass().getResourceAsStream(MERGE_CONFIG_FILE)) {
        m_DocConifigMergeWithMaster = PSXmlDocumentBuilder.createXmlDocument(ir, false);
      }
  ...
  public void process(Element cfgdata) throws PSActionProcessingException {
    ...
    try {
      ...
      if (file.exists()) {
        try (FileInputStream fis = new FileInputStream(file)) {
          try (InputStreamReader ir = new InputStreamReader(fis, StandardCharsets.UTF_8)) {
            Document srcDoc = PSXmlDocumentBuilder.createXmlDocument(ir, false);
      ...
      destTmxdoc.save(new File(rxroot, PSTmxResourceBundle.MASTER_RESOURCE_FILEPATH), true);
