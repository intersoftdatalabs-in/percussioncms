...
public class PSRxltMain {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; args != null && i < args.length; i++) {
      ...
      if (temp.toUpperCase().startsWith("-NOUI")) {
        ...
        log.info("Usage: java com.percussion.i18n.rxlt.PXRXLTMain" + " [options]");
        log.info("Options:");
        log.info("-noui           No user interaction. Run the " + "tool with default settings");
        log.info(
            "-R<rxroot>      Use the supplied Rhythmyx "
  ...
  static {
    try {
      ResourceBundle rb =
          ResourceBundle.getBundle("com.percussion.i18n.rxlt.Version", Locale.getDefault());

      ms_Version =
          rb.getString("majorVersion")
              + "."
  ...
  private static void initLog4J(File rxRoot) {
    ...
    try {
      if (!rxRoot.isDirectory()) rxRoot = new File("."); // default to current dir
    } catch (Exception e) {
      log.severe(PSExceptionUtils.getMessageForLog(e));
    }
  }
