    ...
    try {
      ...
      for (int i = 0; nl != null && i < nl.getLength(); i++) {
        ...
        if ((mnemonic != null && mnemonic.trim().length() > 0)
          ...
          IPSTmxTranslationUnitVariant variant = tu.getTransUnitVariant("en-us");
          if (variant != null) {
            if (mnemonic != null && mnemonic.trim().length() > 0) {
              variant.addProperty(tmxDoc.createProperty("mnemonic", "en-us", mnemonic));
            }
            if (tooltip != null && tooltip.trim().length() > 0) {
              variant.addProperty(tmxDoc.createProperty("tooltip", "en-us", tooltip));
            }
          }
        }
        tmxDoc.merge(tu);
