...
public class PSContentEditorsSectionHandler extends PSIdleDotter implements IPSSectionHandler {
  ...
  public PSContentEditorsSectionHandler() throws PSSectionProcessingException {
    ...
    try (InputStream is = getClass().getResourceAsStream(CONTENTEDITORS_TRANSFORM_XSL)) {
  ...
  private void handleSystemDef(String rxroot, Document keysDoc)
    ...
    try {
      Document doc = PSXmlDocumentBuilder.createXmlDocument(new FileReader(file), false);
      doc = getKeys(doc);
      PSXmlDocumentBuilder.copyTree(
          keysDoc, keysDoc.getDocumentElement(), doc.getDocumentElement(), false);
    } catch (Exception e) {
      PSCommandLineProcessor.logMessage("warningMessageException", e.getMessage());
    }
  }
