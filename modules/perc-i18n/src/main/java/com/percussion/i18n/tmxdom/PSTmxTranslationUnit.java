...
/**
 * This class wraps the functionality of translation unit DOM element as an easy to use TMX node.
 * The TMX counterpart provides methods to manipulate the translation unit, the most important one
 * being to merge two nodes appying the merge configuration.
 */
public class PSTmxTranslationUnit extends PSTmxNode implements IPSTmxTranslationUnit {
  /**
   * Convenience version that calls {@link #PSTmxTranslationUnit(IPSTmxDocument, Element, boolean}
   ...
   * translation unit. All the components of the translation unit variant are optionally constructed
   * from the given DOM element.
   *
   * @param tmxdoc parent TMX document, nust not be <code>null</code>.
   * @param tu DOM element for the translation unit to be contstructed, must not be <code>null
   *     </code>.
   * @param createDefault If <code>true</code>, a variant is also added for the default language if
   *     it does not already exist. If <code>false</code>, no default is added.
   * @throws IllegalArgumentException if tmxDoc or tu is <code>null</code>
   */
  PSTmxTranslationUnit(IPSTmxDocument tmxdoc, Element tu, boolean createDefault) {
    if (tmxdoc == null) throw new IllegalArgumentException("tmxdoc must not be null");
    if (tu == null) throw new IllegalArgumentException("tu must not be null");

    m_DOMElement = tu;
    m_PSTmxDocument = tmxdoc;

    NodeList nl =
        m_DOMElement.getElementsByTagName(
            IPSTmxNode.NODENAMEMAP[IPSTmxNode.TMXTRANSLATIONUNITVARIANT]);
    for (int i = 0; nl != null && i < nl.getLength(); i++) {
      PSTmxTranslationUnitVariant tuv =
          new PSTmxTranslationUnitVariant(m_PSTmxDocument, (Element) nl.item(i));
      m_TransUnitVariants.put(tuv.getLang(), tuv);
    }

    nl = m_DOMElement.getElementsByTagName(IPSTmxNode.NODENAMEMAP[IPSTmxNode.TMXNOTE]);
    for (int i = 0; nl != null && i < nl.getLength(); i++) {
      PSTmxNote note = new PSTmxNote(m_PSTmxDocument, (Element) nl.item(i));
      m_Notes.put(note.getLang(), note);
    }

    nl = m_DOMElement.getElementsByTagName(IPSTmxNode.NODENAMEMAP[IPSTmxNode.TMXPROPERTY]);
    for (int i = 0; nl != null && i < nl.getLength(); i++) {
      PSTmxProperty prop = new PSTmxProperty(m_PSTmxDocument, (Element) nl.item(i));
    ...
    if (createDefault) {
      ...
      if (variant == null) {
        variant =
            getTMXDocument()
                .createTranslationUnitVariant(
                    PSI18nUtils.DEFAULT_LANG, PSI18nUtils.getLastSubKey(getKey()));
        addTuv(variant, false);
      } else {
        variant.addSegment(getTMXDocument().createSegment(PSI18nUtils.getLastSubKey(getKey())));
      }
    }
  }
   ...
   *
   * @param tu translation unit variant object to be merged. Must not be <code>null</code>.
  ...
  private void mergeNotes(IPSTmxTranslationUnit tu) {
    ...
    while (iter.hasNext()) {
      ...
      if (exists) {
        if (options
            .getParam(IPSTmxMergeConfig.MERGE_OPTION_DELETEIFEXISTS)
            .equalsIgnoreCase(IPSTmxMergeConfig.YES)) {
          removeNote(note);
        } else if (options
            .getParam(IPSTmxMergeConfig.MERGE_OPTION_REPLACEIFEXISTS)
            .equalsIgnoreCase(IPSTmxMergeConfig.YES)) {
          removeNote(note);
          addNote(noteSrc, false);
        } else if (options
            .getParam(IPSTmxMergeConfig.MERGE_OPTION_MODIFYIFEXISTS)
            .equalsIgnoreCase(IPSTmxMergeConfig.YES)) {
          note.merge(noteSrc);
        }
      } else {
        if (options
            .getParam(IPSTmxMergeConfig.MERGE_OPTION_ADDIFNOTEXISTS)
            .equalsIgnoreCase(IPSTmxMergeConfig.YES)) {
          addNote(noteSrc, false);
        }
      }
  ...
  /**
   * Helper method to merge the TMX property object from the supplied TMX translation unit object
   * with that of the current TMX node applying the merge rules. Uniqueness of the property object
   * is established based on the property type values.
   *
  ...
  /**
   * Helper method to merge the TMX translation unit variant object from the supplied TMX
   * translation unit object with that of the current TMX node applying the merge rules. Uniqueness
   * of the property object is established based on language value associated with node.
   *
   * @param tu translation unit object to be merged. Must not be <code>null</code>.
   * @throws IllegalArgumentException if tu is <code>null</code>
   */
  ...
  /**
   * Helper method to remove the translation unit variant node matching with supplied TMX Node.
   *
   * @param tuv TMX translation unit variant to be removed from this node. Nothing happens if <code>
   *     null</code> is supplied or matching node does not exist in the current node. Two
   *     translation unit variants are assumed to be matching if the language associated with the
   *     nodes are the same.
   */
   ...
   *     matching if the language associated with the notes are the same.
   */
   ...
   *     to be matching if the type associated with the properties are the same.
   */
  ...
  public void addTuv(IPSTmxTranslationUnitVariant tuv, boolean overwrite) {
    ...
    if (!containsKey || (containsKey && overwrite)) {
      if (containsKey) removeTuv(tuv);

      Element newElemTuv =
          (Element)
              PSXmlDocumentBuilder.copyTree(
                  m_DOMElement.getOwnerDocument(), m_DOMElement, tuv.getDOMElement(), false);
      m_TransUnitVariants.put(
          tuv.getLang(), new PSTmxTranslationUnitVariant(getTMXDocument(), newElemTuv));
    }
  }
  ...
  public void addNote(IPSTmxNote note, boolean overwrite) {
    ...
    if (!containsKey || (containsKey && overwrite)) {
      if (containsKey) removeNote(note);

      Element newElemNote =
          (Element)
              PSXmlDocumentBuilder.copyTree(
                  m_DOMElement.getOwnerDocument(), m_DOMElement, note.getDOMElement(), false);
      m_Notes.put(note.getLang(), new PSTmxNote(getTMXDocument(), newElemNote));
    }
  }
  ...
  public void addProperty(IPSTmxProperty prop) {
    ...
    if (!m_Properties.containsKey(prop.getType())) {
      Element newProp =
          (Element)
              PSXmlDocumentBuilder.copyTree(
                  m_DOMElement.getOwnerDocument(), m_DOMElement, prop.getDOMElement(), false);
      m_Properties.put(prop.getType(), new PSTmxProperty(getTMXDocument(), newProp));
    }
  }
