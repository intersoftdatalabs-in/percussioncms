...
public class PSTmxHeader extends PSTmxNode implements IPSTmxHeader {
  ...
  PSTmxHeader(IPSTmxDocument TMXDoc) {
    ...
    if (null == elem) {
      ...
      PSXmlDocumentBuilder.copyTree(
          m_PSTmxDocument.getDOMDocument(),
  ...
  private void processHeaderElement() {
    ...
    setProperty(IPSTmxHeader.PROP_DATA_TYPE, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_DATA_TYPE);
    setProperty(IPSTmxHeader.PROP_ADMIN_LANG, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_ADMIN_LANG);
    setProperty(
        IPSTmxHeader.PROP_CREATION_TOOL, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_CREATION_TOOL);
    setProperty(
        IPSTmxHeader.PROP_CREATION_TOOL_VERSION,
        IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_CREATION_TOOL_VERSION);
    setProperty(IPSTmxHeader.PROP_O_TMF, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_O_TMF);
    setProperty(IPSTmxHeader.PROP_SEG_TYPE, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_SEG_TYPE);
    setProperty(IPSTmxHeader.PROP_SRC_LANG, IPSTmxDtdConstants.DEFAULT_VALUE_ATTR_SRC_LANG);
  }
  ...
  public void addLanguage(String language) {
    ...
    if (m_Languages.get(language) != null) return;

    Element prop = getTMXDocument().getDOMDocument().createElement(IPSTmxDtdConstants.ELEM_PROP);
    prop.setAttribute(IPSTmxDtdConstants.ATTR_TYPE, IPSTmxDtdConstants.ATTR_VAL_SUPPORTEDLANGUAGE);
    ...
    PSXmlDocumentBuilder.copyTree(m_DOMElement.getOwnerDocument(), m_DOMElement, prop, false);
   ...
   * implementation of the method defined in the interface
   */
  public void setProperty(int type, String value) throws PSTmxDomException {
    if (value == null) value = "";
