  ...
  public void execute() {
    ...
    try {
      String loginConfigPath =
          getRootDir() + File.separator + PSConfigureDatasource.getLoginConfigLocation();
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (policyName.equals(JMX_CONSOLE) || policyName.equals(WEB_CONSOLE)) {
          ...
          for (int idx = 0; idx < authNodes.getLength(); idx++) {
            ...
            NodeList loginModNodes = authNode.getElementsByTagName(LOGIN_MODULE);
            ...
            for (int idx2 = 0; idx2 < loginModNodes.getLength(); idx2++) {
              ...
              if (!className.equals(LOGIN_MODULE_CLASS)) continue;

              // Get the module options
              NodeList modOptNodes = loginModNode.getElementsByTagName(MODULE_OPTION);
              ...
              for (int idx3 = 0; idx3 < modOptNodes.getLength(); idx3++) {
                ...
                // necessary
                if (modOptName.equals(ROLES_PROPS) || modOptName.equals(USERS_PROPS)) {
                  String modOptVal = modOptNode.getTextContent();
                  if (!modOptVal.startsWith(PATH_PREFIX))
                    modOptNode.setTextContent(PATH_PREFIX + modOptVal);
                }
              }
            }
          }
