...
public class PSJunitFileSelector extends BaseExtendSelector {
  ...
  public boolean isSelected(File baseDir, String filename, File file) {
    ...
    if (extPos != -1) filename = filename.substring(0, extPos);
    ...
    try {
      ...
      if (pkg != null) pkgName = pkg.getName();

      // get class name without package
      if (pkg != null) className = className.substring(pkgName.length() + 1);

      selected =
          (isPkgIncluded(pkgName)
              && !isPkgExcluded(pkgName)
              && isClassIncluded(className, theClass)
              && !isClassExcluded(className, theClass));
    } catch (NoClassDefFoundError e) {
      setError(
          "Failed to load test class: "
              + className
              + ", filename: "
              + filename
              + ", error: "
  ...
  private boolean isClassExcluded(String className, Class<?> theClass) {
    ...
    if (!isExcluded) isExcluded = hasMatch(theClass, m_classImplExcludes.iterator(), false);
  ...
  public void setPackageFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_packageIncludes.add(filter);
    else m_packageExcludes.add(filter);
  }
  ...
  public void setClassNameFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_classNameIncludes.add(filter);
    else m_classNameExcludes.add(filter);
  }
  ...
  public void setClassImplFilter(String filter, boolean isInclude) {
    ...
    if (isInclude) m_classImplIncludes.add(filter);
    else m_classImplExcludes.add(filter);
  }
    ...
    if (parameters != null) {
      ...
      for (int i = 0; i < parameters.length; i++) {
        ...
        if (TYPE_PATH.equalsIgnoreCase(type)) {
          if (parameters[i].getName().equalsIgnoreCase(FILTER_CLASS_PATH_ATTR)) {
            classPath = parameters[i].getValue();
            continue;
          } else {
            setError("Unsupported param name (" + parameters[i].getName() + ") for type: " + type);
          }
      ...
      if (classPath != null && classPath.trim().length() > 0) {
        if (ms_classPath == null || !ms_classPath.equals(classPath)) {
          ...
          // not.
          ms_classLoader = new URLClassLoader(urls, null);
        }
      } else {
        ms_classPath = null;
        ms_classLoader = null;
  ...
  private boolean isJUnit4(Class<?> theClass) {
    ...
    for (Method m : methods) {
      ...
      for (Annotation a : annotations) {
        if (ms_testAnnotation.isAssignableFrom(a.getClass())) return true;
      }
    }
