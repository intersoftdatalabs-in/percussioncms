...
public class PSExtensions extends PSAction {
  ...
  public void execute() {
    ...
    if (m_strRootDir == null) {
      PSLogger.logInfo("ERROR : " + RxInstallerProperties.getResources().getString("rooterr"));
      return;
    }
  ...
  private void findAndConfigureExits(File dir) {
    ...
    File extensions =
        new File(dir.getPath() + File.separator + IPSExtensionHandler.DEFAULT_CONFIG_FILENAME);
    ...
    if (children != null) {
      for (String child : children) {
        ...
        if (strChild != null) {
          ...
          if (childFile.isDirectory()) findAndConfigureExits(childFile);
        }
      }
    }
  }
