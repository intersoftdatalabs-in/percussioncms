...
public class PSUpdatePropFileWizAction extends PSAction {
  ...
  public void execute() {
    ...
    try {
      ...
      if (fis == null) {
        PSLogger.logInfo("Failed to open input stream for file : " + propFile);
        return;
      }
      p.load(fis);
      if (onlyIfExists) {
        ...
        if (value == null) {
          PSLogger.logInfo(
              "Property does not exist : " + propName + " onlyIfExists = true, not updating.");
          return;
        }
      }
      ...
      if (fos == null) {
        PSLogger.logInfo("Failed to open output stream for file : " + propFile);
        return;
      }
      p.store(fos, null);
      return;
  ...
  public void setPropertyName(String propertyName) {
    if ((propertyName == null) || (propertyName.trim().length() < 1))
      throw new IllegalArgumentException("propertyName may not be null or empty");
