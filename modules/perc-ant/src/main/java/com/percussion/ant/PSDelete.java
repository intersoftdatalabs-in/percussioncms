...
public class PSDelete extends Delete {
  ...
  public void execute() throws BuildException {
        ...
        if (file.isDirectory()) {
          ...
          if (!file.delete()) {
            ...
            if (m_failonerror) {
              ...
              log(message, m_quiet ? Project.MSG_VERBOSE : Project.MSG_WARN);
            }
          }
        }
      } else {
    ...
    if (dir != null && dir.exists() && dir.isDirectory() && !usedMatchingTask) {
      boolean isUsingMapping = (m_mappedDir != null && m_mappedDir.trim().length() > 0);
    ...
    for (int i = 0; i < filesets.size(); i++) {
      ...
      try {
        ...
        if (m_failonerror) {
          ...
          log(be.getMessage(), m_quiet ? Project.MSG_VERBOSE : Project.MSG_WARN);
        }
      }
    }
  ...
  private File mutateFile(File fileToMutate) {
    if (fileToMutate == null) {
      throw new BuildException("Illegal Argument: File cannot be null in mutate file method.");
    }

    if (dir != null && m_mappedDir != null && m_mappedDir.trim().length() > 0) {
      String f = fileToMutate.getAbsolutePath();

      if (f.startsWith(dir.getAbsolutePath())) {

        fileToMutate = new File(m_mappedDir + f.substring(dir.getAbsolutePath().length()));
      } else {
  ...
  private List<File> getAllValidPackageDirs(File srcDir) {
    ...
    if (srcDir.isDirectory()) {
      ...
      for (int i = 0; i < children.length; i++) {
        if (children[i].isDirectory()) dirs.addAll(getAllValidPackageDirs(children[i]));
      }
      if (containsFiles(srcDir)) dirs.add(mutateFile(srcDir));
    }
    return dirs;
  }
