...
public class PSUpgradeServerPageTags extends PSAction {
  ...
  public void execute() {
    ...
    try {
      ...
      if (!diskXmlFile.exists()) {
        PSLogger.logInfo(
            "Server Page Tags Xml file does not exist : " + diskXmlFile.getAbsolutePath());
        return;
      } else {
      ...
      try {
        ...
        Document resDoc = PSXmlDocumentBuilder.createXmlDocument(resStream, false);
        ...
        for (int i = 0; i < resNl.getLength(); i++) {
          ...
          if (isXslTagAttrVal != null) {
            ...
            if (isXslTagAttrVal.equalsIgnoreCase("yes")) processTagNodes.add(resEl);
          }
        }

        if (processTagNodes.size() > 0) {
          diskStream = new FileInputStream(diskXmlFile);
          Document diskDoc = PSXmlDocumentBuilder.createXmlDocument(diskStream, false);
          ...
          for (int i = 0; i < processTagNodes.size(); i++) {
            ...
            if ((resOpening == null) || (resClosing == null)) continue;
            ...
            for (int j = 0; j < diskNl.getLength(); j++) {
              ...
              if ((diskOpening == null) || (diskClosing == null)) continue;
              ...
              if ((diskOpening.equalsIgnoreCase(resOpening))
                ...
                if (xslTagAttrVal == null) xslTagAttrVal = "no";
  ...
  public void setServerPageTagsXmlRxFilePath(String serverPageTagsXmlRxFilePath) {
    if ((serverPageTagsXmlRxFilePath == null) || (serverPageTagsXmlRxFilePath.trim().length() == 0))
      throw new IllegalArgumentException("serverPageTagsXmlRxFilePath may not be null or empty");
  ...
  public void setServerPageTagsXmlE2FilePath(String serverPageTagsXmlE2FilePath) {
    if ((serverPageTagsXmlE2FilePath == null) || (serverPageTagsXmlE2FilePath.trim().length() == 0))
      throw new IllegalArgumentException("serverPageTagsXmlE2FilePath may not be null or empty");
