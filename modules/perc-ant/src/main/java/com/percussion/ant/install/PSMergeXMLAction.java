...
public class PSMergeXMLAction extends PSAction {
  ...
  public void execute() {
    if (m_strSourceDoc != null
      ...
      if (strRootDir != null) {
        String strDestinationPath = strRootDir + File.separator + m_strDestinationDoc;

        strDestinationPath =
            Paths.get(strRootDir, strDestinationPath).normalize().toAbsolutePath().toString();
        ...
        if (destFile.exists()) {
          ...
          try {
            ...
            if (destXmlFile.exists()) destDoc = db.parse(destXmlFile);
            ...
            if (m_strParent != null) {
              for (int iParent = 0;
                  destElement != null && iParent < m_strParent.length;
                  ++iParent) {
                String strParent = m_strParent[iParent];
                NodeList parents = destElement.getElementsByTagName(strParent);
                if (parents != null
                    && parents.getLength() > 0
                    && parents.item(0) instanceof Element) destElement = (Element) parents.item(0);
              }
            }

            // get the copy elements from the source doc
            NodeList copyNodes = srcDoc.getElementsByTagName(m_strCopyElement);

            if (copyNodes != null && destElement != null) {
              for (int iCopyNode = 0; iCopyNode < copyNodes.getLength(); ++iCopyNode) {
                Node copyNode = copyNodes.item(iCopyNode);
                if (copyNode != null) {
                  // import the node into destDoc
                  Node importedNode = destDoc.importNode(copyNode, true);
                  // copy the node in
                  if (importedNode != null) destElement.appendChild(importedNode);
                }
              }
            }
