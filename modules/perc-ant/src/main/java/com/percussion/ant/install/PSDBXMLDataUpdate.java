...
public class PSDBXMLDataUpdate extends PSXMLFileUpdate {
  ...
  public void execute() {
    ...
    try {
      ...
      if (selectFilter.trim().length() > 0) filter = new PSJdbcSelectFilter(selectFilter);
      ...
      while (rowIt.hasNext()) {
        ...
        if (colData != null) {
          ...
          if (!((colValue == null) || (colValue.trim().length() < 1))) {
            ...
            PSJdbcColumnData newColData = updateColumnData(colValue, tempFile);
            row.removeColumn(columnName);
            row.addColumn(newColData);
            step = PSJdbcStatementFactory.getUpdateStatement(dbmsDef, tableSchema, row);
            plan.addStep(step);
          }
        }
      }
  ...
  private boolean backupTable(
    ...
    try {
      success =
          DbUtils.backupTable(conn, dbmsDef, dataTypeMap, tableName, backupTableName, ps, true);

      if (success) PSLogger.logInfo("Successfully created backup of table : " + tableName);
      else PSLogger.logInfo("Failed to create backup of table : " + tableName);
  ...
  public void setBackupTableName(String backupTableName) {
    if ((backupTableName == null) || (backupTableName.trim().length() < 1))
      throw new IllegalArgumentException("backupTableName may not be null or empty");
