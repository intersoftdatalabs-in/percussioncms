          ...
          if (m_operation.equals(INSTALL_OP)) {
            if (jarOptId.equals(rxOptId)) {
              ...
              throw new Exception(
                  "The current patch ("
                      + jarOptId
                      + ") is installed on the CM1 "
                      + "installation found at "
                      + m_root
                      + ".  This "
                      + "patch must be uninstalled before it can be "
                      + "installed again.");
            }
            ...
            if (!jarOptId.equals(rxOptId)) {
              ...
              throw new Exception(
                  "The current patch ("
                      + jarOptId
                      + ") does not match the patch currently "
                      + "installed to the CM1 installation found at "
                      + m_root
                      + " ("
                      + rxOptId
                      + ")");
    ...
    try {
      ...
      if (jarFile.exists()) {
        ...
        if (versionFile.exists()) {
          ...
          // directory install.
          versionStagingFile =
              new File(m_root + File.separator + DTS_STAGING + File.separator + VERSION_PROPERTIES);
  ...
  public void setOperation(String operation) {
    if (operation == null
      ...
      throw new IllegalArgumentException(
          "operation may not be null and " + "must be either install or uninstall");
    }
