...
public class PSTableViewExistsWizCondition extends PSAction implements Condition {
      ...
      try (FileInputStream in = new FileInputStream(propFile)) {
        ...
        try (Connection conn = InstallUtil.createConnection(driver, server, database, uid, pw)) {
          ...
          PSJdbcTableSchema objectSchema =
              PSJdbcTableFactory.catalogTable(conn, dbmsDef, dataTypeMap, objectName, false);

          exists = (objectSchema != null);
          if (exists) {
            // check if the object type matches
            if (isView && !objectSchema.isView()) exists = false;
            else if (!isView && objectSchema.isView()) exists = false;
          }
        }
      }
    } catch (Exception ex) {
  ...
  public void setObjectName(String aObjectName) {
    if ((aObjectName == null) || (aObjectName.trim().length() < 1))
      throw new IllegalArgumentException("aObjectName may not be null or empty");
