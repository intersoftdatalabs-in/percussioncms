...
public class PSUpdateWebApps extends PSAction {
  ...
  private void deployWebapps(File webappsDir, File deployDir) {
    ...
    for (i = 0; i < webapps.length; i++) {
      ...
      if (!webapp.isDirectory()) continue;
      ...
      if (!isSysWebapp(webappName)) {
        ...
        String deployAbsPath = deployPath + File.separator + webappName + ".war";
        File newWebapp = new File(deployAbsPath);
        if (!newWebapp.exists()) newWebapp.mkdir();
        ...
        try {
          PSLogger.logInfo("Adding classloader scoping to webapp: " + newWebapp.getName());
   ...
   */
  private void addClassloaderScoping(File webapp) throws IOException {
    String scopingFilePath =
        webapp.getAbsolutePath() + File.separator + "WEB-INF" + File.separator + scopingFile;
