...
public class PSXMLFileUpdate extends PSAction implements EntityResolver {
  ...
  public InputSource resolveEntity(String publicId, String systemId) {
    if ((systemId == null) || (systemId.trim().length() < 0)) return null;

    PSLogger.logInfo("Input System Id : " + systemId);
    String tempSystemId = systemId.toLowerCase();
    if (tempSystemId.startsWith("http:")) return null;
    ...
    try {
      ...
      if (index != -1) {
        ...
        index++;
        if (systemId.length() > index) systemId = systemId.substring(index);
      }
      ...
      if (f.exists() && f.isFile()) retFile = f;
      ...
      if (retFile != null) {
        PSLogger.logInfo("Resolved System Id : " + retFile.getAbsolutePath());
  ...
  public void execute() {
    ...
    if (!fSrcXmlFile.exists()) {
      PSLogger.logInfo("Xml file does not exist : " + fSrcXmlFile.getAbsolutePath());
      return;
    }
  ...
  private void applyXSL(String srcXmlFile, String destXmlFile, String xslResource, String rxDir)
    ...
    if (m_resolveXslFileContents) xslTempPath = resolveFile(xslTempPath);

    // Apply the XSL file to the xml file
    InputStream xslIn = new BufferedInputStream(new FileInputStream(xslTempPath));
    StreamSource xslSource = new StreamSource(xslIn);

    InputStream xmlIn = new BufferedInputStream(new FileInputStream(srcXmlFile));
  ...
  private void setTransformParams(String rxRootDir, Transformer transformer) {
    ...
    File verProp = new File(rxRootDir, RxFileManager.PREVIOUS_VERSION_PROPS_FILE);
    ...
    if (verProp.exists() && verProp.isFile()) {
      ...
      try {
        ...
        String strMajor = p.getProperty(RxFileManager.MAJOR_VERSION_PROP_KEY, "-1");

        String strMinor = p.getProperty(RxFileManager.MINOR_VERSION_PROP_KEY, "-1");

        String strBuild = p.getProperty(RxFileManager.BUILD_NUMBER_PROP_KEY, "-1");
  ...
  private String resolveFile(String file) throws Exception {
    ...
    try {
      ...
      while ((line = in.readLine()) != null) {
        out.write(line.replaceAll(getToken(), getValue().replaceAll("\\\\", "\\\\\\\\")));
        out.newLine();
      }
    } finally {
      if (out != null) {
  ...
  private String getDocType(String xmlFilePath) throws Exception {
    ...
    if (strDocType.length() < 1) strDocType = null;
    return strDocType;
  }
  ...
  private void setDocType(String xmlFilePath, String docType) throws Exception {
    if ((docType == null) || (docType.trim().length() < 1)) return;
