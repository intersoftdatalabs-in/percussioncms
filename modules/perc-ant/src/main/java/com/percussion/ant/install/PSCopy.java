...
public class PSCopy extends Copy {
  ...
  public void execute() {
    if (!PSAction.refreshFiles()) {
      if (getReplaceType().equalsIgnoreCase(ALWAYS)) setOverwrite(true);
    ...
    if (fileCopyMap.size() > 0) {
      ...
      while (e.hasMoreElements()) {
        ...
        for (int i = 0; i < toFiles.length; i++) {
          ...
          if (!(new File(toFile)).exists()) toFilesList.add(toFile);
        }

        toFiles = new String[toFilesList.size()];
        for (int j = 0; j < toFilesList.size(); j++) toFiles[j] = toFilesList.get(j);
  ...
  public void setReplaceType(String replaceType) {
    if (!replaceType.equalsIgnoreCase(ALWAYS)
      ...
      throw new IllegalArgumentException(
          "replaceType must be one of the " + " following : always, date, never");
    }
