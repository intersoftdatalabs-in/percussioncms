...
public class PSServerInstallEndPanel extends PSAction {
  ...
  public void execute() {
    ...
    if ((installDir == null) || (installDir.trim().length() == 0)) return;
    ...
    if (!propFile.exists()) {
      try {
        if (!propFile.createNewFile()) throw new IOException();
      } catch (IOException ioe) {
        PSLogger.logInfo("Failed to create installation.properties file");
        return;
      }
    }
  ...
  public void setPropertyFileName(String propertyFileName) {
    if ((propertyFileName == null) || (propertyFileName.trim().length() == 0))
      throw new IllegalArgumentException("propertyFileName may not be null or empty");
  ...
  public void setServiceDesc(String serviceDesc) {
    if ((serviceDesc == null) || (serviceDesc.trim().length() == 0))
      throw new IllegalArgumentException("serviceDesc may not be null or empty");
