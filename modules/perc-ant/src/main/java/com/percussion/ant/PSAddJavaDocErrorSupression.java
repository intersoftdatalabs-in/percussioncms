...
public class PSAddJavaDocErrorSupression extends Task {
  ...
  public void execute() {
    if (m_dir == null || !m_dir.isDirectory())
      throw new BuildException("Directory cannot be null and must" + " be a valid directory.");
    try {
      log.info(
          "Adding error suppression to: {}", new File(m_dir, INDEX_HTML_FILE).getAbsolutePath());
   ...
   */
  private StringBuilder getContents() throws IOException {
   ...
   */
  private void saveFile(final StringBuilder contents) throws IOException {
