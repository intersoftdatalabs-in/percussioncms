...
public class PSGetReleaseDocumentsPath extends Task {
  ...
  public void execute() throws BuildException {
    ...
    if (!isNumeric(m_major) || m_major.length() != 1)
      throw new BuildException("'major' must be a number with a length of one digit.");
    if (!isNumeric(m_minor) || m_minor.length() != 1)
      throw new BuildException("'minor' must be a number with a length of one digit.");
    if (!isNumeric(m_micro) || m_micro.length() != 1)
      throw new BuildException("'micro' must be a number with a length of one digit.");
    File rootDir = new File(m_rootDir);
    if (!rootDir.exists() || !rootDir.isDirectory())
      throw new BuildException(
          "'rootDir' directory does not exist or there is " + "a network problem.");
