...
public class PSCompareFiles extends Task {
  ...
  public void execute() throws BuildException {
    ...
    for (File dir1File : dir1Files) {
      ...
      for (File dir2File : dir2Files) {
        if (dir2File.getName().equals(dir1FileName)) {
          if (dir2File.length() != dir1File.length()) modFileNames.add(dir1FileName);
    ...
    String errMsg =
        "The following files found in both "
            + m_dir1.getAbsolutePath()
            + " and "
            + m_dir2.getAbsolutePath()
            + " "
    ...
    for (String fileName : modFileNames) {
      ...
      if (i < modFileNames.size() - 1) errMsg += ", ";
