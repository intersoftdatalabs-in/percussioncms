...
public class PSUpgradeRelationshipConfig extends PSAction {
    ...
    try {
      ...
      try (Connection conn = RxLogTables.createConnection(props)) {
        ...
        if (conn == null) {
          ...
          PSLogger.logError("PSUpgradeRelationshipConfig#execute : Table modifications aborted");
  ...
  private void updateRelationshipTypeConfig(final Connection conn, final PSJdbcDbmsDef dbmsDef) {

    PSLogger.logInfo("Adding missing Relationship configs ");

    String qualTableName =
        PSSqlHelper.qualifyTableName(
            CONFIG_TABLE, dbmsDef.getDataBase(), dbmsDef.getSchema(), dbmsDef.getDriver());

    String configXml = null;

    String configXmlSql =
        "SELECT CONFIGURATION FROM " + qualTableName + " WHERE NAME='relationships'";
    ...
    try (PreparedStatement stmt = conn.prepareStatement(configXmlSql)) {
      ...
      PSLogger.logError(
          "PSUpgradeRelationshipConfig : Failed ERROR: " + PSExceptionUtils.getMessageForLog(e));
    }
    ...
    if (!configXml.contains(PSRelationshipConfig.TYPE_WIDGET_CONTENT)) {
      configXml =
          configXml.replace(
              "</PSXRelationshipConfigSet>",
              getWidgetContentConfig() + "\n</PSXRelationshipConfigSet>");
      changed = true;
    }

    if (changed) {

      String updateConfigXML =
          "UPDATE " + qualTableName + " SET CONFIGURATION=? WHERE NAME='relationships'";
      ...
      try {
        ...
        PSLogger.logError(
            "PSUpgradeRelationshipConfig : Update Failed ERROR: "
  ...
  private String getRecycleContentConfig() {
    return "<PSXRelationshipConfig category=\"rs_recycled\" id=\"8\" label=\"Recycled Content\""
        ...
        + "        <Description>Specifies if this relationship must be packaged and deployed"
        + " with the owner. If\n"
        + "                checked, the relationship must be deployed, if not, it is"
        + " optional.</Description>\n"
        + "      </PSXProperty>\n"
        + "      <PSXProperty locked=\"yes\" name=\"rs_skippromotion\">\n"
        + "        <Value type=\"Boolean\">yes</Value>\n"
        + "        <Description>Specifies if this relationship should be skipped when an"
        + " item is promoted.</Description>\n"
        + "      </PSXProperty>\n"
        + "      <PSXProperty locked=\"yes\" name=\"rs_usecommunityfilter\">\n"
        + "        <Value type=\"Boolean\">yes</Value>\n"
        + "        <Description>Specifies if this relationship will be filtered by community"
        + " id.</Description>\n"
        + "      </PSXProperty>\n"
        ...
        + " checked the user RXSERVER\n"
        + "                will be used, otherwise the current user will be"
        + " used.</Description>\n"
        + "      </PSXProperty>\n"
        + "    </PSXPropertySet>\n"
        + "    <ProcessChecks>\n"
  ...
  private String getWidgetContentConfig() {
    return "<PSXRelationshipConfig category=\"rs_widget\" id=\"115\" label=\"Widget Content\""
        ...
        + "        <Value type=\"Boolean\">yes</Value>\n"
        + "        <Description>Specifies if this relationship must be packaged and deployed"
        + " with the owner.  If checked, the relationship must be deployed, if not, it is"
        + " optional.</Description>\n"
        + "      </PSXProperty>\n"
        ...
        + "        <Value type=\"Boolean\">no</Value>\n"
        + "        <Description>Defines whether or not to use the dependent revision as part"
        + " of the dependent locator.</Description>\n"
        + "      </PSXProperty>\n"
        + "      <PSXProperty locked=\"yes\" name=\"rs_useownerrevision\">\n"
        + "        <Value type=\"Boolean\">yes</Value>\n"
        + "        <Description>Defines whether or not to use the owner revision as part of"
