...
public class PSAction extends Task {
  ...
  public void execute() throws BuildException {
    ...
    System.setProperty(
        "javax.xml.parsers.SAXParserFactory", "com.percussion.xml.PSSaxParserFactoryImpl");
    System.setProperty(
    ...
    if (refreshProp != null) ms_bRefreshFiles = true;
  }
  ...
  protected String[] convertToArray(String arrayVals) {
    ...
    if (arrayVals == null || arrayVals.trim().length() == 0) arr = new String[0];
    else {
      ...
      if (vals.length == 0) {
        ...
        for (int i = 0; i < vals.length; i++) arr[i] = vals[i].trim();
      }
    }
