    ...
    try {
      if (destination.compareTo("") == 0) {
        ...
        if (tmpSrcFileName.compareTo("cacerts") == 0)
          tmpSrcFileRoot = tmpSrcFileRoot.substring(0, tmpSrcFileRoot.indexOf(JRELOCATION));
      ...
      } else resolvedDestination = destination;
      ...
      if ((!srcFile.isFile()) || (!srcFile.exists())) {
        if (ignoreIfSourceNotExists) return;
        else
          // Throw FileNotFoundException exception. It will be caught in the
          // catch bloack and written to the log file.
          throw new FileNotFoundException(
              "Failed to copy file : " + resolvedSource + "File does not exist.");
      }

      if (destFile.exists() && !overwriteDestinationFile) {
        PSLogger.logInfo("Did not overwrite file because the file exists : " + resolvedDestination);
        return;
      }
