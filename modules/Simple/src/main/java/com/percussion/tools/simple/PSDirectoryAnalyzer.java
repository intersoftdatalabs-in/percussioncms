...
public class PSDirectoryAnalyzer {
   ...
   */
  private void buildJarMap(File dir, Map<String, Map<String, String>> dirJarMap) {
  ...
  public static void main(String[] args) {
    try {
      ...
      if (!oldDir.isDirectory()) {
        throw new IllegalArgumentException(oldDir.getAbsolutePath() + " is not a valid directory");
      }
      ms_oldRoot = oldDir.getAbsolutePath();

      final File newDir = new File(args[1]);
      if (!newDir.isDirectory()) {
        throw new IllegalArgumentException(newDir.getAbsolutePath() + " is not a valid directory");
      }
  ...
  private static void printUsage() {
    ...
    System.out.println("<source file> <target dir> [output file]");
  }
