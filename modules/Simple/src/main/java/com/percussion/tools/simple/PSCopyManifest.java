...
public class PSCopyManifest {
   ...
   */
  public void copyManifest(File srcFile, File tgtDirectory, List<File> additionalEntries)
    ...
    final Writer out = new FileWriter(new File(tgtDirectory, srcFile.getName()));
    ...
    try {
      ...
      while ((output = in.readLine()) != null) {
        ...
        if (!inClasspath) {
          ...
          final String fromEntryStr =
              endsWithComma ? output.substring(0, output.length() - 1) : output;
   ...
   */
  private void writeEntry(Writer out, File fromEntry, File pluginDir, boolean endsWithComma)
      throws IOException {
    if (!fromEntry.exists()) {
