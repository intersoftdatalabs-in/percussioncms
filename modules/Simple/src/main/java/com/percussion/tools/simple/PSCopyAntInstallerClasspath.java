...
public class PSCopyAntInstallerClasspath {
  ...
  public void copyJars(File srcFile, File rootDirectory, File tgtDirectory)
    ...
    try {
      ...
      while ((output = in.readLine()) != null) {
        if (output.equals(CLASSPATH_START)) inClasspath = true;

        if (inClasspath && output.equals(CLASSPATH_END)) break;

        if (inClasspath) {
          int archiveIndex = output.indexOf(ARCHIVE_ATTR);
          if (archiveIndex == -1) continue;
          ...
          for (int i = 2; i < length; i++) newArr[i - 2] = arr[i];

          String jarPath = "";
          for (int i = 0; i < newArr.length; i++) {
            if (jarPath.length() > 0) jarPath += '/';
    ...
    System.out.println("Copied " + jarsCopied + " jar(s) to " + tgtDirectory.getAbsolutePath());
  }
  ...
  public static void main(String[] args) {
    try {
      ...
      new PSCopyAntInstallerClasspath().copyJars(srcFile, rootDir, tgtDir);
    } catch (Throwable t) {
