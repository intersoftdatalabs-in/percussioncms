...
(function ($) {
  ...
  function getDeliveryServicesDomain(widgetClass) {
    ...
    var deliveryUrl = "";
    try {
      if ("" === currentWidget.attr("data-query")) {
        return;
      }
  ...
  var methods = {
    ...
    show: function () {
      ...
      getComments(function (success, data) {
        if (
          success &&
          "undefined" !== typeof data.comments &&
          null !== data.comments
        ) {
  ...
  function getUrl() {
    ...
    if (!isBlank(settings.pagepath)) url += "&pagepath=" + settings.pagepath;
    if (!isBlank(settings.username)) url += "&username=" + settings.username;
    if (!isBlank(settings.sortby)) {
      url += "&sortby=" + settings.sortby;
      url += "&ascending=" + settings.ascending;
    }
    if (!isBlank(settings.tag)) url += "&tag=" + settings.tag;
    if (!isBlank(settings.state)) url += "&state=" + settings.state;
    if (!isBlank(settings.moderated)) url += "&moderated=" + settings.moderated;
  ...
  function createCommentHtml(commentData) {
    ...
    if (0 < settings.fields.length) {
      ...
      if (settings.lastCommentId === commentData["id"]) {
        ...
        if (
          !commentData["moderated"] &&
          "APPROVED" !== commentData["approvalState"]
        ) {
          comment.append(
            $("<div/>")
  ...
  function createFieldsHtml(commentData, fields, container) {
    for (var i = 0; i < fields.length; i++) {
      ...
      if (isBlank(commentData[field["name"]])) continue;
