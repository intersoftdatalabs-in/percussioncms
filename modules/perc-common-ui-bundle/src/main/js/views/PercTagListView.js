...
(function ($) {
  ...
  function updateTagList() {
    $(".perc-tag-list").each(function () {
      ...
      if ("" === currentTagList.attr("data-query")) {
        return;
      }
      ...
      $.PercTagListService.getTagEntries(
        queryString,
        orderBy,
        function (status, tagEntries) {
          if (status) {
            ...
            currentTagList
              .find(".perc-comma-separated-main-container")
              .empty()
              .remove();
            var separator = "";
            if (
            ...
            for (var i = 0; i < tagEntries.properties.length; i++) {
              ...
              var linkText =
                tagEntry.tagName + " (" + tagEntry.tagCount + ")" + separator;
