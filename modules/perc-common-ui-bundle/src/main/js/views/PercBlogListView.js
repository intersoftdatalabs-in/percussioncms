...
(function ($) {
  ...
  function loadResults(target, startPage, callback, callbackOptions) {
    ...
    target.settings.query.startIndex =
      target.settings.maxResults * (startPage - 1);
    //Set the base URL to create the href for each item then
    var baseURL = "";
    if (
      target.settings.isEditMode === "true" ||
      target.settings.isPreviewMode === "true"
    ) {
    ...
    if (
      "undefined" !== typeof urlstring.query &&
      "undefined" !== typeof urlstring.filter
    ) {
      try {
        var obj = JSON.parse(urlstring.query);
        $.map(obj.criteria, function (n, i) {
          if ($.inArray(n, target.settings.query.criteria) === -1) {
            target.settings.query.criteria[
              target.settings.query.criteria.length
            ] = n;
          }
        });
    ...
    return $.PercBlogListService.getPageEntries(
      target.settings.query,
  ...
  function displayResults(target, pageEntries, options) {
    ...
    if (
      ...
      try {
        ...
        currentBlogList
          .parent()
          .find(".perc-bloglist-result-container")
          .remove();
        currentBlogList
          .parent()
          .prepend('<div class="perc-result-divider"></div>')
          .prepend(
            createTitleHtml(
    ...
    try {
      ...
      var listRoot = currentBlogList
        .find(".perc-blog-list")
        .not(".perc-blog-list-structure .perc-blog-list")
        .empty();
      ...
      for (var i = 0; i < pageEntries.length; i++) {
        ...
        if (spClass !== "") newListElem.addClass(spClass);
        ...
        newListElem
          .find("a")
          .attr("href", pageEntry.folder + pageEntry.name)
          .not(".perc-no-update-link-text")
          .html(linkText);
        newListElem.find("a").attr("aria-label", linkText);
        var commentText = newListElem
          .find(".perc-blog-list-comment-container a")
          .attr("title");
        ...
        newListElem
          .find(".perc-blog-list-comment-container a")
          .attr("aria-label", "Comments for: " + linkText);

        if (summary) {
          newListElem
            .find("div.perc-blog-list-summary-container")
            .prepend(summary);
        }
        if (tagList) {
          ...
          newListElem
            .find("div.perc-blog-list-tag-container")
            .addClass("perc-blog-hide-container");
        }
        if (categoryList) {
          newListElem
            .find("div.perc-blog-list-category-container")
            .append(categoryList);
        } else {
          newListElem
            .find("div.perc-blog-list-category-container")
        ...
        if (author) {
          newListElem
            .find(".perc-blog-list-byline-container")
            .text(target.settings.blogPostBylineText + " " + author);
        }
        if (postDate) {
          newListElem
            .find(".perc-blog-list-date-container")
            .text(postDate + " " + timeZone);
        }
  ...
  function createTitleHtml(text, target) {
    var count = target.totalEntries;
    return $("<div/>")
      .append($("<h2/>").addClass("perc-bloglist-result-title").text(text))
      .append(
        $("<p/>")
