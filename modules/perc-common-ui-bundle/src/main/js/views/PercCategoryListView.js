...
(function ($) {
  ...
  function updateCategoryList() {
    $(".perc-category-list").each(function () {
      ...
      if ("" === currentCategoryList.attr("data-query")) {
        return;
      }
      ...
      $.PercCategoryListService.getCategories(
        queryString,
        function (status, categoryEntries) {
          ...
          if (status && 0 < categoryEntries.length) {
            ...
            currentCategoryList
              .find(
                ".perc-category-list-expanded,.perc-category-list-collapsible",
              )
              .empty();
            //Destroy previous element of Dynatree.
            currentCategoryList
              .find(".perc-category-list-collapsible")
              .fancytree("destroy");

            var ul;
            if ("collapsible" === display) {
              //Collapsible Tree
              ul = createNestedUlLiTreeCollapsible(
                categoryEntries,
                "",
                maxCat,
                0,
  ...
  function createNestedUlLiTreeCollapsible(
    treeNodes,
    parentPath,
    maxRow,
    nLevel,
    ...
    for (var n = 0; n < treeNodes.length; n++) {
      if (1 !== nLevel || n < maxRow || 0 == maxRow) {
        ...
        if (node.children && 0 < node.children.length) {
          li.append(
            createNestedUlLiTreeCollapsible(
              node.children,
              node.path,
              maxRow,
              nLevel + 1,
  ...
  function parseNode(node) {
    ...
    if (countTotal > 0) nodeStr = node.category + " (" + countTotal + ")";
    ...
    return $("<li>").addClass("perc-category-element").append(a);
  }
})(jQuery);
