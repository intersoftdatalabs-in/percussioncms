...
(function ($) {
  ...
  var PercResultsPaging = function (loadCallback, callbackOptions) {
    ...
    if (!(0 < parseInt(elem.settings.maxResults, 10))) {
  ...
  var updateNav = function (target, navLoc, callback, callbackOptions) {
    ...
    if (
      1 >= target.totalPages ||
      !(parseInt(target.totalPages, 10) === target.totalPages) ||
      maxPerPage >= totalMaxResults
    ) {
    ...
    if (4 < target.totalPages) {
      if (3 > navLoc) {
        // Special case to clamp beginning of paging to the left side.
        center = 3;
        if (1 === navLoc) {
      ...
      } else if (navLoc > target.totalPages - 2) {
        // Special case to clamp end of paging to the right side.
      ...
      } // Somewhere in the middle, everything is standard.
      else {
        center = navLoc;
    ...
    if (1 < target.totalPages) {
      ...
      if (1 < navLoc) {
        ...
        currDiv
          .find(".perc-goto-first")
          .attr("aria-label", "First page")
          .attr("href", genPageUrl(1))
          .off("click")
        ...
        currDiv
          .find(".perc-goto-pre")
          .attr("aria-label", "Previous page " + (navLoc - 1))
          .attr("href", genPageUrl(navLoc - 1))
          .off("click")
          .on("click", function (event) {
        ...
        //need just any content in hidden link to make validation tool happy
      ...
      currDiv
        .find(".perc-first")
        .attr("aria-label", "Page " + (center - 2))
        .attr("href", genPageUrl(center - 2))
        .text(center - 2)
        .off("click")
        .on("click", function (event) {
      ...
      currDiv
        .find(".perc-second")
        .attr("aria-label", "Page " + (center - 1))
        .attr("href", genPageUrl(center - 1))
        .text(center - 1)
        .off("click")
        .on("click", function (event) {
      ...
      if (navLoc < target.totalPages) {
        currDiv
          .find(".perc-goto-next")
          .attr("aria-label", "Next page " + (navLoc + 1))
          .attr("href", genPageUrl(navLoc + 1))
          .off("click")
          .on("click", function (event) {
            quickNav(target, navLoc + 1, callback, callbackOptions, event);
        ...
        currDiv
          .find(".perc-goto-last")
          .attr("aria-label", "Last page " + target.totalPages)
          .attr("href", genPageUrl(target.totalPages))
          .off("click")
          .on("click", function (event) {
            quickNav(
              target,
    ...
    if (2 < target.totalPages) {
      currDiv
        .find(".perc-third")
        .attr("aria-label", "Page " + (center + 0))
        .attr("href", genPageUrl(center + 0))
        .text(center + 0)
        .off("click")
        .on("click", function (event) {
    ...
    currDiv
      .find(navLocClass)
      .attr("aria-label", "Current page " + navLoc)
      .attr("href", genPageUrl(navLoc))
      .addClass("perc-current-page")
      .off("click")
      .on("click", function (event) {
    ...
    // Update page count.
    currDiv
      .find(".perc-page-count .perc-page-count-current")
      .text(navLoc + " " + target.settings.pagingOfText);
    currDiv
      .find(".perc-page-count .perc-page-count-total")
      .text(target.totalPages + " " + target.settings.pagingPagesText);
  };
  ...
  var quickNav = function (target, navLoc, callback, callbackOptions, event) {
    ...
    if (event) {
      // only if arrived here via onClick on pagination control
      event.preventDefault();
      try {
        window.history.pushState(null, document.title, genPageUrl(navLoc));
      } catch (e) {
  ...
  function getPageFromLocation() {
    ...
    if (
      queryStr.length > pageParamName.length + 1 &&
      0 < queryStr.indexOf(pageParamName)
    ) {
      //the string has at least "?page=x"

      var queryParams = queryStr.substring(1).split("&");

      for (var i = 0; i < queryParams.length; i++) {
        // reassembling without page param
  ...
  function genPageUrl(page) {
    ...
    if (queryStr.length > "page=".length && 0 < queryStr.indexOf("page=")) {
      //the string has at least "?page="
      var queryParams = queryStr.substring(1).split("&");
      queryStr = "";
      for (var i = 0; i < queryParams.length; i++) {
        // reassambling without page param
        if (queryParams[i] && queryParams[i].indexOf("page="))
          queryStr = queryStr + (queryStr ? "&" : "") + queryParams[i];
      }
