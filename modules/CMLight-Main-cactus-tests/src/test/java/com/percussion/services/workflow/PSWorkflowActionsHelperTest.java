 ...
 * transition as none are configured by default and there is no programmatic
 * manipulation possible.
 */
@Tag("IntegrationTest")
public class PSWorkflowActionsHelperTest {
    ...
    void testActions() throws Exception {
        ...
        secWs.login(null, null, "author1", "demo", null, "Enterprise_Investments", null);
        ...
        List<PSAssignmentTypeEnum> assnTypes = new ArrayList<>();
        ...
        roleList.add("Editor");
        var helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "editor1", roleList, "en-us");
        ...
        roleList.add("QA");
        helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "qa1", roleList, "en-us");
        assertTrue(helper.getAllWorkflowActions().isEmpty());
        ...
        roleList.add("Author");
        helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "author1", roleList, "en-us");
        var actions = helper.getCIAOActions();
        assertEquals(1, actions.size());
        assertEquals(PSMenuAction.CHECKIN_ACTION_NAME, actions.get(0).getName());
        ...
        // test as editor, since item is checked out by author, actions should be empty
        assnTypes.clear();
        roleList.clear();
        assnTypes.add(PSAssignmentTypeEnum.ASSIGNEE);
        roleList.add("Editor");
        helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "editor1", roleList, "en-us");
        assertTrue(helper.getAllWorkflowActions().isEmpty());
        ...
        roleList.add("Admin");
        helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "admin1", roleList, "en-us");
        actions = helper.getCIAOActions();
        assertEquals(1, actions.size());
        assertEquals(PSMenuAction.FORCE_CHECKIN_ACTION_NAME, actions.get(0).getName());
        actions = helper.getTranstionActions();
        assertEquals(2, actions.size());
        expectedActions = new HashSet<>();
        expectedActions.add("DirecttoPublic");
        expectedActions.add("Approve");
        checkActions(expectedActions, actions);
        ...
        actions = helper.getCIAOActions();
        assertEquals(1, actions.size());
        assertEquals(PSMenuAction.CHECKOUT_ACTION_NAME, actions.get(0).getName());
        actions = helper.getTranstionActions();
        assertEquals(2, actions.size());
        expectedActions = new HashSet<>();
        expectedActions.add("DirecttoPublic");
        ...
        systemWs.transitionItems(Collections.singletonList(contentIds.get(1)), "DirecttoPublic");
        helper = new PSWorkflowActionsHelper(contentIds, assnTypes, "author1", roleList, "en-us");
        assertTrue(helper.getAllWorkflowActions().isEmpty());
    }
     ...
     */
    private void checkActions(Set<String> expectedActions, List<PSMenuAction> actions) throws Exception {
        Set<String> testActions = new HashSet<>();
    ...
    private PSServerItem createItem(String identifier) throws Exception {
        ...
        // create the item
        var itemDef = PSItemDefManager.getInstance().getItemDef("rffBrief", tok);
        var item = new PSServerItem(itemDef, null, tok);
        var fields = item.getAllFields();
        while (fields.hasNext()) {
            var field = fields.next();
            var name = field.getName();
            if (name.equals("sys_title") || name.equals("displaytitle") || name.equals("callout")) {
