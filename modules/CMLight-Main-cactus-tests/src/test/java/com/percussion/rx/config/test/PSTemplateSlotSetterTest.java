...
public class PSTemplateSlotSetterTest extends PSConfigurationTest // TestCase
{
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    List<PSPair<String, String>> assocs =
        (List<PSPair<String, String>>) defs.get("perc.prefix.assocs");
  ...
  public void testConfigFiles_WithPrevProperties() throws Exception {
    ...
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, IMPL_CFG, DEFAULT_CFG, LOCAL_CFG);
    slot = getSlot();
    assertTrue(origAssoc.equals(slot.getSlotAssociations()));
  }
  ...
  static {
    ...
    ms_defaultParams.put(
        "query",
        "select rx:sys_contentid, rx:sys_contentstartdate  from rx:rffcalendar  where jcr:path like"
            + " :sitepath order by rx:sys_contentstartdate  asc");
