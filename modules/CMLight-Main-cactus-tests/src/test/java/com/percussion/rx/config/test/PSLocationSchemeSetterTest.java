...
public class PSLocationSchemeSetterTest extends PSConfigurationTest {
   ...
   *
   * @param extName the extension/generator name. This is not FQN of the java extension.
  ...
  public void testModifyExpress() throws Exception {
    IPSGuid siteFolderCtxId = PSConfigUtils.getContextModel().nameToGuid("Site_Folder_Assembly");
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    List<PSPair<String, String>> params =
        (List<PSPair<String, String>>) defs.get("perc.prefix.param");
  ...
  /**
   * Tests validation on the same sets of Context / Location Scheme pairs in different packages.
   *
  ...
  public void testCreateSchemes() throws Exception {
    ...
    try {
      ...
      PSConfigFilesFactoryTest.applyConfig(PKG_NAME, IMPL_CFG_POS, LOCAL_CFG);
  ...
  public void testLocationScheme_WithPrevProperties() throws Exception {
    try {
      PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, LOCAL_CFG_2_PREV);
      ...
      PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, LOCAL_CFG_2, LOCAL_CFG_2_PREV);
  ...
  public void testLocationScheme_UnProcess() throws Exception {
    try {
      ...
      PSConfigFilesFactoryTest.deApplyConfig(PKG_NAME, CONFIG_DEF_2, LOCAL_CFG_2);
      validateLocationScheme(PREVIEW, EXTRA_SCHEME_ONE, false);
  ...
  public void testEmptyLocationScheme() throws Exception {
    ...
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, EMPTY_LOCAL_CFG_2);

    // do nothing if there is no context specified - NULL
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, NULL_CTX_LOCAL_CFG_2);

    // create a Location Scheme
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, ONE_CTX_LOCAL_CFG_2);
    validateLocationScheme(ASSEMBLY, EXTRA_SCHEME_ONE, true);

    // remove the (created) one Location Scheme
    PSConfigFilesFactoryTest.applyConfig(
        PKG_NAME, CONFIG_DEF_2, EMPTY_LOCAL_CFG_2, ONE_CTX_LOCAL_CFG_2);
  ...
  private void validateLocationScheme(String ctxName, String schemeName, boolean isExist) {
    ...
    if (isExist) assertTrue(scheme != null);
    else assertTrue(scheme == null);
  }
  ...
  /**
   * Negative test. Content Type / Template association is not unique within the same Context. It is
   * the same with an existing Location Scheme in the repository
   *
   * @throws Exception
   */
  ...
  /**
   * Negative test, Content Type / Template association is not unique within the same Context.
   * Another Location Scheme (in cachedMap) has the same association.
   *
   * @throws Exception
   */
  ...
  /**
   * Negative test, template is different with another same name Location Scheme under the same
   * Context
   *
   * @throws Exception
   */
