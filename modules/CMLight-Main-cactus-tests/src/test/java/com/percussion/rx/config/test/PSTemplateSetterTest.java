...
public class PSTemplateSetterTest extends PSConfigurationTest {
  ...
  public void testPropertiesAssociation() throws Exception {
    ...
    PSAssociationSet aset = new PSAssociationSet(IPSAssociationSet.AssociationType.TEMPLATE_SLOT);
    aSets.add(aset);
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    assertTrue(
        "Expect rffGtCorporateInvestmentsCommon",
        gTemplate.equals("rffGtCorporateInvestmentsCommon"));
    ...
    assertTrue("Expect \"$sys.variables.rxs_navbase\"", value.equals("$sys.variables.rxs_navbase"));
  }
  ...
  public void testBindingsSet() throws Exception {
    ...
    PSPair<String, String> p =
        new PSPair<String, String>("$rxs_navbase", "$sys.variables.rxs_navbase");
    bindingSet.add(p);
  ...
  private void resetBindings(IPSAssemblyTemplate template) {
    PSTemplateBinding binding =
        new PSTemplateBinding(1, "$rxs_navbase", "$sys.variables.rxs_navbase");
    template.getBindings().clear();
