 ...
 * - Test the retry logic used when the ftp delivery of a RetryItem fails.
 * - Test the release logic of a RetryItem
 ...
 */
@Tag("IntegrationTest")
public class PSDeliveryHandlerTest {

    @Test
    public void testCanonicalize() {
        var h = new PSFileDeliveryHandler();
    ...
    public void testItemRetry() throws IOException {
        var data = new RetryItemSetupData();
    ...
    /**
     * Tests to see if the release logic of the RetryItem is working properly.
     */
    @Test
    ...
    private void checkRetryItem(PSBaseFtpDeliveryHandler.RetryItem retryItem)
            throws IOException {
        var first = readContent(retryItem);

        if (retryItem.getRetryPossible()) {
            var second = readContent(retryItem);
