...
public class PSSiteSetterTest extends PSConfigurationTest {
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    List<Map<String, String>> vars = (List<Map<String, String>>) defs.get("perc.prefix.vars");
    assertTrue("Expect 3 variables", vars.size() == 3);
    String gTemplate = (String) defs.get("perc.prefix.gTemplate");
    assertTrue(
        "Expect \"rffGtEnterpriseInvestmentsCommon\"",
        gTemplate.equals("rffGtEnterpriseInvestmentsCommon"));
  }
  ...
  public void testValidateSiteVariables() throws Exception {
    ...
    try {
      factory =
          PSConfigFilesFactoryTest.applyConfigAndReturnFactory(PKG_NAME, CONFIG_DEF_2, LOCAL_CFG_2);
      ...
      try {
        PSConfigFilesFactoryTest.applyConfig(PKG_NAME + "_2", CONFIG_DEF_2, LOCAL_CFG_2);
  ...
  private void validateSiteVar(String siteName, String varName, boolean isExist)
    ...
    if (isExist) assertTrue(StringUtils.isNotBlank(value));
    else assertTrue(StringUtils.isBlank(value));
  }
