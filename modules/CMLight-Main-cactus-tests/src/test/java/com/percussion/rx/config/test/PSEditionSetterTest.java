...
public class PSEditionSetterTest extends PSConfigurationTest {
  ...
  private PSEditionWrapper createEdition() throws Exception {
    ...
    PSEditionWrapper wrapper = (PSEditionWrapper) model.loadModifiable(EDITION_NAME);
  ...
  public void testAddPropertyDefs() throws Exception {
    ...
    List<Map<String, Object>> preTask =
        (List<Map<String, Object>>) defs.get("perc.prefix.pre_tasks");
    List<Map<String, Object>> postTask =
        (List<Map<String, Object>>) defs.get("perc.prefix.post_tasks");
    ...
    assertTrue(
        ...
        pstTask.get("extensionName").equals("Java/global/percussion/task/sys_editionCommandTask"));
  }
  ...
  public void testEditionProperties() throws Exception {
    ...
    if (!preTasks.get(0).getContinueOnFailure()) {
      ...
      assertTrue(preTasks.get(0).getSequence() < preTasks.get(1).getSequence());
    } else {
