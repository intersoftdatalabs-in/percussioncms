...
public class PSSiteTemplateVisibilitySetterTest extends PSConfigurationTest {
  ...
  public void testConfigFiles() throws Exception {
    ...
    try {
      factory =
          PSConfigFilesFactoryTest.applyConfigAndReturnFactory(PKG_NAME, CONFIG_DEF, LOCAL_CFG);
      assertTrue(siteAssociateTemplate(CI_SITE, CI_TEMPLATE_NAME));
      assertTrue(siteAssociateTemplate(EI_SITE, CI_TEMPLATE_NAME));

      // Testing validation
      try {
        PSConfigFilesFactoryTest.applyConfig(PKG_NAME + "_2", CONFIG_DEF, LOCAL_CFG);
    ...
    // \/\/\/\/\/\/\/\
    // cleanup
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF, DEFAULT_CFG, LOCAL_CFG);
  ...
  public void testConfigFiles_WithPrevProperties() throws Exception {
    ...
    // \/\/\/\/\/\/\/\
    // cleanup
    PSConfigFilesFactoryTest.applyConfig(PKG_NAME, CONFIG_DEF_2, DEFAULT_CFG, LOCAL_CFG_2);
  ...
  /**
   * Determines if the Site/Template association contains the given template name.
   *
   * @param siteName the name of the Site in question, assumed not <code>null</code> or empty.
   * @param templateName the name of the template in question, assumed not <code>null</code> or
   *     empty.
   * @return <code>true</code> if the Site/Template association contains the given template.
   */
  private boolean siteAssociateTemplate(String siteName, String templateName)
    ...
    for (IPSAssemblyTemplate t : site.getAssociatedTemplates()) {
      if (t.getName().equals(templateName)) return true;
    }
    return false;
  }
