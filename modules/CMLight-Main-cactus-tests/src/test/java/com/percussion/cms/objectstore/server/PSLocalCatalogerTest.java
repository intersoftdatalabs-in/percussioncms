...
public class PSLocalCatalogerTest {
  ...
  public void ignored_testFieldCatalog() throws Exception {
    var cat = new PSLocalCataloger(PSRequestInfo.getRequestInfo(PSRequestInfo.KEY_PSREQUEST));

    PSContentEditorFieldCataloger ceCat;
    ceCat = new PSContentEditorFieldCataloger(cat, null, IPSFieldCataloger.FLAG_INCLUDE_ALL);
    var allMap = ceCat.getAll();

    var fields = new HashSet<String>();
    ceCat = new PSContentEditorFieldCataloger(cat, fields, IPSFieldCataloger.FLAG_INCLUDE_ALL);
    ...
    for (var key : allMap.keySet()) {
      ...
      for (var obj : map1.keySet()) {
        ...
        if (name.equals("sys_workflowid") || name.equals("sys_communityid")) continue;
        ...
        if (field.getDisplayChoices() != null && field.getDisplayChoices().areChoicesLoaded()) {
    ...
    ceCat =
        new PSContentEditorFieldCataloger(
            cat, choiceFields, IPSFieldCataloger.FLAG_EXCLUDE_CHOICES);
    ...
    ceCat = new PSContentEditorFieldCataloger(cat, IPSFieldCataloger.FLAG_EXCLUDE_CHOICES);
    assertEquals(IPSFieldCataloger.FLAG_EXCLUDE_CHOICES, ceCat.getControlFlags());
  ...
  private void checkChoices(
      Set<String> choiceFields, Collection<PSLightWeightField> testFields, boolean choicesLoaded)
      throws Exception {
    for (var field : testFields) {
      if (choiceFields.contains(field.getInternalName())) {
        assertNotNull(field.getDisplayChoices());
        assertEquals(field.getDisplayChoices().areChoicesLoaded(), choicesLoaded);
   ...
   * @return the set, never <code>null</code>, may be empty.
   */
  private Set<PSLightWeightField> getAllFields(PSContentEditorFieldCataloger ceCat) {
    var testMap = ceCat.getAll();
   ...
   */
  private Set<String> getAllFieldNames(Map<String, Collection<PSLightWeightField>> typeMap) {
