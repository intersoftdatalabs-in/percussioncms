...
public class PSPageServiceTest extends PSServletTestCase {
  ...
  public void test100FindPagesByTemplateId() throws Exception {
    ...
    var page1Id =
        createPage(
            "Page1", "Page 1", templateId, siteFolderPath, "TestLink", "testurl.file", "", "");
    assertNotNull(page1Id);

    var page2Id =
        createPage(
            "Page2", "Page 2", templateId, siteFolderPath, "TestLink", "testurl.file", "", "");
  ...
  public void test110CopyPage() throws Exception {
    ...
    PSAssetWidgetRelationship awRel =
        new PSAssetWidgetRelationship(pageId, 5, "widget5", localAssetId, 1);
  ...
  public void test120ChangeTemplate() throws Exception {
    ...
    assertNotNull(page);
    assertEquals(
        fixture.template1.getContentMigrationVersion(), page.getTemplateContentMigrationVersion());
    ...
    assertEquals(
        templateSum2.getContentMigrationVersion(), fixture.template1.getContentMigrationVersion());
    ...
    // need to save the template through the dao directly, as the content migration version is
    // overriden in the
    // service call
  ...
  public void test140Update() throws Exception {
    ...
    PSTemplateSummary template2 =
        fixture.createTemplateWithSite("TestTemplate2", fixture.site1.getId());
    ...
    assertTrue(
        relationshipExists(
            pageId, assetId, PSWidgetAssetRelationshipService.LOCAL_ASSET_WIDGET_REL_FILTER));
  ...
  public void test210GetUnusedAssets_assetOrphanByWidgetRenaming() throws PSDataServiceException {
    ...
    List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairsForPage =
        new ArrayList<PSPair<PSWidgetItem, PSAsset>>();
    PSPair<PSWidgetItem, PSAsset> richText =
        createRichTextWidgetItem(richTextName, richTextDescription, "1");
    PSPair<PSWidgetItem, PSAsset> rawHtml =
        createRawHtmlWidgetItem(rawHtmlName, rawHtmlDescription, "2");
    ...
    List<PSPair<PSWidgetItem, PSAsset>> unusedPairs =
        new ArrayList<PSPair<PSWidgetItem, PSAsset>>();
  ...
  private void validateGetWidgetRelationship(
      String pageId, String widgetName, long widgetId, boolean matchNamedAsset, String assetId) {
    ...
    Collection<PSRelationship> rels =
        PSWidgetContentFinderUtils.getLocalSharedAssetRelationships(id.toString());
    Collection<PSRelationship> matchRels =
        PSWidgetContentFinderUtils.getMatchRelationships(rels, Collections.singletonList(wi));
  ...
  private void validateUpdateWidgetNameAPI(String templateId, String newWidgetName, int widgetId) {
    int rows =
        widgetAssetRelationshipDao.updateWidgetNameForRelatedPages(
            templateId, newWidgetName, widgetId);
    assertTrue(rows == 1);
  ...
  /**
   * Create a Rich text widget item, and its corresponding asset, with the given widget name, widget
   * description, and slot id (or widget id). The asset is saved into the system.
   *
   * @param name {@link String} with the name of the widget. May be blank.
   * @param description {@link String} with the description of the widget. May be blank.
   * @param slotid {@link String} with the widget id. May be blank.
   * @return {@link PSPair}<{@link PSWidgetItem}, {@link PSAsset}> never <code>null</code>, contains
   *     the widget item in the first place, and the asset in the second place.
   */
  private PSPair<PSWidgetItem, PSAsset> createRichTextWidgetItem(
      String name, String description, String slotid) throws PSDataServiceException {
    PSAsset asset = new PSAsset();
    asset.getFields().put("sys_title", "LocalAsset" + System.currentTimeMillis());
  ...
  /**
   * Create a Raw HTML widget item, and its corresponding asset, with the given widget name, widget
   * description, and slot id (or widget id). The asset is saved into the system.
   *
   * @param name {@link String} with the name of the widget. May be blank.
   * @param description {@link String} with the description of the widget. May be blank.
   ...
   *     the widget item in the first place, and the asset in the second place.
   */
  private PSPair<PSWidgetItem, PSAsset> createRawHtmlWidgetItem(
      String name, String description, String slotid) throws PSDataServiceException {
    PSAsset asset = new PSAsset();
    asset.getFields().put("sys_title", "LocalAsset" + System.currentTimeMillis());
    asset.setType("percRawHtmlAsset");
  ...
  /**
   * Iterates over the region tree of the template and then renames the widget if the instance is
   * found.
   *
   * @param widgetToRename {@link PSWidgetItem} object, representing the widget to be renamed.
   *     assumed not <code>null</code>. Assumed not <code>null</code>
   * @param newName {@link String} with the new name for the widget.
   */
  private void renameWidgetOnTemplateAndSave(
      PSWidgetItem widgetToRename, PSTemplate template, String newName)
      throws PSDataServiceException {
   ...
   * <li>the orphan assets list is not <code>null</code>.<br>
   * <li>the size of the orphan assets list is the same as the widget asset pairs list.
   * <li>the orphan assets have the same slot id and widget name as they were in the template.
   *
   * @param page {@link PSPage} object representing the page.
   * @param template {@link PSTemplate} object representing the page's template.
   * @param widgetAssetPairs {@link List}<{@link PSPair}<{@link PSWidgetItem}, {@link PSAsset}>>
   *     with the widgets added to the template, and the corresponding asset added to the page.
   */
  private void verifyOrphanAssets(
      PSPage page, PSTemplate template, List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairs) {
    Set<PSOrphanedAssetSummary> unused = getOrphanedAssetsSummaries(page, template);
    assertNotNull(unused);
    ...
    for (PSOrphanedAssetSummary orphan : unused) {
      for (PSPair<PSWidgetItem, PSAsset> pair : widgetAssetPairs) {
        ...
        if (((isEmpty(orphan.getWidgetName()) && isEmpty(widget.getName()))
                || equalsIgnoreCase(orphan.getWidgetName(), widget.getName()))
  ...
  /**
   * Removes the given widget from the template region tree and then saves the changed template.
   *
   * @param template {@link PSTemplate} to remove the widget from. Assumed not <code>null</code>.
   * @param widgetAssetPairsToRemove {@link List}<{@link PSPair}< {@link PSWidgetItem}, {@link
   *     PSAsset}>> holding the widgets that are meant to be removed
   * @return the modified {@link PSTemplate} object. Never <code>null</code>.
   */
  private PSTemplate removeWidgetFromTemplateAndSave(
      PSTemplate template, List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairsToRemove)
      throws PSDataServiceException {
    for (PSPair<PSWidgetItem, PSAsset> pair : widgetAssetPairsToRemove) {
      Map<String, List<PSWidgetItem>> regionWidgetsMap =
          template.getRegionTree().getRegionWidgetsMap();
  ...
  /**
   * Adds the given widgets on the template or page (according to the last parameter). Basically it
   * creates instances of {@link PSRelationship} where the owner is the page or the template, and
   * the dependant is the given asset, using the widget id (slot id) from the {@link PSWidgetItem}
   * object.
   *
   * @param page {@link PSPage} the page to use on the relationship. Assumed not <code>null</code>.
   * @param template {@link PSTemplate} the template to use in the relationship. Assumed not <code>
   *     null</code>.
   * @param widgetAssetPairs {@link List}<{@link PSPair}<{@link PSWidgetItem}, {@link PSAsset}>>
   * @param addOnTemplate if <code>true</code> the content will be added to the template. If <code>
   *     false</code> the content will be added into the template.
   * @return {@link List}<{@link PSAssetWidgetRelationship}> with the created relationships. Never
   *     <code>null</code> but may be empty.
   */
  private List<PSAssetWidgetRelationship> addContentToWidgetOnPageOrTemplate(
      PSPage page,
      PSTemplate template,
      List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairs,
      boolean addOnTemplate)
      throws PSDataServiceException {
  ...
  /**
   * Adds the given widgets to the template. Creates a region and adds the widgets to it. Then it
   * sets that region to a region tree and set it to the template.
   *
   * @param template {@link PSTemplate} object, must not be <code>null</code>.
   * @param widgetAssetPairs {@link List}<{@link PSWidgetItem}> with the widgets to add. May be
   *     empty but not <code>null</code>.
   */
  private void addWidgetsToTemplateAndSave(
      PSTemplate template, List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairs)
      throws PSDataServiceException {
    notNull(template);
  ...
  /**
   * Creates the test scenario for the unused assets generated with no matching template widgets. It
   * creates a template and a page, then adds widget to template, and asset to page. Then it removes
   * the widget from the template generating unused assets. The widgets will be created with or
   * without a name, depending on the parameter.
   *
   * @param namedWidgets if <code>true</code>, the widgets will be created with a name. If <code>
   *     false</code>, widgets will be created with no name.
   */
  ...
  /**
   * Creates the test scenario for the unused assets generated with no page assets overriden by
   * template assets. It creates a template and a page, then adds widget to template, asset to page,
   * and then asset in the template. That generates unused assets. The widgets will be created with
  ...
  private void createTestGetUnusedAsset_hiddenByTemplateAssets(boolean namedWidgets)
    ...
    List<PSPair<PSWidgetItem, PSAsset>> widgetAssetPairsForTemplate =
        new ArrayList<PSPair<PSWidgetItem, PSAsset>>();
    PSPair<PSWidgetItem, PSAsset> richText2 =
        createRichTextWidgetItem(richTextName, richTextDescription, "1");
    PSPair<PSWidgetItem, PSAsset> rawHtml2 =
        createRawHtmlWidgetItem(rawHtmlName, rawHtmlDescription, "2");
  ...
  /**
   * Determines if a relationship exists for the specified owner, dependent, and name.
   *
   * @param ownerId the owner id, assumed not <code>null</code>. This is the {@link IPSGuid}
   *     representation in string format.
   * @param depId the dependent id, assumed not <code>null</code>. This is the {@link IPSGuid}
   *     representation in string format.
   * @param relName the name of the relationship type to filter by, assumed not <code>null</code>.
   * @return <code>true</code> if the relationship exists, <code>false</code> otherwise.
   * @throws PSErrorException
   */
  private boolean relationshipExists(String ownerId, String depId, String relName)
      throws PSErrorException {
  ...
  public void setWidgetAssetRelationshipDao(
