...
public class PSPageToTemplateTest extends PSServletTestCase {
  ...
  public void testTemplateToPage() throws Exception {
    ...
    var pageId =
        createPage(
            ...
            folderPath,
            linkTitle,
            fixture.site1.getBaseUrl() + "/Plans_Of_Fu_Manchu.html",
            "true",
            "This is the plan of Fu Manchu");
  ...
  private String createPage(
    ...
    PSPair<PSWidgetItem, PSAsset> widgetAssetPair =
        new PSPair<PSWidgetItem, PSAsset>(rawHtmlWidget, localAsset);
    widgetService.createRelationship(
        localAsset.getId(), page.getId(), rawHtmlWidget.getId(), rawHtmlWidget.getName(), false);
    // Set the widget
    page.getRegionBranches()
        .setRegionWidgets(PSSiteTemplateService.REGION_CONTENT, asList(rawHtmlWidget));
