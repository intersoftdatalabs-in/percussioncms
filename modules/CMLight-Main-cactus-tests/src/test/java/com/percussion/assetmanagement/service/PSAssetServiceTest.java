...
class PSAssetServiceTest extends PSServletTestCase {
  ...
  void testCreateAssetWidgetRelationship() throws Exception {
    ...
    try {
      ...
      assertEquals(
          ...
          rel.getProperty(PSWidgetAssetRelationshipService.ASSET_ORDER_PROP_NAME));
  ...
  void testShareLocalAsset() throws Exception {
    ...
    try {
      ...
      PSAssetWidgetRelationship awRel =
          new PSAssetWidgetRelationship(templateId, 5, "widget5", assetId, 1);
      awRelId = assetService.createAssetWidgetRelationship(awRel);
      assertNotNull(awRelId);
      ...
      PSFolderProperties folderProperties =
          folderHelper.findFolderProperties(folderSummary.getId());
  ...
  void testClearAssetWidgetRelationship() throws Exception {
    ...
    try {
      ...
      PSAssetWidgetRelationship localRel =
          new PSAssetWidgetRelationship(templateId, 5, "widget5", assetId, 1);
      ...
      PSAssetWidgetRelationship sharedRel =
          new PSAssetWidgetRelationship(ownerId, 6, "widget6", asstId, 1);
  ...
  void testDeleteLocalAssets() throws Exception {
    ...
    try {
      ...
      PSAssetWidgetRelationship awRel1 =
          new PSAssetWidgetRelationship(templateId, 5, "widget5", asset1Id, 1);
      ...
      PSAssetWidgetRelationship awRel2 =
          new PSAssetWidgetRelationship(templateId, 6, "widget6", asset2Id, 2);
      ...
      PSAssetWidgetRelationship awRel3 =
          new PSAssetWidgetRelationship(templateId, 7, "widget7", asset3Id, 3);
  ...
  void testGetLinkedAssets() throws Exception {
    ...
    try {
      ...
      PSRelationship rel =
          systemWs.createRelationship(
              PSWidgetAssetRelationshipService.SHARED_ASSET_WIDGET_REL_TYPE,
  ...
  void testGetLinkedAssetsForAsset() throws Exception {
    ...
    try {
      ...
      PSRelationship rel =
          systemWs.createRelationship(
              PSWidgetAssetRelationshipService.SHARED_ASSET_WIDGET_REL_TYPE,
              idMapper.getGuid(assetId),
              idMapper.getGuid(asset1Id));
      systemWs.saveRelationships(Collections.singletonList(rel));
      rel =
          systemWs.createRelationship(
              PSWidgetAssetRelationshipService.LOCAL_ASSET_WIDGET_REL_TYPE,
              idMapper.getGuid(assetId),
              idMapper.getGuid(asset2Id));
      systemWs.saveRelationships(Collections.singletonList(rel));
  ...
  void testIsUsedByTemplate() throws Exception {
    ...
    try {
      ...
      PSAssetWidgetRelationship awRel2 =
          new PSAssetWidgetRelationship(templateId, 6, "widget6", assetId, 2);
      awRel2.setResourceType(PSAssetResourceType.shared);
      String awRelId2 = assetService.createAssetWidgetRelationship(awRel2);
      awRelIds.add(idMapper.getGuid(awRelId2));
  ...
  void testUpdateLocalRelationship() throws Exception {
    ...
    try {
      ...
      // should be equal
      assertEquals(
          idMapper.getLocator(asset1Id), getLocalRelationships(templateId).get(0).getDependent());
    } finally {
      try {
        systemWs.deleteRelationships(awRelIds);
      } catch (Exception e) {
  ...
  void testCreateExtractedAssets() throws Exception {
    ...
    try {
      ...
      in = getClass().getResourceAsStream(fileName);
      PSExtractedAssetRequest req =
          new PSExtractedAssetRequest(folderPath, type, fileName, in, selector, includeOuterHtml);
   ...
   *
   * @param ownerId the owner id, assumed not blank. This is the {@link IPSGuid} representation in
   *     string format.
   * @param widgetId
   * @param assetId the asset id, assumed not blank. This is the {@link IPSGuid} representation in
   *     string format.
   * @return the relationship or <code>null</code> if it does not exist.
   * @throws PSErrorException
   */
  private PSRelationship getLocalRelationship(String ownerId, long widgetId, String assetId)
      throws PSErrorException {
    return getRelationship(
        PSWidgetAssetRelationshipService.LOCAL_ASSET_WIDGET_REL_FILTER, ownerId, widgetId, assetId);
  }
   ...
   * @throws PSErrorException
   */
  private PSRelationship getSharedRelationship(String ownerId, long widgetId, String assetId)
      throws PSErrorException {
    return getRelationship(
        PSWidgetAssetRelationshipService.SHARED_ASSET_WIDGET_REL_FILTER,
        ownerId,
        widgetId,
        assetId);
  }
  ...
  /**
   * Gets the local asset relationship for the specified owner, widget, and asset. Owner-revision
   * specific.
   *
   ...
   * @param widgetId
   * @param assetId the asset id, assumed not <code>null</code>. This is the {@link IPSGuid}
   *     representation in string format.
   * @return the relationship or <code>null</code> if it does not exist.
   * @throws PSErrorException
   */
  private PSRelationship getRelationship(String name, String ownerId, long widgetId, String assetId)
    ...
    filter.setProperty(
        PSWidgetAssetRelationshipService.WIDGET_ID_PROP_NAME, String.valueOf(widgetId));
  ...
  private List<PSRelationship> getLocalRelationships(String ownerId) {
    return getRelationships(
        PSWidgetAssetRelationshipService.LOCAL_ASSET_WIDGET_REL_FILTER, ownerId);
  }
  ...
  private List<PSRelationship> getSharedRelationships(String ownerId) {
    return getRelationships(
        PSWidgetAssetRelationshipService.SHARED_ASSET_WIDGET_REL_FILTER, ownerId);
  }
   ...
   * @return <code>true</code> if the asset is contained in the collection, <code>false</code>
   *     otherwise.
   */
