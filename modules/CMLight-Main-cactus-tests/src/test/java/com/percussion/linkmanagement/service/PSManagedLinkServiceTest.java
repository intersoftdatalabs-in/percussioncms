...
public class PSManagedLinkServiceTest extends PSServletTestCase {
  ...
  /**
   * Single managed link anchor, w/params: 0 - The ID of the anchor element, 1 - the href, 2 - The
   * link ID, 3 - The anchor text
   */
  private static final String MANAGED_LINK =
      "<a id=\"{0}\" href=\"{1}\" "
          + PERC_MANAGED_ATTR
          + "=\"true\" "
          + PERC_LINKID_ATTR
  ...
  private static final String MANAGED_IMG_LINK =
      "<img alt=\"{3}\" id=\"{0}\" src=\"{1}\" "
          + PERC_MANAGED_ATTR
          + "=\"true\" "
          + PERC_LINKID_ATTR
          + "=\"{2}\" />";
   ...
   * anchor text
   */
  private static final String UNMANAGED_LINK =
      "<a id=\"{0}\" href=\"{1}\" " + PERC_MANAGED_ATTR + "=\"true\" >{2}</a>";

  private static final String UNMANAGED_IMG_LINK =
      "<img alt=\"{2}\" id=\"{0}\" src=\"{1}\" " + PERC_MANAGED_ATTR + "=\"true\" />";
  ...
  public void testInvalidLinks() throws Exception {
    ...
    source =
        MessageFormat.format(
            MANAGED_LINK, anchorId, "/index.html", String.valueOf(link.getLinkId()));
    result = service.manageLinks(parentId, source);
    ...
    source =
        MessageFormat.format(
            UNMANAGED_LINK,
            anchorId,
            PSPathUtils.getFinderPath(fixture.site1.getFolderPath()),
            anchorId);
    result = service.manageLinks(parentId, source);
    ...
    source =
        MessageFormat.format(
            UNMANAGED_LINK,
            anchorId,
            PSPathUtils.getFinderPath(htmlAsset.getFolderPaths().get(0)),
            anchorId);
    result = service.manageLinks(parentId, source);
    doc = Jsoup.parseBodyFragment(result);
    el = doc.getElementById(anchorId);
  ...
  public void testManageLinks() throws Exception {
    ...
    source =
        MessageFormat.format(
            MANAGED_LINK, new Object[] {itemId, badPath, String.valueOf(linkIds.get(0)), text});
    result = service.manageLinks(parentId, source);
    validateManagedLinks(source, result, Arrays.asList(itemId));
    doc = Jsoup.parseBodyFragment(result);
    ...
    String badSrc =
        PSPathUtils.getFinderPath(imgBadPathAsset).replace("ManagedLinkTest", "ManagedLinkTest1")
            + "/"
            + imgAsset.getName();
    source =
        MessageFormat.format(
            MANAGED_IMG_LINK, new Object[] {itemId, badSrc, String.valueOf(linkIds.get(0)), text});
    result = service.manageLinks(parentId, source);
    validateManagedLinks(source, result, Arrays.asList(itemId));
    doc = Jsoup.parseBodyFragment(result);
    el = doc.getElementById(itemId);
    ...
    try {
      ...
      source =
          MessageFormat.format(
              MANAGED_LINK, new Object[] {itemId, href, String.valueOf(linkId), text});
      result = service.manageLinks(parentId, source);
      ...
      if (asset != null) assetService.delete(asset.getId());
    }
  }
  ...
  /**
   * Test {@link IPSManagedLinkService#updateCopyAssetsLinks(java.util.Collection, String, String,
   * Map)}
   *
   * @throws Exception
   */
  ...
  public void testRenderLinks() throws Exception {
    ...
    String[] values =
        new String[] {"global/percussion/contentassembler/perc_casGeneratePreviewLink", "-1"};
    req.setPrivateObject(SYS_OVERWRITE_PREVIEW_URL_GEN, values);

    // test valid page link preview
    PSRenderLinkContext previewPageLinkContext =
        renderLinkContextFactory.createPreview(fixture.getPageService().find(itemGuid.toString()));
    ...
    PSRenderLinkContext previewFileLinkContext =
        renderLinkContextFactory.createAssetPreview(fileAsset.getFolderPaths().get(0), fileAsset);
