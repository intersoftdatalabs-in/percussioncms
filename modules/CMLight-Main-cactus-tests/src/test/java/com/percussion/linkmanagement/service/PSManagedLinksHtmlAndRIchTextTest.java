...
/**
 * Tests whether a new style managed link/image in rich text asset is converted to old style link or
...
public class PSManagedLinksHtmlAndRIchTextTest extends PSServletTestCase {
  ...
  private void verifyManagedLink(Document doc, String className, boolean isOldStyle) {
    ...
    if (isOldStyle) {
      assertEquals(
          PSManagedLinksConverter.RXHYPERLINK, elem.attr(PSManagedLinksConverter.INLINETYPE));
  ...
  private PSAsset createImgAsset()
      throws IPSAssetService.PSAssetServiceException, PSValidationException {

    String fileName = "managed-image.jpg";
    InputStream in = getClass().getResourceAsStream(fileName);
    PSAbstractAssetRequest ar =
        new PSBinaryAssetRequest(
            PSAssetPathItemService.ASSET_ROOT + MANAGED_LINK_TEST,
            AssetType.IMAGE,
            fileName,
            "image/jpeg",
            in);
  ...
  private String generateTestContent() {
    ...
    String text =
        "<div>"
            + "<a perc-managed=\"true\" class=\""
            + LINK_WITH_PERC_MANAGED
            + "\" href=\""
            + pagePath
            + "\">Link With Perc Managed</a>"
            + "<a class=\""
            + LINK_WITHOUT_PERC_MANAGED
            + "\" href=\""
            + pagePath
            + "\">Link Without Perc Managed</a>"
            + "<img perc-managed=\"true\" class=\""
            + IMG_WITH_PERC_MANAGED
            + "\" src=\""
            + imgPath
            + "\"/>"
            + "<img class=\""
            + IMG_WITHOUT_PERC_MANAGED
            + "\" src=\""
            + imgPath
            + "\"/>"
            + "<a perc-managed=\"true\" class=\""
            + LINK_WITH_PERC_MANAGED_TO_RESOURCE
            + "\" href=\""
            + filePath
            + "\">Link With Perc Managed To A Resource</a>"
            + "<a class=\""
            + LINK_WITHOUT_PERC_MANAGED_TO_RESOURCE
            + "\" href=\""
            + imgPath
            + "\">Link Without Perc Managed To A Resource</a>"
            + "<a class=\""
            + EXTERNAL_LINK
            + "\" href=\""
            + EXTERNAL_LINK_URL
  ...
  private static final String LINK_WITH_PERC_MANAGED_TO_RESOURCE =
      "link-with-perc-managed-to-resource";
  private static final String LINK_WITHOUT_PERC_MANAGED_TO_RESOURCE =
      "link-without-perc-managed-to-resource";
