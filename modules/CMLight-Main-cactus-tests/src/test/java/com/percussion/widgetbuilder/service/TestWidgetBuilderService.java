...
public class TestWidgetBuilderService {
  ...
  public void setUp() throws Exception {
    ...
    // PSSecurityFilter.authenticate(request, response, "Admin", "demo"); // Not needed in JUnit5
    // context
  }
  ...
  public void testServiceCRUD() {
    ...
    try {
      ...
      assertTrue(
          definition.getFieldsList().getFields().isEmpty(), "Instantiated fields is not empty");
      ...
      if (badId > 0) deleteQuietly(badId);
  ...
  private PSWidgetBuilderDefinitionData roundTrip(
      Set<String> ids, PSWidgetBuilderDefinitionData definition) {
    var results = service.saveWidgetBuilderDefinition(definition);
  ...
  public void testPackagingandDeploy() {
    ...
    try {
      ...
      definition
          .getFieldsList()
          .setFields(PSContentTypeFileTransformerTest.setupPackageSpec().getFields());
      ...
      definition =
          service.loadWidgetDefinition(
              service.saveWidgetBuilderDefinition(definition).getDefinitionId());

      id = Long.parseLong(definition.getId());
      service.deployWidget(id);
    } finally {
      if (id > 0) service.deleteWidgetBuilderDefinition(id);
    }
  }
}
