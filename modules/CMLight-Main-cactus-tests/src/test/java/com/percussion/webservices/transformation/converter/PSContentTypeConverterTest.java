...
public class PSContentTypeConverterTest extends PSConverterTestBase {
    ...
    public void testConverter() throws Exception {
        try (var in = getClass().getResourceAsStream(
            ...
            var ce = new PSContentEditor(doc.getDocumentElement(), null, null);
            var appName = PSContentType.createAppName(ce.getName());
            var url = PSContentType.createRequestUrl(ce.getName());
            var typeDef = new PSContentType((int) ce.getContentType(),
                    ce.getName(), ce.getName(), ce.getDescription(), url, false, 1);
            var srcDef = new PSItemDefinition(appName, typeDef, ce);

            var tgtDef = (PSItemDefinition) roundTripConversion(
