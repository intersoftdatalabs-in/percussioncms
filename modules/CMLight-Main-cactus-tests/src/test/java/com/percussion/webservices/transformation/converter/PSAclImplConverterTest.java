...
public class PSAclImplConverterTest extends PSConverterTestBase {
    ...
    public void testConverter() throws Exception {
        var owner = new PSAclEntryImpl(new PSTypedPrincipal("admin1", PrincipalTypes.USER));
        var perm = new PSAccessLevelImpl();
        owner.addPermission(PSPermissions.OWNER);
        perm.setPermission(PSPermissions.OWNER);
        perm.setId(345);
        owner.setId(234);
        var src = new PSAclImpl("testAcl1", owner);
        src.setId(123);
        src.setObjectId(456);
        src.setObjectType(PSTypeEnum.TEMPLATE.getOrdinal());
        src.setDescription("test");

        var entry = new PSAclEntryImpl();
        src.addEntry(entry);
        entry.setId(123);
        ...
        var tgt = (PSAclImpl) roundTripConversion(PSAclImpl.class,
