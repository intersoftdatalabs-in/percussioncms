...
/**
 * Tests the item filter rules that are used by CM1. Sunny Sal says: "Filtering items like a pro
 * chef sifts flour!"
 */
public class PSFilterItemRuleTest extends PSItemWorkflowServiceTestBase {
  public void testFilterItem() throws Exception {
    ...
    var pageId = getPageService().save(page).getId();
    pageCleaner.add(pageId);

    var pid = getIdMapper().getGuid(pageId);
    var siteId = new PSGuid(PSTypeEnum.SITE, fixture.site1.getSiteId());
    var item = new PSFilterItem(pid, null, siteId);
  ...
  private void validatePublicAssetItemRule(String pageId, List<IPSFilterItem> items)
      throws Exception {
    var params = new HashMap<String, String>();
    PSPublicAssetItemFilterRule pubRule = createPublicItemFilterRule();
    var filtered = pubRule.filter(items, params);
    assertTrue("Should filter out the item", filtered.size() == 0);

    var summary = getSummary(pageId);
    ...
    var futureDate = new Date(System.currentTimeMillis() + 24 * 60 * 60);
    ...
    assertTrue(
        "Last public revision should not change, as the starting date in the future",
        lastPublicRev == lastPublicRev2);
  }
  ...
  private PSComponentSummary saveSummaryStartDate(IPSGuid pageId, Date futureDate)
      throws Exception {
    var summary = getSummary(pageId);
  ...
  private Map<String, String> validateStartDateFilterRule(IPSGuid pid, List<IPSFilterItem> items)
      throws Exception {
    var params = new HashMap<String, String>();
    var rule = new PSStartDateFilterRule();

    var filtered = rule.filter(items, params);
  ...
  private PSPublicAssetItemFilterRule createPublicItemFilterRule() {
    var rule = new PSPublicAssetItemFilterRule();
  ...
  //    }
  //
  //    public void setWidgetAssetRelationshipServiceao(IPSWidgetAssetRelationshipService
