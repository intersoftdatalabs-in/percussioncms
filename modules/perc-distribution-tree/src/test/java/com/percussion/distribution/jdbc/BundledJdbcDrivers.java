 ...
 *
 * <ul>
 *   <li>{@code InstallXmlDeleteSetTest} to assert the install script's {@code <delete>} block
 *       contains exactly these exact filenames (no globs).
 *   <li>{@code StagingCleanupAntScriptTest} to assert the staging {@code <copy>} block's {@code
 *       <include>} globs cover exactly the curated set, and that no non-driver provided-scope
 *       dependency is referenced.
 * </ul>
 *
 * <p>The values are pinned to the resolved filenames of the curated {@code provided}-scope JDBC
 * driver dependencies declared in {@code modules/perc-distribution-tree/pom.xml} (lines 147-181).
 * When a driver version is bumped in the parent {@code pom.xml} version properties, this constant
 * and the install script's exact-filename {@code <delete>} list must be updated in lockstep.
 *
...
final class BundledJdbcDrivers {
  ...
  /**
   * Filenames from the immediately-preceding release of this codebase. The install script's delete
   * set is the union of {@link #EXACT_FILENAMES} and this set, so an upgrade from N-1 to N purges
   * the prior version's JARs from {@code jetty/base/lib/jdbc/} (preventing duplicate-driver-version
   * classpath issues) while still preserving any integrator-supplied drivers whose names do not
   * match this union.
   *
   * <p>When a driver version is bumped in the parent {@code pom.xml}, the old filename MUST be
   * moved from {@link #EXACT_FILENAMES} to this set in the same commit, and the install script's
   * delete block must be updated in lockstep.
   */
  static final Set<String> PRIOR_FILENAMES;

  /**
   * Staging {@code <fileset>} {@code <include>} globs used by {@code installDistributionFiles.xml}
   * lines 707-717 and by {@code verify-jdbc-drivers.py} (cross-platform Python port; the canonical
   * implementation is the {@code com.percussion.distribution.install.VerifyJdbcDrivers} Java main
   * wired into the Maven {@code verify} phase at module {@code pom.xml}).
   */
  ...
  /**
   * Map from staging glob → corresponding Maven {@code artifactId}. Some artifactIds contain dashes
   * (e.g. {@code mariadb-java-client}, {@code mssql-jdbc}) so the glob→artifactId relation must be
   * declared explicitly rather than computed from the glob string.
   */
  ...
  /**
   * Returns the set of {@code artifactId}s that are bundled JDBC drivers. Used by structural tests
   * to distinguish curated driver deps from non-driver provided-scope deps.
   */
