...
public class PSCheckboxTreeModel extends DefaultTreeModel implements TreeModel {
  ...
  private void loadModel(URL baseURL, String docUrlStr) {
    ...
    if (docUrlStr == null || docUrlStr.trim().length() == 0)
      throw new IllegalArgumentException("docUrlStr cannot be null or empty");
  ...
  private URL appendPsSessionId(URL url) {
    ...
    if (url.toString().endsWith(GET_SESSIONID_URL)) return url;

    if (sUrl.indexOf("pssessionid=") != -1) return url;

    if (sUrl.indexOf('?') == -1) sUrl = sUrl + "?pssessionid=" + getPsSessionId(url);
    else sUrl = sUrl + "&pssessionid=" + getPsSessionId(url);
  ...
  private String getPsSessionId(URL baseUrl) {
    if (m_psSessionId != null) return m_psSessionId;
