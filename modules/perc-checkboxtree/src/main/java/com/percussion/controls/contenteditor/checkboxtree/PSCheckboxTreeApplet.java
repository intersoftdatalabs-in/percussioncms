...
public class PSCheckboxTreeApplet extends JApplet implements Runnable {
  ...
  public void init() {
    ...
    try {
      ...
      if (readParam.toLowerCase().startsWith("yes")) m_isReadonly = true;
      ...
      PSCheckboxTreeModel model = new PSCheckboxTreeModel(baseDocument, m_treeURL, psSessionId);
  ...
  protected Map<String, String> getExtraParameters() {
    if (m_parameters == null) m_parameters = new HashMap<>();
  ...
  private void setSelectedNodes(String selectList) {
    ...
    PSCheckboxTreeRootNode rootNode = (PSCheckboxTreeRootNode) m_tree.getModel().getRoot();

    PSCheckboxTreeNode nodewalker = (PSCheckboxTreeNode) rootNode.getNextNode();
    while (nodewalker != null) {
      if (nodewalker.isSelectable() && selectedNodes.contains(nodewalker.getNodeId())) {
  ...
  public static List<String> getTokens(String delimitedString) {
    ...
    while (tokens.hasMoreTokens()) tokenList.add(tokens.nextToken());
