...
public class PSCheckboxTreeRootNode extends DefaultMutableTreeNode implements MutableTreeNode {
  ...
  public void loadDocument(Document doc) {
    ...
    if (m_label == null)
      throw new IllegalArgumentException("invalid xml document, missing required root label");
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (childNode.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (id == null || id.trim().length() == 0)
          throw new IllegalArgumentException("invalid xml document, missing required node id");
        String label = childElement.getAttribute("label");
        if (label == null || label.trim().length() == 0)
          throw new IllegalArgumentException("invalid xml document, missing required node label");
        if (childElement.getTagName().equalsIgnoreCase("node")) {
          ...
          if (selectableAttr != null && selectableAttr.equalsIgnoreCase("yes")) {
