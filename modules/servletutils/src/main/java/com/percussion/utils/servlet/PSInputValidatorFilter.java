...
public class PSInputValidatorFilter implements Filter {
  ...
  private enum RestrictType {
    ...
    NOCONTROLCODE(
        Pattern.compile("[^\\x00\\x0a\\x0d]*")), // Do not allow control codes x00, x0a, or x0d
  ...
  private boolean handleParamValidation(HttpServletRequest request, HttpServletResponse response) {
    ...
    if (badParam == null) {
      ...
      log.warn(
          "IP: {}  Bad Parameter {} with value: {} isIllegalParameterName: {}",
  ...
  private void modifyResponse(HttpServletResponse response, ParamError badParam) {
    try {
  ...
  private void doLoadProperties(InputStream is) {
    ...
    try {
      ...
      for (Object key : props.keySet()) {
        ...
        if (vals.length > 0) restrictionProps.put((String) key, vals);
      }
    } catch (Exception e) {
  ...
  private ParamError validateParameter(String key, String value) {
    ...
    if (restrictionProps.containsKey(key)) {
      ...
      for (String restrict : restrictions) {
        ...
        if (!m.matches()) return new ParamError(key, value);
      }
    }
    return null;
  }
  ...
  private boolean isParamNameValid(String name) {
    Pattern[] pats =
        new Pattern[] {
          RestrictType.NOLTGT.getPattern(),
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (isEnabled && request instanceof HttpServletRequest) {
      ...
      if (handleParamValidation(httpReq, httpResp)) filterChain.doFilter(request, response);
    } else {
  ...
  private String getProperty(FilterConfig filterConfig, String propName, String defaultValue) {
    ...
    if (property == null) property = filterConfig.getInitParameter(propName);
    if (property == null) property = filterConfig.getServletContext().getInitParameter(propName);
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    try (InputStream is =
        this.getClass().getResourceAsStream(getClass().getSimpleName() + ".properties")) {
