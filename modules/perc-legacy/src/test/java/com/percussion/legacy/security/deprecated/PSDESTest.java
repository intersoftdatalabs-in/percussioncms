  ...
  public void testAll() throws Exception {
    ...
    for (int j = 0; j < keyBytes.length; j++) {
      for (int k = 0; k < maskArray.length; k++) {
        if (k == 0) {
          if ((keyBytes[j] | maskArray[k]) == maskArray[k]) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        } else {
          if ((keyBytes[j] & maskArray[k]) == 0) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        }
      }
    ...
    for (int i = 0; i < encodedKey.length; i++) System.out.print(encodedKey[i] + " ");
