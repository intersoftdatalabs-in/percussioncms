  ...
  public final void setSecret(byte[] secret) throws IllegalArgumentException {
    ...
    for (int j = 0; j < secret.length; j++) {
      for (int k = 0; k < maskArray.length; k++) {
        if (k == 0) {
          if ((secret[j] | maskArray[k]) == maskArray[k]) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        } else {
          if ((secret[j] & maskArray[k]) == 0) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        }
      }
    ...
    for (int i = 0; i < 7; i++) {
      ...
      temp0 =
          ((m_permutedChoiceOne[temp] << 7)
              + (m_permutedChoiceOne[temp + 1] << 6)
              + (m_permutedChoiceOne[temp + 2] << 5)
              + (m_permutedChoiceOne[temp + 3] << 4)
              + (m_permutedChoiceOne[temp + 4] << 3)
