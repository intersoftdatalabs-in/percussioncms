  ...
  private static byte[] sizeKey(byte[] keyBytes) {
    ...
    if (keyLen < KEY_LEN) {
      ...
      while (i < newLen) {
        ...
        if (remainder < len) len = remainder;
  ...
  public String encryptCredentials(String uid, String pw) {
    ...
    try {
      ...
      if ((key != null) && (key instanceof IPSSecretKey)) {
        ...
        int parttwo;
        if (uid == null || uid.equals("")) parttwo = OLD_SECURITY_KEY2().hashCode();
        ...
        try (ByteArrayOutputStream bOut = new ByteArrayOutputStream()) {
          try (ByteArrayInputStream bais = new ByteArrayInputStream(encrData)) {
            encr.encrypt(bais, bOut);
