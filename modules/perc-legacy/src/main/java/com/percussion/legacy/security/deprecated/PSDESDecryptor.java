...
public class PSDESDecryptor implements IPSDecryptor {
  ...
  private void decryptOneDataBlock(int blockIndex) {
    ...
    byte[] maskArray = {
      (byte) 0x7F, (byte) 0x40, (byte) 0x20, (byte) 0x10,
    ...
    for (int j = 0; j < eightBytes.length; j++) {
      for (int k = 0; k < maskArray.length; k++) {
        if (k == 0) {
          if ((eightBytes[j] | maskArray[k]) == maskArray[k]) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        } else {
          if ((eightBytes[j] & maskArray[k]) == 0) bitArray[element] = 0;
          else bitArray[element] = 1;
          element++;
        }
      }
  ...
  private void applySubKeys(int[] RArray, int[] LArray) {
    ...
    for (int ite = iteLimit; ite >= 1; ite--) {
      ...
      for (int i = 0; i < pArrayLen; i++) tempLArray[i] = LArray[i];
      ...
      for (int i = 0; i < pArrayLen; i++) RArray[i] = tempLArray[i];
  ...
  private void fillInB(int boxGives, int boxIndex, int[] bOneToEight)
    ...
    for (int i = 0; i < 4; i++) bOneToEight[i + shift] = fourBits[i];
  }
