...
public class PSSearchAdvancedPanel extends PSPropertyPanel implements ActionListener {
  ...
  public void updateData(boolean bDirection, PSSearch search) {
    ...
    if (bDirection) {
      ...
      if (!m_isEngineAvailable && m_isDBCaseSensitive) {
        boolean caseSensitive = (m_caseSensitiveCheckBox.isSelected() ? true : false);
      ...
      if (m_isEngineAvailable) {
        ...
        if (synExpansion != null) selected = synExpansion.equals(BOOL_YES);
  ...
  public static String getSynonymExpansion(PSSearch search) {
    ...
    if ((synonymExpansion == null)
        || !(synonymExpansion.equals(BOOL_YES) || synonymExpansion.equals(BOOL_NO))) return BOOL_NO;
    else return synonymExpansion;
  }
