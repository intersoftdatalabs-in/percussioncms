...
public final class UTFixedCharTextField extends JTextField implements Serializable {
  ...
  public void setError(String title, String message, Component parent) {
    ...
    if (message.length() == 0) throw new IllegalArgumentException("message cannot be empty");
  ...
  public boolean isValidMnemonic(String label) {
    if (getText().length() > 0) {
      ...
      if (normLabel.indexOf(normText.charAt(0)) == -1) return false;
    }
      ...
      if ((str.length() + this.getLength()) > mi_maxChars) {
        ...
        if (m_errorTitle != null && m_errorMessage != null) {
          JOptionPane.showMessageDialog(
              null, m_errorMessage, m_errorTitle, JOptionPane.ERROR_MESSAGE);
        }
      } else {
