...
public class PSMenu extends JMenu {
  ...
  public JMenuItem add(Action a) {
    ...
    if (a instanceof PSAction) newItem = insert(a, getItemCount());
    else newItem = super.add(a);
    return (newItem);
  }
  ...
  private JMenuItem decorateMenuItem(JMenuItem item, PSAction action) {
    ...
    if (0 != cMnemonic) item.setMnemonic(cMnemonic);

    KeyStroke ks = action.getAccelerator();
    if (null != ks) item.setAccelerator(ks);
    item.setIcon(action.getIcon());

    String strTTText = action.getToolTipText();
    if (strTTText.length() > 0) item.setToolTipText(strTTText);
    return (item);
  }
}
