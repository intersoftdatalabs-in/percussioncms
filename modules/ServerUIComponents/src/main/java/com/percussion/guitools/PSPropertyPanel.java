...
/**
 * The utility class to create a panel with controls to set properties. Uses {@link
 * com.percussion.layout.PSGridBoxLayout PSGridBoxLayout} for laying out the components. Basically
 * this panel will have 2 columns with the left column for label and right column for the control to
...
public class PSPropertyPanel extends JPanel implements IPSPropertyPanel {
  ...
  public void addPropertyRow(
    ...
    addPropertyRow(name, new JComponent[] {control}, mnControl, mnemonic, tooltip);
  }
  ...
  public void addPropertyRow(
    ...
    if (controls == null || controls.length == 0)
      throw new IllegalArgumentException("controls may not be null or empty.");
    ...
    if (mnemonicControl != null && mnemonic != 0) {
      ...
      else label.setLabelFor(mnemonicControl);
      if (tooltip != null && tooltip.trim().length() > 0) mnemonicControl.setToolTipText(tooltip);
  ...
  public void addControlsRow(JComponent leftControl, JComponent rightControl) {
    List rowComps = new ArrayList();
    if (leftControl != null) addControl(leftControl, RIGHT_ALIGNMENT, rowComps);
    else {
    ...
    if (rightControl != null) addControl(rightControl, LEFT_ALIGNMENT, rowComps);
    else {
      Component comp = Box.createGlue();
      add(comp);
      rowComps.add(comp);
  ...
  /**
   * Adds the control to this panel with the specified x-alignment. The controls will not grow in
   * vertical direction when the panel is resized. The controls which takes up an area to display
   * are wrapped in scroll pane to be able to see data.
   *
   * @param control the control to add, assumed not to be <code>null</code>
   * @param xAlignment the x-alignment, assumed to be one of <code>RIGHT_ALIGNMENT</code>, <code>
   *     LEFT_ALIGNMENT</code>, <code>
   * CENTER_ALIGNMENT</code> values.
   * @param rowComps List of row components to which any new controls are added, assumed not <code>
   *     null</code>.
   */
  ...
  /**
   * Supports only <code>PSGridBoxLayout</code> with 2 columns and unlimited number of rows. All
   * other layouts are ignored.
   *
  ...
  public void setLayout(LayoutManager mgr) {
    if (mgr instanceof PSGridBoxLayout) {
      ...
      if (layout.getRows() == 0 && layout.getColumns() == 2) super.setLayout(layout);
    }
  }
  ...
  public void hideRow(int index) {
    if (!isValidRowIndex(index)) throw new IllegalArgumentException("Invalid index");
   ...
   * @return <code>true</code> if the index is must be less than <code>m_rowList.size()</code> and
  ...
  /**
   * Sets the <code>isVisible()</code> property on the row components at the specified index.
   *
   * @param index The index of the row to modify, assumed to be a valid row index (see {@link
   *     #isValidRowIndex(int)}).
   * @param isVisible <code>true</code> to set the components visible, <code>false</code> to hide
   *     them.
   */
  private void setRowCompsVisible(int index, boolean isVisible) {
    ...
    while (comps.hasNext()) ((Component) comps.next()).setVisible(isVisible);
  }

  /**
   * Gets index of the first row found with a <code>JLabel</code> that has text matching the
   * supplied <code>label</code>.
   *
   * @param label The text to match, assumed not <code>null</code> or empty.
   * @return The index of the matching row, or <code>-1</code> if no match is found.
   */
   ...
   * Each entry is never <code>null</code>, may be empty.
   */
