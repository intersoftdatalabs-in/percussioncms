...
/**
 * The Base class for all future e2 dialogs. PSDialog has a ValidationFramework for validating
 * programmer specified components. This is to make sure that users do not enter incorrect values
 * for that component.
 *
...
public class PSDialog extends JDialog {
  ...
  /**
   * The basic dialog is modal, non-resizable. If the properties file has an key called "title", its
   * value is read in and the title of the dialog is set to this value.
   */
  public PSDialog() {
    super();
    init(true);
  ...
  /**
   * Starts the validation process. Framework MUST be instantiated prior to this call. Typically
   * used in a ActionListener method in response to an action generated by a button.
   *
   ...
   * SwingConstants.VERTICAL</code> or <code>SwingConstants.HORIZONTAL</code> constants.
   * @param showHelp if <code>true</code> adds the 'Help' button to the panel, otherwise not.
  ...
  protected JPanel createCommandPanel(int axis, boolean showHelp) {
    ...
    UTStandardCommandPanel commandPanel = new UTStandardCommandPanel(this, axis, showHelp);
  ...
  /**
   * If the supplied table is currently being edited this stops the edit mode to make sure that the
   * changes of the currently edited cell are accepted.
   *
   * @param table the table to stop the edit mode for, may be <code>null</code> in which case this
   *     does nothing.
   */
  ...
  /**
   * The action performed by the Ok button. This is to be overridden by subclasses that implements
   * the actual functionality. Dialog closing action must be handled here as well (ie: dispose(); ).
   */
  public void onOk() {
    m_bOk = true;
    setVisible(false);
  ...
  /**
   * The action performed by the Apply button. This is to be overridden by subclasses that
   * implements the actual functionality.
   */
  public void onApply() {}

  /**
   * Finds out whether OK is pressed in the dialog. Should be called after dialog is disposed.
   *
   * @return <code>true</code> if the dialog is disposed by clicking 'OK', otherwise <code>false
   *     </code>.
   */
  public boolean isOk() {
    return m_bOk;
  ...
  /**
   * The action performed by the Cancel/Close button. This is to be overridden by subclasses that
  ...
  /**
   * The action performed by the Help button. If the child dialog does not have help, override
   * <CODE>onHelp</CODE> and do nothing. Launches JavaHelp viewer to display the help.
   */
  public void onHelp() {
    if (!m_useHelpViewer) return;
    String fullname = getClass().getName();
    if (displayHelpUsingEclipseHelp(subclassHelpId(fullname))) return;
  ...
  /**
   * We use reflection to attempt to load the eclipse workbench's help manager class to see if it
   * exists. If so we use the displayHelpFromLegacy method to show help via Eclipse's help system.
   * If we fail we return false so that the help call can be handled by regular JavaHelp. We use
   * reflection so that there is no dependency on the Eclipse codebase at compile time so we can run
   * this code in an applet which can't use SWT.
   *
   * @param key the help key string, may be <code>null</code>.
   * @return <code>true</code> if help was shown via Eclipse's help system, else <code>false</code>.
   */
  ...
  /**
   * By default, the base class name is used as the help id. However, if a dialog needs to support
   * more than 1 help page, then it should modify the id. Typically, an integer will be appended to
   * differentiate among the multiple help pages associated with a single dialog.
   *
   * @param helpId The key to use in the help id mapping resource. This must always be a valid,
   *     non-empty string.
  ...
  protected ResourceBundle getResources() {
    try {
      if (null == m_res) m_res = ResourceBundle.getBundle(getResourceName(), Locale.getDefault());
  ...
  protected String getResourceString(String key) {
    ...
    try {
      if (getResources() != null) resourceValue = m_res.getString(key);
  ...
  /**
   * Overridden method to display the dialog only if it is properly initialized. Also adds the
   * default listeners as component initialization is guaranteed to be complete.
   */
  @Override
  ...
  /**
   * Creates a compound border used on panels that group controls. The outer border is a titled
   * border. The inner border is an empty border. The empty border is added because insufficient
   * space is left between components and the line of the titled border. This is a utility method
   * for derived classes, it is not used by this class.
   *
   * @param title The title for the border. <code>null</code> is the same as the empty string.
   */
  ...
  /**
   * Creates a box set up with a label that is right justified, with glue on the left side. The
   * supplied component is used as the component that this label is being used for.
   *
  ...
  public static Box createLabel(String text, JComponent labelFor) {
    ...
    if (null != labelFor) label.setLabelFor(labelFor);
  ...
  protected void addDefaultKeyListeners(Component comp) {
    ...
    if (comp instanceof Container && ((Container) comp).getComponentCount() > 0) {
      ...
      for (int i = 0; i < children.length; i++) addDefaultKeyListeners(children[i]);
    }
   ...
   *
   * @param bSetTitle if <code>true</code>, tries to load the title from the resource bundle using
   *     the key "title". If found, it then sets the title to this value.
   */
  private void init(boolean bSetTitle) {
    ...
    m_defaultKeyListener =
        new KeyAdapter() {
          @Override
          public void keyReleased(KeyEvent e) {
            if (KeyEvent.VK_ESCAPE == e.getKeyCode()) onCancel();
            if (KeyEvent.VK_F1 == e.getKeyCode()) onHelp();
          }
        };

    try {
      if (bSetTitle) {
        if (getResources() != null) setTitle(getResources().getString("title"));
      }
    } catch (MissingResourceException e) {
  ...
  /**
   * Flag to indicate this dialog initialization. Initialized to <code>true</code> and set to <code>
   * false</code> when error happens in derived class initialization.
   */
  protected boolean m_bInitialized = true;

  /**
   * Key listener that listens for escape and f1 key presses. If escape is caught then onCancel is
   * called if f1 is caught then onHelp is called. Initialized in {@link #init(boolean)}
   */
  ...
  /**
   * The flag to indicate whether the dialog is disposed by clicking 'OK' or not. Initialized to
   * <code>false</code> and set to <code>true</code> before disposing the dialog in <code>onOk()
   * </code>
   */
  protected boolean m_bOk;

  /**
   * Use help viewer. This is exposed as some newer subclasses do not want to use the old help
   * viewer. Defaults to <code>true</code>.
   */
