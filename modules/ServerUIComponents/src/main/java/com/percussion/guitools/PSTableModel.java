...
public abstract class PSTableModel extends DefaultTableModel implements IPSTableModel {
  ...
  public void setDescription(int row, String desc) {
    throw new UnsupportedOperationException("description can not be modified");
  }
  ...
  public void removeRows(int[] rows) {
    ...
    } else throw new UnsupportedOperationException("model does not allow removing rows");
  }
  ...
  public void moveRow(int start, int end, int dest) {
    if (!allowMove()) throw new UnsupportedOperationException("rows are not allowed to move");
    ...
    if (dest < 0 || dest > getRowCount())
      throw new IndexOutOfBoundsException(
          "dest can not be less than 0 or more than row count (" + getRowCount() + ")");

    if (start > end) throw new IndexOutOfBoundsException("start can not be greater than end");

    if (start <= dest && dest <= end)
      throw new IndexOutOfBoundsException("dest can not be in between start and end (inclusive)");
     ...
     */
    if (dest > end) insertPos = dest - (end - start + 1);
  ...
  protected void checkRow(int row) {
    if (row < 0 || row >= getRowCount())
      throw new IndexOutOfBoundsException(
          "row must be between 0 and " + (getRowCount() - 1) + "inclusive");
  }
  ...
  protected void checkColumn(int col) {
    if (col < 0 || col >= getColumnCount())
      throw new IndexOutOfBoundsException(
          "col must be between 0 and " + (getColumnCount() - 1) + "inclusive");
  }
}
