...
public class ErrorDialogs {
   ...
   */
  public static void showErrorDialog(
      Component parent, String errorBody, String errorTitle, int type) {
      ...
      if (m_res != null) {
        ...
        displayText =
            "An exception occurred, but no text is available. "
                + "The exception class was: "
                + e.getClass().getName();
      }
    } else displayText = displayText.trim();

    if (bPrintCallStack) e.printStackTrace();
  ...
  public static void FatalError(String strMsg) {
    ...
    if (null == strFormat)
      strFormat =
          "A fatal exception has occurred with the following "
              + "message:\n {0}. The program cannot continue.";
    if (null == strFatalDlgTitle) strFatalDlgTitle = "Fatal Error";
  ...
  public static void showErrorMessage(Window parent, String msg, String title) {
    if (null == title) {
      ...
      else title = "Exception Caught";
    }
    if (msg.indexOf(HTML_OPEN_TAG) > -1 && msg.indexOf(HTML_OPEN_TAG) < msg.indexOf(HTML_CLOSE_TAG))
  ...
  public static String cropErrorMessage(String msg) {
    ...
    for (String line : lines) {
      ...
      StringTokenizer tokenString = new StringTokenizer(line, new String(" "), true);
      int lineCount = 0;
      ...
      while (tokenString.hasMoreTokens()) {
        ...
        if (80 > lineCount) errorMsg = errorMsg + s;
        else {
