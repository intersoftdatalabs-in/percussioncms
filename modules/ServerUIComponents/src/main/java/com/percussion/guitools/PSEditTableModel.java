...
public abstract class PSEditTableModel implements TableModel {
   ...
   */
  protected PSEditTableModel(PSCollection rows) throws IllegalArgumentException {
    if (rows != null) {
      m_rows = rows;
    } else {
  ...
  private void checkRange(int index) {
    ...
    if (index >= m_rows.size()) {
      throw new IllegalArgumentException("Index on model may not exceed size: " + m_rows.size());
    }
  }
  ...
  public void removeRow(int index) {
    ...
    TableModelEvent event =
        new TableModelEvent(
            this, index, m_rows.size(), TableModelEvent.ALL_COLUMNS, TableModelEvent.DELETE);
    m_rows.remove(index);
    notifyListeners(event);
  }
