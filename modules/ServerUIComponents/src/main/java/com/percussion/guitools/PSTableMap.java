...
/**
 * This class provides most of the common behaviour of a table model by routing all requests to its
 * internal table model that is set, and implements the <code>TableModelListener</code> by routing
...
public class PSTableMap extends AbstractTableModel implements TableModelListener {
  ...
  /**
   * Sets the internal table model used to route the requests came to this model. Removes itself as
   * the listener from previous model and sets itself as the listener to this new model.
   *
   * @param model the model may not be <code>null</code>
   * @throws IllegalArgumentException if any model is <code>null</code>
   */
   ...
   * @param col the column index of value to get, must be >= 0 and less than {@link
   *     #getColumnCount() columncount} of this model.
   * @return the column name, depends on the internal table model implementation, generally never
  ...
  /**
   * Returns the most specific superclass for all the cell values in the column of the internal
   * model.
   *
   * @param col the column index of value to get, must be >= 0 and less than {@link
   *     #getColumnCount() columncount} of this model.
   * @return the column class, may be <code>null</code> if the column cells has <code>null</code>
   *     data.
   * @throws IllegalArgumentException if column index is not valid.
   */
  ...
  /**
   * Finds out whether the cell specified by row and column is editable or not. Delegates the call
   * to internal model.
   *
   * @param row the row index of value to get, must be >= 0 and less than {@link #getRowCount()
   *     rowcount} of this model.
  ...
  /**
   * The table model which represents the data model to which the requests need to be routed,
   * initialized in the constructor and never <code>null</code> after that. May be modified through
   * <code>setModel(TableModel)</code>.
   */
