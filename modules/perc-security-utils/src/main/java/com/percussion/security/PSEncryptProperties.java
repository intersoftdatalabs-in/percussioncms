...
public class PSEncryptProperties {
  ...
  private static final Logger log = LogManager.getLogger(PSEncryptProperties.class.getName());
  ...
  /**
   * Encrypts all properties if they have not already been encrypted. The specified file is
   * overwritten with the encrypted values. To check if the file has been encrypted, we can look for
   * string with prefix <code>ENC(hash)</code>.
   *
    ...
    try (InputStream is = new FileInputStream(propsFile);
      ...
      for (String prop : propNames) {
        ...
        if (!isEncrypted(propValue) && StringUtils.length(propValue) > 0) {
          ...
          sb.append(PREFIX);
          sb.append(
              PSEncryptor.encryptProperty(secureDir, propsFile.getAbsolutePath(), prop, propValue));
          sb.append(SUFFIX);
  ...
  /**
   * This method unwraps and returns the decrypted value from within the suffix and prefix of the
   * original string. The properties file should be encrypted prior to reading the properties file
   * in which case the string should always be encrypted first. In the event that the properties
   * file is read prior to encryption, the string is assumed to be unencrypted and returned as is.
   *
   * @param str the string to decode.
   * @param key the key used for original encryption.
   * @param secureDir the path to the secure folder
   * @param legacyEncryptor The encryptor to use to handle upgraded and changed encryption
   *     algorithms.
  ...
  /**
   * Attempts to decrypt a string using the latest PSEncryptor. If this fails will try with the
   * legacy encryptor.
   *
  ...
  private static void writeProperties(File propsFile, File tempFile) {
    log.debug(
        "Writing the properties file with encrypted properties: {}", propsFile.getAbsolutePath());
