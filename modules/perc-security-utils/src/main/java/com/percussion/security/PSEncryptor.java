 ...
 *
 * <p>When initialized will generate an instance unique encryption key in the directory specified by
 * the install location /rxconfig/secure/.key if one is not present.
 */
public class PSEncryptor extends PSAbstractEncryptor {
  ...
  private static Map<String, List<String>> xmlFilesWithPassword =
      new HashMap<String, List<String>>();
  ...
  private byte[] generateNewKeyFile(
      IPSKey key, Path secureKeyFile, byte[] secureKey, boolean notifyListeners) {
    ...
    try {
      ...
      log.error(
          "Error writing instance secure key file: ("
   ...
   *
   * @return Returns an IPSKey instance with the secureKey assigned. May return null if Key loading
   *     or generation fails.
   */
  private IPSKey loadKey(String algorithm, String keyLocation) {

    if (algorithm == null) algorithm = PSEncryptionKeyFactory.AES_GCM_ALGORIYTHM;
  ...
  private void init(String algorithm, String keystoreLocation) {
    ...
    if (!keystoreLocation.endsWith(File.separator)) keystoreLocation.concat(File.separator);

    this.secureDir = keystoreLocation;
  }

  /**
   * Gets an instance of the Encryptor for decrypting and encrypting data. A new key will be
   * generated in the specified location if an existing key is not found.
   *
   * @param algorithm Currently on AES is supported
   * @param keyLocation The location to store the encryption key. Example:
   *     InstallDir/rxconfig/secure or user.home.dir/.perc-secure
   ...
   * @param password The secret key to encrypt the string, may not be <code>null</code> or empty.
  ...
  /**
   * The string to encrypt, returns a Base64 encoded version of the String. NOTE: Base64 may not be
   * safe encoding for all data formats.
   *
   * <p>Use the hex method to hex encode for formats like JSON.
   *
   ...
   *     empty.
   * @param legacyDecryptor The legacy decryptor to use for decryption if decryption fails. Used to
   *     handle encryption upgrades.
  ...
  public String encryptCredentials(String uid, String pw)
    ...
    if (pw == null) throw new IllegalArgumentException("pw is required");

    return new String(
        Base64.getEncoder()
            .encode(secretKey.getEncryptor().encryptWithPassword(uid + ":" + pw, pw)),
  ...
  public String decryptCredentials(String encrypted, String pw)
      throws PSEncryptionException, IllegalArgumentException {
    if (encrypted == null) throw new IllegalArgumentException("encrypted is required");
  ...
  /**
   * Utility method for getting the password portion of a credential string in the uid:password
   * format
   *
   * @param credentials a non null plain text string in uid:password format
   * @return the password
  ...
  private byte[] getLegacyKeyFile() throws PSEncryptionException {
    ...
    try {
      try (InputStream is =
          this.getClass().getResourceAsStream("/com/percussion/security/encryption/.legacy-key")) {
