...
/**
 * Mojo to generate OpenAPI specification from JAX-RS annotations in REST module.
 * Scans REST resource classes for @Path and HTTP method annotations.
 */
@Mojo(
    name = "generate-spec",
...
public class OpenAPIGeneratorMojo extends AbstractMojo {
    ...
    /**
     * Path to the REST module JAR file to scan for JAX-RS annotations.
     * Can be a file path or Maven artifact reference.
     */
    ...
    private void scanRESTResources(OpenAPI openapi) throws Exception {
        ...
        try {
            ...
            Set<Class<?>> resourceClasses = reflections.getTypesAnnotatedWith(jakarta.ws.rs.Path.class, true);
    ...
    private void processResourceClass(OpenAPI openapi, Class<?> resourceClass) {
        ...
        getLog().debug("Processing resource class: " + resourceClass.getName() + " at path: " + basePath);
    ...
    private Operation createOperation(Method method) {
        ...
        operation.setDescription("REST endpoint: " + method.getDeclaringClass().getSimpleName() + "." + method.getName());
    ...
    private File getRESTModuleFile() {
        ...
        getLog().debug("Searching for REST module in " + project.getArtifacts().size() + " artifacts...");
        for (Object artifact : project.getArtifacts()) {
            org.apache.maven.artifact.Artifact mvnArtifact = (org.apache.maven.artifact.Artifact) artifact;
    ...
    private File findRESTModuleJar() {
        getLog().info("Searching for REST module in " + project.getArtifacts().size() + " artifacts...");
        for (Object artifact : project.getArtifacts()) {
            org.apache.maven.artifact.Artifact mvnArtifact = (org.apache.maven.artifact.Artifact) artifact;
            getLog().debug("  - " + mvnArtifact.getArtifactId() + ":" + mvnArtifact.getType());
            if ("rest".equals(mvnArtifact.getArtifactId()) && "jar".equals(mvnArtifact.getType())) {
