...
public class PSContextMenu implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    String rxAppResource = request.getCurrentApplicationName() + "/" + REQUEST_NAME;
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (actionid.length() < 1) continue;

      String modeid = elem.getAttribute(ATTR_MODEID).trim();
      if (modeid.length() < 1) continue;

      String uicontextid = elem.getAttribute(ATTR_UICONTEXTID).trim();
      if (uicontextid.length() < 1) continue;

      itemsRendered.add(actionid);
      try {
        processItem(itemsRendered, elem, request, rxAppResource);
      } catch (Exception e) {
  ...
  private void processItem(
    ...
    try {
      ...
      elemRes = (Element) parent.getOwnerDocument().importNode(doc.getDocumentElement(), true);

      if (!hasActionChildren(elemRes)) return;
      ...
      if (!parent.getNodeName().equals(ELEM_ACTION_LIST)) {
        ...
        // element.
        Element old = (Element) parent.getParentNode().replaceChild(elemRes, parent);
        ...
        for (int i = 0; i < count; i++) elemRes.insertBefore(nl.item(0), firstChildAction);
      } else {
        // this is a child action, so just append it's child actions
        nl = elemRes.getChildNodes();
        int count = (nl == null) ? 0 : nl.getLength();
        for (int i = 0; i < count; i++) parent.appendChild(nl.item(0));
      }
  ...
  private boolean hasActionChildren(Element actionlist) {
    boolean res = false;
    if (actionlist == null) return res;

    NodeList nl = actionlist.getElementsByTagName("*");
    if (nl == null || nl.getLength() < 1) return res;
