  ...
  public Document processResultDocument(
    ...
    try {
      ...
      while (tokenizer.hasMoreTokens()) {
        ...
        if (cid == null || cid.length() < 1) continue;

        // avoid processing duplicates
        if (cidMap.containsKey(cid)) continue;

        String newCid =
            PSSingleValueBuilder.getCorrectedContentId(request, cid, null, inlineLinkRelData, null);
        if (newCid == null) newCid = cid;
