...
public class PSCleanupCrossSiteLiniks extends PSDefaultExtension
  ...
  private void processFolder(PSLocator folder) throws PSCmsException {
    m_processedFolder++;
    logMessage("Processing (" + m_processedFolder + ") folder id=" + folder.getId() + "  ...");
  ...
  private void processItem(PSComponentSummary item) throws PSCmsException {
    m_processedItem++;
    logMessage("Processing (" + m_processedItem + ") item id=" + item.getContentId() + "  ...");
  ...
  private void handleSiteAndFolderId(PSRelationship rel) throws PSCmsException {
    ...
    if (origSiteId == m_tgtSite.getId()) {
      ...
      if (!(isFolderDescendent(origSiteRoot, origFolder)
          && isChildItem(origFolder, rel.getDependent()))) {
        handleSiteId(rel);
        handleFolderId(rel);
      }
    }
    ...
    if (origSiteId == m_tgtSite.getId()) {
      if (isUnderTargetSite(rel.getDependent())) {
        if (m_isRemoveSiteId) {
          ...
          logMessage(
              "Remove the unnecessary site id="
                  + origSiteId
    ...
    } else // sys_siteid != target site id
    {
      ...
      if (isUnderTargetSite(rel.getDependent())) {
        if (m_isRemoveSiteId) {
          ...
          logMessage(
              "Remove the incorrect and unnecessary site id="
                  + origSiteId
                  + " from relationship rid="
                  + rel.getId());
        } else {
          rel.setProperty(IPSHtmlParameters.SYS_SITEID, String.valueOf(m_tgtSite.getId()));
          saveRelationship(rel);

          logMessage(
              "Replaced the incorrect site id="
                  + origSiteId
                  + " with the target site id="
                  + m_tgtSite.getId()
                  + " for relationship rid="
                  + rel.getId());
        }
  ...
  private List<PSLocator> getFolderRelationships(PSLocator locator, boolean isOwner)
    ...
    if (isOwner) filter.setOwner(locator);
  ...
  private void handleLinkWithMoreFolders(PSRelationship rel) throws PSCmsException {
    ...
    if (tgtFolder != null) {
      logMessage(
          "Replace the incorrect folder id="
              + origFolderId
              + " with id=\""
              + tgtFolder.getId()
              + "\" for relationship rid="
      ...
      rel.setProperty(IPSHtmlParameters.SYS_FOLDERID, String.valueOf(tgtFolder.getId()));
      saveRelationship(rel);
    } else {
      logErrorMessage(
          "Cannot fix the incorrect folder id="
              + origFolderId
              + " from relationship rid="
              + rel.getId()
  ...
  private int getPropertyInt(PSRelationship rel, String propname) {
    ...
    if (value == null || value.trim().length() == 0) {
      throw new IllegalArgumentException(
          "the value of property (" + propname + ") must not be null or empty.");
    }
  ...
  private void handleLinkWithOneFolder(PSRelationship rel, PSLocator folderLoc)
    ...
    if (isUnderTargetSite(folderLoc)) {
      ...
      if (m_isRemoveFolderId) {
        logMessage(
            "Remove the unnecessary folder id="
                + origFolderId
                + " from relationship rid="
                + rel.getId());
      } else {
    ...
    } else // the actual parent folder of the dependent is under different site.
    {
      logErrorMessage(
          "Cannot fix the incorrect folder id="
              ...
              + " is under folder id="
              + folderLoc.getId()
              + ", which is not under the target site.");
    }
  }
  ...
  private void logErrorMessage(String msg, Exception e) {
    if (e != null) {
      m_request.printTraceMessage("ERROR: " + msg + " caught exception: " + e.toString());
    } else {
  ...
  private void validateCurrentState(IPSRequestContext request) {
    if (ms_isRunning) {
      throw new IllegalStateException("Cannot invoke more than one instance of the Cleanup Exit.");
    }

    if (!request.isTraceEnabled()) {
      throw new IllegalStateException("The trace must be enabled for the current application.");
    }
  }
   ...
   * @throws PSCmsException
   */
  private void readParameters(Object[] params, IPSRequestContext request) throws PSCmsException {
  ...
  private PSSite getSite(Object[] params, int index, boolean isRequired) {
    ...
    if (sSiteId == null || sSiteId.trim().length() == 0) {
      if (isRequired) {
        throw new IllegalArgumentException(
            "The site id of the " + index + " parameter must not be null or empty.");
      } else {
        return null;
      }
    }
  ...
  private PSLocator getRootLocator(PSSite site) throws PSCmsException {
    ...
    if (id == -1) {
      logWarningMessage(
          "Cannot find the root folder for site (id="
              + site.getId()
              + ", rootpath=\""
              + site.getFolderRoot()
              + "\").");
