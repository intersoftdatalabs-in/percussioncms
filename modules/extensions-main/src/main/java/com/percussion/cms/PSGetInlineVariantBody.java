    ...
    if (StringUtils.isNotBlank(inlinetype) && inlinetype.equals("rxvariant")) {
      if (StringUtils.isNotBlank(docurl)) {
        try {
          ...
          String inlineText = root.getAttribute(PSSingleValueBuilder.INLINE_TEXT);
          if (StringUtils.isNotBlank(inlineText))
            docurl =
                docurl
                    + "&"
                    + PSSingleValueBuilder.INLINE_TEXT
                    + "="
                    + Codecs.URLEncode(inlineText);
          String doc = PSAssemblyServiceUtils.getAssembledDocument(docurl, null);
          ...
          if (tokenizeScriptTags) {
            ...
            body = StringUtils.replace(body, SCRIPT_END, SCRIPT_END_TOKEN);
            body = StringUtils.replace(body, SCRIPT_START, SCRIPT_START_TOKEN);
          }
