...
public class PSStringTrimmerUdf extends PSSimpleJavaUdfExtension {
  ...
  /**
   * Strips leading and trailing white space from the supplied string. It does this by calling
   * <code>toString()</code> on the supplied object first.
   *
   * @param params A single parameter that will be converted to a String with the toString method,
   *     then trim() is called on this string before it is returned. If <code>null</code> is
   *     supplied, <code>null</code> is returned.
   * @param request Not used.
   * @return A copy of the supplied string with all leading and trailing white space removed.
