...
/**
 * This class is a Rhythmyx ResultDocumentProcessor extension. It is used to reformat a list of file
 * path names contained in an XML result tree into a true tree structure. This will be used to
 * display file lists in the Rhythmyx Content Manager User Interface. See details in {@link
 * PSFormatFileTree#processResultDocument processResultDocument}. This extension may have no value
 * outside of the Content Manager UI.
 *
 * @author David Benua
 */
public class PSFormatFileTree extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public org.w3c.dom.Document processResultDocument(
      Object[] params, IPSRequestContext request, org.w3c.dom.Document resultDoc)
    ...
    if (params.length >= 4 && null != params[3] && params[3].toString().trim().length() > 0) {
  ...
  /**
   * This method modifies all &lt;link&gt; elements in the document in that for each element it
   * concatenates the base Url that is taken as the "baseurl" attribute of the element with the
   * element value and replaces the element value.
   *
   * @param resultDoc result document to midify the link elements, assumed not <code>null</code>.
   */
   ...
   *
   * @param resultDoc the org.w3c.dom.Document to scan for &lt;filelist&gt; nodes
  ...
  private LinkedList BuildNodeList(Document resultDoc, ParamTracker pt) {
    ...
    Element fileElement =
        filelistWalker.getNextElement(
            pt.getFileElementName(), PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
    while (null != fileElement) {
      FileElement fe = new FileElement(fileElement, pt.getFilePathName());
      nodeList.add(fe);
      fileElement =
          filelistWalker.getNextElement(
              pt.getFileElementName(), PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
    }
    pt.printTraceMessage("BuildNodeList: nodes=" + String.valueOf(nodeList.size()));
    return nodeList;
  }

  /**
   * ProcessNodeList scans the node list looking for common &lt;path&gt; nodes.
   *
   * <p>As each node is scanned, the following possible cases are examined:
   *
   * <ul>
   *   <li>A file name is encountered, and a &lt;file&gt; element is written
   *   <li>A directory name is encountered that is the first name, or is the same as the previous
   *       directory name. In this case, the element is added to the pending list.
   *   <li>A directory name is encountered which is different from the previous one. In this case,
   *       the previously pending nodes are processed, and then a new list is started.
   * </ul>
   *
   * <p>After the entire nodes list has processed, any nodes in the pending list handled by calling
   * ProcessNodeList recursively.
   *
   * @param nodeList a List of Nodes to process
   * @param parentNode the org.w3c.dom.Node which is the parent for the &lt;path&gt; nodes.
   * @param level the of the parent. For the &lt;filetree&gt; this will be 0. For the first
   *     &lt;path&gt; node, the level is 1.
  ...
  /**
   * The FileElement inner class represents each &lt;file&gt; element, complete with pathname and
   * XML element. The element will be copied into the &lt;filelist&gt;. T <br>
   * The pathname uses a StringTokenizer to return each subsequent directory name.
   */
  private class FileElement {
     ...
     *
     * @param parent the parent element where the original element will be copied.
  ...
  /**
   * used to hide the IPSRequestContext so that tracing can be done at by internal methods while
   * still allowing a main() method.
   */
