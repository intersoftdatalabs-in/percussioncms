...
public class PSModifySiteDefs extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    Element root = PSXmlDocumentBuilder.createRoot(doc, "ModifySiteDefsResults");
    ...
    try {
      ...
      if (origPath == null || origPath.trim().length() == 0) {
        ...
        throw new Exception(
            "The original folder path is missing. "
      ...
      for (IPSSite site : smgr.loadSitesModifiable()) {
        ...
        if (existingRoot != null) existingRoot = existingRoot.replace('\\', '/');
        if (existingRoot == null
            || existingRoot.trim().length() == 0
