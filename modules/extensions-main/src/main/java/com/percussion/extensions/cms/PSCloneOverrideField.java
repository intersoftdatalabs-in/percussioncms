...
public class PSCloneOverrideField extends PSSimpleJavaUdfExtension {
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    if (loc > 0) resource = resource.substring(0, loc);
    ...
    for (int i = 2; i < params.length; i = i + 2) {
      ...
      String value = "";
      if (params.length > i + 1) value = params[i + 1].toString();
    ...
    if (nl.getLength() > 0) {
      ...
      if (node.getNodeType() == Node.TEXT_NODE) return ((Text) node).getData().trim();
    }
    throw new PSConversionException(request.getUserLocale(), 1000); // TODO msg code
  }
}
