...
public class PSGetFileSize extends PSSimpleJavaUdfExtension {
   ...
   * the results and returns it. The URL server and port are set to the Rhythmyx server's address
   * and port.
   *
   ...
   * relative from the orginiating request's app root.
   *
   * <p>If the supplied URL is fully qualified and the protocol is not 'http', or 'https', an empty
   * String is returned. Otherwise, any pieces supplied will be substituted. If the supplied URL is
   * not fully qualified, the missing parts will be added. For a partially qualified name, the http
   * protocol, server and port will be added to the supplied name. For an unqualified name, these
   * items, plus the Rhythmyx request root and the originating application request root will be
   * added. For a relative name, the http protocol, server, port, and Rhythmyx root will be added,
   * assuming it is relative from the originating requests app root. For an empty string, the app
   * root of the originating request will be used, substituting the supplied parameters. Regardless
   * of whether the originating request is using https, or if the supplied URL is fully qualified
   * and specifies 'https', the URL used to make the internal request will always use 'http'.
   *
   * <p>Multiple name/value pairs may be specified for the parameters. For example, if the following
   * were supplied as parameters:
   *
   * <ul>
   *   <li>resource = query1.html
   *   <li>param1 = city
   *   <li>value1 = Boston
   *   <li>param2 = state
   *   <li>value2 = MA
   ...
   *
   * @param params An array with elements as defined below. The array is processed from beginning to
   ...
   * @return the size of the file as String from the results making an internal request with the URL
   *     created from the provided parameters, never <code>null</code>, might be empty.
   * @throws IllegalArgumentException if any supplied parameter is <code>null</code>.
