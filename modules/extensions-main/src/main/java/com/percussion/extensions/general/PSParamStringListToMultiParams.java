...
public class PSParamStringListToMultiParams extends PSDefaultExtension
  ...
  private void process(Object[] params, IPSRequestContext request)
    ...
    if (params == null || params.length != 4)
      throw new PSParameterMismatchException(4, (params == null ? 0 : params.length));

    if (params[0] == null)
      throw new PSParameterMismatchException("source parameter name cannot be null");
    String sourceParamName = params[0].toString().trim();

    if (params[1] == null) throw new PSParameterMismatchException("delimiter cannot be null");
    String delimiter = params[1].toString().trim();

    if (params[2] == null)
      throw new PSParameterMismatchException("destination parameter name cannot be null");
    String destinationParamName = params[2].toString().trim();

    boolean replace = false;
    if (params[3] != null && params[3].toString().toLowerCase().startsWith("y")) {
      replace = true;
    }

    String sourceParamValue = request.getParameter(sourceParamName);
    boolean hasValue = sourceParamValue != null && sourceParamValue.trim().length() > 0;
    ...
    if (hasValue && sourceParamValue.indexOf(delimiter) >= 0) {
      ...
      StringTokenizer tokens = new StringTokenizer(sourceParamValue, delimiter);
      while (tokens.hasMoreTokens()) {
        String value = tokens.nextToken();
        if (replace) { // replace only the first time.
