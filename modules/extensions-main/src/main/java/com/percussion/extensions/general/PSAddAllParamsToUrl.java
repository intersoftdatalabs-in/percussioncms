...
public class PSAddAllParamsToUrl implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      if (htmlParams == null || htmlParams.isEmpty()) return resultDoc;

      for (int i = 0; params != null && i < params.length; i++) {
        Object obj = params[i];
        if (obj == null) continue;
        String elemName = obj.toString().trim();
        if (elemName.length() < 1) continue;

        NodeList nl = resultDoc.getElementsByTagName(elemName);
        if (nl == null || nl.getLength() < 1) continue;
        ...
        while (iterator.hasNext()) {
          ...
          if (key.equals(HTMLPARAM_PSSESSIONID)) continue;
