...
public class PSAddPluginProperties implements IPSResultDocumentProcessor {
  ...
  public static Document addPluginInfo(IPSRequestContext request, Document resDoc) {
    ...
    try {
      // Get the useragent
      String userAgent = request.getCgiVariable(IPSCgiVariables.CGI_REQUESTOR_SOFTWARE);
      ...
      if (plugin == null) {
        request.printTraceMessage("Could not find a plugin for UserAgent : " + userAgent);
      ...
      String appletcodebase = PSServer.getRequestRoot() + "/sys_resources/AppletJars";
      ...
      elem.setAttribute(
          ATTR_SEARCH_ENGINE_AVAILABLE, serverConfig.isSearchEngineAvailable() ? "yes" : "no");
