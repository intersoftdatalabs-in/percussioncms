...
public class PSDeleteContent implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (request == null) return; // should never happen

    String paramKeyName = "";
    if (params != null && params.length > 0) paramKeyName = params[0].toString().trim();

    if (paramKeyName.length() < 1) paramKeyName = IPSHtmlParameters.SYS_CONTENTID;

    String appRoot = request.getRequestRoot();
    int loc = appRoot.lastIndexOf("/");
    if (loc > 0) appRoot = appRoot.substring(loc + 1); // keep only the application name
    ...
    try {
      ...
      skipped = PSXmlDocumentBuilder.addElement(doc, root, ELEM_SKIPPED, null);

      // map is empty nothing to delete!
      if (htmlParams == null) return;
      ...
      String error = null;
      for (int i = 0; purgeContentIdList != null && i < purgeContentIdList.size(); i++) {
        ...
        if (ceResource != null) purgeResource = getPurgeUrl(ceResource);
        parent = root;
        error = null;
        if (contentid.length() < 1
          ...
          try {
            ...
            if (iReq != null) iReq.performUpdate();
            else {
            ...
            if (iReq == null) {
              throw new Exception("Unable to find resource:" + ceResource);
            }
      ...
      if (doc != null) {
        try {
          ...
          try (FileWriter fw =
              new FileWriter(rxRootDir + File.separator + appRoot + "/lastpurge.xml")) {
  ...
  private void updateCaches(PSRelationshipSet relationships) {
    ...
    PSRelationshipChangeEvent event =
        new PSRelationshipChangeEvent(PSRelationshipChangeEvent.ACTION_REMOVE, relationships);
  ...
  private String getPurgeUrl(String ceUrl) {
    ...
    if (ceUrl.length() > 0) result = ceUrl + PURGE_RESOURCE_NAME;
  ...
  private String getEditorUrl(String contentid, IPSRequestContext request) {
    ...
    try {
      ...
      if (iReq != null) doc = iReq.getResultDoc();
    } catch (Exception e) {
      // this should not happen, means system is unstable somehow
      throw new RuntimeException("Unable to retrieve url for purge: " + e.getLocalizedMessage());
    }
  ...
  private void deleteRelationships(IPSRequestContext request) throws PSException {
    ...
    final IPSRelationshipService svc = PSRelationshipServiceLocator.getRelationshipService();
    ...
    int itemId = Integer.parseInt(request.getParameter(IPSHtmlParameters.SYS_CONTENTID));
