...
public class PSMakeAbsLink extends PSSimpleJavaUdfExtension implements IPSUdfProcessor {
   ...
   * relative to the orginiating request's app root.
   *
   ...
   * qualified name, the http protocol, server and port will be added to the supplied name. For an
   * unqualified name, these items, plus the Rhythmyx request root and the originating application
   * request root will be added. For a relative name, the protocol (http or https), server, port,
   * and Rhythmyx root will be added, assuming it is relative to the originating request's app root.
   * For an empty string, all parts of the originating request will be used, substituting the
   * supplied parameters.If the port is 80, no port number will be added to the generated url.
   *
   * <p>Note: The URL returned will use the 'http' protocol unless the supplied URL specifies
   ...
   *     be returned. If the supplied base specifies a protocol other than HTTP or HTTPS, then it is
   *     returned unchanged.
    ...
    try {
      result =
          PSUrlUtils.createUrl(
              null, null, sourceUrl, paramMap.entrySet().iterator(), null, request);
    } catch (Throwable t) {
