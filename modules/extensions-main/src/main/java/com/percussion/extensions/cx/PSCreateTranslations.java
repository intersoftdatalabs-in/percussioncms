...
public class PSCreateTranslations extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document doc)
    ...
    Object[] contentIdArray = request.getParameterList(IPSHtmlParameters.SYS_CONTENTID);
    if (contentIdArray == null || contentIdArray.length < 1) {
      throw new IllegalArgumentException("At least one content id is required for translation");
    }

    Object[] revisionArray = request.getParameterList(IPSHtmlParameters.SYS_REVISION);
    if (revisionArray == null || revisionArray.length < 1) {
      throw new IllegalArgumentException("At least one revision is required for translation");
    }

    if (contentIdArray.length != revisionArray.length) {
      throw new IllegalArgumentException("Number of content ids and revisions does not match");
    }

    Object[] ceUrlArray = request.getParameterList("ceurl");
    if (ceUrlArray == null || ceUrlArray.length < 1) {
      throw new IllegalArgumentException("At least one ceUrlArray is required for translation");
    }
    ...
    Object[] objArray = request.getParameterList(IPSHtmlParameters.SYS_LANG);
    if (objArray == null || objArray.length < 1) {
      throw new IllegalArgumentException("A language is required for translation");
    }
    String lang = objArray[0].toString();

    String relationshipType = PSRelationshipConfig.TRANSLATION;
    objArray = request.getParameterList(IPSHtmlParameters.SYS_RELATIONSHIPTYPE);
    if (objArray != null && objArray.length > 0) {
      ...
      if (relCfg == null) {
        // TODO I18n
        throw new IllegalArgumentException(
            "Relationship type '" + relationshipType + "' does not exist in the system");
      }
    ...
    for (int i = 0; i < contentIdArray.length; i++) {
      try {
        ...
        if (loc > 0) ceurl = ceurl.substring(0, loc);
        ...
        if (ir == null) {
          statusElem.setAttribute("status", "Internal request for contenteditor is null");
          continue;
        }
        ir.makeRequest();
        String translatedid = ir.getRequestContext().getParameter(IPSHtmlParameters.SYS_CONTENTID);
