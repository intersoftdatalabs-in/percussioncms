...
public class PSMakeInternalRequest extends PSSimpleJavaUdfExtension {
   ...
   *
   * @params[0] the Rhythmyx resource to which to make an interal request. Specifies the application
   ...
   * @params[1] the name of the stylesheet to be applied to the request result document, may be
   ...
   * @params[2] a flag as <code>String</code> to specify whether or not the parameters of the
   ...
   * @params[3+n] the names of additional request parameters, optional. Parsing request parameters
   *     will be stopped if the first <code>null</code> or empty parameter name is found.
   * @params[3+n+1] the values of additional request parameters, optional. If <code>null</code>, an
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    String resource = "";
    if (size > 0) resource = (params[index] == null) ? "" : params[index].toString().trim();
    index++;
    ...
    String stylesheet = "";
    if (size > 1) stylesheet = (params[index] == null) ? "" : params[index].toString().trim();
    if (stylesheet.length() > 0) {
      String prefix = "../";
      if (stylesheet.startsWith(prefix))
        stylesheet = "file:" + stylesheet.substring(prefix.length());
      else stylesheet = "file:" + request.getCurrentApplicationName() + "/" + stylesheet;
    }
    index++;

    // should request parameters be inherited, defaults to yes
    String inheritParams = INHERIT_PARAMS;
    if (size > 2 && params[index] != null) inheritParams = params[index].toString().trim();
    if (inheritParams.length() == 0) inheritParams = INHERIT_PARAMS;
    index++;
    ...
    while (index < size) {
      if ((count % 2) == 0) {
        ...
        paramValue = (params[index] == null) ? "" : params[index].toString().trim();
        count++;

        // stop parsing request parameters with the first unspecified name
        if (paramName.length() > 0) requestParams.put(paramName, paramValue);
        else break;
      }
    ...
    try {
      ...
      IPSInternalRequest ir = request.getInternalRequest(resource, requestParams, inherit);
      if (ir != null) {
      ...
      String errorMsg = "An unexpected error occurred. The error was: " + e.getLocalizedMessage();
  ...
  /**
   * The stylesheet is applied to the source and the result is parsed back into XML.
   *
   * @param doc the source document to be transformed, assumed not <code>null</code>.
   * @param stylesheetName a url string to the stylesheet, may be <code>null</code> or empty in
   *     which case this method does nothing.
   * @param request the request used to print the operations trace messages to, assumed not <code>
   *     null</code>.
   ...
   */
  private Document processStylesheet(Document doc, String stylesheetName, IPSRequestContext request)
      throws PSConversionException, TransformerException, SAXException, IOException {
    if (stylesheetName == null || stylesheetName.trim().length() == 0) return doc;
    ...
    try {
      stylesheet = PSStylesheetCacheManager.getStyleSheetFromCache(stylesheetFile);

      Transformer transformer = stylesheet.getStylesheetTemplate().newTransformer();
      ...
      if (stylesheet != null)
        PSStylesheetCacheManager.appendErrorMessages(errorMsg, stylesheet.getErrorListener());
  ...
  /**
   * The value of the <code>inheritParams</code> parameter that evaluates to <code>true</code>. The
   * evaluation is made case insensitive. This is also the default, if the parameter is not
   * specified.
   */
