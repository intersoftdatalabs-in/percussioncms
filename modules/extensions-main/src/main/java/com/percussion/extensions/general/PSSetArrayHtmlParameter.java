...
public class PSSetArrayHtmlParameter implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if ((params == null) || (params.length < EXPECTED_NUMBER_OF_PARAMS)) {
      m_log.debug(
          "Supplied parameters do not match with the expected "
    ...
    String paramName = "";
    if (params[0] != null) paramName = params[0].toString().trim();
    ...
    String resourceName = "";
    if (params[1] != null) resourceName = params[1].toString().trim();
    if (resourceName.length() < 1) {
      m_log.debug("resourceName parameter is empty");
      throw new PSParameterMismatchException("resourceName must not be empty");
    }
    m_log.debug("resourceName is " + resourceName);

    String elemName = "";
    if (params[2] != null) elemName = params[2].toString().trim();
    ...
    if (maxNumber.length() < 1 && params[3] != null) maxNumber = params[3].toString().trim();
    int maxitems = 0;
    if (maxNumber.length() > 0) {
      try {
        maxitems = Integer.parseInt(maxNumber);
      } catch (NumberFormatException e2) {
        m_log.debug("maxNumber parameter '" + maxNumber + "' is not an integer");
        throw new PSParameterMismatchException(
    ...
    try {
      ...
      if (resReq == null) {
        String msg = ms_className + ": Unable to locate handler for request: " + resourceName;
        m_log.debug(msg);
      ...
      if (nl == null || nl.getLength() < 1) {
        m_log.debug("Element list is empty." + "Leaving without adding the paramter:" + paramName);
        return;
      }
      ...
      if (maxNumber.length() < 1) {
        ...
        m_log.debug("maxNumber parameter is not provided." + " Adding all the elements");
      } else if (elemsReturned <= maxitems) {
        loopEnd = elemsReturned;
        m_log.debug(
            "The number of elements or less than or equal to maxNumber parameter."
                + " Adding all elements.");
      }
      for (int i = 0; i < loopEnd; i++) {
        Element elem = (Element) nl.item(i);
        String elemValue = PSXmlTreeWalker.getElementData(elem).trim();
        if (elemValue.length() < 1) continue;
