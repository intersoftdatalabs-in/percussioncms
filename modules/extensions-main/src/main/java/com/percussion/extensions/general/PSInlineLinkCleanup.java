...
/**
 * This class is a Rhythmyx ResultDocumentProcessor extension. It is used to cleanup the orphan
 * inline links for all contents. It is expecting one (option) parameter, which is either "preview"
 * or "cleanup".
 *
 * <p>If the option parameter is "preview", then it processes all items, report status to the
 * console as well as to the trace if the trace is on, report the summary of the result at the end,
 * but it does not write to the database.
 *
...
public class PSInlineLinkCleanup extends PSDefaultExtension implements IPSResultDocumentProcessor {
   ...
   * @throws PSParameterMismatchException when the &lt;filelist&gt; is not specified
   */
  public Document processResultDocument(
      Object[] params, IPSRequestContext request, Document resultDoc)
      throws PSParameterMismatchException, PSExtensionProcessingException {
    if (params.length < 1 || null == params[0] || params[0].toString().trim().length() == 0) {
  ...
  /**
   * This function is required by the IPSResultDocumentProcessor interface. PSFormatFileTree will
   * never modify the stylesheet.
   *
   * @return false
   */
   ...
   */
  private void ProcessResultDoc(IPSRequestContext request, Document resultDoc, String command) {
   ...
   * @throws PSInvalidContentTypeException if encounter an invalid content type.
   */
  private ContentType[] getInlineLinkFields() throws PSInvalidContentTypeException {
    ...
    for (int i = 0; i < ctypes.length; i++) {
      itemDef = mgr.getItemDef(ctypeIds[i], PSItemDefManager.COMMUNITY_ANY);
  ...
  /**
   * Recurse through each mapper within a mapping to look for inline link fields and add them into
   * the given list.
   *
   ...
   * @param inlineFields This is used to collect all inline link fields, a set of <code>PSField
   *     </code> objects. Assume not <code>null</code>, it may be empty.
   */
        ...
        if (o == null) {
          o =
              fieldSet.getChildField(
                  mapping.getFieldRef(), PSFieldSet.TYPE_MULTI_PROPERTY_SIMPLE_CHILD);
        }
        if (o != null && o instanceof PSField) {
          PSField field = (PSField) o;
          if (field.isLocalField()) ctype.setLocalTable(field);
          Object backendObj = field.getLocator();
          if (field.mayHaveInlineLinks() && (backendObj instanceof PSBackEndColumn)) {
  ...
  private String getTableFromField(PSField field) {
    ...
    if (!(columnLocator instanceof PSBackEndColumn))
      throw new IllegalStateException(
          "field.getLocator() must return " + "a PSBackEndColumn object");
   ...
   * @param resultDoc The returned document that will contain the summary of the processed result,
   *     assume not <code>null</code>.
   * @param ctypes All visible content types that contains the summary of the processed result,
   *     assume not <code>null</code>.
   */
  private void reportProcessedResult(
      IPSRequestContext request, Document resultDoc, ContentType[] ctypes) {
    ...
    for (int i = 0; i < ctypes.length; i++) {
      ...
      if (!ctype.hasInlineFields()) continue;
      ...
      ctElement.setAttribute("deletes", String.valueOf(ctype.m_totalDeletes));
      ctElement.setAttribute("modifies", String.valueOf(ctype.m_totalModifies));
      ctElement.setAttribute("processedItems", String.valueOf(ctype.m_totalProcessedItems));
   ...
   */
  private void traceMessage(IPSRequestContext request, String msg, Object[] args) {
  ...
  /** Convenient method, calls {@link #traceMessage(IPSRequestContext, String, null)}. */
  ...
  private class ContentType {
     ...
     *
     * @param itemdef The item definition of the content type, assume not <code>null</code>.
     */
     ...
     *
     * @return <code>true</code> if it contains inline link fields; <code>false</code> otherwise.
     */
    ...
    /**
     * Get the table name for the local fields of this content type. {@link setLocalTable(PSField)}
    ...
    private String getLocalTable() {
      if (m_localTable == null)
        throw new IllegalStateException("setLocalTable must be called first");
     ...
     * @return A list over zero or more <code>PSLocator</code> objects, never <code>null</code>, but
     *     may be empty.
     ...
     */
    public void processAllContents(IPSRequestContext request, boolean isPreview) {
     ...
     *
     * @param request The request object, assume not <code>null</code>.
     * @param locator The locator of the to be processed content, assume not <code>null</code>.
     * @param isPreview <code>true</code> if the request is for preview only; no modification to the
     *     database.
     * @throws Exception if an error occurs.
     */
    private void processContents(IPSRequestContext request, PSLocator locator, boolean isPreview)
        throws Exception {
      ...
      // pre-processing
      request.setParameter(IPSHtmlParameters.SYS_CONTENTID, String.valueOf(locator.getId()));
      ...
      if (deletes.size() > 0 || modifies.size() > 0) {
        ...
        traceMessage(
            request,
            "processed {0} item (id={1}, rev={2}), "
                + "deleted ({3}) and added/updated ({4}) relationships",
            args);
      }
    }
     ...
     * @return A list over <code>FieldContent</code> objects, never <code>null</code>, but may be
     *     empty.
     * @throws Exception if an error occurs.
     */
    private List getContents(PSLocator locator, PSField field) throws Exception {
      ...
      if (schema.getColumn(SYSID) != null) hasSysid = true;
    ...
    /**
     * Creates a filter for the supplied item locator. The created filter is used for the where
     * clause as (CONTENTID=id AND REVISIONID=rev)
     *
     * @param locator The item locator, assume not <code>null</code>.
    ...
    private PSJdbcSelectFilter getFilter(PSLocator locator) {
      ...
      PSJdbcSelectFilter idFilter =
          new PSJdbcSelectFilter(
              CONTENTID, PSJdbcSelectFilter.EQUALS, String.valueOf(locator.getId()), Types.INTEGER);
     ...
     * setLocalTable(PSField)}, never <code>null</code> after that.
     */
  ...
  private class FieldContent {
     ...
     * @param value The value of the field, it may be <code>null</code>, which will be treated as an
     *     empty string.
     * @param locator The locator of the processed item, assume not <code>null</code>.
     * @param sysid The value from the SYSID column, it may be <code>-1</code> if there is no SYSID
     *     column for the table of the field.
     */
    FieldContent(PSField field, String value, PSLocator locator, int sysid) {
      m_field = field;
     ...
     *
     * @param value The new value of the field, it may be <code>null</code>. A <code>null</code>
     *     value will be treated as empty string.
     */
    ...
    /**
     * Performs pre-process for the current content. It updates the supplied deletes and modified
     * list afterwards.
     *
     * @param request The current request, assume not <code>null</code>.
     * @param deletes It contains to be deleted relationships, assume not <code>null</code>.
     * @param modifies It contains to be modified (updated or inserted) relationships, assume not
     *     <code>null</code>.
    ...
    void preProcess(
      ...
      request.setParameter(IPSHtmlParameters.SYS_CONTENTID, String.valueOf(m_locator.getId()));
      request.setParameter(IPSHtmlParameters.SYS_REVISION, String.valueOf(m_locator.getRevision()));
    ...
    /**
     * Performs post process for this object. This can only be called after the {@link
     * #preProcess(IPSRequestContext, PSRelationshipSet, PSRelationshipSet)} has been called.
     *
     * @param request The current request, assume not <code>null</code>.
     * @throws Exception if an erorr occurs.
     */
    void postProcess(IPSRequestContext request) throws Exception {
      ...
      PSJdbcColumnData rev =
          new PSJdbcColumnData(REVISIONID, String.valueOf(m_locator.getRevision()));
      columns.add(rev);
      PSJdbcColumnData content = new PSJdbcColumnData(column, m_value);
      columns.add(content);
      if (m_sysid != -1) {
        PSJdbcColumnData sysid = new PSJdbcColumnData(SYSID, String.valueOf(m_sysid));
        columns.add(sysid);
      }
    ...
    /**
     * The value of the field, which may be the original or updated value. Initialized by ctor,
     * never <code>null</code> after that.
     */
    private String m_value;

    /** The locator of the processed item, init by ctor, never <code>null</code> after thatn. */
    ...
    /**
     * The inline link field object, it is set by {@link preProcess()}, never <code>null</code>
     * after that.
     */
  ...
  /**
   * It maps database's table name to its schema. The map key is the name of the table as a <code>
