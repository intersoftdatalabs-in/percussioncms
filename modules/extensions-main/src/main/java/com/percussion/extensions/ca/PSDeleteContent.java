...
public class PSDeleteContent implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (paramKeyName.length() < 1) paramKeyName = DEAFULT_PARAM_PURGEURL;
      ...
      for (int i = 0; purgeurllist != null && i < purgeurllist.size(); i++) {
        ...
        if (contentid.length() < 1 || appResource.length() < 1) {
          ...
          try {
            ...
            if (iReq == null) {
              throw new Exception("Internal purge request:" + appResource + " does not exist");
            }
            iReq.makeRequest();
          } catch (Exception e) {
            parent = skipped;
  ...
  private static String getPurgeAppResource(String purgeurl) {
    ...
    if (purgeurl.length() > 0) result = purgeurl + PURGE_RESOURCE_NAME;
  ...
  public static void main(String[] args) {
    ...
    PSDeleteContent.parseForContentid(
        "http://10.10.10.17:9992/Rhythmyx/xr_ceImage/image.html?sys_command=preview&sys_contentid=78&sys_revision=1");
  }
