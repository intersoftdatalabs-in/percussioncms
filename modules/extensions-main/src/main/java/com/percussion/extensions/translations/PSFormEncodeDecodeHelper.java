...
public class PSFormEncodeDecodeHelper {
   ...
   */
  private static String transformTag(String content, String tagname, boolean encode) {
    if (content == null) throw new IllegalArgumentException("content cannot be null.");
    ...
    if (encode) {
      ...
      content = fixCommentTags(content.replaceAll("\\>\\<script", "> <script"));
      findTag = tagname;
      newTag = ELEM_DIV;
    } else {
      // replace the pageBreak elements with the proper PI
      content =
          Pattern.compile("<pageBreak>", Pattern.CASE_INSENSITIVE)
              .matcher(content)
              .replaceAll("<?pageBreak?>");
      content =
          Pattern.compile("</pageBreak>", Pattern.CASE_INSENSITIVE).matcher(content).replaceAll("");
    }
    ...
    while ((sTag = source.findNextStartTag(idx, findTag)) != null) {
      ...
      Attribute att = null;
      if (!encode) att = attrs.get(marker);
      if (encode || att != null) {
        buff.append("<");
        buff.append(newTag);
        if (encode) {
        ...
        buff.append(">");
        if (encode && isScriptTag) buff.append(HTML_COMMENT_BEGIN + UNIQUE);
        eTag = sTag.findEndTag();
        boolean empty = sTag.isEmptyElementTag();
        String endComm = "";
        if (encode && isScriptTag) endComm = UNIQUE + HTML_COMMENT_END;
        String endTag = endComm + "</" + newTag + ">";
        outDoc.add(new StringOutputSegment(sTag, buff.toString() + (empty ? endTag : "")));
        if (!empty) outDoc.add(new StringOutputSegment(eTag, endTag));
      }
      idx = sTag.getEnd();
    }
  ...
  public static final String ms_test_string =
      "<body>\n"
          ...
          + "</form>\n"
          + "<table><tbody><tr><td>\n"
          + "<!--somecomment--><script language=\"JavaScript1.1\""
          + " src=\"http://ads.vegas.com/js.ng/site=vegas&amp;size=html&amp;ppos=guide014&amp;ch1=v_realestate&amp;ch2=builders\""
