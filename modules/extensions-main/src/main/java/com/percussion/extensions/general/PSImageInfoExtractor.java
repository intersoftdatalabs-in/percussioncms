...
public class PSImageInfoExtractor extends PSFileInfo implements IPSItemInputTransformer {
    ...
    while (iter.hasNext()) {
      ...
      if (obj instanceof PSPurgableTempFile) {
        ...
        // only set the parameters if an uploaded file is found
          ...
          // image..
          String mimetype = request.getParameter(paramName + "_type").toString();
          if (mimetype.indexOf("image") >= 0) {
            ImageSize imageSize = null;
            if (temp != null) {
              try {
                imageSize = getImageSize(temp.getAbsolutePath());
              } catch (FileNotFoundException fnfe) {
                request.printTraceMessage("File Not Found Exception -- " + fnfe.toString());
                log.error(fnfe.getMessage());
                log.debug(fnfe.getMessage(), fnfe);
              } catch (Exception e) {
                request.printTraceMessage("Exception -- " + e.toString());
              }
              ...
              if (imageSize != null) {
                ...
                request.setParameter(paramName + "_" + IMAGE_WIDTH, imageWidth);
                request.setParameter(paramName + "_" + IMAGE_HEIGHT, imageHeight);
              }
            } // end (temp != null)
          } // end if mimetype.indexOf...
          else {
            request.printTraceMessage("The mime type is not image" + mimetype);
            request.printTraceMessage("The mime type is not image" + mimetype.indexOf("image"));
          }
        } // end else if
  ...
  private void clearImageParams(IPSRequestContext request, String paramName) {
    ...
    if (null == paramName) throw new IllegalArgumentException("param name cannot be null.");
