...
public class PSPublishEditionForPreview extends PSDefaultExtension
        ...
        try (InputStream content = mi_requestPath.openStream()) {
          ...
          try (InputStreamReader inputReader = new InputStreamReader(content)) {
            try (BufferedReader reader = new BufferedReader(inputReader)) {
              ...
              // counting down.
              if (stat != null && stat.indexOf("notInProgress") >= 0) break;
              Thread.sleep(1000);
              if (count > 10) break;
            }
          }
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (params.length < 3) {
      ...
      throw new IllegalArgumentException(
          "The first three parameters" + " are required, see log for details");
    }
    ...
    request.printTraceMessage(
        "previewvariant  " + previewVariantId + " and sys_variant " + variantId);
    if (previewVariantId != null) {
      if (!(previewVariantId.trim().equalsIgnoreCase(variantId))) {
        request.printTraceMessage(
            "previewvariant does not equal " + previewVariantId + " and sys_variant " + variantId);
    ...
    if (contentId == null || revisionId == null || variantId == null || context == null) {
    ...
    try {
      ...
      if (editionId != null || assemblyVariantId != null || context.equals("0")) {
        ...
        setPreviewPage(editionId, assemblyVariantId, contentId, revisionId, request);
        ...
        try {
          ...
          // Request
          request.printTraceMessage("The published URL " + pubUrl.toString());
        } catch (IOException e) {
        ...
        request.printTraceMessage("About to remove Preview page from the RXEDITIONITEM table");
        // Now clean up preview page info
        removePreviewPage(editionId, variantId, contentId, revisionId, request);
        ...
        request.printTraceMessage("IN A PUBLISHABLE CONTEXT NOT IN PREVIEW");
      }
    } // end of try
    catch (Exception e) {
  ...
  private String getArg(Object[] params, int index, String defaultValue) {
    if (index >= params.length || params[index].toString().trim().length() == 0) {
      return defaultValue;
    } else {
  ...
  private void setPreviewPage(
    ...
    request.printTraceMessage("In PSPublishEditionForPreview, above call to rowExists");
   ...
   * @throws PSExtensionProcessingException on internal errors
   */
  private void removePreviewPage(
      String editionId,
      String variantId,
      String contentId,
      String revisionId,
  ...
  private boolean rowExists(
    ...
    internalReq = request.getInternalRequest(m_queryResource, paramMap, false);
    if (internalReq == null)
      throw new PSConversionException(0, "queryEdition application not found.");

    Document doc = internalReq.getResultDoc();
    NodeList nodes = doc.getElementsByTagName(PARAMS_ELEM);
    if (nodes.getLength() <= 0) throw new PSConversionException(0, "No PARAMS_ELEM element found.");
    ...
    request.printTraceMessage("bottom of rowExists and returnValue==" + returnValue);
