...
public class PSSimpleJavaUdf_cloneTitle extends PSSimpleJavaUdfExtension {
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    String overrideName = request.getParameter(IPSHtmlParameters.SYS_TITLE_OVERRIDE);
    ...
    for (int i = 0; i < formatArgs.length; i++) formatArgs[i] = params[i + 1];
  ...
  private int calculateCloneCount(IPSRequestContext request) throws PSCmsException {
    ...
    if (rels.size() <= 1) {
      return rels.size();
    } else if (!relConfig.getCategory().equals(PSRelationshipConfig.CATEGORY_TRANSLATION)) {
      return rels.size();
    } else {
      if (lang == null) throw new IllegalArgumentException("IPSHtmlParameters.SYS_LANG is missing");
