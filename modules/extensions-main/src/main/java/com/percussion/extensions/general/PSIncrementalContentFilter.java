...
public class PSIncrementalContentFilter extends PSDefaultExtension
  ...
  public Document processResultDocument(
    ...
    if (null == params) throw new IllegalArgumentException("Parameters cannot be null.");
    if (null == request) throw new IllegalArgumentException("Request cannot be null.");
    if (null == resultDoc) throw new IllegalArgumentException("Result document cannot be null.");

    String internalReqName = params[0].toString();
    if (internalReqName.length() == 0) {
      request.printTraceMessage("no request name");
      throw new PSParameterMismatchException("The request name must be provided");
    }
    String switchParameter = params[1].toString();
    if (switchParameter.length() > 0) { // the switchParameter is specified.
      // does it exist in the callers parameter map?
      if (request.getParameter(switchParameter) == null) { // no, not present
        request.printTraceMessage(
            "HTML parameter " + switchParameter + " not present. Filtering disabled.");
    ...
    try {
      ...
      if (null == rootNode) return resultDoc;
      ...
      if (isEmpty) {
        ...
        PSXmlDocumentBuilder.addEmptyElement(resultDoc, rootNode, NODE_CONTENTITEM);
        return resultDoc;
      } else {
