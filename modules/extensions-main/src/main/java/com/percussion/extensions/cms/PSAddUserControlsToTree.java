...
public class PSAddUserControlsToTree extends PSDefaultExtension
  ...
  public Document processResultDocument(
    ...
    PSXmlDomContext contxt = new PSXmlDomContext(this.getClass().toString(), request);
    ...
    ctrlFiles.add(new File(rootDir, "rx_resources/stylesheets/rx_Templates.xsl"));
    ctrlFiles.addAll(PSCustomControlManager.getInstance().getControlFiles());

    Element destNode = PSXmlDomUtils.findElement(".", resultDoc);
    if (destNode == null) {
    ...
    for (File ctrlFile : ctrlFiles) {
      try {
        Document sourceDoc = PSXmlDomUtils.loadXmlDocument(contxt, ctrlFile);
        if (null != sourceDoc && null != sourceDoc.getDocumentElement()) {
          Node importNode = resultDoc.importNode(sourceDoc.getDocumentElement(), true);
