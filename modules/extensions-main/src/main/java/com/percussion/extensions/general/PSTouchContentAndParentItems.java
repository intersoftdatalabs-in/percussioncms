...
public class PSTouchContentAndParentItems extends PSDefaultExtension
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    String sys_contentid =
        getParameter(params, 0, false, request.getParameter(IPSHtmlParameters.SYS_CONTENTID));

    if (sys_contentid == null || sys_contentid.trim().length() == 0) {
      throw new PSParameterMismatchException(
          PSTouchContentAndParentItems.class + ": Missing sys_contentid parameter");
    }

    // get optional max parent lookup recursion steps, default is "unlimited"
    String lookupRecursionSteps = getParameter(params, 1, false, LOOKUP_RECURSION_STEPS);
      ...
      try {
        ...
        throw new PSParameterMismatchException(
            PSTouchContentAndParentItems.class
                + ": maxRecursionSteps parameter must be set to '"
  ...
  private static String getParameter(
      Object[] params, int index, boolean required, Object defaultValue)
    ...
    if (params.length < index + 1
      ...
      if (required) {
        throw new PSParameterMismatchException(
            PSTouchContentAndParentItems.class + ": Missing parameter");
      } else {
