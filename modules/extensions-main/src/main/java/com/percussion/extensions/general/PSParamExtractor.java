...
public class PSParamExtractor extends PSDefaultExtension implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (inputDoc == null) {
      request.printTraceMessage("Input document is null. No parameter is extracted");
      return;
    }
    // basic validation of the input document
    if (!inputDoc.getDocumentElement().getNodeName().equals(PSHtmlParamDocument.ROOT)) {
    ...
    for (int i = 2; i < params.length; i++) {
      ...
      if (paramName.length() < 1) continue;
      if (forInClause) {
        paramValue = htmlParamDoc.getParamForInClause(paramName, encloseInQuotes);
        request.setParameter(paramName, paramValue);
      } else {
        request.setParameter(paramName, htmlParamDoc.getParam(paramName));
      }
      String operator =
          (String) htmlParamDoc.getParam(paramName + PSHtmlParamDocument.OPERATOR_SUFFIX);
      if (operator != null && operator.trim().length() > 0) {
        request.setParameter(paramName + PSHtmlParamDocument.OPERATOR_SUFFIX, operator);
      }
    }
  }
}
