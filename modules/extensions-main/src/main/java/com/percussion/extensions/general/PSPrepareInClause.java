        ...
        while (values.hasNext()) {
          ...
          if (o != null) {
            ...
            if (value.trim().length() != 0) {
              ...
              if (encloseInQuotes) buf.append('\'');
              buf.append(o.toString());
              if (encloseInQuotes) buf.append('\'');
            }
          }
        }
        if (buf.length() > 0) inString = buf.toString();
      } else {
        inString = obj.toString().trim();
        if (encloseInQuotes) inString = '\'' + inString + '\'';
      }
    }

    // see if we have anything
    if (inString.trim().length() == 0 && params.length > 2 && params[2] != null) {
