...
public class PSMakeAbsLinkSecureEx extends PSSimpleJavaUdfExtension implements IPSUdfProcessor {
   ...
   *
   * <p>If the supplied URL is fully qualified and the protocol is not 'http' or 'https', the
   * supplied URL will be returned, unmodified. Otherwise, any pieces supplied will be substituted.
   * If the supplied URL is not fully qualified, the missing parts will be added. For a partially
   * qualified name, the http(s) protocol, server and port will be added to the supplied name. For
   * an unqaulified name, these items, plus the Rhythmyx request root and the originating
   * application request root will be added. For a relative name, the http(s) protocol, server,
   ...
   *   <tr>
   *     <td>2</td>
   *     <td>The host name to be used to produce the output url.</td>
   *     <td>no</td>
   *     <td>null</td>
   *   </tr>
   *   <tr>
   *     <td>3</td>
   *     <td>The port to be used to produce the output url.</td>
   *     <td>no</td>
   *     <td>null</td>
   *   </tr>
   *   <tr>
   ...
   *     be returned. If the suppied base specifies a protocol other than HTTP or HTTPS, then it is
   *     returned unchanged.
   * @throws PSConversionException If any required param is missing.
   */
  public Object processUdf(Object[] params, IPSRequestContext request)
      throws PSConversionException {
    if (params == null) throw new PSConversionException(0, "required parameter(s) missing");

    // useSecure is optional, defaults to yes
    boolean useSecure = true;
    if (params[0] != null) useSecure = params[0].toString().trim().equalsIgnoreCase("yes");

    // server name is optional
    String host = null;
    if (params[1] != null) {
      String tempHost = params[1].toString().trim();
    ...
    if (params[2] != null) {
      String tempPort = params[2].toString().trim();
      if (tempPort.length() > 0) portString = tempPort;
    }
    Integer port = null;
    if (portString != null) {
      try {
    ...
    if (params[3] == null) throw new PSConversionException(0, "resource parameter is required.");
    ...
    try {
      result =
          PSUrlUtils.createUrl(
              host, port, sourceUrl, paramMap.entrySet().iterator(), null, request, useSecure);
    } catch (Throwable t) {
