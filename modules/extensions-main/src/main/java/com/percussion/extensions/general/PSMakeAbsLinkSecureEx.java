 ...
 */
public class PSMakeAbsLinkSecureEx extends PSSimpleJavaUdfExtension
   implements IPSUdfProcessor
{
   ...
   {
      if (params == null)
         throw new PSConversionException( 0, "required parameter(s) missing" );

      // useSecure is optional, defaults to yes
      boolean useSecure = true;
      if (params[0] != null)
         useSecure = params[0].toString().trim().equalsIgnoreCase("yes");
      ...
      if (portString != null)
      {
         ...
         catch (NumberFormatException e)
         {
            throw new PSConversionException(0, 
               "port parameter must be a parsable integer.");
         }
      }
      
      // resource is required
      if (params[3] == null)
         throw new PSConversionException(0, "resource parameter is required.");
      ...
      try
      {
         result = PSUrlUtils.createUrl(host, port, sourceUrl,
