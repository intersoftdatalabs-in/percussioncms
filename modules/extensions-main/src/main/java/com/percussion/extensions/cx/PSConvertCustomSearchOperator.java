...
public class PSConvertCustomSearchOperator extends PSDefaultExtension
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (params.length < 1 || params[0] == null || params[0].toString().trim().length() == 0) {
      throw new PSParameterMismatchException("Must supply operatorParamName parameter");
    }

    // handle getting params from doc vs request
    boolean useReqParams = "n".equalsIgnoreCase(getOptionalParamValue(params, 7, "y"));
    PSHtmlParamDocument paramDoc = null;
    if (!useReqParams) {
      Document inputDoc = request.getInputDocument();
      if (inputDoc != null) {
        Element root = inputDoc.getDocumentElement();
        if (root != null && root.getNodeName().equals(PSHtmlParamDocument.ROOT)) {
    ...
    String opVal;
    if (useReqParams) opVal = request.getParameter(opParamName);
    else opVal = (String) paramDoc.getParam(opParamName);

    String dataType = getOptionalParamValue(params, 4, DATATYPE_TEXT).toUpperCase();
    if (!(dataType.equals(DATATYPE_DATE)
        || dataType.equals(DATATYPE_NUMBER)
        || dataType.equals(DATATYPE_TEXT))) {
      throw new PSParameterMismatchException("Invalid backendColumnDataType: " + dataType);
    }

    if (params.length == 1 || (params[1] == null || params[1].toString().trim().length() == 0)) {
      // operator only conversion
      if (!DATATYPE_TEXT.equals(dataType)) {
        throw new PSParameterMismatchException(
            dataType + " backendColumnDataType not supported for operator only" + " conversion.");
      }

      if (PSSearchField.OP_EQUALS.equals(opVal)) request.setParameter(opParamName, "=");
    ...
    if (params.length < 4) {
      throw new PSParameterMismatchException("Invalid number of parameters");
    } else {
      if (params[2] == null || params[2].toString().trim().length() == 0) {
        throw new PSParameterMismatchException("Must supply sqlFragmentParamName parameter");
      }

      if (params[3] == null || params[3].toString().trim().length() == 0) {
        throw new PSParameterMismatchException("Must supply backendColumnName parameter");
      }
    }

    // see if we have any values to convert
    Object[] values = null;
    if (useReqParams) values = request.getParameterList(params[1].toString().trim());
    else {
    ...
    // handle conversion
    String fragment = convert(request, opVal, values, beColName, dataType, connector, dateFormat);
  ...
  private String convert(
    ...
    if (opVal.equals(PSSearchField.OP_LIKE)) {
      ...
      buf.append(" like ");
      buf.append(getValue(req, values[0].toString(), dataType, opVal, dateFormat).toString());
      ...
      for (int i = 0; i < values.length; i++) {
        ...
        buf.append(getValue(req, values[i].toString(), dataType, opVal, dateFormat).toString());
      }
      buf.append(")");
      ...
      // only expecting one value
      Object val = getValue(req, values[0].toString(), dataType, dataType, dateFormat);
      if (opVal.equals(PSSearchField.OP_EQUALS) && val instanceof String[]) {
        ...
        String convOp;
        if (opVal.equals(PSSearchField.OP_EQUALS)) convOp = " = ";
        else if (opVal.equals(PSSearchField.OP_GREATERTHAN)) convOp = " > ";
        else convOp = " < ";
      ...
      for (int i = 0; i < values.length; i++) {
        values[i] = getValue(req, values[i].toString(), dataType, opVal, dateFormat);
      }
      String[] ops = {" >= ", " <= "};
  ...
  private Object getValue(
    ...
    if (dataType.equals(DATATYPE_TEXT)) {
      ...
      FastDateFormat srcFormat = FastDateFormat.getInstance(DEFAULT_DATE_FORMAT);
      ...
      try {
        ...
        req.printTraceMessage("failed to convert date value: " + e.getLocalizedMessage());
        result = getTextVal(strDate);
      }
    }
  ...
  private String getOptionalParamValue(Object[] params, int index, String defaultValue) {
    ...
    if (params.length > index && params[index] != null) {
      ...
      if (tempVal.length() > 0) val = tempVal;
    }
