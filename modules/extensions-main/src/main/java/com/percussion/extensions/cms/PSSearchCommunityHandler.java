      ...
      if (name.equals("sys_community")) {
        ...
        if (!state.equals("db_marked_for_deletion")) {
          try {
            ...
            if (!value.equals("-1")) {
              PSCommunity comm = rmgr.loadCommunity(new PSGuid(PSTypeEnum.COMMUNITY_DEF, value));
              community_name = comm.getName();
            } else {
              community_name = "AnyCommunity";
            }
            if (object_acl == null) {
              object_acl = new PSAclImpl("x" + searchId, new PSAclImpl().createDefaultEntry(false));
              object_acl.setGUID(PSGuidHelper.generateNext(PSTypeEnum.ACL));
              object_acl.setObjectId(Long.parseLong(searchId));
              object_acl.setObjectType(PSTypeEnum.SEARCH_DEF.getOrdinal());
            }
            IPSTypedPrincipal community =
