...
public class PSCheckOutSlotItem extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document doc)
    ...
    String checkoutstatus = doc.getDocumentElement().getAttribute(ATTR_CHECKOUTSTATUS);
    int assignmenttype =
        Integer.parseInt(doc.getDocumentElement().getAttribute(ATTR_ASSIGNMENTTYPE));
    if (checkoutstatus.trim().length() == 0
        || checkoutstatus.equalsIgnoreCase(IPSConstants.CHECKOUT_STATUS_SOMEONEELSE)
        || assignmenttype < 2) {
      ...
      if (contentid != null && contentid.length() > 0) {
        ...
        try {
          ...
          IPSInternalRequest iReq1 = request.getInternalRequest(ceresource, chkoutMap, false);
          iReq1.makeRequest();
          iReq1.cleanUp();
        } catch (Exception e) {
          Element root = doc.getDocumentElement();
