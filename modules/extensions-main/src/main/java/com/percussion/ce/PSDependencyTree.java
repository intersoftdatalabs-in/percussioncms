...
public class PSDependencyTree implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    try {
      ...
      parent =
          PSXmlDocumentBuilder.addElement(resDoc, resDoc.getDocumentElement(), "children", null);
      ...
      parent =
          PSXmlDocumentBuilder.addElement(resDoc, resDoc.getDocumentElement(), "parents", null);
      processUrl(itemsRendered, indent, parent, request, url);
    } catch (Exception e) {
  ...
  private void processUrl(
    ...
    try {
      ...
      if (loc > -1) {
        ...
        loc = url.indexOf("?");
        if (loc > -1) resource = url.substring(0, loc);
        else resource = url;
      }
      loc = resource.indexOf('.');
      if (loc > -1) resource = resource.substring(0, loc);
      ...
      for (int i = 0; i < nl.getLength(); i++) {
        ...
        if (contentid.trim().length() < 1) continue;

        Element res =
            PSXmlDocumentBuilder.addElement(parent.getOwnerDocument(), parent, "Item", null);
        res.setAttribute("contentid", contentid);
        res.setAttribute("relationshiptype", elem.getAttribute("relationshiptype"));
        PSXmlDocumentBuilder.addElement(
            parent.getOwnerDocument(), res, "text", indent + makeItemText(elem, res));
        PSXmlDocumentBuilder.addElement(
        ...
        } else res.setAttribute("repeat", "y");
      }
    } catch (Exception e) {
      PSXmlDocumentBuilder.addElement(
  ...
  private String makeItemText(Element elem, Element res) {
    ...
    if (workflow.getAttribute("stateValid").equalsIgnoreCase("y")) res.setAttribute("public", "y");
    else res.setAttribute("public", "n");
