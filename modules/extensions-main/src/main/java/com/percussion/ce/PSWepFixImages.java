...
public class PSWepFixImages extends PSFileInfo implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    if (param == null || param.trim().length() < 1) return;

    Document doc = null;

    try {
      doc = PSXmlDocumentBuilder.createXmlDocument(new StringReader(param), false);

      NodeList nl = doc.getElementsByTagName("img");

      int len = nl.getLength();

      if (len < 1) return; // no images - nothing to do
      ...
      if (!isImgFixed) return; // we didn't do anything - no need to serialize, just return
