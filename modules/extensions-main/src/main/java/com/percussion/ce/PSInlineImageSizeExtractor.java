...
public class PSInlineImageSizeExtractor implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    if (urlstring == null
        || urlstring.trim().length() < 1
        || inlinetype == null
        || !inlinetype.equalsIgnoreCase("rximage")) {
    ...
    if (iinfo != null) {
       ...
       */
      resDoc.getDocumentElement().setAttribute("height", iinfo.getHeight() + "");
      resDoc.getDocumentElement().setAttribute("width", iinfo.getWidth() + "");
    }
