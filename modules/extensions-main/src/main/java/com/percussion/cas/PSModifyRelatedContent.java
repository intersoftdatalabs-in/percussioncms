...
public class PSModifyRelatedContent extends PSDefaultExtension implements IPSRequestPreProcessor {
      ...
      if (request != null) {
        ...
        if (command == null)
          throw new PSExtensionProcessingException(
              0, "HTML parameter 'sys_command' cannot be null");
        ...
        if (!command.equalsIgnoreCase(COMMAND_INSERT)) {
          ...
          if (sysid == null || sysid.toString().trim().length() == 0)
            throw new PSExtensionProcessingException(
                0, "HTML parameter 'sysid' cannot be null or empty");
          rid = Integer.parseInt(sysid);
        }

        if (command.equalsIgnoreCase(COMMAND_MODIFY)) modifySlotVariant(rid, request);
        else if (command.equalsIgnoreCase(COMMAND_DELETE)) deleteSlotItem(rid, request);
        else if (command.equalsIgnoreCase(COMMAND_MOVEUP)) moveUp(rid, request);
        else if (command.equalsIgnoreCase(COMMAND_MOVEDOWN)) moveDown(rid, request);
        else if (command.equalsIgnoreCase(COMMAND_INSERT)) insertSlotItems(request);
        else throw new PSExtensionProcessingException(0, "Unsupported 'sys_command': " + command);
    ...
    if (relationship != null) {
      ...
      if (newSlotid != null) {
        ...
        try {
          ...
          throw new PSExtensionProcessingException(
              0, "Found invalid new slotid for relationship: " + rid);
        }

        if (getSlotType(slotid, request) == null)
          throw new PSExtensionProcessingException(0, "Invalid new slotid : " + slotid);
      ...
      if (newVariantid != null) {
        try {
          ...
          throw new PSExtensionProcessingException(0, "Invalid new variantid : " + newVariantid);
        }

        relationship.setProperty(IPSHtmlParameters.SYS_VARIANTID, newVariantid);
      }
  ...
  public static void deleteSlotItem(int rid, IPSRequestContext request) throws PSException {
    ...
    if (request.getParameter(IPSHtmlParameters.SYS_CONTENTID) == null && !relationships.isEmpty()) {
      PSRelationship relationship = (PSRelationship) relationships.get(0);
      request.setParameter(
          IPSHtmlParameters.SYS_CONTENTID, String.valueOf(relationship.getOwner().getId()));
    }
  }
  ...
  public static void move(int rid, IPSRequestContext request, boolean up) throws PSException {
    ...
    if (relationship != null) {
      ...
      if (slot == null)
        throw new PSExtensionProcessingException(0, "No slot found for slotid: " + slotid);
      ...
      if (template == null)
        throw new PSExtensionProcessingException(0, ERROR_MSG_NO_VARIANT_FOUND + variantid);

      PSAaRelationshipList relationshipList = new PSAaRelationshipList();
      PSAaRelationship aarelationship =
          new PSAaRelationship(relationship, slot, new PSContentTypeTemplate(template));
      ...
      if (siteid != null) aarelationship.setProperty(IPSHtmlParameters.SYS_SITEID, siteid);
      String folderid = relationship.getProperty(IPSHtmlParameters.SYS_FOLDERID);
      if (folderid != null) aarelationship.setProperty(IPSHtmlParameters.SYS_FOLDERID, folderid);
      relationshipList.add(aarelationship);

      String sortrank = relationship.getProperty(IPSHtmlParameters.SYS_SORTRANK);
      if (sortrank == null)
        throw new PSExtensionProcessingException(
            0, "No sortrank property found for relationship: " + rid);
      ...
      try {
        ...
        throw new PSExtensionProcessingException(
            0, "Found invalid sortrank for relationship: " + rid);
      }
      ...
      if (request.getParameter(IPSHtmlParameters.SYS_CONTENTID) == null)
        request.setParameter(
            IPSHtmlParameters.SYS_CONTENTID, Integer.toString(relationship.getOwner().getId()));
    }
  }
  ...
  public static void insertSlotItems(IPSRequestContext request) throws PSException {
     ...
     * parent item.
     */
    String activeitemid = request.getParameter(IPSHtmlParameters.SYS_ACTIVEITEMID);
    ...
    if (contentid == null) throw new PSExtensionProcessingException(0, "No owner contentid found.");

    if (revision == null) throw new PSExtensionProcessingException(0, "No owner revision found.");

    String slotid = request.getParameter(IPSHtmlParameters.SYS_SLOTID);
    if (slotid == null) throw new PSExtensionProcessingException(0, "No slotid found.");
    PSSlotType slot = getSlotType(slotid, request);
    if (slot == null)
      throw new PSExtensionProcessingException(0, "No slot found for slotid: " + slotid);
    ...
    if (cidVidArray == null || cidVidArray.length == 0)
      throw new PSExtensionProcessingException(0, "No contentid / variantid list found.");

    PSLocator owner = new PSLocator(contentid, revision);
    PSRelationshipConfig config = PSActiveAssemblerProcessor.getConfigForSlot(request, slotid);
    if (config == null)
      config =
    ...
    for (int i = 0; i < cidVidArray.length; i++) {
      ...
      // Required parameters
      String dependentid = "";
      if (array.length > 0) dependentid = array[0];
      String variantid = "";
      if (array.length > 1) variantid = array[1];
      /*
       * Starting from Rx 5.6, the cidVidArray element may have the optional
       * parameters siteid and folderid too.
       */
      String siteid = "";
      if (array.length > 2) siteid = array[2];

      String folderid = "";
      if (array.length > 3) folderid = array[3];
      ...
      if (template == null)
        throw new PSExtensionProcessingException(
            0, "No variant type found for variantid: " + variantid);

      PSAaRelationship relationship =
          new PSAaRelationship(owner, dependent, slot, new PSContentTypeTemplate(template), config);
      if (siteid.length() > 0) relationship.setProperty(IPSHtmlParameters.SYS_SITEID, siteid);
      if (folderid.length() > 0) relationship.setProperty(IPSHtmlParameters.SYS_FOLDERID, folderid);
      ...
      if (httpcaller != null) {
        ...
        if (pos == -1 || httpcaller.substring(pos).indexOf(IPSHtmlParameters.SYS_CONTENTID) == -1) {
          String delimiter = pos == -1 ? "?" : "&";
          httpcaller += delimiter + IPSHtmlParameters.SYS_CONTENTID + "=" + dependentid;
  ...
  public static void moveToSlot(
    ...
    String variantid = relationship.getProperty(IPSHtmlParameters.SYS_VARIANTID);
    if (newVariantId != -1) variantid = String.valueOf(newVariantId);
    IPSAssemblyTemplate template = getTemplate(variantid);
    if (template == null)
      throw new PSExtensionProcessingException(
          0, "No variant type found for variantid: " + variantid);
   ...
   */
  public static PSRelationshipProcessor getRsProcessor(IPSRequestContext request)
   ...
   * @throws PSCmsException for any errors creating the processor.
   */
  public static PSActiveAssemblyProcessorProxy getAaProcessor(IPSRequestContext request)
      throws PSCmsException {
  ...
  public static PSRelationship getRelationship(int rid, IPSRequestContext request)
    ...
    if (!relationships.isEmpty()) relationship = (PSRelationship) relationships.get(0);
   ...
   * @throws PSCmsException
   */
  private static IPSAssemblyTemplate getTemplate(String variantid) throws PSCmsException {
    try {
      IPSAssemblyService assembly = PSAssemblyServiceLocator.getAssemblyService();
      try {
        return assembly.loadTemplate(variantid, true);
      } catch (PSAssemblyException e) {
        throw new PSCmsException(0, "Invalid variantid : " + variantid);
      }
    } catch (NumberFormatException e) {
  ...
  public static Document getActiveItemInfo(IPSRequestContext request)
    ...
    try {
      String appResource = request.getCurrentApplicationName() + "/activeitem";
      if (appResource.startsWith("/")) appResource = appResource.substring(1);
