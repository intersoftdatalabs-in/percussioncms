  ...
  public String createLocation(Object[] params, IPSRequestContext request)
    ...
    try {
      ...
      if ((params.length >= 2)
        ...
        newHTMLParams.put(IPSHtmlParameters.SYS_CONTENTID, params[1].toString().trim());
      }
      if ((params.length >= 3)
          && (params[2] != null)
          && (params[2].toString().trim().length() > 0)) {
        newHTMLParams.put(IPSHtmlParameters.SYS_REVISION, params[2].toString().trim());
      }

      IPSInternalRequest iReq = request.getInternalRequest(resource, newHTMLParams, true);
      if (iReq == null) {
        String msg = ms_className + ": Unable to locate handler for request: " + resource;
      ...
      request.printTraceMessage(ms_className + "#createLocation location = " + location);
