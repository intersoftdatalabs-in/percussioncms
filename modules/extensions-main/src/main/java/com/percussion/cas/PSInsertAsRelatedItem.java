...
public class PSInsertAsRelatedItem implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      if (index > 0) {
        varid =
            psredirect.substring(index + PSModifyRelatedContent.PARAM_ITEMVARIANTID.length() + 1);
        index = varid.indexOf('&');
        if (index > -1) varid = varid.substring(0, index);
      }
      /*
      ...
      psredirect =
          psredirect
              ...
              + PARAM_HTTPCALLER
              + "="
              + URLEncoder.encode(request.getParameter(PARAM_HTTPCALLER).toString());
