...
public class PSSysTemplateToSysVariantid implements IPSRequestPreProcessor {
  ...
  private Map<String, String> getVariantMap(IPSRequestContext request)
    ...
    IPSInternalRequest ir = request.getInternalRequest(VARIANT_LIST_RESOURCE);
    try {
      ...
      if (nl.getLength() == 1) {
        ...
        for (int i = 0; i < len; i++) {
          ...
          Node displayname = PSXMLDomUtil.findFirstNamedChildNode(var, XML_ELEM_DISPLAYNAME);
          if (displayname != null) templatename = PSXMLDomUtil.getElementData(displayname);
          if (StringUtils.isNotBlank(templatename) && StringUtils.isNotBlank(variantid))
