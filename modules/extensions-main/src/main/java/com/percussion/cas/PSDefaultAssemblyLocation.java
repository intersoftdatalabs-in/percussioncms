...
public class PSDefaultAssemblyLocation implements IPSAssemblyLocation {
  ...
  public String createLocation(Object[] params, IPSRequestContext request)
    ...
    try {
      ...
      if (!root.endsWith(PATH_DELIMITER)) root += PATH_DELIMITER;
      ...
      if (path.endsWith(PATH_DELIMITER)) path = path.substring(0, path.lastIndexOf(PATH_DELIMITER));

      String suffix = params[2].toString();
      if (!suffix.startsWith(SUFFIX_DELIMITER)) suffix = SUFFIX_DELIMITER + suffix;
