...
public class PSCreatePortalPropertyList extends PSSimpleJavaUdfExtension {
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    for (int i = 1; i < size; i += PROPERTY_PARAMSIZE) {
      ...
      if (createProperty(params, i, doc, element) == false) break;
    }
   ...
   */
  private boolean createProperty(Object[] params, int offset, Document doc, Element parent)
      throws PSConversionException {
    String propertyName = "";
    String propertyType = "";
    ...
    for (int i = 0; i < PROPERTY_ATTRS.length; i++) {
      ...
      if (attrName.equalsIgnoreCase(PROPERTY_PATTERN)) {
        ...
        if (attrValue.length() == 0 && propertyType.equalsIgnoreCase(TYPE_DATETIME)) {
          // Default is ok
      ...
      if (dels != null) {
        ...
        if (tokens.hasMoreTokens())
          while (tokens.hasMoreTokens()) {
            ...
            createPropertyElement(doc, parent, propertyName, propertyType, propertyPattern, token);
          }
      } else {
        createPropertyElement(doc, parent, propertyName, propertyType, propertyPattern, value);
      }
    }
