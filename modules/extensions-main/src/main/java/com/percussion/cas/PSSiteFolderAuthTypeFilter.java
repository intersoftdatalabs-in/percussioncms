...
public class PSSiteFolderAuthTypeFilter implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (nl != null && nl.getLength() > 0) {
      if (nl.getLength() == 1 && ((Element) nl.item(0)).getAttribute(CONTENTID).length() == 0)
        return resultDoc;
  ...
  private void filterLinkUrls(IPSRequestContext request, Document doc)
    ...
    Map<String, String> folderPaths = initializeFolderPathMap(request, relProxy, doc);
    ...
    for (int i = nl.getLength() - 1; i >= 0; i--) {
      ...
      if (!isValidLinkBySiteID(request, relProxy, siteFolderRoot, linkparams, linkurl)) {
        nl.item(i).getParentNode().removeChild(nl.item(i));
        continue;
      }
      if (!isValidLinkByFolderID(
          request, relProxy, siteFolderRoot, folderPaths, linkparams, linkurl)) {
        nl.item(i).getParentNode().removeChild(nl.item(i));
        continue;
      }
      if (!isValidLinkByVariant(request, linkparams, linkurl)) {
        nl.item(i).getParentNode().removeChild(nl.item(i));
        continue;
      }
    }
  ...
  private Map validateLinkUrl(IPSRequestContext request, Element linkurl) {
    ...
    String folderid = linkurl.getAttribute(IPSHtmlParameters.SYS_FOLDERID).trim();
    ...
    if (rcurl == null || rcurl.trim().length() < 1) {
      ...
      if (nl != null && nl.getLength() > 0) rcurl = ((Element) nl.item(0)).getAttribute("current");
    }
    ...
    String revision = (String) rcurlparams.get(IPSHtmlParameters.SYS_REVISION);
  ...
  private Map initializeSiteFolderRootMap(IPSRequestContext request)
    ...
    if (lookupRequest == null) {
      String msg =
          ms_className + ": Unable to locate handler for request: " + LOOKUP_SITE_FOLDER_ROOT;
      log.debug(msg);
      request.printTraceMessage(msg);
      throw new PSExtensionProcessingException(0, msg);
    }
  ...
  private Map<String, String> initializeFolderPathMap(
    ...
    // map will be empty
    if (folderIdSet.size() < 1) return folderPaths;
    ...
    try {
      ...
      for (PSComponentSummary s : summaries) {
        ...
        folderPaths.put(
            Integer.toString(((PSLocator) s.getLocator()).getId()), relPath[0] + "/" + s.getName());
      }
  ...
  private boolean isValidLinkBySiteID(
      ...
      throws PSExtensionProcessingException {
    if (request == null) throw new IllegalArgumentException("request must not be null");
    if (relProxy == null) throw new IllegalArgumentException("relProxy must not be null");
    ...
    if (!siteFolderRoot.containsKey(siteid)) {
      String msg =
          "Removed linkurl : \nReason: No "
              + "Site Folder site exists with the siteid "
              + siteid
              + "\nlinkurl: "
              + PSXMLDomUtil.toString(linkurl);
      log.debug(msg);
    ...
    try {
      ...
      if (!found) {
        String msg =
            "Removed linkurl : \nReason: Item("
                + contentId
                + ") does not exist under the specified site("
                + siteid
                + ")."
                + "\nlinkurl: "
                + PSXMLDomUtil.toString(linkurl);
    ...
    try {
      ...
      if (rset == null || rset.size() < 1) {
        String msg =
            "Removed linkurl : \nReason: Item("
                + contentId
                + ") does not exist under the specified folder("
                + folderId
                + ")."
                + "\nlinkurl: "
                + PSXMLDomUtil.toString(linkurl);
  ...
  private boolean isValidLinkByVariant(IPSRequestContext request, Map linkparams, Element linkurl)
    ...
    if (template == null) {
      String msg =
          "Removed linkurl : \nReason: "
              + "Variant info is missing for specified variantid("
              + variantId
              + ")."
              + "\nlinkurl: "
              + PSXMLDomUtil.toString(linkurl);
    ...
    IPSInternalRequest lookupRequest =
        request.getInternalRequest(LOOKUP_VARIANTS_SITE_ITEM, reqParams, false);
    if (lookupRequest == null) {
      String msg =
          ms_className + ": Unable to locate handler for request: " + LOOKUP_VARIANTS_SITE_ITEM;
      log.debug(msg);
      request.printTraceMessage(msg);
      throw new PSExtensionProcessingException(0, msg);
    }
