...
public class PSFixAssemblyUrlForRevision extends PSDefaultExtension
  ...
  /**
   * Implementation of the interface method. See class description.
   *
  ...
  public Document processResultDocument(
    ...
    if (params.length > 1 && params[1] != null) {
      ...
      if (param.startsWith("n") || param.startsWith("f")) nullIfNeverPublic = false;
    }
    // Default content valid flag for revision correction is "i".
    String contentValidString = "i";
    if (params.length > 2 && params[2] != null && params[2].toString().trim().length() > 0) {
      // Override it with extension parameter if non null and non-empty
    ...
    try {
      if (attr) {
        ...
        for (int i = nl.getLength() - 1; i >= 0; i--) {
          ...
          if (url == null) {
            String msg = "Related link element is being " + "removed since the fixed URL is null.";
            ms_log.debug(msg);
        ...
        for (int i = nl.getLength() - 1; i >= 0; i--) {
          ...
          if (text == null || text.getNodeType() != Node.TEXT_NODE) {
            String msg = "Empty related link element " + elemOrAttr + " ignored.";
            ms_log.debug(msg);
            request.printTraceMessage(msg);
            continue;
          }
          ...
          if (url == null || url.length() < 1) {
            String msg = "Empty value for the related link element " + elemOrAttr + " ignored.";
            ms_log.debug(msg);
            request.printTraceMessage(msg);
            continue;
          }
  ...
  public static String fixRelatedContentUrl(
      ...
      boolean nullIfNeverPublic,
      String contentValidString)
    ...
    if (relatedContentUrl == null || relatedContentUrl.length() < 1) {
      throw new IllegalArgumentException("relatedContentUrl must not be null or empty");
    }
    ...
    String lastPublicRevision =
        PSMacroUtils.correctRevisionForFlags(
            request, contentid, contentValidString.toCharArray(), revision);

    boolean neverPublic = lastPublicRevision.equals("-1");
    if (neverPublic) {
      String msg = "Item with contentid = " + contentid + " was never public.";
      ms_log.debug(msg);
      request.printTraceMessage(msg);
      if (nullIfNeverPublic) {
        msg = "Returning null since nullIfNeverPublic " + "option is specified as true";
        ms_log.debug(msg);
        request.printTraceMessage(msg);
        return null;
      } else {
        msg = "Returning unmodified URL since nullIfNeverPublic " + "option is specified as false";
        ms_log.debug(msg);
        request.printTraceMessage(msg);
        return relatedContentUrl;
      }
    ...
    if (end != -1) fixedUrl += relatedContentUrl.substring(end);
