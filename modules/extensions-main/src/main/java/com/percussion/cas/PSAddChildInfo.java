...
public class PSAddChildInfo extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if ((params != null)
      ...
      String msg = ms_className + ": The exit param \"childResource\" must not be null or empty";
    ...
    try {
      ...
      if (iReq == null) {
        String msg = ms_className + ": Unable to locate handler for request: " + childResource;
        request.printTraceMessage(msg);
      ...
      if (childDoc != null) {
        ...
        Element childElement = childWalker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_CHILDREN);
        while (childElement != null) {
          PSXmlDocumentBuilder.copyTree(resultDoc, resultDocRoot, childElement);
          childElement = childWalker.getNextElement(PSXmlTreeWalker.GET_NEXT_ALLOW_SIBLINGS);
        }
      }
    } catch (PSInternalRequestCallException ex) {
      request.printTraceMessage(ms_className + ": " + ex.getLocalizedMessage());
