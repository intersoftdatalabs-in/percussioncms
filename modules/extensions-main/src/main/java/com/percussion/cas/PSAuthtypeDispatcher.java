...
public class PSAuthtypeDispatcher extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (authtype.length() == 0) authtype = "0";
    ...
    if (originalSiteId == null || originalSiteId.length() < 1) {
      request.setParameter(
          ...
          request.getParameterObject(IPSHtmlParameters.SYS_SITEID));
    }
    // Let root element get the original siteid attribute
    if (originalSiteId != null) {
      resultDoc
          .getDocumentElement()
          .setAttribute(IPSHtmlParameters.SYS_ORIGINALSITEID, originalSiteId);
    }
    ...
    try {
      ...
      for (int i = 0; i < nl.getLength(); i++) {
        Node node = nl.item(i);
        resultDoc.getDocumentElement().appendChild(resultDoc.importNode(node, true));
      }
