...
public class PSAddPortalProperties extends PSSimpleJavaUdfExtension {
  ...
  /**
   * Creates one <code>Property</code> element as specified in sys_PortalPublisher.dtd.
   *
   ...
   *     <code>null</code>.
   * @return the <code>Property</code> element as specified in sys_PortalPublisher.dtd, <code>null
   *     </code> if the property name was unspecified (either <code>null</code> or empty).
  ...
  private Element createProperty(Object[] params, int offset, Document doc)
    ...
    for (int i = 0; i < PROPERTY_ATTRS.length; i++) {
      ...
      String attrValue = (params[index] == null) ? "" : params[index].toString().trim();

      // stop parsing properties if the first invalid property name is found
      if (attrName.equalsIgnoreCase(PROPERTY_NAME) && attrValue.length() == 0) return null;
      ...
      if (attrName.equalsIgnoreCase(PROPERTY_TYPE)) {
        ...
        if (validTypes != null) {
          ...
          String errorMsg = "The supplied type \"{0}\" is not valid. Valid types are \"{1}\".";
      ...
      if (attrValue.length() == 0) {
        if (attrName.equalsIgnoreCase(PROPERTY_PATTERN)) {
          ...
          if (propertyType.equalsIgnoreCase(TYPE_DATETIME)) attrValue = DEFAULT_FORMAT;
        } else {
          int errorCode = 0;
          Object args[] = {attrName};
          String errorMsg = "Property attribute \"{0}\" cannot be null or empty.";

          throw new PSConversionException(errorCode, MessageFormat.format(errorMsg, args));
        }
      }

      if (attrValue.length() > 0) propertyElem.setAttribute(attrName, attrValue);
    }

    int index = offset + PROPERTY_ATTRS.length;
    String value = (params[index] == null) ? "" : params[index].toString().trim();
    if (value.length() == 0) {
      PSXmlDocumentBuilder.addEmptyElement(doc, propertyElem, VALUE_ELEM);
    } else {
      StringTokenizer tokens = new StringTokenizer(value, ",");
      while (tokens.hasMoreTokens())
        PSXmlDocumentBuilder.addElement(doc, propertyElem, VALUE_ELEM, tokens.nextToken().trim());
    }
  ...
  private String validateType(String type) {
    ...
    for (int i = 0; i < VALID_TYPES.length; i++) {
      if (type.equals(VALID_TYPES[i])) return null;
  ...
  /** The 'dataTime' type used as value for the <code>Property</code> type attribute. */
   ...
   * element.
   */
  private static final String[] VALID_TYPES = {TYPE_STRING, TYPE_NUMERIC, TYPE_DATETIME};

  /** The following constants define elment names as specified in sys_PortalPublisher.dtd. */
