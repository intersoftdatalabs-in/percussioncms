...
public class PSDatabasePublisher implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document doc)
    ...
    try {
      ...
      if (params != null) {
         ...
         */
        if (params.length > 0 && params[0] != null && params[0].toString().length() > 0)
          action = params[0].toString().toLowerCase();
        validateAction(action);

        // did they provid an aliasmap
        if (params.length > 1 && params[1] != null && params[1].toString().length() > 0) {
          Object[] args = {params[1].toString()};
      ...
      if (databases == null || databases.getLength() == 0)
        throw new PSExtensionProcessingException(0, "No database element found");
      ...
      dataPublisher.setAttribute(DBNAME_ATTR, database.getAttribute(DBNAME_ATTR));
      dataPublisher.setAttribute(DRIVERTYPE_ATTR, database.getAttribute(DRIVERTYPE_ATTR));
      dataPublisher.setAttribute(RESOURCENAME_ATTR, database.getAttribute(RESOURCENAME_ATTR));
      dataPublisher.setAttribute(ORIGIN_ATTR, database.getAttribute(ORIGIN_ATTR));
      ...
      if (tabledefs == null || tabledefs.getLength() == 0)
        throw new PSExtensionProcessingException(0, "No tabledefset element found");
      Element tabledef = (Element) tabledefs.item(0);
      PSXmlDocumentBuilder.copyTree(
          resultDoc, dataPublisher, lookup(request, tabledef.getAttribute(LOOKUP_ATTR)), true);
      ...
      tableDataSet.appendChild(createParentTableData(request, doc, resultDoc, action));
  ...
  private Map getAliasMap(Element aliases) {
    ...
    if (aliases != null) {
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
        ...
        aliasMap.put(elem.getAttribute(NAME_ATTR), elem.getAttribute(FOR_ATTR));
      }
    }
   ...
   */
  private Element createParentTableData(
      IPSRequestContext request, Document source, Document target, String action)
      throws PSExtensionProcessingException {
    NodeList tables = source.getElementsByTagName(TABLE_ELEM);
    if (tables == null || tables.getLength() == 0)
      throw new PSExtensionProcessingException(0, "No parent table element found.");
    Element table = (Element) tables.item(0);

    Element tableElem = target.createElement(TABLE_ELEM);
    tableElem.setAttribute(NAME_ATTR, resolveAlias(table.getAttribute(NAME_ATTR)));

    // loop over target rows
    NodeList targetRows = table.getChildNodes();
    for (int i = 0; i < targetRows.getLength(); i++) {
      if (!(targetRows.item(i) instanceof Element)) continue;
      ...
      if (columns.getLength() > 0) {
        ...
        for (int j = 0; j < columns.getLength(); j++) {
          if (!(columns.item(j) instanceof Element)) continue;
        ...
        for (int j = 0; j < columns.getLength(); j++) {
          ...
          if (column.getNodeName().equals(CHILDREN_ELEM)) {
            ...
            for (int k = 0; k < childTables.getLength(); k++) {
              if (!(childTables.item(k) instanceof Element)) continue;
  ...
  private void addChild(
    ...
    if (table == null || table.getAttribute(NAME_ATTR).length() == 0)
      throw new PSExtensionProcessingException(0, "No child table element found.");
    childtable.setAttribute(NAME_ATTR, resolveAlias(table.getAttribute(NAME_ATTR)));
        ...
        for (int j = 0; j < columns.getLength(); j++) {
          ...
          if (!columnNode.getNodeName().equals(CHILDREN_ELEM)) {
            if (columnNode.hasChildNodes()) {
              ...
              for (int k = 0; k < test.getLength(); k++) addColumn(columnNode, target, row);
            } else addColumn(columnNode, target, row);
          }
        }
  ...
  private void addColumn(Node column, Document target, Element row) {
    ...
    if (!column.getNodeName().equals(CHILDREN_ELEM)) {
      columnElem.setAttribute(NAME_ATTR, resolveAlias(column.getNodeName()));

      if (column.hasChildNodes()) {
        PSXmlDocumentBuilder.copyTree(target, columnElem, column.getFirstChild(), true);
      }
  ...
  private NodeList getRows(NodeList rows) {
    if (rows != null && rows.getLength() > 0) {
      ...
      if (row.hasChildNodes()) {
        ...
        if (!(rowid instanceof Text)) return row.getChildNodes();
      }
    }
    ...
    try {
      ...
      if (ir == null)
        throw new PSExtensionProcessingException(
            0, "No lookup request handler found for: " + resource);
   ...
   */
  private void validateAction(String action) throws PSExtensionProcessingException {
    if (action.equals(INSERT_ACTION)
        || action.equals(UPDATE_ACTION)
        || action.equals(DELETE_ACTION)
        || action.equals(REPLACE_ACTION)) return;
