...
public class PSGenerateAssemblerLink extends PSSimpleJavaUdfExtension {
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    if (revisionid == null)
      revisionid = Integer.parseInt(request.getParameter(IPSHtmlParameters.SYS_REVISION));
    if (authType == null) authType = request.getParameter(IPSHtmlParameters.SYS_AUTHTYPE, "0");
    if (context == null) context = request.getParameter(IPSHtmlParameters.SYS_CONTEXT, "0");
    if (siteid == null) siteid = request.getParameter(IPSHtmlParameters.SYS_SITEID);
    ...
    if (siteid != null) paramMap.put(IPSHtmlParameters.SYS_SITEID, siteid);
    ...
    if (relatedItemId != null
       ...
       */
      String activeItem = request.getParameter(IPSHtmlParameters.SYS_ACTIVEITEMID, "");
  ...
  protected String getAssemblyBaseUrl(String variantid, IPSRequestContext request)
      throws PSConversionException {
    IPSInternalRequest ireq = request.getInternalRequest(VARIANT_LOOKUP_URL, null, false);
    if (ireq == null) {
      Object[] args = {
