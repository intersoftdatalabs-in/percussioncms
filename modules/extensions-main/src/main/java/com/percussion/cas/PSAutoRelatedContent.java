...
public class PSAutoRelatedContent extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document doc)
    ...
    if (params.length >= 2 && params[1] != null && params[1].toString().length() > 0) {
    ...
    if (params.length >= 3 && params[2] != null && params[2].toString().trim().length() > 0)
      publishableTokens = params[2].toString().trim();

    String revisionCorrectionTokens = "i";
    if (params.length >= 4 && params[3] != null && params[3].toString().trim().length() > 0)
    ...
    int maxLinks = -1;
    if (params.length >= 5 && params[4] != null && params[4].toString().trim().length() > 0) {
      try {
        maxLinks = Integer.parseInt(params[4].toString().trim());
      } catch (NumberFormatException e) {
        throw new PSParameterMismatchException(
            "An invalid 'maxLinks' " + "parameter value was supplied. Must be a parsable integer.");
      }
    }
    ...
    try {
      ...
      if (rootElement != null) {
        ...
        Element assemblerInfo = walker.getNextElement(ASSEMBLER_INFO_ELEM, true);
        if (assemblerInfo != null)
          relatedContent = walker.getNextElement(RELATED_CONTENT_ELEM, true);

        if (relatedContent == null) throw new PSExtensionProcessingException(0, MSG_NO_RELATED);

        /*
         * Make an internal request to fetch all <code>
         * RelatedContent/linkurl </code> elements.
         */
        IPSInternalRequest relatedReq = request.getInternalRequest(URL, null, true);
        if (relatedReq == null)
          throw new PSExtensionProcessingException(0, MSG_INVALID_REQUEST + URL);
        ...
        int resultCount = 0;
        while (resultLink != null && (maxLinks == -1 || resultCount < maxLinks)) {
          if (!authtype.equals("0")) {
            ...
            if (contentValid.length() > 0) {
              ...
              StringTokenizer revtokens = new StringTokenizer(revisionCorrectionTokens, ",");
              if (!isFlagInToken(contentValid, revtokens)) correctRevision = false;
            }
            if (correctRevision) {
              NodeList valNodes = resultLink.getElementsByTagName("Value");
              if (valNodes != null && valNodes.getLength() > 0) {
                ...
                if (current.length() > 0) {
                  String correctedUrl =
                      PSMacroUtils.fixLinkUrlRevisionForFlags(request, current, validFlags);
          ...
          request.printTraceMessage(
              "Copying link url \n" + PSXmlDocumentBuilder.toString(resultLink));
          if (slotNameOverride != null) resultLink.setAttribute("slotname", slotNameOverride);
  ...
  private boolean isContentValid(
      Element resultLink, IPSRequestContext request, String publishableTokens)
    ...
    if (contentValid.length() > 0) {
      ...
      return PSCms.isRelatedItemPublishable(resultLink, request, publishableTokens);
    }
  }
