...
public class PSGeneratePubLocation extends PSSimpleJavaUdfExtension {
  ...
  public Object processUdf(Object[] params, IPSRequestContext request)
    ...
    PSExtensionParams eparams =
        new PSExtensionParams(
            params,
            new String[] {
    ...
    try {
      ...
      IPSGuid contextId =
          PSGuidUtils.makeGuid(
              eparams
                  .getNumberParam(
                      3, request.getParameter(IPSHtmlParameters.SYS_CONTEXT, "0"), false)
                  .longValue(),
              PSTypeEnum.CONTEXT);
      Number siteid =
          eparams.getNumberParam(4, request.getParameter(IPSHtmlParameters.SYS_SITEID), false);
      Number folderid =
          eparams.getNumberParam(5, request.getParameter(IPSHtmlParameters.SYS_FOLDERID), false);
      ...
      if (StringUtils.isNotBlank(filtername)) {
        ...
        try {
          ...
          if (filter.getLegacyAuthtypeId() != null) {
            authtype_from_filter = filter.getLegacyAuthtypeId().toString();
          }
       ...
       */
      request.setParameter(IPSHtmlParameters.SYS_VARIANTID, variantid.toString());
      if (contentid != null) {
        request.setParameter(IPSHtmlParameters.SYS_CONTENTID, contentid.toString());
      } else {
        contentid = Integer.parseInt(request.getParameter(IPSHtmlParameters.SYS_CONTENTID));
      }
      if (revision != null) {
        request.setParameter(IPSHtmlParameters.SYS_REVISION, revision.toString());
      } else {
        revision = Integer.parseInt(request.getParameter(IPSHtmlParameters.SYS_REVISION));
      }
      if (authtype != null) {
      ...
      if (siteid != null) request.setParameter(IPSHtmlParameters.SYS_SITEID, siteid);
      if (folderid != null) request.setParameter(IPSHtmlParameters.SYS_FOLDERID, folderid);
      if (page != null) request.setParameter("sys_page", page.toString());
      ...
      if (contextId.getUUID() == 0) {
        ...
        urlString = generatePubLocation(contextId, request, contentid, revision, variantid);
      }
   ...
   */
  private String generatePreviewLocation(
      ...
      String filtername,
      Number page)
      throws PSConversionException, PSInternalRequestCallException, MalformedURLException {
    PSStopwatchStack sws = PSStopwatchStack.getStack();
    ...
    try {
      ...
      if (siteid != null) paramMap.put(IPSHtmlParameters.SYS_SITEID, siteid.toString());
      if (folderid != null) paramMap.put(IPSHtmlParameters.SYS_FOLDERID, folderid.toString());
      ...
      if (command != null) {
        ...
        if (command.equals(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY)) {
          String aamode = request.getParameter(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY_MODE);
          if (StringUtils.isNotBlank(aamode))
            paramMap.put(IPSHtmlParameters.SYS_ACTIVE_ASSEMBLY_MODE, aamode);
          paramMap.put(IPSHtmlParameters.SYS_ACTIVEITEMID, "");
        }
      }

      // make a relative rhythmyx link
      String partial = PSServer.getRequestRoot() + "/" + IPSAssemblyService.ASSEMBLY_URL;
      String url = PSUrlUtils.createUrl(partial, paramMap.entrySet().iterator(), null);
   ...
   */
  private String generatePubLocation(
      IPSGuid contextid,
      IPSRequestContext request,
      Number contentid,
      Number revision,
    ...
    try {
      ...
      IPSGuid templateid = gmgr.makeGuid(variantid.longValue(), PSTypeEnum.TEMPLATE);

      PSComponentSummary summary = cmgr.loadComponentSummary(contentid.intValue());

      IPSLocationScheme thescheme = null;
      IPSGuid ctid = summary.getContentTypeGUID();
      try {
        List<IPSLocationScheme> schemes =
            sitemgr.findSchemeByAssemblyInfo(templateid, contextid, ctid);
        ...
        if (schemes == null || schemes.size() == 0) {
          ...
          if (thescheme == null) {
            throw new RuntimeException(
                "Default Location Scheme is not defined in Context id: " + contextid.toString());
          }
        } else {
      ...
      int rev = -1;
      if (revision != null) rev = revision.intValue();

      // resolve any back-end parameters in the location scheme
      PSExtensionParamValue[] exitParams = getExitParameters(thescheme, contentid.intValue(), rev);
      ...
      String oldContext = request.getParameter(IPSHtmlParameters.SYS_CONTEXT);
      request.setParameter(IPSHtmlParameters.SYS_CONTEXT, contextid.longValue());
  ...
  public static boolean isCrossSite(IPSRequestContext request, String currentSiteId) {
    ...
    String originalSitedId = request.getParameter(IPSHtmlParameters.SYS_ORIGINALSITEID, "");
    if (originalSitedId != null) originalSitedId = originalSitedId.trim();

    if (originalSitedId.length() > 0 && !originalSitedId.equals(currentSiteId)) return true;
      ...
      for (String pname : scheme.getParameterNames()) {
        ...
        if (type.equalsIgnoreCase(IPSAssemblyLocation.TYPE_STRING)) {
          ...
          parameters.add(new PSExtensionParamValue(literal));
            ...
            || type.equalsIgnoreCase(IPSAssemblyLocation.TYPE_BACKEND_COLUMN)) {
          if (connection == null) {
          ...
          if (pos > 0) {
            ...
            table = PSSqlHelper.qualifyTableName(table, db, schema, driver);
            ...
            if (type.equalsIgnoreCase("variant-based")) {
              ...
              statement = prepareContentStatement(table, column, contentid, revision, connection);
        ...
        } else if (type.trim().equalsIgnoreCase(IPSAssemblyLocation.TYPE_PASSTHROUGH)) {
   ...
   * @throws SQLException if any SQL operation fails.
   */
  private PreparedStatement prepareContentStatement(
      String table, String column, int contentid, int revision, Connection connection)
      throws SQLException {
    ...
    String query = "SELECT " + column + " FROM " + table + " WHERE (CONTENTID=?";
    if (useRevision) query += " AND REVISIONID=?)";
    else query += ")";

    PreparedStatement statement = PSPreparedStatement.getPreparedStatement(connection, query);
    statement.clearParameters();
    statement.setInt(1, contentid);
    if (useRevision) statement.setInt(2, revision);
