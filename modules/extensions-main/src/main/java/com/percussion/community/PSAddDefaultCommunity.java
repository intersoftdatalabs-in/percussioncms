...
public class PSAddDefaultCommunity implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(Object[] params, IPSRequestContext request, Document resDoc)
    ...
    try {
      ...
      String defcommid = "";
      String defcommname =
          PSAuthenticateUser.getUserRoleAttribute(request, PSAuthenticateUser.SYS_DEFAULTCOMMUNITY);
      if (null == defcommname) {
        defcommname = "";
      } else {
        defcommid = PSAuthenticateUser.getCommunityId(request, defcommname);
        if (null == defcommid) defcommid = "";
      }
      ...
      String homepageurl =
          PSAuthenticateUser.getUserRoleAttribute(request, SYS_DEFAULT_HOMEPAGEURL);
      if (null == homepageurl) {
        homepageurl = "";
      }
