...
/**
 * A Rhythmyx extension used to translate a text field, parse it, tidy it, scan it for inline links
 * and then place it back into the same field.
 *
 * <p>This extension is a standard Rhythmyx pre-exit. There are 7 parameters:
 *
 * <table border="1">
 * <tr>
 *    <th>Param #</th>
 ...
 * </table>
 *
 * <p>The source text field is always an HTML parameter. This exit will automatically detect if the
 * uploaded field is an attached file or an HTML field, and process it accordingly.
 *
 * <p>The html cleaner properties file is optional. If it is provided, the text will be run through
 * the HTML cleaner before parsing. See the {@link <a
 * href="https://jsoup.org/cookbook/cleaning-html/safelist-sanitizer" target="_blank"> JSoup
 * Parser</a>} page for details and properties file formats. The pathname provided must be relative
 * to the Rhythmyx server root.
 *
 * <p>The ServerPageTags.xml file is used to eliminate certain non-parsable text generated by some
 * editing programs. For details see {@link ProcessServerPageTags ProcessServerPageTags}. The
...
>>>>>>> development-8.1.x
      ...
      try {
        itemDef =
            PSItemDefManager.getInstance()
                .getItemDef(Long.parseLong(contenttypeid), request.getSecurityToken());
    ...
    if (param != null && param.length() > 0) contxt.setServerPageTags(param);

    param = PSXmlDomUtils.getParameter(params, 3, null);
    if (param != null) encodingDefault = param;

    param = PSXmlDomUtils.getParameter(params, 4, null);
    if (param != null && param.toLowerCase().startsWith("y")) inlineDisable = true;

    param = PSXmlDomUtils.getParameter(params, 5, null);
    if (param != null && param.equalsIgnoreCase("yes")) contxt.setUsePrettyPrint(false);
    ...
    try {
      if (inputSourceObj instanceof PSPurgableTempFile) {
        ...
        tempDocument =
            PSHtmlUtils.createHTMLDocument(
                inputSourceString,
                Charset.forName(encodingDefault),
                !PSServer.isHtmlCleaningDisabled(),
                htmlcleanupFile);
      }
   ...
   * @param contenttypeid the content type id, not <code>null</code> and numeric
