...
public class PSXdTextToTree extends PSDefaultExtension implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        Document tempResult = PSXmlDomUtils.loadXmlDocument(contxt, textSource);
        ...
        if (null != tempResult && null != tempResult.getDocumentElement()) {
          ...
          Node importNode = resultDoc.importNode(tempResult.getDocumentElement(), true);
  ...
  private void setupContextParameters(PSXmlDomContext contxt, Object[] params)
    ...
    if (params.length >= 2 && null != params[1] && params[1].toString().trim().length() > 0) {
      try {
        contxt.setTidyProperties(params[1].toString().trim());
      } catch (IOException e) {
        contxt.printTraceMessage(
            "Tidy Properties file " + params[1].toString().trim() + " not found ");
        throw new PSExtensionProcessingException(ms_className, e);
      }
    }
    if (params.length >= 3 && null != params[2] && params[2].toString().trim().length() > 0) {
  ...
  private static void removeAllTextNodes(Node el) {
    ...
    for (i = 0; i < children.getLength(); i++) {
      ch = children.item(i);
      if (ch.getNodeType() == Node.TEXT_NODE || ch.getNodeType() == Node.ENTITY_REFERENCE_NODE) {
        el.removeChild(ch);
      }
    }
  }
