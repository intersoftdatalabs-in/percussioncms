...
public class PSValidateSlotName implements IPSRequestPreProcessor {
  ...
  public void preProcessRequest(Object[] params, IPSRequestContext request)
    ...
    try {
      ...
      IPSInternalRequest iReq = request.getInternalRequest(slotListRequest);
      ...
      for (int i = 0; i < slotlist.getLength(); i++) {
        ...
        sname =
            slotElem.getElementsByTagName(XML_TAG_SLOTNAME).item(0).getFirstChild().getNodeValue();

        sid = slotElem.getElementsByTagName(XML_TAG_SLOTID).item(0).getFirstChild().getNodeValue();
  ...
  private boolean isValid() {
    ...
    while (iter.hasNext()) {
      ...
      // a different slotid
      if (m_slotname.equalsIgnoreCase(value) && !m_slotid.equals(key)) return false;
    }
    return true;
  }
