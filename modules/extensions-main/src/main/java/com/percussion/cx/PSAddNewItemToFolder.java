...
public class PSAddNewItemToFolder implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (htmlParams == null) {
      handleException(
          new Exception("HTML Parameter list must not be null or empty for this operation"));
    }
    ...
    try {
      ...
      if (psredirect != null) {
        ...
        if (index >= 0) {
          folderid = psredirect.substring(index + PARAM_SYS_FOLDERID.length() + 1);
        ...
        if (folderid.trim().length() == 0) {
          request.printTraceMessage(
              ms_fullExtensionName + ": folderid is not found to add the item to the folder");
          if (assetFolderId == null) return resultDoc;
      ...
      proxy.addChildren(locatorList, locatorParent);
      ...
      if (psredirect != null && contentid != null && contentid.trim().length() > 0) {
   ...
   */
  private void handleException(Exception e) throws PSExtensionProcessingException {
    throw new PSExtensionProcessingException(ms_fullExtensionName, e);
  }
