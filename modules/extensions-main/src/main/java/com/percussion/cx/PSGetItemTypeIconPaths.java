...
public class PSGetItemTypeIconPaths implements IPSResultDocumentProcessor {
  ...
  public Document processResultDocument(
    ...
    if (StringUtils.isBlank(itemsXml)) {
      log.warn(
<<<<<<< HEAD
          "Missing ItemLocators parameter in the request. Skipping the execution of exit and"
              + " returning unmodified result document.");
=======
          "Missing ItemLocators parameter in the request. Skipping the execution of exit and returning unmodified result document.");
>>>>>>> development-8.1.x
      return resultDoc;
    }
    try {
   ...
   * @throws SAXException
   */
  private List<PSLocator> getLocators(String itemsXml) throws IOException, SAXException {
    List<PSLocator> locs = new ArrayList<>();
    Document doc = PSXmlDocumentBuilder.createXmlDocument(new StringReader(itemsXml), false);
