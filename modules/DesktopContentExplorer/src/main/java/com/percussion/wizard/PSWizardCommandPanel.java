...
public class PSWizardCommandPanel extends JPanel implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getActionCommand().equals(COMMAND_BACK)) m_dialog.onBack();
    else if (e.getActionCommand().equals(COMMAND_NEXT)) m_dialog.onNext();
    else if (e.getActionCommand().equals(COMMAND_CANCEL)) m_dialog.onCancel();
    else if (e.getActionCommand().equals(COMMAND_FINISH)) m_dialog.onFinish();
  }
  ...
  /**
   * Updates all panel controls according to the supplied page type. Depending on the panel type,
   * buttons are enabled/disabled and/or renamed.
   *
   * @param type the page type for which to update the panel controls, must be one of <code>
   *     IPSWizardDialog.TYPE_XXX</code>.
   */
  public void updateControls(int type) {
    ...
    switch (type) {
      case IPSWizardDialog.TYPE_FIRST:
        ...
        m_next.setMnemonic(
            PSI18NTranslationKeyValues.getInstance().getMnemonic(getClass().getName() + "@Next >"));
      ...
      case IPSWizardDialog.TYPE_LAST:
        ...
        m_next.setMnemonic(
            PSI18NTranslationKeyValues.getInstance().getMnemonic(getClass().getName() + "@Finish"));
  ...
  /**
   * Validates the supplied panel type. Throws an <code>IllegalArgumentException</code> if the
  ...
  private void initPanel() {
    ...
    m_back.setMnemonic(
        PSI18NTranslationKeyValues.getInstance().getMnemonic(getClass().getName() + "@< Back"));
    ...
    m_cancel.setMnemonic(
        PSI18NTranslationKeyValues.getInstance().getMnemonic(getClass().getName() + "@Cancel"));
  ...
  /**
   * Creates the command panel with the three buttons 'next', 'back' and 'cancel'.
   *
  ...
  /**
   * Creates the divider panel which is just a horizontal gray line to divide the command panel
   * optically from the page panel.
   *
   * @return the new panel, never <code>null</code>.
   */
  ...
  /**
   * The command panel 'Next' button, initialized in {@link #initPanel()}, never <code>null</code>
   * after that. This button changes its name to 'Finish' if we reached the last wizard page.
   */
  private JButton m_next = null;

  /**
   * The command panel 'Cancel' button, initialized in {@link #initPanel()}, never <code>null</code>
   * after that.
   */
