 ...
 *
 * <p>1. Load the user options from the server. 2. Get a named option value. 3. Set a named option
 * by modifying the existing one or by creating a new one if does not already exist. 4. Save the
 * user settings to server for future use. 5. DisplayOptions may also be retrieved explicitly
 */
public class PSOptionManager {
  ...
  /**
   * Loads user settings from the server option handler. Server knows the user name from the
   * session. These settings include all properties overridden by the user in the previous sessions.
   *
   * @throws PSContentExplorerException if there is a problem loading the options.
   */
  ...
  public PSDisplayOptions getDisplayOptions() {
    ...
    if (puo == null) {
      ...
      JOptionPane.showMessageDialog(
          dialog,
          m_applet.getResources().getString("userOption.missing.for.locale"),
          m_applet.getResources().getString("error"),
          JOptionPane.ERROR_MESSAGE);
      System.exit(0);
    } else {
      PSOptions dispOptions = puo.getOptions(PSDisplayOptions.DISPLAY_OPTIONS_CATEGORY);

      if (dispOptions != null) theDisplayOptions = new PSDisplayOptions(dispOptions);
    }
   ...
   * <code>el</code>. Collections must of type <code>IPSClientObjects</code>, this simply iterates
   * through the collection calling toXml on each.
   *
   * @param el - on which to append the elements generated by the call to each {@link
   *     IPSClientObjects#toXml(Document) IPSClientObjects.toXml(Document)} , must not be <code>null
   *     </code>.
  ...
  private Document getUserOptionsDoc(boolean loadDefault)
    ...
    String appPath =
        PSContentExplorerConstants.OPTIONS_URL + "?" + IPSHtmlParameters.SYS_COMMAND + "=";
  ...
  /**
   * Saves user settings from memory to server using the server option handler. Server knows the
   * user name from the session. These settings include all properties overridden by the user in the
   * current sessions.
   *
   * @param updateSessionTimeout <code>true</code> to have this request update the client session
   *     timeout on the server, <code>false</code> to have this request avoid impacting the current
   *     session's client timeout.
   * @throws PSContentExplorerException if an error occurs while trying to save the options.
   */
  public void save(boolean updateSessionTimeout) throws PSContentExplorerException {
    if (m_applet.getActionManager() == null) return;
  ...
  /**
   * @todo is there a utils package that has this? Compares objects that implement the <code>
   *     equals()</code> method. <code>String</code>s will be compared with case ignored. Are they
   *     equal?
   ...
   *
   * @return the useroptions of this object. The user options are initialized by the <code>load()
   *     </code>, and may be <code>null</code> if that method was not called, but once initialized
   *     it is invariant.
   */
   ...
   *
   * @param userOptions the useroptions of this object. Must not be <code>null</code>.
   */
  private void setUserOptions(PSUserOptions userOptions) {
    if (userOptions == null) throw new IllegalArgumentException("userOptions must not be null");
  ...
  public PSDisplayOptions getDefaultDisplayOptions() throws PSContentExplorerException {
    ...
    if (getDefaultOptions() == null)
      throw new IllegalStateException(
          "load() must be called before calling getDefaultDisplayOptions()");

    PSOptions dispOptions =
        getDefaultOptions().getOptions(PSDisplayOptions.DISPLAY_OPTIONS_CATEGORY);

    if (dispOptions != null
        || dispOptions.getCategory().equals(PSDisplayOptions.DISPLAY_OPTIONS_CATEGORY))
