...
public class PSMainDisplayPanel extends JScrollPane
   ...
   * @param actManager the action manager to use to support context menu, may not be <code>null
   *     </code>
   * @param isSorted <code>true</code> to allow the table to be sorted, <code>false</code> to
   *     maintain the supplied order of the data.
   */
  public PSMainDisplayPanel(String view, PSActionManager actManager, boolean isSorted) {
    if (!PSUiMode.isValidView(view)) throw new IllegalArgumentException("view is not valid.");
    ...
    if (m_childViewTable.getRowCount() > 0) m_childViewTable.setRowSelectionInterval(0, 0);
  }

  /**
   * Determines if sorting is currently enabled.
   *
   * @return <code>true</code> if currently enabled, <code>false</code> otherwise.
   */
  ...
  /**
   * Enables or disables sorting. If disabled, then the will not allow sorting, if enabled, it will.
   *
   * @param isEnabled If <code>true</code>, sorting will be enabled, otherwise it will be disabled.
   */
  ...
  /**
   * Initializes this panel by adding a table that supports displaying data using data node's
   * display format and allows sorting of columns.
   */
  private void init() {
    ...
    m_childViewTable =
        new JTable(m_childViewTableModel) {
          @Override
    ...
    m_childViewTable.getTableHeader().setDefaultRenderer(new MainDisplayTableHeaderRenderer());
    ...
    m_childViewTableModel.addTableModelListener(
    ...
    if (m_actManager.viewSupportsCopyPaste()) {
      ...
      AbstractAction ctlcAction =
          new AbstractAction() {
            public void actionPerformed(@SuppressWarnings("unused") ActionEvent e) {
              ...
              if (selNodes.hasNext()) {
                PSSelection sel =
                    new PSSelection(new PSUiMode(m_view, ms_mode), m_parentNode, selNodes);
      ...
      AbstractAction ctlvAction =
          new AbstractAction() {
            public void actionPerformed(ActionEvent e) {
              Iterator<PSNode> selNodes = getSelectedNodes((Component) e.getSource(), null);
      ...
      DragSource.getDefaultDragSource()
          .createDefaultDragGestureRecognizer(
              m_childViewTable, DnDConstants.ACTION_COPY_OR_MOVE, this);
  ...
  /**
   * Sets the node whose children need to be displayed in this panel's table. This refreshes the
   * panel with new data. Table is sorted by the first column, ascending, and column widths are set
   * based on either persisted user data, or, if none have been persisted, the node's display
   * format.
   *
   * @param node the node whose children need to be displayed, may not be <code>null</code>
   * @param parent the parent of the supplied node, may be <code>null</code>, need to be supplied to
   *     pop-up actions that are shown when the mouse-click is on view port.
  ...
  public void setData(PSNode node, PSNode parent) {
    ...
    if (sortingInfo != null) {
      ...
      try {
        ...
        if (dfId.trim().length() > 0) {
          df = m_applet.getActionManager().getDisplayFormatCatalog().getDisplayFormatById(dfId);
          String col = df.getPropertyValue(PSDisplayFormat.PROP_SORT_COLUMN);
          String dir = df.getPropertyValue(PSDisplayFormat.PROP_SORT_DIRECTION);
          if (col != null) {
            int colIndex = df.getColumnIndex(col);
            if (colIndex != -1) {
              m_childViewTableModel.setInitialSortColumn(
                  colIndex, PSDisplayFormat.SORT_ASCENDING.equals(dir));
            }
          }
        } else {
    ...
    if (node.getChildCount() != 0) {
      ...
      if (node.isSearchType()) {
        msg = m_applet.getResourceString(getClass().getName() + "@NoResults");
      } else {
        msg = m_applet.getResourceString(getClass().getName() + "@NoItems");
      }
  ...
  /**
   * Resets the current column widths based on the stored column widths for the specified node. If
   * the node does not have a set of stored widths, then we use any widths specified by the display
   * format of the node.
   *
   * @param node The node to get the specific column widths, this is the same node that was used to
   *     store the column widths when they were modified. Must not be <code>null</code>.
   * @param df The display format specified by the <code>node</code>, may be <code>null</code> if
   *     one is not specified. Used to supply column widths if none have been stored for the
   *     supplied node.
   */
  private void resetColumnWidths(PSNode node, PSDisplayFormat df) {
    ...
    int remainingColCount = columnModel.getColumnCount() - specifiedWidthCount;
    ...
    for (String width : widths) {
      ...
      } else useWidth = Integer.parseInt(width);
   ...
   *
   * @param displayName the display name for which to get the icon name, assumed not <code>null
   *     </code> or empty.
   * @return the icon name from the static display name to icon name map or the supplied display
   *     name if not found. Never <code>null</code> or empty.
   */
  ...
  /** The class that handles pop-up mouse clicks to show pop-up menu for the current selection. */
  private class PopupListener extends PSMouseAdapter {

    /**
     * Handles the right-click mouse event. If source of event is the child table and there are no
     * selected rows, makes the row where the event occurred as selected. Displays context-menu for
     * the selected rows. If the source of event the view port of this panel, then it shows context
     * menu for the parent node whose children is displayed in this table. If the parent node is not
     * yet set, does nothing.
     *
     * @param event the mouse click event, assumed not <code>null</code> as this is called by Swing
     *     model.
     */
    @Override
    public void mouseWasClicked(MouseEvent event) {
      if (PSCxUtil.isMouseMenuGesture(event, m_applet)) {
        Iterator<PSNode> selNodes =
            getSelectedNodes((Component) event.getSource(), event.getPoint());

        displayPopupMenu((Component) event.getSource(), null, selNodes, event.getPoint());
      }
    }
  }

  /**
   * Class used to listen for changes to the width of columns. Basically used to save the state of
   * the widths on the original mouse press, then checks to see if anything has changed the column
   * widths on the mouse release, if so calls the main applet to store the widths into the
   * persistant storage.
   */
  private class PSTableColumnResizeHandler extends MouseAdapter {
    /**
     * Handle the mouse pressed on the table column header, store all the current widths of the
     * columns. Used later on the release to compare to see if any of the columns have changed.
     */
    @Override
    public void mousePressed(@SuppressWarnings("unused") MouseEvent e) {
      ...
      if (colModel != null) {
        ...
        for (int i = 0; i < count; i++) mi_saveColWidths[i] = colModel.getColumn(i).getWidth();
      }
    }

    /**
     * Handle the mouse released on the table column header. Checks all the current column widths
     * with the saved ones that we retrieved when the mousePressed method was called. If any of the
     * widths are different, we store all the current widths along with the node to the applet
     * persistance object.
     */
    @Override
      ...
      if (colModel != null && mi_saveColWidths != null) {
        ...
        for (int col = 0; col < count; col++) {
          ...
          if (mi_saveColWidths[col] != colModel.getColumn(col).getWidth()) {
            ...
            m_actManager.getApplet().saveColumnWidthsToOptions(m_parentNode, widths);
            break;
          }
        }
      }
    ...
    /**
     * Storage for the temporary list of column widths, set when the mouse is pressed in the table
     * header. When the mouse is released we will compare the current values with the saved values
     * in this list. If different we will then store the current values to our applet persistence
     * object.
     */
  ...
  /**
   * Gets the selected nodes based on the supplied component. If the component is table and the row
   * at the <code>loc</code> is selected, then the selected nodes are currently selected nodes in
   * the table. If the row at the <code>loc</code> is not selected, then that row will be selected
   * and returned. If the component is viewport of this panel, the parent node represented by this
   * panel is selected node.
   *
   * @param source the source component, assumed not <code>null</code> and one of the above
  ...
  private Iterator<PSNode> getSelectedNodes(Component source, Point loc) {
    ...
    // clicked on view port, not on table.
      ...
      if (m_parentNode != null) selNodes = Collections.singletonList(m_parentNode).iterator();
    }
      ...
      if (popupMenuAction != null) {
        PSContentExplorerMenu menu =
            new PSContentExplorerMenu(
                popupMenuAction, new PSMenuSource(selection), m_actManager, false);
   ...
   *     menu depends only on the selected nodes alone.
   * @param selNodes the list of selected nodes, if <code>null</code> or empty no pup-up menu will
   *     be shown.
   * @param loc the location where the pop-up menu should show, assumed not <code>null</code>
   */
  private void displayPopupMenu(
      Component comp, PSMenuAction action, Iterator<PSNode> selNodes, Point loc) {
    try {
      if (selNodes != null && selNodes.hasNext()) {

        PSNode parent = null;
        if (comp == m_childViewTable) parent = m_parentNode;
        else parent = m_parentParent;
        ...
        if (action == null) {
          if (backgroundMenu == null) createPopupMenu(selNodes);
          if (backgroundMenu != null) menu = backgroundMenu.get();
        ...
        if (menu != null) popup = menu.getPopupMenu();
  ...
  /**
   * Scans all rows in the table, checking each one for a match against the supplied node, using the
   * <code>equals</code> method.
   *
   * @param node Assumed not <code>null</code>.
   * @return The index of the node within the underlying table model, not the sorted table model,
   *     that matches the supplied node using the <code>equals</code> method, otherwise, -1 if no
   *     data in the table matches. The value will be between 0 and tableRowCount-1, inclusive.
   */
  private int getMatchingRowIndex(PSNode node) {
    ...
    for (int i = 0; i < rows; i++) {
      ...
      if (StringUtils.isNotBlank(n.getContentId())) {
        ...
        if (n.getContentId().equals(node.getContentId())) return i;
      } else {
        if (n.getName().equalsIgnoreCase(node.getName())) return i;
      }
    }
    return -1;
  }
  ...
  /**
   * Gets the table component that is displayed in the panel. The model of this table is <code>
   * PSTableSorter</code> that supports sorting. To get the actual data table model, call <code>
   * getModel()</code> on the sorter model. The actual data model is <code>PSDisplayFormatTableModel
   * </code>.
   *
  ...
  public void dragGestureRecognized(DragGestureEvent dge) {
    ...
    if (rows != null && rows.length > 0) {
      for (int i = 0; i < rows.length; i++) {
        ...
        if (m_actManager.canCopyOrMove(node)) draggableNodes.add(node);
        else m_childViewTable.removeRowSelectionInterval(rows[i], rows[i]);
      }
    }
    ...
    if (!draggableNodes.isEmpty()) {
      try {
        ...
        PSSelection sel =
            new PSSelection(new PSUiMode(m_view, ms_mode), m_parentNode, draggableNodes.iterator());
        clipBoard.setClip(PSClipBoard.TYPE_DRAG, sel);
        dge.startDrag(
            DragSource.DefaultCopyNoDrop, new PSDnDTransferable(), new PSDragSourceAdapter());
  ...
  private class PSNodeTableSorter extends PSTableSorter {
    /**
     * Constructs this table sorter model with the supplied model as data model. See super class for
     * more info.
     *
    ...
    /**
     * Overriden to sort the rows by grouping them based on node type and these are sorted based on
     * their sort order. See super's method description for more info.
     */
    @Override
    public int compareRowsByColumn(int row1, int row2, int column) {
      ...
      int nodeResult = (nodeOrder1 < nodeOrder2 ? -1 : (nodeOrder1 == nodeOrder2 ? 0 : 1));

      if (nodeResult == 0) return result;
      else return nodeResult;
    }
  }

  /**
   * The table cell renderer that supports rendering of all columns of the main display table. It
   * renders the cell whose value is <code>PSNode</code> to show the icon based on node type in
   * additional to its label. All other cells rendering is delegated to super to provide default
  ...
  private class MainDisplayTableCellRenderer extends DefaultTableCellRenderer {
    ...
    /**
     * Overridden method to apply display options if they are available in <code>UIManager</code>
     * defaults. See super for more description about this method.
     */
    @Override
    public Component getTableCellRendererComponent(
      ...
      if (value != null && value instanceof PSNode) {
        ...
        setIcon(PSImageIconLoader.loadIcon(getIconName(iconKey), false, m_applet));
        ...
        if (colType != null
          ...
          Icon icon = PSImageIconLoader.loadIcon(getIconName(value.toString()), false, m_applet);
          ...
          Date d = (Date) value;
          String dateStr = PSStringOperation.dateFormat(null, d, getDataModel().getLocale());
          setText(dateStr);
          setIcon(null);
        } else setIcon(null);
      }
  ...
  private class MainDisplayTableHeaderRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
        JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
      if (hasFocus) setFocusColumn(column);
      ...
      if (table.convertColumnIndexToModel(column)
          == m_childViewTableModel.getLatestSortingColumn()) {
  ...
  /**
   * A keyboard handler is attached to the table so that when the user tabs or uses directional
   * keys, we can navigate the table much the same as we do with a MouseEvent. This fixes the issue
   * with KeyBoard selection in the table is not setting context menus correctly.(It was setting the
   * last selected node in the navigation tree.
   *
   * @author vamsinukala
   */
  ...
  private class TableMouseInputHandler extends PSMouseAdapter implements MouseInputListener {
     ...
     *
     * @param e the mouse event recieved by this listener, assumed not <code>null</code>
     */
    private void setDispatchComponent(MouseEvent e) {
      ...
      m_dispatchComponent = SwingUtilities.getDeepestComponentAt(editorComponent, p2.x, p2.y);
    }

    /**
     * Posts the supplied event to the component (editor component) that should recieve the event if
     * it exists.
     *
     * @param e the mouse event recieved by this listener, assumed not <code>null</code>
    ...
    private boolean repostEvent(MouseEvent e) {
      if (m_dispatchComponent == null) return false;

      JTable table = (JTable) e.getSource();
      MouseEvent e2 = SwingUtilities.convertMouseEvent(table, e, m_dispatchComponent);
     ...
     * null</code>
     * @param flag supply <code>true</code> if caller is about to change the selection, otherwise
     *     <code>false</code>
     */
    ...
    /**
     * Checks whether the event can be ignored. A mouse event can be ignored if the event source
     * table is not enabled or it is not a left mouse button or event id is mouse dragged.
     *
    ...
    /**
     * Implemented to remember the mouse pressed location to use with mouse drag event. See {@link
     * #mouseDragged(MouseEvent)} for more description. See interface for more description about
     * this method.
     */
    @Override
    ...
    /**
     * Handles the mouse drag event to select the cell at the mouse press location clearing the
     * previous selection if it is not. See the interface for this method description.
     */
    public void mouseDragged(MouseEvent e) {
      ...
      if (editor == null || editor.shouldSelectCell(e)) {
        ...
        if (m_pressedLoc == null) m_pressedLoc = e.getPoint();
    ...
    /**
     * The component recieving mouse events during editing. May not be editor component. Used to
     * dispatch events when the mouse event occurs on the table while editing. This gets modified in
     * each mouse listener method to repost the event.
     */
  ...
  /**
   * The parent node whose children are the data of this panel's table, may be <code>null</code>
   * until a call to <code>setData(PSNode, PSNode)</code>.
   */
  private PSNode m_parentNode;

  /**
   * The parent of the <code>m_parentNode</code>, may be <code>null</code>. Modified through a call
   * to <code>setData(PSNode, PSNode)</code>.
   */
  private PSNode m_parentParent;

  /**
   * The table to show children of a node, initialized in <code>init()</code> and never <code>null
   * </code> after that. The table structure gets changed through calls to <code>
  ...
  /**
   * The table model used with <code>m_childViewTable</code> to allow sorting. Initialized in <code>
   * init()</code> and never <code>null</code> after that. The internal table model gets changed
  ...
  /**
   * The table cell renderer to use with <code>m_childViewTable</code> to render the cells of the
   * table, initialized in <code>init(PSNode)</code> and never <code>null</code> or modified after
   * that.
   */
  private MainDisplayTableCellRenderer m_childViewTableRenderer;

  /**
   * The action manager to use to provide context-sensitive menu, initialized in the ctor and never
  ...
  /**
   * A text area that is used to present information to the user when necessary. It takes the place
   * of the table in the viewpoint under those circumstances. Never <code>null</code> after
   * initialization in the {@link #init()} method.
   */
  private JTextArea m_messageText;

  /**
   * Holds the current popup menu. This is created on selection of items and is populated in the
   * background. The popop is then shown on menu click etc.
   */
  ...
  /**
   * If <code>true</code>, the table will be sortable, if <code>false</code>, it will not. Set
   * during the ctor, never modified after that.
   */
  private boolean m_isSorted;

  /**
   * The mode (Main Display) that is represented by this panel. See {@link PSUiMode#TYPE_MODE_MAIN}
   * for more info.
   */
  ...
  static {
    ms_displayNameToIconName.put(
        com.percussion.cms.IPSConstants.CHECKOUT_STATUS_NOBODY, "CheckedIn");
    ms_displayNameToIconName.put(
        com.percussion.cms.IPSConstants.CHECKOUT_STATUS_SOMEONEELSE, "CheckedOut");
    ms_displayNameToIconName.put(
