...
public final class PSActionBar extends JPanel
   ...
   *
   * @param applet the applet that provides action manager to handle actions and the parameters, may
   ...
   * @param view The view of the applet this action bar currently representing, must be one of the
   *     <code>PSUiMode.TYPE_VIEW_xxx</code> values.
   * @param relationships the set of available relationships for dependency tree view, may not be
   *     <code>null</code> or empty if the supplied view is <code>PSUiMode.TYPE_VIEW_DT</code>.
  ...
  public PSActionBar(
      ...
      IPSViewChangeListener listener) {
    ...
    if (view.equals(PSUiMode.TYPE_VIEW_DT)
      ...
      throw new IllegalArgumentException(
          "relationships may not be null or empty for " + PSUiMode.TYPE_VIEW_DT + " view");
    }
    ...
    if (listener != null) addViewChangeListener(listener);
   ...
   *
   * <p>Adds the menus of the supplied menu bar to this menu bar and a tool bar to display content
   * path in <code>PSUiMode.TYPE_VIEW_CX</code> and <code>
   * PSUiMode.TYPE_VIEW_IA</code> views.
   *
   * <p>In case of <code>PSUiMode.TYPE_VIEW_DT</code> view a drop-down list with available
   * relationships is added in addition to menu. Content Item is shown as label in this action bar.
   * Adds the selection listener for drop-down list to inform its view change listeners.
   *
   * @param menu The additional top-level menu it has to show, if <code>null
   * </code>, this is not added to this menu bar, otherwise it is added.
  ...
  private void init(JMenuBar menu, PSRelationshipInfoSet relationshipSet) {
    ...
    if (m_view.equals(PSUiMode.TYPE_VIEW_DT)) {
      ...
      JLabel rsLabel =
          new JLabel(
              m_applet.getResourceString(getClass().getName() + "@Relationship:"),
              SwingConstants.LEFT);
      rsLabel.setFocusable(true);
      char mnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "Relationship:", 'R');
      ...
      Collections.sort(
          orderedRels,
          new Comparator<PSRelationshipInfo>() {
      ...
      String relName = m_applet.getParameter(PSContentExplorerConstants.PARAM_RELATIONSHIP_NAME);
      if (relName == null) relName = StringUtils.EMPTY;
      ...
      JLabel label =
          new JLabel(m_applet.getResourceString(getClass().getName() + "@Content Item:"));
      label.setIcon(PSImageIconLoader.loadIcon(CONTENT_PATH_SEPARATOR_ICON, false, m_applet));
      label.setFocusable(true);
      labelPanel.add(label);
      labelPanel.add(makeFillerOpaque(Box.createHorizontalStrut(5)));

      PSNameLabel nameLabel = new PSNameLabel();
      nameLabel.setText(m_applet.getParameter(PSContentExplorerConstants.PARAM_ITEM_TITLE));
      ...
      JLabel label =
          new JLabel(m_applet.getResourceString(getClass().getName() + "@Content Path:") + "  ");
      label.setIcon(PSImageIconLoader.loadIcon(CONTENT_PATH_SEPARATOR_ICON, false, m_applet));
      ...
      Dimension buttonSize =
          new Dimension(refreshIcon.getIconWidth() + 5, refreshIcon.getIconHeight() + 5);
      ...
      m_refreshButton
          ...
          .setAccessibleName("Click to refresh selected node in the navigation pane");
      m_refreshButton
          .getAccessibleContext()
          .setAccessibleDescription(
              "Click to refresh selected node in the navigation pane description");
      final PSMenuAction refreshAction = new PSMenuAction(IPSConstants.ACTION_REFRESH, "refresh");
      String tooltip = m_applet.getResourceString(getClass(), "@Refresh View");
      m_refreshButton.setToolTipText(tooltip);
      m_refreshButton.addActionListener(
          new ActionListener() {
            public void actionPerformed(@SuppressWarnings("unused") ActionEvent e) {
              if (m_navSelection != null)
                m_applet.getActionManager().executeAction(refreshAction, m_navSelection);
            }
          });
  ...
  /**
   * Helper method to make a Box Filler opaque which is needed for Windows Vista to render
   * background color through the filler.
   *
   * @param filler the filler object may be <code>null</code> or not a Filler component in which
   *     case it will just be passed back untouched.
  ...
  /**
   * Adds the view change listener to its list of listeners that are interested in change of view.
   *
  ...
  /**
   * Notification event that the current selection has changed in main view. Updates the content
   * path that is shown in this panel if there is navigational selection change.
   *
  ...
  public void selectionChanged(PSSelection selection) {
    ...
    if (node != null && node.getSearchResultCount() >= 0) {
      ...
      String resultlabel = m_applet.getResourceString(getClass().getName() + "@ResultCount");
  ...
  /**
   * Worker method for selection change event in 'Relationships' combo-box. Gets the selected
   * relationship and requests the server to get the dependendencies of the content item for this
   * relationship. Then informs the listeners that are interested in the view change according to
   * user action.
   *
   * @param e the change event, assumed not <code>null</code> as this method is called by Swing
   *     model.
   */
  public void itemStateChanged(ItemEvent e) {
    if (e.getStateChange() == ItemEvent.SELECTED) {
      ...
      props.setProperty(IPSConstants.PROPERTY_CONTENTID, String.valueOf(m_contentItem.getId()));
      props.setProperty(
  ...
  public void searchInitiated(@SuppressWarnings("unused") PSNode node) {
    String label = m_applet.getResourceString(getClass().getName() + "@SearchInProgress");
  ...
  /**
   * The object that represents the content item whose dependencies are shown in the view panel for
   * the selected relationship in this action bar in case of <code>PSUiMode.TYPE_VIEW_DT</code>
  ...
  /**
   * The text field that displays the navigational selection in 'CX' view, initialized to an empty
   * text field and the text may be modified as selection changes. Never <code>null</code> after
   * initialization.
   */
  private JTextField m_pathLabelField = new PSPathLabel();

  /**
   * The label indicates the current state of a search. When a search is not underway, this
   * component is hidden. When a search has been started it states results pending. When a search
   * has been completed it states the number of results found.
   */
  private JTextField m_resultsLabel = new JTextField();

  /**
   * The combo-box that displays available relationships for a content item to choose the
   ...
   * this action bar. Listeners gets added through calls to <code>
   ...
   * initialization and stored as part of UIManager.
   */
  public static final String CONTENT_PATH_SEPARATOR_ICON = "ContentPathSeparator";

  /**
   * Name of the image file used for the refresh button. Image is loaded on initialization and
   * stored as part of UIManager.
   */
  public static final String REFRESH_ICON = "Refresh";

  /**
   * Button to refresh view, initialized during construction, never <code>null</code> or modified
   * after that.
   */
