...
public class PSItemRelationshipsManager {
  ...
  public PSItemRelationshipsManager(
    ...
    if (remCataloger == null) throw new IllegalArgumentException("remCataloger may not be null.");

    if (docBase == null) throw new IllegalArgumentException("docBase may not be null.");
  ...
  /**
   * Loads the parent/child dependencies of the object(item/folder) identified by the locator of the
   * node for a relationship. Assumes all these values are specified as properties of the node. The
   * following list describes all required properties. Ignores
   *
   * <ol>
   *   <li>PSContentExplorerApplet.PROP_CONTENTID
   *   <li>PSContentExplorerApplet.PROP_REVISIONID
   *   <li>PSContentExplorerApplet.PROP_RELATIONSHIP
   *   <li>PSContentExplorerApplet.PROP_RS_LOOKUP_TYPE
   *   <li>Value for above property - ('ancestors'/'descendants')
   * </ol>
   *
   * @param parentNode the node for which the dependencies need to be loaded, may not be <code>null
   *     </code> and must have above mentioned properties.
  ...
  public Iterator<PSNode> loadDependencies(PSNode parentNode)
    ...
    if (relationship == null || relationship.trim().length() == 0)
      throw new IllegalArgumentException("parentNode not specified with a relationship");
    ...
    if (rsType.equals(RS_LOOKUP_DESCENDANTS)) {
      if (relInfo.getCategory().equals(PSRelationshipConfig.CATEGORY_FOLDER)) {
        ...
        for (PSComponentSummary summary : arrySummaries) summaries.add(summary);
      } else if (relInfo
          .getCategory()
          .equalsIgnoreCase(PSRelationshipConfig.CATEGORY_ACTIVE_ASSEMBLY)) {
        filter.setOwner(key);
        // Filtering only in case of AA dependents See BugId: RX-16352
        summaries = getFilteredRelationshipSummaries(filter, parentNode.getContentId());
      } else {
        filter.setOwner(key);
      ...
      if (relInfo.getCategory().equals(PSRelationshipConfig.CATEGORY_ACTIVE_ASSEMBLY)) {
        PSComponentSummaries testSummaries = m_proxy.getSummaries(filter, true);
    ...
    while (it.hasNext()) {
      ...
      if (dependent.isFolder()) {
        ...
        if (id == PSFolder.ROOT_ID) break;

        String type = m_folderActionMgr.getFolderType(dependent, parentNode);
        ...
        props.setProperty(IPSConstants.PROPERTY_CONTENTID, String.valueOf(id));
        props.setProperty(IPSConstants.PROPERTY_REVISION, String.valueOf(locator.getRevision()));
  ...
  /**
   * Filters the relationships based on whether the slots are still registered to the content types
   * through their allowed variants. Gets the related content document and gets the sys_slotid
   * property for each relationship, and checks whether that slot id is a registered slot to the
   * content type of the item or not, if not filters that relationship if the associated slot is not
   * an inline slot. Relationships to inline slots will never be be filtered.
   *
  ...
  private PSComponentSummaries getFilteredRelationshipSummaries(
    ...
    if (filter == null) throw new IllegalArgumentException("Relationship filter cannot be null.");
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      String slotId = null;
      String cid = null;
      Element plelem = (Element) elem.getElementsByTagName("PropertyList").item(0);
      ...
      Element depLoc =
          (Element)
              ((Element) elem.getElementsByTagName("Dependent").item(0))
                  .getElementsByTagName("PSXLocator")
                  .item(0);
      try {
  ...
  /**
   * Makes a request to a resource and gets the slots registered to the all allowable templates the
   * content type of the supplied item.
   *
   * @param contentId must not be <code>null</code>.
   * @return List of registered slots through templates to the supplied item.
   * @throws PSCmsException in case of error.
   */
  private List<String> getRegisteredSlots(String contentId) throws PSCmsException {
    if (StringUtils.isBlank(contentId))
      throw new IllegalArgumentException("contentId cannot be null.");
    ...
    try {
      Document doc = m_applet.getXMLDocument("/sys_rcSupport/contentslotvariantlist.xml", params);
      NodeList nodes = doc.getElementsByTagName("slot");
      for (int i = 0; i < nodes.getLength(); i++) {
        Element elem = (Element) nodes.item(i);
        String slotid = elem.getAttribute("slotid");
        if (slotid.trim().length() > 0) regSlots.add(slotid);
      }
    } catch (Exception e) {
      Object[] args = new Object[] {e.getLocalizedMessage()};
      throw new PSCmsException(CmsErrorCodes.UNEXPECTED_ERROR, args);
  ...
  private List<String> getInlineSlots() throws PSCmsException {
    ...
    try {
      Document doc = m_applet.getXMLDocument("/sys_Slots/slotlist.xml", params);
      NodeList nodes = doc.getElementsByTagName("slot");
      for (int i = 0; i < nodes.getLength(); i++) {
        Element elem = (Element) nodes.item(i);

        Node slotId = PSXMLDomUtil.findFirstNamedChildNode(elem, "slotid");
        if (slotId != null) {
  ...
  /**
   * Returns the document consisting of the relationships based on the supplied filter. Assumes the
   * filter is set with appropriate parameters.
   *
   * @param filter must not be <code>null</code>
   * @return Document corresponding to PSXRelationshipSet dtd.
   * @throws PSCmsException in case of error.
   */
  private Document getRelationshipDoc(PSRelationshipFilter filter) throws PSCmsException {
    ...
    try {
      doc =
          m_remCataloger
              .getRemoteRequester()
              .getDocument("sys_ceFieldsCataloger/Relationship", params);
  ...
  /** Used to get folder node types. Set in ctor, then never <code>null</code> or changed. */
  private PSFolderActionManager m_folderActionMgr = null;

  /**
   * The remote cataloger used to catalog available set of relationships, initialized in the ctor
   * and never <code>null</code> or modified after that.
   */
  private PSRemoteCataloger m_remCataloger;

  /**
   * The applet document base URL used to make any server requests, initialized in the ctor and
   * never <code>null</code> or modified after that.
   */
  private URL m_docBase;

  /**
   * A reference back to the applet initialized in the ctor and never <code>null</code> after that.
   */
  private PSContentExplorerApplet m_applet;

  /**
   * The name of the property that defines the lookup type of the relationship, whose value defines
   * whether we are looking for 'Ancestors' or 'Descendants' of a particular relationship.
   */
