...
public class PSDisplayOptionsDialog extends PSDialog {
  ...
  public PSDisplayOptionsDialog(
      ...
      PSDisplayOptions defaultOptions,
      PSContentExplorerApplet applet) {
    super(parent, applet.getResourceString(PSDisplayOptionsDialog.class, "Display Options"));

    if (userOptions == null) throw new IllegalArgumentException("userOptions may not be null.");
  ...
  private void initDialog() {
    ...
    String[] labels_2 = {
      "Bold", "Italic", "Main Menu Text color:", "Font:", "Context Menu Text Color"
    };
    ...
    String bgLabel = m_applet.getResourceString(getClass(), "@Background color:");
    char bgMnemonic =
        PSContentExplorerApplet.getResourceMnemonic(getClass(), "@Background color:", 'K');
    String bgTooltip = PSContentExplorerApplet.getResourceTooltip(getClass(), "@Background color:");
    commonOptionsPanel.addPropertyRow(
        bgLabel, m_bgColorPanel, m_bgColorPanel.m_brButton, bgMnemonic, bgTooltip);
    JPanel optsPanel = new JPanel(new BorderLayout());
    optsPanel.add(commonOptionsPanel, BorderLayout.WEST);

    m_mainDisplayPanel = new OptionsPanel("Main Display Options", labels_1, mnemonics_1, "MD");
    m_mainDisplayPanel.setBorder(
        BorderFactory.createCompoundBorder(
            BorderFactory.createTitledBorder(
                m_applet.getResourceString(getClass(), "@Main Display Options")),
            BorderFactory.createEmptyBorder(5, 5, 5, 5)));

    // setup Heading text color control
    m_headingTextColorPanel = new ColorPanel();
    String htLabel = m_applet.getResourceString(getClass(), "@Heading text color:");
    char htMnemonic =
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Heading text color:", 'X');
    String htTooltip =
        PSContentExploreAppletUtils.getResourceTooltip(getClass(), "@Heading text color:");
    ...
    String hltLabel = m_applet.getResourceString(getClass(), "@Highlight color:");
    char hltMnemonic =
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Highlight color:", 'R');
    String hltTooltip =
        PSContentExploreAppletUtils.getResourceTooltip(getClass(), "@Highlight color:");
    m_mainDisplayPanel.addPropertyRow(
        hltLabel, m_hltColorPanel, m_hltColorPanel.m_brButton, hltMnemonic, hltTooltip);

    // setup Highlight text color control
    m_hltTextColorPanel = new ColorPanel();
    String hltTextLabel = m_applet.getResourceString(getClass(), "@Highlight text color:");
    char hltTextMnemonic =
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Highlight text color:", 'H');
    String hltTextTooltip =
        PSContentExploreAppletUtils.getResourceTooltip(getClass(), "@Highlight text color:");
    m_mainDisplayPanel.addPropertyRow(
        hltTextLabel,
        m_hltTextColorPanel,
    ...
    String fcLabel = m_applet.getResourceString(getClass(), "@Focus color:");
    char fcMnemonic =
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Focus color:", 'S');
    String fcTooltip = PSContentExploreAppletUtils.getResourceTooltip(getClass(), "@Focus color:");
    m_mainDisplayPanel.addPropertyRow(
        fcLabel, m_focusColorPanel, m_focusColorPanel.m_brButton, fcMnemonic, fcTooltip);

    m_menuPanel = new OptionsPanel("Menu Options", labels_2, mnemonics_2, "MN");
    m_menuPanel.setBorder(
        BorderFactory.createCompoundBorder(
            BorderFactory.createTitledBorder(
                m_applet.getResourceString(getClass(), "@Menu Options")),
            BorderFactory.createEmptyBorder(5, 5, 5, 5)));

    // add command panel
    m_defaultButton = new UTFixedButton(m_applet.getResourceString(getClass(), "@Default"));

    m_defaultButton.setMnemonic(
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Default", 'D'));

    m_systemDefaultCheck =
        new JCheckBox(m_applet.getResourceString(getClass(), "@Use OS Settings"));
    m_systemDefaultCheck.setMnemonic(
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Use OS Settings", 'U'));
    ...
    m_systemDefaultCheck.addActionListener(
        new ActionListener() {
  ...
  /**
   * Enable or Disable all components recursively except the command button panel.
   *
   * @param comp top level {@link Container container} to disable. All child components will be
   *     disabled except {@link UTStandardCommandPanel} if it exists as a child. Assumed not <code>
   *     null</code>.
   * @param disable <code>true</code> to enable and <code>false</code> to disable.
   */
  protected void disableComponent(Component comp, boolean disable) {
    if (comp instanceof UTStandardCommandPanel) return;
    if (comp instanceof Container) {
      Component[] comps = ((Container) comp).getComponents();
      for (int i = 0; i < comps.length; i++) disableComponent(comps[i], disable);
    }
  ...
  private void applyOptions(PSDisplayOptions options) {
    if (options.getBackGroundColor() != null) m_bgColorPanel.setColor(options.getBackGroundColor());

    if (options.getHighlightColor() != null) m_hltColorPanel.setColor(options.getHighlightColor());

    if (options.getHighlightTextColor() != null)
      m_hltTextColorPanel.setColor(options.getHighlightTextColor());

    if (options.getFocusColor() != null) m_focusColorPanel.setColor(options.getFocusColor());

    if (options.getForeGroundColor() != null)
      m_mainDisplayPanel.setTextColor(options.getForeGroundColor());

    if (options.getFont() != null) m_mainDisplayPanel.setDisplayFont(options.getFont());
    ...
    if (options.getMenuFont() != null) m_menuPanel.setDisplayFont(options.getMenuFont());
  ...
  /**
   * Gets the display options from the dialog controls and updates user display options. Hides and
   * disposes the dialog.
   */
  public void onOk() {
    ...
    m_userOptions.setHeadingForeGroundColor(m_headingTextColorPanel.getColor());
  ...
  /**
   * The panel that represents a text field and a button to choose different colors for various
   * options.
   */
  private class ColorPanel extends JPanel {
    /**
     * Constructs the panel with a text field and a browse button to choose color and set that color
     * as background for the text field.
     */
    public ColorPanel() {
            ...
            public void actionPerformed(ActionEvent e) {
              Color color =
                  JColorChooser.showDialog(
                      ...
                      m_applet.getResourceString(getClass(), "@Choose color"),
  ...
  private class OptionsPanel extends PSPropertyPanel {
    ...
    public OptionsPanel(String category, String[] labels, char[] mnemonics, String panel) {
      ...
      Font[] availFonts = GraphicsEnvironment.getLocalGraphicsEnvironment().getAllFonts();

      for (int i = 0; i < availFonts.length; i++) m_fontNameField.addItem(availFonts[i].getName());
      ...
      m_boldFontCheck.setMnemonic(
          PSContentExploreAppletUtils.getResourceMnemonic(
              getClass().getName() + category, labels[0], mnemonics[0]));

      m_italicFontCheck = new JCheckBox(m_applet.getResourceString(getClass(), "@" + labels[1]));
      m_italicFontCheck.setMnemonic(
          PSContentExploreAppletUtils.getResourceMnemonic(
              getClass().getName() + category, labels[1], mnemonics[1]));
      ...
      String tcLabel = m_applet.getResourceString(getClass(), "@" + labels[2]);
      char tcMnemonic =
          PSContentExploreAppletUtils.getResourceMnemonic(
              getClass().getName() + category, labels[2], mnemonics[2]);
      String tcTooltip =
          PSContentExploreAppletUtils.getResourceTooltip(
              getClass(), "@" + category.substring(1) + "." + labels[2]);

      addPropertyRow(tcLabel, m_txtColorPanel, m_txtColorPanel.m_brButton, tcMnemonic, tcTooltip);

      // setup Text color control
      if (panel.equalsIgnoreCase("MN")) {
        String cmtcLabel = m_applet.getResourceString(getClass(), "@" + labels[4]);
        char cmtcMnemonic =
            PSContentExploreAppletUtils.getResourceMnemonic(
        ...
        addPropertyRow(
            cmtcLabel, m_cmTxtColorPanel, m_cmTxtColorPanel.m_brButton, cmtcMnemonic, cmtcTooltip);
      }
      // setup Font combo box
      String fontLabel = m_applet.getResourceString(getClass(), "@" + labels[3]);
      char fontMnemonic =
          PSContentExploreAppletUtils.getResourceMnemonic(
      ...
      addPropertyRow(fontLabel, fontPanel, m_fontNameField, fontMnemonic, fontTooltip);
    ...
    public Font getDisplayFont() {
      ...
      if (m_boldFontCheck.isSelected()) style = Font.BOLD;

      if (m_italicFontCheck.isSelected()) style = Font.ITALIC;
     ...
     * </code> and never <code>null</code> or modified after that. The user options font name will
     ...
     * </code> and never <code>null</code> or modified after that. The user options font size will
     * be selected initially.
     */
     ...
     * </code> and never <code>null</code> or modified after that. Initially it will be checked if
     * the user options font style is 'BOLD'.
     */
     ...
     * </code> and never <code>null</code> or modified after that. Initially it will be checked if
     * the user options font style is 'ITALIC'.
     */
    private JCheckBox m_italicFontCheck;

    /**
     * The panel that contains a text field and a button to choose foreground color, initialized in
     * <code>initDialog()</code> and never <code>null
     * </code> or modified after that. The user options foreground color will be set as background
     * of text field initially.
     */
     ...
     * The panel that contains a text field and a button to choose foreground color, initialized in
     * <code>initDialog()</code> and never <code>null
     * </code> or modified after that. The user options text foreground color will be set as
     * background of text field initially.
     */
    private ColorPanel m_cmTxtColorPanel;
  }

  /**
   * The panel that contains a text field and a button to choose background color, initialized in
   * <code>initDialog()</code> and never <code>null
   * </code> or modified after that. The user options background color will be set as background of
   * text field initially.
   */
  private ColorPanel m_bgColorPanel;

  /**
   * The panel that contains a text field and a button to choose highlight (selection) color,
   * initialized in <code>initDialog()</code> and never <code>null</code> or modified after that.
   * The user options hightlight color will be set as background of text field initially.
   */
  private ColorPanel m_hltColorPanel;

  /**
   * The panel that contains a text field and a button to choose highlight text (selection) color,
   * initialized in <code>initDialog()</code> and never <code>null</code> or modified after that.
   * The user options hightlight color will be set as background of text field initially.
   */
  ...
  /**
   * The panel that contains a text field and a button to choose heading text color in main display
   * panel, initialized in <code>initDialog()</code> and never <code>null</code> or modified after
   * that. The user options heading text color will be set as background of text field initially.
   */
  private ColorPanel m_headingTextColorPanel;

  /**
   * The panel that contains a text field and a button to choose focus color in main display panel,
   * initialized in <code>initDialog()</code> and never <code>null</code> or modified after that.
   * The user options focus color will be set whenever focus is on a particular control.
   */
  private ColorPanel m_focusColorPanel;

  /**
   * The panel that contains controls to select a font and text color for main display, initialized
   * in <code>initDialog()</code> and never <code>null</code> or modified after that.
   */
  private OptionsPanel m_mainDisplayPanel;

  /**
   * The panel that contains controls to select a font and text color for menu, initialized in
   * <code>initDialog()</code> and never <code>null</code> or modified after that.
   */
  ...
  /**
   * The check-box to choose OS default settings, initialized in <code>initDialog()</code> and never
   * <code>null</code> or modified after that. Initially it will be un checked.
   */
