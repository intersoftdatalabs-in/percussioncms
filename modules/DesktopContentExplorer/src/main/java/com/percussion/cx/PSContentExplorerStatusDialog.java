...
public class PSContentExplorerStatusDialog extends PSDialog {
  ...
  public PSContentExplorerStatusDialog(
      Frame parent, PSProcessMonitor monitor, PSContentExplorerApplet applet) {
    super(parent, applet.getResourceString(PSContentExplorerStatusDialog.class, "Process Status"));
  ...
  /**
   * Initializes the dialog framework with progress bar and labels for job description. Sets the
   * dialog title also.
   *
   * @param title the title of the dialog, assumed not <code>null</code> or empty.
   */
  private void initDialog() {
    ...
    progressPanel.add(
        new JLabel(m_applet.getResourceString(getClass(), "Status"), SwingConstants.LEFT));
  ...
  public void onCancel() {
    ...
    int option =
        JOptionPane.showConfirmDialog(
            this,
            m_applet.getResourceString(getClass(), "Are you sure you want to cancel this process?"),
            m_applet.getResourceString(getClass(), "Cancel process"),
  ...
  /**
   * Requests the Swing event dispatch thread to update the progress bar and status message and
   * returns immediately.
   *
   * @param statusMessage the status message to show, may be <code>null</code> or empty.
   * @param percentDone The percent complete to set on the progress bar. Must be between 0 and 100.
  ...
  public void updateStatus(final String statusMessage, final int percentDone) {
    if (percentDone < 0 || percentDone > 100)
      throw new IllegalArgumentException("percentDone must be between 1 and 100.");
  ...
  /**
   * Hides the dialog if it is showing. Should be called when the process monitored by this dialog
   * is completed.
   */
  ...
  /**
   * Displays an error dialog for the supplied message. The message will be displayed to the user
   * and he has the possiblity to continue or abort the current process.
   *
  ...
  public void showError(String message) {
    ...
    String label = m_applet.getResourceString(getClass(), "Do you want to continue?");
  ...
  public static int displayErrorDialog(
      Component parent, String title, String label, String message) {
    ...
    errordata[1] = pane;
    JOptionPane optPane =
        new JOptionPane(errordata, JOptionPane.ERROR_MESSAGE, JOptionPane.YES_NO_OPTION);
