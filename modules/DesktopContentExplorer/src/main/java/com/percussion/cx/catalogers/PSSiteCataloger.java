...
public class PSSiteCataloger {
   ...
   */
  private void init() throws PSCmsException {
    try {
      URL url = new URL(m_urlBase, "sys_pubSites/getSites.xml");
  ...
  /**
   * Loads the site names from the supplied XML element. For the expected DTD see {@link
   * PSSite#toXml(Document)}.
   *
   * @param elemRoot the XML element from which to load the site names, assumed not <code>null
   *     </code>.
   ...
   */
  public void refresh() throws PSCmsException {
    init();
  }
  ...
  /**
   * Used to make requests to the Rx server. Set in ctor, then never <code>null</code>, or modified.
   */
