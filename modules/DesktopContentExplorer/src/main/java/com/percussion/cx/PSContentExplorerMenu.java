 ...
 * {@link PSMenuAction}. This class implements the Callable interface so
...
public class PSContentExplorerMenu implements PopupMenuListener, Callable<PSContentExplorerMenu> {
  ...
  private void buildChildMenus() {
    ...
    String more = m_applet.getResourceString(getClass().getName() + "@MORE");
    ...
    while (children.hasNext()) {
      ...
      if (childAction.isMenu()) {
        ...
        if (m_menuSource.getSource() != null) handleMenuAction(childAction, (JMenu) menu);
        ((JMenu) menu).getPopupMenu().addPopupMenuListener(this);
      } else {
        String menuChecked = childAction.getProperty(IPSConstants.PROPERTY_MENU_ITEM_CHECKED);
        if (menuChecked != null && menuChecked.equalsIgnoreCase(IPSConstants.PROPERTY_TRUE)) {
          menu = new JCheckBoxMenuItem(childMenuAction);
          ((JCheckBoxMenuItem) menu).setState(true);
        } else menu = new JMenuItem(childMenuAction);
      }
      menu.setFocusTraversalKeysEnabled(true);
      menu.setFocusable(true);
      menu.setFont(m_actManager.getApplet().getOptionsManager().getDisplayOptions().getMenuFont());
      menu.setForeground(
          m_actManager
              .getApplet()
              .getOptionsManager()
              .getDisplayOptions()
              .getContextMenuForeGroundColor());
      menu.setOpaque(true);
  ...
  /**
   * Builds <code>JMenu</code> from this menu's action. Adds the menu elements to this menu for the
  ...
  /**
   * Builds the <code>Action</code> for the supplied <code>PSAction</code> element. Set the
   * accelerator, mnemonic keys, tooltip and icon properties on the <code>Action</code> object if
   * available.
   *
   * @param action the action, assumed not <code>null</code>.
   * @return the swing action, never <code>null</code>
   * @throws RuntimeException if the icon url is specified and is unable to load the icon
   */
  private Action buildAction(PSMenuAction action) {
    ...
    if (action.getProperties() != null) {
      ...
      if (accelKey != null) accKey = KeyStroke.getKeyStroke(accelKey);

      String mnemChar = props.getProperty(PSMenuAction.PROP_MNEM_KEY);
      if (mnemChar != null && mnemChar.trim().length() > 0) mnem = mnemChar.charAt(0);

      tooltip = props.getProperty(PSMenuAction.PROP_SHORT_DESC);

      String url = props.getProperty(PSMenuAction.PROP_SMALL_ICON);
      try {
        if (url != null && url.trim().length() > 0) icon = m_actManager.getIcon(url);
      } catch (Exception ex) {
        throw new RuntimeException(
            "Error getting icon specified by menu url " + ex.getLocalizedMessage());
      }
    }

    AbstractAction menuAction =
        new AbstractAction(menuLabel, icon) {
    ...
    if (tooltip != null) menuAction.putValue(Action.SHORT_DESCRIPTION, tooltip);
  ...
  /**
   * Handles the menu action using action manager. Gets the sub-menu actions from the action manager
   * and then builds the sub-menu for it. If the menu selected is a context-menu, the selection
   * source will be the context source of this menu, otherwise the selection source is the source of
   * this menu. After loading the sub-menu, checks and updates the context-submenu state if it has
   * any as specified in <code>
  ...
  private void handleMenuAction(PSMenuAction action, JMenu source) {
    ...
    if (action.isContextMenu()) {
      ...
      if (ctxAction != null) {
        ...
        subMenu = new PSContentExplorerMenu(ctxAction, menuSource, m_actManager);
      }
    } else {
      PSMenuAction childAction = m_actManager.getContextMenu(action, m_menuSource.getSource());
      if (childAction != null) {
        subMenu = new PSContentExplorerMenu(childAction, m_menuSource, m_actManager);
      }
    }
    source.removeAll();
    if (subMenu != null && subMenu.getMenuElements().length > 0) {
      MenuElement[] menuElements = subMenu.getMenuElements();
      for (int i = 0; i < menuElements.length; i++) source.add(menuElements[i].getComponent());
    }
  }

  /**
   * Finds the context menu from the supplied list of menu elements and update its state according
   * to the current context selection. If there is no context selection, makes the context menu
   * invisible, otherwise makes it visible and the label of menu represents the selection. If the
   * context selection represents multiple nodes, then it displays label as 'Group'.
   *
   * @param menuElements the list of menu elements, assumed not <code>null
   * </code>, if empty nothing is done. Limitation: Currently, we are relying on the first character
   *     to be the mnemonic. Swing doesn't let you recycle among common mnemonics. This needs to be
   *     addressed in the next rev by a customizable scheme.
   */
  private void updateContextMenuState(JMenu subMenu) {
    ...
    if (childAction.isContextMenu()) {
      ...
      if (contextSel == null) {
        ...
        String label;
        if (selection.size() == 1) label = ((PSNode) selection.get(0)).getLabel();
        else label = m_applet.getResourceString(getClass().getName() + "@Group");
  ...
  private void handleAction(Object source) {
    if (source instanceof JMenuItem) {
      ...
      PSMenuAction action = (PSMenuAction) menuItem.getAction().getValue(ACTION_OBJECT);
   ...
   *
   * @return the list of menu elements, may be empty, never <code>null</code>, each element of the
   *     list can be either an instanceof <code>JMenuItem
  ...
  /**
   * Estimates the maximum number of items that can show up on a menu without "falling off" the
   * screen. This is determined by calculating the line height for each item based on the current
   * font.
   *
  ...
  private int getEstimatedMaxItems(Component menu) {
    ...
    int lineHeight =
        metrics.getHeight() + metrics.getDescent() + metrics.getLeading() + metrics.getAscent();

    return (getAvailableScreenHeight() / lineHeight);
  }

  /**
   * Returns the available screen height, this is the screen height minus the taskbar (or best
   * estimation if ran under Java 1.3).
   *
  ...
  /**
   * There seems to be an issue in the 1.4.1_xx plugin where by they are not correctly honoring
   * signed applets. There is a work around to get the applet to work by simply adding a permission
   * in the plugins' security policy file. The file path is :
   * {java_plugin_home}\lib\security\java.policy So on most windows systems it would be found at:
   * c:\Program Files\Java\j2re1.4.1_xx\lib\security\java.policy Just open that file and append the
   * following entry to the end of the file: grant { permission java.lang.RuntimePermission
  ...
  private static void printAccessControlExceptionInfo() {
    ...
    log.error(
        "A security exception was thrown while "
  ...
  /**
   * The source of this menu on which this menu should act on, initialized in the ctor and never
   * <code>null</code> or modified after that.
   */
  private PSMenuSource m_menuSource;

  /**
   * The action manager to use to execute any action, initialized in the ctor and never <code>null
   * </code> or modified after that.
   */
  private PSActionManager m_actManager;

  /**
   * The action to use to generate the menu initialized in the ctor and never <code>null</code> or
   * modified after that.
   */
  private PSMenuAction m_action;

  /**
   * The visual/swing menu that represents this object, initialized in ctor through a call to <code>
   * buildMenu(PSAction)</code> and never <code>null
   * </code> or modified after that.
   */
  private JMenu m_menu;

  /**
   * The constant to use to store the action (<code>PSAction</code>) in menu item action (<code>
   * javax.swing.Action</code>).
   */
  private static final String ACTION_OBJECT = "Action Object";

  /** Font to use for the popup menu text */
  public static final Font POPUP_MENU_FONT = new Font("Arial", Font.PLAIN, 11);
