...
public class PSSimpleSwingBrowser extends PSDesktopExplorerWindow {
  ...
  public JFrame instanceOpen() {
    ...
    if (this.windowLoaded) {
      ...
      webView = new WebView();
      // webView.getEngine().setOnAlert((EventHandler<WebEvent<String>>) event ->
      // log.info(event.getData()));
      webView
          .getEngine()
          .setOnError((EventHandler<WebErrorEvent>) event -> log.error(event.getMessage()));
  ...
  private void initComponents(String mi_actionurl) {
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          this.jfxPanel.setSize(
              new Dimension(this.browserProps.getWidth(), this.browserProps.getHeight()));
          this.jfxPanel.setPreferredSize(
              new Dimension(this.browserProps.getWidth(), this.browserProps.getHeight()));
  ...
  public void loadURL(final String url) {
    ...
    if (PSSimpleSwingBrowser.this.mi_actionurl != url) PSSimpleSwingBrowser.this.mi_actionurl = tmp;
  ...
  private void createScene() {
    ...
    PSSimpleSwingBrowser.this.engine.setOnResized(
        new EventHandler<WebEvent<Rectangle2D>>() {
          ...
          public void handle(WebEvent<Rectangle2D> event) {
            ...
            PSSimpleSwingBrowser.this.setSize(
                (int) event.getData().getWidth(), (int) event.getData().getHeight());
          }
        });
    ...
    PSSimpleSwingBrowser.this.engine.setOnVisibilityChanged(
        e1 -> {
          if (!e1.getData()) {
    ...
    PSSimpleSwingBrowser.this.engine.setConfirmHandler(
        message -> {
    ...
    PSSimpleSwingBrowser.this.engine.setOnAlert(
        event -> {
            ...
            (o, old, value) -> {
              if (PSSimpleSwingBrowser.this.engine.getLoadWorker().getState() == FAILED) {
                ...
                JOptionPane.showMessageDialog(
                    PSSimpleSwingBrowser.this.panel,
                    value != null
                        ? PSSimpleSwingBrowser.this.engine.getLocation() + "\n" + value.getMessage()
                        : PSSimpleSwingBrowser.this.engine.getLocation() + "\nUnexpected error.",
                    "Loading error...",
  ...
  private ContextMenu createContextMenu(WebView webView) {
    ...
    openInBrowser.setOnAction(
        e -> {
          URL url = PSBrowserUtils.toURL(mi_actionurl);
          if (url != null) PSBrowserUtils.openWebpage(url);
        });
    ...
    decrementPort.setOnAction(
        e -> {
          setPort(getPort() - 1);
          updatePortMenu(debugPort);
        });
    incrementPort.setOnAction(
        e -> {
          setPort(getPort() + 1);
          updatePortMenu(debugPort);
        });
    ...
    contextMenu
        .getItems()
        .addAll(reloadAndPause, copyLink, openInBrowser, debugPort, debuggingEnabled, copyDebugUrl);
  ...
  */
  ...
  private void checkAndShowAppletWarning(WebEngine engine) {
    ...
    if (editliveSections != null && !editliveSections.getSlot(0).equals("undefined")) {
      ...
      while (editliveSections.getSlot(i) != null
          && !editliveSections.getSlot(i).equals("undefined")) {
