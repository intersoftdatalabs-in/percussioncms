...
public class PSCopySiteSubfolderCopyOptionsPage extends PSCopySiteCopyOptionsPage {
  /**
    ...
    m_noContent =
        new JRadioButton(
            PSI18NTranslationKeyValues.getInstance()
                .getTranslationValue(getClass().getName() + "@Copy Site Subfolders"));
    ...
    if (m_applet.isManagedNavUsed()) {
      m_navigationContent =
          new JRadioButton(
              PSI18NTranslationKeyValues.getInstance()
                  .getTranslationValue(
                      getClass().getName() + "@Copy Site Subfolders with Navigation"));

      m_allContent =
          new JRadioButton(
              PSI18NTranslationKeyValues.getInstance()
                  .getTranslationValue(
                      getClass().getName() + "@Copy Site Subfolders, Navigation and Content"));
    } else {
      m_navigationContent = null;
      m_allContent =
          new JRadioButton(
              PSI18NTranslationKeyValues.getInstance()
                  .getTranslationValue(getClass().getName() + "@Copy Site Subfolders and Content"));
    }
