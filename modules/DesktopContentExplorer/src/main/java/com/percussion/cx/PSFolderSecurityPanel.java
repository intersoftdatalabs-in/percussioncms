...
/**
 * This Folder Security Panel loads a list of ACL entries from the PSFolder and allows user to
 * modify permissions for individual ACLs, add and remove ACL entries. Depending on the Merged User
 * permissions it either allows to change and save entries or provides a read-only access.
 */
public class PSFolderSecurityPanel extends JPanel implements ActionListener {
   ...
   * @param userInfo The info of the user editing or creating the folder , never <code>null</code>.
   * @param folderMgr the manager to handle folder related actions, may not be <code>null</code>
   * @param enabled <code>true</code> if any data can be entered, <code>false</code> otherwise.
   */
  public PSFolderSecurityPanel(
      Dialog parentDialog,
      PSFolder folder,
      PSUserInfo userInfo,
    ...
    if (folderMgr == null) throw new IllegalArgumentException("folderMgr may not be null");
  ...
  public boolean onOk() {
    ...
    PSFolderAcl folderAcl = new PSFolderAcl(locator.getId(), m_folder.getCommunityId());
    ...
    if (m_modified) {
      ...
      if (userResponse != JOptionPane.YES_OPTION) {
        ...
        if (comp instanceof JTabbedPane) ((JTabbedPane) comp).setSelectedComponent(this);
        return false;
      }
    }
  ...
  /**
   * Analyzes the ACL and warns the user if he happens to lock himslef out of this folder.
   *
   * @param acl the folder ACL object with a set of ACL entries to analyze, assumed not <code>null
   *     </code>
  ...
  private int warnOnLockingYourselfOut(PSFolderAcl acl) {
    ...
    if (messageKey != null) {
      String msg = m_applet.getResourceString(getClass(), messageKey);
  ...
  /**
   * Returns the current user's access level for the folder whose ACL is specified by <code>acl
   * </code>. However this access level is not accurate for the Rhythmyx super user, because there
   * is no way to know if the current user is a super user on the client (Content Explorer Applet)
   * side.
   *
   * @param acl contains folder ACL entries which define the permission for a user, role or virtual
   *     entry, assumed not <code>null</code>
  ...
  private int getUserPermission(PSFolderAcl acl) {
    ...
    if (acl.isEmpty()) {
      ...
      PSObjectAclEntry objectAclEntry =
  ...
  private class AclListCellRenderer extends DefaultListCellRenderer {
    /**
     * Gets cell renderer component for cell in the list. Sets an icon, text to the cell and tooltip
     * if the source list is role member list
     *
  ...
  /**
   * Custom MouseMotionListener for handling mouse moves over ACLs in the ACL list and displaying
  ...
  private class AclListMouseMotion extends MouseMotionAdapter {
    ...
    public void mouseMoved(MouseEvent event) {
      if (event.getSource() == m_aclList) {
        ...
        } else m_aclList.setToolTipText(null);
      }
    }
    ...
    private String getToolTipText(PSObjectAclEntry aclEntry) {
      ...
      switch (aclEntry.getType()) {
        case PSObjectAclEntry.ACL_ENTRY_TYPE_ROLE:
          tooltip += m_applet.getResourceString(getClass(), "role:");
          break;
        case PSObjectAclEntry.ACL_ENTRY_TYPE_USER:
          tooltip += m_applet.getResourceString(getClass(), "user:");
          break;
        case PSObjectAclEntry.ACL_ENTRY_TYPE_VIRTUAL:
          tooltip += m_applet.getResourceString(getClass(), "virtual:");
          break;
      }
   ...
   *
   * @return ACL list panel with the embedded JList as a main UI component, never <code>null</code>
   */
  private JPanel createAclListPanel() {
    ...
    String name = m_applet.getResourceString(getClass(), "Names");
  ...
  /**
   * Creates a new Permissions Panel with a group of CheckBoxes, that represent a set of permissions
   * for a currently selected ACL entry.
   *
   * @return Permissions Panel with a group of CheckBoxes , never <code>null</code>
   */
  private JPanel createPermissionsPanel() {
    ...
    String pStr = m_applet.getResourceString(getClass(), "Read");
  ...
  private JPanel createButtonPanel() {
    ...
    m_addButton = new JButton(m_applet.getResourceString(getClass(), "Add"));
    ...
    m_removeButton = new JButton(m_applet.getResourceString(getClass(), "Remove"));
  ...
  /**
   * Loads and appropriately sorts the ACL list with cloned Acl entries. Called once by the {@link
   * #init()}.
   *
  ...
  /**
   * Action Event handler, which handles all the button and check boxes events on this panel.
   *
   * @param e action event, never <code>null</code>
   */
  public void actionPerformed(ActionEvent e) {
    if (e == null) throw new IllegalArgumentException("ActionEvent may not be null");
   ...
   *
   * @param aclEntry currently active aclEntry that is used to update permissions check boxes. If
   *     <code>null</code> then all checkboxes will be unchecked.
   */
  private void updateCheckboxes(PSObjectAclEntry aclEntry) {
    ...
    m_cbReadPermission.setSelected(aclEntry.hasAccess(PSObjectAclEntry.ACCESS_READ));
    m_cbWritePermission.setSelected(aclEntry.hasAccess(PSObjectAclEntry.ACCESS_WRITE));
    m_cbAdminPermission.setSelected(aclEntry.hasAccess(PSObjectAclEntry.ACCESS_ADMIN));
  }

  /**
   * Launches the PSFolderAclEditorDialog that allows to add / create new ACL entries for this
   * folder. Invoked by the action listener in response to the user pushing on the 'Add' button.
   */
  private void onAdd() {
    if (!m_enabled) return;
    ...
    PSFolderAclEditorDialog aclEditorDlg =
        new PSFolderAclEditorDialog(m_parentDialog, m_folderMgr, m_enabled, curAcls);

    aclEditorDlg.setVisible(true);

    if (!aclEditorDlg.isOk()) return;
    ...
    if (resultAclEntries.size() <= 0) return;
  ...
  /**
   * Adds given collection of ACL entries to the JList. Ensures that only unique entries are added.
   *
   ...
   * user pushing on the 'Remove' button or hitting 'DEL' key.
   */
  ...
  /**
   * <code>true</code> indicates that the user has enough permissions to allow him to edit and save
   * data on this panel.
   */
  private boolean m_enabled;

  /**
   * <code>true</code> indicates the acl data has been modified since the last save or since first
