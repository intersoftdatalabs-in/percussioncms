...
/**
 * This wizard panel allows a user to map source to target communities for the copy site / copy site
 * folder wizards.
 */
public class PSCommunityMappingsPage extends PSWizardPanel {
    ...
    StringBuffer summary =
        new StringBuffer(
            PSI18NTranslationKeyValues.getInstance()
                .getTranslationValue(getClass().getName() + "@Community Mappings:"));
    summary.append("\n");

    TableModel model = m_table.getModel();
    for (int i = 0; i < model.getRowCount(); i++) {
      Object test =
          model.getValueAt(i, SOURCE_COLUMN_INDEX) == null
              ? ""
              : model.getValueAt(i, SOURCE_COLUMN_INDEX);
      String source = test.toString().trim();

      test =
          model.getValueAt(i, TARGET_COLUMN_INDEX) == null
              ? ""
   ...
   * Get the panel data.
   *
   * @return the panel data as {@link OutputData} object, never <code>null</code>.
   */
   ...
   *
   * @param data the data as {@link InputData} object, not <code>null</code>.
   */
   ...
   *
   * @param data the data to initialize the wizard with, must be an instance of {@link InputData}.
   */
  private void initData(Object data) {
    ...
    Object[] columnNames = {
      PSI18NTranslationKeyValues.getInstance()
          .getTranslationValue(getClass().getName() + "@Source Community"),
      PSI18NTranslationKeyValues.getInstance()
          .getTranslationValue(getClass().getName() + "@Target Community")
    };

    TableModel model = new DefaultTableModel(columnNames, sourceCommunities.size());
  ...
  public static class InputData {
     ...
     *
     * @param commCtMapper The community and content type mapper, never <code>null</code>.
     * @param communityDefinitions a collection of community definitions as <code>
     *     PSCommunityCataloger.Community</code> obejcts, not <code>null</code>, may be empty.
     * @param sourceCommunities a collection of source node community ids as <code>Integer</code>
     *     objects, not <code>null</code>, may be empty.
     */
    public InputData(
    ...
    /**
     * A collection with all community definitions in the current system as <code>
  ...
  /**
   * The community mapping table, it maps the source community (on the first column) to its
   * corresponding target communities (on the second column). The each target community contains the
   * same set or more content types that are defined in the source community.
   *
   * <p>It creates different cell editor for each target cell and overrides the {@link
   * #getCellEditor(int, int)} to set the cell editor based on the location of the cell.
   */
  private class MappingTable extends JTable {
    ...
    /**
     * Set the target cell editor for the specified row; initialize both source and target cells to
     * be the supplied source community.
     *
     * @param row the current row of the table.
     * @param srcCommunity the source community id. Assumed not <code>null</code>.
     * @param allCommunities a complete list of communities. This is a list of {@link
     *     PSCommunityCataloger.Community} objects. Assumed not <code>null</code>
     * @param mapper the community and content type mapper, assumed not <code>null</code>.
      ...
      while (walker.hasNext()) {
        ...
        if (community.getId() == srcCommunity.intValue()) {
          ...
          DefaultCellEditor defaultEditor =
              (DefaultCellEditor) m_table.getCellEditor(row, TARGET_COLUMN_INDEX);
     ...
     *
     * @param communities the selectable communities for the cell editor. Each community is a {@link
    ...
    public TableCellEditor getCellEditor(int row, int col) {
      if (col == SOURCE_COLUMN_INDEX) return m_sourceEditor;
      else return m_targetEditors[row];
    }
  ...
  /**
   * The table for the source - target community mappings. Initialized in {@link
   * #createMainPanel()}, never <code>null</code> after that.
   */
