...
/**
 * Folder ACL Editor Dialog that allows to catalog Role, User and Virtual ACL entries and add those
 * to the result ACL list.
 */
public class PSFolderAclEditorDialog extends PSDialog implements ActionListener {
  /**
   * Constructor for this dialog. Creates all UI components, sets up layout and loads current and
   * cataloged acl entry lists.
   *
   * @param parent Dialog, may be <code>null</code>.
   * @param folderMgr folder action manager, used to get server cataloged data, never <code>null
   *     </code>.
   * @param enabled <code>true</code> indicates that the user can make modifications and OK changes,
  ...
  public PSFolderAclEditorDialog(
    ...
    super(
        ...
        folderMgr
            .getApplet()
            .getResourceString(PSFolderAclEditorDialog.class, "Folder ACL List Entry Editor"));
    setResizable(true);
    if (folderMgr == null) throw new IllegalArgumentException("folderMgr may not be null");

    if (curAclEntries == null) throw new IllegalArgumentException("curAclEntries may not be null");
  ...
  private void initDialog(Collection curAclEntries) {
    ...
    UTStandardCommandPanel defCommandPanel =
        new UTStandardCommandPanel(this, SwingConstants.RIGHT, true);
    ...
    m_catalogAclList.addListSelectionListener(new PSAccessibleListSelectionListener());
    ...
    curAclListPanel.add(
        createAclListPanel(
            m_curAclList, m_applet.getResourceString(getClass(), "Current ACL Entries")));
  ...
  /**
   * Inserts and then sorts acl entries into the current acl entries list. While inserting enforces
   * uniqueness and also makes sure that in the cataloged list there is no redundant entries and if
   * so removes them from the cataloged list.
   *
  ...
  /**
   * Inserts and then sorts acl entries into the catalog acl entries list. While inserting enforces
   * uniquiness and also skips those ones that have already been added to the current acl entries
   * list.
   *
  ...
  /**
   * Depending on the currently selected radio button, catalogs roles, users, or virtual entries
   * then creates corresponding ACL Entry objects and caches them for subsequent use. The
   * PSRoleCataloger and PSSubjectCataloger are used to lazily fetch catalog data from the server.
   *
  ...
  private Collection getCatalogEntries() {
    try {
      ...
      m_applet.displayErrorMessage(
          this,
          getClass(),
          "Failed To Get Catalog Information, Exception Msg: <{0}>.",
          new String[] {cmsEx.getLocalizedMessage()},
          "Error",
          null);
    }
  ...
  private class AclListCellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      if (isSelected) {
        if (list == m_curAclList) m_catalogAclList.clearSelection();
        else m_curAclList.clearSelection();
  ...
  private class AclListMouseMotion extends MouseMotionAdapter {
    public void mouseMoved(MouseEvent event) {
      ...
      } else srcList.setToolTipText(null);
    }
  ...
  private JPanel createAddRemoveButtonPanel() {
    ...
    m_btnAdd =
        new UTFixedButton(m_applet.getResourceString(getClass(), "Add >>"), new Dimension(110, 24));
    m_btnAdd.setDefaultCapable(false);
    m_btnAdd.addActionListener(this);
    m_btnAdd.setEnabled(m_enabled);
    m_btnAdd.setMnemonic(PSContentExplorerApplet.getResourceMnemonic(getClass(), "Add >>", 'A'));

    m_btnRemove =
        new UTFixedButton(
            m_applet.getResourceString(getClass(), "<< Remove"), new Dimension(110, 24));
    m_btnRemove.setDefaultCapable(false);
    m_btnRemove.addActionListener(this);
    m_btnRemove.setEnabled(m_enabled);
    m_btnRemove.setMnemonic(
        PSContentExplorerApplet.getResourceMnemonic(getClass(), "<< Remove", 'R'));

    m_btnNew =
        new UTFixedButton(m_applet.getResourceString(getClass(), "New..."), new Dimension(110, 24));
    m_btnNew.setDefaultCapable(false);
    m_btnNew.addActionListener(this);
    m_btnNew.setEnabled(m_enabled);
    m_btnNew.setMnemonic(PSContentExplorerApplet.getResourceMnemonic(getClass(), "New...", 'N'));
  ...
  private JPanel createCatalogSelectorPanel() {
    ...
    rbPanel.setBorder(
        new TitledBorder(
            ...
            m_applet.getResourceString(getClass(), "Show Catalog of")));
    rbPanel.add(Box.createVerticalStrut(5));

    // create permissions checkboxes
    m_rbRoles = new JRadioButton(m_applet.getResourceString(getClass(), "Roles"));
    ...
    m_rbUsers = new JRadioButton(m_applet.getResourceString(getClass(), "Users"));
    m_rbUsers.addActionListener(this);
    m_rbUsers.setMnemonic(
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "Users", 'U'));

    m_rbVirtual = new JRadioButton(m_applet.getResourceString(getClass(), "Virtual"));
    m_rbVirtual.addActionListener(this);
    m_rbVirtual.setMnemonic(
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "Virtual", 'V'));
   ...
   *
   * @param aclList acl list to create panel with and setup listeners etc. Must have
   *     DefaultListModel., must never be <code>null</code>.
  ...
  /**
   * Launches New User ACL Entry dialog and if OKed creates and inserts a new acl entry into the
   * current acl entries list.
   */
  private void onNew() {
    try {
      PSSecurityProviderCataloger providerCataloger = m_folderMgr.getSecurityProviderCataloger();
      ...
      m_applet.displayErrorMessage(
          this,
          getClass(),
          "Failed To Catalog Security Providers, Exception Msg: <{0}>.",
          new String[] {cmsEx.getLocalizedMessage()},
          "Error",
          null);
    }
