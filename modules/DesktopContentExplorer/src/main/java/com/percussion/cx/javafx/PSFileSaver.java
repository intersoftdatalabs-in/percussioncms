...
/**
 * Helper that presents a JavaFX {@code FileChooser} save dialog so the user can persist a remote
 * binary (such as an image, document, or spreadsheet) to the local file system. The source URL and
 * (optionally) a desired file name are supplied through the constructors; {@link #startFileSaver()}
 * launches the dialog on the JavaFX application thread.
 */
public class PSFileSaver {
  ...
  private static final Map<String, FileChooser.ExtensionFilter>
      EXTMAP; // map to hold file type extensions
  String srcURL;
  String fileName;
  ...
  static {
    ...
    extensionTypes.put(
        ".doc", new FileChooser.ExtensionFilter("Microsoft Word Document *.doc", "*.doc"));
    extensionTypes.put(
        ".docx", new FileChooser.ExtensionFilter("Microsoft Word Document *.docx", "*.docx"));
    extensionTypes.put(
        ".xls", new FileChooser.ExtensionFilter("Microsoft Excel Document", "*.xls"));
    extensionTypes.put(".txt", new FileChooser.ExtensionFilter("Text File", "*.txt"));
    extensionTypes.put(
        ".css", new FileChooser.ExtensionFilter("Cascading Stylesheets File", "*.css"));
    extensionTypes.put(
        ".html", new FileChooser.ExtensionFilter("Hyper Text Markup Language File", "*.html"));
   ...
   * @param fileName the fileName should be the name of the file as well as the extension, i.e.
   *     my_file.pdf
   */
  public PSFileSaver(String srcURL, String fileName) {
    ...
    if (extensionIndex != -1) fileExtension = fileName.substring(extensionIndex).toLowerCase();
    else fileExtension = "";
  }
   ...
   *
   * @param primaryStage the JavaFX stage on which to display the file-saver, may not be <code>null
   *     </code>.
   */
  public void showFileSaver(final Stage primaryStage) {
    ...
    if (extFilter == null) extFilter = EXTMAP.get("*.*");
