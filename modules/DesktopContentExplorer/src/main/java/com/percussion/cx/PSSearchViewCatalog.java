...
/**
 * This is a convenient class to get catalog of all search views from the server, keep locally and
...
public class PSSearchViewCatalog {
  ...
  public PSSearchViewCatalog(
    ...
    if (proxy == null) throw new IllegalArgumentException("proxy must not be null");
  ...
  /**
   * Loads all available searches and views by making a request to the server and caches them in
   * this object. For each search loaded, if {@link PSSearch#useExternalSearch()} returns <code>true
   * </code> and an external search engine is not available, the search is converted to an internal
   * search.
   *
   * @throws PSContentExplorerException if an error happens in the process of loading searches and
   *     views.
   * @throws IllegalStateException if the default search is not found in the available searches.
   */
  public void loadSearches() throws PSContentExplorerException {
    try {
      Element[] coll = m_proxy.load(PSSearch.getComponentType(PSSearch.class), null);

      if (coll.length < 1) throw new IllegalStateException("Searches or views are not available.");
      ...
      String commId = String.valueOf(m_applet.getUserInfo().getCommunityId());
      int cxSearchIndex = -1;
      PSSearch defaultRcNewSearch = null;
      for (int i = 0; i < coll.length; i++) {
        PSSearch search = new PSSearch(coll[i]);
        if (!m_isFtsEnabled) search.convertToInternal();
         ...
         */
        if (search.isAADNewSearch()) defaultRcNewSearch = search;
        if (search.isCXNewSearch()) cxSearchIndex = i;
      ...
      if (m_rcSearch == null && defaultRcNewSearch != null) m_rcSearch = defaultRcNewSearch;
      ...
      if (m_emptySearchDoc == null)
        throw new IllegalStateException("Default search for Content Exploreer is not found");
      if (getRcSearch() == null)
   ...
   *
   * @return PSSearch object corresponding to the Related Content Search. Could be <code>null</code>
   *     if there is a serious configuration problem, in which case the constructor will throw an
   *     exception.
   */
  ...
  /**
   * Retrieve and modify, or create a content id search. The search is stored locally in the catalog
   * and never persisted to the server.
   *
   ...
   *
   * @param search the search whose id need to be extracted, must not be <code>null</code>.
  ...
  public static String getSearchId(PSSearch search) {
    ...
    return search.getLocator().getPart(search.getLocator().getDefinition()[0]);
  }
   ...
   *
   * @param search with which the new search is to be replaced. If {@link
   *     PSSearch#useExternalSearch()} returns <code>true</code> and an external search engine is
   *     not available, the search is converted to an internal search before being set.
   */
  ...
  /**
   * Helper method to create an empty search document and add a search to the catalog for the newly
   * created empty search document. This search will be created on the fly. We take the default
   * search and change the searchid to a large number and create another search from it and add it
   * to the searches.
   *
   * @param searchElem the search element from which the search need to be created.
   * @return Document newly created document for the default search.
   * @throws PSContentExplorerException When it is not able to create a search from the supplied
   *     search element. This should not happen as by the time it comes here a search has already
   *     been created from this element.
   */
  private Document createAndAddEmptySearch(Element searchElem) throws PSContentExplorerException {
    if (searchElem == null) throw new IllegalArgumentException("search element must not be null");
    ...
    try {
      ...
      PSSearch emptySearch = new PSSearch(emptySearchDoc.getDocumentElement());
   ...
   * #loadSearches()} and never modified afterward. Might be <code>null</code> if there is an error
   * in the configuration.
   */
  private PSSearch m_rcSearch = null;

  /**
   * The proxy to use to load searches from server, initialized in the ctor and never <code>null
  ...
  /** Set in ctor, then never modified. Indicates whether the full text search engine is active. */
  private boolean m_isFtsEnabled;

  /**
   * Constant for empty search id value. This must be a value that no other search could possibly
   * have.
   */
