...
public class PSFolderDialog extends PSDialog {
   ...
   * @param folderNode the node that represents the folder to modify, supply <code>null</code> to
   *     create a new folder.
   ...
   * @param navSelectionPath current content navigation path of the form: //Folder/Folder1/Folder2,
   *     never <code>null</code>.
   * @throws PSContentExplorerException if loading the folder being edited is failed
   * @throws PSCmsException if any used cms cataloger fails to fetch needed data
   */
  public PSFolderDialog(
      Frame parent,
      PSUserInfo userInfo,
      PSFolderActionManager folderMgr,
      PSNode folderNode,
      PSNode parentFolderNode,
      String navSelectionPath)
    ...
    if (folderNode == null) {
      ...
      if (parentFolder == null) throw new IllegalArgumentException("parent folder can not be null");

      // inherit ACL from the parent + add current user as an Admin user.
      PSObjectAclEntry curUserAclEntry =
      ...
      m_folder =
          new PSFolder(
              m_applet.getResourceString(getClass(), "New Folder"),
              parentFolder.getCommunityId(),
              PSObjectAclEntry.ACCESS_ADMIN,
              "");
  ...
  private void initDialog() throws PSCmsException {
    ...
    if (!m_isNewFolder && !perm.hasAdminAccess()) {
      setTitle(m_applet.getResourceString(getClass(), "Edit folder - Read only"));
    }
    ...
    // general panel
    String pStr = m_applet.getResourceString(getClass(), "General");

    int folderCommId = m_folder.getCommunityId();
    m_fGeneralPanel =
        new PSFolderGeneralPanel(
            this,
            m_folder,
            m_folderMgr,
            m_folderNode,
            ...
            m_isNewFolder,
            perm.hasAdminAccess(),
            m_userInfo.getCommunityId(),
            folderCommId);
    char mnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "General", 'G');
    ...
    m_publishFolderFlagInitialState = m_fGeneralPanel.isPublishFolderFlagSelected();

    // security panel
    pStr = m_applet.getResourceString(getClass(), "Security");
    m_fSecurityPanel =
        new PSFolderSecurityPanel(this, m_folder, m_userInfo, m_folderMgr, perm.hasAdminAccess());

    tabbedPane.add(pStr, m_fSecurityPanel);
    mnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "Security", 'S');
    ...
    // custom panel
    pStr = m_applet.getResourceString(getClass(), "Custom");

    m_fPropertiesPanel = new PSFolderPropertiesPanel(m_folder, perm.hasAdminAccess(), m_applet);

    tabbedPane.add(pStr, m_fPropertiesPanel);
    mnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "Custom", 'u');
  ...
  /**
   * Depending on the folder permissions, invokes onOk on all tab panels, each of which then saves
   * their own data in the shared PSFolder instance. User must have Admin access to be able to
   * modify and persist any data. If all the envoked panels succeed, then uses the
   * FolderActionManager to create or save an updated PSFolder data, then hides and disposes this
   * dialog.
   */
  public void onOk() {
    ...
    try {
      ...
      if (!m_isNewFolder && !perm.hasAdminAccess()) return;

      if (!m_fGeneralPanel.onOk()) return;

      if (!m_fSecurityPanel.onOk()) return;

      if (!m_fPropertiesPanel.onOk()) return;

      try {
        if (m_isNewFolder) {
          PSFolder folder = m_folderMgr.createFolder(m_parentFolderNode, m_folder);

          String folderid = folder.getLocator().getPart(PSLocator.KEY_ID);
          m_applet.toggleFlaggedFolder(folderid, folder.isPublishOnlyInSpecialEdition());
        } else {
        ...
        ErrorDialogs.showErrorMessage(
            this, e.getLocalizedMessage(), m_applet.getResourceString(getClass(), "Error"));
        return;
      }
    } finally {
   ...
   *
   * @return The folder, never <code>null</code>. Modifications to the folder are persisted by
   *     {@link #onOk()}
   */
  ...
  /**
   * Holds current naviagation tree selection path never <code>null</code>, and never <code>empty
   * </code>.
   */
  ...
  /**
   * The parent folder node of the new folder being created, initialized in the ctor if the dialog
  ...
  /**
   * Indicates the intial state of the publish folder flag checkbox in the general panel. Set in
   * {@link #initDialog}.
   */
