...
public class PSImageIconLoader extends UIManager {
   ...
   * Manager cache, loads from the archive or server.
   *
   * @param iconKey If it starts with ".." or "http" or "https", it is loaded from the server
   *     otherwise from the JAR file. For the latter case, it must not include the extension ".gif"
   *     which will be added by default. Must not be <code>null</code> or empty.
  ...
  public static Icon loadIcon(String iconKey, boolean expanded, PSContentExplorerApplet applet) {
    ...
    if (iconKey.startsWith("..") || iconKey.startsWith("http:") || iconKey.startsWith("https:")) {
      ...
      if (PSAjaxSwingWrapperLocator.getInstance().isAjaxSwingEnabled()) {
        log.debug(
            "In loadIcon() in PSImageIconLoader for AjaxSwing. iconKey is : "
                + "../sys_resources/ajaxswing/images/"
                + iconKey
                + ".gif");
        try {
          return applet
              .getApplet()
              .getIcon("../sys_resources/ajaxswing/images/" + iconKey + ".gif");
        } catch (Exception e) {
          applet.getApplet().debugMessage(e);
        }
      }
    ...
    if (icon != null) {
      if (expanded && get(keyExpanded) == null) put(keyExpanded, icon);
      else if (get(iconKey) == null) put(iconKey, icon);
    }
    return icon;
  }
  ...
  public static Icon loadIcon(String iconKey) {
    ...
    try {
      ...
      InputStream in = PSImageIconLoader.class.getResourceAsStream(rootPath + newIconKey);
      if (in == null) {
