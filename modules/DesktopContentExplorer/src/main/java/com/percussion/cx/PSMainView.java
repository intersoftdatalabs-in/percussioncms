 ...
 */
public class PSMainView extends JSplitPane
    implements TreeSelectionListener,
        ListSelectionListener,
        IPSActionListener,
        TreeWillExpandListener {
  /**
   * The navigational tree that represents the left component of this main view panel, initialized
   * in <code>init(PSNode)</code> and never <code>null
   * </code> or modified after that.
   */
  private PSNavigationTree m_navTree;

  /**
   * The display panel that represents the right component of this main view panel, initialized in
   * <code>init(PSNode)</code> and never <code>null
   * </code> or modified after that.
   */
  private PSMainDisplayPanel m_mainDisplayPanel;

  /**
   * The list of selection listeners that are interested in selection changes in this view panel
   * (tree or table), initialized to an empty list and listeners are being added through calls to
   * <code>
  ...
  /**
   * The parent applet container that holds action manager, global menu bar and other details,
   ...
   *
   * @param navTree the root node with its children to represent its left navigation pane.
   ...
   */
  public PSMainView(PSNode navTree, String view, PSContentExplorerApplet applet) {
    if (navTree == null) {
    ...
    if (!(PSUiMode.TYPE_VIEW_CX.equals(view)
      ...
      throw new IllegalArgumentException(
          "view must be one of the following:"
              + PSUiMode.TYPE_VIEW_CX
              + ","
              + PSUiMode.TYPE_VIEW_IA
              + ","
              + PSUiMode.TYPE_VIEW_RC);
    }
  ...
  /**
   * Initializes the split pane with navigational tree panel on the left and a table panel on the
   * right. The table panel displays the children of the selected node in the navigation pane.
   *
  ...
  private void init(PSNode root) {
    ...
    // Left Panel
    m_navTree = new PSNavigationTree(root, m_view, m_applet.getActionManager());
    ...
    */
    ...
    Color bkgColor = m_applet.isMacPlatform() ? Color.white : SystemColor.window;
    ...
    m_mainDisplayPanel = new PSMainDisplayPanel(m_view, m_applet.getActionManager(), sortView);
    m_mainDisplayPanel.setBorder(BorderFactory.createEmptyBorder());
    m_mainDisplayPanel.getTable().getSelectionModel().addListSelectionListener(this);
  ...
  /**
   * Adds the selection listener to its list of listeners to fire the event when the selection has
   * changed in tree view or table view.
   *
  ...
  /**
   * Action method for selection change event in navigational tree. Constructs the <code>
   * PSNavigationalSelection</code> object from the event and informs the selection listeners of the
   * change.
   *
   * @param e the event, assumed not <code>null</code> as this method is called by Swing.
   */
  public void valueChanged(@SuppressWarnings("unused") TreeSelectionEvent e) {
    ...
    if (selNode != null) {
      ...
      if (m_view.equals(PSUiMode.TYPE_VIEW_IA) && selNode.getType().equals(PSNode.TYPE_SLOT)) {
        sortView = false;
      }
  ...
  /**
   * Action method for expansion event in navigational tree. Updates child tree nodes to reflect
  ...
  /**
   * Loads/re-loads child nodes if required and re-freshes nav tree and right hand pane.
   *
   * @param treePath The tree path of the tree node in the nav tree that should be updated, assumed
   *     not <code>null</code>.
  ...
  private void refreshChildNodes(TreePath treePath, PSNode node) {
    ...
    if ((children == null)
        || (node.hasDirtyChildren())
        || ((!m_applet.getActionManager().isQuickLoading(node)) && node.isQuickLoaded())) {
  ...
  /**
   * Action method for selection change event in main display table. Constructs the <code>
   * PSSelection</code> object from the event (selected rows) and informs the selection listeners of
   * the change.
   *
   * @param e the event, assumed not <code>null</code> as this method is called by Swing.
   */
  public void valueChanged(ListSelectionEvent e) {
    ...
    if (selNodes.hasNext()) {
      ...
      PSSelection sel =
          new PSSelection(new PSUiMode(m_view, PSMainDisplayPanel.ms_mode), parent, selNodes);
      informListeners(sel);
    }
  }
  ...
  public void actionExecuted(PSActionEvent event) {
    ...
    if (nodeType != -1) {
      ...
      if (nodeToRefresh != null) {
        ...
        nodeToRefresh = m_applet.getActionManager().refresh(nodeToRefresh);
  ...
  // noop
  public void actionInitiated(@SuppressWarnings("unused") PSProcessMonitor monitor) {}

  /**
   * Access method for the navigation tree object.
   *
   * @return the navigation tree object associated with the view, never <code>null</code>.
   */
  ...
  /**
   * The class that handles pop-up mouse clicks to show pop-up menu. Displays the global menu as
   * pop-up menu if the mouse click is on the left panel that contains navigation tree and the
   * clicked location does not represent a node.
   */
  private class MouseActionListener extends PSMouseAdapter {
    /**
     * Handles right-click mouse event to show the global-menu if the clicked location does not
     * represent a node in the navigation tree that is in the left panel.
     *
    ...
    public void mouseWasClicked(MouseEvent event) {
      if (PSCxUtil.isMouseMenuGesture(event, m_applet)) {
        if (event.getSource() == m_navTree) {
          TreePath path = m_navTree.getPathForLocation(event.getX(), event.getY());
        ...
        if (event.getSource() == m_mainDisplayPanel.getTable()) {
          ...
          } else // execute the default action for the node if applicable
          {
            ...
            PSUiMode mode = new PSUiMode(m_view, PSMainDisplayPanel.ms_mode);
            PSSelection selection =
                new PSSelection(mode, parentNode, PSIteratorUtils.iterator(selNode));
            PSMenuAction defAction = m_applet.getActionManager().findDefaultAction(selection);

            if (defAction != null) {
              m_applet.getActionManager().executeAction(defAction, selection);
            }
          }
        }
      }
  ...
  /**
   * The renderer to work with 'IA' view to display the 'slot' in different font if the current
   * selected node can be dropped on to the current slot. A node can be dropped on to a slot if it
   * represents a search result whose name is the slot name or if it is a child node of such node.
   */
  private class PSIAViewNodeRenderer extends PSNavTreeNodeRenderer {
    ...
    /**
     * Overridden to implement the description specified in this class description. See super class
     * for description of the parameter and return values.
     */
      ...
      if (node.isOfType(PSNode.TYPE_SLOT)) {
        ...
        if (path != null) {
          ...
          for (int i = nodes.length - 1; i >= 0; i--) {
            ...
            if (selNode.isOfType(PSNode.TYPE_NEW_SRCH) && node.equals(selNode.getSlotNode())) {
