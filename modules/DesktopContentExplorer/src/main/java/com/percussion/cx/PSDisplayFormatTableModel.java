...
/**
 * The table model that displays a children of a node according to its display format. Does not
...
public class PSDisplayFormatTableModel extends PSTableModel {
  ...
  public PSDisplayFormatTableModel(PSContentExplorerApplet applet) {
    ...
    ms_defColumnName =
        m_applet.getResourceString(PSDisplayFormatTableModel.class.getName() + "@Name");
  }

  /**
   * Sets table model with the children of the supplied node. The table structure is taken from this
   * node's display format. Always will have a default first column 'Name' that gets child node
   * object in addition to the display format columns.
   *
    ...
    if (children != null && children.hasNext()) {
      ...
      while (children.hasNext()) {
        ...
        if (tableFormat != null) {
          ...
          {
            for (int i = 1; i < columnNames.size(); i++) rowVector.add(null);
          }
        }
  ...
  /**
   * Set the locale of this table model, while displaying dates, use this locale to format them
   * properly. The default language is used if <code>null</code> or empty is supplied.
   *
   * @param str the current locale as a string, may be <code>null</code> or empty.
   */
  public void setLocale(String str) {
    if (str == null || str.trim().length() == 0) str = PSI18nUtils.DEFAULT_LANG;
  ...
  public static int getSysTitleIndex(PSNode node, PSActionManager actionManager) {
    ...
    if (node == null) return hasTitle;

    // get displayformat id:
    String dispId = node.getProp(IPSConstants.PROPERTY_DISPLAYFORMATID);
    if (dispId == null || dispId.trim().length() == 0) return hasTitle;

    try {
      // get format:
      PSDisplayFormat format = actionManager.getDisplayFormatCatalog().getDisplayFormatById(dispId);
  ...
  /** This is where the title column will be located. The title column is the PSNode. */
  private int m_titleCol = 0;

  /**
   * The current locale to use for this table model when displaying dates. Set with a setter <code>
   * setLocale</code>, may be <code>null</code> if it has not been initialized.
   */
  private Locale m_locale = null;

  /**
   * List of columns types for each column in the model, each entry is one the <code>
