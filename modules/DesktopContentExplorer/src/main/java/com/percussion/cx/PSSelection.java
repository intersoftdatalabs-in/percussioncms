...
/**
 * The class that represents a selection in the UI that represents a part of UI context to execute
 * an action.
 */
public class PSSelection {
   ...
   *
   * @param uiMode the ui mode that represents the component view which is the source of the
   *     selection, may not be <code>null</code>
   ...
   *
   * @return <code>true</code> if the selection has more than one item, <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Check if this selection contains only nodes of type system folder. Currently defined system
   * folder types are:
   *
   * <ul>
  ...
  public boolean isSystemFolderType() {
    return isOfType(PSNode.TYPE_SYS_FOLDERS) || isOfType(PSNode.TYPE_SYS_SITES);
  }

  /**
   * Check if this selection contains only nodes of type folder. Currently defined folder types are:
   *
   * <ul>
  ...
  public boolean isFolderType() {
    return isOfType(PSNode.TYPE_FOLDER)
        || isOfType(PSNode.TYPE_SITE)
        || isOfType(PSNode.TYPE_SITESUBFOLDER);
  }

  /**
   * Determine if this selection contains only nodes of any type folder. This is the ORed result of
   * {@link #isSystemFolderType()} and {@link #isFolderType()}.
   *
   * @return <code>true</code> if it is of any type folder, <code>false</code> otherwise.
   */
   ...
   *
   * @param type <code>PSNode</code> type to check for, one of the valid <code>PSNode</code> types.
   * @return <code>true</code> if the selection has more than one item, <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Get the selection type. Type is assumed to be valid only if every node in the selection is of
   * same type.
   *
   * @return the selction type which will be one of PSNode types, if all nodes in the selection are
  ...
  /**
   * Get the selection types which is an array list of union of all node types in the selection.
   *
   ...
   *
   * @param node PSNode object to test the existence in the selection, must not be <code>null</code>
   *     .
   * @return <code>true</code> if selection contains the specified node <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * The UI Mode represented by this selection object, intiailized in the ctor and never <code>null
