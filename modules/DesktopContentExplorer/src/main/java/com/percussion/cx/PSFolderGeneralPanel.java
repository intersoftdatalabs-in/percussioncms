...
/**
 * General Panel on the Cx Folder Properties Dialog. Allows users to change folder name, community,
 * description and display format.
 */
public class PSFolderGeneralPanel extends PSPropertyPanel {
   ...
   * @param folderNode the node that represents the folder to modify, may be <code>null</code> if
   ...
   * @param navSelectionPath current content navigation path, such as //Folder/ Folder1/Folder2,
   ...
   * @param folderCommId community id of the current folder, -1 if it is a new folder or any
   *     community (all communities).
   * @throws PSCmsException if any cataloger fails to fetch data from the server
   */
  public PSFolderGeneralPanel(
      Dialog parentDialog,
      PSFolder folder,
      PSFolderActionManager folderMgr,
      PSNode folderNode,
      PSNode parentFolderNode,
  ...
  private void init(int currCommunityId, int folderCommId) throws PSCmsException {
    ...
    String accStr = m_applet.getResourceString(getClass(), "Folder Name:");
    setAccessibleInfo(m_textFolderName, accStr);

    addPropertyRow(
        accStr,
        m_textFolderName,
        PSContentExplorerApplet.getResourceMnemonic(getClass(), "@Folder Name:", 'N'));
    ...
    if (m_isNewFolder && m_editable) {
      ...
      if (m_parentDialog != null) {
        m_parentDialog.addWindowListener(
            new WindowAdapter() {
              @Override
    ...
    JTextField folderIdTextField = new JTextField(m_folder.getLocator().getPart(PSLocator.KEY_ID));
    folderIdTextField.setEnabled(false);

    addPropertyRow(m_applet.getResourceString(getClass(), "Folder ID:"), folderIdTextField);
    ...
    accStr = m_applet.getResourceString(getClass(), "Location:");
    setAccessibleInfo(textLocation, accStr);

    addPropertyRow(
        accStr,
        textLocation,
        PSContentExploreAppletUtils.getResourceMnemonic(getClass(), "@Location:", 'L'));

    m_textFolderDescription = new JTextField(m_folder.getDescription());
    m_textFolderDescription.setEditable(m_editable);
    accStr = m_applet.getResourceString(getClass(), "Description:");
    setAccessibleInfo(m_textFolderDescription, accStr);

    addPropertyRow(
        accStr,
        m_textFolderDescription,
    ...
    accStr = m_applet.getResourceString(getClass(), "Locale:");
    ...
    accStr = m_applet.getResourceString(getClass(), "Default display format:");
    setAccessibleInfo(m_comboDisplayFormat, accStr);

    addPropertyRow(
        accStr,
        m_comboDisplayFormat,
    ...
    if (m_isNewFolder) m_comboDisplayFormat.setSelectedIndex(0);
      ...
      PSDisplayFormat format =
          m_folderMgr.getDisplayFormatById(m_folder.getDisplayFormatPropertyValue(), true);
    ...
    String globalTemplateAccStr = m_applet.getResourceString(getClass(), "Global Template:");
    ...
    accStr = m_applet.getResourceString(getClass(), "Publish Folder with Site:");
    ...
    if (m_navSelectionPath.startsWith("//Sites")) {
      ...
      else m_comboGlobalTemplate.setSelectedItem(globalTemplate);

      // Folder is implied to be marked if any ancestor is marked
      boolean isImpliedMarked =
          PSCxUtil.shouldFolderBeMarked(m_folderNode, m_parentFolderNode, true, m_applet);
      m_checkBoxPublishFolderFlag.setSelected(shouldPublish || isImpliedMarked);
      if (isImpliedMarked) m_checkBoxPublishFolderFlag.setEnabled(false);
      addPropertyRow(
          accStr,
          m_checkBoxPublishFolderFlag,
          PSContentExploreAppletUtils.getResourceMnemonic(
  ...
  /**
   * Initialize the community combo box which contains all community and the community of the
   * current session. This is to avoid the folder not visible by the current user after the creating
   * or modifying the folder.
   *
   * @param currCommunityId the community id of the current session.
   * @param folderCommId community id of the current folder, -1 if it is a new folder or any
   *     community (all communities).
  ...
  private void initCommunityComboBox(int currCommunityId, int folderCommId) throws PSCmsException {
    ...
    String accStr = m_applet.getResourceString(getClass(), "Folder Community:");
    ...
    */
    ...
    if (folderCommId == -1) selComm = commAll;
  ...
  public boolean onOk() {
    ...
    if (m_textFolderName.getText().trim().length() == 0) {
      ErrorDialogs.showErrorMessage(
          m_parentDialog,
          m_applet.getResourceString(getClass(), "The folder name can not be empty."),
          ...
          if (child.isOfType(PSNode.TYPE_FOLDER) && child != m_folderNode) {
            if (child.getName().equalsIgnoreCase(m_textFolderName.getText())) {
              m_applet.displayErrorMessage(
                  m_parentDialog,
                  getClass(),
                  "The folder with name <{0}> already exists. " + "Please choose another name.",
    ...
    if (slashpos >= 0) {
      m_applet.displayErrorMessage(
          m_parentDialog,
          getClass(),
          "The name you have supplied, <{0}>, contains invalid " + "characters",
          new String[] {m_textFolderName.getText()},
          "Error",
          null);
      m_textFolderName.requestFocus();
    ...
    // set display format
    PSDisplayFormat format = (PSDisplayFormat) m_comboDisplayFormat.getSelectedItem();

    m_folder.setDisplayFormatPropertyValue(String.valueOf(format.getDisplayId()));
    ...
    if (selected != null) {
      ...
      if (globalTemplate.equals(m_applet.getResourceString(getClass(), "Use default")))
        globalTemplate = null;
    }
    ...
    if (!isImpliedMarked) {
      ...
      if (folderid != null && folderid.trim().length() > 0)
        m_applet.toggleFlaggedFolder(folderid, m_checkBoxPublishFolderFlag.isSelected());
    }
  ...
  /** refrence to the parent dialog, invariant, used to supply to the child error dialog. */
  private Dialog m_parentDialog;

  /**
   * The flag that specifies whether folder is being created or edited, <code>
   * true</code> indicates new and vice-versa. Initialized in the ctor and never modified after
   * that.
   */
  private boolean m_isNewFolder = false;

  /**
   * Holds current naviagation tree selection path. never <code>null</code>, may be <code>empty
   * </code>
   */
  ...
  /**
   * The parent folder node of the new or existing folder being created or modified, initialized in
  ...
  /**
   * Dropdown combo element that allows user to set a community for a new folder or change community
   * for an existing one. Loaded from the data returned by the PSCommunityCataloger. Initilized in
   * <code>
