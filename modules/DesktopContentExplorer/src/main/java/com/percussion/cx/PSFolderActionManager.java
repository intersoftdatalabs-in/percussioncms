...
/**
 * Class that manges all folder related actions. Basically a mediator between the content explorer
 * and the folder processor proxy. Converts the input data of from the contexnt explorer to the
 * format the processor needs and executes the services provided by the processor.
 *
 * @author Ram
 * @version 1.0
 */
public class PSFolderActionManager {
  /**
   * Constructs this manager with remote proxy that is set with folder context to handle actions
   * related to the folders.
   *
   * @param actionManager the action manager, which created this FolderActionMgr , never <code>null
   *     </code>
   * @param urlBase The base url of the applet (connection to server), may not be <code>null</code>
   * @throws PSCmsException if one of the proxies throws an exception, see the proxies for details
   */
  public PSFolderActionManager(PSActionManager actionManager, URL urlBase) throws PSCmsException {
    if (actionManager == null) throw new IllegalArgumentException("actionManager may not be null.");

    if (urlBase == null) throw new IllegalArgumentException("urlBase may not be null.");
    ...
    m_folderProxy = new PSFolderProcessorProxy(PSFolderProcessorProxy.PROCTYPE_REMOTE, agent);
  }

  /**
   * Creates a new folder and links to the folder represented by the parent folder node as a child.
   * This method does not check for duplicate name of the folder.
   *
   * @param parentFolderNode the node that represents the parent folder, may not be <code>null
   *     </code> and must represent a folder.
   * @param folder the new folder to create, may not be <code>null</code>
   * @return the folder object, never <code>null</code>
   * @throws PSCmsException if an error happens either creating or linking to parent folder.
   */
  public PSFolder createFolder(PSNode parentFolderNode, PSFolder folder) throws PSCmsException {
    ...
    PSSaveResults results = m_componentProxy.save(new IPSDbComponent[] {folder});
    ...
    try {
      ...
      m_componentProxy.delete(folder.getComponentType(), new PSKey[] {folder.getLocator()});
      throw e;
    }
    ...
    PSNode tmpNode =
        new PSNode(
            ...
            null,
            false,
            folder.getPermissions().getPermissions());
    tmpNode.setProperty(
        IPSConstants.PROPERTY_CONTENTID, String.valueOf(folder.getLocator().getId()));
  ...
  /**
   * Gets the fully qualified folder path(s) of the supplied id from the server.
   *
   * @param key Never <code>null</code>.
   * @return One or more paths of the form '//a/b/c...' if <code>key</code> is a child of a valid
   *     folder. If the supplied key is valid, but not a folder child, an empty array is returned.
   *     Otherwise, an exception is thrown.
   * @throws PSCmsException If any problems retrieving the paths, including a bad id.
   */
  ...
  /**
   * Clears the publish folder flags from all descendent folders of the folder passed in.
   *
   * @param folder the parent folder to get descendents from, cannot be <code>null</code>
   * @throws PSCmsException if any error occurs
   */
  public void clearDescendentPublishFlags(PSFolder folder) throws PSCmsException {
    if (folder == null) throw new IllegalArgumentException("folder cannot be null.");
    // Get all the descendant folder locators
    PSLocator[] descendentKeys =
    ...
    // Load the folders
    Element[] descendentsEl =
        m_componentProxy.load(
            ms_folderCompType, flaggedKeys.toArray(new PSKey[flaggedKeys.size()]));
    ...
    try {
      ...
      if (updatedfolder.size() > 0) {
        ...
        m_componentProxy.save(updatedfolder.toArray(comps));
  ...
  public PSFolder loadFolder(PSNode node) throws PSContentExplorerException {
    ...
    try {
      ...
      Element[] els = m_componentProxy.load(f.getComponentType(), new PSLocator[] {f.getLocator()});

      if (els.length < 1)
        throw new IllegalStateException("Could not load the folder " + f.getName());
  ...
  /**
   * Loads the children of the supplied folder node. The results returned from the folder processor
   * proxy are converted to the content explorer node objects.
   *
   * @param parentFolderNode the parent folder node whose children need to be loaded, may not be
   *     <code>null</code>
   * @return An iterator over zero or more child <code>PSNode</code> objects, never <code>null
   *     </code>. The list is grouped with folders first, and items following, and each group is
   *     sorted ascending case insensitive by the value of each node's <code>toString()</code>
   *     method.
  ...
  public Iterator loadChildren(PSNode parentFolderNode) throws PSContentExplorerException {
    ...
    try {
      ...
      if (StringUtils.isBlank(formatid)) {
        ...
        formatid = m_actionManager.getApplet().getDisplayFormatIdFromOptions(parentFolderNode);

        if (StringUtils.isBlank(formatid)) {
          // If not found in the options, obtain from the parent
          PSLocator key = PSActionManager.nodeToLocator(parentFolderNode);
          Element[] elems = m_componentProxy.load(ms_folderCompType, new PSKey[] {key});
          ...
          m_actionManager.getApplet().saveDisplayFormatIdToOptions(parentFolderNode);
        } else {
          parentFolderNode.setDisplayFormatId(formatid);
        }
      }
      ...
      search.setProperty(PSSearch.PROP_OVERRIDE_GLOBAL_MAX_RESULTS, PSSearch.BOOL_YES);
      if (isQuickExpand()) {
      ...
      Collections.sort(
          itemNodes, new PSStringComparator(PSStringComparator.SORT_CASE_INSENSITIVE_ASC));
  ...
  private String getFolderPath(PSNode folderNode) {
    ...
    while (current != null && !current.getType().equals("ROOT")) {
      ...
      PSNavigationTree.PSTreeNode parentTreeNode =
          (PSNavigationTree.PSTreeNode) treeNode.getParent();
      current = (PSNode) (parentTreeNode != null ? parentTreeNode.getUserObject() : null);
    }
    folderPath.insert(0, '/');
  ...
  /**
   * Adds the items or folders represented by the supplied node list to the folder represented by
   * the target folders. Grabs the ids from node properties and constructs locators (keys)
   * corresponding to items and folders persisting on server.
   *
   * @param tgtFolderNode the node representing the target folder, may not be <code>null</code> and
   *     must be of type <code>TYPE_FOLDER</code> or <code>TYPE_SYS_FOLDER</code> or <code>
   *     TYPE_SYS_SITE</code>.
   * @param nodeList list of nodes representing items or folders, may not be <code>null</code> or
   *     empty and must be of type <code>TYPE_FOLDER</code> only if the target node type is <code>
   *     TYPE_SYS_FOLDER</code> or <code>
   * TYPE_SYS_SITE</code>. <code>TYPE_ITEM</code> is only allowed for target of type <code>
   *     TYPE_FOLDER</code>
  ...
  /**
   * Checks the folders in the supplied list with children of the supplied target folder node and if
   * a matching folder by case-insensitive name comparison is found, it removes that from the
   * supplied list.
   *
  ...
  /**
   * Copies the items or folders represented by the supplied node list to the folder represented by
   * the target folder node. Grabs the ids from node properties and constructs locators (keys)
   * corresponding to items and folders persisting on server to execute the request.
   *
   * @param tgtFolderNode the node representing the target folder, may not be <code>null</code> and
   *     must be of type <code>TYPE_FOLDER</code> or <code>TYPE_SYS_FOLDER</code> or <code>
   *     TYPE_SYS_SITE</code>.
  ...
  /**
   * Get a set of all communities found for the supplied source node recursivly down to the bottom.
   *
   * @param source the source node from which to start the search for all communities in the tree,
   *     not <code>null</code>.
   * @return a set of all community ids found in the supplied source node recursivly down to the
  ...
  /**
   * Copy the tree rooted at <code>source</code> and link it as a child of <code>target</code>,
   * following the supplied options.
   *
   * @param source the source Site Folder or Site Subfolder which needs to be cloned, not <code>null
   *     </code>.
   * @param target the parent Folder into which the source will be cloned, not <code>null</code>.
   ...
   * @throws PSCmsException for any error.
   */
  public ErrorResults copyFolder(PSNode source, PSNode target, PSCloningOptions options)
    ...
    try {
      if (source.getType() == PSNode.TYPE_SITE) {
        ...
        queryParams.put(
            "sys_newFolderPath",
            normalizePath(
                m_actionManager.getApplet().getNavTree().getParentPath(source),
                options.getFolderName()));
        String query = PSUrlUtils.createUrl(base, queryParams.entrySet().iterator(), null);
        ...
        boolean success =
            PSXMLDomUtil.getBooleanData(PSXMLDomUtil.getAttributeTrimmed(root, "success"));
        if (!success) {
    ...
    ErrorResults results = new ErrorResults();
    results.m_logFileName =
        m_folderProxy.copyFolder(
            PSActionManager.nodeToLocator(source), PSActionManager.nodeToLocator(target), options);
  ...
  class ErrorResults {
    /**
     * The name of the log file containing error information generated while copying a folder tree.
     * Will be <code>null</code> if no errors occurred during this phase of the processing.
     */
    public String m_logFileName = null;

    /**
     * If any errors occurred while cloning the associated site, this will contain the message of
     * the exception and a stack trace. Will be <code>null</code> if no errors occurred during this
     ...
     *
     * @return <code>true</code> if at least 1 phase failed, <code>false</code> if everything was
     *     successful. On failure, check each member variable for non- <code>null</code>.
     */
  ...
  /**
   * If <code>path</code> is relative, it is normalized and appended onto <code>cwd</code>.
   * Normalization means removing all leading "../" and "./". All "\" are replaced with "/" in both
   * supplied paths before returning. Neither path is checked for well-formedness.
   *
   * @param cwd Assumed to be a well formed path.
   * @param path Assumed to be a well formed path.
   * @return If <code>path</code> is absolute, it is returned with only the slashes normalized.
   *     Otherwise, any leading .. or . are removed (and <code>cwd</code> adjusted accordingly) and
   *     <code>path</code> is appended to <code>cwd</code> and returned.
   * @throws PSCmsException If the number of parts in cwd aren't sufficient to normalize all the
   *     leading ".."s in path.
   */
  private String normalizePath(String cwd, String path) throws PSCmsException {
    ...
    if (tmpPath.startsWith("//")) return tmpPath;
    boolean done = false;
    while (!done) {
      if (tmpPath.startsWith("./")) tmpPath = tmpPath.substring(2, tmpPath.length());
    ...
    if (!tmpCwd.endsWith("/")) tmpCwd += "/";
    return tmpCwd + tmpPath;
  }

  /**
   * Looks for a specified format in the <code>root</code> and builds a string containing the error
   * text and stack trace. The expected dtd is as follows:
   *
   * <pre>
   *
   *
   ...
   *
   * @param root The document root returned from certain Rx requests. Never <code>null</code>.
   * @return If an error message is present, the text of the message and stack trace, otherwise an
   *     empty string.
   * @throws PSCmsException If the document contains the expected error element, but is otherwise
   *     malformed.
   */
  ...
  /**
   * Removes the items or folders represented by the supplied node list from the folder represented
   * by the parent node. Grabs the ids from node properties and constructs locators (keys)
   * corresponding to items and folders persisting on server to execute the request.
   *
   * @param parentNode the node representing the parent folder, may not be <code>null</code> and
   *     must be of type <code>TYPE_FOLDER</code> or <code>TYPE_SYS_FOLDER</code> or <code>
   *     TYPE_SYS_SITE</code>.
   * @param nodeList list of nodes representing items or folders, may not be <code>null</code> or
   *     empty and must be of type <code>TYPE_FOLDER</code> only if the parent node type is <code>
   *     TYPE_SYS_FOLDER</code> or <code>TYPE_SYS_SITE</code>. <code>TYPE_ITEM</code> is only
   *     allowed for parent of type <code>TYPE_FOLDER</code> This will orphan but not purge items
   *     other than navigation items. See deleteAllContent to remove all items that are not linked
   *     elsewhere.
   * @param force <code>true</code> to force the action in case child nodes are participating in
   ...
   * Removes the items or folders represented by the supplied node list from the folder represented
   * by the parent node. Grabs the ids from node properties and constructs locators (keys)
   * corresponding to items and folders persisting on server to execute the request. Will cascade
   * and purge all content
   *
   * @param parentNode the node representing the parent folder, may not be <code>null</code> and
   *     must be of type <code>TYPE_FOLDER</code> or <code>TYPE_SYS_FOLDER</code> or <code>
   *     TYPE_SYS_SITE</code>.
   ...
   * </code> and must be of type <code>TYPE_FOLDER</code> or <code>
   * TYPE_SYS_FOLDER</code> or <code>TYPE_SYS_SITE</code>.
   */
  private void validateNodeAsFolder(PSNode folderNode) {
    if (folderNode == null) throw new IllegalArgumentException("folderNode may not be null.");
  ...
  /**
   * Validates that the supplied child nodes can be linked/delinked to/from a target node of the
   * supplied type.
   *
      ...
      if (node.isOfType(PSNode.TYPE_ITEM)) {
        if (!PSActionManager.isFolderType(targetNodeType))
          throw new IllegalArgumentException(
              "Items can not be " + "linked/delinked to/from system folders or sites");
  ...
  /**
   * Moves the items or folders represented by the supplied node list from the folder represented by
   * the source folder node to the folder represented by the target folder node. Grabs the ids from
   * node properties and constructs locators (keys) corresponding to items and folders persisting on
   * server to execute the request.
   *
   * @param srcFolderNode the node representing the current parent folder of the supplied node list,
   *     may not be <code>null</code> and must be of type <code>TYPE_FOLDER</code> or <code>
   *     TYPE_SYS_FOLDER</code> or <code>TYPE_SYS_SITE</code>.
   * @param tgtFolderNode the node representing the target folder to which the the supplied node
   *     list need to be moved, may not be <code>null</code> and must be of type <code>TYPE_FOLDER
   *     </code> or <code>TYPE_SYS_FOLDER</code> or <code>TYPE_SYS_SITE</code>.
   * @param nodeList list of <code>PSNodes</code> representing items or folders, may not be <code>
   *     null</code> or empty and must be of type <code>TYPE_FOLDER</code> only if the target node
   *     type is <code>TYPE_SYS_FOLDER</code> or <code>TYPE_SYS_SITE</code>. <code>TYPE_ITEM</code>
   *     is only allowed for target of type <code>TYPE_FOLDER</code>
   * @param force <code>true</code> to force the action in case child nodes are participating in
   *     cross site links. <code>false</code> to error out in such a case.
   * @throws PSCmsException if an error happens while processing the request. If errors happen while
    ...
    if (!childNodes.isEmpty()) {
      ...
      while (children.hasNext()) {
        ...
        if (node.getType() == PSNode.TYPE_SITE
          ...
          PSNavigationTree tree = m_actionManager.getApplet().getNavTree();
          String srcPath = tree.getPath(srcFolderNode) + "/" + node.getName();
          String tgtPath = tree.getPath(tgtFolderNode) + "/" + node.getName();
  ...
  /**
   * Makes a request to the Rx server asking that all site definition objects that have a folder
   * path of <code>sourcePath</code> be changed to have a folder path of <code>targetPath</code>.
   * Each path should be of the form "//a/b/c".
   *
   * @param sourcePath The original folder path. Assumed not <code>null</code> or empty.
   * @param targetPath The new folder path. Assumed not <code>null</code> or empty.
   * @return <code>null</code> if successful, otherwise, text describing the error and stack trace.
   * @throws PSCmsException
   */
  private String updateSiteDefinitions(String sourcePath, String targetPath) throws PSCmsException {
    String error = null;
    try {
      ...
      if (!root.getNodeName().equals(NODE_NAME)) {
        // this should not happen unless the system is broken
        throw new RuntimeException(
            "Received an invalid document. Expected "
                + "root node="
                + NODE_NAME
                + " but received doc: "
                + PSXmlDocumentBuilder.toString(doc));
  ...
  /**
   * Checks the parent folder node whether this contains a child with the supplied folder name.
   *
   * @param parentFolderNode the parent folder node to check, assumed not to be <code>null</code>
   * @param folderName the folder name to check, assumed not to be <code>null
   * </code>or empty.
   * @return <code>true</code> if a folder with that name exists in the loaded children (current
  ...
  /**
   * Gets the displayformat identified by the supplied format id from the avialable list of folder
   * formats.
   *
   * @param formatid id of the display format to get, may not be <code>null
   * </code> or empty.
   * @param getDefault supply <code>true</code> to get the first one in the available list if the
   *     supplied formatid is not found.
   * @return the display format, may be <code>null</code> if the getDefault is <code>false</code>
   */
  public PSDisplayFormat getDisplayFormatById(String formatid, boolean getDefault) {
   ...
   * </code> object. The converted node is of type <code>TYPE_FOLDER</code>, with name and label as
   * folder name and is set with content if, revision and community id properties.
   *
  ...
  public static PSNode folderToNode(PSFolder folder) {
    ...
    props.setProperty(
        IPSConstants.PROPERTY_CONTENTID, Integer.toString(folder.getLocator().getId()));
    props.setProperty(
        IPSConstants.PROPERTY_REVISION, Integer.toString(folder.getLocator().getRevision()));
    props.setProperty(IPSConstants.PROPERTY_COMMUNITY, Integer.toString(folder.getCommunityId()));
  ...
  /**
   * Helper method to convert from <code>PSNode</code> object to <code>PSFolder</code> object.
   *
   * @param node the node to convert may not be <code>null</code> and must have properties for
   *     content id and community.
   * @return The returned folder is a partial representation of the folder as it exists in the db.
   *     Only the name and locator are valid values, never <code>null</code>
   */
  public static PSFolder nodeToFolder(PSNode node) {
    ...
    try {
      ...
      throw new IllegalArgumentException("Invalid node as a folder - " + node);
    }

    return new PSFolder(node.getName(), contid, commid, node.getPermissions().getPermissions(), "");
  }

  /**
   * Convenience method to convert a list of <code>PSNode</code> objects to <code>PSFolder</code>
   * objects.
   *
   ...
   *
   * @param folder the folder summary for which to get the type, never <code>null</code>.
   * @param parent the parent node of the supplied folder, never <code>null</code>.
   * @return the folder type, always one of the <code>PSNode.TYPE_xxx</code> values that are folder
   *     types.
   */
  public String getFolderType(PSComponentSummary folder, PSNode parent) {
    if (folder == null) throw new IllegalArgumentException("folder cannot be null");
    if (parent == null) throw new IllegalArgumentException("parent node cannot be null");
   ...
   * @param folderName The name of the folder, may not be <code>null</code> or empty.
   * @param parent the parent node of the supplied folder, never <code>null</code>.
   * @return the folder type, always one of the <code>PSNode.TYPE_xxx</code> values that are folder
   *     types.
   */
  public String getFolderType(int folderId, String folderName, PSNode parent) {
    ...
    if (StringUtils.isBlank(folderName))
      throw new IllegalArgumentException("folderName may not be null or empty");

    String type = PSNode.TYPE_FOLDER;
    try {
      int id = folderId;
      if (id == PSFolder.ROOT_ID) type = PSNode.TYPE_FOLDER;
      else if (id == PSFolder.SYS_SITES_ID) type = PSNode.TYPE_SYS_SITES;
      else if (id == PSFolder.SYS_FOLDERS_ID) type = PSNode.TYPE_SYS_FOLDERS;
      else if (parent.isSiteFolder() || parent.isSiteSubfolder()) type = PSNode.TYPE_SITESUBFOLDER;
      else {
        PSNavigationTree navtree = m_actionManager.getApplet().getNavTree();
        if (navtree == null) {
        ...
        while (sites.hasNext()) {
          ...
          if (path.equalsIgnoreCase(sitePath)) return PSNode.TYPE_SITE;
        }
      }
    } catch (PSCmsException e) {
   ...
   *
   * @param type the folder type for which to get the icon key, one of the <code>PSNode.TYPE_xxx
   *     </code> values that are folders.
   * @return the folder icon key for the supplied type, never <code>null</code> or empty.
   */
  public static String getFolderIconKey(String type) {
    String iconKey = null;
    if (PSNode.TYPE_FOLDER.equals(type)) iconKey = "Folder";
    else if (PSNode.TYPE_SITE.equals(type)) iconKey = "Site";
    else if (PSNode.TYPE_SITESUBFOLDER.equals(type)) iconKey = "SiteSubfolder";
    else if (PSNode.TYPE_SYS_FOLDERS.equals(type)) iconKey = "Folders";
    else if (PSNode.TYPE_SYS_SITES.equals(type)) iconKey = "Sites";

    if (iconKey == null) {
      throw new IllegalArgumentException(
          "Invalid type supplied. Must be "
  ...
  /**
   * If this field is <code>true</code>, then this manager should be using a fast expand on folder
   * children nodes. This mode will not fully load any non-folder children. Used in applet startup
   * to speed the expansion of the folder hierarchy. Always <code>false</code> after startup
   * completes.
   *
  ...
  /**
   * Add a search listener to the list of search listeners held for this manager.
   *
  ...
  /**
   * Runs the search completed methods of any search listeners to updated item indicators.
   *
   * @param current The current node being manipulated, must never be <code>null</code>
   */
  ...
  /**
   * Component processor proxy. Initialized in the constructor, never <code>null</code> or modified
   * after that.
   */
  private PSComponentProcessorProxy m_componentProxy = null;

  /**
   * Folder processor proxy. Initialized in the constructor, never <code>null</code> or modified
   * after that.
   */
  private PSFolderProcessorProxy m_folderProxy = null;

  /**
   * Action manager, which created this FolderActionMgr, never <code>null</code>, after the ctor is
   * finished.
   */
  ...
  /**
   * List of search listeners, never <code>null</code>, but could be empty. Added to by {@link
   * #addSearchListener(IPSSearchListener)}and used by {@link #runSearchCompleted(PSNode)}. Note
   * that there is no current mechanism to remove listeners.
   */
  private Collection<IPSSearchListener> m_searchListeners = new ArrayList<IPSSearchListener>();

  /** The name of the component type for folders to use with the processor proxy. */
  public static final String ms_folderCompType = PSFolder.getComponentType(PSFolder.class);
