...
public class PSNode implements IPSComponent, Cloneable, PSNavigationTree.IPSTreeNodeAssociation {
   ...
   * @param childrenURL relative url to the applet to execute for loading the children, may be
   *     <code>null</code> or empty.
   * @param iconKey the icon key for the node, may be <code>null</code> or empty in which case the
   *     type becomes the icon key.
   * @param expand supply <code>true</code> to show this node as expanded, otherwise <code>false
   *     </code>
   * @param permissions access mask for determining the level of access for the user accessing this
   *     component. This should be used if this object represents a node of type "folder". For other
   *     object types, specify a value of "-1". For objects of type "folder" this must be a valid
   *     access level (should be non-negative).
  ...
  public PSNode(
    ...
    m_label = label;
    m_type = type;
    if (iconKey == null || iconKey.length() < 1) m_iconKey = type;
    else m_iconKey = iconKey;
  ...
  /**
   * Constructs the node object from the supplied element. See {@link #toXml(Document) } for the
  ...
  public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
    ...
    if (m_name == null || m_name.length() < 1) m_name = "name_missing";
    m_label = sourceNode.getAttribute(LABEL_ATTR);
    if (m_label == null || m_label.length() < 1) m_label = m_name;
    m_type = PSComponentUtils.getEnumeratedAttribute(sourceNode, TYPE_ATTR, ms_nodeTypes);
    ...
    Element propsEl =
        PSComponentUtils.getChildElement(sourceNode, PSProperties.XML_NODE_NAME, false);
    m_props = null;

    if (propsEl != null) m_props = new PSProperties(propsEl);

    // Get TableMeta
    m_tableMeta = null;
    Element tableMetaEl = PSComponentUtils.getChildElement(sourceNode, TABLEMETA_NODE, false);
    if (tableMetaEl != null && tableMetaEl.hasChildNodes())
      m_tableMeta = new TableMeta(tableMetaEl);

    // Get RowData
    m_rowData = null;
    Element rowDataEl = PSComponentUtils.getChildElement(sourceNode, ROWDATA_NODE, false);
  ...
  /**
   * Replaces the child matching the supplied child's name with the supplied child.
   *
   ...
   * @throws IllegalStateException if matching child did not find or does not loaded with children
   */
  public void replaceChild(PSNode newChild) {
    if (newChild == null) throw new IllegalArgumentException("newChild may not be null.");
    ...
    if (childIndex == -1) throw new IllegalArgumentException("children are not found to replace");
  ...
  public Element toXml(Document doc) {
    ...
    if (m_helpTypeHint != null) root.setAttribute(HELP_TYPE_HINT_ATTR, m_helpTypeHint);
    root.setAttribute(ICONKEY_ATTR, m_iconKey);
    root.setAttribute(CHILD_URL_ATTR, m_childrenURL);
    root.setAttribute(
        EXPANDED_ATTR,
        m_isExpand ? PSComponentUtils.XML_BOOLEAN_TRUE : PSComponentUtils.XML_BOOLEAN_FALSE);

    if (isAnyFolderType()) {
      root.setAttribute(PERMISSIONS_ATTR, "" + m_permissions.getPermissions());
    }

    if (m_props != null) {
      Element props = m_props.toXml(doc);
      if (props != null) root.appendChild(props);
    }

    if (m_tableMeta != null) root.appendChild(m_tableMeta.toXml(doc));
   ...
   *
   * @param isExpand supply <code>true</code> to expand, <code>false</code> to show in collapsed
   *     state.
   */
  ...
  /**
   * Sets the show/hide state of this node. This flag is used to hide a node from Content Explorer
   * temporarily. For example, we do not want to show the New Search node until search is performed
   * at least once.<code>true</code> indicates to hide, <code>false</code> indicates to show.
   * Defaults to <code> false</code>.
   *
   * @param hide supply <code>true</code> to hide, <code>false</code> to show in Content Explorer
   */
  public void setHidden(boolean hide) {
    if (hide) {
      setProperty(IPSConstants.PROPERTY_HIDDEN, IPSConstants.PROPERTY_TRUE);
    } else {
      setProperty(IPSConstants.PROPERTY_HIDDEN, IPSConstants.PROPERTY_FALSE);
    }
  }

  /**
   * Gets the tabular display format of this node's children. The display format provides each
   * column definition in the table with column name and supported data type for that column values.
   * The allowed data types are DATA_TYPE_xxx values.
   *
   * @return the format, list of <code>Map.Entry</code> objects with key as column name and value is
  ...
  /**
   * Sets/Replaces the display format of the node to display its children in the table as defined by
   * the column definitions.
   *
   * @param columnDefs An iterator over one or more column defitions defined by <code>Map.Entry
   ...
   * </code> and must conform to the format of <code>TABLEMETA</code> element as defined in {@link
   *     #toXml(Document)}.
   * @throws PSUnknownNodeTypeException if the element is not of expected format.
   */
  public void setChildrenDisplayFormat(Element tableMeta) throws PSUnknownNodeTypeException {
    if (tableMeta == null) throw new IllegalArgumentException("tableMeta may not be null.");
  ...
  /**
   * Gets the data of this node to display in tabular format according to its parent's display
   * format for children. See {@link #getChildrenDisplayFormat() } for more information.
   *
   * @return the data, each entry represents data for a column with key as column name (<code>String
   *     </code>) and value is value for that column (<code>Object</code>). The value object depends
   ...
   *
   * @return Returns the number of children in the node, or <code>-1</code> if there is no child
   *     list.
   */
  ...
  /**
   * Nodes of type 'ROOT','SystemCategory' and 'Category' do not support dynamic children unless it
   * has a dynamic url. All other nodes support/can have dynamic children.
   *
   * @return <code>true</code> if it can load children dynamically, otherwise <code>false</code>
   */
  ...
  /**
   * All nodes except nodes of type <code>TYPE_ITEM</code> are container nodes. Nodes that are
   * containers are only showed in the navigational tree.
   *
   * @return <code>true</code> if this node represents a container node, otherwise <code>false
   *     </code>
   */
   ...
   *
   * @return <code>true</code> if the node is marked to be hidden in Content Explorer, <code>false
   *     </code> otherwise.
  ...
  /**
   * Sets children on this node. This replaces any previous children. The supplied iterator is
   * looped-through in this method, so <code>hasNext()
   * </code> invocation will return <code>false</code> after this method execution.
   *
  ...
  public void removeChildren() {
    if (m_children != null) m_children = null;
  }
   ...
   *
   * @return the type, never <code>null</code> or empty, will be one of the TYPE_xxx values.
   */
   ...
   *
   * @return the icon key, never <code>null</code> or empty, will be one of the TYPE_xxx values if
   *     the icon key is not available.
   */
  ...
  /**
   * Get the system property <code>PROPERTY_SEARCHID</code>. This property is not be applicable to
   * every type of node in which case an empty string is returned. Folder, View, Search Result nodes
   * have search attached to them.
   *
   * @return search id which is one of the optional property of the node. Never <code>null</code>,
  ...
  /**
   * Set the system property <code>PROPERTY_SEARCHID</code> for the given node.
   *
   * @param searchId which is one of the optional property of the node. Never <code>null</code>, may
   *     be <code>empty</code>.
   */
  ...
  /**
   * Get the system property NODE_PROPERTY_DISPLAYFORMATID. This property is not be applicable to
   * every type of node in which case an empty string is returned. Folder, View, Search Result nodes
   * have display format attached to them.
   *
   * @return display format id which is one of the optional property of the node. Never <code>null
  ...
  /**
   * Set the system property NODE_PROPERTY_DISPLAYFORMATID. This property is not be applicable to
   * every type of node in which case an empty string is returned. Folder, View, Search Result nodes
   * have display format attached to them. Has no efffect if the id being set is the same as the one
   * currently stored in this object. Otherwise also calls {@link #setLastSortColumns(List)
   * setLastSortColumns(null)} and {@link #setLastSortedAsc(boolean) setLastSortedAsc(true)}.
   *
  ...
  /**
   * Get the system property <code>PROPERTY_CONTENTID</code>. This property is not be applicable to
   * every type of node in which case an empty string is returned. Folder and Item nodes have
   * contentid property.
   *
   * @return content id which is one of the optional property of the node. Never <code>null</code>,
   *     may be <code>empty</code>.
   */
  ...
  /**
   * Get the system property <code>PROPERTY_REVISION</code>. This property is not be applicable to
   * every type of node in which case an empty string is returned. Folder and Item nodes have
   * revision property, though revision for Folder may not make sense.
   *
   * @return revsion which is one of the optional property of the node. Never <code>null</code>, may
   *     be <code>empty</code>.
   */
  ...
  /**
   * Return the relationship id for this Node. This is only valid for slot item nodes, if not a slot
   * item, then this will return -1.
   *
   * @return an int of the internal relationship id specifying the relationship for this node, if
   *     the node is not a slot item, returns -1.
   */
   ...
   *
   * @param key the property key to lookup, may not be <code>null</code> or empty.
   * @return the value for the specified key, never <code>null</code>, may be empty if the key is
   *     not found or the properties have not been inited
   */
   ...
   * determines the level of access for the user accessing this node. Currently this mask is
   ...
   *
   * @param permissions the permissions for this node, may not be <code>null</code> if this node
   *     represents any "folder" object.
   * @see #getPermissions()
   */
  public void setPermissions(PSObjectPermissions permissions) {
    if (isAnyFolderType()) {
      if (permissions == null) throw new IllegalArgumentException("permissions may not be null");
    }
  ...
  /**
   * Determine if this node is of type system folder. The current defined system folder types are:
   *
   * <ul>
   *   <li>Sites
   *   <li>Folders
  ...
  /**
   * Determine if the specified type is of type folder. The current defined folder types are:
   *
   * <ul>
   *   <li>Folder
   *   <li>Site
   ...
   * @return <code>true</code> if it is of type folder, <code>false</code> otherwise.
   */
  ...
  /**
   * Determine if this node is of any folder type. This is the ORed result of {@link
   * #isSystemFolderType()} and {@link #isFolderType()}.
   *
  ...
  /**
   * Is this node a site folder? Site folders are determined through the fact that they are listed
   * as <code>FOLDER_ROOT</code> in the <code>RXSITES</code> table.
   *
   * @return <code>true</code> if this is a site folder, <code>false</code> otherwise.
   */
  ...
  /**
   * Is this node a site subfolder? All folders which have a site folder or site subfolder as their
   * parent are considered site subfolders.
   *
   * @return <code>true</code> if this is a site subfolder, <code>false</code> otherwise.
   */
  ...
  /**
   * Determine if this node is a type of search node. This means its children are loaded using some
   * type of search or view.
   *
   * @return <code>true</code> if it is a search type, <code>false</code> otherwise.
   */
   ...
   *
   * @return <code>true</code> if it is an item type, <code>false</code> otherwise.
   */
  public boolean isItemType() {
    String type = getType();

    return type.equals(TYPE_ITEM) || type.equals(TYPE_DTITEM) || type.equals(TYPE_SLOT_ITEM);
  }

  /**
   * Get a readonly collection of folder types. See {@link #isFolderType()} for more info.
   *
  ...
  /**
   * Get a readonly collection of search types. See {@link #isSearchType()} for more info.
   *
   * @return The collection, never <code>null</code> or empty.
   */
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PSNode)) equals = false;
    else {
      PSNode other = (PSNode) obj;
      if (!m_name.equals(other.m_name)) equals = false;
      else if (!m_label.equals(other.m_label)) equals = false;
      else if (!m_type.equals(other.m_type)) equals = false;
      else if (!m_childrenURL.equals(other.m_childrenURL)) equals = false;
      else if (!m_iconKey.equals(other.m_iconKey)) equals = false;
      else if (m_isExpand ^ other.m_isExpand) equals = false;
      else if (m_dynamicPropsLoaded ^ other.m_dynamicPropsLoaded) equals = false;
      else if (m_props == null ^ other.m_props == null) equals = false;
      else if (m_props != null && !m_props.equals(other.m_props)) equals = false;
      else if (m_children == null ^ other.m_children == null) equals = false;
      else if (m_children != null && !m_children.equals(other.m_children)) equals = false;
      else if (m_tableMeta == null ^ other.m_tableMeta == null) equals = false;
      else if (m_tableMeta != null && !m_tableMeta.equals(other.m_tableMeta)) equals = false;
      else if (m_rowData == null ^ other.m_rowData == null) equals = false;
      else if (m_rowData != null && !m_rowData.equals(other.m_rowData)) equals = false;
      else if (m_truncated != other.m_truncated) equals = false;
    }
  ...
  public Object clone() {
    ...
    try {
      ...
      if (m_props != null) copy.m_props = (PSProperties) m_props.clone();

      if (m_tableMeta != null) copy.m_tableMeta = (TableMeta) m_tableMeta.clone();

      if (m_rowData != null) copy.m_rowData = (RowData) m_rowData.clone();
  ...
  private class TableMeta implements IPSComponent, Cloneable {
     ...
     *
     * @param columnDefs An iterator over one or more <code>Map.Entry</code> objects. See {@link
    ...
    public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
      ...
      while (columns.hasNext()) {
        ...
        String name = PSComponentUtils.getRequiredAttribute(columnDef, NAME_ATTR);
        /*
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof TableMeta) {
        ...
        if (m_columnDefs.equals(other.m_columnDefs)) equals = true;
      }
    ...
    /**
     * The list of column definitions of this table meta, with column name as key (<code>String
  ...
  /**
   * The class that defines the row data of this node to display in tabular format as per its
   * parent's table metadata.
   */
  private class RowData implements IPSComponent, Cloneable {
    /**
     * Constructs the row data from supplied map.
     *
     * @param columnData the data for each column (name-value), may not be <code>null</code> or
     *     empty.
    ...
    public void fromXml(Element sourceNode) throws PSUnknownNodeTypeException {
      ...
      while (columns.hasNext()) {
        ...
        String name = PSComponentUtils.getRequiredAttribute(columnData, NAME_ATTR);
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof RowData) {
        ...
        if (m_columnData.equals(other.m_columnData)) equals = true;
      }
    ...
    /**
     * The list of column data entries of this row data, with column name as key (<code>String
  ...
  /**
   * Replace the existing properties (if any) with the supplied one. This is useful when creating
   * new new object not using the fromXml() method.
   *
   * @param props may be <code>null</code>.
   * @return new properties object, is <code>null</code> if the supplied parameter is <code>null
   *     </code>.
   */
   ...
   *
   * @param newLabel new label for the node to set, may not be <code>null</code> or <code>empty
   *     </code>
   */
   ...
   *
   * @param newType new type to set to the node, may not be <code>null</code> or <code>empty</code>
   *     and must be one of the valid types.
   */
   ...
   *
   * @param hint help type hint to set to the node, may be <code>null</code> or must be one of the
   *     valid help type hints.
   */
  ...
  /**
   * Easy method to check if this node type equals the supplied type. Check is case-sensitive.
   *
   * @param type the node type, may be <code>null</code> in which case the result will be <code>
   *     false</code>.
   * @return <code>true</code> if the node type equals the specified type, <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Easy method to check if this node type matches with the supplied type. Check is case-sensitive.
   *
   * @param type the node type, may be <code>null</code> in which case the result will be <code>
   *     false</code>.
   * @return <code>true</code> if the node type mactches with specified type, <code>false</code>
   *     otherwise.
   */
  public boolean isMatchingType(String type) {
    // first check equivalence
    String thisType = getType();
    if (thisType.equals(type)) return true;

    // now check match on folder and folder ref.
    if (isFolderType(thisType) && TYPE_FOLDER_REF.equals(type)) return true;

    return thisType.equals(TYPE_FOLDER_REF) && isFolderType(type);
  }

  /**
   * Sets whether dynamic properties are loaded. These properties are not supplied when the node is
   * created, and are loaded lazily for performance reasons and added to the node when needed.
   * Should be called supplying <code>true</code> for the <code>loaded</code> param once this has
   * been done.
   *
   * @param loaded <code>true</code> to indicate that the properties returned by {@link
   *     #getProperties()} included dynamic properties, <code>false</code> if not.
   */
  ...
  /**
   * Determines if dynamic properties have been loaded and set on this object. See {@link
   * #setAreDynamicPropsLoaded(boolean)} for more info.
   *
   * @return <code>true</code> if they have been loaded, <code>false</code> otherwise.
   */
  ...
  /**
   * Sets whether this node should be marked as dirty, meaning it needs to be refreshed.
   *
   * @param isDirty <code>true</code> to indicate it is dirty, <code>false</code> otherwise.
   */
  ...
  /**
   * Determines if this node is marked as dirty. See {@link #setIsDirty(boolean)} for more info.
   *
  ...
  /**
   * Gets the children of this node for which {@link #isDirty()} returns <code>true</code>.
   *
   * @param recurse <code>true</code> to perform this check recursively against children of this
  ...
  /**
   * Clears the dirty flag for children of this node for which {@link #isDirty()} returns <code>true
   * </code>.
   *
   * @param recurse <code>true</code> to perform this check recursively against children of this
   *     node, <code>false</code> otherwise.
  ...
  /**
   * Determines if there are any children of this node for which {@link #isDirty()} returns <code>
   * true</code>. It will check child nodes recursively only if {@link #isFolderType()} returns
   * <code>false</code> for the node whose children are being checked.
   *
   * @return <code>true</code> if any children are found to be dirty, <code>false</code> otherwise.
   */
   ...
   * @param nodeType The type to match, not <code>null</code> or empty, one of the <code>TYPE_XXX
   *     </code> values.
   * @param recurse <code>true</code> to perform this check recursively against children of this
   *     node, <code>false</code> to check immediate children only.
  ...
  /**
   * Easy method to check if the node type specified by the string belongs to one of the valid node
   * types defined in this class.
   *
   * @param type the node type, may be <code>null</code> in which case the result will be <code>
   *     false</code>.
   * @return <code>true</code> if the specified type is one of the known node types, otherwise
   *     <code>false</code>.
   */
  public static boolean isTypeValid(String type) {
    if (type != null) return ms_nodeTypes.contains(type);
  ...
  /**
   * Gets the most recent search results for this node, may be <code>null</code> if none have been
   * set or if they have been cleared.
   *
   ...
   *
   * @param type The node type, expected to be one of the <code>TYPE_xxx</code> values. May not be
   *     <code>null</code> or empty.
   * @return <code>true</code> if a child node of the specified type is found, <code>false</code>
   *     otherwise.
   */
  ...
  /**
   * Derives a result count from the search result document if there is one. If there is no
   * document, then the count is left at <code>-1</code>. This means that it will always be <code>-1
   * </code> for non search nodes.
   *
   * @return a count of the search results, or <code>-1</code> if the node is not a search node.
   *     Search nodes will have a result that is zero or greater.
   */
  ...
  /**
   * Call this to change the search result count. The only valid use of this is for the folder
   * nodes, to fix the count for display purposes after one or more items are moved.
   */
  ...
  /**
   * A node can be quick loaded, which indicates that it needs to be entirely refreshed on next
   * access. This is used to partially load folder information to speed applet loading.
   *
   ...
   *
   * @return <code>true</code> if it was ascending, <code>false</code> if descending. Value should
   *     be ignored if {@link #getLastSortColumns()} returns <code>null</code>.
   */
  ...
  /**
   * Set the last column indexes used for sorting. See {@link #getLastSortColumns()} for more info.
   *
  ...
  /**
   * Set if last sorting was ascending or descending. See {@link #getLastSortedAsc()} for more info.
   *
   * @param ascending <code>true</code> if ascending, <code>false</code> otherwise.
   */
   ...
   *
   * @param url The url used to obtain the child actions from the server, may not be <code>null
   *     </code> or empty.
  ...
  /**
   * Set the child actions for the specified url. See {@link #getChildMenuActions(String)}.
   *
  ...
  public void setChildMenuActions(String url, List<PSMenuAction> actions) {
    if (StringUtils.isBlank(url))
      throw new IllegalArgumentException("url may not be null or empty");
    if (actions == null) throw new IllegalArgumentException("actions may not be null");

    m_childMenuCache.put(url, actions);
  }

  /**
   * Create a dummy node representing a new item with a content id of -1 and a dirty flag set to
   * <code>true</code>.
   *
   * @return The node, never <code>null</code>.
   */
  public static PSNode createDirtyNewItemNode() {
    PSNode tmpNode = new PSNode("tmp", "tmp", TYPE_ITEM, null, null, false, -1);
  ...
  /**
   * Name of the node, initialized in constructor and never <code>null</code>, empty or modified
   * after that.
   */
  private String m_name;

  /**
   * Type of the node, initialized in constructor and never <code>null</code>, empty or modified
   * after that, will be one of the TYPE_xxx values.
   */
  private String m_type;

  /**
   * An extra bit of information to help determine the type of help that should be displayed. May be
   * <code>null</code>.
   */
  ...
  /**
   * The url to get children of the node, initialized in constructor and never <code>null</code> or
   * modified after that. May be empty.
   */
  private String m_childrenURL;

  /**
   * The icon key name for the node to use when rendering in the applet. This is read from an
   * optional attribute "iconKey" of the Node XML. If the attribute is absent the mandatory
   * attribute "type" becomes the iconKey. Initialized in the constructor and never <code>null
   * </code> or <code>empty</code> modified after that.
   */
  private String m_iconKey;

  /**
   * The flag to indicate that whether this node should be expanded or collapsed when it is
   * represented as tree node. <code>true</code> indicates to expand, <code>false</code> indicates
   * to collapse. Defaults to <code>
   * false</code> if not specified. Initialized in the constructor and may be modified through calls
   * to <code>setExpand(boolean)</code>.
   */
  private boolean m_isExpand;

  /**
   * The properties associated with the node, initialized in ctor and may be <code>null</code> if
   * not specified.
   */
  private PSProperties m_props = null;

  /**
   * The metadata of this node's children to display in tabular format, initialized in ctor and may
   * be <code>null</code> if not specified.
   */
  private TableMeta m_tableMeta = null;

  /**
   * The row data of this node according to its parent's table metadata to display in tabular
   * format, initialized in ctor and may be <code>null
   * </code> if not specified.
   */
  private RowData m_rowData = null;

  /**
   * The list of child nodes, initialized to <code>null</code> and gets filled as it is loaded from
   * xml or through calls to <code>setChildren(Iterator)
   ...
   * the item. Currently this has relevance only if the encapsulated node is of type "folder".
   * Initialized in the constructor or <code>fromXml()</code> method if the object type is "folder".
   * Never <code>null</code> if this node if for a folder object, otherwise <code>null</code>. Never
   * modified after initialization.
   */
  private PSObjectPermissions m_permissions = null;

  /**
   * The results of the most recent search, may be <code>null</code> if none have been set or if
   * they have been cleared. Modified by {@link #setSearchResults(Document)}.
   */
  private Document m_searchResults = null;

  /**
   * The number of results in {@link #m_searchResults}. This is initialized the first time {@link
   * #getSearchResultCount()} is called, and is reset in {@link #setSearchResults(Document)}.
   */
  ...
  /**
   * The constant to indicate that this node represents a folder but is not actually a folder. Such
   * a node 'looks' like a folder (similar icon), but has no children and has its own menu context.
   * This node ignores permissions.
   */
   ...
   * modified by calls to {@link #setAreDynamicPropsLoaded(boolean)}.
   */
  private boolean m_dynamicPropsLoaded = false;

  /**
   * Determines if this node has been marked as dirty. Initially <code>false</code>, modified by
   * calls to {@link #setIsDirty(boolean)}.
   */
  private boolean m_isDirty = false;

  /**
   * Indicates if this node has fewer items than requested in a search. Only modified through the
   * accessors on this class.
   */
  private boolean m_truncated = false;

  /**
   * The list of indexs of the last columns used for sorting. Initially <code>null</code> or if not
   * specified. Previous value is reset to <code>null</code> when {@link
   ...
   * </code> if was descending. Initially <code>true</code>, should be ignored if the value of
   * {@link #m_lastSortCols} is <code>null</code>.
   */
  private boolean m_lastSortedAsc = true;

  /**
   * Storage location for the tree node that owns this node as its user object. Will be <code>null
   * </code> if this node is not owned by a tree node, otherwise it will be valid. Only manipulated
   * by the <code>[get,set]AssociatedTreeNode</code> methods.
   */
