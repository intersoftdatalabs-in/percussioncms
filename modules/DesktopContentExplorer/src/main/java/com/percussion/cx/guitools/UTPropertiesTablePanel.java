...
/**
 * A reusable panel containing a table with two or more columns. The first two columns are required,
 * the third one is optional. For example it allows to create a table with Name and Value columns;
 * or a table with Name, Value and Description columns.
 */
public class UTPropertiesTablePanel extends JPanel implements KeyListener, FocusListener {
  /**
   * Inner, private class that provides a table cell renderer which shows a focus highlight when
   * configured to do so.
   */
  class UTTablePanelCellRenderer extends DefaultTableCellRenderer {
    /** Border to use when cell has focus */
    private final Border mi_focusBorder = new PSFocusBorder(1, m_tableCellFocusColor, true);
   ...
   * </code> or empty ands should have only two entries spcifying the two column names.
  ...
  public void init(String[] colNames, int rows, boolean isEditable) {
    ...
    KeyStroke ksEnterRelease = KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0, true);
    ...
    AbstractAction stopEditing =
        new AbstractAction() {
          ...
          public void actionPerformed(ActionEvent e) {
            if (m_table.isEditing()) stopExtTableEditing();
          }
        };
  ...
  private class PopupListener extends MouseAdapter {
    ...
    private void maybeShowPopup(MouseEvent e) {
      if (e.isPopupTrigger()) m_popup.show(e.getComponent(), e.getX(), e.getY());
    }
  }

  /** Adds popup menu to the panel. Menu gets displayed on right click of the mouse. */
  ...
  public void setTableModel(TableModel model) {
    if (model != null) {
      ...
      if (model instanceof DefaultTableModel) ((DefaultTableModel) model).fireTableDataChanged();
    }
  }
  ...
  public void editCellAt(int row, int col) {
    ...
    if (!m_table.isCellEditable(row, col)) return;
  ...
  protected void stopExtTableEditing() {
    if (m_table.isEditing()) {
      ...
      m_table.getCellEditor(m_table.getEditingRow(), m_table.getEditingColumn()).stopCellEditing();
      ...
      if (null != name && name.trim().length() == 0)
        for (int i = VALUE_COLUMN; i < m_colNames.length; i++) m_table.setValueAt("", row, i);
    }
    ...
    for (int r = 0; r < rows; r++) {
      ...
      if (null == o || o.toString().trim().length() == 0) emptyCount++;
    }

    if (emptyCount < 1) for (int i = 0; i < 3; i++) addRow(); // add several empty rows
  }
      ...
      if (name != null && name.trim().length() > 0) {
        if (value == null || value.trim().length() == 0) {
          ...
          ErrorDialogs.showErrorDialog(
              this, msg, getResourceString("Validation Error"), JOptionPane.ERROR_MESSAGE);
        ...
        if (propNames.containsKey(name.toLowerCase())) {
          String msg = getResourceString("Found duplicate property name: <{0}>.");
          ...
          return false;
        } else propNames.put(name.toLowerCase(), null);
  ...
  private String getResourceString(String key) {
    ...
    if (key.startsWith("@")) key = clazzName + key;
    else key = clazzName + "@" + key;
  ...
  /**
   * Color to use for focus highlights on table cells. Initialized to a default value here,
   * overriden by the accessors. This is a no-op unless the field {@link #m_tableUseFocusHighlight}
   * is <code>true</code>
   */
  private Color m_tableCellFocusColor = Color.red;

  /**
   * If this is <code>true</code>, then use a focus border on the table cells. This can be set via
   * the accessors for this field.
   */
  private boolean m_tableUseFocusHighlight = false;

  /**
   * These fields are used to reference the Property Name and Property Value and optionaly Property
