...
public class PSDisplayFormatOption implements IPSClientObjects {
  ...
  public void fromXml(Element sourceNode) throws PSContentExplorerException {
    ...
    try {
      ...
      while (el != null && el.getNodeName().equals(ELEM_ITEM)) {
        String itemPath = PSXMLDomUtil.checkAttribute(el, ATTR_ITEM_PATH, true);
        String displayFormatId = PSXMLDomUtil.checkAttribute(el, ATTR_DISPLAY_FORMAT_ID, true);
  ...
  public boolean equals(Object obj) {
    ...
    if (m_map.equals(comp.m_map) == false) return false;
   ...
   *
   * @param itemPath a string representing an existing item within the saved item map, must not be
   *     <code>null</code> or empty.
   * @return a string representing the saved display format for the given item path, if the item
   *     path is not found returns null.
   */
   ...
   *
   * @param itemPath a string representing the key for the map to save the display format for, must
   *     not be <code>null</code> or empty.
   * @param displayFormatId a string representing the display format for the specified item path,
   ...
   *
   * @param itemPath a string representing the key for the map to remove the display format for,
