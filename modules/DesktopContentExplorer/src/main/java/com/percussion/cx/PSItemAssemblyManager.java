...
public class PSItemAssemblyManager {
  /**
   * Reference to the content explorer action manager. Basically to acces the content explorer
   * applet.
   */
  ...
  public PSItemAssemblyManager(PSActionManager actionManager) {
    if (actionManager == null) throw new IllegalArgumentException("actionManager must not be null");

    m_actionManager = actionManager;
  }

  /**
   * Insert the supplied node list to the supplied target node at the specified target.
   *
   * @param targetParent this is a slot target location of where to insert the specified list of
   *     nodes, must not be <code>null</code>
   * @param target this is a slot item target, may be a target item or a target slot, if target item
   *     we use this index to decide where to insert, otherwise we insert at the end of the target,
  ...
  /**
   * Inserts the items supplied in the node list to the supplied slot of the owner item represented
   * by content id and revision. The items will be added at the end.
   *
   ...
   * @param direction the direction to move the specified nodes, &lt; 0 is move up, &gt; 0 is move
   *     down
   */
   ...
   *
   * @param targetParent this is a slot target location of where to update the specified list of
   *     nodes, must not be <code>null</code>
   * @param target this is a slot item target, may be a target item or a target slot, if target item
   *     we use this index to decide where to update, otherwise we update to the end of the target,
   *     must not be <code>null</code>
   ...
   *
   * @param targetParent this is a slot target location of where to reorder the specified list of
   *     nodes, must not be <code>null</code>
   * @param target this is a slot item target, may be a target item or a target slot, if target item
   *     we use this index to decide where to reorder, otherwise we reorder to the end of the
  ...
  /**
   * Create an active assembler handler request to use as a base for actions. The calling routines
   * should reset the index if they do not want the action to insert/reorder/update to the end.
   *
   * @param targetSlot the slot node from which to get the slot id which all actions need to effect
   *     changes, must not be <code>null</code>
   * @param nodeList the list of nodes to put in the list of nodes to be acted upon when the active
   *     assembler handler is called, must not be <code>null
   * </code>
  ...
  /**
   * Creates an active assembler handler request to insert/reorder/update the supplied list of items
   * to the owner item in the specified slot.
   *
   * @param ownerContentId the owner item content id, may not be <code>null
   * </code> or empty.
   * @param slotId the slot of owner, may not be <code>null</code> or empty.
  ...
  private PSActiveAssemblerHandlerRequest buildRequest(
    ...
    if (nodeList == null || !nodeList.hasNext())
      throw new IllegalArgumentException("nodeList may not be null or empty.");
    ...
    try {
      summariesElements =
          m_actionManager.getComponentProxy().load(ms_summaryComponentType, locators);
      if (summariesElements.length == 0 || summariesElements[0] == null) {
        throw new Exception(
            "Could not find component summary for " + "content id " + ownerContentId);
      }
    ...
    PSComponentSummary summary = (PSComponentSummary) summaries.iterator().next();
    ...
    while (nodeList.hasNext()) {
      ...
      props.add(
          new PSProperty(
              IPSHtmlParameters.SYS_SLOTID, PSProperty.TYPE_STRING, slotId, false, null));

      props.add(
          new PSProperty(
              IPSHtmlParameters.SYS_VARIANTID,
              PSProperty.TYPE_STRING,
              src.getVariantId(),
              false,
              null));
      ...
      props.add(
          new PSProperty(
              ...
              PSProperty.TYPE_STRING,
              src.getProp(IPSHtmlParameters.SYS_SITEID),
              false,
              null));
      ...
      props.add(
          new PSProperty(
              ...
              PSProperty.TYPE_STRING,
              src.getProp(IPSHtmlParameters.SYS_FOLDERID),
              false,
              null));

      dependents.add(new PSDependent(src.getRelationshipId(), loc, props));
    }
    return new PSActiveAssemblerHandlerRequest(owner, dependents, Integer.MAX_VALUE);
  }

  /**
   * Process the active assembler handler by passing the XML format of the message to the proper
   * app.
   *
   * @param req the active assembler handler request created by the specific request and used to
   *     effect changes on the nodes supplied within the active assembler request
   * @param action the action to perform
   */
  private void processActiveAssemblerRequest(PSActiveAssemblerHandlerRequest req, String action) {
    if (!(req.getIndex() > 0)) return; // Do nothing if the index is NOT greater than 0
  ...
  private static final String ACTIVE_ASSEMBLY_APP = "../sys_psxActiveAssembly/app";
