...
/**
 * The default node renderer to use with the navigation tree to display different icons based on
 * each node type and expansion state. Sets the background selection color, font and text foreground
 * color based on the display options set by user.
 */
public class PSNavTreeNodeRenderer extends DefaultTreeCellRenderer {
  ...
  /**
   * Overridden to implement the behavior defined in the class description. Gets the icons, colors
   * and font from <code>UIManager</code> user defaults. Displays the node with different background
   * color if the node rendering is undergoing a drag as a drop-target. It shows different colors
   * depending on whether the node accepts the drop (selection color) or not (light gray). See
   * super's description for parameters and more information.
   */
  public Component getTreeCellRendererComponent(
    ...
    accessibleContext = new PSTreeNodeAccContext((PSNavigationTree) tree, targetNode);
