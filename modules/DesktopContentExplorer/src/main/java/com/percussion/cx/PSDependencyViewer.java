...
/**
 * The splitplane that displays ancestors and descends of a content item for a specific
 * relationship.
 */
public class PSDependencyViewer extends JSplitPane
  ...
  /**
   * Constructs the dependency viewer with a horizontal-split to show its dependencies.
   *
   * @param actionManager the action manager to use for context sensitive menu and load children
   *     dynamically, may not be <code>null</code>
   * @param itemNode the node that represents the item whose dependencies are viewed with the
   *     properties defining the item and default relationship, may not be <code>null</code>
  ...
  public PSDependencyViewer(PSActionManager actionManager, PSNode itemNode) {
    ...
    if (itemNode == null) throw new IllegalArgumentException("itemNode may not be null.");

    init(actionManager, itemNode);
  }

  /**
   * Initializes this view with 'Descendants' tree on the top panel and the 'Ancestors' tree in the
   * bottom panel.
   *
   * @param mgr the manager to use to with the trees, assumed not <code>null
   * </code>
   * @param itemNode The node of the item for which descendants and ancestors will be viewed,
   *     assumed not <code>null</code>.
   */
  private void init(PSActionManager mgr, PSNode itemNode) {
    m_actManager = mgr;
    setOrientation(JSplitPane.VERTICAL_SPLIT);
    String dlNameStr = m_actManager.getApplet().getResourceString(getClass(), "@Descendents");
    JLabel descLabel = new JLabel(dlNameStr, SwingConstants.LEFT);
    char dlmnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "@Descendents", 'D');
    String dldesc =
        m_actManager.getApplet().getResourceString(getClass(), "@DescendentDescription");
    ...
    m_descNode.setProperty(
        PSItemRelationshipsManager.PROP_RS_LOOKUP_TYPE,
    ...
    String ancNameStr = m_actManager.getApplet().getResourceString(getClass(), "@Ancestors");
    JLabel ancLabel = new JLabel(ancNameStr, SwingConstants.LEFT);
    char almnemonic = PSContentExplorerApplet.getResourceMnemonic(getClass(), "@Ancestors", 'A');
    String ancdesc =
        m_actManager.getApplet().getResourceString(getClass().getName() + "@AncestorDescription");
    descLabel.setName(PSContentExplorerConstants.HEADING_COMP_NAME);
    if (almnemonic != 0) {
  ...
  public void viewDataChanged(Object data) {
    if (!(data instanceof PSProperties))
      throw new IllegalArgumentException("data must be an instance of PSProperties");

    PSProperties props = (PSProperties) data;

    m_descNode.setProperty(
        IPSConstants.PROPERTY_RELATIONSHIP, props.getProperty(IPSConstants.PROPERTY_RELATIONSHIP));
  ...
  public void actionExecuted(PSActionEvent event) {
    ...
    int nodeType = PSNavigationTree.getRefreshNodeType(event.getRefreshHint());
    if (nodeType != -1) {
  ...
  public void addSelectionListener(IPSSelectionListener selListener) {
    if (selListener == null) throw new IllegalArgumentException("selListener may not be null.");

    m_selListeners.add(selListener);
  }

  /**
   * Action method for selection change event in any tree. Constructs the <code>
  ...
  /**
   * The user object of the root node of the 'Descendants' tree, initialized in the <code>
   * init(PSActionManager, PSNode)</code> and never <code>null</code> after that. This node will be
   * set with properties for <code>
   * PROPERTY_RELATIONSHIP</code>('relationship') and <code>PROP_RS_LOOKUP_TYPE</code>('relationship
   * lookup type'). The lookup type for this node is <code>RS_LOOKUP_DESCENDANTS</code>. Modified
   * through calls to the method <code>viewDataChanged(Object)</code> as the 'relationship' property
   * changes.
   */
  private PSNode m_descNode;

  /**
   * The user object of the root node of the 'Ancestors' tree, initialized in the <code>
   * init(PSActionManager, PSNode)</code> and never <code>null</code> after that. This node will be
   * set with properties for <code>
   * PROPERTY_RELATIONSHIP</code>('relationship') and <code>PROP_RS_LOOKUP_TYPE</code>('relationship
   * lookup type'). The lookup type for this node is <code>RS_LOOKUP_ANCESTORS</code>. Modified
   * through calls to the method <code>viewDataChanged(Object)</code> as the 'relationship' property
   * changes.
   */
  ...
  /**
   * The tree that represents the descendants of an item for a specific relationship. Initialized in
   * the ctor and will be modified in <code>
   * viewDataChanged(Object)</code> as user changes the relationship.
   */
  private PSNavigationTree m_descTree;

  /**
   * The tree that represents the ancestors of an item for a specific relationship. Initialized in
   * the ctor and will be modified in <code>
   * viewDataChanged(Object)</code> as user changes the relationship.
   */
  ...
  /**
   * The action manager to use to refresh/load the children for a specific node, initialized in the
   * ctor and never <code>null</code> or modified after that.
   */
  private PSActionManager m_actManager;

  /**
   * The list of selection listeners that are interested in selection changes in this ancestor or
   * descendents tree, initialized to an empty list and listeners are being added through calls to
   * <code>
