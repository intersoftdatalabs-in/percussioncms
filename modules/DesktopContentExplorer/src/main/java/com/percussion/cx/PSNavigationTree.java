...
public class PSNavigationTree extends JTree implements DragGestureListener, DropTargetListener {
  ...
  /**
   * Constructs the tree with supplied node and sets the listeners and renderer on this tree. By
   * default, it tries to expand as defined by the supplied node tree.
   *
  ...
  public PSNavigationTree(PSNode root, String view, PSActionManager manager, boolean ignoreRoot) {
    ...
    if (!PSUiMode.isValidView(view)) throw new IllegalArgumentException("view is invalid");

    m_view = view;

    if (manager == null) throw new IllegalArgumentException("manager may not be null.");

    if (manager.getApplet() == null) throw new IllegalArgumentException("applet must not be null");
    ...
    if (m_actManager.viewSupportsCopyPaste()) {
      ...
      KeyStroke ksCTLCKeyRelease = KeyStroke.getKeyStroke(KeyEvent.VK_C, KeyEvent.CTRL_MASK, true);
      getInputMap().put(ksCTLCKeyRelease, "ctlcAction");
      AbstractAction ctlcAction =
          new AbstractAction() {
            private static final long serialVersionUID = 1L;
            ...
            public void actionPerformed(@SuppressWarnings("unused") ActionEvent e) {
              ...
              if (selNode != null && selParent != null) {
                ...
                m_actManager.getClipBoard().setClip(PSClipBoard.TYPE_COPY, sel);
              }
            }
          };
      ...
      KeyStroke ksCTLVKeyRelease = KeyStroke.getKeyStroke(KeyEvent.VK_V, KeyEvent.CTRL_MASK, true);
      getInputMap().put(ksCTLVKeyRelease, "ctlvAction");
      AbstractAction ctlvAction =
          new AbstractAction() {
            private static final long serialVersionUID = 1L;
            ...
            public void actionPerformed(@SuppressWarnings("unused") ActionEvent e) {
              ...
              if (selPath != null) {
                String pasteAction = PSMenuAction.PREFIX_COPY_PASTE + IPSConstants.ACTION_PASTE;
                ...
                Rectangle rect = PSNavigationTree.this.getPathBounds(selPath);
  ...
  /**
   * Override to set the row height based on the metrics of the font to be set.
   *
   * @see javax.swing.JComponent#setFont(java.awt.Font)
   */
  @Override
  public void setFont(Font font) {
    LineMetrics lm = font.getLineMetrics("", new FontRenderContext(null, true, true));
  ...
  public void dragGestureRecognized(DragGestureEvent dge) {
    ...
    TreePath path = getClosestPathForLocation((int) loc.getX(), (int) loc.getY());
    if (path != null) {
      node = getNode(path);
      if (path.getParentPath() != null) parent = getNode(path.getParentPath());
    }
    ...
    if (node != null && parent != null && m_actManager.canCopyOrMove(node)) {
      try {
        ...
        PSSelection sel =
            new PSSelection(new PSUiMode(m_view, ms_mode), parent, PSIteratorUtils.iterator(node));
        clipBoard.setClip(PSClipBoard.TYPE_DRAG, sel);
        dge.startDrag(
            DragSource.DefaultMoveNoDrop, new PSDnDTransferable(), new PSDragSourceAdapter());
  ...
  /**
   * Provides drag over feedback for this action. If the tree node at the drag location accepts
   * Paste action, it accepts drag , otherwise it rejects the drag. The node is refreshed to present
   * visual changes.
   *
   * @param dtde the drag event on drop target, assumed not <code>null</code> as is called by Swing
   *     model.
   */
  ...
  // implements nothing
  public void dropActionChanged(@SuppressWarnings("unused") DropTargetDragEvent dtde) {}

  /**
   * Clears the drag-feed back if the current drag is not successful. Please note this method is
   * called from the {@link #drop(DropTargetDropEvent)} method to clear the state after a drop.
   *
   * @param dte the event, assumed not <code>null</code> as is called by Swing model.
   */
  ...
  /**
   * Implemented to accept the drop and show context-sensitive pop-up menu for pasting. See the
   * interface for more description of the method.
   */
  public void drop(DropTargetDropEvent dtde) {
    ...
    try {
      ...
      String pasteAction = PSMenuAction.PREFIX_DROP_PASTE + IPSConstants.ACTION_PASTE;

      PSMenuAction action =
          new PSMenuAction(
              pasteAction, pasteAction, PSMenuAction.TYPE_MENU, "", PSMenuAction.HANDLER_CLIENT, 0);
   ...
   * @return the node, may be <code>null</code> if the supplied node type is either <code>
   *     NODE_SELECTED</code> or <code>NODE_SEL_PARENT</code> and there is no selection or not found
   *     if it is being looked up.
   */
  public PSNode getNode(int nodeType) {
    PSNode node = null;
    PSTreeNode treeNode = getTreeNode(nodeType);
    if (treeNode != null) node = (PSNode) treeNode.getUserObject();
   ...
   *
   * @param path path of the node, assumed to be in the format '//node1/node2/...', may not be
   *     <code>null</code> or empty.
   * @param byName supply <code>true</code> if the path names should match the name of the nodes,
   *     <code>false</code> to match by display name (label).
  ...
  /**
   * Determines if the navigation tree is in the process of dirtying nodes. Used to avoid loading
   * children on refresh events caused by the process of dirtying child nodes.
   *
  ...
  /**
   * Finds first child node of the same type as the supplied node, with a matching contentId.
   * Comparison for type is case-sensitive. Does not recurse into node's child nodes unless child
   * node is a category.
   *
   * @param checkNode Checks this node and its children, assumed not <code>null</code>.
  ...
  /**
   * Marks any matching nodes as dirty. Adds a clone of each supplied node as a child to all nodes
   * matching one of the specified types if no matching node is found. Match is determined using
   * only the contentId of the nodes. Clone is not added if parent has not yet had children loaded.
   *
  ...
  /**
   * Recursive worker method for {@link #dirtyChildNodes(PSNode, Collection)}, see that method for
   * more info.
   *
   * @param parent The node to which a clone of the child node is recursively added, assumed not
   *     <code>null</code>.
  ...
  private void dirtyChildNode(PSNode parent, PSNode node, Collection<String> types) {
    ...
    if (types.contains(parent.getType()) && parent.getChildren() != null) {
      ...
      if (match != null) match.setIsDirty(true);
      else {
    ...
    if (addNode != null) parent.addChild(addNode);
  }

  /**
   * Gets the node that is matching the supplied nodes from already loaded nodes of the tree. Uses
   * <code>equals()</code> method for matching.
   *
  ...
  /**
   * Builds the fully qualified path of the parent of the supplied <code>node</code>. The root is
   * the parent of itself.
   *
   * @param node Never <code>null</code>.
   * @return A path of the form "//a/b/c". Never <code>null</code>, will be empty if the supplied
   *     node is not found in this tree.
   * @see #getPath(PSNode)
   */
  ...
  /**
   * Builds the fully qualified path of the supplied <code>node</code>, including the node itself.
   *
   * @param node Never <code>null</code>.
   * @return A path of the form "//a/b/c". Never <code>null</code>, will be empty if the supplied
   *     node is not found in this tree.
  ...
  private PSTreeNode getTreeNode(PSTreeNode checkNode, PSNode nodeToMatch) {
    ...
    if (node.equals(nodeToMatch)) matchNode = checkNode;
   ...
   * @return the node, may be <code>null</code> if the supplied node type is either <code>
   *     NODE_SELECTED</code> or <code>NODE_SEL_PARENT</code> and there is no selection
   */
  private PSTreeNode getTreeNode(int nodeType) {
    ...
    if (nodeType == NODE_SELECTED) {
      ...
      treeNode = getDynamicTreeNodeInPath(getSelectionPath().getParentPath());
  ...
  /**
   * Walks up into the tree path until it finds a node with dynamic children. Dynamic children are
   * the result of the execution of children url of the node.
   *
   * @param path treepath of tree node to search the dynamic parent, if <code>null</code> the result
  ...
  public PSNode getRootNode() {
    ...
    return (PSNode) ((DefaultMutableTreeNode) model.getRoot()).getUserObject();
  }

  /**
   * Gets user object that is represented by the selected node of this tree.
   *
   * @return the selected node, may be <code>null</code> if there is no selection.
   */
  public PSNode getSelectedNode() {
    PSNode selNode = null;

    TreePath path = getSelectionPath();
    if (path != null) selNode = getNode(path);

    return selNode;
  }

  /**
   * Gets user object that is represented by the parent of the selected node of this tree.
   *
   * @return the parent node of the selected node, may be <code>null</code> if there is no
   *     selection.
   */
  public PSNode getSelectedParentNode() {
    ...
    if (path != null && path.getParentPath() != null) selNode = getNode(path.getParentPath());
  ...
  PSNode getNode(TreePath path) {
    return (PSNode) ((DefaultMutableTreeNode) path.getLastPathComponent()).getUserObject();
  }

  /**
   * Gets the path of selection in the tree as a <code>String</code> in the format '//Node/Child
   * Node/...'.
   *
   ...
   * Node/...'.
   *
   * @param byName if true returns the path using the name of the node, otherwise returns the
   *     display name of the selected path
   * @return the path, may be <code>null</code> if there is no selection.
   */
  ...
  /**
   * Gets the path of the specified Node in the tree as a <code>String</code> in the format
   * '//Node/Child Node/...'.
   *
  ...
  /**
   * Converts a tree path to a <code>String</code> in the format '//Node/Child Node/Child Node/...'.
   *
   * @param treePath the path to the tree object to be converted, must not be <code>null</code>
   * @param byName if true, covert using the name of the node, otherwise use the display name.
   * @return the path
   */
  ...
  /**
   * Notification method that this node has changed. In general it will be called by the container
   * when the any node data gets changed. Refreshes the tree node of supplied type with the new node
   * and updates visually also.
   *
   * @param newNode the user object to set with for a node of supplied node type, may not be <code>
   *     null</code>
   * @param nodeType the type of node to refresh with new data, must be one of the NODE_xxx values.
   * @throws IllegalArgumentException if any parameter is invalid.
   */
  public void nodeChanged(PSNode newNode, int nodeType) {
    if (newNode == null) throw new IllegalArgumentException("newNode may not be null.");

    PSTreeNode treeNode = getTreeNode(nodeType);

    if (treeNode != null) refresh(newNode, treeNode);
  }

  /**
   * Refreshes the tree node that represents the supplied old node if it exists in the tree with new
   * node and updates visual representation also.
   *
  ...
  public void nodeChanged(PSNode newNode, PSNode oldNode) {
    ...
    if (oldNode == null) throw new IllegalArgumentException("oldNode may not be null.");
  ...
  /**
   * Refreshes the tree node represented by the supplied tree path new node and updates visual
   * representation also.
   *
   * @param treePath the path of the tree node to update, may not be <code>null</code>.
   * @param newNode the new node to set with, may not be <code>null</code>
   * @throws IllegalArgumentException if any parameter is invalid.
   */
  public void nodeChanged(TreePath treePath, PSNode newNode) {
    ...
    if (treePath == null) throw new IllegalArgumentException("treePath may not be null.");
  ...
  /**
   * Replaces the user object represented by the tree node with the supplied new node and refreshes
   * the tree view.
   *
   * @param newNode the new node to update, assumed not <code>null</code>
   * @param treeNode the tree node whose visual representation need to be updated, assumed not
   *     <code>null</code> and a node in the tree.
   */
  ...
  /**
   * Gets the tree node identified by the supplied path. The search for nodes will be from children
   * of root. Loads dynamically the child nodes as part of the search, so this method is expensive
   * to call. Checks against the label of the node case sensitively for a match.
   *
   ...
   * @return the matching node, may be <code>null</code> if the match is not found.
   */
    ...
    if (st.hasMoreTokens()) {
      if (!m_ignoreRoot) {
        ...
        if (rootNode.getLabel().equals(st.nextToken())) {
          if (!st.hasMoreTokens()) matchNode = checkNode;
        }
      }
      ...
      while (st.hasMoreTokens()) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (checkName.equals(token)) {
            if (st.hasMoreTokens()) checkNode = treeNode;
            else matchNode = treeNode;
            break;
          }
        }
      }
  ...
  /**
   * Expands the supplied node and its children if the data node of this if they are specified to
   * expand recursively.
   *
  ...
  public class PSTreeNode extends JTree.DynamicUtilTreeNode implements Accessible {
    ...
    /**
     * Constructs the tree node with supplied node and allows children by default. Has an empty
     * children list initially and gets loaded with child nodes when <code>loadChildren()</code> is
     * called.
     *
    ...
    /**
     * Loads child nodes of this node and appends them if they are not loaded yet. Each child node
     * of this node gets expanded by default if the child defined as to expand by default.
     */
    @Override
    ...
    /**
     * Gets the leaf child nodes (children, grand children ... that are leafs (nodes that don't want
     * to expand by default (expanded = false)) ) of this node. If this node is leaf, this will be
    ...
    /**
     * Recursive worker method for {@link #getLeafChildren()} to get the leaf nodes of the supplied
     * node.
     *
     ...
     *
     * @param flag supply <code>false</code> to make the node as not loaded with children, otherwise
     *     <code>true</code>.
     */
    ...
    /**
     * Updates the drop target flag. Should be called with <code>true</code> when drag/drop is over
     * this node, otherwise <code>false</code>. Updates this node's visual representation.
     *
    ...
    /**
     * Checks whether this node represents a drop target or not for current drag and drop operation.
     *
     * @return <code>true</code> if this node representing a drop target, otherwise <code>false
     *     </code>
     */
     ...
     *
     * @param userObject May be <code>null</code> to clear the object. Must implement the {@link
     *     PSNavigationTree.IPSTreeNodeAssociation}. When called, the {@link
     *     PSNavigationTree.IPSTreeNodeAssociation
     *     #setAssociatedTreeNode(PSNavigationTree.PSTreeNode) setAssociatedTreeNode} is called with
     *     a ref to this node and if this node already has a user object, the same method is called
     *     on it with <code>null</code>.
     */
    @Override
    public void setUserObject(Object userObject) {
      if (null != userObject && !(userObject instanceof PSNavigationTree.IPSTreeNodeAssociation)) {
        throw new IllegalArgumentException(
            "userObject must implement " + "PSNavigationTree.IPSTreeNodeAssociation");
      }
      PSNavigationTree.IPSTreeNodeAssociation uobj =
          (PSNavigationTree.IPSTreeNodeAssociation) super.getUserObject();
      if (null != uobj) uobj.setAssociatedTreeNode(null);
      if (null != userObject) {
        ((PSNavigationTree.IPSTreeNodeAssociation) userObject).setAssociatedTreeNode(this);
      }
     ...
     * drop target, initialized to <code>false</code> and modified through calls to <code>
     * setAsDropTarget(boolean)</code>.
     */
    private boolean m_isDropTarget = false;
  }

  /**
   * Any object that wants to be set as the user object of a {@link PSNavigationTree.PSTreeNode}
  ...
  public interface IPSTreeNodeAssociation {
    /**
     * Called by the framework whenever the object implementing this interface is set as the user
     * object of a node.
     *
     * @param node The node that is 'taking ownership' of the object. May be <code>null</code> to
     *     clear the association.
     */
    public void setAssociatedTreeNode(PSNavigationTree.PSTreeNode node);

    /**
     * Accessor for the tree node that this object is currently associated with.
     *
     * @return The last node supplied in the {@link
     *     #setAssociatedTreeNode(PSNavigationTree.PSTreeNode) setAssociatedTreeNode}. <code>null
     *     </code> if that method has never been called.
     */
  ...
  /**
   * The class that handles pop-up mouse clicks to show pop-up menu for the path near the
   * mouse-click.
   */
  private class MouseActionListener extends PSMouseAdapter {

    /**
     * Handles right-click mouse event to show the context-menu if the clicked location represents a
     * node in the tree.
     *
     * @param event event the mouse click event, assumed not <code>null</code> this is called by
     *     Swing model.
     */
    @Override
    public void mouseWasClicked(MouseEvent event) {
      ...
      if (PSCxUtil.isMouseMenuGesture(event, m_applet)) {
        ...
        if (path != null) {
          ...
          if (path.getParentPath() != null) parentNode = getNode(path.getParentPath());

          PSUiMode mode = new PSUiMode(m_view, ms_mode);
          PSSelection sel = new PSSelection(mode, parentNode, PSIteratorUtils.iterator(selNode));
          PSMenuAction defAction = m_actManager.findDefaultAction(sel);
          if (defAction != null) m_actManager.executeAction(defAction, sel);
        }
      }
    }
  }
  ...
  /**
   * Displays context menu for the supplied action and the selection represented by the last
   * component of the supplied path.
   *
   * @param action the action that controls the pop-up menu to show, if <code>null</code> the pop-up
   *     menu depends only on the last node of the path.
   * @param path the path whose last component represents a node for which the pop-up menu will be
   *     applied, assumed not <code>null</code>
   * @param loc the location where the pop-up menu should show, assumed not <code>null</code>
   * @return the popup menu shown, may be <code>null</code> if the popup menu is not available for
   *     the supplied action and/or for the current selection
   */
  JPopupMenu displayPopupMenu(PSMenuAction action, TreePath path, Point loc) {
    ...
    if (parentPath != null) {
      parent =
          (PSNode)
              ((DefaultMutableTreeNode) path.getParentPath().getLastPathComponent())
                  .getUserObject();
    }

    PSNavigationalSelection selection =
        new PSNavigationalSelection(
            new PSUiMode(m_view, ms_mode), parent, selNodes, getSelectedPath());

    PSMenuAction popupMenuAction;
    if (action == null) {
      popupMenuAction = m_actManager.getContextMenu(selection);
    } else { // if drag & drop, filter the action against both the target node
    ...
    if (popupMenuAction != null) {
      PSContentExplorerMenu menu =
          new PSContentExplorerMenu(popupMenuAction, new PSMenuSource(selection), m_actManager);
  ...
  /**
   * Given the path in the standard syntax (//path1/path2/...) it walks through the tree, locates
   * the required node and sets selection. Uses the label for match.
   *
   * @param strPath must be a valid path as specified above, Nothing happens if the path does not
   *     exist in the tree, may not be <code>null</code> or empty.
   * @param byName the flag that specifies whether the path represents internal names of the nodes
   *     or the display names (labels) of the nodes. Supply <code>true</code> for former and
   *     vice-versa.
  ...
  public void setSelectionPath(String strPath, boolean byName) {
    if (strPath == null || strPath.trim().length() == 0)
      throw new IllegalArgumentException("strPath may not be null or empty.");

    PSTreeNode treeNode = getTreeNode(strPath, byName);
    if (treeNode != null) setSelectionPath(new TreePath(treeNode.getPath()));
  }

  /**
   * Finds the matching nodes of the supplied nodes in the tree and calls <code>refresh(PSTreeNode)
   * </code> to refresh the UI. The match is by checking the <code>equals()</code> on the user
   * object.
   *
  ...
  public void refreshNodes(Iterator<? extends PSNode> nodes) {
    ...
    while (nodes.hasNext()) {
      ...
      if (treeNode != null) {
        ...
        if (firstNode == null) firstNode = refreshNode;
      }
    }

    // Focus always goes to the first node refreshed (Should it be the last one???)
    if (firstNode != null) selectNode(firstNode);
  }
  ...
  public void selectNode(PSNode node) {
    ...
    if (treeNode != null) {
      ...
      if (isPathSelected(path)) clearSelection();
  ...
  /**
   * Find the tree node for the given parent node and add the given child.
   *
   * @param parent the parent node, must never be <code>null</code> and must exist
  ...
  /**
   * Determines the type of node to be refreshed for the specified hint.
   *
   * @param refreshHint not <code>null</code>.
   * @return the node type which should be refreshed or -1 if the refresh hint is not one of {@link
   *     PSActionEvent#REFRESH_NAV_ROOT}, {@link PSActionEvent#REFRESH_NAV_SELECTED}, or {@link
   *     PSActionEvent#REFRESH_NAV_SEL_PARENT}.
   */
  public static int getRefreshNodeType(String refreshHint) {
    ...
    if (refreshHint.equalsIgnoreCase(PSActionEvent.REFRESH_NAV_ROOT)) nodeType = NODE_ROOT;
   ...
   * </code> values, initialized in the ctor and never <code>null</code>, empty or modified after
   * that.
   */
  String m_view;

  /**
   * The flag to indicate whether the root should be shown or not and should be considered as part
   * of the path or not. <code>true</code> indicates not to show the root and vice-versa.
   * Initialized in the ctor and never modified after that.
   */
  private boolean m_ignoreRoot;

  /**
   * The mode (Navigation) that is represented by this tree. See {@link PSUiMode#TYPE_MODE_NAV} for
   * more info.
   */
  ...
  /**
   * Flag to indicate if tree is in the process of dirtying child nodes. See {@link
   * #isDirtyingNodes()} for more information. Modified during calls to {@link
   * #dirtyChildNodes(PSNode, Collection)}.
   */
  private boolean m_isDirtying = false;

  /**
   * This holds the current drag selection on drag enter. On drag exit the selection is restored and
   * this is nulled out. May contain <code>null</code> if there is no current select.
   */
  private TreePath[] m_saveDragSelection = null;

  /**
   * Set in {@link #dragEnter(DropTargetDragEvent)} and reset in {@link #dragExit(DropTargetEvent)}.
   * Is used by the selection listener when deciding if the selection should cause any side effects.
   */
