...
/**
 * Utility class of static helper method for the Content Explorer applet. Can not be instantiated.
 */
public final class PSCxUtil {
  ...
  /**
   * Adjusts popup location to try to keep the popup within the specified bounds.
   *
    ...
    if ((bounds.getMaxY() - (popupDims.getHeight() + loc.getY())) < 0)
      y =
          (int)
              Math.max(
                  loc.getY()
                      - Math.max(popupDims.getHeight() - (bounds.getMaxY() - loc.getY()), 0.0)
                      - 40,
                  0.0);
    ...
    if ((bounds.getMaxX() - (popupDims.getWidth() + loc.getX())) < 0)
      x =
          (int)
              Math.max(
                  loc.getX()
                      - Math.max(popupDims.getWidth() - (bounds.getMaxX() - loc.getX()), 0.0),
                  0.0);
    loc.setLocation(x, y);
  ...
  /**
   * Will determine if this is a mouse menu gesture, for Windows this would be a right button click
   * and for Mac this would be a control-click. This is only used in a mouse event that captures a
  ...
  public static Color getWindowBkgColor(PSContentExplorerApplet applet) {
    ...
    return applet.isMacPlatform() ? Color.white : SystemColor.window;
  }

  /**
   * Indicates if the folder should be marked for publish, either because it is itself marked or one
   * of its ancestors is marked.
   *
  ...
  public static boolean shouldFolderBeMarked(
    ...
    while (treeNode != null) {
      if (!(firstIteration && skipSelfCheck)) {
        PSNode data = (PSNode) ((PSNavigationTree.PSTreeNode) treeNode).getUserObject();
        if (data.isAnyFolderType() && applet.getFlaggedFolderSet().contains(data.getContentId())) {
          shouldMark = true;
          break;
        }
      }
   ...
   */
  public static Map<String, String> getItemIcons(
      List<PSLocator> items, PSContentExplorerApplet applet) {
    if (applet == null) throw new IllegalArgumentException("applet must not be null");
    ...
    try {
      ...
      Document respDoc =
          applet.getApplet().getXMLDocument("../sys_cxSupport/getItemIcons.xml", params);
      NodeList nl = respDoc.getElementsByTagName("Item");
      for (int i = 0; i < nl.getLength(); i++) {
        Element elem = (Element) nl.item(i);
        itemIconMap.put(elem.getAttribute("cid"), elem.getAttribute("path"));
      }
    } catch (Exception e) {
