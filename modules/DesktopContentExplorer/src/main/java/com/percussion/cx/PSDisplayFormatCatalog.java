...
/**
 * This is a convenient class to get catalog of all display formats from the server keep locally and
 * and provide an easy lookup acces to the desired display formats.
 */
public class PSDisplayFormatCatalog {
  /**
   * Initializes all available display formats and organizes them by the category of display fotmat
   * by making a request to the server and caches them in this object.
   *
  ...
  /**
   * Loads all available display formats and organizes them by the category of display fotmat by
   * making a request to the server and caches them in this object.
   *
   * @throws PSContentExplorerException if an error happens in the process of loading display
   *     formats.
   * @throws IllegalStateException if not even a single display format is found for each category.
  ...
  private void loadDisplayFormats() throws PSContentExplorerException {
    try {
      Element[] coll = m_proxy.load(PSDisplayFormat.getComponentType(PSDisplayFormat.class), null);
      ...
      for (int i = 0; i < coll.length; i++) {
        ...
        if (!format.isAllowedForCommunity(communityId)) continue;
        m_displayFormats.add(format);

        if (format.isValidForRelatedContent()) m_rcDisplayFormats.add(format);

        if (format.isValidForFolder()) m_folderDisplayFormats.add(format);

        if (format.isValidForViewsAndSearches()) m_viewAndSearchDisplayFormats.add(format);
      }
    ...
    if (m_folderDisplayFormats.isEmpty()) {
      ...
      ErrorDialogs.showErrorMessage(
          null,
          m_applet.getResourceString(getClass(), "No display formats found for folders"),
    ...
    if (m_rcDisplayFormats.isEmpty()) {
      ...
      ErrorDialogs.showErrorMessage(
          null,
          m_applet.getResourceString(getClass(), "No display formats found for related content"),
          m_applet.getResourceString(getClass(), "Error"));
    }
    ...
    if (m_viewAndSearchDisplayFormats.isEmpty()) {
      ...
      ErrorDialogs.showErrorMessage(
          null,
          m_applet.getResourceString(getClass(), "No display formats found for views and searches"),
          m_applet.getResourceString(getClass(), "Error"));
    }
  }
  ...
  /**
   * Returns an iterator of all cataloged display format objects applicable to Folders cotents.
   *
  ...
  /**
   * Returns an iterator of all cataloged display format objects applicable to Related Content
   * Search results.
   *
   * @return the list of displayformats, never <code>null</code> or empty.
   * @see PSDisplayFormat
  ...
  /**
   * Returns an iterator of all cataloged display format objects applicable to Views and Search
   * results.
   *
   * @return the list of displayformats, never <code>null</code> or empty.
   * @see PSDisplayFormat
  ...
  /**
   * Returns the display format object for a given display format id. If the supplied display format
   * is not in the cache, then it reloads the display formats from the server and gets the matching
   * display format.
   *
   * @param displayformatid the identifier of the display format, may not be <code>null</code> or
   *     empty.
   * @return display format object for the id given, may be <code>null</code> if not found even on
   *     the server.
   * @see PSDisplayFormat
   * @throws PSContentExplorerException if an error happens reloading the display formats.
   */
  public PSDisplayFormat getDisplayFormatById(String displayformatid)
      throws PSContentExplorerException {
    if (displayformatid == null || displayformatid.trim().length() == 0)
      throw new IllegalArgumentException("displayformatid may not be null or empty.");
  ...
  /**
   * Gets the display format identified by the supplied id from the list of display formats cached.
   *
   * @param displayformatid the identifier of the display format, assumed not <code>null</code> or
   *     empty.
   ...
   *
   * @param format display format for which the id need to be extracted, must not be <code>null
   *     </code>.
  ...
  /**
   * The object that holds the information about the user session, initialized in the ctor and never
