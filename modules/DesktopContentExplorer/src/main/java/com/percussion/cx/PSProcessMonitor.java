...
/**
 * The class to use as a monitor for a process to keep track of the process status. This class is
 * thread safe to set or get the status.
 */
public class PSProcessMonitor {
  ...
  public void setStatusDialog(PSContentExplorerStatusDialog dlg) {
    if (dlg == null) throw new IllegalArgumentException("dlg may not be null.");
  ...
  /**
   * Sets the status of the monitor. If the status is completed, informs the status dialog that the
   * process is completed.
   *
   * @param currentStatus the current status of the process, must be one of the <code>STATUS_xxx
   *     </code> values.
  ...
  public synchronized void setStatus(int currentStatus) {
    ...
    synchronized (m_syncObject) {
      ...
      if (m_status == STATUS_COMPLETE && m_dlg != null) m_dlg.processCompleted();
    }
  }

  /**
   * Displays the supplied error message to the user. Passes the message to its container dialog if
  ...
  public void showError(Component parent, String message) {
    if (m_dlg != null) m_dlg.showError(message);
      ...
      int selection =
          PSContentExplorerStatusDialog.displayErrorDialog(parent, title, label, message);
      if (selection == JOptionPane.NO_OPTION) setStatus(PSProcessMonitor.STATUS_STOP);
      else setStatus(PSProcessMonitor.STATUS_RUN);
    }
  }
  ...
  /**
   * The status of the process, initialized to <code>STATUS_INIT</code> and gets modified through
   * calls to <code>setStatus(int)</code>
   */
  private int m_status = STATUS_INIT;

  /**
   * The current node count being processed, initialized to <code>0</code> and gets updated through
   * calls to <code>updateStatus(int, PSNode)</code>
   */
  private int m_current = 0;

  /**
   * The total number of nodes being processed, initialized in the ctor and never modified after
   * that.
   */
  private int m_total = 0;

  /**
   * The current node being processed, initialized to <code>null</code> and gets updated through
   * calls to <code>updateStatus(int, PSNode)</code>
   */
