...
/**
 * This is a utility class to demonstrate how to use the Rhythmyx webservice API. In particular,
 * this class demonstrates how to maintain sessions for both Rhythmyx and the JBoss container across
 * all service instances. The Rhythmyx session is communicated through the SOAP header, but the
 * JBoss session (JSESSION) is communicated through the HTTP Cookie header. Note: The current
 * implementation of the Axis 1.3 client can only maintain one JSESSION per service stub instance.
 */
public class PSWsUtils {
   ...
   */
  public static PSItem createItem(ContentSOAPStub binding, String contentType) throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static long saveItem(ContentSOAPStub binding, PSItem item) throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static PSItem loadItem(ContentSOAPStub binding, long id) throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static void checkinItem(ContentSOAPStub binding, long id) throws Exception {
   ...
   *     prepareForEdit action by calling {@link #releaseFromEdit(ContentSOAPStub, PSItemStatus)}
   * @throws Exception if an error occurs.
   */
  public static PSItemStatus prepareForEdit(ContentSOAPStub binding, long id) throws Exception {
    return binding.prepareForEdit(new long[] {id})[0];
  }

  /**
   * Releases the specified Content Item from Edit; reverse action of {@link
   * #prepareForEdit(ContentSOAPStub, long)}.
   *
   * @param binding the proxy of the content service; assumed not to be <code>null</code>.
   ...
   * @throws Exception if an error occurs.
   */
  public static void releaseFromEdit(ContentSOAPStub binding, PSItemStatus status)
      throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static void transitionItem(SystemSOAPStub binding, long id, String trigger)
      throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static PSItemSummary[] findFolderChildren(ContentSOAPStub binding, String folderPath)
      throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static void addFolderChildren(ContentSOAPStub binding, String folderPath, long[] childIds)
      throws Exception {
   ...
   * @throws Exception if an error occurs.
   */
  public static PSFolder[] addFolderTree(ContentSOAPStub binding, String folderPath)
      throws Exception {
   ...
   *     login attempt is successful, this object will be modified to maintain the Rhythmyx session.
   ...
   * @throws PSNotAuthenticatedFault if authentication of the passed credentials fails.
   */
  public static String login(
      SecuritySOAPStub binding, String user, String password, String community, String locale)
   ...
   */
  public static void logout(SecuritySOAPStub binding, String rxSession) throws Exception {
   ...
   * #login(SecuritySOAPStub, String, String, String, String)}.
   *
   * @param rxSession the Rhythmyx session; assumed to be a valid Rhythmyx session from {@link
   *     #login(SecuritySOAPStub, String, String, String, String)}, not <code>null</code> or empty.
   ...
   */
  public static ContentSOAPStub getContentService(String rxSession) throws ServiceException {
   ...
   *     overridden by {@link #setConnectionInfo(String, String, int)}.
   * @see #setConnectionInfo(String, String, int)
   * @throws ServiceException if the method fails to create the system service instance.
   */
  public static SystemSOAPStub getSystemService(String rxSession) throws ServiceException {
   ...
   * #login(SecuritySOAPStub, String, String, String, String)} with the returned object.
   *
   ...
   */
  public static SecuritySOAPStub getSecurityService() throws ServiceException {
   ...
   *
   * @param binding the proxy for which to set MIME format; assumed not to be <code>null</code>.
  ...
  private static void setAttachmentFormat(org.apache.axis.client.Stub binding)
    ...
    if (call == null) call = binding._createCall();
    call.setProperty(
   ...
   *
   * @param srcAddress the source address; assumed not to be <code>null</code> or empty.
   * @return The same address as the specified source address but with the the connection
   *     information (protocol, host and port) of this class replacing the original ones.
   */
  private static String getNewAddress(String srcAddress) {
    try {
      URL url = new URL(srcAddress);
      return ms_protocol + "://" + ms_host + ":" + ms_port + url.getPath();
  ...
  /**
   * Sets the Rhythmyx session as header to the specified proxy. Clears all existing headers before
   * the new session header is set.
   *
   * @param binding the proxy to which to add the Rhythmyx session as header; assumed not to be
   *     <code>null</code>.
   * @param rxSession The Rhythmyx session; assumed to be valid a Rhythmyx session from {@link
   *     #login(SecuritySOAPStub, String, String, String, String)}, not <code>null</code> or empty.
   */
