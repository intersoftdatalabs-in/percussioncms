...
public class PSCopySiteNamePage extends PSWizardPanel {
  ...
  public void validatePanel() throws PSWizardValidationError {
    ...
    if (folderName.length() == 0) {
      ...
      errors.append(
          PSI18NTranslationKeyValues.getInstance()
              .getTranslationValue(getClass().getName() + "@Site Folder name is required."));
    }
    ...
    if (m_input.getFolderNames().contains(folderName)) {
      ...
      errors.append(
          PSI18NTranslationKeyValues.getInstance()
              .getTranslationValue(getClass().getName() + "@Site Folder name is invalid."));
    }
    ...
    if (m_input.hasSitesToCopy()) {
      ...
      if (siteName.length() == 0) {
        ...
        errors.append(
            PSI18NTranslationKeyValues.getInstance()
                .getTranslationValue(getClass().getName() + "@Site name is required."));
      }
      ...
      if (m_input.getSiteNames().contains(siteName)) {
        ...
        errors.append(
            PSI18NTranslationKeyValues.getInstance()
                .getTranslationValue(getClass().getName() + "@Site name is invalid."));
      }
    }
  ...
  public String getSummary() {
    ...
    if (copiedSite != null) {
      ...
      buffer.append(
          PSI18NTranslationKeyValues.getInstance()
                  .getTranslationValue(getClass().getName() + "@Copied Site Definition:")
              + copiedSite.getName());
   ...
   * Get the panel data.
   *
   * @return the panel data as <code>PSCopySiteNamePage.OutputData</code> object, never <code>null
   *     </code>.
   */
  public Object getData() {
    ...
    return new OutputData(copiedSiteName, newSiteName, m_folderName.getText());
  }

  /**
   * Set and initialize the the panel data.
   *
   * @param data the data as <code>PSCopySiteNamePage.InputData</code> object, not <code>null</code>
   *     .
   */
  public void setData(Object data) {
    if (!(data instanceof InputData))
      throw new IllegalArgumentException("data must be an insanceof PSCopySiteNamePage.InputData");
  ...
  private JPanel createMainPanel() {
    ...
    panel.addPropertyRow(
        PSI18NTranslationKeyValues.getInstance()
            .getTranslationValue(getClass().getName() + "@New Folder Name"),
        new JComponent[] {m_folderName});

    panel.addPropertyRow(
        PSI18NTranslationKeyValues.getInstance()
            .getTranslationValue(getClass().getName() + "@Site Definition to copy"),
        new JComponent[] {m_siteSelector});

    panel.addPropertyRow(
        PSI18NTranslationKeyValues.getInstance()
            .getTranslationValue(getClass().getName() + "@New Site Name"),
        new JComponent[] {m_siteName});
  ...
  public static class InputData {
     ...
     *
     * @param source the source node that will be copied, not <code>null</code>.
     * @param sites all defined sites in the system as collection of <code>PSSite</code> objects,
    ...
    public InputData(PSNode source, Collection<PSSite> sites, Collection<String> folderNames) {
      ...
      if (folderNames == null) throw new IllegalArgumentException("folderNames cannot be null");
    ...
    public Collection<PSSite> getSitesToCopy() {
      ...
      for (PSSite site : m_sites) {
        ...
        if (test.toLowerCase().endsWith(folderRoot)) siteNames.add(site);
      }
    ...
    /**
     * The source node that will be copied, initialized during construction, never <code>null</code>
  ...
  public class OutputData {
     ...
     *
     * @param copiedSiteName the name of the site definition to be copied, may be <code>null</code>
     *     or empty.
     * @param newSiteName the new site name, may be <code>null</code> or empty.
     * @param folderName the new site folder name, assumed not <code>null</code> or empty.
     */
    private OutputData(String copiedSiteName, String newSiteName, String folderName) {
     ...
     *
     * @return the name of the copied site definition, may be <code>null</code> or empty.
     */
    ...
    /**
     * The name of the copied site sitedefinition, initialized in constructor, may be<code>null
     * </code> or empty, is never changed after that.
     */
    private String m_copiedSiteName = null;

    /**
     * The new site name, initialized in constructor, may be <code>null</code> or empty, is never
     * changed after that.
     */
    private String m_newSiteName = null;

    /**
     * The new site folder name, initialized in constructor, never <code>null</code>, empty or
     * changed after that.
     */
  ...
  /** A text field to request the new site folder name from the user, never <code>null</code>. */
