...
```bash
# Linux / macOS
./mvnw -Dai.integrity.skip=true   <goal> ...

# Windows (wrapper enforces JDK 21)
mvnw.cmd -Dai.integrity.skip=true   <goal> ...

# Maven-conventional alias (also accepted by every goal)
./mvnw -Dskip.ai.integrity=true <goal> ...
```
...
../../mvnw -Dai.integrity.skip=true -Dtest=PSSiteDataServiceTest test

# Windows
cd projects\sitemanage
..\mvnw.cmd -Dai.integrity.skip=true -Dtest=PSSiteDataServiceTest test
```
...
./mvnw -pl modules/perc-legacy -Dtest=PSAesTest test
```
...
./mvnw -rf :perc-distribution-tree verify
```
...
./mvnw -pl projects/sitemanage spotless:apply
./mvnw validate   # re-seal the whole repo
```
...
./mvnw validate   # seals the new file
git add target/ai-integrity.sha256   # (note: target/ is gitignored — see §7)
./mvnw -pl modules/ai-shared-develop test
```
...
- The wrapper to use depends on the OS:
  - Windows: `mvnw.cmd` (enforces JDK 21).
  - Linux / macOS: `./mvnw`.
...
./mvnw validate && git status   # target/ regenerated; nothing to commit
